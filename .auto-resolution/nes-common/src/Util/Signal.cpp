...
{
    const auto result = signal(signalNo, handler);
    if (result == SIG_ERR)
    {
        fmt::println(stderr, "Error registering signal handler for signal {}", signalName(signalNo));
    }
    if (result != nullptr)
    {
        fmt::println(stderr, "Signal handler for {} was overriden", signalName(signalNo));
    }
}
}
