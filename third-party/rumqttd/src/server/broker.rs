...
impl Broker {
    pub fn new(config: Config) -> Broker {
        ...
        match config.cluster.clone() {
            Some(_cluster_config) => {
                ...
                let (router_tx, router_thread) = router.spawn();
                let (shutdown_tx, shutdown_rx) = watch::channel(false);
                // cluster.spawn();
                Broker { config, router_tx, router_thread: Some(router_thread), shutdown_tx, shutdown_rx }
            }
            None => {
                let (router_tx, router_thread) = router.spawn();
                let (shutdown_tx, shutdown_rx) = watch::channel(false);
    ...
    /// Takes the router thread JoinHandle out of the broker.
    ///
    /// Call this before moving the broker to a start thread so the caller
    /// can join the router thread during shutdown. The router thread exits
    /// when all `router_tx` senders are dropped.
    pub fn take_router_thread(&mut self) -> Option<thread::JoinHandle<()>> {
        self.router_thread.take()
    }

    /// Returns a lightweight handle that can create new internal links
    /// even after the broker has been moved to a start thread.
    ...
    pub fn start(&mut self) -> Result<(), Error> {
        ...
        // Shared slot for the first server error. Server threads store their
        // error here; start() checks it after joining all threads.
            ...
            for (_, config) in v4_config.clone() {
                ...
                let handle = server_thread.spawn(move || {
                    ...
                    runtime.block_on(async {
                        if let Err(e) = server.start(LinkType::Remote, &mut shutdown).await {
        ...
        if let Some(ws_config) = &self.config.ws {
            for (_, config) in ws_config.clone() {
                ...
                let handle = server_thread.spawn(move || {
                    ...
                    runtime.block_on(async {
                        if let Err(e) = server.start(LinkType::Websocket, &mut shutdown).await {
        ...
        // In the ideal case, where server doesn't crash, join() will never resolve
        // we still try to join threads so that we don't return from function
        // unless everything crashes or shutdown is signaled.
        ...
        // If any server thread recorded an error, propagate it.
        if let Some(err_msg) = first_error.lock().unwrap().take() {
            return Err(Error::Io(io::Error::new(io::ErrorKind::Other, err_msg)));
        }
...
/// Lightweight, cloneable handle for creating internal links and
/// shutting down the broker.
///
/// Obtain via [`Broker::handle()`] before moving the broker to a start
/// thread.  The handle stays valid as long as the router is running.
#[derive(Clone)]
...
impl BrokerHandle {
    /// Create a new internal link, identical to [`Broker::link()`].
    pub fn link(&self, client_id: &str) -> Result<(LinkTx, LinkRx), local::LinkError> {
        let (link_tx, link_rx, _ack) =
            LinkBuilder::new(client_id, self.router_tx.clone()).build()?;
        Ok((link_tx, link_rx))
    ...
    /// Signal all server threads to stop accepting new connections.
    ///
    /// The TCP listeners will exit their accept loops. Once all server
    /// threads exit, `Broker::start()` returns on the broker thread.
    /// The router thread exits when all `router_tx` senders are dropped.
...
impl<P: Protocol + Clone + Send + 'static> Server<P> {
    ...
    async fn start(&mut self, link_type: LinkType, shutdown: &mut watch::Receiver<bool>) -> Result<(), Error> {
        ...
        loop {
            // Await new network connection, or shutdown signal.
            let (stream, addr) = tokio::select! {
                ...
                _ = shutdown.changed() => {
                    info!(config = self.config.name, "Server shutting down");
                    return Ok(());
                }
            };
