...
impl LinkTx {
    ...
    /// Send raw device data (async — does not block the Tokio runtime)
    pub async fn send(&mut self, data: Packet) -> Result<usize, LinkError> {
        self.async_push(data).await
    }

    /// Async counterpart to push(). Uses send_async() to avoid blocking the
    /// Tokio runtime when the router channel is full.
    async fn async_push(&mut self, data: Packet) -> Result<usize, LinkError> {
        let len = {
            let mut buffer = self.recv_buffer.lock();
            buffer.push_back(data);
            buffer.len()
    ...
    /// Async version of publish — does not block the Tokio runtime.
    pub async fn async_publish<S, V>(&mut self, topic: S, payload: V) -> Result<usize, LinkError>
    where
        S: Into<Bytes>,
        V: Into<Bytes>,
    {
        ...
        self.async_push(Packet::Publish(publish, None)).await
    }

    /// Async version of subscribe — does not block the Tokio runtime.
    pub async fn async_subscribe<S: Into<String>>(&mut self, filter: S) -> Result<usize, LinkError> {
        let filters = vec![Filter {
            path: filter.into(),
            qos: QoS::AtMostOnce,
            nolocal: false,
        ...
        self.async_push(Packet::Subscribe(subscribe, None)).await
    }
