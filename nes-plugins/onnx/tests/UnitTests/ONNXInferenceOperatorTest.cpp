...
class ONNXInferenceOperatorTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ONNXInferenceOperatorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down ONNXInferenceOperatorTest test class.");
  }
};

static std::string_view from_text_value_type(
    const NES::Nautilus::Value<Text>& value) {
  auto textValue = reinterpret_cast<TextValue*>(
      value.getValue().getReference().value->getValue().value);
  return {textValue->str(), textValue->length()};
}

static std::vector<float> decode_floats_from_base64(
    const std::string_view& base64_encoded) {
  ...
  for (size_t i = 0; i < (decoded.size() / sizeof(float)); ++i) {
    result.push_back(
        *reinterpret_cast<float*>(decoded.data() + (i * sizeof(float))));
  }
  return result;
}
...
std::string read_file(const std::string& file_name) {
  ...
  NES_ASSERT(t.is_open(), "Could not open file: " +
                              std::string(TEST_DATA_DIRECTORY) + file_name);
...
TEST_F(ONNXInferenceOperatorTest, testInferModelForONNXRuntimeYOLOV8) {
  ...
  auto inferModelOperator =
      std::make_shared<ONNXInferenceOperator>(0, inputFields, outputFields);

  auto collector = std::make_shared<CollectOperator>();
  inferModelOperator->setChild(collector);

  auto handler = std::make_shared<ONNXInferenceOperatorHandler>(
      std::string(TEST_DATA_DIRECTORY) + "yolov8n-cls.onnx");

  auto pipelineContext = MockedPipelineExecutionContext({handler});

  auto buffer_manager = std::make_shared<BufferManager>();
  // needed for Text Allocation
  auto worker_context = std::make_shared<WorkerContext>(INITIAL<WorkerThreadId>,
                                                        buffer_manager, 10);

  auto ctx = ExecutionContext(Value<MemRef>((int8_t*)worker_context.get()),
  ...
  auto firstRecord = Record(
      {{"data",
        Value<Text>(read_file("base64_encoded_cropped_cat.txt").c_str())}});

  // evaluated using python onnxruntime
  auto expected_outputs = decode_floats_from_base64(
      read_file("base64_encoded_cropped_cat_output.txt"));
  ...
  auto outputs = decode_floats_from_base64(
      from_text_value_type(collector->records[0].read(data_out).as<Text>()));
...
TEST_F(ONNXInferenceOperatorTest, testInferModelForONNXRuntimeBase64Encoding) {
  ...
  auto firstRecord =
      Record({{"data", Value<Text>("MzOjQAAAYEAzM7M/zcxMPg==")}});
  auto secondRecord =
      Record({{"data", Value<Text>("zcycQAAAQEAzM7M/zcxMPg==")}});
  auto thirdRecord =
      Record({{"data", Value<Text>("ZmaWQM3MTEBmZqY/zcxMPg==")}});
  auto fourthRecord =
      Record({{"data", Value<Text>("MzOTQGZmRkAAAMA/zcxMPg==")}});
  ...
  outputs = decode_floats_from_base64(
      from_text_value_type(collector->records[1].read(data_out).as<Text>()));
  ...
  outputs = decode_floats_from_base64(
      from_text_value_type(collector->records[2].read(data_out).as<Text>()));
  ...
  outputs = decode_floats_from_base64(
      from_text_value_type(collector->records[3].read(data_out).as<Text>()));
...
TEST_F(ONNXInferenceOperatorTest, testInferModelForONNXRuntime) {
  ...
  auto ctx = ExecutionContext(Value<MemRef>(nullptr),
                              Value<MemRef>((int8_t*)&pipelineContext));
  ...
  EXPECT_NEAR(collector->records[0].read(iris0).as<Float>()->getValue(),
              expectedOutput[0].iris0, delta);
  EXPECT_NEAR(collector->records[0].read(iris1).as<Float>()->getValue(),
              expectedOutput[0].iris1, delta);
  EXPECT_NEAR(collector->records[0].read(iris2).as<Float>()->getValue(),
              expectedOutput[0].iris2, delta);
  ...
  EXPECT_NEAR(collector->records[1].read(iris0).as<Float>()->getValue(),
              expectedOutput[1].iris0, delta);
  EXPECT_NEAR(collector->records[1].read(iris1).as<Float>()->getValue(),
              expectedOutput[1].iris1, delta);
  EXPECT_NEAR(collector->records[1].read(iris2).as<Float>()->getValue(),
              expectedOutput[1].iris2, delta);
  ...
  EXPECT_NEAR(collector->records[2].read(iris0).as<Float>()->getValue(),
              expectedOutput[2].iris0, delta);
  EXPECT_NEAR(collector->records[2].read(iris1).as<Float>()->getValue(),
              expectedOutput[2].iris1, delta);
  EXPECT_NEAR(collector->records[2].read(iris2).as<Float>()->getValue(),
              expectedOutput[2].iris2, delta);
  ...
  EXPECT_NEAR(collector->records[3].read(iris0).as<Float>()->getValue(),
              expectedOutput[3].iris0, delta);
  EXPECT_NEAR(collector->records[3].read(iris1).as<Float>()->getValue(),
              expectedOutput[3].iris1, delta);
  EXPECT_NEAR(collector->records[3].read(iris2).as<Float>()->getValue(),
              expectedOutput[3].iris2, delta);
