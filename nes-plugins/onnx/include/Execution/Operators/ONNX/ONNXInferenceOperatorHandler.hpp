...
using ONNXInferenceOperatorHandlerPtr =
    std::shared_ptr<ONNXInferenceOperatorHandler>;

/**
 * @brief OperatorHandler for the ModelInferenceOperator using the ONNX Runtime
 * @details This class is responsible for loading and managing the model (which
 * includes the ONNX Runtime Session) and loading the input and output tensors
 * into the model
 */
...
public:
  ...
  void reconfigure(Runtime::ReconfigurationMessage &task,
                   Runtime::WorkerContext &context) override;

  void
  postReconfigurationCallback(Runtime::ReconfigurationMessage &task) override;
  ...
  /**
   * @brief copies the output of the inference model into a base64 encoded
   * string
   ...
   */
  template <typename T> void appendToByteArray(const T &data);
