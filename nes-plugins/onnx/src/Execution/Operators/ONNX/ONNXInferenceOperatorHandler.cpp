...
ONNXInferenceOperatorHandlerPtr ONNXInferenceOperatorHandler::create(
    std::string model) {
  return std::make_shared<ONNXInferenceOperatorHandler>(std::move(model));
}

ONNXInferenceOperatorHandler::ONNXInferenceOperatorHandler(std::string model)
...
void ONNXInferenceOperatorHandler::start(
    Runtime::Execution::PipelineExecutionContextPtr, uint32_t) {}

void ONNXInferenceOperatorHandler::stop(
    Runtime::QueryTerminationType,
    Runtime::Execution::PipelineExecutionContextPtr) {}

void ONNXInferenceOperatorHandler::reconfigure(Runtime::ReconfigurationMessage&,
                                               Runtime::WorkerContext&) {}

void ONNXInferenceOperatorHandler::postReconfigurationCallback(
    Runtime::ReconfigurationMessage&) {}

const std::string& ONNXInferenceOperatorHandler::getModel() const {
  return model;
}

void ONNXInferenceOperatorHandler::appendBase64EncodedData(
    const std::string_view& base64_encoded_data) {
...
std::string ONNXInferenceOperatorHandler::getBase64EncodedData() const {
  auto data =
      std::string_view(ONNX_HANDLER_CAST<const char*>(output_buffer.data()),
                       output_buffer.size());
  return base64_encode(data);
}

void ONNXInferenceOperatorHandler::infer() {
  NES_DEBUG("Buffer Size: {}  ModelInputSize: {}", input_buffer.size(),
            getModelInputSizeInBytes())
  NES_ASSERT(input_buffer.size() == getModelInputSizeInBytes(),
             "Input buffer size does not match model input size");

  inferInternal(absl::Span<int8_t>(input_buffer),
                absl::Span<int8_t>(output_buffer));
  input_buffer.clear();
}

void ONNXInferenceOperatorHandler::initializeModel(
...
size_t ONNXInferenceOperatorHandler::getModelOutputSizeInBytes() {
  ...
  return std::accumulate(this->output_shape.begin(), this->output_shape.end(),
                         1, std::multiplies<>()) *
         sizeof(float);
}
...
size_t ONNXInferenceOperatorHandler::getModelInputSizeInBytes() {
  ...
  return std::accumulate(this->input_shape.begin(), this->input_shape.end(), 1,
                         std::multiplies<>()) *
         sizeof(float);
}

void ONNXInferenceOperatorHandler::inferInternal(absl::Span<int8_t> input,
                                                 absl::Span<int8_t> output) {
  NES_ASSERT(session, "Session not initialized");
  NES_ASSERT(input.size() % sizeof(float) == 0,
             "Input size must be a multiple of sizeof(float)");
  NES_ASSERT(output.size() % sizeof(float) == 0,
             "Output size must be a multiple of sizeof(float)");
  ...
  auto input_tensor =
      Ort::Value::CreateTensor<float>(memory_info, input_data, input_data_size,
                                      input_shape.data(), input_shape.size());
  auto output_tensor = Ort::Value::CreateTensor<float>(
      memory_info, output_data, output_data_size, output_shape.data(),
      output_shape.size());
...
void ONNXInferenceOperatorHandler::loadModel(const std::string& pathToModel) {
  ...
  this->session = std::make_unique<Ort::Session>(
      Ort::Env(), pathToModel.c_str(), Ort::SessionOptions{nullptr});
  ...
  for (size_t i = 0; i < session->GetInputCount(); ++i) {
    ...
    if (std::any_of(shape.begin(), shape.end(),
      ...
      NES_ERROR("Model: {}, Input: {} Shape: {}", pathToModel, i,
                fmt::join(shape, ","));
  ...
  for (size_t i = 0; i < session->GetOutputCount(); ++i) {
    ...
    if (std::any_of(shape.begin(), shape.end(),
                    [](int64_t dim) { return dim == -1; })) {
      NES_ERROR("Model: {}, Output: {} Shape: {}", pathToModel, i,
                fmt::join(shape, ","));
      NES_ASSERT(false, "Dynamic shape not supported!");
    }
  ...
  NES_DEBUG("Producer name: {}",
            metaData.GetProducerNameAllocated(allocator).get());
  NES_DEBUG("Graph name: {}", metaData.GetGraphNameAllocated(allocator).get());
  NES_DEBUG("Description: {}",
            metaData.GetDescriptionAllocated(allocator).get());
  NES_DEBUG("Domain: {}", metaData.GetDomainAllocated(allocator).get());
  NES_DEBUG("Version: {}", metaData.GetVersion());

  NES_ASSERT(session->GetOutputCount() == 1 && session->GetInputCount() == 1,
             "Only single input/output models are supported");
