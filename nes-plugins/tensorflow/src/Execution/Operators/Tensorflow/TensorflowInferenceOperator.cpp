...
void addValueToModel(int index, T value, void *inferModelHandler) {
  auto handler =
      static_cast<TensorflowInferenceOperatorHandler *>(inferModelHandler);
...
void TensorflowInferenceOperator::execute(ExecutionContext &ctx,
  ...
  for (uint32_t i = 0; i < inputFieldNames.size(); i++) {
    ...
    if (value->isType<Boolean>()) {
      FunctionCall("addValueToModel", addValueToModel<bool>, Value<UInt32>(i),
                   value.as<Boolean>(), inferModelHandler);
    } else if (value->isType<Float>()) {
      FunctionCall("addValueToModel", addValueToModel<float>, Value<UInt32>(i),
                   value.as<Float>(), inferModelHandler);
    } else if (value->isType<Double>()) {
      FunctionCall("addValueToModel", addValueToModel<double>, Value<UInt32>(i),
                   value.as<Double>(), inferModelHandler);
    } else if (value->isType<UInt64>()) {
      FunctionCall("addValueToModel", addValueToModel<uint64_t>,
                   Value<UInt32>(i), value.as<UInt64>(), inferModelHandler);
    } else if (value->isType<UInt32>()) {
      FunctionCall("addValueToModel", addValueToModel<uint32_t>,
                   Value<UInt32>(i), value.as<UInt32>(), inferModelHandler);
    } else if (value->isType<UInt16>()) {
      FunctionCall("addValueToModel", addValueToModel<uint16_t>,
                   Value<UInt32>(i), value.as<UInt16>(), inferModelHandler);
    } else if (value->isType<UInt8>()) {
      FunctionCall("addValueToModel", addValueToModel<uint8_t>,
                   Value<UInt32>(i), value.as<UInt8>(), inferModelHandler);
    } else if (value->isType<Int64>()) {
      FunctionCall("addValueToModel", addValueToModel<int64_t>,
                   Value<UInt32>(i), value.as<Int64>(), inferModelHandler);
    } else if (value->isType<Int32>()) {
      FunctionCall("addValueToModel", addValueToModel<int32_t>,
                   Value<UInt32>(i), value.as<Int32>(), inferModelHandler);
    } else if (value->isType<Int16>()) {
      FunctionCall("addValueToModel", addValueToModel<int16_t>,
                   Value<UInt32>(i), value.as<Int16>(), inferModelHandler);
    } else if (value->isType<Int8>()) {
      FunctionCall("addValueToModel", addValueToModel<int8_t>, Value<UInt32>(i),
                   value.as<Int8>(), inferModelHandler);
    } else {
      NES_ERROR("Can not handle inputs other than of type int, bool, float, "
                "and double");
    }
  }
