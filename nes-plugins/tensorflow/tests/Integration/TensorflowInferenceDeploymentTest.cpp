...
class TensorflowInferenceDeploymentTest
    : public Testing::BaseIntegrationTest,
      public testing::WithParamInterface<
          std::tuple<std::string, SchemaPtr, std::string, std::string>> {
 public:
  ...
  // The test data is a four-tuple which contains the name of the test, the
  // input schema, the CSV file containing the operator input, and the expected
  // model output.
 ...
 *
 * Disabled because it is not clear what this test tests. There is no code path
 *that supports different data types in the input values. The results are also
 *non-deterministic.
 **/
TEST_F(TensorflowInferenceDeploymentTest,
       testSimpleMLModelDeploymentMixedTypes) {
  ...
  // We set the predictions data type to FLOAT32 since the trained
  // iris_95acc.tflite model defines tensors of data type float32 as output
  // tensors.
  auto query =
      Query::from("irisData")
          .inferModel(
              std::filesystem::path(TEST_DATA_DIRECTORY) / "iris_95acc.tflite",
  ...
  auto tmpBuffers = TestUtils::createExpectedBufferFromCSVString(
      expectedOutputString, outputSchema, testHarness.getBufferManager(),
      false);
  ...
  auto actualBuffers =
      testHarness.runQuery(expectedTuples, "TopDown").getOutput();

  // Comparing equality
  NES_INFO("Actualbuffer:\n{}",
           NES::Util::printTupleBufferAsCSV(actualBuffers[0].getBuffer(),
                                            outputSchema));
  NES_INFO("ExpectedBuffers:\n{}",
           NES::Util::printTupleBufferAsCSV(expectedBuffers[0].getBuffer(),
                                            outputSchema));
...
TEST_P(TensorflowInferenceDeploymentTest, testSimpleMLModelDeployment) {
  ...
  for (auto i = 0_u64; i < actualTuples; ++i) {
    EXPECT_NEAR(expectedBuffers[0][i]["irisData$iris0"].read<float>(),
                actualBuffers[0][i]["irisData$iris0"].read<float>(), delta);
    EXPECT_NEAR(expectedBuffers[0][i]["irisData$iris1"].read<float>(),
                actualBuffers[0][i]["irisData$iris1"].read<float>(), delta);
    EXPECT_NEAR(expectedBuffers[0][i]["irisData$iris2"].read<float>(),
                actualBuffers[0][i]["irisData$iris2"].read<float>(), delta);
  }
}

INSTANTIATE_TEST_CASE_P(
    TestInputs, TensorflowInferenceDeploymentTest,
    ::testing::Values(
        TensorflowInferenceDeploymentTest::createBooleanTestData(),
        TensorflowInferenceDeploymentTest::createFloatTestData(),
        TensorflowInferenceDeploymentTest::createIntTestData()),
    [](const testing::TestParamInfo<
        TensorflowInferenceDeploymentTest::ParamType>& info) {
