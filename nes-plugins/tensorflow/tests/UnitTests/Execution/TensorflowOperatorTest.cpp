...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TensorflowOperatorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down TensorflowOperatorTest test class.");
  }
};
...
TEST_F(TensorflowOperatorTest, testInferModelForBoolInput) {
  ...
  auto inferModelOperator = std::make_shared<TensorflowInferenceOperator>(
      0, inputFields, outputFields);

  auto collector = std::make_shared<CollectOperator>();
  inferModelOperator->setChild(collector);

  auto handler = std::make_shared<TensorflowInferenceOperatorHandler>(
  ...
  auto firstRecord = Record({{"f1", Value<>(false)},
                             {"f2", Value<>(true)},
                             {"f3", Value<>(false)},
                             {"f4", Value<>(true)}});
  auto secondRecord = Record({{"f1", Value<>(false)},
                              {"f2", Value<>(true)},
                              {"f3", Value<>(false)},
                              {"f4", Value<>(true)}});
  ...
  // Assertion
  std::vector<Output> expectedOutput{{0.43428239, 0.31287873, 0.25283894},
...
TEST_F(TensorflowOperatorTest, testInferModelForIntInput) {
  ...
  auto firstRecord = Record({{"f1", Value<>(5)},
                             {"f2", Value<>(3)},
                             {"f3", Value<>(1)},
                             {"f4", Value<>(0)}});
  auto secondRecord = Record({{"f1", Value<>(4)},
                              {"f2", Value<>(3)},
                              {"f3", Value<>(1)},
                              {"f4", Value<>(0)}});
  inferModelOperator->execute(ctx, firstRecord);
