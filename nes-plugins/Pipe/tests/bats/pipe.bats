...
@test "pipe: generator feeds single pipe source" {
  ...
  wait_for_status "$TOPO" "$PRODUCER_ID" 5 "Running"
  wait_for_status "$TOPO" "$CONSUMER_ID" 5 "Running"
...
@test "pipe: generator feeds 3 pipe sources" {
  ...
  for qid in "${query_ids[@]}"; do
    wait_for_status "$TOPO" "$qid" 5 "Running"
  done
...
@test "pipe: aggressive chunking with window aggregation" {
  ...
  for qid in "$CONSUMER_1" "$CONSUMER_2"; do
    wait_for_status "$TOPO" "$qid" 10 "Stopped" "Finished"
  done
...
@test "pipe: variable-sized data (VARSIZED) passes through pipe with child buffers" {
  setup_distributed
  TOPO="tests/good/pipe-varsized-topology.yaml"

  run DOCKER_NES_CLI -t "$TOPO" start \
    'SELECT id, payload FROM GEN INTO PIPE_SINK'
  [ "$status" -eq 0 ]
  PRODUCER=$output
  ...
  run DOCKER_NES_CLI -t "$TOPO" start \
    'SELECT id, payload FROM READER INTO FILE_SINK'
  [ "$status" -eq 0 ]
  CONSUMER=$output

  # Wait for generator to finish and EoS to propagate
  for qid in "$PRODUCER" "$CONSUMER"; do
    wait_for_status "$TOPO" "$qid" 5 "Stopped" "Finished"
  done

  sync_workdir

  # Verify output file exists and has data
  [ -f worker-node/varsized_output.csv ]
  local line_count
  line_count=$(wc -l < worker-node/varsized_output.csv)
  echo "# varsized output lines: $line_count" >&3
  [ "$line_count" -gt 1 ]  # at least header + 1 data row

  # Verify the payload column is non-empty for every row (not garbage/empty from missing child buffers)
  local empty_payloads
  empty_payloads=$(tail -n +2 worker-node/varsized_output.csv | awk -F',' '{if ($2 == "" || $2 == "NULL") print}' | wc -l)
  echo "# empty/null payloads: $empty_payloads" >&3
  [ "$empty_payloads" -eq 0 ]

  # Verify payload strings are valid base64-ish (only alphanumeric + /+=), not binary garbage
  local garbage_payloads
  garbage_payloads=$(tail -n +2 worker-node/varsized_output.csv | awk -F',' '{if ($2 !~ /^[A-Za-z0-9+/=]+$/) print}' | wc -l)
  echo "# garbage payloads: $garbage_payloads" >&3
  [ "$garbage_payloads" -eq 0 ]
}
