...
SourceValidationRegistryReturnType RegisterInProcessSourceValidation(SourceValidationRegistryArguments sourceConfig)
{
    return InProcessSource::validateAndFormat(std::move(sourceConfig.config));
}

///NOLINTNEXTLINE (performance-unnecessary-value-param)
SourceRegistryReturnType SourceGeneratedRegistrar::RegisterInProcessSource(SourceRegistryArguments sourceRegistryArguments)
{
    return std::make_unique<InProcessSource>(sourceRegistryArguments.sourceDescriptor);
}
