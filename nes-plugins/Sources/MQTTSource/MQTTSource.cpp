...
MQTTSource::MQTTSource(const SourceDescriptor& sourceDescriptor)
    ...
    , presenceTopic(sourceDescriptor.getFromConfig(ConfigParametersMQTTSource::PRESENCE_TOPIC))
{
}
...
void MQTTSource::open(std::shared_ptr<AbstractBufferProvider>)
{
    ...
    try
    {
        ...
        if (!presenceTopic.empty())
        {
            client->publish(presenceTopic, clientId, qos, /*retained=*/false)->wait();
        }
    }
