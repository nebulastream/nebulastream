...
void MQTTSource::writePayloadToBuffer(const std::string_view payload, TupleBuffer& tb, size_t& tbOffset)
{
    ...
    {
        std::memcpy(tb.getAvailableMemoryArea<>().subspan(tbOffset).data(), payload.data(), payload.size());
    ...
    std::memcpy(tb.getAvailableMemoryArea<>().subspan(tbOffset).data(), payload.data(), prefixSize);
