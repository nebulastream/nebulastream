...
::timeval toTimeval(std::chrono::microseconds duration)
{
    ...
    const auto useconds = duration - seconds;

    return {.tv_sec = static_cast<time_t>(seconds.count()), .tv_usec = static_cast<suseconds_t>(useconds.count())};
}
...
namespace NES
{
std::expected<void, std::string> Socket::setNonBlock()
{
    return addFlag(O_NONBLOCK);
}

std::expected<void, std::string> Socket::setBlock()
{
...
std::expected<Connection, std::string> Connection::from(Socket sock, AddressInfo addrinfo, std::chrono::microseconds connectionTimeout)
{
    ...
    {
        if (auto blockResult = sock.setBlock(); !blockResult)
        {
            return std::unexpected(fmt::format("Failed to set socket to blocking after connect: {}", blockResult.error()));
        }
...
Connection::receive(std::span<uint8_t> result, std::chrono::microseconds timeout)
{
    ...
    while (true)
    {
        ...
        if (resultSize == -1)
        {
            ...
            if (reconnectResult)
            {
                NES_INFO("TCPSource: Successfully reconnected after recv error");
                continue;
            }
...
Source::FillTupleBufferResult TCPSource::fillTupleBuffer(TupleBuffer& tupleBuffer, const std::stop_token&)
{
    ...
    {
        ++generatedBuffers;
        return FillTupleBufferResult::withBytes(receiveResult->size());
    }
    ...
    throw CannotProduceSourceBuffer(std::get<std::string>(receiveResult.error()));
}
