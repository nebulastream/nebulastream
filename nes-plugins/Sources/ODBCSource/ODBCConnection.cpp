...
/// Raw in-buffer size of a NebulaStream type. The ODBC source writes non-nullable
/// native tuples (it rejects SQL NULLs), so no null-handling space is reserved.
size_t rawTypeSize(const NES::DataType::Type type)
{
    return NES::DataType{type, NES::DataType::NULLABLE::NOT_NULLABLE}.getSizeInBytesWithoutNull();
}
...
TypeInfo getTypeInfo(const SQLSMALLINT sqlType, const SQLULEN sqlColLen)
{
    static std::unordered_map<SQLSMALLINT, NES::DataType::Type> typeMap
        = {{SQL_CHAR, NES::DataType::Type::CHAR},
           ...
           /// SQL_FLOAT is a driver-defined-precision float whose ODBC C type is SQL_C_DOUBLE (8 bytes); psqlodbc
           /// reports PostgreSQL `double precision` as SQL_FLOAT. SQL_REAL is the 4-byte single-precision type.
           {SQL_FLOAT, NES::DataType::Type::FLOAT64},
    ...
    if (const auto type = typeMap.find(sqlType); type != typeMap.end())
    {
        if (type->second == NES::DataType::Type::VARSIZED)
        {
            return TypeInfo{
                .sqlType = SQL_VARCHAR,
                .nesType = type->second,
                .nesTypeSize = rawTypeSize(NES::DataType::Type::VARSIZED),
        ...
        if (type->second == NES::DataType::Type::FLOAT32 and sqlColLen > maxDigitsIn32BitFloat)
        {
            return TypeInfo{
                ...
                .nesTypeSize = rawTypeSize(NES::DataType::Type::FLOAT64),
                .sqlColumnSize = sqlColLen};
        }
        if (type->second == NES::DataType::Type::INT64)
        {
            return TypeInfo{
                ...
                .nesTypeSize = rawTypeSize(NES::DataType::Type::INT64),
                .sqlColumnSize = sqlColLen};
        }
        /// Normalize every 8-byte float (SQL_DOUBLE and SQL_FLOAT) to SQL_DOUBLE so readDataIntoBuffer reads it
        /// through the SQL_C_DOUBLE path; reading an 8-byte column as a 4-byte float would corrupt the row layout.
        if (type->second == NES::DataType::Type::FLOAT64)
        {
            return TypeInfo{
                .sqlType = SQL_DOUBLE,
                .nesType = NES::DataType::Type::FLOAT64,
        ...
        return TypeInfo{.sqlType = sqlType, .nesType = type->second, .nesTypeSize = rawTypeSize(type->second), .sqlColumnSize = sqlColLen};
    }
...
void ODBCConnection::connect(
    ...
    const std::string_view idColumn)
{
    ...
    const size_t colIdx, const TypeInfo& typeInfo, SQLLEN& indicator, TupleBuffer& buffer, AbstractBufferProvider& bufferProvider) const
{
    ...
    if (auto varSizedBuffer = bufferProvider.getUnpooledBuffer(typeInfo.sqlColumnSize))
    {
        ...
        *reinterpret_cast<VariableSizedAccess*>(&buffer.getAvailableMemoryArea<char>()[buffer.getNumberOfTuples()])
        ...
        buffer.setNumberOfTuples(buffer.getNumberOfTuples() + rawTypeSize(DataType::Type::VARSIZED));
        return ret;
    }
...
std::vector<SQLCHAR> ODBCConnection::buildNewRowFetchString(const std::string_view userQuery, const uint64_t numRowsToFetch)
{
    ...
    /// Wrap the user query so each poll fetches only the latest `numRowsToFetch` rows, ordered by the key column.
    const std::string fetchQuery = fmt::format("SELECT {} ORDER BY {} DESC LIMIT {}", selectSplit.at(0), idColumn, numRowsToFetch);
    std::vector<SQLCHAR> queryBuffer(fetchQuery.begin(), fetchQuery.end());
    ...
    const std::string_view query, TupleBuffer& tupleBuffer, AbstractBufferProvider& bufferProvider, const size_t rowsPerBuffer)
{
    ...
    auto queryBuffer = buildNewRowFetchString(query, rowsToFetch);
