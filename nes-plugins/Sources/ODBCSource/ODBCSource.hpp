...
private:
    ...
    /// Key column the incremental-fetch query orders by (see ODBCConnection::buildNewRowFetchString).
    std::string idColumn;
    std::string query;
    size_t maxRetries;
...
struct ConfigParametersODBC
{
    ...
    static inline const DescriptorConfig::ConfigParameter<std::string> ID_COLUMN{
        "id_column",
        std::nullopt,
        [](const std::unordered_map<std::string, std::string>& config) { return DescriptorConfig::tryGet(ID_COLUMN, config); }};
