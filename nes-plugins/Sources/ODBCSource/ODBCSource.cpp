...
struct PreparedStatement
{
    ...
    void updateTimestampParam()
    {
        ...
        *timestampUpper = convertToTimestamp(std::chrono::system_clock::now());
    }
    ...
    static PreparedStatement bind(const Connection& connection, const Schema& schema, std::string query)
    {
        ...
        INVARIANT(
            ...
            parameterTypes.size());
        auto timestampLower = std::make_unique<SQL_TIMESTAMP_STRUCT>(convertToTimestamp(std::chrono::system_clock::now()));
        auto timestampUpper = std::make_unique<SQL_TIMESTAMP_STRUCT>(convertToTimestamp(std::chrono::system_clock::now()));
