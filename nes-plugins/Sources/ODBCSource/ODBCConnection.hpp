...
public:
    ...
    std::vector<SQLCHAR> buildNewRowFetchString(std::string_view userQuery, uint64_t numRowsToFetch);
...
private:
    ...
    /// Monotonically-increasing key column the incremental-fetch query orders by.
    std::string idColumn;
};
}
