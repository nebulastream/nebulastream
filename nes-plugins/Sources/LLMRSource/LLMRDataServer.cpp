...
namespace NES
{
LLMRDataServer::LLMRDataServer(std::vector<std::string> tuples)
...
LLMRDataServer::LLMRDataServer(std::filesystem::path filePath)
    : acceptor(io_context, tcp::endpoint(tcp::v4(), 0)), work_guard(boost::asio::make_work_guard(io_context))
{
...
void LLMRDataServer::run(const std::stop_token& stopToken)
{
...
void LLMRDataServer::startAccept(const std::stop_token& stopToken)
{
...
void LLMRDataServer::handleConnection(const std::shared_ptr<tcp::socket>& socket, const std::stop_token& stopToken)
{
    ...
    boost::asio::post(
        ...
        [this, socket, stopToken]()
        {
            ...
            catch (const std::exception&)
            {
                ...
                INVARIANT(socket->close(boostErrorCode), "Failed to close socket of LLMRDataServer: {}", boostErrorCode.message());
            }
        });
}

void LLMRDataServer::stop()
{
    boost::system::error_code boostErrorCode;
