...
LLMRSource::LLMRSource(const SourceDescriptor& sourceDescriptor)
    ...
    , bytesUsedForSocketBufferSizeTransfer(sourceDescriptor.getFromConfig(ConfigParametersLLMR::SOCKET_BUFFER_TRANSFER_SIZE))
    , flushIntervalInMs(sourceDescriptor.getFromConfig(ConfigParametersLLMR::FLUSH_INTERVAL_MS))
    , connectionTimeout(sourceDescriptor.getFromConfig(ConfigParametersLLMR::CONNECT_TIMEOUT))
{
    NES_TRACE("Init LLMRSource.");
}

std::ostream& LLMRSource::toString(std::ostream& str) const
{
    str << "\nLLMRSource(";
...
bool LLMRSource::tryToConnect(const addrinfo* result, const int flags)
{
    ...
    /// if the LLMRSource did not establish a connection, try with timeout
    if (connection < 0)
    {
...
void LLMRSource::open(std::shared_ptr<AbstractBufferProvider>)
{
    NES_TRACE("LLMRSource::open: Trying to create socket and connect.");
...
bool LLMRSource::fillBuffer(TupleBuffer& tupleBuffer, size_t& numReceivedBytes)
{
...
DescriptorConfig::Config LLMRSource::validateAndFormat(std::unordered_map<std::string, std::string> config)
{
    return DescriptorConfig::validateAndFormat<ConfigParametersLLMR>(std::move(config), name());
}

void LLMRSource::close()
{
