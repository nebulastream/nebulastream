...
VideoSource::fillTupleBuffer(NES::Memory::TupleBuffer& tupleBuffer, Memory::AbstractBufferProvider& bufferProvider, const std::stop_token&)
{
    try
    {
        while (true)
        {
            ...
            NES_DEBUG("Some Data: {}", *std::bit_cast<size_t*>(data.data()))
            std::ranges::copy(data, imageBuffer.getBuffer<std::byte>() + sizeof(uint32_t));
            NES_DEBUG("Sanity Check: {}", *imageBuffer.getBuffer<size_t>())
...
VideoSource::validateAndFormat(std::unordered_map<std::string, std::string> config)
{
...
{
    return VideoSource::validateAndFormat(sourceConfig.config);
}

std::unique_ptr<SourceRegistryReturnType> SourceGeneratedRegistrar::RegisterVideoSource(SourceRegistryArguments sourceRegistryArguments)
{
