...
TimeControlledSource::TimeControlledSource(const SourceDescriptor& sourceDescriptor)
    ...
    , emissionGranularity(sourceDescriptor.getFromConfig(ConfigParametersTimeControlled::EMISSION_GRANULARITY))
...
Source::FillTupleBufferResult TimeControlledSource::fillTupleBuffer(TupleBuffer& tupleBuffer, const std::stop_token& stopToken)
{
    const auto getScheduledTime = [&](const std::string& row) -> std::chrono::system_clock::time_point
    {
        ...
        switch (this->emissionGranularity)
        {
            case EmissionGranularity::MILLISECOND: {
                return openTime + std::chrono::milliseconds{*timestamp};
            }
            case EmissionGranularity::SECOND: {
                return openTime + std::chrono::seconds{*timestamp};
            }
            default: {
                NES_ERROR("Unknown case {} for emission granularity!", static_cast<int>(emissionGranularity));
                throw InvalidConfigParameter("Unknown case {} for emission granularity!", static_cast<int>(emissionGranularity));
            }
        }
    };

    const auto checkAndUpdateHighestSeenTime = [&](std::chrono::system_clock::time_point scheduledTime) -> void
    {
