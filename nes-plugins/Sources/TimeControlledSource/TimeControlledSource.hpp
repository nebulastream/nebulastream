...
public:
    ...
    enum class EmissionGranularity : uint8_t
    {
        MILLISECOND,
        SECOND,
    };
...
private:
    ...
    EmissionGranularity emissionGranularity;
...
struct ConfigParametersTimeControlled
{
    ...
    static inline const DescriptorConfig::ConfigParameter<EnumWrapper, TimeControlledSource::EmissionGranularity> EMISSION_GRANULARITY{
        "emission_granularity",
        std::optional(EnumWrapper(TimeControlledSource::EmissionGranularity::MILLISECOND)),
        [](const std::unordered_map<std::string, std::string>& config)
        {
            const auto optEmissionGranularity = DescriptorConfig::tryGet(EMISSION_GRANULARITY, config);
            if (not optEmissionGranularity.has_value()
                || not optEmissionGranularity.value().asEnum<TimeControlledSource::EmissionGranularity>().has_value())
            {
            ...
            switch (optEmissionGranularity.value().asEnum<TimeControlledSource::EmissionGranularity>().value())
            {
                case TimeControlledSource::EmissionGranularity::MILLISECOND: {
                    return std::optional(EnumWrapper(TimeControlledSource::EmissionGranularity::MILLISECOND));
                }
                case TimeControlledSource::EmissionGranularity::SECOND: {
                    return std::optional(EnumWrapper(TimeControlledSource::EmissionGranularity::SECOND));
                }
                default: {
                    NES_ERROR("Cannot validate emission_granularity: {}!", config.at("emission_granularity"));
                    throw InvalidConfigParameter("Cannot validate emission_granularity: {}!", config.at("emission_granularity"));
    ...
    static inline std::unordered_map<std::string, DescriptorConfig::ConfigParameterContainer> parameterMap
        = DescriptorConfig::createConfigParameterContainerMap(
            SourceDescriptor::parameterMap, FILEPATH, TIMESTAMP_COLUMN_IDX, EMISSION_GRANULARITY);
};
