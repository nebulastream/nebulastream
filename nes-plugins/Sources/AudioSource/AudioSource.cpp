...
size_t AudioSource::fillTupleBuffer(NES::Memory::TupleBuffer& tupleBuffer, const std::stop_token&)
{
    ...
    try
    {
        ...
        auto validBufferMemory = std::span(tupleBuffer.getBuffer<std::byte>(), numberOfBytesReceived);
        /// Fixup the received data by inserting timestamps.
        auto tuples = std::span{tupleBuffer.getBuffer<Tuple>(), numberOfTuplesReceived};
        auto values = validBufferMemory | std::views::chunk(sampleWidth);


        auto convertValue = [&](std::span<std::byte> audioBytes)
        {
            int16_t value = *std::bit_cast<int16_t*>(audioBytes.data());
        ...
        {
            auto [index, value] = p;
            return Tuple{static_cast<uint64_t>((current + (index * timePerSample)).count()), convertValue(value)};
        };

        std::ranges::copy_backward(std::views::enumerate(values) | std::views::transform(convert), tuples.end());
