...
namespace NES
    ...
    RestApiSource::RestApiSource(const SourceDescriptor& sd)
        ...
        size_t RestApiSource::fetchOnceAndWriteBody(TupleBuffer& tupleBuffer)
        {
            ...
            {
                NES_ERROR("RestApiSource response body too large ({} bytes) for TupleBuffer ({} bytes).",
                          body.size(),
                          mem.size());
                return 0;
            }
        ...
        std::string RestApiSource::httpGetBody(const ParsedUrl& u)
        {
            ...
            {
                NES_WARN("RestApiSource received non-200 response. Headers: {}", headers);
        ...
        {
            return RestApiSource::RestApiSource(std::move(sourceConfig.config));
        }

        SourceRegistryReturnType SourceGeneratedRegistrar::RegisterHTTPSource(SourceRegistryArguments args)
        {
