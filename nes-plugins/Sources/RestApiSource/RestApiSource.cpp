...
namespace NES
    {
        ...
        bool RestApiSource::shouldPoll() const
        {
            ...
            NES_INFO(
                "RestApiSource::shouldPoll | now={} | lastPoll={} | elapsed={} ms | interval={} ms",
                std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count(),
                std::chrono::duration_cast<std::chrono::milliseconds>(lastPoll.time_since_epoch()).count(),
        ...
        Source::FillTupleBufferResult RestApiSource::fillTupleBuffer(TupleBuffer& tupleBuffer,
                                                                     const std::stop_token& stopToken)
        {
            static std::atomic<uint64_t> callCounter{0};
            const auto call = ++callCounter;

            NES_INFO("[RestApiSource] fillTupleBuffer call={} stop={}", call, stopToken.stop_requested());

            try
            {
                if (stopToken.stop_requested())
                {
                    NES_INFO("[RestApiSource] stop requested -> EOS");
                ...
                {
                    const auto now = std::chrono::steady_clock::now();
                    const auto nextPoll = lastPoll + interval;
                    ...
                    auto remaining = nextPoll - now;
                    const auto sliceMax =
                        std::chrono::duration_cast<std::chrono::steady_clock::duration>(std::chrono::milliseconds{10});
                    const auto slice = (remaining < sliceMax) ? remaining : sliceMax;

                    //            NES_INFO("[RestApiSource] waiting {}ms until next poll",
                ...
                if (stopToken.stop_requested())
                {
                    NES_INFO("[RestApiSource] stop requested during wait -> EOS");
                    return FillTupleBufferResult::eos();
                }

                // Poll now
                lastPoll = std::chrono::steady_clock::now();
                NES_INFO("[RestApiSource] polling now (interval={}ms)", interval.count());

                const auto bytesWritten = fetchOnceAndWriteBody(tupleBuffer);
                NES_INFO("[RestApiSource] poll done bytesWritten={}", bytesWritten);
                ...
                // IMPORTANT: return actual bytes, not 0
                return FillTupleBufferResult::withBytes(bytesWritten);
            }
            catch (const std::exception& e)
            {
                NES_ERROR("RestApiSource failed in fillTupleBuffer: {}", e.what());
                throw;
            }
        }
        ...
        size_t RestApiSource::fetchOnceAndWriteBody(TupleBuffer& tupleBuffer)
        {
            ...
            NES_INFO("[RestApiSource] got body size={} first='{}'",
                     body.size(),
