...
#include <JSONRawScanIndexer.hpp>
...
{
    using JSONFormatter = NES::JSONRawScanIndexer;
...
void JSONRawScanIndexer::indexRawBuffer(
...
std::ostream& operator<<(std::ostream& os, const JSONRawScanIndexer&)
{
    return os << fmt::format(
               "JSONRawScanIndexer(tupleDelimiter: {}, fieldDelimiter: {})",
               JSONRawScanIndexer::TUPLE_DELIMITER,
               JSONRawScanIndexer::FIELD_DELIMITER);
}

RawScanIndexerRegistryReturnType RegisterJSONRawScanIndexer(RawScanIndexerRegistryArguments arguments)
{
    return arguments.createRawScanWithIndexer(JSONRawScanIndexer{});
}
