...
struct ConfigParametersGrpcSink
{
    ...
    /// NOLINTNEXTLINE(cert-err58-cpp)
    static inline const DescriptorConfig::ConfigParameter<uint64_t> PROBE_ID{
        "PROBE_ID",
        0,
        [](const std::unordered_map<std::string, std::string>& config) { return DescriptorConfig::tryGet(PROBE_ID, config); }};
    ...
    static inline std::unordered_map<std::string, DescriptorConfig::ConfigParameterContainer> parameterMap
        = DescriptorConfig::createConfigParameterContainerMap(SinkDescriptor::parameterMap, HOST, PORT, PROBE_ID);
};

/// Reports each incoming tuple to a StatisticReportService as a StatisticReport.
///
...
private:
    bool appendRow(std::string_view line, StatisticReportBatch& batch) const;

    static constexpr size_t MissingColumn = static_cast<size_t>(-1);

    std::string grpcHost;
    uint32_t grpcPort;
    uint64_t probeId;

    size_t statisticIdColumn{MissingColumn};
    size_t startTsColumn{MissingColumn};
    size_t endTsColumn{MissingColumn};
    size_t valueColumn{MissingColumn};
    size_t columnCount{0};

    std::unique_ptr<StatisticReportService::Stub> stub;
};
