...
///
/// Thread-safety: execute() is called concurrently by many worker threads on one
/// MQTTSink instance and deliberately takes no per-sink lock — paho's
/// async_client::publish is thread-safe, and the BackpressureHandler is internally
/// synchronized (folly::Synchronized). Contrast ODBCSink, which must serialize on a
/// mutex because its single ODBC statement handle is not concurrency-safe.
