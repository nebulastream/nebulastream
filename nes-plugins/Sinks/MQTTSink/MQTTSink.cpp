...
MQTTSink::MQTTSink(BackpressureController backpressureController, const SinkDescriptor& sinkDescriptor)
    ...
    , sendWhileDisconnected(sinkDescriptor.getFromConfig(ConfigParametersMQTTSink::SEND_WHILE_DISCONNECTED))
    , automaticReconnect(sinkDescriptor.getFromConfig(ConfigParametersMQTTSink::AUTOMATIC_RECONNECT))
    , backpressureHandler(
...
std::ostream& MQTTSink::toString(std::ostream& os) const
{
    ...
    os << "\n  sendWhileDisconnected: " << sendWhileDisconnected;
...
void MQTTSink::start(PipelineExecutionContext&)
{
    ...
    /// Survive a transient connection loss instead of failing the query (both knobs configurable):
    /// `send_while_disconnected` lets execute() keep buffering publishes while
    /// the link is down (capped at `maxOutstandingMessages`), and
    /// `automatic_reconnect` re-establishes the connection and flushes the
    /// buffer. Mirrors MQTTSource's reconnect handling. With buffering off, a
    /// publish issued while disconnected is rejected (CannotOpenSink) instead.
    const auto createOptions = mqtt::create_options_builder()
                                   .server_uri(serverURI)
                                   .client_id(clientId)
                                   .send_while_disconnected(sendWhileDisconnected)
                                   .max_buffered_messages(maxOutstandingMessages)
                                   .finalize();
    client = std::make_unique<mqtt::async_client>(createOptions);
    try
    {
        const auto connectOptions = mqtt::connect_options_builder().automatic_reconnect(automaticReconnect).clean_session(true).finalize();
        auto connectToken = client->connect(connectOptions);
        connectToken->wait();
    ...
    while (currentBuffer)
    {
        switch (tryPublish(*currentBuffer))
        {
            ...
            case PublishResult::Closed: {
                /// The broker connection is dead: drop this buffer (stashing it for a retry that can
                /// never succeed would only leave the BackpressureHandler non-empty) and fail the query.
