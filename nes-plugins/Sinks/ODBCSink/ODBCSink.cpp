...
struct Connection
{
    ...
    explicit Connection(const ConnectionInformation& info)
    {
        ...
        /// Turn off autocommit so each WRITE batch (one execute() call) commits once instead of
        /// once per row. The caller (ODBCSink::execute) ends the transaction per buffer.
        CHECK_SINK(
            SQLSetConnectAttr(connection.get(), SQL_ATTR_AUTOCOMMIT, reinterpret_cast<SQLPOINTER>(SQL_AUTOCOMMIT_OFF), 0),
            SQL_HANDLE_DBC,
            connection.get(),
            "SQLSetConnectAttr AUTOCOMMIT_OFF");
    }

    void commit() const
    {
        CHECK_SINK(SQLEndTran(SQL_HANDLE_DBC, connection.get(), SQL_COMMIT), SQL_HANDLE_DBC, connection.get(), "SQLEndTran COMMIT");
    }

    /// Best-effort: called while unwinding a failed execute(), so it logs rather than throws —
    /// the original SQLExecute failure is the error worth surfacing, and a dead connection rolls
    /// back on disconnect anyway.
    void rollback() const noexcept
    {
        if (const auto rc = SQLEndTran(SQL_HANDLE_DBC, connection.get(), SQL_ROLLBACK); rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
        {
            NES_WARNING("ODBCSink rollback failed: {}", showError(SQL_HANDLE_DBC, connection.get(), "SQLEndTran ROLLBACK"));
        }
    }
};
...
ODBCSink::ODBCSink(BackpressureController backpressureController, const SinkDescriptor& sinkDescriptor)
    ...
    , host(sinkDescriptor.getFromConfig(ConfigParametersODBCSink::DB_HOST))
    , port(sinkDescriptor.getFromConfig(ConfigParametersODBCSink::DB_PORT))
...
void ODBCSink::execute(const TupleBuffer& inputTupleBuffer, PipelineExecutionContext&)
{
    ...
    /// NOLINTNEXTLINE(misc-include-cleaner): CPPTRACE_TRY is a macro from <cpptrace/from_current.hpp>; clang-tidy doesn't see the include-symbol link.
    CPPTRACE_TRY
    {
        /// One transaction per buffer (autocommit is off): commit the whole batch at once, and
        /// only count the rows once they are durably committed.
        const auto rows = context->insert.execute(inputTupleBuffer);
        context->connection.commit();
        insertedRows += rows;
    }
    /// NOLINTNEXTLINE(misc-include-cleaner): CPPTRACE_CATCH is a macro from <cpptrace/from_current.hpp>; clang-tidy doesn't see the include-symbol link.
    CPPTRACE_CATCH(...)
    {
