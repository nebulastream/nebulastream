...
@test "example test with small data" {
  ...
  # SEQUENCE UINT64 1 10 1 is end-exclusive, so it generates 1..9.
...
@test "test child buffer" {
  ...
  run docker_nes_cli -t tests/good/single-worker-with-4k-buffers.yaml start "$(cat <<'EOF'
    ...
    GENERATOR (
        ...
        'kafka-sink-test' AS "SINK"."TOPIC",
        'kafka-broker:9092' AS "SINK"."BROKERS",
...
EOF
  ...
  assert_success
  wait_until_status tests/good/single-worker-with-4k-buffers.yaml "Stopped" $output --require-healthy "worker-1"

  wait_until assert_file_line_count results.csv 1000 --ignore-empty-lines
}
...
@test "fails query when broker is unavailable at startup" {
  setup_distributed tests/good/single-worker-with-4k-buffers.yaml
  docker compose kill kafka-broker
  run docker_nes_cli -t tests/good/single-worker-with-4k-buffers.yaml start "$(cat <<'EOF'
    SELECT * FROM GENERATOR(
        'CSV' AS "INPUT_FORMATTER"."TYPE",
        'worker-1:8080' AS "SOURCE"."HOST",
