...
struct LatencyMeasurements
{
    ...
    {
        return os << fmt::format("average latency: {}, min latency: {}, max latency: {}", latencyMeasurements.avgLatency, latencyMeasurements.minLatency, latencyMeasurements.maxLatency);
    }
};
}
...
void MonitoringSink::stop(PipelineExecutionContext&)
{
    ...
    const auto latencyMeasurements = [](const std::unordered_map<SequenceNumber, uint64_t>& bufferLatencies)
    {
        ...
        return LatencyMeasurements{.minLatency =  minLatency, .maxLatency = maxLatency, .avgLatency = latencySum / static_cast<double>(bufferLatencies.size())};
...
void MonitoringSink::execute(const Memory::TupleBuffer& inputBuffer, PipelineExecutionContext&)
{
    ...
    if (inputBuffer.getSourceCreationTimestampInMS().getRawValue() != Timestamp::INITIAL_VALUE)
    {
        ...
        const auto arrivalTimeInMS = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count());
