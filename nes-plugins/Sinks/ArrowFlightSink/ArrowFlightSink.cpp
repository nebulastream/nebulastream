...
ArrowFlightSink::ArrowFlightSink(BackpressureController backpressureController, const SinkDescriptor& sinkDescriptor)
    : Sink(std::move(backpressureController))
    , endpoint(sinkDescriptor.getFromConfig(ConfigParametersArrowFlight::ENDPOINT))
    , streamName(sinkDescriptor.getFromConfig(ConfigParametersArrowFlight::STREAM_NAME))
    , nesSchema(sinkDescriptor.getSchema())
{
}

void ArrowFlightSink::start(PipelineExecutionContext&)
{
    NES_DEBUG("Setting up Arrow Flight sink: {}", *this);

    arrowSchema = nesSchemaToArrowSchema(*nesSchema);

    auto location = arrow::flight::Location::Parse(endpoint);
    INVARIANT(location.ok(), "Failed to parse Flight endpoint '{}': {}", endpoint, location.status().ToString());

    auto clientResult = arrow::flight::FlightClient::Connect(*location);
    INVARIANT(clientResult.ok(), "Failed to connect to Flight server at '{}': {}", endpoint, clientResult.status().ToString());
    client = std::move(*clientResult);

    auto descriptor = arrow::flight::FlightDescriptor::Path({streamName});
    auto putResult = client->DoPut(descriptor, arrowSchema);
    INVARIANT(putResult.ok(), "Failed to start DoPut stream for '{}': {}", streamName, putResult.status().ToString());
    writer = std::move(putResult->writer);
    metadataReader = std::move(putResult->reader);

    NES_INFO("Arrow Flight sink connected to {} stream '{}'", endpoint, streamName);
}

void ArrowFlightSink::stop(PipelineExecutionContext&)
{
    if (writer)
    {
        auto status = writer->DoneWriting();
        if (!status.ok())
        {
            NES_WARNING("ArrowFlightSink: error completing write stream: {}", status.ToString());
        }
        status = writer->Close();
        if (!status.ok())
        {
            NES_WARNING("ArrowFlightSink: error closing write stream: {}", status.ToString());
        }
        writer.reset();
    }
    metadataReader.reset();
    client.reset();
    NES_INFO("Arrow Flight sink completed for stream '{}'", streamName);
}

std::shared_ptr<arrow::RecordBatch> ArrowFlightSink::wrapBufferAsRecordBatch(const TupleBuffer& buffer) const
{
...
void ArrowFlightSink::execute(const TupleBuffer& inputTupleBuffer, PipelineExecutionContext&)
{
    PRECONDITION(inputTupleBuffer, "Invalid input buffer in ArrowFlightSink.");
    ...
    INVARIANT(status.ok(), "ArrowFlightSink: failed to write RecordBatch: {}", status.ToString());
}

DescriptorConfig::Config ArrowFlightSink::validateAndFormat(std::unordered_map<std::string, std::string> config)
{
    return DescriptorConfig::validateAndFormat<ConfigParametersArrowFlight>(std::move(config), NAME);
}

SinkValidationRegistryReturnType RegisterArrowFlightSinkValidation(SinkValidationRegistryArguments sinkConfig)
{
    return ArrowFlightSink::validateAndFormat(std::move(sinkConfig.config));
}

SinkRegistryReturnType RegisterArrowFlightSink(SinkRegistryArguments sinkRegistryArguments)
{
    return std::make_unique<ArrowFlightSink>(
