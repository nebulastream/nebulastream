...
/// A single CASE WHEN branch: the condition (when) and the value to return if it matches (then).
struct WhenThenExpr
{
...
private:
    /// One entry per WHEN/THEN branch, in declaration order. Iterated at compile time
    /// via nautilus::static_iterable in execute().
    std::vector<WhenThenExpr> whenThenFns;
    /// The final fallthrough else function, if no WHEN matches.
