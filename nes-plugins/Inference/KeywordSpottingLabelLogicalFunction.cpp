...
class KeywordSpottingLabelLogicalFunction final : public LogicalFunctionConcept
{
...
public:
    ...
    {
        return fmt::format("KEYWORD_SPOTTING_LABEL({})", child.explain(verbosity));
    }
    ...
    [[nodiscard]] LogicalFunction withInferredDataType(const Schema& schema) const override
    {
        ...
        {
            throw TypeInferenceException("KeywordSpottingLabel expects integer input");
        }
    ...
    [[nodiscard]] std::string_view getType() const override { return "KeywordSpottingLabel"; }
    ...
    {
        if (const auto* casted = dynamic_cast<const KeywordSpottingLabelLogicalFunction*>(&rhs))
        {
...
{
LogicalFunctionRegistryReturnType RegisterKeywordSpottingLabelLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    ...
    return LogicalFunction(KeywordSpottingLabelLogicalFunction{}.withChildren(std::move(arguments.children)));
}
} // namespace NES::LogicalFunctionGeneratedRegistrar
