...
class MelSpectrogramLogicalFunction final : public LogicalFunctionConcept
{
...
public:
    ...
    {
        return fmt::format("MELSPECTROGRAM({})", child.explain(verbosity));
    }
    ...
    [[nodiscard]] LogicalFunction withInferredDataType(const Schema& schema) const override
    {
        ...
        {
            throw TypeInferenceException("MelSpectrogram expects VARSIZED input");
        }
    ...
    [[nodiscard]] std::string_view getType() const override { return "MelSpectrogram"; }
    ...
    {
        if (const auto* casted = dynamic_cast<const MelSpectrogramLogicalFunction*>(&rhs))
        {
...
{
LogicalFunctionRegistryReturnType RegisterMelSpectrogramLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    ...
    return LogicalFunction(MelSpectrogramLogicalFunction{}.withChildren(std::move(arguments.children)));
}
} // namespace NES::LogicalFunctionGeneratedRegistrar
