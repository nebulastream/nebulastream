...
public:
    ...
    /// Non-copyable, non-movable: raw IREE handle ownership is not transferable safely
    /// without explicit reference count management.
    IREERuntimeWrapper(const IREERuntimeWrapper&) = delete;
    IREERuntimeWrapper& operator=(const IREERuntimeWrapper&) = delete;
    IREERuntimeWrapper(IREERuntimeWrapper&&) = delete;
    IREERuntimeWrapper& operator=(IREERuntimeWrapper&&) = delete;
