...
class ArgMaxLogicalFunction : public LogicalFunctionConcept
{
    LogicalFunction child;
    DataType type = DataTypeProvider::provideDataType(DataType::Type::UINT64);

public:
    [[nodiscard]] std::string explain(ExplainVerbosity verbosity) const override
    {
        return fmt::format("ARG_MAX({})", child.explain(verbosity));
    }
    ...
    [[nodiscard]] LogicalFunction withInferredDataType(const Schema& schema) const override
    {
        ...
        copy.type = DataTypeProvider::provideDataType(DataType::Type::UINT64);
        return copy;
    }
    ...
    {
        if (const auto* casted = dynamic_cast<const ArgMaxLogicalFunction*>(&rhs))
        {
...
LogicalFunctionRegistryReturnType RegisterArgMaxLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    ...
    return LogicalFunction(ArgMaxLogicalFunction{}.withChildren(std::move(arguments.children)));
}
}
