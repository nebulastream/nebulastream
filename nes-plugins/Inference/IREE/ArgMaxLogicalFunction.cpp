...
class ArgMaxLogicalFunction : public NodeFunction
{
public:
    ArgMaxLogicalFunction() : NodeFunction(DataTypeProvider::provideDataType("UINT64"), "ArgMax") { }

    void setChildren(std::shared_ptr<NodeFunction> inputTensor) { addChildWithEqual(inputTensor); }

    ~ArgMaxLogicalFunction() noexcept override = default;
    ...
    {
        auto copy = std::make_shared<ArgMaxLogicalFunction>();
    ...
    {
        if (auto other = std::dynamic_pointer_cast<ArgMaxLogicalFunction>(rhs))
        {
    ...
    void inferStamp(const Schema& schema) override
    {
        ...
        this->setStamp(DataTypeProvider::provideDataType("UINT64"));
    }
...
protected:
    ...
    {
        fmt::println(os, "ARG_MAX({})", *children.at(0));
        return os;
    }

    [[nodiscard]] std::ostream& toDebugString(std::ostream& os) const override
    {
        fmt::println(os, "ARG_MAX()");
        return os;
    }
};
...
LogicalFunctionRegistryReturnType RegisterArgMaxLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    ...
    auto function = std::make_shared<ArgMaxLogicalFunction>();
