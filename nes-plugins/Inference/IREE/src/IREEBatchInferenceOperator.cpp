    ...
    ExecutionContext& executionCtx) const
{
    ...
    nautilus::invoke(applyModelProxy, operatorHandler, executionCtx.workerThreadId);
    ...
    for (auto it = pagedVectorRef.begin(fields); it != pagedVectorRef.end(fields); ++it)
    {
        ...
        if (!this->isVarSizedOutput)
        {
            ...
            {
                VarVal result = VarVal(nautilus::invoke(getValueFromModelProxy, rowIdx, operatorHandler, executionCtx.workerThreadId));
        ...
        else
        {
            ...
            nautilus::invoke(copyVarSizedFromModelProxy, output.getContent(), output.getContentSize(), operatorHandler, executionCtx.workerThreadId);
