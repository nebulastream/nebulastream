...
struct MaxValueExecutableFunction : Function
{
    ...
    MaxValueExecutableFunction(std::unique_ptr<Function> child) : child(std::move(child)) { }
...
ExecutableFunctionRegistryReturnType RegisterMaxValueExecutableFunction(ExecutableFunctionRegistryArguments arguments)
{
    ...
    return std::make_unique<MaxValueExecutableFunction>(std::move(arguments.childFunctions[0]));
}
}
