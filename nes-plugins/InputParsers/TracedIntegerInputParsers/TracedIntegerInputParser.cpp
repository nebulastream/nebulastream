...
{
    /// Traced parsing never throws, so we always parse the magnitude and attach the
    /// already-known null flag (configured null markers are not matched by traced parsers).
    const nautilus::val<int8_t> result = tracedSigned<int8_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<int8_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedINT16InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    /// Traced parsing never throws, so we always parse the magnitude and attach the
    /// already-known null flag (configured null markers are not matched by traced parsers).
    const nautilus::val<int16_t> result = tracedSigned<int16_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<int16_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedINT32InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    /// Traced parsing never throws, so we always parse the magnitude and attach the
    /// already-known null flag (configured null markers are not matched by traced parsers).
    const nautilus::val<int32_t> result = tracedSigned<int32_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<int32_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedINT64InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    /// Traced parsing never throws, so we always parse the magnitude and attach the
    /// already-known null flag (configured null markers are not matched by traced parsers).
    const nautilus::val<int64_t> result = tracedSigned<int64_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<int64_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedUINT8InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    /// Traced parsing never throws, so we always parse the magnitude and attach the
    /// already-known null flag (configured null markers are not matched by traced parsers).
    const nautilus::val<uint8_t> result = tracedUnsigned<uint8_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<uint8_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedUINT16InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    ...
    const nautilus::val<uint16_t> result
        = tracedUnsigned<uint16_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<uint16_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedUINT32InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    ...
    const nautilus::val<uint32_t> result
        = tracedUnsigned<uint32_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<uint32_t>>();
    return VarVal{result, nullable, isNull};
}

VarVal TracedUINT64InputParser::parseLazyToVarVal(
    const bool& nullable,
    const nautilus::val<bool>& isNull,
    const nautilus::val<int8_t*>& fieldAddress,
    const nautilus::val<uint64_t>& fieldSize) const
{
    ...
    const nautilus::val<uint64_t> result
        = tracedUnsigned<uint64_t>(false, fieldAddress, fieldSize).getRawValueAs<nautilus::val<uint64_t>>();
    return VarVal{result, nullable, isNull};
}
