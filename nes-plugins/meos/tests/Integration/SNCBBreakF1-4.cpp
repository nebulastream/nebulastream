...
void exportToCsv(const std::vector<NES::Runtime::MemoryLayouts::TestTupleBuffer>& actualBuffers,
    ...
    // Write CSV header
    outFile << "start,end,id,PCFA_min_value,PCFA_max_value,variationPCFA, time\n";
...
TEST_F(ReadSNCB, testF2) {
    ...
    try {
        ...
        for (const auto& buffer : actualBuffers) {
            ...
            for (size_t i = 0; i < numTuples; ++i) {
                auto tuple = buffer[i];
                NES_INFO(" Result - start: {}, end: {}, id: {}, PCFA_min_value: {}, PCFA_max_value: {}, variationPCFA: {}, time: {}",
                    ...
                    tuple[5].read<double>(), //variationPCFA
                    tuple[6].read<uint64_t>()); //time
            }
        }
        ...
        delete meos;
        exportToCsv(actualBuffers, "outputfilterwindowminmaxvariationfilter.csv");   
