...
StagingHandler::StagingHandler(uint64_t stageBufferSize, uint64_t schemaSize)
    : stageBufferSize(stageBufferSize),
      stageBufferCapacity(stageBufferSize / schemaSize), tupleBuffer(nullptr),
      currentWritePosition(0) {
  NES_ASSERT(schemaSize < stageBufferSize,
             "Stage buffer is too small for schema");
}

void StagingHandler::start(Runtime::Execution::PipelineExecutionContextPtr ctx,
                           uint32_t) {
  auto bufferManager = ctx->getBufferManager();
  NES_INFO("Requesting unpooled buffer of size {} from buffer manager",
           stageBufferSize);
...
void StagingHandler::stop(Runtime::QueryTerminationType,
