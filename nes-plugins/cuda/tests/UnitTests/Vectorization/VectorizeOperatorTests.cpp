...
class VectorizeOperatorTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("VectorizeOperatorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down VectorizeOperatorTest test class.");
  }
};
...
TEST_F(VectorizeOperatorTest, vectorizeTupleBuffer__GPU) {
  ...
  // Set the stage buffer size to half of the tuple buffer's size. Hence, the
  // number of invocations should be two.
  ...
  auto stagingHandler =
      std::make_shared<StagingHandler>(stageBufferSize, schemaSize);
  ...
  auto workerContext = std::make_shared<WorkerContext>(INITIAL<WorkerThreadId>,
                                                       bufferManager, 100);
  auto pipelineContext = std::make_shared<MockedPipelineExecutionContext>(
      handlers, true, bufferManager);

  auto ctx = ExecutionContext(Value<MemRef>(nullptr),
                              Value<MemRef>((int8_t*)pipelineContext.get()));

  stagingHandler->start(pipelineContext, 0);

  std::vector<Record::RecordFieldIdentifier> projections = {"f1", "f2"};
  auto collectMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(memoryLayout);
  auto collectOperator = std::make_shared<VectorizedCollectOperator>(
      std::move(collectMemoryProviderPtr), projections);
  auto vectorizeMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(memoryLayout);
  auto vectorizeOperator =
      Vectorize(pipelineContext->getOperatorHandlers().size() - 1,
                std::move(vectorizeMemoryProviderPtr));
