...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("UnvectorizeOperatorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down UnvectorizeOperatorTest test class.");
  }
};

/**
 * @brief Unvectorize operator that processes a tuple buffer by calling the
 * child operator on each tuple one by one.
 */
TEST_F(UnvectorizeOperatorTest, unvectorizeTupleBuffer__GPU) {
  ...
  auto unvectorizeOperator =
      Unvectorize(std::move(memoryProviderPtr), projections);
