...
/**
 * @brief This class is a wrapper for executing CUDA kernel as part of
 ...
 */
template <class InputRecord, class OutputRecord> class CUDAKernelWrapper {
public:
  CUDAKernelWrapper() = default;

  /**
   * @brief Allocate GPU memory to store the input and output of the kernel
   * program. Must be called before kernel execution in the execute() method.
   ...
   */
  void setup(const char *const kernelCode, uint64_t bufferSize,
    ...
    kernelProgramPtr = std::make_shared<jitify::Program>(
        kernelCache.program(kernelCode, headers, 0, callback));
  }

  /**
   * @brief execute the kernel program
   * @param hostInputBuffer input buffer in the host memory. The kernel reads
   * from this buffer and copy back the output to this buffer.
   * @param numberOfInputTuples number of tuple to be processed in this kernel
   * call
   * @param kernelName name of the kernel function
   * @param args additional arguments for the kernel
   */
  template <typename... Args>
  void execute(InputRecord *hostInputBuffer, uint64_t numberOfInputTuples,
               OutputRecord *hostOutputBuffer, uint64_t numberOfOutputTuples,
               const KernelDescriptor &kernel, Args &&...args) {
    if (gpuBufferSize < numberOfInputTuples * sizeof(InputRecord)) {
      std::cout << "Tuples to process exceed the allocated GPU buffer."
                << std::endl;
      throw std::runtime_error(
          "Tuples to process exceed the allocated GPU buffer.");
    }

    // copy the hostInputBuffer (input) to deviceInputBuffer
    cudaMemcpy(deviceInputBuffer, hostInputBuffer,
               numberOfInputTuples * sizeof(InputRecord),
    ...
    kernelProgramPtr->kernel(std::move(kernel.name))
        .instantiate()
        .configure(kernel.grid, kernel.block, kernel.smem,
                   kernel.stream) // the configuration
        .launch(
            deviceInputBuffer, numberOfInputTuples, deviceOutputBuffer,
            std::forward<Args>(args)...); // the parameter of the kernel program

    // copy the result of kernel execution back to the cpu
    cudaMemcpy(hostOutputBuffer, deviceOutputBuffer,
               numberOfOutputTuples * sizeof(OutputRecord),
