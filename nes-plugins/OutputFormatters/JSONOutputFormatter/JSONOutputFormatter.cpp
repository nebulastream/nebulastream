    ...
    nautilus::val<uint64_t>& currentRemainingSize)
{
    switch (fieldType.type)
    {
        ...
        case DataType::Type::VECTOR: {
            /// Vector can use the same pattern as Fixedsized
            const auto vectorVal = value.getRawValueAs<VectorData>();
            for (nautilus::val<uint64_t> i = 0; i < vectorVal.getNumElements(); ++i)
            {
                const nautilus::val<uint64_t> amountWritten = nautilus::invoke(
                    writeArrayPrefix,
                    i == nautilus::val<uint64_t>{0},
                ...
                currentRemainingSize -= amountWritten;
                writeValue(
                    fieldType.elementType[0], vectorVal.at(i), fieldPointer, recordBuffer, bufferProvider, written, currentRemainingSize);
            }
            const nautilus::val<uint64_t> amountWritten = nautilus::invoke(
                writeArrayEnd, currentRemainingSize, recordBuffer.getReference(), bufferProvider, fieldPointer + written);
            written += amountWritten;
