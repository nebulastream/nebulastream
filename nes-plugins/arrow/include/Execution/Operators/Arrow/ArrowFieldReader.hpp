 ...
 * The field reader accesses a column in an RecordBatch and provides access to
 * individual fields.
 */
class AbstractArrowFieldReader {
 public:
  AbstractArrowFieldReader(const uint64_t fieldIndex,
                           const Record::RecordFieldIdentifier& fieldName);
  virtual Value<> getColumn(const Value<MemRef>& recordBatch) = 0;
  virtual Value<> getValue(const Value<MemRef>& column,
                           const Value<UInt64>& index) = 0;
...
class ArrowFieldReader : public AbstractArrowFieldReader {
 public:
  ArrowFieldReader(const uint64_t fieldIndex,
                   const Record::RecordFieldIdentifier& fieldName);
  Value<> getColumn(const Value<MemRef>& recordBatch) override;
  Value<> getValue(const Value<MemRef>& column,
                   const Value<UInt64>& index) override;
};
