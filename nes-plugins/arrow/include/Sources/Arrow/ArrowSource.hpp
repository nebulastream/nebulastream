 ...
 * @brief more specifically we currently support the Arrow IPC format (a.k.a
 * Feather v2)
 * @see
...
public:
   ...
   * @param arrowSourceType points to the current source configuration object,
   * look at mqttSourceType for info
   * @param operatorId current operator id
   * @param numSourceLocalBuffers
   * @param gatheringMode
   * @param successors
   */
  explicit ArrowSource(
      SchemaPtr schema, Runtime::BufferManagerPtr bufferManager,
      Runtime::QueryManagerPtr queryManager, ArrowSourceTypePtr arrowSourceType,
...
private:
  ...
  // TODO #4083: these should move to an ArrowWrapper when we support other
  // formats from Arrow Arrow status returns at every operation also do not play
  // well currently
  std::shared_ptr<arrow::io::ReadableFile> inputFile;
  // A record batch in Arrow is two-dimensional data structure that is
  // semantically a sequence of fields, each a contiguous Arrow array See:
  // https://arrow.apache.org/docs/cpp/api/table.html#_CPPv4N5arrow11RecordBatchE
  // At any point in time that we read a record batch from the
  // RecordBatchStreamReader, we maintain it in currentRecordBatch and
  // subsequently transfer the records from it to the TestTupleBuffer
  ...
  /**
   * @brief opens the Arrow inputSource, and initializes the
   ...
   * @return
   */
  void populateOptionsFromSchema(arrow::csv::ReadOptions &readOptions,
  ...
  /**
   * @brief this function writes the data from the record batches to
   * TestTupleBuffer
   * @param tupleCountInBuffer is the count of total filled buffers in
   * tupleBuffer
   ...
   */
  void writeRecordBatchToTupleBuffer(
      uint64_t tupleCount, Runtime::MemoryLayouts::TestTupleBuffer &buffer,
   ...
   * @return returns true if success
   */
  void writeArrowArrayToTupleBuffer(
      uint64_t tupleCountInBuffer, uint64_t schemaFieldIndex,
