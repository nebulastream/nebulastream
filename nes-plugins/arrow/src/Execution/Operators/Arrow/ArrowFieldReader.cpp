...
AbstractArrowFieldReader::AbstractArrowFieldReader(
    const uint64_t fieldIndex, const Record::RecordFieldIdentifier &fieldName)
    : fieldIndex(fieldIndex), fieldName(fieldName) {}

template <typename ArrowType>
ArrowFieldReader<ArrowType>::ArrowFieldReader(
    const uint64_t fieldIndex, const Record::RecordFieldIdentifier &fieldName)
    : AbstractArrowFieldReader(fieldIndex, fieldName) {}

template <typename T> void *getRawArrowColumn(void *ptr, uint64_t index) {
...
template <typename T> void *getArrowColumn(void *ptr, uint64_t index) {
  auto wrapper = (RecordBufferWrapper *)ptr;
  auto column = wrapper->batch->column(index);
  return (void *)std::static_pointer_cast<T>(column).get();
}

template <typename T> auto getArrayValue(void *ptr, uint64_t index) {
  auto array = (T *)ptr;
...
template <typename ArrowType>
Value<> ArrowFieldReader<ArrowType>::getColumn(
    const Value<NES::Nautilus::MemRef> &recordBatch) {
  if constexpr (HasRawValues<ArrowType>) {
    return FunctionCall<>(
        "getRawArrowColumn" + std::string(typeid(ArrowType).name()),
        getRawArrowColumn<ArrowType>, recordBatch, Value<UInt64>(fieldIndex));
  } else {
    return FunctionCall<>("getColumn" + std::string(typeid(ArrowType).name()),
                          getArrowColumn<ArrowType>, recordBatch,
                          Value<UInt64>(fieldIndex));
  }
}

template <typename ArrowType>
Value<> ArrowFieldReader<ArrowType>::getValue(
    const Value<NES::Nautilus::MemRef> &column, const Value<UInt64> &index) {
  if constexpr (HasRawValues<ArrowType>) {
    ...
    return FunctionCall<>("getValue" + std::string(typeid(ArrowType).name()),
                          getArrayValue<ArrowType>, column, index);
  }
}
...
std::vector<std::shared_ptr<AbstractArrowFieldReader>>
createArrowFieldReaderFromSchema(const SchemaPtr &schema) {
  std::vector<std::shared_ptr<AbstractArrowFieldReader>> readers;
  auto df = DefaultPhysicalTypeFactory();
  for (auto fieldIndex = 0_u64; fieldIndex < schema->fields.size();
       fieldIndex++) {
    ...
    if (physicalType->isBasicType()) {
      ...
      case NES::BasicPhysicalType::NativeType::INT_8: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::Int8Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::INT_16: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::Int16Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::INT_32: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::Int32Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::INT_64: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::Int64Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::UINT_8: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::UInt8Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::UINT_16: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::UInt16Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::UINT_32: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::UInt32Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::UINT_64: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::UInt64Array>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::FLOAT: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::FloatArray>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::DOUBLE: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::DoubleArray>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::TEXT: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::StringArray>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      case NES::BasicPhysicalType::NativeType::BOOLEAN: {
        auto reader = std::make_shared<ArrowFieldReader<arrow::BooleanArray>>(
            fieldIndex, field->getName());
        readers.emplace_back(reader);
        break;
      }
      ...
      // We do not support any other ARROW types (such as Lists, Maps, Tensors)
      // yet. We could however later store them in the childBuffers similar to
      // how we store TEXT and push the computation supported by arrow down to
      // them.
      NES_NOT_IMPLEMENTED();
    }
  }
