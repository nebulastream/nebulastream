...
ArrowSource::ArrowSource(
    ...
    : DataSource(schema, std::move(bufferManager), std::move(queryManager),
      ...
      fileEnded(false), arrowSourceType(arrowSourceType),
  ...
  NES_DEBUG("ArrowSource: tupleSize={} freq={}ms numBuff={} "
            "numberOfTuplesToProducePerBuffer={}",
...
void ArrowSource::populateOptionsFromSchema(
  ...
  for (uint64_t schemaFieldIdx = 0; schemaFieldIdx < numberOfSchemaFields;
    ...
    if (!physicalType->isBasicType()) {
      NES_THROW_RUNTIME_ERROR("ArrowSource::populateConvertOptionsFromSchema: "
                              "error inferring types");
    }
  ...
  try {
    if (physicalType->isBasicType()) {
      ...
      case NES::BasicPhysicalType::NativeType::CHAR: {
        NES_FATAL_ERROR("ArrowSource::writeArrowArrayToTupleBuffer: type CHAR "
      ...
      case NES::BasicPhysicalType::NativeType::BOOLEAN: {
        NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::BOOL,
                        "ArrowSource::writeArrowArrayToTupleBuffer: "
                        "inconsistent BOOL data types. Type found"
                        " in file : " +
                            arrowArray->type()->ToString() + ", and type id: " +
                            std::to_string(arrowArray->type()->id()));
      ...
      case NES::BasicPhysicalType::NativeType::UNDEFINED:
        NES_FATAL_ERROR(
            "ArrowSource::writeArrowArrayToTupleBuffer: Field Type UNDEFINED");
      }
    } else {
