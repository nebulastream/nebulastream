    ...
    , numberOfTuplesToProducePerBuffer(arrowSourceType->getNumberOfTuplesToProducePerBuffer()->getValue())
{
    ...
    NES_DEBUG(
        "ArrowSource: tupleSize={} freq={}ms numBuff={} numberOfTuplesToProducePerBuffer={}",
        this->tupleSize,
...
{
    std::stringstream ss;
    ss << "ARROW_SOURCE(SCHEMA(" << schema->toString() << "), FILE=" << filePath << " freq=" << this->gatheringInterval.count() << "ms"
...
void ArrowSource::fillBuffer(Runtime::MemoryLayouts::TestTupleBuffer& buffer)
{
    ...
    else
    {
        ...
        NES_ASSERT2_FMT(
            generatedTuplesThisPass * tupleSize < buffer.getBuffer().getBufferSize(),
...
SourceType ArrowSource::getType() const
{
    return SourceType::ARROW_SOURCE;
}

std::string ArrowSource::getFilePath() const
{
    return filePath;
}
...
arrow::Status ArrowSource::openCsvFile()
{
    ...
    auto maybe_reader
        = arrow::csv::StreamingReader::Make(arrow::io::default_io_context(), inputFile, read_options, parse_options, convert_options);
    if (!maybe_reader.ok())
    {
    ...
    const std::shared_ptr<arrow::Array> arrowArray)
{
    ...
    try
    {
        if (physicalType->isBasicType())
        {
            ...
            switch (basicPhysicalType->nativeType)
            {
                case NES::BasicPhysicalType::NativeType::INT_8: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::INT8,
                ...
                case NES::BasicPhysicalType::NativeType::INT_16: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::INT16,
                ...
                case NES::BasicPhysicalType::NativeType::INT_32: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::INT32,
                ...
                case NES::BasicPhysicalType::NativeType::INT_64: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::INT64,
                ...
                case NES::BasicPhysicalType::NativeType::UINT_8: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::UINT8,
                ...
                case NES::BasicPhysicalType::NativeType::UINT_16: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::UINT16,
                ...
                case NES::BasicPhysicalType::NativeType::UINT_32: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::UINT32,
                ...
                case NES::BasicPhysicalType::NativeType::UINT_64: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::UINT64,
                ...
                case NES::BasicPhysicalType::NativeType::FLOAT: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::FLOAT,
                ...
                case NES::BasicPhysicalType::NativeType::DOUBLE: {
                    NES_ASSERT2_FMT(
                        arrowArray->type()->id() == arrow::Type::type::DOUBLE,
                ...
                case NES::BasicPhysicalType::NativeType::BOOLEAN: {
                    NES_ASSERT2_FMT(
                        ...
                        " in file : "
                            + arrowArray->type()->ToString() + ", and type id: " + std::to_string(arrowArray->type()->id()));
        ...
        {
            NES_ASSERT2_FMT(
                arrowArray->type()->id() == arrow::Type::type::STRING,
