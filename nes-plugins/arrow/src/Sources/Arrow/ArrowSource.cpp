...
ArrowSource::ArrowSource(
    ...
    : DataSource(schema, std::move(bufferManager), std::move(queryManager),
      ...
      filePath(arrowSourceType->getFilePath()->getValue()),
      skipHeader(arrowSourceType->getSkipHeader()->getValue()),
      delimiter(arrowSourceType->getDelimiter()->getValue()),
      numberOfTuplesToProducePerBuffer(
          arrowSourceType->getNumberOfTuplesToProducePerBuffer()->getValue()) {
  this->numberOfBuffersToProduce =
      arrowSourceType->getNumberOfBuffersToProduce()->getValue();
  this->gatheringInterval = std::chrono::milliseconds(
      arrowSourceType->getGatheringInterval()->getValue());
  ...
  if (path == nullptr) {
    NES_THROW_RUNTIME_ERROR(
        "ArrowSource::ArrowSource: Could not determine absolute pathname: "
        << filePath.c_str());
  }
  ...
  if (!openFileStatus.ok()) {
    NES_THROW_RUNTIME_ERROR(
        "ArrowSource::ArrowSource file error: " << openFileStatus.ToString());
  }

  NES_DEBUG("ArrowSource: Opened Arrow file {}", path.get());
  NES_DEBUG(
      "ArrowSource: tupleSize={} freq={}ms numBuff={} "
      "numberOfTuplesToProducePerBuffer={}",
...
std::optional<Runtime::TupleBuffer> ArrowSource::receiveData() {
  ...
  fillBuffer(buffer);
  NES_TRACE("ArrowSource::receiveData filled buffer with tuples= {}",
...
std::string ArrowSource::toString() const {
  std::stringstream ss;
  ss << "ARROW_SOURCE(SCHEMA(" << schema->toString() << "), FILE=" << filePath
...
void ArrowSource::fillBuffer(Runtime::MemoryLayouts::TestTupleBuffer& buffer) {
  ...
  NES_TRACE("ArrowSource::fillBuffer: start at record_batch={} fileSize={}",
            currentRecordBatch->ToString(), fileSize);
  ...
  if (numberOfTuplesToProducePerBuffer == 0) {
    ...
    NES_ASSERT2_FMT(generatedTuplesThisPass * tupleSize <
                    ...
                    "ArrowSource::fillBuffer: not enough space in tuple buffer "
                    "to fill tuples in this pass.");
  }
  NES_TRACE("ArrowSource::fillBuffer: fill buffer with #tuples={} of size={}",
            generatedTuplesThisPass, tupleSize);

  uint64_t tupleCount = 0;

  // Compute how many tuples we can generate from the current batch
  uint64_t tuplesRemainingInCurrentBatch =
      currentRecordBatch->num_rows() - indexWithinCurrentRecordBatch;

  // Case 1. The number of remaining records in the currentRecordBatch are less
  // than generatedTuplesThisPass. Copy the records from the record batch and
  // keep reading and copying new record batches to fill the buffer.
  if (tuplesRemainingInCurrentBatch < generatedTuplesThisPass) {
    // get the slice of the record batch, slicing is a no copy op
    auto recordBatchSlice = currentRecordBatch->Slice(
        indexWithinCurrentRecordBatch, tuplesRemainingInCurrentBatch);
    ...
    // keep reading record batch and writing to tuple buffer until we have
    // generated generatedTuplesThisPass number of tuples
    while (tupleCount < generatedTuplesThisPass) {
      ...
      if ((tupleCount + currentRecordBatch->num_rows()) <=
          generatedTuplesThisPass) {
      ...
      // case when we have to write only a part of the batch to the tuple buffer
        ...
        recordBatchSlice = currentRecordBatch->Slice(
  ...
  // Case 2. The number of remaining records in the currentRecordBatch are
  // greater than generatedTuplesThisPass. simply copy the desired number of
  // tuples from the recordBatch to the tuple buffer
  else {
...
const ArrowSourceTypePtr& ArrowSource::getSourceConfig() const {
...
arrow::Status ArrowSource::openFile() {
  ...
  ARROW_ASSIGN_OR_RAISE(inputFile, arrow::io::ReadableFile::Open(
                                       filePath, arrow::default_memory_pool()));
  ARROW_ASSIGN_OR_RAISE(recordBatchStreamReader,
                        arrow::ipc::RecordBatchStreamReader::Open(inputFile));
...
arrow::Status ArrowSource::openCsvFile() {
  ...
  auto maybe_reader = arrow::csv::StreamingReader::Make(
      arrow::io::default_io_context(), inputFile, read_options, parse_options,
      convert_options);
  if (!maybe_reader.ok()) {
    // Handle TableReader instantiation error...
    NES_THROW_RUNTIME_ERROR(
        "ArrowSource::ArrowSource CSV reader error: " << maybe_reader.status());
  }
  csvStreamingReader = *maybe_reader;
  return arrow::Status::OK();
}

void ArrowSource::populateOptionsFromSchema(
  ...
  for (uint64_t schemaFieldIdx = 0; schemaFieldIdx < numberOfSchemaFields;
       ++schemaFieldIdx) {
    ...
    if (!physicalType->isBasicType()) {
      NES_THROW_RUNTIME_ERROR(
          "ArrowSource::populateConvertOptionsFromSchema: error inferring "
          "types");
    }
    ...
    switch (basicPhysicalType->nativeType) {
      ...
      case BasicPhysicalType::NativeType::UNDEFINED:
        NES_THROW_RUNTIME_ERROR(
            "ArrowSource::writeArrowArrayToTupleBuffer: illegal type");
        break;
    }
  }
}
...
void ArrowSource::readNextBatch() {
  ...
  if (!readStatus.ok()) {
    NES_THROW_RUNTIME_ERROR(
        "ArrowSource::fillBuffer: error reading recordBatch: "
        << readStatus.ToString());
  }
...
// Core Logic in writeRecordBatchToTupleBuffer() and
// writeArrowArrayToTupleBuffer(): Arrow (and Parquet) format(s) is(are)
// column-oriented. Instead of reconstructing each tuple due to high
// reconstruction cost, we instead retrieve each column from the arrow
// RecordBatch and then write out the whole column in the tuple buffer.
void ArrowSource::writeRecordBatchToTupleBuffer(
...
void ArrowSource::writeArrowArrayToTupleBuffer(
  ...
  if (arrowArray == nullptr) {
    NES_THROW_RUNTIME_ERROR(
        "ArrowSource::writeArrowArrayToTupleBuffer: arrowArray is null.");
  }
  ...
  try {
      ...
      switch (basicPhysicalType->nativeType) {
        case NES::BasicPhysicalType::NativeType::INT_8: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::INT8,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent INT8 data types. Type found"
                          " in IPC file: " +
        ...
        case NES::BasicPhysicalType::NativeType::INT_16: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::INT16,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent INT16 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());
        ...
        case NES::BasicPhysicalType::NativeType::INT_32: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::INT32,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent INT32 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());
        ...
        case NES::BasicPhysicalType::NativeType::INT_64: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::INT64,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent INT64 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());
        ...
        case NES::BasicPhysicalType::NativeType::UINT_8: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::UINT8,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent UINT8 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());
        ...
        case NES::BasicPhysicalType::NativeType::UINT_16: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::UINT16,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent UINT16 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());

          // cast the arrow array to the uint16_t type
          auto values =
              std::static_pointer_cast<arrow::UInt16Array>(arrowArray);

          // write all values to the tuple buffer
          for (uint64_t index = 0; index < arrayLength; ++index) {
            uint64_t bufferRowIndex = tupleCountInBuffer + index;
            uint16_t value = values->Value(index);
            tupleBuffer[bufferRowIndex][schemaFieldIndex].write<uint16_t>(
                value);
          }
          break;
        }
        case NES::BasicPhysicalType::NativeType::UINT_32: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::UINT32,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent UINT32 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());

          // cast the arrow array to the uint32_t type
          auto values =
              std::static_pointer_cast<arrow::UInt32Array>(arrowArray);

          // write all values to the tuple buffer
          for (uint64_t index = 0; index < arrayLength; ++index) {
            uint64_t bufferRowIndex = tupleCountInBuffer + index;
            uint32_t value = values->Value(index);
            tupleBuffer[bufferRowIndex][schemaFieldIndex].write<uint32_t>(
                value);
          }
          break;
        }
        case NES::BasicPhysicalType::NativeType::UINT_64: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::UINT64,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent UINT64 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());

          // cast the arrow array to the uint64_t type
          auto values =
              std::static_pointer_cast<arrow::UInt64Array>(arrowArray);

          // write all values to the tuple buffer
          for (uint64_t index = 0; index < arrayLength; ++index) {
            uint64_t bufferRowIndex = tupleCountInBuffer + index;
            uint64_t value = values->Value(index);
            tupleBuffer[bufferRowIndex][schemaFieldIndex].write<uint64_t>(
                value);
          }
          break;
        }
        case NES::BasicPhysicalType::NativeType::FLOAT: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::FLOAT,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent FLOAT data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());
        ...
        case NES::BasicPhysicalType::NativeType::DOUBLE: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::DOUBLE,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent FLOAT64 data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());

          // cast the arrow array to the float64 type
          auto values =
              std::static_pointer_cast<arrow::DoubleArray>(arrowArray);
        ...
        case NES::BasicPhysicalType::NativeType::CHAR: {
          NES_FATAL_ERROR(
              "ArrowSource::writeArrowArrayToTupleBuffer: type CHAR not "
              "supported by Arrow.");
        ...
        case NES::BasicPhysicalType::NativeType::TEXT: {
          NES_ASSERT2_FMT(arrowArray->type()->id() == arrow::Type::type::STRING,
                          "ArrowSource::writeArrowArrayToTupleBuffer: "
                          "inconsistent STRING data types. Type found"
                          " in IPC file: " +
                              arrowArray->type()->ToString());

          // cast the arrow array to the string type
          auto values =
              std::static_pointer_cast<arrow::StringArray>(arrowArray);
          ...
          for (uint64_t index = 0; index < arrayLength; ++index) {
            ...
            auto childTupleBuffer =
                allocateVariableLengthField(localBufferManager, totalSize);

            NES_ASSERT2_FMT(childTupleBuffer.getBufferSize() >= totalSize,
                            "Parser::writeFieldValueToTupleBuffer(): Could not "
                            "write TEXT field to tuple buffer, there was not "
                            "sufficient space available. Required space: "
                                << totalSize << ", available space: "
                                << childTupleBuffer.getBufferSize());

            // write out the length and the variable-sized text to the child
            // buffer
            (*childTupleBuffer.getBuffer<uint32_t>()) = sizeOfValue;
            std::memcpy(childTupleBuffer.getBuffer() + sizeof(uint32_t),
                        value.data(), sizeOfValue);

            // attach the child buffer to the parent buffer and write the child
            // buffer index in the schema field index of the tuple buffer
            auto childIdx =
                tupleBuffer.getBuffer().storeChildBuffer(childTupleBuffer);
            tupleBuffer[bufferRowIndex][schemaFieldIndex]
                .write<Runtime::TupleBuffer::NestedTupleBufferKey>(childIdx);
          }
          break;
        }
        case NES::BasicPhysicalType::NativeType::BOOLEAN: {
          NES_ASSERT2_FMT(
              arrowArray->type()->id() == arrow::Type::type::BOOL,
              "ArrowSource::writeArrowArrayToTupleBuffer: inconsistent BOOL "
              "data types. Type found"
              " in file : " +
                  arrowArray->type()->ToString() +
          ...
          auto values =
              std::static_pointer_cast<arrow::BooleanArray>(arrowArray);
        ...
        case NES::BasicPhysicalType::NativeType::UNDEFINED:
          NES_FATAL_ERROR(
              "ArrowSource::writeArrowArrayToTupleBuffer: Field Type "
              "UNDEFINED");
      }
    } else {
    ...
    NES_ERROR(
        "Failed to convert the arrowArray to desired NES data type. Error: {}",
        e.what());
  }
}
