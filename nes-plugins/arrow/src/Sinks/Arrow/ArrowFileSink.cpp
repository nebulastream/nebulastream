...
ArrowFileSink::ArrowFileSink(SinkFormatPtr format,
  ...
  if (sinkFormat->getSinkFormat() == FormatTypes::ARROW_IPC_FORMAT) {
    // raise a warning if the file path does not have the streaming "arrows"
    // extension some other system might thus interpret the file differently
    // with different extension the MIME types for arrow files are ".arrow" for
    // file format, and ".arrows" for streaming file format see
    // https://arrow.apache.org/faq/
    if (!(filePath.find(".arrows"))) {
      NES_WARNING("ArrowFileSink: An arrow ipc file without '.arrows' "
                  "extension created as a file sink.");
    }
  }
}
...
bool ArrowFileSink::writeData(Runtime::TupleBuffer &inputBuffer,
...
bool ArrowFileSink::writeDataToArrowFile(Runtime::TupleBuffer &inputBuffer) {
  ...
  NES_TRACE("ArrowFileSink: getSchema medium {} format {} and mode {}",
            toString(), sinkFormat->toString(), this->getAppendAsString());
  ...
  arrow::Status openStatus =
      openArrowFile(outfileArrow, arrowSchema, arrowWriter);
  ...
  std::vector<std::shared_ptr<arrow::Array>> arrowArrays =
      arrowFormat->getArrowArrays(inputBuffer);

  // make a record batch
  std::shared_ptr<arrow::RecordBatch> recordBatch = arrow::RecordBatch::Make(
      arrowSchema, arrowArrays[0]->length(), arrowArrays);
...
arrow::Status ArrowFileSink::openArrowFile(
  ...
  ARROW_ASSIGN_OR_RAISE(arrowFileOutputStream,
                        arrow::io::FileOutputStream::Open(filePath, append));
  ARROW_ASSIGN_OR_RAISE(
      arrowRecordBatchWriter,
      arrow::ipc::MakeStreamWriter(arrowFileOutputStream, arrowSchema));
