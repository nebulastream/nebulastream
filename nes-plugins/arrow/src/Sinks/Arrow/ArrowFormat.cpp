...
ArrowFormat::getArrowArrays(Runtime::TupleBuffer &inputBuffer) {
    ...
    try {
      if (physicalType->isBasicType()) {
        ...
        case NES::BasicPhysicalType::NativeType::INT_8: {
          ...
          if (!buildArray.ok()) {
            NES_FATAL_ERROR("ArrowFormat::getArrowArrays: could not convert "
                            "INT_8 field to arrow array.");
          }
        ...
        case NES::BasicPhysicalType::NativeType::TEXT: {
          ...
          for (uint64_t rowIndex = 0; rowIndex < numberOfTuples; ++rowIndex) {
            ...
            if (sizeOfTextField > 0) {
              ...
              NES_WARNING("ArrowFormat::getArrowArrays: Variable-length field "
                          "could not be read."
                          "Invalid size in the variable-length TEXT field. "
        ...
        case NES::BasicPhysicalType::NativeType::CHAR: {
          NES_FATAL_ERROR(
              "ArrowFormat::getArrowArrays: type CHAR not supported by Arrow.");
      ...
      NES_ERROR("ArrowFormat::getArrowArrays: Failed to convert the arrowArray "
                "to desired NES data type. "
                "Error: {}",
                e.what());
    }
    offset += fieldSize;
  ...
  for (uint64_t i = 0; i < numberOfFields; i++) {
    ...
    try {
      if (physicalType->isBasicType()) {
        ...
        case NES::BasicPhysicalType::NativeType::CHAR: {
          NES_FATAL_ERROR(
              "ArrowFormat::getArrowSchema: type CHAR not supported by Arrow.");
          throw std::invalid_argument("Arrow does not support CHAR");
          break;
        }
        case NES::BasicPhysicalType::NativeType::UNDEFINED: {
      ...
      NES_ERROR("Failed to convert the arrowArray to desired NES data type. "
                "Error: {}",
                e.what());
    }
  }
