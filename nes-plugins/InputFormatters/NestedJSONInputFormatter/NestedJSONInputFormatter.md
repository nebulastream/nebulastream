# NestedJSONInputFormatter

The NestedJSONInputFormatter parses JSON-encoded data from sources (files, TCP streams, etc.) into NebulaStream's internal TupleBuffer representation, with support for accessing fields inside nested JSON objects. It is built on top of [SIMDJSON](https://github.com/simdjson/simdjson)'s `ondemand` parser for high-performance, lazy evaluation of JSON documents.
...
CREATE PHYSICAL SOURCE FOR mySource TYPE File SET('NestedJSON' AS PARSER."TYPE");
```
...
Arrays are not supported yet. Array fields in the JSON are ignored/skipped, and there is no syntax for array indexing (`field[0]`) or array flattening/unnesting.

Stubs for array support live in `NestedJSONParsingUtil.hpp` (`parseJsonArrayProxy`, `parseJsonArray`) and in the `writeValueToRecord` switch in `NestedJSONRawBufferIndex.cpp`. Search for `TODO` to find the insertion points. To turn this on end-to-end, `DataType` (in `nes-data-types`) needs an `ARRAY` type that carries an element type, the SQL parser needs to accept `ARRAY<...>` syntax, and `VarVal`/`Record` need a way to carry an array payload (most plausibly a length-prefixed blob inside `VariableSizedData`).
