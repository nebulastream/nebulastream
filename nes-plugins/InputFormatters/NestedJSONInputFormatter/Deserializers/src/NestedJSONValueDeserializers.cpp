...
VarVal JSONVECTORValueDeserializer::deserializeToVarVal(
    ...
    ArenaRef& arena) const
{
    /// Allocate memory for the vector's buffer
    const auto elementSize = valueType.elementType[0].getSizeInBytesWithoutNull();
    /// The number of elements in a vector depends on the value itself. We obtain them from the value itself.
    const nautilus::val<size_t> vectorCount
        = nautilus::invoke(NestedJSONValueDeserializer::getNumberOfVectorElements, fieldAddress, fieldSize);
    const nautilus::val<int8_t*> buffer = arena.allocateMemory(vectorCount * elementSize);
    ...
    for (nautilus::val<size_t> i; i < vectorCount; ++i)
    {
        /// Get address and size of element i
    ...
    const VectorData vectorData{buffer, valueType.elementType[0], vectorCount * elementSize};
    ...
    const nautilus::val<int8_t*>& bufferAddress) const
{
    ...
    /// Write the vector buffer address and size into the buffer provided by the function
    ...
    nautilus::invoke(
        ...
        nautilus::val<uint64_t>{elementSize * vectorCount});
}
...
ValueDeserializerRegistryReturnType
ValueDeserializerGeneratedRegistrar::RegisterJSONVECTORValueDeserializer(ValueDeserializerRegistryArguments)
{
