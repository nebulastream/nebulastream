    ...
    int8_t* output)
{
    ...
    switch (dataType.type)
    {
        ...
        case DataType::Type::VECTOR: {
            /// Treat like fixedsized, but do not check if the number of elements in the vector is higher than the count member of the datatype
            const DataType elementType{dataType.elementType, DataType::NULLABLE::NOT_NULLABLE};
            const auto elementSize = elementType.getSizeInBytesWithoutNull();
            auto array = jsonValue.get_array();
            if (not array.has_value())
...
struct JsonRecordParser
{
    ...
    vectorProxy(const FieldIndex fieldIndex, RawBufferIndex* rawBufferIndex, const InputFormatIndexer* indexer, Arena* arena)
    {
        ...
        /// Get the needed value out of the indexer early to allocate the memory for the vector
        auto* concreteRbi = static_cast<typename Traits::BufferIndex*>(rawBufferIndex);
        const auto* concreteIndexer = static_cast<const typename Traits::Indexer*>(indexer);
        ...
        ArenaRef& arena)
    {
        ...
        const nautilus::val<ParseResultVarSized*> parseResult
            = nautilus::invoke({nautilus::ModRefInfo::Ref}, vectorProxy, fieldIndex, rawBufferIndex, indexer, arena.getArena());
