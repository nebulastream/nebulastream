    ...
    Arena* arena)
{
    ...
    {
        if (dataType.nullable && dataType.type != DataType::Type::FIXEDSIZED && dataType.type != DataType::Type::VECTOR)
        {
    ...
    switch (dataType.type)
    {
            ...
            for (auto element : array.value())
            {
                ...
                switch (dataType.elementType[0].type)
                {
                    ...
                    case DataType::Type::VECTOR: {
                        /// Store the ptr and size of the vector byte aligned -> uses 16 bytes, which were allocated for this purpose
        ...
        case DataType::Type::VECTOR: {
            /// Treat like fixedsized, but do not check if the number of element of the array is higher than the count member of the datatype
            const auto elementSize = dataType.elementType[0].getSizeInBytesWithoutNull();
            auto array = jsonValue.get_array();
            if (not array.has_value())
            {
            ...
            const uint64_t vectorSize = elementSize * array.value().count_elements();
            int8_t* vectorBuffer = reinterpret_cast<int8_t*>(arena->allocateMemory(vectorSize).data());

            /// Iterate over vector elements and write them into the buffer
            uint64_t i = 0;
            for (auto element : array.value())
            {
                parseValue(dataType.elementType[0], element, fieldName, vectorBuffer + i * elementSize, arena);
                ++i;
            }
            /// Set the ptr to the buffer memory and the size to the VectorResult
            *reinterpret_cast<ParseResultVector*>(output) = {.ptr = reinterpret_cast<char*>(vectorBuffer), .size = vectorSize};
            return;
        }
        case DataType::Type::STRUCT: {
            ...
            for (const auto& [name, fieldType] : dataType.fields)
            {
                ...
                {
                    case DataType::Type::VECTOR: {
                        /// Byte align the ptr to the vector and the size of the vector (16 bytes)
                        ParseResultVector parseResult{};
...
struct JsonRecordParser
{
        ...
        const FieldIndex fieldIndex, RawBufferIndex* rawBufferIndex, const InputFormatIndexer* indexer, int8_t* output, Arena* arena)
    {
        ...
        const auto& fieldName = concreteIndexer->getFieldNameInJsonAt(fieldIndex).asCanonicalString();
    ...
    /// nautilus::invoke target for a VECTOR field. Allocates buffer via the arena based on the number of elements in the json array,
    /// and writes the parsed elements into the allocated buffer. Returns it as a pointer to a ParseResultVector object containing the ptr
    /// to the buffer and the size of the buffer.
    static ParseResultVector*
    vectorProxy(const FieldIndex fieldIndex, RawBufferIndex* rawBufferIndex, const InputFormatIndexer* indexer, Arena* arena)
    {
        thread_local ParseResultVector slot{};
        navigateAndParse(fieldIndex, rawBufferIndex, indexer, reinterpret_cast<int8_t*>(&slot), arena);
        return &slot;
    }
        ...
        ArenaRef& arena)
    {
        switch (dataType.type)
        {
            ...
            case DataType::Type::VECTOR:
                if constexpr (Traits::supportsFixedSized)
                {
                    return wrapVector(dataType, fieldIndex, rawBufferIndex, indexer, arena);
                }
                else
                {
                    throw NotImplemented(
                        "This JSON formatter does not support vectors; use the NestedJSON formatter instead.");
                }
            case DataType::Type::STRUCT:
                if constexpr (Traits::supportsFixedSized)
                {
        ...
        ArenaRef& arena)
    {
        ...
        const nautilus::val<ParseResultVector*> parseResult
            = nautilus::invoke(vectorProxy, fieldIndex, rawBufferIndex, indexer, arena.getArena());
        const VectorData vectorData{
            *getMemberWithOffset<int8_t*>(parseResult, offsetof(ParseResultVector, ptr)),
            dataType.elementType[0],
            *getMemberWithOffset<uint64_t>(parseResult, offsetof(ParseResultVector, size))};

        /// This is a POC, we omit the is null case for now
        return VarVal{vectorData, dataType.nullable, nautilus::val<bool>{false}};
    }
