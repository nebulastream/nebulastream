    ...
    const nautilus::val<const InputFormatIndexer*>& indexer)
{
    if (nullable)
    {
        const auto parseResult
            = nautilus::invokeGuarded<&parseJsonFixedSizeIntoVarValProxy<T, true>, FieldIndex, RawBufferIndex*, const InputFormatIndexer*>(
    ...
    const auto parseResult
        = nautilus::invokeGuarded<&parseJsonFixedSizeIntoVarValProxy<T, false>, FieldIndex, RawBufferIndex*, const InputFormatIndexer*>(
            fieldIndex, rawBufferIndex, indexer);
    const nautilus::val<T> nautilusValue = *getMemberWithOffset<T>(parseResult, offsetof(ParseResultFixed<T>, value));
...
parseJsonFixedSizeIntoVarValProxy(const FieldIndex fieldIndex, RawBufferIndex* rawBufferIndex, const InputFormatIndexer* indexer)
{
    ...
    /// landing pads and would skip the traced `destruct` of the result buffer), invokeGuarded catches the throw,
...
ParsedResultVariableSized parseJsonVarSizedProxy(FieldIndex fieldIndex, RawBufferIndex* rawBufferIndex, const InputFormatIndexer* indexer)
{
    ...
    /// and would skip the traced `destruct` of the result buffer), invokeGuarded catches the throw, parks it, and
    ...
    const bool nullable)
{
    if (nullable)
    {
        const auto varSizedResult
            = nautilus::invokeGuarded<&parseJsonVarSizedProxy<true>, FieldIndex, RawBufferIndex*, const InputFormatIndexer*>(
                fieldIndex, rawBufferIndex, indexer);
    ...
    const auto varSizedResult
        = nautilus::invokeGuarded<&parseJsonVarSizedProxy<false>, FieldIndex, RawBufferIndex*, const InputFormatIndexer*>(
            fieldIndex, rawBufferIndex, indexer);
    const VariableSizedData varSizedString{
        *getMemberWithOffset<int8_t*>(varSizedResult, offsetof(ParsedResultVariableSized, varSizedPointer)),
        *getMemberWithOffset<uint64_t>(varSizedResult, offsetof(ParsedResultVariableSized, size))};
