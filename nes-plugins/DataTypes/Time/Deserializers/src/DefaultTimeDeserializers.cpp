    ...
    ArenaRef& arena) const
{
    ...
    {
        const auto truncatedField = nautilus::invoke(DefaultTimeDeserializer::truncateTrailingSpaces, fieldAddress, fieldSize);
        trueFieldAddress = *getMemberWithOffset<int8_t*>(truncatedField, offsetof(DefaultTimeDeserializer::TruncateSpacesResult, ptr));
        trueFieldSize = *getMemberWithOffset<uint64_t>(truncatedField, offsetof(DefaultTimeDeserializer::TruncateSpacesResult, size));
    }
