...
void printBuffer(std::vector<Operation::BasicType> types, uint64_t numTuples, int8_t* bufferPointer) {
                        ...
                        ->addThenBlock(
                            createBB("loopBodyBB", 2, "iOp")
                                ...
                                ->addOperation(make_shared<IfOperation>("loopBodyIfCompareOp",
                                                                        std::vector<std::string>{"iOp", "loadValOp"},
                                                                        std::vector<std::string>{"iOp", "loadValOp"}))
                                ->addThenBlock(
                                    createBB("thenBB", 3, "iOp", "loadValOp")
                                        ->addOperation(std::make_shared<ConstantIntOperation>("thenAddConstOp", 10, 64))
                                        ->addOperation(
                                            std::make_shared<AddIntOperation>("thenAddOp", "loadValOp", "thenAddConstOp"))
                                        ->addOperation(
