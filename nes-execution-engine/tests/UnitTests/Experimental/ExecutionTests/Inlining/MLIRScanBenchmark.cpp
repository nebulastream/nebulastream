...
class MLIRScanBenchmark : public testing::Test {
  public:
...
TEST_F(MLIRScanBenchmark, mlirDoubleColumnAggregation) {
    ...
    auto outBufferMemRef = new MemRefDescriptor<int64_t, 1>{int64OutBufferPtr, int64OutBufferPtr, 0, {static_cast<intptr_t>(buffer.getNumberOfTuples())}, {1}};

    //==------------------------------------------------------==//
    //==-------------- Scan Bench UNOPTIMIZED ----------------==//
    //==-----------------------------------------------------==//
    ...
    for(int i = 0; i < NUM_ITERATIONS; ++i) {
        ...
        timer.start();
        function(buffer.getNumberOfTuples(), bufferMemRef, outBufferMemRef);
        timer.pause();
        executionTimeSum += timer.getPrintTime();
    }
    std::cout << "\n\n--------------------------------------\n";
    std::cout << "Scan Output Buffer at n-1: " << outBuffer.getBuffer<int64_t>()[buffer.getNumberOfTuples()-1] << '\n';
    std::cout << "Scan Unoptimized Total Time Sum: " << executionTimeSum << '\n';
    std::cout << "Scan Unoptimized Execution Time Average: " << executionTimeSum / (double)NUM_ITERATIONS << '\n';
    ...
    modulePath ="/home/alepping/MLIRCode/loadAddStore.mlir"; 
    ...
    auto functionO3 = (void(*)(int64_t, MemRefDescriptor<long, 1> *, MemRefDescriptor<long, 1> *)) engine->lookup("_mlir_ciface_execute").get();
    // Execute function
    executionTimeSum = 0.0;
    for(int i = 0; i < NUM_ITERATIONS; ++i) {
        Timer timer("MLIR Benchmark Timer." + std::to_string(i));
        timer.start();
        functionO3(buffer.getNumberOfTuples(), bufferMemRef, outBufferMemRef);
        timer.pause();
        executionTimeSum += timer.getPrintTime();
    }
    std::cout << "\n\n--------------------------------------\n";
    std::cout << "Scan Output Buffer at n-1: " << outBuffer.getBuffer<int64_t>()[buffer.getNumberOfTuples()-1] << '\n';
    std::cout << "Scan LLVM O3 Total Time Sum: " << executionTimeSum << '\n';
    std::cout << "Scan LLVM O3 Execution Time Average: " << executionTimeSum / (double)NUM_ITERATIONS << '\n';


    // ==-----------------------------------------------------------------------==//
    // ==------------ Scan Bench Optimization + Auto Vectorization -------------==//
    // ==-----------------------------------------------------------------------==//
    mlir::MLIRContext context3;
    modulePath ="/home/alepping/MLIRCode/autoVecUnrollScan.mlir"; 
    ...
    auto function3 = (void(*)(int64_t, MemRefDescriptor<long, 1> *, MemRefDescriptor<long, 1> *)) engine->lookup("_mlir_ciface_execute").get();
    // Execute function
    executionTimeSum = 0.0;
    for(int i = 0; i < NUM_ITERATIONS; ++i) {
        Timer timer("MLIR Benchmark Timer." + std::to_string(i));
        timer.start();
        function3(buffer.getNumberOfTuples(), bufferMemRef, outBufferMemRef);
        timer.pause();
        executionTimeSum += timer.getPrintTime();
    }
    std::cout << "\n\n--------------------------------------\n";
    std::cout << "Scan Output Buffer at n-1: " << outBuffer.getBuffer<int64_t>()[buffer.getNumberOfTuples()-1] << '\n';
    std::cout << "Scan Optimization + Auto Vectorized Total Time Sum: " << executionTimeSum << '\n';
    std::cout << "Scan Optimization + Auto Vectorized Execution Time Average: " << executionTimeSum / (double)NUM_ITERATIONS << '\n';


    //==----------------------------------------------------------==//
    //==------------ Scan Bench Manual Vectorization -------------==//
    //==----------------------------------------------------------==//
    mlir::MLIRContext context2;
    modulePath ="/home/alepping/MLIRCode/loadAddStoreOptimized.mlir";
    module = Backends::MLIR::MLIRUtility::loadMLIRModuleFromFilePath(modulePath, context2);
    engine = Backends::MLIR::MLIRUtility::lowerAndCompileMLIRModuleToMachineCode(module, Nautilus::Backends::MLIR::LLVMIROptimizer::None);
    auto function2 = (void(*)(int64_t, MemRefDescriptor<long, 1> *, MemRefDescriptor<long, 1> *)) engine->lookup("_mlir_ciface_execute").get();
    ...
    for(int i = 0; i < NUM_ITERATIONS; ++i) {
        Timer timer("MLIR Benchmark Timer." + std::to_string(i));
        timer.start();
        function2(buffer.getNumberOfTuples(), bufferMemRef, outBufferMemRef);
        timer.pause();
        executionTimeSum += timer.getPrintTime();
    }
    std::cout << "\n\n--------------------------------------\n";
    std::cout << "Scan Output Buffer at n-1: " << outBuffer.getBuffer<int64_t>()[buffer.getNumberOfTuples()-1] << '\n';
    std::cout << "Scan Manually Vectorized Total Time Sum: " << executionTimeSum << '\n';
    std::cout << "Scan Manually Vectorized Execution Time Average: " << executionTimeSum / (double)NUM_ITERATIONS << '\n';
}
}