...
//==-- UTILITY FUNCTIONS --==//
//==-----------------------==//
mlir::Type MLIRGenerator::getMLIRType(IR::Types::StampPtr type) {
    if (type->isVoid()) {
...
mlir::Value MLIRGenerator::getConstInt(const std::string& location, IR::Types::StampPtr stamp, int64_t value) {
    auto type = getMLIRType(stamp);
    auto constOp = builder->create<mlir::arith::ConstantOp>(getNameLoc(location), type, builder->getIntegerAttr(type, value));
...
mlir::Value MLIRGenerator::getConstBool(const std::string& location, bool value) {
...
void MLIRGenerator::generateMLIR(std::shared_ptr<IR::Operations::CastOperation> castOperation, MLIRGenerator::ValueFrame& frame) {
...
void MLIRGenerator::generateMLIR(std::shared_ptr<IR::Operations::ConstBooleanOperation> constBooleanOp,
                                 MLIRGenerator::ValueFrame& frame) {
    auto constOp =
