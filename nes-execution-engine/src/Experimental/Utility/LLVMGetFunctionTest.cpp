...
int main(int argc, char **argv) {
    ...
    for(auto func = LLVMModule->getFunctionList().begin(); func != LLVMModule->getFunctionList().end(); func++) {
        unmangledToMangledNamesMap.emplace(std::pair<std::string, std::string>{demangleToBaseName(func->getName().str()), func->getName().str()});
    }

    // Get functions from LLVM IR module. Use mapping to mangled function names, if needed.
    std::vector<std::string> ExtractFuncs{"getGlobalOperatorStateProxy", "getNumberOfTuples", "setNumberOfTuples"}; //getDataBuffer
    for (size_t i = 0; i != ExtractFuncs.size(); ++i) {
        ...
        if (!GV) {
            ...
            if (!GV) {
                errs() << "Program doesn't contain function named: '"
                    << ExtractFuncs[i] << "'!\n";
                return 1;
            } 
        }
