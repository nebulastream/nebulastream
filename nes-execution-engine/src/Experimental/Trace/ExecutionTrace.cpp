...
void ExecutionTrace::addOperation(Operation& operation) {
    if (blocks.empty()) {
        createBlock();
    }
...
*/
    ...
    oldBlock.operations.emplace_back(Operation(JMP, ValueRef(0, 0, IR::Operations::VOID), {oldBlockRef}));
    auto operation = Operation(JMP, ValueRef(0, 0, IR::Operations::VOID), {BlockRef(mergedBlockId)});
