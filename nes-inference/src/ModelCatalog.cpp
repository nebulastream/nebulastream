...
void ModelCatalog::registerModel(std::string name, std::filesystem::path path, ModelSchema schema)
{
    ...
    const auto validateSide =
        [&](const ModelFieldList& fields, const std::vector<size_t>& tensorShape, TensorElementType tensorElementType, std::string_view role)
    {
        ...
        for (const auto& field : fields)
        {
            ...
            if (type != expectedFieldType && type != DataType::Type::VARSIZED)
            {
                throw NES::CannotLoadModel(
                    "Model '{}' {} field '{}': type must be {} or VARSIZED", name, role, field.getFullyQualifiedName(), expectedDataType);
            }
