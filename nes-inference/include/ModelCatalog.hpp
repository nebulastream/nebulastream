...
class ModelCatalog
{
    ...
    std::unordered_map<std::string, NES::CLI::Model> registeredModels;

public:
    void registerModel(const NES::CLI::Model& model);
    Model load(const std::string Model) const;
};

inline void ModelCatalog::registerModel(const NES::CLI::Model& model)
{
        ...
        if (result)
        {
            ...
            result->outputs = it->second.outputs
                | std::views::transform([](const auto& schemaField)
                                        { return std::pair<std::string, std::shared_ptr<DataType>>{schemaField.name, schemaField.type}; })
