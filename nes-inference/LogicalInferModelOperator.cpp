...
std::string LogicalInferModelOperator::explain(ExplainVerbosity verbosity, OperatorId id) const
{
    return fmt::format(
...
void LogicalInferModelOperator::serialize(SerializableOperator& serializableOperator) const
{
...
LogicalInferModelOperator LogicalInferModelOperator::withInferredSchema(std::vector<Schema> inputSchemas) const
{
...
NES::LogicalOperatorGeneratedRegistrar::RegisterInferenceModelLogicalOperator(NES::LogicalOperatorRegistryArguments arguments)
{
    ...
    return logicalOperator.setInputSchema(arguments.inputSchemas).setOutputSchema(arguments.outputSchema);
}
