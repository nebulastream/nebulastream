...
TEST_F(QueryPlanTest, testHasOperator) {
  ...
  LogicalOperatorPtr op1 = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create("test_stream"));
...
TEST_F(QueryPlanTest, testLeafOperators) {
  ...
  EXPECT_TRUE(std::find(leafOptrs.begin(), leafOptrs.end(), op1) !=
              leafOptrs.end());
}
