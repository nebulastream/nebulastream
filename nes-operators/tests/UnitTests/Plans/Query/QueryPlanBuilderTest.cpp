...
TEST_F(QueryPlanBuilderTest, testHasOperator) {
    ...
    EXPECT_TRUE(queryPlan->getOperatorByType<LogicalRenameSourceOperator>().size() == 1);
    EXPECT_EQ(queryPlan->getOperatorByType<LogicalRenameSourceOperator>()[0]->getNewSourceName(), "testStream");
    //test addFilter
    ...
    EXPECT_TRUE(queryPlan->getOperatorByType<LogicalSinkOperator>().size() == 1);
    EXPECT_TRUE(queryPlan->getOperatorByType<LogicalSinkOperator>()[0]->getSinkDescriptor()->equal(sinkDescriptorPtr));
}
