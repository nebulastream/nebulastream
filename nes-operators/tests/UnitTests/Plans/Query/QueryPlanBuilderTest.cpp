...
class QueryPlanBuilderTest : public Testing::BaseUnitTest {
 public:
  ...
  void SetUp() override {
    NES::Logger::setupLogging("QueryPlanBuilderTest.log",
  ...
  void TearDown() override {
    NES_INFO("Setup QueryPlanBuilderTest test case.");
  }

  /* Will be called after all tests in this class are finished. */
  static void TearDownTestCase() {
    NES_INFO("Tear down QueryPlanBuilderTest test class.");
  }
};
...
TEST_F(QueryPlanBuilderTest, testHasOperator) {
  ...
  EXPECT_EQ(queryPlan->getOperatorByType<RenameSourceOperator>()[0]
                ->getNewSourceName(),
            "testStream");
  // test addFilter
  auto filterExpression = ExpressionNodePtr(
      LessExpressionNode::create(NES::Attribute("a").getExpressionNode(),
                                 NES::Attribute("b").getExpressionNode()));
  queryPlan = QueryPlanBuilder::addFilter(filterExpression, queryPlan);
  EXPECT_TRUE(queryPlan->getOperatorByType<LogicalFilterOperator>().size() ==
              1);
  EXPECT_EQ(
      queryPlan->getOperatorByType<LogicalFilterOperator>()[0]->getPredicate(),
      filterExpression);
  // test addProjection
  ...
  EXPECT_TRUE(
      queryPlan->getOperatorByType<LogicalProjectionOperator>().size() == 1);
  EXPECT_EQ(queryPlan->getOperatorByType<LogicalProjectionOperator>()[0]
                ->getExpressions(),
            expressions);
  // test addMap
  ...
  EXPECT_TRUE(queryPlan->getOperatorByType<SinkLogicalOperator>()[0]
                  ->getSinkDescriptor()
                  ->equal(sinkDescriptorPtr));
}
