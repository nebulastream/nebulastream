...
/**
 * Utility class to create default and non-default Java UDF descriptors for
 * testing.
 *
 * The class JavaUDFDescriptor performs a number of checks in its constructor.
 * Creating the required inputs everytime a test needs a Java UDF descriptor
 * leads to code repetition.
 */
class JavaUDFDescriptorBuilder {
public:
  /**
   * Create a new builder for a JavaUDFDescriptor with valid default values for
   * the fields required by the JavaUDFDescriptor.
   */
  JavaUDFDescriptorBuilder() = default;

  /**
   * @return A Java UDF descriptor with the fields either set to default values
   * or with explicitly specified values in setters.
   */
  ...
  setInstance(const jni::JavaSerializedInstance &newInstance);
   ...
   */
  JavaUDFDescriptorBuilder &
  setByteCodeList(const jni::JavaUDFByteCodeList &newByteCodeList);
   ...
   * @param newInputClassName The class name of the input type of the UDF
   * method.
   * @return The JavaUDFDescriptorBuilder instance.
   */
  JavaUDFDescriptorBuilder &
  setInputClassName(std::string_view newInputClassName);

  /**
   * Set the class name of the return type of the UDF method.
   * @param newOutputClassName The class name of the return type of the UDF
   * method.
   * @return The JavaUDFDescriptorBuilder instance.
   */
  JavaUDFDescriptorBuilder &
  setOutputClassName(std::string_view newOutputClassName);
...
private:
  ...
  jni::JavaSerializedInstance instance =
      jni::JavaSerializedInstance{1}; // byte-array containing 1 byte
  jni::JavaUDFByteCodeList byteCodeList =
      jni::JavaUDFByteCodeList{{"some_package.my_udf"s, jni::JavaByteCode{1}}};
