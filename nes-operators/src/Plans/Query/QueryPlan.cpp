...
std::vector<OperatorPtr> QueryPlan::getLeafOperators() const {
  ...
  for (const auto& rootOperator : rootOperators) {
    ...
    for (auto itr = bfsIterator.begin();
      ...
      NES_DEBUG(
          "QueryPlan: Inserting operator in collection of already visited "
          "node.");
      visitedOpIds.insert(visitingOp->getId());
      if (visitingOp->getChildren().empty()) {
        NES_DEBUG(
            "QueryPlan: Found leaf node. Adding to the collection of leaf "
            "nodes.");
...
OperatorPtr QueryPlan::getOperatorWithOperatorId(OperatorId operatorId) const {
  NES_DEBUG(
      "QueryPlan: Checking if the operator with id {} exists in the query plan "
      "or not",
      operatorId);
...
OperatorPtr QueryPlan::getOperatorWithStatisticId(
    StatisticId statisticId) const {
  NES_DEBUG(
      "QueryPlan: Checking if the operator with statisticId {} exists in the "
      "query plan or not",
      statisticId);
  for (auto rootOperator : rootOperators) {
...
void QueryPlan::removeAsRootOperator(OperatorPtr root) {
  ...
  if (found != rootOperators.end()) {
    NES_DEBUG(
        "QueryPlan: Found root operator in the root operator list. Removing "
        "the operator as the root of the query plan.",
        root->toString());
  ...
  while (!operatorsToProcess.empty()) {
    ...
    for (const auto& parentNode : operatorNode->getParents()) {
      ...
      if (operatorIdToOperatorMap[parentOperatorId]) {
        NES_TRACE(
            "QueryPlan: Found the parent operator. Adding as parent to the "
            "current operator.");
        ...
        NES_ERROR(
            "QueryPlan: unable to find the parent operator. This should not "
            "have occurred!");
        return nullptr;
      }
    }
