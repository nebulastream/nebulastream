...
QueryPlanPtr QueryPlan::create(QueryId queryId,
  ...
  return std::make_shared<QueryPlan>(
      QueryPlan(queryId, std::move(rootOperators)));
}
...
std::vector<SourceLogicalOperatorPtr> QueryPlan::getSourceOperators() const {
  NES_DEBUG(
      "QueryPlan: Get all source operators by traversing all the root nodes.");
  ...
  std::vector<SourceLogicalOperatorPtr> sourceOperators{
      sourceOperatorsSet.begin(), sourceOperatorsSet.end()};
  return sourceOperators;
}

std::vector<SinkLogicalOperatorPtr> QueryPlan::getSinkOperators() const {
  NES_DEBUG(
      "QueryPlan: Get all sink operators by traversing all the root nodes.");
...
void QueryPlan::appendOperatorAsNewRoot(const OperatorPtr &operatorNode) {
  NES_DEBUG("QueryPlan: Appending operator {} as new root of the plan.",
            operatorNode->toString());
  for (const auto &rootOperator : rootOperators) {
    if (!rootOperator->addParent(operatorNode)) {
      NES_THROW_RUNTIME_ERROR("QueryPlan: Unable to add operator " +
                              operatorNode->toString() + " as parent to " +
...
std::vector<OperatorPtr> QueryPlan::getLeafOperators() const {
  ...
  for (const auto &rootOperator : rootOperators) {
    ...
    for (auto itr = bfsIterator.begin();
      ...
      NES_DEBUG("QueryPlan: Inserting operator in collection of already "
                "visited node.");
      visitedOpIds.insert(visitingOp->getId());
      if (visitingOp->getChildren().empty()) {
        NES_DEBUG("QueryPlan: Found leaf node. Adding to the collection of "
                  "leaf nodes.");
...
bool QueryPlan::hasOperatorWithId(OperatorId operatorId) {
  NES_DEBUG(
      "QueryPlan: Checking if the operator exists in the query plan or not");
...
OperatorPtr QueryPlan::getOperatorWithOperatorId(OperatorId operatorId) const {
  NES_DEBUG("QueryPlan: Checking if the operator with id {} exists in the "
            "query plan or not",
            operatorId);
...
OperatorPtr
QueryPlan::getOperatorWithStatisticId(StatisticId statisticId) const {
  NES_DEBUG("QueryPlan: Checking if the operator with statisticId {} exists in "
            "the query plan or not",
            statisticId);
  for (auto rootOperator : rootOperators) {
    if (rootOperator->getStatisticId() == statisticId) {
      NES_DEBUG("QueryPlan: Found operator {} in the query plan",
                rootOperator->toString());
      return rootOperator;
    }
    ...
    if (matchingOperator) {
      NES_DEBUG("QueryPlan: Found operator {} in the query plan",
                matchingOperator->toString());
      return matchingOperator->as<Operator>();
    }
  }
  NES_WARNING("QueryPlan: Unable to find operator with matching statisticId {}",
              statisticId);
  return nullptr;
}
...
void QueryPlan::addRootOperator(const OperatorPtr &newRootOperator) {
  ...
  auto found = std::find_if(rootOperators.begin(), rootOperators.end(),
                            [&](const OperatorPtr &root) {
  ...
  if (found == rootOperators.end()) {
    ...
    NES_WARNING("Root operator with id {} already present int he plan",
                newRootOperator->getId());
  }
}

void QueryPlan::removeAsRootOperator(OperatorPtr root) {
  NES_DEBUG("QueryPlan: removing operator {} as root operator.",
            root->toString());
  auto found = std::find_if(rootOperators.begin(), rootOperators.end(),
                            [&](const OperatorPtr &rootOperator) {
                              return rootOperator->getId() == root->getId();
                            });
  if (found != rootOperators.end()) {
    NES_DEBUG("QueryPlan: Found root operator in the root operator list. "
              "Removing the operator as the root of the query plan.",
              root->toString());
...
QueryPlanPtr QueryPlan::copy() {
  ...
  // 1. We start by copying the root operators of this query plan to the queue
  // of operators to be processed
  std::map<OperatorId, OperatorPtr> operatorIdToOperatorMap;
  std::deque<NodePtr> operatorsToProcess{rootOperators.begin(),
                                         rootOperators.end()};
  while (!operatorsToProcess.empty()) {
    ...
    // 2. We add each non existing operator to a map and skip adding the
    // operator that already exists in the map.
    // 3. We use the already existing operator whenever available other wise we
    // create a copy of the operator and add it to the map.
    ...
    // 4. We then check the parent operators of the current operator by looking
    // into the map and add them as the parent of the current operator.
    for (const auto &parentNode : operatorNode->getParents()) {
      ...
      if (operatorIdToOperatorMap[parentOperatorId]) {
        NES_TRACE("QueryPlan: Found the parent operator. Adding as parent to "
        ...
        NES_ERROR("QueryPlan: unable to find the parent operator. This should "
                  "not have occurred!");
        return nullptr;
      }
    }
    ...
    NES_TRACE(
        "QueryPlan: add the child global query nodes for further processing.");
    // 5. We push the children operators to the queue of operators to be
    // processed.
  ...
  for (const auto &rootOperator : rootOperators) {
    ...
    duplicateRootOperators.push_back(
        operatorIdToOperatorMap[rootOperator->getId()]);
  }
...
void QueryPlan::setPlacementStrategy(
    Optimizer::PlacementStrategy placementStrategy) {
  this->placementStrategy = placementStrategy;
}

std::set<OperatorPtr> QueryPlan::findAllOperatorsBetween(
    const std::set<OperatorPtr> &downstreamOperators,
    const std::set<OperatorPtr> &upstreamOperators) {

  std::set<OperatorPtr> operatorsBetween;

  // initialize the operators to visit with upstream operators of all downstream
  // operators
  for (const auto &downStreamOperator : downstreamOperators) {
    auto operatorsBetweenChildAndTargetUpstream =
        findOperatorsBetweenSourceAndTargetOperators(
            downStreamOperator->as_if<Operator>(), upstreamOperators);
  ...
  if (!operatorsBetween.empty()) {
    ...
    for (const auto &upstreamOperator : upstreamOperators) {
      erase_if(operatorsBetween,
               [upstreamOperator](const auto &operatorToErase) {
    ...
    for (const auto &downstreamOperator : downstreamOperators) {
      erase_if(operatorsBetween,
               [downstreamOperator](const auto &operatorToErase) {
...
std::set<OperatorPtr> QueryPlan::findOperatorsBetweenSourceAndTargetOperators(
  ...
  auto found = std::find_if(targetOperators.begin(), targetOperators.end(),
                            [&](const auto &upstreamOperator) {
                              return upstreamOperator->getId() ==
                                     sourceOperator->getId();
                            });

  // If downstream operator is in the list of target operators then return the
  // operator
  ...
  for (const auto &nextUpstreamOperatorToCheck :
       sourceOperator->getChildren()) {

    // Fetch the operators between upstream and target operators
    auto operatorsBetweenUpstreamAndTargetUpstream =
        findOperatorsBetweenSourceAndTargetOperators(
            nextUpstreamOperatorToCheck->as_if<Operator>(), targetOperators);

    // If there are operators between upstream and target operators then mark
    // the input down stream operator for return
