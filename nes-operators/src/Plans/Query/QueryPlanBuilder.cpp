...
QueryPlanPtr QueryPlanBuilder::createQueryPlan(std::string sourceName) {
  NES_DEBUG("QueryPlanBuilder: create query plan for input source  {}",
            sourceName);
  auto sourceOperator = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create(sourceName));
  auto queryPlanPtr = QueryPlan::create(sourceOperator);
  queryPlanPtr->setSourceConsumed(sourceName);
  return queryPlanPtr;
}

QueryPlanPtr QueryPlanBuilder::addProjection(
    const std::vector<ExpressionNodePtr> &expressions, QueryPlanPtr queryPlan) {
  NES_DEBUG("QueryPlanBuilder: add projection operator to query plan");
  OperatorPtr op =
      LogicalOperatorFactory::createProjectionOperator(expressions);
  queryPlan->appendOperatorAsNewRoot(op);
  return queryPlan;
}

QueryPlanPtr QueryPlanBuilder::addRename(std::string const &newSourceName,
...
QueryPlanPtr
QueryPlanBuilder::addFilter(ExpressionNodePtr const &filterExpression,
                            QueryPlanPtr queryPlan) {
  ...
  if (!filterExpression->getNodesByType<FieldRenameExpressionNode>().empty()) {
    NES_THROW_RUNTIME_ERROR("QueryPlanBuilder: Filter predicate cannot have a "
                            "FieldRenameExpression");
  }
  OperatorPtr op =
      LogicalOperatorFactory::createFilterOperator(filterExpression);
  queryPlan->appendOperatorAsNewRoot(op);
  return queryPlan;
}

QueryPlanPtr QueryPlanBuilder::addLimit(const uint64_t limit,
                                        QueryPlanPtr queryPlan) {
...
QueryPlanPtr
QueryPlanBuilder::addMapUDF(Catalogs::UDF::UDFDescriptorPtr const &descriptor,
                            QueryPlanPtr queryPlan) {
...
QueryPlanPtr QueryPlanBuilder::addFlatMapUDF(
    Catalogs::UDF::UDFDescriptorPtr const &descriptor, QueryPlanPtr queryPlan) {
...
QueryPlanPtr
QueryPlanBuilder::addMap(FieldAssignmentExpressionNodePtr const &mapExpression,
                         QueryPlanPtr queryPlan) {
  ...
  if (!mapExpression->getNodesByType<FieldRenameExpressionNode>().empty()) {
    NES_THROW_RUNTIME_ERROR(
        "QueryPlanBuilder: Map expression cannot have a FieldRenameExpression");
  }
...
QueryPlanPtr QueryPlanBuilder::addUnion(QueryPlanPtr leftQueryPlan,
  ...
  leftQueryPlan =
      addBinaryOperatorAndUpdateSource(op, leftQueryPlan, rightQueryPlan);
  return leftQueryPlan;
}

QueryPlanPtr QueryPlanBuilder::addStatisticBuildOperator(
  ...
  auto op = LogicalOperatorFactory::createStatisticBuildOperator(
      window, std::move(statisticDescriptor), metricHash, sendingPolicy,
      triggerCondition);
  queryPlan->appendOperatorAsNewRoot(op);
  return queryPlan;
}
...
QueryPlanPtr QueryPlanBuilder::addJoin(
    QueryPlanPtr leftQueryPlan, QueryPlanPtr rightQueryPlan,
  ...
  NES_DEBUG(
      "QueryPlanBuilder: Iterate over all ExpressionNode to check join field.");
  ...
  for (auto itr = bfsIterator.begin(); itr != BreadthFirstNodeIterator::end();
    ...
    if ((*itr)->instanceOf<BinaryExpressionNode>()) {
      ...
      if (visitedExpressions.contains(visitingOp)) {
        ...
        NES_DEBUG(
            "QueryPlanBuilder: Check if Expressions are FieldExpressions.");
        auto leftKeyFieldAccess = checkExpression(onLeftKey, "leftSide");
        auto rightQueryPlanKeyFieldAccess =
            checkExpression(onRightKey, "rightSide");
      }
    }
  }

  NES_ASSERT(rightQueryPlan && !rightQueryPlan->getRootOperators().empty(),
             "invalid rightQueryPlan query plan");
  ...
  // TODO 1,1 should be replaced once we have distributed joins with the number
  // of child input edges
  // TODO(Ventura?>Steffen) can we know this at this query submission time?
  auto joinDefinition = Join::LogicalJoinDescriptor::create(
      joinExpression, windowType, 1, 1, joinType);
...
QueryPlanPtr QueryPlanBuilder::addBatchJoin(QueryPlanPtr leftQueryPlan,
  ...
  auto joinDefinition = Join::Experimental::LogicalBatchJoinDescriptor::create(
      buildKeyFieldAccess, probeKeyFieldAccess, 1, 1);
...
QueryPlanPtr QueryPlanBuilder::addSink(QueryPlanPtr queryPlan,
                                       SinkDescriptorPtr sinkDescriptor,
                                       WorkerId workerId) {
  OperatorPtr op =
      LogicalOperatorFactory::createSinkOperator(sinkDescriptor, workerId);
  queryPlan->appendOperatorAsNewRoot(op);
  return queryPlan;
}

QueryPlanPtr QueryPlanBuilder::assignWatermark(
    QueryPlanPtr queryPlan, Windowing::WatermarkStrategyDescriptorPtr const
                                &watermarkStrategyDescriptor) {
  OperatorPtr op = LogicalOperatorFactory::createWatermarkAssignerOperator(
      watermarkStrategyDescriptor);
  queryPlan->appendOperatorAsNewRoot(op);
  return queryPlan;
}

QueryPlanPtr QueryPlanBuilder::checkAndAddWatermarkAssignment(
    QueryPlanPtr queryPlan, const Windowing::WindowTypePtr windowType) {
  NES_DEBUG(
      "QueryPlanBuilder: checkAndAddWatermarkAssignment for a (sub)query plan");
  auto timeBasedWindowType = windowType->as<Windowing::TimeBasedWindowType>();

  if (queryPlan->getOperatorByType<WatermarkAssignerLogicalOperator>()
          .empty()) {
...
QueryPlanPtr QueryPlanBuilder::addBinaryOperatorAndUpdateSource(
  ...
  NES_DEBUG("QueryPlanBuilder: addBinaryOperatorAndUpdateSource: update the "
            "source names");
  auto newSourceName = Util::updateSourceName(
      leftQueryPlan->getSourceConsumed(), rightQueryPlan->getSourceConsumed());
  leftQueryPlan->setSourceConsumed(newSourceName);
  return leftQueryPlan;
}

std::shared_ptr<FieldAccessExpressionNode>
QueryPlanBuilder::checkExpression(ExpressionNodePtr expression,
                                  std::string side) {
  if (!expression->instanceOf<FieldAccessExpressionNode>()) {
    NES_ERROR("QueryPlanBuilder: window key ({}) has to be an "
              "FieldAccessExpression but it was a  {}",
              side, expression->toString());
    NES_THROW_RUNTIME_ERROR(
        "QueryPlanBuilder: window key has to be an FieldAccessExpression");
  }
