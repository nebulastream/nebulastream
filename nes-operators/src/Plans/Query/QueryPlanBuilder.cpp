...
std::shared_ptr<QueryPlan>
QueryPlanBuilder::addSelection(const std::shared_ptr<LogicalFunction>& selectionFunction, std::shared_ptr<QueryPlan> queryPlan)
{
...
std::shared_ptr<QueryPlan>
QueryPlanBuilder::addMap(const std::shared_ptr<FieldAssignmentLogicalFunction>& mapFunction, std::shared_ptr<QueryPlan> queryPlan)
{
...
std::shared_ptr<QueryPlan> QueryPlanBuilder::addUnion(std::shared_ptr<QueryPlan> leftQueryPlan, std::shared_ptr<QueryPlan> rightQueryPlan)
{
    ...
    const std::shared_ptr<Operator> op = std::make_shared<UnionLogicalOperator>();
    ...
    JoinLogicalOperator::JoinType joinType = JoinLogicalOperator::JoinType::CARTESIAN_PRODUCT)
{
    ...
    auto leftJoinType = dynamic_cast<LogicalOperator*>(leftQueryPlan->getRootOperators()[0].get())->getOutputSchema();
    auto rightQueryPlanJoinType = dynamic_cast<LogicalOperator*>(rootOperatorRhs.get())->getOutputSchema();
    ...
    auto op = std::make_shared<JoinLogicalOperator>(joinFunction, windowType, 1, 1, joinType);
    leftQueryPlan = addBinaryOperatorAndUpdateSource(op, leftQueryPlan, rightQueryPlan);
    return leftQueryPlan;
}
