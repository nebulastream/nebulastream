...
QueryPlan QueryPlanBuilder::addProjection(std::vector<LogicalFunction> functions, QueryPlan queryPlan)
{
    ...
    JoinLogicalOperator::JoinType joinType = JoinLogicalOperator::JoinType::CARTESIAN_PRODUCT)
{
    ...
    leftQueryPlan = addBinaryOperatorAndUpdateSource(
        JoinLogicalOperator(joinFunction, std::move(windowType), 1, 1, joinType), leftQueryPlan, rightQueryPlan);
    return leftQueryPlan;
}
...
QueryPlan
QueryPlanBuilder::addBinaryOperatorAndUpdateSource(LogicalOperator operatorNode, QueryPlan leftQueryPlan, QueryPlan rightQueryPlan)
{
