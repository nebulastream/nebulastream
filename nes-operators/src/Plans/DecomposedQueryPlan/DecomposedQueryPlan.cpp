...
DecomposedQueryPlanPtr DecomposedQueryPlan::create(
...
bool DecomposedQueryPlan::removeAsRootOperator(OperatorId rootOperatorId) {
  ...
  if (found == rootOperators.end()) {
    NES_WARNING(
        "Unable to locate a root operator with id {}. Skipping remove root "
        "operator operation.",
        rootOperatorId);
    return false;
  }
...
std::vector<SourceLogicalOperatorPtr> DecomposedQueryPlan::getSourceOperators()
    const {
...
std::vector<SinkLogicalOperatorPtr> DecomposedQueryPlan::getSinkOperators()
    const {
...
OperatorPtr DecomposedQueryPlan::getOperatorWithOperatorId(
    OperatorId operatorId) const {
  NES_DEBUG(
  ...
  while (!operatorsToProcess.empty()) {
    ...
    for (const auto& parentNode : operatorNode->getParents()) {
      ...
      if (operatorIdToOperatorMap.contains(parentOperatorId)) {
        NES_TRACE(
            "DecomposedQueryPlan: Found the parent operator. Adding as parent "
        ...
        NES_ASSERT(false,
                   "DecomposedQueryPlan: Copying the plan failed because "
                   "parent operator not found.");
      }
    }

    NES_TRACE(
        "DecomposedQueryPlan: add the child global query nodes for further "
        "processing.");
