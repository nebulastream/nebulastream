...
DecomposedQueryPlan::create(DecomposedQueryPlanId decomposedQueryPlanId,
                            SharedQueryId sharedQueryId, WorkerId workerId) {
  return std::make_shared<DecomposedQueryPlan>(decomposedQueryPlanId,
...
DecomposedQueryPlan::create(DecomposedQueryPlanId decomposedQueryPlanId,
  ...
  return std::make_shared<DecomposedQueryPlan>(
      decomposedQueryPlanId, sharedQueryId, workerId, rootOperators);
}
...
DecomposedQueryPlan::DecomposedQueryPlan(
    ...
    : sharedQueryId(sharedQueryId),
      decomposedQueryPlanId(decomposedQueryPlanId), workerId(workerId) {}
...
bool DecomposedQueryPlan::removeAsRootOperator(OperatorId rootOperatorId) {
  ...
  if (found == rootOperators.end()) {
    NES_WARNING("Unable to locate a root operator with id {}. Skipping remove "
                "root operator operation.",
                rootOperatorId);
    return false;
  }
...
DecomposedQueryPlanId DecomposedQueryPlan::getDecomposedQueryPlanId() const {
  return decomposedQueryPlanId;
}

SharedQueryId DecomposedQueryPlan::getSharedQueryId() const {
  return sharedQueryId;
}

void DecomposedQueryPlan::setDecomposedQueryPlanId(
    DecomposedQueryPlanId newDecomposedQueryPlanId) {
  decomposedQueryPlanId = newDecomposedQueryPlanId;
}

std::vector<SourceLogicalOperatorPtr>
DecomposedQueryPlan::getSourceOperators() const {
  ...
  for (const auto &rootOperator : rootOperators) {
    auto sourceOperators =
        rootOperator->getNodesByType<SourceLogicalOperator>();
...
std::vector<SinkLogicalOperatorPtr>
DecomposedQueryPlan::getSinkOperators() const {
...
void DecomposedQueryPlan::setState(QueryState newState) {
  currentState = newState;
}

DecomposedQueryPlanVersion DecomposedQueryPlan::getVersion() const {
  return decomposedQueryPlanVersion;
}

void DecomposedQueryPlan::setVersion(DecomposedQueryPlanVersion newVersion) {
...
OperatorPtr
DecomposedQueryPlan::getOperatorWithOperatorId(OperatorId operatorId) const {
  NES_DEBUG(
      "Checking if the operator with id {} exists in the query plan or not",
      operatorId);
...
bool DecomposedQueryPlan::replaceRootOperator(const OperatorPtr &oldRoot,
...
void DecomposedQueryPlan::appendOperatorAsNewRoot(
...
DecomposedQueryPlanPtr DecomposedQueryPlan::copy() {
  ...
  while (!operatorsToProcess.empty()) {
    ...
    for (const auto &parentNode : operatorNode->getParents()) {
      ...
      if (operatorIdToOperatorMap.contains(parentOperatorId)) {
        NES_TRACE("DecomposedQueryPlan: Found the parent operator. Adding as "
                  "parent to the current operator.");
        ...
        NES_ASSERT(false, "DecomposedQueryPlan: Copying the plan failed "
                          "because parent operator not found.");
      }
    }

    NES_TRACE("DecomposedQueryPlan: add the child global query nodes for "
              "further processing.");
    ...
    for (const auto &childrenOperator : operatorNode->getChildren()) {
      ...
      for (const auto &parentOperator : parentOperators) {
        if (!operatorIdToOperatorMap.contains(
                parentOperator->as<Operator>()->getId())) {
  ...
  for (const auto &rootOperator : rootOperators) {
    NES_TRACE(
        "DecomposedQueryPlan: Finding the operator with same id in the map.");
  ...
  auto copiedDecomposedQueryPlan = DecomposedQueryPlan::create(
      decomposedQueryPlanId, sharedQueryId, workerId, duplicateRootOperators);
