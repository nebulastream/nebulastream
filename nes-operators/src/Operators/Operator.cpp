...
OperatorPtr Operator::getDuplicateOfParent(const OperatorPtr& operatorNode) {
  ...
  if (operatorNode->getParents().empty()) {
    NES_TRACE(
        "Operator: No ancestor of the input node. Returning the copy of the "
        "input operator");
    return copyOfOperator;
  }

  NES_TRACE(
      "Operator: For all parents get copy of the ancestor and add as parent to "
      "the copy of the input operator");
...
OperatorPtr Operator::getDuplicateOfChild(const OperatorPtr& operatorNode) {
  ...
  if (operatorNode->getChildren().empty()) {
    NES_TRACE(
        "Operator: No children of the input node. Returning the copy of the "
        "input operator");
    return copyOfOperator;
  }

  NES_TRACE(
      "Operator: For all children get copy of their children and add as child "
      "to the copy of the input operator");
...
bool Operator::addChild(NodePtr newNode) {
  ...
  NES_TRACE(
      "Operator: the node is already part of its children so skip add child "
      "operation.");
  return false;
}
...
bool Operator::addParent(NodePtr newNode) {
  ...
  NES_TRACE(
      "Operator: the node is already part of its parent so skip add parent "
      "operation.");
  return false;
}
