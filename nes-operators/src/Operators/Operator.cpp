...
Operator::Operator(OperatorId id, StatisticId statisticId)
    : id(id), statisticId(statisticId) {
...
bool Operator::hasMultipleChildrenOrParents() const {
  ...
  bool hasMultipleChildren =
      (!getChildren().empty()) && getChildren().size() > 1;
  // has multiple parent operator
  bool hasMultipleParent = (!getParents().empty()) && getParents().size() > 1;
  NES_DEBUG("Operator: has multiple children {} or has multiple parent {}",
            hasMultipleChildren, hasMultipleParent);
...
OperatorPtr Operator::duplicate() {
  ...
  for (const auto &parent : getParents()) {
    if (!copyOperator->addParent(
            getDuplicateOfParent(parent->as<Operator>()))) {
  ...
  for (const auto &child : getChildren()) {
    if (!copyOperator->addChild(
            getDuplicateOfChild(child->as<Operator>()->duplicate()))) {
...
OperatorPtr Operator::getDuplicateOfParent(const OperatorPtr &operatorNode) {
  ...
  if (operatorNode->getParents().empty()) {
    NES_TRACE("Operator: No ancestor of the input node. Returning the copy of "
              "the input operator");
    return copyOfOperator;
  }

  NES_TRACE("Operator: For all parents get copy of the ancestor and add as "
            "parent to the copy of the input operator");
...
OperatorPtr Operator::getDuplicateOfChild(const OperatorPtr &operatorNode) {
  ...
  if (operatorNode->getChildren().empty()) {
    NES_TRACE("Operator: No children of the input node. Returning the copy of "
              "the input operator");
    return copyOfOperator;
  }

  NES_TRACE("Operator: For all children get copy of their children and add as "
            "child to the copy of the input operator");
...
bool Operator::addChild(NodePtr newNode) {
  ...
  auto found = std::find_if(currentChildren.begin(), currentChildren.end(),
                            [&](const NodePtr &child) {
                              return child->as<Operator>()->getId() ==
                                     newNode->as<Operator>()->getId();
                            });
  ...
  NES_TRACE("Operator: the node is already part of its children so skip add "
            "child operation.");
  return false;
}
...
bool Operator::addParent(NodePtr newNode) {
  ...
  auto found = std::find_if(currentParents.begin(), currentParents.end(),
                            [&](const NodePtr &child) {
                              return child->as<Operator>()->getId() ==
                                     newNode->as<Operator>()->getId();
                            });
  ...
  NES_TRACE("Operator: the node is already part of its parent so skip add "
            "parent operation.");
  return false;
}
...
bool Operator::containAsGrandChild(NodePtr operatorNode) {
  ...
  for (auto &child : children) {
    std::vector<NodePtr> childAndGrandChildren =
        child->getAndFlattenAllChildren(false);
    ancestors.insert(ancestors.end(), childAndGrandChildren.begin(),
                     childAndGrandChildren.end());
  }
  // Check if an operator with the id exists as ancestor
  return std::any_of(ancestors.begin(), ancestors.end(),
                     [operatorIdToCheck](const NodePtr &ancestor) {
                       return ancestor->as<Operator>()->getId() ==
                              operatorIdToCheck;
                     });
}
...
bool Operator::containAsGrandParent(NES::NodePtr operatorNode) {
  ...
  for (const auto &parent : parents) {
    std::vector<NodePtr> parentAndAncestors =
        parent->getAndFlattenAllAncestors();
    ancestors.insert(ancestors.end(), parentAndAncestors.begin(),
                     parentAndAncestors.end());
  }
  // Check if an operator with the id exists as ancestor
  return std::any_of(ancestors.begin(), ancestors.end(),
                     [operatorIdToCheck](const NodePtr &ancestor) {
