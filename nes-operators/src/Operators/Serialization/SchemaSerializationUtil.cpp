...
SerializableSchemaPtr SchemaSerializationUtil::serializeSchema(
    const SchemaPtr& schema, SerializableSchema* serializedSchema) {
  NES_DEBUG("SchemaSerializationUtil:: serialize schema {}",
            schema->toString());
  ...
  for (const auto& field : schema->fields) {
    ...
    DataTypeSerializationUtil::serializeDataType(
        field->getDataType(), serializedField->mutable_type());
  }

  // Serialize layoutType
  if (schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
    serializedSchema->set_layouttype(
        SerializableSchema_MemoryLayoutType_ROW_LAYOUT);
    ...
    serializedSchema->set_layouttype(
        SerializableSchema_MemoryLayoutType_COL_LAYOUT);
...
SchemaPtr SchemaSerializationUtil::deserializeSchema(
    const SerializableSchema& serializedSchema) {
  ...
  for (const auto& serializedField : serializedSchema.fields()) {
    ...
    auto type =
        DataTypeSerializationUtil::deserializeDataType(serializedField.type());
  ...
  switch (serializedSchema.layouttype()) {
    ...
    case SerializableSchema_MemoryLayoutType_COL_LAYOUT: {
      deserializedSchema->setLayoutType(
          Schema::MemoryLayoutType::COLUMNAR_LAYOUT);
    ...
    default: {
      NES_ERROR(
          "SchemaSerializationUtil:: Wrong memory layout in serialization "
          "format");
    }
  }
