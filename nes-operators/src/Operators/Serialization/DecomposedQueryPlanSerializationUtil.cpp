...
void DecomposedQueryPlanSerializationUtil::serializeDecomposedQueryPlan(
  ...
  NES_DEBUG("QueryPlanSerializationUtil: serializing query plan {}",
            decomposedQueryPlan->toString());
  std::vector<OperatorPtr> rootOperators =
      decomposedQueryPlan->getRootOperators();
  NES_DEBUG("QueryPlanSerializationUtil: serializing the operator chain for "
            "each root operator independently");

  // Serialize Query Plan operators
  auto &serializedOperatorMap =
      *serializableDecomposedQueryPlan->mutable_operatormap();
  ...
  for (auto itr = bfsIterator.begin(); itr != PlanIterator::end(); ++itr) {
    ...
    if (serializedOperatorMap.find(visitingOp->getId().getRawValue()) !=
        serializedOperatorMap.end()) {
      // skip rest of the steps as the operator is already serialized
      continue;
    }
    NES_TRACE(
        "QueryPlan: Inserting operator in collection of already visited node.");
    SerializableOperator serializeOperator =
        OperatorSerializationUtil::serializeOperator(visitingOp, false);
    serializedOperatorMap[visitingOp->getId().getRawValue()] =
        serializeOperator;
  }
  ...
  for (const auto &rootOperator : rootOperators) {
    ...
    serializableDecomposedQueryPlan->add_rootoperatorids(
        rootOperatorId.getRawValue());
  }

  // Serialize the sub query plan and query plan id
  NES_TRACE("QueryPlanSerializationUtil: serializing the Query sub plan id and "
            "query id");
  serializableDecomposedQueryPlan->set_decomposedqueryplanid(
      decomposedQueryPlan->getDecomposedQueryPlanId().getRawValue());
  serializableDecomposedQueryPlan->set_sharedqueryplanid(
      decomposedQueryPlan->getSharedQueryId().getRawValue());
  serializableDecomposedQueryPlan->set_state(
      serializeQueryState(decomposedQueryPlan->getState()));
}

DecomposedQueryPlanPtr
DecomposedQueryPlanSerializationUtil::deserializeDecomposedQueryPlan(
    const NES::SerializableDecomposedQueryPlan
        *serializableDecomposedQueryPlan) {
  NES_TRACE("QueryPlanSerializationUtil: Deserializing query plan {}",
            serializableDecomposedQueryPlan->DebugString());
  std::vector<OperatorPtr> rootOperators;
  std::map<uint64_t, OperatorPtr> operatorIdToOperatorMap;

  // Deserialize all operators in the operator map
  for (const auto &operatorIdAndSerializedOperator :
       serializableDecomposedQueryPlan->operatormap()) {
  ...
  // add root operators
  for (auto rootOperatorId :
       serializableDecomposedQueryPlan->rootoperatorids()) {
    rootOperators.emplace_back(operatorIdToOperatorMap[rootOperatorId]);
  }

  // set properties of the query plan
  DecomposedQueryPlanId decomposableQueryPlanId = DecomposedQueryPlanId(
      serializableDecomposedQueryPlan->decomposedqueryplanid());
  SharedQueryId sharedQueryId =
      SharedQueryId(serializableDecomposedQueryPlan->sharedqueryplanid());

  auto decomposedQueryPlan =
      DecomposedQueryPlan::create(decomposableQueryPlanId, sharedQueryId,
                                  INVALID_WORKER_NODE_ID, rootOperators);
  if (serializableDecomposedQueryPlan->has_state()) {
    auto state =
        deserializeQueryState(serializableDecomposedQueryPlan->state());
    decomposedQueryPlan->setState(state);
  }
  return decomposedQueryPlan;
}

QueryState DecomposedQueryPlanSerializationUtil::deserializeQueryState(
    SerializableQueryState serializedQueryState) {
...
DecomposedQueryPlanSerializationUtil::serializeQueryState(
