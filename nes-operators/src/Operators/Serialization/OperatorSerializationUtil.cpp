...
SerializableOperator OperatorSerializationUtil::serializeOperator(
    const OperatorPtr& operatorNode, bool isClientOriginated) {
  NES_TRACE("OperatorSerializationUtil:: serialize operator {}",
  ...
  if (operatorNode->instanceOf<SourceLogicalOperator>()) {
    ...
    serializeSourceOperator(*operatorNode->as<SourceLogicalOperator>(),
                            serializedOperator, isClientOriginated);

  } else if (operatorNode->instanceOf<SinkLogicalOperator>()) {
    // serialize sink operator
    serializeSinkOperator(*operatorNode->as<SinkLogicalOperator>(),
                          serializedOperator);

  } else if (operatorNode->instanceOf<LogicalFilterOperator>()) {
    // serialize filter operator
    serializeFilterOperator(*operatorNode->as<LogicalFilterOperator>(),
                            serializedOperator);

  } else if (operatorNode->instanceOf<LogicalProjectionOperator>()) {
    // serialize projection operator
    serializeProjectionOperator(*operatorNode->as<LogicalProjectionOperator>(),
                                serializedOperator);
    ...
    serializeMapOperator(*operatorNode->as<LogicalMapOperator>(),
                         serializedOperator);

  } else if (operatorNode
                 ->instanceOf<InferModel::LogicalInferModelOperator>()) {
    // serialize infer model
    serializeInferModelOperator(
        *operatorNode->as<InferModel::LogicalInferModelOperator>(),
        serializedOperator);
  } else if (operatorNode->instanceOf<LogicalWindowOperator>()) {
    // serialize window operator
    serializeWindowOperator(*operatorNode->as<LogicalWindowOperator>(),
                            serializedOperator);
  } else if (operatorNode->instanceOf<LogicalJoinOperator>()) {
    // serialize streaming join operator
    serializeJoinOperator(*operatorNode->as<LogicalJoinOperator>(),
                          serializedOperator);
  } else if (operatorNode
                 ->instanceOf<Experimental::LogicalBatchJoinOperator>()) {
    // serialize batch join operator
    serializeBatchJoinOperator(
        *operatorNode->as<Experimental::LogicalBatchJoinOperator>(),
        serializedOperator);

  } else if (operatorNode->instanceOf<LogicalLimitOperator>()) {
    // serialize limit operator
    serializeLimitOperator(*operatorNode->as<LogicalLimitOperator>(),
                           serializedOperator);

  } else if (operatorNode->instanceOf<WatermarkAssignerLogicalOperator>()) {
    // serialize watermarkAssigner operator
    serializeWatermarkAssignerOperator(
        *operatorNode->as<WatermarkAssignerLogicalOperator>(),
        serializedOperator);
    ...
    renameDetails.set_newsourcename(
        operatorNode->as<RenameSourceOperator>()->getNewSourceName());
    ...
    serializeJavaUDFOperator<MapUDFLogicalOperator,
    ...
    serializeJavaUDFOperator<FlatMapUDFLogicalOperator,
                             SerializableOperator_FlatMapJavaUdfDetails>(
    ...
    serializeOpenCLOperator(*operatorNode->as<LogicalOpenCLOperator>(),
                            serializedOperator);
  } else if (operatorNode
                 ->instanceOf<Statistic::LogicalStatisticWindowOperator>()) {
    // Serialize map udf operator
    serializeStatisticWindowOperator(
        *operatorNode->as<Statistic::LogicalStatisticWindowOperator>(),
        serializedOperator);
  } else {
    NES_FATAL_ERROR(
        "OperatorSerializationUtil: could not serialize this operator: {}",
        operatorNode->toString());
  }
  ...
  SchemaSerializationUtil::serializeSchema(
      ...
      serializedOperator.mutable_outputschema());
  ...
  for (const auto& child : operatorNode->getChildren()) {
    serializedOperator.add_childrenids(
        child->as<Operator>()->getId().getRawValue());
  }
  ...
  NES_TRACE("OperatorSerializationUtil:: serialize {} to {}",
            operatorNode->toString(), serializedOperator.details().type_url());
  return serializedOperator;
}

OperatorPtr OperatorSerializationUtil::deserializeOperator(
    SerializableOperator serializedOperator) {
  NES_TRACE("OperatorSerializationUtil:: de-serialize {}",
  ...
  if (details.Is<SerializableOperator_SourceDetails>()) {
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialize to SourceLogicalOperator");
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialize to SinkLogicalOperator");
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialize to FilterLogicalOperator");
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialize to "
        "ProjectionLogicalOperator");
    auto serializedProjectionOperator =
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialize to UnionLogicalOperator");
    auto serializedUnionDescriptor = SerializableOperator_UnionDetails();
    details.UnpackTo(&serializedUnionDescriptor);
    operatorNode =
        LogicalOperatorFactory::createUnionOperator(getNextOperatorId());
    ...
    auto serializedInferModelOperator =
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialize to WindowLogicalOperator");
    auto serializedWindowOperator = SerializableOperator_WindowDetails();
    details.UnpackTo(&serializedWindowOperator);
    auto windowNode = deserializeWindowOperator(serializedWindowOperator,
                                                getNextOperatorId());
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: deserialize to rename source operator");
    auto renameDetails = SerializableOperator_RenameDetails();
    details.UnpackTo(&renameDetails);
    operatorNode = LogicalOperatorFactory::createRenameSourceOperator(
        renameDetails.newsourcename());
    ...
    NES_THROW_RUNTIME_ERROR(
        "OperatorSerializationUtil: could not de-serialize this serialized "
        "operator: ");
  }

  // de-serialize operator output schema
  operatorNode->setOutputSchema(SchemaSerializationUtil::deserializeSchema(
      serializedOperator.outputschema()));
  ...
  if (details.Is<SerializableOperator_JoinDetails>()) {
    ...
    joinOp->getJoinDefinition()->updateSourceTypes(
        joinOp->getLeftInputSchema(), joinOp->getRightInputSchema());
    joinOp->getJoinDefinition()->updateOutputDefinition(
        joinOp->getOutputSchema());
  }

  if (details.Is<SerializableOperator_BatchJoinDetails>()) {
    auto joinOp = operatorNode->as<Experimental::LogicalBatchJoinOperator>();
    joinOp->getBatchJoinDefinition()->updateInputSchemas(
        joinOp->getLeftInputSchema(), joinOp->getRightInputSchema());
    joinOp->getBatchJoinDefinition()->updateOutputDefinition(
        joinOp->getOutputSchema());
  }
  ...
  NES_TRACE("OperatorSerializationUtil:: de-serialize {} to {}",
            serializedOperator.DebugString(), operatorNode->toString());
  return operatorNode;
}

void OperatorSerializationUtil::serializeSourceOperator(
    const SourceLogicalOperator& sourceOperator,
    SerializableOperator& serializedOperator, bool isClientOriginated) {
...
LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeSourceOperator(
...
void OperatorSerializationUtil::serializeFilterOperator(
  ...
  ExpressionSerializationUtil::serializeExpression(
      filterOperator.getPredicate(), filterDetails.mutable_predicate());
  serializedOperator.mutable_details()->PackFrom(filterDetails);
}

LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeFilterOperator(
    const SerializableOperator_FilterDetails& filterDetails) {
  auto filterExpression = ExpressionSerializationUtil::deserializeExpression(
      filterDetails.predicate());
  return LogicalOperatorFactory::createFilterOperator(filterExpression,
                                                      getNextOperatorId());
}

void OperatorSerializationUtil::serializeProjectionOperator(
    const LogicalProjectionOperator& projectionOperator,
    SerializableOperator& serializedOperator) {
  NES_TRACE(
      "OperatorSerializationUtil:: serialize to LogicalProjectionOperator");
...
OperatorSerializationUtil::deserializeProjectionOperator(
    const SerializableOperator_ProjectionDetails& projectionDetails) {
  // serialize and append children if the node has any
  std::vector<ExpressionNodePtr> exps;
  for (const auto& serializedExpression : projectionDetails.expression()) {
    auto projectExpression = ExpressionSerializationUtil::deserializeExpression(
        serializedExpression);
    exps.push_back(projectExpression);
  }

  return LogicalOperatorFactory::createProjectionOperator(exps,
                                                          getNextOperatorId());
}

void OperatorSerializationUtil::serializeSinkOperator(
    const SinkLogicalOperator& sinkOperator,
    SerializableOperator& serializedOperator) {
  ...
  serializeSinkDescriptor(*sinkDescriptor, sinkDetails,
                          sinkOperator.getInputOriginIds().size());
  serializedOperator.mutable_details()->PackFrom(sinkDetails);
}

LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeSinkOperator(
    const SerializableOperator_SinkDetails& sinkDetails) {
  auto sinkDescriptor = deserializeSinkDescriptor(sinkDetails);
  return LogicalOperatorFactory::createSinkOperator(
      sinkDescriptor, INVALID_WORKER_NODE_ID, getNextOperatorId());
}

void OperatorSerializationUtil::serializeMapOperator(
    const LogicalMapOperator& mapOperator,
    SerializableOperator& serializedOperator) {
  NES_TRACE("OperatorSerializationUtil:: serialize to LogicalMapOperator");
  auto mapDetails = SerializableOperator_MapDetails();
  ExpressionSerializationUtil::serializeExpression(
      mapOperator.getMapExpression(), mapDetails.mutable_expression());
  serializedOperator.mutable_details()->PackFrom(mapDetails);
}

LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeMapOperator(
    const SerializableOperator_MapDetails& mapDetails) {
  auto fieldAssignmentExpression =
      ExpressionSerializationUtil::deserializeExpression(
          mapDetails.expression());
  return LogicalOperatorFactory::createMapOperator(
      fieldAssignmentExpression->as<FieldAssignmentExpressionNode>(),
      getNextOperatorId());
}

void OperatorSerializationUtil::serializeWindowOperator(
    const WindowOperator& windowOperator,
    SerializableOperator& serializedOperator) {
  ...
  if (windowType->instanceOf<Windowing::TimeBasedWindowType>()) {
    ...
    if (timeCharacteristic->getType() ==
        Windowing::TimeCharacteristic::Type::EventTime) {
      timeCharacteristicDetails.set_type(
          SerializableOperator_TimeCharacteristic_Type_EventTime);
      timeCharacteristicDetails.set_field(
          timeCharacteristic->getField()->getName());
    } else if (timeCharacteristic->getType() ==
               Windowing::TimeCharacteristic::Type::IngestionTime) {
      timeCharacteristicDetails.set_type(
          SerializableOperator_TimeCharacteristic_Type_IngestionTime);
    } else {
      NES_ERROR(
          "OperatorSerializationUtil: Cant serialize window Time "
          "Characteristic");
    }
    timeCharacteristicDetails.set_multiplier(
    ...
    if (windowType->instanceOf<Windowing::TumblingWindow>()) {
      ...
      tumblingWindowDetails.mutable_timecharacteristic()->CopyFrom(
          timeCharacteristicDetails);
      ...
      slidingWindowDetails.mutable_timecharacteristic()->CopyFrom(
          timeCharacteristicDetails);
    ...
    if (contentBasedWindowType->getContentBasedSubWindowType() ==
      ...
      auto thresholdWindow =
          std::dynamic_pointer_cast<Windowing::ThresholdWindow>(windowType);
      auto thresholdWindowDetails = SerializableOperator_ThresholdWindow();
      ExpressionSerializationUtil::serializeExpression(
          thresholdWindow->getPredicate(),
          thresholdWindowDetails.mutable_predicate());
      thresholdWindowDetails.set_minimumcount(
          thresholdWindow->getMinimumCount());
      windowDetails.mutable_windowtype()->PackFrom(thresholdWindowDetails);
    } else {
      NES_ERROR(
          "OperatorSerializationUtil: Cant serialize this content based window "
          "Type");
    }
  }
  ...
  for (auto aggregation : windowDefinition->getWindowAggregation()) {
    ...
    ExpressionSerializationUtil::serializeExpression(
        aggregation->as(), windowAggregation->mutable_asfield());
    ExpressionSerializationUtil::serializeExpression(
        aggregation->on(), windowAggregation->mutable_onfield());

    using enum Windowing::WindowAggregationDescriptor::Type;
    switch (aggregation->getType()) {
      case Count:
        windowAggregation->set_type(
            SerializableOperator_WindowDetails_Aggregation_Type_COUNT);
        break;
      case Max:
        windowAggregation->set_type(
            SerializableOperator_WindowDetails_Aggregation_Type_MAX);
        break;
      case Min:
        windowAggregation->set_type(
            SerializableOperator_WindowDetails_Aggregation_Type_MIN);
        break;
      case Sum:
        windowAggregation->set_type(
            SerializableOperator_WindowDetails_Aggregation_Type_SUM);
        break;
      case Avg:
        windowAggregation->set_type(
            SerializableOperator_WindowDetails_Aggregation_Type_AVG);
        break;
      case Median:
        windowAggregation->set_type(
            SerializableOperator_WindowDetails_Aggregation_Type_MEDIAN);
        break;
      default:
        NES_FATAL_ERROR(
            "OperatorSerializationUtil: could not cast aggregation type");
    }
  }
...
LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeWindowOperator(
    const SerializableOperator_WindowDetails& windowDetails,
  ...
  for (const auto& serializedWindowAggregation : serializedWindowAggregations) {
    auto onField = ExpressionSerializationUtil::deserializeExpression(
                       serializedWindowAggregation.onfield())
                       ->as<FieldAccessExpressionNode>();
    auto asField = ExpressionSerializationUtil::deserializeExpression(
                       serializedWindowAggregation.asfield())
                       ->as<FieldAccessExpressionNode>();
    if (serializedWindowAggregation.type() ==
        SerializableOperator_WindowDetails_Aggregation_Type_SUM) {
      aggregation.emplace_back(
          Windowing::SumAggregationDescriptor::create(onField, asField));
    } else if (serializedWindowAggregation.type() ==
               SerializableOperator_WindowDetails_Aggregation_Type_MAX) {
      aggregation.emplace_back(
          Windowing::MaxAggregationDescriptor::create(onField, asField));
    } else if (serializedWindowAggregation.type() ==
               SerializableOperator_WindowDetails_Aggregation_Type_MIN) {
      aggregation.emplace_back(
          Windowing::MinAggregationDescriptor::create(onField, asField));
    } else if (serializedWindowAggregation.type() ==
               SerializableOperator_WindowDetails_Aggregation_Type_COUNT) {
      aggregation.emplace_back(
          Windowing::CountAggregationDescriptor::create(onField, asField));
    } else if (serializedWindowAggregation.type() ==
               SerializableOperator_WindowDetails_Aggregation_Type_AVG) {
      aggregation.emplace_back(
          Windowing::AvgAggregationDescriptor::create(onField, asField));
    } else if (serializedWindowAggregation.type() ==
               SerializableOperator_WindowDetails_Aggregation_Type_MEDIAN) {
      aggregation.emplace_back(
          Windowing::MedianAggregationDescriptor::create(onField, asField));
    } else {
      NES_FATAL_ERROR(
          "OperatorSerializationUtil: could not de-serialize window "
          "aggregation: {}",
  ...
  if (serializedWindowType.Is<SerializableOperator_TumblingWindow>()) {
    ...
    auto serializedTimeCharacteristic =
        serializedTumblingWindow.timecharacteristic();
    auto multiplier = serializedTimeCharacteristic.multiplier();
    if (serializedTimeCharacteristic.type() ==
        SerializableOperator_TimeCharacteristic_Type_EventTime) {
      auto field = FieldAccessExpressionNode::create(
          serializedTimeCharacteristic.field());
      window = Windowing::TumblingWindow::of(
          Windowing::TimeCharacteristic::createEventTime(
              field, Windowing::TimeUnit(multiplier)),
      ...
      NES_FATAL_ERROR(
          "OperatorSerializationUtil: could not de-serialize window time "
          "characteristic: {}",
    ...
    auto serializedTimeCharacteristic =
        serializedSlidingWindow.timecharacteristic();
    auto multiplier = serializedTimeCharacteristic.multiplier();
    if (serializedTimeCharacteristic.type() ==
        SerializableOperator_TimeCharacteristic_Type_EventTime) {
      auto field = FieldAccessExpressionNode::create(
    ...
    auto thresholdExpression =
        ExpressionSerializationUtil::deserializeExpression(
            serializedThresholdWindow.predicate());
    ...
    NES_FATAL_ERROR(
        "OperatorSerializationUtil: could not de-serialize window type: {}",
        serializedWindowType.DebugString());
  }
  ...
  for (auto& key : windowDetails.keys()) {
    keyAccessExpression.emplace_back(
        ExpressionSerializationUtil::deserializeExpression(key)
            ->as<FieldAccessExpressionNode>());
  }
  auto windowDef = Windowing::LogicalWindowDescriptor::create(
      keyAccessExpression, aggregation, window, allowedLateness);
...
void OperatorSerializationUtil::serializeJoinOperator(
    const LogicalJoinOperator& joinOperator,
    SerializableOperator& serializedOperator) {
  ...
  ExpressionSerializationUtil::serializeExpression(
      joinDefinition->getJoinExpression(),
      joinDetails.mutable_joinexpression());
  ...
  auto timeCharacteristicDetails = SerializableOperator_TimeCharacteristic();
  timeCharacteristicDetails.set_multiplier(
      timeCharacteristic->getTimeUnit().getMillisecondsConversionMultiplier());
  if (timeCharacteristic->getType() ==
      Windowing::TimeCharacteristic::Type::EventTime) {
    timeCharacteristicDetails.set_type(
        SerializableOperator_TimeCharacteristic_Type_EventTime);
    ...
    NES_ERROR(
        "OperatorSerializationUtil: Cant serialize window Time Characteristic");
  }
  if (windowType->instanceOf<Windowing::TumblingWindow>()) {
    auto tumblingWindow = windowType->as<Windowing::TumblingWindow>();
    auto tumblingWindowDetails = SerializableOperator_TumblingWindow();
  ...
  joinDetails.set_numberofinputedgesleft(
      joinDefinition->getNumberOfInputEdgesLeft());
  joinDetails.set_numberofinputedgesright(
      joinDefinition->getNumberOfInputEdgesRight());
  ...
  if (joinDefinition->getJoinType() ==
      Join::LogicalJoinDescriptor::JoinType::INNER_JOIN) {
    joinDetails.mutable_jointype()->set_jointype(
        SerializableOperator_JoinDetails_JoinTypeCharacteristic_JoinType_INNER_JOIN);
  } else if (joinDefinition->getJoinType() ==
...
LogicalJoinOperatorPtr OperatorSerializationUtil::deserializeJoinOperator(
    const SerializableOperator_JoinDetails& joinDetails,
    OperatorId operatorId) {
  ...
  if (serializedJoinType.jointype() ==
      SerializableOperator_JoinDetails_JoinTypeCharacteristic_JoinType_CARTESIAN_PRODUCT) {
  ...
  if (serializedWindowType.Is<SerializableOperator_TumblingWindow>()) {
    auto serializedTumblingWindow = SerializableOperator_TumblingWindow();
    serializedWindowType.UnpackTo(&serializedTumblingWindow);
    const auto& serializedTimeCharacteristic =
        serializedTumblingWindow.timecharacteristic();
    auto multiplier = serializedTimeCharacteristic.multiplier();
    if (serializedTimeCharacteristic.type() ==
        SerializableOperator_TimeCharacteristic_Type_EventTime) {
  ...
  auto joinExpression = ExpressionSerializationUtil::deserializeExpression(
                            serializedJoinExpression)
                            ->as<BinaryExpressionNode>();

  auto joinDefinition = Join::LogicalJoinDescriptor::create(
      joinExpression, window, joinDetails.numberofinputedgesleft(),
      joinDetails.numberofinputedgesright(), joinType);
  auto joinOperator =
      LogicalOperatorFactory::createJoinOperator(joinDefinition, operatorId)
          ->as<LogicalJoinOperator>();
  joinOperator->setWindowStartEndKeyFieldName(
      joinDetails.windowstartfieldname(), joinDetails.windowendfieldname());
  ...
  //     {
  //         return
  //         LogicalOperatorFactory::createCentralWindowSpecializedOperator(windowDef,
  //         operatorId)->as<CentralWindowOperator>();
  //     } else if (distrChar.distr() ==
  //     SerializableOperator_JoinDetails_DistributionCharacteristic_Distribution_Combining)
  //     {
  //         return
  //         LogicalOperatorFactory::createWindowComputationSpecializedOperator(windowDef,
  //         operatorId)->as<WindowComputationOperator>();
  //     } else if (distrChar.distr() ==
  //     SerializableOperator_JoinDetails_DistributionCharacteristic_Distribution_Slicing)
  //     {
  //         return
  //         LogicalOperatorFactory::createSliceCreationSpecializedOperator(windowDef,
  //         operatorId)->as<SliceCreationOperator>();
  //     } else {
  //         NES_NOT_IMPLEMENTED();
  //     }
}
...
void OperatorSerializationUtil::serializeBatchJoinOperator(
    const Experimental::LogicalBatchJoinOperator& joinOperator,
    SerializableOperator& serializedOperator) {
  NES_TRACE(
      "OperatorSerializationUtil:: serialize to LogicalBatchJoinOperator");

  auto joinDetails = SerializableOperator_BatchJoinDetails();
  auto joinDefinition = joinOperator.getBatchJoinDefinition();

  ExpressionSerializationUtil::serializeExpression(
      joinDefinition->getBuildJoinKey(), joinDetails.mutable_onbuildkey());
  ExpressionSerializationUtil::serializeExpression(
      joinDefinition->getProbeJoinKey(), joinDetails.mutable_onprobekey());

  joinDetails.set_numberofinputedgesbuild(
      joinDefinition->getNumberOfInputEdgesBuild());
  joinDetails.set_numberofinputedgesprobe(
      joinDefinition->getNumberOfInputEdgesProbe());

  serializedOperator.mutable_details()->PackFrom(joinDetails);
}

Experimental::LogicalBatchJoinOperatorPtr
OperatorSerializationUtil::deserializeBatchJoinOperator(
    const SerializableOperator_BatchJoinDetails& joinDetails,
    OperatorId operatorId) {
  auto buildKeyAccessExpression =
      ExpressionSerializationUtil::deserializeExpression(
          joinDetails.onbuildkey())
          ->as<FieldAccessExpressionNode>();
  auto probeKeyAccessExpression =
      ExpressionSerializationUtil::deserializeExpression(
          joinDetails.onprobekey())
          ->as<FieldAccessExpressionNode>();
  auto joinDefinition = Join::Experimental::LogicalBatchJoinDescriptor::create(
      buildKeyAccessExpression, probeKeyAccessExpression,
      joinDetails.numberofinputedgesprobe(),
      joinDetails.numberofinputedgesbuild());
  auto retValue = LogicalOperatorFactory::createBatchJoinOperator(
                      joinDefinition, operatorId)
...
void OperatorSerializationUtil::serializeSourceDescriptor(
  ...
  NES_DEBUG(
      "OperatorSerializationUtil:: serialize to SourceDescriptor with ={}",
      sourceDescriptor.toString());
  if (sourceDescriptor.instanceOf<const ZmqSourceDescriptor>()) {
    // serialize zmq source descriptor
    NES_TRACE(
        "OperatorSerializationUtil:: serialized SourceDescriptor as "
    ...
    auto zmqSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableZMQSourceDescriptor();
    ...
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        zmqSerializedSourceDescriptor);
  }
#ifdef ENABLE_MQTT_BUILD
    ...
    auto mqttSourceDescriptor =
        sourceDescriptor.as<const MQTTSourceDescriptor>();
    // init serializable source config
    auto serializedPhysicalSourceType = new SerializablePhysicalSourceType();
    serializedPhysicalSourceType->set_sourcetype(
        mqttSourceDescriptor->getSourceConfigPtr()->getSourceTypeAsString());
    // init serializable mqtt source config
    auto mqttSerializedSourceConfig =
        SerializablePhysicalSourceType_SerializableMQTTSourceType();
    mqttSerializedSourceConfig.set_clientid(
        mqttSourceDescriptor->getSourceConfigPtr()->getClientId()->getValue());
    mqttSerializedSourceConfig.set_url(
        mqttSourceDescriptor->getSourceConfigPtr()->getUrl()->getValue());
    mqttSerializedSourceConfig.set_username(
        mqttSourceDescriptor->getSourceConfigPtr()->getUserName()->getValue());
    mqttSerializedSourceConfig.set_topic(
        mqttSourceDescriptor->getSourceConfigPtr()->getTopic()->getValue());
    mqttSerializedSourceConfig.set_qos(
        mqttSourceDescriptor->getSourceConfigPtr()->getQos()->getValue());
    mqttSerializedSourceConfig.set_cleansession(
    ...
    mqttSerializedSourceConfig.set_flushintervalms(
        mqttSourceDescriptor->getSourceConfigPtr()
            ->getFlushIntervalMS()
            ->getValue());
    switch (mqttSourceDescriptor->getSourceConfigPtr()
                ->getInputFormat()
                ->getValue()) {
      case Configurations::InputFormat::JSON:
        mqttSerializedSourceConfig.set_inputformat(
            SerializablePhysicalSourceType_InputFormat_JSON);
        break;
      case Configurations::InputFormat::CSV:
        mqttSerializedSourceConfig.set_inputformat(
            SerializablePhysicalSourceType_InputFormat_CSV);
        break;
      case Configurations::InputFormat::NES_BINARY:
        NES_NOT_IMPLEMENTED();
    }
    serializedPhysicalSourceType->mutable_specificphysicalsourcetype()
        ->PackFrom(mqttSerializedSourceConfig);
    // init serializable mqtt source descriptor
    auto mqttSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableMQTTSourceDescriptor();
    mqttSerializedSourceDescriptor.set_allocated_physicalsourcetype(
        serializedPhysicalSourceType);
    // serialize source schema
    SchemaSerializationUtil::serializeSchema(
        mqttSourceDescriptor->getSchema(),
        mqttSerializedSourceDescriptor.mutable_sourceschema());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        mqttSerializedSourceDescriptor);
  }
#endif
#ifdef ENABLE_OPC_BUILD
    ...
    auto opcSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableOPCSourceDescriptor();
    char* ident = (char*)UA_malloc(
        sizeof(char) *
    ...
    opcSerializedSourceDescriptor.set_namespaceindex(
        opcSourceDescriptor->getNodeId().namespaceIndex);
    opcSerializedSourceDescriptor.set_identifiertype(
        opcSourceDescriptor->getNodeId().identifierType);
    opcSerializedSourceDescriptor.set_user(opcSourceDescriptor->getUser());
    opcSerializedSourceDescriptor.set_password(
        opcSourceDescriptor->getPassword());
    // serialize source schema
    SchemaSerializationUtil::serializeSchema(
        opcSourceDescriptor->getSchema(),
        opcSerializedSourceDescriptor.mutable_sourceschema());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        opcSerializedSourceDescriptor);
  }
#endif
    ...
    auto serializedPhysicalSourceType = new SerializablePhysicalSourceType();
    serializedPhysicalSourceType->set_sourcetype(
        tcpSourceDescriptor->getSourceConfig()->getSourceTypeAsString());
    // init serializable tcp source config
    auto tcpSerializedSourceConfig =
        SerializablePhysicalSourceType_SerializableTCPSourceType();
    tcpSerializedSourceConfig.set_sockethost(
        tcpSourceDescriptor->getSourceConfig()->getSocketHost()->getValue());
    tcpSerializedSourceConfig.set_socketport(
        tcpSourceDescriptor->getSourceConfig()->getSocketPort()->getValue());
    tcpSerializedSourceConfig.set_socketdomain(
        tcpSourceDescriptor->getSourceConfig()->getSocketDomain()->getValue());
    tcpSerializedSourceConfig.set_sockettype(
        tcpSourceDescriptor->getSourceConfig()->getSocketType()->getValue());
    std::string tupleSeparator;
    tupleSeparator =
        tcpSourceDescriptor->getSourceConfig()->getTupleSeparator()->getValue();
    ...
    tcpSerializedSourceConfig.set_flushintervalms(
        tcpSourceDescriptor->getSourceConfig()
            ...
            ->getValue());
    switch (
        tcpSourceDescriptor->getSourceConfig()->getInputFormat()->getValue()) {
      case Configurations::InputFormat::JSON:
        tcpSerializedSourceConfig.set_inputformat(
            SerializablePhysicalSourceType_InputFormat_JSON);
        break;
      case Configurations::InputFormat::CSV:
        tcpSerializedSourceConfig.set_inputformat(
      ...
      case Configurations::InputFormat::NES_BINARY:
        tcpSerializedSourceConfig.set_inputformat(
            SerializablePhysicalSourceType_InputFormat_NES_BINARY);
        break;
    }

    using enum Configurations::TCPDecideMessageSize;
    switch (tcpSourceDescriptor->getSourceConfig()
                ->getDecideMessageSize()
                ->getValue()) {
    ...
    tcpSerializedSourceConfig.set_bytesusedforsocketbuffersizetransfer(
        tcpSourceDescriptor->getSourceConfig()
            ->getBytesUsedForSocketBufferSizeTransfer()
            ->getValue());
    serializedPhysicalSourceType->mutable_specificphysicalsourcetype()
        ->PackFrom(tcpSerializedSourceConfig);
    // init serializable tcp source descriptor
    auto tcpSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableTCPSourceDescriptor();
    tcpSerializedSourceDescriptor.set_allocated_physicalsourcetype(
        serializedPhysicalSourceType);
    ...
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        tcpSerializedSourceDescriptor);
  } else if (sourceDescriptor.instanceOf<const MonitoringSourceDescriptor>()) {
    // serialize network source descriptor
    NES_TRACE(
        "OperatorSerializationUtil:: serialized SourceDescriptor as "
        "SerializableOperator_SourceDetails_"
        "SerializableNetworkSourceDescriptor");
    auto monitoringSourceDescriptor =
        sourceDescriptor.as<const MonitoringSourceDescriptor>();
    auto monitoringSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableMonitoringSourceDescriptor();
    auto metricCollectorType =
        monitoringSourceDescriptor->getMetricCollectorType();
    auto waitTime = monitoringSourceDescriptor->getWaitTime();
    // serialize source schema
    monitoringSerializedSourceDescriptor.set_metriccollectortype(
        magic_enum::enum_integer(metricCollectorType));
    monitoringSerializedSourceDescriptor.set_waittime(waitTime.count());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        monitoringSerializedSourceDescriptor);
  } else if (sourceDescriptor
                 .instanceOf<const Network::NetworkSourceDescriptor>()) {
    // serialize network source descriptor
    NES_TRACE(
        "OperatorSerializationUtil:: serialized SourceDescriptor as "
        "SerializableOperator_SourceDetails_"
    ...
    auto networkSourceDescriptor =
        sourceDescriptor.as<const Network::NetworkSourceDescriptor>();
    auto networkSerializedSourceDescriptor =
    ...
    networkSerializedSourceDescriptor.mutable_nespartition()->set_operatorid(
        nesPartition.getOperatorId().getRawValue());
    networkSerializedSourceDescriptor.mutable_nespartition()->set_partitionid(
        nesPartition.getPartitionId().getRawValue());
    networkSerializedSourceDescriptor.mutable_nespartition()->set_queryid(
        nesPartition.getQueryId().getRawValue());
    networkSerializedSourceDescriptor.mutable_nespartition()
        ->set_subpartitionid(nesPartition.getSubpartitionId().getRawValue());
    networkSerializedSourceDescriptor.mutable_nodelocation()->set_port(
        nodeLocation.getPort());
    networkSerializedSourceDescriptor.mutable_nodelocation()->set_hostname(
        nodeLocation.getHostname());
    networkSerializedSourceDescriptor.mutable_nodelocation()->set_nodeid(
        nodeLocation.getNodeId().getRawValue());
    ...
    networkSerializedSourceDescriptor.set_version(
        networkSourceDescriptor->getVersion());
    networkSerializedSourceDescriptor.set_uniqueid(
        networkSourceDescriptor->getUniqueId().getRawValue());
    networkSerializedSourceDescriptor.set_retrytimes(
        networkSourceDescriptor->getRetryTimes());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        networkSerializedSourceDescriptor);
    ...
    auto defaultSourceDescriptor =
        sourceDescriptor.as<const DefaultSourceDescriptor>();
    auto defaultSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableDefaultSourceDescriptor();
    defaultSerializedSourceDescriptor.set_sourcegatheringinterval(
        defaultSourceDescriptor->getSourceGatheringIntervalCount());
    defaultSerializedSourceDescriptor.set_numberofbufferstoproduce(
        defaultSourceDescriptor->getNumbersOfBufferToProduce());
    // serialize source schema
    SchemaSerializationUtil::serializeSchema(
        defaultSourceDescriptor->getSchema(),
        defaultSerializedSourceDescriptor.mutable_sourceschema());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        defaultSerializedSourceDescriptor);
    ...
    auto binarySourceDescriptor =
        sourceDescriptor.as<const BinarySourceDescriptor>();
    auto binarySerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableBinarySourceDescriptor();
    binarySerializedSourceDescriptor.set_filepath(
        binarySourceDescriptor->getFilePath());
    // serialize source schema
    SchemaSerializationUtil::serializeSchema(
        binarySourceDescriptor->getSchema(),
        binarySerializedSourceDescriptor.mutable_sourceschema());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        binarySerializedSourceDescriptor);
    ...
    serializedSourceConfig->set_sourcetype(
        csvSourceDescriptor->getSourceConfig()->getSourceTypeAsString());
    // init serializable csv source config
    auto csvSerializedSourceConfig =
        SerializablePhysicalSourceType_SerializableCSVSourceType();
    ...
    csvSerializedSourceConfig.set_numberoftuplestoproduceperbuffer(
        csvSourceDescriptor->getSourceConfig()
            ->getNumberOfTuplesToProducePerBuffer()
            ->getValue());
    csvSerializedSourceConfig.set_sourcegatheringinterval(
        csvSourceDescriptor->getSourceConfig()
            ->getGatheringInterval()
            ->getValue());
    csvSerializedSourceConfig.set_filepath(
        csvSourceDescriptor->getSourceConfig()->getFilePath()->getValue());
    csvSerializedSourceConfig.set_skipheader(
        csvSourceDescriptor->getSourceConfig()->getSkipHeader()->getValue());
    csvSerializedSourceConfig.set_delimiter(
        csvSourceDescriptor->getSourceConfig()->getDelimiter()->getValue());
    serializedSourceConfig->mutable_specificphysicalsourcetype()->PackFrom(
        csvSerializedSourceConfig);
    // init serializable csv source descriptor
    auto csvSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableCsvSourceDescriptor();
    csvSerializedSourceDescriptor.set_allocated_physicalsourcetype(
    ...
    auto senseSourceDescriptor =
        sourceDescriptor.as<const SenseSourceDescriptor>();
    auto senseSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableSenseSourceDescriptor();
    senseSerializedSourceDescriptor.set_udfs(senseSourceDescriptor->getUdfs());
    // serialize source schema
    SchemaSerializationUtil::serializeSchema(
        senseSourceDescriptor->getSchema(),
        senseSerializedSourceDescriptor.mutable_sourceschema());
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        senseSerializedSourceDescriptor);
    ...
    auto logicalSourceDescriptor =
        sourceDescriptor.as<const LogicalSourceDescriptor>();
    auto logicalSourceSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableLogicalSourceDescriptor();
    logicalSourceSerializedSourceDescriptor.set_logicalsourcename(
        logicalSourceDescriptor->getLogicalSourceName());
    logicalSourceSerializedSourceDescriptor.set_physicalsourcename(
        logicalSourceDescriptor->getPhysicalSourceName());

    if (!isClientOriginated) {
      // serialize source schema
      SchemaSerializationUtil::serializeSchema(
          logicalSourceDescriptor->getSchema(),
          logicalSourceSerializedSourceDescriptor.mutable_sourceschema());
    }
    sourceDetails.mutable_sourcedescriptor()->PackFrom(
        logicalSourceSerializedSourceDescriptor);
  } else {
    NES_ERROR("OperatorSerializationUtil: Unknown Source Descriptor Type {}",
              sourceDescriptor.toString());
    throw std::invalid_argument("Unknown Source Descriptor Type");
  }
}

SourceDescriptorPtr OperatorSerializationUtil::deserializeSourceDescriptor(
    const SerializableOperator_SourceDetails& sourceDetails) {
  NES_TRACE("OperatorSerializationUtil:: de-serialized SourceDescriptor id={}",
            sourceDetails.DebugString());
  const auto& serializedSourceDescriptor = sourceDetails.sourcedescriptor();

  if (serializedSourceDescriptor.Is<
          SerializableOperator_SourceDetails_SerializableZMQSourceDescriptor>()) {
    // de-serialize zmq source descriptor
    NES_DEBUG(
        "OperatorSerializationUtil:: de-serialized SourceDescriptor as "
        "ZmqSourceDescriptor");
    auto zmqSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableZMQSourceDescriptor();
    serializedSourceDescriptor.UnpackTo(&zmqSerializedSourceDescriptor);
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        zmqSerializedSourceDescriptor.sourceschema());
    auto ret = ZmqSourceDescriptor::create(
        schema, zmqSerializedSourceDescriptor.host(),
        zmqSerializedSourceDescriptor.port());
    return ret;
  }
#ifdef ENABLE_MQTT_BUILD
  if (serializedSourceDescriptor.Is<
    ...
    NES_DEBUG(
        "OperatorSerializationUtil:: de-serialized SourceDescriptor as "
        "MQTTSourceDescriptor");
    auto* mqttSerializedSourceDescriptor =
        new SerializableOperator_SourceDetails_SerializableMQTTSourceDescriptor();
    serializedSourceDescriptor.UnpackTo(mqttSerializedSourceDescriptor);
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        mqttSerializedSourceDescriptor->sourceschema());
    auto mqttSourceConfig =
        new SerializablePhysicalSourceType_SerializableMQTTSourceType();
    SerializablePhysicalSourceType physicalSourceType =
        mqttSerializedSourceDescriptor->physicalsourcetype();
    auto sourceConfig =
        MQTTSourceType::create(physicalSourceType.logicalsourcename(),
                               physicalSourceType.physicalsourcename());
    ...
    sourceConfig->setInputFormat(static_cast<Configurations::InputFormat>(
        mqttSourceConfig->inputformat()));
    ...
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        opcSerializedSourceDescriptor.sourceschema());
    char* ident = (char*)UA_malloc(
        sizeof(char) * opcSerializedSourceDescriptor.identifier().length() + 1);
    memcpy(ident, opcSerializedSourceDescriptor.identifier().data(),
           opcSerializedSourceDescriptor.identifier().length());
    ident[opcSerializedSourceDescriptor.identifier().length()] = '\0';
    UA_NodeId nodeId =
        UA_NODEID_STRING(opcSerializedSourceDescriptor.namespaceindex(), ident);
    auto ret = OPCSourceDescriptor::create(
        schema, opcSerializedSourceDescriptor.url(), nodeId,
    ...
    NES_DEBUG(
        ...
        "TCPSourceDescriptor");
    auto* tcpSerializedSourceDescriptor =
        new SerializableOperator_SourceDetails_SerializableTCPSourceDescriptor();
    serializedSourceDescriptor.UnpackTo(tcpSerializedSourceDescriptor);
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        tcpSerializedSourceDescriptor->sourceschema());
    SerializablePhysicalSourceType physicalSourceType =
        tcpSerializedSourceDescriptor->physicalsourcetype();
    auto sourceConfig =
        TCPSourceType::create(physicalSourceType.logicalsourcename(),
                              physicalSourceType.physicalsourcename());
    auto tcpSourceConfig =
        new SerializablePhysicalSourceType_SerializableTCPSourceType();
    tcpSerializedSourceDescriptor->physicalsourcetype()
        .specificphysicalsourcetype()
        .UnpackTo(tcpSourceConfig);
    ...
    sourceConfig->setInputFormat(static_cast<Configurations::InputFormat>(
        tcpSourceConfig->inputformat()));
    sourceConfig->setDecideMessageSize(
        static_cast<Configurations::TCPDecideMessageSize>(
            tcpSourceConfig->tcpdecidemessagesize()));
    ...
    sourceConfig->setBytesUsedForSocketBufferSizeTransfer(
        tcpSourceConfig->bytesusedforsocketbuffersizetransfer());
    ...
    NES_DEBUG(
        "OperatorSerializationUtil:: de-serialized SourceDescriptor as "
        "monitoringSourceDescriptor");
    auto monitoringSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableMonitoringSourceDescriptor();
    serializedSourceDescriptor.UnpackTo(&monitoringSerializedSourceDescriptor);
    // de-serialize source schema
    auto waitTime = std::chrono::milliseconds(
        monitoringSerializedSourceDescriptor.waittime());
    auto metricCollectorType =
        monitoringSerializedSourceDescriptor.metriccollectortype();
    auto ret = MonitoringSourceDescriptor::create(
        waitTime, Monitoring::MetricCollectorType(metricCollectorType));
    return ret;
  } else if (
      serializedSourceDescriptor.Is<
          SerializableOperator_SourceDetails_SerializableNetworkSourceDescriptor>()) {
    ...
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        networkSerializedSourceDescriptor.sourceschema());
    Network::NesPartition nesPartition{
        SharedQueryId(
            networkSerializedSourceDescriptor.nespartition().queryid()),
        OperatorId(
    ...
    auto waitTime =
        std::chrono::milliseconds(networkSerializedSourceDescriptor.waittime());
    auto ret = Network::NetworkSourceDescriptor::create(
        schema, nesPartition, nodeLocation, waitTime,
    ...
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        defaultSerializedSourceDescriptor.sourceschema());
    auto ret = DefaultSourceDescriptor::create(
        schema, defaultSerializedSourceDescriptor.numberofbufferstoproduce(),
    ...
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        binarySerializedSourceDescriptor.sourceschema());
    auto ret = BinarySourceDescriptor::create(
        schema, binarySerializedSourceDescriptor.filepath());
    return ret;
  } else if (
      serializedSourceDescriptor.Is<
          SerializableOperator_SourceDetails_SerializableCsvSourceDescriptor>()) {
    ...
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        csvSerializedSourceDescriptor.sourceschema());
    auto physicalSourceType =
        csvSerializedSourceDescriptor.physicalsourcetype();
    auto sourceConfig =
        CSVSourceType::create(physicalSourceType.logicalsourcename(),
                              physicalSourceType.physicalsourcename());
    auto csvSourceConfig =
        new SerializablePhysicalSourceType_SerializableCSVSourceType();
    ...
    sourceConfig->setGatheringInterval(
        csvSourceConfig->sourcegatheringinterval());
    sourceConfig->setNumberOfBuffersToProduce(
        csvSourceConfig->numberofbufferstoproduce());
    ...
    NES_DEBUG(
        "OperatorSerializationUtil:: de-serialized SourceDescriptor as "
        "SenseSourceDescriptor");
    auto senseSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableSenseSourceDescriptor();
    serializedSourceDescriptor.UnpackTo(&senseSerializedSourceDescriptor);
    // de-serialize source schema
    auto schema = SchemaSerializationUtil::deserializeSchema(
        senseSerializedSourceDescriptor.sourceschema());
    return SenseSourceDescriptor::create(
        schema, senseSerializedSourceDescriptor.udfs());
  } else if (
      serializedSourceDescriptor.Is<
          SerializableOperator_SourceDetails_SerializableLogicalSourceDescriptor>()) {
    ...
    NES_DEBUG(
        "OperatorSerializationUtil:: de-serialized SourceDescriptor as "
        "LogicalSourceDescriptor");
    auto logicalSourceSerializedSourceDescriptor =
        SerializableOperator_SourceDetails_SerializableLogicalSourceDescriptor();
    serializedSourceDescriptor.UnpackTo(
        &logicalSourceSerializedSourceDescriptor);

    // de-serialize source schema
    SourceDescriptorPtr logicalSourceDescriptor =
        LogicalSourceDescriptor::create(
            logicalSourceSerializedSourceDescriptor.logicalsourcename());
    logicalSourceDescriptor->setPhysicalSourceName(
        logicalSourceSerializedSourceDescriptor.physicalsourcename());
    // check if the schema is set
    if (logicalSourceSerializedSourceDescriptor.has_sourceschema()) {
      auto schema = SchemaSerializationUtil::deserializeSchema(
          logicalSourceSerializedSourceDescriptor.sourceschema());
    ...
    NES_ERROR("OperatorSerializationUtil: Unknown Source Descriptor Type {}",
              serializedSourceDescriptor.type_url());
    throw std::invalid_argument("Unknown Source Descriptor Type");
  }
}
...
void OperatorSerializationUtil::serializeSinkDescriptor(
    ...
    SerializableOperator_SinkDetails& sinkDetails, uint64_t numberOfOrigins) {
  ...
  if (sinkDescriptor.instanceOf<const PrintSinkDescriptor>()) {
    ...
    NES_TRACE(
        "OperatorSerializationUtil:: serialized SinkDescriptor as "
    ...
    auto nullSinkDescriptor =
        sinkDescriptor.as<const NullOutputSinkDescriptor>();
    NES_TRACE(
        "OperatorSerializationUtil:: serialized SinkDescriptor as "
        "SerializableOperator_SinkDetails_"
        "SerializableNullOutputSinkDescriptor");
    auto serializedSinkDescriptor =
    ...
    auto monitoringSinkDescriptor =
        sinkDescriptor.as<const MonitoringSinkDescriptor>();
    auto serializedSinkDescriptor =
        SerializableOperator_SinkDetails_SerializableMonitoringSinkDescriptor();
    serializedSinkDescriptor.set_collectortype(
        magic_enum::enum_integer(monitoringSinkDescriptor->getCollectorType()));
    sinkDetails.mutable_sinkdescriptor()->PackFrom(serializedSinkDescriptor);
    sinkDetails.set_numberoforiginids(numberOfOrigins);
  }
#ifdef ENABLE_OPC_BUILD
    ...
    auto opcSerializedSinkDescriptor =
        SerializableOperator_SinkDetails_SerializableOPCSinkDescriptor();
    char* ident = (char*)UA_malloc(
        sizeof(char) * opcSinkDescriptor->getNodeId().identifier.string.length +
        1);
    ...
    opcSerializedSinkDescriptor.set_namespaceindex(
        opcSinkDescriptor->getNodeId().namespaceIndex);
    opcSerializedSinkDescriptor.set_identifiertype(
        opcSinkDescriptor->getNodeId().identifierType);
    ...
    auto mqttSerializedSinkDescriptor =
        SerializableOperator_SinkDetails_SerializableMQTTSinkDescriptor();
    mqttSerializedSinkDescriptor.set_address(mqttSinkDescriptor->getAddress());
    mqttSerializedSinkDescriptor.set_clientid(
        mqttSinkDescriptor->getClientId());
    ...
    mqttSerializedSinkDescriptor.set_maxbufferedmsgs(
        mqttSinkDescriptor->getMaxBufferedMSGs());
    mqttSerializedSinkDescriptor.set_timeunit(
        (SerializableOperator_SinkDetails_SerializableMQTTSinkDescriptor_TimeUnits)
            mqttSinkDescriptor->getTimeUnit());
    mqttSerializedSinkDescriptor.set_msgdelay(
        mqttSinkDescriptor->getMsgDelay());
    mqttSerializedSinkDescriptor.set_asynchronousclient(
        mqttSinkDescriptor->getAsynchronousClient());

    sinkDetails.mutable_sinkdescriptor()->PackFrom(
        mqttSerializedSinkDescriptor);
    sinkDetails.set_numberoforiginids(numberOfOrigins);
  }
#endif
#ifdef ENABLE_KAFKA_BUILD
    ...
    const auto kafkaSinkDescriptor =
        sinkDescriptor.as<const KafkaSinkDescriptor>();
    SerializableOperator_SinkDetails_SerializableKafkaSinkDescriptor
    ...
    serializedDescriptor.set_kafkaconnecttimeout(
    ...
    auto networkSinkDescriptor =
        sinkDescriptor.as<const Network::NetworkSinkDescriptor>();
    auto serializedSinkDescriptor =
        SerializableOperator_SinkDetails_SerializableNetworkSinkDescriptor();
    // set details of NesPartition
    auto* serializedNesPartition =
        serializedSinkDescriptor.mutable_nespartition();
    auto nesPartition = networkSinkDescriptor->getNesPartition();
    serializedNesPartition->set_queryid(
        nesPartition.getQueryId().getRawValue());
    serializedNesPartition->set_operatorid(
        nesPartition.getOperatorId().getRawValue());
    serializedNesPartition->set_partitionid(
        nesPartition.getPartitionId().getRawValue());
    serializedNesPartition->set_subpartitionid(
        nesPartition.getSubpartitionId().getRawValue());
    // set details of NodeLocation
    auto* serializedNodeLocation =
        serializedSinkDescriptor.mutable_nodelocation();
    ...
    serializedSinkDescriptor.set_retrytimes(
        networkSinkDescriptor->getRetryTimes());
    serializedSinkDescriptor.set_version(networkSinkDescriptor->getVersion());
    serializedSinkDescriptor.set_uniquenetworksinkdescriptorid(
        networkSinkDescriptor->getUniqueId().getRawValue());
    // pack to output
    sinkDetails.mutable_sinkdescriptor()->PackFrom(serializedSinkDescriptor);
    sinkDetails.set_numberoforiginids(numberOfOrigins);
  } else if (sinkDescriptor.instanceOf<const FileSinkDescriptor>()) {
    // serialize file sink descriptor. The file sink has different types which
    ...
    serializedSinkDescriptor.set_addtimestamp(
    ...
    auto statisticSinkDescriptor =
        sinkDescriptor.as<const Statistic::StatisticSinkDescriptor>();
    SerializableOperator_SinkDetails_StatisticSinkDescriptor
        sinkDescriptorMessage;
    ...
    NES_ERROR("OperatorSerializationUtil: Unknown Sink Descriptor Type - {}",
              sinkDescriptor.toString());
    throw std::invalid_argument("Unknown Sink Descriptor Type");
  }
}

SinkDescriptorPtr OperatorSerializationUtil::deserializeSinkDescriptor(
    const SerializableOperator_SinkDetails& sinkDetails) {
  // de-serialize a sink descriptor and all its properties to a SinkDescriptor.
  NES_TRACE("OperatorSerializationUtil:: de-serialized SinkDescriptor {}",
            sinkDetails.DebugString());
  ...
  if (deserializedSinkDescriptor.Is<
          SerializableOperator_SinkDetails_SerializablePrintSinkDescriptor>()) {
    // de-serialize print sink descriptor
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialized SinkDescriptor as "
        "PrintSinkDescriptor");
  ...
  if (deserializedSinkDescriptor.Is<
    ...
    return ZmqSinkDescriptor::create(
        serializedSinkDescriptor.host(), serializedSinkDescriptor.port(),
        serializedSinkDescriptor.isinternal(), deserializedNumberOfOrigins);
  } else if (
      deserializedSinkDescriptor.Is<
          SerializableOperator_SinkDetails_SerializableMonitoringSinkDescriptor>()) {
    // de-serialize zmq sink descriptor
    ...
    char* ident = (char*)UA_malloc(
        sizeof(char) * opcSerializedSinkDescriptor.identifier().length() + 1);
    memcpy(ident, opcSerializedSinkDescriptor.identifier().data(),
           opcSerializedSinkDescriptor.identifier().length());
    ident[opcSerializedSinkDescriptor.identifier().length()] = '\0';
    UA_NodeId nodeId =
        UA_NODEID_STRING(opcSerializedSinkDescriptor.namespaceindex(), ident);
    return OPCSinkDescriptor::create(opcSerializedSinkDescriptor.url(), nodeId,
    ...
    return MQTTSinkDescriptor::create(
        std::string{serializedSinkDescriptor.address()},
        ...
        (MQTTSinkDescriptor::ServiceQualities)
            serializedSinkDescriptor.qualityofservice(),
    ...
    SerializableOperator_SinkDetails_SerializableKafkaSinkDescriptor
        serializedSinkDescriptor;
    deserializedSinkDescriptor.UnpackTo(&serializedSinkDescriptor);
    return KafkaSinkDescriptor::create(
        "CSV_FORMAT", std::string{serializedSinkDescriptor.topic()},
    ...
    auto waitTime =
        std::chrono::milliseconds(serializedSinkDescriptor.waittime());
    return Network::NetworkSinkDescriptor::create(
        nodeLocation, nesPartition, waitTime,
        serializedSinkDescriptor.retrytimes(),
        serializedSinkDescriptor.version(), deserializedNumberOfOrigins,
    ...
    return FileSinkDescriptor::create(
        ...
        serializedSinkDescriptor.addtimestamp(), deserializedNumberOfOrigins);
  } else if (deserializedSinkDescriptor.Is<
                 SerializableOperator_SinkDetails_StatisticSinkDescriptor>()) {
    SerializableOperator_SinkDetails_StatisticSinkDescriptor
        serializedSinkDescriptor;
    deserializedSinkDescriptor.UnpackTo(&serializedSinkDescriptor);
    return Statistic::StatisticSinkDescriptor::create(
        // Be careful changing the order of the enum values, as this works as
        // long as the order is the same in the cpp and the proto file
        (Statistic::StatisticSynopsisType)
            serializedSinkDescriptor.sinkformattype(),
...
void OperatorSerializationUtil::serializeLimitOperator(
    const LogicalLimitOperator& limitOperator,
    SerializableOperator& serializedOperator) {
...
LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeLimitOperator(
    const SerializableOperator_LimitDetails& limitDetails) {
  return LogicalOperatorFactory::createLimitOperator(limitDetails.limit(),
                                                     getNextOperatorId());
}
...
void OperatorSerializationUtil::serializeWatermarkAssignerOperator(
    ...
    SerializableOperator& serializedOperator) {
  NES_TRACE(
      "OperatorSerializationUtil:: serialize watermark assigner operator ");

  auto watermarkStrategyDetails =
      SerializableOperator_WatermarkStrategyDetails();
  auto watermarkStrategyDescriptor =
      watermarkAssignerOperator.getWatermarkStrategyDescriptor();
  serializeWatermarkStrategyDescriptor(*watermarkStrategyDescriptor,
                                       watermarkStrategyDetails);
  serializedOperator.mutable_details()->PackFrom(watermarkStrategyDetails);
}

LogicalUnaryOperatorPtr
OperatorSerializationUtil::deserializeWatermarkAssignerOperator(
    const SerializableOperator_WatermarkStrategyDetails&
        watermarkStrategyDetails) {
  auto watermarkStrategyDescriptor =
      deserializeWatermarkStrategyDescriptor(watermarkStrategyDetails);
  return LogicalOperatorFactory::createWatermarkAssignerOperator(
      watermarkStrategyDescriptor, getNextOperatorId());
}
...
void OperatorSerializationUtil::serializeWatermarkStrategyDescriptor(
  ...
  if (watermarkStrategyDescriptor.instanceOf<
          const Windowing::EventTimeWatermarkStrategyDescriptor>()) {
    auto eventTimeWatermarkStrategyDescriptor =
        watermarkStrategyDescriptor
            .as<const Windowing::EventTimeWatermarkStrategyDescriptor>();
    ...
    ExpressionSerializationUtil::serializeExpression(
        eventTimeWatermarkStrategyDescriptor->getOnField(),
    ...
    serializedWatermarkStrategyDescriptor.set_multiplier(
        eventTimeWatermarkStrategyDescriptor->getTimeUnit()
            .getMillisecondsConversionMultiplier());
    watermarkStrategyDetails.mutable_strategy()->PackFrom(
        serializedWatermarkStrategyDescriptor);
  } else if (watermarkStrategyDescriptor.instanceOf<
                 const Windowing::IngestionTimeWatermarkStrategyDescriptor>()) {
    auto serializedWatermarkStrategyDescriptor =
    ...
    NES_ERROR(
        "OperatorSerializationUtil: Unknown Watermark Strategy Descriptor "
        "Type");
...
OperatorSerializationUtil::deserializeWatermarkStrategyDescriptor(
    const SerializableOperator_WatermarkStrategyDetails&
        watermarkStrategyDetails) {
  NES_TRACE("OperatorSerializationUtil:: de-serialize watermark strategy ");
  const auto& deserializedWatermarkStrategyDescriptor =
      watermarkStrategyDetails.strategy();
  if (deserializedWatermarkStrategyDescriptor.Is<
          SerializableOperator_WatermarkStrategyDetails_SerializableEventTimeWatermarkStrategyDescriptor>()) {
    // de-serialize print sink descriptor
    NES_TRACE(
        "OperatorSerializationUtil:: de-serialized WatermarkStrategy as "
        "EventTimeWatermarkStrategyDescriptor");
    auto serializedEventTimeWatermarkStrategyDescriptor =
        SerializableOperator_WatermarkStrategyDetails_SerializableEventTimeWatermarkStrategyDescriptor();
    deserializedWatermarkStrategyDescriptor.UnpackTo(
        &serializedEventTimeWatermarkStrategyDescriptor);

    auto onField = ExpressionSerializationUtil::deserializeExpression(
                       serializedEventTimeWatermarkStrategyDescriptor.onfield())
                       ->as<FieldAccessExpressionNode>();
    NES_DEBUG("OperatorSerializationUtil:: deserialized field name {}",
              onField->getFieldName());
    auto eventTimeWatermarkStrategyDescriptor =
        Windowing::EventTimeWatermarkStrategyDescriptor::create(
            FieldAccessExpressionNode::create(onField->getFieldName()),
            Windowing::TimeMeasure(
                serializedEventTimeWatermarkStrategyDescriptor
                    .allowedlateness()),
            Windowing::TimeUnit(
                serializedEventTimeWatermarkStrategyDescriptor.multiplier()));
  ...
  if (deserializedWatermarkStrategyDescriptor.Is<
    ...
    NES_ERROR(
        "OperatorSerializationUtil: Unknown Serialized Watermark Strategy "
        "Descriptor Type");
    throw std::invalid_argument(
        "Unknown Serialized Watermark Strategy Descriptor Type");
  }
}

void OperatorSerializationUtil::serializeInputSchema(
    const OperatorPtr& operatorNode, SerializableOperator& serializedOperator) {
  NES_TRACE("OperatorSerializationUtil:: serialize input schema");
  if (!operatorNode->instanceOf<BinaryOperator>()) {
    SchemaSerializationUtil::serializeSchema(
        operatorNode->as<UnaryOperator>()->getInputSchema(),
        serializedOperator.mutable_inputschema());
  } else {
    auto binaryOperator = operatorNode->as<BinaryOperator>();
    SchemaSerializationUtil::serializeSchema(
        binaryOperator->getLeftInputSchema(),
        serializedOperator.mutable_leftinputschema());
    SchemaSerializationUtil::serializeSchema(
        binaryOperator->getRightInputSchema(),
...
void OperatorSerializationUtil::deserializeInputSchema(
  ...
  if (!operatorNode->instanceOf<BinaryOperator>()) {
    operatorNode->as<UnaryOperator>()->setInputSchema(
        SchemaSerializationUtil::deserializeSchema(
            serializedOperator.inputschema()));
  } else {
    auto binaryOperator = operatorNode->as<BinaryOperator>();
    binaryOperator->setLeftInputSchema(
        SchemaSerializationUtil::deserializeSchema(
            serializedOperator.leftinputschema()));
    binaryOperator->setRightInputSchema(
        SchemaSerializationUtil::deserializeSchema(
            serializedOperator.rightinputschema()));
  }
}

void OperatorSerializationUtil::serializeInferModelOperator(
    const InferModel::LogicalInferModelOperator& inferModelOperator,
    SerializableOperator& serializedOperator) {
  // serialize infer model operator
  NES_TRACE(
      "OperatorSerializationUtil:: serialize to LogicalInferModelOperator");
...
LogicalUnaryOperatorPtr
OperatorSerializationUtil::deserializeInferModelOperator(
    const SerializableOperator_InferModelDetails& inferModelDetails) {
  ...
  for (const auto& serializedInputField : inferModelDetails.inputfields()) {
    auto inputField = ExpressionSerializationUtil::deserializeExpression(
        serializedInputField);
    inputFields.push_back(inputField);
  }
  for (const auto& serializedOutputField : inferModelDetails.outputfields()) {
    auto outputField = ExpressionSerializationUtil::deserializeExpression(
  ...
  return LogicalOperatorFactory::createInferModelOperator(
      inferModelDetails.mlfilename(), inputFields, outputFields,
      getNextOperatorId());
}

template <typename T, typename D>
void OperatorSerializationUtil::serializeJavaUDFOperator(
    const T& javaUdfOperator, SerializableOperator& serializedOperator) {
  NES_TRACE("OperatorSerializationUtil:: serialize {}",
            javaUdfOperator.toString());
  auto javaUdfDetails = D();
  UDFSerializationUtil::serializeJavaUDFDescriptor(
      javaUdfOperator.getUDFDescriptor(),
      *javaUdfDetails.mutable_javaudfdescriptor());
  serializedOperator.mutable_details()->PackFrom(javaUdfDetails);
}

LogicalUnaryOperatorPtr
OperatorSerializationUtil::deserializeMapJavaUDFOperator(
    const SerializableOperator_MapJavaUdfDetails& mapJavaUdfDetails) {
  auto javaUDFDescriptor = UDFSerializationUtil::deserializeJavaUDFDescriptor(
      mapJavaUdfDetails.javaudfdescriptor());
  return LogicalOperatorFactory::createMapUDFLogicalOperator(javaUDFDescriptor);
}

LogicalUnaryOperatorPtr
OperatorSerializationUtil::deserializeFlatMapJavaUDFOperator(
    const SerializableOperator_FlatMapJavaUdfDetails& flatMapJavaUdfDetails) {
  auto javaUDFDescriptor = UDFSerializationUtil::deserializeJavaUDFDescriptor(
      flatMapJavaUdfDetails.javaudfdescriptor());
  return LogicalOperatorFactory::createFlatMapUDFLogicalOperator(
      javaUDFDescriptor);
}

void OperatorSerializationUtil::serializeOpenCLOperator(
    const LogicalOpenCLOperator& openCLLogicalOperator,
    SerializableOperator& serializedOperator) {
  NES_TRACE("OperatorSerializationUtil:: serialize to LogicalOpenCLOperator");
  auto openCLDetails = SerializableOperator_OpenCLOperatorDetails();
  UDFSerializationUtil::serializeJavaUDFDescriptor(
      openCLLogicalOperator.getJavaUDFDescriptor(),
...
LogicalUnaryOperatorPtr OperatorSerializationUtil::deserializeOpenCLOperator(
    const SerializableOperator_OpenCLOperatorDetails& openCLDetails) {
  auto javaUDFDescriptor = UDFSerializationUtil::deserializeJavaUDFDescriptor(
      openCLDetails.javaudfdescriptor());
  auto openCLOperator =
      LogicalOperatorFactory::createOpenCLLogicalOperator(javaUDFDescriptor)
          ->as<LogicalOpenCLOperator>();
...
void OperatorSerializationUtil::serializeStatisticWindowOperator(
  ...
  if (windowType->instanceOf<Windowing::TumblingWindow>()) {
    ...
    tumblingWindowDetails.mutable_timecharacteristic()->CopyFrom(
        timeCharacteristicDetails);
    tumblingWindowDetails.set_size(tumblingWindow->getSize().getTime());
    statisticWindowDetails.mutable_windowtype()->PackFrom(
        tumblingWindowDetails);
  } else if (windowType->instanceOf<Windowing::SlidingWindow>()) {
    auto slidingWindow = windowType->as<Windowing::SlidingWindow>();
    auto slidingWindowDetails = SerializableOperator_SlidingWindow();
    slidingWindowDetails.mutable_timecharacteristic()->CopyFrom(
  ...
  auto statisticWindowDescriptor =
      statisticWindowOperator.getWindowStatisticDescriptor();
  ExpressionSerializationUtil::serializeExpression(
      statisticWindowDescriptor->getField(),
      statisticWindowDescriptorMessage.mutable_field());
  StatisticSerializationUtil::serializeDescriptorDetails(
      *statisticWindowDescriptor, statisticWindowDescriptorMessage);
  statisticWindowDescriptorMessage.set_width(
      statisticWindowDescriptor->getWidth());
  statisticWindowDetails.mutable_statisticwindowdescriptor()->CopyFrom(
  ...
  StatisticSerializationUtil::serializeSendingPolicy(
      *statisticWindowOperator.getSendingPolicy(),
      *statisticWindowDetails.mutable_sendingpolicy());
  StatisticSerializationUtil::serializeTriggerCondition(
      *statisticWindowOperator.getTriggerCondition(),
      *statisticWindowDetails.mutable_triggercondition());

  // 4. Serializing the metric hash and then packing everything into
  // serializedOperator
  statisticWindowDetails.set_metrichash(
      statisticWindowOperator.getMetricHash());
  serializedOperator.mutable_details()->PackFrom(statisticWindowDetails);
}

LogicalUnaryOperatorPtr
OperatorSerializationUtil::deserializeStatisticWindowOperator(
  ...
  auto sendingPolicy = StatisticSerializationUtil::deserializeSendingPolicy(
      statisticWindowDetails.sendingpolicy());
  auto triggerCondition =
      StatisticSerializationUtil::deserializeTriggerCondition(
          statisticWindowDetails.triggercondition());

  // 3. Deserializing the statistic descriptor, the metric hash, and then
  // creating the operator
  auto statisticDescriptor = StatisticSerializationUtil::deserializeDescriptor(
      statisticWindowDetails.statisticwindowdescriptor());
  ...
  auto statisticWindowOperator =
      LogicalOperatorFactory::createStatisticBuildOperator(
          window, statisticDescriptor, metricHash, sendingPolicy,
          triggerCondition);
