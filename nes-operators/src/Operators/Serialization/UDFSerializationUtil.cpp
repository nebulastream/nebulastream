...
void UDFSerializationUtil::serializeJavaUDFDescriptor(
  ...
  JavaUdfDescriptorMessage.set_input_class_name(
      javaUDFDescriptor->getInputClassName());
  JavaUdfDescriptorMessage.set_output_class_name(
      javaUDFDescriptor->getOutputClassName());
}

Catalogs::UDF::JavaUDFDescriptorPtr
UDFSerializationUtil::deserializeJavaUDFDescriptor(
    const JavaUdfDescriptorMessage& JavaUdfDescriptorMessage) {
  // C++ represents the bytes type of serialized_instance and byte_code as
  // std::strings which have to be converted to typed byte arrays.
  auto serializedInstance = jni::JavaSerializedInstance{
      JavaUdfDescriptorMessage.serialized_instance().begin(),
  ...
  for (const auto& classDefinition : JavaUdfDescriptorMessage.classes()) {
    ...
    javaUdfByteCodeList.emplace_back(
        ...
        jni::JavaByteCode{classDefinition.byte_code().begin(),
                          classDefinition.byte_code().end()});
  }
  // Deserialize the input and output schema.
  auto inputSchema = SchemaSerializationUtil::deserializeSchema(
      JavaUdfDescriptorMessage.inputschema());
  auto outputSchema = SchemaSerializationUtil::deserializeSchema(
      JavaUdfDescriptorMessage.outputschema());
  // Create Java UDF descriptor.
  return Catalogs::UDF::JavaUDFDescriptor::create(
      JavaUdfDescriptorMessage.udf_class_name(),
      JavaUdfDescriptorMessage.udf_method_name(), serializedInstance,
      javaUdfByteCodeList, inputSchema, outputSchema,
