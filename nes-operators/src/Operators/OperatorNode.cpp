...
bool OperatorNode::hasMultipleChildrenOrParents() {
    ...
    NES_DEBUG("OperatorNode: has multiple children {} or has multiple parent {}", hasMultipleChildren, hasMultipleParent);
...
bool OperatorNode::addChild(NodePtr newNode) {
    ...
    if (found == currentChildren.end()) {
        NES_TRACE("OperatorNode: Adding node {} to the children.", newNode->toString());
        children.push_back(newNode);
        newNode->addParent(shared_from_this());
        return true;
    }
    NES_TRACE("OperatorNode: the node is already part of its children so skip add child operation.");
    return false;
}
...
bool OperatorNode::addParent(NodePtr newNode) {
    ...
    if (found == currentParents.end()) {
        NES_TRACE("OperatorNode: Adding node {} to the Parents.", newNode->toString());
        parents.push_back(newNode);
        newNode->addChild(shared_from_this());
        return true;
    }
    NES_TRACE("OperatorNode: the node is already part of its parent so skip add parent operation.");
    return false;
}
