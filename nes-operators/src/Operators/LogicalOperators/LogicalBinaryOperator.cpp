...
bool LogicalBinaryOperator::inferSchema() {
  ...
  if (children.size() < 2) {
    NES_ERROR("BinaryOperator: this operator should have at least two child "
              "operators");
...
std::vector<OperatorPtr>
LogicalBinaryOperator::getOperatorsBySchema(const SchemaPtr &schema) const {
