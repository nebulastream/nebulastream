...
LogicalFilterOperator::LogicalFilterOperator(ExpressionNodePtr const &predicate,
                                             OperatorId id)
...
bool LogicalFilterOperator::inferSchema() {
  ...
  if (!predicate->isPredicate()) {
    NES_THROW_RUNTIME_ERROR("FilterLogicalOperator: the filter expression is "
...
OperatorPtr LogicalFilterOperator::copy() {
  auto copy =
      LogicalOperatorFactory::createFilterOperator(predicate->copy(), id);
...
void LogicalFilterOperator::inferStringSignature() {
  ...
  NES_TRACE("LogicalFilterOperator: Inferring String signature for {}",
            operatorNode->toString());
  NES_ASSERT(!children.empty(),
             "LogicalFilterOperator: Filter should have children");
  ...
  signatureStream << "FILTER(" + predicate->toString() + ")."
...
LogicalFilterOperator::getFieldNamesUsedByFilterPredicate() const {
  NES_TRACE(
      "LogicalFilterOperator: Find all field names used in filter operator");
  ...
  for (auto itr = depthFirstNodeIterator.begin();
    ...
    // if it finds a fieldAccessExpressionNode this means that the predicate
    // uses this specific field that comes from any source
