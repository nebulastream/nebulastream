...
std::string StatisticSinkDescriptor::toString() const {
  return fmt::format("StatisticSinkDescriptor({})",
                     magic_enum::enum_name(sinkFormatType));
}

SinkDescriptorPtr
StatisticSinkDescriptor::create(StatisticSynopsisType sinkFormatType,
                                StatisticDataCodec sinkDataCodec,
                                uint64_t numberOfOrigins) {
  return std::make_shared<StatisticSinkDescriptor>(
      StatisticSinkDescriptor(sinkFormatType, sinkDataCodec, numberOfOrigins));
}

bool StatisticSinkDescriptor::equal(const SinkDescriptorPtr &other) {
  if (other->instanceOf<StatisticSinkDescriptor>()) {
    const auto otherStatisticSinkDescriptor =
        other->as<StatisticSinkDescriptor>();
...
StatisticSinkDescriptor::StatisticSinkDescriptor(
    StatisticSynopsisType sinkFormatType, StatisticDataCodec sinkDataCodec,
    uint64_t numberOfOrigins)
    : SinkDescriptor(numberOfOrigins), sinkFormatType(sinkFormatType),
