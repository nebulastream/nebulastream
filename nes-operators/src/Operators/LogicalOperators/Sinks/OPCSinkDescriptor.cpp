...
OPCSinkDescriptor::OPCSinkDescriptor(std::string url, UA_NodeId nodeId,
                                     std::string user, std::string password)
    : url(std::move(url)), nodeId(std::move(nodeId)), user(std::move(user)),
      password(std::move(password)) {

  NES_DEBUG("OPCSINKDESCRIPTOR {} : Init OPC Sink descriptor.",
            this->toString());
}
...
SinkDescriptorPtr OPCSinkDescriptor::create(std::string url, UA_NodeId nodeId,
                                            std::string user,
                                            std::string password) {
  return std::make_shared<OPCSinkDescriptor>(OPCSinkDescriptor(
      std::move(url), std::move(nodeId), std::move(user), std::move(password)));
}

std::string OPCSinkDescriptor::toString() const {
...
bool OPCSinkDescriptor::equal(SinkDescriptorPtr const &other) {
  ...
  memcpy(newIdent, nodeId.identifier.string.data,
         nodeId.identifier.string.length);
  newIdent[nodeId.identifier.string.length] = '\0';
  char *otherSinkIdent = (char *)UA_malloc(
      sizeof(char) * otherSinkDescriptor->getNodeId().identifier.string.length +
      1);
  ...
  otherSinkIdent[otherSinkDescriptor->getNodeId().identifier.string.length] =
      '\0';
  return url == otherSinkDescriptor->getUrl() &&
         !strcmp(newIdent, otherSinkIdent) &&
         nodeId.namespaceIndex ==
             otherSinkDescriptor->getNodeId().namespaceIndex &&
         nodeId.identifierType ==
             otherSinkDescriptor->getNodeId().identifierType &&
         user == otherSinkDescriptor->getUser() &&
