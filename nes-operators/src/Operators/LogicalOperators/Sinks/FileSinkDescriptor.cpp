...
SinkDescriptorPtr FileSinkDescriptor::create(std::string_view fileName,
  ...
  return std::make_shared<FileSinkDescriptor>(FileSinkDescriptor(
      fileName, sinkFormat, append == "APPEND", addTimestamp, numberOfOrigins));
}

SinkDescriptorPtr FileSinkDescriptor::create(const std::string &fileName,
                                             const std::string &sinkFormat,
                                             const std::string &append) {
  return create(fileName, sinkFormat, append, false, 1);
}

FileSinkDescriptor::FileSinkDescriptor(std::string_view fileName,
                                       std::string_view sinkFormat, bool append,
                                       bool addTimestamp,
                                       uint64_t numberOfOrigins)
    : SinkDescriptor(numberOfOrigins, addTimestamp), fileName(fileName),
      sinkFormat(sinkFormat), append(append) {}
