...
NetworkSourceDescriptor::NetworkSourceDescriptor(
    SchemaPtr schema, const NesPartition &nesPartition,
    const NodeLocation &nodeLocation, std::chrono::milliseconds waitTime,
    uint32_t retryTimes, DecomposedQueryPlanVersion version,
    OperatorId uniqueNetworkSourceId)
    : SourceDescriptor(std::move(schema)), nesPartition(nesPartition),
      nodeLocation(nodeLocation), waitTime(waitTime), retryTimes(retryTimes),
      version(version), uniqueNetworkSourceId(uniqueNetworkSourceId) {}
...
SourceDescriptorPtr NetworkSourceDescriptor::create(
    ...
    uint32_t retryTimes, DecomposedQueryPlanVersion version,
    OperatorId uniqueNetworkSourceId) {
  return std::make_shared<NetworkSourceDescriptor>(NetworkSourceDescriptor(
      std::move(schema), nesPartition, nodeLocation, waitTime, retryTimes,
      version, uniqueNetworkSourceId));
}
...
bool NetworkSourceDescriptor::equal(SourceDescriptorPtr const &other) const {
  ...
  return schema->equals(otherNetworkSource->schema) &&
         nesPartition == otherNetworkSource->nesPartition;
}

std::string NetworkSourceDescriptor::toString() const {
  return fmt::format("NetworkSourceDescriptor{{Version={};Partition={};"
                     "NetworkSinkNodeLocation={}}}",
                     version, nesPartition.toString(),
...
SourceDescriptorPtr NetworkSourceDescriptor::copy() {
  auto copy = NetworkSourceDescriptor::create(
      schema->copy(), nesPartition, nodeLocation, waitTime, retryTimes, version,
