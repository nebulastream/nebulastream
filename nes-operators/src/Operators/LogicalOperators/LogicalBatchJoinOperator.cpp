...
bool LogicalBatchJoinOperator::inferSchema() {
  ...
  if (!leftInputSchema) {
    NES_ERROR(
        "LogicalBatchJoinOperator: Left input schema is not initialized. Make "
        "sure that left join key is present: {}",
        buildJoinKeyName);
    throw TypeInferenceException(
  ...
  if (!rightInputSchema) {
    NES_ERROR(
        "LogicalBatchJoinOperator: Right input schema is not initialized. Make "
        "sure that right join key is present: {}",
        probeJoinKeyName);
    throw TypeInferenceException(
  ...
  if (rightInputSchema->equals(leftInputSchema, false)) {
    NES_ERROR(
        "LogicalBatchJoinOperator: Found both left and right input schema to "
        "be same.");
    throw TypeInferenceException(
        "LogicalBatchJoinOperator: Found both left and right input schema to "
        "be same.");
