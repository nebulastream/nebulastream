...
WatermarkAssignerLogicalOperator::getWatermarkStrategyDescriptor() const {
...
bool WatermarkAssignerLogicalOperator::isIdentical(NodePtr const& rhs) const {
  return equal(rhs) &&
         rhs->as<WatermarkAssignerLogicalOperator>()->getId() == id;
}
...
bool WatermarkAssignerLogicalOperator::equal(NodePtr const& rhs) const {
  if (rhs->instanceOf<WatermarkAssignerLogicalOperator>()) {
    auto watermarkAssignerOperator =
        rhs->as<WatermarkAssignerLogicalOperator>();
    return watermarkStrategyDescriptor->equal(
...
OperatorPtr WatermarkAssignerLogicalOperator::copy() {
  auto copy = LogicalOperatorFactory::createWatermarkAssignerOperator(
...
void WatermarkAssignerLogicalOperator::inferStringSignature() {
  ...
  signatureStream << "WATERMARKASSIGNER("
