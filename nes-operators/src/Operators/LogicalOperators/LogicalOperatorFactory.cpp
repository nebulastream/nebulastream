    ...
    OperatorId id)
{
    return std::make_shared<Statistic::LogicalStatisticWindowOperator>(
        id, window, statisticDescriptor, metricHash, sendingPolicy, triggerCondition);
}
...
LogicalBinaryOperatorPtr LogicalOperatorFactory::createBatchJoinOperator(
    const Join::Experimental::LogicalBatchJoinDescriptorPtr & batchJoinDefinition, OperatorId id)
{
