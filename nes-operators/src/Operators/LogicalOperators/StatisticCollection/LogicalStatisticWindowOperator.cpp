...
LogicalStatisticWindowOperator::LogicalStatisticWindowOperator(
    ...
    : Operator(id), LogicalUnaryOperator(id), windowType(std::move(windowType)),
      ...
      metricHash(metricHash), sendingPolicy(sendingPolicy),
...
bool LogicalStatisticWindowOperator::inferSchema() {
  ...
  // Inferring the stamp for the windowType and the fieldToTrackStatisticsOver
  // that is part of the descriptor
  ...
  outputSchema->clear();
  outputSchema->addField(qualifierNameWithSeparator + BASE_FIELD_NAME_START,
                         UINT64);
  outputSchema->addField(qualifierNameWithSeparator + BASE_FIELD_NAME_END,
                         UINT64);
  outputSchema->addField(qualifierNameWithSeparator + STATISTIC_HASH_FIELD_NAME,
                         UINT64);
  outputSchema->addField(qualifierNameWithSeparator + STATISTIC_TYPE_FIELD_NAME,
                         UINT64);
  outputSchema->addField(
      qualifierNameWithSeparator + OBSERVED_TUPLES_FIELD_NAME, UINT64);
  windowStatisticDescriptor->addDescriptorFields(*outputSchema,
                                                 qualifierNameWithSeparator);
...
bool LogicalStatisticWindowOperator::equal(const NodePtr &rhs) const {
  if (rhs->instanceOf<LogicalStatisticWindowOperator>()) {
    ...
    return windowType->equal(rhsStatisticOperatorNode->windowType) &&
           ...
           windowStatisticDescriptor->equal(
               rhsStatisticOperatorNode->windowStatisticDescriptor) &&
...
std::string LogicalStatisticWindowOperator::toString() const {
  return fmt::format(
      "LogicalStatisticWindowOperator({}, {}): Windowtype: {} Descriptor: {} "
      "InputOriginIds: {} MetricHash: {} "
      "SendingPolicy: {} TriggerCondition: {}",
      id, statisticId, windowType->toString(),
      windowStatisticDescriptor->toString(),
      fmt::join(inputOriginIds.begin(), inputOriginIds.end(), ", "), metricHash,
      sendingPolicy->toString(), triggerCondition->toString());
}

OperatorPtr LogicalStatisticWindowOperator::copy() {
  auto copy = LogicalOperatorFactory::createStatisticBuildOperator(
      windowType, windowStatisticDescriptor, metricHash, sendingPolicy,
      triggerCondition, id);
...
LogicalStatisticWindowOperator::getWindowStatisticDescriptor() const {
...
TriggerConditionPtr
LogicalStatisticWindowOperator::getTriggerCondition() const {
  return triggerCondition;
}

void LogicalStatisticWindowOperator::inferStringSignature() {
  auto op = shared_from_this()->as<Operator>();
  NES_TRACE("StatisticWindowOperatorNode: Inferring String signature for {}",
            op->toString());

  std::stringstream signatureStream;
  signatureStream << "STATISTIC_BUILD_OPERATOR(" +
                         windowStatisticDescriptor->toString() + ").";
