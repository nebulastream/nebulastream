...
LogicalProjectionOperator::LogicalProjectionOperator(
    std::vector<ExpressionNodePtr> expressions, OperatorId id)
    : Operator(id), LogicalUnaryOperator(id),
      expressions(std::move(expressions)) {}

std::vector<ExpressionNodePtr>
LogicalProjectionOperator::getExpressions() const {
  return expressions;
}
...
bool LogicalProjectionOperator::inferSchema() {
  ...
  NES_DEBUG("proj input={}  outputSchema={} this proj={}",
            inputSchema->toString(), outputSchema->toString(), toString());
  outputSchema->clear();
  for (const auto &expression : expressions) {
    ...
    if (expression->instanceOf<FieldRenameExpressionNode>()) {
      ...
      outputSchema->addField(fieldRename->getNewFieldName(),
                             fieldRename->getStamp());
    } else if (expression->instanceOf<FieldAccessExpressionNode>()) {
      auto fieldAccess = expression->as<FieldAccessExpressionNode>();
      outputSchema->addField(fieldAccess->getFieldName(),
                             fieldAccess->getStamp());
    } else {
      NES_ERROR("LogicalProjectionOperator: Expression has to be an "
                "FieldAccessExpression or a FieldRenameExpression "
                "but it was a {}",
                expression->toString());
      throw TypeInferenceException("LogicalProjectionOperator: Expression has "
                                   "to be an FieldAccessExpression or a "
...
OperatorPtr LogicalProjectionOperator::copy() {
  ...
  auto copy = LogicalOperatorFactory::createProjectionOperator(
      copyOfProjectionExpressions, id);
...
void LogicalProjectionOperator::inferStringSignature() {
  ...
  NES_TRACE("LogicalProjectionOperator: Inferring String signature for {}",
            operatorNode->toString());
  NES_ASSERT(!children.empty(),
             "LogicalProjectionOperator: Project should have children.");
