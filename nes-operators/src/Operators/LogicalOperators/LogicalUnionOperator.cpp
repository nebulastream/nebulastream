...
LogicalUnionOperator::LogicalUnionOperator(OperatorId id)
    : Operator(id), LogicalBinaryOperator(id) {}
...
bool LogicalUnionOperator::inferSchema() {
  ...
  if (!leftInputSchema->hasEqualTypes(rightInputSchema)) {
    NES_ERROR("Found Schema mismatch for left and right schema types. Left "
              "schema {} and Right schema {} ",
              leftInputSchema->toString(), rightInputSchema->toString());
    throw TypeInferenceException(
        "Found Schema mismatch for left and right schema types. Left schema " +
        leftInputSchema->toString() + " and Right schema " +
  ...
  if (leftInputSchema->getLayoutType() != rightInputSchema->getLayoutType()) {
    ...
    throw TypeInferenceException(
        "Left and right should have same memory layout");
  }
...
bool LogicalUnionOperator::equal(NodePtr const &rhs) const {
  if (rhs->instanceOf<LogicalUnionOperator>()) {
    ...
    return leftInputSchema->equals(rhsUnion->getLeftInputSchema()) &&
           outputSchema->equals(rhsUnion->getOutputSchema());
  }
  return false;
}
...
void LogicalUnionOperator::inferStringSignature() {
  ...
  NES_TRACE("LogicalUnionOperator: Inferring String signature for {}",
            operatorNode->toString());
  NES_ASSERT(!children.empty() && children.size() == 2,
             "LogicalUnionOperator: Union should have 2 children.");
  ...
  auto rightChildSignature =
      children[0]->as<LogicalOperator>()->getHashBasedSignature();
  auto leftChildSignature =
      children[1]->as<LogicalOperator>()->getHashBasedSignature();
...
void LogicalUnionOperator::inferInputOrigins() {
  ...
  for (auto child : this->children) {
    ...
    combinedInputOriginIds.insert(combinedInputOriginIds.end(),
