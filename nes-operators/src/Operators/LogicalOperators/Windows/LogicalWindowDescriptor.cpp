...
LogicalWindowDescriptor::LogicalWindowDescriptor(
    ...
    WindowTypePtr windowType, uint64_t allowedLateness)
    : windowAggregation(std::move(windowAggregation)),
      windowType(std::move(windowType)), onKey(keys),
...
LogicalWindowDescriptorPtr LogicalWindowDescriptor::create(
    const std::vector<WindowAggregationDescriptorPtr> &windowAggregations,
    const WindowTypePtr &windowType, uint64_t allowedLateness) {
...
LogicalWindowDescriptorPtr LogicalWindowDescriptor::create(
    ...
    std::vector<WindowAggregationDescriptorPtr> windowAggregation,
    const WindowTypePtr &windowType, uint64_t allowedLateness) {
  return std::make_shared<LogicalWindowDescriptor>(keys, windowAggregation,
                                                   windowType, allowedLateness);
}

uint64_t LogicalWindowDescriptor::getNumberOfInputEdges() const {
  return numberOfInputEdges;
}
void LogicalWindowDescriptor::setNumberOfInputEdges(
    uint64_t numberOfInputEdges) {
  this->numberOfInputEdges = numberOfInputEdges;
}
std::vector<WindowAggregationDescriptorPtr>
LogicalWindowDescriptor::getWindowAggregation() const {
...
LogicalWindowDescriptor::getKeys() const {
  return onKey;
}

void LogicalWindowDescriptor::setWindowAggregation(
    const std::vector<WindowAggregationDescriptorPtr> &windowAggregation) {
...
void LogicalWindowDescriptor::setOnKey(
...
bool LogicalWindowDescriptor::equal(
    LogicalWindowDescriptorPtr otherWindowDefinition) const {
  ...
  if (this->getWindowAggregation().size() !=
      otherWindowDefinition->getWindowAggregation().size()) {
    return false;
  }

  for (uint64_t i = 0; i < this->getWindowAggregation().size(); i++) {
    if (!this->getWindowAggregation()[i]->equal(
            otherWindowDefinition->getWindowAggregation()[i])) {
      return false;
    }
  }
...
const std::vector<OriginId> &
LogicalWindowDescriptor::getInputOriginIds() const {
  return inputOriginIds;
}
void LogicalWindowDescriptor::setInputOriginIds(
    const std::vector<OriginId> &inputOriginIds) {
