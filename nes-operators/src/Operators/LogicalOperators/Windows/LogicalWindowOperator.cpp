...
LogicalWindowOperator::LogicalWindowOperator(const Windowing::LogicalWindowDefinitionPtr& windowDefinition, OperatorId id)
...
bool LogicalWindowOperator::isIdentical(NodePtr const& rhs) const {
    return equal(rhs) && (rhs->as<LogicalWindowOperator>()->getId() == id) && !rhs->instanceOf<CentralWindowOperator>();
}
...
OperatorNodePtr LogicalWindowOperator::copy() {
