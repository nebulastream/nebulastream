...
LogicalJoinOperator::LogicalJoinOperator(
    Join::LogicalJoinDescriptorPtr joinDefinition, OperatorId id,
    OriginId originId)
...
bool LogicalJoinOperator::inferSchema() {
  ...
  if (distinctSchemas.size() != 2) {
    throw TypeInferenceException(
        fmt::format("LogicalJoinOperator: Found {} distinct schemas but "
                    "expected 2 distinct schemas.",
  ...
  // Finds the join schema that contains the joinKey and returns an iterator to
  // the schema
  auto findSchemaInDistinctSchemas = [&](FieldAccessExpressionNode &joinKey,
    ...
    for (auto itr = distinctSchemas.begin(); itr != distinctSchemas.end();) {
      ...
      if (joinKeyName.find(Schema::ATTRIBUTE_NAME_SEPARATOR) !=
          std::string::npos) {
  ...
  NES_DEBUG("LogicalJoinOperator: Iterate over all ExpressionNode to if check "
            "join field is in schema.");
  // Maintain a list of visited nodes as there are multiple root nodes
  std::unordered_set<std::shared_ptr<BinaryExpressionNode>> visitedExpressions;
  auto bfsIterator =
      BreadthFirstNodeIterator(joinDefinition->getJoinExpression());
    ...
    if ((*itr)->instanceOf<BinaryExpressionNode>()) {
      ...
      if (visitedExpressions.contains(visitingOp)) {
        ...
        if (!(*itr)
          ...
          const auto foundLeftKey =
              findSchemaInDistinctSchemas(*leftJoinKey, leftInputSchema);
          NES_ASSERT_THROW_EXCEPTION(
              foundLeftKey, TypeInferenceException,
              "LogicalJoinOperator: Unable to find left join key " +
                  leftJoinKeyName + " in schemas.");
          const auto rightJoinKey = (*itr)
                                        ->as<BinaryExpressionNode>()
                                        ->getRight()
                                        ->as<FieldAccessExpressionNode>();
          const auto rightJoinKeyName = rightJoinKey->getFieldName();
          const auto foundRightKey =
              findSchemaInDistinctSchemas(*rightJoinKey, rightInputSchema);
          NES_ASSERT_THROW_EXCEPTION(
              foundRightKey, TypeInferenceException,
              "LogicalJoinOperator: Unable to find right join key " +
                  rightJoinKeyName + " in schemas.");
  ...
  NES_ASSERT_THROW_EXCEPTION(!rightInputSchema->equals(leftInputSchema, false),
                             TypeInferenceException,
                             "LogicalJoinOperator: Found both left and right "
                             "input schema to be same.");
  ...
  outputSchema->clear();
  const auto &sourceNameLeft =
      leftInputSchema->getQualifierNameForSystemGeneratedFields();
  const auto &sourceNameRight =
      rightInputSchema->getQualifierNameForSystemGeneratedFields();
...
bool LogicalJoinOperator::equal(NodePtr const &rhs) const {
  if (rhs->instanceOf<LogicalJoinOperator>()) {
    ...
    return joinDefinition->getWindowType()->equal(
               rhsJoin->joinDefinition->getWindowType()) &&
           joinDefinition->getJoinExpression()->equal(
               rhsJoin->joinDefinition->getJoinExpression()) &&
           joinDefinition->getOutputSchema()->equals(
               rhsJoin->joinDefinition->getOutputSchema()) &&
           joinDefinition->getRightSourceType()->equals(
               rhsJoin->joinDefinition->getRightSourceType()) &&
           joinDefinition->getLeftSourceType()->equals(
               rhsJoin->joinDefinition->getLeftSourceType());
  }
  return false;
}
...
void LogicalJoinOperator::inferStringSignature() {
  ...
  NES_TRACE("LogicalJoinOperator: Inferring String signature for {}",
            operatorNode->toString());
  NES_ASSERT(!children.empty() && children.size() == 2,
             "LogicalJoinOperator: Join should have 2 children.");
...
void LogicalJoinOperator::setWindowStartEndKeyFieldName(
    std::string_view windowStartFieldName,
