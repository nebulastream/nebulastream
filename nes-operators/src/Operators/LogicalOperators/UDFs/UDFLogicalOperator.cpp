...
void UDFLogicalOperator::verifySchemaCompatibility(
  ...
  if (udfInputSchema.getSize() != childOperatorOutputSchema.getSize()) {
    errors.emplace_back(
        "UDF input schema and child operator output schema have different "
        "sizes.");
  }
  for (const auto& field : udfDescriptor->getInputSchema()->fields) {
    ...
    if (type->equals(DataTypeFactory::createInt64()) &&
      ...
      NES_WARNING(
          "Mapping UINT64 field in child operator output schema to signed Java "
          "long in UDF input schema: {}",
          fieldName)
      ...
      errors.push_back(
          fmt::format("Field data type differs in UDF input schema and child "
                      "operator output schema: "
                      "fieldName={}, udfType={}, parentType={}",
                      fieldName, type->toString(), childType->toString()));
    }
  }
  if (!errors.empty()) {
