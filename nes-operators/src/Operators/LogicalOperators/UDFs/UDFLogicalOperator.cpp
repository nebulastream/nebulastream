...
UDFLogicalOperator::UDFLogicalOperator(
...
void UDFLogicalOperator::inferStringSignature() {
  NES_TRACE("UDFLogicalOperator: Inferring String signature for {}",
            toString());
  NES_ASSERT(children.size() == 1,
             "UDFLogicalOperator should have exactly 1 child.");
...
bool UDFLogicalOperator::inferSchema() {
  ...
  // names
  const auto newQualifierName =
      inputSchema->getQualifierNameForSystemGeneratedFields() +
      Schema::ATTRIBUTE_NAME_SEPARATOR;
  ...
  verifySchemaCompatibility(udfDescriptor->getInputSchema(),
                            children[0]->as<Operator>()->getOutputSchema());
  return true;
}

Catalogs::UDF::UDFDescriptorPtr UDFLogicalOperator::getUDFDescriptor() const {
...
void UDFLogicalOperator::verifySchemaCompatibility(
    const Schema &udfInputSchema,
    const Schema &childOperatorOutputSchema) const {
  // The code below detects all schema violations, prints them to the ERROR
  // output log, then throws an exception containing all of them. This makes it
  // easier to users to fix all violations at once.
  std::vector<std::string> errors;
  if (udfInputSchema.getSize() != childOperatorOutputSchema.getSize()) {
    errors.emplace_back("UDF input schema and child operator output schema "
  ...
  for (const auto &field : udfDescriptor->getInputSchema()->fields) {
    ...
    if (!fieldInChild) {
      ...
      // In this case the UDF is assumed to not have a complex type but a boxed
      // simple type, e.g., Integer, which is not named. The UDF execution code
      // just reads the first attribute from the child operator output schema,
      // regardless of its name.
      if (udfInputSchema.getSize() == 1) {
        fieldInChild = childOperatorOutputSchema.fields[0];
      } else {
        errors.push_back(fmt::format(
            "Could not find field in child operator output schema: {}",
            fieldName));
        continue;
      }
    }
    ...
    if (type->equals(DataTypeFactory::createInt64()) &&
        childType->equals(DataTypeFactory::createUInt64())) {
      // This is not an error condition because we need to map timestamps, which
      // are always UINT64, to Java long.
      NES_WARNING("Mapping UINT64 field in child operator output schema to "
                  "signed Java long in UDF input schema: {}",
                  fieldName)
    } else if ((type->equals(DataTypeFactory::createInt8()) &&
                childType->equals(DataTypeFactory::createUInt8())) ||
               (type->equals(DataTypeFactory::createInt16()) &&
                childType->equals(DataTypeFactory::createUInt16())) ||
               (type->equals(DataTypeFactory::createInt32()) &&
                childType->equals(DataTypeFactory::createUInt32()))) {
      errors.push_back(
          fmt::format("Field data type is unsigned integer in child operator "
                      "output schema; UDFs only support signed integers: {}",
                      fieldName));
    } else if (!type->equals(childType)) {
      errors.push_back(fmt::format("Field data type differs in UDF input "
                                   "schema and child operator output schema: "
  ...
  if (!errors.empty()) {
    ...
    if (errors.size() == 1) {
      ...
      std::for_each(
          ...
          [&message](const std::string &error) { message << "\n- " << error; });
    }
