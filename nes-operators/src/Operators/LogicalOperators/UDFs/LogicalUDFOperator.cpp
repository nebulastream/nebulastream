...
LogicalUDFOperator::LogicalUDFOperator(const Catalogs::UDF::UDFDescriptorPtr udfDescriptor, OperatorId id)
    : OperatorNode(id), LogicalUnaryOperator(id), udfDescriptor(udfDescriptor) {}

void LogicalUDFOperator::inferStringSignature() {
    NES_TRACE("LogicalUDFOperator: Inferring String signature for {}", toString());
    NES_ASSERT(children.size() == 1, "LogicalUDFOperator should have exactly 1 child.");
...
bool LogicalUDFOperator::inferSchema() {
...
Catalogs::UDF::UDFDescriptorPtr LogicalUDFOperator::getUDFDescriptor() const { return udfDescriptor; }

bool LogicalUDFOperator::equal(const NodePtr& other) const {
    return other->instanceOf<LogicalUDFOperator>() && *udfDescriptor == *other->as<LogicalUDFOperator>()->udfDescriptor;
}

bool LogicalUDFOperator::isIdentical(const NodePtr& other) const {
    return equal(other) && id == other->as<LogicalUDFOperator>()->id;
}

void LogicalUDFOperator::verifySchemaCompatibility(const Schema& udfInputSchema, const Schema& childOperatorOutputSchema) const {
