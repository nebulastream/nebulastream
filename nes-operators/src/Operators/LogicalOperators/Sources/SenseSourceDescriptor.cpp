...
SenseSourceDescriptor::SenseSourceDescriptor(SchemaPtr schema,
...
SourceDescriptorPtr SenseSourceDescriptor::create(SchemaPtr schema,
                                                  std::string sourceName,
                                                  std::string udfs) {
  return std::make_shared<SenseSourceDescriptor>(SenseSourceDescriptor(
      std::move(schema), std::move(sourceName), std::move(udfs)));
}

SourceDescriptorPtr SenseSourceDescriptor::create(SchemaPtr schema,
                                                  std::string udfs) {
  return std::make_shared<SenseSourceDescriptor>(
      SenseSourceDescriptor(std::move(schema), std::move(udfs)));
}
...
bool SenseSourceDescriptor::equal(SourceDescriptorPtr const &other) const {
  ...
  return udfs == otherSource->getUdfs() &&
         getSchema()->equals(otherSource->getSchema());
}

std::string SenseSourceDescriptor::toString() const {
  return "SenseSourceDescriptor()";
}

SourceDescriptorPtr SenseSourceDescriptor::copy() {
  auto copy =
      SenseSourceDescriptor::create(schema->copy(), logicalSourceName, udfs);
