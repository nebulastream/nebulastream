...
SourceDescriptorPtr
ZmqSourceDescriptor::create(SchemaPtr schema, std::string host, uint16_t port) {
  return std::make_shared<ZmqSourceDescriptor>(
      ZmqSourceDescriptor(std::move(schema), std::move(host), port));
}

SourceDescriptorPtr ZmqSourceDescriptor::create(SchemaPtr schema,
                                                std::string sourceName,
                                                std::string host,
                                                uint16_t port) {
  return std::make_shared<ZmqSourceDescriptor>(ZmqSourceDescriptor(
      std::move(schema), std::move(sourceName), std::move(host), port));
}

ZmqSourceDescriptor::ZmqSourceDescriptor(SchemaPtr schema, std::string host,
                                         uint16_t port)
    : SourceDescriptor(std::move(schema)), host(std::move(host)), port(port) {}

ZmqSourceDescriptor::ZmqSourceDescriptor(SchemaPtr schema,
                                         std::string sourceName,
...
bool ZmqSourceDescriptor::equal(SourceDescriptorPtr const &other) const {
  ...
  return host == otherZMQSource->getHost() &&
         port == otherZMQSource->getPort() &&
         getSchema()->equals(other->getSchema());
}

std::string ZmqSourceDescriptor::toString() const {
...
SourceDescriptorPtr ZmqSourceDescriptor::copy() {
  auto copy = ZmqSourceDescriptor::create(schema->copy(), logicalSourceName,
                                          host, port);
