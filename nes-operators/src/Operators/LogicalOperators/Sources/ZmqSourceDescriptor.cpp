...
{
    return std::make_shared<ZmqSourceDescriptor>(ZmqSourceDescriptor(std::move(schema), std::move(sourceName), std::move(host), port));
}
...
void ZmqSourceDescriptor::setPort(uint16_t newPort)
{
    this->port = newPort;
}
