...
SourceDescriptorPtr
OPCSourceDescriptor::create(SchemaPtr schema, std::string url, UA_NodeId nodeId,
                            std::string user, std::string password) {
  return std::make_shared<OPCSourceDescriptor>(
      OPCSourceDescriptor(std::move(schema), std::move(url), std::move(nodeId),
                          std::move(user), std::move(password)));
}

SourceDescriptorPtr
OPCSourceDescriptor::create(SchemaPtr schema, std::string logicalSourceName,
                            std::string url, UA_NodeId nodeId, std::string user,
                            std::string password) {
  return std::make_shared<OPCSourceDescriptor>(OPCSourceDescriptor(
      std::move(schema), std::move(logicalSourceName), std::move(url),
      std::move(nodeId), std::move(user), std::move(password)));
}

OPCSourceDescriptor::OPCSourceDescriptor(SchemaPtr schema, std::string url,
                                         UA_NodeId nodeId, std::string user,
                                         std::string password)
    : SourceDescriptor(std::move(schema)), url(std::move(url)),
      nodeId(std::move(nodeId)), user(std::move(user)),
...
OPCSourceDescriptor::OPCSourceDescriptor(SchemaPtr schema,
    ...
    : SourceDescriptor(std::move(schema), std::move(logicalSourceName)),
      url(std::move(url)), nodeId(std::move(nodeId)), user(std::move(user)),
      password(std::move(password)) {}
...
bool OPCSourceDescriptor::equal(SourceDescriptorPtr const &other) const {
  ...
  char *otherOperatorIdent = (char *)UA_malloc(
      sizeof(char) * otherOPCSource->getNodeId().identifier.string.length + 1);
  ...
  otherOperatorIdent[otherOPCSource->getNodeId().identifier.string.length] =
      '\0';
  char *ident =
  ...
  return url == otherOPCSource->getUrl() &&
         !strcmp(ident, otherOperatorIdent) &&
         nodeId.namespaceIndex == otherOPCSource->getNodeId().namespaceIndex &&
         nodeId.identifierType == otherOPCSource->getNodeId().identifierType &&
         user == otherOPCSource->getUser() &&
         password == otherOPCSource->getPassword();
}

std::string OPCSourceDescriptor::toString() const {
  return "OPCSourceDescriptor()";
}

SourceDescriptorPtr OPCSourceDescriptor::copy() {
  auto copy = OPCSourceDescriptor::create(schema->copy(), logicalSourceName,
                                          url, nodeId, user, password);
