...
namespace NES
{
SourceDescriptorLogicalOperator::SourceDescriptorLogicalOperator(const Sources::SourceDescriptor& sourceDescriptor, const OperatorId id)
...
bool SourceDescriptorLogicalOperator::inferSchema()
{
    ...
    outputSchema = Schema{sourceDescriptor.getSchema()->memoryLayoutType};
    for (auto& field : sourceDescriptor.getSchema()->getFields())
    {
        outputSchema.addField(
            sourceDescriptor.getLogicalSource().getLogicalSourceName() + Schema::ATTRIBUTE_NAME_SEPARATOR + field.name, field.dataType);
    }
    return true;
}
