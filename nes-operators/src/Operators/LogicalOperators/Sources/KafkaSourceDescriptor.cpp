...
KafkaSourceDescriptor::KafkaSourceDescriptor(
    SchemaPtr schema, std::string brokers, std::string topic,
    std::string groupId, bool autoCommit, uint64_t kafkaConnectTimeout,
    const std::string &offsetMode, const KafkaSourceTypePtr &kafkaSourceType,
    uint64_t numbersOfBufferToProduce, uint64_t batchSize)
    : SourceDescriptor(std::move(schema)), brokers(std::move(brokers)),
      topic(std::move(topic)), groupId(std::move(groupId)),
      kafkaSourceType(kafkaSourceType), autoCommit(autoCommit),
      kafkaConnectTimeout(kafkaConnectTimeout), offsetMode(offsetMode),
...
KafkaSourceDescriptor::KafkaSourceDescriptor(SchemaPtr schema,
    ...
    : SourceDescriptor(std::move(schema)),
      kafkaSourceType(std::move(kafkaSourceType)) {}

KafkaSourceDescriptor::KafkaSourceDescriptor(
    SchemaPtr schema, std::string logicalSourceName, std::string brokers,
    std::string topic, std::string groupId, bool autoCommit,
    uint64_t kafkaConnectTimeout, const std::string &offsetMode,
    const KafkaSourceTypePtr &kafkaSourceType,
    uint64_t numbersOfBufferToProduce, uint64_t batchSize)
    : SourceDescriptor(std::move(schema), std::move(logicalSourceName)),
      brokers(std::move(brokers)), topic(std::move(topic)),
      groupId(std::move(groupId)), kafkaSourceType(kafkaSourceType),
...
SourceDescriptorPtr KafkaSourceDescriptor::create(
    SchemaPtr schema, std::string brokers, std::string logicalSourceName,
    std::string topic, std::string groupId, bool autoCommit,
    uint64_t kafkaConnectTimeout, const std::string &offsetMode,
    const KafkaSourceTypePtr &kafkaSourceType,
    uint64_t numbersOfBufferToProduce, uint64_t batchSize) {
  return std::make_shared<KafkaSourceDescriptor>(KafkaSourceDescriptor(
      std::move(schema), std::move(logicalSourceName), std::move(brokers),
      std::move(topic), std::move(groupId), autoCommit, kafkaConnectTimeout,
      offsetMode, kafkaSourceType, numbersOfBufferToProduce, batchSize));
}
...
SourceDescriptorPtr KafkaSourceDescriptor::create(
    ...
    uint64_t numbersOfBufferToProduce, uint64_t batchSize) {
  return std::make_shared<KafkaSourceDescriptor>(KafkaSourceDescriptor(
      std::move(schema), std::move(brokers), std::move(topic),
      std::move(groupId), autoCommit, kafkaConnectTimeout, offsetMode,
      kafkaSourceType, numbersOfBufferToProduce, batchSize));
}

SourceDescriptorPtr
KafkaSourceDescriptor::create(SchemaPtr schema,
                              KafkaSourceTypePtr sourceConfig) {
  return std::make_shared<KafkaSourceDescriptor>(
      KafkaSourceDescriptor(std::move(schema), std::move(sourceConfig)));
}
...
bool KafkaSourceDescriptor::equal(SourceDescriptorPtr const &other) const {
  ...
  return brokers == otherKafkaSource->getBrokers() &&
         topic == otherKafkaSource->getTopic() &&
         kafkaConnectTimeout == otherKafkaSource->getKafkaConnectTimeout() &&
         groupId == otherKafkaSource->getGroupId() &&
         getSchema()->equals(other->getSchema()) &&
         kafkaSourceType->equal(otherKafkaSource->getSourceConfigPtr());
}
...
SourceDescriptorPtr KafkaSourceDescriptor::copy() {
  auto copy = KafkaSourceDescriptor::create(
      schema->copy(), std::move(logicalSourceName), std::move(brokers),
      std::move(topic), std::move(groupId), autoCommit, kafkaConnectTimeout,
      offsetMode, kafkaSourceType, numbersOfBufferToProduce, batchSize);
