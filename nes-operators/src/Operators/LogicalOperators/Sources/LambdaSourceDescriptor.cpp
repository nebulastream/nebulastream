...
LambdaSourceDescriptor::LambdaSourceDescriptor(
    ...
    : SourceDescriptor(std::move(schema), logicalSourceName,
      ...
      numBuffersToProcess(numBuffersToProduce), gatheringValue(gatheringValue),
...
std::shared_ptr<LambdaSourceDescriptor> LambdaSourceDescriptor::create(
  ...
  return std::make_shared<LambdaSourceDescriptor>(
      schema, std::move(generationFunction), numBuffersToProcess,
      gatheringValue, gatheringMode, sourceAffinity, taskQueueId,
...
SourceDescriptorPtr LambdaSourceDescriptor::copy() {
  auto copy = LambdaSourceDescriptor::create(
      schema->copy(), std::move(generationFunction), numBuffersToProcess,
      gatheringValue, gatheringMode, sourceAffinity, taskQueueId,
      logicalSourceName, physicalSourceName);
