...
bool LogicalShuffleBufferOperator::isIdentical(std::shared_ptr<Node> const& rhs) const
{
    return equal(rhs) && NES::Util::as<LogicalShuffleBufferOperator>(rhs)->getId() == id;
}

bool LogicalShuffleBufferOperator::equal(std::shared_ptr<Node> const& rhs) const
{
