...
void LogicalOperator::inferZ3Signature(
    const Optimizer::QuerySignatureContext &context) {
  if (z3Signature) {
    return;
  }
...
LogicalOperator::getHashBasedSignature() const {
  return hashBasedSignature;
}

void LogicalOperator::setHashBasedSignature(
    std::map<size_t, std::set<std::string>> signature) {
  this->hashBasedSignature = std::move(signature);
}

void LogicalOperator::updateHashBasedSignature(
    size_t hashCode, const std::string &stringSignature) {
...
void LogicalOperator::setOperatorState(NES::OperatorState newOperatorState) {
  ...
  case TO_BE_PLACED:
    // an operator in the state TO_BE_PLACED or TO_BE_REPLACED can be changed to
    // TO_BE_PLACED
    if (this->operatorState == TO_BE_PLACED ||
        this->operatorState == TO_BE_REPLACED) {
      this->operatorState = newOperatorState;
      break;
    }
    throw Exceptions::InvalidOperatorStateException(id, {TO_BE_REMOVED, PLACED},
                                                    this->operatorState);
  case TO_BE_REMOVED:
    ...
    // an operator can be marked as TO_BE_REMOVED only if it is not in the state
    // REMOVED.
    throw Exceptions::InvalidOperatorStateException(
        id, {TO_BE_REMOVED, TO_BE_PLACED, PLACED, TO_BE_REPLACED},
        this->operatorState);
  case TO_BE_REPLACED:
    if (this->operatorState != REMOVED &&
        this->operatorState != TO_BE_REMOVED) {
    ...
    // an operator can be marked as TO_BE_REPLACED only if it is not in the
    // state REMOVED or TO_BE_REMOVED.
    throw Exceptions::InvalidOperatorStateException(
        id, {TO_BE_PLACED, PLACED, OperatorState::TO_BE_REPLACED},
        this->operatorState);
  case PLACED:
    if (this->operatorState != REMOVED &&
        this->operatorState != TO_BE_REMOVED) {
    ...
    // an operator can be marked as PLACED only if it is not in the state
    // REMOVED or TO_BE_REMOVED or already PLACED.
    throw Exceptions::InvalidOperatorStateException(
        id, {TO_BE_PLACED, TO_BE_REPLACED}, this->operatorState);
  case REMOVED:
    if (this->operatorState == TO_BE_PLACED ||
        this->operatorState == TO_BE_REMOVED) {
    ...
    // an operator can be marked as REMOVED only if it is in the state
    // TO_BE_REMOVED.
    throw Exceptions::InvalidOperatorStateException(id, {TO_BE_REMOVED},
                                                    this->operatorState);
  }
}
