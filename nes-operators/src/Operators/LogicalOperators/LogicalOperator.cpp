...
std::map<size_t, std::set<std::string>> LogicalOperator::getHashBasedSignature()
    const {
...
void LogicalOperator::setOperatorState(NES::OperatorState newOperatorState) {
  ...
  switch (newOperatorState) {
    case TO_BE_PLACED:
      // an operator in the state TO_BE_PLACED or TO_BE_REPLACED can be changed
      // to TO_BE_PLACED
      ...
      throw Exceptions::InvalidOperatorStateException(
          id, {TO_BE_REMOVED, PLACED}, this->operatorState);
    case TO_BE_REMOVED:
      if (this->operatorState != REMOVED) {
        this->operatorState = TO_BE_REMOVED;
        break;
      }
      // an operator can be marked as TO_BE_REMOVED only if it is not in the
      // state REMOVED.
      throw Exceptions::InvalidOperatorStateException(
