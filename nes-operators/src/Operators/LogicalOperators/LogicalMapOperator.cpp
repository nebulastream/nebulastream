...
LogicalMapOperator::LogicalMapOperator(
    const FieldAssignmentExpressionNodePtr &mapExpression, OperatorId id)
...
bool LogicalMapOperator::inferSchema() {
  ...
  if (outputSchema->getField(fieldName)) {
    ...
    NES_TRACE("MAP Logical Operator: the field {} is already in the schema, so "
              "we updated its type.",
              fieldName);
    ...
    NES_TRACE("MAP Logical Operator: the field {} is not part of the schema, "
              "so we added it.",
              fieldName);
...
std::string LogicalMapOperator::toString() const {
  ...
  ss << "MAP(opId: " << id << ", statisticId: " << statisticId
     << ", predicate: " << mapExpression->toString() << ")";
  return ss.str();
}

OperatorPtr LogicalMapOperator::copy() {
  auto copy = LogicalOperatorFactory::createMapOperator(
      mapExpression->copy()->as<FieldAssignmentExpressionNode>(), id);
...
void LogicalMapOperator::inferStringSignature() {
  NES_TRACE("LogicalMapOperator: Inferring String signature for {}",
            toString());
  NES_ASSERT(children.size() == 1,
             "LogicalMapOperator: Map should have 1 child.");
  ...
  signatureStream << "MAP(" + mapExpression->toString() + ")."
