...
LogicalInferModelOperator::LogicalInferModelOperator(
    ...
    std::vector<ExpressionNodePtr> outputFields, OperatorId id)
    : Operator(id), LogicalUnaryOperator(id), model(std::move(model)),
...
OperatorPtr LogicalInferModelOperator::copy() {
  auto copy = LogicalOperatorFactory::createInferModelOperator(
      model, inputFields, outputFields, id);
...
bool LogicalInferModelOperator::equal(NodePtr const &rhs) const {
  if (rhs->instanceOf<LogicalInferModelOperator>()) {
    ...
    return this->getDeployedModelPath() ==
           inferModelOperator->getDeployedModelPath();
  }
  return false;
}
...
void LogicalInferModelOperator::updateToFullyQualifiedFieldName(
...
bool LogicalInferModelOperator::inferSchema() {
  ...
  for (auto outputField : outputFields) {
    ...
    if (outputSchema->getField(fieldName)) {
      ...
      NES_TRACE("Infer Model Logical Operator: the field {} is already in the "
                "schema, so we updated its type.",
                fieldName);
      ...
      NES_TRACE("Infer Model Logical Operator: the field {} is not part of the "
                "schema, so we added it.",
                fieldName);
...
void LogicalInferModelOperator::inferStringSignature() {
  ...
  NES_TRACE("InferModelOperator: Inferring String signature for {}",
            operatorNode->toString());
  NES_ASSERT(!children.empty(),
             "LogicalInferModelOperator: InferModel should have children (?)");
  ...
  signatureStream << "INFER_MODEL(" + model + ")."
...
const std::string LogicalInferModelOperator::getDeployedModelPath() const {
  ...
  // If there exist a / in the model path name. If so, then we have to remove
  // the path to only get the file name
...
LogicalInferModelOperator::getInputFields() const {
  return inputFields;
}

const std::vector<ExpressionNodePtr> &
LogicalInferModelOperator::getOutputFields() const {
  return outputFields;
}
