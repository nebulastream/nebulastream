...
public:
  ...
  /**
   * @brief Gets the statisticId of this operator for example to pass it down to
  ...
  /**
   * @brief Create duplicate of this operator by copying its context information
   * and also its parent and child operator set.
   * @return duplicate of this logical operator
   */
  OperatorPtr duplicate();

  /**
   * @brief Create a shallow copy of the operator by copying its operator
   * properties but not its children or parent operator tree.
   * @return shallow copy of the operator
   */
  virtual OperatorPtr copy() = 0;

  /**
   * @brief detect if this operator is a n-ary operator, i.e., it has multiple
   * parent or children.
  ...
  /**
   * Check if a node with the id is either a child or a grandchild of this
   * operator
  ...
  /**
   * Check if a node with the id is either a parent or a grandparent of this
   * operator
   * @param operatorNode : the operator node whose id need to be looked up
   * @return true if operator with the input id is a child or grand child
   */
