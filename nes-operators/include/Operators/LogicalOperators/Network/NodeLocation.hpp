...
/**
 * @brief This is a network location of a nes node. It contains a node id, an ip
...
class NodeLocation {
 public:
  ...
  explicit NodeLocation(WorkerId workerId, std::string hostname, uint32_t port)
    ...
    NES_ASSERT2_FMT(!this->hostname.empty(),
                    "Empty hostname passed on " << workerId);
  }

  NodeLocation(const NodeLocation& other)
      : workerId(other.workerId), hostname(other.hostname), port(other.port) {}
   ...
   */
  [[nodiscard]] std::string createZmqURI() const {
    return fmt::format("tcp://{}:{}", hostname, std::to_string(port));
  }
  ...
  friend bool operator==(const NodeLocation& lhs, const NodeLocation& rhs) {
    return lhs.workerId == rhs.workerId && lhs.hostname == rhs.hostname &&
           lhs.port == rhs.port;
  }
