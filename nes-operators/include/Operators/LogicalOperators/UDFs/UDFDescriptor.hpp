...
public:
   ...
   *
   * @return A SchemaPtr instance describing the output schema of the UDF
   * method.
   */
   ...
   * The input schema must correspond to the input type of the UDF method.
   *
   * @param inputSchema A SchemaPtr instance describing the input schema of the
   * UDF method.
   */
   ...
   */
  template <class UDFDescriptor> bool instanceOf() {
   ...
   * @return True, if both UDFDescriptors are the same, i.e., same UDF class and
   * method name, same serialized instance (state), and same byte code list;
   * False, otherwise.
   */
