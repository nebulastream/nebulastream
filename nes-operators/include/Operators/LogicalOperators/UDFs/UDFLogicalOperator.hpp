...
/**
 * Logical operator node for a udf. This class acts as a parent class for any
 * udf logical operator node
 */
...
public:
   ...
   */
  UDFLogicalOperator(const Catalogs::UDF::UDFDescriptorPtr udfDescriptor,
                     OperatorId id);
   ...
   *
   * Sets the output schema contained in the UDFDescriptor as the output schema
   * of the operator.
   */
  ...
  /**
   * Two MapUdfLogicalOperator are equal when the wrapped UDFDescriptor are
   * equal.
   */
...
private:
  /**
   * Verify that the UDF input type is compatible with the schema of the child
   * operator, i.e., they contain exactly the same attributes with the same
   * types. <p> The test assumes that a UDF input schema supplied by the client
   * only contains signed integers (Java integers are signed), therefore the
   * child output schema cannot contained unsigned integers. The only exception
   * is UINT64 which is mapped to a signed Java long in the UDF input schema.
   *
   ...
   */
  void verifySchemaCompatibility(const Schema &udfInputSchema,
                                 const Schema &childOperatorOutputSchema) const;
