...
public:
   ...
   */
  static WindowStatisticDescriptorPtr
  create(FieldAccessExpressionNodePtr field);
  ...
  static WindowStatisticDescriptorPtr create(FieldAccessExpressionNodePtr field,
                                             double error, double probability);
   ...
   */
  void
  addDescriptorFields(Schema &outputSchema,
                      const std::string &qualifierNameWithSeparator) override;
...
private:
   ...
   * @param depth: Number of rows for the CountMin sketch
   */
  CountMinDescriptor(const FieldAccessExpressionNodePtr &field, uint64_t width,
                     uint64_t depth);
