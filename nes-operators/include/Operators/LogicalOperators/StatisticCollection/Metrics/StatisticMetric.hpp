...
/**
 * @brief The metric hash is the hash corresponding to a specific metric. We use
 * this in combination with the StatisticId to uniquely identify a statistic in
 * our system. By using the StatisticMetricHash, we do not have to send the
 * whole metric down to the physical operator but can rather send the
 * StatisticMetricHash. In the physical operator, we receive the statistic id as
 * part of the tuple buffer and therefore, we can uniquely identify a statistic
 */
...
public:
   ...
   */
  template <class StatisticMetricType> bool instanceOf() {
  ...
  std::shared_ptr<StatisticMetricType> as() {
    if (instanceOf<StatisticMetricType>()) {
      return std::dynamic_pointer_cast<StatisticMetricType>(
          this->shared_from_this());
    }
    throw std::logic_error("We performed an invalid cast of operator " +
                           this->toString() + " to type " +
   ...
   * @return bool true if node is of Metric
   */
  template <class StatisticMetricType> [[nodiscard]] bool instanceOf() const {
