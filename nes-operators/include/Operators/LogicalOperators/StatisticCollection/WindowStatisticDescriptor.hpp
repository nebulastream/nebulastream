 ...
 */
class WindowStatisticDescriptor
    : public std::enable_shared_from_this<WindowStatisticDescriptor> {
public:
  /**
   * @brief Constructor for a WindowStatisticDescriptor
   * @param field: Over which field to track the statistic
   */
  WindowStatisticDescriptor(const FieldAccessExpressionNodePtr &field,
                            uint64_t width);
  ...
  /**
   * @brief Getter for the FieldAccessExpression over which the statistic is
   * being collected
  ...
  /**
   * @brief Adds fields to the output schema specific to the
  ...
  addDescriptorFields(Schema &outputSchema,
                      const std::string &qualifierNameWithSeparator) = 0;
  ...
  /**
   * @brief Checks if the current WindowStatisticDescriptor is of type
   * WindowStatisticDescriptorType
   * @tparam WindowStatisticDescriptorType
   * @return bool true if node is of WindowStatisticDescriptorType
   */
  template <class WindowStatisticDescriptorType> bool instanceOf() {
   ...
   * WindowStatisticDescriptorType
   * @tparam WindowStatisticDescriptorType
   * @return bool true if WindowStatisticDescriptor is of
   * WindowStatisticDescriptorType
   */
  template <class WindowStatisticDescriptorType> bool instanceOf() const {
    if (dynamic_cast<WindowStatisticDescriptorType *>(this)) {
      return true;
    }
    return false;
  ...
  /**
   * @brief Dynamically casts the WindowStatisticDescriptor to a
   * WindowStatisticDescriptorType
   * @tparam WindowStatisticDescriptorType
  ...
  std::shared_ptr<WindowStatisticDescriptorType> as() {
    if (instanceOf<WindowStatisticDescriptorType>()) {
      return std::dynamic_pointer_cast<WindowStatisticDescriptorType>(
