...
class LogicalOperatorFactory {
 public:
   ...
   * @param predicate: the filter predicate is represented as an expression
   * node, which has to return true.
   * @param id: the id of the operator if not defined then next free operator id
   * is used.
   * @return UnaryOperatorPtr
   */
  static LogicalUnaryOperatorPtr createFilterOperator(
      ExpressionNodePtr const& predicate, OperatorId id = getNextOperatorId());
   ...
   * @return UnaryOperatorPtr
   */
  static LogicalUnaryOperatorPtr createRenameSourceOperator(
      std::string const& newSourceName, OperatorId id = getNextOperatorId());
   ...
   * @return UnaryOperatorPtr
   */
  static LogicalUnaryOperatorPtr createLimitOperator(
      const uint64_t limit, OperatorId id = getNextOperatorId());
   ...
   */
  static LogicalUnaryOperatorPtr createProjectionOperator(
  ...
  static LogicalUnaryOperatorPtr createSinkOperator(
      SinkDescriptorPtr const& sinkDescriptor,
      WorkerId workerId = INVALID_WORKER_NODE_ID,
      OperatorId id = getNextOperatorId());

  /**
   * @brief Create a new map operator with a field assignment expression as a
   * map expression.
   ...
   * @return UnaryOperatorPtr
   */
  static LogicalUnaryOperatorPtr createMapOperator(
  ...
  static LogicalUnaryOperatorPtr createSourceOperator(
      SourceDescriptorPtr const& sourceDescriptor,
      OperatorId id = getNextOperatorId(),
   ...
   * @return UnaryOperatorPtr
   */
  static LogicalUnaryOperatorPtr createWindowOperator(
      Windowing::LogicalWindowDescriptorPtr const& windowDefinition,
      OperatorId id = getNextOperatorId());
   ...
   */
  static LogicalBinaryOperatorPtr createUnionOperator(
      OperatorId id = getNextOperatorId());
   ...
   * @return BinaryOperator
   */
  static LogicalBinaryOperatorPtr createJoinOperator(
   ...
   * @param javaUdfDescriptor The descriptor of the Java UDF represented by this
   * logical operator node.
   * @param id The operator ID.
   * @return A logical operator node which encapsulates the Java UDF.
   */
  static LogicalUnaryOperatorPtr createMapUDFLogicalOperator(
   ...
   * @return A logical operator node which encapsulates the Java UDF.
   */
  static LogicalUnaryOperatorPtr createFlatMapUDFLogicalOperator(
      const Catalogs::UDF::UDFDescriptorPtr udfDescriptor,
      OperatorId id = getNextOperatorId());
   ...
   */
  static LogicalUnaryOperatorPtr createOpenCLLogicalOperator(
