...
class LogicalFilterOperator : public LogicalUnaryOperator {
 public:
   ...
   * @param newPredicate the predicate which will be the new predicate of the
   * filter
   */
  ...
  /**
   * @brief infers the input and output schema of this operator depending on its
   * child.
  ...
  /**
   * @brief returns the names of every attribute that is accessed in the
   * predicate of this filter
   * @return a vector containing every attribute name that is accessed by the
   * predicate
   */
