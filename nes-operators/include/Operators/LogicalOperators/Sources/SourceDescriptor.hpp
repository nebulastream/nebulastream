...
public:
   ...
   */
  template <class SourceType> bool instanceOf() const {
   ...
   */
  template <class SourceType> std::shared_ptr<SourceType> as() const {
    if (instanceOf<SourceType>()) {
      return std::dynamic_pointer_cast<SourceType>(this->shared_from_this());
    }
    throw Exceptions::RuntimeException(
        "SourceDescriptor: We performed an invalid cast");
  }
  template <class SourceType> std::shared_ptr<SourceType> as() {
    return std::const_pointer_cast<SourceType>(
        const_cast<const SourceDescriptor *>(this)->as<const SourceType>());
  }

  template <class SourceType> std::shared_ptr<SourceType> as_if() {
    return std::dynamic_pointer_cast<SourceType>(this->shared_from_this());
  }

  /**
   * @brief Returns the logicalSourceName. If no logicalSourceName is defined it
