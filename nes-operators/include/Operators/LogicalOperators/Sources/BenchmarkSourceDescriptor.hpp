...
#ifndef NES_OPERATORS_INCLUDE_OPERATORS_LOGICALOPERATORS_SOURCES_BENCHMARKSOURCEDESCRIPTOR_HPP_
...
namespace NES {
/**
 * @brief Descriptor defining properties used for creating physical memory
 * source
 */
...
public:
   ...
   * @param memoryArea a non-null pointer to the area of memory to use in the
   * source
   ...
   */
  static std::shared_ptr<BenchmarkSourceDescriptor>
  create(const SchemaPtr &schema, const std::shared_ptr<uint8_t> &memoryArea,
         size_t memoryAreaSize, uint64_t numBuffersToProcess,
         uint64_t gatheringValue, GatheringMode gatheringMode,
         SourceMode sourceMode, uint64_t sourceAffinity = 0,
         uint64_t taskQueueId = 0, const std::string &logicalSourceName = "",
         const std::string &physicalSourceName = "");
