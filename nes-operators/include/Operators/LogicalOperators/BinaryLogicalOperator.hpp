...
public:
    ...
    void setRightInputSchema(std::shared_ptr<Schema> schema) { rightInputSchema = schema; }
    std::shared_ptr<Schema> getRightInputSchema() const { return rightInputSchema; }

    void setLeftInputSchema(std::shared_ptr<Schema> schema) { leftInputSchema = schema; }
    std::shared_ptr<Schema> getLeftInputSchema() const { return leftInputSchema; }

    void setLeftInputOriginIds(std::vector<OriginId> originIds) { leftInputOriginIds = originIds; }

    std::vector<OriginId> getLeftInputOriginIds() const { return leftInputOriginIds; }

    void setRightInputOriginIds(std::vector<OriginId> originIds) { rightInputOriginIds = originIds; }
    std::vector<OriginId> getRightInputOriginIds() const { return rightInputOriginIds; }
