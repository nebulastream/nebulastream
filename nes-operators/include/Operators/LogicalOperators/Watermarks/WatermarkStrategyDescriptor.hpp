...
class WatermarkStrategyDescriptor
    : public std::enable_shared_from_this<WatermarkStrategyDescriptor> {
public:
   ...
   */
  template <class WatermarkStrategyType> bool instanceOf() const {
  ...
  template <class WatermarkStrategyType> bool instanceOf() {
    if (dynamic_cast<WatermarkStrategyType *>(this)) {
      return true;
    }
    return false;
  ...
  std::shared_ptr<WatermarkStrategyType> as() const {
    if (instanceOf<const WatermarkStrategyType>()) {
      return std::dynamic_pointer_cast<const WatermarkStrategyType>(
  ...
  std::shared_ptr<WatermarkStrategyType> as() {
    if (instanceOf<WatermarkStrategyType>()) {
      return std::dynamic_pointer_cast<WatermarkStrategyType>(
          this->shared_from_this());
    }
    throw std::bad_cast();
  }
