...
/**
 * Abstract class for window aggregations. All window aggregations operate on a
 * field and output another field.
 */
...
public:
  ...
  /**
   * @brief Check if input window aggregation is equal to this window
   * aggregation definition by checking the aggregation type, on field, and as
   * field
   * @param otherWindowAggregationDescriptor : other window aggregation
   * definition
   * @return true if equal else false
   */
  bool
  equal(WindowAggregationDescriptorPtr otherWindowAggregationDescriptor) const;

protected:
  explicit WindowAggregationDescriptor(
      const FieldAccessExpressionNodePtr &onField);
  WindowAggregationDescriptor(const ExpressionNodePtr &onField,
                              const ExpressionNodePtr &asField);
