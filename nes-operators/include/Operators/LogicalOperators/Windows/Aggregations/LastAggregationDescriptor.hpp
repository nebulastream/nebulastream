...
class LastAggregationDescriptor : public WindowAggregationDescriptor
{
public:
    ...
    virtual ~LastAggregationDescriptor() = default;

private:
    explicit LastAggregationDescriptor(const std::shared_ptr<NodeFunctionFieldAccess>& onField);
    LastAggregationDescriptor(const std::shared_ptr<NodeFunction>& onField, const std::shared_ptr<NodeFunction>& asField);
};
}
