...
/**
 * @brief The logical window definition encapsulates all information, which are
 * required for windowed aggregations on data streams. It contains the key
 * attributes, the aggregation functions, and the window type.
 */
...
public:
   ...
   */
  static LogicalWindowDescriptorPtr
  create(const std::vector<WindowAggregationDescriptorPtr> &windowAggregations,
   ...
   */
  static LogicalWindowDescriptorPtr
  create(std::vector<FieldAccessExpressionNodePtr> keys,
         std::vector<WindowAggregationDescriptorPtr> windowAggregation,
         const WindowTypePtr &windowType, uint64_t allowedLateness);
  ...
  /**
   * @brief Getter for the number of input edges, which is used for the low
   * watermarks.
   */
   ...
   * how long the system should wait for out of order events before a window is
   * triggered.
  ...
  /**
   * @brief Checks if the input window definition is equal to this window
   * definition by comparing the window key, type, and aggregation
