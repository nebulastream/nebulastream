...
public:
  /**
   * @brief Create an instance of decomposed query plan with initial state in
  ...
  create(DecomposedQueryPlanId decomposedQueryPlanId,
         SharedQueryId sharedQueryId, WorkerId workerId);
   ...
   */
  bool replaceRootOperator(const OperatorPtr &oldRoot,
                           const OperatorPtr &newRoot);

  /**
   * @brief Appends an operator to the query plan and make the new operator as
   * root.
  ...
  /**
   * @brief Get all the leaf operators in the query plan (leaf operator is the
   * one without any child)
  ...
  /**
   * @brief Check if the decomposed query plan contains an operator with input
   * id
   ...
   */
  template <class T> std::vector<std::shared_ptr<T>> getOperatorByType() {
    ...
    for (const auto &rootOperator : rootOperators) {
      ...
      for (auto itr = bfsIterator.begin();
           itr != NES::BreadthFirstNodeIterator::end(); ++itr) {
        auto visitingOp = (*itr)->as<Operator>();
        if (visitedOpIds.contains(visitingOp->getId())) {
          // skip rest of the steps as the node found in already visited node
          // list
          continue;
        }
