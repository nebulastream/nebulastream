...
namespace NES {
/**
 * This class adds the logical operators to the queryPlan and handles further
 * conditions and updates on the updated queryPlan and its nodes, e.g., update
 * the consumed sources after a binary operator or adds window characteristics
 * to the join operator.
 */
class QueryPlanBuilder {
 public:
  /**
   * @brief: Creates a query plan from a particular source. The source is
   * identified by its name. During query processing the underlying source
   * descriptor is retrieved from the source catalog.
   ...
   * @return the updated queryPlan
   */
  static QueryPlanPtr addProjection(
      const std::vector<ExpressionNodePtr>& expressions,
      QueryPlanPtr queryPlan);

  /**
   * @brief this call add the rename operator to the queryPlan, this operator
   * renames the source
   * @param newSourceName source name
   * @param queryPlan the queryPlan to add the rename node
   * @return the updated queryPlan
   */
  static QueryPlanPtr addRename(std::string const& newSourceName,
                                QueryPlanPtr queryPlan);

  /**
   * @brief: this call add the filter operator to the queryPlan, the operator
   * filters records according to the predicate. An exemplary usage would be:
   ...
   * @return the updated queryPlan
   */
  static QueryPlanPtr addFilter(ExpressionNodePtr const& filterExpression,
                                QueryPlanPtr queryPlan);

  /**
   * @brief: this call adds the limit operator to the queryPlan, the operator
   * limits the number of produced records.
   * @param filterExpression as expression node containing the predicate
   * @param queryPlanPtr the queryPlan the filter node is added to
   * @return the updated queryPlan
   */
   ...
   */
  static QueryPlanPtr addMap(
      FieldAssignmentExpressionNodePtr const& mapExpression,
      QueryPlanPtr queryPlan);
  ...
  static QueryPlanPtr addStatisticBuildOperator(
      ...
      Statistic::TriggerConditionPtr triggerCondition, QueryPlanPtr queryPlan);
   ...
   * @return the updated queryPlanPtr
   */
  static QueryPlanPtr addMapUDF(
      Catalogs::UDF::UDFDescriptorPtr const& descriptor,
      QueryPlanPtr queryPlan);

  /**
   * @brief: FlatMap java udf according to the java method given in the
   * descriptor.
   * @param descriptor as java udf descriptor
   * @param queryPlan the queryPlan the map is added to
   * @return the updated queryPlanPtr
   */
  static QueryPlanPtr addFlatMapUDF(
      Catalogs::UDF::UDFDescriptorPtr const& descriptor,
      QueryPlanPtr queryPlan);
   ...
   */
  static QueryPlanPtr addUnion(QueryPlanPtr leftQueryPlan,
   ...
   * @return the updated queryPlan
   */
  static QueryPlanPtr addSink(QueryPlanPtr queryPlan,
                              SinkDescriptorPtr sinkDescriptor,
                              WorkerId workerId = INVALID_WORKER_NODE_ID);

  /**
   * @brief Create watermark assigner operator and adds it to the queryPlan
   * @param watermarkStrategyDescriptor which represents the semantic of this
   * watermarkStrategy.
   * @return queryPlan
   */
  static QueryPlanPtr assignWatermark(
      QueryPlanPtr queryPlan, Windowing::WatermarkStrategyDescriptorPtr const&
                                  watermarkStrategyDescriptor);

  /**
   * @brief: Method that checks in case a window is contained in the query
   * if a watermark operator exists in the queryPlan and if not adds a watermark
   * strategy to the queryPlan
   ...
   * @return the updated queryPlan
   */
  static QueryPlanPtr checkAndAddWatermarkAssignment(
      QueryPlanPtr queryPlan, const Windowing::WindowTypePtr windowType);

 private:
  /**
   * @brief This method checks if an ExpressionNode is instance Of
   * FieldAccessExpressionNode for Join and BatchJoin
   * @param expression the expression node to test
   * @param side points out from which side, i.e., left or right query plan, the
   * ExpressionNode is
   * @return expressionNode as FieldAccessExpressionNode
   */
  static std::shared_ptr<FieldAccessExpressionNode> checkExpression(
      ExpressionNodePtr expression, std::string side);

  /**
   * @brief: This method adds a binary operator to the query plan and updates
   * the consumed sources
   ...
   * @return the updated queryPlan
   */
  static QueryPlanPtr addBinaryOperatorAndUpdateSource(
      OperatorPtr operatorNode, QueryPlanPtr leftQueryPlan,
      QueryPlanPtr rightQueryPlan);
};
}  // end namespace NES
