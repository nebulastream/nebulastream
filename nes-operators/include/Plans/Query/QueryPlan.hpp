...
/**
 * @brief The query plan encapsulates a set of operators and provides a set of
 * utility functions.
 */
class QueryPlan {
public:
  /**
   * @brief Creates a new query plan with a query id, a query sub plan id and a
   * vector of root operators.
   ...
   */
  template <class T> std::vector<std::shared_ptr<T>> getOperatorByType() const {
   ...
   * Note: This method only check if there exists another operator with same Id
   * or not. Note: The system generated operators are ignored from this check.
  ...
  /**
   * @brief Gets the operator node for the statistic id. This method traverses
   * all operators in the query plan.
  ...
  /**
   * @brief Find all operators between given set of downstream and upstream
   * operators
   ...
   */
  std::set<OperatorPtr>
  findAllOperatorsBetween(const std::set<OperatorPtr> &downstreamOperators,
   ...
   * @return true, if this and other plan are equal in their structure and
   * operators, false else
   */
...
private:
   ...
   */
  std::set<OperatorPtr> findOperatorsBetweenSourceAndTargetOperators(
  ...
  // Default placement strategy is top-down; we set the correct placement
  // strategy in the Experimental Add Request
