...
class ModFunctionNodeTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("ModFunctionNodeTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup ModFunctionNodeTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(ModFunctionNodeTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        const auto modFunctionNode = ModFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        modFunctionNode->inferStamp(dummySchema);
        modFunctionNode->removeChildren();
        EXPECT_FALSE(modFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto modFunctionNode = ModFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        modFunctionNode->inferStamp(dummySchema);
        modFunctionNode->removeChild(readFunctionNodeLeft);
        EXPECT_FALSE(modFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto modFunctionNode = ModFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        modFunctionNode->inferStamp(dummySchema);
        modFunctionNode->removeChild(readFunctionNodeRight);
        EXPECT_FALSE(modFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto modFunctionNode = ModFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        modFunctionNode->inferStamp(dummySchema);
        EXPECT_TRUE(modFunctionNode->validateBeforeLowering());
    }
}

TEST_F(ModFunctionNodeTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(modFunctionNode->inferStamp(dummySchema));
        }
        EXPECT_EQ(modFunctionNode->validateBeforeLowering(), expectedValue);
    };
