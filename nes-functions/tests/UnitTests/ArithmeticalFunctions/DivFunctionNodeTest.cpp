...
class DivFunctionNodeTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("DivFunctionNodeTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup DivFunctionNodeTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(DivFunctionNodeTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        const auto divFunctionNode = DivFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        divFunctionNode->inferStamp(dummySchema);
        divFunctionNode->removeChildren();
        EXPECT_FALSE(divFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto divFunctionNode = DivFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        divFunctionNode->inferStamp(dummySchema);
        divFunctionNode->removeChild(readFunctionNodeLeft);
        EXPECT_FALSE(divFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto divFunctionNode = DivFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        divFunctionNode->inferStamp(dummySchema);
        divFunctionNode->removeChild(readFunctionNodeRight);
        EXPECT_FALSE(divFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto divFunctionNode = DivFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        divFunctionNode->inferStamp(dummySchema);
        EXPECT_TRUE(divFunctionNode->validateBeforeLowering());
    }
}

TEST_F(DivFunctionNodeTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(divFunctionNode->inferStamp(dummySchema));
        }
        EXPECT_EQ(divFunctionNode->validateBeforeLowering(), expectedValue);
    };
