...
class PowFunctionNodeTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("PowFunctionNodeTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup PowFunctionNodeTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(PowFunctionNodeTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        const auto readFunctionNodeLeft = FieldAccessFunctionNode::create("i64");
        const auto readFunctionNodeRight = FieldAccessFunctionNode::create("f32");
        const auto powFunctionNode = PowFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        powFunctionNode->inferStamp(dummySchema);
        powFunctionNode->removeChildren();
        EXPECT_FALSE(powFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto powFunctionNode = PowFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        powFunctionNode->inferStamp(dummySchema);
        powFunctionNode->removeChild(readFunctionNodeLeft);
        EXPECT_FALSE(powFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto powFunctionNode = PowFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        powFunctionNode->inferStamp(dummySchema);
        powFunctionNode->removeChild(readFunctionNodeRight);
        EXPECT_FALSE(powFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto powFunctionNode = PowFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        powFunctionNode->inferStamp(dummySchema);
        EXPECT_TRUE(powFunctionNode->validateBeforeLowering());
    }
}

TEST_F(PowFunctionNodeTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(powFunctionNode->inferStamp(dummySchema));
        }
        EXPECT_EQ(powFunctionNode->validateBeforeLowering(), expectedValue);
    };
