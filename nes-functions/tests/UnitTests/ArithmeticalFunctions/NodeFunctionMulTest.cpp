...
class NodeFunctionMulTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("NodeFunctionMulTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup NodeFunctionMulTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(NodeFunctionMulTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        nodeFunctionMul->removeChildren();
        EXPECT_FALSE(nodeFunctionMul->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionMul = NodeFunctionMul::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMul->inferStamp(dummySchema);
        nodeFunctionMul->removeChild(nodeFunctionReadLeft);
        EXPECT_FALSE(nodeFunctionMul->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionMul = NodeFunctionMul::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMul->inferStamp(dummySchema);
        nodeFunctionMul->removeChild(nodeFunctionReadRight);
        EXPECT_FALSE(nodeFunctionMul->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionMul = NodeFunctionMul::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMul->inferStamp(dummySchema);
        EXPECT_TRUE(nodeFunctionMul->validateBeforeLowering());
    }
}

TEST_F(NodeFunctionMulTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(nodeFunctionMul->inferStamp(dummySchema));
        }
        EXPECT_EQ(nodeFunctionMul->validateBeforeLowering(), expectedValue);
    };
