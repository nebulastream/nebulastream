...
class NodeFunctionModTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("NodeFunctionModTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup NodeFunctionModTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(NodeFunctionModTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        const auto nodeFunctionMod = NodeFunctionMod::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMod->inferStamp(dummySchema);
        nodeFunctionMod->removeChildren();
        EXPECT_FALSE(nodeFunctionMod->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionMod = NodeFunctionMod::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMod->inferStamp(dummySchema);
        nodeFunctionMod->removeChild(nodeFunctionReadLeft);
        EXPECT_FALSE(nodeFunctionMod->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionMod = NodeFunctionMod::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMod->inferStamp(dummySchema);
        nodeFunctionMod->removeChild(nodeFunctionReadRight);
        EXPECT_FALSE(nodeFunctionMod->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionMod = NodeFunctionMod::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionMod->inferStamp(dummySchema);
        EXPECT_TRUE(nodeFunctionMod->validateBeforeLowering());
    }
}

TEST_F(NodeFunctionModTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(nodeFunctionMod->inferStamp(dummySchema));
        }
        EXPECT_EQ(nodeFunctionMod->validateBeforeLowering(), expectedValue);
    };
