...
class NodeFunctionAddTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("NodeFunctionAddTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup NodeFunctionAddTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(NodeFunctionAddTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        nodeFunctionAdd->removeChildren();
        EXPECT_FALSE(nodeFunctionAdd->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionAdd = NodeFunctionAdd::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionAdd->inferStamp(dummySchema);
        nodeFunctionAdd->removeChild(nodeFunctionReadLeft);
        EXPECT_FALSE(nodeFunctionAdd->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionAdd = NodeFunctionAdd::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionAdd->inferStamp(dummySchema);
        nodeFunctionAdd->removeChild(nodeFunctionReadRight);
        EXPECT_FALSE(nodeFunctionAdd->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionAdd = NodeFunctionAdd::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionAdd->inferStamp(dummySchema);
        EXPECT_TRUE(nodeFunctionAdd->validateBeforeLowering());
    }
}

TEST_F(NodeFunctionAddTest, validateBeforeLoweringDifferentDataTypes)
{
        ...
        = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue)
    {
        ...
        {
            EXPECT_ANY_THROW(nodeFunctionAdd->inferStamp(dummySchema));
        }
        EXPECT_EQ(nodeFunctionAdd->validateBeforeLowering(), expectedValue);
    };
