...
class NodeFunctionSubTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("NodeFunctionSubTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup NodeFunctionSubTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(NodeFunctionSubTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        nodeFunctionSub->removeChildren();
        EXPECT_FALSE(nodeFunctionSub->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionSub = NodeFunctionSub::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionSub->inferStamp(dummySchema);
        nodeFunctionSub->removeChild(nodeFunctionReadLeft);
        EXPECT_FALSE(nodeFunctionSub->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionSub = NodeFunctionSub::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionSub->inferStamp(dummySchema);
        nodeFunctionSub->removeChild(nodeFunctionReadRight);
        EXPECT_FALSE(nodeFunctionSub->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionSub = NodeFunctionSub::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionSub->inferStamp(dummySchema);
        EXPECT_TRUE(nodeFunctionSub->validateBeforeLowering());
    }
}

TEST_F(NodeFunctionSubTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(nodeFunctionSub->inferStamp(dummySchema));
        }
        EXPECT_EQ(nodeFunctionSub->validateBeforeLowering(), expectedValue);
    };
