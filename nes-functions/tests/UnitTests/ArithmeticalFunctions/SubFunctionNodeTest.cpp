...
class SubFunctionNodeTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("SubFunctionNodeTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup SubFunctionNodeTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(SubFunctionNodeTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        const auto subFunctionNode = SubFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        subFunctionNode->inferStamp(dummySchema);
        subFunctionNode->removeChildren();
        EXPECT_FALSE(subFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto subFunctionNode = SubFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        subFunctionNode->inferStamp(dummySchema);
        subFunctionNode->removeChild(readFunctionNodeLeft);
        EXPECT_FALSE(subFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto subFunctionNode = SubFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        subFunctionNode->inferStamp(dummySchema);
        subFunctionNode->removeChild(readFunctionNodeRight);
        EXPECT_FALSE(subFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto subFunctionNode = SubFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        subFunctionNode->inferStamp(dummySchema);
        EXPECT_TRUE(subFunctionNode->validateBeforeLowering());
    }
}

TEST_F(SubFunctionNodeTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        {
            EXPECT_ANY_THROW(subFunctionNode->inferStamp(dummySchema));
        }
        EXPECT_EQ(subFunctionNode->validateBeforeLowering(), expectedValue);
    };
