...
class LessFunctionNodeTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("LessFunctionNodeTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup LessFunctionNodeTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(LessFunctionNodeTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        const auto lessFunctionNode = LessFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        lessFunctionNode->inferStamp(dummySchema);
        lessFunctionNode->removeChildren();
        EXPECT_TRUE(lessFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto lessFunctionNode = LessFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        lessFunctionNode->inferStamp(dummySchema);
        lessFunctionNode->removeChild(readFunctionNodeLeft);
        EXPECT_FALSE(lessFunctionNode->validateBeforeLowering());
    }

    {
        ...
        const auto lessFunctionNode = LessFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        lessFunctionNode->inferStamp(dummySchema);
        lessFunctionNode->removeChild(readFunctionNodeRight);
        EXPECT_FALSE(lessFunctionNode->validateBeforeLowering());
    }
...
TEST_F(LessFunctionNodeTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        const auto lessFunctionNode = LessFunctionNode::create(readFunctionNodeLeft, readFunctionNodeRight);
        lessFunctionNode->inferStamp(dummySchema);
        EXPECT_EQ(lessFunctionNode->validateBeforeLowering(), expectedValue);
    };
