...
class nodeFunctionLessEqualsTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("nodeFunctionLessEqualsTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup nodeFunctionLessEqualsTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(nodeFunctionLessEqualsTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
        ...
        const auto nodeFunctionLessEquals = NodeFunctionLessEquals::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionLessEquals->inferStamp(dummySchema);
        nodeFunctionLessEquals->removeChildren();
        EXPECT_TRUE(nodeFunctionLessEquals->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionLessEquals = NodeFunctionLessEquals::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionLessEquals->inferStamp(dummySchema);
        nodeFunctionLessEquals->removeChild(nodeFunctionReadLeft);
        EXPECT_FALSE(nodeFunctionLessEquals->validateBeforeLowering());
    }

    {
        ...
        const auto nodeFunctionLessEquals = NodeFunctionLessEquals::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionLessEquals->inferStamp(dummySchema);
        nodeFunctionLessEquals->removeChild(nodeFunctionReadRight);
        EXPECT_FALSE(nodeFunctionLessEquals->validateBeforeLowering());
    }
...
TEST_F(nodeFunctionLessEqualsTest, validateBeforeLoweringDifferentDataTypes)
{
    auto testValidateBeforeLoweringDifferentDataTypes = [&](const std::string& leftType, const std::string& rightType, const bool expectedValue) {
        ...
        const auto nodeFunctionLessEquals = NodeFunctionLessEquals::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionLessEquals->inferStamp(dummySchema);
        EXPECT_EQ(nodeFunctionLessEquals->validateBeforeLowering(), expectedValue);
    };
