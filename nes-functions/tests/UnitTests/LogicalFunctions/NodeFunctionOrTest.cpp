...
class NodeFunctionOrTest : public Testing::BaseUnitTest
{
public:
    static void SetUpTestCase()
    {
        Logger::setupLogging("NodeFunctionOrTest.log", LogLevel::LOG_DEBUG);
        NES_INFO("Setup NodeFunctionOrTest test class.");
    }
    void SetUp() override
    {
        BaseUnitTest::SetUp();
...
TEST_F(NodeFunctionOrTest, validateBeforeLoweringDifferentChildNumbers)
{
    {
...

    {
        ...
        const auto nodeFunctionOr = NodeFunctionOr::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionOr->inferStamp(dummySchema);
        nodeFunctionOr->removeChild(nodeFunctionReadLeft);
...

    {
        ...
        const auto nodeFunctionOr = NodeFunctionOr::create(nodeFunctionReadLeft, nodeFunctionReadRight);
        nodeFunctionOr->inferStamp(dummySchema);
        nodeFunctionOr->removeChild(nodeFunctionReadRight);
        EXPECT_FALSE(nodeFunctionOr->validateBeforeLowering());
    }
