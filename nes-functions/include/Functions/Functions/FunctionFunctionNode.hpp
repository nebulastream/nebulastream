...
/**
 * @brief This function node represents a function with a specific name.
 * Internally it stores a LogicalFunction, which is used for inference.
 */
class FunctionFunction final : public FunctionNode
{
public:
    /**
     * @brief Factory method to create a ConstantValueFunctionNode.
     */
    static FunctionNodePtr
    create(const DataTypePtr& stamp, const std::string& functionName, const std::vector<FunctionNodePtr>& arguments);

    /**
     * @brief On a function value function infer stamp invokes inferStamp on the child LogicalFunction
    ...
    */
     ...
     */
    std::vector<FunctionNodePtr> getArguments() const;

    ~FunctionFunction() noexcept override = default;

    explicit FunctionFunction(DataTypePtr stamp, std::string functionName, std::unique_ptr<LogicalFunction> function);
