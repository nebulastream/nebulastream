...
{
    if (rhs->instanceOf<NodeFunctionExp>())
    {
        auto otherExpNode = rhs->as<NodeFunctionExp>();
...
{
    return NodeFunctionExp::create(children[0]->as<FunctionNode>()->deepCopy());
}

bool NodeFunctionExp::validateBeforeLowering() const
{
    if (children.size() != 1)
    {
        return false;
