...
{
    if (NES::Util::instanceOf<FloorFunctionNode>(rhs))
    {
        auto otherFloorNode = NES::Util::as<FloorFunctionNode>(rhs);
...
{
    return FloorFunctionNode::create(Util::as<FunctionNode>(children[0])->copy());
}
