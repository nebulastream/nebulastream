...
{
    if (rhs->instanceOf<NodeFunctionPow>())
    {
        auto otherAddNode = rhs->as<NodeFunctionPow>();
...
{
    return NodeFunctionPow::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}
