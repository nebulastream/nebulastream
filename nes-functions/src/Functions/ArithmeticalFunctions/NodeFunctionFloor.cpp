...
{
    if (rhs->instanceOf<NodeFunctionFloor>())
    {
        auto otherFloorNode = rhs->as<NodeFunctionFloor>();
...
{
    return NodeFunctionFloor::create(children[0]->as<FunctionNode>()->deepCopy());
}

bool NodeFunctionFloor::validateBeforeLowering() const
{
    if (children.size() != 1)
    {
        return false;
