...
NodeFunctionArithmeticalBinary::NodeFunctionArithmeticalBinary(DataTypePtr stamp, std::string name)
    : NodeFunctionBinary(std::move(stamp), std::move(name)), ArithmeticalFunctionNode()
{
}
...
{
    if (rhs->instanceOf<NodeFunctionArithmeticalBinary>())
    {
        auto otherAddNode = rhs->as<NodeFunctionArithmeticalBinary>();
