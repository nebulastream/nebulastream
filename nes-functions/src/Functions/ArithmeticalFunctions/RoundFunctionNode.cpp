...
{
    if (NES::Util::instanceOf<RoundFunctionNode>(rhs))
    {
        auto otherRoundNode = NES::Util::as<RoundFunctionNode>(rhs);
...
{
    return RoundFunctionNode::create(Util::as<FunctionNode>(children[0])->copy());
}
