...
{
    if (rhs->instanceOf<NodeFunctionMul>())
    {
        auto otherMulNode = rhs->as<NodeFunctionMul>();
...
{
    return NodeFunctionMul::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}
