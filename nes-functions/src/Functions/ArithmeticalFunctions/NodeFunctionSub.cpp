...
{
    if (rhs->instanceOf<NodeFunctionSub>())
    {
        auto otherSubNode = rhs->as<NodeFunctionSub>();
...
{
    return NodeFunctionSub::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}
