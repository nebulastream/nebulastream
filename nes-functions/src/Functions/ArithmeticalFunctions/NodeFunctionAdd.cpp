...
{
    if (rhs->instanceOf<NodeFunctionAdd>())
    {
        auto otherAddNode = rhs->as<NodeFunctionAdd>();
...
{
    return NodeFunctionAdd::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}
