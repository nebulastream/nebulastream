...
{
    if (rhs->instanceOf<NodeFunctionGreater>())
    {
        auto other = rhs->as<NodeFunctionGreater>();
...
{
    return NodeFunctionGreater::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}

bool NodeFunctionGreater::validateBeforeLowering() const
{
