...
NodeFunctionLogicalBinary::NodeFunctionLogicalBinary(std::string name)
    : NodeFunctionBinary(DataTypeProvider::provideDataType(DataType::Type::BOOLEAN), std::move(name)), LogicalNodeFunction()
{
}
...
bool NodeFunctionLogicalBinary::validateBeforeLowering() const
{
    ...
    if (childLeft->getStamp().isType(DataType::Type::VARSIZED) or childRight->getStamp().isType(DataType::Type::VARSIZED))
    {
        return false;
    }
