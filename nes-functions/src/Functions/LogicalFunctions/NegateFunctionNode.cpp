...
{
    if (NES::Util::instanceOf<NegateFunctionNode>(rhs))
    {
        auto other = NES::Util::as<NegateFunctionNode>(rhs);
...
{
    return NegateFunctionNode::create(Util::as<FunctionNode>(children[0])->deepCopy());
}
