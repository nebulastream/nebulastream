...
{
    if (rhs->instanceOf<NodeFunctionEquals>())
    {
        auto other = rhs->as<NodeFunctionEquals>();
...
{
    return NodeFunctionEquals::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}
