...
{
    if (rhs->instanceOf<NodeFunctionLessEquals>())
    {
        auto other = rhs->as<NodeFunctionLessEquals>();
...
{
    return NodeFunctionLessEquals::create(children[0]->as<FunctionNode>()->deepCopy(), children[1]->as<FunctionNode>()->deepCopy());
}

bool NodeFunctionLessEquals::validateBeforeLowering() const
{
    if (children.size() != 2)
    {
        return false;
