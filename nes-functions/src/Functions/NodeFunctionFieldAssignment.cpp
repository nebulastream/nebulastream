...
{
    if (rhs->instanceOf<NodeFunctionFieldAssignment>())
    {
        auto otherFieldAssignment = rhs->as<NodeFunctionFieldAssignment>();
...
{
    return getLeft()->as<NodeFunctionFieldAccess>();
}
...
{
    return NodeFunctionFieldAssignment::create(getField()->deepCopy()->as<NodeFunctionFieldAccess>(), getAssignment()->deepCopy());
}
