...
void CaseFunctionNode::inferStamp(SchemaPtr schema)
{
    ...
    for (auto elem : whenChildren)
    {
        ...
        if (!NES::Util::instanceOf<WhenFunctionNode>(elem))
        {
            NES_THROW_RUNTIME_ERROR(
...
std::vector<FunctionNodePtr> CaseFunctionNode::getWhenChildren() const
{
    ...
    {
        whenChildren.push_back(Util::as<FunctionNode>(*whenIter));
    }
    return whenChildren;
}
...
FunctionNodePtr CaseFunctionNode::getDefaultExp() const
{
    ...
    return Util::as<FunctionNode>(*(children.end() - 1));
}

bool CaseFunctionNode::equal(NodePtr const& rhs) const
{
    if (NES::Util::instanceOf<CaseFunctionNode>(rhs))
    {
        auto otherCaseNode = NES::Util::as<CaseFunctionNode>(rhs);
