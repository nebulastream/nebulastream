...
{
    std::unordered_map<std::shared_ptr<Operator>, std::shared_ptr<OperatorPipeline>> pipelineOperatorMap;
    auto pipelinePlan = std::make_shared<PipelineQueryPlan>(queryPlan->getQueryId());
    ...
    {
        INVARIANT(
            dynamic_cast<SinkLogicalOperator*>(sinkOperator.get()),
        ...
        pipeline->prependOperator(std::make_shared<PhysicalOperatorNode>(dynamic_cast<SinkLogicalOperator*>(sinkOperator.get())));
