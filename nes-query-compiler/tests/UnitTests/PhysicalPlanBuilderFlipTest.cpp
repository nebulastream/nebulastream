...
public:
    ...
    std::shared_ptr<PhysicalOperatorWrapper> makeSourceWrapper()
    {
        ...
        auto descriptor = sourceCatalog.getInlineSource(
            Identifier::parse("File"), schema, Host("localhost"), {{Identifier::parse("type"), "CSV"}}, {{Identifier::parse("file_path"), "/dev/null"}});
    ...
    {
        auto schema = createSchema();
        auto descriptor = sinkCatalog.getInlineSink(schema, Identifier::parse("Print"), Host("localhost"), {{Identifier::parse("output_format"), "CSV"}});
        EXPECT_TRUE(descriptor.has_value());
