...
/// Two independent sink->source chains. After flip, two separate source roots, no cross-linking.
TEST_F(PhysicalPlanBuilderFlipTest, DisjointForestFlip)
{
    auto sourceA = makeSourceWrapper();
    auto sinkA = makeSinkWrapper();
    auto sourceB = makeSourceWrapper();
    auto sinkB = makeSinkWrapper();
    sinkA->addChild(sourceA);
    sinkB->addChild(sourceB);
