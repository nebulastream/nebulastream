...
public:
    ...
    LogicalPlan makeOptimizedSingleSinkPlan(const std::string& sourceName, const std::string& sinkName)
    {
        ...
        return RuleBasedOptimizer{QueryOptimizerConfiguration{}, sourceCatalog, sinkCatalog, modelCatalog}.optimize(std::move(plan));
    }
...
TEST_F(MultiRootLoweringTest, SecondSinkRootIsNotDropped)
{
    ...
    /// optimizer-bound first sink (see file header), everything below is shared as-is.
...
TEST_F(MultiRootLoweringTest, SharedSourceIsLoweredOnce)
{
    ...
    /// Same trait-copy scaffolding as above (see file header).
