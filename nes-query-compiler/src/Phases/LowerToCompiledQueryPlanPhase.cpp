...
void LowerToCompiledQueryPlanPhase::processSource(const std::shared_ptr<Pipeline>& pipeline)
{
    ...
    {
        if (auto executableSuccessor = processSuccessor(sourceOperator.getOriginId(), successor))
        {
    ...
    sources.emplace_back(sourceOperator.getOriginId(), sourceOperator.id, sourceOperator.getDescriptor(), std::move(executableSuccessorPipelines));
}
