...
std::shared_ptr<OperatorPipeline> apply(std::shared_ptr<OperatorPipeline> pipeline)
{
    ...
    //    "toConsole",
        ...
        if (const auto provider
            = ExecutablePipelineProviderRegistry::instance().create(pipeline->getPipelineProviderType(), providerArguments))
        {
            auto pipelineStage = provider.value()->create(pipeline, options);
