...
[[nodiscard]] [[maybe_unused]] bool endsInEmitPhysicalOperator(const Pipeline& pipeline)
{
    ...
    const std::shared_ptr<Pipeline>& pipeline, const PhysicalOperatorWrapper& wrappedOp, const uint64_t configuredBufferSize)
{
    ...
    INVARIANT(
        !endsInEmitPhysicalOperator(*pipeline), "Pipeline is already closed with an emit; adding a second one would duplicate its output");
    ...
    const uint64_t configuredBufferSize)
{
    ...
    const auto outputFormatterDescriptor = sink.getDescriptor().getOutputFormatterDescriptor();
    ...
    addOutputFormattingEmit(formattingPipeline, sinkWrapper, configuredBufferSize, outputFormatterDescriptor);
    ...
    const MergePointSet& mergePoints)
{
    ...
    INVARIANT(
        policy != PipelinePolicy::ForceNewClosed || endsInEmitPhysicalOperator(*currentPipeline)
        ...
        if (currentPipeline->isSourcePipeline())
        {
            ...
            if (not(sourceFormat == Identifier::parse("NATIVE") and sinkOutputFormatterDescriptor.isNative()))
            {
                const auto sourcePipeline = std::make_shared<Pipeline>(createScanOperator(
                    *currentPipeline,
                    currentPipeline->getRootOperator().get<SourceDescriptorPhysicalOperator>().getDescriptor().getSchema(),
                ...
                INVARIANT(
                    ...
                    "Formatting pipelines are expected to have shape ScanPhysicalOperator -> EmitPhysicalOperator");
