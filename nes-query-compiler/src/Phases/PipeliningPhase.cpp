...
createScanOperator(const Pipeline& prevPipeline, const std::optional<Schema>& inputSchema, const uint64_t configuredBufferSize)
{
    ...
    const auto memoryProvider = Interface::BufferRef::TupleBufferRef::create(configuredBufferSize, inputSchema.value());
    ...
    uint64_t configuredBufferSize)
{
    ...
    {
        /// Add a formatting pipeline if the source and sink format don't match
        if (currentPipeline->isSourcePipeline())
        {
            const auto sourceFormat = NES::Util::toUpperCase(
                currentPipeline->getRootOperator().get<SourcePhysicalOperator>().getDescriptor().getParserConfig().parserType);
            const auto sinkFormat = NES::Util::toUpperCase(sink->getDescriptor().getSinkType());
