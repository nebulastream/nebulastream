...
INLJJoinCondition getCondition(const LogicalFunction& function, const LogicalOperator& leftChild, const LogicalOperator& rightChild)
{
...
LoweringRuleResultSubgraph LowerToPhysicalINLJoin::apply(LogicalOperator logicalOperator)
{
    ...
    const auto timeCharacteristicsAreBound = std::holds_alternative<std::array<Windowing::BoundTimeCharacteristic, 2>>(timeCharacteristics);
    ...
    auto createTriggerStrategy = [&]() -> JoinTriggerStrategy
    {
        ...
        throw UnknownJoinStrategy("Unsupported INLJ join type");
    };
    auto handler = std::make_shared<INLJOperatorHandler>(
        inputOriginIds, (*outputOriginIds)[0], std::move(sliceAndWindowStore), createTriggerStrategy());
    ...
    const auto createProbeWrapper = [&](auto probe)
    {
    ...
    static_assert(JoinProbeOperator<INLJOuterProbePhysicalOperator>);
    std::shared_ptr<PhysicalOperatorWrapper> probeWrapper;
    if (isOuterJoin(currentJoinType))
    {
        PRECONDITION(
            INLJOuterProbePhysicalOperator::supportsJoinType(currentJoinType), "INLJOuterProbePhysicalOperator does not support join type");
        probeWrapper = createProbeWrapper(INLJOuterProbePhysicalOperator{
            handlerId,
    ...
    {
        PRECONDITION(
            INLJInnerProbePhysicalOperator::supportsJoinType(currentJoinType), "INLJInnerProbePhysicalOperator does not support join type");
        probeWrapper = createProbeWrapper(INLJInnerProbePhysicalOperator{
            handlerId,
            std::move(joinFunction),
            WindowMetaData{join->getStartField(), join->getEndField()},
            joinSchema,
