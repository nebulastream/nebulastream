...
LoweringRuleResultSubgraph LowerToPhysicalIntervalJoin::apply(LogicalOperator logicalOperator)
{
    ...
    const uint64_t tupleSizeAnchor = getSizeInBytes(anchorTupleLayout->getSchema());
    const uint64_t tupleSizePartner = getSizeInBytes(partnerTupleLayout->getSchema());
    ...
    auto sliceStoreRefAnchor = handler->getAnchorStore().createSliceStoreRef(
        [](Slice& slice, const WorkerThreadId workerThreadId, AbstractBufferProvider&)
        {
            ...
            return ijSlice.getPagedVectorRef(workerThreadId);
        },
        [tupleSizeAnchor](IntervalJoinOperatorHandler& operatorHandler, AbstractBufferProvider& bufferProvider)
        {
            const CreateNewIntervalJoinSliceArgs intervalSliceArgs{bufferProvider, tupleSizeAnchor};
            return operatorHandler.getCreateNewSlicesFunction(intervalSliceArgs);
        });
    auto sliceStoreRefPartner = handler->getPartnerStore().createSliceStoreRef(
        ...
        [tupleSizePartner](IntervalJoinOperatorHandler& operatorHandler, AbstractBufferProvider& bufferProvider)
        {
