...
#include <LoweringRules/LowerToPhysical/LowerToPhysicalDelta.hpp>
...
LoweringRuleResultSubgraph LowerToPhysicalDelta::apply(LogicalOperator logicalOperator)
{
    ...
    for (const auto& [alias, function] : delta->getDeltaExpressions())
    {
        ...
        deltaFieldLayout.push_back({targetField, dataType});
        deltaFieldsEntrySize += dataType.getSizeInBytesWithoutNull();
    }
    ...
    {
        fullRecordLayout.push_back({field.name, field.dataType});
        fullRecordEntrySize += field.dataType.getSizeInBytesWithoutNull();
    }
...
LoweringRuleGeneratedRegistrar::RegisterDeltaLoweringRule(LoweringRuleRegistryArguments argument) /// NOLINT
{
