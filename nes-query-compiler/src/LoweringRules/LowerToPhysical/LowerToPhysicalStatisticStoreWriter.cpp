...
Record::RecordFieldIdentifier resolveField(const Schema<QualifiedUnboundField, Ordered>& schema, const std::string& name)
{
    ...
    throw QueryCompilerError("StatisticStoreWriter: field '{}' not found in input schema", name);
}
}

LoweringRuleResultSubgraph LowerToPhysicalStatisticStoreWriter::apply(LogicalOperator logicalOperator)
{
    const auto writer = logicalOperator.getAs<StatisticStoreWriterLogicalOperator>();
    ...
    const auto inputSchema = createPhysicalOutputSchema(writer->getChild().getTraitSet());

    const auto handlerId = getNextOperatorHandlerId();
    auto handler = std::make_shared<StatisticStoreOperatorHandler>(statisticStore);

    auto physicalOperator = StatisticStoreWriter(
        handlerId,
        writer->getStatisticId(),
        resolveField(inputSchema, writer->getStartFieldName()),
        resolveField(inputSchema, writer->getEndFieldName()),
        resolveField(inputSchema, writer->getValueFieldName()));
...
{
    return std::make_unique<LowerToPhysicalStatisticStoreWriter>(argument.conf, argument.statisticStore);
}
