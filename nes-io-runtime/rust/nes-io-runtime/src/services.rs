...
impl ServiceRegistry {
        ...
        T: Any + Send + Sync + 'static,
    {
        ...
        let guard = slot
            .cell
            .lock()
            .expect("ServiceRegistry slot mutex poisoned");
        let weak = guard.as_ref()?;
        let strong = weak.upgrade()?;
        Some(
            strong
                .downcast::<T>()
                .expect("ServiceRegistry type mismatch"),
        )
    }
...
mod tests {
    ...
    fn try_get_or_init_error_does_not_poison_slot() {
        let reg = ServiceRegistry::default();
        let err: Result<Arc<Foo>, &'static str> = reg.try_get_or_init::<Foo, _, _>(|| Err("nope"));
