...
TEST_F(RandomSampleWithoutReplacementTest, sampleTestCount) {
    ...
    EXPECT_EQ(dynamicBuffer[0][idString].read<uint64_t>(), 0);
    EXPECT_EQ(dynamicBuffer[0][approximateString].read<int64_t>(), 1);

    EXPECT_EQ(dynamicBuffer[1][idString].read<uint64_t>(), 1);
    EXPECT_EQ(dynamicBuffer[1][approximateString].read<int64_t>(), 0);

    EXPECT_EQ(dynamicBuffer[2][idString].read<uint64_t>(), 2);
    EXPECT_EQ(dynamicBuffer[2][approximateString].read<int64_t>(), 1);

    EXPECT_EQ(dynamicBuffer[3][idString].read<uint64_t>(), 3);
    EXPECT_EQ(dynamicBuffer[3][approximateString].read<int64_t>(), 0);

    EXPECT_EQ(dynamicBuffer[4][idString].read<uint64_t>(), 4);
    EXPECT_EQ(dynamicBuffer[4][approximateString].read<int64_t>(), 1);
}
