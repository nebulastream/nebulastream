...
namespace NES::QueryCompilation::LowerToPhysicalOperators
{

std::unique_ptr<PhysicalQueryPlan> apply(const std::unique_ptr<QueryPlan> queryPlan)
{
    ...
    for (const auto& operatorNode : BFSRange<Operator>(queryPlan->getRootOperators()[0]))
    {
        ...
        else if (auto loweredOperator = RewriteRuleRegistry::instance().create(operatorNode->getName()))
        {
            auto tmp = currentOperator;
            currentOperator = loweredOperator;
            tmp->children.push_back(currentOperator);
        }
        else
        {
    ...
    return std::make_unique<PhysicalQueryPlan>(queryPlan->getQueryId(), std::vector{rootOperator});
}
}
