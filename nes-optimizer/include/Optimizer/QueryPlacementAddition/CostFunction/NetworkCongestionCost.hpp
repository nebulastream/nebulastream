...
/**
 * Represent a network congestion (i.e., bandwidth over-utilization) as a result
 * of a placement.
 */
class NetworkCongestionCost : public AbstractQueryPlacementCost {
 ...
 private:
  /**
   * @brief calculate the total output rate of event streams produced by all
   * running pipelines on a single node.
  ...
  /**
   * @brief obtain the average ingestion rate in the last 'period' of second
   * from the Statistic coordinator
   ...
   * @param period time in seconds
   * @param pinnedUpstreamOperators operator of which the selectivity is to be
   * identified
   ...
   * // TODO #4664: implement actual method to obtain the operator Id from a
   * given global query plan
   */
  OperatorId stubGetOperatorIdFromCandidateEntry(
      const PlacementMatrix& candidate, uint32_t row, uint32_t col);

  WorkerId stubGetExecutionNodeFromCandidateEnetry(
