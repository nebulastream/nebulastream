...
/**
 * @brief This class implements Integer Linear Programming strategy to perform
...
public:
  ...
  /**
   * @brief set the relative weight for the overutilization cost to be used when
   * computing weighted sum in the final cost
  ...
  /**
   * @brief set the relative weight for the network cost to be used when
   * computing weighted sum in the final cost
   * @param weight the relative weight
   */
...
private:
   ...
   * @param placementVariables a mapping between concatenation of operator id
   * and placement id and their z3 expression
   */
  bool pinOperators(z3::model &z3Model,
                    std::map<std::string, z3::expr> &placementVariables);

  /**
   * @brief is called from addConstraints and calling itself recursivly with
   * parents of operator Node to identify their location on topologyPath
   * @param logicalOperator the current logical operator
   * @param opt an instance of the Z3 optimize class
   * @param placementVariable a mapping between concatenation of operator id and
   * placement id and their z3 expression
   * @param pinnedDownStreamOperators
   * @param operatorDistanceMap a mapping between operators (represented by ids)
   * to their next operator in the topology
   * @param nodeUtilizationMap a mapping of topology nodes and their node
   * utilization
   * @param nodeMileageMap a mapping of topology node (represented by string id)
   * and their distance to the root node
   */
  void identifyPinningLocation(
      const LogicalOperatorPtr &logicalOperator, z3::optimize &opt,
      std::unordered_map<OperatorId, std::vector<WorkerId>>
  ...
  /**
   * @brief Populate the placement variables and adds constraints to the
   * optimizer
   * @param opt an instance of the Z3 optimize class
   ...
   */
  void
  addConstraints(z3::optimize &opt,
                 const std::set<LogicalOperatorPtr> &pinnedUpStreamOperators,
                 const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators,
                 std::map<std::string, z3::expr> &placementVariable,
                 std::map<OperatorId, z3::expr> &operatorDistanceMap,
   ...
   * @return a mapping of topology node (represented by string id) and their
   * distance to the root node
   */
  std::map<WorkerId, double>
  computeMileage(const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators);

  /**
   * @brief calculates the mileage property for a node
   * @param node topology node for which mileage is calculated
   * @param mileages a mapping of topology node (represented by string id) and
   * their distance to the root node
   */
  void computeDistance(const TopologyNodePtr &node,
                       std::map<WorkerId, double> &mileages);
