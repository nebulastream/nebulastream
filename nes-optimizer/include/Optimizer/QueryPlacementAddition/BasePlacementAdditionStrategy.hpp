...
using ComputedDecomposedQueryPlans =
    std::unordered_map<WorkerId, std::vector<DecomposedQueryPlanPtr>>;
using ExecutionNodeWLock =
    std::shared_ptr<folly::Synchronized<ExecutionNodePtr>::WLockedPtr>;

/**
 * @brief: This is the interface for base optimizer that needed to be
 * implemented by any new query optimizer.
 */
class BasePlacementAdditionStrategy {
 public:
  explicit BasePlacementAdditionStrategy(
  ...
  /**
   * Update Global execution plan by placing operators including and between
   * input pinned upstream and downstream operators for the query with input id
   * and input fault tolerance strategy
   ...
   */
  virtual PlacementAdditionResult updateGlobalExecutionPlan(
 ...
 protected:
  /**
   * Find topology path for placing operators between the input pinned upstream
   ...
   */
  void performPathSelection(
      const std::set<LogicalOperatorPtr>& upStreamPinnedOperators,
      const std::set<LogicalOperatorPtr>& downStreamPinnedOperators);

  /**
   * @brief Iterate through operators between pinnedUpStreamOperators and
   * pinnedDownStreamOperators and compute decomposed query plans on the
   ...
   */
  ComputedDecomposedQueryPlans computeDecomposedQueryPlans(
      SharedQueryId sharedQueryId,
      const std::set<LogicalOperatorPtr>& pinnedUpStreamOperators,
      const std::set<LogicalOperatorPtr>& pinnedDownStreamOperators);
   ...
   * @param computedDecomposedQueryPlans: update the computed sub query plan by
   * adding network source and sink operators
   */
  void addNetworkOperators(
   ...
   * @return vector of deployment contexts
   */
  PlacementAdditionResult updateExecutionNodes(
      SharedQueryId sharedQueryId,
   ...
   * @return vector of topology nodes where child operator was placed or empty
   * if not all children operators are placed
   */
  std::vector<TopologyNodePtr> getTopologyNodesForChildrenOperators(
      const LogicalOperatorPtr& operatorNode);

  /**
   * @brief Perform locking of all topology nodes selected by the path selection
   * algorithm. We use "back-off and retry" mechanism to lock topology nodes
   * following a strict breadth-first order. This allows us to prevent deadlocks
   * and starvation situation.
   * @param sourceTopologyNodes: the topology nodes hosting the pinned upstream
   * operators
  ...
  /**
   * @brief Perform unlocking of all topology nodes on which the lock was
   * acquired. We following an order inverse of the lock acquisition. This
   * allows us to prevent starvation situation.
   * @return true if successful else false
   */
 ...
 private:
   ...
   * @param sourceOperatorId : the operator id of the corresponding source
   * operator
   * @param sourceTopologyNode : the topology node to which sink operator will
   * send the data
   * @return the instance of network sink operator
   */
  static LogicalOperatorPtr createNetworkSinkOperator(
   ...
   */
  static LogicalOperatorPtr createNetworkSourceOperator(
      SharedQueryId sharedQueryId, SchemaPtr inputSchema, OperatorId operatorId,
      const TopologyNodePtr& sinkTopologyNode);

  /**
   * @brief Select path for placement using pessimistic 2PL strategy. If attempt
   * fails then an exponential retries are performed. NOTE: These paths are
   * local copies of the topology nodes. Any changes done on these nodes are not
   * reflected in the topology catalog.
   * @param topologyNodesWithUpStreamPinnedOperators : topology nodes hosting
   * the pinned upstream operators
   * @param topologyNodesWithDownStreamPinnedOperators : topology nodes hosting
   * the pinned downstream operators
   * @return true if successful else false
   */
  ...
  /**
   * @brief Select path for placement using optimistic approach where no locks
   * are acquired on chosen topology nodes after the path selection. If attempt
   * fails then an exponential retries are performed. NOTE: These paths are
   * local copies of the topology nodes. Any changes done on these nodes are not
   * reflected in the topology catalog.
   * @param topologyNodesWithUpStreamPinnedOperators : topology nodes hosting
   ...
   * the pinned downstream operators
   */
  bool optimisticPathSelection(
      const std::set<WorkerId>& topologyNodesWithUpStreamPinnedOperators,
      const std::set<WorkerId>& topologyNodesWithDownStreamPinnedOperators);

  /**
   * @brief call the appropriate find path method on the topology to select the
   * placement path
   * @param topologyNodesWithUpStreamPinnedOperators : topology nodes hosting
   * the pinned upstream operators
   * @param topologyNodesWithDownStreamPinnedOperators : topology nodes hosting
   ...
   */
  std::vector<TopologyNodePtr> findPath(
      const std::set<WorkerId>& topologyNodesWithUpStreamPinnedOperators,
      const std::set<WorkerId>& topologyNodesWithDownStreamPinnedOperators);
 ...
 private:
   ...
   * @param decomposedQueryPlan: the  decomposed query plan updated by the
   * placement addition strategy
   */
  void markOperatorsAsPlaced(WorkerId workerId,
                             DecomposedQueryPlanPtr decomposedQueryPlan);

  /**
   * @brief check if a computed sink operator corresponds to a placed sink that
   * is to be reconfigured. If so, update the version, the receiver location and
   * the partition of the placed sink
   * @param querySubPlanVersion the version to apply to the reconfigured sink
   * @param computedQuerySubPlan the sub plan containing the newly computed
   * changed to be applied to the placed plans
   * @param upstreamOperatorOfPlacedSinksToCheck the placed operator whose
   * parents are to be checked if they can be merged with the new sink
   * @param newNetworkSinkOperator the new sink which is to be checked if ti can
   * be merged with an existing sink
   * @return true if merging was performed, false if no matching sink could be
   * found
   */
  bool tryMergingNetworkSink(
  ...
  /**
   * @brief chack if a computed source operator corresponds to a placed source
   * that is to be reconfigured. If so, update the version and the sender
   * location of the placed source
   * @param querySubPlanVersion the version to apply to the reconfigured sink
   * @param placedDownstreamOperator
   * @param newNetworkSourceOperator
   * @return
   */
  bool tryMergingNetworkSource(
      DecomposedQueryPlanVersion querySubPlanVersion,
  ...
  // Time interval in which to retry
  static constexpr auto PATH_SELECTION_RETRY_WAIT =
      std::chrono::milliseconds(1000);
  static constexpr auto MAX_PATH_SELECTION_RETRY_WAIT =
