...
/**
 * @brief This class allows us to communicate with external planner service to
 * perform operator placement.
 */
class ElegantPlacementStrategy : public BasePlacementAdditionStrategy {
public:
  // Keys for information that is used during ELEGANT placement and stored in
  // node properties. This key is also accessed in
  // SampleCodeGenerationPhase.cpp.
...
public:
  ...
  create(const std::string &serviceURL, PlacementStrategy placementStrategy,
...
private:
  explicit ElegantPlacementStrategy(
   ...
   * @param pinnedUpStreamOperators: pinned upstream operators of the plan to be
   * placed
   * @param pinnedDownStreamOperators: pinned downstream operators of the plan
   * to be placed
   * @return json representing the payload
   */
  void prepareQueryPayload(
      const std::set<LogicalOperatorPtr> &pinnedUpStreamOperators,
      const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators,
      nlohmann::json &);
  ...
  /**
   * @brief Add pinning information to the operators based on the response
   * received from the external placement service
   * @param sharedQueryId: shared query id
   * @param pinnedDownStreamOperators: pinned downstream operators
   * @param response: Json representing the placement response received from the
   * external service
   */
  void pinOperatorsBasedOnElegantService(
      SharedQueryId sharedQueryId,
      const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators,
      cpr::Response &response) const;

  /**
   * @brief Add a base64-transformed Java bytecode list to the JSON
   * representation of the operator, if the operator is a MapUDFLogicalOperator
   * or FlatMapUDFLogicalOperator. Otherwise, add an empty field.
   * @param logicalOperator The logical operator that is processed.
   * @param node Target JSON operator.
   */
  void addJavaUdfByteCodeField(const OperatorPtr &logicalOperator,
                               nlohmann::json &node);
