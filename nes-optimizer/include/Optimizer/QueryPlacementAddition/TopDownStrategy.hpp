...
private:
   ...
   */
  void
  pinOperators(const std::set<LogicalOperatorPtr> &pinnedUpStreamOperators,
               const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators);

  /**
   * @brief Try to place input operator on the input topology node
   * @param logicalOperator : the input operator to place
   * @param candidateTopologyNode : the candidate topology node to place
   * operator on
   */
  ...
  /**
   * @brief Get topology node where all parent operators of the input operator
   * are placed
   * @param candidateOperator: the input operator
   * @return vector of topology nodes where parent operator was placed or empty
   * if not all parent operators are placed
   */
  std::vector<TopologyNodePtr> getTopologyNodesForDownStreamOperators(
      const LogicalOperatorPtr &candidateOperator);

  /**
   * @brief Get topology node where all children operators of the input operator
   * are to be placed
   * @param candidateOperator: the input operator
   * @return vector of topology nodes where child operator are to be placed
   */
  std::vector<TopologyNodePtr> getTopologyNodesForUpStreamOperators(
      const LogicalOperatorPtr &candidateOperator);
};
