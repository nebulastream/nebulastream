...
/**\brief:
 *          This class implements Bottom Up placement strategy. In this
 * strategy, the source and sink operators are placed at respective nes nodes
 * but rest of the operators are placed starting near to the source and then if
 * the resources are not available they are placed on a node neighbouring to the
 * node or one level up.
 */
...
private:
  ...
  /**
   * @brief Try to pin input operator on the input topology node otherwise find
   * appropriate node to pin the operator
   * @param logicalOperator : the input operator to place
   * @param candidateTopologyNode : the candidate topology node to place
   * operator on
   * @param pinnedDownStreamOperators: list of pinned downstream node after
   * which placement stops
   */
