...
class SignatureContainmentCheck {
 public:
   ...
   * @param allowExhaustiveContainmentCheck if true, we allow exhaustive
   * containment checks that can result in the whole SQP needing re-deployment,
   * if it's false, we only check if a new query is contained by the SQP
   * resulting in a faster containment check but we might miss some valuable
   * optimization opportunities
   * @return instance of SignatureContainmentUtil
   */
  static SignatureContainmentCheckPtr create(
   ...
   */
  ContainmentRelationshipAndOperatorChainPtr checkContainmentForBottomUpMerging(
      const LogicalOperatorPtr& leftSignature,
   ...
   *      In case of window equality, we continue to check for projection
   * containment In case of projection equality, we finally check for filter
   * containment We assume that a merging algorithm checks the containment
   * relationships in a top-down manner and merges the upstream operator chains
   * in case of an equivalence. In case of a containment relationship, we
   * extract the contained operators and add them to the container's upstream
   * operator chain
   * @param leftOperator the current operator of the left query
   * @param rightOperator the current operator of the right query
   * @return tuple with the containment relationship and a vector of the
   * contained upstream operators. The vector is empty if equivalence or no
   * containment was detected.
   */
 ...
 private:
   ...
   * else if allowExhaustiveContainmentCheck && (rightFOL && !leftFOL == unsat,
   * aka rightFOL ⊆ leftFOL)
   *      && filters are equal
   ...
   *      false: check if allowExhaustiveContainmentCheck && left sig ⊆ right
   * sig true: return LEFT_SIG_CONTAINED false: return NO_CONTAINMENT
   * @param leftSignature
   * @param rightSignature
   * @return enum with containment relationships
   */
   ...
   *                  true: RIGHT_SIG_CONTAINED
   *                  false: NO_CONTAINMENT
   *              false: check if allowExhaustiveContainmentCheck && right sig ⊆
   * left sig true: LEFT_SIG_CONTAINED false: NO_CONTAINMENT if
   * containmentRelationship != EQUALITY: break loop
   * @param leftSignature
   * @param rightSignature
   * @return enum with containment relationships
   */
   ...
   * if we are given a map value for the attribute, we create a FOL as
   * attributeStringName == mapCondition, e.g. age == 25 else we indicate that
   * the attribute is involved in the projection as attributeStingName == true
   ...
   */
  void createProjectionFOL(const QuerySignaturePtr& signature,
                           z3::expr_vector& projectionFOL);

  /**
   * @brief checks if the combination (combined via &&) of negated conditions
   * and non negated conditions is unsatisfiable it also pops the given number
   * of conditions and calls resetSolver() if the counter hits the threshold for
   * resetting
   ...
   * @return true if the combination of the given conditions is unsatisfiable,
   * false otherwise
   */
  bool checkContainmentConditionsUnsatisfied(z3::expr_vector& negatedCondition,
                                             z3::expr_vector& condition);

  /**
   * @brief checks if window containment is possible, i.e. no join window, no
   * avg or median aggregation, and filter conditions are equal further,
   * window-time-size and window-time-slide must be equal
   ...
   * @return true if all conditions for window containment are met, false
   * otherwise
   */
  ...
  /**
   * @brief checks whether for the remaining attributes all map and aggregation
   * functions are equal
  ...
  /**
   * @brief checks if a smaller attribute list is contained in a larger
   * attribute
   ...
   * @return true, if no union is present or all unions have an equal amount of
   * filters per attribute, false otherwise
   */
  bool checkFilterContainmentPossible(const QuerySignaturePtr& container,
