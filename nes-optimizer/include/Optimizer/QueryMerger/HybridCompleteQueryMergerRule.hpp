...
using HybridCompleteQueryMergerRulePtr =
    std::shared_ptr<HybridCompleteQueryMergerRule>;

/**
 * @brief HybridCompleteQueryMergerRule is responsible for merging together all
 * equal Queries within the Global Query Plan, such that, after running this
 * rule only a single representative operator chain should exists in the Global
 * Query Plan for all of them. Effectively this rule will prune the global query
 * plan for duplicate operators.
 *
 ...
 *
 *
 * After running the HybridCompleteQueryMergerRule, the resulting Global Query
 * Plan will look as follow:
 *
 *                                                         GQPRoot
 *                                                         /     \
 *                                                        /       \
