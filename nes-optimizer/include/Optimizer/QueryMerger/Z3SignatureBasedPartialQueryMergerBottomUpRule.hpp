...
using Z3SignatureBasedPartialQueryMergerBottomUpRulePtr =
    std::shared_ptr<Z3SignatureBasedPartialQueryMergerBottomUpRule>;

/**
 * @brief Z3SignatureBasedPartialQueryMergerBottomUpRule is responsible for
 * merging together all Queries sharing a common upstream operator chain. After
 * running this rule only a single representative operator chain should exists
 * in the Global Query Plan for the common upstream operator chain. Effectively
 * this rule will prune the global query plan for duplicate operators.
 *
 ...
 *
 *
 * After running the Z3SignatureBasedPartialQueryMergerBottomUpRule, the
 * resulting Global Query Plan will look as follow:
 *
 *                                                         GQPRoot
 *                                                         /     \
 *                                                        /       \
 ...
 *
 */
class Z3SignatureBasedPartialQueryMergerBottomUpRule final
    : public BaseQueryMergerRule {
 public:
  static Z3SignatureBasedPartialQueryMergerBottomUpRulePtr create(
 ...
 private:
  explicit Z3SignatureBasedPartialQueryMergerBottomUpRule(
      z3::ContextPtr context);
  /**
   ...
   */
  std::map<LogicalOperatorPtr, LogicalOperatorPtr> areQueryPlansEqual(
   ...
   */
  std::map<LogicalOperatorPtr, LogicalOperatorPtr> areOperatorEqual(
