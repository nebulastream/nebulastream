...
/**
 * @brief This class is responsible for placing and removing operators
 * (depending on their status) from the global execution.
 */
class QueryPlacementAmendmentPhase {
public:
  /**
   ...
   */
  static QueryPlacementAmendmentPhasePtr
  create(GlobalExecutionPlanPtr globalExecutionPlan, TopologyPtr topology,
         TypeInferencePhasePtr typeInferencePhase,
         Configurations::CoordinatorConfigurationPtr coordinatorConfiguration);

  /**
   * @brief Method takes input a shared query plan and performs first query
   * operator placement removal and then addition based on the selected query
   * placement strategy
   ...
   */
  std::set<DeploymentContextPtr>
  execute(const SharedQueryPlanPtr &sharedQueryPlan);
...
private:
   ...
   */
  bool containsOnlyPinnedOperators(
      const std::set<LogicalOperatorPtr> &pinnedOperators);

  /**
   * @brief Check if in the provided set at least one operator is in the state
   * To_Be_Placed or Placed
   * @param operatorsToCheck the logical operator nodes
   * @return true if at least one operator passes the condition
   */
  bool containsOperatorsForPlacement(
      const std::set<LogicalOperatorPtr> &operatorsToCheck);

  /**
   * @brief Check if in the provided set at least one operator is in the state
   * To_Be_RePlaced, Placed, or To_Be_Removed Note: TODO
   * @param operatorsToCheck the logical operator nodes
   * @return true if at least one operator passes the condition
   */
  bool containsOperatorsForRemoval(
      const std::set<LogicalOperatorPtr> &operatorsToCheck);
