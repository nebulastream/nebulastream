...
/**
 * @brief DistributedNemoJoinRule which is rewriting a central to a distributed
 * grid-partitioned join based on the physical streams.
 */
...
public:
  ...
  create(Configurations::OptimizerConfiguration configuration,
         ...
         const std::map<Catalogs::Source::SourceCatalogEntryPtr,
                        std::set<uint64_t>> &distMap);
  virtual ~DistributedNemoJoinRule() = default;

  /**
   * @brief Searches query plan for a join operator and identifes partial joins.
   * Then it replicates the join operators and pushes them closer to the sources
...
private:
   ...
   * @return A map that points from key -> a set of workers that share the same
   * key
   */
  std::map<uint64_t, std::set<WorkerId>> getNodesWithCommonKeys();
...
private:
  TopologyPtr topology;
  Configurations::OptimizerConfiguration configuration;
  std::map<Catalogs::Source::SourceCatalogEntryPtr, std::set<uint64_t>>
      keyDistributionMap;
};
} // namespace NES::Optimizer
