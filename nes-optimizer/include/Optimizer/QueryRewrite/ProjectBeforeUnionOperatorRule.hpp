...
/**
 * @brief This rule is defined for adding a Projection Operator before a Union
 * Operator. Following are the conditions:
 * - If there exists a Union operator with different schemas then add on one
 * side a project operator to make both schema equal.
 */
...
private:
  ...
  /**
   * @brief Construct the project operator to be added between union and one of
   * the child.
   * @param sourceSchema : the source schema for project.
   * @param destinationSchema : the destination schema for project.
   * @return LogicalOperatorPtr: the project operator based on source and
   * destination schema
   */
  static LogicalOperatorPtr
  constructProjectOperator(const SchemaPtr &sourceSchema,
