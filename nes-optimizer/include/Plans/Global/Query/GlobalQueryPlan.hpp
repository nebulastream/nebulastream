...
/**
 * @brief This class is responsible for storing all currently running and to be
 * deployed QueryPlans in the NES system. The QueryPlans included in the
 * GlobalQueryPlan can be fused together and therefore each operator in GQP
 * contains information about the set of queryIdAndCatalogEntryMapping it
 * belongs to. The QueryPlans are bound together by a dummy logical root
 * operator.
 */
class GlobalQueryPlan {
public:
   ...
   * @throws: QueryNotFoundException if the given query plan's id was invalid;
   * GlobalQueryPlanUpdateException if the given query plan was already added
   */
  ...
  /**
   * @brief remove the operators belonging to the query with input query Id from
   ...
   * @throws RuntimeException: "GlobalQueryPlan: Unable to remove query with id
   * {queryId} from shared query plan with id {sharedQueryId}
   */
  void removeQuery(QueryId queryId, RequestType requestType);

  /**
   * @brief This method will remove all empty shared query plans that are
   * deployed
   */
  void removeFailedOrStoppedSharedQueryPlans();

  /**
   * @brief This method will remove the shared query plan if all involved
   * queries are either stopped or failed
  ...
  /**
   * Fetch the Shared query plan consuming the sources and having the same
   * placement strategy with the input source names and the placement strategy
...
private:
  ...
  std::map<std::string, std::vector<SharedQueryPlanPtr>>
