...
/**
 * This class holds the global execution plan for the NES system. The nodes in
 * this graph are represented by ExecutionNode class.
 */
class GlobalExecutionPlan {
...
public:
  ...
  /**
   * @brief Add a decomposed query plan belonging to a shared query plan on the
   * given execution node.
   * @note If the execution node does not exists then create one and create
   * parent child relationships based on the topology node.
   * @param lockedTopologyNode: the locked topology node that will be
   * represented by the execution node
   * @param decomposedQueryPlan: the decomposed query plan
   * @return true if success else false
   */
  bool addDecomposedQueryPlan(const TopologyNodeWLock &lockedTopologyNode,
                              DecomposedQueryPlanPtr decomposedQueryPlan);

  /**
   * @brief Update the decomposed query plan state to the new query state.
   * Note: the operation is successful only if the given decomposed plan version
   * matches the stored decomposed query plan version.
   ...
   */
  bool
  updateDecomposedQueryPlanState(WorkerId WorkerId, SharedQueryId sharedQueryId,
                                 DecomposedQueryPlanId decomposedQueryPlanId,
                                 DecomposedQueryPlanVersion expectedVersion,
                                 QueryState newDecomposedQueryPlanState);

  /**
   * @brief Get the identifier of the shared query plans hosted on the given
   * execution node
  ...
  getCopyOfDecomposedQueryPlan(WorkerId WorkerId, SharedQueryId sharedQueryId,
                               DecomposedQueryPlanId decomposedQueryPlanId);

  /**
   * @brief Get the copy of all decomposed query plans originating from the
   * given shared query id and hosted on the execution node with given id.
   * @param WorkerId : the id of the execution node
  ...
  getCopyOfAllDecomposedQueryPlans(WorkerId WorkerId,
   ...
   * @param sharedQueryId : the shared query id used for removing the input
   * query
   * @return true if successful else false
   */
  ...
  bool removeDecomposedQueryPlan(
      WorkerId WorkerId, SharedQueryId sharedQueryId,
      DecomposedQueryPlanId decomposedQueryPlanId,
  ...
  /**
   * Return list of Execution Serialization used for placing operators of the
   * input query Id
   * @param sharedQueryId : Id of the shared query
   * @return vector containing execution nodes
   */
  std::vector<ExecutionNodeWLock>
  getLockedExecutionNodesHostingSharedQueryId(SharedQueryId sharedQueryId);
   ...
   * @param sharedQueryId of the shared query plan for which the execution plan
   * is needed
...
private:
   ...
   */
  folly::Synchronized<std::map<WorkerId, folly::Synchronized<ExecutionNodePtr>>>
      idToExecutionNodeMap;

  /**
   * Index based on shared query Id for faster access to the execution nodes
   */
  folly::Synchronized<std::map<SharedQueryId, std::set<WorkerId>>>
      sharedQueryIdToWorkerIdMap;
