...
{
    LowerToPhysicalNLJoin(const Configurations::QueryOptimizerConfiguration& conf) : conf(conf) {}
    std::vector<std::shared_ptr<PhysicalOperator>> applyToPhysical(DynamicTraitSet<QueryForSubtree, Operator>*) override;
    const Configurations::QueryOptimizerConfiguration& conf;
};
