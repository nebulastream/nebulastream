...
SignatureEqualityUtilPtr
SignatureEqualityUtil::create(const z3::ContextPtr &context) {
...
bool SignatureEqualityUtil::checkEquality(const QuerySignaturePtr &signature1,
  ...
  try {
    ...
    if (!conditions || !otherConditions) {
      NES_WARNING(
          "QuerySignature: Can't compare equality between null signatures");
      return false;
    }

    // Check the number of columns extracted by both
    ...
    if (columns.size() != otherColumns.size()) {
      NES_WARNING(
          "QuerySignature: Both signatures have different column entries");
      return false;
    }

    // Check the number of window expressions extracted from both
    // queryIdAndCatalogEntryMapping
    ...
    if (windowsExpressions.size() != otherWindowExpressions.size()) {
      NES_WARNING(
          "QuerySignature: Both signatures have different window expressions");
      return false;
    }
    ...
    if (otherSchemaFieldToExprMaps.size() != schemaFieldToExprMaps.size()) {
      NES_WARNING("QuerySignature: Both signatures have different number of "
                  "Schema Filed to Expr Maps");
      return false;
    }

    // If column from one signature doesn't exist in other signature then they
    // are not equal.
    for (auto schemaFieldToExprMap : schemaFieldToExprMaps) {
      ...
      for (auto otherSchemaMapItr = otherSchemaFieldToExprMaps.begin();
        ...
        // If schema is matched then remove the other schema from the list to
      ...
      // If a matching schema doesn't exists in other signature then two
    ...
    // If window key from one signature doesn't exist in other signature then
    // they are not equal.
    if (windowsExpressions.size() > 1 && otherWindowExpressions.size() > 1) {
      ...
      for (const auto &windowExpression : windowsExpressions[0]) {
        if (otherWindowExpressions[0].find(windowExpression.first) ==
            otherWindowExpressions[0].end()) {
          NES_WARNING(
              "Window expression with key {}", windowExpression.first,
              " doesn't exists in window expressions of other signature");
          return false;
        }
        // For each column expression of the column in other signature we try to
        // create a DNF using
        //  each column expression of the same column in this signature.
        z3::ExprPtr otherWindowExpression =
            otherWindowExpressions[0][windowExpression.first];
        allConditions.push_back(
            to_expr(*context, Z3_mk_eq(*context, *otherWindowExpression,
                                       *windowExpression.second)));
      }
    }

    // Add conditions from both signature into the collection of all conditions
    allConditions.push_back(
        to_expr(*context, Z3_mk_eq(*context, *conditions, *otherConditions)));
    ...
    try {
      ...
      NES_ERROR("SignatureEqualityUtil: Exception occurred while performing "
                "equality check among "
                "queryIdAndCatalogEntryMapping {}",
                e.what());
    }
    return false;
