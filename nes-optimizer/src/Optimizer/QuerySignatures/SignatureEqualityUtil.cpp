...
SignatureEqualityUtilPtr SignatureEqualityUtil::create(
...
bool SignatureEqualityUtil::checkEquality(const QuerySignaturePtr& signature1,
  ...
  try {
    ...
    if (otherSchemaFieldToExprMaps.size() != schemaFieldToExprMaps.size()) {
      NES_WARNING(
          "QuerySignature: Both signatures have different number of Schema "
          "Filed to Expr Maps");
      return false;
    }
    ...
    try {
      ...
      NES_ERROR(
          "SignatureEqualityUtil: Exception occurred while performing equality "
          "check among "
          "queryIdAndCatalogEntryMapping {}",
          e.what());
    }
