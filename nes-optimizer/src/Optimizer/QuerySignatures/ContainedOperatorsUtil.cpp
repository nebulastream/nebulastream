...
std::vector<LogicalOperatorPtr> ContainedOperatorsUtil::createContainedWindowOperator(
    const LogicalOperatorPtr& containedOperator, const LogicalOperatorPtr& containerOperator)
{
    ...
    const auto containerWindowType = containerWindowOperators->as<LogicalWindowOperator>()->getWindowDefinition()->getWindowType();
    ...
    if (!containedWindowOperators.empty())
    {
        ...
        auto watermarkOperatorCopy = containedWindowOperators.front()->getChildren()[0]->as<WatermarkAssignerLogicalOperator>()->copy();
        ...
        if (windowType->instanceOf<Windowing::TimeBasedWindowType>())
        {
            ...
            if (watermarkOperator->getWatermarkStrategyDescriptor()->instanceOf<Windowing::EventTimeWatermarkStrategyDescriptor>())
            {
    ...
    if (!containmentOperators.empty()
        && !checkDownstreamOperatorChainForSingleParent(
            containedOperator, containedWindowOperators.front()->getChildren()[0]->as<LogicalOperator>()))
    {
        return {};
    }
...
LogicalOperatorPtr
ContainedOperatorsUtil::createContainedFilterOperators(const LogicalOperatorPtr& container, const LogicalOperatorPtr& containee)
{
    ...
    try
    {
        ...
        else if (upstreamFilterOperators.size() == 1)
        {
            ...
            if (!checkDownstreamOperatorChainForSingleParentAndMapOperator(containee, upstreamFilterOperators.front(), mapAttributeNames))
            {
                return nullptr;
            }
            auto predicate = upstreamFilterOperatorsCopy.front()->getPredicate();
            if (!isMapTransformationAppliedToPredicate(
                    upstreamFilterOperatorsCopy.front(), mapAttributeNames, container->getOutputSchema()))
            {
                return nullptr;
            }
        ...
        else
        {
            ...
            if (checkDownstreamOperatorChainForSingleParentAndMapOperator(containee, upstreamFilterOperators.back(), mapAttributeNames))
            {
                NES_TRACE("Filter predicate: {}", upstreamFilterOperators.back()->toString());
                auto predicate = upstreamFilterOperatorsCopy.front()->getPredicate();
                if (!isMapTransformationAppliedToPredicate(
                ...
                {
                    if (!isMapTransformationAppliedToPredicate(
                            upstreamFilterOperatorsCopy.at(i), mapAttributeNames, container->getOutputSchema()))
                    {
                        return nullptr;
                    }
...
bool ContainedOperatorsUtil::isMapTransformationAppliedToPredicate(
    LogicalFilterOperatorPtr const& filterOperator, const std::vector<std::string>& fieldNames, const SchemaPtr& containerOutputSchema)
{
    NES_DEBUG(
        "Create an iterator for traversing the filter {} predicates {}, and check output schema {}",
    ...
    for (auto itr = depthFirstNodeIterator.begin(); itr != DepthFirstNodeIterator::end(); ++itr)
    {
        ...
        if ((*itr)->instanceOf<FieldAccessExpressionNode>())
        {
            ...
            NES_TRACE(
                "Is field {} still in container output schema {}? {}",
...
bool ContainedOperatorsUtil::checkDownstreamOperatorChainForSingleParent(
    const LogicalOperatorPtr& containedOperator, const LogicalOperatorPtr& extractedContainedOperator)
{
        ...
        while (!parent->equal(containedOperator))
        {
            ...
            if (foundExtracted)
            {
                if (parent->instanceOf<LogicalUnionOperator>()
                    || (extractedContainedOperator->instanceOf<LogicalProjectionOperator>() && parent->instanceOf<LogicalWindowOperator>()))
                {
                    return false;
                }
            }
