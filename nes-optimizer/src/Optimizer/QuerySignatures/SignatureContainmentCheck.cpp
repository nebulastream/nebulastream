...
SignatureContainmentCheckPtr SignatureContainmentCheck::create(const z3::ContextPtr& context, bool allowExhaustiveContainmentCheck)
{
...
ContainmentRelationshipAndOperatorChainPtr SignatureContainmentCheck::checkContainmentForBottomUpMerging(
    const LogicalOperatorPtr& leftOperator, const LogicalOperatorPtr& rightOperator)
{
    ...
    catch (...)
    {
        ...
        {
            NES_ERROR(
                "SignatureContainmentUtil: Exception occurred while performing containment check among "
...
ContainmentRelationshipAndOperatorChainPtr SignatureContainmentCheck::checkContainmentRelationshipForTopDownMerging(
    const LogicalOperatorPtr& leftOperator, const LogicalOperatorPtr& rightOperator)
{
    NES_TRACE("Checking for containment.");
    ContainmentRelationship containmentRelationship = ContainmentRelationship::NO_CONTAINMENT;
    if (leftOperator->instanceOf<WatermarkAssignerLogicalOperator>() || rightOperator->instanceOf<WatermarkAssignerLogicalOperator>())
    {
...
SignatureContainmentCheck::checkProjectionContainment(const QuerySignaturePtr& leftSignature, const QuerySignaturePtr& rightSignature)
{
        ...
        && checkForEqualTransformations(leftSignature, rightSignature))
    {
        ...
        allowExhaustiveContainmentCheck && checkContainmentConditionsUnsatisfied(leftQueryProjectionFOL, rightQueryProjectionFOL)
        && checkForEqualTransformations(leftSignature, rightSignature)
...
ContainmentRelationship
SignatureContainmentCheck::checkWindowContainment(const QuerySignaturePtr& leftSignature, const QuerySignaturePtr& rightSignature)
{
    ...
    for (size_t i = 0; i < rightSignature->getWindowsExpressions().size(); ++i)
    {
        ...
        if (leftWindow.at("number-of-aggregates")->get_numeral_int() == rightWindow.at("number-of-aggregates")->get_numeral_int())
        {
            ...
            if (checkContainmentConditionsUnsatisfied(rightQueryWindowConditions, leftQueryWindowConditions))
            {
                ...
                allowExhaustiveContainmentCheck && i + 1 == rightSignature->getWindowsExpressions().size()
            ...
            i + 1 == rightSignature->getWindowsExpressions().size()
            && leftWindow.at("number-of-aggregates")->get_numeral_int() < rightWindow.at("number-of-aggregates")->get_numeral_int())
        {
            ...
            if (allowExhaustiveContainmentCheck && checkWindowContainmentPossible(rightWindow, leftWindow, leftSignature, rightSignature)
                && checkContainmentConditionsUnsatisfied(leftQueryWindowConditions, rightQueryWindowConditions))
            {
                NES_TRACE("Left window contained.");
                containmentRelationship = ContainmentRelationship::LEFT_SIG_CONTAINED;
...
ContainmentRelationship
SignatureContainmentCheck::checkFilterContainment(const QuerySignaturePtr& leftSignature, const QuerySignaturePtr& rightSignature)
{
    ...
    if (checkContainmentConditionsUnsatisfied(leftQueryFilterConditions, rightQueryFilterConditions))
    {
        ...
        allowExhaustiveContainmentCheck && checkContainmentConditionsUnsatisfied(rightQueryFilterConditions, leftQueryFilterConditions))
    {
...
bool SignatureContainmentCheck::checkForEqualTransformations(
    const QuerySignaturePtr& leftSignature, const QuerySignaturePtr& rightSignature)
{
    ...
    for (auto schemaFieldToExprMap : schemaFieldToExprMaps)
    {
             ...
             otherSchemaMapItr++)
        {
            ...
            for (uint64_t index = 0; index < containedAttributes.size(); index++)
            {
                ...
                NES_TRACE(
                    "Equivalence check for transformations: {} on attribute {}", equivalenceCheck.to_string(), containedAttributes[index]);
...
bool SignatureContainmentCheck::checkContainmentConditionsUnsatisfied(z3::expr_vector& negatedCondition, z3::expr_vector& condition)
{
    solver->push();
...
bool SignatureContainmentCheck::checkAttributeOrder(const QuerySignaturePtr& leftSignature, const QuerySignaturePtr& rightSignature) const
{
...
void SignatureContainmentCheck::combineWindowAndProjectionFOL(
    const QuerySignaturePtr& leftSignature, const QuerySignaturePtr& rightSignature, z3::expr_vector& leftFOL, z3::expr_vector& rightFOL)
{
    ...
    const QuerySignaturePtr& rightSignature)
{
    ...
    NES_TRACE(
        "Current window-id: {}, Current window-id != JoinWindow: {}",
    ...
    bool windowContainmentPossible = containerWindow.at("window-id")->to_string() != "\"JoinWindow\""
        ...
        && std::find(
               containerWindow.at("aggregate-types")->to_string().begin(),
    ...
    if (windowContainmentPossible)
    {
        ...
        windowContainmentPossible
            = containedWindow.at("window-time-size")->get_numeral_int() % containerWindow.at("window-time-size")->get_numeral_int() == 0
            && containedWindow.at("window-time-slide")->get_numeral_int() % containerWindow.at("window-time-slide")->get_numeral_int() == 0;
        ...
        if (windowContainmentPossible)
        {
            ...
            windowContainmentPossible
                = containedWindow.at("window-time-slide")->get_numeral_int() == containerWindow.at("window-time-slide")->get_numeral_int()
            ...
            if (windowContainmentPossible)
            {
                ...
                windowContainmentPossible = (checkFilterContainment(leftSignature, rightSignature) == ContainmentRelationship::EQUALITY);
...
bool SignatureContainmentCheck::checkFilterContainmentPossible(const QuerySignaturePtr& container, const QuerySignaturePtr& containee)
{
    if (container->getSchemaFieldToExprMaps().size() > 1)
    {
        ...
        {
            NES_TRACE("Container source name: {}; Container conditions: {}", containerSourceName, containerConditions->to_string());
