...
QueryRewritePhase::create(const Configurations::CoordinatorConfigurationPtr
                              &coordinatorConfiguration) {

  auto optimizerConfigurations = coordinatorConfiguration->optimizer;

  // If query merger rule is using string based signature or graph isomorphism
  // to identify the sharing opportunities then apply special rewrite rules for
  // improving the match identification
  bool applyRulesImprovingSharingIdentification =
      optimizerConfigurations.queryMergerRule ==
          QueryMergerRule::SyntaxBasedCompleteQueryMergerRule ||
      optimizerConfigurations.queryMergerRule ==
  ...
  return std::make_shared<QueryRewritePhase>(QueryRewritePhase(
      accelerateJavaUDFs, applyRulesImprovingSharingIdentification));
}
...
QueryRewritePhase::QueryRewritePhase(
    ...
    : isElegantAccelerationEnabled(elegantAccelerationEnabled),
      applyRulesImprovingSharingIdentification(
          applyRulesImprovingSharingIdentification) {
...
QueryPlanPtr QueryRewritePhase::execute(const QueryPlanPtr &queryPlan) {
  ...
  // Apply rules necessary for enabling query execution when stream alias or
  // union operators are involved
  duplicateQueryPlan =
      renameSourceToProjectOperatorRule->apply(duplicateQueryPlan);
  duplicateQueryPlan =
      projectBeforeUnionOperatorRule->apply(duplicateQueryPlan);
