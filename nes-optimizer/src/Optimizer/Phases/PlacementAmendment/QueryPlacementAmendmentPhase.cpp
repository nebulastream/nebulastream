...
QueryPlacementAmendmentPhase::QueryPlacementAmendmentPhase(
  ...
  placementAmendmentMode =
      this->coordinatorConfiguration->optimizer.placementAmendmentMode;
}

QueryPlacementAmendmentPhasePtr QueryPlacementAmendmentPhase::create(
    GlobalExecutionPlanPtr globalExecutionPlan, TopologyPtr topology,
    TypeInferencePhasePtr typeInferencePhase,
    Configurations::CoordinatorConfigurationPtr coordinatorConfiguration) {
  return std::make_shared<QueryPlacementAmendmentPhase>(
      QueryPlacementAmendmentPhase(
          std::move(globalExecutionPlan), std::move(topology),
          std::move(typeInferencePhase), std::move(coordinatorConfiguration)));
}

std::set<DeploymentContextPtr> QueryPlacementAmendmentPhase::execute(
    const SharedQueryPlanPtr &sharedQueryPlan) {
  NES_INFO("QueryPlacementAmendmentPhase: Perform query placement phase for "
           "shared query plan {}",
           sharedQueryPlan->getId());
  // TODO: At the time of placement we have to make sure that there are no
  // changes done on nesTopologyPlan (how to handle the case of dynamic
  // topology?)
  //  one solution could be: 1.) Take the snapshot of the topology and perform
  //  the placement 2.) If the topology changed meanwhile, repeat step 1.
  bool enableIncrementalPlacement =
      coordinatorConfiguration->optimizer.enableIncrementalPlacement;

  auto sharedQueryId = sharedQueryPlan->getId();
  auto queryPlan = sharedQueryPlan->getQueryPlan();
  NES_DEBUG("QueryPlacementAmendmentPhase: Perform query placement for query "
            "plan\n{}",
            queryPlan->toString());
  ...
  if (enableIncrementalPlacement) {
    ...
    for (const auto &changeLogEntry :
         sharedQueryPlan->getChangeLogEntries(nowInMicroSec)) {
      try {
        ...
        auto pinnedDownStreamOperators =
            changeLogEntry.second->downstreamOperators;
        ...
        if (containsOperatorsForRemoval(pinnedDownStreamOperators)) {
          auto placementRemovalStrategy = PlacementRemovalStrategy::create(
          ...
          auto placementRemovalDeploymentContexts =
              placementRemovalStrategy->updateGlobalExecutionPlan(
                  sharedQueryId, pinnedUpstreamOperators,
                  pinnedDownStreamOperators, nextDecomposedQueryPlanVersion);

          // Collect all deployment contexts returned by placement removal
          // strategy
          for (const auto &[decomposedQueryPlanId, deploymentContext] :
               placementRemovalDeploymentContexts) {
            deploymentContexts[decomposedQueryPlanId] = deploymentContext;
          }

        } else {
          NES_WARNING("Skipping placement removal phase as no pinned "
                      "downstream operator in the state TO_BE_REMOVED or "
        ...
        if (containsOperatorsForPlacement(pinnedDownStreamOperators)) {
          ...
          auto placementAdditionResults =
              placementAdditionStrategy->updateGlobalExecutionPlan(
                  sharedQueryId, pinnedUpstreamOperators,
                  pinnedDownStreamOperators, nextDecomposedQueryPlanVersion);
          ...
          // strategy
          for (const auto &[decomposedQueryPlanId, deploymentContext] :
               placementAdditionResults.deploymentContexts) {
            deploymentContexts[decomposedQueryPlanId] = deploymentContext;
          }

          if (!placementAdditionResults.completedSuccessfully) {
            throw std::runtime_error(
                "Placement addition phase unsuccessfully completed");
          }
        } else {
          NES_WARNING("Skipping placement addition phase as no pinned "
                      "downstream operator in the state PLACED or "
                      "TO_BE_PLACED state.");
        }
      } catch (std::exception &ex) {
        NES_ERROR("Failed to process change log. Marking shared query plan as "
                  "partially processed and recording the "
    ...
    // Update the change log's till processed timestamp and clear all entries
    ...
    try {
      ...
      if (containsOperatorsForRemoval(pinnedDownStreamOperators)) {
        ...
        NES_WARNING("Skipping placement removal phase as no pinned downstream "
                    "operator in the state TO_BE_REMOVED or "
                    "TO_BE_REPLACED state.");
      }
      ...
      if (containsOperatorsForPlacement(pinnedDownStreamOperators)) {
        ...
        NES_WARNING("Skipping placement addition phase as no pinned downstream "
                    "operator in the state PLACED or "
                    "TO_BE_PLACED state.");
      }
    } catch (std::exception &ex) {
...
bool QueryPlacementAmendmentPhase::containsOnlyPinnedOperators(
    const std::set<LogicalOperatorPtr> &pinnedOperators) {

  // Find if one of the operator does not have PINNED_WORKER_ID property
  return !std::any_of(pinnedOperators.begin(), pinnedOperators.end(),
                      [](const LogicalOperatorPtr &pinnedOperator) {
                        return !pinnedOperator->hasProperty(PINNED_WORKER_ID);
                      });
}

bool QueryPlacementAmendmentPhase::containsOperatorsForPlacement(
    const std::set<LogicalOperatorPtr> &operatorsToCheck) {
  return std::any_of(operatorsToCheck.begin(), operatorsToCheck.end(),
                     [](const LogicalOperatorPtr &operatorToCheck) {
                       return (operatorToCheck->getOperatorState() ==
                                   OperatorState::TO_BE_PLACED ||
                               operatorToCheck->getOperatorState() ==
                                   OperatorState::PLACED);
                     });
}

bool QueryPlacementAmendmentPhase::containsOperatorsForRemoval(
    const std::set<LogicalOperatorPtr> &operatorsToCheck) {
  return std::any_of(operatorsToCheck.begin(), operatorsToCheck.end(),
                     [](const LogicalOperatorPtr &operatorToCheck) {
                       return (operatorToCheck->getOperatorState() ==
...
void QueryPlacementAmendmentPhase::pinAllSinkOperators(
    const std::set<LogicalOperatorPtr> &operators) {
  auto rootNodeId = topology->getRootWorkerNodeIds()[0];
  for (const auto &operatorToCheck : operators) {
    if (!operatorToCheck->hasProperty(PINNED_WORKER_ID) &&
        operatorToCheck->instanceOf<SinkLogicalOperator>()) {
...
QueryPlacementAmendmentPhase::getStrategy(PlacementStrategy placementStrategy) {

  auto plannerURL = coordinatorConfiguration->elegant.plannerServiceURL;

  switch (placementStrategy) {
  case PlacementStrategy::ILP:
    return ILPStrategy::create(globalExecutionPlan, topology,
                               typeInferencePhase, placementAmendmentMode);
  case PlacementStrategy::BottomUp:
    return BottomUpStrategy::create(globalExecutionPlan, topology,
                                    typeInferencePhase, placementAmendmentMode);
  case PlacementStrategy::TopDown:
    return TopDownStrategy::create(globalExecutionPlan, topology,
                                   typeInferencePhase, placementAmendmentMode);
  case PlacementStrategy::ELEGANT_PERFORMANCE:
  case PlacementStrategy::ELEGANT_ENERGY:
  case PlacementStrategy::ELEGANT_BALANCED:
    return ElegantPlacementStrategy::create(
        plannerURL, placementStrategy, globalExecutionPlan, topology,
        typeInferencePhase, placementAmendmentMode);
    // #2486        case PlacementStrategy::IFCOP:
    //            return IFCOPStrategy::create(globalExecutionPlan, topology,
    //            typeInferencePhase);
  case PlacementStrategy::MlHeuristic:
    return MlHeuristicStrategy::create(globalExecutionPlan, topology,
                                       typeInferencePhase,
                                       placementAmendmentMode);
  default:
    throw Exceptions::RuntimeException(
        "Unknown placement strategy type " +
