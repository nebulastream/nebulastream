...
std::set<DeploymentContextPtr> QueryPlacementAmendmentPhase::execute(
  ...
  NES_INFO(
      "QueryPlacementAmendmentPhase: Perform query placement phase for shared "
      "query plan {}",
  ...
  NES_DEBUG(
      "QueryPlacementAmendmentPhase: Perform query placement for query "
      "plan\n{}",
      queryPlan->toString());
  ...
  if (enableIncrementalPlacement) {
    ...
    for (const auto& changeLogEntry :
      ...
      try {
        ...
        if (containsOperatorsForRemoval(pinnedDownStreamOperators)) {
          ...
          NES_WARNING(
              "Skipping placement removal phase as no pinned downstream "
        ...
        if (containsOperatorsForPlacement(pinnedDownStreamOperators)) {
          ...
          NES_WARNING(
              "Skipping placement addition phase as no pinned downstream "
              "operator in the state PLACED or "
              "TO_BE_PLACED state.");
        }
      } catch (std::exception& ex) {
        NES_ERROR(
            "Failed to process change log. Marking shared query plan as "
    ...
    try {
      ...
      if (containsOperatorsForRemoval(pinnedDownStreamOperators)) {
        ...
        NES_WARNING(
            "Skipping placement removal phase as no pinned downstream operator "
            "in the state TO_BE_REMOVED or "
            "TO_BE_REPLACED state.");
      }
...
bool QueryPlacementAmendmentPhase::containsOperatorsForPlacement(
  ...
  return std::any_of(
      operatorsToCheck.begin(), operatorsToCheck.end(),
...
BasePlacementStrategyPtr QueryPlacementAmendmentPhase::getStrategy(
