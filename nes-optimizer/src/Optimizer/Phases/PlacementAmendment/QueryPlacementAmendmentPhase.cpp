...
QueryPlacementAmendmentPhasePtr QueryPlacementAmendmentPhase::create(
    ...
    Configurations::CoordinatorConfigurationPtr coordinatorConfiguration)
{
    return std::make_shared<QueryPlacementAmendmentPhase>(QueryPlacementAmendmentPhase(
        std::move(globalExecutionPlan), std::move(topology), std::move(typeInferencePhase), std::move(coordinatorConfiguration)));
}
...
std::set<DeploymentContextPtr> QueryPlacementAmendmentPhase::execute(const SharedQueryPlanPtr& sharedQueryPlan)
{
    ...
    if (enableIncrementalPlacement)
    {
        ...
        for (const auto& changeLogEntry : sharedQueryPlan->getChangeLogEntries(nowInMicroSec))
        {
            try
            {
                ...
                if (!containsOnlyPinnedOperators(pinnedDownStreamOperators) || !containsOnlyPinnedOperators(pinnedUpstreamOperators))
                {
                    throw Exceptions::QueryPlacementAdditionException(
                        sharedQueryId, "QueryPlacementAmendmentPhase: Found operators without pinning.");
                }
                ...
                if (containsOperatorsForRemoval(pinnedDownStreamOperators))
                {
                    ...
                    auto placementRemovalDeploymentContexts = placementRemovalStrategy->updateGlobalExecutionPlan(
                        sharedQueryId, pinnedUpstreamOperators, pinnedDownStreamOperators, nextDecomposedQueryPlanVersion);
                ...
                if (containsOperatorsForPlacement(pinnedDownStreamOperators))
                {
                    ...
                    auto placementAdditionResults = placementAdditionStrategy->updateGlobalExecutionPlan(
                        sharedQueryId, pinnedUpstreamOperators, pinnedDownStreamOperators, nextDecomposedQueryPlanVersion);
            ...
            {
                NES_ERROR(
                    "Failed to process change log. Marking shared query plan as partially processed and recording the "
...
bool QueryPlacementAmendmentPhase::containsOnlyPinnedOperators(const std::set<LogicalOperatorPtr>& pinnedOperators)
{
    ...
    return !std::any_of(
        pinnedOperators.begin(),
        pinnedOperators.end(),
        [](const LogicalOperatorPtr& pinnedOperator) { return !pinnedOperator->hasProperty(PINNED_WORKER_ID); });
}

bool QueryPlacementAmendmentPhase::containsOperatorsForPlacement(const std::set<LogicalOperatorPtr>& operatorsToCheck)
{
    return std::any_of(
        operatorsToCheck.begin(),
        operatorsToCheck.end(),
        [](const LogicalOperatorPtr& operatorToCheck)
        {
            return (
                operatorToCheck->getOperatorState() == OperatorState::TO_BE_PLACED
...
bool QueryPlacementAmendmentPhase::containsOperatorsForRemoval(const std::set<LogicalOperatorPtr>& operatorsToCheck)
{
    return std::any_of(
        ...
        {
            return (
                operatorToCheck->getOperatorState() == OperatorState::TO_BE_REPLACED
    ...
    switch (placementStrategy)
    {
        ...
        case PlacementStrategy::ELEGANT_BALANCED:
            return ElegantPlacementStrategy::create(
                plannerURL, placementStrategy, globalExecutionPlan, topology, typeInferencePhase, placementAmendmentMode);
        ...
        default:
            throw Exceptions::RuntimeException("Unknown placement strategy type " + std::string(magic_enum::enum_name(placementStrategy)));
    }
}
