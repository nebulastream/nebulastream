...
DecomposedQueryPlanPtr TypeInferencePhase::execute(
...
void TypeInferencePhase::performTypeInference(
  ...
  for (const auto& source : sourceOperators) {
    ...
    if (sourceDescriptor->instanceOf<LogicalSourceDescriptor>() &&
      ...
      NES_DEBUG(
          "TypeInferencePhase: update source descriptor for source {} with "
          "schema: {}",
  ...
  for (auto& sink : sinkOperators) {
    if (!sink->inferSchema()) {
      NES_ERROR(
          "TypeInferencePhase: Exception occurred during type inference "
          "phase.");
