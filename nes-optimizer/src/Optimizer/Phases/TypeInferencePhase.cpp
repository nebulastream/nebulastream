...
TypeInferencePhase::create(Catalogs::Source::SourceCatalogPtr sourceCatalog,
                           Catalogs::UDF::UDFCatalogPtr udfCatalog) {
  return std::make_shared<TypeInferencePhase>(
      TypeInferencePhase(std::move(sourceCatalog), std::move(udfCatalog)));
}
...
QueryPlanPtr TypeInferencePhase::execute(QueryPlanPtr queryPlan) {
  ...
  if (sourceOperators.empty() || sinkOperators.empty()) {
    throw TypeInferenceException(queryPlan->getQueryId(),
...
TypeInferencePhase::execute(DecomposedQueryPlanPtr decomposedQueryPlan) {
  ...
  if (sourceOperators.empty() || sinkOperators.empty()) {
    throw TypeInferenceException(
        UNSURE_CONVERSION_TODO_4761(
            decomposedQueryPlan->getDecomposedQueryPlanId(), QueryId),
        "Found no source or sink operators");
  }
  ...
  performTypeInference(
      UNSURE_CONVERSION_TODO_4761(
          decomposedQueryPlan->getDecomposedQueryPlanId(), QueryId),
      sourceOperators, sinkOperators);
  NES_DEBUG("TypeInferencePhase: we inferred all schemas");
  return decomposedQueryPlan;
}

void TypeInferencePhase::performTypeInference(
    QueryId planId, std::vector<SourceLogicalOperatorPtr> sourceOperators,
    std::vector<SinkLogicalOperatorPtr> sinkOperators) {

  // first we have to check if all source operators have a correct source
  // descriptors
  for (const auto &source : sourceOperators) {
    auto sourceDescriptor = source->getSourceDescriptor();

    // if the source descriptor has no schema set and is only a logical source
    // we replace it with the correct source descriptor form the catalog.
    if (sourceDescriptor->instanceOf<LogicalSourceDescriptor>() &&
        sourceDescriptor->getSchema()->empty()) {
      ...
      if (!sourceCatalog->containsLogicalSource(logicalSourceName)) {
        ...
        throw Exceptions::LogicalSourceNotFoundException(
            "Logical source not registered. Source Name: " + logicalSourceName);
      }
      auto originalSchema =
      ...
      std::string qualifierName =
          logicalSourceName + Schema::ATTRIBUTE_NAME_SEPARATOR;
      ...
      NES_DEBUG("TypeInferencePhase: update source descriptor for source {} "
                "with schema: {}",
  ...
  // to this end we call at each sink the infer method to propagate the schemata
  // across the whole query.
  for (auto &sink : sinkOperators) {
    if (!sink->inferSchema()) {
      NES_ERROR("TypeInferencePhase: Exception occurred during type inference "
                "phase.");
