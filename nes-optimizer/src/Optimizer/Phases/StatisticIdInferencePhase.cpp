...
StatisticIdInferencePhase::execute(DecomposedQueryPlanPtr decomposedQueryPlan) {
...
void StatisticIdInferencePhase::performInference(
  ...
  for (auto &op : allOperators) {
    if (op->instanceOf<SourceLogicalOperatorPtr>()) {
      NES_DEBUG("Not setting the statisticId of a logical source, as we first "
                "have to expand the logical source"
