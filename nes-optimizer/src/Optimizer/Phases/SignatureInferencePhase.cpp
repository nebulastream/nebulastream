...
SignatureInferencePhasePtr SignatureInferencePhase::create(
    z3::ContextPtr context, Optimizer::QueryMergerRule queryMergerRule) {
...
void SignatureInferencePhase::execute(const QueryPlanPtr& queryPlan) {
  if (queryMergerRule ==
    ...
    NES_INFO(
        "SignatureInferencePhase: computing String based signature for the "
        "query {}",
    ...
    NES_INFO(
        "SignatureInferencePhase: computing Z3 based signature for the query "
        "{}",
        queryPlan->getQueryId());
    auto sinkOperators = queryPlan->getRootOperators();
    for (auto& sinkOperator : sinkOperators) {
