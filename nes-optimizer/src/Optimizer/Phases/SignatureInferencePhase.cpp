...
SignatureInferencePhase::SignatureInferencePhase(
...
SignatureInferencePhase::create(z3::ContextPtr context,
                                Optimizer::QueryMergerRule queryMergerRule) {
  return std::make_shared<SignatureInferencePhase>(
      SignatureInferencePhase(std::move(context), queryMergerRule));
}
...
void SignatureInferencePhase::execute(const QueryPlanPtr &queryPlan) {
  if (queryMergerRule ==
      ...
      queryMergerRule ==
          QueryMergerRule::ImprovedHashSignatureBasedPartialQueryMergerRule) {
    NES_INFO("SignatureInferencePhase: computing String based signature for "
             "the query {}",
    ...
    for (auto &sinkOperator : sinkOperators) {
      ...
      queryMergerRule ==
          QueryMergerRule::Z3SignatureBasedPartialQueryMergerBottomUpRule ||
      queryMergerRule ==
          QueryMergerRule::Z3SignatureBasedBottomUpQueryContainmentRule ||
      queryMergerRule ==
          QueryMergerRule::Z3SignatureBasedTopDownQueryContainmentMergerRule) {
    NES_INFO("SignatureInferencePhase: computing Z3 based signature for the "
             "query {}",
             queryPlan->getQueryId());
    auto sinkOperators = queryPlan->getRootOperators();
    for (auto &sinkOperator : sinkOperators) {
