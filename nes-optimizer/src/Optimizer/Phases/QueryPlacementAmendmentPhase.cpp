...
DeploymentUnit QueryPlacementAmendmentPhase::execute(const SharedQueryPlanPtr& sharedQueryPlan) {
    ...
    auto checkpointStorageType = sharedQueryPlan->getCheckpointStorageType();
...
void QueryPlacementAmendmentPhase::handleMigrationPlacement(
    ...
    for (const auto& [migratingOperatorId, migratingOperatorProperties] : migratingOperatorToProperties) {
        ...
        auto fakeMigrationSourceOperator =
        ...
        fakeMigrationSourceOperator->addProperty(Optimizer::PINNED_WORKER_ID, migratingOperatorProperties->getOldWorkerId());
        ...
        fakeMigrationSourceOperator->addParent(migrateSinkOperator);
        ...
        if (fakeMigrationSourceOperator->hasProperty(PLACED_DECOMPOSED_PLAN_ID)) {
            migrationPlanId = std::any_cast<DecomposedQueryId>(fakeMigrationSourceOperator->getProperty(PLACED_DECOMPOSED_PLAN_ID));
        } else {
