...
Z3SignatureBasedPartialQueryMergerRulePtr
Z3SignatureBasedPartialQueryMergerRule::create(z3::ContextPtr context) {
  return std::make_shared<Z3SignatureBasedPartialQueryMergerRule>(
      Z3SignatureBasedPartialQueryMergerRule(std::move(context)));
}

bool Z3SignatureBasedPartialQueryMergerRule::apply(
    GlobalQueryPlanPtr globalQueryPlan) {

  NES_INFO("Z3SignatureBasedPartialQueryMergerRule: Applying Signature Based "
           "Equal Query Merger Rule to the Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING("Z3SignatureBasedPartialQueryMergerRule: Found only a single "
                "query metadata in the global query plan."
                " Skipping the Signature Based Equal Query Merger Rule.");
    return true;
  }

  NES_DEBUG("Z3SignatureBasedPartialQueryMergerRule: Iterating over all Shared "
            "Query MetaData in the Global Query Plan");
  ...
  for (const auto &targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto &hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      // Iterate over the target query plan and compare the operator signatures
      // with the host query plan When a match is found then store the matching
      // operators in the matchedTargetToHostOperatorMap
      for (const auto &targetRootOperator :
        ...
        while (!targetOperators.empty()) {
          ...
          for (const auto &hostRootOperator :
            ...
            // host operators are available to
            //  perform matching
            while (!hostOperators.empty()) {
              ...
              if (signatureEqualityUtil->checkEquality(
              ...
              // Check for the children operators if a host operator with
          ...
          // Check for the children operators if a host operator with matching
      ...
      if (!matchedTargetToHostOperatorMap.empty()) {
        ...
        for (auto [targetOperator, hostOperator] :
             matchedTargetToHostOperatorMap) {
          matchedOperatorPairs.emplace_back(
              MatchedOperatorPair::create(hostOperator->as<LogicalOperator>(),
