...
bool Z3SignatureBasedPartialQueryMergerRule::apply(
  ...
  NES_INFO(
      "Z3SignatureBasedPartialQueryMergerRule: Applying Signature Based Equal "
      "Query Merger Rule to the Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "Z3SignatureBasedPartialQueryMergerRule: Found only a single query "
  ...
  NES_DEBUG(
      "Z3SignatureBasedPartialQueryMergerRule: Iterating over all Shared Query "
      "MetaData in the Global Query Plan");
