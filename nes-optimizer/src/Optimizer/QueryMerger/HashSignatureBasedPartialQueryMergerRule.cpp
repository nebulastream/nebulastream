...
bool HashSignatureBasedPartialQueryMergerRule::apply(
  ...
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "HashSignatureBasedPartialQueryMergerRule: Found no new query metadata "
  ...
  NES_DEBUG(
      "HashSignatureBasedPartialQueryMergerRule: Iterating over all Shared "
  ...
  for (auto& targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (!matchedTargetToHostOperatorMap.empty()) {
        NES_TRACE(
            "HashSignatureBasedPartialQueryMergerRule: Merge target Shared "
    ...
    if (!merged) {
      NES_DEBUG(
          "HashSignatureBasedPartialQueryMergerRule: computing a new Shared "
          "Query Plan");
...
HashSignatureBasedPartialQueryMergerRule::areQueryPlansEqual(
  ...
  NES_DEBUG(
      "HashSignatureBasedPartialQueryMergerRule: check if the target and "
  ...
  if (targetSourceOperators.size() != hostSourceOperators.size()) {
    NES_WARNING(
        "HashSignatureBasedPartialQueryMergerRule: Not matched as number of "
        "sink in target and host query plans are "
        "different.");
    return {};
  }
...
HashSignatureBasedPartialQueryMergerRule::areOperatorEqual(
  ...
  if (targetOperator->instanceOf<SinkLogicalOperator>() &&
    ...
    NES_TRACE(
        "HashSignatureBasedPartialQueryMergerRule: Both target and host "
  ...
  if (targetHashBased == hostOperator->getHashBasedSignature()) {
    NES_TRACE(
        "HashSignatureBasedPartialQueryMergerRule: Check if parents of target "
  ...
  NES_WARNING(
      "HashSignatureBasedPartialQueryMergerRule: Target and host operators are "
      "not matched.");
  return {};
}
}  // namespace NES::Optimizer
