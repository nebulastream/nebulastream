...
bool Z3SignatureBasedTreeBasedQueryContainmentMergerRule::apply(
  ...
  NES_INFO("Applying Z3SignatureBasedPartialQueryContainmentMergerRule to the "
           "Global Query Plan");
  ...
  for (const auto &targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto &hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (!matchedTargetToHostOperatorMap.empty()) {
        NES_TRACE("Z3SignatureBasedPartialQueryMergerRule: Merge target Shared "
    ...
    if (!matched) {
      NES_DEBUG("Z3SignatureBasedPartialQueryMergerRule: computing a new "
                "Shared Query Plan");
...
void Z3SignatureBasedTreeBasedQueryContainmentMergerRule::
  ...
  NES_TRACE("Children upstreamContainedOperator size: {}; Children "
