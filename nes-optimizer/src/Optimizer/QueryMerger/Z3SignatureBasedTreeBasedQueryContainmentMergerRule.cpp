...
Z3SignatureBasedTreeBasedQueryContainmentMergerRule::
    Z3SignatureBasedTreeBasedQueryContainmentMergerRule(
        z3::ContextPtr context, bool allowExhaustiveContainmentCheck) {
  SignatureContainmentUtil = SignatureContainmentCheck::create(
      std::move(context), allowExhaustiveContainmentCheck);
}
...
bool Z3SignatureBasedTreeBasedQueryContainmentMergerRule::apply(
    GlobalQueryPlanPtr globalQueryPlan) {
  NES_INFO(
      "Applying Z3SignatureBasedPartialQueryContainmentMergerRule to the "
      "Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "Found only a single query metadata in the global query plan."
        " Skipping the Signature Based Equal Query Merger Rule.");
    return true;
  }

  NES_DEBUG(
      "Iterating over all Shared Query MetaData in the Global Query Plan");
  ...
  for (const auto& targetQueryPlan : queryPlansToAdd) {
    ...
    auto hostSharedQueryPlans =
        globalQueryPlan
            ->getSharedQueryPlansConsumingSourcesAndPlacementStrategy(
                targetQueryPlan->getSourceConsumed(),
    ...
    for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      std::map<OperatorPtr, std::tuple<OperatorPtr, ContainmentRelationship,
      ...
      // Iterate over the target query plan from sink to source and compare the
      // operator signatures with the host query plan When a match is found then
      // store the matching operators in the matchedTargetToHostOperatorMap
      for (const auto& targetRootOperator :
           targetQueryPlan->getRootOperators()) {
        // Iterate the target query plan in DFS order.
        auto targetChildren = targetRootOperator->getChildren();
        std::deque<NodePtr> targetOperators = {targetChildren.begin(),
                                               targetChildren.end()};
        // Iterate till target operators are remaining to be matched
        while (!targetOperators.empty()) {
          // Extract the front of the queue and check if there is a matching
          // operator in the
          //  host query plan
          ...
          if (matchedTargetToHostOperatorMap.find(targetOperator) !=
              matchedTargetToHostOperatorMap.end()) {
            continue;
          }
          ...
          // Iterate the host query plan in BFS order and check if an operator
          // with matching signature with the target operator
          //  exists.
          for (const auto& hostRootOperator :
               hostQueryPlan->getRootOperators()) {
            ...
            for (const auto& hostChildren : children) {
              ...
              if (std::find(visitedHostOperators.begin(),
                            visitedHostOperators.end(),
                            hostChildren) == visitedHostOperators.end()) {
                hostOperators.push_back(hostChildren);
              }
            }

            // Iterate till a matching host operator is not found or till the
            // host operators are available to perform matching
            while (!hostOperators.empty()) {
              // Take out the front of the queue and add the host operator to
              // the visited list
              ...
              if (std::find(matchedHostOperators.begin(),
                            matchedHostOperators.end(),
                            hostOperator) != matchedHostOperators.end()) {
                continue;
              }

              // Match the target and host operator signatures to see if a match
              // is present
              relationshipAndOperators =
                  SignatureContainmentUtil
                      ->checkContainmentRelationshipForTopDownMerging(
                          hostOperator, targetOperator);
              if (relationshipAndOperators->containmentRelationship !=
                  ContainmentRelationship::NO_CONTAINMENT) {
                // Add the matched host operator to the map
                matchedTargetToHostOperatorMap[targetOperator] = std::tuple(
                    hostOperator,
              ...
              // Check for the children operators of a host operator with
              // matching is found on the host
              auto hostOperatorChildren = hostOperator->getChildren();
              for (const auto& hostChild : hostOperatorChildren) {
                // Only add the host operators in the back of the queue which
                // were not traversed earlier
                if (std::find(visitedHostOperators.begin(),
                              visitedHostOperators.end(),
                              hostChild) == visitedHostOperators.end()) {
          ...
          // If a match is found then no need to look for a matching downstream
          // operator chain
          if (foundMatch) {
            continue;
          }

          // Check for the children operators if no host operator with matching
      ...
      if (!matchedTargetToHostOperatorMap.empty()) {
        NES_TRACE(
            "Z3SignatureBasedPartialQueryMergerRule: Merge target Shared "
            "metadata into address metadata");

        // As we merge partially equivalent queryIdAndCatalogEntryMapping, we
        // can potentially find matches across multiple operators. As upstream
        // matched operators are covered by downstream matched operators. We
        // need to retain only the downstream matched operator containing any
        // upstream matched operator. This will prevent in computation of
        // inconsistent shared query plans.
        if (matchedTargetToHostOperatorMap.size() > 1) {
          ...
          // Iterate over the target operators and remove the upstream operators
          // covered by downstream matched operators
          for (uint64_t i = 0; i < matchedTargetOperators.size(); i++) {
            for (uint64_t j = 0; j < matchedTargetOperators.size(); j++) {
              ...
              if (matchedTargetOperators[i]->containAsGrandChild(
                      matchedTargetOperators[j])) {
                matchedTargetToHostOperatorMap.erase(matchedTargetOperators[j]);
              } else if (matchedTargetOperators[i]->containAsGrandParent(
                             matchedTargetOperators[j])) {
        ...
        NES_TRACE("Size of matchedTargetToHostOperatorMap: {}",
                  std::to_string(matchedTargetToHostOperatorMap.size()));

        // Iterate over all matched pairs of operators and merge the query plan
        for (auto [targetOperator,
                   hostOperatorContainmentRelationshipContainedOperatorChain] :
             matchedTargetToHostOperatorMap) {
          // In case of equivalence detection, just add the matched operator
          // pair to the list with the correct containment relationship
          if (get<1>(
                  hostOperatorContainmentRelationshipContainedOperatorChain) ==
            ...
            matchedOperatorPairs.emplace_back(MatchedOperatorPair::create(
                hostOperator, targetOperator->as<LogicalOperator>(),
                ContainmentRelationship::EQUALITY));
            // In case the host contains the target, we first need to prepare
            // the query plan and then add the matched operators
          } else if (
              get<1>(
                  hostOperatorContainmentRelationshipContainedOperatorChain) ==
              ContainmentRelationship::RIGHT_SIG_CONTAINED) {
            auto containerOperator = get<0>(
                hostOperatorContainmentRelationshipContainedOperatorChain);
            auto containedOperatorChain = get<2>(
                hostOperatorContainmentRelationshipContainedOperatorChain);
            // prepare the new query plan with the correct containment
            // relationship, i.e., add the containedOperatorChain to the
            // containerOperator and set the targetOperator's parents as parents
            // of the containedOperatorChain
            addContainmentOperatorChain(hostSharedQueryPlan, containerOperator,
                                        targetOperator, containedOperatorChain);
            matchedOperatorPairs.emplace_back(MatchedOperatorPair::create(
                containerOperator->as<LogicalOperator>(),
                containedOperatorChain.back()->as<LogicalOperator>(),
                ContainmentRelationship::RIGHT_SIG_CONTAINED));
            // In case the target contains the host, we first need to prepare
            // the query plan and then add the matched operators if
            // allowExhaustiveContainmentCheck is not explicitly set to true,
            ...
            auto containedOperatorChain = get<2>(
                hostOperatorContainmentRelationshipContainedOperatorChain);
            auto containedOperator = get<0>(
                hostOperatorContainmentRelationshipContainedOperatorChain);
            // prepare the new query plan with the correct containment
            // relationship, i.e., add the containedOperatorChain to the
            // targetOperator and set the containedOperator's parents as parents
            // of the containedOperatorChain
            addContainmentOperatorChain(hostSharedQueryPlan, targetOperator,
                                        containedOperator,
        ...
        // exit the for loop as we found a matching address shared query
        // metadata
        matched = true;
        break;
      }
    ...
    if (!matched) {
      NES_DEBUG(
          "Z3SignatureBasedPartialQueryMergerRule: computing a new Shared "
          "Query Plan");
...
void Z3SignatureBasedTreeBasedQueryContainmentMergerRule::
    addContainmentOperatorChain(
  ...
  NES_TRACE(
      "ContainerQueryPlan: {}; ContainerOperator: {}; DownstreamOperator: {}; "
      "ContainedOperator: {}; "
      "UpstreamContainedOperator: {}; DownstreamOperator's children size: {}",
      containerQueryPlan->getQueryPlan()->toString(),
      containerOperator->toString(), downstreamOperator->toString(),
      containedOperator->toString(), upstreamContainedOperator->toString(),
  ...
  for (const auto& parent : parents) {
    // for each parent from the containedOperator, remove all children and add
  ...
  NES_TRACE(
      "Children upstreamContainedOperator size: {}; Children "
      "upstreamContainedOperator: {}",
      upstreamContainedOperator->getChildren().size(),
      upstreamContainedOperator->getChildren()[0]->toString());
  if (!addedNewParent) {
    NES_WARNING(
        "Z3SignatureBasedPartialQueryMergerRule: Failed to add new parent");
  }
}
}  // namespace NES::Optimizer
