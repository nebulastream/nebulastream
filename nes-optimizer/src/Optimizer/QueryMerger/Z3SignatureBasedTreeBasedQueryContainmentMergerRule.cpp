    ...
    for (const auto & targetQueryPlan : queryPlansToAdd)
    {
        ...
        for (auto & hostSharedQueryPlan : hostSharedQueryPlans)
        {
                    ...
                    for (const auto & hostRootOperator : hostQueryPlan->getRootOperators())
                    {
                        ...
                        while (!hostOperators.empty())
                        {
                            ...
                            relationshipAndOperators
                                = SignatureContainmentUtil->checkContainmentRelationshipForTopDownMerging(hostOperator, targetOperator);
            ...
            if (!matchedTargetToHostOperatorMap.empty())
            {
                ...
                for (auto [targetOperator, hostOperatorContainmentRelationshipContainedOperatorChain] : matchedTargetToHostOperatorMap)
                {
                    ...
                    if (get<1>(hostOperatorContainmentRelationshipContainedOperatorChain) == ContainmentRelationship::EQUALITY)
                    {
                        ...
                        matchedOperatorPairs.emplace_back(MatchedOperatorPair::create(
                            hostOperator, targetOperator->as<LogicalOperator>(), ContainmentRelationship::EQUALITY));
                        //In case the host contains the target, we first need to prepare the query plan and then add the matched operators
                    }
                    else if (
                        get<1>(hostOperatorContainmentRelationshipContainedOperatorChain) == ContainmentRelationship::RIGHT_SIG_CONTAINED)
                    {
                        ...
                        addContainmentOperatorChain(hostSharedQueryPlan, containerOperator, targetOperator, containedOperatorChain);
                        matchedOperatorPairs.emplace_back(MatchedOperatorPair::create(
                        ...
                        get<1>(hostOperatorContainmentRelationshipContainedOperatorChain) == ContainmentRelationship::LEFT_SIG_CONTAINED)
                    {
                        auto containedOperatorChain = get<2>(hostOperatorContainmentRelationshipContainedOperatorChain);
                        ...
                        addContainmentOperatorChain(hostSharedQueryPlan, targetOperator, containedOperator, containedOperatorChain);
                        matchedOperatorPairs.emplace_back(MatchedOperatorPair::create(
    ...
    const std::vector<LogicalOperatorPtr> containedOperatorChain) const
{
    ...
    NES_TRACE(
        "ContainerQueryPlan: {}; ContainerOperator: {}; DownstreamOperator: {}; ContainedOperator: {}; "
    ...
    NES_TRACE(
        "Children upstreamContainedOperator size: {}; Children upstreamContainedOperator: {}",
