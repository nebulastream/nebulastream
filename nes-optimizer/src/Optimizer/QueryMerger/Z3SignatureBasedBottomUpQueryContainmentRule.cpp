...
Z3SignatureBasedBottomUpQueryContainmentRule::Z3SignatureBasedBottomUpQueryContainmentRule(
            ...
            else
            {
                ...
                if (!matchedTargetToHostOperatorMap.empty())
                {
                    ...
                    for (auto [targetOp, hostOperatorAndRelationship] : matchedTargetToHostOperatorMap)
                    {
                        ...
                        {
                            NES_TRACE(
                                "Current host sqp {}; Output schema equality target {}; Output schema equality host {}; "
                                "Target parent size {}",
                            ...
                            std::get<1>(hostOperatorAndRelationship) == ContainmentRelationship::RIGHT_SIG_CONTAINED
                            && checkWindowContainmentPossible(hostOperator, targetOperator))
                        {
                            ...
                            std::get<1>(hostOperatorAndRelationship) == ContainmentRelationship::LEFT_SIG_CONTAINED
                            && checkWindowContainmentPossible(targetOperator, hostOperator))
                        {
                            ...
                            if (hostOperator->instanceOf<LogicalUnionOperator>() || hostOperator->instanceOf<LogicalJoinOperator>())
                            {
                                unionOrJoin = true;
                                break;
                            }
                    ...
                    {
                        NES_DEBUG(
                            "Adding root operator {} to host query plan {}", targetRootOperator->toString(), hostQueryPlan->toString());
...
bool Z3SignatureBasedBottomUpQueryContainmentRule::checkWindowContainmentPossible(
    const LogicalOperatorPtr& container, const LogicalOperatorPtr& containee) const
{
...
Z3SignatureBasedBottomUpQueryContainmentRule::areQueryPlansContained(const QueryPlanPtr& hostQueryPlan, const QueryPlanPtr& targetQueryPlan)
{
...
Z3SignatureBasedBottomUpQueryContainmentRule::areOperatorsContained(
    const LogicalOperatorPtr& hostOperator, const LogicalOperatorPtr& targetOperator)
{
    std::map<LogicalOperatorPtr, std::tuple<LogicalOperatorPtr, ContainmentRelationship>> targetHostOperatorMap;
    ...
    if (containmentType == ContainmentRelationship::EQUALITY)
    {
        ...
        for (const auto& targetParent : targetOperator->getParents())
        {
            ...
            for (const auto& hostParent : hostOperator->getParents())
            {
                ...
                auto matchedOperators = areOperatorsContained(hostParent->as<LogicalOperator>(), targetParent->as<LogicalOperator>());
                if (!matchedOperators.empty())
                {
                    targetHostOperatorMap.merge(matchedOperators);
                    matchCount++;
    ...
    {
        NES_TRACE(
            "Target and host operators are contained. Host (leftSig): {}, Target (rightSig): {}, ContainmentType: {}",
