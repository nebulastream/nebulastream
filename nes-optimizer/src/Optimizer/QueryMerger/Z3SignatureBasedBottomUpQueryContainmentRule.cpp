...
bool Z3SignatureBasedBottomUpQueryContainmentRule::apply(
  ...
  NES_INFO(
      "Z3SignatureBasedQueryContainmentRule: Applying Signature Based Equal "
      "Query Merger Rule to the "
      "Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "Z3SignatureBasedQueryContainmentRule: Found no new query plan to add "
  ...
  NES_DEBUG(
      "Z3SignatureBasedQueryContainmentRule: Iterating over all Shared Query "
      "MetaData in the Global "
      "Query Plan");
  ...
  for (const auto& targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (signatureContainmentUtil
        ...
        NES_TRACE(
            "Z3SignatureBasedCompleteQueryMergerRule: Merge target Shared "
        ...
        for (auto& targetSinkChild : targetSinkChildren) {
          for (auto& hostChild : hostSinkChildren) {
            ...
            if (!addedNewParent) {
              NES_WARNING(
                  "Z3SignatureBasedCompleteQueryMergerRule: Failed to add new "
                  "parent");
            }
        ...
        if (!matchedTargetToHostOperatorMap.empty()) {
          ...
          for (auto [targetOp, hostOperatorAndRelationship] :
            ...
            if (containmentType == ContainmentRelationship::EQUALITY) {
              NES_TRACE(
                  "Current host sqp {}; Output schema equality target {}; "
    ...
    if (!matched) {
      NES_DEBUG(
          "Z3SignatureBasedQueryContainmentRule: computing a new Shared Query "
          "Plan");
...
Z3SignatureBasedBottomUpQueryContainmentRule::areQueryPlansContained(
  ...
  NES_DEBUG(
      "Check if the target and address query plans are syntactically "
      "contained.");
...
Z3SignatureBasedBottomUpQueryContainmentRule::areOperatorsContained(
  ...
  if (containmentType == ContainmentRelationship::EQUALITY) {
    ...
    NES_TRACE(
        "Target and host operators are contained. Host (leftSig): {}, Target "
        "(rightSig): {}, ContainmentType: {}",
