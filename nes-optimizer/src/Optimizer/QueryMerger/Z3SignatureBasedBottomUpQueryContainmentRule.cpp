...
Z3SignatureBasedBottomUpQueryContainmentRule::
    ...
    : BaseQueryMergerRule() {
  // For the bottom up case, we always allow the SQP as containee and therefore
...
Z3SignatureBasedBottomUpQueryContainmentRule::create(
    const z3::ContextPtr &context, bool allowExhaustiveContainmentCheck) {
...
bool Z3SignatureBasedBottomUpQueryContainmentRule::apply(
    GlobalQueryPlanPtr globalQueryPlan) {

  NES_INFO("Z3SignatureBasedQueryContainmentRule: Applying Signature Based "
           "Equal Query Merger Rule to the "
           "Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING("Z3SignatureBasedQueryContainmentRule: Found no new query plan "
                "to add in the global query plan."
                " Skipping the Signature Based Equal Query Merger Rule.");
    return true;
  }

  NES_DEBUG("Z3SignatureBasedQueryContainmentRule: Iterating over all Shared "
            "Query MetaData in the Global "
            "Query Plan");
  ...
  for (const auto &targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto &hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      // Before the bottom up check, we first check the whole query for
      // equality.
      if (signatureContainmentUtil
              ->checkContainmentForBottomUpMerging(hostSink, targetSink)
              ->containmentRelationship == ContainmentRelationship::EQUALITY) {
        NES_TRACE("Z3SignatureBasedCompleteQueryMergerRule: Merge target "
                  "Shared metadata into address metadata");
        ...
        // Iterate over target children operators and migrate their parents to
        // the host children operators.
        ...
        for (auto &targetSinkChild : targetSinkChildren) {
          for (auto &hostChild : hostSinkChildren) {
            ...
            if (!addedNewParent) {
              NES_WARNING("Z3SignatureBasedCompleteQueryMergerRule: Failed to "
                          "add new parent");
            }
            // hostSharedQueryPlan->addAdditionToChangeLog(hostChild->as<Operator>(),
            // targetSink);
          }
        ...
        auto matchedTargetToHostOperatorMap =
            areQueryPlansContained(targetQueryPlan, hostQueryPlan);
        NES_DEBUG("matchedTargetToHostOperatorMap empty? {}",
                  matchedTargetToHostOperatorMap.empty());
        if (!matchedTargetToHostOperatorMap.empty()) {
          ...
          if (matchedTargetToHostOperatorMap.size() > 1) {
            ...
            matchedTargetOperators.reserve(
                matchedTargetToHostOperatorMap.size());
            for (auto &[leftQueryOperators,
                        rightQueryOperatorsAndRelationship] :
                 matchedTargetToHostOperatorMap) {
              if (std::get<1>(rightQueryOperatorsAndRelationship) ==
                  ContainmentRelationship::EQUALITY) {
                matchedTargetOperators.emplace_back(leftQueryOperators);
              }
            }
            // Iterate over the target operators and remove the upstream
            // operators covered by downstream matched operators
            for (uint64_t i = 0; i < matchedTargetOperators.size(); i++) {
              for (uint64_t j = 0; j < matchedTargetOperators.size(); j++) {
                ...
                if (matchedTargetOperators[i]->containAsGrandChild(
                  ...
                  matchedTargetToHostOperatorMap.erase(
                      matchedTargetOperators[j]);
                } else if (matchedTargetOperators[i]->containAsGrandParent(
                               matchedTargetOperators[j])) {
                  matchedTargetToHostOperatorMap.erase(
                      matchedTargetOperators[i]);
                  break;
                }
              }
            }
          ...
          // Iterate over all matched pairs of operators and merge the query
          // plan
          for (auto [targetOp, hostOperatorAndRelationship] :
               matchedTargetToHostOperatorMap) {
            LogicalOperatorPtr targetOperator = targetOp;
            LogicalOperatorPtr hostOperator =
                std::get<0>(hostOperatorAndRelationship);
            ContainmentRelationship containmentType =
                std::get<1>(hostOperatorAndRelationship);
            if (containmentType == ContainmentRelationship::EQUALITY) {
              NES_TRACE("Current host sqp {}; Output schema equality target "
                        "{}; Output schema equality host {}; "
                        "Target parent size {}",
            ...
            } else if (std::get<1>(hostOperatorAndRelationship) ==
                           ContainmentRelationship::RIGHT_SIG_CONTAINED &&
                       checkWindowContainmentPossible(hostOperator,
                                                      targetOperator)) {
              // if we're adding a window, we first need to obtain the watermark
              // for that window
              if (targetOperator->instanceOf<WindowOperator>()) {
                targetOperator =
                    targetOperator->getChildren()[0]->as<LogicalOperator>();
              }
              // obtain the child operation of the sink operator to merge the
              ...
              if (hostOperator->instanceOf<SinkLogicalOperator>()) {
                ...
                hostOperator =
                    hostOperator->getChildren()[0]->as<LogicalOperator>();
              }
              NES_TRACE("Adding parent {} to {}", targetOperator->toString(),
                        hostOperator->toString());
              // in case target operator has more than one child (e.g. join or
              // union) obtain the parent operator
              if (targetOperator->getChildren().size() > 1) {
                // will only have one parent operator
                targetOperator =
                    targetOperator->getParents()[0]->as<LogicalOperator>();
              }
              ...
              // targetOperator);
              NES_TRACE("New shared query plan: {}",
                        hostSharedQueryPlan->getQueryPlan()->toString());
            } else if (std::get<1>(hostOperatorAndRelationship) ==
                           ContainmentRelationship::LEFT_SIG_CONTAINED &&
                       checkWindowContainmentPossible(targetOperator,
                                                      hostOperator)) {
              // if we're adding a window, we first need to obtain the watermark
              // for that window
              ...
              NES_TRACE("Adding parent {} to {}", hostOperator->toString(),
                        targetOperator->toString());
              // we cannot match union or logical operator nodes because they
              // cannot be safely merged as the sqp is not a tree
              ...
              NES_TRACE("Current host operator: {}",
                        targetOperator->toString());
              bool addedNewParent = targetOperator->addParent(hostOperator);
              if (!addedNewParent) {
                NES_WARNING("Failed to add new parent");
              }
              // hostSharedQueryPlan->addAdditionToChangeLog(targetOperator,
              // hostOperator);
              NES_DEBUG("New shared query plan: {}",
                        hostSharedQueryPlan->getQueryPlan()->toString());
            }
          }
    ...
    if (!matched) {
      NES_DEBUG("Z3SignatureBasedQueryContainmentRule: computing a new Shared "
                "Query Plan");
...
bool Z3SignatureBasedBottomUpQueryContainmentRule::
    checkWindowContainmentPossible(const LogicalOperatorPtr &container,
  ...
  if (containee->instanceOf<WindowOperator>()) {
    auto containeeWindowDefinition =
        containee->as<WindowOperator>()->getWindowDefinition();
    ...
    if (containeeWindowType->instanceOf<Windowing::TimeBasedWindowType>()) {
      auto containeeTimeBasedWindow =
          containeeWindowType->as<Windowing::TimeBasedWindowType>();
      // we need to set the time characteristic field to start because the
      // previous timestamp will not exist anymore
...
std::map<LogicalOperatorPtr,
         std::tuple<LogicalOperatorPtr, ContainmentRelationship>>
Z3SignatureBasedBottomUpQueryContainmentRule::areQueryPlansContained(
    const QueryPlanPtr &hostQueryPlan, const QueryPlanPtr &targetQueryPlan) {

  std::map<LogicalOperatorPtr,
           std::tuple<LogicalOperatorPtr, ContainmentRelationship>>
      targetHostOperatorMap;
  ...
  if (targetSourceOperators.size() != hostSourceOperators.size()) {
    NES_WARNING(
        "Not matched as number of Sources in target and host query plans are "
        "different.");
    return {};
  }

  // Fetch the first source operator and find a corresponding matching source
  // operator in the address source operator list
  for (auto &targetSourceOperator : targetSourceOperators) {
    ...
    for (auto &hostSourceOperator : hostSourceOperators) {
      ...
      auto matchedOperators =
          areOperatorsContained(hostSourceOperator, targetSourceOperator);
...
std::map<LogicalOperatorPtr,
         std::tuple<LogicalOperatorPtr, ContainmentRelationship>>
Z3SignatureBasedBottomUpQueryContainmentRule::areOperatorsContained(
  ...
  std::map<LogicalOperatorPtr,
      ...
      targetHostOperatorMap;
  if (targetOperator->instanceOf<SinkLogicalOperator>() &&
      hostOperator->instanceOf<SinkLogicalOperator>()) {
    NES_DEBUG("Both target and host operators are of sink type.");
    return {};
  }

  NES_DEBUG("Compare target {} and host {} operators.",
            targetOperator->toString(), hostOperator->toString());
  auto containmentInformation =
      signatureContainmentUtil->checkContainmentForBottomUpMerging(
  ...
  if (containmentType == ContainmentRelationship::EQUALITY) {
    ...
    for (const auto &targetParent : targetOperator->getParents()) {
      ...
      for (const auto &hostParent : hostOperator->getParents()) {
        ...
        auto matchedOperators =
            areOperatorsContained(hostParent->as<LogicalOperator>(),
                                  targetParent->as<LogicalOperator>());
        if (!matchedOperators.empty()) {
          targetHostOperatorMap.merge(matchedOperators);
          matchCount++;
          break;
    ...
    NES_TRACE("Target and host operators are contained. Host (leftSig): {}, "
              "Target (rightSig): {}, ContainmentType: {}",
              hostOperator->toString(), targetOperator->toString(),
              hostOperator->toString(), magic_enum::enum_name(containmentType));
    if (targetOperator->instanceOf<LogicalJoinOperator>() &&
        hostOperator->instanceOf<LogicalJoinOperator>()) {
      return targetHostOperatorMap;
    }
    targetHostOperatorMap[targetOperator] = {hostOperator, containmentType};
