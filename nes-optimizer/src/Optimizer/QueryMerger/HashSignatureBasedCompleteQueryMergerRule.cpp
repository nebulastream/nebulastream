...
bool HashSignatureBasedCompleteQueryMergerRule::apply(
  ...
  NES_INFO(
      "HashSignatureBasedCompleteQueryMergerRule: Applying Signature Based "
      "Equal Query Merger Rule to the Global Query Plan");
  auto queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "HashSignatureBasedCompleteQueryMergerRule: Found no new query "
  ...
  NES_DEBUG(
      "HashSignatureBasedCompleteQueryMergerRule: Iterating over all Shared "
  ...
  for (auto& targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (foundMatch) {
        NES_TRACE(
            "HashSignatureBasedCompleteQueryMergerRule: Merge target Shared "
    ...
    if (!merged) {
      NES_DEBUG(
          "HashSignatureBasedCompleteQueryMergerRule: computing a new Shared "
          "Query Plan");
