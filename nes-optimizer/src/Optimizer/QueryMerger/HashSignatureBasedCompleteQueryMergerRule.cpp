...
HashSignatureBasedCompleteQueryMergerRulePtr
HashSignatureBasedCompleteQueryMergerRule::create() {
  return std::make_shared<HashSignatureBasedCompleteQueryMergerRule>();
}

bool HashSignatureBasedCompleteQueryMergerRule::apply(
    GlobalQueryPlanPtr globalQueryPlan) {
  NES_INFO("HashSignatureBasedCompleteQueryMergerRule: Applying Signature "
           "Based Equal Query Merger Rule to the Global Query Plan");
  auto queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING("HashSignatureBasedCompleteQueryMergerRule: Found no new query "
                "metadata in the global query plan."
                " Skipping the Signature Based Equal Query Merger Rule.");
    return true;
  }

  NES_DEBUG("HashSignatureBasedCompleteQueryMergerRule: Iterating over all "
  ...
  for (auto &targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto &hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      // if there are no matching global query nodes then the shared query
      // metadata are not matched
      ...
      auto targetSignatureStringValue =
          *targetSignature.begin()->second.begin();

      if (hostSignature.find(targetSignatureHashValue) != hostSignature.end()) {
        auto hostSignatureStringValues =
            hostSignature[targetSignatureHashValue];
        auto match = std::find_if(
            hostSignatureStringValues.begin(), hostSignatureStringValues.end(),
        ...
        if (match != hostSignatureStringValues.end()) {
          targetToHostSinkOperatorMap[targetSinkOperators[0]] =
              hostSharedQueryPlan->getSinkOperators()[0];
          foundMatch = true;
        }
      }

      if (foundMatch) {
        NES_TRACE("HashSignatureBasedCompleteQueryMergerRule: Merge target "
        ...
        // Iterate over all matched pairs of sink operators and merge the query
        // plan
        for (auto &[targetSinkOperator, hostSinkOperator] :
    ...
    if (!merged) {
      NES_DEBUG("HashSignatureBasedCompleteQueryMergerRule: computing a new "
                "Shared Query Plan");
