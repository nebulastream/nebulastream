...
HybridCompleteQueryMergerRulePtr HybridCompleteQueryMergerRule::create(
    z3::ContextPtr context) {
...
bool HybridCompleteQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
  NES_INFO(
      "HybridCompleteQueryMergerRule: Applying Signature Based Equal Query "
      "Merger Rule to the Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "HybridCompleteQueryMergerRule: Found no new query plan to add in the "
        "global query plan."
  ...
  NES_DEBUG(
      "HybridCompleteQueryMergerRule: Iterating over all Shared Query MetaData "
  ...
  for (const auto& targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (foundMatch) {
        NES_TRACE(
            "HybridCompleteQueryMergerRule: Merge target Shared metadata into "
            "address metadata");
