...
HybridCompleteQueryMergerRule::HybridCompleteQueryMergerRule(
    z3::ContextPtr context)
    : BaseQueryMergerRule(), context(context) {
  signatureEqualityUtil = SignatureEqualityUtil::create(context);
}

HybridCompleteQueryMergerRulePtr
HybridCompleteQueryMergerRule::create(z3::ContextPtr context) {
  return std::make_shared<HybridCompleteQueryMergerRule>(
      HybridCompleteQueryMergerRule(std::move(context)));
}

bool HybridCompleteQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
  NES_INFO("HybridCompleteQueryMergerRule: Applying Signature Based Equal "
           "Query Merger Rule to the Global Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING("HybridCompleteQueryMergerRule: Found no new query plan to add "
                "in the global query plan."
                " Skipping the Signature Based Equal Query Merger Rule.");
    return true;
  }

  NES_DEBUG("HybridCompleteQueryMergerRule: Iterating over all Shared Query "
  ...
  for (const auto &targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto &hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      auto targetHashSignature =
          targetSinkOperators[0]->getHashBasedSignature();
      auto targetSignatureHashValue = targetHashSignature.begin()->first;
      auto targetSignatureStringValue =
          *targetHashSignature.begin()->second.begin();

      // Check if the host and target sink operator signatures match each other
      if (hostHashSignature.find(targetSignatureHashValue) !=
          hostHashSignature.end()) {
        auto hostSignatureStringValues =
            hostHashSignature[targetSignatureHashValue];
      ...
      // Not all sinks found an equivalent entry in the target shared query
      // metadata
      if (foundMatch) {
        NES_TRACE("HybridCompleteQueryMergerRule: Merge target Shared metadata "
    ...
    if (!matched) {
      NES_DEBUG(
          "HybridCompleteQueryMergerRule: computing a new Shared Query Plan");
