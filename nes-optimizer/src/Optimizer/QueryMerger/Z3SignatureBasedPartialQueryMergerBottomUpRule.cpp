...
Z3SignatureBasedPartialQueryMergerBottomUpRule::
    Z3SignatureBasedPartialQueryMergerBottomUpRule(z3::ContextPtr context) {
  signatureEqualityUtil = SignatureEqualityUtil::create(std::move(context));
}

Z3SignatureBasedPartialQueryMergerBottomUpRulePtr
Z3SignatureBasedPartialQueryMergerBottomUpRule::create(z3::ContextPtr context) {
...
bool Z3SignatureBasedPartialQueryMergerBottomUpRule::apply(
    GlobalQueryPlanPtr globalQueryPlan) {
  NES_INFO("Z3SignatureBasedPartialQueryMergerBottomUpRule: Applying Signature "
           "Based Equal Query Merger Rule to the Global "
           "Query Plan");
  std::vector<QueryPlanPtr> queryPlansToAdd =
      globalQueryPlan->getQueryPlansToAdd();
  if (queryPlansToAdd.empty()) {
    NES_WARNING("Z3SignatureBasedPartialQueryMergerBottomUpRule: Found only a "
                "single query metadata in the global query plan."
                " Skipping the Signature Based Equal Query Merger Rule.");
    return true;
  }

  NES_DEBUG("Z3SignatureBasedPartialQueryMergerBottomUpRule: Iterating over "
            "all Shared Query MetaData in the Global Query Plan");
  ...
  for (const auto &targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto &hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (!matchedTargetToHostOperatorMap.empty()) {
        NES_TRACE("Z3SignatureBasedPartialQueryMergerBottomUpRule: Merge "
                  "target Shared metadata into address metadata");
        ...
        for (auto [targetOperator, hostOperator] :
          ...
          matchedOperatorPairs.emplace_back(MatchedOperatorPair::create(
              hostOperator, targetOperator, ContainmentRelationship::EQUALITY));
        }
    ...
    if (!matched) {
      NES_DEBUG("Z3SignatureBasedPartialQueryMergerBottomUpRule: computing a "
                "new Shared Query Plan");
...
Z3SignatureBasedPartialQueryMergerBottomUpRule::areQueryPlansEqual(
  ...
  NES_DEBUG("Z3SignatureBasedPartialQueryMergerBottomUpRule: check if the "
            "target and address query plans are syntactically "
            "equal or not");
  ...
  if (targetSourceOperators.size() != hostSourceOperators.size()) {
    NES_WARNING("Z3SignatureBasedPartialQueryMergerBottomUpRule: Not matched "
                "as number of Sources in target and host query plans are "
                "different.");
    return {};
  }
  ...
  for (auto &targetSourceOperator : targetSourceOperators) {
    for (auto &hostSourceOperator : hostSourceOperators) {
      auto matchedOperators =
          areOperatorEqual(targetSourceOperator, hostSourceOperator);
...
std::map<LogicalOperatorPtr, LogicalOperatorPtr>
Z3SignatureBasedPartialQueryMergerBottomUpRule::areOperatorEqual(
  ...
  if (targetOperator->instanceOf<SinkLogicalOperator>() &&
    ...
    NES_TRACE("Z3SignatureBasedPartialQueryMergerBottomUpRule: Both target and "
              "host operators are of sink type.");
    return {};
  }

  NES_TRACE("HashSignatureBasedPartialQueryMergerRule: Compare target and host "
            "operators.");
  if (signatureEqualityUtil->checkEquality(targetOperator->getZ3Signature(),
                                           hostOperator->getZ3Signature())) {
    NES_TRACE("Z3SignatureBasedPartialQueryMergerBottomUpRule: Check if "
              "parents of target and address operators are equal.");
    uint16_t matchCount = 0;
    for (const auto &targetParent : targetOperator->getParents()) {
      for (const auto &hostParent : hostOperator->getParents()) {
        auto matchedOperators =
            areOperatorEqual(targetParent->as<LogicalOperator>(),
                             hostParent->as<LogicalOperator>());
        if (!matchedOperators.empty()) {
          targetHostOperatorMap.merge(matchedOperators);
          matchCount++;
          break;
  ...
  NES_WARNING("Z3SignatureBasedPartialQueryMergerBottomUpRule: Target and host "
