...
bool Z3SignatureBasedPartialQueryMergerBottomUpRule::apply(
  ...
  NES_INFO(
      "Z3SignatureBasedPartialQueryMergerBottomUpRule: Applying Signature "
  ...
  if (queryPlansToAdd.empty()) {
    NES_WARNING(
        "Z3SignatureBasedPartialQueryMergerBottomUpRule: Found only a single "
  ...
  NES_DEBUG(
      "Z3SignatureBasedPartialQueryMergerBottomUpRule: Iterating over all "
  ...
  for (const auto& targetQueryPlan : queryPlansToAdd) {
    ...
    for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
      ...
      if (!matchedTargetToHostOperatorMap.empty()) {
        NES_TRACE(
            "Z3SignatureBasedPartialQueryMergerBottomUpRule: Merge target "
    ...
    if (!matched) {
      NES_DEBUG(
          "Z3SignatureBasedPartialQueryMergerBottomUpRule: computing a new "
          "Shared Query Plan");
...
Z3SignatureBasedPartialQueryMergerBottomUpRule::areQueryPlansEqual(
  ...
  NES_DEBUG(
      "Z3SignatureBasedPartialQueryMergerBottomUpRule: check if the target and "
      "address query plans are syntactically "
      "equal or not");
  ...
  if (targetSourceOperators.size() != hostSourceOperators.size()) {
    NES_WARNING(
        "Z3SignatureBasedPartialQueryMergerBottomUpRule: Not matched as number "
        "of Sources in target and host query plans are "
        "different.");
    return {};
  }
...
Z3SignatureBasedPartialQueryMergerBottomUpRule::areOperatorEqual(
  ...
  if (targetOperator->instanceOf<SinkLogicalOperator>() &&
    ...
    NES_TRACE(
        "Z3SignatureBasedPartialQueryMergerBottomUpRule: Both target and host "
        "operators are of sink type.");
    return {};
  }

  NES_TRACE(
      "HashSignatureBasedPartialQueryMergerRule: Compare target and host "
      "operators.");
  if (signatureEqualityUtil->checkEquality(targetOperator->getZ3Signature(),
                                           hostOperator->getZ3Signature())) {
    NES_TRACE(
        "Z3SignatureBasedPartialQueryMergerBottomUpRule: Check if parents of "
  ...
  NES_WARNING(
      "Z3SignatureBasedPartialQueryMergerBottomUpRule: Target and host "
