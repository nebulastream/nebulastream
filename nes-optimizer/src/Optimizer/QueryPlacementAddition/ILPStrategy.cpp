    ...
    PlacementAmendmentMode placementAmendmentMode)
{
    ...
    return std::make_unique<ILPStrategy>(ILPStrategy(globalExecutionPlan, topology, typeInferencePhase, z3Context, placementAmendmentMode));
}
...
ILPStrategy::ILPStrategy(
    ...
    PlacementAmendmentMode placementAmendmentMode)
    : BasePlacementAdditionStrategy(globalExecutionPlan, topology, typeInferencePhase, placementAmendmentMode), z3Context(z3Context)
{
}
    ...
    DecomposedQueryPlanVersion querySubPlanVersion)
{
    try
    {
        ...
        NES_INFO(
            "Processing {} num of upstream and {} num of downstream optrs",
        ...
        opt.minimize(weightNetwork * costNet + weightOverUtilization * overUtilizationCost); // where the actual optimization happen
        ...
        for (auto const& [topologyID, P] : placementVariables)
        {
            ...
            { // means we place the operator in the node
    ...
    std::map<WorkerId, double>& nodeMileageMap)
{
    ...
    while (!operatorsToProcess.empty())
    {
        ...
        { // Compute the possible pinning locations
            ...
            if (connectedToPinnedOperator)
            {
                ...
                for (const auto& connectedPinnedUpstreamNode : connectedPinnedUpstreamNodes)
                {
                    ...
                    auto workerId
                        = std::any_cast<WorkerId>(connectedPinnedUpstreamNode->as_if<LogicalOperator>()->getProperty(PINNED_WORKER_ID));

                    // Check if the topology node was already added
                    bool alreadyAdded = std::find_if(
                                            upstreamTopologyNodes.begin(),
                                            upstreamTopologyNodes.end(),
                                            [&](const TopologyNodePtr& topologyNode) { return topologyNode->getId() == workerId; })
                        != upstreamTopologyNodes.end();
                    if (!alreadyAdded)
                    {
                ...
                for (const auto& connectedPinnedDownstreamNode : connectedPinnedDownstreamNodes)
                {
                    ...
                    auto workerId
                        = std::any_cast<WorkerId>(connectedPinnedDownstreamNode->as_if<LogicalOperator>()->getProperty(PINNED_WORKER_ID));
                    // Check if the topology node was already added
                    bool alreadyAdded = std::find_if(
            ...
            { // Compute the possible locations based on the connected upstream operators
                ...
                for (const auto& upstreamOperator : upstreamOperators)
                {
                    ...
                    {
                        possiblePinningLocations = potentialOperatorPinningLocation[upstreamOperator->as_if<LogicalOperator>()->getId()];
                    }
                    else
                    { // Compute intersection with upstream operator to find common worker nodes
                        ...
                        possiblePinningLocations.erase(
                            std::remove_if(
                                ...
                                [&](WorkerId workerId)
                                {
                                    ...
                                    return std::find(
                                               upstreamOperatorPossiblePinningLocations.begin(),
        ...
        std::for_each(
            ...
            [&](const NodePtr& operatorNode) { operatorsToProcess.emplace(operatorNode->as<LogicalOperator>()); });
    }
    ...
    for (auto& pinnedUpStreamOperator : pinnedUpStreamOperators)
    {
        ...
        {
            NES_DEBUG(
                "Skip: Operator {} is already placed and thus skipping placement of it. Continuing with placing its "
            ...
            for (auto& downStreamNode : pinnedUpStreamOperator->getParents())
            {
                ...
                {
                    NES_DEBUG(
                        "Skipping downstream operator with id {} because it is already placed.",
...
void ILPStrategy::identifyPinningLocation(
    const LogicalOperatorPtr& currentOperatorNode,
    z3::optimize& opt,
    std::unordered_map<OperatorId, std::vector<WorkerId>> potentialOperatorPinningLocation,
    ...
    std::map<WorkerId, double>& nodeMileageMap)
{
    ...
    if (operatorNode->getOperatorState() != OperatorState::PLACED)
    {
        ...
        for (uint64_t j = 0; j < workerNodeIdsInBFS.size(); j++)
        {
            ...
            bool canBePlaced
                = (std::find(potentialPinningLocations.begin(), potentialPinningLocations.end(), topologyID)
                    ...
                    for (const auto& child : operatorNode->getChildren())
                    {
                        ...
                        for (uint64_t n = 0; n <= j; ++n)
                        {
                            ...
                            bool canUpstreamOperatorBePlaced
                                = (std::find(
                                       potentialUpstreamOperatorPinningLocations.begin(),
                        ...
                        {
                            operatorPinningConstraint = (operatorPinningConstraint && z3::mk_or(upstreamOperatorPinningConstraints));
                        }
                        counter++;
                    }
                }
