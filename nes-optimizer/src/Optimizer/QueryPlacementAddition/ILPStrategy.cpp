...
ILPStrategy::create(const GlobalExecutionPlanPtr &globalExecutionPlan,
  ...
  return std::make_unique<ILPStrategy>(
      ILPStrategy(globalExecutionPlan, topology, typeInferencePhase, z3Context,
...
static std::pair<OperatorId, WorkerId>
deconstructTopologyId(std::string_view topologyId, const char *separator) {
  uint64_t operatorId;
  ...
  if (std::from_chars(operatorIdSubstring.data(),
                      operatorIdSubstring.data() + operatorIdSubstring.length(),
                      operatorId)
          .ec == std::errc::invalid_argument) {
    NES_THROW_RUNTIME_ERROR("Could not parse OperatorId: " << topologyId);
  }
  if (std::from_chars(executionNodeSubstr.data(),
                      executionNodeSubstr.data() + executionNodeSubstr.length(),
                      executionNodeId)
          .ec == std::errc::invalid_argument) {
...
PlacementAdditionResult ILPStrategy::updateGlobalExecutionPlan(
  ...
  try {
    NES_INFO("Performing placement of the input query plan with id {}",
             sharedQueryId);
    NES_INFO("Processing {} num of upstream and {} num of downstream optrs",
             pinnedUpStreamOperators.size(), pinnedDownStreamOperators.size());
    ...
    std::unordered_map<OperatorId, LogicalOperatorPtr>
        operatorIdToCopiedOperatorMap;
    auto nodeMileageMap = computeMileage(copy.copiedPinnedDownStreamOperators);

    // 2. Construct the placementVariable, compute distance, utilization and
    // mileages
    for (const auto &pinnedUpStreamOperator :
      ...
      operatorIdToCopiedOperatorMap[pinnedUpStreamOperator->getId()] =
          pinnedUpStreamOperator;
      while (!toProcess.empty()) {
        auto operatorToProcess = toProcess.front();
        toProcess.pop();
        // Before further processing please identify if the operator to be
        // processed is among the collection of pinned downstream operators
        auto isPinnedDownStreamOperator = std::find_if(
            ...
            [operatorToProcess](
                const LogicalOperatorPtr &pinnedDownStreamOperator) {
              return pinnedDownStreamOperator->getId() ==
                     operatorToProcess->as_if<LogicalOperator>()->getId();
            });

        // Skip further processing if encountered pinned downstream operator
        if (isPinnedDownStreamOperator !=
            copy.copiedPinnedDownStreamOperators.end()) {
          NES_DEBUG("Found pinned downstream operator. Skipping further "
        ...
        for (auto &downstreamOperator : downstreamOperators) {
          auto downstreamLogicalOperator =
              downstreamOperator->as_if<LogicalOperator>();
          operatorIdToCopiedOperatorMap[downstreamLogicalOperator->getId()] =
              downstreamLogicalOperator;

          // Only include unplaced operators in the path
          if (downstreamLogicalOperator->getOperatorState() !=
      ...
      for (const auto &pinnedDownstreamOperator :
           copy.copiedPinnedDownStreamOperators) {
        if (!pinnedDownstreamOperator->hasProperty("cost") ||
            !pinnedDownstreamOperator->hasProperty("output")) {
          assignOperatorDefaultProperties(pinnedDownstreamOperator);
        }
      }

      // 2.4 Add constraints to Z3 solver and compute operator distance, node
      // utilization, and node mileage map
      addConstraints(opt, copy.copiedPinnedUpStreamOperators,
                     copy.copiedPinnedDownStreamOperators, placementVariables,
                     operatorPositionMap, nodeUtilizationMap, nodeMileageMap);
    }

    // 3. Calculate the network cost. (Network cost = sum over all operators
    // (output of operator * distance of operator))
    ...
    for (auto const &[operatorID, position] : operatorPositionMap) {
      LogicalOperatorPtr logicalOperator =
          operatorMap[operatorID]->as<LogicalOperator>();
      ...
      for (const auto &downStreamOperator : logicalOperator->getParents()) {
        ...
        if (operatorMap.find(downStreamOperatorId) != operatorMap.end()) {
          ...
          auto distance =
              operatorPositionMap.find(downStreamOperatorId)->second - position;
          NES_DEBUG("Distance of {} to {} is: {}", operatorID,
                    downStreamOperatorId, distance.to_string());
          std::any prop = logicalOperator->getProperty("output");
          double output = std::any_cast<double>(prop);
          NES_DEBUG("Property output of {} is: {}", logicalOperator->getId(),
                    output);
          // Summing up the amount of data multiplied by the distance to the
          // position of the next operator and adding it to already summed up
          // values of the same kind
          costNet =
              costNet +
              z3Context->real_val(std::to_string(output).c_str()) * distance;
        }
      }
    }
    ...
    auto overUtilizationCost =
        z3Context->int_val(0); // initialize the over-utilization cost with 0
    for (auto const &[topologyID, utilization] : nodeUtilizationMap) {
      std::string overUtilizationId = fmt::format("S{}", topologyID);
      auto currentOverUtilization = z3Context->int_const(
          overUtilizationId.c_str()); // an integer expression of the slack

      // Obtain the available slot in the current node
      TopologyNodePtr topologyNode =
          workerIdToTopologyNodeMap[topologyID]->as<TopologyNode>();
      auto availableSlot = topologyNode->getAvailableResources();

      opt.add(currentOverUtilization >=
              0); // we only penalize over-utilization, hence its value should
                  // be >= 0.
      opt.add(utilization - currentOverUtilization <=
              availableSlot); // formula for the over-utilization

      overUtilizationCost = overUtilizationCost + currentOverUtilization;
    }

    auto weightOverUtilization = z3Context->real_val(
        std::to_string(this->overUtilizationCostWeight).c_str());
    auto weightNetwork =
        z3Context->real_val(std::to_string(this->networkCostWeight).c_str());

    NES_DEBUG("Values of weightNetwork, costNet, weightOverUtilization,  "
              "overUtilizationCost:");
    NES_DEBUG("WeightNetwork: {} \n costNet: {} \n weightOverUtilization: {} "
              "\n overUtilizationCost:{}",
              weightNetwork.to_string(), costNet.to_string(),
              weightOverUtilization.to_string(),
              overUtilizationCost.to_string());

    // 5. Optimize ILP problem and print solution.
    opt.minimize(
        weightNetwork * costNet +
        weightOverUtilization *
            overUtilizationCost); // where the actual optimization happen

    // 6. Check if we have solution, return false if that is not the case
    z3::check_result result;
    uint8_t maxRetryCount = 4;
    uint8_t retryCount = 0;
    while (((result = opt.check()) == z3::unknown) &&
    ...
    if (z3::sat != result) {
      NES_ERROR("Solver failed for {} with {} after {} retries.", sharedQueryId,
                result, retryCount)
      return {false, {}};
    }
    ...
    NES_INFO("Solver found solution with cost: {}",
             z3Model.eval(costNet).get_decimal_string(4));

    // 7. Pick the solution which has placement decision of 1, i.e., the ILP
    // decide to place the operator in that node
    std::map<OperatorPtr, TopologyNodePtr> operatorToTopologyNodeMap;
    for (auto const &[topologyID, P] : placementVariables) {
      if (z3Model.eval(P).get_numeral_int() ==
          1) { // means we place the operator in the node
        auto [operatorId, nodeId] =
            deconstructTopologyId(topologyID, KEY_SEPARATOR);

        auto operatorIdToCopiedOperator =
            operatorIdToCopiedOperatorMap.find(operatorId);
        LogicalOperatorPtr logicalOperator;
        ...
        if (!logicalOperator->hasProperty(PINNED_WORKER_ID)) {
          ...
          NES_DEBUG("Optr {} is pinned on {}", logicalOperator->toString(),
                    std::any_cast<WorkerId>(
        ...
        operatorToTopologyNodeMap.insert(
            std::make_pair(logicalOperator, topologyNode));
      }
    }
    ...
    for (auto const &[operatorNode, topologyNode] : operatorToTopologyNodeMap) {
      NES_INFO("Operator {} is executed on Topology Node {}",
               operatorNode->toString(), topologyNode->toString());
    }
...
std::map<WorkerId, double> ILPStrategy::computeMileage(
    const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators) {
  ...
  for (const auto &pinnedDownStreamOperator : pinnedDownStreamOperators) {
    auto nodeId = std::any_cast<WorkerId>(
...
void ILPStrategy::computeDistance(const TopologyNodePtr &node,
  ...
  // if the current node is not a root node, recursively compute the mileage of
  // its first parent assuming a node only has a single parent
  ...
  if (!node->getLinkProperty(childID) ||
      !node->getLinkProperty(childID)->bandwidth) {
    auto linkProperty = std::make_shared<LinkProperty>(512, 100);
    node->addLinkProperty(childID, linkProperty);
  }
  mileages[topologyID] =
      1.0 / node->getLinkProperty(childID)->bandwidth + mileages[childID];
}

void ILPStrategy::addConstraints(
    z3::optimize &opt,
    const std::set<LogicalOperatorPtr> &pinnedUpStreamOperators,
    const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators,
    std::map<std::string, z3::expr> &placementVariables,
  ...
  // FIXME (#4958): there is few short coming of the trailing algorithm (More
  // may exists):
  //  1. Possible missing pinning locations can occur as the current
  //  implementation only analyzes the upstream operators to find the possible
  //  locations. For example, if an operator is connected to a Union operator
  //  serving multiple downstream operators located on different nodes. This
  //  union operator can only be placed on topology nodes that can reach the
  //  downstream operators. Any further connected downstream operator to this
  //  union operator will, therefore, also be constrained to the possible
  //  pinning locations of the union. This can result in missed pinning
  //  opportunities.
  //  2. It assumes that connected most upstream and downstream operators are
  //  always pinned (this is generally an assumption in our system. I can not
  //  think of an example where this won't be the case).
  ...
  while (!operatorsToProcess.empty()) {
    ...
    if (operatorToProcess->hasProperty(PINNED_WORKER_ID)) {
      auto pinnedWorkerId = std::any_cast<WorkerId>(
          operatorToProcess->getProperty(PINNED_WORKER_ID));
      potentialOperatorPinningLocation[operatorToProcess->getId()] = {
          pinnedWorkerId};
    } else { // Compute the possible pinning locations

      bool upstreamAnalyzed = true;
      bool connectedToPinnedOperator = false;
      // 1. Check if all upstream operators are analyzed and their potential
      // pinning locations are identified.
      ...
      if (connectedToPinnedOperator) {
        ...
        auto connectedPinnedUpstreamNodes =
            operatorToProcess->getAllLeafNodes();
        auto connectedPinnedDownstreamNodes =
            operatorToProcess->getAllRootNodes();

        std::vector<TopologyNodePtr> upstreamTopologyNodes;
        // Fetch the pinned workerId
        for (const auto &connectedPinnedUpstreamNode :
             connectedPinnedUpstreamNodes) {
          if (!connectedPinnedUpstreamNode->as_if<LogicalOperator>()
                   ->hasProperty(PINNED_WORKER_ID)) {
            NES_ERROR("The connected leaf nodes should always carry pinning "
                      "location");
            throw std::logic_error("The connected leaf nodes should always "
                                   "carry pinning location");
          }
          auto workerId = std::any_cast<WorkerId>(
              connectedPinnedUpstreamNode->as_if<LogicalOperator>()
                  ->getProperty(PINNED_WORKER_ID));

          // Check if the topology node was already added
          bool alreadyAdded =
              std::find_if(upstreamTopologyNodes.begin(),
                           upstreamTopologyNodes.end(),
                           [&](const TopologyNodePtr &topologyNode) {
                             return topologyNode->getId() == workerId;
                           }) != upstreamTopologyNodes.end();
          if (!alreadyAdded) {
            upstreamTopologyNodes.emplace_back(
                workerIdToTopologyNodeMap[workerId]);
          }
        }

        std::vector<TopologyNodePtr> downstreamTopologyNodes;
        // Fetch the pinned workerId
        for (const auto &connectedPinnedDownstreamNode :
             connectedPinnedDownstreamNodes) {
          if (!connectedPinnedDownstreamNode->as_if<LogicalOperator>()
                   ->hasProperty(PINNED_WORKER_ID)) {
            NES_ERROR("The connected root nodes should always carry pinning "
                      "location");
            throw std::logic_error("The connected root nodes should always "
                                   "carry pinning location");
          }
          auto workerId = std::any_cast<WorkerId>(
              connectedPinnedDownstreamNode->as_if<LogicalOperator>()
                  ->getProperty(PINNED_WORKER_ID));
          // Check if the topology node was already added
          bool alreadyAdded =
              std::find_if(downstreamTopologyNodes.begin(),
                           downstreamTopologyNodes.end(),
                           [&](const TopologyNodePtr &topologyNode) {
                             return topologyNode->getId() == workerId;
                           }) != downstreamTopologyNodes.end();
          if (!alreadyAdded) {
            downstreamTopologyNodes.emplace_back(
                workerIdToTopologyNodeMap[workerId]);
          }
        }

        // Find common topology nodes
        auto commonTopologyNodes = pathFinder->findNodesBetween(
            upstreamTopologyNodes, downstreamTopologyNodes);

        // Possible pinning locations
        std::vector<WorkerId> possiblePinningLocations;

        // If only one upstream topology node then add the upstream topology
        // node to the possible pinning location
        if (upstreamTopologyNodes.size() == 1) {
          possiblePinningLocations.emplace_back(
              upstreamTopologyNodes[0]->getId());
        }

        // Iterate over common topology nodes to compute remaining possible
        // pinning locations
      ...
      } else { // Compute the possible locations based on the connected upstream
               // operators

        std::vector<WorkerId> possiblePinningLocations;
        for (const auto &upstreamOperator : upstreamOperators) {
          // If possible pinning location not computed then assign the possible
          // locations of the upstream operator
          if (possiblePinningLocations.empty()) {
            possiblePinningLocations = potentialOperatorPinningLocation
                [upstreamOperator->as_if<LogicalOperator>()->getId()];
          } else { // Compute intersection with upstream operator to find common
                   // worker nodes
            // Note: we are not using std::set_intersection to preserve the
            // workerId order
            ...
            // Remove the element from the possible pinning locations if the
            // worker id is not listed in the possible pinning location of the
            // upstream operator
            possiblePinningLocations.erase(
                std::remove_if(
                    possiblePinningLocations.begin(),
                    possiblePinningLocations.end(),
                    [&](WorkerId workerId) {
                      // return true if element not in the upstream Operator's
                      // Possible Pinning locations
                      return std::find(
                                 upstreamOperatorPossiblePinningLocations
                                     .begin(),
  ...
  for (auto &pinnedUpStreamOperator : pinnedUpStreamOperators) {
    ...
    if (pinnedUpStreamOperator->getOperatorState() == OperatorState::PLACED) {
      NES_DEBUG("Skip: Operator {} is already placed and thus skipping "
                "placement of it. Continuing with placing its "
                "downstream operators",
                pID);

      // Place all its downstream nodes
      for (auto &downStreamNode : pinnedUpStreamOperator->getParents()) {
        if (downStreamNode->as<LogicalOperator>()->getOperatorState() !=
            OperatorState::PLACED) {
          identifyPinningLocation(downStreamNode->as<LogicalOperator>(), opt,
                                  potentialOperatorPinningLocation,
                                  placementVariables, pinnedDownStreamOperators,
                                  operatorDistanceMap, nodeUtilizationMap,
                                  nodeMileageMap);
        } else {
          NES_DEBUG("Skipping downstream operator with id {} because it is "
                    "already placed.",
      ...
      identifyPinningLocation(
          pinnedUpStreamOperator, opt, potentialOperatorPinningLocation,
          placementVariables, pinnedDownStreamOperators, operatorDistanceMap,
          nodeUtilizationMap, nodeMileageMap);
    }
  }
}

void ILPStrategy::identifyPinningLocation(
    const LogicalOperatorPtr &currentOperatorNode, z3::optimize &opt,
    std::unordered_map<OperatorId, std::vector<WorkerId>>
        potentialOperatorPinningLocation,
  ...
  NES_DEBUG("Handling operatorNode {} with id: {}", operatorNode->toString(),
            operatorID);
  ...
  if (operatorMap.contains(operatorID)) {
    ...
    for (auto topologyID : workerNodeIdsInBFS) {
      std::string variableID =
          fmt::format("{}{}{}", operatorID, KEY_SEPARATOR, topologyID);
  ...
  if (operatorNode->getOperatorState() != OperatorState::PLACED) {
    ...
    auto potentialPinningLocations =
        potentialOperatorPinningLocation[operatorID];

    // for the current operatorNode we iterate here through all topologyNodes
    // and calculate costs, possibility of placement etc.
    for (uint64_t j = 0; j < workerNodeIdsInBFS.size(); j++) {
      ...
      bool canBePlaced =
          (std::find(potentialPinningLocations.begin(),
                     potentialPinningLocations.end(),
                     topologyID) != potentialPinningLocations.end());

      if (operatorNode->instanceOf<SinkLogicalOperator>() ||
          operatorNode->instanceOf<SourceLogicalOperator>()) {
        ...
        int sizeOfPIJ_stored_out = operatorNode->getChildren().size() +
                                   1; // all children + operatorNode itself
        int sizeOfPIJ_stored_in = j + 1;
        std::vector P_IJ_stored(
            sizeOfPIJ_stored_out,
        ...
        if (canBePlaced) {
          ...
          // Iterating over all children of the current operator node to make
          // sure that children should be placed on a lower topology node than
          // the current operatorNode (or the same)
          for (const auto &child : operatorNode->getChildren()) {
            auto childID = child->as<LogicalOperator>()->getId();
            auto id = fmt::format("{}{}{}", childID, KEY_SEPARATOR,
                                  workerNodeIdsInBFS[j]);
            auto P_IJ_child = this->z3Context->int_const(id.c_str());

            // Fetch the potential pinning locations
            auto potentialUpstreamOperatorPinningLocations =
                potentialOperatorPinningLocation[childID];

            z3::expr_vector upstreamOperatorPinningConstraints(*z3Context);
            // Iterating over all positions of topologyNodePath up to the
            // current one
            for (uint64_t n = 0; n <= j; ++n) {
              // n < j here and n == j in initialising checkTopologyNodes means
              // we actually check n <= j
              auto variableID_loop = fmt::format(
                  "{}{}{}", childID, KEY_SEPARATOR, workerNodeIdsInBFS[n]);
              auto P_IJ_nodes =
                  this->z3Context->int_const(variableID_loop.c_str());
              P_IJ_stored[counter][n] = P_IJ_nodes;

              bool canUpstreamOperatorBePlaced =
                  (std::find(potentialUpstreamOperatorPinningLocations.begin(),
                             potentialUpstreamOperatorPinningLocations.end(),
                             workerNodeIdsInBFS[n]) !=
                   potentialUpstreamOperatorPinningLocations.end());
              if (canUpstreamOperatorBePlaced) {
                // see if for all the children c of the operator nodes one
                // topologyNode (n) before the current one or the current one
                // exists where it (child of operatorNode) is placed
                upstreamOperatorPinningConstraints.push_back(
                    (P_IJ_stored[counter][n] == 1));
              }
            }
            if (!upstreamOperatorPinningConstraints.empty()) {
              operatorPinningConstraint =
                  (operatorPinningConstraint &&
                   z3::mk_or(upstreamOperatorPinningConstraints));
            }
            counter++;
          }
        }
        // either we do NOT place current operatorNode on current topologyNode
        // (first half) or we place it but then for all children must be true:
        // they are placed not after the current operatorNode add that condition
        // to the optimizer
        opt.add((P_IJ == 0) || operatorPinningConstraint);

        NES_DEBUG("Print the condition {}",
                  (operatorPinningConstraint).to_string());
      }

      placementVariable.insert(std::make_pair(variableID, P_IJ));
      sum_i = sum_i + P_IJ;
      NES_DEBUG("placement variables for operator {} look like this: ",
                operatorID);
      ...
      if (!operatorNode->hasProperty("cost")) {
        NES_ERROR("Cost for the operator id {} is missing",
                  operatorNode->getId());
      }
      ...
      if (iterator != nodeUtilizationMap.end()) {
        ...
        // utilization of a node = slots (i.e. computing cost of operator) *
        // placement variable
  ...
  std::stringstream v;
  v << D_i << std::endl;
  NES_DEBUG("Operator {} inserted into operatorPositionMap with value {}",
            operatorID, v.str());
  ...
  for (const auto &parent : currentOperatorNode->getParents()) {
    NES_DEBUG("parent for {} is {}", operatorID,
              parent->as<LogicalOperator>()->getId());
    if (currentOperatorNode->getOperatorState() != OperatorState::PLACED) {
      identifyPinningLocation(
          parent->as<LogicalOperator>(), opt, potentialOperatorPinningLocation,
          placementVariable, pinnedDownStreamOperators, operatorDistanceMap,
          nodeUtilizationMap, nodeMileageMap);
    } else {
      NES_DEBUG("Skipping operator with id {} because it's already placed.",
                parent->as<LogicalOperator>()->getId());
    }
  }
}

bool ILPStrategy::pinOperators(
    z3::model &z3Model, std::map<std::string, z3::expr> &placementVariables) {
  for (const auto &placementMapping : placementVariables) {
    auto key = placementMapping.first;
    auto [operatorId, topologyNodeId] =
        deconstructTopologyId(key, KEY_SEPARATOR);
...
double ILPStrategy::getOverUtilizationCostWeight() const {
  return this->overUtilizationCostWeight;
}

double ILPStrategy::getNetworkCostWeight() const {
  return this->networkCostWeight;
}

void ILPStrategy::setOverUtilizationWeight(double weight) {
  this->overUtilizationCostWeight = weight;
}

void ILPStrategy::setNetworkCostWeight(double weight) {
  this->networkCostWeight = weight;
}

void ILPStrategy::assignOperatorDefaultProperties(
