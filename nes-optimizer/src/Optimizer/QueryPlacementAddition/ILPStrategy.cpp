...
BasePlacementStrategyPtr ILPStrategy::create(
...
static std::pair<OperatorId, WorkerId> deconstructTopologyId(
    std::string_view topologyId, const char* separator) {
  uint64_t operatorId;
...
PlacementAdditionResult ILPStrategy::updateGlobalExecutionPlan(
  ...
  try {
    ...
    for (const auto& pinnedUpStreamOperator :
      ...
      while (!toProcess.empty()) {
        ...
        if (isPinnedDownStreamOperator !=
            copy.copiedPinnedDownStreamOperators.end()) {
          NES_DEBUG(
              "Found pinned downstream operator. Skipping further downstream "
              "operators.");
          break;
        }
    ...
    NES_DEBUG(
        "Values of weightNetwork, costNet, weightOverUtilization,  "
        "overUtilizationCost:");
    NES_DEBUG(
        "WeightNetwork: {} \n costNet: {} \n weightOverUtilization: {} \n "
        "overUtilizationCost:{}",
        weightNetwork.to_string(), costNet.to_string(),
        weightOverUtilization.to_string(), overUtilizationCost.to_string());
...
void ILPStrategy::addConstraints(
  ...
  while (!operatorsToProcess.empty()) {
    ...
    if (operatorToProcess->hasProperty(PINNED_WORKER_ID)) {
      ...
      if (connectedToPinnedOperator) {
        ...
        for (const auto& connectedPinnedUpstreamNode :
          ...
          if (!connectedPinnedUpstreamNode->as_if<LogicalOperator>()
            ...
            NES_ERROR(
                "The connected leaf nodes should always carry pinning "
                "location");
        ...
        for (const auto& connectedPinnedDownstreamNode :
          ...
          if (!connectedPinnedDownstreamNode->as_if<LogicalOperator>()
                   ->hasProperty(PINNED_WORKER_ID)) {
            NES_ERROR(
                "The connected root nodes should always carry pinning "
                "location");
            throw std::logic_error(
                "The connected root nodes should always carry pinning "
                "location");
      ...
      } else {  // Compute the possible locations based on the connected
                // upstream operators
        ...
        for (const auto& upstreamOperator : upstreamOperators) {
          ...
          } else {  // Compute intersection with upstream operator to find
                    // common worker nodes
  ...
  for (auto& pinnedUpStreamOperator : pinnedUpStreamOperators) {
    ...
    if (pinnedUpStreamOperator->getOperatorState() == OperatorState::PLACED) {
      NES_DEBUG(
          "Skip: Operator {} is already placed and thus skipping placement of "
          "it. Continuing with placing its "
      ...
      for (auto& downStreamNode : pinnedUpStreamOperator->getParents()) {
        if (downStreamNode->as<LogicalOperator>()->getOperatorState() !=
          ...
          NES_DEBUG(
              "Skipping downstream operator with id {} because it is already "
              "placed.",
