...
float NES::Optimizer::NetworkCongestionCost::getCost(PlacementMatrix candidate)
{
    ...
    for (uint32_t rowIdx = 0; rowIdx < candidate.size(); rowIdx++)
    {
        ...
        totalNetworkCongestion += std::max((float)0, (getNodeOutputRate(candidate, rowIdx) - stubGetNodeOutputBandwidth(currentWorkerId)));
    }

    return totalNetworkCongestion;
}

NES::OperatorId NES::Optimizer::NetworkCongestionCost::stubGetOperatorIdFromCandidateEntry(
    [[maybe_unused]] const NES::PlacementMatrix& candidate, [[maybe_unused]] uint32_t row, [[maybe_unused]] uint32_t col)
{
...
NES::WorkerId NES::Optimizer::NetworkCongestionCost::stubGetExecutionNodeFromCandidateEnetry(
    [[maybe_unused]] const NES::PlacementMatrix& candidate, [[maybe_unused]] uint32_t rowIdx)
{
