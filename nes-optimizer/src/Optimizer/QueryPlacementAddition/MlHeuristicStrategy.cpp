...
std::unique_ptr<BasePlacementAdditionStrategy> MlHeuristicStrategy::create(
...
void MlHeuristicStrategy::identifyPinningLocation(
  ...
  if (!logicalOperator->hasProperty(PINNED_WORKER_ID)) {
    ...
    if ((logicalOperator->hasMultipleChildrenOrParents() &&
      ...
      if (childTopologyNodes.empty()) {
        NES_WARNING(
            "No topology node isOperatorAPinnedDownStreamOperator where child "
            "operators are "
            "placed.");
        return;
      }
      ...
      NES_TRACE(
          "Find a node reachable from all topology nodes where child operators "
          "are placed.");
  ...
  if (isOperatorAPinnedDownStreamOperator != pinnedDownStreamOperators.end()) {
    NES_DEBUG(
        "Found pinned downstream operator. Skipping placement of further "
        "operators.");
    return;
  }
