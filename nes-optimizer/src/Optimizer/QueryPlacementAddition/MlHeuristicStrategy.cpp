...
std::unique_ptr<BasePlacementAdditionStrategy>
MlHeuristicStrategy::create(const GlobalExecutionPlanPtr &globalExecutionPlan,
                            const TopologyPtr &topology,
                            const TypeInferencePhasePtr &typeInferencePhase,
                            PlacementAmendmentMode placementAmendmentMode) {
  return std::make_unique<MlHeuristicStrategy>(
      MlHeuristicStrategy(globalExecutionPlan, topology, typeInferencePhase,
                          placementAmendmentMode));
}

MlHeuristicStrategy::MlHeuristicStrategy(
...
PlacementAdditionResult MlHeuristicStrategy::updateGlobalExecutionPlan(
  ...
  try {
    ...
    performOperatorPlacement(sharedQueryId, copy.copiedPinnedUpStreamOperators,
                             copy.copiedPinnedDownStreamOperators);
...
void MlHeuristicStrategy::performOperatorPlacement(
    SharedQueryId sharedQueryId,
    const std::set<LogicalOperatorPtr> &pinnedUpStreamOperators,
    const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators) {
  ...
  for (auto &pinnedUpStreamOperator : pinnedUpStreamOperators) {
    ...
    auto workerId = std::any_cast<WorkerId>(
        pinnedUpStreamOperator->getProperty(PINNED_WORKER_ID));
    TopologyNodePtr candidateTopologyNode = getTopologyNode(workerId);

    // 1. If pinned up stream node was already placed then place all its
    ...
    if (pinnedUpStreamOperator->getOperatorState() == OperatorState::PLACED) {
      ...
      for (auto &downStreamNode : pinnedUpStreamOperator->getParents()) {
        identifyPinningLocation(
            sharedQueryId, downStreamNode->as<LogicalOperator>(),
            candidateTopologyNode, pinnedDownStreamOperators);
      }
    } else { // 2. If pinned operator is not placed then start by placing the
             // operator
      if (candidateTopologyNode->getAvailableResources() == 0) {
        NES_ERROR("Unable to find resources on the physical node for placement "
                  "of source operator");
        throw Exceptions::RuntimeException(
            "Unable to find resources on the physical node for placement of "
            "source operator");
      }
      identifyPinningLocation(sharedQueryId, pinnedUpStreamOperator,
                              candidateTopologyNode, pinnedDownStreamOperators);
    }
  }
...
bool MlHeuristicStrategy::pushUpBasedOnFilterSelectivity(
...
void MlHeuristicStrategy::identifyPinningLocation(
    SharedQueryId sharedQueryId, const LogicalOperatorPtr &logicalOperator,
  ...
  if (!logicalOperator->hasProperty(PINNED_WORKER_ID)) {
    ...
    if ((logicalOperator->hasMultipleChildrenOrParents() &&
      ...
      std::vector<TopologyNodePtr> childTopologyNodes =
          getTopologyNodesForChildrenOperators(logicalOperator);
      if (childTopologyNodes.empty()) {
        NES_WARNING("No topology node isOperatorAPinnedDownStreamOperator "
                    "where child operators are "
                    "placed.");
        return;
      }

      NES_TRACE("Find a node reachable from all topology nodes where child "
                "operators are placed.");
      if (childTopologyNodes.size() == 1) {
        candidateTopologyNode = childTopologyNodes[0];
      } else {
        candidateTopologyNode =
            pathFinder->findCommonAncestor(childTopologyNodes);
      }
      if (!candidateTopologyNode) {
        NES_ERROR("Unable to find a common ancestor topology node to place the "
                  "binary operator, "
                  "operatorId: {}",
                  logicalOperator->getId());
        topology->print();
        throw Exceptions::RuntimeException(
            "Unable to find a common ancestor topology node to place the "
            "binary operator");
      }
      ...
      if (logicalOperator->instanceOf<SinkLogicalOperator>()) {
        ...
        if (pinnedSinkOperatorLocation->getId() ==
          ...
          NES_ERROR("Unexpected behavior. Could not find Topology node where "
                    "sink operator is to be "
                    "placed.");
          throw Exceptions::RuntimeException(

              "Unexpected behavior. Could not find Topology node where sink "
              "operator is to be "
              "placed.");
        }

        if (candidateTopologyNode->getAvailableResources() == 0) {
          NES_ERROR("Topology node where sink operator is to be placed has no "
                    "capacity.");
          throw Exceptions::RuntimeException(
              "Topology node where sink operator is to be placed has no "
              "capacity.");
        }
      }
    }
    ...
    bool tfNotInstalled =
        logicalOperator->instanceOf<InferModel::LogicalInferModelOperator>() &&
        (!candidateTopologyNode->hasNodeProperty("tf_installed") ||
         !std::any_cast<bool>(
             candidateTopologyNode->getNodeProperty("tf_installed")));
    if (!candidateTopologyNode ||
        candidateTopologyNode->getAvailableResources() == 0 || tfNotInstalled) {
    ...
    if (logicalOperator->instanceOf<InferModel::LogicalInferModelOperator>()) {
      ...
      if (candidateTopologyNode->hasNodeProperty("enable_cpu_saver_mode")) {
        ENABLE_CPU_SAVER_MODE = std::any_cast<bool>(
            candidateTopologyNode->getNodeProperty("enable_cpu_saver_mode"));
      }
      if (candidateTopologyNode->hasNodeProperty("min_resource_limit")) {
        MIN_RESOURCE_LIMIT = std::any_cast<int>(
            candidateTopologyNode->getNodeProperty("min_resource_limit"));
      }
      if (candidateTopologyNode->hasNodeProperty("low_throughput_source")) {
        LOW_THROUGHPUT_SOURCE = std::any_cast<bool>(
            candidateTopologyNode->getNodeProperty("low_throughput_source"));
      }
      if (candidateTopologyNode->hasNodeProperty("ml_hardware")) {
        ML_HARDWARE = std::any_cast<bool>(
            candidateTopologyNode->getNodeProperty("ml_hardware"));
      }

      if (candidateTopologyNode->getAvailableResources() < MIN_RESOURCE_LIMIT &&
          ENABLE_CPU_SAVER_MODE) {
        shouldPushUp = true;
      }
    ...
    if (shouldPushUp) {
      identifyPinningLocation(
          sharedQueryId, logicalOperator,
          candidateTopologyNode->getParents()[0]->as<TopologyNode>(),
          pinnedDownStreamOperators);
    }
    if (!canBePlacedHere) {
      NES_ERROR("Operator can not be placed on {}",
                candidateTopologyNode->getId());
      return;
    }
    ...
    if (candidateTopologyNode->getAvailableResources() == 0) {
      ...
      NES_DEBUG(
          "Find the next NES node in the path where operator can be placed");
  ...
  auto isOperatorAPinnedDownStreamOperator = std::find_if(
      pinnedDownStreamOperators.begin(), pinnedDownStreamOperators.end(),
  ...
  if (isOperatorAPinnedDownStreamOperator != pinnedDownStreamOperators.end()) {
    NES_DEBUG("Found pinned downstream operator. Skipping placement of further "
              "operators.");
    return;
  }
  ...
  for (const auto &parent : logicalOperator->getParents()) {
    identifyPinningLocation(sharedQueryId, parent->as<LogicalOperator>(),
                            candidateTopologyNode, pinnedDownStreamOperators);
  }
}
