    ...
    const std::set<LogicalOperatorPtr>& pinnedDownStreamOperators)
{
        ...
        || logicalOperator->instanceOf<SinkLogicalOperator>())
    {
        ...
        {
            NES_ERROR(
                "Unable to find a common ancestor topology node to place the binary operator, operatorId: {}", logicalOperator->getId());
            topology->print();
        ...
        if (logicalOperator->instanceOf<SinkLogicalOperator>())
        {
            ...
            else
            {
                NES_ERROR("Unexpected behavior. Could not find Topology node where sink operator is to be "
                          "placed.");
                throw Exceptions::RuntimeException("Unexpected behavior. Could not find Topology node where sink operator is to be "
                                                   "placed.");
            }
