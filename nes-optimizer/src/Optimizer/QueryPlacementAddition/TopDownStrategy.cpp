...
{
    return std::make_unique<TopDownStrategy>(TopDownStrategy(globalExecutionPlan, topology, typeInferencePhase, placementAmendmentMode));
}
    ...
    const std::set<LogicalOperatorPtr> & pinnedUpStreamOperators, const std::set<LogicalOperatorPtr> & pinnedDownStreamOperators)
{
    ...
    for (const auto & pinnedDownStreamOperator : pinnedDownStreamOperators)
    {
        ...
        { // 2. If pinned operator is not placed then start by placing the operator
    ...
    { // If operator is not pinned yet
            ...
            && !logicalOperator->instanceOf<SinkLogicalOperator>())
        {
            ...
            if (!candidateTopologyNode)
            {
                ...
                throw Exceptions::RuntimeException(
                    "Unable to find the candidate topology node for placing Nary operator " + logicalOperator->toString());
            }
            ...
            if (logicalOperator->instanceOf<SourceLogicalOperator>())
            {
                ...
                else
                {
                    NES_ERROR("Unexpected behavior. Could not find Topology node where source operator is to be "
                              "placed.");
                    throw Exceptions::RuntimeException("Unexpected behavior. Could not find Topology node where source operator is "
                                                       "to be placed.");
                }
...
std::vector<TopologyNodePtr> TopDownStrategy::getTopologyNodesForDownStreamOperators(const LogicalOperatorPtr & candidateOperator)
{
    ...
    for (auto & downstreamOperator : downstreamOperators)
    {
        ...
        TopologyNodePtr parentTopologyNode
            = workerIdToTopologyNodeMap[std::any_cast<WorkerId>(downstreamOperator->as_if<Operator>()->getProperty(PINNED_WORKER_ID))];
...
std::vector<TopologyNodePtr> TopDownStrategy::getTopologyNodesForUpStreamOperators(const LogicalOperatorPtr & candidateOperator)
{
    ...
    NES_TRACE("TopDownStrategy::getTopologyNodesForUpStreamOperators: Get the pinned or closest placed upStreamOperators nodes for "
    ...
    while (!upStreamOperators.empty())
    {
        ...
        upStreamOperators.insert(upStreamOperators.end(), upStreamOperator->getChildren().begin(), upStreamOperator->getChildren().end());
    }
