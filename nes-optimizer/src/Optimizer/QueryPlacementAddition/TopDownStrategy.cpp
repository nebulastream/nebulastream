...
BasePlacementStrategyPtr TopDownStrategy::create(
...
void TopDownStrategy::identifyPinningLocation(
  ...
  if (logicalOperator->getOperatorState() == OperatorState::PLACED) {
    NES_DEBUG(
        "Operator is already placed and thus skipping placement of this and "
        "its down stream operators.");
    return;
  }
  ...
  if (!logicalOperator->hasProperty(
    ...
    if ((logicalOperator->hasMultipleChildrenOrParents() ||
      ...
      NES_TRACE(
          "Get the topology nodes where children source operators are to be "
          "placed.");
  ...
  if (isOperatorAPinnedUpStreamOperator != pinnedUpStreamOperators.end()) {
    NES_DEBUG(
        "BottomUpStrategy: Found pinned downstream operator. Skipping "
...
TopDownStrategy::getTopologyNodesForDownStreamOperators(
  ...
  for (auto& downstreamOperator : downstreamOperators) {
    if (!downstreamOperator->as<Operator>()->hasProperty(PROCESSED)) {
      NES_WARNING(
          "Not all downstream operators are processed for placement yet. "
          "Placement of this "
...
TopDownStrategy::getTopologyNodesForUpStreamOperators(
  ...
  NES_TRACE(
      "TopDownStrategy::getTopologyNodesForUpStreamOperators: Get the pinned "
      "or closest placed upStreamOperators nodes for "
