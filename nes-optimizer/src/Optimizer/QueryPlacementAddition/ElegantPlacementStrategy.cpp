...
PlacementAdditionResult ElegantPlacementStrategy::updateGlobalExecutionPlan(
  ...
  try {
    ...
    NES_INFO("Sending placement request to ELEGANT planner with payload: "
             "url={}, payload={}",
    ...
    if (response.status_code != 200) {
      throw Exceptions::QueryPlacementAdditionException(
          sharedQueryId, "ElegantPlacementStrategy::updateGlobalExecutionPlan: "
                         "Error in call to Elegant planner with code " +
                             std::to_string(response.status_code) +
                             " and msg " + response.reason);
    }
...
void ElegantPlacementStrategy::addJavaUdfByteCodeField(
  ...
  if (logicalOperator->instanceOf<MapUDFLogicalOperator>() ||
    ...
    const auto *udfDescriptor =
        dynamic_cast<Catalogs::UDF::JavaUDFDescriptor *>(
            logicalOperator->as<UDFLogicalOperator>()
                ->getUDFDescriptor()
                .get());
