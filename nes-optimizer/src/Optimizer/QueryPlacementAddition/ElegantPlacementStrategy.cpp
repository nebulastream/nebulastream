...
BasePlacementStrategyPtr ElegantPlacementStrategy::create(
    const std::string& serviceURL, PlacementStrategy placementStrategy,
  ...
  switch (placementStrategy) {
    ...
    case PlacementStrategy::ELEGANT_BALANCED:
      ...
      break;
    default:
      NES_ERROR("Unknown placement strategy for elegant {}",
                magic_enum::enum_name(placementStrategy));
  }

  return std::make_unique<ElegantPlacementStrategy>(ElegantPlacementStrategy(
      serviceURL, timeWeight, globalExecutionPlan, topology, typeInferencePhase,
      placementAmendmentMode));
}

ElegantPlacementStrategy::ElegantPlacementStrategy(
    const std::string& serviceURL, const float timeWeight,
    const GlobalExecutionPlanPtr& globalExecutionPlan,
    const TopologyPtr& topology,
    const TypeInferencePhasePtr& typeInferencePhase,
    PlacementAmendmentMode placementAmendmentMode)
    : BasePlacementAdditionStrategy(globalExecutionPlan, topology,
                                    typeInferencePhase, placementAmendmentMode),
...
PlacementAdditionResult ElegantPlacementStrategy::updateGlobalExecutionPlan(
  ...
  try {
    NES_ASSERT(serviceURL != EMPTY_STRING,
               "ELEGANT planner URL is not set in elegant.plannerServiceURL");
    nlohmann::json payload{};
    prepareQueryPayload(pinnedUpStreamOperators, pinnedDownStreamOperators,
                        payload);
    prepareTopologyPayload(payload);
    payload[TIME_WEIGHT_KEY] = timeWeight;
    NES_INFO(
        "Sending placement request to ELEGANT planner with payload: url={}, "
        "payload={}",
        serviceURL, payload.dump());
    cpr::Response response = cpr::Post(
        cpr::Url{serviceURL}, cpr::Header{{"Content-Type", "application/json"}},
        cpr::Body{payload.dump()}, cpr::Timeout(ELEGANT_SERVICE_TIMEOUT));
    if (response.status_code != 200) {
      throw Exceptions::QueryPlacementAdditionException(
          sharedQueryId,
          "ElegantPlacementStrategy::updateGlobalExecutionPlan: Error in call "
          "to Elegant planner with code " +
    ...
    pinOperatorsBasedOnElegantService(
        sharedQueryId, copy.copiedPinnedDownStreamOperators, response);
...
void ElegantPlacementStrategy::pinOperatorsBasedOnElegantService(
  ...
  for (const auto& placement : placementData) {
    ...
    for (const auto& item : pinnedDownStreamOperators) {
      auto operatorToPin =
          item->getChildWithOperatorId(operatorId)->as<Operator>();
      if (operatorToPin) {
    ...
    if (!pinned) {
      throw Exceptions::QueryPlacementAdditionException(
          sharedQueryId, fmt::format("Unable to find operator with id {} in "
...
void ElegantPlacementStrategy::addJavaUdfByteCodeField(
    const OperatorPtr& logicalOperator, nlohmann::json& node) {
  if (logicalOperator->instanceOf<MapUDFLogicalOperator>() ||
      logicalOperator->instanceOf<FlatMapUDFLogicalOperator>()) {
    const auto* udfDescriptor = dynamic_cast<Catalogs::UDF::JavaUDFDescriptor*>(
        logicalOperator->as<UDFLogicalOperator>()->getUDFDescriptor().get());
    const auto& byteCode = udfDescriptor->getByteCodeList();
    std::vector<std::pair<std::string, std::string>> base64ByteCodeList;
    std::transform(
        byteCode.cbegin(), byteCode.cend(),
        std::back_inserter(base64ByteCodeList),
        [](const jni::JavaClassDefinition& classDefinition) {
          return std::pair<std::string, std::string>{
              classDefinition.first,
              base64_encode(std::string(classDefinition.second.data(),
                                        classDefinition.second.size()))};
        });
...
void ElegantPlacementStrategy::prepareQueryPayload(
  ...
  while (!operatorsToVisit.empty()) {
    auto logicalOperator = operatorsToVisit.front();  // fetch the front
                                                      // operator
    ...
    if (visitedOperator.insert(logicalOperator).second) {
      ...
      node[CONSTRAINT_KEY] =
          pinnedNodeId.has_value()
              ? std::any_cast<WorkerId>(pinnedNodeId).toString()
              : EMPTY_STRING;
      auto sourceCode = logicalOperator->getProperty(sourceCodeKey);
      node[sourceCodeKey] = sourceCode.has_value()
                                ? std::any_cast<std::string>(sourceCode)
                                : EMPTY_STRING;
      node[INPUT_DATA_KEY] =
          logicalOperator->getOutputSchema()->getSchemaSizeInBytes();
      ...
      // Only explore further upstream operators if this operator is not in the
      // list of pinned upstream operators
      if (found == pinnedUpStreamOperators.end()) {
        for (const auto& upstreamOperator : logicalOperator->getChildren()) {
          upstreamOperatorIds.push_back(
              upstreamOperator->as<Operator>()->getId());
          operatorsToVisit.emplace(
              upstreamOperator
                  ->as<Operator>());  // add children for future visit
        }
      }
