        ...
        if (computedDecomposedQueryPlans.contains(pinnedWorkerId)) {
            ...
            if (!newDecomposedQueryPlan) {
                if (pinnedOperator->getOperatorState() == OperatorState::PLACED) {
                    ...
                    newDecomposedQueryPlan =
                        DecomposedQueryPlan::create(INVALID_DECOMPOSED_QUERY_PLAN_ID, sharedQueryId, {copyOfPinnedOperator});
                } else {
...
bool BasePlacementAdditionStrategy::updateExecutionNodes(SharedQueryId sharedQueryId,
    ...
    for (const auto& workerNodeId : workerNodeIdsInBFS) {
        ...
        try {
            ...
            auto placedDecomposedQueryPlans = executionNode->getAllDecomposedQueryPlans(sharedQueryId);
            ...
            placedDecomposedQueryPlans = executionNode->getAllDecomposedQueryPlans(sharedQueryId);
            for (auto placedQuerySubPlan : placedDecomposedQueryPlans) {
...
bool BasePlacementAdditionStrategy::tryMergingNetworkSource(DecomposedQueryPlanVersion querySubPlanVersion,
    ...
    for (const auto& existingChild : placedDownstreamOperator->getChildren()) {
        ...
        if (!existingChild->instanceOf<SourceLogicalOperatorNode>()) {
            continue;
        }
        auto existingSource = existingChild->as<SourceLogicalOperatorNode>();
...
bool BasePlacementAdditionStrategy::tryMergingNetworkSink(DecomposedQueryPlanVersion querySubPlanVersion,
    ...
    for (const auto& existingParent : upstreamOperatorOfPlacedSinksToCheck->getParents()) {
        ...
        if (!existingParent->instanceOf<SinkLogicalOperatorNode>()) {
            continue;
        }
        auto existingSink = existingParent->as<SinkLogicalOperatorNode>();
