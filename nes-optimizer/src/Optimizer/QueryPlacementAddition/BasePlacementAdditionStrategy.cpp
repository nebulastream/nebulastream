...
void BasePlacementAdditionStrategy::performPathSelection(
    const std::set<LogicalOperatorPtr>& upStreamPinnedOperators, const std::set<LogicalOperatorPtr>& downStreamPinnedOperators)
{
    ...
    for (const auto& pinnedOperator : upStreamPinnedOperators)
    {
        ...
        if (!value.has_value())
        {
            throw Exceptions::RuntimeException(
                "LogicalSourceExpansionRule: Unable to find pinned node identifier for the logical operator " + pinnedOperator->toString());
        }
    ...
    const std::set<WorkerId>& topologyNodesWithDownStreamPinnedOperators)
{
    ...
    while (!success)
    {
        ...
        if (!sourceTopologyNodesInSelectedPath.empty())
        {
            ...
            std::for_each(
                sourceTopologyNodesInSelectedPath.begin(),
                sourceTopologyNodesInSelectedPath.end(),
                [&](const TopologyNodePtr& topologyNode) { topologyNodesInBFSOrder.push(topologyNode); });
            ...
            while (!topologyNodesInBFSOrder.empty())
            {
                ...
                std::for_each(
                    downstreamTopologyNodes.begin(),
                    downstreamTopologyNodes.end(),
                    [&](const NodePtr& topologyNode) { topologyNodesInBFSOrder.push(topologyNode->as<TopologyNode>()); });
            }
            success = true;
        }
    }
...
bool BasePlacementAdditionStrategy::unlockTopologyNodesInSelectedPath()
{
    ...
    std::for_each(
        workerNodeIdsInBFS.rbegin(),
        workerNodeIdsInBFS.rend(),
    ...
    const std::set<WorkerId>& topologyNodesWithDownStreamPinnedOperators)
{
    ...
    std::vector upstreamTopologyNodes(topologyNodesWithUpStreamPinnedOperators.begin(), topologyNodesWithUpStreamPinnedOperators.end());
    std::vector downstreamTopologyNodes(
        topologyNodesWithDownStreamPinnedOperators.begin(), topologyNodesWithDownStreamPinnedOperators.end());
...
ComputedDecomposedQueryPlans BasePlacementAdditionStrategy::computeDecomposedQueryPlans(
    ...
    while (!pinnedOperators.empty())
    {
        ...
        if (computedDecomposedQueryPlans.contains(pinnedWorkerId))
        {
            ...
            for (const auto& existingDecomposedQueryPlan : existingDecomposedQueryPlans)
            {
                ...
                if (foundConnectedUpstreamPlacedOperator)
                { // If an upstream operator found in the placed query sub
                    ...
                    else
                    {
                        ...
                        std::for_each(
                            rootOperators.begin(),
                            rootOperators.end(),
                            [&](const auto& rootOperator) { newDecomposedQueryPlan->addRootOperator(rootOperator); });
                    }
                }
                else if (foundConnectedDownstreamPlacedOperator)
                { // If a downstream operator found in the placed query sub
                ...
                { // Else add the disjoint query sub plan to the updated query plan list
            ...
            if (!newDecomposedQueryPlan)
            {
                if (pinnedOperator->getOperatorState() == OperatorState::PLACED)
                {
                    ...
                    newDecomposedQueryPlan = DecomposedQueryPlan::create(
                        INVALID_DECOMPOSED_QUERY_PLAN_ID, sharedQueryId, pinnedWorkerId, {copyOfPinnedOperator});
                }
                else
                {
                    newDecomposedQueryPlan = DecomposedQueryPlan::create(
                        PlanIdGenerator::getNextDecomposedQueryPlanId(), sharedQueryId, pinnedWorkerId, {copyOfPinnedOperator});
                }
            }
            else if (pinnedOperator->getOperatorState() == OperatorState::PLACED)
            {
                newDecomposedQueryPlan->setDecomposedQueryPlanId(INVALID_DECOMPOSED_QUERY_PLAN_ID); // set invalid query sub plan id
            }
        ...
        { // 2.2.1. If no query sub plan placed on the pinned worker node
            ...
            if (pinnedOperator->getOperatorState() == OperatorState::PLACED)
            {
                ...
                newDecomposedQueryPlan
                    = DecomposedQueryPlan::create(INVALID_DECOMPOSED_QUERY_PLAN_ID, sharedQueryId, pinnedWorkerId, {copyOfPinnedOperator});
            }
            else
            {
                newDecomposedQueryPlan = DecomposedQueryPlan::create(
        ...
        if (pinnedOperator->getOperatorState() == OperatorState::PLACED)
        {
            ...
            const auto& isPinnedDownStreamOperator = std::find_if(
                ...
                [operatorId](const auto& pinnedDownStreamOperator) { return pinnedDownStreamOperator->getId() == operatorId; });
        ...
        for (const auto& decomposedQueryPlan : decomposedQueryPlans)
        {
            ...
            for (const auto& candidateOperator : candidateOperators)
            {
                ...
                for (const auto& upstreamOperator : originalCopiedOperator->getChildren())
                {
                    ...
                    NES_DEBUG(
                        "Num of nodes between topology node {} and {} is {}",
                    ...
                    if (!querySubPlanWithUpstreamOperator)
                    {
                        ...
                        throw Exceptions::RuntimeException("Unable to find a query sub plan hosting the upstream operator to connect.");
                    }
                    ...
                    for (uint16_t pathIndex = 0; pathIndex < topologyNodesBetween.size(); pathIndex++)
                    {
                        ...
                        if (currentWorkerId == upstreamWorkerId)
                        {
                            ...
                            auto networkSinkOperator
                                = createNetworkSinkOperator(sharedQueryId, networkSourceOperatorId, topologyNodesBetween[pathIndex + 1]);
                        ...
                        else if (currentWorkerId == workerId)
                        {
                            ...
                            {
                                decomposedPlanId
                                    = std::any_cast<DecomposedQueryPlanId>(candidateOperator->getProperty(PLACED_DECOMPOSED_PLAN_ID));
                            }
                            else if (decomposedPlanId == INVALID_DECOMPOSED_QUERY_PLAN_ID)
                            {
                                ...
                                for (const auto& placedOperator : decomposedQueryPlan->getAllOperators())
                                {
                                    ...
                                    {
                                        decomposedPlanId
                                            = std::any_cast<DecomposedQueryPlanId>(placedOperator->getProperty(PLACED_DECOMPOSED_PLAN_ID));
                                        break;
                                    }
                                }
                            ...
                            auto networkSourceOperator = createNetworkSourceOperator(
                                sharedQueryId, sourceSchema, networkSourceOperatorId, topologyNodesBetween[pathIndex - 1]);
                        ...
                        else
                        {
                            ...
                            auto newDecomposedQueryPlan = DecomposedQueryPlan::create(
                                PlanIdGenerator::getNextDecomposedQueryPlanId(), sharedQueryId, currentWorkerId, {networkSinkOperator});
                    ...
                    downStreamOperatorToConnectedSysPlansMetaDataMap[downStreamNonSystemOperatorId] = connectedSysDecomposedPlanDetails;
                    operatorToConnectInMatchedPlan->addProperty(
                        CONNECTED_SYS_DECOMPOSED_PLAN_DETAILS, downStreamOperatorToConnectedSysPlansMetaDataMap);
                }
            }
        }
    }
...
PlacementAdditionResult BasePlacementAdditionStrategy::updateExecutionNodes(
    SharedQueryId sharedQueryId, ComputedDecomposedQueryPlans& computedSubQueryPlans, DecomposedQueryPlanVersion decomposedQueryPlanVersion)
{
            ...
            for (const auto& computedDecomposedQueryPlan : computedDecomposedQueryPlans)
            {
                ...
                if (computedDecomposedQueryPlan->getDecomposedQueryPlanId() == INVALID_DECOMPOSED_QUERY_PLAN_ID)
                {
                    if (containPinnedUpstreamOperator)
                    {
                        ...
                        auto decomposedQueryPlanComparator = [](const DecomposedQueryPlanPtr& first, const DecomposedQueryPlanPtr& second)
                                    ...
                                    if (matchingPlacedLeafOperator)
                                    {
                                        if (computedOperator->hasProperty(CONNECTED_SYS_DECOMPOSED_PLAN_DETAILS))
                                        {
                                            ...
                                            matchingPlacedLeafOperator->addProperty(
                                                CONNECTED_SYS_DECOMPOSED_PLAN_DETAILS, connectedSysSubPlanDetails);
                                        }
                                        else
                                        {
                                ...
                                for (const auto& placedDecomposedQueryPlan : placedDecomposedQueryPlans)
                                {
                                    ...
                                    if (matchingPinnedRootOperator)
                                    {
                                        ...
                                        for (const auto& upstreamOperator : computedOperator->getChildren())
                                        {
                                            ...
                                            {
                                                if (placedChild->as<Operator>()->getId() == upstreamOperator->as<Operator>()->getId())
                                                {
                                                    NES_DEBUG("Not modifying operator because it is a non system operator");
                                                    foundOperatorWithEqualId = true;
                                                }
                    ...
                    {
                        NES_ERROR(
                            "A decomposed query plan {} with invalid decomposed query plan id that has no pinned upstream "
        ...
        if (operatorIdToOriginalOperatorMap.contains(operatorId))
        {
            ...
            if (placedOperator->hasProperty(CONNECTED_SYS_DECOMPOSED_PLAN_DETAILS))
            {
                originalOperator->addProperty(
                    CONNECTED_SYS_DECOMPOSED_PLAN_DETAILS, placedOperator->getProperty(CONNECTED_SYS_DECOMPOSED_PLAN_DETAILS));
            }
        }
    }
}
    ...
    const SourceLogicalOperatorPtr& newNetworkSourceOperator)
{
    ...
    for (const auto& existingChild : placedDownstreamOperator->getChildren())
    {
        ...
        NES_ASSERT(
            existingSource->hasProperty(UPSTREAM_LOGICAL_OPERATOR_ID),
            "Network source does not have the UPSTREAM_LOGICAL_OPERATOR_ID property set");
    ...
    const SinkLogicalOperatorPtr& newNetworkSinkOperator)
{
    ...
    for (const auto& existingParent : upstreamOperatorOfPlacedSinksToCheck->getParents())
    {
        ...
        NES_ASSERT(
            existingSink->hasProperty(DOWNSTREAM_LOGICAL_OPERATOR_ID),
            "Network sink does not have the DOWNSTREAM_LOGICAL_OPERATOR_ID property set");
...
TopologyNodePtr BasePlacementAdditionStrategy::getTopologyNode(WorkerId workerId)
{
    ...
    if (!workerIdToTopologyNodeMap.contains(workerId))
    {
        ...
        throw Exceptions::RuntimeException(
            "Topology node with id " + workerId.toString() + " not considered during the path selection phase.");
    }
...
LogicalOperatorPtr BasePlacementAdditionStrategy::createNetworkSinkOperator(
    SharedQueryId sharedQueryId, OperatorId sourceOperatorId, const TopologyNodePtr& sourceTopologyNode)
{
    ...
    Network::NesPartition nesPartition(sharedQueryId, sourceOperatorId, Network::DEFAULT_PARTITION_ID, Network::DEFAULT_SUBPARTITION_ID);
    ...
    return LogicalOperatorFactory::createSinkOperator(
        Network::NetworkSinkDescriptor::create(nodeLocation, nesPartition, SINK_RETRY_WAIT, SINK_RETRIES, sinkVersion, numberOfOrigins, id),
        INVALID_WORKER_NODE_ID,
        id);
}

LogicalOperatorPtr BasePlacementAdditionStrategy::createNetworkSourceOperator(
    SharedQueryId sharedQueryId, SchemaPtr inputSchema, OperatorId operatorId, const TopologyNodePtr& sinkTopologyNode)
{
    ...
    Network::NodeLocation upstreamNodeLocation(
        sinkTopologyNode->getId(), sinkTopologyNode->getIpAddress(), sinkTopologyNode->getDataPort());
    ...
    const SourceDescriptorPtr& networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
        std::move(inputSchema), nesPartition, upstreamNodeLocation, SOURCE_RETRY_WAIT, SOURCE_RETRIES, 0, operatorId);
    return LogicalOperatorFactory::createSourceOperator(networkSourceDescriptor, operatorId);
}

std::vector<TopologyNodePtr> BasePlacementAdditionStrategy::getTopologyNodesForChildrenOperators(const LogicalOperatorPtr& operatorNode)
{
    ...
    for (auto& child : children)
    {
        ...
        auto existingNode = std::find_if(
            childTopologyNodes.begin(),
            childTopologyNodes.end(),
            [&childTopologyNode](const auto& node) { return node->getId() == childTopologyNode->getId(); });
