...
BasePlacementAdditionStrategy::BasePlacementAdditionStrategy(
    ...
    : globalExecutionPlan(globalExecutionPlan), topology(topology),
...
void BasePlacementAdditionStrategy::performPathSelection(
    const std::set<LogicalOperatorPtr> &upStreamPinnedOperators,
  ...
  for (const auto &pinnedOperator : upStreamPinnedOperators) {
    ...
    if (!value.has_value()) {
      throw Exceptions::RuntimeException(
          "LogicalSourceExpansionRule: Unable to find pinned node identifier "
          "for the logical operator " +
          pinnedOperator->toString());
    }
    WorkerId workerId = std::any_cast<WorkerId>(value);
    // NOTE: Add the physical node to the set (we used set here to prevent
    // inserting duplicate physical node in-case of self join or
  ...
  case PlacementAmendmentMode::PESSIMISTIC: {
    success = pessimisticPathSelection(pinnedUpStreamTopologyNodeIds,
                                       pinnedDownStreamTopologyNodeIds);
    break;
  }
  case PlacementAmendmentMode::OPTIMISTIC: {
    success = optimisticPathSelection(pinnedUpStreamTopologyNodeIds,
                                      pinnedDownStreamTopologyNodeIds);
    break;
  }
  }

  // 4. Raise exception if unable to select and lock all topology nodes in the
  // path
  if (!success) {
...
bool BasePlacementAdditionStrategy::optimisticPathSelection(
  ...
  // 1. Perform path selection and if failure than use the exponential back-off
  // and retry strategy
  while (!success && retryCount < MAX_PATH_SELECTION_RETRIES) {
    ...
    if (!sourceTopologyNodesInSelectedPath.empty()) {
      ...
      while (!topologyNodesInBFSOrder.empty()) {
        ...
        std::for_each(
            downstreamTopologyNodes.begin(), downstreamTopologyNodes.end(),
    ...
    // 1.5. If unable to lock the topology nodes then wait for other processes
    // to release locks on the topology node
    if (!success) {
      NES_WARNING("Unable to lock topology nodes in the path. Waiting for the "
                  "process to release locks and retry "
                  "path selection.");
...
bool BasePlacementAdditionStrategy::pessimisticPathSelection(
  ...
  while (!success && retryCount < MAX_PATH_SELECTION_RETRIES) {
    ...
    if (!sourceTopologyNodesInSelectedPath.empty()) {
      // 1.2 Lock the selected topology nodes exclusively and create a topology
      // map
      success =
          lockTopologyNodesInSelectedPath(sourceTopologyNodesInSelectedPath);
    }

    // 1.3 If unable to lock the topology nodes then wait for other processes to
    // release locks on the topology node
    if (!success) {
      NES_WARNING("Unable to lock topology nodes in the path. Waiting for the "
                  "process to release locks and retry "
                  "path selection.");
...
bool BasePlacementAdditionStrategy::lockTopologyNodesInSelectedPath(
    const std::vector<TopologyNodePtr> &sourceTopologyNodes) {
  workerIdToTopologyNodeMap.clear();
  // Temp container for iteration
  std::queue<TopologyNodePtr> topologyNodesInBFSOrder;
  // Iterate topology nodes in a true breadth first order
  // Initialize with the upstream nodes
  std::for_each(sourceTopologyNodes.begin(), sourceTopologyNodes.end(),
                [&](const TopologyNodePtr &topologyNode) {
                  topologyNodesInBFSOrder.push(topologyNode);
                });
  ...
  while (!topologyNodesInBFSOrder.empty()) {
    ...
    if (!lock) {
      NES_ERROR(
          "Unable to Lock the topology node {} selected in the path selection.",
          idOfTopologyNodeToLock);
...
bool BasePlacementAdditionStrategy::unlockTopologyNodesInSelectedPath() {
  ...
  std::for_each(workerNodeIdsInBFS.rbegin(), workerNodeIdsInBFS.rend(),
                [&](const WorkerId &lockedTopologyNodeId) {
...
std::vector<TopologyNodePtr> BasePlacementAdditionStrategy::findPath(
    const std::set<WorkerId> &topologyNodesWithUpStreamPinnedOperators,
    const std::set<WorkerId> &topologyNodesWithDownStreamPinnedOperators) {

  // 1 Create the pinned upstream and downstream topology node vector
  std::vector upstreamTopologyNodes(
      topologyNodesWithUpStreamPinnedOperators.begin(),
      topologyNodesWithUpStreamPinnedOperators.end());
  std::vector downstreamTopologyNodes(
      topologyNodesWithDownStreamPinnedOperators.begin(),
      topologyNodesWithDownStreamPinnedOperators.end());

  // 2 Do the path selection
  //  NOTE: this call can be replaced with any arbitrary path selection
  //  algorithm
  return topology->findPathBetween(upstreamTopologyNodes,
                                   downstreamTopologyNodes);
}

ComputedDecomposedQueryPlans
BasePlacementAdditionStrategy::computeDecomposedQueryPlans(
    SharedQueryId sharedQueryId,
    const std::set<LogicalOperatorPtr> &pinnedUpStreamOperators,
    const std::set<LogicalOperatorPtr> &pinnedDownStreamOperators) {

  // The trailing logic iterates over the completely pinned query plan in strict
  // BFS order. Starting from the upstream to downstream operators. For example,
  // a shared query plan with id 1:
  //
  // OP1[N1] --> OP2[N2]                         --> OP7[N2] --> OP8[N3] -->
  // OP9[N3]
  //                     \                     /
  //                      -> OP5[N2] -> OP6[N2]
  //                     /                     \
  ...
  //
  // The strict BFS iteration order will be: OP1, OP3, OP2, OP4, OP5, OP6, OP7,
  // OP10, OP8, OP9
  //
  // During the BFS iteration it computes query sub plans that need to be placed
  // on the execution nodes. It starts with query sub plans containing only the
  // upstream operators and then append to them the next downstream operator. In
  // the above example following will be the output after each iteration.
  //
  ...
  //      N1: {(1, INVALID, OP1-->OP2), (1, INVALID, OP3)}
  //      N2: {(1, 3, {OP2,OP4}->OP5->OP6->OP7)}
  //      N3: {(1, INVALID, OP10), (1, INVALID, OP8->OP9)} //Note: shared query
  //      plan id was changed to INVALID id as pinned operator found
  //    }
  ...
  std::for_each(
      ...
      [&](const auto &operatorNode) { pinnedOperators.push(operatorNode); });
  ...
  while (!pinnedOperators.empty()) {
    ...
    auto copyOfPinnedOperator =
        pinnedOperator->copy()
            ->as<LogicalOperator>(); // Make a copy of the operator
    auto pinnedWorkerId =
        std::any_cast<WorkerId>(pinnedOperator->getProperty(PINNED_WORKER_ID));
    ...
    // 2.2. If the pinnedWorkerId has already placed query sub plans then
    // compute an updated list of query sub plans
    if (computedDecomposedQueryPlans.contains(pinnedWorkerId)) {
      // 2.2.1. Prepare an updated list of query sub plans and a new placed
      // query plan for the operator under consideration
      std::vector<DecomposedQueryPlanPtr> updatedDecomposedQueryPlans;
      DecomposedQueryPlanPtr newDecomposedQueryPlan;

      // 2.2.2. Iterate over all already placed query sub plans to check if the
      // iterated operator contains the root operator of already placed query
      // sub plan as its upstream operator
      auto existingDecomposedQueryPlans =
          computedDecomposedQueryPlans[pinnedWorkerId];
      for (const auto &existingDecomposedQueryPlan :
           existingDecomposedQueryPlans) {

        // 2.2.2.1. For all upstream operators of the pinned operator check if
        // they are in the placed query sub plan
        ...
        for (const auto &childOperator : pinnedOperator->getChildren()) {
          ...
          if (existingDecomposedQueryPlan->hasOperatorWithId(childOperatorId)) {
            auto placedOperator =
                existingDecomposedQueryPlan->getOperatorWithOperatorId(
                    childOperatorId);
            // Remove the placed operator as the root
            existingDecomposedQueryPlan->removeAsRootOperator(
                placedOperator->getId());
            // Add the copy as the parent of the iterated root operator and
            // update the root of the iterated
            placedOperator->addParent(copyOfPinnedOperator);
            foundConnectedUpstreamPlacedOperator = true;
          }
        }

        // 2.2.2.2. For all downstream operators of the pinned operator check if
        // they are in the placed query sub plan
        ...
        for (const auto &parentOperator : pinnedOperator->getParents()) {
          ...
          if (existingDecomposedQueryPlan->hasOperatorWithId(
                  parentOperatorId)) {
            auto placedOperator =
                existingDecomposedQueryPlan->getOperatorWithOperatorId(
                    parentOperatorId);
            ...
            // Check if all upstream operators of the considered pinnedOperator
            // are co-located on same node We will use this information to
            // compute network source and sink operators
            auto actualPlacedOperator =
                operatorIdToCopiedOperatorMap[placedOperator->getId()];
            if (actualPlacedOperator->getChildren().size() ==
                placedOperator->getChildren().size()) {
        ...
        if (foundConnectedUpstreamPlacedOperator) { // If an upstream operator
                                                    // found in the placed query
                                                    // sub plan then create a
                                                    // new or merge the placed
                                                    // query plan with the new
                                                    // query sub plan.
          if (!newDecomposedQueryPlan) {
            newDecomposedQueryPlan = DecomposedQueryPlan::create(
                existingDecomposedQueryPlan->getDecomposedQueryPlanId(),
                sharedQueryId, pinnedWorkerId,
            ...
            const auto &rootOperators =
                existingDecomposedQueryPlan->getRootOperators();
            std::for_each(rootOperators.begin(), rootOperators.end(),
                          [&](const auto &rootOperator) {
                            newDecomposedQueryPlan->addRootOperator(
                                rootOperator);
                          });
          }
        } else if (foundConnectedDownstreamPlacedOperator) { // If a downstream
                                                             // operator found
                                                             // in the placed
                                                             // query sub plan
                                                             // then create a
          ...
          if (!newDecomposedQueryPlan) {
            ...
            newDecomposedQueryPlan->removeAsRootOperator(
                copyOfPinnedOperator->getId());
            std::for_each(rootOperators.begin(), rootOperators.end(),
                          [&](const auto &rootOperator) {
                            newDecomposedQueryPlan->addRootOperator(
                                rootOperator);
        ...
        } else { // Else add the disjoint query sub plan to the updated query
                 // plan list
          updatedDecomposedQueryPlans.emplace_back(existingDecomposedQueryPlan);
        }
      }

      // 2.2.3. Create a new query sub plan for the operator under consideration
      // if all plans are disjoint.
      if (!newDecomposedQueryPlan) {
        if (pinnedOperator->getOperatorState() == OperatorState::PLACED) {
          // Create a temporary query sub plans for the operator as it might
          // need to be merged with another query sub plan that is already
          // placed on the execution node. Thus, we assign it an invalid sub
          // query plan id.
          newDecomposedQueryPlan = DecomposedQueryPlan::create(
              INVALID_DECOMPOSED_QUERY_PLAN_ID, sharedQueryId, pinnedWorkerId,
              {copyOfPinnedOperator});
        } else {
          newDecomposedQueryPlan = DecomposedQueryPlan::create(
              PlanIdGenerator::getNextDecomposedQueryPlanId(), sharedQueryId,
              pinnedWorkerId, {copyOfPinnedOperator});
        }
      ...
      computedDecomposedQueryPlans[pinnedWorkerId] =
          updatedDecomposedQueryPlans;
    } else { // 2.2.1. If no query sub plan placed on the pinned worker node
      // Create a new placed query sub plan
      DecomposedQueryPlanPtr newDecomposedQueryPlan;
      if (pinnedOperator->getOperatorState() == OperatorState::PLACED) {
        // Create a temporary query sub plans for the operator as it might need
        // to be merged with another query sub plan that is already placed on
        // the execution node. Thus, we assign it an invalid sub query plan id.
        newDecomposedQueryPlan = DecomposedQueryPlan::create(
            INVALID_DECOMPOSED_QUERY_PLAN_ID, sharedQueryId, pinnedWorkerId,
            {copyOfPinnedOperator});
      } else {
    ...
    // Check if all upstream operators of the considered pinnedOperator are
    // co-located on same node We will use this information to compute network
    // source and sink operators
    if (copyOfPinnedOperator->getChildren().size() ==
        pinnedOperator->getChildren().size()) {
      copyOfPinnedOperator->addProperty(CO_LOCATED_UPSTREAM_OPERATORS, true);
    }

    // 2.3. If the operator is in the state placed then check if it is one of
    // the pinned downstream operator
    if (pinnedOperator->getOperatorState() == OperatorState::PLACED) {
      // 2.3.1. Check if this operator in the pinned downstream operator list.
      const auto &isPinnedDownStreamOperator = std::find_if(
      ...
      // 2.3.2. If reached the pinned downstream operator then skip further
      // traversal
    ...
    std::for_each(downStreamOperators.begin(), downStreamOperators.end(),
                  [&](const NodePtr &operatorNode) {
                    // 2.5. Only add those operators that are to be processed as
                    // part of the given query plan. NOTE: It can happen that an
                    // operator is connected to multiple downstream operators
                    // but some of them are not to be considered during the
                    // placement.
                    const auto &downStreamOperator =
                        operatorNode->as<LogicalOperator>();
                    if (operatorIdToOriginalOperatorMap.contains(
...
void BasePlacementAdditionStrategy::addNetworkOperators(
    ComputedDecomposedQueryPlans &computedDecomposedQueryPlans) {

  // Iterate over all computed query sub plans and then add network source and
  // sink operators
  //
  // Input:
  // {
  //    N1: {(1, INVALID, OP1-->OP2), (1, INVALID, OP3)}
  //    N2: {(1, 3, {OP2,OP4}->OP5->OP6->OP7)}
  //    N3: {(1, INVALID, OP9), (1, INVALID, OP8->OP9)} //Note: shared query
  //    plan id was changed to INVALID id as pinned operator found
  // }
  //
  // Updated Map:
  ...
  // Iterate over all computed sub query plans and add network source and sink
  // operators.
  for (const auto &[workerId, decomposedQueryPlans] :
       computedDecomposedQueryPlans) {
    ...
    for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
      ...
      // 2. Fetch all logical operators whose upstream logical operators are
      // missing. In the previous call, we stored this information by setting
      // the property CO_LOCATED_UPSTREAM_OPERATORS for all operators with
      // co-located upstream operators
      ...
      // 3. Iterate over all candidate operators and add network source sink
      // operators
      for (const auto &candidateOperator : candidateOperators) {

        // 4. Check if candidate operator in the state "Placed" or is of type
        // Network Source or Sink then skip the operation
        ...
        auto originalCopiedOperator =
            operatorIdToCopiedOperatorMap[candidateOperator->getId()];
        auto downStreamNonSystemOperatorId = candidateOperator->getId();

        // 5. For each candidate operator not in the state "Placed" find the
        // topology node hosting the immediate upstream logical operators.
        for (const auto &upstreamOperator :
             originalCopiedOperator->getChildren()) {

          // 6. Fetch the id of the topology node hosting the upstream operator
          // to connect.
          const auto &upstreamOperatorToConnect =
              upstreamOperator->as<LogicalOperator>();
          auto upstreamWorkerId = std::any_cast<WorkerId>(
              upstreamOperatorToConnect->getProperty(PINNED_WORKER_ID));
          auto upstreamNonSystemOperatorId = upstreamOperatorToConnect->getId();

          // 7. If both operators are co-located we do not need to add network
          // source or sink
          ...
          // 9. Find topology nodes connecting the iterated workerId and
          // pinnedUpstreamWorkerId.
          std::vector<TopologyNodePtr> topologyNodesBetween =
              pathFinder->findNodesBetween(upstreamTopologyNode,
                                           downstreamTopologyNode);

          DecomposedQueryPlanPtr querySubPlanWithUpstreamOperator;

          // 10. Fetch all query sub plans placed on the upstream node and find
          // the query sub plan hosting the upstream operator to connect.
          for (const auto &upstreamQuerySubPlan :
               computedDecomposedQueryPlans[upstreamWorkerId]) {
            if (upstreamQuerySubPlan->hasOperatorWithId(
                    upstreamOperatorToConnect->getId())) {
          ...
          if (!querySubPlanWithUpstreamOperator) {
            NES_ERROR("Unable to find a query sub plan hosting the upstream "
                      "operator to connect.");
            throw Exceptions::RuntimeException(
                "Unable to find a query sub plan hosting the upstream operator "
                "to connect.");
          }

          // 11. Get the schema and operator id for the source network operators
          auto sourceSchema = upstreamOperatorToConnect->as<LogicalOperator>()
                                  ->getOutputSchema();
          auto networkSourceOperatorId = getNextOperatorId();

          // compute a vector of sub plan id and worker node id where the system
          // generated plans are placed
          std::vector<SysPlanMetaData> connectedSysSubPlanDetails;
          OperatorPtr operatorToConnectInMatchedPlan;
          // 12. Starting from the upstream to downstream topology node and add
          // network sink source pairs.
          for (uint16_t pathIndex = 0; pathIndex < topologyNodesBetween.size();
               pathIndex++) {

            WorkerId currentWorkerId = topologyNodesBetween[pathIndex]->getId();

            if (currentWorkerId == upstreamWorkerId) {
              // 13. Add a network sink operator as new root operator to the
              // upstream operator to connect

              // 14. create network sink operator to attach.
              auto networkSinkOperator = createNetworkSinkOperator(
                  sharedQueryId, networkSourceOperatorId,
                  topologyNodesBetween[pathIndex + 1]);

              // 14. Find the upstream operator to connect in the matched query
              // sub plan
              // set properties on new sink
              networkSinkOperator->addProperty(UPSTREAM_LOGICAL_OPERATOR_ID,
                                               upstreamNonSystemOperatorId);
              networkSinkOperator->addProperty(DOWNSTREAM_LOGICAL_OPERATOR_ID,
                                               downStreamNonSystemOperatorId);
              operatorToConnectInMatchedPlan =
                  querySubPlanWithUpstreamOperator->getOperatorWithOperatorId(
                      upstreamOperatorToConnect->getId());
              operatorToConnectInMatchedPlan->addParent(networkSinkOperator);
              querySubPlanWithUpstreamOperator->removeAsRootOperator(
                  operatorToConnectInMatchedPlan->getId());
              querySubPlanWithUpstreamOperator->addRootOperator(
                  networkSinkOperator);
              ...
              DecomposedQueryPlanId decomposedPlanId =
                  decomposedQueryPlan->getDecomposedQueryPlanId();
              if (candidateOperator->as<LogicalOperator>()
                      ->getOperatorState() == OperatorState::PLACED) {
              ...
              auto networkSourceOperator = createNetworkSourceOperator(
                  sharedQueryId, sourceSchema, networkSourceOperatorId,
                  topologyNodesBetween[pathIndex - 1]);
              candidateOperator->addChild(networkSourceOperator);

              // set properties on new source
              networkSourceOperator->addProperty(UPSTREAM_LOGICAL_OPERATOR_ID,
                                                 upstreamNonSystemOperatorId);
              networkSourceOperator->addProperty(DOWNSTREAM_LOGICAL_OPERATOR_ID,
                                                 downStreamNonSystemOperatorId);
              break;
            } else {
              ...
              auto newDecomposedQueryPlan = DecomposedQueryPlan::create(
                  PlanIdGenerator::getNextDecomposedQueryPlanId(),
                  sharedQueryId, currentWorkerId, {networkSinkOperator});

              // 18. Record information about the query plan and worker id
              connectedSysSubPlanDetails.emplace_back(SysPlanMetaData(
                  newDecomposedQueryPlan->getDecomposedQueryPlanId(),
                  currentWorkerId));

              // 19. add the new query plan
              if (computedDecomposedQueryPlans.contains(currentWorkerId)) {
                computedDecomposedQueryPlans[currentWorkerId].emplace_back(
                    newDecomposedQueryPlan);
              } else {
                computedDecomposedQueryPlans[currentWorkerId] = {
                    newDecomposedQueryPlan};
              }
              networkSourceOperator->addProperty(UPSTREAM_LOGICAL_OPERATOR_ID,
                                                 upstreamNonSystemOperatorId);
          ...
          // 15. Add metadata about the plans and topology nodes hosting the
          // system generated operators.
          operatorToConnectInMatchedPlan->addProperty(
              CONNECTED_SYS_SUB_PLAN_DETAILS, connectedSysSubPlanDetails);
        }
      }
    }
  }
...
PlacementAdditionResult BasePlacementAdditionStrategy::updateExecutionNodes(
    SharedQueryId sharedQueryId,
    ComputedDecomposedQueryPlans &computedSubQueryPlans,
    DecomposedQueryPlanVersion decomposedQueryPlanVersion) {

  std::unordered_map<DecomposedQueryPlanId, DeploymentContextPtr>
      deploymentContexts;

  for (const auto &workerNodeId : workerNodeIdsInBFS) {

    TopologyNodeWLock lockedTopologyNode;
    // 1. If using optimistic strategy then, lock the topology node with the
    // workerId and perform the "validation" before continuing.
    ...
    const std::string &ipAddress =
        lockedTopologyNode->operator*()->getIpAddress();
    uint32_t grpcPort = lockedTopologyNode->operator*()->getGrpcPort();

    try {
      // 1.2. Perform validation by checking if we can occupy the resources the
      // operator placement algorithm reserved for placing the operators.
      ...
      if (!lockedTopologyNode->operator*()->occupySlots(consumedResources)) {
        NES_ERROR("Unable to occupy resources on the topology node {} to "
                  "successfully place operators.",
                  workerNodeId);
      ...
      for (const auto &computedDecomposedQueryPlan :
           computedDecomposedQueryPlans) {

        // 1.5.1. Perform the type inference phase on the updated query sub plan
        // and update execution node.
        if (computedDecomposedQueryPlan->getDecomposedQueryPlanId() ==
            INVALID_DECOMPOSED_QUERY_PLAN_ID) {
          if (containPinnedUpstreamOperator) {

            // Record all placed query decomposed query plans that host pinned
            // leaf operators
            auto decomposedQueryPlanComparator =
                [](const DecomposedQueryPlanPtr &first,
                   const DecomposedQueryPlanPtr &second) {
                  return first->getDecomposedQueryPlanId() >
                         second->getDecomposedQueryPlanId();
                };
            std::set<DecomposedQueryPlanPtr,
                     decltype(decomposedQueryPlanComparator)>
                hostDecomposedQueryPlans;

            auto placedDecomposedQueryPlans =
                globalExecutionPlan->getCopyOfAllDecomposedQueryPlans(
                    workerNodeId, sharedQueryId);

            // Iterate over all pinned leaf operators and find a host placed
            // query sub plan that hosts
            auto computedOperators =
                computedDecomposedQueryPlan->getAllOperators();
            for (const auto &computedOperator : computedOperators) {

              // If the pinned leaf operator is not of type logical source and
              // was already placed. Then find and merge the operator with query
              // sub plan containing the placed leaf operator
              if (computedOperator->as<LogicalOperator>()->getOperatorState() ==
                  OperatorState::PLACED) {

                // Flag indicating, if we found a placed decomposed query plan
                // that contains pinned operator
                bool found = false;

                // Find and merge with the shared query plan
                for (const auto &placedDecomposedQueryPlan :
                     placedDecomposedQueryPlans) {

                  // If the placed query sub plan contains the pinned upstream
                  // operator
                  auto matchingPlacedLeafOperator =
                      placedDecomposedQueryPlan->getOperatorWithOperatorId(
                          computedOperator->getId());
                  if (matchingPlacedLeafOperator) {
                    if (computedOperator->hasProperty(
                            CONNECTED_SYS_SUB_PLAN_DETAILS)) {
                      auto connectedSysSubPlanDetails =
                          computedOperator->getProperty(
                              CONNECTED_SYS_SUB_PLAN_DETAILS);
                      matchingPlacedLeafOperator->addProperty(
                          CONNECTED_SYS_SUB_PLAN_DETAILS,
                          connectedSysSubPlanDetails);
                    } else {
                      NES_WARNING("connected sys sub plan details not found");
                    }
                    // Add all newly computed pinned downstream operators to the
                    // matching placed leaf operator
                    auto pinnedDownstreamOperators =
                        computedOperator->getParents();
                    for (const auto &pinnedDownstreamOperator :
                         pinnedDownstreamOperators) {
                      bool foundOperatorWithEqualId = false;
                      // if the operator matches a placed operator that was not
                      // system generated, do not replace anything and leave the
                      // placed operator where it is
                      for (const auto &placedParent :
                           matchingPlacedLeafOperator->getParents()) {
                        if (placedParent->as<Operator>()->getId() ==
                            pinnedDownstreamOperator->as<Operator>()->getId()) {
                          NES_DEBUG("Not modifying operator because it is a "
                                    "non system operator");
                      ...
                      if (pinnedDownstreamOperator
                        ...
                        mergedOperator = tryMergingNetworkSink(
                            ...
                            pinnedDownstreamOperator
                                ->as<SinkLogicalOperator>());
                      }
                      if (!mergedOperator) {
                        pinnedDownstreamOperator->removeChild(computedOperator);
                        pinnedDownstreamOperator->addChild(
                            matchingPlacedLeafOperator);
                      }
                    }
                ...
                // If no query sub plan found that hosts the placed leaf
                // operator
                if (!found) {
                  NES_ERROR("Unable to find query sub plan hosting the placed "
                            "and pinned upstream operator.");
                  throw Exceptions::RuntimeException(
                      "Unable to find query sub plan hosting the placed and "
                      "pinned upstream operator.");
                }
              }
            }
            ...
            if (!hostDecomposedQueryPlans.empty()) {
              for (const auto &hostQuerySubPlan : hostDecomposedQueryPlans) {
                if (!updatedDecomposedQueryPlan) {
                  ...
                  for (const auto &hostRootOperators :
                       hostQuerySubPlan->getRootOperators()) {
                    updatedDecomposedQueryPlan->addRootOperator(
                        hostRootOperators);
                  }
                }
              }
            ...
            for (const auto &rootOperator :
                 computedDecomposedQueryPlan->getRootOperators()) {
              updatedDecomposedQueryPlan->addRootOperator(rootOperator);
            }

            // As we are updating an existing query sub plan we mark the plan
            // for re-deployment
            updatedDecomposedQueryPlan->setState(
                QueryState::MARKED_FOR_REDEPLOYMENT);
            updatedDecomposedQueryPlan =
                typeInferencePhase->execute(updatedDecomposedQueryPlan);
            updatedDecomposedQueryPlan->setVersion(decomposedQueryPlanVersion);
            // Add decomposed query plan to the global execution plan
            globalExecutionPlan->addDecomposedQueryPlan(
                lockedTopologyNode, updatedDecomposedQueryPlan);
            // 1.6. Update state and properties of all operators placed on the
            // execution node
            markOperatorsAsPlaced(workerNodeId, updatedDecomposedQueryPlan);
            // 1.7. Compute deployment context
            deploymentContexts[updatedDecomposedQueryPlan
                                   ->getDecomposedQueryPlanId()] =
                DeploymentContext::create(ipAddress, grpcPort,
                                          updatedDecomposedQueryPlan->copy());
              ...
              if (computedOperator->as<LogicalOperator>()->getOperatorState() ==
                ...
                for (const auto &placedDecomposedQueryPlan :
                  ...
                  if (matchingPinnedRootOperator) {
                    // Add all newly computed downstream operators to matching
                    // placed leaf operator
                    for (const auto &upstreamOperator :
                         computedOperator->getChildren()) {
                      bool foundOperatorWithEqualId = false;
                      // if the operator matches a placed operator that was not
                      // system generated, do not replace anything and leave the
                      // placed operator where it is
                      for (const auto &placedChild :
                           matchingPinnedRootOperator->getChildren()) {
                      ...
                      if (upstreamOperator
                        ...
                        mergedSource = tryMergingNetworkSource(
                            decomposedQueryPlanVersion,
            ...
            NES_ERROR("A decomposed query plan {} with invalid decomposed "
                      "query plan id that has no pinned upstream "
                      "or downstream operator.",
                      computedDecomposedQueryPlan->toString());
            throw Exceptions::RuntimeException(
                "A decomposed plan with invalid decomposed query plan id that "
                "has no "
                "pinned upstream or downstream operator.");
          }
        } else {
          auto updatedDecomposedQueryPlan =
              typeInferencePhase->execute(computedDecomposedQueryPlan);
          updatedDecomposedQueryPlan->setState(
      ...
      NES_ERROR("Exception occurred during pinned operator placement {}.",
                ex.what());
      throw ex;
    }
  }
...
void BasePlacementAdditionStrategy::markOperatorsAsPlaced(
    WorkerId workerId, DecomposedQueryPlanPtr decomposedQueryPlan) {
  ...
  for (const auto &placedOperator : allPlacedOperators) {
    ...
    // Set status to Placed and copy over metadata properties for all operators
    // considered in this placement
    if (operatorIdToOriginalOperatorMap.contains(operatorId)) {
      ...
      originalOperator->addProperty(PLACED_DECOMPOSED_PLAN_ID,
                                    decomposedQueryPlanId);
      if (placedOperator->hasProperty(CONNECTED_SYS_SUB_PLAN_DETAILS)) {
        originalOperator->addProperty(
            CONNECTED_SYS_SUB_PLAN_DETAILS,
...
bool BasePlacementAdditionStrategy::tryMergingNetworkSource(
  ...
  auto newNetworkSourceDescriptor =
      newSourceDescriptor->as<Network::NetworkSourceDescriptor>();

  NES_ASSERT(
      newNetworkSourceOperator->hasProperty(UPSTREAM_LOGICAL_OPERATOR_ID),
      "Network source does not have the UPSTREAM_LOGICAL_OPERATOR_ID property "
      "set");
  ...
  for (const auto &existingChild : placedDownstreamOperator->getChildren()) {
    ...
    if (!existingSourceDescriptor
             ->instanceOf<Network::NetworkSourceDescriptor>()) {
      continue;
    }
    auto existingNetworkSourceDescriptor =
        existingSourceDescriptor->as<Network::NetworkSourceDescriptor>();

    NES_ASSERT(existingSource->hasProperty(UPSTREAM_LOGICAL_OPERATOR_ID),
               "Network source does not have the UPSTREAM_LOGICAL_OPERATOR_ID "
               "property set");

    if (std::any_cast<OperatorId>(
            existingSource->getProperty(UPSTREAM_LOGICAL_OPERATOR_ID)) ==
        std::any_cast<OperatorId>(newNetworkSourceOperator->getProperty(
      ...
      auto mergedNetworkSourceDescriptor =
          Network::NetworkSourceDescriptor::create(
              ...
              newNetworkSourceDescriptor->getNodeLocation(), SOURCE_RETRY_WAIT,
              SOURCE_RETRIES, querySubPlanVersion,
...
bool BasePlacementAdditionStrategy::tryMergingNetworkSink(
    DecomposedQueryPlanVersion querySubPlanVersion,
  ...
  auto newNetworkSinkDescriptor =
      newSinkDescriptor->as<Network::NetworkSinkDescriptor>();

  NES_ASSERT(
      newNetworkSinkOperator->hasProperty(DOWNSTREAM_LOGICAL_OPERATOR_ID),
      "Network sink does not have the DOWNSTREAM_LOGICAL_OPERATOR_ID property "
      "set");

  bool replacedOperator = false;
  for (const auto &existingParent :
       upstreamOperatorOfPlacedSinksToCheck->getParents()) {
    ...
    auto existingNetworkSinkDescriptor =
        existingSinkDescriptor->as<Network::NetworkSinkDescriptor>();

    NES_ASSERT(existingSink->hasProperty(DOWNSTREAM_LOGICAL_OPERATOR_ID),
               "Network sink does not have the DOWNSTREAM_LOGICAL_OPERATOR_ID "
               "property set");

    // check if the new sink corresponds to a pleced sink that needs to be
    // reconfigured
    if (std::any_cast<OperatorId>(
            existingSink->getProperty(DOWNSTREAM_LOGICAL_OPERATOR_ID)) ==
        std::any_cast<OperatorId>(newNetworkSinkOperator->getProperty(
      ...
      auto mergedNetworkSinkDescriptor = Network::NetworkSinkDescriptor::create(
          ...
          newNetworkSinkDescriptor->getNesPartition(), SINK_RETRY_WAIT,
          SINK_RETRIES, querySubPlanVersion,
      ...
      computedQuerySubPlan->removeAsRootOperator(
          newNetworkSinkOperator->getId());
      replacedOperator = true;
      break;
    }
  }
...
TopologyNodePtr
BasePlacementAdditionStrategy::getTopologyNode(WorkerId workerId) {

  NES_TRACE("Get the topology node {}", workerId);
  if (!workerIdToTopologyNodeMap.contains(workerId)) {
    NES_ERROR("Topology node with id {} not considered during the path "
              "selection phase.",
              workerId);
    throw Exceptions::RuntimeException(
        "Topology node with id " + workerId.toString() +
...
LogicalOperatorPtr BasePlacementAdditionStrategy::createNetworkSinkOperator(
    SharedQueryId sharedQueryId, OperatorId sourceOperatorId,
  ...
  Network::NesPartition nesPartition(sharedQueryId, sourceOperatorId,
  ...
  return LogicalOperatorFactory::createSinkOperator(
      Network::NetworkSinkDescriptor::create(nodeLocation, nesPartition,
                                             SINK_RETRY_WAIT, SINK_RETRIES,
                                             sinkVersion, numberOfOrigins, id),
      INVALID_WORKER_NODE_ID, id);
}

LogicalOperatorPtr BasePlacementAdditionStrategy::createNetworkSourceOperator(
    SharedQueryId sharedQueryId, SchemaPtr inputSchema, OperatorId operatorId,
    const TopologyNodePtr &sinkTopologyNode) {
  NES_TRACE("create Network Source operator");
  NES_ASSERT2_FMT(sinkTopologyNode,
                  "Invalid sink node while placing query " << sharedQueryId);
  ...
  const Network::NesPartition nesPartition = Network::NesPartition(
      sharedQueryId, operatorId, Network::DEFAULT_PARTITION_ID,
      Network::DEFAULT_SUBPARTITION_ID);
  const SourceDescriptorPtr &networkSourceDescriptor =
      Network::NetworkSourceDescriptor::create(
          std::move(inputSchema), nesPartition, upstreamNodeLocation,
          SOURCE_RETRY_WAIT, SOURCE_RETRIES, 0, operatorId);
  return LogicalOperatorFactory::createSourceOperator(networkSourceDescriptor,
                                                      operatorId);
}

std::vector<TopologyNodePtr>
BasePlacementAdditionStrategy::getTopologyNodesForChildrenOperators(
  ...
  for (auto &child : children) {
    ...
    auto existingNode =
        std::find_if(childTopologyNodes.begin(), childTopologyNodes.end(),
                     [&childTopologyNode](const auto &node) {
  ...
  NES_DEBUG(
      "returning list of topology nodes where children operators are placed");
