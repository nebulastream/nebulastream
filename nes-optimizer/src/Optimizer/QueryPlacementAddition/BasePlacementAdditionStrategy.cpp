...
bool BasePlacementAdditionStrategy::lockTopologyNodesInSelectedPath(const std::vector<TopologyNodePtr> & sourceTopologyNodes)
{
    ...
    // Initialize with the upstream nodes
    std::for_each(
        sourceTopologyNodes.begin(),
        sourceTopologyNodes.end(),
        [&](const TopologyNodePtr & topologyNode) { topologyNodesInBFSOrder.push(topologyNode); });
                            ...
                            if (computedOperator->as<LogicalOperator>()->getOperatorState() == OperatorState::PLACED)
                            {
                                ...
                                for (const auto & placedDecomposedQueryPlan : placedDecomposedQueryPlans)
                                {
                                    ...
                                    if (matchingPlacedLeafOperator)
                                    {
                                        ...
                                        {
                                            auto connectedSysSubPlanDetails = computedOperator->getProperty(CONNECTED_SYS_SUB_PLAN_DETAILS);
...
void BasePlacementAdditionStrategy::markOperatorsAsPlaced(WorkerId workerId, DecomposedQueryPlanPtr decomposedQueryPlan)
{
    ...
    for (const auto & placedOperator : allPlacedOperators)
    {
        ...
        if (operatorIdToOriginalOperatorMap.contains(operatorId))
        {
            ...
            {
                originalOperator->addProperty(CONNECTED_SYS_SUB_PLAN_DETAILS, placedOperator->getProperty(CONNECTED_SYS_SUB_PLAN_DETAILS));
            }
        }
    }
}
