...
bool BasePlacementAdditionStrategy::optimisticPathSelection(
  ...
  while (!success && retryCount < MAX_PATH_SELECTION_RETRIES) {
    ...
    if (!success) {
      NES_WARNING(
          "Unable to lock topology nodes in the path. Waiting for the process "
          "to release locks and retry "
          "path selection.");
    ...
    if (computedDecomposedQueryPlans.contains(pinnedWorkerId)) {
      ...
      for (const auto& existingDecomposedQueryPlan :
        ...
        if (foundConnectedUpstreamPlacedOperator) {  // If an upstream operator
                                                     // found in the placed
                                                     // query sub plan then
                                                     // create a new or merge
                                                     // the placed query plan
                                                     // with the new query sub
                                                     // plan.
          if (!newDecomposedQueryPlan) {
            ...
            const auto& rootOperators =
                existingDecomposedQueryPlan->getRootOperators();
            std::for_each(
                rootOperators.begin(), rootOperators.end(),
  ...
  for (const auto& workerNodeId : workerNodeIdsInBFS) {
    ...
    try {
      ...
      if (!lockedTopologyNode->operator*()->occupySlots(consumedResources)) {
        NES_ERROR(
            "Unable to occupy resources on the topology node {} to "
      ...
      for (const auto& computedDecomposedQueryPlan :
        ...
        if (computedDecomposedQueryPlan->getDecomposedQueryPlanId() ==
          ...
          if (containPinnedUpstreamOperator) {
                    ...
                    for (const auto& pinnedDownstreamOperator :
                      ...
                      for (const auto& placedParent :
                        ...
                        if (placedParent->as<Operator>()->getId() ==
                          ...
                          NES_DEBUG(
                              "Not modifying operator because it is a non "
                              "system operator");
            ...
            NES_ERROR(
                "A decomposed query plan {} with invalid decomposed query plan "
                "id that has no pinned upstream "
...
TopologyNodePtr BasePlacementAdditionStrategy::getTopologyNode(
    WorkerId workerId) {
  NES_TRACE("Get the topology node {}", workerId);
  if (!workerIdToTopologyNodeMap.contains(workerId)) {
    NES_ERROR(
        "Topology node with id {} not considered during the path selection "
        "phase.",
        workerId);
    throw Exceptions::RuntimeException(
