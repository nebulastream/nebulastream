...
QueryPlanPtr PredicateReorderingRule::apply(NES::QueryPlanPtr queryPlan) {
  ...
  NES_DEBUG(
      "PredicateReorderingRule: Identified {} filter nodes in the query plan",
  ...
  for (auto &filter : filterOperators) {
    if (visitedOperators.find(filter->getId()) == visitedOperators.end()) {
      ...
      NES_TRACE("PredicateReorderingRule: Filter {} has {} consecutive filters "
                "as children",
                filter->getId(), consecutiveFilters.size());
      if (consecutiveFilters.size() >= 2) {
        std::vector<NodePtr> filterChainParents =
            consecutiveFilters.front()->getParents();
        std::vector<NodePtr> filterChainChildren =
            consecutiveFilters.back()->getChildren();
        NES_TRACE("PredicateReorderingRule: If the filters are already sorted, "
                  "no change is needed");
        auto already_sorted = std::is_sorted(
            consecutiveFilters.begin(), consecutiveFilters.end(),
            [](const LogicalFilterOperatorPtr &lhs,
               const LogicalFilterOperatorPtr &rhs) {
              return lhs->getSelectivity() < rhs->getSelectivity();
            });
        if (!already_sorted) {
          NES_TRACE("PredicateReorderingRule: Sort all filter nodes in "
                    "increasing order of selectivity");
          std::sort(consecutiveFilters.begin(), consecutiveFilters.end(),
                    [](const LogicalFilterOperatorPtr &lhs,
                       const LogicalFilterOperatorPtr &rhs) {
                      return lhs->getSelectivity() < rhs->getSelectivity();
                    });
          NES_TRACE(
              "PredicateReorderingRule: Start re-writing the new query plan");
          NES_TRACE(
              "PredicateReorderingRule: Remove parent/children references");
          for (auto &filterToReassign : consecutiveFilters) {
            filterToReassign->removeAllParent();
            filterToReassign->removeChildren();
          }
          NES_TRACE("PredicateReorderingRule: For each filter, reassign "
                    "children according to new order");
          ...
          NES_TRACE("PredicateReorderingRule: Retrieve most and least "
                    "selective filters");
          auto leastSelectiveFilter = consecutiveFilters.front();
          auto mostSelectiveFilter = consecutiveFilters.back();
          NES_TRACE("PredicateReorderingRule: Least selective filter goes to "
                    "the top (closer to sink), his new "
          ...
          NES_TRACE("PredicateReorderingRule: Most selective filter goes to "
                    "the bottom (closer to the source), his "
        ...
        NES_TRACE(
            "PredicateReorderingRule: Mark the involved nodes as visited");
...
PredicateReorderingRule::getConsecutiveFilters(
