...
QueryPlanPtr PredicateReorderingRule::apply(NES::QueryPlanPtr queryPlan) {
  ...
  for (auto& filter : filterOperators) {
    if (visitedOperators.find(filter->getId()) == visitedOperators.end()) {
      ...
      NES_TRACE(
          "PredicateReorderingRule: Filter {} has {} consecutive filters as "
          "children",
      ...
      if (consecutiveFilters.size() >= 2) {
        ...
        NES_TRACE(
            "PredicateReorderingRule: If the filters are already sorted, no "
            "change is needed");
        ...
        if (!already_sorted) {
          NES_TRACE(
              "PredicateReorderingRule: Sort all filter nodes in increasing "
          ...
          NES_TRACE(
              "PredicateReorderingRule: For each filter, reassign children "
          ...
          NES_TRACE(
              "PredicateReorderingRule: Retrieve most and least selective "
              "filters");
          auto leastSelectiveFilter = consecutiveFilters.front();
          auto mostSelectiveFilter = consecutiveFilters.back();
          NES_TRACE(
              "PredicateReorderingRule: Least selective filter goes to the top "
              "(closer to sink), his new "
          ...
          NES_TRACE(
              "PredicateReorderingRule: Most selective filter goes to the "
              "bottom (closer to the source), his "
