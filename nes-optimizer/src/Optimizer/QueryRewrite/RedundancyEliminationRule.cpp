...
NES::ExpressionNodePtr RedundancyEliminationRule::eliminatePredicateRedundancy(
  ...
  if (predicate->instanceOf<EqualsExpressionNode>() ||
    ...
    NES_DEBUG(
        "The predicate has a comparison operator, proceed by moving constants "
        "if possible");
...
NES::ExpressionNodePtr RedundancyEliminationRule::constantMoving(
    const ExpressionNodePtr& predicate) {
...
NES::ExpressionNodePtr RedundancyEliminationRule::constantFolding(
    const ExpressionNodePtr& predicate) {
  ...
  if (predicate->instanceOf<AddExpressionNode>() ||
    ...
    if (leftOperand->instanceOf<ConstantValueExpressionNode>() &&
      ...
      NES_DEBUG(
          "Both of the predicate expressions are constant and can be folded "
          "together");
      ...
      NES_DEBUG(
          "Not all the predicate expressions are constant, cannot apply "
          "folding");
    }
  } else {
    NES_DEBUG(
...
NES::ExpressionNodePtr RedundancyEliminationRule::arithmeticSimplification(
  ...
  NES_DEBUG(
      "Applying RedundancyEliminationRule.arithmeticSimplification to "
      "predicate {}",
      predicate->toString());
  if (predicate->instanceOf<AddExpressionNode>() ||
      predicate->instanceOf<MulExpressionNode>()) {
    NES_DEBUG(
        "The predicate involves an addition or multiplication, the rule can be "
        "applied");
    auto operands = predicate->getChildren();
    ...
    if (operands.size() == 2) {
      NES_DEBUG(
          "Check if the operands are a combination of a field access and a "
          "constant");
      ...
      if (constantOperand && fieldAccessOperand) {
        ...
        if (constantValue == 0 && predicate->instanceOf<AddExpressionNode>()) {
          ...
          return fieldAccessOperand->as<ExpressionNode>();
                   ...
                   predicate->instanceOf<MulExpressionNode>()) {
          NES_DEBUG(
              "Case 2: Multiplication by 0: return the "
              "ConstantValueExpressionNode, that is 0");
          return constantOperand->as<ExpressionNode>();
        } else if (constantValue == 1 &&
                   predicate->instanceOf<MulExpressionNode>()) {
          NES_DEBUG(
              "Case 3: Multiplication by 1: return the "
              "FieldAccessExpressionNode");
          return fieldAccessOperand->as<ExpressionNode>();
        } else {
          NES_DEBUG(
              "Given the combination of the constant value and predicate, no "
              "arithmetic simplification is possible");
        }
      } else {
        NES_DEBUG(
            "The predicate is not a combination of constant and value access, "
            "no arithmetic simplification is possible");
      }
    } else {
      NES_DEBUG(
          "The predicate does not have two children, no arithmetic "
          "simplification is possible");
    }
  } else {
    NES_DEBUG(
        "The predicate does not involve an addition or multiplication, no "
        "arithmetic simplification is possible");
  }
  NES_DEBUG("Returning original unmodified predicate");
  return predicate;
...
RedundancyEliminationRule::conjunctionDisjunctionSimplification(
  ...
  NES_DEBUG(
      "At the moment boolean operator cannot be specified in the queries, no "
      "simplification is possible");
  NES_DEBUG("Returning original unmodified predicate");
  return predicate;
}
