...
void BinaryOperatorSortRule::sortChildren(
    const BinaryOperatorPtr &binaryOperator) {
  // Extract the children operators
  auto children = binaryOperator->getChildren();
  NES_ASSERT(children.size() == 2,
             "Binary operator should have only 2 children");
