...
QueryPlanPtr FilterMergeRule::apply(NES::QueryPlanPtr queryPlan) {
  ...
  NES_DEBUG("FilterMergeRule: Identified {} filter nodes in the query plan",
            filterOperators.size());
  ...
  for (auto &filter : filterOperators) {
    if (visitedOperators.find(filter->getId()) == visitedOperators.end()) {
      std::vector<LogicalFilterOperatorPtr> consecutiveFilters =
          getConsecutiveFilters(filter);
      NES_DEBUG(
          "FilterMergeRule: Filter {} has {} consecutive filters as children",
          filter->getId(), consecutiveFilters.size());
      if (consecutiveFilters.size() >= 2) {
        ...
        NES_DEBUG("FilterMergeRule: The predicate of each consecutive filter "
                  "should be appended to the conjunction");
        for (unsigned int i = 1; i < consecutiveFilters.size(); i++) {
          auto predicate = consecutiveFilters.at(i)->getPredicate();
          combinedPredicate =
              AndExpressionNode::create(combinedPredicate, predicate);
        }
        NES_DEBUG("FilterMergeRule: Create new combined filter with the "
                  "conjunction of all filter predicates");
        auto combinedFilter =
            LogicalOperatorFactory::createFilterOperator(combinedPredicate);
        ...
        NES_DEBUG("FilterMergeRule: Remove parent/children references for the "
                  "consecutive filters");
        for (auto &filterToRemove : consecutiveFilters) {
          filterToRemove->removeAllParent();
          filterToRemove->removeChildren();
        }
        NES_DEBUG("FilterMergeRule: Fix references, the parent of new filter "
                  "are the parents of the filter chain");
        for (auto &filterChainParent : filterChainParents) {
          combinedFilter->addParent(filterChainParent);
        }
        NES_DEBUG("FilterMergeRule: Fix references, the chain children have "
                  "only one parent, which is the new combined filter");
        ...
        NES_DEBUG("FilterMergeRule: Only one filter was found, no optimization "
                  "is possible")
      }
    } else {
...
std::vector<LogicalFilterOperatorPtr> FilterMergeRule::getConsecutiveFilters(
    const NES::LogicalFilterOperatorPtr &filter) {
  std::vector<LogicalFilterOperatorPtr> consecutiveFilters = {};
