  ...
  for (auto& filter : filterOperators) {
    if (visitedOperators.find(filter->getId()) == visitedOperators.end()) {
      ...
      if (consecutiveFilters.size() >= 2) {
        ...
        NES_DEBUG(
            "FilterMergeRule: The predicate of each consecutive filter should "
            "be appended to the conjunction");
        ...
        NES_DEBUG(
            "FilterMergeRule: Create new combined filter with the conjunction "
            "of all filter predicates");
        auto combinedFilter =
        ...
        NES_DEBUG(
            "FilterMergeRule: Remove parent/children references for the "
            "consecutive filters");
        ...
        NES_DEBUG(
            "FilterMergeRule: Fix references, the parent of new filter are the "
            "parents of the filter chain");
        for (auto& filterChainParent : filterChainParents) {
          combinedFilter->addParent(filterChainParent);
        }
        NES_DEBUG(
            "FilterMergeRule: Fix references, the chain children have only one "
            "parent, which is the new combined filter");
        ...
        NES_DEBUG(
            "FilterMergeRule: Only one filter was found, no optimization is "
            "possible")
      }
    } else {
