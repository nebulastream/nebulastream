...
LogicalSourceExpansionRulePtr LogicalSourceExpansionRule::create(
    const Catalogs::Source::SourceCatalogPtr &sourceCatalog,
    bool expandSourceOnly) {
  return std::make_shared<LogicalSourceExpansionRule>(
      LogicalSourceExpansionRule(sourceCatalog, expandSourceOnly));
}
...
QueryPlanPtr LogicalSourceExpansionRule::apply(QueryPlanPtr queryPlan) {
  ...
  if (expandSourceOnly) {
    ...
    for (const auto &sourceOperator : sourceOperators) {
      for (auto &downStreamOp : sourceOperator->getParents()) {
        blockingOperators[downStreamOp->as<Operator>()->getId()] =
            downStreamOp->as<Operator>();
      }
    }
  } else {
    for (const auto &rootOperator : queryPlan->getRootOperators()) {
      ...
      for (auto itr = depthFirstNodeIterator.begin();
           itr != NES::DepthFirstNodeIterator::end(); ++itr) {
        NES_TRACE("FilterPushDownRule: Iterate and find the predicate with "
                  "FieldAccessExpression Node");
  ...
  // After we duplicate all non-blocking logical operators, we have the same
  // statistic id for multiple operators. Furthermore, we require to know all
  // statistic ids that belong to one non-duplicated logical operator.
  // Therefore, we do the following two steps:
  //      1. First, we create new statistic id so that the statistic id is
  //      unique across the whole system
  //      2. We store for the old statistic ids, all sibling statistic ids in
  //      the property. This way, we can still
  //         deduce the new statistic ids from an old one.
  std::unordered_map<StatisticId, std::vector<StatisticId>>
      siblingStatisticIdToNewStatisticIds;

  // Iterate over all source operators
  for (const auto &sourceOperator : sourceOperators) {
    SourceDescriptorPtr sourceDescriptor =
        sourceOperator->getSourceDescriptor();
    NES_TRACE("LogicalSourceExpansionRule: Get the number of physical source "
              "locations in the topology.");
    ...
    NES_TRACE("LogicalSourceExpansionRule: Found {} physical source locations "
              "in the topology.",
              sourceCatalogEntries.size());
    if (sourceCatalogEntries.empty()) {
      throw Exceptions::PhysicalSourceNotFoundException(
          "LogicalSourceExpansionRule: Unable to find physical source "
          "locations for the logical source " +
          logicalSourceName);
    }
    ...
    if (!expandSourceOnly) {
      ...
      for (const auto &downStreamOperator : sourceOperator->getParents()) {
        // If downStreamOperator is blocking then remove source operator as its
        // upstream operator.
        if (!downStreamOperator->removeChild(sourceOperator)) {
          throw Exceptions::RuntimeException(
              "LogicalSourceExpansionRule: Unable to remove non-blocking "
              "upstream operator from the blocking operator");
        }

        // Add information about blocking operator to the source operator
        addBlockingDownStreamOperator(
            sourceOperator, downStreamOperator->as<Operator>()->getId());
      }
    }
    NES_TRACE("LogicalSourceExpansionRule: Create {} duplicated logical "
              "sub-graph and add to original graph",
              sourceCatalogEntries.size());

    // Create one duplicate operator for each physical source
    for (const auto &sourceCatalogEntry : sourceCatalogEntries) {
      NES_TRACE(
          "LogicalSourceExpansionRule: Create duplicated logical sub-graph");
      auto duplicateSourceOperator =
          sourceOperator->duplicate()->as<SourceLogicalOperator>();
      // Add to the source operator the id of the physical node where we have to
      // pin the operator NOTE: This is required at the time of placement to
      // know where the source operator is pinned
      duplicateSourceOperator->addProperty(
          PINNED_WORKER_ID, sourceCatalogEntry->getTopologyNodeId());
      // Add Physical Source Name to the source descriptor
      auto duplicateSourceDescriptor = sourceDescriptor->copy();
      duplicateSourceDescriptor->setPhysicalSourceName(
      ...
      duplicateSourceOperator->setStatisticId(
          sourceCatalogEntry->getPhysicalSource()->getStatisticId());

      // Flatten the graph to duplicate and find operators that need to be
      // connected to blocking parents.
      const std::vector<NodePtr> &allOperators =
          duplicateSourceOperator->getAndFlattenAllAncestors();

      std::unordered_set<OperatorId> visitedOperators;
      for (const auto &node : allOperators) {
        auto operatorNode = node->as<Operator>();

        // Check if the operator has the property containing list of connected
        // blocking downstream operator ids.
        //  If so, then connect the operator to the blocking downstream operator
        if (operatorNode->hasProperty(
                LIST_OF_BLOCKING_DOWNSTREAM_OPERATOR_IDS)) {
          // Fetch the blocking upstream operators of this operator
          const std::any &value = operatorNode->getProperty(
              LIST_OF_BLOCKING_DOWNSTREAM_OPERATOR_IDS);
          auto listOfConnectedBlockingParents =
              std::any_cast<std::vector<OperatorId>>(value);
          // Iterate over all blocking parent ids and connect the duplicated
          // operator
          for (auto blockingParentId : listOfConnectedBlockingParents) {
            ...
            if (!blockingOperator) {
              throw Exceptions::OperatorNotFoundException(
                  fmt::format("LogicalSourceExpansionRule: Unable to find "
                              "blocking operator with id {}",
                              blockingParentId));
            }
            ...
            if (!visitedOperators.contains(operatorNode->getId())) {
              ...
              siblingStatisticIdToNewStatisticIds[siblingStatisticId]
                  .emplace_back(newStatisticId);
            }
...
void LogicalSourceExpansionRule::distributeSiblingStatisticId(
    QueryPlan &queryPlan,
    std::unordered_map<StatisticId, std::vector<StatisticId>>
        &siblingStatisticIdToNewStatisticIds) const {

  NES_DEBUG(
      "LogicalSourceExpansionRule: Plan before updating the statistic ids\n{}",
      queryPlan.toString());

  // For each statisticId, we store all new statistic ids in the property map
  // and distribute the new statistic ids
  for (auto &[oldStatisticId, allNewStatisticIds] :
       siblingStatisticIdToNewStatisticIds) {
    for (auto &newStatisticId : allNewStatisticIds) {
      const auto operatorNode =
          queryPlan.getOperatorWithStatisticId(oldStatisticId);
      if (operatorNode->instanceOf<SourceLogicalOperator>()) {
        NES_INFO("We do not update the statistic id of a source!");
        continue;
      }

      auto value = operatorNode->as<Operator>()->getProperty(
          LIST_OF_SIBLING_STATISTIC_IDS);
      ...
      listOfStatisticIds.insert(listOfStatisticIds.end(),
                                allNewStatisticIds.begin(),
                                allNewStatisticIds.end());
...
void LogicalSourceExpansionRule::removeConnectedBlockingOperators(
    const NodePtr &operatorNode) {

  // Check if downstream (parent) operator of this operator is blocking or not
  // if not then recursively call this method for the
  //  downstream operator
  auto downStreamOperators = operatorNode->getParents();
  NES_TRACE("LogicalSourceExpansionRule: For each parent look if their "
            "ancestor has a n-ary operator or a sink operator.");
  for (const auto &downStreamOperator : downStreamOperators) {
    ...
    if (!isBlockingOperator(downStreamOperator)) {
      ...
      // If downStreamOperator is blocking then remove current operator as its
      // upstream operator.
      ...
      // Add to the current operator information about operator id of the
      // removed downStreamOperator. We will use this information post expansion
      // to re-add the connection later.
      addBlockingDownStreamOperator(
          operatorNode, downStreamOperator->as_if<Operator>()->getId());
    }
  }
}

void LogicalSourceExpansionRule::addBlockingDownStreamOperator(
    const NodePtr &operatorNode, OperatorId downStreamOperatorId) {
  // extract the list of connected blocking parents and add the current parent
  // to the list
  std::any value = operatorNode->as_if<Operator>()->getProperty(
      LIST_OF_BLOCKING_DOWNSTREAM_OPERATOR_IDS);
  if (value.has_value()) { // update the existing list
    auto listOfConnectedBlockingParents =
        std::any_cast<std::vector<OperatorId>>(value);
    listOfConnectedBlockingParents.emplace_back(downStreamOperatorId);
    operatorNode->as_if<Operator>()->addProperty(
        LIST_OF_BLOCKING_DOWNSTREAM_OPERATOR_IDS,
...
bool LogicalSourceExpansionRule::isBlockingOperator(
    const NodePtr &operatorNode) {
  return (operatorNode->instanceOf<SinkLogicalOperator>() ||
          operatorNode->instanceOf<LogicalWindowOperator>() ||
          operatorNode->instanceOf<LogicalUnionOperator>() ||
          operatorNode->instanceOf<LogicalJoinOperator>() ||
