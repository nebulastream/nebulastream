...
QueryPlanPtr LogicalSourceExpansionRule::apply(QueryPlanPtr queryPlan) {
  ...
  if (expandSourceOnly) {
    ...
    for (const auto& rootOperator : queryPlan->getRootOperators()) {
      ...
      for (auto itr = depthFirstNodeIterator.begin();
        ...
        NES_TRACE(
            "FilterPushDownRule: Iterate and find the predicate with "
  ...
  for (const auto& sourceOperator : sourceOperators) {
    ...
    NES_TRACE(
        "LogicalSourceExpansionRule: Get the number of physical source "
    ...
    NES_TRACE(
        "LogicalSourceExpansionRule: Found {} physical source locations in the "
        "topology.",
...
void LogicalSourceExpansionRule::removeConnectedBlockingOperators(
  ...
  NES_TRACE(
      "LogicalSourceExpansionRule: For each parent look if their ancestor has "
      "a n-ary operator or a sink operator.");
