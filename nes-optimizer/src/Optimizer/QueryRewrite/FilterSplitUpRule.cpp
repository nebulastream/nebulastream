...
QueryPlanPtr FilterSplitUpRule::apply(NES::QueryPlanPtr queryPlan) {
  ...
  NES_DEBUG(
      "FilterSplitUpRule: Sort all filter nodes in increasing order of the "
      "operator id")
  ...
  try {
    NES_DEBUG(
        "FilterSplitUpRule: Iterate over all the filter operators to split "
        "them");
...
void FilterSplitUpRule::splitUpFilters(
  ...
  if (filterOperator->getPredicate()->instanceOf<AndExpressionNode>()) {
    ...
    if (!filterOperator->insertBetweenThisAndChildNodes(child1)) {
      NES_ERROR(
          "FilterSplitUpRule: Error while trying to insert a filterOperator "
          "into the queryPlan");
    ...
    if (!filterOperator->removeAndJoinParentAndChildren()) {
      NES_ERROR(
          "FilterSplitUpRule: Error while trying to remove a filterOperator "
          "from the queryPlan");
      throw std::logic_error("FilterSplitUpRule: query plan not valid anymore");
    }
