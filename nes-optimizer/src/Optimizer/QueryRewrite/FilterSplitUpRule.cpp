...
QueryPlanPtr FilterSplitUpRule::apply(NES::QueryPlanPtr queryPlan) {
  ...
  std::vector<LogicalFilterOperatorPtr> filterOperators(
      filterOperatorsSet.begin(), filterOperatorsSet.end());
  NES_DEBUG("FilterSplitUpRule: Sort all filter nodes in increasing order of "
            "the operator id")
  std::sort(filterOperators.begin(), filterOperators.end(),
            [](const LogicalFilterOperatorPtr &lhs,
               const LogicalFilterOperatorPtr &rhs) {
              return lhs->getId() < rhs->getId();
            });
  auto originalQueryPlan = queryPlan->copy();
  try {
    NES_DEBUG("FilterSplitUpRule: Iterate over all the filter operators to "
              "split them");
    ...
    NES_ERROR("FilterSplitUpRule: Error while applying FilterSplitUpRule: {}",
              exc.what());
    NES_ERROR("FilterSplitUpRule: Returning unchanged original query plan {}",
              originalQueryPlan->toString());
    return originalQueryPlan;
  }
}

void FilterSplitUpRule::splitUpFilters(
    LogicalFilterOperatorPtr filterOperator) {
  // if our query plan contains a parentOperaters->filter(expression1 &&
  // expression2)->childOperator. We can rewrite this plan to
  // parentOperaters->filter(expression1)->filter(expression2)->childOperator.
  if (filterOperator->getPredicate()->instanceOf<AndExpressionNode>()) {
    ...
    child1->setPredicate(
        filterOperator->getPredicate()->getChildren()[0]->as<ExpressionNode>());
    ...
    child2->setPredicate(
        filterOperator->getPredicate()->getChildren()[1]->as<ExpressionNode>());

    // insert new filter with expression1 of the andExpression
    if (!filterOperator->insertBetweenThisAndChildNodes(child1)) {
      NES_ERROR("FilterSplitUpRule: Error while trying to insert a "
                "filterOperator into the queryPlan");
    ...
    if (!filterOperator->removeAndJoinParentAndChildren()) {
      NES_ERROR("FilterSplitUpRule: Error while trying to remove a "
                "filterOperator from the queryPlan");
      throw std::logic_error("FilterSplitUpRule: query plan not valid anymore");
    }

    // newly created filters could also be andExpressions that can be further
    // split up
  ...
  // it might be possible to reformulate negated expressions
    ...
    // In the case that the predicate is of the form !( expression1 ||
    // expression2 ) it can be reformulated to ( !expression1 && !expression2 ).
    // The reformulated predicate can be used to apply the split up filter rule
    // again.
    if (filterOperator->getPredicate()
            ->getChildren()[0]
            ->instanceOf<OrExpressionNode>()) {
      auto orExpression = filterOperator->getPredicate()->getChildren()[0];
      auto negatedChild1 = NegateExpressionNode::create(
          orExpression->getChildren()[0]->as<ExpressionNode>());
      auto negatedChild2 = NegateExpressionNode::create(
          orExpression->getChildren()[1]->as<ExpressionNode>());

      auto equivalentAndExpression =
          AndExpressionNode::create(negatedChild1, negatedChild2);
      filterOperator->setPredicate(
          equivalentAndExpression); // changing predicate to equivalent
                                    // AndExpression
      splitUpFilters(filterOperator); // splitting up the filter
    }
    // Reformulates predicates in the form (!!expression) to (expression)
    else if (filterOperator->getPredicate()
                 ->getChildren()[0]
                 ->instanceOf<NegateExpressionNode>()) {
      // getPredicate() is the first NegateExpression; first getChildren()[0] is
      // the second NegateExpression; second getChildren()[0] is the
      // expressionNode that was negated twice. copy() only copies children of
      // this expressionNode. (probably not mandatory but no reference to the
      // negations needs to be kept)
      filterOperator->setPredicate(filterOperator->getPredicate()
                                       ->getChildren()[0]
                                       ->getChildren()[0]
                                       ->as<ExpressionNode>()
                                       ->copy());
