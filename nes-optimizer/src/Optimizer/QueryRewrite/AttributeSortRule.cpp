...
QueryPlanPtr AttributeSortRule::apply(NES::QueryPlanPtr queryPlan) {
  ...
  for (auto const &filterOperator : filterOperators) {
    ...
    auto updatedFilter =
        LogicalOperatorFactory::createFilterOperator(updatedPredicate);
  ...
  for (auto const &mapOperator : mapOperators) {
    ...
    auto updatedMapExpression = sortAttributesInExpression(mapExpression)
                                    ->as<FieldAssignmentExpressionNode>();
    auto updatedMap =
        LogicalOperatorFactory::createMapOperator(updatedMapExpression);
...
NES::ExpressionNodePtr AttributeSortRule::sortAttributesInExpression(
    NES::ExpressionNodePtr expression) {
  NES_DEBUG("Sorting attributed for input expression {}",
            expression->toString());
  ...
  if (expression->instanceOf<NES::ArithmeticalExpressionNode>()) {
    ...
    auto fieldAssignmentExpressionNode =
        expression->as<NES::FieldAssignmentExpressionNode>();
  ...
  NES_THROW_RUNTIME_ERROR(
      "No conversion to Z3 expression implemented for the expression: " +
      expression->toString());
  return nullptr;
}

ExpressionNodePtr AttributeSortRule::sortAttributesInArithmeticalExpressions(
    ExpressionNodePtr expression) {
  NES_DEBUG("Create Z3 expression for arithmetical expression {}",
            expression->toString());
  if (expression->instanceOf<NES::AddExpressionNode>()) {
    ...
    auto sortedRight =
        sortAttributesInExpression(addExpressionNode->getRight());

    auto leftCommutativeFields =
        fetchCommutativeFields<NES::AddExpressionNode>(sortedLeft);
    auto rightCommutativeFields =
        fetchCommutativeFields<NES::AddExpressionNode>(sortedRight);

    std::vector<ExpressionNodePtr> allCommutativeFields;
    allCommutativeFields.insert(allCommutativeFields.end(),
                                leftCommutativeFields.begin(),
                                leftCommutativeFields.end());
    allCommutativeFields.insert(allCommutativeFields.end(),
                                rightCommutativeFields.begin(),
                                rightCommutativeFields.end());
    ...
    std::sort(
        sortedCommutativeFields.begin(), sortedCommutativeFields.end(),
        [](const NES::ExpressionNodePtr &lhsField,
           const NES::ExpressionNodePtr &rhsField) {
          std::string leftValue;
          std::string rightValue;

          if (lhsField->instanceOf<NES::ConstantValueExpressionNode>()) {
            auto constantValue =
                lhsField->as<NES::ConstantValueExpressionNode>()
                    ->getConstantValue();
            auto basicValueType =
                std::dynamic_pointer_cast<BasicValue>(constantValue);
            leftValue = basicValueType->value;
          } else {
            leftValue =
                lhsField->as<NES::FieldAccessExpressionNode>()->getFieldName();
          }

          if (rhsField->instanceOf<ConstantValueExpressionNode>()) {
            auto constantValue =
                rhsField->as<ConstantValueExpressionNode>()->getConstantValue();
            auto basicValueType =
                std::dynamic_pointer_cast<BasicValue>(constantValue);
            rightValue = basicValueType->value;
          } else {
            rightValue =
                rhsField->as<NES::FieldAccessExpressionNode>()->getFieldName();
          }
    ...
    for (unsigned long i = 0; i < sortedCommutativeFields.size(); i++) {
      ...
      if (sortedLeft.get() == originalField.get()) {
        ...
        bool replaced = replaceCommutativeExpressions(sortedLeft, originalField,
                                                      updatedField);
        if (replaced) {
          continue;
        }
      ...
      if (sortedRight.get() == originalField.get()) {
        ...
        bool replaced = replaceCommutativeExpressions(
            sortedRight, originalField, updatedField);
        if (replaced) {
          continue;
        }
      }
    ...
    if (!sortedLeft->instanceOf<AddExpressionNode>() ||
        !sortedRight->instanceOf<AddExpressionNode>()) {
      auto leftSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedLeft);
      auto rightSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedRight);
  ...
  if (expression->instanceOf<SubExpressionNode>()) {
    ...
    auto leftCommutativeFields =
        fetchCommutativeFields<MulExpressionNode>(sortedLeft);
    auto rightCommutativeFields =
        fetchCommutativeFields<MulExpressionNode>(sortedRight);
    ...
    std::sort(
        sortedCommutativeFields.begin(), sortedCommutativeFields.end(),
        [](const ExpressionNodePtr &lhsField,
           const ExpressionNodePtr &rhsField) {
          std::string leftValue;
          std::string rightValue;

          if (lhsField->instanceOf<ConstantValueExpressionNode>()) {
            auto constantValue =
                lhsField->as<ConstantValueExpressionNode>()->getConstantValue();
            auto basicValueType =
                std::dynamic_pointer_cast<BasicValue>(constantValue);
            leftValue = basicValueType->value;
          } else {
            leftValue =
                lhsField->as<FieldAccessExpressionNode>()->getFieldName();
          }
          ...
          if (rhsField->instanceOf<ConstantValueExpressionNode>()) {
            ...
            rightValue =
                rhsField->as<FieldAccessExpressionNode>()->getFieldName();
          }
          return leftValue.compare(rightValue) < 0;
        });
    ...
    if (!sortedLeft->instanceOf<MulExpressionNode>() ||
        !sortedRight->instanceOf<MulExpressionNode>()) {
      auto leftSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedLeft);
      auto rightSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedRight);
  ...
  NES_THROW_RUNTIME_ERROR("No conversion to Z3 expression implemented for the "
                          "arithmetical expression node: " +
                          expression->toString());
  return nullptr;
}

ExpressionNodePtr AttributeSortRule::sortAttributesInLogicalExpressions(
    const ExpressionNodePtr &expression) {
  NES_DEBUG("Create Z3 expression node for logical expression {}",
            expression->toString());
  if (expression->instanceOf<AndExpressionNode>()) {
    ...
    auto leftCommutativeFields =
        fetchCommutativeFields<AndExpressionNode>(sortedLeft);
    auto rightCommutativeFields =
        fetchCommutativeFields<AndExpressionNode>(sortedRight);
    ...
    if (!sortedLeft->instanceOf<AndExpressionNode>() ||
        !sortedRight->instanceOf<AndExpressionNode>()) {
      auto leftSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedLeft);
      auto rightSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedRight);
  ...
  if (expression->instanceOf<OrExpressionNode>()) {
    ...
    auto leftCommutativeFields =
        fetchCommutativeFields<OrExpressionNode>(sortedLeft);
    auto rightCommutativeFields =
        fetchCommutativeFields<OrExpressionNode>(sortedRight);
    ...
    if (!sortedLeft->instanceOf<OrExpressionNode>() ||
        !sortedRight->instanceOf<OrExpressionNode>()) {
      auto leftSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedLeft);
      auto rightSortedFieldName =
          fetchLeftMostConstantValueOrFieldName(sortedRight);
  ...
  NES_THROW_RUNTIME_ERROR("No conversion to Z3 expression possible for the "
...
bool AttributeSortRule::replaceCommutativeExpressions(
  ...
  if (rightChild.get() == originalExpression.get()) {
    ...
    for (const auto &child : children) {
      if (!(child->instanceOf<FieldAccessExpressionNode>() ||
            child->instanceOf<ConstantValueExpressionNode>())) {
        bool replaced = replaceCommutativeExpressions(
            child->as<ExpressionNode>(), originalExpression, updatedExpression);
        if (replaced) {
          return true;
        }
      }
...
std::string AttributeSortRule::fetchLeftMostConstantValueOrFieldName(
    ExpressionNodePtr expression) {
  ExpressionNodePtr startPoint = std::move(expression);
  while (!(startPoint->instanceOf<FieldAccessExpressionNode>() ||
           startPoint->instanceOf<ConstantValueExpressionNode>())) {
  ...
  const ValueTypePtr &constantValue =
      startPoint->as<ConstantValueExpressionNode>()->getConstantValue();
  if (auto basicValueType =
          std::dynamic_pointer_cast<BasicValue>(constantValue);
      basicValueType) {
    return basicValueType->value;
  }

  if (auto arrayValueType =
          std::dynamic_pointer_cast<ArrayValue>(constantValue);
      arrayValueType) {
    return std::accumulate(arrayValueType->values.begin(),
                           arrayValueType->values.end(), std::string());
  }

  NES_THROW_RUNTIME_ERROR(
      "AttributeSortRule not equipped for handling value type!");
}
