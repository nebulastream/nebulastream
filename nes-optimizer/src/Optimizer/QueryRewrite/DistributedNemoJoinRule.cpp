...
DistributedNemoJoinRule::DistributedNemoJoinRule(
    Configurations::OptimizerConfiguration configuration, TopologyPtr topology,
    const std::map<Catalogs::Source::SourceCatalogEntryPtr, std::set<uint64_t>>
        &keyDistributionMap)
    : topology(topology), configuration(configuration),
      keyDistributionMap(keyDistributionMap) {}
...
DistributedNemoJoinRulePtr DistributedNemoJoinRule::create(
  ...
  return std::make_shared<DistributedNemoJoinRule>(
      DistributedNemoJoinRule(configuration, topology, distributionMap));
}
...
QueryPlanPtr DistributedNemoJoinRule::apply(QueryPlanPtr queryPlan) {
  ...
  if (!joinOps.empty()) {
    std::map<uint64_t, std::set<WorkerId>> commonNodes =
        getNodesWithCommonKeys();
    ...
    if (!commonOperators.empty()) {
      for (const auto &pair : commonNodes) {
        ...
        NES_DEBUG("DistributedNemoJoinRule: Common nodes {}->{}", pair.first,
                  sourceIds);
      }
      ...
      for (const auto &pair : commonOperators) {
        ...
        NES_DEBUG("DistributedNemoJoinRule: Common operators {}->{}",
                  pair.first, sourceIds);
      }

      for (const LogicalJoinOperatorPtr &joinOp : joinOps) {
        NES_DEBUG("DistributedNemoJoinRule::apply: join operator {}",
                  joinOp->toString());
        ...
        for (const auto &pair : commonOperators) {
          for (auto it1 = pair.second.begin(); it1 != pair.second.end();
               ++it1) {
            for (auto it2 = std::next(it1); it2 != pair.second.end(); ++it2) {
              auto l_op = it1->get();
              auto r_op = it2->get();
              NES_DEBUG(
                  "DistributedNemoJoinRule::apply: sub join operator {} <-> {}",
                  l_op->toString(), r_op->toString());

              LogicalJoinOperatorPtr newJoin =
                  LogicalOperatorFactory::createJoinOperator(
                      joinOp->getJoinDefinition())
...
DistributedNemoJoinRule::getOperatorsWithCommonKeys(
...
std::map<uint64_t, std::set<WorkerId>>
DistributedNemoJoinRule::getNodesWithCommonKeys() {
  std::map<uint64_t, std::set<WorkerId>> result;

  // Iterate through each pair in the map
  for (auto it1 = keyDistributionMap.begin(); it1 != keyDistributionMap.end();
       ++it1) {
