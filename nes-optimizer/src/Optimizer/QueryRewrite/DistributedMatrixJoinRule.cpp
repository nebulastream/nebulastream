...
DistributedMatrixJoinRule::DistributedMatrixJoinRule(
    Configurations::OptimizerConfiguration configuration, TopologyPtr topology)
    : topology(topology), configuration(configuration) {}

DistributedMatrixJoinRulePtr DistributedMatrixJoinRule::create(
    Configurations::OptimizerConfiguration configuration,
    TopologyPtr topology) {
  return std::make_shared<DistributedMatrixJoinRule>(
      DistributedMatrixJoinRule(configuration, topology));
}
...
QueryPlanPtr DistributedMatrixJoinRule::apply(QueryPlanPtr queryPlan) {
  ...
  if (!joinOps.empty()) {
    NES_DEBUG("DistributedMatrixJoinRule::apply: found {} join operators",
              joinOps.size());
    for (const LogicalJoinOperatorPtr& joinOp : joinOps) {
      NES_DEBUG("DistributedMatrixJoinRule::apply: join operator {}",
                joinOp->toString());
