...
PlacementRemovalStrategyPtr PlacementRemovalStrategy::create(
    const GlobalExecutionPlanPtr &globalExecutionPlan,
    const TopologyPtr &topology,
    const TypeInferencePhasePtr &typeInferencePhase,
    PlacementAmendmentMode placementAmendmentMode) {
  return std::make_shared<PlacementRemovalStrategy>(
      PlacementRemovalStrategy(globalExecutionPlan, topology,
...
PlacementRemovalStrategy::updateGlobalExecutionPlan(
  ...
  try {
    NES_INFO("Placement removal strategy called for the shared query plan {}",
             sharedQueryId);
    ...
    // 2. Find all topology nodes used to place the operators in the state
    // Placed, To-Be-Re-Placed, and To-Be-Removed.
    //      2.1. Iterate over the operators and collect the PINNED_WORKER_ID and
    //      information about the nodes where sys
    ...
    // 4. If no operator in the state PLACED or TO_BE_REPLACED or TO_BE_REMOVED
    // then skip rest of the process.
    if (workerIdsInBFS.empty()) {
      NES_WARNING("Skipping placement removal phase as nothing to be removed.");
      return {};
    }

    // 5. Fetch all placed query sub plans by analyzing the PLACED_SUB_PLAN_ID
    // and CONNECTED_SYS_SUB_PLAN_DETAILS.
    updateQuerySubPlans(sharedQueryId);

    // 6. Iterate over the identified topology nodes in a strict order and
    // perform validation as follow:
    //      6.1. Check if after releasing the occupied resources, the new
    //      occupied resources are >= 0. 6.2. The local query sub plan version
    //      and query sub plan version on the execution node are the same
    //           (If they are different then some parallel task has also
    //           manipulated the sub plan).
    // 7. If validation is successful, then lock the topology node and update
    // the operator to Removed or To-Be-Placed
    //    state, either mark the place query sub plan as "Stopped" (if all
    //    operators to be removed) or "Updated" (if plan was modified by
    //    removing operators).
    return updateExecutionNodes(sharedQueryId, querySubPlanVersion);
  } catch (std::exception &ex) {
    NES_ERROR("Exception occurred during placement removal phase: {}",
              ex.what());
    throw Exceptions::QueryPlacementRemovalException(sharedQueryId, ex.what());
  }
}

void PlacementRemovalStrategy::performPathSelection(
    const std::set<LogicalOperatorPtr> &upStreamPinnedOperators,
    const std::set<LogicalOperatorPtr> &downStreamPinnedOperators) {

  // 1. We iterate over the query plan in BFS order and record worker ids where
  ...
  std::for_each(upStreamPinnedOperators.begin(), upStreamPinnedOperators.end(),
  ...
  while (!operatorsToProcessInBFSOrder.empty()) {
    ...
    if (operatorState == OperatorState::REMOVED) {
      throw Exceptions::RuntimeException(
          "Found an operator in the state removed. The sub query should not "
          "contain already removed operators.");
    }

    // 6. If operator is not in the state to be placed and has no pinned worker
    // id property then raise an exception.
    if (operatorState != OperatorState::TO_BE_PLACED &&
        !operatorToProcess->hasProperty(PINNED_WORKER_ID)) {
      throw Exceptions::RuntimeException(
          "Found an operator without pinned worker id. The sub query should "
          "not contain "
    ...
    if (operatorState != OperatorState::TO_BE_PLACED) {
      ...
      // 8. Fetch the query sub plan id that hosts the operator and record the
      // sub query plan id
      auto subQueryPlanId = std::any_cast<DecomposedQueryPlanId>(
          operatorToProcess->getProperty(PLACED_DECOMPOSED_PLAN_ID));
      ...
      // 9. Record the operator id to be processed such that we can change its
      // status later
    ...
    // 10. If the operator is one of the downstream pinned operator then
    // continue
    auto found = std::find_if(
        downStreamPinnedOperators.begin(), downStreamPinnedOperators.end(),
    ...
    for (const auto &parent : operatorToProcess->getParents()) {
      ...
      if (downStreamInToBePlacedState &&
          downStreamOperator->getOperatorState() !=
              OperatorState::TO_BE_PLACED) {
        downStreamInToBePlacedState = false;
      }
      operatorsToProcessInBFSOrder.emplace(downStreamOperator);
    }

    // 12. If the operator is connected by system generated operators then
    // record the worker ids and query sub plan ids
    if (operatorState != OperatorState::TO_BE_PLACED &&
        !downStreamInToBePlacedState) {
      if (operatorToProcess->hasProperty(CONNECTED_SYS_SUB_PLAN_DETAILS)) {
        auto sysPlansMetaData = std::any_cast<std::vector<SysPlanMetaData>>(
            operatorToProcess->getProperty(CONNECTED_SYS_SUB_PLAN_DETAILS));
        for (const auto &sysPlanMetaData : sysPlansMetaData) {
          workerIdsInBFS.emplace(sysPlanMetaData.workerId);
          if (workerIdToDecomposedQueryPlanIds.contains(
                  sysPlanMetaData.workerId)) {
            auto subQueryPlanIds =
  ...
  if (!workerIdsInBFS.empty() &&
      placementAmendmentMode == PlacementAmendmentMode::PESSIMISTIC) {
    // 14. Raise exception if unable to Lock all identified worker ids before
    // processing if the mode is pessimistic
    if (!pessimisticPathSelection()) {
      throw Exceptions::RuntimeException("Unable to find topology nodes or "
                                         "lock the identified topology nodes.");
    }
  }
...
bool PlacementRemovalStrategy::pessimisticPathSelection() {
  ...
  while (!success && retryCount < MAX_PATH_SELECTION_RETRIES) {
    NES_INFO("Pessimistically locking selected path using retry {} of {}.",
             retryCount, MAX_PATH_SELECTION_RETRIES);
    success = true;
    // 2. Perform path selection and if failure than use the exponential
    // back-off and retry strategy
    for (const auto &workerId : workerIdsInBFS) {
      // 3. Try to acquire the lock
      TopologyNodeWLock lock = topology->lockTopologyNode(workerId);
      if (!lock) {
        NES_ERROR("Unable to Lock the topology node {} selected in the path "
                  "selection.",
                  workerId);
        // 4. Release all the acquired locks as part of back-off and retry
        // strategy.
        unlockTopologyNodesInSelectedPath();
        success = false;
        break;
    ...
    // 5. If unable to lock the topology nodes then wait for other processes to
...
bool PlacementRemovalStrategy::unlockTopologyNodesInSelectedPath() {
  ...
  std::for_each(workerIdsInBFS.rbegin(), workerIdsInBFS.rend(),
...
void PlacementRemovalStrategy::updateQuerySubPlans(
    SharedQueryId sharedQueryId) {

  NES_INFO("Update all query sub plans by removing the operators that are in "
           "the state To-Be-Removed or To-Be-RePlaced.");
  // 1. Iterate over the target query sub plans placed on different worker nodes
  // to update them
  for (const auto &[workerId, decomposedQueryPlanIds] :
       workerIdToDecomposedQueryPlanIds) {

    std::vector<DecomposedQueryPlanPtr> updatedDecomposedQueryPlans;
    uint32_t releasedSlots = 0;
    // 2. Update the placed query sub plans on the execution node and record
    // them
    for (const auto &decomposedQueryPlanId : decomposedQueryPlanIds) {
      ...
      auto querySubPlanToUpdate =
          globalExecutionPlan->getCopyOfDecomposedQueryPlan(
              workerId, sharedQueryId, decomposedQueryPlanId);

      // 4. Check if plan is a sys generated query sub plan.
      // A Sys generated plan will contain only network source and sink
      // operators.
      ...
      // 7. Modify the query plan by removing the operators that are in
      // to-be-Removed or in to-be-replaced state
      ...
      for (const auto &placedOperator : allOperators) {
        ...
        if (!operatorIdToOriginalOperatorMap.contains(operatorId)) {
          NES_WARNING("Found the operator {} not in the submitted query plan.",
                      placedOperator->toString());
          continue;
        }

        auto operatorState =
            operatorIdToOriginalOperatorMap[operatorId]->getOperatorState();

        // 8. Remove all upstream and downstream operators of the operators in
        // the state TO_BE_REMOVED or TO_BE_REPLACED
        if (operatorState == OperatorState::TO_BE_REMOVED ||
            operatorState == OperatorState::TO_BE_REPLACED) {

          // increment the released slot count
          releasedSlots++;

          // 9. If the operator is of type sink then also remove it as the root
          // operator of the query sub plan
          if (placedOperator->instanceOf<SinkLogicalOperator>()) {
            querySubPlanToUpdate->removeAsRootOperator(operatorId);
          }

          // 10. Check all upstream operators of the considered operator and if
          // any of the operator is of type network sink then remove it as the
          // root of the query sub plan
          for (auto placedDownStreamOperator : placedOperator->getParents()) {
            if (placedDownStreamOperator->instanceOf<SinkLogicalOperator>() &&
                placedDownStreamOperator->as_if<SinkLogicalOperator>()
                    ->getSinkDescriptor()
                    ->instanceOf<Network::NetworkSinkDescriptor>()) {
              querySubPlanToUpdate->removeAsRootOperator(
                  placedDownStreamOperator->as_if<Operator>()->getId());
            }
          }

          // 11. Remove the operator from the plan by clearing the upstream and
      ...
      // 12. Mark the plan for migration if the plan is empty else for
      // re-deployment
    ...
    workerIdToUpdatedDecomposedQueryPlans[workerId] =
...
std::map<DecomposedQueryPlanId, DeploymentContextPtr>
PlacementRemovalStrategy::updateExecutionNodes(
    SharedQueryId sharedQueryId,
    DecomposedQueryPlanVersion querySubPlanVersion) {
  ...
  for (const auto &workerId : workerIdsInBFS) {
    ...
    if (!workerIdToUpdatedDecomposedQueryPlans.contains(workerId)) {
      NES_WARNING("No plan to updated query sub plan on the worker {}. "
                  "Skipping remainder of the processing.",
                  workerId);
      continue;
    }
    ...
    try {
      // 2. Step1:: Perform validation by checking if we can release the
      // resources the operator placement algorithm reserved for placing the
      // operators.
      auto releasedSlots = workerIdToReleasedSlotMap[workerId];
      auto totalResources =
          lockedTopologyNode->operator*()->getTotalResources();
      auto occupiedResources =
          lockedTopologyNode->operator*()->getOccupiedResources();
      bool nodeValidationPassed = (releasedSlots <= occupiedResources &&
                                   releasedSlots <= totalResources);
      if (!nodeValidationPassed) {
        NES_ERROR("Unable to release resources on the topology node {} to "
                  "successfully remove operators.",
                  workerId);
      ...
      while (!(executionNode =
                   globalExecutionPlan->getLockedExecutionNode(workerId))) {
        std::this_thread::sleep_for(PATH_SELECTION_RETRY_WAIT);
      }

      // 3. Step2:: Perform validation by checking if the currently placed query
      // sub plan has the same version as the updated query sub plan or not
      auto updatedDecomposedQueryPlans =
          workerIdToUpdatedDecomposedQueryPlans[workerId];
      for (const auto &updatedDecomposedQueryPlan :
           updatedDecomposedQueryPlans) {
        auto decomposedQueryPlanId =
            updatedDecomposedQueryPlan->getDecomposedQueryPlanId();
        auto actualQuerySubPlan =
            executionNode->operator*()->getDecomposedQueryPlan(
                sharedQueryId, decomposedQueryPlanId);

        // 4. Check is the updated and actual query sub plan has the same
        // version number
        if (actualQuerySubPlan->getVersion() !=
            updatedDecomposedQueryPlan->getVersion()) {
          NES_ERROR("Query sub plan with id {} got updated. Current version {} "
                    "Read version {}",
                    decomposedQueryPlanId, actualQuerySubPlan->getVersion(),
                    updatedDecomposedQueryPlan->getVersion());
          return deploymentContexts;
        }
      ...
      executionNode->operator*()->updateDecomposedQueryPlans(
          sharedQueryId, updatedDecomposedQueryPlans);
      ...
      for (const auto &updatedOperatorId : updatedOperatorIds) {
        auto originalOperator =
            operatorIdToOriginalOperatorMap[updatedOperatorId];
      ...
      NES_ERROR("Exception occurred during pinned operator removal {}.",
                ex.what());
      throw ex;
    }
  }
