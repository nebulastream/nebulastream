...
Z3ExprAndFieldMapPtr DataTypeToZ3ExprUtil::createForField(
    const std::string& fieldName, const DataTypePtr& dataType,
  ...
  if (dataType->isInteger()) {
    ...
    expr =
        std::make_shared<z3::expr>(context->fpa_const<64>(fieldName.c_str()));
  } else if (dataType->isBoolean()) {
    expr = std::make_shared<z3::expr>(context->bool_const(fieldName.c_str()));
  } else if (dataType->isChar()) {
    expr = std::make_shared<z3::expr>(
        context->constant(fieldName.c_str(), context->string_sort()));
  } else if (dataType->isCharArray()) {  // TODO: Enable Arrays in Z3
    expr = std::make_shared<z3::expr>(
        context->constant(fieldName.c_str(), context->string_sort()));
  } else {
    NES_THROW_RUNTIME_ERROR("Creating Z3 expression is not possible for " +
                            dataType->toString());
  }
...
Z3ExprAndFieldMapPtr DataTypeToZ3ExprUtil::createForDataValue(
    const ValueTypePtr& valueType, const z3::ContextPtr& context) {
  NES_DEBUG("DataTypeToZ3ExprUtil: creating z3 expression for value type");
  if (valueType->dataType->isArray()) {
    NES_THROW_RUNTIME_ERROR(
        "Can't support creating Z3 expression for data value of array type.");
    return nullptr;
  }
  ...
  if (valueTypeType->isUndefined()) {
    NES_THROW_RUNTIME_ERROR(
        "Can't support creating Z3 expression for data value of undefined "
        "type.");
  } else if (valueTypeType->isInteger()) {
    expr = std::make_shared<z3::expr>(
        context->int_val(std::stoi(basicValueType->value)));
  } else if (valueTypeType->isFloat()) {
    expr = std::make_shared<z3::expr>(
        context->fpa_val(std::stod(basicValueType->value)));
    ...
    expr =
        std::make_shared<z3::expr>(context->string_val(basicValueType->value));
  } else if (valueTypeType->isCharArray()) {  // TODO: Enable Arrays for z3.
    expr =
        std::make_shared<z3::expr>(context->string_val(basicValueType->value));
  } else {
    NES_THROW_RUNTIME_ERROR("Creating Z3 expression is not possible for " +
                            valueTypeType->toString());
  }
