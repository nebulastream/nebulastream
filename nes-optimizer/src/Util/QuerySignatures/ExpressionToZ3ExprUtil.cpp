...
Z3ExprAndFieldMapPtr ExpressionToZ3ExprUtil::createForExpression(
    const ExpressionNodePtr& expression, const z3::ContextPtr& context) {
  NES_DEBUG("Creating Z3 expression for input expression {}",
            expression->toString());
...
Z3ExprAndFieldMapPtr ExpressionToZ3ExprUtil::createForArithmeticalExpressions(
    const ExpressionNodePtr& expression, const z3::ContextPtr& context) {
  ...
  if (expression->instanceOf<AddExpressionNode>()) {
    ...
    auto expr = std::make_shared<z3::expr>(
        to_expr(*context, Z3_mk_add(*context, 2, array)));
  ...
  if (expression->instanceOf<SubExpressionNode>()) {
    ...
    Z3_ast array[] = {*left->getExpr(), *right->getExpr()};
    auto expr = std::make_shared<z3::expr>(
        to_expr(*context, Z3_mk_sub(*context, 2, array)));
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    ...
    Z3_ast array[] = {*left->getExpr(), *right->getExpr()};
    auto expr = std::make_shared<z3::expr>(
        to_expr(*context, Z3_mk_mul(*context, 2, array)));
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    ...
    auto expr = std::make_shared<z3::expr>(to_expr(
        *context, Z3_mk_div(*context, *left->getExpr(), *right->getExpr())));
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
  }
  NES_THROW_RUNTIME_ERROR(
...
Z3ExprAndFieldMapPtr ExpressionToZ3ExprUtil::createForLogicalExpressions(
    const ExpressionNodePtr& expression, const z3::ContextPtr& context) {
  ...
  if (expression->instanceOf<OrExpressionNode>()) {
    ...
    Z3_ast array[] = {*left->getExpr(), *right->getExpr()};
    auto expr = std::make_shared<z3::expr>(
        to_expr(*context, Z3_mk_or(*context, 2, array)));
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    ...
    if (left->getExpr()->is_fpa() || right->getExpr()->is_fpa()) {
      auto expr = std::make_shared<z3::expr>(
          to_expr(*context,
                  Z3_mk_fpa_lt(*context, *left->getExpr(), *right->getExpr())));
      return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    }
    auto expr = std::make_shared<z3::expr>(to_expr(
        *context, Z3_mk_lt(*context, *left->getExpr(), *right->getExpr())));
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    ...
    auto left =
        createForExpression(lessEqualsExpressionNode->getLeft(), context);
    auto right =
        createForExpression(lessEqualsExpressionNode->getRight(), context);
    ...
    if (left->getExpr()->is_fpa() || right->getExpr()->is_fpa()) {
      auto expr = std::make_shared<z3::expr>(to_expr(
          *context,
          Z3_mk_fpa_leq(*context, *left->getExpr(), *right->getExpr())));
      return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    }
    auto expr = std::make_shared<z3::expr>(to_expr(
        *context, Z3_mk_le(*context, *left->getExpr(), *right->getExpr())));
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    ...
    auto right =
        createForExpression(greaterExpressionNode->getRight(), context);
    ...
    if (left->getExpr()->is_fpa() || right->getExpr()->is_fpa()) {
      expr = std::make_shared<z3::expr>(
          to_expr(*context,
                  Z3_mk_fpa_gt(*context, *left->getExpr(), *right->getExpr())));
    } else {
      expr = std::make_shared<z3::expr>(to_expr(
          *context, Z3_mk_gt(*context, *left->getExpr(), *right->getExpr())));
    }
    ...
    auto left =
        createForExpression(greaterEqualsExpressionNode->getLeft(), context);
    auto right =
        createForExpression(greaterEqualsExpressionNode->getRight(), context);
    ...
    if (left->getExpr()->is_fpa() || right->getExpr()->is_fpa()) {
      expr = std::make_shared<z3::expr>(to_expr(
          *context,
          Z3_mk_fpa_geq(*context, *left->getExpr(), *right->getExpr())));
    } else {
      expr = std::make_shared<z3::expr>(to_expr(
          *context, Z3_mk_ge(*context, *left->getExpr(), *right->getExpr())));
    }
    return Z3ExprAndFieldMap::create(expr, leftFieldMap);
    ...
    auto expr = std::make_shared<z3::expr>(to_expr(
        *context, Z3_mk_eq(*context, *left->getExpr(), *right->getExpr())));
    ...
    auto updatedExpr = std::make_shared<z3::expr>(
        to_expr(*context, Z3_mk_not(*context, *expr->getExpr())));
