...
createQuerySignatureForOperator(const z3::ContextPtr &context,
  ...
  return QuerySignatureUtil::createQuerySignatureForOperator(context,
                                                             operatorNode);
}
} // namespace Utils

QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForOperator(
    const z3::ContextPtr &context, const OperatorPtr &operatorNode) {

  try {
    NES_DEBUG("QuerySignatureUtil: Creating query signature for operator {}",
    ...
    if (operatorNode->instanceOf<UnaryOperator>()) {
      if (operatorNode->instanceOf<SourceLogicalOperator>() &&
        ...
        NES_THROW_RUNTIME_ERROR(
            "QuerySignatureUtil: Source can't have children : " +
            operatorNode->toString());
      } else if (operatorNode->instanceOf<SinkLogicalOperator>() &&
                 children.empty()) {
        NES_THROW_RUNTIME_ERROR(
            "QuerySignatureUtil: Source can't have empty children set : " +
            operatorNode->toString());
      } else if (!(operatorNode->instanceOf<SourceLogicalOperator>() ||
                   operatorNode->instanceOf<SinkLogicalOperator>()) &&
                 children.size() != 1) {
        NES_THROW_RUNTIME_ERROR(
            "QuerySignatureUtil: Unary operator can have only one children : " +
            operatorNode->toString() +
            " found : " + std::to_string(children.size()));
      }
    } else if (operatorNode->instanceOf<BinaryOperator>() &&
               children.size() != 2) {
      NES_THROW_RUNTIME_ERROR("QuerySignatureUtil: Binary operator can't have "
                              "empty or only one children : " +
                              operatorNode->toString());
    }

    if (operatorNode->instanceOf<SourceLogicalOperator>()) {
      NES_TRACE("QuerySignatureUtil: Computing Signature for Source operator");
      SourceLogicalOperatorPtr sourceOperator =
          operatorNode->as<SourceLogicalOperator>();
    ...
    if (operatorNode->instanceOf<SinkLogicalOperator>()) {
      ...
      NES_ASSERT(!children.empty(),
                 "Sink operator should have at least one child.");
      ...
      NES_TRACE(
          "QuerySignatureUtil: Computing Signature for watermark operator");
      auto watermarkAssignerOperator =
          operatorNode->as<WatermarkAssignerLogicalOperator>();
      return createQuerySignatureForWatermark(context,
      ...
      NES_TRACE(
          "QuerySignatureUtil: Computing Signature for infer model operator");
      auto imOperator =
          operatorNode->as<InferModel::LogicalInferModelOperator>();
      return createQuerySignatureForInferModel(context, imOperator);
    }
    throw SignatureComputationException(
        "No conversion to Z3 expression possible for operator: " +
        operatorNode->toString());
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForSource(
  ...
  for (auto &field : outputSchema->fields) {
    ...
    auto fieldExpr = DataTypeToZ3ExprUtil::createForField(
                         fieldName, field->getDataType(), context)
                         ->getExpr();
    fieldToZ3ExprMap[fieldName] = fieldExpr;
  }
  auto updatedSchemaFieldToExprMaps = {fieldToZ3ExprMap};

  // Create an equality expression for example: <logical source
  // name>.logicalSourceName == "<logical source name>"
  std::string sourceName =
      sourceOperator->getSourceDescriptor()->getLogicalSourceName();
  auto sourceNameVarName = sourceName + ".logicalSourceName";
  auto sourceNameVar = context->constant(
      context->str_symbol(sourceNameVarName.c_str()), context->string_sort());
  auto sourceNameVal = context->string_val(sourceName);
  // Construct Z3 expression using source variable name and source variable
  // value
  auto conditions = std::make_shared<z3::expr>(
      to_expr(*context, Z3_mk_eq(*context, sourceNameVar, sourceNameVal)));

  // Compute signature
  return QuerySignature::create(std::move(conditions), std::move(columns),
                                updatedSchemaFieldToExprMaps, {}, {});
}

QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForProject(
  ...
  NES_ASSERT(children.size() == 1,
             "Project operator should only have one and non null children.");
  auto childQuerySignature =
      children[0]->as<LogicalOperator>()->getZ3Signature();
  ...
  // Iterate over schema field to expression maps of the upstream child and
  // create new schema map based on
  ...
  for (auto &schemaFieldToExprMap : schemaFieldToExprMaps) {
    ...
    // Iterate over projection expression and select the column name and
    // expression from the schemaField to expression map of upstream operator
    for (auto &expression : expressions) {

      // Identify the new field name and the old field name in the upstream
      // operator
      ...
      if (found == schemaFieldToExprMap.end()) {
        NES_THROW_RUNTIME_ERROR(
            "QuerySignatureUtil: Unable to find projected field " + fieldName +
  ...
  return QuerySignature::create(
      std::move(conditions), std::move(updatedColumns),
      std::move(updatedSchemaFieldToExprMaps), std::move(windowExpressions),
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForInferModel(
  ...
  NES_ASSERT(children.size() == 1,
             "InferModel operator should only have one non null children.");
  ...
  for (auto &schemaFieldToExprMap : schemaFieldToExprMaps) {
    ...
    for (auto in_field : inputfields) {
      auto fieldname =
          in_field->as<FieldAccessExpressionNode>()->getFieldName();
    }
  ...
  // Create an equality expression for example: <logical stream name>.streamName
  // == "<logical stream name>"
  std::string modelName = inferModelOperator->getModel();
  auto modelNameVarName = modelName + ".model";
  auto streamNameVar = context->constant(
      context->str_symbol(modelNameVarName.c_str()), context->string_sort());
  auto streamNameVal = context->string_val(modelName);

  // Construct Z3 expression using ML model name
  auto modelCondtition =
      to_expr(*context, Z3_mk_eq(*context, streamNameVar, streamNameVal));
  auto childConditions = childQuerySignature->getConditions();
  Z3_ast array[] = {modelCondtition, *childConditions};
  auto conditions = std::make_shared<z3::expr>(
      to_expr(*context, Z3_mk_and(*context, 2, array)));
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForMap(
    const z3::ContextPtr &context, const LogicalMapOperatorPtr &mapOperator) {

  // Fetch query signature of the child operator
  std::vector<NodePtr> children = mapOperator->getChildren();
  NES_ASSERT(children.size() == 1,
             "Map operator should only have one non null children.");
  auto child = children[0];
  auto childQuerySignature = child->as<LogicalOperator>()->getZ3Signature();
  auto exprAndFieldMap = ExpressionToZ3ExprUtil::createForExpression(
      mapOperator->getMapExpression(), context);
  ...
  for (auto &schemaFieldToExprMap : schemaFieldToExprMaps) {
    ...
    for (auto &[operandExprName, operandExpr] : rhsOperandFieldMap) {
      ...
      if (found == schemaFieldToExprMap.end()) {
        NES_THROW_RUNTIME_ERROR("QuerySignatureUtil: " + operandExprName +
                                " doesn't exists");
      }
      ...
      // Perform replacement
      updatedMapExpr =
          std::make_shared<z3::expr>(updatedMapExpr->substitute(from, to));
    }
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForFilter(
    const z3::ContextPtr &context,
  ...
  auto child = children[0];
  auto childQuerySignature = child->as<LogicalOperator>()->getZ3Signature();
  auto filterExprAndFieldMap = ExpressionToZ3ExprUtil::createForExpression(
      filterOperator->getPredicate(), context);
  auto filterFieldMap = filterExprAndFieldMap->getFieldMap();
  auto filterExpr = filterExprAndFieldMap->getExpr();

  NES_TRACE("QuerySignatureUtil: Replace Z3 Expression for the filed with "
            "corresponding column values from "
            "children signatures");
  // Fetch the signature of only children and get the column values
  auto schemaFieldToExprMaps = childQuerySignature->getSchemaFieldToExprMaps();
  ...
  for (auto &schemaFieldToExprMap : schemaFieldToExprMaps) {
    ...
    for (auto &[operandExprName, operandExpr] : filterFieldMap) {
      ...
      // Perform replacement
      updatedExpr =
          std::make_shared<z3::expr>(updatedExpr->substitute(from, to));
    }
    filterExpressions.push_back(*updatedExpr);
  }

  // disjunction needed in some cases, e.g. when two different maps are before a
  // union and a filter is applied later
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForWatermark(
  ...
  if (watermarkDescriptor
    ...
    auto allowedLateness =
        eventTimeWatermarkStrategy->getAllowedLateness().getTime();
    auto allowedLatenessVal = context->int_val(allowedLateness);
    auto allowedLatenessExpr = to_expr(
        *context, Z3_mk_eq(*context, allowedLatenessVar, allowedLatenessVal));

    // Compute equality conditions for event time field
    auto eventTimeFieldName = eventTimeWatermarkStrategy->getOnField()
                                  ->as<FieldAccessExpressionNode>()
                                  ->getFieldName();
    auto eventTimeFieldNameAndSource =
        NES::Util::splitWithStringDelimiter<std::string>(eventTimeFieldName,
                                                         "$")[0] +
        "." + "eventTimeField";
    auto eventTimeFieldVar = context->constant(
        context->str_symbol(eventTimeFieldNameAndSource.c_str()),
        context->string_sort());
    auto eventTimeFieldVal = context->string_val(eventTimeFieldName);
    auto eventTimeFieldExpr = to_expr(
        *context, Z3_mk_eq(*context, eventTimeFieldVar, eventTimeFieldVal));
    ...
    watermarkDescriptorConditions =
        to_expr(*context, Z3_mk_and(*context, 2, andConditions));
  } else if (watermarkDescriptor->instanceOf<
                 Windowing::IngestionTimeWatermarkStrategyDescriptor>()) {
    // Create an equality expression <source>.watermarkAssignerType ==
    // "IngestionTime"
    auto var = context->constant(context->str_symbol("watermarkAssignerType"),
                                 context->string_sort());
    auto val = context->constant(context->str_symbol("IngestionTime"),
                                 context->string_sort());
    watermarkDescriptorConditions =
        to_expr(*context, Z3_mk_eq(*context, var, val));
  } else {
    NES_THROW_RUNTIME_ERROR(
        "QuerySignatureUtil: Unrecognized watermark descriptor found.");
  }

  // CNF the watermark conditions to the original condition
  Z3_ast andConditions[] = {*conditions, watermarkDescriptorConditions};
  conditions = std::make_shared<z3::expr>(
      z3::to_expr(*context, Z3_mk_and(*context, 2, andConditions)));
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForUnion(
    const z3::ContextPtr &context,
  ...
  QuerySignaturePtr rightSignature =
      children[1]->as<LogicalOperator>()->getZ3Signature();
  ...
  NES_TRACE("LeftSignature Conditions: {}",
            leftSignature->getConditions()->to_string());
  NES_TRACE("RightSignature Conditions: {}",
            rightSignature->getConditions()->to_string());
  ...
  NES_TRACE("LeftSignature source name qualifier: {}",
            unionOperator->getLeftInputSchema()->getSourceNameQualifier());
  std::map<std::string, z3::ExprPtr> combinedUnionExpressions =
      leftSignature->getUnionExpressions();
  for (const auto &[sourceName, conditions] :
       rightSignature->getUnionExpressions()) {
    combinedUnionExpressions[sourceName] = conditions;
  }

  combinedUnionExpressions[unionOperator->getRightInputSchema()
                               ->getSourceNameQualifier()] =
      rightSignature->getConditions();
  combinedUnionExpressions[unionOperator->getLeftInputSchema()
                               ->getSourceNameQualifier()] =
      leftSignature->getConditions();

  // Create a CNF using all conditions from children signatures
  z3::ExprPtr conditions =
      std::make_shared<z3::expr>(z3::mk_and(allConditions));
  return QuerySignature::create(std::move(conditions), std::move(leftColumns),
                                std::move(updatedSchemaFieldToExprMaps),
                                std::move(combinedWindowExpressions),
                                std::move(combinedUnionExpressions));
}

QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForJoin(
    const z3::ContextPtr &context, const LogicalJoinOperatorPtr &joinOperator) {
  ...
  if (children.size() != 2) {
    NES_THROW_RUNTIME_ERROR("Join operator can have only 2 children. Found " +
  ...
  // std::make_pair(leftJoinKeyNameEqui,rightJoinKeyNameEqui);
  auto equiJoinKeyNames =
      NES::findEquiJoinKeyNames(joinDefinition->getJoinExpression());
  ...
  // Iterate over all left and right schemas and compute new join predicates and
  // schemas
  ...
  for (auto &leftSchemaMap : leftSchemaFieldToExprMaps) {
    ...
    for (auto &rightSchemaMap : rightSchemaFieldToExprMaps) {
      // Compute the new field to z3 expression map by inserting all fields from
      // left and right children
      std::map<std::string, z3::ExprPtr> updatedFieldToZ3ExprMap;
      updatedFieldToZ3ExprMap.insert(leftSchemaMap.begin(),
                                     leftSchemaMap.end());
      updatedFieldToZ3ExprMap.insert(rightSchemaMap.begin(),
                                     rightSchemaMap.end());
      //
      ...
      auto joinPredicate = z3::to_expr(
          *context, Z3_mk_eq(*context, *leftPredicate, *rightPredicate));
  ...
  Z3_ast andConditions[] = {*leftSignature->getConditions(),
                            *rightSignature->getConditions(), joinCondition};
  auto conditions = std::make_shared<z3::expr>(z3::to_expr(
      *context, z3::to_expr(*context, Z3_mk_and(*context, 3, andConditions))));

  // Compute the expression for window time key
  auto windowType =
      joinDefinition->getWindowType()->as<Windowing::TimeBasedWindowType>();
  auto timeCharacteristic = windowType->getTimeCharacteristic();
  // FIXME: problem is that only one time key is defined during the join
  // definition
  //  fix it as part of #1592
  //     z3::expr windowTimeKeyVal(*context);
  //     Windowing::TimeCharacteristic::Type type =
  //     timeCharacteristic->getType(); if (type ==
  //     Windowing::TimeCharacteristic::EventTime) {
  //         windowTimeKeyVal =
  //         context->string_val(timeCharacteristic->getField()->getName());
  //     } else if (type == Windowing::TimeCharacteristic::IngestionTime) {
  //         windowTimeKeyVal =
  //         context->string_val(timeCharacteristic->getField()->getName());
  //     } else {
  //         NES_THROW_RUNTIME_ERROR("QuerySignatureUtil: Unknown window Time
  //         Characteristic");
  //     }
  //     auto windowTimeKeyVar =
  //     context->constant(context->str_symbol("time-key"),
  //     context->string_sort()); auto windowTimeKeyExpression =
  //     to_expr(*context, Z3_mk_eq(*context, windowTimeKeyVar,
  //     windowTimeKeyVal));
  ...
  auto windowTimeSizeExpression = to_expr(
      *context, Z3_mk_le(*context, windowTimeSizeVar, windowTimeSizeVal));
  auto windowTimeSlideExpression = to_expr(
      *context, Z3_mk_le(*context, windowTimeSlideVar, windowTimeSlideVal));

  // Compute join window key expression
  auto windowKeyVar = context->constant(context->str_symbol("window-key"),
                                        context->string_sort());
  std::string windowKey = "JoinWindow";
  z3::expr windowKeyVal = context->string_val(windowKey);
  auto windowKeyExpression =
      to_expr(*context, Z3_mk_eq(*context, windowKeyVar, windowKeyVal));

  // Compute the CNF based on the window-key, window-size, and window-slide
  Z3_ast expressionArray[] = {windowKeyExpression, windowTimeSlideExpression,
  ...
  joinWindowExpression.insert(
      {"z3-window-expressions",
       std::make_shared<z3::expr>(
           z3::to_expr(*context, Z3_mk_and(*context, 3, expressionArray)))});
  // Need number of aggregates, aggregate type, and window id for heuristic
  // checks for query containment identification
  joinWindowExpression.insert(
      {"number-of-aggregates",
       std::make_shared<z3::expr>(context->int_val(0))});
  joinWindowExpression.insert(
      {"aggregate-types", std::make_shared<z3::expr>(context->string_val(""))});
  joinWindowExpression.insert(
      {"window-id",
       std::make_shared<z3::expr>(context->string_val(windowKey))});
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForWindow(
    const z3::ContextPtr &context,
    const LogicalWindowOperatorPtr &windowOperator) {

  // Fetch query signature of the child operator
  std::vector<NodePtr> children = windowOperator->getChildren();
  NES_ASSERT(children.size() == 1 && children[0],
             "Window operator should only have one non null child.");
  auto child = children[0];
  auto childQuerySignature = child->as<LogicalOperator>()->getZ3Signature();
  ...
  if (windowDefinition->isKeyed()) {
    windowKey =
        windowOperator->getInputSchema()->getSourceNameQualifier() + "$";
  ...
  if (windowType->instanceOf<Windowing::TimeBasedWindowType>()) {
    ...
    if (timeCharacteristic->getType() ==
      ...
      NES_ERROR(
          "QuerySignatureUtil: Cant serialize window Time Characteristic");
    }
    auto windowTimeKeyVar = context->constant(context->str_symbol("time-key"),
                                              context->string_sort());
    auto windowTimeKeyExpression = to_expr(
        *context, Z3_mk_eq(*context, windowTimeKeyVar, windowTimeKeyVal));
    ...
    windowExpression.insert(
        {"window-id", std::make_shared<z3::expr>(windowId)});
    ...
    if (timeBasedWindowType->instanceOf<Windowing::TumblingWindow>()) {
      auto tumblingWindow =
          timeBasedWindowType->as<Windowing::TumblingWindow>();
      length = tumblingWindow->getSize().getTime() * multiplier;
      slide = length;
      ...
      NES_THROW_RUNTIME_ERROR(
          "QuerySignatureUtil: Unknown window Time Characteristic");
    }
    auto windowTimeSizeVar = context->int_const("window-time-size");
    z3::expr windowTimeSizeVal = context->int_val(length);
    auto windowTimeSlideVar = context->int_const("window-time-slide");
    ...
    // Compute the CNF based on the window-key, window-time-key, window-size,
    // and window-slide
    Z3_ast expressionArray[] = {windowKeyExpression, windowTimeKeyExpression,
                                windowTimeSlideExpression,
                                windowTimeSizeExpression};
    windowExpression.insert(
        {"z3-window-expressions",
         std::make_shared<z3::expr>(
             z3::to_expr(*context, Z3_mk_and(*context, 4, expressionArray)))});
    ...
    windowExpression.insert(
        {"z3-window-expressions",
         std::make_shared<z3::expr>(
             z3::to_expr(*context, Z3_mk_and(*context, 1, expressionArray)))});
  }
  ...
  for (auto windowAggregation : windowDefinition->getWindowAggregation()) {
    ...
    onFieldNames.push_back(windowAggregation->on()
                               ->as<FieldAccessExpressionNode>()
                               ->getFieldName());
    asFieldNames.push_back(windowAggregation->as()
                               ->as<FieldAccessExpressionNode>()
                               ->getFieldName());
    aggregateTypes += (std::to_string(magic_enum::enum_integer(
                           windowAggregation->getType())) +
                       ".");
    allAggregates.push_back(aggregate);
  }
  auto schemaFieldToExprMaps = childQuerySignature->getSchemaFieldToExprMaps();
  auto outputSchema = windowOperator->getOutputSchema();
  // number of aggregates, and aggregate type for heuristic checks for query
  // containment identification
  uint64_t numberOfAggregates = allAggregates.size();
  windowExpression.insert(
      {"number-of-aggregates",
       std::make_shared<z3::expr>(context->int_val(numberOfAggregates))});
  windowExpression.insert(
      {"aggregate-types",
       std::make_shared<z3::expr>(context->string_val(aggregateTypes))});
  windowExpression.insert({"window-time-size", std::make_shared<z3::expr>(
                                                   context->int_val(length))});
  windowExpression.insert({"window-time-slide", std::make_shared<z3::expr>(
                                                    context->int_val(slide))});
    ...
    for (auto &outputField : outputSchema->fields) {
      ...
      if (originalAttributeName.find("start") != std::string ::npos ||
        ...
        updatedSchemaMap[originalAttributeName] =
            DataTypeToZ3ExprUtil::createForField(
                originalAttributeName, outputField->getDataType(), context)
                ->getExpr();
      } else if (Util::splitWithStringDelimiter<std::string>(
                     originalAttributeName, "$")[1] == "count") {
        NES_TRACE("Count Attribute");
        auto fieldAggregation = allAggregates[std::distance(
            asFieldNames.begin(),
            std::find(asFieldNames.begin(), asFieldNames.end(),
                      originalAttributeName))];
        auto expr = std::make_shared<z3::expr>(
            context->int_const(originalAttributeName.c_str()));
        auto updatedFieldExpr = std::make_shared<z3::expr>(
            z3::to_expr(*context, fieldAggregation(*expr)));
        NES_TRACE("UpdatedFieldExpr: {}", updatedFieldExpr->to_string());
        updatedSchemaMap[originalAttributeName] = updatedFieldExpr;
      } else if (std::find(asFieldNames.begin(), asFieldNames.end(),
                           originalAttributeName) != asFieldNames.end()) {
        ...
        auto fieldAggregation = allAggregates[std::distance(
            ...
            std::find(asFieldNames.begin(), asFieldNames.end(),
                      originalAttributeName))];
        auto updatedFieldExpr = std::make_shared<z3::expr>(
            z3::to_expr(*context, fieldAggregation(*fieldExpr)));
        updatedSchemaMap[originalAttributeName] = updatedFieldExpr;
      } else {
        updatedSchemaMap[originalAttributeName] =
            schemaFieldToExprMap[originalAttributeName];
      }
    }
    updatedSchemaFieldToExprMaps.emplace_back(updatedSchemaMap);
  }
