...
void SharedQueryPlan::addQuery(
  ...
  NES_DEBUG("SharedQueryPlan: Add the matched operators of query with id {} to "
            "the shared query plan.",
            queryId);
  ...
  for (const auto &matchedOperatorPair : matchedOperatorPairs) {
    ...
    case Optimizer::ContainmentRelationship::EQUALITY:
      ...
      if (hostOperator->instanceOf<SinkLogicalOperator>() &&
        ...
        // Make a copy of the target operator so that we do not have to perform
        ...
        // fetch all upstream operators of the host operator and add the target
        // operator as their parent operator
        ...
        // fetch all root operator of the target operator to compute downstream
        // operator list for the change log entry
    ...
    case Optimizer::ContainmentRelationship::LEFT_SIG_CONTAINED:
      // Here we need to re-deploy the whole shared query plan and therefore add
      // all source operators to the change log
    ...
    case Optimizer::ContainmentRelationship::NO_CONTAINMENT:
      NES_WARNING("NO_CONTAINMENT relationship was passed to "
...
bool SharedQueryPlan::removeQuery(QueryId queryId) {
  NES_DEBUG("SharedQueryPlan: Remove the Query Id {} and associated Global "
            "Query Nodes with sink operators.",
            queryId);
  ...
  NES_TRACE("SharedQueryPlan: Remove the Global Query Nodes with sink "
            "operators for query  {}",
            queryId);
  ...
  for (const auto &sinkOperator : sinkOperatorsToRemove) {
    ...
    if (upstreamOperators.empty()) {
      NES_ERROR("SharedQueryPlan: unable to remove Root operator from the "
                "shared query plan {}",
                sharedQueryId);
      return false;
    }
...
std::vector<LogicalOperatorPtr> SharedQueryPlan::getSinkOperators() {
  NES_TRACE("SharedQueryPlan: Get all Global Query Nodes with sink operators "
...
void SharedQueryPlan::recordFailedChangeLogEntries(
  ...
  for (const auto &changeLogEntry : changeLogEntries) {
    ...
    if (newDownStreamOperators.empty()) {
      NES_ERROR("Unable to find the most downstream operator. Skipping rest of "
                "the operation. The failed changelog will "
                "not be processed.");
      continue;
    }
    ...
    if (newUpstreamOperators.empty()) {
      NES_ERROR("Unable to find the most upstream operator. Skipping rest of "
                "the operation. The failed changelog will "
                "not be processed.");
      continue;
    }
