...
SharedQueryPlan::SharedQueryPlan(const QueryPlanPtr& queryPlan)
    : sharedQueryId(PlanIdGenerator::getNextSharedQueryId()),
      sharedQueryPlanStatus(SharedQueryPlanStatus::CREATED) {
  // Create a new query plan
  this->queryPlan = queryPlan->copy();
  this->queryPlan->setQueryId(UNSURE_CONVERSION_TODO_4761(
      sharedQueryId,
      QueryId));  // overwrite the query id with shared query plan id
  ...
  hashBasedSignatures =
      (*sinkOperators.begin())->as<LogicalOperator>()->getHashBasedSignature();
  // The query id
  ...
  changeLog->addChangeLogEntry(now,
                               Optimizer::Experimental::ChangeLogEntry::create(
                                   upstreamOperators, downstreamOperators));
}
...
void SharedQueryPlan::addQuery(
    QueryId queryId, const std::vector<Optimizer::MatchedOperatorPairPtr>&
                         matchedOperatorPairs) {
  NES_DEBUG(
      "SharedQueryPlan: Add the matched operators of query with id {} to the "
      "shared query plan.",
      queryId);
  ...
  for (const auto& matchedOperatorPair : matchedOperatorPairs) {
    ...
    switch (matchedOperatorPair->containmentRelationship) {
      case Optimizer::ContainmentRelationship::EQUALITY:
        // If host and target operator are of sink type then connect the target
        // sink to the upstream of the host sink.
        if (hostOperator->instanceOf<SinkLogicalOperator>() &&
            targetOperator->instanceOf<SinkLogicalOperator>()) {
          // Make a copy of the target operator so that we do not have to
          // perform additional operation to
          //  add it to the shared query plan.
          //  Note: we otherwise have to remove the upstream operator of the
          //  target to decouple it from the original target plan.
          auto targetOperatorCopy =
              targetOperator->copy()->as<LogicalOperator>();

          // fetch all upstream operators of the host operator and add the
          // target operator as their parent operator
          for (const auto& hostUpstreamOperator : hostOperator->getChildren()) {
            ...
            clEntryUpstreamOperators.insert(
                hostUpstreamOperator->as<LogicalOperator>());
          }
          ...
          // If host operator is of sink type then connect the downstream
          // operators of target operator to the upstream of the host operator.
          ...
          // Get the downstream operator of the target and add them as the
          // downstream operator to the host operator
          auto downstreamOperatorsOfTarget = targetOperator->getParents();
          for (const auto& downstreamOperatorOfTarget :
               downstreamOperatorsOfTarget) {
            // Clear as upstream the target operator
            downstreamOperatorOfTarget->removeChild(targetOperator);
            // fetch all upstream operators of the host operator and add the
            // target operator as their parent operator
            for (const auto& hostUpstreamOperator :
                 hostOperator->getChildren()) {
              // add target operator as the parent to the host upstream operator
          ...
          // If target operator is of sink type then connect the target sink to
          // the host operator.
          ...
          // If both host and target operator are not of sink type then connect
          // the downstream operators of target operator to the host operator.
        } else {
          // set host operator as the upstream operator in the change log
          clEntryUpstreamOperators.insert(hostOperator);

          // fetch all root operator of the target operator to compute
          // downstream operator list for the change log entry
          for (const auto& newRootOperator :
               targetOperator->getAllRootNodes()) {
            clEntryDownstreamOperators.insert(
                newRootOperator->as<LogicalOperator>());
            // add the new sink operators as root to the set
            sinkOperators.insert(newRootOperator->as<LogicalOperator>());
          }

          // add all downstream operators of the target operator as downstream
          // operator to the host operator
          auto downstreamTargetOperators = targetOperator->getParents();
          for (const auto& downstreamTargetOperator :
               downstreamTargetOperators) {
            // Clear as upstream the target operator
            downstreamTargetOperator->removeChild(targetOperator);
            // add host operator as the upstream operator to the
            // downstreamTargetOperator
      ...
      case Optimizer::ContainmentRelationship::LEFT_SIG_CONTAINED:
        // Here we need to re-deploy the whole shared query plan and therefore
        // add all source operators to the change log
        for (const auto& sourceOperator : hostOperator->getAllLeafNodes()) {
          clEntryUpstreamOperators.insert(
              sourceOperator->as<LogicalOperator>());
        }
        // We also need to add all root operators from the existing shared query
        // plan
        for (const auto& existingRootOperatorsToBeReDeployed :
             targetOperator->getAllRootNodes()) {
          clEntryDownstreamOperators.insert(
              existingRootOperatorsToBeReDeployed->as<LogicalOperator>());
        }
        ...
        // add the new sink operators as root to the set
        // in case of left signature containment, the host operator marks the
        // newly added query plan
        sinkOperators.insert(
            hostOperator->getAllRootNodes().front()->as<LogicalOperator>());
        break;
      // In case of no containment, do nothing
      case Optimizer::ContainmentRelationship::NO_CONTAINMENT:
        NES_WARNING(
            "NO_CONTAINMENT relationship was passed to "
            "SharedQueryPlan::addQuery(). This should not happen. "
            "Please check the code.");
        break;
    }

    // Add new hash based signatures to the shared query plan for newly added
    // downstream operators
    for (const auto& newDownstreamOperator : clEntryDownstreamOperators) {
      auto hashBasedSignature =
          newDownstreamOperator->as<LogicalOperator>()->getHashBasedSignature();
    ...
    changeLog->addChangeLogEntry(
        now, Optimizer::Experimental::ChangeLogEntry::create(
                 clEntryUpstreamOperators, clEntryDownstreamOperators));
  }
  ...
  NES_TRACE("Size of queryIdToSinkOperatorMap: {}",
            queryIdToSinkOperatorMap.size());
...
bool SharedQueryPlan::removeQuery(QueryId queryId) {
  NES_DEBUG(
      "SharedQueryPlan: Remove the Query Id {} and associated Global Query "
      "Nodes with sink operators.",
      queryId);
  if (queryIdToSinkOperatorMap.find(queryId) ==
      queryIdToSinkOperatorMap.end()) {
    NES_ERROR(
        "SharedQueryPlan: query id {} is not present in metadata information.",
        queryId);
    return false;
  }

  NES_TRACE(
      "SharedQueryPlan: Remove the Global Query Nodes with sink operators for "
      "query  {}",
      queryId);
  std::set<LogicalOperatorPtr> sinkOperatorsToRemove =
      queryIdToSinkOperatorMap[queryId];
  // Iterate over all sink global query nodes for the input query and remove the
  // corresponding exclusive upstream operator chains
  for (const auto& sinkOperator : sinkOperatorsToRemove) {
    ...
    if (upstreamOperators.empty()) {
      NES_ERROR(
          "SharedQueryPlan: unable to remove Root operator from the shared "
          "query plan {}",
          sharedQueryId);
      return false;
    }
    ...
    auto now = std::chrono::duration_cast<std::chrono::microseconds>(
                   ...
                   .count();
    changeLog->addChangeLogEntry(
        now, Optimizer::Experimental::ChangeLogEntry::create(upstreamOperators,
                                                             {sinkOperator}));
  }
...
bool SharedQueryPlan::isEmpty() {
  NES_TRACE(
      "SharedQueryPlan: Check if Global Query Metadata is empty. Found :  {}",
      queryIdToSinkOperatorMap.empty());
  return queryIdToSinkOperatorMap.empty();
}

std::vector<LogicalOperatorPtr> SharedQueryPlan::getSinkOperators() {
  NES_TRACE(
      "SharedQueryPlan: Get all Global Query Nodes with sink operators for the "
      "current Metadata");
...
std::map<QueryId, std::set<LogicalOperatorPtr>>
SharedQueryPlan::getQueryIdToSinkOperatorMap() {
...
std::set<LogicalOperatorPtr> SharedQueryPlan::markOperatorsToBeRemoved(
    const NES::LogicalOperatorPtr& operatorToRemove) {
  ...
  for (const auto& upstreamOperator : upstreamOperators) {
    ...
    // If chas more than 1 downstream operators and not all downstream operators
    // of this upstream operator is in to-be-removed state then add this
    // operator
    //  to the upstream operators to return.
    if (connectedDownstream.size() > 1) {
      bool allInToBeRemoved = std::any_of(
          connectedDownstream.begin(), connectedDownstream.end(),
          [](const NodePtr connectedDownStreamOperator) {
            return connectedDownStreamOperator->as_if<LogicalOperator>()
                       ->getOperatorState() == OperatorState::TO_BE_REMOVED;
          });

      if (!allInToBeRemoved) {
        // add this upstream operator to operators to return
        upstreamOperatorsToReturn.insert(
    ...
    auto lastUpstreamOperators =
        markOperatorsToBeRemoved(upstreamOperator->as<LogicalOperator>());
    // add returned operators to operators to return
    upstreamOperatorsToReturn.insert(lastUpstreamOperators.begin(),
                                     lastUpstreamOperators.end());
  }
  return upstreamOperatorsToReturn;
}

void SharedQueryPlan::removeOperator(
    const LogicalOperatorPtr& operatorToRemove) {
  // Iterate over all child operator
  auto upstreamOperators = operatorToRemove->getChildren();
  ...
  for (const auto& upstreamOperator : upstreamOperators) {
    // If the upstream operator is in the state REMOVED then recursively process
    // its upstream operators.
    if (upstreamOperator->as_if<LogicalOperator>()->getOperatorState() ==
        OperatorState::REMOVED) {
...
void SharedQueryPlan::recordFailedChangeLogEntries(
    std::vector<Optimizer::Experimental::ChangeLogEntryPtr> changeLogEntries) {
  for (const auto& changeLogEntry : changeLogEntries) {
    ...
    while (!downStreamOperatorsToCheck.empty()) {
      ...
      if (!isDownStreamCandidate) {
        for (const auto& upstreamOperator : upstreamOperators) {
          downStreamOperatorsToCheck.emplace(
              upstreamOperator->as<LogicalOperator>());
        }
      }
    }
    ...
    if (newDownStreamOperators.empty()) {
      NES_ERROR(
          "Unable to find the most downstream operator. Skipping rest of the "
          "operation. The failed changelog will "
          "not be processed.");
      continue;
    }
    ...
    while (!upStreamOperatorsToCheck.empty()) {
      ...
      for (const auto& downstreamOperator : downstreamOperators) {
        if (downstreamOperator->as_if<LogicalOperator>()->getOperatorState() !=
            OperatorState::PLACED) {
      ...
      if (!isDownStreamCandidate) {
        for (const auto& downstreamOperator : downstreamOperators) {
          upStreamOperatorsToCheck.emplace(
              downstreamOperator->as<LogicalOperator>());
        }
      }
    }
    ...
    if (newUpstreamOperators.empty()) {
      NES_ERROR(
          "Unable to find the most upstream operator. Skipping rest of the "
          "operation. The failed changelog will "
          "not be processed.");
      continue;
    }
    ...
    changeLog->addChangeLogEntry(
        now, Optimizer::Experimental::ChangeLogEntry::create(
                 newUpstreamOperators, newDownStreamOperators));
  }
}

std::map<size_t, std::set<std::string>>
SharedQueryPlan::getHashBasedSignature() {
  return hashBasedSignatures;
}

void SharedQueryPlan::updateHashBasedSignature(
    size_t hashValue, const std::string& stringSignature) {
...
SharedQueryPlanStatus SharedQueryPlan::getStatus() const {
  return sharedQueryPlanStatus;
}

void SharedQueryPlan::setStatus(SharedQueryPlanStatus newStatus) {
  this->sharedQueryPlanStatus = newStatus;
}

Optimizer::PlacementStrategy SharedQueryPlan::getPlacementStrategy() const {
...
void SharedQueryPlan::performReOperatorPlacement(
    const std::set<OperatorId>& upstreamOperatorIds,
  ...
  for (const auto& upstreamOperatorId : upstreamOperatorIds) {
    upstreamLogicalOperators.emplace(
        queryPlan->getOperatorWithOperatorId(upstreamOperatorId)
            ->as<LogicalOperator>());
  }

  std::set<LogicalOperatorPtr> downstreamLogicalOperators;
  for (const auto& downstreamOperatorId : downstreamOperatorIds) {
    downstreamLogicalOperators.emplace(
        queryPlan->getOperatorWithOperatorId(downstreamOperatorId)
            ->as<LogicalOperator>());
  }

  std::set<OperatorPtr> downstreamOperator{downstreamLogicalOperators.begin(),
                                           downstreamLogicalOperators.end()};
  std::set<OperatorPtr> upstreamOperator{upstreamLogicalOperators.begin(),
                                         upstreamLogicalOperators.end()};

  auto operatorsToBeRePlaced =
      queryPlan->findAllOperatorsBetween(downstreamOperator, upstreamOperator);

  for (const auto& operatorToRePlace : operatorsToBeRePlaced) {
    operatorToRePlace->as_if<LogicalOperator>()->setOperatorState(
        OperatorState::TO_BE_REPLACED);
  }
  ...
  changeLog->addChangeLogEntry(
      now, Optimizer::Experimental::ChangeLogEntry::create(
               upstreamLogicalOperators, downstreamLogicalOperators));
}

void SharedQueryPlan::updateOperators(
    const std::set<LogicalOperatorPtr>& updatedOperators) {
  // Iterate over all updated operators and update the corresponding operator in
  // the shared query plan with correct properties and state.
  for (const auto& placedOperator : updatedOperators) {
    auto topologyNodeId = std::any_cast<WorkerId>(
        placedOperator->getProperty(Optimizer::PINNED_WORKER_ID));
    auto operatorInQueryPlan =
        queryPlan->getOperatorWithOperatorId(placedOperator->getId());
    operatorInQueryPlan->addProperty(Optimizer::PINNED_WORKER_ID,
                                     topologyNodeId);
