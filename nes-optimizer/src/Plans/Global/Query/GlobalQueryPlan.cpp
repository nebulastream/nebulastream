...
void GlobalQueryPlan::addQueryPlan(const QueryPlanPtr & queryPlan)
{
    ...
    {
        throw GlobalQueryPlanUpdateException(fmt::format("GlobalQueryPlan: Query plan with id {} already present.", inputQueryPlanId));
    }
    ...
    else if (RequestType::StopQuery == requestType)
    {
        ...
        if (queryIdToSharedQueryIdMap.contains(queryId))
        {
            ...
            if (!sharedQueryPlan->removeQuery(queryId))
            {
                ...
                throw Exceptions::RuntimeException(fmt::format(
                    "GlobalQueryPlan: Unable to remove query with id {} from shared query plan with id {}", queryId, sharedQueryId));
            }
        ...
        else
        {
            ...
            queryPlansToAdd.erase(std::find_if(
                queryPlansToAdd.begin(),
                queryPlansToAdd.end(),
                [&queryId](const QueryPlanPtr & queryPlan) { return queryPlan->getQueryId() == queryId; }));
        }
    }
    else
    {
...
void GlobalQueryPlan::removeFailedOrStoppedSharedQueryPlans()
{
    ...
    for (auto itr = sharedQueryIdToPlanMap.begin(); itr != sharedQueryIdToPlanMap.end();)
    {
        ...
        if (sharedQueryPlan->getStatus() == SharedQueryPlanStatus::FAILED || sharedQueryPlan->getStatus() == SharedQueryPlanStatus::STOPPED)
        {
...
std::vector<SharedQueryPlanPtr> GlobalQueryPlan::getSharedQueryPlansConsumingSourcesAndPlacementStrategy(
    const std::string & sourceNames, Optimizer::PlacementStrategy placementStrategy)
{
