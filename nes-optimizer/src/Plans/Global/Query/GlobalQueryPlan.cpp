...
std::vector<SharedQueryPlanPtr> GlobalQueryPlan::getSharedQueryPlansToDeploy() {
  ...
  for (auto& [sharedQueryId, sharedQueryPlan] : sharedQueryIdToPlanMap) {
    if (SharedQueryPlanStatus::DEPLOYED == sharedQueryPlan->getStatus()) {
      NES_TRACE(
          "GlobalQueryPlan: Skipping! found already deployed shared query "
          "plan.");
      continue;
    }
...
void GlobalQueryPlan::removeSharedQueryPlan(SharedQueryId sharedQueryPlanId) {
  NES_INFO(
      "GlobalQueryPlan: remove metadata information for empty shared query "
      "plan id {}",
      sharedQueryPlanId);
  ...
  if (sharedQueryPlan->getStatus() == SharedQueryPlanStatus::STOPPED ||
    ...
    NES_TRACE(
        "Found stopped or failed query plan. Removing query plan from shared "
        "query plan.");
...
SharedQueryPlanPtr GlobalQueryPlan::getSharedQueryPlan(
...
std::vector<QueryId> GlobalQueryPlan::getQueryIds(
