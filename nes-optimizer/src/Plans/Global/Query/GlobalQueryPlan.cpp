...
void GlobalQueryPlan::addQueryPlan(const QueryPlanPtr &queryPlan) {
  ...
  if (inputQueryPlanId == INVALID_QUERY_ID) {
    throw Exceptions::QueryNotFoundException(
        "GlobalQueryPlan: Can not add query plan with invalid id.");
  }
  if (queryIdToSharedQueryIdMap.find(inputQueryPlanId) !=
      queryIdToSharedQueryIdMap.end()) {
    throw GlobalQueryPlanUpdateException(
        fmt::format("GlobalQueryPlan: Query plan with id {} already present.",
                    inputQueryPlanId));
  }
...
void GlobalQueryPlan::removeQuery(QueryId queryId, RequestType requestType) {
  ...
  if (RequestType::FailQuery == requestType) {
    ...
    auto sharedQueryPlan = sharedQueryIdToPlanMap[UNSURE_CONVERSION_TODO_4761(
    ...
    // Instead of removing query we mark the status of the shared query plan to
    // failed
    ...
    if (queryIdToSharedQueryIdMap.contains(queryId)) {
      // Fetch the shared query plan id and remove the query and associated
      // operators
      ...
      if (!sharedQueryPlan->removeQuery(queryId)) {
        ...
        throw Exceptions::RuntimeException(
            fmt::format("GlobalQueryPlan: Unable to remove query with id {} "
                        "from shared query plan with id {}",
                        queryId, sharedQueryId));
      }
      ...
      // Check if the query is in the list of query plans to add and then remove
      // it
      queryPlansToAdd.erase(
          std::find_if(queryPlansToAdd.begin(), queryPlansToAdd.end(),
                       [&queryId](const QueryPlanPtr &queryPlan) {
                         return queryPlan->getQueryId() == queryId;
                       }));
    }
  } else {
    NES_ERROR("Unknown request type {}",
              std::string(magic_enum::enum_name(requestType)));
    NES_NOT_IMPLEMENTED();
  }
}
...
std::vector<SharedQueryPlanPtr> GlobalQueryPlan::getSharedQueryPlansToDeploy() {
  ...
  for (auto &[sharedQueryId, sharedQueryPlan] : sharedQueryIdToPlanMap) {
    if (SharedQueryPlanStatus::DEPLOYED == sharedQueryPlan->getStatus()) {
      NES_TRACE("GlobalQueryPlan: Skipping! found already deployed shared "
                "query plan.");
      continue;
    }
    sharedQueryPlansToDeploy.push_back(sharedQueryPlan);
  }
  NES_DEBUG("GlobalQueryPlan: Found {} Shared Query plan to be deployed.",
            sharedQueryPlansToDeploy.size());
  return sharedQueryPlansToDeploy;
}

SharedQueryId GlobalQueryPlan::getSharedQueryId(QueryId queryId) {
  NES_TRACE("GlobalQueryPlan: Get the Global Query Id for the query  {}",
            queryId);
  if (queryIdToSharedQueryIdMap.contains(queryId)) {
    return queryIdToSharedQueryIdMap.at(queryId);
  }
  NES_TRACE("GlobalQueryPlan: Unable to find Global Query Id for the query  {}",
            queryId);
  return INVALID_SHARED_QUERY_ID;
}

bool GlobalQueryPlan::updateSharedQueryPlan(
...
void GlobalQueryPlan::removeFailedOrStoppedSharedQueryPlans() {
  ...
  for (auto itr = sharedQueryIdToPlanMap.begin();
       itr != sharedQueryIdToPlanMap.end();) {
...
void GlobalQueryPlan::removeSharedQueryPlan(SharedQueryId sharedQueryPlanId) {
  NES_INFO("GlobalQueryPlan: remove metadata information for empty shared "
           "query plan id {}",
           sharedQueryPlanId);
  if (sharedQueryPlanId == INVALID_SHARED_QUERY_ID) {
    throw Exceptions::RuntimeException(
        "GlobalQueryPlan: Cannot remove shared query plan with invalid id.");
  }
  ...
  if (sharedQueryPlan->getStatus() == SharedQueryPlanStatus::STOPPED ||
    ...
    NES_TRACE("Found stopped or failed query plan. Removing query plan from "
              "shared query plan.");
...
std::vector<SharedQueryPlanPtr> GlobalQueryPlan::getAllSharedQueryPlans() {
  ...
  NES_TRACE("GlobalQueryPlan: Found {} Shared Query MetaData.",
            sharedQueryPlans.size());
  return sharedQueryPlans;
}

SharedQueryPlanPtr
GlobalQueryPlan::getSharedQueryPlan(SharedQueryId sharedQueryId) {
...
bool GlobalQueryPlan::createNewSharedQueryPlan(const QueryPlanPtr &queryPlan) {
  ...
  std::string sourceNameAndPlacementStrategy =
      queryPlan->getSourceConsumed() + "_" +
      std::to_string(
          magic_enum::enum_integer(queryPlan->getPlacementStrategy()));
  // Add Shared Query Plan to the SourceName index
  auto item = sourceNamesAndPlacementStrategyToSharedQueryPlanMap.find(
      sourceNameAndPlacementStrategy);
  if (item != sourceNamesAndPlacementStrategyToSharedQueryPlanMap.end()) {
    auto sharedQueryPlans = item->second;
    sharedQueryPlans.emplace_back(sharedQueryPlan);
    sourceNamesAndPlacementStrategyToSharedQueryPlanMap
        [sourceNameAndPlacementStrategy] = sharedQueryPlans;
  } else {
    sourceNamesAndPlacementStrategyToSharedQueryPlanMap
        [sourceNameAndPlacementStrategy] = {sharedQueryPlan};
  }

  return true;
}

const std::vector<QueryPlanPtr> &GlobalQueryPlan::getQueryPlansToAdd() const {
  return queryPlansToAdd;
}
...
GlobalQueryPlan::getSharedQueryPlansConsumingSourcesAndPlacementStrategy(
    const std::string &sourceNames,
    Optimizer::PlacementStrategy placementStrategy) {
  std::string sourceNameAndPlacementStrategy =
      sourceNames + "_" +
      std::to_string(magic_enum::enum_integer(placementStrategy));
  auto item = sourceNamesAndPlacementStrategyToSharedQueryPlanMap.find(
      sourceNameAndPlacementStrategy);
  if (item != sourceNamesAndPlacementStrategyToSharedQueryPlanMap.end()) {
    return item->second;
...
std::vector<QueryId>
GlobalQueryPlan::getQueryIds(SharedQueryId sharedQueryPlanId) {
