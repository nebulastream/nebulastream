...
bool GlobalExecutionPlan::addDecomposedQueryPlan(
    const TopologyNodeWLock &lockedTopologyNode,
  ...
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    ...
    if ((*lockedExecutionNode)
            ->registerDecomposedQueryPlan(decomposedQueryPlan)) {
  ...
  NES_WARNING("No execution node found with the id {}. Creating a new one.",
              WorkerId);

  // Otherwise, create a new execution node and register the decomposed query
  // plan to it.
  auto newExecutionNode = ExecutionNode::create(WorkerId);
  // Add child execution nodes
  for (const auto &childTopologyNode :
       lockedTopologyNode->operator*()->getChildren()) {
    auto childWorkerId = childTopologyNode->as<TopologyNode>()->getId();
    if (lockedExecutionNodeMap->contains(childWorkerId)) {
      auto lockedChildExecutionNode =
          (*lockedExecutionNodeMap)[childWorkerId].wlock();
  ...
  if (lockedTopologyNode->operator*()->getParents().empty()) {
    ...
    for (const auto &parentTopologyNode :
         lockedTopologyNode->operator*()->getParents()) {
      auto parentWorkerId = parentTopologyNode->as<TopologyNode>()->getId();
      if (lockedExecutionNodeMap->contains(parentWorkerId)) {
        auto lockedParentExecutionNode =
            (*lockedExecutionNodeMap)[parentWorkerId].wlock();
...
bool GlobalExecutionPlan::updateDecomposedQueryPlanState(
  ...
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    ...
    auto decomposedPlan =
        (*lockedExecutionNode)
            ->getDecomposedQueryPlan(sharedQueryId, decomposedQueryPlanId);
    if (!decomposedPlan) {
      NES_ERROR("No decomposed query plan with the id {} found.",
...
GlobalExecutionPlan::getPlacedSharedQueryIds(WorkerId WorkerId) const {
  auto lockedExecutionNodeMap = idToExecutionNodeMap.wlock();
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    NES_DEBUG("Returning execution node with id  {}", WorkerId);
...
DecomposedQueryPlanPtr GlobalExecutionPlan::getCopyOfDecomposedQueryPlan(
  ...
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    NES_DEBUG("Returning execution node with id  {}", WorkerId);
    auto lockedExecutionNode = (*lockedExecutionNodeMap).at(WorkerId).wlock();
    const auto &decomposedQueryPlan =
        (*lockedExecutionNode)
            ->getDecomposedQueryPlan(sharedQueryId, decomposedQueryPlanId);
...
GlobalExecutionPlan::getCopyOfAllDecomposedQueryPlans(
  ...
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    NES_DEBUG("Returning execution node with id  {}", WorkerId);
    auto lockedExecutionNode = (*lockedExecutionNodeMap).at(WorkerId).wlock();
    const auto &decomposedQueryPlans =
        (*lockedExecutionNode)->getAllDecomposedQueryPlans(sharedQueryId);
...
ExecutionNodeWLock
GlobalExecutionPlan::getLockedExecutionNode(WorkerId WorkerId) {
  auto lockedExecutionNodeMap = idToExecutionNodeMap.wlock();
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    NES_DEBUG("Returning execution node with id  {}", WorkerId);
    ...
    if (lockedExecutionNode) {
      return std::make_shared<
          folly::Synchronized<ExecutionNodePtr>::WLockedPtr>(
          std::move(lockedExecutionNode));
    }
  }
  NES_ERROR("Execution node doesn't exists with the id {}", WorkerId);
  return nullptr;
}
...
bool GlobalExecutionPlan::addExecutionNodeAsRoot(WorkerId WorkerId) {
  ...
  auto found = std::find(lockedRootWorkerIds->begin(),
                         lockedRootWorkerIds->end(), WorkerId);
...
bool GlobalExecutionPlan::removeExecutionNode(WorkerId WorkerId) {
  ...
  // Lock execution node map and root execution node id
  auto [lockedExecutionNodeMap, lockedRootWorkerIds] =
      folly::acquireLocked(idToExecutionNodeMap, rootWorkerIds);
  if (lockedExecutionNodeMap->contains(WorkerId)) {
    ...
    auto lockedExecutionNodeToRemove =
        (*lockedExecutionNodeMap)[WorkerId].wlock();
    const auto &parentExecutionNodes =
        (*lockedExecutionNodeToRemove)->getParents();
    for (const auto &parentExecutionNode : parentExecutionNodes) {
      parentExecutionNode->removeChild((*lockedExecutionNodeToRemove));
    }
    const auto &childrenExecutionNodes =
        (*lockedExecutionNodeToRemove)->getChildren();
    ...
    return lockedExecutionNodeMap->erase(WorkerId) == 1;
  }
  NES_DEBUG("Failed to remove Execution node with id  {}", WorkerId);
  return false;
}

bool GlobalExecutionPlan::removeAllDecomposedQueryPlans(
    SharedQueryId sharedQueryId) {
  NES_DEBUG("Removing all decomposed query plans for shared query {}",
            sharedQueryId);
  // Lock execution node map and root execution node id
  auto [lockedExecutionNodeMap, lockedSharedQueryIdToWorkerIdMap] =
      folly::acquireLocked(idToExecutionNodeMap, sharedQueryIdToWorkerIdMap);
  ...
  auto WorkerIds = (*lockedSharedQueryIdToWorkerIdMap)[sharedQueryId];
  NES_DEBUG("Found {} Execution node for shared query with id {}",
            WorkerIds.size(), sharedQueryId);
  for (const auto &WorkerId : WorkerIds) {
    auto lockedExecutionNode = (*lockedExecutionNodeMap)[WorkerId].wlock();
    if (!(*lockedExecutionNode)->removeDecomposedQueryPlans(sharedQueryId)) {
      NES_ERROR("Unable to remove query sub plan with id {} from execution "
                "node with id {}",
                sharedQueryId, WorkerId);
      return false;
    }
    ...
    if ((*lockedExecutionNode)->getAllQuerySubPlans().empty()) {
      ...
      lockedSharedQueryIdToWorkerIdMap.unlock();
      removeExecutionNode(WorkerId);
    }
  }
  lockedSharedQueryIdToWorkerIdMap->erase(sharedQueryId);
  NES_DEBUG("Removed all Execution nodes for the shared query with id {}",
            sharedQueryId);
  return true;
}

bool GlobalExecutionPlan::removeDecomposedQueryPlan(
    NES::WorkerId WorkerId, NES::SharedQueryId sharedQueryId,
    NES::DecomposedQueryPlanId decomposedQueryPlanId,
    NES::DecomposedQueryPlanVersion decomposedQueryPlanVersion) {

  NES_DEBUG("Removing decomposed query plan {} for shared query {}",
            decomposedQueryPlanVersion, sharedQueryId);
  // Lock execution node map and root execution node id
  auto [lockedExecutionNodeMap, lockedSharedQueryIdToWorkerIdMap] =
      folly::acquireLocked(idToExecutionNodeMap, sharedQueryIdToWorkerIdMap);
  ...
  if (decomposedQueryPlan->getVersion() != decomposedQueryPlanVersion) {
    NES_WARNING("The current version {} of the decomposed query plan do not "
                "match with the input version {}.",
                decomposedQueryPlan->getVersion(), decomposedQueryPlanVersion);
    return false;
  }

  if ((*lockedExecutionNode)
          ->removeDecomposedQueryPlan(sharedQueryId, decomposedQueryPlanId)) {

    if (!(*lockedExecutionNode)
             ->hasRegisteredDecomposedQueryPlans(sharedQueryId)) {
      auto WorkerIds = (*lockedSharedQueryIdToWorkerIdMap)[sharedQueryId];
    ...
    NES_DEBUG("Removed decomposed query plan {} for shared query {}",
              decomposedQueryPlanVersion, sharedQueryId);
    return true;
  }
  NES_WARNING("Failed to removed decomposed query plan {} for shared query {}",
              decomposedQueryPlanVersion, sharedQueryId);
  return false;
}
...
GlobalExecutionPlan::getLockedExecutionNodesHostingSharedQueryId(
  ...
  if (lockedSharedQueryIdToWorkerIdMap->contains(sharedQueryId)) {
    NES_DEBUG(
        "Returning vector of Execution nodes for the shared query with id  {}",
        sharedQueryId);
    auto WorkerIds = (*lockedSharedQueryIdToWorkerIdMap)[sharedQueryId];
    ...
    for (const auto &WorkerId : WorkerIds) {
      ...
      lockedExecutionNodes.emplace_back(
          std::make_shared<folly::Synchronized<ExecutionNodePtr>::WLockedPtr>(
              std::move(lockedExecutionNode)));
    }
    return lockedExecutionNodes;
  }
  NES_WARNING(
      "unable to find the Execution nodes for the shared query with id {}",
      sharedQueryId);
  return {};
}

// bool GlobalExecutionPlan::removeQuerySubPlanFromNode(WorkerId WorkerId,
//                                                      SharedQueryId
//                                                      sharedQueryId,
//                                                      DecomposedQueryPlanId
//                                                      decomposedQueryPlanId) {
...
//     }
//
//     auto executionNode = idToExecutionNodeMap[WorkerId];
//
//     //return false if no query sub plan with the given id was found at the
//     node if (!executionNode->removeDecomposedQueryPlan(sharedQueryId,
//     decomposedQueryPlanId)) {
//         return false;
//     }
//
//     /* Check if the node still hosts query sub plans belonging the shared
//     query with the given id. If not, remove
...
//         auto& mappedNodes = sharedQueryIdToWorkerIdMap[sharedQueryId];
//         if (mappedNodes.size() == 1) {
//             /* if this was the only node associated with this shared query
//             id, remove the entry for this shared query
//              * from the index */
//             sharedQueryIdToWorkerIdMap->erase(sharedQueryId);
//         } else {
//             /* if other nodes are still hosting sub queries of this shared
//             query, remove only this node, from the list
//              * of nodes which host sub query plans of this shared query */
//             mappedNodes.erase(std::find(mappedNodes.begin(),
//             mappedNodes.end(), executionNode));
//         }
//     }
//
...
nlohmann::json GlobalExecutionPlan::getAsJson(SharedQueryId sharedQueryId) {
  ...
  for (const auto &WorkerId : WorkerIds) {
    ...
    for (const auto &decomposedQueryPlan : allDecomposedQueryPlans) {
      ...
      currentQuerySubPlan["DecomposedQueryPlanId"] =
          decomposedQueryPlan->getDecomposedQueryPlanId();

      // add the string containing operator to the json object of current query
      // sub plan
      currentQuerySubPlan["DecomposedQueryPlan"] =
          decomposedQueryPlan->toString();

      scheduledSubQueries.push_back(currentQuerySubPlan);
    }
    sharedQueryPlanToDecomposedQueryPlans["DecomposedQueryPlans"] =
        scheduledSubQueries;
    currentExecutionNodeJsonValue["ScheduledDecomposedQueries"] =
        sharedQueryPlanToDecomposedQueryPlans;
...
// void GlobalExecutionPlan::scheduleExecutionNode(const ExecutionNodePtr&
// executionNode) {
//     NES_DEBUG("Schedule execution node for deployment");
//     auto found = std::find(executionNodesToSchedule.begin(),
//     executionNodesToSchedule.end(), executionNode); if (found !=
//     executionNodesToSchedule.end()) {
//         NES_DEBUG("Execution node {} marked as to be scheduled",
//         executionNode->getId());
//         executionNodesToSchedule.push_back(executionNode);
//     } else {
//         NES_WARNING("Execution node {} already scheduled",
//         executionNode->getId());
//     }
//     mapExecutionNodeToSharedQueryId(executionNode);
// }

/*void GlobalExecutionPlan::mapExecutionNodeToSharedQueryId(const
ExecutionNodePtr& executionNode) { NES_DEBUG("Mapping execution node {} to the
query Id index.", executionNode->getId()); auto querySubPlanMap =
executionNode->getAllQuerySubPlans(); for (const auto& [sharedQueryId,
querySubPlans] : querySubPlanMap) { if
(!sharedQueryIdToWorkerIdMap->contains(sharedQueryId)) { NES_DEBUG("Query Id {}
does not exists adding a new entry with execution node {}", sharedQueryId,
                      executionNode->getId());
            sharedQueryIdToWorkerIdMap[sharedQueryId] = {executionNode};
        } else {
            auto executionNodes = sharedQueryIdToWorkerIdMap[sharedQueryId];
            auto found = std::find(executionNodes.begin(), executionNodes.end(),
executionNode); if (found == executionNodes.end()) { NES_DEBUG("Adding execution
node {} to the query Id {}", executionNode->getId(), sharedQueryId);
                executionNodes.push_back(executionNode);
                sharedQueryIdToWorkerIdMap[sharedQueryId] = executionNodes;
            } else {
                NES_DEBUG("Skipping as execution node {} already mapped to the
query Id {}", executionNode->getId(), sharedQueryId);
            }
        }
    }
}*/

// std::map<WorkerId, uint32_t>
// GlobalExecutionPlan::getMapOfWorkerIdToOccupiedResource(SharedQueryId
// sharedQueryId) {
//     NES_INFO("Get a map of occupied resources for the shared query {}",
//     sharedQueryId); std::map<WorkerId, uint32_t>
//     mapOfWorkerIdToOccupiedResources; auto executionNodes =
//     sharedQueryIdToWorkerIdMap[sharedQueryId]; NES_DEBUG("Found {} Execution
//     node for the shared query with id {}", executionNodes.size(),
//     sharedQueryId); for (auto& executionNode : executionNodes) {
//         uint32_t occupiedResource =
//         executionNode->getOccupiedResources(sharedQueryId);
//         mapOfWorkerIdToOccupiedResources[executionNode->getTopologyNode()->getId()]
//         = occupiedResource;
//     }
//     NES_DEBUG("returning the map of occupied resources for the shared query
//     {}", sharedQueryId); return mapOfWorkerIdToOccupiedResources;
// }
