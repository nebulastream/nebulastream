...
ChangeLogEntryPtr ChangeLogEntry::create(std::set<LogicalOperatorPtr> upstreamOperators, std::set<LogicalOperatorPtr> downstreamOperators)
{
        ...
        while (!operatorsToVisit.empty())
        {
            ...
            if (downStreamOperatorsToVisit.size() > 1)
            {
                ...
                for (const auto& downStreamOperatorToVisit : downStreamOperatorsToVisit)
                {
                    ...
                    { // Check if the path is to be explored
