...
class Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging(
        "Z3SignatureBasedBottomUpQueryContainmentRuleTest.log",
        NES::LogLevel::LOG_TRACE);
    NES_INFO(
        "Setup Z3SignatureBasedBottomUpQueryContainmentRuleTest test case.");
  }
  ...
  void TearDown() override {
    NES_DEBUG("Tear down Z3SignatureBasedBottomUpQueryContainmentRuleTest test "
              "case.");
  }
  ...
  void setupSensorNodeAndSourceCatalog() {
    ...
    auto csvSourceWindTurbine =
        CSVSourceType::create("windTurbines", "windTurbines");
    LogicalSourcePtr windTurbinesLogicalSource =
        LogicalSource::create("windTurbines", schema);
    PhysicalSourcePtr windTurbinesPhysicalSource =
        PhysicalSource::create(csvSourceWindTurbine);
    auto sce1 = Catalogs::Source::SourceCatalogEntry::create(
        windTurbinesPhysicalSource, windTurbinesLogicalSource,
        physicalNode->getId());
    sourceCatalog->addPhysicalSource("windTurbines", sce1);

    auto csvSourceSolarPanel1 =
        CSVSourceType::create("solarPanels1", "solarPanels1");
    LogicalSourcePtr solarPanels1LogicalSource =
        LogicalSource::create("solarPanels1", schema);
    PhysicalSourcePtr solarPanels1PhysicalSource =
        PhysicalSource::create(csvSourceSolarPanel1);
    auto sce2 = Catalogs::Source::SourceCatalogEntry::create(
        solarPanels1PhysicalSource, solarPanels1LogicalSource,
        physicalNode->getId());
    sourceCatalog->addPhysicalSource("solarPanels2", sce2);

    LogicalSourcePtr solarPanels2LogicalSource =
        LogicalSource::create("solarPanels2", schema);
    auto csvSourceSolarPanel2 =
        CSVSourceType::create("solarPanels2", "solarPanels2");
    PhysicalSourcePtr solarPanels2PhysicalSource =
        PhysicalSource::create(csvSourceSolarPanel2);
    auto sce3 = Catalogs::Source::SourceCatalogEntry::create(
        solarPanels2PhysicalSource, solarPanels2LogicalSource,
        physicalNode->getId());
    sourceCatalog->addPhysicalSource("solarPanels2", sce3);

    auto csvSourceHouseHolds =
        CSVSourceType::create("households", "households");
    LogicalSourcePtr householdsLogicalSource =
        LogicalSource::create("households", schemaHouseholds);
    PhysicalSourcePtr householdsPhysicalSource =
        PhysicalSource::create(csvSourceHouseHolds);
    auto sce4 = Catalogs::Source::SourceCatalogEntry::create(
        householdsPhysicalSource, householdsLogicalSource,
        physicalNode->getId());
...
/**
 * @brief Test that TD-CQM correctly identifies the equivalent filter and map
 * operations here and merges them.
 */
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
       testMultipleEqualFilters) {
  ...
  DepthFirstNodeIterator queryPlan1NodeIterator(
      queryPlan1->getRootOperators()[0]);
  ...
  DepthFirstNodeIterator queryPlan2NodeIterator(
      queryPlan2->getRootOperators()[0]);
  ...
  auto z3InferencePhase = Optimizer::SignatureInferencePhase::create(
      context, Optimizer::QueryMergerRule::
  ...
  auto signatureBasedEqualQueryMergerRule =
      Optimizer::Z3SignatureBasedTreeBasedQueryContainmentMergerRule::create(
          context, true);
  ...
  // Validate
  DepthFirstNodeIterator updatedQueryPlanNodeIteratorFirstSink(
      updatedSharedQueryPlan->getRootOperators()[0]);
  ...
  DepthFirstNodeIterator updatedQueryPlanNodeIteratorSecondSink(
      updatedSharedQueryPlan->getRootOperators()[1]);
...
/**
 * @brief: Tests that TD-CQM correctly identifies and merges the equivalent map
 * operation
 *
 * Query 1:
 ...
 */
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
       testEqualSourceOperationsDistinctQueries) {
  setupSensorNodeAndSourceCatalog();
...
/**
 * @brief Tests that the merger correctly identifies that the joins are
 * different
 *
 * Query 1:
 ...
 */
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
       testEqualSourcesDifferentJoins) {
  setupSensorNodeAndSourceCatalog();
  ...
  Query query1 = Query::from("windTurbines")
                     ...
                     .window(TumblingWindow::of(EventTime(Attribute("ts")),
                                                Milliseconds(1000)))
                     .sink(PrintSinkDescriptor::create());
  Query query2 = Query::from("windTurbines")
                     .joinWith(Query::from("solarPanels1"))
 ...
 */
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
       testEqualSourcesWithEqualJoinsAndFilterContainment) {
  setupSensorNodeAndSourceCatalog();
...
/**
 * @brief Here we are testing for correct merging when there is a different
 * number of windows involved
 *
 * Query 1:
...
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
  ...
  Query query1 = Query::from("windTurbines")
                     ...
                     .window(TumblingWindow::of(EventTime(Attribute("start")),
                                                Milliseconds(10000)))
...
/**
 * @brief: In this test, we verify that given two queries with two windows each
 * and multiple containment relationships, the merger will create a correct
 * query plan:
 *
 * Query 1:
 * Source -> Window1 -> Window2 -> Sink
 ...
 */
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
       testSameNumberOfWindowsTwoContainmentRelationships) {
  setupSensorNodeAndSourceCatalog();
...
/**
 * @brief: In this test, we verify that TD-CQM correctly merges equivalent union
 * operations followed by distinct window operations:
 *
 * Query 1:
 ...
 */
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
       testEquivalentUnionDifferentWindows) {
  setupSensorNodeAndSourceCatalog();
 ...
 *
 * Query 1:
 * Source1 -> (Source2) -> Union1 -> Filter1 -> Project1 -> Join1 -> (Source3)
 * -> Sink1 Query 2: Source1 -> (Source2) -> Union1 -> (Source3) -> Join1 ->
 * Filter1 -> Project2 -> Sink1
 *
 * Query 1 contains Query 2:
 * Source1 -> (Source2) -> Union1 -> Filter1 -> Project1 -> Join1 -> (Source3)
 * -> Sink1
...
TEST_F(Z3SignatureBasedTreeBasedQueryContainmentMergerRuleTest,
  ...
  Query query1 =
      Query::from("windTurbines")
          ...
          .project(Attribute("value"), Attribute("id1"), Attribute("value1"),
                   Attribute("ts"))
          .joinWith(Query::from("households")
                        .project(Attribute("value"), Attribute("id"),
                                 Attribute("value1"), Attribute("ts")))
  ...
  Query query2 =
      Query::from("windTurbines")
          ...
          .project(Attribute("windTurbines$value"),
                   Attribute("windTurbines$id1"), Attribute("households$value"),
                   Attribute("households$id"), Attribute("windTurbines$ts"),
