...
class Z3SignatureBasedPartialQueryMergerRuleTest
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("Z3SignatureBasedPartialQueryMergerRuleTest.log",
  ...
  void SetUp() override {
    ...
    TopologyNodePtr sourceNode2 =
        TopologyNode::create(WorkerId(3), "localhost", 123, 124, 4, properties);
    ...
    auto sourceCatalogEntry2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceCar, logicalSourceCar, sourceNode2->getId());
    ...
    auto sourceCatalogEntry4 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceBike, logicalSourceBike, sourceNode2->getId());
    ...
    auto sourceCatalogEntry5 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceCar, logicalSourceCar, sourceNode1->getId());
    auto sourceCatalogEntry6 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceCar, logicalSourceCar, sourceNode2->getId());
...
/**
 * @brief Test applying SignatureBasedPartialQueryMergerRuleTest on Global query
 * plan with same queryIdAndCatalogEntryMapping
 */
TEST_F(Z3SignatureBasedPartialQueryMergerRuleTest, testMergingEqualQueries) {
  ...
  auto z3InferencePhase = Optimizer::SignatureInferencePhase::create(
      context,
      Optimizer::QueryMergerRule::Z3SignatureBasedPartialQueryMergerRule);
...
/**
 * @brief Test applying SignatureBasedPartialQueryMergerRuleTest on Global query
 * plan with partially same queryIdAndCatalogEntryMapping
 */
TEST_F(Z3SignatureBasedPartialQueryMergerRuleTest,
       testMergingPartiallyEqualQueries) {
...
TEST_F(Z3SignatureBasedPartialQueryMergerRuleTest,
       testMergingPartiallyEqualQueriesMoreThanTwoQueries) {
  ...
  auto updatedSharedQueryPlansDeploy =
  ...
  // assert
  updatedSharedQueryPlansDeploy =
      globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_EQ(updatedSharedQueryPlansDeploy.size(), 1U);

  auto updatedSharedQueryPlan1 =
      updatedSharedQueryPlansDeploy[0]->getQueryPlan();
  EXPECT_TRUE(updatedSharedQueryPlan1);
  ...
  now = std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::system_clock::now().time_since_epoch())
            .count();
  auto changeLogEntries =
      updatedSharedQueryPlansDeploy[0]->getChangeLogEntries(now);

  EXPECT_EQ(changeLogEntries.size(), 1U);

  for (const auto &upstreamOperator :
       changeLogEntries.at(0).second->upstreamOperators) {
...
TEST_F(Z3SignatureBasedPartialQueryMergerRuleTest,
       testMergingPartiallyEqualQueriesWithQueryStopChangeLog) {
  ...
  auto updatedSharedPlanAfterStopToDeploy =
      globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_TRUE(updatedSharedPlanAfterStopToDeploy.size() == 1);

  auto updatedSharedPlanAfterStop =
      updatedSharedPlanAfterStopToDeploy[0]->getQueryPlan();
  auto now = std::chrono::duration_cast<std::chrono::microseconds>(
                 std::chrono::system_clock::now().time_since_epoch())
                 .count();
  auto changeLogEntries =
      updatedSharedPlanAfterStopToDeploy[0]->getChangeLogEntries(now);
  ...
  EXPECT_TRUE(std::any_of(
      rootOperatorsAfterStop.begin(), rootOperatorsAfterStop.end(),
      [](const OperatorPtr operatorToCheck) {
        return operatorToCheck->as_if<LogicalOperator>()->getOperatorState() ==
               OperatorState::TO_BE_REMOVED;
      }));
}

TEST_F(Z3SignatureBasedPartialQueryMergerRuleTest,
       testMergingPartiallyEqualQueriesWithQueryStop) {
  ...
  auto updatedSharedPlanAfterStopToDeploy =
      globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_EQ(updatedSharedPlanAfterStopToDeploy.size(), 1);
