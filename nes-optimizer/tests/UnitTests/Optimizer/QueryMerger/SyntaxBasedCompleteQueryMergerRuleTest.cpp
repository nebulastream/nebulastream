...
class SyntaxBasedCompleteQueryMergerRuleTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SyntaxBasedEqualQueryMergerRuleTest.log",
  ...
  void SetUp() override {
    ...
    TopologyNodePtr sourceNode1 =
        TopologyNode::create(WorkerId(2), "localhost", 123, 124, 4, properties);

    auto logicalSourceCar = sourceCatalog->getLogicalSource("car");
    auto physicalSourceCar =
        PhysicalSource::create(DefaultSourceType::create("car", "testCar"));
    auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceCar, logicalSourceCar, sourceNode1->getId());
    sourceCatalog->addPhysicalSource("car", sourceCatalogEntry1);

    auto logicalSourceBike = sourceCatalog->getLogicalSource("bike");
    auto physicalSourceBike =
        PhysicalSource::create(DefaultSourceType::create("bike", "testBike"));
    auto sourceCatalogEntry2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceBike, logicalSourceBike, sourceNode1->getId());
    sourceCatalog->addPhysicalSource("bike", sourceCatalogEntry2);

    auto logicalSourceTruck = sourceCatalog->getLogicalSource("truck");
    auto physicalSourceTruck =
        PhysicalSource::create(DefaultSourceType::create("truck", "testTruck"));
    auto sourceCatalogEntry3 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceCar, logicalSourceCar, sourceNode1->getId());
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with same queryIdAndCatalogEntryMapping
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest, testMergingEqualQueries) {
  ...
  // execute
  auto syntaxBasedEqualQueryMergerRule =
      Optimizer::SyntaxBasedCompleteQueryMergerRule::create();
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with same queryIdAndCatalogEntryMapping with multiple same source
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingEqualQueriesWithMultipleSameSources) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();

  auto sourceOperator11 = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create("car"));

  auto sourceOperator21 = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create("car"));

  auto sinkOperator11 =
      LogicalOperatorFactory::createSinkOperator(printSinkDescriptor);
  ...
  auto sourceOperator12 = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create("car"));

  auto sourceOperator22 = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create("car"));
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with two queryIdAndCatalogEntryMapping with different source
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentSources) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query1 = Query::from("car")
                     .map(Attribute("value") = 40)
  ...
  // assert
  for (const auto& sink1ChildOperator :
       updatedRootOperators1[0]->getChildren()) {
    for (const auto& sink2ChildOperator :
         updatedRootOperators2[0]->getChildren()) {
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with same queryIdAndCatalogEntryMapping with unionWith operators
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithUnionOperators) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with queryIdAndCatalogEntryMapping with different order of unionWith operator
 * children
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithMergeOperatorChildrenOrder) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with queryIdAndCatalogEntryMapping with unionWith operators but different
 * children
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithMergeOperatorsWithDifferentChildren) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with two queryIdAndCatalogEntryMapping with different filters
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentFilters) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query1 = Query::from("car")
                     .map(Attribute("value") = 40)
 ...
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentFiltersField) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query1 = Query::from("car")
                     .map(Attribute("value") = 40)
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with two queryIdAndCatalogEntryMapping with different map
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentMapAttribute) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query1 = Query::from("car")
                     .map(Attribute("value") = 40)
 ...
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentMapValue) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query1 = Query::from("car")
                     .map(Attribute("value") = 40)
...
/**
 * @brief Test applying SyntaxBasedEqualQueryMergerRule on Global query plan
 * with two queryIdAndCatalogEntryMapping with different windows
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentWindows) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  auto windowType1 =
      SlidingWindow::of(EventTime(Attribute("ts")), Minutes(66), Minutes(66));
  ...
  auto windowType2 = SlidingWindow::of(EventTime(Attribute("ts")),
                                       Seconds(3960), Seconds(3960));
  ...
  coordinatorConfiguration->optimizer.queryMergerRule = Optimizer::
      QueryMergerRule::ImprovedHashSignatureBasedCompleteQueryMergerRule;
  ...
  auto topoSpecificRewrite =
      Optimizer::TopologySpecificQueryRewritePhase::create(
          Topology::create(), sourceCatalog,
          Configurations::OptimizerConfiguration(), statisticProbeHandler);
 ...
 */
TEST_F(SyntaxBasedCompleteQueryMergerRuleTest,
       testMergingQueriesWithDifferentWindowsWithoutKeyBy) {
  // Prepare
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  auto windowType1 =
      SlidingWindow::of(EventTime(Attribute("ts")), Minutes(66), Minutes(66));
