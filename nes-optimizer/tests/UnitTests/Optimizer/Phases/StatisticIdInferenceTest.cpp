...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("StatisticIdInferenceTest.log",
...
/**
 * @brief Tests if we can infer the statistic id of a query with one physical
 * source and one sink
 */
TEST_F(StatisticIdInferenceTest, oneSourceOneQuery) {

  auto query =
      Query::from("default_logical").sink(FileSinkDescriptor::create(""));
  ...
  // 2. Checking if all operators have a unique statistic id, by iterating over
  // all operators and get the statisticId of each operator. We check for
  // uniqueness by comparing the size of both sets (allOperators,
  // allStatisticIds).
  auto allOperators = queryPlan->getAllOperators();
  std::set<StatisticId> allStatisticIds;
  std::transform(allOperators.begin(), allOperators.end(),
...
/**
 * @brief Tests if we can infer the statistic id of a query with two physical
...
/**
 * @brief Tests if we can infer the statistic id of two queries with two
 * physical sources and a sink
 */
TEST_F(StatisticIdInferenceTest, twoSourcesTwoQueries) {
  ...
  std::transform(allOperators1.begin(), allOperators1.end(),
                 std::inserter(allStatisticIds, allStatisticIds.end()),
                 [](const OperatorPtr &op) { return op->getStatisticId(); });
  auto allOperators2 = queryPlan2->getAllOperators();
  std::transform(allOperators2.begin(), allOperators2.end(),
                 std::inserter(allStatisticIds, allStatisticIds.end()),
                 [](const OperatorPtr &op) { return op->getStatisticId(); });

  // We have to subtract here one, as both queries use the same two sources and
  // a source always has the same statisticId across multiple queries.
  EXPECT_EQ(allStatisticIds.size(),
            allOperators1.size() + allOperators2.size() - 2);
}

/**
 * @brief Tests if we can infer the statistic id of a query with one physical
 * source, one sink, and one filter and a map
 */
...
/**
 * @brief Tests if we can infer the statistic id of a query with two physical
 * sources, one sink, and one filter and a map
 */
TEST_F(StatisticIdInferenceTest, twoSourcesOneQueryWithFilterAndMap) {
  ...
  std::transform(allOperators.begin(), allOperators.end(),
                 ...
                 [](const OperatorPtr &op) {
                   auto statisticId = op->getStatisticId();
                   NES_INFO("StatisticId {} for op {}", statisticId,
                            op->toString());
                   EXPECT_NE(statisticId, INVALID_STATISTIC_ID);
                   return statisticId;
                 });
...
/**
 * @brief Tests if we can infer the statistic id of two queries with two
 * physical sources, a sink, and one filter and a map.
 */
TEST_F(StatisticIdInferenceTest, twoSourcesTwoQueriesWithFilterAndMap) {
  ...
  // We have to subtract here one, as both queries use the same source and a
  // source always has the same statisticId across multiple queries.
  EXPECT_EQ(allStatisticIds.size(),
            allOperators1.size() + allOperators2.size() - 1);
}
