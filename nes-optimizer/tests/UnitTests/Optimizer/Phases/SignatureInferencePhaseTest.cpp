...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SignatureInferencePhaseTest.log",
...
/**
 * @brief In this test we execute query merger phase on a single invalid query
 * plan.
 */
TEST_F(SignatureInferencePhaseTest,
       executeQueryMergerPhaseForSingleInvalidQueryPlan) {

  // Prepare
  NES_INFO("SignatureInferencePhaseTest: Create a new query without assigning "
           "it a query id.");
  ...
  TopologyNodePtr physicalNode =
      TopologyNode::create(WorkerId(1), "localhost", 4000, 4002, 4, properties);

  PhysicalSourcePtr physicalSource =
      PhysicalSource::create("default_logical", "test1");
  auto logicalSource =
  ...
  auto typeInferencePhase =
      TypeInferencePhase::create(sourceCatalog, udfCatalog);
  z3::ContextPtr context = std::make_shared<z3::context>();
  auto signatureInferencePhase = SignatureInferencePhase::create(
      context, QueryMergerRule::Z3SignatureBasedCompleteQueryMergerRule);
  ...
  EXPECT_TRUE(signatureEqualityChecker->checkEquality(
      mapOperators1[0]->getZ3Signature(), mapOperators2[0]->getZ3Signature()));
  ...
  EXPECT_TRUE(signatureEqualityChecker->checkEquality(
      srcOperators1[0]->getZ3Signature(), srcOperators2[0]->getZ3Signature()));
}
} // namespace NES::Optimizer
