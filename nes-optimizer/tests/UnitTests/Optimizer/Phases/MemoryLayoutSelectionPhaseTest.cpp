...
class MemoryLayoutSelectionPhaseTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MemoryLayoutSelectionPhase.log",
...
void fillBufferRowLayout(
    TupleBuffer& buf, const Runtime::MemoryLayouts::RowLayoutPtr& memoryLayout,
...
void fillBufferColLayout(
    TupleBuffer& buf,
    const Runtime::MemoryLayouts::ColumnLayoutPtr& memoryLayout,
    uint64_t numberOfTuples) {
  auto recordIndexFields =
      Runtime::MemoryLayouts::ColumnLayoutField<int64_t, true>::create(
          0, memoryLayout, buf);
  auto fields01 =
      Runtime::MemoryLayouts::ColumnLayoutField<int64_t, true>::create(
          1, memoryLayout, buf);
...
TEST_F(MemoryLayoutSelectionPhaseTest, setColumnarLayoutMapQuery) {
  ...
  auto query =
      TestQuery::from(DefaultSourceDescriptor::create(
                          inputSchema, numbersOfBufferToProduce, frequency))
  ...
  for (auto node : PlanIterator(plan)) {
    if (auto op = node->as_if<Operator>()) {
      ASSERT_EQ(op->getOutputSchema()->getLayoutType(),
...
TEST_F(MemoryLayoutSelectionPhaseTest, setColumnLayoutWithTypeInference) {
  ...
  auto query =
      TestQuery::from(DefaultSourceDescriptor::create(
          ...
          .map(Attribute("default_logical$f1") =
                   Attribute("default_logical$f1") * 42)
          .sink(FileSinkDescriptor::create(""));
  auto plan = query.getQueryPlan();
