...
class TypeInferencePhaseTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TypeInferencePhaseTest.log",
...
/**
 * @brief In this test we infer the output and input schemas of each operator in
 * a query.
 */
TEST_F(TypeInferencePhaseTest, inferQueryPlan) {
  ...
  NES_DEBUG("first={} second={}", map->getOutputSchema()->toString(),
...
TEST_F(TypeInferencePhaseTest, inferWindowQuery) {
  ...
  NES_DEBUG("{}",
            resultPlan->getSinkOperators()[0]->getOutputSchema()->toString());
  // we just access the old references
  ASSERT_EQ(resultPlan->getSinkOperators()[0]->getOutputSchema()->getSize(),
            4UL);
}

/**
 * @brief In this test we try to infer the output and input scheas of an invalid
...
/**
 * @brief In this test we ensure that the source descriptor is correctly
 * replaced, such that the schema can be propagated.
 */
...
/**
 * @brief In this test we ensure that the schema can be propagated properly when
 * unionWith operator is present.
 */
...
TEST_F(TypeInferencePhaseTest, inferQueryRenameBothAttributes) {
  auto inputSchema = Schema::create()
                         ->addField("f1", BasicType::INT32)
                         ->addField("f2", BasicType::INT8);
...
TEST_F(TypeInferencePhaseTest, inferQueryMapAssignment) {
  ...
  // we have to forbit the renaming of the attribute in the assignment statement
  // of the map
...
TEST_F(TypeInferencePhaseTest, inferTypeForPowerOperatorQuery) {
  ...
  EXPECT_TRUE(
      powFloat32->getDataType()->equals(DataTypeFactory::createDouble()));
...
/**
 * @brief In this test we test the type inference for query with Project
 * operator
 */
...
/**
 * @brief In this test we test the type inference for query with Source Rename
 * operator
 */
TEST_F(TypeInferencePhaseTest, inferQueryWithRenameSource) {
  ...
  SchemaPtr renameSourceOutputSchema =
      renameSourceOperator[0]->getOutputSchema();
...
/**
 * @brief In this test we test the type inference for query with fully qualified
 * attribute names
 */
...
/**
 * @brief In this test we test the type inference for query with Source Rename
 * and Project operators with fully qualified source name
 */
TEST_F(TypeInferencePhaseTest,
       inferQueryWithRenameSourceAndProjectWithFullyQualifiedNames) {
  auto inputSchema = Schema::create();
  inputSchema->addField("f1", BasicType::INT32);
  inputSchema->addField("f2", BasicType::INT8);
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
...
/**
 * @brief In this test we test the type inference for query with Merge, Source
 * Rename and Project operators with fully qualified source name
 */
TEST_F(
    TypeInferencePhaseTest,
    inferQueryWithRenameSourceAndProjectWithFullyQualifiedNamesAndMergeOperator) {
  auto inputSchema = Schema::create();
  inputSchema->addField("f1", BasicType::INT32);
  inputSchema->addField("f2", BasicType::INT8);
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
...
/**
 * @brief In this test we test the type inference for query with Join, Source
 * Rename and Project operators with fully qualified source name
 */
TEST_F(
    TypeInferencePhaseTest,
    inferQueryWithRenameSourceAndProjectWithFullyQualifiedNamesAndJoinOperator) {
  auto inputSchema = Schema::create()
                         ->addField("f1", BasicType::INT32)
                         ->addField("f2", BasicType::INT8)
  ...
  auto query = Query::from("default_logical")
                   ...
                   .project(Attribute("x$default_logical$f1").as("f3"),
                            Attribute("y$default_logical$f2").as("f4"))
                   .map(Attribute("default_logical$f3") = Attribute("f4") + 2)
                   .as("x")
                   .sink(FileSinkDescriptor::create(""));
  auto plan = query.getQueryPlan();
...
/**
 * @brief In this test we test the type inference for query with two Joins,
 * Source Rename, map, and Project operators with fully qualified source name
 */
TEST_F(TypeInferencePhaseTest, testInferQueryWithMultipleJoins) {
  ...
  auto inputSchema2 = Schema::create()
                          ->addField("f3", BasicType::INT32)
                          ->addField("f4", BasicType::INT8)
                          ->addField("ts", BasicType::INT64);
  sourceCatalog->addLogicalSource("default_logical2", inputSchema2);

  auto inputSchema3 = Schema::create()
                          ->addField("f5", BasicType::INT32)
                          ->addField("f6", BasicType::INT8)
                          ->addField("ts", BasicType::INT64);
  ...
  auto query = Query::from("default_logical")
                   ...
                   .project(Attribute("default_logical$f1").as("f23"),
                            Attribute("default_logical2$f3").as("f44"))
  ...
  SchemaPtr sourceOutputSchema = sourceOperator[0]->getOutputSchema();
  NES_DEBUG("expected src0= {}",
            sourceOperator[0]->getOutputSchema()->toString());
  EXPECT_TRUE(sourceOutputSchema->fields.size() == 3);
  ...
  NES_DEBUG("expected src2= {}",
            sourceOperator[1]->getOutputSchema()->toString());
  ...
  EXPECT_TRUE(filterOutputSchema->getField(
      "default_logical3default_logicaldefault_logical2$start"));
  EXPECT_TRUE(filterOutputSchema->getField(
      "default_logical3default_logicaldefault_logical2$end"));
  ...
  EXPECT_TRUE(
      filterOutputSchema->getField("default_logicaldefault_logical2$start"));
  EXPECT_TRUE(
      filterOutputSchema->getField("default_logicaldefault_logical2$end"));
  EXPECT_TRUE(filterOutputSchema->getField("default_logical$f1"));
  EXPECT_TRUE(filterOutputSchema->getField("default_logical$f2"));
  ...
  SchemaPtr renameSourceOutputSchema =
      renameSourceOperator[0]->getOutputSchema();
  NES_DEBUG("expected = {}",
            renameSourceOperator[0]->getOutputSchema()->toString());
  EXPECT_TRUE(renameSourceOutputSchema->fields.size() == 2);
...
/**
 * @brief In this test we infer the output and input schemas of each operator
 * window join query
 */
TEST_F(TypeInferencePhaseTest, inferWindowJoinQuery) {
  ...
  EXPECT_TRUE(sinkOutputSchema->fields.size() == 4);
  EXPECT_TRUE(
      sinkOutputSchema->getField("default_logicaldefault_logical2$start"));
  EXPECT_TRUE(
      sinkOutputSchema->getField("default_logicaldefault_logical2$end"));
...
TEST_F(TypeInferencePhaseTest, inferBatchJoinQueryManuallyInserted) {
  ...
  SchemaPtr schemaProbeSide =
      Schema::create()
          ...
          ->addField(
              "timestamp",
              BasicType::INT64)  // todo should be called value. only called
                                 // timestamp for watermark operator to work.
      ;
  ...
  LogicalJoinOperatorPtr joinOp =
      queryPlan->getOperatorByType<LogicalJoinOperator>()[0];
  Experimental::LogicalBatchJoinOperatorPtr batchJoinOp;
  {
    ...
    batchJoinOp = LogicalOperatorFactory::createBatchJoinOperator(batchJoinDef)
  ...
  for (auto wmaOp :
       queryPlan->getOperatorByType<WatermarkAssignerLogicalOperator>()) {
  ...
  SchemaPtr sinkOutputSchema = sinkOperator[0]->getOutputSchema();
  NES_DEBUG("inferred output schema = {}",
            sinkOperator[0]->getOutputSchema()->toString());
...
TEST_F(TypeInferencePhaseTest, testJoinOnFourSources) {
  ...
  auto inputSchema4 = Schema::create()
                          ->addField("f7", BasicType::INT32)
                          ->addField("f8", BasicType::INT8)
                          ->addField("ts", BasicType::INT64);
  ...
  auto subQuery2 = Query::from("default_logical3")
                       .joinWith(subQuery3)
                       .where(Attribute("f5") == Attribute("f7"))
                       .window(windowType3);
  auto query = Query::from("default_logical")
                   .joinWith(subQuery)
                   .where(Attribute("f1") == Attribute("f3"))
  ...
  NES_DEBUG("expected src3= {}",
            sourceOperator[2]->getOutputSchema()->toString());
  ...
  NES_DEBUG("expected join1= {}",
            joinOperators[0]->getOutputSchema()->toString());
  EXPECT_TRUE(joinOutSchema1->fields.size() == 18);
  EXPECT_TRUE(joinOutSchema1->getField(
      "default_logicaldefault_logical2default_logical3default_logical4$start"));
  EXPECT_TRUE(joinOutSchema1->getField(
      "default_logicaldefault_logical2default_logical3default_logical4$end"));
  EXPECT_TRUE(
      joinOutSchema1->getField("default_logicaldefault_logical2$start"));
  ...
  EXPECT_TRUE(
      joinOutSchema1->getField("default_logical3default_logical4$start"));
  ...
  NES_DEBUG("expected join2= {}",
            joinOperators[1]->getOutputSchema()->toString());
  EXPECT_TRUE(joinOutSchema2->fields.size() == 8);
  EXPECT_TRUE(
      joinOutSchema2->getField("default_logicaldefault_logical2$start"));
  ...
  NES_DEBUG("expected join3= {}",
            joinOperators[2]->getOutputSchema()->toString());
  EXPECT_TRUE(joinOutSchema3->fields.size() == 8);
  EXPECT_TRUE(
      joinOutSchema3->getField("default_logical3default_logical4$start"));
  ...
  EXPECT_TRUE(sinkOutputSchema->getField(
      "default_logicaldefault_logical2default_logical3default_logical4$start"));
  EXPECT_TRUE(sinkOutputSchema->getField(
      "default_logicaldefault_logical2default_logical3default_logical4$end"));
  EXPECT_TRUE(
  ...
  EXPECT_TRUE(
      sinkOutputSchema->getField("default_logical3default_logical4$start"));
  EXPECT_TRUE(
      sinkOutputSchema->getField("default_logical3default_logical4$end"));
...
/**
 * @brief In this test we infer the output schemas of multiple orWith Operators
 * (equivalent to union)
 */
TEST_F(TypeInferencePhaseTest, inferOrwithQuery) {
  ...
  auto query = Query::from("QnV1")
                   ...
                   .orWith(Query::from("QnV2")
                               ...
                               .orWith(Query::from("QnV3").filter(
                                   Attribute("quantity") > 7)))
                   .sink(FileSinkDescriptor::create(""));
...
TEST_F(TypeInferencePhaseTest, inferTypeForQueryWithMapUDF) {
  ...
  auto sinkOperator = LogicalOperatorFactory::createSinkOperator(
      NullOutputSinkDescriptor::create());

  auto javaUdfDescriptor =
      Catalogs::UDF::JavaUDFDescriptorBuilder{}
          .setInputSchema(udfInputSchema)
          .setOutputSchema(std::make_shared<Schema>()->addField(
              "outputAttribute", DataTypeFactory::createBoolean()))
          .build();
  auto mapUdfLogicalOperator = std::make_shared<MapUDFLogicalOperator>(
  ...
  auto actualSinkOperator =
      resultPlan->getOperatorByType<SinkLogicalOperator>();
  SchemaPtr sinkOutputSchema = actualSinkOperator[0]->getOutputSchema();
  NES_DEBUG("expected = {}",
            actualSinkOperator[0]->getOutputSchema()->toString());
...
TEST_F(TypeInferencePhaseTest, inferTypeForQueryWithMapUDFAfterBinaryOperator) {
  ...
  auto sourceOperator1 =
      LogicalOperatorFactory::createSourceOperator(descriptor1);

  auto descriptor2 = LogicalSourceDescriptor::create("logicalSource2");
  auto sourceOperator2 =
      LogicalOperatorFactory::createSourceOperator(descriptor2);
 ...
 */
TEST_F(TypeInferencePhaseTest,
       inferTypeForQueryWithMapUDFBeforeBinaryOperator) {
  Catalogs::Source::SourceCatalogPtr streamCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  auto sourceSchema =
      Schema::create()
  ...
  auto javaUdfDescriptor1 =
      Catalogs::UDF::JavaUDFDescriptorBuilder{}
          .setInputSchema(udfInputSchema)
          .setOutputSchema(std::make_shared<Schema>()->addField(
              "outputAttribute1", DataTypeFactory::createBoolean()))
          .build();
  auto mapUdfLogicalOperator1 = std::make_shared<MapUDFLogicalOperator>(
      javaUdfDescriptor1, getNextOperatorId());

  auto javaUdfDescriptor2 =
      Catalogs::UDF::JavaUDFDescriptorBuilder{}
          .setOutputSchema(std::make_shared<Schema>()->addField(
              "outputAttribute2", DataTypeFactory::createBoolean()))
          .build();
  auto mapUdfLogicalOperator2 = std::make_shared<MapUDFLogicalOperator>(
      javaUdfDescriptor1, getNextOperatorId());
...
/**
 * @brief We test that the datatype of a event time field is always an integer.
 * We create a query with a window that has a timestamp field with a wrong
 * datatype and run the type inference phase. We expect an exception to be
 * thrown.
 */
TEST_F(TypeInferencePhaseTest, windowDataTypeForTimestamp) {
  ...
  Catalogs::Source::SourceCatalogPtr streamCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  auto sourceSchema =
      TestSchemas::getSchemaTemplate("id_time_u64")
  ...
  auto queryWrong = Query::from("source")
                        .window(TumblingWindow::of(
                            EventTime(Attribute("wrongDataTypeTimeStamp")),
                            Milliseconds(1000)))
                        .apply(Sum(Attribute("id")))
                        .sink(NullOutputSinkDescriptor::create());
