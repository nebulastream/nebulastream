...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("QueryPlacementTest.log",
  ...
  void setupTopologyAndSourceCatalog(std::vector<uint16_t> resources) {
    ...
    topology->registerWorker(rootNodeId, "localhost", 123, 124, resources[0],
                             properties, 0, 0);
    topology->addAsRootWorkerId(rootNodeId);

    auto sourceNode1Id = WorkerId(2);
    topology->registerWorker(sourceNode1Id, "localhost", 123, 124, resources[1],
                             properties, 0, 0);

    auto sourceNode2Id = WorkerId(3);
    topology->registerWorker(sourceNode2Id, "localhost", 123, 124, resources[2],
                             properties, 0, 0);
    ...
    CSVSourceTypePtr csvSourceTypeForCar1 =
        CSVSourceType::create(carSourceName, "carPhysicalSourceName1");
    csvSourceTypeForCar1->setGatheringInterval(0);
    csvSourceTypeForCar1->setNumberOfTuplesToProducePerBuffer(0);
    CSVSourceTypePtr csvSourceTypeForCar2 =
        CSVSourceType::create(carSourceName, "carPhysicalSourceName2");
    ...
    auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceForCar1, logicalSource, sourceNode1Id);
    auto sourceCatalogEntry2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceForCar2, logicalSource, sourceNode2Id);
  ...
  static void assignDataModificationFactor(QueryPlanPtr queryPlan) {
    ...
    for (auto qPlanItr = queryPlanIterator.begin();
         qPlanItr != PlanIterator::end(); ++qPlanItr) {
      ...
      if ((*qPlanItr)->instanceOf<LogicalMapOperator>()) {
        ...
        NES_DEBUG("input schema in bytes: {}",
                  op->getInputSchema()->getSchemaSizeInBytes());
        NES_DEBUG("output schema in bytes: {}",
                  op->getOutputSchema()->getSchemaSizeInBytes());
        double schemaSizeComparison =
            1.0 * op->getOutputSchema()->getSchemaSizeInBytes() /
            op->getInputSchema()->getSchemaSizeInBytes();
...
TEST_F(QueryPlacementAmendmentTest, testPlacingQueryWithBottomUpStrategy) {
  ...
  auto topologySpecificQueryRewrite =
      Optimizer::TopologySpecificQueryRewritePhase::create(
          topology, sourceCatalog, Configurations::OptimizerConfiguration(),
  ...
  for (const auto &executionNode : executionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(1)) {
      ...
      ASSERT_EQ(actualRootOperator->getId(),
                queryPlan->getRootOperators()[0]->getId());
...
TEST_F(QueryPlacementTest, testElegantPlacingQueryWithTopDownStrategy) {
    ...
    Optimizer::GlobalExecutionPlanPtr globalExecutionPlan =
Optimizer::GlobalExecutionPlan::create(); auto typeInferencePhase =
Optimizer::TypeInferencePhase::create(sourceCatalog, udfCatalog);

    Query query = Query::from("car").filter(Attribute("id") <
45).sink(PrintSinkDescriptor::create()); QueryPlanPtr queryPlan =
query.getQueryPlan();
    queryPlan->setPlacementStrategy(Optimizer::PlacementStrategy::ELEGANT_ENERGY);

    auto coordinatorConfiguration =
Configurations::CoordinatorConfiguration::createDefault(); auto
queryReWritePhase =
Optimizer::QueryRewritePhase::create(coordinatorConfiguration); queryPlan =
queryReWritePhase->execute(queryPlan); typeInferencePhase->execute(queryPlan);

    auto sampleCodeGenerationPhase =
Optimizer::SampleCodeGenerationPhase::create(); queryPlan =
sampleCodeGenerationPhase->execute(queryPlan);

    auto topologySpecificQueryRewrite =
        Optimizer::TopologySpecificQueryRewritePhase::create(topology,
sourceCatalog, Configurations::OptimizerConfiguration(), statisticProbeHandler);
    topologySpecificQueryRewrite->execute(queryPlan);
    typeInferencePhase->execute(queryPlan);
}*/
...
TEST_F(QueryPlacementAmendmentTest, testPlacingQueryWithTopDownStrategy) {
  ...
  Optimizer::GlobalExecutionPlanPtr globalExecutionPlan =
      Optimizer::GlobalExecutionPlan::create();
  ...
  for (const auto &executionNode : executionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(1)) {
      ...
      std::vector<SourceLogicalOperatorPtr> sourceOperators =
          decomposedQueryPlan->getSourceOperators();
...
TEST_F(QueryPlacementAmendmentTest,
       testPlacingQueryWithMultipleSinkOperatorsWithBottomUpStrategy) {
      ...
      for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
        ...
        auto found = std::find_if(expectedRootOperators.begin(),
                                  ...
                                  [&](const OperatorPtr &expectedRootOperator) {
                                    return expectedRootOperator->getId() ==
                                           actualRootOperator->getId();
                                  });
...
/* Test query placement of query with multiple sinks and multiple source
 * operators with bottom up strategy  */
TEST_F(
    QueryPlacementAmendmentTest,
    testPlacingQueryWithMultipleSinkAndOnlySourceOperatorsWithBottomUpStrategy) {
...
TEST_F(QueryPlacementAmendmentTest,
       testPlacingQueryWithMultipleSinkOperatorsWithTopDownStrategy) {
  ...
  for (const auto &executionNode : executionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(1)) {
      ...
      std::vector<OperatorPtr> actualRootOperators =
          decomposedQueryPlans[0]->getRootOperators();
...
/* Test query placement of query with multiple sinks with Bottom up strategy  */
TEST_F(QueryPlacementAmendmentTest,
       testPartialPlacingQueryWithMultipleSinkOperatorsWithBottomUpStrategy) {
  ...
  auto signatureBasedEqualQueryMergerRule =
      Optimizer::Z3SignatureBasedPartialQueryMergerRule::create(context);
  auto globalQueryPlan = GlobalQueryPlan::create();

  auto queryPlan1 = Query::from("car")
                        .filter(Attribute("id") < 45)
                        .sink(PrintSinkDescriptor::create())
                        .getQueryPlan();
  ...
  SharedQueryId sharedQueryId =
      UNSURE_CONVERSION_TODO_4761(planToDeploy->getQueryId(), SharedQueryId);
  auto executionNodes =
      globalExecutionPlan->getLockedExecutionNodesHostingSharedQueryId(
          sharedQueryId);
  ASSERT_EQ(executionNodes.size(), 3UL);
      ...
      for (const auto &sourceOperator :
        ...
        for (const auto &filterParent : filterParents) {
          if (filterParent->instanceOf<LogicalMapOperator>()) {
            EXPECT_TRUE(filterParent->getParents()[0]
...
TEST_F(QueryPlacementAmendmentTest,
       testPartialPlacingQueryWithMultipleSinkOperatorsWithTopDownStrategy) {
...
/* Test query placement of query with multiple sinks and multiple source
 * operators with Top Down strategy  */
TEST_F(
    QueryPlacementAmendmentTest,
    testPlacingQueryWithMultipleSinkAndOnlySourceOperatorsWithTopDownStrategy) {
...
TEST_F(QueryPlacementAmendmentTest, DISABLED_testIFCOPPlacement) {
  ...
  topology->registerWorker(rootNodeId, "localhost", 4000, 5000, 4, properties,
                           0, 0);
  topology->addAsRootWorkerId(rootNodeId);
  ...
  topology->registerWorker(sourceNodeId, "localhost", 4002, 5002, 4, properties,
                           0, 0);
  ...
  auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
      physicalSource, logicalSource, sourceNodeId);
  ...
  auto mapOperator = LogicalOperatorFactory::createMapOperator(
      Attribute("value2") = Attribute("value") * 2);
  auto sourceOperator = LogicalOperatorFactory::createSourceOperator(
      LogicalSourceDescriptor::create("car"));
  ...
  for (const auto &executionNode : executionNodes) {
    ...
    for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
      ...
      // if the current operator is the sink of the query, it must be placed in
      // the sink node (topology node with id 0)
      if (root->as<SinkLogicalOperator>()->getId() ==
          testQueryPlan->getSinkOperators()[0]->getId()) {
        isSinkPlacementValid =
            executionNode->operator*()->getId() == INVALID_WORKER_NODE_ID;
      }
      ...
      for (const auto &child : root->getChildren()) {
        if (child->instanceOf<LogicalMapOperator>()) {
          ...
          for (const auto &childrenOfMapOp : child->getChildren()) {
            // if the current operator is a source, it should be placed in
            // topology node with id=2 (source nodes)
          ...
          // if the current operator is a source, it should be placed in
          // topology node with id=2 (source nodes)
          if (child->as<SourceLogicalOperator>()->getId() ==
              testQueryPlan->getSourceOperators()[0]->getId()) {
            isSource1PlacementValid =
                executionNode->operator*()->getId() == WorkerId(2);
          }
...
// TODO: enable this test after fixing #2486
TEST_F(QueryPlacementAmendmentTest,
       DISABLED_testIFCOPPlacementOnBranchedTopology) {
  // Setup the topology
  std::map<std::string, std::any> properties;
  properties[NES::Worker::Properties::MAINTENANCE] = false;
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
  ...
  topology->registerWorker(sourceNodeId1, "localhost", 4002, 5002, 4,
                           properties, 0, 0);
  topology->addTopologyNodeAsChild(middleNodeId1, sourceNodeId1);

  auto sourceNodeId2 = WorkerId(4);
  topology->registerWorker(sourceNodeId2, "localhost", 4002, 5002, 4,
                           properties, 0, 0);
  ...
  auto physicalSource = PhysicalSource::create(csvSourceType);
  auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
      physicalSource, logicalSource, sourceNodeId1);
  auto sourceCatalogEntry2 = Catalogs::Source::SourceCatalogEntry::create(
      physicalSource, logicalSource, sourceNodeId2);
  sourceCatalog->addPhysicalSource(sourceName, sourceCatalogEntry1);
  ...
  auto queryPlacementAmendmentPhase =
      Optimizer::QueryPlacementAmendmentPhase::create(
          ...
          coordinatorConfiguration);
  queryPlacementAmendmentPhase->execute(sharedQueryPlan);
  NES_DEBUG("RandomSearchTest: globalExecutionPlanAsString={}",
    ...
    for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
      ...
      for (const auto &child : root->getChildren()) {
        if (child->instanceOf<LogicalMapOperator>()) {
          ...
          for (const auto &childrenOfMapOp : child->getChildren()) {
            // if the current operator is a source, it should be placed in
            // topology node with id 3 or 4 (source nodes)
            if (childrenOfMapOp->as<SourceLogicalOperator>()->getId() ==
                testQueryPlan->getSourceOperators()[0]->getId()) {
              isSource1PlacementValid =
                  executionNode->operator*()->getId() == WorkerId(3) ||
...
TEST_F(QueryPlacementAmendmentTest, testTopDownPlacementOfSelfJoinQuery) {
  ...
  auto rootNodeId = WorkerId(1);
  topology->registerWorker(rootNodeId, "localhost", 4000, 5000, 14, properties,
                           0, 0);
  topology->addAsRootWorkerId(rootNodeId);
  ...
  topology->registerWorker(srcNodeId, "localhost", 4003, 5003, 4, properties, 0,
                           0);
  ...
  auto physicalSource = PhysicalSource::create(csvSourceType);
  auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
      physicalSource, logicalSource, srcNodeId);
  sourceCatalog->addPhysicalSource(sourceName, sourceCatalogEntry1);
  ...
  for (const auto &executionNode : executionNodes) {
    for (const auto &decomposedQueryPlan :
         executionNode->operator*()->getAllDecomposedQueryPlans(
             sharedQueryId)) {
      OperatorPtr root = decomposedQueryPlan->getRootOperators()[0];
      ...
      if (root->as<SinkLogicalOperator>()->getId() ==
          testQueryPlan->getSinkOperators()[0]->getId()) {
        isSinkPlacementValid =
            executionNode->operator*()->getId() == WorkerId(1);
      }

      auto sourceOperators = decomposedQueryPlan->getSourceOperators();

      for (const auto &sourceOperator : sourceOperators) {
        if (sourceOperator->as<SourceLogicalOperator>()->getId() ==
            testQueryPlan->getSourceOperators()[0]->getId()) {
          isSource1PlacementValid =
              executionNode->operator*()->getId() == WorkerId(3);
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testTopDownPlacementWthTightResourcesConstrains) {
  // Setup the topology
  std::map<std::string, std::any> properties;
  properties[NES::Worker::Properties::MAINTENANCE] = false;
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
  ...
  auto rootNodeId = WorkerId(1);
  topology->registerWorker(rootNodeId, "localhost", 4000, 5000, 1, properties,
                           0, 0);
  topology->addAsRootWorkerId(rootNodeId);
  auto middleNodeId = WorkerId(2);
  topology->registerWorker(middleNodeId, "localhost", 4001, 5001, 1, properties,
                           0, 0);
  auto srcNodeId = WorkerId(3);
  topology->registerWorker(srcNodeId, "localhost", 4003, 5003, 2, properties, 0,
                           0);
  topology->addTopologyNodeAsChild(middleNodeId, srcNodeId);
  topology->removeTopologyNodeAsChild(rootNodeId, srcNodeId);
  ...
  auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
      physicalSourceCar, logicalSource, srcNodeId);
  sourceCatalog->addPhysicalSource(sourceName, sourceCatalogEntry1);
  ...
  for (const auto &executionNode : executionNodes) {
    ...
    for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
      ...
      if (executionNode->operator*()->getId() == WorkerId(1)) {
        ASSERT_EQ(ops[0]->getId(),
                  testQueryPlan->getRootOperators()[0]->getId());
        ASSERT_EQ(ops[0]->getChildren().size(), 1);
        EXPECT_TRUE(
            ops[0]->getChildren()[0]->instanceOf<SourceLogicalOperator>());
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testBottomUpPlacementWthTightResourcesConstrains) {
  // Setup the topology
  std::map<std::string, std::any> properties;
  properties[NES::Worker::Properties::MAINTENANCE] = false;
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
  ...
  topology->registerWorker(middleNodeId, "localhost", 4001, 5001, 1, properties,
                           0, 0);
  auto srcNodeId = WorkerId(3);
  topology->registerWorker(srcNodeId, "localhost", 4003, 5003, 1, properties, 0,
                           0);
  topology->addTopologyNodeAsChild(middleNodeId, srcNodeId);
  topology->removeTopologyNodeAsChild(rootNodeId, srcNodeId);
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testBottomUpPlacementWthTightResourcesConstrainsInAJoin) {
  // Setup the topology
  std::map<std::string, std::any> properties;
  properties[NES::Worker::Properties::MAINTENANCE] = false;
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
  ...
  topology->addAsRootWorkerId(rootNodeId);
  auto middleNodeId = WorkerId(2);
  topology->registerWorker(middleNodeId, "localhost", 4001, 5001, 5, properties,
                           0, 0);
  auto srcNodeId1 = WorkerId(3);
  topology->registerWorker(srcNodeId1, "localhost", 4003, 5003, 1, properties,
                           0, 0);
  topology->addTopologyNodeAsChild(middleNodeId, srcNodeId1);
  auto srcNodeId2 = WorkerId(4);
  topology->registerWorker(srcNodeId2, "localhost", 4003, 5003, 1, properties,
                           0, 0);
  ...
  sourceCatalog = std::make_shared<Catalogs::Source::SourceCatalog>();
  {
    ...
    auto physicalSource = PhysicalSource::create(csvSourceType);
    auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource, logicalSource, srcNodeId1);
    sourceCatalog->addPhysicalSource(sourceName, sourceCatalogEntry1);
  }
  {
    ...
    auto physicalSource = PhysicalSource::create(csvSourceType);
    auto sourceCatalogEntry2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource, logicalSource, srcNodeId2);
    sourceCatalog->addPhysicalSource(sourceName, sourceCatalogEntry2);
  }
  ...
  auto watermark1 =
      join->getChildren()[0]->as<WatermarkAssignerLogicalOperator>();
  auto source1 = watermark1->getChildren()[0]->as<SourceLogicalOperator>();
  auto watermark2 =
      join->getChildren()[1]->as<WatermarkAssignerLogicalOperator>();
  ...
  for (const auto &executionNode : executionNodes) {
    ...
    for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
      ...
      if (executionNode->operator*()->getId() == WorkerId(1)) {
        ...
        ASSERT_EQ(placedJoin->as<LogicalJoinOperator>()->getId(),
                  join->getId());
        ASSERT_EQ(placedJoin->getChildren().size(), 2);
        auto placedWatermark1 = placedJoin->getChildren()[0];
        ASSERT_TRUE(
            placedWatermark1->instanceOf<WatermarkAssignerLogicalOperator>());
        ASSERT_EQ(
            placedWatermark1->as<WatermarkAssignerLogicalOperator>()->getId(),
            watermark1->getId());

        auto placedWatermark2 = placedJoin->getChildren()[1];
        ASSERT_TRUE(
            placedWatermark2->instanceOf<WatermarkAssignerLogicalOperator>());
        ASSERT_EQ(
            placedWatermark2->as<WatermarkAssignerLogicalOperator>()->getId(),
            watermark2->getId());
      } else if (executionNode->operator*()->getId() == WorkerId(3)) {
        auto placedSink = ops[0];
        ...
        ASSERT_TRUE(placedSource->instanceOf<SourceLogicalOperator>());
        ASSERT_EQ(placedSource->as<SourceLogicalOperator>()->getId(),
                  source2->getId());
      }
      NES_INFO("Sub Plan: {}", decomposedQueryPlan->toString());
    }
 ...
 *
 * Q1: Query::from("car").filter(Attribute("id") <
 * 45).sink(PrintSinkDescriptor::create()) Q2:
 * Query::from("car").filter(Attribute("id") <
 * 45).sink(PrintSinkDescriptor::create())
 *
 ...
 *  The Expectations are that both placements should be successful and should
 * result in a consistent global execution plan.
 *
 */
TEST_F(QueryPlacementAmendmentTest,
       testConcurrentOperatorPlacementUsingPessimisticBottomUpStrategy) {
  ...
  std::vector<std::future<std::set<Optimizer::DeploymentContextPtr>>>
      placementResults;
  for (auto i = 0; i < numOfQueries; i++) {
    std::future<std::set<Optimizer::DeploymentContextPtr>> placementResult =
        std::async(std::launch::async, [&, index = i]() {
          auto queryPlacementPhaseInstance =
              Optimizer::QueryPlacementAmendmentPhase::create(
                  globalExecutionPlan, topology, typeInferencePhase,
                  coordinatorConfiguration);
  ...
  for (uint16_t i = 0; i < numOfQueries; i++) {
    auto executionNodes =
        globalExecutionPlan->getLockedExecutionNodesHostingSharedQueryId(
            sharedQueryPlans[i]->getId());

    // Assertion
    ASSERT_EQ(executionNodes.size(), 3u);
    for (const auto &executionNode : executionNodes) {
      if (executionNode->operator*()->getId() == WorkerId(1)) {
        auto decomposedQueryPlans =
            executionNode->operator*()->getAllDecomposedQueryPlans(
                sharedQueryPlans[i]->getId());
        ASSERT_EQ(decomposedQueryPlans.size(), 1u);
        auto decomposedQueryPlan = decomposedQueryPlans[0u];
        ...
        OperatorPtr actualRootOperator = actualRootOperators[0];
        ASSERT_EQ(actualRootOperator->getId(),
                  queryPlans[i]->getRootOperators()[0]->getId());
        ASSERT_EQ(actualRootOperator->getChildren().size(), 2u);
        for (const auto &children : actualRootOperator->getChildren()) {
          EXPECT_TRUE(children->instanceOf<SourceLogicalOperator>());
        }
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testConcurrentOperatorPlacementUsingPessimisticTopDownStrategy) {
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testConcurrentOperatorPlacementUsingOptimisticTopDownStrategy) {
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testConcurrentOperatorPlacementUsingOptimisticBottomUpStrategy) {
 ...
 * Test query placement using BottomUp strategy for two queries:
 *
 * Q1: Query::from("car").filter(Attribute("id") < 45).map(Attribute("value") =
 * 45).sink(PrintSinkDescriptor::create()) Q2:
 * Query::from("car").filter(Attribute("id") <
 * 45).sink(PrintSinkDescriptor::create())
 *
...
TEST_F(
    QueryPlacementAmendmentTest,
    testIfCanPlaceQueryAfterPlacementFailureConcurrentOperatorPlacementUsingOCCBottomUpStrategy) {
  ...
  EXPECT_EQ(sharedQueryPlans[0]->getStatus(),
            SharedQueryPlanStatus::PARTIALLY_PROCESSED);
...
TEST_F(
    QueryPlacementAmendmentTest,
    testIfCanPlaceQueryAfterPlacementFailureConcurrentOperatorPlacementUsing2PLBottomUpStrategy) {
...
TEST_F(QueryPlacementAmendmentTest, testTopDownForRePlacement) {
  ...
  Network::NesPartition sourcePartitionNode0beforeReplacement(
      INVALID_SHARED_QUERY_ID, INVALID_OPERATOR_ID, PartitionId(0),
      SubpartitionId(0));
...

  {
    ...
    for (const auto &executionNode : executionNodes) {
      ...
      for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
        ...
        ASSERT_EQ(ops.size(), 1);
        if (executionNode->operator*()->getId() == WorkerId(1)) {
          EXPECT_EQ(decomposedQueryPlan->getState(),
          ...
          EXPECT_TRUE(
              ops[0]->getChildren()[0]->instanceOf<SourceLogicalOperator>());
          sourcePartitionNode0beforeReplacement =
              ops[0]
                  ->getChildren()[0]
                  ->as<SourceLogicalOperator>()
                  ->getSourceDescriptor()
          ...
          sourcePartitionNode1beforeReplacement =
              filter->getChildren()[0]
                  ->as<SourceLogicalOperator>()
                  ->getSourceDescriptor()
                  ->as<Network::NetworkSourceDescriptor>()
                  ->getNesPartition();
          subPlanIdToRemoveInNextIteration =
    ...
    ASSERT_EQ(sinkPartitionNode1BeforeReplacement,
              sourcePartitionNode0beforeReplacement);
    ASSERT_EQ(sinkPartitionNode2BeforeReplacement,
              sourcePartitionNode1beforeReplacement);
  }
  ...
  std::for_each(sourceOperators.begin(), sourceOperators.end(),
                [&](const OperatorPtr &item) {
  ...
  std::for_each(
      sinkOperators.begin(), sinkOperators.end(),
      [&](const OperatorPtr &item) { sinkOperatorIds.emplace(item->getId()); });

  sharedQueryPlan->performReOperatorPlacement(sourceOperatorIds,
                                              sinkOperatorIds);
  queryPlacementAmendmentPhase->execute(sharedQueryPlan);
...

  {
    ...
    for (const auto &executionNode : executionNodes) {
      ...
      for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
        ...
        if (executionNode->operator*()->getId() == WorkerId(1)) {
          ...
          EXPECT_TRUE(
              ops[0]->getChildren()[0]->instanceOf<SourceLogicalOperator>());
          sourcePartitionNode0afterReplacement =
              ops[0]
                  ->getChildren()[0]
                  ->as<SourceLogicalOperator>()
                  ->getSourceDescriptor()
                  ...
                  ->getNesPartition();
                   ...
                   decomposedQueryPlan->getDecomposedQueryPlanId() ==
                       subPlanIdToRemoveInNextIteration) {
          EXPECT_EQ(decomposedQueryPlan->getState(),
                    QueryState::MARKED_FOR_MIGRATION);
        } else if (executionNode->operator*()->getId() == WorkerId(2) &&
                   decomposedQueryPlan->getDecomposedQueryPlanId() !=
                       subPlanIdToRemoveInNextIteration) {
          EXPECT_EQ(decomposedQueryPlan->getState(),
          ...
          ASSERT_EQ(filter->as<LogicalFilterOperator>()->getId(),
                    testQueryPlan->getRootOperators()[0]
                        ...
                        ->as<LogicalFilterOperator>()
                        ->getId());
          sourcePartitionNode1afterReplacement =
              filter->getChildren()[0]
                  ->as<SourceLogicalOperator>()
                  ->getSourceDescriptor()
                  ->as<Network::NetworkSourceDescriptor>()
    ...
    ASSERT_EQ(sinkPartitionNode1afterReplacement,
              sourcePartitionNode0afterReplacement);
    ASSERT_EQ(sinkPartitionNode2afterReplacement,
              sourcePartitionNode1afterReplacement);

    // check that the descriptors have actually been updated
    ASSERT_NE(sinkPartitionNode1afterReplacement,
              sinkPartitionNode1BeforeReplacement);
    ASSERT_NE(sinkPartitionNode2afterReplacement,
              sinkPartitionNode2BeforeReplacement);
  }
}
...
TEST_F(QueryPlacementAmendmentTest, testBottomUpForRePlacement) {
    ...
    for (const auto &executionNode : executionNodes) {
      for (const auto &decomposedQueryPlan :
        ...
        if (executionNode->operator*()->getId() == WorkerId(1)) {
          ...
          sourcePartitionNode1beforeReplacement =
              source->as<SourceLogicalOperator>()
                  ->getSourceDescriptor()
                  ->as<Network::NetworkSourceDescriptor>()
                  ->getNesPartition();
          planIdToRemoveInNextIteration =
              decomposedQueryPlan->getDecomposedQueryPlanId();
        } else if (executionNode->operator*()->getId() == WorkerId(3)) {
          auto sink = ops[0];
          EXPECT_EQ(decomposedQueryPlan->getState(),
  ...
  std::set<OperatorId> sourceOperatorIds;
  std::for_each(filterOperators.begin(), filterOperators.end(),
                [&](const OperatorPtr &item) {
                  sourceOperatorIds.emplace(item->getId());
                });
    ...
    for (const auto &executionNode : executionNodes) {
      ...
      for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
        ...
        if (executionNode->operator*()->getId() == WorkerId(1)) {
          ...
          EXPECT_TRUE(sink->instanceOf<SinkLogicalOperator>());
          EXPECT_TRUE(
              sink->getChildren()[0]->instanceOf<SourceLogicalOperator>());
          sourcePartitionNode1afterReplacement =
              sink->getChildren()[0]
                  ->as<SourceLogicalOperator>()
                  ->getSourceDescriptor()
 ...
 */
TEST_F(QueryPlacementAmendmentTest,
       testBottomUpForProcessingSharedQueryPlanToBeRemoved) {
  // Setup the topology
  std::map<std::string, std::any> properties;
  properties[NES::Worker::Properties::MAINTENANCE] = false;
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
