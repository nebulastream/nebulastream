...
public:
  ...
  Optimizer::TopologySpecificQueryRewritePhasePtr
      topologySpecificQueryRewritePhase;

  static void SetUpTestCase() {
    NES::Logger::setupLogging("OriginIdInferencePhaseTest.log",
  ...
  void SetUp() override {
    ...
    typeInferencePhase = Optimizer::TypeInferencePhase::create(
        sourceCatalog, Catalogs::UDF::UDFCatalog::create());
    ...
    auto statisticProbeHandler = Statistic::StatisticProbeHandler::create(
        statisticRegistry, Statistic::DefaultStatisticProbeGenerator::create(),
        Statistic::DefaultStatisticCache::create(), Topology::create());
    topologySpecificQueryRewritePhase =
        Optimizer::TopologySpecificQueryRewritePhase::create(
            Topology::create(), sourceCatalog, optimizerConfiguration,
            statisticProbeHandler);
  }

  void setupTopologyNodeAndSourceCatalog(
    ...
    TopologyNodePtr physicalNode = TopologyNode::create(
        WorkerId(1), "localhost", 4000, 4002, 4, properties);

    auto schemaA = Schema::create()
    ...
    PhysicalSourcePtr physicalSourceA1 =
        PhysicalSource::create(DefaultSourceType::create("A", "A1"));
    auto sceA1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceA1, logicalSourceA, physicalNode->getId());
    sourceCatalog->addPhysicalSource("A", sceA1);

    PhysicalSourcePtr physicalSourceA2 =
        PhysicalSource::create(DefaultSourceType::create("A", "A2"));
    auto sceA2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceA2, logicalSourceA, physicalNode->getId());
    sourceCatalog->addPhysicalSource("A", sceA2);

    auto schemaB = Schema::create()
                       ->addField("id", BasicType::INT32)
                       ->addField("value", BasicType::UINT32);
    sourceCatalog->addLogicalSource("B", schemaB);
    LogicalSourcePtr logicalSourceB = sourceCatalog->getLogicalSource("B");

    PhysicalSourcePtr physicalSourceB1 =
        PhysicalSource::create(DefaultSourceType::create("B", "B1"));
    auto sceB1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSourceB1, logicalSourceB, physicalNode->getId());
...
TEST_F(OriginIdInferencePhaseTest, testRuleForSinglePhysicalSource) {
  ...
  auto source = LogicalOperatorFactory::createSourceOperator(
                    LogicalSourceDescriptor::create("B"))
  ...
  updatedQueryPlan =
      topologySpecificQueryRewritePhase->execute(updatedQueryPlan);
  ...
  auto sourceOperators =
      updatedQueryPlan->getOperatorByType<SourceLogicalOperator>();
  ASSERT_EQ(sourceOperators.size(), 1);
  ASSERT_EQ(sourceOperators[0]->getOutputOriginIds().size(), 1);
  ASSERT_EQ(sourceOperators[0]->getOutputOriginIds()[0],
            sourceOperators[0]->getOriginId());

  // the sink should always have one input origin id.
  auto sinkOperators =
      updatedQueryPlan->getOperatorByType<SinkLogicalOperator>();
  ...
  ASSERT_EQ(sinkOperators[0]->getInputOriginIds()[0],
            sourceOperators[0]->getOriginId());
}
...
TEST_F(OriginIdInferencePhaseTest, testRuleForMultiplePhysicalSources) {
  const QueryPlanPtr queryPlan = QueryPlan::create();
  auto source = LogicalOperatorFactory::createSourceOperator(
                    LogicalSourceDescriptor::create("A"))
                    ->as<SourceLogicalOperator>();
  queryPlan->addRootOperator(source);
  auto sink =
      LogicalOperatorFactory::createSinkOperator(PrintSinkDescriptor::create());
  ...
  ASSERT_EQ(sinkOperators[0]->getInputOriginIds()[0],
            sourceOperators[0]->getOriginId());
  ASSERT_EQ(sinkOperators[0]->getInputOriginIds()[1],
            sourceOperators[1]->getOriginId());
}
...
TEST_F(OriginIdInferencePhaseTest, testRuleForMultipleSources) {
  ...
  auto source3 = LogicalOperatorFactory::createSourceOperator(
                     LogicalSourceDescriptor::create("A"))
                     ->as<SourceLogicalOperator>();
  ...
  // input origin ids of the sink should contain all origin ids from the
  // sources.
  ASSERT_EQ(sinkOperators[0]->getInputOriginIds()[0],
            sourceOperators[0]->getOriginId());
  ASSERT_EQ(sinkOperators[0]->getInputOriginIds()[1],
            sourceOperators[1]->getOriginId());
  ASSERT_EQ(sinkOperators[0]->getInputOriginIds()[2],
            sourceOperators[2]->getOriginId());
}

/**
 * @brief Tests inference on a query plan with multiple sources and intermediate
 * unary operators. Thus the all intermediate operators should contain the
 * origin ids from all sources.
 */
TEST_F(OriginIdInferencePhaseTest,
       testRuleForMultipleSourcesAndIntermediateUnaryOperators) {
  const QueryPlanPtr queryPlan = QueryPlan::create();
  auto source1 = LogicalOperatorFactory::createSourceOperator(
                     LogicalSourceDescriptor::create("A"))
                     ->as<SourceLogicalOperator>();
  ...
  auto source3 = LogicalOperatorFactory::createSourceOperator(
                     ...
                     ->as<SourceLogicalOperator>();
  queryPlan->addRootOperator(source3);
  auto filter = LogicalOperatorFactory::createFilterOperator(Attribute("id") ==
                                                             Attribute("id"));
  queryPlan->appendOperatorAsNewRoot(filter);
  auto map = LogicalOperatorFactory::createMapOperator(Attribute("x") =
                                                           Attribute("id"));
  queryPlan->appendOperatorAsNewRoot(map);
  auto project = LogicalOperatorFactory::createProjectionOperator(
      {Attribute("x").getExpressionNode(),
       Attribute("id").getExpressionNode()});
...
/**
 * @brief Tests inference on a query plan with multiple sources and a central
 * window operator. Thus the root operator should contain the origin id from the
 * window operator and the window operator from all sources.
 */
TEST_F(OriginIdInferencePhaseTest, testRuleForMultipleSourcesAndWindow) {
  ...
  auto source3 = LogicalOperatorFactory::createSourceOperator(
                     ...
                     ->as<SourceLogicalOperator>();
  queryPlan->addRootOperator(source3);
  auto dummyWindowDefinition =
      LogicalWindowDescriptor::create({}, WindowTypePtr(), 0);
  auto window =
      LogicalOperatorFactory::createWindowOperator(dummyWindowDefinition)
  ...
  // input origin ids of the window should contain all origin ids from the
  // sources.
  ...
  // input origin ids of the sink should be the same as the window operator
  // origin id
  ASSERT_EQ(sink->getInputOriginIds()[0], window->getOriginId());
}

/**
 * @brief: This test infer origin id for a union operator fetching data from two
 * distinct sources. Therefore, the output origin ids for union operator should
 * return 3 distinct ids. 2 for each physical source of A and 1 for physical
 * source B.
 */
TEST_F(OriginIdInferencePhaseTest, testRuleForUnionOperators) {
  ...
  ASSERT_EQ(unionOps[0]->getOutputOriginIds()[0],
            sourceOps[0]->getOutputOriginIds()[0]);
  ASSERT_EQ(unionOps[0]->getOutputOriginIds()[1],
            sourceOps[1]->getOutputOriginIds()[0]);
  ASSERT_EQ(unionOps[0]->getOutputOriginIds()[2],
            sourceOps[2]->getOutputOriginIds()[0]);

  auto sinkOps = updatedQueryPlan->getOperatorByType<SinkLogicalOperator>();
  ASSERT_EQ(sinkOps[0]->getOutputOriginIds()[0],
            unionOps[0]->getOutputOriginIds()[0]);
}

/**
 * @brief: This test infer origin id for a union operator fetching data from
 * same sources. Therefore, the output origin ids for union operator should
 * return 4 distinct ids. 2 for each physical source of A on the right side and
 * 2 for physical source A on the left side.
 */
TEST_F(OriginIdInferencePhaseTest, testRuleForSelfUnionOperators) {
  ...
  auto sourceOps = updatedQueryPlan->getOperatorByType<SourceLogicalOperator>();
  auto unionOps = updatedQueryPlan->getOperatorByType<LogicalUnionOperator>();
  const std::vector<OriginId> &unionOutputOriginIds =
      unionOps[0]->getOutputOriginIds();
  ASSERT_EQ(unionOutputOriginIds.size(), 4);

  auto found =
      std::find(unionOutputOriginIds.begin(), unionOutputOriginIds.end(),
                sourceOps[0]->getOutputOriginIds()[0]);
  ASSERT_TRUE(found != unionOutputOriginIds.end());
  found = std::find(unionOutputOriginIds.begin(), unionOutputOriginIds.end(),
                    sourceOps[1]->getOutputOriginIds()[0]);
  ASSERT_TRUE(found != unionOutputOriginIds.end());
  found = std::find(unionOutputOriginIds.begin(), unionOutputOriginIds.end(),
                    sourceOps[2]->getOutputOriginIds()[0]);
  ASSERT_TRUE(found != unionOutputOriginIds.end());
  found = std::find(unionOutputOriginIds.begin(), unionOutputOriginIds.end(),
                    sourceOps[3]->getOutputOriginIds()[0]);
  ASSERT_TRUE(found != unionOutputOriginIds.end());
...
/**
 * @brief: This test infer origin id for a join operator joining data from same
 * source. Therefore, the output origin ids for join operator should return 4
 * distinct ids. 2 for each physical source of A on the left side and 2 for each
 * physical source of A with alias C on the right side.
 */
TEST_F(OriginIdInferencePhaseTest, testRuleForSelfJoinOperator) {
  ...
  joinInputOriginIds.insert(joinInputOriginIds.end(),
                            rightInputOriginIds.begin(),
                            rightInputOriginIds.end());

  ASSERT_EQ(joinInputOriginIds.size(), 4);

  auto found = std::find(joinInputOriginIds.begin(), joinInputOriginIds.end(),
                         sourceOps[0]->getOutputOriginIds()[0]);
  ASSERT_TRUE(found != joinInputOriginIds.end());
  found = std::find(joinInputOriginIds.begin(), joinInputOriginIds.end(),
                    sourceOps[1]->getOutputOriginIds()[0]);
  ASSERT_TRUE(found != joinInputOriginIds.end());
  found = std::find(joinInputOriginIds.begin(), joinInputOriginIds.end(),
                    sourceOps[2]->getOutputOriginIds()[0]);
  ...
  const std::vector<OriginId> &joinOutputOriginIds =
      joinOps[0]->getOutputOriginIds();
...
/**
 * @brief: This test infer origin id for a query involving join, aggregation,
 * and union operators. Therefore, the output origin ids for union operator
 * should return 3 distinct ids. 2 for each physical source of A on the left
 * side and 2 for each physical source of A with alias C on the right side.
 */
TEST_F(OriginIdInferencePhaseTest,
       testRuleForJoinAggregationAndUnionOperators) {
      ...
      Query::from("B")
          ...
          .joinWith(Query::from("A")
                        ...
                        .window(TumblingWindow::of(
                            EventTime(Attribute("value")), Seconds(3)))
                        .byKey(Attribute("id"))
  ...
  // Window aggregations
  auto aggregations =
      windowOps[0]->getWindowDefinition()->getWindowAggregation();
  ASSERT_EQ(aggregations.size(), 1);
  if (aggregations[0]->getType() ==
      NES::Windowing::WindowAggregationDescriptor::Type::Sum) {
    ASSERT_EQ(windowOps[0]->getInputOriginIds().size(), 3);
  } else if (aggregations[0]->getType() ==
             NES::Windowing::WindowAggregationDescriptor::Type::Avg) {
