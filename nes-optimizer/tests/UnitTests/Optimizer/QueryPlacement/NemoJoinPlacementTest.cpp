...
class NemoJoinPlacementTest : public Testing::BaseUnitTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NemoJoinPlacementTest.log",
  ...
  static TopologyPtr setupTopology(uint64_t layers, uint64_t nodesPerNode,
    ...
    topology->registerWorker(rootId, "localhost", 4000, 5000, resources,
                             properties, 0, 0);
  ...
  static Catalogs::Source::SourceCatalogPtr setupJoinSourceCatalog(
    ...
    std::map<NES::Catalogs::Source::SourceCatalogEntryPtr, std::set<uint64_t>>
        distributionMap;
    uint64_t keyVal = 10;
    uint64_t cnt = 0;
...
TEST_F(NemoJoinPlacementTest, testNemoJoin) {
  ...
  const std::vector<WorkerId>& sourceNodes = {WorkerId(5), WorkerId(6),
                                              WorkerId(7), WorkerId(8),
                                              WorkerId(9), WorkerId(10)};
  ...
  EXPECT_EQ(executionNodes.size(),
            10);  // topology contains 1 coordinator, 3 workers, 6 sources

  for (const auto& executionNode : executionNodes) {
    std::vector<DecomposedQueryPlanPtr> querySubPlans =
        executionNode->operator*()->getAllDecomposedQueryPlans(queryId);
    auto isSourceIt = std::find(sourceNodes.begin(), sourceNodes.end(),
                                executionNode->operator*()->getId());
    if (executionNode->operator*()->getId() == WorkerId(1)) {
      // coordinator should have 3 network sinks for the three joins at
      // intermediate nodes
      ...
      // node is an intermediate worker and not a source node which contains the
      // joins
      verifyChildrenOfType<LogicalJoinOperator>(querySubPlans, 1, 1);
      verifySourceOperators<NES::Network::NetworkSourceDescriptor>(
          querySubPlans, 1, 2);
    } else {
      // node is a source node and only contains the watermark and source
      // operator
