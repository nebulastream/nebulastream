...
class MlHeuristicPlacementTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MlHeuristicPlacementTest.log",
  ...
  void topologyGenerator() {
    ...
    std::vector<int> resources = {20, 20, 20, 20, 21, 22, 23,
                                  23, 24, 25, 25, 26, 27};
    ...
    for (int i = 0; i < (int)resources.size(); i++) {
      ...
      if (std::count(low_throughput_sources.begin(),
                     low_throughput_sources.end(), i)) {
      ...
      workerId = topology->registerWorker(workerId, "localhost", 123, 124,
                                          resources[i], properties, 0, 0);
      if (i == 0) {
        topology->addAsRootWorkerId(workerId);
      } else if (i > 1) {
        topology->addTopologyNodeAsChild(WorkerId(workerId.getRawValue() - 1),
                                         workerId);
    ...
    //                                   ->addField(createField("id",
    //                                   BasicType::UINT64))
    //                                   ->addField(createField("SepalLengthCm",
    //                                   BasicType::FLOAT32))
    //                                   ->addField(createField("SepalWidthCm",
    //                                   BasicType::FLOAT32))
    //                                   ->addField(createField("PetalLengthCm",
    //                                   BasicType::FLOAT32))
    //                                   ->addField(createField("PetalWidthCm",
    //                                   BasicType::FLOAT32))
    //                                   ->addField(createField("SpeciesCode",
    //                                   BasicType::UINT64))
    //                                   ->addField(createField("CreationTime",
    //                                   BasicType::UINT64));
    ...
    for (int source : sources) {
      auto streamCatalogEntry = Catalogs::Source::SourceCatalogEntry::create(
          physicalSource, logicalSource, WorkerId(source + 1));
...
TEST_F(MlHeuristicPlacementTest, testPlacingQueryWithMlHeuristicStrategy) {
  ...
  Query query =
      Query::from("iris")
          .inferModel(std::string(TEST_DATA_DIRECTORY) + "/iris.tflite",
                      {Attribute("SepalLengthCm"), Attribute("SepalWidthCm"),
                       Attribute("PetalLengthCm"), Attribute("PetalWidthCm")},
  ...
  std::vector querySubPlanSizeCompare = {
      totalQuerySubPlansOnNode1,  totalQuerySubPlansOnNode2,
      totalQuerySubPlansOnNode3,  totalQuerySubPlansOnNode4,
      totalQuerySubPlansOnNode5,  totalQuerySubPlansOnNode6,
      totalQuerySubPlansOnNode7,  totalQuerySubPlansOnNode8,
      totalQuerySubPlansOnNode9,  totalQuerySubPlansOnNode10,
      totalQuerySubPlansOnNode11, totalQuerySubPlansOnNode12,
  ...
  for (const auto& executionNode : executionNodes) {
    ...
    EXPECT_EQ(querySubPlans.size(),
              querySubPlanSizeCompare
                  [executionNode->operator*()->getId().getRawValue() - 1]);
