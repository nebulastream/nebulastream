...
public:
  ...
  void setupTopologyAndSourceCatalogForILP() {
    ...
    topologyForILP->registerWorker(rootNodeId, "localhost", 123, 124, 100,
                                   properties, 0, 0);
    topologyForILP->addAsRootWorkerId(rootNodeId);

    WorkerId middleNodeId = WorkerId{2};
    topologyForILP->registerWorker(middleNodeId, "localhost", 123, 124, 10,
                                   properties, 0, 0);
    topologyForILP->addTopologyNodeAsChild(rootNodeId, middleNodeId);

    WorkerId srcNodeId1 = WorkerId{3};
    topologyForILP->registerWorker(srcNodeId1, "localhost", 123, 124, 3,
                                   properties, 0, 0);
    ...
    topologyForILP->registerWorker(srcNodeId2, "localhost", 123, 124, 3,
                                   properties, 0, 0);
    ...
    CSVSourceTypePtr csvSourceType1 =
        CSVSourceType::create(sourceName1, "test2");
    ...
    auto sourceCatalogEntry1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource1, logicalSource1, srcNodeId1);
    ...
    CSVSourceTypePtr csvSourceType2 =
        CSVSourceType::create(sourceName2, "test2");
    ...
    auto sourceCatalogEntry2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource2, logicalSource2, srcNodeId2);
...
TEST_F(ILPPlacementTest, testPlacingFilterQueryWithILPStrategy) {
  ...
  auto typeInferencePhase =
      Optimizer::TypeInferencePhase::create(sourceCatalogForILP, udfCatalog);
  auto queryPlacementAmendmentPhase =
      Optimizer::QueryPlacementAmendmentPhase::create(
          globalExecutionPlan, topologyForILP, typeInferencePhase,
  ...
  auto topologySpecificQueryRewrite =
      Optimizer::TopologySpecificQueryRewritePhase::create(
          topologyForILP, sourceCatalogForILP,
  ...
  for (const auto &executionNode : lockedExecutionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(3)) {
      ...
      EXPECT_TRUE(actualRootOperator->getChildren()[0]
                      ->instanceOf<LogicalFilterOperator>());
      EXPECT_TRUE(actualRootOperator->getChildren()[0]
                      ->getChildren()[0]
...
TEST_F(ILPPlacementTest, testPlacingWindowQueryWithILPStrategy) {
  ...
  for (const auto &executionNode : lockedExecutionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(1)) {
      ...
      EXPECT_TRUE(
          actualRootOperator->getChildren()[0]->instanceOf<WindowOperator>());
      EXPECT_TRUE(actualRootOperator->getChildren()[0]
                      ->getChildren()[0]
...
/* Test query placement with ILP strategy with a window query with default
 * parameters*/
TEST_F(ILPPlacementTest,
       testPlacingWindowQueryWithILPStrategyWithDefaultParameters) {
  setupTopologyAndSourceCatalogForILP();
...
TEST_F(ILPPlacementTest, testPlacingSlidingWindowQueryWithILPStrategy) {
  ...
  // Prepare query plan
  Query query = Query::from("car")
                    .window(SlidingWindow::of(EventTime(Attribute("id")),
...
TEST_F(ILPPlacementTest, testPlacingMapQueryWithILPStrategy) {
  ...
  for (const auto &executionNode : executionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(3)) {
      ...
      ASSERT_EQ(actualRootOperators.size(), 1U);
      OperatorPtr actualRootOperator = actualRootOperators[0];
      EXPECT_EQ(actualRootOperator->getId(),
                queryPlan->getRootOperators()[0]->getId());
      EXPECT_TRUE(actualRootOperator->instanceOf<SinkLogicalOperator>());
      EXPECT_TRUE(actualRootOperator->getChildren()[0]
                      ->instanceOf<LogicalMapOperator>());
      EXPECT_TRUE(actualRootOperator->getChildren()[0]
                      ->getChildren()[0]
                      ->instanceOf<LogicalMapOperator>());
    }
...
/* Test query placement with ILP strategy - simple query of source - filter -
 * map - sink */
...
/* Test incremental query placement with ILP strategy - simple query of source -
 * filter - map - sink and then added filter - sink to filter operator*/
TEST_F(ILPPlacementTest, testPlacingUpdatedSharedQueryPlanWithILPStrategy) {
  ...
  auto queryMergerPhase = Optimizer::QueryMergerPhase::create(
      this->z3Context, coordinatorConfiguration->optimizer);
  ...
  // Assertion to check correct amount of shared query plans to deploy are
  // extracted.
  ...
  // Here we add two partially equivalent queries to build a shared query plan
  ...
  queryPlacementAmendmentPhase->execute(sharedQueryPlansToDeploy[0]);
  {
    ...
    for (const auto &executionNode : executionNodes) {
      if (executionNode->operator*()->getId() == WorkerId(3)) {
        ...
        std::vector<OperatorPtr> rootOperatorsForPlan1 =
            decomposedQueryPlan1->getRootOperators();
        ...
        EXPECT_TRUE(rootOperator1->getChildren()[0]
                        ->instanceOf<SourceLogicalOperator>());

        // Assertion for second subquery plan
        auto decomposedQueryPlan2 = decomposedQueryPlans[1U];
        std::vector<OperatorPtr> rootOperatorsForPlan2 =
            decomposedQueryPlan2->getRootOperators();
        ...
        ASSERT_EQ(rootOperatorsForPlan1.size(), 1U);
        OperatorPtr rootOperator1 = rootOperatorsForPlan1[0];
        ASSERT_EQ(rootOperator1->getId(),
                  queryPlan1->getRootOperators()[0]->getId());
        EXPECT_TRUE(rootOperator1->instanceOf<SinkLogicalOperator>());
        ASSERT_EQ(rootOperator1->getChildren().size(), 1U);
        EXPECT_TRUE(
            rootOperator1->getChildren()[0]->instanceOf<LogicalMapOperator>());
        ...
        ASSERT_EQ(rootOperatorsForPlan2.size(), 1U);
        OperatorPtr rootOperator2 = rootOperatorsForPlan2[0];
        ASSERT_EQ(rootOperator2->getId(),
                  queryPlan2->getRootOperators()[0]->getId());
        EXPECT_TRUE(rootOperator2->instanceOf<SinkLogicalOperator>());
        ASSERT_EQ(rootOperator2->getChildren().size(), 1U);
      }
    }
...
/* Test incremental query placement with ILP strategy - simple query of source -
 * filter - map - sink and then added map - sink and filter - sink to the filter
 * operator*/
TEST_F(ILPPlacementTest,
       testPlacingMulitpleUpdatesOnASharedQueryPlanWithILPStrategy) {
        ...
        for (const auto &decomposedQueryPlan : decomposedQueryPlans) {
          ...
          if (rootOperator->getId() ==
            ...
            EXPECT_TRUE(rootOperator->getChildren()[0]
                     ...
                     queryPlan3->getRootOperators()[0]->getId()) {
            EXPECT_TRUE(rootOperator->instanceOf<SinkLogicalOperator>());
            ASSERT_EQ(rootOperator->getChildren().size(), 1U);
            EXPECT_TRUE(rootOperator->getChildren()[0]
                            ->instanceOf<LogicalMapOperator>());
          } else {
            // Sub query plan should contain for one of the three sink operator
            // as its root
            FAIL();
          }
        }
...
/* Test query placement with ILP strategy for query: source - filter - map -
 * sink and then added map - sink and filter - sink to the filter operator*/
TEST_F(ILPPlacementTest,
       testPlacingMultipleSinkSharedQueryPlanWithILPStrategy) {
  ...
  queryPlacementAmendmentPhase->execute(sharedQueryPlansToDeploy[0]);
  {
    ...
    for (const auto &executionNode : executionNodes) {
      if (executionNode->operator*()->getId() == WorkerId(3)) {
        ...
        EXPECT_TRUE(rootOperator2->instanceOf<SinkLogicalOperator>());
        ASSERT_EQ(rootOperator2->getChildren().size(), 1U);
        EXPECT_TRUE(rootOperator2->getChildren()[0]
                        ->instanceOf<SourceLogicalOperator>());
      } else if (executionNode->operator*()->getId() == WorkerId(1)) {
        // map should be placed on cloud node
        auto decomposedQueryPlans =
...
/* Test query placement with ILP strategy - this tests checks if ILP strategy
 * works when operators have multiple children/parents */
TEST_F(ILPPlacementTest, testMultipleChildrenQueryWithILPStrategy) {
  ...
  for (const auto &executionNode : lockedExecutionNodes) {
    if (executionNode->operator*()->getId() == WorkerId(3)) {
      ...
      OperatorPtr actualRootOperator = actualRootOperators[0];
      EXPECT_TRUE(actualRootOperator->getChildren()[0]
                      ->instanceOf<LogicalUnionOperator>());
    } else if (executionNode->operator*()->getId() == WorkerId(1)) {
      auto decomposedQueryPlans =
          executionNode->operator*()->getAllDecomposedQueryPlans(queryId);
      ASSERT_EQ(decomposedQueryPlans.size(), 1U);
