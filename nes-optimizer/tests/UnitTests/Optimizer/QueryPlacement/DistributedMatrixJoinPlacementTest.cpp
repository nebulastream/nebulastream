...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("DistributedMatrixJoinPlacementTest.log",
   ...
   */
  static TopologyPtr setupTopology(uint64_t layers, uint64_t nodesPerNode,
    ...
    topology->registerWorker(WorkerId(rootId), "localhost", 4000, 5000,
    ...
    for (uint64_t i = 2; i <= layers; i++) {
      ...
      for (auto parent : parents) {
        ...
        for (uint64_t j = 0; j < nodeCnt; j++) {
          ...
          nodeId++;
          topology->registerWorker(WorkerId(nodeId), "localhost", 4000 + nodeId,
                                   5000 + nodeId, resources, properties, 0, 0);
          topology->removeTopologyNodeAsChild(WorkerId(rootId),
                                              WorkerId(nodeId));
    ...
    NES_DEBUG("DistributedMatrixJoinPlacementTest: topology: {}",
              topology->toString());
    return topology;
  }

  /**
   * Creates a SourceCatalog where all even sources are part of the left join
   * and odd nodes part of the right join.
   */
  static Catalogs::Source::SourceCatalogPtr
  setupJoinSourceCatalog(const std::vector<WorkerId> &sourceNodes) {
    ...
    for (auto nodeId : sourceNodes) {
      ...
      std::string logSourceName =
          isEven ? logSourceNameLeft : logSourceNameRight;
      std::string phySourceName =
          isEven ? "left_" + nodeId.toString() : "right_" + nodeId.toString();
      CSVSourceTypePtr csvSourceType =
          CSVSourceType::create(logSourceName, phySourceName);

      auto physicalSource = PhysicalSource::create(csvSourceType);
      auto logicalSource = sourceCatalog->getLogicalSource(logSourceName);

      auto entry = Catalogs::Source::SourceCatalogEntry::create(
          physicalSource, logicalSource, nodeId);
  ...
  /**
   * Performs an operator placement based on the given query, topology, source
   * catalog and optimizer config.
   */
  static std::tuple<SharedQueryId, GlobalExecutionPlanPtr> runPlacement(
      const Query &query, const TopologyPtr &topology,
    ...
    auto topologySpecificQueryRewrite =
        Optimizer::TopologySpecificQueryRewritePhase::create(
            topology, sourceCatalog, optimizerConfig, statisticProbeHandler);
    ...
    auto queryPlacementPhase = Optimizer::QueryPlacementAmendmentPhase::create(
  ...
  template <typename T>
  static void
  verifyChildrenOfType(const std::vector<DecomposedQueryPlanPtr> &querySubPlans,
  ...
  template <typename T>
  static void
  verifySourceOperators(std::vector<DecomposedQueryPlanPtr> &querySubPlans,
    ...
    std::vector<SourceLogicalOperatorPtr> sourceOperators =
        querySubPlan->getSourceOperators();
    ...
    for (const auto &sourceOperator : sourceOperators) {
      ...
      auto sourceDescriptor =
          sourceOperator->as_if<SourceLogicalOperator>()->getSourceDescriptor();
...
TEST_F(DistributedMatrixJoinPlacementTest, testMatrixJoin) {
  ...
  const std::vector<WorkerId> &sourceNodes = {WorkerId(2), WorkerId(3),
                                              WorkerId(4), WorkerId(5)};
  ...
  optimizerConfig.joinOptimizationMode =
  ...
  Query query = Query::from(logSourceNameLeft)
                    ...
                    .window(TumblingWindow::of(
                        EventTime(Attribute("timestamp")), Milliseconds(1000)))
                    .sink(NullOutputSinkDescriptor::create());

  auto outputTuple = runPlacement(query, topology, sourceCatalog,
                                  optimizerConfig, statisticProbeHandler);
  auto queryId = std::get<0>(outputTuple);
  auto executionPlan = std::get<1>(outputTuple);
  auto executionNodes =
      executionPlan->getLockedExecutionNodesHostingSharedQueryId(queryId);
  EXPECT_EQ(executionNodes.size(),
            5); // topology contains 1 coordinator, 4 workers

  for (const auto &executionNode : executionNodes) {
    std::vector<DecomposedQueryPlanPtr> querySubPlans =
        executionNode->operator*()->getAllDecomposedQueryPlans(queryId);
    if (executionNode->operator*()->getId() == WorkerId(1)) {
      ...
      verifySourceOperators<NES::Network::NetworkSourceDescriptor>(
          querySubPlans, 1, 2 * sourceNodes.size());
    } else {
      // 2x replication factor, therefore each source should have 2 network
      // sources
      verifyChildrenOfType<WatermarkAssignerLogicalOperator>(querySubPlans, 1,
                                                             2);
