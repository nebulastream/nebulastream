...
class DistributedMatrixJoinPlacementTest : public Testing::BaseUnitTest {
 public:
   ...
   */
  static Catalogs::Source::SourceCatalogPtr setupJoinSourceCatalog(
  ...
  static void verifyChildrenOfType(
      const std::vector<DecomposedQueryPlanPtr>& querySubPlans,
      uint64_t expectedSubPlanSize = 1, uint64_t expectedRootOperators = 1) {
  ...
  static void verifySourceOperators(
      ...
      uint64_t expectedSubPlanSize, uint64_t expectedSourceOperatorSize) {
    ASSERT_EQ(querySubPlans.size(), expectedSubPlanSize);
