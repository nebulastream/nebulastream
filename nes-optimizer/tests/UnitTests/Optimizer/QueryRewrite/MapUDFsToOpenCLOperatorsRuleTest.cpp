...
TEST_F(MapUDFsToOpenCLOperatorsRuleTest, testAddingSingleSourceRenameOperator) {
    ...
    auto mapJavaUDFOperators = queryPlan->getOperatorByType<LogicalMapUDFOperator>();
    ...
    mapJavaUDFOperators = updatedQueryPlan->getOperatorByType<LogicalMapUDFOperator>();
    ...
    EXPECT_TRUE(openCLOperators[0]->getParents()[0]->instanceOf<LogicalSinkOperator>());
    EXPECT_EQ(openCLOperators[0]->getChildren().size(), 1);
    EXPECT_TRUE(openCLOperators[0]->getChildren()[0]->instanceOf<LogicalSourceOperator>());
}
...
TEST_F(MapUDFsToOpenCLOperatorsRuleTest, testAddingMultipleSourceRenameOperator) {
    ...
    mapJavaUDFOperators = queryPlan->getOperatorByType<LogicalMapUDFOperator>();
    EXPECT_TRUE(mapJavaUDFOperators.empty());
    ...
    EXPECT_TRUE(openCLOperators[1]->getChildren()[0]->instanceOf<LogicalSourceOperator>());
}
