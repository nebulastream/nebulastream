...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("FilterPushDownRuleTest.log",
...
TEST_F(FilterPushDownRuleTest, testPushingFilterBelowProjectionWithRename) {
  ...
  ++itr;
  EXPECT_TRUE(filterOperator->equal((*itr)));
  LogicalFilterOperatorPtr filterLogicalOperator =
      (*itr)->as<LogicalFilterOperator>();
  std::vector<FieldAccessExpressionNodePtr> filterAttributeNames =
      Optimizer::FilterPushDownRule::getFilterAccessExpressions(
          filterLogicalOperator->getPredicate());
  bool attributeChangedName =
      std::any_of(filterAttributeNames.begin(), filterAttributeNames.end(),
...
TEST_F(FilterPushDownRuleTest, testPushingOneFilterBelowABinaryOperator) {
  ...
  // the order of the children of the union operator might have changed, but it
  // both orders are valid.
  auto leavesLeft = unionOperator->getChildren()[0]->getAllLeafNodes();
  if (std::find(leavesLeft.begin(), leavesLeft.end(), srcOperatorSQ) ==
      leavesLeft.end()) {
    ++itr;
...
TEST_F(FilterPushDownRuleTest,
       testPushingTwoFiltersAlreadyBelowABinaryOperator) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  setupSensorNodeAndSourceCatalog(sourceCatalog);
...
TEST_F(FilterPushDownRuleTest,
       testPushingOneFilterAlreadyBelowAndTwoFiltersBelowABinaryOperator) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  setupSensorNodeAndSourceCatalog(sourceCatalog);
...
TEST_F(FilterPushDownRuleTest,
       testPushingTwoFiltersAlreadyAtBottomAndTwoFiltersBelowABinaryOperator) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  setupSensorNodeAndSourceCatalog(sourceCatalog);
  ...
  // Validate
  // The order of the branches of the union operator matters for this tests and
  // stays the same in this test.
  DepthFirstNodeIterator updatedQueryPlanNodeIterator(
      updatedPlan->getRootOperators()[0]);
  itr = updatedQueryPlanNodeIterator.begin();
  EXPECT_TRUE(sinkOperator->equal((*itr)));
  NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
            sinkOperator->toString(), (*itr)->toString());
  ++itr;
  EXPECT_TRUE(mapOperatorPQ1->equal((*itr)));
  NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
            mapOperatorPQ1->toString(), (*itr)->toString());
  ++itr;
  EXPECT_TRUE(mapOperatorPQ2->equal((*itr)));
  NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
            mapOperatorPQ2->toString(), (*itr)->toString());
  ++itr;
  EXPECT_TRUE(mergeOperator->equal((*itr)));
  NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
            mergeOperator->toString(), (*itr)->toString());

  // the order of the children of the union operator might have changed, but it
  // both orders are valid.
  auto leavesLeft = mergeOperator->getChildren()[0]->getAllLeafNodes();
  if (std::find(leavesLeft.begin(), leavesLeft.end(), srcOperatorPQ) ==
      leavesLeft.end()) {
    ++itr;
    EXPECT_TRUE(filterOperatorPQ1->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              filterOperatorPQ1->toString(), (*itr)->toString());
    ++itr;
    EXPECT_TRUE(filterOperatorPQ2->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              filterOperatorPQ2->toString(), (*itr)->toString());
    ++itr;
    EXPECT_TRUE(filterOperatorPQ3->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              filterOperatorPQ3->toString(), (*itr)->toString());
    ++itr;
    EXPECT_TRUE(srcOperatorPQ->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              srcOperatorPQ->toString(), (*itr)->toString());
    ++itr;
    EXPECT_TRUE(filterOperatorPQ1->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              filterOperatorPQ1->toString(), (*itr)->toString());
    ...
    EXPECT_TRUE(filterOperatorSQ->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              filterOperatorSQ->toString(), (*itr)->toString());
    ++itr;
    EXPECT_TRUE(srcOperatorSQ->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
              srcOperatorSQ->toString(), (*itr)->toString());
  } else {
    ++itr;
    EXPECT_TRUE(filterOperatorPQ1->equal((*itr)));
    NES_DEBUG("Expected Plan Node: {}  Actual in updated Query plan: {}",
...
TEST_F(FilterPushDownRuleTest,
       testPushingFilterBelowThreeMapsWithOneFieldSubstitution) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  ...
  PhysicalSourcePtr physicalSource = PhysicalSource::create(csvSourceType);
  LogicalSourcePtr logicalSource =
      LogicalSource::create("example", Schema::create());
  auto sce1 = Catalogs::Source::SourceCatalogEntry::create(
      physicalSource, logicalSource, physicalNode->getId());
  ...
  itr = updatedQueryPlanNodeIterator.begin();
  EXPECT_TRUE(sinkOperator->equal((*itr)));
  NES_DEBUG("New filter Predicate: {}",
            filterOperatorPQ1->as<LogicalFilterOperator>()
                ->getPredicate()
                ->toString());
  ++itr;
  EXPECT_TRUE(mapOperatorPQ1->equal((*itr)));
  ...
  NES_DEBUG("filterOperatorPQ1 Predicate: {}",
            filterOperatorPQ1->as<LogicalFilterOperator>()
                ->getPredicate()
                ->getNodesByType<SubExpressionNode>()[0]
                ->toString());
  NES_DEBUG("mapOperatorPQ2 map expression: {}",
            mapOperatorPQ2->as<LogicalMapOperator>()
                ->getMapExpression()
                ->getAssignment()
                ->toString());
  EXPECT_TRUE(filterOperatorPQ1->as<LogicalFilterOperator>()
                  ->getPredicate()
                  ->getNodesByType<SubExpressionNode>()[0]
                  ->equal(mapOperatorPQ2->as<LogicalMapOperator>()
                              ->getMapExpression()
                              ->getAssignment()));
  ++itr;
  EXPECT_TRUE(srcOperatorPQ->equal((*itr)));
...
TEST_F(FilterPushDownRuleTest,
       testPushingFilterBelowTwoMapsWithTwoFieldSubstitutions) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  NES::SchemaPtr schema = NES::Schema::create()
                              ->addField("id", NES::BasicType::UINT64)
  ...
  NES_DEBUG("filterOperatorPQ1 Predicate: {}",
            filterOperatorPQ1->as<LogicalFilterOperator>()
                ->getPredicate()
                ->getNodesByType<MulExpressionNode>()[0]
                ->toString());
  NES_DEBUG("mapOperatorPQ3 map expression: {}",
            mapOperatorPQ3->as<LogicalMapOperator>()
                ->getMapExpression()
                ->getAssignment()
                ->toString());
  EXPECT_TRUE(filterOperatorPQ1->as<LogicalFilterOperator>()
                  ->getPredicate()
                  ->getNodesByType<MulExpressionNode>()[0]
                  ->equal(mapOperatorPQ3->as<LogicalMapOperator>()
                              ->getMapExpression()
                              ->getAssignment()));
  ++itr;
  EXPECT_TRUE(srcOperatorPQ->equal((*itr)));
...
/* tests if a filter is correctly pushed below a join if all its attributes
belong to source 1. The order of the operators in the updated query plan is
validated, and it is checked that the input and output schema of the filter that
is now at a new position is still correct */
TEST_F(FilterPushDownRuleTest, testPushingFilterBelowJoinToSrc1) {
  ...
  // type inference
  auto typeInferencePhase = Optimizer::TypeInferencePhase::create(
...
/* tests if a filter is correctly pushed below a join if all its attributes are
part of the join condition. The order of the operators in the updated query plan
is validated, and it is checked that the input and output schema of the filter
that is now at a new position is still correct. Original filter would go to the
left branch*/
TEST_F(FilterPushDownRuleTest, testPushingFilterBelowJoinToBothSourcesLeft) {
  ...
  ++itr;
  std::vector<std::string> accessedFields;
  accessedFields.push_back("src2$id"); // a duplicate filter that accesses
                                       // src2$id should be pushed down
  ...
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getInputSchema()->equals(
      srcOperatorSrc2->as<UnaryOperator>()->getOutputSchema()));
  ++itr;
  EXPECT_TRUE(srcOperatorSrc2->equal((*itr)));
  ++itr;
  EXPECT_TRUE(watermarkOperatorAboveSrc1->equal((*itr)));
  ...
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getInputSchema()->equals(
      srcOperatorSrc1->as<UnaryOperator>()->getOutputSchema()));
  ++itr;
  EXPECT_TRUE(srcOperatorSrc1->equal((*itr)));
}
...
TEST_F(FilterPushDownRuleTest, testPushingFilterBelowJoinToBothSourcesRight) {
  ...
  ++itr;
  std::vector<std::string> accessedFields;
  accessedFields.push_back("src1$id"); // a duplicate filter that accesses
                                       // src2$id should be pushed down
  EXPECT_TRUE(isFilterAndAccessesCorrectFields((*itr), accessedFields));
  // check if schema updated correctly
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
...
TEST_F(FilterPushDownRuleTest,
       testPushingOneFilterBelowWindowNotPossibleMultipleAttributes) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  setupSensorNodeAndSourceCatalog(sourceCatalog);
...
TEST_F(FilterPushDownRuleTest,
       testPushingDifferentFiltersThroughDifferentOperators) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  ...
  Query query =
      Query::from("src1")
          ...
          .window(TumblingWindow::of(EventTime(Attribute("ts")),
                                     Milliseconds(1000)))
          .filter(Attribute("src3$id") !=
                  0) // should be pushed directly above every src with the
                     // adequate predicate for each source as the filter
                     // predicate is applied to the join key
          .filter(Attribute("A") !=
                  1) // should be pushed above src1, plus, substitute field
                     // access with map transformation (Attribute("A") =
                     // Attribute("A") * 3)
          .filter(Attribute("B") !=
                  2) // should be pushed above id filter above src2
          .filter(Attribute("C") !=
                  3) // should be pushed above id filter above src3
          .filter(Attribute("A") > 0 ||
                  Attribute("B") > 0) // should be pushed above join src1 & src2
          .filter(Attribute("A") > 0 ||
                  Attribute("C") >
                      0) // can not be pushed through any join; would need to
                         // split the filter otherwise (Not implemented)
          .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
  ...
  NES_DEBUG("FilterOperatorAorC Predicate: {}",
            filterOperatorAorC->as<LogicalFilterOperator>()
                ->getPredicate()
                ->toString());
  // check if schema still correct
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
      sinkOperator->as<UnaryOperator>()->getInputSchema()));
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getInputSchema()->equals(
  ...
  NES_DEBUG(
      "filterOperatorC Predicate: {}",
      filterOperatorC->as<LogicalFilterOperator>()->getPredicate()->toString());
  // check if schema updated correctly
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
      mapOperatorTs->as<UnaryOperator>()->getInputSchema()));
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getInputSchema()->equals(
      srcOperatorSrc3->as<UnaryOperator>()->getOutputSchema()));
  ++itr;
  EXPECT_TRUE(filterOperatorId->equal((*itr)));
  NES_DEBUG("filterOperatorId: {}", filterOperatorId->toString());
  NES_DEBUG("filterOperatorId Predicate: {}",
            filterOperatorId->as<LogicalFilterOperator>()
                ->getPredicate()
                ->toString());
  // check if schema updated correctly
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
  ...
  NES_DEBUG("filterOperatorAorB Predicate: {}",
            filterOperatorAorB->as<LogicalFilterOperator>()
                ->getPredicate()
                ->toString());
  // check if schema updated correctly
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
  ...
  NES_DEBUG(
      "filterOperatorB Predicate: {}",
      filterOperatorB->as<LogicalFilterOperator>()->getPredicate()->toString());
  // check if schema updated correctly
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
  ...
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getInputSchema()->equals(
      mapOperatorA->as<UnaryOperator>()->getOutputSchema()));
  ++itr;
  ...
  NES_DEBUG("filterOperatorA Predicate: {}",
            filterOperatorA->as<LogicalFilterOperator>()
                ->getPredicate()
                ->getNodesByType<MulExpressionNode>()[0]
                ->toString());
  NES_DEBUG("mapOperatorA map expression: {}",
            mapOperatorA->as<LogicalMapOperator>()
                ->getMapExpression()
                ->getAssignment()
                ->toString());
  EXPECT_TRUE(filterOperatorA->as<LogicalFilterOperator>()
                  ->getPredicate()
                  ->getNodesByType<MulExpressionNode>()[0]
                  ->equal(mapOperatorA->as<LogicalMapOperator>()
                              ->getMapExpression()
                              ->getAssignment()));
  ++itr;
  std::vector<std::string> accessedFields2;
  accessedFields2.push_back("src1$id"); // a duplicate filter that accesses
                                        // src2$id should be pushed down
  EXPECT_TRUE(isFilterAndAccessesCorrectFields((*itr), accessedFields2));
  // check if schema updated correctly
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getOutputSchema()->equals(
      mapOperatorA->as<UnaryOperator>()->getInputSchema()));
  EXPECT_TRUE((*itr)->as<UnaryOperator>()->getInputSchema()->equals(
      srcOperatorSrc1->as<UnaryOperator>()->getOutputSchema()));
  ++itr;
  EXPECT_TRUE(srcOperatorSrc1->equal((*itr)));
