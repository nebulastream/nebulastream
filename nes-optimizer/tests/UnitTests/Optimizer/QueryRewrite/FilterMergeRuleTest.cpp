...
class FilterMergeRuleTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("PredicateReorderingRuleTest.log",
  ...
  bool isFilterAndHasCorrectPredicate(NodePtr filter,
                                      ExpressionNodePtr expectedPredicate) {
    if (!filter->instanceOf<LogicalFilterOperator>()) {
      return false;
    }

    auto filterPredicates = filter->as<LogicalFilterOperator>()
                                ->getPredicate()
                                ->getAndFlattenAllChildren(true);
...
TEST_F(FilterMergeRuleTest, testMergeTwoConsecutiveFilters) {
  ...
  auto predicate1 =
      filterOperator1->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  const NodePtr filterOperator2 = (*itr);
  auto predicate2 =
      filterOperator2->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  ...
  ++itr;
  ExpressionNodePtr expectedPredicate =
      AndExpressionNode::create(predicate1, predicate2);
...
TEST_F(FilterMergeRuleTest, testMergeThreeConsecutiveComplexFilters) {
  ...
  auto predicate3 =
      filterOperator3->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  const NodePtr srcOperator = (*itr);
...
TEST_F(FilterMergeRuleTest, testMergeDifferentFilterGroups) {
  ...
  auto predicatePQ1 =
      filterOperatorPQ1->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  const NodePtr filterOperatorPQ2 = (*itr);
  auto predicatePQ2 =
      filterOperatorPQ2->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  const NodePtr filterOperatorPQ3 = (*itr);
  auto predicatePQ3 =
      filterOperatorPQ3->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  ...
  auto predicateSQ =
      filterOperatorSQ->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  const NodePtr srcOperatorSQ = (*itr);
  ++itr;
  const NodePtr filterOperatorPQ4 = (*itr);
  auto predicatePQ4 =
      filterOperatorPQ4->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  const NodePtr filterOperatorPQ5 = (*itr);
  auto predicatePQ5 =
      filterOperatorPQ5->as<LogicalFilterOperator>()->getPredicate();
  ++itr;
  ...
  EXPECT_TRUE(sinkOperator->equal((*itr)));
  ++itr;
  ExpressionNodePtr expectedPredicate1 =
      AndExpressionNode::create(predicatePQ1, predicatePQ2);
  expectedPredicate1 =
      AndExpressionNode::create(expectedPredicate1, predicatePQ3);
  ...
  ++itr;
  ExpressionNodePtr expectedPredicate2 =
      AndExpressionNode::create(predicatePQ4, predicatePQ5);
