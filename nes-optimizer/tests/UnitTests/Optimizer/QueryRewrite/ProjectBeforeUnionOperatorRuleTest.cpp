...
class ProjectBeforeUnionOperatorRuleTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ProjectBeforeUnionOperatorRuleTest.log",
  ...
  void setupSensorNodeAndSourceCatalog(
    ...
    auto sce1 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource1, logicalSource1, physicalNode->getId());
    auto sce2 = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource1, logicalSource2, physicalNode->getId());
...
TEST_F(ProjectBeforeUnionOperatorRuleTest,
       testAddingProjectForUnionWithDifferentSchemas) {
  // Prepare
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
  ...
  auto projectionOperators =
      queryPlan->getOperatorByType<LogicalProjectionOperator>();
  ...
  auto projectBeforeUnionOperatorRule =
      Optimizer::ProjectBeforeUnionOperatorRule::create();
...
TEST_F(ProjectBeforeUnionOperatorRuleTest,
       testAddingProjectForUnionWithSameSchemas) {
  // Prepare
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  setupSensorNodeAndSourceCatalog(sourceCatalog);
