...
class AttributeSortRuleTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("AttributeSortRuleTest.log",
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator1) {
  ...
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=FieldAccessNode(a[Undefined])+"
      "FieldAccessNode(b[Undefined])).SOURCE(src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
                              .begin()
                              ->second.begin();
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator2) {
  ...
  Query query = Query::from("src")
                    .map(Attribute("b") =
                             Attribute("c") + Attribute("b") + Attribute("a"))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator3) {
  ...
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query = Query::from("src")
                    .map(Attribute("b") = (Attribute("d") + Attribute("a")) +
                                          ((Attribute("c") + Attribute("b")) +
                                           Attribute("a")))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
  ...
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=FieldAccessNode(a[Undefined])+"
      "FieldAccessNode(a[Undefined]"
      ")+FieldAccessNode(b[Undefined])+FieldAccessNode(c[Undefined])+"
      "FieldAccessNode(d[Undefined])).SOURCE(src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
                              .begin()
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator4) {
  ...
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query = Query::from("src")
                    .map(Attribute("b") = (Attribute("d") + Attribute("c")) *
  ...
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=FieldAccessNode(a[Undefined])+"
      "FieldAccessNode(b[Undefined]"
      ")*FieldAccessNode(c[Undefined])+FieldAccessNode(d[Undefined])).SOURCE("
      "src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator5) {
  ...
  const auto* expectedSignature =
      ...
      ")>FieldAccessNode(a[Undefined])+FieldAccessNode(c[Undefined])).SOURCE("
      "src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
  ...
  Query query = Query::from("src")
                    .map(Attribute("b") =
                             ((Attribute("d") + Attribute("c")) *
                              Attribute("a")) > Attribute("c") + Attribute("a"))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator8) {
  ...
  Query query = Query::from("src")
                    .map(Attribute("b") =
                             ((Attribute("d") + Attribute("c")) *
                              Attribute("d")) < Attribute("c") + Attribute("a"))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
  ...
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=FieldAccessNode(a[Undefined])+"
      "FieldAccessNode(c[Undefined])>"
      "FieldAccessNode(c[Undefined])+FieldAccessNode(d[Undefined])*"
      "FieldAccessNode(d[Undefined])).SOURCE(src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator14) {
  ...
  Query query =
      Query::from("src")
          .map(Attribute("b") =
                   !(((Attribute("d") + Attribute("c")) * Attribute("d")) &&
                     Attribute("c") + Attribute("a")))
          .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator15) {
  ...
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query = Query::from("src")
                    .map(Attribute("b") = 10 + Attribute("a"))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
  ...
  EXPECT_TRUE(rootOperators.size() == 1);
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=ConstantValue(BasicValue(10))+"
      "FieldAccessNode(a[Undefined])).SOURCE(src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
                              .begin()
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator16) {
  ...
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query = Query::from("src")
                    .map(Attribute("b") = (10 + Attribute("c")) *
                                          ((Attribute("a") + Attribute("b"))))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
  ...
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=ConstantValue(BasicValue(10))+"
      "FieldAccessNode(c[Undefined]"
      ")*FieldAccessNode(a[Undefined])+FieldAccessNode(b[Undefined])).SOURCE("
      "src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator17) {
  ...
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query = Query::from("src")
                    .map(Attribute("b") = (100 + Attribute("c") + 10) +
  ...
  const auto* expectedSignature =
      "SINK().MAP(FieldAccessNode(b[Undefined])=ConstantValue(BasicValue(10))+"
      "ConstantValue(BasicValue(100))+FieldAccessNode(a["
      "Undefined])+FieldAccessNode(b[Undefined])+FieldAccessNode(c[Undefined]))"
      ".SOURCE(src)";
  auto actualSignature = *rootOperators[0]
                              ->as<LogicalOperator>()
                              ->getHashBasedSignature()
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleFilterOperator1) {
  ...
  SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
  Query query = Query::from("src")
                    .filter(((Attribute("d") + Attribute("c")) *
                             Attribute("d")) > Attribute("c") + Attribute("a"))
                    .sink(printSinkDescriptor);
  const QueryPlanPtr queryPlan = query.getQueryPlan();
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForFilterOperator5) {
  ...
  EXPECT_TRUE(rootOperators.size() == 1);
  const auto* expectedSignature =
      "SINK().FILTER(!FieldAccessNode(a[Undefined])+FieldAccessNode(c["
      "Undefined])=="
      "FieldAccessNode(c[Undefined])+FieldAccessNode(d[Undefined])*"
      "FieldAccessNode(d[Undefined])).SOURCE(src)";
  auto actualSignature = *rootOperators[0]
