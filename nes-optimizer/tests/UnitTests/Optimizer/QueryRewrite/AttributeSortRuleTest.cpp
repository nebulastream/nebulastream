...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator1)
{
    ...
    auto signatureInferencePhase = Optimizer::SignatureInferencePhase::create(
        /*Z3context*/ nullptr, Optimizer::QueryMergerRule::HashSignatureBasedCompleteQueryMergerRule);
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator2)
{
    // Prepare
    SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
    Query query = Query::from("src").map(Attribute("b") = Attribute("c") + Attribute("b") + Attribute("a")).sink(printSinkDescriptor);
    const QueryPlanPtr queryPlan = query.getQueryPlan();
...
TEST_F(AttributeSortRuleTest, testAttributeSortRuleForMapOperator16)
{
    ...
    SinkDescriptorPtr printSinkDescriptor = PrintSinkDescriptor::create();
    Query query
        = Query::from("src").map(Attribute("b") = (10 + Attribute("c")) * ((Attribute("a") + Attribute("b")))).sink(printSinkDescriptor);
    const QueryPlanPtr queryPlan = query.getQueryPlan();
