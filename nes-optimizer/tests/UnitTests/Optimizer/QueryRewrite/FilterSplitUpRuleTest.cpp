...
class FilterSplitUpRuleTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("FilterSplitUpRuleTest.log",
  ...
  bool isFilterAndAccessesCorrectFields(
      NodePtr filter, std::vector<std::string> accessedFields) {
    ...
    DepthFirstNodeIterator depthFirstNodeIterator(
        filter->as<LogicalFilterOperator>()->getPredicate());
    for (auto itr = depthFirstNodeIterator.begin();
      ...
      if ((*itr)->instanceOf<FieldAccessExpressionNode>()) {
        ...
        if (std::find(accessedFields.begin(), accessedFields.end(),
...
TEST_F(FilterSplitUpRuleTest, testSplittingFilterWithOneAnd) {
  ...
  DepthFirstNodeIterator queryPlanNodeIterator(
  ...
  // Validate
  DepthFirstNodeIterator updatedQueryPlanNodeIterator(
      updatedPlan->getRootOperators()[0]);
...
// filter(!(!e1) || (!e2)) translates temporarily to filter( !(!(e1)) &&
// !(!(e2)) ) which translates to filter(e1)->filter(e2)
TEST_F(FilterSplitUpRuleTest, testSplittingFilterNotNegatedOr) {
  ...
  EXPECT_TRUE((*itr)
                  ->as<LogicalFilterOperator>()
                  ->getPredicate()
                  ->instanceOf<EqualsExpressionNode>());
  ++itr;
