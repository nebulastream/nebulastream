...
class QueryContainmentIdentificationTest
    : public Testing::BaseUnitTest,
      public testing::WithParamInterface<
          std::vector<QueryContainmentTestEntry>> {
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("QueryContainmentIdentificationTest.log",
  ...
  void TearDown() override {
    NES_DEBUG("QueryContainmentIdentificationTest: Tear down "
              "QueryContainmentIdentificationTest test case.");
  }
    ...
    return std::vector<QueryContainmentTestEntry>{
        ...
        QueryContainmentTestEntry(
            ...
            Query::from("windTurbines")
                ...
                .apply(Sum(Attribute("value")))
                .joinWith(Query::from("households")
                              .project(Attribute("value3"), Attribute("id"),
                                       Attribute("ts").as("start")))
                .where(Attribute("id") == Attribute("id"))
                .window(
                    TumblingWindow::of(EventTime(Attribute("start")), Hours(1)))
                .map(Attribute("DifferenceProducedConsumedPower") =
                         Attribute("value") - Attribute("value3"))
    ...
    return std::vector<QueryContainmentTestEntry>{
        ...
        QueryContainmentTestEntry(
            ...
            Query::from("windTurbines")
                ...
                .sink(PrintSinkDescriptor::create()),
            Optimizer::ContainmentRelationship::NO_CONTAINMENT),
        // another limit of our algorithm. If one query does not have a window,
        // we cannot detect containment
        QueryContainmentTestEntry(
            "TestNoContainment",
            Query::from("windTurbines")
                .unionWith(Query::from("solarPanels"))
  ...
  static auto createFilterContainmentCases() {
    return std::vector<QueryContainmentTestEntry>{
        ...
        QueryContainmentTestEntry(
            "TestProjectionContainment",
            Query::from("windTurbines")
                .project(Attribute("value").as("newValue"), Attribute("id1"),
                         Attribute("ts"))
                .sink(PrintSinkDescriptor::create()),
            Query::from("windTurbines")
                .project(Attribute("value"), Attribute("id1"),
                         Attribute("value1"), Attribute("ts"), Attribute("id"))
                .sink(PrintSinkDescriptor::create()),
            Optimizer::ContainmentRelationship::NO_CONTAINMENT)};
  }
    ...
    return std::vector<QueryContainmentTestEntry>{
        ...
        QueryContainmentTestEntry(
            ...
            Query::from("windTurbines")
                ...
                .apply(Sum(Attribute("value")),
                       Min(Attribute("value"))
                           ->as(FieldAccessExpressionNode::create("newValue")))
                .sink(PrintSinkDescriptor::create()),
            Query::from("windTurbines")
                .window(TumblingWindow::of(EventTime(Attribute("ts")),
                                           Milliseconds(1000)))
        ...
        QueryContainmentTestEntry(
            ...
            Query::from("windTurbines")
                .unionWith(Query::from("solarPanels"))
                .window(SlidingWindow::of(EventTime(Attribute("ts")),
                                          Milliseconds(1000), Milliseconds(20)))
                .byKey(Attribute("id"))
                .apply(Sum(Attribute("value")))
                .sink(PrintSinkDescriptor::create()),
            Optimizer::ContainmentRelationship::NO_CONTAINMENT)};
...
/**
 * @brief tests if the correct containment relationship is returned by the
 * signature containment util
 */
TEST_P(QueryContainmentIdentificationTest, testContainmentIdentification) {
  ...
  for (auto containmentCase : containmentCases) {
    ...
    SinkLogicalOperatorPtr sinkOperatorSQPQuery =
        queryPlanSQPQuery->getSinkOperators()[0];
    SinkLogicalOperatorPtr sinkOperatorNewQuery =
        queryPlanSQPQuery->getSinkOperators()[0];
    auto signatureContainmentUtil =
        Optimizer::SignatureContainmentCheck::create(context, true);
    ...
    // Check if the host and target sink operator signatures have a containment
    // relationship
    Optimizer::ContainmentRelationship containment =
        signatureContainmentUtil
            ->checkContainmentForBottomUpMerging(sqpSink, newSink)
            ->containmentRelationship;
    NES_TRACE("Z3SignatureBasedContainmentBasedCompleteQueryMergerRule: "
              "containment: {}",
...
INSTANTIATE_TEST_CASE_P(
    testContainment, QueryContainmentIdentificationTest,
    ::testing::Values(
        QueryContainmentIdentificationTest::createEqualityCases(),
    ...
    [](const testing::TestParamInfo<
        QueryContainmentIdentificationTest::ParamType> &info) {
