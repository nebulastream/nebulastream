...
/**
 * @brief This test is for creation of a global query plan and adding a query
 * plan with empty query id
 */
TEST_F(GlobalQueryPlanTest,
       testNewGlobalQueryPlanAndAddingQueryPlanWithEmptyId) {

  NES_DEBUG("GlobalQueryPlanTest: creating an empty global query plan");
  GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
  NES_DEBUG("GlobalQueryPlanTest: Adding a query plan without id to the global "
            "query plan");
...
/**
 * @brief This test is for creation of a global query plan and extracting a list
 * of global query nodes with specific type of operators
 */
TEST_F(
    GlobalQueryPlanTest,
    testNewGlobalQueryPlanAndGetAllNewGlobalQueryNodesWithATypeOfLogicalOperator) {
  ...
  // Assert
  NES_DEBUG("GlobalQueryPlanTest: A global query node containing operator of "
            "type SinkLogicalOperator should be returned");
  auto listOfGQMsToDeploy = globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_TRUE(listOfGQMsToDeploy.size() == 1);
}

/**
 * @brief This test is for creation of a global query plan and adding query plan
 * twice
 */
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanByAddingSameQueryPlanTwice) {
  ...
  NES_DEBUG("GlobalQueryPlanTest: creating an empty global query plan");
  GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
  NES_DEBUG("GlobalQueryPlanTest: Adding same query plan twice to the global "
            "query plan");
  auto query =
...
/**
 * @brief This test is for creation of a global query plan and adding multiple
 * query plans
 */
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndAddMultipleQueries) {
  ...
  // Assert
  NES_DEBUG("GlobalQueryPlanTest: should return 1 global query node with "
            "logical sink operator");
  auto listOfGQMsToDeploy = globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_TRUE(listOfGQMsToDeploy.size() == 1);

  NES_DEBUG("GlobalQueryPlanTest: Adding another query plan to the global "
            "query plan");
  ...
  // Assert
  NES_DEBUG("GlobalQueryPlanTest: should return 2 global query node with "
            "logical sink");
...
/**
 * @brief This test is for creation of a global query plan and adding and
 * removing the query plan
 */
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndAddAndRemoveQuery) {
  ...
  // Assert
  NES_DEBUG(
      "GlobalQueryPlanTest: should 1 global query node with logical sink");
  auto listOfSQPsToDeploy = globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_TRUE(listOfSQPsToDeploy.size() == 1);

  NES_DEBUG(
      "GlobalQueryPlanTest: Removing the query plan for the query with id Q1");
...
/**
 * @brief This test is for creation of a global query plan by adding multiple
 * query plans and computing the Meta Data information
 */
TEST_F(GlobalQueryPlanTest,
       testUpdateMetaDataInformationForGlobalQueryPlanWithMultipleQueries) {
  ...
  auto queryIdToSinkGQNs1 =
      sharedQueryPlansToDeploy[0]->getQueryIdToSinkOperatorMap();
  EXPECT_EQ(queryIdToSinkGQNs1.size(), 1u);
  EXPECT_TRUE(queryIdToSinkGQNs1.find(queryId1) != queryIdToSinkGQNs1.end());
  auto queryIdToSinkGQNs2 =
      sharedQueryPlansToDeploy[1]->getQueryIdToSinkOperatorMap();
  EXPECT_EQ(queryIdToSinkGQNs2.size(), 1u);
  EXPECT_TRUE(queryIdToSinkGQNs2.find(queryId2) != queryIdToSinkGQNs1.end());
}

/**
 * @brief This test is for updating Meta Data information after applying syntax
 * based query merging
 */
TEST_F(
    GlobalQueryPlanTest,
    testUpdateMetaDataInformationForGlobalQueryPlanWithMultipleQueriesAfterSyntaxBAsedQuaeryMerge) {
  ...
  auto syntaxBasedCompleteQueryMergerRule =
  ...
  auto queryIdToSinkGQNsMap =
      sharedQueryPlansToDeploy[0]->getQueryIdToSinkOperatorMap();
  EXPECT_TRUE(queryIdToSinkGQNsMap.find(queryId1) !=
              queryIdToSinkGQNsMap.end());
  EXPECT_TRUE(queryIdToSinkGQNsMap.find(queryId2) !=
              queryIdToSinkGQNsMap.end());
}

/**
 * @brief This test is for updating Meta Data information after applying Syntax
 * Based Equal Query Merger merging on 3 queryIdAndCatalogEntryMapping where 2
 * are identical
 */
TEST_F(
    GlobalQueryPlanTest,
    testUpdateMetaDataInformationForGlobalQueryPlanWithMixedQueriesAfterApplyingMergeRules) {
  ...
  auto lessExpression = Attribute("field_1") <= 10;
  auto query3 =
      Query::from("car").filter(lessExpression).sink(printSinkDescriptor);
  ...
  // Assert
  EXPECT_TRUE(sharedQueryPlansToDeploy.size() == 2u);
  auto queryIdToSinkOperatorMap1 =
      sharedQueryPlansToDeploy[0]->getQueryIdToSinkOperatorMap();
  EXPECT_TRUE(queryIdToSinkOperatorMap1.find(queryId1) !=
              queryIdToSinkOperatorMap1.end());
  EXPECT_TRUE(queryIdToSinkOperatorMap1.find(queryId2) !=
              queryIdToSinkOperatorMap1.end());
  ...
  queryIdToSinkOperatorMap1 =
      sharedQueryPlansToDeploy[0]->getQueryIdToSinkOperatorMap();

  auto queryIdToSinkOperatorMap2 =
      sharedQueryPlansToDeploy[1]->getQueryIdToSinkOperatorMap();
  EXPECT_EQ(queryIdToSinkOperatorMap2.size(), 1u);
  EXPECT_TRUE(queryIdToSinkOperatorMap2.find(queryId3) !=
              queryIdToSinkOperatorMap1.end());
}
