...
TEST_F(GlobalQueryPlanTest,
  ...
  GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
  NES_DEBUG(
      "GlobalQueryPlanTest: Adding a query plan without id to the global query "
      "plan");
...
TEST_F(
  ...
  // Assert
  NES_DEBUG(
      "GlobalQueryPlanTest: A global query node containing operator of type "
      "SinkLogicalOperator should be returned");
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanByAddingSameQueryPlanTwice) {
  ...
  GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
  NES_DEBUG(
      "GlobalQueryPlanTest: Adding same query plan twice to the global query "
      "plan");
  auto query =
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndAddMultipleQueries) {
  ...
  // Assert
  NES_DEBUG(
      "GlobalQueryPlanTest: should return 1 global query node with logical "
      "sink operator");
  auto listOfGQMsToDeploy = globalQueryPlan->getSharedQueryPlansToDeploy();
  EXPECT_TRUE(listOfGQMsToDeploy.size() == 1);

  NES_DEBUG(
      "GlobalQueryPlanTest: Adding another query plan to the global query "
      "plan");
  ...
  // Assert
  NES_DEBUG(
      "GlobalQueryPlanTest: should return 2 global query node with logical "
      "sink");
