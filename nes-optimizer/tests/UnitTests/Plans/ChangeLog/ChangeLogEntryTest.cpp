...
class ChangeLogEntryTest : public Testing::BaseUnitTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ChangeLogEntryTest.log",
...
TEST_F(ChangeLogEntryTest, FetchPoSetOfChangeLogEntry) {
  ...
  auto changelogEntry = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});

  EXPECT_EQ(changelogEntry->poSetOfSubQueryPlan.size(), 3);
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(sourceOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(filterOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(sinkOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
}
 ...
 *
 */
TEST_F(ChangeLogEntryTest,
       FetchPoSetOfChangeLogEntryWithMultipleDownStreamOptrs) {
  // Compute Plan
  auto queryPlan = QueryPlan::create(sourceOp1);
  queryPlan->appendOperatorAsNewRoot(filterOp1);
  ...
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(filterOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(filterOp2->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(sinkOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(sinkOp2->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
}
 ...
 */
TEST_F(ChangeLogEntryTest,
       FetchPoSetOfChangeLogEntryWithMultipleUpStreamOptrs) {
  // Compute Plan
  auto queryPlan = QueryPlan::create(sourceOp1);
  queryPlan->appendOperatorAsNewRoot(filterOp1);
  queryPlan->appendOperatorAsNewRoot(sinkOp1);
  ...
  // Initialize change log
  auto changelogEntry = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1, sourceOp2}, {sinkOp1});

  EXPECT_EQ(changelogEntry->poSetOfSubQueryPlan.size(), 5);
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(sourceOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(sourceOp2->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(filterOp1->getId()) !=
              changelogEntry->poSetOfSubQueryPlan.end());
  EXPECT_TRUE(changelogEntry->poSetOfSubQueryPlan.find(filterOp2->getId()) !=
