...
TEST_F(ChangeLogTest, InsertAndFetchChangeLogEntry) {
  ...
  auto extractedChangeLogEntries =
      changeLog->getCompactChangeLogEntriesBeforeTimestamp(1);
  EXPECT_EQ(extractedChangeLogEntries.size(), 0);

  // Fetch change log entries before timestamp 2
  extractedChangeLogEntries =
      changeLog->getCompactChangeLogEntriesBeforeTimestamp(2);
...
TEST_F(ChangeLogTest, InsertAndFetchMultipleChangeLogEntries) {
  ...
  // Initialize change log
  auto changeLog = NES::Optimizer::Experimental::ChangeLog::create();
  auto changelogEntry1 = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});
  auto changelogEntry2 = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});
  auto changelogEntry3 = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});
  auto changelogEntry4 = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});
  auto changelogEntry5 = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});
  auto changelogEntry6 = NES::Optimizer::Experimental::ChangeLogEntry::create(
      {sourceOp1}, {sinkOp1});
  ...
  auto extractedChangeLogEntries =
      changeLog->getCompactChangeLogEntriesBeforeTimestamp(4);
  EXPECT_EQ(1, extractedChangeLogEntries.size());

  const auto actualUpstreamOperators =
      extractedChangeLogEntries[0].second->upstreamOperators;
  EXPECT_EQ(1, actualUpstreamOperators.size());
  EXPECT_EQ(sourceOp1, (*actualUpstreamOperators.begin()));

  const auto actualDownstreamOperators =
      extractedChangeLogEntries[0].second->downstreamOperators;
...
TEST_F(ChangeLogTest, UpdateChangeLogProcessingTime) {
  ...
  extractedChangeLogEntries =
      changeLog->getCompactChangeLogEntriesBeforeTimestamp(7);
  // Will return one change log entry as it will merge change logs between 4
  // till 6
  EXPECT_EQ(1, extractedChangeLogEntries.size());

  const auto actualUpstreamOperatorsNext =
      extractedChangeLogEntries[0].second->upstreamOperators;
  EXPECT_EQ(1, actualUpstreamOperatorsNext.size());
  EXPECT_EQ(sourceOp1, (*actualUpstreamOperatorsNext.begin()));

  const auto actualDownstreamOperatorsNext =
      extractedChangeLogEntries[0].second->downstreamOperators;
...
TEST_F(ChangeLogTest, PerformLogCompactionForPartiallyOverlappingChangeLogs) {
  ...
  // Fetch change log entries before timestamp 4
  auto extractedChangeLogEntries =
      changeLog->getCompactChangeLogEntriesBeforeTimestamp(3);
  // Will return one change log entry as it will merge change logs between 1
  // till 2
  EXPECT_EQ(1, extractedChangeLogEntries.size());
 ...
 *
 */
TEST_F(ChangeLogTest,
       PerformLogCompactionForPartiallyNonOverlappingChangeLogs) {
  ...
  const auto actualUpstreamOperatorsNext =
      extractedChangeLogEntries[1].second->upstreamOperators;
  EXPECT_EQ(1, actualUpstreamOperatorsNext.size());
  EXPECT_EQ(filterOp3, (*actualUpstreamOperatorsNext.begin()));

  const auto actualDownstreamOperatorsNext =
      extractedChangeLogEntries[1].second->downstreamOperators;
  EXPECT_EQ(1, actualDownstreamOperatorsNext.size());
  EXPECT_EQ(sinkOp1, (*actualDownstreamOperatorsNext.begin()));
}
} // namespace NES
