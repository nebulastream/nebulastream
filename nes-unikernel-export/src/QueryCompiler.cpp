...
static std::map<NES::PipelineId, SinkStage> getSinks(std::vector<NES::QueryCompilation::OperatorPipelinePtr> pipelines) {
...
static std::map<NES::PipelineId, NES::Network::NetworkSourceDescriptorPtr>
getSources(std::vector<NES::QueryCompilation::OperatorPipelinePtr> pipelines) {
    ...
    std::map<NES::PipelineId, NES::Network::NetworkSourceDescriptorPtr> sourceMap;
    std::ranges::for_each(pipelines | filterSource, [&sourceMap](NES::QueryCompilation::OperatorPipelinePtr pipeline) {
        auto sources =
        ...
        auto descriptor = source->getSourceDescriptor()->as<NES::Network::NetworkSourceDescriptor>();
        sourceMap[pipeline->getPipelineId()] = descriptor;
    });
...
getStages(std::vector<NES::QueryCompilation::OperatorPipelinePtr> pipelines,
          ...
          size_t bufferSize) {
    auto lowerToNautilus = std::make_shared<NES::QueryCompilation::LowerPhysicalToNautilusOperators>(queryCompilerOptions);
    ...
    std::ranges::transform(
        ...
        [&compilationPhase, &lowerToNautilus, bufferSize, &uniqueOperatorHandlers](
            ...
            auto nautilusStage =
                static_unique_pointer_cast<NES::Runtime::Execution::CompiledExecutablePipelineStage>(std::move(stage));
...
ExportQueryCompiler::compile(NES::QueryPlanPtr queryPlan, size_t bufferSize) {
    ...
    return {getSinks(pipelines), getSources(pipelines), std::move(stages), sharedHandler};
}
