...
NES::SourceDescriptorPtr NES::NoOpSourceDescriptor::copy() {
    return std::make_shared<NoOpSourceDescriptor>(schema, schemaType, getLogicalSourceName(), tcp);
}
...
NES::SourceDescriptorPtr NES::NoOpSourceDescriptor::create(NES::SchemaPtr schemaPtr,
                                                           ...
                                                           std::string logicalSourceName,
                                                           std::optional<TCPSourceConfiguration> tcp) {
    return std::make_shared<NoOpSourceDescriptor>(std::move(schemaPtr), schemaType, std::move(logicalSourceName), std::move(tcp));
}
