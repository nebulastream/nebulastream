...
- On-Demand Gathering - Applies processing as early as possible along the data path.
...
# Terminal 2: prepare and register a demo query
cat > demo.sql <<EOF
CREATE LOGICAL SOURCE demo(value UINT64);
CREATE PHYSICAL SOURCE FOR demo TYPE File;
ATTACH INLINE
1
2
3

CREATE SINK result(value UINT64) TYPE File SET('./demo-output.csv' as `SINK`.FILE_PATH, 'CSV' as `SINK`.INPUT_FORMAT);
SELECT value FROM demo INTO result;
EOF

cmake-build-debug/nes-nebuli/nes-nebuli register -s localhost:8080 -i demo.sql -x -w
```
