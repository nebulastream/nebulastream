    ...
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        mlirBinary = pkgs.stdenv.mkDerivation rec {
          ...
          src = if system == "aarch64-linux" then
            pkgs.fetchurl {
        ...
        buildTools = with pkgs; [
          cmake ninja pkg-config git ccache
        ];
        ...
        # LLVM 19 toolchain
        llvmTools = [
          llvm.llvm clangWithVersions llvm.lld llvm.libcxx
          llvm.clang-tools # clang-format, clang-tidy
        ];
        
        # Runtime dependencies for MLIR
        mlirDeps = with pkgs; [
          libffi libxml2 openjdk21
        ];
        
        # Development tools
        devTools = with pkgs; [
          gdb llvm.lldb python3
        ];
      in {
