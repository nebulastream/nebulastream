...
TEST_F(SourceCatalogTest, AddRemovePhysicalSources)
{
    ...
    const auto physical1Opt
        ...
        *sourceOpt, Identifier::parse("File"), Host("localhost"), {{Identifier::parse("file_path"), "/dev/null"}}, {{Identifier::parse("type"), "CSV"}});
...
TEST_F(SourceCatalogTest, AddInvalidPhysicalSource)
{
    ...
    const auto sourceOpt = sourceCatalog.addLogicalSource(Identifier::parse("testSource"), schema);
    ASSERT_TRUE(sourceOpt.has_value());
    const auto physical1Opt = sourceCatalog.addPhysicalSource(*sourceOpt, Identifier::parse("THIS_DOES_NOT_EXIST"), Host("localhost"), {}, {});
...
TEST_F(SourceCatalogTest, RemoveLogicalSource)
{
    ...
    const auto physical1Opt
        = sourceCatalog.addPhysicalSource(
        logicalSource, Identifier::parse("File"), Host("localhost"), {{Identifier::parse("file_path"), "/dev/null"}}, {{Identifier::parse("type"), "CSV"}});
    const auto physical2Opt
        = sourceCatalog.addPhysicalSource(
        logicalSource, Identifier::parse("File"), Host("localhost"), {{Identifier::parse("file_path"), "/dev/null"}}, {{Identifier::parse("type"), "CSV"}});
