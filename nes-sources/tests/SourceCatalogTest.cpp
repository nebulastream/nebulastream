...
TEST_F(SourceCatalogTest, AddRemovePhysicalSources)
{
    ...
    const auto physical1Opt
        = sourceCatalog.addPhysicalSource(*sourceOpt, "File", WorkerId("localhost"), {{"file_path", "/dev/null"}}, {{"type", "CSV"}});
...
TEST_F(SourceCatalogTest, AddInvalidPhysicalSource)
{
    ...
    const auto sourceOpt = sourceCatalog.addLogicalSource("testSource", schema);
    ASSERT_TRUE(sourceOpt.has_value());
    const auto physical1Opt = sourceCatalog.addPhysicalSource(*sourceOpt, "THIS_DOES_NOT_EXIST", WorkerId("localhost"), {}, {});
...
TEST_F(SourceCatalogTest, RemoveLogicalSource)
{
    ...
    const auto physical1Opt
        = sourceCatalog.addPhysicalSource(logicalSource, "File", WorkerId("localhost"), {{"file_path", "/dev/null"}}, {{"type", "CSV"}});
    const auto physical2Opt
        = sourceCatalog.addPhysicalSource(logicalSource, "File", WorkerId("localhost"), {{"file_path", "/dev/null"}}, {{"type", "CSV"}});
        ...
        for (int num : nums)
        {
            ...
            if (logicalSourceOpt.has_value())
            {
                auto physicalSourceOpt = sourceCatalog.addPhysicalSource(
                    *logicalSourceOpt, "File", WorkerId("localhost"), {{"file_path", "/dev/null"}}, {{"type", "CSV"}});
