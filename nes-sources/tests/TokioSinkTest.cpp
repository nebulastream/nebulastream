...
#include <nes-io-runtime-bindings/lib.h> // init_io_runtime
#include <nes-sink-bindings/lib.h>      // SinkHandle, SendResult
...
public:
    ...
    /// Create a TokioFileSink config pointing to a temp file.
    static DescriptorConfig::Config makeTestSinkConfig()
    {
        auto path = std::filesystem::temp_directory_path() / ("nes_sink_test_" + std::to_string(::getpid()) + "_" + std::to_string(testFileCounter++));
    ...
    static inline std::atomic<uint64_t> testFileCounter{0};
};
...
TEST_F(TokioSinkTest, BasicLifecycle)
{
    ...
    auto sink = std::make_unique<TokioSink>(std::move(bpc), makeNamedSinkSpawnFn("TokioFileSink", makeTestSinkConfig()));
...
TEST_F(TokioSinkTest, BackpressureRetry)
{
    ...
    auto sink = std::make_unique<TokioSink>(
        std::move(bpc), makeNamedSinkSpawnFn("TokioFileSink", makeTestSinkConfig()),
        /*channelCapacity=*/1);
    MockPipelineContext pec(bufferManager);
    sink->start(pec);
