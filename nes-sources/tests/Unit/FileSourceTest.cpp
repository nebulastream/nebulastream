...
{
    std::string filePath;
    Schema schema = Schema{}.addField("id", DataType::Type::INT32);
    ParserConfig parserConfig = {.parserType = "CSV", .tupleDelimiter = "\n", .fieldDelimiter = ","};
...
TEST_F(FileSourceTest, FillBuffer)
{
    ...
    const auto sourceOpt = sourceCatalog.addLogicalSource("fileSource", config.schema);
    const auto fileSourceDesc = sourceCatalog.addPhysicalSource(*sourceOpt, "File", {{"filePath", config.filePath}}, config.parserConfig);
    auto fileSource = Sources::FileSource{fileSourceDesc.value()};
...
TEST_F(FileSourceTest, ReadSameFileTwice)
{
    ...
    const auto sourceOpt = sourceCatalog.addLogicalSource("fileSource", config.schema);
    const auto fileSourceDesc = sourceCatalog.addPhysicalSource(*sourceOpt, "File", {{"filePath", config.filePath}}, config.parserConfig);
    auto fileSource1 = Sources::FileSource{fileSourceDesc.value()};
    auto fileSource2 = Sources::FileSource{fileSourceDesc.value()};
