...
TEST_F(TCPSourceTest, FillBuffer)
{
    ...
    const ParserConfig parserConfig = {.parserType = "CSV", .tupleDelimiter = "\n", .fieldDelimiter = ","};

    SourceCatalog sourceCatalog{};
    const auto sourceOpt = sourceCatalog.addLogicalSource("fileSource", schema);
    const auto fileSourceDesc = sourceCatalog.addPhysicalSource(*sourceOpt, "TCP", {{"socketHost", "localHost"}, {"socketPort", std::to_string(DEFAULT_PORT)}},parserConfig);
    auto tcpSource = Sources::TCPSource{fileSourceDesc.value()};
