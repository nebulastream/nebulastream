...
private:
    ...
    /// PROFILING-only (env NES_INMEM_REPEAT=N, default 1): re-emit the prefilled buffers N passes back-to-back
    /// (as refcount copies, originals retained) so workers stay continuously busy for a sustained, reload-free
    /// window. Safe at 128 KiB buffers: a buffer is re-emitted ~#buffers later, far beyond the in-flight cap,
    /// so per-emit metadata stamping never overlaps an in-flight use. One malformed stitched row per wrap.
    size_t numPasses = 1;
    size_t passesDone = 0;
};
