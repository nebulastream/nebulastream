...
{
    std::stop_source stopSource; // owns the stop state
public:
    AsyncSourceRunner() = delete;
    explicit AsyncSourceRunner(std::unique_ptr<AsyncSource> asyncSource, SourceReturnType::EmitFunction&& emitFn);
    ...
    [[nodiscard]] asio::awaitable<void, Executor>
    runningRoutine(OriginId sourceId, std::shared_ptr<AbstractBufferProvider> bufferProvider) const;

private:
    // SourceReturnType::SourceReturnType context;
    std::unique_ptr<AsyncSource> source;
    SourceReturnType::EmitFunction emitFn;

    void handleSourceResult(
        TupleBuffer& buffer,
