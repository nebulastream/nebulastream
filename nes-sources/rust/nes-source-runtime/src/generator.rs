...
use crate::plugin::{SourcePluginEntry, SOURCE_PLUGINS};
...
fn spawn_generator(
    ...
    source_id: u64,
    buffer_provider: crate::buffer::BufferProviderHandle,
    inflight_limit: u32,
    emit_fn: crate::bridge::EmitFnPtr,
    emit_ctx: *mut std::ffi::c_void,
    error_fn: crate::source::ErrorFnPtr,
    error_ctx: *mut std::ffi::c_void,
) -> Result<Box<crate::handle::SourceTaskHandle>, String> {
    let source = create_generator_source(config)?;
    Ok(crate::source::spawn_source(
...
#[cfg(not(test))]
#[used]
#[linkme::distributed_slice(SOURCE_PLUGINS)]
static GENERATOR_PLUGIN: SourcePluginEntry = SourcePluginEntry {
    name: "TokioGenerator",
    schema: CONFIG_SCHEMA,
    spawn: spawn_generator,
};
