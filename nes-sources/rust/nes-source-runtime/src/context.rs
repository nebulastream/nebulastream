...
#[cfg(not(test))]
use crate::bridge::{BridgeMessage, EmitCallback};
...
pub struct SourceContext {
    ...
    origin_id: u64,
    /// Per-source C++ emit callback, carried in every BridgeMessage.
    #[cfg(not(test))]
...
// - u64 is Send
// - EmitCallback is Send (unsafe impl, validated: emit_ctx outlives source)
...
// - EmitCallback is Sync (function pointer + immutable context pointer)
...
impl SourceContext {
    ...
    /// Returns the origin ID for this source context.
    ...
    /// 3. **Channel send:** Sends `BridgeMessage { callback, origin_id, buffer, permit }`
    ...
    pub(crate) async fn emit_eos(&self) {
        ...
        // Cleanup is handled by the monitoring task directly.
    }

    /// Returns a reference to the cancellation token for cooperative shutdown.
