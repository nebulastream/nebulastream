...
asio::awaitable<AsyncSource::InternalSourceResult, Executor> TCPSource::fillBuffer(TupleBuffer& buffer)
{
    ...
    auto [errorCode, bytesRead] = co_await async_read(
        socket.value(),
        asio::mutable_buffer(buffer.getAvailableMemoryArea().data(), buffer.getBufferSize()),
...
SourceValidationRegistryReturnType RegisterTCPSourceValidation(SourceValidationRegistryArguments arguments)
{
