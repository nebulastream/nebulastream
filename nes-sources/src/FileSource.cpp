...
DescriptorConfig::Config FileSource::validateAndFormat(std::unordered_map<UppercaseString, std::string> config)
{
...
{
    if (systestAdaptorArguments.physicalSourceConfig.sourceConfig.contains(UppercaseString(std::string(SYSTEST_FILE_PATH_PARAMETER))))
    {
    ...
    systestAdaptorArguments.physicalSourceConfig.sourceConfig.try_emplace(
        UppercaseString(std::string(SYSTEST_FILE_PATH_PARAMETER)), systestAdaptorArguments.testFilePath.string());
