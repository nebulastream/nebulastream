...
DataSource::DataSource(
    ...
    : originId(originId)
    , schema(schema)
    , localBufferManager(std::move(bufferManager)) //Todo: move buffer manager out of runtime, use AbstractBufferProvider
...
bool DataSource::stop()
{
    ...
    catch (...)
    {
        ...
        catch (std::exception const& e) /// e needs to be passed by reference
        {
            ...
            if (!wasStarted)
            {
                emitFunction(originId, SourceReturnType::TerminationType::FAILURE); //Todo: add better failure type/message
            }
            isStopped = true;
        }
    }
...
void DataSource::runningRoutine()
{
    try
    {
        ...
        /// open
        // Todo: replace with interface from AbstractBufferProvider
