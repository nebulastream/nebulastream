...
BlockingTCPSource::BlockingTCPSource(const SourceDescriptor& sourceDescriptor)
    : socketHost(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::HOST))
    , socketPort(std::to_string(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::PORT)))
    , socketType(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::TYPE))
    , socketDomain(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::DOMAIN))
    , tupleDelimiter(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::SEPARATOR))
    , socketBufferSize(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::SOCKET_BUFFER_SIZE))
    , bytesUsedForSocketBufferSizeTransfer(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::SOCKET_BUFFER_TRANSFER_SIZE))
    , flushIntervalInMs(sourceDescriptor.getFromConfig(ConfigParametersBlockingTCP::FLUSH_INTERVAL_MS))
{
    /// init physical types
...
void BlockingTCPSource::open()
{
    ...
    NES_DEBUG("BlockingTCPSource::open: Connected to server.");
}

size_t BlockingTCPSource::fillBuffer(IOBuffer& buffer, const std::stop_token&)
{
    try
    {
        size_t numReceivedBytes = 0;
        while (fillBuffer(buffer, numReceivedBytes))
        {
    ...
    {
        NES_ERROR("BlockingTCPSource::receiveData: Failed to fill the TupleBuffer. Error: {}.", e.what());
        throw e;
    }
}
...
{
    NES_TRACE("BlockingTCPSource::close: trying to close connection.");
    if (connection >= 0)
    {
        ::close(sockfd);
        NES_DEBUG("BlockingTCPSource::close: connection closed.");
    }
}

SourceValidationRegistryReturnType
SourceValidationGeneratedRegistrar::RegisterBlockingTCPSourceValidation(SourceValidationRegistryArguments sourceConfig)
{
    return BlockingTCPSource::validateAndFormat(sourceConfig.config);
}
...
InlineDataRegistryReturnType InlineDataGeneratedRegistrar::RegisterBlockingTCPInlineData(InlineDataRegistryArguments systestAdaptorArguments)
{
    ...
    {
        if (const auto port = systestAdaptorArguments.physicalSourceConfig.sourceConfig.find(ConfigParametersBlockingTCP::PORT);
            port != systestAdaptorArguments.physicalSourceConfig.sourceConfig.end())
        {
            ...
            if (const auto host = systestAdaptorArguments.physicalSourceConfig.sourceConfig.find(ConfigParametersBlockingTCP::HOST);
