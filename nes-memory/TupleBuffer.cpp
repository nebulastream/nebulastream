...
uint32_t TupleBuffer::getReferenceCounter() const noexcept { return controlBlock ? controlBlock->getReferenceCount() : 0; }
...
void TupleBuffer::setSequenceNumber(uint64_t sequenceNumber) noexcept { controlBlock->setSequenceNumber(sequenceNumber); }
...
SequenceData TupleBuffer::getSequenceData() const noexcept { return {getSequenceNumber(), getChunkNumber(), isLastChunk()}; }
void TupleBuffer::setChunkNumber(uint64_t chunkNumber) noexcept { controlBlock->setChunkNumber(chunkNumber); }
void TupleBuffer::setLastChunk(bool isLastChunk) noexcept { controlBlock->setLastChunk(isLastChunk); }
void TupleBuffer::setCreationTimestampInMS(uint64_t value) noexcept { controlBlock->setCreationTimestamp(value); }
void TupleBuffer::setOriginId(OriginId id) noexcept { controlBlock->setOriginId(id); }
void TupleBuffer::setStatisticId(StatisticId statisticId) noexcept { controlBlock->setStatisticId(statisticId); }
...
TupleBuffer::TupleBuffer(TupleBuffer const& other) noexcept
...
auto TupleBuffer::operator!() const noexcept -> bool { return ptr == nullptr; }
...
uint64_t TupleBuffer::getCreationTimestampInMS() const noexcept { return controlBlock->getCreationTimestamp(); }
uint64_t TupleBuffer::getSequenceNumber() const noexcept { return controlBlock->getSequenceNumber(); }
uint64_t TupleBuffer::getChunkNumber() const noexcept { return controlBlock->getChunkNumber(); }
bool TupleBuffer::isLastChunk() const noexcept { return controlBlock->isLastChunk(); }
OriginId TupleBuffer::getOriginId() const noexcept { return controlBlock->getOriginId(); }
StatisticId TupleBuffer::getStatisticId() const noexcept { return controlBlock->getStatisticId(); }
uint32_t TupleBuffer::getNumberOfChildrenBuffer() const noexcept { return controlBlock->getNumberOfChildrenBuffer(); }
