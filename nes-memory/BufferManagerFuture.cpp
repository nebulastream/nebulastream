...
std::variant<TupleBuffer, uint32_t> RepinBufferFuture::await_resume() const noexcept
{
    INVARIANT(promise->getResult(), "Resumed coroutine but no result was set in promise");
    return *promise->getResult();
}

RepinBufferFuture RepinBufferFuture::fromPinnedBuffer(TupleBuffer buffer) noexcept
{
    co_return buffer;
}
}