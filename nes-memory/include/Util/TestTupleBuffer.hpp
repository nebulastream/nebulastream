...
public:
    ...
    [[nodiscard]] PhysicalType& getPhysicalType() const;

    [[nodiscard]] const uint8_t* getAddressPointer() const;

private:
    const uint8_t* address;
    const std::unique_ptr<PhysicalType> physicalType;
};
...
public:
    ...
    std::string toString(const Schema& schema, bool showHeader = true);
