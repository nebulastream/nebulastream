...
template <class Type>
concept IsNesType
    = std::is_fundamental_v<Type> || std::is_fundamental_v<std::remove_pointer_t<Type>> || std::is_same_v<Type, VariableSizedAccess>;
...
public:
    ...
    [[nodiscard]] Type read() const
    {
        ...
        if (not physicalType.isSameDataType<Type>()
            and not(physicalType.isType(DataType::Type::VARSIZED) and std::is_same_v<std::remove_cvref_t<Type>, VariableSizedAccess>))
        {
...
private:
    ...
    void copyRecordFromBufferToTuple(std::tuple<Types...>& record, uint64_t recordIndex)
    {
        ...
        if constexpr (I != sizeof...(Types))
        {
            ...
            {
                auto data = (*this)[recordIndex][I].read<VariableSizedAccess>().access(this->getBuffer());
                std::get<I>(record) = std::string{reinterpret_cast<const char*>(data.data()), data.size()};
            }
            else
            {
