...
class RepinBufferPromise : public detail::VirtualPromise
{
    ...
    std::variant<TupleBuffer, detail::CoroutineError> result{};
...
public:
    ...
    void return_value(const std::variant<TupleBuffer, detail::CoroutineError>& result)
    {
        this->result = result;
    ...
    std::optional<std::variant<TupleBuffer, detail::CoroutineError>> getResult() const noexcept
    {
        if (isDone.test())
        {
