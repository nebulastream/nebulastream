...
public:
    ...
    std::optional<std::variant<TupleBuffer, uint32_t>> waitUntilDone(std::chrono::milliseconds timeout) const;
    ...
    std::variant<TupleBuffer, uint32_t> await_resume() const noexcept;


    static RepinBufferFuture fromPinnedBuffer(TupleBuffer) noexcept;
