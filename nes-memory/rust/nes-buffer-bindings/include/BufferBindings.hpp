...
/// Asynchronously obtain a pooled buffer. Drives the provider's getBufferAsync() coroutine on a
/// detached task; when a buffer becomes available the segment is handed to Rust via `done`. If the
/// Rust side has been dropped in the meantime, `done` releases the buffer back to the pool.
void allocate_async(
