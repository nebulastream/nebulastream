...
void allocate_async(
    const std::shared_ptr<BufferProviderHandle>& handle,
    rust::Fn<void(rust::Box<OnBufferAllocated>, NES::detail::MemorySegment*)> done,
    rust::Box<OnBufferAllocated> ctx)
{
    [](std::shared_ptr<BufferProviderHandle> handle,
       rust::Fn<void(rust::Box<OnBufferAllocated>, NES::detail::MemorySegment*)> done,
       rust::Box<OnBufferAllocated> ctx) -> NES::DetachedTask
    {
        NES::TupleBuffer buffer = co_await handle->getBufferAsync();
        /// Same retain/release dance as try_allocate: bump the ref count so the buffer survives the
        /// handoff to Rust, which takes ownership via from_raw() and balances the extra reference.
        buffer.retain();
        done(std::move(ctx), NES::toRust(buffer));
    }(handle, std::move(done), std::move(ctx));
}
