...
std::shared_ptr<ColumnLayout> ColumnLayout::create(uint64_t bufferSize, const Schema& schema)
{
    auto* layout = new ColumnLayout(bufferSize, std::move(schema));
    return std::shared_ptr<ColumnLayout>(layout);
}
