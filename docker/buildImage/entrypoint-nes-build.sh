...
if [ $# -eq 0 ]; then
  ...
  ccache -M 10G
  ccache -s
  cmake --fresh -B /build_dir -DCMAKE_BUILD_TYPE=Release -DBoost_NO_SYSTEM_PATHS=TRUE -DNES_SELF_HOSTING=1 -DNES_USE_CCACHE=1 -DNES_USE_OPC=0 -DNES_USE_MQTT=1 -DNES_ENABLE_EXPERIMENTAL_EXECUTION_JNI=1 -DNES_JAVA_UDF_UTILS_PATH=lib/NebulaStream/Java -DNES_TEST_PARALLELISM=$NesTestParallelism -DNES_USE_TF=1 -DNES_USE_S2=1 /nebulastream/
