...
if [ $# -eq 1 ]; then
  ...
  ccache -M 10G
  ccache -s
  cmake --fresh -B /build_dir -DCMAKE_BUILD_TYPE=Release -DNES_SELF_HOSTING=1 -DNES_USE_CCACHE=1 -DNES_TEST_PARALLELISM=$NesTestParallelism  -DNES_USE_OPENCL=1 ${EXTRA_CMAKE_FLAG} /nebulastream/
