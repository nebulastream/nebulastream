...
# -- Install oh-my-pi coding agent (prebuilt binary) -------------------------
ARG OMP_VERSION=17.3.4
RUN curl -fsSL https://omp.sh/install | PI_INSTALL_DIR=/usr/local/bin sh -s -- --binary --ref v${OMP_VERSION} && \
    omp --version
...
# Check out the clone at the manifest's builtin-baseline so the helper
# scripts, the bootstrapped vcpkg tool, and the port recipes all come from
# the same era.  An unpinned HEAD drifts ahead of the baseline and can
# require newer system tools than the base image ships (e.g. scripts using
# CMake features the installed cmake does not have) and invalidates every
# binary-cache entry on each image rebuild.
RUN cd /vcpkg_input \
    && git clone https://github.com/microsoft/vcpkg.git vcpkg_repository \
    && git -C vcpkg_repository checkout "$(jq -r '."builtin-baseline"' vcpkg.json)" \
