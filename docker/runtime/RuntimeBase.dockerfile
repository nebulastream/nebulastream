...
# Contains only the runtime dependencies: libc++, unixODBC, grpc_health_probe, and
# basic utilities.
...
# The ODBC source plugin is statically linked into nes-sources, so every NebulaStream
# binary hard-links the unixODBC driver manager; without libodbc here the binary fails
# to load (exit 127) the instant a runtime container starts it as its entrypoint.
# msodbcsql18 is the driver the study's SQL Server instance is reached through — it
# registers in /etc/odbcinst.ini, which is why ODBCSource points ODBCSYSINI at /etc.
