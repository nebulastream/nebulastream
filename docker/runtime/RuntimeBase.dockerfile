...
# Contains only the runtime dependencies: libc++, unixODBC, grpc_health_probe,
# and basic utilities.
...
RUN apt update -y && apt install curl wget gpg -y \
    ...
    # The ODBC source plugin is statically linked into nes-sources, so every
    # NebulaStream binary (worker, single-node-worker, ...) now hard-links the
    # unixODBC driver manager. Without libodbc here the binary fails to load
    # (exit 127) the instant a runtime container starts it as its entrypoint.
    # odbc-postgresql ships the psqlodbc driver so a containerized worker can
    # actually serve ODBC sources, mirroring Development.dockerfile.
