 ...
 * It processes all watermark updates from one specific origin and applies all
 * updates in sequential order.
 * @assumptions This watermark processor assumes strictly monotonic update
 * sequence numbers. To handle out of order processing, it stores in flight
 * updates in a transaction log.
 */
class WatermarkProcessor {
 public:
  ...
  /**
   * @brief In this implementation, update watermark processes a watermark
   * barrier and applies all outstanding updates from the transaction log. To
   * this end, it leverage the implicit sorting of the priority queue.
   ...
   * that haven't arrived yet than current seen last tuple
   * @return Success
   */
 ...
 private:
  struct WatermarkBarrierComparator {
    bool operator()(std::tuple<WatermarkTs, SequenceNumber> const& wb1,
                    std::tuple<WatermarkTs, SequenceNumber> const& wb2) {
