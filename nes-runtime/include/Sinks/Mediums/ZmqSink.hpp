...
public:
  ...
  ZmqSink(SinkFormatPtr format, Runtime::NodeEnginePtr nodeEngine,
          uint32_t numOfProducers, const std::string &host, uint16_t port,
          bool internal, SharedQueryId sharedQueryId,
...
namespace fmt {
template <> struct formatter<NES::ZmqSink> : formatter<std::string> {
  auto format(const NES::ZmqSink &zmq_sink, format_context &ctx)
      -> decltype(ctx.out()) {
    return fmt::format_to(ctx.out(), "{} {}", zmq_sink.getHost(),
                          zmq_sink.getPort());
  }
};
} // namespace fmt
