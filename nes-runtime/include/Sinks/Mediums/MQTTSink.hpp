...
class MQTTSink : public SinkMedium {
 public:
   ...
   * @param asynchronousClient: determine whether client is async- or
   * synchronous
   * @param numberOfOrigins: number of origins of a given query
   * @return MQTT sink
   */
  // TODO change MSGS to Messages
  explicit MQTTSink(SinkFormatPtr sinkFormat, Runtime::NodeEnginePtr nodeEngine,
                    uint32_t numOfProducers, SharedQueryId sharedQueryId,
                    DecomposedQueryPlanId decomposedQueryPlanId,
                    std::string const& address, std::string const& clientId,
                    std::string const& topic, std::string const& user,
  ...
  /**
   * @brief get the number of MSGs that can maximally be buffered (default is
   * 60)
  ...
  /**
   * @brief get bool that indicates whether the client is asynchronous or
 ...
 private:
  [[maybe_unused]] DecomposedQueryPlanId decomposedQueryPlanId =
