...
/**
 * @brief The watermark processor receives watermark barriers and provides the
 * current watermark across multiple origins. The watermark processor guarantees
 * strict serializable watermark updates. Thus, a watermark processor is only
 * executed if all preceding updates have been processed. Consequently, the
 * watermark processor expects a exactly once delivery on the input channel.
 *
...
class MultiOriginWatermarkProcessor {
 public:
   ...
   * @param numberOfOrigins
   */
  static std::shared_ptr<MultiOriginWatermarkProcessor> create(
  ...
  /**
   * @brief Returns success if there are no tuples with smaller sequence number
   * that haven't arrived yet than the last tuple seen
 ...
 private:
  ...
  // The watermark processor maintains a local watermark processor for each
  // origin.
  std::map<uint64_t, std::unique_ptr<WatermarkProcessor>>
      localWatermarkProcessor;
};
using MultiOriginWatermarkProcessorPtr =
    std::unique_ptr<MultiOriginWatermarkProcessor>;
}  // namespace NES::Windowing
