...
class Iterator {
 ...
 public:
  ...
  explicit Iterator(uint64_t currentSeek, Runtime::TupleBuffer buffer,
                    const SchemaPtr& schema, FormatTypes sinkFormatType)
      : buffer(std::move(buffer)), sinkFormatType(sinkFormatType) {
    auto physicalDataTypeFactory = DefaultPhysicalTypeFactory();

    // Iterate over all fields of a tuple. Store sizes in fieldOffsets, to
    // calculate correct offsets in the step below. Also, store types of fields
    // in a separate array. Is later used to convert values to strings
    // correctly. Iteratively add up all the sizes in the offset array, to
    // correctly determine where each field starts in the TupleBuffer
  ...
  /**
   * @brief Accesses the TupleBuffer at the current bufferIndex and returns the
  ...
  friend bool operator==(const Iterator& a, const Iterator& b) {
    return a.currentSeek == b.currentSeek;
  };
  friend bool operator!=(const Iterator& a, const Iterator& b) {
    return a.currentSeek != b.currentSeek;
  };
 ...
 private:
   ...
   * From field with Size = {64, 32, 32, 128} corresponding fieldOffsets = {64,
   * 96, 128, 256} (actual field offsets)
   */
