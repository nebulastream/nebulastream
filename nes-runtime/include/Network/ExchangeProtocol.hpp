...
/**
 * @brief This class is used by the ZmqServer and defines the reaction for
 * events onDataBuffer, clientAnnouncement, endOfStream and exceptionHandling
 * between all nodes of NES.
 */
class ExchangeProtocol {
public:
  /**
   * @brief Create an exchange protocol object with a partition manager and a
   * listener
  ...
  onClientAnnouncement(Messages::ClientAnnounceMessage msg);
...
private:
  ...
  folly::Synchronized<std::unordered_map<
      NesPartition, Sequencing::NonBlockingMonotonicSeqQueue<uint64_t>>>
