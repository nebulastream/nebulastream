...
class NetworkEventSender : public BaseChannelType {
 public:
  ...
  bool sendEvent(Arguments&&... args) {
    ...
    auto* event =
        new (buffer.getBuffer()) Event(std::forward<Arguments>(args)...);
    buffer.setNumberOfTuples(sizeof(Event));
    return sendEvent(std::move(buffer), event->getEventType());
  }

  bool sendEvent(Runtime::TupleBuffer&& inputBuffer,
    ...
    sendMessage<Messages::EventBufferMessage, kZmqSendMore>(
        this->zmqSocket, eventType, payloadSize);
    // We need to retain the `inputBuffer` here, because the send function
    // operates asynchronously and we therefore need to pass the responsibility
    // of freeing the tupleBuffer instance to ZMQ's callback.
    inputBuffer.retain();
    auto const sentBytesOpt = this->zmqSocket.send(
        zmq::message_t(ptr, payloadSize,
                       &Runtime::detail::zmqBufferRecyclingCallback,
                       inputBuffer.getControlBlock()),
        kZmqSendDefault);
    if (sentBytesOpt.has_value()) {
      NES_TRACE("DataChannel: Sending buffer with {}/{}-{}",
                inputBuffer.getNumberOfTuples(), inputBuffer.getBufferSize(),
