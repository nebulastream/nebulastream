...
/**
 * @brief This class represent the network channel for data transfer that NES
 * uses to send data among nodes. This class can send data and event packets.
 * This class is not thread-safe.
 */
class NetworkChannel
    : public detail::NetworkEventSender<
          detail::NetworkDataSender<detail::BaseNetworkChannel>> {
  using inherited = detail::NetworkEventSender<
      detail::NetworkDataSender<detail::BaseNetworkChannel>>;
...
public:
   ...
   */
  explicit NetworkChannel(zmq::socket_t &&zmqSocket, ChannelId channelId,
                          std::string &&address,
   ...
   * @param bufferManager the buffer manager
   * @param highWaterMark the max number of buffers the channel takes before
   * blocking
   * @param waitTime the backoff time in case of failure when connecting
   * @param retryTimes the number of retries before the methods will raise
   * error. Set this to zero to retry indefinitely
   * @param version the version number that will be used by the receiver to
   * determine if it can already accept messages from this client
   * @param abortConnection a future which which will be checked on every
   * connection retry. By setting a value in the corresponding promise, the
   * calling thread can abort the connection process if the connection is
   * performed asynchronously.
   * @return the network channel or nullptr on error
   */
  static NetworkChannelPtr
  create(const std::shared_ptr<zmq::context_t> &zmqContext,
         std::string &&socketAddr, NesPartition nesPartition,
         ExchangeProtocol &protocol, Runtime::BufferManagerPtr bufferManager,
...
/**
 * @brief This class represent the network channel for event transfer that NES
 * uses to send events among nodes. This class can send only event packets. This
 * class is not thread-safe.
 */
class EventOnlyNetworkChannel
    : public detail::NetworkEventSender<detail::BaseNetworkChannel> {
  using inherited = detail::NetworkEventSender<detail::BaseNetworkChannel>;

public:
  /**
   * @brief Creates a network channel for events-only instance with the given
   * parameters
   * @param zmqContext the local zmq server context
   * @param channelId the remote nes channel id to connect to
   * @param address the socket address of the remote server
  ...
  explicit EventOnlyNetworkChannel(zmq::socket_t &&zmqSocket,
                                   ChannelId channelId, std::string &&address,
                                   Runtime::BufferManagerPtr bufferManager);
  ...
  /**
   * @brief Creates a networkf channel instance for event transmission with the
   * given parameters
   * @param zmqContext the local zmq server context
   * @param address the ip address of the remote server
   * @param nesPartition the remote nes partition to connect to
   ...
   * @return the network channel or nullptr on error
   */
  static EventOnlyNetworkChannelPtr
  create(const std::shared_ptr<zmq::context_t> &zmqContext,
         std::string &&socketAddr, NesPartition nesPartition,
         ExchangeProtocol &protocol, Runtime::BufferManagerPtr bufferManager,
         int highWaterMark, std::chrono::milliseconds waitTime,
