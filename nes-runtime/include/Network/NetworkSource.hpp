...
class NetworkSource : public DataSource {
 public:
   ...
   * @param uniqueNetworkSourceIdentifier system wide unique id that persists
   * even if the partition of this source is changed
   * @param physicalSourceName
   */
  NetworkSource(
      SchemaPtr schema, Runtime::BufferManagerPtr bufferManager,
      Runtime::QueryManagerPtr queryManager, NetworkManagerPtr networkManager,
      NesPartition nesPartition, NodeLocation sinkLocation,
      size_t numSourceLocalBuffers, std::chrono::milliseconds waitTime,
      uint8_t retryTimes,
  ...
  /**
   * @brief this method is just dummy and is replaced by the ZmqServer in the
   * NetworkStack. Do not use!
  ...
  /**
   * @brief This method is called once an event is triggered for the current
   * source.
   * @param event
   */
  ...
  /**
   * @brief This method is overridden here to prevent the NetworkSoure to start
   * a thread. It registers the source on the NetworkManager
   * @return true if registration on the network stack is successful
   */
  bool start() final;

  /**
   * @brief This method is overridden here to prevent the NetworkSource to start
   * a thread. It de-registers the source on the NetworkManager
   * @return true if deregistration on the network stack is successful
   */
  bool stop(Runtime::QueryTerminationType =
                Runtime::QueryTerminationType::Graceful) final;
  ...
  /**
   * @brief This method is invoked when the source receives a reconfiguration
   * message.
   * @param message the reconfiguration message
   * @param context thread context
   */
  void reconfigure(Runtime::ReconfigurationMessage& message,
                   Runtime::WorkerContext& context) override;
  ...
  /**
   * @brief API method called upon receiving an event, send event further
   * upstream via Network Channel.
   * @param event
   * @param workerContext
   */
  void onEvent(Runtime::BaseEvent& event,
  ...
  /**
   * @brief Reconfigures this source with ReconfigurationType::UpdateVersion
   * causing it to close event channels to the old upstream sink and open
   * channels to the new one
   * @return true if a scheduled new version was found and applied, false
   * otherwise
   */
  ...
  /**
   * @brief set a new source descriptor to be applied once startNewVersion() is
   * called
   * @param networkSourceDescriptor the new descriptor
   * @return true if the partition to be scheduled if different from the current
   * one and the descriptor was scheduled.
   */
  bool scheduleNewDescriptor(
      const NetworkSourceDescriptor& networkSourceDescriptor);
