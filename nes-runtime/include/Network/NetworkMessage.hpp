...
/**
 * @brief This magic number is written as first 64bits of every NES network
 * message. We use this as a checksum to validate that we are not transferring
 * garbage data.
 */
...
enum class MessageType : uint8_t {
  ...
  ClientAnnouncement,
  /// message type that the servers uses to reply to the client regarding the
  /// availability
  /// of a partition
  ServerReady,
  ...
  ErrorMessage,
  /// type of a message that marks a source subpartition as finished, i.e., no
...
enum class ErrorType : uint8_t {
  ...
  /// error raised when a data/event buffer arrives for a partition that is not
  /// known on the current node
  UnknownPartitionError,
  /// error raised when requesting a partition that has been previously deleted
  DeletedPartitionError,
  /// error raised when the version expected by the receiver does not match the
  /// one in the client anouncement
...
public:
  explicit MessageHeader(MessageType msgType, uint32_t msgLength)
      : magicNumber(NES_NETWORK_MAGIC_NUMBER), msgType(msgType),
        msgLength(msgLength) {}

  [[nodiscard]] nes_magic_number_t getMagicNumber() const {
    return magicNumber;
  }
...
public:
  explicit ExchangeMessage(ChannelId channelId)
...
/**
 * @brief This message is sent when a client announces itself to a server. It's
 * the first message that is sent.
 */
...
public:
  ...
  explicit ClientAnnounceMessage(ChannelId channelId, ChannelType mode,
                                 DecomposedQueryPlanVersion version = 0)
      : ExchangeMessage(channelId), mode(mode), version(version) {}

  [[nodiscard]] DecomposedQueryPlanVersion getVersion() const {
    return version;
  }
...
/**
 * @brief This message is sent back to a client when a server is ready to
 * receive data.
 */
...
public:
  ...
  explicit ServerReadyMessage(ChannelId channelId)
...
public:
  ...
  explicit EndOfStreamMessage(ChannelId channelId, ChannelType channelType,
                              Runtime::QueryTerminationType terminationType,
                              uint16_t numSendingThreads,
                              uint64_t maxMessageSequenceNumber)
      : ExchangeMessage(channelId), channelType(channelType),
        terminationType(terminationType), numSendingThreads(numSendingThreads),
        maxMessageSequenceNumber(maxMessageSequenceNumber) {}

  [[nodiscard]] Runtime::QueryTerminationType getQueryTerminationType() const {
    return terminationType;
  }

  [[nodiscard]] bool isDataChannel() const {
    return channelType == ChannelType::DataChannel;
  }

  [[nodiscard]] bool isEventChannel() const {
    return channelType == ChannelType::EventOnlyChannel;
  }

  [[nodiscard]] uint16_t getNumberOfSendingThreads() const {
    return numSendingThreads;
  }

  [[nodiscard]] uint64_t getMaxMessageSequenceNumber() const {
    return maxMessageSequenceNumber;
  }
...
/**
 * @brief This message represent an error that is sent from the client to the
...
public:
  ...
  explicit ErrorMessage(ChannelId channelId, ErrorType error)
      : ExchangeMessage(channelId), errorCode(error) {
    // nop
  }
   ...
   */
  [[nodiscard]] bool isPartitionNotFound() const {
    return errorCode == ErrorType::PartitionNotRegisteredError;
  }
   ...
   */
  [[nodiscard]] bool isPartitionDeleted() const {
    return errorCode == ErrorType::DeletedPartitionError;
  }
   ...
   */
  [[nodiscard]] bool isVersionMismatch() const {
    return errorCode == ErrorType::VersionMismatchError;
  }
...
public:
  ...
  explicit inline DataBufferMessage(uint32_t payloadSize, uint32_t numOfRecords,
                                    OriginId originId, uint64_t watermark,
      ...
      : payloadSize(payloadSize), numOfRecords(numOfRecords),
        originId(originId), watermark(watermark),
        creationTimestamp(creationTimestamp), sequenceData(sequenceData),
