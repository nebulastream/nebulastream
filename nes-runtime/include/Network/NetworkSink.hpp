...
/**
 * @brief This represent a sink operator that acts as a connecting API between
 * query processing and network stack.
 */
...
public:
  ...
  explicit NetworkSink(
      ...
      NodeLocation const &destination, NesPartition nesPartition,
...
public:
   ...
   * @param newReceiverLocation the location of the node where the new
   * downstream source is located
   * @param newVersion The new version number assigned to this sink
   */
  void configureNewSinkDescriptor(
      const NetworkSinkDescriptor &newNetworkSinkDescriptor);

  /**
   * @brief schedule a new receiver location and new receiver partition and
   * versio number to be set for this sink.
   * @param networkSinkDescriptor : a sink descriptor containing the new
   * location and partition which will be set as pending
   * @return false if the changes have already been applied, true otherwise
   */
  bool
  scheduleNewDescriptor(const NetworkSinkDescriptor &networkSinkDescriptor);

  /**
   * @brief apply pending changes to the receiver location, receiver partition
   * and version number
   * @return true if pending changes were found and applied, false if no pending
...
private:
  /**
   * @brief store a future in the worker context, spawn a new thread that will
   * create a new network channel and on establishing a connection pass the the
   * new channel into the future and pass a reconfiguration message to the sink
   * to signal that the connection has completed
   * @param workerContext the worker context to store the future in
   * @param newNodeLocation the location of the node to which the connection is
   * to be established
   * @param newNesPartition the partition of the source to which the connection
   * is to be established
   * @param newVersion The new version number used by the receiver to determine
   * if it can already accept this channel
   */
  void
  clearOldAndConnectToNewChannelAsync(Runtime::WorkerContext &workerContext,
                                      const NodeLocation &newNodeLocation,
                                      NesPartition newNesPartition,
                                      DecomposedQueryPlanVersion newVersion);

  /**
   * @brief write all data from the reconnect buffer to the currently active
   * network channel
  ...
  /**
   * @brief Checks if a network channel has been established. If so, stores it
   * in the worker context and unbuffers tuples into the channel
   * @param workerContext
