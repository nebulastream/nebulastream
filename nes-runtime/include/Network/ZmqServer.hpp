...
/**
 * @brief ZMQ server on hostname:port with numNetworkThreads i/o threads and a
 * set of callbacks in exchangeProtocol. This class is not copyable.
 */
class ZmqServer {
private:
...
public:
  /**
   * Create a ZMQ server on hostname:port with numNetworkThreads i/o threads and
   * a set of callbacks in exchangeProtocol
   * @param hostname
   * @param port
   * @param numNetworkThreads
   * @param exchangeProtocol
   */
  explicit ZmqServer(std::string hostname, uint16_t requestedPort,
...
private:
  ...
  /**
   * @brief the receiving thread where clients send their messages to the
   * server, here messages are forwarded to the handlerEventLoop by the proxy
   * @param numHandlerThreads number of HandlerThreads
   * @param startPromise the promise that is passed to the thread
   */
  void routerLoop(uint16_t numHandlerThreads,
   ...
   */
  void messageHandlerEventLoop(const std::shared_ptr<ThreadBarrier> &barrier,
                               int threadId);
...
namespace fmt {
template <> struct formatter<NES::Network::ZmqServer> : formatter<std::string> {
  auto format(const NES::Network::ZmqServer &zmq, format_context &ctx)
      -> decltype(ctx.out()) {
    return fmt::format_to(
        ctx.out(),
        "Hostname: {} requested Port:{} number of network threads: {}",
        zmq.getHostname(), zmq.getRequestedPort(), zmq.getNumOfThreads());
  }
};
} // namespace fmt
