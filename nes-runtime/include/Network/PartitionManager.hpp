...
/**
 * @brief this class keeps track of all ready partitions (and their
 * subpartitions) It keeps track of the ref cnt for each partition and
 * associated data emitter A data emitter is notified once there is data for its
 * partition. Pinning a partition for a Consumer/Producer means increasing its
 * reference counter to n: Consumer: there are n "listeners" that are to consume
 * the data of a partition Producer: there are n "emitters" that are to produce
 * the data for a partition When the reference counter reaches 0, it means that
 * none requires a partition.
 */
class PartitionManager {
 ...
 private:
  ...
  class PartitionProducerEntry {
   public:
     ...
     * @param emitter the data emitter that must be notified upon arrival of new
     * data
     */
  ...
  class PartitionConsumerEntry {
   public:
     ...
     * data
     */
    explicit PartitionConsumerEntry(NodeLocation&& senderLocation,
                                    DataEmitterPtr&& consumer);
    ...
    /**
     * @brief returns the number of disconnects that have occurred for this
     * partition
 ...
 public:
  ...
  /**
   * @brief Registers a subpartition in the PartitionManager. If the
   * subpartition does not exist a new entry is added in the partition table,
   * otherwise the counter is incremented.
   * @param the partition
   * @emitter
   * @return true if this is  the first time the partition was registered, false
   * otherwise
   */
  bool registerSubpartitionConsumer(NesPartition partition,
  ...
  /**
   * @brief Unregisters a subpartition in the PartitionManager. If the
   * subpartition does not exist or the current counter is 0 an error is thrown.
  ...
  /**
   * @brief Returns the current counter of a given partition. Throws error if
   * not existing.
   * @param the partition
   * @return the counter of the partition
   * @throw  std::out_of_range  If no such data is present.
   */
  std::optional<uint64_t> getSubpartitionConsumerCounter(
      NesPartition partition);

  /**
   * @brief get the current version number of the operator associated with this
   * partition
   ...
   */
  PartitionRegistrationStatus getConsumerRegistrationStatus(
   ...
   * otherwise
   */
  bool registerSubpartitionProducer(NesPartition partition,
                                    NodeLocation nodeLocation);
   ...
   * @throw  std::out_of_range  If no such data is present.
   */
  std::optional<uint64_t> getSubpartitionProducerCounter(
      NesPartition partition);
   ...
   * @return a PartitionRegistrationStatus
   */
  PartitionRegistrationStatus getProducerRegistrationStatus(
      NesPartition partition) const;
   ...
   */
  bool addSubpartitionEventListener(
      NesPartition partition, NodeLocation nodeLocation,
      Runtime::RuntimeEventListenerPtr eventListener);
   ...
   */
  Runtime::RuntimeEventListenerPtr getEventListener(
      NesPartition partition) const;
