 ...
 * 1. Pooled Buffers: preallocated fixed-size buffers of memory that must be
 * reference counted
 * 2. Unpooled Buffers: variable sized buffers that are allocated on-the-fly.
 * They are also subject to reference counting.
 *
 * The reference counting mechanism of the TupleBuffer is explained in
 * TupleBuffer.hpp
 *
 * The BufferManager stores the pooled buffers as MemorySegment-s. When a
 * component asks for a Pooled buffer, then the BufferManager retrieves an
 * available buffer (it blocks the calling thread, if no buffer is available).
 * It then hands out a TupleBuffer that is constructed through the pointer
 * stored inside a MemorySegment. This is necessary because the BufferManager
 * must keep all buffers stored to ensure that when its destructor is called,
 * all buffers that it has ever created are deallocated. Note the BufferManager
 * will check also that no reference counter is non-zero and will throw a fatal
 * exception, if a component hasnt returned every buffers. This is necessary to
 * avoid memory leaks.
 *
 * Unpooled buffers are either allocated on the spot or served via a previously
 * allocated, unpooled buffer that has been returned to the BufferManager by
 * some component.
 *
 */
class BufferManager : public std::enable_shared_from_this<BufferManager>,
 ...
 private:
  class UnpooledBufferHolder {
   public:
    ...
    UnpooledBufferHolder(std::unique_ptr<detail::MemorySegment>&& mem,
                         uint32_t size);

    void markFree();

    friend bool operator<(const UnpooledBufferHolder& lhs,
                          const UnpooledBufferHolder& rhs) {
 ...
 public:
   ...
   * @param withAlignment the alignment of each buffer, default is 64 so ony
   * cache line aligned buffers, This value must be a pow of two and smaller
   * than page size
   */
 ...
 private:
  /**
   * @brief Configure the BufferManager to use numOfBuffers buffers of size
   * bufferSize bytes. This is a one shot call. A second invocation of this call
   * will fail
   * @param withAlignment
   */
 ...
 public:
   ...
   * buffer
   */
  LocalBufferPoolPtr createLocalBufferPool(
      size_t numberOfReservedBuffers) override;
   ...
   * buffer
   */
  FixedSizeBufferPoolPtr createFixedSizeBufferPool(
      size_t numberOfReservedBuffers) override;
  ...
  /**
   * @brief this method clears all local buffers pools and remove all buffers
   * from the global buffer manager
   */
