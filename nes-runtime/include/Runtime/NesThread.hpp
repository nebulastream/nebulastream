...
/// Turn this on to have Thread::current_num_threads_ keep a count of
...
/// Gives every thread a unique, numeric thread ID, and recycles IDs when
/// threads exit. do not use this class from components that do not belong to
/// the worker execution engine!!!
class NesThread {
public:
  /// The number of entries in table. Currently, this is fixed at 256 and never
  /// changes or grows. If the table runs out of entries, then the current
  /// implementation will throw a std::runtime_error. do not change this value
  static constexpr WorkerThreadId::Underlying MaxNumThreads = 256;

private:
  /// Encapsulates a thread ID, getting a free ID from the Thread class when the
  /// thread starts, and releasing it back to the Thread class, when the thread
  /// exits.
  class ThreadId {
  public:
...
private:
  ...
  inline static void releaseEntry(WorkerThreadId workerThreadId) {
    ...
    auto threadWasInUse =
        id_used[workerThreadId.getRawValue() % MaxNumThreads].exchange(false);
