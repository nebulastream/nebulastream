...
public:
  ...
  /**
   * @brief Returns a new TupleBuffer wrapped in an optional or an invalid
   * option if there is no buffer.
   * @return a new buffer
   */
  virtual std::optional<TupleBuffer> getBufferNoBlocking() = 0;

  /**
   * @brief Returns a new Buffer wrapped in an optional or an invalid option if
   * there is no buffer available within timeout_ms.
   * @param timeout_ms the amount of time to wait for a new buffer to be retuned
   * @return a new buffer
   */
  virtual std::optional<TupleBuffer>
  getBufferTimeout(std::chrono::milliseconds timeout_ms) = 0;

  /**
   * @brief Returns an unpooled buffer of size bufferSize wrapped in an optional
   * or an invalid option if an error occurs.
   * @param bufferSize
   * @return a new buffer
   */
...
public:
  /**
   * @brief Create a local buffer manager that is assigned to one pipeline or
   * thread
   * @param numberOfReservedBuffers number of exclusive buffers to give to the
   * pool
   * @return a local buffer manager with numberOfReservedBuffers exclusive
   * buffer
   */
  virtual LocalBufferPoolPtr
  createLocalBufferPool(size_t numberOfReservedBuffers) = 0;
   ...
   * buffer
   */
  virtual FixedSizeBufferPoolPtr
  createFixedSizeBufferPool(size_t numberOfReservedBuffers) = 0;
};
