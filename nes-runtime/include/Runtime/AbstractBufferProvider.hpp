...
class AbstractBufferProvider {
 public:
   ...
   * @return a new buffer
   */
  virtual std::optional<TupleBuffer> getBufferTimeout(
...
class AbstractPoolProvider {
 public:
   ...
   * buffer
   */
  virtual LocalBufferPoolPtr createLocalBufferPool(
      size_t numberOfReservedBuffers) = 0;
   ...
   * buffer
   */
  virtual FixedSizeBufferPoolPtr createFixedSizeBufferPool(
      size_t numberOfReservedBuffers) = 0;
};
