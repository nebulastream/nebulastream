...
/**
 * @brief This class is responsible to look up OS/HW specs of the underlying
 * hardware, e.g., numa.
 */
...
public:
  ...
  public:
    explicit CpuDescriptor(uint16_t coreId = -1, uint16_t cpuId = -1)
        : coreId(coreId), cpuId(cpuId) {
      // nop
    }
    ...
    friend bool operator<(const CpuDescriptor &lhs, const CpuDescriptor &rhs) {
  ...
  class NumaDescriptor {
  public:
    explicit NumaDescriptor(uint32_t node_id = -1)
        : nodeId(node_id), physicalCpus() {
      // nop
    }
    ...
    NumaDescriptor &operator=(const NumaDescriptor &other) {
      nodeId = other.nodeId;
      physicalCpus = std::map<uint16_t, CpuDescriptor>(
          other.physicalCpus.begin(), other.physicalCpus.end());
      return *this;
    }
    ...
    void addCpu(uint16_t cpu, uint16_t core) {
      if (physicalCpus.find(core) == physicalCpus.end()) {
        ...
        physicalCpus[core].cpuId =
            std::min<uint16_t>(physicalCpus[core].cpuId, cpu);
      }
    }
