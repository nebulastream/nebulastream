...
class WorkerContext {
 private:
  ...
  using WorkerContextBufferProvider =
      WorkerContextBufferProviderPtr::element_type;
  using WorkerContextBufferProviderRawPtr =
      WorkerContextBufferProviderPtr::element_type*;
  ...
  std::unordered_map<OperatorId, Network::EventOnlyNetworkChannelPtr>
      reverseEventChannels;
  /// reverse event channels that have not established a connection yet
  std::unordered_map<OperatorId,
                     std::pair<std::future<Network::EventOnlyNetworkChannelPtr>,
                               std::promise<bool>>>
  ...
  std::unordered_map<OperatorId, std::queue<NES::Runtime::TupleBuffer>>
      reconnectBufferStorage;
 ...
 public:
   ...
   * This can be accessed at any point in time also without the pointer to the
   * context. Calling this method from a non worker thread results in undefined
   * behaviour.
  ...
  /**
   * @brief get current worker context thread id. This is assigned by calling
   * NesThread::getId()
  ...
  /**
   * @brief Sets the ref counter for a generic object using its pointer address
   * as lookup
  ...
  /**
   * @brief Reduces by one the ref cnt. It deletes the object as soon as ref cnt
   * reaches 0.
  ...
  /**
   * @brief This stores a future for network channel creation and a promise
   * which can be used to abort the creation
   * @param id of the operator that we want to store the output channel
   * @param channelFuture a pair of a future waiting for the output channel to
   * be connected and a promise to be used if the connection process is to be
   * aborted
   */
   ...
   */
  void insertIntoStorage(Network::NesPartition nesPartition,
   ...
   */
  std::optional<NES::Runtime::TupleBuffer> getTopTupleFromStorage(
      Network::NesPartition nesPartition);

  /**
   * @brief if the storage is not empty remove the oldest buffered tuple for the
   * specified partition
   * @param nesPartition partition
   */
   ...
   * @param currentMessageSequenceNumber represents the total number of data
   * buffer messages sent
   */
  bool releaseNetworkChannel(OperatorId id, Runtime::QueryTerminationType type,
   ...
   * @param channel the output channel
   */
  void storeEventOnlyChannel(OperatorId id,
   ...
   */
  bool releaseEventOnlyChannel(OperatorId id,
   ...
   */
  std::optional<Network::NetworkChannelPtr> getAsyncConnectionResult(
      OperatorId operatorId);

  /**
   * @brief blocks until async connection of a network channel has succeeded or
   * timed out
   * @param operatorId id of the operator which will use the network channel
   * @return a pointer to the network channel or nullptr if the connection timed
   * out
   */
  Network::NetworkChannelPtr waitForAsyncConnection(OperatorId operatorId);

  /**
   * @brief check if an async connection that was started by the operator with
   * the specified id is currently in progress
   * @param operatorId id of the operator which will use the network channel
  ...
  /**
   * @brief retrieve a registered output channel
   * @param operatorId id of the operator that we want to store the output
   * channel
   * @return an output channel
   */
  Network::EventOnlyNetworkChannel* getEventOnlyNetworkChannel(
      OperatorId operatorId);
   ...
   */
  void insertIntoReconnectBufferStorage(OperatorId operatorId,
                                        NES::Runtime::TupleBuffer buffer);
   ...
   */
  std::optional<TupleBuffer> removeBufferFromReconnectBufferStorage(
      OperatorId operatorId);

  /**
   * @brief stop a connection process which is currently in progress
   * @param operatorId the id of the operator that started the connection
   * process
   */
  ...
  /**
   * @brief store a future for an event channel that is in the process of
   * connecting
  ...
  getAsyncEventChannelConnectionResult(OperatorId operatorId);

  /**
   * @brief blocks until async connection of an event channel has succeeded or
   * timed out
   * @param operatorId id of the operator which will use the event channel
   * @return a pointer to the event channel or nullptr if the connection timed
   * out
   */
  Network::EventOnlyNetworkChannelPtr waitForAsyncConnectionEventChannel(
      OperatorId operatorId);

  /**
   * @brief check if a network channel exists for the operator in question
   * @param operatorId the unique identifier of the operator to which the
   * channel belongs
