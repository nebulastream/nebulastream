...
/**
 * @brief The RowLayoutField enables assesses to a specific field in a row
 * layout. It overrides the operator[] for a more user friendly access of tuples
 * for a predefined field. As this required direct knowledge of a particular
 * memory layout at compile-time, consider to use the TestTupleBuffer.
 * @tparam T the type of the field
 * @tparam boundaryChecks flag to identify if buffer bounds should be checked at
 * runtime.
 * @caution This class is non-thread safe
 */
template <class T, bool boundaryChecks = true> class RowLayoutField {
public:
  /**
   *  Factory to create a RowLayoutField for a specific memory layout and a
  ...
  create(uint64_t fieldIndex, std::shared_ptr<RowLayout> layout,
         TupleBuffer &buffer);
   ...
   */
  static inline RowLayoutField<T, boundaryChecks>
  create(const std::string &fieldName, std::shared_ptr<RowLayout> layout,
         TupleBuffer &buffer);
...
private:
   ...
   * @param recordSize
   */
  RowLayoutField(std::shared_ptr<RowLayout> layout, uint8_t *basePointer,
                 FIELD_SIZE fieldIndex, FIELD_SIZE recordSize)
      : fieldIndex(fieldIndex), recordSize(recordSize),
...
RowLayoutField<T, boundaryChecks>::create(uint64_t fieldIndex,
                                          std::shared_ptr<RowLayout> layout,
                                          TupleBuffer &buffer) {
  ...
  return RowLayoutField<T, boundaryChecks>(layout, basePointer, fieldIndex,
                                           layout->getTupleSize());
}

template <class T, bool boundaryChecks>
inline RowLayoutField<T, boundaryChecks>
RowLayoutField<T, boundaryChecks>::create(const std::string &fieldName,
                                          std::shared_ptr<RowLayout> layout,
                                          TupleBuffer &buffer) {
  auto fieldIndex = layout->getFieldIndexFromName(fieldName);
  if (fieldIndex.has_value()) {
    return RowLayoutField<T, boundaryChecks>::create(fieldIndex.value(), layout,
                                                     buffer);
  }
