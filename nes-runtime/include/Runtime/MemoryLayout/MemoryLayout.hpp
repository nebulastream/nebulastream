...
/**
 * @brief Reads the variable sized data from the child buffer at the provided
 * index
 * @param buffer
 * @param childBufferIdx
 * @return Variable sized data as a string
 */
std::string readVarSizedData(const TupleBuffer& buffer,
 ...
 */
std::optional<uint32_t> writeVarSizedData(const TupleBuffer& buffer,
                                          const std::string_view value,
                                          BufferManager& bufferManager);

/**
 * @brief A MemoryLayout defines a strategy in which a specific schema / a
 * individual tuple is mapped to a tuple buffer. To this end, it requires the
 * definition of an schema and a specific buffer size. Currently. we support a
 * RowLayout and a ColumnLayout.
 */
class MemoryLayout {
 public:
  /**
   * @brief Constructor for MemoryLayout.
   * @param bufferSize A memory layout is always created for a specific buffer
   * size.
  ...
  /**
   * Gets the field index for a specific field name. If the field name not
   * exists, we return an empty optional.
   * @param fieldName
   * @return either field index for fieldName or empty optional
   */
  [[nodiscard]] std::optional<uint64_t> getFieldIndexFromName(
  ...
  /**
   * @brief Calculates the offset in the tuple buffer of a particular field for
   * a specific tuple. Depending on the concrete MemoryLayout, e.g., Columnar or
   * Row-Layout, this may result in different calculations.
   ...
   */
  [[nodiscard]] virtual uint64_t getFieldOffset(uint64_t tupleIndex,
                                                uint64_t fieldIndex) const = 0;
   ...
   */
  [[nodiscard]] virtual std::optional<uint64_t> getFieldOffset(
      uint64_t tupleIndex, const std::string_view fieldName) const;

  /**
   * @brief Gets the number of tuples a tuple buffer with this memory layout
   * could occupy. Depending on the concrete memory layout this value may
   * change, e.g., some layouts may add some padding or alignment.
