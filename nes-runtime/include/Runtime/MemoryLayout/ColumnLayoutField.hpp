...
/**
 * @brief The ColumnLayoutField enables assesses to a specific field in a
 * columnar layout. It overrides the operator[] for a more user friendly access
 * of tuples for a predefined field. As this required direct knowledge of a
 * particular memory layout at compile-time, consider to use the
 * TestTupleBuffer.
 * @tparam T the type of the field
 * @tparam boundaryChecks flag to identify if buffer bounds should be checked at
 * runtime.
 * @caution This class is non-thread safe
 */
template <class T, bool boundaryChecks = true> class ColumnLayoutField {

public:
  /**
   * Factory to create a ColumnLayoutField for a specific memory layout and a
   * specific tuple buffer.
  ...
  create(uint64_t fieldIndex, std::shared_ptr<ColumnLayout> layout,
         TupleBuffer &buffer);

  /**
   * Creates a ColumnLayoutField for a specific memory layout and a specific
   * tuple buffer.
   * @param fieldIndex
   * @param layout the memory layout
   * @tparam buffer the tuple buffer
   ...
   */
  static inline ColumnLayoutField<T, boundaryChecks>
  create(const std::string &fieldName, std::shared_ptr<ColumnLayout> layout,
         TupleBuffer &buffer);

  /**
   * Accesses the value of this field for a specific record.
   * @param recordIndex
   * @return reference to a field attribute from the created field handler
   * accessed by recordIndex
   */
...
ColumnLayoutField<T, boundaryChecks>::create(
    uint64_t fieldIndex, std::shared_ptr<ColumnLayout> layout,
    TupleBuffer &buffer) {
  if (boundaryChecks && fieldIndex >= layout->getFieldSizes().size()) {
    NES_THROW_RUNTIME_ERROR("fieldIndex out of bounds! "
...
ColumnLayoutField<T, boundaryChecks>::create(
    const std::string &fieldName, std::shared_ptr<ColumnLayout> layout,
    TupleBuffer &buffer) {
  auto fieldIndex = layout->getFieldIndexFromName(fieldName);
  if (fieldIndex.has_value()) {
    return ColumnLayoutField<T, boundaryChecks>::create(fieldIndex.value(),
                                                        layout, buffer);
  }
  NES_THROW_RUNTIME_ERROR(
      "DynamicColumnLayoutField: Could not find fieldIndex for " << fieldName);
}

template <class T, bool boundaryChecks>
inline T &ColumnLayoutField<T, boundaryChecks>::operator[](size_t recordIndex) {
  if (boundaryChecks && recordIndex >= layout->getCapacity()) {
    NES_THROW_RUNTIME_ERROR("recordIndex out of bounds!"
                            << layout->getCapacity() << " >= " << recordIndex);
  }
