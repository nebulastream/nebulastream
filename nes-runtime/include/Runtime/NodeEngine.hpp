 ...
 */
class NodeEngine
    : public Network::ExchangeProtocolListener,
...
public:
   ...
   * @param callstack
   */
  void onFatalException(std::shared_ptr<std::exception> exception,
   ...
   */
  [[nodiscard]] bool deployQueryInNodeEngine(
   ...
   * @return true if succeeded, else false
   */
  [[nodiscard]] bool registerExecutableQueryPlan(
      const Execution::ExecutableQueryPlanPtr &queryExecutionPlan);
   ...
   * @return true if succeeded, else false
   */
  [[nodiscard]] bool registerDecomposableQueryPlan(
  ...
  /**
   * @brief method to trigger the buffering of data on a NetworkSink of a Query
   * Sub Plan with the given id
   * @param decomposedQueryPlanId : the id of the Query Sub Plan to which the
   * Network Sink belongs to
   * @param uniqueNetworkSinkDescriptorId : the id of the Network Sink
   * Descriptor. Helps identify the Network Sink on which to buffer data
   * @return bool indicating success
   */
  bool bufferData(DecomposedQueryPlanId decomposedQueryPlanId,
                  OperatorId uniqueNetworkSinkDescriptorId);

  /**
   * @brief method to trigger the reconfiguration of a NetworkSink so that it
   * points to a new downstream node.
   ...
   * Network Sink belongs to
   * @param uniqueNetworkSinkDescriptorId : the id of the Network Sink
   * Descriptor. Helps identify the Network Sink to reconfigure.
   * @return bool indicating success
   */
  bool updateNetworkSink(WorkerId newNodeId, const std::string &newHostname,
                         uint32_t newPort,
                         DecomposedQueryPlanId decomposedQueryPlanId,
                         OperatorId uniqueNetworkSinkDescriptorId);
  ...
  getQueryStatus(SharedQueryId sharedQueryId);
  ...
  getQueryStatistics(SharedQueryId sharedQueryId);

  /**
   * @brief method to return the query statistics
   * @param withReset specifies if the statistics is deleted after reading (so
   * we start with 0)
   * @return vector of queryStatistics
   */
  ...
  /**
   * @brief this callback is called once a tuple buffer arrives on the network
   * manager for a given nes partition
   */
  ...
  /**
   * @brief this callback is called once an error is raised on the
   * channel(client) side
   */
  ...
  getDecomposedQueryIds(SharedQueryId sharedQueryId);
  ...
  /**
   * @return applies reconfigurations to the sources or sinks of a sub plan.
   * Reconfigured sources will start expecting connections from a new upstream
   * sink. Reconfigured sinks will scheduled a pending change of the downstream
   * source to which they send their data.
   * @param reconfiguredDecomposedQueryPlan A query plan containing source or
   * sink descriptors which contain the updated sender/receiver date.
   * @return true if a running sub query with a matching id was found and
   * reconfigured. False if the id of the supplied plan did not match any
   * running sub query
   */
  bool
  reconfigureSubPlan(DecomposedQueryPlanPtr &reconfiguredDecomposedQueryPlan);
...
public:
   ...
   */
  explicit NodeEngine(
      std::vector<PhysicalSourceTypePtr> physicalSources, HardwareManagerPtr &&,
      std::vector<BufferManagerPtr> &&, QueryManagerPtr &&,
      std::function<Network::NetworkManagerPtr(std::shared_ptr<NodeEngine>)> &&,
      Network::PartitionManagerPtr &&, QueryCompilation::QueryCompilerPtr &&,
      std::weak_ptr<AbstractQueryStatusListener> &&, OpenCLManagerPtr &&,
...
private:
  ...
  std::map<SharedQueryId, std::vector<DecomposedQueryPlanId>>
      sharedQueryIdToDecomposedQueryPlanIds;
  std::map<DecomposedQueryPlanId, Execution::ExecutableQueryPlanPtr>
      deployedExecutableQueryPlans;
