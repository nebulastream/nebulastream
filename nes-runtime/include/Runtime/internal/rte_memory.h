 ...
 * locations should not overlap.
 */
static inline void rte_mov128blocks(uint8_t* dst, const uint8_t* src,
                                    size_t n) {
  __m512i zmm0, zmm1;
 ...
 * locations should not overlap.
 */
static inline void rte_mov512blocks(uint8_t* dst, const uint8_t* src,
                                    size_t n) {
...
static inline void* rte_memcpy_generic(void* dst, const void* src, size_t n) {
  ...
  COPY_BLOCK_128_BACK63:
    ...
    if (n > 0) rte_mov64((uint8_t*)dst - 64 + n, (const uint8_t*)src - 64 + n);
    return ret;
  }
