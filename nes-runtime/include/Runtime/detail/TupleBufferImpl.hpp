...
/**
 * @brief Computes aligned buffer size based on original buffer size and
 * alignment
 */
constexpr uint32_t alignBufferSize(uint32_t bufferSize,
...
/**
 * @brief This class provides a convenient way to track the reference counter as
 * well metadata for its owning MemorySegment/TupleBuffer. In particular, it
 * stores the atomic reference counter that tracks how many live reference
 * exists of the owning MemorySegment/TupleBuffer and it also stores the
 * callback to execute when the reference counter reaches 0.
 *
...
public:
  explicit BufferControlBlock(
      MemorySegment *owner, BufferRecycler *recycler,
   ...
   */
  void addRecycleCallback(
      std::function<void(MemorySegment *, BufferRecycler *)> &&func) noexcept;

  /**
   * @brief This method must be called before the BufferManager hands out a
   * TupleBuffer. It ensures that the internal reference counter is zero. If
   * that's not the case, an exception is thrown.
  ...
  [[nodiscard]] bool loadChildBuffer(uint16_t index,
                                     BufferControlBlock *&control,
                                     uint8_t *&ptr, uint32_t &size) const;
...
private:
  ...
  public:
    ...
    friend std::ostream &operator<<(std::ostream &os,
                                    const ThreadOwnershipInfo &info) {
      os << info.threadName << " buffer is used in " << info.callstack;
      return os;
    }
  };
  std::mutex owningThreadsMutex;
  std::unordered_map<std::thread::id, std::deque<ThreadOwnershipInfo>>
      owningThreads;
#endif
};
...
/**
 * @brief The MemorySegment is a wrapper around a pointer to allocated memory of
 * size bytes and a control block
 * (@see class BufferControlBlock). The MemorySegment is intended to be used
 * **only** in the BufferManager. The BufferManager is the only class that can
 * store MemorySegments. A MemorySegment has no clue of what it's stored inside
 * its allocated memory and has no way to expose the pointer to outside world.
 * The public companion of a MemorySegment is the TupleBuffer, which can "leak"
 * the pointer to the outside world.
 *
 * Reminder: this class should be header-only to help inlining
 *
 */
...
public:
  ...
  explicit MemorySegment(
      uint8_t *ptr, uint32_t size, BufferRecycler *recycler,
...
private:
  ...
  /*
   ...
   Layout of the mem segment (padding might be added differently depending on
   the compiler in-use).
   +--------------------------------+-----------+-------------------+----------------------+
   | pointer to control block  (8b) | size (4b) | likely 4b padding | pointer to
   data (8b) |
   +------------+-------------------+-----------+-------------------+---------+------------+
                |                                                    |
   +------------+                +-----------------------------------+
...
/**
 * @brief This is the callback that is called when ZMQ is done with the sending
 * of the buffer with payload in ptr. The hint parameter is the size of the
 * whole buffer (casted as void*)
 */
