...
/**
 * @brief Container for storing information, related to the state of a sequence
 * number
 */
...
/**
 * @brief The PipelineExecutionContext is passed to a compiled pipeline and
 * offers basic functionality to interact with the Runtime. Via the context, the
 * compile code is able to allocate new TupleBuffers and to emit tuple buffers
 * to the Runtime.
 */
class PipelineExecutionContext
    : public std::enable_shared_from_this<PipelineExecutionContext> {

public:
  /**
   * @brief The PipelineExecutionContext is passed to the compiled pipeline and
   * enables interaction with the NES Runtime.
   * @param emitFunctionHandler an handler to receive the emitted buffers from
   * the pipeline.
   * @param emitToQueryManagerFunctionHandler an handler to receive emitted
   * buffers, which are then dispatched to the query manager.
   * @param operatorHandlers a list of operator handlers managed by the pipeline
   * execution context.
   */
  explicit PipelineExecutionContext(
      PipelineId pipelineId, DecomposedQueryPlanId queryId,
      Runtime::BufferManagerPtr bufferProvider, size_t numberOfWorkerThreads,
      std::function<void(TupleBuffer &, WorkerContextRef)>
          &&emitFunctionHandler,
      std::function<void(TupleBuffer &)> &&emitToQueryManagerFunctionHandler,
      std::vector<OperatorHandlerPtr> operatorHandlers);

  /**
   * @brief Emits a output tuple buffer to the Runtime. Internally we call the
   * emit function which is a callback to the correct handler.
  ...
  /**
   * @brief Retrieves a Operator Handler at a specific index and cast its to an
   * OperatorHandlerType.
  ...
  /**
   * @brief Returns the next chunk number belonging to a sequence number for
   * emitting a buffer
  ...
  /**
   * @brief Checks if this PipelineExecutionContext has seen all chunks for a
   ...
   */
  bool isLastChunk(const SeqNumberOriginId seqNumberOriginId,
                   const uint64_t chunkNumber, const bool isLastChunk);

  /**
   * @brief Removes the sequence state in seqNumberOriginIdToChunkStateInput and
   * seqNumberOriginIdToOutputChunkNumber for the seqNumberOriginId
   * @param seqNumberOriginId
   */
...
private:
   ...
   */
  const std::vector<std::shared_ptr<NES::Runtime::Execution::OperatorHandler>>
      operatorHandlers;

  folly::Synchronized<std::map<SeqNumberOriginId, SequenceState>>
      seqNumberOriginIdToChunkStateInput;
  folly::Synchronized<std::map<SeqNumberOriginId, uint64_t>>
      seqNumberOriginIdToOutputChunkNumber;
