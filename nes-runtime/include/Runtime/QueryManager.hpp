...
class ThreadPool;
using ThreadPoolPtr =
    std::shared_ptr<ThreadPool>; // TODO consider moving this atomic in c++20

class AsyncTaskExecutor;
using AsyncTaskExecutorPtr = std::shared_ptr<AsyncTaskExecutor>;

class AbstractQueryManager
    : public NES::detail::virtual_enable_shared_from_this<AbstractQueryManager,
                                                          false>,
      public Reconfigurable {
public:
  using inherited0 =
      NES::detail::virtual_enable_shared_from_this<AbstractQueryManager, false>;
  ...
  /**
   * @brief register a query by extracting sources, windows and sink and add
   * them to respective map
   * @param QueryExecutionPlan to be deployed
   */
  virtual bool registerQuery(const Execution::ExecutableQueryPlanPtr &qep);

  /**
   * @brief deregister a query by extracting sources, windows and sink and
   * remove them from respective map
   * @param QueryExecutionPlan to be deployed
   ...
   */
  virtual ExecutionResult processNextTask(bool running,
                                          WorkerContext &workerContext) = 0;

  /**
   * @brief add work to the query manager, this methods is source-driven and is
   * called for each buffer generated by the window trigger
   * @param Pointer to the tuple buffer containing the data
   * @param Pointer to the pipeline stage that will be executed next
   * @param id of the queue where to put the task (only necessary if multiple
   * queues are used)
   */
  virtual void
  addWorkForNextPipeline(TupleBuffer &buffer,
                         Execution::SuccessorExecutablePipeline executable,
                         uint32_t queueId = 0) = 0;
  ...
  getQueryExecutionPlan(DecomposedQueryPlanId id) const;

  [[nodiscard]] bool canTriggerEndOfStream(DataSourcePtr source,
  ...
  /**
   * @brief reset query manager. After this call, it wont be possible to use the
   * query manager.
   */
   ...
   * @return true if the reconfiguration task was added correctly on the worker
   * queue N.B.: this does not not mean that the reconfiguration took place but
   * it means that it was scheduled to be executed!
   ...
   * @param blocking: whether to block until the reconfiguration is done. Mind
   * this parameter because it blocks!
   */
...
private:
   ...
   */
  virtual bool
  addReconfigurationMessage(SharedQueryId sharedQueryId,
                            DecomposedQueryPlanId queryExecutionPlanId,
                            TupleBuffer &&buffer, bool blocking = false) = 0;
...
public:
   ...
   */
  void notifyTaskFailure(Execution::SuccessorExecutablePipeline pipeline,
   ...
   */
  void notifySourceFailure(DataSourcePtr source,
   ...
   */
  SharedQueryId
  getSharedQueryId(DecomposedQueryPlanId decomposedQueryPlanId) const;
   ...
   */
  bool addEndOfStream(DataSourcePtr source,
   ...
   * @param terminationType the type of termination (e.g., failure, soft)
   */
  void notifySinkCompletion(DecomposedQueryPlanId decomposedQueryPlanId,
                            DataSinkPtr sink,
                            QueryTerminationType terminationType);
...
protected:
   ...
   * @param workerContext
   */
  virtual void updateStatistics(const Task &task, SharedQueryId sharedQueryId,
...
protected:
  ...
  /// worker thread for async maintenance task, e.g., fail
  /// queryIdAndCatalogEntryMapping
  AsyncTaskExecutorPtr asyncTaskExecutor;

  std::unordered_map<DecomposedQueryPlanId, Execution::ExecutableQueryPlanPtr>
      runningQEPs;

  // TODO:check if it would be better to put it in the thread context
  mutable std::mutex statisticsMutex;
  cuckoohash_map<DecomposedQueryPlanId, QueryStatisticsPtr>
  ...
  std::atomic<QueryManagerStatus> queryManagerStatus{
      QueryManagerStatus::Created};

  std::vector<AtomicCounter<uint64_t>> tempCounterTasksCompleted;

  std::shared_ptr<AbstractQueryStatusListener> queryStatusListener;

  std::unordered_map<OperatorId, std::vector<Execution::ExecutableQueryPlanPtr>>
      sourceToQEPMapping;
...
public:
  explicit DynamicQueryManager(
      std::shared_ptr<AbstractQueryStatusListener> queryStatusListener,
      std::vector<BufferManagerPtr> bufferManager, WorkerId nodeEngineId,
      uint16_t numThreads, HardwareManagerPtr hardwareManager,
      uint64_t numberOfBuffersPerEpoch,
      std::vector<uint64_t> workerToCoreMapping = {});
   ...
   * @return an execution result
   */
  ExecutionResult processNextTask(bool running,
...
protected:
   ...
   * @param workerContext
   */
  void updateStatistics(const Task &task, SharedQueryId sharedQueryId,
                        DecomposedQueryPlanId decomposedQueryPlanId,
                        PipelineId pipeId,
                        WorkerContext &workerContext) override;
...
public:
  explicit MultiQueueQueryManager(
      ...
      uint64_t numberOfBuffersPerEpoch,
      std::vector<uint64_t> workerToCoreMapping = {},
      uint64_t numberOfQueues = 1, uint64_t numberOfThreadsPerQueue = 1);
