...
/**
 * @brief Task abstraction to bind processing (compiled binary) and data
 * (incoming buffers
 * @Limitations:
 *    -
 */
...
public:
   ...
   * @param pointer to query execution plan that should be applied on the
   * incoming buffer
   ...
   */
  explicit Task(Execution::SuccessorExecutablePipeline pipeline,
                TupleBuffer buf, uint64_t taskId);

  constexpr explicit Task() noexcept = default;

  /**
   * @brief execute the task by calling executeStage of QEP and providing the
   * stageId and the buffer
   */
