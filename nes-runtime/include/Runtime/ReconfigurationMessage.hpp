...
public:
  /**
   * @brief create a reconfiguration task that will be used to kickstart the
  ...
  explicit ReconfigurationMessage(const SharedQueryId sharedQueryId,
      ...
      : type(type), instance(std::move(instance)), syncBarrier(nullptr),
        postSyncBarrier(nullptr), sharedQueryId(sharedQueryId),
  ...
  /**
   * @brief create a reconfiguration task that will be passed to every running
   * thread
  ...
  explicit ReconfigurationMessage(const SharedQueryId sharedQueryId,
                                  const DecomposedQueryPlanId parentPlanId,
                                  ReconfigurationType type, uint64_t numThreads,
                                  ReconfigurablePtr instance,
                                  std::any &&userdata = nullptr,
                                  bool blocking = false)
      : type(type), instance(std::move(instance)), postSyncBarrier(nullptr),
        sharedQueryId(sharedQueryId), parentPlanId(parentPlanId),
  ...
  explicit ReconfigurationMessage(const ReconfigurationMessage &other,
                                  uint64_t numThreads, bool blocking = false)
  ...
  ReconfigurationMessage(const ReconfigurationMessage &that)
      : type(that.type), instance(that.instance), syncBarrier(nullptr),
        postSyncBarrier(nullptr), sharedQueryId(that.sharedQueryId),
   ...
   * @return the plan id
   */
  [[nodiscard]] DecomposedQueryPlanId getParentPlanId() const {
    return parentPlanId;
  }
   ...
   */
  template <typename T> [[nodiscard]] T getUserData() const {
