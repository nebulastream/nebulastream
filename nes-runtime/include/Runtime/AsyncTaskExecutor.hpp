...
/**
 * @brief This is a multi-threaded task executor that execute asynchronously
 * tasks. Submitted task are paired to a future value that can be retrieved when
 * the async task is executed.
 */
class AsyncTaskExecutor {
public:
  /**
   * @brief This is a future that the async executor returns, which can be
   * retrieved using `.wait()`
   * @tparam R the return type
   */
  template <class R> class AsyncTaskFuture {
  public:
    /**
     * @brief Creates an AsyncTaskFuture from a promise and a reference to the
     * executor
     * @param promise the promise that the executor will fulfill
     * @param owner the executor
     */
    explicit AsyncTaskFuture(std::shared_ptr<std::promise<R>> promise,
                             AsyncTaskExecutor &owner)
        : promise(std::move(promise)), owner(owner) {
      future = this->promise->get_future();
    }

    AsyncTaskFuture(const AsyncTaskFuture &that)
        : promise(that.promise), future(that.future), owner(that.owner) {}
    ...
    /**
     * @brief This call blocks until the promise is fulfilled and returns the
     * produced value
     * @return
     */
    ...
    /**
     * @brief This method creates a new (inner) AsyncTaskFuture from an already
     * existing (*this) AsyncTaskFuture. When the *this AsyncTaskFuture is
     * fulfilled, its internal value is passed to f and the return value is set
     * as internal value of the inner AsyncTaskFuture.
     * @tparam Function the function type to execute on the promise value of the
     * *this AsyncTaskFuture
     * @param f the function to execute on the promise value of the *this
     * AsyncTaskFuture
     * @return a new inner AsyncTaskFuture
     */
    template <class Function>
    [[nodiscard]] AsyncTaskFuture<
        std::invoke_result_t<std::decay_t<Function>, std::decay_t<R>>>
    thenAsync(Function &&f) {
      return owner.runAsync(
          [this, f = std::move(f)]() { return f(future.get()); });
    }
  ...
  private:
    std::shared_ptr<std::promise<R>>
        promise; // we need this variable to keep the promise machinery alive
...
private:
   ...
   */
  template <class R, class... ArgTypes> class AsyncTask {
  public:
    /**
    ...
    template <
        class Function,
        std::enable_if_t<
            std::is_same_v<R, std::invoke_result_t<std::decay_t<Function>,
                                                   std::decay_t<ArgTypes>...>>,
            bool> = true>
    explicit AsyncTask(Function &&f)
        : func(std::move(f)), promise(std::make_shared<std::promise<R>>()) {}
    ...
    /**
     * @brief Creates an AsyncTaskFuture out of the current task. The
     * AsyncTaskFuture is to be used to retrieve the AsyncTask produced value
     * @param owner the owning executor
     * @return the AsyncTaskFuture to wait on
     */
    inline AsyncTaskFuture<R> makeFuture(AsyncTaskExecutor &owner) {
      return AsyncTaskFuture(promise, owner);
    }
  ...
  /**
   * @brief a type-less wrapper of the AsyncTask. Necessary to store this in a
   * simple container like an `std::dequeue`
   */
  class AsyncTaskWrapper {
  public:
    /**
     * @brief Creates a new AsyncTaskWrapper from a function that wraps an
     * AsyncTask and a void pointer to the asyncTaskPtr (for garbage collection)
     * @param func the function to execute
     * @param asyncTaskPtr
     */
    explicit AsyncTaskWrapper(std::function<void(void)> &&func,
...
public:
   ...
   */
  explicit AsyncTaskExecutor(const HardwareManagerPtr &hardwareMananger,
  ...
  /**
   * @brief Method to clean up the internal resources (called by the
   * destructor). Only the first invocation cleans inner resources. Subsequent
   * calls won't produced any effect.
   * @return
   */
  bool destroy();
  ...
  runAsync(Function &&f, Args &&...args) {
    if (!running) {
      throw Exceptions::RuntimeException("Async Executor is destroyed");
    }
    // allocates a task on the heap and captures all arguments and types in the
    // AsyncTask next, everything is captured in the AsyncTaskWrapper, which has
    // no types.
    constexpr auto taskSize = sizeof(
        AsyncTask<
            std::invoke_result_t<std::decay_t<Function>, std::decay_t<Args>...>,
            Args...>);
    auto *taskPtr = allocateAsyncTask(taskSize);
    NES_ASSERT(taskPtr, "Cannot allocate async task");
    auto *task = new (taskPtr) AsyncTask<
        std::invoke_result_t<std::decay_t<Function>, std::decay_t<Args>...>,
        Args...>(std::move(f));
    ...
    NES_ASSERT(!!releaseCallback, "Cannot allocate async task");
    {
      ...
      asyncTaskQueue.emplace_back(std::move(asyncCallback),
