...
public:
    ...
    UIntOption rpcPort = {"rpc_port", "0", "RPC server port of the NES Worker.", {std::make_shared<NumberValidation>()}};
    ...
    UIntOption numberOfBuffersInGlobalBufferManager
        = {"number_of_buffers_in_global_buffer_manager", "1024", "Number buffers in global buffer pool.", {std::make_shared<NumberValidation>()}};

    UIntOption numberOfBuffersPerWorker
        = {"number_of_buffers_per_worker", "128", "Number buffers in task local buffer pool.", {std::make_shared<NumberValidation>()}};
