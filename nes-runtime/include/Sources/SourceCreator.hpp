...
namespace NES {
/**
 * @brief function to create a test source which produces 10 tuples within one
 * buffer with value one based on a schema
 ...
 * @param statisticId represents the unique identifier of components that we can
 * track statistics for
 * @param numSourceLocalBuffers the number of buffers allocated to a source
 * @param physicalSourceName the name and unique identifier of a physical source
 * @param successors the subsequent operators in the pipeline to which the data
 * is pushed
...
/**
 * @brief function to create a test source which produces   tuples with value
 * one in N buffers of based on a schema
 * @param schema schema of the data source
 * @param bufferManager pointer to the buffer manager
 * @param queryManager pointer to the query manager
...
/**
 * @brief function to create a test source which produces 10 tuples with value
 ...
 * track statistics for
 * @param numSourceLocalBuffers the number of buffers allocated to a source
 * @param gatheringMode the gathering mode (INTERVAL_MODE, INGESTION_RATE_MODE,
 * or ADAPTIVE_MODE)
 ...
 * @param nesPartition the unique identifier of the network source that consumes
 * the input buffers
 * @param sinkLocation location of the sink that writes to the network source
 * @param numSourceLocalBuffers the number of buffers allocated to a source
 * @param waitTime the amount of time the networkSource is supposed to wait for
 * a reply
 * @param retryTimes the number of times the networkSource resends a message, if
 * it does not receive a reply within waitTime
 * @param physicalSourceName the name and unique identifier of a physical source
 * @param version the version number of the source at its time of creation
 * @param successors the subsequent operators in the pipeline to which the data
 * is pushed
 * @param uniqueNetworkSourceId a unique identifier of the network source which
 * does not change over its lifetime
 * @return a data source pointer
 */
 ...
 * @param topic the kafka topic which organizes tuples of the same entity.
 * Usually the name of a data stream
 * @param groupId the ID of a logical group which consumes messages from one or
 * more kafka topics
 * @param autoCommit bool indicating if offset has to be committed automatically
 * or not
 * @param kafkaConsumerTimeout the timeperiod after which a timeout is issued
 * for the consumer
 * @param offsetMode instructs the broker from which point of a topic a consumer
 * wants consume messages from
 ...
 * track statistics for
 * @param numSourceLocalBuffers the number of buffers allocated to a source
 * @param batchSize the maximum amount of data (in bytes) that a Kafka producer
 * can accumulate before sending a batch of messages to the Kafka
 * @param physicalSourceName the name and unique identifier of a physical source
 * @param successors the subsequent operators in the pipeline to which the data
 * is pushed
 * @return a data source pointer
...
const DataSourcePtr createKafkaSource(
    ...
    OriginId originId, StatisticId statisticId, size_t numSourceLocalBuffers,
    size_t batchSize, const std::string& physicalSourceName,
    const std::vector<Runtime::Execution::SuccessorExecutablePipeline>&
        successors);
#endif
#ifdef ENABLE_OPC_BUILD
 ...
 * @param bufferManager The BufferManager is responsible for: 1. Pooled Buffers:
 * preallocated fixed-size buffers of memory that must be reference counted 2.
 * Unpooled Buffers: variable sized buffers that are allocated on-the-fly. They
 * are also subject to reference counting.
