...
class TCPSource : public DataSource {
 public:
   ...
   * @param bufferManager The BufferManager is responsible for: 1. Pooled
   * Buffers: preallocated fixed-size buffers of memory that must be reference
   * counted 2. Unpooled Buffers: variable sized buffers that are allocated
   * on-the-fly. They are also subject to reference counting.
   * @param queryManager comes with functionality to manage the queries
   * @param tcpSourceType points at current TCPSourceType config object, look at
   * same named file for info
  ...
  explicit TCPSource(
      ...
      OperatorId operatorId, OriginId originId, size_t numSourceLocalBuffers,
  ...
  /**
   * @brief search from the back (first inputted item) to the front for the
   * given search token
   * @token to search for
   * @return number of places until first occurrence of token (place of token
   * not included)
   */
  uint64_t sizeUntilSearchToken(char token);

  /**
   * @brief pop number of values given and fill temp with popped values. If
   * popTextDevider true, pop one more value and discard
   * @param numberOfValuesToPop number of values to pop and fill temp with
   * @param popTextDivider if true, pop one more value and discard, if false,
   * only pop given number of values to pop
   * @return true if number of values to pop successfully popped, false
   * otherwise
   */
  bool popGivenNumberOfValues(uint64_t numberOfValuesToPop,
                              bool popTextDivider);
