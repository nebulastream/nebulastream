 ...
 *      1.1) if the source e.g. file is large enough we will read in
 * numBuffersToProcess and then terminate 1.2) if the file is not large enough,
 * we will start at the beginning until we produced numBuffersToProcess 2.) If
 * the user set numBuffersToProcess to 0, we read the source until it ends, e.g,
 * until the file ends 3.) If the user just set numBuffersToProcess to n but
 * does not say how many tuples he wants per buffer, we loop over the source
...
public:
   ...
   * @Note the number of buffers to process is set to UINT64_MAX and the value
   * is needed by some test to produce a deterministic behavior
   ...
   * data is pushed
   * @param sourceAffinity the subsequent operators in the pipeline to which the
   * data is pushed
  ...
  /**
   * @brief This methods initializes thread-local state. For instance, it
   * creates the local buffer pool and is necessary because we cannot do it in
   * the constructor.
   */
  virtual void open();
   ...
   * @dev    I made this function non-virtual. If implementations of this class
   * should be able to override this function, we have to ensure that
   * `isRunning` and this class' private member `running` are consistent or that
   * this class does not evaluate `running` directly when checking if it is
   * running.
   */
   ...
   * @note the source will sleep for interval seconds and then produce the next
   * buffer
  ...
  /**
   * @brief Internal destructor to make sure that the data source is stopped
   * before deconstrcuted
   ...
   */
  void addExecutableSuccessors(
      std::vector<Runtime::Execution::SuccessorExecutablePipeline>
          newPipelines);

  /**
   * @brief This method is necessary to avoid problems with the shared_from_this
   * machinery combined with multi-inheritance
   ...
   * @note Currently has no behaviour. We need to overwrite DataEmitter::onEvent
   * for compliance.
   * @param event
   */
  virtual void onEvent(Runtime::BaseEvent &) override;
  /**
   * @brief API method called upon receiving an event, whose handling requires
   * the WorkerContext (e.g. its network channels).
   * @note Only calls onEvent(event) of this class or derived classes.
   * @param event
   * @param workerContext
   */
  virtual void onEvent(Runtime::BaseEvent &event,
...
protected:
  ...
  uint64_t numberOfBuffersToProduce =
      std::numeric_limits<decltype(numberOfBuffersToProduce)>::max();
  uint64_t numSourceLocalBuffers;
  uint64_t gatheringIngestionRate{};
  std::chrono::milliseconds gatheringInterval{0};
  GatheringMode gatheringMode;
  SourceType type;
  Runtime::QueryTerminationType wasGracefullyStopped{
      Runtime::QueryTerminationType::Graceful}; // protected by mutex
  ...
  void emitWork(Runtime::TupleBuffer &buffer,
                bool addBufferMetaData = true) override;
