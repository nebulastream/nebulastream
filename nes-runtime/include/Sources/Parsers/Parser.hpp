...
class Parser {
 public:
  ...
  /**
   * @brief takes a tuple as string, casts its values to the correct types and
   * writes it to the TupleBuffer
   * @param inputTuple: string value that is cast to the PhysicalType and
  ...
  virtual bool writeInputTupleToTupleBuffer(
      std::string_view inputTuple, uint64_t tupleCount,
      Runtime::MemoryLayouts::TestTupleBuffer& tupleBuffer,
      const SchemaPtr& schema,
      const Runtime::BufferManagerPtr& bufferManager) = 0;

  /**
   * @brief casts a value in string format to the correct type and writes it to
   * the TupleBuffer
   * @param value: string value that is cast to the PhysicalType and written to
   * the TupleBuffer
   ...
   * @param tupleCount: current tuple count, i.e. how many tuples have already
   * been produced
   * @param bufferManager: the buffer manager
   */
  void writeFieldValueToTupleBuffer(
      std::string value, uint64_t schemaFieldIndex,
      Runtime::MemoryLayouts::TestTupleBuffer& tupleBuffer,
