...
/**
 * @brief Benchmark Source is a special source for benchmarking purposes only
 * and stripes away all overhead The memory area out of which buffers will be
 * produced must be initialized beforehand and allocated as a shared_ptr that
 * must have ownership of the area, i.e., it must control when to free it. Do
 * not use in distributed settings but only for single node dev and testing.
 */
class BenchmarkSource : public GeneratorSource, public Runtime::BufferRecycler {
 public:
   ...
   * @param memoryArea the non-null memory area that stores the data that will
   ...
   * @param numberOfBuffersToProcess the number of buffers to be produced by the
   * source
  ...
  explicit BenchmarkSource(
      ...
      GatheringMode gatheringMode, SourceMode sourceMode,
  ...
  /**
   * @brief This method is implemented only to comply with the API: it will
   * crash the system if called.
   * @return a nullopt
   */
  ...
  /**
   * @brief This methods creates the local buffer pool and is necessary because
