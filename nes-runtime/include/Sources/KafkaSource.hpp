...
public:
   ...
   * @param groupId the ID of a logical group which consumes messages from one
   * or more kafka topics
   * @param autoCommit bool indicating if offset has to be committed
   * automatically or not
   * @param kafkaConsumerTimeout the timeperiod after which a timeout is issued
   * @param offsetMode instructs the broker from which point of a topic a
   * consumer wants consume messages from
   ...
   * @param batchSize the maximum amount of data (in bytes) that a Kafka
   * producer can accumulate before sending a batch of messages to the Kafka
   ...
   * @return
   */
  KafkaSource(SchemaPtr schema, Runtime::BufferManagerPtr bufferManager,
              Runtime::QueryManagerPtr queryManager,
              uint64_t numberOfBuffersToProduce, std::string brokers,
              std::string topic, std::string groupId, bool autoCommit,
              uint64_t kafkaConsumerTimeout, std::string offsetMode,
              const KafkaSourceTypePtr &kafkaSourceType, OriginId originId,
              StatisticId statisticId, OperatorId operatorId,
...
private:
   ...
   * check if already connected, if not connect try to connect, if already
   * connected return
