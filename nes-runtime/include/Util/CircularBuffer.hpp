...
template <class, bool> class CircularBufferIterator;
 ...
 */
template <class cbT, bool isConst> class CircularBufferIterator {
public:
  ...
  friend CircularBufferIterator operator-(CircularBufferIterator it,
                                          int i) noexcept {
    it -= i;
    return it;
  }
...
private:
  ...
  CircularBufferIterator(
      uint64_type idx, std::conditional_t<isConst, const cbT, cbT> *rv) noexcept
...
public:
  ...
  explicit CircularBuffer(uint64_t size)
      : maxSize(size), buffer(std::make_unique<T[]>(size)){};
  ...
  void
  push(const T &value) noexcept(std::is_nothrow_copy_assignable<T>::value) {
    if (full()) {
      decrementHead();
    } else {
  ...
  template <bool b = true,
            typename = std::enable_if_t<b && std::is_move_assignable<T>::value>>
  void push(T values[],
            size_t size) noexcept(std::is_nothrow_move_assignable<T>::value) {
...
private:
  ...
  reference back_() noexcept {
    return buffer[(head + currentSize - 1) % maxSize];
  }
  [[nodiscard]] const_reference back_() const noexcept {
    return buffer[(head + currentSize - 1) % maxSize];
  }
  ...
  void incrementHeadByGivenSize(size_t increment) noexcept {
    head = (head + maxSize - increment) % maxSize;
  }
  void decrementHeadByGivenSize(size_t decrement) noexcept {
    head = (head + maxSize - decrement) % maxSize;
  }
