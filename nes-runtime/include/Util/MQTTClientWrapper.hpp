...
namespace NES {
/**
 * @brief Helper class for MQTTSink. Provides a convenient way to choose between
...
class MQTTClientWrapper {
 public:
   ...
   * @param useAsyncClient: if true: use asynchronous client, else: use
   * synchronous client
   * @param address: address name of MQTT broker
   * @param clientId: client ID for MQTT client
   * @param maxBufferedMSGs: maximal number of messages that can be buffered by
   * the client before disconnecting
   * @return MQTTClientWrapper
   */
  MQTTClientWrapper(bool useAsyncClient, std::string const& address,
                    std::string const& clientId, uint64_t maxBufferedMSGs,
                    std::string const& topic, int qualityOfService);

  /**
   * @brief get a pointer to the MQTT asynchronous client ONLY IF
   * 'useAsyncClient' = true
   */
  mqtt::async_client_ptr getAsyncClient();

  /**
   * @brief get a pointer to the MQTT synchronous client ONLY IF
  ...
  /**
   * @brief get the number of elements currently residing in the buffer
   * (messages that have not been delivered yet)
   */
  uint64_t getNumberOfUnsentMessages();

  /**
   * @brief Helper class for synchronous client - Defines how to interact with
   * broker replies and connection loss
   */
