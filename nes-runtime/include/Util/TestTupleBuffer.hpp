 ...
 * For all field accesses we check that the template type is the same as the
 * selected physical field type. If the type is not compatible accesses result
 * in a BufferAccessException.
 */
class DynamicField {
public:
  /**
   ...
   * @throws BufferAccessException if the passed Type is not the same as the
   * physicalType of the field.
   * @return Pointer type
   */
  template <class Type>
  ...
  inline Type read() const {
    if (!PhysicalTypes::isSamePhysicalType<Type>(physicalType)) {
      throw BufferAccessException("Wrong field type passed. Field is of type " +
  ...
  /**
   * @brief Reads a field with a value Type. Checks if the passed Type is the
   * same as the physical field type.
  ...
  inline Type read() const {
    if (!PhysicalTypes::isSamePhysicalType<typename Type::Underlying>(
            physicalType)) {
      throw BufferAccessException("Wrong field type passed. Field is of type " +
                                  physicalType->toString() +
                                  " but accessed as " + typeid(Type).name());
    }
    return Type(*reinterpret_cast<typename Type::Underlying *>(
        const_cast<uint8_t *>(address)));
  };

  /**
   * @brief Writes a value to a specific field address.
   * @tparam Type of the field. Type has to be a NesType and to be compatible
   * with the physical type of this field.
  ...
  inline void write(Type value) {
    ...
    *reinterpret_cast<typename Type::Underlying *>(
        const_cast<uint8_t *>(address)) = value.getRawValue();
  };
...
public:
   ...
   * Each tuple contains the index, to the memory layout and to the tuple
   * buffer.
   * @param tupleIndex
   * @param memoryLayout
   * @param buffer
   */
  DynamicTuple(uint64_t tupleIndex, MemoryLayoutPtr memoryLayout,
               TupleBuffer buffer);
  /**
   ...
   * @param bufferManager
   */
  void writeVarSized(std::variant<const uint64_t, const std::string> field,
                     std::string value, BufferManager *bufferManager);
   ...
   */
  std::string
  readVarSized(std::variant<const uint64_t, const std::string> field);
  ...
  /**
   * @brief Compares if the values of both tuples are equal. This means that the
...
/**
 * @brief The TestTupleBuffers allows to read records and individual fields from
 * an tuple buffer. To this end, it assumes a specific data layout, i.e.,
 * RowLayout or ColumnLayout. This allows for dynamic accesses to a tuple buffer
 * in the sense that at compile-time a user has not to specify a specific memory
 * layout. Therefore, the memory layout can be a runtime option, whereby the
 * code that operates on the tuple buffer stays the same. Furthermore, the
 * TestTupleBuffers trades-off performance for safety. To this end, it checks
 * field bounds and field types and throws BufferAccessException if the passed
 * parameters would lead to invalid buffer accesses. The TestTupleBuffers
 ...
 *    ```
 *
 * @caution This class is non-thread safe, i.e. multiple threads can manipulate
 * the same tuple buffer at the same time.
 * @caution Do NOT use this class in performance critical code, as it is
 * designed for testing and not for performance.
 */
...
public:
   ...
   */
  explicit TestTupleBuffer(const MemoryLayoutPtr &memoryLayout,
                           TupleBuffer buffer);
   ...
   */
  static TestTupleBuffer createTestTupleBuffer(Runtime::TupleBuffer buffer,
  ...
  /**
   * @brief Gets the current number of tuples that are currently stored in the
   * underling tuple buffer
   ...
   * Take into account that it is invalid to add tuples to the tuple buffer
  ...
  /**
   * @brief Push a record to the underlying tuple buffer. Simply appends record
   to the end of the buffer. Boundary checks are performed by the write function
   of the TestTupleBuffer.
   * @note Recursive templates have a limited depth. The recommended (C++
   standard) depth is 1024.
   ...
   * @brief Push a record to the underlying tuple buffer. Simply appends record
   ...
   of the TestTupleBuffer.
   * @note  Recursive templates have a limited depth. The recommended (C++
   standard) depth is 1024. Thus, a record with more than 1024 fields might not
   be supported.
   * @param record: The record to be pushed to the buffer.
   * @param bufferManager: BufferManager required for storing the variable sized
   data in the child buffers
   */
  template <typename... Types>
    requires(ContainsString<Types> || ...)
  void pushRecordToBuffer(std::tuple<Types...> record,
  ...
  /**
   * @brief Push a record to the underlying tuple buffer at given recordIndex.
   ...
   *
   * @param record: The record to be pushed to the buffer.
   * @param recordIndex: The index at which the record should be pushed to the
   buffer.
   * @return true if the record was pushed successfully, false otherwise.
   */
  template <typename... Types>
  void pushRecordToBufferAtIndex(std::tuple<Types...> record,
    ...
    if (recordIndex >= buffer.getBufferSize()) {
      throw BufferAccessException(
          "Current buffer is not big enough for index. Current buffer size: " +
          std::to_string(buffer.getBufferSize()) +
          ", Index: " + std::to_string(recordIndex));
    }
    // std::apply allows us to iterate over a tuple (with template recursion)
    // with a lambda function. On each iteration, the lambda function is called
    // with the current field value, and the field index is increased. If the
    // value is a std::string, we call writeVarSized() instead of write().
    std::apply(
        [&](auto &&...fieldValue) {
          (([&]() {
             if constexpr (IsString<decltype(fieldValue)>) {
               NES_ASSERT(bufferManager != nullptr,
                          "BufferManager can not be null while using variable "
                          "sized data!");
               (*this)[recordIndex].writeVarSized(fieldIndex++, fieldValue,
                                                  bufferManager);
             } else {
  ...
  /**
   * @brief Copy a record from the underlying tuple buffer to a tuple. Boundary
   checks are performed by the read function of the TestTupleBuffer.
   *
   ...
   * @return true if the record was read from the TupleBuffer successfully,
   false otherwise.
   */
  template <typename... Types>
  std::tuple<Types...> readRecordFromBuffer(uint64_t recordIndex) {
    NES_ASSERT(
        ...
        "Provided tuple types: "
            << sizeof...(Types)
            << " do not match the number of fields in the memory layout: "
...
private:
  /**
   * @brief Takes a tuple as a reference and a recordIndex. Copies the record in
   the TupleBuffer at the given recordIndex to the tuple.
   * @note Recursive templates have a limited depth. The recommended (C++
   standard) depth is 1024.
   *       Thus, a record with more than 1024 fields might not be supported.
   *
  ...
  void copyRecordFromBufferToTuple(std::tuple<Types...> &record,
                                   uint64_t recordIndex) {
    // Check if I matches the size of the tuple, which means that all fields of
    // the record have been processed.
    if constexpr (I != sizeof...(Types)) {
      if constexpr (IsString<typename std::tuple_element<
                        I, std::tuple<Types...>>::type>) {
        auto childBufferIdx =
            (*this)[recordIndex][I].read<TupleBuffer::NestedTupleBufferKey>();
        std::get<I>(record) = readVarSizedData(this->buffer, childBufferIdx);
      } else {
        // Get type of current tuple element and cast field value to this type.
        // Add value to return tuple.
        std::get<I>(record) = ((*this)[recordIndex][I])
                                  .read<typename std::tuple_element<
                                      I, std::tuple<Types...>>::type>();
      }

      // Recursive call to copyRecordFromBufferToTuple with the field index (I)
      // increased by 1.
