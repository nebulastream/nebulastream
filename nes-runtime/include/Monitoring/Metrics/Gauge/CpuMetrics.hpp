 ...
 * %steal – % CPU usage being forced to wait for a hypervisor handling other
 * virtual processors %guest – % CPU usage spent running a virtual processor
 * %idle – % CPU usage on idle time (no processes, and not waiting on a disk
 * read/write)
 */
class CpuMetrics {
 public:
 ...
 * @param the tuple index indicating the location of the tuple
 */
void writeToBuffer(const CpuMetrics& metrics, Runtime::TupleBuffer& buf,
                   uint64_t tupleIndex);
 ...
 * @return The object
 */
void readFromBuffer(CpuMetrics& metrics, Runtime::TupleBuffer& buf,
                    uint64_t tupleIndex);
