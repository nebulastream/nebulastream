...
/**
 * @brief DiskMetrics class, that is responsible for collecting and managing
 * disk metrics.
 */
class DiskMetrics {
 public:
 ...
 * @param the tuple index indicating the location of the tuple
 */
void writeToBuffer(const DiskMetrics& metrics, Runtime::TupleBuffer& buf,
                   uint64_t tupleIndex);
 ...
 * @param the tuple index indicating the location of the tuple
 */
void readFromBuffer(DiskMetrics& metrics, Runtime::TupleBuffer& buf,
                    uint64_t tupleIndex);
