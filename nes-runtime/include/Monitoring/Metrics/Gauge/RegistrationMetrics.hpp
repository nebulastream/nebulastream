...
using SerializableRegistrationMetricsPtr =
    std::shared_ptr<SerializableRegistrationMetrics>;
...
/**
 * Class representing the static metrics that are transmitted during node
 * registration in NES. totalMemoryBytes; the total available memory on the node
 * in bytes cpuCoreNum; number of cores totalCPUJiffies; user+idle+system (Note:
 * This value can change everytime it is read via AbstractSystemResourcesReader)
 * Using 1.5 CPUs is equivalent to --cpu-period="100000" and
 * --cpu-quota="150000" cpuPeriodUS; the CPU CFS scheduler period in
 * microseconds cpuQuotaUS; CPU CFS quota in microseconds isMoving; flag to
 * indicate if the node is changing geo-location hasBattery; flag to indicate if
 * the node is running on a battery
 */
...
public:
   ...
   * @return the schema
   */
  static Configurations::SchemaTypePtr
  getSchemaType(const std::string &prefix = "");
  ...
  /**
   * @brief Converts the object into a grpc protobuf object that can be
   * serialized.
  ...
  uint64_t cpuCoreNum;
  uint64_t totalCPUJiffies; // user+idle+system (This value can change everytime
                            // it is read via AbstractSystemResourcesReader)
  // Using 1.5 CPUs is equivalent to --cpu-period="100000" and
  // --cpu-quota="150000"
...
/**
 * @brief The serialize method to write metrics into the given Schema and
 * TupleBuffer.
 ...
 */
void writeToBuffer(const RegistrationMetrics &metrics,
                   Runtime::TupleBuffer &buf, uint64_t tupleIndex);
 ...
 * @return The object
 */
void readFromBuffer(RegistrationMetrics &metrics, Runtime::TupleBuffer &buf,
                    uint64_t tupleIndex);
