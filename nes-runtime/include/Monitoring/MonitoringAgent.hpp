...
/**
 * @brief The MonitoringAgent which is responsible for collecting metrics on a
 * local level.
 */
class MonitoringAgent {
public:
  ...
  MonitoringAgent(MonitoringPlanPtr monitoringPlan,
                  MonitoringCatalogPtr catalog, bool enabled);

  static MonitoringAgentPtr create();
  static MonitoringAgentPtr create(bool enabled);
  static MonitoringAgentPtr create(MonitoringPlanPtr monitoringPlan,
                                   MonitoringCatalogPtr catalog, bool enabled);

  /**
   * @brief Collect the metrics and store them in to the given output. The
   * collected metrics depend on the monitoring plan. If monitoring is disabled
  ...
  /**
   * @brief Adds the monitoring streams based on the monitoring plan to the
   * worker config
   * @param workerConfig
   * @return true if success, else false
   */
  bool addMonitoringStreams(
