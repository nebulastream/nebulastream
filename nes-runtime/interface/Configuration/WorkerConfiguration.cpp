...
{
    return {
        .queryEngine = QueryEngineConfiguration::fromConfig(config),
        .defaultQueryExecution = QueryExecutionConfiguration::fromConfig(config),
        .network = WorkerNetworkConfiguration::fromConfig(config),
        .totalMemoryInBytes = config.get(TOTAL_MEMORY_IN_BYTES),
        .unpooledMemoryFraction = config.get(UNPOOLED_MEMORY_FRACTION),
        .bufferAlignmentInBytes = config.get(BUFFER_ALIGNMENT_IN_BYTES),
        .defaultMaxInflightBuffers = config.get(DEFAULT_MAX_INFLIGHT_BUFFERS),
        .dumpQueryCompilationIR = config.get(DUMP_COMPILATION_RESULT),
        .dumpGraph = config.get(DUMP_GRAPH)};
}
}
