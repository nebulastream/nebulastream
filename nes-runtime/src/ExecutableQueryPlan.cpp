    ...
    CompiledQueryPlan& compiledQueryPlan, const std::shared_ptr<AbstractPoolProvider>& poolProvider, int numberOfBuffersInSourceLocalPools)
{
    ...
    {
        auto sink = ExecutablePipeline::create(pipelineId, lower(valve, descriptor), {});
    ...
    for (auto [originId, operatorId, descriptor, successors] : compiledQueryPlan.sources)
    {
        ...
        instantiatedSources.emplace_back(
            SourceProvider::lower(originId, ingestion, descriptor, poolProvider, numberOfBuffersInSourceLocalPools), std::move(successors));
    }
