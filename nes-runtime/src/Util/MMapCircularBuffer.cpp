...
bool smokeTest(char* data, size_t capacity_) {
...
MMapCircularBuffer::MMapCircularBuffer(size_t capacity) : capacity_(capacity) {
     ...
     * mapped into the processes address space twice. That way there exists now two disting memory addresses that refer to the
...
MMapCircularBuffer::reader MMapCircularBuffer::read() {
    ...
    const auto memory = std::span(data + read_idx, size());
...
MMapCircularBuffer::writer MMapCircularBuffer::write() {
    ...
    const auto memory = std::span(data + write_idx, capacity_ - size());
    NES_TRACE("Writer for {} bytes. {} - {}", memory.size(), fmt::ptr(memory.data()), fmt::ptr(memory.data() + memory.size()));
    acive_write = true;
    return writer(memory, *this);
}

void MMapCircularBuffer::confirmWrite(std::span<char>, size_t used_bytes) {
    acive_write = false;
    write_idx += used_bytes;
    NES_TRACE("Write: Advancing {} bytes. WriteIdx: {}", used_bytes, write_idx);
}

void MMapCircularBuffer::confirmRead(std::span<const char>, size_t used_bytes) {
    acive_read = false;
    read_idx += used_bytes;
