...
/**
 * Quick check to verify that the mmapped region of memory has the desired
 * effect.
 */
static bool smokeTest(char* data, size_t capacity_) {
  ...
  test_ptr[3] = 'D';
  test_ptr[4] = '\0';
  return strncmp(((const char*)test_ptr), ((const char*)test_ptr + capacity_),
                 4) == 0;
}
...
MMapCircularBuffer::MMapCircularBuffer(size_t capacity) : capacity_(capacity) {
  NES_ASSERT(capacity_ % getpagesize() == 0,
             "Cirular Buffer requires a capacity that is divisible by the "
             "getpagesize()");
  size_t err;

  /**
   * We create a anomynous memory segement, by creating an inmemory File,
   * resizing it to the capacity. The memory segment is mapped into the
   * processes address space twice. That way there exists now two distinct
   * memory addresses that refer to the same memory. A write to address 0 will
   * be equivalent to a write to address capacity. This allows to create the
   * illusion of a ring in memory. Reads and writes that would previously need
   * to account for a wrap around can now be contigous. This illusion is
   * maintained by wrapping the read and write index during reads.
   */

#ifdef __APPLE__
  // MacOS does not have the memfd_create syscall to create an anonymous memory
  // region. Workaround is to create a file and immediatly unlink (delete) it.
  {
    std::scoped_lock sl(anonymousFileLock);
    fd = open("queue_buffer", O_CREAT | O_RDWR, 0600);
    if (fd > 0) {
      NES_ASSERT(unlink("queue_buffer") == 0,
                 "Could not unlink the anonymous file");
    }
  }
#else
  ...
  data = static_cast<char*>(
      mmap(NULL, 2 * capacity_, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
  if (data == MMAP_FAIL) {
    err = errno;
    goto fd;
  }

  if (mmap(data, capacity_, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fd,
           0) == MMAP_FAIL) {
    err = errno;
    goto mmap_1;
  }

  if (mmap(data + capacity_, capacity_, PROT_READ | PROT_WRITE,
           MAP_SHARED | MAP_FIXED, fd, 0) == MMAP_FAIL) {
    err = errno;
    goto mmap_1;
  }
...
fd:
  close(fd);
exit_with_exception:
  NES_THROW_RUNTIME_ERROR(fmt::format("Could not create MMapCircularBuffer: {}",
                                      std::strerror(err)));
}
...
MMapCircularBuffer::reader MMapCircularBuffer::read() {
  ...
  NES_TRACE("Reader for {} bytes. {} - {}", memory.size(),
            fmt::ptr(memory.data()), fmt::ptr(memory.data() + memory.size()));
  acive_read = true;
...
MMapCircularBuffer::writer MMapCircularBuffer::write() {
  ...
  NES_TRACE("Writer for {} bytes. {} - {}", memory.size(),
            fmt::ptr(memory.data()), fmt::ptr(memory.data() + memory.size()));
  acive_write = true;
