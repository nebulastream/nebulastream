        ...
        [this, index](const auto& key) {
          if constexpr (std::is_convertible_v<std::decay_t<decltype(key)>,
            ...
            NES_THROW_RUNTIME_ERROR(
                "We expect either a uint64_t or a std::string to access a "
                "DynamicField!");
          }
        },
        field);
...
const TestTupleBuffer::TupleIterator TestTupleBuffer::TupleIterator::operator++(
    int) {
...
TestTupleBuffer TestTupleBuffer::createTestTupleBuffer(
    Runtime::TupleBuffer buffer, const SchemaPtr& schema) {
