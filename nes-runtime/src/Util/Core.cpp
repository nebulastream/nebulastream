...
std::string Util::printTupleBufferAsCSV(Runtime::TupleBuffer tbuffer,
  ...
  for (uint64_t i = 0; i < numberOfTuples; i++) {
    ...
    for (uint64_t j = 0; j < schema->getSize(); j++) {
      ...
      if (dataType->isText()) {
        NES_DEBUG(
            "Util::printTupleBufferAsCSV(): trying to read the variable length "
            "TEXT field: "
...
Runtime::MemoryLayouts::MemoryLayoutPtr Util::createMemoryLayout(
    SchemaPtr schema, uint64_t bufferSize) {
...
bool Util::assignPropertiesToQueryOperators(
  ...
  if (numOperators != properties.size()) {
    NES_ERROR(
        "UtilityFunctions::assignPropertiesToQueryOperators: the number of "
        "properties does not match the number of "
...
std::vector<Runtime::TupleBuffer> Util::createBuffersFromCSVFile(
    ...
    const std::string& timeStampFieldName, uint64_t lastTimeStamp) {
  ...
  auto parser = std::make_shared<CSVParser>(
      schema->fields.size(), getPhysicalTypes(schema), delimiter);
  ...
  do {
    ...
    auto testTupleBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(buffer,
                                                                       schema);
    parser->writeInputTupleToTupleBuffer(line, tupleCount, testTupleBuffer,
                                         schema, bufferManager);
    ++tupleCount;

    // If we have read enough tuples from the csv file, then stop iterating over
    // it
    auto curTimeStamp =
        testTupleBuffer[tupleCount - 1][timeStampFieldName].read<uint64_t>();
...
// If NES is build with NES_ENABLES_TESTS the linker is instructed to wrap the
// read function to keep the usual functionality __wrap_read just calls
// __real_read which is the real read function. However, this allows to mock
// calls to read (e.g. TCPSourceTest)
extern "C" ssize_t __real_read(int fd, void* data, size_t size);
__attribute__((weak)) extern "C" ssize_t __wrap_read(int fd, void* data,
                                                     size_t size) {
