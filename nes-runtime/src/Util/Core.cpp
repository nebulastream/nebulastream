...
std::string Util::printTupleBufferAsCSV(Runtime::TupleBuffer tbuffer,
  ...
  for (uint64_t i = 0; i < numberOfTuples; i++) {
    ...
    for (uint64_t j = 0; j < schema->getSize(); j++) {
      ...
      if (dataType->isText()) {
        NES_DEBUG("Util::printTupleBufferAsCSV(): trying to read the variable "
                  "length TEXT field: "
...
Util::createMemoryLayout(SchemaPtr schema, uint64_t bufferSize) {
...
bool Util::assignPropertiesToQueryOperators(
  ...
  if (numOperators != properties.size()) {
    NES_ERROR("UtilityFunctions::assignPropertiesToQueryOperators: the number "
              "of properties does not match the number of "
