...
StatisticFormatPtr StatisticFormatFactory::createFromSchema(
    SchemaPtr schema, uint64_t bufferSize, StatisticSynopsisType type, StatisticDataCodec sinkDataCodec)
{
    ...
    switch (type)
    {
        case StatisticSynopsisType::COUNT_MIN:
            return createCountMinFormat(memoryLayout, postProcessingData, preProcessingData);
        case StatisticSynopsisType::HLL:
            return createHyperLogLogFormat(memoryLayout, postProcessingData, preProcessingData);
    }
}
