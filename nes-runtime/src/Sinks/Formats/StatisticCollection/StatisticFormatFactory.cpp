...
StatisticFormatPtr StatisticFormatFactory::createFromSchema(
    SchemaPtr schema, uint64_t bufferSize, StatisticSynopsisType type,
  ...
  switch (schema->getLayoutType()) {
    case Schema::MemoryLayoutType::ROW_LAYOUT: {
      memoryLayout =
          Runtime::MemoryLayouts::RowLayout::create(schema, bufferSize);
      break;
    }
    case Schema::MemoryLayoutType::COLUMNAR_LAYOUT: {
      memoryLayout =
          Runtime::MemoryLayouts::ColumnLayout::create(schema, bufferSize);
      break;
    }
    default:
      NES_NOT_IMPLEMENTED();
  ...
  // 2. We decide how the post- and preprocessing functions should be, i.e., do
  // we perform some compression for example
  ...
  switch (type) {
    case StatisticSynopsisType::COUNT_MIN:
      return createCountMinFormat(memoryLayout, postProcessingData,
                                  preProcessingData);
    case StatisticSynopsisType::HLL:
      return createHyperLogLogFormat(memoryLayout, postProcessingData,
                                     preProcessingData);
  }
}

StatisticFormatPtr StatisticFormatFactory::createCountMinFormat(
    const Runtime::MemoryLayouts::MemoryLayoutPtr& memoryLayout,
    std::function<std::string(const std::string&)> postProcessingData,
    std::function<std::string(const std::string&)> preProcessingData) {
  return CountMinStatisticFormat::create(memoryLayout, postProcessingData,
                                         preProcessingData);
}

StatisticFormatPtr StatisticFormatFactory::createHyperLogLogFormat(
    const Runtime::MemoryLayouts::MemoryLayoutPtr& memoryLayout,
    std::function<std::string(const std::string&)> postProcessingData,
    std::function<std::string(const std::string&)> preProcessingData) {
  return HyperLogLogStatisticFormat::create(memoryLayout, postProcessingData,
                                            preProcessingData);
}
