...
StatisticFormatPtr StatisticFormatFactory::createFromSchema(SchemaPtr schema,
    ...
    switch (type) {
        ...
        case StatisticSynopsisType::EQUI_WIDTH_HISTOGRAM: return createEquiWidthSampleFormat(memoryLayout, postProcessingData, preProcessingData);
...
StatisticFormatPtr
StatisticFormatFactory::createEquiWidthSampleFormat(const Runtime::MemoryLayouts::MemoryLayoutPtr& memoryLayout,
                                                    std::function<std::string(const std::string&)> postProcessingData,
                                                    std::function<std::string(const std::string&)> preProcessingData) {
    return EquiWidthHistogramStatisticFormat::create(memoryLayout, postProcessingData, preProcessingData);
}
