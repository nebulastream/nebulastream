...
NesFormat::NesFormat(SchemaPtr schema) : SinkFormat(std::move(schema))
{
    serializedSchema = std::make_shared<SerializableSchema>();
}

std::string NesFormat::getFormattedBuffer(const Memory::TupleBuffer& inputBuffer, std::shared_ptr<Memory::AbstractBufferProvider>)
{
