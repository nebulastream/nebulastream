    ...
    , connected(false)
{
    ...
    client = std::make_shared<MQTTClientWrapper>(
        asynchronousClient, address, clientId, maxBufferedMessages, topic, magic_enum::enum_integer(qualityOfService));
...
std::string MQTTSink::getAddress() const
{
    return address;
}
std::string MQTTSink::getClientId() const
{
    return clientId;
}
std::string MQTTSink::getTopic() const
{
    return topic;
}
std::string MQTTSink::getUser() const
{
    return user;
}
uint64_t MQTTSink::getMaxBufferedMessages() const
{
    return maxBufferedMessages;
}
MQTTSinkDescriptor::TimeUnits MQTTSink::getTimeUnit() const
{
    return timeUnit;
}
uint64_t MQTTSink::getMsgDelay() const
{
    return messageDelay;
}
...
bool MQTTSink::getAsynchronousClient() const
{
