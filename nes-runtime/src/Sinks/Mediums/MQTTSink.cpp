...
namespace NES {
/*
 the user can specify the time unit for the delay and the duration of the delay
 in that time unit in order to avoid type switching types (different time units
 require different duration types), the user input for the duration is treated
 as nanoseconds and then multiplied to 'convert' to milliseconds or seconds
 accordingly
*/
const uint32_t NANO_TO_MILLI_SECONDS_MULTIPLIER = 1000000;
const uint32_t NANO_TO_SECONDS_MULTIPLIER = 1000000000;

SinkMediumTypes MQTTSink::getSinkMediumType() {
  return SinkMediumTypes::MQTT_SINK;
}

MQTTSink::MQTTSink(SinkFormatPtr sinkFormat, Runtime::NodeEnginePtr nodeEngine,
    ...
    : SinkMedium(std::move(sinkFormat), nodeEngine, numOfProducers,
                 sharedQueryId, decomposedQueryPlanId, numberOfOrigins),
      address(address), clientId(clientId), topic(topic), user(user),
      maxBufferedMSGs(maxBufferedMSGs), timeUnit(timeUnit),
      messageDelay(messageDelay), qualityOfService(qualityOfService),
  ...
  minDelayBetweenSends = std::chrono::nanoseconds(
      ...
      ((timeUnit == MQTTSinkDescriptor::TimeUnits::milliseconds)
           ...
           : (NANO_TO_SECONDS_MULTIPLIER *
                  (timeUnit != MQTTSinkDescriptor::TimeUnits::nanoseconds) |
              (timeUnit == MQTTSinkDescriptor::TimeUnits::nanoseconds))));

  client = std::make_shared<MQTTClientWrapper>(
      asynchronousClient, address, clientId, maxBufferedMSGs, topic,
      magic_enum::enum_integer(qualityOfService));
  NES_TRACE("MQTTSink::MQTTSink {}: Init MQTT Sink to {}", this->toString(),
            address);
}
...
MQTTSink::~MQTTSink() NES_NOEXCEPT(false) {
  ...
  if (success) {
    ...
    NES_ASSERT2_FMT(
        ...
        "MQTTSink::~MQTTSink "
            << this
            << ": Destroy MQTT Sink failed cause it could not be disconnected");
  }
}

bool MQTTSink::writeData(Runtime::TupleBuffer &inputBuffer,
                         Runtime::WorkerContextRef) {
  std::unique_lock lock(writeMutex);
  NES_ASSERT(connected, "MQTTSink::writeData: cannot write buffer because "
  ...
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      sinkFormat->getSchemaPtr(), inputBuffer.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, inputBuffer);
  NES_TRACE("MQTTSink::writeData {}",
            buffer.toString(sinkFormat->getSchemaPtr()));

  try {
    // Main share work performed here. The input TupleBuffer is iterated over
    // and each tuple is converted to a json string and afterward sent to an
    // MQTT broker, via the MQTT client
    for (auto formattedTuple : sinkFormat->getTupleIterator(inputBuffer)) {
      if (formattedTuple == "") {
        NES_ERROR(
            "MQTTSink:: Error during tuple creation from tuple buffer:  {}",
            formattedTuple);
        continue;
      }
    ...
    // When the client is asynchronous it can happen that the client's buffer is
    // large enough to buffer all messages that were not successfully sent to an
    // MQTT broker.
    if ((asynchronousClient && client->getNumberOfUnsentMessages() > 0)) {
      NES_ERROR("MQTTSink::writeData: {} messages could not be sent",
                client->getNumberOfUnsentMessages());
      return false;
    }
  } catch (const mqtt::exception &ex) {
    NES_ERROR("MQTTSink::writeData: Error during writeData in MQTT sink: {}",
              ex.what());
    return false;
  }
  return true;
...
std::string MQTTSink::toString() const {
  ...
  ss << "SEND_DURATION_IN_NS=" << std::to_string(minDelayBetweenSends.count())
     << ", ";
  ss << "QUALITY_OF_SERVICE="
     << std::string(magic_enum::enum_name(qualityOfService)) << ", ";
  ss << "CLIENT_TYPE="
     << ((asynchronousClient) ? "ASYMMETRIC_CLIENT" : "SYMMETRIC_CLIENT");
  ss << ")";
  return ss.str();
}
...
MQTTSinkDescriptor::ServiceQualities MQTTSink::getQualityOfService() const {
