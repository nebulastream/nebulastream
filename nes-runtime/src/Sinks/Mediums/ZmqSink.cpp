...
SinkMediumTypes ZmqSink::getSinkMediumType() {
  return SinkMediumTypes::ZMQ_SINK;
}

ZmqSink::ZmqSink(SinkFormatPtr format, Runtime::NodeEnginePtr nodeEngine,
                 uint32_t numOfProducers, const std::string &host,
                 uint16_t port, bool internal, SharedQueryId sharedQueryId,
    ...
    : SinkMedium(std::move(format), std::move(nodeEngine), numOfProducers,
      ...
      host(host.substr(0, host.find(':'))), port(port), internal(internal),
...
ZmqSink::~ZmqSink() {
  ...
  if (success) {
    ...
    /// XXX:
    NES_ERROR(
        "ZmqSink: Destroy ZMQ Sink failed cause it could not be disconnected");
    NES_ASSERT2_FMT(false, "ZMQ Sink destruction failed");
  }
  NES_DEBUG("ZmqSink: Destroy ZMQ Sink");
}

bool ZmqSink::writeData(Runtime::TupleBuffer &inputBuffer,
  ...
  if (!schemaWritten && !internal) { // TODO:atomic
    ...
    if (!fSchema.empty()) {
      ...
      try {
        // Send Header
        std::array<uint64_t, 2> const envelopeData{fSchema.size(),
        ...
        if (auto const sentEnvelopeSize =
                socket.send(envelope, zmq::send_flags::sndmore).value_or(0);
        ...
        if (auto const sentPayloadSize =
                socket.send(payload, zmq::send_flags::none).value_or(0);
  ...
  NES_DEBUG(
      "ZmqSink: writes buffer with tupleCnt ={} watermark={} content=\n{}",
      inputBuffer.getNumberOfTuples(), inputBuffer.getWatermark(), buffer);
  try {
    ++sentBuffer;

    // Create envelope
    std::array<uint64_t, 2> const envelopeData{inputBuffer.getNumberOfTuples(),
                                               inputBuffer.getWatermark()};
