...
OPCSink::OPCSink(SinkFormatPtr format, Runtime::NodeEnginePtr nodeEngine,
                 const std::string &url, UA_NodeId nodeId, std::string user,
                 std::string password, QueryId queryId,
                 QuerySubPlanId querySubPlanId)
    : SinkMedium(std::move(format), nodeEngine, 1, queryId, querySubPlanId),
      connected(false), url(url), nodeId(nodeId),
      user(std::move(std::move(user))), password(std::move(password)),
...
OPCSink::~OPCSink() {
  ...
  if (success) {
    ...
    NES_ASSERT2_FMT(false, "OPCSink  " << this
                                       << ": Destroy OPC Sink failed because "
                                          "it could not be disconnected");
  }
  NES_DEBUG("OPCSink   {} : Destroy OPC Sink", this->toString());
}

bool OPCSink::writeData(Runtime::TupleBuffer &inputBuffer,
  ...
  if (connect()) {
    ...
    /* Read current value of attribute, also necessary to get the type
     * information of the node */
    auto *val = new UA_Variant;
    retval = UA_Client_readValueAttribute(client, nodeId, val);
    if (retval == UA_STATUSCODE_GOOD && UA_Variant_isScalar(val)) {
      NES_DEBUG("OPCSINK::writeData: Node exists, successfully obtained "
                "information about current node. ");
      /* Write node attribute with scalar value*/
      retval =
          UA_Variant_setScalarCopy(val, inputBuffer.getBuffer(), val->type);
    } else if (retval == UA_STATUSCODE_GOOD &&
               UA_Variant_hasArrayType(val, val->type)) {
      NES_DEBUG("OPCSINK::writeData: Node exists, successfully obtained "
                "information about current node. ");
      /* Write node attribute with array value*/
      retval = UA_Variant_setArrayCopy(val, inputBuffer.getBuffer(),
                                       inputBuffer.getBufferSize(), val->type);
    } else {
    ...
    if (retval != UA_STATUSCODE_GOOD) {
      NES_ERROR("OPCSINK::writeData: Format of value in input buffer and "
                "format of nodeid do not match.");
      return false;
    }
    ...
    if (retval == UA_STATUSCODE_GOOD && UA_Variant_isScalar(val)) {
      NES_DEBUG("OPCSINK::writeData: New value is:  {}",
                *(UA_Int32 *)var->data);
    } else {
      NES_ERROR("OPCSINK::writeData: Node does not exist or is not a scalar.");
      return false;
...
std::string OPCSink::toString() const {
  char *ident =
      (char *)UA_malloc(sizeof(char) * nodeId.identifier.string.length + 1);
...
bool OPCSink::connect() {
  ...
  if (!connected) {
    ...
    NES_DEBUG("OPCSINK::connect(): was !conncect now connect  {} : connected",
              this->toString());
    ...
    if (retval != UA_STATUSCODE_GOOD) {
      ...
      connected = false;
      NES_ERROR("OPCSINK::connect(): ERROR with Status Code: {} OPCSINK {}: "
                "set connected false",
  ...
  if (connected) {
    ...
    NES_DEBUG("Exception: OPCSINK::connect():   {} : NOT connected",
              this->toString());
  }
  return connected;
}
...
bool OPCSink::disconnect() {
  ...
  if (!connected) {
    ...
    NES_DEBUG("OPCSINK::disconnect()   {} : NOT disconnected",
              this->toString());
  }
  return !connected;
}
...
SinkMediumTypes OPCSink::getSinkMediumType() {
  return SinkMediumTypes::OPC_SINK;
}
