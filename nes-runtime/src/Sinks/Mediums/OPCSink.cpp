...
bool OPCSink::writeData(Runtime::TupleBuffer& inputBuffer,
  ...
  if (connect()) {
    ...
    if (retval == UA_STATUSCODE_GOOD && UA_Variant_isScalar(val)) {
      NES_DEBUG(
          "OPCSINK::writeData: Node exists, successfully obtained information "
          "about current node. ");
    ...
    if (retval != UA_STATUSCODE_GOOD) {
      NES_ERROR(
          "OPCSINK::writeData: Format of value in input buffer and format of "
          "nodeid do not match.");
      return false;
    }
...
bool OPCSink::connect() {
  ...
  if (!connected) {
    ...
    if (retval != UA_STATUSCODE_GOOD) {
      ...
      connected = false;
      NES_ERROR(
          "OPCSINK::connect(): ERROR with Status Code: {} OPCSINK {}: set "
          "connected false",
