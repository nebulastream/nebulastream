...
FileSink::FileSink(
    ...
    : SinkMedium(std::move(format), numOfProducers, queryId, numberOfOrigins), filePath(filePath), append(append)
{
}
...
uint32_t FileSink::setup(Runtime::Execution::PipelineExecutionContext&)
{
    NES_DEBUG(
        "Setting up file sink; filePath={}, schema={}, sinkFormat={}, append={}",
        filePath,
...
void FileSink::execute(const Memory::TupleBuffer& inputTupleBuffer, Runtime::Execution::PipelineExecutionContext& pec)
{
    ...
    {
        NES_DEBUG(
            "Writing schema to file sink; filePath = {}, schema = {}, sinkFormat = {}",
            filePath,
            sinkFormat->getSchemaPtr()->toString(),
            sinkFormat->toString());
        auto schemaStr = sinkFormat->getFormattedSchema();
        outputFile.write(schemaStr.c_str(), static_cast<int64_t>(schemaStr.length()));
        schemaWritten = true;
    }
    ...
    auto fBuffer = sinkFormat->getFormattedBuffer(inputTupleBuffer, pec.getBufferManager());
    NES_TRACE("Writing tuples to file sink; filePath={}, fBuffer={}", filePath, fBuffer);
