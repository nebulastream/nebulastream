...
MonitoringSink::MonitoringSink(SinkFormatPtr sinkFormat,
    ...
    : SinkMedium(std::move(sinkFormat), std::move(nodeEngine), numOfProducers,
...
SinkMediumTypes MonitoringSink::getSinkMediumType() {
  return SinkMediumTypes::MONITORING_SINK;
}

bool MonitoringSink::writeData(Runtime::TupleBuffer &inputBuffer,
                               Runtime::WorkerContextRef) {
  std::unique_lock lock(writeMutex);

  if (!inputBuffer) {
    throw Exceptions::RuntimeException(
        "MonitoringSink::writeData input buffer invalid");
  }
  ...
  NES_TRACE(
      "MonitoringSink: Received buffer for {} with {} tuple and size {}:{}",
      workerId, inputBuffer.getNumberOfTuples(),
      getSchemaPtr()->getSchemaSizeInBytes(), asJson(parsedMetric));
