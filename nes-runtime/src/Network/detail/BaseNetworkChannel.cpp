...
BaseNetworkChannel::BaseNetworkChannel(
    ...
    : socketAddr(std::move(address)), zmqSocket(std::move(zmqSocket)), channelId(channelId), bufferManager(std::move(bufferManager))
{
}
...
void BaseNetworkChannel::close(
    bool isEventOnly, Runtime::QueryTerminationType terminationType, uint16_t numSendingThreads, uint64_t currentMessageSequenceNumber)
{
    if (isClosed)
    {
        return;
