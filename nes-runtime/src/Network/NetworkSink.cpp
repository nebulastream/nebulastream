...
bool NetworkSink::writeData(Runtime::TupleBuffer& inputBuffer, Runtime::WorkerContext& workerContext)
{
    ...
    if (!channel)
    {
        ...
        NES_ASSERT2_FMT(
            workerContext.isAsyncConnectionInProgress(getUniqueNetworkSinkDescriptorId()),
...
{
    NES_DEBUG("NetworkSink: shutdown() called {} queryId {} qepsubplan {}", nesPartition.toString(), sharedQueryId, decomposedQueryPlanId);
...
void NetworkSink::reconfigure(Runtime::ReconfigurationMessage& task, Runtime::WorkerContext& workerContext)
{
    ...
    switch (task.getType())
    {
        case Runtime::ReconfigurationType::Initialize: {
            ...
            if (networkManager->getConnectSinksAsync())
            {
                ...
                auto networkChannelFuture = networkManager->registerSubpartitionProducerAsync(
                    receiverLocation, nesPartition, bufferManager, waitTime, retryTimes, reconf, queryManager, version);
        ...
        case Runtime::ReconfigurationType::ConnectionEstablished: {
            ...
            {
                NES_DEBUG(
                    "NetworkSink: reconfigure() No network channel future found for operator {} Thread {}",
                    nesPartition.toString(),
                    Runtime::NesThread::getId());
                break;
            }
    ...
    if (terminationType != Runtime::QueryTerminationType::Invalid)
    {
        ...
        if (workerContext.decreaseObjectRefCnt(this) == 1)
        {
            if (workerContext.isAsyncConnectionInProgress(getUniqueNetworkSinkDescriptorId()))
            {
                ...
                NES_DEBUG(
                    "NetworkSink: reconfigure() waiting for channel to connect in order to unbuffer before shutdown. "
                    "operator {} Thread {}",
                    nesPartition.toString(),
                    Runtime::NesThread::getId());
                ...
                {
                    NES_DEBUG(
                        "NetworkSink: reconfigure() established connection for operator {} Thread {}",
                        nesPartition.toString(),
                        Runtime::NesThread::getId());
                    workerContext.storeNetworkChannel(getUniqueNetworkSinkDescriptorId(), std::move(channel));
                }
            ...
            NES_ASSERT2_FMT(
                workerContext.releaseNetworkChannel(
                    getUniqueNetworkSinkDescriptorId(), terminationType, queryManager->getNumberOfWorkerThreads(), messageSequenceNumber),
            ...
            workerContext.storeNetworkChannel(getUniqueNetworkSinkDescriptorId(), nullptr);
            NES_DEBUG("NetworkSink: reconfigure() released channel on {} Thread {}", nesPartition.toString(), Runtime::NesThread::getId());
        }
    }
}
...
{
    NES_DEBUG("NetworkSink: postReconfigurationCallback() called {} parent plan {}", nesPartition.toString(), decomposedQueryPlanId);
...
Runtime::NodeEnginePtr NetworkSink::getNodeEngine()
{
    return nodeEngine;
}
...
void NetworkSink::configureNewSinkDescriptor(const NetworkSinkDescriptor& newNetworkSinkDescriptor)
{
    ...
    Runtime::ReconfigurationMessage message = Runtime::ReconfigurationMessage(
        nesPartition.getQueryId(),
        decomposedQueryPlanId,
        Runtime::ReconfigurationType::ConnectToNewReceiver,
        inherited0::shared_from_this(),
        newReceiverTuple);
    queryManager->addReconfigurationMessage(nesPartition.getQueryId(), decomposedQueryPlanId, message, false);
}
...
{
    NES_DEBUG(
        "NetworkSink: method clearOldAndConnectToNewChannelAsync() called {} qep {}, by thread {}",
        nesPartition.toString(),
        decomposedQueryPlanId,
        Runtime::NesThread::getId());
    ...
    auto networkChannelFuture = networkManager->registerSubpartitionProducerAsync(
        newNodeLocation, newNesPartition, bufferManager, waitTime, retryTimes, reconf, queryManager, newVersion);
...
bool NetworkSink::retrieveNewChannelAndUnbuffer(Runtime::WorkerContext& workerContext)
{
    ...
    {
        NES_DEBUG(
            "NetworkSink: reconfigure() network channel has not connected yet for operator {} Thread {}",
            nesPartition.toString(),
            Runtime::NesThread::getId());
        return false;
    }
    ...
    {
        NES_DEBUG(
            "NetworkSink: reconfigure() network channel retrieved from future is null for operator {} Thread {}",
            nesPartition.toString(),
            Runtime::NesThread::getId());
