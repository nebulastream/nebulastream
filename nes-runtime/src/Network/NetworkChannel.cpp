    ...
    while (i < retryTimes || retryTimes == 0) {
      ...
      switch (recvHeader->getMsgType()) {
        case Messages::MessageType::ServerReady: {
          ...
          // check if the server has the correct corresponding channel
          // registered, this is guaranteed by matching IDs
          if (!(serverReadyMsg->getChannelId().getNesPartition() ==
                channelId.getNesPartition())) {
            NES_ERROR(
                "{}: Connection failed with server {} for {}"
        ...
        case Messages::MessageType::ErrorMessage: {
          ...
          if (errorMsg.isPartitionDeleted()) {
            if constexpr (std::is_same_v<T, EventOnlyNetworkChannel>) {
              ...
              // handled somewhere else. For instance, what does this mean for
              // FT and upstream backup?
              NES_ERROR(
                  "EventOnlyNetworkChannel: Received partition deleted error "
                  "from server {}",
                  socketAddr);
              return nullptr;
            }
          ...
          if (errorMsg.isVersionMismatch()) {
            NES_INFO(
                "Receiver version does not match. Keep retrying until the new "
                "version has been started on the "
                "receiver side",
                socketAddr);
            break;
          }
      ...
      NES_DEBUG(
          "{}: Connection with server failed! Reconnecting attempt {} of {} "
          "backoff time {}",
          channelName, i, retryTimes, std::to_string(backOffTime.count()));
...
NetworkChannelPtr NetworkChannel::create(
    std::shared_ptr<zmq::context_t> const& zmqContext, std::string&& socketAddr,
    NesPartition nesPartition, ExchangeProtocol& protocol,
    Runtime::BufferManagerPtr bufferManager, int highWaterMark,
    std::chrono::milliseconds waitTime, uint8_t retryTimes,
