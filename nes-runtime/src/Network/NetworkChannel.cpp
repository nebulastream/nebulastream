...
/// 10s as ZMQ_RCVTIMEO: Maximum time before a recv operation returns with
/// EAGAIN : http://api.zeromq.org/3-0:zmq-setsockopt
static constexpr int DEFAULT_RCVTIMEO_VALUE = 10 * 1000;
namespace detail {
template <typename T>
std::unique_ptr<T> createNetworkChannel(
    std::shared_ptr<zmq::context_t> const &zmqContext, std::string &&socketAddr,
    NesPartition nesPartition, ExchangeProtocol &protocol,
    Runtime::BufferManagerPtr bufferManager, int highWaterMark,
  ...
  NES_ASSERT2_FMT(abortConnection.has_value() || retryTimes != 0,
                  "Cannot use indefinite retries without supplying a future to "
                  "abort connection");
  ...
  try {
    ...
    // available. For all other values, it will wait for a message for that
    // amount of time before returning with an EAGAIN error.
    zmqSocket.set(zmq::sockopt::rcvtimeo, rcvtimeo);
    // set the high watermark: this zmqSocket will accept only highWaterMark
    // messages and then it ll block until more space is available
    if (highWaterMark > 0) {
      zmqSocket.set(zmq::sockopt::sndhwm, highWaterMark);
    }
    zmqSocket.set(zmq::sockopt::routing_id,
                  zmq::const_buffer{&channelId, sizeof(ChannelId)});
    zmqSocket.connect(socketAddr);
    int i = 0;
    constexpr auto mode = (T::canSendEvent && !T::canSendData)
                              ? Network::Messages::ChannelType::EventOnlyChannel
    ...
    while (i < retryTimes || retryTimes == 0) {
      ...
      if (abortConnection.has_value() &&
          abortConnection.value().wait_for(std::chrono::seconds(0)) ==
              std::future_status::ready) {
        NES_DEBUG(
            "Aborting network channel connection process on caller request");
        return nullptr;
      }
      ...
      if (recvHeader->getMagicNumber() != Messages::NES_NETWORK_MAGIC_NUMBER) {
        NES_THROW_RUNTIME_ERROR(
            "OutputChannel: Message from server is corrupt!");
      }
      ...
      case Messages::MessageType::ServerReady: {
        ...
        // check if the server has the correct corresponding channel registered,
        // this is guaranteed by matching IDs
        if (!(serverReadyMsg->getChannelId().getNesPartition() ==
              channelId.getNesPartition())) {
          NES_ERROR("{}: Connection failed with server {} for {}"
                    "->Wrong server ready message! Reason: Partitions are not "
                    "matching",
                    channelName, socketAddr,
                    channelId.getNesPartition().toString());
          break;
        }
        NES_DEBUG("{}: Connection established with server {} for {}",
                  channelName, socketAddr, channelId);
        return std::make_unique<T>(std::move(zmqSocket), channelId,
                                   std::move(socketAddr),
      ...
      case Messages::MessageType::ErrorMessage: {
        ...
        if (errorMsg.isPartitionDeleted()) {
          if constexpr (std::is_same_v<T, EventOnlyNetworkChannel>) {
            ...
            // handled somewhere else. For instance, what does this mean for FT
            // and upstream backup?
            NES_ERROR("EventOnlyNetworkChannel: Received partition deleted "
                      "error from server {}",
                      socketAddr);
            return nullptr;
          }
        ...
        if (errorMsg.isVersionMismatch()) {
          NES_INFO("Receiver version does not match. Keep retrying until the "
                   "new version has been started on the "
                   "receiver side",
                   socketAddr);
          break;
        }

        NES_WARNING("{}: Received error from server-> {}", channelName,
                    errorMsg.getErrorTypeAsString());
        protocol.onChannelError(errorMsg);
        break;
      }
      default: {
        // got a wrong message type!
        NES_ERROR("{}: received unknown message {}", channelName,
                  static_cast<int>(recvHeader->getMsgType()));
        return nullptr;
      }
      }
      NES_DEBUG("{}: Connection with server failed! Reconnecting attempt {} of "
                "{} backoff time {}",
                channelName, i, retryTimes,
    ...
    NES_ERROR(
        "{}: Error establishing a connection with server: {} Closing socket!",
...
NetworkChannel::NetworkChannel(zmq::socket_t &&zmqSocket,
                               const ChannelId channelId, std::string &&address,
                               Runtime::BufferManagerPtr bufferManager)
    : inherited(std::move(zmqSocket), channelId, std::move(address),
                std::move(bufferManager)) {}

NetworkChannel::~NetworkChannel() {
  NES_ASSERT2_FMT(this->isClosed,
                  "Destroying non-closed NetworkChannel " << channelId);
}
...
void NetworkChannel::close(Runtime::QueryTerminationType terminationType,
  ...
  inherited::close(canSendEvent && !canSendData, terminationType,
                   numSendingThreads, currentMessageSequenceNumber);
}

NetworkChannelPtr
NetworkChannel::create(std::shared_ptr<zmq::context_t> const &zmqContext,
                       std::string &&socketAddr, NesPartition nesPartition,
                       ExchangeProtocol &protocol,
                       Runtime::BufferManagerPtr bufferManager,
                       int highWaterMark, std::chrono::milliseconds waitTime,
                       uint8_t retryTimes, DecomposedQueryPlanVersion version,
                       std::optional<std::future<bool>> abortConnection) {
  return detail::createNetworkChannel<NetworkChannel>(
      zmqContext, std::move(socketAddr), nesPartition, protocol, bufferManager,
      highWaterMark, waitTime, retryTimes, version, std::move(abortConnection));
}

EventOnlyNetworkChannel::EventOnlyNetworkChannel(
    zmq::socket_t &&zmqSocket, const ChannelId channelId, std::string &&address,
    Runtime::BufferManagerPtr bufferManager)
    : inherited(std::move(zmqSocket), channelId, std::move(address),
                std::move(bufferManager)) {
...
void EventOnlyNetworkChannel::close(
    Runtime::QueryTerminationType terminationType) {
  NES_DEBUG("Closing EventOnlyNetworkChannel {}", channelId);
  inherited::close(canSendEvent && !canSendData, terminationType);
}

EventOnlyNetworkChannelPtr EventOnlyNetworkChannel::create(
    std::shared_ptr<zmq::context_t> const &zmqContext, std::string &&socketAddr,
    NesPartition nesPartition, ExchangeProtocol &protocol,
    Runtime::BufferManagerPtr bufferManager, int highWaterMark,
    std::chrono::milliseconds waitTime, uint8_t retryTimes) {
  return detail::createNetworkChannel<EventOnlyNetworkChannel>(
      zmqContext, std::move(socketAddr), nesPartition, protocol, bufferManager,
      highWaterMark, waitTime, retryTimes);
}
