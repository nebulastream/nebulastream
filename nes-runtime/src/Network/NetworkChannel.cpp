...
std::unique_ptr<T> createNetworkChannel(std::shared_ptr<zmq::context_t> const& zmqContext,
    ...
    try {
        ...
        //FIXME
        retryTimes = 0;
        NES_DEBUG("create network channel abort future optional has value = {}", abortConnection.has_value());
            ...
            while (i < retryTimes || retryTimes == 0) {
                ...
                switch (recvHeader->getMsgType()) {
                    case Messages::MessageType::ServerReady: {
                        ...
                        return std::make_unique<T>(std::move(zmqSocket),
                                                   channelId,
                                                   std::move(socketAddr),
        ...
        NES_DEBUG("could not create network channel: {}", e.what());
        return nullptr;
    }
}
...
NetworkChannel::~NetworkChannel() {
    if (!this->isClosed) {
        NES_ERROR("trying to destroy non closed channel");
    }
