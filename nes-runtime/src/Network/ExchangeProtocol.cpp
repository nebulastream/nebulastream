...
void ExchangeProtocol::onEndOfStream(Messages::EndOfStreamMessage endOfStreamMessage,
    ...
    if (partitionManager->getConsumerRegistrationStatus(eosNesPartition) == PartitionRegistrationStatus::Registered) {
        ...
        // NES_ERROR("Received lastEOS {}, max message seq number {} and max stored seq number {} for parition {}", lastEOS, endOfStreamMessage.getMaxMessageSequenceNumber(), partitionManager->getMaxRegisteredSequenceNumber(eosNesPartition), eosNesPartition.getOperatorId());
        if (lastEOS) {
            ...
            while ((*maxSeqNumberPerNesPartition.rlock()).at(eosNesPartition).getCurrentValue() < eosMessageMaxSeqNumber) {
                auto seqNumber = (*maxSeqNumberPerNesPartition.rlock()).at(eosNesPartition).getCurrentValue();
