...
ZmqServer::ZmqServer(
    ...
    , numNetworkThreads(std::max(DEFAULT_NUM_SERVER_THREADS, numNetworkThreads))
    , isRunning(false)
    , keepRunning(true)
    , exchangeProtocol(exchangeProtocol)
    , bufferManager(std::move(bufferManager))
{
    ...
    {
        NES_WARNING("ZmqServer({}:{}) numNetworkThreads is smaller than DEFAULT_NUM_SERVER_THREADS", this->hostname, this->currentPort);
    }
}
...
bool ZmqServer::start()
{
    ...
    routerThread = std::make_unique<std::thread>(
        [this, numHandlerThreads, startPromise]()
        {
...
bool ZmqServer::stop()
{
    ...
    if (future.valid())
    {
        ...
        try
        {
            ...
            if (gracefullyClosed)
            {
                NES_DEBUG("ZmqServer({}:{}): gracefully closed on: {}", this->hostname, this->currentPort, hostname, this->currentPort);
            }
            else
            {
                NES_WARNING(
                    "ZmqServer({}:{}): non gracefully closed on: {}", this->hostname, this->currentPort, hostname, this->currentPort);
            }
        }
...
void ZmqServer::routerLoop(uint16_t numHandlerThreads, const std::shared_ptr<std::promise<bool>>& startPromise)
{
    ...
    {
        handlerThreads.emplace_back(std::make_unique<std::thread>(
            [this, &barrier, i]()
            {
    ...
    catch (...)
    {
        ...
        catch (zmq::error_t& zmqError)
        {
            ...
            if (zmqError.num() == ETERM)
            {
                ...
                NES_INFO(
                    "ZmqServer({}:{}): Frontend: Shutdown completed! address: tcp://{}:{}",
                    this->hostname,
                    this->currentPort,
                    hostname,
...
void ZmqServer::messageHandlerEventLoop(const std::shared_ptr<ThreadBarrier>& barrier, int threadId)
{
        ...
        while (keepRunning)
        {
            ...
            switch (msgHeader->getMsgType())
            {
                        ...
                        detail::overloaded{
                            [&dispatcherSocket, &outIdentityEnvelope](Messages::ServerReadyMessage serverReadyMessage) {
                                sendMessageWithIdentity<Messages::ServerReadyMessage>(
                                    dispatcherSocket, outIdentityEnvelope, serverReadyMessage);
                            },
                            [this, &dispatcherSocket, &outIdentityEnvelope](Messages::ErrorMessage errorMessage)
                            {
                                sendMessageWithIdentity<Messages::ErrorMessage>(dispatcherSocket, outIdentityEnvelope, errorMessage);
                ...
                case MessageType::DataBuffer: {
                    ...
                    NES_TRACE(
                        "ZmqServer({}:{}):  DataBuffer received from origin={} and seqNumber={} and NesPartition={} "
                    ...
                    for (uint32_t i = 0u, numOfChildren = bufferHeader->numOfChildren; i < numOfChildren; ++i)
                    {
                        ...
                        auto optRetSize = dispatcherSocket.recv(
                            zmq::mutable_buffer(childBuffer->getBuffer(), childBufferHeader->payloadSize), kZmqRecvDefault);
                        NES_ASSERT2_FMT(optRetSize.has_value(), "Invalid recv size");
                        NES_ASSERT2_FMT(
                            optRetSize.value().size == childBufferHeader->payloadSize,
                            "Recv not matching sizes " << optRetSize.value().size << "!=" << childBufferHeader->payloadSize);
                    ...
                    NES_ASSERT2_FMT(
                        bufferHeader->payloadSize <= buffer.getBufferSize(),
                        "Buffer size [" << buffer.getBufferSize() << " is smaller than payload " << bufferHeader->payloadSize);
                    auto optRetSize
                        = dispatcherSocket.recv(zmq::mutable_buffer(buffer.getBuffer(), bufferHeader->payloadSize), kZmqRecvDefault);
                    NES_ASSERT2_FMT(optRetSize.has_value(), "Invalid recv size");
                    NES_ASSERT2_FMT(
                        optRetSize.value().size == bufferHeader->payloadSize,
                    ...
                    exchangeProtocol.onBuffer(
                        *nesPartition, buffer, {bufferHeader->messageSequenceNumber, defaultChunkNumber, isLastChunk});
                    break;
                }
                ...
                case MessageType::EndOfStream: {
                    ...
                    NES_DEBUG(
                        "ZmqServer({}:{}): EndOfStream received for channel ", this->hostname, this->currentPort, eosMsg.getChannelId());
    ...
    catch (zmq::error_t& err)
    {
        if (err.num() == ETERM)
        {
            ///            dispatcherSocket.close();
            NES_DEBUG(
                "ZmqServer({}:{}):  Handler #{}  closed on server{}: {}",
                this->hostname,
                this->currentPort,
                threadId,
                hostname,
