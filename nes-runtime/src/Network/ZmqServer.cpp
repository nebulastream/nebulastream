...
ZmqServer::ZmqServer(std::string hostname, uint16_t requestedPort,
    ...
    : hostname(std::move(hostname)),
      ...
      numNetworkThreads(
          std::max(DEFAULT_NUM_SERVER_THREADS, numNetworkThreads)),
      isRunning(false),
      keepRunning(true),
      exchangeProtocol(exchangeProtocol),
      bufferManager(std::move(bufferManager)) {
  NES_DEBUG("ZmqServer({}:{}) Creating ZmqServer()", this->hostname,
            this->currentPort);
  if (numNetworkThreads < DEFAULT_NUM_SERVER_THREADS) {
    NES_WARNING(
        "ZmqServer({}:{}) numNetworkThreads is smaller than "
        "DEFAULT_NUM_SERVER_THREADS",
        this->hostname, this->currentPort);
  }
}

bool ZmqServer::start() {
  NES_DEBUG("ZmqServer({}:{}): Starting server..", this->hostname,
            this->currentPort);
  std::shared_ptr<std::promise<bool>> startPromise =
  ...
  // NES_ASSERT(MAX_ZMQ_SOCKET == zmqContext->get(zmq::ctxopt::max_sockets),
  // "Cannot set max num of sockets");
  routerThread =
      std::make_unique<std::thread>([this, numHandlerThreads, startPromise]() {
...
bool ZmqServer::stop() {
  ...
  NES_INFO("ZmqServer({}:{}): Initiating shutdown", this->hostname,
           this->currentPort);
  if (!zmqContext) {
  ...
  /// 1. call shutdown on the zmq context (this tells ZMQ to prepare for
  /// termination)
  /// 2. wait for all zmq sockets and threads to be closed (including the
  /// WorkerPool as it holds DataChannels)
  ...
  if (future.valid()) {
    ...
    try {
      ...
      if (gracefullyClosed) {
        NES_DEBUG("ZmqServer({}:{}): gracefully closed on: {}", this->hostname,
                  this->currentPort, hostname, this->currentPort);
      } else {
        NES_WARNING("ZmqServer({}:{}): non gracefully closed on: {}",
                    this->hostname, this->currentPort, hostname,
                    this->currentPort);
      }
    } catch (std::exception& e) {
      NES_ERROR("ZmqServer({}:{}):  Server failed to start due to {}",
                this->hostname, this->currentPort, e.what());
      zmqContext->close();
      zmqContext.reset();
      throw e;
    }
  }
  NES_INFO("ZmqServer({}:{}): Going to close zmq context...", this->hostname,
           this->currentPort);
  zmqContext->close();
  NES_INFO("ZmqServer({}:{}): Zmq context is now closed", this->hostname,
           this->currentPort);
  zmqContext.reset();
  return true;
}
...
void ZmqServer::routerLoop(
    uint16_t numHandlerThreads,
    const std::shared_ptr<std::promise<bool>>& startPromise) {
  ...
  try {
    ...
    NES_DEBUG("ZmqServer({}:{}):  Trying to bind on {}", this->hostname,
              requestedPort, address);
    ...
    NES_DEBUG("ZmqServer({}:{}):  Created socket on {}: {}", this->hostname,
              this->currentPort, hostname, currentPort);
  } catch (std::exception& ex) {
    NES_ERROR("ZmqServer({}:{}):   Error in routerLoop() {}", this->hostname,
              this->currentPort, ex.what());
    startPromise->set_value(false);
    errorPromise.set_exception(std::make_exception_ptr(ex));
    return;
  }

  const auto currentSocketAddress =
      frontendSocket.get(zmq::sockopt::last_endpoint);
  uint8_t host[4];
  int actualPort = -1;
  NES_ASSERT2_FMT(currentSocketAddress.size(),
                  "cannot read zmq identity on " << hostname);
  sscanf(currentSocketAddress.c_str(), "tcp://%hhu.%hhu.%hhu.%hhu:%d", &host[0],
         &host[1], &host[2], &host[3], &actualPort);
  NES_ASSERT2_FMT(actualPort > 0,
                  "Wrong port on zmq server " << this->hostname);
  ...
  for (int i = 0; i < numHandlerThreads; ++i) {
    handlerThreads.emplace_back(
        std::make_unique<std::thread>([this, &barrier, i]() {
  ...
  try {
    ...
    try {
      ...
      if (zmqError.num() == ETERM) {
        ...
        NES_INFO(
            "ZmqServer({}:{}): Frontend: Shutdown completed! address: "
            "tcp://{}:{}",
            this->hostname, this->currentPort, hostname,
            std::to_string(actualPort));

      } else {
        NES_ERROR("ZmqServer({}:{}):{}", this->hostname, this->currentPort,
                  zmqError.what());
...
void ZmqServer::messageHandlerEventLoop(
    const std::shared_ptr<ThreadBarrier>& barrier, int threadId) {
  using namespace Messages;
  zmq::socket_t dispatcherSocket(*zmqContext, zmq::socket_type::dealer);
  try {
    dispatcherSocket.connect(dispatcherPipe);
    barrier->wait();
    NES_DEBUG("Created Zmq Handler Thread #{} on {}: {}", threadId, hostname,
              this->currentPort);
    while (keepRunning) {
      zmq::message_t identityEnvelope;
      zmq::message_t headerEnvelope;
      auto const identityEnvelopeReceived =
      ...
      if (msgHeader->getMagicNumber() != Messages::NES_NETWORK_MAGIC_NUMBER ||
          !identityEnvelopeReceived.has_value() ||
          !headerEnvelopeReceived.has_value()) {
        // TODO handle error -- need to discuss how we handle errors on the node
        // engine
        NES_THROW_RUNTIME_ERROR("ZmqServer(" << this->hostname << ":"
                                             << this->currentPort
                                             << "):  Source is corrupted");
      }
      ...
      switch (msgHeader->getMsgType()) {
        case MessageType::ClientAnnouncement: {
          // if server receives announcement, that a client wants to send
          // buffers
          zmq::message_t outIdentityEnvelope;
          zmq::message_t clientAnnouncementEnvelope;
          auto optRecvStatus = dispatcherSocket.recv(clientAnnouncementEnvelope,
                                                     kZmqRecvDefault);
          NES_ASSERT2_FMT(optRecvStatus.has_value(), "invalid recv");
          outIdentityEnvelope.copy(identityEnvelope);
          auto receivedMsg = *clientAnnouncementEnvelope
                                  .data<Messages::ClientAnnounceMessage>();
          NES_DEBUG(
              "ZmqServer({}:{}): ClientAnnouncement received for channel {}",
              this->hostname, this->currentPort, receivedMsg.getChannelId());

          // react after announcement is received
          auto returnMessage =
              exchangeProtocol.onClientAnnouncement(receivedMsg);
          std::visit(
              detail::overloaded{
                  [&dispatcherSocket, &outIdentityEnvelope](
                      Messages::ServerReadyMessage serverReadyMessage) {
                    sendMessageWithIdentity<Messages::ServerReadyMessage>(
                        dispatcherSocket, outIdentityEnvelope,
                        serverReadyMessage);
                  },
                  [this, &dispatcherSocket,
                   &outIdentityEnvelope](Messages::ErrorMessage errorMessage) {
                    sendMessageWithIdentity<Messages::ErrorMessage>(
                        dispatcherSocket, outIdentityEnvelope, errorMessage);
        ...
        case MessageType::DataBuffer: {
          ...
          auto optRecvStatus =
              dispatcherSocket.recv(bufferHeaderMsg, kZmqRecvDefault);
          NES_ASSERT2_FMT(optRecvStatus.has_value(), "invalid recv");
          // parse buffer header
          auto* bufferHeader =
              bufferHeaderMsg.data<Messages::DataBufferMessage>();
          auto nesPartition = identityEnvelope.data<NesPartition>();

          NES_TRACE(
              "ZmqServer({}:{}):  DataBuffer received from origin={} and "
              "seqNumber={} and NesPartition={} "
              "with payload size {} with num children buffer {}",
              this->hostname, this->currentPort, bufferHeader->originId,
              bufferHeader->sequenceData.toString(), nesPartition->toString(),
              bufferHeader->payloadSize, bufferHeader->numOfChildren);
          // get children if necessary
          std::vector<Runtime::TupleBuffer> children;
          for (uint32_t i = 0u, numOfChildren = bufferHeader->numOfChildren;
               i < numOfChildren; ++i) {
            zmq::message_t childBufferHeaderMsg;
            auto optRecvStatus =
                dispatcherSocket.recv(childBufferHeaderMsg, kZmqRecvDefault);
            NES_ASSERT2_FMT(optRecvStatus.has_value(), "invalid recv");
            auto* childBufferHeader =
                childBufferHeaderMsg.data<Messages::DataBufferMessage>();

            auto childBuffer = bufferManager->getUnpooledBuffer(
                childBufferHeader->payloadSize);
            auto optRetSize = dispatcherSocket.recv(
                zmq::mutable_buffer(childBuffer->getBuffer(),
                                    childBufferHeader->payloadSize),
                kZmqRecvDefault);
            NES_ASSERT2_FMT(optRetSize.has_value(), "Invalid recv size");
            NES_ASSERT2_FMT(
                optRetSize.value().size == childBufferHeader->payloadSize,
          ...
          NES_ASSERT2_FMT(bufferHeader->payloadSize <= buffer.getBufferSize(),
                          "Buffer size [" << buffer.getBufferSize()
                                          << " is smaller than payload "
                                          << bufferHeader->payloadSize);
          auto optRetSize = dispatcherSocket.recv(
              zmq::mutable_buffer(buffer.getBuffer(),
                                  bufferHeader->payloadSize),
              kZmqRecvDefault);
          NES_ASSERT2_FMT(optRetSize.has_value(), "Invalid recv size");
          NES_ASSERT2_FMT(optRetSize.value().size == bufferHeader->payloadSize,
                          "Recv not matching sizes "
                              << optRetSize.value().size
                              << "!=" << bufferHeader->payloadSize);
          ...
          exchangeProtocol.onBuffer(*nesPartition, buffer,
                                    {bufferHeader->messageSequenceNumber,
                                     defaultChunkNumber, isLastChunk});
          break;
        }
        case MessageType::EventBuffer: {
          ...
          // parse buffer header
          auto* bufferHeader =
              bufferHeaderMsg.data<Messages::EventBufferMessage>();
          auto nesPartition = *identityEnvelope.data<NesPartition>();
          auto optBuffer =
              bufferManager->getUnpooledBuffer(bufferHeader->payloadSize);
          ...
          switch (bufferHeader->eventType) {
            ...
            case Runtime::EventType::kStartSourceEvent: {
              ...
              exchangeProtocol.onEvent(nesPartition,
                                       event);  // todo jm finish up the funtion
                                                // calls folowing from here
              break;
            }
            default: {
        ...
        case MessageType::EndOfStream: {
          ...
          NES_DEBUG("ZmqServer({}:{}): EndOfStream received for channel ",
                    this->hostname, this->currentPort, eosMsg.getChannelId());
    ...
    if (err.num() == ETERM) {
      ...
      NES_DEBUG("ZmqServer({}:{}):  Handler #{}  closed on server{}: {}",
                this->hostname, this->currentPort, threadId, hostname,
                this->currentPort);
    } else {
      errorPromise.set_exception(std::current_exception());
      NES_ERROR("ZmqServer({}:{}): event loop {} got {}", this->hostname,
                this->currentPort, threadId, err.what());
