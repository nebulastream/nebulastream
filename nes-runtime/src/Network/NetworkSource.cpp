...
NetworkSource::NetworkSource(
    ...
    : DataSource(std::move(schema), std::move(bufferManager),
                 std::move(queryManager), nesPartition.getOperatorId(),
                 /*invalid origin id for the network source, as the network
                    source does not change the origin id*/
                 INVALID_ORIGIN_ID,
                 /*invalid statistic id for the network source, as the network
                    source does not change the statistic id*/
      ...
      sinkLocation(std::move(sinkLocation)), waitTime(waitTime),
      retryTimes(retryTimes), version(version),
      uniqueNetworkSourceIdentifier(uniqueNetworkSourceIdentifier) {
  NES_ASSERT(this->networkManager, "Invalid network manager");
}

std::optional<Runtime::TupleBuffer> NetworkSource::receiveData() {
  NES_THROW_RUNTIME_ERROR("NetworkSource: ReceiveData() called, but method is "
...
// this is necessary to use std::visit below (see example:
...
bool NetworkSource::start() {
  ...
  if (running.compare_exchange_strong(expected, true)) {
    for (const auto &successor : executableSuccessors) {
      auto decomposedQueryPlanId = std::visit(
          detail::overloaded{
              [](DataSinkPtr sink) { return sink->getParentPlanId(); },
              [](Execution::ExecutablePipelinePtr pipeline) {
                return pipeline->getDecomposedQueryPlanId();
              }},
          successor);
      auto sharedQueryId = std::visit(
          detail::overloaded{
              [](DataSinkPtr sink) { return sink->getSharedQueryId(); },
              [](Execution::ExecutablePipelinePtr pipeline) {
      ...
      auto newReconf =
          ReconfigurationMessage(sharedQueryId, decomposedQueryPlanId,
...
bool NetworkSource::fail() {
  ...
  if (running.compare_exchange_strong(expected, false)) {
    ...
    queryManager->notifySourceCompletion(
        shared_from_base<DataSource>(), Runtime::QueryTerminationType::Failure);
    return queryManager->addEndOfStream(shared_from_base<NetworkSource>(),
...
bool NetworkSource::stop(Runtime::QueryTerminationType type) {
  ...
  bool expected = true;
  NES_ASSERT2_FMT(
      type == QueryTerminationType::HardStop,
      "NetworkSource::stop only supports HardStop or Failure :: partition "
          << nesPartition);
  if (running.compare_exchange_strong(expected, false)) {
    ...
    queryManager->addEndOfStream(shared_from_base<DataSource>(),
                                 Runtime::QueryTerminationType::HardStop);
    NES_DEBUG("NetworkSource: stop called on {} sent hard eos", nesPartition);
  } else {
    NES_DEBUG("NetworkSource: stop called on {} but was already stopped",
              nesPartition);
  }
  return true;
}
...
void NetworkSource::reconfigure(Runtime::ReconfigurationMessage &task,
                                Runtime::WorkerContext &workerContext) {
  NES_DEBUG("NetworkSource: reconfigure() called {} for the type {}",
            nesPartition.toString(), magic_enum::enum_name(task.getType()));
  ...
  case Runtime::ReconfigurationType::UpdateVersion: {
    if (!networkManager->getConnectSourceEventChannelsAsync()) {
      NES_THROW_RUNTIME_ERROR(
          "Attempt to reconfigure a network source but asynchronous connecting "
          "of event channels is not "
          "activated. To use source reconfiguration allow asynchronous "
          "connecting in the the configuration");
    }
    workerContext.releaseEventOnlyChannel(uniqueNetworkSourceIdentifier,
                                          terminationType);
  ...
  case Runtime::ReconfigurationType::Initialize: {
    ...
    // NetworkSource::start() and NetworkSource::reconfigure() the query might
    // have been stopped for some reason
    if (networkManager->isPartitionConsumerRegistered(nesPartition) ==
        PartitionRegistrationStatus::Deleted) {
      NES_WARNING("NetworkManager shows the partition {} to be deleted, but "
                  "now we should init it here, so we simply return!",
    ...
    if (networkManager->getConnectSourceEventChannelsAsync()) {
      auto channelFuture =
          networkManager->registerSubpartitionEventProducerAsync(
              sinkLocation, nesPartition, localBufferManager, waitTime,
              retryTimes);
      workerContext.storeEventChannelFuture(this->operatorId,
                                            std::move(channelFuture));
      break;
    } else {
      auto channel = networkManager->registerSubpartitionEventProducer(
          sinkLocation, nesPartition, localBufferManager, waitTime, retryTimes);
      if (channel == nullptr) {
        NES_WARNING("NetworkSource: reconfigure() cannot get event channel {} "
                    "on Thread {}",
                    nesPartition.toString(), Runtime::NesThread::getId());
        return; // partition was deleted on the other side of the channel... no
                // point in waiting for a channel
      }
      workerContext.storeEventOnlyChannel(this->operatorId, std::move(channel));
      NES_DEBUG(
          "NetworkSource: reconfigure() stored event-channel {} Thread {}",
          nesPartition.toString(), Runtime::NesThread::getId());
    }
    break;
  }
  case Runtime::ReconfigurationType::Destroy: {
    // necessary as event channel are lazily created so in the case of an
    // immediate stop they might not be established yet
  ...
  if (isTermination) {
    if (!workerContext.doesEventChannelExist(this->operatorId)) {
      ...
      auto channel =
          workerContext.waitForAsyncConnectionEventChannel(this->operatorId);
      if (channel) {
...
void NetworkSource::postReconfigurationCallback(
    Runtime::ReconfigurationMessage &task) {
  NES_DEBUG("NetworkSource: postReconfigurationCallback() called {}",
            nesPartition.toString());
  ...
  if (Runtime::QueryTerminationType::Invalid != terminationType) {
    NES_DEBUG("NetworkSource: postReconfigurationCallback(): unregistering "
              "SubpartitionConsumer {}",
              nesPartition.toString());
    networkManager->unregisterSubpartitionConsumer(nesPartition);
    bool expected = true;
    if (running.compare_exchange_strong(expected, false)) {
      NES_DEBUG("NetworkSource is stopped on reconf task with id {}",
                nesPartition.toString());
...
void NetworkSource::runningRoutine(const Runtime::BufferManagerPtr &,
  ...
  NES_THROW_RUNTIME_ERROR("NetworkSource: runningRoutine() called, but method "
                          "is invalid and should not be used.");
}

void NetworkSource::onEndOfStream(
    Runtime::QueryTerminationType terminationType) {
  // propagate EOS to the locally running QEPs that use the network source
  NES_DEBUG("Going to inject eos for {} terminationType={}", nesPartition,
            terminationType);
...
void NetworkSource::onEvent(Runtime::BaseEvent &event,
  ...
  NES_DEBUG("NetworkSource::onEvent(event, wrkContext) called. operatorId: {}",
            this->operatorId);
  if (event.getEventType() == Runtime::EventType::kStartSourceEvent) {
    auto senderChannel =
        workerContext.getEventOnlyNetworkChannel(this->operatorId);
    if (!senderChannel) {
      auto senderChannelOptional =
          workerContext.getAsyncEventChannelConnectionResult(this->operatorId);
      if (!senderChannelOptional) {
        NES_DEBUG("NetworkSource::onEvent(event, wrkContext) operatorId: {}: "
                  "could not send event because event channel "
                  "has not been established yet",
                  this->operatorId);
        return;
      }
      workerContext.storeEventOnlyChannel(
          this->operatorId, std::move(senderChannelOptional.value()));
      senderChannel =
          workerContext.getEventOnlyNetworkChannel(this->operatorId);
      if (!senderChannel) {
        return;
...
OperatorId NetworkSource::getUniqueId() const {
  return uniqueNetworkSourceIdentifier;
}

bool NetworkSource::scheduleNewDescriptor(
