        ...
        for (const auto& successor : executableSuccessors)
        {
            auto decomposedQueryPlanId = std::visit(
                detail::overloaded{
                    ...
                    [](Execution::ExecutablePipelinePtr pipeline) { return pipeline->getDecomposedQueryPlanId(); }},
                successor);
            auto sharedQueryId = std::visit(
                detail::overloaded{
                    [](DataSinkPtr sink) { return sink->getSharedQueryId(); },
                    [](Execution::ExecutablePipelinePtr pipeline) { return pipeline->getSharedQueryId(); }},
                successor);
...
bool NetworkSource::stop(Runtime::QueryTerminationType type)
{
    ...
    bool expected = true;
    NES_ASSERT2_FMT(
        type == QueryTerminationType::HardStop, "NetworkSource::stop only supports HardStop or Failure :: partition " << nesPartition);
    if (running.compare_exchange_strong(expected, false))
    {
...
{
    NES_DEBUG("NetworkSource: reconfigure() called {} for the type {}", nesPartition.toString(), magic_enum::enum_name(task.getType()));
    ...
    switch (task.getType())
    {
        case Runtime::ReconfigurationType::UpdateVersion: {
            ...
            NES_DEBUG(
                "NetworkSource: reconfigure() released channel on {} Thread {}", nesPartition.toString(), Runtime::NesThread::getId());
            break;
        }
        case Runtime::ReconfigurationType::Initialize: {
            ...
            if (networkManager->getConnectSourceEventChannelsAsync())
            {
                auto channelFuture = networkManager->registerSubpartitionEventProducerAsync(
            ...
            else
            {
                ...
                NES_DEBUG(
                    "NetworkSource: reconfigure() stored event-channel {} Thread {}", nesPartition.toString(), Runtime::NesThread::getId());
            }
            break;
        }
    ...
    if (isTermination)
    {
        ...
        NES_DEBUG("NetworkSource: reconfigure() released channel on {} Thread {}", nesPartition.toString(), Runtime::NesThread::getId());
    }
}
...
DecomposedQueryPlanVersion NetworkSource::getVersion() const
{
    return version;
}
...
void NetworkSource::onEvent(Runtime::BaseEvent& event, Runtime::WorkerContextRef workerContext)
{
    ...
    if (event.getEventType() == Runtime::EventType::kStartSourceEvent)
    {
        ...
        if (!senderChannel)
        {
            ...
            {
                NES_DEBUG(
                    "NetworkSource::onEvent(event, wrkContext) operatorId: {}: could not send event because event channel "
