...
std::optional<Runtime::TupleBuffer> NetworkSource::receiveData() {
  NES_THROW_RUNTIME_ERROR(
      "NetworkSource: ReceiveData() called, but method is invalid and should "
      "not be used.");
}
  ...
  switch (task.getType()) {
    case Runtime::ReconfigurationType::UpdateVersion: {
      if (!networkManager->getConnectSourceEventChannelsAsync()) {
        NES_THROW_RUNTIME_ERROR(
            "Attempt to reconfigure a network source but asynchronous "
            "connecting of event channels is not "
    ...
    case Runtime::ReconfigurationType::Initialize: {
      ...
      if (networkManager->isPartitionConsumerRegistered(nesPartition) ==
        ...
        NES_WARNING(
            "NetworkManager shows the partition {} to be deleted, but now we "
            "should init it here, so we simply return!",
      ...
      if (networkManager->getConnectSourceEventChannelsAsync()) {
        ...
        if (channel == nullptr) {
          NES_WARNING(
              "NetworkSource: reconfigure() cannot get event channel {} on "
              "Thread {}",
              nesPartition.toString(), Runtime::NesThread::getId());
          return;  // partition was deleted on the other side of the channel...
                   // no point in waiting for a channel
        }
        workerContext.storeEventOnlyChannel(this->operatorId,
                                            std::move(channel));
        NES_DEBUG(
...
void NetworkSource::postReconfigurationCallback(
  ...
  if (Runtime::QueryTerminationType::Invalid != terminationType) {
    NES_DEBUG(
        "NetworkSource: postReconfigurationCallback(): unregistering "
...
void NetworkSource::runningRoutine(const Runtime::BufferManagerPtr&,
  ...
  NES_THROW_RUNTIME_ERROR(
      "NetworkSource: runningRoutine() called, but method is invalid and "
      "should not be used.");
}
  ...
  if (event.getEventType() == Runtime::EventType::kStartSourceEvent) {
    ...
    if (!senderChannel) {
      ...
      if (!senderChannelOptional) {
        NES_DEBUG(
            "NetworkSource::onEvent(event, wrkContext) operatorId: {}: could "
            "not send event because event channel "
