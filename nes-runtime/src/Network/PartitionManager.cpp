...
PartitionManager::getSubpartitionConsumerCounter(NesPartition partition) {
  std::unique_lock lock(consumerPartitionsMutex);
...
DecomposedQueryPlanVersion
PartitionManager::getVersion(NesPartition partition) {
  std::unique_lock lock(consumerPartitionsMutex);
  if (auto it = consumerPartitions.find(partition);
      it != consumerPartitions.end()) {
...
Runtime::RuntimeEventListenerPtr
PartitionManager::getEventListener(NesPartition partition) const {
...
PartitionManager::getConsumerRegistrationStatus(NesPartition partition) const {
  // check if partition is present
  std::unique_lock lock(consumerPartitionsMutex);
  if (auto it = consumerPartitions.find(partition);
      it != consumerPartitions.end()) {
...
PartitionRegistrationStatus
PartitionManager::getProducerRegistrationStatus(NesPartition partition) const {
  // check if partition is present
  std::unique_lock lock(producerPartitionsMutex);
  if (auto it = producerPartitions.find(partition);
      it != producerPartitions.end()) {
...
std::optional<uint64_t>
PartitionManager::getSubpartitionProducerCounter(NesPartition partition) {
  std::unique_lock lock(producerPartitionsMutex);
  if (auto it = producerPartitions.find(partition);
      it != producerPartitions.end()) {
    return it->second.count();
