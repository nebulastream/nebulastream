...
// rationale: we need a placeholder to keep track of information regarding
// partitions that were present on a node

namespace NES::Network {

PartitionManager::PartitionConsumerEntry::PartitionConsumerEntry(
    NodeLocation&& senderLocation, DataEmitterPtr&& emitter)
    : senderLocation(std::move(senderLocation)), consumer(std::move(emitter)) {
  auto networkSource =
      std::dynamic_pointer_cast<Network::NetworkSource>(consumer);
}

uint64_t PartitionManager::PartitionConsumerEntry::count() const {
...
DataEmitterPtr PartitionManager::PartitionConsumerEntry::getConsumer() {
  return consumer;
}

DecomposedQueryPlanVersion
PartitionManager::PartitionConsumerEntry::getVersion() {
  return consumer->getVersion();
}

PartitionManager::PartitionProducerEntry::PartitionProducerEntry(
...
void PartitionManager::PartitionProducerEntry::registerEventListener(
    Runtime::RuntimeEventListenerPtr eventListener) {
  this->eventListener = eventListener;
}

Runtime::RuntimeEventListenerPtr
PartitionManager::PartitionProducerEntry::getEventListener() const {
...
void PartitionManager::pinSubpartitionConsumer(NesPartition partition) {
  ...
  NES_ASSERT2_FMT(
      false, "Cannot increment partition counter as partition does not exists "
                 << partition);
}
...
bool PartitionManager::registerSubpartitionConsumer(NesPartition partition,
  ...
  if (it != consumerPartitions.end()) {
    ...
    it = consumerPartitions.insert_or_assign(
        it, partition,
        PartitionConsumerEntry(std::move(senderLocation),
                               std::move(emitterPtr)));
  }
  NES_DEBUG("PartitionManager: Registering Subpartition Consumer {}={}",
            partition.toString(), (*it).second.count());
...
bool PartitionManager::unregisterSubpartitionConsumer(NesPartition partition) {
  ...
  NES_ASSERT2_FMT(it != consumerPartitions.end(),
                  "PartitionManager: error while unregistering partition "
                      << partition << " reason: partition not found");

  // safeguard
  if (it->second.count() == 0) {
    NES_DEBUG("PartitionManager: Partition {}, counter is at 0.",
              partition.toString());
    return true;
  }
  ...
  NES_INFO("PartitionManager: Unregistering Consumer {}; newCnt({})",
           partition.toString(), it->second.count());
  if (it->second.count() == 1) {
    NES_DEBUG("PartitionManager: Consumer {}, counter is at 1.",
              partition.toString());
    return true;
  }
  return false;
...
std::optional<uint64_t> PartitionManager::getSubpartitionConsumerCounter(
    NesPartition partition) {
  std::unique_lock lock(consumerPartitionsMutex);
  if (auto it = consumerPartitions.find(partition);
...
DecomposedQueryPlanVersion PartitionManager::getVersion(
    NesPartition partition) {
  std::unique_lock lock(consumerPartitionsMutex);
  if (auto it = consumerPartitions.find(partition);
      it != consumerPartitions.end()) {
...
Runtime::RuntimeEventListenerPtr PartitionManager::getEventListener(
    NesPartition partition) const {
  std::unique_lock lock(producerPartitionsMutex);
  if (auto it = producerPartitions.find(partition);
...
void PartitionManager::clear() {
  ...
  NES_DEBUG(
      "PartitionManager: Running sanity check on partitions with refCnt > 0");
  for (auto&& [partition, metadata] : producerPartitions) {
    NES_ASSERT2_FMT(metadata.count() == 0,
                    "PartitionManager: Producer Partition "
                        << partition
                        << " is still alive: " << metadata.count());
  }
  for (auto&& [partition, metadata] : consumerPartitions) {
    NES_ASSERT2_FMT(metadata.count() == 0,
                    "PartitionManager: Consumer Partition "
                        << partition
                        << " is still alive: " << metadata.count());
  }
...
PartitionRegistrationStatus PartitionManager::getConsumerRegistrationStatus(
    NesPartition partition) const {
  // check if partition is present
  std::unique_lock lock(consumerPartitionsMutex);
  if (auto it = consumerPartitions.find(partition);
      it != consumerPartitions.end()) {
    return it->second.count() > 0 ? PartitionRegistrationStatus::Registered
                                  : PartitionRegistrationStatus::Deleted;
  }
  return PartitionRegistrationStatus::NotFound;
}

PartitionRegistrationStatus PartitionManager::getProducerRegistrationStatus(
    NesPartition partition) const {
  // check if partition is present
  std::unique_lock lock(producerPartitionsMutex);
  if (auto it = producerPartitions.find(partition);
...
bool PartitionManager::registerSubpartitionProducer(
    NesPartition partition, NodeLocation receiverLocation) {
  std::unique_lock lock(producerPartitionsMutex);
  // check if partition is present
  ...
  if (it != producerPartitions.end()) {
    ...
    it = producerPartitions.insert_or_assign(
        it, partition, PartitionProducerEntry(std::move(receiverLocation)));
  }
  NES_DEBUG("PartitionManager: Registering Subpartition Producer {}={}",
            partition.toString(), (*it).second.count());
...
bool PartitionManager::addSubpartitionEventListener(
  ...
  if (auto it = producerPartitions.find(partition);
    ...
    NES_DEBUG("PartitionManager: Registering Subpartition Event Consumer {}={}",
              partition.toString(), (*it).second.count());
    return true;
  }
...
bool PartitionManager::unregisterSubpartitionProducer(NesPartition partition) {
  ...
  NES_INFO(
      "PartitionManager: Unregistering Subpartition Producer {}; newCnt({})",
      partition.toString(), it->second.count());
  if (it->second.count() == 0) {
    NES_DEBUG("PartitionManager: Producer Partition {}, counter is at 0.",
              partition.toString());
    return true;
  }
  return false;
...
std::optional<uint64_t> PartitionManager::getSubpartitionProducerCounter(
    NesPartition partition) {
  std::unique_lock lock(producerPartitionsMutex);
  if (auto it = producerPartitions.find(partition);
      it != producerPartitions.end()) {
