...
DecomposedQueryPlanVersion PartitionManager::PartitionConsumerEntry::getVersion()
{
...
void PartitionManager::PartitionProducerEntry::pin()
{
    partitionCounter++;
}

void PartitionManager::PartitionProducerEntry::unpin()
{
    partitionCounter--;
}
...
Runtime::RuntimeEventListenerPtr PartitionManager::PartitionProducerEntry::getEventListener() const
{
...
bool PartitionManager::registerSubpartitionConsumer(NesPartition partition, NodeLocation senderLocation, DataEmitterPtr emitterPtr)
{
    ...
    {
        it = consumerPartitions.insert_or_assign(it, partition, PartitionConsumerEntry(std::move(senderLocation), std::move(emitterPtr)));
    }
...
void PartitionManager::clear()
{
    ...
    {
        NES_ASSERT2_FMT(
            metadata.count() == 0, "PartitionManager: Producer Partition " << partition << " is still alive: " << metadata.count());
    }
    for (auto&& [partition, metadata] : consumerPartitions)
    {
        NES_ASSERT2_FMT(
            metadata.count() == 0, "PartitionManager: Consumer Partition " << partition << " is still alive: " << metadata.count());
    }
