...
DataSource::getExecutableSuccessors() {
  return executableSuccessors;
}

void DataSource::addExecutableSuccessors(
    std::vector<Runtime::Execution::SuccessorExecutablePipeline> newPipelines) {
...
DataSource::DataSource(
    SchemaPtr pSchema, Runtime::BufferManagerPtr bufferManager,
    Runtime::QueryManagerPtr queryManager, OperatorId operatorId,
    OriginId originId, StatisticId statisticId, size_t numSourceLocalBuffers,
    GatheringMode gatheringMode, const std::string &physicalSourceName,
    std::vector<Runtime::Execution::SuccessorExecutablePipeline>
    ...
    : Runtime::Reconfigurable(), DataEmitter(),
      ...
      schema(std::move(pSchema)), numSourceLocalBuffers(numSourceLocalBuffers),
      gatheringMode(gatheringMode), sourceAffinity(sourceAffinity),
      taskQueueId(taskQueueId), physicalSourceName(physicalSourceName) {
  NES_DEBUG("DataSource  {} : Init Data Source with schema  {}", operatorId,
            schema->toString());
  NES_ASSERT(this->localBufferManager, "Invalid buffer manager");
  NES_ASSERT(this->queryManager, "Invalid query manager");
  // TODO #4094: enable this exception -- currently many UTs are designed to
  // assume empty executableSuccessors
  //    if (this->executableSuccessors.empty()) {
  //        throw Exceptions::RuntimeException("empty executable successors");
  //    }
  if (schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
    memoryLayout = Runtime::MemoryLayouts::RowLayout::create(
        schema, localBufferManager->getBufferSize());
  } else if (schema->getLayoutType() ==
             Schema::MemoryLayoutType::COLUMNAR_LAYOUT) {
    memoryLayout = Runtime::MemoryLayouts::ColumnLayout::create(
        schema, localBufferManager->getBufferSize());
  }
}

void DataSource::emitWork(Runtime::TupleBuffer &buffer,
                          bool addBufferMetaData) {
  if (addBufferMetaData) {
    ...
    NES_DEBUG("Setting the buffer metadata for source {} with originId={} "
              "sequenceNumber={} chunkNumber={} lastChunk={} "
              "statisticId={}",
              buffer.getOriginId(), buffer.getOriginId(),
              buffer.getSequenceNumber(), buffer.getChunkNumber(),
              buffer.isLastChunk(), buffer.getStatisticId());
  }
  ...
  for (const auto &successor : executableSuccessors) {
    ...
    if (!sourceSharing) {
      ...
      NES_DEBUG("push task for queueid= {} successor= ",
                queueId /*, &successor*/);
...
void DataSource::setOperatorId(OperatorId operatorId) {
...
DataSource::~DataSource() NES_NOEXCEPT(false) {
  NES_ASSERT(running == false, "Data source destroyed but thread still "
                               "running... stop() was not called");
      ...
      // only CPU i as set.
        ...
        if (sourceAffinity != std::numeric_limits<uint64_t>::max()) {
          NES_ASSERT(sourceAffinity < std::thread::hardware_concurrency(),
                     "pinning position is out of cpu range maxPosition="
                         << sourceAffinity);
          cpu_set_t cpuset;
          CPU_ZERO(&cpuset);
          ...
          if (rc != 0) {
            NES_THROW_RUNTIME_ERROR(
                "Cannot set thread affinity on source thread " << operatorId);
          }
        } else {
...
bool DataSource::fail() {
  bool isStopped =
      stop(Runtime::QueryTerminationType::Failure); // this will block until the
                                                    // thread is stopped
  NES_DEBUG("Source {} stop executed= {}", operatorId,
            (isStopped ? "stopped" : "cannot stop"));
  {
    // it may happen that the source failed prior of sending its eos
    std::unique_lock lock(
        startStopMutex); // do not call stop if holding this mutex
    ...
    if (!this->endOfStreamSent) {
      endOfStreamSent = queryManager->addEndOfStream(
          self, Runtime::QueryTerminationType::Failure);
      queryManager->notifySourceCompletion(
          self, Runtime::QueryTerminationType::Failure);
      NES_DEBUG("Source {} injecting failure  {}", operatorId,
                (endOfStreamSent ? "EoS sent" : "cannot send EoS"));
    }
...
bool waitForFuture(std::future<bool> &&future,
...
bool DataSource::stop(Runtime::QueryTerminationType graceful) {
  ...
  {
    std::unique_lock lock(
        startStopMutex); // this mutex guards the thread variable
  ...
  NES_DEBUG("DataSource {}: Stop called and source is {}", operatorId,
  ...
  // TODO this is also an issue in the current development of the
  // StaticDataSource. If it is still running here, we give up to early and
  // never join the thread.

  try {
    if (!running.compare_exchange_strong(expected, false)) {
      NES_DEBUG("DataSource {} was not running, retrieving future now...",
                operatorId);
      auto expected = false;
      if (wasStarted &&
          futureRetrieved.compare_exchange_strong(expected, true)) {
        NES_ASSERT2_FMT(
            detail::waitForFuture(completedPromise.get_future(), 60s),
      ...
      NES_DEBUG("DataSource {} was running, retrieving future now...",
                operatorId);
      auto expected = false;
      NES_ASSERT2_FMT(
          wasStarted &&
    ...
    try {
      if (expPtr) {
        ...
        std::exception const &e) { // it would not work if you pass by value
      // I leave the following lines just as a reminder:
      // here we do not need to call notifySourceFailure because it is done from
      // the main thread the only reason to call notifySourceFailure is when the
      // main thread was not stated
      if (!wasStarted) {
        queryManager->notifySourceFailure(shared_from_base<DataSource>(),
                                          std::string(e.what()));
      }
      return true;
    }
...
void DataSource::close() {
  ...
  if (queryTerminationType != Runtime::QueryTerminationType::Graceful ||
      queryManager->canTriggerEndOfStream(shared_from_base<DataSource>(),
                                          queryTerminationType)) {
    // inject reconfiguration task containing end of stream
    std::unique_lock lock(startStopMutex);
    NES_ASSERT2_FMT(!endOfStreamSent,
                    "Eos was already sent for source " << toString());
    NES_DEBUG("DataSource {} : Data Source add end of stream. Gracefully={}",
              operatorId, queryTerminationType);
    endOfStreamSent = queryManager->addEndOfStream(
        shared_from_base<DataSource>(), queryTerminationType);
    NES_ASSERT2_FMT(endOfStreamSent,
                    "Cannot send eos for source " << toString());
...
void DataSource::runningRoutineWithIngestionRate() {
  NES_ASSERT(this->operatorId != INVALID_OPERATOR_ID,
             "The id of the source is not set properly");
  NES_ASSERT(
      gatheringIngestionRate >= 10,
      "As we generate on 100 ms base we need at least an ingestion rate of 10");
  ...
  NES_DEBUG("DataSource {} Running Data Source of type={} ingestion rate={}",
            operatorId, magic_enum::enum_name(getType()),
            gatheringIngestionRate);
  if (numberOfBuffersToProduce == 0) {
    NES_DEBUG("DataSource: the user does not specify the number of buffers to "
              "produce therefore we will produce buffers until "
              "the source is empty");
  } else {
    NES_DEBUG("DataSource: the user specify to produce {} buffers",
  ...
  while (running && (processedOverallBufferCnt < numberOfBuffersToProduce ||
                     numberOfBuffersToProduce == 0)) {
    // create as many tuples as requested and then sleep
    auto startPeriod = std::chrono::duration_cast<std::chrono::milliseconds>(
                           std::chrono::system_clock::now().time_since_epoch())
                           .count();
    uint64_t buffersProcessedCnt = 0;

    // produce buffers until limit for this second or for all periods is reached
    // or source is topped
    while (buffersProcessedCnt < buffersToProducePer100Ms && running &&
           (processedOverallBufferCnt < numberOfBuffersToProduce ||
            numberOfBuffersToProduce == 0)) {
      ...
      NES_TRACE("DataSource: buffersProcessedCnt={} buffersPerSecond={}",
                buffersProcessedCnt, gatheringIngestionRate);
    }
    ...
    NES_TRACE("DataSource: startTimeSendBuffers={} endTimeSendBuffers={} "
              "nextPeriodStartTime={}",
              startPeriod, endPeriod, nextPeriodStartTime);

    // If this happens then the second was not enough to create so many tuples
    // and the ingestion rate should be decreased
    if (nextPeriodStartTime < endPeriod) {
      NES_ERROR("Creating buffer(s) for DataSource took longer than "
                "periodLength. nextPeriodStartTime={} endTimeSendBuffers={}",
                nextPeriodStartTime, endPeriod);
    }
    ...
    NES_DEBUG("DataSource: Done with period {} and overall buffers={} "
              "sleepCnt={} startPeriod={} endPeriod={} "
              "nextPeriodStartTime={} curTime={}",
              curPeriod++, processedOverallBufferCnt, sleepCnt, startPeriod,
              endPeriod, nextPeriodStartTime, curTime);
  } // end of while
...
void DataSource::runningRoutineWithGatheringInterval() {
  ...
  if (numberOfBuffersToProduce == 0) {
    NES_DEBUG("DataSource: the user does not specify the number of buffers to "
              "produce therefore we will produce buffer until "
              "the source is empty");
  } else {
    NES_DEBUG("DataSource: the user specify to produce {} buffers",
              numberOfBuffersToProduce);
  ...
  while (running) {
    ...
    if (numberOfBuffersToProduce == 0 ||
        numberOfBuffersProduced < numberOfBuffersToProduce) {
      auto optBuf = receiveData(); // note that receiveData might block
      if (!running) { // necessary if source stops while receiveData is called
                      // due to stricter shutdown logic
        break;
      }
      // this checks we received a valid output buffer
      if (optBuf.has_value()) {
        auto &buf = optBuf.value();
        NES_TRACE("DataSource produced buffer {} type= {} string={}: Received "
                  "Data: {} "
                  "operatorId={} orgID={}",
                  numberOfBuffersProduced, magic_enum::enum_name(getType()),
                  toString(), buf.getNumberOfTuples(), this->operatorId,
                  this->operatorId);

        if (Logger::getInstance()->getCurrentLogLevel() ==
            LogLevel::LOG_TRACE) {
          auto layout = Runtime::MemoryLayouts::RowLayout::create(
              schema, buf.getBufferSize());
          auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
          NES_TRACE("DataSource produced buffer content={}",
                    buffer.toString(schema));
        }

        emitWork(buf);
        ++numberOfBuffersProduced;
      } else {
        NES_DEBUG("DataSource {}: stopping cause of invalid buffer",
                  operatorId);
        running = false;
        NES_DEBUG(
            "DataSource {}: Thread going to terminating with graceful exit.",
            operatorId);
      }
    } else {
      NES_DEBUG("DataSource {}: Receiving thread terminated ... stopping "
                "because cnt={} smaller than "
                "numBuffersToProcess={} now return",
                operatorId, numberOfBuffersProduced, numberOfBuffersToProduce);
      running = false;
    }
    NES_TRACE("DataSource {} : Data Source finished processing iteration {}",
              operatorId, numberOfBuffersProduced);

    // this checks if the interval is zero or a ZMQ_Source, we don't create a
    // watermark-only buffer
...
void DataSource::onEvent(Runtime::BaseEvent &event) {
  NES_DEBUG("DataSource::onEvent(event) called. operatorId: {}",
            this->operatorId);
...
void DataSource::onEvent(Runtime::BaseEvent &event, Runtime::WorkerContextRef) {
  NES_DEBUG("DataSource::onEvent(event, wrkContext) called. operatorId:  {}",
            this->operatorId);
  onEvent(event);
}
