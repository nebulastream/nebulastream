...
void DataSource::emitWork(Runtime::TupleBuffer& buffer,
  ...
  if (addBufferMetaData) {
    ...
    NES_DEBUG(
        "Setting the buffer metadata for source {} with originId={} "
        "sequenceNumber={} chunkNumber={} lastChunk={} "
        "statisticId={}",
        buffer.getOriginId(), buffer.getOriginId(), buffer.getSequenceNumber(),
        buffer.getChunkNumber(), buffer.isLastChunk(), buffer.getStatisticId());
  }
...
DataSource::~DataSource() NES_NOEXCEPT(false) {
  NES_ASSERT(running == false,
             "Data source destroyed but thread still running... stop() was not "
             "called");
...
bool DataSource::fail() {
  bool isStopped =
      stop(Runtime::QueryTerminationType::Failure);  // this will block until
...
void DataSource::runningRoutineWithIngestionRate() {
  ...
  if (numberOfBuffersToProduce == 0) {
    NES_DEBUG(
        "DataSource: the user does not specify the number of buffers to "
  ...
  while (running && (processedOverallBufferCnt < numberOfBuffersToProduce ||
    ...
    NES_TRACE(
        "DataSource: startTimeSendBuffers={} endTimeSendBuffers={} "
        "nextPeriodStartTime={}",
    ...
    if (nextPeriodStartTime < endPeriod) {
      NES_ERROR(
          "Creating buffer(s) for DataSource took longer than periodLength. "
          "nextPeriodStartTime={} endTimeSendBuffers={}",
    ...
    NES_DEBUG(
        "DataSource: Done with period {} and overall buffers={} sleepCnt={} "
...
void DataSource::runningRoutineWithGatheringInterval() {
  ...
  while (running) {
    ...
    if (numberOfBuffersToProduce == 0 ||
      ...
      NES_DEBUG(
          "DataSource {}: Receiving thread terminated ... stopping because "
          "cnt={} smaller than "
