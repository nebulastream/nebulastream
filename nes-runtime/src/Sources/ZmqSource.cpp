...
ZmqSource::ZmqSource(
    ...
    Runtime::QueryManagerPtr queryManager, const std::string &host,
    uint16_t port, OperatorId operatorId, OriginId originId,
    StatisticId statisticId, uint64_t numSourceLocalBuffers,
    ...
    : DataSource(std::move(schema), std::move(bufferManager),
                 ...
                 std::move(successors)),
      host(host), port(port), connected(false), context(zmq::context_t(1)),
...
ZmqSource::~ZmqSource() NES_NOEXCEPT(false) {
  ...
  if (success) {
    ...
    NES_ASSERT2_FMT(false, "ZMQSOURCE  " << this
                                         << ": Destroy ZMQ Source failed cause "
...
std::optional<Runtime::TupleBuffer> ZmqSource::receiveData() {
  ...
  if (connect()) {
    try {
      ...
      if (auto const receivedSize = socket.recv(metadata).value_or(0);
          receivedSize != metadataSize) {
        NES_ERROR("ZMQSource: Error: Unexpected payload size. Expected: {} "
                  "Received: {}",
                  metadataSize, receivedSize);
        return std::nullopt;
      }
      ...
      NES_DEBUG("ZMQSource received #tups  {}  watermark= {}",
                buffer.getNumberOfTuples(), buffer.getWatermark());

      // Receive payload
      // XXX: I guess we don't actually know the size here, it would be nice to
      // be able to check that here
      zmq::mutable_buffer payload{buffer.getBuffer(), buffer.getBufferSize()};
      if (auto const receivedSize = socket.recv(payload);
          !receivedSize.has_value()) {
        NES_ERROR("ZMQSource: Error: Unexpected payload size. Expected: {} "
                  "Received: {}",
                  buffer.getBufferSize(), receivedSize.has_value());
        return std::nullopt;
      } else {
        NES_DEBUG("ZMQSource: received buffer of size  {}",
                  receivedSize.has_value());
        return buffer;
      }
...
bool ZmqSource::connect() {
  if (!connected) {
    ...
    auto address =
        std::string("tcp://") + host + std::string(":") + std::to_string(port);
