...
LambdaSource::LambdaSource(
    ...
    : GeneratorSource(std::move(schema), std::move(bufferManager),
                      std::move(queryManager), numbersOfBufferToProduce,
  ...
  NES_DEBUG("Create LambdaSource with id={} func is {}", operatorId,
  ...
  numberOfTuplesToProduce = this->localBufferManager->getBufferSize() /
                            this->schema->getSchemaSizeInBytes();
...
std::optional<Runtime::TupleBuffer> LambdaSource::receiveData() {
  ...
  NES_ASSERT2_FMT(numberOfTuplesToProduce * schema->getSchemaSizeInBytes() <=
  ...
  NES_TRACE(
      "LambdaSource: ReceiveData filled buffer with tuples={}, outOrgID={}",
      buffer.getNumberOfTuples(), rawBuffer.getOriginId());
