...
bool StaticDataSource::start() {
  ...
  if (lateStart) {
    NES_DEBUG(
        "StaticDataSource::start called while lateStart==true. Will start at "
        "StartSourceEvent. operatorId: {}",
        this->operatorId);
  ...
  lock.unlock();
  NES_DEBUG(
      "StaticDataSource::start called; lateStart==false. Starting now. "
      "operatorId: {}",
      this->operatorId);
...
void StaticDataSource::onEvent(Runtime::BaseEvent& event) {
  ...
  if (event.getEventType() == Runtime::EventType::kStartSourceEvent) {
    ...
    if (startCalled) {
      lock.unlock();
      NES_DEBUG(
          "StaticDataSource::onEvent: start() method was previously called but "
          "delayed. Starting source now.");
      startStaticDataSourceManually();
    } else {
      NES_DEBUG(
          "StaticDataSource::onEvent: Received start event. As soon as "
...
void StaticDataSource::fillBuffer(
  ...
  generatedBuffers++;
  NES_DEBUG(
      "StaticDataSource::fillBuffer: reading finished read {} tuples at "
      "posInFile={}",
