...
StaticDataSource::StaticDataSource(
    SchemaPtr schema, std::string pathTableFile, const bool lateStart,
    ::NES::Runtime::BufferManagerPtr bufferManager,
    ::NES::Runtime::QueryManagerPtr queryManager, OperatorId operatorId,
    OriginId originId, StatisticId statisticId, size_t numSourceLocalBuffers,
    const std::string &physicalSourceName,
    std::vector<::NES::Runtime::Execution::SuccessorExecutablePipeline>
        successors)
    : GeneratorSource(
          std::move(schema), std::move(bufferManager), std::move(queryManager),
          0, // todo  <-- dumb
          operatorId, originId, statisticId, numSourceLocalBuffers,
          GatheringMode::INTERVAL_MODE, // todo: this is a placeholder.
                                        // gathering mode is unnecessary for
                                        // static data.
          std::move(successors), physicalSourceName),
  ...
  NES_DEBUG(
      "StaticDataSource: id {} Initialize source with schema: |{}| size: {}",
      operatorId, this->schema->toString(), std::to_string(tupleSizeInBytes));
  ...
  numberOfBuffersToProduce =
      numTuples / numTuplesPerBuffer + (numTuples % numTuplesPerBuffer != 0);
  ...
  inputParser = std::make_shared<CSVParser>(this->schema->getSize(),
                                            physicalTypes, delimiter);

  NES_DEBUG(
      "StaticDataSource() operatorId: {}: eagerLoading={} "
      "numTuplesToProcess={} numBuffersToProcess={} numTuplesPerBuffer={}",
      operatorId, eagerLoading, numTuples, numberOfBuffersToProduce,
      numTuplesPerBuffer);
...
bool StaticDataSource::start() {
  ...
  startCalled = true;
  if (lateStart) {
    NES_DEBUG("StaticDataSource::start called while lateStart==true. Will "
              "start at StartSourceEvent. operatorId: {}",
              this->operatorId);
    return true; // we didn't start but still signal a success
  }

  lock.unlock();
  NES_DEBUG("StaticDataSource::start called; lateStart==false. Starting now. "
            "operatorId: {}",
            this->operatorId);
...
void StaticDataSource::onEvent(Runtime::BaseEvent &event) {
  ...
  NES_DEBUG("StaticDataSource::onEvent(event) called. operatorId: {}",
            this->operatorId);
  if (event.getEventType() == Runtime::EventType::kStartSourceEvent) {
    NES_DEBUG("StaticDataSource: received startSourceEvent. operatorId: {}",
              this->operatorId);
    if (startCalled) {
      lock.unlock();
      NES_DEBUG("StaticDataSource::onEvent: start() method was previously "
                "called but delayed. Starting source now.");
      startStaticDataSourceManually();
    } else {
      NES_DEBUG("StaticDataSource::onEvent: Received start event. As soon as "
                "DataSource::start is called the source will start.");
      lateStart = false;
    }
  } else {
...
void StaticDataSource::open() {
  ...
  // but we might want to wait for the preloading to finish, for benchmarking
  // reasons:
  if (eagerLoading &&
      this->bufferManager != nullptr // <- has previously been opened
...
std::optional<::NES::Runtime::TupleBuffer> StaticDataSource::receiveData() {
  ...
  if (numberOfBuffersToProduce == numBuffersEmitted ||
      numTuples == numTuplesEmitted) {
    NES_DEBUG(
        "StaticDataSource::receiveData: All data emitted, return nullopt");
    return std::nullopt;
  }
  ...
  if (eagerLoading) {
    ...
    NES_ASSERT2_FMT(!filledBuffers.empty(),
                    "StaticDataSource buffers should be preloaded.");
    auto buffer = filledBuffers.front();
    filledBuffers.erase(filledBuffers.begin());
    return buffer;
  }

  NES_DEBUG("StaticDataSource::receiveData: Read and emit new buffer from {}",
            pathTableFile);
  auto testBuffer = DataSource::allocateBuffer();
  fillBuffer(testBuffer);
...
void StaticDataSource::fillBuffer(
    ::NES::Runtime::MemoryLayouts::TestTupleBuffer &buffer) {
  NES_DEBUG("StaticDataSource::fillBuffer: start at pos={}",
  ...
  NES_ASSERT2_FMT(generatedTuplesThisPass * tupleSizeInBytes <
                      buffer.getBuffer().getBufferSize(),
                  "Wrong parameters");
  NES_DEBUG(
      "StaticDataSource::fillBuffer: fill buffer with #tuples= {}  of size= {}",
      numTuplesPerBuffer, tupleSizeInBytes);
  ...
  generatedBuffers++;
  NES_DEBUG("StaticDataSource::fillBuffer: reading finished read {} tuples at "
            "posInFile={}",
            tupleCount, currentPositionInFile);
  NES_TRACE("StaticDataSource::fillBuffer: read filled buffer={} ",
            Util::printTupleBufferAsCSV(buffer.getBuffer(), schema));
}

std::string StaticDataSource::toString() const {
  std::stringstream ss;
  ss << "STATIC_DATA_SOURCE(SCHEMA(" << schema->toString()
     << "), FILE=" << pathTableFile << " numTuples=" << this->numTuples
     << " numBuff=" << this->numberOfBuffersToProduce << ")";
  return ss.str();
}

NES::SourceType StaticDataSource::getType() const {
