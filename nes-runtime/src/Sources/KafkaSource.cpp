    ...
    , batchSize(batchSize)
{
    ...
    numberOfTuplesPerBuffer = std::floor(double(localBufferManager->getBufferSize()) / double(this->schema->getSchemaSizeInBytes()));
...
{
    NES_INFO(
        "Kafka source {} partition/group={} produced={} batchSize={} successFullPollCnt={} failedFullPollCnt={}",
        topic,
        groupId,
        bufferProducedCnt,
        batchSize,
            ...
            for (auto & message : messages)
            {
                if (tupleCount < tupleBufferCapacity)
                {
                    ...
                    inputParser->writeInputTupleToTupleBuffer(
                        std::string(message.get_payload()), tupleCount, tupleBuffer, schema, localBufferManager);
                    tupleCount++;
                }
                else
                {
...
{
    return SourceType::KAFKA_SOURCE;
}

std::string KafkaSource::getBrokers() const
{
    return brokers;
}

std::string KafkaSource::getTopic() const
{
    return topic;
}

std::string KafkaSource::getOffsetMode() const
{
    return offsetMode;
}

std::string KafkaSource::getGroupId() const
{
    return groupId;
}

uint64_t KafkaSource::getBatchSize() const
{
    return batchSize;
}
