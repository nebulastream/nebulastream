...
KafkaSource::KafkaSource(
    ...
    Runtime::QueryManagerPtr queryManager, uint64_t numberOfBuffersToProduce,
    const std::string brokers, const std::string topic,
    const std::string groupId, bool autoCommit, uint64_t kafkaConsumerTimeout,
    std::string offsetMode, const KafkaSourceTypePtr &kafkaSourceType,
    OriginId originId, StatisticId statisticId, OperatorId operatorId,
    size_t numSourceLocalBuffers, uint64_t batchSize,
    ...
    : DataSource(schema, std::move(bufferManager), std::move(queryManager),
                 ...
                 GatheringMode::INTERVAL_MODE, physicalSourceName,
                 std::move(successors)),
      ...
      offsetMode(offsetMode), batchSize(batchSize) {
  ...
  numberOfTuplesPerBuffer =
      std::floor(double(localBufferManager->getBufferSize()) /
                 double(this->schema->getSchemaSizeInBytes()));
...
KafkaSource::~KafkaSource() {
  NES_INFO("Kafka source {} partition/group={} produced={} batchSize={} "
           "successFullPollCnt={} failedFullPollCnt={}",
           topic, groupId, bufferProducedCnt, batchSize, successFullPollCnt,
           failedFullPollCnt);
}
...
std::optional<Runtime::TupleBuffer> KafkaSource::receiveData() {
  ...
  try {
    ...
    NES_ERROR(
        "KafkaSource::receiveData: Failed to fill the TupleBuffer. Error: {}.",
        e.what());
    throw e;
  }
      ...
      for (auto &message : messages) {
        if (tupleCount < tupleBufferCapacity) {
          ...
          if (message.get_error()) {
            if (!message.is_eof()) {
              NES_ERROR("KafkaSource received error notification: {}",
          ...
          inputParser->writeInputTupleToTupleBuffer(
              std::string(message.get_payload()), tupleCount, tupleBuffer,
              schema, localBufferManager);
          tupleCount++;
        } else {
          // FIXME: how to handle messages that are of size >
          // tupleBufferCapacity NOTE: this will lead to missing tuples as we
          // drop messages that do not fit in the buffer
          NES_ERROR(
              "KafkaSource polled messages do not fit into a single buffer");
          tupleBuffer.setNumberOfTuples(tupleCount);
          return true;
        }
      }
    ...
    if ((bufferFlushIntervalMs > 0 && tupleCount > 0 &&
         std::chrono::duration_cast<std::chrono::milliseconds>(
             std::chrono::system_clock::now() - flushIntervalTimerStart)
                 .count() >= bufferFlushIntervalMs)) {
      NES_DEBUG("KafkaSource::fillBuffer: Reached TupleBuffer flush interval. "
  ...
  if (!connected) {
    ...
    consumer->set_assignment_callback(
        [](const cppkafka::TopicPartitionList &partitions) {
          // TODO do we want to keep this way as a work around for missing
          // toString methods for cppkafka:: ?
          std::stringstream s;
          s << partitions;
    ...
    consumer->assign(vec);
    NES_DEBUG("kafka source={} connect to topic={} partition={}",
              this->operatorId, topic, std::atoi(groupId.c_str()));
...
std::vector<PhysicalTypePtr> KafkaSource::getPhysicalTypes() const {
  return physicalTypes;
}

const KafkaSourceTypePtr &KafkaSource::getSourceConfigPtr() const {
  return sourceConfig;
}
} // namespace NES
#endif
