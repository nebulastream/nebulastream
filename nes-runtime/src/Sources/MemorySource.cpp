...
MemorySource::MemorySource(
    ...
    : GeneratorSource(std::move(schema), std::move(bufferManager),
                      std::move(queryManager), numBuffersToProcess, operatorId,
  ...
  NES_DEBUG("MemorySource() numberOfTuplesToProduce= {}  memoryAreaSize= {} "
...
std::optional<Runtime::TupleBuffer> MemorySource::receiveData() {
  ...
  const auto numTuplesToWrite =
      std::min(tupleCapacityOfMemoryArea, noTuplesPerBuffer);
  ...
  NES_TRACE("MemorySource::receiveData filled buffer with tuples= {}",
