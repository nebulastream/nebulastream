...
BenchmarkSource::BenchmarkSource(
    SchemaPtr schema, const std::shared_ptr<uint8_t> &memoryArea,
    size_t memoryAreaSize, Runtime::BufferManagerPtr bufferManager,
    Runtime::QueryManagerPtr queryManager, uint64_t numberOfBuffersToProcess,
    uint64_t gatheringValue, OperatorId operatorId, OriginId originId,
    StatisticId statisticId, size_t numSourceLocalBuffers,
    GatheringMode gatheringMode, SourceMode sourceMode, uint64_t sourceAffinity,
    uint64_t taskQueueId, const std::string &physicalSourceName,
    std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
    : GeneratorSource(std::move(schema), std::move(bufferManager),
                      std::move(queryManager), numberOfBuffersToProcess,
                      operatorId, originId, statisticId, numSourceLocalBuffers,
                      gatheringMode, std::move(successors), physicalSourceName),
      memoryArea(memoryArea), memoryAreaSize(memoryAreaSize),
      currentPositionInBytes(0), sourceMode(sourceMode) {
  NES_ASSERT(this->memoryArea && this->memoryAreaSize > 0,
             "invalid memory area");
  ...
  if (gatheringMode == GatheringMode::INTERVAL_MODE) {
    ...
    NES_THROW_RUNTIME_ERROR("Mode not implemented "
                            << magic_enum::enum_name(gatheringMode));
  }
  ...
  if (memoryAreaSize <= bufferSize) {
    numberOfTuplesToProduce =
        std::floor(double(memoryAreaSize) / double(this->schemaSize));
  } else {
  ...
  NES_DEBUG("BenchmarkSource() numberOfBuffersToProcess={} memoryAreaSize={}",
            numberOfBuffersToProcess, memoryAreaSize);
}
...
void BenchmarkSource::open() {
  ...
  std::memcpy(numaLocalMemoryArea.getBuffer(), memoryArea.get(),
              memoryAreaSize);
  memoryArea.reset();
  memoryAreaRefCnt = 1;
}
    ...
    for (uint64_t i = 0; i < numberOfBuffersToProduce && running; ++i) {
      ...
      case SourceMode::COPY_BUFFER_SIMD_RTE: {
        ...
        NES_THROW_RUNTIME_ERROR(
            "COPY_BUFFER_SIMD_RTE source mode is not supported.");
#endif
        break;
      }
      ...
      case SourceMode::CACHE_COPY: {
        buffer = bufferManager->getBufferBlocking();
        memcpy(buffer.getBuffer(), numaLocalMemoryArea.getBuffer(),
               buffer.getBufferSize());
        break;
      }
      ...
      case SourceMode::WRAP_BUFFER: {
        buffer = Runtime::TupleBuffer::wrapMemory(
            numaLocalMemoryArea.getBuffer() + currentPositionInBytes,
            bufferSize, this);
...
std::optional<Runtime::TupleBuffer> BenchmarkSource::receiveData() {
  // the benchmark source does not follow the receive pattern and overwrites the
  // running routine
  NES_NOT_IMPLEMENTED();
  Runtime::TupleBuffer buffer;
  return buffer;
