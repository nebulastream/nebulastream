...
MonitoringSource::MonitoringSource(
    ...
    std::chrono::milliseconds waitTime, Runtime::BufferManagerPtr bufferManager,
    ...
    : DataSource(Schema::create(), bufferManager, queryManager, operatorId,
                 originId, statisticId, numSourceLocalBuffers,
                 GatheringMode::INTERVAL_MODE, physicalSourceName, successors),
...
std::optional<Runtime::TupleBuffer> MonitoringSource::receiveData() {
  ...
  NES_TRACE("MonitoringSource: Generated buffer with{} tuple and size {}",
            buf.getNumberOfTuples(), schema->getSchemaSizeInBytes());
  ...
  if (Logger::getInstance()->getCurrentLogLevel() == LogLevel::LOG_TRACE) {
    auto layout =
        Runtime::MemoryLayouts::RowLayout::create(schema, buf.getBufferSize());
