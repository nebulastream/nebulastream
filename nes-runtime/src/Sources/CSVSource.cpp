...
CSVSource::CSVSource(
    ...
    Runtime::QueryManagerPtr queryManager, CSVSourceTypePtr csvSourceType,
    ...
    : DataSource(schema, std::move(bufferManager), std::move(queryManager),
                 ...
                 gatheringMode, physicalSourceName, std::move(successors)),
      fileEnded(false),
      csvSourceType(csvSourceType),
      filePath(csvSourceType->getFilePath()->getValue()),
      numberOfTuplesToProducePerBuffer(
          csvSourceType->getNumberOfTuplesToProducePerBuffer()->getValue()),
  ...
  this->gatheringInterval = std::chrono::milliseconds(
      csvSourceType->getGatheringInterval()->getValue());
  this->tupleSize = schema->getSchemaSizeInBytes();

  struct Deleter {
    void operator()(const char* ptr) { std::free(const_cast<char*>(ptr)); }
  };
  auto path = std::unique_ptr<const char, Deleter>(
      const_cast<const char*>(realpath(filePath.c_str(), nullptr)));
  if (path == nullptr) {
    NES_THROW_RUNTIME_ERROR(
        "Could not determine absolute pathname: " << filePath.c_str());
  }

  input.open(path.get());
  if (!(input.is_open() && input.good())) {
    throw Exceptions::RuntimeException("Cannot open file: " +
                                       std::string(path.get()));
  }
  ...
  if (auto const reportedFileSize = input.tellg(); reportedFileSize == -1) {
    throw Exceptions::RuntimeException("CSVSource::CSVSource File " + filePath +
                                       " is corrupted");
  } else {
  ...
  NES_DEBUG(
      "CSVSource: tupleSize={} freq={}ms numBuff={} "
      "numberOfTuplesToProducePerBuffer={}",
      this->tupleSize, this->gatheringInterval.count(),
      this->numberOfBuffersToProduce, this->numberOfTuplesToProducePerBuffer);
  ...
  this->inputParser =
      std::make_shared<CSVParser>(schema->getSize(), physicalTypes, delimiter);
}
...
std::optional<Runtime::TupleBuffer> CSVSource::receiveData() {
  ...
  fillBuffer(buffer);
  NES_TRACE("CSVSource::receiveData filled buffer with tuples= {}",
...
std::string CSVSource::toString() const {
  ...
  ss << "CSV_SOURCE(SCHEMA(" << schema->toString() << "), FILE=" << filePath
     << " freq=" << this->gatheringInterval.count() << "ms"
     << " numBuff=" << this->numberOfBuffersToProduce << ")";
  return ss.str();
}

void CSVSource::fillBuffer(Runtime::MemoryLayouts::TestTupleBuffer& buffer) {
  NES_TRACE("CSVSource::fillBuffer: start at pos={} fileSize={}",
            currentPositionInFile, fileSize);
  if (this->fileEnded) {
  ...
  if (numberOfTuplesToProducePerBuffer == 0) {
    ...
    NES_ASSERT2_FMT(generatedTuplesThisPass * tupleSize <
                        buffer.getBuffer().getBufferSize(),
                    "Wrong parameters");
  }
  NES_TRACE("CSVSource::fillBuffer: fill buffer with #tuples={} of size={}",
            generatedTuplesThisPass, tupleSize);
  ...
  while (tupleCount < generatedTuplesThisPass) {
    ...
    if (auto const tg = input.tellg();
        (tg >= 0 && static_cast<uint64_t>(tg) >= fileSize) || tg == -1) {
      NES_TRACE("CSVSource::fillBuffer: reset tellg()={} fileSize={}",
                input.tellg(), fileSize);
      input.clear();
    ...
    // TODO: there will be a problem with non-printable characters (at least
    // with null terminators). Check sources
  ...
  generatedBuffers++;
  NES_TRACE(
      "CSVSource::fillBuffer: reading finished read {} tuples at posInFile={}",
      tupleCount, currentPositionInFile);
  NES_TRACE("CSVSource::fillBuffer: read produced buffer=  {}",
