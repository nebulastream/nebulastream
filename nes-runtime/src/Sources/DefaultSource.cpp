...
DefaultSource::DefaultSource(
    ...
    const uint64_t numberOfBufferToProduce, uint64_t gatheringInterval,
    ...
    : GeneratorSource(std::move(schema), std::move(bufferManager),
                      std::move(queryManager), numberOfBufferToProduce,
                      operatorId, originId, statisticId, numSourceLocalBuffers,
                      GatheringMode::INTERVAL_MODE, std::move(successors),
                      physicalSourceName) {
...
std::optional<Runtime::TupleBuffer> DefaultSource::receiveData() {
  ...
  NES_TRACE("Source: id={} Generated buffer with {} / {}", operatorId,
