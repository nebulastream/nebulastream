...
bool OPCSource::connect()
{
    ...
    if (!connected)
    {
        ...
        if (retval != UA_STATUSCODE_GOOD)
        {
            ...
            connected = false;
            NES_ERROR("OPCSOURCE::connect(): ERROR with Status Code: {} OPCSOURCE {}: set connected false", retval, this->toString());
        }
        else
        {
            connected = true;
...
SourceType OPCSource::getType() const
{
    return SourceType::OPC_SOURCE;
}

std::string OPCSource::getUrl() const
{
    return url;
}
...
std::string OPCSource::getUser() const
{
    return user;
}

std::string OPCSource::getPassword() const
{
    return password;
}
