...
OPCSource::~OPCSource() {
  ...
  if (success) {
    ...
    NES_FATAL_ERROR(
        "OPCSOURCE {} : Destroy OPC Source failed cause it could not be "
        "disconnected",
        this->toString());
  }
}
...
std::optional<Runtime::TupleBuffer> OPCSource::receiveData() {
  ...
  if (connect()) {
    ...
    if (retval == UA_STATUSCODE_GOOD && UA_Variant_isScalar(val)) {
      ...
      UA_delete(val, val->type);
      NES_ERROR(
          "OPCSOURCE::receiveData() error: Could not retrieve data. Further "
          "inspection needed.");
      return std::nullopt;
    }
...
bool OPCSource::connect() {
  ...
  if (!connected) {
    ...
    if (retval != UA_STATUSCODE_GOOD) {
      ...
      connected = false;
      NES_ERROR(
          "OPCSOURCE::connect(): ERROR with Status Code: {} OPCSOURCE {}: set "
          "connected false",
