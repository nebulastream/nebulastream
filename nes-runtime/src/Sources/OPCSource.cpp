...
OPCSource::OPCSource(
    ...
    Runtime::QueryManagerPtr queryManager, const std::string &url,
  ...
  NES_DEBUG("OPCSOURCE {} : Init OPC Source to  {}  with user and password.",
            this->toString(), url);
}
...
OPCSource::~OPCSource() {
  ...
  if (success) {
    ...
    NES_FATAL_ERROR("OPCSOURCE {} : Destroy OPC Source failed cause it could "
                    "not be disconnected",
                    this->toString());
  }
}
...
std::optional<Runtime::TupleBuffer> OPCSource::receiveData() {
  ...
  if (connect()) {
    ...
    if (retval == UA_STATUSCODE_GOOD && UA_Variant_isScalar(val)) {
      NES_DEBUG("OPCSOURCE::receiveData() Value datatype is: {}",
                val->type->typeName);
      ...
      UA_delete(val, val->type);
      NES_ERROR("OPCSOURCE::receiveData() error: Could not retrieve data. "
...
bool OPCSource::connect() {
  ...
  if (!connected) {
    ...
    NES_DEBUG("OPCSOURCE::connect(): was !conncect now connect {}: connected",
              this->toString());
    ...
    if (retval != UA_STATUSCODE_GOOD) {
      ...
      connected = false;
      NES_ERROR("OPCSOURCE::connect(): ERROR with Status Code: {} OPCSOURCE "
                "{}: set connected false",
  ...
  if (connected) {
    ...
    NES_DEBUG("Exception: OPCSOURCE::connect():  {}: NOT connected",
              this->toString());
  }
  return connected;
}
...
bool OPCSource::disconnect() {
  ...
  if (!connected) {
    ...
    NES_DEBUG("OPCSOURCE::disconnect()  {}: NOT disconnected",
              this->toString());
  }
  return !connected;
}
