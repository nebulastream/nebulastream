...
TCPSource::TCPSource(
    ...
    Runtime::QueryManagerPtr queryManager, TCPSourceTypePtr tcpSourceType,
    ...
    : DataSource(schema, std::move(bufferManager), std::move(queryManager),
                 operatorId, originId, statisticId, numSourceLocalBuffers,
                 gatheringMode, physicalSourceName,
  ...
  // Extracting the schema keys in order to parse incoming data correctly (e.g.
  // use as keys for JSON objects) Also, extracting the field types in order to
  // parse and cast the values of incoming data to the correct types
  for (const auto &field : schema->fields) {
    ...
    schemaKeys.push_back(
        fieldName.substr(fieldName.find('$') + 1, fieldName.size()));
  }

  switch (sourceConfig->getInputFormat()->getValue()) {
  case Configurations::InputFormat::JSON:
    inputParser = std::make_unique<JSONParser>(schema->getSize(), schemaKeys,
                                               physicalTypes);
    break;
  case Configurations::InputFormat::CSV:
    inputParser =
        std::make_unique<CSVParser>(schema->getSize(), physicalTypes, ",");
    break;
...
void TCPSource::open() {
  ...
  if (sockfd < 0) {
    sockfd = socket(sourceConfig->getSocketDomain()->getValue(),
                    sourceConfig->getSocketType()->getValue(), 0);
    NES_TRACE("Socket created with  {}", sockfd);
  }
  if (sockfd < 0) {
    NES_ERROR("TCPSource::connected: Failed to create socket. Error: {}",
              strerror(errno));
    connection = -1;
    return;
  }
  ...
  servaddr.sin_addr.s_addr =
      inet_addr(sourceConfig->getSocketHost()->getValue().c_str());
  servaddr.sin_port = htons(sourceConfig->getSocketPort()
                                ->getValue()); // htons is necessary to convert
                                               // a number to network byte order
  ...
  if (connection < 0) {
    ...
    NES_THROW_RUNTIME_ERROR(
        "TCPSource::connected: Connection with server failed. Error: "
        << strerror(errno));
  }
...
std::optional<Runtime::TupleBuffer> TCPSource::receiveData() {
  ...
  try {
    ...
    NES_ERROR(
        "TCPSource::receiveData: Failed to fill the TupleBuffer. Error: {}.",
        e.what());
    throw e;
  }
...
std::pair<bool, size_t> sizeUntilSearchToken(SPAN_TYPE<const char> data,
                                             char token) {
  auto result = std::find_if(data.begin(), data.end(),
                             [token](auto &c) { return c == token; });
...
size_t binaryBufferSize(SPAN_TYPE<const char> data) {
  ...
  NES_ASSERT(data.size() <= sizeof(size_t),
             "Not implemented for " << data.size() << "socket buffer size");
...
size_t TCPSource::parseBufferSize(SPAN_TYPE<const char> data) const {
  if (sourceConfig->getInputFormat()->getValue() ==
      Configurations::InputFormat::NES_BINARY) {
...
bool TCPSource::fillBuffer(
  ...
  NES_DEBUG("TCPSource::fillBuffer: Fill buffer with #tuples= {}  of size= {}",
            tuplesThisPass, tupleSize);
  ...
  while (tupleCount < tuplesThisPass && !flushIntervalPassed) {
    ...
    if (!circularBuffer.full()) {
      ...
      if (bufferSizeReceived == -1) {
        NES_ERROR("TCPSource::fillBuffer: an error occurred while reading from "
                  "socket. Error: {}",
                  strerror(errno));
        return false;
      }
    ...
    if (!circularBuffer.empty()) {
      ...
      // Every protocol returns a view into the tuple (or Buffer for Binary)
      // memory in tupleData; switch case depends on the message receiving that
      // was chosen when creating the source. Three choices are available:
      switch (sourceConfig->getDecideMessageSize()->getValue()) {
      // The user inputted a tuple separator that indicates the end of a tuple.
      // We're going to search for that tuple seperator and assume that all data
      // until then belongs to the current tuple
      case Configurations::TCPDecideMessageSize::TUPLE_SEPARATOR: {
        // search the circularBuffer until Tuple seperator is found to obtain
        // size of tuple
        auto [foundSeparator, inputTupleSize] = sizeUntilSearchToken(
            reader, this->sourceConfig->getTupleSeparator()->getValue());
        ...
        reader.consume(
            sizeof(this->sourceConfig->getTupleSeparator()->getValue()));
        break;
      }
      ...
      // Before each message, the server uses a fixed number of bytes
      // (bytesUsedForSocketBufferSizeTransfer) to indicate the size of the next
      // tuple.
      case Configurations::TCPDecideMessageSize::BUFFER_SIZE_FROM_SOCKET: {
        // Tuple (or Buffer for Binary) Size preceds the actual data.
        // Peek BytesUserForSocketBufferSize so if the buffer contains not
        // enough bytes the next iteration does not loose the tuple size
        // information.
        auto bufferSizeSize =
        ...
        auto bufferSizeMemory =
            SPAN_TYPE<const char>(reader).subspan(0, bufferSizeSize);
        auto size = parseBufferSize(bufferSizeMemory);
        NES_TRACE("tuple size from socket: {}", size);
        if (reader.size() < bufferSizeSize + size) {
          NES_TRACE("Waiting for data current {}",
                    reader.size() - bufferSizeSize);
          break;
        }

        // Consume the size and data iff a complete tuple (or Buffer) is
        // available.
      ...
      // if we were able to obtain a complete tuple from the circular buffer, we
      // are going to forward it ot the appropriate parser
      if (!tupleData.empty()) {
        std::string_view buf(tupleData.data(), tupleData.size());
        if (sourceConfig->getInputFormat()->getValue() ==
            Configurations::InputFormat::NES_BINARY) {
          inputParser->writeInputTupleToTupleBuffer(
              buf, tupleCount, tupleBuffer, schema, localBufferManager);
          tupleCount = tupleBuffer.getNumberOfTuples();
        } else {
          NES_TRACE("TCPSOURCE::fillBuffer: Client consume message: '{}'.",
                    buf);
          inputParser->writeInputTupleToTupleBuffer(
              buf, tupleCount, tupleBuffer, schema, localBufferManager);
          tupleCount++;
    ...
    // If bufferFlushIntervalMs was defined by the user (> 0), we check whether
    // the time on receiving and writing data exceeds the user defined limit
    // (bufferFlushIntervalMs). If so, we flush the current TupleBuffer(TB) and
    // proceed with the next TB.
    if ((sourceConfig->getFlushIntervalMS()->getValue() > 0 &&
         std::chrono::duration_cast<std::chrono::milliseconds>(
             std::chrono::system_clock::now() - flushIntervalTimerStart)
                 .count() >= sourceConfig->getFlushIntervalMS()->getValue())) {
      NES_DEBUG("TCPSource::fillBuffer: Reached TupleBuffer flush interval. "
                "Finishing writing to current TupleBuffer.");
