...
void TCPSource::open() {
  ...
  servaddr.sin_port =
      htons(sourceConfig->getSocketPort()
                ->getValue());  // htons is necessary to convert a number to
                                // network byte order
  ...
  while (tupleCount < tuplesThisPass && !flushIntervalPassed) {
    ...
    if (!circularBuffer.full()) {
      ...
      if (bufferSizeReceived == -1) {
        NES_ERROR(
            "TCPSource::fillBuffer: an error occurred while reading from "
            "socket. Error: {}",
            strerror(errno));
        return false;
      }
    ...
    if (!circularBuffer.empty()) {
      ...
      switch (sourceConfig->getDecideMessageSize()->getValue()) {
        // The user inputted a tuple separator that indicates the end of a
        // tuple. We're going to search for that tuple seperator and assume that
        // all data until then belongs to the current tuple
        ...
        // (bytesUsedForSocketBufferSizeTransfer) to indicate the size of the
        // next tuple.
        case Configurations::TCPDecideMessageSize::BUFFER_SIZE_FROM_SOCKET: {
          ...
          // information.
          auto bufferSizeSize =
              sourceConfig->getBytesUsedForSocketBufferSizeTransfer()
                  ->getValue();
    ...
    if ((sourceConfig->getFlushIntervalMS()->getValue() > 0 &&
      ...
      NES_DEBUG(
          "TCPSource::fillBuffer: Reached TupleBuffer flush interval. "
