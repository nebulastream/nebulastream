...
void writeToBuffer(const uint64_t &metrics, Runtime::TupleBuffer &, uint64_t) {
  NES_THROW_RUNTIME_ERROR(
      "Metric: Serialization for uint64_t not possible for metric " << metrics);
}

void writeToBuffer(const std::string &metrics, Runtime::TupleBuffer &,
                   uint64_t) {
  NES_THROW_RUNTIME_ERROR(
      "Metric: Serialization for std::string not possible for metric "
      << metrics);
}
