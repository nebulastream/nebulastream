...
CpuMetricsWrapper::CpuMetricsWrapper(std::vector<CpuMetrics> &&arr)
  ...
  if (!arr.empty()) {
    ...
    NES_THROW_RUNTIME_ERROR("CpuMetricsWrapper: Object cannot be allocated "
                            "with less than 0 cores.");
  }
  NES_TRACE("CpuMetricsWrapper: Allocating memory for {} metrics.",
            std::to_string(arr.size()));
}

CpuMetrics CpuMetricsWrapper::getValue(const unsigned int cpuCore) const {
  return cpuMetrics.at(cpuCore);
}

void CpuMetricsWrapper::writeToBuffer(Runtime::TupleBuffer &buf,
  ...
  if (schema->getSchemaSizeInBytes() > buf.getBufferSize()) {
    NES_ERROR("CpuMetricsWrapper: At least one tuple of CpuMetrics has to fit "
              "into buffer");
  } else if (totalSize > buf.getBufferSize()) {
    NES_WARNING("CpuMetricsWrapper: Content does not fit in TupleBuffer "
...
void CpuMetricsWrapper::readFromBuffer(Runtime::TupleBuffer &buf,
                                       uint64_t tupleIndex) {
  auto schema = CpuMetrics::getSchema("");
  auto cpuList = std::vector<CpuMetrics>();
  NES_TRACE("CpuMetricsWrapper: Parsing buffer with number of tuples {}",
...
bool CpuMetricsWrapper::operator==(const CpuMetricsWrapper &rhs) const {
  if (cpuMetrics.size() != rhs.size()) {
    NES_ERROR("CpuMetricsWrapper: Sizes are not equal {}!={}",
              cpuMetrics.size(), rhs.size());
    return false;
  }
...
void writeToBuffer(const CpuMetricsWrapper &metrics, Runtime::TupleBuffer &buf,
                   uint64_t tupleIndex) {
  metrics.writeToBuffer(buf, tupleIndex);
}

void readFromBuffer(CpuMetricsWrapper &wrapper, Runtime::TupleBuffer &buf,
                    uint64_t tupleIndex) {
