...
void CpuMetricsWrapper::writeToBuffer(Runtime::TupleBuffer& buf, uint64_t tupleIndex) const
{
    ...
    {
        NES_WARNING(
            "CpuMetricsWrapper: Content does not fit in TupleBuffer totalSize: {} > getBufferSize:{}", totalSize, buf.getBufferSize());
    }
...
uint64_t CpuMetricsWrapper::size() const
{
    return cpuMetrics.size();
}

CpuMetrics CpuMetricsWrapper::getTotal() const
{
    return getValue(0);
}
...
WorkerId CpuMetricsWrapper::getNodeId() const
{
    return nodeId;
}
