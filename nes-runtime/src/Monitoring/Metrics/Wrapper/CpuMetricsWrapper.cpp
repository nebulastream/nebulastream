...
CpuMetricsWrapper::CpuMetricsWrapper(std::vector<CpuMetrics>&& arr)
  ...
  if (!arr.empty()) {
    ...
    NES_THROW_RUNTIME_ERROR(
        "CpuMetricsWrapper: Object cannot be allocated with less than 0 "
        "cores.");
  }
...
void CpuMetricsWrapper::writeToBuffer(Runtime::TupleBuffer& buf,
  ...
  if (schema->getSchemaSizeInBytes() > buf.getBufferSize()) {
    NES_ERROR(
        "CpuMetricsWrapper: At least one tuple of CpuMetrics has to fit into "
        "buffer");
  } else if (totalSize > buf.getBufferSize()) {
    NES_WARNING(
        "CpuMetricsWrapper: Content does not fit in TupleBuffer totalSize: {} "
        "> getBufferSize:{}",
