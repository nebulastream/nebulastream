...
NetworkMetricsWrapper::NetworkMetricsWrapper()
    : NetworkMetricsWrapper(INVALID_WORKER_NODE_ID) {}
...
NetworkMetricsWrapper::NetworkMetricsWrapper(std::vector<NetworkMetrics> &&arr)
  ...
  if (!arr.empty()) {
    ...
    NES_THROW_RUNTIME_ERROR("NetworkMetricsWrapper: Object cannot be allocated "
                            "with less than 0 cores.");
  }
  NES_TRACE("NetworkMetricsWrapper: Allocating memory for {} metrics.",
            std::to_string(arr.size()));
}

void NetworkMetricsWrapper::writeToBuffer(Runtime::TupleBuffer &buf,
  ...
  if (schema->getSchemaSizeInBytes() > buf.getBufferSize()) {
    NES_ERROR("NetworkMetricsWrapper: At least one tuple of NetworkMetrics has "
              "to fit into buffer");
  } else if (totalSize > buf.getBufferSize()) {
    NES_WARNING("NetworkMetricsWrapper: Content does not fit in TupleBuffer "
...
void NetworkMetricsWrapper::readFromBuffer(Runtime::TupleBuffer &buf,
                                           uint64_t tupleIndex) {
  auto schema = NetworkMetrics::getSchema("");
  auto interfaceList = std::vector<NetworkMetrics>();
  NES_TRACE("NetworkMetricsWrapper: Parsing buffer with number of tuples {}",
...
NetworkMetrics
NetworkMetricsWrapper::getNetworkValue(uint64_t interfaceNo) const {
  if (interfaceNo >= size()) {
    NES_THROW_RUNTIME_ERROR(
        "NetworkMetricsWrapper: ArrayType index out of bound " +
        std::to_string(interfaceNo) + ">=" + std::to_string(size()));
  }
...
bool NetworkMetricsWrapper::operator==(const NetworkMetricsWrapper &rhs) const {
  ...
  for (auto i = static_cast<decltype(networkMetrics)::size_type>(0);
       i < networkMetrics.size(); ++i) {
...
void readFromBuffer(NetworkMetricsWrapper &metrics, Runtime::TupleBuffer &buf,
                    uint64_t tupleIndex) {
