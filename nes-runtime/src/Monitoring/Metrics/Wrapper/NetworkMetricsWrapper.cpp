...
NetworkMetricsWrapper::NetworkMetricsWrapper(WorkerId nodeId)
    : nodeId(nodeId), timestamp(duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count())
{
}
...
void NetworkMetricsWrapper::writeToBuffer(Runtime::TupleBuffer& buf, uint64_t tupleIndex) const
{
    ...
    {
        NES_WARNING(
            "NetworkMetricsWrapper: Content does not fit in TupleBuffer totalSize: {} > getBufferSize: {}", totalSize, buf.getBufferSize());
    }
...
NetworkMetrics NetworkMetricsWrapper::getNetworkValue(uint64_t interfaceNo) const
{
    if (interfaceNo >= size())
    {
        NES_THROW_RUNTIME_ERROR(
            "NetworkMetricsWrapper: ArrayType index out of bound " + std::to_string(interfaceNo) + ">=" + std::to_string(size()));
    }
...
WorkerId NetworkMetricsWrapper::getNodeId() const
{
    return nodeId;
}
