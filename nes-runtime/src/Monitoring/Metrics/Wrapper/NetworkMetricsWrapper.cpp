...
NetworkMetricsWrapper::NetworkMetricsWrapper(std::vector<NetworkMetrics>&& arr)
  ...
  if (!arr.empty()) {
    ...
    NES_THROW_RUNTIME_ERROR(
        "NetworkMetricsWrapper: Object cannot be allocated with less than 0 "
        "cores.");
  }
...
void NetworkMetricsWrapper::writeToBuffer(Runtime::TupleBuffer& buf,
  ...
  if (schema->getSchemaSizeInBytes() > buf.getBufferSize()) {
    NES_ERROR(
        "NetworkMetricsWrapper: At least one tuple of NetworkMetrics has to "
        "fit into buffer");
  } else if (totalSize > buf.getBufferSize()) {
    NES_WARNING(
        "NetworkMetricsWrapper: Content does not fit in TupleBuffer totalSize: "
        "{} > getBufferSize: {}",
...
NetworkMetrics NetworkMetricsWrapper::getNetworkValue(
    uint64_t interfaceNo) const {
  if (interfaceNo >= size()) {
    NES_THROW_RUNTIME_ERROR(
