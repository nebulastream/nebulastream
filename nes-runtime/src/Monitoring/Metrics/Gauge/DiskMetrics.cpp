...
DiskMetrics::DiskMetrics()
    : nodeId(0), timestamp(0), fBsize(0), fFrsize(0), fBlocks(0), fBfree(0),
      fBavail(0) {}

Configurations::SchemaTypePtr
DiskMetrics::getSchemaType(const std::string &prefix) {
...
void DiskMetrics::writeToBuffer(Runtime::TupleBuffer &buf,
                                uint64_t tupleIndex) const {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      DiskMetrics::getSchema(""), buf.getBufferSize());
  ...
  NES_ASSERT(totalSize <= buf.getBufferSize(),
             "DiskMetrics: Content does not fit in TupleBuffer totalSize:" +
...
void DiskMetrics::readFromBuffer(Runtime::TupleBuffer &buf,
                                 uint64_t tupleIndex) {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      DiskMetrics::getSchema(""), buf.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
...
bool DiskMetrics::operator==(const DiskMetrics &rhs) const {
  return nodeId == rhs.nodeId && timestamp == rhs.timestamp &&
         fBavail == rhs.fBavail && fBfree == rhs.fBfree &&
         fBlocks == rhs.fBlocks && fBsize == rhs.fBsize &&
         fFrsize == rhs.fFrsize;
}
...
void writeToBuffer(const DiskMetrics &metric, Runtime::TupleBuffer &buf,
                   uint64_t tupleIndex) {
