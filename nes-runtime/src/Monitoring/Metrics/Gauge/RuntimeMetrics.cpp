...
RuntimeMetrics::RuntimeMetrics()
    : nodeId(0), wallTimeNs(0), memoryUsageInBytes(0), cpuLoadInJiffies(0),
      blkioBytesRead(0), blkioBytesWritten(0), batteryStatusInPercent(0),
      latCoord(0), longCoord(0) {
  NES_DEBUG("RuntimeMetrics: Default ctor");
}

Configurations::SchemaTypePtr
RuntimeMetrics::getSchemaType(const std::string &prefix) {

  std::vector<Configurations::SchemaFieldDetail> schemaFiledDetails;
  const char *length = "0";
  const char *unsignedIntegerDataType = "UINT64";
  schemaFiledDetails.emplace_back(prefix + "node_id", unsignedIntegerDataType,
                                  length);
  schemaFiledDetails.emplace_back(prefix + "wallTimeNs",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "memoryUsageInBytes",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "cpuLoadInJiffies",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "blkioBytesRead",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "blkioBytesWritten",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "batteryStatusInPercent",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "latCoord", unsignedIntegerDataType,
                                  length);
  schemaFiledDetails.emplace_back(prefix + "longCoord", unsignedIntegerDataType,
                                  length);
...
void RuntimeMetrics::writeToBuffer(Runtime::TupleBuffer &buf,
                                   uint64_t tupleIndex) const {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      RuntimeMetrics::getSchema(""), buf.getBufferSize());
  ...
  NES_ASSERT(totalSize <= buf.getBufferSize(),
             "RuntimeMetrics: Content does not fit in TupleBuffer totalSize:" +
...
void RuntimeMetrics::readFromBuffer(Runtime::TupleBuffer &buf,
                                    uint64_t tupleIndex) {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      RuntimeMetrics::getSchema(""), buf.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
...
bool RuntimeMetrics::operator==(const RuntimeMetrics &rhs) const {
  return nodeId == rhs.nodeId && wallTimeNs == rhs.wallTimeNs &&
         memoryUsageInBytes == rhs.memoryUsageInBytes &&
         cpuLoadInJiffies == rhs.cpuLoadInJiffies &&
         blkioBytesRead == rhs.blkioBytesRead &&
         blkioBytesWritten == rhs.blkioBytesWritten &&
...
void writeToBuffer(const RuntimeMetrics &metrics, Runtime::TupleBuffer &buf,
                   uint64_t tupleIndex) {
  metrics.writeToBuffer(buf, tupleIndex);
}

void readFromBuffer(RuntimeMetrics &metrics, Runtime::TupleBuffer &buf,
                    uint64_t tupleIndex) {
