...
RegistrationMetrics::RegistrationMetrics()
    : nodeId(0), totalMemoryBytes(0), cpuCoreNum(0), totalCPUJiffies(0),
      cpuPeriodUS(0), cpuQuotaUS(0), isMoving(false), hasBattery(false) {
  NES_DEBUG("RegistrationMetrics: Default ctor");
}

RegistrationMetrics::RegistrationMetrics(bool isMoving, bool hasBattery)
    : nodeId(0), totalMemoryBytes(0), cpuCoreNum(0), totalCPUJiffies(0),
      cpuPeriodUS(0), cpuQuotaUS(0), isMoving(isMoving),
      hasBattery(hasBattery) {
  NES_DEBUG("RegistrationMetrics: Init with flag moving:{}, hasBattery:{}",
            std::to_string(isMoving), std::to_string(hasBattery));
}

RegistrationMetrics::RegistrationMetrics(
    const SerializableRegistrationMetrics &metrics)
    : nodeId(0), totalMemoryBytes(metrics.totalmemorybytes()),
      cpuCoreNum(metrics.cpucorenum()),
      totalCPUJiffies(metrics.totalcpujiffies()),
      cpuPeriodUS(metrics.cpuperiodus()), cpuQuotaUS(metrics.cpuquotaus()),
...
RegistrationMetrics::getSchemaType(const std::string &prefix) {
  ...
  schemaFiledDetails.emplace_back(prefix + "node_id", unsignedIntegerDataType,
                                  length);
  schemaFiledDetails.emplace_back(prefix + "totalMemoryBytes",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "cpuCoreNum",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "totalCPUJiffies",
                                  unsignedIntegerDataType, length);
  schemaFiledDetails.emplace_back(prefix + "cpuPeriodUS", integerDataType,
                                  length);
  schemaFiledDetails.emplace_back(prefix + "cpuQuotaUS", integerDataType,
                                  length);
  schemaFiledDetails.emplace_back(prefix + "isMoving", booleanDataType, length);
  schemaFiledDetails.emplace_back(prefix + "hasBattery", booleanDataType,
                                  length);
...
void RegistrationMetrics::writeToBuffer(Runtime::TupleBuffer &buf,
                                        uint64_t tupleIndex) const {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      RegistrationMetrics::getSchema(""), buf.getBufferSize());
  ...
  NES_ASSERT(
      ...
      "RegistrationMetrics: Content does not fit in TupleBuffer totalSize:" +
...
void RegistrationMetrics::readFromBuffer(Runtime::TupleBuffer &buf,
                                         uint64_t tupleIndex) {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      RegistrationMetrics::getSchema(""), buf.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
...
bool RegistrationMetrics::operator==(const RegistrationMetrics &rhs) const {
  return nodeId == rhs.nodeId && totalMemoryBytes == rhs.totalMemoryBytes &&
         cpuCoreNum == rhs.cpuCoreNum &&
         totalCPUJiffies == rhs.totalCPUJiffies &&
         cpuPeriodUS == rhs.cpuPeriodUS && cpuQuotaUS == rhs.cpuQuotaUS &&
