...
MemoryMetrics::MemoryMetrics()
    : nodeId(0), timestamp(0), TOTAL_RAM(0), TOTAL_SWAP(0), FREE_RAM(0),
      SHARED_RAM(0), BUFFER_RAM(0), FREE_SWAP(0), TOTAL_HIGH(0), FREE_HIGH(0),
      PROCS(0), MEM_UNIT(0), LOADS_1MIN(0), LOADS_5MIN(0), LOADS_15MIN(0) {}

Configurations::SchemaTypePtr
MemoryMetrics::getSchemaType(const std::string &prefix) {
...
void MemoryMetrics::writeToBuffer(Runtime::TupleBuffer &buf,
                                  uint64_t tupleIndex) const {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      MemoryMetrics::getSchema(""), buf.getBufferSize());
  ...
  NES_ASSERT(totalSize <= buf.getBufferSize(),
             "MemoryMetrics: Content does not fit in TupleBuffer totalSize:" +
...
void MemoryMetrics::readFromBuffer(Runtime::TupleBuffer &buf,
                                   uint64_t tupleIndex) {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      MemoryMetrics::getSchema(""), buf.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
...
bool MemoryMetrics::operator==(const MemoryMetrics &rhs) const {
  return nodeId == rhs.nodeId && timestamp == rhs.timestamp &&
         TOTAL_RAM == rhs.TOTAL_RAM && TOTAL_SWAP == rhs.TOTAL_SWAP &&
         FREE_RAM == rhs.FREE_RAM && SHARED_RAM == rhs.SHARED_RAM &&
         BUFFER_RAM == rhs.BUFFER_RAM && FREE_SWAP == rhs.FREE_SWAP &&
         TOTAL_HIGH == rhs.TOTAL_HIGH && FREE_HIGH == rhs.FREE_HIGH &&
         PROCS == rhs.PROCS && MEM_UNIT == rhs.MEM_UNIT &&
         LOADS_1MIN == rhs.LOADS_1MIN && LOADS_5MIN == rhs.LOADS_5MIN &&
...
void writeToBuffer(const MemoryMetrics &metrics, Runtime::TupleBuffer &buf,
                   uint64_t tupleIndex) {
  metrics.writeToBuffer(buf, tupleIndex);
}

void readFromBuffer(MemoryMetrics &metrics, Runtime::TupleBuffer &buf,
                    uint64_t tupleIndex) {
