...
CpuMetrics::CpuMetrics()
    : nodeId(0), timestamp(0), coreNum(0), user(0), nice(0), system(0), idle(0),
      iowait(0), irq(0), softirq(0), steal(0), guest(0), guestnice(0) {}

Configurations::SchemaTypePtr
CpuMetrics::getSchemaType(const std::string &prefix) {
...
void CpuMetrics::writeToBuffer(Runtime::TupleBuffer &buf,
                               uint64_t tupleIndex) const {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      CpuMetrics::getSchema(""), buf.getBufferSize());
  ...
  NES_ASSERT(totalSize <= buf.getBufferSize(),
             "CpuMetrics: Content does not fit in TupleBuffer totalSize:" +
...
void CpuMetrics::readFromBuffer(Runtime::TupleBuffer &buf,
                                uint64_t tupleIndex) {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      CpuMetrics::getSchema(""), buf.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
...
std::ostream &operator<<(std::ostream &os, const CpuMetrics &values) {
  os << "nodeId: " << values.nodeId << "timestamp:" << values.timestamp
     << "coreNum: " << values.coreNum << "user: " << values.user
     << " nice: " << values.nice << " system: " << values.system
     << " idle: " << values.idle << " iowait: " << values.iowait
     << " irq: " << values.irq << " softirq: " << values.softirq
     << " steal: " << values.steal << " guest: " << values.guest
...
bool CpuMetrics::operator==(const CpuMetrics &rhs) const {
  return nodeId == rhs.nodeId && timestamp == rhs.timestamp &&
         coreNum == rhs.coreNum && user == rhs.user && nice == rhs.nice &&
         system == rhs.system && idle == rhs.idle && iowait == rhs.iowait &&
         irq == rhs.irq && softirq == rhs.softirq && steal == rhs.steal &&
