...
NetworkMetrics::NetworkMetrics()
    : nodeId(0), timestamp(0), interfaceName(0), rBytes(0), rPackets(0),
      rErrs(0), rDrop(0), rFifo(0), rFrame(0), rCompressed(0), rMulticast(0),
      tBytes(0), tPackets(0), tErrs(0), tDrop(0), tFifo(0), tColls(0),
      tCarrier(0), tCompressed(0) {}

Configurations::SchemaTypePtr
NetworkMetrics::getSchemaType(const std::string &prefix) {
...
void NetworkMetrics::writeToBuffer(Runtime::TupleBuffer &buf,
  ...
  NES_ASSERT(totalSize <= buf.getBufferSize(),
             "NetworkMetrics: Content does not fit in TupleBuffer totalSize:" +
                 std::to_string(totalSize) + " < " +
                 " getBufferSize:" + std::to_string(buf.getBufferSize()));

  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      NetworkMetrics::getSchema(""), buf.getBufferSize());
...
void NetworkMetrics::readFromBuffer(Runtime::TupleBuffer &buf,
                                    uint64_t tupleIndex) {
  auto layout = Runtime::MemoryLayouts::RowLayout::create(
      NetworkMetrics::getSchema(""), buf.getBufferSize());
  auto buffer = Runtime::MemoryLayouts::TestTupleBuffer(layout, buf);
...
bool NetworkMetrics::operator==(const NetworkMetrics &rhs) const {
  return nodeId == rhs.nodeId && timestamp == rhs.timestamp &&
         interfaceName == rhs.interfaceName && rBytes == rhs.rBytes &&
         rPackets == rhs.rPackets && rErrs == rhs.rErrs && rDrop == rhs.rDrop &&
         rFifo == rhs.rFifo && rFrame == rhs.rFrame &&
         rCompressed == rhs.rCompressed && rMulticast == rhs.rMulticast &&
         tBytes == rhs.tBytes && tPackets == rhs.tPackets &&
         tErrs == rhs.tErrs && tDrop == rhs.tDrop && tFifo == rhs.tFifo &&
         tColls == rhs.tColls && tCarrier == rhs.tCarrier &&
