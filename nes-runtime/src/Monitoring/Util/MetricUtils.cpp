...
bool MetricUtils::validateFieldsInSchema(SchemaPtr metricSchema,
                                         SchemaPtr bufferSchema, uint64_t i) {
  if (i >= bufferSchema->getSize()) {
    return false;
  }

  auto hasName = NES::Util::endsWith(bufferSchema->fields[i]->getName(),
                                     metricSchema->get(0)->getName());
  auto hasLastField = NES::Util::endsWith(
      bufferSchema->fields[i + metricSchema->getSize() - 1]->getName(),
...
nlohmann::json MetricUtils::toJson(
    std::unordered_map<MetricType, std::shared_ptr<Metric>> metrics) {
  nlohmann::json metricsJson{};
  for (const auto& metric : metrics) {
    nlohmann::json jMetric = asJson(metric.second);
    metricsJson[std::string(
        magic_enum::enum_name(metric.second->getMetricType()))] = jMetric;
  }
  return metricsJson;
}
...
nlohmann::json MetricUtils::toJson(StoredNodeMetricsPtr metrics) {
  ...
  for (auto metricTypeEntry : *metrics.get()) {
    std::shared_ptr<std::vector<TimestampMetricPtr>> metricVec =
        metricTypeEntry.second;
    nlohmann::json arr{};
    int i = 0;
    ...
    metricsJson[std::string(magic_enum::enum_name(metricTypeEntry.first))] =
        arr;
  }
  return metricsJson;
}

MetricCollectorPtr MetricUtils::createCollectorFromCollectorType(
  ...
  switch (type) {
    ...
    default:
      NES_FATAL_ERROR("MetricUtils: Not supported collector type {}",
...
MetricPtr MetricUtils::createMetricFromCollectorType(MetricCollectorType type) {
  switch (type) {
    case MetricCollectorType::CPU_COLLECTOR:
      return std::make_shared<Metric>(CpuMetricsWrapper{},
                                      MetricType::WrappedCpuMetrics);
    case MetricCollectorType::DISK_COLLECTOR:
      return std::make_shared<Metric>(DiskMetrics{}, MetricType::DiskMetric);
    case MetricCollectorType::MEMORY_COLLECTOR:
      return std::make_shared<Metric>(MemoryMetrics{},
                                      MetricType::MemoryMetric);
    case MetricCollectorType::NETWORK_COLLECTOR:
      return std::make_shared<Metric>(NetworkMetricsWrapper{},
                                      MetricType::WrappedNetworkMetrics);
    default: {
      NES_FATAL_ERROR("MetricUtils: Collector type not supported {}",
                      std::string(magic_enum::enum_name(type)));
    }
  }
  return nullptr;
...
Configurations::SchemaTypePtr MetricUtils::getSchemaFromCollectorType(
    MetricCollectorType type) {
  switch (type) {
    case MetricCollectorType::CPU_COLLECTOR:
...
MetricCollectorType MetricUtils::createCollectorTypeFromMetricType(
    MetricType type) {
  switch (type) {
    ...
    case MetricType::WrappedNetworkMetrics:
      return MetricCollectorType::NETWORK_COLLECTOR;
    default: {
      NES_ERROR("MetricUtils: Metric type not supported {}",
                std::string(magic_enum::enum_name(type)));
