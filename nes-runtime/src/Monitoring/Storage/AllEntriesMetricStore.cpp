...
void AllEntriesMetricStore::addMetrics(WorkerId nodeId, MetricPtr metric)
{
    ...
    if (storedMetrics.contains(nodeId))
    {
        ...
        {
            NES_TRACE(
                "AllEntriesMetricStore: Creating metrics {} of {}", nodeId, std::string(magic_enum::enum_name(metric->getMetricType())));
            nodeMetrics->insert({metric->getMetricType(), std::make_shared<std::vector<TimestampMetricPtr>>()});
        }
    }
    else
    {
        NES_TRACE("AllEntriesMetricStore: Creating node {} of {}", nodeId, std::string(magic_enum::enum_name(metric->getMetricType())));
    ...
    NES_TRACE(
        "AllEntriesMetricStore: Adding metrics for {} with type {}: {}",
        nodeId,
