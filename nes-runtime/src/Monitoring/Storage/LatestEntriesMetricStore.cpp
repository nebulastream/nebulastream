...
void LatestEntriesMetricStore::addMetrics(WorkerId nodeId, MetricPtr metric)
{
    ...
    if (storedMetrics.contains(nodeId))
    {
        ...
        {
            NES_TRACE("LatestEntriesMetricStore: Creating metrics {} of {}", nodeId, std::string(magic_enum::enum_name(metricType)));
    ...
    NES_TRACE(
        "LatestEntriesMetricStore: Adding metrics for {} with type {}: {}",
        nodeId,
