...
LatestEntriesMetricStore::LatestEntriesMetricStore() {
  NES_DEBUG("LatestEntriesMetricStore: Init NewestMetricStore");
}

MetricStoreType LatestEntriesMetricStore::getType() const {
  return MetricStoreType::NewestEntry;
}
...
void LatestEntriesMetricStore::addMetrics(WorkerId nodeId, MetricPtr metric) {
  ...
  if (storedMetrics.contains(nodeId)) {
    NES_TRACE(
        "LatestEntriesMetricStore: Found stored metrics for node with ID {}",
        nodeId);
    nodeMetrics = storedMetrics[nodeId];
    // check if the metric type exists
    if (nodeMetrics->contains(metricType)) {
      NES_TRACE("LatestEntriesMetricStore: Removing metrics {} of {}", nodeId,
                std::string(magic_enum::enum_name(metricType)));
      nodeMetrics->at(metricType)->clear();
    } else {
      NES_TRACE("LatestEntriesMetricStore: Creating metrics {} of {}", nodeId,
                std::string(magic_enum::enum_name(metricType)));
      nodeMetrics->insert(
          {metricType, std::make_shared<std::vector<TimestampMetricPtr>>()});
    }
  } else {
    NES_TRACE("LatestEntriesMetricStore: Creating node {} of {}", nodeId,
              std::string(magic_enum::enum_name(metricType)));
  ...
  TimestampMetricPtr entry = std::make_shared<std::pair<uint64_t, MetricPtr>>(
      timestamp, std::move(metric));
