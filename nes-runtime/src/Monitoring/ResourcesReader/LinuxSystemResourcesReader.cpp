...
RuntimeMetrics LinuxSystemResourcesReader::readRuntimeNesMetrics() {
  ...
  try {
    NES_TRACE(
        "LinuxSystemResourcesReader: Reading memory.usage_in_bytes for "
        "metrics");
    ...
    if (access(metricLocations[0].c_str(), F_OK) != -1) {
      ...
      NES_ERROR(
          "LinuxSystemResourcesReader: File for memory.usage_in_bytes not "
          "available");
  ...
  try {
    NES_TRACE(
        "LinuxSystemResourcesReader: Reading blkio.throttle.io_service_bytes "
        "for metrics");
    ...
    if (access(metricLocations[2].c_str(), F_OK) != -1) {
      ...
      NES_ERROR(
          "LinuxSystemResourcesReader: File for "
...
RegistrationMetrics LinuxSystemResourcesReader::readRegistrationMetrics() {
  ...
  try {
    ...
    if (access(metricLocations[1].c_str(), F_OK) != -1) {
      ...
      NES_WARNING(
          "LinuxSystemResourcesReader: File for cpu.cfs_period_us not "
          "available");
    ...
    if (access(metricLocations[2].c_str(), F_OK) != -1) {
      ...
      NES_WARNING(
          "LinuxSystemResourcesReader: File for cpu.cfs_quota_us not "
          "available");
      output.cpuQuotaUS = 0;
    }
  } catch (const Exceptions::RuntimeException& e) {
