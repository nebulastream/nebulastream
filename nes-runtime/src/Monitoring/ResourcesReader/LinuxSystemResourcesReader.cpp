...
LinuxSystemResourcesReader::LinuxSystemResourcesReader() {
  readerType = SystemResourcesReaderType::LinuxReader;
}
...
RuntimeMetrics LinuxSystemResourcesReader::readRuntimeNesMetrics() {
  ...
  try {
    NES_TRACE("LinuxSystemResourcesReader: Reading memory.usage_in_bytes for "
              "metrics");

    if (access(metricLocations[0].c_str(), F_OK) != -1) {
      std::ifstream memoryLoc(metricLocations[0]);
      std::string memoryStr((std::istreambuf_iterator<char>(memoryLoc)),
                            std::istreambuf_iterator<char>());

      output.memoryUsageInBytes = std::stoull(memoryStr);
    } else {
      NES_ERROR("LinuxSystemResourcesReader: File for memory.usage_in_bytes "
                "not available");
      output.memoryUsageInBytes = 0;
    }
  } catch (const Exceptions::RuntimeException &e) {
    NES_ERROR("LinuxSystemResourcesReader: Error reading memory metrics {}",
              e.what());
  }
  ...
  try {
    ...
    if (access(metricLocations[1].c_str(), F_OK) != -1) {
      ...
      NES_ERROR(
          "LinuxSystemResourcesReader: File for cpuacct.stat not available");
      output.cpuLoadInJiffies = 0;
    }
  } catch (const Exceptions::RuntimeException &e) {
    NES_ERROR("LinuxSystemResourcesReader: Error reading cpu metrics {}",
              e.what());
  }

  try {
    NES_TRACE("LinuxSystemResourcesReader: Reading "
              "blkio.throttle.io_service_bytes for metrics");
    ...
    if (access(metricLocations[2].c_str(), F_OK) != -1) {
      ...
      NES_ERROR("LinuxSystemResourcesReader: File for "
                "blkio.throttle.io_service_bytes not available");
      output.blkioBytesRead = 0;
      output.blkioBytesWritten = 0;
    }

  } catch (const Exceptions::RuntimeException &e) {
    NES_ERROR("LinuxSystemResourcesReader: Error reading disk metrics {}",
              e.what());
  }
...
RegistrationMetrics LinuxSystemResourcesReader::readRegistrationMetrics() {
  ...
  try {
    NES_TRACE("LinuxSystemResourcesReader: Reading memory.usage_in_bytes for "
              "metrics");
    uint64_t systemMem =
        LinuxSystemResourcesReader::readMemoryStats().TOTAL_RAM;
    output.totalMemoryBytes = systemMem;
  } catch (const Exceptions::RuntimeException &e) {
    NES_ERROR(
        "LinuxSystemResourcesReader: Error reading static memory metrics {}",
        e.what());
  }
  ...
  try {
    ...
    output.totalCPUJiffies =
        totalStats.user + totalStats.system + totalStats.idle;
    ...
    if (access(metricLocations[1].c_str(), F_OK) != -1) {
      ...
      std::string periodStr((std::istreambuf_iterator<char>(periodLoc)),
                            std::istreambuf_iterator<char>());

      output.cpuPeriodUS = std::stoll(periodStr);
    } else {
      NES_WARNING("LinuxSystemResourcesReader: File for cpu.cfs_period_us not "
                  "available");
    ...
    if (access(metricLocations[2].c_str(), F_OK) != -1) {
      ...
      std::string quotaStr((std::istreambuf_iterator<char>(quotaLoc)),
                           std::istreambuf_iterator<char>());

      output.cpuQuotaUS = std::stoll(quotaStr);
    } else {
      NES_WARNING("LinuxSystemResourcesReader: File for cpu.cfs_quota_us not "
                  "available");
      output.cpuQuotaUS = 0;
    }
  } catch (const Exceptions::RuntimeException &e) {
    NES_ERROR("LinuxSystemResourcesReader: Error reading static cpu metrics {}",
              e.what());
  }
  return output;
}
...
CpuMetricsWrapper LinuxSystemResourcesReader::readCpuStats() {
  ...
  try {
    NES_TRACE(
        "LinuxSystemResourcesReader: Reading CPU stats for number of CPUs {}",
        numCpuMetrics);
      ...
      if (!line.compare(0, 3, "cpu")) {
        ...
        if (tokens.size() != 11) {
          NES_THROW_RUNTIME_ERROR(
              "LinuxSystemResourcesReader: /proc/stat incorrect");
        }
    ...
    NES_ERROR("LinuxSystemResourcesReader: Error calling readCpuStats() {}",
              e.what());
  }
  ...
  try {
    ...
    while (fgets(buf, 200, fp)) {
      sscanf(buf,
             ...
             "%lu %lu",
             ifname, &rBytes, &rPackets, &rErrs, &rDrop, &rFifo, &rFrame,
    ...
    NES_ERROR("LinuxSystemResourcesReader: Error reading network stats {}",
              e.what());
  }
  return output;
}
...
MemoryMetrics LinuxSystemResourcesReader::readMemoryStats() {
  ...
  try {
    ...
    if (ret == EFAULT) {
      NES_THROW_RUNTIME_ERROR(
          "LinuxSystemResourcesReader: Error reading memory stats");
    }

    output.timestamp = duration_cast<std::chrono::milliseconds>(
    ...
    NES_ERROR("LinuxSystemResourcesReader: Error reading memory stats {}",
              e.what());
  }
  return output;
}
...
DiskMetrics LinuxSystemResourcesReader::readDiskStats() {
  ...
  try {
    ...
    if (ret == EFAULT) {
      NES_THROW_RUNTIME_ERROR(
          "LinuxSystemResourcesReader: Error reading disk stats");
    }
    ...
    NES_ERROR("LinuxSystemResourcesReader: Error reading disk stats {}",
              e.what());
  }
  return output;
}
