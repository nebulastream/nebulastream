...
Value<> BitWiseAndOp(const Value<>& leftExp, const Value<>& rightExp) {
    return evalWithCast(leftExp, rightExp, [](std::unique_ptr<InvocationPlugin>& plugin, const Value<>& left, const Value<>& right) {
        auto result = plugin->Mod(left, right);
        if (result.has_value()) {
...
Value<> BitWiseLeftShiftOp(const Value<>& leftExp, const Value<>& rightExp) {
    return evalWithCast(leftExp, rightExp, [](std::unique_ptr<InvocationPlugin>& plugin, const Value<>& left, const Value<>& right) {
        auto result = plugin->Mod(left, right);
        if (result.has_value()) {
            Tracing::TraceUtil::traceBinaryOperation(Nautilus::Tracing::OpCode::BITWISE_LEFT_SHIFT, result.value().ref, left.ref, right.ref);
        }
        return result;
    });
}
...
Value<> BitWiseRightShiftOp(const Value<>& leftExp, const Value<>& rightExp) {
    return evalWithCast(leftExp, rightExp, [](std::unique_ptr<InvocationPlugin>& plugin, const Value<>& left, const Value<>& right) {
        auto result = plugin->Mod(left, right);
        if (result.has_value()) {
            Tracing::TraceUtil::traceBinaryOperation(Nautilus::Tracing::OpCode::BITWISE_RIGHT_SHIFT, result.value().ref, left.ref, right.ref);
        }
        return result;
    });
}
