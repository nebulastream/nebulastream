...
HashJoinOperatorHandler::HashJoinOperatorHandler(SchemaPtr joinSchemaLeft,
...
void HashJoinOperatorHandler::recyclePooledBuffer(Runtime::detail::MemorySegment*) {}

void HashJoinOperatorHandler::recycleUnpooledBuffer(Runtime::detail::MemorySegment*) {}

const std::string& HashJoinOperatorHandler::getJoinFieldNameLeft() const { return joinFieldNameLeft; }
const std::string& HashJoinOperatorHandler::getJoinFieldNameRight() const { return joinFieldNameRight; }

SchemaPtr HashJoinOperatorHandler::getJoinSchemaLeft() const { return joinSchemaLeft; }
SchemaPtr HashJoinOperatorHandler::getJoinSchemaRight() const { return joinSchemaRight; }

void HashJoinOperatorHandler::start(PipelineExecutionContextPtr, StateManagerPtr, uint32_t) {
    NES_DEBUG("start HashJoinOperatorHandler");
}

void HashJoinOperatorHandler::stop(QueryTerminationType, PipelineExecutionContextPtr) {
    NES_DEBUG("stop HashJoinOperatorHandler");
}

void HashJoinOperatorHandler::setup(uint64_t newNumberOfWorkerThreads) {
    if (alreadySetup) {
...
void HashJoinOperatorHandler::createNewWindow(bool isLeftSide) {
...
void HashJoinOperatorHandler::deleteWindow(uint64_t timeStamp) {
...
bool HashJoinOperatorHandler::checkWindowExists(uint64_t timeStamp) {
...
HashJoinWindow& HashJoinOperatorHandler::getWindow(uint64_t timeStamp) {
    for (auto& hashJoinWindow : hashJoinWindows) {
        if (timeStamp <= hashJoinWindow.getWindowEnd()) {
...
uint64_t HashJoinOperatorHandler::getLastTupleTimeStamp(bool isLeftSide) const {
    if (isLeftSide) {
...
HashJoinWindow& HashJoinOperatorHandler::getWindowToBeFilled(bool isLeftSide) {
    if (isLeftSide) {
...
void HashJoinOperatorHandler::incLastTupleTimeStamp(uint64_t increment, bool isLeftSide) {
    if (isLeftSide) {
...
size_t HashJoinOperatorHandler::getWindowSize() const { return windowSize; }

size_t HashJoinOperatorHandler::getNumPartitions() const { return numPartitions; }

size_t HashJoinOperatorHandler::getNumActiveWindows() { return hashJoinWindows.size(); }

HashJoinOperatorHandlerPtr HashJoinOperatorHandler::create(const SchemaPtr& joinSchemaLeft,
    ...
    return std::make_shared<HashJoinOperatorHandler>(joinSchemaLeft,
                                                     joinSchemaRight,
                                                     joinFieldNameLeft,
                                                     joinFieldNameRight,
                                                     counterFinishedBuildingStart,
...
const std::vector<OperatorId>& HashJoinOperatorHandler::getJoinOperatorsId() const { return joinOperatorsId; }

void HashJoinOperatorHandler::addOperatorId(OperatorId id) { joinOperatorsId.emplace_back(id); }
