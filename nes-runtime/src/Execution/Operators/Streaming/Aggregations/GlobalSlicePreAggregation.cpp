...
void* getSliceStoreProxy(void*, uint64_t);
void* findSliceStateByTsProxy(void*, uint64_t);
void* triggerThreadLocalStateProxy(void* wctx, void* pctx, uint64_t originId, uint64_t sequenceNumber, uint64_t watermarkTs);
...
void GlobalSlicePreAggregation::execute(NES::Runtime::Execution::ExecutionContext& ctx, NES::Nautilus::Record& record) const {
    ...
    // 3. manipulate the current aggregate value
    // TODO generalize this to other then count aggregations
    auto value = sliceState.load<UInt32>();
    value = value + 1u;
    sliceState.store(value);
}
