...
// Furthermore, OS limits for mmapping and memory locking should be configured
// appropriately to allow memory allocations
...
void* NumaRegionMemoryAllocator::do_allocate(size_t sizeInBytes, size_t) {
  ...
  NES_ASSERT2_FMT(mem != MAP_FAILED, "Cannot allocate memory "
                                         << sizeInBytes << " with errno "
                                         << strerror(errno));
  // bind it to a given numa region with strict assignment: the numa region must
  // be capable of allocating the area
  auto ret =
      mbind(mem, sizeInBytes, mode, &mask, 32, MPOL_MF_STRICT | MPOL_MF_MOVE);
...
void NumaRegionMemoryAllocator::do_deallocate(void* pointer, size_t sizeInBytes,
                                              size_t) {
