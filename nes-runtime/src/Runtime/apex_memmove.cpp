  ...
  if (size <= 112) {
    ...
    if (((size_t)dst - (size_t)src) >= size) {
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size &
                        -0x40);     // "Round down to nearest multiple of 64"
    ...
    } else  // src < dst ... do reverse copy
    {
      ...
      if (size >= 8) {
        ...
        if (size > 8) {
          size = -size;  // that's right, we're converting an unsigned value to
                         // a negative, saves 2 clock cycles!
  ...
  if (size <= 112) {
    ...
    if (((size_t)dst - (size_t)src) >= size) {
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size &
                        -0x20);     // "Round down to nearest multiple of 64"
        dst = (char*)dst + offset;  // "Point to the end"
        src = (char*)src + offset;  // "Point to the end"
        size -= offset;             // "Remaining data after loop"
        offset = -offset;           // "Negative index from the end"
  ...
  if (size <= 32) {
    ...
    if (((size_t)dst - (size_t)src) >= size) {
      ...
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size &
                        -0x20);     // "Round down to nearest multiple of 32"
        dst = (char*)dst + offset;  // "Point to the end"
        src = (char*)src + offset;  // "Point to the end"
        size -= offset;             // "Remaining data after loop"
        offset = -offset;           // "Negative index from the end"
    ...
    } else  // src < dst ... do reverse copy
    {
      ...
      } else  // do reversed streaming copy/move
      {
        ...
        long long offset =
            (long long)(size &
                        -0x40);  // "Round down to nearest multiple of 32"
        size -= offset;          // "Remaining data after loop"
        offset -= 4096;          // stage 1 INCLUDES prefetches
        dst =
            (char*)dst -
  ...
  if (cpuid[2] &
    ...
    apex::memcpy = apex::memmove =
        &apex::mithril;  //	this is for very, very old computers with SSE2
                         ...
                         //made `tiberium` a bit slower. So this is for super
                         //old (P4) PC's!
  else {
