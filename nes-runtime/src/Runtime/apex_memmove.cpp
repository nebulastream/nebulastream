...
#include <cstring> //	std::memmove, std::memcpy
    ...
    defined(_M_IX86)   //	Test for Intel/AMD architecture
...
namespace apex {
//	apex memmove (tiberium, kryptonite and mithril) memcpy/memmove functions
//written by Trevor Herselman in 2014

#if defined(_MSC_VER)
#pragma warning(push)
#pragma warning(                                                               \
        disable : 4146) //	warning C4146: unary minus operator applied to
                        //unsigned type, result still unsigned
#pragma warning(                                                               \
        disable : 4244) //	warning C4244: '-=': conversion from '__int64'
                        //to 'std::size_t', possible loss of data
#endif

#if defined(__x86_64__) || defined(_M_X64) || defined(__i386) ||               \
    defined(_M_IX86) //	Test for Intel/AMD architecture

void *APEXCALL tiberium(void *dst, const void *src,
                        size_t size) { // based on memmove09 for "size <= 112"
                                       // and memmove40 for "size > 112"
  if (size <= 112) {
    if (size >= 16) {
      ...
      if (size > 16) {
        if (size >= 32) {
          ...
          if (size > 32) {
            ...
            if (size > 48) {
              const __m128i xmm2 =
                  _mm_loadu_si128((__m128i *)((char *)src + 32));
              if (size > 64) {
                const __m128i xmm3 =
                    _mm_loadu_si128((__m128i *)((char *)src + 48));
                if (size > 80) {
                  const __m128i xmm4 =
                      _mm_loadu_si128((__m128i *)((char *)src + 64));
                  if (size > 96) {
                    const __m128i xmm5 =
                        _mm_loadu_si128((__m128i *)((char *)src + 80));
    ...
    if (((size_t)dst - (size_t)src) >= size) {
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size & -0x40); // "Round down to nearest multiple of 64"
        ...
        do {
          const __m128i xmm0 =
              _mm_loadu_si128((__m128i *)((char *)src + offset));
          const __m128i xmm1 =
              _mm_loadu_si128((__m128i *)((char *)src + offset + 16));
          const __m128i xmm2 =
              _mm_loadu_si128((__m128i *)((char *)src + offset + 32));
          const __m128i xmm3 =
              _mm_loadu_si128((__m128i *)((char *)src + offset + 48));
        ...
        if (size >= 16) {
          ...
          if (size > 16) {
            const __m128i xmm3 =
                _mm_loadu_si128((__m128i *)((char *)src + size - 16));
            if (size > 32) {
              const __m128i xmm1 =
                  _mm_loadu_si128((__m128i *)((char *)src + 16));
              if (size > 48) {
                const __m128i xmm2 =
                    _mm_loadu_si128((__m128i *)((char *)src + 32));
                _mm_storeu_si128((__m128i *)dst, xmm0);
      ...
      } else // do forward streaming copy/move
      {
        ...
        _mm_prefetch(((char *)src + size - 64),
                     _MM_HINT_NTA); // prefetch the final tail section
          ...
          if (size > 16) {
            if (size > 32) {
              const __m128i xmm1 =
                  _mm_loadu_si128((__m128i *)((char *)src + 16));
              const __m128i xmm6 =
                  _mm_loadu_si128((__m128i *)((char *)src + size - 32));
              const __m128i xmm7 =
                  _mm_loadu_si128((__m128i *)((char *)src + size - 16));
    ...
    } else // src < dst ... do reverse copy
    {
      ...
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size & -0x40); // "Round down to nearest multiple of 64"
        dst = (char *)dst -
              offset; // "Point to the end" ... actually, we point to the start!
        src = (char *)src -
              offset; // "Point to the end" ... actually, we point to the start!
        size -= offset; // "Remaining data after loop"
        // offset = -offset;									//
        ...
        if (size >= 16) {
          ...
          if (size > 16) {
            size = -size;
            const __m128i xmm3 =
                _mm_loadu_si128((__m128i *)((char *)src + size));
            if (size > 32) {
              const __m128i xmm1 =
                  _mm_loadu_si128((__m128i *)((char *)src - 32));
              if (size > 48) {
        ...
        if (prealign) {
          ...
          if (prealign >= 8) {
            ...
            if (prealign > 8) {
              ...
              *(long long *)dst =
                  rax; // different on purpose, because we know the exact size
                       // now, which is 8, and "dst" has already been aligned!
          } else if (prealign >= 4) {
      ...
      if (size >= 8) {
        ...
        if (size > 8) {
          size = -size; // that's right, we're converting an unsigned value to a
                        // negative, saves 2 clock cycles!
...
void *APEXCALL
kryptonite(void *dst, const void *src,
           size_t size) { // based on memmove09 for "size <= 112" and memmove41
                          // for "size > 112"; memmove09's "size <= 112" proved
                          // fastest overall (weighted), even on Core i5!
  if (size <= 112) {
    if (size >= 16) {
      const __m128i xmm0 = _mm_loadu_si128((__m128i *)src);
      if (size > 16) {
    ...
    if (((size_t)dst - (size_t)src) >= size) {
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size & -0x20); // "Round down to nearest multiple of 64"
        dst = (char *)dst + offset;    // "Point to the end"
        src = (char *)src + offset;    // "Point to the end"
        size -= offset;                // "Remaining data after loop"
        offset = -offset;              // "Negative index from the end"
    ...
    } else // src < dst ... do reverse copy
    {
      ...
      if (size < (1024 * 256)) {
        ...
        if (size >= 16) {
          if (size > 16) {
            size = -size;
            // The order has been mixed so the compiler will not re-order the
            // statements!
            const __m128i xmm7 =
                _mm_loadu_si128((__m128i *)((char *)src + size));
            const __m128i xmm0 = _mm_loadu_si128((__m128i *)((char *)src - 16));
          ...
          _mm_storeu_si128((__m128i *)((char *)dst - 16),
                           _mm_loadu_si128((__m128i *)((char *)src - 16)));
          return ret;
        }
      } else // do reversed streaming copy/move
...
#if (!defined(_M_X64) &&                                                       \
     !defined(__x86_64__)) //	`mithril` is only used in 32-bit code (not
                           //necessary for 64-bit)

// "Mithril" - Written by Trevor Herselman on 4 December 2013 @ 9pm
// This is my "compact power house"! Decided to make a "mini" powerful version,
// one that can easily beat Microsoft's memmove, but doesn't contain all the
// "bells and whistles"! Probably Very similar to mm04 in terms of performance,
// but this should be faster under all conditions and ranges! Includes a new
// "prealignment" check that detects if "size <= 48", then uses 3x SSE loads
// instead of passing through the prealignment code! originally called
// memmove13() during development (mm13 is based on mm04)
    ...
    if (((size_t)dst - (size_t)src) >= size) {
      if ((size_t)dst & 0xf) {
        ...
        if (size <= 48) {
          ...
          // we could also check "size - prealign <= 32", but since we're
          // already here, and these statements can load upto 48-bytes, we might
          // as well just do it! We `could` copy up to 64-bytes without any
          // additional checks by using another SSE "loadu" & "storeu"
          const __m128i xmm0 = _mm_loadu_si128((__m128i *)src);
      ...
      if (size < (1024 * 256)) {
        long long offset =
            (long long)(size & -0x20); // "Round down to nearest multiple of 32"
        dst = (char *)dst + offset;    // "Point to the end"
        src = (char *)src + offset;    // "Point to the end"
        size -= offset;                // "Remaining data after loop"
        offset = -offset;              // "Negative index from the end"
        ...
        if (((size_t)src & 0xf) == 0) {
          do {
            const __m128i xmm0 =
                _mm_load_si128((__m128i *)((char *)src + offset));
            const __m128i xmm1 =
                _mm_load_si128((__m128i *)((char *)src + offset + 16));
    ...
    } else // src < dst ... do reverse copy
    {
      ...
      if (prealign) {
        if (size <= 48) {
          ...
          // we could also check "size - prealign <= 32", but since we're
          // already here, and these statements can load upto 48-bytes, we might
          // as well just do it! Actually, we can copy upto 64-bytes without any
          // additional checks!
          size = -size;
          const __m128i xmm0 = _mm_loadu_si128(
              (__m128i *)((char *)src - 16)); // first 32-bytes in reverse
          const __m128i xmm1 = _mm_loadu_si128((__m128i *)((char *)src - 32));
          const __m128i xmm7 = _mm_loadu_si128((__m128i *)((char *)src + size));
          _mm_storeu_si128((__m128i *)((char *)dst - 16), xmm0);
          _mm_storeu_si128((__m128i *)((char *)dst - 32), xmm1);
          _mm_storeu_si128(
              (__m128i *)((char *)dst + size),
        ...
        if (prealign >= 8) {
          ...
          if (prealign > 8) {
            ...
            *(long long *)dst = rax; // different on purpose, because we know
                                     // the exact size now!
        } else if (prealign >= 4) {
          int eax = *(int *)((char *)src + prealign - 4);
          if (prealign > 4) {
      ...
      if (size < (1024 * 256)) {
        ...
        if (((size_t)src & 0xf) == 0) {
          ...
          if (size >= 16) {
            ...
            if (size > 16) {
              size = -size;
              // const __m128i xmm7 = _mm_loadu_si128( (__m128i*)((char*)src +
              // size) ); // SSE2 alternative
      ...
      } else // do reversed streaming copy/move
      {
        ...
        long long offset =
            (long long)(size & -0x40); // "Round down to nearest multiple of 32"
        size -= offset;                // "Remaining data after loop"
        offset -= 4096;                // stage 1 INCLUDES prefetches
        dst = (char *)dst -
              offset; // "Point to the end" ... actually, we point to the start!
...
#if (defined(_M_X64) || defined(__x86_64__) || defined(__i386) ||              \
     defined(_M_IX86)) &&                                                      \
    (defined(_MSC_VER) ||                                                      \
     defined(__GNUC__)) //	Compiler and architecture test (Intel/AMD
                        //Architecture)! Visual Studio and GCC / LLVM (Clang)
#if defined(_MSC_VER)
  int cpuid[4] = {-1}; //	Visual Studio
  __cpuid(cpuid, 1);
#define bit_SSE2                                                               \
  (1 << 26) //	Taken from GCC <cpuid.h> ... just more visual & descriptive!
...
#define bit_SSE4_2 (1 << 20)
  ...
  if (cpuid[2] &
      bit_SSE4_2) //	detect SSE4.2, available on Core i and newer processors,
                  //they include "fast unaligned" memory access
  ...
  if (cpuid[2] &
    ...
    apex::memcpy = apex::memmove = &apex::kryptonite;
           ...
           bit_SSSE3) //	detect SSSE3, available on Core/Core 2 and newer
    apex::memcpy = apex::memmove =
        &apex::tiberium; //	`tiberium` CAN run with the SSE2 instructions
                         //alone, however there was something about very old
                         //SSE2 (only) computers, some penalty when using
                         //`tiberiums` algorithm, so I've restricted `tiberium`
                         //to Core/Core 2 based machines on 32-bit by doing
                         //this!
  else if (cpuid[3] & bit_SSE2)
    apex::memcpy = apex::memmove =
        &apex::mithril; //	this is for very, very old computers with SSE2
                        //only! eg. old Pentium 4! There was something about
                        //computers WITHOUT SSSE3 (Core microarchitecture) that
                        //made `tiberium` a bit slower. So this is for super old
                        //(P4) PC's!
  else {
...
#endif
#else //	unknown compiler or architecture! eg. __arm__ / __mips__ /
      //__ppc__ / __sparc / __ia64 (Itanium) etc.
#if defined(__x86_64__)
  apex::memcpy = apex::memmove =
      &apex::tiberium; //	unknown compiler BUT it declared support for
                       //Intel/AMD x64/AMD64 (64-bit), so it should come with
                       //the <emmintrin.h> file from Intel! 64-bit x64/AMD64
                       //includes SSE2! This includes `Portland (PGCC/PGCPP)`,
                       //`Oracle Solaris Studio` and `Intel compiler (ICC)`! I
                       //don't have code for CPUID for these compilers so I
                       //don't know how to check for SSE2/SSSE3/SSE4.2 on them!
#else
  apex::memcpy = &::memcpy; //	unknown compiler or architecture!
  apex::memmove = &::memmove;
#endif
#endif
  return apex::memmove(dst, src,
                       size); //	safe to call memmove even for memcpy!
}

void *(APEXCALL *memcpy)(void *dst, const void *src,
                         size_t size) = apex::memmove_dispatcher;
void *(APEXCALL *memmove)(void *dst, const void *src,
                          size_t size) = apex::memmove_dispatcher;
} // namespace apex
