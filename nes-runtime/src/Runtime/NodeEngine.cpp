...
NodeEngine::NodeEngine(
    ...
    OpenCLManagerPtr &&openCLManager, WorkerId nodeEngineId,
    uint64_t numberOfBuffersInGlobalBufferManager,
    uint64_t numberOfBuffersInSourceLocalBufferPool,
    uint64_t numberOfBuffersPerWorker, bool sourceSharing)
    : nodeId(INVALID_WORKER_NODE_ID),
      physicalSources(std::move(physicalSources)),
      ...
      partitionManager(std::move(partitionManager)),
      nesWorker(std::move(nesWorker)),
      // TODO for now, we always use the DefaultStatisticStore. A configuration
      // will be done with #4687
      statisticManager(Statistic::StatisticManager::create(
          Statistic::DefaultStatisticStore::create())),
  ...
  // here shared_from_this() does not work because of the machinery behind
  // make_shared as a result, we need to use a trick, i.e., a shared ptr that
  // does not deallocate the node engine plz make sure that ExchangeProtocol
  // never leaks the impl pointer
  // TODO refactor to decouple the two components!
  networkManager =
      networkManagerCreator(std::shared_ptr<NodeEngine>(this, [](NodeEngine *) {
        // nop
      }));
...
bool NodeEngine::deployQueryInNodeEngine(
    const Execution::ExecutableQueryPlanPtr &queryExecutionPlan) {
  std::unique_lock lock(engineMutex);
  NES_DEBUG(
      "Runtime: deployQueryInNodeEngine query using qep with sharedQueryId: {}",
      queryExecutionPlan->getSharedQueryId());
  ...
  bool successStart =
      startQuery(queryExecutionPlan->getSharedQueryId(),
                 queryExecutionPlan->getDecomposedQueryPlanId());
  if (!successStart) {
...
bool NodeEngine::registerDecomposableQueryPlan(
...
bool NodeEngine::registerExecutableQueryPlan(
    const Execution::ExecutableQueryPlanPtr &queryExecutionPlan) {
  std::unique_lock lock(engineMutex);
  SharedQueryId sharedQueryId = queryExecutionPlan->getSharedQueryId();
  DecomposedQueryPlanId decomposedQueryPlanId =
      queryExecutionPlan->getDecomposedQueryPlanId();
  NES_DEBUG("Runtime: registerExecutableQueryPlan query with sharedQueryId= {} "
            "decomposedQueryPlanId = {}",
            sharedQueryId, decomposedQueryPlanId);
  NES_ASSERT(queryManager->isThreadPoolRunning(),
             "Registering query but thread pool not running");
  if (deployedExecutableQueryPlans.find(decomposedQueryPlanId) ==
    ...
    if (found == sharedQueryIdToDecomposedQueryPlanIds.end()) {
      sharedQueryIdToDecomposedQueryPlanIds[sharedQueryId] = {
          decomposedQueryPlanId};
      NES_DEBUG("Runtime: register of QEP  {}  as a singleton",
                decomposedQueryPlanId);
    } else {
      (*found).second.push_back(decomposedQueryPlanId);
      NES_DEBUG("Runtime: register of QEP  {}  added", decomposedQueryPlanId);
    }
    /* We have to unlock here, as we do not want to hold the lock for the
     * queryManager->registerQuery(). Otherwise, it can lead to the case, that
     * we still hold the lock but another query wants to register itself on this
     * Node(1) and connect to Node(2). On Node(2), the queries are doing the
     * reverse and thus, each query is waiting on the other query to start all
     * network sources and sinks. Leading to a deadlock!
     */
    lock.unlock();
    ...
    if (queryManager->registerQuery(queryExecutionPlan)) {
      ...
      NES_DEBUG("Runtime: register of subqep  {}  succeeded",
                decomposedQueryPlanId);
      return true;
    }
    ...
    NES_DEBUG("Runtime: qep already exists. register failed {}",
              decomposedQueryPlanId);
    return false;
  }
}
...
bool NodeEngine::startQuery(SharedQueryId sharedQueryId,
  ...
  if (sharedQueryIdToDecomposedQueryPlanIds.find(sharedQueryId) !=
      sharedQueryIdToDecomposedQueryPlanIds.end()) {
    std::vector<DecomposedQueryPlanId> decomposedQueryPlanIds =
        sharedQueryIdToDecomposedQueryPlanIds[sharedQueryId];
    if (decomposedQueryPlanIds.empty()) {
      NES_ERROR(
          "Runtime: Unable to find qep ids for the query {}. Start failed.",
          sharedQueryId);
      return false;
    }

    if (std::find(decomposedQueryPlanIds.begin(), decomposedQueryPlanIds.end(),
                  decomposedQueryPlanId) == decomposedQueryPlanIds.end()) {
      NES_ERROR("Runtime: Unable to find qep with id {} for the shared query "
                "{}. Start failed.",
                decomposedQueryPlanId, sharedQueryId);
      return false;
    }

    try {
      if (queryManager->startQuery(
              deployedExecutableQueryPlans[decomposedQueryPlanId])) {
        NES_DEBUG("Runtime: start of QEP  {}  succeeded",
                  decomposedQueryPlanId);
      } else {
  ...
  NES_ERROR("Runtime: qep does not exists. start failed for query={}",
            sharedQueryId);
  return false;
}
...
bool NodeEngine::unregisterQuery(SharedQueryId sharedQueryId) {
  ...
  bool ret = true;
  if (auto it = sharedQueryIdToDecomposedQueryPlanIds.find(sharedQueryId);
      it != sharedQueryIdToDecomposedQueryPlanIds.end()) {
    ...
    for (const auto querySubPlanId : querySubPlanIds) {
      ...
      case Execution::ExecutableQueryPlanStatus::Running: {
        NES_DEBUG(
            "Runtime: unregister of query  {}  is not Stopped... stopping now",
            querySubPlanId);
        isStopped = queryManager->stopQuery(
            qep, Runtime::QueryTerminationType::HardStop);
        break;
      }
      default: {
        isStopped = true;
      ...
      NES_DEBUG(
          "Runtime: unregister of query  {} : current status is stopped= {}",
          querySubPlanId, isStopped);
      if (isStopped && queryManager->deregisterQuery(qep)) {
        deployedExecutableQueryPlans.erase(querySubPlanId);
        NES_DEBUG("Runtime: unregister of query  {}  succeeded",
                  querySubPlanId);
      } else {
...
bool NodeEngine::stopQuery(SharedQueryId sharedQueryId,
  ...
  std::unique_lock lock(engineMutex);
  NES_DEBUG("Runtime:stopQuery for qep= {}  termination= {}", sharedQueryId,
            terminationType);
  ...
  if (it != sharedQueryIdToDecomposedQueryPlanIds.end()) {
    ...
    if (decomposedQueryPlanIds.empty()) {
      NES_ERROR(
          "Runtime: Unable to find qep ids for the query {}. Stop failed.",
          sharedQueryId);
      return false;
    }
    ...
    case QueryTerminationType::HardStop: {
      try {
        if (queryManager->stopQuery(
                deployedExecutableQueryPlans[decomposedQueryPlanId],
                terminationType)) {
          NES_DEBUG("Runtime: stop of QEP  {}  succeeded",
                    decomposedQueryPlanId);
          return true;
        } else {
    ...
    case QueryTerminationType::Failure: {
      try {
        if (queryManager->failQuery(
                deployedExecutableQueryPlans[decomposedQueryPlanId])) {
          NES_DEBUG("Runtime: failure of QEP  {}  succeeded",
                    decomposedQueryPlanId);
          return true;
        } else {
    ...
    case QueryTerminationType::Invalid:
      NES_NOT_IMPLEMENTED();
    }
    return true;
  }
...
bool NodeEngine::stop(bool markQueriesAsFailed) {
  ...
  // TODO @all: imho, when this method terminates, nothing must be running still
  // and all resources must be returned to the engine
  // TODO @all: error handling, e.g., is it an error if the query is stopped but
  // not non-deployed? @Steffen?
  ...
  for (auto it = deployedExecutableQueryPlans.begin();
       it != deployedExecutableQueryPlans.end();) {
    ...
    try {
      if (queryManager->deregisterQuery(queryExecutionPlan)) {
        NES_DEBUG("Runtime: deregisterQuery of QEP  {}  succeeded",
                  querySubPlanId);
  ...
  // release components
  // TODO do not touch the sequence here as it will lead to errors in the
  // shutdown sequence
...
BufferManagerPtr
NodeEngine::getBufferManager(uint32_t bufferManagerIndex) const {
  NES_ASSERT2_FMT(bufferManagerIndex < bufferManagers.size(),
                  "invalid buffer manager index=" << bufferManagerIndex);
...
AbstractQueryStatusListenerPtr NodeEngine::getQueryStatusListener() {
  return nesWorker;
}
...
NodeEngine::getQueryStatus(SharedQueryId sharedQueryId) {
  std::unique_lock lock(engineMutex);
  if (sharedQueryIdToDecomposedQueryPlanIds.find(sharedQueryId) !=
      sharedQueryIdToDecomposedQueryPlanIds.end()) {
    std::vector<DecomposedQueryPlanId> querySubPlanIds =
        sharedQueryIdToDecomposedQueryPlanIds[sharedQueryId];
    if (querySubPlanIds.empty()) {
      NES_ERROR(
          "Runtime: Unable to find qep ids for the query {}. Start failed.",
...
void NodeEngine::onEvent(NES::Network::NesPartition,
                         NES::Runtime::BaseEvent &) {
  // nop :: kept as legacy
}
...
void NodeEngine::onServerError(Network::Messages::ErrorMessage err) {
  ...
  case Network::Messages::ErrorType::PartitionNotRegisteredError: {
    NES_WARNING("Runtime: Unable to find the NES Partition {}",
                err.getChannelId());
    break;
  }
  case Network::Messages::ErrorType::DeletedPartitionError: {
    NES_WARNING("Runtime: Requesting deleted NES Partition {}",
                err.getChannelId());
    break;
  }
  case Network::Messages::ErrorType::VersionMismatchError: {
    NES_INFO("Runtime: Node {} encountered server error: Version mismatch for "
             "requested partition {}",
             nodeId, err.getChannelId());
    break;
  }
  default: {
...
void NodeEngine::onChannelError(Network::Messages::ErrorMessage err) {
  ...
  case Network::Messages::ErrorType::VersionMismatchError: {
    NES_INFO("Runtime: Expected version is not running yet for channel {}",
             err.getChannelId());
    break;
  }
  default: {
...
std::vector<QueryStatisticsPtr>
NodeEngine::getQueryStatistics(SharedQueryId sharedQueryId) {
  ...
  auto foundQuerySubPlanIds =
      sharedQueryIdToDecomposedQueryPlanIds.find(sharedQueryId);
  ...
  if (foundQuerySubPlanIds == sharedQueryIdToDecomposedQueryPlanIds.end()) {
    NES_ERROR(
        "AbstractQueryManager::getQueryStatistics: query does not exists {}",
        sharedQueryId);
    return queryStatistics;
  }

  NES_TRACE(
      "QueryManager: Extracting query execution ids for the input query {}",
      sharedQueryId);
  std::vector<DecomposedQueryPlanId> querySubPlanIds =
      (*foundQuerySubPlanIds).second;
  for (auto querySubPlanId : querySubPlanIds) {
    queryStatistics.emplace_back(
        queryManager->getQueryStatistics(querySubPlanId));
  }
  return queryStatistics;
}
...
std::vector<QueryStatistics> NodeEngine::getQueryStatistics(bool withReset) {
  ...
  for (auto &plan : sharedQueryIdToDecomposedQueryPlanIds) {
    ...
    for (auto querySubPlanId : querySubPlanIds) {
      NES_TRACE("querySubPlanId={} stat= {}", querySubPlanId,
                queryManager->getQueryStatistics(querySubPlanId)
                    ->getQueryStatisticsAsString());

      queryStatistics.push_back(
          queryManager->getQueryStatistics(querySubPlanId).operator*());
      if (withReset) {
...
std::vector<DecomposedQueryPlanId>
NodeEngine::getDecomposedQueryIds(SharedQueryId sharedQueryId) {
...
void NodeEngine::onFatalError(int signalNumber, std::string callstack) {
  if (callstack.empty()) {
    ...
    std::cerr << "Runtime failed fatally"
              << std::endl; // it's necessary for testing and it wont harm us to
                            // write to stderr
    std::cerr << "Error: " << strerror(errno) << std::endl;
    std::cerr << "Signal: " << std::to_string(signalNumber) << std::endl;
  } else {
    NES_ERROR("onFatalError: signal [{}] error [{}] callstack {}", signalNumber,
              strerror(errno), callstack);
    std::cerr << "Runtime failed fatally"
              << std::endl; // it's necessary for testing and it won't harm us
                            // to write to stderr
    std::cerr << "Error: " << strerror(errno) << std::endl;
    std::cerr << "Signal: " << std::to_string(signalNumber) << std::endl;
    std::cerr << "Callstack:\n " << callstack << std::endl;
...
void NodeEngine::onFatalException(
    const std::shared_ptr<std::exception> exception, std::string callstack) {
  if (callstack.empty()) {
...
const std::vector<PhysicalSourceTypePtr> &
NodeEngine::getPhysicalSourceTypes() const {
  return physicalSources;
}

std::shared_ptr<const Execution::ExecutableQueryPlan>
NodeEngine::getExecutableQueryPlan(
...
bool NodeEngine::bufferData(DecomposedQueryPlanId decomposedQueryPlanId,
  ...
  if (deployedExecutableQueryPlans.find(decomposedQueryPlanId) ==
    ...
    auto it = std::find_if(
        ...
        [uniqueNetworkSinkDescriptorId](const DataSinkPtr &dataSink) {
          Network::NetworkSinkPtr networkSink =
              std::dynamic_pointer_cast<Network::NetworkSink>(dataSink);
          return networkSink &&
                 networkSink->getUniqueNetworkSinkDescriptorId() ==
    ...
    if (it != sinks.end()) {
      ...
      // ReconfigurationMessage(querySubPlanId,BufferData,networkSink);
    ...
    NES_DEBUG("Query Sub Plan with ID {} did not contain a Network Sink with a "
              "Descriptor with ID {}",
              decomposedQueryPlanId, uniqueNetworkSinkDescriptorId);
    return false;
  }
}
...
bool NodeEngine::updateNetworkSink(WorkerId newNodeId,
  ...
  if (deployedExecutableQueryPlans.find(decomposedQueryPlanId) ==
    ...
    // make sure that query sub plan has network sink with specified id
    auto it = std::find_if(
        networkSinks.begin(), networkSinks.end(),
        [uniqueNetworkSinkDescriptorId](const DataSinkPtr &dataSink) {
          Network::NetworkSinkPtr networkSink =
              std::dynamic_pointer_cast<Network::NetworkSink>(dataSink);
          return networkSink &&
    ...
    if (it != networkSinks.end()) {
      ...
      // ReconfigurationMessage message =
      // ReconfigurationMessage(querySubPlanId,UpdateSinks,networkSink,
      // newNodeLocation);
      // queryManager->addReconfigurationMessage(querySubPlanId,message,true);
      NES_NOT_IMPLEMENTED();
      return true;
...
bool NodeEngine::reconfigureSubPlan(
    DecomposedQueryPlanPtr &reconfiguredDecomposedQueryPlan) {
  std::unique_lock lock(engineMutex);
  NES_DEBUG("Received for shared query plan {} the decomposed query plan {} "
            "for reconfiguration.",
            reconfiguredDecomposedQueryPlan->getSharedQueryId(),
            reconfiguredDecomposedQueryPlan->getDecomposedQueryPlanId());
  auto deployedPlanIterator = deployedExecutableQueryPlans.find(
      reconfiguredDecomposedQueryPlan->getDecomposedQueryPlanId());
  ...
  /* iterator over all network sinks of the running plan and apply the new
   * descriptors. If the version number of the new descriptor is the same as the
   * running version, nothing will be changed */
  for (auto &sink : deployedPlan->getSinks()) {
    ...
    if (networkSink != nullptr) {
      for (auto &reconfiguredSink :
           reconfiguredDecomposedQueryPlan->getSinkOperators()) {
        auto reconfiguredNetworkSinkDescriptor =
            std::dynamic_pointer_cast<const Network::NetworkSinkDescriptor>(
                reconfiguredSink->getSinkDescriptor());
        if (reconfiguredNetworkSinkDescriptor &&
            reconfiguredNetworkSinkDescriptor->getUniqueId() ==
                networkSink->getUniqueNetworkSinkDescriptorId()) {
          NES_DEBUG(
              "Reconfiguring the network sink {} with new descriptor for "
              "shared query plan {} and the decomposed "
              "query plan {}.",
              reconfiguredNetworkSinkDescriptor->getUniqueId(),
              reconfiguredDecomposedQueryPlan->getSharedQueryId(),
              reconfiguredDecomposedQueryPlan->getDecomposedQueryPlanId());
          networkSink->scheduleNewDescriptor(
              *reconfiguredNetworkSinkDescriptor);
        }
      }
    }
  }
  ...
  for (auto &source : deployedPlan->getSources()) {
    auto networkSource =
        std::dynamic_pointer_cast<Network::NetworkSource>(source);
    if (networkSource != nullptr) {
      for (auto &reconfiguredSource :
           reconfiguredDecomposedQueryPlan->getSourceOperators()) {
        auto reconfiguredNetworkSourceDescriptor =
            std::dynamic_pointer_cast<const Network::NetworkSourceDescriptor>(
                reconfiguredSource->getSourceDescriptor());
        if (reconfiguredNetworkSourceDescriptor->getUniqueId() ==
            networkSource->getUniqueId()) {
          NES_DEBUG(
              "Reconfiguring the network source {} with new descriptor for "
              "shared query plan {} and the "
              "decomposed query plan {}.",
              reconfiguredNetworkSourceDescriptor->getUniqueId(),
              reconfiguredDecomposedQueryPlan->getSharedQueryId(),
              reconfiguredDecomposedQueryPlan->getDecomposedQueryPlanId());
          networkSource->scheduleNewDescriptor(
              *reconfiguredNetworkSourceDescriptor);
        }
      }
    }
  }
...
void NodeEngine::updatePhysicalSources(
    const std::vector<PhysicalSourceTypePtr> &physicalSources) {
