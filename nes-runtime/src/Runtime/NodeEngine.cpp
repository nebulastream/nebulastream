...
bool NodeEngine::registerDecomposableQueryPlan(
  ...
  NES_INFO(
      "Creating ExecutableQueryPlan for shared query plan {} and decomposed "
      "query plan {}",
...
bool NodeEngine::registerExecutableQueryPlan(
  ...
  DecomposedQueryPlanId decomposedQueryPlanId =
      queryExecutionPlan->getDecomposedQueryPlanId();
  NES_DEBUG(
      "Runtime: registerExecutableQueryPlan query with sharedQueryId= {} "
...
bool NodeEngine::startQuery(SharedQueryId sharedQueryId,
  ...
  if (sharedQueryIdToDecomposedQueryPlanIds.find(sharedQueryId) !=
    ...
    if (std::find(decomposedQueryPlanIds.begin(), decomposedQueryPlanIds.end(),
      ...
      NES_ERROR(
          "Runtime: Unable to find qep with id {} for the shared query {}. "
          "Start failed.",
    ...
    for (const auto querySubPlanId : querySubPlanIds) {
      ...
      switch (qep->getStatus()) {
        ...
        case Execution::ExecutableQueryPlanStatus::Running: {
          NES_DEBUG(
              "Runtime: unregister of query  {}  is not Stopped... stopping "
              "now",
              querySubPlanId);
      ...
      case QueryTerminationType::Failure: {
        try {
          if (queryManager->failQuery(
            ...
            NES_ERROR("Runtime: failure of QEP {} failed",
                      decomposedQueryPlanId);
            return false;
          }
        } catch (std::exception const& exception) {
...
BufferManagerPtr NodeEngine::getBufferManager(
    uint32_t bufferManagerIndex) const {
...
Execution::ExecutableQueryPlanStatus NodeEngine::getQueryStatus(
...
void NodeEngine::onServerError(Network::Messages::ErrorMessage err) {
  switch (err.getErrorType()) {
    ...
    case Network::Messages::ErrorType::VersionMismatchError: {
      NES_INFO(
          "Runtime: Node {} encountered server error: Version mismatch for "
          "requested partition {}",
...
std::vector<DecomposedQueryPlanId> NodeEngine::getDecomposedQueryIds(
    SharedQueryId sharedQueryId) {
...
void NodeEngine::onFatalError(int signalNumber, std::string callstack) {
  if (callstack.empty()) {
    NES_ERROR(
        "onFatalError: signal [{}] error [{}] (enable NES_DEBUG to view "
        "stacktrace)",
        signalNumber, strerror(errno));
    std::cerr << "Runtime failed fatally"
              << std::endl;  // it's necessary for testing and it wont harm us
                             // to write to stderr
...
void NodeEngine::onFatalException(
  ...
  if (callstack.empty()) {
    NES_ERROR(
        "onFatalException: exception=[{}] (enable NES_DEBUG to view "
        "stacktrace)",
        exception->what());
...
const std::vector<PhysicalSourceTypePtr>& NodeEngine::getPhysicalSourceTypes()
    const {
...
bool NodeEngine::bufferData(DecomposedQueryPlanId decomposedQueryPlanId,
  ...
  if (deployedExecutableQueryPlans.find(decomposedQueryPlanId) ==
    ...
    NES_DEBUG(
        "Query Sub Plan with ID {} did not contain a Network Sink with a "
        "Descriptor with ID {}",
...
bool NodeEngine::reconfigureSubPlan(
  ...
  std::unique_lock lock(engineMutex);
  NES_DEBUG(
      "Received for shared query plan {} the decomposed query plan {} for "
      "reconfiguration.",
