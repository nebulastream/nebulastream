    ...
    , sourceSharing(sourceSharing)
{
    ...
    networkManager = networkManagerCreator(std::shared_ptr<NodeEngine>(
        this,
        [](NodeEngine*)
        {
            // nop
...
bool NodeEngine::registerDecomposableQueryPlan(const DecomposedQueryPlanPtr& decomposedQueryPlan)
{
    ...
    NES_INFO("Creating ExecutableQueryPlan for shared query plan {} and decomposed query plan {}", sharedQueryId, decomposedQueryPlanId);
...
bool NodeEngine::registerExecutableQueryPlan(const Execution::ExecutableQueryPlanPtr& queryExecutionPlan)
{
    ...
    NES_DEBUG(
        "Runtime: registerExecutableQueryPlan query with sharedQueryId= {} decomposedQueryPlanId = {}",
        sharedQueryId,
...
bool NodeEngine::startQuery(SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId)
{
    ...
    if (sharedQueryIdToDecomposedQueryPlanIds.find(sharedQueryId) != sharedQueryIdToDecomposedQueryPlanIds.end())
    {
        ...
        if (std::find(decomposedQueryPlanIds.begin(), decomposedQueryPlanIds.end(), decomposedQueryPlanId) == decomposedQueryPlanIds.end())
        {
            NES_ERROR(
                "Runtime: Unable to find qep with id {} for the shared query {}. Start failed.", decomposedQueryPlanId, sharedQueryId);
            return false;
        }
...
bool NodeEngine::stopQuery(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId, Runtime::QueryTerminationType terminationType)
{
    std::unique_lock lock(engineMutex);
...
WorkerId NodeEngine::getWorkerId()
{
    return nodeEngineId;
}
...
const std::vector<PhysicalSourceTypePtr>& NodeEngine::getPhysicalSourceTypes() const
{
    return physicalSources;
}

std::shared_ptr<const Execution::ExecutableQueryPlan> NodeEngine::getExecutableQueryPlan(DecomposedQueryPlanId decomposedQueryPlanId) const
{
    std::unique_lock lock(engineMutex);
...
bool NodeEngine::bufferData(DecomposedQueryPlanId decomposedQueryPlanId, OperatorId uniqueNetworkSinkDescriptorId)
{
    ...
    else
    {
        ...
        NES_DEBUG(
            "Query Sub Plan with ID {} did not contain a Network Sink with a Descriptor with ID {}",
            decomposedQueryPlanId,
...
bool NodeEngine::updateNetworkSink(
    WorkerId newNodeId,
...
bool NodeEngine::reconfigureSubPlan(DecomposedQueryPlanPtr& reconfiguredDecomposedQueryPlan)
{
    std::unique_lock lock(engineMutex);
    NES_DEBUG(
        "Received for shared query plan {} the decomposed query plan {} for reconfiguration.",
    ...
    for (auto& sink : deployedPlan->getSinks())
    {
        ...
        if (networkSink != nullptr)
        {
            for (auto& reconfiguredSink : reconfiguredDecomposedQueryPlan->getSinkOperators())
            {
                ...
                {
                    NES_DEBUG(
                        "Reconfiguring the network sink {} with new descriptor for shared query plan {} and the decomposed "
                        "query plan {}.",
    ...
    for (auto& source : deployedPlan->getSources())
    {
        ...
        if (networkSource != nullptr)
        {
            for (auto& reconfiguredSource : reconfiguredDecomposedQueryPlan->getSourceOperators())
            {
                ...
                {
                    NES_DEBUG(
                        "Reconfiguring the network source {} with new descriptor for shared query plan {} and the "
...
Monitoring::MetricStorePtr NodeEngine::getMetricStore()
{
    return metricStore;
}
...
WorkerId NodeEngine::getNodeId() const
{
    return nodeId;
}
void NodeEngine::setNodeId(const WorkerId NodeId)
{
    nodeId = NodeId;
}
