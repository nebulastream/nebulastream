...
NodeEngine::NodeEngine(
    std::shared_ptr<Memory::BufferManager> buffer_manager, std::shared_ptr<QueryLog> query_log, std::unique_ptr<QueryEngine> query_manager)
...
void NodeEngine::startQuery(QueryId queryId)
{
    ...
    {
        queryManager->start(queryId, InstantiatedQueryPlan::instantiate(std::move(qep), bufferManager));
    }
    else
    {
