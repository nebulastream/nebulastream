...
{
public:
    OpenCLInitializationException(const std::string & message, cl_int statusCode) : std::runtime_error(message), statusCode(statusCode) { }
...
template <>
std::string
retrieveOpenCLDeviceInfo<char[], std::string>(const cl_device_id deviceId, const cl_device_info param, const std::string & paramName)
{
    size_t size;
...
bool retrieveOpenCLDoubleFPSupport(const cl_device_id deviceId)
{
    ...
    auto deviceFpConfig = retrieveOpenCLDeviceInfo<cl_device_fp_config, cl_device_fp_config>(
...
{
    auto dimensions
        = retrieveOpenCLDeviceInfo<cl_uint, unsigned>(deviceId, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, "CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS");
    ...
    try
    {
        ...
        for (const auto & platformId : platforms)
        {
            ...
            for (const auto & device : devices)
            {
                ...
                auto deviceExtensions = retrieveOpenCLDeviceInfo<char[], std::string>(device, CL_DEVICE_EXTENSIONS, "CL_DEVICE_EXTENSIONS");
                auto availableProcessors
                    = retrieveOpenCLDeviceInfo<cl_uint, unsigned>(device, CL_DEVICE_MAX_COMPUTE_UNITS, "CL_DEVICE_MAX_COMPUTE_UNITS");
                ...
                auto globalMemory
                    = retrieveOpenCLDeviceInfo<cl_ulong, unsigned long>(device, CL_DEVICE_GLOBAL_MEM_SIZE, "CL_DEVICE_GLOBAL_MEM_SIZE");
                NES_INFO(
