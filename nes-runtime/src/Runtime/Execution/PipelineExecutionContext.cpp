...
PipelineExecutionContext::PipelineExecutionContext(
    ...
    : pipelineId(pipelineId), queryId(queryId),
...
void PipelineExecutionContext::emitBuffer(TupleBuffer &buffer,
...
std::vector<OperatorHandlerPtr>
PipelineExecutionContext::getOperatorHandlers() {
...
uint64_t PipelineExecutionContext::getNextChunkNumber(
    const SeqNumberOriginId seqNumberOriginId) {
  auto lockedMap = seqNumberOriginIdToOutputChunkNumber.wlock();
  (*lockedMap)[seqNumberOriginId] += 1;
  return (*lockedMap)[seqNumberOriginId];
}

void PipelineExecutionContext::removeSequenceState(
    const SeqNumberOriginId seqNumberOriginId) {
...
bool PipelineExecutionContext::isLastChunk(
    const SeqNumberOriginId seqNumberOriginId, const uint64_t chunkNumber,
  ...
  NES_TRACE("seqNumberOriginId = {} chunkNumber = {} isLastChunk = {} "
            "seenChunks = {} lastChunkNumber = {}",
            seqNumberOriginId.toString(), chunkNumber, isLastChunk,
            chunkState.seenChunks, chunkState.lastChunkNumber)
