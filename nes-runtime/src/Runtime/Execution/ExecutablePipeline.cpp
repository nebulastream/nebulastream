...
ExecutablePipeline::ExecutablePipeline(
    PipelineId pipelineId, SharedQueryId sharedQueryId,
    DecomposedQueryPlanId decomposedQueryPlanId, QueryManagerPtr queryManager,
    ...
    : pipelineId(pipelineId), sharedQueryId(sharedQueryId),
      decomposedQueryPlanId(decomposedQueryPlanId), queryManager(queryManager),
      executablePipelineStage(std::move(executablePipelineStage)),
      pipelineContext(std::move(pipelineExecutionContext)),
      reconfiguration(reconfiguration),
      pipelineStatus(reconfiguration ? PipelineStatus::PipelineRunning
                                     : PipelineStatus::PipelineCreated),
      activeProducers(numOfProducingPipelines),
      successorPipelines(std::move(successorPipelines)) {
  // nop
  NES_ASSERT(this->executablePipelineStage && this->pipelineContext &&
                 numOfProducingPipelines > 0,
             "Wrong pipeline stage argument");
}

ExecutionResult ExecutablePipeline::execute(TupleBuffer &inputBuffer,
                                            WorkerContextRef workerContext) {
  NES_TRACE("Execute Pipeline Stage with id={} originId={} stage={}",
            decomposedQueryPlanId, inputBuffer.getOriginId(), pipelineId);

  switch (this->pipelineStatus.load()) {
  case PipelineStatus::PipelineRunning: {
    auto res = executablePipelineStage->execute(
        inputBuffer, *pipelineContext.get(), workerContext);
    return res;
  }
  ...
  default: {
    NES_ERROR("Cannot execute Pipeline Stage with id={} originId={} stage={} "
...
bool ExecutablePipeline::setup(const QueryManagerPtr &,
                               const BufferManagerPtr &) {
...
bool ExecutablePipeline::start() {
  ...
  if (pipelineStatus.compare_exchange_strong(expected,
                                             PipelineStatus::PipelineRunning)) {
    auto newReconf = ReconfigurationMessage(
        sharedQueryId, decomposedQueryPlanId, ReconfigurationType::Initialize,
    ...
    queryManager->addReconfigurationMessage(
        sharedQueryId, decomposedQueryPlanId, newReconf, true);
...
bool ExecutablePipeline::stop(QueryTerminationType) {
  ...
  if (pipelineStatus.compare_exchange_strong(expected,
                                             PipelineStatus::PipelineStopped)) {
...
const std::vector<SuccessorExecutablePipeline> &
ExecutablePipeline::getSuccessors() const {
  return successorPipelines;
}

void ExecutablePipeline::onEvent(Runtime::BaseEvent &event) {
  NES_DEBUG("ExecutablePipeline::onEvent(event) called. pipelineId:  {}",
            this->pipelineId);
  if (event.getEventType() == EventType::kStartSourceEvent) {
    NES_DEBUG("ExecutablePipeline: Propagate startSourceEvent further upstream "
              "to predecessors, without workerContext.");

    for (auto predecessor : this->pipelineContext->getPredecessors()) {
      if (const auto *sourcePredecessor =
              std::get_if<std::weak_ptr<NES::DataSource>>(&predecessor)) {
        NES_DEBUG("ExecutablePipeline: Found Source in predecessor. Start it "
                  "with startSourceEvent, without workerContext.");
        sourcePredecessor->lock()->onEvent(event);
      } else if (const auto *pipelinePredecessor = std::get_if<std::weak_ptr<
                     NES::Runtime::Execution::ExecutablePipeline>>(
                     &predecessor)) {
        NES_DEBUG("ExecutablePipeline: Found Pipeline in Predecessors. "
                  "Propagate startSourceEvent to it, without "
                  "workerContext.");
...
void ExecutablePipeline::onEvent(Runtime::BaseEvent &event,
                                 WorkerContextRef workerContext) {
  NES_DEBUG(
      "ExecutablePipeline::onEvent(event, wrkContext) called. pipelineId:  {}",
      this->pipelineId);
  if (event.getEventType() == EventType::kStartSourceEvent) {
    NES_DEBUG("ExecutablePipeline: Propagate startSourceEvent further upstream "
              "to predecessors, with workerContext.");

    for (auto predecessor : this->pipelineContext->getPredecessors()) {
      if (const auto *sourcePredecessor =
              std::get_if<std::weak_ptr<NES::DataSource>>(&predecessor)) {
        NES_DEBUG("ExecutablePipeline: Found Source in predecessor. Start it "
                  "with startSourceEvent, with workerContext.");
        sourcePredecessor->lock()->onEvent(event, workerContext);
      } else if (const auto *pipelinePredecessor = std::get_if<std::weak_ptr<
                     NES::Runtime::Execution::ExecutablePipeline>>(
                     &predecessor)) {
        NES_DEBUG("ExecutablePipeline: Found Pipeline in Predecessors. "
                  "Propagate startSourceEvent to it, with workerContext.");
...
SharedQueryId ExecutablePipeline::getSharedQueryId() const {
  return sharedQueryId;
}

DecomposedQueryPlanId ExecutablePipeline::getDecomposedQueryPlanId() const {
  return decomposedQueryPlanId;
}

bool ExecutablePipeline::isReconfiguration() const { return reconfiguration; }

ExecutablePipelinePtr ExecutablePipeline::create(
    PipelineId pipelineId, SharedQueryId sharedQueryId,
  ...
  NES_ASSERT2_FMT(executablePipelineStage != nullptr,
                  "Executable pipelinestage is null for "
                      << pipelineId << "within the following query sub plan: "
                      << decomposedQueryPlanId);
  NES_ASSERT2_FMT(pipelineExecutionContext != nullptr,
                  "Pipeline context is null for "
                      << pipelineId << "within the following query sub plan: "
                      << decomposedQueryPlanId);

  return std::make_shared<ExecutablePipeline>(
      pipelineId, sharedQueryId, decomposedQueryPlanId, queryManager,
      pipelineExecutionContext, executablePipelineStage,
      numOfProducingPipelines, successorPipelines, reconfiguration);
}

void ExecutablePipeline::reconfigure(ReconfigurationMessage &task,
                                     WorkerContext &context) {
  NES_DEBUG(
      "Going to reconfigure pipeline {} belonging to query id: {} stage id: {}",
      pipelineId, decomposedQueryPlanId, pipelineId);
  Reconfigurable::reconfigure(task, context);
  switch (task.getType()) {
  case ReconfigurationType::Initialize: {
    NES_ASSERT2_FMT(isRunning(), "Going to reconfigure a non-running pipeline "
                                     << pipelineId << " belonging to query id: "
                                     << decomposedQueryPlanId
                                     << " stage id: " << pipelineId);
  ...
  case ReconfigurationType::SoftEndOfStream: {
    if (context.decreaseObjectRefCnt(this) == 1) {
      for (const auto &operatorHandler :
           pipelineContext->getOperatorHandlers()) {
...
void ExecutablePipeline::postReconfigurationCallback(
    ReconfigurationMessage &task) {
  NES_DEBUG("Going to execute postReconfigurationCallback on pipeline "
            "belonging to subplanId: {} stage id: {}",
            decomposedQueryPlanId, pipelineId);
  ...
  case ReconfigurationType::FailEndOfStream: {
    ...
    if (prevProducerCounter == 1) { // all producers sent EOS
      ...
      for (const auto &successorPipeline : successorPipelines) {
        if (auto *pipe =
                std::get_if<ExecutablePipelinePtr>(&successorPipeline)) {
          auto newReconf = ReconfigurationMessage(
              sharedQueryId, decomposedQueryPlanId, task.getType(), *pipe);
          queryManager->addReconfigurationMessage(
              sharedQueryId, decomposedQueryPlanId, newReconf, false);
          NES_DEBUG("Going to reconfigure next pipeline belonging to "
                    "subplanId: {} stage id: {} got "
                    "FailEndOfStream with nextPipeline",
                    decomposedQueryPlanId, (*pipe)->getPipelineId());
        } else if (auto *sink = std::get_if<DataSinkPtr>(&successorPipeline)) {
          auto newReconf = ReconfigurationMessage(
              sharedQueryId, decomposedQueryPlanId, task.getType(), *sink);
          queryManager->addReconfigurationMessage(
              sharedQueryId, decomposedQueryPlanId, newReconf, false);
          NES_DEBUG("Going to reconfigure next sink belonging to subplanId: {} "
                    "sink id:{} got FailEndOfStream  "
                    "with nextPipeline",
                    decomposedQueryPlanId, (*sink)->toString());
        }
      }
    }
  }
  case ReconfigurationType::HardEndOfStream:
  case ReconfigurationType::SoftEndOfStream: {
    // we mantain a set of producers, and we will only trigger the end of stream
    // once all producers have sent the EOS, for this we decrement the counter
    auto prevProducerCounter = activeProducers.fetch_sub(1);
    if (prevProducerCounter == 1) { // all producers sent EOS
      NES_DEBUG("Reconfiguration of pipeline belonging to subplanId:{} stage "
                "id:{} reached prev=1",
                decomposedQueryPlanId, pipelineId);
      auto terminationType =
          task.getType() == Runtime::ReconfigurationType::SoftEndOfStream
      ...
      queryManager->notifyPipelineCompletion(
          decomposedQueryPlanId,
          inherited0::shared_from_this<ExecutablePipeline>(), terminationType);
      ...
      for (const auto &successorPipeline : successorPipelines) {
        if (auto *pipe =
          ...
          queryManager->addReconfigurationMessage(
              sharedQueryId, decomposedQueryPlanId, newReconf, false);
          NES_DEBUG("Going to reconfigure next pipeline belonging to "
                    "subplanId: {} stage id: {} got EndOfStream  "
                    "with nextPipeline",
                    decomposedQueryPlanId, (*pipe)->getPipelineId());
        } else if (auto *sink = std::get_if<DataSinkPtr>(&successorPipeline)) {
          auto newReconf = ReconfigurationMessage(
      ...
      NES_DEBUG("Requested reconfiguration of pipeline belonging to subplanId: "
                "{} stage id: {} but refCount was {} "
                "and now is {}",
                decomposedQueryPlanId, pipelineId, (prevProducerCounter),
