...
void readCpuConfig(
    uint32_t &numa_nodes_count, uint32_t &num_physical_cpus,
...
HardwareManager::HardwareManager()
    : globalAllocator(std::make_shared<NesDefaultMemoryAllocator>()) {
...
#ifdef NES_USE_ONE_QUEUE_PER_NUMA_NODE
uint32_t HardwareManager::getNumberOfNumaRegions() const {
  return numaRegions.size();
}

NumaRegionMemoryAllocatorPtr
HardwareManager::getNumaAllocator(uint32_t numaNodeIndex) const {
  NES_ASSERT2_FMT(numaNodeIndex < numaRegions.size(),
                  "Invalid numa region " << numaNodeIndex);
  return numaRegions[numaNodeIndex];
}
#endif
NesDefaultMemoryAllocatorPtr HardwareManager::getGlobalAllocator() const {
