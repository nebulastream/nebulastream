...
std::string readVarSizedData(const TupleBuffer& buffer,
  ...
  std::memcpy(varSizedData.data(),
              childBuffer.getBuffer<char>() + sizeof(uint32_t), stringSize);
  return varSizedData;
}
...
MemoryLayout::MemoryLayout(uint64_t bufferSize, SchemaPtr schema)
    : bufferSize(bufferSize), schema(schema), recordSize(0) {
  auto physicalDataTypeFactory = DefaultPhysicalTypeFactory();
  for (size_t fieldIndex = 0; fieldIndex < schema->fields.size();
       fieldIndex++) {
    auto field = schema->fields[fieldIndex];
    auto physicalFieldSize =
...
std::optional<uint64_t> MemoryLayout::getFieldIndexFromName(
...
std::optional<uint64_t> MemoryLayout::getFieldOffset(
    uint64_t tupleIndex, std::string_view fieldName) const {
  ...
  NES_ERROR("Field with name {} not found in schema {}", fieldName,
            schema->toString());
  return {};
}
...
bool MemoryLayout::operator==(const MemoryLayout& rhs) const {
  return bufferSize == rhs.bufferSize && schema->equals(rhs.schema) &&
         recordSize == rhs.recordSize && capacity == rhs.capacity &&
         physicalFieldSizes == rhs.physicalFieldSizes &&
         physicalTypes == rhs.physicalTypes &&
