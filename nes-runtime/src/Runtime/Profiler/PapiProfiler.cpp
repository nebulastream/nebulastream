...
public:
  ...
  PapiInitializer() {
    NES_ASSERT2_FMT(PAPI_library_init(PAPI_VER_CURRENT) == PAPI_VER_CURRENT,
                    "Failed to load PAPI " << PAPI_VERSION
                                           << "!=" << PAPI_VER_CURRENT);
    NES_ASSERT2_FMT(PAPI_multiplex_init() == PAPI_OK,
                    "Failed to init multiplexing PAPI");
    NES_ASSERT2_FMT(PAPI_thread_init(static_cast<unsigned long (*)()>(
...
PapiCpuProfiler::PapiCpuProfiler(Presets preset, std::ofstream &&csvWriter,
                                 uint32_t threadId, uint32_t coreId)
    : csvWriter(std::move(csvWriter)), threadId(threadId), coreId(coreId),
      freqGhz(detail::measureRdtscFreq()), startTsc(detail::rdtsc()),
      preset(preset), eventSet(PAPI_NULL), isStarted(false) {
  auto err = PAPI_register_thread();
  NES_ASSERT2_FMT(err == PAPI_OK,
                  "Cannot register thread on PAPI worker err=" << err);
  ...
  case Presets::BranchPresets: {
    ...
    this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_BR_MSP,PAPI_BR_"
                       "INS,PAPI_BR_TKN,PAPI_BR_NTK,"
                       "mispred_per_record\n";
    err = PAPI_add_events(eventSet, currEvents.data(), currEvents.size());
    NES_ASSERT2_FMT(err == PAPI_OK,
                    "Cannot register events on PAPI reason: " << err);
    break;
  }
  case Presets::CachePresets: {
    currEvents = {PAPI_L1_TCM, PAPI_L2_TCM, PAPI_L3_TCM,
                  PAPI_L3_TCA, PAPI_L1_DCM, PAPI_L1_ICM};
    currSamples.resize(currEvents.size(), 0);
    this->csvWriter
        << "core_id,numRecords,worker_id,ts,PAPI_L1_TCM,PAPI_L2_TCM,"
  ...
  case Presets::CachePrefetcherPresets: {
    //            currEvents = {PAPI_PRF_DM, PAPI_CA_SHR, PAPI_CA_ITV,
    //            PAPI_CA_CLN, PAPI_TOT_INS, PAPI_TOT_CYC};
    currEvents = {PAPI_PRF_DM, PAPI_CA_SHR, PAPI_CA_ITV, PAPI_CA_CLN};
    currSamples.resize(currEvents.size(), 0);
    this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_PRF_DM,PAPI_CA_"
                       "SHR,PAPI_CA_ITV,PAPI_CA_CLN\n";
    err = PAPI_add_events(eventSet, currEvents.data(), currEvents.size());
    NES_ASSERT2_FMT(err == PAPI_OK,
                    "Cannot register events on PAPI reason: " << err);
    break;
  ...
  case Presets::IPC: {
    ...
    currSamples.resize(currEvents.size(), 0);
    this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_TOT_INS,PAPI_TOT_"
                       "CYC,PAPI_REF_CYC,"
  ...
  case Presets::ResourceUsage: { // PAPI_RES_STL PAPI_MEM_WCY PAPI_PRF_DM
                                 // PAPI_STL_CCY PAPI_REF_CYC
    currEvents = {PAPI_RES_STL, PAPI_MEM_WCY, PAPI_PRF_DM, PAPI_STL_CCY,
                  PAPI_REF_CYC};
    currSamples.resize(currEvents.size(), 0);
    this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_RES_STL,PAPI_MEM_"
                       "WCY,PAPI_PRF_DM,PAPI_STL_CCY,PAPI_REF_CYC\n";
    NES_ASSERT2_FMT(err == PAPI_OK,
                    "Cannot register events on PAPI reason: " << err);
    break;
  }
  case Presets::CachePresetsEx: {
    ...
    currSamples.resize(currEvents.size(), 0);
    this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_TLB_DM,PAPI_L3_"
                       "TCM,PAPI_TLB_IM,"
  ...
  case Presets::ICacheMisses: {
    ...
    currSamples.resize(currEvents.size(), 0);
    this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_L2_ICM,PAPI_L1_"
                       "ICM,PAPI_TLB_IM,"
                       "l1i_misses_per_record,l2i_misses_per_record,itlb_"
                       "misses_per_record\n";
    err = PAPI_add_events(eventSet, currEvents.data(), currEvents.size());
    NES_ASSERT2_FMT(err == PAPI_OK,
                    "Cannot register events on PAPI reason: " << err);
  ...
  case Presets::MemoryBound: {
    // EXE_ACTIVITY:BOUND_ON_STORES L1D_PEND_MISS:FB_FULL:c=1
    // CPU_CLK_THREAD_UNHALTED  MEM_LOAD_RETIRED.L1_MISS MEM_LOAD_RETIRED.L2_HIT
    // MEM_LOAD_RETIRED.FB_HIT CYCLE_ACTIVITY.STALLS_L1D_MISS
    ...
    NES_ASSERT2_FMT(
        err == PAPI_OK,
        "Cannot assign PAPI event set to component 0 worker={} with err {}"
            << err);
    err = PAPI_set_multiplex(eventSet);
    NES_ASSERT2_FMT(
        err == PAPI_OK,
        "Cannot promote PAPI event set to multiplex worker={} with err {}"
            << err);
    std::array events = {
        ...
        "MEM_LOAD_RETIRED.L1_MISS",       "CPU_CLK_THREAD_UNHALTED",
    ...
    for (const auto &event_name : events) {
      ...
      NES_ASSERT2_FMT(err == PAPI_OK,
                      "Cannot register event {} on PAPI worker={}: error={}"
                          << event_name << " err=" << err);
  ...
  case Presets::CachePrefetcherPresetsExt: {
    std::array events = {"L2_RQSTS:PF_MISS", "L2_RQSTS:PF_HIT",
                         "L2_RQSTS:ALL_PF"};
    currSamples.resize(events.size(), 0);
    this->csvWriter << "core_id,numRecords,worker_id,ts";
    err = PAPI_assign_eventset_component(eventSet, 0); // 0 is cpu component
  ...
  case Presets::FrontendLatency: {
    ...
    this->csvWriter << ",frontend_latency,frontend_bound,frontend_bandwidth,"
                       "icache_misses,itlb_misses,ilp\n";
    break;
  }
  case Presets::CacheBandwidth: {
    // LONGEST_LAT_CACHE:MISS OFFCORE_REQUESTS:ALL_REQUESTS L2_LINES_IN:ALL
    // L1D:REPLACEMENT
    NES_ASSERT2_FMT(PAPI_add_named_event(eventSet, "LONGEST_LAT_CACHE:MISS") ==
                        PAPI_OK,
                    "Cannot register events on PAPI worker={}");
    NES_ASSERT2_FMT(PAPI_add_named_event(
                        eventSet, "OFFCORE_REQUESTS:ALL_REQUESTS") == PAPI_OK,
                    "Cannot register events on PAPI worker={}");
    NES_ASSERT2_FMT(PAPI_add_named_event(eventSet, "L2_LINES_IN:ALL") ==
                        PAPI_OK,
                    "Cannot register events on PAPI worker={}");
    NES_ASSERT2_FMT(PAPI_add_named_event(eventSet, "L1D:REPLACEMENT") ==
                        PAPI_OK,
                    "Cannot register events on PAPI worker={}");
    currSamples.resize(4, 0);
    this->csvWriter
        << "core_id,numRecords,worker_id,ts,LONGEST_LAT_CACHE:MISS,OFFCORE_"
           "REQUESTS:ALL_REQUESTS,L2_LINES_IN:ALL,"
  ...
  case Presets::CoreBound: {
    ...
    // EXE_ACTIVITY:2_PORTS_UTIL UOPS_RETIRED:RETIRE_SLOTS

    break;
  }
  case Presets::MultiplexExtended: { // skylake only
    // L1D_PEND_MISS:FB_FULL:c=1 CPU_CLK_THREAD_UNHALTED
    // MEM_LOAD_RETIRED.L1_MISS MEM_LOAD_RETIRED.L2_HIT MEM_LOAD_RETIRED.FB_HIT
    // CYCLE_ACTIVITY.STALLS_L1D_MISS CYCLE_ACTIVITY.STALLS_L2_MISS
    // CYCLE_ACTIVITY.STALLS_L3_MISS
    err = PAPI_assign_eventset_component(eventSet, 0); // 0 is cpu component
    NES_ASSERT2_FMT(
  ...
  case Presets::Multiplexing: {                        // skylake only
    ...
    this->csvWriter
        << ",frontend_bound,bad_speculation,branch_mispred,retiring,"
...
PapiCpuProfiler::~PapiCpuProfiler() {
  ...
  NES_ASSERT2_FMT(err == PAPI_OK,
                  "Cannot remove PAPI event set with err= " << err);
  err = PAPI_destroy_eventset(&eventSet);
  NES_ASSERT2_FMT(err == PAPI_OK,
                  "Cannot destroy PAPI event set with err=" << err);
  NES_ASSERT2_FMT(PAPI_unregister_thread() == PAPI_OK,
                  "Cannot unregister thread on PAPI worker=" << threadId);
}

uint64_t PapiCpuProfiler::startSampling() {
  NES_ASSERT2_FMT(PAPI_start(eventSet) == PAPI_OK,
                  "Cannot start PAPI sampling on worker=" << threadId);
  isStarted = true;
...
uint64_t PapiCpuProfiler::stopSampling(std::size_t numRecords) {
  ...
  NES_ASSERT2_FMT(PAPI_stop(eventSet, currSamples.data()) == PAPI_OK,
                  "Cannot stop PAPI sampling on worker=" << threadId);
  ...
  case Presets::IPC: {
    ...
    double cycles_per_record =
        numRecords == 0 ? 0 : cycles / double(numRecords);
    // double l1_misses_per_record = numRecords == 0 ? 0 : currSamples[0] /
    // double(numRecords); double l2_misses_per_record = numRecords == 0 ? 0 :
    // currSamples[1] / double(numRecords); double l3_misses_per_record =
    // numRecords == 0 ? 0 : currSamples[2] / double(numRecords);
    this->csvWriter << "," << ipc << "," << cpi << "," << instr_per_record
                    << "," << cycles_per_record;
    break;
  }
  case Presets::BranchPresets: {
    double mispred_per_record =
        numRecords == 0 ? 0 : currSamples[0] / double(numRecords);
  ...
  case Presets::ICacheMisses: {
    ...
    //            double ipc = instr / cycles;
    //            double cpi = cycles / instr;
    //            double instr_per_record = numRecords == 0 ? 0 : instr /
    //            double(numRecords); double cycles_per_record = numRecords == 0
    //            ? 0 : cycles / double(numRecords);
    double l1i_misses_per_record =
        numRecords == 0 ? 0 : currSamples[0] / double(numRecords);
    double l2i_misses_per_record =
        numRecords == 0 ? 0 : currSamples[1] / double(numRecords);
    double itlb_misses_per_record =
        numRecords == 0 ? 0 : currSamples[2] / double(numRecords);
    this->csvWriter << "," << l1i_misses_per_record << ","
                    << l2i_misses_per_record << "," << itlb_misses_per_record;
    break;
  }
  case Presets::CachePresets: {
    double l1MissesPerRecord =
        numRecords == 0 ? 0 : currSamples[4] / double(numRecords);
    double l2MissesPerRecord =
        numRecords == 0 ? 0 : currSamples[1] / double(numRecords);
    double l3MissesPerRecord =
        numRecords == 0 ? 0 : currSamples[2] / double(numRecords);
    double l1iMissesPerRecord =
        numRecords == 0 ? 0 : currSamples[5] / double(numRecords);
    csvWriter << "," << l1MissesPerRecord << "," << l2MissesPerRecord << ","
              << l3MissesPerRecord;
    csvWriter << "," << (1. / l1MissesPerRecord) << ","
              << (1. / l2MissesPerRecord) << "," << (1. / l3MissesPerRecord)
              << "," << l1iMissesPerRecord;
    break;
  }
  default: {
