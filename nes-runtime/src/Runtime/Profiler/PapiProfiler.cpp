...
PapiCpuProfiler::PapiCpuProfiler(Presets preset, std::ofstream&& csvWriter,
  ...
  switch (preset) {
    ...
    case Presets::IPC: {
      ...
      currSamples.resize(currEvents.size(), 0);
      this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_TOT_INS,PAPI_"
                         "TOT_CYC,PAPI_REF_CYC,"
    ...
    case Presets::ResourceUsage: {  // PAPI_RES_STL PAPI_MEM_WCY PAPI_PRF_DM
      ...
      currEvents = {PAPI_RES_STL, PAPI_MEM_WCY, PAPI_PRF_DM, PAPI_STL_CCY,
                    PAPI_REF_CYC};
      currSamples.resize(currEvents.size(), 0);
      this->csvWriter << "core_id,numRecords,worker_id,ts,PAPI_RES_STL,PAPI_"
                         "MEM_WCY,PAPI_PRF_DM,PAPI_STL_CCY,PAPI_REF_CYC\n";
      NES_ASSERT2_FMT(err == PAPI_OK,
                      "Cannot register events on PAPI reason: " << err);
      break;
    }
    ...
    case Presets::MemoryBound: {
      ...
      // CPU_CLK_THREAD_UNHALTED  MEM_LOAD_RETIRED.L1_MISS
    ...
    case Presets::CacheBandwidth: {
      ...
      // L1D:REPLACEMENT
      NES_ASSERT2_FMT(
          PAPI_add_named_event(eventSet, "LONGEST_LAT_CACHE:MISS") == PAPI_OK,
          "Cannot register events on PAPI worker={}");
      NES_ASSERT2_FMT(PAPI_add_named_event(
                          eventSet, "OFFCORE_REQUESTS:ALL_REQUESTS") == PAPI_OK,
                      "Cannot register events on PAPI worker={}");
      NES_ASSERT2_FMT(
          PAPI_add_named_event(eventSet, "L2_LINES_IN:ALL") == PAPI_OK,
          "Cannot register events on PAPI worker={}");
      NES_ASSERT2_FMT(
          PAPI_add_named_event(eventSet, "L1D:REPLACEMENT") == PAPI_OK,
          "Cannot register events on PAPI worker={}");
...
uint64_t PapiCpuProfiler::stopSampling(std::size_t numRecords) {
  ...
  switch (preset) {
    case Presets::IPC: {
      ...
      double instr_per_record =
          numRecords == 0 ? 0 : instr / double(numRecords);
    ...
    case Presets::ICacheMisses: {
      ...
      //            double(numRecords); double cycles_per_record = numRecords ==
      //            0 ? 0 : cycles / double(numRecords);
