...
AbstractQueryManager::AbstractQueryManager(
    ...
    std::vector<BufferManagerPtr> bufferManagers, WorkerId nodeEngineId,
    uint16_t numThreads, HardwareManagerPtr hardwareManager,
    uint64_t numberOfBuffersPerEpoch, std::vector<uint64_t> workerToCoreMapping)
    : nodeEngineId(nodeEngineId), bufferManagers(std::move(bufferManagers)),
      numThreads(numThreads), hardwareManager(std::move(hardwareManager)),
  ...
  asyncTaskExecutor =
      std::make_shared<AsyncTaskExecutor>(this->hardwareManager, 1);
}
...
DynamicQueryManager::DynamicQueryManager(
    ...
    : AbstractQueryManager(std::move(queryStatusListener),
                           std::move(bufferManagers), nodeEngineId, numThreads,
                           std::move(hardwareManager), numberOfBuffersPerEpoch,
...
MultiQueueQueryManager::MultiQueueQueryManager(
    ...
    uint16_t numThreads, HardwareManagerPtr hardwareManager,
    uint64_t numberOfBuffersPerEpoch, std::vector<uint64_t> workerToCoreMapping,
    uint64_t numberOfQueues, uint64_t numberOfThreadsPerQueue)
    : AbstractQueryManager(std::move(queryStatusListener),
                           std::move(bufferManagers), nodeEngineId, numThreads,
                           std::move(hardwareManager), numberOfBuffersPerEpoch,
                           std::move(workerToCoreMapping)),
  ...
  NES_DEBUG("QueryManger: use static mode for numberOfQueues={} numThreads={} "
            "numberOfThreadsPerQueue=",
            numberOfQueues, numThreads, numberOfThreadsPerQueue);
...
bool DynamicQueryManager::startThreadPool(uint64_t numberOfBuffersPerWorker) {
  NES_DEBUG("startThreadPool: setup thread pool for nodeEngineId= {}  with "
            "numThreads= {}",
            nodeEngineId, numThreads);
  // Note: the shared_from_this prevents from starting this in the ctor because
  // it expects one shared ptr from this
  auto expected = QueryManagerStatus::Created;
  if (queryManagerStatus.compare_exchange_strong(expected,
                                                 QueryManagerStatus::Running)) {
#ifdef ENABLE_PAPI_PROFILER
    cpuProfilers.resize(numThreads);
#endif

    threadPool = std::make_shared<ThreadPool>(
        nodeEngineId, inherited0::shared_from_this(), numThreads,
        bufferManagers, numberOfBuffersPerWorker, hardwareManager,
        workerToCoreMapping);
...
bool MultiQueueQueryManager::startThreadPool(
    uint64_t numberOfBuffersPerWorker) {
  NES_DEBUG(
      "startThreadPool: setup thread pool for nodeId= {}  with numThreads= {}",
      nodeEngineId, numThreads);
  // Note: the shared_from_this prevents from starting this in the ctor because
  // it expects one shared ptr from this
  auto expected = QueryManagerStatus::Created;
  if (queryManagerStatus.compare_exchange_strong(expected,
    ...
    for (uint64_t queueId = 0; queueId < taskQueues.size(); queueId++) {
      for (uint64_t threadId = 0; threadId < numberOfThreadsPerQueue;
           threadId++) {
...
void AbstractQueryManager::destroy() {
  ...
  NES_ASSERT2_FMT(successful,
                  "Cannot stop running queryIdAndCatalogEntryMapping upon "
                  "query manager destruction");
  // 2. attempt transition from Stopped -> Destroyed
  expected = QueryManagerStatus::Stopped;
  if (queryManagerStatus.compare_exchange_strong(
          expected, QueryManagerStatus::Destroyed)) {
    {
...
SharedQueryId AbstractQueryManager::getSharedQueryId(
...
AbstractQueryManager::getQepStatus(DecomposedQueryPlanId id) {
  std::unique_lock lock(queryMutex);
...
AbstractQueryManager::getQueryExecutionPlan(DecomposedQueryPlanId id) const {
  std::unique_lock lock(queryMutex);
  auto it = runningQEPs.find(id);
  if (it != runningQEPs.end()) {
    return it->second;
...
QueryStatisticsPtr
AbstractQueryManager::getQueryStatistics(DecomposedQueryPlanId qepId) {
...
void AbstractQueryManager::reconfigure(ReconfigurationMessage &task,
...
void AbstractQueryManager::postReconfigurationCallback(
    ReconfigurationMessage &task) {
  ...
  case ReconfigurationType::Destroy: {
    ...
    NES_ASSERT(status == Execution::ExecutableQueryPlanStatus::Stopped ||
               ...
               "query plan " << qepId << " is not in valid state "
                             << int(status));
    std::unique_lock lock(queryMutex);
    if (auto it = runningQEPs.find(qepId);
        it != runningQEPs.end()) { // note that this will release all shared
                                   // pointers stored in a QEP object
      it->second->destroy();
