...
MultiQueueQueryManager::MultiQueueQueryManager(
  ...
  NES_DEBUG(
      "QueryManger: use static mode for numberOfQueues={} numThreads={} "
...
bool DynamicQueryManager::startThreadPool(uint64_t numberOfBuffersPerWorker) {
  NES_DEBUG(
      "startThreadPool: setup thread pool for nodeEngineId= {}  with "
      "numThreads= {}",
...
Execution::ExecutableQueryPlanStatus AbstractQueryManager::getQepStatus(
    DecomposedQueryPlanId id) {
  std::unique_lock lock(queryMutex);
...
Execution::ExecutableQueryPlanPtr AbstractQueryManager::getQueryExecutionPlan(
    DecomposedQueryPlanId id) const {
  std::unique_lock lock(queryMutex);
  auto it = runningQEPs.find(id);
  if (it != runningQEPs.end()) {
    return it->second;
...
QueryStatisticsPtr AbstractQueryManager::getQueryStatistics(
...
void AbstractQueryManager::postReconfigurationCallback(
  ...
  switch (task.getType()) {
    case ReconfigurationType::Destroy: {
      ...
      NES_ASSERT(
          status == Execution::ExecutableQueryPlanStatus::Stopped ||
