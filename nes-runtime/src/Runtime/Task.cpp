...
ExecutionResult Task::operator()(WorkerContextRef workerContext) {
  ...
  try {
    ...
    if (auto* executablePipeline =
            std::get_if<Execution::ExecutablePipelinePtr>(&pipeline)) {
...
std::string Task::toString() const {
  ...
  ss << " inputBuffer=" << reinterpret_cast<std::size_t>(buf.getBuffer())
     << " inputTuples=" << buf.getNumberOfTuples()
     << " bufferSize=" << buf.getBufferSize()
     << " watermark=" << buf.getWatermark()
     << " originID=" << buf.getOriginId();
  return ss.str();
}
