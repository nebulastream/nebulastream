  ...
  if (isDestroyed.compare_exchange_strong(expected, true)) {
    ...
    if (!success) {
      NES_THROW_RUNTIME_ERROR(
          "[BufferManager] Requested buffer manager shutdown but a buffer is "
          "still used allBuffers="
...
void BufferManager::initialize(uint32_t withAlignment) {
  ...
  NES_TRACE(
      "Allocated {} bytes with alignment {} buffer size {} num buffer {} "
      "controlBlockSize {} {}",
...
std::optional<TupleBuffer> BufferManager::getBufferTimeout(
...
std::optional<TupleBuffer> BufferManager::getUnpooledBuffer(size_t bufferSize) {
  ...
  NES_TRACE(
      "Ptr: {} alignedBufferSize: {} alignedBufferSizePlusControlBlock: {} "
      "controlBlockSize: {}",
...
LocalBufferPoolPtr BufferManager::createLocalBufferPool(
...
FixedSizeBufferPoolPtr BufferManager::createFixedSizeBufferPool(
    size_t numberOfReservedBuffers) {
  std::unique_lock lock(availableBuffersMutex);
  std::deque<detail::MemorySegment*> buffers;
  NES_ASSERT2_FMT(
      (size_t)availableBuffers.size() >= numberOfReservedBuffers,
      "BufferManager: Not enough buffers: " << availableBuffers.size() << "<"
...
TupleBuffer allocateVariableLengthField(
    std::shared_ptr<AbstractBufferProvider> provider, uint32_t size) {
