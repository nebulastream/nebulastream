...
BufferManager::BufferManager(
    uint32_t bufferSize, uint32_t numOfBuffers,
...
void BufferManager::destroy() {
  ...
  if (isDestroyed.compare_exchange_strong(expected, true)) {
    ...
    if (allBuffers.size() != numOfAvailableBuffers) {
      NES_ERROR("[BufferManager] total buffers {} :: available buffers {}",
                allBuffers.size(), numOfAvailableBuffers);
      success = false;
    }
    ...
    if (!success) {
      NES_THROW_RUNTIME_ERROR("[BufferManager] Requested buffer manager "
                              "shutdown but a buffer is still used allBuffers="
                              << allBuffers.size()
                              << " available=" << numOfAvailableBuffers);
    }
    ...
    for (auto &holder : unpooledBuffers) {
      if (!holder.segment ||
          holder.segment->controlBlock->getReferenceCount() != 0) {
        NES_ASSERT2_FMT(
            false,
...
void BufferManager::initialize(uint32_t withAlignment) {
  ...
  uint64_t requiredMemorySpace =
      (uint64_t)this->bufferSize * (uint64_t)this->numOfBuffers;
  double percentage = (100.0 * requiredMemorySpace) / memorySizeInBytes;
  NES_DEBUG(
      "NES memory allocation requires {} out of {} (so {}%) available bytes",
      requiredMemorySpace, memorySizeInBytes, percentage);

  //    NES_ASSERT2_FMT(bufferSize && !(bufferSize & (bufferSize - 1)), "size
  //    must be power of two " << bufferSize);
  NES_ASSERT2_FMT(
  ...
  if (withAlignment > 0) {
    ...
    NES_THROW_RUNTIME_ERROR(
        "NES tries to align memory but alignment is invalid");
  }
  ...
  auto controlBlockSize =
      alignBufferSize(sizeof(detail::BufferControlBlock), withAlignment);
  auto alignedBufferSize = alignBufferSize(bufferSize, withAlignment);
  allocatedAreaSize =
      alignBufferSize(controlBlockSize + alignedBufferSize, withAlignment);
  size_t offsetBetweenBuffers = allocatedAreaSize;
  allocatedAreaSize *= numOfBuffers;
  basePointer = static_cast<uint8_t *>(
      memoryResource->allocate(allocatedAreaSize, withAlignment));
  NES_TRACE("Allocated {} bytes with alignment {} buffer size {} num buffer {} "
            "controlBlockSize {} {}",
            allocatedAreaSize, withAlignment, alignedBufferSize, numOfBuffers,
            controlBlockSize, alignof(detail::BufferControlBlock));
  ...
  NES_DEBUG("BufferManager configuration bufferSize={} numOfBuffers={}",
            this->bufferSize, this->numOfBuffers);
}
...
TupleBuffer BufferManager::getBufferBlocking() {
  ...
  if (memSegment->controlBlock->prepare()) {
    return TupleBuffer(memSegment->controlBlock.get(), memSegment->ptr,
                       memSegment->size);
  }
  NES_THROW_RUNTIME_ERROR(
      "[BufferManager] got buffer with invalid reference counter");
}
...
BufferManager::getBufferTimeout(std::chrono::milliseconds timeout_ms) {
#ifndef NES_USE_LATCH_FREE_BUFFER_MANAGER
  std::unique_lock lock(availableBuffersMutex);
  auto pred = [this]() { return !availableBuffers.empty(); };
...
std::optional<TupleBuffer> BufferManager::getUnpooledBuffer(size_t bufferSize) {
  ...
  auto candidate =
      std::lower_bound(unpooledBuffers.begin(), unpooledBuffers.end(), probe);
  ...
  // we have to align the buffer size as ARM throws an SIGBUS if we have
  // unaligned accesses on atomics.
  auto alignedBufferSize = alignBufferSize(bufferSize, DEFAULT_ALIGNMENT);
  auto alignedBufferSizePlusControlBlock = alignBufferSize(
      bufferSize + sizeof(detail::BufferControlBlock), DEFAULT_ALIGNMENT);
  auto controlBlockSize =
      alignBufferSize(sizeof(detail::BufferControlBlock), DEFAULT_ALIGNMENT);
  auto *ptr = static_cast<uint8_t *>(memoryResource->allocate(
      alignedBufferSizePlusControlBlock, DEFAULT_ALIGNMENT));
  if (ptr == nullptr) {
    NES_THROW_RUNTIME_ERROR("BufferManager: unpooled memory allocation failed");
  }
  NES_TRACE("Ptr: {} alignedBufferSize: {} alignedBufferSizePlusControlBlock: "
            "{} controlBlockSize: {}",
            reinterpret_cast<uintptr_t>(ptr), alignedBufferSize,
            alignedBufferSizePlusControlBlock, controlBlockSize);
  auto memSegment = std::make_unique<detail::MemorySegment>(
      ptr + controlBlockSize, alignedBufferSize, this,
      [](detail::MemorySegment *segment, BufferRecycler *recycler) {
  ...
  if (leakedMemSegment->controlBlock->prepare()) {
    return TupleBuffer(leakedMemSegment->controlBlock.get(),
                       leakedMemSegment->ptr, bufferSize);
  }
  NES_THROW_RUNTIME_ERROR(
      "[BufferManager] got buffer with invalid reference counter");
}
...
void BufferManager::recyclePooledBuffer(detail::MemorySegment *segment) {
  ...
  if (!segment->isAvailable()) {
    NES_THROW_RUNTIME_ERROR(
        "Recycling buffer callback invoked on used memory segment");
  }
...
BufferManager::UnpooledBufferHolder::UnpooledBufferHolder(uint32_t bufferSize)
    : size(bufferSize), free(false) {
  segment.reset();
}

BufferManager::UnpooledBufferHolder::UnpooledBufferHolder(
    std::unique_ptr<detail::MemorySegment> &&mem, uint32_t size)
...
LocalBufferPoolPtr
BufferManager::createLocalBufferPool(size_t numberOfReservedBuffers) {
  std::unique_lock lock(availableBuffersMutex);
  std::deque<detail::MemorySegment *> buffers;
  NES_DEBUG("availableBuffers.size()={} requested buffers={}",
            availableBuffers.size(), numberOfReservedBuffers);
  NES_ASSERT2_FMT((size_t)availableBuffers.size() >= numberOfReservedBuffers,
                  "not enough buffers"); // TODO improve error
  ...
  auto ret = std::make_shared<LocalBufferPool>(
      shared_from_this(), std::move(buffers), numberOfReservedBuffers);
  {
    std::unique_lock lock(localBufferPoolsMutex);
...
BufferManager::createFixedSizeBufferPool(size_t numberOfReservedBuffers) {
  std::unique_lock lock(availableBuffersMutex);
  std::deque<detail::MemorySegment *> buffers;
  NES_ASSERT2_FMT((size_t)availableBuffers.size() >= numberOfReservedBuffers,
                  "BufferManager: Not enough buffers: "
...
TupleBuffer
allocateVariableLengthField(std::shared_ptr<AbstractBufferProvider> provider,
                            uint32_t size) {
