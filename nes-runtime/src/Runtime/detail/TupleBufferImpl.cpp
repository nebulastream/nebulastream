...
MemorySegment::MemorySegment(
  ...
  this->controlBlock = new (controlBlock)
      BufferControlBlock(this, recycler, std::move(recycleFunction));
  this->ptr = ptr;
  if (!this->ptr) {
...
MemorySegment::MemorySegment(
  ...
  controlBlock.reset(
      new BufferControlBlock(this, recycler, std::move(recycleFunction)),
...
MemorySegment::~MemorySegment() {
  if (ptr) {
    /// XXX: If we want to make `release` noexcept as we discussed, we need to
    /// make sure that the
    ///      MemorySegment is noexcept destructible. I therefore transformed
    ///      this error into an assertion (I also consider this to be consistent
    ///      with our handeling of the referenceCount in the release function in
    ///      general. Do you agree?).
    {
      auto const refCnt = controlBlock->getReferenceCount();
      NES_ASSERT(refCnt == 0, "[MemorySegment] invalid reference counter"
                                  << refCnt << " on mem segment dtor");
    }

    // Release the controlBlock, which is either allocated via 'new' or
    // placement new. In the latter case, we only have to call the destructor,
    // as the memory segment that contains the controlBlock is managed
    // separately.
    if (controlBlock.tag() ==
        magic_enum::enum_integer(MemorySegmentType::Wrapped)) {
...
BufferControlBlock::BufferControlBlock(
    ...
    : owner(owner),
      owningBufferRecycler(recycler),
      recycleCallback(std::move(recycleCallback)) {
  // nop
}
...
BufferControlBlock& BufferControlBlock::operator=(
    const BufferControlBlock& that) {
  referenceCounter.store(that.referenceCounter.load());
  numberOfTuples = that.numberOfTuples;
  recycleCallback = that.recycleCallback;
  owner = that.owner;
...
void BufferControlBlock::addRecycleCallback(
    std::function<void(MemorySegment*, BufferRecycler*)>&& func) noexcept {
  auto oldRecycleCallback = this->recycleCallback;
  recycleCallback = [oldRecycleCallback, func](MemorySegment* memorySegment,
...
#ifdef NES_DEBUG_TUPLE_BUFFER_LEAKS
/**
 * @brief This function collects the thread name and the callstack of the
 * calling thread
 * @param threadName
 * @param callstack
 */
...
bool BufferControlBlock::prepare() {
  int32_t expected = 0;
#ifdef NES_DEBUG_TUPLE_BUFFER_LEAKS
  // store the current thread that owns the buffer and track which function
  // obtained the buffer
...
#ifdef NES_DEBUG_TUPLE_BUFFER_LEAKS
  // store the current thread that owns the buffer (shared) and track which
  // function increased the coutner of the buffer
  std::unique_lock lock(owningThreadsMutex);
  ThreadOwnershipInfo info;
  fillThreadOwnershipInfo(info.threadName, info.callstack);
  owningThreads[std::this_thread::get_id()].emplace_back(info);
...
void BufferControlBlock::dumpOwningThreadInfo() {
  std::unique_lock lock(owningThreadsMutex);
  NES_FATAL_ERROR("Buffer {} has {} live references", getOwner(),
                  referenceCounter.load());
  for (auto& item : owningThreads) {
    for (auto& v : item.second) {
      NES_FATAL_ERROR("Thread {} has buffer {} requested on callstack: {}",
                      v.threadName, getOwner(), v.callstack);
    }
  }
}
#endif
...
bool BufferControlBlock::release() {
  if (uint32_t const prevRefCnt = referenceCounter.fetch_sub(1);
      prevRefCnt == 1) {
    numberOfTuples = 0;
    ...
    NES_ASSERT(prevRefCnt != 0,
               "BufferControlBlock: releasing an already released buffer");
  }
#ifdef NES_DEBUG_TUPLE_BUFFER_LEAKS
  {
    std::unique_lock lock(owningThreadsMutex);
...
BufferControlBlock::ThreadOwnershipInfo::ThreadOwnershipInfo(
    std::string&& threadName, std::string&& callstack)
...
bool BufferControlBlock::loadChildBuffer(uint16_t index,
