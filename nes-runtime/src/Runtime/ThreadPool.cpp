...
ThreadPool::ThreadPool(WorkerId nodeId, QueryManagerPtr queryManager,
                       uint32_t numThreads,
                       std::vector<BufferManagerPtr> bufferManagers,
                       uint64_t numberOfBuffersPerWorker,
                       HardwareManagerPtr hardwareManager,
                       std::vector<uint64_t> workerPinningPositionList)
    : nodeId(nodeId), numThreads(numThreads),
      queryManager(std::move(queryManager)), bufferManagers(bufferManagers),
...
void ThreadPool::runningRoutine(WorkerContext &&workerContext) {
  while (running) {
    try {
      ...
      NES_ERROR("Got fatal error on thread {}: {}", workerContext.getId(),
                taskException.what());
      queryManager->notifyTaskFailure(taskException.getExecutable(),
                                      std::string(taskException.what()));
    }
  }
  ...
  try {
    ...
    NES_ERROR("Got fatal error on thread {}: {}", workerContext.getId(),
              error.what());
    NES_THROW_RUNTIME_ERROR("Got fatal error on thread "
                            << workerContext.getId() << ": " << error.what());
  }
...
bool ThreadPool::start(const std::vector<uint64_t> threadToQueueMapping) {
  ...
  for (uint64_t i = 0; i < numThreads; ++i) {
    threads.emplace_back([this, i, barrier, &threadToQueueMapping]() {
      ...
      uint64_t queueIdx =
          threadToQueueMapping.size() ? threadToQueueMapping[i] : 0;
      ...
      if (workerPinningPositionList.size() != 0) {
        ...
        uint64_t maxPosition = *std::max_element(
            workerPinningPositionList.begin(), workerPinningPositionList.end());
        NES_ASSERT(maxPosition < std::thread::hardware_concurrency(),
                   "pinning position thread is out of cpu range maxPosition="
                       << maxPosition);
        // pin core
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        ...
        if (rc != 0) {
          ...
          NES_WARNING("worker {} pins to core={}", i,
                      workerPinningPositionList[i]);
        }
      }
#endif
      ...
      auto path = std::filesystem::path(
          "worker_" + std::to_string(NesThread::getId()) + ".csv");
      auto profiler = std::make_shared<Profiler::PapiCpuProfiler>(
          Profiler::PapiCpuProfiler::Presets::CachePresets,
          std::ofstream(path, std::ofstream::out), NesThread::getId(),
          NesThread::getId());
      queryManager->cpuProfilers[NesThread::getId() %
                                 queryManager->cpuProfilers.size()] = profiler;
#endif
      // TODO (2310) properly initialize the profiler with a file, thread, and
      // core id
      auto workerId = NesThread::getId();
      NES_DEBUG("worker {} with workerId {} pins to queue {}", i, workerId,
                queueIdx);
      runningRoutine(WorkerContext(workerId, localBufferManager,
                                   numberOfBuffersPerWorker, queueIdx));
    });
  }
  barrier->wait();
...
bool ThreadPool::stop() {
  std::unique_lock lock(reconfigLock);
  NES_DEBUG("ThreadPool: stop thread pool while {} with {} threads",
            (running.load() ? "running" : "not running"), numThreads);
  auto expected = true;
