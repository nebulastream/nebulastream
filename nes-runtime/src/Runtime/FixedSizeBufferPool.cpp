...
void FixedSizeBufferPool::destroy() {
  ...
  NES_DEBUG("buffers after={} size of local buffers={}",
            bufferManager->getAvailableBuffers(), exclusiveBuffers.size());
#else
...
FixedSizeBufferPool::getBufferTimeout(std::chrono::milliseconds timeout) {
#ifndef NES_USE_LATCH_FREE_BUFFER_MANAGER
  std::unique_lock lock(mutex);
  auto pred = [this]() {
  ...
  if (memSegment->controlBlock->prepare()) {
    return TupleBuffer(memSegment->controlBlock, memSegment->ptr,
                       memSegment->size);
  }
  NES_THROW_RUNTIME_ERROR(
      "[FixedSizeBufferPool] got buffer with invalid reference counter: "
...
TupleBuffer FixedSizeBufferPool::getBufferBlocking() {
  ...
  buffer << this;
  NES_TRACE("TupleBuffer FixedSizeBufferPool::getBufferBlocking: {}",
            buffer.str());
#ifndef NES_USE_LATCH_FREE_BUFFER_MANAGER
...
void FixedSizeBufferPool::recyclePooledBuffer(
    detail::MemorySegment *memSegment) {
  NES_VERIFY(memSegment, "null memory segment");
  if (isDestroyed) {
