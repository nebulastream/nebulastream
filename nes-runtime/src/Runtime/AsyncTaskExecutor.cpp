...
AsyncTaskExecutor::AsyncTaskExecutor(const HardwareManagerPtr& hardwareManager,
  ...
  for (uint32_t i = 0; i < numOfThreads; ++i) {
    ...
    runningThreads.emplace_back(
        std::make_shared<std::thread>([this, i, promise]() {
          try {
...
bool AsyncTaskExecutor::destroy() {
  ...
  if (running.compare_exchange_strong(expected, false)) {
    try {
      ...
      auto result =
          std::accumulate(completionPromises.begin(), completionPromises.end(),
                          true, [](bool acc, auto promise) {
