...
void LocalBufferPool::destroy() {
  ...
  NES_ASSERT2_FMT(numberOfReservedBuffers == exclusiveBufferCount,
                  "one or more buffers were not returned to the pool: "
                      << exclusiveBufferCount << " but expected "
                      << numberOfReservedBuffers);

  NES_DEBUG("buffers before={} size of local buffers={}",
...
TupleBuffer LocalBufferPool::getBufferBlocking() {
  ...
  // TODO potential problem here: what if we are blocked here but one exclusive
...
std::optional<TupleBuffer> LocalBufferPool::getBufferTimeout(
