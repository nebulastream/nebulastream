...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TestTupleBufferTest.log",
  ...
  void SetUp() override {
    ...
    testBuffer = std::make_unique<TestTupleBuffer>(
        TestTupleBuffer::createTestTupleBuffer(tupleBuffer, schema));
    testBufferVarSize = std::make_unique<TestTupleBuffer>(
        TestTupleBuffer::createTestTupleBuffer(tupleBufferVarSizedData,
                                               varSizedDataSchema));
  }
};
...
TEST_P(TestTupleBufferTest, readWritetestBufferTest) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    ASSERT_EQ((testBuffer->readRecordFromBuffer<FIXED_SIZED_DATA_TYPES>(i)),
              testTuple);
  }
...
TEST_P(TestTupleBufferTest, readWritetestBufferTestVarSizeData) {
  for (int i = 0; i < 10; ++i) {
    auto testTuple =
        std::make_tuple((uint16_t)i, "" + std::to_string(i) + std::to_string(i),
                        i * 2.0, std::to_string(i));
    testBufferVarSize->pushRecordToBuffer(testTuple, bufferManager.get());
    ASSERT_EQ(
        (testBufferVarSize->readRecordFromBuffer<VAR_SIZED_DATA_TYPES>(i)),
        testTuple);
  }
  ...
  for (auto i = 0_u64; i < 10; ++i) {
    ...
    (*testBufferVarSize)[i].writeVarSized(
        "test$t2", "" + std::to_string(i) + std::to_string(i),
        bufferManager.get());
    (*testBufferVarSize)[i].writeVarSized("test$t4", std::to_string(i),
                                          bufferManager.get());

    ASSERT_EQ((*testBufferVarSize)[i]["test$t1"].read<uint16_t>(), i);
    ASSERT_EQ((*testBufferVarSize)[i]["test$t3"].read<double_t>(), i * 42.0);
    ASSERT_EQ((*testBufferVarSize)[i].readVarSized("test$t2"),
              "" + std::to_string(i) + std::to_string(i));
    ASSERT_EQ((*testBufferVarSize)[i].readVarSized("test$t4"),
              std::to_string(i));
  }
}
...
TEST_P(TestTupleBufferTest, countOccurrencesTest) {
  ...
  for (auto item : vec) {
    for (auto i = 0_u64; i < item.occurrences; ++i) {
      ...
      ASSERT_EQ((testBuffer->readRecordFromBuffer<FIXED_SIZED_DATA_TYPES>(
                    testBuffer->getNumberOfTuples() - 1)),
                item.tuple);
    }
...
TEST_P(TestTupleBufferTest, countOccurrencesTestVarSizeData) {
  ...
  for (auto item : vec) {
    for (auto i = 0_u64; i < item.occurrences; ++i) {
      ...
      ASSERT_EQ((testBufferVarSize->readRecordFromBuffer<VAR_SIZED_DATA_TYPES>(
                    testBufferVarSize->getNumberOfTuples() - 1)),
                item.tuple);
    }

    auto dynamicTuple = (*testBufferVarSize)[posTuple];
    ASSERT_EQ(item.occurrences,
              testBufferVarSize->countOccurrences(dynamicTuple));
    posTuple += item.occurrences;
  }
}
...
INSTANTIATE_TEST_CASE_P(
    TestInputs, TestTupleBufferTest,
