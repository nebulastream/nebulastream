...
public:
        ...
        const std::vector<IR::BasicBlockPtr>& dpsSortedBlocks, const std::unordered_map<std::string, CorrectBlockValuesPtr>& correctBlocks)
    {
            ...
            if (currentBlock->getTerminatorOp()->getOperationType() == IR::Operations::Operation::IfOp)
            {
                ...
                if (correctBlocks.contains(currentBlock->getIdentifier()))
                {
                    ...
                    if (!correctMergeBlockId.empty())
                    {
                        ...
                        {
                            NES_ERROR(
                                "CurrentBlock: " << currentBlock->getIdentifier() << " did not contain a merge block"
                    ...
                    else
                    {
                        ...
                        {
                            NES_ERROR(
                                "The current merge block: " << currentBlock->getIdentifier()
                ...
                else
                {
                    mergeBlocksAreCorrect = false;
                    NES_ERROR(
                        "CurrentBlock with id: " << currentBlock->getIdentifier()
            ...
            else if (currentBlock->getTerminatorOp()->getOperationType() == IR::Operations::Operation::LoopOp)
            {
                ...
                if (correctBlocks.contains(currentBlock->getIdentifier()))
                {
                    if (correctBlocks.at(currentBlock->getIdentifier())->countedLoopInfo)
                    {
                        ...
                        {
                            NES_ERROR(
                                "Loop operation in block: " << currentBlock->getIdentifier() << " should -not- contain counted loop info.");
                        }
                        else
                        {
                            ...
                            if (!loopInfoIsCorrect)
                            {
                                NES_ERROR(
                                    ...
                                    << ", LoopEndBlock: " << loopOp->getLoopEndBlock().getNextBlock()->getIdentifier() << " vs "
                                    << correctBlocks.at(currentBlock->getIdentifier())->countedLoopInfo->loopEndBlockId);
                            }
                        }
                    }
        ...
        {
            NES_ERROR(
                "The number of checked IR blocks " << numCheckedBlocks
