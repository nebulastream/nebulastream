 ...
 * @brief Todo
 */
class ValueScopingPhaseTest : public testing::Test,
  ...
  bool checkIRForCorrectness(
    ...
    for (auto currentBlock : dpsSortedBlocks) {
      if (currentBlock->getTerminatorOp()->getOperationType() ==
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          ...
          if (!correctMergeBlockId.empty()) {
            if (!ifOp->getMergeBlock()) {
              NES_ERROR("CurrentBlock: " << currentBlock->getIdentifier()
                                         << " did not contain a merge block"
                                         << " even though the solution suggest "
                                            "it has a merge-block with id: "
                                         << correctMergeBlockId);
          ...
          NES_ERROR("CurrentBlock with id: "
                    << currentBlock->getIdentifier()
                    << " was not part of solution set(correctBlocks), but it "
                       "contains an if-operation.");
        }
        ++numCheckedBlocks;
      } else if (currentBlock->getTerminatorOp()->getOperationType() ==
                 IR::Operations::Operation::LoopOp) {
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          if (correctBlocks.at(currentBlock->getIdentifier())
            ...
            if (!loopInfoIsCorrect) {
              NES_ERROR("Loop operation in block: "
                        << currentBlock->getIdentifier()
              ...
              loopInfoIsCorrect &=
                  ...
                  correctBlocks.at(currentBlock->getIdentifier())
                      ->countedLoopInfo->stepSize;
              loopInfoIsCorrect &=
                  loopOp->getLoopEndBlock().getNextBlock()->getIdentifier() ==
                  correctBlocks.at(currentBlock->getIdentifier())
                      ->countedLoopInfo->loopEndBlockId;
              if (!loopInfoIsCorrect) {
                NES_ERROR(
                    "Loop info set incorrectly. Check values: "
          ...
          if (!backLinksAreCorrect) {
            NES_ERROR("\nBlock -"
                      << currentBlock->getIdentifier() << "- contained -"
                      << currentBlock->getNumLoopBackEdges()
                      << "- backLinks instead of: -"
                      << correctBlocks.at(currentBlock->getIdentifier())
                             ->correctNumberOfBackLinks
                      << "-.");
          }
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          ...
          NES_ERROR("CurrentBlock with id: "
                    << currentBlock->getIdentifier()
                    << " was part of solution set(correctBlocks), but it does "
                       "not contain an if-operation.");
        }
      }
    }
    ...
    if (!numCheckedBlocksMatchesNumCorrectBlocks) {
      NES_ERROR("The number of checked IR blocks "
                << numCheckedBlocks
                << " does not match the number of given 'correctBlocks': "
                << correctBlocks.size());
    }
...
INSTANTIATE_TEST_CASE_P(
    testLoopCompilation, ValueScopingPhaseTest,
