...
class ModExpressionTest : public testing::Test {
  public:
    /* Will be called before any test in this class are executed. */
    static void SetUpTestCase() {
        NES::Logger::setupLogging("ModExpressionTest.log", NES::LogLevel::LOG_DEBUG);
        std::cout << "Setup ModExpressionTest test class." << std::endl;
    }
...
TEST_F(ModExpressionTest, evaluateModExpressionInteger) {
    ...
    // Int32
    {
        auto resultValue = expression.eval(Value<Int32>((int32_t) 4), Value<Int32>((int32_t) 4));
