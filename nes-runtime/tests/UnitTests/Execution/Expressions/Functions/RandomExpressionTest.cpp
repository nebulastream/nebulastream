...
TEST_F(RandomExpressionTest, evaluatePiExpressionInteger) {
    auto expression = UnaryExpressionWrapper<RandomExpression>();
    // Int8
    {
        auto resultValue = expression.eval(Value<Int8>((int8_t) 1));
        ASSERT_EQ(resultValue, resultValue > 0 and resultValue < 1);
...
TEST_F(RandomExpressionTest, evaluatePiExpressionFloat) {
    auto expression = UnaryExpressionWrapper<RandomExpression>();
    // Float
    {
