...
*/
        TEST_F(KeyedThresholdWindowOperatorTest, thresholdWindowWithSumAggTest) {
            ...
            auto hashFunction = std::make_unique<Nautilus::Interface::MurMur3HashFunction>();
            ...
            auto keyedSumAggregationValue = std::map<uint32_t, std::unique_ptr<Aggregation::AggregationValue>>();
            aggValues.emplace_back(std::move(keyedSumAggregationValue));
            auto handler = std::make_shared<KeyedThresholdWindowOperatorHandler>(std::move(aggValues));
...
*/
        TEST_F(KeyedThresholdWindowOperatorTest, thresholdWindowWithMaxAggTest) {
            ...
            auto keyedMaxAggregationValue = std::map<uint32_t, std::unique_ptr<Aggregation::AggregationValue>>();
            aggValues.emplace_back(std::move(keyedMaxAggregationValue));
            auto handler = std::make_shared<KeyedThresholdWindowOperatorHandler>(std::move(aggValues));
            auto pipelineContext = MockedPipelineExecutionContext({handler});
...
*/
        TEST_F(KeyedThresholdWindowOperatorTest, thresholdWindowWithMaxSumTestDifferentKey) {
            auto readF1 = std::make_shared<Expressions::ReadFieldExpression>("f1");
            auto readF2 = std::make_shared<Expressions::ReadFieldExpression>("f2");
            auto readKey = std::make_shared<Expressions::ReadFieldExpression>("k1");
            auto fortyTwo = std::make_shared<Expressions::ConstantInt32ValueExpression>(42);
            ...
            auto recordFifty = Record({{"f1", Value<>(50)}, {"k1", Value<>((int64_t) 0)}, {"f2", Value<>((int64_t) 2)}});
            auto recordNinety = Record({{"f1", Value<>(90)}, {"k1", Value<>((int64_t) 1)}, {"f2", Value<>((int64_t) 3)}});
            auto recordTwenty =
                Record({{"f1", Value<>(20)}, {"k1", Value<>((int64_t) 0)}, {"f2", Value<>((int64_t) 4)}});// closes the window
            thresholdWindowOperator->execute(ctx, recordFifty);
            thresholdWindowOperator->execute(ctx, recordNinety);
            ...
            EXPECT_EQ(collector->records[1].read(aggregationResultFieldName), 3);
