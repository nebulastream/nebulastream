...
static std::string_view from_text_value_type(const NES::Nautilus::Value<Text>& value) {
    return reinterpret_cast<TextValue*>(value.getValue().getReference().value->getValue().value)->view();
}
...
static std::vector<float> decode_floats_from_base64(const std::string_view& base64_encoded) {
    ...
    std::copy(Base64DecodeIterator(base64_encoded.begin()),
              Base64DecodeIterator(base64_encoded.end()),
...
TEST_F(InferModelOperatorTest, testInferModelForONNXRuntimeYOLOV8) {
    ...
    auto handler = std::make_shared<InferModelHandler>(std::string(TEST_DATA_DIRECTORY) + "yolov8n-cls.onnx");
...
TEST_F(InferModelOperatorTest, testInferModelForONNXRuntimeBase64Encoding) {
    std::string data = "data";
    ...
    auto handler = std::make_shared<InferModelHandler>(std::string(TEST_DATA_DIRECTORY) + "iris_95acc.onnx");
    auto pipelineContext = MockedPipelineExecutionContext({handler});
...
TEST_F(InferModelOperatorTest, testInferModelForONNXRuntime) {
    std::string f1 = "f1";
    std::string f2 = "f2";
    std::string f3 = "f3";
    std::string f4 = "f4";
 ...
 * This Test is disabled because using bools as input is not supported
 */
TEST_F(InferModelOperatorTest, DISABLED_testInferModelForBoolInput) {
    std::string f1 = "f1";
    std::string f2 = "f2";
    std::string f3 = "f3";
    std::string f4 = "f4";
