...
protected:
    ...
    const QueryId testQueryId{NES::QueryId(NES::LocalQueryId(NES::UUIDToString(NES::generateUUID())))};
...
{
    const auto log = queryLog->getLogForQuery(QueryId::createLocal());
...
{
    const auto status = queryLog->getQueryStatus(QueryId::createLocal());
    EXPECT_FALSE(status.has_value());
}

TEST_F(QueryLogTest, MultipleQueriesIndependentLogs)
{
    const QueryId query1 = QueryId::createLocal();
    const QueryId query2 = QueryId::createLocal();
...
TEST_F(QueryLogTest, MultiThreadedLogging)
{
    ...
    std::vector<QueryId> queryIds(numQueries, QueryId());
    for (auto& id : queryIds)
    {
        id = QueryId::createLocal();
    }
    ...
    const auto logSubmission = [this, baseTime, &barrier, &queryIds]
    {
        ...
        for (const auto& [index, queryId] : queryIds | views::enumerate)
        {
            ...
            const auto qid = QueryId{queryId};
    ...
    {
        const auto qid = QueryId{queryIds[queryId]};
