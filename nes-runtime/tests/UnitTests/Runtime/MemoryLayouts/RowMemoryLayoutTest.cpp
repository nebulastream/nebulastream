...
TEST_F(RowMemoryLayoutTest, rowLayoutMapCalcOffsetTest)
{
    ...
    ASSERT_EQ(testBuffer->getCapacity(), tupleBuffer.getBufferSize() / schema.sizeOfSchemaInBytes);
    ASSERT_EQ(testBuffer->getNumberOfTuples(), 0U);
    ASSERT_EQ(rowLayout->getFieldOffset(1, 2), schema.sizeOfSchemaInBytes * 1 + (1 + 2));
    ASSERT_EQ(rowLayout->getFieldOffset(4, 0), schema.sizeOfSchemaInBytes * 4 + 0);
}
...
TEST_F(RowMemoryLayoutTest, rowLayoutPushRecordAndReadRecordTestMultipleRecord)
{
    ...
    const size_t numTuples = 230; ///tupleBuffer.getBufferSize() / schema.sizeOfSchemaInBytes;
