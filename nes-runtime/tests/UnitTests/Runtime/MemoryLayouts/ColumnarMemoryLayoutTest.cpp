...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutMapCalcOffsetTest)
{
    ...
    const auto testBuffer = std::make_unique<Memory::MemoryLayouts::TestTupleBuffer>(columnLayout->clone(), tupleBuffer);
...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutLayoutFieldBoundaryCheck)
{
    ...
    auto testBuffer = std::make_unique<Memory::MemoryLayouts::TestTupleBuffer>(columnLayout->clone(), tupleBuffer);
...
TEST_F(ColumnarMemoryLayoutTest, deepCopy)
{
    auto schema
        = Schema().addField("t1", BasicType::UINT8).addField("t2", BasicType::UINT8).addField("t3", BasicType::UINT8);
    auto rowLayout = ColumnLayout::create(schema, bufferManager->getBufferSize());

    auto originalSchema = schema;
    const auto deepCopy = rowLayout->clone();

    const auto schema2 = schema.addField("r1", BasicType::UINT8);
    rowLayout = ColumnLayout::create(schema2, bufferManager->getBufferSize());

    EXPECT_TRUE(deepCopy->getSchema() == originalSchema);
    EXPECT_FALSE(deepCopy->getSchema() == rowLayout->getSchema());
}
