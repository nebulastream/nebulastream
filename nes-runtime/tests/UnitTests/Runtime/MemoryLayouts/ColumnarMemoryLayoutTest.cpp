...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutMapCalcOffsetTest)
{
    ...
    const auto capacity = tupleBuffer.getBufferSize() / schema.sizeOfSchemaInBytes;
...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutPushRecordAndReadRecordTestMultipleRecord)
{
    ...
    const size_t numTuples = (tupleBuffer.getBufferSize() / schema.sizeOfSchemaInBytes);
...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutLayoutFieldBoundaryCheck)
{
    ...
    size_t NUM_TUPLES = (tupleBuffer.getBufferSize() / schema.sizeOfSchemaInBytes);
