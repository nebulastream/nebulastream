...
class ColumnarMemoryLayoutTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ColumnarMemoryLayoutTest.log",
...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutCreateTest) {
  ...
  ASSERT_NO_THROW(columnLayout = ColumnLayout::create(
                      schema, bufferManager->getBufferSize()));
  ASSERT_NE(columnLayout, nullptr);
}

/**
 * @brief Tests that the field offsets are are calculated correctly using a
 * TestTupleBuffer.
 */
TEST_F(ColumnarMemoryLayoutTest, columnLayoutMapCalcOffsetTest) {
  ...
  auto testBuffer = std::make_unique<Runtime::MemoryLayouts::TestTupleBuffer>(
      columnLayout, tupleBuffer);
  ...
  ASSERT_EQ(columnLayout->getFieldOffset(1, 2),
            capacity * 1 + capacity * 2 + 1 * 4);
  ASSERT_EQ(columnLayout->getFieldOffset(5, 1), capacity * 1 + 5 * 2);
  ASSERT_EQ(columnLayout->getFieldOffset(4, 0), capacity * 0 + 4);
}

/**
 * @brief Tests that we can write a single record to and read from a
 * TestTupleBuffer correctly.
 */
TEST_F(ColumnarMemoryLayoutTest,
  ...
  std::tuple<uint8_t, uint16_t, uint32_t> readRecord =
      testBuffer->readRecordFromBuffer<uint8_t, uint16_t, uint32_t>(0);
...
/**
 * @brief Tests that we can write many records to and read from a
 * TestTupleBuffer correctly.
 */
TEST_F(ColumnarMemoryLayoutTest,
  ...
  size_t NUM_TUPLES =
      (tupleBuffer.getBufferSize() / schema->getSchemaSizeInBytes());
  ...
  for (size_t i = 0; i < NUM_TUPLES; i++) {
    std::tuple<uint8_t, uint16_t, uint32_t> readRecord =
        testBuffer->readRecordFromBuffer<uint8_t, uint16_t, uint32_t>(i);
...
/**
 * @brief Tests that we can access fields of a TupleBuffer that is used in a
 * TestTupleBuffer correctly.
 */
TEST_F(ColumnarMemoryLayoutTest, columnLayoutLayoutFieldSimple) {
  ...
  std::shared_ptr<ColumnLayout> columnLayout =
      ColumnLayout::create(schema, bufferManager->getBufferSize());
  ASSERT_NE(columnLayout, nullptr);
  ...
  auto field0 =
      ColumnLayoutField<uint8_t, true>::create(0, columnLayout, tupleBuffer);
  auto field1 =
      ColumnLayoutField<uint16_t, true>::create(1, columnLayout, tupleBuffer);
  auto field2 =
      ColumnLayoutField<uint32_t, true>::create(2, columnLayout, tupleBuffer);
...
/**
 * @brief Tests whether whether an error is thrown if we try to access
 * non-existing fields of a TupleBuffer.
 */
TEST_F(ColumnarMemoryLayoutTest, columnLayoutLayoutFieldBoundaryCheck) {
  ...
  auto field2 =
      ColumnLayoutField<uint32_t, true>::create(2, columnLayout, tupleBuffer);
  ASSERT_THROW(
      (ColumnLayoutField<uint8_t, true>::create(3, columnLayout, tupleBuffer)),
      NES::Exceptions::RuntimeException);
  ASSERT_THROW(
      (ColumnLayoutField<uint16_t, true>::create(4, columnLayout, tupleBuffer)),
      NES::Exceptions::RuntimeException);
  ASSERT_THROW(
      (ColumnLayoutField<uint32_t, true>::create(5, columnLayout, tupleBuffer)),
      NES::Exceptions::RuntimeException);
...
TEST_F(ColumnarMemoryLayoutTest, getFieldViaFieldNameColumnLayout) {
  ...
  ASSERT_NO_THROW((ColumnLayoutField<uint8_t, true>::create("t1", columnLayout,
                                                            tupleBuffer)));
  ASSERT_NO_THROW((ColumnLayoutField<uint16_t, true>::create("t2", columnLayout,
                                                             tupleBuffer)));
  ASSERT_NO_THROW((ColumnLayoutField<uint32_t, true>::create("t3", columnLayout,
                                                             tupleBuffer)));

  ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create("t4", columnLayout,
                                                          tupleBuffer)),
               NES::Exceptions::RuntimeException);
  ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create("t5", columnLayout,
                                                          tupleBuffer)),
               NES::Exceptions::RuntimeException);
  ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create("t6", columnLayout,
                                                          tupleBuffer)),
               NES::Exceptions::RuntimeException);
}
...
/**
 * @brief Tests if an error is thrown if more tuples are added to a TupleBuffer
 * than the TupleBuffer can store.
 */
TEST_F(ColumnarMemoryLayoutTest, pushRecordTooManyRecordsColumnLayout) {
  ...
  for (; i < NUM_TUPLES + 1; i++) {
    std::tuple<uint8_t, uint16_t, uint32_t> writeRecord(rand(), rand(), rand());
    allTuples.emplace_back(writeRecord);
    ASSERT_THROW(testBuffer->pushRecordToBuffer(writeRecord),
                 NES::Exceptions::RuntimeException);
  }
