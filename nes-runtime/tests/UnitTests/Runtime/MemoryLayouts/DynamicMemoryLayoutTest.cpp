...
class DynamicMemoryLayoutTestParameterized
 ...
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("DynamicMemoryLayoutTest.log",
  ...
  void SetUp() override {
    ...
    if (GetParam() == Schema::MemoryLayoutType::ROW_LAYOUT) {
      RowLayoutPtr layout;
      ASSERT_NO_THROW(
          layout = RowLayout::create(schema, bufferManager->getBufferSize()));
      ...
      ASSERT_NO_THROW(layout = ColumnLayout::create(
...
TEST_P(DynamicMemoryLayoutTestParameterized, readWriteColumnartestBufferTest) {
  for (int i = 0; i < 10; i++) {
    ...
    ASSERT_EQ((testBuffer->readRecordFromBuffer<uint16_t, bool, double>(i)),
              testTuple);
  }
}
...
TEST_P(DynamicMemoryLayoutTestParameterized, toStringTestRowLayout) {
  for (uint32_t i = 0; i < 10; i++) {
    testBuffer->pushRecordToBuffer(
        std::tuple<uint16_t, bool, double>{i, true, i * 2.0});
  }
...
INSTANTIATE_TEST_CASE_P(
    TestInputs, DynamicMemoryLayoutTestParameterized,
    ::testing::Values(Schema::MemoryLayoutType::COLUMNAR_LAYOUT,
                      Schema::MemoryLayoutType::ROW_LAYOUT),
    [](const testing::TestParamInfo<
        DynamicMemoryLayoutTestParameterized::ParamType>& info) {
