...
TEST_F(MemorySourceIntegrationTest, testMemorySourceHdfs) {
    NES_INFO("MemorySourceIntegrationTest: Start coordinator");
    NesCoordinatorPtr crd = std::make_shared<NesCoordinator>(ipAddress, restPort, rpcPort);
    uint64_t port = crd->startCoordinator(/**blocking**/ false);
    EXPECT_NE(port, 0);
    ...
    //register query
    std::string queryString =
        R"(Query::from("memory_stream").sink(HdfsSinkDescriptor::create(")" + (std::string) filePath + R"(" , "CSV_FORMAT", "APPEND"));)";
    QueryId queryId = queryService->validateAndQueueAddRequest(queryString, "BottomUp");
    EXPECT_NE(queryId, INVALID_QUERY_ID);
    auto globalQueryPlan = crd->getGlobalQueryPlan();
    ASSERT_TRUE(TestUtils::waitForQueryToStart(queryId, queryCatalog));
...
//    deszBuffer.setNumberOfTuples(4);
    ...
    hdfsFile inputFile = hdfsStreamBuilderBuild(hdfsStreamBuilderAlloc(fs, filePath, O_RDONLY));
