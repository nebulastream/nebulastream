...
TEST_F(MemorySourceIntegrationTest, testMemorySource) {
    ...
    AbstractPhysicalStreamConfigPtr conf =
        MemorySourceStreamConfig::create("MemorySource", "memory_stream_0", "memory_stream", memArea, memAreaSize);
...
//TEST_F(MemorySourceIntegrationTest, testMemorySourceHdfsCSV) {
//    NES_INFO("MemorySourceIntegrationTest: Start coordinator");
//    NesCoordinatorPtr crd = std::make_shared<NesCoordinator>(ipAddress, restPort, rpcPort);
//    uint64_t port = crd->startCoordinator(/**blocking**/ false);
//    EXPECT_NE(port, 0);
...
//
//    AbstractPhysicalStreamConfigPtr conf =
//        MemorySourceStreamConfig::create("MemorySource", "memory_stream_0",
//                                         "memory_stream", memArea, memAreaSize);
//    wrk1->registerPhysicalStream(conf);
//
//    // local fs
//    char *filePath = "/testData/memorySourceTestOut.csv";
...
//    if (hdfsExists(fs, filePath)) {
//        hdfsDelete(fs, filePath, 0);
//    }
//
//    std::string filePathStr = filePath;
//    //register query
//    std::string queryString =
//        R"(Query::from("memory_stream").sink(HdfsFileSinkDescriptor::create(")" + filePathStr + R"(" , "CSV_FORMAT", "APPEND"));)";
//    QueryId queryId = queryService->validateAndQueueAddRequest(queryString, "BottomUp");
//    EXPECT_NE(queryId, INVALID_QUERY_ID);
//    auto globalQueryPlan = crd->getGlobalQueryPlan();
//    ASSERT_TRUE(TestUtils::waitForQueryToStart(queryId, queryCatalog));
...
//
//    hdfsFS fs_ = hdfsBuilderConnect(builder);
//    hdfsFile readFile = hdfsStreamBuilderBuild(hdfsStreamBuilderAlloc(fs_, filePath, O_RDONLY));
