...
TEST_F(WindowDeploymentTest, DISABLED_testDeployOneWorkerCentralTumblingWindowQueryProcessingTime) {
    ...
    QueryId queryId = queryService->validateAndQueueAddRequest(query, "Botx tomUp");
    //todo will be removed once the new window source is in place
    sleep(20);
    ...
    while (std::getline(ifs, line)) {
        ...
        if (rowNumber == 4) {
            ...
            NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): content=" << content.at(3));
            NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): expContent=2524687220000");
            EXPECT_EQ(content.at(3), "2524687220000");
        }
        if (rowNumber == 5) {
            std::vector<string> content = UtilityFunctions::split(line, '|');
            NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): content=" << content.at(3));
            NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): expContent=1262343620010");
            EXPECT_EQ(content.at(3), "1262343620010");
        }
        if (rowNumber > 17 && rowNumber < 28) {
            std::vector<string> content = UtilityFunctions::split(line, '|');
            NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): content=" << content.at(3));
            NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): expContent=0");
            EXPECT_EQ(content.at(3), "0");
        }
    }
    NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): content=" << rowNumber);
    NES_INFO("WindowDeploymentTest(testDeployOneWorkerCentralWindowQueryProcessingTime): expContent=57");
    //FIXME : #1096
 ...
 */
TEST_F(WindowDeploymentTest, DISABLED_testDeployOneWorkerDistributedTumblingWindowQueryEventTime) {
