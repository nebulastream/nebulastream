...
TEST_F(SimplePatternTest, testPatternWithWindowandAggregation) {
    ...
    //register physical stream
    PhysicalStreamConfigPtr conf = PhysicalStreamConfig::create("CSVSource", "../tests/test_data/QnV_short_intID.csv",
                                                                1, 0, 1,
                                                                "test_stream", "QnV");
    wrk1->registerPhysicalStream(conf);

    std::string outputFilePath =
        "testPatternWithWindowandAggregation.out";
    remove(outputFilePath.c_str());
 ...
 * Here, we test the translation of a simple pattern (1 Stream) into a query using a real data set (QnV) and check the output
 */
TEST_F(SimplePatternTest, testPatternWithTestStreamSingleOutput) {
    NES_DEBUG("start coordinator");
    NesCoordinatorPtr crd = std::make_shared<NesCoordinator>(ipAddress, restPort, rpcPort);
    size_t port = crd->startCoordinator(/**blocking**/ false);
    EXPECT_NE(port, 0);
...
TEST_F(SimplePatternTest, testPatternWithTestStreamAndMultiWorkerMerge) {
    ...
    std::string qnv1 =
        R"(Schema::create()->addField("sensor_id", DataTypeFactory::createFixedChar(8))->addField(createField("timestamp", UINT64))->addField(createField("velocity", FLOAT32))->addField(createField("quantity", UINT64));)";
    std::string testSchemaFileName1 = "QnV1.hpp";
    std::ofstream out1(testSchemaFileName1);
    out1 << qnv1;
    out1.close();
    wrk1->registerLogicalStream("QnV1", testSchemaFileName1);
    ...
    //register query
    std::string query = R"(Pattern::from("QnV").merge(Pattern::from("QnV1")).filter(Attribute("velocity") > 100).sink(FileSinkDescriptor::create(")"
                        + outputFilePath + "\")); ";
