...
TEST_F(SimplePatternTest, DISABLED_testMultiAndPattern) {
  ...
  // Pattern - 2 ANDs
     std::string query2 =
        ...
        .window(TumblingWindow::of(EventTime(Attribute("timestamp")),Minutes(5)))
        .andWith(Query::from("QnV2").filter(Attribute("velocity") > 75))
        .window(TumblingWindow::of(EventTime(Attribute("timestamp")),Minutes(5)))
        .sink(FileSinkDescriptor::create(")"
        + outputFilePath + "\"));";
   ...
   std::string queryjoin =
        ...
        .window(TumblingWindow::of(EventTime(Attribute("timestamp")),Minutes(5)))
        .joinWith(Query::from("QnV2").filter(Attribute("velocity") > 75)
...
TEST_F(SimplePatternTest, DISABLED_testMultiAndPattern2) {
    coConf->resetCoordinatorOptions();
    wrkConf->resetWorkerOptions();
    srcConf->resetSourceOptions();
    srcConf1->resetSourceOptions();
    ...
    std::string testSchemaFileName2 = "window2.hpp";
    ...
    srcConf2->as<CSVSourceConfig>()->setNumberOfTuplesToProducePerBuffer(2);
    srcConf2->as<CSVSourceConfig>()->setNumberOfBuffersToProduce(2);
    srcConf2->as<CSVSourceConfig>()->setPhysicalStreamName("test_stream3");
  ...
  std::string query =
    ...
    std::string query =
        ...
        .andWith(Query::from("window2").filter(Attribute("win2") < 20))
        .window(TumblingWindow::of(EventTime(Attribute("timestamp")),Milliseconds(2000)))
        .sink(FileSinkDescriptor::create(")"
        + outputFilePath + "\"));";
