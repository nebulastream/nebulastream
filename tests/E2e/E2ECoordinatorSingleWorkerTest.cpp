...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithFileOutputExdraUseCase) {
    ...
    string expectedContent =
        "exdra$id:INTEGER,exdra$metadata_generated:INTEGER,exdra$metadata_title:Char,exdra$metadata_id:Char,exdra$features_type:Char,exdra$features_properties_capacity:INTEGER,exdra$features_properties_efficiency:(Float),exdra$features_properties_mag:(Float),exdra$features_properties_time:INTEGER,exdra$features_properties_updated:INTEGER,exdra$features_properties_type:Char,exdra$features_geometry_type:Char,exdra$features_geometry_coordinates_longitude:(Float),exdra$features_geometry_coordinates_latitude:(Float),exdra$features_eventId :Char\n"
        ...
        "12,1262343720110,Wind Turbine Data Generated for Nebula Stream,f3c1611f-db1c-49bf-9376-3ccae7248644,Features,3593,0.586449,597841.062500,1411456,282737285664290,,Point,13.546017,47.870770,2ab9f413-848c-4ab4-a386-8615426e5c47\n"
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingSimplePattern) {
    ...
    string path2 = "./nesWorker --coordinatorPort=" + coordinatorRPCPort + " --rpcPort=" + worker1RPCPort
        ...
        + " --logicalStreamName=QnV --physicalStreamName=test_stream --sourceType=CSVSource "
          "--sourceConfig=../tests/test_data/QnV_short.csv --numberOfBuffersToProduce=1 --sourceFrequency=1 --numberOfTuplesToProducePerBuffer=146";
    bp::child workerProc(path2.c_str());
    NES_INFO("started worker with pid = " << workerProc.id());
    uint64_t workerPid = workerProc.id();
    EXPECT_TRUE(TestUtils::waitForWorkers(restPort, timeout, 1));
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithTumblingWindowFileOutput) {
    ...
    string path2 = "./nesWorker --coordinatorPort=" + coordinatorRPCPort + " --rpcPort=" + worker1RPCPort
        ...
        + " --logicalStreamName=window --physicalStreamName=test_stream --sourceType=CSVSource "
          "--sourceConfig=../tests/test_data/window.csv --numberOfBuffersToProduce=1 --sourceFrequency=1 --numberOfTuplesToProducePerBuffer=28";
    bp::child workerProc(path2.c_str());
    NES_INFO("started worker with pid = " << workerProc.id());
    uint64_t workerPid = workerProc.id();
    EXPECT_TRUE(TestUtils::waitForWorkers(restPort, timeout, 1));
