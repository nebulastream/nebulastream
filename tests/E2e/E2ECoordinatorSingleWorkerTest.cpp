...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithFileOutputExdraUseCase) {
    ...
    string path2 = "../nesWorker --coordinatorPort=12346 --sourceType=CSVSource --sourceConfig=tests/test_data/exdra.csv "
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingSimplePattern) {
    ...
    string path2 =
        "../nesWorker --coordinatorPort=12267 --logicalStreamName=QnV --physicalStreamName=test_stream --sourceType=CSVSource "
...
TEST_F(E2ECoordinatorSingleWorkerTest, testRating) {
    NES_INFO(" start coordinator");
    std::string testFile = "calculateNewRating100k.csv";
    remove(testFile.c_str());
    ...
    std::stringstream schema;
    //std::string rating = R"(Schema::create()->addField("userId", BasicType::UINT64)->addField("movieId", BasicType::UINT64)->addField("rating", BasicType::UINT64)->addField("timestamp", UINT64)->addField("time1", UINT64))";

    schema << "{\"streamName\" : \"Rating\",\"schema\" : \"Schema::create()->addField(\\\"userId\\\", UINT64)->addField(createField(\\\"movieId\\\", UINT64))->addField(createField(\\\"rating\\\", UINT64))->addField(createField(\\\"timestamp\\\", UINT64))->addField(createField(\\\"time1\\\", UINT64));\"}";
    schema << endl;
    NES_INFO("schema submit=" << schema.str());
    ...
    string path2 =
        "../nesWorker --coordinatorPort=12346 --sourceType=CSVSource --sourceConfig=../tests/test_data/ratings100k.csv --numberOfBuffersToProduce=10 --sourceFrequency=1 --physicalStreamName=test_stream --logicalStreamName=Rating --endlessRepeat=off";
...
TEST_F(E2ECoordinatorSingleWorkerTest, testRating) {
    ...
    string path2 =
        "../nesWorker --coordinatorPort=12346 --sourceType=CSVSource --sourceConfig=../tests/test_data/ratings100k.csv --numberOfBuffersToProduce=0 --sourceFrequency=1 --physicalStreamName=test_stream --logicalStreamName=Rating --endlessRepeat=off";
