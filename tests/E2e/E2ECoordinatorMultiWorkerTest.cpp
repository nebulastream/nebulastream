...
// by assigning a different RPC port for each test case
    ...
    TEST_F(E2ECoordinatorMultiWorkerTest, DISABLED_testExecutingValidUserQueryWithFileOutputTwoWorker) {
        ...
        auto coordinator = TestUtils::startCoordinator(
                {TestUtils::coordinatorPort(rpcPort), TestUtils::restPort(restPort)});
        ...
        auto worker2 = TestUtils::startWorker(
                {TestUtils::rpcPort(rpcPort + 6), TestUtils::dataPort(dataPort + 2),
    ...
    TEST_F(E2ECoordinatorMultiWorkerTest, DISABLED_testExecutingMonitoringTwoWorker) {
        ...
        web::http::client::http_client client(
                "http://localhost:" + std::to_string(restPort) + "/v1/nes/monitoring/metrics/");
    ...
    TEST_F(E2ECoordinatorMultiWorkerTest, testMQTTourceWithMigrationDiamond) {
        ...
        auto coordinator = TestUtils::startCoordinator(
                {TestUtils::coordinatorPort(rpcPort), TestUtils::restPort(restPort),TestUtils::logLevel("LOG_ERROR")});
        EXPECT_TRUE(TestUtils::waitForWorkers(restPort, timeout, 0));
...
//        NES_INFO("started worker 1 with pid = " << workerProc1.id());
        ...
        sleep(1);
//        auto worker4 = TestUtils::startWorker({TestUtils::rpcPort(rpcPort + 12),
//                                               TestUtils::dataPort(dataPort +6),
//                                               TestUtils::coordinatorPort(rpcPort),
//                                               TestUtils::logLevel("LOG_ERROR"),
...
//                                              });
        ...
        sleep(1);
        auto worker3 = TestUtils::startWorker({TestUtils::rpcPort(rpcPort + 9),
                                               TestUtils::dataPort(dataPort +4),
                                               TestUtils::coordinatorPort(rpcPort),
                                               TestUtils::numberOfSlots(1),
...
//        ss << endl;
        ...
        NES_ERROR("Submitting maint request over REST");
        sleep(8);
