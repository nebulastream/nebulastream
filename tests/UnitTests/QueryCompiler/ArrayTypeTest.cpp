...
TEST_F(ArrayTypeTest, testCheckNullTerminator) {
    ...
    EXPECT_NO_THROW((NES::Array{std::array{'a', 'b', '\0'}}));
    EXPECT_THROW((NES::Array{std::array{'a', 'b'}}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array{std::array{1, 2, 3}}));
    EXPECT_THROW((NES::Array{std::array{'1', '2', '3'}}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array{std::array{'1', '2', '\0'}}));

    // From content
    EXPECT_THROW(((volatile void) NES::Array{'a', 'b', 'c'}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array{'a', 'b', '\0'}));
    EXPECT_NO_THROW((NES::Array<char, 5>{'a', 'b', '\0'}));
    EXPECT_NO_THROW((NES::Array{1, 2, 3}));

    // char: From c-style array
    char const cStyleArray[3] = {'a', 'b', 'c'};
    EXPECT_THROW(((volatile void) NES::Array{cStyleArray}), std::runtime_error);
    char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
    EXPECT_NO_THROW(((volatile void) NES::Array{cStyleArrayNull}));
    EXPECT_NO_THROW(((volatile void) NES::Array<char, 3>{cStyleArrayNull}));
    EXPECT_THROW(((volatile void) NES::Array<char, 2>{cStyleArrayNull}), std::runtime_error);
    int const cStyleArrayInt[3] = {1, 2, 3};
    EXPECT_NO_THROW(((volatile void) NES::Array{cStyleArrayInt}));

    EXPECT_THROW(((volatile void) NES::Array<char, 3>{{'a', 'b', 'c'}}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array<char, 3>{{'a', 'b', '\0'}}));
    EXPECT_NO_THROW((NES::Array<char, 3>{{'1', '\0'}}));
    EXPECT_NO_THROW((NES::Array<char, 3>{{'1'}}));//< automatically append \0

    // From vector
    EXPECT_THROW((NES::Array<char, 3>{std::vector{'a', 'b', 'c'}}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array<char, 3>{std::vector{'a', 'b', '\0'}}));
    EXPECT_THROW((NES::Array<char, 2>{std::vector{'a', 'b', '\0'}}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array<char, 2>{std::vector{'\0'}}));
    EXPECT_NO_THROW((NES::Array<int, 3>{std::vector{1, 2, 3}}));
    EXPECT_THROW((NES::Array<int, 2>{std::vector{1, 2, 3, 4, 5}}), std::runtime_error);
    EXPECT_THROW((NES::Array<int, 2>{std::vector{1}}), std::runtime_error);

    // From cstr / std::string
    EXPECT_NO_THROW((NES::Array<char, 4>{"abc"}));
    EXPECT_NO_THROW((NES::Array<char, 10>{"abc"}));
    EXPECT_THROW((NES::Array<char, 1>{"abc"}), std::runtime_error);
    EXPECT_NO_THROW((NES::Array<char, 4>{std::string("abc")}));
    EXPECT_NO_THROW((NES::Array<char, 10>{std::string("abc")}));
    EXPECT_THROW((NES::Array<char, 1>{std::string("abc")}), std::runtime_error);
}
...
TEST_F(ArrayTypeTest, testExceptionSpecifier) {
    ...
    // From supertype's constructor
    EXPECT_TRUE(!noexcept(NES::Array{std::array{'a', 'b', '\0'}}));
    EXPECT_TRUE(noexcept(NES::Array{std::array{1, 2, 3}}));

    // From content
    EXPECT_TRUE(!noexcept(NES::Array{'a', 'b', '\0'}));
    EXPECT_TRUE(noexcept(NES::Array<char, 5>{'a', 'b', 'c'}));
    EXPECT_TRUE(noexcept(NES::Array{1, 2, 3}));

    // char: From c-style array
    char const cStyleArray[3] = {'a', 'b', 'c'};
    EXPECT_TRUE(!noexcept(NES::Array{cStyleArray}));
    int const cStyleArrayInt[3] = {1, 2, 3};
    EXPECT_TRUE(noexcept(NES::Array{cStyleArrayInt}));

    EXPECT_TRUE(!noexcept(NES::Array<char, 3>{{'a', 'b', '\0'}}));
    EXPECT_TRUE(!noexcept(NES::Array<char, 3>{{'1', '\0'}}));
    EXPECT_TRUE(!noexcept(NES::Array<char, 3>{{'1'}}));
    EXPECT_TRUE(noexcept(NES::Array<int, 3>{{1}}));
    EXPECT_TRUE(noexcept(NES::Array<int, 3>{{1, 2, 3}}));

    // From vector
    EXPECT_TRUE(!noexcept(NES::Array<char, 4>{std::vector{'a', 'b', 'c'}}));
    EXPECT_TRUE(!noexcept(NES::Array<char, 4>{std::vector{'a', 'b', 'c', '\0'}}));
    EXPECT_TRUE(!noexcept(NES::Array<int, 3>{std::vector{1, 2, 3}}));

    // From cstr / std::string
    EXPECT_TRUE(!noexcept(NES::Array<char, 4>{"abc"}));
    EXPECT_TRUE(!noexcept(NES::Array<char, 4>{std::string("abc")}));
}

/// @brief Utility for checking equality
template<std::size_t s, typename J>
auto eq(NES::Array<char, s> const& l, J r) -> bool {
    return std::string(&(l[0])) == r;
}
template<typename T, std::size_t s>
auto eq(NES::Array<T, s> const& l, std::vector<T>&& r) -> bool {
    if (s != r.size())
        return false;
...
/// @brief Test type, size and content of NES::Arrays.
TEST_F(ArrayTypeTest, testInitialization) {

    // From supertype's constructor
    {
        NES::Array ca{std::array{'a', 'b', '\0'}};
    ...
    {
        char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
        NES::Array{cStyleArrayNull};
        NES::Array<char, 3> cca{cStyleArrayNull};
    ...
    {
        char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
        NES::Array<char, 3> ccai{cStyleArrayNull};
    ...
    }
    {
        ...
        NES::Array<int, 3> iv{std::vector{1, 2, 3}};
    ...
    {
        NES::Array<int, 3> ivi = {std::vector{1, 2, 3}};
    ...
    }
    {
        ...
        NES::Array<char, 4> strF{std::string("abc")};
    ...
    }
    {
        ...
        NES::Array<char, 10> strP{std::string("abc")};
