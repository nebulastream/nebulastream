...
TEST_F(ArrayTypeTest, testCheckNullTerminator) {
    ...
    EXPECT_NO_THROW((QueryCompilation::Array{std::array{'a', 'b', '\0'}}));
    EXPECT_THROW((QueryCompilation::Array{std::array{'a', 'b'}}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array{std::array{1, 2, 3}}));
    EXPECT_THROW((QueryCompilation::Array{std::array{'1', '2', '3'}}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array{std::array{'1', '2', '\0'}}));

    // From content
    EXPECT_THROW(((volatile void) QueryCompilation::Array{'a', 'b', 'c'}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array{'a', 'b', '\0'}));
    EXPECT_NO_THROW((QueryCompilation::Array<char, 5>{'a', 'b', '\0'}));
    EXPECT_NO_THROW((QueryCompilation::Array{1, 2, 3}));

    // char: From c-style array
    char const cStyleArray[3] = {'a', 'b', 'c'};
    EXPECT_THROW(((volatile void) QueryCompilation::Array{cStyleArray}), std::runtime_error);
    char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
    EXPECT_NO_THROW(((volatile void) QueryCompilation::Array{cStyleArrayNull}));
    EXPECT_NO_THROW(((volatile void) QueryCompilation::Array<char, 3>{cStyleArrayNull}));
    EXPECT_THROW(((volatile void) QueryCompilation::Array<char, 2>{cStyleArrayNull}), std::runtime_error);
    int const cStyleArrayInt[3] = {1, 2, 3};
    EXPECT_NO_THROW(((volatile void) QueryCompilation::Array{cStyleArrayInt}));

    EXPECT_THROW(((volatile void) QueryCompilation::Array<char, 3>{{'a', 'b', 'c'}}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array<char, 3>{{'a', 'b', '\0'}}));
    EXPECT_NO_THROW((QueryCompilation::Array<char, 3>{{'1', '\0'}}));
    EXPECT_NO_THROW((QueryCompilation::Array<char, 3>{{'1'}}));//< automatically append \0

    // From vector
    EXPECT_THROW((QueryCompilation::Array<char, 3>{std::vector{'a', 'b', 'c'}}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array<char, 3>{std::vector{'a', 'b', '\0'}}));
    EXPECT_THROW((QueryCompilation::Array<char, 2>{std::vector{'a', 'b', '\0'}}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array<char, 2>{std::vector{'\0'}}));
    EXPECT_NO_THROW((QueryCompilation::Array<int, 3>{std::vector{1, 2, 3}}));
    EXPECT_THROW((QueryCompilation::Array<int, 2>{std::vector{1, 2, 3, 4, 5}}), std::runtime_error);
    EXPECT_THROW((QueryCompilation::Array<int, 2>{std::vector{1}}), std::runtime_error);

    // From cstr / std::string
    EXPECT_NO_THROW((QueryCompilation::Array<char, 4>{"abc"}));
    EXPECT_NO_THROW((QueryCompilation::Array<char, 10>{"abc"}));
    EXPECT_THROW((QueryCompilation::Array<char, 1>{"abc"}), std::runtime_error);
    EXPECT_NO_THROW((QueryCompilation::Array<char, 4>{std::string("abc")}));
    EXPECT_NO_THROW((QueryCompilation::Array<char, 10>{std::string("abc")}));
    EXPECT_THROW((QueryCompilation::Array<char, 1>{std::string("abc")}), std::runtime_error);
}
...
TEST_F(ArrayTypeTest, testExceptionSpecifier) {
    ...
    // From supertype's constructor
    EXPECT_TRUE(!noexcept(QueryCompilation::Array{std::array{'a', 'b', '\0'}}));
    EXPECT_TRUE(noexcept(QueryCompilation::Array{std::array{1, 2, 3}}));

    // From content
    EXPECT_TRUE(!noexcept(QueryCompilation::Array{'a', 'b', '\0'}));
    EXPECT_TRUE(noexcept(QueryCompilation::Array<char, 5>{'a', 'b', 'c'}));
    EXPECT_TRUE(noexcept(QueryCompilation::Array{1, 2, 3}));

    // char: From c-style array
    char const cStyleArray[3] = {'a', 'b', 'c'};
    EXPECT_TRUE(!noexcept(QueryCompilation::Array{cStyleArray}));
    int const cStyleArrayInt[3] = {1, 2, 3};
    EXPECT_TRUE(noexcept(QueryCompilation::Array{cStyleArrayInt}));

    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 3>{{'a', 'b', '\0'}}));
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 3>{{'1', '\0'}}));
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 3>{{'1'}}));
    EXPECT_TRUE(noexcept(QueryCompilation::Array<int, 3>{{1}}));
    EXPECT_TRUE(noexcept(QueryCompilation::Array<int, 3>{{1, 2, 3}}));

    // From vector
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 4>{std::vector{'a', 'b', 'c'}}));
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 4>{std::vector{'a', 'b', 'c', '\0'}}));
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<int, 3>{std::vector{1, 2, 3}}));

    // From cstr / std::string
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 4>{"abc"}));
    EXPECT_TRUE(!noexcept(QueryCompilation::Array<char, 4>{std::string("abc")}));
}

/// @brief Utility for checking equality
template<std::size_t s, typename J>
auto eq(QueryCompilation::Array<char, s> const& l, J r) -> bool {
...
TEST_F(ArrayTypeTest, testInitialization) {
    ...
    // From supertype's constructor
    {
        QueryCompilation::Array ca{std::array{'a', 'b', '\0'}};
    ...
    {
        QueryCompilation::Array cai = {std::array{'a', 'b', '\0'}};
    ...
    }
    {
        ...
        QueryCompilation::Array ia{std::array{1, 2, 3}};
    ...
    {
        QueryCompilation::Array iai = {std::array{1, 2, 3}};
    ...
    // From content
    {
        QueryCompilation::Array cc{'a', 'b', '\0'};
    ...
    {
        QueryCompilation::Array cci = {'a', 'b', '\0'};
    ...
    }
    {
        ...
        QueryCompilation::Array<char, 5> ccs{'a', 'b', '\0'};
    ...
    {
        QueryCompilation::Array<char, 5> ccsi{'a', 'b', '\0'};
    ...
    }
    {
        ...
        QueryCompilation::Array ic{1, 2, 3};
    ...
    {
        QueryCompilation::Array ici{1, 2, 3};
    ...
    {
        char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
        QueryCompilation::Array{cStyleArrayNull};
        QueryCompilation::Array<char, 3> cca{cStyleArrayNull};
    ...
    {
        char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
        QueryCompilation::Array<char, 3> ccai{cStyleArrayNull};
    ...
    }
    {
        ...
        int const cStyleArrayInt[3] = {1, 2, 3};
        QueryCompilation::Array ica{cStyleArrayInt};
    ...
    // From vector
    {
        QueryCompilation::Array<char, 3> cv{std::vector{'a', 'b', '\0'}};
        EXPECT_TRUE((std::is_same_v<typename decltype(cv)::type, char>) );
        EXPECT_TRUE(cv.size == 3);
        EXPECT_TRUE(eq(cv, "ab"));
    }
    {
        QueryCompilation::Array<char, 3> cvi = {std::vector{'a', 'b', '\0'}};
    ...
    }
    {
        ...
        QueryCompilation::Array<int, 3> iv{std::vector{1, 2, 3}};
    ...
    {
        QueryCompilation::Array<int, 3> ivi = {std::vector{1, 2, 3}};
    ...
    // From cstr / std::string
    {
        QueryCompilation::Array<char, 4> cstrF{"abc"};
    ...
    }
    {
        ...
        QueryCompilation::Array<char, 10> cstrP{"abc"};
    ...
    }
    {
        ...
        QueryCompilation::Array<char, 4> strF{std::string("abc")};
    ...
    }
    {
        ...
        QueryCompilation::Array<char, 10> strP{std::string("abc")};
