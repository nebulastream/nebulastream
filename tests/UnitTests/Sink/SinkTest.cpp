...
TEST_F(SinkTest, testCSVFileSink) {
    ...
    const DataSinkPtr csvSink = createCSVFileSink(test_schema, 0, 1, nodeEngine, path_to_csv_file, true);
...
TEST_F(SinkTest, testCSVPrintSink) {
    ...
    NodeEngine::WorkerContext wctx(NodeEngine::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    auto csvSink = createCSVPrintSink(test_schema, 0, 1, nodeEngine, os);
    for (uint64_t i = 0; i < 2; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j] = j;
        }
...
TEST_F(SinkTest, testTextPrintSink) {
    ...
    const DataSinkPtr binSink = createTextPrintSink(test_schema, 0, 1, nodeEngine, os);
    for (uint64_t i = 0; i < 5; ++i) {
        for (uint64_t j = 0; j < 5; ++j) {
            buffer.getBuffer<uint64_t>()[j] = j;
        }
...
TEST_F(SinkTest, testCSVZMQSink) {
    ...
    NodeEngine::WorkerContext wctx(NodeEngine::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createCSVZmqSink(test_schema, 0, 1, nodeEngine, "localhost", 666555);
...
TEST_F(SinkTest, testTextZMQSink) {
    ...
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createTextZmqSink(test_schema, 0, 1, nodeEngine, "localhost", 666555);
    for (uint64_t i = 1; i < 3; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j * i] = j;
        }
...
TEST_F(SinkTest, testBinaryZMQSink) {
    ...
    NodeEngine::WorkerContext wctx(NodeEngine::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createBinaryZmqSink(test_schema, 0, 1, nodeEngine, "localhost", 666555, false);
    for (uint64_t i = 1; i < 3; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j * i] = j;
        }
