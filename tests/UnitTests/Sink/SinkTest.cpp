...
TEST_F(SinkTest, testCSVFileSink) {
    ...
    const DataSinkPtr csvSink = createCSVFileSink(0, test_schema, 0, nodeEngine, path_to_csv_file, true);
...
TEST_F(SinkTest, testTextFileSink) {
    ...
    const DataSinkPtr binSink = createTextFileSink(0, test_schema, 0, nodeEngine, path_to_csv_file, true);
...
TEST_F(SinkTest, testNESBinaryFileSink) {
    ...
    const DataSinkPtr binSink = createBinaryNESFileSink(0, test_schema, 0, nodeEngine, path_to_bin_file, true);
    for (uint64_t i = 0; i < 2; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j] = j;
        }
...
TEST_F(SinkTest, testCSVPrintSink) {
    ...
    Runtime::WorkerContext wctx(Runtime::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    auto csvSink = createCSVPrintSink(0, test_schema, 0, nodeEngine, os);
    for (uint64_t i = 0; i < 2; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j] = j;
        }
...
TEST_F(SinkTest, testNullOutSink) {
    ...
    Runtime::WorkerContext wctx(Runtime::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    auto nullSink = createNullOutputSink(0, 1);
    for (uint64_t i = 0; i < 2; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j] = j;
        }
...
TEST_F(SinkTest, testTextPrintSink) {
    ...
    const DataSinkPtr binSink = createTextPrintSink(0, test_schema, 0, nodeEngine, os);
    for (uint64_t i = 0; i < 5; ++i) {
        for (uint64_t j = 0; j < 5; ++j) {
            buffer.getBuffer<uint64_t>()[j] = j;
        }
...
TEST_F(SinkTest, testCSVZMQSink) {
    ...
    Runtime::WorkerContext wctx(Runtime::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createCSVZmqSink(0, test_schema, 0, nodeEngine, "localhost", zmqPort);
...
TEST_F(SinkTest, testTextZMQSink) {
    ...
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createTextZmqSink(0, test_schema, 0, nodeEngine, "localhost", zmqPort);
    for (uint64_t i = 1; i < 3; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j * i] = j;
        }
...
TEST_F(SinkTest, testBinaryZMQSink) {
    ...
    Runtime::WorkerContext wctx(Runtime::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createBinaryZmqSink(0, test_schema, 0, nodeEngine, "localhost", zmqPort, false);
    for (uint64_t i = 1; i < 3; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j * i] = j;
        }
