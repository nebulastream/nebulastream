...
TEST_F(SinkTest, testCSVZMQSink) {
    ...
    NodeEngine::WorkerContext wctx(NodeEngine::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createCSVZmqSink(0, test_schema, 0, nodeEngine, "localhost", 666555);
...
TEST_F(SinkTest, testTextZMQSink) {
    ...
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createTextZmqSink(0, test_schema, 0, nodeEngine, "localhost", 666555);
    for (uint64_t i = 1; i < 3; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j * i] = j;
        }
...
TEST_F(SinkTest, testBinaryZMQSink) {
    ...
    NodeEngine::WorkerContext wctx(NodeEngine::NesThread::getId());
    TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    const DataSinkPtr zmq_sink = createBinaryZmqSink(0, test_schema, 0, nodeEngine, "localhost", 666555, false);
    for (uint64_t i = 1; i < 3; ++i) {
        for (uint64_t j = 0; j < 2; ++j) {
            buffer.getBuffer<uint64_t>()[j * i] = j;
        }
