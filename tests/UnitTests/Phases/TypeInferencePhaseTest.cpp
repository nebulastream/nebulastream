...
TEST_F(TypeInferencePhaseTest, inferQueryWithRenameStreamAndProjectWithFullyQualifiedNamesAndJoinOperator) {
    ...
    auto query = Query::from("default_logical")
                     .as("y")
                     .joinWith(subQuery).where(Attribute("f1")).equalsTo(Attribute("f1")).window(windowType1)
...
TEST_F(TypeInferencePhaseTest, testInferQueryWithMultipleJoins) {
    ...
    auto query = Query::from("default_logical")
                     .joinWith(subQuery).where(Attribute("f1")).equalsTo(Attribute("f3")).window(windowType1)
                     .joinWith(subQuery2).where(Attribute("f5")).equalsTo(Attribute("f3")).window(windowType2)
...
TEST_F(TypeInferencePhaseTest, testJoinOnFourStreams) {
    ...
    auto subQuery2 = Query::from("default_logical3").joinWith(subQuery3).where(Attribute("f5")).equalsTo( Attribute("f7")).window(windowType3);
    auto query = Query::from("default_logical")
                     .joinWith(subQuery).where(Attribute("f1")).equalsTo(Attribute("f3")).window(windowType1)
                     .joinWith(subQuery2).where( Attribute("f1")).equalsTo(Attribute("f5")).window(windowType2)
                     .sink(FileSinkDescriptor::create(""));
    auto plan = query.getQueryPlan();
