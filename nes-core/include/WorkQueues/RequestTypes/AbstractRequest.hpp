...
class AbstractRequest {
  public:
     ...
     */
    virtual void rollBack(std::exception& ex, StorageAccessHandlePtr storageAccessHandle);

    /**
     * Calls rollBack and executes additional error handling based on the exception if necessary
     * @param ex
     * @param storageAccessHandle
     */
    void handleError(std::exception ex, StorageAccessHandlePtr storageAccessHandle);
  ...
  protected:
     ...
     */
    virtual void preRollbackErrorHandling(std::exception ex, StorageAccessHandlePtr storageAccessHandle);
     ...
     * @param storageAccessHandle: The storage access handle used by the request
     */
    virtual void afterRollbackErrorHandling(std::exception ex, StorageAccessHandlePtr storageAccessHandle);
