...
class TopologyChangeLog {
  public:
     ...
     * @return a vector of node ids
     */
    std::vector<TopologyNodeId> getRemovedChildren(TopologyNodeId nodeId);
  ...
  private:
     ...
     */
    static void removeEdgesFromMap(std::unordered_map<TopologyNodeId, std::vector<TopologyNodeId>>& map, const std::vector<Edge>& edges);

    /**
     * @brief adds the elements in the vectors contained in newMap to the vectors found under the same key in additionTarget, but
     * only if the element is not contained in the vector found under the same key in toSubtract
     * @param newMap map containing vectors with elements to add to the corresponding entries in addition target
     ...
     */
    static void addMap(const std::unordered_map<TopologyNodeId, std::vector<TopologyNodeId>>& newMap,
