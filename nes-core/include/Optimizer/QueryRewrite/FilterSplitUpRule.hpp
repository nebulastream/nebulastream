...
class FilterSplitUpRule : public BaseRewriteRule {
  ...
  private:
     ...
     * If our query plan contains a parentOperators->filter(expression1 && expression2)->childOperator.
     * This plan gets rewritten to parentOperators->filter(expression1)->filter(expression2)->childOperator.
     * We will call splitUpFilters() on the new filters as well
     * If our query plan contains a parentOperators->filter(!(expression1 || expression2))->childOperator, we use deMorgan to
