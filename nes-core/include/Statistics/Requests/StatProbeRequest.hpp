...
class StatProbeRequest : public StatRequest {
  public:
    StatProbeRequest(const std::string& logicalSourceName,
        ...
        : StatRequest(logicalSourceName, fieldName, statCollectorType), probeExpression(probeExpression),
     ...
     */
    std::string getProbeExpression() const { return probeExpression; }

    /**
     * @return returns the physicalSourceNames over which the statCollectors were generated that we wish to probe/query
     */
    std::vector<std::string> getPhysicalSourceNames() const { return physicalSourceNames; }
