...
class GlobalQueryPlanUpdatePhaseTest : public Testing::TestWithErrorHandling<testing::Test> {
  public:
    ...
    void SetUp() override {
        ...
        SourceCatalogEntryPtr sourceCatalogEntry1 = std::make_shared<SourceCatalogEntry>(physicalSource, logicalSource, node);
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, DISABLED_executeQueryMergerPhaseForSingleInvalidQueryPlan) {
    ...
    auto catalogEntry1 = QueryCatalogEntry(INVALID_QUERY_ID, "", "topdown", q1.getQueryPlan(), QueryStatus::Scheduling);
    auto request = RunQueryRequest::create(catalogEntry1.getInputQueryPlan(),
                                           catalogEntry1.getQueryPlacementStrategy());
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForMultipleValidQueryPlan) {
    ...
    auto request1 = RunQueryRequest::create(catalogEntry1->getInputQueryPlan(),
                                            catalogEntry1->getQueryPlacementStrategy());
    auto request2 = RunQueryRequest::create(catalogEntry2->getInputQueryPlan(),
                                            catalogEntry2->getQueryPlacementStrategy());
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, DISABLED_executeQueryMergerPhaseForAValidQueryPlanInInvalidState) {
    ...
    auto phase = Optimizer::GlobalQueryPlanUpdatePhase::create(queryCatalogService,
                                                               ...
                                                               optimizerConfiguration);
    NES_INFO("GlobalQueryPlanUpdatePhaseTest: Create the batch of query plan with duplicate query plans.");
    auto nesRequest1 = RunQueryRequest::create(catalogEntry1->getInputQueryPlan(),
                                               catalogEntry1->getQueryPlacementStrategy());
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForMultipleValidQueryRequestsWithAddAndRemoval) {
    ...
    auto nesRequest1 = RunQueryRequest::create(catalogEntry1->getInputQueryPlan(),
                                               catalogEntry1->getQueryPlacementStrategy());
    auto nesRequest2 = RunQueryRequest::create(catalogEntry2->getInputQueryPlan(),
                                               catalogEntry1->getQueryPlacementStrategy());
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, queryMergerPhaseForSingleQueryPlan) {
    ...
    for (auto& [key, value] : allQueries) {
        auto nesRequest = RunQueryRequest::create(value->getInputQueryPlan(),
                                                  value->getQueryPlacementStrategy());
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, queryMergerPhaseForSingleQueryPlan1) {
    ...
    //Setup source catalog
    auto sourceCatalog = std::make_shared<SourceCatalog>(QueryParsingServicePtr());
