...
using namespace NES;
        ...
        class QueryPlacementTest : public Testing::TestWithErrorHandling<testing::Test> {
          public:
            ...
            void setupTopologyWith3Nodes(std::vector<int> availableSlots, std::vector<int> availableMemory, std::vector<int> availableBandwidth){
                ...
                node2->setAvailableBandwidth(availableBandwidth[0]);
            ...
            void setupQuery1(){
                Query query = Query::from("iris").filter(Attribute("sepLength")>=Attribute("sepWidth"))
                                  .map(Attribute("petWidth")=Attribute("petWidth") * Attribute("petLength") * Attribute("sepWidth") * Attribute("sepLength"))
                                  .map(Attribute("sepLength")=10).filter(Attribute("petWidth")<Attribute("petWidth") * 10).
                              project(Attribute("sepLength"),Attribute("sepWidth")).filter(Attribute("sepLength")<=57).
                              filter(Attribute("sepWidth")==47).sink(NullOutputSinkDescriptor::create());
                QueryPlanPtr queryPlan = query.getQueryPlan();
    ...
    globalExecutionPlan->getExecutionNodeByNodeId(3)->setAvailableBandwidth(2000000);*/
        ...
        TEST_F(QueryPlacementTest, finalTestWith3Nodes){
            std::ofstream logFile;
            logFile.open("/home/noah/Desktop/nodeTest/nodes.csv");
            ...
            std::vector<int> availableSlots = {5,3,1};
            std::vector<int> availableMemory = {512,512,512};
            std::vector<int> availableBandwidth = {500000,500000};

            for (int i = 0; i < 25; ++i) {

                setupTopologyWith3Nodes(availableSlots,{5120,5120,5120},{2000000,1000000});
                setupQuery1();
                std::transform(std::begin(availableSlots),std::end(availableSlots),std::begin(availableSlots),[](int x){return x+2;});
    ...
    Optimizer::BasePlacementStrategy::firstFitRecursivePlacement(sortedList, ftConfig, topology, globalExecutionPlan);*/
        ...
        TEST_F(QueryPlacementTest, loopingPlacementTest){
            setupTopologyWith3Nodes({5,3,1},{5120,5120,5120},{2000000,1000000});
            setupQuery1();
            ...
            for (int i = 0; i < 25; ++i) {
                ...
                ftConfig->setIngestionRate(csvSourceType->getGatheringInterval()->getValue());
                ftConfig->setTupleSize(sourceCatalog->getSchemaForLogicalSource("iris")->getSchemaSizeInBytes());
                ftConfig->setAckRate(epochRate);
                ftConfig->setAckSize(ack_size);
                ftConfig->setProcessingGuarantee(FaultToleranceType::AT_LEAST_ONCE);
                ftConfig->setQueryId(queryId);
        ...
        TEST_F(QueryPlacementTest, csvTest){
            setupTopologyWith3Nodes({5,3,1},{5120,5120,5120},{1000000,250000});
            setupQuery1();
            ...
            Optimizer::BasePlacementStrategy::setupLogger("setupTest", globalExecutionPlan, executionNodes, topology, ftConfig);

            Optimizer::BasePlacementStrategy::nodeLogger("nodeTest",globalExecutionPlan,executionNodes,topology,ftConfig);
