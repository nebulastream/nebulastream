...
TEST_F(TopologyManagerServiceTest, testRegisterUnregisterNode) {
    ...
    uint64_t nodeId =
        topologyManagerService->registerNode(ip, publish_port, 5000, 6, false, NES::Spatial::Index::Experimental::Location(), false);
    EXPECT_NE(nodeId, 0u);

    uint64_t nodeId1 =
        topologyManagerService->registerNode(ip, publish_port + 2, 5000, 6, false, NES::Spatial::Index::Experimental::Location(), false);
