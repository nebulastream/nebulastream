...
TEST_F(TopologyManagerServiceTest, testRegisterUnregisterNode) {
    ...
    uint64_t nodeId = topologyManagerService->registerNode(ip, publish_port, 5000, 6, false, NES::Experimental::Mobility::GeographicalLocation());
    EXPECT_NE(nodeId, 0u);

    uint64_t nodeId1 = topologyManagerService->registerNode(ip, publish_port + 2, 5000, 6, false, NES::Experimental::Mobility::GeographicalLocation());
    EXPECT_NE(nodeId1, 0u);

    //test register existing node
    uint64_t nodeId2 = topologyManagerService->registerNode(ip, publish_port, 5000, 6, false, NES::Experimental::Mobility::GeographicalLocation());
