...
TEST_F(OperatorCodeGenerationTest, codeGenerationCopy) {
    ...
    if (auto buffer = source->receiveData().value(); !!buffer) {
        /* execute Stage */
        NES_INFO("Processing {} tuples: ", buffer.getNumberOfTuples());
...
TEST_F(OperatorCodeGenerationTest, codeGenerationFilterPredicate) {
    ...
    NES_INFO("Processing {} tuples: ", inputBuffer.getNumberOfTuples());
...
TEST_F(OperatorCodeGenerationTest, codeGenerationDistributedCombiner) {
    ...
    NES_DEBUG("buffer={}", dynamicTupleBufferAsString.str());
    ...
    for (auto& [key, val] : stateVar->rangeAll()) {
        ...
        valueAsString << val;
        NES_DEBUG("Key: {} Value: {}", key, valueAsString.str());
        for (auto& slice : val->getSliceMetadata()) {
            NES_DEBUG("start={} end={}", slice.getStartTs(), slice.getEndTs());
...
#ifdef ONNXDEF
/**
 * @brief This test generates an infer model operator using the ONNX runtime
 */
TEST_F(OperatorCodeGenerationTest, codeGenerationInferModelONNXTest) {
    auto defaultSourceType = DefaultSourceType::create();
    auto physicalSource = PhysicalSource::create("default", "defaultPhysical", defaultSourceType);
    auto workerConfiguration = WorkerConfiguration::create();
    workerConfiguration->dataPort.setValue(*dataPort);
    ...
    auto inferModelOperatorHandler =
        InferModel::InferModelOperatorHandler::create(std::string(TEST_DATA_DIRECTORY) + "iris_95acc.onnx");
...
TEST_F(OperatorCodeGenerationTest, DISABLED_codeGenerations) {
    ...
    std::stringstream dynamicTupleBufferAsString;
    dynamicTupleBufferAsString << dynamicTupleBuffer;
    NES_DEBUG("buffer content={}", dynamicTupleBufferAsString.str());
...
TEST_F(OperatorCodeGenerationTest, codeGenerationCEPIterationOPinitialTest) {
    ...
    std::stringstream dynamicTupleBufferAsString;
    dynamicTupleBufferAsString << dynamicTupleBuffer;
    NES_DEBUG("{}", dynamicTupleBufferAsString.str());
}
}// namespace NES
