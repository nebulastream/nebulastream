...
class KeyedSliceTest : public testing::Test {
  public:
    /* Will be called before any test in this class are executed. */
    static void SetUpTestCase() { std::cout << "Setup KeyedSliceTest test class." << std::endl; }

    /* Will be called before a test is executed. */
    void SetUp() override {
        NES::Logger::setupLogging("KeyedSliceTest.log", NES::LogLevel::LOG_DEBUG);
        std::cout << "Setup KeyedSliceTest test case." << std::endl;
...
TEST_F(KeyedSliceTest, assignTumblingWindow) {
    auto tumblingWindowSize = 100;
    auto sliceStore = KeyedThreadLocalSliceStore(hashMapFactory, tumblingWindowSize, 100);
    ...
    ASSERT_EQ(sliceStore.getNumberOfSlices(), 9);
...
TEST_F(KeyedSliceTest, invalidTs) {
    auto tumblingWindowSize = 100;
    auto sliceStore = KeyedThreadLocalSliceStore(hashMapFactory, tumblingWindowSize, 100);
