 ...
 * Server that sends "42,0.5,hello" and "100,0.89,help" alternating via socket and inbetween it sends the message size total number of messages is 4
 */
TEST_F(SourceTest, DISABLED_TCPSourceReadCSVDataWithVariableMessageLength) {
    TCPSourceTypePtr sourceConfig = TCPSourceType::create();
    ...
    EXPECT_EQ(tcpDataSource.getNumberOfGeneratedTuples(), 4u);
 ...
 */
TEST_F(SourceTest, DISABLED_TCPSourceReadCSVDataWithUserDefinedMessageLength) {
    TCPSourceTypePtr sourceConfig = TCPSourceType::create();
    sourceConfig->setSocketPort(9000);
    sourceConfig->setSocketHost("127.0.0.1");
    sourceConfig->setSocketDomainViaString("AF_INET");
 ...
 */
TEST_F(SourceTest, DISABLED_TCPSourceReadJSONDataWithTupleSeparator) {
    TCPSourceTypePtr sourceConfig = TCPSourceType::create();
    sourceConfig->setSocketPort(9000);
    sourceConfig->setSocketHost("127.0.0.1");
    sourceConfig->setSocketDomainViaString("AF_INET");
...
/**
 * @brief tests JSON schema. We also obtain the size of the buffer from the server. We again used an external TCP Python Server
 * that sends {'id': '42', 'value': '0.5', 'name': 'hello'} this JSON 5 times, before each message, we also need to send the size of the message as four bytes
 */
TEST_F(SourceTest, DISABLED_TCPSourceReadJSONDataObtainSizeFromSocket) {
    TCPSourceTypePtr sourceConfig = TCPSourceType::create();
    sourceConfig->setSocketPort(9000);
    sourceConfig->setSocketHost("127.0.0.1");
    sourceConfig->setSocketDomainViaString("AF_INET");
 ...
 * that sends {'id': '42', 'value': '0.5', 'name': 'hello'} this JSON 5 times
 */
TEST_F(SourceTest, DISABLED_TCPSourceReadJSONDataWithFixedSize) {
    TCPSourceTypePtr sourceConfig = TCPSourceType::create();
    sourceConfig->setSocketPort(9000);
    sourceConfig->setSocketHost("127.0.0.1");
    sourceConfig->setSocketDomainViaString("AF_INET");
...
TEST_F(SourceTest, testMonitoringSourceReceiveDataOnceMemoryConfiguration) {
    // create metrics and plan for MonitoringSource
    std::list<std::string> configuredAttributes {"TOTAL_SWAP", "SHARED_RAM", "TOTAL_HIGH", "BUFFER_RAM", "MEM_UNIT", "FREE_RAM"};
    std::list<std::string> notConfiguredAttributes {"LOADS_15MIN", "LOADS_5MIN", "LOADS_1MIN", "PROCS", "FREE_HIGH", "FREE_SWAP", "TOTAL_RAM"};
...
TEST_F(SourceTest, testMonitoringSourceReceiveDataOnceNetworkConfiguration) {
    // create metrics and plan for MonitoringSource
    std::list<std::string> configuredAttributes {"rBytes", "rPackets", "tBytes", "tPackets", "interfaceName"};
    std::list<std::string> notConfiguredAttributes {"tCompressed", "tCarrier", "tColls", "tFifo", "tDrop", "tErrs", "rMulticast",
                                                   "rCompressed", "rFrame", "rFifo", "rDrop", "rErrs"};
...
TEST_F(SourceTest, testMonitoringSourceReceiveDataOnceCPUConfiguration) {
    // create metrics and plan for MonitoringSource
    std::list<std::string> configuredAttributes {"coreNum", "idle", "system", "iowait", "user"};
    std::list<std::string> notConfiguredAttributes {"guestnice", "guest", "steal", "softirq", "irq", "nice"};
