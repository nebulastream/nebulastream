...
/**
   ...
   * @return true if successful
   */
[[nodiscard]] bool checkOrderedOutputOrTimeout(string expectedContent, const string& outputFilePath, uint64_t customTimeout = 0) {
    std::chrono::seconds timeoutInSec;
    if (customTimeout == 0) {
        timeoutInSec = std::chrono::seconds(defaultTimeout);
    } else {
    ...
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        ...
        if (ifs.good() && ifs.is_open()) {
            ...
            if (found == count) {
                ...
                auto wordsIt = words.begin();
                for (auto& expectedline : expectedlines) {
                    if (*wordsIt == expectedline) {
                        found++;
                        wordsIt++;
                    } else {
                        NES_TRACE("could not match: ");
                        NES_TRACE("word " << *wordsIt);
                        NES_TRACE("line " << expectedline);
                        return false;
                    }
