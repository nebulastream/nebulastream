...
TEST_F(MonitoringIntegrationTest, DISABLED_requestAllMetricsViaRestLennart03) {
    uint64_t noWorkers = 1;
...
TEST_F(MonitoringIntegrationTest, requestAllMetricsFromMonitoringStreamsConfiguration) {
    ...
    std::set<std::string> expectedMonitoringStreams {"wrapped_network", "wrapped_cpu", "memory", "disk"};
    std::string configMonitoring01 = " - cpu: attributes: \"user, nice, idle, iowait, irq, softirq\" sampleRate: 1000"
                                     " - disk: attributes: \"F_BAVAIL, F_BLOCKS, F_FRSIZE\" sampleRate: 1000"
                                     " - memory: attributes: \"TOTAL_SWAP\" sampleRate: 1000"
                                     " - network: attributes: \"rBytes, rPackets, rCompressed, rMulticast, "
                                     "tBytes, tPackets, tCarrier, tCompressed\" ";
//    std::set<std::string> expectedMonitoringStreams {"wrapped_cpu, disk"};
    std::string configMonitoring02 = " - cpu: attributes: \"nice, user, system, idle, iowait\" cores: \"0, 2, 3, 6\" sampleRate: 1000"
                                     " - disk: attributes: \"F_BAVAIL, F_BLOCKS, F_BSIZE\" sampleRate: 1000";
