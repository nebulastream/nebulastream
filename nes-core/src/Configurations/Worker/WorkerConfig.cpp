...
WorkerConfig::WorkerConfig() {
    ...
    queuePinList = ConfigOption<std::string>::create("queuePinList", "", "comma separated list of where to map the worker on the queue");
...
const StringConfigOption& WorkerConfig::getQueuePinList() const { return queuePinList; }
...
void WorkerConfig::setQueuePinList(const std::string list) {
    if (!list.empty()) {
        WorkerConfig::queuePinList->setValue(list);
    }
}
