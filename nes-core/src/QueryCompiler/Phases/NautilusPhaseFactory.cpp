...
PhaseFactoryPtr NautilusPhaseFactory::create() { return std::make_shared<NautilusPhaseFactory>(); }

PipeliningPhasePtr NautilusPhaseFactory::createPipeliningPhase(QueryCompilerOptionsPtr options) {
...
LowerLogicalToPhysicalOperatorsPtr NautilusPhaseFactory::createLowerLogicalQueryPlanPhase(QueryCompilerOptionsPtr options) {
...
AddScanAndEmitPhasePtr NautilusPhaseFactory::createAddScanAndEmitPhase(QueryCompilerOptionsPtr) {
    NES_DEBUG("Create add scan and emit phase");
    return AddScanAndEmitPhase::create();
}
LowerPhysicalToGeneratableOperatorsPtr
NautilusPhaseFactory::createLowerPhysicalToGeneratableOperatorsPhase(QueryCompilerOptionsPtr) {
    NES_NOT_IMPLEMENTED();
}
CodeGenerationPhasePtr NautilusPhaseFactory::createCodeGenerationPhase(QueryCompilerOptionsPtr,
                                                                       Compiler::JITCompilerPtr) {
    NES_NOT_IMPLEMENTED();
}
LowerToExecutableQueryPlanPhasePtr NautilusPhaseFactory::createLowerToExecutableQueryPlanPhase(QueryCompilerOptionsPtr options,
                                                                                               bool sourceSharing) {
...
BufferOptimizationPhasePtr NautilusPhaseFactory::createBufferOptimizationPhase(QueryCompilerOptionsPtr options) {
    NES_DEBUG("Create buffer optimization phase");
    return BufferOptimizationPhase::create(options->getOutputBufferOptimizationLevel());
}
PredicationOptimizationPhasePtr NautilusPhaseFactory::createPredicationOptimizationPhase(QueryCompilerOptionsPtr options) {
