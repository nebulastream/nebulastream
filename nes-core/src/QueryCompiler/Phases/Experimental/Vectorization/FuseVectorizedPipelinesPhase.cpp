...
FuseVectorizedPipelinesPhase::FuseVectorizedPipelinesPhase(const QueryCompilerOptionsPtr& options)
    : options(std::move(options))
{

}

PipelineQueryPlanPtr FuseVectorizedPipelinesPhase::apply(PipelineQueryPlanPtr pipelinedQueryPlan) {
...
OperatorPipelinePtr FuseVectorizedPipelinesPhase::lower(OperatorPipelinePtr operatorPipeline) {
    auto queryPlan = operatorPipeline->getQueryPlan();
    auto nodes = QueryPlanIterator(queryPlan).snapshot();

    NES_DEBUG("Looking for fusable vectorized pipelines...");
    for (const auto& node : nodes) {
        auto vectorizedPipeline = node->as_if<PhysicalVectorizedPipelineOperator>();
        if (vectorizedPipeline) {
            // TODO Merge adjacent PhysicalVectorizedPipelineOperator
        }
    }
