...
LowerVectorizedPipelineToKernelPhase::LowerVectorizedPipelineToKernelPhase(const QueryCompilerOptionsPtr& options)
    : options(std::move(options))
{

}

PipelineQueryPlanPtr LowerVectorizedPipelineToKernelPhase::apply(PipelineQueryPlanPtr pipelinedQueryPlan) {
...
OperatorPipelinePtr LowerVectorizedPipelineToKernelPhase::lower(OperatorPipelinePtr operatorPipeline) {
    ...
    for (const auto& node : nodes) {
        ...
        if (vectorizedPipeline) {
            ...
            vectorizedPipeline->replace(vectorize);
        }
    }
