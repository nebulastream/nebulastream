...
void QueryCatalogController::handleGet(const std::vector<utility::string_t>& path, web::http::http_request& request) {
    ...
    if (path[1] == "queryIdAndCatalogEntryMapping") {
        ...
        try {
            //Prepare the response
            web::json::value result{};
            std::map<uint64_t, Catalogs::QueryCatalogEntryPtr> queryCatalogEntries = queryCatalogService->getAllQueryCatalogEntries();
        ...
        try {
            ...
            //Prepare the response
            web::json::value result{};
            const Catalogs::QueryCatalogEntryPtr queryCatalogEntry = queryCatalogService->getEntryForQuery(std::stoi(queryId));
