...
void BaseController::handlePut(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::PUT, request);
}

void BaseController::handlePost(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::TRCE, request);
}

void BaseController::handleDelete(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::DEL, request);
}

void BaseController::handlePatch(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::TRCE, request);
}
void BaseController::handleHead(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::HEAD, request);
}

void BaseController::handleTrace(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::TRCE, request);
}

void BaseController::handleMerge(const std::vector<utility::string_t>&, web::http::http_request& request) {
    RestEngine::returnDefaultNotImplementedReply(web::http::methods::MERGE, request);
}

void BaseController::handleOptions(const web::http::http_request& request) {
...
void BaseController::internalServerErrorImpl(const web::http::http_request& message) {
...
void BaseController::resourceNotFoundImpl(const web::http::http_request& message) {
    web::http::http_response response(web::http::status_codes::NotFound);
    response.headers().add(U("Access-Control-Allow-Origin"), U("*"));
    message.reply(response);
}

void BaseController::noContentImpl(const web::http::http_request& message) {
...
std::map<utility::string_t, utility::string_t> BaseController::getParameters(web::http::http_request& request) {
    return web::uri::split_query(request.request_uri().query());
}
    ...
    if (strcmp(exceptionMsg, "Key not found") == 0) {
        ...
        if (paths[0] == "streamCatalog") {
            if (paths[1] == "allPhysicalStream" || paths[1] == "deleteLogicalStream") {
                errorResponse["detail"] = web::json::value::string("Parameter streamName must be provided");
            } else if (paths[1] == "addLogicalStream" || paths[1] == "updateLogicalStream") {
                errorResponse["detail"] = web::json::value::string("Parameter streamName and schema must be provided");
            }
