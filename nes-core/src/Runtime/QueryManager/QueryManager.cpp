...
bool QueryManager::addReconfigurationMessage(QueryId queryId,
    ...
    if (mode == Dynamic) {
        ...
        for (uint64_t threadId = 0; threadId < numberOfThreadsPerQueue; threadId++) {
            std::cout << "add reconf for query " << queryId << " uses queue=" << queryToTaskQueueIdMap[queryId] << " numberOfThreadsPerQueue=" << numberOfThreadsPerQueue << " type=" << task->getType() << std::endl;
