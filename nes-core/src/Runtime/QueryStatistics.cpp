...
void QueryStatistics::incTasksPerPipelineId(uint64_t pipelineId, uint64_t workerId) { this->pipelineIdToTaskThroughputMap[pipelineId][workerId]++; }
...
std::map<uint64_t, std::map<uint64_t ,std::atomic<uint64_t>>>& QueryStatistics::getPipelineIdToTaskMap(){return pipelineIdToTaskThroughputMap;};
