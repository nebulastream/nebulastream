    ...
    healthCheckingThread = std::make_shared<std::thread>(([this]() {
        ...
        while (isRunning) {
            for (auto node : nodeIdToTopologyNodeMap.lock_table()) {
                ...
                auto nodeParents = node.second->getParents();
                for (auto parent : nodeParents) {
                    if (parent->as<TopologyNode>()->getId() == 1) {
                        NES_TRACE("NesCoordinator::healthcheck: coordinator will check on direct child with id={}",
                                  node.second->getId());
