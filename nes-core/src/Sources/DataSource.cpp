...
bool DataSource::start() {
    ...
    // only CPU i as set.
        ...
        if (sourceAffinity  != std::numeric_limits<uint64_t>::max()) {
            NES_ASSERT(sourceAffinity < std::thread::hardware_concurrency(), "pinning data source position is out of cpu range maxPosition=" << sourceAffinity);
            cpu_set_t cpuset;
            CPU_ZERO(&cpuset);
...
void DataSource::runningRoutineWithGatheringInterval() {
    ...
    while (running) {
        ...
        NES_DEBUG("DataSource " << operatorId << ": eion " << cnt);
