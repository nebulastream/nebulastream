...
namespace NES {
  ...
  void DataSource::emitWork(Runtime::TupleBuffer& buffer) {
    ...
    if (true) {
      // create dynamic buffer out of buffer
      ...
      // pass dynamic buffer to updateStats
      StatManager::updateStats(schema, dynamicTupleBuffer);
    }
  ...
  const std::atomic<uint64_t>& DataSource::getActiveStats() const { return activeStats; }

  void DataSource::setActiveStats(const std::atomic<uint64_t>& activeStats) { DataSource::activeStats.store(activeStats); }

  const std::atomic_bool& DataSource::getCollectingStat() const { return collectingStat; }

  void DataSource::setCollectingStat(const std::atomic_bool& collectingStat) { DataSource::collectingStat.store(collectingStat); }
...
// debugging
  ...
  OriginId DataSource::getOriginId() const { return originId; }

  void DataSource::setOriginId(OriginId originId) { DataSource::originId = originId; }
