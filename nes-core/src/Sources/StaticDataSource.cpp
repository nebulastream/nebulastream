...
StaticDataSource::StaticDataSource(SchemaPtr schema,
    ...
    NES_DEBUG2("StaticDataSource: id {} Initialize source with schema: |{}| size: {}", std::to_string(operatorId), this->schema->toString(), std::to_string(tupleSizeInBytes));
    ...
    NES_DEBUG2("StaticDataSource() operatorId: {}: eagerLoading={} numTuplesToProcess={} numBuffersToProcess={} numTuplesPerBuffer={}", operatorId, eagerLoading, numTuples, numBuffersToProcess, numTuplesPerBuffer);
...
void StaticDataSource::fillBuffer(::NES::Runtime::MemoryLayouts::DynamicTupleBuffer& buffer) {
    ...
    generatedBuffers++;
    NES_DEBUG2("StaticDataSource::fillBuffer: reading finished read {} tuples at posInFile={}", tupleCount, currentPositionInFile);
