...
QueryPlanPtr DistributeWindowRule::apply(QueryPlanPtr queryPlan) {
    NES_DEBUG("DistributeWindowRule: Apply DistributeWindowRule.");
    NES_DEBUG("DistributeWindowRule::apply: plan before replace \n" << queryPlan->toString());
    if (!performDistributedWindowOptimization) {
        return queryPlan;
    }
...
void DistributeWindowRule::createDistributedNemoWindowOperator(const WindowOperatorNodePtr& windowOp,
