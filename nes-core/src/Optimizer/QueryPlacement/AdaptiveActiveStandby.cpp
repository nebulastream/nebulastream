...
AdaptiveActiveStandbyPtr AdaptiveActiveStandby::create(TopologyPtr topology) {
    return std::make_unique<AdaptiveActiveStandby>(AdaptiveActiveStandby(std::move(topology)));
}

AdaptiveActiveStandby::AdaptiveActiveStandby(TopologyPtr topology) : topology(std::move(topology)) {}

//
//bool AdaptiveActiveStandby::execute(QueryId queryId,
...
bool AdaptiveActiveStandby::execute(const std::vector<OperatorNodePtr>& pinnedUpStreamOperators) {
    ...
    // 3. find an initial placement using a greedy algorithm
    ...
    // 4. evaluate placement, add scores to operator and topology nodes
    double score = evaluateEntireCandidatePlacement();
    ...
    // 5. save placement as current best
...
//    // 6. start repeated local search as long as there is time left
//    auto currentTime = std::chrono::steady_clock::now();
//    auto elapsedSeconds = duration_cast<std::chrono::seconds>(start - currentTime);
//
//    while (elapsedSeconds < timeConstraint) {
//        NES_DEBUG("AdaptiveActiveStandby: Starting new Local Search, time left: " << (timeConstraint - elapsedSeconds).count() << "s.");
//        auto scoreChange = executeLocalSearch(timeConstraint - elapsedSeconds);
//        if (scoreChange > 0) {
//            NES_DEBUG("AdaptiveActiveStandby: Local Search found a better placement with a score improvement of " << scoreChange << ".");
    ...
    for (const auto& currentOperator : pinnedUpStreamOperators) {
        ...
        for (const auto& currentParent : ancestorsOnDifferentNodes) {
            ...
            if (exists) {
                ...
                auto nodesToExclude = getAllTopologyNodesWherePinned(allAncestors);
...
bool AdaptiveActiveStandby::separatePathExists(const OperatorNodePtr& primaryOperator, const TopologyNodePtr& startTopologyNode) {
    ...
    auto nodesToExclude = getAllTopologyNodesWherePinned(allAncestors);
    nodesToExclude.erase(topology->getRoot()->getId());
    ...
    return topology->findAllPathBetween(startTopologyNode, topology->getRoot(),
...
double AdaptiveActiveStandby::executeLocalSearch(std::chrono::seconds timeLeft) {
    ...
    while (elapsedSeconds < timeLeft) {
        //NES_DEBUG("AdaptiveActiveStandby: Starting new iteration in Local Search, time left: " << (timeLeft - elapsedSeconds).count() << "s.");
...
std::map<TopologyNodeId, double> AdaptiveActiveStandby::getCandidatePlacementsGreedy(const OperatorNodePtr& primaryOperator) {
    ...
    while (!candidateNodes.empty()) {
        ...
        for (const auto& childOperator: childrenOperators) {
            ...
            auto distance = calculateDistance(childOperatorsNode, candidateNode);
...
double AdaptiveActiveStandby::calculateOverUtilizationPenalty(const TopologyNodePtr& topologyNode,
                                                              const OperatorNodePtr& primaryOperator) {
    double penalty = 0.0;

    int availableResources = calculateAvailableResources(topologyNode);

    if (primaryOperator != nullptr) {
        int operatorCost = getOperatorCost(primaryOperator);
        availableResources -= operatorCost;
    }
...
OperatorNodePtr AdaptiveActiveStandby::createReplica(const OperatorNodePtr& primaryOperator) {
    ...
    /// temporary solution
    /// - creates a copy operator with ID*100
    /// - points to a different object, properties can be set independently (but they are copied!)
...
double AdaptiveActiveStandby::evaluateSinglePlacement(const OperatorNodePtr& secondaryOperator,
    ...
    for (const auto& parentNode: secondaryOperator->getParents()) {
        ...
        // network cost = output * distance of nodes
        auto distance = calculateDistance(topologyNode, parentOperatorsNode);
    ...
    for (const auto& childNode: secondaryOperator->getChildren()) {
        ...
        // network cost = output * distance of nodes
        auto output = getOperatorOutput(childOperator);
        auto distance = calculateDistance(childOperatorsNode, topologyNode);
...
std::set<TopologyNodeId> AdaptiveActiveStandby::getAllTopologyNodesWherePinned(const std::vector<NodePtr>& operatorNodes) {
