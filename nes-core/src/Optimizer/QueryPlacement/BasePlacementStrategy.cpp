...
return downstreamLinkWeights;
        ...
        float BasePlacementStrategy::getNetworkConnectivity(const TopologyPtr& topology, const ExecutionNodePtr& upstreamNode){
            ...
            if(upstreamNode->getParents().empty()){
                ...
                TopologyNodePtr topDownstreamNode = topology->findNodeWithId(getExecutionNodeParent(upstreamNode)->getId());
        ...
        void BasePlacementStrategy::initAdjustedCosts(const TopologyPtr& topology, const GlobalExecutionPlanPtr& globalExecutionPlan, const ExecutionNodePtr& rootNode, QueryId queryId){
            ...
            for(auto& node : globalExecutionPlan->getExecutionNodesByQueryId(queryId)){
                if(!node->getParents().empty()){
                    float parent = topology->findNodeWithId(node->getParents()[0]->as<ExecutionNode>()->getId())->getResourceCapacity();
                    float child = topology->findNodeWithId(node->getId())->getResourceCapacity();
                    float ratio = parent / child;
                    node->setDownstreamRatio(ratio);
                }else{
...
switch(indexOfBestScore) {
    ...
    default:
        ...
        int BasePlacementStrategy::getNumberOfOperators(const ExecutionNodePtr& executionNode, QueryId queryId){
            ...
            for(auto& subPlan : executionNode->getQuerySubPlans(queryId)) {
                for (auto& op : subPlan->getRootOperators()) {
                    costs += getNumberOfOperatorsRecursively(op->as<LogicalOperatorNode>());
                }
            }
            return costs;
        }
        ...
        int BasePlacementStrategy::getNumberOfOperatorsRecursively(const LogicalOperatorNodePtr& operatorNode){
            ...
            for(auto& child : operatorNode->getChildren()){
                cost += getNumberOfOperatorsRecursively(child->as<LogicalOperatorNode>());
            }
            return cost;
        }
...
return (executionNode->getDownstreamRatio() * getNumberOfStatefulOperatorsOnExecutionNodeRecursively(executionNode, queryId));*/
        ...
        float BasePlacementStrategy::calcUpstreamBackupNetworking(const ExecutionNodePtr& executionNode, const FaultToleranceConfigurationPtr& ftConfig, const TopologyPtr topology){
            ...
            float result =  (newUsedBand / topNode->getAvailableBandwidth()) * upstreamBackupNetworkingFormula;

            for(auto& parent : executionNode->getParents()){
                ExecutionNodePtr parentNode = parent->as<ExecutionNode>();
                if(parentNode->getId() != 1){
                    result += calcUpstreamBackupNetworking(parent->as<ExecutionNode>(), ftConfig, topology);
                }
            }
            return result;
        ...
        double BasePlacementStrategy::calcUpstreamBackupCost(const ExecutionNodePtr& executionNode, std::vector<ExecutionNodePtr> executionNodes, const FaultToleranceConfigurationPtr& ftConfig, const TopologyPtr& topology){
            ...
            std::transform(executionNodes.begin(), executionNodes.end(), std::back_inserter(allUpstreamBackupNetworkingCosts),
                           [ftConfig, topology](const ExecutionNodePtr& executionNode) { return calcUpstreamBackupNetworking(executionNode, ftConfig, topology); });
            ...
            double upstreamBackupCost = calcExecutionNodeNormalizedFTCost(calcUpstreamBackupProcessing(executionNode, ftConfig->getQueryId()),
                                                                          calcUpstreamBackupNetworking(executionNode, ftConfig, topology),
        ...
        float BasePlacementStrategy::calcActiveStandbyNetworking(const ExecutionNodePtr& executionNode, const FaultToleranceConfigurationPtr& ftConfig, const TopologyPtr topology){
            ...
            for(auto& parent : executionNode->getParents()){
                ExecutionNodePtr parentNode = parent->as<ExecutionNode>();
                if(parentNode->getId() != 1){
                    result += calcActiveStandbyNetworking(parent->as<ExecutionNode>(), ftConfig, topology);
                }
            }
            return result;
        }
        ...
        int BasePlacementStrategy::calcActiveStandbyMemory(const ExecutionNodePtr& executionNode, const TopologyPtr& topology, const FaultToleranceConfigurationPtr& ftConfig){
            ...
            float networkConnectivityInSeconds = getNetworkConnectivity(topology, executionNode) / 1000;
        ...
        double BasePlacementStrategy::calcActiveStandbyCost(const ExecutionNodePtr& executionNode, std::vector<ExecutionNodePtr> executionNodes, const FaultToleranceConfigurationPtr& ftConfig, const TopologyPtr& topology){
            ...
            std::transform(executionNodes.begin(), executionNodes.end(), std::back_inserter(allActiveStandbyNetworkingCosts),
                           [ftConfig, topology](const ExecutionNodePtr& executionNode) { return calcActiveStandbyNetworking(executionNode, ftConfig, topology); });
            ...
            double activeStandbyCost = calcExecutionNodeNormalizedFTCost(calcActiveStandbyProcessing(executionNode, ftConfig->getQueryId()),
                                                                         calcActiveStandbyNetworking(executionNode, ftConfig, topology),
        ...
        float BasePlacementStrategy::calcCheckpointingNetworking(const ExecutionNodePtr& executionNode, const FaultToleranceConfigurationPtr& ftConfig, const TopologyPtr topology){
            ...
            float result =  (newUsedBand / topNode->getAvailableBandwidth()) * checkpointingNetworkingFormula;

            for(auto& parent : executionNode->getParents()){
                ExecutionNodePtr parentNode = parent->as<ExecutionNode>();
                if(parentNode->getId() != 1){
                    result += calcCheckpointingNetworking(parent->as<ExecutionNode>(), ftConfig, topology);
                }
            }
            return result;
        }
        ...
        double BasePlacementStrategy::calcCheckpointingCost(const ExecutionNodePtr& executionNode, std::vector<ExecutionNodePtr> executionNodes, const FaultToleranceConfigurationPtr& ftConfig, const TopologyPtr& topology){
            ...
            std::transform(executionNodes.begin(), executionNodes.end(), std::back_inserter(allCheckpointingNetworkingCosts),
                           [ftConfig, topology](const ExecutionNodePtr& executionNode) { return calcCheckpointingNetworking(executionNode, ftConfig, topology); });
            ...
            double checkpointingCost = calcExecutionNodeNormalizedFTCost(calcCheckpointingProcessing(executionNode, ftConfig->getQueryId()),
                                                                         calcCheckpointingNetworking(executionNode, ftConfig, topology),
 ...
 */
        ...
        bool BasePlacementStrategy::checkActiveStandbyConstraints(const ExecutionNodePtr& executionNode, const FaultToleranceConfigurationPtr& ftConfig,
            ...
            float activeStandbyNetworkConnectivityInSeconds = getNetworkConnectivity(topology, executionNode) / 1000;
