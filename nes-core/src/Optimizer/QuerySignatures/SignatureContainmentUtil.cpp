...
SignatureContainmentUtil::checkContainmentRelationshipTopDown(const LogicalOperatorNodePtr& leftOperator,
    ...
    if (leftOperator->instanceOf<WatermarkAssignerLogicalOperatorNode>()
        ...
        NES_TRACE2("Watermark assigner detected. Skipping containment check.");
        return {ContainmentType::NO_CONTAINMENT, {}};
    }
    ContainmentType containmentRelationship = ContainmentType::NO_CONTAINMENT;
    ...
    try {
        ...
        NES_TRACE2("Left signature: {}", otherConditions->to_string());
        auto conditions = rightOperator->getZ3Signature()->getConditions();
        NES_TRACE2("Right signature: {}", conditions->to_string());
        if (!conditions || !otherConditions) {
            NES_WARNING("Can't obtain containment relationships for null signatures");
            return {};
        }
...
ContainmentType SignatureContainmentUtil::checkFilterContainment(const QuerySignaturePtr& leftSignature,
    ...
    NES_TRACE2("content of left sig expression vectors: {}", leftQueryFilterConditions.to_string());
    NES_TRACE2("content of right sig expression vectors: {}", rightQueryFilterConditions.to_string());
    ...
    if (checkContainmentConditionsUnsatisfied(leftQueryFilterConditions, rightQueryFilterConditions)) {
        if (checkContainmentConditionsUnsatisfied(rightQueryFilterConditions, leftQueryFilterConditions)) {
            NES_TRACE2("Equal filters.");
            return ContainmentType::EQUALITY;
        }
        NES_TRACE2("left sig contains right sig for filters.");
        return ContainmentType::RIGHT_SIG_CONTAINED;
    } else if (checkContainmentConditionsUnsatisfied(rightQueryFilterConditions, leftQueryFilterConditions)) {
        NES_TRACE2("right sig contains left sig for filters.");
        return ContainmentType::LEFT_SIG_CONTAINED;
    }
    return ContainmentType::NO_CONTAINMENT;
}
...
std::vector<LogicalOperatorNodePtr> SignatureContainmentUtil::createFilterOperators(const LogicalOperatorNodePtr& container,
    ...
    NES_DEBUG2("Check if filter containment is possible for container {}, containee {}.",
               container->toString(),
               containee->toString());
    ...
    try {
        ...
        NES_WARNING2("Filter extraction failed: {}", e.what());
        return {};
    }
}
...
bool SignatureContainmentUtil::mapPredicateSupstitutionSuccessful(FilterLogicalOperatorNodePtr const& filterOperator,
    ...
    NES_DEBUG2("Create an iterator for traversing the filter {} predicates {}, and check output schema {}",
...
std::pair<std::string, ExpressionNodePtr> SignatureContainmentUtil::getFieldNameUsedByMapOperator(const NodePtr& node) {
    ...
    return std::pair<std::string, ExpressionNodePtr>(field->getFieldName(), mapExpression->getAssignment());
}
    ...
    for (auto schemaFieldToExprMap : schemaFieldToExprMaps) {
        ...
        for (auto otherSchemaMapItr = otherSchemaFieldToExprMaps.begin(); otherSchemaMapItr != otherSchemaFieldToExprMaps.end();
            ...
            for (uint64_t index = 0; index < containedAttributes.size(); index++) {
                ...
                NES_TRACE2("Equivalence check for transformations: {} on attribute {}",
        ...
        if (!schemaMatched) {
            NES_TRACE2("Both signatures have different transformations.");
            return false;
        }
    }
    return true;
...
bool SignatureContainmentUtil::checkDownstreamOperatorChainForSingleParent(
    ...
    NES_INFO2("Extracted contained operator: {}", extractedContainedOperator->toString());
