...
void FaultToleranceBasedQueryMergerRule::checkFaultTolerance (GlobalQueryPlanPtr globalQueryPlan, std::map<LogicalOperatorNodePtr, LogicalOperatorNodePtr> matchedTargetToHostOperatorMap){

    for(auto& mapEntry : matchedTargetToHostOperatorMap){
        NES_INFO("FIRST: " + mapEntry.first->getChildWithOperatorId(mapEntry.first->getId())->toString())
        NES_INFO("SECOND: " + mapEntry.second->toString())

        GlobalExecutionPlan gep;



        NES_INFO("SIZZY: " + std::to_string(gep.getExecutionNodesByQueryId(globalQueryPlan->getQueryPlansToAdd()[0]->getQueryId()).size()))

        NES_INFO("OpNo: " + mapEntry.first->as<OperatorNode>()->toString())

        for(auto& rootNode : mapEntry.first->getAllRootNodes()){
            NES_INFO("ROOTY: " + rootNode->toString())
        }



        //ExecutionNodePtr en = mapEntry.first->as<ExecutionNode>();
        //NES_INFO("EXXY: " + std::to_string(en->getId()))
...
bool FaultToleranceBasedQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    auto queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
        if (queryPlansToAdd.empty()) {
            NES_WARNING("FaultToleranceBasedQueryMergerRule: Found no new query metadata in the global query plan."
                            " Skipping the Signature Based Equal Query Merger Rule.");
                return true;
            }

            NES_DEBUG("FaultToleranceBasedQueryMergerRule: Iterating over all Shared Query MetaData in the Global Query Plan");
            ...
            for (auto& targetQueryPlan : queryPlansToAdd) {
                ...
                for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
                    ...
                    checkFaultTolerance(globalQueryPlan, matchedTargetToHostOperatorMap);
                ...
                if (!merged) {
                    NES_DEBUG("FaultToleranceBasedQueryMergerRule: computing a new Shared Query Plan");
...
FaultToleranceBasedQueryMergerRule::areQueryPlansEqual(const QueryPlanPtr& targetQueryPlan,
                                                                       const QueryPlanPtr& hostQueryPlan) {
            std::map<LogicalOperatorNodePtr, LogicalOperatorNodePtr> targetHostOperatorMap;
            NES_DEBUG(
                "FaultToleranceBasedQueryMergerRule: check if the target and address query plans are syntactically equal or not");
            ...
            if (targetSourceOperators.size() != hostSourceOperators.size()) {
                NES_WARNING(
                    "FaultToleranceBasedQueryMergerRule: Not matched as number of sink in target and host query plans are "
                    "different.");
                return {};
            }
...
std::map<LogicalOperatorNodePtr, LogicalOperatorNodePtr>
FaultToleranceBasedQueryMergerRule::areOperatorEqual(const LogicalOperatorNodePtr& targetOperator,
            ...
            if (targetOperator->instanceOf<SinkLogicalOperatorNode>() && hostOperator->instanceOf<SinkLogicalOperatorNode>()) {
                NES_TRACE("FaultToleranceBasedQueryMergerRule: Both target and host operators are of sink type.");
                return {};
            }

            NES_TRACE("FaultToleranceBasedQueryMergerRule: Compare target and host operators.");
            ...
            if (targetHashBased == hostOperator->getHashBasedSignature()) {
                NES_TRACE("FaultToleranceBasedQueryMergerRule: Check if parents of target and address operators are equal.");
            ...
            NES_WARNING("FaultToleranceBasedQueryMergerRule: Target and host operators are not matched.");
            return {};
        }
}
// namespace NES::Optimizer