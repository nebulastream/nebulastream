...
conntest-harness?" pre-flight (now probed via a stepper BIND).
"""
...
from conntest_runner.config_rewrite import apply_overrides, single_table_of, value_of
...
def render_config(
    ...
    *,
    ...
    Swap the loader's dynamic keys by key, leaving every other line as authored.
    A source's authored ``query`` names the table it reads; that one table is
    repointed to the per-test ``target`` so the projection, predicate, and
    ``ORDER BY`` are preserved verbatim.
    """
    loader = plugin.loader_cls(endpoint)
    text = config_path.read_text(encoding="utf-8")
    overrides = dict(loader.config_overrides(target=target, endpoint=endpoint))
    pristine_query = value_of(text, "query")
    if pristine_query is not None and "query" not in overrides:
        original_table = single_table_of(pristine_query)
        overrides["query"] = re.sub(rf"\b{re.escape(original_table)}\b", target, pristine_query)
    rendered = apply_overrides(text, overrides)
...
@cache
def _unknown_type_codes() -> frozenset[int]:
    """Codes a BIND returns when the connector's TYPE isn't compiled in.

    Resolved lazily (the .inc parse happens on first call, then caches) so
    importing this helper module touches no filesystem.
    """
    return frozenset({resolve_code("UnknownSourceType"), resolve_code("UnknownSinkType")})
...
def probe_registered(harness: Path, plugin: PluginEntry) -> bool:
    ...
    Drives a stepper BIND on a pristine valid config: an unregistered
    type binds to UnknownSource/SinkType; a registered type binds clean
