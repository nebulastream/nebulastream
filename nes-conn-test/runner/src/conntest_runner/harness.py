...
@dataclass(frozen=True)
class FillResult:
    """The result of a source FILL step.
...
class _Stepper:
    ...
    def kill(self) -> None:
        """SIGKILL the harness process; the next command respawns a fresh one.

        Models an ungraceful worker death — no graceful CLOSE/STOP, the backend
        sees the connection drop. Nulling ``_proc`` is what arms the respawn:
        the next ``_command`` finds ``_proc is None`` and ``_spawn``s a new
        process that connect()s to the still-listening control socket (this is
        the "reconnect-after-death" path the class docstring describes).
        """
        if self._proc is not None:
            self._proc.kill()
            self._proc = None
...
class Source(_Stepper):
    """Source-mode stepper: BIND | OPEN | FILL <n> | CLOSE."""
    ...
    def bind(self) -> Got:
        """Run the BIND step (bind/validate the config; reply carries row_width)."""
        return self._command("BIND")

    def open(self) -> Got:
        """Run the OPEN step (open the source connection)."""
        return self._command("OPEN")

    def fill(self, n_rows: int, n_bytes: int) -> FillResult:
        """Run a FILL step, returning the reply + observed bytes.

        Sends both measures of the seeded batch — the row count and the JSONL byte
        length; the harness compares its observed count against the one matching its
        native unit (tuples for a NATIVE source, bytes for an opaque one).
        """
        got = self._command(f"FILL {n_rows} {n_bytes}")
...
class Sink(_Stepper):
    """Sink-mode stepper: BIND | START | WRITE <n> | STOP."""
    ...
    def start(self, start_offset: int = 0) -> Got:
        """Run the START step (open the sink connection).

        The reply's ``n_buffers`` is how many buffers the harness materialized
        from the input — the total, independent of ``start_offset``.

        ``start_offset`` positions the WRITE cursor past the buffers a prior
        (crashed) session already committed, so a respawned sink resumes the
        tail instead of re-writing the whole input — the sink-side analog of a
        source naturally resuming its backend read position. Default 0 writes
        the whole input; the wire stays a bare ``START`` in that case.
        """
        return self._command("START" if start_offset == 0 else f"START {start_offset}")
