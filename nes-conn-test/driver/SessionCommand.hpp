...
/// ("BIND", "WRITE", ...). The verb is everything up to the first space.
...
/// Parse the two unsigned arguments of a `VERB <a> <b>` command line. Returns
/// nullopt when either is missing or is not a plain non-negative integer. Used by
/// source FILL, which carries both measures of the seeded data (the row count and
/// the JSONL byte length) so the harness can pick the one matching its native unit.
std::optional<std::pair<std::uint64_t, std::uint64_t>> twoArgsOf(std::string_view line);
