...
///
///   VALIDATE | START | WRITE <n> | STOP
///
/// The config is bound once (lazily) and cached; START lowers the
/// descriptor to a fresh Sink and start()s it, WRITE packs and concurrently
/// drains the next n units of the input through the sink, STOP runs the
/// repeat-aware stop loop. Each command produces one JSON reply line; EOF
/// ends the session.
