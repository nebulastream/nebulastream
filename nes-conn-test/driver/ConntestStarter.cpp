...
void configureArgumentParser(argparse::ArgumentParser& program)
{
    ...
    /// Source-mode options.
    program.add_argument("--observed-path").help("write the bytes observed per FILL/DRAIN here (RowModel decode)");
    program.add_argument("--native-row-width").help("native tuple width for NATIVE-formatter sources");
    /// Sink-mode options.
...
int runSource(const argparse::ArgumentParser& program, ControlChannel& channel)
{
    ...
    if (program.present<std::string>("--native-row-width"))
    {
        std::size_t nativeRowWidth = 0;
        if (not applyUnsigned(program, "--native-row-width", nativeRowWidth))
        {
            return 2;
        }
        options.nativeRowWidth = nativeRowWidth;
    }
