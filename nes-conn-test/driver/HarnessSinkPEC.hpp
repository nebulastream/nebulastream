...
public:
    ...
    void repeatTask(const TupleBuffer& buffer, std::chrono::milliseconds retryInterval) override
    {
        if (!buffer)
        {
        ...
        /// Valid buffer: re-enqueue it for another drain attempt (execute()). Honor the sink's
        /// retry interval first, so a sustained-Full broker throttles to one retry per interval
        /// instead of a hot publish -> Full -> re-enqueue spin that pegs a core. The interval is
        /// short (e.g. 10ms) so the global watchdog still bounds total drain time.
