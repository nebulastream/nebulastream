...
using BoundStatementContexts
    = std::vector<NES::AntlrSQLQueryParser::ManagedAntlrParser::ManagedContext<AntlrSQLParser::StatementContext>>;
...
std::optional<SourceDescriptor> bindSourceStatements(const std::string& sql, Report& report)
{
    ...
    for (const auto& statementContext : *statements)
    {
        ...
        if (auto* phys = std::get_if<CreatePhysicalSourceStatement>(&*bound))
        {
            ...
            {
                captureHarness(
                    report, "descriptor", "HarnessProtocol", "physical source statement is missing SET (...) `SOURCE`.HOST clause");
                return std::nullopt;
            }
            try
            {
                auto created = sourceCatalog->addPhysicalSource(
                    *logicalSource, phys->sourceType, *phys->host, phys->sourceConfig, phys->parserConfig);
...
std::optional<SinkDescriptor> bindSinkStatements(const std::string& sql, Report& report)
{
    ...
    try
    {
        /// The descriptor's host is a placement hint the sink connectors ignore (they read
        /// their endpoint from config), so default it when the CREATE SINK omits SINK.host.
        auto created = sinkCatalog->addSinkDescriptor(
            sink->name,
            sink->schema,
            sink->sinkType,
            sink->host.value_or(Host{std::string_view{"conntest"}}),
            sink->sinkConfig,
            sink->formatConfig);
