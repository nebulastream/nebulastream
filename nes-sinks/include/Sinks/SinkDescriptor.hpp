...
struct SinkDescriptor : Configurations::Descriptor
{
    ...
    explicit SinkDescriptor(
        std::shared_ptr<Schema>, std::string sinkType, Configurations::DescriptorConfig::Config&& config, bool addTimestamp);

    ~SinkDescriptor() = default;

    /// Iterates over all config pairs to create a DescriptorConfig::Config containing only strings.
    static std::unique_ptr<SinkDescriptor> create(std::string sinkType, std::unordered_map<std::string, std::string> configPairs);
