...
using AnonymousSinkSchema = std::variant<std::monostate, std::shared_ptr<const Schema<UnqualifiedUnboundField, Unordered>>, std::shared_ptr<const Schema<UnqualifiedUnboundField, Ordered>>>;
...
public:
    ...
    static inline const ConfigField<bool> ADD_TIMESTAMP{
        "ADD_TIMESTAMP", [](const ConfigLiteral& literal) { return tryGetOr<bool>(literal, expectedType<bool>()); }, false};
