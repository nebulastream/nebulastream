...
public:
    std::optional<SinkDescriptor> addSinkDescriptor(
        Identifier sinkName, const Schema<UnqualifiedUnboundField, Ordered>& schema, const Identifier& sinkType, std::unordered_map<Identifier, std::string> config);

    std::optional<SinkDescriptor> getSinkDescriptor(const Identifier& sinkName) const;

    [[nodiscard]] std::optional<SinkDescriptor>
    getInlineSink(const std::optional<Schema<UnqualifiedUnboundField, Ordered>>& schema, const Identifier& sinkType, std::unordered_map<Identifier, std::string> config) const;
