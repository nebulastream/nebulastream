...
use crate::context::{SinkContext, SinkMessage};
...
pub fn spawn_sink(
    ...
    let runtime = nes_source_runtime::runtime::io_runtime();
...
mod tests {
    ...
    async fn spawn_sink_runs_to_completion() {
        ...
        assert!(crate::handle::is_sink_done(&handle));
    ...
    async fn stop_sink_sends_close() {
        ...
        // Stop the sink
        let sent = crate::handle::stop_sink(&handle);
        assert!(sent);
