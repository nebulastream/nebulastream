    ...
    std::string sinkName, const Schema& schema, const std::string_view sinkType, std::unordered_map<std::string, std::string> config)
{
    ...
    auto schemaCopy = std::make_shared<Schema>(schema);
    ...
    auto sinkDescriptor = Sinks::SinkDescriptor(sinkName, std::move(schemaCopy), sinkType, std::move(descriptorConfigOpt.value()));
