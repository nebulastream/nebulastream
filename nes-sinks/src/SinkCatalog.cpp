    ...
    const std::unordered_map<Identifier, std::string>& formatConfig)
{
    ...
    auto sinkDescriptor
        = SinkDescriptor{NamedSinkDescriptor{sinkName, schema, sinkType.asCanonicalString(), std::move(host), formatConfig, std::move(descriptorConfigOpt.value())}};
    ...
    const std::unordered_map<Identifier, std::string>& formatConfig) const
{
    ...
    auto sinkDescriptor = SinkDescriptor{
        InlineSinkDescriptor{inlineSinkId.getRawValue(), schemaVar, sinkType.asCanonicalString(), std::move(host), formatConfig, std::move(descriptorConfigOpt.value())}};
