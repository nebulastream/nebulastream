...
void TCPSink::execute(const Memory::TupleBuffer& inputTupleBuffer, Runtime::Execution::PipelineExecutionContext&)
{
    ...
    assert(write(*socket, buffer.getBuffer<uint8_t>() + sizeof(uint32_t), size) == size);
}
...
std::unique_ptr<Configurations::DescriptorConfig::Config> TCPSink::validateAndFormat(std::unordered_map<std::string, std::string>&& config)
{
...
SinkValidationGeneratedRegistrar::RegisterTCPSinkValidation(SinkValidationRegistryArguments sinkConfig)
{
    return TCPSink::validateAndFormat(std::move(sinkConfig.config));
}

std::unique_ptr<SinkRegistryReturnType> SinkGeneratedRegistrar::RegisterTCPSink(SinkRegistryArguments sinkRegistryArguments)
{
