...
{
    return std::visit(
        [](const auto& var) -> const PluginSinkConfiguration& { return var.getPluginSinkConfiguration(); }, underlying);
}

const OutputFormatterDescriptor& SinkDescriptor::getOutputFormatterDescriptor() const
{
    return std::visit(
        [](const auto& var) -> const OutputFormatterDescriptor& { return var.getOutputFormatterDescriptor(); }, underlying);
}
...
{
    auto [name, schema, host, addTimestamp, backpressureUpperThreshold, backpressureLowerThreshold, pluginSinkConfig, outputFormatterDescriptor]
