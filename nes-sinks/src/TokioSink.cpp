...
addHeader(DescriptorConfig::Config config, const std::shared_ptr<const Schema<UnqualifiedUnboundField, Ordered>>& schema)
{
    config.insert_or_assign("HEADER", SchemaFormatter(schema).getFormattedSchema());
    config.insert_or_assign("TUPLE_SIZE", std::to_string(schema->getSizeInBytes()));
    return config;
}
...
struct TokioSinkContext
{
    ...
    std::shared_ptr<folly::Synchronized<std::optional<std::string>>> asyncError
        = std::make_shared<folly::Synchronized<std::optional<std::string>>>();
    ...
    TokioSinkContext(const QueryId& queryId, PipelineId pipelineId, const std::string& sinkType, const SinkDescriptor& descriptor)
        ...
        , handle(create_handle(
              QueryContext{
                  ...
                  .sink_type = sinkType,
                  .sink_config = configToFlatJson(addHeader(
                      descriptor.getConfig(),
                      NES::get<std::shared_ptr<const Schema<UnqualifiedUnboundField, Ordered>>>(descriptor.getSchema())))},
              context))
    {
    }
