...
UncompiledSTBuffer::WithoutDelimiterSearchResult UncompiledSTBuffer::searchAndTryClaimWithoutDelimiter(const SequenceNumber sequenceNumber)
{
...
void UncompiledSTBuffer::claimSTBuffers(const SequenceNumber stStartSn, const std::span<UncompiledStagedBuffer> spanningTupleBuffers)
{
...
UncompiledSTBuffer::tryFindLeadingSTForBufferWithDelimiter(const SequenceNumber sequenceNumber, const UncompiledStagedBuffer& indexedRawBuffer)
{
    ...
    {
        return UncompiledSequenceShredderResult{.isInRange = false, .spanningBuffers = {}};
    }
...
UncompiledSpanningBuffers UncompiledSTBuffer::tryFindTrailingSTForBufferWithDelimiter(const SequenceNumber sequenceNumber)
{
    ...
    if (firstSTStartBuffer.has_value())
    {
        ...
        return UncompiledSpanningBuffers(std::move(spanningTupleBuffers));
    }
    return UncompiledSpanningBuffers{};
}

UncompiledSequenceShredderResult
UncompiledSTBuffer::tryFindSTForBufferWithoutDelimiter(const SequenceNumber sequenceNumber, const UncompiledStagedBuffer& indexedRawBuffer)
{
    /// Try to set 'indexedRawBuffer' at corresponding index in buffer. Return as 'out-of-range' buffer, if setting the buffer fails
    if (const auto [bufferIdx, abaItNumber] = getBufferIdxAndABAItNo(sequenceNumber);
...
bool UncompiledSTBuffer::validate() const
{
    bool isValid = true;
    ...
    for (const auto& [idx, metaData] : buffer | NES::views::enumerate)
    {
        isValid &= metaData.validateFinalState(
            static_cast<UncompiledSTBufferIdx>(idx), buffer.at((idx + 1) % buffer.size()), static_cast<UncompiledSTBufferIdx>(lastIdxOfRB));
    }
    return isValid;
}

std::ostream& operator<<(std::ostream& os, const UncompiledSTBuffer& sequenceRingBuffer)
{
    return os << fmt::format("RingBuffer({})", sequenceRingBuffer.buffer.size());
}

std::pair<UncompiledSTBufferIdx, UncompiledABAItNo> UncompiledSTBuffer::getBufferIdxAndABAItNo(const SequenceNumber sequenceNumber) const
{
    const auto sequenceNumberBufferIdx = UncompiledSTBufferIdx{static_cast<uint32_t>(sequenceNumber.getRawValue() % buffer.size())};
    const auto abaItNumber = static_cast<UncompiledABAItNo>((sequenceNumber.getRawValue() / buffer.size()) + 1);
...
std::optional<size_t> UncompiledSTBuffer::searchLeading(const UncompiledSTBufferIdx searchStartBufferIdx, const UncompiledABAItNo abaItNumber) const
{
    ...
    auto entryState = this->buffer[(searchStartBufferIdxV - leadingDistance) % buffer.size()].getEntryState(
        static_cast<UncompiledABAItNo>(abaItNumber.getRawValue() - static_cast<size_t>(isPriorIteration)));
...
std::optional<size_t> UncompiledSTBuffer::searchTrailing(const UncompiledSTBufferIdx searchStartBufferIdx, const UncompiledABAItNo abaItNumber) const
{
    ...
    auto entryState = this->buffer[(searchStartBufferIdxV + trailingDistance) % buffer.size()].getEntryState(
        static_cast<UncompiledABAItNo>(abaItNumber.getRawValue() + static_cast<size_t>(isNextIteration)));
    while (entryState.hasCorrectABA and not(entryState.hasDelimiter))
    {
        ++trailingDistance;
...
UncompiledSTBuffer::ClaimedSpanningTuple UncompiledSTBuffer::claimingLeadingDelimiterSearch(const SequenceNumber stEndSN)
{
    ...
    if (const auto leadingDistance = searchLeading(stEndBufferIdx, stEndAbaItNo))
    {
        ...
        return ClaimedSpanningTuple{
            .firstBuffer = buffer[stStartBufferIdx].tryClaimSpanningTuple(
                static_cast<UncompiledABAItNo>(stEndAbaItNo.getRawValue() - static_cast<size_t>(isPriorIteration))),
...
UncompiledSTBuffer::ClaimedSpanningTuple UncompiledSTBuffer::claimingTrailingDelimiterSearch(const SequenceNumber stStartSN)
{
