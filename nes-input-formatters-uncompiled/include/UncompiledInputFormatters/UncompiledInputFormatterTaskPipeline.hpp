...
/// Type-erased wrapper around UncompiledInputFormatterTask
class UncompiledInputFormatterTaskPipeline final : public ExecutablePipelineStage
{
public:
    template <typename T>
    requires(not std::same_as<std::decay_t<T>, UncompiledInputFormatterTaskPipeline>)
    explicit UncompiledInputFormatterTaskPipeline(T&& inputFormatterTask)
        : inputFormatterTask(std::make_unique<InputFormatterTaskModel<T>>(std::forward<T>(inputFormatterTask)))
    {
    }

    ~UncompiledInputFormatterTaskPipeline() override = default;

    void start(PipelineExecutionContext&) override;
    /// Attempts to flush out a final (spanning) tuple that ends in the last byte of the last seen raw buffer.
    void stop(PipelineExecutionContext&) override;
    /// (concurrently) executes an UncompiledInputFormatterTask.
    /// First, uses the concrete UncompiledInputFormatIndexer implementation to determine the indexes of all fields of all full tuples.
    /// Second, uses the UncompiledSequenceShredder to find spanning tuples.
    ...
    /// Describes what a UncompiledInputFormatterTask that is in the UncompiledInputFormatterTaskPipeline does (interface).
    struct InputFormatterTaskConcept
    {
        virtual ~InputFormatterTaskConcept() = default;
        virtual void startTask() = 0;
        virtual void stopTask() = 0;
        virtual void executeTask(const UncompiledRawTupleBuffer& rawTupleBuffer, PipelineExecutionContext& pec) = 0;
    ...
    struct InputFormatterTaskModel final : InputFormatterTaskConcept
    {
        ...
        void executeTask(const UncompiledRawTupleBuffer& rawTupleBuffer, PipelineExecutionContext& pec) override
        {
...
FMT_OSTREAM(NES::UncompiledInputFormatterTaskPipeline);
