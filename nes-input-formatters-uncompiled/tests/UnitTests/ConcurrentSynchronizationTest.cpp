...
/// Tests whether the UncompiledSequenceShredder correctly finds spanning tuples given random orders of sequence numbers and random occurrences of
/// tuple delimiters in the buffers that belong to the sequence numbers.
/// Uses multiple threads that call the UncompiledSequenceShredder to determine spanning tuples. Each thread randomly (seeded) determines whether its current
/// request has a tuple delimiter or not, calls the 'processSequenceNumber' function of the UncompiledSequenceShredder and tracks the resulting spanning tuples.
...
{
    using UncompiledSequenceShredder = NES::UncompiledSequenceShredder;
...
public:
    ...
    public:
        TestThreadPool(
            const NES::UncompiledSequenceNumberType upperBound,
            const std::optional<NES::UncompiledSequenceNumberType> fixedSeed,
            const NES::TupleBuffer& dummyBuffer)
            : sequenceShredder(UncompiledSequenceShredder{1}), currentSequenceNumber(1), completionLatch(NUM_THREADS)
        {
            for (size_t i = 0; i < NUM_THREADS; ++i)
            {
                ...
                else
                {
                    ...
                    const NES::UncompiledSequenceNumberType randomSeed = dis(seedGen);
        ...
        [[nodiscard]] NES::UncompiledSequenceNumberType getCheckSum() const
        {
            NES::UncompiledSequenceNumberType globalCheckSum = 0;
            for (size_t i = 0; i < NUM_THREADS; ++i)
            {
    ...
    private:
        UncompiledSequenceShredder sequenceShredder;
        std::atomic<size_t> currentSequenceNumber;
        std::atomic<NES::UncompiledSequenceNumberType> indexOfLastDetectedTupleDelimiter;
        std::array<std::jthread, NUM_THREADS> threads;
        std::array<NES::UncompiledSequenceNumberType, NUM_THREADS> threadLocalCheckSum;
            ...
            const NES::TupleBuffer& dummyBuffer)
        {
                 ...
                 threadLocalSequenceNumber = currentSequenceNumber.fetch_add(1))
            {
                ...
                /// of the UncompiledStagedBuffer, which we create during each iteration and which is not manipulated by other threads
                const auto dummyStagedBuffer
                    = NES::UncompiledStagedBuffer{NES::UncompiledRawTupleBuffer{dummyBuffer}, 0, static_cast<uint32_t>(threadLocalSequenceNumber)};
                if (tupleDelimiter)
                {
                    NES::UncompiledSequenceShredderResult leadingSTResult
                ...
                {
                    NES::UncompiledSequenceShredderResult result
    ...
    template <size_t NUM_THREADS>
    static void executeTest(const uint32_t upperBound, const std::optional<NES::UncompiledSequenceNumberType> fixedSeed)
    {
