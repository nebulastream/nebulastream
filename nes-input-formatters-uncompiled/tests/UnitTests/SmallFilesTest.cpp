...
class SmallFilesTest : public Testing::BaseUnitTest
{
    struct TestFile
    {
        ...
        std::vector<UncompiledInputFormatterTestUtil::TestDataTypes> schemaFieldTypes;
        std::vector<std::string> schemaFieldNames;
    };

    using enum UncompiledInputFormatterTestUtil::TestDataTypes;
...
public:
    ...
    SetupResult setupTest(const TestConfig& testConfig, UncompiledInputFormatterTestUtil::ThreadSafeVector<TupleBuffer>& rawBuffers)
    {
        const auto currentTestFile = testFileMap.at(testConfig.testFileName);
        const auto schema = UncompiledInputFormatterTestUtil::createSchema(currentTestFile.schemaFieldTypes, currentTestFile.schemaFieldNames);
        ...
        const auto fileSource = UncompiledInputFormatterTestUtil::createFileSource(
            sourceCatalog, testFilePath, schema, std::move(sourceBufferPool), numberOfRequiredSourceBuffers);
        fileSource->start(UncompiledInputFormatterTestUtil::getEmitFunction(rawBuffers));
        ...
        BufferManager& testBufferManager)
    {
        ...
        return UncompiledInputFormatterTestUtil::compareTestTupleBuffersOrderSensitive(resultBufferVec, expectedBuffers, setupResult.schema);
    }
    ...
    void runTest(const TestConfig& testConfig)
    {
        ...
        UncompiledInputFormatterTestUtil::ThreadSafeVector<TupleBuffer> rawBuffers;
        ...
        for (size_t i = 0; i < testConfig.numberOfIterations; ++i)
        {
            ...
            auto inputFormatterTask = UncompiledInputFormatterTestUtil::createInputFormatterTask(setupResult.schema, testConfig.formatterType);
...
TEST_F(SmallFilesTest, testTwoIntegerColumnsNoSpanningBinary)
{
    runTest(TestConfig{
        ...
        /// Only one iteration possible, because the UncompiledInputFormatterTask replaces the number of bytes with the number of tuples in a
