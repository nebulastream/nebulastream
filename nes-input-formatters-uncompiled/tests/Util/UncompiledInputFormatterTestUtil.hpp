...
namespace NES::UncompiledInputFormatterTestUtil
{
...
std::shared_ptr<UncompiledInputFormatterTaskPipeline> createInputFormatterTask(const Schema& schema, std::string formatterType);
...
TestPipelineTask createInputFormatterTask(
    ...
    std::shared_ptr<UncompiledInputFormatterTaskPipeline> inputFormatterTask);
...
{
    UncompiledInputFormatterTestUtil::sortTupleBuffers(actualResult);
    UncompiledInputFormatterTestUtil::sortTupleBuffers(expectedResult);
...
{
    const std::shared_ptr<UncompiledInputFormatterTaskPipeline> inputFormatterTask
        = provideUncompiledInputFormatterTask(testHandle.schema, testHandle.testConfig.parserConfig);
