...
namespace NES
{
using UncompiledSequenceNumberType = SequenceNumber::Underlying;

/// Takes a tuple buffer containing raw, unformatted data and wraps it into an object that fulfills the following purposes:
/// 1. The UncompiledRawTupleBuffer allows its users to operate on string_views, instead of handling raw pointers (which a TupleBuffer would require)
/// 2. It exposes only those functions of the TupleBuffer that are required for formatting
/// 3. It selectively exposes the reduced set of functions, to prohibit setting, e.g., the SequenceNumber in UncompiledInputFormatIndexer
/// 4. It exposes functions with more descriptive names, e.g., `getNumberOfBytes()` instead of `getNumberOfTuples`
/// 5. The type (UncompiledRawTupleBuffer) makes it clear that we are dealing with raw data and not with (formatted) tuples
class UncompiledRawTupleBuffer
{
    TupleBuffer rawBuffer;
    std::string_view bufferView;

public:
    UncompiledRawTupleBuffer() = default;
    ~UncompiledRawTupleBuffer() = default;
    explicit UncompiledRawTupleBuffer(TupleBuffer rawTupleBuffer)
        : rawBuffer(std::move(rawTupleBuffer))
        , bufferView(rawBuffer.getAvailableMemoryArea<char>().data(), rawBuffer.getNumberOfTuples()) { };


    UncompiledRawTupleBuffer(UncompiledRawTupleBuffer&& other) noexcept = default;
    UncompiledRawTupleBuffer& operator=(UncompiledRawTupleBuffer&& other) noexcept = default;
    UncompiledRawTupleBuffer(const UncompiledRawTupleBuffer& other) = default;
    UncompiledRawTupleBuffer& operator=(const UncompiledRawTupleBuffer& other) = default;
...
/// all spanning tuple(s) starting/ending in or containing the UncompiledStagedBuffer.
struct UncompiledStagedBuffer
{
private:
    friend class UncompiledSequenceShredder;
...
public:
    ...
    [[nodiscard]] const UncompiledRawTupleBuffer& getRawTupleBuffer() const { return rawBuffer; }

    void setSpanningTuple(const std::string_view spanningTuple) { rawBuffer.setSpanningTuple(spanningTuple); }


protected:
    UncompiledRawTupleBuffer rawBuffer;
