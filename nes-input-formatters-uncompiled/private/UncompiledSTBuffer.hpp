...
/// The UncompiledSTBuffer enables threads to concurrently resolve spanning tuples.
...
/// The UncompiledSTBuffer makes sure that exactly one thread processes an ST (avoiding missing an ST or producing duplicate STs)
///
/// The central abstraction that allows the UncompiledSTBuffer to thread-safely determine STs is the UncompiledAtomicState attached to each of its entries.
/// The atomic state can thread-safely denote whether an UncompiledSTBufferEntry has a delimiter, whether its buffers are used already, and
/// very importantly, which iteration the entry belongs to.
/// The UncompiledSTBuffer derives the iteration by dividing the sequence number (SN) of an incoming buffer by the size of the UncompiledSTBuffer and adding 1
/// e.g., for a buffer size of 1024 entries and an SN of 2025: 2025 / 1024 + 1 = 2 (see 'getBufferIdxAndABAItNo' of UncompiledSTBuffer)
...
/// The UncompiledSTBuffer takes a buffer and then iterates over its entries, using their atomic states to determine valid STs
/// The UncompiledSTBuffer guarantees that at least one thread determines a valid ST, because an entry can only be replaced with a new entry, if both
...
/// entries.
/// The UncompiledSTBuffer guarantees that only one thread can claim an ST, because (multiple) threads that found the same ST can only claim the ST
/// by using a CAS-loop to atomically set the 'claimedSpanningTupleBit' of the UncompiledSTBufferEntry that represents the first buffer of the ST to 1.
...
class UncompiledSTBuffer
{
    ...
    {
        std::optional<UncompiledStagedBuffer> firstBuffer;
...
public:
    ...
    {
        std::optional<UncompiledStagedBuffer> leadingSTStartBuffer = std::nullopt;
        SequenceNumber firstSequenceNumber = INVALID<SequenceNumber>;
        SequenceNumber lastSequenceNumber = INVALID<SequenceNumber>;
    };

    explicit UncompiledSTBuffer(size_t initialSize, TupleBuffer dummyBuffer);
    ...
    [[nodiscard]] UncompiledSequenceShredderResult
    tryFindLeadingSTForBufferWithDelimiter(SequenceNumber sequenceNumber, const UncompiledStagedBuffer& indexedRawBuffer);
    [[nodiscard]] UncompiledSpanningBuffers tryFindTrailingSTForBufferWithDelimiter(SequenceNumber sequenceNumber);
    ...
    [[nodiscard]] UncompiledSequenceShredderResult
    tryFindSTForBufferWithoutDelimiter(SequenceNumber sequenceNumber, const UncompiledStagedBuffer& indexedRawBuffer);
    ...
    void claimSTBuffers(SequenceNumber stStartSn, std::span<UncompiledStagedBuffer> spanningTupleBuffers);

    [[nodiscard]] bool validate() const;

    friend std::ostream& operator<<(std::ostream& os, const UncompiledSTBuffer& sequenceRingBuffer);

private:
    std::vector<UncompiledSTBufferEntry> buffer;

    [[nodiscard]] std::pair<UncompiledSTBufferIdx, UncompiledABAItNo> getBufferIdxAndABAItNo(SequenceNumber sequenceNumber) const;
    ...
    [[nodiscard]] std::optional<size_t> searchLeading(UncompiledSTBufferIdx searchStartBufferIdx, UncompiledABAItNo abaItNumber) const;
    /// Analog to 'searchLeading', but searches in trailing direction (larger SNs)
    [[nodiscard]] std::optional<size_t> searchTrailing(UncompiledSTBufferIdx searchStartBufferIdx, UncompiledABAItNo abaItNumber) const;
...
FMT_OSTREAM(NES::UncompiledSTBuffer);
