...
TEST_F(E2eRestTest, testExecutingValidUserQuery) {
    ...
    if (pidC = fork()) {
        ...
        if (pidW = fork()) {
            ...
            string body = "{\"userQuery\" : \"Schema schema = Schema::create().addField(\\\"id\\\", BasicType::UINT32).addField(\\\"value\\\", BasicType::UINT64);Stream stream = Stream(\\\"cars\\\", schema);InputQuery inputQuery = InputQuery::from(stream).writeToFile(\\\"" + outputFilePath +"\\\"); return inputQuery;\",\"strategyName\" : \"BottomUp\"}";
