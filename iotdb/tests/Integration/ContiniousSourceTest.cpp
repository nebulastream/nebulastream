...
TEST_F(ContiniousSourceTest, testMultipleOutputBufferFromDefaultSourcePrint) {
    ...
    //register query
    std::string
        queryString = "InputQuery::from(testStream).filter(testStream[\"campaign_id\"] > 42).print(std::cout); ";
    std::string id = crd->executeQuery(queryString, "BottomUp");
    EXPECT_NE(id, "");
...
TEST_F(ContiniousSourceTest, DISABLED_testExdraUseCaseWithOutput) {
    ...
    //register query
    std::string queryString = "InputQuery::from(exdra).writeToCSVFile(\"" + outputFilePath + "\"); ";
    std::string id = crd->executeQuery(queryString, "BottomUp");
    EXPECT_NE(id, "");
