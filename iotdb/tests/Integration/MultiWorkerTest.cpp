...
TEST_F(MultiWorkerTest, startWithConnectStopWorkerCoordinator) {
    ...
    cout << "start worker 2" << endl;
    NesWorkerPtr wrk2 = std::make_shared<NesWorker>("localhost", std::to_string(port), "localhost", std::to_string(port+1), NESNodeType::Sensor);
...
TEST_F(MultiWorkerTest, multiWorkerTest) {
    ...
    for (size_t i = 0; i < numWorkers; i++) {
        ...
        wPtrs.push_back(std::make_shared<NesWorker>("localhost", std::to_string(port), "localhost", std::to_string(port+i), NESNodeType::Sensor));
