...
class SelectionDataGenSource : public GeneratorSource {
 public:
  SelectionDataGenSource(const Schema &schema,
...
class PredicateTestingDataGeneratorSource : public GeneratorSource {
 public:
  PredicateTestingDataGeneratorSource(const Schema &schema,
                                      const uint64_t pNum_buffers_to_process)
      :
      GeneratorSource(schema, pNum_buffers_to_process) {
  }
  ...
  DataSourcePtr source(
      std::make_shared<PredicateTestingDataGeneratorSource>(
          Schema::create().addField("id", BasicType::UINT32).addField(
              ...
              "valueChar", BasicType::CHAR).addField(
              "text", createArrayDataType(BasicType::CHAR, 12)),
          1));
...
class WindowTestingDataGeneratorSource : public GeneratorSource {
 public:
  WindowTestingDataGeneratorSource(const Schema &schema,
                                   const uint64_t pNum_buffers_to_process)
      :
      GeneratorSource(schema, pNum_buffers_to_process) {
  }
...
TEST_F(CodeGenerationTest, codeGenRunningSum) {
   ...
   */
  auto structDeclTupleBuffer = StructDeclaration::create("TupleBuffer", "")
      .addField(
      VariableDeclaration::create(
   ...
   */
  auto structDeclState = StructDeclaration::create("WindowState", "").addField(
      VariableDeclaration::create(
          createPointerDataType(createDataType(BasicType(VOID_TYPE))),
          "windowState"));
  ...
  auto declFieldCampaignId = structDeclTuple.getVariableDeclaration(
      "campaign_id");
  auto declFieldNumTuplesStructTupleBuffer = structDeclTupleBuffer
      .getVariableDeclaration("num_tuples");
  auto declFieldDataPtrStructTupleBuf = structDeclTupleBuffer
      .getVariableDeclaration("data");
  auto varDeclFieldResultTupleSum =
      structDeclResultTuple.getVariableDeclaration("sum");
  ...
  BinaryOperatorStatement initTupleBufferPtr(
      VarRefStatement(varDeclTupleBuffer1).assign(
          VarRefStatement(varDeclTupleBuffers)));
  /*  tuples = (Tuple *)tuple_buffer_1->data;*/
  BinaryOperatorStatement initTuplePtr(
      VarRef(varDeclTuple).assign(
          TypeCast(
              VarRefStatement(varDeclTupleBuffer1).accessPtr(
                  VarRef(declFieldDataPtrStructTupleBuf)),
  ...
  BinaryOperatorStatement initResultTuplePtr(
      VarRef(varDeclResultTuple).assign(
          TypeCast(
              VarRef(varDeclTupleBufferOutput).accessPtr(
                  VarRef(declFieldDataPtrStructTupleBuf)),
  ...
  FOR loopStmt(
      varDeclId,
      (VarRef(varDeclId)
          < (VarRef(varDeclTupleBuffer1).accessPtr(
              VarRef(declFieldNumTuplesStructTupleBuffer)))),
      ++VarRef(varDeclId));
  ...
  auto mainFunction =
      FunctionBuilder::create("compiled_query").returns(
          createDataType(BasicType(UINT32))).addParameter(varDeclTupleBuffers)
          .addParameter(varDeclWindow).addParameter(varDeclWindowManager)
          .addParameter(varDeclTupleBufferOutput).addVariableDeclaration(
          varDeclReturn).addVariableDeclaration(varDeclTuple)
          .addVariableDeclaration(varDeclResultTuple).addVariableDeclaration(
          varDeclTupleBuffer1).addVariableDeclaration(varDeclSum).addStatement(
          initTupleBufferPtr.copy()).addStatement(initTuplePtr.copy())
  ...
  auto file = FileBuilder::create("query.cpp").addDeclaration(
      structDeclTupleBuffer).addDeclaration(structDeclState).addDeclaration(
      structDeclTuple).addDeclaration(structDeclResultTuple).addDeclaration(
      mainFunction).build();
...
TEST_F(CodeGenerationTest, codeGenerationFilterPredicate) {
  ...
  auto resultData = (SelectionDataGenSource::InputTuple*) resultBuffer
      ->getBuffer();
...
TEST_F(CodeGenerationTest, codeGenerationStringComparePredicateTest) {
  ...
  codeGenerator->generateCode(
      createPredicate(
          (inputSchema[2] > 30.4)
              && (inputSchema[4] == 'F' || (inputSchema[5] == "HHHHHHHHHHH"))),
      context, std::cout);
...
TEST_F(CodeGenerationTest, codeGenerationMapPredicateTest) {
  ...
  codeGenerator->generateCode(
      mappedValue, createPredicate((inputSchema[2] * inputSchema[3]) + 2),
      context, std::cout);
