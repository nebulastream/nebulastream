...
TEST_F(ActorsCliTest, testConnectDisconnect) {
  ...
  NES_INFO(
      "ACTORSCLITEST (testRegisterUnregisterSensor):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testRegisterUnregisterSensor): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
  ...
  bool bookKeep = false;
  self->request(worker, task_timeout, disconnect_atom::value).receive(
      [&bookKeep](const bool &c) mutable {
        bookKeep = c;
...
TEST_F(ActorsCliTest, testSpawnDespawnCoordinatorWorkers) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testSpawnDespawnCoordinatorWorkers):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testSpawnDespawnCoordinatorWorkers): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
      "ACTORSCLITEST (testSpawnDespawnCoordinatorWorkers): coordinator successfully published at port " << *expected_port);
...
TEST_F(ActorsCliTest, testShowTopology) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testShowTopology):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testShowTopology): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
...
TEST_F(ActorsCliTest, testShowRegistered) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testShowRegistered):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testShowRegistered): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
...
TEST_F(ActorsCliTest, testDeleteQuery) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testDeleteQuery):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testDeleteQuery): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
...
TEST_F(ActorsCliTest, testShowRunning) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testShowRunning):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testShowRunning): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
...
TEST_F(ActorsCliTest, testShowOperators) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testShowOperators):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordinator, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testShowOperators): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
...
TEST_F(ActorsCliTest, testSequentialMultiQueries) {
  ...
  // try to publish actor at given port
  NES_INFO(
      "ACTORSCLITEST (testSequentialMultiQueries):  try publish at port" << c_cfg.publish_port);
  auto expected_port = io::publish(coordHandler, c_cfg.publish_port);
  if (!expected_port) {
    NES_ERROR(
        "ACTORSCLITEST (testSequentialMultiQueries): publish failed: " << system_coord.render(expected_port.error()));
    return;
  }
  NES_INFO(
  ...
  for (int i = 0; i < 1; i++) {
    ...
    self->request(coordHandler, task_timeout,
                  show_running_operators_atom::value);
    self->request(coordHandler, task_timeout, deregister_query_atom::value,
                  uuid);
    self->request(coordHandler, task_timeout,
                  show_running_operators_atom::value);
  }
