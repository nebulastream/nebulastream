...
TEST_F(NesTopologyManagerTest, create_link) {
  ...
  auto link_node_sensor = NESTopologyManager::getInstance()
      .createNESTopologyLink(worker_node_2, sensor_node_0);
  ...
  auto link_sensor_node = NESTopologyManager::getInstance()
      .createNESTopologyLink(sensor_node_1, worker_node_3);
...
TEST_F(NesTopologyManagerTest, create_existing_link) {
  auto node_0 = NESTopologyManager::getInstance().createNESWorkerNode(1,
      "localhost", CPUCapacity::MEDIUM);
  auto node_1 = NESTopologyManager::getInstance().createNESWorkerNode(2,
      "localhost", CPUCapacity::MEDIUM);
  auto link = NESTopologyManager::getInstance().createNESTopologyLink(node_0,
                                                                      node_1);
  EXPECT_EQ(
      link,
      NESTopologyManager::getInstance().createNESTopologyLink(node_0, node_1));
}
...
TEST_F(NesTopologyManagerTest, remove_non_existing_link) {
  ...
  link = NESTopologyManager::getInstance().createNESTopologyLink(node_0,
                                                                 node_1);
...
TEST_F(NesTopologyManagerTest, many_nodes) {
  ...
  for (uint32_t i = 0; i < 15; ++i) {
    workers.push_back(
        NESTopologyManager::getInstance().createNESWorkerNode(i,
            "localhost", CPUCapacity::MEDIUM));
  }
...
TEST_F(NesTopologyManagerTest, many_links) {
  ...
  for (uint32_t i = 0; i != 30; ++i) {
    if (i % 2 == 0) {
      ...
      links.push_back(
          NESTopologyManager::getInstance().createNESTopologyLink(
              sensors.at(i), workers.at(i / 2)));
    } else {
      // odd sensor
      links.push_back(
          NESTopologyManager::getInstance().createNESTopologyLink(
              sensors.at(i), workers.at((i - 1) / 2)));
    }
  }
...
TEST_F(NesTopologyManagerTest, print_graph) {
  ...
  for (uint32_t i = 0; i != 15; ++i) {
    if (i % 3 == 0) {
      links.push_back(
          NESTopologyManager::getInstance().createNESTopologyLink(
              sensors.at(i), workers.at(i / 3)));
    } else if (i % 3 == 1) {
      links.push_back(
          NESTopologyManager::getInstance().createNESTopologyLink(
              sensors.at(i), workers.at((i - 1) / 3)));
    } else {
      links.push_back(
          NESTopologyManager::getInstance().createNESTopologyLink(
              sensors.at(i), workers.at((i - 2) / 3)));
    }
  }
