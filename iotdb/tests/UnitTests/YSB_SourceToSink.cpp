...
int test()
{
    ...
    while (source->isRunning() || sink->getNumberOfProcessedBuffers() != 2) {
        std::cout << "sourceRunnin=" << source->isRunning()
                  << " numberOfProcBuffer=" << sink->getNumberOfProcessedBuffers() << std::endl;
    ...
    if (ySink->getNumberOfPrintedTuples() != 36) {
        ...
        std::cout << "right result with sourceRunnin=" << source->isRunning()
                  << " numberOfProcBuffer=" << sink->getNumberOfProcessedBuffers() << std::endl;
    }
    thread_pool.stop();
