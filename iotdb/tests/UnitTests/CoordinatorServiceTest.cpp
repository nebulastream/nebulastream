...
TEST_F(CoordinatorCafTest, test_register_query) {
    string queryId = coordinatorServicePtr->register_query(queryString, "BottomUp");
...
TEST_F(CoordinatorCafTest, test_local_distributed_deployment) {
    ...
    coordinatorServicePtr->deregister_query(queryId);
    EXPECT_TRUE(
        coordinatorServicePtr->getRegisteredQueries().empty() && coordinatorServicePtr->getRunningQueries().empty());
}
...
TEST_F(CoordinatorCafTest, DISABLED_test_sequential_local_distributed_deployment) {
    ...
    for (int i = 0; i < 15; i++) {
        ...
        unordered_map<FogTopologyEntryPtr, ExecutableTransferObject>
            etos = coordinatorServicePtr->make_deployment(queryId);
        EXPECT_TRUE(etos.size() == 2);
