...
TEST_F(StreamCatalogRemoteTest, test_add_log_stream_remote_test) {
    ...
    self->request(workerHandler, task_timeout, register_log_stream_atom::value, "testStream1", testSchemaFileName)
        .receive([&registered](const bool& c) mutable {
          std::this_thread::sleep_for(std::chrono::seconds(1));
          registered = c;
        }, [=](const error& er) {
          string error_msg = to_string(er);
    ...
    SchemaPtr sPtr = StreamCatalog::instance().getSchemaForLogicalStream("testStream1");
