...
int test()
{
    ...
    while (source1->isRunning() || sink1->getNumberOfProcessedBuffers() != 1 || source2->isRunning() ||
           sink2->getNumberOfProcessedBuffers() != 1) {
        std::cout << "source1->isRunning()=" << source1->isRunning()
                  << " sink1->getNumberOfProcessedBuffers()=" << sink1->getNumberOfProcessedBuffers()
                  << " source2->isRunning()=" << source2->isRunning()
                  << " sink2->getNumberOfProcessedBuffers()=" << sink2->getNumberOfProcessedBuffers() << std::endl;
