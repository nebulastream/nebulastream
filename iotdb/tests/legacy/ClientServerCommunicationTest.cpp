...
class ClientServerCommunicationTest : public testing::Test {
 public:
  ...
  void SetUp() {
    NES::setupLogging("ClientServerCommunicationTest.log", NES::LOG_DEBUG);
    ...
    client_queries->insert(
        IotServer::client_query_t(
            (*client_queries_id)++,
            std::tuple<std::string, std::string>(
                std::string("query_0.cpp"), std::string("Content\nFile 0"))));
    client_queries->insert(
        IotServer::client_query_t(
            (*client_queries_id)++,
            std::tuple<std::string, std::string>(
                std::string("query_1.cpp"), std::string("Content\nFile 1"))));
  }
...
TEST_F(ClientServerCommunicationTest, add_query) {
  ...
  auto test_file_path = std::string(TEST_DATA_DIRECTORY).append(
      "/ClientServerCommunication_TestQuery.cpp");
  ...
  auto reply = IotServer::add_query_request(request, client_queries,
                                            client_queries_id);
  ...
  EXPECT_TRUE(
      std::get<0>((*client_queries)[2])
          == std::string("ClientServerCommunication_TestQuery.cpp"));
