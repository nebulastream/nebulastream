...
bool PipelineStage::execute(const TupleBuffer *input_buffers,
  ...
  if (ret)
    NES_ERROR("Execution of Compled PipelineStage Failed!");
...
class CPipelineStage : public PipelineStage {
 ...
 protected:
  ...
  virtual uint32_t callCFunction(TupleBuffer **tuple_buffers, void *state,
...
uint32_t CPipelineStage::execute_impl(
    const std::vector<TupleBuffer*> &input_buffers, void *state,
    WindowManager *window_manager, TupleBuffer *result_buf) {
  return callCFunction((TupleBuffer**) input_buffers.data(), state,
                       window_manager, result_buf);
}

typedef uint32_t (*SharedCLibPipelineQueryPtr)(TupleBuffer**, void*,
                                               WindowManager*, TupleBuffer*);

uint32_t CPipelineStage::callCFunction(TupleBuffer **tuple_buffers, void *state,
                                       WindowManager *window_manager,
                                       TupleBuffer *result_buffer) {
  return (*compiled_code_->getFunctionPointer<SharedCLibPipelineQueryPtr>(
      "_Z14compiled_queryPP11TupleBufferPvPN3NES13WindowManagerES0_"))(
//          "_Z14compiled_queryPP11TupleBufferPvPN5iotdb13WindowManagerES0_"))(
      tuple_buffers, state, window_manager, result_buffer);
}
...
DataSinkPiplineStage::DataSinkPiplineStage(DataSinkPtr sink)
    :
    sink(sink) {
}

uint32_t DataSinkPiplineStage::execute_impl(
    const std::vector<TupleBuffer*> &input_buffers, void *state,
    WindowManager *window_manager, TupleBuffer *result_buf) {
