...
GeneratedCode::GeneratedCode()
    ...
    var_decl_tuple_buffers(
        VariableDeclaration::create(createDataType(INT32), "input_buffers")),
    var_declare_window(
        VariableDeclaration::create(createDataType(INT32), "window")),
    var_decl_tuple_buffer_output(
        VariableDeclaration::create(createDataType(INT32), "output_buffer")),
    var_decl_state(VariableDeclaration::create(createDataType(INT32), "state")),
    decl_field_num_tuples_struct_tuple_buf(
        VariableDeclaration::create(createDataType(INT32),
                                    "decl_field_num_tuples_NOT_DEFINED")),
    decl_field_data_ptr_struct_tuple_buf(
        VariableDeclaration::create(createDataType(INT32),
                                    "decl_field_data_ptr_NOT_DEFINED")),
    var_decl_input_tuple(
        VariableDeclaration::create(createDataType(INT32),
                                    "input_tuple_NOT_DEFINED")),
    var_num_for_loop(
        VariableDeclaration::create(createDataType(UINT64),
                                    "num_result_tuples")),
    type_decls() {
}
...
CodeGenerator::CodeGenerator(const CodeGenArgs &args)
    :
    args_(args) {
}
...
class CCodeGenerator : public CodeGenerator {
 public:
  ...
  virtual bool generateCode(const DataSourcePtr &source,
                            const PipelineContextPtr &context,
                            std::ostream &out) override;
  virtual bool generateCode(const PredicatePtr &pred,
                            const PipelineContextPtr &context,
                            std::ostream &out) override;
  virtual bool generateCode(const AttributeFieldPtr field,
                            ...
                            std::ostream &out) override;
  virtual bool generateCode(const DataSinkPtr &sink,
                            const PipelineContextPtr &context,
                            std::ostream &out) override;
...
CCodeGenerator::CCodeGenerator(const CodeGenArgs &args)
    :
    CodeGenerator(args) {
}
...
const StructDeclaration getStructDeclarationTupleBuffer() {
   ...
   */
  StructDeclaration struct_decl_tuple_buffer = StructDeclaration::create(
      "TupleBuffer", "").addField(
      VariableDeclaration::create(
          createPointerDataType(createDataType(BasicType(VOID_TYPE))), "data"))
      .addField(
      VariableDeclaration::create(createDataType(BasicType(UINT64)),
                                  "buffer_size")).addField(
      VariableDeclaration::create(createDataType(BasicType(UINT64)),
                                  "tuple_size_bytes")).addField(
      VariableDeclaration::create(createDataType(BasicType(UINT64)),
                                  "num_tuples"));
...
const StructDeclaration getStructDeclarationFromSchema(
    const std::string struct_name, const Schema &schema) {
  ...
  for (size_t i = 0; i < schema.getSize(); ++i) {
    struct_decl_tuple.addField(
        VariableDeclaration::create(schema[i]->getDataType(), schema[i]->name));
    std::cout << "Field " << i << ": " << schema[i]->getDataType()->toString()
              << " " << schema[i]->name << std::endl;
  }
...
const VariableDeclarationPtr getVariableDeclarationForField(
    const StructDeclaration &struct_decl, const AttributeFieldPtr field) {
  if (struct_decl.getField(field->name))
    return std::make_shared<VariableDeclaration>(
        struct_decl.getVariableDeclaration(field->name));
  else {
...
std::string toString(TupleBuffer *buffer, const Schema &schema) {
  ...
  for (uint32_t i = 0; i < schema.getSize(); ++i) {
    ...
    NES_DEBUG(
        std::string("Field Size ") + schema[i]->toString() + std::string(": ")
  ...
  for (uint32_t i = 0; i < offsets.size(); ++i) {
    ...
    prefix_sum += val;
    NES_DEBUG(
        std::string("Prefix Sum: ") + schema[i]->toString() + std::string(": ")
...
bool CCodeGenerator::generateCode(const DataSourcePtr &source,
                                  const PipelineContextPtr &context,
                                  std::ostream &out) {
  ...
  StructDeclaration struct_decl_tuple_buffer =
      getStructDeclarationTupleBuffer();
  ...
  StructDeclaration struct_result_tuple = getStructDeclarationResultTuple(
      result_schema_);
  ...
  VariableDeclaration var_decl_tuple_buffers = VariableDeclaration::create(
      createPointerDataType(
          createPointerDataType(
              createUserDefinedType(struct_decl_tuple_buffer))),
      "input_buffer");
  VariableDeclaration var_decl_tuple_buffer_output =
      VariableDeclaration::create(
          createPointerDataType(
              createUserDefinedType(struct_decl_tuple_buffer)),
          "output_tuple_buffer");
  VariableDeclaration var_decl_window = VariableDeclaration::create(
      createPointerDataType(createAnnonymUserDefinedType("void")), "state_var");
  VariableDeclaration var_decl_window_manager = VariableDeclaration::create(
  ...
  VariableDeclaration var_decl_tuple_buffer_1 = VariableDeclaration::create(
      createPointerDataType(
          createUserDefinedType(getStructDeclarationTupleBuffer())),
      "tuple_buffer_1");
  ...
  VariableDeclaration decl_field_data_ptr_struct_tuple_buf =
      struct_decl_tuple_buffer.getVariableDeclaration("data");

  code_.decl_field_num_tuples_struct_tuple_buf =
      decl_field_num_tuples_struct_tuple_buf;
  code_.decl_field_data_ptr_struct_tuple_buf =
      decl_field_data_ptr_struct_tuple_buf;
  ...
  code_.variable_init_stmts.push_back(
      VarRefStatement(var_decl_tuple_buffer_1).assign(
          VarRefStatement(var_decl_tuple_buffers)[ConstantExprStatement(INT32,
                                                                        "0")])
          .copy());
  ...
  code_.variable_init_stmts.push_back(
      VarRef(var_decl_tuple).assign(
          TypeCast(
              VarRefStatement(var_decl_tuple_buffer_1).accessPtr(
                  VarRef(decl_field_data_ptr_struct_tuple_buf)),
              createPointerDataType(createUserDefinedType(struct_decl_tuple))))
          .copy());
  ...
  code_.for_loop_stmt = std::make_shared<FOR>(
      *(code_.var_decl_id.get()),
      (VarRef(*(code_.var_decl_id.get()))
          < (VarRef(var_decl_tuple_buffer_1).accessPtr(
              VarRef(decl_field_num_tuples_struct_tuple_buf)))),
      ++VarRef(*(code_.var_decl_id.get())));
  code_.current_code_insertion_point =
      code_.for_loop_stmt->getCompoundStatement();

  code_.return_stmt = std::make_shared<ReturnStatement>(
      VarRefStatement(*code_.var_decl_return));
 ...
 */
bool CCodeGenerator::generateCode(const PredicatePtr &pred,
                                  const PipelineContextPtr &context,
                                  std::ostream &out) {
...
bool CCodeGenerator::generateCode(const AttributeFieldPtr field,
  ...
  StructDeclaration struct_decl_result_tuple = (getStructDeclarationFromSchema(
      "result_tuples", result_schema_));
  ...
  DeclarationPtr declaredMapVar = getVariableDeclarationForField(
      code_.struct_decl_result_tuple, field);
  if (!declaredMapVar) {
    result_schema_.addField(field);
    code_.struct_decl_result_tuple.addField(
        VariableDeclaration::create(field->data_type, field->name));
    declaredMapVar = getVariableDeclarationForField(
        code_.struct_decl_result_tuple, field);
  }
  code_.override_fields.push_back(declaredMapVar);
  VariableDeclaration var_map_i = code_.struct_decl_result_tuple
      .getVariableDeclaration(field->name);

  BinaryOperatorStatement callVar = VarRef(var_decl_result_tuple)[VarRef(
      code_.var_num_for_loop)].accessRef(VarRef(var_map_i));
  ExpressionStatmentPtr expr = pred->generateCode(this->code_);
...
bool CCodeGenerator::generateCode(const DataSinkPtr &sink,
                                  const PipelineContextPtr &context,
                                  std::ostream &out) {
  ...
  VariableDeclaration var_decl_num_result_tuples = VariableDeclaration::create(
      createDataType(BasicType(UINT64)), "num_result_tuples",
  ...
  code_.variable_init_stmts.push_back(
      VarRef(var_decl_result_tuple).assign(
          TypeCast(
              VarRef(code_.var_decl_tuple_buffer_output).accessPtr(
                  VarRef(code_.decl_field_data_ptr_struct_tuple_buf)),
              createPointerDataType(
                  createUserDefinedType(struct_decl_result_tuple)))).copy());
  ...
  for (size_t i = 0; i < result_schema_.getSize(); ++i) {
    VariableDeclarationPtr var_decl = getVariableDeclarationForField(
        struct_decl_result_tuple, result_schema_[i]);
    if (!var_decl) {
      NES_ERROR(
          "Could not extract field " << result_schema_[i]->toString() << " from struct " << struct_decl_result_tuple.getTypeName());
      NES_DEBUG("W>");
    }
    code_.variable_decls.push_back(*var_decl);

    DeclarationPtr var_decl_input = getVariableDeclarationForField(
        code_.struct_decl_input_tuple, result_schema_[i]);
    if (var_decl_input) {
      bool override = false;
      for (size_t j = 0; j < code_.override_fields.size(); j++) {
        if (code_.override_fields.at(j)->getIdentifierName().compare(
            var_decl_input->getIdentifierName()) == 0) {
          override = true;
          break;
        }
      }
      if (!override) {
        AssignmentStatment as = { var_decl_result_tuple, *(var_decl),
            var_decl_num_result_tuples, code_.var_decl_input_tuple, *(var_decl),
            *(code_.var_decl_id) };
  ...
  code_.current_code_insertion_point->addStatement(
      (++VarRef(var_decl_num_result_tuples)).copy());

  /*  TODO: set number of output tuples to result buffer */
  code_.cleanup_stmts.push_back(
      VarRef(code_.var_decl_tuple_buffer_output).accessPtr(
          VarRef(code_.decl_field_num_tuples_struct_tuple_buf)).assign(
          VarRef(var_decl_num_result_tuples)).copy());
  return true;
}
...
bool CCodeGenerator::generateCode(const WindowDefinitionPtr &window,
  ...
  auto constStatement = ConstantExprStatement(
      (createBasicTypeValue(BasicType::UINT64, "0")));

  auto state_var = VariableDeclaration::create(
      createPointerDataType(
          createAnnonymUserDefinedType(
              "NES::StateVariable<int64_t, NES::WindowSliceStore<int64_t>*>")),
      "state_variable");

  auto state_var_decl = VarDeclStatement(state_var).assign(
      TypeCast(VarRef(code_.var_decl_state), state_var.getDataType()));
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(state_var_decl));

  // Read key value from record
  auto key_var = VariableDeclaration::create(createDataType(BasicType::INT64),
                                             "key");
  VariableDeclaration key_var_decl_attr = code_.struct_decl_input_tuple
      .getVariableDeclaration(window->onKey->name);
  auto key_var_decl = VarDeclStatement(key_var).assign(
      VarRef(code_.var_decl_input_tuple)[VarRef(*code_.var_decl_id)].accessRef(
          VarRef(key_var_decl_attr)));
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(key_var_decl));
  ...
  auto key_handle_dec = VarDeclStatement(key_handle_var).assign(
      VarRef(state_var).accessPtr(getKeyStateVariable));
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(key_handle_dec));
  ...
  auto window_state_decl = VarDeclStatement(window_state_var).assign(
      VarRef(key_handle_var).accessRef(getValueFromKeyHandle));
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(window_state_decl));
  ...
  auto getCurrentTsStatement = VarDeclStatement(current_time_var).assign(
      getCurrentTs);
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(getCurrentTsStatement));
  ...
  auto call = std::make_shared<BinaryOperatorStatement>(
      VarRef(code_.var_declare_window).accessPtr(sliceStream));
  ...
  auto getSliceIndexByTsCall = VarRef(window_state_var).accessPtr(
      getSliceIndexByTs);
  ...
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(var_decl_current_slice_stm));

  // get the partial aggregates
  auto getPartialAggregates = FunctionCallStatement("getPartialAggregates");
  auto getPartialAggregatesCall = VarRef(window_state_var).accessPtr(
      getPartialAggregates);
  VariableDeclaration var_decl_partial_aggregates = VariableDeclaration::create(
      createAnnonymUserDefinedType("std::vector<int64_t>&"),
      "partialAggregates");
  auto assignment = VarDeclStatement(var_decl_partial_aggregates).assign(
      getPartialAggregatesCall);
  code_.current_code_insertion_point->addStatement(
      std::make_shared<BinaryOperatorStatement>(assignment));

  // update partial aggregate
  const BinaryOperatorStatement &partialRef = VarRef(
      var_decl_partial_aggregates)[current_slice_ref];
  window->windowAggregation->compileLiftCombine(
      code_.current_code_insertion_point,
      partialRef,
      code_.struct_decl_input_tuple,
      VarRef(code_.var_decl_input_tuple)[VarRefStatement(
          VarRef(*(code_.var_decl_id)))]);
...
PipelineStagePtr CCodeGenerator::compile(const CompilerArgs&) {
  ...
  FunctionBuilder func_builder = FunctionBuilder::create("compiled_query")
      .returns(createDataType(BasicType(UINT32))).addParameter(
      code_.var_decl_tuple_buffers).addParameter(code_.var_decl_state)
      .addParameter(code_.var_declare_window).addParameter(
      code_.var_decl_tuple_buffer_output);
...
CodeGeneratorPtr createCodeGenerator() {
  return std::make_shared<CCodeGenerator>(CodeGenArgs());
}
