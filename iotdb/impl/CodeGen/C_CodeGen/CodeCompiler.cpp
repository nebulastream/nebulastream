...
class SystemCompilerCompiledCCode : public CompiledCCode {
  public:
    SystemCompilerCompiledCCode(Timestamp compile_time, SharedLibraryPtr library, const std::string& base_name)
...
class JITCompilerCompiledCCode : public CompiledCCode {
  public:
    JITCompilerCompiledCCode(Timestamp compile_time, const std::shared_ptr<llvm::LLVMContext>& context,
...
CompiledCCodePtr CCodeCompiler::compileWithJITCompiler(const std::string& source, const Timestamp pch_time)
{
    auto start = getTimestamp();
    ...
    args.push_back("-I/usr/lib/clang/" QUERY_COMPILATION_CLANG_VERSION "/include");
    ...
    auto compile_time = end - start + pch_time;
    return std::make_shared<JITCompilerCompiledCCode>(compile_time, ctxt_and_engine.first, ctxt_and_engine.second);
}
...
void CCodeCompiler::prepareClangCompiler(const std::string& source, const std::vector<const char*>& args,
                                         clang::CompilerInstance& compiler)
{
    auto llvm_args = clang::ArrayRef<const char*>(args);

    std::unique_ptr<clang::CompilerInvocation> invocation(clang::createInvocationFromCommandLine(llvm_args));
    if (invocation.get() == nullptr) {
        IOTDB_FATAL_ERROR("Failed to create compiler invocation!");
    }

#if LLVM_VERSION == 60
    clang::CompilerInvocation::setLangDefaults(*invocation->getLangOpts(), clang::InputKind::CXX,
    ...
    clang::CompilerInvocation::setLangDefaults(*invocation->getLangOpts(), clang::IK_C,
                                               llvm::Triple(invocation->getTargetOpts().Triple),
                                               invocation->getPreprocessorOpts(), clang::LangStandard::lang_cxx11);
#else
    clang::CompilerInvocation::setLangDefaults(*invocation->getLangOpts(), clang::IK_CXX,
...
#if LLVM_VERSION >= 40
    compiler.setInvocation(std::shared_ptr<clang::CompilerInvocation>(invocation.release()));
#else
    compiler.setInvocation(invocation.release());
#endif
    compiler.createDiagnostics();

    clang::PreprocessorOptions& po = compiler.getInvocation().getPreprocessorOpts();
    // replace the string-input argument with the actual source code
    po.addRemappedFile("string-input", llvm::MemoryBuffer::getMemBufferCopy(source).release());
}
...
std::vector<const char*> CCodeCompiler::convertStringToCharPtrVec(const std::vector<std::string>& data)
{
