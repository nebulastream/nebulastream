...
bool CoordinatorActor::registerPhysicalStream(std::string ip,
    ...
    size_t hashId = getIdFromHandle(sap);
    NES_DEBUG(
        "CoordinatorActor: register pyhsical stream id=" << sap->id() << " sap=" << to_string(sap) << " hashID="
                                                         << hashId)
...
bool CoordinatorActor::deregisterSensor(const string& ip) {
    ...
    size_t hashId = getIdFromHandle(sap);
    NES_DEBUG(
        "CoordinatorActor: deregister node id=" << sap->id() << " sap=" << to_string(sap) << " hashID=" << hashId
                                                << " sap=" << to_string(sap))
...
void CoordinatorActor::registerSensor(const string& ip, uint16_t publish_port,
    ...
    size_t hashId = getIdFromHandle(sap);
    NES_DEBUG(
        "CoordinatorActor: connect attempt with id=" << sap->id() << " handle=" << to_string(hdl) << " hashID="
                                                     << hashId)
