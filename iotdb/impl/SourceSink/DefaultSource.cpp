    ...
    for (uint64_t recordIndex = 0; recordIndex < tupleCnt; recordIndex++) {
        for (uint64_t fieldIndex = 0; fieldIndex < fields.size();
            ...
            if (dataType->isEqual(createDataType(UINT8))) {
                layout->getValueField<uint8_t>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(UINT16))) {
                layout->getValueField<uint16_t>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(UINT32))) {
                layout->getValueField<uint32_t>(recordIndex, fieldIndex)->write(buf, 1);
                ...
                layout->getValueField<int8_t>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(INT16))) {
                layout->getValueField<int16_t>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(INT32))) {
                layout->getValueField<int32_t>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(INT64))) {
                layout->getValueField<int64_t>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(FLOAT32))) {
                layout->getValueField<float>(recordIndex, fieldIndex)->write(buf, 1);
            } else if (dataType->isEqual(createDataType(FLOAT64))) {
                layout->getValueField<double>(recordIndex, fieldIndex)->write(buf, 1);
            }else{
