...
void Dispatcher::deregisterSource(DataSourcePtr){
    std::unique_lock<std::mutex> lock(mutex);
}
...
void Dispatcher::deregisterQuery(const QueryExecutionPlanPtr){
    std::unique_lock<std::mutex> lock(mutex);
...
void Dispatcher::completedWork(TaskPtr){
    std::unique_lock<std::mutex> lock(mutex);
