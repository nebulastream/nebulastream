...
WindowOperator::WindowOperator(const WindowPtr& window_spec)
  : Operator (), window_spec_(iotdb::copy(window_spec))
{
}

WindowOperator::WindowOperator(const WindowOperator& other)
...
const OperatorPtr WindowOperator::copy() const{
  return std::make_shared<WindowOperator>(*this);
}
...
OperatorType WindowOperator::getOperatorType() const{
  return WINDOW_OP;
}
