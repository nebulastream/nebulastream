...
SortOperator::SortOperator(const Sort& sort_spec)
  : Operator (), sort_spec_(iotdb::copy(sort_spec))
{
}

SortOperator::SortOperator(const SortOperator& other)
...
const OperatorPtr SortOperator::copy() const{
  return std::make_shared<SortOperator>(*this);
}
...
OperatorType SortOperator::getOperatorType() const{
  return SORT_OP;
}
...
const OperatorPtr createSortOperator(const Sort& sort_spec){
  return std::make_shared<SortOperator>(sort_spec);
}
