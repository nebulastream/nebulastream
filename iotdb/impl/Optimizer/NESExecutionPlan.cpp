...
json::value NESExecutionPlan::getExecutionGraphAsJson() const {
    ...
    for (u_int i = 0; i < allEdges.size(); i++) {
        ...
        edgeInfo["linkCapacity"] = json::value::string(std::to_string(edge.ptr->getLinkCapacity()));
        edgeInfo["linkLatency"] = json::value::string(std::to_string(edge.ptr->getLinkLatency()));
...
ExecutionNodeLinkPtr NESExecutionPlan::createExecutionNodeLink(size_t id, ExecutionNodePtr sourceNode, ExecutionNodePtr destinationNode,
    ...
    // create new link
    auto linkPtr = std::make_shared<ExecutionNodeLink>(id, sourceNode, destinationNode, linkCapacity, linkLatency);
...
void NESExecutionPlan::freeResources() {
    for (const ExecutionVertex& v: getExecutionGraph()->getAllVertex()) {
        if (v.ptr->getRootOperator()) {
            ...
            NES_INFO("NESEXECUTIONPLAN: Capacity before-" << v.ptr->getNESNode()->getId() << "->"
            ...
            NES_INFO("NESEXECUTIONPLAN: Capacity after-" << v.ptr->getNESNode()->getId() << "->"
                                                         << v.ptr->getNESNode()->getRemainingCpuCapacity())
        }
    }
}
