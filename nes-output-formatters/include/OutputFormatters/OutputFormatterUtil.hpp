    ...
    AbstractBufferProvider* bufferProvider)
{
    ...
    /// Write string into the memory at starting address. If there is not enough space, allocate children
...
/// Should the space not suffice, it uses writeWithChildrenBuffers to allocate new space.
        ...
        if (lazyVal->getSize() > remainingSize)
        {
            nautilus::invoke(
                    ...
                    AbstractBufferProvider* bufferProvider)
                {
                    ...
                    writeWithChildBuffers(lazyValAsString.c_str(), remainingSpace, tupleBuffer, bufferProvider, address);
                },
                lazyVal->getContent(),
                lazyVal->getSize(),
                address,
