...
template <typename T>
nautilus::val<uint64_t> writeFixedSizeField(
    ...
    const nautilus::val<AbstractBufferProvider*>& bufferProvider) const
{
    ...
    const auto writeField
        = [&compilationContext, &value, &fieldPointer, &remainingSize, &recordBuffer, &bufferProvider, delimiter]<typename T>(
              const std::string& functionName)
    {
        return writeFixedSizeField<T>(
            compilationContext, functionName, value, fieldPointer, remainingSize, recordBuffer, bufferProvider, delimiter);
    };
