...
size_t formatValToString(
    const T val, int8_t* bufferStartingAddress, const uint64_t remainingSpace, const bool isLast, const DataType* physicalType)
{
...
nautilus::val<size_t> CSVOutputFormatter::getFormattedValue(
    ...
    const nautilus::val<uint64_t>& remainingSize) const
{
    ...
    /// For varsized values, we cast to VariableSizedData. We ignore the 4 bytes of payload size at the start of the value.
