...
/// output formatter defines for itself (e.g. CSVOutputFormatterConfig). The struct travels
/// through the OutputFormatterDescriptor type-erased as std::any; reflect/unreflect keep the
/// descriptor serializable — only the one struct per formatter needs to be reflectable, and only
/// this entry knows its concrete type.
struct OutputFormatterConfigEntry
{
...
class OutputFormatterConfigRegistry
    : public RuntimeRegistry<OutputFormatterConfigRegistry, std::string, OutputFormatterConfigEntry, /*CaseSensitive*/ false>
{
public:
    static OutputFormatterConfigRegistry& instance();
};
...
/// (see create_runtime_registry(OutputFormatterConfig ...) in nes-output-formatters/CMakeLists.txt).
template <typename ConfigStruct>
OutputFormatterConfigEntry makeOutputFormatterConfigEntry()
{
    return OutputFormatterConfigEntry{
        .instantiate =
