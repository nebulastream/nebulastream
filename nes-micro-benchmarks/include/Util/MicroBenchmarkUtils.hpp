...
public:
  MicroBenchmarkUtils(uint64_t selectivity,
    uint64_t bufferSize,
  ...
  std::shared_ptr<Runtime::RunningQueryPlanNode> createTasks(folly::MPMCQueue<Runtime::WorkTask>& taskQueue,
...
public:
  ...
  void emitBuffer(const Memory::TupleBuffer &buffer, const ContinuationPolicy policy) override {
    handler(buffer, policy);
  }
...
public:
  ...
  void emitWork(QueryId,
    const std::shared_ptr<Runtime::RunningQueryPlanNode> &,
    Memory::TupleBuffer,
    onComplete,
    onFailure) override;
  void emitPipelineStart(QueryId,
    const std::shared_ptr<Runtime::RunningQueryPlanNode> &,
    onComplete,
    onFailure) override;
