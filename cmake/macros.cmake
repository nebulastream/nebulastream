...
function(project_enable_clang_format)
    ...
    add_custom_target(format       COMMAND python3 ${CMAKE_SOURCE_DIR}/scripts/run_clang_format.py ${CLANG_FORMAT_EXECUTABLE} --exclude_globs ${CMAKE_SOURCE_DIR}/clang_suppressions.txt --source_dirs ${NES_FOLDER_NAMES_COMMA_SEPARATED} --fix USES_TERMINAL)
    add_custom_target(check-format COMMAND python3 ${CMAKE_SOURCE_DIR}/scripts/run_clang_format.py ${CLANG_FORMAT_EXECUTABLE} --exclude_globs ${CMAKE_SOURCE_DIR}/clang_suppressions.txt --source_dirs ${NES_FOLDER_NAMES_COMMA_SEPARATED}       USES_TERMINAL)
...
macro(project_enable_check_comment_format)
    ...
    if (NOT_ALLOWED_COMMENT_STYLE_REGEX)
        ...
        add_custom_target(check-comment-format COMMAND python3 ${CMAKE_SOURCE_DIR}/scripts/run_check_correct_comment_style.py --source_dirs ${NES_FOLDER_NAMES_COMMA_SEPARATED} --not_allowed_comment_style \"${NOT_ALLOWED_COMMENT_STYLE_REGEX}\" USES_TERMINAL)
    else ()
...
macro(project_enable_check_preamble)
    ...
    if (NOT_ALLOWED_COMMENT_STYLE_REGEX)
        ...
        add_custom_target(check-preamble COMMAND python3 ${CMAKE_SOURCE_DIR}/scripts/check_preamble.py ${CMAKE_SOURCE_DIR} ${CMAKE_SOURCE_DIR}/.no-license-check)
    else ()
