...
# missing registries, optional entries, type checks).
#
# Each registry/entry gets a generated glue translation-units (TUs) whose static initializer merely COLLECTS a
# provision-producing function pointer in BuiltinPlugins (trivial, order-independent
# bookkeeping). loadBuiltinPlugins() assembles the descriptor and hands it to the PluginLoader.
# Glue TUs live in a per-registry static glue sub-library
...
function(create_runtime_registry name target)
    ...
    # the INTERFACE line (the final consumer — the executable — pulls the glue in); an
    ...
    # Provide the registry to the plugin system: a collector TU feeds the built-in descriptor
    # of loadBuiltinPlugins().
...
function(_runtime_registry_derive_plugin_header out_var header_override header_template plugin_name src_dir list_dir)
    ...
    elseif(header_template)
        ...
        while(NOT component_src_dir)
            ...
            # Ensures to never treat the repository root (or anything
            # above it) as a component src/ root — a checkout under a directory that happens
            # to be named "src" (e.g. ~/src/nebulastream or a /src container mount) must not
            # match, or the derived include path silently breaks the build.
            if(search_dir STREQUAL CMAKE_SOURCE_DIR OR search_dir STREQUAL parent_dir)
