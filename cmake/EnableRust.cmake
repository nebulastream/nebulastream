...
### Development images contain vendored dependencies for both the NES workspace and nightly's
### standard library. The image installs its source replacement in $CARGO_HOME so it applies to
### Cargo commands run from source directories as well as commands generated in the build tree.
set(_NES_CARGO_CONFIG "${CMAKE_BINARY_DIR}/.cargo/config.toml")
if (DOCKER_DEV_IMAGE
    AND DEFINED ENV{NES_CARGO_VENDOR_CONFIG}
    AND EXISTS "$ENV{NES_CARGO_VENDOR_CONFIG}")
    message(STATUS "Cargo: using vendored dependencies from the development image")
    file(REMOVE "${_NES_CARGO_CONFIG}")
elseif (_NES_CORROSION_USE_LOCAL)
    ### Older development images have a populated Cargo cache instead of a vendor directory.
    file(MAKE_DIRECTORY "${CMAKE_BINARY_DIR}/.cargo")
    file(WRITE "${_NES_CARGO_CONFIG}" "[net]\noffline = true\n")
else ()
    file(REMOVE "${_NES_CARGO_CONFIG}")
endif ()
...
# Defer Rust link finalization twice so it runs after other deferred crate
# registration calls.
...
# path resolution.
get_property(_nes_rustc CACHE Rust_COMPILER_CACHED PROPERTY VALUE)

execute_process(
        COMMAND ${CMAKE_COMMAND} -E env "RUSTC=${_nes_rustc}" ${_CORROSION_CARGO} metadata --no-deps --format-version=1
...
foreach(_pkg_idx RANGE 0 ${_last_pkg})
    ...
    foreach(_tgt_idx RANGE 0 ${_last_target})
        string(JSON _crate_types GET "${_cargo_metadata_json}" "packages" ${_pkg_idx} "targets" ${_tgt_idx} "crate_types")
        if(_crate_types MATCHES "staticlib")
...
execute_process(
        COMMAND ${CMAKE_COMMAND} -E env "RUSTC=${_nes_rustc}" ${_CORROSION_CARGO} tree -i cxx --depth=0
        WORKING_DIRECTORY "${PROJECT_SOURCE_DIR}"
...
# Cache Cargo flags and environment variables for generated umbrella builds.
