...
# Create a header-only INTERFACE target from a CXX bridge target.
# C++ consumers link this instead of the full bridge to avoid dependency cycles.
# The full bridge (with compiled code) is linked only through the umbrella.
# Additional arguments are targets whose headers the bridge's public headers need.
function(cxxbridge_create_header_target bridge_target)
    set(header_target "${bridge_target}-headers")
    add_library(${header_target} INTERFACE)
    target_include_directories(${header_target} INTERFACE
        $<TARGET_PROPERTY:${bridge_target},INTERFACE_INCLUDE_DIRECTORIES>)
    target_compile_features(${header_target} INTERFACE
        $<TARGET_PROPERTY:${bridge_target},INTERFACE_COMPILE_FEATURES>)
    # Propagate header deps (e.g. nes-memory for BufferBindings.hpp → TupleBuffer.hpp)
    foreach(_dep ${ARGN})
        target_link_libraries(${header_target} INTERFACE ${_dep})
    endforeach()
endfunction()
