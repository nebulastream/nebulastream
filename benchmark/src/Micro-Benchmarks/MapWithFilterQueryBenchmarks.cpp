//
// Created by anirudh on 13.05.21.
//
/*
...
/**
 * @brief This file/main shows how a benchmark can be created. The benchmark seen below is a map and filter query that was implemented by using the BM_AddBenchmark macro from <util/BenchmarkUtils.hpp>.
 */
int main() {
    ...
    std::string benchmarkFolderName = "MapWithFilterQueries_" + BenchmarkUtils::getCurDateTimeStringWithNESVersion();
    ...
    //-----------------------------------------Start of BM_MapWithFilterQuery----------------------------------------------------------------------------------------------
    ...
    for (auto selectivity : allSelectivities) {
        BM_AddBenchmark("BM_MapWithFilterQuery",
                        TestQuery::from(thisSchema).map(Attribute("value") = Attribute("key") + Attribute("value")).filter(Attribute("test$key") < selectivity).sink(DummySink::create()),
