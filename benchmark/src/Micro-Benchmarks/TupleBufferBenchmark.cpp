...
static void BM_DefaultFilling_V1(benchmark::State& state) {
    ...
    switch (state.range(0)) {
        case 0: {
            for (auto singleState : state) {
                for (uint64_t i = 0; i < maxTuplesPerBuffer; ++i) {
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<uint8_t>(i, 0)->write(buffer, 42);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<uint16_t>(i, 1)->write(buffer, 1);
                }
            }
            break;
        }
        case 1: {
            for (auto singleState : state) {
                for (uint64_t i = 0; i < maxTuplesPerBuffer; ++i) {
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int16_t>(i, 0)->write(buffer, 42);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int16_t>(i, 1)->write(buffer, 1);
                }
            }
            break;
        }
        case 2: {
            for (auto singleState : state) {
                for (uint64_t i = 0; i < maxTuplesPerBuffer; ++i) {
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 0)->write(buffer, 42);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 1)->write(buffer, 1);
                }
            }
            break;
        }
        case 3: {
            for (auto singleState : state) {
                for (uint64_t i = 0; i < maxTuplesPerBuffer; ++i) {
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int64_t>(i, 0)->write(buffer, 42);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int64_t>(i, 1)->write(buffer, 1);
                }
            }
            break;
        }
        case 4: {
            for (auto singleState : state) {
                for (uint64_t i = 0; i < maxTuplesPerBuffer; ++i) {
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 0)->write(buffer, 42);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 1)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 2)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 3)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 4)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 5)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 6)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 7)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 8)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 9)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 10)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 11)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 12)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 13)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 14)->write(buffer, 1);
                    Runtime::createRowLayout(benchmarkSchema)->getValueField<int32_t>(i, 15)->write(buffer, 1);
                }
            }
            break;
        }
...
static void BM_DefaultFilling_V2(benchmark::State& state) {
    ...
    switch (state.range(0)) {
        case 0: {
            for (auto singleState : state) {
                auto rowLayout = Runtime::createRowLayout(benchmarkSchema);
                for (uint64_t i = 0; i < maxTuplesPerBuffer; ++i) {
