...
folder = "./"
df_changingBufferSize = pd.read_csv(folder + 'changingBufferSizeSourceSink.csv')
df_changingGlobalBufferCnt = pd.read_csv(folder + 'changingGlobalBufferCntSourceSink.csv')
df_changingLocalBufferSize = pd.read_csv(folder + 'changingLocalBufferSizeSourceSink.csv')

df_changingSourceCnt = pd.read_csv(folder + 'changingSourceCntSourceSink.csv')
df_chaningWorkerCnt = pd.read_csv(folder + 'changingWorkerCntSourceSink.csv')
df_changingThreadsAndSourceNoProc = pd.read_csv(folder + 'changingThreadsAndSourceNoProcSourceSink.csv')
df_scalingLarge = pd.read_csv(folder + 'scalingLargeSourceSink.csv')
...
fig.update_layout(
    title={
        'text': "<b>NebulaStream Performance Numbers -- MemoryMode -- SourceSinkWorkload </b><br>"
                "<span style=\"font-size:0.6em\" line-height=1em;margin=-4px>Default Config(GlobalBufferPool=65536, "
                "LocalBufferPool=1024, BufferSize=4MB, Threads=1, Source=1, TupleSize=24Byte, Query::from(input).sink(NullOutputSinkDescriptor::create());)"
                "<br>",
        'y': 0.98,
        'x': 0.5,
        'xanchor': 'center',
