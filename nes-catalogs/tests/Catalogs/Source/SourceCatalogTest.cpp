...
auto testSchema = Schema::create() -> addField("id", BasicType::UINT32)
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SourceCatalogTest.log",
...
TEST_F(SourceCatalogTest, testAddGetLogSource) {
  ...
  map<std::string, SchemaPtr> allLogicalSource =
      sourceCatalog->getAllLogicalSource();
...
TEST_F(SourceCatalogTest, testRemovalOfAllPhysicalSourcesByWorkerId) {
  ...
  auto registerPhysicalSource = [&](const std::string &phyName,
                                    WorkerId workerId) {
    sourceCatalog->addPhysicalSource(
        "test_stream", Catalogs::Source::SourceCatalogEntry::create(
                           PhysicalSource::create(DefaultSourceType::create(
                               "test_stream", phyName)),
                           logicalSource, workerId));
  };
  ...
  EXPECT_THAT(sourceCatalog->getPhysicalSources("test_stream"),
              ::testing::SizeIs(7));
  ...
  EXPECT_THAT(sourceCatalog->getPhysicalSources("test_stream"),
              ::testing::SizeIs(1));
  EXPECT_EQ(
      sourceCatalog->getPhysicalSources("test_stream")[0]->getTopologyNodeId(),
      WorkerId(3));
  EXPECT_EQ(sourceCatalog->getPhysicalSources("test_stream")[0]
                ->getPhysicalSource()
...
TEST_F(SourceCatalogTest, testAddRemoveLogSource) {
  ...
  EXPECT_THROW(sourceCatalog->getSchemaForLogicalSource("test_stream"),
               Exceptions::LogicalSourceNotFoundException);

  string exp = "logical stream name=default_logical schema: name=id UINT32 "
               "name=value UINT64\n\nlogical stream "
...
TEST_F(SourceCatalogTest, testGetNotExistingKey) {
  Catalogs::Source::SourceCatalogPtr sourceCatalog =
      std::make_shared<Catalogs::Source::SourceCatalog>();
  EXPECT_THROW(sourceCatalog->getSchemaForLogicalSource("test_stream22"),
               Exceptions::LogicalSourceNotFoundException);
}
...
TEST_F(SourceCatalogTest, testAddGetPhysicalSource) {
  ...
  auto defaultSourceType = DefaultSourceType::create(
      logicalSource->getLogicalSourceName(), "physicalSource");
  ...
  EXPECT_TRUE(sourceCatalog->addPhysicalSource(
      logicalSource->getLogicalSourceName(), sce));
}
...
TEST_F(SourceCatalogTest, testAddRemovePhysicalSource) {
  ...
  EXPECT_TRUE(sourceCatalog->removePhysicalSource(
      ...
      physicalSource->getPhysicalSourceName(), physicalNode->getId()));
...
TEST_F(SourceCatalogTest, testGetAllLogicalSource) {
  ...
  const map<std::string, std::string> &allLogicalSource =
...
TEST_F(SourceCatalogTest, testUpdateLogicalSourceWithInvalidSourceName) {
  ...
  bool success =
      sourceCatalog->updateLogicalSource(logicalSourceName, newSchema);
