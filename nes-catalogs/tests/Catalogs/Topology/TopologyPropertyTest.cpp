...
class TopologyPropertiesTest : public Testing::BaseUnitTest {
 ...
 protected:
  static void setupLogging() {
    NES::Logger::setupLogging("TopologyPropertiesTest.log",
...
TEST_F(TopologyPropertiesTest, testAssignTopologyNodeProperties) {
  ...
  // create a node
  auto node = TopologyNode::create(WorkerId(1), "localhost", grpcPort, dataPort,
                                   8, properties);
  ...
  EXPECT_EQ(std::any_cast<std::string>(node->getNodeProperty("architecture")),
            "arm64");
...
TEST_F(TopologyPropertiesTest, testAssignLinkProperty) {
  ...
  topology->registerWorker(sourceWorkerId, "localhost", grpcPort, dataPort, 8,
                           properties, 0, 0);

  grpcPort++;
  dataPort++;
  auto destinationWorkerId = WorkerId(2);
  topology->registerWorker(destinationWorkerId, "localhost", grpcPort, dataPort,
                           8, properties, 0, 0);
  ...
  auto destinationNode =
      topology->getCopyOfTopologyNodeWithId(destinationWorkerId);
...
TEST_F(TopologyPropertiesTest, testRemovingLinkProperty) {
  ...
  ASSERT_EQ(sourceNode->getLinkProperty(destinationWorkerId)->bandwidth,
            512ULL);
  ...
  ASSERT_EQ(destinationNode->getLinkProperty(sourceWorkerId)->bandwidth,
            512ULL);
