...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NesTopologyManager.log",
...
TEST_F(TopologyTest, createNode) {
  ...
  auto physicalNode = TopologyNode::create(node1Id, node1Address, grpcPort,
                                           dataPort, resources, properties);
  EXPECT_NE(physicalNode.get(), nullptr);
  EXPECT_EQ(physicalNode->toString(),
            "PhysicalNode[id=" + node1Id.toString() + ", ip=" + node1Address +
                ", resourceCapacity=" + std::to_string(resources) +
                ", usedResource=0]");
...
TEST_F(TopologyTest, removeRootNode) {
  ...
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
      Spatial::Experimental::SpatialType::NO_LOCATION;
  topology->registerWorker(node1Id, node1Address, grpcPort, dataPort, resources,
                           properties, 0, 0);
...
TEST_F(TopologyTest, removeAnExistingNode) {
  ...
  grpcPort++;
  dataPort++;
  topology->registerWorker(node2Id, node2Address, grpcPort, dataPort, resources,
                           properties, 0, 0);
...
TEST_F(TopologyTest, removeNodeWithNonRootParent) {
  ...
  grpcPort++;
  dataPort++;
  topology->registerWorker(node3Id, node3Address, grpcPort, dataPort, resources,
                           properties, 0, 0);
  ...
  grpcPort++;
  dataPort++;
  topology->registerWorker(node4Id, node4Address, grpcPort, dataPort, resources,
                           properties, 0, 0);
...
TEST_F(TopologyTest, removeLink) {
  ...
  std::map<std::string, std::any> properties;
  properties[NES::Worker::Properties::MAINTENANCE] = false;
  node1Id = topology->registerWorker(node1Id, node1Address, grpcPort, dataPort,
                                     resources, properties, 0, 0);
  topology->addAsRootWorkerId(node1Id);
  ...
  grpcPort++;
  dataPort++;
  node2Id = topology->registerWorker(node2Id, node2Address, grpcPort, dataPort,
                                     resources, properties, 0, 0);
...
TEST_F(TopologyTest, printGraph) {
  ...
  for (uint32_t i = 0; i < 7; ++i) {
    auto workerId =
        topology->registerWorker(WorkerId(i), "localhost", grpcPort, dataPort,
                                 resource, properties, 0, 0);
...
TEST_F(TopologyTest, findPathBetweenNodesWithMultipleParentsAndChildren) {
  ...
  const std::optional<TopologyNodePtr> startNode =
      topology->findAllPathBetween(workerIds.at(9), workerIds.at(2));
...
TEST_F(TopologyTest, findPathBetweenTwoNotConnectedNodes) {
  ...
  const std::optional<TopologyNodePtr> startNode =
      topology->findAllPathBetween(node3Id, node1Id);
...
TEST_F(TopologyTest, findPathBetweenSetOfSourceAndDestinationNodes) {
  ...
  // link each worker with its neighbor
  topology->addTopologyNodeAsChild(topologyNodeIds.at(1),
                                   topologyNodeIds.at(3));
  topology->addTopologyNodeAsChild(topologyNodeIds.at(1),
                                   topologyNodeIds.at(4));

  topology->addTopologyNodeAsChild(topologyNodeIds.at(2),
                                   topologyNodeIds.at(5));
  topology->addTopologyNodeAsChild(topologyNodeIds.at(2),
                                   topologyNodeIds.at(6));

  topology->addTopologyNodeAsChild(topologyNodeIds.at(4),
                                   topologyNodeIds.at(7));
  topology->addTopologyNodeAsChild(topologyNodeIds.at(5),
                                   topologyNodeIds.at(7));

  topology->addTopologyNodeAsChild(topologyNodeIds.at(7),
                                   topologyNodeIds.at(8));
  topology->addTopologyNodeAsChild(topologyNodeIds.at(7),
                                   topologyNodeIds.at(9));

  // Remove the registration of connection with the root node
  topology->removeTopologyNodeAsChild(topologyNodeIds.at(0),
                                      topologyNodeIds.at(3));
  topology->removeTopologyNodeAsChild(topologyNodeIds.at(0),
                                      topologyNodeIds.at(4));
  topology->removeTopologyNodeAsChild(topologyNodeIds.at(0),
  ...
  const std::vector<TopologyNodePtr> startNodes =
      topology->findPathBetween(sourceNodes, destinationNodes);
  ...
  EXPECT_TRUE(startNode2->getParents().size() ==
              startNode1->getParents().size());
 ...
 */
TEST_F(TopologyTest,
       findPathBetweenSetOfSourceAndDestinationNodesAndSelectTheShortest) {
  TopologyPtr topology = Topology::create();
...
/**
 * @brief Tests if the path finding function find() properly ignores nodes
 marked for maintenance in a complex topology
 * Topology:
    ...
    |  |  |--PhysicalNode[id=11, ip=localhost, resourceCapacity=4,
 usedResource=0] |  |  |  |--PhysicalNode[id=14, ip=localhost,
 resourceCapacity=4, usedResource=0] |  |  |--PhysicalNode[id=10, ip=localhost,
 resourceCapacity=4, usedResource=0] |  |  |  |--PhysicalNode[id=14,
 ip=localhost, resourceCapacity=4, usedResource=0] |  |  |
 ...
 |  |  |--PhysicalNode[id=13, ip=localhost, resourceCapacity=4, usedResource=0]
    ...
    |  |--PhysicalNode[id=6, ip=localhost, resourceCapacity=4, usedResource=0]
    |  |  |--PhysicalNode[id=10, ip=localhost, resourceCapacity=4,
 usedResource=0] |  |  |  |--PhysicalNode[id=14, ip=localhost,
 resourceCapacity=4, usedResource=0] |  |  |  |--PhysicalNode[id=13,
 ip=localhost, resourceCapacity=4, usedResource=0] |  |--PhysicalNode[id=5,
 ip=localhost, resourceCapacity=4, usedResource=0] |  |  |--PhysicalNode[id=9,
 ip=localhost, resourceCapacity=4, usedResource=0] |  |  |
 |--PhysicalNode[id=13, ip=localhost, resourceCapacity=4, usedResource=0] |  |
 ...
 |  |--PhysicalNode[id=12, ip=localhost, resourceCapacity=4, usedResource=0]
    ...
    |  |--PhysicalNode[id=8, ip=localhost, resourceCapacity=4, usedResource=0]
    |  |  |--PhysicalNode[id=12, ip=localhost, resourceCapacity=4,
 usedResource=0]
*/
/*TEST_F(TopologyTest, testPathFindingWithMaintenance) {
    ...
    int resource = 4;
    for (uint32_t i = 0; i < 15; ++i) {
        topology->registerTopologyNode(i, "localhost", grpcPort, dataPort,
resource, properties, 0, 0); topologyNodeIds.emplace_back(i); grpcPort =
grpcPort + 2; dataPort = dataPort + 2;
    }

    topology->addAsRootWorkerId(topologyNodeIds.at(0));
    //sets up Topology
    // link each worker with its neighbor
    topology->addTopologyNodeAsChild(topologyNodeIds.at(0),
topologyNodeIds.at(1)); topology->addTopologyNodeAsChild(topologyNodeIds.at(0),
topologyNodeIds.at(2)); topology->addTopologyNodeAsChild(topologyNodeIds.at(0),
topologyNodeIds.at(3)); topology->addTopologyNodeAsChild(topologyNodeIds.at(0),
topologyNodeIds.at(4));
...
topologyNodeIds.at(8));
    ...
    topology->addTopologyNodeAsChild(topologyNodeIds.at(2),
topologyNodeIds.at(5)); topology->addTopologyNodeAsChild(topologyNodeIds.at(2),
topologyNodeIds.at(6));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(3),
topologyNodeIds.at(6)); topology->addTopologyNodeAsChild(topologyNodeIds.at(3),
topologyNodeIds.at(7));
...
topologyNodeIds.at(7));
    ...
    topology->addTopologyNodeAsChild(topologyNodeIds.at(5),
topologyNodeIds.at(8)); topology->addTopologyNodeAsChild(topologyNodeIds.at(5),
topologyNodeIds.at(9));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(6),
topologyNodeIds.at(10));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(7),
topologyNodeIds.at(10)); topology->addTopologyNodeAsChild(topologyNodeIds.at(7),
topologyNodeIds.at(11));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(8),
topologyNodeIds.at(12));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(9),
topologyNodeIds.at(12)); topology->addTopologyNodeAsChild(topologyNodeIds.at(9),
topologyNodeIds.at(13));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(10),
topologyNodeIds.at(13));
    topology->addTopologyNodeAsChild(topologyNodeIds.at(10),
topologyNodeIds.at(14));

    topology->addTopologyNodeAsChild(topologyNodeIds.at(11),
topologyNodeIds.at(14));

    topology->print();

    std::vector<WorkerId> sourceNodes{topologyNodeIds.at(12),
topologyNodeIds.at(13), topologyNodeIds.at(14)};
...
topology->findPathBetween(sourceNodes, destinationNodes);
    ...
    TopologyNodePtr firstStartNodeParent1 =
startNodes[0]->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(firstStartNodeParent1->getId() == topologyNodeIds[8]);
    TopologyNodePtr firstStartNodeParent2 =
firstStartNodeParent1->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(firstStartNodeParent2->getId() == topologyNodeIds[1]);
    TopologyNodePtr firstStartNodeParent3 =
firstStartNodeParent2->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(firstStartNodeParent3->getId() == topologyNodeIds[0]);
    //checks path from source node 13 to sink
    TopologyNodePtr secondStartNodeParent1 =
startNodes[1]->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(secondStartNodeParent1->getId() == topologyNodeIds[10]);
    TopologyNodePtr secondStartNodeParent2 =
secondStartNodeParent1->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(secondStartNodeParent2->getId() == topologyNodeIds[6]);
    TopologyNodePtr secondStartNodeParent3 =
secondStartNodeParent2->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(secondStartNodeParent3->getId() == topologyNodeIds[2]);
    TopologyNodePtr secondStartNodeParent4 =
secondStartNodeParent3->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(secondStartNodeParent4->getId() == topologyNodeIds[0]);
    //checks path from source node 14 to sink
    TopologyNodePtr thirdStartNodeParent1 =
startNodes[2]->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(thirdStartNodeParent1->getId() == topologyNodeIds[10]);
    TopologyNodePtr thirdStartNodeParent2 =
thirdStartNodeParent1->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(thirdStartNodeParent2->getId() == topologyNodeIds[6]);
    TopologyNodePtr thirdStartNodeParent3 =
thirdStartNodeParent2->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(thirdStartNodeParent3->getId() == topologyNodeIds[2]);
    TopologyNodePtr thirdStartNodeParent4 =
thirdStartNodeParent3->getParents()[0]->as<TopologyNode>();
    ...
    const std::vector<TopologyNodePtr> mStartNodes =
topology->findPathBetween(sourceNodes, destinationNodes);
    //checks path from source node 12 to sink
    TopologyNodePtr mFirstStartNodeParent1 =
mStartNodes[0]->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mFirstStartNodeParent1->getId() == topologyNodeIds[9]);
    TopologyNodePtr mFirstStartNodeParent2 =
mFirstStartNodeParent1->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mFirstStartNodeParent2->getId() == topologyNodeIds[5]);
    TopologyNodePtr mFirstStartNodeParent3 =
mFirstStartNodeParent2->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mFirstStartNodeParent3->getId() == topologyNodeIds[2]);
    TopologyNodePtr mFirstStartNodeParent4 =
mFirstStartNodeParent3->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mFirstStartNodeParent4->getId() == topologyNodeIds[0]);
    //checks path from source node 13 to sink
    TopologyNodePtr mSecondStartNodeParent1 =
mStartNodes[1]->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mSecondStartNodeParent1->getId() == topologyNodeIds[9]);
    TopologyNodePtr mSecondStartNodeParent2 =
mSecondStartNodeParent1->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mSecondStartNodeParent2->getId() == topologyNodeIds[5]);
    TopologyNodePtr mSecondStartNodeParent3 =
mSecondStartNodeParent2->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mSecondStartNodeParent3->getId() == topologyNodeIds[2]);
    TopologyNodePtr mSecondStartNodeParent4 =
mSecondStartNodeParent3->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mSecondStartNodeParent4->getId() == topologyNodeIds[0]);
    //checks path from source node 14 to sink
    TopologyNodePtr mThirdStartNodeParent1 =
mStartNodes[2]->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mThirdStartNodeParent1->getId() == topologyNodeIds[11]);
    TopologyNodePtr mThirdStartNodeParent2 =
mThirdStartNodeParent1->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mThirdStartNodeParent2->getId() == topologyNodeIds[7]);
    TopologyNodePtr mThirdStartNodeParent3 =
mThirdStartNodeParent2->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mThirdStartNodeParent3->getId() == topologyNodeIds[4]);
    TopologyNodePtr mThirdStartNodeParent4 =
mThirdStartNodeParent3->getParents()[0]->as<TopologyNode>();
    EXPECT_TRUE(mThirdStartNodeParent4->getId() == topologyNodeIds[0]);
}*/

/**
 * @brief Tests if findCommonAncestor properly ignores nodes marked for
 * maintenance
 */
TEST_F(TopologyTest, testFincCommonAncestorWithMaintenance) {
  ...
  auto copyTopology = topology->findPathBetween(sourceTopologyNodeIds,
                                                destinationTopologyNodeIds);
  ...
  copyTopology = topology->findPathBetween(sourceTopologyNodeIds,
                                           destinationTopologyNodeIds);
...
/**
 * @brief test for expected behavior of findPathBetween in conjunction with
 * findCommonAncestor/Child as well as ignoring nodes marked for maintenance.
 */
TEST_F(
    TopologyTest,
    testPathFindingBetweenAllChildAndParentNodesOfANodeMarkedForMaintenance) {
  TopologyPtr topology = Topology::create();
  ...
  // Idea: Subquery deployed on 5 with Child Operators on nodes 7 and 8. Parent
  // Operators on nodes 1 and 2 try to find a new node onto which we could
  // potentially migrate the subqueries on node 5. this node must be reachable
  // from node 7 and 8 as well as 1 and 2. In this topology the only such node
  // is node 3
