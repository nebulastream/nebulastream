...
class TopologyTimelineTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static std::vector<WorkerId::Underlying> getIdVector(
...
TEST_F(TopologyTimelineTest, DISABLED_testUpdatingMultiplePredictions) {
  ...
  NES_DEBUG(
      "updating prediction that node 3 will reconnect to node 4 at time 4 "
      "instead of time 7");
  ...
  NES_DEBUG(
      "adding prediction that a new node with id 5 will connect to the system "
      "at time 5");
  Experimental::TopologyPrediction::TopologyDelta delta5({{5, 1}}, {});
  versionTimeline.addTopologyDelta(5, delta5);

  NES_DEBUG(
      "node 4 changes prediction that it will connect to node 5 at time 7 "
      "instead of to 1 at time 6");
