...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TopologyTimelineTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG("Setup TopologyTimeline test class.");
  }

  static std::vector<WorkerId::Underlying>
  getIdVector(const std::vector<NodePtr> &nodes) {
  ...
  static void compareIdVectors(const std::vector<NodePtr> &original,
                               const std::vector<NodePtr> &copy) {
  ...
  static void
  compareNodeAt(Experimental::TopologyPrediction::TopologyTimeline versions,
                Timestamp time, uint64_t id, std::vector<uint64_t> parents,
                std::vector<uint64_t> children) {
    NES_DEBUG("compare node {}", id);
    auto predictedNode =
        versions.getTopologyVersion(time)->getCopyOfTopologyNodeWithId(
            WorkerId(id));
...
TEST_F(TopologyTimelineTest, testNoChangesPresent) {
  ...
  topology->registerWorker(rootNodeId, "localhost", 4001, 5001, 4, properties,
                           0, 0);
  ...
  topology->registerWorker(srcNodeId1, "localhost", 4001, 5001, 4, properties,
                           0, 0);
  auto srcNodeId2 = WorkerId(4);
  topology->registerWorker(srcNodeId2, "localhost", 4001, 5001, 4, properties,
                           0, 0);
...
TEST_F(TopologyTimelineTest, DISABLED_testUpdatingMultiplePredictions) {
  ...
  NES_DEBUG(
      "adding new prediction that node 3 will reconnect to node 4 at time 7");
  ...
  NES_DEBUG("updating prediction that node 3 will reconnect to node 4 at time "
            "4 instead of time 7");
  ...
  NES_DEBUG(
      "adding new prediction that node 4 will reconnect to node 1 at time 6");
  ...
  NES_DEBUG("adding prediction that a new node with id 5 will connect to the "
            "system at time 5");
  Experimental::TopologyPrediction::TopologyDelta delta5({{5, 1}}, {});
  versionTimeline.addTopologyDelta(5, delta5);

  NES_DEBUG("node 4 changes prediction that it will connect to node 5 at time "
            "7 instead of to 1 at time 6");
  // changing prediction for node 4 to connect to 5 at time 7 instead of to 1 at
  // time 6 (old prediction should not be visible anymore)
