...
TEST_F(UDFCatalogTest, RetrieveRegisteredJavaUdfDescriptor) {
  ...
  // then
  ASSERT_EQ(udfDescriptor, UDFDescriptor::as<JavaUDFDescriptor>(
...
/**
 * @brief Test that an attempt to register a UDF under an existing name results
 * in an exception.
 */
...
/**
 * @brief Test that a null UDF descriptor is returned when attempting to
 * retrieve an unknown UDF.
 */
...
/**
 * @brief Test that an attempt to remove an unknown UDF does not result in any
...
/**
 * @brief Test that a UDF with the same name can be registered again after
 * removal.
 */
