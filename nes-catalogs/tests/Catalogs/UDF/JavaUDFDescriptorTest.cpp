...
// The following tests check that constructing a Java UDF descriptor with
// invalid data is not possible. In order to eliminate the need to change every
// construction of a JavaUDFDescriptor instance when adding fields to the
// JavaUDFDescriptor class, the construction is done using a builder pattern.
// By default, the JavaUDFDescriptorBuilder contains valid data for all fields
// required for the Java UDF descriptor, e.g., class name, method name,
// serialized instance, bytecode list, and others.
...
TEST_F(
    JavaUDFDescriptorTest,
    TheListOfByteCodeDefinitionsMustContainTheFullyQualifiedNameOfTheUdfClass) {
  // when
  auto unknownClassName = "some_other_package.some_other_class_name"s;
  auto byteCodeList = jni::JavaUDFByteCodeList{
      {"some_package.unknown_class_name"s, jni::JavaByteCode{1}}};
  // then
  EXPECT_THROW(JavaUDFDescriptorBuilder{}
                   .setClassName(unknownClassName)
                   .setByteCodeList(byteCodeList)
                   .build(),
               UDFException);
}

TEST_F(JavaUDFDescriptorTest,
       TheListOfByteCodeDefinitionsMustNotContainEmptyByteCode) {
  // when
  auto className = "some_package.my_udf"s;
  auto byteCodeListWithEmptyByteCode = jni::JavaUDFByteCodeList{
      {className, jni::JavaByteCode{}}};  // empty byte array
  // then
  EXPECT_THROW(JavaUDFDescriptorBuilder{}
                   .setClassName(className)
                   .setByteCodeList(byteCodeListWithEmptyByteCode)
                   .build(),
               UDFException);
}
...
TEST_F(JavaUDFDescriptorTest, Equality) {
  ...
  // then
  ASSERT_TRUE(*descriptor1 ==
              *descriptor2);  // Compare pointed-to variables, not the pointers.
}

TEST_F(JavaUDFDescriptorTest, InEquality) {
  // when
  auto descriptor = JavaUDFDescriptorBuilder::createDefaultJavaUDFDescriptor();
  // Check a different class name. In this case, the byte code list must contain
  // the byte code for the different class name.
  ...
  auto descriptorWithDifferentMethodName =
      JavaUDFDescriptorBuilder{}.setMethodName("different_method_name").build();
  EXPECT_FALSE(*descriptor == *descriptorWithDifferentMethodName);
  // Check a different serialized instance (internal state of the UDF).
  auto descriptorWithDifferentSerializedInstance =
      JavaUDFDescriptorBuilder{}.setInstance({2}).build();
  ...
  auto descriptorWithDifferentByteCodeList =
      JavaUDFDescriptorBuilder{}
          .setByteCodeList(
              {{descriptor->getClassName(), {1}}, {differentClassName, {1}}})
          .build();
  EXPECT_FALSE(*descriptor == *descriptorWithDifferentByteCodeList);
  // Check a different input type.
  auto descriptorWithDifferentInputType =
      JavaUDFDescriptorBuilder{}
          .setInputClassName("different_input_type")
          .build();
  // Check a different return type.
  auto descriptorWithDifferentOutputType =
      JavaUDFDescriptorBuilder{}
          .setOutputClassName("different_output_type")
          .build();
  EXPECT_FALSE(*descriptor == *descriptorWithDifferentOutputType);
  // The output schema is ignored because it can be derived from the method
  // signature. We trust the Java client to do this correctly; it would cause
  // errors otherwise during query execution.
}
