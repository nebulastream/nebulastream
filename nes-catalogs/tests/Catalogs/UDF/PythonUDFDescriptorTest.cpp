...
// The following tests check that constructing a Python UDF descriptor with
// invalid data is not possible. In order to eliminate the need to change every
// construction of a PythonUDFDescriptor instance when adding fields to the
// PythonUDFDescriptor class, the construction is done using a builder pattern.
// By default, the PythonUDFDescriptorBuilder contains valid data for all fields
// required for the Python UDF descriptor, e.g., class name, method name,
// serialized instance, bytecode list, and others.
const std::string functionName = "udf_function";
const std::string functionString =
    "def udf_function(x):\n\ty = x + 10\n\treturn y\n";
const SchemaPtr inputSchema = std::make_shared<Schema>()->addField(
    "inputAttribute", DataTypeFactory::createUInt64());
const SchemaPtr outputSchema = std::make_shared<Schema>()->addField(
