...
bool SourceCatalog::removeLogicalSource(const std::string& logicalSourceName) {
  ...
  if (logicalToPhysicalSourceMapping[logicalSourceName].size() != 0) {
    NES_DEBUG(
        "SourceCatalog: cannot remove {} because there are physical entries "
        "for this source",
        logicalSourceName);
    return false;
  }
...
bool SourceCatalog::addPhysicalSource(
  ...
  if (!containsLogicalSource(logicalSourceName)) {
    NES_ERROR(
        "SourceCatalog: logical source {} does not exists when inserting "
        "physical source {} ",
        logicalSourceName,
...
bool SourceCatalog::removePhysicalSource(const std::string& logicalSourceName,
  ...
  if (logicalSourceNameToSchemaMapping.find(logicalSourceName) ==
      logicalSourceNameToSchemaMapping.end()) {
    NES_ERROR(
        "SourceCatalog: logical source {} does not exists when trying to "
        "remove physical source with hashId {}",
        logicalSourceName, topologyNodeId);
    return false;
  }
  NES_DEBUG(
      "SourceCatalog: logical source {} exists try to remove physical source{} "
      "from node {}",
  ...
  NES_DEBUG(
      "SourceCatalog: physical source {} does not exist on node with id {} and "
      "with logicalSourceName {}",
...
size_t SourceCatalog::removeAllPhysicalSourcesByWorker(
    WorkerId topologyNodeId) {
  std::unique_lock lock(catalogMutex);
...
SchemaPtr SourceCatalog::getSchemaForLogicalSource(
    const std::string& logicalSourceName) {
  std::unique_lock lock(catalogMutex);
  if (logicalSourceNameToSchemaMapping.find(logicalSourceName) ==
      logicalSourceNameToSchemaMapping.end()) {
...
LogicalSourcePtr SourceCatalog::getLogicalSource(
    const std::string& logicalSourceName) {
  std::unique_lock lock(catalogMutex);
...
LogicalSourcePtr SourceCatalog::getLogicalSourceOrThrowException(
  ...
  NES_ERROR(
      "SourceCatalog::getLogicalSourceOrThrowException: source does not exists "
      "{}",
      logicalSourceName);
...
std::vector<SourceCatalogEntryPtr> SourceCatalog::getPhysicalSources(
    const std::string& logicalSourceName) {
  std::unique_lock lock(catalogMutex);
  if (!logicalToPhysicalSourceMapping.contains(logicalSourceName)) {
    NES_ERROR(
        "SourceCatalog: Unable to find source catalog entry with logical "
        "source name {}",
        logicalSourceName);
