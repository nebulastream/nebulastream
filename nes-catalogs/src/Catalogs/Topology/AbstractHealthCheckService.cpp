...
void AbstractHealthCheckService::addNodeToHealthCheck(
    WorkerId workerId, const std::string &rpcAddress) {
  NES_DEBUG("HealthCheckService: adding node with id {}", workerId);
  auto exists = topologyIdToRPCAddressMap.contains(workerId);
  if (exists) {
    NES_THROW_RUNTIME_ERROR(
        "HealthCheckService want to add node that already exists id="
        << workerId);
  }
...
void AbstractHealthCheckService::removeNodeFromHealthCheck(WorkerId workerId) {
  if (!topologyIdToRPCAddressMap.contains(workerId)) {
    NES_THROW_RUNTIME_ERROR(
        "HealthCheckService want to remove a node that does not exists id="
        << workerId);
  }
...
bool AbstractHealthCheckService::isWorkerInactive(WorkerId workerId) {
  NES_DEBUG("HealthCheckService: checking if node with id {} is inactive",
            workerId);
