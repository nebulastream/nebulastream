...
WorkerId Topology::registerWorkerAsRoot(
    ...
    std::map<std::string, std::any> workerProperties, uint32_t bandwidthInMbps,
  ...
  if (newRootWorkerId == INVALID_WORKER_NODE_ID ||
    ...
    NES_WARNING(
        "Worker id {} either invalid or a node with this id already "
  ...
  NES_WARNING(
      "Topology node with id {} already exists. Failed to register the new "
      "topology node.",
      newRootWorkerId);
  return INVALID_WORKER_NODE_ID;
}

WorkerId Topology::registerWorker(
    WorkerId workerId, const std::string& address, const int64_t grpcPort,
    const int64_t dataPort, const uint16_t numberOfSlots,
    std::map<std::string, std::any> workerProperties, uint32_t bandwidthInMbps,
    uint32_t latencyInMs) {
...
bool Topology::addTopologyNodeAsChild(WorkerId parentWorkerId,
  ...
  for (const auto& child : children) {
    if (child->as<TopologyNode>()->getId() == childWorkerId) {
      NES_ERROR(
          "TopologyManagerService::AddParent: parent relationship between "
          "nodes {} and {} already exists",
...
bool Topology::unregisterWorker(WorkerId topologyNodeId) {
  NES_DEBUG(
      "TopologyManagerService::UnregisterNode: try to disconnect sensor with "
      "id  {}",
      topologyNodeId);
...
std::vector<TopologyNodePtr> Topology::findPathBetween(
  ...
  if (!lockedWorkerIdToTopologyNodeMap) {
    NES_WARNING(
        "Unable to acquire write lock on the topology to process the find path "
        "between request");
    return {};
  }
  ...
  for (auto sourceTopologyNodeId : sourceTopologyNodeIds) {
    ...
    if (!readLockedTopologyNode) {
      NES_WARNING(
          "Unable to acquire read lock on the topology node {} to process the "
  ...
  for (const auto& sourceTopologyNode : sourceTopologyNodes) {
    NES_TRACE(
        "Topology: Finding all paths between the source node {} and a set of "
        "destination nodes",
    ...
    for (const auto& destinationTopologyNode : destinationTopologyNodes) {
      if (mapOfUniqueNodes.find(destinationTopologyNode->getId()) ==
        ...
        NES_ERROR(
            "Topology: Unable to find path between source node {} and "
            "destination node{}",
            sourceTopologyNode->toString(),
    ...
    NES_TRACE(
        "Topology: Push the start node of the graph into a collection of start "
        "nodes");
    startNodesOfGraph.push_back(startNodeOfGraph);
  }
  NES_TRACE(
      "Topology: Merge all found sub-graphs together to create a single sub "
      "graph and return the set of start nodes of "
      "the merged graph.");
...
std::optional<TopologyNodePtr> Topology::findAllPathBetween(
...
std::vector<TopologyNodePtr> Topology::mergeSubGraphs(
    const std::vector<TopologyNodePtr>& startNodes) {
  NES_INFO("Topology: Merge {} sub-graphs to create a single sub-graph",
           startNodes.size());

  NES_DEBUG(
      "Topology: Compute a map storing number of times a node occurred in "
  ...
  for (const auto& startNode : startNodes) {
    ...
    NES_TRACE(
        "Topology: Iterate over the family members and add the information in "
        "the node count map about the node occurrence");
    for (const auto& member : family) {
      WorkerId workerId = member->as<TopologyNode>()->getId();
      if (nodeCountMap.find(workerId) != nodeCountMap.end()) {
        NES_TRACE(
            "Topology: Family member already present increment the occurrence "
            "count");
  ...
  NES_DEBUG(
      "Topology: Iterate over each sub-graph and compute a single merged "
      "sub-graph");
  std::vector<TopologyNodePtr> result;
  std::map<WorkerId, TopologyNodePtr> mergedGraphNodeMap;
  for (const auto& startNode : startNodes) {
    NES_DEBUG(
        "Topology: Check if the node already present in the new merged graph "
        "and add a copy of the node if not present");
    if (mergedGraphNodeMap.find(startNode->getId()) ==
        mergedGraphNodeMap.end()) {
      TopologyNodePtr copyOfStartNode = startNode->copy();
      NES_DEBUG(
          "Topology: Add the start node to the list of start nodes for the new "
          "merged graph");
      result.push_back(copyOfStartNode);
      mergedGraphNodeMap[startNode->getId()] = copyOfStartNode;
    }
    NES_DEBUG(
        "Topology: Iterate over the ancestry of the start node and add the "
        "eligible nodes to new merged graph");
    TopologyNodePtr childNode = startNode;
    while (childNode) {
      NES_TRACE(
          "Topology: Get all parents of the child node to select the next "
          "parent to traverse.");
      ...
      if (parents.size() > 1) {
        ...
        NES_TRACE(
            "Topology: Iterate over all parents and select the parent node "
            "that has the max cost value.");
        double maxCost = 0;
        for (auto& parent : parents) {
          NES_TRACE(
              "Topology: Get all ancestor of the node and aggregate their "
              "occurrence counts.");
          ...
          NES_TRACE(
              "Topology: Compute cost by multiplying aggregate occurrence "
              "count with base multiplier and "
          ...
          if (cost > maxCost) {
            ...
            if (selectedParent) {
              NES_TRACE(
                  "Topology: Remove the previously selected parent as parent "
            ...
            if (selectedParent) {
              NES_TRACE(
                  "Topology: Remove this parent as parent to the current child "
                  "node.");
      ...
      if (selectedParent) {
        ...
        if (mergedGraphNodeMap.find(selectedParent->getId()) !=
            mergedGraphNodeMap.end()) {
          NES_TRACE(
              "Topology: New next parent is already present in the new merged "
              "graph.");
          ...
          NES_TRACE(
              "Topology: Add the existing node, with id same as new next "
          ...
          NES_TRACE(
              "Topology: New next parent is not present in the new merged "
              "graph.");
          NES_TRACE(
              "Topology: Add copy of new next parent as parent to the existing "
              "child node in the new merged graph.");
      ...
      NES_TRACE(
          "Topology: Assign new selected parent as next child node to "
          "traverse.");
...
TopologyNodePtr Topology::find(
    TopologyNodePtr testNode, std::vector<TopologyNodePtr> searchedNodes,
  ...
  if (found != searchedNodes.end()) {
    ...
    if (!uniqueNodes.contains(testNode->getId())) {
      NES_TRACE(
          "Topology: Insert the information about the test node in the unique "
          "node map");
  ...
  for (auto& parent : updatedParents) {
    ...
    if (foundInParent) {
      NES_TRACE(
          "Topology: found the destination node as the parent of the physical "
          "node.");
      if (!foundNode) {
