...
std::vector<WorkerId> Topology::getRootWorkerNodeIds()
{
    return rootWorkerIds;
}
    ...
    uint32_t latencyInMs)
{
    ...
    {
        TopologyNodePtr newTopologyNode = TopologyNode::create(workerId, address, grpcPort, dataPort, numberOfSlots, workerProperties);
        NES_INFO("Adding New Node {} to the catalog of nodes.", newTopologyNode->toString());
...
bool Topology::addTopologyNodeAsChild(WorkerId parentWorkerId, WorkerId childWorkerId)
{
    ...
    for (const auto & child : children)
    {
        ...
        {
            NES_ERROR(
                "TopologyManagerService::AddParent: parent relationship between nodes {} and {} already exists",
                childWorkerId,
                parentWorkerId);
            return false;
        }
...
bool Topology::removeTopologyNodeAsChild(WorkerId parentWorkerId, WorkerId childWorkerId)
{
    ...
    auto [lockedParentTopologyNode, lockedChildTopologyNode]
        = folly::acquireLocked((*lockedWorkerIdToTopologyNodeMap)[parentWorkerId], (*lockedWorkerIdToTopologyNodeMap)[childWorkerId]);
...
Topology::findPathBetween(const std::vector<WorkerId> & sourceTopologyNodeIds, const std::vector<WorkerId> & destinationTopologyNodeIds)
{
    ...
    {
        auto readLockedTopologyNode = (*lockedWorkerIdToTopologyNodeMap)[sourceTopologyNodeId].tryRLock();
        if (!readLockedTopologyNode)
        {
            NES_WARNING(
                "Unable to acquire read lock on the topology node {} to process the find path between request", sourceTopologyNodeId);
            return {};
        }
    ...
    {
        auto readLockedTopologyNode = (*lockedWorkerIdToTopologyNodeMap)[destinationTopologyNodeId].tryRLock();
        if (!readLockedTopologyNode)
        {
            NES_WARNING(
                "Unable to acquire read lock on the topology node {} to process the find path between request", destinationTopologyNodeId);
            return {};
        }
    ...
    {
        NES_TRACE("Topology: Finding all paths between the source node {} and a set of destination nodes", sourceTopologyNode->toString());
        ...
        for (const auto & destinationTopologyNode : destinationTopologyNodes)
        {
            ...
            {
                NES_ERROR(
                    "Topology: Unable to find path between source node {} and destination node{}",
...
bool Topology::findAllDownstreamNodes(
    const WorkerId & startNode, std::set<WorkerId> & reachableDownstreamNodes, std::vector<WorkerId> targetNodes)
{
    ...
    while (!targetNodes.empty() && !queue.empty())
    {
        ...
        if (!lockedNode)
        {
            NES_WARNING("Unable to acquire read lock on the topology node {} to process the find path between request", currentNodeId);
            return false;
        }
...
std::optional<TopologyNodePtr> Topology::findAllPathBetween(WorkerId sourceTopologyNodeId, WorkerId destinationTopologyNodeId)
{
    ...
    auto readLockedSourceTopologyNode = (*lockedWorkerIdToTopologyNodeMap)[sourceTopologyNodeId].tryRLock();
    if (!readLockedSourceTopologyNode)
    {
        NES_WARNING("Unable to acquire read lock on the topology node {} to process the find path between request", sourceTopologyNodeId);
        return {};
    }
    ...
    std::vector<TopologyNodePtr> destinationTopologyNodes;
    auto readLockedDestinationTopologyNode = (*lockedWorkerIdToTopologyNodeMap)[destinationTopologyNodeId].tryRLock();
    ...
    NES_WARNING("Topology: Unable to find path between {} and {}", sourceTopologyNode->toString(), destinationTopologyNode->toString());
    return std::nullopt;
}
...
TopologyNodePtr
Topology::find(TopologyNodePtr testNode, std::vector<TopologyNodePtr> searchedNodes, std::map<WorkerId, TopologyNodePtr> & uniqueNodes)
{
    NES_TRACE("Topology: check if test node is one of the searched node");
    auto found = std::find_if(
        searchedNodes.begin(),
        searchedNodes.end(),
        [&](const TopologyNodePtr & searchedNode) { return searchedNode->getId() == testNode->getId(); });
...
void Topology::print()
{
    NES_DEBUG("Topology print:{}", toString());
}
...
bool Topology::addGeoLocation(WorkerId workerId, NES::Spatial::DataTypes::Experimental::GeoLocation && geoLocation)
{
    ...
    if (geoLocation.isValid() && (*lockedTopologyNode)->getSpatialNodeType() == Spatial::Experimental::SpatialType::FIXED_LOCATION)
    {
...
nlohmann::json Topology::convertNodeLocationInfoToJson(WorkerId workerId, NES::Spatial::DataTypes::Experimental::GeoLocation geoLocation)
{
    nlohmann::json nodeInfo;
    nodeInfo["id"] = workerId;
