...
bool LocationIndex::initializeFieldNodeCoordinates(
...
bool LocationIndex::updateFieldNodeCoordinates(
    WorkerId topologyNodeId,
    Spatial::DataTypes::Experimental::GeoLocation &&geoLocation) {
...
bool LocationIndex::setFieldNodeCoordinates(
    WorkerId topologyNodeId,
    Spatial::DataTypes::Experimental::GeoLocation &&geoLocation) {
...
#ifdef S2DEF
    S2Point point(S2LatLng::FromDegrees(geoLocation.getLatitude(),
...
std::optional<WorkerId> LocationIndex::getClosestNodeTo(
    const Spatial::DataTypes::Experimental::GeoLocation &&geoLocation,
    int radius) const {
#ifdef S2DEF
  S2ClosestPointQuery<WorkerId> query(&workerPointIndex);
  query.mutable_options()->set_max_distance(
      S1Angle::Radians(S2Earth::KmToRadians(radius)));
  S2ClosestPointQuery<WorkerId>::PointTarget target(
      S2Point(S2LatLng::FromDegrees(geoLocation.getLatitude(),
                                    geoLocation.getLongitude())));
  S2ClosestPointQuery<WorkerId>::Result queryResult =
...
#else
  NES_WARNING(
      "Files were compiled without s2. Nothing inserted into spatial index");
  (void)geoLocation;
  (void)radius;
  return {};
#endif
...
std::optional<WorkerId> LocationIndex::getClosestNodeTo(WorkerId topologyNodeId,
                                                        int radius) const {
#ifdef S2DEF
  auto workerGeoLocation = workerGeoLocationMap.find(topologyNodeId);
  if (workerGeoLocation == workerGeoLocationMap.end()) {
  ...
  auto geoLocation = workerGeoLocation->second;
  if (!geoLocation.isValid()) {
    NES_WARNING("Trying to get the closest node to a node that does not have a "
                "location")
    return {};
  }
  ...
  // if the closest node is equal to our input node, we need to look for the
  // second closest
...
LocationIndex::getNodeIdsInRange(
    const Spatial::DataTypes::Experimental::GeoLocation &center,
    double radius) const {
#ifdef S2DEF
  S2ClosestPointQuery<WorkerId> query(&workerPointIndex);
  query.mutable_options()->set_max_distance(
  ...
  S2ClosestPointQuery<WorkerId>::PointTarget target(S2Point(
      S2LatLng::FromDegrees(center.getLatitude(), center.getLongitude())));
  ...
  for (auto r : result) {
    ...
    closestNodeList.emplace_back(
        r.data(), Spatial::DataTypes::Experimental::GeoLocation(
...
void LocationIndex::addMobileNode(
    WorkerId topologyNodeId,
...
std::vector<std::pair<WorkerId, Spatial::DataTypes::Experimental::GeoLocation>>
LocationIndex::getAllNodeLocations() const {
  std::vector<
      std::pair<WorkerId, Spatial::DataTypes::Experimental::GeoLocation>>
      locationVector;
