...
bool LocationIndex::initializeFieldNodeCoordinates(WorkerId topologyNodeId, Spatial::DataTypes::Experimental::GeoLocation&& geoLocation)
{
    return setFieldNodeCoordinates(topologyNodeId, std::move(geoLocation));
}

bool LocationIndex::updateFieldNodeCoordinates(WorkerId topologyNodeId, Spatial::DataTypes::Experimental::GeoLocation&& geoLocation)
{
...
bool LocationIndex::setFieldNodeCoordinates(WorkerId topologyNodeId, Spatial::DataTypes::Experimental::GeoLocation&& geoLocation)
{
...
std::optional<WorkerId> LocationIndex::getClosestNodeTo(const Spatial::DataTypes::Experimental::GeoLocation&& geoLocation, int radius) const
{
#ifdef S2DEF
...
LocationIndex::getNodeIdsInRange(const Spatial::DataTypes::Experimental::GeoLocation& center, double radius) const
{
    ...
    S2ClosestPointQuery<WorkerId>::PointTarget target(S2Point(S2LatLng::FromDegrees(center.getLatitude(), center.getLongitude())));
    ...
    for (auto r : result)
    {
        ...
        closestNodeList.emplace_back(
            r.data(), Spatial::DataTypes::Experimental::GeoLocation(latLng.lat().degrees(), latLng.lng().degrees()));
    }
