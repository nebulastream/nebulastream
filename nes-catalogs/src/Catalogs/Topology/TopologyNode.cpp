...
WorkerId TopologyNode::getId() const
{
    return workerId;
}
...
uint16_t TopologyNode::getTotalResources() const
{
    return totalSlots;
}

uint16_t TopologyNode::getOccupiedResources() const
{
...
{
    return std::any_cast<bool>(nodeProperties[NES::Worker::Properties::MAINTENANCE]);
};
...
std::string TopologyNode::getIpAddress() const
{
    return ipAddress;
}
...
{
    return fmt::format("PhysicalNode[id={}, ip={}, resourceCapacity={}, usedResource={}]", workerId, ipAddress, totalSlots, occupiedSlots);
}
