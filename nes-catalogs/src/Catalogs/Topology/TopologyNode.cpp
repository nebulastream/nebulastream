...
TopologyNode::TopologyNode(WorkerId workerId, const std::string &ipAddress,
                           uint32_t grpcPort, uint32_t dataPort,
                           uint16_t totalSlots,
                           const std::map<std::string, std::any> &properties)
    : workerId(workerId), ipAddress(ipAddress), grpcPort(grpcPort),
      dataPort(dataPort), totalSlots(totalSlots), occupiedSlots(0),
      nodeProperties(properties) {}

TopologyNodePtr
TopologyNode::create(WorkerId workerId, const std::string &ipAddress,
                     uint32_t grpcPort, uint32_t dataPort, uint16_t resources,
                     const std::map<std::string, std::any> &properties) {
  NES_DEBUG("Creating node with ID {} and resources {}", workerId, resources);
  return std::make_shared<TopologyNode>(workerId, ipAddress, grpcPort, dataPort,
...
bool TopologyNode::isUnderMaintenance() {
  return std::any_cast<bool>(
      nodeProperties[NES::Worker::Properties::MAINTENANCE]);
};

void TopologyNode::setForMaintenance(bool flag) {
  nodeProperties[NES::Worker::Properties::MAINTENANCE] = flag;
}

bool TopologyNode::releaseSlots(uint16_t freedSlots) {
  NES_DEBUG(
      "Releasing slots {} on topology node {}. Currently occupied {} of {}",
      freedSlots, workerId, occupiedSlots, totalSlots);
  NES_ASSERT(freedSlots <= totalSlots,
             "Amount of slots to free can't be more than actual resources");
  NES_ASSERT(
      freedSlots <= occupiedSlots,
      "Amount of slots to free can't be more than actual consumed resources");
...
bool TopologyNode::occupySlots(uint16_t occupySlots) {
  NES_DEBUG(
      "Reducing slots {} on topology node {}. Currently occupied {} of {}",
      occupySlots, workerId, occupiedSlots, totalSlots);
  if (occupySlots > (totalSlots - occupiedSlots)) {
    NES_WARNING("Amount of resources to be used should not be more than "
                "available resources.");
    return false;
  }
...
TopologyNodePtr TopologyNode::copy() {
  ...
  TopologyNodePtr copy = std::make_shared<TopologyNode>(
      workerId, ipAddress, grpcPort, dataPort, totalSlots, properties);
  copy->occupySlots(occupiedSlots);
  // Copy the link properties
  for (const auto &[nodeId, linkProperty] : this->linkProperties) {
    copy->linkProperties[nodeId] = std::make_shared<LinkProperty>(
        linkProperty->bandwidth, linkProperty->latency);
  }
  return copy;
}
...
std::string TopologyNode::toString() const {
  return fmt::format(
      "PhysicalNode[id={}, ip={}, resourceCapacity={}, usedResource={}]",
      workerId, ipAddress, totalSlots, occupiedSlots);
}

bool TopologyNode::containAsParent(NodePtr parentTopologyNode) {
  std::vector<NodePtr> ancestors = this->getAndFlattenAllAncestors();
  auto found =
      std::find_if(ancestors.begin(), ancestors.end(),
                   [parentTopologyNode](const NodePtr &familyMember) {
                     return familyMember->as<TopologyNode>()->getId() ==
                            parentTopologyNode->as<TopologyNode>()->getId();
                   });
...
bool TopologyNode::containAsChild(NodePtr childTopologyNode) {
  ...
  auto found =
      std::find_if(children.begin(), children.end(),
                   [childTopologyNode](const NodePtr &familyMember) {
                     return familyMember->as<TopologyNode>()->getId() ==
                            childTopologyNode->as<TopologyNode>()->getId();
                   });
  return found != children.end();
}

void TopologyNode::addNodeProperty(const std::string &key,
                                   const std::any &value) {
...
void TopologyNode::addLinkProperty(WorkerId linkedNodeId,
                                   const LinkPropertyPtr &topologyLink) {
...
bool TopologyNode::removeLinkProperty(WorkerId linkedNodeId) {
  if (!linkProperties.contains(linkedNodeId)) {
    NES_WARNING("Link property to node with id='{}' does not exist",
                linkedNodeId);
    return false;
  }
...
void TopologyNode::setSpatialType(
...
Spatial::Experimental::SpatialType TopologyNode::getSpatialNodeType() {
  return std::any_cast<Spatial::Experimental::SpatialType>(
      nodeProperties[NES::Worker::Configuration::SPATIAL_SUPPORT]);
}
} // namespace NES
