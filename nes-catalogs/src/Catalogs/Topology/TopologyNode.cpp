...
TopologyNodePtr TopologyNode::create(
    WorkerId workerId, const std::string& ipAddress, uint32_t grpcPort,
...
bool TopologyNode::occupySlots(uint16_t occupySlots) {
  ...
  if (occupySlots > (totalSlots - occupiedSlots)) {
    NES_WARNING(
        "Amount of resources to be used should not be more than available "
        "resources.");
    return false;
  }
