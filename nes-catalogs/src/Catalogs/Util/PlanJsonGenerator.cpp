...
std::string
PlanJsonGenerator::getOperatorType(const OperatorPtr &operatorNode) {
  ...
  if (operatorNode->instanceOf<SourceLogicalOperator>()) {
    ...
    if (operatorNode->as<SinkLogicalOperator>()
            ->getSinkDescriptor()
...
void PlanJsonGenerator::getChildren(OperatorPtr const &root,
  ...
  NES_DEBUG("UtilityFunctions::getChildren : children size =  {}",
            children.size());
  for (const NodePtr &child : children) {
    ...
    if (childOperatorType == "WINDOW AGGREGATION") {
      // window operator node needs more information, therefore we added
      // information about window type and aggregation
      node["name"] =
          childLogicalOperator->as<LogicalWindowOperator>()->toString();
      NES_DEBUG("{}",
                childLogicalOperator->as<LogicalWindowOperator>()->toString());
    } else {
      // use concatenation of <operator type>(OP-<operator id>) to fill name
      // field e.g. FILTER(OP-1)
      node["name"] = fmt::format("{} (OP-{})", childOperatorType,
                                 childLogicalOperator->getId());
    }
    ...
    if (childOperatorType == "WINDOW AGGREGATION") {
      ...
      edge["source"] = fmt::format("{} (OP-{})", childOperatorType,
                                   childLogicalOperator->getId());
    }
    ...
    if (getOperatorType(root) == "WINDOW AGGREGATION") {
      ...
      edge["target"] =
          fmt::format("{} (OP-{})", getOperatorType(root), root->getId());
    }
...
nlohmann::json
PlanJsonGenerator::getQueryPlanAsJson(const QueryPlanPtr &queryPlan) {

  NES_DEBUG(
      "UtilityFunctions: Getting the json representation of the query plan");
  ...
  if (!root) {
    ...
    NES_DEBUG(
        "UtilityFunctions::getQueryPlanAsJson : root operator is not empty");
