...
QueryState QueryCatalog::getQueryState(QueryId queryId) {
  ...
  if (!lockedQueryCatalogEntryMapping->contains(queryId)) {
    NES_ERROR(
        "QueryCatalogService: Query Catalog does not contains the input "
        "queryId {}",
        queryId);
...
void QueryCatalog::linkSharedQuery(QueryId queryId,
  ...
  if (!lockedSharedQueryCatalogEntryMapping->contains(sharedQueryId)) {
    NES_ERROR(
        "QueryCatalogService: Shared Query Catalog does not contains the input "
        "shared queryId {}",
        sharedQueryId);
    throw Exceptions::QueryNotFoundException(fmt::format(
  ...
  switch (queryStatus) {
    ...
    default:
      throw Exceptions::InvalidQueryStateException(
          {QueryState::REGISTERED, QueryState::OPTIMIZING,
           QueryState::RESTARTING, QueryState::MIGRATING, QueryState::DEPLOYED,
           QueryState::RUNNING, QueryState::STOPPED, QueryState::FAILED,
...
bool QueryCatalog::checkAndMarkQueryForHardStop(QueryId queryId) {
  ...
  if (currentStatus == QueryState::MARKED_FOR_SOFT_STOP ||
    ...
    NES_ERROR(
        "QueryCatalog: Found query status already as {}. Ignoring stop query "
        "request.",
...
void QueryCatalog::checkAndMarkSharedQueryForFailure(
  ...
  if (sharedQueryStatus == QueryState::MARKED_FOR_FAILURE ||
    ...
    NES_WARNING(
        "QueryCatalogService: Query can not be marked for failure as query in "
        "{} status.",
...
bool QueryCatalog::handleDecomposedQueryPlanSoftStopTriggered(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId) {
  NES_DEBUG(
      "Handling soft stop triggered for decomposed query with id {} for shared "
      "query with id {}",
  ...
  if (currentDecomposedQueryState == QueryState::SOFT_STOP_COMPLETED) {
    ...
    NES_WARNING(
        "Received multiple soft stop triggered for decomposed query with id {} "
        "for shared query {}.",
        decomposedQueryPlanId, sharedQueryId);
    NES_WARNING("Skipping remaining operation");
    return false;
...
bool QueryCatalog::handleDecomposedQueryPlanMarkedForSoftStop(
  ...
  if (currentSharedQueryState == QueryState::MARKED_FOR_HARD_STOP ||
    ...
    NES_WARNING(
        "QueryCatalogService: Soft stop can not be initiated as query in {} "
        "status.",
...
bool QueryCatalog::handleDecomposedQueryPlanSoftStopCompleted(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId) {
  NES_DEBUG(
      "QueryCatalogService: Updating the status of decomposed query with id {} "
      "to SOFT_STOP_COMPLETED for shared query "
      "with id {}",
      decomposedQueryPlanId, sharedQueryId);
  ...
  if (currentSharedQueryState != QueryState::MARKED_FOR_SOFT_STOP &&
    ...
    NES_WARNING(
        "Found shared query with id {} in {} but received SOFT_STOP_COMPLETED "
        "for the decomposed query with id {}",
  ...
  if (currentDecomposedQueryState == QueryState::SOFT_STOP_COMPLETED) {
    NES_WARNING(
        "Received multiple soft stop completed for decomposed query with id {} "
        "for shared query {}",
        decomposedQueryPlanId, sharedQueryId);
    NES_WARNING("Skipping remaining operation");
    return false;
  ...
  if (currentSharedQueryState == QueryState::MIGRATING) {
    ...
    for (auto& decomposedQueryPlanMetaDatum :
      ...
      NES_ASSERT(
          currentDecomposedQuerySubPlanStatus == QueryState::RUNNING ||
...
QueryPlanPtr QueryCatalog::getCopyOfLogicalInputQueryPlan(
    const QueryId& queryId) const {
  // Fetch query catalogs
  auto lockedQueryCatalogEntryMapping = queryCatalogEntryMapping.rlock();
  if (!lockedQueryCatalogEntryMapping->contains(queryId)) {
    NES_ERROR(
...
nlohmann::json QueryCatalog::getQueryEntriesWithStatus(
    const std::string& queryState) {
  auto state = magic_enum::enum_cast<QueryState>(queryState);
  if (state.has_value()) {
    NES_INFO(
        "QueryCatalog : fetching all queryIdAndCatalogEntryMapping with status "
        "{}",
        queryState);
    auto lockedQueryCatalogEntryMapping = queryCatalogEntryMapping.wlock();
    nlohmann::json response;
