    ...
    QueryState queryState)
{
    ...
    auto queryCatalogEntry = std::make_shared<QueryCatalogEntry>(queryId, queryString, placementStrategyName, queryPlan, queryState);
...
bool QueryCatalog::checkAndMarkQueryForHardStop(QueryId queryId)
{
    ...
    if (currentStatus == QueryState::MARKED_FOR_SOFT_STOP || currentStatus == QueryState::MARKED_FOR_HARD_STOP
        || currentStatus == QueryState::MARKED_FOR_FAILURE || currentStatus == QueryState::DEPLOYED || currentStatus == QueryState::STOPPED
        || currentStatus == QueryState::FAILED)
    {
        NES_ERROR(
            "QueryCatalog: Found query status already as {}. Ignoring stop query request.",
...
void QueryCatalog::createSharedQueryCatalogEntry(SharedQueryId sharedQueryId, std::set<QueryId> queryIds, QueryState queryStatus)
{
...
void QueryCatalog::updateSharedQueryStatus(SharedQueryId sharedQueryId, QueryState queryState, const std::string & terminationReason)
{
    ...
    if (!lockedSharedQueryCatalogEntryMapping->contains(sharedQueryId))
    {
        ...
        throw Exceptions::QueryNotFoundException(fmt::format("Query Catalog does not contains the input queryId {}", sharedQueryId));
    }
...
void QueryCatalog::checkAndMarkSharedQueryForFailure(SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId)
{
    ...
    {
        NES_WARNING(
            "QueryCatalogService: Query can not be marked for failure as query in {} status.", magic_enum::enum_name(sharedQueryStatus));
        return;
    }
    ...
    switch (newDecomposedQueryState)
    {
        case QueryState::MARKED_FOR_DEPLOYMENT: {
            ...
            sharedQueryCatalogEntry->addDecomposedQueryPlanMetaData(
                decomposedQueryPlanId, decomposedQueryPlanVersion, workerId, newDecomposedQueryState);
            break;
        }
        case QueryState::RUNNING:
        case QueryState::FAILED:
        ...
        case QueryState::SOFT_STOP_TRIGGERED: {
            return handleDecomposedQueryPlanSoftStopTriggered(sharedQueryId, decomposedQueryPlanId);
        }
        case QueryState::MARKED_FOR_SOFT_STOP: {
            return handleDecomposedQueryPlanMarkedForSoftStop(sharedQueryId, decomposedQueryPlanId);
        }
        case QueryState::SOFT_STOP_COMPLETED: {
            return handleDecomposedQueryPlanSoftStopCompleted(sharedQueryId, decomposedQueryPlanId);
        }
        default:
            throw Exceptions::InvalidQueryStateException(
                {QueryState::SOFT_STOP_TRIGGERED, QueryState::SOFT_STOP_COMPLETED}, newDecomposedQueryState);
    }
    return true;
}

bool QueryCatalog::handleDecomposedQueryPlanSoftStopTriggered(SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId)
{
    NES_DEBUG(
        "Handling soft stop triggered for decomposed query with id {} for shared query with id {}", decomposedQueryPlanId, sharedQueryId);
    ...
    {
        NES_WARNING(
            "Received soft stop triggered for decomposed query with id {} for shared query {} but decomposed query is "
    ...
    {
        NES_WARNING(
            "Received multiple soft stop triggered for decomposed query with id {} for shared query {}.",
            decomposedQueryPlanId,
            sharedQueryId);
        NES_WARNING("Skipping remaining operation");
        return false;
...
bool QueryCatalog::handleDecomposedQueryPlanMarkedForSoftStop(SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId)
{
    ...
    {
        NES_WARNING(
            "QueryCatalogService: Soft stop can not be initiated as query in {} status.", magic_enum::enum_name(currentSharedQueryState));
        return false;
    }
...
bool QueryCatalog::handleDecomposedQueryPlanSoftStopCompleted(SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId)
{
    NES_DEBUG(
    ...
    {
        NES_WARNING(
            "Found shared query with id {} in {} but received SOFT_STOP_COMPLETED for the decomposed query with id {}",
            sharedQueryId,
        ...
        NES_ASSERT(
            ...
            "Found query in " << magic_enum::enum_name(currentSharedQueryState)
                              << " but received SOFT_STOP_COMPLETED for the decomposed query with id " << decomposedQueryPlanId
                              << " for shared query id " << sharedQueryId);
    }
    ...
    {
        NES_WARNING(
            "Received multiple soft stop completed for decomposed query with id {} for shared query {}",
            decomposedQueryPlanId,
            sharedQueryId);
        NES_WARNING("Skipping remaining operation");
        return false;
    ...
    else
    {
        ...
        {
            NES_DEBUG(
                "Updating query decomposed query plan status for shared query with id {} and decomposed query plan "
                "with id {} to {}",
                sharedQueryId,
