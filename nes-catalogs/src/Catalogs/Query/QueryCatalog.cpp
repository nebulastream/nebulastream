...
void QueryCatalog::createQueryCatalogEntry(
    const std::string &queryString, const NES::QueryPlanPtr &queryPlan,
  ...
  auto queryCatalogEntry = std::make_shared<QueryCatalogEntry>(
      queryId, queryString, placementStrategyName, queryPlan, queryState);
...
QueryState QueryCatalog::getQueryState(QueryId queryId) {
  ...
  if (!lockedQueryCatalogEntryMapping->contains(queryId)) {
    NES_ERROR("QueryCatalogService: Query Catalog does not contains the input "
              "queryId {}",
              queryId);
    throw Exceptions::QueryNotFoundException(fmt::format(
        "Query Catalog does not contains the input queryId {}", queryId));
  }
  return (*lockedQueryCatalogEntryMapping)[queryId]->getQueryState();
}

void QueryCatalog::linkSharedQuery(QueryId queryId,
  ...
  auto [lockedSharedQueryCatalogEntryMapping, lockedQueryCatalogEntryMapping] =
      folly::acquireLocked(sharedQueryCatalogEntryMapping,
  ...
  if (!lockedSharedQueryCatalogEntryMapping->contains(sharedQueryId)) {
    NES_ERROR("QueryCatalogService: Shared Query Catalog does not contains the "
              "input shared queryId {}",
              sharedQueryId);
    throw Exceptions::QueryNotFoundException(fmt::format(
        "Shared Query Catalog does not contains the input shared queryId {}",
        sharedQueryId));
  }

  auto queryCatalogEntry = (*lockedQueryCatalogEntryMapping)[queryId];
  queryCatalogEntry->setSharedQueryId(sharedQueryId);
  auto sharedQueryCatalogEntry =
      (*lockedSharedQueryCatalogEntryMapping)[sharedQueryId];
...
void QueryCatalog::updateQueryStatus(QueryId queryId, QueryState queryStatus,
  ...
  case QueryState::FAILED: {
    ...
    // Check if query exists
    if (!lockedQueryCatalogEntryMapping->contains(queryId)) {
      NES_ERROR("QueryCatalogService: Query Catalog does not contains the "
                "input queryId {}",
                queryId);
      throw Exceptions::QueryNotFoundException(fmt::format(
          "Query Catalog does not contains the input queryId {}", queryId));
  ...
  default:
    throw Exceptions::InvalidQueryStateException(
        {QueryState::REGISTERED, QueryState::OPTIMIZING, QueryState::RESTARTING,
         QueryState::MIGRATING, QueryState::DEPLOYED, QueryState::RUNNING,
...
bool QueryCatalog::checkAndMarkQueryForHardStop(QueryId queryId) {
  ...
  if (currentStatus == QueryState::MARKED_FOR_SOFT_STOP ||
      currentStatus == QueryState::MARKED_FOR_HARD_STOP ||
      currentStatus == QueryState::MARKED_FOR_FAILURE ||
      currentStatus == QueryState::DEPLOYED ||
      currentStatus == QueryState::STOPPED ||
      currentStatus == QueryState::FAILED) {
    NES_ERROR("QueryCatalog: Found query status already as {}. Ignoring stop "
              "query request.",
              magic_enum::enum_name(queryCatalogEntry->getQueryState()));
    throw Exceptions::InvalidQueryStateException(
        {QueryState::OPTIMIZING, QueryState::REGISTERED, QueryState::DEPLOYED,
         QueryState::RUNNING, QueryState::RESTARTING},
        currentStatus);
  }
...
void QueryCatalog::createSharedQueryCatalogEntry(SharedQueryId sharedQueryId,
  ...
  NES_INFO("Create shared query catalog entry for the shared query with id {}",
           sharedQueryId);
  auto sharedQueryCatalogEntry = std::make_shared<SharedQueryCatalogEntry>(
      sharedQueryId, queryIds, queryStatus);
  auto lockedSharedQueryCatalogEntryMapping =
      sharedQueryCatalogEntryMapping.wlock();
  (*lockedSharedQueryCatalogEntryMapping)[sharedQueryId] =
      std::move(sharedQueryCatalogEntry);
}

void QueryCatalog::updateSharedQueryStatus(
    SharedQueryId sharedQueryId, QueryState queryState,
    const std::string &terminationReason) {

  NES_DEBUG("Updating status of the shared query plan {} to {}", sharedQueryId,
  ...
  if (!lockedSharedQueryCatalogEntryMapping->contains(sharedQueryId)) {
    NES_ERROR("QueryCatalogService: Query Catalog does not contains the input "
              ...
              sharedQueryId);
    throw Exceptions::QueryNotFoundException(fmt::format(
        "Query Catalog does not contains the input queryId {}", sharedQueryId));
  }
    ...
    for (const auto &containedQueryId : containedQueryIds) {
      if (lockedQueryCatalogEntryMapping->contains(containedQueryId)) {
        auto queryCatalogEntry =
            (*lockedQueryCatalogEntryMapping)[containedQueryId];
  ...
  case QueryState::FAILED: {
    ...
    for (const auto &decomposedQueryPlanMetaData :
  ...
  default:
    throw Exceptions::InvalidQueryStateException(
        {QueryState::RESTARTING, QueryState::DEPLOYED, QueryState::STOPPED,
         QueryState::RUNNING, QueryState::OPTIMIZING, QueryState::FAILED},
        queryState);
  }
}

void QueryCatalog::checkAndMarkSharedQueryForFailure(
  ...
  if (!lockedSharedQueryCatalogEntryMapping->contains(sharedQueryId)) {
    ...
    throw Exceptions::QueryNotFoundException(fmt::format(
        "Unable to find the shared query plan with id {}", sharedQueryId));
  }

  auto sharedQueryCatalogEntry =
      (*lockedSharedQueryCatalogEntryMapping)[sharedQueryId];

  // If query is doing hard stop or has failed or already stopped then soft stop
  // can not be triggered
  auto sharedQueryStatus = sharedQueryCatalogEntry->getQueryState();
  if (sharedQueryStatus == QueryState::MARKED_FOR_FAILURE ||
      sharedQueryStatus == QueryState::FAILED ||
      sharedQueryStatus == QueryState::STOPPED) {
    NES_WARNING("QueryCatalogService: Query can not be marked for failure as "
                "query in {} status.",
  ...
  NES_INFO("QueryCatalogService: Shared query id {} is marked as failed",
           sharedQueryId);
}

bool QueryCatalog::updateDecomposedQueryPlanStatus(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId,
    DecomposedQueryPlanVersion decomposedQueryPlanVersion,
    QueryState newDecomposedQueryState, WorkerId workerId) {
  ...
  case QueryState::MARKED_FOR_DEPLOYMENT: {
    ...
    if (!lockedSharedQueryCatalogEntryMapping->contains(sharedQueryId)) {
      ...
      throw Exceptions::QueryNotFoundException(
          fmt::format("Query Catalog does not contains the input queryId {}",
                      sharedQueryId));
    }
    auto sharedQueryCatalogEntry =
        (*lockedSharedQueryCatalogEntryMapping)[sharedQueryId];
    sharedQueryCatalogEntry->addDecomposedQueryPlanMetaData(
        decomposedQueryPlanId, decomposedQueryPlanVersion, workerId,
  ...
  case QueryState::MARKED_FOR_MIGRATION: {
    ...
    auto sharedQueryCatalogEntry =
        (*lockedSharedQueryCatalogEntryMapping)[sharedQueryId];
    auto decomposedQueryPlanMetaData =
  ...
  case QueryState::SOFT_STOP_TRIGGERED: {
    return handleDecomposedQueryPlanSoftStopTriggered(sharedQueryId,
                                                      decomposedQueryPlanId);
  }
  case QueryState::MARKED_FOR_SOFT_STOP: {
    return handleDecomposedQueryPlanMarkedForSoftStop(sharedQueryId,
                                                      decomposedQueryPlanId);
  }
  case QueryState::SOFT_STOP_COMPLETED: {
    return handleDecomposedQueryPlanSoftStopCompleted(sharedQueryId,
                                                      decomposedQueryPlanId);
  }
  default:
    throw Exceptions::InvalidQueryStateException(
        {QueryState::SOFT_STOP_TRIGGERED, QueryState::SOFT_STOP_COMPLETED},
        newDecomposedQueryState);
  }
  return true;
}

bool QueryCatalog::handleDecomposedQueryPlanSoftStopTriggered(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId) {
  NES_DEBUG("Handling soft stop triggered for decomposed query with id {} for "
            "shared query with id {}",
  ...
  auto currentDecomposedQueryState =
      decomposedQueryPlanMetaData->getDecomposedQueryPlanStatus();

  if (currentDecomposedQueryState == QueryState::SOFT_STOP_COMPLETED) {
    NES_WARNING("Received soft stop triggered for decomposed query with id {} "
                "for shared query {} but decomposed query is "
    ...
    NES_WARNING("Received multiple soft stop triggered for decomposed query "
                "with id {} for shared query {}.",
                decomposedQueryPlanId, sharedQueryId);
    NES_WARNING("Skipping remaining operation");
    return false;
  } else if (currentDecomposedQueryState == QueryState::MARKED_FOR_SOFT_STOP) {
    NES_WARNING(
        "Received soft stop triggered for decomposed query with id {} for "
        "shared query {} but decomposed query is "
...
bool QueryCatalog::handleDecomposedQueryPlanMarkedForSoftStop(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId) {
  ...
  if (currentSharedQueryState == QueryState::MARKED_FOR_HARD_STOP ||
      currentSharedQueryState == QueryState::FAILED ||
      currentSharedQueryState == QueryState::STOPPED) {
    NES_WARNING("QueryCatalogService: Soft stop can not be initiated as query "
                "in {} status.",
  ...
  sharedQueryCatalogEntry->getDecomposedQueryPlanMetaData(decomposedQueryPlanId)
      ->updateState(QueryState::MARKED_FOR_SOFT_STOP);
  ...
  NES_INFO("QueryCatalogService: Shared query id {} is marked as soft stopped",
           sharedQueryId);
  return true;
}

bool QueryCatalog::handleDecomposedQueryPlanSoftStopCompleted(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId) {
  NES_DEBUG("QueryCatalogService: Updating the status of decomposed query with "
            "id {} to SOFT_STOP_COMPLETED for shared query "
            "with id {}",
            decomposedQueryPlanId, sharedQueryId);
  ...
  // todo #4396: when query migration has its own termination type, this
  // function will not be called during migration. remove MIGRATING below
  if (currentSharedQueryState != QueryState::MARKED_FOR_SOFT_STOP &&
      currentSharedQueryState != QueryState::MIGRATING) {
    NES_WARNING("Found shared query with id {} in {} but received "
                "SOFT_STOP_COMPLETED for the decomposed query with id {}",
                sharedQueryId, magic_enum::enum_name(currentSharedQueryState),
                decomposedQueryPlanId);
    // FIXME: #4396 fix what to do when this occurs
    NES_ASSERT(false, "Found query in "
                          << magic_enum::enum_name(currentSharedQueryState)
                          << " but received SOFT_STOP_COMPLETED for the "
                             "decomposed query with id "
                          << decomposedQueryPlanId << " for shared query id "
                          << sharedQueryId);
  }
  ...
  auto currentDecomposedQueryState =
      decomposedQueryPlanMetaData->getDecomposedQueryPlanStatus();
  if (currentDecomposedQueryState == QueryState::SOFT_STOP_COMPLETED) {
    NES_WARNING("Received multiple soft stop completed for decomposed query "
                "with id {} for shared query {}",
                decomposedQueryPlanId, sharedQueryId);
    NES_WARNING("Skipping remaining operation");
    return false;
  ...
  if (currentSharedQueryState == QueryState::MIGRATING) {
    ...
    /* receiving a soft stop for a query sub plan of a migrating query marks the
     * completion of the migration of that specific query sub plan.*/
    // todo #4396: once a specifig drain EOS is implemented, remove this as the
    // received state should already equal migration completed
    decomposedQueryPlanMetaData->updateState(QueryState::MIGRATION_COMPLETED);
    /*if the query is in MIGRATING status, check if there are still any
     * querySubPlans that did not finish their migration. If no subplan in state
     * MIGRATING could be found, the migration of the whole query is complete
     * and the state is set to RUNNING again */
    bool queryMigrationComplete = true;
    for (auto &decomposedQueryPlanMetaDatum :
         sharedQueryCatalogEntry->getAllDecomposedQueryPlanMetaData()) {
      auto currentDecomposedQuerySubPlanStatus =
          decomposedQueryPlanMetaDatum->getDecomposedQueryPlanStatus();
      ...
      NES_ASSERT(currentDecomposedQuerySubPlanStatus == QueryState::RUNNING ||
                     currentDecomposedQuerySubPlanStatus ==
                         QueryState::SOFT_STOP_COMPLETED ||
                     currentDecomposedQuerySubPlanStatus ==
                         QueryState::MIGRATION_COMPLETED ||
                     currentDecomposedQuerySubPlanStatus ==
                         QueryState::REDEPLOYED,
                 "Unexpected decomposed query plan status.");
    }
    if (queryMigrationComplete) {
      NES_DEBUG(
          "Migration completed. Marking shared query with id {} as running.",
          sharedQueryId);
      sharedQueryCatalogEntry->setQueryState(QueryState::RUNNING);
      for (auto containedQueryId :
           sharedQueryCatalogEntry->getContainedQueryIds()) {
    ...
    // Check if all decomposed queries are stopped when a decomposed query soft
    // stop completes
    bool stopQuery = true;
    for (auto &decomposedQueryPlanMetaDatum :
         sharedQueryCatalogEntry->getAllDecomposedQueryPlanMetaData()) {
      NES_DEBUG(
          "Updating query decomposed query plan status for shared query with "
          "id {} and decomposed query plan "
          "with id {} to {}",
          sharedQueryId,
          decomposedQueryPlanMetaDatum->getDecomposedQueryPlanId(),
          std::string(magic_enum::enum_name(
              decomposedQueryPlanMetaDatum->getDecomposedQueryPlanStatus())));
      if (decomposedQueryPlanMetaDatum->getDecomposedQueryPlanStatus() !=
          QueryState::SOFT_STOP_COMPLETED) {
        stopQuery = false;
        break;
      }
    }
    // Mark the shared query and contained queries as stopped if all decomposed
    // query plans are stopped
    if (stopQuery) {
...
void QueryCatalog::addUpdatedQueryPlan(QueryId queryId, std::string step,
...
QueryPlanPtr
QueryCatalog::getCopyOfLogicalInputQueryPlan(const QueryId &queryId) const {
  // Fetch query catalogs
  auto lockedQueryCatalogEntryMapping = queryCatalogEntryMapping.rlock();
  if (!lockedQueryCatalogEntryMapping->contains(queryId)) {
    NES_ERROR("QueryCatalogService: Query Catalog does not contains the input "
              ...
              queryId);
    throw Exceptions::QueryNotFoundException(
        "Query Catalog does not contains the input queryId " +
        queryId.toString());
  }
...
nlohmann::json QueryCatalog::getQueryEntry(QueryId queryId) {
  ...
  for (auto const &[phaseName, queryPlan] :
       queryCatalogEntry->getOptimizationPhases()) {
...
nlohmann::json
QueryCatalog::getQueryEntriesWithStatus(const std::string &queryState) {

  auto state = magic_enum::enum_cast<QueryState>(queryState);
  if (state.has_value()) {
    NES_INFO("QueryCatalog : fetching all queryIdAndCatalogEntryMapping with "
             "status {}",
             queryState);
    auto lockedQueryCatalogEntryMapping = queryCatalogEntryMapping.wlock();
    nlohmann::json response;
    for (auto const &[queryId, queryCatalogEntry] :
         (*lockedQueryCatalogEntryMapping)) {
