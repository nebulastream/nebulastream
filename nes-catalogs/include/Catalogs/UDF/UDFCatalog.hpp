...
/**
 * @brief The UDF catalog stores all the data required to execute a Java UDF
...
class UDFCatalog {
 public:
  ...
  /**
   * @brief Register the descriptor data of a UDF. UDFs can not share the same
   * name, even if they are implemented in different programming languages.
   * @param name The name of the UDF as it is used in
   * queryIdAndCatalogEntryMapping.
   * @param descriptor The implementation data of the UDF.
   * @throws UdfException If descriptor is a nullptr or if a UDF under the name
   ...
   * @param name The name of the UDF as it is used in
   * queryIdAndCatalogEntryMapping.
   * @return The implementation data of the UDF, or nullptr if the UDF is not
   * registered.
   */
   ...
   *
   * Removing an unregistered UDF is not an error condition because it could
   * have been removed by another user. In this case, the user should just be
   * notified.
   */
  ...
  // I'd rather return an iterator over the keys. Or maybe we don't need this
  // function at all. It's not really used to support UDFs. It could be used by
  // a visualization tool. It would be nice to show more information about the
  // UDFs (signature, size of dependencies), but that's not needed right now.
