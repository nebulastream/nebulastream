...
/**
 * This class processes predicted topology changes and allows inspecting the
 * exepcted future states of the topology based on those predictions.
 * Predictions can be added by supplying a TopologyDelta containing the expected
 * link changes for a certain point in time. Predictions can be removed by
 * supplying a delta containing all link changes that are to be removed.
 * Topology versions for a specified time are created by making a copy of the
 * existing topology and applying all changes expected to happen before the
...
class TopologyTimeline {
 public:
  ...
  /**
   * @brief adds a topology delta to the changes expected to happen at a
  ...
  /**
   * @brief return a new topology object which represents the expected state of
   * the topology at the specified time
   * @param time the point in time for which the expected state should be
   * returned
   * @return a copy of the original topology onto which all changes, which are
   * expected to happen before the specified time, have been applied
   */
  TopologyPtr getTopologyVersion(Timestamp time);

  /**
   * @brief removes a topology delta from the changes expected to happen at a
   * specific point in time
   * @param predictedTime the time at which the changes are expected to happen
   * @param delta a delta containing the edges to be removed or added
   */
 ...
 private:
  /**
   * @brief create a new topology representing the state which would result if
   * all changes in the changelog were applied onto the original topology
   * @param changeLog a change log containing the changes to be applied
   * @return a pointer to the new topology object onto which the changes are
   * applied
   */
  ...
  /**
   * @brief creates an aggregated changelog representing the difference between
   * the original topology and the expected state at the specified time
   * @param time all changelogs which are less or equal to this time will be
