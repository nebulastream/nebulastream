 ...
 * changelog for a certain point in time. It consists of prediction from one or
 * more nodes.
 */
class TopologyDelta {
public:
  /**
   * @brief constructor
   * @param added a list of edges to be added to the topology. It is the callers
   * responsibility to ensure that this list does not contain any duplicates
   * @param removed a list of edges to be removed from the topology. It is the
   * callers responsibility to ensure that this list does not contain any
   * duplicates
   */
  TopologyDelta(std::vector<TopologyLinkInformation> added,
                std::vector<TopologyLinkInformation> removed);

  /**
   * @brief this function returns a string visualizing the content of the
   * changelog
   * @return a string in the format "added: {CHILD->PARENT, CHILD->PARENT, ...}
   * removed: {CHILD->PARENT, CHILD->PARENT, ...}"
   */
...
private:
   ...
   */
  static std::string topologyLinkInformationListToString(
