...
/**
 * @brief This class represents information about a physical node participating
 * in the NES infrastructure
 */
class TopologyNode : public Node {

public:
  static TopologyNodePtr
  create(WorkerId workerId, const std::string &ipAddress, uint32_t grpcPort,
         uint32_t dataPort, uint16_t resources,
         const std::map<std::string, std::any> &properties);

  explicit TopologyNode(WorkerId workerId, const std::string &ipAddress,
   ...
   * Note: It is the responsibility of the callee class to call releaseLock
   * after the processing at the callee is done.
   ...
   * Note: It is the responsibility of the callee class to have called
   * acquireLock before calling this method.
  ...
  /**
   * @brief Create a shallow copy of the physical node i.e. without copying the
   ...
   * @param linkedTopologyNodeId topology node to which the property will be
   * associated
   * @param linkProperty the link property
   */
  void addLinkProperty(WorkerId linkedTopologyNodeId,
                       const LinkPropertyPtr &linkProperty);

  /**
   * @brief get a the value of a link property
   * @param linkedTopologyNodeId topology node associated with the link property
   * to retrieve
   ...
   * @param linkedNodeId topology node associated with the link property to
   * remove
   * @return true if the removal is successful
   */
  ...
  /**
   * Experimental
   * @brief sets the status of the node as running on a mobile device or a fixed
   * location device. To be run right after node creation. Fixed nodes should
   * not become mobile or vice versa at a later point
   * @param spatialType
   */
...
private:
  ...
  /**
   * @brief A field to store a map of the id of the linked nodes and its link
   * property
   */
...
namespace fmt {
template <> struct formatter<NES::TopologyNode> : formatter<std::string> {
  auto format(const NES::TopologyNode &topology_node, format_context &ctx)
      -> decltype(ctx.out()) {
    return fmt::format_to(
        ctx.out(), "{} {} {} {} {}", topology_node.getId(),
        topology_node.getIpAddress(), topology_node.getGrpcPort(),
        topology_node.getDataPort(), topology_node.getAvailableResources());
  }
};
} // namespace fmt
