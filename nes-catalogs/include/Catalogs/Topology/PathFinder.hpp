...
/**
 * @brief This is a utility class that operates on the topology nodes to find
 * different types of topology nodes between given pair of topology nodes.
 */
class PathFinder {
...
public:
   ...
   * @return the immediate common ancestor (when many then the first identified
   * ancestor is given) else nullptr.
   */
  TopologyNodePtr
  findCommonAncestor(std::vector<TopologyNodePtr> topologyNodes);

  /**
   * @brief Find the immediate common child for the set of Topology nodes
   * @param topologyNodes: the set of topology nodes
   * @return the immediate common child (when many then the first identified
   * ancestor is given) else nullptr.
   */
   ...
   * @return common node (when many then the first identified ancestor is given)
   * else nullptr.
   */
  TopologyNodePtr
  findCommonNodeBetween(std::vector<TopologyNodePtr> childNodes,
                        std::vector<TopologyNodePtr> parenNodes);

  /**
   * @brief Find the set of nodes (inclusive of) between a source and
   * destination topology node
   * @param sourceNode : the source topology node
   * @param destinationNode : the destination topology node
   * @return returns a vector of nodes (inclusive of) between a source and
   * destination topology node if no path exists then an empty vector
   */
  std::vector<TopologyNodePtr>
  findNodesBetween(const TopologyNodePtr &sourceNode,
                   const TopologyNodePtr &destinationNode);

  /**
   * @brief Find the set of shared nodes (inclusive of) between a set of source
   * and destination topology nodes
   ...
   */
  std::vector<TopologyNodePtr>
  findNodesBetween(std::vector<TopologyNodePtr> sourceNodes,
