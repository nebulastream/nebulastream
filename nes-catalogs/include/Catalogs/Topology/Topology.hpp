...
using TopologyNodeWLock =
    std::shared_ptr<folly::Synchronized<TopologyNodePtr>::WLockedPtr>;

/**
 * @brief This class represents the overall physical infrastructure with
 * different nodes
 */
class Topology {
...
public:
   ...
   */
  WorkerId
  registerWorkerAsRoot(WorkerId newRootWorkerId, const std::string &address,
                       const int64_t grpcPort, const int64_t dataPort,
                       const uint16_t numberOfSlots,
                       std::map<std::string, std::any> workerProperties,
                       uint32_t bandwidthInMbps, uint32_t latencyInMs);
   ...
   * @return worker id
   */
  WorkerId registerWorker(WorkerId workerId, const std::string &address,
                          const int64_t grpcPort, const int64_t dataPort,
                          const uint16_t numberOfSlots,
                          std::map<std::string, std::any> workerProperties,
                          uint32_t bandwidthInMbps, uint32_t latencyInMs);
  ...
  /**
   * @brief returns a vector of parent topology node ids connected to the
   * specified topology node
  ...
  /**
   * @brief returns a vector of child topology node ids connected to the
   * specified topology node
   * @param nodeId: id of the specified topology node
   * @return vector of parent node ids
   */
  ...
  /**
   * @brief This method will add the a topology node as child to the parent with
   * provided Id
   ...
   * @warning If the root node is removed then the next worker that sends a
   * register call will become the root
   ...
   * @return true if successful
   */
  bool removeTopologyNodeAsChild(WorkerId parentWorkerId,
   ...
   */
  bool addLinkProperty(WorkerId parentWorkerId, WorkerId childWorkerId,
                       uint64_t bandwidthInMBPS, uint64_t latencyInMS);

  /**
   * @brief This method will find ad return a copy of the topology node with
   * given worker id
   * @param workerId : the id of the topology node
  ...
  /**
   * @brief This method will return a "duplicate" subgraph containing all the
   * paths between start and destination node. Note: this method will only look
   * for the destination nodes that are parent nodes of the source node.
   * @param sourceTopologyNodeId: the source topology node
   * @param destinationTopologyNodeId: the destination topology node
   * @return topology nodes representing the source nodes of the returned
   * subgraph.
   */
  std::optional<TopologyNodePtr>
  findAllPathBetween(WorkerId sourceTopologyNodeId,
                     WorkerId destinationTopologyNodeId);

  /**
   * @brief Find a sub-graph such that each start node in the given set of start
   * nodes can connect to each destination node in the given set of destination
   * nodes. Note: the returned sub graph is a duplicate of the original sub
   * graph. During the
   * @param sourceTopologyNodeIds: the topology nodes where to start the path
   * identification
   * @param destinationTopologyNodeIds: the topology nodes where to end the path
   * identification
   * @return a vector of start/upstream topology nodes of the sub-graph if all
   * start nodes can connect to all destination nodes else an empty vector
   */
  std::vector<TopologyNodePtr>
  findPathBetween(const std::vector<WorkerId> &sourceTopologyNodeIds,
   ...
   */
  std::vector<
      std::pair<WorkerId, NES::Spatial::DataTypes::Experimental::GeoLocation>>
  getTopologyNodeIdsInRange(
      NES::Spatial::DataTypes::Experimental::GeoLocation center, double radius);
  ...
  getGeoLocationForNode(WorkerId nodeId);
  ...
  /**
   * Get a json containing the id and the location of any node. In case the node
   is neither a field nor a mobile node,
  ...
  /**
   * @brief get a list of all mobile nodes in the system with known locations
   * and their current positions as well as their parent nodes. Mobile nodes
   * without known locations will not appear in the list
  ...
  /**
   * @brief starts a BFS at the node with the supplied id and traverses
   * downstream until either no all downstream roots are reached or all of the
   * supplied target nodes have been found
   * @param startNode the starting point of the BFS
   * @param reachableDownstreamNodes reference to a into which the ids of all
   * visited nodes will be inserted
   * @param targetNodes a list of node ids. Once all nodes in this list have
   * been visited the function will return, even if there are further downstream
   * nodes that have not been visited
   * @return true if all needed locks could be acquired, false if the iteration
   * was aborted because a node could not be locked
   */
...
private:
   ...
   * ]
   */
  static nlohmann::json convertLocationToJson(
      NES::Spatial::DataTypes::Experimental::GeoLocation geoLocation);

  /**
   * Use a node id and a Location to construct a Json representation containing
   these values.
   * @param workerId : the nodes id
   * @param geoLocation : the nodes location. if this is a nullptr then the
   "location" attribute of the returned json will be null.
   * @return a json in the format:
      {
          "id": <node id>,
          "location": [
   ...
   */
  static nlohmann::json convertNodeLocationInfoToJson(
      WorkerId workerId,
      NES::Spatial::DataTypes::Experimental::GeoLocation geoLocation);
   ...
   */
  static std::vector<TopologyNodePtr>
  mergeSubGraphs(const std::vector<TopologyNodePtr> &startNodes);

  /**
   * @brief Find if searched nodes are the parent or grand parents of the input
   * test node
   * @param testNode: the test node
   * @param searchedNodes: the searched node
   * @param uniqueNodes: map of all unique worker id to topology nodes observed
   * during the iteration
   * @return the node where the searched node is found
   */
  TopologyNodePtr find(TopologyNodePtr testNode,
                       std::vector<TopologyNodePtr> searchedNodes,
                       std::map<WorkerId, TopologyNodePtr> &uniqueNodes);

  /**
   * @brief method to generate the next (monotonically increasing) topology node
   * id
  ...
  folly::Synchronized<std::map<WorkerId, folly::Synchronized<TopologyNodePtr>>>
      workerIdToTopologyNode;
  folly::Synchronized<NES::Spatial::Index::Experimental::LocationIndexPtr>
      locationIndex;
  static constexpr int BASE_MULTIPLIER = 10000;
  std::atomic_uint64_t topologyNodeIdCounter =
      INITIAL_WORKER_NODE_ID.getRawValue();
};
} // namespace NES
