...
/**
 * @brief catalog class to handle the queryIdAndCatalogEntryMapping in the
 * system
 * @note: This class is not thread safe. Please use QueryCatalogService to
 * access this object.
 */
class QueryCatalog {
 public:
   ...
   * @param placementStrategyName: the placement strategy (e.g. bottomUp,
   * topDown, etc)
   * @param queryState: the state of the query
   */
  void createQueryCatalogEntry(
      const std::string& queryString, const QueryPlanPtr& queryPlan,
   ...
   */
  void updateQueryStatus(QueryId queryId, QueryState queryStatus,
   ...
   */
  void createSharedQueryCatalogEntry(SharedQueryId sharedQueryId,
   ...
   * @return true if updated successfully
   */
  void updateSharedQueryStatus(SharedQueryId sharedQueryId,
                               QueryState queryState,
                               const std::string& terminationReason);
   ...
   */
  bool handleDecomposedQueryPlanMarkedForSoftStop(
   ...
   */
  bool handleDecomposedQueryPlanSoftStopCompleted(
      SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId);
   ...
   * @return true if handled or false
   */
  bool handleDecomposedQueryPlanSoftStopTriggered(
      SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId);
   ...
   */
  void checkAndMarkSharedQueryForFailure(
      SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId);
   ...
   */
  void addUpdatedQueryPlan(QueryId queryId, std::string step,
                           QueryPlanPtr updatedQueryPlan);
  ...
  bool updateDecomposedQueryPlanStatus(
      ...
      QueryState decomposedQueryState, WorkerId workerId);
  ...
  /**
   * @brief Gets a copy of the logical input query plan without any rules
   * applied
   * @param queryId
 ...
 private:
  folly::Synchronized<std::map<QueryId, QueryCatalogEntryPtr>>
      queryCatalogEntryMapping;
  folly::Synchronized<std::map<SharedQueryId, SharedQueryCatalogEntryPtr>>
      sharedQueryCatalogEntryMapping;
};
