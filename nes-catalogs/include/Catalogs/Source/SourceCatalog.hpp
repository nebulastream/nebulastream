...
class SourceCatalog {
 public:
   ...
   * @caution this method only remove the entry from the catalog not from the
   * topology
   ...
   */
  bool addPhysicalSource(const std::string& logicalSourceName,
   ...
   */
  bool removePhysicalSource(const std::string& logicalSourceName,
  ...
  /**
   * @brief method to return the source for an existing logical source or throw
   * exception
   ...
   */
  LogicalSourcePtr getLogicalSourceOrThrowException(
  ...
  /**
   * @brief return all topology node ids that host physical sources that
   * contribute to this logical source
   * @param logicalSourceName name of logical source
   * @return list of topology nodes ids
   */
  std::vector<WorkerId> getSourceNodesForLogicalSource(
      const std::string& logicalSourceName);
   ...
   */
  std::vector<SourceCatalogEntryPtr> getPhysicalSources(
      const std::string& logicalSourceName);
 ...
 private:
   ...
   * @return bool indicating if source exists
   */
  bool testIfLogicalSourceExistsInLogicalToPhysicalMapping(
      const std::string& logicalSourceName);
  ...
  std::map<std::string, std::vector<SourceCatalogEntryPtr>>
