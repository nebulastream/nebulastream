...
{
public:
    // WASM patch: on wasm32, 'unsigned long' is distinct from uint32_t (unsigned int)
    // and uint64_t (unsigned long long), so we add it to the variant to prevent
    // template instantiation failures in ConfigParameter<unsigned long>::validate().
#if defined(__wasm32__) || defined(__EMSCRIPTEN__)
    using ConfigType = std::variant<int32_t, uint32_t, int64_t, uint64_t, unsigned long, bool, char, float, double, std::string, EnumWrapper>;
#else
    ...
    struct ConfigParameter
    {
        ...
        {
            auto result = this->validateFunc(config);
            if (result.has_value())
                return ConfigType(result.value());
            return std::nullopt;
        }
    ...
    public:
        ...
        struct ConfigParameterModel : ConfigParameterConcept
        {
            ...
            std::optional<ConfigType> getDefaultValue() const override {
                if (configParameter.defaultValue.has_value())
                    return ConfigType(configParameter.defaultValue.value());
                return std::nullopt;
            }
