...
- [ ] **SNK-01**: AsyncSink Rust trait with open/write/flush/close lifecycle methods
- [ ] **SNK-02**: SinkContext providing CancellationToken for cooperative shutdown
- [ ] **SNK-03**: Per-sink bounded async channel from C++ pipeline thread to Rust async task
- [ ] **SNK-04**: spawn_sink/stop_sink lifecycle management on shared Tokio runtime

### Pipeline Integration

- [ ] **PLN-01**: TokioSink C++ operator implementing ExecutablePipelineStage (execute/stop)
- [ ] **PLN-02**: Non-blocking try_send with repeatTask retry when channel is full
- [ ] **PLN-03**: BackpressureController with hysteresis thresholds (apply/release pressure)
- [ ] **PLN-04**: Guaranteed flush on pipeline stop via repeatTask polling until Rust confirms drain

### Registration

- [ ] **REG-01**: TokioSinkRegistry for string-name factory registration (mirrors TokioSourceRegistry)

### Validation

- [ ] **VAL-01**: AsyncFileSink reference implementation using tokio::fs async file I/O
...
|-------------|-------|--------|
| SNK-01 | Phase 4 | Pending |
| SNK-02 | Phase 4 | Pending |
| SNK-03 | Phase 4 | Pending |
| SNK-04 | Phase 4 | Pending |
| PLN-01 | Phase 5 | Pending |
| PLN-02 | Phase 5 | Pending |
| PLN-03 | Phase 5 | Pending |
| PLN-04 | Phase 5 | Pending |
