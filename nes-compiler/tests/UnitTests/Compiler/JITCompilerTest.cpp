...
static constexpr auto TEST_FILE_BODY =
    "/*\n"
    "    Copyright (C) 2020 by the NebulaStream project "
    "(https://nebula.stream)\n"
    "\n"
    "    Licensed under the Apache License, Version 2.0 (the \"License\");\n"
    "    you may not use this file except in compliance with the License.\n"
...
TEST_F(JITCompilerTest, compileCppCode) {
  ...
  auto request = CompilationRequest::create(std::move(sourceCode), "test_1",
  ...
  auto mulFunction = compilationResult.getDynamicObject()
                         ->getInvocableMember<int (*)(int, int)>("_Z3mulii");
