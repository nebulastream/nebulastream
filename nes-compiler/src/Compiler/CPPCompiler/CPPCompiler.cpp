...
const std::string NESCoreIncludePath =
    PATH_TO_NES_SOURCE_CODE "/nes-core/include/";
const std::string NESCommonIncludePath =
    PATH_TO_NES_SOURCE_CODE "/nes-common/include/";
...
CPPCompiler::CPPCompiler()
    : format(std::make_unique<ClangFormat>("cpp")),
      runtimePathConfig(ExecutablePath::loadRuntimePathConfig()) {}

CPPCompiler::~CPPCompiler() noexcept { NES_DEBUG("~CPPCompiler"); }

Language CPPCompiler::getLanguage() const { return Language::CPP; }

CompilationResult CPPCompiler::compile(
    std::shared_ptr<const CompilationRequest> request) const {
  // Compile and load shared library.
  Timer timer("CPPCompiler");
  timer.start();

  std::string fileName =
      (std::filesystem::temp_directory_path() / request->getName());
...
#error "Unknown platform"
  ...
  NES_ASSERT2_FMT(sourceCode.size(),
                  "empty source code for " << sourceFileName);
  ...
  // the log level of the compiled code is the same as the currently selected
  // log level of the runtime.
  auto logLevel = getLogLevel(Logger::getInstance()->getCurrentLogLevel());
  compilationFlags.addFlag("-DFMT_HEADER_ONLY"s);
  compilationFlags.addFlag("-DNES_COMPILE_TIME_LOG_LEVEL=" +
  ...
  timer.pause();
  NES_INFO("[CPPCompiler] Compilation time: {}ms",
           (double)timer.getRuntime() / (double)1000000);
