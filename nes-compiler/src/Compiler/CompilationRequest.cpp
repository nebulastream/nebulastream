...
CompilationRequest::CompilationRequest(
    std::unique_ptr<SourceCode> sourceCode, std::string name,
    bool profileCompilation, bool profileExecution, bool optimizeCompilation,
    bool debug, std::vector<std::shared_ptr<ExternalAPI>> externalApis)
    : sourceCode(std::move(sourceCode)), name(std::move(name)),
...
std::shared_ptr<CompilationRequest> CompilationRequest::create(
    std::unique_ptr<SourceCode> sourceCode, std::string identifier,
    bool profileCompilation, bool profileExecution, bool optimizeCompilation,
    bool debug, std::vector<std::shared_ptr<ExternalAPI>> externalApis) {
  ...
  requestName << identifier << "_"
              << std::put_time(&localtime, "%d-%m-%Y_%H-%M-%S") << "_"
              << dist(rng);

  return std::make_shared<CompilationRequest>(
      std::move(sourceCode), requestName.str(), profileCompilation,
      profileExecution, optimizeCompilation, debug, externalApis);
};
...
CompilationRequest::getExternalAPIs() const {
  return externalApis;
}
...
bool CompilationRequest::operator==(const CompilationRequest &rhs) const {
  return sourceCode == rhs.sourceCode && name == rhs.name &&
         profileCompilation == rhs.profileCompilation &&
         profileExecution == rhs.profileExecution &&
         optimizeCompilation == rhs.optimizeCompilation && debug == rhs.debug;
}
bool CompilationRequest::operator!=(const CompilationRequest &rhs) const {
  return !(rhs == *this);
}
