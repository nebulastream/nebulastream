...
std::string DEFAULT_PUBLIC_INCLUDE_DIR_UNIX_INSTALL =
...
bool isInLocalInstallDir() {
  ...
  return std::filesystem::exists(executablePath.append("bin")) &&
         std::filesystem::exists(executablePath.append("lib")) &&
...
RuntimePathConfig loadRuntimePathConfig() {
  ...
  if (isInUNIXInstallDir()) {
    ...
    runtimePathConfig.includePaths.push_back(
    ...
    runtimePathConfig.includePaths.push_back(
        executablePath.append("include/nebulatstream"));
    ...
    const std::string dataTypesBinaryDir =
        PATH_TO_BINARY_DIR "/nes-data-types/";
    ...
    runtimePathConfig.includePaths.push_back(PATH_TO_BINARY_DIR
                                             "/include/nebulastream");
  } else {
  ...
  if (!std::filesystem::exists(runtimePathConfig.clangBinaryPath)) {
    throw CompilerException(
        "Selected clang binary path dose not exists. Path: " +
...
std::filesystem::path recursiveFindFileReverse(
    std::filesystem::path currentPath, const std::string targetFileName) {
...
std::filesystem::path getLibPath(std::string libName) {
  ...
  if (std::filesystem::is_regular_file(libPath)) {
    ...
    NES_DEBUG(
        "Invalid {} file found at {}. Searching next in DYLD_LIBRARY_PATH.",
        libName, libPathStr.str());
  ...
  NES_FATAL_ERROR("No valid {} found in executable path or DYLD_LIBRARY_PATH.",
                  libName);
  return std::filesystem::current_path();
}
...
std::filesystem::path getLibPath(std::string libName) {
  ...
  auto libPath =
      detail::recursiveFindFileReverse(executablePath, "lib").append(libName);
  ...
  throw CompilerException(
      "Path to " + libName +
      " not found. Executable path is: " + executablePath.string());
}
...
std::filesystem::path getPublicIncludes() {
  ...
  throw CompilerException(
      "NebulaStream include path found not found. Executable path is: " +
      executablePath.string());
}
std::filesystem::path getClangPath() {
  // Depending on the current environment the clang executable could be found at
  // different places.
  // 1. if the system is installed then we should find a nes-clang executable
  // next to the current executable, e.g. nesCoordinator.
  // 2. if we are in the build environment CLANG_EXECUTABLE should indicate the
  // location of clang.
