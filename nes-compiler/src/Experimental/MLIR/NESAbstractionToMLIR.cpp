...
mlir::Type MLIRGenerator::getMLIRType(NESAbstractionNode::BasicType type) {
    switch(type) {
      ...
      case NESAbstractionNode::BOOLEAN:
        return builder->getIntegerType(1);
      case NESAbstractionNode::INT8:
      case NESAbstractionNode::CHAR:
        return builder->getIntegerType(8);
      case NESAbstractionNode::INT16:
        return builder->getIntegerType(16);
      case NESAbstractionNode::INT32:
        return builder->getIntegerType(32);
      case NESAbstractionNode::INT64:
        return builder->getIntegerType(64);
      case NESAbstractionNode::FLOAT:
        return Float32Type::get(context);
      case NESAbstractionNode::DOUBLE:
...
mlir::Value MLIRGenerator::getConstInt(Location loc, int numBits, 
                                              int64_t value) {
  return builder->create<LLVM::ConstantOp>(
      loc, builder->getIntegerType(numBits),
...
void MLIRGenerator::insertComment(const std::string& comment) {
  ...
  builder->create<LLVM::UndefOp>(getNameLoc(comment), 
                  LLVM::LLVMVoidType::get(context), emptyRange, 
                  builder->getNamedAttr(comment, 
...
mlir::arith::CmpIPredicate convertToMLIRComparison(
    NESAbstractionIfNode::ComparisonType comparisonType) {
//TODO extend to all comparison types
  switch(comparisonType) {
    case(NESAbstractionIfNode::ComparisonType::SIGNED_INT_GT):
      return mlir::arith::CmpIPredicate::sgt;
    case(NESAbstractionIfNode::ComparisonType::SIGNED_INT_GE):
      return mlir::arith::CmpIPredicate::sgt;
    case(NESAbstractionIfNode::ComparisonType::SIGNED_INT_LT):
      return mlir::arith::CmpIPredicate::slt;
    case(NESAbstractionIfNode::ComparisonType::SIGNED_INT_LE):
...
FlatSymbolRefAttr MLIRGenerator::insertExternalFunction(const std::string &name, 
    ...
    if(numResultBits != 0) {
      llvmFnType = LLVM::LLVMFunctionType::get(
      builder->getIntegerType(numResultBits), argTypes, varArgs);
    } else {
      llvmFnType = LLVM::LLVMFunctionType::get(
      LLVM::LLVMVoidType::get(context), argTypes, varArgs);
    }
    ...
    builder->create<LLVM::LLVMFuncOp>(theModule.getLoc(), name, 
                                      llvmFnType, LLVM::Linkage::External, false);
    return SymbolRefAttr::get(context, name);
}

Value MLIRGenerator::createGlobalString(Location loc, StringRef name, 
                                      StringRef value) {
  ...
  auto type = LLVM::LLVMArrayType::get(builder->getIntegerType(8), 
                                        value.size());
  LLVM::GlobalOp global = builder->create<LLVM::GlobalOp>(loc, type, true,
  ...
  auto globalString = builder->create<LLVM::GEPOp>(loc, LLVM::LLVMPointerType::get(
                                      builder->getIntegerType(8)), globalPtr, ArrayRef<Value>({constZero, constZero}));
  return globalString; 
}

void MLIRGenerator::insertValuePrint(mlir::Value printedValue, 
                                   NESAbstractionNode::BasicType type) {
  mlir::Value typeInt = getConstInt(getNameLoc("type"), 8, type);
  builder->create<LLVM::CallOp>(getNameLoc("printFromMLIR"), 
                                builder->getIntegerType(32), printfReference,
...
Value MLIRGenerator::insertProxyCall(const std::string &funcName, 
  ...
  auto objectGEP = builder->create<LLVM::GlobalOp>(
                    getNameLoc("TB Pointer"), LLVM::LLVMPointerType::get( builder->getIntegerType(8)), false, LLVM::Linkage::External, StringRef{ptrName}, Attribute{});

  // Load object address locally. Call proxy member function with objectPtr.
  builder->restoreInsertionPoint(currentInsertionPoint);
  Value objectPtr = builder->create<LLVM::AddressOfOp>(getNameLoc("loadPtr"), 
                                                        objectGEP);
  return builder->create<LLVM::CallOp>(getNameLoc("memberCall"), 
                                       getMLIRType(returnType), 
                                       proxyFunc, objectPtr).getResult(0);
}

void MLIRGenerator::insertTupleBuffers(
  ...
  auto tupleStruct = LLVM::LLVMStructType::getLiteral(
                      theModule.getContext(), structTypes, true);
  auto arrayType = LLVM::LLVMArrayType::get(tupleStruct, types.size());
  
  // Insert TupleBuffers globally.
  PatternRewriter::InsertionGuard insertGuard(*builder);
  builder->restoreInsertionPoint(*globalInsertPoint);
  inputBufferGlobal = builder->create<LLVM::GlobalOp>(
                        getNameLoc("TupleBuffer"), arrayType, false,LLVM::Linkage::External, StringRef{"IB"}, Attribute{});
  outputBufferGlobal = builder->create<LLVM::GlobalOp>(
                        getNameLoc("OutputBuffer"), arrayType, false,LLVM::Linkage::External, StringRef{"OB"}, Attribute{});
}

void MLIRGenerator::debugBufferPrint(
                      mlir::Value currentTuple, std::vector<uint64_t> indexes,
                      std::vector<NESAbstractionNode::BasicType> types) {

  insertTupleBuffers(types);

  Value result = insertProxyCall("getNumTuples", "IBPtr",  
                                          NESAbstractionNode::BasicType::INT64);
  auto printValFunc = insertExternalFunction("printValueFromMLIR", 0, {}, true);
  builder->create<LLVM::CallOp>(getNameLoc("printFunc"), mlir::None, printValFunc,
                                result);
  Value globalArrayPtr = builder->create<LLVM::AddressOfOp>(
                                  getNameLoc("loadedArray"), 
                                  inputBufferGlobal);
  Value outputBufferPtr = builder->create<LLVM::AddressOfOp>(
                                  getNameLoc("loadedArray"), 
                                  outputBufferGlobal);
  ...
  for(int i = 0; i < (int) indexes.size(); ++i) {
    ...
    auto obGEP = builder->create<LLVM::GEPOp>(getNameLoc("obGEP"), 
                                        ...
                                        outputBufferPtr, 
                                        ArrayRef<Value>({constZero, currentTuple, fieldIdx}));
    mlir::Value loadedValue = builder->create<LLVM::LoadOp>(
                                getNameLoc("loadedValue"), ibGEP);
...
//==---------------------------------==//
MLIRGenerator::MLIRGenerator(mlir::MLIRContext &context, std::string queryId) :
  context(&context) {
  ...
  executeFunction = mlir::FuncOp::create(getNameLoc("EntryPoint"), "execute",   
                                                    functionInOutTypes);
  //avoid mangling
  ...
  globalInsertPoint = new mlir::RewriterBase::InsertPoint(theModule.getBody(), 
                                                          theModule.begin());
  // Insert printFromMLIR function for debugging use.
  printfReference = insertExternalFunction("printFromMLIR", 32,
...
void MLIRGenerator::generateMLIRFromNode(std::shared_ptr<NESAbstractionNode> node,
                                         Value tuple) {
  switch(node->getNodeType()) {
    ...
    case NESAbstractionNode::BinOpNode:
      generateMLIRFromNode(std::static_pointer_cast<NESAbstractionBinOpNode>(node), 
                           tuple);
      break;
    case NESAbstractionNode::WriteNode:
      generateMLIRFromNode(std::static_pointer_cast<NESAbstractionWriteNode>(node), 
                           tuple);    
      break;
    }
}
...
void MLIRGenerator::generateMLIRFromNode(
                      std::shared_ptr<NESAbstractionIfNode> abstractionNode, 
                      Value tuple) {
  ...
  Value comparisonValue = builder->create<arith::ConstantOp>(
                                   getNameLoc("comparisonvalue"), 
                                   builder->getI32IntegerAttr(comparisonValues
                                   .front()));
  ...
  // Create IF Operation
  auto ifOp = builder->create<scf::IfOp>(getNameLoc("ifOperation"), 
                                         builder->getI32Type(), cmpOp, true);
...
void MLIRGenerator::generateMLIRFromNode(std::shared_ptr<NESAbstractionBinOpNode>
  abstractionNode, Value tuple) {
  Value elseReturnValue = builder->create<arith::ConstantOp>(
    getNameLoc("elseReturnValue"), 
  ...
  Value additionResult = builder->create<LLVM::AddOp>(
    getNameLoc("binOpResult"), elseReturnValue, tuple);
...
mlir::ModuleOp MLIRGenerator::generateMLIR(std::shared_ptr<NESAbstractionNode> 
                                           rootNode) {
  ...
  if (failed(mlir::verify(theModule))) { 
    theModule.emitError("module verification error"); return nullptr; 
  }
  return theModule;
}