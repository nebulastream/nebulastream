...
std::string MLIRUtility::insertComments(const std::string &moduleString) {
  ...
  for(std::string line; std::getline(moduleStream, line);) {
    ...
    if(line.substr(equalIdx + 2, 15) == "llvm.mlir.undef") {
      ...
      std::string commentString = line.substr(commentStartIdx,  
                                    commentEndIdx-commentStartIdx);
      commentString.erase(
        std::remove(commentString.begin(), commentString.end(), '"'), 
        commentString.end());
...
int MLIRUtility::runJit(const std::vector<std::string> &symbols, 
  ...
  auto maybeEngine = mlir::ExecutionEngine::create(*module, nullptr, printOptimizingTransformer, llvm::CodeGenOpt::Level::Aggressive);
  ...
  auto runtimeSymbolMap =  [&](llvm::orc::MangleAndInterner interner) {
    ...
    for(int i = 0; i < (int) symbols.size(); ++i) {
        symbolMap[interner(symbols.at(i))] =
        llvm::JITEvaluatedSymbol(jitAddresses.at(i), llvm::JITSymbolFlags::Callable);
    }
    return symbolMap;
    };
