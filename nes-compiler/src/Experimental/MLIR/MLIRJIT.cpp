...
*/
//===- ExecutionEngineCopy.cpp - MLIR Execution engine and utils --------------===//
//
...
//
//===----------------------------------------------------------------------===//
//#include "mlir/ExecutionEngineCopy/ExecutionEngineCopy.h"
...
void SimpleObjectCacheCopy::notifyObjectCompiled(const Module *M,
                                             MemoryBufferRef ObjBuffer) {
  cachedObjects[M->getModuleIdentifier()] = MemoryBuffer::getMemBufferCopy(
          ObjBuffer.getBuffer(), ObjBuffer.getBufferIdentifier());
}

std::unique_ptr<MemoryBuffer> SimpleObjectCacheCopy::getObject(const Module *M) {
...
void SimpleObjectCacheCopy::dumpToObjectFile(StringRef outputFilename) {
...
void ExecutionEngineCopy::dumpToObjectFile(StringRef filename) {
  cache->dumpToObjectFile(filename);
}

void ExecutionEngineCopy::registerSymbols(
...
bool ExecutionEngineCopy::setupTargetTriple(Module *llvmModule) {
...
ExecutionEngineCopy::ExecutionEngineCopy(bool enableObjectCache,
                                 bool enableGDBNotificationListener,
                                 bool enablePerfNotificationListener)
        : cache(enableObjectCache ? new SimpleObjectCacheCopy() : nullptr),
...
Expected<std::unique_ptr<ExecutionEngineCopy>> ExecutionEngineCopy::create(
        ModuleOp m,
  ...
  auto engine = std::make_unique<ExecutionEngineCopy>(
  ...
  auto objectLinkingLayerCreator = [&](ExecutionSession &session,
      ...
      // cf llvm/lib/ExecutionEngineCopy/Orc/LLJIT.cpp LLJIT::createObjectLinkingLayer
...
Expected<void (*)(void **)> ExecutionEngineCopy::lookup(StringRef name) const {
  auto expectedSymbol = jit->lookup(makePackedFunctionName(name));

  // JIT lookup may return an Error referring to strings stored internally by
  // the JIT. If the Error outlives the ExecutionEngineCopy, it would want have a
...
Error ExecutionEngineCopy::invokePacked(StringRef name,
