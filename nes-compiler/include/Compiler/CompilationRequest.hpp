 ...
 * A compilation request consists of a unique identifier, a source code artifact
 * and a set of compilation options.
 */
...
public:
   ...
   * @param externalApis a vector of external APIs to add to the compilation
   ...
   * @return std::unique_ptr<CompilationRequest>
   */
  static std::shared_ptr<CompilationRequest>
  create(std::unique_ptr<SourceCode> sourceCode, std::string name,
         bool profileCompilation, bool profileExecution,
         bool optimizeCompilation, bool debug,
   ...
   */
  [[nodiscard]] std::vector<std::shared_ptr<ExternalAPI>>
  getExternalAPIs() const;
