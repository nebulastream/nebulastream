...
/**
 * @brief An enum for representing the target programming language of a
 * `SourceCode` object.
 */
enum class Language {
  CPP,
...
template <> struct hash<const NES::Compiler::SourceCode> {
  std::size_t operator()(const NES::Compiler::SourceCode &k) const {
    ...
    return (
        (hash<string>()(k.getCode()) ^
         (hash<string>()(NES::Compiler::getLanguageAsString(k.getLanguage()))
          << 1)) >>
        1);
  }
};
