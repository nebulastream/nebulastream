...
/*
     ...
     II: To address the concern of arbitrarily increasing sequence numbers, the SequenceShredder implements a dynamically increasing ring buffer.
         ...
         in the range of the ring buffer, the thread puts its task back to the queue (rarely the case). The SequenceShredder keeps track of
     ...
     III: The SequenceShredder needs to synchronize between concurrently executing threads. Every buffer produced by the
          ...
          the only thread that was able to find the spanning tuple and can therefore safely process it.
...
private:
    ...
    enum class WrappingMode : uint8_t
    {
        ...
        CHECK_WRAPPING_TO_HIGHER = 2, /// 01: the sequence number could complete an ST starting in a higher bitmap
