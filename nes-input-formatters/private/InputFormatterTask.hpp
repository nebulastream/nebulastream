...
void processSpanningTuple(
    ...
    Memory::TupleBuffer& formattedBuffer,
    const TupleMetaData& tupleMetaData,
    const InputFormatIndexer<FieldIndexFunctionType>& inputFormatIndexer,
    const std::vector<RawValueParser::ParseFunctionSignature>& parseFunctions)
{
...
public:
    ...
    explicit InputFormatterTask(
        ...
        std::unique_ptr<InputFormatIndexer<FieldIndexFunctionType>> inputFormatIndexer,
        const Schema& schema,
...
private:
    OriginId originId;
    std::unique_ptr<InputFormatIndexer<FieldIndexFunctionType>>
