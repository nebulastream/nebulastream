    ...
    const bool isCompiled)
{
    ...
    auto fields = schema.getFieldNames()
        | std::ranges::transform([](const std::string& fieldName) { return convertLegacyIdList(fieldName); })
        | std::ranges::to<std::vector>();
    auto scanOp = ScanPhysicalOperator(provideInputFormatterTupleBufferRef(parserConfiguration, memoryProvider), fields);
