...
Schema<UnqualifiedUnboundField, Ordered> createSchema(const std::vector<TestDataTypes>& testDataTypes, const std::vector<Identifier>& testFieldNames);
...
public:
    ...
    std::optional<Record> getNextTuple()
    {
        ...
        auto record = bufferRef->readRecord(schema.getUniqueFieldNames(), recordBuffer, currentTupleIdx);
        ++currentTupleIdx;
        return record;
    }
...
inline bool compareTestTupleBuffersOrderSensitive(
    std::vector<TupleBuffer>& actualResult, std::vector<TupleBuffer>& expectedResult, Schema<QualifiedUnboundField, Ordered> schema)
{
...
TupleBuffer createTupleBufferFromTuples(const Schema<QualifiedUnboundField, Ordered>& schema, BufferManager& bufferManager, const std::vector<TupleSchema>& tuples)
{
