...
{
public:
    TupleIterator(std::vector<TupleBuffer> buffers, const PhysicalSchema& schema, const MemoryLayoutType layoutType)
...
inline bool compareTestTupleBuffersOrderSensitive(
    std::vector<TupleBuffer>& actualResult, std::vector<TupleBuffer>& expectedResult, PhysicalSchema schema)
{
...
TupleBuffer createTupleBufferFromTuples(const PhysicalSchema& schema, BufferManager& bufferManager, const std::vector<TupleSchema>& tuples)
{
    ...
    for (size_t taskIndex = 0; auto& actualResultVector : *testHandle.resultBuffers)
    {
        ...
        for (size_t bufferIndex = 0; auto& actualResultBuffer : actualResultVector)
        {
            ...
            isValid &= checkIfBuffersAreEqual(
                actualResultBuffer, testHandle.expectedResultVectors[taskIndex][bufferIndex], physicalTupleByteSize(testHandle.schema));
            ++bufferIndex;
        }
        ++taskIndex;
    }
