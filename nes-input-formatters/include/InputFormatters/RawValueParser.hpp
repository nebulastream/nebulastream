...
ParseResult<T> parseIntoVarValProxy(int8_t* fieldAddress, const uint64_t fieldSize, const std::vector<std::string>* nullValues)
{
    ...
    {
        /// A non-nullable field cannot absorb the failure. invokeGuarded parks the exception and rethrows it
    ...
    const std::vector<std::string>& nullValues)
{
    ...
    if (nullable)
    {
        const auto parseResult
            = nautilus::invokeGuarded<&parseIntoVarValProxy<T, true>, int8_t*, uint64_t, const std::vector<std::string>*>(
    ...
    const auto parseResult = nautilus::invokeGuarded<&parseIntoVarValProxy<T, false>, int8_t*, uint64_t, const std::vector<std::string>*>(
        fieldAddress, fieldSize, nautilus::val<const std::vector<std::string>*>{&nullValues});
    const nautilus::val<T> nautilusValue = *getMemberWithOffset<T>(parseResult, offsetof(ParseResult<T>, value));
