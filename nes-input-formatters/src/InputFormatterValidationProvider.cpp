...
provide(const std::string_view inputFormat, std::unordered_map<std::string, std::string> stringConfig)
{
    ...
    const auto validatedConf
        = InputFormatterValidationRegistry::instance().create(std::string{inputFormat}, std::move(inputFormatValidationArgs));
    ...
    if (threadingMode.asEnum<InputFormatterThreadingMode>() == InputFormatterThreadingMode::SEQUENTIAL
        and not toUpperCase(type).contains("SEQUENTIAL"))
    {
