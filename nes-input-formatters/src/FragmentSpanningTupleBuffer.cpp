...
FragmentSpanningTupleBuffer::FragmentSpanningTupleBuffer(
    const size_t initialSize, const size_t delimiterSize, const size_t maxFragmentBytes)
    : buffer(std::vector<FragmentSpanningTupleBufferEntry>(initialSize)), delimiterSize(delimiterSize), maxFragmentBytes(maxFragmentBytes)
{
    PRECONDITION(initialSize > 0, "Constructing an FragmentSpanningTupleBuffer with an initial size of 0 is not allowed");
    buffer.at(0).setStateOfFirstIndex();
}

FragmentSpanningTupleBuffer::WithoutDelimiterSearchResult
FragmentSpanningTupleBuffer::searchAndTryClaimWithoutDelimiter(const SequenceNumber sequenceNumber)
{
...
void FragmentSpanningTupleBuffer::claimSpanningTupleBuffers(
...
SequenceShredderResult FragmentSpanningTupleBuffer::tryFindLeadingSpanningTupleForBufferWithDelimiter(
    const SequenceNumber sequenceNumber, const StagedBuffer& indexedRawBuffer)
{
    ...
    if (const auto [bufferIdx, abaItNumber] = getBufferIdxAndABAItNo(sequenceNumber);
        not buffer[bufferIdx.getRawValue()].trySetWithDelimiter(abaItNumber, indexedRawBuffer, delimiterSize, maxFragmentBytes))
    {
...
SpanningBuffers FragmentSpanningTupleBuffer::tryFindTrailingSpanningTupleForBufferWithDelimiter(const SequenceNumber sequenceNumber)
{
...
SpanningBuffers FragmentSpanningTupleBuffer::tryFindTrailingSpanningTupleForBufferWithDelimiter(
...
SequenceShredderResult FragmentSpanningTupleBuffer::tryFindSpanningTupleForBufferWithoutDelimiter(
    const SequenceNumber sequenceNumber, const StagedBuffer& indexedRawBuffer)
{
    /// Try to set 'indexedRawBuffer' at corresponding index in buffer. Return as 'out-of-range' buffer, if setting the buffer fails
    if (const auto [bufferIdx, abaItNumber] = getBufferIdxAndABAItNo(sequenceNumber);
...
bool FragmentSpanningTupleBuffer::validate() const
{
    bool isValid = true;
...
std::ostream& operator<<(std::ostream& os, const FragmentSpanningTupleBuffer& sequenceRingBuffer)
{
    return os << fmt::format("FragmentRingBuffer({})", sequenceRingBuffer.buffer.size());
}

std::pair<SpanningTupleBufferIdx, ABAItNo> FragmentSpanningTupleBuffer::getBufferIdxAndABAItNo(const SequenceNumber sequenceNumber) const
{
...
std::optional<size_t>
FragmentSpanningTupleBuffer::searchLeading(const SpanningTupleBufferIdx searchStartBufferIdx, const ABAItNo abaItNumber) const
{
...
std::optional<size_t>
FragmentSpanningTupleBuffer::searchTrailing(const SpanningTupleBufferIdx searchStartBufferIdx, const ABAItNo abaItNumber) const
{
    const auto searchStartBufferIdxV = searchStartBufferIdx.getRawValue();
...
FragmentSpanningTupleBuffer::ClaimedSpanningTuple FragmentSpanningTupleBuffer::claimingLeadingDelimiterSearch(const SequenceNumber stEndSN)
{
...
FragmentSpanningTupleBuffer::ClaimedSpanningTuple
FragmentSpanningTupleBuffer::claimingTrailingDelimiterSearch(const SequenceNumber stStartSN)
{
    return claimingTrailingDelimiterSearch(stStartSN, stStartSN);
}

FragmentSpanningTupleBuffer::ClaimedSpanningTuple
FragmentSpanningTupleBuffer::claimingTrailingDelimiterSearch(const SequenceNumber stStartSN, const SequenceNumber searchStartSN)
{
