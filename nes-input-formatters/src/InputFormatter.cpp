...
namespace detail
{
RawTupleBuffer makeSpanningTupleBuffer(const char* rawDataPtr, const std::size_t sizeOfSpanningTuple)
{
    return RawTupleBuffer{rawDataPtr, sizeOfSpanningTuple};
}
}
...
{
    assignLeadingRawBufferIndex(indexer.indexRawBuffer(RawTupleBuffer{}));
    assignTrailingRawBufferIndex(indexer.indexRawBuffer(RawTupleBuffer{}));
    assignRawBufferIndex(indexer.indexRawBuffer(RawTupleBuffer{}));
}
...
public:
    ...
    {
        auto result = indexer.indexRawBuffer(RawTupleBuffer{tupleBuffer});
        ...
        const size_t sizeOfTrailingSpanningTuple)
    {
        ...
        assignTrailingRawBufferIndex(indexer.indexRawBuffer(detail::makeSpanningTupleBuffer(
            std::bit_cast<const char*>(tlIndexPhaseResult.trailingSpanningTuple.data()), tlIndexPhaseResult.trailingSpanningTuple.size())));
    }
        ...
        const std::vector<StagedBuffer>& leadingSpanningTupleBuffers, InputFormatIndexer& indexer, Arena& arenaRef)
    {
        ...
        assignLeadingRawBufferIndex(indexer.indexRawBuffer(detail::makeSpanningTupleBuffer(
