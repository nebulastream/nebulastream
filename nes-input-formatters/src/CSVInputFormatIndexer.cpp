...
std::unique_ptr<RawBufferIndex> CSVInputFormatIndexer::indexRawBuffer(const RawTupleBuffer& rawBuffer) const
{
    auto fieldOffsets = std::make_unique<FieldOffsetRawBufferIndex>();

    fieldOffsets->startSetup(this->numberOfFields, SIZE_OF_TUPLE_DELIMITER);

    const auto offsetOfFirstTupleDelimiter = static_cast<FieldIndex>(rawBuffer.getBufferView().find(this->tupleDelimiter));
