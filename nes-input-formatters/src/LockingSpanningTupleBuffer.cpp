...
LockingSpanningTupleBuffer::LockingSpanningTupleBuffer(const size_t initialSize, TupleBuffer dummyBuffer)
    : buffer(std::vector<PlainSpanningTupleBufferEntry>(initialSize))
{
    PRECONDITION(initialSize > 0, "Constructing a LockingSpanningTupleBuffer with an initial size of 0 is not allowed");
    buffer.at(0).setStateOfFirstIndex(std::move(dummyBuffer));
}

LockingSpanningTupleBuffer::WithoutDelimiterSearchResult
LockingSpanningTupleBuffer::searchAndTryClaimWithoutDelimiter(const SequenceNumber sequenceNumber)
{
...
SequenceShredderResult LockingSpanningTupleBuffer::tryFindLeadingSpanningTupleForBufferWithDelimiter(
...
SpanningBuffers LockingSpanningTupleBuffer::tryFindTrailingSpanningTupleForBufferWithDelimiter(const SequenceNumber sequenceNumber)
{
...
SpanningBuffers LockingSpanningTupleBuffer::tryFindTrailingSpanningTupleForBufferWithDelimiter(
...
SequenceShredderResult LockingSpanningTupleBuffer::tryFindSpanningTupleForBufferWithoutDelimiter(
    const SequenceNumber sequenceNumber, const StagedBuffer& indexedRawBuffer)
{
    if (const auto [bufferIdx, abaItNumber] = getBufferIdxAndABAItNo(sequenceNumber);
...
bool LockingSpanningTupleBuffer::validate() const
{
    bool isValid = true;
...
std::ostream& operator<<(std::ostream& os, const LockingSpanningTupleBuffer& spanningTupleBuffer)
{
    return os << fmt::format("LockingSpanningTupleBuffer({})", spanningTupleBuffer.buffer.size());
}

std::pair<SpanningTupleBufferIdx, ABAItNo> LockingSpanningTupleBuffer::getBufferIdxAndABAItNo(const SequenceNumber sequenceNumber) const
{
...
std::optional<size_t>
LockingSpanningTupleBuffer::searchTrailing(const SpanningTupleBufferIdx searchStartBufferIdx, const ABAItNo abaItNumber) const
{
    const auto searchStartBufferIdxV = searchStartBufferIdx.getRawValue();
...
LockingSpanningTupleBuffer::ClaimedSpanningTuple LockingSpanningTupleBuffer::claimingLeadingDelimiterSearch(const SequenceNumber stEndSN)
{
...
LockingSpanningTupleBuffer::ClaimedSpanningTuple LockingSpanningTupleBuffer::claimingTrailingDelimiterSearch(const SequenceNumber stStartSN)
{
    return claimingTrailingDelimiterSearch(stStartSN, stStartSN);
}

LockingSpanningTupleBuffer::ClaimedSpanningTuple
LockingSpanningTupleBuffer::claimingTrailingDelimiterSearch(const SequenceNumber stStartSN, const SequenceNumber searchStartSN)
{
