...
SequenceShredder::SequenceShredder(const size_t sizeOfTupleDelimiter)
    : tail(0)
...
bool SequenceShredder::isInRangeOfRingBuffer(const SequenceNumberType sequenceNumber)
{
    ...
    /// protect: write(resizeRequestCount), read(tail,numberOfBitmaps)
    {
        ...
        for (size_t offsetToTail = 1; offsetToTail <= numberOfBitmaps; ++offsetToTail)
        {
            ...
            if ((seenAndUsedBitmap | tupleDelimiterBitmap) != 0)
            {
                ...
                /// Determine whether the formatter produce a buffer using the largest sequence number already.
                /// That is the case, if the buffer of the largest sequence number contains a tuple delimiter and it was used alread (uses != 2)
...
SequenceShredder::processSequenceNumber(StagedBuffer stagedBufferOfSequenceNumber, const SequenceNumberType sequenceNumber)
{
    ...
    {
        const std::scoped_lock lock(readWriteMutex);
        sequenceNumberBufferPosition = sequenceNumber & (stagedBuffers.size() - 1);
...
void SequenceShredder::incrementTail()
{
    ...
    auto tailBitmapIndex = this->tail & numberOfBitmapsModulo;
...
std::ostream& operator<<(std::ostream& os, SequenceShredder& sequenceShredder)
{
    ...
    os << "SequenceShredder: {\n";
    os << "  number of bitmaps: " << sequenceShredder.numberOfBitmaps << ",\n";
    os << "  resize request count: " << sequenceShredder.resizeRequestCount << ",\n";
    os << "  tail: " << sequenceShredder.tail << ",\n";
    os << "  tupleDelimiterBitmaps: ";
    bitmapToString(sequenceShredder.tupleDelimiterBitmaps.front(), os);
    for (const auto& tupleDelimiterBitmap : sequenceShredder.tupleDelimiterBitmaps | std::views::drop(1))
    {
        os << "-";
        bitmapToString(tupleDelimiterBitmap, os);
    }
    os << "\n  seenAndUsedBitmaps   : ";
    bitmapToString(sequenceShredder.seenAndUsedBitmaps.front(), os);
    for (const auto& seenAndUsedBitmap : sequenceShredder.seenAndUsedBitmaps | std::views::drop(1))
    {
        os << "-";
        bitmapToString(seenAndUsedBitmap, os);
    }
    os << "\n}\n";
