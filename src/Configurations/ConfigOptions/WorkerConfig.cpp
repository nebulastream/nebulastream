...
WorkerConfig::WorkerConfig() {
    ...
    locationCoordinates = ConfigOption<std::string>::create("locationCoordinates", "", "lat/lng of physical node location");
}
...
void WorkerConfig::overwriteConfigWithYAMLFileInput(const std::string& filePath) {
    ...
    if (!filePath.empty() && std::filesystem::exists(filePath)) {
        ...
        try {
            ...
            if (!config["locationCoordinates"].As<std::string>().empty() && config["locationCoordinates"].As<std::string>() != "\n") {
                setLocationCoordinates(config["locationCoordinates"].As<std::string>());
            }
    ...
    try {
        ...
        for (auto it = inputParams.begin(); it != inputParams.end(); ++it) {
            if (it->first == "--localWorkerIp" && !it->second.empty()) {
                ...
                setNumaAware(true);
            } else if (it->first == "--locationCoordinates") {
                setLocationCoordinates(it->second);
            } else {
...
void WorkerConfig::setLocationCoordinates(std::string coordinates) {
    locationCoordinates->setValue(std::move(coordinates));
}
StringConfigOption WorkerConfig::getLocationCoordinates() { return locationCoordinates; }
