...
bool OutputChannel::sendBuffer(NodeEngine::TupleBuffer& inputBuffer, uint64_t tupleSize) {
    ...
    // FIXME below commented out line leads to error message on my system (comment by Aljoscha)
//    sendMessage<Messages::DataBufferMessage, kSendMore>(zmqSocket, payloadSize, numOfTuples, originId, watermark);
//    kSendMore
    sendMessage<Messages::DataBufferMessage>(zmqSocket, payloadSize, numOfTuples, originId, watermark);
    inputBuffer.retain();
    auto sentBytesOpt =
