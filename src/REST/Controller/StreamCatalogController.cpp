...
void StreamCatalogController::handleGet(std::vector<utility::string_t> path, web::http::http_request message) {
    ...
    if (path[1] == "allLogicalStream") {
        const map<std::string, std::string>& allLogicalStreamAsString = streamCatalog->getAllLogicalStreamAsString();
        ...
        message.extract_string(true)
            .then([this, message](utility::string_t body) {
                try {
                    ...
                    const vector<StreamCatalogEntryPtr>& allPhysicalStream = streamCatalog->getPhysicalStreams(logicalStreamName);
