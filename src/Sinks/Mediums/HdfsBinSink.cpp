...
std::string HdfsBinSink::toString() { return "HDFS_SINK"; }

SinkMediumTypes HdfsBinSink::getSinkMediumType() { return FILE_SINK; }

HdfsBinSink::HdfsBinSink(SinkFormatPtr format, char *filePath, bool append, QuerySubPlanId parentPlanId)
...
//    NES_DEBUG("HdfsBinSink: open file=" << filePath);
//    if (!append) {
...
HdfsBinSink::~HdfsBinSink() {
    NES_DEBUG("HdfsBinSinkink: close file=" << filePath);
    if (hdfsFileIsOpenForWrite(outputFile)) {
        hdfsCloseFile(fs, outputFile);
    }
}

const std::string HdfsBinSink::toString() const {
    std::stringstream ss;
    ss << "HdfsBinSink(";
...
void HdfsBinSink::setup() {}

void HdfsBinSink::shutdown() {}

bool HdfsBinSink::writeData(NodeEngine::TupleBuffer& inputBuffer, NodeEngine::WorkerContextRef) {
    std::unique_lock lock(writeMutex);

    NES_DEBUG("HdfsBinSink: getSchema medium " << toString() << " format " << sinkFormat->toString() << " and mode "
    ...
    if (!inputBuffer.isValid()) {
        NES_ERROR("HdfsBinSink::writeData input buffer invalid");
        return false;
    }
    if (!schemaWritten) {
        ...
        NES_DEBUG("HdfsBinSink::getData: schema already written");
    }

    NES_DEBUG("HdfsBinSink::getData: write data to file=" << filePath);
    auto dataBuffers = sinkFormat->getData(inputBuffer);

    for (auto buffer : dataBuffers) {
        NES_DEBUG("HdfsBinSink::writeData: write buffer of size " << buffer.getNumberOfTuples());
        hdfsWrite(fs, outputFile, (char*) buffer.getBuffer(), buffer.getBufferSize());
        NES_DEBUG("HdfsBinSink::writeData: data written: " << (char*) buffer.getBuffer());
    }
...
const char * HdfsBinSink::getFilePath() const { return filePath; }
