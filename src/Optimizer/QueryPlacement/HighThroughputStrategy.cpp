...
NESExecutionPlanPtr HighThroughputStrategy::initializeExecutionPlan(QueryPlanPtr queryPlan, NESTopologyPlanPtr nesTopologyPlan,
    ...
    vector<NESTopologyEntryPtr> HighThroughputStrategy::getCandidateNodesForFwdOperatorPlacement(
        const vector<NESTopologyEntryPtr>& sourceNodes, const NES::NESTopologyEntryPtr rootNode) const {
                ...
                while (node->getRemainingCpuCapacity() > 0 && targetOperator) {
                    ...
                    if (!executionPlanPtr->hasVertex(node->getId())) {
                        ...
                        if (exists != residentOperatorIds.end()) {
                            ...
                            stringstream operatorName;
                            operatorName << existingExecutionNode->getOperatorName() << "=>" << targetOperator->toString()
