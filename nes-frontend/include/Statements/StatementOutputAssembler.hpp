...
constexpr std::array<std::string_view, 6> semanticModelInfoOutputColumns{
    "model_name", "base_url", "model", "api_key_env", "input_schema", "output_schema"};
...
struct StatementOutputAssembler<CreateSemanticModelStatementResult>
{
    ...
    auto convert(const CreateSemanticModelStatementResult& result)
    {
        return std::make_pair(
            ...
            std::vector{std::make_tuple(
                result.name, result.config.baseUrl, result.config.model, result.config.apiKeyEnv, result.inputSchema, result.outputSchema)});
    }
};
...
struct StatementOutputAssembler<ShowSemanticModelsStatementResult>
{
    ...
    auto convert(const ShowSemanticModelsStatementResult& result)
    {
        ...
        {
            output.emplace_back(
                model.name, model.config.baseUrl, model.config.model, model.config.apiKeyEnv, model.inputSchema, model.outputSchema);
        }
