...
using SinkDescriptorOutputRowType = std::tuple<std::string, Schema, std::string, NES::DescriptorConfig::Config, Host>;
...
struct StatementOutputAssembler<ShowSinksStatementResult>
{
    ...
    auto convert(const ShowSinksStatementResult& result)
    {
        ...
        {
            output.emplace_back(sink.getSinkName(), *sink.getSchema(), sink.getSinkType(), sink.getConfig(), sink.getHost());
        }
...
struct StatementOutputAssembler<WorkerStatusStatementResult>
{
    ...
    auto convert(const WorkerStatusStatementResult& result)
    {
        ...
        {
            output.emplace_back(
                activeQuery.queryId.getLocalQueryId(), QueryState::Running, std::nullopt, activeQuery.started, std::nullopt);
        }
        for (const auto& terminatedQuery : status.terminatedQueries)
        {
            output.emplace_back(
                terminatedQuery.queryId.getLocalQueryId(),
