...
class QueryManager
{
    class QueryManagerBackends
    {
        ...
        mutable std::unordered_map<WorkerId, UniquePtr<QuerySubmissionBackend>> backends;
        mutable uint64_t cachedWorkerCatalogVersion = 0;
        static std::unordered_map<WorkerId, UniquePtr<QuerySubmissionBackend>>
    ...
    public:
        ...
        bool contains(const WorkerId& worker) const
        {
            rebuildBackendsIfNeeded();
            return backends.contains(worker);
        }

        const QuerySubmissionBackend& at(const WorkerId& worker) const
        {
            rebuildBackendsIfNeeded();
            return *backends.at(worker);
        }

        QuerySubmissionBackend& at(const WorkerId& worker)
        {
            rebuildBackendsIfNeeded();
            return *backends.at(worker);
        }
