...
fn run_inner() -> anyhow::Result<()> {
    ...
    match cli.command {
        Command::Start { until_completed, queries } => {
            ...
            let errors: Vec<_> = std::thread::scope(|scope| {
                query_ids
                    ...
                    .map(|id| {
                        ...
                        scope.spawn(|| {
                            send_with_timeout(&handle, Payload::structured(stmt), timeout)
                        })
                    })
                    .collect::<Vec<_>>()
                    .into_iter()
        ...
        Command::Status { wait, query_ids } => {
            let mut req = if query_ids.is_empty() {
            ...
            let result = send_with_timeout(
                &handle,
                Payload::structured(Statement::GetQuery(req)),
                timeout,
            )?;
