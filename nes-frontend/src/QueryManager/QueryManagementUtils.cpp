...
std::jthread stopLocalQuery(LocalQuery& query, QuerySubmissionBackend& backend, bool graceful)
{
    ...
    return std::jthread(
        [&]
        {
            while (true)
            {
                if (!query.couldBeRunning())
                {
                    break;
                }
                auto stopResponse = graceful ? backend.stop(query.getCurrentQueryId()) : backend.terminate(query.getCurrentQueryId());
                if (stopResponse)
                {
    ...
    return std::jthread(
        [&]
        {
            ...
            while (true)
            {
                ...
                static const auto QUERY_ALREADY_REGISTERED = QueryAlreadyRegistered().code();
                if (newId || newId.error().code() == QUERY_ALREADY_REGISTERED)                {
