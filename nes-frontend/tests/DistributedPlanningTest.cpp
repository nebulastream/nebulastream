...
std::vector<NES::Statement> loadStatements(const NES::Test::QueryConfig& topologyConfig)
{
    ...
    for (const auto& [host, capacity, downstream] : workers)
    {
        statements.emplace_back(NES::CreateWorkerStatement{
            .host = host, .workerManagementEndpoint = host, .capacity = capacity, .downstream = downstream, .config = {}});
    }
...
TEST_F(DistributedPlanningTest, BasicPlacementSingleNode)
{
    ...
    const LogicalPlan localPlan = plan[WorkerId("localhost:8080")].front();
...
TEST_F(DistributedPlanningTest, BasicPlacementTwoNodes)
{
    ...
    const auto sourceNodePlan = plan[WorkerId("source-node:8080")].front();
    ...
    const auto sinkNodePlan = plan[WorkerId("sink-node:8080")].front();
...
TEST_F(DistributedPlanningTest, JoinPlacementWithOneSelection)
{
    ...
    const auto sourceNode0Plan = plan[WorkerId("source-node0:8080")].front();
    ...
    const auto sourceNode1Plan = plan[WorkerId("source-node1:8080")].front();
...
TEST_F(DistributedPlanningTest, PlacementWithThreeNodes)
{
    ...
    const auto plan0 = plan[WorkerId("sink-node:8080")].front();
    ...
    const auto plan1 = plan[WorkerId("source-node0:8080")].front();
    ...
    const auto plan2 = plan[WorkerId("source-node1:8080")].front();
...
TEST_F(DistributedPlanningTest, JoinPlacementWithLimitedCapacityOnTwoNodes)
{
    ...
    const auto sinkPlan = plan[WorkerId("sink-node:8080")].front();
    ...
    const auto sourcePlan1 = plan[WorkerId("source-node:8080")][0];
    ...
    const auto sourcePlan2 = plan[WorkerId("source-node:8080")][1];
...
TEST_F(DistributedPlanningTest, BridgePlacement)
{
    ...
    const auto sourcePlan = plan[WorkerId("source-node:8080")].front();
    ...
    const auto intermediatePlan = plan[WorkerId("intermediate-node:8080")].front();
...
TEST_F(DistributedPlanningTest, LongBridgePlacement)
{
    ...
    const auto intermediatePlan0 = plan[WorkerId("intermediate-node0:8080")].front();
    ...
    const auto intermediatePlan1 = plan[WorkerId("intermediate-node1:8080")].front();
...
TEST_F(DistributedPlanningTest, BridgePlacementJoin)
{
    ...
    const auto intermediatePlan1 = plan[WorkerId("intermediate-node:8080")][0];
    EXPECT_EQ(flatten(intermediatePlan1).size(), 2);
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(intermediatePlan1).size(), 1);
    EXPECT_EQ(
        getOperatorByType<SourceDescriptorLogicalOperator>(intermediatePlan1)[0].get().getSourceDescriptor().getSourceType(), "Network");
    ...
    const auto intermediatePlan2 = plan[WorkerId("intermediate-node:8080")][1];
...
TEST_F(DistributedPlanningTest, ComplexJoinQuery)
{
    ...
    const auto sourceNode2Plan = plan[WorkerId("source-node2:8080")].front();
    ...
    const auto intermediateNode0Plan = plan[WorkerId("intermediate-node0:8080")].front();
    ...
    const auto intermediateNode1Plan = plan[WorkerId("intermediate-node1:8080")].front();
