...
std::vector<NES::Statement> loadStatements(const NES::Test::QueryConfig& topologyConfig)
{
    ...
    for (const auto& [logical, host] : physical)
    {
        const NES::Schema<NES::LiteralConfigValue, NES::Ordered> values{
            {"file_path", "does_not_exist"}, {"host", host}, {"type", "CSV"}};
        auto configSchema
            = NES::SourceCatalog::getConfigSchema(NES::Identifier::parse("File"), NES::Identifier::parse("CSV")).value();
    ...
    statements.emplace_back(NES::ExplainQueryStatement{.plan = NES::AntlrSQLQueryParser::QueryBinder{{}, {}}.createLogicalQueryPlanFromSQLString(query)});
    return statements;
}
...
workers:
  ...
  - host: "source-node1:8080"
    ...
    EXPECT_EQ(
        getLeafOperators(sinkNodePlan)[0].getAs<SourceDescriptorLogicalOperator>().get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(
        getLeafOperators(sinkNodePlan)[1].getAs<SourceDescriptorLogicalOperator>().get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
...
workers:
  ...
  - host: "source-node1:8080"
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(plan0)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(plan0)[1].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
...
workers:
  ...
  - host: "source-node:8080"
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sourcePlan1)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("FILE"));
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sourcePlan2)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("FILE"));
...
workers:
  ...
  - host: "sink-node:8080"
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sourcePlan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("FILE"));
    ...
    EXPECT_EQ(
        getOperatorByType<SourceDescriptorLogicalOperator>(intermediatePlan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(getOperatorByType<SinkLogicalOperator>(sinkPlan).size(), 1);
...
workers:
  ...
  - host: "sink-node:8080"
    ...
    EXPECT_EQ(
        getOperatorByType<SourceDescriptorLogicalOperator>(intermediatePlan0)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    ...
    EXPECT_EQ(
        getOperatorByType<SourceDescriptorLogicalOperator>(intermediatePlan1)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
...
workers:
  ...
  - host: "sink-node:8080"
    ...
    EXPECT_EQ(
        getOperatorByType<SourceDescriptorLogicalOperator>(intermediatePlan2)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan).size(), 2);
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan)[1].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(getOperatorByType<JoinLogicalOperator>(sinkPlan).size(), 1);
    EXPECT_EQ(getOperatorByType<SinkLogicalOperator>(sinkPlan).size(), 1);
    EXPECT_EQ(getOperatorByType<SinkLogicalOperator>(sinkPlan)[0].get().getSinkDescriptor()->getSinkType(), "VOID");
}
...
workers:
  ...
  - host: "sink-node:8080"
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sourceNode0Plan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("FILE"));
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sourceNode0Plan)[1].get().getSourceDescriptor().getSourceType(), Identifier::parse("FILE"));
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sourceNode1Plan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("FILE"));
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan)[0].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan)[1].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(sinkPlan)[2].get().getSourceDescriptor().getSourceType(), Identifier::parse("NETWORK"));
