...
NES::Schema<NES::LiteralConfigValue, NES::Ordered> bindSourceConfig(const std::unordered_map<std::string, std::string>& config)
{
    auto boundConfig = config
        | std::views::transform(
                           [](const auto& rawPair)
                           {
                               /// Raw string map: whatever does not parse as a typed literal stays a raw string
                               /// (delimiters like "\n" trim to empty and must survive as strings).
                               return NES::LiteralConfigValue{
                                   NES::QualifiedIdentifier::create(bindIdentifierName(rawPair.first)),
                                   NES::parseConfigLiteral(rawPair.second).value_or(NES::ConfigLiteral{rawPair.second})};
                           })
