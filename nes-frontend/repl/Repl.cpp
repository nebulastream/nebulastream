...
struct Repl::Impl
{
    ...
    {
        const auto outcome = coordinator.submit(rust::Str{query.data(), query.size()}, Bridge::WaitMode::None, 0, jsonOutput);
        if (not Bridge::isNone(outcome.error))
        {
            /// Raised as the exception for its code, so a fail-fast run exits with that code.
            handleError(Exception{std::string{outcome.error.msg}, outcome.error.code});
            return false;
        }
        std::cout << std::string_view{outcome.result.data(), outcome.result.size()} << '\n';
        std::flush(std::cout);
        return true;
    }
