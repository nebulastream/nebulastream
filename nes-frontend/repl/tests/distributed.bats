...
@test "launch query from topology" {
  ...
  assert_json_contains '{"id":1}' "$(echo "$results" | jq '.[5].CreatedQuery.query')"
  # SHOW QUERIES WHERE id = 1 lists it.
  assert_json_contains '{"id":1}' "$(echo "$results" | jq '.[6].Queries[0].query')"
  ...
  assert_json_contains '{"id":1}' "$(echo "$results" | jq '.[8].Queries[0].query')"
}
...
@test "launch multiple queries" {
  ...
  assert_json_contains '{"id":1}' "$(echo "$results" | jq '.[3].CreatedQuery.query')"
}
...
@test "quoted identifiers and uppercase compatibility survive distributed repl" {
  ...
  assert_json_contains \
    ...
    "$(echo "$results" | jq '.[3]')"
  echo "$results" | jq -e '.[4].CreatedQuery.query.id == 1'
}
