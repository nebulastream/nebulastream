...
@test "launch query from topology" {
  ...
  # running, so the result reports the running query.
  assert_json_contains '{"id":1,"state":"Running"}' "$(echo "$results" | jq '.[5].CreatedQuery[0]')"
  # SHOW QUERIES WHERE id = 1 lists the running query.
  assert_json_contains '{"id":1,"state":"Running"}' "$(echo "$results" | jq '.[6].Queries[0][0]')"
  # DROP QUERY WHERE id = 1 blocks until the query has stopped.
  assert_json_contains '{"id":1,"state":"Stopped"}' "$(echo "$results" | jq '.[7].DroppedQueries[0]')"
  # A stopped query stays in the catalog, so the final SHOW still lists it, now in the Stopped state.
  assert_json_contains '{"id":1,"state":"Stopped"}' "$(echo "$results" | jq '.[8].Queries[0][0]')"
}
...
@test "launch query and terminate query on exit behavior" {
  ...
  # The SELECT installs a running query. STOP_QUERIES issues an implicit DROP QUERY on exit, but
  # that on-exit result is a frontend action and is not emitted to stdout, so the fast exit above is
  # what proves it ran; here we only assert the query was created and running.
  results=$(printf '%s' "$output" | jq -s '.')
  assert_json_contains '{"id":1,"state":"Running"}' "$(echo "$results" | jq '.[3].CreatedQuery[0]')"
}
