...
RawScanPhysicalOperator::RawScanPhysicalOperator(std::unique_ptr<RawScanWrapper> RawScan)
    : RawScan(std::move(RawScan))
{
}

void RawScanPhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    this->RawScan->scan(executionCtx, recordBuffer, child.value());
}
