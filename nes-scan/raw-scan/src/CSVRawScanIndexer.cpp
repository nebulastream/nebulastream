...
void CSVRawScanIndexer::indexRawBuffer(
    FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD>& fieldOffsets, const RawTupleBuffer& rawBuffer, const CSVMetaData& metaData) const
{
    ...
    /// the RawScanIndexerTask that there was no tuple delimiter in the buffer and return
...
RawScanIndexerRegistryReturnType
RegisterCSVRawScanIndexer(RawScanIndexerRegistryArguments arguments) ///NOLINT(performance-unnecessary-value-param)
{
    return arguments.createRawScanWithIndexer(CSVRawScanIndexer{});
}

std::ostream& operator<<(std::ostream& os, const CSVRawScanIndexer&)
{
    return os << fmt::format("CSVRawScanIndexer()");
}
}
