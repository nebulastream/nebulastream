    ...
    const std::span<const StagedBuffer>& stagedBuffersSpan, int8_t* spanningTuplePointer, const IndexerMetaData& indexerMetaData)
{
    ...
    /// Wrap the spanning tuple in delimiter bytes (if exist) to allow the RawScanIndexer to detect the spanning tuple
    ...
    /// Wrap the end of the spanning tuple in a newline to allow the RawScanIndexer to detect the tuple
    std::memcpy(
...
/// RawScans concurrently take (potentially) raw input buffers and format all full tuples in these raw input buffers that the
/// individual RawScans see during execution.
/// The only point of synchronization is a call to the SequenceShredder data structure, which determines which buffers the RawScan
/// needs to process (resolving tuples that span multiple raw buffers).
/// An RawScan belongs to exactly one source. The source reads raw data into buffers, constructs a Task from the
/// raw buffer and its successor (the RawScan) and writes it to the task queue of the QueryEngine.
/// The QueryEngine concurrently executes RawScans. Thus, even if the source writes the RawScans to the task queue sequentially,
...
public:
    ...
    RawScan(const RawScan&) = delete;
    RawScan& operator=(const RawScan&) = delete;
    RawScan(RawScan&&) = default;
    RawScan& operator=(RawScan&&) = delete;

    void startTask() { /* noop */ }

    void stopTask() const
    {
        /// If the RawScan needs to handle spanning tuples, it uses the SequenceShredder.
    ...
    public:
        ...
        std::pair<FieldIndex, FieldIndex> indexRawBuffer(RawScan& RawScan, const TupleBuffer& tupleBuffer)
        {
            RawScan.RawScanIndexer.indexRawBuffer(
                spanningTuplePOD.rawBufferFIF, RawTupleBuffer{tupleBuffer}, RawScan.indexerMetaData);
            ...
            Arena& arenaRef)
        {
            calculateSizeOfSpanningTuplesWithDelimiter(
                stagedBuffers, indexOfSequenceNumberInStagedBuffers, RawScan.indexerMetaData.getTupleDelimitingBytes().size());
            ...
            if (spanningTuplePOD.leadingSpanningTupleSizeInBytes > 0)
            {
                ...
                processSpanningTuple<typename FormatterType::IndexerMetaData>(
                    leadingSpanningTupleBuffers, spanningTuplePOD.leadingSpanningTuplePtr, RawScan.indexerMetaData);
                RawScan.RawScanIndexer.indexRawBuffer(
            ...
            if (spanningTuplePOD.trailingSpanningTupleSizeInBytes > 0)
            {
                ...
                processSpanningTuple<typename FormatterType::IndexerMetaData>(
                    trailingSpanningTupleBuffers, spanningTuplePOD.trailingSpanningTuplePtr, RawScan.indexerMetaData);
                RawScan.RawScanIndexer.indexRawBuffer(
        ...
        void handleWithoutDelimiter(
            const std::vector<StagedBuffer>& spanningTupleBuffers, RawScan& RawScan, Arena& arenaRef)
        {
            calculateSizeOfSpanningTuple(
                [this](const size_t bytes) { increaseLeadingSpanningTupleSize(bytes); },
                spanningTupleBuffers,
                RawScan.indexerMetaData.getTupleDelimitingBytes().size());
            allocateForLeadingSpanningTuple(arenaRef);

            processSpanningTuple<typename FormatterType::IndexerMetaData>(
                spanningTupleBuffers, spanningTuplePOD.leadingSpanningTuplePtr, RawScan.indexerMetaData);
            RawScan.RawScanIndexer.indexRawBuffer(
                spanningTuplePOD.leadingSpanningTupleFIF,
                RawTupleBuffer{
        ...
        Arena* arenaRef)
    {
        ...
        if (not RawScan->sequenceShredder->isInRange(tupleBuffer->getSequenceNumber().getRawValue()))
        {
        ...
        const auto [offsetOfFirstTupleDelimiter, offsetOfSecondTupleDelimiter]
            = spanningTupleData.indexRawBuffer(*RawScan, *tupleBuffer);
        ...
        if (/* hasTupleDelimiter */ offsetOfFirstTupleDelimiter < configuredBufferSize)
        {
            const auto [indexOfSequenceNumberInStagedBuffers, stagedBuffers]
                = RawScan->sequenceShredder->processSequenceNumber<true>(
                    StagedBuffer{
            ...
            spanningTupleData.handleWithDelimiter(stagedBuffers, indexOfSequenceNumberInStagedBuffers, *RawScan, *arenaRef);
        }
        else /* has no tuple delimiter */
        {
            const auto [indexOfSequenceNumberInStagedBuffers, stagedBuffers]
                = RawScan->sequenceShredder->processSequenceNumber<false>(
                    StagedBuffer{
                        RawTupleBuffer{*tupleBuffer},
                        tupleBuffer->getNumberOfTuples(), /// size in bytes
                        offsetOfFirstTupleDelimiter,
            ...
            spanningTupleData.handleWithoutDelimiter(stagedBuffers, *RawScan, *arenaRef);
        }
        return spanningTupleData.getThreadLocalSpanningTuplePODPtr();
    }
    ...
    std::ostream& taskToString(std::ostream& os) const
    {
        ...
        os << "RawScan(" << ", RawScanIndexer: " << RawScanIndexer << ", sequenceShredder: " << *sequenceShredder
           << ")\n";
        return os;
    }
