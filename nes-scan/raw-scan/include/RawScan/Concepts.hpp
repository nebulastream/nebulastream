...
/// Restricts the IndexerMetaData that an RawScanIndexer receives from the InputFormatterTask
template <typename T>
concept IndexerMetaDataType
    = requires(ParserConfig config, const MemoryLayout& memoryLayout, T indexerMetaData, std::ostream& spanningTuple) {
          ...
          /// RawScanIndexers without tuple delimiters should return an empty string
...
template <typename T>
concept RawScanIndexerType = IndexerMetaDataType<typename T::IndexerMetaData>
