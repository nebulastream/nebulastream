...
struct SpanningTuplePOD
{
    ...
    FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD> leadingSpanningTupleFIF;
    FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD> trailingSpanningTupleFIF;
    FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD> rawBufferFIF;
};
...
CSVTupleBufferMemoryProvider::CSVTupleBufferMemoryProvider(ParserConfig config, std::shared_ptr<MemoryLayout> layout)
    : shredder(std::make_unique<SequenceShredder>(config.tupleDelimiter.size()))
    , metadata(std::make_unique<CSVMetaData>(std::move(config), *layout))
    , layout(std::move(layout))
{
}

std::shared_ptr<MemoryLayout> CSVTupleBufferMemoryProvider::getMemoryLayout() const
{
    return layout;
}
...
public:
    ...
    std::pair<FieldIndex, FieldIndex> indexRawBuffer(const CSVMetaData& metaData, const TupleBuffer& tupleBuffer)
    {
        CSVInputFormatIndexer().indexRawBuffer(spanningTuplePOD.rawBufferFIF, RawTupleBuffer{tupleBuffer}, metaData);
        ...
        Arena& arenaRef)
    {
        ...
        if (spanningTuplePOD.leadingSpanningTupleSizeInBytes > 0)
        {
            ...
            processSpanningTuple<CSVMetaData>(
                ...
                metaData);
            CSVInputFormatIndexer().indexRawBuffer(
    ...
    void handleWithoutDelimiter(const std::vector<StagedBuffer>& spanningTupleBuffers, const CSVMetaData& metaData, Arena& arenaRef)
    {
...
Record CSVTupleBufferMemoryProvider::readRecord(
    ...
    nautilus::val<uint64_t>& recordIndex) const
{
    ...
    auto leadingFIF = Nautilus::Util::getMemberWithOffset<FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD>>(
        spanningTuplePOD, offsetof(SpanningTuplePOD, leadingSpanningTupleFIF));
    auto trailingFIF = Nautilus::Util::getMemberWithOffset<FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD>>(
        spanningTuplePOD, offsetof(SpanningTuplePOD, trailingSpanningTupleFIF));
    auto rawFieldAccessFunction = Nautilus::Util::getMemberWithOffset<FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD>>(
    ...
    if (recordIndex == 0)
    {
        ...
        if (hasLeading)
        {
            ...
            return FieldOffsets<CSV_NUM_OFFSETS_PER_FIELD>{}.readSpanningRecord(
...
void CSVTupleBufferMemoryProvider::writeRecord(
...
void CSVTupleBufferMemoryProvider::open(const RecordBuffer& recordBuffer, ArenaRef& arenaRef)
{
    nautilus::val<SpanningTuplePOD*> spanningTuple = nautilus::invoke(
        +[](const CSVMetaData* metaData, SequenceShredder* shredder, TupleBuffer* buffer, Arena* arena)
        { return indexTuplesProxy(buffer, *shredder, *metaData, 4096, arena); },
        nautilus::val<const CSVMetaData*>(metadata.get()),
...
nautilus::val<uint64_t> CSVTupleBufferMemoryProvider::getNumberOfRecords(const RecordBuffer&) const
{
