...
OpenReturnState RawScanPhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    PRECONDITION(child.has_value(), "Cannot call open without a child value.");
    return this->rawInputFormatScan->scan(executionCtx, recordBuffer, child.value());
}
