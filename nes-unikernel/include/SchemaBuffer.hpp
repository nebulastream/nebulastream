...
struct SchemaBuffer {
    ...
    [[nodiscard]] constexpr size_t getCapacity() const {
       return BufferSize / Schema::TupleSize;
    }
...
template<typename T>
struct type {
    static void write(std::span<uint8_t>, T::ctype);
...
struct Schema {
    ...
    static void writeTupleAtBufferAddress(std::span<uint8_t> memoryLocation, TupleType tuple) {
        std::apply([memoryLocation](auto&&... field){
            size_t i = 0;
...
template<typename T>
inline void type<T>::write(std::span<uint8_t> memory, T::ctype value){
...
template<typename T>
inline T::ctype type<T>::read(std::span<uint8_t> memory){
    NES_ASSERT(memory.size() == T::size, "Memory size does not match");
    NES_ASSERT(reinterpret_cast<std::uintptr_t>(memory.data()) % alignof(typename T::ctype) == 0, "Bad Alignment");
