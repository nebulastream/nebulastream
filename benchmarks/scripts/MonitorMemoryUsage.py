...
def main():
    ...
    command = '$(pwd)/cmake-build-relnologging/nes-systests/systest/systest -t $(pwd)/nes-systests/benchmark/Nexmark.test:05 -b --workingDir=$(pwd)/.cache/working-dir --data $(pwd)/cmake-build-relnologging/nes-systests/testdata --groups large -- --worker.queryCompiler.nautilusBackend=COMPILER --worker.queryEngine.numberOfWorkerThreads=4 --worker.numberOfBuffersInGlobalBufferManager=200000'
