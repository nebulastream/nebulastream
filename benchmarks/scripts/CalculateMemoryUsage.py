...
df1 = pd.read_csv('measurements/data/baseline.csv', header=None, names=['numAvailablePooledBuf', 'numUnpooledBuf', 'timestamp'])
df2 = pd.read_csv('measurements/data/variant.csv', header=None, names=['numAvailablePooledBuf', 'numUnpooledBuf', 'timestamp'])
...
df1_downsampled['normalized_time'] = (df1_downsampled['datetime'] - df1_downsampled['datetime'].min()) / (df1_downsampled['datetime'].max() - df1_downsampled['datetime'].min())
df2_downsampled['normalized_time'] = (df2_downsampled['datetime'] - df2_downsampled['datetime'].min()) / (df2_downsampled['datetime'].max() - df2_downsampled['datetime'].min())

# Plot
fig, ax1 = plt.subplots(figsize=(12, 6))

# Plot solution 1
ax1.plot(df1_downsampled['normalized_time'], df1_downsampled['total_memory'], marker='o', linestyle='-', color='b', label='Baseline')
...
ax2 = ax1.twinx()
ax2.plot(df2_downsampled['normalized_time'], df2_downsampled['total_memory'], marker='s', linestyle='--', color='r', label='Variant')
