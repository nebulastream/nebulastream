...
BUILD_PROJECT = False
BUILD_DIR = "build"
NES_DIR = "nebulastream"
SOURCE_DIR = os.path.join("/home/ubuntu/Documents", NES_DIR)
...
TEST_DATA = os.path.join("$(pwd)", "nes-systests/testdata")
...
TEST_NAME = "JoinMultipleStreams.test:05"
TEST_FILE_PATH = os.path.join("$(pwd)", "nes-systests/operator/join", TEST_NAME)
...
def start_systest(output_folder, current_benchmark_config):
    ...
    cmd = f"{SYSTEST_PATH} -t {TEST_FILE_PATH} -b --timeout=0 --data {TEST_DATA} --groups large -- " \
    ...
    process = subprocess.Popen(cmd, shell=True, cwd=SOURCE_DIR, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    pid = process.pid
...
def run_benchmark(current_benchmark_config):
    ...
    try:
        ...
        systest_process = start_systest(output_folder, current_benchmark_config)
        ...
        try:
            ...
            stdout, stderr = systest_process.communicate(timeout=WAIT_BEFORE_SIGKILL)
            if stderr != '':
                print(output_folder + ": " + stderr + "\n")
            else:
                print(output_folder + ": " + stdout + "\n")
    finally:
...
def main():
    ...
    print("################################################################\n")
    ALL_SYSTEST_CONFIGS = BenchmarkConfig.create_configs()
