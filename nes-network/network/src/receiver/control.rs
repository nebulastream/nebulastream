...
async fn socket_listener<C: Communication + 'static>(
    ...
    let mut communication_listener = communication
        ...
        .await
        .expect(&format!("Failed to bind control socket to {}", this_connection_identifier));
...
pub(super) async fn create_control_socket_handler(
    ...
    loop {
        match listener.recv().await {
            ...
            Ok(NetworkingServiceControlMessage::RegisterChannel(ident, emit_fn, response)) => {
            ...
            Ok(NetworkingServiceControlMessage::RetryChannel(ident, emit_fn, token)) => {
