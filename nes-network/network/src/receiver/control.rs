...
pub(super) enum NetworkingServiceControlCommand {
...
pub(super) type NetworkingServiceController =
    async_channel::Sender<NetworkingServiceControlCommand>;
...
type NetworkingServiceControlListener = async_channel::Receiver<NetworkingServiceControlCommand>;
...
pub(super) async fn create_control_socket_handler(
    ...
    loop {
        match listener.recv().await {
            ...
            Ok(NetworkingServiceControlCommand::RegisterChannel(ident, emit_fn, response)) => {
            ...
            Ok(NetworkingServiceControlCommand::RetryChannel(ident, emit_fn, token)) => {
