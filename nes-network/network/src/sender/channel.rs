...
impl<R: AsyncRead + Unpin, W: AsyncWrite + Unpin> ChannelHandler<R, W> {
    ...
    async fn handle_request(
        ...
        match channel_control_message {
            ...
            ChannelControlMessage::Flush(done) => {
                Self::cancelable(&self.cancellation_token, self.writer.flush())
                    .await?
    ...
    async fn cancelable<F: Future>(token: &CancellationToken, fut: F) -> InternalResult<F::Output> {
    ...
    async fn read_from_software(
        ...
        let Ok(r) = Self::cancelable(cancellation_token, s.recv()).await? else {
    ...
    async fn read_from_other_side(
        ...
        let Some(r) = Self::cancelable(cancellation_token, s.next()).await? else {
