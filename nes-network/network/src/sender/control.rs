...
///
/// # Relationship to `NetworkingServiceControlCommand`
///
/// `NetworkingServiceControlCommand::RegisterChannel` is dispatched to the appropriate
...
pub(super) async fn network_sender_dispatcher(
    ...
    while let Ok(NetworkingServiceControlCommand::RegisterChannel(target_connection, channel, tx)) =
        control.recv().await
    {
