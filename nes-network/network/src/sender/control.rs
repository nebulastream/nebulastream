...
pub(super) async fn establish_channel<R: AsyncRead + Unpin, W: AsyncWrite + Unpin>(
    ...
    let Some(channel_request_result) = control_channel_sender_reader.next().await else {
        return EstablishChannelResult::BadConnection(channel, "Connection closed".into());
    };

    let response = match channel_request_result {
        Ok(response) => response,
        Err(e) => {
            return EstablishChannelResult::BadConnection(channel, e.into());
        }
    };
    ...
    let channel_connection_identifier = match response {
        ...
        ControlChannelResponse::DenyChannelResponse => {
            warn!("Channel '{channel}' was rejected");
...
> {
    ...
    match response {
        ...
        IdentificationResponse::NotOk => RetryError::to_transient(
            "Could not establish connection. Identification was denied".into(),
        ),
    }
}

type ChannelEstablishRequest =
...
async fn attempt_channel_registration<C: Communication + 'static>(
    ...
    channel_tx: ChannelEstablishRequest,
    ...
    let target_channel_identifier = Retry::spawn(
        ...
        async || -> core::result::Result<ConnectionIdentifier, RetryError<Error>> {
            ...
            if channel_tx.send((channel.clone(), tx)).await.is_err() {
...
async fn connection_handler<C: Communication + 'static>(
    ...
    // This task is responsible for establishing connections to the target_connection.
    // It implements exponential backoff.
    // Once a connection has been established and transfers the Reader/Writer pair to `connection_tx`
    // `connection_tx` also receives an oneshot channel to signal that the keepalive task should
    // once again retry to establish the connection.
    let _connection_keep_alive = ScopedTask::new(tokio::spawn(
        {
            ...
            async move {
                loop {
                    let Some(await_connection) = await_connection_request.recv().await else {
                        return;
                    };
