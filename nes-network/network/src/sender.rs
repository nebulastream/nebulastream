...
mod channel_handler {
    use crate::protocol::{
        ...
        OriginSequenceNumber, TupleBuffer,
    };
    use futures::SinkExt;
...
async fn channel_handler(
    ...
    match handler.run().await {
        ...
        Err(channel_handler::ChannelHandlerError::Protocol(e)) => {
            // Protocol errors are unacceptable and usually indicate a bug
...
async fn connection_handler<L: Communication + 'static>(
    ...
    let (channel_tx, mut channel_rx) = tokio::sync::mpsc::channel::<(
        ...
        oneshot::Sender<EstablishChannelResult>,
    )>(10);
