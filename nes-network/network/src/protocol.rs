...
impl Debug for TupleBuffer {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_fmt(format_args!("TupleBuffer{{ sequence_number: {}, source_insertion_ts: {}, origin_id: {}, chunk_number: {}, watermark: {}, number_of_tuples: {}, encoded_data: {}, bufferSize: {}, encoded_children: {:?}, child_buffer_sizes: {:?}, children: {:?}}}", self.sequence_number, self.source_insertion_ts, self.origin_id, self.chunk_number, self.watermark, self.number_of_tuples, self.encoded_data, self.data.len(), self.encoded_children, self.child_buffer_sizes, self.child_buffers.iter().map(|buffer| buffer.len()).collect::<Vec<_>>()))
    }
}
