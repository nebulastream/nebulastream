...
void TupleBufferBuilder::set_data(rust::Slice<const uint8_t> data)
{
    ...
    std::ranges::copy(data, buffer.getMemArea());
}
...
void TupleBufferBuilder::add_child_buffer(const rust::Slice<const uint8_t> child)
{
    ...
    std::ranges::copy(child, childBuffer->getMemArea());
    [[maybe_unused]] auto _ = buffer.storeChildBuffer(*childBuffer); /// index should already be present in the owning parent buffer
}
