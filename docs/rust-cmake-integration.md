...
The project has multiple Rust crates that produce CXX bindings (e.g. `nes_rust_bindings`, `nes_buffer_bindings`, `nes_source_bindings`, `nes_sink_bindings`, `nes_rust_answer`). Many executables transitively need all of them.
...
members = [
    ...
    "nes-network/nes-rust-bindings",
    "nes-network/network",
    "nes-network/nes-rust-bindings/network",
    "nes-network/nes-rust-bindings/spdlog",
...
nes-cli (executable)
  ...
  '-- target_link_rust(nes-cli) --> hashes crate set --> reuses or generates:
       nes_umbrella_9813de0fc3e5 (single staticlib, shared by all exes with same crate set)
         depends on: nes_rust_bindings + nes_buffer_bindings + nes_source_bindings + ...
