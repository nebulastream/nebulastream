...
Cargo tests are registered explicitly. Add the following call to a crate's
`CMakeLists.txt` only when that crate and its tests can compile and link without
CMake-built C++ sources:

```cmake
add_rust_cargo_test(my_func)
```

When `NES_ENABLES_TESTS` is enabled, this registers the CTest entry
`rust-my_func`, which runs `cargo test -p my_func` from the root workspace. It
uses the Cargo executable, Rust compiler, toolchain, and sanitizer environment
selected by Corrosion. Registered Cargo tests carry the `rust` label and are
...
| `cmake/RustLibCollection.cmake` | Public helpers `add_cxx_bridge`, `target_link_rust_lib`, and `add_rust_cargo_test`, plus umbrella generation |
