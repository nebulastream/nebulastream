...
For the full set of keys and defaults, see the worker configuration definitions starting at
`nes-runtime/interface/Configuration/WorkerConfiguration.hpp`.

File-global optimizer configuration uses the `optimizer.` prefix and must select exactly one value per file:
```
GlobalConfiguration optimizer.join_strategy: [NESTED_LOOP_JOIN]
```
Optimizer configuration applies while queries and `EXPLAIN` statements in that file are optimized; it is not passed to the worker.
