...
<!Outdated: Builds on top of schema inference. Nothing significant on main since then, but it uses the serializer registries>
...
4. <!Outdated: Deserializers take care of arrays and structs.>
...
<!Addition: XML, JSON, HL7 formats all support composition in combination with nesting. Not possible with flattening the schema>
...
<!Comment: Not entirely true, as varsized does not even support random access. It's just a sequence of bytes which can be compared to other sequences.>
- P4: There is no vector- or array-like container type that is type-specific. VarSized is essentially a string/byte-vector.

# Goals
<!Comment: Mention the extension of the serializers working DataType-Plugin based: Each composite type can have their own default serializers and deserializers. Also mention the vector type>
...
<!Comment: Add the new examples here like MEOS stuff, Timestamps, Decimal>
...
- NG2: Structural typing.
  <!Comment: Is this really the case now? It seems like the POC functions only check for matching field names, but the name of the employed struct could be different.>
...
<!Comment: This is varsized basically. A number of bytes to be viewed as a singular entity. We even support concat for varsized now>
- NG4: Create a dedicated string datatype.
<!Comment: Does not apply anymore. We created this, but with a constraint on how this should be used? See only one varsized / vararray in a tree path per plugin. Vector / Array constants are also not supported yet>
...
# Solution Background
<!Comment: Mention Serializers + Deserializers and the vectors>
...
# Proposed Solution
<!Comment: Mention vectors and new use cases like timestamp>
...
<!Comment: After rebase with main, adjust this to Leonhard's registry refactor>
...
# Proof of Concept
## Branch
<!Comment: Mention other branches like the timestamp one, which is generally the most complete currently. Also mention MEOS. Also show that the UnsignedTimestamp plugin can be used as a ts for windows. Show that we do not need to depend on a default struct parser and instead can expect differently formatted data for each plugin thanks to serializers & deserializers. Showcase vector.>
...
# Implementation Plan
<!Comment: Adjust to were we are now>
