...
The worker threads call `execute` once for each `Buffer` (more precisely.
...
classDiagram
    class ExecutablePipelineStage {
        ...
        +execute(Buffer&, PipelineExecutionContext&) uint32_t
...
Currently, only `Sources` are able to emit an `EoS` or a `Failure`, in addition to `Buffers` during normal execution.
