...
A7b (process-wide venv override): a `WorkerConfiguration` option (`worker.python_udf.venv`, `PythonUdfConfiguration` in `WorkerConfiguration.hpp`), set once when `SingleNodeWorker` is constructed — the one point production worker, REPL, and systest's embedded backend all pass through — which `setenv`s `NES_UDF_VENV`, read at bridge init alongside `NES_UDF_PATH`. The bridge scans `<venv>/lib/` for its one version-specific dir (`pythonX.Y`, whatever the venv's own creator named it) rather than assuming it matches the bridge's own linked Python, and prepends its `site-packages` to `sys.path`. A venv reuses the base interpreter's `libpython`, so this needs no ABI/grammar/catalog change on the bridge side — same mechanism as `NES_UDF_PATH` today, just surfaced through the existing worker-config system instead of a bespoke flag. Constraint that scanning cannot remove: a *compiled* extension inside the venv still needs to match the ABI of whichever CPython/PyPy build the bridge links against — in-process embedding cannot get around that. A pure-Python-only venv has no such constraint and works regardless of version drift between venv and bridge. One venv covers every Python UDF in that worker process; `sys.modules` is process-global, so two UDFs needing conflicting versions of the same package cannot both be satisfied this way.
...
Chosen: A7b for v1. No grammar/ABI/catalog change, mirrors `NES_UDF_PATH` exactly, and rides the existing worker-config mechanism rather than adding a new one — sufficient for the common case of one worker process and one set of Python dependencies. A7c stays a documented option if per-UDF isolation is later needed.

Granularity in practice: `worker.python_udf.venv` is read once, at `SingleNodeWorker` construction, ahead of any query that might load a Python UDF — into a process that embeds its interpreter once (`Py_InitializeEx`/PyPy embedding, both one-shot). So switching venvs means starting a new process, not toggling mid-run — e.g. for `systest`, two separate invocations differing only in the trailing `-- --worker.python_udf.venv=<path>` (or its absence). This bites even *within* one systest invocation: `CONFIGURATION`/`GLOBALCONFIGURATION` can set a different `worker.python_udf.venv` per query, and each override group does get its own `SingleNodeWorker` — but the CPython bridge `.so` backing all of them is one process-wide `dlopen`, whose interpreter init (`std::call_once`) fires on the first Python UDF call and is shared by every `SingleNodeWorker` after that. Run two test groups needing different venvs (e.g. `PythonUdfVenv` and `PythonUdfOnnx`) in the same invocation and only whichever query happens to run first gets its venv; the rest silently share that same interpreter state. Confirmed empirically: `-g PythonUdfVenv PythonUdfOnnx` in one process fails whichever group's query lost the race, passes both when run as separate invocations. Being a worker option, it's a worker-config-side argument, so it belongs after `systest`'s `--` like every other `--worker.xxx` override, never before it. Default (option unset) is always whatever Python is local to the process running the bridge — the system's `python3` for a bare-metal worker, the Docker image's `python3` for a containerized one, never the host's. Docker adds a wrinkle bare-metal doesn't have: the venv directory must be reachable *inside* the container (bind-mounted, with `worker.python_udf.venv` given the in-container path, not the host path), and for any package with compiled extensions, built against that container's libc/Python ABI — a venv built on the host and mounted in works for pure-Python packages (e.g. `inflection`), but is not reliable for one with native extensions (e.g. `onnxruntime`). Safest: create the venv from inside the container, not outside it.

Gotcha with multiple dev containers (this repo has several: `local`, `local-tsan`, `local-asan`). They all bind-mount the same repo directory, but each has its own private `/tmp`. Build the venv under a container's `/tmp` and only that one container can see it -- a different container, or a fresh one spun up for the next run, gets `ModuleNotFoundError` as if no venv existed at all.

Fix: build the venv inside the bind-mounted repo path, e.g. `nes-systests/udf/.iree_venv`. That path is the same on the host and in every container.

Note: unlike `small_venv`, this venv is not committed -- it's large and tied to the container's ABI. Each dev/CI run has to build its own (`pip install iree-base-compiler iree-base-runtime onnx`, or `onnxruntime` for the ONNX variant) and pass its path via `--worker.python_udf.venv`.
