...
It enables efficient `state persistency`, Apache Flink for example uses general purpose compression to encode checkpoints [7].
Furthermore, after implementing light-weight and general purpose compression codecs, we could utilize formats like the columnar `Apache Parquet` [5], which cascade codecs for a high compression ratio, as internal storage format.
...
**P2** Many compression codecs such as `LZ4`[4] need the data to arrive `in-order` for incremental decoding, since encoded data usually reference reoccurring byte patterns via pointer to previous raw bytes. However, the `input-formatter` component already receives the buffers `out-of-order`.
...
The exact sizes of each encoded dataset can be found on the table at [8].
...
[7] https://nightlies.apache.org/flink/flink-docs-masterdocs/ops/state/large_state_tuning/#compression

[8] https://docs.google.com/spreadsheets/d/1V_6wUDSKYtx51CS7hssRD_hR5veEw3AOnpYKd9kX82Y/edit?usp=sharing
