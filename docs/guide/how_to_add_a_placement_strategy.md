...
A placement strategy should follow the shape of [`BottomUpOperatorPlacer`](../../nes-query-optimizer/include/Placement/BottomUpPlacement.hpp):
...
`BottomUpOperatorPlacer` is an example: it uses [HiGHS](https://github.com/ERGO-Code/HiGHS) and `nes-query-optimizer` links `highs::highs` privately.
