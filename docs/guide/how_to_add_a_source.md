...
Each source plugin adds its implementation to `nes-sources`. The same hook can register with both the runtime source registry and the validation registry.
...
target_sources(nes-sources PRIVATE MQTTSource.cpp)
target_include_directories(nes-sources
        PUBLIC include
        PRIVATE .
)
target_link_libraries(nes-sources PRIVATE PahoMqttCpp::paho-mqttpp3-static)
...
It looks like this:
```c++
SourceRegistryReturnType RegisterMQTTSource(SourceRegistryArguments arguments)
{
    return std::make_unique<MQTTSource>(arguments.sourceDescriptor);
}

SourceValidationRegistryReturnType RegisterMQTTSourceValidation(SourceValidationRegistryArguments arguments)
{
    return MQTTSource::validateAndFormat(std::move(arguments.config));
}

ADD_PLUGIN(SourceRegistry, "MQTT", RegisterMQTTSource);
ADD_PLUGIN(SourceValidationRegistry, "MQTT", RegisterMQTTSourceValidation);
```
...
The source file emits one init hook into a linker section; CMake does not generate a registrar.
...
The `ADD_PLUGIN` hook above registers the validation function with the validation registry.
