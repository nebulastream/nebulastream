...
This page is the reference for every scalar and aggregate function NebulaStream exposes in SQL: the
operations you can apply to fields inside a `SELECT` list, `WHERE` clause, or aggregation. For the
operators that functions are used inside of (`SELECT`, `WHERE`, `WINDOW`, `JOIN`, …), see
[Operators](operators.md).

A function is either unary (one argument) or binary (two arguments). Passing the wrong number of
arguments throws `InvalidQuerySyntax` at query-compile time. A function name is matched
...
| `CAST(x AS <type>)` | Converts `x` to `<type>` using standard SQL cast syntax. Takes exactly one argument (`x`); the target type is a type annotation, not a second function argument. See [Casting rules](#casting-rules). | `SELECT CAST(x AS FLOAT64) FROM s INTO sink` |
...
`CAST` converts a field from one type to another:
