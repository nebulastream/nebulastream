...
This page is the reference for every scalar and aggregate function NebulaStream exposes in SQL. Scalar
functions can be applied to [fields](sources-and-sinks.md#logical-sources) within the `SELECT` list or
`WHERE` clause. Aggregate functions may only appear in the `SELECT` list (or `HAVING` clause) of a
windowed aggregation — see [Aggregation](operators.md#aggregation) for the operator these clauses
belong to.
