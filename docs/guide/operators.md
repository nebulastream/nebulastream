...
|---|---|---|
| [Projection](#projection) | Stateless | Enumerate fields and functions |
...
| [Model Inference](#model-inference) | Stateless | Run an ML model on each tuple in a stream |

---

## Projection

**Syntax:** `SELECT <field | function> [AS alias], ... FROM <source> INTO <sink>`
...
💡 NebulaStream reads an event-time timestamp field's raw value as a `UINT64` millisecond timestamp,
regardless of the field's declared schema type.
...
💡 For binary windowed operators (joins), the event-time timestamp fields of the two input streams can
have different names: supply both as `WINDOW TUMBLING(<left_ts>, <right_ts>, SIZE <size> <unit>)`.
...
that satisfy the join predicate. The two input streams' timestamp fields can have different names, see
...
💡 `CREATE MODEL` requires the OpenVINO model converter (`ovc`) to be installed on the process that
handles the statement. If it is not available, model registration will fail.
