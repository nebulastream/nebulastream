...
input formatter.
The system currently configures a specific deserializer type per data type. For each deserializer type, a separate deserializer for
...
The used Value Deserializer implementations for specific datatypes can be adjusted with the `VALUE_DESERIALIZERS` parameter of the input formatter by providing a comma-separated list of <Typename>:<Deserializer-Key> entries:
```sql
CREATE PHYSICAL SOURCE FOR source_name TYPE TCP SET(
'CSV' as INPUT_FORMATTER."TYPE",
'FLOAT32:FastFloatF32,FLOAT64:FastFloatF64' as "INPUT_FORMATTER".VALUE_DESERIALIZERS,
...
);
```
---
...
The used Value Serializer implementations for specific datatypes can be adjusted with the `VALUE_SERIALIZERS` parameter of the output formatter by providing a comma-separated list of <Typename>:<Serializer-Key> entries:
```sql
CREATE SINK sink_name TYPE FILE SET(
       'CSV' as "SINK".OUTPUT_FORMAT,
       'FLOAT32:ZmijF32,FLOAT64:ZmijF64' as "OUTPUT_FORMATTER".VALUE_SERIALIZERS,
       ...
);
```
---
