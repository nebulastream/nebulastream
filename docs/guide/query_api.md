...
- A scalar aggregation becomes a statistic through `STATISTIC_BUILD(statisticId, <aggregation>)`.
...
```sql
SELECT STATISTIC_BUILD(43, SUM(value)) FROM sensor WINDOW TUMBLING(ts, SIZE 5 SEC) INTO sink
```
