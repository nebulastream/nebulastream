...
- [Overview](#overview)
- [Nebuli (Interactive REPL)](#nebuli-interactive-repl)
    - [Starting the REPL](#starting-the-repl)
    - [Embedded Mode](#embedded-mode)
    - [Distributed Mode](#distributed-mode)
- [NebuCLI (One-Shot Topology Controller)](#nebucli-one-shot-topology-controller)
...
1. **`nebuli-embedded`** - Single-node embedded worker with local query execution via interactive REPL
2. **`nebuli`** - Distributed query controller for multi-node deployments via interactive REPL
3. **`nebucli`** - Stateless one-shot CLI for deploying and controlling queries from topology files

All interfaces support JSON output for programmatic access.

---

## Nebuli (Interactive REPL)

### Starting the REPL

```bash
# Embedded mode (single-node)
nebuli-embedded -d -f JSON
...
- `-s <address>` - Server address to connect to (default: `localhost:8080`). Not required for `nebuli-embedded`.
...
> [!NOTE]
> `nebuli` does not create or start workers - they must be started independently before registration.
...
## NebuCLI (One-Shot Topology Controller)

NebuCLI is a stateless CLI tool for deploying and managing queries based on YAML topology files. Unlike the interactive
REPL, `nebucli` performs single operations and exits.
...
nebucli -t topology.yaml dump
nebucli -d -t topology.yaml dump  # With debug output

# Start query from topology file
nebucli -t topology.yaml start

# Start ad-hoc query (override topology query)
nebucli -t topology.yaml start 'SELECT * FROM GENERATOR_SOURCE INTO VOID_SINK'

# Check query status
nebucli -t topology.yaml status <query-id>

# Stop query
nebucli -t topology.yaml stop <query-id>

# Stop multiple queries
nebucli -t topology.yaml stop <query-id-1> <query-id-2> <query-id-3>
...
> [!NOTE]
> Providing a query via the command line (e.g., `nebucli -t topology.yaml start 'SELECT ...'`) will override any queries
...
# Stop multiple queries
nebucli -t topology.yaml stop <id1> <id2> <id3>
```
