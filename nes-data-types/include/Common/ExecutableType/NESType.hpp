...
template <class Type>
concept IsNesType = std::is_fundamental_v<Type> ||
                    std::is_fundamental_v<std::remove_pointer_t<Type>> ||
                    std::is_base_of_v<NESType, Type>;

/**
 * @brief This concept checks via tuple unpacking if Types contains at least one
 * string.
 * @tparam Types
 */
template <class... Types>
concept ContainsString =
    requires { requires(std::is_same_v<std::string, Types> || ...); };
...
template <class Type>
concept IsArray = std::is_fundamental_v<std::remove_pointer_t<Type>> ||
                  std::is_base_of_v<NESType, Type>;
