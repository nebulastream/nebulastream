...
class DataTypeFactory {
 public:
  ...
  /**
   * @brief Create a new Float data type, with a bits size and a lower and upper
   * bound.
   ...
   * @return DataTypePtr
   */
  static DataTypePtr createFloat(int8_t bits, double lowerBound,
                                 double upperBound);

  /**
   * @brief Create a new Float data type, which infers the bit size from the
  ...
  /**
   * @brief Creates a 32bit Float data type, which corresponds to a C++ float
   * type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Creates a 64bit Float data type, which corresponds to a C++ double
   * type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Create a new Integer data type, with a bits size and a lower and
   * upper bound.
   ...
   * @return DataTypePtr
   */
  static DataTypePtr createInteger(int8_t bits, int64_t lowerBound,
                                   int64_t upperBound);

  /**
   * @brief Create a new Integer data type, which infers the bit size from the
   * lower and upper bound.
   * @param lowerBound lower bound for this integer
   * @param upperBound upper bound for this integer
   * @return DataTypePtr
  ...
  /**
   * @brief Creates a 16bit Integer data type, which corresponds to a C++
   * int16_t type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Creates a unsighted 16bit Integer data type, which corresponds to a
   * C++ uint16_t type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Creates a 32bit Integer data type, which corresponds to a C++
   * int32_t type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Creates a unsighted 32bit Integer data type, which corresponds to a
   * C++ uint32_t type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Creates a 364bit Integer data type, which corresponds to a C++
   * int64_t type.
   * @return DataTypePtr
   */
  ...
  /**
   * @brief Creates a unsighted 64bit Integer data type, which corresponds to a
   * C++ uint64_t type.
   * @return DataTypePtr
   */
   ...
   *
   *        For instance: DataTypePtr: ArrayType with componentType int, values
   * = {42, 21}.
   *
   * @param type the data type as a DataTypePtr
   * @param values the value as a vector of strings, which represent the
   * individual values.
   * @return ValueTypePtr
   */
  static ValueTypePtr createArrayValueWithContainedType(
      const DataTypePtr& type, std::vector<std::string>&& values) noexcept;

  /**
   * @brief Create an array value that is supposed to fit into the contained
   * type passed as argument to this function. Has only support for non-nested
   * array types.
   *
   *        For instance: DataTypePtr: ArrayType with componentType int, values
   * = {42, 21}.
   ...
   * individual values.
   */
  static ValueTypePtr createArrayValueFromContainerType(
   ...
   * @param values represents the fixed char as a vecotr of strings which should
   * contain only one char each.
   * @return ValueTypePtr
   */
  static ValueTypePtr createFixedCharValue(
      std::vector<std::string>&& values) noexcept;
  ...
  /**
   * @brief Create a data type from a BasicType, this many is used to support
   * the old type system API.
  ...
  /**
   * @brief If the given lowerBound is tighter than the one on the given stamp,
   * create a new stamp with tighter lowerBound. If not, return original stamp.
   * @param stamp Datatype to copy
   * @param minLowerBound lowerBound that can not be exceeded in the returned
   * type
   * @return DataTypePtr
   */
  static DataTypePtr copyTypeAndIncreaseLowerBound(DataTypePtr stamp,
  ...
  /**
   * @brief If the given upperBound is tighter than the one on the given stamp,
   * create a new stamp with tighter upperBound. If not, return original stamp.
   * @param stamp Datatype to copy
   * @param maxUpperBound upperBound that can not be exceeded in the returned
   * type
   * @return DataTypePtr
   */
  static DataTypePtr copyTypeAndDecreaseUpperBound(DataTypePtr stamp,
  ...
  /**
   * @brief If the given bounds are tighter than the one on the given stamp,
   * create a new stamp with tighter bounds. If not, return original stamp.
   * @param stamp Datatype to copy
   * @param minLowerBound lowerBound that can not be exceeded in the returned
   * type
   * @param maxUpperBound upperBound that can not be exceeded in the returned
   ...
   * @return DataTypePtr
   */
  static DataTypePtr copyTypeAndTightenBounds(DataTypePtr stamp,
                                              int64_t minLowerBound,
                                              int64_t maxUpperBound);
  /**
   * @brief If the given bounds are tighter than the one on the given stamp,
   ...
   * @param Integer stamp Datatype to copy. If float is passed the same stamp is
   * returned. Error at non-numeric stamp.
   * @return DataTypePtr
   */
