...
/**
 * @brief The array physical type, which represent ArrayType and FixedChar types
 * in NES.
 */
...
public:
   ...
   */
  inline ArrayPhysicalType(DataTypePtr type, uint64_t length,
                           PhysicalTypePtr component) noexcept
      : PhysicalType(std::move(type)), length(length),
        physicalComponentType(std::move(component)) {}
   ...
   */
  static inline PhysicalTypePtr
  create(const DataTypePtr &type, uint64_t length,
         PhysicalTypePtr const &component) noexcept {
  ...
  /**
   * @brief Converts the binary representation of this value to a string without
   * filling up the difference between the length of the string and the end of
   * the schema definition with unrelated characters
   * @param rawData a pointer to the raw value
   * @return string
   */
  std::string
  ...
  /// Fixed number of elements of type `physicalComponentType` contained in the
  /// array.
