...
/**
 * @brief Function to check that a compile-time type is the same as a particular
 * physical type. Depending on the compile-time type parameter, this method
 * selects the suitable type check on the physical type at runtime.
 * @tparam Type parameter
 * @param physicalType the physical type at runtime
 * @return returns true if the type is correct.
 */
template <class Type> bool isSamePhysicalType(PhysicalTypePtr physicalType) {
  ...
  if constexpr (IsChar<Type>) {
    ...
    return isArray(physicalType) &&
           isSamePhysicalType<std::remove_pointer_t<Type>>(
               getArrayComponent(physicalType));
