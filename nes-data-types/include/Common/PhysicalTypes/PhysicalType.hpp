...
/**
 * @brief The physical data type represents the physical representation of a NES
 * data type.
 */
class PhysicalType {
 public:
  inline explicit PhysicalType(DataTypePtr type) noexcept
      : type(std::move(type)) {}
   ...
   * @return string
   */
  virtual std::string convertRawToString(
      void const* rawData) const noexcept = 0;
   ...
   * @return string
   */
  virtual std::string convertRawToStringWithoutFill(
      void const* rawData) const noexcept = 0;
  ...
  /// @brief true only for arrays which contain chars as their immediate child
  /// type.
