...
template <>
struct EnumToType<DataType::Type::INT8>
{
    using type = int8_t;
};
template <>
struct EnumToType<DataType::Type::INT16>
{
    using type = int16_t;
};
template <>
struct EnumToType<DataType::Type::INT32>
{
    using type = int32_t;
};
template <>
struct EnumToType<DataType::Type::INT64>
{
    using type = int64_t;
};
template <>
struct EnumToType<DataType::Type::UINT8>
{
    using type = uint8_t;
};
template <>
struct EnumToType<DataType::Type::UINT16>
{
    using type = uint16_t;
};
template <>
struct EnumToType<DataType::Type::UINT32>
{
    using type = uint32_t;
};
template <>
struct EnumToType<DataType::Type::UINT64>
{
    using type = uint64_t;
};
template <>
struct EnumToType<DataType::Type::FLOAT32>
{
    using type = float;
};
template <>
struct EnumToType<DataType::Type::FLOAT64>
{
    using type = double;
};
template <>
struct EnumToType<DataType::Type::BOOLEAN>
{
    using type = bool;
};
template <>
struct EnumToType<DataType::Type::CHAR>
{
    using type = char;
};

template <DataType::Type EnumType>
using EnumToTypeT = typename EnumToType<EnumType>::type;
template <DataType::Type EnumType>
constexpr auto getTypeFromEnum()
{
    return EnumToType<EnumType>();
}
...
/// @throws if instantiated with a non-numerical data type
template <typename Func>
auto dispatchByNumericalTypeOrBool(const DataType::Type type, Func&& func)
{
