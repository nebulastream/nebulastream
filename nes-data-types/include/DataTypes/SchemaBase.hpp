        ...
        for (const auto& field : fields)
        {
            ...
            for (size_t i = 0; i < std::ranges::size(fullName); i++)
            {
                ...
                if (auto existingCollisions = collisions.find(idSubSpan); existingCollisions == collisions.end())
                {
                    ...
                    {
                        collisions.insert(std::pair{idSubSpan, std::vector<FieldRef>{existingIdList->second, field}});
...
public:
    ...
    static std::expected<SchemaBase, std::unordered_map<IdentifierList, std::vector<FieldType>>>
    ...
    requires(Ordered)
    {
        ...
        if (!collisions.empty())
        {
            std::unordered_map<IdentifierList, std::vector<FieldType>> collisionMap;
            for (const auto& [idSpan, fieldRefs] : collisions)
            {
                ...
                collisionMap.emplace(IdentifierList{idSpan}, std::move(fieldVec));
            }
    ...
    requires(!Ordered)
    {
        ...
        if (fieldSet.size() != fields.size())
        {
            auto collisions = fieldSet
                ...
                | std::ranges::to<std::unordered_map<IdentifierList, std::vector<FieldType>>>();
    ...
    static std::string createCollisionString(const std::unordered_map<IdentifierList, std::vector<FieldType>>& collisions)
    {
        return fmt::format(
            "{}",
            fmt::join(
    ...
    template <size_t OtherExtent, typename OtherFieldType>
    requires(IdListExtent == std::dynamic_extent)
    explicit SchemaBase(const SchemaBase<OtherExtent, OtherFieldType, Ordered>& other)
    ...
    [[nodiscard]] std::optional<FieldType> operator[](const IdentifierList& name) const
    {
    ...
    [[nodiscard]] std::optional<FieldType> getFieldByName(const IdentifierList& fieldName) const { return (*this)[fieldName]; }

    bool contains(const IdentifierList& qualifiedFieldName) const { return fieldsByName.contains(qualifiedFieldName); }
