...
public:
  /**
   * @brief Enum to identify the memory layout in which we want to represent the
   * schema physically.
   */
   ...
   */
  static SchemaPtr
  create(MemoryLayoutType layoutType = MemoryLayoutType::ROW_LAYOUT);

  /**
   * @brief Factory method to create a new SchemaPtr from schema type.
   * @return SchemaPtr
   */
  static SchemaPtr createFromSchemaType(
      const Configurations::SchemaTypePtr &schemaType,
      MemoryLayoutType layoutType = MemoryLayoutType::ROW_LAYOUT);

  /**
   * @brief Prepends the srcName to the substring after the last occurrence of
   * ATTRIBUTE_NAME_SEPARATOR in every field name of the schema.
   * @param srcName
   * @return SchemaPtr
   */
  ...
  /**
   * @brief appends a AttributeField to the schema and returns a copy of this
   * schema.
   * @param attribute
   * @return a copy of this schema.
   */
  ...
  /**
   * @brief appends a field with a basic type to the schema and returns a copy
   * of this schema.
   * @param field
   * @return a copy of this schema.
   */
  ...
  /**
   * @brief appends a field with a data type to the schema and returns a copy of
   * this schema.
   * @param field
   * @return a copy of this schema.
   */
  ...
  /**
   * @brief Returns the attribute field based on a qualified or unqualified
   * field name.
   *
   * @details
   * If an unqualified field name is given (e.g., `getField("fieldName")`), the
   * function will match attribute fields with any source name. If a qualified
   * field name is given (e.g., `getField("source$fieldName")`), the entire
   * qualified field must match.
   *
  ...
  /**
   * @brief Checks if attribute field name is defined in the schema and returns
   * its index. If item not in the list, then the return value is equal to
   * fields.size().
   * @param fieldName
   * @return the index
   */
   ...
   * @param schema
   * @param considerOrder takes into account if the order of fields in a schema
   * matter.
   * @return boolean
   */
   ...
   */
  [[nodiscard]] std::string toString(const std::string &prefix = "",
  ...
  /**
   * @brief Method to return the source name qualifier, thus everything that is
   * before $
   * @return string
   */
...
private:
   ...
   */
  static DataTypePtr stringToFieldType(const std::string &fieldType,
                                       const std::string &fieldLength);
