...
class SchemaTest : public Testing::BaseUnitTest {
 public:
  ...
  static void TearDownTestCase() {
    NES_INFO("SchemaTest test class TearDownTestCase.");
  }
  ...
  auto getRandomFields(const auto numberOfFields) {
    ...
    for (auto fieldCnt = 0_u64; fieldCnt < numberOfFields; ++fieldCnt) {
      ...
      rndFields.emplace_back(AttributeField::create(
          fieldName, DataTypeFactory::createType(basicType)));
    }
...
TEST_F(SchemaTest, createTest) {
  {
    ...
    ASSERT_EQ(testSchema->getLayoutType(),
...

  {
    ...
    SchemaPtr testSchema;
    ASSERT_NO_THROW(testSchema =
                        Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT));
    ASSERT_TRUE(testSchema);
    ASSERT_EQ(testSchema->getLayoutType(),
              Schema::MemoryLayoutType::ROW_LAYOUT);
  }

  {
    ...
    SchemaPtr testSchema;
    ASSERT_NO_THROW(
        testSchema = Schema::create(Schema::MemoryLayoutType::COLUMNAR_LAYOUT));
    ASSERT_TRUE(testSchema);
    ASSERT_EQ(testSchema->getLayoutType(),
...
TEST_F(SchemaTest, addFieldTest) {
  {
    ...
    for (const auto& basicTypeVal : magic_enum::enum_values<BasicType>()) {
      ...
      ASSERT_TRUE(testSchema->fields[0]->getDataType()->equals(
          DataTypeFactory::createType(basicTypeVal)));
    }
  }
...
TEST_F(SchemaTest, replaceFieldTest) {
...

  {
    ...
    for (const auto& replaceField : replacingFields) {
      testSchema->replaceField(replaceField->getName(),
                               replaceField->getDataType());
    }
    ...
    for (const auto& basicTypeVal : magic_enum::enum_values<BasicType>()) {
      ...
      ASSERT_EQ(testSchema->getSchemaSizeInBytes(),
                defaultPhysicalTypeFactory
                    .getPhysicalType(DataTypeFactory::createType(basicTypeVal))
                    ->size());
    }
  }
...
TEST_F(SchemaTest, copyTest) {
  ...
  ASSERT_EQ(testSchema->getSchemaSizeInBytes(),
            testSchemaCopy->getSchemaSizeInBytes());
  ...
  // Comparing fields
  for (auto fieldCnt = 0_u64; fieldCnt < testSchemaCopy->fields.size();
       ++fieldCnt) {
