...
AttributeField::AttributeField(std::string name, DataTypePtr dataType)
    : name(std::move(name)), dataType(std::move(dataType)) {}

AttributeFieldPtr AttributeField::create(const std::string &name,
...
bool AttributeField::isEqual(const AttributeFieldPtr &attr) const {
  ...
  const bool equalDataType =
      (this->dataType == nullptr && attr->dataType == nullptr) ||
      (this->dataType != nullptr && attr->dataType != nullptr &&
       this->dataType->equals(attr->dataType));
...
uint64_t AttributeField::hash() const {
  ...
  hashValue =
      hashValue * 0x9e3779b1 + std::hash<std::string>{}(dataType->toString());
  return hashValue;
}
