...
Schema::Schema(const SchemaPtr &schema, MemoryLayoutType layoutType)
...
AttributeFieldPtr Schema::get(const std::string &fieldName) const {
  ...
  NES_FATAL_ERROR("Schema: No field in the schema with the identifier {}",
                  fieldName);
...
AttributeFieldPtr Schema::get(uint32_t index) {
  ...
  throw std::invalid_argument("field id " + std::to_string(index) +
                              " does not exist");
}
...
bool Schema::equals(const SchemaPtr &schema, bool considerOrder) {
  ...
  if (considerOrder) {
    for (auto i = static_cast<decltype(fields)::size_type>(0);
         i < fields.size(); ++i) {
  ...
  for (auto const &fieldAttribute : fields) {
    ...
    if (!(otherFieldAttribute &&
          otherFieldAttribute->isEqual(fieldAttribute))) {
      return false;
    }
  }
  return true;
...
bool Schema::hasEqualTypes(const SchemaPtr &otherSchema) {
  ...
  // Iterate over all fields and check in both schemas the same index that they
  // have is the same attribute type
...
std::string Schema::toString(const std::string &prefix, const std::string &sep,
...
std::string Schema::getSourceNameQualifier() const {
  ...
  return fields[0]->getName().substr(
      0, fields[0]->getName().find(ATTRIBUTE_NAME_SEPARATOR));
}
...
std::string
Schema::getQualifierNameForSystemGeneratedFieldsWithSeparator() const {
...
std::string Schema::getQualifierNameForSystemGeneratedFields() const {
  ...
  NES_ERROR("Schema::getQualifierNameForSystemGeneratedFields: a schema is not "
            "allowed to be empty when a qualifier is "
            "requested");
  return "";
}
...
bool Schema::contains(const std::string &fieldName) const {
  for (const auto &field : this->fields) {
    NES_TRACE("contain compare field={} with other={}", field->getName(),
              fieldName);
    if (field->getName() == fieldName) {
      return true;
    }
...
AttributeFieldPtr Schema::getField(const std::string &fieldName) const {
  ...
  if (stringToMatch.find(ATTRIBUTE_NAME_SEPARATOR) == std::string::npos) {
    ...
    // E.g: if schema contains car$speed and truck$speed and user wants to check
    // if attribute speed is present then system should throw invalid field
    // exception
  ...
  for (auto &field : fields) {
    ...
    if (stringToMatch.length() <= fullyQualifiedFieldName.length()) {
      ...
      auto startingPos =
          fullyQualifiedFieldName.length() - stringToMatch.length();
      auto found = fullyQualifiedFieldName.compare(
          startingPos, stringToMatch.length(), stringToMatch);
      if (found == 0) {
  ...
  if (matchedFields.size() > 1) {
    //        throw InvalidFieldException("Schema: Found ambiguous field with
    //        name " + fieldName);
    // TODO: workaround we choose the first one to join we will replace this in
    // issue #1543
    return matchedFields[0];
  }
  return nullptr;
...
std::vector<std::string> Schema::getFieldNames() const {
  ...
  // Todo: 4049: size can be corrupted if schema gets corrupted
  // (18446741141687656808 fields)
...
DataTypePtr Schema::stringToFieldType(const std::string &fieldNodeType,
                                      const std::string &fieldNodeLength) {
  if (fieldNodeType == "CHAR") {
    if (fieldNodeLength.empty() || fieldNodeLength == "\n" ||
      ...
      NES_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. "
                              "Please define Schema Field Length properly.");
    }
...
SchemaPtr
Schema::createFromSchemaType(const Configurations::SchemaTypePtr &schemaType,
  ...
  for (const auto &schemaFieldDetail : schemaType->getSchemaFieldDetails()) {
    schema->addField(
        schemaFieldDetail.fieldName,
        stringToFieldType(schemaFieldDetail.fieldType,
                          schemaFieldDetail.variableLengthInBytes));
  }
  return schema;
}
...
SchemaPtr Schema::updateSourceName(const std::string &srcName) const {
  for (const auto &field : fields) {
    auto currName = Util::splitWithStringDelimiter<std::string>(
        field->getName(), ATTRIBUTE_NAME_SEPARATOR);
