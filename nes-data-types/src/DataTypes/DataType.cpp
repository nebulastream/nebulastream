...
///
/// VARSIZED and VECTOR are not inlined and instead represented via ptr into the child buffer
...
std::optional<DataType> DataType::join(const DataType& otherDataType) const
{
    ...
    if (this->type == Type::VECTOR)
    {
        ...
        {
            return DataType{Type::VECTOR, isNullableResult, this->elementType[0]};
        }
        return std::nullopt;
    }
...
DataType Unreflector<DataType>::operator()(const Reflected& rfl, const ReflectionContext& context) const
{
    ...
    if (reflected.type == DataType::Type::VECTOR || reflected.type == DataType::Type::FIXEDSIZED)
    {
        ...
        if (reflected.type == DataType::Type::VECTOR)
        {
