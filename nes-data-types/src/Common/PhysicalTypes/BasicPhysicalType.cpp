...
PhysicalTypePtr BasicPhysicalType::create(const DataTypePtr &type,
...
uint64_t BasicPhysicalType::size() const {
  ...
  case CHAR:
    return sizeof(char);
  case TEXT:
    return sizeof(uint32_t); // we store the child buffer index
                             // (NestedTupleBufferKey) in the tuple
  case UNDEFINED:
    return -1;
  }
  return -1;
...
std::string
BasicPhysicalType::convertRawToString(void const *data) const noexcept {
  if (!data) {
    return std::string();
  }
  switch (nativeType) {
  ...
  case UINT_8:
    return std::to_string(*reinterpret_cast<uint8_t const *>(data));
  case INT_16:
  ...
  case INT_64:
    return std::to_string(*reinterpret_cast<int64_t const *>(data));
  case UINT_64:
  ...
  case TEXT: {
    NES_DEBUG("BasicPhysicalType::convertRawToString(): TEXT conversion to "
              "string not implemented. Should never be here.");
...
std::string BasicPhysicalType::convertRawToStringWithoutFill(
  ...
  case BOOLEAN:
    return std::to_string(*reinterpret_cast<bool const *>(data));
  case TEXT: {
    return "BasicPhysicalType::convertRawToString(): TEXT conversion to string "
           "not implemented. Should never be here.";
  }
  case CHAR:
    if (size() != 1) {
      return "invalid char type";
