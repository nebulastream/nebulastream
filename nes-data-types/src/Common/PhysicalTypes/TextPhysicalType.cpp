...
std::string TextPhysicalType::convertRawToStringWithoutFill(void const* data) const noexcept {
    NES_ASSERT(data, "Pointer to variable sized data is invalid. Buffer must at least contain the length (0 if empty).");
    ...
    NES_ASSERT(textPointer, "Pointer to text is not valid after casting to char const*.");
