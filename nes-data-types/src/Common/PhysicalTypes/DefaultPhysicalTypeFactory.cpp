...
PhysicalTypePtr
DefaultPhysicalTypeFactory::getPhysicalType(DataTypePtr dataType) const {
  if (dataType->isBoolean()) {
    return BasicPhysicalType::create(dataType,
    ...
    NES_THROW_RUNTIME_ERROR(
        "It was not possible to infer a physical type for: " +
        dataType->toString());
  }
}

PhysicalTypePtr
DefaultPhysicalTypeFactory::getPhysicalType(const IntegerPtr &integer) {
...
PhysicalTypePtr
DefaultPhysicalTypeFactory::getPhysicalType(const FloatPtr &floatType) {
  if (floatType->getBits() <= 32) {
    return BasicPhysicalType::create(floatType,
...
PhysicalTypePtr
DefaultPhysicalTypeFactory::getPhysicalType(const ArrayPtr &arrayType) const {
...
PhysicalTypePtr
DefaultPhysicalTypeFactory::getPhysicalType(const CharPtr &charType) {
  return BasicPhysicalType::create(charType,
                                   BasicPhysicalType::NativeType::CHAR);
}

PhysicalTypePtr
DefaultPhysicalTypeFactory::getPhysicalType(const TextPtr &textType) const {
  return BasicPhysicalType::create(textType,
                                   BasicPhysicalType::NativeType::TEXT);
}
