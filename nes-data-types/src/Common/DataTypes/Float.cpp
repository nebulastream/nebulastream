...
bool Float::equals(DataTypePtr otherDataType) {
  if (otherDataType->isFloat()) {
    ...
    return bits == otherFloat->bits && lowerBound == otherFloat->lowerBound &&
           upperBound == otherFloat->upperBound;
  }
  return false;
}
...
DataTypePtr Float::join(DataTypePtr otherDataType) {
  ...
  if (otherDataType->isInteger()) {
    ...
    auto newUpperBound =
        fmax(upperBound, static_cast<double>(otherInteger->upperBound));
    auto newLowerBound =
        fmin(lowerBound, static_cast<double>(otherInteger->lowerBound));
    return DataTypeFactory::createFloat(newBits, newLowerBound, newUpperBound);
  }
