...
std::unique_ptr<DataTypeRegistryReturnType> DataTypeGeneratedRegistrar::RegisterFLOAT32DataType(DataTypeRegistryArguments args)
{
    return std::make_unique<Float>(args.nullable, 32, std::numeric_limits<float>::lowest(), std::numeric_limits<float>::max());
}

std::unique_ptr<DataTypeRegistryReturnType> DataTypeGeneratedRegistrar::RegisterFLOAT64DataType(DataTypeRegistryArguments args)
{
    return std::make_unique<Float>(args.nullable, 64, std::numeric_limits<double>::lowest(), std::numeric_limits<double>::max());
}
