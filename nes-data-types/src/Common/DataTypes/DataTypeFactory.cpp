...
DataTypePtr DataTypeFactory::createDouble() {
  return createFloat(64, std::numeric_limits<double>::max() * -1,
                     std::numeric_limits<double>::max());
}

DataTypePtr DataTypeFactory::createInteger(int8_t bits, int64_t lowerBound,
                                           int64_t upperBound) {
  return std::make_shared<Integer>(bits, lowerBound, upperBound);
}

DataTypePtr DataTypeFactory::createInteger(int64_t lowerBound,
                                           int64_t upperBound) {
  // derive the correct bite size for the correct lower and upper bound
  auto bits = upperBound <= INT8_MAX    ? 8
              : upperBound <= INT16_MAX ? 16
...
}; // TODO / BUG: upper bound is a INT64 and can not capture this upper bound.
   // -> upperbound overflows and is set to -1.
...
DataTypePtr DataTypeFactory::createArray(uint64_t length,
...
ValueTypePtr DataTypeFactory::createBasicValue(DataTypePtr type,
                                               std::string value) {
...
ValueTypePtr DataTypeFactory::createBasicValue(BasicType type,
                                               std::string value) {
  return createBasicValue(createType(type), std::move(value));
}

ValueTypePtr DataTypeFactory::createArrayValueFromContainerType(
...
ValueTypePtr DataTypeFactory::createArrayValueWithContainedType(
    const DataTypePtr &type, std::vector<std::string> &&values) noexcept {
  auto const length = values.size();
  return std::make_shared<ArrayValue>(createArray(length, type),
                                      std::move(values));
}

ValueTypePtr
DataTypeFactory::createFixedCharValue(const std::string &values) noexcept {
  return createFixedCharValue(values.c_str());
}

ValueTypePtr DataTypeFactory::createFixedCharValue(
    std::vector<std::string> &&values) noexcept {
  return createArrayValueWithContainedType(createChar(), std::move(values));
}

ValueTypePtr
DataTypeFactory::createFixedCharValue(char const *values) noexcept {
...
DataTypePtr
DataTypeFactory::copyTypeAndIncreaseLowerBound(DataTypePtr stamp,
                                               double minLowerBound) {
  if (stamp->isFloat()) {
    if (auto const floatStamp = DataType::as<Float>(stamp);
        floatStamp->lowerBound < minLowerBound) {
      return createFloat(floatStamp->getBits(), minLowerBound,
                         floatStamp->upperBound);
    }
  } else if (stamp->isInteger()) {
    if (auto const intStamp = DataType::as<Integer>(stamp);
        intStamp->lowerBound < minLowerBound) {
      NES_WARNING("DataTypeFactory: A Float is passed as the minimum lower "
                  "bound of an Integer data type. Will be executed "
                  "with the Floor of the Float argument instead.");
      return createInteger(intStamp->getBits(),
                           static_cast<int64_t>(minLowerBound),
                           intStamp->upperBound);
    }
  } else {
    // non-numeric data types do not have a lower bound
    NES_ERROR("DataTypeFactory: Can not increase a lower bound on a "
              "non-numeric data type.");
  }
  return stamp; // increase does not apply -> return shared pointer given as
                // argument
}

DataTypePtr
DataTypeFactory::copyTypeAndIncreaseLowerBound(DataTypePtr stamp,
                                               int64_t minLowerBound) {
  if (stamp->isInteger()) {
    if (auto const intStamp = DataType::as<Integer>(stamp);
        intStamp->lowerBound < minLowerBound) {
      return createInteger(intStamp->getBits(), minLowerBound,
                           intStamp->upperBound);
    }
  } else if (stamp->isFloat()) {
    if (auto const floatStamp = DataType::as<Float>(stamp);
        floatStamp->lowerBound < minLowerBound) {
      NES_INFO("DataTypeFactory: An Integer is passed as the minimum lower "
               "bound of a Float data type. Progresses with "
               "standard casting to Double.");
      return createFloat(floatStamp->getBits(),
  ...
  NES_INFO("DataTypeFactory: Increase of lower bound does not apply. Returning "
           "original stamp.");
  return stamp; // increase does not apply -> return shared pointer given as
                // argument
}
...
DataTypePtr
DataTypeFactory::copyTypeAndDecreaseUpperBound(DataTypePtr stamp,
                                               double maxUpperBound) {
  if (stamp->isFloat()) {
    if (auto const floatStamp = DataType::as<Float>(stamp);
        maxUpperBound < floatStamp->upperBound) {
      return createFloat(floatStamp->getBits(), floatStamp->lowerBound,
                         maxUpperBound);
    }
  } else if (stamp->isInteger()) {
    if (auto const intStamp = DataType::as<Integer>(stamp);
        maxUpperBound < intStamp->upperBound) {
      NES_WARNING("DataTypeFactory: A Float is passed as the maximum upper "
                  "bound of an Integer data type. Progresses with "
                  "the Ceiling of the Float argument instead.");
      return createInteger(intStamp->getBits(), intStamp->lowerBound,
  ...
  NES_INFO("DataTypeFactory: Decrease of upper bound does not apply. Returning "
           "original stamp.");
  return stamp; // decrease does not apply -> return shared pointer given as
                // argument
}

DataTypePtr
DataTypeFactory::copyTypeAndDecreaseUpperBound(DataTypePtr stamp,
                                               int64_t maxUpperBound) {
  if (stamp->isInteger()) {
    if (auto const intStamp = DataType::as<Integer>(stamp);
        maxUpperBound < intStamp->upperBound) {
      return createInteger(intStamp->getBits(), intStamp->lowerBound,
    ...
    if (auto const floatStamp = DataType::as<Float>(stamp);
        maxUpperBound < floatStamp->upperBound) {
      NES_INFO("DataTypeFactory: An Integer is passed as the maximum upper "
               "bound of an Float data type. Progresses with "
               "standard casting to Double.");
      return createFloat(floatStamp->getBits(), floatStamp->lowerBound,
...
DataTypePtr DataTypeFactory::copyTypeAndTightenBounds(DataTypePtr stamp,
                                                      int64_t minLowerBound,
                                                      int64_t maxUpperBound) {
  if (stamp->isInteger()) {
    if (auto const intStamp = DataType::as<Integer>(stamp);
        intStamp->lowerBound < minLowerBound) {
      // we must create a new stamp for an increased lower bound, so we
      // calculate the upper bound by predication
    ...
    NES_INFO("DataTypeFactory: Integers are passed as new bounds of an Float "
             "data type. Progresses with standard casting to "
             "Double.");
    return DataTypeFactory::copyTypeAndTightenBounds(
        stamp, static_cast<double>(minLowerBound),
        static_cast<double>(maxUpperBound));
  } else {
    // non-numeric data types do not have a lower bound
    NES_ERROR(
        "DataTypeFactory: Can not modify bounds on a non-numeric data type.");
  }
  NES_INFO("DataTypeFactory: Lower and upper bound do not need to be changed. "
           "Returning original stamp.");
  return stamp; // neither bound needs to be modified -> return shared pointer
                // given as argument
}
...
DataTypePtr DataTypeFactory::copyTypeAndTightenBounds(DataTypePtr stamp,
  ...
  if (stamp->isFloat()) {
    ...
    NES_INFO("DataTypeFactory: Floats are passed as new bounds of an Integer "
             "data type. Progresses with the floor of the "
             "lower bound and ceiling of the upper bound instead.");
    return DataTypeFactory::copyTypeAndTightenBounds(
        stamp, static_cast<int64_t>(std::floor(minLowerBound)),
        static_cast<int64_t>(std::ceil(maxUpperBound)));
  } else {
    // non-numeric data types do not have a lower bound
    NES_ERROR(
...
DataTypePtr DataTypeFactory::createFloatFromInteger(DataTypePtr stamp) {
  if (stamp->isInteger()) {
    ...
    return DataTypeFactory::createFloat(intStamp->lowerBound,
                                        intStamp->upperBound);
  } else if (stamp->isFloat()) {
    NES_INFO("DataTypeFactory: A Float is passed to be converted to a Float. "
             "Return stamp passed as argument.");
  } else {
