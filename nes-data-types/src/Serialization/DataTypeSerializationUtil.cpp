...
SerializableDataType *DataTypeSerializationUtil::serializeDataType(
    const DataTypePtr &dataType, SerializableDataType *serializedDataType) {
  if (dataType->isUndefined()) {
    ...
    // store dimension and datatype into ArrayDetails by invoking this function
    // recursively.
    ...
    serializeDataType(arrayType->component,
                      serializedArray.mutable_componenttype());
    ...
    NES_THROW_RUNTIME_ERROR(
        "DataTypeSerializationUtil: serialization is not possible for " +
        dataType->toString());
  }
  NES_TRACE("DataTypeSerializationUtil:: serialized {} to {}",
            dataType->toString(), serializedDataType->SerializeAsString());
  return serializedDataType;
}

DataTypePtr DataTypeSerializationUtil::deserializeDataType(
    const SerializableDataType &serializedDataType) {
  NES_TRACE("DataTypeSerializationUtil:: de-serialized {}",
            serializedDataType.DebugString());
  ...
  if (serializedDataType.type() == SerializableDataType_Type_CHAR) {
    ...
    return DataTypeFactory::createInteger(integerDetails.bits(),
                                          integerDetails.lowerbound(),
                                          integerDetails.upperbound());
  ...
  NES_THROW_RUNTIME_ERROR("DataTypeSerializationUtil: data type which is to be "
                          "serialized not registered. "
                          "Deserialization is not possible");
}

std::shared_ptr<ArrayType> DataTypeSerializationUtil::deserializeArrayType(
    const SerializableDataType &serializedDataType) {
  // for arrays get additional information from the
  // SerializableDataType_ArrayDetails
...
SerializableDataValue *DataTypeSerializationUtil::serializeDataValue(
    const ValueTypePtr &valueType, SerializableDataValue *serializedDataValue) {
  ...
  if (valueType->dataType->isArray()) {
    ...
    serializeDataType(arrayValueType->dataType,
                      serializedArrayValue.mutable_type());
    ...
    auto const basicValueType =
        std::dynamic_pointer_cast<BasicValue>(valueType);
    ...
    serializeDataType(basicValueType->dataType,
                      serializedBasicValue.mutable_type());
    // 4. serialize basic type
    serializedDataValue->mutable_value()->PackFrom(serializedBasicValue);
  }
  NES_TRACE("DataTypeSerializationUtil:: serialized {} as {} ",
            valueType->toString(), serializedDataValue->DebugString());
  return serializedDataValue;
}

ValueTypePtr DataTypeSerializationUtil::deserializeDataValue(
    const SerializableDataValue &serializedDataValue) {
  ...
  if (dataValue.Is<SerializableDataValue_BasicValue>()) {
    ...
    return DataTypeFactory::createBasicValue(dataTypePtr,
                                             serializedBasicValue.value());
  }
  if (dataValue.Is<SerializableDataValue_ArrayValue>()) {
    ...
    return DataTypeFactory::createArrayValueFromContainerType(
        std::move(dataTypePtr), std::move(values));
  }
  NES_THROW_RUNTIME_ERROR("DataTypeSerializationUtil: deserialization of value "
