...
SerializableUnboundSchema* UnboundSchemaSerializationUtil::serializeUnboundSchema(
    const UnboundOrderedSchema& unboundSchema, SerializableUnboundSchema* serializedUnboundSchema)
{
    for (const auto& field : unboundSchema)
    {
        auto* unboundField = serializedUnboundSchema->add_fields();
        IdentifierSerializationUtil::serializeIdentifierList(field.getFullyQualifiedName(), unboundField->mutable_name());
        DataTypeSerializationUtil::serializeDataType(field.getDataType(), unboundField->mutable_type());
    }
    return serializedUnboundSchema;
}

UnboundOrderedSchema UnboundSchemaSerializationUtil::deserializeUnboundSchema(const SerializableUnboundSchema& serializedUnboundSchema)
{
    ...
    return UnboundOrderedSchema{fields};
}
