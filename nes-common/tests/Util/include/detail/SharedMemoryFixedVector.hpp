...
template <typename T> class SharedMemoryFixedVector {
  struct Metadata {
...
public:
  explicit SharedMemoryFixedVector(const std::string &name, size_t capacity)
      : name(name), mmapSize(sizeof(T) * capacity + sizeof(Metadata)),
        capacity(capacity), created(false) {}
  ...
  void open() {
    ...
    if (shmemFd == -1) {
      std::cerr << "cannot create shared area: " << strerror(errno)
                << std::endl;
    }
    NES_ASSERT2_FMT(shmemFd != -1,
                    "cannot create shared area: " << strerror(errno));
    if (created) {
      auto ret = ftruncate(shmemFd, mmapSize) == 0;
      if (!ret) {
        std::cerr << "cannot ftruncate shared area: " << strerror(errno)
                  << std::endl;
      }
      NES_ASSERT(ret, "cannot create shared area");
    } else {
      std::string filePath =
          std::filesystem::temp_directory_path() / "nes.tests.begin";
      while (!std::filesystem::exists(filePath)) {
        NES_DEBUG("File {} does not exists", filePath);
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
      }
    }
    mem = reinterpret_cast<uint8_t *>(mmap(
        nullptr, mmapSize, PROT_READ | PROT_WRITE, MAP_SHARED, shmemFd, 0));
    if (mem == MAP_FAILED) {
      std::cerr << "cannot create shared area: " << strerror(errno)
                << std::endl;
    }
  ...
  uint64_t getNextIndex() {
    return metadata->currentIndex.fetch_add(1) % capacity;
  }
