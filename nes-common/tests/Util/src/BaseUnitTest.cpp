...
void BaseUnitTest::SetUp() {
  testing::Test::SetUp();
  Exceptions::installGlobalErrorListener(
      self = std::shared_ptr<Exceptions::ErrorListener>(this, Deleter()));
...
void BaseUnitTest::onFatalException(std::shared_ptr<std::exception> exception,
        ...
        switch (future.wait_for(std::chrono::minutes(WAIT_TIME_SETUP))) {
          case std::future_status::ready: {
            try {
              ...
              NES_FATAL_ERROR("Got exception in test [{}]: {}", testName,
                              exception.what());
              FAIL();
            }
            break;
