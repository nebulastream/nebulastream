...
PortDispatcher::PortDispatcher(uint16_t startPort, uint32_t numberOfPorts)
    : mutex(detail::getMutexPath()),
      data(detail::getPortPoolName(), numberOfPorts) {
  std::unique_lock<Util::FileMutex> lock(mutex, std::defer_lock);
  if (lock.try_lock()) {
    data.open();
    if (data.isCreated()) {
      for (uint16_t current = startPort, i = 0; i < numberOfPorts;
           ++i, ++current) {
        data[i].port = current;
        data[i].free = true;
...
BorrowedPortPtr PortDispatcher::getNextPort() {
  ...
  while (++tryCount < 100) {
    ...
    if (data[nextIndex].checksum != CHECKSUM) {
      std::cerr << "invalid checksum " << data[nextIndex].checksum
                << "!=" << CHECKSUM << std::endl;
    }
    ...
    if (data[nextIndex].free.compare_exchange_strong(expected, false)) {
      ++numOfBorrowedPorts;
      return std::make_shared<BorrowedPort>(data[nextIndex].port, nextIndex,
                                            this);
    }
...
void PortDispatcher::recyclePort(uint32_t portIndex) {
  ...
  if (data[portIndex].free.compare_exchange_strong(expected, true)) {
    ...
    NES_ASSERT2_FMT(false,
                    "Port " << data[portIndex].port << " recycled twice");
  }
}
...
void PortDispatcher::tearDown() {
  NES_ASSERT2_FMT(numOfBorrowedPorts == 0,
                  "There are "
                      << numOfBorrowedPorts
                      << " leaked ports - please check your test logic");
}
