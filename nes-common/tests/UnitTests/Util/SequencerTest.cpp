...
TEST_F(SequencerTest, InOrderSingleSequence)
{
    ...
    SequenceData data1 = {INITIAL<SequenceNumber>, INITIAL<ChunkNumber>, false};
    SequenceData data2 = {INITIAL<SequenceNumber>, ChunkNumber(ChunkNumber::INITIAL + 1), false};
    SequenceData data3 = {INITIAL<SequenceNumber>, ChunkNumber(ChunkNumber::INITIAL + 2), true};
...
TEST_F(SequencerTest, MultipleSequences)
{
    ...
    SequenceData seq1data1 = {INITIAL<SequenceNumber>, INITIAL<ChunkNumber>, true}; // First sequence with single chunk
    SequenceData seq2data1 = {SequenceNumber(SequenceNumber::INITIAL + 1), INITIAL<ChunkNumber>, false}; // Second sequence, first chunk
    SequenceData seq2data2 = {
...
class ConcurrentSequencerTest : public ::testing::TestWithParam<std::tuple<size_t, int, size_t>>
{
    ...
    {
        int pre = result[0];
        for (int result1 : result | std::views::drop(1))
        {
...
TEST_P(ConcurrentSequencerTest, MultiThreaded)
{
    ...
    {
        threadData[i % numThreads].push_back(values[i]);
    }
