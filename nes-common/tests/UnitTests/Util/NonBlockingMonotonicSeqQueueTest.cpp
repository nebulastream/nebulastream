...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NonBlockingMonotonicSeqQueueTest.log",
   ...
   * @return CurrentValue
   */
  uint64_t emplaceInMockupQueue(const SequenceData &seqDataToInsert,
    ...
    while (chunkStateNextSeq != seenSequenceData.end()) {
      if (chunkStateNextSeq->second.seenChunks !=
          chunkStateNextSeq->second.lastChunkNumber) {
        break;
      }
 ...
 * We create a sequential list of 10k updates, monotonically increasing from 1
 * to 10k and push them to the watermark processor. Assumption: As we insert all
 * updates in a sequential fashion we assume that the getCurrentWatermark is
 * equal to the latest processed update.
 */
TEST_F(NonBlockingMonotonicSeqQueueTest, singleThreadSequentialUpdaterTest) {
  auto updates = 10000_u64;
  auto watermarkProcessor =
      Sequencing::NonBlockingMonotonicSeqQueue<uint64_t>();
  ...
  for (auto i = 0_u64; i < updates; i++) {
    ...
    watermarkProcessor.emplace(std::get<0>(currentWatermarkBarrier),
                               std::get<1>(currentWatermarkBarrier));
    ASSERT_EQ(watermarkProcessor.getCurrentValue(),
              std::get<1>(currentWatermarkBarrier));
  }
  ASSERT_EQ(watermarkProcessor.getCurrentValue(),
            std::get<1>(watermarkBarriers.back()));
}

/**
 * @brief A single thread test for the lock free watermark processor.
 * We create a reverse sequential list of 10k updates, monotonically decreasing
 * from 10k to 1 and push them to the watermark processor. Assumption: As we
 * insert all updates in a sequential fashion we assume that the
 * getCurrentWatermark is equal to the latest processed update.
 */
TEST_F(NonBlockingMonotonicSeqQueueTest,
  ...
  // add the last remaining watermark, as a result we now apply all remaining
  // watermarks.
  watermarkProcessor.emplace(std::get<0>(watermarkBarriers.back()),
                             std::get<1>(watermarkBarriers.back()));
  ASSERT_EQ(watermarkProcessor.getCurrentValue(),
            std::get<1>(watermarkBarriers.front()));
}
...
TEST_F(NonBlockingMonotonicSeqQueueTest, singleThreadRandomeUpdaterTest) {
  ...
  std::shuffle(watermarkBarriers.begin(), watermarkBarriers.end(),
               randomGenerator);
...
TEST_F(NonBlockingMonotonicSeqQueueTest,
  ...
  auto watermarkProcessor =
      Sequencing::NonBlockingMonotonicSeqQueue<uint64_t, 10000>();
  ...
  for (int threadId = 0; threadId < threadsCount; threadId++) {
    threads.emplace_back([&watermarkProcessor, this, &globalUpdateCounter]() {
      ...
      for (auto i = 0; i < updates; i++) {
        ...
        auto oldWatermark = watermarkProcessor.getCurrentValue();
        // check if the watermark manager does not return a watermark higher
        // than the current one
        ASSERT_LT(oldWatermark, std::get<1>(currentWatermark));
        watermarkProcessor.emplace(std::get<0>(currentWatermark),
                                   std::get<1>(currentWatermark));
        // check that the watermark manager returns a watermark that is <= to
        // the max watermark
...
TEST_F(NonBlockingMonotonicSeqQueueTest,
  ...
  const auto threadsCount = 10;
  auto watermarkProcessor =
      Sequencing::NonBlockingMonotonicSeqQueue<uint64_t, 1000>();
  ...
  for (int threadId = 0; threadId < threadsCount; threadId++) {
    threads.emplace_back([&watermarkProcessor, this, &globalUpdateCounter]() {
      ...
      for (auto i = 0; i < updates; i++) {
        ...
        // the max watermark
        auto globalCurrentWatermark = watermarkProcessor.getCurrentValue();
        ASSERT_LE(globalCurrentWatermark,
                  std::get<1>(watermarkBarriers[lostUpdate]));
      }
    });
  }
  ...
  watermarkProcessor.emplace(std::get<0>(watermarkBarriers[lostUpdate]),
                             std::get<1>(watermarkBarriers[lostUpdate]));
...
/**
 * @brief We test here to insert sequence and chunks numbers in a "random"
 * fashion and then check, if the correct output is produced
 */
TEST_F(NonBlockingMonotonicSeqQueueTest,
  ...
  for (auto i = 1_u64; i <= noSeqNumbers; i++) {
    ...
    for (auto chunk = 1_u64; chunk < noChunks; ++chunk) {
      watermarkBarriers.emplace_back(std::tuple<SequenceData, uint64_t>(
          /*sequence data*/ {i, chunk, false}, /*ts*/ i));
    }
    watermarkBarriers.emplace_back(std::tuple<SequenceData, uint64_t>(
        /*sequence data*/ {i, noChunks, true},
        /*ts*/ i));
  }
  ...
  for (const auto &currentWatermarkBarrier : watermarkBarriers) {
    ...
    auto currentValueExpected =
        emplaceInMockupQueue(seqDataToInsert, valueToInsert);
...
/**
 * @brief We test here to insert sequence and chunks numbers in a "random"
 * fashion and then check, if the correct output is produced. We do this in a
 * concurrent fashion
 */
TEST_F(NonBlockingMonotonicSeqQueueTest,
  ...
  while (globalUpdateCounter < watermarkBarriers.size()) {
    ...
    const auto updatesThisRound =
        std::min(missingUpdates, 1 + rand() % averageUpdatesPerRound);
    ...
    // It can happen that multiple threads write over the maxUpdatePos.
    // Therefore, we have to set it back.
