...
TEST_F(VersionTest, formatVersionContainsBinaryName)
{
    const auto output = formatVersion("my-binary");
    EXPECT_THAT(output, testing::HasSubstr("my-binary"));
}

TEST_F(VersionTest, formatVersionContainsAllFields)
{
...
TEST_F(VersionTest, formatVersionFirstLineIsBinaryNameAndVersion)
{
...
TEST_F(VersionTest, hasVersionFlagDetectsShortAndLongForms)
{
    {
...
TEST_F(VersionTest, hasVersionFlagReturnsFalseWithoutFlag)
{
    const std::array argv{"binary", "--help", "foo"};
    EXPECT_FALSE(hasVersionFlag(static_cast<int>(argv.size()), argv.data()));
}

TEST_F(VersionTest, hasVersionFlagHandlesEmptyArgv)
{
