...
TEST(WorkerLocalSingletonTest, PropagatesAcrossNestedThreads)
{
    ...
    Thread outer(
        ...
        []()
        {
            ...
            Thread inner("inner", []() { EXPECT_EQ(DummyService::instance().value, 9); });
        });
}
...
TEST(WorkerLocalSingletonTest, ChildThreadSeesShadowedInstance)
{
    ...
    {
        DummyService inner(2);
        Thread t("child", Host("w1"), []() { EXPECT_EQ(DummyService::instance().value, 2); });
    }
    Thread t("child2", Host("w1"), []() { EXPECT_EQ(DummyService::instance().value, 1); });
}
