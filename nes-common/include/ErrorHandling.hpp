...
public:
    ...
    Exception(std::string message, cpptrace::raw_trace&& trace)
        : cpptrace::lazy_exception(std::move(trace)), message(std::move(message)), errorCode(static_cast<ErrorCode>(9999)) { };
...
#undef EXCEPTION
    ...
    #define PRECONDITION(condition, formatString, ...) \
        ...
        { \
            if (!(condition)) \
            { \
                throw NES::UnknownException(formatString __VA_OPT__(, ) __VA_ARGS__); \
            } \
        } while (false)
