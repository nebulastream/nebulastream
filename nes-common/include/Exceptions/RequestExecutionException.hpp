...
/**
 * @brief This is the base class for exceptions thrown during the execution of
 * coordinator side requests which indicate an error that possibly require a
 * rollback or other kinds of specific error handling
 */
class RequestExecutionException
    : public std::enable_shared_from_this<RequestExecutionException>,
      public std::runtime_error {
public:
  explicit RequestExecutionException(const std::string &message);
  explicit RequestExecutionException(QueryId queryId,
   ...
   */
  template <class ExceptionType> bool instanceOf() {
   ...
   */
  template <class ExceptionType> std::shared_ptr<ExceptionType> as() {
    if (instanceOf<ExceptionType>()) {
      return std::dynamic_pointer_cast<ExceptionType>(this->shared_from_this());
    }
    throw std::logic_error(
        "Exception:: we performed an invalid cast of exception");
  }
