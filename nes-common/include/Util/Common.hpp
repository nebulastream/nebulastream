...
public:
  BufferMetaData(const uint64_t watermarkTs, const SequenceData seqNumber,
...
namespace NES::Util {
/**
 * @brief escapes all non text characters in a input string, such that the
 * string could be processed as json.
 ...
 */
template <typename T> struct SplitFunctionHelper {
  ...
  static constexpr auto FUNCTION = [](std::string_view str) {
    ...
    auto result = std::from_chars(
        trimmed.data(), trimmed.data() + trimmed.size(), result_value);
 ...
 */
template <> struct SplitFunctionHelper<std::string> {
...
/**
 * @brief splits a string given a delimiter into multiple substrings stored in a
 * T vector the delimiter is allowed to be a string rather than a char only.
 * @param data - the string that is to be split
 * @param delimiter - the string that is to be split upon e.g. / or -
 * @param fromStringtoT - the function that converts a string to an arbitrary
 * type T
 * @return
 */
template <typename T>
std::vector<T>
splitWithStringDelimiter(std::string_view inputString, std::string_view delim,
                         std::function<T(std::string_view)> fromStringToT =
                             detail::SplitFunctionHelper<T>::FUNCTION) {
  size_t prev_pos = 0;
  size_t next_pos = 0;
  std::vector<T> elems;
  ...
  while ((next_pos = inputString.find(delim, prev_pos)) != std::string::npos) {
    elems.push_back(
        fromStringToT(inputString.substr(prev_pos, next_pos - prev_pos)));
    prev_pos = next_pos + delim.size();
  }

  if (auto rest = inputString.substr(prev_pos, inputString.size());
      !rest.empty()) {
 ...
 */
template <typename T>
std::shared_ptr<T> checkNonNull(std::shared_ptr<T> ptr,
...
/**
 * @brief This function replaces the first occurrence of search term in a string
 ...
 * @return
 */
std::string replaceFirst(std::string origin, const std::string &search,
                         const std::string &replace);

/**
 * @brief: Update the source names by sorting and then concatenating the source
 * names from the sub- and query plan
 ...
 */
std::string updateSourceName(std::string queryPlanSourceConsumed,
...
/**
 * Partition a vector in n chunks, e.g., ([1, 2, 3, 4, 5], 3) -> [[1, 2], [3,
 * 4], [5]]
...
/**
 * @brief Tries to update curVal until it succeeds or curVal is larger then
 * newVal
 * @tparam T
 * @param curVal
 * @param newVal
...
void updateAtomicMax(std::atomic<T> &curVal, const T &newVal) {
  T prev_value = curVal;
  while (prev_value < newVal &&
         !curVal.compare_exchange_weak(prev_value, newVal)) {
  }
};
