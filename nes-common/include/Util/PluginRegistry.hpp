...
/**
 * @brief The plugin registry allows the dynamic registration of plugins at
 * runtime. A plugin is a provider of a specific type T, which defines the
 * plugin interface. Plugins use [[maybe_unused]] static T::Add<PluginXType>
 * pluginX; to register them self to the plugin.
 * @tparam T plugin interface type
 */
template <typename T> class PluginRegistry {

private:
  static inline std::list<std::unique_ptr<T>> items =
 ...
 * runtime. A plugin is a provider of a specific type T, which defines the
 * plugin interface. Plugins use [[maybe_unused]] static T::Add<PluginXType>
 * pluginX; to register them self to the registry.
 * @tparam T plugin interface type
 */
template <typename T> class NamedPluginRegistry {

private:
  static inline std::list<std::string> names = std::list<std::string>();
  static inline std::map<std::string, std::unique_ptr<T>> items =
...
public:
  ...
  static std::unique_ptr<T> &getPlugin(std::string name) {
    ...
    if (found == items.end()) {
      NES_THROW_RUNTIME_ERROR("No plugin with name " << name.c_str()
                                                     << " found.");
    }
    return found->second;
  }
...
/**
 * @brief The plugin factory allows the dynamic registration of plugins at that
 * dynamically create instances of the plugin at runtime. A plugin is a provider
 * of a specific type T, which defines the plugin interface. Plugins use
 * [[maybe_unused]] static T::Add<PluginXType> pluginX; to register them self to
 * the registry.
 * @tparam T plugin interface type
 */
template <typename T> class PluginFactory {
  template <typename X> class Provider {
  public:
    Provider() = default;
  ...
  template <class Base, class Sub> class TypedProvider : public Provider<Base> {
  public:
    [[nodiscard]] std::unique_ptr<Base> create() const override {
...
public:
   ...
   */
  template <typename V> class Add {
    static std::unique_ptr<Provider<T>> CtorFn() {
      return std::make_unique<TypedProvider<T, V>>();
    }
