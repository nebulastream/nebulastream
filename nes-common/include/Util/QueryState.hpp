 ...
 *
 * Registered: Query is registered to be scheduled to the worker nodes (added to
 * the queue). Optimizing: Coordinator is optimizing the query. Running: Query
 * is now running successfully. MarkedForHardStop: A request arrived into the
 * system for stopping a query and system marks the query for stopping (added to
 * the queue). MarkedForSoftStop: marked for soft stop after a source completed
