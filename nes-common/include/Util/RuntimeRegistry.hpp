...
/// auto-generated per-plugin glue translation units instead of at compile time via a
/// generated registerAll function (the older BaseRegistry mechanism). The linker keeps
...
/// upper case on insertion and lookup (mirrors the behavior of the compile-time Registry).
...
public:
    ...
    /// Register an entry. Returns false if an entry with this key already exists; the
    /// caller decides how to react. The generated glue throws at static initialization time
    /// so duplicates are loud and unrecoverable, while a runtime plugin-loading path can roll
    /// back or report the conflict instead of leaving the registry in a half-initialized state.
    ...
    /// Returns a pointer to the registered entry, or nullptr if the key is unknown. The
    /// pointer stays valid for the lifetime of the registry (entries are never removed).
    [[nodiscard]] const EntryTypeT* find(const KeyTypeT& name) const
    {
