...
/// A registry whose entries are registered at runtime, NOT static-initialization time but from main(), by
...
public:
    ...
    [[nodiscard]] std::vector<KeyTypeT> getRegisteredNames() const
    {
        ...
        std::ranges::transform(entries, std::back_inserter(names), [](const auto& entry) { return entry.first; });
        return names;
    }
