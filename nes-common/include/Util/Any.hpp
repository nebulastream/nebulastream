...
/// expected's converting constructors (__cons_from_expected) recursively depend on themselves.
/// Restricting the constructor to exactly std::any makes this type not constructible from the surrounding expected, breaking the cycle.
/// Without the same_as constraint, any parameter would be accepted via std::any's
...
public:
    ...
    [[nodiscard]] const std::any& getUnderlying() const { return value; }
