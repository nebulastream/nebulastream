...
/// The period is part of the static type, so it does not need to be serialized.
template <typename Rep, typename Period>
struct Reflector<std::chrono::duration<Rep, Period>>
{
    Reflected operator()(const std::chrono::duration<Rep, Period>& data, const ReflectionContext& context) const
    {
        return context.reflect(data.count());
    }
};
