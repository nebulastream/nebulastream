...
{
    Reflected operator()(const NESStrongType<T, Tag, invalid, initial>& data) const
    {
        return reflect(data.getRawValue());
    }
};
...
{
    Reflected operator()(const NESStrongStringType<Tag, Invalid>& data) const
    {
        return reflect(data.getRawValue());
    }
};
...
{
    Reflected operator()(const NESStrongUUIDType<Tag>& data) const
    {
        return reflect(data.getRawValue());
    }
};
