...
/**
 * @brief a representation of geographical location used to specify the fixed
 * location of field nodes and the changing location of mobile devices
 */
class GeoLocation {
 public:
  /**
   * @brief the default constructor which constructs an object with lat=200 and
   * lng=200 which represents an invalid location
   */
  GeoLocation();

  /**
   * @brief constructs a Geographical location from latitude and longitude in
   * degrees
   * @throws CoordinatesOutOfRangeException if the entered parameters do not
   * correspond to a valid lat/long pair
  ...
  /**
   * @brief constructs a Geographical location object from a Coordinates object
   * used as members of protobuf messages
   * @throws CoordinatesOutOfRangeException if the entered parameters do not
   * correspond to a valid lat/long pair
  ...
  /**
   * @brief compares two GeographicalLocations and checks if they represent the
   * same point on the map
   * @param other: the object to compare to
   * @return true both objects have the same latitude and longitude. false
   * otherwise
   */
   ...
   * @throws InvalidCoordinateFormatException if the input string is not of the
