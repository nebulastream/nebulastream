...
struct AtomicState
{
    ...
    bool transition(const TransitionFunctions&... transitionFunctions)
    {
        ...
        return (false || ... || try_transition_locked(std::function(transitionFunctions)));
    }
    ...
    {
        std::scoped_lock lock(mutex);
        return try_transition_locked(transitionFunction);
    }

private:
    template <typename FromState, typename ToState>
    bool try_transition_locked(const std::function<ToState(FromState&&)>& transitionFunction)
    {
