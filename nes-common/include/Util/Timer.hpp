...
public:
  ...
  public:
    ...
    PrintTimePrecision getPrintTime() {
      return std::chrono::duration_cast<
                 std::chrono::duration<PrintTimePrecision, PrintTimeUnit>>(
                 runtime)
          .count();
    }
  ...
  void start() {
    if (running) {
      NES_DEBUG("Timer: Trying to start an already running timer so will skip "
                "this operation");
    } else {
      running = true;
  ...
  void pause() {
    if (!running) {
      NES_DEBUG("Timer: Trying to stop an already stopped timer so will skip "
                "this operation");
    } else {
      running = false;
   ...
   */
  std::string
  createFullyQualifiedSnapShotName(const std::string &snapshotName) {
  ...
  void snapshot(std::string snapshotName) {
    if (!running) {
      NES_DEBUG("Timer: Trying to take a snapshot of an non-running timer so "
      ...
      snapshots.emplace_back(
          Snapshot(createFullyQualifiedSnapShotName(snapshotName), duration,
                   std::vector<Snapshot>()));
  ...
  void merge(Timer timer) {
    if (running) {
      NES_DEBUG("Timer: Trying to merge while timer is running so will skip "
                "this operation");
    } else {
      this->runtime += timer.runtime;
      snapshots.emplace_back(
          Snapshot(componentName + '_' + timer.getComponentName(),
                   timer.runtime, timer.getSnapshots()));
    }
  };
  ...
  int64_t getRuntime() const {
    if (!running) {
      ...
      NES_DEBUG("Timer: Trying get runtime while timer is running so will "
                "return zero");
      return 0;
    }
  };
  ...
  friend std::ostream &operator<<(std::ostream &str, const Timer &t) {
    ...
    for (auto &s : t.getSnapshots()) {
      str << Timer<TimeUnit, PrintTimeUnit, PrintTimePrecision>::printHelper(
          std::string(), s);
    }
    return str;
  };
  ...
  /**
   * @brief return timer runtime converted ConvertUnit with ConvertPrecision
   * e.g. for printing purposes.
   */
  template <typename ConvertUnit = PrintTimeUnit,
            typename ConvertPrecision = PrintTimePrecision>
  ConvertPrecision getPrintTime() const {
    auto printDuration = std::chrono::duration_cast<
        std::chrono::duration<ConvertPrecision, ConvertUnit>>(runtime);
  ...
  /**
   * @brief helper function to return a time unit literal string based on
   * PrintTimeUnit
   */
  template <typename ConvertUnit = PrintTimeUnit>
