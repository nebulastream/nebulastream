...
class Timer {
 public:
  ...
  void start() {
    if (running) {
      NES_DEBUG(
          "Timer: Trying to start an already running timer so will skip this "
          "operation");
    } else {
      running = true;
  ...
  void pause() {
    if (!running) {
      NES_DEBUG(
          "Timer: Trying to stop an already stopped timer so will skip this "
          "operation");
    } else {
      running = false;
   ...
   */
  std::string createFullyQualifiedSnapShotName(
  ...
  void snapshot(std::string snapshotName) {
    if (!running) {
      NES_DEBUG(
          "Timer: Trying to take a snapshot of an non-running timer so will "
          "skip this operation");
    } else {
      running = true;
      stop_p = ClockType::now();
  ...
  void merge(Timer timer) {
    if (running) {
      NES_DEBUG(
          "Timer: Trying to merge while timer is running so will skip this "
          "operation");
    } else {
  ...
  int64_t getRuntime() const {
    if (!running) {
      ...
      NES_DEBUG(
          "Timer: Trying get runtime while timer is running so will return "
          "zero");
      return 0;
    }
  };
