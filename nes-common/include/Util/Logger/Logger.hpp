...
// This macro indicates the log level, which was chosen at compilation time and
// enables the complete elimination of log messages.
...
/**
 * @brief LogCaller is our compile-time trampoline to invoke the Logger method
 * for the desired level of logging L
 * @tparam L the level of logging
 */
template <LogLevel L> struct LogCaller {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&,
...
template <> struct LogCaller<LogLevel::LOG_INFO> {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&loc,
                                fmt::format_string<arguments...> format,
                                arguments &&...args) {
    if (auto instance = NES::Logger::getInstance()) {
      instance->info(std::move(loc), std::move(format),
                     std::forward<arguments>(args)...);
    }
  }
};

template <> struct LogCaller<LogLevel::LOG_TRACE> {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&loc,
                                fmt::format_string<arguments...> format,
                                arguments &&...args) {
    if (auto instance = NES::Logger::getInstance()) {
      instance->trace(std::move(loc), std::move(format),
                      std::forward<arguments>(args)...);
    }
  }
};

template <> struct LogCaller<LogLevel::LOG_DEBUG> {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&loc,
                                fmt::format_string<arguments...> format,
                                arguments &&...args) {
    if (auto instance = NES::Logger::getInstance()) {
      instance->debug(std::move(loc), std::move(format),
                      std::forward<arguments>(args)...);
    }
  }
};

template <> struct LogCaller<LogLevel::LOG_ERROR> {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&loc,
                                fmt::format_string<arguments...> format,
                                arguments &&...args) {
    if (auto instance = NES::Logger::getInstance()) {
      instance->error(std::move(loc), std::move(format),
                      std::forward<arguments>(args)...);
    }
  }
};

template <> struct LogCaller<LogLevel::LOG_FATAL_ERROR> {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&loc,
                                fmt::format_string<arguments...> format,
                                arguments &&...args) {
    if (auto instance = NES::Logger::getInstance()) {
      instance->fatal(std::move(loc), std::move(format),
                      std::forward<arguments>(args)...);
    }
  }
};

template <> struct LogCaller<LogLevel::LOG_WARNING> {
  template <typename... arguments>
  constexpr static void do_call(spdlog::source_loc &&loc,
                                fmt::format_string<arguments...> format,
                                arguments &&...args) {
    if (auto instance = NES::Logger::getInstance()) {
      instance->warn(std::move(loc), std::move(format),
                     std::forward<arguments>(args)...);
    }
  }
};

/// @brief this is the new logging macro that is the entry point for logging
/// calls
#define NES_LOG(LEVEL, ...)                                                    \
  do {                                                                         \
    auto constexpr __level = NES::getLogLevel(LEVEL);                          \
    if constexpr (NES_COMPILE_TIME_LOG_LEVEL >= __level) {                     \
      NES::LogCaller<LEVEL>::do_call(                                          \
...
#define NES_FATAL_ERROR(...)                                                   \
  NES_LOG(NES::LogLevel::LOG_FATAL_ERROR, __VA_ARGS__);

/// I am aware that we do not like __ before variable names but here we need
/// them to avoid name collions, e.g., __buffer, __stacktrace that should not be
/// a problem because of the scope, however, better be safe than sorry :P

/// Additionally, we do not want to print stack traces when the currentLogLevel,
/// as passed over at runtime in tests for example, is lower than DEBUG. Also,
/// NES_DEBUG_MODE should be enabled as well. For that we define
/// NES_DEBUG_PRINT_TRACE. When a stack trace should not be printed, an empty
/// string will be given over instead, which is then handled appropriately in
/// invokeErrorHandlers/RunTimeException.
...
#define NES_THROW_RUNTIME_ERROR(...)                                           \
  do {                                                                         \
    ...
    if (__currentlevel >= __level && NES_COMPILE_TIME_LOG_LEVEL >= __level) {  \
      auto __stacktrace = NES::collectStacktrace();                            \
      throw NES::Exceptions::RuntimeException(                                 \
          __buffer.str(), std::move(__stacktrace), std::move(__location));     \
    } else {                                                                   \
      throw NES::Exceptions::RuntimeException(__buffer.str(), "",              \
                                              std::move(__location));          \
    }                                                                          \
  } while (0)
