...
public:
   ...
   *    and new nodes's children. If there's duplicated children among old nodes
   * and new nodes's children, the children in new noeds will overwrite that
   *    inside old noeds's.
   * 2)
   * @param newNode
   * @param oldNode
  ...
  /**
   * @brief Swaps the left and right branch of binary nodes, by reversing the
   * vector of children.
  ...
  /**
   * @brief remove the given node and add its children to the children of the
   * current node
   * @param node
   * @return bool true if successful
   */
  ...
  /**
   * @brief Remove this node as child to its parents and as parent to its
   * children. Once done, the method joins the parent and children together.
   * @return bool true if successful
   */
  ...
  /**
   * @brief checks if the current node and its children are equal a other node
   * and its children
  ...
  /**
   * @brief checks if the current node and its parents are equal a other node
   * and its parents
   * @param node the node to compare
   * @return bool
   */
   ...
   */
  [[nodiscard]] virtual bool isIdentical(NodePtr const &rhs) const {
    return rhs.get() == this;
  };

  /**
   * @brief split graph into multiple sub-graphs. The graph starts at current
   * splitNode. If the given splitNode is not in the graph, throw exception
   ...
   * @return true if there is no ring/node inside this node's children,
   * otherwise false
   */
  bool isValid();
   ...
   */
  template <class NodeType> bool instanceOf() {
   ...
   */
  template <class NodeType> std::shared_ptr<NodeType> as() {
    if (instanceOf<NodeType>()) {
      return std::dynamic_pointer_cast<NodeType>(this->shared_from_this());
    }
    throw std::logic_error("Node:: we performed an invalid cast of operator " +
   ...
   * @tparam NodeType
   * @return returns a shared pointer of the NodeType or nullptr if the type
   * can't be casted.
   */
  template <class NodeType> std::shared_ptr<NodeType> as_if() {
    return std::dynamic_pointer_cast<NodeType>(this->shared_from_this());
  }

  /**
   * @brief Collects all nodes that are of a specific node type, e.g. all
   * FilterOperatorNodes.
   * @tparam NodeType
   ...
   * @param withDuplicateChildren: set true to allow to retrieve duplicate
   * elements
  ...
  /**
   * @brief Add input node as parent to the current node and move the parents of
   * current node as parent to the input node. If the node is already exists as
   * parent then skip the operation
   * @return true if operation succeeded else false
   */
  bool insertBetweenThisAndParentNodes(NodePtr const &newNode);

  /**
   * @brief Add input node as child to the current node and add the input node
   * as new parent to the old child
   * @return true if operation succeeded else false
   */
   ...
   * @return return true if the given nodeToFind is found in the graph of root,
   * otherwise false
   */
  static NodePtr findRecursively(NodePtr const &root,
...
private:
  ...
  template <class NodeType>
  void
  getNodesByTypeHelper(std::vector<std::shared_ptr<NodeType>> &foundNodes) {
   ...
   */
  bool vectorContainsTheNode(const std::vector<NodePtr> &nodes,
                             NodePtr const &nodeToFind);
   ...
   */
  static NodePtr find(std::vector<NodePtr> const &nodes,
                      NodePtr const &nodeToFind);
