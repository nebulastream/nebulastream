...
protected:
    ...
    /// `VerbosityLevel::QueryPlan` (`ss << LogVerbosity::VerbosityLevel::QueryPlan << node`).
...
{
    switch (LogVerbosity::getVerbosityLevel(os))
    {
        case LogVerbosity::VerbosityLevel::Debug:
        default:
            return node.toDebugString(os);
            break;
        case LogVerbosity::VerbosityLevel::QueryPlan:
...
struct formatter<T> : ostream_formatter
{
    ...
    constexpr auto parse(fmt::format_parse_context& ctx)
    {
        ...
        {
            if (*pos == queryPlanOptionCharacter)
            {
                isQueryPlan = true;
            }
            ++pos;
    ...
    auto format(const T& node, format_context& ctx) const
    {
        if (isQueryPlan)
        {
            ...
            std::stringstream ss;
            ss << NES::LogVerbosity::VerbosityLevel::QueryPlan;
            ss << node;
