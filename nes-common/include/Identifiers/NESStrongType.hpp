...
namespace NES {
/**
 * Identifiers in NebulaStream are based on a Strong Type. This prevents
 * accidental conversion between different Entity Identifiers. In general a
 * Identifier should not expose its underlying Type and no code should depend on
 * specific values. This is the reason why only a limited subset of operations
 * are supported, and it is not default constructible. Identifiers are orderable
 * and hashable to be used as keys in maps. We Introduce overloads for nlohmann,
 * yaml and fmt to make the identifiers feel ergonomic.
 ...
 */
template <typename T, typename Tag, T invalid, T initial> class NESStrongType {
public:
  ...
  operator<=>(const NESStrongType &lh, const NESStrongType &rh) noexcept {
    return lh.v <=> rh.v;
  }
  ...
  friend std::ostream &operator<<(std::ostream &os, const NESStrongType &t) {
    return os << t.getRawValue();
  }

  [[nodiscard]] std::string toString() const { return std::to_string(v); }

  /**
   * Serializes the Identifier. Useful for protobuf. Yaml and Json should not
   * require this method.
...
concept NESIdentifier = requires(T t) {
  requires(
      std::same_as<T, NESStrongType<typename T::Underlying, typename T::TypeTag,
                                    T::INVALID, T::INITIAL>>);
...
struct hash<NES::NESStrongType<T, Tag, invalid, initial>> {
  size_t
  operator()(NES::NESStrongType<T, Tag, invalid, initial> const &x) const {
    return std::hash<T>()(x.getRawValue());
  }
};
} // namespace std
