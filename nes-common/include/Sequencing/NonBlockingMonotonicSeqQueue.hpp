 ...
 * returns the value with the highest sequence number in strictly monotonic
 * increasing order.
 *
 * Internally this queue is implemented as a linked-list of blocks and each
 * block stores a array of <seq,T> pairs.
 * |------- |       |------- |
 * | s1, s2 | ----> | s3, s4 |
 * |------- |       | ------ |
 *
...
private:
  /**
   * @brief This class contains the state for one sequence number. It checks if
   * the SeqQueue has seen all chunks for the sequence number.
   */
  class Container {
  public:
    Container()
        : seqNumber(INVALID_SEQ_NUMBER), lastChunkNumber(INVALID_CHUNK_NUMBER),
          seenChunks(0), mergeValues(Util::updateAtomicMax<T>) {}

    /**
     * @brief This methods emplaces <chunkNumber, lastChunk, value> into the
     * container.
     * @param chunkNumber
     * @param lastChunk
     * @param value
    ...
    void setSeqNumber(const SequenceNumber &seqNumber) {
    ...
    bool seenAllChunks() {
      return (lastChunkNumber != INVALID_CHUNK_NUMBER) &&
...
public:
  NonBlockingMonotonicSeqQueue()
      : head(std::make_shared<Block>(0)), currentSeq(0) {}

  ~NonBlockingMonotonicSeqQueue() = default;

  NonBlockingMonotonicSeqQueue &
  operator=(const NonBlockingMonotonicSeqQueue &other) {
    head = other.head;
   ...
   * @throws RuntimeException if an element with the same sequence number was
   * already inserted.
   */
  void emplace(SequenceData sequenceData, T value) {
    if (sequenceData.sequenceNumber < currentSeq) {
      NES_FATAL_ERROR("Invalid sequence number {} as it is < {}",
                      sequenceData.sequenceNumber, currentSeq);
      // TODO add exception, currently tests fail
      // throw Exceptions::RuntimeException("Invalid sequence number " +
      // std::to_string(sequenceNumber)
      //                                   + " as it is <= " +
      //                                   std::to_string(currentSeq));
    }
    // First emplace the value to the specific block of the sequenceNumber.
    // After this call it is safe to assume that a block, which contains the
   ...
   * This method is thread save, however it is not guaranteed that current value
   * dose not change concurrently.
   * @return T value
   */
...
private:
  /**
   * @brief Emplace a value T to the specific location of the passed sequence
   * number
   *
   * The method is split in two phased:
   * 1. Find the correct block for this sequence number. If the block not yet
   * exists we add a new block to the linked-list.
  ...
  void emplaceValueInBlock(SequenceData seq, T value) {
    ...
    auto currentBlock = std::atomic_load(&head);
    // if the blockIndex is smaller the target block index we travers the next
    // block
    while (currentBlock->blockIndex < targetBlockIndex) {
      ...
      if (nextBlock == nullptr) {
        ...
        std::atomic_compare_exchange_weak(&currentBlock->next, &nextBlock,
                                          newBlock);
        // we don't care if this or another thread succeeds, as we just start
        // over again in the loop and use what ever is now stored in
        // currentBlock.next
      } else {
    ...
    currentBlock->log[seqIndexInBlock].emplaceChunk(seq.chunkNumber,
                                                    seq.lastChunk, value);
  }

  /**
   * @brief This method shifts tries to shift the current value.
   * To this end, it checks if the next expected sequence number (currentSeq +
   * 1) is already inserted. If the next sequence number is available it
   * replaces the currentSeq with the next one. If the next sequence number is
   * in a new block this method also replaces the pointer to the next block.
   */
  void shiftCurrentValue() {
    ...
    while (checkForUpdate) {
      ...
      if (nextSeqNumber % blockSize == 0) {
        ...
        if (nextBlock != nullptr) {
          ...
          if (value.getSeqNumber() == nextSeqNumber && value.seenAllChunks()) {
            ...
            if (std::atomic_compare_exchange_weak(
                    &currentSeq, &currentSequenceNumber, nextSeqNumber)) {
              std::atomic_compare_exchange_weak(&head, &currentBlock,
                                                nextBlock);
            }
            continue;
          }
        ...
        auto &value = currentBlock->log[seqIndexInBlock];
        if (value.getSeqNumber() == nextSeqNumber && value.seenAllChunks()) {
          // the next sequence number is still in the current block thus we only
          // have to exchange the currentSeq.
          std::atomic_compare_exchange_weak(&currentSeq, &currentSequenceNumber,
                                            nextSeqNumber);
          continue;
        }
      }
  ...
  /**
   * @brief This function traverses the linked list of blocks, till the target
   * block index is found. It assumes, that the target block index exists. If
   * not, the function throws a runtime exception.
   ...
   */
  std::shared_ptr<Block> getTargetBlock(std::shared_ptr<Block> currentBlock,
                                        uint64_t targetBlockIndex) const {
    while (currentBlock->blockIndex < targetBlockIndex) {
      // append new block if the next block is a nullptr
      auto nextBlock = std::atomic_load(&currentBlock->next);
      if (!nextBlock) {
        throw Exceptions::RuntimeException(
            "The next block dose not exist. This should not happen here.");
      }
      // move to the next block
      currentBlock = nextBlock;
    }
