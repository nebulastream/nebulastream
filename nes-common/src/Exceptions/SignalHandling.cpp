...
void invokeErrorHandlers(std::shared_ptr<std::exception> exception,
                         std::string&& stacktrace) {
  std::unique_lock lock(globalErrorListenerMutex);
  if (globalErrorListeners.empty()) {
    if (stacktrace.empty()) {
      std::cerr << "No error listener is set, you need to revise your bin "
                   "logic\n got error=["
                << exception->what() << "]\n"
                << std::endl;
    } else {
      std::cerr << "No error listener is set, you need to revise your bin "
                   "logic\n got error=["
                << exception->what() << "] with stacktrace=\n"
                << stacktrace << std::endl;
    }
  }
...
void invokeErrorHandlers(int signal, std::string&& stacktrace) {
  ...
  if (globalErrorListeners.empty()) {
    if (stacktrace.empty()) {
      ...
      std::cerr << "No error listener is set, you need to revise your bin "
                   "logic\n got error=["
                << strerror(errno) << "] with stacktrace=\n"
                << stacktrace << std::endl;
    }
  }
  for (auto& listener : globalErrorListeners) {
...
void invokeErrorHandlers(const std::string& buffer, std::string&& stacktrace) {
  if (stacktrace.empty()) {
    ...
    NES_TRACE("invokeErrorHandlers with buffer={} trace={}", buffer,
              stacktrace);
  }
...
void installGlobalErrorListener(
    std::shared_ptr<ErrorListener> const& listener) {
...
void removeGlobalErrorListener(const std::shared_ptr<ErrorListener>& listener) {
  ...
  std::unique_lock lock(globalErrorListenerMutex);
  for (auto it = globalErrorListeners.begin(); it != globalErrorListeners.end();
       ++it) {
