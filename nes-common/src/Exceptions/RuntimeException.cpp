...
namespace fmt {
template <> struct formatter<std::source_location> : formatter<std::string> {
  auto format(const std::source_location &loc, format_context &ctx)
      -> decltype(ctx.out()) {
...
RuntimeException::RuntimeException(std::string msg, std::string &&stacktrace,
