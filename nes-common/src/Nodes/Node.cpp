...
bool Node::addChildWithEqual(const NodePtr& newNode) {
  if (newNode.get() == this) {
    NES_DEBUG(
        "Node: Adding node to its self so skip add child with equal "
        "operation.");
    return false;
  }
...
bool Node::addChild(const NodePtr newNode) {
  ...
  if (vectorContainsTheNode(children, newNode)) {
    NES_ERROR(
        "Node: the node is already part of its children so skip add child "
        "operation.");
    return false;
  }
  // add the node to the children
...
bool Node::addParent(const NodePtr newNode) {
  ...
  if (vectorContainsTheNode(parents, newNode)) {
    NES_WARNING(
        "Node: the node is already part of its parents so ignore add parent "
        "operation.");
    return false;
  }
...
bool Node::insertBetweenThisAndParentNodes(NodePtr const& newNode) {
  ...
  if (newNode.get() == this) {
    NES_WARNING(
        "Node:  Adding node to its self so will skip "
  ...
  if (vectorContainsTheNode(parents, newNode)) {
    NES_WARNING(
        "Node: the node is already part of its parents so ignore "
        "insertBetweenThisAndParentNodes operation.");
    return false;
  }
    ...
    for (uint64_t i = 0; i < parent->children.size(); i++) {
      if (parent->children[i] == shared_from_this()) {
        ...
        if (!newNode->addParent(parent)) {
          NES_ERROR(
              "Node: Unable to add parent of this node as parent to input "
              "node.");
          return false;
        }
      }
...
bool Node::replace(const NodePtr& newNode, const NodePtr& oldNode) {
  ...
  if (!oldNode->equal(newNode)) {
    ...
    if (find(children, newNode) || find(parents, newNode)) {
      NES_DEBUG(
          "Node: the new node is already part of the children or predecessors "
          "of the current node.");
      return false;
    }
  }
...
bool Node::swapLeftAndRightBranch() {
  if (children.size() != 2) {
    NES_WARNING(
        "Operation failed: method is supposed to swap left and right branch, "
        "but the node had ",
