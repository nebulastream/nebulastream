...
Node::Node() {
// The CMAKE_NES_TRACE_NODE_CREATION flag enables the stack trace collection for
// every node creation. This can be useful for the debugging of optimizations,
// but could lead to a substantial performance impact.
...
bool Node::addChildWithEqual(const NodePtr &newNode) {
  if (newNode.get() == this) {
    NES_DEBUG("Node: Adding node to its self so skip add child with equal "
              "operation.");
    return false;
  }
...
bool Node::addChild(const NodePtr newNode) {
  if (newNode.get() == this) {
    NES_ERROR(
        "Node: Adding node to its self so will skip add child operation.");
    return false;
  }
  // checks if current new node is not part of children
  if (vectorContainsTheNode(children, newNode)) {
    NES_ERROR("Node: the node is already part of its children so skip add "
              "child operation.");
    return false;
  }
  // add the node to the children
...
bool Node::removeChild(NodePtr const &node) {
  ...
  for (auto nodeItr = children.begin(); nodeItr != children.end(); ++nodeItr) {
    if ((*nodeItr).get() == node.get()) {
      ...
      for (auto it = (*nodeItr)->parents.begin();
           it != (*nodeItr)->parents.end(); it++) {
        if ((*it).get() == this) {
...
bool Node::addParent(const NodePtr newNode) {
  if (newNode.get() == this) {
    NES_WARNING(
        "Node: Adding node to its self so will skip add parent operation.");
    return false;
  }

  // checks if current new node is not part of parents
  if (vectorContainsTheNode(parents, newNode)) {
    NES_WARNING("Node: the node is already part of its parents so ignore add "
                "parent operation.");
    return false;
  }
...
bool Node::insertBetweenThisAndParentNodes(NodePtr const &newNode) {
  ...
  if (newNode.get() == this) {
    NES_WARNING("Node:  Adding node to its self so will skip "
                "insertBetweenThisAndParentNodes operation.");
    return false;
  }

  if (vectorContainsTheNode(parents, newNode)) {
    NES_WARNING("Node: the node is already part of its parents so ignore "
                "insertBetweenThisAndParentNodes operation.");
    return false;
  }
  ...
  for (auto &parent : copyOfParents) {
    for (uint64_t i = 0; i < parent->children.size(); i++) {
      if (parent->children[i] == shared_from_this()) {
        ...
        NES_DEBUG(
            "Node: Add copy of this nodes parent as parent to the input node.");
        if (!newNode->addParent(parent)) {
          NES_ERROR("Node: Unable to add parent of this node as parent to "
                    "input node.");
          return false;
        }
      }
...
bool Node::insertBetweenThisAndChildNodes(const NodePtr &newNode) {
  ...
  for (const NodePtr &child : copyOfChildren) {
    if (!newNode->addChild(child)) {
      NES_ERROR(
          "Node: Unable to add child of this node as child to input node.");
      return false;
    }
  }
...
bool Node::removeParent(NodePtr const &node) {
  ...
  for (auto nodeItr = parents.begin(); nodeItr != parents.end(); ++nodeItr) {
    if ((*nodeItr).get() == node.get()) {
      for (auto it = (*nodeItr)->children.begin();
           it != (*nodeItr)->children.end(); it++) {
        if ((*it).get() == this) {
...
bool Node::replace(const NodePtr &newNode, const NodePtr &oldNode) {
  ...
  if (oldNode->isIdentical(newNode)) {
    NES_WARNING(
        "Node: the new node was the same so will skip replace operation.");
    return true;
  }
  ...
  if (!oldNode->equal(newNode)) {
    ...
    if (find(children, newNode) || find(parents, newNode)) {
      NES_DEBUG("Node: the new node is already part of the children or "
  ...
  NES_ERROR("Node: could not remove child from  old node: {}",
            oldNode->toString());
  ...
  NES_ERROR("Node: could not remove parent from  old node: {}",
            oldNode->toString());
...
bool Node::swapLeftAndRightBranch() {
  if (children.size() != 2) {
    NES_WARNING("Operation failed: method is supposed to swap left and right "
                "branch, but the node had ",
                children.size(), " children, instead of 2");
    return false;
  }
...
bool Node::remove(const NodePtr &node) {
  // NOTE: if there is a cycle inside the operator topology, it won't behave
  // correctly.
  ...
  try {
    ...
    for (auto &parent : parentCopy) {
      for (auto &child : childCopy) {
        ...
        NES_DEBUG(
            "Node: Add child of this node as child of this node's parent");
    ...
    NES_ERROR(
        "Node: Error occurred while joining this node's children and parents");
    return false;
  }
}
...
bool Node::containAsChild(NodePtr node) {
  NES_DEBUG(
      "Node: Checking if the input node is contained in the children list");
...
bool Node::containAsGrandChild(NodePtr node) {
  ...
  for (auto &child : children) {
    ...
    std::vector<NodePtr> childAndGrandChildren =
        child->getAndFlattenAllChildren(true);
    childAndGrandChildren.emplace_back(child);
    NES_TRACE("Node: Add them to the result");
    grandChildren.insert(grandChildren.end(), childAndGrandChildren.begin(),
  ...
  for (auto &parent : parents) {
    if (parent->getParents().empty()) {
      ...
      for (const auto &parentOfParent : parent->getParents()) {
        ...
        NES_DEBUG(
            "Node: inserting parent nodes into the collection of parent nodes");
        rootNodes.insert(rootNodes.end(), parentNodes.begin(),
                         parentNodes.end());
      }
    }
  }
  ...
  for (auto &child : children) {
    if (child->getChildren().empty()) {
      ...
      for (const auto &childOfChild : child->getChildren()) {
        std::vector<NodePtr> childrenLeafNodes =
            childOfChild->getAllLeafNodes();
        NES_DEBUG(
            "Node: inserting leaf nodes into the collection of leaf nodes");
        leafNodes.insert(leafNodes.end(), childrenLeafNodes.begin(),
                         childrenLeafNodes.end());
      }
    }
  }
...
bool Node::vectorContainsTheNode(std::vector<NodePtr> const &nodes,
                                 NES::NodePtr const &nodeToFind) {
  return find(nodes, nodeToFind) != nullptr;
}

/**
 * Note that, operators with same description are not necessarily the same
 * object.
 * @param nodes
 * @param nodeToFind
 * @return
 */
NodePtr Node::find(const std::vector<NodePtr> &nodes,
                   const NodePtr &nodeToFind) {
  for (auto &&currentNode : nodes) {
    if (nodeToFind->equal(
            currentNode)) { // TODO: need to check this when merge is used.
                            // nodeToFind.get() == currentNode.get()
      return currentNode;
    }
  }
  return nullptr;
...
bool Node::equalWithAllChildrenHelper(const NodePtr &node1,
                                      const NodePtr &node2) {
...
bool Node::equalWithAllParentsHelper(const NodePtr &node1,
                                     const NodePtr &node2) {
...
std::vector<NodePtr>
Node::getAndFlattenAllChildren(bool withDuplicateChildren) {
  std::vector<NodePtr> allChildren{};
  getAndFlattenAllChildrenHelper(shared_from_this(), allChildren,
                                 shared_from_this(), withDuplicateChildren);
  return allChildren;
}
  ...
  for (auto &&currentNode : node->children) {
    if (allowDuplicate) {
      ...
      getAndFlattenAllChildrenHelper(currentNode, allChildren, excludedNode,
                                     allowDuplicate);
    } else if (!find(allChildren, currentNode) &&
...
std::vector<NodePtr> Node::getAndFlattenAllAncestors() {
  ...
  for (auto &parent : parents) {
    ...
    std::vector<NodePtr> parentAndAncestors =
        parent->getAndFlattenAllAncestors();
    NES_TRACE("Node: Add them to the result");
    result.insert(result.end(), parentAndAncestors.begin(),
                  parentAndAncestors.end());
  }
  return result;
}
