...
jclass findClass(const std::string_view& clazzName) {
    jclass clazz = getEnv()->FindClass(clazzName.data());
    jniErrorCheck();
    return clazz;
}
template<typename T>
...
jstring createString(const std::string_view& value) {
    ...
    return static_cast<jstring>(string);
}
...
void loadClassesFromByteList(const std::unordered_map<std::string, std::vector<char>>& byteCodeList) {
    for (auto& [className, byteCode] : byteCodeList) {
        jbyteArray jData = jni::getEnv()->NewByteArray(byteCode.size());
        jni::jniErrorCheck();
        jbyte* jCode = jni::getEnv()->GetByteArrayElements(jData, nullptr);
        jni::jniErrorCheck();
        std::memcpy(jCode, byteCode.data(), byteCode.size());// copy the byte array into the JVM byte array
        jni::getEnv()->DefineClass(className.c_str(), nullptr, jCode, (jint) byteCode.size());
        jni::jniErrorCheck();
        jni::getEnv()->ReleaseByteArrayElements(jData, jCode, JNI_ABORT);
        jni::jniErrorCheck();
    }
}
