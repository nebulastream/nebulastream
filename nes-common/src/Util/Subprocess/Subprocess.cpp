...
enum class ends_of_pipe : uint8_t
{
    READ = 0,
    WRITE = 1
};
...
void Subprocess::executeCommandInChildProcess(const std::vector<std::string>& argv)
{
    ...
    std::transform(std::begin(argv), std::end(argv), std::back_inserter(cargs), [&](const std::string& str) { return (char*)str.c_str(); });
