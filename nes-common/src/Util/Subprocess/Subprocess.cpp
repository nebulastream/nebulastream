...
Subprocess::Subprocess(std::string cmd, std::vector<std::string> argv) {
  ...
  this->outputFile =
      fdopen(outPipe[magic_enum::enum_integer(ends_of_pipe::READ)], "r");
...
void Subprocess::executeCommandInChildProcess(
    const std::vector<std::string>& argv) {
  if (dup2(outPipe[magic_enum::enum_integer(ends_of_pipe::WRITE)],
           STDOUT_FILENO) == -1) {
  ...
  std::transform(std::begin(argv), std::end(argv), std::back_inserter(cargs),
