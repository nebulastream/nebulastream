...
DumpHelper::create(const std::string& contextIdentifier, bool dumpToConsole, bool dumpToFile, const std::string& outputPath) {
    if (!dumpToFile && !dumpToConsole) {
        return DumpHelper("", dumpToConsole, dumpToFile, "");
    }
...
void DumpHelper::dump(const std::string_view& name, std::function<const std::string()> function) const {
    if (this->dumpToConsole || this->dumpToFile) {
        dump(name, function());
    }
}
