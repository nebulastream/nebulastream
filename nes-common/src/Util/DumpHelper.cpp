...
void DumpHelper::dump(const std::string_view &name,
  ...
  if (this->dumpToFile) {
    ...
    auto path = std::string{outputPath} +
...
DumpHelper::DumpHelper(std::string contextIdentifier, bool dumpToConsole,
                       bool dumpToFile, std::string outputPath)
    : contextIdentifier(std::move(contextIdentifier)),
      dumpToConsole(dumpToConsole), dumpToFile(dumpToFile),
