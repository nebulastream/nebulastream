...
GeoLocation::GeoLocation(double latitude, double longitude) {
  // Coordinates with the value NaN lead to the creation of an object which
  // symbolizes an invalid location
  if (!(std::isnan(latitude) && std::isnan(longitude)) &&
      !checkValidityOfCoordinates(latitude, longitude)) {
...
GeoLocation GeoLocation::fromString(const std::string &coordinates) {
  ...
  if (separator != ',') {
    NES_ERROR("input string is not of format \"<latitude>, <longitude>\". Node "
              "will be created as non field node");
    throw NES::Spatial::Exception::CoordinatesOutOfRangeException();
  }
  double lng;
  stringStream >> lng;
...
bool GeoLocation::checkValidityOfCoordinates(double latitude,
                                             double longitude) {
