...
static constexpr auto SPDLOG_PATTERN =
    "%^[%H:%M:%S.%f] [%L] [thread %t] [%s:%#] [%!] %v%$";
...
auto createEmptyLogger() -> std::shared_ptr<spdlog::logger> {
  return std::make_shared<spdlog::logger>(
      "null", std::make_shared<spdlog::sinks::basic_file_sink_st>(DEV_NULL));
}
...
Logger::Logger(const std::string& logFileName, LogLevel level) {
  ...
  loggerThreadPool =
      std::make_shared<spdlog::details::thread_pool>(QUEUE_SIZE, THREADS);
  ...
  impl = std::make_shared<spdlog::async_logger>(
      SPDLOG_NES_LOGGER_NAME, sinks.begin(), sinks.end(), loggerThreadPool,
  ...
  flusher = std::make_unique<spdlog::details::periodic_worker>(
      [this]() { impl->flush(); }, std::chrono::seconds(1));
}
