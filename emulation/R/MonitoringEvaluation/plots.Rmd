...
dfResult = dfResult[[1]]
dfResult$memoryUsage = dfResult$memoryUsage/(1024^2)
dfResult
```
...
```{r}
ggplot(dfResult, aes(x=iteration, y=memoryUsage, color=topology, shape=type, size=system)) + geom_point() +  scale_shape_manual(values=c(1, 3)) + labs(y="memory usage (mb)")
```
```{r}
