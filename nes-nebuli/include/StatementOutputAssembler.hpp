...
using SourceDescriptorOutputRowType = std::tuple<uint64_t, std::string, Schema, std::string, ParserConfig>;
constexpr std::array<std::string_view, 5> sourceDescriptorOutputColumns{
    "physical_source_id", "source_name", "schema", "source_type", "parser_config"};

using SinkDescriptorOutputRowType = std::tuple<std::string, Schema, std::string>;
constexpr std::array<std::string_view, 3> sinkDescriptorOutputColumns{"sink_name", "schema", "sink_type"};
...
struct StatementOutputAssembler<CreateSinkStatementResult>
{
    ...
    auto convert(const CreateSinkStatementResult& result)
    {
        return std::make_pair(
            ...
            std::vector{std::make_tuple(result.created.getSinkName(), *result.created.getSchema(), result.created.getSinkType())});
    }
};
...
struct StatementOutputAssembler<ShowSinksStatementResult>
{
    ...
    auto convert(const ShowSinksStatementResult& result)
    {
        ...
        {
            output.emplace_back(sink.getSinkName(), *sink.getSchema(), sink.getSinkType());
        }
...
struct StatementOutputAssembler<DropSinkStatementResult>
{
    ...
    auto convert(const DropSinkStatementResult& result)
    {
        return std::make_pair(
            sinkDescriptorOutputColumns,
            std::vector{std::make_tuple(result.dropped.getSinkName(), *result.dropped.getSchema(), result.dropped.getSinkType())});
    }
};
