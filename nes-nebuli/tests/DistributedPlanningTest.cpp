...
std::vector<NES::Statement> loadStatements(const NES::Test::QueryConfig& topologyConfig)
{
    ...
    for (const auto& [name, schemaFields] : logical)
    {
        ...
        {
            schema.addField(NES::Util::toUpperCase(schemaField), NES::DataType::Type::UINT64);
        }

        statements.emplace_back(NES::CreateLogicalSourceStatement{.name = NES::Util::toUpperCase(name), .schema = schema});
    }

    for (const auto& [logical, host] : physical)
    {
        statements.emplace_back(NES::CreatePhysicalSourceStatement{
            .attachedTo = NES::LogicalSourceName(NES::Util::toUpperCase(logical)),
            .sourceType = "File",
            .workerId = host,
    ...
    for (const auto& [name, schemaFields, host] : sinks)
    {
        ...
        statements.emplace_back(NES::CreateSinkStatement{
            .name = NES::Util::toUpperCase(name), .sinkType = "Void", .workerId = host, .schema = schema, .sinkConfig = {}});
    }
    statements.emplace_back(NES::ExplainQueryStatement{.plan = NES::AntlrSQLQueryParser::createLogicalQueryPlanFromSQLString(query)});
    return statements;
}

std::pair<NES::QueryPlanningContext, NES::PlanStage::BoundLogicalPlan> loadAndBind(std::string_view testFileName)
{
    ...
    NES::SinkStatementHandler sinkStatementHandler{sinks};
    ...
    return {
        ...
        NES::PlanStage::BoundLogicalPlan{std::get<NES::ExplainQueryStatement>(statements.back()).plan}};
}
...
TEST_F(DistributedPlanningTest, BasicPlacementSingleNode)
{
    ...
    auto plan = QueryPlanner::with(std::move(ctx)).plan(std::move(boundPlan));
...
TEST_F(DistributedPlanningTest, Disconnected)
{
    ...
    EXPECT_ANY_THROW(QueryPlanner::with(std::move(ctx)).plan(std::move(boundPlan)));
}
