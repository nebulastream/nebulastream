...
TEST_F(DistributedPlanningTest, BasicPlacementSingleNode)
{
    ...
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "basic_placement_single_node.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));

    const LogicalPlan localPlan = plan["localhost:8081"];
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "basic_placement_two_nodes.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));

    const auto sourceNodePlan = plan["source-node"];
    ...
    const auto sinkNodePlan = plan["sink-node"];
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "join_placement_with_one_selection.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));

    const auto sourceNode0Plan = plan["source-node0"];
    ...
    const auto sourceNode1Plan = plan["source-node1"];
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "placement_with_three_nodes.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));

    const auto plan0 = plan["sink-node"];
    ...
    const auto plan1 = plan["source-node0"];
    ...
    const auto plan2 = plan["source-node1"];
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "join_placement_with_limited_capacity.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "join_placement_with_limited_capacity_on_two_nodes.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));

    const auto sinkPlan = plan["sink-node"];
    ...
    const auto sourcePlan = plan["source-node"];
    ...
    EXPECT_EQ(getOperatorByType<SinkLogicalOperator>(sourcePlan)[1].getSinkDescriptor()->getSinkType(), "Network");
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "bridge_placement.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));
    ...
    const auto intermediatePlan = plan["intermediate-node"];
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "long_bridge_placement.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));
    ...
    const auto intermediatePlan0 = plan["intermediate-node0"];
    ...
    const auto intermediatePlan1 = plan["intermediate-node1"];
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "bridge_placement_join.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));
    ...
    EXPECT_EQ(getOperatorByType<SinkLogicalOperator>(intermediatePlan)[0].getSinkDescriptor()->getSinkType(), "Network");
    EXPECT_EQ(getOperatorByType<SinkLogicalOperator>(intermediatePlan)[1].getSinkDescriptor()->getSinkType(), "Network");
...
{
    auto queryConfig = CLI::YamlLoader<CLI::QueryConfig>::load(std::filesystem::path{NEBULI_TEST_DATA_DIR} / "complex_join_query.yaml");
    auto boundPlan = CLI::YamlBinder::from(std::move(queryConfig)).bind();
    auto plan = QueryPlanner::plan(std::move(boundPlan));
    ...
    const auto sourceNode2Plan = plan["source-node2"];
    ...
    const auto intermediateNode0Plan = plan["intermediate-node0"];
    ...
    const auto intermediateNode1Plan = plan["intermediate-node1"];
