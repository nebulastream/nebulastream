...
{
    const std::unordered_map<GrpcAddr, std::vector<QueryId>> localQueries
        = {{GrpcAddr("worker-1:8080"), {QueryId(LocalQueryId(generateUUID())), QueryId(LocalQueryId(generateUUID()))}},
           {GrpcAddr("worker-2:8080"), {QueryId(LocalQueryId(generateUUID()))}}};

    const DistributedQuery query(localQueries);

    std::unordered_map<GrpcAddr, std::vector<QueryId>> collected;
...
TEST_F(DistributedQueryTest, EqualityOperator)
{
    ...
    const std::unordered_map<GrpcAddr, std::vector<QueryId>> localQueries1
        = {{GrpcAddr("worker-1:8080"), {qid1}}, {GrpcAddr("worker-2:8080"), {qid2}}};

    const std::unordered_map<GrpcAddr, std::vector<QueryId>> localQueries2
        = {{GrpcAddr("worker-1:8080"), {qid1}}, {GrpcAddr("worker-2:8080"), {qid2}}};

    const std::unordered_map<GrpcAddr, std::vector<QueryId>> localQueries3 = {{GrpcAddr("worker-3:8080"), {qid3}}};
...
TEST_F(DistributedQueryTest, MultipleQueriesPerNode)
{
    ...
    auto qid2 = QueryId(LocalQueryId(generateUUID()));
    auto qid3 = QueryId(LocalQueryId(generateUUID()));
    const std::unordered_map<GrpcAddr, std::vector<QueryId>> localQueries = {{GrpcAddr("worker-1:8080"), {qid1, qid2, qid3}}};
...
TEST_F(DistributedQueryTest, DistributedQueryStatusGlobalStateAllRegistered)
{
    ...
    status.localStatusSnapshots[GrpcAddr("worker-1:8080")][QueryId(qid1)]
        = LocalQueryStatus{.queryId = QueryId(qid1), .state = QueryState::Registered, .metrics = {}};
    status.localStatusSnapshots[GrpcAddr("worker-2:8080")][QueryId(qid1)]
...
TEST_F(DistributedQueryTest, DistributedQueryStatusGlobalStateAllRunning)
{
    ...
    status.localStatusSnapshots[GrpcAddr("worker-2:8080")][QueryId(qid2)]
...
TEST_F(DistributedQueryTest, DistributedQueryStatusGlobalStateUnreachable)
{
    ...
    status.localStatusSnapshots[GrpcAddr("worker-1:8080")][QueryId(qid1)]
        = LocalQueryStatus{.queryId = QueryId(qid1), .state = QueryState::Running, .metrics = {}};
    status.localStatusSnapshots[GrpcAddr("worker-2:8080")][QueryId(qid1)] = std::unexpected(QueryStatusFailed("Connection error"));
...
TEST_F(DistributedQueryTest, DistributedQueryStatusGetExceptionsFromExpected)
{
    ...
    DistributedQueryStatus status;
    status.queryId = DistributedQueryId("test-distributed-query-1");
    status.localStatusSnapshots[GrpcAddr("worker-1:8080")][QueryId(qid1)] = std::unexpected(QueryStatusFailed("Communication error"));
...
TEST_F(DistributedQueryTest, DistributedQueryStatusCoalesceExceptionMultiple)
{
    ...
    status.localStatusSnapshots[GrpcAddr("worker-3:8080")][QueryId(qid1)]
