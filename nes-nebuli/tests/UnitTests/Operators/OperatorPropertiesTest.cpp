...
bool assignPropertiesToQueryOperators(const QueryPlanPtr& queryPlan, std::vector<std::map<std::string, std::any>> properties)
{
    ...
    EXPECT_EQ(numOperators, properties.size()) << "UtilityFunctions::assignPropertiesToQueryOperators: the number of properties "
                                                  "does not match the number of operators. The query plan is"
                                               << queryPlan->toString();
