...
class WindowSerializationTest : public Testing::BaseUnitTest, public ::testing::WithParamInterface<std::tuple<TimeMeasure, TimeUnit>>
{
public:
...
TEST_P(WindowSerializationTest, testSerializeDeserializeWindowOperators)
{
    ...
    auto descriptors
        = {SlidingWindow::of(
               EventTime(Attribute("event_time"), std::get<1>(GetParam())),
               std::get<0>(GetParam()),
...
TEST_P(WindowSerializationTest, testSerializeDeserializeWindowJoinOperators)
{
    ...
    for (const auto& logical_window_descriptor : descriptors)
    {
        auto joinDescriptor = Join::LogicalJoinDescriptor::create(
            joinExpression, logical_window_descriptor, 1, 1, Join::LogicalJoinDescriptor::JoinType::INNER_JOIN);
