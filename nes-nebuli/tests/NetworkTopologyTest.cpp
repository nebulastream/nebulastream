...
TEST_F(NetworkTopologyTest, FindLCAInComplexTopology)
{
    ...
    EXPECT_THAT(topology.findLCA(left1, dest, left2, Topology::Upstream), IsEmpty()); ///NOLINT(readability-suspicious-call-argument)

    EXPECT_THAT(
        topology.findLCA(left2, right2, src, Topology::Upstream), ///NOLINT(readability-suspicious-call-argument)
        ElementsAre(AllOf(
            Field(&Topology::LowestCommonAncestor::lca, src),
            Field(
                &Topology::LowestCommonAncestor::paths,
...
TEST_F(NetworkTopologyTest, InvalidDAGWithCycle)
{
    const auto node1 = HostAddr("node1");
    const auto node2 = HostAddr("node2");
    const auto node3 = HostAddr("node3");
    auto topology = Topology{};
    topology.addNode(node1, {node2});
...
TEST_F(NetworkTopologyTest, InvalidDAGWithSubtleCycle)
{
    ...
    const auto left = HostAddr("left");
    const auto right = HostAddr("right");
    const auto middle1 = HostAddr("middle1");
    const auto middle2 = HostAddr("middle2");
    const auto sink = HostAddr("sink");
