...
struct MockSubmissionBackend : QuerySubmissionBackend
{
    [[nodiscard]] std::expected<LocalQueryId, Exception> registerQuery(LogicalPlan) override
    {
        ...
        auto id = LocalQueryId(state->counter++);
    ...
    struct State
    {
        LocalQueryId::Underlying counter = LocalQueryId::INITIAL;
...
PlanStage::DistributedLogicalPlan dummyPlan()
{
    return PlanStage::DistributedLogicalPlan{
        PlanStage::DecomposedLogicalPlan{{{"grpc:8080", {LogicalPlan{}}}}}, PlanStage::OptimizedLogicalPlan{LogicalPlan{}}};
}
...
TEST_F(QueryManagerTest, Test)
{
    ...
    workerCatalog->addWorker("host:9090", "grpc:8080", 100000, {});
    QueryManager qm(
        workerCatalog,
    ...
    EXPECT_THAT(status.workerStatus.at("grpc:8080").value().activeQueries, ::testing::SizeIs(1));
    backend->stopQuery(LocalQueryId(1));

    auto statusAfter = qm.workerStatus(std::chrono::system_clock::time_point{std::chrono::milliseconds(0)}).value();
    EXPECT_THAT(statusAfter.workerStatus.at("grpc:8080").value().terminatedQueries, ::testing::SizeIs(1));
}
}