...
public:
    ...
    CLI::GrpcAddr getGrpcAddressFor(const CLI::HostAddr& hostAddr) const { return nodes.at(hostAddr).grpc; }

    size_t getCapacityFor(const CLI::HostAddr& hostAddr) const { return nodes.at(hostAddr).capacity; }

    bool hasCapacity(const CLI::HostAddr& hostAddr) const { return getCapacityFor(hostAddr) > 0; }
