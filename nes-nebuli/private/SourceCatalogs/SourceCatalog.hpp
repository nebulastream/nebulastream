...
public:
    ...
    bool addLogicalSource(const IdentifierList& logicalSourceName, Schema schema);
    ...
    bool removeLogicalSource(const IdentifierList& logicalSourceName);
    ...
    bool addPhysicalSource(const IdentifierList& logicalSourceName, const std::shared_ptr<SourceCatalogEntry>& sourceCatalogEntry);
    ...
    bool removePhysicalSource(const IdentifierList& logicalSourceName, const IdentifierList& physicalSourceName, WorkerId topologyNodeId);
    ...
    Schema getSchemaForLogicalSource(const IdentifierList& logicalSourceName);
    ...
    std::shared_ptr<LogicalSource> getLogicalSource(const IdentifierList& logicalSourceName);
    ...
    std::shared_ptr<LogicalSource> getLogicalSourceOrThrowException(const IdentifierList& logicalSourceName);

    /// @brief Check if logical source with this name exists
    /// @param logicalSourceName name of the logical source
    /// @return bool indicating if source exists
    bool containsLogicalSource(const IdentifierList& logicalSourceName);
    ...
    std::vector<WorkerId> getSourceNodesForLogicalSource(const IdentifierList& logicalSourceName);
    ...
    const std::unordered_map<IdentifierList, Schema>& getAllLogicalSource();


    /// @brief get all physical sources for a logical source
    /// @param logicalSourceName name of the logical source
    /// @return vector containing source catalog entries
    std::vector<std::shared_ptr<SourceCatalogEntry>> getPhysicalSources(const IdentifierList& logicalSourceName);

    /// @brief method to update a logical source schema
    /// @param logicalSourceName logical source name
    /// @param schema of logical source as object
    /// @return bool indicating if update was successful
    bool updateLogicalSource(const IdentifierList& logicalSourceName, Schema schema);

private:
    /// @brief test if logical source with this name exists
    /// @param logicalSourceName name of the logical source to test
    /// @return bool indicating if source exists
    bool testIfLogicalSourceExistsInLogicalToPhysicalMapping(const IdentifierList& logicalSourceName);

    std::recursive_mutex catalogMutex;
    /// map logical source to schema
    std::unordered_map<IdentifierList, Schema> logicalSourceNameToSchemaMapping;
    /// map logical source to physical source
    std::unordered_map<IdentifierList, std::vector<std::shared_ptr<SourceCatalogEntry>>> logicalToPhysicalSourceMapping;
};
}
}
