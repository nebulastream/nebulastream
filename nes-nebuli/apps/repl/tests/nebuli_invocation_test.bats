...
@test "basic test" {
  ...
  assert_json_equal '[{"local_query_id":1}]' "${lines[4]}"

  echo "${lines[5]}" | jq -e '(. | length) == 1'
  echo "${lines[5]}" | jq -e '.[].query_status | test("^Running|Registered|Started$")'

  assert_json_equal '[{"local_query_id":1}]' "${lines[6]}"
  assert_json_contains '[{"local_query_id":1,"query_status":"Stopped"}]' "${lines[7]}"
}
