...
@test "basic test" {
  ...
  assert_json_equal '[{"local_query_id":"d1041672-d4c9-40ac-a2b7-6b11342764b4"}]' "${lines[4]}"

  echo "${lines[5]}" | jq -e '(. | length) == 1'
  echo "${lines[5]}" | jq -e '.[].query_status | test("^Running|Registered|Started$")'

  assert_json_equal '[{"local_query_id":"d1041672-d4c9-40ac-a2b7-6b11342764b4"}]' "${lines[6]}"
