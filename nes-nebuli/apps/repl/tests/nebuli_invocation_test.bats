...
@test "basic test" {
  ...
  assert_json_equal '[{"global_query_id":1}]' "${lines[4]}"

  # One global and one local query
  echo "${lines[5]}" | jq -e '(. | length) == 2'
  echo "${lines[5]}" | jq -e '.[].query_status | test("^Running|Registered|Started$")'

  assert_json_equal '[{"global_query_id":1}]' "${lines[6]}"
  assert_json_contains '[{"global_query_id":1,"query_status":"Stopped"}]' "${lines[7]}"
}
