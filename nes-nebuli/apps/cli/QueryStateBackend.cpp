...
PersistedQueryId QueryStateBackend::store(DistributedQueryId distributedQueryId, const DistributedQuery& distributedQuery)
{
    ...
    for (const auto& [grpcAddr, localQueryIds] : distributedQuery.iterate())
    {
        ...
        localQueriesJson[grpcKey].push_back(localQueryIds.getRawValue());
    }
...
DistributedQuery QueryStateBackend::load(PersistedQueryId persistedId)
{
    ...
    for (const auto& [grpcKey, localQueryIdsJson] : jsonObject["local_queries"].items())
    {
        GrpcAddr grpcAddr(grpcKey);
        std::vector<LocalQueryId> localQueryIds;
        for (const auto& localQueryIdStr : localQueryIdsJson)
        {
            localQueryIds.emplace_back(LocalQueryId(localQueryIdStr.get<std::string>()));
        }
        localQueries.emplace(grpcAddr, std::move(localQueryIds));
    }
