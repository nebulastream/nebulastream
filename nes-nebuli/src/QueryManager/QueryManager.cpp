...
std::expected<void, Exception> QueryManager::start(LocalQueryId queryId)
{
    ...
    try
    {
        ...
        for (size_t i = 0; i < statusRetries; ++i)
        {
            ...
            std::this_thread::sleep_for(statusPollInterval * (1 << i));
        }

        return std::unexpected{QueryStartFailed("Query Start failed Query state remains `Registered` after {} retries", statusRetries)};
    }
...
std::expected<void, Exception> QueryManager::unregister(LocalQueryId queryId)
{
    ...
    try
    {
        ...
        if (result)
        {
            ...
            INVARIANT(erased == 1, "A query that was not regisetered within the query managers state should not haven been unregistered");
