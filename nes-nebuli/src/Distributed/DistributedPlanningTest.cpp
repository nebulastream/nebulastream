...
TEST_F(DistributedPlanningTest, BasicPlacementSingleNode)
{
    const QueryDecomposer::DecomposedLogicalPlan plan
        = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "basic_placement_single_node.yaml");
...
{
    const QueryDecomposer::DecomposedLogicalPlan plan
        = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "basic_placement_two_nodes.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "join_placement_with_one_selection.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "placement_with_three_nodes.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "join_placement_with_limited_capacity.yaml");
...
TEST_F(DistributedPlanningTest, JoinPlacementWithLimitedCapacityOnTwoNodes)
{
    const auto decomposedPlan
        = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "join_placement_with_limited_capacity_on_two_nodes.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "bridge_placement.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "long_bridge_placement.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "bridge_placement_join.yaml");
...
{
    const auto decomposedPlan = QueryPlanner::plan(std::filesystem::path{TEST_DATA_DIR} / "complex_join_query.yaml");
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(intermediateNode0Plan)[0].getSourceDescriptor().getSourceType(), "Network");
    ...
    EXPECT_EQ(getOperatorByType<SourceDescriptorLogicalOperator>(intermediateNode1Plan)[0].getSourceDescriptor().getSourceType(), "Network");
