...
{
    return op.withAdditionalTraits({PlacementTrait{nodeId}});
}

TopologyGraph::NodeId OperatorPlacer::findFirstNodeWithCapacityFromLCAs(const std::vector<TopologyGraph::LowestCommonAncestor>& candidates) const
{
...
OperatorPlacer::OperatorPlacer(TopologyGraph& topology, NodeCatalog& nodeCatalog)
    : topology{topology}, nodeCatalog{nodeCatalog}
{
}

OperatorPlacer OperatorPlacer::from(TopologyGraph& topology, NodeCatalog& nodeCatalog)
{
    return OperatorPlacer{topology, nodeCatalog};
}

OperatorPlacer::PlacedLogicalPlan OperatorPlacer::place(LegacyOptimizer::OptimizedLogicalPlan&& inputPlan) &&
{
    ...
    sinkPlacement.emplace(root.get<SinkLogicalOperator>().sinkDescriptor->workerId);

    return PlacedLogicalPlan{
        .plan = LogicalPlan{
            plan.getQueryId(), {this->placeOperatorRecursive(root)}, plan.getOriginalSql()
        }
    };
}
...
TopologyGraph::NodeId OperatorPlacer::findFirstNodeWithCapacityOnPath(const TopologyGraph::NodeId& src, const TopologyGraph::NodeId& dest) const
{
