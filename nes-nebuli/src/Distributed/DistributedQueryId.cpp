...
{
template <>
struct convert<NES::Distributed::LocalQueryId>
{
    static bool decode(const YAML::Node& node, NES::Distributed::LocalQueryId& rhs)
    {
        rhs.grpcAddr = node["connection"].as<std::string>();
        rhs.id = node["id"].as<size_t>();
        return true;
    }
    static Node encode(const NES::Distributed::LocalQueryId& rhs)
    {
        YAML::Node node;
        node["id"] = rhs.id;
        node["connection"] = rhs.grpcAddr;
        return node;
    }
};
...
struct convert<NES::Distributed::QueryId>
{
    ...
    {
        rhs.localQueries = node["queries"].as<std::vector<NES::Distributed::LocalQueryId>>();
        return true;
    }
    static Node encode(const NES::Distributed::QueryId& rhs)
    {
        YAML::Node node;
        node["queries"] = rhs.localQueries;
        return node;
    }
};
}
