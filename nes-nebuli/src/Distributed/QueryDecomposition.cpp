...
LogicalOperator BridgeLogicalOperator::withAdditionalTraits(TraitSet traitSet) const
{
    auto copy = *this;
...
LogicalOperator QueryDecomposer::createNetworkSource(const NetworkChannel& channel) const
{
    ...
    return SourceDescriptorLogicalOperator{std::move(descriptor)}
        ...
        .withAdditionalTraits({PlacementTrait{*getPlacementFor(downstreamOp)}});
}
...
LogicalOperator QueryDecomposer::createNetworkSink(const NetworkChannel& channel) const
{
    ...
    const auto placement = *getPlacementFor(downstreamOp);

    networkSink.sinkDescriptor = std::make_shared<Sinks::SinkDescriptor>(
        "Network", DescriptorConfig::Config{{"type", "Network"}, {"channel", channel.id}, {"connection", placement}}, placement, false);

    return networkSink.withInputOriginIds({upstreamOp.getOutputOriginIds()})
        .withOutputOriginIds(upstreamOp.getOutputOriginIds())
        .withAdditionalTraits({PlacementTrait{placement}})
        .withInferredSchema({upstreamOp.getOutputSchema()});
}

QueryDecomposer::QueryDecomposer(
    OperatorPlacer::PlacedLogicalPlan&& placedPlan, const TopologyGraph& topology, const std::shared_ptr<SourceCatalog>& sourceCatalog)
    : inputPlan{std::move(placedPlan.plan)}, topology{topology}, sourceCatalog{sourceCatalog}
{
...
QueryDecomposer QueryDecomposer::from(
    OperatorPlacer::PlacedLogicalPlan&& placedPlan, const TopologyGraph& topology, const std::shared_ptr<SourceCatalog>& sourceCatalog)
{
    return QueryDecomposer(std::move(placedPlan), topology, sourceCatalog);
}
...
void QueryDecomposer::addOperatorAsRoot(const LogicalOperator& op)
{
    ...
    if (const auto [_, wasInserted] = planByNode.try_emplace(
            opPlacement, LogicalPlan{inputPlan.getQueryId(), {opWithoutChildren}, inputPlan.getOriginalSql()});
        not wasInserted)
    {
        planByNode.at(opPlacement) = planByNode.at(opPlacement).withAdditionalRoots({std::move(opWithoutChildren)});
    }
}
