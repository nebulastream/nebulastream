...
{
    auto children
        = visiting.getChildren() | std::views::transform([&sourceCatalog](const auto& child) { return applyRecursive(child, sourceCatalog); });
    if (const auto sourceNameOperator = visiting.tryGetAs<SourceNameLogicalOperator>())
    {
        ...
        auto expandedSourceOperators = entries
            | std::views::transform([&children](const auto& entry)
                                    { return LogicalOperator{SourceDescriptorLogicalOperator{entry}.withChildren(children | std::ranges::to<std::vector>())}; })
