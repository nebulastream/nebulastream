...
SourceStatementHandler::operator()(const ShowLogicalSourcesStatement& statement) const noexcept
{
    if (statement.name)
    {
...
SourceStatementHandler::operator()(const ShowPhysicalSourcesStatement& statement) const noexcept
{
...
SourceStatementHandler::operator()(const DropLogicalSourceStatement& statement) noexcept
{
    ...
    return std::unexpected{UnknownSource(statement.source.getLogicalSourceName())};
}

std::expected<DropPhysicalSourceStatementResult, Exception>
SourceStatementHandler::operator()(const DropPhysicalSourceStatement& statement) noexcept
{
    if (sourceCatalog->removePhysicalSource(statement.descriptor))
    {
        return DropPhysicalSourceStatementResult{statement.descriptor};
    }
    return std::unexpected{UnknownSource("Unknown physical source: {}", statement.descriptor)};
}
...
std::expected<CreateSinkStatementResult, Exception> SinkStatementHandler::operator()(const CreateSinkStatement& statement) noexcept
{
...
std::expected<ShowSinksStatementResult, Exception> SinkStatementHandler::operator()(const ShowSinksStatement& statement) const noexcept
{
    if (statement.name)
    {
...
std::expected<DropSinkStatementResult, Exception> SinkStatementHandler::operator()(const DropSinkStatement& statement) noexcept
{
    if (sinkCatalog->removeSinkDescriptor(statement.descriptor))
    {
        return DropSinkStatementResult{statement.descriptor};
    }
    return std::unexpected{UnknownSink(statement.descriptor.getSinkName())};
}
...
std::expected<DropQueryStatementResult, Exception> QueryStatementHandler::operator()(const DropQueryStatement& statement) noexcept
{
...
std::expected<QueryStatementResult, Exception> QueryStatementHandler::operator()(const QueryStatement& statement) noexcept
{
    ...
    return id.and_then([this](const auto& queryId) { return queryManager->start(queryId); })
        .transform([&id] { return QueryStatementResult{id.value()}; });
}
std::expected<ShowQueriesStatementResult, Exception> QueryStatementHandler::operator()(const ShowQueriesStatement& statement) const noexcept
{
    if (not statement.id.has_value())
    {
        return std::unexpected{NotImplemented("Showing all running queries")};
    }
