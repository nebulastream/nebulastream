...
std::expected<ShowSinksStatementResult, Exception> SinkStatementHandler::operator()(const ShowSinksStatement& statement) noexcept
{
    if (statement.name)
    {
...
{
    queryManager->stop(statement.id);
    return DropQueryStatementResult{statement.id};
}

std::expected<QueryStatementResult, Exception> QueryStatementHandler::operator()(const LogicalPlan& statement)
{
