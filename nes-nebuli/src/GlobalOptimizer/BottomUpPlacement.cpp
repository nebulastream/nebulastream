...
solvePlacement(const Topology& topology, const LogicalPlan& logicalPlan, const std::unordered_map<Topology::NodeId, size_t>& capacity)
{
    ...
    {
        LinearExpr totalPerNodeDemand;
        for (const LogicalOperator& op : BFSRange(logicalPlan.getRootOperators().front()))
        {
            totalPerNodeDemand += LinearExpr::Term(operatorPlacementMatrix.at({op.getId(), nodeId}), operatorCapacityDemand(op));
        }
        modelBuilder.AddLessOrEqual(totalPerNodeDemand, capacity.at(nodeId));
    }
    ...
    auto sinkOperator = logicalPlan.getRootOperators().front().tryGetAs<SinkLogicalOperator>().value().get();
    modelBuilder.AddEquality(operatorPlacementMatrix.at({rootOperatorId, HostAddr(sinkOperator.getSinkDescriptor()->getWorkerId())}), true);

    /// Set other placements for sink to 0
    for (const Topology::NodeId& nodeId : topology | std::views::keys)
    {
        if (nodeId != HostAddr(sinkOperator.getSinkDescriptor()->getWorkerId()))
        {
    ...
    if (response.status() == OPTIMAL || response.status() == FEASIBLE)
    {
        ...
        for (const LogicalOperator& op : BFSRange(logicalPlan.getRootOperators().front()))
        {
            for (const Topology::NodeId& nodeId : topology | std::views::keys)
            {
                ...
                {
                    solution.try_emplace(op.getId(), nodeId);
                    break;
                }
            }
        }
...
{
    return addAdditionalTraits(op, PlacementTrait{placement.at(op.getId()).getRawValue()})
        .withChildren(
            op.getChildren()
            | std::views::transform(
