    ...
    LogicalFilterOperatorPtr filterOperator, NodePtr windowOperator, NodePtr parOperator)
{
    ...
    std::vector<NodeFunctionFieldAccessPtr> groupByKeys = windowOperator->as<LogicalWindowOperator>()->getWindowDefinition()->getKeys();
...
{
    renameFilterAttributesByFunctionNodes(filterOperator, projectionOperator->as<LogicalProjectionOperator>()->getFunctions());
...
std::vector<NodeFunctionFieldAccessPtr> FilterPushDownRule::getFilterAccessFunctions(const NodeFunctionPtr& filterPredicate)
{
    ...
    for (auto itr = depthFirstNodeIterator.begin(); itr != NES::DepthFirstNodeIterator::end(); ++itr)
    {
        ...
        if ((*itr)->instanceOf<NodeFunctionFieldAccess>())
        {
            const NodeFunctionFieldAccessPtr accessFunctionNode = (*itr)->as<NodeFunctionFieldAccess>();
