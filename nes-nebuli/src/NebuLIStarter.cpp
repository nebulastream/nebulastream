...
int main(int argc, char** argv)
{
    ...
    if (program.is_subcommand_used("dump"))
    {
        ...
        auto output_file_path = dumpArgs.get<std::string>("-o");
        std::ostream* output;
        std::ofstream file;
        if (output_file_path == "-")
        {
        ...
        {
            file = std::ofstream(output_file_path);
            if (!file)
            {
                NES_FATAL_ERROR("Could not open output file: {}", output_file_path);
                std::exit(1);
            }
            output = &file;
        }
        ...
        if (output_file_path == "-")
        {
            NES_INFO("Wrote protobuf to {}", output_file_path);
        }
    }
