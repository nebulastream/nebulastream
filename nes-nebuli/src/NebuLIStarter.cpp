...
void handleStatus(const argparse::ArgumentParser& statusArgs)
{
    ...
    const auto queryConfig = NES::CLI::YamlLoader<NES::CLI::QueryConfig>::load(inputPath);
...
void handleDump(const argparse::ArgumentParser& dumpArgs, const std::vector<NES::LogicalPlan>& plans)
{
...
{
    for (auto [queries] = NES::Distributed::QueryId::load(queryIdStr); const auto& [queryId, grpcAddr] : queries)
    {
        try
        {
            auto channel = grpc::CreateChannel(grpcAddr, grpc::InsecureChannelCredentials());
            auto client = NES::GRPCClient{std::move(channel)};
            func(client, NES::QueryId{queryId});
            NES_INFO("Successfully executed command for query {} on host {}", queryId, grpcAddr);
        }
        catch (const std::exception& e)
        {
            NES_ERROR("Failed to execute command for query {} on host {}: {}", queryId, grpcAddr, e.what());
            throw;
        }
    }
}
...
void handleRegister(const argparse::ArgumentParser& registerArgs, const NES::QueryPlanner::FinalizedLogicalPlan& finalPlan)
{
    std::vector<NES::Distributed::LocalQueryId> queryFragments;
    const bool autoStart = registerArgs.is_used(ARG_AUTO_START);

    for (const auto& [grpcAddr, plans] : finalPlan)
    {
        try
        {
            auto channel = grpc::CreateChannel(grpcAddr, grpc::InsecureChannelCredentials());
            ...
            for (const auto& plan : plans)
            {
                ...
                queryFragments.emplace_back(queryId.getRawValue(), grpcAddr);
    ...
    const auto id = NES::Distributed::QueryId::save(NES::Distributed::QueryId{queryFragments});
    std::cout << id << '\n';
    ...
    CPPTRACE_TRY
    {
        ...
        try
        {
            ...
            else if (program.is_subcommand_used(CMD_REGISTER) || program.is_subcommand_used(CMD_DUMP))
            {
                ...
                const auto input = subparser.get<std::string>(ARG_INPUT_SHORT);

                auto queryConfig = NES::CLI::YamlLoader<NES::CLI::QueryConfig>::load(input);
                auto boundPlan = NES::CLI::YamlBinder::from(std::move(queryConfig)).bind();
                auto finalPlan = NES::QueryPlanner::plan(std::move(boundPlan));

                if (command == CMD_DUMP)
                {
                    for (const auto& nodePlan : finalPlan | std::views::values)
                    {
                ...
                {
                    handleRegister(subparser, finalPlan);
                }
            }
            else
            {
