...
int main(int argc, char** argv)
{
    CPPTRACE_TRY
    {
        ...
        bool anySubcommandUsed = std::any_of(
            subcommands.begin(), subcommands.end(), [&](auto& subparser) { return program.is_subcommand_used(subparser.get()); });
        if (!anySubcommandUsed)
        {
            auto serverUri = program.get<std::string>("-s");
            if (not program.is_used("-s"))
            {
                std::cout << "No server URI provided, using default server URI: " << serverUri << std::endl;
            }
