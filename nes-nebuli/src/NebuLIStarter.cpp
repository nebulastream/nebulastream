...
{
    auto [queries] = NES::Distributed::QueryId::load(queryIdStr);

    for (const auto& [hostAddr, queryId] : queries)
    {
        try
        {
...
NES::QueryDecomposer::DecomposedLogicalPlan prepareQueryPlan(const std::string& inputPath)
{
    // Load and process query configuration
    auto queryConfig = NES::CLI::YAMLLoader::load(inputPath);
    auto [plan, topology, nodeCatalog, sourceCatalog, sinkCatalog] = NES::CLI::YAMLBinder::from(std::move(queryConfig)).bind();

    /// Optimize the plan
    auto optimizedPlan = NES::LegacyOptimizer::optimize(std::move(plan), sourceCatalog);

    /// Place operators
    auto placedPlan = NES::OperatorPlacer::from(std::move(optimizedPlan), topology, nodeCatalog, sinkCatalog).place();

    /// Decompose the plan
    return NES::QueryDecomposer::from(std::move(placedPlan), topology, sourceCatalog, sinkCatalog).decompose();
}
    ...
    CPPTRACE_TRY
    {
        ...
        try
        {
            ...
            else if (program.is_subcommand_used(CMD_REGISTER) || program.is_subcommand_used(CMD_DUMP))
            {
                ...
                auto decomposedPlan = prepareQueryPlan(inputPath);
