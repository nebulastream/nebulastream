...
NES::DistributedException::DistributedException(std::unordered_map<GrpcAddr, std::vector<Exception>> errors) : errors(std::move(errors))
{
...
NES::DistributedQuery::DistributedQuery(std::unordered_map<GrpcAddr, std::vector<QueryId>> localQueries)
