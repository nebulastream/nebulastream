...
std::string generateHorseName()
{
    ...
    std::string_view attribute = ATTRIBUTES[randomAttribute(rng)];
    std::string_view breed = HORSE_BREEDS[randomBreed(rng)];

    return fmt::format("{}_{}{}", attribute, breed, counter++);
}

NES::DistributedQueryId NES::getNextDistributedQueryId()
{
    return DistributedQueryId(generateHorseName());
}
