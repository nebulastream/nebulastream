...
QueryDecomposer::DecomposedLogicalPlan QueryPlanner::plan(const std::string& inputPath)
{
    ...
    auto [plan, topology, nodeCatalog, sourceCatalog] = CLI::YAMLBinder::from(std::move(queryConfig)).bind();
    ...
    /// Decompose the plan
    return QueryDecomposer::from(std::move(placedPlan), topology, sourceCatalog).decompose();
}
}
