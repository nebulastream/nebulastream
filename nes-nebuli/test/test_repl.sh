...
EXPECTED_OUTPUT=$(cat "$EXPECTED_OUTPUT_PATH" | sed "s|TESTDATA|$TESTDATA_PATH|g" | sed "s|WORKDIR|$WORKDIR_PATH|g")
echo "$EXPECTED_OUTPUT" > "$GEN_OUT_PATH"/"$(basename "$EXPECTED_OUTPUT_PATH" .sql)"_gen.sql
...
if [ "$ACTUAL_OUTPUT" = "$EXPECTED_OUTPUT" ]; then
  ...
  echo "Actual output does not match expected, diff was:"
  echo "$(diff <(echo "$EXPECTED_OUTPUT") <(echo "$ACTUAL_OUTPUT"))"
  exit 1
fi
