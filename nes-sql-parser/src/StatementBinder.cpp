    ...
    for (const char character : snakeCase)
    {
        ...
        else
        {
            if (capitalizeNext)
            {
                ...
                INVARIANT(capitalized > 0 && capitalized <= CHAR_MAX, "Trying to uppercase invalid character of value {}", capitalized);
            ...
            else
            {
                ...
                INVARIANT(lowered > 0 && lowered <= CHAR_MAX, "Trying to uppercase invalid character of value {}", lowered);
...
public:
    ...
    std::string bindIdentifier(AntlrSQLParser::StrictIdentifierContext* strictIdentifier) const
    {
        ...
        INVARIANT(false, "Unknown identifier type");
    }
    ...
    Literal bindLiteral(AntlrSQLParser::ConstantContext* literalAST) const
    {
        ...
        INVARIANT(false, "Unknown literal type");
    }
