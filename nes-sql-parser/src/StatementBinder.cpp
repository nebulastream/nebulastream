...
namespace
{
std::string snakeToCamelCase(const std::string_view snake_case)
{
    if (snake_case.empty())
    {
        return std::string{snake_case};
    }

    std::string camelCase;
    camelCase.reserve(snake_case.length()); // Reserve space for efficiency

    bool capitalizeNext = false;

    for (const char c : snake_case)
    {
        if (c == '_')
        {
            capitalizeNext = true;
...
public:
    ...
    std::unordered_map<std::string, std::unordered_map<std::string, Literal>>
    bindConfigOptions(std::vector<AntlrSQLParser::NamedConfigExpressionContext*> configOptions) const
    {
    ...
    bindCreatePhysicalSourceStatement(AntlrSQLParser::CreatePhysicalSourceDefinitionContext* physicalSourceDefAST) const
    {
        ...
        /// TODO use normal identifiers for types
        ...
        if (not Sources::SourceValidationProvider::provide(type, sourceOptions).has_value())
        {
            ...
            throw InvalidConfigParameter("Invalid source configuration");
        }
    ...
    CreateSinkStatement bindCreateSinkStatement(AntlrSQLParser::CreateSinkDefinitionContext* sinkDefAST) const
    {
        ...
        return CreateSinkStatement{sinkName, sinkType, schema, sinkOptions};
    }
    ...
    {
        const StatementOutputFormat format = showFormat != nullptr ? bindFormat(showFormat) : StatementOutputFormat::TEXT;
        ...
        AntlrSQLParser::ShowFormatContext* showFormat) const
    {
        ...
        if (physicalSourcesSubject->logicalSourceName != nullptr)
        {
            ...
            {
                throw UnknownSource("There is no logical source with name {}", logicalSourceName);
            }
        }
        if (showFilter != nullptr)
        {
        ...
        if (AntlrSQLParser::DropSourceContext* dropSourceAst = dropAst->dropSubject()->dropSource(); dropSourceAst != nullptr)
        {
            if (const auto* const logicalSourceSubject = dropSourceAst->dropLogicalSourceSubject(); logicalSourceSubject != nullptr)
            {
                    ...
                    sourceCatalog->containsLogicalSource(logicalSourceName))
                {
                    ...
                    throw UnknownSource(logicalSourceSubject->name->getText());
                }
            }
            if (const auto* const physicalSourceSubject = dropSourceAst->dropPhysicalSourceSubject(); physicalSourceSubject != nullptr)
            {
                ...
                throw UnknownSource("There is no physical source with id {}", physicalSourceSubject->id->getText());
            }
        }
