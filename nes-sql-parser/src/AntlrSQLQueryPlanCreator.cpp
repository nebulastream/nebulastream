...
void AntlrSQLQueryPlanCreator::exitFunctionCall(AntlrSQLParser::FunctionCallContext* context)
{
    ...
    switch (tokenType)
    {
        ...
        default: {
            ...
            /// its first argument. Say so before the generic "arguments did not reach the stack" error would.
            if (funcName == "STATISTIC_BUILD" and helpers.top().statisticBuild.has_value() and helpers.top().windowAggs.empty())
            {
                throw InvalidQuerySyntax(
            ...
            if (funcName == "STATISTIC_BUILD")
            {
                ...
                {
                    throw InvalidQuerySyntax(
                        "STATISTIC_BUILD expects an aggregation call and a statisticId, e.g. STATISTIC_BUILD(SUM(x), 42), at {}",
                        context->getText());
                }
                const auto aggregationOutput = arguments.front().tryGetAs<UnboundFieldAccessLogicalFunction>();
                const auto wrapsLastAggregation = aggregationOutput.has_value() and not helpers.top().windowAggs.empty()
                    and helpers.top().windowAggs.back().second == aggregationOutput.value()->getFieldName();
                if (not wrapsLastAggregation)
                {
                    throw InvalidQuerySyntax(
                        "STATISTIC_BUILD expects an aggregation call as its first argument, e.g. STATISTIC_BUILD(SUM(x), 42), at {}",
                        context->getText());
                }
                if (helpers.top().statisticBuild.has_value())
                {
                ...
                const auto statisticId = parseStatisticId(arguments.back(), funcName, context->getText());
                const auto aggregation = helpers.top().windowAggs.back().first;
                helpers.top().windowAggs.pop_back();
                helpers.top().statisticBuild = AntlrSQLHelper::StatisticBuildInfo{
                    .statisticId = statisticId, .statisticFunction = aggregation, .functionName = funcName};
                break;
            }
