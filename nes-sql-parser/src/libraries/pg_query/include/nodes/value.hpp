 ...
 *		PGValue node
 *
 * The same PGValue struct is used for five node types: pgquery::T_PGInteger,
 * pgquery::T_PGFloat, pgquery::T_PGString, pgquery::T_PGBitString, T_Null.
 *
 * Integral values are actually represented by a machine integer,
 * but both floats and strings are represented as strings.
 * Using pgquery::T_PGFloat as the node type simply indicates that
 * the contents of the string look like a valid numeric literal.
 *
 * (Before Postgres 7.0, we used a double to represent pgquery::T_PGFloat,
 * but that creates loss-of-precision problems when the value is
 * ultimately destined to be converted to NUMERIC.  Since PGValue nodes
 * are only used in the parsing process, not for runtime data, it's
 * better to use the more general representation.)
 *
 * Note that an integer-looking string will get lexed as pgquery::T_PGFloat if
...
typedef struct PGValue {
	PGNodeTag type; /* tag appropriately (eg. pgquery::T_PGString) */
	union ValUnion {
