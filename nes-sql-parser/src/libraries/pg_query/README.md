...
-  The parser and all its auxiliary structures are wrapped in the `pgquery` namespace.
...
Bison is defined in terms of rules. Each rule defines a _return type_. The return types of the rules should be defined in the corresponding `.yh` file (for example, for the `select.y`, the return types are defined in `third_party/libpg_query/grammar/types/select.yh`).
...
Note that in the generated C code we see two special symbols: `$$` and `$1`. `$$` is the _return value_ of the rule. The `$1, $2, $3, etc` symbols refer to the sub-rules.
...
The other lists exist for keywords that are not _entirely_ reserved, but are partially reserved.
...
Note that these lists are not mutually exclusive. A keyword can be added to _both_ `column_name_keywords.list` and `func_name_keywords.list`.
