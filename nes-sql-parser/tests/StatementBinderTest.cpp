...
{
    const auto plan = bindPlan(fmt::format("SELECT STATISTIC_BUILD(SUM(x), 43){}", WINDOWED));
...
{
    const auto plan = bindPlan(fmt::format("SELECT STATISTIC_BUILD(COUNT(*), 46){}", WINDOWED));
    const auto writers = getOperatorByType<StatisticStoreWriterLogicalOperator>(plan);
    ASSERT_EQ(writers.size(), 1);
...
{
    const auto plan = bindPlan(fmt::format("SELECT STATISTIC_BUILD(SUM(x + UINT64(1)), 43){}", WINDOWED));
...
TEST_F(StatementBinderTest, StatisticCallsWithBadArgumentsAreInvalidQuerySyntax)
{
    const std::vector<std::string> selects{
        ...
        "TESTSYNOPSIS(42, x)",
        "TESTSYNOPSIS(42, 1, 2)",
        "STATISTIC_BUILD(TESTSYNOPSIS(42, 100), 43)",
        "STATISTIC_BUILD(x, 43)",
        "STATISTIC_BUILD(SUM(x) + UINT64(1), 43)",
        "STATISTIC_BUILD(SUM(x), 0)",
        "STATISTIC_BUILD(42, 'AVG', x)",
        "STATISTIC_BUILD(SUM(x), 43), AVG(y)",
        "STATISTIC_BUILD(SUM(x), 1), STATISTIC_BUILD(SUM(y), 2)",
    ...
    EXPECT_EQ(bindError("SELECT STATISTIC_BUILD(SUM(x), 43) FROM s INTO sink"), ErrorCode::InvalidQuerySyntax) << "build without window";
