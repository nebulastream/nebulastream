...
public:
    ...
    void SetUp() override
    {
        ...
        binder = std::make_shared<StatementBinder>(
            sourceCatalog, sinkCatalog, std::bind(AntlrSQLQueryParser::bindLogicalQueryPlan, std::placeholders::_1));
...
TEST_F(StatementBinderTest, BindCreateBindSource)
{
    ...
    const std::string createPhysicalSourceStatement
        = R"(CREATE PHYSICAL SOURCE FOR testSource TYPE File SET ('LOCAL' as `SOURCE`.LOCATION, -1 as `SOURCE`.NUMBER_OF_BUFFERS_IN_LOCAL_POOL, '/dev/null' AS `SOURCE`.FILE_PATH, 'CSV' AS PARSER.`TYPE`, '\n' AS PARSER.TUPLE_DELIMITER, ',' AS PARSER.FIELD_DELIMITER))";
    ...
    std::unordered_map<std::string, std::string> unvalidatedConfig{{"filePath", "/dev/null"}, {"location", "1"}};
...
{
    const std::string createSinkStatement = "CREATE SINK testSink (attribute1 UINT32, attribute2 VARSIZED) TYPE File SET ('LOCAL' as "
                                            "`SINK`.LOCATION, '/dev/null' AS `SINK`.FILE_PATH, 'CSV' AS `SINK`.INPUT_FORMAT)";
    ...
    ASSERT_EQ(actualSink.getFromConfig(Sinks::SinkDescriptor::LOCATION), INITIAL<WorkerId>.getRawValue());
    ASSERT_EQ(actualSink.getFromConfig(Sinks::SinkDescriptor::FILE_PATH), "/dev/null");
    ASSERT_EQ(actualSink.getFromConfig(Sinks::ConfigParametersFile::INPUT_FORMAT), Configurations::InputFormat::CSV);
...
TEST_F(StatementBinderTest, BindCreateSinkWithQualifiedColumns)
{
    ...
    const std::string createSinkStatement = "CREATE SINK testSink (testSource.attribute1 UINT32, testSource.attribute2 VARSIZED) TYPE File SET ('LOCAL' as "
                                            "`SINK`.LOCATION, '/dev/null' AS `SINK`.FILE_PATH, 'CSV' AS `SINK`.INPUT_FORMAT)";
    const auto statement = binder->parseAndBindSingle(createSinkStatement);
    ASSERT_TRUE(statement.has_value());
    ASSERT_TRUE(std::holds_alternative<CreateSinkStatement>(*statement));
...
TEST_F(StatementBinderTest, ShowLogicalSources)
{
    ...
    ASSERT_TRUE(format2 == StatementOutputFormat::TEXT);
...
TEST_F(StatementBinderTest, ShowPhysicalSources)
{
    const std::vector<std::string_view> createLogicalSources{
        "CREATE LOGICAL SOURCE testSource1 (attribute1 UINT32, attribute2 VARSIZED)",
        "CREATE LOGICAL SOURCE testSource2 (attribute1 UINT32, attribute2 INT32)",
        "CREATE PHYSICAL SOURCE FOR testSource1 TYPE File SET ('LOCAL' as `SOURCE`.LOCATION, 200 as "
        ...
        "CREATE PHYSICAL SOURCE FOR testSource2 TYPE File SET ('LOCAL' as `SOURCE`.LOCATION, -1 as "
        ...
        "PARSER.TUPLE_DELIMITER, ',' AS PARSER.FIELD_DELIMITER)"};
    for (const auto& sourceStatementString : createLogicalSources)
    {
    ...
    const std::string_view filteredPhysicalSourcesStatementString = "SHOW PHYSICAL SOURCES WHERE ID = 1 FORMAT JSON";
    const std::string_view physicalSourceForLogicalSourceStatementString = "SHOW PHYSICAL SOURCES FOR testSourCe1";
    const std::string_view physicalSourceForLogicalSourceStatementFilteredString = "SHOW PHYSICAL SOURCES FOR testSource2 WHERE ID = 2";
    ...
    ASSERT_TRUE(format == StatementOutputFormat::TEXT);
    ...
    ASSERT_TRUE(*id4 == 2);
    ASSERT_TRUE(format4 == StatementOutputFormat::TEXT);
...
TEST_F(StatementBinderTest, ShowSinks)
{
    const std::vector<std::string_view> sinkStatements{
        "CREATE SINK testSink1 (attribute1 UINT32, attribute2 VARSIZED) TYPE File SET ('LOCAL' as "
        "`SINK`.LOCATION, '/dev/null' AS `SINK`.FILE_PATH, 'CSV' AS `SINK`.INPUT_FORMAT)",
        "CREATE SINK testSink2 (attribute1 UINT32, attribute2 INT32) TYPE File SET ('LOCAL' as "
        "`SINK`.LOCATION, '/dev/null' AS `SINK`.FILE_PATH, 'CSV' AS `SINK`.INPUT_FORMAT)"};
