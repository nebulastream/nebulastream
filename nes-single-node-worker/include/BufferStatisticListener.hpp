...
/// Publishes what the buffer providers are doing as a regular time series of CSV rows into this worker's
/// buffer event feed, which a 'BufferEvents' source turns into an ordinary stream. Buffer pool pressure
...
struct BufferStatisticListener final : BufferProviderStatisticListener
{
    ...
    BufferStatisticListener(const Host& host, size_t feedCapacity, std::chrono::milliseconds flushInterval);
