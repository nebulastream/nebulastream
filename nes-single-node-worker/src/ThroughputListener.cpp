...
void threadRoutine(
    ...
    std::ofstream& file,
    folly::MPMCQueue<ThroughputListener::CombinedEventType>& events)
{
    ...
    {
        ThroughputListener::CombinedEventType event = QueryStart{WorkerThreadId(0), QueryId(0)}; /// Will be overwritten
...
void ThroughputListener::onEvent(SystemEvent event)
{
    events.writeIfNotFull(std::visit([]<typename T>(T&& arg) { return CombinedEventType(std::forward<T>(arg)); }, std::move(event)));
}

ThroughputListener::ThroughputListener(const std::filesystem::path& path)
    : file(path, std::ios::out | std::ios::app)
    , timeIntervalInMilliSeconds(100)
    , calculateThread([this](const std::stop_token& stopToken)
                      { threadRoutine(stopToken, this->timeIntervalInMilliSeconds, file, events); })
