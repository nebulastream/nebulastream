    ...
    const std::string& name, Category cat, Phase ph, uint64_t ts, uint64_t dur, const nlohmann::json& args)
{
    ...
    switch (cat)
    {
        case Category::Query: event["cat"] = "query"; break;
        case Category::Pipeline: event["cat"] = "pipeline"; break;
        case Category::Task: event["cat"] = "task"; break;
        case Category::System: event["cat"] = "system"; break;
    }
    
    /// Convert phase enum to string
    switch (ph)
    {
        case Phase::Begin: event["ph"] = "B"; break;
        case Phase::End: event["ph"] = "E"; break;
        case Phase::Instant: event["ph"] = "i"; break;
    }
        ...
        std::visit(
            Overloaded{
                ...
                [&](const QueryStop& queryStop)
                {
                    ...
                    uint64_t originalTid = (it != activeQueries.end())
                                            ? it->second.second.getRawValue()
                                            : queryStop.threadId.getRawValue(); /// fallback
