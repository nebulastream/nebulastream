...
{
    handleEvent(std::visit([]<typename T>(T&& arg) { return CombinedEventType(std::forward<T>(arg)); }, std::move(event)), files, systemEventFile);
}
...
PrintingStatisticListener::PrintingStatisticListener(const Configurations::StringOption& statisticDir, const Configurations::UIntOption& numberOfWorkerThreads)
{
