...
BufferStatisticListener::BufferStatisticListener(const Host& host, const size_t feedCapacity, const std::chrono::milliseconds flushInterval)
    : flushInterval(flushInterval), feed(EventFeedRegistry::instance().create(host, FeedName::BUFFER_EVENTS, feedCapacity))
{
    NES_INFO(
        "Publishing the buffer statistics of worker {} every {}, buffered up to {} rows",
        host.getRawValue(),
        flushInterval,
        feed->capacity());
}
...
void BufferStatisticListener::threadRoutine(const std::stop_token& token)
{
    ...
    while (!token.stop_requested())
    {
        ...
        /// A row is rendered even when nothing happened, so that the series stays regular and an idle engine
        /// is visible as zeros rather than as a gap. The interval is rolled over whether or not anyone reads
        /// it, so the first row a query sees covers one interval rather than the worker's entire uptime.
        auto row = flushRow(std::chrono::system_clock::now());
        if (feed->hasConsumer())
        {
            feed->tryPush(std::move(row));
        }
    }
}
