...
/// Builds plans by hand and runs them on a worker, for shapes SQL cannot currently express.
///
/// The shape of interest is a diamond: one source feeding two branches that a union merges again.
///
///        SELECTION(value > 3)
///       /                     \
///   SOURCE                     UNION -> SINK
...
TEST_F(SharedOperatorPlanExecutionTest, DiamondOverSharedSourceCompilesDeploysAndRuns)
{
    ...
    auto singleBranch
        = LogicalPlan{QueryId::create(LocalQueryId{LocalQueryId::INVALID}, DistributedQueryId{DistributedQueryId::INVALID}), {singleBranchSink}};
    ...
    const auto merged
        = LogicalOperator{UnionLogicalOperator::create({optimizedHigh, low})}.withTraitSet(optimizedHigh.getTraitSet());
    const auto sink = LogicalOperator{SinkLogicalOperator::create(merged, createFileSink("diamond_sink"))}.withTraitSet(
        optimizedSink.getTraitSet());
