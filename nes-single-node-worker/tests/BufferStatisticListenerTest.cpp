...
protected:
    ...
    void TearDown() override { InProcessFeedRegistry::instance().clear(); }
};
...
{
    BufferStatisticListener listener("schema_feed", FEED_CAPACITY, FLUSH_INTERVAL);
...
{
    BufferStatisticListener listener("aggregate_feed", FEED_CAPACITY, FLUSH_INTERVAL);
...
{
    BufferStatisticListener listener("reset_feed", FEED_CAPACITY, FLUSH_INTERVAL);
...
/// The published rows have to end up on the feed an InProcess source reads, which is what makes them queryable.
TEST_F(BufferStatisticListenerTest, PublishesRowsIntoItsFeed)
{
    ...
    BufferStatisticListener listener("published_feed", FEED_CAPACITY, fastInterval);
    listener.onEvent(PooledBufferAcquired{5});
    listener.start();

    /// The source resolves the very same feed by name, so this is what a query would observe.
    const auto feed = InProcessFeedRegistry::instance().getOrCreate("published_feed", FEED_CAPACITY);
    const auto row = feed->tryPop(popTimeout);
    ...
    EXPECT_TRUE(feed->tryPop(popTimeout).has_value());
    EXPECT_EQ(feed->droppedRows(), 0);
}
