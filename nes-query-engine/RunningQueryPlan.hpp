...
struct CallbackOwner
{
    ...
    ~CallbackOwner();
    CallbackOwner& operator=(CallbackOwner&& other);
    operator bool() const { return !owner.expired(); }
    void addCallback(absl::AnyInvocable<void()> fn) const;
    ...
    void addCallbackUnsafe(CallbackRef ref, absl::AnyInvocable<void()> fn) const;

    CallbackRef addCallbackAssumeNonShared(CallbackRef&&, absl::AnyInvocable<void()> fn) const;
    std::optional<CallbackRef> getRef() const;
...
/// If the lifetime of the RunningQueryPlan object ends, all sources, pipelines, operator are terminated.
...
struct RunningQueryPlanNode
{
    ...
    std::atomic_bool requireTermination;
    std::vector<std::shared_ptr<RunningQueryPlanNode>> successors;
    std::unique_ptr<Execution::ExecutablePipelineStage> stage;
...
{
    static std::unique_ptr<InstantiatedQueryPlan> dispose(std::unique_ptr<StoppingQueryPlan> rp);
...
struct RunningQueryPlan
{
    ...
    static std::pair<std::unique_ptr<StoppingQueryPlan>, CallbackRef> stop(std::unique_ptr<RunningQueryPlan> rp);
    ...
    static std::unique_ptr<InstantiatedQueryPlan> dispose(std::unique_ptr<RunningQueryPlan> rp);
...
private:
    explicit RunningQueryPlan(CallbackOwner allPipelinesExpired, CallbackOwner pipelineSetupDone)
        : allPipelinesExpired(std::move(allPipelinesExpired)), pipelineSetupDone(std::move(pipelineSetupDone))
    {
    }
    ...
    CallbackOwner pipelineSetupDone;
};
