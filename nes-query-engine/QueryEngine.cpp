...
public:
    ...
    void emitQueryStopEvent(QueryId queryId) { statistic->onEvent(QueryStop{WorkerThread::id, queryId}); }
    ...
    WorkEmitter& emitter)
{
    ...
    struct RealQueryLifeTimeListener : QueryLifetimeListener
    {
        RealQueryLifeTimeListener(
            QueryId queryId, std::shared_ptr<AbstractQueryStatusListener> listener, QueryLifetimeController& controller)
