...
/// The TaskQueue is a central component within the QueryEngine. External components like sources or
/// users of the system can add new tasks to an admission queue which is bounded and will backpressure
/// sources if necessary. Internally WorkerThreads communicate via a shared internal queue, which is
/// unbounded to deal with occasionaly bursty loads like a large join. Access to the internal task queue
/// is always non blocking.
/// The TaskQueue exposes a blocking `nextTask` method which reads from either queue without spinning and
/// is supposed to be used by the worker threads.
...
public:
    ...
    bool admissionTask(const std::stop_token& stoken, T&& task)
    {
    ...
    /// Non-Blocking version of `nextTask` if the queue is empty, this method returns an empty optional.
