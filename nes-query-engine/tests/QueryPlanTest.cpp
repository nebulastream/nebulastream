...
struct EmittedTask
{
    ...
    ::testing::AssertionResult handleAll()
    {
        ...
        {
            auto r = executeEmittedTask(std::move(*it).second);
            if (!r)
            {
                return r;
            }
    ...
    ::testing::AssertionResult waitForTasks(size_t numberOfTasks)
    {
        ...
        {
            auto result = addCondition.wait_for(
                lock, std::chrono::milliseconds(100), [this, numberOfTasks]() { return arguments.size() >= numberOfTasks; });
            if (!result)
            {
    ...
    ::testing::AssertionResult handle(KeyT stage)
    {
        Args task;
        {
            ...
            {
                auto result
                    = addCondition.wait_for(lock, std::chrono::milliseconds(100), [this, stage]() { return arguments.contains(stage); });
                if (!result)
                {
...
std::unique_ptr<Terminations> Terminations::setup(RangeOf<Execution::ExecutablePipelineStage*> auto stages, TArgs&&... args)
{
    ...
    {
        EXPECT_CALL(emitter, emitStop(::testing::_, UniquePtrStageMatcher(stage), ::testing::_, ::testing::_))
            .WillOnce(::testing::Invoke([&terminations, stage](auto, auto n, auto cb, auto fail)
                                        { terminations->add(stage, std::move(n), std::move(cb), std::move(fail)); }));
    }
...
::testing::AssertionResult Terminations::executeEmittedTask(TerminatePipelineArgs&& args)
{
    ...
    try
    {
        if (args.target->stage->stop(pec) == 0)
        {
            args.onComplete();
        }
    }
...
std::unique_ptr<Setups> Setups::setup(RangeOf<Execution::ExecutablePipelineStage*> auto stages, TArgs&&... args)
{
    ...
    for (auto* stage : stages)
    {
        EXPECT_CALL(emitter, emitSetup(::testing::_, StageMatcher(stage), ::testing::_, ::testing::_))
            .WillOnce(::testing::Invoke([&setups, stage](auto, auto n, auto cb, auto fail)
                                        { setups->add(stage, std::move(n), std::move(cb), std::move(fail)); }));
    }
...
::testing::AssertionResult Setups::executeEmittedTask(SetupPipelineArgs&& args)
{
    ...
    try
    {
        ...
        {
            if (strongRef->stage->setup(pec) == 0)
            {
                args.onComplete();
            }
        }
...
std::unique_ptr<SourceStops> SourceStops::setup(RangeOf<OriginId> auto originIds, TArgs&&... args)
{
    ...
    for (auto originId : originIds)
    {
        EXPECT_CALL(controller, initializeSourceStop(::testing::_, ::testing::_, ::testing::_))
            .WillOnce(::testing::Invoke([&setups, originId](auto, auto, auto n) { setups->add(originId, std::move(n)); }));
    }
...
void leak(std::unique_ptr<RunningQueryPlan>&& rp, std::shared_ptr<TestQueryLifetimeListener>&& listener)
{
...
TEST_F(QueryPlanTest, RunningQueryPlanDefaultDestructor)
{
    ...
    auto q = test.addNewQuery(std::move(builder));
    auto srcCtrl = test.sourceControls[s];
    ...
    {
        auto rq = RunningQueryPlan::start(QueryId(0), std::move(q), controller, emitter, listener);
    }
...
TEST_F(QueryPlanTest, RunningQueryPlanDispose)
{
    ...
    {
        auto rq = dropRef(RunningQueryPlan::start(QueryId(0), std::move(q), controller, emitter, listener));
        RunningQueryPlan::dispose(std::move(rq));
    }
...
TEST_F(QueryPlanTest, RunningQueryPlanTestSourceSetup)
{
    ...
    std::unique_ptr<StoppingQueryPlan> stopping;
    {
        ...
        stopping = dropRef(RunningQueryPlan::stop(std::move(rq)));
    }

    EXPECT_TRUE(terminations->handle(test.stages.at(p)));
...
TEST_F(QueryPlanTest, RunningQueryPlanIndividualSourceTermination)
{
    ...
    auto p = builder.addPipeline({s1});
    auto p1 = builder.addPipeline({s, s1, s2});
    auto sink = builder.addSink({p, p1});
    auto q = test.addNewQuery(std::move(builder));
    auto srcCtrl = test.sourceControls[s];
    auto src1Ctrl = test.sourceControls[s1];
    auto src2Ctrl = test.sourceControls[s2];
...

    {
        ...
        rq->stopSource(test.sourceIds.at(s1));
        EXPECT_TRUE(src1Ctrl->waitUntilDestroyed());
        EXPECT_TRUE(src1Ctrl->wasClosed());

        EXPECT_TRUE(terminations->handle(test.stages.at(p)));
        /// No Further Terminations, as sink is kept alive by p1 which is kept alive by s, and s2
        EXPECT_THAT(*terminations, ::testing::IsEmpty());

        rq->stopSource(test.sourceIds.at(s));
        EXPECT_TRUE(srcCtrl->waitUntilDestroyed());
        EXPECT_TRUE(srcCtrl->wasClosed());
        ...
        rq->stopSource(test.sourceIds.at(s2));
        EXPECT_TRUE(src2Ctrl->waitUntilDestroyed());

        EXPECT_TRUE(terminations->handle(test.stages.at(p1)));
        EXPECT_TRUE(terminations->handle(test.stages.at(sink)));

        leak(std::move(rq), std::move(listener));
    }
}
...
TEST_F(QueryPlanTest, RunningQueryPlanTestPartialConstruction)
{
    ...
    auto s = builder.addSource();
    auto p = builder.addPipeline({s});
    auto p1 = builder.addPipeline({p});
    auto sink = builder.addSink({p1});
    auto q = test.addNewQuery(std::move(builder));
    auto srcCtrl = test.sourceControls[s];
    ...
    /// The RunningQueryPlan Requested Setup for all Pipelines in the QueryPlan.
    auto setups = Setups::setup(stdv::values(test.stages), emitter);
    auto terminations = Terminations::setup(std::vector{test.stages[p1]}, emitter);
    ...
    std::unique_ptr<StoppingQueryPlan> stopping;
    {
        ...
        EXPECT_TRUE(setups->waitForTasks(3));
        /// Only setup the p1 pipeline
        EXPECT_TRUE(setups->handle(test.stages[p1]));
        stopping = dropRef(RunningQueryPlan::stop(std::move(rq)));
    }

    EXPECT_TRUE(terminations->handle(test.stages[p1]));
    EXPECT_THAT(*terminations, ::testing::IsEmpty());
...
TEST_F(QueryPlanTest, RefCountTestSourceEoS)
{
    ...
    auto sourceStops = SourceStops::setup(std::vector{test.sourceIds.at(s)}, controller);
...

    {
        ...
        EXPECT_TRUE(setups->handleAll());
        EXPECT_TRUE(test.sourceControls[s]->waitUntilOpened());
        EXPECT_FALSE(test.sourceControls[s]->waitUntilClosed());

        test.sourceControls[s]->injectEoS();
        EXPECT_TRUE(sourceStops->handle(test.sourceIds.at(s)));
        EXPECT_TRUE(test.sourceControls[s]->waitUntilDestroyed());
        EXPECT_TRUE(test.sourceControls[s]->wasOpened());
        EXPECT_TRUE(test.sourceControls[s]->wasClosed());

        EXPECT_THAT(*terminations, ::testing::SizeIs(1));
        EXPECT_TRUE(terminations->handle(test.stages[p]));
        EXPECT_THAT(*terminations, ::testing::SizeIs(1));
...
TEST_F(QueryPlanTest, RefCountTestMultipleSourceOneOfThemEoS)
{
    ...
    auto s = builder.addSource();
    auto s1 = builder.addSource();
    auto p = builder.addPipeline({s, s1});
    auto sink = builder.addSink({p});
    auto q = test.addNewQuery(std::move(builder));
    ...
    /// The RunningQueryPlan Requested Setup for all Pipelines in the QueryPlan.
    /// No pipeline is terminated, because p is kept alive by s1
    auto sourceStops = SourceStops::setup(std::vector{test.sourceIds.at(s)}, controller);
    auto setups = Setups::setup(stdv::values(test.stages), emitter);
    {
        auto rq = dropRef(RunningQueryPlan::start(QueryId(0), std::move(q), controller, emitter, listener));
        ...
        test.sourceControls[s]->injectEoS();
        EXPECT_TRUE(test.sourceControls[s]->waitUntilClosed());
        EXPECT_TRUE(sourceStops->handle(test.sourceIds.at(s)));

        EXPECT_TRUE(test.sourceControls[s]->waitUntilDestroyed());
        EXPECT_FALSE(test.sourceControls[s1]->waitUntilClosed());
        leak(std::move(rq), std::move(listener));
    }
}
