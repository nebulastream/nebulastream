...
struct TestPipeline final : ExecutablePipelineStage
{
    ...
    void execute(const TupleBuffer& inputTupleBuffer, PipelineExecutionContext& pipelineExecutionContext) override
    {
        ...
        size_t maxRepeats = controller->repeatCount.load();
        if (maxRepeats > 0)
        {
            /// Get current repeat count from creation timestamp
            uint64_t currentRepeatCount = inputTupleBuffer.getWatermark().getRawValue();
            if (currentRepeatCount < maxRepeats)
            {
                ...
                pipelineExecutionContext.repeatTask(std::move(copiedBuffer), std::chrono::milliseconds(10));
                return;
            }
        }
...
public:
    ...
    void execute(const TupleBuffer& inputBuffer, PipelineExecutionContext& pipelineExecutionContext) override
    {
        ...
        {
            /// Get current repeat count from creation timestamp
            uint64_t currentRepeatCount = inputBuffer.getWatermark().getRawValue();
            if (currentRepeatCount < maxRepeats)
            {
