...
void TestingHarness::start()
{
    ...
    qm = std::make_unique<NES::Runtime::QueryEngine>(configuration, std::vector<std::shared_ptr<Runtime::QueryEngineStatisticListener>>{this->statListener}, this->status, this->bm);
}
