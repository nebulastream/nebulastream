...
createSinkPipeline(PipelineId id, Valve valve, std::shared_ptr<Memory::AbstractBufferProvider> bm)
{
...
QueryPlanBuilder::TestPlanCtrl QueryPlanBuilder::build(LocalQueryId queryId, std::shared_ptr<Memory::BufferManager> bm) &&
{
    ...
    for (auto source : objects | isSource)
    {
        ...
        auto [s, ctrl] = Sources::getTestSource(ingestion, std::get<SourceDescriptor>(source.second).sourceId, bm);
...
void TestingHarness::start()
{
    ...
    qm = std::make_unique<NES::QueryEngine>(configuration, this->statListener, this->status, this->bm, WorkerId("test"));
}
