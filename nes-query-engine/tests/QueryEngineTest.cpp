...
TEST_F(QueryEngineTest, singleQueryWithExternalStop)
{
    ...
    test.expectQueryStatusEvents(LocalQueryId(1), {QueryState::Started, QueryState::Running, QueryState::Stopped});
    test.expectSourceTermination(LocalQueryId(1), source, QueryTerminationType::Graceful);
    ...
    EXPECT_TRUE(ctrl->wasOpened());
    EXPECT_TRUE(ctrl->wasClosed());
    ASSERT_TRUE(test.waitForQepTermination(LocalQueryId(1), DEFAULT_LONG_AWAIT_TIMEOUT));
    test.stop();
...
TEST_F(QueryEngineTest, singleQueryWithSourceFailure)
{
    ...
    auto ctrl = test.sourceControls[source];
    auto sinkCtrl = test.sinkControls[sink];
    test.expectQueryStatusEvents(LocalQueryId(1), {QueryState::Started, QueryState::Running, QueryState::Failed});
    test.expectSourceTermination(LocalQueryId(1), source, QueryTerminationType::Failure);
...
TEST_F(QueryEngineTest, singleQueryWithTwoSourcesShutdown)
{
    ...
    auto sinkCtrl = test.sinkControls[sink];
    test.expectQueryStatusEvents(LocalQueryId(1), {QueryState::Started, QueryState::Running});
...
TEST_F(QueryEngineTest, singleQueryWithTwoSourcesWaitingForTwoStops)
{
    ...
    auto sinkCtrl = test.sinkControls[sink];
    test.expectQueryStatusEvents(LocalQueryId(1), {QueryState::Started, QueryState::Running, QueryState::Stopped});
    test.expectSourceTermination(LocalQueryId(1), source1, QueryTerminationType::Graceful);
    test.expectSourceTermination(LocalQueryId(1), source2, QueryTerminationType::Graceful);
...
TEST_F(QueryEngineTest, singleQueryWithManySources)
{
    ...
    std::ranges::transform(
        ...
        [&](auto identifier)
        {
            test.expectSourceTermination(LocalQueryId(1), identifier, QueryTerminationType::Graceful);
...
TEST_F(QueryEngineTest, singleQueryWithManySourcesOneOfThemFails)
{
    ...
    test.expectSourceTermination(LocalQueryId(1), sources[0], QueryTerminationType::Failure);
...
TEST_F(QueryEngineTest, ManyQueriesWithTwoSources)
{
    ...
    for (const auto& [index, _] : queryPlans | views::enumerate)
    {
        ...
        test.expectSourceTermination(LocalQueryId(1 + index), sources[index * 2], QueryTerminationType::Graceful);
        test.expectSourceTermination(LocalQueryId(1 + index), sources[(index * 2) + 1], QueryTerminationType::Graceful);
        test.expectQueryStatusEvents(LocalQueryId(1 + index), {QueryState::Started, QueryState::Running, QueryState::Stopped});
    }
    ...
    test.start();
    {
        ...
        auto queryIds = queryPlans
            | std::views::transform(
                            [&test](std::unique_ptr<ExecutableQueryPlan>& query) -> LocalQueryId
                            {
                                auto queryId = query->queryId;
                                test.startQuery(std::move(query));
                                return queryId;
                            })
            | std::ranges::to<std::vector<LocalQueryId>>();
...
TEST_F(QueryEngineTest, ManyQueriesWithTwoSourcesOneSourceFails)
{
    ...
    test.expectQueryStatusEvents(LocalQueryId(2), {QueryState::Started, QueryState::Running, QueryState::Stopped});
    ...
    test.start();
    {
        ...
        /// start all queries
        for (const LocalQueryId::Underlying qidGenerator = LocalQueryId::INITIAL; auto& query : queryPlans)
        {
            auto queryId = query->queryId;
            test.startQuery(std::move(query));
            ASSERT_TRUE(test.waitForQepRunning(queryId, DEFAULT_LONG_AWAIT_TIMEOUT));
        ...
        {
            EXPECT_FALSE(test.waitForQepTermination(LocalQueryId(queryId), std::chrono::milliseconds(10)));
            queryId++;
        }

        /// Internally stop Query 2 and wait for termination
        test.stopQuery(LocalQueryId(2));
        ASSERT_TRUE(test.waitForQepTermination(LocalQueryId(2), DEFAULT_LONG_AWAIT_TIMEOUT));
        ...
        {
            ASSERT_TRUE(test.waitForQepTermination(LocalQueryId(queryId), DEFAULT_LONG_AWAIT_TIMEOUT));
            queryId++;
        }
    }
...
TEST_F(QueryEngineTest, singleQueryWithSlowlyFailingSourceDuringEngineTermination)
{
    ...
    test.start();
    {
        ...
        ASSERT_TRUE(test.waitForQepRunning(LocalQueryId(1), DEFAULT_LONG_AWAIT_TIMEOUT));
    }
    test.stop();
}
...
TEST_F(QueryEngineTest, singleQueryWithSlowlyFailingSourceDuringQueryPlanTermination)
{
    ...
    test.start();
    {
        ...
        ASSERT_TRUE(test.waitForQepTermination(LocalQueryId(1), 2 * DEFAULT_LONG_AWAIT_TIMEOUT));
    }
    test.stop();
}
...
TEST_F(QueryEngineTest, ManyQueriesWithTwoSourcesAndPipelineFailures)
{
    ...
    sourcesCtrls.push_back(test.sourceControls[sources[1]]);
    sinkCtrls.push_back(test.sinkControls[sinks[0]]);
    test.expectSourceTermination(LocalQueryId(1), sources[0], QueryTerminationType::Graceful);
    test.expectSourceTermination(LocalQueryId(1), sources[1], QueryTerminationType::Graceful);
    test.expectQueryStatusEvents(LocalQueryId(1), {QueryState::Started, QueryState::Running, QueryState::Stopped});
    ...
    for (size_t index = 1; const auto& query : queryPlans | std::ranges::views::drop(1))
    {
        ...
        sourcesCtrls.push_back(test.sourceControls[sources[(index * 2) + 1]]);
        sinkCtrls.push_back(test.sinkControls[sinks[index]]);
        test.expectQueryStatusEvents(LocalQueryId(1 + index), {QueryState::Started, QueryState::Running, QueryState::Failed});
        index++;
    }
    ...
    test.start();
    {
        ...
        ASSERT_FALSE(test.waitForQepTermination(LocalQueryId(1), DEFAULT_AWAIT_TIMEOUT));
