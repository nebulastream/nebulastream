...
TEST_F(QueryEngineTest, singleQueryWithSystemShutdown)
{
    ...
    EXPECT_TRUE(verifyIdentifier(
        std::get<Memory::TupleBuffer>(test.bm->repinBuffer(std::move(buffers[0])).waitUntilDone()), NUMBER_OF_TUPLES_PER_BUFFER));
    test.stop();
...
TEST_F(QueryEngineTest, singleQueryWithManySources)
{
    ...
    test.start();
    {
        ...
        EXPECT_TRUE(sinkCtrl->waitForNumberOfReceivedBuffersOrMore(numberOfBuffersBeforeTermination));
        dataGenerator.stop();
        ...
        auto uniqueIdentifier
            ...
            | std::views::transform(
                  [&](auto& buffer)
                  {
                      ...
                      return readIdentifier(std::get<Memory::TupleBuffer>(test.bm->repinBuffer(std::move(floating)).waitUntilDone()));
                  })
            | ranges::to<std::unordered_set>();

        EXPECT_EQ(uniqueIdentifier.size(), numberOfBuffersBeforeTermination);

        test.waitForQepTermination(QueryId(1), DEFAULT_LONG_AWAIT_TIMEOUT);
    }
    test.stop();
}
