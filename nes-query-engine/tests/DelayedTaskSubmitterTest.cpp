...
protected:
    ...
    folly::Synchronized<std::vector<QueryId>> executionOrder;
...
TEST_F(DelayedTaskSubmitterTest, testBasicTaskSubmission)
{
    ...
    auto task = WorkTask(nextQueryId(), PipelineId(1), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
...
TEST_F(DelayedTaskSubmitterTest, testMultipleTasksOrderedExecution)
{
    ...
    auto submitter = DelayedTaskSubmitter(
        [this](Task task) noexcept
        {
            ...
            if (std::holds_alternative<WorkTask>(task))
            {
                ...
                executionOrder.wlock()->push_back(workTask.queryId);
            }
            submitTask(std::move(task));
        });

    /// Submit tasks with different delays
    const auto qid1 = nextQueryId();
    const auto qid2 = nextQueryId();
    const auto qid3 = nextQueryId();
    ...
    /// Check execution order
    {
        ...
        EXPECT_THAT(*order, ::testing::ElementsAre(qid1, qid2, qid3));
    }
}
...
TEST_F(DelayedTaskSubmitterTest, testDifferentDurationTypes)
{
    ...
    auto task1 = WorkTask(nextQueryId(), PipelineId(1), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
    auto task2 = WorkTask(nextQueryId(), PipelineId(2), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
    auto task3 = WorkTask(nextQueryId(), PipelineId(3), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
    ...
    for (int i = 0; i < numThreads; ++i)
    {
        threads.emplace_back(
            [&submitter, i]()
            {
                ...
                {
                    auto task = WorkTask(
                        nextQueryId(), PipelineId((i * tasksPerThread) + j), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
                    if (j % 2 == 0)
                    {
