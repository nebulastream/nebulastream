...
TEST_F(DelayedTaskSubmitterTest, testBasicTaskSubmission)
{
    ...
    auto task
        = WorkTask(QueryId(1), PipelineId(1), std::weak_ptr<RunningQueryPlanNode>(), Memory::TupleBuffer(), [] { }, [](Exception) { });
...
TEST_F(DelayedTaskSubmitterTest, testMultipleTasksOrderedExecution)
{
    ...
    auto task1
        = WorkTask(QueryId(1), PipelineId(1), std::weak_ptr<RunningQueryPlanNode>(), Memory::TupleBuffer(), [] { }, [](Exception) { });
    auto task2
        = WorkTask(QueryId(2), PipelineId(2), std::weak_ptr<RunningQueryPlanNode>(), Memory::TupleBuffer(), [] { }, [](Exception) { });
    auto task3
        = WorkTask(QueryId(3), PipelineId(3), std::weak_ptr<RunningQueryPlanNode>(), Memory::TupleBuffer(), [] { }, [](Exception) { });
...
TEST_F(DelayedTaskSubmitterTest, testDifferentTaskTypes)
{
    ...
    auto workTask
        = WorkTask(QueryId(1), PipelineId(1), std::weak_ptr<RunningQueryPlanNode>(), Memory::TupleBuffer(), [] { }, [](Exception) { });
    auto startPipelineTask = StartPipelineTask(QueryId(2), PipelineId(2), [] { }, [](Exception) { }, std::weak_ptr<RunningQueryPlanNode>());
    auto stopSourceTask = StopSourceTask(QueryId(3), std::weak_ptr<RunningSource>(), [] { }, [](Exception) { });
