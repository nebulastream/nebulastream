...
TEST_F(DelayedTaskSubmitterTest, testMultipleTasksOrderedExecution)
{
    ...
    const auto qid1 = QueryId{1};
    const auto qid2 = QueryId{2};
    const auto qid3 = QueryId{3};
    ...
    for (int i = 0; i < numThreads; ++i)
    {
        threads.emplace_back(
            [&submitter, i]()
            {
                ...
                {
                    auto task = WorkTask(
                        QueryId{1}, PipelineId((i * tasksPerThread) + j), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
                    if (j % 2 == 0)
                    {
    ...
    for (int threadId = 0; threadId < numThreads; ++threadId)
    {
        threads.emplace_back(
            [&submitter, threadId]()
            {
                ...
                {
                    auto task = WorkTask(
                        QueryId{1}, PipelineId((threadId * tasksPerThread) + i), std::weak_ptr<RunningQueryPlanNode>(), TupleBuffer(), {});
