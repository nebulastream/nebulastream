...
ArithmeticalBinaryExpressionNode::ArithmeticalBinaryExpressionNode(
    DataTypePtr stamp)
...
/**
 * @brief The current implementation of type inference for arithmetical
 * expressions expects that both operands of an arithmetical expression have
 * numerical stamps. If this is valid we derived the joined stamp of the left
 * and right operand. (e.g., left:int8, right:int32 -> int32)
...
void ArithmeticalBinaryExpressionNode::inferStamp(SchemaPtr schema) {
  ...
  if (!left->getStamp()->isNumeric() || !right->getStamp()->isNumeric()) {
    NES_THROW_RUNTIME_ERROR(
        "Error during stamp inference. Types need to be Numerical but Left "
        "was: {} Right was: {}",
        left->getStamp()->toString(), right->getStamp()->toString());
  }
  ...
  if (commonStamp->isUndefined()) {
    // the common stamp was not valid -> in this case the common stamp is
    // undefined.
    NES_THROW_RUNTIME_ERROR("{} is not supported by arithmetical expressions",
                            commonStamp->toString());
  }

  stamp = commonStamp;
  NES_TRACE(
      "ArithmeticalBinaryExpressionNode: we assigned the following stamp: {}",
      toString());
}
...
std::string ArithmeticalBinaryExpressionNode::toString() const {
