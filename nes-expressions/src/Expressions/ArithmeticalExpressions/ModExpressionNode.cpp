...
ModExpressionNode::ModExpressionNode(DataTypePtr stamp)
    : ArithmeticalBinaryExpressionNode(std::move(stamp)){};

ModExpressionNode::ModExpressionNode(ModExpressionNode *other)
    : ArithmeticalBinaryExpressionNode(other) {}

ExpressionNodePtr ModExpressionNode::create(const ExpressionNodePtr &left,
                                            const ExpressionNodePtr &right) {
  auto addNode = std::make_shared<ModExpressionNode>(
      DataTypeFactory::createFloat()); // TODO: stamp should always be float,
...
void ModExpressionNode::inferStamp(SchemaPtr schema) {
  ...
  if (stamp->isInteger()) {
    ...
    // e.g. when calculating MOD(..., -128) the result will always be in range
    // [-127, 127]. And no other INT8 divisor will yield a wider range than -128
    // (=INT8_MIN).
    int64_t range = std::max(std::abs(rightAsInt->lowerBound),
                             std::abs(rightAsInt->upperBound)) -
                    1;

    // further, we know that the result of MOD(X, ...) can not be larger/smaller
    // than X:
    int64_t newLowerBound = std::max(-range, leftAsInt->lowerBound);
    int64_t newUpperBound = std::min(range, leftAsInt->upperBound);

    stamp = DataTypeFactory::copyTypeAndTightenBounds(stamp, newLowerBound,
                                                      newUpperBound);
    ...
    if (leftStamp->isFloat()) {
      auto leftAsFloat =
          DataType::as<Float>(static_cast<DataTypePtr>(leftStamp));
      leftL = leftAsFloat->lowerBound;
      leftU = leftAsFloat->upperBound;
    } else if (leftStamp->isInteger()) {
      auto leftAsInteger =
          DataType::as<Integer>(static_cast<DataTypePtr>(leftStamp));
    ...
    if (rightStamp->isFloat()) {
      auto rightAsFloat =
          DataType::as<Float>(static_cast<DataTypePtr>(rightStamp));
      rightL = rightAsFloat->lowerBound;
      rightU = rightAsFloat->upperBound;
    } else if (rightStamp->isInteger()) {
      auto rightAsInteger =
          DataType::as<Integer>(static_cast<DataTypePtr>(rightStamp));
  ...
  // do nothing if the stamp is of type undefined (from
  // ArithmeticalBinaryExpressionNode::inferSchema(typeInferencePhaseContext,
  // schema);)
}
...
ExpressionNodePtr ModExpressionNode::copy() {
  return ModExpressionNode::create(children[0]->as<ExpressionNode>()->copy(),
