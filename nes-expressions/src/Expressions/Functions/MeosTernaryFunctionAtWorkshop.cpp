...
*/
class MeosTernaryFunctionAtWorkshop : public TernaryLogicalFunction {
  public:
    ...
    inferTernary(const DataTypePtr& longitude, const DataTypePtr& latitude, const DataTypePtr& timestamp) const override {
        if (!longitude->isNumeric() || !latitude->isNumeric() || !timestamp->isNumeric()) {
            NES_THROW_RUNTIME_ERROR("tpointatsworkshop can only be evaluated on numeric values.");
        }
...
[[maybe_unused]] const static LogicalFunctionRegistry::Add<MeosTernaryFunctionAtWorkshop> MeosTernaryFunctionAtWorkshop("tpointatsworkshop");
