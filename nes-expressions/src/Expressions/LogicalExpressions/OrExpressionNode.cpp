...
void OrExpressionNode::inferStamp(SchemaPtr schema)
{
    ...
    if (!getLeft()->isPredicate())
    {
        NES_THROW_RUNTIME_ERROR(
            "OR Expression Node: the stamp of left child must be boolean, but was: " + getLeft()->getStamp()->toString());
    }
    if (!getRight()->isPredicate())
    {
        NES_THROW_RUNTIME_ERROR(
            "OR Expression Node: the stamp of left child must be boolean, but was: " + getRight()->getStamp()->toString());
    }
}
