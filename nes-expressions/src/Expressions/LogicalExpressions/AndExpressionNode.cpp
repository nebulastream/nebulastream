...
AndExpressionNode::AndExpressionNode(AndExpressionNode *other)
...
void AndExpressionNode::inferStamp(SchemaPtr schema) {
  ...
  if (!getLeft()->isPredicate()) {
    NES_THROW_RUNTIME_ERROR("AND Expression Node: the stamp of left child must "
                            "be boolean, but was: " +
...
ExpressionNodePtr AndExpressionNode::copy() {
  return AndExpressionNode::create(children[0]->as<ExpressionNode>()->copy(),
