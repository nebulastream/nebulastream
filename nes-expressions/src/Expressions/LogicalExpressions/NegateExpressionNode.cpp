...
NegateExpressionNode::NegateExpressionNode(NegateExpressionNode *other)
    : LogicalUnaryExpressionNode(other) {}
...
void NegateExpressionNode::inferStamp(SchemaPtr schema) {
  ...
  if (!child()->isPredicate()) {
    NES_THROW_RUNTIME_ERROR("Negate Expression Node: the stamp of child must "
                            "be boolean, but was: " +
                            child()->getStamp()->toString());
  }
}
ExpressionNodePtr NegateExpressionNode::copy() {
  return NegateExpressionNode::create(
