...
namespace NES {
WhenExpressionNode::WhenExpressionNode(DataTypePtr stamp)
    : BinaryExpressionNode(std::move(stamp)){};

WhenExpressionNode::WhenExpressionNode(WhenExpressionNode *other)
    : BinaryExpressionNode(other) {}

ExpressionNodePtr WhenExpressionNode::create(const ExpressionNodePtr &left,
...
void WhenExpressionNode::inferStamp(SchemaPtr schema) {
  ...
  if (!left->getStamp()->isBoolean()) {
    NES_THROW_RUNTIME_ERROR("Error during stamp inference. Left type needs to "
                            "be Boolean, but Left was: {} Right was: {}",
  ...
  NES_TRACE("WhenExpressionNode: we assigned the following stamp: {}",
            stamp->toString())
}
...
bool WhenExpressionNode::equal(NodePtr const &rhs) const {
  if (rhs->instanceOf<WhenExpressionNode>()) {
    ...
    return getLeft()->equal(otherWhenNode->getLeft()) &&
           getRight()->equal(otherWhenNode->getRight());
  }
  return false;
}
...
ExpressionNodePtr WhenExpressionNode::copy() {
  ...
  return WhenExpressionNode::create(children[0]->as<ExpressionNode>()->copy(),
