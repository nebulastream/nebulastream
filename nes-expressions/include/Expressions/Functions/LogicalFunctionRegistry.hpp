...
class LogicalFunction {
 public:
   ...
   * @return DataTypePtr
   */
  [[nodiscard]] virtual DataTypePtr inferStamp(
      const std::vector<DataTypePtr>& inputStamps) const = 0;
  virtual ~LogicalFunction() = default;
};

class UnaryLogicalFunction : public LogicalFunction {
 public:
  [[nodiscard]] DataTypePtr inferStamp(
      const std::vector<DataTypePtr>& inputStamps) const final;
  [[nodiscard]] virtual DataTypePtr inferUnary(
      const DataTypePtr& input) const = 0;
};
...
class BinaryLogicalFunction : public LogicalFunction {
 public:
  [[nodiscard]] DataTypePtr inferStamp(
      const std::vector<DataTypePtr>& inputStamps) const final;
  [[nodiscard]] virtual DataTypePtr inferBinary(
      const DataTypePtr& left, const DataTypePtr& right) const = 0;
};
