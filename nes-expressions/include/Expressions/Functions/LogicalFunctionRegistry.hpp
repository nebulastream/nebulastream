...
public:
   ...
   * @return DataTypePtr
   */
  [[nodiscard]] virtual DataTypePtr
  inferStamp(const std::vector<DataTypePtr> &inputStamps) const = 0;
...
public:
  ...
  inferStamp(const std::vector<DataTypePtr> &inputStamps) const final;
  [[nodiscard]] virtual DataTypePtr
  inferUnary(const DataTypePtr &input) const = 0;
};
...
public:
  ...
  inferStamp(const std::vector<DataTypePtr> &inputStamps) const final;
  [[nodiscard]] virtual DataTypePtr
  inferBinary(const DataTypePtr &left, const DataTypePtr &right) const = 0;
};
