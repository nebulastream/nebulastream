...
public:
  ...
  create(const DataTypePtr &stamp, const std::string &functionName,
         const std::vector<ExpressionNodePtr> &arguments);

  /**
   * @brief On a function value expression infer stamp invokes inferStamp on the
   * child LogicalFunction
  ...
  explicit FunctionExpression(DataTypePtr stamp, std::string functionName,
