...
namespace NES {
/**
 * @brief A Ternary expression is represents expressions with three children.
 */
class TernaryExpressionNode : public ExpressionNode {
  public:
    ~TernaryExpressionNode() noexcept override = default;

    /**
     * @brief set the children node of this expression.
     */
    void setChildren(ExpressionNodePtr const& left, ExpressionNodePtr const& middle, ExpressionNodePtr const& right);
  ...
  protected:
    explicit TernaryExpressionNode(DataTypePtr stamp);
    explicit TernaryExpressionNode(TernaryExpressionNode* other);
};
