...
{
    /// Directory that a test name is relative to. Fixed at build time.
    std::string testDiscoverRoot = TEST_DISCOVER_DIR;
    /// Directories to lookup test files in. Default: the discovery root; replaced when directories are passed via -t/--testLocations.
    std::vector<std::string> testDiscoverDirs;
