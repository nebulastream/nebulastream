...
/// need, so it holds its printed plan and has none to run.
...
public:
    ...
    /// Stages the data that this file's sources read, writes its setup statements into the shared catalogs, and plans its cases.
    ...
    [[nodiscard]] PlannedTest plan(const RunnableTestFile& runnable);
