...
public:
    ...
    /// Takes the declaration, because the attached rows move into that CSV plan or the served data.
...
private:
    ...
    /// The clause pins the source to one worker unless the test chose one, so it has a worker to be placed on.
...
/// It pins the source to the run's source worker, because an omitted host resolves to the worker that answers, and a
/// run placed on a topology answers none, so such a source would have nowhere to run.
