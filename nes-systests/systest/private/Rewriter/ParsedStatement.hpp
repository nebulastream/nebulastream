...
/// One parse of a SQL statement, and the queries the rewriter runs against it.
/// Everything here reads the statement the test file wrote.
...
/// The lexer puts whitespace on a hidden channel, so rendering preserves every part the rewriter did not touch.
class ParsedStatement
{
public:
    explicit ParsedStatement(const std::string& statement)
...
std::vector<Context*> findAll(antlr4::tree::ParseTree* node)
{
    ...
    findAll<Context>(node, found);
    return found;
}

/// Returns whether a config option has exactly the given qualified name.
/// Both names are canonicalized the way the binder does, so a quoted and an unquoted spelling of one name match.
...
inline bool namesOption(AntlrSQLParser::NamedConfigExpressionContext* option, const std::string_view group, const std::string_view key)
{
    ...
    return parts.size() == 2 and Identifier::parse(parts.at(0)->getText()) == Identifier::parse(std::string{group})
        and Identifier::parse(parts.at(1)->getText()) == Identifier::parse(std::string{key});
}

/// Returns whether an option list sets the given name.
/// The rewriter keeps a value the test chose instead of writing its own default.
...
/// Returns the string a config option is set to, or nullopt when the list does not set it or sets it to something other than a
/// string literal.
/// The caller needs the value the test chose, not only whether it chose one.
