...
@test "large scale tests on two nodes" {
  ...
  # --testLocations replaces the default discover directory, so the core test tree is listed alongside the plugin tree.
  run docker_systest -g large -e tcp "${EXTRA_EXCLUDE_GROUPS[@]}" --testLocations $NES_DIR/nes-systests $NES_DIR/nes-plugins --clusterConfig $NES_DIR/nes-systests/configs/topologies/two-node.yaml --remote
  [ "$status" -eq 0 ]
}
