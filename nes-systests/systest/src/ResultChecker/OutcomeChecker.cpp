...
/// Checks the plan that an EXPLAIN printed, which is computed while compiling because an EXPLAIN never reaches a worker.
...
Verdict checkExplained(const StatementOutcome& outcome, const ExpectedPlan& expected, const std::string_view qualifyingPrefix)
{
    if (not outcome.explained.has_value())
    {
        return checkFailed(failureOf(outcome), Expectation{expected});
    }
    auto actual = unqualified(*outcome.explained, qualifyingPrefix);
...
Verdict checkCase(const std::span<const StatementOutcome> outcomes, const RewrittenCase& testCase, const std::string_view qualifyingPrefix)
{
