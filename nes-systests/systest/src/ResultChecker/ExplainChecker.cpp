...
{
    return fmt::format("Invalid Explain Regex Assertion (line {}): {}", line + 1, message);
}

std::expected<ExplainRegexAssertion, std::string>
parseSingleLineExplainRegex(const std::string_view line, const ExplainRegexTags& tags, size_t& expectedLineIndex)
{
    ...
    for (const auto& [pattern, shouldMatch, line] : assertions)
    {
        ...
        catch (const std::regex_error& exception)
        {
            return std::unexpected(
                Mismatch{fmt::format("Invalid Explain Output Regex (line {}, pattern \"{}\"): {}", line + 1, pattern, exception.what())});
        }
    }
    return Success{};
}
...
/// Both sides are normalized the same way: only the end of a line is trimmed, because indentation matters, and blank
/// lines are dropped, because a blank line ends an expected block in the test file and so can never be expected.
/// Takes the range by forwarding reference, because a split view is not const iterable.
...
bool hasExplainRegexTags(const std::vector<std::string>& expected)
{
    return std::ranges::any_of(expected, [](const auto& line) { return containsExplainRegexTag(line); });
}
...
Verdict ExplainRegexCheck::check() const
{
    auto assertions = parseExplainRegexAssertions(normalize(expected));
    if (!assertions)
    {
