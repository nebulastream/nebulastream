...
/// Splits the plan the coordinator printed into trimmed lines.
...
Verdict checkExplain(const ExplainCheck& check)
{
    const auto expected = normalize(check.expected);
    const auto actual = normalizedLinesOf(check.actualOutput);
    ...
    return reportFirstDifference(expected, actual);
}
