                ...
                else
                {
                    processQueryWithError(
                        ...
                        DistributedException(
                            std::unordered_map<GrpcAddr, std::vector<Exception>>{{GrpcAddr("systest"), std::vector{reg.error()}}}),
            ...
            else
            {
                ...
                processQueryWithError(
                    ...
                    DistributedException(std::unordered_map<GrpcAddr, std::vector<Exception>>{
                        {GrpcAddr("systest"), std::vector{nextQuery.planInfoOrException.error()}}}),
                    queryPerformanceMessage);
            }
        }
    ...
    SystestProgressTracker& progressTracker)
{
    ...
    catalog->addWorker(HostAddr("localhost:9090"), GrpcAddr("localhost:8080"), INFINITE_CAPACITY, {});
...
{
    auto catalog
        = std::make_shared<WorkerCatalog>();
    catalog->addWorker(HostAddr("localhost:9090"), GrpcAddr(serverURI), INFINITE_CAPACITY, {});
