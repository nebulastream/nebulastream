...
RewrittenTest Emitter::emit(std::vector<PreparedStatement>& prepared) &&
{
        ...
        std::visit(
            Overloaded{
                ...
                [this](const Systest::QueryStatement* query)
                {
                    ...
                    runnable.cases.push_back(RewrittenCase{.action = std::move(rewritten), .runsAfterPrevious = query->sequential});
                },
    ...
    std::visit(
        Overloaded{
            ...
            [&](const CreateSinkStatement& declaration)
            {
                ...
                if (declaresSinks)
                {
                    runnable.createStmts.push_back(RewrittenCreateStatement{
...
RewrittenCreateStatement Emitter::modelStatement(ParsedStatement& parsed, const CreateModelStatement& declaration) const
{
    ...
    return RewrittenCreateStatement{.sql = rewriteIdentifiers(rewriter.getText(), names), .staged = std::nullopt};
}
