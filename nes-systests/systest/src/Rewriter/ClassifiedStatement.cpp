...
/// Binds attached data only to a physical source.
...
CreateDeclaration classify(const SqlParse& parse, CreateStatement create)
{
    if (auto* physicalSource = findFirst<AntlrSQLParser::CreatePhysicalSourceDefinitionContext>(parse.tree()))
    {
        return PhysicalSourceDeclaration{.definition = physicalSource, .attached = std::move(create.attach)};
    }
...
ClassifiedTestFile classifyStatements(ParsedTestFile testFile)
{
    ClassifiedTestFile classified;
    for (auto& statement : testFile.statements)
    {
        std::visit(
            Overloaded{
                [&](CreateStatement&& create)
                {
                    ...
                    auto declaration = classify(*parse, std::move(create));
                    classified.setup.push_back(ClassifiedCreate{.parse = std::move(parse), .declaration = std::move(declaration)});
                },
                [&](SelectStatement&& query) { classified.testCases.emplace_back(std::move(query)); },
                [&](DifferentialStatement&& block) { classified.testCases.emplace_back(std::move(block)); },
                [&](ExplainStatement&& explain)
                {
                    classified.containsExplain = true;
                    classified.testCases.emplace_back(std::move(explain));
                }},
            std::move(statement));
    }
    return classified;
