    ...
    for (auto& statement : testFile.statements)
    {
        std::visit(
            Overloaded{
                ...
                [&](SelectStatement&& query) { classified.cases.emplace_back(std::move(query)); },
                [&](DifferentialStatement&& block) { classified.cases.emplace_back(std::move(block)); },
                [&](ExplainStatement&& explain)
                {
                    classified.containsExplain = true;
                    classified.cases.emplace_back(std::move(explain));
                }},
            std::move(statement));
    }
    return classified;
