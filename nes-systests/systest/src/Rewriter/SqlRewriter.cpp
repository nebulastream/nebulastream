...
RunnableTestFile rewriteTestFile(ParsedTestFile testFile, const RewriteTarget& target)
{
    auto classified = classifyStatements(std::move(testFile));
    auto declarations = declareAll(classified, target.testFileKey);
    return Emitter{target, std::move(declarations)}.emit(std::move(classified));
}
