...
InlinedSink
inlineDeclaredSink(const SinkDefinition& declaration, const std::filesystem::path& candidateResultFile, const RewriteTarget& target)
{
    ...
    /// A declared sink writes CSV, which is the format the rewriter chooses for it.
    return {.sql = Sql::sink(declaration.type, Sql::optionList(options)), .resultFile = std::move(resultFile), .rowsAreJson = false};
}

/// Inlines a sink the query wrote itself, keeping it anonymous and keeping its options as the test wrote them.
    ...
    const RewriteTarget& target)
{
    if (auto* sinkName = sink->identifier(); sinkName != nullptr)
    {
        ...
        return inlineDeclaredSink(declaration->second, candidateResultFile, target);
    }
    if (const auto* anonymous = sink->anonymousSink(); anonymous != nullptr)
    {
        return inlineQuerySink(parsed, anonymous, candidateResultFile, target);
    }
    throw TestException(
