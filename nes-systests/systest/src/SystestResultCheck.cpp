...
using ExpectedResultSchema = ResultCheckStrongType<NES::UnboundOrderedSchema, struct ExpectedResultSchema_>;
using ActualResultSchema = ResultCheckStrongType<NES::UnboundOrderedSchema, struct ActualResultSchema_>;
...
NES::UnboundOrderedSchema parseFieldNames(const std::string_view fieldNamesRawLine)
{
    ...
    return NES::UnboundOrderedSchema{fields | std::ranges::to<std::vector>()};
}

struct QueryResult
{
    NES::UnboundOrderedSchema schema;
