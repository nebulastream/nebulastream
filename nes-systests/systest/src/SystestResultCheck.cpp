...
NES::Schema<NES::UnqualifiedUnboundField, NES::Ordered> parseFieldNames(const std::string_view fieldNamesRawLine)
{
    ...
    auto fields
        ...
        {
            const auto inner = typeTrimmed.substr(std::string_view("VECTOR<").size(),
                typeTrimmed.size() - std::string_view("VECTOR<").size() - 1);
            ...
            dataType = NES::DataType{NES::DataType::Type::VECTOR, isNullable, elementType.value()};
        }
