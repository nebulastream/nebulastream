...
ExpectedToActualFieldMap compareSchemas(const ExpectedResultSchema& expectedResultSchema, const ActualResultSchema& actualResultSchema)
{
    ...
    const bool schemasMatch = std::ranges::equal(
        expectedResultSchema.getRawValue(),
        actualResultSchema.getRawValue(),
        fieldsMatchCaseInsensitive);
    if (!schemasMatch)
    {
    ...
    {
        if (const auto& matchingFieldIt
                = std::ranges::find_if(actualResultSchema.getRawValue(), [&](const auto& f) { return fieldsMatchCaseInsensitive(expectedField, f); });
