...
bool compareStringAsTypeWithError(const NES::DataType::Type type, const ExpectedResultField& left, const ActualResultField& right)
{
    ...
    switch (type)
    {
        ...
        case NES::DataType::Type::VECTOR:
            /// FIXEDSIZED and VECTOR render as a JSON array literal `[v0,v1,...]` from the JSON
...
NES::Schema<NES::UnqualifiedUnboundField, NES::Ordered> parseFieldNames(const std::string_view fieldNamesRawLine)
{
    ...
    auto fields
        ...
        else if (typeTrimmed.starts_with("VECTOR<") && typeTrimmed.ends_with('>'))
        {
            const auto inner
                = typeTrimmed.substr(std::string_view("VECTOR<").size(), typeTrimmed.size() - std::string_view("VECTOR<").size() - 1);
            const auto elementTypeStr = NES::trimWhiteSpaces(inner);
            const auto elementType = magic_enum::enum_cast<NES::DataType::Type>(elementTypeStr);
            if (not elementType.has_value())
            {
                throw NES::SLTUnexpectedToken("Unknown VECTOR element type: {}", elementTypeStr);
            }
            dataType = NES::DataType{
                NES::DataType::Type::VECTOR, isNullable, NES::DataType{elementType.value(), NES::DataType::NULLABLE::NOT_NULLABLE}};
        }
