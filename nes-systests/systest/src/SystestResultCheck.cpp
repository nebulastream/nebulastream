...
NES::Schema parseFieldNames(const std::string_view fieldNamesRawLine)
{
    ...
    /// window$val_i8_i8:INT32:TRUE, window$val_i8_i8_plus_1:INT16:False
    NES::Schema schema;
             ...
             | std::views::filter([](const auto& stringViewSplit) { return !stringViewSplit.empty(); }))
    {
            ...
            = [](const std::string_view field) -> std::tuple<std::string_view, std::string_view, bool>
        {
            ...
            INVARIANT(fieldAndTypeVector.size() == 3, "Field and type pairs should always be pairs of a key and a value");

            const auto isNullableString = fieldAndTypeVector.at(2);
            bool isNullable = false;
            if (NES::Util::toLowerCase(isNullableString) == "true")
            {
                isNullable = true;
            }
            else if (NES::Util::toLowerCase(isNullableString) == "false")
            {
                isNullable = false;
            }
            else
            ...
            return std::make_tuple(fieldAndTypeVector.at(0), fieldAndTypeVector.at(1), isNullable);
        }(field);
