...
DiscoveryFilters createDiscoveryFilters(const NES::Config& config)
{
...
/// Names each file by its path relative to `keyRoot`, so a file keeps the same name however the run narrowed the search.
...
DiscoveredTestFiles discoverTestFiles(const Config& config)
{
    ...
    const std::filesystem::path keyRoot{config.testsDiscoverDir.getValue()};
    const auto searchRoots = config.searchDirs.empty() ? std::vector<std::filesystem::path>{keyRoot} : config.searchDirs;
    DiscoveredTestFiles testFiles;
