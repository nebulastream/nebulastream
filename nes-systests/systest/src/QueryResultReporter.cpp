...
void QueryResultReporter::reportFailure(
    const SystestQueryContext& ctx, const std::string& errorMessage, const std::string_view performanceMessage)
{
    printQueryResultToStdOut(ctx, errorMessage, finishedCount++, totalQueries, performanceMessage, false);
}

void QueryResultReporter::reportExpectedError(
    const SystestQueryContext& ctx, const ExpectedError& expectedError, const Exception& actualException)
{
