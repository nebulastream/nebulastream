...
Schedule::File::File(const size_t runnable, const size_t offset, const RunnableTest& test)
...
Schedule::Schedule(const std::vector<std::reference_wrapper<const RunnableTest>>& tests)
{
    files.reserve(tests.size());
    for (const RunnableTest& test : tests)
    {
