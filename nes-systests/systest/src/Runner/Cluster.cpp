...
Cluster::Cluster(const Bridge::Coordinator& coordinator, const Settings& settings)
    ...
    , baseSettings{settings.workerSettings}
{
    ...
    if (mode == Bridge::WorkerMode::Remote and settings.topology.workers.empty())
    {
    ...
    INVARIANT(mode == Bridge::WorkerMode::Remote or not defaultHost.view().empty(), "the coordinator answers no default host");
...
Placement Cluster::registerTopology(const ClusterConfiguration& topology, const Host& fallback) const
{
    ...
    for (const auto& worker : topology.workers)
    {
        ...
        Bridge::raiseReported(
            coordinator.get().submit(rust::Str{statement.data(), statement.size()}, Bridge::WaitMode::None, 0, false).error);
    }
...
const Host& Cluster::registerWorker(const ConfigurationOverride& settings)
{
    ...
    Bridge::raiseReported(coordinator.get().submit(rust::Str{statement.data(), statement.size()}, Bridge::WaitMode::None, 0, false).error);
