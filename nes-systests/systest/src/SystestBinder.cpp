...
public:
    ...
    bool registerSink(const Identifier& sinkType, Identifier sinkNameInFile, const Schema<UnqualifiedUnboundField, Ordered>& schema)
    {
...
struct SystestBinder::Impl
{
        ...
        const TypedLogicalOperator<InlineSinkLogicalOperator>& sinkOperator) const
    {
        ...
        if (not(sinkConfig.contains(Identifier::parse("input_format"))) and sinkOperator->getSinkType() != Identifier::parse("CHECKSUM"))
        {
