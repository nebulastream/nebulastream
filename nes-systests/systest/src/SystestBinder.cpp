...
struct SystestBinder::Impl
{
    ...
    [[nodiscard]] PhysicalSourceConfig setUpSourceWithTestData(
        const PhysicalSourceConfig& physicalSourceConfig,
        const std::shared_ptr<std::vector<std::jthread>>& sourceThreads,
        const std::pair<TestDataIngestionType, std::vector<std::string>>& testData) const
    {
        switch (testData.first)
        {
            case TestDataIngestionType::INLINE: {
                const auto testFile = generateSourceFilePath();
                return SourceDataProvider::provideInlineDataSource(physicalSourceConfig, testData.second, sourceThreads, testFile);
            }
            case TestDataIngestionType::FILE: {
                ...
                return SourceDataProvider::provideFileDataSource(physicalSourceConfig, sourceThreads, testFilePath);
            }
            default:
                std::unreachable();
        }
    ...
    void createPhysicalSource(
        ...
        const std::optional<std::pair<TestDataIngestionType, std::vector<std::string>>>& testData) const
    {
        ...
        const std::optional<std::pair<TestDataIngestionType, std::vector<std::string>>>& testData) const
    {
        ...
        {
            createPhysicalSource(sourceCatalog, sourceThreads, std::get<CreatePhysicalSourceStatement>(statement), testData);
        }
        ...
        parser.registerOnDifferentialQueryBlockCallback(
                ...
                const SystestQueryId)
            {
                differentialQueryBlocksCallback(
                    lastParsedQueryId, testFileName, plans, sltSinkProvider, leftQuery, rightQuery, currentQueryNumberInTest);
            });

        parser.registerOnCreateCallback(
            [&](const std::string& query, const std::optional<std::pair<TestDataIngestionType, std::vector<std::string>>>& input)
