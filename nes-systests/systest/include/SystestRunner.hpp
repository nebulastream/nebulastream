...
[[nodiscard]] std::vector<RunningQuery> runQueries(
    const std::vector<SystestQuery>& queries, uint64_t numConcurrentQueries, QuerySubmitter& querySubmitter, uint64_t timeoutInSeconds = 0);
...
[[nodiscard]] std::vector<RunningQuery> runQueriesAtRemoteWorker(
    const std::vector<SystestQuery>& queries, uint64_t numConcurrentQueries, const std::string& serverURI, uint64_t timeoutInSeconds);
