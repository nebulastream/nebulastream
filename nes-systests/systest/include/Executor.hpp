...
/// Runs every discovered test file and counts the queries that passed.
class Executor
{
public:
...
private:
    ...
    struct PreparedRun
    {
        std::vector<RewrittenTest> ready;
        /// The settings of each ready test file, in the same order.
    ...
    /// settings at startup.
    [[nodiscard]] static std::expected<std::vector<RewrittenTest>, CheckedQuery>
    ...
    [[nodiscard]] static ExecutorResult runOnce(TestRunner& runner, const RunPlan& plan, PreparedRun prepared);
    ...
    [[nodiscard]] static ExecutorResult runRounds(TestRunner& runner, const RunPlan& plan, PreparedRun prepared);
