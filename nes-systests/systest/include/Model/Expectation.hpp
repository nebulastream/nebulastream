...
/// The lines a query should produce, one string per line: the output tuples of a query, or the plan of an `EXPLAIN`.
using ExpectedResult = std::vector<std::string>;

/// The rows a query should produce, and how to read the file they are written to.
struct ExpectedRows
{
    ExpectedResult rows;
    /// Whether each row of the result file is one JSON object, which the checker reads whole rather than splitting into fields.
    /// The rewriter learns this from the sink it inlines, so a parsed test file always carries false here.
    bool rowsAreJson = false;
};

/// The error a query should fail with: the error code, and optionally a message to match against.
...
/// The plan an `EXPLAIN` should print, and the prefix to strip from it before comparing.
