...
/// A case that ran and answered what the test expects.
struct Passed
{
};

/// A case that never ran because something it needed failed, and the reason why.
/// Reported rather than dropped, so the total stays the number of cases the run selected.
struct Skipped
{
    std::string reason;
};
...
/// Folds a checker's verdict into the reported state of a case.
...
/// The execution is the span the coordinator recorded between the query starting and stopping, which a measurement wants.
struct QueryTiming
{
