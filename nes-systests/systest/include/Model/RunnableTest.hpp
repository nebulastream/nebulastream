...
/// The CSV file an `ATTACH INLINE` physical source reads, and the rows the block declared for it.
...
/// The data for a source that reads from a socket: the rows the test declared, or the file it pointed at.
...
/// The data the runner puts in place before submitting a setup statement.
...
struct RunnableCreateStatement
{
    std::string sql;
    std::optional<StagedData> staged;
};

/// One query to submit, the result file its sink writes, and the answer the test expects in that file.
/// A query whose sink discards its input writes no file, so the result file is absent and there is nothing to compare.
/// The id is the number the test file gave the query, so a reported result points back at it.
struct RunnableQuery
{
    std::string sql;
    SystestQueryId id;
    std::optional<std::filesystem::path> resultFile;
    /// The data files the sources this query reads were pointed at, once per reference, so a query that joins a source
...
struct RunnableDifferential
{
    ...
    /// The data files the sources of both halves read, once per reference.
    std::vector<std::filesystem::path> inputFiles;
};

/// One case of a test file: what to submit and check, and where it stands in the file's order.
/// A case is the unit the scheduler hands out and the unit that gets one verdict.
struct RunnableCase
{
    std::variant<RunnableQuery, RunnableDifferential> action;
...
/// The number a case reports under, which for a differential block is the number the test file shows for the block.
inline SystestQueryId caseNumber(const RunnableCase& testCase)
{
    if (const auto* query = std::get_if<RunnableQuery>(&testCase.action))
    {
        return query->id;
    }
    return std::get<RunnableDifferential>(testCase.action).firstId;
}
...
struct RunnableTest
{
    /// The file name a failure is reported under.
    std::string name;
    /// Which settings part of the file this is, counted from zero, because every part reports the same name.
    uint32_t variant = 0;
    std::vector<RunnableCreateStatement> createStmts;
    std::vector<RunnableCase> cases;
};
