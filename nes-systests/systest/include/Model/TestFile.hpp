...
/// The data an `ATTACH` clause supplies for a physical source: inline rows, or a single element holding a path.
struct AttachedData
{
    TestDataIngestionType kind;
    std::vector<std::string> rows;
};

namespace Systest
{
/// One source or sink DDL statement, and the data an `ATTACH` clause supplied for a physical source.
...
/// One query and the answer the test expects: the result rows, or an error it should raise.
/// The settings are the ones that applied where the test wrote the query, and they decide which worker runs it.
...
/// One `EXPLAIN` and the plan text the test expects it to print.
