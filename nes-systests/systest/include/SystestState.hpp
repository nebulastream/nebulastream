...
struct RunningQuery
{
    ...
    std::chrono::duration<double> getElapsedTime() const
    {
        ...
        if (not lastRun.stop.has_value() || not lastRun.running.has_value())
        {
            throw QueryInvalid("Query {} has no querySummary timestamps!", queryId);
        }
    ...
    [[nodiscard]] std::string getThroughput() const
    {
        ...
        const auto lastRun = querySummary.runs.back();
        if (not lastRun.stop.has_value() or not lastRun.running.has_value())
        {
            throw QueryInvalid("Query {} has no querySummary timestamps!", queryId);
        }
