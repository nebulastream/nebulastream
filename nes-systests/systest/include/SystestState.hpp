...
struct SystestQuery
{
    ...
    struct PlanInfo
    {
        ...
        UnboundOrderedSchema sinkOutputSchema;

        PlanInfo() = delete;

        PlanInfo(LogicalPlan plan, std::unordered_map<SourceDescriptor, std::pair<SourceInputFile, uint64_t>> sources, UnboundOrderedSchema sinkSchema)
            : queryPlan(std::move(plan)), sourcesToFilePathsAndCounts(std::move(sources)), sinkOutputSchema(std::move(sinkSchema))
        {
        }

        PlanInfo(LogicalPlan plan, UnboundOrderedSchema sinkSchema) : queryPlan(std::move(plan)), sinkOutputSchema(std::move(sinkSchema)) { }
