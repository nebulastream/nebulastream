...
struct SystestQuery
{
    ...
    struct PlanInfo
    {
        ...
        PlanInfo(LogicalPlan plan, std::unordered_map<SourceDescriptor, std::pair<SourceInputFile, uint64_t>> sources, Schema<UnqualifiedUnboundField, Ordered> sinkSchema)
            : queryPlan(std::move(plan)), sourcesToFilePathsAndCounts(std::move(sources)), sinkOutputSchema(std::move(sinkSchema))
        {
        }

        PlanInfo(LogicalPlan plan, Schema<UnqualifiedUnboundField, Ordered> sinkSchema) : queryPlan(std::move(plan)), sinkOutputSchema(std::move(sinkSchema)) { }
