...
/// Derives the key one part of a test file qualifies its catalog entries with.
/// A file needing only one worker keeps its own key, so the SQL it emits does not change.
...
/// The key type compares and hashes by canonical form, so two unquoted spellings differing only in case share one entry.
using QualifiedByName = std::unordered_map<Identifier, std::string>;
/// Maps a qualified spelling back to the spelling it was registered under.
using OriginalByQualified = std::unordered_map<std::string, std::string>;

/// The catalog-visible names of one test file and the spelling each got, fixed once the registry has seen every name.
...
public:
    ...
    /// Returns the spelling a qualified name was registered under, or nullopt when this never qualified it.
    /// Failure output prints the name the test author wrote.
...
/// Sealing hands the finished names to the rewriter and spends the registry, so no caller can add a name once rewriting started.
class NameRegistry
{
public:
    ...
    /// Returns the qualified spelling of a name, registering it on first sight.
    ...
    /// Rvalue-qualified, so the call site shows that the registry is spent.
