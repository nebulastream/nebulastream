...
RunnableTest fileOf(const std::vector<bool>& runsAfterPrevious)
{
    RunnableTest test;
    for (const bool waits : runsAfterPrevious)
    {
        test.cases.push_back(RunnableCase{
            .action
            = RunnableQuery{.sql = "SELECT 1", .id = SystestQueryId{1}, .resultFile = std::nullopt, .inputFiles = {}, .expectation = ExpectedRows{}},
            .runsAfterPrevious = waits});
    }
    return test;
}

std::vector<std::reference_wrapper<const RunnableTest>> allOf(const std::vector<RunnableTest>& tests)
{
