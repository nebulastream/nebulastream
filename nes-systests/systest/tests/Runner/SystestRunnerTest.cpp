...
/// An error code that no test expects, so a failure with it is always unexpected.
constexpr uint64_t UnexpectedErrorCode = 10000;
...
TEST_F(SystestRunnerTest, RuntimeFailureWithUnexpectedCode)
{
    ...
    const auto runtimeErr = std::make_shared<Exception>(Exception{"runtime boom", UnexpectedErrorCode});
...
TEST_F(SystestRunnerTest, StopsTheFirstHalfWhenTheSecondHalfOfADifferentialPairDoesNotStart)
{
    ...
    auto [submitter, mockBackend] = createQuerySubmitter();
    /// Starting polls the status until the query leaves Registered, so the status is returned outside the start-stop sequence.
    ...
    EXPECT_CALL(*mockBackend, start(::testing::_))
        ...
        .WillOnce(testing::Return(std::unexpected{Exception{"second half rejected", UnexpectedErrorCode}}));
