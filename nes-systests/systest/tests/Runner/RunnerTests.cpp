...
public:
    ...
    static RunnableTestFile rewriteOneTuple(TestRunner& runner, const std::filesystem::path& workingDir, const std::string& name)
    {
        ...
        runnable.key = key.value();
        return runnable;
    }

    /// No test file below asks for worker settings of its own.
    static std::vector<ConfigurationOverride> noSettings(const std::vector<RunnableTestFile>& runnables)
    {
        return std::vector<ConfigurationOverride>(runnables.size());
    }
};
...
TEST_F(RunnerTest, RunsOneTupleEndToEnd)
{
    ...
    const std::vector runnables{rewriteOneTuple(runner, workingDir.get(), "OneTuple")};
    const auto checked = runner.runAll(runnables, noSettings(runnables), 1);

    ASSERT_EQ(checked.size(), 1U);
    EXPECT_EQ(checked.at(0).id.originFile, "OneTuple");
    EXPECT_TRUE(std::holds_alternative<Success>(checked.at(0).outcome)) << detailOf(checked.at(0));
}
...
TEST_F(RunnerTest, RunsTwoTestFilesAtOnce)
{
    const Testing::TemporaryDirectory workingDir;
    TestRunner runner{configFor(workingDir.get())};
    const std::vector runnables{rewriteOneTuple(runner, workingDir.get(), "First"), rewriteOneTuple(runner, workingDir.get(), "Second")};
    const auto checked = runner.runAll(runnables, noSettings(runnables), 2);

    ASSERT_EQ(checked.size(), 2U);
    EXPECT_EQ(checked.at(0).id.originFile, "First");
    EXPECT_EQ(checked.at(1).id.originFile, "Second");
    EXPECT_TRUE(std::holds_alternative<Success>(checked.at(0).outcome)) << detailOf(checked.at(0));
    EXPECT_TRUE(std::holds_alternative<Success>(checked.at(1).outcome)) << detailOf(checked.at(1));
}
