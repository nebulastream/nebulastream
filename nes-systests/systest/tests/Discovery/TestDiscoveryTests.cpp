...
namespace
{
/// This file's discovered name, so a test can name the file rather than its position in the result.
...
/// Giving a directory narrows what a run searches without moving the root that a test file is keyed against.
...
TEST_F(TestDiscoveryTest, NarrowsTheSearchToADirectoryWithoutMovingTheKeyRoot)
{
    ...
    EXPECT_EQ(config.testDiscoverRoot.getValue(), tempDir.get().string());
}
...
/// A stem that two files share keeps the directory that tells them apart, and a stem that only one file has does not.
...
/// A file named on the command line reports under its stem, whatever directory it is in.
...
/// A directory outside the root has no name relative to it, so its files are named against the directory itself.
/// A name that walked up out of the root would carry `..`, and the result file built from it would land outside the
/// working directory.
TEST_F(TestDiscoveryTest, DirectoryOutsideTheRootNamesItsFilesAgainstItself)
{
    ...
    ASSERT_EQ(discovered.size(), 1);
    EXPECT_EQ(discovered.front().name(), "Alpha");
}

/// Two directories can hold a file of the same name, and naming each against its own directory would give both the
/// same name. The run reports them apart, and their result files stay apart, because the name grows by the directory
/// above until it is unique.
