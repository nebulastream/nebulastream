...
TEST_F(SystestE2ETest, CheckThatOnlyWrongQueriesFailInFileWithManyQueries)
{
    ...
    config.directlySpecifiedTestFiles.add(fmt::format("{}/errors/{}", SYSTEST_DATA_DIR, testFileName));
...
TEST_P(SystestE2ETest, correctAndIncorrectSchemaTestFile)
{
    ...
    SystestConfiguration config{};
    config.testDiscoverDirs.add(SYSTEST_DATA_DIR);
    config.directlySpecifiedTestFiles.add(fmt::format("{}/errors/{}/{}", SYSTEST_DATA_DIR, directory, testFileName));
