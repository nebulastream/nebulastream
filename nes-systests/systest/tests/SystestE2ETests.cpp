...
static size_t countFailedTests(const std::string_view failedTestString)
{
    return std::ranges::count_if(
        std::views::iota(0UZ, failedTestString.length() - SystestE2ETest::EXTENSION.length() + 1),
        [&](const size_t idx) { return failedTestString.substr(idx, SystestE2ETest::EXTENSION.length()) == SystestE2ETest::EXTENSION; });
};
...
TEST_F(SystestE2ETest, CheckThatOnlyWrongQueriesFailInFileWithManyQueries)
{
    ...
    ASSERT_TRUE(systestResult.returnType == SystestMainResult::ReturnType::FAILED) << " Return type not as expected.";
    ASSERT_EQ(countFailedTests(systestResult.outputMessage), 4) << "Number of failed queries is unexpected.";
...
TEST_P(SystestE2ETest, correctAndIncorrectSchemaTestFile)
{
    ...
    const auto systestResult = NES::Systest::executeSystests(config);
    ASSERT_TRUE(systestResult.returnType == SystestMainResult::ReturnType::FAILED) << " Return type not as expected.";
    ASSERT_EQ(countFailedTests(systestResult.outputMessage), 1) << "Too many failed queries.";
