...
/// An identifier that the qualifier never registered is left untouched, so column names and aliases pass through.
...
/// with the schema that it declares.
...
TEST_F(SqlRewriterTest, PassesAQueryThatDoesNotParseThrough)
{
    ...
    ASSERT_EQ(queries.size(), 1U);
    /// The sink keeps the name that the test wrote, because a declared sink is captured for inlining rather than registered as a name.
...
/// A source that reads from a socket gets no file path: its attached data is served to it, and the endpoint is only known once the server
...
/// The endpoint that a data server bound is only known once the run is under way, so it is merged into the statement then, alongside
...
/// A source that is written into the query and carries its own data has no file to resolve, so it keeps the options that the test wrote.
...
/// The query of an EXPLAIN can name a declared sink, and the plan prints that name, so the sink has to exist in the
/// catalog. The declaration is submitted with its mandatory options spliced in, and the reference keeps the
/// qualified name instead of being inlined.
TEST_F(SqlRewriterTest, ExplainKeepsTheDeclaredSinkAndSubmitsItsDeclaration)
{
    ...
    ASSERT_EQ(queries.size(), 1U);
    EXPECT_EQ(queryOf(queries.at(0)).sql, "EXPLAIN (OPTIMIZED) SELECT id FROM TESTKEY_STREAM INTO TESTKEY_OUT;");
    /// An EXPLAIN runs nothing, so it writes no result file.
    EXPECT_FALSE(queryOf(queries.at(0)).resultFile.has_value());
    const auto* plan = std::get_if<ExpectedPlan>(&queryOf(queries.at(0)).expectation);
    ASSERT_NE(plan, nullptr);
    EXPECT_EQ(plan->qualifyingPrefix, "TESTKEY_");
}

/// An EXPLAIN whose query writes its sink inline needs no declaration: the sink is inlined as it is for a query,
/// and nothing is submitted for it.
TEST_F(SqlRewriterTest, ExplainInlinesTheSinkThatItsQueryWrites)
{
    ...
    ASSERT_EQ(setup.size(), 1U);
    ASSERT_EQ(queries.size(), 1U);
    EXPECT_TRUE(queryOf(queries.at(0)).sql.contains("INTO Void("));
    EXPECT_TRUE(queryOf(queries.at(0)).sql.contains(R"('localhost:8080' AS "SINK"."HOST")"));
    EXPECT_FALSE(queryOf(queries.at(0)).resultFile.has_value());
}

/// Pins a limitation of the file-global sink mode: an EXPLAIN registers the declared sink names, so the reference in
/// an executed query is qualified before the sink lookup, which is keyed by the written name. Such a file is rejected
/// rather than rewritten wrongly. Lifting this needs the lookup to resolve the qualified spelling back to the
/// written one.
