...
{
    const auto [name, key, qualifyingPrefix, setup, queries] = rewriteSlt("CREATE LOGICAL SOURCE oneTuple(field_1 UINT64 NOT NULL);\n"
...
{
    const auto [name, key, qualifyingPrefix, setup, queries]
...
{
    const auto [name, key, qualifyingPrefix, setup, queries] = rewriteSlt("CREATE SINK sinkOneTuple(field_1 UINT64 NOT NULL) TYPE File;\n"
                                                                          "\n"
                                                                          "SELECT field_1 FROM oneTuple INTO sinkOneTuple;\n"
                                                                          "----\n"
                                                                          "1\n"
...
{
    const auto [name, key, qualifyingPrefix, setup, queries] = rewriteSlt("CREATE LOGICAL SOURCE stream(id UINT64 NOT NULL);\n"
                                                                          "CREATE PHYSICAL SOURCE FOR stream TYPE File;\n"
