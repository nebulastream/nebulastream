...
public:
    ...
    /// Parses a statement and prefixes its names on its own, which is what every rewriting pass does alongside its own edits.
...
/// The rewrite leaves an identifier that the registry never saw unchanged.
...
/// An identifier that only contains a registered name as a substring is a distinct token, so the rewrite leaves it unchanged.
...
/// A registered name that appears inside a string literal is not an identifier token, so the rewrite leaves it unchanged.
...
/// Rewriting is a fixed point: the prefixed spelling is not itself registered, so a second pass is idempotent.
...
/// A function name is a plain identifier in the grammar, so a source that shares its name must not rename the calls.
...
/// A sink's declared schema must match the query's output, which a renamed column would break.
...
/// The parser checks the spelling before the dot against the source, so the qualifier is renamed and the field is not.
