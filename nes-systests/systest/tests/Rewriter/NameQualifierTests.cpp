...
/// A plain stem and its `_with_varsized` variant sit in one directory, so the suffix has to separate them on its own.
...
///
/// A relative path only names a directory while the process sits somewhere it exists, so the test moves there rather than
/// assuming the directory the run happened to start in.
...
/// A name the test quoted keeps the case and the punctuation it was written with, and only quotes preserve that in a statement.
