...
/// Statement builders that fix the fields the partition never reads, so a test states only the SQL and the overrides.
TestStatement create(std::string sql)
{
    return CreateStatement{.sql = std::move(sql), .attach = std::nullopt};
}

TestStatement query(std::string sql, ConfigurationOverride overrides)
{
    return SelectStatement{.sql = std::move(sql), .id = SystestQueryId{1}, .expected = ExpectedRows{}, .overrides = std::move(overrides)};
}

TestStatement query(std::string sql)
{
    return query(std::move(sql), {});
}
...
/// The SQL of every statement of a part, so a part can be stated as what it holds, not as a variant.
...
/// A differential block states a configuration like a query does, so it joins the part whose overrides match.
...
/// Two queries with different configurations need two workers, and each part repeats the setup.
TEST(TestFilePartitionTest, SplitsQueriesThatAskForDifferentOverrides)
{
    const ParsedTestFile file{
        .path = "two.test",
        .statements = {create("CREATE A"), query("Q1", {{"a", "1"}}), query("Q2", {{"a", "2"}}), query("Q3", {{"a", "1"}})}};
...
/// The reader expands a query with listed alternatives into one statement per alternative, all sharing one number,
/// and each has to reach the worker that its configuration selects.
TEST(TestFilePartitionTest, SplitsTheAlternativesOfOneQueryApart)
{
    const ParsedTestFile file{
        .path = "alternatives.test",
        .statements = {create("CREATE A"), query("Q1", {{"bloom", "true"}}), query("Q1", {{"bloom", "false"}})}};
...
/// An EXPLAIN states no configuration of its own, and a file that also holds a plain query must not lose it.
...
{
    const ParsedTestFile file{.path = "mixed.test", .statements = {create("CREATE A"), query("Q1", {{"a", "1"}}), explain("EXPLAIN Q1")}};
