...
/// TupleLargerThanBuffer relies on this: it asks for one buffer size, checks that a tuple does not fit, then asks for a
/// larger one and checks that it does.
...
/// A query that asks for nothing runs exactly once, so listing no configuration is not an empty set of alternatives.
TEST_F(TestFileParserTest, RunsAQueryWithoutSettingsExactlyOnce)
{
    ...
    ASSERT_NE(query, nullptr);
    EXPECT_TRUE(query->settings.overrideParameters.empty());
}
...
TEST_F(TestFileParserTest, ReadsCreatesAndQueryWithExpectedRows)
{
    ...
    ASSERT_TRUE(physicalSource->attach.has_value());
    EXPECT_EQ(physicalSource->attach->kind, TestDataIngestionType::INLINE);
    EXPECT_EQ(physicalSource->attach->rows, (std::vector<std::string>{"1"}));
...
/// An ATTACH FILE source keeps the referenced path as its single attach row rather than materializing anything.
TEST_F(TestFileParserTest, ReadsFileAttach)
{
    ...
    ASSERT_NE(physicalSource, nullptr);
    ASSERT_TRUE(physicalSource->attach.has_value());
    EXPECT_EQ(physicalSource->attach->kind, TestDataIngestionType::FILE);
    EXPECT_EQ(physicalSource->attach->rows, std::vector<std::string>{"small/stream8.csv"});
}
