...
/**
 * @brief Combines together a triggerCondition, callback function and a queryId
 * for a specific StatisticKey
 */
class StatisticInfo {
 public:
   ...
   * @param window: Over what window do we built the statistic, e.g., Tumbling,
   * Sliding, Threshold, ...
   * @param triggerCondition: Condition that is checked for each newly created
   * window of a given statistic.
  ...
  /**
   * @brief The query with the id is not running anymore, therefore, we have to
   * set the queryId to INVALID_QUERY_ID
   */
  void stoppedQuery();

  /**
   * @brief Compares the underlying queryId to INVALID_QUERY_ID. If they are
   * equal, the query is not running
...
using StatisticInfoWLock =
    std::shared_ptr<folly::Synchronized<StatisticInfo>::WLockedPtr>;
