...
/**
 * @brief Parent class for all different statistic characteristic, e.g.,
 * workload, data, infrastructure.
 */
...
public:
  ...
  /**
   * @brief Checks if the current Characteristic is of type const
   * CharacteristicType
   * @tparam Characteristic
   * @return bool true if node is of Characteristic
   */
  template <class Characteristic> bool instanceOf() const {
   ...
   */
  template <class CharacteristicType> std::shared_ptr<CharacteristicType> as() {
    if (instanceOf<CharacteristicType>()) {
      return std::dynamic_pointer_cast<CharacteristicType>(
   ...
   */
  virtual bool operator==(const Characteristic &rhs) const {
