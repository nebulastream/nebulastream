...
class AbstractStatisticCache {
 public:
  /**
   * @brief Inserts a statistic into the cache. If a statistic exists for the
   * same timestamp then, it will NOT be overwritten and discarded
   * @param statisticHash
  ...
  virtual bool insertStatistic(const StatisticHash& statisticHash,
                               const StatisticValue<>& statisticValue) = 0;
   ...
   */
  virtual std::vector<StatisticValuePtr> getStatistic(
      const StatisticHash& statisticHash, const Windowing::TimeMeasure& startTs,
      const Windowing::TimeMeasure& endTs) = 0;

  /**
   * @brief Deletes the statistics for the given hash and interval [startTs,
   * endTs]
   * @param statisticHash
   * @param startTs
   * @param endTs
