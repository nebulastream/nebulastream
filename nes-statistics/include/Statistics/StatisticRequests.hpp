...
struct StatisticRequest {
  explicit StatisticRequest(const StatisticHash &statisticHash)
      : statisticHash(statisticHash) {}
  const StatisticHash statisticHash;
};

/**
 * @brief Struct that represents a request to track a statistic, for now, we do
 * not use it but we will later
 */
struct StatisticTrackRequest : public StatisticRequest {
  StatisticTrackRequest(const StatisticHash &statisticHash,
      ...
      : StatisticRequest(statisticHash), characteristic(characteristic),
        triggerCondition(triggerCondition), sendingPolicy(sendingPolicy),
        windowType(windowType), callBack(callBack) {}
  const CharacteristicPtr characteristic;
  const TriggerConditionPtr triggerCondition;
  const SendingPolicyPtr sendingPolicy;
  const Windowing::WindowTypePtr windowType;
...
struct StatisticProbeRequest : public StatisticRequest {
  explicit StatisticProbeRequest(const StatisticHash &statisticHash,
      ...
      : StatisticRequest(statisticHash), startTs(startTs), endTs(endTs),
        probeExpression(probeExpression), granularity(granularity) {}

  explicit StatisticProbeRequest(const StatisticHash &statisticHash,
                                 const Windowing::TimeMeasure &granularity,
                                 const ProbeExpression &probeExpression)
      : StatisticProbeRequest(statisticHash, Windowing::TimeMeasure(0),
                              Windowing::TimeMeasure(UINT64_MAX), granularity,
                              probeExpression) {}
  const Windowing::TimeMeasure startTs;
  const Windowing::TimeMeasure endTs;
  const ProbeExpression probeExpression;
...
/**
 * @brief Struct that represents a request to probe a statistic plus a grpc
...
struct StatisticProbeRequestGRPC : public StatisticProbeRequest {
  explicit StatisticProbeRequestGRPC(const StatisticHash &statisticHash,
      ...
      : StatisticProbeRequestGRPC(StatisticProbeRequest(statisticHash, startTs,
                                                        endTs, granularity,
                                                        probeExpression),
                                  workerId) {}
  explicit StatisticProbeRequestGRPC(const StatisticProbeRequest &probeRequest,
                                     const WorkerId &workerId)
