...
class CountMinStatistic : public SynopsesStatistic {
 public:
   ...
   * @param startTs: Timestamp of the first tuple for that this CountMin was
   * created
   * @param endTs: Timestamp of the last tuple for that this CountMin was
   * created
   * @param observedTuples: Number of tuples seen during the creation
   * @param width: Number of columns
   * @param depth: Number of rows
   * @param countMinDataString: String that stores the CountMin data, so the
   * underlying 2-D array of counters
   * @return StatisticPtr
   */
  ...
  /**
   * @brief Creates a CountMin for the given startTs, endTs, width, and depth.
   * The #observedTuples and all counters are set to 0
   * @param startTs
   * @param endTs
   * @param width
   * @param depth
