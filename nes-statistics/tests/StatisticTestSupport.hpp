...
inline const Host TEST_HOST{"localhost:8080"};
...
inline LogicalPlan buildStatisticPlan(const std::filesystem::path& inputPath)
{
    ...
    const TypedLogicalFunction<UnboundFieldAccessLogicalFunction> fieldAccess{
        UnboundFieldAccessLogicalFunction{Identifier::parse("value")}};

    return LogicalPlanBuilder::addStatisticBuild(
        std::move(plan),
        windowType,
        timeCharacteristic,
        StatisticId{STATISTIC_ID},
        WindowAggregationLogicalFunction{AvgAggregationLogicalFunction{fieldAccess}});
}

inline LogicalPlan addStatisticStoreReader(const LogicalPlan& plan)
{
    return LogicalPlanBuilder::addStatisticProbe(
        plan,
        StatisticId{STATISTIC_ID},
...
inline std::optional<LogicalPlan> optimizeToLocalPlan(const LogicalPlan& plan)
{
    ...
    const auto seedWorker
        = fmt::format("CREATE WORKER '{}' SET ('localhost:9090' AS DATA, 10 AS \"CAPACITY\");", std::string{TEST_HOST.getRawValue()});
    const auto planned = plan_sql(context->context(), rust::Str{seedWorker.data(), seedWorker.size()}, rust::Str{}, rust::Str{});
