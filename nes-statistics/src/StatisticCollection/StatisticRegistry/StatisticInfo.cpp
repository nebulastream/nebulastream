...
StatisticInfo::StatisticInfo(
    ...
    : window(window), triggerCondition(std::move(triggerCondition)),
      callBack(callBack), queryId(queryId), metric(std::move(metric)) {}

[[maybe_unused]] TriggerConditionPtr
StatisticInfo::getTriggerCondition() const {
...
const std::function<void(CharacteristicPtr)> &
StatisticInfo::getCallBack() const {
  return callBack;
}
