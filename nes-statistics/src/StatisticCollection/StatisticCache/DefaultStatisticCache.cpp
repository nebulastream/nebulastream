...
bool DefaultStatisticCache::insertStatistic(
    const StatisticHash &statisticHash,
    const StatisticValue<> &statisticValue) {
  auto lockedKeyToStatisticMap = keyToStatistics.wlock();
  auto &statisticVec = (*lockedKeyToStatisticMap)[statisticHash];

  // For now, we do not allow duplicate statistics. Meaning the same statistic
  // key with the same startTs and endTs
  ...
  for (const auto &stat : statisticVec) {
    if (startTs.equals(stat->getStartTs()) && endTs.equals(stat->getEndTs())) {
      NES_ERROR("Duplicate statistic key with the same startTs {} and endTs {}",
                startTs.toString(), endTs.toString());
      return false;
    }
  }

  statisticVec.emplace_back(std::make_shared<StatisticValue<>>(
      statisticValue.getValue(), statisticValue.getStartTs(),
      statisticValue.getEndTs()));
  return true;
}

std::vector<StatisticValuePtr>
DefaultStatisticCache::getStatistic(const StatisticHash &statisticHash,
  ...
  /* We describe a bool lambda that checks if the statistics lies between
   * [startTs, endTs]. Afterward, we use the lambda function in the copy_if()
   * that iterates over the statisticVec and copies all items, that satisfy the
   * lambda condition.
   */
  ...
  std::copy_if(statisticVec.begin(), statisticVec.end(),
               std::back_inserter(returnStatisticsVector), getCondition);
...
bool DefaultStatisticCache::deleteStatistics(
  ...
  auto removeBeginIt =
      std::remove_if(statisticVec.begin(), statisticVec.end(), deleteCondition);
