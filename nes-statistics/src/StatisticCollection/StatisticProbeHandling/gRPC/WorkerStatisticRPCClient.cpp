...
std::vector<StatisticValue<>> WorkerStatisticRPCClient::probeStatistics(
    const StatisticProbeRequestGRPC &probeRequest,
    const std::string &gRPCAddress) {
  NES_DEBUG("Requesting statistics from workerId {} at address {}",
            probeRequest.workerId, gRPCAddress);
  ...
  ExpressionSerializationUtil::serializeExpression(
      probeRequest.probeExpression.getProbeExpression(), expressionDetails);

  // 2. Sending the request and building a reply
  auto chan =
      grpc::CreateChannel(gRPCAddress, grpc::InsecureChannelCredentials());
  ...
  if (status.ok()) {
    ...
    std::transform(reply.statistics().begin(), reply.statistics().end(),
    ...
    NES_ERROR("WorkerStatisticRPCClient::probeStatistics error={}: {}",
