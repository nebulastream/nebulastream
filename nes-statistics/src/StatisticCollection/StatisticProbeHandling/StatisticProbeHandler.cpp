...
QueryId StatisticProbeHandler::getStatisticQueryId(
    const StatisticKey& statisticKey) const {
  return statisticRegistry->getQueryId(statisticKey.hash());
}

ProbeResult<> StatisticProbeHandler::probeStatistic(
    const StatisticProbeRequest& probeRequest) {
  return probeStatistic(
      probeRequest, false,
      [](const ProbeResult<>& probeResult) { return probeResult; });
}

ProbeResult<> StatisticProbeHandler::probeStatistic(
    const StatisticProbeRequest& statisticProbeRequest,
  ...
  if (!statisticRegistry->contains(statisticProbeRequest.statisticHash)) {
    NES_INFO("Could not find a statistic collection query for StatisticHash={}",
             statisticProbeRequest.statisticHash);
    return {};
  }

  // 2. Check if the statistic is tracked with the same granularity as wished
  const auto statisticInfo = statisticRegistry->getStatisticInfoWithGranularity(
      statisticProbeRequest.statisticHash, statisticProbeRequest.granularity);
  if (!statisticInfo.has_value()) {
    NES_INFO(
        "Could not find a statistic collection query for StatisticHash={} with "
        "granularity={}",
        statisticProbeRequest.statisticHash,
  ...
  // 3. Getting all probe requests of nodes that we have to query for the
  // statistic
  const auto workerIds = topology->getAllRegisteredNodeIds();
  const auto allProbeRequests = statisticProbeGenerator->generateProbeRequests(
      *statisticRegistry, *statisticCache, statisticProbeRequest, workerIds);

  // 4. Receive all statistics by sending the probe requests to the nodes
  ProbeResult<> probeResult;
  for (const auto& probeRequest : allProbeRequests) {
    const auto topologyNode =
        topology->getCopyOfTopologyNodeWithId(probeRequest.workerId);
    const auto gRPCAddress = topologyNode->getIpAddress() + ":" +
                             std::to_string(topologyNode->getGrpcPort());
    const auto statisticValues =
        workerRpcClientPtr->probeStatistics(probeRequest, gRPCAddress);
...
StatisticProbeHandlerPtr StatisticProbeHandler::create(
  ...
  return std::make_shared<StatisticProbeHandler>(StatisticProbeHandler(
      statisticRegistry, statisticProbeGenerator, statisticCache, topology));
}

StatisticProbeHandler::StatisticProbeHandler(
    const StatisticRegistryPtr statisticRegistry,
    const StatisticProbeGeneratorPtr statisticProbeGenerator,
    const StatisticCachePtr statisticCache, const TopologyPtr topology)
