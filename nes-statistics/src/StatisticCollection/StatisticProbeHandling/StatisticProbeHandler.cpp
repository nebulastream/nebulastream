    ...
    std::function<ProbeResult<>(ProbeResult<>)>&& aggFunction)
{
    ...
    const auto statisticInfo
        = statisticRegistry->getStatisticInfoWithGranularity(statisticProbeRequest.statisticHash, statisticProbeRequest.granularity);
    if (!statisticInfo.has_value())
    {
        NES_INFO(
            "Could not find a statistic collection query for StatisticHash={} with granularity={}",
