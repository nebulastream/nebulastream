...
StatisticPtr CountMinStatistic::create(
    const Windowing::TimeMeasure &startTs, const Windowing::TimeMeasure &endTs,
    uint64_t observedTuples, uint64_t width, uint64_t depth,
    uint64_t numberOfBitsInKey, const std::string_view countMinDataString) {
  // We just store the CountMin data as a string. So we have to copy the bytes
  // from the string to a vector We do this by first creating a large enough
  // vector and then copy the bytes
  std::vector<uint64_t> countMinData(width * depth, 0);
  std::memcpy(countMinData.data(), countMinDataString.data(),
              sizeof(uint64_t) * width * depth);

  return std::make_shared<CountMinStatistic>(
      CountMinStatistic(startTs, endTs, observedTuples, width, depth,
                        numberOfBitsInKey, countMinData));
}

StatisticPtr CountMinStatistic::createInit(
    const Windowing::TimeMeasure &startTs, const Windowing::TimeMeasure &endTs,
    uint64_t width, uint64_t depth, uint64_t numberOfBitsInKey) {
  // Initializing the underlying count min data to all 0 and setting the
  // observed tuples to 0
  std::vector<uint64_t> countMinData(width * depth, 0);
...
CountMinStatistic::CountMinStatistic(const Windowing::TimeMeasure &startTs,
                                     ...
                                     uint64_t observedTuples, uint64_t width,
                                     uint64_t depth, uint64_t numberOfBitsInKey,
                                     const std::vector<uint64_t> &countMinData)
    : SynopsesStatistic(startTs, endTs, observedTuples), width(width),
      depth(depth), numberOfBitsInKey(numberOfBitsInKey),
      countMinData(countMinData) {}

StatisticValue<> CountMinStatistic::getStatisticValue(
    const ProbeExpression &probeExpression) const {
  const auto expression = probeExpression.getProbeExpression();
  if (!expression->instanceOf<EqualsExpressionNode>()) {
    NES_THROW_RUNTIME_ERROR("For now, we can only get the statistic for a "
  ...
  // We expect that one expression is a constant value and the other a field
  // access expression
  if (!((leftExpr->instanceOf<ConstantValueExpressionNode>() &&
         rightExpr->instanceOf<FieldAccessExpressionNode>()) ||
        (leftExpr->instanceOf<FieldAccessExpressionNode>() &&
         rightExpr->instanceOf<ConstantValueExpressionNode>()))) {
    NES_THROW_RUNTIME_ERROR(
        "For now, we expect that one expression is a constant value and the "
        "other a field access expression!");
  }

  // Getting the constant value and the field access
  const auto constantExpression =
      leftExpr->instanceOf<ConstantValueExpressionNode>() ? leftExpr
                                                          : rightExpr;
  const auto constantValue =
      constantExpression->as<ConstantValueExpressionNode>()->getConstantValue();
  const auto basicValue = constantValue->as<BasicValue>();

  // Iterating over each row. Calculating the column and then taking the minimum
  // over all rows
  uint64_t minCount = UINT64_MAX;
  for (auto row = 0_u64; row < depth; ++row) {
    const auto hashValue = StatisticUtil::getH3HashValue(
        *basicValue, row, depth, numberOfBitsInKey);
...
bool CountMinStatistic::equal(const Statistic &other) const {
  if (other.instanceOf<CountMinStatistic>()) {
    ...
    return startTs.equals(otherCountMinStatistic->startTs) &&
           endTs.equals(otherCountMinStatistic->endTs) &&
           observedTuples == otherCountMinStatistic->observedTuples &&
           width == otherCountMinStatistic->width &&
           depth == otherCountMinStatistic->depth &&
           numberOfBitsInKey == otherCountMinStatistic->numberOfBitsInKey &&
...
void CountMinStatistic::merge(const SynopsesStatistic &other) {
  ...
  if (!other.instanceOf<CountMinStatistic>()) {
    NES_ERROR("Other is not of type CountMinStatistic. Therefore, we skipped "
  ...
  auto otherCountMinStatistic = other.as<const CountMinStatistic>();
  if (depth != otherCountMinStatistic->depth ||
      width != otherCountMinStatistic->width) {
    NES_ERROR("Can not combine sketches <{},{}> and <{},{}>, as they do not "
              "share the same dimensions",
              width, depth, otherCountMinStatistic->width,
