...
StatisticPtr HyperLogLogStatistic::create(
    const Windowing::TimeMeasure &startTs, const Windowing::TimeMeasure &endTs,
    uint64_t observedTuples, uint64_t bitWidth,
    const std::string_view hyperLogLogDataString, double estimate) {
  // We just store the HyperLogLog data as a string. So we have to copy the
  // bytes from the string to a vector We do this by first creating a large
  // enough vector and then copy the bytes
  std::vector<uint8_t> hyperLogLogData((1 << bitWidth), 0);
  std::memcpy(hyperLogLogData.data(), hyperLogLogDataString.data(),
              sizeof(uint8_t) * hyperLogLogData.size());

  return std::make_shared<HyperLogLogStatistic>(HyperLogLogStatistic(
      startTs, endTs, observedTuples, bitWidth, hyperLogLogData, estimate));
}

StatisticPtr
HyperLogLogStatistic::createInit(const Windowing::TimeMeasure &startTs,
                                 const Windowing::TimeMeasure &endTs,
                                 uint64_t bitWidth) {
  std::vector<uint8_t> hyperLogLogData((1 << bitWidth), 0);
  constexpr auto observedTuples = 0;
  return std::make_shared<HyperLogLogStatistic>(HyperLogLogStatistic(
      startTs, endTs, observedTuples, bitWidth, hyperLogLogData,
      std::numeric_limits<double>::quiet_NaN()));
}

HyperLogLogStatistic::HyperLogLogStatistic(
    const Windowing::TimeMeasure &startTs, const Windowing::TimeMeasure &endTs,
    uint64_t observedTuples, uint64_t bitWidth,
    const std::vector<uint8_t> &registers, double estimate)
    : SynopsesStatistic(startTs, endTs, observedTuples), bitWidth(bitWidth),
      estimate(estimate), registerSize(1 << bitWidth), registers(registers) {
...
StatisticValue<>
HyperLogLogStatistic::getStatisticValue(const ProbeExpression &) const {
  // If the estimate is not calculated yet, we throw an exception
#ifdef __APPLE__
  if (std::isnan(estimate)) {
    NES_THROW_RUNTIME_ERROR("HyperLogLogStatistic has no estimate value. "
                            "Please call performEstimation() first.");
  }
#else
  if (isnanl(estimate)) {
    NES_THROW_RUNTIME_ERROR("HyperLogLogStatistic has no estimate value. "
...
bool HyperLogLogStatistic::equal(const Statistic &other) const {
  if (other.instanceOf<HyperLogLogStatistic>()) {
    ...
    return startTs.equals(otherHyperLogLogStatistic->startTs) &&
           endTs.equals(otherHyperLogLogStatistic->endTs) &&
           observedTuples == otherHyperLogLogStatistic->observedTuples &&
           bitWidth == otherHyperLogLogStatistic->bitWidth &&
           alphaMM == otherHyperLogLogStatistic->alphaMM &&
           registerSize == otherHyperLogLogStatistic->registerSize &&
...
void HyperLogLogStatistic::merge(const SynopsesStatistic &other) {
  ...
  if (!other.instanceOf<HyperLogLogStatistic>()) {
    NES_ERROR("Other is not of type HyperLogLogStatistic. Therefore, we "
              "skipped merging them together");
    return;
  }
  ...
  if (bitWidth != otherHyperLogLog->bitWidth ||
      registerSize != otherHyperLogLog->registerSize) {
    NES_ERROR("Bit bitWidth or register size is different. Therefore, we "
              "skipped merging them together");
    return;
  }
...
void HyperLogLogStatistic::update(uint64_t hash) {
  ...
  // Rank is the position of the leftmost 1-bit in the binary representation of
  // the hash
...
void HyperLogLogStatistic::performEstimation() {
  ...
  if (estimate < 2.5 * registerSize) {
    ...
    if (zeros != 0) {
      estimate =
          registerSize * std::log(static_cast<double>(registerSize) / zeros);
    }
