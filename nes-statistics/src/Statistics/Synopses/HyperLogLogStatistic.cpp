...
StatisticPtr HyperLogLogStatistic::createInit(const Windowing::TimeMeasure& startTs, const Windowing::TimeMeasure& endTs, uint64_t bitWidth)
{
    std::vector<uint8_t> hyperLogLogData((1 << bitWidth), 0);
    constexpr auto observedTuples = 0;
    return std::make_shared<HyperLogLogStatistic>(
        HyperLogLogStatistic(startTs, endTs, observedTuples, bitWidth, hyperLogLogData, std::numeric_limits<double>::quiet_NaN()));
}
...
uint64_t HyperLogLogStatistic::getWidth() const
{
    return bitWidth;
}

double HyperLogLogStatistic::getEstimate() const
{
    return estimate;
}
