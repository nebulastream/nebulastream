...
StatisticPtr HyperLogLogStatistic::createInit(
    const Windowing::TimeMeasure& startTs, const Windowing::TimeMeasure& endTs,
    uint64_t bitWidth) {
  std::vector<uint8_t> hyperLogLogData((1 << bitWidth), 0);
...
StatisticValue<> HyperLogLogStatistic::getStatisticValue(
    const ProbeExpression&) const {
  // If the estimate is not calculated yet, we throw an exception
#ifdef __APPLE__
  if (std::isnan(estimate)) {
    NES_THROW_RUNTIME_ERROR(
        "HyperLogLogStatistic has no estimate value. Please call "
        "performEstimation() first.");
  }
#else
  if (isnanl(estimate)) {
    NES_THROW_RUNTIME_ERROR(
...
void HyperLogLogStatistic::merge(const SynopsesStatistic& other) {
  ...
  if (!other.instanceOf<HyperLogLogStatistic>()) {
    NES_ERROR(
        "Other is not of type HyperLogLogStatistic. Therefore, we skipped "
  ...
  if (bitWidth != otherHyperLogLog->bitWidth ||
    ...
    NES_ERROR(
        "Bit bitWidth or register size is different. Therefore, we skipped "
        "merging them together");
    return;
  }
