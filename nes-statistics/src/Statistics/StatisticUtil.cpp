...
uint64_t StatisticUtil::getH3HashValue(BasicValue &value, uint64_t row,
                                       uint64_t depth,
                                       uint64_t numberOfBitsInKey) {
  // Creating here the H3-Seeds with the same seed, as used in the creation of
  ...
  // We do not require the data type but only the size, as H3 operators on the
  // bits and not on the value
  uint64_t key = 0;
