...
LogLevel Benchmark::E2EBenchmarkConfig::getLogLevel(
    const std::string& yamlConfigFile, LogLevel defaultLogLevel) {
  LogLevel retLogLevel = defaultLogLevel;
  try {
    Yaml::Node configFile;
    Yaml::Parse(configFile, yamlConfigFile.c_str());
    if (configFile.IsNone()) {
      std::cerr << "Error while reading the log level. Setting the loglevel to "
                << getLogName(defaultLogLevel) << std::endl;
...
Benchmark::E2EBenchmarkConfig Benchmark::E2EBenchmarkConfig::createBenchmarks(
    const std::string& yamlConfigFile) {
  ...
  try {
    ...
    NES_THROW_RUNTIME_ERROR(
        "Error while trying to parse the yaml config file: " << e.what());
  }

  NES_INFO("Generating configOverAllRuns...");
  e2EBenchmarkConfig.configOverAllRuns =
      E2EBenchmarkConfigOverAllRuns::generateConfigOverAllRuns(configFile);

  NES_INFO("Generating configsPerRun...");
  e2EBenchmarkConfig.allConfigPerRuns =
      E2EBenchmarkConfigPerRun::generateAllConfigsPerRun(configFile);
...
std::string Benchmark::E2EBenchmarkConfig::toString() {
  ...
  for (size_t experiment = 0; experiment < allConfigPerRuns.size();
       ++experiment) {
