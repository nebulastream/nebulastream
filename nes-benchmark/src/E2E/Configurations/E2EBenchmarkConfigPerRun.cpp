...
std::vector<E2EBenchmarkConfigPerRun> E2EBenchmarkConfigPerRun::generateAllConfigsPerRun(Yaml::Node yamlConfig)
{
    ...
    auto numWorkerOfThreads = Util::splitAndFillIfEmpty<uint32_t>(
        yamlConfig["numberOfWorkerThreads"].As<std::string>(), configPerRun.numberOfWorkerThreads->getDefaultValue());

    auto bufferSizeInBytes = Util::splitAndFillIfEmpty<uint32_t>(
        yamlConfig["bufferSizeInBytes"].As<std::string>(), configPerRun.bufferSizeInBytes->getDefaultValue());
    auto numberOfQueriesToDeploy = Util::splitAndFillIfEmpty<uint32_t>(
        yamlConfig["numberOfQueriesToDeploy"].As<std::string>(), configPerRun.numberOfQueriesToDeploy->getDefaultValue());

    auto numberOfBuffersInGlobalBufferManager = Util::splitAndFillIfEmpty<uint32_t>(
    ...
    auto numberOfBuffersInSourceLocalBufferPool = Util::splitAndFillIfEmpty<uint32_t>(
    ...
    auto preAllocPageCnts = Util::splitAndFillIfEmpty<uint32_t>(
        yamlConfig["preAllocPageCnt"].As<std::string>(), configPerRun.preAllocPageCnt->getDefaultValue());
    auto numberOfPartitions = Util::splitAndFillIfEmpty<uint32_t>(
        yamlConfig["numberOfPartitions"].As<std::string>(), configPerRun.numberOfPartitions->getDefaultValue());

    auto maxHashTableSizes = Util::splitAndFillIfEmpty<uint64_t>(
        yamlConfig["maxHashTableSize"].As<std::string>(), configPerRun.maxHashTableSize->getDefaultValue());
    ...
    Util::padVectorToSize<uint32_t>(
        numberOfBuffersInSourceLocalBufferPool, totalBenchmarkRuns, numberOfBuffersInSourceLocalBufferPool.back());
    ...
    Util::padVectorToSize<std::map<std::string, uint64_t>>(
        allLogicalSrcToPhysicalSources, totalBenchmarkRuns, allLogicalSrcToPhysicalSources.back());
