...
void E2ESingleRun::setupCoordinatorConfig() {
  ...
  coordinatorConf->worker.numWorkerThreads =
      configPerRun.numberOfWorkerThreads->getValue();
  coordinatorConf->worker.bufferSizeInBytes =
      configPerRun.bufferSizeInBytes->getValue();

  coordinatorConf->worker.numberOfBuffersInGlobalBufferManager =
  ...
  coordinatorConf->worker.coordinatorHost =
      coordinatorConf->coordinatorHost.getValue();
  coordinatorConf->worker.localWorkerHost =
      coordinatorConf->coordinatorHost.getValue();
  ...
  coordinatorConf->worker.queryCompiler.pageSize =
      configPerRun.pageSize->getValue();
  coordinatorConf->worker.queryCompiler.numberOfPartitions =
      configPerRun.numberOfPartitions->getValue();
  coordinatorConf->worker.queryCompiler.preAllocPageCnt =
      configPerRun.preAllocPageCnt->getValue();
  coordinatorConf->worker.queryCompiler.maxHashTableSize =
      configPerRun.maxHashTableSize->getValue();
  ...
  if (configOverAllRuns.joinStrategy->getValue() == "HASH_JOIN_LOCAL") {
  ...
  } else if (configOverAllRuns.joinStrategy->getValue() ==
    ...
    NES_THROW_RUNTIME_ERROR("Join Strategy "
                            << configOverAllRuns.joinStrategy->getValue()
                            << " not supported");
  }

  NES_INFO("Using joinStrategy {}",
           magic_enum::enum_name<QueryCompilation::StreamJoinStrategy>(
               coordinatorConf->worker.queryCompiler.joinStrategy));
...
void E2ESingleRun::createSources() {
  NES_INFO("Creating sources and the accommodating data generation and data "
           "providing...");
  ...
  for (const auto &item : configOverAllRuns.sourceNameToDataGenerator) {
    ...
    auto logicalSource = LogicalSourceType::create(
        logicalSourceName, dataGenerator->getSchemaType());
    coordinatorConf->logicalSourceTypes.add(logicalSource);

    auto numberOfPhysicalSrc =
        configPerRun
            .logicalSrcToNoPhysicalSrc[logicalSource->getLogicalSourceName()];
    auto numberOfTotalBuffers =
        configOverAllRuns.numberOfPreAllocatedBuffer->getValue() *
        numberOfPhysicalSrc;

    auto bufferManager = std::make_shared<Runtime::BufferManager>(
        configPerRun.bufferSizeInBytes->getValue(), numberOfTotalBuffers);
    ...
    for (uint64_t i = 0; i < numberOfPhysicalSrc; i++) {
      ...
      auto createdBuffers = dataGenerator->createData(
          configOverAllRuns.numberOfPreAllocatedBuffer->getValue(),
          configPerRun.bufferSizeInBytes->getValue());

      auto sourceConfig =
          createPhysicalSourceType(logicalSourceName, physicalStreamName,
                                   createdBuffers, sourceCnt, i, generatorName);
      coordinatorConf->worker.physicalSourceTypes.add(sourceConfig);

      sourceCnt++;
      NES_INFO("Created {} for {}", physicalStreamName,
               logicalSource->getLogicalSourceName());
    }
  }
  NES_INFO("Created sources and the accommodating data generation and data "
           "providing!");
}

void E2ESingleRun::submitQueries(
    RequestHandlerServicePtr requestHandlerService,
    Catalogs::Query::QueryCatalogPtr queryCatalog) {
  for (size_t i = 0; i < configPerRun.numberOfQueriesToDeploy->getValue();
       i++) {
    for (const auto &query : configOverAllRuns.queries) {

      // If custom delay is set introduce a delay before submitting
      std::this_thread::sleep_for(
          std::chrono::seconds(query.getCustomDelayInSeconds()));

      NES_INFO("E2EBase: Submitting query = {}", query.getQueryString());
      auto queryId = requestHandlerService->validateAndQueueAddQueryRequest(
          query.getQueryString(), Optimizer::PlacementStrategy::BottomUp);
      submittedIds.push_back(queryId);

      if (!waitForQueryToStart(queryId, queryCatalog,
                               defaultStartQueryTimeout)) {
        NES_THROW_RUNTIME_ERROR(
            "E2EBase: Could not start query with id = " << queryId);
      }
...
void E2ESingleRun::runQueries() {
  ...
  while (found != submittedIds.size()) {
    for (auto id : submittedIds) {
      auto sharedQueryId =
          coordinator->getGlobalQueryPlan()->getSharedQueryId(id);
      auto stats =
          coordinator->getNodeEngine()->getQueryStatistics(sharedQueryId);
      for (auto iter : stats) {
        while (iter->getProcessedTuple() < 1) {
          NES_DEBUG("Query with id = {} not ready with no. tuples = {}. "
                    "Sleeping for a second now...",
                    id, iter->getProcessedTuple());
          sleep(1);
        }
        NES_INFO("Query with id = {} Ready with no. tuples = {}", id,
...
void E2ESingleRun::writeMeasurementsToCsv() {
  NES_INFO("Writing the measurements to {}",
           configOverAllRuns.outputFile->getValue());
  ...
  for (const auto &measurementsCsv : measurements.getMeasurementsAsCSV(
           schemaSizeInB, configPerRun.numberOfQueriesToDeploy->getValue())) {
  ...
  NES_INFO("Done writing the measurements to {}",
           configOverAllRuns.outputFile->getValue());
  NES_INFO("Statistics are: {}", outputCsvStream.str())
  std::cout << "Throughput=" << measurements.getThroughputAsString()
            << std::endl;
}
...
E2ESingleRun::E2ESingleRun(E2EBenchmarkConfigPerRun &configPerRun,
    ...
    : configPerRun(configPerRun), configOverAllRuns(configOverAllRuns),
      rpcPortSingleRun(rpcPort), restPortSingleRun(restPort) {}
...
bool E2ESingleRun::waitForQueryToStart(
    QueryId queryId, const Catalogs::Query::QueryCatalogPtr &queryCatalog,
    std::chrono::seconds timeoutInSec) {
  NES_TRACE("checkCompleteOrTimeout: Wait until the query {} is running",
            queryId);
  ...
  while (std::chrono::system_clock::now() < startTimestamp + timeoutInSec) {
    ...
    NES_TRACE("checkCompleteOrTimeout: Query with id = {} is currently {}",
              queryId, magic_enum::enum_name(queryState));
    switch (queryState) {
    ...
    case QueryState::FAILED: {
      NES_ERROR(
          "Query failed to start. Expected: Running or Optimizing but found {}",
          std::string(magic_enum::enum_name(queryState)));
      return false;
    }
    default: {
      NES_WARNING("Expected: Running or Scheduling but found {}",
                  std::string(magic_enum::enum_name(queryState)));
      break;
    }
    }
  ...
  NES_TRACE("checkCompleteOrTimeout: waitForStart expected status is not "
            "reached after timeout");
  return false;
}

bool E2ESingleRun::waitForQueryToStop(
    QueryId queryId, const Catalogs::Query::QueryCatalogPtr &queryCatalog,
    std::chrono::seconds timeoutInSec) {
  NES_TRACE("checkCompleteOrTimeout: Wait until the query {} is stopped",
            queryId);
  ...
  while (std::chrono::system_clock::now() < startTimestamp + timeoutInSec) {
    ...
    if (queryState == QueryState::STOPPED) {
      NES_TRACE(
          "checkStoppedOrTimeout: Status for query with id = {} is stopped",
          queryId);
      return true;
    }
    NES_DEBUG(
        "checkStoppedOrTimeout: Query with id = {} not stopped as status is {}",
        queryId, magic_enum::enum_name(queryState));
    std::this_thread::sleep_for(sleepDuration);
  }
  NES_TRACE("checkCompleteOrTimeout: waitForStop expected status is not "
            "reached after timeout");
  return false;
}

PhysicalSourceTypePtr E2ESingleRun::createPhysicalSourceType(
    std::string logicalSourceName, std::string physicalSourceName,
    std::vector<Runtime::TupleBuffer> &createdBuffers, size_t sourceCnt,
    uint64_t groupId, std::string &generator) {
  if (generator == "YSBKafka") {
    ((void)
         groupId); // We have to do this, as on the macs, we have disabled Kafka
#ifdef ENABLE_KAFKA_BUILD
    ...
    auto kafkaSourceType =
        KafkaSourceType::create(logicalSourceName, physicalSourceName);
    ...
    kafkaSourceType->setNumberOfBuffersToProduce(
    ...
    auto dataProvider = DataProvision::DataProvider::createProvider(
        /* sourceIndex */ sourceCnt, configOverAllRuns, createdBuffers);
    ...
    auto dataProvidingFunc = [this, sourceCnt, generatorQueueIndex](
                                 Runtime::TupleBuffer &buffer, uint64_t) {
      allDataProviders[sourceCnt]->provideNextBuffer(buffer,
                                                     generatorQueueIndex);
    };

    size_t sourceAffinity = std::numeric_limits<uint64_t>::max();
    // TODO #3336: static query manager mode is currently not ported therefore
    // only one queue
    size_t taskQueueId = 0;
    auto sourceConfig = LambdaSourceType::create(
        logicalSourceName, physicalSourceName, dataProvidingFunc,
        configOverAllRuns.numberOfBuffersToProduce->getValue(),
        /* gatheringValue */ 0, GatheringMode::INTERVAL_MODE, sourceAffinity,
        taskQueueId);
...
void E2ESingleRun::collectMeasurements() {
  // We have to measure once more than the required numMeasurementsToCollect as
  // we calculate deltas later on
  for (uint64_t cnt = 0;
       cnt <= configOverAllRuns.numMeasurementsToCollect->getValue(); ++cnt) {
    // Calculate the next start time
    int64_t nextPeriodStartTime =
        configOverAllRuns.experimentMeasureIntervalInSeconds->getValue() * 1000;
    nextPeriodStartTime +=
    ...
    for (auto id : submittedIds) {
      ...
      for (auto &subPlanStatistics : statisticsCoordinator) {
        if (subPlanStatistics->getProcessedBuffers() != 0) {
          ...
          latencySum += (subPlanStatistics->getLatencySum() /
                         subPlanStatistics->getProcessedBuffers());
          queueSizeSum += (subPlanStatistics->getQueueSizeSum() /
                           subPlanStatistics->getProcessedBuffers());
          availGlobalBufferSum +=
              (subPlanStatistics->getAvailableGlobalBufferSum() /
               subPlanStatistics->getProcessedBuffers());
          availFixedBufferSum +=
              (subPlanStatistics->getAvailableFixedBufferSum() /
               subPlanStatistics->getProcessedBuffers());
        }

        printQuerySubplanStatistics(timeStamp, subPlanStatistics,
                                    processedTasks);
        measurements.addNewMeasurement(
            processedTasks, processedBuffers, processedTuples, latencySum,
            queueSizeSum, availGlobalBufferSum, availFixedBufferSum, timeStamp);
      }
    }
...
void E2ESingleRun::printQuerySubplanStatistics(
    uint64_t timestamp, const Runtime::QueryStatisticsPtr &subPlanStatistics,
    size_t processedTasks, std::ostream &outStream) {
  std::stringstream ss;
  ss << "time=" << timestamp
     << " subplan=" << subPlanStatistics->getSubQueryId()
     << " procTasks=" << processedTasks;
  auto lockedPipelineStatistic =
      subPlanStatistics->getPipelineIdToTaskMap().rlock();
  for (auto &pipe : *lockedPipelineStatistic) {
    for (auto &worker : pipe.second) {
      ss << " pipeNo:" << pipe.first << " worker=" << worker.first
         << " tasks=" << worker.second;
    }
  }
