...
void E2ESingleRun::createSources() {
  NES_INFO(
      "Creating sources and the accommodating data generation and data "
      "providing...");
  ...
  NES_INFO(
      "Created sources and the accommodating data generation and data "
      "providing!");
}
    ...
    for (auto id : submittedIds) {
      ...
      for (auto iter : stats) {
        while (iter->getProcessedTuple() < 1) {
          NES_DEBUG(
              "Query with id = {} not ready with no. tuples = {}. Sleeping for "
              "a second now...",
...
E2ESingleRun::E2ESingleRun(E2EBenchmarkConfigPerRun& configPerRun,
    ...
    : configPerRun(configPerRun),
      configOverAllRuns(configOverAllRuns),
      rpcPortSingleRun(rpcPort),
      restPortSingleRun(restPort) {}
...
bool E2ESingleRun::waitForQueryToStart(
  ...
  NES_TRACE(
      "checkCompleteOrTimeout: waitForStart expected status is not reached "
      "after timeout");
  return false;
}
...
bool E2ESingleRun::waitForQueryToStop(
  ...
  NES_TRACE(
      "checkCompleteOrTimeout: waitForStop expected status is not reached "
      "after timeout");
  return false;
}
...
PhysicalSourceTypePtr E2ESingleRun::createPhysicalSourceType(
  ...
  if (generator == "YSBKafka") {
    ((void)groupId);  // We have to do this, as on the macs, we have disabled
                      // Kafka
#ifdef ENABLE_KAFKA_BUILD
