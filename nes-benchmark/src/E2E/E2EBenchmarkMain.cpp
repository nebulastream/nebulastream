...
int main(int argc, const char* argv[]) {
  ...
  if (argc > 3 || argc == 0) {
    std::cerr << "Error: Only --configPath= and --logPath= are allowed as a "
  ...
  if (argMap.size() < 2) {
    std::cerr << "Error: Missing --configPath or --logPath could not been "
                 "found in arguments!"
              << std::endl;
    return -1;
  }
  ...
  if (configPath.empty() || !std::filesystem::exists(configPath)) {
    std::cerr << "No yaml file provided or the file does not exist!"
              << std::endl;
    return -1;
  }

  auto e2EBenchmarkConfig =
      NES::Benchmark::parseYamlConfig(configPath, logPath);
  NES::Benchmark::writeHeaderToCsvFile(
  ...
  for (auto& configPerRun : e2EBenchmarkConfig.getAllConfigPerRuns()) {
    ...
    NES::Benchmark::executeSingleRun(configPerRun,
                                     e2EBenchmarkConfig.getConfigOverAllRuns(),
                                     rpcPort, restPort);
