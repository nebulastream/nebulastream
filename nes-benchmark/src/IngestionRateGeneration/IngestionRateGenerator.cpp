...
IngestionRateGeneratorPtr IngestionRateGenerator::createIngestionRateGenerator(
    E2EBenchmarkConfigOverAllRuns& configOverAllRuns) {
  auto ingestionRateDistributionStr =
      configOverAllRuns.ingestionRateDistribution->getValue();
  auto ingestionRateDistribution =
      getDistributionFromString(ingestionRateDistributionStr);
  ...
  if (ingestionRateDistribution == IngestionRateDistribution::UNIFORM) {
    return std::make_unique<UniformIngestionRateGenerator>(
    ...
    return std::make_unique<CustomIngestionRateGenerator>(ingestionRateCount,
                                                          customValues);
  } else {
...
IngestionRateDistribution IngestionRateGenerator::getDistributionFromString(
  ...
  else if (ingestionRateDistribution == "SINUS" ||
           ingestionRateDistribution == "Sinus" ||
           ingestionRateDistribution == "sinus")
  ...
  else if (ingestionRateDistribution == "CUSTOM" ||
