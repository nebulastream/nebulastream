...
DataProviderPtr DataProvider::createProvider(
  ...
  if (configOverAllRuns.dataProvider->getValue() == "Internal") {
    return std::make_shared<InternalProvider>(providerId, dataProviderMode,
                                              buffers);
  } else if (configOverAllRuns.dataProvider->getValue() == "External") {
    auto ingestionRateGenerator = IngestionRateGeneration::
        IngestionRateGenerator::createIngestionRateGenerator(configOverAllRuns);
    return std::make_shared<ExternalProvider>(
        providerId, dataProviderMode, buffers,
        std::move(ingestionRateGenerator));
  } else {
    NES_THROW_RUNTIME_ERROR("Could not parse dataProvider = "
                            << configOverAllRuns.dataProvider->getValue()
                            << "!");
  }
}

DataProvider::DataProvider(uint64_t id,
                           DataProvider::DataProviderMode providerMode)
    : id(id), providerMode(providerMode) {}

void DataProvider::provideNextBuffer(Runtime::TupleBuffer& buffer,
                                     uint64_t sourceId) {
  ...
  if (providedBuffer.has_value()) {
    switch (providerMode) {
      case DataProviderMode::ZERO_COPY: {
        ...
        auto gcCallback = [dataPtr, this](Runtime::detail::MemorySegment*,
                                          Runtime::BufferRecycler*) {
      ...
      case DataProviderMode::MEM_COPY: {
        std::memcpy(buffer.getBuffer(), providedBuffer.value().getBuffer(),
                    buffer.getBufferSize());
        providedBuffer.value().setCreationTimestampInMS(
