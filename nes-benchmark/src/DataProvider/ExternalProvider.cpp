...
ExternalProvider::ExternalProvider(
  ...
  predefinedIngestionRates =
      this->ingestionRateGenerator->generateIngestionRates();

  uint64_t maxIngestionRateValue = *(std::max_element(
      predefinedIngestionRates.begin(), predefinedIngestionRates.end()));
  bufferQueue = folly::MPMCQueue<TupleBufferHolder>(
      maxIngestionRateValue == 0 ? 1 : maxIngestionRateValue * 1000);
}
...
void ExternalProvider::generateData() {
  ...
  // calculate the number of buffers to produce per working time delta and
  // append to the queue note that the predefined ingestion rates represent the
  // number of buffers to be produced per second, so we multiply them with the
  // working time delta and ingest the resulting number of buffers as quickly as
  // possible into the system, before we wait for the next period to start
  auto workingTimeDeltaInSec = workingTimeDeltaInMillSeconds / 1000.0;
  auto buffersToProducePerWorkingTimeDelta =
      predefinedIngestionRates[0] * workingTimeDeltaInSec;
  NES_ASSERT(buffersToProducePerWorkingTimeDelta > 0,
             "Ingestion rate is too small!");
  ...
  // thread only to block again (see notify_one for details). Taken from
  ...
  while (started) {
    ...
    while (buffersProcessedCount < buffersToProducePerWorkingTimeDelta) {
      ...
      auto wrapBuffer = Runtime::TupleBuffer::wrapMemory(
          buffer.getBuffer(), buffer.getBufferSize(), this);
      ...
      if (!bufferQueue.write(std::move(bufferHolder))) {
        NES_ERROR_OR_THROW_RUNTIME(
            throwException, "The queue is too small! This should not happen!");
      }

      // for the next second, recalculate the number of buffers to produce based
      // on the next predefined ingestion rate
      if (lastSecond != currentSecond) {
        auto nextIndex = ingestionRateIndex % predefinedIngestionRates.size();
        buffersToProducePerWorkingTimeDelta = std::max(
            1.0, predefinedIngestionRates[nextIndex] * workingTimeDeltaInSec);
    ...
    if (nextPeriodStartTime < currentTime) {
      NES_ERROR_OR_THROW_RUNTIME(
          throwException, "The generator cannot produce data fast enough!");
    }
...
ExternalProvider::readNextBuffer(uint64_t sourceId) {
  ...
  if (res) {
    ...
    NES_THROW_RUNTIME_ERROR("This should not happen! An empty buffer was "
                            "returned while provider is started!");
  }
}
