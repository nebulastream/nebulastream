...
InternalProvider::InternalProvider(
    uint64_t id, DataProvider::DataProviderMode providerMode,
...
std::optional<Runtime::TupleBuffer> InternalProvider::readNextBuffer(
    uint64_t sourceId) {
  // For now, we only have a single source
  ((void)sourceId);
  while (!started) {
    // wait with data production until the source is really started and also
    // block if the source gets stopped
    usleep(std::chrono::microseconds(150).count());
  }

  if (!preAllocatedBuffers.empty()) {
    auto buffer = preAllocatedBuffers[currentlyEmittedBuffer %
                                      preAllocatedBuffers.size()];
