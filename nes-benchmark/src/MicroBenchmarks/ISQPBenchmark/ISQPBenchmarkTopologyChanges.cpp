    ...
    for (const auto& file : directory_iterator(querySetLocation)) {
        ...
        for (auto entry = runConfig.Begin(); entry != runConfig.End(); entry++) {
            ...
            for (uint32_t run = 0; run < numberOfRun; run++) {
                ...
                setUp(9, 10, 100, requestHandlerService, topology, globalExecutionPlan);
                ...
                while (isqpEvents.size() < numOfTopologyChangeEvents) {
                    ...
                    auto addLink = RequestProcessor::ISQPAddLinkEvent::create(WorkerId(intermediateNodeId), WorkerId(leafNodeId));
                    ...
                    attachedLeafNodes = baseStationToConnectedWorkerIds[WorkerId(intermediateNodeId)];
