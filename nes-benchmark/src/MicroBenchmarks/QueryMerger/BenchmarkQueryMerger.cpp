...
void setupSources(NesCoordinatorPtr nesCoordinator,
                  uint64_t noOfPhysicalSource) {
  Catalogs::Source::SourceCatalogPtr streamCatalog =
      nesCoordinator->getSourceCatalog();
  ...
  for (uint64_t j = 0; j < numberOfDistinctSources; j++) {
    ...
    if (counter == 1) {
      streamCatalog->addLogicalSource("example" + std::to_string(j + 1),
                                      schema1);
    } else if (counter == 2) {
      streamCatalog->addLogicalSource("example" + std::to_string(j + 1),
                                      schema2);
    ...
    LogicalSourcePtr logicalSource =
        streamCatalog->getLogicalSource("example" + std::to_string(j + 1));
    counter++;
    ...
    for (uint64_t i = 1; i <= noOfPhysicalSource; i++) {
      ...
      auto sce = Catalogs::Source::SourceCatalogEntry::create(
          physicalSource, logicalSource, WorkerId(i));
...
void setUp(const std::string queryMergerRule, uint64_t noOfPhysicalSources) {
  ...
  NES::Testing::BorrowedPortPtr restPort =
      NES::Testing::detail::getPortDispatcher().getNextPort();
  ...
  optimizerConfiguration.queryMergerRule =
      magic_enum::enum_cast<Optimizer::QueryMergerRule>(queryMergerRule)
          .value();
  coordinatorConfig->optimizer = optimizerConfiguration;
  coordinatorConfig->logLevel =
...
void loadConfigFromYAMLFile(const std::string &filePath) {
  ...
  if (!filePath.empty() && std::filesystem::exists(filePath)) {
    try {
      NES_INFO("NesE2EBenchmarkConfig: Using config file with path: {} .",
               filePath);
      Yaml::Node config = *(new Yaml::Node());
      Yaml::Parse(config, filePath.c_str());

      // Number of Measurements to collect
      noOfMeasurementsToCollect =
          config["numberOfMeasurementsToCollect"].As<uint64_t>();
      // Query set location
      querySetLocation = config["querySetLocation"].As<std::string>();
      // Startup sleep interval
      startupSleepIntervalInSeconds =
          config["startupSleepIntervalInSeconds"].As<uint64_t>();
      // Number of distinct sources
      numberOfDistinctSources =
          config["numberOfDistinctSources"].As<uint64_t>();
      // Query merger rules
      queryMergerRules =
          split(config["queryMergerRule"].As<std::string>(), ',');
      // Enable Query Merging
      auto enableQueryMergingOpts =
          split(config["enableQueryMerging"].As<std::string>(), ',');
      ...
      auto configuredNoOfPhysicalSources =
          split(config["noOfPhysicalSources"].As<std::string>(), ',');
      ...
      NES_ERROR("NesE2EBenchmarkConfig: Error while initializing configuration "
                "parameters from YAML file. {}",
                e.what());
    }
    return;
  }
  NES_ERROR("NesE2EBenchmarkConfig: No file path was provided or file could "
            "not be found at {}.",
            filePath);
...
int main(int argc, const char *argv[]) {
  ...
  NES::Logger::setupLogging("BenchmarkQueryMerger.log",
                            NES::LogLevel::LOG_INFO);
  std::cout << "Setup BenchmarkQueryMerger test class." << std::endl;

  std::stringstream benchmarkOutput;
  benchmarkOutput
      << "Time,BM_Name,Merge_Rule,Num_of_Phy_Src,Num_Of_Queries,Num_Of_"
         "SharedQueryPlans,ActualOperator,"
         "SharedOperators,OperatorEfficiency,NES_Version,Run_Num,Start_"
         "Time,End_Time,Total_Run_Time,typeInferencePhase1,"
         "queryRewritePhaseTime,typeInferencePhase2,signatureInferencePhase1,"
         "topologySpecificRewritePhase,typeInferencePhase3,"
         "globalQueryPlanAddition,mergerExecutionPhase"
      << std::endl;
  ...
  for (const auto &file : directory_iterator(querySetLocation)) {
    ...
    if (numThreads == 0) {
      NES_WARNING("No available threads. Going to use only 1 thread for "
    ...
    for (auto queryObject : queryObjects) {
      totalOperators =
          totalOperators + PlanIterator(queryObject).snapshot().size();
    }

    // For the input query set run the experiments with different type of query
    // merger rule
    auto queryIter = 0;
    for (size_t configNum = 0; configNum < queryMergerRules.size();
         configNum++) {
      ...
      for (uint64_t expRun = 1; expRun <= noOfMeasurementsToCollect; expRun++) {
        ...
        NES::RequestHandlerServicePtr requestHandlerService =
            coordinator->getRequestHandlerService();
        ...
        for (uint64_t i = 1; i <= numOfQueries; i++) {
          ...
          requestHandlerService->validateAndQueueAddQueryRequest(
              queryPlan, Optimizer::PlacementStrategy::TopDown);
        }

        // Wait till the status of the last query is set as running
        QueryState lastQueryState;
        while ((lastQueryState = queryCatalog->getQueryState(QueryId(
                    numOfQueries + queryIter))) != QueryState::RUNNING) {
          std::cout << "Query status " << magic_enum::enum_name(lastQueryState)
                    << std::endl;
        ...
        // Fetch the global query plan and count the number of operators
        // produced post merging the queries
        auto gqp = coordinator->getGlobalQueryPlan();
        auto allSQP = gqp->getAllSharedQueryPlans();
        uint64_t mergedOperators = 0;
        for (auto sqp : allSQP) {
          unsigned long planSize =
              PlanIterator(sqp->getQueryPlan()).snapshot().size();
          mergedOperators = mergedOperators + planSize;
        }

        // Compute efficiency
        float efficiency = (((float)totalOperators - (float)mergedOperators) /
                            (float)totalOperators) *
                           100;

        // Add the information in the log
        benchmarkOutput << endTime << "," << file.path().filename() << ","
                        << queryMergerRules[configNum] << ","
                        << noOfPhysicalSources[configNum] << "," << numOfQueries
                        << ","
                        << globalQueryPlan->getAllSharedQueryPlans().size()
                        << "," << totalOperators << "," << mergedOperators
                        << "," << efficiency << "," << NES_VERSION << ","
                        << expRun << "," << startTime << "," << endTime << ","
                        << endTime - startTime << std::endl;
        std::cout << "Finished Run " << expRun << "/"
                  << noOfMeasurementsToCollect << std::endl;
