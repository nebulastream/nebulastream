...
/**
 * This benchmark measures the performance of the output channel. This is done
 * by spawning a zmq server.
 */
static double
BM_TestMassiveSending(uint64_t bufferSize, uint64_t buffersManaged,
                      uint64_t numSenderThreads, uint64_t numServerThreads,
                      uint64_t dataSize, uint32_t port, uint32_t rep) {
  ...
  auto nesPartition =
      Network::NesPartition(SharedQueryId(1 + rep), OperatorId(22 + rep),
                            PartitionId(333 + rep), SubpartitionId(444 + rep));
  try {
    ...
    public:
      ...
      void onDataBuffer(Network::NesPartition id,
                        Runtime::TupleBuffer &buffer) override {
        ((void)id);
        ((void)buffer);
        bufferReceived++;
      }
    ...
    auto netManager = Network::NetworkManager::create(
        WorkerId(0), "127.0.0.1", port,
        Network::ExchangeProtocol(partMgr, std::make_shared<ExchangeListener>(
                                               bufferReceived, completedProm)),
        buffMgr, 16, numServerThreads);

    auto firstBarrier = std::make_shared<NES::ThreadBarrier>(2);
    std::vector<std::thread> allThreads;
    allThreads.emplace_back([&netManager, &nesPartition, totalNumBuffer,
                             bufferSize, &bytesSent, &elapsedSeconds,
      ...
      NES_INFO("Sent {}bytes / {}seconds = throughput {}MiB/s", bytes,
               (elapsed.count() / 1e9), throughput);
    ...
    for (uint64_t thread = 0; thread < numSenderThreads; ++thread) {
      allThreads.emplace_back([totalNumBuffer, &completedProm, nesPartition,
                               bufferSize, &buffMgr, &netManager, port,
                               secondBarrier] {
        ...
        if (senderChannel == nullptr) {
          ...
          for (uint64_t sentBuffer = 0; sentBuffer < totalNumBuffer;
               ++sentBuffer) {
    ...
    throughputRet =
        bytesSent.get_future().get() / elapsedSeconds.get_future().get();
  } catch (...) {
...
int main(int argc, char **argv) {
  ((void)argc);
  ((void)argv);
  NES::Logger::setupLogging("NetworkChannelBenchmark.log",
  ...
  NES::Benchmark::Util::createRangeVectorPowerOfTwo<uint64_t>(
      allBufferSizes, 32 * 1024, 128 * 1024);
  ...
  NES::Benchmark::Util::createRangeVector<uint64_t>(
      allDataSizesToBeSent, 1_u64 * 1024 * 1024 * 1024,
      4_u64 * 1024 * 1024 * 1024, 1_u64 * 1024 * 1024 * 1024);
  ...
  if (!std::filesystem::create_directory(benchmarkFolderName))
    throw NES::Exceptions::RuntimeException("Could not create folder " +
                                            benchmarkFolderName);

  // Writing header to csv file
  std::ofstream benchmarkFile;
  benchmarkFile.open(benchmarkFolderName + "/" + (benchmarkName) +
                         "_results.csv",
                     std::ios_base::app);
  benchmarkFile << "DATASIZE,NUM_SERVER_THREADS,NUM_SENDER_THREADS,BUFFERSIZE,"
                   "BUFFERSMANAGED,NUM_REPS,THROUGHPUT\n";
  benchmarkFile.close();

  uint64_t totalParamCombinations =
      allDataSizesToBeSent.size() * allServerThreads.size() *
      allSenderThreads.size() * allBufferSizes.size();
  uint64_t curComb = 0;
      ...
      for (auto numSenderThreads : allSenderThreads) {
        for (auto bufferSize : allBufferSizes) {
          ...
          for (uint64_t i = 0; i < NUM_REPS; ++i) {
            double throughput = NES::Benchmark::BM_TestMassiveSending(
                bufferSize, buffersManaged, numSenderThreads, numServerThreads,
                dataSize, port, curComb);
          ...
          for (auto throughput : throughputVec) {
            benchmarkFile.open(benchmarkFolderName + "/" + (benchmarkName) +
                                   "_results.csv",
                               std::ios_base::app);
            benchmarkFile << std::to_string(dataSize) << ","
                          << std::to_string(numServerThreads) << ","
                          << std::to_string(numSenderThreads) << ","
                          << std::to_string(bufferSize) << ","
                          << std::to_string(buffersManaged) << ","
                          << std::to_string(NUM_REPS) << ","
                          << std::to_string(throughput) << "\n";
            benchmarkFile.close();
          }

          // After a single experiment was done NUM_REPS, it is now time to
          ...
          std::cout
              << "Performed experiment [" << (++curComb) << "/"
              << totalParamCombinations
              << "] with [size,serverThreads,senderThreads,buffSize,port] ["
              << dataSize << "," << numServerThreads << "," << numSenderThreads
              << "," << bufferSize << "," << port
