...
using namespace NES;
    ...
    void startCoordinator(uint64_t bufferSize, uint64_t numberOfBuffersToProduce, uint64_t numOfOrigins, uint64_t numOfIntermediateNodes) {
...
//        }
        ...
        setupLogicalSources(2, crd->getRequestHandlerService(), numOfIntermediateNodes);
...
//        wrkConf->physicalSourceTypes.add(lambdaSourceType);
    ...
    void setUp(std::atomic<bool>& shouldProduce, uint64_t bufferSize, uint64_t numberOfBuffersToProduce, uint64_t numberOfIntermediateNodes) {
        setupSourceNames(2, numberOfIntermediateNodes);
...
//        auto wrkSources = crd->getNesWorker()->getNodeEngine()->getExecutableQueryPlan(decompPlanIdToCheck, decompPlanVersionToCheck)->getSources();
//        for (auto& source: wrkSources) {
