...
SmartGridDataGenerator::createData(size_t numberOfBuffers, size_t bufferSize) {
  ...
  for (uint64_t currentBuffer = 0; currentBuffer < numberOfBuffers;
    ...
    for (uint64_t currentRecord = 0; currentRecord < testBuffer.getCapacity();
      ...
      testBuffer[currentRecord]["household"].write<int16_t>(
          std::stoi(words[4]));
