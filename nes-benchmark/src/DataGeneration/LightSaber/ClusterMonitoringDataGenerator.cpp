...
std::string ClusterMonitoringDataGenerator::getName() {
  return "ClusterMonitoring";
}

std::vector<Runtime::TupleBuffer> ClusterMonitoringDataGenerator::createData(
    size_t numberOfBuffers, size_t bufferSize) {
  std::vector<Runtime::TupleBuffer> buffers;
  buffers.reserve(numberOfBuffers);

  auto memoryLayout = getMemoryLayout(bufferSize);
  // read input file
  std::ifstream file(std::string(BENCHMARK_DATA_DIRECTORY) +
                     "/cluster-monitoring/google-cluster-data.txt");
  ...
  for (uint64_t currentBuffer = 0; currentBuffer < numberOfBuffers;
       currentBuffer++) {
    auto buffer = allocateBuffer();
    auto testBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer(memoryLayout, buffer);
    for (uint64_t currentRecord = 0; currentRecord < testBuffer.getCapacity();
         currentRecord++) {
      ...
      testBuffer[currentRecord]["creationTS"].write<int64_t>(
          std::stol(words[0]));
      ...
      testBuffer[currentRecord]["machineId"].write<int64_t>(
          std::stol(words[3]));
      testBuffer[currentRecord]["eventType"].write<int16_t>(
          (int16_t)std::stoi(words[4]));
      testBuffer[currentRecord]["userId"].write<int16_t>(
          (int16_t)std::stoi(words[5]));
      testBuffer[currentRecord]["category"].write<int16_t>(
          (int16_t)std::stoi(words[6]));
      testBuffer[currentRecord]["priority"].write<int16_t>(
          (int16_t)std::stoi(words[7]));
      ...
      testBuffer[currentRecord]["constraints"].write<int16_t>(
          (int16_t)std::stoi(words[11]));
      currentLineIndex++;
    }
