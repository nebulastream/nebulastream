...
ManufacturingEquipmentDataGenerator::createData(size_t numberOfBuffers,
                                                size_t bufferSize) {
  ...
  std::ifstream file(std::string(BENCHMARK_DATA_DIRECTORY) +
                     "/manufacturing-equipment/DEBS2012-small.txt");
  std::string line;
  ...
  for (uint64_t currentBuffer = 0; currentBuffer < numberOfBuffers;
    ...
    for (uint64_t currentRecord = 0; currentRecord < testBuffer.getCapacity();
      ...
      testBuffer[currentRecord]["creationTS"].write<int64_t>(
          std::stol(words[0]));
      testBuffer[currentRecord]["messageIndex"].write<int64_t>(
          std::stol(words[1]));
...
ManufacturingEquipmentDataGenerator::getSchemaType() {
