...
Runtime::MemoryLayouts::MemoryLayoutPtr DataGenerator::getMemoryLayout(
    size_t bufferSize) {
...
DataGeneratorPtr DataGenerator::createGeneratorByName(
    std::string type, Yaml::Node generatorNode) {
  ...
  if (type.empty() || type == "Default") {
    ...
    if (generatorNode["minValue"].IsNone() ||
      ...
      NES_THROW_RUNTIME_ERROR(
          "Alpha, minValue and maxValue are necessary for a Uniform "
          "Datagenerator!");
    }
    ...
    if (generatorNode["alpha"].IsNone() || generatorNode["minValue"].IsNone() ||
        generatorNode["maxValue"].IsNone()) {
      NES_THROW_RUNTIME_ERROR(
          "Alpha, minValue and maxValue are necessary for a Zipfian "
          "Datagenerator!");
    }
