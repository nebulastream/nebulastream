...
NexmarkBid::NexmarkBid(uint64_t minValue, uint64_t maxValue)
    : DataGenerator(), minValue(minValue), maxValue(maxValue) {}

std::vector<Runtime::TupleBuffer> NexmarkBid::createData(size_t numberOfBuffers, size_t bufferSize) {
    ...
    for (uint64_t curBuffer = 0; curBuffer < numberOfBuffers; ++curBuffer) {
        ...
        if (curBuffer % noTuplesInFivePercent == 0) {
            NES_INFO("NexmarkBid: currently at " << (((double) curBuffer / numberOfBuffers) * 100) << "%");
        }
...
NES::SchemaPtr NexmarkBid::getSchema() {
...
std::string NexmarkBid::toString() {
