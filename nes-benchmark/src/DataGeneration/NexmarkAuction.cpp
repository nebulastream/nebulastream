...
NexmarkAuction::NexmarkAuction(uint64_t minValue, uint64_t maxValue)
    : DataGenerator(), minValue(minValue), maxValue(maxValue) {}

std::vector<Runtime::TupleBuffer> NexmarkAuction::createData(size_t numberOfBuffers, size_t bufferSize) {
    ...
    for (uint64_t curBuffer = 0; curBuffer < numberOfBuffers; ++curBuffer) {
        ...
        if (memoryLayout->getSchema()->getLayoutType() == Schema::ROW_LAYOUT) {
            ...
            for (uint64_t curRecord = 0; curRecord < dynamicBuffer.getCapacity(); ++curRecord) {
                ...
                dynamicBuffer[curRecord]["itemname"].write<uint64_t>(value);
                dynamicBuffer[curRecord]["description"].write<uint64_t>(curRecord);
                dynamicBuffer[curRecord]["initialbid"].write<uint64_t>(curRecord);
                dynamicBuffer[curRecord]["reserve"].write<uint64_t>(curRecord);
                dynamicBuffer[curRecord]["expires"].write<uint64_t>(curRecord);
                dynamicBuffer[curRecord]["seller"].write<uint64_t>(curRecord);
        ...
        if (curBuffer % noTuplesInFivePercent == 0) {
            NES_INFO("NexmarkAuction: currently at " << (((double) curBuffer / numberOfBuffers) * 100) << "%");
        }
...
NES::SchemaPtr NexmarkAuction::getSchema() {
    return Schema::create()
        ->addField(createField("id", NES::UINT64))
        ->addField(createField("itemname", NES::UINT64))
        ->addField(createField("description", NES::UINT64))
        ->addField(createField("initialbid", NES::UINT64))
        ->addField(createField("reserve", NES::UINT64))
        ->addField(createField("expires", NES::UINT64))
        ->addField(createField("seller", NES::UINT64))
        ->addField(createField("category", NES::UINT64));
}

std::string NexmarkAuction::getName() { return "NexmarkAuction"; }

std::string NexmarkAuction::toString() {
