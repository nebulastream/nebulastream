...
int main(int argc, const char* argv[]) {
  ...
  if (argc != 5) {
    std::cout << "Usage: --broker= --topic= --numberOfPartitions "
                 "--numberOfBuffersToProduce= required as a command line "
                 "argument!\nExiting now..."
              << std::endl;
    return -1;
  }
  ...
  for (int i = 0; i < argc; ++i) {
    ...
    if (pathArg.find("--broker") != std::string::npos) {
      argMap["broker"] =
          pathArg.substr(pathArg.find("=") + 1, pathArg.length() - 1);
    } else if (pathArg.find("--topic") != std::string::npos) {
      argMap["topic"] =
          pathArg.substr(pathArg.find("=") + 1, pathArg.length() - 1);
    } else if (pathArg.find("--numberOfPartitions") != std::string::npos) {
      argMap["numberOfPartitions"] =
          pathArg.substr(pathArg.find("=") + 1, pathArg.length() - 1);
    } else if (pathArg.find("--numberOfBuffersToProduce") !=
               std::string::npos) {
      argMap["numberOfBuffersToProduce"] =
          pathArg.substr(pathArg.find("=") + 1, pathArg.length() - 1);
    } else {
  ...
  uint64_t numberOfBuffersToProduce =
      std::atoi(argMap["numberOfBuffersToProduce"].c_str());
  uint64_t numberOfPartitions = std::atoi(argMap["numberOfPartitions"].c_str());

  std::cout << "start with config broker=" << argMap["broker"]
            << " topic=" << argMap["topic"]
            << " numberOfPartitions=" << argMap["numberOfPartitions"]
            << " numberOfBuffersToProduce="
            << argMap["numberOfBuffersToProduce"] << std::endl;
  ...
  auto bufferManager = std::make_shared<NES::Runtime::BufferManager>(
      bufferSizeInBytes, numberOfBuffersToProduce);

  auto dataGenerator =
      NES::Benchmark::DataGeneration::DataGenerator::createGeneratorByName(
          "YSBKafka", Yaml::Node());
  dataGenerator->setBufferManager(bufferManager);
  auto createdBuffers =
      dataGenerator->createData(numberOfBuffersToProduce, bufferSizeInBytes);
  ...
  for (uint64_t partition = 0; partition < numberOfPartitions; partition++) {
    futures.push_back(std::async(std::launch::async, [&, partition]() {
      std::cout << "start producer thread for partition=" << partition
                << std::endl;
      for (uint64_t prodBuffer = 0; prodBuffer < numberOfBuffersToProduce;
           prodBuffer++) {
        ...
        try {
          producer.produce(cppkafka::MessageBuilder(argMap["topic"])
                               .partition(partition)
                               .payload(bytes));
        ...
        if (prodBuffer % 1000 == 0) {
          std::cout << "number of buffers prod =" << prodBuffer
                    << " for partition=" << partition << std::endl;
        }
      }
    }));
  }
