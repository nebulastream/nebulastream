...
NEBitDataGenerator::createData(size_t numberOfBuffers, size_t bufferSize) {
  ...
  for (uint64_t currentBuffer = 0; currentBuffer < numberOfBuffers;
    ...
    for (uint64_t currentRecord = 0; currentRecord < testBuffer.getCapacity();
      ...
      if (rand() % 100 > NexmarkCommon::HOT_AUCTIONS_PROB) {
        auction = (((epoch * NexmarkCommon::AUCTION_EVENT_RATIO +
                     NexmarkCommon::AUCTION_EVENT_RATIO - 1) /
                    NexmarkCommon::HOT_AUCTION_RATIO) *
                   NexmarkCommon::HOT_AUCTION_RATIO);
      } else {
        long a =
            std::max(0L, epoch * NexmarkCommon::AUCTION_EVENT_RATIO +
                             NexmarkCommon::AUCTION_EVENT_RATIO - 1 - 20000);
        long b = epoch * NexmarkCommon::AUCTION_EVENT_RATIO +
                 NexmarkCommon::AUCTION_EVENT_RATIO - 1;
        auction = a + rand() % (b - a + 1 + 100);
      }

      if (rand() % 100 > 85) {
        long personId = epoch * NexmarkCommon::PERSON_EVENT_RATIO +
                        NexmarkCommon::PERSON_EVENT_RATIO - 1;
        bidder = (personId / NexmarkCommon::HOT_SELLER_RATIO) *
