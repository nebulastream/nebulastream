...
std::vector<Runtime::TupleBuffer> NEAuctionDataGenerator::createData(
  ...
  for (uint64_t currentBuffer = 0; currentBuffer < numberOfBuffers;
    ...
    for (uint64_t currentRecord = 0; currentRecord < testBuffer.getCapacity();
      ...
      long auctionId = epoch * NexmarkCommon::AUCTION_EVENT_RATIO +
                       offset;  // r.nextLong(minAuctionId, maxAuctionId);
      ...
      if (rand() % 100 > 85) {
        ...
        matchingPerson = (personId / NexmarkCommon::HOT_SELLER_RATIO) *
                         NexmarkCommon::HOT_SELLER_RATIO;
      } else {
