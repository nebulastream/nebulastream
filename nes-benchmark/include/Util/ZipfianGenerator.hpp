...
class ZipfianGenerator {
 ...
 public:
   ...
   * @param zipfianFactor
   */
  explicit ZipfianGenerator(uint64_t min, uint64_t max,
                            double zipfianFactor = DEFAULT_ZIPFIAN_GENERATOR)
      : ZipfianGenerator(min, max, zipfianFactor,
                         computeZeta(0, max - min + 1, zipfianFactor, 0)) {}

  explicit ZipfianGenerator(uint64_t min, uint64_t max, double zipfianConstant,
                            double zetan_)
      : dist(0.0, 1.0) {
    numItems = max - min + 1;
    this->min = min;
  ...
  /**
   * @brief overrides the operator() and generates a new random value drawn from
  ...
  uint64_t operator()(std::mt19937& rng, uint64_t newItemCount) {
    if (newItemCount > countForZeta) {
      ...
      eta =
          (1 - std::pow(2.0 / numItems, 1 - theta)) / (1 - zeta2Theta / zetan);
    }
    double u = dist(rng);
    double uz = u * zetan;
    if (uz < 1.0) {
 ...
 private:
  /**
   * @brief this function calculates the zeta constant needed for the
   * distribution
   * @param st
   * @param n
   * @param thetaVal
  ...
  /**
   * @brief Computes the zeta function by iterating over all numbers in the
   * range of st .. n
   * @param st
   * @param n
   * @param theta
   ...
   */
  static double computeZeta(uint64_t st, uint64_t n, double theta,
                            double initialSum) {
