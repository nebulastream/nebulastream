...
class ZipfianDataGenerator : public DataGenerator {
 public:
  explicit ZipfianDataGenerator(double alpha, uint64_t minValue,
                                uint64_t maxValue);

  /**
   * @brief creates Zipfian data with the schema "id, value, payload, timestamp"
   * the id, payload, and timestamp are just counters that increment whereas the
   * value gets drawn randomly from a Zipfian distribution in the range
   * [minValue, maxValue]
