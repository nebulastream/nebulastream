...
class Measurements {
 public:
   ...
   */
  void addNewMeasurement(size_t processedTasks, size_t processedBuffers,
                         size_t processedTuples, size_t latencySum,
                         size_t queueSizeSum, size_t availGlobalBufferSum,
  ...
  /**
   * @brief returns the measurements and calculates the deltas of the
   * measurements and then returns them as comma separated values
 ...
 private:
   ...
   * @param processedTasks
   */
  void addNewProcessedTasks(size_t timestamp, size_t processedTasks) {
    allProcessedTasks[timestamp] += processedTasks;
  }
   ...
   * @param processedBuffers
   */
  void addNewProcessedBuffers(size_t timestamp, size_t processedBuffers) {
    allProcessedBuffers[timestamp] += processedBuffers;
  }
   ...
   * @param processedTuples
   */
  void addNewProcessedTuples(size_t timestamp, size_t processedTuples) {
    allProcessedTuples[timestamp] += processedTuples;
  }
   ...
   * @param newLatencySum
   */
  void addNewLatencySum(size_t timestamp, size_t latencySum) {
    allLatencySum[timestamp] += latencySum;
  }
   ...
   * @param queueSizeSum
   */
  void addNewQueueSizeSum(size_t timestamp, size_t queueSizeSum) {
    allQueueSizeSums[timestamp] += queueSizeSum;
  }
   ...
   * @param availGlobalBufferSum
   */
  void addNewAvailGlobalBufferSum(size_t timestamp,
