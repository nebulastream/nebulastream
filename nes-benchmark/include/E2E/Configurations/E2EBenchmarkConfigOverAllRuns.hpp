...
public:
  ...
  public:
    E2EBenchmarkQueryConfig(std::string queryString,
                            uint32_t customDelayInSeconds)
        : queryString(std::move(queryString)),
          customDelayInSeconds(customDelayInSeconds) {}
    friend std::ostream &operator<<(std::ostream &os,
                                    const E2EBenchmarkQueryConfig &config);
  ...
  /**
   * @brief creates a E2EBenchmarkConfigPerRun object and sets the default
   * values
   */
   ...
   * @return
   */
  static E2EBenchmarkConfigOverAllRuns
  generateConfigOverAllRuns(Yaml::Node yamlConfig);
...
public:
  ...
  std::map<std::string, DataGeneration::DataGeneratorPtr>
      sourceNameToDataGenerator;
