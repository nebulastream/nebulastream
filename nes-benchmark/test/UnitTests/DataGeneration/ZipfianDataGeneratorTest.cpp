...
class ZipfianDataGeneratorTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ZipfianDataGeneratorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down ZipfianDataGeneratorTest test class.");
  }
};

/**
 * @brief Testing if ZipfianDataGenerator::getSchema() works by comparing versus
 * a hardcoded truth
 */
TEST_F(ZipfianDataGeneratorTest, getSchemaTest) {
  ...
  auto zipfianDataGenerator =
      std::make_unique<ZipfianDataGenerator>(alpha, minValue, maxValue);
...
/**
 * @brief Testing if ZipfianDataGenerator::getName() works by comparing versus a
 * hardcoded truth
 */
...
/**
 * @brief Testing if ZipfianDataGenerator::toString() works by comparing versus
 * a hardcoded truth
 */
TEST_F(ZipfianDataGeneratorTest, toStringTest) {
  ...
  oss << zipfianDataGenerator->getName() << " (" << minValue << ", " << maxValue
      << ", " << alpha << ")";
...
/**
 * @brief Testing if ZipfianDataGenerator::createData() works by creating tuples
 * and then comparing the expected tupleBuffers with the created one's from the
 * ZipfianDataGenerator
 */
TEST_F(ZipfianDataGeneratorTest, createDataTest) {
  ...
  auto dataDefault = zipfianDataGenerator->createData(
  ...
  auto memoryLayout =
      zipfianDataGenerator->getMemoryLayout(bufferManager->getBufferSize());
