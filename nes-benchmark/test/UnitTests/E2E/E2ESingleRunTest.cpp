...
TEST_F(E2ESingleRunTest, createE2ESingleRun) {
  ...
  ASSERT_NO_THROW(E2ESingleRun singleRun(configPerRun, configOverAllRuns,
                                         *rpcCoordinatorPort, *restPort));
}

/**
 * @brief Testing if E2ESingleRun::setupCoordinatorConfig() is correct by
 *comparing with a hardcoded truth
 **/
TEST_F(E2ESingleRunTest, setUpCoordinatorAndWorkerConfig) {
  ...
  E2ESingleRun singleRun(configPerRun, configOverAllRuns,
                         rpcCoordinatorPortSaved, restPortSaved);
  ...
  ASSERT_EQ(coordinatorConf->worker.numWorkerThreads.getValue(),
            configPerRun.numberOfWorkerThreads->getValue());
  ASSERT_EQ(coordinatorConf->worker.bufferSizeInBytes.getValue(),
...
/**
 * @brief Testing if E2ESingleRun::createSources() is correct by creating
 *several configPerRuns and then comparing versus a hardcoded truth
 **/
TEST_F(E2ESingleRunTest, createSources) {
  E2EBenchmarkConfigOverAllRuns configOverAllRuns;
  std::vector<E2EBenchmarkConfigPerRun> allConfigPerRuns;

  auto defaultDataGenerator =
      std::make_unique<DataGeneration::DefaultDataGenerator>(0, 1000);
  auto zipfianDataGenerator =
  ...
  configOverAllRuns.sourceNameToDataGenerator[defaultDataGeneratorName] =
      std::move(defaultDataGenerator);
  configOverAllRuns.sourceNameToDataGenerator[zipfianDataGeneratorName] =
      std::move(zipfianDataGenerator);
  configOverAllRuns.numberOfPreAllocatedBuffer->setValue(10);

  for (auto i = 0; i < 3; ++i) {
    E2EBenchmarkConfigPerRun configPerRun;
    configPerRun.logicalSrcToNoPhysicalSrc = {
        {defaultDataGeneratorName, i + 1}, {zipfianDataGeneratorName, i + 2}};
  ...
  for (auto cnt = 0UL; cnt < allConfigPerRuns.size(); ++cnt) {
    E2ESingleRun singleRun(allConfigPerRuns[cnt], configOverAllRuns,
                           *rpcCoordinatorPort, *restPort);
    singleRun.setupCoordinatorConfig();
    ...
    ASSERT_EQ(coordinatorConf->logicalSourceTypes[0]
                  .getValue()
                  ->getLogicalSourceName(),
              defaultDataGeneratorName);
    ASSERT_EQ(coordinatorConf->logicalSourceTypes[1]
                  .getValue()
                  ->getLogicalSourceName(),
              zipfianDataGeneratorName);
    ASSERT_EQ(coordinatorConf->worker.physicalSourceTypes.size(),
              cnt + 1 + cnt + 2);

    std::map<std::string, uint64_t> tmpMap{{defaultDataGeneratorName, 0},
                                           {zipfianDataGeneratorName, 0}};
    for (auto i = 0UL; i < coordinatorConf->worker.physicalSourceTypes.size();
         ++i) {
...
/**
 * @brief Testing if
 *E2ESingleRun::getStringLogicalSourceToNumberOfPhysicalSources() is correct by
 *comparing versus a hardcoded truth
 **/
TEST_F(E2ESingleRunTest, getNumberOfPhysicalSources) {
  ...
  E2EBenchmarkConfigPerRun configPerRun;
  configPerRun.logicalSrcToNoPhysicalSrc = {
      {defaultDataGenerator->getName(), 123},
      {zipfianDataGenerator->getName(), 456}};

  E2EBenchmarkConfigOverAllRuns configOverAllRuns;
  E2ESingleRun singleRun(configPerRun, configOverAllRuns, *rpcCoordinatorPort,
                         *restPort);
  ...
  ASSERT_EQ(assertedString.str(),
            configPerRun.getStringLogicalSourceToNumberOfPhysicalSources());
}

/**
 * @brief Testing if E2ESingleRun::writeHeaderToCsvFile() is correct by creating
 *multiple different measurements and then compare the written csv file to a
 *hardcoded truth
 **/
TEST_F(E2ESingleRunTest, writeMeasurementsToCSV) {
  auto bmName = "Some awesome BM Name", inputType = "Auto",
       dataProviderMode = "ZeroCopy";
  auto queryString = "Query::from(source)", csvFile = "tmp.csv";
  auto numberOfWorkerThreads = 12, numberOfQueriesToDeploy = 123,
       bufferSizeInBytes = 8 * 1024;
  auto defaultDataGenerator =
      std::make_unique<DataGeneration::DefaultDataGenerator>(0, 1000);
  auto zipfianDataGenerator =
      std::make_unique<DataGeneration::ZipfianDataGenerator>(0.8, 0, 1000);
  ...
  auto schemaSizeInB =
      defaultDataGenerator->getSchema()->getSchemaSizeInBytes() +
  ...
  configOverAllRuns.queries.emplace_back(
      E2EBenchmarkConfigOverAllRuns::E2EBenchmarkQueryConfig(queryString, 0));
  ...
  assertedCsvFile
      << "BenchmarkName,NES_VERSION,SchemaSize,timestamp,processedTasks,"
  ...
  for (auto i = 0; i < MAX_TIMESTAMP; ++i) {
    ...
    measurements.addNewMeasurement(
        processedTasksStart + i, processedBuffersStart + i,
        processedTuplesStart + i, latencySumStart + i, queueSizeSumStart + i,
        availGlobalBufferSumStart + i, availFixedBufferSumStart + i, timeStamp);

    if (i < MAX_TIMESTAMP - 1) {
      assertedCsvFile
          << "\"" << bmName << "\""
          << "," << NES_VERSION << "," << schemaSizeInB << "," << timeStamp
          ...
          << "," << 1 << "," << 1 << "," << 1 << ","
          << (1.0 * schemaSizeInB) / (1024.0 * 1024.0) << ","
  ...
  std::string writtenCsvFile((std::istreambuf_iterator<char>(ifs)),
