...
class E2EBenchmarkConfigOverAllRunsTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("E2EBenchmarkConfigOverAllRunsTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down E2EBenchmarkConfigOverAllRunsTest test class.");
  }
};

/**
 * @brief Testing if E2EBenchmarkConfigOverAllRuns::toStringTest() is correct by
...
/**
 * @brief Testing if E2EBenchmarkConfigOverAllRuns::generateConfigOverAllRuns()
 * is correct by parsing a yaml file and comparing against a hardcoded truth
 */
TEST_F(E2EBenchmarkConfigOverAllRunsTest,
       generateConfigOverAllRunsInternalProviderTest) {
  E2EBenchmarkConfigOverAllRuns defaultConfigOverAllRuns;
  Yaml::Node yamlConfig;

  std::string configPath =
      std::string(TEST_CONFIGS_DIRECTORY) + "/filter_one_source.yaml";
  Yaml::Parse(yamlConfig, configPath.c_str());

  defaultConfigOverAllRuns =
      E2EBenchmarkConfigOverAllRuns::generateConfigOverAllRuns(yamlConfig);

  ASSERT_EQ(defaultConfigOverAllRuns.startupSleepIntervalInSeconds->getValue(),
            1);
  ASSERT_EQ(defaultConfigOverAllRuns.numMeasurementsToCollect->getValue(), 1);
  ASSERT_EQ(
      defaultConfigOverAllRuns.experimentMeasureIntervalInSeconds->getValue(),
      1);
  ASSERT_EQ(defaultConfigOverAllRuns.outputFile->getValue(),
            "FilterOneSource.csv");
  ASSERT_EQ(defaultConfigOverAllRuns.benchmarkName->getValue(),
            "FilterOneSource");
  ASSERT_EQ(
  ...
  ASSERT_EQ(defaultConfigOverAllRuns.numberOfPreAllocatedBuffer->getValue(),
            100);
  ...
  ASSERT_EQ(defaultConfigOverAllRuns.ingestionRateDistribution->getValue(),
            "Uniform");
  ...
  ASSERT_EQ(defaultConfigOverAllRuns.getStrLogicalSrcDataGenerators(),
            "input1: YSB");
}

/**
 * @brief Testing if E2EBenchmarkConfigOverAllRuns::generateConfigOverAllRuns()
 * is correct by parsing a yaml file with a dynamic ingestion rate and comparing
 * against a hardcoded truth
 */
TEST_F(E2EBenchmarkConfigOverAllRunsTest,
       dynamicGenerateConfigOverAllRunsTest) {
  E2EBenchmarkConfigOverAllRuns defaultConfigOverAllRuns;
  Yaml::Node yamlConfig;

  std::string configPath = std::string(TEST_CONFIGS_DIRECTORY) +
                           "/filter_with_dynamic_ingestion_rate.yaml";
  Yaml::Parse(yamlConfig, configPath.c_str());
  ...
  ASSERT_EQ(
      ...
      1);
  ASSERT_EQ(defaultConfigOverAllRuns.outputFile->getValue(),
            "FilterWithDynamicIngestionRate.csv");
  ASSERT_EQ(defaultConfigOverAllRuns.benchmarkName->getValue(),
            "FilterWithDynamicIngestionRate");
  ASSERT_EQ(
      defaultConfigOverAllRuns.queries[0].getQueryString(),
  ...
  ASSERT_EQ(defaultConfigOverAllRuns.numberOfPreAllocatedBuffer->getValue(),
            10000000);
  ASSERT_EQ(defaultConfigOverAllRuns.batchSize->getValue(), 1);
  ASSERT_EQ(defaultConfigOverAllRuns.numberOfBuffersToProduce->getValue(),
            5000000);
...
/**
 * @brief Testing if E2EBenchmarkConfigOverAllRuns::generateConfigOverAllRuns()
 * is correct by parsing a yaml file with a custom ingestion rate and comparing
 * against a hardcoded truth
 */
TEST_F(E2EBenchmarkConfigOverAllRunsTest, customGenerateConfigOverAllRunsTest) {
  ...
  std::string configPath = std::string(TEST_CONFIGS_DIRECTORY) +
                           "/filter_with_custom_ingestion_rate.yaml";
  Yaml::Parse(yamlConfig, configPath.c_str());
  ...
  ASSERT_EQ(
      ...
      1);
  ASSERT_EQ(defaultConfigOverAllRuns.outputFile->getValue(),
            "FilterWithCustomIngestionRate.csv");
  ASSERT_EQ(defaultConfigOverAllRuns.benchmarkName->getValue(),
            "FilterWithCustomIngestionRate");
  ASSERT_EQ(
      defaultConfigOverAllRuns.queries[0].getQueryString(),
      R"(Query::from("input1").filter(Attribute("value") < 100).sink(NullOutputSinkDescriptor::create());)");
  ASSERT_EQ(defaultConfigOverAllRuns.queries[0].getCustomDelayInSeconds(), 0);
...
/**
 * @brief Testing if E2EBenchmarkConfigOverAllRuns::generateConfigOverAllRuns()
 * is correct by parsing a yaml file with concurrent queries and comparing
 * against a hardcoded truth
 */
TEST_F(E2EBenchmarkConfigOverAllRunsTest,
       generateConfigOverAllRunsConcurrentQueriesTest) {
  E2EBenchmarkConfigOverAllRuns defaultConfigOverAllRuns;
  Yaml::Node yamlConfig;

  std::string configPath = std::string(TEST_CONFIGS_DIRECTORY) +
                           "/e2e_concurrent_queries_test_config.yaml";
  Yaml::Parse(yamlConfig, configPath.c_str());
...
/**
 * @brief Testing if E2EBenchmarkConfigOverAllRuns::getTotalSchemaSize() is
...
/**
 * @brief Testing if
 * E2EBenchmarkConfigOverAllRuns::getStrLogicalSrcDataGenerators() is correct by
 * comparing against a hardcoded truth
 */
TEST_F(E2EBenchmarkConfigOverAllRunsTest, getStrLogicalSrcDataGeneratorsTest) {
  ...
  auto defaultString =
      defaultConfigOverAllRuns.getStrLogicalSrcDataGenerators();
