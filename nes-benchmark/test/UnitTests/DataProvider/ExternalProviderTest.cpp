...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ExternalProviderTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down ExternalProviderTest test class.");
  }
...
/**
 * @brief This test should not be run on the CI, as here we use a sleep to
 * generate x amount of buffers and then compare to an expected. This might fail
 * randomly as the CI is not fast enough to produce large amounts of buffers
 */
...
TEST_F(ExternalProviderTest, DISABLED_uniformIngestionRateTest) {
  ...
  auto externalProviderDefault =
      std::dynamic_pointer_cast<ExternalProvider>(DataProvider::createProvider(
  ...
  // we expect queueSize to not exactly match the theoretical value of 50000 as
  // we cannot ensure that exactly 1 second has passed between starting the
  // provider and getting the queue size
  NES_INFO("{}", queueSize);
  ASSERT_TRUE(50000 <= queueSize && queueSize <= 50500);
}

/**
 * @brief Testing if ExternalProvider::readNextBuffer() works by creating
...
TEST_F(ExternalProviderTest, readNextBufferRowLayoutTest) {
  ...
  auto externalProviderDefault =
      std::dynamic_pointer_cast<ExternalProvider>(DataProvider::createProvider(
          sourceId, configOverAllRuns, createdBuffers));
  /* We do not want to throw an exception in our test environment regarding
   * timing, because this could induce randomly failing tests due to the CI
   * being overwhelmed
   */
  ...
  ASSERT_TRUE(memcmp(defaultBuffer, expectedBuffer,
...
/**
 * @brief Testing if ExternalProvider::start() works by creating tupleBuffers
 * and then checks if the thread can be joined for row layout
 */
...
/**
 * @brief Testing if ExternalProvider::start() works by creating tupleBuffers
 * and then checks if the thread can be joined for column layout
 */
