...
protected:
    ...
    LogicalOperator makeSource(const std::string& sourceType, std::optional<std::string> channel = std::nullopt)
    {
        const auto logicalSource
            = sourceCatalog.addLogicalSource(Identifier::parse(fmt::format("S{}", ++counter)), Schema<UnqualifiedUnboundField, Ordered>{})
                  .value();
        std::unordered_map<Identifier, std::string> cfg;
        if (channel)
        {
            cfg.emplace(Identifier::parse("CHANNEL"), *channel);
            cfg.emplace(Identifier::parse("DATA_ENDPOINT"), "somewhere:9090");
        }
        else
        {
            cfg.emplace(Identifier::parse("FILE_PATH"), "i am a file");
        }
        const std::unordered_map<Identifier, std::string> parser = {
            {Identifier::parse("TYPE"), "CSV"}, {Identifier::parse("TUPLE_DELIMITER"), "\n"}, {Identifier::parse("FIELD_DELIMITER"), ","}};
        auto descriptor
            = sourceCatalog.addPhysicalSource(logicalSource, Identifier::parse(sourceType), Host("localhost"), std::move(cfg), parser)
                  .value();
        return SourceDescriptorLogicalOperator::create(std::move(descriptor));
    }

    TypedLogicalOperator<SinkLogicalOperator> makeSink(const std::string& sinkType, std::optional<std::string> channel = std::nullopt)
    {
        std::unordered_map<Identifier, std::string> cfg;
        if (channel)
        {
        ...
        auto descriptor = sinkCatalog
                              .addSinkDescriptor(
                                  Identifier::parse(fmt::format("snk{}", ++counter)),
                                  Schema<UnqualifiedUnboundField, Ordered>{},
                                  Identifier::parse(sinkType),
                                  Host("localhost"),
                                  std::move(cfg),
                                  {})
                              .value();
        return SinkLogicalOperator::create(descriptor);
    }

    static QueryId nextQueryId() { return QueryId::createLocal(LocalQueryId(generateUUID())); }

    LogicalPlan planWith(QueryId id, TypedLogicalOperator<SinkLogicalOperator> sink, LogicalOperator source)
    {
        return LogicalPlan(std::move(id), {LogicalOperator{sink.withChildren({std::move(source)})}});
    }

    LogicalPlan planWith(QueryId id, TypedLogicalOperator<SinkLogicalOperator> sink, std::vector<LogicalOperator> sources)
    {
...
TEST_F(DistributedLogicalPlanTest, ProducerBeforeConsumerAcrossNetworkChannel)
{
    ...
    EXPECT_THAT(idsOf(waves), ElementsAre(UnorderedElementsAre(producerId), UnorderedElementsAre(consumerId)));
}
...
TEST_F(DistributedLogicalPlanTest, MixedSourcesIntermediateOrdersAfterAllNetworkInputsReady)
{
    ...
    const auto producerPlan = planWith(producerId, makeSink("NETWORK", "c1"), makeSource("File"));
    const auto intermediatePlan = planWith(intermediateId, makeSink("NETWORK", "c2"), {makeSource("File"), makeSource("NETWORK", "c1")});
    const auto consumerPlan = planWith(consumerId, makeSink("Print"), makeSource("NETWORK", "c2"));

    const auto waves = drain({{producerHost, {producerPlan}}, {intermediateHost, {intermediatePlan}}, {consumerHost, {consumerPlan}}});

    EXPECT_THAT(
        idsOf(waves),
        ElementsAre(UnorderedElementsAre(producerId), UnorderedElementsAre(intermediateId), UnorderedElementsAre(consumerId)));
}
...
TEST_F(DistributedLogicalPlanTest, IntermediateWaitsForAllNetworkInputs)
{
    ...
    EXPECT_THAT(idsOf(waves), ElementsAre(UnorderedElementsAre(idA, idB), UnorderedElementsAre(idJoin)));
}
...
TEST_F(DistributedLogicalPlanTest, DiamondTopology)
{
    ...
    const auto sinkPlan = planWith(sinkId, makeSink("Print"), {makeSource("NETWORK", "left"), makeSource("NETWORK", "right")});

    const auto waves = drain({{rootHost, {rootPlan}}, {leftHost, {leftPlan}}, {rightHost, {rightPlan}}, {sinkHost, {sinkPlan}}});

    EXPECT_THAT(
        idsOf(waves), ElementsAre(UnorderedElementsAre(rootId), UnorderedElementsAre(leftId, rightId), UnorderedElementsAre(sinkId)));
}
...
TEST_F(DistributedLogicalPlanTest, MultiplePlansPerHostOrderedIndependently)
{
    ...
    EXPECT_THAT(idsOf(waves), ElementsAre(UnorderedElementsAre(producerId), UnorderedElementsAre(relayId), UnorderedElementsAre(finalId)));
}
