...
Field Unreflector<Field>::operator()(const Reflected& reflected, const ReflectionContext& context) const
{
    ...
    auto fromOperatorOpt = operatorMapping->getOperator(id);
    if (!fromOperatorOpt)
    {
...
// std::function<UnqualifiedUnboundField(Field)> Field::unbinder()
// {
//     return [](const Field& field) { return UnqualifiedUnboundField{field.name, field.dataType}; };
// }
//
// std::function<Field(UnqualifiedUnboundField)> Field::binder(LogicalOperator logicalOperator)
// {
//     return [logicalOperator](const UnqualifiedUnboundField& unboundField)
