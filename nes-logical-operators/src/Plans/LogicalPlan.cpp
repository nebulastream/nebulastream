...
LogicalPlan LogicalPlan::withAdditionalRoots(const std::vector<LogicalOperator>& operators) const
{
    auto copy = *this;
    copy.rootOperators.insert(copy.rootOperators.end(), operators.begin(), operators.end());
    return copy;
}

LogicalPlan::LogicalPlan(const LogicalPlan& other)
    : queryId(other.queryId), rootOperators(other.rootOperators), originalSql(other.originalSql)
{
}
