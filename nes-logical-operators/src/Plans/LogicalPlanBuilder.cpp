...
LogicalPlan LogicalPlanBuilder::createLogicalPlan(Identifier logicalSourceName)
{
    ...
    return LogicalPlan(INVALID_QUERY_ID, {TypedLogicalOperator<SourceNameLogicalOperator>{std::move(logicalSourceName)}});
}
    ...
    const LogicalPlan& queryPlan)
{
    return promoteOperatorToRoot(
        queryPlan,
        TypedLogicalOperator<InlineSinkLogicalOperator>{std::move(type), std::move(schema), std::move(sinkConfig), std::move(formatConfig)});
}
