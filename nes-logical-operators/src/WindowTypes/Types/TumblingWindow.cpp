...
{
    return reflect(ReflectedTumblingWindow{.size = tumblingWindow.getSize(), .timeCharacteristic = tumblingWindow.getTimeCharacteristic()});
}

Windowing::TumblingWindow Unreflector<Windowing::TumblingWindow>::operator()(const Reflected& reflected) const
{
    auto [size, timeCharacteristics] = unreflect<ReflectedTumblingWindow>(reflected);
