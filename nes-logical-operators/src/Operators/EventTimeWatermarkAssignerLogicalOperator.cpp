...
EventTimeWatermarkAssignerLogicalOperator::EventTimeWatermarkAssignerLogicalOperator(
    WeakLogicalOperator self, LogicalOperator child, LogicalFunction onField, const Windowing::TimeUnit& unit): self(std::move(self)), unit(unit), onField(std::move(onField)), child(std::move(child))
{
    inferLocalSchema();
}
...
Reflected Reflector<TypedLogicalOperator<EventTimeWatermarkAssignerLogicalOperator>>::operator()(const TypedLogicalOperator<EventTimeWatermarkAssignerLogicalOperator>& op) const
{
    return reflect(detail::ReflectedEventTimeWatermarkAssignerLogicalOperator{.operatorId = op.getId(), .onField = op->getOnField(), .timeUnit = op->getUnit()});
}

Unreflector<TypedLogicalOperator<EventTimeWatermarkAssignerLogicalOperator>>::Unreflector(ContextType operatorMapping): plan(std::move(operatorMapping))
{
}
TypedLogicalOperator<EventTimeWatermarkAssignerLogicalOperator>
Unreflector<TypedLogicalOperator<EventTimeWatermarkAssignerLogicalOperator>>::operator()(const Reflected& reflected, const ReflectionContext& context) const
{
