...
SourceNameLogicalOperator::SourceNameLogicalOperator(WeakLogicalOperator self, Identifier logicalSourceName) : self(std::move(self)), logicalSourceName(std::move(logicalSourceName))
{
}
...
Schema<Field, Unordered> SourceNameLogicalOperator::getOutputSchema() const
{
...
Reflected Reflector<TypedLogicalOperator<SourceNameLogicalOperator>>::operator()(const TypedLogicalOperator<SourceNameLogicalOperator>&) const
{
...
TypedLogicalOperator<SourceNameLogicalOperator> Unreflector<TypedLogicalOperator<SourceNameLogicalOperator>>::operator()(const Reflected&, const ReflectionContext&) const
{
    PRECONDITION(false, "no serialize for SourceNameLogicalOperator defined. Serialization happens with SourceDescriptorLogicalOperator");
    std::unreachable();
}
