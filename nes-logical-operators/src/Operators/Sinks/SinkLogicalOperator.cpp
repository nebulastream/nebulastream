...
LogicalOperator SinkLogicalOperator::withInferredSchema(std::vector<Schema> inputSchemas) const
{
    ...
    {
        throw CannotInferSchema("The schema of the sink must be equal to the schema of the input");
    }
    return copy;
}
...
{
    return sinkDescriptor.has_value() ? *sinkDescriptor->getSchema() : Schema{};
}
