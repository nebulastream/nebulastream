...
{
    return TypedLogicalOperator<SinkLogicalOperator>{std::move(sinkDescriptor)};
}

TypedLogicalOperator<SinkLogicalOperator> SinkLogicalOperator::create(const SinkDescriptor& sinkDescriptor, LogicalOperator child)
{
    return TypedLogicalOperator<SinkLogicalOperator>{std::move(sinkDescriptor), std::move(child)};
}
...
Unreflector<TypedLogicalOperator<SinkLogicalOperator>>::operator()(const Reflected& reflected, const ReflectionContext& context) const
{
    ...
    if (descriptor.has_value())
    {
        ...
        return SinkLogicalOperator::create(descriptor.value(), std::move(children.at(0)));
    }
