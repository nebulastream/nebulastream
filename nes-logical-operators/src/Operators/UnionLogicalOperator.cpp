...
namespace NES
{
UnionLogicalOperator::UnionLogicalOperator(WeakLogicalOperator self) : self(std::move(self))
{
}

UnionLogicalOperator::UnionLogicalOperator(WeakLogicalOperator self, std::vector<LogicalOperator> children) : self(std::move(self)), children(std::move(children))
{
...
Schema<Field, Unordered> UnionLogicalOperator::getOutputSchema() const
{
    ...
    return bind(self.lock(), outputSchema.value());
}
...
{
    INVARIANT(children.size() >= 1, "Children not set when trying to get ordered output schema");

    return bind(self.lock(), unbind(orderProvider(children.at(0))));
}
...
TypedLogicalOperator<UnionLogicalOperator> Unreflector<TypedLogicalOperator<UnionLogicalOperator>>::operator()(const Reflected& reflected, const ReflectionContext& context) const
{
    auto id = context.unreflect<OperatorId>(reflected);
    return TypedLogicalOperator<UnionLogicalOperator>{plan->getChildrenFor(id, context)};
}
}
