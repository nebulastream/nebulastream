...
namespace
{
SchemaBase<1, UnboundFieldBase<1>, false> inferOutputSchema(const std::vector<LogicalOperator>& children)
{
    auto inputSchemas
...
UnionLogicalOperator::UnionLogicalOperator(WeakLogicalOperator self, std::vector<LogicalOperator> children) : self(std::move(self))
{
    PRECONDITION(!children.empty(), "Union expects at least one child");

    this->children = std::move(children);
    // Validate schema compatibility and infer output schema
    this->outputSchema = inferOutputSchema(children);
}
