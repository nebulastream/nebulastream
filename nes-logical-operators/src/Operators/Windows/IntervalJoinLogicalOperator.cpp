...
TypedLogicalOperator<IntervalJoinLogicalOperator> IntervalJoinLogicalOperator::create(
    LogicalFunction joinFunction,
    JoinTimeCharacteristic timeCharacteristics,
    const IntervalBound lowerBound,
    const IntervalBound upperBound,
    const JoinLogicalOperator::JoinType joinType)
{
    return TypedLogicalOperator<IntervalJoinLogicalOperator>{
        std::move(joinFunction), std::move(timeCharacteristics), lowerBound, upperBound, joinType};
}
...
{
    return TypedLogicalOperator<IntervalJoinLogicalOperator>{
        std::move(children), std::move(joinFunction), std::move(timeCharacteristics), lowerBound, upperBound, joinType};
}
    ...
    const Reflected& reflected, const ReflectionContext& context) const
{
    ...
    return IntervalJoinLogicalOperator::create(
