...
namespace
{
SchemaBase<1, UnboundFieldBase<1>, false>
inferOutputSchema(const std::array<LogicalOperator, 2>& children, const std::array<UnboundFieldBase<1>, 2>& startEndFields)
{
    ...
    auto outputSchemaOrCollisions = SchemaBase<1, UnboundFieldBase<1>, false>::tryCreateCollisionFree(outputFields);

    if (!outputSchemaOrCollisions.has_value())
    {
        throw CannotInferSchema(
            "Found collisions in of input schemas with added fields from windows join: "
            + SchemaBase<1, UnboundFieldBase<1>, false>::createCollisionString(outputSchemaOrCollisions.error()));
    }
