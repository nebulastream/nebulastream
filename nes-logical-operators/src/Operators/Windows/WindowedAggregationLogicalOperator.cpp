...
void WindowedAggregationLogicalOperator::inferLocalSchema()
{
    ...
    groupingKeys = std::visit(
        ...
        {
            using VectorType = std::remove_cvref_t<decltype(visitingGroupingKey)>;
            return visitingGroupingKey
                | std::views::transform(
                       [&](const auto& key) -> typename VectorType::value_type
                       { return std::make_pair(key.first.withInferredDataType(inputSchema), key.second); })
...
{
    return aggregationFunctions | std::views::transform([](const auto& agg) { return agg.name; }) | RangeBinder{self.lock()}
