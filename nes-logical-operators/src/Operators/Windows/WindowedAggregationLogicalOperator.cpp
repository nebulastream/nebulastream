...
auto inferSchema(
    ...
    const WindowedAggregationLogicalOperator::GroupingKeyType& groupingKeys) -> SchemaBase<1, UnboundFieldBase<1>, false>
{
    ...
    const auto outputSchemaOrCollisions = SchemaBase<1, UnboundFieldBase<1>, false>::tryCreateCollisionFree(outputFields);
    if (not outputSchemaOrCollisions.has_value())
    {
        throw CannotInferSchema(
            "Found collisions in inpu schema: " + SchemaBase<1, UnboundFieldBase<1>, false>::createCollisionString(outputSchemaOrCollisions.error()));
    }
