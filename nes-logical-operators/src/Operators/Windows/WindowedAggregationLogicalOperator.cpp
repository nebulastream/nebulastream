...
WindowedAggregationLogicalOperator::WindowedAggregationLogicalOperator(
    ...
    : Operator(), WindowOperator(), windowAggregation(windowAggregation), windowType(windowType), onKey(onKey)
{
}
...
{
    return *this == rhs && (dynamic_cast<const WindowedAggregationLogicalOperator*>(&rhs)->id == id);
}

bool WindowedAggregationLogicalOperator::operator==(const Operator& rhs) const
{
...
void WindowedAggregationLogicalOperator::setWindowAggregation(const std::vector<std::shared_ptr<Windowing::WindowAggregationFunction>>& wa)
{
...
void WindowedAggregationLogicalOperator::setOnKey(const std::vector<std::shared_ptr<FieldAccessLogicalFunction>>& keys)
{
    onKey = keys;
}
