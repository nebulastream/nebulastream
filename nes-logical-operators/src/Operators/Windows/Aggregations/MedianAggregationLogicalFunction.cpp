...
MedianAggregationLogicalFunction::MedianAggregationLogicalFunction(
    std::unique_ptr<LogicalFunction> field, std::unique_ptr<LogicalFunction> asField)
    : WindowAggregationLogicalFunction(
          field->getStamp().clone(),
          DataTypeProvider::provideDataType(LogicalType::UNDEFINED),
          DataTypeProvider::provideDataType(LogicalType::FLOAT64),
...
{
    return std::make_unique<MedianAggregationLogicalFunction>(
...
{
    return std::make_unique<MedianAggregationLogicalFunction>(
        Util::unique_ptr_dynamic_cast<FieldAccessLogicalFunction>(std::move(onField)));
}
...
void MedianAggregationLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    {
        NES_FATAL_ERROR("MedianAggregationLogicalFunction: aggregations on non numeric fields is not supported.");
    }
