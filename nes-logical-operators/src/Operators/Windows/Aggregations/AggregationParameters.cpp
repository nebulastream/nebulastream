...
uint64_t parseUnsignedParameter(const LogicalFunction& parameter, const std::string_view description)
{
    ...
    const auto literal = constant.value()->getConstantValue();
    if (const auto parsed = from_chars<uint64_t>(literal))
    {
        return parsed.value();
    }
    throw InvalidQuerySyntax("Expected an unsigned integer constant for {}, but got {}", description, literal);
}
