...
SchemaBase<1, UnboundFieldBase<1>, false> inferOutputSchema(
    ...
    bool asterisk)
{
    ...
    auto outputSchema = SchemaBase<1, UnboundFieldBase<1>, false>::tryCreateCollisionFree(
        std::vector{projections, fieldProjections} | std::views::join | std::ranges::to<std::vector>());
    if (!outputSchema.has_value())
    {
        throw CannotInferSchema(
            "Found collisions of field names in projection: {}", SchemaBase<1, UnboundFieldBase<1>, false>::createCollisionString(outputSchema.error()));
    }
