    ...
    : Operator(id), UnaryLogicalOperator(id), functions(std::move(functions))
{
    ...
    {
        return not(
            NES::Util::instanceOf<FieldAccessLogicalFunction>(function) or NES::Util::instanceOf<FieldAssignmentLogicalFunction>(function));
    };
...
std::string getFieldName(const LogicalFunction& function)
{
    ...
    return dynamic_cast<const FieldAssignmentLogicalFunction*>(&function)->getField()->getFieldName();
}
