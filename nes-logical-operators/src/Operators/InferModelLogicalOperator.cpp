...
InferModelLogicalOperator::InferModelLogicalOperator(Model model, std::vector<std::string> inputFieldNames)
...
Model InferModelLogicalOperator::getModel() const
{
    return model;
}
...
InferModelLogicalOperator InferModelLogicalOperator::withInferredSchema(std::vector<Schema> inputSchemas) const
{
    ...
    if (inputFieldNames.size() != model.getInputs().size())
    {
        throw CannotInferSchema(
            "Model expects {} inputs, but {} input field names were provided", model.getInputs().size(), inputFieldNames.size());
    }
    ...
    for (const auto& [name, dataType] : model.getOutputs())
    {
        if (copy.outputSchema.getFieldByName(name).has_value())
        {
            ...
            [[maybe_unused]] const bool replaced = copy.outputSchema.replaceTypeOfField(name, dataType);
        }
        else
        {
...
Reflected Reflector<InferModelLogicalOperator>::operator()(const InferModelLogicalOperator& op) const
{
    return reflect(detail::ReflectedInferModelLogicalOperator{
        .model = std::make_optional(Reflector<Model>{}(op.getModel())), .inputFieldNames = std::make_optional(op.getInputFieldNames())});
}
