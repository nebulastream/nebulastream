...
Reflected Reflector<TypedLogicalOperator<SNDeduplicationLogicalOperator>>::operator()(
    const TypedLogicalOperator<SNDeduplicationLogicalOperator>& op, const ReflectionContext& context) const
{
    return context.reflect(detail::ReflectedSNDeduplicationLogicalOperator{.operatorId = op.getId(), .filePath = op->getFilePath()});
}
