...
bool ProjectionLogicalOperator::operator==(const Operator& rhs) const
{
...
std::string getFieldName(const LogicalFunction& function)
{
    ...
    return dynamic_cast<const FieldAssignmentBinaryLogicalFunction*>(&function)->getField()->getFieldName();
}
...
LogicalOperatorGeneratedRegistrar::deserializeProjectionLogicalOperator(const SerializableOperator& serializableOperator)
{
