...
#include <Operators/LogicalOperators/Windows/WindowedAggregationLogicalOperator.hpp>
...
{
    return *this == rhs &&
           (dynamic_cast<const WindowedAggregationLogicalOperator*>(&rhs)->getId() == id);
}
...
{
    auto copy = std::make_shared<WindowedAggregationLogicalOperator>(onKey, windowAggregation, windowType, id);
