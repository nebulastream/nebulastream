...
JoinLogicalOperator::JoinLogicalOperator(const std::shared_ptr<LogicalFunction>& joinFunction,
    ...
    :  Operator(id), BinaryLogicalOperator(id), OriginIdAssignmentOperator(id, originId), joinFunction(joinFunction), windowType(windowType), numberOfInputEdgesLeft(numberOfInputEdgesLeft), numberOfInputEdgesRight(numberOfInputEdgesRight), joinType(joinType)
{
}
...
bool JoinLogicalOperator::operator==(Operator const& rhs) const
{
    if (const auto rhsOperator = dynamic_cast<const JoinLogicalOperator*>(&rhs)) {
        return (getWindowType() == rhsOperator->getWindowType())
            ...
            && (*getLeftSourceType() == *rhsOperator->getRightSourceType())
            && (*getLeftSourceType() == *rhsOperator->getLeftSourceType());
    }
    return false;
}
...
bool JoinLogicalOperator::inferSchema()
{
    ...
    updateSourceTypes(leftInputSchema, rightInputSchema);
    return true;
}

std::shared_ptr<Operator> JoinLogicalOperator::clone() const
{
    auto copy = std::make_shared<JoinLogicalOperator>(joinFunction, windowType, numberOfInputEdgesLeft, numberOfInputEdgesRight, joinType, id);
