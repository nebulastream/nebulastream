...
bool LogicalWindowOperator::inferSchema()
{
    ...
    for (const auto& agg : windowAggregation)
    {
        outputSchema->addField(
            AttributeField::create(NES::Util::as<FieldAccessLogicalFunction>(agg->as())->getFieldName(), agg->as()->getStamp()));
    }
...
std::vector<std::string> LogicalWindowOperator::getGroupByKeyNames() const
{
    ...
    if (windowDefinition->isKeyed())
    {
        std::vector<std::shared_ptr<FieldAccessLogicalFunction>> groupByKeys = windowDefinition->getKeys();
