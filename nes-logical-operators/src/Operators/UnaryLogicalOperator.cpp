...
UnaryLogicalOperator::UnaryLogicalOperator() : Operator(), LogicalOperator(), inputSchema(Schema())
{
}
...
bool UnaryLogicalOperator::inferSchema()
{
    ...
    {
        const auto logOp = dynamic_cast<LogicalOperator*>(child.get());
    ...
    const auto childSchema = dynamic_cast<LogicalOperator*>(children[0].get())->getOutputSchema();
    for (const auto& child : children)
    {
        const auto logOp = dynamic_cast<LogicalOperator*>(child.get());
        if (!(logOp->getOutputSchema() == childSchema))
        {
            NES_ERROR(
                "UnaryOperator: infer schema failed. The schema has to be the same across all child operators."
                "this op schema= {} child schema={}",
                logOp->getOutputSchema().toString(),
...
void UnaryLogicalOperator::inferInputOrigins()
{
    ...
    {
        const auto logOp = dynamic_cast<LogicalOperator*>(child.get());
        logOp->inferInputOrigins();
        auto childInputOriginIds = logOp->getOutputOriginIds();
