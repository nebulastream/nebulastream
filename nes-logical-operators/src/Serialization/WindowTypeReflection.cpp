...
Reflected reflectWindowType(std::shared_ptr<Windowing::WindowType> windowType)
{
    ...
    {
        if (const auto tumblingWindow = std::dynamic_pointer_cast<Windowing::TumblingWindow>(windowType))
        {
            return std::make_pair("TumblingWindow", reflect(tumblingWindow));
        }
        if (const auto slidingWindow = std::dynamic_pointer_cast<Windowing::SlidingWindow>(windowType))
        {
            return std::make_pair("SlidingWindow", reflect(slidingWindow));
        }
        throw CannotDeserialize("Invalid Window Type");
    }();
