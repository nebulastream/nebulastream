...
{
MaxAggregationFunction::MaxAggregationFunction(const FieldAccessFunction& field)
    : WindowAggregationFunction(field.getStamp(), field.getStamp(), field.getStamp(), field)
{
}

MaxAggregationFunction::MaxAggregationFunction(FieldAccessFunction field, FieldAccessFunction asField)
    : WindowAggregationFunction(field.getStamp(), field.getStamp(), field.getStamp(), field, asField)
{
}

std::shared_ptr<WindowAggregationFunction> MaxAggregationFunction::create(FieldAccessFunction onField)
{
    return std::make_shared<MaxAggregationFunction>(onField);
}

std::shared_ptr<WindowAggregationFunction>
MaxAggregationFunction::create(const FieldAccessFunction& onField, const FieldAccessFunction& asField)
{
    return std::make_shared<MaxAggregationFunction>(onField, asField);
}

std::string_view MaxAggregationFunction::getName() const noexcept
{
    return NAME;
}
...
AggregationFunctionRegistryReturnType AggregationFunctionGeneratedRegistrar::RegisterMaxAggregationFunction(AggregationFunctionRegistryArguments arguments)
{
    PRECONDITION(arguments.fields.size() == 2, "MaxAggregationFunction requires exactly two fields, but got {}", arguments.fields.size());
    return MaxAggregationFunction::create(arguments.fields[0], arguments.fields[1]);
}
}
