...
std::string_view JoinOperator::getName() const noexcept
{
    return NAME;
}

bool JoinOperator::operator==(const OperatorConcept& rhs) const
{
    if (const auto rhsOperator = dynamic_cast<const JoinOperator*>(&rhs))
    {
...
std::string JoinOperator::explain(ExplainVerbosity verbosity) const
{
...
Operator JoinOperator::withInferredSchema(std::vector<Schema> inputSchemas) const
{
...
Optimizer::TraitSet JoinOperator::getTraitSet() const
{
    return {originIdTrait};
}

Operator JoinOperator::withChildren(std::vector<Operator> children) const
{
    auto copy = *this;
...
std::vector<Schema> JoinOperator::getInputSchemas() const
{
    return {leftInputSchema, rightInputSchema};
};

Schema JoinOperator::getOutputSchema() const
{
    return outputSchema;
}

std::vector<std::vector<OriginId>> JoinOperator::getInputOriginIds() const
{
    return inputOriginIds;
}

std::vector<OriginId> JoinOperator::getOutputOriginIds() const
{
    return outputOriginIds;
}

Operator JoinOperator::withInputOriginIds(std::vector<std::vector<OriginId>> ids) const
{
...
Operator JoinOperator::withOutputOriginIds(std::vector<OriginId> ids) const
{
    auto copy = *this;
...
std::vector<Operator> JoinOperator::getChildren() const
{
    return children;
}

Schema JoinOperator::getLeftSchema() const
{
    return leftInputSchema;
}

Schema JoinOperator::getRightSchema() const
{
    return rightInputSchema;
}

std::shared_ptr<Windowing::WindowType> JoinOperator::getWindowType() const
{
    return windowType;
}

std::string JoinOperator::getWindowStartFieldName() const
{
    return windowStartFieldName;
}

std::string JoinOperator::getWindowEndFieldName() const
{
    return windowEndFieldName;
}

Function JoinOperator::getJoinFunction() const
{
    return joinFunction;
}

NES::SerializableOperator JoinOperator::serialize() const
{
    ...
    NES::SerializableOperator serializableOperator;
...
OperatorRegistryReturnType OperatorGeneratedRegistrar::RegisterJoinOperator(OperatorRegistryArguments arguments)
{
    PRECONDITION(arguments.inputSchemas.size() == 2, "Expected two input schemas, but got {}", arguments.inputSchemas.size());

    auto functionVariant = arguments.config[JoinOperator::ConfigParameters::JOIN_FUNCTION];
    auto joinTypeVariant = arguments.config[JoinOperator::ConfigParameters::JOIN_TYPE];
    auto windowInfoVariant = arguments.config[JoinOperator::ConfigParameters::WINDOW_INFOS];
    auto windowStartVariant = arguments.config[JoinOperator::ConfigParameters::WINDOW_START_FIELD_NAME];
    auto windowEndVariant = arguments.config[JoinOperator::ConfigParameters::WINDOW_END_FIELD_NAME];
        ...
        and std::holds_alternative<std::string>(windowStartVariant) and std::holds_alternative<std::string>(windowEndVariant))
    {
        ...
        if (std::holds_alternative<WindowInfos>(windowInfoVariant)) {
            ...
            if (windowInfoProto.has_tumbling_window()) {
                auto timeChar = Windowing::TimeCharacteristic::createEventTime(
                    FieldAccessFunction(timeCharProto.field()),
        ...
        auto logicalOperator = JoinOperator(function, windowType, joinType.asEnum<JoinOperator::JoinType>().value());
