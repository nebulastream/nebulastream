...
std::string_view UnionOperator::getName() const noexcept
{
    return NAME;
}

bool UnionOperator::operator==(const OperatorConcept& rhs) const
{
    if (const auto rhsOperator = dynamic_cast<const UnionOperator*>(&rhs))
    {
...
std::string UnionOperator::explain(ExplainVerbosity verbosity) const
{
...
Operator UnionOperator::withInferredSchema(std::vector<Schema> inputSchemas) const
{
...
Optimizer::TraitSet UnionOperator::getTraitSet() const
{
    return {};
}

Operator UnionOperator::withChildren(std::vector<Operator> children) const
{
    auto copy = *this;
...
std::vector<Schema> UnionOperator::getInputSchemas() const
{
    return {leftInputSchema, rightInputSchema};
};

Schema UnionOperator::getOutputSchema() const
{
    return outputSchema;
}

std::vector<std::vector<OriginId>> UnionOperator::getInputOriginIds() const
{
    return inputOriginIds;
}

std::vector<OriginId> UnionOperator::getOutputOriginIds() const
{
    return outputOriginIds;
}

Operator UnionOperator::withInputOriginIds(std::vector<std::vector<OriginId>> ids) const
{
...
Operator UnionOperator::withOutputOriginIds(std::vector<OriginId> ids) const
{
    auto copy = *this;
...
std::vector<Operator> UnionOperator::getChildren() const
{
    return children;
}

SerializableOperator UnionOperator::serialize() const
{
...
OperatorRegistryReturnType OperatorGeneratedRegistrar::RegisterUnionOperator(OperatorRegistryArguments arguments)
{
    auto logicalOperator = UnionOperator();
