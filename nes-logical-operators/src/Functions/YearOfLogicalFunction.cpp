...
LogicalFunction YearOfLogicalFunction::withInferredDataType(const Schema& schema) const
{
    ...
    {
        throw DifferentFieldTypeExpected("YEAROF expects a UINT64 input, but got {}", newDataType);
    }
...
YearOfLogicalFunction Unreflector<YearOfLogicalFunction>::operator()(const Reflected& reflected) const
{
    auto [function] = unreflect<detail::ReflectedYearOfLogicalFunction>(reflected);

    if (!function.has_value())
    {
        throw CannotDeserialize("Failed to deserialize child of YearOfLogicalFunction");
    }
    return YearOfLogicalFunction(std::move(function.value()));
}

LogicalFunctionRegistryReturnType
LogicalFunctionGeneratedRegistrar::RegisterYearOfLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    if (!arguments.reflected.isEmpty())
    {
        return unreflect<YearOfLogicalFunction>(arguments.reflected);
    }
