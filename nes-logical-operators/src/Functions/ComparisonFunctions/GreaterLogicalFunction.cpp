...
GreaterLogicalFunction::GreaterLogicalFunction(LogicalFunction left, LogicalFunction right) : left(std::move(left)), right(std::move(right))
{
}
...
LogicalFunction GreaterLogicalFunction::withInferredDataType(const Schema<Field, Unordered>& schema) const
{
    auto copy = *this;
...
LogicalFunctionRegistryReturnType GreaterLogicalFunction::createGreater(LogicalFunctionRegistryArguments arguments)
{
    ...
    {
        throw InvalidLogicalFunctionArgument("GreaterLogicalFunction requires exactly two children, but got {}", arguments.children.size());
    }
