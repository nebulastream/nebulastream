...
UnboundFieldAccessLogicalFunction::UnboundFieldAccessLogicalFunction(IdentifierList fieldName)
    : fieldName(std::move(fieldName)), dataType(DataTypeProvider::provideDataType(DataType::Type::UNDEFINED)) { };

UnboundFieldAccessLogicalFunction::UnboundFieldAccessLogicalFunction(IdentifierList fieldName, DataType dataType)
...
IdentifierList UnboundFieldAccessLogicalFunction::getFieldName() const
{
    return fieldName;
}

LogicalFunction UnboundFieldAccessLogicalFunction::withFieldName(IdentifierList field) const
{
    auto copy = *this;
...
LogicalFunctionGeneratedRegistrar::RegisterUnboundFieldAccessLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    ...
    auto fieldName = get<IdentifierList>(arguments.config[UnboundFieldAccessLogicalFunction::ConfigParameters::FIELD_NAME]);
    PRECONDITION(!std::ranges::empty(fieldName), "FieldName cannot be empty");
    return UnboundFieldAccessLogicalFunction(fieldName, arguments.dataType);
}
