...
OrLogicalFunction::OrLogicalFunction(const std::shared_ptr<LogicalFunction>& left, const std::shared_ptr<LogicalFunction>& right)
    : BinaryLogicalFunction(std::move(stamp), "Or")
{
    this->setLeftChild(left);
    this->setRightChild(right);
}

bool OrLogicalFunction::operator==(const std::shared_ptr<LogicalFunction>& rhs) const
{
...
void OrLogicalFunction::inferStamp(const Schema& schema)
{
    ...
    INVARIANT(
        getRightChild()->isPredicate(), "the stamp of right child must be boolean, but was: " + getRightChild()->getStamp()->toString());
}
