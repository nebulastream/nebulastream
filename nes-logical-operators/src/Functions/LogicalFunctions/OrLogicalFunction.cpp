...
LogicalFunction OrLogicalFunction::withInferredStamp(Schema schema) const
{
    ...
    INVARIANT(*right.getStamp().get() == Boolean(), "the stamp of right child must be boolean, but was: " + right.getStamp().get()->toString());
...
LogicalFunctionGeneratedRegistrar::RegisterOrLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
