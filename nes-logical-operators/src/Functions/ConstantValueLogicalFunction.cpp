...
{
    return reflect(ReflectedConstantValueLogicalFunction{.value = function.getConstantValue(), .type = function.getDataType().type});
}

ConstantValueLogicalFunction Unreflector<ConstantValueLogicalFunction>::operator()(const Reflected& reflected) const
{
    auto [value, type] = unreflect<ReflectedConstantValueLogicalFunction>(reflected);
