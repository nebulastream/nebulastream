...
LogicalFunction VarSizedToNumericLogicalFunction::withInferredDataType(const Schema<Field, Unordered>& schema) const
{
    ...
    INVARIANT(
        newChildren.size() == 1, "VarSizedToNumericLogicalFunction expects exactly one child function but has {}", newChildren.size());
    ...
    inferredOutputType.nullable
        = std::ranges::any_of(newChildren, [](const auto& currentChild) { return currentChild.getDataType().nullable; });
...
std::string VarSizedToNumericLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    ...
    {
        return fmt::format("VarSizedToNumericLogicalFunction({} -> {}, outputType={})", child.explain(verbosity), targetType, outputType);
    }
