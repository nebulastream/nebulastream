...
LogicalFunction DayOfLogicalFunction::withInferredDataType(const Schema& schema) const
{
    ...
    {
        throw DifferentFieldTypeExpected("DAYOF expects a UINT64 input, but got {}", newDataType);
    }
...
DayOfLogicalFunction Unreflector<DayOfLogicalFunction>::operator()(const Reflected& reflected) const
{
    auto [function] = unreflect<detail::ReflectedDayOfLogicalFunction>(reflected);

    if (!function.has_value())
    {
        throw CannotDeserialize("Failed to deserialize child of DayOfLogicalFunction");
    }
    return DayOfLogicalFunction(std::move(function.value()));
}

LogicalFunctionRegistryReturnType
LogicalFunctionGeneratedRegistrar::RegisterDayOfLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    if (!arguments.reflected.isEmpty())
    {
        return unreflect<DayOfLogicalFunction>(arguments.reflected);
    }
