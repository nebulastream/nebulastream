...
LogicalType AddLogicalFunction::getLogicalType() const
{
    return logicalType;
};

AddLogicalFunction AddLogicalFunction::withLogicalType(const LogicalType& logicalType) const
{
    auto copy = *this;
...
LogicalFunction AddLogicalFunction::withInferredLogicalType(const Schema& schema) const
{
...
std::string AddLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    ...
    {
        return fmt::format("AddLogicalFunction({} + {} : {})", left.explain(verbosity), right.explain(verbosity), logicalType);
    }
