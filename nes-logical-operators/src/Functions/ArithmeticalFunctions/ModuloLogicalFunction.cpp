...
std::shared_ptr<DataType> ModuloLogicalFunction::getDataType() const
{
    return dataType;
};

LogicalFunction ModuloLogicalFunction::withDataType(std::shared_ptr<DataType> dataType) const
{
    auto copy = *this;
...
std::string ModuloLogicalFunction::explain(ExplainVerbosity verbosity) const
{
    ...
    {
        return fmt::format("ModuloLogicalFunction({} % {} : {})", left.explain(verbosity), right.explain(verbosity), dataType->toString());
    }
