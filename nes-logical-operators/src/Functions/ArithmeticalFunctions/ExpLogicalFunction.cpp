...
LogicalFunction ExpLogicalFunction::withInferredDataType(const Schema& schema) const
{
    ...
    return PowLogicalFunction(expConstantValue, newChild).withDataType(DataTypeProvider::provideDataType(DataType::Type::FLOAT64, newChild.getDataType().isNullable));
};
