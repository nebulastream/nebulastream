...
LogicalFunction MonthOfLogicalFunction::withInferredDataType(const Schema& schema) const
{
    ...
    {
        throw DifferentFieldTypeExpected("MONTHOF expects a UINT64 input, but got {}", newDataType);
    }
...
MonthOfLogicalFunction Unreflector<MonthOfLogicalFunction>::operator()(const Reflected& reflected) const
{
    auto [function] = unreflect<detail::ReflectedMonthOfLogicalFunction>(reflected);

    if (!function.has_value())
    {
        throw CannotDeserialize("Failed to deserialize child of MonthOfLogicalFunction");
    }
    return MonthOfLogicalFunction(std::move(function.value()));
}

LogicalFunctionRegistryReturnType
LogicalFunctionGeneratedRegistrar::RegisterMonthOfLogicalFunction(LogicalFunctionRegistryArguments arguments)
{
    if (!arguments.reflected.isEmpty())
    {
        return unreflect<MonthOfLogicalFunction>(arguments.reflected);
    }
