...
{
    auto sourceOp = LogicalOperator{SourceNameLogicalOperator("source")};
    auto selectionOp = LogicalOperator{SelectionLogicalOperator(FieldAccessLogicalFunction("field"))};
...
{
    auto sourceOp = LogicalOperator{SourceNameLogicalOperator("source")};
    auto sourceOp2 = LogicalOperator{SourceNameLogicalOperator("source2")};
    auto plan = LogicalPlan(sourceOp);
    auto result = replaceOperator(plan, sourceOp.getId(), sourceOp2);
...
TEST_F(LogicalPlanTest, replaceSubtree)
{
    ...
    auto sourceOp2 = LogicalOperator{SourceNameLogicalOperator("source2")};
    auto plan = LogicalPlan(sourceOp);
    auto result = replaceSubtree(plan, sourceOp.getId(), sourceOp2);
    EXPECT_TRUE(result.has_value());
...
{
    auto sourceOp = LogicalOperator{SourceNameLogicalOperator{"TestSource"}};
    auto selectionOp = LogicalOperator{SelectionLogicalOperator{FieldAccessLogicalFunction{"fn"}}.withChildren({sourceOp})};
    const auto sinkOp = LogicalOperator{SinkLogicalOperator{"TestSink"}.withChildren({selectionOp})};
