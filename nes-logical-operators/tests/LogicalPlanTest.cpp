...
TEST_F(LogicalPlanTest, GetOperatorsById)
{
    ...
    const auto selectionOp = SelectionLogicalOperator::create(UnboundFieldAccessLogicalFunction{Identifier::parse("fn")})
                                 .withChildren(std::vector<LogicalOperator>{sourceOp});
    const LogicalOperator sinkOp{SinkLogicalOperator::create(Identifier::parse("TestSink")).withChildren({selectionOp})};
