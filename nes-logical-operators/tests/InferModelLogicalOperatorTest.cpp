...
/// withTraitSet/getTraitSet round-trip, plus copy/equality semantics.
TEST_F(InferModelLogicalOperatorTest, TraitsAndEquality)
{
    const auto op = TypedLogicalOperator<InferModelLogicalOperator>{defaultModel()};

    const TraitSet emptySet = op->getTraitSet();
    const auto opWithTraits = op->withTraitSet(emptySet);
...
/// getChildren is empty pre-binding (an InferModel without a child is a valid intermediate state).
TEST_F(InferModelLogicalOperatorTest, ChildrenEmptyByDefault)
{
    const auto op = TypedLogicalOperator<InferModelLogicalOperator>{defaultModel()};
    EXPECT_TRUE(op->getChildren().empty());
}

/// Reflector produces a non-empty reflected representation.
TEST_F(InferModelLogicalOperatorTest, ReflectorProducesReflected)
{
    const auto op = TypedLogicalOperator<InferModelLogicalOperator>{defaultModel()};
    const auto reflected = reflect(op);
    EXPECT_FALSE(reflected.isEmpty());
}
...
/// InferModelNameLogicalOperator's withInferredSchema and getOrderedOutputSchema both throw —
/// the operator is a pre-resolution placeholder that must be replaced by InferModelResolutionRule first.
TEST_F(InferModelLogicalOperatorTest, NameVariantSchemaInferenceThrows)
{
    const auto nameOp = TypedLogicalOperator<InferModelNameLogicalOperator>{std::string{"myModel"}};
    ASSERT_EXCEPTION_ERRORCODE((void)nameOp->withInferredSchema(), NES::ErrorCode::CannotInferSchema);
}
