...
/// Streaming interval join: pairs each left (anchor) tuple `a` with right (partner) tuples `b` where
/// `b.ts in [a.ts + lowerBound, a.ts + upperBound]`. "anchor"/"partner" match the physical operators.
/// Bounds are signed ms offsets, so intervals may be past/future-anchored (e.g. `[-4, -1]`). Slice
/// width `W = upperBound - lowerBound`. The constructor rejects `lowerBound >= upperBound` (W = 0 is degenerate).
...
public:
    ...
    static TypedLogicalOperator<IntervalJoinLogicalOperator> create(
        LogicalFunction joinFunction,
        JoinTimeCharacteristic timeCharacteristics,
        IntervalBound lowerBound,
        IntervalBound upperBound,
...
{
    Reflected operator()(const TypedLogicalOperator<IntervalJoinLogicalOperator>& op, const ReflectionContext& context) const;
};
