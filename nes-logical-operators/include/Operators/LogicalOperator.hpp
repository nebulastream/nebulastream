...
struct TypedLogicalOperator
{
    ...
    [[nodiscard]] TypedLogicalOperator withTraitSet(TraitSet traitSet) const { return self->withTraitSet(std::move(traitSet)).getAs<Checked>(); }
...
struct OperatorModel : ErasedLogicalOperator
{
    ...
    explicit OperatorModel(Args&&... args)
        : id(getNextLogicalOperatorId()), selfRef(std::make_shared<SelfRef<OperatorType>>(this)), impl(WeakLogicalOperator{selfRef}, std::forward<Args>(args)...)
    {
    ...
    [[nodiscard]] Reflected reflect() const override { return Reflector<TypedLogicalOperator<OperatorType>>{}(impl); }
