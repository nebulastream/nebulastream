...
/// Operators carry primitive Schemas. Compound LogicalTypes (Point, ...)
/// only live on LogicalFunction expressions; they're flattened to N primitive
/// schema fields by the operator that introduces them (e.g. Projection over
/// a Point constructor) using the LogicalTypeLoweringRegistry.
template <typename T>
...
struct OperatorModel : ErasedLogicalOperator
{
    ...
    {
        return impl.withInferredSchema(std::move(inputSchemas));
    }
