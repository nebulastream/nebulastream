...
concept LogicalOperatorConcept = requires(
    ...
    { thisOperator.getInputSchemas() } -> std::convertible_to<std::vector<LegacySchema>>;

    /// Returns the output schema of the operator
    { thisOperator.getOutputSchema() } -> std::convertible_to<LegacySchema>;
...
struct ErasedLogicalOperator : std::enable_shared_from_this<ErasedLogicalOperator>
{
    ...
    [[nodiscard]] virtual std::vector<LegacySchema> getInputSchemas() const = 0;
    [[nodiscard]] virtual LegacySchema getOutputSchema() const = 0;
    [[nodiscard]] virtual LogicalOperator withInferredSchema(std::vector<LegacySchema> inputSchemas) const = 0;
...
struct TypedLogicalOperator
{
    ...
    [[nodiscard]] std::vector<LegacySchema> getInputSchemas() const { return self->getInputSchemas(); }

    [[nodiscard]] LegacySchema getOutputSchema() const { return self->getOutputSchema(); }

    [[nodiscard]] TypedLogicalOperator withInferredSchema(std::vector<LegacySchema> inputSchemas) const
    {
...
struct OperatorModel : ErasedLogicalOperator
{
    ...
    [[nodiscard]] std::vector<LegacySchema> getInputSchemas() const override { return impl.getInputSchemas(); }

    [[nodiscard]] LegacySchema getOutputSchema() const override { return impl.getOutputSchema(); }

    [[nodiscard]] LogicalOperator withInferredSchema(std::vector<LegacySchema> inputSchemas) const override
    {
