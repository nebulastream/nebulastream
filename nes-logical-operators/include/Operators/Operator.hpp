...
public:
    ...
    virtual bool isIdentical(const std::shared_ptr<Operator>& rhs) const = 0;
    virtual bool equal(const std::shared_ptr<Operator>& rhs) const = 0;
    ...
    bool vectorContainsTheNode(const std::vector<std::shared_ptr<Operator>>& nodes, const std::shared_ptr<Operator>& nodeToFind)
    {
        return find(nodes, nodeToFind) != nullptr;
    }

    bool removeParent(const std::shared_ptr<Operator>& node)
    {
        if (!node)
        {
    ...
    bool insertBetweenThisAndParentNodes(const std::shared_ptr<Operator>& newNode)
    {
    ...
    bool removeChild(const std::shared_ptr<Operator>& node)
    {
        if (!node)
        {
            NES_ERROR("Node: Can't remove null node");
    ...
    bool replace(std::shared_ptr<Operator> other) { return replace(std::move(other), shared_from_this()); }
    ...
    std::vector<std::shared_ptr<Operator>> getParents() const { return parents; }

    std::vector<std::shared_ptr<Operator>> getChildren() const { return children; }
