...
/// An IntervalWindow models the closed bound range [lowerBound, upperBound] of a streaming interval
/// join: each anchor (left) tuple `a` pairs with partner (right) tuples `b` whose timestamp lies in
/// [a.ts + lowerBound, a.ts + upperBound]. Unlike tumbling/sliding windows the bounds are signed
/// milliseconds so the range can be past- or future-anchored (e.g. [-4, -1]).
...
{
    Reflected operator()(const Windowing::IntervalWindow& intervalWindow) const;
};
