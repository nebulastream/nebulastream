...
// ─── Preprocessor utilities (needed only to compute arity = nargs - 1) ───────────
...
// ─── Internal: generic IMPL (single macro, any arity) ───────────────────────────
// Uses vector<LogicalFunction> storage. Generates the full class, all methods,
// and leaves inferOutputType_ open for a body.
...
#define NES_LOGICAL_FUNCTION_IMPL(name, arity)                                                                                        \
    ...
    bool name##LogicalFunction::operator==(const name##LogicalFunction& rhs) const { return children_ == rhs.children_; }             \
    DataType name##LogicalFunction::getDataType() const { return dataType; }                                                          \
    ...
    std::string_view name##LogicalFunction::getType() const { return NAME; }                                                          \
    std::vector<LogicalFunction> name##LogicalFunction::getChildren() const { return children_; }                                     \
    ...
    LogicalFunctionRegistryReturnType LogicalFunctionGeneratedRegistrar::Register##name##LogicalFunction(                              \
    ...
    {                                                                                                                                 \
        ...
        {                                                                                                                             \
            throw CannotDeserialize(                                                                                                   \
                #name "LogicalFunction requires " #arity " children, but got {}", arguments.children.size());                         \
        }                                                                                                                             \
        return name##LogicalFunction(std::move(arguments.children));                                                                  \
    }                                                                                                                                 \
    DataType::Type name##LogicalFunction::inferOutputType_(                                                                           \
        [[maybe_unused]] const std::vector<LogicalFunction>& children) const
