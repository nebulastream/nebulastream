...
//     SchemaBase<1, UnboundFieldBase<1>, false> unbind() const
//     {
//         return SchemaBase{*this | std::views::transform(Field::unbinder()) | std::ranges::to<std::vector>()};
//     }
//
//     static Schema bind(LogicalOperator logicalOperator, SchemaBase<1, UnboundFieldBase<1>, false> unboundSchema);
//
// private:
...
using Schema = SchemaBase<std::dynamic_extent, Field, false>;
static_assert(std::ranges::range<Schema>);
static_assert(std::same_as<std::ranges::range_value_t<Schema>, Field>);

SchemaBase<1, UnboundFieldBase<1>, false> unbind(const Schema& schema)
{
    return schema | std::views::transform(Field::unbinder()) | std::ranges::to<SchemaBase<1, UnboundFieldBase<1>, false>>();
}

template <bool Ordered>
Schema bind(const LogicalOperator& logicalOperator, const SchemaBase<1, UnboundFieldBase<1>, Ordered>& unboundSchema);
