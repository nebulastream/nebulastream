...
struct Field
{
    ...
    // static std::function<UnqualifiedUnboundField(Field)> unbinder();
    // static std::function<Field(UnqualifiedUnboundField)> binder(LogicalOperator logicalOperator);
...
{
    using ContextType = std::shared_ptr<OperatorMappingContext>;
    std::shared_ptr<OperatorMappingContext> operatorMapping;
