...
/// In the SourceExpansionRule, we use the logical source name as input to the source catalog, to retrieve all (physical) source descriptors
/// configured for the specific logical source name. We then expand 1 SourceNameOperator to N SourceDescriptorOperators,
/// one SourceDescriptorOperator for each descriptor found in the source catalog with the logical source name as input.
class SourceNameOperator : public OperatorConcept
{
public:
    explicit SourceNameOperator(std::string logicalSourceName);
    explicit SourceNameOperator(std::string logicalSourceName, const Schema& schema);
    ...
    [[nodiscard]] Operator withSchema(const Schema& schema) const;
