...
{
public:
    SumAggregationFunction(const FieldAccessFunction& onField, const FieldAccessFunction& asField);
    explicit SumAggregationFunction(const FieldAccessFunction& onField);
    virtual ~SumAggregationFunction() = default;

    static std::shared_ptr<WindowAggregationFunction> create(Function onField);
