...
test_files=("YahooStreamingBenchmark")
# "DEBS" "Nexmark_multiple_GB_of_Bids")
    ...
    for buffer_size in "${buffer_sizes[@]}"; do
      for threads in "${worker_threads[@]}"; do
        for query in "${debs_queries[@]}"; do
          ...
          RUN_ID="$(date +%s)_${test_file}_${layout}_buffer${buffer_size}_threads${threads}_query${query}"
          ...
          /tmp/nebulastream/cmake-build-release/nes-systests/systest/systest -b \
            -t /tmp/nebulastream/nes-systests/benchmark/${test_file}.test:$query -- \
          ...
          if [ -n "$enginestat" ]; then
            ...
            new_filename="${BENCHMARK_DIR}/logs/GoogleEventTrace_${layout}_buffer${buffer_size}_threads${threads}_query${query}.json"
