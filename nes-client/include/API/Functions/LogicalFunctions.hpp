...
FunctionNodePtr operator!(FunctionItem leftExp);

/**
 * @brief Defines common operations on at least one operator which is not of type FunctionNodePtr but
 * either a constant or an instance of the type FunctionItem.
 */
/// Utility which converts a constant or an function item to an FunctionNodePtr.
template <
    typename T,
    typename = std::enable_if_t<std::disjunction_v<
        std::is_same<std::decay_t<T>, FunctionNodePtr>,
        std::is_same<std::decay_t<T>, FunctionItem>,
        std::is_constructible<FunctionItem, std::decay_t<T>>>>>
inline auto toFunctionNodePtr(T&& t) -> FunctionNodePtr
{
    using Arg = std::decay_t<T>;
    if constexpr (std::is_same_v<Arg, FunctionNodePtr>)
    {
    ...
    else if constexpr (std::is_same_v<Arg, FunctionItem>)
    {
        ...
        return t.getFunctionNode();
    }
    /// Guaranteed copy elision.
    return FunctionItem{std::forward<T>(t)}.getFunctionNode();
}
...
static constexpr bool function_generator_v = std::conjunction_v<
    std::negation<std::conjunction<std::is_same<FunctionNodePtr, std::decay_t<T>>...>>,
    std::disjunction<std::is_same<FunctionNodePtr, std::decay_t<T>>..., std::is_same<FunctionItem, std::decay_t<T>>...>,
 ...
 *        If both the LHS and RHS are FunctionNodePtrs, this overload is not used.
 *
 ...
 *
 * @return FunctionNodePtr which reflects the operator.
 */
template <typename LHS, typename RHS, typename = std::enable_if_t<function_generator_v<LHS, RHS>>>
inline auto operator&&(LHS&& lhs, RHS&& rhs) -> FunctionNodePtr
{
