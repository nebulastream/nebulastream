...
public:
  ...
  /**
   * @brief is called to append all joinExpressions (key predicates) to the
   * previous defined join, i.e., it sets all condition for the join matches
   * @param joinExpression : a set of binary expressions to compare left and
   * right tuples
   * @return object of type JoinWhere on which window function is defined and
   * can be called.
   */
...
public:
  /**
   * @brief Constructor. Initialises always subQueryRhs, original Query and the
   * joinExpression
   * @param subQueryRhs
   * @param originalQuery
   * @param joinExpression : a set of binary expressions to compare left and
   * right tuples
   */
  JoinWhere(const Query &subQueryRhs, Query &originalQuery,
            ExpressionNodePtr joinExpressions);

  /**
   * @brief: calls internal the original joinWith function with all the gathered
   * parameters.
   * @param windowType
   * @return the query with the result of the original joinWith function is
   * returned.
   */
 ...
 * @note In contrast to the JoinOperatorBuilder only .where() and .key() need to
 * be applied to join the query.
...
public:
   ...
   * @param joinExpression : a set of binary expressions to compare left and
   * right tuples
   * @return object of type JoinWhere on which equalsTo function is defined and
   * can be called.
   */
...
public:
  ...
  /**
   * @brief: calls internal the original andWith function with all the gathered
   * parameters.
   * @param windowType
   * @return the query with the result of the original andWith function is
   * returned.
   */
  [[nodiscard]] Query &window(Windowing::WindowTypePtr const &windowType) const;
...
public:
  ...
  /**
   * @brief: calls internal the original seqWith function with all the gathered
   * parameters.
   * @param windowType
   * @return the query with the result of the original seqWith function is
   * returned.
   */
  [[nodiscard]] Query &window(Windowing::WindowTypePtr const &windowType) const;
...
/**
 * @brief: This operator is a CEP operator, in CEP engines also called iteration
 * operator. It allows for multiple occurrences of a specified event, i.e.,
 * tuples. Thus, 'times' enables patterns of arbitrary length (when only
 * minOccurrences are defined) or requires a specified number of tuples
 * (minOccurrence, maxOccurrence) to occur The Times operator requires the call
 * of the window operator afterwards
 * @return cepBuilder
 */
...
public:
  /**
   * @brief Constructor (bounded variant to a number of minOccurrences to
   * maxOccurrences of event occurrence)
   * @param minOccurrences: minimal number of occurrences of a specified event,
   * i.e., tuples
   * @param maxOccurrences: maximal number of occurrences of a specified event,
   * i.e., tuples
   * @param originalQuery
   * @return cepBuilder
   */
  Times(const uint64_t minOccurrences, const uint64_t maxOccurrences,
        Query &originalQuery);
...
// TODO this method is a quick fix to generate unique keys for andWith chains
// and should be removed after implementation of Cartesian Product (#2296)
/**
 * @brief: this function creates a virtual key for the left side of the binary
 * operator
 ...
 * The current api exposes method to create queryIdAndCatalogEntryMapping using
 * all currently supported operators.
 */
class Query {
public:
  Query(const Query &);
  ...
  // both, Join and CEPOperatorBuilder friend classes, are required as they use
  // the private joinWith method.
  ...
  window(Windowing::WindowTypePtr const &windowType);

  /**
   * @brief can be called on the original query with the query to be joined with
   * and sets this query in the class Join.
   * @param subQueryRhs
  ...
  /**
   * @brief can be called on the original query with the query to be joined with
   * and sets this query in the class BatchJoinOperatorBuilder::Join.
   * @warning The batch join is an experimental feature.
   * @param subQueryRhs
   * @return object where where() function is defined and can be called by user
   */
  NES::Experimental::BatchJoinOperatorBuilder::Join
  batchJoinWith(const Query &subQueryRhs);

  /**
   * @brief can be called on the original query with the query to be composed
   * with and sets this query in the class And.
   * @param subQueryRhs
   * @return CEPOperatorBuilder object where the window() function is defined
   * and can be called by user
   */
  CEPOperatorBuilder::And andWith(const Query &subQueryRhs);

  /**
   * @brief can be called on the original query with the query to be composed
   * with and sets this query in the class Join.
   * @param subQueryRhs
   * @return CEPOperatorBuilder object where the window() function is defined
   * and can be called by user
   */
  ...
  /**
   * @brief can be called on the original query to detect an number event
   * occurrences between minOccurrence and maxOccurrence in a stream
   * @param minOccurrences
   * @param maxOccurrences
   * @return CEPOperatorBuilder object where the window() function is defined
   * and can be called by user
   */
  CEPOperatorBuilder::Times times(const uint64_t minOccurrences,
                                  const uint64_t maxOccurrences);

  /**
   * @brief can be called on the original query to detect an exact number event
   * occurrences in a stream
   * @param occurrences
   * @return CEPOperatorBuilder object where the window() function is defined
   * and can be called by user
   */
  ...
  /**
   * @brief can be called on the original query to detect multiple occurrences
   * of specified events in a stream
   * @return CEPOperatorBuilder object where the window() function is defined
   * and can be called by user
   */
  ...
  /**
   * @brief can be called on the original query with the query to be composed
   * with and sets this query in the class Or.
   * @param subQueryRhs
  ...
  /**
   * @brief: Creates a query from a particular source. The source is identified
   * by its name. During query processing the underlying source descriptor is
   * retrieved from the source catalog.
   * @param sourceName name of the source to query. This name has to be
   * registered in the query catalog.
   * @return the query
   */
   ...
   * @param metricHash: The hash of the metric, this operator is collecting,
   * e.g., `cardinality` over field `f1`
   ...
   */
  Query &
  buildStatistic(Windowing::WindowTypePtr window,
  ...
  auto project(Args &&...args) -> std::enable_if_t<
      std::conjunction_v<std::is_constructible<ExpressionItem, Args>...>,
      Query &> {
   ...
   * @return query.
   */
  Query &assignWatermark(Windowing::WatermarkStrategyDescriptorPtr const
   ...
   * The Sink operator is defined by the sink descriptor, which represents the
   * semantic of this sink.
...
private:
  /**
   * @new change: Now it's private, because we don't want the user to have
   * access to it. We call it only internal as a last step during the Join
   * operation
   * @brief This methods adds the joinType to the join operator and calls the
   * join function to add the operator to a query
   ...
   * @return the query
   */
  Query &joinWith(const Query &subQueryRhs, ExpressionNodePtr joinExpression,
   ...
   * access to it. We call it only internal as a last step during the Join
   * operation
   * @note In contrast to joinWith(), batchJoinWith() does not require a window
   * to be specified.
   * @param subQueryRhs subQuery to be joined
  ...
  /**
   * @new change: Now it's private, because we don't want the user to have
   * access to it. We call it only internal as a last step during the AND
   * operation
   * @brief This methods adds the joinType to the join operator and calls join
   * function to add the operator to a query
   ...
   * @return the query
   */
  Query &andWith(const Query &subQueryRhs, ExpressionNodePtr joinExpressions,
                 Windowing::WindowTypePtr const &windowType);

  /**
   * @new change: Now it's private, because we don't want the user to have
   * access to it. We call it only internal as a last step during the SEQ
   * operation
   * @brief This methods adds the joinType to the join operator and calls join
   * function to add the operator to a query
   * @param subQueryRhs subQuery to be composed
   ...
   * @return the query
   */
  Query &seqWith(const Query &subQueryRhs, ExpressionNodePtr joinExpressions,
                 Windowing::WindowTypePtr const &windowType);

  /**
   * @new change: similar to join, the original window and windowByKey become
   ...
   * @return query.
   */
  Query &window(Windowing::WindowTypePtr const &windowType,
  ...
  /**
   * @brief: Given a Expression is identifies which JoinType has to be used for
   * processing, i.e., Equi-Join enables different Join algorithms, while all
   * other join types right lead to the NestedLoopJoin
   * @param joinExpressions key expressions
   * @return joinType
   */
  Join::LogicalJoinDescriptor::JoinType
  identifyJoinType(ExpressionNodePtr joinExpressions);
};
