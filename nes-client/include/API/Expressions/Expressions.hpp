...
/**
 * @brief This file contains the user facing api to create expression nodes in a
 * fluent and easy way.
 */
...
public:
  ...
  ExpressionItem(
      std::string const &value);      // NOLINT(google-explicit-constructor)
...
/**
 * @brief Attribute(name) allows the user to reference a field in his
 * expression. Attribute("f1") < 10 todo rename to field if conflict with legacy
 * code is resolved.
 * @param fieldName
 */
...
/**
 * @brief Attribute(name, type) allows the user to reference a field, with a
 * specific type in his expression. Field("f1", Int) < 10. todo remove this case
 * if we added type inference at Runtime from the operator tree. todo rename to
 * field if conflict with legacy code is resolved.
 * @param fieldName, type
 */
ExpressionItem Attribute(std::string name, BasicType type);

/**
 * @brief WHEN(condition,value) can only be used as part of the left vector in a
 * CASE() expression. Allows to only return the value expression if condition is
 * met.
...
/**
 * @brief CASE({WHEN(condition,value),WHEN(condition, value)} , value) allows to
 * evaluate all WHEN expressions from the vector list and only return the first
 * one where the condition evaluated to true, or the value of the default
 * expression. The CASE({WHEN()},default) is evaluated as a concatenated ternary
 * operator in C++.
 * @param whenExpressions : a vector of at least one WHEN expression to
 * evaluate.
 * @param defaultValueExp : an expression which will be returned if no WHEN
