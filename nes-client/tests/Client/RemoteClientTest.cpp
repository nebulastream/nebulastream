...
class RemoteClientTest : public Testing::BaseIntegrationTest {
 protected:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("RemoteClientTest.log", NES::LogLevel::LOG_DEBUG);
  }
  static void TearDownTestCase() {
    NES_INFO("Tear down RemoteClientTest test class.");
  }
  ...
  void SetUp() override {
    ...
    client = std::make_shared<Client::RemoteClient>(
        "localhost", *restPort, std::chrono::seconds(20), true);
  }
  ...
  void checkForQueryStart(QueryId queryId) {
    ...
    while (std::chrono::system_clock::now() < startTs + timeoutInSec) {
      auto status =
          magic_enum::enum_cast<QueryState>(client->getQueryStatus(queryId));
      if (status == QueryState::REGISTERED ||
          status == QueryState::OPTIMIZING || status == QueryState::DEPLOYED) {
    ...
    throw Exceptions::RuntimeException(
        "Test checkForQueryStart timeout exceeds.");
  }
...
TEST_F(RemoteClientTest, DeployQueryTest) {
  Query query =
      Query::from("default_logical").sink(NullOutputSinkDescriptor::create());
...
TEST_F(RemoteClientTest, SubmitQueryWithWrongLogicalSourceNameTest) {
  Query query =
      Query::from("default_l").sink(NullOutputSinkDescriptor::create());
  try {
...
/**
 * @brief Test if correct error message is thrown for query plan retrieval with
 * invalid query id
...
TEST_F(RemoteClientTest, CorrectnessOfGetQueryPlan) {
  ...
  std::string restSDKResponse =
      "Provided QueryId: " + nonExistingQueryId.toString() + " does not exist";
  std::string oatppResponse =
      "No query with given ID: " + nonExistingQueryId.toString();
  EXPECT_TRUE(jsonResponse["message"] == restSDKResponse ||
              jsonResponse["message"] == oatppResponse);
  ASSERT_TRUE(stopQuery(queryId));
}
...
TEST_F(RemoteClientTest, StopQueryTest) {
  ...
  auto res = client->stopQuery(queryId);
  ASSERT_TRUE(!!res);
  ASSERT_NE(crd->getQueryCatalog()->getQueryState(queryId),
            QueryState::RUNNING);
}
