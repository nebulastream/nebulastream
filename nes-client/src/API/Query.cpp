...
CEPOperatorBuilder::Times Query::times(const uint64_t minOccurrences,
...
JoinWhere::JoinWhere(const Query &subQueryRhs, Query &originalQuery,
...
Query &JoinWhere::window(const Windowing::WindowTypePtr &windowType) const {
  return originalQuery.joinWith(
      subQueryRhs, joinExpressions,
      windowType); // call original joinWith() function
}
...
And::And(const Query &subQueryRhs, Query &originalQuery)
  ...
  NES_DEBUG(
      "Query: add map operator to andWith to add virtual key to originalQuery");
  // here, we add artificial key attributes to the sources in order to reuse the
  // join-logic later
  ...
  joinExpression = ExpressionItem(Attribute(cepLeftKey)).getExpressionNode() ==
                   ExpressionItem(Attribute(cepRightKey)).getExpressionNode();
}

Query &And::window(const Windowing::WindowTypePtr &windowType) const {
  return originalQuery.andWith(subQueryRhs, joinExpression,
                               windowType); // call original andWith() function
}
...
Seq::Seq(const Query &subQueryRhs, Query &originalQuery)
    : subQueryRhs(const_cast<Query &>(subQueryRhs)),
      originalQuery(originalQuery) {
  NES_DEBUG(
      "Query: add map operator to seqWith to add virtual key to originalQuery");
  // here, we add artificial key attributes to the sources in order to reuse the
  // join-logic later
  auto cepLeftKey = "cep_leftKey";
  auto cepRightKey = "cep_rightKey";
...
Query &Seq::window(const Windowing::WindowTypePtr &windowType) const {
  ...
  std::string sourceNameLeft =
      originalQuery.getQueryPlan()->getSourceConsumed();
  std::string sourceNameRight = subQueryRhs.getQueryPlan()->getSourceConsumed();
  // to guarantee a correct order of events by time (sequence) we need to
  // identify the correct source and its timestamp in case of composed streams
  // on the right branch
  if (sourceNameRight.find("_") != std::string::npos) {
    // we find the most left source and use its timestamp for the filter
    // constraint
    uint64_t posStart = sourceNameRight.find("_");
    uint64_t posEnd = sourceNameRight.find("_", posStart + 1);
    sourceNameRight =
        sourceNameRight.substr(posStart + 1, posEnd - 2) + "$" + timestamp;
  ...
  if (sourceNameLeft.find("_") != std::string::npos) {
    // we find the most right source and use its timestamp for the filter
    // constraint
  ...
  return originalQuery.seqWith(subQueryRhs, joinExpression, windowType)
      .filter(Attribute(sourceNameLeft) <
              Attribute(sourceNameRight)); // call original seqWith() function
}

Times::Times(const uint64_t minOccurrences, const uint64_t maxOccurrences,
             Query &originalQuery)
    : originalQuery(originalQuery), minOccurrences(minOccurrences),
      maxOccurrences(maxOccurrences), bounded(true) {
  // add a new count attribute to the schema which is later used to derive the
  // number of occurrences
  originalQuery.map(Attribute("Count") = 1);
}

Times::Times(const uint64_t occurrences, Query &originalQuery)
    : originalQuery(originalQuery), minOccurrences(0),
      maxOccurrences(occurrences), bounded(true) {
  // add a new count attribute to the schema which is later used to derive the
  // number of occurrences
  originalQuery.map(Attribute("Count") = 1);
}
...
Times::Times(Query &originalQuery)
    : originalQuery(originalQuery), minOccurrences(0), maxOccurrences(0),
      bounded(false) {
  // add a new count attribute to the schema which is later used to derive the
  // number of occurrences
  originalQuery.map(Attribute("Count") = 1);
...
Query &Times::window(const Windowing::WindowTypePtr &windowType) const {
  ...
  if (!bounded) {
    ...
    return originalQuery.window(windowType)
        .apply(API::Sum(Attribute("Count")), API::Max(Attribute(timestamp)))
        .filter(Attribute("Count") >= minOccurrences &&
                Attribute("Count") <= maxOccurrences);
  }
...
Query &Query::buildStatistic(
  ...
  this->queryPlan = QueryPlanBuilder::addStatisticBuildOperator(
      std::move(window), std::move(statisticDescriptor), metricHash,
      sendingPolicy, triggerCondition, this->queryPlan);
  return *this;
}
...
Query &Query::project(std::vector<ExpressionNodePtr> expressions) {
  ...
  this->queryPlan =
      QueryPlanBuilder::addProjection(expressions, this->queryPlan);
  return *this;
}
...
Query &Query::unionWith(const Query &subQuery) {
  ...
  this->queryPlan =
      QueryPlanBuilder::addUnion(this->queryPlan, subQuery.getQueryPlan());
  return *this;
}
...
Query &Query::joinWith(const Query &subQueryRhs,
  ...
  Join::LogicalJoinDescriptor::JoinType joinType =
      identifyJoinType(joinExpression);
  this->queryPlan =
      QueryPlanBuilder::addJoin(this->queryPlan, subQueryRhs.getQueryPlan(),
                                joinExpression, windowType, joinType);
  return *this;
}
...
Query &Query::batchJoinWith(const Query &subQueryRhs,
  ...
  if (joinExpression->as<EqualsExpressionNode>()) {
    ...
    this->queryPlan = QueryPlanBuilder::addBatchJoin(
        this->queryPlan, subQueryRhs.getQueryPlan(), onProbeKey, onBuildKey);
  } else {
...
Query &Query::orWith(const Query &subQueryRhs) {
  ...
  this->queryPlan =
      QueryPlanBuilder::addUnion(this->queryPlan, subQueryRhs.getQueryPlan());
  return *this;
}
...
Query &Query::filter(const ExpressionNodePtr &filterExpression) {
  ...
  this->queryPlan =
      QueryPlanBuilder::addFilter(filterExpression, this->queryPlan);
  return *this;
}
...
Query &Query::flatMapUDF(const Catalogs::UDF::UDFDescriptorPtr &descriptor) {
  ...
  this->queryPlan =
      QueryPlanBuilder::addFlatMapUDF(descriptor, this->queryPlan);
  return *this;
}
...
Query &
Query::inferModel(const std::string model,
  ...
  OperatorPtr op = LogicalOperatorFactory::createInferModelOperator(
      model, inputFieldsPtr, outputFieldsPtr);
...
Query &Query::sink(const SinkDescriptorPtr sinkDescriptor, WorkerId workerId) {
  ...
  this->queryPlan =
      QueryPlanBuilder::addSink(this->queryPlan, sinkDescriptor, workerId);
  return *this;
}

Query &Query::assignWatermark(const Windowing::WatermarkStrategyDescriptorPtr
                                  &watermarkStrategyDescriptor) {
  NES_DEBUG("Query: add assignWatermark operator to query");
  this->queryPlan = QueryPlanBuilder::assignWatermark(
      this->queryPlan, watermarkStrategyDescriptor);
  return *this;
}
...
Query::identifyJoinType(ExpressionNodePtr joinExpression) {
