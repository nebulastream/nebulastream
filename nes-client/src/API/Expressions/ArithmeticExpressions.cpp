...
ExpressionNodePtr POWER(ExpressionNodePtr leftExp, ExpressionNodePtr rightExp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "power",
                                    {leftExp, rightExp});
}

// calls of unary operators with ExpressionNode
ExpressionNodePtr ABS(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "abs",
                                    {exp});
}
...
ExpressionNodePtr LN(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "ln",
                                    {exp});
}

ExpressionNodePtr LOG2(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "log2",
                                    {exp});
}

ExpressionNodePtr LOG10(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "log10",
                                    {exp});
}

ExpressionNodePtr SIN(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "sin",
                                    {exp});
}

ExpressionNodePtr COS(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(), "cos",
                                    {exp});
}

ExpressionNodePtr RADIANS(const ExpressionNodePtr& exp) {
  return FunctionExpression::create(DataTypeFactory::createUndefined(),
                                    "radians", {exp});
}
...
ExpressionNodePtr operator++(ExpressionNodePtr leftExp) {
  return std::move(leftExp) +
         ConstantValueExpressionNode::create(DataTypeFactory::createBasicValue(
             DataTypeFactory::createUInt16(), /*value*/ "1"));
}
