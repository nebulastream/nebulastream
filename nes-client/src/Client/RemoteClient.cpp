...
RemoteClient::RemoteClient(const std::string &coordinatorHost,
    ...
    : coordinatorHost(coordinatorHost),
      coordinatorRESTPort(coordinatorRESTPort), requestTimeout(requestTimeout) {
  if (!disableLogging) {
    NES::Logger::setupLogging("nesRemoteClientStarter.log",
                              LogLevel::LOG_DEBUG);
  }
...
QueryId RemoteClient::submitQuery(const Query &query, QueryConfig config) {
  ...
  placement.set_value(
      std::string(magic_enum::enum_name(config.getPlacementType())));
  ...
  auto future =
      cpr::PostAsync(cpr::Url{getHostName() + path},
                     cpr::Header{{"Content-Type", "application/json"}},
                     cpr::Body{message}, cpr::Timeout{requestTimeout});
  NES_DEBUG("RemoteClient::send: {} {}", this->coordinatorHost,
            this->coordinatorRESTPort);
  future.wait();
  ...
  if (response.status_code == cpr::status::HTTP_ACCEPTED) {
    if (result.contains("queryId")) {
      ...
      throw ClientException(
          "Invalid response format queryId is not contained in: " +
          result.dump());
    }
  } else {
...
RemoteClient::QueryStopResult RemoteClient::stopQuery(QueryId queryId) {
  ...
  if (withError) {
    r.errorMessage = (result.contains("detail"))
                         ? result["detail"].get<std::string>()
...
std::string RemoteClient::getQueries(QueryState status) {
  ...
  cpr::AsyncResponse future =
      cpr::GetAsync(cpr::Url{getHostName() + "queryCatalog/queries"},
...
std::string RemoteClient::getPhysicalSources(std::string logicalSourceName) {
  ...
  auto path =
      "sourceCatalog/allPhysicalSource?logicalSourceName=" + logicalSourceName;
...
bool RemoteClient::addLogicalSource(const SchemaPtr schema,
...
std::string RemoteClient::getHostName() {
  return "http://" + coordinatorHost + ":" +
         std::to_string(coordinatorRESTPort) + "/v1/nes/";
}
} // namespace NES::Client
