...
std::ostream& operator<<(std::ostream& os, const ConfigResolutionErrors& errors)
{
    ...
    if (not errors.duplicateFields.empty())
    {
        os << fmt::format("\nDuplicate Fields: {}", fmt::join(errors.duplicateFields, ", "));
    }
...
{
    return unresolvableFields.empty() && duplicateFields.empty() && failedInstantiations.empty() && missingFields.empty();
}
...
ConfigResolutionErrors ConfigResolutionErrors::combine(ConfigResolutionErrors lhs, ConfigResolutionErrors rhs)
{
    return ConfigResolutionErrors{
        ...
        .duplicateFields =
            [&]
        {
            auto merged = std::move(lhs.duplicateFields);
            for (auto& [field, literalNames] : rhs.duplicateFields)
            {
                auto& target = merged[field];
                target.insert(target.end(), std::make_move_iterator(literalNames.begin()), std::make_move_iterator(literalNames.end()));
            }
            return merged;
        }(),
    ...
    const std::function<std::expected<QualifiedErasedConfigField, std::vector<QualifiedIdentifier>>(const QualifiedIdentifier&)>& lookupFn)
{
    ...
    std::unordered_map<QualifiedErasedConfigField, QualifiedIdentifier> seenConfigFields;
    ...
    for (const auto& passedValue : passedConfig)
    {
        ...
        if (auto [iter, notExistsYet] = seenConfigFields.emplace(configField.value(), passedValue.getFullyQualifiedName()); !notExistsYet)
        {
            std::erase_if(
                resolvedConfig,
                [&](const InstantiatedConfigValue& val)
                { return val.getOriginalFieldAddress() == configField.value().getOriginalFieldAddress(); });
            auto& duplicateLiteralNames = errors.duplicateFields[configField.value().getFullyQualifiedName()];
            if (duplicateLiteralNames.empty())
            {
                duplicateLiteralNames.emplace_back(iter->second);
            }
            duplicateLiteralNames.emplace_back(passedValue.getFullyQualifiedName());
            continue;
        }
    ...
    Schema<InstantiatedConfigValue, Ordered> config, const Schema<ConfigFieldTransformation, Unordered>& configTransformations)
{
    ...
    return {
        ...
        ConfigResolutionErrors{
            .unresolvableFields = {},
            .duplicateFields = {},
            .failedInstantiations = std::move(failedTransformations),
            .missingFields = {}}};
}
