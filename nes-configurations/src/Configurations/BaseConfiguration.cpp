...
BaseConfiguration::BaseConfiguration(const std::string& name,
...
void BaseConfiguration::parseFromString(
  ...
  if (dynamic_cast<BaseConfiguration*>(option)) {
    dynamic_cast<BaseConfiguration*>(optionMap[identifier])
        ->overwriteConfigWithCommandLineInput(inputParams);
  } else {
    try {
...
void BaseConfiguration::overwriteConfigWithYAMLFileInput(
  ...
  try {
    ...
    throw ConfigurationException(
        "Exception while loading configurations from " + filePath +
        ". Exception: " + ex.what());
  }
}

void BaseConfiguration::overwriteConfigWithCommandLineInput(
  ...
  for (auto parm = inputParams.begin(); parm != inputParams.end(); ++parm) {
    ...
    if (identifier.find('.') != std::string::npos) {
      ...
      auto childrenIdentifier =
          std::string(identifier).substr(index + 1, identifier.length());
...
std::map<std::string, Configurations::BaseOption*>
BaseConfiguration::getOptionMap() {
...
bool BaseConfiguration::persistWorkerIdInYamlConfigFile(
    std::string yamlFilePath, WorkerId workerId, bool withOverwrite) {
  ...
  if (!withOverwrite) {
    ...
    if (!yamlFilePath.empty()) {
      ...
      try {
        ...
        throw ConfigurationException("Exception while persisting in yaml file",
                                     e.what());
      }
    } else {
