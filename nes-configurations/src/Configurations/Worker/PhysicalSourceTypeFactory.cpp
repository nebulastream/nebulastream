...
PhysicalSourceTypePtr PhysicalSourceTypeFactory::createFromString(
  ...
  if (logicalSourceName.empty()) {
    NES_WARNING(
        "No logical source name is not supplied for creating the physical "
        "source. Please supply "
        "logical source name using --{}",
        LOGICAL_SOURCE_NAME_CONFIG);
    return nullptr;
  } else if (physicalSourceName.empty()) {
    NES_WARNING(
        "No physical source name supplied for creating the physical source. "
        "Please supply physical source name using --{}",
        PHYSICAL_SOURCE_NAME_CONFIG);
    return nullptr;
  } else if (sourceType.empty()) {
    NES_WARNING(
        "No source type supplied for creating the physical source. Please "
        "supply source type using --{}",
        SOURCE_TYPE_CONFIG);
    return nullptr;
  }
...
PhysicalSourceTypePtr PhysicalSourceTypeFactory::createFromYaml(
  ...
  if (!yamlConfig[LOGICAL_SOURCE_NAME_CONFIG].As<std::string>().empty() &&
    ...
    NES_THROW_RUNTIME_ERROR(
        "Found Invalid Logical Source Configuration. Please define Logical "
        "Source Name.");
  }
  ...
  if (!yamlConfig[PHYSICAL_SOURCE_NAME_CONFIG].As<std::string>().empty() &&
    ...
    NES_THROW_RUNTIME_ERROR(
        "Found Invalid Physical Source Configuration. Please define Physical "
        "Source Name.");
  }
