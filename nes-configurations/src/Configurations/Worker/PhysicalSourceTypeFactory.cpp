...
PhysicalSourceTypePtr PhysicalSourceTypeFactory::createFromString(
  ...
  for (auto it = commandLineParams.begin(); it != commandLineParams.end();
    ...
    } else if (it->first == PHYSICAL_SOURCE_NAME_CONFIG &&
               !it->second.empty()) {
  ...
  if (logicalSourceName.empty()) {
    NES_WARNING("No logical source name is not supplied for creating the "
                "physical source. Please supply "
                "logical source name using --{}",
                LOGICAL_SOURCE_NAME_CONFIG);
    return nullptr;
  } else if (physicalSourceName.empty()) {
    NES_WARNING("No physical source name supplied for creating the physical "
                "source. Please supply physical source name using --{}",
                PHYSICAL_SOURCE_NAME_CONFIG);
    return nullptr;
  } else if (sourceType.empty()) {
    NES_WARNING("No source type supplied for creating the physical source. "
                "Please supply source type using --{}",
                SOURCE_TYPE_CONFIG);
    return nullptr;
  }

  return createPhysicalSourceType(logicalSourceName, physicalSourceName,
                                  sourceType, commandLineParams);
}

PhysicalSourceTypePtr
PhysicalSourceTypeFactory::createFromYaml(Yaml::Node &yamlConfig) {
  ...
  if (!yamlConfig[LOGICAL_SOURCE_NAME_CONFIG].As<std::string>().empty() &&
    ...
    NES_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. "
                            "Please define Logical Source Name.");
  }
  ...
  if (!yamlConfig[PHYSICAL_SOURCE_NAME_CONFIG].As<std::string>().empty() &&
    ...
    NES_THROW_RUNTIME_ERROR("Found Invalid Physical Source Configuration. "
                            "Please define Physical Source Name.");
  }

  if (!yamlConfig[SOURCE_TYPE_CONFIG].As<std::string>().empty() &&
      yamlConfig[SOURCE_TYPE_CONFIG].As<std::string>() != "\n") {
    sourceType = yamlConfig[SOURCE_TYPE_CONFIG].As<std::string>();
  } else {
    NES_THROW_RUNTIME_ERROR("Found Invalid Physical Source Configuration. "
                            "Please define Source type.");
  }

  if (yamlConfig[PHYSICAL_SOURCE_TYPE_CONFIGURATION].IsMap()) {
    auto physicalSourceTypeConfiguration =
        yamlConfig[PHYSICAL_SOURCE_TYPE_CONFIGURATION];
    return createPhysicalSourceType(logicalSourceName, physicalSourceName,
                                    sourceType,
                                    physicalSourceTypeConfiguration);
  } else {
    NES_THROW_RUNTIME_ERROR(
        "Found Invalid Physical Source Configuration. Please define Source "
...
PhysicalSourceTypePtr PhysicalSourceTypeFactory::createPhysicalSourceType(
  ...
  for (const auto &plugin :
       PhysicalSourceFactoryPluginRegistry ::getPlugins()) {
    if (auto type =
            plugin->createPhysicalSourceType(sourceType, commandLineParams)) {
      return type;
    }
  }
  ...
  case SourceType::CSV_SOURCE:
    return CSVSourceType::create(logicalSourceName, physicalSourceName,
                                 commandLineParams);
  case SourceType::MQTT_SOURCE:
    return MQTTSourceType::create(logicalSourceName, physicalSourceName,
                                  commandLineParams);
  case SourceType::KAFKA_SOURCE:
    return KafkaSourceType::create(logicalSourceName, physicalSourceName,
                                   commandLineParams);
  case SourceType::OPC_SOURCE:
    return OPCSourceType::create(logicalSourceName, physicalSourceName,
                                 commandLineParams);
  case SourceType::BINARY_SOURCE:
    return BinarySourceType::create(logicalSourceName, physicalSourceName,
                                    commandLineParams);
  case SourceType::SENSE_SOURCE:
    return SenseSourceType::create(logicalSourceName, physicalSourceName,
                                   commandLineParams);
  case SourceType::ARROW_SOURCE:
    return ArrowSourceType::create(logicalSourceName, physicalSourceName,
                                   commandLineParams);
  case SourceType::TCP_SOURCE:
    return TCPSourceType::create(logicalSourceName, physicalSourceName,
                                 commandLineParams);
  case SourceType::DEFAULT_SOURCE:
    return DefaultSourceType::create(logicalSourceName, physicalSourceName,
                                     commandLineParams);
  default:
    NES_THROW_RUNTIME_ERROR("SourceConfigFactory:: source type "
...
PhysicalSourceTypePtr PhysicalSourceTypeFactory::createPhysicalSourceType(
    std::string logicalSourceName, std::string physicalSourceName,
    std::string sourceType, Yaml::Node &yamlConfig) {
