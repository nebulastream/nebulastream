...
StaticDataSourceType::StaticDataSourceType(
    const std::string &logicalSourceName, const std::string &physicalSourceName,
    const std::string &pathTableFile, uint64_t numBuffersToProcess,
    SourceMode sourceMode, uint64_t taskQueueId, bool lateStart)
    : PhysicalSourceType(logicalSourceName, physicalSourceName,
                         SourceType::STATIC_DATA_SOURCE),
      pathTableFile(std::move(pathTableFile)),
      numBuffersToProcess(numBuffersToProcess), sourceMode(sourceMode),
      taskQueueId(taskQueueId), lateStart(lateStart) {}

StaticDataSourceTypePtr StaticDataSourceType::create(
    const std::string &logicalSourceName, const std::string &physicalSourceName,
    const std::string &pathTableFile, uint64_t numBuffersToProcess,
    const std::string &sourceMode, uint64_t taskQueueId, bool lateStart) {
  // todo check validity of path
  SourceMode sourceModeEnum =
      magic_enum::enum_cast<SourceMode>(sourceMode).value();
  return std::make_shared<StaticDataSourceType>(
      logicalSourceName, physicalSourceName, pathTableFile, numBuffersToProcess,
      sourceModeEnum, taskQueueId, lateStart);
}
