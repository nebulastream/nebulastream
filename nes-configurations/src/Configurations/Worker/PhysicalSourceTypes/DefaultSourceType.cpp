...
DefaultSourceType::create(const std::string &logicalSourceName,
                          const std::string &physicalSourceName,
                          std::map<std::string, std::string> sourceConfigMap) {
  return std::make_shared<DefaultSourceType>(DefaultSourceType(
    ...
    : PhysicalSourceType(logicalSourceName, physicalSourceName,
      ...
      numberOfBuffersToProduce(
          Configurations::ConfigurationOption<uint32_t>::create(
              Configurations::NUMBER_OF_BUFFERS_TO_PRODUCE_CONFIG, 1,
              "Number of buffers to produce.")),
      sourceGatheringInterval(
          Configurations::ConfigurationOption<uint32_t>::create(
              Configurations::SOURCE_GATHERING_INTERVAL_CONFIG, 1,
...
bool DefaultSourceType::equal(const PhysicalSourceTypePtr &other) {
  return !other->instanceOf<DefaultSourceType>();
}

const Configurations::IntConfigOption &
DefaultSourceType::getNumberOfBuffersToProduce() const {
  return numberOfBuffersToProduce;
}

const Configurations::IntConfigOption &
DefaultSourceType::getSourceGatheringInterval() const {
  return sourceGatheringInterval;
}

Configurations::GatheringModeConfigOption
DefaultSourceType::getGatheringMode() const {
...
void DefaultSourceType::setNumberOfBuffersToProduce(
    uint32_t numberOfBuffersToProduce) {
  this->numberOfBuffersToProduce->setValue(numberOfBuffersToProduce);
}

void DefaultSourceType::setSourceGatheringInterval(
    uint32_t sourceGatheringInterval) {
...
void DefaultSourceType::setGatheringMode(GatheringMode inputGatheringMode) {
