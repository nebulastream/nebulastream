...
KafkaSourceType::KafkaSourceType(const std::string& logicalSourceName, const std::string& physicalSourceName)
    ...
    groupId(Configurations::ConfigurationOption<std::string>::create(
        Configurations::GROUP_ID_CONFIG, "testGroup", "userName, needed for: MQTTSource (can be chosen arbitrary), OPCSource"))
    ,

    topic(Configurations::ConfigurationOption<std::string>::create(Configurations::TOPIC_CONFIG, "testTopic", "topic to listen to"))
    , offsetMode(Configurations::ConfigurationOption<std::string>::create(
          Configurations::OFFSET_MODE_CONFIG, "earliest", "Reading mode default earliest"))
    , connectionTimeout(Configurations::ConfigurationOption<uint32_t>::create(
          Configurations::CONNECTION_TIMEOUT_CONFIG, 10, "connection time out for source, needed for: KafkaSource"))
    , numberOfBuffersToProduce(Configurations::ConfigurationOption<uint32_t>::create(
          Configurations::NUMBER_OF_BUFFER_TO_PRODUCE, 0, "Numbers of events pulled from the queue overall"))
    ,

    batchSize(Configurations::ConfigurationOption<uint32_t>::create(
        Configurations::BATCH_SIZE, 1, "Numbers of events pulled from the queue per pull request"))
    , inputFormat(Configurations::ConfigurationOption<Configurations::InputFormat>::create(
          Configurations::INPUT_FORMAT_CONFIG, Configurations::InputFormat::JSON, "input data format"))
{
...
bool KafkaSourceType::equal(const PhysicalSourceTypePtr& other)
{
    ...
    return brokers->getValue() == otherSourceConfig->brokers->getValue()
        ...
        && groupId->getValue() == otherSourceConfig->groupId->getValue() && topic->getValue() == otherSourceConfig->topic->getValue()
...
Configurations::InputFormatConfigOption KafkaSourceType::getInputFormat() const
{
    return inputFormat;
}
...
Configurations::IntConfigOption KafkaSourceType::getNumberOfBuffersToProduce() const
{
