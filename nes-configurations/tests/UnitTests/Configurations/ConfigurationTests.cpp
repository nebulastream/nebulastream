...
public:
  ...
  makePosixArgs(const std::vector<std::string> &args) {
  ...
  makeCommandLineArgs(const std::vector<std::string> &args) {
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down Configuration test class.");
  }
};
...
TEST_F(ConfigTest, testEmptyParamsAndMissingParamsCoordinatorYAMLFile) {
  ...
  CoordinatorConfigurationPtr coordinatorConfigPtr =
      std::make_shared<CoordinatorConfiguration>();
  coordinatorConfigPtr->overwriteConfigWithYAMLFileInput(
      std::filesystem::path(TEST_DATA_DIRECTORY) / "emptyCoordinator.yaml");
  EXPECT_EQ(coordinatorConfigPtr->restPort.getValue(),
            coordinatorConfigPtr->restPort.getDefaultValue());
  EXPECT_EQ(coordinatorConfigPtr->rpcPort.getValue(),
            coordinatorConfigPtr->rpcPort.getDefaultValue());
  EXPECT_NE(coordinatorConfigPtr->restIp.getValue(),
            coordinatorConfigPtr->restIp.getDefaultValue());
  EXPECT_EQ(coordinatorConfigPtr->coordinatorHost.getValue(),
            coordinatorConfigPtr->coordinatorHost.getDefaultValue());
  EXPECT_NE(coordinatorConfigPtr->worker.numberOfSlots.getValue(),
            coordinatorConfigPtr->worker.numberOfSlots.getDefaultValue());
  EXPECT_EQ(coordinatorConfigPtr->logLevel.getValue(),
            coordinatorConfigPtr->logLevel.getDefaultValue());
  EXPECT_EQ(coordinatorConfigPtr->worker.numberOfBuffersInGlobalBufferManager
                .getValue(),
            coordinatorConfigPtr->worker.numberOfBuffersInGlobalBufferManager
                .getDefaultValue());
  EXPECT_EQ(
  ...
  EXPECT_NE(coordinatorConfigPtr->worker.numberOfBuffersInSourceLocalBufferPool
                .getValue(),
            coordinatorConfigPtr->worker.numberOfBuffersInSourceLocalBufferPool
                .getDefaultValue());
...
TEST_F(ConfigTest, testCoordinatorEPERATPRmptyParamsConsoleInput) {
  ...
  auto commandLineParams = makeCommandLineArgs(
      {"--restIp=localhost", "--worker.numberOfSlots=10",
...
TEST_F(ConfigTest, testEmptyParamsAndMissingParamsWorkerYAMLFile) {
  ...
  workerConfigPtr->overwriteConfigWithYAMLFileInput(
      std::filesystem::path(TEST_DATA_DIRECTORY) / "emptyWorker.yaml");

  EXPECT_NE(workerConfigPtr->localWorkerHost.getValue(),
            workerConfigPtr->localWorkerHost.getDefaultValue());
  EXPECT_EQ(workerConfigPtr->rpcPort.getValue(),
            workerConfigPtr->rpcPort.getDefaultValue());
  EXPECT_EQ(workerConfigPtr->dataPort.getValue(),
            workerConfigPtr->dataPort.getDefaultValue());
  EXPECT_NE(workerConfigPtr->coordinatorPort.getValue(),
            workerConfigPtr->coordinatorPort.getDefaultValue());
  EXPECT_EQ(workerConfigPtr->coordinatorHost.getValue(),
            workerConfigPtr->coordinatorHost.getDefaultValue());
  EXPECT_EQ(workerConfigPtr->numberOfSlots.getValue(),
            workerConfigPtr->numberOfSlots.getDefaultValue());
  EXPECT_EQ(workerConfigPtr->logLevel.getValue(),
            workerConfigPtr->logLevel.getDefaultValue());
  EXPECT_NE(
  ...
  EXPECT_EQ(workerConfigPtr->numberOfBuffersPerWorker.getValue(),
            workerConfigPtr->numberOfBuffersPerWorker.getDefaultValue());
  EXPECT_NE(workerConfigPtr->numberOfBuffersInSourceLocalBufferPool.getValue(),
            workerConfigPtr->numberOfBuffersInSourceLocalBufferPool
                .getDefaultValue());
  EXPECT_EQ(workerConfigPtr->bufferSizeInBytes.getValue(),
            workerConfigPtr->bufferSizeInBytes.getDefaultValue());
  EXPECT_EQ(workerConfigPtr->numberOfBuffersPerEpoch.getValue(),
            workerConfigPtr->numberOfBuffersPerEpoch.getDefaultValue());
  EXPECT_NE(workerConfigPtr->numWorkerThreads.getValue(),
            workerConfigPtr->numWorkerThreads.getDefaultValue());
...
TEST_F(ConfigTest, testWorkerYAMLFileWithMultiplePhysicalSource) {
  ...
  for (const auto &physicalSource :
       workerConfigPtr->physicalSourceTypes.getValues()) {
    EXPECT_TRUE(physicalSource.getValue()->instanceOf<DefaultSourceType>() ||
                physicalSource.getValue()->instanceOf<MQTTSourceType>());
  }
  EXPECT_EQ(workerConfigPtr->locationCoordinates.getValue(),
            workerConfigPtr->locationCoordinates.getDefaultValue());
}
...
TEST_F(ConfigTest, testWorkerYAMLFileFixedLocationNode) {
  WorkerConfigurationPtr workerConfigPtr =
      std::make_shared<WorkerConfiguration>();
  auto yamlPath =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "fixedLocationNode.yaml";
  std::ofstream outFile(yamlPath);
  outFile << "fieldNodeLocationCoordinates: \"23.88,-3.4\"" << std::endl
          << "nodeSpatialType: FIXED_LOCATION" << std::endl;
  workerConfigPtr->overwriteConfigWithYAMLFileInput(yamlPath);

  EXPECT_EQ(workerConfigPtr->locationCoordinates.getValue(),
            NES::Spatial::DataTypes::Experimental::GeoLocation(23.88, -3.4));
  EXPECT_EQ(workerConfigPtr->nodeSpatialType.getValue(),
...
TEST_F(ConfigTest, testWorkerEmptyParamsConsoleInput) {
  ...
  WorkerConfigurationPtr workerConfigPtr =
      std::make_shared<WorkerConfiguration>();
  auto commandLineParams = makeCommandLineArgs(
      {"--localWorkerHost=localhost", "--coordinatorPort=5000",
       "--numWorkerThreads=5", "--numberOfBuffersInGlobalBufferManager=2048",
       ...
       "--queryCompiler.outputBufferOptimizationLevel=ONLY_INPLACE_OPERATIONS_"
       "NO_FALLBACK",
  ...
  EXPECT_NE(
      workerConfigPtr->queryCompiler.pipeliningStrategy.getValue(),
  ...
  EXPECT_NE(
      ...
      workerConfigPtr->queryCompiler.outputBufferOptimizationLevel
          .getDefaultValue());
  EXPECT_NE(workerConfigPtr->locationCoordinates.getValue(),
            workerConfigPtr->locationCoordinates.getDefaultValue());
}
...
TEST_F(ConfigTest, testSourceEmptyParamsConsoleInput) {
  auto commandLineParams =
      makeCommandLineArgs({"type=DEFAULT_SOURCE", "numberOfBuffersToProduce=5",
                           "rowLayout=false", "physicalSourceName=x",
                           "logicalSourceName=default", "offsetMode=earliest"});

  PhysicalSourceTypePtr physicalSourceType1 =
      PhysicalSourceTypeFactory::createFromString("", commandLineParams);
  ...
  EXPECT_TRUE(physicalSource1->getPhysicalSourceType()
                  ->instanceOf<DefaultSourceType>());

  EXPECT_EQ(physicalSourceType1->as<DefaultSourceType>()
                ->getSourceGatheringInterval()
                ->getValue(),
            physicalSourceType1->as<DefaultSourceType>()
                ->getSourceGatheringInterval()
                ->getDefaultValue());
  EXPECT_NE(physicalSourceType1->as<DefaultSourceType>()
                ->getNumberOfBuffersToProduce()
                ->getValue(),
            5u);

  auto commandLineParams1 = makeCommandLineArgs(
      {"type=KAFKA_SOURCE", "physicalSourceName=x", "logicalSourceName=default",
       "topic=newTopic", "connectionTimeout=100", "brokers=testBroker",
       "groupId=testId", "offsetMode=earliest"});

  PhysicalSourceTypePtr physicalSourceType2 =
      PhysicalSourceTypeFactory::createFromString("", commandLineParams1);
  ...
  EXPECT_TRUE(
      physicalSource2->getPhysicalSourceType()->instanceOf<KafkaSourceType>());

  EXPECT_NE(
      physicalSourceType2->as<KafkaSourceType>()->getBrokers()->getValue(),
      physicalSourceType2->as<KafkaSourceType>()
          ->getBrokers()
          ->getDefaultValue());
  EXPECT_EQ(
  ...
  EXPECT_NE(physicalSourceType2->as<KafkaSourceType>()->getTopic()->getValue(),
            physicalSourceType2->as<KafkaSourceType>()
                ->getTopic()
                ->getDefaultValue());
  EXPECT_NE(physicalSourceType2->as<KafkaSourceType>()
...
TEST_F(ConfigTest, testPhysicalSourceAndGatheringModeWorkerConsoleInput) {
  // given
  auto commandLineParams = makeCommandLineArgs(
      {"type=DEFAULT_SOURCE", "numberOfBuffersToProduce=5", "rowLayout=false",
       "physicalSourceName=x", "logicalSourceName=default"});
  // when
  auto physicalSourceType1 =
      PhysicalSourceTypeFactory::createFromString("", commandLineParams);
  // then
  EXPECT_EQ(physicalSourceType1->as<DefaultSourceType>()
                ->getGatheringMode()
...
TEST_F(ConfigTest,
       testCSVPhysicalSourceAndDefaultGatheringModeWorkerConsoleInput) {
  // given
  auto commandLineParams =
      makeCommandLineArgs({"type=CSV_SOURCE", "numberOfBuffersToProduce=5",
                           "rowLayout=false", "physicalSourceName=x",
                           "logicalSourceName=default", "filePath=fileLoc"});
  // when
  ...
  EXPECT_EQ(
      ...
      physicalSourceType->as<CSVSourceType>()
          ->getGatheringMode()
          ->getDefaultValue());
  EXPECT_EQ(
      physicalSourceType->as<CSVSourceType>()->getGatheringMode()->getValue(),
...
TEST_F(ConfigTest,
       testCSVPhysicalSourceAndAdaptiveGatheringModeWorkerConsoleInput) {
  // given
  auto commandLineParams = makeCommandLineArgs(
      {"type=CSV_SOURCE", "numberOfBuffersToProduce=5", "rowLayout=false",
       "physicalSourceName=x", "logicalSourceName=default", "filePath=fileLoc",
...
TEST_F(ConfigTest,
       testWorkerYAMLFileWithCSVPhysicalSourceAdaptiveGatheringMode) {
  ...
  auto csvSourceType = workerConfigPtr->physicalSourceTypes.getValues()[0]
                           .getValue()
                           ->as<CSVSourceType>();
  EXPECT_NE(csvSourceType->getGatheringMode()->getValue(),
            csvSourceType->getGatheringMode()->getDefaultValue());
  EXPECT_EQ(csvSourceType->getGatheringMode()->getValue(),
            magic_enum::enum_cast<GatheringMode>("ADAPTIVE_MODE").value());
}
...
TEST_F(ConfigTest, invalidCommandLineInputForFloatOptions) {
  std::vector<std::pair<std::string, std::vector<std::string>>>
      commandLineArgs = {{"--transferRate", {"not_a_float", "2..5", "-1.0"}}};
...
TEST_F(ConfigTest, invalidCommandLineInputForIpOptions) {
  std::vector<std::pair<std::string, std::vector<std::string>>>
      commandLineArgs = {{"--restIp",
                          {"300.300.300.300", "192.168.1", "not_an_ip",
                           "127.0.0.-1", "192.168..1"}}};
