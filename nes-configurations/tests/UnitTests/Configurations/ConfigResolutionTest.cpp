...
TEST(ConfigResolutionTest, suffixResolvesIntermediateQualification)
{
    ...
    const InstantiatedConfig config{resolved};
...
TEST(ConfigResolutionTest, suffixCollisionMakesLeafNameUnresolvable)
{
    ...
    EXPECT_FALSE(errors.empty());
    EXPECT_EQ(errors.unresolvableFields.size(), 1);
    const auto expectedUnresolvableField = ConfigResolutionErrors::UnresolvableField{
        .targetName = QualifiedIdentifier::parse("timeout"),
        .conflictsWith = std::vector{QualifiedIdentifier::parse("network.timeout"), QualifiedIdentifier::parse("cache.timeout")}};
    EXPECT_EQ(errors.unresolvableFields.at(0), expectedUnresolvableField);
}
...
TEST(ConfigResolutionTest, twoSpellingsResolvingToOneFieldAreReportedAsDuplicates)
{
    const auto schema = twoLevelSchema();
    const Schema<LiteralConfigValue, Ordered> passed{
        literal("threads", ConfigLiteral{int64_t{16}}), literal("engine.query.threads", ConfigLiteral{int64_t{32}})};
    auto [resolved, errors] = resolveConfig(passed, schema);
    EXPECT_FALSE(errors.empty());
    ASSERT_EQ(errors.duplicateFields.size(), 1);
    const auto& [field, literalNames] = *errors.duplicateFields.begin();
    EXPECT_EQ(field, QualifiedIdentifier::parse("engine.query.threads"));
    const std::vector expectedLiteralNames{QualifiedIdentifier::parse("threads"), QualifiedIdentifier::parse("engine.query.threads")};
    EXPECT_EQ(literalNames, expectedLiteralNames);
    const InstantiatedConfig config{resolved};
...
TEST(ConfigResolutionTest, frontendDefaultOverridesFieldDefault)
{
    ...
    const Schema<ConfigFieldDefault, Ordered> frontendDefaults{
        ConfigFieldDefault{QualifiedIdentifier::parse("engine.query.threads"), [] { return ConfigLiteral{int64_t{32}}; }}};
...
TEST(ConfigResolutionTest, frontendDefaultDoesNotMatchOnSuffix)
{
    const auto schema = twoLevelSchema();
    const Schema<ConfigFieldDefault, Ordered> frontendDefaults{
        ConfigFieldDefault{QualifiedIdentifier::parse("threads"), [] { return ConfigLiteral{int64_t{32}}; }}};
    auto [resolved, errors] = resolveConfig(Schema<LiteralConfigValue, Ordered>{}, schema, frontendDefaults);
    EXPECT_TRUE(errors.empty()) << errors;
    const InstantiatedConfig config{resolved};
    EXPECT_EQ(config.get(THREADS), 4);
