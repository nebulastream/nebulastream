...
class QueryCompilerConfiguration : public BaseConfiguration {
 public:
  ...
  QueryCompilerConfiguration(const std::string& name,
                             const std::string& description)
      : BaseConfiguration(name, description){};

  /**
   * @brief Sets the compilation strategy. We differentiate between FAST, DEBUG,
  ...
  EnumOption<QueryCompilation::QueryCompilerType> queryCompilerType = {
      ...
      "Indicates the type for the query compiler "
      "[DEFAULT_QUERY_COMPILER|NAUTILUS_QUERY_COMPILER]."};

  /**
   * @brief Sets the dump mode for the query compiler. We differentiate between
   * NONE, CONSOLE, FILE, and FILE_AND_CONSOLE.
   * @note This setting is only for the nautilus compiler
   */
  EnumOption<QueryCompilation::DumpMode> queryCompilerDumpMode = {
      QUERY_COMPILER_DUMP_MODE, QueryCompilation::DumpMode::NONE,
      "Indicates the type for the query compiler "
      "[NONE|CONSOLE|FILE|FILE_AND_CONSOLE]."};
  ...
  EnumOption<QueryCompilation::CompilationStrategy> compilationStrategy = {
      ...
      "Indicates the optimization strategy for the query compiler "
      "[FAST|DEBUG|OPTIMIZE|PROXY_INLINING]."};

  /**
   * @brief Sets the backend for nautilus. We differentiate between
   * MLIR_COMPILER_BACKEND, INTERPRETER, BC_INTERPRETER_BACKEND, and
   * FLOUNDER_COMPILER_BACKEND compilation.
   */
  EnumOption<QueryCompilation::NautilusBackend> nautilusBackend = {
      ...
      "[MLIR_COMPILER_BACKEND|INTERPRETER|BC_INTERPRETER_BACKEND|FLOUNDER_"
      "COMPILER_BACKEND]."};

  /**
   * @brief Sets the pipelining strategy. We differentiate between an
   * OPERATOR_FUSION and OPERATOR_AT_A_TIME strategy.
   */
  EnumOption<QueryCompilation::PipeliningStrategy> pipeliningStrategy = {
      ...
      "Indicates the pipelining strategy for the query compiler "
      "[OPERATOR_FUSION|OPERATOR_AT_A_TIME]."};

  /**
   * @brief Sets the output buffer optimization level.
   */
  EnumOption<QueryCompilation::OutputBufferOptimizationLevel>
  ...
  EnumOption<QueryCompilation::StreamJoinStrategy> joinStrategy = {
      JOIN_STRATEGY, QueryCompilation::StreamJoinStrategy::NESTED_LOOP_JOIN,
      "Indicates the windowingStrategy"
      "[HASH_JOIN_LOCAL|HASH_JOIN_GLOBAL_LOCKING|HASH_JOIN_GLOBAL_LOCK_FREE|"
      "NESTED_LOOP_JOIN]. "};

  /**
   * @brief Sets the path to the locally installed CUDA SDK.
   */
  StringOption cudaSdkPath = {CUDA_SDK_PATH, "/usr/local/cuda",
                              "Path to CUDA SDK."};
