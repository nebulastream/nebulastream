...
public:
  ...
  /**
   * @brief Get cleanSession true = clean up session after client loses
   * connection, false = keep data for client after connection loss (persistent
   * session)
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<bool>>
  getCleanSession() const;

  /**
   * @brief Set cleanSession true = clean up session after client loses
   * connection, false = keep data for client after connection loss (persistent
   * session)
   */
...
private:
  /**
   * @brief constructor to create a new MQTT source config object initialized
   ...
   */
  MQTTSourceType(const std::string &logicalSourceName,
                 const std::string &physicalSourceName);
