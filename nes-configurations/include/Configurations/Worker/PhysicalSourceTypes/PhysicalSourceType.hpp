 ...
 */
class PhysicalSourceType
    : public std::enable_shared_from_this<PhysicalSourceType> {

public:
  PhysicalSourceType(std::string logicalSourceName,
                     std::string physicalSourceName, SourceType sourceType);
   ...
   */
  template <class SourceType> bool instanceOf() {
  ...
  template <class SourceType> std::shared_ptr<SourceType> as() {
    ...
    throw std::logic_error(
        "PhysicalSourceType:: we performed an invalid cast of operator " +
        this->toString() + " to type " + typeid(SourceType).name());
  }
