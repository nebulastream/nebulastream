...
class TCPSourceType : public PhysicalSourceType {
 public:
   ...
   * @param domain string viable options: AF_INET IPv4 Internet protocols,
   ...
   * fixed maximum length), SOCK_SEQPACKET Provides  a  sequenced,  reliable,
   * two-way connection-based data transmission path  for  datagrams  of  fixed
   * maximum  length;  a consumer is required to read an entire packet with each
   * input system call, SOCK_RAW Provides raw network protocol access, SOCK_RDM
  ...
  getInputFormat() const;

  /**
   * @brief Sets decideMessageSize given as Configuration::TCPDecideMessageSize
   * @param decideMessageSize TUPLE_SEPARATOR: TCP messages are send with a char
   * acting as tuple separator between them, tupleSeperator needs to be set
   * USER_SPECIFIED_BUFFER_SIZE: User specifies the buffer size beforehand,
   * socketBufferSize needs to be set BUFFER_SIZE_FROM_SOCKET: Between each
   * message you also obtain a fixed amount of bytes with the size of the next
   * message, bytesUsedForSocketBufferSizeTransfer needs to be set
   */
  void setDecideMessageSize(
      Configurations::TCPDecideMessageSize decideMessageSizeValue);

  /**
   * @brief Get the decideMessageSize given as
   * Configuration::TCPDecideMessageSize
   * @return decideMessageSizeValue
   * TUPLE_SEPARATOR: TCP messages are send with a char acting as tuple
   * separator between them, tupleSeperator needs to be set
   * USER_SPECIFIED_BUFFER_SIZE: User specifies the buffer size beforehand,
   * socketBufferSize needs to be set BUFFER_SIZE_FROM_SOCKET: Between each
   * message you also obtain a fixed amount of bytes with the size of the next
   * message, bytesUsedForSocketBufferSizeTransfer needs to be set
   */
  [[nodiscard]] Configurations::TCPDecideMessageSizeConfigOption
  getDecideMessageSize() const;

  /**
   * @brief Get tupleBuffer flush interval in milliseconds
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<float>>
  getFlushIntervalMS() const;
   ...
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<char>>
  getTupleSeparator() const;
  ...
  /**
   * @brief get socket buffer size, i.e. fix the message size for tuple data
   * that is send via socket to this value
   * @return socketBufferSize
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<uint32_t>>
  getSocketBufferSize() const;

  /**
   * @brief set socketBufferSize, i.e. fix the message size for tuple data that
   * is send via socket to this value
   * @param socketBufferSizeValue size of the message that is send over the
   * socket
   */
  void setSocketBufferSize(uint32_t socketBufferSizeValue);

  /**
   * @brief get bytesUsedForSocketBufferSizeTransfer, i.e. the number of bytes
   * that are send between messages to indicate the messages' sizes
   * @return bytesUsedForSocketBufferSizeTransfer
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<uint32_t>>
  getBytesUsedForSocketBufferSizeTransfer() const;

  /**
   * @brief bytesUsedForSocketBufferSizeTransfer, i.e. the number of bytes that
   * are send between messages to indicate the messages' sizes
   * @param bytesUsedForSocketBufferSizeTransferValue new socket port
   */
  void setBytesUsedForSocketBufferSizeTransfer(
      uint32_t bytesUsedForSocketBufferSizeTransferValue);

 private:
  /**
   * @brief constructor to create a new TCP source type object initialized with
   * values from sourceConfigMap
   * @param sourceConfigMap inputted config options
   */
