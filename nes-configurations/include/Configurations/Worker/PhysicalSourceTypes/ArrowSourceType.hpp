 ...
 * define configurations for a Arrow IPC file source, i.e. this source reads
 * from data from an Arrow IPC file
 */
...
public:
  ...
  void setNumberOfTuplesToProducePerBuffer(
      const uint32_t numberOfTuplesToProducePerBuffer);

private:
  /**
   * @brief constructor to create a new Arrow source config object initialized
   ...
   * with default values
   */
  ArrowSourceType(const std::string &logicalSourceName,
                  const std::string &physicalSourceName);
