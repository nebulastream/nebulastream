...
class KafkaSourceType : public PhysicalSourceType {
 public:
  ...
  /**
   * @brief Get auto commit, boolean value where 1 equals true, and 0 equals
   * false, needed for: KafkaSource
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<uint32_t>>
  getAutoCommit() const;

  /**
   * @brief Set auto commit, boolean value where 1 equals true, and 0 equals
   * false, needed for: KafkaSource
   */
   ...
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<uint32_t>>
  getNumberOfBuffersToProduce() const;

  /**
   * @brief Get batchSize as the number of messages to pull in one chunk form
   * kafka
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<uint32_t>>
  getBatchSize() const;
  ...
  /**
   * @brief Set batch size as the number of messages pulled from kafka in one
   * chunk
   */
 ...
 private:
  /**
   * @brief constructor to create a new Kafka source config object initialized
   ...
   * with default values
   */
  KafkaSourceType(const std::string& logicalSourceName,
                  const std::string& physicalSourceName);
