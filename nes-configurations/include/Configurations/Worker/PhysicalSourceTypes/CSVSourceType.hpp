 ...
 * define configurations for a csv source, i.e. this source reads from data from
 * a csv file
 */
...
public:
   ...
   * @param logicalSourceName: Name of the logical source that represents this
   * CSV source.
   * @param physicalSourceName:: Name of the physical source, that is attached
   * to the logical source.
   ...
   */
  [[nodiscard]] std::shared_ptr<Configurations::ConfigurationOption<bool>>
  getSkipHeader() const;
  ...
  getGatheringInterval() const;

  /**
   * @brief set the value for sourceGatheringInterval with the appropriate data
   * format
   */
  ...
  /**
   * @brief set the value for numberOfBuffersToProduce with the appropriate data
   * format
   */
  void setNumberOfBuffersToProduce(uint32_t numberOfBuffersToProduce);

  /**
   * @brief gets a ConfigurationOption object with
  ...
  /**
   * @brief set the value for numberOfTuplesToProducePerBuffer with the
   * appropriate data format
   */
  void setNumberOfTuplesToProducePerBuffer(
      uint32_t numberOfTuplesToProducePerBuffer);

private:
  /**
   * @brief constructor to create a new CSV source config object initialized
