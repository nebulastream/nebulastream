...
class WorkerConfiguration : public BaseConfiguration {
 public:
  ...
  WorkerConfiguration(std::string name, std::string description)
  ...
  static WorkerConfigurationPtr create() {
    return std::make_shared<WorkerConfiguration>();
  }
   ...
   */
  StringOption localWorkerHost = {LOCAL_WORKER_HOST_CONFIG, "127.0.0.1",
                                  "Worker IP or hostname."};

  /**
   * @brief Port for the RPC server of the Worker.
   * This is used to receive control messages from the coordinator or other
   * workers .
   */
   ...
   */
  UIntOption dataPort = {DATA_PORT_CONFIG,
                         "0",
                         "Data port of the NES Worker.",
                         {std::make_shared<NumberValidation>()}};

  /**
   * @brief Server IP of the NES Coordinator to which the NES Worker should
   * connect.
   */
  StringOption coordinatorHost = {
      COORDINATOR_HOST_CONFIG, "127.0.0.1",
      "Server IP or hostname of the NES Coordinator to which the NES Worker "
      "should connect."};
  /**
   * @brief RPC server Port of the NES Coordinator to which the NES Worker
   * should connect. Needs to be set and needs to be the same as rpcPort in
   * Coordinator.
   */
  UIntOption coordinatorPort = {
      COORDINATOR_PORT_CONFIG,
      "4000",
      "RPC server Port of the NES Coordinator to which the NES Worker should "
      "connect. Needs to be set and needs "
  ...
  /**
   * @brief Number of Slots define the amount of computing resources that are
   * usable at the coordinator. This enables the restriction of the amount of
   * concurrently deployed queryIdAndCatalogEntryMapping and operators.
   */
   ...
   */
  UIntOption bandwidth = {BANDWIDTH_IN_MBPS,
                          "0",
   ...
   */
  UIntOption latency = {LATENCY_IN_MS,
                        "0",
                        "The link latency in milliseconds.",
                        {std::make_shared<NumberValidation>()}};
  ...
  UIntOption numberOfBuffersPerWorker = {
      NUMBER_OF_BUFFERS_PER_WORKER_CONFIG,
      "128",
   ...
   * This property controls the backpressure mechanism as a data source that
   * can't allocate new records can't ingest more data.
   */
  ...
  /**
   * @brief Configures the wait time for collecting metrics in the monitoring
   * streams. Monitoring has to be enabled for it to work.
   */
  ...
  /**
   * @brief Indicates a list of cpu cores, which are used to pin data sources to
   * specific cores.
   * @deprecated this value is deprecated and will be removed.
   */
  StringOption sourcePinList = {
      SOURCE_PIN_LIST_CONFIG, "",
      "comma separated list of where to map the sources"};

  /**
   * @brief Indicates a list of cpu cores, which are used  to pin worker threads
   * to specific cores.
   * @deprecated this value is deprecated and will be removed.
   */
  StringOption workerPinList = {
      WORKER_PIN_LIST_CONFIG, "",
   ...
   * @deprecated this value is deprecated and will be removed.
   */
  StringOption queuePinList = {
      QUEUE_PIN_LIST_CONFIG, "",
   ...
   */
  QueryCompilerConfiguration queryCompiler = {
      QUERY_COMPILER_CONFIG, "Configuration for the query compiler"};

  /**
   * @brief Indicates a sequence of physical sources.
   */
  SequenceOption<WrapOption<PhysicalSourceTypePtr, PhysicalSourceTypeFactory>>
      physicalSourceTypes = {PHYSICAL_SOURCES, "Physical sources"};
  ...
  WrapOption<NES::Spatial::DataTypes::Experimental::GeoLocation,
      ...
      locationCoordinates = {LOCATION_COORDINATES_CONFIG,
                             "the physical location of the worker"};

  /**
   * @brief specify if the worker is running on a mobile device, if it is a node
   * with a known fixed loction, or if it does not have a known location.
   */
  EnumOption<NES::Spatial::Experimental::SpatialType> nodeSpatialType = {
      SPATIAL_TYPE_CONFIG, NES::Spatial::Experimental::SpatialType::NO_LOCATION,
      "specifies if the worker has no known location or if it is a fixed "
      "location node or mobile node"};

  /**
   * @brief specifies the path to a yaml file containing a mobility
   * configuration
   */
  Spatial::Mobility::Experimental::WorkerMobilityConfiguration
...
#ifdef TFDEF
  BoolOption isTensorflowSupported = {
      TENSORFLOW_SUPPORTED_CONFIG, false,
      "Tensorflow model execution supported by the worker"};
#endif  // TFDEF
   ...
   */
  UIntOption numberOfQueues = {NUMBER_OF_QUEUES,
                               "1",
   ...
   *      - Static: use queue per query and a specified number of threads per
   * queue
   */
  EnumOption<Runtime::QueryExecutionMode> queryManagerMode = {
      QUERY_MANAGER_MODE, Runtime::QueryExecutionMode::Dynamic,
      "Which mode the query manager is running in. (Dynamic, Static, "
      "NumaAware, Invalid)"};
  ...
  UIntOption senderHighwatermark = {SENDER_HIGH_WATERMARK,
                                    "8",
                                    "Number of tuple buffers allowed in one "
                                    "network channel before blocking transfer.",
                                    {std::make_shared<NumberValidation>()}};
  ...
  /**
   * @brief Let network sources use a separate thread to establish an event
   * channel to their upstream sink
   */
  BoolOption connectSourceEventChannelsAsync = {
      CONNECT_SOURCE_ASYNC,
      "false",
      "Let network sources use a separate thread to establish a the upstream "
      "event channel",
      {std::make_shared<BooleanValidation>()}};
