...
enum class OutputBufferOptimizationLevel : uint8_t {
  ...
  ALL,
  // create separate result buffer and copy everything over after all operations
  // are applied.
  // Check size after every written tuple.
  NO,
  // If all records and all fields match up in input and result buffer we can
  // simply emit the input buffer.
