...
template <IsEnum T>
EnumOption<T>::EnumOption(const std::string &name, T defaultValue,
                          const std::string &description)
    : TypedBaseOption<T>(name, defaultValue, description){};

template <IsEnum T> EnumOption<T> &EnumOption<T>::operator=(const T &value) {
  this->value = value;
  return *this;
}

template <IsEnum T> void EnumOption<T>::parseFromYAMLNode(Yaml::Node node) {
  ...
  if (!magic_enum::enum_contains<T>(node.As<std::string>())) {
    ...
    throw ConfigurationException("Enum for " + node.As<std::string>() +
                                 " was not found. Valid options are " +
                                 ss.str());
  }
...
template <IsEnum T>
void EnumOption<T>::parseFromString(
  ...
  if (!magic_enum::enum_contains<T>(value)) {
    ...
    throw ConfigurationException(
        "Enum for " + value + " was not found. Valid options are " + ss.str());
  }
  this->value = magic_enum::enum_cast<T>(value).value();
}

template <IsEnum T> std::string EnumOption<T>::toString() {
  std::stringstream os;
  os << "Name: " << this->name << "\n";
  os << "Description: " << this->description << "\n";
  os << "Value: " << std::string(magic_enum::enum_name(this->value)) << "\n";
  os << "Default Value: "
     << std::string(magic_enum::enum_name(this->defaultValue)) << "\n";
  return os.str();
}
