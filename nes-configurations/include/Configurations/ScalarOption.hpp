...
/**
 * @brief This class provides a general implementation for all ScalarOption<T>
 * this is used e.g., for IntOption, StringOption, BoolOption.
 * @tparam T of the value.
 */
template <class T> class ScalarOption : public TypedBaseOption<T> {
public:
  /**
   ...
   * @param description of the option.
   */
  ScalarOption(const std::string &name, const std::string &defaultValue,
...
private:
  ...
  /**
   * @brief Private constructor to create an scalar option without a name and
   * description. This can only be used in SequenceOptions.
   */
...
template <class T> std::string ScalarOption<T>::toString() {
  std::stringstream os;
  os << "Name: " << this->name << "\n";
...
ScalarOption<T>::ScalarOption(const std::string &name,
                              const std::string &description)
    : TypedBaseOption<T>(name, description) {}

template <class T>
ScalarOption<T>::ScalarOption(const std::string &name, const std::string &value,
                              const std::string &description)
...
template <class T> ScalarOption<T> &ScalarOption<T>::operator=(const T &value) {
  this->value = value;
  return *this;
}

template <class T> bool ScalarOption<T>::operator==(const BaseOption &other) {
  return TypedBaseOption<T>::operator==(other);
}

template <class T> bool ScalarOption<T>::operator==(const T &other) {
  return this->value == other;
}

template <class T> void ScalarOption<T>::parseFromYAMLNode(Yaml::Node node) {
  this->isValid(node.As<std::string>());
  this->value = node.As<T>();
}

template <class T>
void ScalarOption<T>::parseFromString(
  ...
  try {
    ...
    throw ConfigurationException("Conversion failed for " + identifier +
                                 " with value: " + value +
                                 ". Exception: " + e.what());
  }
}
