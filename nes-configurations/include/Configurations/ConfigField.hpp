...
/// source-defined config struct that is created using ConfigFields, not through individual config fields.
...
/// Because the identity of a ConfigField is defined through its instance,
/// this would result in the "same" ConfigField existing multiple times and not resolving correctly.
template <typename T>
class ConfigField
{
    Identifier name;
...
/// This requires each ConfigField to have a static lifetime.
...
/// Type-erased view of a ConfigField, addressable by its fully qualified name, to be used for example in containers.
...
public:
    ...
    [[nodiscard]] bool operator==(const QualifiedErasedConfigField& other) const;
...
/// converts and rejects values outside the target's range. Max and Min restricts it further, for example 65535 for a port:
/// tryGetOr<int64_t>(literal, expectedType<uint32_t>()).and_then(narrowConfigValue<int64_t, uint32_t, 65535>)
template <typename From, typename To, auto Max = std::numeric_limits<To>::max(), auto Min = std::numeric_limits<To>::lowest()>
...
/// Frontend-specific default values for config fields, defined through a fully qualified name and a literal supplier
...
template <>
struct std::hash<NES::QualifiedErasedConfigField>
{
    size_t operator()(const NES::QualifiedErasedConfigField& obj) const noexcept;
};
