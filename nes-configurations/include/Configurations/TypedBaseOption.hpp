...
/**
 * @brief This class is a base class, which represents an option that holds
 * values of a particular type T.
 * @tparam T of the value.
 */
template <class T> class TypedBaseOption : public BaseOption {
public:
  /**
   * @brief Constructor to create a new option without initializing any members.
   * This is required to create nested options, e.g., a IntOption  that is part
   * of a sequence.
   */
  TypedBaseOption();
  ...
  /**
   * @brief Constructor to create a new option that declares a specific default
   * value.
   * @param name of the option.
   * @param defaultValue of the option. Has to be of type T.
   * @param description of the option.
   */
  TypedBaseOption(const std::string &name, T defaultValue,
   ...
   */
  TypedBaseOption(
      const std::string &name, T defaultValue, const std::string &description,
...
protected:
  ...
  /**
   * @brief Iterates over all validators of this option before setting a new
   * value.
...
template <class T> TypedBaseOption<T>::TypedBaseOption() : BaseOption() {}
...
template <class T>
TypedBaseOption<T>::TypedBaseOption(const std::string &name, T defaultValue,
                                    const std::string &description)
    : BaseOption(name, description), value(defaultValue),
...
TypedBaseOption<T>::TypedBaseOption(
    ...
    std::vector<std::shared_ptr<ConfigurationValidation>> validators)
    : BaseOption(name, description), value(defaultValue),
      defaultValue(defaultValue), validators(validators) {}

template <class T> T TypedBaseOption<T>::getValue() const { return value; };

template <class T> void TypedBaseOption<T>::setValue(T newValue) {
  this->value = newValue;
}

template <class T> const T &TypedBaseOption<T>::getDefaultValue() const {
  return defaultValue;
}
template <class T> void TypedBaseOption<T>::clear() {
  this->value = defaultValue;
}

template <class T> void TypedBaseOption<T>::isValid(std::string pValue) {
  bool invalid = false;
  ...
  for (auto validator : this->validators) {
    if (!(validator->isValid(pValue))) {
      ...
      message = "Validator (" + validatorName + ") failed for " + this->name +
                " with value: " + pValue;
