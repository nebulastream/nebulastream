 ...
 */
template <class T> class ConfigurationOption {
public:
  static std::shared_ptr<ConfigurationOption> create(std::string name, T value,
                                                     std::string description) {
    return std::make_shared<ConfigurationOption>(
        ConfigurationOption(name, value, description));
  };
  ...
  /**
   * @brief converts config object to human readable form, only prints name and
   * current value
  ...
  /**
   * @brief converts a string to the appropriate InputFormat enum value and sets
   * it
   * @param inputFormat
   */
  ...
  /**
   * @brief converts a string to the appropriate TCPDecideMessageSize enum value
   * and sets it
...
private:
   ...
   */
  explicit ConfigurationOption(std::string name, T value,
                               std::string description)
      : name(std::move(name)), description(std::move(description)),
        value(value), defaultValue(value) {}

  ConfigurationOption(std::string name, T value, T defaultValue,
                      std::string description)
      : name(std::move(name)), description(std::move(description)),
...
using InputFormatConfigOption =
    std::shared_ptr<ConfigurationOption<InputFormat>>;
using GatheringModeConfigOption =
    std::shared_ptr<ConfigurationOption<GatheringMode>>;
using TCPDecideMessageSizeConfigOption =
    std::shared_ptr<ConfigurationOption<TCPDecideMessageSize>>;
