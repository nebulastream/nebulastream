...
/**
 * @brief This class provides a general option, that can wrap an object of
 * arbitrary Type as an option. To this end, users have to specific a Factory
 * which implements the following two static functions:
 * - Type Factory::createFromString(std::string) -> creates an object of the
 * option Type from an string value.
 * - Type Factory::createFromYaml(Yaml::Node) -> creates an object of the option
 * Type from an YAML node. This allows users to represent custom domain types as
 * members of a configuration object, e.g., see PhysicalSources in the
 * WorkerConfiguration.
 * @note WrapOptions can't have a default type.
 * @tparam Type of the object that is wrapped by the option.
 * @tparam Factory type which implements the static create function to
 * initialize a value of this option.
 */
template <class Type, class Factory>
...
public:
  /**
   * @brief Constructor to create a new option that sets a name, and
   * description.
...
private:
  template <DerivedBaseOption X> friend class SequenceOption;
  WrapOption() : TypedBaseOption<Type>() {}
};

template <class Type, class Factory>
  requires IsFactory<Type, Factory>
WrapOption<Type, Factory>::WrapOption(const std::string &name,
                                      const std::string &description)
    : TypedBaseOption<Type>(name, description) {}

template <class Type, class Factory>
  requires IsFactory<Type, Factory>
void WrapOption<Type, Factory>::parseFromString(
    std::string identifier, std::map<std::string, std::string> &inputParams) {
