...
/// Parse a single literal sanely for CLI. First, trims leading and trailing white space, then tries to infer type.
/// Type inference rules:
/// Quoted with single quotes '             : string
/// true, false (case-insensitive)          : bool
/// null (case-insenstive)                  : std::monostate
/// only digits and optionally a leading -  : an int64, or an error if the number is out of bounds of an int64
/// digits and single . (fixpoint notation) : double
/// otherwise                               : string
[[nodiscard]] std::expected<ConfigLiteral, Exception> parseConfigLiteral(const std::string& raw);
