...
/**
 * @brief This class implements sequential options of a type that has to be a
 * subtype of the BaseOption.
 * @note SequenceOptions can't have default values.
 * @tparam Type of the component.
 */
template <DerivedBaseOption T> class SequenceOption : public BaseOption {
public:
  /**
  ...
  template <class X> void add(X value) {
    auto option = T();
...
SequenceOption<T>::SequenceOption(const std::string &name,
                                  const std::string &description)
    : BaseOption(name, description){};

template <DerivedBaseOption T> void SequenceOption<T>::clear() {
  options.clear();
}
...
void SequenceOption<T>::parseFromYAMLNode(Yaml::Node node) {
  if (node.IsSequence()) {
    ...
    throw ConfigurationException(
        "YAML node should be a sequence but it was a " +
        node.As<std::string>());
  }
}
template <DerivedBaseOption T>
void SequenceOption<T>::parseFromString(
...
template <DerivedBaseOption T> size_t SequenceOption<T>::size() const {
  return options.size();
}
...
template <DerivedBaseOption T> bool SequenceOption<T>::empty() const {
  return options.empty();
}

template <DerivedBaseOption T> std::string SequenceOption<T>::toString() {
  std::stringstream os;
  os << "Name: " << name << "\n";
