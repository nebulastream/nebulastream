...
def main():
    if len(sys.argv) > 1:
        print(
            'Usage: pipe git diff into this to turn added #include "..." to #inlcude <...>, e.g:'
        )
        print()
    ...
    quoted_inlcude_regex = re.compile('(.*)#include "([^"]*)"(.*)')
    ...
    for file, lines_with_quoted_include in foo.items():
        ...
        for line_no, line in lines_with_quoted_include:
            ...
            line = re.sub(quoted_inlcude_regex, r"\1#include <\2>\3", line)
