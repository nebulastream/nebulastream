...
parser = argparse.ArgumentParser(
    description="Python script to run benchmarks."
)
...
CMAKE_TOOLCHAIN_FILE = "/home/saralhamo/vcpkg/scripts/buildsystems/vcpkg.cmake"
NEBULI_PATH = os.path.join(args.source_dir, BUILD_DIR, "nes-nebuli/nes-nebuli")
SINGLE_NODE_PATH = os.path.join(args.source_dir, BUILD_DIR, "nes-single-node-worker/nes-single-node-worker")
TCP_SERVER = os.path.join(args.source_dir, BUILD_DIR, "window_management/tcpserver")
...
def compile_project():
    cmd_cmake = ["cmake",
                 ...
                 f"-DCMAKE_CXX_COMPILER=/usr/bin/clang++-19",
                #  f"-DCMAKE_CXX_COMPILER=/usr/bin/g++-14",
                 f"-DNES_LOG_LEVEL={LOG_LEVEL}",
                 f"-DCMAKE_TOOLCHAIN_FILE={CMAKE_TOOLCHAIN_FILE}",
                 f"-DUSE_LIBCXX_IF_AVAILABLE:BOOL=OFF",
