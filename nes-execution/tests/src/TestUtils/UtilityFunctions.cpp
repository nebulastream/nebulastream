...
std::vector<TupleBuffer> createDataForOneFieldAndTimeStamp(
    int numberOfTuples, BufferManager &bufferManager, SchemaPtr schema,
    const std::string &fieldToBuildCountMinOver,
    const std::string &timestampFieldName, const bool ingestionTime) {
...
void updateTestCountMinStatistic(
  ...
  std::unique_ptr<Nautilus::Interface::HashFunction> h3Hash =
      std::make_unique<Nautilus::Interface::H3Hash>(numberOfBitsInKey);

  // 2. For each tuple in the buffer, we get the corresponding count min
  // statistic and then update it accordingly
  ...
  for (auto tuple : testTupleBuffer) {
    ...
    auto allCountMinStatistics =
        statisticStore->getStatistics(statisticHash, startTs, endTs);
    Statistic::StatisticPtr countMinStatistic;
    if (allCountMinStatistics.empty()) {
      countMinStatistic = Statistic::CountMinStatistic::createInit(
          startTs, endTs, width, depth, numberOfBitsInKey);
      statisticStore->insertStatistic(statisticHash, countMinStatistic);
      NES_DEBUG("Created and inserted new countMinStatistic = {} for "
                "statisticHash = {}",
                countMinStatistic->toString(), statisticHash);
    } else {
    ...
    for (auto row = 0_u64; row < depth; ++row) {
      ...
      auto h3SeedsOffSet =
          row * ((numberOfBitsInKey * numberOfBitsInHashValue) / 8);
      Nautilus::Value<Nautilus::MemRef> h3SeedMemRef(h3SeedsStart +
                                                     h3SeedsOffSet);
      Nautilus::Value<Nautilus::Int64> valKey(
          tuple[fieldToBuildCountMinOver].read<int64_t>());
      auto calcHash = h3Hash->calculateWithState(valKey, h3SeedMemRef)
                          .getValue()
                          .getValue();
...
void updateTestHyperLogLogStatistic(
    MemoryLayouts::TestTupleBuffer &testTupleBuffer,
    Statistic::StatisticStorePtr statisticStore,
    Statistic::StatisticMetricHash metricHash, uint64_t windowSize,
    uint64_t windowSlide, uint64_t width,
    const std::string &fieldToBuildCountMinOver,
    const std::string &timestampFieldName) {

  // For each tuple in the buffer, we get the corresponding hyperloglog
  // statistic and then update it accordingly
  std::unique_ptr<Nautilus::Interface::HashFunction> murmurHash =
  ...
  for (auto tuple : testTupleBuffer) {
    ...
    auto allHyperLogLogStatistics =
        statisticStore->getStatistics(statisticHash, startTs, endTs);
    Statistic::StatisticPtr hllStatistic;
    if (allHyperLogLogStatistics.empty()) {
      hllStatistic =
          Statistic::HyperLogLogStatistic::createInit(startTs, endTs, width);
      statisticStore->insertStatistic(statisticHash, hllStatistic);
      NES_DEBUG(
          "Created and inserted new hllStatistic = {} for statisticHash = {}",
    ...
    hllStatistic->as<Statistic::HyperLogLogStatistic>()->update(
        hash->getValue());
  }
}

Runtime::TupleBuffer getBufferFromPointer(uint8_t *recordPtr,
  ...
  for (auto &field : schema->fields) {
    auto const fieldType =
        physicalDataTypeFactory.getPhysicalType(field->getDataType());
...
void writeNautilusRecord(uint64_t recordIndex, int8_t *baseBufferPtr,
                         Nautilus::Record nautilusRecord, SchemaPtr schema,
                         BufferManagerPtr bufferManager) {
  ...
  if (schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
    auto rowMemoryLayout = Runtime::MemoryLayouts::RowLayout::create(
        schema, bufferManager->getBufferSize());
    auto memoryProviderPtr =
        std::make_unique<MemoryProvider::RowMemoryProvider>(rowMemoryLayout);

    memoryProviderPtr->write(nautilusRecordIndex, nautilusBufferPtr,
                             nautilusRecord);

  } else if (schema->getLayoutType() ==
             Schema::MemoryLayoutType::COLUMNAR_LAYOUT) {
    auto columnMemoryLayout = Runtime::MemoryLayouts::ColumnLayout::create(
        schema, bufferManager->getBufferSize());
    auto memoryProviderPtr =
        std::make_unique<MemoryProvider::ColumnMemoryProvider>(
            columnMemoryLayout);
...
Runtime::TupleBuffer
mergeBuffers(std::vector<Runtime::TupleBuffer> &buffersToBeMerged,
             const SchemaPtr schema, Runtime::BufferManagerPtr bufferManager) {

  auto retBuffer = bufferManager->getBufferBlocking();
  auto retBufferPtr = retBuffer.getBuffer();

  auto maxPossibleTuples =
      retBuffer.getBufferSize() / schema->getSchemaSizeInBytes();
  auto cnt = 0UL;
  for (auto &buffer : buffersToBeMerged) {
    ...
    auto bufferSize =
        buffer.getNumberOfTuples() * schema->getSchemaSizeInBytes();
...
mergeBuffersSameWindow(std::vector<Runtime::TupleBuffer> &buffers,
                       SchemaPtr schema, const std::string &timeStampFieldName,
                       BufferManagerPtr bufferManager, uint64_t windowSize) {
  if (buffers.size() == 0) {
    return {};
  }
  ...
  if (schema->getLayoutType() == Schema::MemoryLayoutType::COLUMNAR_LAYOUT) {
    NES_FATAL_ERROR(
        "Column layout is not support for this function currently!");
  }
  ...
  for (auto buf : buffers) {
    auto memoryLayout = Runtime::MemoryLayouts::RowLayout::create(
        schema, bufferManager->getBufferSize());
    auto testTupleBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer(memoryLayout, buf);

    for (auto curTuple = 0UL; curTuple < testTupleBuffer.getNumberOfTuples();
         ++curTuple) {
      if (testTupleBuffer[curTuple][timeStampFieldName].read<uint64_t>() >
              lastTimeStamp ||
      ...
      memcpy(curBuffer.getBuffer() +
                 schema->getSchemaSizeInBytes() * numberOfTuplesInBuffer,
...
std::vector<Runtime::TupleBuffer>
sortBuffersInTupleBuffer(std::vector<Runtime::TupleBuffer> &buffersToSort,
                         SchemaPtr schema, const std::string &sortFieldName,
                         BufferManagerPtr bufferManager) {
  ...
  for (auto bufRead : buffersToSort) {
    ...
    auto memLayout = Runtime::MemoryLayouts::RowLayout::create(
        schema, bufferManager->getBufferSize());
    auto testTupleBuf =
        Runtime::MemoryLayouts::TestTupleBuffer(memLayout, bufRead);
    ...
    for (auto outer = 0UL; outer < bufRead.getNumberOfTuples(); ++outer) {
      ...
      for (auto inner = 0UL; inner < bufRead.getNumberOfTuples(); ++inner) {
        if (std::find(indexAlreadyInNewBuffer.begin(),
                      indexAlreadyInNewBuffer.end(),
                      inner) != indexAlreadyInNewBuffer.end()) {
        ...
        auto sortValueOld =
            testTupleBuf[smallestIndex][sortFieldName].read<uint64_t>();
      ...
      memcpy(bufRet.getBuffer() + posRet * schema->getSchemaSizeInBytes(),
             bufRead.getBuffer() +
                 smallestIndex * schema->getSchemaSizeInBytes(),
             schema->getSchemaSizeInBytes());
...
Runtime::TupleBuffer getBufferFromRecord(const Nautilus::Record &nautilusRecord,
                                         SchemaPtr schema,
                                         BufferManagerPtr bufferManager) {
  auto buffer = bufferManager->getBufferBlocking();
  auto *bufferPtr = (int8_t *)buffer.getBuffer();

  writeNautilusRecord(0, bufferPtr, nautilusRecord, std::move(schema),
                      bufferManager);
...
std::string printTupleBufferAsCSV(Runtime::TupleBuffer tbuffer,
                                  const SchemaPtr &schema) {
  std::stringstream ss;
  ...
  for (uint64_t i = 0; i < numberOfTuples; i++) {
    ...
    for (uint64_t j = 0; j < schema->getSize(); j++) {
      ...
      auto str = physicalType->convertRawToString(
...
[[maybe_unused]] std::vector<Runtime::TupleBuffer>
createBuffersFromCSVFile(const std::string &csvFile, const SchemaPtr &schema,
                         Runtime::BufferManagerPtr bufferManager,
                         uint64_t originId,
                         const std::string &timestampFieldname) {
  std::vector<Runtime::TupleBuffer> recordBuffers;
  NES_ASSERT2_FMT(std::filesystem::exists(std::filesystem::path(csvFile)),
                  "CSVFile " << csvFile << " does not exist!!!");
  ...
  // Do-while loop for checking, if we have another line to parse from the
  // inputFile
  const auto maxTuplesPerBuffer =
      bufferManager->getBufferSize() / schema->getSchemaSizeInBytes();
  auto it = beginIt;
  auto tupleCount = 0UL;
  ...
  do {
    std::string line = *it;
    auto testBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(
            tupleBuffer, schema);
    auto values =
        NES::Util::splitWithStringDelimiter<std::string>(line, delimiter);
    ...
    for (uint64_t j = 0; j < numberOfSchemaFields; j++) {
      ...
      writeFieldValueToTupleBuffer(values[j], j, testBuffer, schema, tupleCount,
                                   bufferManager);
    }
    if (schema->contains(timestampFieldname)) {
      watermarkTS =
          std::max(watermarkTS,
                   testBuffer[tupleCount][timestampFieldname].read<uint64_t>());
    }
    ++tupleCount;
    ...
    if (tupleCount >= maxTuplesPerBuffer) {
      ...
      NES_DEBUG("watermarkTS {} sequenceNumber {} originId {}", watermarkTS,
                sequenceNumber, originId);
...
void writeFieldValueToTupleBuffer(
    std::string inputString, uint64_t schemaFieldIndex,
    Runtime::MemoryLayouts::TestTupleBuffer &tupleBuffer,
    const SchemaPtr &schema, uint64_t tupleCount,
  ...
  try {
    if (physicalType->isBasicType()) {
      auto basicPhysicalType =
          std::dynamic_pointer_cast<BasicPhysicalType>(physicalType);
      ...
      case NES::BasicPhysicalType::NativeType::CHAR: {
        ...
        if (inputString.size() > 1) {
          NES_FATAL_ERROR("SourceFormatIterator::mqttMessageToNESBuffer: "
                          "Received non char Value for CHAR Field {}",
                          inputString.c_str());
          throw std::invalid_argument("Value " + inputString +
                                      " is not a char");
        }
      ...
      case NES::BasicPhysicalType::NativeType::TEXT: {
        NES_TRACE("Parser::writeFieldValueToTupleBuffer(): trying to write the "
                  "variable length input string: {}"
                  "to tuple buffer",
                  inputString);
        tupleBuffer[tupleCount].writeVarSized(schemaFieldIndex, inputString,
                                              bufferManager.get());
      ...
      case NES::BasicPhysicalType::NativeType::BOOLEAN: {
        ...
        bool value = !strcasecmp(inputString.c_str(), "true") ||
                     !strcasecmp(inputString.c_str(), "1");
        if (!value) {
          if (strcasecmp(inputString.c_str(), "false") &&
              strcasecmp(inputString.c_str(), "0")) {
            NES_FATAL_ERROR("Parser::writeFieldValueToTupleBuffer: Received "
      ...
      case NES::BasicPhysicalType::NativeType::UNDEFINED:
        NES_FATAL_ERROR(
            "Parser::writeFieldValueToTupleBuffer: Field Type UNDEFINED");
      }
      ...
      // remove quotation marks from start and end of value (ASSUMES
      // QUOTATIONMARKS AROUND STRINGS) improve behavior with json library
      strcpy(value, inputString.c_str());
    }
  } catch (const std::exception &e) {
    NES_ERROR(
        "Failed to convert inputString to desired NES data type. Error: {}",
        e.what());
  }
}

void findAndReplaceAll(std::string &data, const std::string &toSearch,
...
std::vector<PhysicalTypePtr> getPhysicalTypes(SchemaPtr schema) {
  ...
  for (const auto &field : schema->fields) {
    auto physicalField =
        defaultPhysicalTypeFactory.getPhysicalType(field->getDataType());
...
bool checkIfBuffersAreEqual(Runtime::TupleBuffer buffer1,
                            Runtime::TupleBuffer buffer2,
                            uint64_t schemaSizeInByte) {
  NES_DEBUG("Checking if the buffers are equal, so if they contain the same "
            "tuples...");
  if (buffer1.getNumberOfTuples() != buffer2.getNumberOfTuples()) {
    NES_DEBUG("Buffers do not contain the same tuples, as they do not have the "
              "same number of tuples");
    return false;
  }

  std::set<uint64_t> sameTupleIndices;
  for (auto idxBuffer1 = 0UL; idxBuffer1 < buffer1.getNumberOfTuples();
       ++idxBuffer1) {
    bool idxFoundInBuffer2 = false;
    for (auto idxBuffer2 = 0UL; idxBuffer2 < buffer2.getNumberOfTuples();
         ++idxBuffer2) {
      if (sameTupleIndices.contains(idxBuffer2)) {
        continue;
      }

      auto startPosBuffer1 =
          buffer1.getBuffer() + schemaSizeInByte * idxBuffer1;
      auto startPosBuffer2 =
          buffer2.getBuffer() + schemaSizeInByte * idxBuffer2;
      auto equalTuple = (std::memcmp(startPosBuffer1, startPosBuffer2,
                                     schemaSizeInByte) == 0);
      if (equalTuple) {
    ...
    if (!idxFoundInBuffer2) {
      NES_DEBUG("Buffers do not contain the same tuples, as tuple could not be "
                "found in both buffers for idx: {}",
                idxBuffer1);
      return false;
    }
  }
