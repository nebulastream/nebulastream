...
void updateTestCountMinStatistic(
  ...
  for (auto tuple : testTupleBuffer) {
    ...
    if (allCountMinStatistics.empty()) {
      ...
      NES_DEBUG(
          "Created and inserted new countMinStatistic = {} for statisticHash = "
          "{}",
...
std::vector<Runtime::TupleBuffer> mergeBuffersSameWindow(
    std::vector<Runtime::TupleBuffer>& buffers, SchemaPtr schema,
...
std::vector<Runtime::TupleBuffer> sortBuffersInTupleBuffer(
    std::vector<Runtime::TupleBuffer>& buffersToSort, SchemaPtr schema,
...
[[maybe_unused]] std::vector<Runtime::TupleBuffer> createBuffersFromCSVFile(
    const std::string& csvFile, const SchemaPtr& schema,
    Runtime::BufferManagerPtr bufferManager, uint64_t originId,
    const std::string& timestampFieldname) {
      ...
      switch (basicPhysicalType->nativeType) {
        ...
        case NES::BasicPhysicalType::NativeType::CHAR: {
          ...
          if (inputString.size() > 1) {
            NES_FATAL_ERROR(
                "SourceFormatIterator::mqttMessageToNESBuffer: Received non "
                "char Value for CHAR Field {}",
                inputString.c_str());
        ...
        case NES::BasicPhysicalType::NativeType::TEXT: {
          NES_TRACE(
              "Parser::writeFieldValueToTupleBuffer(): trying to write the "
        ...
        case NES::BasicPhysicalType::NativeType::BOOLEAN: {
          ...
          if (!value) {
            if (strcasecmp(inputString.c_str(), "false") &&
              ...
              NES_FATAL_ERROR(
                  "Parser::writeFieldValueToTupleBuffer: Received non boolean "
                  "value for BOOLEAN field: {}",
                  inputString.c_str());
              throw std::invalid_argument("Value " + inputString +
                                          " is not a boolean");
            }
...
bool checkIfBuffersAreEqual(Runtime::TupleBuffer buffer1,
  ...
  NES_DEBUG(
      "Checking if the buffers are equal, so if they contain the same "
      "tuples...");
  if (buffer1.getNumberOfTuples() != buffer2.getNumberOfTuples()) {
    NES_DEBUG(
        "Buffers do not contain the same tuples, as they do not have the same "
        "number of tuples");
    return false;
  }
  ...
  for (auto idxBuffer1 = 0UL; idxBuffer1 < buffer1.getNumberOfTuples();
    ...
    if (!idxFoundInBuffer2) {
      NES_DEBUG(
          "Buffers do not contain the same tuples, as tuple could not be found "
          "in both buffers for idx: {}",
          idxBuffer1);
      return false;
    }
  }
