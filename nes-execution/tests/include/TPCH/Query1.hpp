...
public:
  static PipelinePlan getPipelinePlan(
     ...
     *  1998-09-02
     */
    auto const_1998_09_02 =
        std::make_shared<ConstantInt32ValueExpression>(19980831);
    auto readShipdate = std::make_shared<ReadFieldExpression>("l_shipdate");
    auto lessThanExpression1 =
        std::make_shared<LessThanExpression>(readShipdate, const_1998_09_02);
 ...
 * group by
    ...
    count(*)
 */
    auto l_returnflagField =
        std::make_shared<ReadFieldExpression>("l_returnflag");
    auto l_linestatusFiled =
        std::make_shared<ReadFieldExpression>("l_linestatus");
    ...
    auto sumAggFunction1 =
        std::make_shared<Aggregation::SumAggregationFunction>(
            integerType, integerType, l_quantityField, "sum_qty");
    ...
    auto sumAggFunction2 =
        std::make_shared<Aggregation::SumAggregationFunction>(
            floatType, floatType, l_extendedpriceField, "sum_base_price");
    ...
    auto disc_priceExpression =
        std::make_shared<WriteFieldExpression>("disc_price", mulExpression);
    ...
    auto sumAggFunction3 =
        std::make_shared<Aggregation::SumAggregationFunction>(
            floatType, floatType, disc_price, "sum_disc_price");
    ...
    auto mulExpression2 =
        std::make_shared<AddExpression>(disc_price, addExpression);
    auto sumAggFunction4 =
        std::make_shared<Aggregation::SumAggregationFunction>(
            floatType, floatType, mulExpression2, "sum_charge");

    //   count(*)
    auto countAggFunction5 =
        std::make_shared<Aggregation::CountAggregationFunction>(
            uintegerType, uintegerType, Expressions::ExpressionPtr(),
            "count_order");

    std::vector<Expressions::ExpressionPtr> keyFields = {l_returnflagField,
                                                         l_linestatusFiled};
    std::vector<std::shared_ptr<Aggregation::AggregationFunction>>
        aggregationFunctions = {sumAggFunction1, sumAggFunction2,
                                sumAggFunction3, sumAggFunction4,
                                countAggFunction5};
