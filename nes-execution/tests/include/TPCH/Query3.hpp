...
public:
  ...
  static Runtime::Execution::OperatorHandlerPtr createPipeline1(
      PipelinePlan &plan,
      std::unordered_map<TPCHTable, std::unique_ptr<NES::Runtime::Table>>
          &tables) {
    ...
    std::vector<Nautilus::Record::RecordFieldIdentifier> customersProjection = {
        "c_mksegment", "c_custkey"};
    auto customersScan = std::make_shared<Operators::Scan>(
    ...
    auto readC_mktsegment =
        std::make_shared<ReadFieldExpression>("c_mksegment");
    auto equalsExpression =
        std::make_shared<EqualsExpression>(readC_mktsegment, BUILDING);
    ...
    auto joinOp = std::make_shared<Operators::BatchJoinBuild>(
        0 /*handler index*/, std::vector<Expressions::ExpressionPtr>{readC_key},
    ...
    auto customerJoinBuildPipeline =
        std::make_shared<PhysicalOperatorPipeline>();
    ...
    auto pipeline1Context =
        std::make_shared<MockedPipelineExecutionContext>(joinHandler);
  ...
  static std::shared_ptr<BatchJoinHandler> createPipeline2(
    ...
    /**
     * Pipeline 2 with scan orders -> selection -> JoinPrope with customers from
     * pipeline 1
     */
    ...
    auto orderScan = std::make_shared<Operators::Scan>(
        std::move(ordersMemoryProviderPtr), ordersProjection);

    //  o_orderdate < date '1995-03-15'
    auto const_1995_03_15 =
        std::make_shared<ConstantInt32ValueExpression>(19950315);
    auto readO_orderdate = std::make_shared<ReadFieldExpression>("o_orderdate");
    auto orderDateSelection =
        std::make_shared<Selection>(std::make_shared<LessThanExpression>(
            readO_orderdate, const_1995_03_15));
    orderScan->setChild(orderDateSelection);

    // join probe with customers
    std::vector<IR::Types::StampPtr> keyStamps = {
    ...
    std::vector<Record::RecordFieldIdentifier> joinProbeResults = {
    ...
    std::vector<ExpressionPtr> order_customersJoinBuildKeys = {
    ...
    auto order_customersJoinBuild = std::make_shared<Operators::BatchJoinBuild>(
        1 /*handler index*/, order_customersJoinBuildKeys,
        std::vector<PhysicalTypePtr>{integerType},
    ...
    std::vector<Execution::OperatorHandlerPtr> handlers = {joinHandler,
                                                           joinHandler2};
  ...
  static void createPipeline3(
    ...
    /**
     * Pipeline 3 with scan lineitem -> selection -> JoinProbe with
     * order_customers from pipeline 2 -> aggregation
     */
    ...
    auto lineitemsScan = std::make_shared<Operators::Scan>(
        std::move(lineitemsMP), lineItemProjection);

    //   date '1995-03-15' < l_shipdate
    auto readL_shipdate = std::make_shared<ReadFieldExpression>("l_shipdate");
    auto const_1995_03_15 =
        std::make_shared<ConstantInt32ValueExpression>(19950315);
    auto shipDateSelection = std::make_shared<Selection>(
        std::make_shared<LessThanExpression>(const_1995_03_15, readL_shipdate));
    ...
    auto subExpression =
        std::make_shared<SubExpression>(oneConst, l_discountField);
    auto revenueExpression =
        std::make_shared<MulExpression>(l_extendedpriceField, subExpression);
    ...
    auto lineitems_ordersJoinBuild =
        std::make_shared<PhysicalOperatorPipeline>();
    lineitems_ordersJoinBuild->setRootOperator(lineitemsScan);
    auto aggHandler =
        std::make_shared<Operators::BatchKeyedAggregationHandler>();
    std::vector<Execution::OperatorHandlerPtr> handlers2 = {joinHandler,
                                                            aggHandler};
    auto pipeline3Context =
        std::make_shared<MockedPipelineExecutionContext>(handlers2);
