...
const std::unordered_map<TPCHTable, std::underlying_type_t<TPCHTable>>
    tpch_table_to_dbgen_id = {
        {TPCHTable::Part, PART},     {TPCHTable::PartSupp, PSUPP},
        {TPCHTable::Supplier, SUPP}, {TPCHTable::Customer, CUST},
        {TPCHTable::Orders, ORDER},  {TPCHTable::LineItem, LINE},
        {TPCHTable::Nation, NATION}, {TPCHTable::Region, REGION}};

template <typename DSSType, typename MKRetType, typename... Args>
DSSType call_dbgen_mk(size_t idx,
                      MKRetType (*mk_fn)(DSS_HUGE, DSSType *val, Args...),
                      TPCHTable table, Args... args) {
  /**
   * Preserve calling scheme (row_start(); mk...(); row_stop(); as in dbgen's
   * gen_tbl())
   */
...
public:
  const std::unordered_map<TPCHTable, std::string> tpch_table_names = {
      {TPCHTable::Part, "part"},         {TPCHTable::PartSupp, "partsupp"},
      {TPCHTable::Supplier, "supplier"}, {TPCHTable::Customer, "customer"},
      {TPCHTable::Orders, "orders"},     {TPCHTable::LineItem, "lineitem"},
      {TPCHTable::Nation, "nation"},     {TPCHTable::Region, "region"}};

  const SchemaPtr customersSchema =
      Schema::create()
  ...
  const SchemaPtr lineItemSchema =
      Schema::create()
  ...
  const SchemaPtr partsuppSchema =
      Schema::create()
  ...
  const SchemaPtr supplierSchema =
      Schema::create()
  ...
  std::unordered_map<TPCHTable, NES::Runtime::MemoryLayouts::MemoryLayoutPtr>
      layouts;
  TPCHTableGenerator(const Runtime::BufferManagerPtr &bufferManager,
                     TPCH_Scale_Factor scale_factor)
      : bufferManager(bufferManager), scaleFactor(scale_factor) {
    for (auto &[table, schema] : tableSchemas) {
      layouts[table] = Runtime::MemoryLayouts::ColumnLayout::create(
  ...
  auto getLocalScaleFactor() {
    switch (scaleFactor) {
    case TPCH_Scale_Factor::F1:
      return 1.0f;
    case TPCH_Scale_Factor::F0_1:
      return 0.1f;
    case TPCH_Scale_Factor::F0_01:
      return 0.01f;
    }
  }
  ...
  auto generate() {
    ...
    NES_ASSERT(_scale_factor < 1.0f ||
                   std::round(_scale_factor) == _scale_factor,
               "Due to tpch_dbgen limitations, only scale factors less than "
               "one can have a fractional part.");

    const auto cache_directory =
        fmt::format("tpch_cached_tables/sf-{}-c-{}", _scale_factor,
                    bufferManager->getBufferSize()); // NOLINT
    if (std::filesystem::is_directory(cache_directory)) {
      return loadTablesFromPath(cache_directory);
    }

    // Init tpch_dbgen - it is important this is done before any data structures
    // from tpch_dbgen are read.
    dbgen_reset_seeds();
    dbgen_init_scale_factor(_scale_factor);
    using ChunkOffset = uint64_t;
    const auto customer_count =
        static_cast<ChunkOffset>(tdefs[CUST].base * scale);
    const auto order_count =
        static_cast<ChunkOffset>(tdefs[ORDER].base * scale);
    const auto part_count = static_cast<ChunkOffset>(tdefs[PART].base * scale);
    const auto supplier_count =
        static_cast<ChunkOffset>(tdefs[SUPP].base * scale);
    ...
    Runtime::TableBuilder customerBuilder(bufferManager,
                                          layouts[TPCHTable::Customer]);
    Runtime::TableBuilder lineitemBuilder(bufferManager,
                                          layouts[TPCHTable::LineItem]);
    Runtime::TableBuilder ordersBuilder(bufferManager,
                                        layouts[TPCHTable::Orders]);
    Runtime::TableBuilder partBuilder(bufferManager, layouts[TPCHTable::Part]);
    Runtime::TableBuilder partsuppBuilder(bufferManager,
                                          layouts[TPCHTable::PartSupp]);
    Runtime::TableBuilder supplierBuilder(bufferManager,
                                          layouts[TPCHTable::Supplier]);
    Runtime::TableBuilder nationBuilder(bufferManager,
                                        layouts[TPCHTable::Nation]);
    Runtime::TableBuilder regionBuilder(bufferManager,
                                        layouts[TPCHTable::Region]);
    ...
    for (size_t row_idx = 0; row_idx < customer_count; row_idx++) {
      auto customer =
          call_dbgen_mk<customer_t>(row_idx + 1, mk_cust, TPCHTable::Customer);
      // TODO fix mktsegment
      int32_t mktsegment =
          std::strncmp(customer.mktsegment, "BUILDING", 8) == 0;
    ...
    for (size_t order_idx = 0; order_idx < order_count; ++order_idx) {
      const auto order = call_dbgen_mk<order_t>(order_idx + 1, mk_order,
                                                TPCHTable::Orders, 0l);
      ordersBuilder.append(
          std::make_tuple((int32_t)order.okey, (int32_t)order.custkey,
                          /*write as one char*/ (int8_t)order.orderstatus,
                          convert_money(order.totalprice), getDate(order.odate),
                          // order.opriority,
                          // order.clerk,
      ...
      for (auto line_idx = 0; line_idx < order.lines; ++line_idx) {
        ...
        lineitemBuilder.append(std::make_tuple(
            (int32_t)lineitem.okey, (int32_t)lineitem.partkey,
            (int32_t)lineitem.suppkey, (int32_t)lineitem.lcnt,
            (int32_t)lineitem.quantity, convert_money(lineitem.eprice),
            convert_money(lineitem.discount), convert_money(lineitem.tax),
            /*write as one char*/ (int8_t)lineitem.rflag[0],
            /*write as one char*/ (int8_t)lineitem.lstatus[0],
            getDate(lineitem.sdate), getDate(lineitem.cdate),
    ...
    for (size_t part_idx = 0; part_idx < part_count; ++part_idx) {
      const auto part =
          call_dbgen_mk<part_t>(part_idx + 1, mk_part, TPCHTable::Part);
      ...
      // Some scale factors (e.g., 0.05) are not supported by tpch-dbgen as they
      // produce non-unique partkey/suppkey combinations. The reason is probably
      // somewhere in the magic in PART_SUPP_BRIDGE. As the partkey is
      ...
      for (const auto &partsupp : part.s) {
        {
          ...
          if (partsupp.partkey != last_partkey) {
            NES_ASSERT(partsupp.partkey > last_partkey,
                       "Expected partkey to be generated in ascending order");
            last_partkey = partsupp.partkey;
            suppkeys.clear();
          }
          NES_ASSERT(std::find(suppkeys.begin(), suppkeys.end(),
                               partsupp.suppkey) == suppkeys.end(),
                     "Scale factor unsupported by tpch-dbgen. Consider "
                     "choosing a \"round\" number.");
          suppkeys.emplace_back(partsupp.suppkey);
        }

        partsuppBuilder.append(std::make_tuple(
            (int32_t)partsupp.partkey, (int32_t)partsupp.suppkey,
            (int32_t)partsupp.qty, convert_money(partsupp.scost)
            // partsupp.comment
            ));
      }
    }
     ...
     * SUPPLIER
     */
    for (size_t supplier_idx = 0; supplier_idx < supplier_count;
         ++supplier_idx) {
      const auto supplier = call_dbgen_mk<supplier_t>(supplier_idx + 1, mk_supp,
                                                      TPCHTable::Supplier);
    ...
    for (size_t nation_idx = 0; nation_idx < nation_count; ++nation_idx) {
      const auto nation =
          call_dbgen_mk<code_t>(nation_idx + 1, mk_nation, TPCHTable::Nation);
    ...
    for (size_t region_idx = 0; region_idx < region_count; ++region_idx) {
      const auto region =
          call_dbgen_mk<code_t>(region_idx + 1, mk_region, TPCHTable::Region);
    ...
    /**
     * Clean up dbgen every time we finish table generation to avoid memory
     * leaks in dbgen
     */
    dbgen_cleanup();
  ...
  std::unordered_map<TPCHTable, std::unique_ptr<NES::Runtime::Table>>
  loadTablesFromPath(const std::string &cacheDirectory) {
    std::unordered_map<TPCHTable, std::unique_ptr<NES::Runtime::Table>> tables;
    for (auto table : magic_enum::enum_values<TPCHTable>()) {
      ...
      tables[table] =
          Runtime::Table::load(tableDir, bufferManager, layouts[table]);
    }
    return tables;
  }
