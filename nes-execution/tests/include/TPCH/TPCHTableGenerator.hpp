...
enum class TPCH_Scale_Factor : uint8_t
{
    F1,
    F0_1,
    F0_01
...
public:
    ...
    TPCHTableGenerator(const std::shared_ptr<Runtime::AbstractBufferProvider>& bufferManager, TPCH_Scale_Factor scale_factor)
            ...
            {
                {
                    ...
                    NES_ASSERT(
                        std::find(suppkeys.begin(), suppkeys.end(), partsupp.suppkey) == suppkeys.end(),
