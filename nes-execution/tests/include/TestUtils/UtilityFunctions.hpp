...
/**
 * @brief Creates data for the schema <UINT64,UINT64>. The
 * fieldToBuildCountMinOver is rnd() while timestampField gets monotonic
...
std::vector<TupleBuffer> createDataForOneFieldAndTimeStamp(
    ...
    const std::string& timestampFieldName, const bool isIngestionTime = false);

/**
 * @brief Creates a CountMinSketch (if none exists) in the statisticStore and
 * updates the counter at <row,col>
...
/**
 * @brief Creates a HyperLogLogSketch (if none exists) in the statisticStore and
 * updates the sketch
 * @param testTupleBuffer
 * @param statisticStore
 * @param metricHash
...
/**
 * @brief Merges a vector of TupleBuffers into one TupleBuffer. If the buffers
 * in the vector do not fit into one TupleBuffer, the buffers that do not fit
 * will be discarded.
...
/**
 * @brief this function iterates through all buffers and merges all buffers into
 * a newly created vector so that the new buffers contain as much tuples as
 * possible. Additionally, there are only tuples in a buffer that belong to the
 * same window
 * @param buffers
 * @param schema
 * @param timeStampFieldName
...
/**
 * @brief Iterates through buffersToSort and sorts each buffer ascending to
 * sortFieldName
 * @param buffersToSort
 * @param schema
 * @param sortFieldName
...
[[maybe_unused]] std::vector<Runtime::TupleBuffer> createBuffersFromCSVFile(
    ...
    Runtime::BufferManagerPtr bufferManager, uint64_t originId = 0,
 ...
 * @param tupleBuffer: the TupleBuffer to which the value is written containing
 * the currently chosen memory layout
...
PhysicalTypePtr getPhysicalTypePtr() {
  ...
  if (typeid(int32_t) == typeid(T)) {
    type =
        physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt32());
  } else if (typeid(uint32_t) == typeid(T)) {
    type = physicalDataTypeFactory.getPhysicalType(
    ...
    type =
        physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt16());
  } else if (typeid(uint16_t) == typeid(T)) {
    type = physicalDataTypeFactory.getPhysicalType(
        DataTypeFactory::createUInt16());
  } else if (typeid(int8_t) == typeid(T)) {
    type =
        physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt8());
  } else if (typeid(uint8_t) == typeid(T)) {
    type =
        physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createUInt8());
