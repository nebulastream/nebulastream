...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ReplacingRegexTest.log",
  ...
  void SetUp() override {
    ...
    wc = std::make_shared<Runtime::WorkerContext>(INITIAL<WorkerThreadId>, bm,
                                                  1024);
    NES_DEBUG("Setup ReplacingRegexTest test case.")
  }
  /* Will be called after all tests in this class are finished. */
  static void TearDownTestCase() {
    NES_INFO("Tear down ReplacingRegexTest test class.");
  }
  std::shared_ptr<Runtime::BufferManager> bm;
  std::shared_ptr<Runtime::WorkerContext> wc;
};

/** @brief The ReplacingRegex Class provides functionality that replaces the
 * first occurrence of regex with the replacement */
...
TEST_F(ReplacingRegexTest, evaluateReplacingRegex3) {
  ...
  // Regex string correction
  {
    ...
    auto resultValue = expression.eval(l, m, r);
    EXPECT_EQ(resultValue,
              Value<Text>("there is a sub-sequence in the string"));
  }
}
