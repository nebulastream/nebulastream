...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NegateExpressionTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down NegateExpressionTest test class.");
  }
};
...
TEST_F(NegateExpressionTest, baseBoolCases) {
  ...
  {
    auto resultValue =
        expression.eval(Value<Boolean>(true), Value<Boolean>(true));
  ...
  {
    auto resultValue =
        expression.eval(Value<Boolean>(false), Value<Boolean>(true));
    ASSERT_EQ(resultValue, (bool)true);
    ASSERT_TRUE(resultValue->getTypeIdentifier()->isType<Boolean>());
  }
  {
    auto resultValue =
        expression.eval(Value<Boolean>(false), Value<Boolean>(false));
