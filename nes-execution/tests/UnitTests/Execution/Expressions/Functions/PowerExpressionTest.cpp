...
class PowerExpressionTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("PowerExpressionTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down PowerExpressionTest test class.");
  }
};
...
TEST_F(PowerExpressionTest, evaluatePowerExpressionInteger) {
  ...
  {
    auto resultValue =
        expression.eval(Value<Int32>(2_s32), Value<Int32>(4_s32));
    ASSERT_EQ(resultValue, (float)16);
    ASSERT_TRUE(resultValue->getTypeIdentifier()->isType<Double>());
  }
  // Int64
  {
    auto resultValue =
        expression.eval(Value<Int64>(2_s64), Value<Int64>(4_s64));
    ASSERT_EQ(resultValue, (float)16);
    ASSERT_TRUE(resultValue->getTypeIdentifier()->isType<Double>());
  }
}
...
TEST_F(PowerExpressionTest, evaluatePowerExpressionFloat) {
  ...
  {
    auto resultValue =
        expression.eval(Value<Float>((float)4), Value<Float>((float)0));
  ...
  {
    auto resultValue =
        expression.eval(Value<Float>((float)2), Value<Float>((float)4));
    ASSERT_EQ(resultValue, (float)16);
    ASSERT_TRUE(resultValue->getTypeIdentifier()->isType<Double>());
  }
  // Double
  {
    auto resultValue =
        expression.eval(Value<Double>((double)2), Value<Double>((double)4));
    ASSERT_EQ(resultValue, (float)16);
    ASSERT_TRUE(resultValue->getTypeIdentifier()->isType<Double>());
  }
  // Double
  {
    auto resultValue =
        expression.eval(Value<Double>((double)4), Value<Double>((double)0));
