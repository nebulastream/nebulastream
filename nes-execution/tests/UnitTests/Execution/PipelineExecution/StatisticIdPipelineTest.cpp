...
class StatisticIdPipelineTest : public Testing::BaseUnitTest,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("StatisticIdPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down StatisticIdPipelineTest test class.");
  }

  /**
   * @brief This method creates numBuffers buffers (default = 100) with some
   * arbitrary data
   */
  std::vector<TupleBuffer> createData(uint64_t numberOfBuffers = 20) {
    ...
    for (uint64_t bufCnt = 0; bufCnt < numberOfBuffers; ++bufCnt) {
      ...
      // As we do not have a physical source, we set the statisticId to the id
      // of the scan operator. In the actual system, this would be set to the id
      // of the predecessor pipeline
...
/**
 * @brief Tests if the statistic id is set correctly for the emitted tuple
 * buffer of a pipeline consisting of a scan --> filter --> emit. The statistic
 * id of the filter operator should be set
 */
TEST_P(StatisticIdPipelineTest, testScanFilterEmitPipeline) {
  ...
  bool statisticIdCheck = std::accumulate(
      pipelineContext.buffers.begin(), pipelineContext.buffers.end(), true,
...
/**
 * @brief Tests if the statistic id is set correctly for the emitted tuple
 * buffer of a pipeline consisting of a scan --> filter --> map --> emit. The
 * statistic id of the filter operator should be set
 */
...
INSTANTIATE_TEST_CASE_P(
    testStatisticIdCompilation, StatisticIdPipelineTest,
