...
class MapJavaUDFPipelineTest : public testing::Test,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MapJavaUDFPipelineTest.log",
  ...
  /**
   * Initializes a pipeline with a Scan of the input tuples, a MapJavaUDF
   ...
   */
  auto
  initMapHandler(const Catalogs::UDF::JavaUdfDescriptorPtr javaUDFDescriptor) {
    jni::JavaSerializedInstance instance;
    return std::make_shared<Operators::JavaUDFOperatorHandler>(
        ...
        javaUDFDescriptor->getOutputSchema(), std::nullopt);
  }
...
/**
 * @brief Test a pipeline containing a scan, a java map with integers, and a
 * emit operator
 */
  ...
  auto handler = initMapHandler(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .setByteCodeList({{"stream.nebula.MapFunction", {}},
                            {"stream.nebula.IntegerMapFunction", {}}})
...
/**
 * @brief Test a pipeline containing a scan, a java map with shorts, and a emit
 * operator
 */
TEST_P(MapJavaUDFPipelineTest, scanMapEmitPipelineShortMap) {
  ...
  auto handler = initMapHandler(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .setByteCodeList({{"stream.nebula.MapFunction", {}},
                            {"stream.nebula.ShortMapFunction", {}}})
          .setInputSchema(schema)
          .setOutputSchema(schema)
          .loadByteCodeFrom(JAVA_UDF_TEST_DATA)
          .build());
...
/**
 * @brief Test a pipeline containing a scan, a java map with byte, and a emit
 * operator
 */
...
/**
 * @brief Test a pipeline containing a scan, a java map with long, and a emit
 * operator
 */
...
TEST_P(MapJavaUDFPipelineTest, scanMapEmitPipelineDoubleMap) {
  ...
  auto handler = initMapHandler(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .setByteCodeList({{"stream.nebula.MapFunction", {}},
                            {"stream.nebula.DoubleMapFunction", {}}})
          .setInputSchema(schema)
          .setOutputSchema(schema)
          .loadByteCodeFrom(JAVA_UDF_TEST_DATA)
          .build());
...
/**
 * @brief Test a pipeline containing a scan, a java map with booleans, and a
 * emit operator
 */
TEST_P(MapJavaUDFPipelineTest, scanMapEmitPipelineBooleanMap) {
  ...
  auto handler = initMapHandler(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .setByteCodeList({{"stream.nebula.MapFunction", {}},
                            {"stream.nebula.BooleanMapFunction", {}}})
          .setInputSchema(schema)
          .setOutputSchema(schema)
          .loadByteCodeFrom(JAVA_UDF_TEST_DATA)
          .build());
...
/**
 * @brief Test a pipeline containing a scan, a java map with strings, and a emit
 * operator
 */
TEST_P(MapJavaUDFPipelineTest, scanMapEmitPipelineStringMap) {
  ...
  auto handler = initMapHandler(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .setByteCodeList({{"stream.nebula.MapFunction", {}},
                            {"stream.nebula.StringMapFunction", {}}})
          .setInputSchema(schema)
          .setOutputSchema(schema)
          .loadByteCodeFrom(JAVA_UDF_TEST_DATA)
          .build());
  ...
  auto resulttestBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(memoryLayout, resultBuffer);
  for (uint64_t i = 0; i < 10; i++) {
    ASSERT_EQ(resulttestBuffer[i].readVarSized("stringVariable"),
              "Appended String:X");
  }
}

/**
 * @brief Test a pipeline containing a scan, a java map with multiple types, and
 * a emit operator
 */
TEST_P(MapJavaUDFPipelineTest, scanMapEmitPipelineComplexMap) {
  ...
  for (uint64_t i = 0; i < 10; i++) {
    ...
    EXPECT_EQ(resulttestBuffer[i].readVarSized("stringVariable"),
              "XAppended String:");
  }
}

INSTANTIATE_TEST_CASE_P(
    testMapJavaUDF, MapJavaUDFPipelineTest,
