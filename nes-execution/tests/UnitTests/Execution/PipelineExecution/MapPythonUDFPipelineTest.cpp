...
class MapPythonUDFPipelineTest : public testing::Test,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MapPythonUDFPipelineTest.log",
  ...
  std::string testDataPath =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "PythonUDFTestData";
};

/**
 * Initializes a pipeline with a Scan of the input tuples, a MapPythonUDF
 * operator, and a emit of the processed tuples.
...
auto initPipelineOperator(SchemaPtr schema, auto memoryLayout) {
  auto mapOperator =
      std::make_shared<Operators::MapPythonUDF>(0, schema, schema);
 ...
 */
template <typename T>
auto initInputBuffer(std::string variableName, auto bufferManager,
                     auto memoryLayout) {
  auto buffer = bufferManager->getBufferBlocking();
  auto testBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(memoryLayout, buffer);
 ...
 */
auto initMapHandler(std::string function, std::string functionName,
                    SchemaPtr schema) {
  return std::make_shared<Operators::PythonUDFOperatorHandler>(
      function, functionName, schema, schema);
}
 ...
 */
template <typename T>
void checkBufferResult(std::string variableName, auto pipelineContext,
                       auto memoryLayout) {
...
/**
 * @brief Test a pipeline containing a scan, a python map with integers, and a
 * emit operator
 */
...
/**
 * @brief Test a pipeline containing a scan, a python map with shorts, and a
 * emit operator
 */
TEST_P(MapPythonUDFPipelineTest, scanMapEmitPipelineShortMap) {
  auto variableName = "shortVariable";
  auto schema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                    ->addField(variableName, BasicType::INT16);
  auto memoryLayout =
      Runtime::MemoryLayouts::RowLayout::create(schema, bm->getBufferSize());
...
/**
 * @brief Test a pipeline containing a scan, a python map with byte, and a emit
 * operator
 */
TEST_P(MapPythonUDFPipelineTest, scanMapEmitPipelineByteMap) {
  auto variableName = "byteVariable";
  auto schema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                    ->addField(variableName, BasicType::INT8);
  auto memoryLayout =
      Runtime::MemoryLayouts::RowLayout::create(schema, bm->getBufferSize());
...
/**
 * @brief Test a pipeline containing a scan, a python map with long, and a emit
 * operator
 */
...
/**
 * @brief Test a pipeline containing a scan, a python map with booleans, and a
 * emit operator
 */
TEST_P(MapPythonUDFPipelineTest, scanMapEmitPipelineBooleanMap) {
  auto variableName = "BooleanVariable";
  auto schema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                    ->addField(variableName, BasicType::BOOLEAN);
  auto memoryLayout =
      Runtime::MemoryLayouts::RowLayout::create(schema, bm->getBufferSize());
...
/**
 * @brief Test a pipeline containing a scan, a python map with strings, and a
 * emit operator
 *
 */
TEST_P(MapPythonUDFPipelineTest, DISABLED_scanMapEmitPipelineStringMap) {
  ...
  auto schema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                    ->addField(variableName, BasicType::TEXT);
  auto memoryLayout =
      Runtime::MemoryLayouts::RowLayout::create(schema, bm->getBufferSize());
  ...
  for (uint64_t i = 0; i < 10; i++) {
    ...
    *varLengthBuffer.getBuffer<uint32_t>() = value.size();
    std::strcpy(varLengthBuffer.getBuffer<char>() + sizeof(uint32_t),
                value.c_str());
...
/**
 * @brief Test a pipeline containing a scan, a python map with multiple types,
 * and a emit operator
 */
TEST_P(MapPythonUDFPipelineTest, scanMapEmitPipelineComplexMap) {
  ...
  for (uint64_t i = 0; i < 10; i++) {
    ...
    *varLengthBuffer.getBuffer<uint32_t>() = value.size();
    ...
    // testBuffer[i]["stringVariable"].write(strIndex); TODO #3980 enable once
    // string works
    testBuffer.setNumberOfTuples(i + 1);
  }
  ...
  std::string function = "def complex_test(byte_var, short_var, int_var, "
                         "long_var, float_var, double_var, boolean_var):"
                         ...
                         "\n\tboolean_var = False"
                         "\n\treturn byte_var, short_var, int_var, long_var, "
                         "float_var, double_var, False\n";
...
INSTANTIATE_TEST_CASE_P(
    testIfCompilation, MapPythonUDFPipelineTest,
    ::testing::Values("PipelineInterpreter", "PipelineCompiler"),
    [](const testing::TestParamInfo<MapPythonUDFPipelineTest::ParamType>
