...
class NonKeyedThresholdWindowPipelineTest
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NonKeyedThresholdWindowPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down NonKeyedThresholdWindowPipelineTest test class.");
  }
};

/**
 * @brief Test running a pipeline containing a threshold window with a Avg
 * aggregation
 */
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithSum) {
  ...
  auto greaterThanExpression =
      std::make_shared<Expressions::GreaterThanExpression>(readF1, one);
  ...
  auto sumAgg = std::make_shared<Aggregation::SumAggregationFunction>(
      integerPhysicalType, integerPhysicalType, readF2,
      aggregationResultFieldName);

  aggVector.emplace_back(sumAgg);
  auto thresholdWindowOperator =
      std::make_shared<Operators::NonKeyedThresholdWindow>(
  ...
  auto sumAggregationValue =
      std::make_unique<Aggregation::SumAggregationValue<int64_t>>();
  aggValues.emplace_back(std::move(sumAggregationValue));
  auto handler =
      std::make_shared<Operators::NonKeyedThresholdWindowOperatorHandler>(
          std::move(aggValues));
  ...
  auto resulttestBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(emitMemoryLayout, resultBuffer);
...
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithCount) {
  ...
  auto unsignedIntegerType =
      physicalTypeFactory.getPhysicalType(DataTypeFactory::createUInt64());

  auto countAgg = std::make_shared<Aggregation::CountAggregationFunction>(
      integerPhysicalType, unsignedIntegerType, readF2,
      aggregationResultFieldName);
  ...
  auto countAggregationValue =
      std::make_unique<Aggregation::CountAggregationValue<int64_t>>();
...
/**
 * @brief Test running a pipeline containing a threshold window with a Min
 * aggregation
 */
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithMin) {
  ...
  auto minAggregationValue =
      std::make_unique<Aggregation::MinAggregationValue<int64_t>>();
...
/**
 * @brief Test running a pipeline containing a threshold window with a Max
 * aggregation
 */
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithMax) {
  ...
  auto maxAggregationValue =
      std::make_unique<Aggregation::MaxAggregationValue<int64_t>>();
...
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithAvg) {
  ...
  auto avgAggregationValue =
      std::make_unique<Aggregation::AvgAggregationValue<int8_t>>();
...
// This test ensures that the aggregated field does not have to be an integer,
// which is the data type of count aggregation.
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithAvgFloat) {
  ...
  auto resulttestBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(emitMemoryLayout, resultBuffer);
  EXPECT_EQ(resulttestBuffer[0][aggregationResultFieldName].read<float>(),
            25.0);
}
...
TEST_P(NonKeyedThresholdWindowPipelineTest, thresholdWindowWithFloatPredicate) {
  ...
  auto greaterThanExpression =
      std::make_shared<Expressions::GreaterThanExpression>(readF1, half);
  auto aggregationResultFieldName = "test$Sum";
...
// TODO #3468: parameterize the aggregation function instead of repeating the
// similar test
INSTANTIATE_TEST_CASE_P(
    testIfCompilation, NonKeyedThresholdWindowPipelineTest,
    ::testing::Values("PipelineInterpreter", "PipelineCompiler",
                      "CPPPipelineCompiler"),
    [](const testing::TestParamInfo<
        NonKeyedThresholdWindowPipelineTest::ParamType> &info) {
      return info.param;
    });
} // namespace NES::Runtime::Execution
