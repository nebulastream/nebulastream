...
class KeyedTimeWindowPipelineTest : public testing::Test,
 ...
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedTimeWindowPipelineTest.log",
  ...
  void TearDown() override {
    std::cout << "Tear down GlobalTimeWindowPipelineTest test case."
              << std::endl;
  }

  /* Will be called after all tests in this class are finished. */
  static void TearDownTestCase() {
    std::cout << "Tear down GlobalTimeWindowPipelineTest test class."
              << std::endl;
  }
};
...
TEST_P(KeyedTimeWindowPipelineTest, windowWithSum) {
  ...
  auto sliceMergingAction = std::make_unique<Operators::KeyedWindowEmitAction>(
      aggregationFunctions, "start", "end", 8, 8, resultKeyFields, types,
      INVALID_ORIGIN_ID);
  auto sliceMerging = std::make_shared<Operators::KeyedSliceMerging>(
  ...
  auto preAggregationHandler =
      std::make_shared<Operators::KeyedSlicePreAggregationHandler>(10, 10,
                                                                   origins);

  auto sliceMergingExecutablePipeline =
      provider->create(sliceMergingPipeline, options);
  auto sliceMergingHandler =
      std::make_shared<Operators::KeyedSliceMergingHandler>();
  ...
  EXPECT_EQ(resulttestBuffer[0][aggregationResultFieldName].read<int64_t>(),
            50);
  EXPECT_EQ(resulttestBuffer[1][aggregationResultFieldName].read<int64_t>(),
            20);
  EXPECT_EQ(resulttestBuffer[2][aggregationResultFieldName].read<int64_t>(),
            30);
...
TEST_P(KeyedTimeWindowPipelineTest, multiKeyWindowWithSum) {
  ...
  auto slicePreAggregation =
      std::make_shared<Operators::KeyedSlicePreAggregation>(
          ...
          std::make_unique<Operators::EventTimeFunction>(
              readTsField, Windowing::TimeUnit::Milliseconds()),
          keyFields, keyTypes, aggregationFunctions,
          std::make_unique<Nautilus::Interface::MurMur3HashFunction>());
  scanOperator->setChild(slicePreAggregation);
  auto preAggPipeline = std::make_shared<PhysicalOperatorPipeline>();
  preAggPipeline->setRootOperator(scanOperator);
  ...
  auto sliceMergingAction = std::make_unique<Operators::KeyedWindowEmitAction>(
      aggregationFunctions, "start", "end", 16, 8, resultKeyFields, keyTypes,
      INVALID_ORIGIN_ID);
  auto sliceMerging = std::make_shared<Operators::KeyedSliceMerging>(
      0 /*handler index*/, aggregationFunctions, std::move(sliceMergingAction),
      keyTypes, 16, 8);
...
INSTANTIATE_TEST_CASE_P(
    testIfCompilation, KeyedTimeWindowPipelineTest,
    ::testing::Values("PipelineInterpreter", "PipelineCompiler",
                      "CPPPipelineCompiler"),
    [](const testing::TestParamInfo<KeyedTimeWindowPipelineTest::ParamType>&
