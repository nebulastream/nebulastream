...
class HashJoinMockedPipelineExecutionContext
...
class HashJoinPipelineTest : public Testing::BaseUnitTest,
 ...
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("HashJoinPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down HashJoinPipelineTest test class.");
  }
  ...
  bool checkIfHashJoinWorks(const std::string& fileNameBuffersLeft,
    ...
    auto joinProbe = std::make_shared<Operators::HJProbe>(
        handlerIndex, joinSchemaStruct, keyExpressions, windowMetaData,
        QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
        QueryCompilation::WindowingStrategy::SLICING);
    ...
    auto hashJoinOpHandler = Operators::HJOperatorHandlerSlicing::create(
        originIds, outputOriginId, windowSize, windowSlide, leftSchema,
        rightSchema, QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
    ...
    auto pipelineExecCtxLeft = HashJoinMockedPipelineExecutionContext(
        bufferManager, noWorkerThreads, hashJoinOpHandler,
        PipelineId(curPipelineId++));
    auto pipelineExecCtxRight = HashJoinMockedPipelineExecutionContext(
        bufferManager, noWorkerThreads, hashJoinOpHandler,
        PipelineId(curPipelineId++));
    auto pipelineExecCtxSink = HashJoinMockedPipelineExecutionContext(
        bufferManager, noWorkerThreads, hashJoinOpHandler,
        PipelineId(curPipelineId++));

    hashJoinOpHandler->start(
    ...
    hashJoinWorks = hashJoinWorks &&
                    (executablePipelineRight->setup(pipelineExecCtxRight) == 0);
    hashJoinWorks = hashJoinWorks &&
                    (executablePipelineSink->setup(pipelineExecCtxSink) == 0);
    ...
    hashJoinWorks = hashJoinWorks &&
                    (executablePipelineLeft->stop(pipelineExecCtxLeft) == 0);
    hashJoinWorks = hashJoinWorks &&
                    (executablePipelineRight->stop(pipelineExecCtxRight) == 0);
    hashJoinOpHandler->stop(
    ...
    hashJoinWorks =
        hashJoinWorks && (!pipelineExecCtxLeft.emittedBuffers.empty() ||
    ...
    hashJoinWorks = hashJoinWorks &&
                    (executablePipelineSink->stop(pipelineExecCtxSink) == 0);
    ...
    hashJoinWorks =
        hashJoinWorks && (resultBuffer.getNumberOfTuples() ==
                          expectedSinkBuffers[0].getNumberOfTuples());
    hashJoinWorks = hashJoinWorks && (Util::checkIfBuffersAreEqual(
...
TEST_P(HashJoinPipelineTest, simpleHashJoinPipeline) {
  ...
  const std::string fileNameBuffersLeft(std::string(TEST_DATA_DIRECTORY) +
                                        "window.csv");
  const std::string fileNameBuffersRight(std::string(TEST_DATA_DIRECTORY) +
                                         "window2.csv");
  const std::string fileNameBuffersSink(std::string(TEST_DATA_DIRECTORY) +
                                        "window_sink2.csv");

  ASSERT_TRUE(checkIfHashJoinWorks(
      fileNameBuffersLeft, fileNameBuffersRight, fileNameBuffersSink,
      windowSize, windowSize, leftSchema, rightSchema, joinSchema,
      joinFieldNameLeft, joinFieldNameRight, timeStampFieldLeft,
      timeStampFieldRight, windowStartFieldName, windowEndFieldName));
}

INSTANTIATE_TEST_CASE_P(
    testIfCompilation, HashJoinPipelineTest,
    ::testing::Values(
        "PipelineInterpreter",
