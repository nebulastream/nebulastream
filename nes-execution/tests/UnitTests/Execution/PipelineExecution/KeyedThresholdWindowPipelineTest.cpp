...
class KeyedThresholdWindowPipelineTest : public Testing::BaseUnitTest,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedThresholdWindowPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down KeyedThresholdWindowPipelineTest test class.");
  }
};
...
TEST_P(KeyedThresholdWindowPipelineTest, thresholdWindowWithSum) {
  ...
  auto thresholdWindowOperator =
      std::make_shared<Operators::KeyedThresholdWindow>(
          greaterThanExpression, 0,
          std::vector<Expressions::ExpressionPtr>{readF2}, readKey,
          keyFieldName,
  ...
  auto handler =
      std::make_shared<Operators::KeyedThresholdWindowOperatorHandler>();
...
/**
 * @brief Test running a pipeline containing a threshold window with a sum and a
 * max aggregation on rows having different keys.
 */
TEST_P(KeyedThresholdWindowPipelineTest,
       thresholdWindowWithSumAndMaxDifferentKeys) {
  auto scanSchema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT);
  scanSchema->addField("f1", BasicType::INT64);
  scanSchema->addField("f2", BasicType::INT64);
  scanSchema->addField("key", BasicType::UINT32);
  ...
  auto thresholdWindowOperator =
      std::make_shared<Operators::KeyedThresholdWindow>(
          ...
          keyFieldName,
          std::vector<Record::RecordFieldIdentifier>{
              sumAggregationResultFieldName, maxAggregationResultFieldName},
          aggVector, 0);
  scanOperator->setChild(thresholdWindowOperator);
  ...
  // the last tuple closes the window
  testBuffer[3]["f1"].write(
      +1_s64); // does not qualify, closes the threshold window for key 0
  testBuffer[3]["key"].write(0_u32);
  testBuffer[3]["f2"].write(+40_s64);
  ...
  // the last tuple closes the window
  testBuffer[7]["f1"].write(
      +1_s64); // does not qualify, closes the threshold window for key 1
  ...
  auto resulttestBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(emitMemoryLayout, resultBuffer);
  EXPECT_EQ(resulttestBuffer[0][sumAggregationResultFieldName].read<int64_t>(),
            50);
  EXPECT_EQ(resulttestBuffer[0][maxAggregationResultFieldName].read<int64_t>(),
            30);
  EXPECT_EQ(resulttestBuffer[1][sumAggregationResultFieldName].read<int64_t>(),
            500);
  EXPECT_EQ(resulttestBuffer[1][maxAggregationResultFieldName].read<int64_t>(),
            300);
}
...
INSTANTIATE_TEST_CASE_P(
    testIfCompilation, KeyedThresholdWindowPipelineTest,
    ::testing::Values("PipelineInterpreter", "PipelineCompiler",
                      "CPPPipelineCompiler"),
    [](const testing::TestParamInfo<KeyedThresholdWindowPipelineTest::ParamType>
