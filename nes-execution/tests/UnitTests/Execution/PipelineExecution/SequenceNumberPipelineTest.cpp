...
class SequenceNumberPipelineTest : public Testing::BaseUnitTest,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SequenceNumberPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down SequenceNumberPipelineTest test class.");
  }
};

/**
 * @brief This method creates four buffers and sets the f1 = 10 for all tuples
 * in the second and fourth buffer
 */
std::vector<TupleBuffer> createDataAllSeqNumbersEmitted(BufferManagerPtr bm,
                                                        SchemaPtr schema) {
  ...
  for (uint64_t bufCnt = 0; bufCnt < NUM_BUF; ++bufCnt) {
    ...
    auto testBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(buffer,
                                                                       schema);
...
/**
 * @brief Selection pipeline that execute a select operator and filters all
 * tuples that are not satisfying (f1 == 5) We send four buffers and create the
 * data so that no tuple satisfies the condition for the second, and fourth
 * buffer. Then we check if the PipelineExecutionContext has seen all sequence
 * numbers, as we have to pass on all sequence numbers
 */
TEST_P(SequenceNumberPipelineTest, testAllSequenceNumbersGetEmitted) {
  ...
  auto scanMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(memoryLayout);
  auto scanOperator =
      std::make_shared<Operators::Scan>(std::move(scanMemoryProviderPtr));
  ...
  auto equalsExpression =
      std::make_shared<Expressions::EqualsExpression>(readF1, readF2);
  auto selectionOperator =
      std::make_shared<Operators::Selection>(equalsExpression);
  ...
  ASSERT_THAT(seenSeqNumbers,
              ::testing::UnorderedElementsAreArray({1, 2, 3, 4}));
}
...
createDataFullWithConstantFieldValues(BufferManagerPtr bm, SchemaPtr schema) {
  std::vector<TupleBuffer> retBuffers;
  constexpr uint64_t NUM_BUF = 4;
...
/**
 * @brief Map pipeline that has creates an additional field to have a larger
 * output schema than input. With this, we check our implementation of the
 * chunks and see, if they have been assigned correctly.
 */
TEST_P(SequenceNumberPipelineTest, testMultipleSequenceNumbers) {
  ...
  auto memoryLayoutInput = Runtime::MemoryLayouts::RowLayout::create(
      inputSchema, bm->getBufferSize());
  auto memoryLayoutOutput = Runtime::MemoryLayouts::RowLayout::create(
      outputSchema, bm->getBufferSize());

  auto scanMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(memoryLayoutInput);
  auto scanOperator =
      std::make_shared<Operators::Scan>(std::move(scanMemoryProviderPtr));
  ...
  auto addExpression =
      std::make_shared<Expressions::AddExpression>(readF1, readF2);
  auto writeF3 =
      std::make_shared<Expressions::WriteFieldExpression>("f3", addExpression);
  auto mapOperator = std::make_shared<Operators::Map>(writeF3);
  scanOperator->setChild(mapOperator);

  auto emitMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(memoryLayoutOutput);
  auto emitOperator =
      std::make_shared<Operators::Emit>(std::move(emitMemoryProviderPtr));
  ...
  for (const auto &buf : pipelineContext.buffers) {
    auto resulttestBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer(memoryLayoutOutput, buf);
    for (uint64_t i = 0; i < resulttestBuffer.getNumberOfTuples(); i++) {
      ASSERT_EQ(resulttestBuffer[i]["f1"].read<int64_t>(), 10);
      ASSERT_EQ(resulttestBuffer[i]["f2"].read<int64_t>(), 1);
      ASSERT_EQ(resulttestBuffer[i]["f3"].read<int64_t>(), 10 + 1);
    }
    SequenceData resultTriplet = {buf.getSequenceNumber(), buf.getChunkNumber(),
                                  buf.isLastChunk()};
  ...
  // Checking, if we have seen all sequence numbers
  ASSERT_THAT(pipelineContext.seenSeqChunkLastChunk,
              ::testing::UnorderedElementsAreArray(expectedSeqChunkLastChunk));
}

std::shared_ptr<PhysicalOperatorPipeline>
createFirstPipeline(const MemoryLayouts::RowLayoutPtr &memoryLayoutInput,
...
std::shared_ptr<PhysicalOperatorPipeline>
createSecondPipeline(const MemoryLayouts::RowLayoutPtr &memoryLayoutInput,
  ...
  const auto readTsField =
      std::make_shared<Expressions::ReadFieldExpression>("ts");
  std::vector<Aggregation::AggregationFunctionPtr> aggregationFunctions = {
      std::move(aggregationFunction)};
  auto slicePreAggregation =
      std::make_shared<Operators::NonKeyedSlicePreAggregation>(
          0 /*handler index*/,
          std::make_unique<Operators::EventTimeFunction>(
...
std::shared_ptr<PhysicalOperatorPipeline>
createThirdPipeline(const MemoryLayouts::RowLayoutPtr &memoryLayoutOutput,
                    Aggregation::AggregationFunctionPtr aggregationFunction) {
  std::vector<Aggregation::AggregationFunctionPtr> aggregationFunctions = {
      std::move(aggregationFunction)};
  auto sliceMergingAction =
      std::make_unique<Operators::NonKeyedWindowEmitAction>(
          aggregationFunctions, "start", "end", INVALID_ORIGIN_ID);
  auto sliceMerging = std::make_shared<Operators::NonKeyedSliceMerging>(
      0 /*handler index*/, aggregationFunctions, std::move(sliceMergingAction));
...
/**
 * @brief We have a total of three pipelines here with two operators, map and a
 * window aggregation. 1) Map that has a larger output than input schema. 2)
 * PreAggPipeline that calculates pre-Aggregates for a window 3)
 * SliceMergingPipeline that merges the pre-Aggregates
 *
 * Additionally, we will send the output buffers of the first pipeline in a
 * random fashion. This way, we can test if a window operator waits until it has
 * seen all chunks of a sequence number.
 */
TEST_P(SequenceNumberPipelineTest, testMultipleSequenceNumbersWithAggregation) {
  ...
  auto memoryLayoutOutput = Runtime::MemoryLayouts::RowLayout::create(
      outputSchema, bm->getBufferSize());
  auto memoryLayoutOutputWindow = Runtime::MemoryLayouts::RowLayout::create(
      outputSchemaWindow, bm->getBufferSize());
  ...
  PhysicalTypePtr integerType = DefaultPhysicalTypeFactory().getPhysicalType(
      DataTypeFactory::createInt64());
  Aggregation::AggregationFunctionPtr aggregationFunction =
      std::make_shared<Aggregation::CountAggregationFunction>(
          integerType, integerType, readF1, aggregationResultFieldName1);

  // Creating executable pipelines
  auto pipeline1 = provider->create(
      createFirstPipeline(memoryLayoutInput, memoryLayoutOutput), options);
  auto pipeline2 = provider->create(
      createSecondPipeline(memoryLayoutOutput, aggregationFunction), options);
  auto pipeline3 = provider->create(
      createThirdPipeline(memoryLayoutOutputWindow, aggregationFunction),
      options);
  ...
  auto preAggregationHandler =
      std::make_shared<Operators::NonKeyedSlicePreAggregationHandler>(
          windowSize, windowSlide, origins);
  auto sliceMergingHandler =
      std::make_shared<Operators::NonKeyedSliceMergingHandler>();

  // Creating pipeline execution contexts
  auto pipeline1Context = MockedPipelineExecutionContext();
  auto pipeline2Context =
      MockedPipelineExecutionContext({preAggregationHandler}, false);
  ...
  // Comparing the expected number of output buffers and shuffling the buffers
  // to mix up the chunk numbers and last chunks
  EXPECT_EQ(pipeline1Context.buffers.size(), NUM_BUFFERS * 2);
  std::random_device rd;
  std::mt19937 gen(rd());
  std::shuffle(pipeline1Context.buffers.begin(), pipeline1Context.buffers.end(),
               gen);
  ...
  auto numberOfTuples = std::accumulate(
      pipeline3Context.buffers.begin(), pipeline3Context.buffers.end(), 0_u64,
  ...
  for (const auto &buf : pipeline3Context.buffers) {
    auto testBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(
            buf, outputSchemaWindow);
    for (auto i = 0_u64; i < testBuffer.getNumberOfTuples(); ++i) {
      // As we count the number of tuple per window, the count should be the
      // window size
      EXPECT_EQ(windowSize,
                testBuffer[i][aggregationResultFieldName1].read<int64_t>());
    }
  }
}

INSTANTIATE_TEST_CASE_P(
    testIfCompilation, SequenceNumberPipelineTest,
    ::testing::Values("PipelineInterpreter", "BCInterpreter",
                      "PipelineCompiler", "CPPPipelineCompiler"),
    [](const testing::TestParamInfo<SequenceNumberPipelineTest::ParamType>
