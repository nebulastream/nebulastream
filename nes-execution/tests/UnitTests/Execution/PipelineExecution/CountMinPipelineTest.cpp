...
public:
    ...
    void SetUp() override
    {
        ...
        statisticFormat = Statistic::StatisticFormatFactory::createFromSchema(
            outputSchema, bufferManager->getBufferSize(), Statistic::StatisticSynopsisType::COUNT_MIN, sinkDataCodec);
        ...
        const uint64_t numberOfBitsInKey)
    {
        ...
        auto countMinOperatorHandler = Operators::CountMinOperatorHandler::create(
            windowSize, windowSlide, sendingPolicy, width, depth, statisticFormat, inputOrigins, numberOfBitsInKey);
        ...
        auto countMinOperator = std::make_shared<Operators::CountMinBuild>(
            operatorHandlerIndex, fieldToBuildCountMinOver, numberOfBitsInKey, width, depth, metricHash, std::move(timeFunction));
