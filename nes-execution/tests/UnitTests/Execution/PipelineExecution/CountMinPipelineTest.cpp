...
class CountMinPipelineExecutionContext
    : public Runtime::Execution::PipelineExecutionContext {
public:
  CountMinPipelineExecutionContext(BufferManagerPtr bufferManager,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("CountMinPipelineTest.log",
  ...
  createExecutablePipeline(uint64_t windowSize, uint64_t windowSlide,
    ...
    auto countMinOperatorHandler = Operators::CountMinOperatorHandler::create(
        windowSize, windowSlide, sendingPolicy, width, depth, statisticFormat,
        inputOrigins, numberOfBitsInKey);
    pipelineExecutionContext =
        std::make_shared<CountMinPipelineExecutionContext>(
    ...
    auto countMinOperator = std::make_shared<Operators::CountMinBuild>(
        operatorHandlerIndex, fieldToBuildCountMinOver, numberOfBitsInKey,
        width, depth, metricHash, std::move(timeFunction));
...
TEST_P(CountMinPipelineTest, singleInputTuple) {
  ...
  auto executablePipeline = createExecutablePipeline(
      windowSize, windowSlide, width, depth, inputOrigins, numberOfBitsInKey);
  ...
  for (auto &buf : inputBuffers) {
    ...
    Util::updateTestCountMinStatistic(
        ...
        windowSize, windowSlide, width, depth, fieldToBuildCountMinOver,
        timestampFieldName);
  }
...
TEST_P(CountMinPipelineTest, multipleInputBuffers) {
  constexpr auto windowSize = 1000, windowSlide = 1000, width = 8096,
                 depth = 10;
  constexpr auto numberOfBitsInKey = sizeof(64) * 8;
  const std::vector inputOrigins = {OriginId(1)};
  auto executablePipeline = createExecutablePipeline(
...
INSTANTIATE_TEST_CASE_P(
    testCountMinPipeline, CountMinPipelineTest,
    ::testing::Combine(
