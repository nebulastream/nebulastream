...
class NonKeyedTimeWindowPipelineTest : public Testing::BaseUnitTest,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NonKeyedTimeWindowPipelineTest.log",
...
/**
 * @brief Test running a pipeline containing a threshold window with a sum
 * aggregation
 */
TEST_P(NonKeyedTimeWindowPipelineTest, windowWithSum) {
  ...
  auto scanMemoryLayout = Runtime::MemoryLayouts::RowLayout::create(
      scanSchema, bm->getBufferSize());

  auto scanMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(scanMemoryLayout);
  auto scanOperator =
  ...
  PhysicalTypePtr integerType =
      physicalTypeFactory.getPhysicalType(DataTypeFactory::createInt64());
  std::vector<std::shared_ptr<Aggregation::AggregationFunction>>
      aggregationFunctions = {
          std::make_shared<Aggregation::SumAggregationFunction>(
              integerType, integerType, readF2, aggregationResultFieldName)};
  ...
  auto emitMemoryLayout = Runtime::MemoryLayouts::RowLayout::create(
      emitSchema, bm->getBufferSize());
  auto emitMemoryProviderPtr =
      std::make_unique<MemoryProvider::RowMemoryProvider>(emitMemoryLayout);
  auto emitOperator =
  ...
  auto testBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(scanMemoryLayout, buffer);
  ...
  auto preAggregationHandler =
      std::make_shared<Operators::NonKeyedSlicePreAggregationHandler>(10, 10,
                                                                      origins);

  auto pipeline1Context =
      MockedPipelineExecutionContext({preAggregationHandler});
  preAggExecutablePipeline->setup(pipeline1Context);
  preAggExecutablePipeline->execute(buffer, pipeline1Context, *wc);
  auto sliceMergingExecutablePipeline =
      provider->create(sliceMergingPipeline, options);
  ...
  sliceMergingExecutablePipeline->execute(pipeline1Context.buffers[0],
                                          pipeline2Context, *wc);
  ...
  auto resulttestBuffer = Runtime::MemoryLayouts::TestTupleBuffer(
      emitMemoryLayout, pipeline2Context.buffers[0]);
  EXPECT_EQ(resulttestBuffer[0][aggregationResultFieldName].read<int64_t>(),
            100);
...
TEST_P(NonKeyedTimeWindowPipelineTest, windowWithMultiAggregates) {
  ...
  std::vector<std::shared_ptr<Aggregation::AggregationFunction>>
      aggregationFunctions = {
          std::make_shared<Aggregation::SumAggregationFunction>(
              integerType, integerType, readF2, aggregationResultFieldName1),
          std::make_shared<Aggregation::AvgAggregationFunction>(
              integerType, integerType, readF2, aggregationResultFieldName2),
          std::make_shared<Aggregation::MinAggregationFunction>(
              integerType, integerType, readF2, aggregationResultFieldName3),
          std::make_shared<Aggregation::MaxAggregationFunction>(
              integerType, integerType, readF2, aggregationResultFieldName4)};
  auto slicePreAggregation =
      std::make_shared<Operators::NonKeyedSlicePreAggregation>(
          0 /*handler index*/,
          std::make_unique<Operators::EventTimeFunction>(
...
/**
 * @brief Test running a pipeline containing a threshold window with two min
 * aggregations with different data types
 */
TEST_P(NonKeyedTimeWindowPipelineTest,
       windowWithMultiAggregatesOnDifferentDataType) {
  auto scanSchema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT);
  scanSchema->addField("f1", BasicType::INT64);
  scanSchema->addField("f2", BasicType::INT64);
  ...
  PhysicalTypePtr integerType =
      physicalTypeFactory.getPhysicalType(DataTypeFactory::createInt64());
  PhysicalTypePtr floatType =
      physicalTypeFactory.getPhysicalType(DataTypeFactory::createFloat());
  ...
  std::vector<std::shared_ptr<Aggregation::AggregationFunction>>
      aggregationFunctions = {
          std::make_shared<Aggregation::MinAggregationFunction>(
              integerType, integerType, readF2, aggregationResultFieldName1),
          std::make_shared<Aggregation::MinAggregationFunction>(
              floatType, floatType, readF3, aggregationResultFieldName2)};
  auto slicePreAggregation =
      std::make_shared<Operators::NonKeyedSlicePreAggregation>(
          0 /*handler index*/,
          std::make_unique<Operators::EventTimeFunction>(
...
INSTANTIATE_TEST_CASE_P(
    testIfCompilation, NonKeyedTimeWindowPipelineTest,
    ::testing::Values("PipelineInterpreter", "PipelineCompiler",
                      "CPPPipelineCompiler"),
    [](const testing::TestParamInfo<NonKeyedTimeWindowPipelineTest::ParamType>
