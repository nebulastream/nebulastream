...
class HyperLogLogPipelineExecutionContext
    : public Runtime::Execution::PipelineExecutionContext {
 public:
  HyperLogLogPipelineExecutionContext(BufferManagerPtr bufferManager,
                                      OperatorHandlerPtr operatorHandler)
...
class HyperLogLogPipelineTest
    : public Testing::BaseUnitTest,
      public ::testing::WithParamInterface<
          std::tuple<std::string, Statistic::StatisticDataCodec>> {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("HyperLogLogPipelineTest.log",
  ...
  void SetUp() override {
    ...
    if (!ExecutablePipelineProviderRegistry::hasPlugin(
            std::get<0>(GetParam()))) {
      GTEST_SKIP();
    }
    // Creating class members for being able to use them in all test cases
    provider =
        ExecutablePipelineProviderRegistry::getPlugin(std::get<0>(GetParam()))
            .get();
    sinkDataCodec = std::get<1>(GetParam());
    bufferManager = std::make_shared<Runtime::BufferManager>();
    workerContext = std::make_shared<WorkerContext>(INITIAL<WorkerThreadId>,
                                                    bufferManager, 100);
    ...
    statisticFormat = Statistic::StatisticFormatFactory::createFromSchema(
        outputSchema, bufferManager->getBufferSize(),
   ...
   */
  std::unique_ptr<ExecutablePipelineStage> createExecutablePipeline(
      uint64_t windowSize, uint64_t windowSlide, uint64_t width,
      const std::vector<OriginId>& inputOrigins) {
    // 1. Creating the scan operator
    auto memoryLayout = Runtime::MemoryLayouts::RowLayout::create(
        inputSchema, bufferManager->getBufferSize());
    auto scanMemoryProvider =
        std::make_unique<MemoryProvider::RowMemoryProvider>(memoryLayout);
    auto scanOperator =
        std::make_shared<Operators::Scan>(std::move(scanMemoryProvider));

    // 2. Creating the hyper log log operator handler
    auto countMinOperatorHandler =
        Operators::HyperLogLogOperatorHandler::create(
            windowSize, windowSlide, sendingPolicy, statisticFormat, width,
            inputOrigins);
    pipelineExecutionContext =
        std::make_shared<HyperLogLogPipelineExecutionContext>(
            bufferManager, countMinOperatorHandler);
    ...
    const auto readTsField =
        std::make_shared<Expressions::ReadFieldExpression>(timestampFieldName);
    auto timeFunction =
        std::make_unique<Runtime::Execution::Operators::EventTimeFunction>(
            readTsField, Windowing::TimeUnit::Milliseconds());
    auto countMinOperator = std::make_shared<Operators::HyperLogLogBuild>(
        operatorHandlerIndex, fieldToBuildCountMinOver, metricHash,
...
/**
 * @brief Here we test, if we create a hyper loglog sketches for a single input
 * tuple
 */
TEST_P(HyperLogLogPipelineTest, singleInputTuple) {
  ...
  auto executablePipeline =
      createExecutablePipeline(windowSize, windowSlide, width, inputOrigins);

  auto inputBuffers = Util::createDataForOneFieldAndTimeStamp(
      1, *bufferManager, inputSchema, fieldToBuildCountMinOver,
      timestampFieldName);
  ...
  for (auto& buf : inputBuffers) {
    ...
    auto dynamicBuffer =
        MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(buf, inputSchema);
    Util::updateTestHyperLogLogStatistic(
        dynamicBuffer, testStatisticStore, metricHash, windowSize, windowSlide,
        width, fieldToBuildCountMinOver, timestampFieldName);
  }

  for (auto& emitBuf : pipelineExecutionContext->emittedBuffers) {
    auto createdCountMinStatistics =
        statisticFormat->readStatisticsFromBuffer(emitBuf);
...
/**
 * @brief Here we test, if we create multiple hyper loglog sketches for multiple
...
INSTANTIATE_TEST_CASE_P(
    testHyperLogLogPipeline, HyperLogLogPipelineTest,
    ::testing::Combine(
