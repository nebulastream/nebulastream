...
class BatchAggregationPipelineTest : public Testing::BaseUnitTest,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BatchAggregationPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down BatchAggregationPipelineTest test class.");
  }
};
...
TEST_P(BatchAggregationPipelineTest, aggregationPipeline) {
  ...
  std::vector<std::shared_ptr<Aggregation::AggregationFunction>>
      aggregationFunctions = {
          std::make_shared<Aggregation::SumAggregationFunction>(
              integerType, integerType, readF1, aggregationResultFieldName)};
  auto aggregationOp = std::make_shared<Operators::BatchAggregation>(
      0 /*handler index*/, aggregationFunctions);
  ...
  auto preAggregationHandler =
      std::make_shared<Operators::BatchAggregationHandler>();
  auto pipeline1Context =
      MockedPipelineExecutionContext({preAggregationHandler});

  auto aggScan = std::make_shared<Operators::BatchAggregationScan>(
      0 /*handler index*/, aggregationFunctions);
  auto emitOperator =
...
TEST_P(BatchAggregationPipelineTest, keyedAggregationPipeline) {
  auto schema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT);
  schema = schema->addField("f1", BasicType::INT64)
  ...
  auto resultMemoryLayout = Runtime::MemoryLayouts::RowLayout::create(
      resultSchema, bm->getBufferSize());
  ...
  auto aggregationOp = std::make_shared<Operators::BatchKeyedAggregation>(
      0 /*handler index*/, keyFields, types, aggregationFunctions,
  ...
  auto preAggExecutablePipeline = provider->create(pipeline, options);
  auto preAggregationHandler =
      std::make_shared<Operators::BatchKeyedAggregationHandler>();
  auto pipeline1Context =
      MockedPipelineExecutionContext({preAggregationHandler});
  preAggExecutablePipeline->setup(pipeline1Context);
  preAggExecutablePipeline->execute(buffer, pipeline1Context, *wc);
  preAggExecutablePipeline->stop(pipeline1Context);
  auto hmSize =
      preAggregationHandler->getThreadLocalStore(wc->getId())->getCurrentSize();
  ASSERT_EQ(hmSize, 3);
}

INSTANTIATE_TEST_CASE_P(
    testIfCompilation, BatchAggregationPipelineTest,
    ::testing::Values("PipelineInterpreter", "BCInterpreter",
                      "PipelineCompiler", "CPPPipelineCompiler"),
    [](const testing::TestParamInfo<BatchAggregationPipelineTest::ParamType>
