...
class BatchJoinPipelineTest : public Testing::BaseUnitTest,
...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BatchJoinPipelineTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down BatchJoinPipelineTest test class.");
  }
};

TEST_P(BatchJoinPipelineTest, joinBuildPipeline) {
  auto schema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT);
  schema = schema->addField("k1", BasicType::INT64)
  ...
  auto joinOp = std::make_shared<Operators::BatchJoinBuild>(
      0 /*handler index*/, keyFields, types, valueFields, types,
  ...
  auto entries =
      joinHandler->getThreadLocalState(wc->getId())->getNumberOfEntries();
  ASSERT_EQ(entries, 4_u64);
}

INSTANTIATE_TEST_CASE_P(
    testIfCompilation, BatchJoinPipelineTest,
