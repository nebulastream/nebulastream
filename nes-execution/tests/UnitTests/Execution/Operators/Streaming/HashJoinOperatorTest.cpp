...
bool hashJoinBuildAndCheck(HashJoinBuildHelper buildHelper)
{
    ...
    auto hashJoinOpHandler = std::dynamic_pointer_cast<Operators::HJOperatorHandlerSlicing>(Operators::HJOperatorHandlerSlicing::create(
    ...
    for (auto i = 0UL; i < buildHelper.numberOfTuplesToProduce + 1; ++i)
    {
        ...
        auto window = hashJoinOpHandler->getSliceByTimestampOrCreateIt(timeStamp, buildHelper.bufferManager);
...
bool hashJoinProbeAndCheck(HashJoinProbeHelper hashJoinProbeHelper)
{
    auto workerContext = std::make_shared<WorkerContext>(
        INITIAL<WorkerThreadId>, hashJoinProbeHelper.bufferManager, hashJoinProbeHelper.numberOfBuffersPerWorker);
    ...
    for (auto i = 0UL; i < hashJoinProbeHelper.numberOfTuplesToProduce + 1; ++i)
    {
        auto recordLeft = Nautilus::Record(
            {{hashJoinProbeHelper.leftSchema->get(0)->getName(), Value<UInt64>((uint64_t)i)},
        ...
        auto recordRight = Nautilus::Record(
            {{hashJoinProbeHelper.rightSchema->get(0)->getName(), Value<UInt64>((uint64_t)i + 1000)},
        ...
        {
            NES_DEBUG("rects={} >= {}", recordRight.read(hashJoinProbeHelper.timeStampFieldRight)->toString(), lastTupleTimeStampWindow);
    ...
    auto maxWindowIdentifier
        = std::ceil((double)hashJoinProbeHelper.numberOfTuplesToProduce / hashJoinProbeHelper.windowSize) * hashJoinProbeHelper.windowSize;
    ...
    {
        auto expectedNumberOfTuplesInWindowLeft
            = calculateExpNoTuplesInWindow(hashJoinProbeHelper.numberOfTuplesToProduce, windowIdentifier, hashJoinProbeHelper.windowSize);
        auto expectedNumberOfTuplesInWindowRight
            = calculateExpNoTuplesInWindow(hashJoinProbeHelper.numberOfTuplesToProduce, windowIdentifier, hashJoinProbeHelper.windowSize);
                ...
                for (auto& rightRecordOuter : rightRecords)
                {
                    for (auto& rightRecordInner : rightRecordOuter)
                    {
                        auto timestampLeftVal
                            = leftRecordInner.read(hashJoinProbeHelper.timeStampFieldLeft).getValue().staticCast<UInt64>().getValue();
                        auto timestampRightVal
                            = rightRecordInner.read(hashJoinProbeHelper.timeStampFieldRight).getValue().staticCast<UInt64>().getValue();
