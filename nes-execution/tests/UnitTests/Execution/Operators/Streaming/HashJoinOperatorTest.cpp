...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("HashJoinOperatorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down HashJoinOperatorTest test class.");
  }
};
...
struct HashJoinBuildHelper {
  ...
  HashJoinBuildHelper(Operators::HJBuildSlicingPtr hashJoinBuild,
                      ...
                      BufferManagerPtr bufferManager, SchemaPtr schema,
                      const std::string &timeStampField,
                      HashJoinOperatorTest *hashJoinOperatorTest,
                      QueryCompilation::JoinBuildSideType joinBuildSide)
      : pageSize(131072), numPartitions(1), numberOfTuplesToProduce(100),
        numberOfBuffersPerWorker(128), noWorkerThreads(1), preAllocPageCnt(1),
        joinSizeInByte(1 * 1024 * 1024), windowSize(1000),
        hashJoinBuild(hashJoinBuild), joinFieldName(joinFieldName),
        bufferManager(bufferManager), schema(schema),
...
bool hashJoinBuildAndCheck(HashJoinBuildHelper buildHelper) {
  ...
  auto workerContext = std::make_shared<WorkerContext>(
      INITIAL<WorkerThreadId>, buildHelper.bufferManager,
      buildHelper.numberOfBuffersPerWorker);
  auto hashJoinOpHandler =
      std::dynamic_pointer_cast<Operators::HJOperatorHandlerSlicing>(
          Operators::HJOperatorHandlerSlicing::create(
              std::vector({OriginId(1)}), outputOriginId,
              buildHelper.windowSize, buildHelper.windowSize,
              buildHelper.schema, buildHelper.schema,
              QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
              buildHelper.joinSizeInByte, buildHelper.preAllocPageCnt,
              buildHelper.pageSize, buildHelper.numPartitions));
  ...
  auto pipelineContext = PipelineExecutionContext(
      ...
      nullptr, buildHelper.noWorkerThreads,
  ...
  for (auto i = 0UL; i < buildHelper.numberOfTuplesToProduce + 1; ++i) {
    auto record = Nautilus::Record(
        {{buildHelper.schema->get(0)->getName(), Value<UInt64>((uint64_t)i)},
         {buildHelper.schema->get(1)->getName(),
          Value<UInt64>((uint64_t)(i % 10) + 1)},
         {buildHelper.schema->get(2)->getName(), Value<UInt64>((uint64_t)i)}});

    if (i == 0) {
      auto tupleBuffer = Util::getBufferFromRecord(record, buildHelper.schema,
                                                   buildHelper.bufferManager);
      RecordBuffer recordBuffer =
          RecordBuffer(Value<MemRef>((int8_t *)std::addressof(tupleBuffer)));
      buildHelper.hashJoinBuild->open(executionContext, recordBuffer);
    }

    buildHelper.hashJoinBuild->execute(executionContext, record);

    uint64_t joinKey = record.read(buildHelper.joinFieldName)
                           .as<UInt64>()
                           .getValue()
                           .getValue();
    uint64_t timeStamp = record.read(buildHelper.timeStampField)
                             .as<UInt64>()
                             .getValue()
                             .getValue();
    ...
    auto hashTable = hashWindow->getHashTable(buildHelper.joinBuildSide,
                                              workerContext->getId());
    ...
    for (auto &&page : bucket->getPages()) {
      for (auto k = 0UL; k < page->size(); ++k) {
        ...
        auto bucketBuffer = Util::getBufferFromPointer(
            recordPtr, buildHelper.schema, buildHelper.bufferManager);
        auto recordBuffer = Util::getBufferFromRecord(
            record, buildHelper.schema, buildHelper.bufferManager);

        if (memcmp(bucketBuffer.getBuffer(), recordBuffer.getBuffer(),
                   buildHelper.schema->getSchemaSizeInBytes()) == 0) {
    ...
    if (!correctlyInserted) {
      auto recordBuffer = Util::getBufferFromRecord(record, buildHelper.schema,
                                                    buildHelper.bufferManager);
      NES_ERROR("Could not find record {} in bucket!",
                Util::printTupleBufferAsCSV(recordBuffer, buildHelper.schema));
      return false;
    }
  }
...
struct HashJoinProbeHelper {
  ...
  HashJoinProbeHelper(const std::string &joinFieldNameLeft,
                      ...
                      BufferManagerPtr bufferManager, SchemaPtr leftSchema,
                      SchemaPtr rightSchema,
                      const std::string &timeStampFieldLeft,
                      const std::string &timeStampFieldRight,
                      HashJoinOperatorTest *hashJoinOperatorTest)
      : pageSize(131072), numPartitions(1), numberOfTuplesToProduce(100),
        numberOfBuffersPerWorker(128), noWorkerThreads(1),
        joinSizeInByte(1 * 1024 * 1024), preAllocPageCnt(1), windowSize(1000),
...
uint64_t calculateExpNoTuplesInWindow(uint64_t totalTuples,
...
bool hashJoinProbeAndCheck(HashJoinProbeHelper hashJoinProbeHelper) {
  ...
  auto workerContext = std::make_shared<WorkerContext>(
      INITIAL<WorkerThreadId>, hashJoinProbeHelper.bufferManager,
  ...
  auto hashJoinOpHandler = Operators::HJOperatorHandlerSlicing::create(
      inputOriginIds, outputOriginId, hashJoinProbeHelper.windowSize,
      hashJoinProbeHelper.windowSize, hashJoinProbeHelper.leftSchema,
      hashJoinProbeHelper.rightSchema,
      QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
      hashJoinProbeHelper.joinSizeInByte, hashJoinProbeHelper.preAllocPageCnt,
      hashJoinProbeHelper.pageSize, hashJoinProbeHelper.numPartitions);

  auto hashJoinOperatorTest = hashJoinProbeHelper.hashJoinOperatorTest;
  auto pipelineContext = PipelineExecutionContext(
      INVALID_PIPELINE_ID,      // mock pipeline id
      DecomposedQueryPlanId(1), // mock query id
      hashJoinProbeHelper.bufferManager, hashJoinProbeHelper.noWorkerThreads,
      [&hashJoinOperatorTest](TupleBuffer &buffer, Runtime::WorkerContextRef) {
        hashJoinOperatorTest->emittedBuffers.emplace_back(std::move(buffer));
      },
      [&hashJoinOperatorTest](TupleBuffer &buffer) {
  ...
  auto readTsFieldLeft = std::make_shared<Expressions::ReadFieldExpression>(
      hashJoinProbeHelper.timeStampFieldLeft);
  auto readTsFieldRight = std::make_shared<Expressions::ReadFieldExpression>(
      hashJoinProbeHelper.timeStampFieldRight);

  auto hashJoinBuildLeft = std::make_shared<Operators::HJBuildSlicing>(
      handlerIndex, hashJoinProbeHelper.leftSchema,
  ...
  auto hashJoinBuildRight = std::make_shared<Operators::HJBuildSlicing>(
      ...
      std::make_unique<Runtime::Execution::Operators::EventTimeFunction>(
          readTsFieldRight, Windowing::TimeUnit::Milliseconds()),
      QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
      QueryCompilation::WindowingStrategy::SLICING);

  Operators::JoinSchema joinSchema(
      hashJoinProbeHelper.leftSchema, hashJoinProbeHelper.rightSchema,
      Util::createJoinSchema(hashJoinProbeHelper.leftSchema,
                             hashJoinProbeHelper.rightSchema));
  Operators::WindowMetaData windowMetaData(
      joinSchema.joinSchema->get(0)->getName(),
      joinSchema.joinSchema->get(1)->getName());

  auto onLeftKey = std::make_shared<Expressions::ReadFieldExpression>(
      hashJoinProbeHelper.joinFieldNameLeft);
  auto onRightKey = std::make_shared<Expressions::ReadFieldExpression>(
      hashJoinProbeHelper.joinFieldNameRight);
  auto keyExpressions =
      std::make_shared<Expressions::EqualsExpression>(onLeftKey, onRightKey);

  auto hashJoinProbe = std::make_shared<Operators::HJProbe>(
      handlerIndex, joinSchema, keyExpressions, windowMetaData,
      QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
      QueryCompilation::WindowingStrategy::SLICING,
      /*withDeletion*/ false);
  ...
  for (auto i = 0UL; i < hashJoinProbeHelper.numberOfTuplesToProduce + 1; ++i) {
    auto recordLeft =
        Nautilus::Record({{hashJoinProbeHelper.leftSchema->get(0)->getName(),
                           Value<UInt64>((uint64_t)i)},
                          {hashJoinProbeHelper.leftSchema->get(1)->getName(),
                           Value<UInt64>((uint64_t)(i % 10) + 10)},
                          {hashJoinProbeHelper.leftSchema->get(2)->getName(),
                           Value<UInt64>((uint64_t)i)}});
    NES_DEBUG("Tuple left id={} key={} ts={}", i, (i % 10) + 10, i);
    auto recordRight =
        Nautilus::Record({{hashJoinProbeHelper.rightSchema->get(0)->getName(),
                           Value<UInt64>((uint64_t)i + 1000)},
                          {hashJoinProbeHelper.rightSchema->get(1)->getName(),
                           Value<UInt64>((uint64_t)(i % 10) + 10)},
                          {hashJoinProbeHelper.rightSchema->get(2)->getName(),
                           Value<UInt64>((uint64_t)i)}});
    NES_DEBUG("Tuple right f1_left={} kef2_left(key)={} ts={}", i + 1000,
              (i % 10) + 10, i);

    if (recordRight.read(hashJoinProbeHelper.timeStampFieldRight) >
        lastTupleTimeStampWindow) {
      NES_DEBUG(
          "rects={} >= {}",
          recordRight.read(hashJoinProbeHelper.timeStampFieldRight)->toString(),
          lastTupleTimeStampWindow);
      leftRecords.push_back(
          std::vector(tmpRecordsLeft.begin(), tmpRecordsLeft.end()));
      rightRecords.push_back(
          std::vector(tmpRecordsRight.begin(), tmpRecordsRight.end()));
  ...
  for (auto i = 0UL; i < leftRecords.size(); i++) {
    ...
    executionContext.setWatermarkTs(
        leftRecords[i][size - 1]
            .read(hashJoinProbeHelper.timeStampFieldLeft)
            .as<UInt64>());
    executionContext.setCurrentTs(
        leftRecords[i][size - 1]
            .read(hashJoinProbeHelper.timeStampFieldLeft)
            .as<UInt64>());
    ...
    NES_DEBUG("trigger left with ts={}",
              leftRecords[i][size - 1]
                  .read(hashJoinProbeHelper.timeStampFieldLeft)
                  ->toString());
  ...
  for (auto i = 0UL; i < rightRecords.size(); i++) {
    auto tupleBuffer = hashJoinProbeHelper.bufferManager->getBufferBlocking();
    RecordBuffer recordBufferRight =
        RecordBuffer(Value<MemRef>((int8_t *)std::addressof(tupleBuffer)));
    if (i == 0) {
    ...
    executionContext.setWatermarkTs(
        rightRecords[i][size - 1]
            .read(hashJoinProbeHelper.timeStampFieldRight)
            .as<UInt64>());
    executionContext.setCurrentTs(
        rightRecords[i][size - 1]
            .read(hashJoinProbeHelper.timeStampFieldRight)
  ...
  auto numberOfEmittedBuffersBuild =
      hashJoinOperatorTest->emittedBuffers.size();
  NES_DEBUG("trigger Probe for numberOfEmittedBuffersBuild = {}",
            numberOfEmittedBuffersBuild);
  ...
  hashJoinOperatorTest->emittedBuffers.erase(
      hashJoinOperatorTest->emittedBuffers.begin(),
      hashJoinOperatorTest->emittedBuffers.begin() +
          numberOfEmittedBuffersBuild);

  /* Checking if all windows have been deleted except for one.
   * We require always one window as we do not know here if we have to take care
   * of more tuples*/
  if (hashJoinOpHandler->as<Operators::StreamJoinOperatorHandler>()
          ->getNumberOfSlices() != 1) {
    NES_ERROR("Not exactly one active window! {}",
              hashJoinOpHandler->as<Operators::StreamJoinOperatorHandler>()
                  ->getNumberOfSlices());
    // TODO: this is tricky now we can either activate deletion but then the
    // later code cannot check the window size or we test this here
    //         return false;
  }

  Value<UInt64> zeroValue((uint64_t)0UL);
  auto maxWindowIdentifier =
      std::ceil((double)hashJoinProbeHelper.numberOfTuplesToProduce /
                hashJoinProbeHelper.windowSize) *
      hashJoinProbeHelper.windowSize;
  for (auto windowIdentifier = hashJoinProbeHelper.windowSize;
       windowIdentifier < maxWindowIdentifier;
       windowIdentifier += hashJoinProbeHelper.windowSize) {
    auto expectedNumberOfTuplesInWindowLeft = calculateExpNoTuplesInWindow(
        hashJoinProbeHelper.numberOfTuplesToProduce, windowIdentifier,
        hashJoinProbeHelper.windowSize);
    auto expectedNumberOfTuplesInWindowRight = calculateExpNoTuplesInWindow(
        hashJoinProbeHelper.numberOfTuplesToProduce, windowIdentifier,
        hashJoinProbeHelper.windowSize);

    auto existingNumberOfTuplesInWindowLeft =
        hashJoinOpHandler->getNumberOfTuplesInSlice(
            windowIdentifier, QueryCompilation::JoinBuildSideType::Left);

    auto existingNumberOfTuplesInWindowRight =
        hashJoinOpHandler->getNumberOfTuplesInSlice(
            windowIdentifier, QueryCompilation::JoinBuildSideType::Right);

    if (existingNumberOfTuplesInWindowLeft !=
            expectedNumberOfTuplesInWindowLeft ||
        existingNumberOfTuplesInWindowRight !=
            expectedNumberOfTuplesInWindowRight) {
      NES_ERROR("wrong number of inputs are created "
                "existingNumberOfTuplesInWindowLeft={} "
                "expectedNumberOfTuplesInWindowLeft={} "
                "existingNumberOfTuplesInWindowRight={} "
                "expectedNumberOfTuplesInWindowRight={} windowIdentifier={}",
                existingNumberOfTuplesInWindowLeft,
                expectedNumberOfTuplesInWindowLeft,
                existingNumberOfTuplesInWindowRight,
                expectedNumberOfTuplesInWindowRight, windowIdentifier);
      ...
      for (auto &leftRecordInner : leftRecordOuter) {
        for (auto &rightRecordOuter : rightRecords) {
          for (auto &rightRecordInner : rightRecordOuter) {
            auto timestampLeftVal =
                leftRecordInner.read(hashJoinProbeHelper.timeStampFieldLeft)
                    .getValue()
                    .staticCast<UInt64>()
                    .getValue();
            auto timestampRightVal =
                rightRecordInner.read(hashJoinProbeHelper.timeStampFieldRight)
                    .getValue()
                    .staticCast<UInt64>()
                    .getValue();
            ...
            auto rightKey =
                rightRecordInner.read(hashJoinProbeHelper.joinFieldNameRight);
                ...
                physicalDataTypeFactory
                    .getPhysicalType(
                        hashJoinProbeHelper.leftSchema
                            ->get(hashJoinProbeHelper.joinFieldNameLeft)
                            ->getDataType())
                    ->size();
            auto leftTupleSize =
                hashJoinProbeHelper.leftSchema->getSchemaSizeInBytes();
            ...
            if (windowStart <= timestampLeftVal &&
                timestampLeftVal < windowEnd &&
                windowStart <= timestampRightVal &&
              ...
              joinedRecord.write(joinSchema.joinSchema->get(0)->getName(),
                                 windowStartVal);
              joinedRecord.write(joinSchema.joinSchema->get(1)->getName(),
                                 windowEndVal);

              // Writing the leftSchema fields
              for (auto &field : hashJoinProbeHelper.leftSchema->fields) {
                joinedRecord.write(field->getName(),
                                   leftRecordInner.read(field->getName()));
              }

              // Writing the rightSchema fields
              for (auto &field : hashJoinProbeHelper.rightSchema->fields) {
                joinedRecord.write(field->getName(),
                                   rightRecordInner.read(field->getName()));
              }

              // Check if this joinedRecord is in the emitted records
              auto it = std::find(collector->records.begin(),
                                  collector->records.end(), joinedRecord);
              if (it == collector->records.end()) {
                NES_ERROR(
                    "Could not find joinedRecord {} in the emitted records!",
                    joinedRecord.toString());
                return false;
              }
...
public:
  void onFatalError(int signalNumber, std::string callStack) override {
    ...
    fatalErrorMessage << "onFatalError: signal [" << signalNumber << "] error ["
                      << strerror(errno) << "] callstack " << callStack;

    NES_FATAL_ERROR("{}", fatalErrorMessage.str());
    std::cerr << fatalErrorMessage.str() << std::endl;
  }

  void onFatalException(std::shared_ptr<std::exception> exceptionPtr,
                        std::string callStack) override {
    std::ostringstream fatalExceptionMessage;
    fatalExceptionMessage << "onFatalException: exception=["
                          << exceptionPtr->what() << "] callstack=\n"
                          << callStack;
...
TEST_F(HashJoinOperatorTest, joinBuildTest) {
  ...
  auto handlerIndex = 0;
  auto readTsField =
      std::make_shared<Expressions::ReadFieldExpression>(timeStampField);

  auto hashJoinBuild = std::make_shared<Operators::HJBuildSlicing>(
      handlerIndex, leftSchema, joinFieldNameLeft, isLeftSide,
      leftSchema->getSchemaSizeInBytes(),
      std::make_unique<Runtime::Execution::Operators::EventTimeFunction>(
          readTsField, Windowing::TimeUnit::Milliseconds()),
      QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
      QueryCompilation::WindowingStrategy::SLICING);

  HashJoinBuildHelper buildHelper(hashJoinBuild, joinFieldNameLeft, bm,
                                  leftSchema, timeStampField, this, isLeftSide);
...
TEST_F(HashJoinOperatorTest, joinBuildTestRight) {
  ...
  auto handlerIndex = 0;
  auto hashJoinBuild = std::make_shared<Operators::HJBuildSlicing>(
      handlerIndex, rightSchema, joinFieldNameRight, isLeftSide,
      rightSchema->getSchemaSizeInBytes(),
      std::make_unique<Runtime::Execution::Operators::EventTimeFunction>(
          readTsField, Windowing::TimeUnit::Milliseconds()),
      QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
      QueryCompilation::WindowingStrategy::SLICING);
  HashJoinBuildHelper buildHelper(hashJoinBuild, joinFieldNameRight, bm,
                                  rightSchema, timeStampField, this,
                                  isLeftSide);
...
TEST_F(HashJoinOperatorTest, joinProbeTest) {
  ...
  ASSERT_EQ(leftSchema->getSchemaSizeInBytes(),
            rightSchema->getSchemaSizeInBytes());

  HashJoinProbeHelper hashJoinProbeHelper(
      "left$f2_left", "right$f2_right", bm, leftSchema, rightSchema,
      "left$timestamp", "right$timestamp", this);
