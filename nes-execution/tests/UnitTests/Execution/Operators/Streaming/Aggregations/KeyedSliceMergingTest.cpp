...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedSliceMergingTest.log",
  ...
  createSlice(size_t start, size_t end,
...
public:
  void verify(ExecutionContext &, ExecuteOperatorPtr &, Value<UInt64> &,
              Value<UInt64> &, Value<UInt64> &, Value<UInt64> &,
              Value<Boolean> &, Value<MemRef> &globalSlice) {
    ...
    Interface::ChainedHashMapRef ref(
        Value<MemRef>(reinterpret_cast<int8_t *>(slice->getState().get())),
        {integerType}, integerType->size(), 8);

    for (const auto &entryRef : ref) {
      uint64_t key = 0;
      uint64_t value = 0;
      std::memcpy(&key, entryRef.getKeyPtr().getValue().value, sizeof(key));
      std::memcpy(&value, entryRef.getValuePtr().getValue().value,
                  sizeof(value));
  ...
  explicit ChainedHashmapVerifier(std::map<uint64_t, uint64_t> kvs)
      : kvs(std::move(kvs)) {}
...
public:
  MOCK_METHOD(void, emitSlice,
              (ExecutionContext & ctx, ExecuteOperatorPtr &child,
               ...
               Value<Boolean> &lastChunk, Value<MemRef> &globalSlice),
              (const, override));
};
...
TEST_F(KeyedSliceMergingTest, aggregate) {
  ...
  EXPECT_CALL(*sliceMergingAction,
              emitSlice(_, _, Eq(Value<UInt64>(0_u64)),
                        Eq(Value<UInt64>(400_u64)), Eq(Value<UInt64>(1_u64)),
                        Eq(Value<UInt64>(1_u64)), Eq(Value<Boolean>(true)), _))
      .Times(1)
      .WillOnce(Invoke(&expectedResult, &ChainedHashmapVerifier::verify));

  auto merging = KeyedSliceMerging(
      0 /*handler index*/,
      {std::make_shared<Aggregation::SumAggregationFunction>(
          integerType, integerType, readV1, "sum")},
      std::move(sliceMergingAction), std::vector{integerType}, 8, 8);
  ...
  auto sliceMergeTaskBuffer =
      bufferManager->getUnpooledBuffer(sizeof(SliceMergeTask<KeyedSlice>));
  ASSERT_TRUE(sliceMergeTaskBuffer.has_value());
  ASSERT_EQ(reinterpret_cast<uintptr_t>(sliceMergeTaskBuffer->getBuffer()) %
                alignof(SliceMergeTask<KeyedSlice>),
            0);
  new (sliceMergeTaskBuffer->getBuffer())
      SliceMergeTask<KeyedSlice>(1, 1, true, 0, 400,
                                 {
  ...
  auto recordBuffer = RecordBuffer(Value<MemRef>(
      reinterpret_cast<int8_t *>(std::addressof(sliceMergeTaskBuffer))));
