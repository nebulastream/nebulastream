...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MultiOriginWatermarkProcessorTest.log",
...
TEST_F(MultiOriginWatermarkProcessorTest, singleThreadWatermarkUpdaterTest) {
  ...
  auto watermarkManager =
      MultiOriginWatermarkProcessor::create(std::vector{INVALID_ORIGIN_ID});
  // preallocate watermarks for each transaction
  std::vector<std::tuple<WatermarkTs, SequenceData, OriginId>>
      watermarkBarriers;
  for (auto i = 1_u64; i <= updates; i++) {
    watermarkBarriers.emplace_back(
        std::tuple<WatermarkTs, SequenceData, OriginId>(i, {i, 1, true},
                                                        INVALID_ORIGIN_ID));
  }
  for (auto i = 0_u64; i < updates; i++) {
    ...
    ASSERT_LE(watermarkManager->getCurrentWatermark(),
              std::get<0>(currentWatermarkBarrier));
  }
  ASSERT_EQ(watermarkManager->getCurrentWatermark(),
            std::get<0>(watermarkBarriers.back()));
}
...
TEST_F(MultiOriginWatermarkProcessorTest, concurrentWatermarkUpdaterTest) {
  ...
  auto watermarkManager =
      MultiOriginWatermarkProcessor::create({INVALID_ORIGIN_ID});
  ...
  for (int threadId = 0; threadId < threadsCount; threadId++) {
    threads.emplace_back(thread([&watermarkManager, &watermarkBarriers,
                                 &globalUpdateCounter]() {
...
TEST_F(MultiOriginWatermarkProcessorTest,
       singleThreadWatermarkUpdaterMultipleOriginsTest) {
  auto updates = 10000_u64;
  auto origins = 10;
  auto watermarkManager = MultiOriginWatermarkProcessor::create(
      {INVALID_ORIGIN_ID, OriginId(1), OriginId(2), OriginId(3), OriginId(4),
       OriginId(5), OriginId(6), OriginId(7), OriginId(8), OriginId(9)});
  // preallocate watermarks for each transaction
  std::vector<std::tuple<WatermarkTs, SequenceData, OriginId>>
      watermarkBarriers;
  for (auto i = 1_u64; i <= updates; i++) {
