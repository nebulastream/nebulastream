...
class AppendToSliceStoreActionTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("AppendToSliceStoreActionTest.log",
  ...
  std::shared_ptr<NonKeyedSlice> createNonKeyedSlice(size_t start, size_t end,
                                                     int64_t value) {
  ...
  std::shared_ptr<KeyedSlice> createKeyedSlice(
    ...
    auto map = std::make_unique<Interface::ChainedHashMap>(
        8, 8, 1000, std::move(allocator));
    Interface::ChainedHashMapRef ref(
        Value<MemRef>(reinterpret_cast<int8_t*>(map.get())), {integerType},
        integerType->size(), 8);

    for (uint64_t hash = 0; const auto& [k, v] : values) {
      std::memcpy(
          ref.insert(Value<UInt64>(hash), {k}).getValuePtr().getValue().value,
          std::addressof(v), sizeof(v));
      hash++;
    }
...
TEST_F(AppendToSliceStoreActionTest, NonKeyedSlice) {
  ...
  auto handler =
      std::make_shared<AppendToSliceStoreHandler<NonKeyedSlice>>(600, 200);
  ...
  auto emitSlice = [&action, &context](auto slice) {
    ...
    action.emitSlice(context, child, start, end, seq, chunk, lastChunk,
                     combinedSlice);
  };
...

  {
    ...
    auto task =
        reinterpret_cast<SliceMergeTask<NonKeyedSlice>*>(buffer.getBuffer());
...
TEST_F(AppendToSliceStoreActionTest, KeyedSlice) {
  ...
  using namespace std::literals;
  auto handler =
      std::make_shared<AppendToSliceStoreHandler<KeyedSlice>>(600, 200);
  ...
  auto emitSlice = [&action, &ctx](auto slice) {
    ...
    Value<Boolean> lastChunk(true);
    Value<MemRef> combinedSlice(reinterpret_cast<int8_t*>(slice.get()));
    action.emitSlice(ctx, child, start, end, seq, chunk, lastChunk,
                     combinedSlice);
  };
...

  {
    ...
    auto buffer = pipelineContext.buffers[0];
    auto task =
        reinterpret_cast<SliceMergeTask<KeyedSlice>*>(buffer.getBuffer());
