...
class KeyedSlicePreAggregationTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedSlicePreAggregationTest.log",
  ...
  void emitWatermark(const KeyedSlicePreAggregation& slicePreAggregation,
                     ExecutionContext& context, uint64_t wts, uint64_t originId,
  ...
  void emitRecord(const KeyedSlicePreAggregation& slicePreAggregation,
...
TEST_F(KeyedSlicePreAggregationTest, aggregate) {
  ...
  auto slicePreAggregation = KeyedSlicePreAggregation(
      ...
      {std::make_shared<Aggregation::SumAggregationFunction>(
          integerType, integerType, readV1, "sum")},
      std::make_unique<Nautilus::Interface::MurMur3HashFunction>());

  std::vector<OriginId> origins = {INVALID_ORIGIN_ID};
  auto handler =
      std::make_shared<KeyedSlicePreAggregationHandler>(10, 10, origins);
  ...
  emitRecord(slicePreAggregation, ctx,
             Record({{"ts", 11_u64}, {"k1", +11_s64}, {"v1", +2_s64}}));
  emitRecord(slicePreAggregation, ctx,
             Record({{"ts", 12_u64}, {"k1", +12_s64}, {"v1", +42_s64}}));
  emitRecord(slicePreAggregation, ctx,
             Record({{"ts", 12_u64}, {"k1", +11_s64}, {"v1", +3_s64}}));
  ...
  auto smt = reinterpret_cast<SliceMergeTask<KeyedSlice>*>(
