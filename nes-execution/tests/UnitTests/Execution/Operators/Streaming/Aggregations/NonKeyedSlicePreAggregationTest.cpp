...
class NonKeyedSlicePreAggregationTest : public testing::Test {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NonKeyedSlicePreAggregationTest.log",
                              NES::LogLevel::LOG_DEBUG);
    std::cout << "Setup NonKeyedSlicePreAggregationTest test class."
              << std::endl;
  }

  /* Will be called before a test is executed. */
  void SetUp() override {
    std::cout << "Setup NonKeyedSlicePreAggregationTest test case."
              << std::endl;
  ...
  void TearDown() override {
    std::cout << "Tear down NonKeyedSlicePreAggregationTest test case."
              << std::endl;
  }

  /* Will be called after all tests in this class are finished. */
  static void TearDownTestCase() {
    std::cout << "Tear down NonKeyedSlicePreAggregationTest test class."
              << std::endl;
  }

  void emitWatermark(const NonKeyedSlicePreAggregation& slicePreAggregation,
                     ExecutionContext& context, uint64_t wts, OriginId originId,
                     uint64_t sequenceNumber) {
    auto buffer = bufferManager->getBufferBlocking();
    buffer.setWatermark(wts);
    buffer.setOriginId(originId);
    buffer.setSequenceNumber(sequenceNumber);
    auto rb = RecordBuffer(
        Value<MemRef>(reinterpret_cast<int8_t*>(std::addressof(buffer))));
    context.setWatermarkTs(wts);
    context.setOrigin(originId.getRawValue());
    slicePreAggregation.close(context, rb);
  }

  void emitRecord(const NonKeyedSlicePreAggregation& slicePreAggregation,
                  ExecutionContext& ctx, Record record) {
...
TEST_F(NonKeyedSlicePreAggregationTest, performAggregation) {
  ...
  auto slicePreAggregation = NonKeyedSlicePreAggregation(
      ...
      {std::make_shared<Aggregation::CountAggregationFunction>(
          integerType, unsignedIntegerType, readF2, "count")});

  std::vector<OriginId> origins = {INVALID_ORIGIN_ID};
  auto handler =
      std::make_shared<NonKeyedSlicePreAggregationHandler>(10, 10, origins);
  auto pipelineContext = MockedPipelineExecutionContext({handler});

  auto context = ExecutionContext(
      Value<MemRef>(reinterpret_cast<int8_t*>(workerContext.get())),
      Value<MemRef>(reinterpret_cast<int8_t*>(&pipelineContext)));
  auto buffer = bufferManager->getBufferBlocking();

  auto recordBuffer = RecordBuffer(
      Value<MemRef>(reinterpret_cast<int8_t*>(std::addressof(buffer))));
  ...
  emitRecord(slicePreAggregation, context,
             Record({{"f1", 12_u64}, {"f2", +42_s64}}));
  emitRecord(slicePreAggregation, context,
             Record({{"f1", 12_u64}, {"f2", +42_s64}}));
  ...
  auto value =
      static_cast<uint64_t*>(stateStore->getFirstSlice()->getState()->ptr);
  ASSERT_EQ(*value, 2);

  emitRecord(slicePreAggregation, context,
             Record({{"f1", 24_u64}, {"f2", +42_s64}}));
  ...
  auto sliceMergeTask = reinterpret_cast<SliceMergeTask<NonKeyedSlice>*>(
      pipelineContext.buffers[0].getBuffer());
  ASSERT_EQ(sliceMergeTask->startSlice, 10);
  ASSERT_EQ(sliceMergeTask->endSlice, 20);
  ASSERT_EQ(sliceMergeTask->sequenceNumber,
            TupleBuffer::INITIAL_SEQUENCE_NUMBER);
...
TEST_F(NonKeyedSlicePreAggregationTest, performMultipleAggregation) {
  ...
  auto slicePreAggregation = NonKeyedSlicePreAggregation(
      ...
      std::make_unique<EventTimeFunction>(readTs,
                                          Windowing::TimeUnit::Milliseconds()),
      {std::make_shared<Aggregation::SumAggregationFunction>(i64, i64, readF2,
                                                             "sum"),
       std::make_shared<Aggregation::CountAggregationFunction>(
           ui64, ui64, readF2, "count")});
  ...
  auto context = ExecutionContext(Value<MemRef>((int8_t*)workerContext.get()),
                                  Value<MemRef>((int8_t*)&pipelineContext));
  auto buffer = bufferManager->getBufferBlocking();

  auto recordBuffer =
      RecordBuffer(Value<MemRef>((int8_t*)std::addressof(buffer)));
  slicePreAggregation.setup(context);
  auto stateStore = handler->getThreadLocalSliceStore(workerContext->getId());
  ...
  auto value =
      static_cast<State*>(stateStore->getFirstSlice()->getState()->ptr);
