...
class NLJBuildPipelineExecutionContext : public PipelineExecutionContext {
 public:
  NLJBuildPipelineExecutionContext(OperatorHandlerPtr nljOperatorHandler,
                                   BufferManagerPtr bm)
      : PipelineExecutionContext(
            INVALID_PIPELINE_ID,               // mock pipeline id
            INVALID_DECOMPOSED_QUERY_PLAN_ID,  // mock query id
            bm, 1, [](TupleBuffer&, Runtime::WorkerContextRef) {},
            [](TupleBuffer&) {}, {nljOperatorHandler}) {}
};

class NLJProbePipelineExecutionContext : public PipelineExecutionContext {
 public:
  std::vector<TupleBuffer> emittedBuffers;
  NLJProbePipelineExecutionContext(OperatorHandlerPtr nljOperatorHandler,
                                   BufferManagerPtr bm)
      : PipelineExecutionContext(
            INVALID_PIPELINE_ID,               // mock pipeline id
            INVALID_DECOMPOSED_QUERY_PLAN_ID,  // mock query id
...
class NestedLoopJoinOperatorTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NestedLoopJoinOperatorTest.log",
  ...
  void SetUp() override {
    ...
    leftSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
    ...
    nljOperatorHandler = Operators::NLJOperatorHandlerSlicing::create(
        {INVALID_ORIGIN_ID}, OriginId(1), windowSize, windowSize, leftSchema,
  ...
  std::vector<Record> createRandomRecords(
      ...
      QueryCompilation::JoinBuildSideType joinBuildSide, uint64_t minValue = 0,
      uint64_t maxValue = 1000, uint64_t randomSeed = 42) {
    std::vector<Record> retVector;
    std::mt19937 generator(randomSeed);
    std::uniform_int_distribution<uint64_t> distribution(minValue, maxValue);
    auto schema = joinBuildSide == QueryCompilation::JoinBuildSideType::Left
                      ? leftSchema
                      : rightSchema;
    ...
    for (auto i = 0_u64; i < numberOfRecords; ++i) {
      retVector.emplace_back(
          Record({{firstSchemaField, Value<UInt64>(i)},
  ...
  /**
   * @brief this method returns the number of tuples in the given window,
   * presuming that all windows except for the last one contains windowSize many
   * tuples
   * @param totalTuples
  ...
  /**
   * @brief checks for every record in the given window that each field entry
   * equals the corresponding entry in allRecords
   * @param windowIdentifier
  ...
  void checkRecordsInBuild(uint64_t windowIdentifier,
                           ...
                           std::vector<Record>& allRecords, SchemaPtr& schema) {
    Nautilus::Value<UInt64> zeroVal(0_u64);
    Nautilus::Interface::PagedVectorVarSizedRef pagedVector(pagedVectorRef,
                                                            schema);
    ...
    for (auto pos = windowStartPos; pos < windowEndPos; ++pos, ++posInWindow) {
      ...
      NES_TRACE("readRecord {} record{}", record.toString(),
                expectedRecord.toString());

      for (auto& field : schema->fields) {
        EXPECT_EQ(record.read(field->getName()),
                  expectedRecord.read(field->getName()));
      }
    }
  }

  /**
   * @brief checks for every window that it contains the expected number of
   * tuples and calls checkRecordsInBuild()
   * @param maxTimestamp
   * @param allLeftRecords
   * @param allRightRecords
   */
  void checkWindowsInBuild(uint64_t maxTimestamp,
    ...
    auto memoryProviderLeft =
        MemoryProvider::MemoryProvider::createMemoryProvider(
            leftSchema->getSchemaSizeInBytes(), leftSchema);
    auto memoryProviderRight =
        MemoryProvider::MemoryProvider::createMemoryProvider(
            rightSchema->getSchemaSizeInBytes(), rightSchema);

    auto maxWindowIdentifier =
        std::ceil((double)maxTimestamp / windowSize) * windowSize;
    for (auto windowIdentifier = windowSize;
         windowIdentifier < maxWindowIdentifier;
         windowIdentifier += windowSize) {
      auto expectedNumberOfTuplesInWindowLeft =
          calculateExpNoTuplesInWindow(numberOfRecordsLeft, windowIdentifier);
      auto expectedNumberOfTuplesInWindowRight =
          calculateExpNoTuplesInWindow(numberOfRecordsRight, windowIdentifier);

      NES_DEBUG("Check window={}", windowIdentifier);
      ASSERT_EQ(
          nljOperatorHandler->getNumberOfTuplesInSlice(
      ...
      auto nljWindow = std::dynamic_pointer_cast<NLJSlice>(
          nljOperatorHandler->getSliceBySliceIdentifier(windowIdentifier)
              .value());
      Nautilus::Value<UInt64> zeroVal(static_cast<uint64_t>(0));

      auto leftPagedVectorRef =
          Nautilus::Value<Nautilus::MemRef>(static_cast<int8_t*>(
              nljWindow->getPagedVectorRefLeft(INITIAL<WorkerThreadId>)));
      checkRecordsInBuild(windowIdentifier, leftPagedVectorRef,
                          expectedNumberOfTuplesInWindowLeft, allLeftRecords,
                          leftSchema);

      auto rightPagedVectorRef =
          Nautilus::Value<Nautilus::MemRef>(static_cast<int8_t*>(
              nljWindow->getPagedVectorRefRight(INITIAL<WorkerThreadId>)));
      checkRecordsInBuild(windowIdentifier, rightPagedVectorRef,
                          expectedNumberOfTuplesInWindowRight, allRightRecords,
                          rightSchema);
    }
  }

  /**
   * @brief sets up and opens NLJBuild for left and right side, executes it for
   * every record and calls checkWindowsInBuild()
   * @param numberOfRecordsLeft
   * @param numberOfRecordsRight
   */
  void insertRecordsIntoBuild(uint64_t numberOfRecordsLeft,
    ...
    WorkerContextPtr workerContext =
        std::make_shared<WorkerContext>(INITIAL<WorkerThreadId>, bm, 100);
    ...
    // We do not care for the record buffer in the current NLJBuild::open()
    // implementation
    ...
    auto allLeftRecords = createRandomRecords(
        numberOfRecordsLeft, QueryCompilation::JoinBuildSideType::Left);
    auto allRightRecords = createRandomRecords(
        numberOfRecordsRight, QueryCompilation::JoinBuildSideType::Right);
    uint64_t maxTimestamp = 2;

    for (auto& leftRecord : allLeftRecords) {
      maxTimestamp = std::max(leftRecord.read(timestampFieldNameLeft)
                                  .getValue()
                                  .staticCast<UInt64>()
                                  .getValue(),
                              maxTimestamp);
    ...
    for (auto& rightRecord : allRightRecords) {
      maxTimestamp = std::max(rightRecord.read(timestampFieldNameRight)
                                  .getValue()
                                  .staticCast<UInt64>()
                                  .getValue(),
                              maxTimestamp);
  ...
  /**
   * @brief checks for every record in the given window that each joined record
   * is in the emitted records
   * @param allLeftRecords
   * @param allRightRecords
   * @param windowIdentifier
   * @param joinSchema
  ...
  void checkRecordsInProbe(std::vector<Record>& allLeftRecords,
    ...
    NES_DEBUG("Checking records in Probe now for left {} right {}...",
              allLeftRecords.size(), allRightRecords.size());
    for (auto& leftRecord : allLeftRecords) {
      for (auto& rightRecord : allRightRecords) {
        ...
        auto timestampLeftVal = leftRecord.read(timestampFieldNameLeft)
                                    .getValue()
                                    .staticCast<UInt64>()
                                    .getValue();
        auto timestampRightVal = rightRecord.read(timestampFieldNameRight)
                                     .getValue()
                                     .staticCast<UInt64>()
                                     .getValue();
        Record joinedRecord;
  ...
  /**
   * @brief checks for every window that it contains the expected number of
   * tuples and calls checkRecordsInProbe()
   * @param maxTimestamp
   * @param allLeftRecords
   * @param allRightRecords
   */
  void checkWindowsInProbe(uint64_t maxTimestamp,
                           std::vector<Record>& allLeftRecords,
                           std::vector<Record>& allRightRecords) {
    ...
    auto nljProbe = std::make_shared<Operators::NLJProbe>(
        handlerIndex, joinSchema, joinExpression, windowMetaData, leftSchema,
        rightSchema, QueryCompilation::StreamJoinStrategy::NESTED_LOOP_JOIN,
        QueryCompilation::WindowingStrategy::SLICING);
    ...
    for (auto windowIdentifier = windowSize;
      ...
      auto nljOpHandler =
          std::dynamic_pointer_cast<Operators::NLJOperatorHandlerSlicing>(
              nljOperatorHandler);
      NES_DEBUG("Check window={}", windowIdentifier);
      ASSERT_EQ(
          nljOpHandler->getNumberOfTuplesInSlice(
              windowIdentifier, QueryCompilation::JoinBuildSideType::Left),
          expectedNumberOfTuplesInWindowLeft);
      ASSERT_EQ(
          nljOpHandler->getNumberOfTuplesInSlice(
...

      {
        ...
        auto bufferMemory =
            tupleBuffer.getBuffer<Operators::EmittedNLJWindowTriggerTask>();
        bufferMemory->leftSliceIdentifier = windowIdentifier;
        bufferMemory->rightSliceIdentifier = windowIdentifier;
        bufferMemory->windowInfo =
            WindowInfo(windowIdentifier - windowSize, windowIdentifier);
      ...
      checkRecordsInProbe(allLeftRecords, allRightRecords, windowIdentifier,
                          joinSchema.joinSchema, collector);
    }
  }

  /**
   * @brief writes every record to the corresponding window's PagedVector and
   * calls checkWindowsInProbe()
   * @param numberOfRecordsLeft
   * @param numberOfRecordsRight
   */
  void insertRecordsIntoProbe(uint64_t numberOfRecordsLeft,
                              uint64_t numberOfRecordsRight) {
    auto allLeftRecords = createRandomRecords(
        numberOfRecordsLeft, QueryCompilation::JoinBuildSideType::Left);
    auto allRightRecords = createRandomRecords(
    ...
    auto memoryProviderLeft =
        MemoryProvider::MemoryProvider::createMemoryProvider(
            bm->getBufferSize(), leftSchema);
    auto memoryProviderRight =
        MemoryProvider::MemoryProvider::createMemoryProvider(
            bm->getBufferSize(), rightSchema);
    nljOperatorHandler->setBufferManager(bm);

    uint64_t maxTimestamp = 0;
    Value<UInt64> zeroVal(0_u64);
    for (auto& leftRecord : allLeftRecords) {
      auto timestamp = leftRecord.read(timestampFieldNameLeft)
                           .getValue()
                           .staticCast<UInt64>()
                           .getValue();
      ...
      auto nljOpHandler =
          std::dynamic_pointer_cast<Operators::NLJOperatorHandlerSlicing>(
              nljOperatorHandler);
      auto nljWindow = std::dynamic_pointer_cast<NLJSlice>(
          nljOpHandler->getSliceByTimestampOrCreateIt(timestamp));
      auto leftPagedVectorRef = Nautilus::Value<Nautilus::MemRef>(
          (int8_t*)nljWindow->getPagedVectorRefLeft(INITIAL<WorkerThreadId>));
    ...
    for (auto& rightRecord : allRightRecords) {
      auto timestamp = rightRecord.read(timestampFieldNameRight)
                           .getValue()
                           .staticCast<UInt64>()
                           .getValue();
      maxTimestamp = std::max(timestamp, maxTimestamp);
      ...
      auto rightPagedVectorRef = Nautilus::Value<Nautilus::MemRef>(
          (int8_t*)nljWindow->getPagedVectorRefRight(INITIAL<WorkerThreadId>));
    ...
    NES_INFO("left: {} right: {}", allLeftRecords.size(),
             allRightRecords.size());
...
TEST_F(NestedLoopJoinOperatorTest,
       joinProbeSimpleTestOneWindowMulipleExpressions) {
  const auto numberOfRecordsLeft = 250;
  const auto numberOfRecordsRight = 250;

  auto onLeftKey =
      std::make_shared<Expressions::ReadFieldExpression>("test1$value");
  auto onRightKey =
      std::make_shared<Expressions::ReadFieldExpression>("test2$value");
  auto expression = std::make_shared<Expressions::GreaterThanExpression>(
      onLeftKey, onRightKey);
  joinExpression =
      std::make_shared<Expressions::AndExpression>(joinExpression, expression);
