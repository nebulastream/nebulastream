...
class HashJoinOperatorVarSizedTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("HashJoinOperatorVarSizedTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down HashJoinOperatorVarSizedTest test class.");
  }
};
...
bool hashJoinBuildAndCheck(HashJoinBuildHelper buildHelper) {
  ...
  auto pipelineContext = PipelineExecutionContext(
      ...
      buildHelper.bufferManager, buildHelper.noWorkerThreads,
  ...
  auto executionContext = ExecutionContext(
      Nautilus::Value<MemRef>(reinterpret_cast<int8_t*>(workerContext.get())),
      Value<MemRef>(reinterpret_cast<int8_t*>(&pipelineContext)));

  hashJoinOpHandler->start(
      std::make_shared<PipelineExecutionContext>(pipelineContext), 0);
  ...
  for (auto i = 0UL; i < buildHelper.numberOfTuplesToProduce + 1; ++i) {
    ...
    if (i == 0) {
      ...
      RecordBuffer recordBuffer = RecordBuffer(Value<MemRef>(
          reinterpret_cast<int8_t*>(std::addressof(tupleBuffer))));
    ...
    auto hashTable = hashWindow->getHashTable(buildHelper.joinBuildSide,
                                              workerContext->getId());
    auto pagedVectorRef = Nautilus::Value<MemRef>(
        reinterpret_cast<int8_t*>(hashTable->insert(joinKey).get()));
    Interface::PagedVectorVarSizedRef pagedVector(pagedVectorRef,
                                                  buildHelper.schema);
...
bool hashJoinProbeAndCheck(HashJoinProbeHelper hashJoinProbeHelper) {
  ...
  for (auto i = 0UL; i < hashJoinProbeHelper.numberOfTuplesToProduce + 1; ++i) {
    auto recordLeft =
        Record({{hashJoinProbeHelper.leftSchema->get(0)->getName(),
    ...
    auto recordRight =
        Record({{hashJoinProbeHelper.rightSchema->get(0)->getName(),
  ...
  for (auto i = 0UL; i < leftRecords.size(); i++) {
    ...
    RecordBuffer recordBufferLeft = RecordBuffer(
        Value<MemRef>(reinterpret_cast<int8_t*>(std::addressof(tupleBuffer))));
    if (i == 0) {
  ...
  for (auto i = 0UL; i < rightRecords.size(); i++) {
    auto tupleBuffer = hashJoinProbeHelper.bufferManager->getBufferBlocking();
    RecordBuffer recordBufferRight = RecordBuffer(
        Value<MemRef>(reinterpret_cast<int8_t*>(std::addressof(tupleBuffer))));
    if (i == 0) {
