...
bool hashJoinBuildAndCheck(HashJoinBuildHelper buildHelper) {
    ...
    auto executionContext = ExecutionContext(Nautilus::MemRef(reinterpret_cast<int8_t*>(workerContext.get())),
    ...
    for (auto i = 0UL; i < buildHelper.numberOfTuplesToProduce + 1; ++i) {
        auto record = Record({{buildHelper.schema->get(0)->getName(), UInt64((uint64_t) i)},
        ...
        if (i == 0) {
            ...
            RecordBuffer recordBuffer = RecordBuffer(MemRef(reinterpret_cast<int8_t*>(std::addressof(tupleBuffer))));
        ...
        auto pagedVectorRef = Nautilus::MemRef(reinterpret_cast<int8_t*>(hashTable->insert(joinKey).get()));
...
bool hashJoinProbeAndCheck(HashJoinProbeHelper hashJoinProbeHelper) {
    ...
    auto executionContext = ExecutionContext(MemRef(reinterpret_cast<int8_t*>(workerContext.get())),
                                             MemRef(reinterpret_cast<int8_t*>(&pipelineContext)));
    ...
    for (auto i = 0UL; i < hashJoinProbeHelper.numberOfTuplesToProduce + 1; ++i) {
        auto recordLeft = Record({{hashJoinProbeHelper.leftSchema->get(0)->getName(), UInt64((uint64_t) i)},
        ...
        auto recordRight = Record({{hashJoinProbeHelper.rightSchema->get(0)->getName(), UInt64((uint64_t) i + 1000)},
    ...
    for (auto i = 0UL; i < leftRecords.size(); i++) {
        ...
        RecordBuffer recordBufferLeft = RecordBuffer(MemRef(reinterpret_cast<int8_t*>(std::addressof(tupleBuffer))));
        if (i == 0) {
    ...
    for (auto i = 0UL; i < rightRecords.size(); i++) {
        auto tupleBuffer = hashJoinProbeHelper.bufferManager->getBufferBlocking();
        RecordBuffer recordBufferRight = RecordBuffer(MemRef(reinterpret_cast<int8_t*>(std::addressof(tupleBuffer))));
        if (i == 0) {
