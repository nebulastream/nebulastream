...
public:
  ...
  std::vector<
      std::map<uint32_t, std::unique_ptr<Aggregation::AggregationValue>>>
      aggValues;
  /* Will be called before any test in this class are executed. */
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ThresholdWindowOperatorTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down ThresholdWindowOperatorTest test class.");
  }
};
...
TEST_F(KeyedThresholdWindowOperatorTest, thresholdWindowWithSumAggTest) {
  ...
  auto fortyTwo =
      std::make_shared<Expressions::ConstantInt32ValueExpression>(42);
  // Attribute(f1) > 42, sum(f2)
  auto greaterThanExpression =
      std::make_shared<Expressions::GreaterThanExpression>(readF1, fortyTwo);
  ...
  auto thresholdWindowOperator = std::make_shared<KeyedThresholdWindow>(
      greaterThanExpression, 0, aggFieldAccessExpressionsVector, readKey,
      keyFieldName, resultFieldVector, aggVector, 0);
  ...
  auto recordTwenty = Record(
      {{"f1", +20_s64}, {"k1", +0_s64}, {"f2", +4_s64}}); // closes the window
...
TEST_F(KeyedThresholdWindowOperatorTest, thresholdWindowWithMaxAggTest) {
  ...
  auto sumAgg = std::make_shared<Aggregation::MaxAggregationFunction>(
      integerType, integerType, readF2, aggregationResultFieldName);
  resultFieldVector.emplace_back(aggregationResultFieldName);
  aggFieldAccessExpressionsVector.push_back(readF2);
  aggVector.push_back(sumAgg);
...
/**
 * @brief Tests the keyed threshold window operator with a sum aggregation on
 * rows having different keys.
 */
TEST_F(KeyedThresholdWindowOperatorTest,
       thresholdWindowWithSumTestDifferentKey) {
  auto readF1 = std::make_shared<Expressions::ReadFieldExpression>("f1");
  auto readF2 = std::make_shared<Expressions::ReadFieldExpression>("f2");
  auto readKey = std::make_shared<Expressions::ReadFieldExpression>("k1");
  auto fortyTwo =
  ...
  auto recordTenKey0 =
      Record({{"f1", +10_s64}, {"k1", +0_s64}, {"f2", +1_s64}});
  auto recordTenKey1 =
      Record({{"f1", +10_s64}, {"k1", +1_s64}, {"f2", +1_s64}});
  thresholdWindowOperator->execute(ctx, recordTenKey0);
  thresholdWindowOperator->execute(ctx, recordTenKey1);
  EXPECT_EQ(collector->records.size(), 0);

  auto recordFiftyKey0 =
      Record({{"f1", +50_s64}, {"k1", +0_s64}, {"f2", +2_s64}});
  auto recordNinetyKey0 =
      Record({{"f1", +90_s64}, {"k1", +0_s64}, {"f2", +3_s64}});
  auto recordTwentyKey0 = Record(
      {{"f1", +20_s64}, {"k1", +0_s64}, {"f2", +4_s64}}); // closes the window
  auto recordFiftyKey1 =
      Record({{"f1", +50_s64}, {"k1", +1_s64}, {"f2", +7_s64}});
  auto recordNinetyKey1 =
      Record({{"f1", +90_s64}, {"k1", +1_s64}, {"f2", +5_s64}});
  auto recordTwentyKey1 = Record(
      {{"f1", +20_s64}, {"k1", +1_s64}, {"f2", +4_s64}}); // closes the window
...
/**
 * @brief Tests the keyed threshold window operator with a sum and a max
 * aggregation on rows having different keys.
 */
TEST_F(KeyedThresholdWindowOperatorTest,
       thresholdWindowWithMultAggTestDifferentKey) {
  auto readF1 = std::make_shared<Expressions::ReadFieldExpression>("f1");
  auto readF2 = std::make_shared<Expressions::ReadFieldExpression>("f2");
  auto readKey = std::make_shared<Expressions::ReadFieldExpression>("k1");
  auto fortyTwo =
  ...
  auto sumAgg = std::make_shared<Aggregation::SumAggregationFunction>(
      integerType, integerType, readF2, sumAggregationResultFieldName);

  auto maxAggregationResultFieldName = "max";
  auto maxAgg = std::make_shared<Aggregation::MaxAggregationFunction>(
      integerType, integerType, readF2, maxAggregationResultFieldName);
