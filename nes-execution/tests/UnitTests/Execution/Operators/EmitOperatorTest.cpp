...
public:
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down EmitOperatorTest test class.");
  }
};
...
TEST_F(EmitOperatorTest, emitRecordsToRowBuffer) {
  ...
  auto ctx = ExecutionContext(Value<MemRef>((int8_t *)wc.get()),
  ...
  auto testBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(rowMemoryLayout, buffer);
  for (uint64_t i = 0; i < rowMemoryLayout->getCapacity(); i++) {
...
TEST_F(EmitOperatorTest, emitRecordsToRowBufferWithOverflow) {
  ...
  for (uint64_t i = 0; i < rowMemoryLayout->getCapacity() * 2; i++) {
    auto record = Record({{"f1", Value<>(i)}, {"f2", Value<>(10)}});
    emitOperator.execute(ctx, record);
    // If we have filled a buffer, we have to increase the sequence number for
    // now. This should be fixed with issue #4343
...
TEST_F(EmitOperatorTest, emitRecordsToColumnBuffer) {
  ...
  auto columnMemoryLayout =
      Runtime::MemoryLayouts::ColumnLayout::create(schema, bm->getBufferSize());
  ...
  auto testBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(columnMemoryLayout, buffer);
  for (uint64_t i = 0; i < columnMemoryLayout->getCapacity(); i++) {
    EXPECT_EQ(testBuffer[i]["f1"].read<int64_t>(), i);
  }
}
