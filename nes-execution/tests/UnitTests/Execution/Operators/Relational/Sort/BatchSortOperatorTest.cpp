...
class BatchSortOperatorTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BatchSortOperatorTest.log",
...
using TestTypes =
    ::testing::Types<std::pair<uint32_t, uint32_t>, std::pair<int32_t, int32_t>,
                     std::pair<uint64_t, uint64_t>, std::pair<int64_t, int64_t>,
                     std::pair<uint16_t, uint16_t>, std::pair<int16_t, int16_t>,
                     std::pair<uint8_t, uint8_t>, std::pair<int8_t, int8_t>,
                     std::pair<float, uint32_t>, std::pair<double, uint64_t>>;
...
TYPED_TEST(BatchSortOperatorTest, SortOperatorMultipleFieldsTest) {
  ...
  for (size_t i = 0; i < NUM_RECORDS; i++) {
    ...
    ASSERT_EQ(*(reinterpret_cast<EncodedType*>(entry)),
              encodeData(static_cast<Type>(i * 10)));  // f1 encoded value
    ASSERT_EQ(*(reinterpret_cast<Type*>(entry) + 1),
              static_cast<Type>(i * 10));  // f1 original value
    ASSERT_EQ(*(reinterpret_cast<Type*>(entry) + 2),
              static_cast<Type>(i));  // f2 original value
  }
}

/**
 * @brief Tests if the sort operator collects records for sorting on the second
 * field
 */
TYPED_TEST(BatchSortOperatorTest, SortOperatorOnSecondColumnTest) {
  ...
  for (size_t i = 0; i < NUM_RECORDS; i++) {
    auto entry = state->getEntry(i);
    ASSERT_EQ(*(reinterpret_cast<EncodedType*>(entry)),
              encodeData(static_cast<Type>(i)));  // f2 encoded value
    ASSERT_EQ(*(reinterpret_cast<Type*>(entry) + 1),
              static_cast<Type>(i * 10));  // f1 original value
    ASSERT_EQ(*(reinterpret_cast<Type*>(entry) + 2),
              static_cast<Type>(i));  // f2 original value
...
TYPED_TEST(BatchSortOperatorTest, SortOperatorMuliplePagesTest) {
  using Type = typename TypeParam::first_type;
  using EncodedType = typename TypeParam::second_type;
  constexpr auto NUM_RECORDS = 1025;  // 1025 * 2 (Fields) * 2 Bytes = 4100
                                      // Bytes > 4096 Bytes (Page Size)
  ...
  auto pipelineContext = MockedPipelineExecutionContext({handler});
  auto sortOperator =
      BatchSort(0, {integerType, integerType}, {"f1", "f2"}, {"f1"});
  auto collector = std::make_shared<CollectOperator>();
  sortOperator.setChild(collector);
