...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BatchSortScanOperatorTest.log",
...
void fillState(const std::shared_ptr<BatchSortOperatorHandler> &handler,
               const uint64_t numberOfRecord, const uint64_t numberOfFields,
  ...
  auto entrySize =
      sizeof(T) * (numberOfFields + sortFieldIndexes.size() /*sort cols*/);
  auto stateRef = Nautilus::Interface::PagedVectorRef(
      Value<MemRef>((int8_t *)state), entrySize);
  ...
  for (size_t j = 0; j < numberOfRecord; ++j) {
    ...
    for (size_t i = 0; i < sortFieldIndexes.size(); ++i) {
      Value<> encodedVal(
          encodeData<T>(recordValues[sortFieldIndexes[i]], descending));
...
using TestTypes =
    ::testing::Types<std::pair<uint64_t, UInt64>, std::pair<int64_t, Int64>,
                     std::pair<uint32_t, UInt32>, std::pair<int32_t, Int32>,
                     std::pair<uint16_t, UInt16>, std::pair<int16_t, Int16>,
                     std::pair<uint8_t, UInt8>, std::pair<int8_t, Int8>,
...
TYPED_TEST(BatchSortScanOperatorTest, SortOperatorTest) {
  ...
  fillState<NativeType>(handler, NUM_RECORDS, NUM_FIELDS, {0},
                        /* descending */ false);

  auto sortScanOperator = BatchSortScan(
      0, {Util::getPhysicalTypePtr<NativeType>()}, {"f1"}, {"f1"});
  ...
  for (int i = 1; i < NUM_RECORDS; ++i) {
    Value<NautilusType> cur =
        collector->records[i].read("f1").as<NautilusType>();
...
/**
 * @brief Tests if the sort operator sorts correctly when the sort field is not
 * the first column
 */
TYPED_TEST(BatchSortScanOperatorTest, SortOperatorOnSecondColumnTest) {
  ...
  auto pipelineContext = MockedPipelineExecutionContext({handler});
  handler->setup(pipelineContext);
  fillState<NativeType>(handler, NUM_RECORDS, NUM_FIELDS, {1},
                        /* descending */ false);

  auto sortScanOperator =
      BatchSortScan(0, {pType, pType}, {"f1", "f2"}, {"f2"});
  auto collector = std::make_shared<CollectOperator>();
  sortScanOperator.setChild(collector);
  auto ctx = ExecutionContext(Value<MemRef>(nullptr),
                              Value<MemRef>((int8_t *)&pipelineContext));
  ...
  for (int i = 1; i < NUM_RECORDS; ++i) {
    Value<NautilusType> cur =
        collector->records[i].read("f2").as<NautilusType>();
    ASSERT_LE(prev, cur);
    prev = cur;
  }
}
...
/**
 * @brief Tests if the sort operator sorts correctly when there exist multiple
 * fields
 */
TYPED_TEST(BatchSortScanOperatorTest, SortOperatorOnMultipleColumnsTest) {
  ...
  auto pipelineContext = MockedPipelineExecutionContext({handler});
  handler->setup(pipelineContext);
  fillState<NativeType>(handler, NUM_RECORDS, NUM_FIELDS, {1, 0},
                        /* descending */ false, 0, 5);

  auto sortScanOperator =
      BatchSortScan(0, {pType, pType}, {"f1", "f2"}, {"f2", "f1"});
  auto collector = std::make_shared<CollectOperator>();
  sortScanOperator.setChild(collector);
  auto ctx = ExecutionContext(Value<MemRef>(nullptr),
                              Value<MemRef>((int8_t *)&pipelineContext));
  ...
  for (int i = 1; i < NUM_RECORDS; ++i) {
    Value<NautilusType> cur1 =
        collector->records[i].read("f1").as<NautilusType>();
    Value<NautilusType> cur2 =
        collector->records[i].read("f2").as<NautilusType>();
...
TYPED_TEST(BatchSortScanOperatorTest, SortOperatorTestMultiPage) {
  ...
  constexpr int NUM_RECORDS =
      Interface::PagedVector::PAGE_SIZE / sizeof(NativeType) * 2 +
      sizeof(NativeType); // 2 full pages + 1 record -> 3 Pages
  constexpr auto NUM_FIELDS = 1;
