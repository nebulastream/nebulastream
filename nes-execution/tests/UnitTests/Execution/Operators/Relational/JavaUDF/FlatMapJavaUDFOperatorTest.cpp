...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("FlatMapJavaUDFOperatorTest.log",
                              NES::LogLevel::LOG_DEBUG);
  }

  auto setupAndExecuteFlatMapUdf(
...
/**
 * @brief Test flatmap UDF with string objects as input and output
 * (StringFlatMapFunction<String, String>) The UDF splits the input string and
...
TEST_F(FlatMapJavaUDFOperatorTest, StringUDFTest) {
  ...
  auto javaUDFDescriptor =
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .build();
  auto outputRecords =
      setupAndExecuteFlatMapUdf(javaUDFDescriptor, (int8_t *)&wc, inputRecord);
...
/**
 * @brief Test UDF with loaded java classes as input and output
 * (ComplexFlatMapFunction<ComplexPojo, ComplexPojo>) The UDF sets the bool to
 * false, numerics +10 and appends to strings the postfix 'appended'.
 */
TEST_F(FlatMapJavaUDFOperatorTest, ComplexPojoFlatMapFunction) {
  ...
  EXPECT_EQ(result.read("stringVariable"),
            Value<Text>("Appended String:testValue"));
