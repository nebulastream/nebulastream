...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("AddExpressionTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down TraceTest test class.");
  }
};
...
TEST_F(AggregationFunctionTest, sumAggregation) {
  ...
  auto integerType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt64());
  auto readFieldExpression =
      std::make_shared<Expressions::ReadFieldExpression>("value");
  auto sumAgg = Aggregation::SumAggregationFunction(
      integerType, integerType, readFieldExpression, "result");
...
TEST_F(AggregationFunctionTest, countAggregation) {
  ...
  auto integerType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt64());
  auto unsignedIntegerType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createUInt64());

  auto countAgg = Aggregation::CountAggregationFunction(
      integerType, unsignedIntegerType, readFieldExpression, "result");
...
TEST_F(AggregationFunctionTest, AvgAggregation) {
  auto readFieldExpression =
      std::make_shared<Expressions::ReadFieldExpression>("value");
  auto physicalDataTypeFactory = DefaultPhysicalTypeFactory();
  PhysicalTypePtr integerType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt64());
  PhysicalTypePtr doubleType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createDouble());
  auto avgAgg = Aggregation::AvgAggregationFunction(
      integerType, doubleType, readFieldExpression, "result");
...
TEST_F(AggregationFunctionTest, MinAggregation) {
  ...
  auto minAgg = Aggregation::MinAggregationFunction(
      integerType, integerType, readFieldExpression, "result");
  ...
  // lift value in minAgg with an initial value of 5, thus the current min
  // should be 5
  auto inputRecord = Record({{"value", incomingValueFive}});
  minAgg.lift(memref, inputRecord);
  ASSERT_EQ(minValue.min, incomingValueFive);

  // lift value in minAgg with a higher value of 10, thus the current min should
  // still be 5
  inputRecord = Record({{"value", incomingValueTen}});
  minAgg.lift(memref, inputRecord);
  ASSERT_EQ(minValue.min, incomingValueFive);

  // lift value in minAgg with a lower value of 1, thus the current min should
  // change to 1
  inputRecord = Record({{"value", incomingValueOne}});
  minAgg.lift(memref, inputRecord);
  ASSERT_EQ(minValue.min, incomingValueOne);

  // lift value in minAgg with a higher value of 2, thus the current min should
  // still be 1
  inputRecord = Record({{"value", incomingValueOne}});
  minAgg.lift(memref, inputRecord);
  ASSERT_EQ(minValue.min, incomingValueOne);
  ...
  auto anotherMemref =
      Nautilus::Value<Nautilus::MemRef>((int8_t *)&anotherMinValue);
  inputRecord = Record({{"value", incomingValueTen}});
...
TEST_F(AggregationFunctionTest, MaxAggregation) {
  ...
  auto maxAgg = Aggregation::MaxAggregationFunction(
      integerType, integerType, readFieldExpression, "result");
  ...
  // lift value in maxAgg with an initial value of 5, thus the current min
  // should be 5
  ...
  // lift value in maxAgg with a higher value of 10, thus the current min should
  // change to 10
  ...
  // lift value in maxAgg with a lower value of 1, thus the current min should
  // still be 10
  ...
  // lift value in maxAgg with a higher value of 15, thus the current min should
  // change to 15
  ...
  auto anotherMemref =
      Nautilus::Value<Nautilus::MemRef>((int8_t *)&anotherMaxValue);
...
/**
 * Tests the lift, combine, lower and reset functions of the HyperLogLog
 * Aggregation, i.e., a event sequence with identical value (1) that results in
 * a distinct count of 1 (ideal case).
 */
TEST_F(AggregationFunctionTest, HyperLogLogAggregationSimpleTest) {
  ...
  auto distinctCountEstimation =
      Aggregation::HyperLogLogDistinctCountApproximation(
          integerType, integerType, readFieldExpression, "result");
  auto distinctCountValue =
      Aggregation::HyperLogLogDistinctCountApproximationValue();
  auto memref =
      Nautilus::Value<Nautilus::MemRef>((int8_t *)&distinctCountValue);
  auto incomingValue = Nautilus::Value<Nautilus::Int64>(1_s64);
  ...
  EXPECT_TRUE((bool)(result.read("result") > (1.0 * 0.9) &&
                     result.read("result") < (1.0 * 1.1)));
  // HyperLogLog estimates the distinct count thus we use 10% variance of the
  // result
}

/**
 * Tests the lift, combine, lower and reset functions of the HyperLogLog
 * Aggregation, i.e., the following event sequence {4,3,6,2,2,6,1,7} that
 * results in a distinct count of 6 (ideal case).
 */
TEST_F(AggregationFunctionTest, HyperLogLogComplexTest) {
  ...
  auto result = Record();
  distinctCountEstimation.lower(memref, result);
  EXPECT_TRUE((bool)(result.read("result") > (6.0 * 0.9) &&
                     result.read("result") < (6.0 * 1.1)));
  // HyperLogLog estimates the distinct count thus we use 10% variance of the
  // result
}

/**
 * Tests the lift, lower and reset functions of the
 * QuantileEstimationAggregation, i.e., the following event sequence
...
TEST_F(AggregationFunctionTest, scanEmitPipelineQuantile) {
  ...
  auto integerType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createInt64());
  auto outputType =
      physicalDataTypeFactory.getPhysicalType(DataTypeFactory::createFloat());

  auto quantileEstimationAgg = Aggregation::QuantileEstimationAggregation(
      integerType, outputType, readFieldExpression, "result");
  auto quantileEstimationValue = Aggregation::QuantileEstimationValue();
  auto memref =
      Nautilus::Value<Nautilus::MemRef>((int8_t *)&quantileEstimationValue);
  auto incomingValue4 = Nautilus::Value<Nautilus::Int64>(4_s64);
  auto incomingValue3 = Nautilus::Value<Nautilus::Int64>(3_s64);
  auto incomingValue6 = Nautilus::Value<Nautilus::Int64>(6_s64);
  auto incomingValue2 = Nautilus::Value<Nautilus::Int64>(2_s64);
  ...
  double expResult = quantileEstimationValue.digest.min() +
                     (50 * quantileEstimationValue.digest.max() / 100);
  // the estimate quantile result = minVal + (quantile (0-100%) * maxVal/100)
  // = 4.5 for this example
  EXPECT_TRUE((bool)(result.read("result") > (expResult * 0.9) &&
                     result.read("result") < (expResult * 1.1)));
  // Tdigest is an estimation the median thus we use 10% variance of the result
}

/**
 * Tests the combine, lower and reset functions of the
 * QuantileEstimationAggregation, i.e., the following event sequence
...
TEST_F(AggregationFunctionTest, scanEmitPipelineQuantileCombine) {
  ...
  double expResult = quantileEstimationValue.digest.min() +
                     (50 * quantileEstimationValue.digest.max() / 100);
  // the estimate quantile result = minVal + (quantile (0-100%) * maxVal/100) =
  // 1+(50*10/100) = 6 for this example; note that min is in one memref and max
  // is in the other memref to test combine
  EXPECT_TRUE((bool)(result.read("result") > (expResult * 0.9) &&
                     result.read("result") < (expResult * 1.1)));
  // Tdigest is an estimation the median thus we use 10% variance of the result
}
