...
class AggregationFunctionDataTypeTest
...
public:
  ...
  static PhysicalTypePtr getDataType(std::string dataTypeString,
    ...
    } else if (std::equal(dataTypeString.begin(), dataTypeString.end(),
                          "i16")) {
  ...
  static std::shared_ptr<Aggregation::AggregationValue>
  getAggregationValue(std::string aggFnType, std::string dataTypeString) {
    if (std::equal(aggFnType.begin(), aggFnType.end(), "sum")) {
      ...
      NES_THROW_RUNTIME_ERROR(
          "Unknown aggregation function type type: " << aggFnType);
    }
  }
...
TEST_P(AggregationFunctionDataTypeTest, scanEmitPipelineSum) {
  ...
  auto testParam = NES::Runtime::Execution::Expressions::
      AggregationFunctionDataTypeTest_scanEmitPipelineSum_Test::GetParam();
  std::string aggFunctionType = "sum";
  PhysicalTypePtr dataType =
      this->getDataType(testParam, physicalDataTypeFactory);
  ...
  auto sumAgg = Aggregation::SumAggregationFunction(
      dataType, dataType, readFieldExpression, "result");

  // create an aggregation value based on the aggregation function type and data
  // type
  ...
  EXPECT_EQ(result.read("result")->getType()->toString(),
            testParam); // Check if the type corresponds to the input type
}

/**
 * Tests the lift, combine, lower and reset functions of the Avg Aggregation
 */
// TODO #3602 Disabled because this test checks that the output type is the same
// as the input type. However, the average of two ints can be a float.
...
TEST_P(AggregationFunctionDataTypeTest, scanEmitPipelineCount) {
  ...
  PhysicalTypePtr dataType =
      this->getDataType(testParam, physicalDataTypeFactory);
  PhysicalTypePtr unsignedIntegerType =
      this->getDataType(ui64, physicalDataTypeFactory);
  auto readFieldExpression =
      std::make_shared<Expressions::ReadFieldExpression>("value");

  auto countAgg = Aggregation::CountAggregationFunction(
      dataType, unsignedIntegerType, readFieldExpression, "result");
...
INSTANTIATE_TEST_CASE_P(
    scanEmitPipelineAgg, AggregationFunctionDataTypeTest,
    ::testing::Values("i8", "i16", "i32", "i64", "ui8", "ui16", "ui32", "ui64",
                      "f32", "f64"),
    [](const testing::TestParamInfo<AggregationFunctionDataTypeTest::ParamType>
