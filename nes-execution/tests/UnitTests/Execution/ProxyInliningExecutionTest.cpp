...
class ProxyFunctionInliningExecutionTest
    : public Testing::BaseUnitTest,
      public AbstractCompilationBackendTest {
 public:
  /* Will be called before any test in this class are executed. */
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ProxyFunctionInliningExecutionTest.log",
...
Value<UInt64> getNumberOfTuples(Value<MemRef> tupleBufferRef) {
  return FunctionCall<>("NES__Runtime__TupleBuffer__getNumberOfTuples",
                        NES::Runtime::ProxyFunctions::
                            NES__Runtime__TupleBuffer__getNumberOfTuples,
                        tupleBufferRef);
}

void executeFunctionWithOptions(const CompilationOptions& options,
  ...
  Value<MemRef> tupleBufferPointer =
      Value<MemRef>((int8_t*)std::addressof(tupleBuffer));
  tupleBufferPointer.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0, IR::Types::StampFactory::createAddressStamp());
  auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn(
      [&]() { return getNumberOfTuples(tupleBufferPointer); });

  // Execute function that contains 'getNumberOfTuples' as external function
  // call.
  ...
  NES_DEBUG("Function result: {}",
            function((uint8_t*)std::addressof(tupleBuffer)));
}
...
TEST_P(ProxyFunctionInliningExecutionTest, getNumberOfTuplesInliningTest) {
  ...
  auto dumpHelper = DumpHelper::create(options.getIdentifier(), true, true,
                                       options.getDumpOutputPath());

  // Execute the getNumberOfTuples() function.
  executeFunctionWithOptions(options, dumpHelper);

  // When compiling with O0, the generated code should contain a function call
  // to getNumberOfTuples.
  std::ifstream generatedProxyIR(dumpHelper.getOutputPath() +
                                 std::filesystem::path::preferred_separator +
                                 options.getIdentifier());
  NES_ASSERT2_FMT(generatedProxyIR.peek() != std::ifstream::traits_type::eof(),
                  "No proxy file was generated.");

  // When compiling with O1+, the generated LLVM IR should not contain a
  // function call.
  std::string line;
  ...
  while (std::getline(generatedProxyIR, line)) {
    if (!foundExecute) {
      ...
      NES_ASSERT2_FMT(line.find("@tail call") == std::string::npos,
                      "execute contained a tail call even though \
...
TEST_P(ProxyFunctionInliningExecutionTest,
       getNumberOfTuplesInliningWithoutOptimizationTest) {
  // Create the required CompilationOptions and the DumpHelper.
  CompilationOptions options;
  options.setProxyInlining(true);
  options.setDumpToFile(true);
  ...
  // Check if a file that contains the generated LLVM IR (generated function
  // code linked with proxy functions).
  std::ifstream generatedProxyIR(dumpHelper.getOutputPath() +
                                 std::filesystem::path::preferred_separator +
                                 options.getIdentifier());
  NES_ASSERT2_FMT(generatedProxyIR.peek() != std::ifstream::traits_type::eof(),
  ...
  // When compiling with O0, the generated LLVM IR should contain a function
  // call to getNumberOfTuples.
  std::string line;
  bool foundExecute = false;
  ...
  NES_ASSERT2_FMT(
      callFound,
      "The execute function should contain a call to getNumberOfTuples when compiling with \
...
INSTANTIATE_TEST_CASE_P(
    testFunctionCalls, ProxyFunctionInliningExecutionTest,
    ::testing::Values("MLIR"),
    [](const testing::TestParamInfo<
        ProxyFunctionInliningExecutionTest::ParamType>& info) {
      return info.param;
    });
