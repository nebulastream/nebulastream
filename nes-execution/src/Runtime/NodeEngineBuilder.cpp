...
NodeEngineBuilder NodeEngineBuilder::create(
    Configurations::WorkerConfigurationPtr workerConfiguration) {
  return NodeEngineBuilder(workerConfiguration);
}

NodeEngineBuilder& NodeEngineBuilder::setQueryStatusListener(
...
NodeEngineBuilder& NodeEngineBuilder::setPartitionManager(
    Network::PartitionManagerPtr partitionManager) {
  this->partitionManager = partitionManager;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setHardwareManager(
    HardwareManagerPtr hardwareManager) {
  this->hardwareManager = hardwareManager;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setBufferManagers(
    std::vector<BufferManagerPtr> bufferManagers) {
  this->bufferManagers = bufferManagers;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setQueryManager(
    QueryManagerPtr queryManager) {
  this->queryManager = queryManager;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setLanguageCompiler(
    std::shared_ptr<Compiler::LanguageCompiler> languageCompiler) {
  this->languageCompiler = languageCompiler;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setJITCompiler(
    Compiler::JITCompilerPtr jitCompiler) {
  this->jitCompiler = jitCompiler;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setPhaseFactory(
    QueryCompilation::Phases::PhaseFactoryPtr phaseFactory) {
  this->phaseFactory = phaseFactory;
  return *this;
}

NodeEngineBuilder& NodeEngineBuilder::setOpenCLManager(
    NES::Runtime::OpenCLManagerPtr openCLManager) {
...
NES::Runtime::NodeEnginePtr NodeEngineBuilder::build() {
  ...
  try {
    auto nodeEngineId = (this->nodeEngineId == INVALID<WorkerId>)
                            ? getNextWorkerId()
                            : this->nodeEngineId;
    auto partitionManager = (!this->partitionManager)
                                ? std::make_shared<Network::PartitionManager>()
                                : this->partitionManager;
    auto hardwareManager = (!this->hardwareManager)
    ...
    if (numberOfQueues == 1) {
      ...
      for (auto i = 0u; i < numberOfQueues; ++i) {
        bufferManagers.push_back(std::make_shared<BufferManager>(
            workerConfiguration->bufferSizeInBytes.getValue(),
            // if we run in static with multiple queues, we divide the whole
            // buffer manager among the queues
    ...
    if (bufferManagers.empty()) {
      NES_ERROR(
          "Runtime: error while building NodeEngine: no NesWorker provided");
      throw Exceptions::RuntimeException(
          "Error while building NodeEngine : no NesWorker provided",
          NES::collectAndPrintStacktrace());
    }

    QueryManagerPtr queryManager{this->queryManager};
    if (!this->queryManager) {
      auto numOfThreads = static_cast<uint16_t>(
          workerConfiguration->numWorkerThreads.getValue());
      auto numberOfBuffersPerEpoch = static_cast<uint16_t>(
          workerConfiguration->numberOfBuffersPerEpoch.getValue());
      std::vector<uint64_t> workerToCoreMappingVec =
          NES::Util::splitWithStringDelimiter<uint64_t>(
              workerConfiguration->workerPinList.getValue(), ",");
      switch (workerConfiguration->queryManagerMode.getValue()) {
        case QueryExecutionMode::Dynamic: {
          queryManager = std::make_shared<DynamicQueryManager>(
              nesWorker, bufferManagers, nodeEngineId, numOfThreads,
              hardwareManager, numberOfBuffersPerEpoch, workerToCoreMappingVec);
          break;
        }
    ...
    if (!partitionManager) {
      NES_ERROR(
          "Runtime: error while building NodeEngine: error while creating "
          "PartitionManager");
      throw Exceptions::RuntimeException(
          "Error while building NodeEngine : Error while creating "
          "PartitionManager",
          NES::collectAndPrintStacktrace());
    }
    if (!queryManager) {
      NES_ERROR(
          ...
          "QueryManager");
      throw Exceptions::RuntimeException(
          "Error while building NodeEngine : Error while creating QueryManager",
          NES::collectAndPrintStacktrace());
    }

    auto queryCompilationOptions =
        createQueryCompilationOptions(workerConfiguration->queryCompiler);
    auto phaseFactory =
        (!this->phaseFactory)
    ...
    auto compiler = QueryCompilation::NautilusQueryCompiler::create(
        queryCompilationOptions, phaseFactory,
    ...
    if (!compiler) {
      NES_ERROR(
          ...
          "compiler");
      throw Exceptions::RuntimeException(
          "Error while building NodeEngine : failed to create compiler",
          NES::collectAndPrintStacktrace());
    }
    ...
    std::shared_ptr<NodeEngine> engine = std::make_shared<NodeEngine>(
        physicalSources, std::move(hardwareManager), std::move(bufferManagers),
        ...
        std::move(partitionManager), std::move(compiler), std::move(nesWorker),
        std::move(openCLManager), nodeEngineId,
        workerConfiguration->numberOfBuffersInGlobalBufferManager.getValue(),
...
NodeEngineBuilder::createQueryCompilationOptions(
    const Configurations::QueryCompilerConfiguration&
  ...
  queryCompilationOptions->setCompilationStrategy(
      queryCompilerConfiguration.compilationStrategy);

  // set pipelining strategy mode
  queryCompilationOptions->setPipeliningStrategy(
      queryCompilerConfiguration.pipeliningStrategy);
  ...
  if (queryCompilerType ==
    ...
    NES_WARNING(
        "The LEGACY window strategy is not supported by Nautilus. Switch to "
        "SLICING!")
    queryCompilationOptions->setWindowingStrategy(
  ...
  queryCompilationOptions->setQueryCompiler(
      queryCompilerConfiguration.queryCompilerType);

  // set the dump mode
  queryCompilationOptions->setDumpMode(
      queryCompilerConfiguration.queryCompilerDumpMode);

  // set nautilus backend
  queryCompilationOptions->setNautilusBackend(
      queryCompilerConfiguration.nautilusBackend);

  queryCompilationOptions->getHashJoinOptions()->setNumberOfPartitions(
      queryCompilerConfiguration.numberOfPartitions.getValue());
  queryCompilationOptions->getHashJoinOptions()->setPageSize(
      queryCompilerConfiguration.pageSize.getValue());
  queryCompilationOptions->getHashJoinOptions()->setPreAllocPageCnt(
      queryCompilerConfiguration.preAllocPageCnt.getValue());
  // zero indicate that it has not been set in the yaml config
  if (queryCompilerConfiguration.maxHashTableSize.getValue() != 0) {
    queryCompilationOptions->getHashJoinOptions()
        ->setTotalSizeForDataStructures(
            queryCompilerConfiguration.maxHashTableSize.getValue());
  }

  queryCompilationOptions->setStreamJoinStrategy(
      queryCompilerConfiguration.joinStrategy);

  queryCompilationOptions->setCUDASdkPath(
      queryCompilerConfiguration.cudaSdkPath.getValue());
