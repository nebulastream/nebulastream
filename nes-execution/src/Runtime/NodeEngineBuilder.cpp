...
NES::Runtime::NodeEnginePtr NodeEngineBuilder::build() {
  ...
  try {
    ...
    if (!partitionManager) {
      NES_ERROR("Runtime: error while building NodeEngine: error while "
                "creating PartitionManager");
      throw Exceptions::RuntimeException(
...
NodeEngineBuilder::createQueryCompilationOptions(
  ...
  if (queryCompilerType ==
    ...
    NES_WARNING("The LEGACY window strategy is not supported by Nautilus. "
                "Switch to SLICING!")
