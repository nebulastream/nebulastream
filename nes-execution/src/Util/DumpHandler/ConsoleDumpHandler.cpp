...
std::shared_ptr<ConsoleDumpHandler>
ConsoleDumpHandler::create(std::ostream &out) {
  return std::make_shared<ConsoleDumpHandler>(out);
}

void ConsoleDumpHandler::dumpHelper(NodePtr const &op, uint64_t depth,
                                    uint64_t indent, std::ostream &out) const {
...
void ConsoleDumpHandler::multilineDumpHelper(const NodePtr &op, uint64_t depth,
                                             uint64_t indent,
...
void ConsoleDumpHandler::dump(const NodePtr node) {
  multilineDumpHelper(node, /*depth*/ 0, /*indent*/ 2, out);
}

void ConsoleDumpHandler::dump(std::string, std::string,
                              DecomposedQueryPlanPtr decomposedQueryPlan) {
  out << "Dumping pipelineQueryPlan: " << decomposedQueryPlan->toString()
      << std::endl;
}

void ConsoleDumpHandler::dump(
    std::string, std::string,
    QueryCompilation::PipelineQueryPlanPtr pipelineQueryPlan) {
  out << "Dumping pipelineQueryPlan: " << pipelineQueryPlan->toString()
      << std::endl;
}
} // namespace NES
