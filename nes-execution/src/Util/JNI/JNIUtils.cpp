...
void jniErrorCheck(const std::source_location location) {
  ...
  if (exception) {
    ...
    NES_THROW_RUNTIME_ERROR(
        "An error occurred during a map java UDF execution in function "
        << location.function_name() << " at " << location.file_name() << ":"
        << location.line() << ": " << utf);
  }
}
...
jmethodID getMethod(jclass clazz, const std::string_view &methodName,
  ...
  auto method =
      getEnv()->GetMethodID(clazz, methodName.data(), signature.data());
  jniErrorCheck();
  return method;
}
...
void freeObject(jobject object) {
  ...
  case JNIInvalidRefType:
    NES_THROW_RUNTIME_ERROR("Can't delete invalid ref!");
...
template <typename T>
jobject createObjectType(T value, const std::string_view &className,
                         const std::string_view &constructorSignature) {
  auto env = getEnv();
...
jobject createBoolean(bool value) {
  return createObjectType(value, "java/lang/Boolean", "(Z)V");
}

jobject createFloat(float value) {
  return createObjectType(value, "java/lang/Float", "(F)V");
}

jobject createDouble(double value) {
  return createObjectType(value, "java/lang/Double", "(D)V");
}

jobject createInteger(int32_t value) {
  return createObjectType(value, "java/lang/Integer", "(I)V");
}

jobject createLong(int64_t value) {
  return createObjectType(value, "java/lang/Long", "(J)V");
}

jobject createShort(int16_t value) {
  return createObjectType(value, "java/lang/Short", "(S)V");
}

jobject createByte(int8_t value) {
  return createObjectType(value, "java/lang/Byte", "(B)V");
}
...
template <typename T>
T getObjectTypeValue(jobject object, const std::string_view &className,
                     const std::string_view &getterName,
                     const std::string_view &getterSignature) {
  NES_ASSERT(getEnv()->GetObjectRefType(object) != JNIInvalidRefType,
             "object ref is invalid");
  auto env = getEnv();
  auto clazz = findClass(className);
...
bool getBooleanValue(jobject object) {
  return getObjectTypeValue<bool>(object, "java/lang/Boolean", "booleanValue",
                                  "()Z");
}

float getFloatValue(jobject object) {
  return getObjectTypeValue<float>(object, "java/lang/Float", "floatValue",
                                   "()F");
}
double getDoubleValue(jobject object) {
  return getObjectTypeValue<double>(object, "java/lang/Double", "doubleValue",
                                    "()D");
}

int32_t getIntegerValue(jobject object) {
  return getObjectTypeValue<int32_t>(object, "java/lang/Integer", "intValue",
                                     "()I");
}

int64_t getLongValue(jobject object) {
  return getObjectTypeValue<int64_t>(object, "java/lang/Long", "longValue",
                                     "()J");
}

int16_t getShortValue(jobject object) {
  return getObjectTypeValue<int16_t>(object, "java/lang/Short", "shortValue",
                                     "()S");
}

int8_t getByteValue(jobject object) {
  return getObjectTypeValue<int8_t>(object, "java/lang/Byte", "byteValue",
                                    "()B");
}
