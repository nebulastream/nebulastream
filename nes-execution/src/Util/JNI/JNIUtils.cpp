...
void jniErrorCheck(const std::source_location location)
{
    ...
    if (exception)
    {
        ...
        NES_THROW_RUNTIME_ERROR(
            "An error occurred during a map java UDF execution in function " << location.function_name() << " at " << location.file_name()
                                                                             << ":" << location.line() << ": " << utf);
    }
}
...
template <typename T>
T getObjectTypeValue(
    jobject object, const std::string_view& className, const std::string_view& getterName, const std::string_view& getterSignature)
{
...
{
    return getObjectTypeValue<bool>(object, "java/lang/Boolean", "booleanValue", "()Z");
}

float getFloatValue(jobject object)
{
    return getObjectTypeValue<float>(object, "java/lang/Float", "floatValue", "()F");
}
double getDoubleValue(jobject object)
{
    return getObjectTypeValue<double>(object, "java/lang/Double", "doubleValue", "()D");
}

int32_t getIntegerValue(jobject object)
{
    return getObjectTypeValue<int32_t>(object, "java/lang/Integer", "intValue", "()I");
}

int64_t getLongValue(jobject object)
{
    return getObjectTypeValue<int64_t>(object, "java/lang/Long", "longValue", "()J");
}

int16_t getShortValue(jobject object)
{
    return getObjectTypeValue<int16_t>(object, "java/lang/Short", "shortValue", "()S");
}

int8_t getByteValue(jobject object)
{
    return getObjectTypeValue<int8_t>(object, "java/lang/Byte", "byteValue", "()B");
}
