...
getWindowingParameters(Windowing::TimeBasedWindowType& windowType) {
  ...
  switch (type) {
    case Windowing::TimeCharacteristic::Type::IngestionTime: {
      auto timeFunction = std::make_unique<
    ...
    case Windowing::TimeCharacteristic::Type::EventTime: {
      const auto& timeStampFieldName =
          windowType.getTimeCharacteristic()->getField()->getName();
      auto timeStampFieldRecord = std::make_shared<
          Runtime::Execution::Expressions::ReadFieldExpression>(
          timeStampFieldName);
      auto timeFunction =
