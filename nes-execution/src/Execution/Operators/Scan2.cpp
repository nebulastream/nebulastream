...
#include <Execution/Operators/Scan2.hpp>
...
Scan2::Scan2(std::unique_ptr<MemoryProvider::MemoryProvider> memoryProvider, std::vector<Record::RecordFieldIdentifier> projections)
    : memoryProvider(std::move(memoryProvider)), projections(std::move(projections))
{
}

void Scan2::open(ExecutionContext& ctx, RecordBuffer& recordBuffer) const
{
