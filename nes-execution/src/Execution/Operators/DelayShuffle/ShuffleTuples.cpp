...
ShuffleTuplesOperatorHandler::ShuffleTuplesOperatorHandler(float unorderedness, uint64_t minShuffle, uint64_t maxShuffle)
    : unorderedness(unorderedness), gen(std::mt19937(std::random_device()())), unorderednessDistrib(0, 1), ShuffleDistrib(minShuffle, maxShuffle)
{
}

void ShuffleTuplesOperatorHandler::setup(uint64_t numberOfWorkerThreads)
{
...
void ShuffleTuplesOperatorHandler::createEmitIndicesForInputBuffer(WorkerThreadId workerThreadId, uint64_t numberOfTuples)
{
...
std::vector<uint64_t> ShuffleTuplesOperatorHandler::getEmitIndicesForWorker(WorkerThreadId workerThreadId)
{
...
void ShuffleTuples::setup(ExecutionContext& executionCtx) const
{
    ...
    nautilus::invoke(
        +[](const PipelineExecutionContext* pipelineExecutionContext)
        {
            ...
            {
                pipelinesFile << "ShuffleTuples pipelineId: " << pipelineExecutionContext->getPipelineId() << std::endl;
                pipelinesFile.flush();
            }
        },
...
void ShuffleTuples::emitRecordBuffer(ExecutionContext& ctx, RecordBuffer& inputBuffer, RecordBuffer& ouputBuffer) const
{
