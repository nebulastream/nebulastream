...
extern "C" void incrementCount(void *state) {
  ...
  NES_TRACE("Called incrementCount: recordCount = {}",
            handler->recordCount + 1);
  handler->recordCount++;
}
...
NonKeyedThresholdWindow::NonKeyedThresholdWindow(
    ...
    const std::vector<Nautilus::Record::RecordFieldIdentifier>
        &aggregationResultFieldIdentifiers,
    uint64_t minCount,
    ...
    : predicateExpression(std::move(predicateExpression)),
      ...
      minCount(minCount), operatorHandlerIndex(operatorHandlerIndex),
      aggregationFunctions(aggregationFunctions) {}

void NonKeyedThresholdWindow::execute(ExecutionContext &ctx,
                                      Record &record) const {
  NES_TRACE("Execute ThresholdWindow for received record {}",
            record.getAllFields().begin()->c_str())
  ...
  if (val) {
    NES_TRACE("Execute ThresholdWindow for valid predicate {}",
    ...
    if (!FunctionCall("getIsWindowOpen", getIsWindowOpen, handler)) {
      NES_DEBUG("Threshold window starts, opening value:{}",
                std::accumulate(
                    allFieldNames.begin(), allFieldNames.end(), std::string{},
    ...
    for (uint64_t i = 0; i < aggregationFunctions.size(); ++i) {
      auto aggregationValueState =
          FunctionCall("getAggregationValue", getAggregationValue, handler,
                       Value<UInt64>(i));
      aggregationFunctions[i]->lift(aggregationValueState, record);
    }
    FunctionCall("incrementCount", incrementCount, handler);
    FunctionCall("setIsWindowOpen", setIsWindowOpen, handler,
                 Value<Boolean>(true));
    FunctionCall("unlockWindowHandler", unlockWindowHandler, handler);
  } else {
    auto isWindowOpen =
        FunctionCall("getIsWindowOpen", getIsWindowOpen, handler);
    if (isWindowOpen) {
      auto recordCount =
          FunctionCall("getRecordCount", getRecordCount, handler);
      if (recordCount >= minCount) {
        ...
        auto aggregatedValue = Value<Int64>(
            1_s64); // default value to aggregate (i.e., for countAgg)
        auto allFieldNames = record.getAllFields();
        NES_DEBUG("Threshold window ends, closing value:{} | aggVal:{}",
                  ...
                  std::accumulate(
                      aggregationResultFieldIdentifiers.begin(),
                      aggregationResultFieldIdentifiers.end(), std::string{},
                      [&resultRecord](std::string acc, std::string s) {
                        return acc + " " + s + "=" +
                               resultRecord.read(s)->toString();
                      }));

        // crucial to release the handler here before we execute the rest of the
        // pipeline
        child->execute(ctx, resultRecord);
      } else {
        // if the minCount is not reached, we still need to close the window,
        // reset counter and release the lock if the handler
        FunctionCall("setIsWindowOpen", setIsWindowOpen, handler,
                     Value<Boolean>(false));
        FunctionCall("resetCount", resetCount, handler);
        FunctionCall("unlockWindowHandler", unlockWindowHandler, handler);
