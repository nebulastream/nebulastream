...
{
    NES_ASSERT(
        this->aggregationFunctions.size() == this->aggregationResultFieldIdentifiers.size(),
...
void KeyedThresholdWindow::execute(ExecutionContext & ctx, Record & record) const
{
    ...
    if (val)
    {
        ...
        for (uint32_t i = 0; i < aggregationFunctions.size(); ++i)
        {
            auto aggregationValueState
                = FunctionCall("getKeyedAggregationValue", getKeyedAggregationValue, handler, keyValue.as<UInt32>(), Value<UInt32>(i));
    ...
    {
        auto isWindowOpen = FunctionCall("getIsKeyedThresholdWindowOpen", getIsKeyedThresholdWindowOpen, handler, keyValue.as<UInt32>());
        if (isWindowOpen)
        {
            auto recordCount
                = FunctionCall("getKeyedThresholdWindowRecordCount", getKeyedThresholdWindowRecordCount, handler, keyValue.as<UInt32>());
            ...
            if (recordCount >= minCount)
            {
                ...
                for (uint32_t i = 0; i < aggregationFunctions.size(); ++i)
                {
                    auto aggregationValueMemref = FunctionCall(
                        "getKeyedAggregationValue", getKeyedAggregationValue, handler, keyValue.as<UInt32>(), Value<UInt32>(i));
