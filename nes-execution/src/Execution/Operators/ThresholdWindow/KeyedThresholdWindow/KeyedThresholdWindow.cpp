...
extern "C" void incrementKeyedThresholdWindowCount(void *state,
  ...
  NES_TRACE("Called incrementCount: recordCount = {}",
            handler->keyedAggregationStates.at(aggKey).recordCount + 1);
  handler->keyedAggregationStates.at(aggKey).recordCount++;
}

extern "C" void setKeyedThresholdWindowIsWindowOpen(void *state,
                                                    uint32_t aggKey,
                                                    bool isWindowOpen) {
  auto handler = (KeyedThresholdWindowOperatorHandler *)state;
...
extern "C" bool getIsKeyedThresholdWindowOpen(void *state, uint32_t aggKey) {
  auto handler = (KeyedThresholdWindowOperatorHandler *)state;
  NES_TRACE("Called getIsWindowOpen: isWindowOpen = {}",
            handler->keyedAggregationStates.at(aggKey).isWindowOpen);
...
extern "C" uint64_t getKeyedThresholdWindowRecordCount(void *state,
                                                       uint32_t aggKey) {
  auto handler = (KeyedThresholdWindowOperatorHandler *)state;
  NES_TRACE("Called getRecordCount: recordCount = {}",
            handler->keyedAggregationStates.at(aggKey).recordCount);
...
extern "C" void createStateIfNotExist(void *handlerMemref, uint32_t aggKey) {
  ...
  if (handler->keyedAggregationStates.find(aggKey) ==
      handler->keyedAggregationStates.end()) {
    // key does not exist, create a new map entry with aggKey as key
    auto keyedThresholdWindowStates = KeyedThresholdWindowState();
    handler->keyedAggregationStates.insert(
        std::make_pair(aggKey, std::move(keyedThresholdWindowStates)));
  }
}

template <class T>
void addAggregationValues(void *state, uint32_t aggKey,
                          uint32_t aggFunctionCount) {
  auto handler = (KeyedThresholdWindowOperatorHandler *)state;

  // Only add the aggregation value once per key
  if (handler->keyedAggregationStates.at(aggKey).aggregationValues.size() <
      aggFunctionCount) {
    auto aggVal = std::make_unique<T>();
    handler->keyedAggregationStates.at(aggKey).aggregationValues.push_back(
        std::move(aggVal));
  }
}

// TODO #3801: should support key data types other than uint32_t
extern "C" void *getKeyedAggregationValue(void *state, uint32_t aggKey,
                                          uint32_t aggFuncIdx) {
  auto handler = (KeyedThresholdWindowOperatorHandler *)state;
  NES_TRACE("Called getAggregationValue: for aggIdx = {} and aggKey = {}",
            aggFuncIdx, aggKey);
  return (void *)handler->keyedAggregationStates.at(aggKey)
      .aggregationValues[aggFuncIdx]
      .get();
}
...
KeyedThresholdWindow::KeyedThresholdWindow(
    ...
    : predicateExpression(std::move(predicateExpression)),
      ...
      keyExpression(keyExpression), keyFieldIdentifier(keyFieldIdentifier),
      minCount(minCount), operatorHandlerIndex(operatorHandlerIndex),
      aggregationFunctions(aggregationFunctions) {
  NES_ASSERT(this->aggregationFunctions.size() ==
                 this->aggregationResultFieldIdentifiers.size(),
             "The number of aggregation expression and aggregation functions "
             "need to be equals");
}

void KeyedThresholdWindow::execute(ExecutionContext &ctx,
                                   Record &record) const {
  ...
  FunctionCall("createStateIfNotExist", createStateIfNotExist, handler,
               keyValue.as<UInt32>());
  ...
  for (uint32_t i = 0; i < aggregationFunctions.size(); ++i) {
    ...
    // TODO #3801 the aggregation values and key data type should not be
    // hard-coded
    if (std::dynamic_pointer_cast<Aggregation::SumAggregationFunction>(
            aggregationFunctions[i])) {
      FunctionCall(
          "addAggregationValues",
          addAggregationValues<Aggregation::SumAggregationValue<int64_t>>,
          handler, keyValue.as<UInt32>(), aggFunctionsCount);
    } else if (std::dynamic_pointer_cast<Aggregation::MaxAggregationFunction>(
                   aggregationFunctions[i])) {
      FunctionCall(
          "addAggregationValues",
          addAggregationValues<Aggregation::MaxAggregationValue<int64_t>>,
          handler, keyValue.as<UInt32>(), aggFunctionsCount);
    } else if (std::dynamic_pointer_cast<Aggregation::MinAggregationFunction>(
                   aggregationFunctions[i])) {
      FunctionCall(
          "addAggregationValues",
          addAggregationValues<Aggregation::MinAggregationValue<int64_t>>,
          handler, keyValue.as<UInt32>(), aggFunctionsCount);
    } else if (std::dynamic_pointer_cast<Aggregation::CountAggregationFunction>(
                   aggregationFunctions[i])) {
      FunctionCall(
          "addAggregationValues",
          addAggregationValues<Aggregation::CountAggregationValue<int64_t>>,
          handler, keyValue.as<UInt32>(), aggFunctionsCount);
    } else if (std::dynamic_pointer_cast<Aggregation::AvgAggregationFunction>(
                   aggregationFunctions[i])) {
      FunctionCall(
          "addAggregationValues",
  ...
  if (val) {
    ...
    for (uint32_t i = 0; i < aggregationFunctions.size(); ++i) {
      auto aggregationValueState =
          FunctionCall("getKeyedAggregationValue", getKeyedAggregationValue,
                       handler, keyValue.as<UInt32>(), Value<UInt32>(i));
      aggregationFunctions[i]->lift(aggregationValueState, record);
    }

    FunctionCall("incrementKeyedThresholdWindowCount",
                 incrementKeyedThresholdWindowCount, handler,
                 keyValue.as<UInt32>());
    FunctionCall("setKeyedThresholdWindowIsWindowOpen",
                 setKeyedThresholdWindowIsWindowOpen, handler,
                 keyValue.as<UInt32>(), Value<Boolean>(true));
    FunctionCall("unlockKeyedThresholdWindowHandler",
                 unlockKeyedThresholdWindowHandler, handler,
                 keyValue.as<UInt32>());
  } else {
    auto isWindowOpen = FunctionCall("getIsKeyedThresholdWindowOpen",
                                     getIsKeyedThresholdWindowOpen, handler,
                                     keyValue.as<UInt32>());
    if (isWindowOpen) {
      auto recordCount = FunctionCall("getKeyedThresholdWindowRecordCount",
                                      getKeyedThresholdWindowRecordCount,
                                      handler, keyValue.as<UInt32>());
      ...
      if (recordCount >= minCount) {
        ...
        FunctionCall("setKeyedThresholdWindowIsWindowOpen",
                     setKeyedThresholdWindowIsWindowOpen, handler,
                     keyValue.as<UInt32>(), Value<Boolean>(false));
        FunctionCall("resetKeyedThresholdWindowCount",
                     resetKeyedThresholdWindowCount, handler,
                     keyValue.as<UInt32>());
        FunctionCall("unlockKeyedThresholdWindowHandler",
                     unlockKeyedThresholdWindowHandler, handler,
                     keyValue.as<UInt32>());
