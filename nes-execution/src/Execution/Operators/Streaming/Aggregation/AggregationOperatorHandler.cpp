...
{
    AggregationOperatorHandler::keySize = keySize;
    AggregationOperatorHandler::valueSize = valueSize;
    AggregationOperatorHandler::pageSize = pageSize;
    AggregationOperatorHandler::numberOfBuckets = numberOfBuckets;
}
    ...
    return std::function(
        [&](SliceStart sliceStart, SliceEnd sliceEnd) -> std::vector<std::shared_ptr<Slice>>
        {
            return {std::make_shared<AggregationSlice>(
                ...
                sliceStart,
                sliceEnd,
                numberOfWorkerThreads.or_else(numberOfWorkerThreadsValueLess).value())};
        });
}
    ...
    PipelineExecutionContext* pipelineCtx)
{
    ...
    {
        /// Getting all hashmaps for each slice that have at least one tuple
        std::vector<Interface::HashMapInterface*> allHashMaps;
        ...
        /// - all pointers to all hashmap of the window to be triggered
        /// - a new hash map for the probe operator, so that we are not overwriting the thread local hash maps
        /// - size of EmittedAggregationWindow
        const auto neededBufferSize = sizeof(EmittedAggregationWindow) + allHashMaps.size() * sizeof(Interface::HashMapInterface*);
        ...
        else
        {
            ...
            {
                throw QueryRuntimeError("Could not get a buffer of size {} for the aggregation window trigger", neededBufferSize);
            }
        ...
        std::memset(bufferMemory, 0, tupleBuffer.getBufferSize()); /// It might be that the buffer is not zeroed out.
        ...
        bufferMemory->hashMaps = reinterpret_cast<Interface::HashMapInterface**>(addressFirstHashMapPtr);
        std::memcpy(addressFirstHashMapPtr, allHashMaps.data(), allHashMaps.size() * sizeof(Interface::HashMapInterface*));
