...
AggregationOperatorHandler::AggregationOperatorHandler(
    ...
    std::unique_ptr<WindowSlicesStoreInterface> sliceAndWindowStore, bool sequentialProcessing)
    ...
    return std::function(
            ...
            SliceStart sliceStart, SliceEnd sliceEnd) -> std::vector<std::shared_ptr<Slice>>
        {
            ...
            return {std::make_shared<AggregationSlice>(
                keySize, valueSize, numberOfBuckets, pageSize, sliceStart, sliceEnd, numberOfHashmaps)};
        });
}
