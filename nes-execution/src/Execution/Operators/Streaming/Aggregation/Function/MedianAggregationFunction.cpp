    ...
    const nautilus::val<AggregationState*> aggregationState, const nautilus::val<Memory::AbstractBufferProvider*>& bufferProvider)
{
    ...
    const auto numberOfEntries = invoke(
        +[](const Nautilus::Interface::PagedVector* pagedVector) { return pagedVector->getTotalNumberOfEntries(); }, pagedVectorPtr);

    /// Iterating twice over all the records in the paged vector to get the median
    ...
    for (auto candidateIt = pagedVectorRef.begin(allFieldNames); candidateIt != endIt; ++candidateIt)
    {
        ...
        for (auto itemIt = pagedVectorRef.begin(allFieldNames); itemIt != pagedVectorRef.end(allFieldNames); ++itemIt)
        {
...
size_t MedianAggregationFunction::getStateSizeInBytes() const
{
