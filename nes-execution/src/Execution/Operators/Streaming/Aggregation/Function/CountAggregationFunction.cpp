    ...
    const Nautilus::Record&)
{
    ...
    const auto count = Nautilus::VarVal::readVarValFromMemory(memAreaSum, inputType);

    /// Updating the count with the new value
    const auto newSum = count + nautilus::val<uint64_t>(1);
    ...
    const nautilus::val<Memory::AbstractBufferProvider*>&)
{
    ...
    const auto count1 = Nautilus::VarVal::readVarValFromMemory(memAreaSum1, inputType);
    ...
    const auto count2 = Nautilus::VarVal::readVarValFromMemory(memAreaSum2, inputType);

    /// Adding the counts together
    const auto newSum = count1 + count2;

    /// Writing the new count back to the first aggregation state
    newSum.writeToMemory(memAreaSum1);
}

Nautilus::Record CountAggregationFunction::lower(
...
void CountAggregationFunction::reset(
    const nautilus::val<AggregationState*> aggregationState, const nautilus::val<Memory::AbstractBufferProvider*>&)
{
    /// Resetting the count and count to 0
    const auto memArea = static_cast<nautilus::val<int8_t*>>(aggregationState);
    nautilus::memset(memArea, 0, getStateSizeInBytes());
}

size_t CountAggregationFunction::getStateSizeInBytes() const
{
