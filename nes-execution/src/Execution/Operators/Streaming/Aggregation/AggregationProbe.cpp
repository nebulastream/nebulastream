...
void AggregationProbe::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
        ...
        for (const auto entry : currentMap)
        {
            ...
            finalHashMap.insertOrUpdateEntry(
                    ...
                    const nautilus::val<Interface::AbstractHashMapEntry*>& entryOnUpdate)
                {
                    ...
                    {
                        aggFunction->combine(globalState, entryRefState, executionCtx.pipelineMemory);
                    ...
                    const nautilus::val<Interface::AbstractHashMapEntry*>& entryOnInsert)
                {
                    ...
                    for (const auto& aggFunction : nautilus::static_iterable(aggregationFunctions))
                    {
                        ...
                        aggFunction->reset(globalState, executionCtx.pipelineMemory);
                        aggFunction->combine(globalState, entryRefStatePtr, executionCtx.pipelineMemory);
                        globalState = globalState + aggFunction->getSizeOfStateInBytes();
                        entryRefStatePtr = entryRefStatePtr + aggFunction->getSizeOfStateInBytes();
                    }
                },
                executionCtx.pipelineMemory.bufferProvider);
        }
    }
    ...
    for (const auto entry : finalHashMap)
    {
        ...
        for (const auto& aggFunction : nautilus::static_iterable(aggregationFunctions))
        {
            outputRecord.reassignFields(aggFunction->lower(finalStatePtr, executionCtx.pipelineMemory));
