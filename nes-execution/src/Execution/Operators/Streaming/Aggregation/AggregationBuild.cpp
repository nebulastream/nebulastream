...
Interface::HashMapInterface* getHashMapProxy(
    ...
    const Memory::AbstractBufferProvider* bufferProvider)
{
    ...
    INVARIANT(hashMap.size() == 1, "We expect exactly one slice for the given timestamp during the AggregationBuild");
...
void AggregationBuild::execute(ExecutionContext& ctx, Record& record) const
{
    ...
    const auto hashMapEntry = hashMap.findOrCreateEntry(
        ...
        *hashFunction,
        [&](const nautilus::val<Interface::AbstractHashMapEntry*>& entry)
        {
            ...
            const Interface::ChainedEntryRef entryRefReset(entry, fieldKeys, fieldValues);
    ...
    Interface::ChainedEntryRef const entryRef(hashMapEntry, fieldKeys, fieldValues);
