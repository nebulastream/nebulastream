...
std::vector<StreamSlice*>* StreamJoinOperatorHandlerBucketing::getAllWindowsToFillForTs(uint64_t ts, WorkerThreadId workerThreadId, AbstractBufferProvider& bufferProvider)
{
    ...
    for (int64_t start = lastStart; start >= 0 && start > lowerBound; start -= windowSlide)
    {
        ...
        {
            auto newWindow = createNewSlice(windowInfo.windowStart, windowInfo.windowEnd, bufferProvider);
