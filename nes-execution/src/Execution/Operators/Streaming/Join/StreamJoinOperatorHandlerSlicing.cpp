...
StreamSlicePtr StreamJoinOperatorHandlerSlicing::getSliceByTimestampOrCreateIt(
    uint64_t timestamp) {
  auto [slicesWriteLocked, windowToSlicesLocked] =
      folly::acquireLocked(slices, windowToSlices);
  ...
  NES_DEBUG("Creating slice for slice start={} and end={} for ts={}",
            sliceStart, sliceEnd, timestamp);
  ...
  for (auto windowInfo : getAllWindowsForSlice(*newSlice)) {
    ...
    NES_DEBUG("Added slice {} to window {}", newSlice->toString(),
              windowInfo.toString());
  }
  return newSlice;
}
...
StreamSlice* StreamJoinOperatorHandlerSlicing::getCurrentSliceOrCreate() {
  if (slices.rlock()->empty()) {
    return StreamJoinOperatorHandlerSlicing::getSliceByTimestampOrCreateIt(0)
        .get();
  }
  return slices.rlock()->back().get();
}

std::vector<WindowInfo> StreamJoinOperatorHandlerSlicing::getAllWindowsForSlice(
    StreamSlice& slice) {
  ...
  /**
   * To get all windows, we start with the window (firstWindow) that the current
   * slice is the last slice in it. Or in other words: The sliceEnd is equal to
   * the windowEnd. We then add all windows with a slide until we reach the
   * window (lastWindow) that that current slice is the first slice. Or in other
   * words: The sliceStart is equal to the windowStart;
   */
  const auto firstWindowEnd = sliceEnd;
  const auto lastWindowEnd = sliceStart + windowSize;
  for (auto curWindowEnd = firstWindowEnd; curWindowEnd <= lastWindowEnd;
       curWindowEnd += windowSlide) {
