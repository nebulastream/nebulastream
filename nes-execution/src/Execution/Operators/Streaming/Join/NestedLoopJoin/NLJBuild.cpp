...
void NLJBuild::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    auto sliceReference = invoke(getNLJSliceRefProxy, opHandlerMemRef, recordBuffer.getWatermarkTs(), executionCtx.pipelineMemory.bufferProvider);
...
void NLJBuild::execute(ExecutionContext& executionCtx, Record& record) const
{
    ...
    const Interface::PagedVectorRef pagedVectorRef(localJoinState->nljPagedVectorMemRef, memoryProvider, executionCtx.pipelineMemory.bufferProvider);
...
{
    localJoinState->sliceReference = invoke(getNLJSliceRefProxy, operatorHandlerRef, timestamp, executionCtx.pipelineMemory.bufferProvider);
