    ...
    : StreamSlice(windowStart, windowEnd)
{
    ...
    {
        leftTuples.emplace_back(std::make_unique<Nautilus::Interface::PagedVectorVarSized>(bufferManager, leftSchema, leftPageSize));
    }

    for (uint64_t i = 0; i < numberOfWorker; ++i)
    {
        rightTuples.emplace_back(std::make_unique<Nautilus::Interface::PagedVectorVarSized>(bufferManager, rightSchema, rightPageSize));
    }
    NES_TRACE(
