...
void NLJBuildSlicing::execute(ExecutionContext& ctx, Record& record) const {
  // Get the local state
  auto localJoinState =
      dynamic_cast<LocalNestedLoopJoinState*>(ctx.getLocalState(this));
  auto operatorHandlerMemRef = localJoinState->joinOperatorHandler;
  Value<UInt64> timestampVal = timeFunction->getTs(ctx, record);

  if (!(localJoinState->sliceStart <= timestampVal &&
        timestampVal < localJoinState->sliceEnd)) {
  ...
  auto nljPagedVectorMemRef = Nautilus::FunctionCall(
      "getNLJPagedVectorProxy", getNLJPagedVectorProxy,
      localJoinState->sliceReference, ctx.getWorkerThreadId(),
      Value<UInt64>(to_underlying(joinBuildSide)));
  Nautilus::Interface::PagedVectorVarSizedRef pagedVectorVarSizedRef(
      nljPagedVectorMemRef, schema);
...
void NLJBuildSlicing::updateLocalJoinState(
  ...
  // Retrieving the slice of the current watermark, as we expect that more
  // tuples will be inserted into this slice
  localJoinState->sliceReference =
      Nautilus::FunctionCall("getNLJSliceRefProxy", getNLJSliceRefProxy,
                             operatorHandlerMemRef, timestamp);
  localJoinState->sliceStart =
      Nautilus::FunctionCall("getNLJSliceStartProxy", getNLJSliceStartProxy,
                             localJoinState->sliceReference);
  localJoinState->sliceEnd =
      Nautilus::FunctionCall("getNLJSliceEndProxy", getNLJSliceEndProxy,
                             localJoinState->sliceReference);
}
...
void NLJBuildSlicing::open(ExecutionContext& ctx, RecordBuffer&) const {
  ...
  auto workerThreadId = ctx.getWorkerThreadId();
  auto sliceReference = Nautilus::FunctionCall(
      "getCurrentWindowProxy", getCurrentWindowProxy, opHandlerMemRef);
  auto nljPagedVectorMemRef = Nautilus::FunctionCall(
      "getNLJPagedVectorProxy", getNLJPagedVectorProxy, sliceReference,
      workerThreadId, Value<UInt64>(to_underlying(joinBuildSide)));
  auto pagedVectorVarSizedRef =
      Nautilus::Interface::PagedVectorVarSizedRef(nljPagedVectorMemRef, schema);
  auto localJoinState = std::make_unique<LocalNestedLoopJoinState>(
      opHandlerMemRef, sliceReference, pagedVectorVarSizedRef);
...
NLJBuildSlicing::NLJBuildSlicing(
