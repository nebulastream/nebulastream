...
void StreamJoinOperatorHandler::start(PipelineExecutionContextPtr pipelineCtx,
                                      uint32_t) {
...
void StreamJoinOperatorHandler::stop(QueryTerminationType queryTerminationType,
                                     PipelineExecutionContextPtr pipelineCtx) {
  NES_INFO("Stopped StreamJoinOperatorHandler with {}!",
...
StreamJoinOperatorHandler::getSliceBySliceIdentifier(uint64_t sliceIdentifier) {
  auto slicesLocked = slices.rlock();
  return getSliceBySliceIdentifier(slicesLocked, sliceIdentifier);
}

std::optional<StreamSlicePtr>
StreamJoinOperatorHandler::getSliceBySliceIdentifier(
...
void StreamJoinOperatorHandler::triggerAllSlices(
    PipelineExecutionContext* pipelineCtx) {
  {
    auto [slicesLocked, windowToSlicesLocked] =
    ...
    for (auto& [windowInfo, slicesAndStateForWindow] : *windowToSlicesLocked) {
      switch (slicesAndStateForWindow.windowState) {
        case WindowInfoState::BOTH_SIDES_FILLING:
          slicesAndStateForWindow.windowState =
              WindowInfoState::ONCE_SEEN_DURING_TERMINATION;
        case WindowInfoState::EMITTED_TO_PROBE:
          continue;
        case WindowInfoState::ONCE_SEEN_DURING_TERMINATION: {
          slicesAndStateForWindow.windowState =
              WindowInfoState::EMITTED_TO_PROBE;

          // Performing a cross product of all slices to make sure that each
          // slice gets probe with each other slice For bucketing, this should
          // be only done once
          for (auto& sliceLeft : slicesAndStateForWindow.slices) {
            for (auto& sliceRight : slicesAndStateForWindow.slices) {
              emitSliceIdsToProbe(*sliceLeft, *sliceRight, windowInfo,
                                  pipelineCtx);
            }
          }
        }
...
void StreamJoinOperatorHandler::checkAndTriggerWindows(
    const BufferMetaData& bufferMetaData,
    PipelineExecutionContext* pipelineCtx) {
  // The watermark processor handles the minimal watermark across both streams
  uint64_t newGlobalWatermark = watermarkProcessorBuild->updateWatermark(
...

  {
    ...
    for (auto& [windowInfo, slicesAndStateForWindow] : *windowToSlicesLocked) {
      ...
      // Performing a cross product of all slices to make sure that each slice
      // gets probe with each other slice For bucketing, this should be only
      // done once
      for (auto& sliceLeft : slicesAndStateForWindow.slices) {
        for (auto& sliceRight : slicesAndStateForWindow.slices) {
...
void StreamJoinOperatorHandler::deleteSlices(
    const BufferMetaData& bufferMetaData) {
  uint64_t newGlobalWaterMarkProbe = watermarkProcessorProbe->updateWatermark(
      bufferMetaData.watermarkTs, bufferMetaData.seqNumber,
      bufferMetaData.originId);
  NES_DEBUG("newGlobalWaterMarkProbe {} bufferMetaData {}",
            newGlobalWaterMarkProbe, bufferMetaData.toString());
  ...
  for (auto it = slicesLocked->begin(); it != slicesLocked->end(); ++it) {
    ...
    if (curSlice->getSliceStart() + windowSize < newGlobalWaterMarkProbe) {
      ...
      NES_DEBUG(
          "Deleting slice: {} as sliceStart+windowSize {} is smaller then "
          "watermark {}",
          curSlice->toString(), curSlice->getSliceStart() + windowSize,
...
uint64_t StreamJoinOperatorHandler::getNumberOfTuplesInSlice(
...
void StreamJoinOperatorHandler::setNumberOfWorkerThreads(
...
void StreamJoinOperatorHandler::updateWatermarkForWorker(
    uint64_t watermark, WorkerThreadId workerThreadId) {
...
void StreamJoinOperatorHandler::setBufferManager(
    const NES::Runtime::BufferManagerPtr& bufManager) {
...
StreamJoinOperatorHandler::StreamJoinOperatorHandler(
    ...
    : numberOfWorkerThreads(1),
      ...
      watermarkProcessorProbe(std::make_unique<MultiOriginWatermarkProcessor>(
