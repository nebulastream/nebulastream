...
/**
 * @brief Updates the sliceState of all slices and emits buffers, if the slices
 * can be emitted
 */
void checkWindowsTriggerProxy(void *ptrOpHandler, void *ptrPipelineCtx,
                              void *ptrWorkerCtx, uint64_t watermarkTs,
  ...
  auto *opHandler = StreamJoinOperator::getSpecificOperatorHandler(
      ptrOpHandler, joinStrategyInt, windowingStrategy);
  ...
  BufferMetaData bufferMetaData(minWatermark,
...
void StreamJoinBuild::close(ExecutionContext &ctx, RecordBuffer &) const {
  ...
  Nautilus::FunctionCall(
      "checkWindowsTriggerProxy", checkWindowsTriggerProxy,
      operatorHandlerMemRef, ctx.getPipelineContext(), ctx.getWorkerContext(),
...
StreamJoinBuild::StreamJoinBuild(
    ...
    : StreamJoinOperator(joinStrategy, windowingStrategy),
      operatorHandlerIndex(operatorHandlerIndex), schema(schema),
      joinFieldName(joinFieldName), joinBuildSide(joinBuildSide),
