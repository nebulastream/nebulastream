...
Operators::StreamJoinHashTable *
HJSlice::getHashTable(QueryCompilation::JoinBuildSideType joinBuildSide,
  ...
  if (joinBuildSide == QueryCompilation::JoinBuildSideType::Left) {
    return hashTableLeftSide.at(workerThreadId % hashTableLeftSide.size())
        .get();
  } else {
    return hashTableRightSide.at(workerThreadId % hashTableRightSide.size())
        .get();
  }
}

uint64_t HJSlice::getNumberOfTuplesOfWorker(
    QueryCompilation::JoinBuildSideType joinBuildSide,
    WorkerThreadId workerThreadId) {
  return getHashTable(joinBuildSide, workerThreadId)->getNumberOfTuples();
}

Operators::MergingHashTable &HJSlice::getMergingHashTable(
    QueryCompilation::JoinBuildSideType joinBuildSide) {
  if (joinBuildSide == QueryCompilation::JoinBuildSideType::Left) {
...
void HJSlice::mergeLocalToGlobalHashTable() {
  ...
  for (const auto &workerHashTableLeft : hashTableLeftSide) {
    for (auto bucketPos = 0_u64;
         bucketPos < workerHashTableLeft->getNumBuckets(); ++bucketPos) {
      mergingHashTableLeftSide.insertBucket(
          bucketPos, workerHashTableLeft->getBucketLinkedList(bucketPos));
    }
  }
  ...
  for (const auto &workerHashTableRight : hashTableRightSide) {
    for (auto bucketPos = 0_u64;
         bucketPos < workerHashTableRight->getNumBuckets(); ++bucketPos) {
      mergingHashTableRightSide.insertBucket(
          bucketPos, workerHashTableRight->getBucketLinkedList(bucketPos));
    }
  }
}

HJSlice::HJSlice(size_t numberOfWorker, uint64_t sliceStart, uint64_t sliceEnd,
                 size_t sizeOfRecordLeft, size_t sizeOfRecordRight,
                 size_t maxHashTableSize, size_t pageSize,
                 size_t preAllocPageSizeCnt, size_t numPartitions,
                 QueryCompilation::StreamJoinStrategy joinStrategy)
    : StreamSlice(sliceStart, sliceEnd),
      mergingHashTableLeftSide(Operators::MergingHashTable(numPartitions)),
      mergingHashTableRightSide(Operators::MergingHashTable(numPartitions)),
      fixedPagesAllocator(maxHashTableSize),
      alreadyMergedLocalToGlobalHashTable(false), joinStrategy(joinStrategy) {

  std::cout << "HJSlice for sliceStart: " << sliceStart
            << " and sliceEnd: " << sliceEnd << std::endl;
  if (joinStrategy == QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL) {
    ...
    for (auto i = 0UL; i < numberOfWorker; ++i) {
      hashTableLeftSide.emplace_back(
          std::make_unique<Operators::LocalHashTable>(
              sizeOfRecordLeft, numPartitions, fixedPagesAllocator, pageSize,
              preAllocPageSizeCnt));
    }
    ...
    for (auto i = 0UL; i < numberOfWorker; ++i) {
      hashTableRightSide.emplace_back(
          std::make_unique<Operators::LocalHashTable>(
              sizeOfRecordRight, numPartitions, fixedPagesAllocator, pageSize,
              preAllocPageSizeCnt));
    }
  } else if (joinStrategy ==
             QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCKING) {
    hashTableLeftSide.emplace_back(
        std::make_unique<Operators::GlobalHashTableLocking>(
            sizeOfRecordLeft, numPartitions, fixedPagesAllocator, pageSize,
            preAllocPageSizeCnt));
    ...
    hashTableRightSide.emplace_back(
        std::make_unique<Operators::GlobalHashTableLocking>(
            ...
            preAllocPageSizeCnt));
             ...
             QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE) {
    hashTableLeftSide.emplace_back(
        std::make_unique<Operators::GlobalHashTableLockFree>(
            sizeOfRecordLeft, numPartitions, fixedPagesAllocator, pageSize,
            preAllocPageSizeCnt));
  ...
  NES_DEBUG("Create new StreamHashJoinWindow with numberOfWorkerThreads={} HTs "
            "with numPartitions={} of pageSize={} "
            "sizeOfRecordLeft={} sizeOfRecordRight={}",
            numberOfWorker, numPartitions, pageSize, sizeOfRecordLeft,
            sizeOfRecordRight);
}

std::string HJSlice::toString() {
  std::ostringstream basicOstringstream;
  basicOstringstream << "StreamHashJoinWindow(sliceStart: " << sliceStart
                     << " sliceEnd: " << sliceEnd << ")";
