...
Operators::StreamJoinHashTable* HJSlice::getHashTable(QueryCompilation::JoinBuildSideType joinBuildSide, WorkerThreadId workerThreadId)
{
    ...
    , joinStrategy(joinStrategy)
{
    ...
    if (joinStrategy == QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL)
    {
        ...
        for (auto i = 0UL; i < numberOfWorker; ++i)
        {
            hashTableLeftSide.emplace_back(std::make_unique<Operators::LocalHashTable>(
                sizeOfRecordLeft, numPartitions, fixedPagesAllocator, pageSize, preAllocPageSizeCnt));
        }

        for (auto i = 0UL; i < numberOfWorker; ++i)
        {
            hashTableRightSide.emplace_back(std::make_unique<Operators::LocalHashTable>(
                sizeOfRecordRight, numPartitions, fixedPagesAllocator, pageSize, preAllocPageSizeCnt));
        }
    }
    else if (joinStrategy == QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCKING)
    {
        hashTableLeftSide.emplace_back(std::make_unique<Operators::GlobalHashTableLocking>(
            sizeOfRecordLeft, numPartitions, fixedPagesAllocator, pageSize, preAllocPageSizeCnt));

        hashTableRightSide.emplace_back(std::make_unique<Operators::GlobalHashTableLocking>(
            sizeOfRecordRight, numPartitions, fixedPagesAllocator, pageSize, preAllocPageSizeCnt));
    }
    else if (joinStrategy == QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE)
    {
        hashTableLeftSide.emplace_back(std::make_unique<Operators::GlobalHashTableLockFree>(
            sizeOfRecordLeft, numPartitions, fixedPagesAllocator, pageSize, preAllocPageSizeCnt));

        hashTableRightSide.emplace_back(std::make_unique<Operators::GlobalHashTableLockFree>(
            sizeOfRecordRight, numPartitions, fixedPagesAllocator, pageSize, preAllocPageSizeCnt));
    }
    else
    {
    ...
    NES_DEBUG(
        "Create new StreamHashJoinWindow with numberOfWorkerThreads={} HTs with numPartitions={} of pageSize={} "
