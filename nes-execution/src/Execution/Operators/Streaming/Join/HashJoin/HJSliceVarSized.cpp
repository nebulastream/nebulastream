...
HJSliceVarSized::getHashTable(QueryCompilation::JoinBuildSideType joinBuildSide,
...
uint64_t HJSliceVarSized::getNumberOfTuplesOfWorker(
    QueryCompilation::JoinBuildSideType joinBuildSide,
    WorkerThreadId workerThreadId) const {
  return getHashTable(joinBuildSide, workerThreadId)->getNumberOfTuples();
}

Operators::MergingHashTableVarSized &HJSliceVarSized::getMergingHashTable(
    QueryCompilation::JoinBuildSideType joinBuildSide) {
  if (joinBuildSide == QueryCompilation::JoinBuildSideType::Left) {
...
HJSliceVarSized::HJSliceVarSized(size_t numberOfWorker, uint64_t sliceStart,
                                 uint64_t sliceEnd, SchemaPtr &leftSchema,
                                 SchemaPtr &rightSchema,
                                 BufferManagerPtr &bufferManager,
                                 size_t pageSize, size_t numPartitions)
    : StreamSlice(sliceStart, sliceEnd),
      mergingHashTableLeftSide(
          Operators::MergingHashTableVarSized(numPartitions)),
      mergingHashTableRightSide(
          Operators::MergingHashTableVarSized(numPartitions)),
      alreadyMergedLocalToGlobalHashTable(false) {

  for (auto i = 0UL; i < numberOfWorker; ++i) {
    hashTableLeftSide.emplace_back(
        std::make_unique<Operators::StreamJoinHashTableVarSized>(
            numPartitions, bufferManager, pageSize, leftSchema));
  }
  for (auto i = 0UL; i < numberOfWorker; ++i) {
    hashTableRightSide.emplace_back(
        std::make_unique<Operators::StreamJoinHashTableVarSized>(
            numPartitions, bufferManager, pageSize, rightSchema));
  }

  NES_DEBUG("Create new StreamHashJoinWindow with numberOfWorkerThreads={} HTs "
            "with numPartitions={} of pageSize={} ",
            numberOfWorker, numPartitions, pageSize);
}
