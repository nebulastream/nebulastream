...
uint8_t *FixedPagesLinkedList::appendLocal(const uint64_t hash) {
  ...
  if (retPointer == nullptr) {
    ...
    if (++pos >= pages.size()) {
      ...
      pages.emplace_back(std::make_unique<Nautilus::Interface::FixedPage>(
          ptr, sizeOfRecord, pageSize));
    } else {
...
uint8_t *
FixedPagesLinkedList::appendConcurrentUsingLocking(const uint64_t hash) {
...
uint8_t *FixedPagesLinkedList::appendConcurrentLockFree(const uint64_t hash) {
  ...
  bool expected = false;
  if (insertInProgress.compare_exchange_strong(expected, true,
                                               std::memory_order::release,
                                               std::memory_order::relaxed)) {
    // check if in the meantime somebody else already inserts a page. if so,
    // return
    if (pos != oldPos) {
    ...
    while (!currentPage.compare_exchange_strong(oldPtr, newPtr,
                                                std::memory_order::release,
                                                std::memory_order::relaxed)) {
    }
...
FixedPagesLinkedList::getPages() const {
  return pages;
}

FixedPagesLinkedList::FixedPagesLinkedList(
    FixedPagesAllocator &fixedPagesAllocator, size_t sizeOfRecord,
    size_t pageSize, size_t preAllocPageSizeCnt)
    : pos(0), fixedPagesAllocator(fixedPagesAllocator),
      sizeOfRecord(sizeOfRecord), pageSize(pageSize), insertInProgress(false) {

  NES_ASSERT2_FMT(preAllocPageSizeCnt >= 1,
                  "We need at least one page preallocated");
  // pre allocate pages
...
std::string FixedPagesLinkedList::getStatistics() {
  std::stringstream ss;
  ss << "FixPagesLinkedList reports pageFullCnt=" << pageFullCnt
     << " allocateNewPageCnt=" << allocateNewPageCnt
