...
void MergingHashTableVarSized::insertBucket(
    size_t bucketPos, Nautilus::Interface::PagedVectorVarSizedPtr pagedVector) {
...
uint64_t MergingHashTableVarSized::getNumberOfTuplesForPage(
    size_t bucketPos, size_t pageNo) const {
  auto lockedBucketHeads = bucketHeads[bucketPos].rlock();
  NES_ASSERT2_FMT(lockedBucketHeads->get()->getNumberOfPages() > pageNo,
