...
void MergingHashTable::insertBucket(
    size_t bucketPos, const FixedPagesLinkedList *pagesLinkedList) {
  // Note that this function creates a new FixedPage (that only moves ptr)
  ...
  numPages.fetch_add(pagesLinkedList->getPages().size(),
                     std::memory_order::seq_cst);
}

Nautilus::Interface::FixedPage *
MergingHashTable::getPageFromBucketAtPos(size_t bucketPos, size_t pageNo) {
  auto lockedBucketHeads = bucketHeads[bucketPos].rlock();
  NES_ASSERT2_FMT(lockedBucketHeads->size() > pageNo,
                  "Accessing a page that does not exist!");
  return lockedBucketHeads->at(pageNo).get();
}

uint64_t MergingHashTable::getNumberOfTuplesForPage(size_t bucketPos,
                                                    size_t pageNo) {
  auto lockedBucketHeads = bucketHeads[bucketPos].rlock();
  NES_ASSERT2_FMT(lockedBucketHeads->size() > pageNo,
                  "Accessing a page that does not exist!");
...
size_t MergingHashTable::getNumItems(size_t bucketPos) const {
  return bucketNumItems[bucketPos].load();
}

size_t MergingHashTable::getNumPages(size_t bucketPos) const {
  return bucketNumPages[bucketPos].load();
}

MergingHashTable::MergingHashTable(size_t numBuckets)
    : bucketHeads(numBuckets), bucketNumItems(numBuckets),
      bucketNumPages(numBuckets) {}
