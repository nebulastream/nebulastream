...
/**
 * @brief Stores the reference to the hash table, slice start, slice end, and
 * the slice reference
 */
class LocalJoinState : public Operators::OperatorState {
public:
  LocalJoinState(Value<MemRef> &operatorHandler,
                 Value<MemRef> &hashTableReference,
                 Value<MemRef> &sliceReference)
      : joinOperatorHandler(operatorHandler),
        hashTableReference(hashTableReference), sliceReference(sliceReference),
...
void *getLocalHashTableProxy(void *ptrHashSlice, WorkerThreadId workerThreadId,
...
void HJBuildSlicing::execute(ExecutionContext &ctx, Record &record) const {
  ...
  if (!(joinState->sliceStart <= tsValue && tsValue < joinState->sliceEnd)) {
    ...
    joinState->sliceReference =
        Nautilus::FunctionCall("getHJSliceProxy", getHJSliceProxy,
                               operatorHandlerMemRef, Value<UInt64>(tsValue));

    joinState->hashTableReference = Nautilus::FunctionCall(
        "getLocalHashTableProxy", getLocalHashTableProxy,
        joinState->sliceReference, ctx.getWorkerThreadId(),
        Value<UInt64>(to_underlying(joinBuildSide)));

    joinState->sliceStart = Nautilus::FunctionCall(
        "getSliceStartProxy", getSliceStartProxy, joinState->sliceReference);
    joinState->sliceEnd = Nautilus::FunctionCall(
        "getSliceEndProxy", getSliceEndProxy, joinState->sliceReference);
  ...
  auto entryMemRef = Nautilus::FunctionCall(
      "insertFunctionProxy", insertFunctionProxy, joinState->hashTableReference,
...
void HJBuildSlicing::open(ExecutionContext &ctx,
  ...
  Value<MemRef> dummyRef1 =
      Nautilus::FunctionCall("getDefaultMemRef", getDefaultMemRef);
  Value<MemRef> dummyRef2 =
      Nautilus::FunctionCall("getDefaultMemRef", getDefaultMemRef);
  auto joinState = std::make_unique<LocalJoinState>(operatorHandlerMemRef,
                                                    dummyRef1, dummyRef2);
