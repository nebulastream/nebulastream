...
/**
 * @brief Stores the reference to the pagedVector, slice start, slice end, and
 * the slice reference
 */
class LocalHashJoinState : public OperatorState {
public:
  LocalHashJoinState(Value<MemRef> &operatorHandler,
      ...
      : joinOperatorHandler(operatorHandler),
        ...
        sliceReference(sliceReference), sliceStart(0_u64), sliceEnd(0_u64){};
...
uint64_t getSliceStartVarSizedProxy(void *ptrHashSlice) {
  NES_ASSERT2_FMT(ptrHashSlice != nullptr,
                  "hash window handler context should not be null");
...
void *getHJPagedVectorVarSizedProxy(void *ptrHashSlice,
                                    WorkerThreadId workerThreadId,
                                    uint64_t joinBuildSideInt, uint64_t key) {
  NES_ASSERT2_FMT(ptrHashSlice != nullptr,
                  "hash window handler context should not be null");
  auto *hashSlice = static_cast<HJSliceVarSized *>(ptrHashSlice);
  auto joinBuildSide =
  ...
  NES_DEBUG("Insert into HT for window={} is left={} workerThreadId={}",
            ...
            magic_enum::enum_name(joinBuildSide), workerThreadId);
  auto ptr =
      hashSlice->getHashTable(joinBuildSide, workerThreadId)->insert(key);
  auto pagedVectorVarSizedMemRef = ptr.get();
  return pagedVectorVarSizedMemRef;
}

HJBuildSlicingVarSized::HJBuildSlicingVarSized(
...
void HJBuildSlicingVarSized::execute(ExecutionContext &ctx,
                                     Record &record) const {
  ...
  if (!(joinState->sliceStart <= tsValue && tsValue < joinState->sliceEnd)) {
    joinState->sliceReference =
        FunctionCall("getHJSliceVarSizedProxy", getHJSliceVarSizedProxy,
                     operatorHandlerMemRef, Value<UInt64>(tsValue));

    joinState->sliceStart =
        FunctionCall("getSliceStartVarSizedProxy", getSliceStartVarSizedProxy,
                     joinState->sliceReference);
    joinState->sliceEnd =
        FunctionCall("getSliceEndVarSizedProxy", getSliceEndVarSizedProxy,
                     joinState->sliceReference);

    NES_DEBUG(
        "reinit join state with start={} end={} for ts={} for isLeftSide={}",
        joinState->sliceStart->toString(), joinState->sliceEnd->toString(),
        tsValue->toString(), to_underlying(joinBuildSide));
  }

  // Write record to the pagedVector
  auto hjPagedVectorMemRef = FunctionCall(
      "getHJPagedVectorVarSizedProxy", getHJPagedVectorVarSizedProxy,
      joinState->sliceReference, ctx.getWorkerThreadId(),
      Value<UInt64>(to_underlying(joinBuildSide)),
      record.read(joinFieldName).as<UInt64>());

  Interface::PagedVectorVarSizedRef pagedVectorVarSizedRef(hjPagedVectorMemRef,
                                                           schema);
...
void HJBuildSlicingVarSized::open(ExecutionContext &ctx,
  ...
  Value<MemRef> dummyRef1 =
      FunctionCall("getDefaultMemRef", getDefaultMemRefVarSized);
  Value<MemRef> dummyRef2 =
      FunctionCall("getDefaultMemRef", getDefaultMemRefVarSized);
  auto joinState = std::make_unique<LocalHashJoinState>(operatorHandlerMemRef,
                                                        dummyRef1, dummyRef2);
