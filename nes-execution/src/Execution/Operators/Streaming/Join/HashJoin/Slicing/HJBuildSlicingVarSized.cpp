...
void* getHJPagedVectorVarSizedProxy(void* ptrHashSlice, uint32_t workerThreadId, uint64_t joinBuildSideInt, uint64_t key) {
    NES_ASSERT2_FMT(ptrHashSlice != nullptr, "hash window handler context should not be null");
    auto* hashSlice = static_cast<HJSliceVarSized*>(ptrHashSlice);
    ...
    NES_DEBUG("Insert into HT for window={} is left={} workerThreadId={}",
              ...
              workerThreadId);
    auto ptr = hashSlice->getHashTable(joinBuildSide, WorkerThreadId(workerThreadId))->insert(key);
