...
void* getHJPagedVectorVarSizedProxy(void* ptrHashSlice, WorkerThreadId workerThreadId, uint64_t joinBuildSideInt, uint64_t key)
{
    ...
    NES_DEBUG(
        "Insert into HT for window={} is left={} workerThreadId={}",
...
void HJBuildSlicingVarSized::execute(ExecutionContext& ctx, Record& record) const
{
    ...
    {
        joinState->sliceReference = FunctionCall(
            "getHJSliceVarSizedProxy", getHJSliceVarSizedProxy, operatorHandlerMemRef, Value<UInt64>(tsValue), ctx.getPipelineContext());
