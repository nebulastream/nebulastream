...
uint64_t getHJSliceIdVarSizedProxy(void *ptrJoinPartitionIdSliceId,
...
void *getHashSliceRefFromIdVarSizedProxy(void *ptrOpHandler,
...
void *getHJBucketAtPosVarSizedProxy(void *hashSlicePtr,
                                    uint64_t joinBuildSideInt,
                                    uint64_t bucketPos) {
  ...
  return hashSlice->getMergingHashTable(joinBuildSide)
      .getBucketAtPos(bucketPos);
}

void HJProbeVarSized::open(ExecutionContext &ctx,
  ...
  const auto windowStart =
      FunctionCall("getHJWindowStartVarSizedProxy",
                   getHJWindowStartVarSizedProxy, joinPartitionIdSliceIdMemRef);
  const auto windowEnd =
      FunctionCall("getHJWindowEndVarSizedProxy", getHJWindowEndVarSizedProxy,
                   joinPartitionIdSliceIdMemRef);
  const auto partitionId =
      FunctionCall("getPartitionIdVarSizedProxy", getPartitionIdVarSizedProxy,
                   joinPartitionIdSliceIdMemRef);

  const auto sliceIdLeft = FunctionCall(
      "getHJSliceIdVarSizedProxy", getHJSliceIdVarSizedProxy,
      joinPartitionIdSliceIdMemRef,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Left)));
  const auto sliceIdRight = FunctionCall(
      "getHJSliceIdVarSizedProxy", getHJSliceIdVarSizedProxy,
      joinPartitionIdSliceIdMemRef,
  ...
  const ValueId<WorkerThreadId> workerThreadIdForPagedVectors(
      INITIAL<WorkerThreadId>);

  const auto hashSliceRefLeft = FunctionCall(
      "getHashSliceRefFromIdVarSizedProxy", getHashSliceRefFromIdVarSizedProxy,
      operatorHandlerMemRef, sliceIdLeft);
  const auto hashSliceRefRight = FunctionCall(
      "getHashSliceRefFromIdVarSizedProxy", getHashSliceRefFromIdVarSizedProxy,
      operatorHandlerMemRef, sliceIdRight);

  const auto leftPagedVectorRef = FunctionCall(
      "getHJBucketAtPosVarSizedProxy", getHJBucketAtPosVarSizedProxy,
      hashSliceRefLeft,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Left)),
      partitionId);
  const auto rightPagedVectorRef = FunctionCall(
      "getHJBucketAtPosVarSizedProxy", getHJBucketAtPosVarSizedProxy,
      hashSliceRefRight,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Right)),
      partitionId);

  Interface::PagedVectorVarSizedRef leftPagedVector(leftPagedVectorRef,
                                                    leftSchema);
  Interface::PagedVectorVarSizedRef rightPagedVector(rightPagedVectorRef,
                                                     rightSchema);
...
HJProbeVarSized::HJProbeVarSized(
    const uint64_t operatorHandlerIndex, const JoinSchema &joinSchema,
    Expressions::ExpressionPtr joinExpression,
    const WindowMetaData &windowMetaData, const SchemaPtr &leftSchema,
    const SchemaPtr &rightSchema,
    QueryCompilation::StreamJoinStrategy joinStrategy,
    QueryCompilation::WindowingStrategy windowingStrategy, bool withDeletion)
