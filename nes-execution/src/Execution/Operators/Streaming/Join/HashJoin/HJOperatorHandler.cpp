...
StreamSlicePtr HJOperatorHandler::createNewSlice(uint64_t sliceStart,
                                                 uint64_t sliceEnd) {
  switch (joinStrategy) {
    case QueryCompilation::StreamJoinStrategy::HASH_JOIN_VAR_SIZED:
      return std::make_shared<HJSliceVarSized>(
          numberOfWorkerThreads, sliceStart, sliceEnd, leftSchema, rightSchema,
          bufferManager, pageSize, numPartitions);
    ...
    case QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE:
      return std::make_shared<HJSlice>(
          numberOfWorkerThreads, sliceStart, sliceEnd, sizeOfRecordLeft,
          sizeOfRecordRight, totalSizeForDataStructures, pageSize,
          preAllocPageSizeCnt, numPartitions, joinStrategy);
  ...
  if (sliceLeft.getNumberOfTuplesLeft() > 0 &&
    ...
    switch (joinStrategy) {
      case QueryCompilation::StreamJoinStrategy::HASH_JOIN_VAR_SIZED:
        ...
        dynamic_cast<HJSliceVarSized&>(sliceRight)
            .mergeLocalToGlobalHashTable();
        break;
      case QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL:
      case QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCKING:
      case QueryCompilation::StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE:
    ...
    for (auto i = 0UL; i < getNumPartitions(); ++i) {
      ...
      /** As we are here "emitting" a buffer, we have to set the originId, the
       * seq number, and the watermark. As we emit one buffer for each
       * partition, the watermark can not be the slice end as some buffer might
       * be still waiting for getting processed.
       */
      ...
      buffer.setSequenceData(
          {getNextSequenceNumber(), /*chunkNumber*/ 1, true});
      buffer.setWatermark(watermark);

      pipelineCtx->dispatchBuffer(buffer);
      NES_INFO(
          "Emitted leftSliceId {} rightSliceId {} with watermarkTs {} "
          "sequenceNumber {} originId {} for no. left "
          "tuples {} and no. right tuples {}",
          bufferAs->sliceIdentifierLeft, bufferAs->sliceIdentifierRight,
          buffer.getWatermark(), buffer.getSequenceNumber(),
          buffer.getOriginId(), sliceLeft.getNumberOfTuplesLeft(),
...
void* insertFunctionProxy(void* ptrLocalHashTable, uint64_t key) {
  NES_ASSERT2_FMT(ptrLocalHashTable != nullptr,
                  "ptrLocalHashTable should not be null");
  LocalHashTable* localHashTable =
      static_cast<LocalHashTable*>(ptrLocalHashTable);
