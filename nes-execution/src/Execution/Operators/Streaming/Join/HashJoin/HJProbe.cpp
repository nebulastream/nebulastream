...
uint64_t getWindowStartProxyForHashJoin(void *ptrJoinPartitionIdSliceId) {
  NES_ASSERT2_FMT(ptrJoinPartitionIdSliceId != nullptr,
                  "join partition id slice id should not be null");
  auto probeInfo =
      static_cast<JoinPartitionIdSliceIdWindow *>(ptrJoinPartitionIdSliceId);
  NES_INFO("Probing for leftId {} rightId {} partitionId {} windowStart {} "
           "windowEnd {}",
           probeInfo->sliceIdentifierLeft, probeInfo->sliceIdentifierRight,
           probeInfo->partitionId, probeInfo->windowInfo.windowStart,
           probeInfo->windowInfo.windowEnd);

  return static_cast<JoinPartitionIdSliceIdWindow *>(ptrJoinPartitionIdSliceId)
...
uint64_t getPartitionIdProxy(void *ptrJoinPartitionIdSliceId) {
  NES_ASSERT2_FMT(ptrJoinPartitionIdSliceId != nullptr,
                  "ptrJoinPartitionIdSliceId should not be null");
  return static_cast<JoinPartitionIdSliceIdWindow *>(ptrJoinPartitionIdSliceId)
      ->partitionId;
}

uint64_t getSliceIdHJProxy(void *ptrJoinPartitionIdSliceId,
  ...
  case QueryCompilation::JoinBuildSideType::Left:
    return static_cast<JoinPartitionIdSliceIdWindow *>(
               ptrJoinPartitionIdSliceId)
        ->sliceIdentifierLeft;
  case QueryCompilation::JoinBuildSideType::Right:
    return static_cast<JoinPartitionIdSliceIdWindow *>(
               ptrJoinPartitionIdSliceId)
        ->sliceIdentifierRight;
  }
}
...
void *getPageFromBucketAtPosProxyForHashJoin(void *hashSlicePtr,
  ...
  return hashSlice->getMergingHashTable(joinBuildSide)
      .getPageFromBucketAtPos(bucketPos, pageNo);
}

uint64_t getNumberOfPagesProxyForHashJoin(void *ptrHashSlice,
                                          uint64_t joinBuildSideInt,
                                          uint64_t bucketPos) {
  ...
  const auto joinBuildSide =
      magic_enum::enum_cast<QueryCompilation::JoinBuildSideType>(
          joinBuildSideInt)
          .value();
  auto numPages =
      hashSlice->getMergingHashTable(joinBuildSide).getNumPages(bucketPos);
  auto numItems =
      hashSlice->getMergingHashTable(joinBuildSide).getNumItems(bucketPos);

  NES_INFO("numPages: {}, numItems {} for slide start {} end {}", numPages,
           numItems, hashSlice->getSliceStart(), hashSlice->getSliceEnd());
  return numPages;
}

uint64_t getNumberOfTuplesForPageProxy(void *hashWindowPtr,
                                       uint64_t joinBuildSideInt,
                                       uint64_t bucketPos, uint64_t pageNo) {
  ...
  const auto joinBuildSide =
      magic_enum::enum_cast<QueryCompilation::JoinBuildSideType>(
          joinBuildSideInt)
          .value();
  return hashWindow->getMergingHashTable(joinBuildSide)
      .getNumberOfTuplesForPage(bucketPos, pageNo);
}
...
void HJProbe::open(ExecutionContext &ctx, RecordBuffer &recordBuffer) const {
  ...
  const auto windowStart = Nautilus::FunctionCall(
      "getWindowStartProxyForHashJoin", getWindowStartProxyForHashJoin,
      joinPartitionIdSliceIdMemRef);
  const auto windowEnd = Nautilus::FunctionCall("getWindowEndProxyForHashJoin",
                                                getWindowEndProxyForHashJoin,
                                                joinPartitionIdSliceIdMemRef);
  const auto partitionId = Nautilus::FunctionCall(
      "getPartitionIdProxy", getPartitionIdProxy, joinPartitionIdSliceIdMemRef);
  const auto sliceIdLeft = Nautilus::FunctionCall(
      "getSliceIdHJProxy", getSliceIdHJProxy, joinPartitionIdSliceIdMemRef,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Left)));
  const auto sliceIdRight = Nautilus::FunctionCall(
      "getSliceIdHJProxy", getSliceIdHJProxy, joinPartitionIdSliceIdMemRef,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Right)));

  const auto hashSliceRefLeft =
      Nautilus::FunctionCall("getHashSliceProxy", getHashSliceProxy,
                             operatorHandlerMemRef, sliceIdLeft);
  const auto hashSliceRefRight =
      Nautilus::FunctionCall("getHashSliceProxy", getHashSliceProxy,
                             operatorHandlerMemRef, sliceIdRight);
  const auto numberOfPagesLeft = Nautilus::FunctionCall(
      "getNumberOfPagesProxyForHashJoin", getNumberOfPagesProxyForHashJoin,
      hashSliceRefLeft,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Left)),
      partitionId);
  const auto numberOfPagesRight = Nautilus::FunctionCall(
      "getNumberOfPagesProxyForHashJoin", getNumberOfPagesProxyForHashJoin,
      hashSliceRefRight,
      Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Right)),
      partitionId);
  ...
  // for every left page
  for (Value<UInt64> leftPageNo(0_u64); leftPageNo < numberOfPagesLeft;
       leftPageNo = leftPageNo + 1) {
    // for every key in left page
    auto leftPageRef = Nautilus::FunctionCall(
        "getPageFromBucketAtPosProxyForHashJoin",
        getPageFromBucketAtPosProxyForHashJoin, hashSliceRefLeft,
        Value<UInt64>(to_underlying(QueryCompilation::JoinBuildSideType::Left)),
        partitionId, leftPageNo);
    ...
    for (auto leftRecordRef : leftFixedPageRef) {
      ...
      for (Value<UInt64> rightPageNo((uint64_t)0);
           rightPageNo < numberOfPagesRight; rightPageNo = rightPageNo + 1) {
        // TODO: introduce Bloomfilter here #3909
        //                 if (!rhsPage->bloomFilterCheck(lhsKeyPtr,
        //                 sizeOfLeftKey)) {
        //                     continue;
        //                 }
        ...
        auto rightPageRef = Nautilus::FunctionCall(
            "getPageFromBucketAtPosProxyForHashJoin",
            getPageFromBucketAtPosProxyForHashJoin, hashSliceRefRight,
            Value<UInt64>(
                to_underlying(QueryCompilation::JoinBuildSideType::Right)),
            partitionId, rightPageNo);
        Nautilus::Interface::FixedPageRef rightFixedPageRef(rightPageRef);

        for (auto rightRecordRef : rightFixedPageRef) {
          auto rightRecord =
              rightMemProvider->read({}, rightRecordRef, zeroValue);
...
HJProbe::HJProbe(const uint64_t operatorHandlerIndex,
                 ...
                 bool withDeletion)
    : StreamJoinProbe(operatorHandlerIndex, joinSchema, joinExpression,
                      windowMetaData, joinStrategy, windowingStrategy,
                      withDeletion),
      // As we are only ever reading a single record, we do not care about the
      // buffer size
      leftMemProvider(
          Runtime::Execution::MemoryProvider::MemoryProvider::
              createMemoryProvider(/*bufferSize*/ 1, joinSchema.leftSchema)),
      rightMemProvider(
          Runtime::Execution::MemoryProvider::MemoryProvider::
              createMemoryProvider(/*bufferSize*/ 1, joinSchema.rightSchema)) {}
} // namespace NES::Runtime::Execution::Operators
