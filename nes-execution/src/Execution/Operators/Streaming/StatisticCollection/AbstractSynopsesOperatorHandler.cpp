...
void AbstractSynopsesOperatorHandler::stop(QueryTerminationType terminationType, PipelineExecutionContextPtr pipelineCtx)
{
    if (terminationType == QueryTerminationType::Graceful)
    {
        ...
        for (auto & statisticStore : operatorStatisticStores)
        {
            ...
            allStatisticsPlusHashesToSend.insert(
                allStatisticsPlusHashesToSend.end(), statisticsPlusHashesToSend.begin(), statisticsPlusHashesToSend.end());
        }
...
AbstractSynopsesOperatorHandler::getStatistic(WorkerThreadId workerThreadId, Statistic::StatisticHash statisticHash, uint64_t timestamp)
{
...
void AbstractSynopsesOperatorHandler::checkStatisticsSending(
    const BufferMetaData & bufferMetaData, Statistic::StatisticHash statisticHash, PipelineExecutionContext * pipelineCtx)
{
    ...
    for (auto & statisticStore : operatorStatisticStores)
    {
        ...
        std::transform(
            statisticsToSend.begin(),
            statisticsToSend.end(),
            std::back_inserter(allStatisticsPlusHashesToSend),
            [statisticHash](const Statistic::StatisticPtr statistic) { return std::make_pair(statisticHash, statistic); });
    }
...
AbstractSynopsesOperatorHandler::mergeStatistics(const std::vector<Statistic::HashStatisticPair> & statisticsPlusHashes)
{
    ...
    struct MergeStatisticKey
    {
        ...
        /**
          ...
          */
        MergeStatisticKey(
            Statistic::StatisticHash statisticHash, const Windowing::TimeMeasure & startTs, const Windowing::TimeMeasure & endTs)
