...
void DDSketchBuild::execute(ExecutionContext& ctx, Record& record) const {
    ...
    if (lessThanZero->execute(record)) {
        ...
        FunctionCall("addValueToDDSketchProxy", addValueToDDSketchProxy, ddsketchRef, logFloorIndex, logFloorIndexOffSet, value.as<UInt64>());
        ...
        FunctionCall("addValueToDDSketchProxy", addValueToDDSketchProxy, ddsketchRef, logFloorIndex, Value<Double>(0.0), value.as<UInt64>());
    }
}
