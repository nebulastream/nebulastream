...
void *getCountMinRefProxy(void *ptrOpHandler,
                          ...
                          WorkerThreadId workerThreadId, uint64_t timestamp) {
...
void *getH3SeedsProxy(void *ptrOpHandler) {
  ...
  return reinterpret_cast<int8_t *>(
      const_cast<uint64_t *>(opHandler->getH3Seeds().data()));
}
...
void checkCountMinSketchesSendingProxy(
    void *ptrOpHandler, void *ptrPipelineCtx, uint64_t watermarkTs,
    uint64_t sequenceNumber, uint64_t chunkNumber, bool lastChunk,
    uint64_t originId, Statistic::StatisticMetricHash metricHash,
    StatisticId statisticId) {
  NES_ASSERT2_FMT(ptrOpHandler != nullptr,
                  "opHandler context should not be null!");
...
void CountMinBuild::execute(ExecutionContext &ctx, Record &record) const {
  ...
  auto countMinMemRef = Nautilus::FunctionCall(
      "getCountMinRefProxy", getCountMinRefProxy, operatorHandlerMemRef,
  ...
  auto h3SeedsMemRef = Nautilus::FunctionCall(
      "getH3SeedsProxy", getH3SeedsProxy, operatorHandlerMemRef);
  for (Value<UInt64> row(0_u64); row < depth; row = row + 1_u64) {
    ...
    Value<UInt64> calcHash =
        h3HashFunction->calculateWithState(valToTrack, h3SeedsThisRow);
    Value<UInt64> col = (calcHash % Value<UInt64>(width)).as<UInt64>();

    // 2.3. Having this function call for each record for each row is not
    // efficient. But we take care of the efficiency later
    Nautilus::FunctionCall("updateCountMinProxy", updateCountMinProxy,
                           countMinMemRef, row, col);
  }
}

void CountMinBuild::open(ExecutionContext &executionCtx,
...
void CountMinBuild::close(ExecutionContext &ctx,
                          RecordBuffer &recordBuffer) const {
  // Update the watermark for the count min build operator and send the created
  // statistics upward
  auto operatorHandlerMemRef =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  Nautilus::FunctionCall(
      "checkCountMinSketchesSendingProxy", checkCountMinSketchesSendingProxy,
...
CountMinBuild::CountMinBuild(const uint64_t operatorHandlerIndex,
                             ...
                             const uint64_t width, const uint64_t depth,
    ...
    : operatorHandlerIndex(operatorHandlerIndex),
      ...
      width(width), depth(depth), metricHash(metricHash),
