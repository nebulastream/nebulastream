...
SlidingWindowSliceStore<SliceType>::SlidingWindowSliceStore(
    uint64_t windowSize, uint64_t windowSlide)
    : windowSize(windowSize), windowSlide(windowSlide){};

template <class SliceType>
void SlidingWindowSliceStore<SliceType>::insertSlice(
    SlidingWindowSliceStore::SliceTypePtr slice) {
  const std::lock_guard<std::mutex> lock(sliceStagingMutex);
  auto sliceStart = slice->getStart();
  auto sliceIter = slices.rbegin();
  // reverse iterate from the end of the slice store to the front as long as the
  // slice.start > currentSlice.start
  while (sliceIter != slices.rend() && (*sliceIter)->getStart() > sliceStart) {
    sliceIter++;
  }
  if (sliceIter == slices.rend()) {
    // we reached the front of all slices -> so insert in the front of the slice
    // store
    ...
    throw WindowProcessingException(
        "Could not find the correct slot in the global slice store.");
  }
}

template <class SliceType>
typename SlidingWindowSliceStore<SliceType>::SliceTypePtr &
SlidingWindowSliceStore<SliceType>::getFirstSlice() {
  const std::lock_guard<std::mutex> lock(sliceStagingMutex);
  return slices.front();
}
template <class SliceType>
typename SlidingWindowSliceStore<SliceType>::SliceTypePtr &
SlidingWindowSliceStore<SliceType>::getLastSlice() {
  const std::lock_guard<std::mutex> lock(sliceStagingMutex);
  return slices.back();
}
template <class SliceType>
std::set<std::tuple<uint64_t, uint64_t>>
SlidingWindowSliceStore<SliceType>::collectWindows(uint64_t oldTs,
                                                   uint64_t currentTs) {
  const std::lock_guard<std::mutex> lock(sliceStagingMutex);
  ...
  for (auto &slice : slices) {
    ...
    if ((windowEnd > oldTs) && (windowEnd <= currentTs) &&
...
template <class SliceType>
std::set<std::tuple<uint64_t, uint64_t>>
SlidingWindowSliceStore<SliceType>::collectAllWindows(uint64_t oldTs) {
  const std::lock_guard<std::mutex> lock(sliceStagingMutex);
  std::set<std::tuple<uint64_t, uint64_t>> windows;
  // collect all windows, for which the list of slices contains the slice end.
  // iterate over all slices that would open a window that ends before endTs.
...
SlidingWindowSliceStore<SliceType>::collectSlicesForWindow(uint64_t windowStart,
                                                           uint64_t windowEnd) {
  const std::lock_guard<std::mutex> lock(sliceStagingMutex);
  std::vector<SliceTypePtr> windowSlices;
  // iterate over all slices that are contained in the window that ends before
  // endTs.
  for (auto &slice : slices) {
