...
KeyedBucketPreAggregationHandler::KeyedBucketPreAggregationHandler(
    ...
    : AbstractBucketPreAggregationHandler<KeyedSlice, KeyedBucketStore>(
          windowSize, windowSlide, origins) {}

void KeyedBucketPreAggregationHandler::setup(
    Runtime::Execution::PipelineExecutionContext& ctx, uint64_t keySize,
    uint64_t valueSize) {
  NES_ASSERT(threadLocalBuckets.empty(),
             "The thread local slice store must be empty");
  for (uint64_t i = 0; i < ctx.getNumberOfWorkerThreads(); i++) {
    auto threadLocal = std::make_unique<KeyedBucketStore>(
