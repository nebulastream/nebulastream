...
void triggerKeyedBucketsProxy(void *op, void *wctx, void *pctx,
                              uint64_t originId, uint64_t sequenceNumber,
...
void setupKeyedBucketWindowHandler(void *ss, void *ctx, uint64_t keySize,
                                   uint64_t valueSize) {
...
public:
  explicit LocalKeyedBucketStoreState(
      ...
      uint64_t valueSize, const Value<MemRef> &sliceStoreState)
...
KeyedBucketPreAggregation::KeyedBucketPreAggregation(
    ...
    : operatorHandlerIndex(operatorHandlerIndex),
      timeFunction(std::move(timeFunction)), keyExpressions(keyExpressions),
      keyDataTypes(keyDataTypes), aggregationFunctions(aggregationFunctions),
...
void KeyedBucketPreAggregation::setup(ExecutionContext &executionCtx) const {
  ...
  Nautilus::FunctionCall("setupKeyedBucketWindowHandler",
                         setupKeyedBucketWindowHandler, globalOperatorHandler,
                         executionCtx.getPipelineContext(),
                         Value<UInt64>(keySize), Value<UInt64>(valueSize));
}

void KeyedBucketPreAggregation::open(ExecutionContext &ctx,
...
void KeyedBucketPreAggregation::execute(
  ...
  auto state =
      reinterpret_cast<LocalKeyedBucketStoreState *>(ctx.getLocalState(this));

  auto buckets =
      Nautilus::FunctionCall("findKeyedBucketsByTs", findKeyedBucketsByTs,
                             state->sliceStoreState, timestampValue);
  auto numberOfBuckets = Nautilus::FunctionCall(
      "getKeyedBucketListSize", getKeyedBucketListSize, buckets);
  for (Value<UInt64> i = 0_u64; i < numberOfBuckets; i = i + 1_u64) {
    auto bucketState =
        Nautilus::FunctionCall("getKeyedBucket", getKeyedBucket, buckets, i);
    auto map = Interface::ChainedHashMapRef(bucketState, keyDataTypes, keySize,
                                            valueSize);
    // 4. calculate hash
    auto hash = hashFunction->calculate(keyValues);

    // 5. create entry in the slice hash map. If the entry is new set default
...
void KeyedBucketPreAggregation::close(ExecutionContext &ctx,
                                      RecordBuffer &) const {
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);

  // After we processed all records in the record buffer we call
  // triggerKeyedThreadLocalWindow with the current watermark ts to check if we
  // can trigger a window.
  Nautilus::FunctionCall("triggerKeyedBucketsProxy", triggerKeyedBucketsProxy,
