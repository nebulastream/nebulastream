...
void* createKeyedState(void* op, void* sliceMergeTaskPtr) {
  ...
  auto sliceMergeTask =
      static_cast<SliceMergeTask<KeyedSlice>*>(sliceMergeTaskPtr);
...
void setupKeyedSliceMergingHandler(void* ss, void* ctx, uint64_t keySize,
                                   uint64_t valueSize) {
...
KeyedSliceMerging::KeyedSliceMerging(
...
void KeyedSliceMerging::setup(ExecutionContext& executionCtx) const {
  ...
  Nautilus::FunctionCall("setupSliceMergingHandler",
                         setupKeyedSliceMergingHandler, globalOperatorHandler,
                         executionCtx.getPipelineContext(),
                         Value<UInt64>(keySize), Value<UInt64>(valueSize));
  if (this->child != nullptr) this->child->setup(executionCtx);
}

void KeyedSliceMerging::open(ExecutionContext& ctx,
                             RecordBuffer& buffer) const {
  ...
  if (this->child != nullptr) this->child->open(ctx, buffer);

  // 1. get the operator handler and extract the slice information that should
  // be combined.
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  auto sliceMergeTask = buffer.getBuffer();
  auto startSliceTs =
      getMember(sliceMergeTask, SliceMergeTask<KeyedSlice>, startSlice)
          .load<UInt64>();
  auto endSliceTs =
      getMember(sliceMergeTask, SliceMergeTask<KeyedSlice>, endSlice)
          .load<UInt64>();
  auto sequenceNumber =
      getMember(sliceMergeTask, SliceMergeTask<KeyedSlice>, sequenceNumber)
          .load<UInt64>();
  auto chunkNumber =
      getMember(sliceMergeTask, SliceMergeTask<KeyedSlice>, chunkNumber)
          .load<UInt64>();
  auto lastChunk =
      getMember(sliceMergeTask, SliceMergeTask<KeyedSlice>, lastChunk)
          .load<Boolean>();

  // 2. initialize global slice state, which is represented by a chained
  // hashtable
  auto globalSlice =
      Nautilus::FunctionCall("createKeyedState", createKeyedState,
                             globalOperatorHandler, sliceMergeTask);
  ...
  FunctionCall("freeKeyedSliceMergeTask", freeKeyedSliceMergeTask,
               sliceMergeTask);

  // 4. emit global slice when we have a tumbling window.
  sliceMergingAction->emitSlice(ctx, child, startSliceTs, endSliceTs,
                                sequenceNumber, chunkNumber, lastChunk,
                                globalSlice);
}

void KeyedSliceMerging::combineThreadLocalSlices(
  ...
  auto numberOfSlices =
      Nautilus::FunctionCall("getKeyedNumberOfSlicesFromTask",
                             getKeyedNumberOfSlicesFromTask, sliceMergeTask);
  NES_DEBUG("combine {} slices", numberOfSlices->toString());

  for (Value<UInt64> i = 0_u64; i < numberOfSlices; i = i + 1_u64) {
    auto partitionState =
        Nautilus::FunctionCall("getKeyedSliceStateFromTask",
                               getKeyedSliceStateFromTask, sliceMergeTask, i);
    auto partitionStateHashTable = Interface::ChainedHashMapRef(
        partitionState, keyDataTypes, keySize, valueSize);
    mergeHashTable(globalHashTable, partitionStateHashTable);
  }
}

void KeyedSliceMerging::mergeHashTable(
    Interface::ChainedHashMapRef& globalSliceHashMap,
    Interface::ChainedHashMapRef& threadLocalSliceHashMap) const {
  // inserts all entries from the thread local hash map into the global hash
  // map.
  ...
  for (const auto& threadLocalEntry : threadLocalSliceHashMap) {
    ...
    globalSliceHashMap.insertEntryOrUpdate(
        threadLocalEntry, [&](auto& globalEntry) {
          // 2b. update aggregation if the entry was already existing in the
          // global hash map
