...
void triggerKeyedThreadLocalWindow(void *op, void *wctx, void *pctx,
...
void setupWindowHandler2(void *ss, void *ctx, uint64_t keySize,
                         uint64_t valueSize) {
...
public:
  ...
  auto findSliceStateByTs(Value<UInt64> &timestampValue) {
    auto htPtr = Nautilus::FunctionCall("findKeyedSliceStateByTsProxy",
                                        findKeyedSliceStateByTsProxy,
                                        sliceStoreState, timestampValue);
    return Interface::ChainedHashMapRef(htPtr, keyDataTypes, keySize,
                                        valueSize);
  }
...
void KeyedSlicePreAggregation::setup(ExecutionContext &executionCtx) const {
  ...
  Nautilus::FunctionCall("setupWindowHandler", setupWindowHandler2,
                         globalOperatorHandler,
                         executionCtx.getPipelineContext(),
                         Value<UInt64>(keySize), Value<UInt64>(valueSize));
}

void KeyedSlicePreAggregation::open(ExecutionContext &ctx,
...
void KeyedSlicePreAggregation::execute(
  ...
  auto sliceStore =
      reinterpret_cast<LocalKeyedSliceStoreState *>(ctx.getLocalState(this));
...
void KeyedSlicePreAggregation::close(ExecutionContext &ctx,
                                     RecordBuffer &) const {
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  ...
  Nautilus::FunctionCall(
      "triggerKeyedThreadLocalWindow", triggerKeyedThreadLocalWindow,
      globalOperatorHandler, ctx.getWorkerContext(), ctx.getPipelineContext(),
      ctx.getOriginId(), ctx.getSequenceNumber(), ctx.getChunkNumber(),
