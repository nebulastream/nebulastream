...
void KeyedSliceMergingHandler::setup(
    Runtime::Execution::PipelineExecutionContext &, uint64_t keySize,
    uint64_t valueSize) {
  this->keySize = keySize;
  this->valueSize = valueSize;
}

void KeyedSliceMergingHandler::start(
    Runtime::Execution::PipelineExecutionContextPtr, uint32_t) {
  NES_DEBUG("start GlobalSliceMergingHandler");
}

void KeyedSliceMergingHandler::stop(
    Runtime::QueryTerminationType queryTerminationType,
    Runtime::Execution::PipelineExecutionContextPtr) {
  NES_DEBUG("stop GlobalSliceMergingHandler: {}", queryTerminationType);
}

KeyedSlicePtr KeyedSliceMergingHandler::createGlobalSlice(
    SliceMergeTask<KeyedSlice> *sliceMergeTask, uint64_t numberOfKeys) {
  // allocate hash map
  ...
  return std::make_unique<KeyedSlice>(
      std::move(hashMap), sliceMergeTask->startSlice, sliceMergeTask->endSlice);
}
KeyedSliceMergingHandler::~KeyedSliceMergingHandler() {
