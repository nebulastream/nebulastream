...
template <class Slice> void appendToGlobalSliceStore(void *ss, void *slicePtr) {
...
void triggerSlidingWindows(void *sh, void *wctx, void *pctx,
  ...
  handler->triggerSlidingWindows(*workerContext, *pipelineExecutionContext,
                                 {sequenceNumber, chunkNumber, lastChunk},
                                 sliceEnd);
}

template <class Slice>
AppendToSliceStoreHandler<Slice>::AppendToSliceStoreHandler(
  ...
  sliceStore =
      std::make_unique<SlidingWindowSliceStore<Slice>>(windowSize, windowSlide);
}

template <class Slice>
void AppendToSliceStoreHandler<Slice>::appendToGlobalSliceStore(
...
void AppendToSliceStoreHandler<Slice>::triggerSlidingWindows(
    Runtime::WorkerContext &wctx,
    Runtime::Execution::PipelineExecutionContext &ctx,
    SequenceData sequenceNumber, uint64_t slideEnd) {
  ...
  // current watermark.
  auto currentWatermark = watermarkProcessor->updateWatermark(
      slideEnd, sequenceNumber, INVALID_ORIGIN_ID);
  ...
  // lock
  NES_TRACE("Trigger sliding windows between {}-{}", lastTriggerWatermark,
            currentWatermark);

  auto windows =
      sliceStore->collectWindows(lastTriggerWatermark, currentWatermark);
  ...
  for (const auto &[windowStart, windowEnd] : windows) {
    auto slicesForWindow =
        sliceStore->collectSlicesForWindow(windowStart, windowEnd);
    if (slicesForWindow.empty()) {
      continue;
    }
    NES_TRACE("Deploy window ({}-{}) merge task for {} slices  ", windowStart,
              windowEnd, slicesForWindow.size());
...
void AppendToSliceStoreHandler<Slice>::stop(
    NES::Runtime::QueryTerminationType queryTerminationType,
...
template <class Slice>
AppendToSliceStoreAction<Slice>::AppendToSliceStoreAction(
    const uint64_t operatorHandlerIndex)
    : operatorHandlerIndex(operatorHandlerIndex) {}

template <class Slice>
void AppendToSliceStoreAction<Slice>::emitSlice(
    ExecutionContext &ctx, ExecuteOperatorPtr &, Value<UInt64> &,
    Value<UInt64> &sliceEnd, Value<UInt64> &sequenceNumber,
  ...
  FunctionCall("appendToGlobalSliceStore", appendToGlobalSliceStore<Slice>,
               actionHandler, combinedSlice);
  FunctionCall("triggerSlidingWindows", triggerSlidingWindows<Slice>,
               actionHandler, ctx.getWorkerContext(), ctx.getPipelineContext(),
               sequenceNumber, chunkNumber, lastChunk, sliceEnd);
}
