...
void* createGlobalState(void* op, void* sliceMergeTaskPtr) {
  ...
  auto sliceMergeTask =
      static_cast<SliceMergeTask<NonKeyedSlice>*>(sliceMergeTaskPtr);
...
void NonKeyedSliceMerging::setup(ExecutionContext& executionCtx) const {
  ...
  Nautilus::FunctionCall("setupSliceMergingHandler", setupSliceMergingHandler,
                         globalOperatorHandler,
                         executionCtx.getPipelineContext(), entrySize);
  auto defaultState = Nautilus::FunctionCall(
      "getDefaultMergingState", getDefaultMergingState, globalOperatorHandler);
  for (auto& function : aggregationFunctions) {
...
void NonKeyedSliceMerging::open(ExecutionContext& ctx,
  ...
  auto startSliceTs =
      getMember(sliceMergeTask, SliceMergeTask<NonKeyedSlice>, startSlice)
          .load<UInt64>();
  auto endSliceTs =
      getMember(sliceMergeTask, SliceMergeTask<NonKeyedSlice>, endSlice)
          .load<UInt64>();
  auto sequenceNumber =
      getMember(sliceMergeTask, SliceMergeTask<NonKeyedSlice>, sequenceNumber)
          .load<UInt64>();
  auto chunkNumber =
      getMember(sliceMergeTask, SliceMergeTask<NonKeyedSlice>, chunkNumber)
          .load<UInt64>();
  auto lastChunk =
      getMember(sliceMergeTask, SliceMergeTask<NonKeyedSlice>, lastChunk)
          .load<Boolean>();
  // 2. load the thread local slice store according to the worker id.
  auto combinedSlice =
      combineThreadLocalSlices(globalOperatorHandler, sliceMergeTask);
  FunctionCall("freeNonKeyedSliceMergeTask", freeNonKeyedSliceMergeTask,
               sliceMergeTask);
...
Value<MemRef> NonKeyedSliceMerging::combineThreadLocalSlices(
    Value<MemRef>& globalOperatorHandler, Value<MemRef>& sliceMergeTask) const {
  auto globalSlice =
      Nautilus::FunctionCall("createGlobalState", createGlobalState,
                             globalOperatorHandler, sliceMergeTask);
  auto globalSliceState = Nautilus::FunctionCall(
      "getGlobalSliceState", getGlobalSliceState, globalSlice);
  auto numberOfSlices = Nautilus::FunctionCall(
      "getNonKeyedNumberOfSlices", getNonKeyedNumberOfSlices, sliceMergeTask);
  for (Value<UInt64> i = 0_u64; i < numberOfSlices; i = i + 1_u64) {
    auto srcSliceState = Nautilus::FunctionCall(
        "getNonKeyedSliceState", getNonKeyedSliceState, sliceMergeTask, i);
    ...
    for (const auto& function : aggregationFunctions) {
      ...
      function->combine(globalValuePtr.as<MemRef>(),
                        partitionValuePtr.as<MemRef>());
