...
void triggerThreadLocalStateProxy(
    void* op,
    void* wctx,
    void* pctx,
    WorkerThreadId,
    ...
    uint64_t watermarkTs)
{
    ...
    handler->trigger(*workerContext, *pipelineExecutionContext, OriginId(originId), {sequenceNumber, chunkNumber, lastChunk}, watermarkTs);
}
...
NonKeyedSlicePreAggregation::NonKeyedSlicePreAggregation(
    ...
    : operatorHandlerIndex(operatorHandlerIndex), timeFunction(std::move(timeFunction)), aggregationFunctions(aggregationFunctions)
{
}
...
void NonKeyedSlicePreAggregation::setup(ExecutionContext& executionCtx) const
{
    ...
    Nautilus::FunctionCall("setupWindowHandler", setupWindowHandler, globalOperatorHandler, executionCtx.getPipelineContext(), entrySize);
