...
void triggerThreadLocalStateProxy(void *op, void *wctx, void *pctx,
                                  WorkerThreadId, uint64_t originId,
...
public:
  explicit LocalGlobalPreAggregationState(const Value<MemRef> &sliceStoreState)
      : sliceStoreState(sliceStoreState){};
...
void NonKeyedSlicePreAggregation::setup(ExecutionContext &executionCtx) const {
  ...
  Nautilus::FunctionCall("setupWindowHandler", setupWindowHandler,
                         globalOperatorHandler,
                         executionCtx.getPipelineContext(), entrySize);
  auto defaultState = Nautilus::FunctionCall("getDefaultState", getDefaultState,
                                             globalOperatorHandler);
...
void NonKeyedSlicePreAggregation::open(ExecutionContext &ctx,
...
void NonKeyedSlicePreAggregation::execute(
  ...
  auto sliceStore =
      static_cast<LocalGlobalPreAggregationState *>(ctx.getLocalState(this));
  auto sliceState =
      Nautilus::FunctionCall("findSliceStateByTsProxy", findSliceStateByTsProxy,
                             sliceStore->sliceStoreState, timestampValue);
...
void NonKeyedSlicePreAggregation::close(ExecutionContext &ctx,
                                        RecordBuffer &) const {
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  ...
  Nautilus::FunctionCall(
      "triggerThreadLocalStateProxy", triggerThreadLocalStateProxy,
      globalOperatorHandler, ctx.getWorkerContext(), ctx.getPipelineContext(),
      ctx.getWorkerThreadId(), ctx.getOriginId(), ctx.getSequenceNumber(),
      ctx.getChunkNumber(), ctx.getLastChunk(), ctx.getWatermarkTs());
}
