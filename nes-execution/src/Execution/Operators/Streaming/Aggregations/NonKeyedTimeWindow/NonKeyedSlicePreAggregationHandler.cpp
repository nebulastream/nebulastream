...
NonKeyedSlicePreAggregationHandler::NonKeyedSlicePreAggregationHandler(
    uint64_t windowSize, uint64_t windowSlide,
    const std::vector<OriginId>& origins)
    : AbstractSlicePreAggregationHandler<NonKeyedSlice,
                                         NonKeyedThreadLocalSliceStore>(
          windowSize, windowSlide, origins) {}

void NonKeyedSlicePreAggregationHandler::setup(
  ...
  for (uint64_t i = 0; i < ctx.getNumberOfWorkerThreads(); i++) {
    auto threadLocal = std::make_unique<NonKeyedThreadLocalSliceStore>(
...
NonKeyedSlicePreAggregationHandler::~NonKeyedSlicePreAggregationHandler() {
  NES_DEBUG("~NonKeyedSlicePreAggregationHandler");
}
