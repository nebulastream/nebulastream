...
void Emit::execute(ExecutionContext &ctx, Record &record) const {
  ...
  if (emitState->outputIndex >= maxRecordsPerBuffer) {
    emitRecordBuffer(ctx, emitState->resultBuffer, emitState->outputIndex,
                     false);
    auto resultBufferRef = ctx.allocateBuffer();
  ...
  /* We need to first check if the buffer has to be emitted and then write to
   * it. Otherwise, it can happen that we will emit a tuple twice. Once in the
   * execute() and then again in close(). This happens only for buffers that are
   * filled to the brim, i.e., have no more space left.
   */
  memoryProvider->write(emitState->outputIndex, emitState->bufferReference,
                        record);
...
void Emit::emitRecordBuffer(ExecutionContext &ctx, RecordBuffer &recordBuffer,
