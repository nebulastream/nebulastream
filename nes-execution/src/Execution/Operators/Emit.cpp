...
uint64_t getNextChunkNumberProxy(void* operatorHandlerPtr, uint64_t originId, uint64_t sequenceNumber)
{
    ...
    NES_DEBUG("(Sequence Number: {}, Chunk Number: {})", sequenceNumber, chunkNumber);
    return chunkNumber;
}
...
static nautilus::val<bool> isLastChunk(ExecutionContext& context, size_t operatorHandlerIndex)
{
...
static nautilus::val<ChunkNumber> getNextChunkNr(ExecutionContext& context, size_t operatorHandlerIndex)
{
    return nautilus::invoke(
        getNextChunkNumberProxy, context.getGlobalOperatorHandler(operatorHandlerIndex), context.originId, context.sequenceNumber);
}

static void removeSequenceState(ExecutionContext& context, size_t operatorHandlerIndex)
{
    nautilus::invoke(
...
Emit::Emit(size_t operatorHandlerIndex, std::unique_ptr<MemoryProvider::TupleBufferMemoryProvider> memoryProvider)
