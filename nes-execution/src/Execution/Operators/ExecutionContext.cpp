...
ExecutionContext::ExecutionContext(
    const Value<NES::Nautilus::MemRef> &workerContext,
    const Value<NES::Nautilus::MemRef> &pipelineContext)
    : workerContext(workerContext), pipelineContext(pipelineContext),
      origin(0_u64), statisticId(INVALID_STATISTIC_ID), watermarkTs(0_u64),
      currentTs(0_u64), sequenceNumber(0_u64), chunkNumber(0_u64),
      lastChunk(true) {}
...
void *allocateBufferProxy(void *workerContextPtr) {
  ...
  // As we can only return it to operator code as a ptr we create a new
  // TupleBuffer on the heap. This increases the reference counter in the
  // buffer. When the heap allocated buffer is not required anymore, the
  // operator code has to clean up the allocated memory to prevent memory leaks.
...
Value<MemRef> ExecutionContext::allocateBuffer() {
  auto bufferPtr = Nautilus::FunctionCall("allocateBufferProxy",
                                          allocateBufferProxy, workerContext);
  return bufferPtr;
}
...
void emitBufferProxy(void *wc, void *pc, void *tupleBuffer) {
  ...
  NES_TRACE("Emitting buffer with SequenceData = {}",
            tb->getSequenceData().toString());

  /* We have to emit all buffer, regardless of their number of tuples. This is
   * due to the fact, that we expect all sequence numbers to reach any operator.
   * Sending empty buffers will have some overhead. As we are performing
   * operator fusion, this should only happen occasionally.
   */
  pipelineCtx->emitBuffer(*tb, *workerCtx);

  // delete tuple buffer as it was allocated within the pipeline and is not
  // required anymore
  delete tb;
}

void ExecutionContext::emitBuffer(
    const NES::Runtime::Execution::RecordBuffer &buffer) {
  FunctionCall<>("emitBufferProxy", emitBufferProxy, workerContext,
                 pipelineContext, buffer.getReference());
}
...
ValueId<WorkerThreadId> ExecutionContext::getWorkerThreadId() {
  return FunctionCall("getWorkerThreadIdProxy", getWorkerThreadIdProxy,
                      workerContext);
}

Operators::OperatorState *
ExecutionContext::getLocalState(const Operators::Operator *op) {
...
void ExecutionContext::setLocalOperatorState(
    const Operators::Operator *op,
    std::unique_ptr<Operators::OperatorState> state) {
  if (localStateMap.contains(op)) {
    NES_THROW_RUNTIME_ERROR(
        "Operators state already registered for operator: " << op);
  }
...
void *getGlobalOperatorHandlerProxy(void *pc, uint64_t index) {
  ...
  if (index >= size) {
    NES_THROW_RUNTIME_ERROR("operator handler at index " +
                            std::to_string(index) + " is not registered");
  }
  return handlers[index].get();
}

Value<MemRef>
ExecutionContext::getGlobalOperatorHandler(uint64_t handlerIndex) {
  Value<UInt64> handlerIndexValue = Value<UInt64>(handlerIndex);
  return FunctionCall<>("getGlobalOperatorHandlerProxy",
                        getGlobalOperatorHandlerProxy, pipelineContext,
                        handlerIndexValue);
}
...
uint64_t getNextChunkNumberProxy(void *ptrPipelineCtx, uint64_t originId,
                                 uint64_t sequenceNumber) {
  NES_ASSERT2_FMT(ptrPipelineCtx != nullptr,
                  "operator handler should not be null");
  auto *pipelineCtx = static_cast<PipelineExecutionContext *>(ptrPipelineCtx);
  return pipelineCtx->getNextChunkNumber(
      {SequenceNumber(sequenceNumber), OriginId(originId)});
}

bool isLastChunkProxy(void *ptrPipelineCtx, uint64_t originId,
                      uint64_t sequenceNumber, uint64_t chunkNumber,
                      bool isLastChunk) {
  NES_ASSERT2_FMT(ptrPipelineCtx != nullptr,
                  "operator handler should not be null");
  auto *pipelineCtx = static_cast<PipelineExecutionContext *>(ptrPipelineCtx);
  return pipelineCtx->isLastChunk(
      {SequenceNumber(sequenceNumber), OriginId(originId)},
      ChunkNumber(chunkNumber), isLastChunk);
}

void removeSequenceStateProxy(void *ptrPipelineCtx, uint64_t originId,
                              uint64_t sequenceNumber) {
  NES_ASSERT2_FMT(ptrPipelineCtx != nullptr,
                  "operator handler should not be null");
  auto *pipelineCtx = static_cast<PipelineExecutionContext *>(ptrPipelineCtx);
...
Value<Boolean> ExecutionContext::isLastChunk() const {
  return Nautilus::FunctionCall("isLastChunkProxy", isLastChunkProxy,
                                this->getPipelineContext(), this->getOriginId(),
                                this->getSequenceNumber(),
                                this->getChunkNumber(), this->getLastChunk());
}
