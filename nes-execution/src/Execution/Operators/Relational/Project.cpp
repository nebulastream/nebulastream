...
void Project::execute(ExecutionContext& ctx, Record& record) const {
  // If the number of inputFields and outputFields match, map inputFields to
  // outputFields. Todo: 4068: We plan to use maps to support renaming,
  // reordering and reducing the number of attributes
  //             using projections at the same time (Currently, we cannot e.g.
  //             reduce and rename at the same time).
  if (inputFields.size() == outputFields.size()) {
    ...
    // If the number of input and output fields do not match, simply relay the
    // record to the next operator.
