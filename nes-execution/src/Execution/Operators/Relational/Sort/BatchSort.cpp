...
BatchSort::BatchSort(
    ...
    : operatorHandlerIndex(operatorHandlerIndex),
      ...
      sortOrder(sortOrder) {
  NES_ASSERT(dataTypes.size() == fieldIdentifiers.size(),
             "Data types and field identifiers must have the same size");
  for (const auto& sortFieldIdentifier : sortFieldIdentifiers) {
    auto it = std::find(fieldIdentifiers.begin(), fieldIdentifiers.end(),
                        sortFieldIdentifier);
    NES_ASSERT(it != fieldIdentifiers.end(),
               "Sort field not found in field identifiers");
  }
}
...
void BatchSort::setup(ExecutionContext& ctx) const {
  ...
  Nautilus::FunctionCall("setupHandlerProxy", setupHandlerProxy,
                         globalOperatorHandler, ctx.getPipelineContext());
}

Value<> encodeData(const Value<>& value, const Value<>& sortOrder) {
  if (value->isType<Boolean>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<bool>,
                                  value.as<Boolean>(), sortOrder.as<Boolean>());
  } else if (value->isType<Int8>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<int8_t>,
                                  value.as<Int8>(), sortOrder.as<Boolean>());
  } else if (value->isType<Int16>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<int16_t>,
                                  value.as<Int16>(), sortOrder.as<Boolean>());
  } else if (value->isType<Int32>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<int32_t>,
                                  value.as<Int32>(), sortOrder.as<Boolean>());
  } else if (value->isType<Int64>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<int64_t>,
                                  value.as<Int64>(), sortOrder.as<Boolean>());
  } else if (value->isType<UInt8>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<uint8_t>,
                                  value.as<UInt8>(), sortOrder.as<Boolean>());
  } else if (value->isType<UInt16>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<uint16_t>,
                                  value.as<UInt16>(), sortOrder.as<Boolean>());
  } else if (value->isType<UInt32>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<uint32_t>,
                                  value.as<UInt32>(), sortOrder.as<Boolean>());
  } else if (value->isType<UInt64>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<uint64_t>,
                                  value.as<UInt64>(), sortOrder.as<Boolean>());
  } else if (value->isType<Float>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<float>,
                                  value.as<Float>(), sortOrder.as<Boolean>());
  } else if (value->isType<Double>()) {
    return Nautilus::FunctionCall("encodeData", encodeData<double>,
                                  value.as<Double>(), sortOrder.as<Boolean>());
  } else {
    throw Exceptions::NotImplementedException(
        "encodeData is not implemented for the given type.");
  }
}
...
void BatchSort::execute(ExecutionContext& ctx, Record& record) const {
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  auto state = Nautilus::FunctionCall("getStackProxy", getStackProxy,
                                      globalOperatorHandler);

  auto entrySize = Nautilus::FunctionCall(
      "getStateEntrySizeProxy", getStateEntrySizeProxy, globalOperatorHandler);
  ...
  for (uint64_t i = 0; i < fieldIdentifiers.size(); ++i) {
    for (uint64_t j = 0; j < sortFieldIdentifiers.size(); ++j) {
      if (fieldIdentifiers[i] == sortFieldIdentifiers[j]) {
        ...
        val =
            encodeData(val, Value<Boolean>(sortOrder == SortOrder::Descending));
        entry.store(val);
