...
Record AbstractJavaUDFOperator::extractRecordFromPojo(const Value<MemRef>& handler, const Value<MemRef>& outputPojoPtr) const
{
    ...
    if (operatorOutputSchema->fields.size() == 1)
    {
        ...
        else
        {
            NES_THROW_RUNTIME_ERROR(
                "Could not extract value from Java UDF output object (data type not supported): type="
    ...
    else
    {
        ...
        for (int i = 0; i < (int)operatorOutputSchema->fields.size(); i++)
        {
            ...
            if (field->getDataType()->equals(DataTypeFactory::createBoolean()))
            {
                Value<> val = FunctionCall<>("getBooleanField", getBooleanField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createFloat()))
            {
                Value<> val = FunctionCall<>("getFloatField", getFloatField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createDouble()))
            {
                Value<> val = FunctionCall<>("getDoubleField", getDoubleField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createInt32()))
            {
                Value<> val = FunctionCall<>("getIntegerField", getIntegerField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createInt64()))
            {
                Value<> val = FunctionCall<>("getLongField", getLongField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createInt16()))
            {
                Value<> val = FunctionCall<>("getShortField", getShortField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createInt8()))
            {
                Value<> val = FunctionCall<>("getByteField", getByteField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else if (field->getDataType()->equals(DataTypeFactory::createText()))
            {
                Value<> val = FunctionCall<>("getStringField", getStringField, handler, outputClassPtr, outputPojoPtr, Value<Int32>(i));
                resultRecord.write(fieldName, val);
            }
            else
            {
                NES_THROW_RUNTIME_ERROR(
                    "Could not extract value from Java UDF output POJO (data type not supported): field="
...
Nautilus::Value<MemRef> AbstractJavaUDFOperator::createInputPojo(Record& record, Value<MemRef>& handler) const
{
    ...
    else
    {
        ...
        for (int i = 0; i < (int)operatorInputSchema->fields.size(); i++)
        {
            ...
            {
                FunctionCall<>(
                    "setByteField", setByteField, handler, inputClassPtr, inputPojoPtr, Value<Int32>(i), record.read(fieldName).as<Int8>());
            }
            else if (type->equals(DataTypeFactory::createText()))
            {
                FunctionCall<>(
            ...
            {
                NES_THROW_RUNTIME_ERROR(
                    "Could not create Java UDF input POJO (data type not supported): field=" << fieldName
