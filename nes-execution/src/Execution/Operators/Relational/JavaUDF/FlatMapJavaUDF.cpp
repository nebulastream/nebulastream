...
bool hasNext(void*, void* instance) {
  ...
  jmethodID hasNextMethod =
      env->GetMethodID(iteratorInterface, "hasNext", "()Z");
...
void* next(void*, void* instance) {
  ...
  jmethodID nextMethod =
      env->GetMethodID(iteratorInterface, "next", "()Ljava/lang/Object;");
...
void* getIteratorFromCollection(void*, void* collection) {
  ...
  jmethodID iteratorMethod = env->GetMethodID(collectionInterface, "iterator",
                                              "()Ljava/util/Iterator;");
...
void FlatMapJavaUDF::execute(ExecutionContext& ctx, Record& record) const {
  ...
  auto outputPojoPtr = FunctionCall<>("executeFlatMapUDF", executeFlatMapUDF,
                                      handler, state->instance, inputPojoPtr);
  FunctionCall<>("freeObject", freeObject, inputPojoPtr);

  // get the iterator of the collection

  auto iterator =
      FunctionCall("getIteratorFromCollection", getIteratorFromCollection,
                   handler, outputPojoPtr);
