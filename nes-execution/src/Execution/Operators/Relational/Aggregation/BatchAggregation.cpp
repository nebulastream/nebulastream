...
public:
  explicit ThreadLocalAggregationState(const Value<MemRef> &stateReference)
...
void BatchAggregation::setup(ExecutionContext &ctx) const {
  ...
  Nautilus::FunctionCall("setupHandler", setupHandler, globalOperatorHandler,
                         ctx.getPipelineContext(), entrySize);
}
...
void BatchAggregation::open(ExecutionContext &ctx, RecordBuffer &rb) const {
  ...
  // here, to load the thread local state and stores the pointer/memref to it in
  ...
  auto defaultState =
      Nautilus::FunctionCall("getThreadLocalState", getThreadLocalState,
                             globalOperatorHandler, ctx.getWorkerThreadId());
  auto threadLocalState =
      std::make_unique<ThreadLocalAggregationState>(defaultState);
...
void BatchAggregation::execute(ExecutionContext &ctx, Record &record) const {
  // 1. get local state
  auto aggregationState =
      (ThreadLocalAggregationState *)ctx.getLocalState(this);
  // 2. update aggregates
