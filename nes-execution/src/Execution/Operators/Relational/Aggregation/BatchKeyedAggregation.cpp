...
public:
  explicit LocalKeyedStoreState(Interface::ChainedHashMapRef hm)
      : hm(std::move(hm)){};
...
void BatchKeyedAggregation::open(ExecutionContext &ctx, RecordBuffer &) const {
  ...
  auto hashMap =
      Nautilus::FunctionCall("getKeyedStateProxy", getKeyedStateProxy,
                             globalOperatorHandler, ctx.getWorkerThreadId());
  auto hashMapRef =
      Interface::ChainedHashMapRef(hashMap, keyDataTypes, keySize, valueSize);
...
void BatchKeyedAggregation::execute(
  ...
  auto state =
      reinterpret_cast<LocalKeyedStoreState *>(ctx.getLocalState(this));
  auto hashMap = state->hm;

  // 4. calculate hash
  auto hash = hashFunction->calculate(keyValues);

  // 5. create entry in the hash map. If the entry is new set default values for
  // aggregations.
