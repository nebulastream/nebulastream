...
void setupJoinBuildHandler(void *ss, void *ctx, uint64_t entrySize,
...
public:
  explicit LocalJoinBuildState(Interface::PagedVectorRef pagedVector)
...
BatchJoinBuild::BatchJoinBuild(
    ...
    : operatorHandlerIndex(operatorHandlerIndex),
      keyExpressions(keyExpressions), keyDataTypes(keyDataTypes),
      valueExpressions(valueExpressions), valueDataTypes(valueDataTypes),
...
void BatchJoinBuild::setup(ExecutionContext &executionCtx) const {
  ...
  auto entrySize = keySize + valueSize + /*next ptr*/ sizeof(int64_t) +
                   /*hash*/ sizeof(int64_t);
  Nautilus::FunctionCall(
      "setupJoinBuildHandler", setupJoinBuildHandler, globalOperatorHandler,
      executionCtx.getPipelineContext(), Value<UInt64>(entrySize),
...
void BatchJoinBuild::open(ExecutionContext &ctx, RecordBuffer &) const {
  ...
  // here, to load the thread local pagedVector and store it in the local state.
  // 1. get the operator handler
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  // 2. load the thread local pagedVector according to the worker id.
  auto state =
      Nautilus::FunctionCall("getPagedVectorProxy", getPagedVectorProxy,
...
void BatchJoinBuild::execute(NES::Runtime::Execution::ExecutionContext &ctx,
...
void BatchJoinBuild::storeKeys(std::vector<Value<>> keys,
                               Value<MemRef> keyPtr) const {
...
void BatchJoinBuild::storeValues(std::vector<Value<>> values,
                                 Value<MemRef> valuePtr) const {
