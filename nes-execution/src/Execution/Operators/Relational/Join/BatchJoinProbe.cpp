...
public:
  explicit LocalJoinProbeState(Interface::ChainedHashMapRef hashMap)
...
BatchJoinProbe::BatchJoinProbe(
    ...
    : operatorHandlerIndex(operatorHandlerIndex),
      ...
      valueDataTypes(valueDataTypes), hashFunction(std::move(hashFunction)),
...
void BatchJoinProbe::open(ExecutionContext &ctx, RecordBuffer &rb) const {
  ...
  // here, to load the global probe hash table.
  // 1. get the operator handler
  auto globalOperatorHandler =
      ctx.getGlobalOperatorHandler(operatorHandlerIndex);
  // 2. load the hash map
  auto state = Nautilus::FunctionCall(
      "getProbeHashMapProxy", getProbeHashMapProxy, globalOperatorHandler);
  auto chainedHM =
      Interface::ChainedHashMapRef(state, keyDataTypes, keySize, valueSize);
...
void BatchJoinProbe::execute(NES::Runtime::Execution::ExecutionContext &ctx,
