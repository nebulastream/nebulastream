...
Nautilus::TextValue *loadAssociatedTextValue(void *tupleBuffer,
                                             uint32_t childIndex) {
...
Nautilus::Value<>
MemoryProvider::load(const PhysicalTypePtr &type,
  ...
  if (type->isBasicType()) {
    ...
    case BasicPhysicalType::NativeType::TEXT: {
      ...
      auto variableSizeBuffer = Nautilus::FunctionCall(
          "loadAssociatedTextValue", loadAssociatedTextValue, bufferReference,
          childIndex);
      return variableSizeBuffer;
    };
    default: {
      NES_ERROR("Physical Type: {} is currently not supported",
                type->toString());
      NES_NOT_IMPLEMENTED();
    };
    }
  } else if (type->isArrayType()) {
    NES_ERROR("Physical Type: array type {} is currently not supported",
              type->toString());
    NES_NOT_IMPLEMENTED();
  } else {
    NES_ERROR("Physical Type: type {} is currently not supported",
              type->toString());
    NES_NOT_IMPLEMENTED();
  }
}

uint32_t storeAssociatedTextValue(void *tupleBuffer,
                                  const Nautilus::TextValue *textValue) {
  auto tb = TupleBuffer::reinterpretAsTupleBuffer(tupleBuffer);
...
Nautilus::Value<>
MemoryProvider::store(const NES::PhysicalTypePtr &type,
                      Nautilus::Value<Nautilus::MemRef> &bufferReference,
  ...
  if (type->isBasicType()) {
    ...
    case BasicPhysicalType::NativeType::TEXT: {
      ...
      auto childIndex = Nautilus::FunctionCall(
          "storeAssociatedTextValue", storeAssociatedTextValue, bufferReference,
...
bool MemoryProvider::includesField(
  ...
  return std::find(projections.begin(), projections.end(), fieldIndex) !=
         projections.end();
}
...
MemoryProviderPtr
MemoryProvider::createMemoryProvider(const uint64_t bufferSize,
                                     const SchemaPtr schema) {
  if (schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
    auto rowMemoryLayout = MemoryLayouts::RowLayout::create(schema, bufferSize);
    return std::make_unique<
        Runtime::Execution::MemoryProvider::RowMemoryProvider>(rowMemoryLayout);
  } else if (schema->getLayoutType() ==
             Schema::MemoryLayoutType::COLUMNAR_LAYOUT) {
    auto columnMemoryLayout =
        MemoryLayouts::ColumnLayout::create(schema, bufferSize);
    return std::make_unique<
        Runtime::Execution::MemoryProvider::ColumnMemoryProvider>(
        columnMemoryLayout);
  } else {
    NES_NOT_IMPLEMENTED();
  }
