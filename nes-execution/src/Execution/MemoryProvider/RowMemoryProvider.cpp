...
RowMemoryProvider::RowMemoryProvider(
    Runtime::MemoryLayouts::RowLayoutPtr rowMemoryLayoutPtr)
...
Nautilus::Value<Nautilus::MemRef> RowMemoryProvider::calculateFieldAddress(
    Nautilus::Value<>& recordOffset, uint64_t fieldIndex) const {
...
Nautilus::Record RowMemoryProvider::read(
  ...
  // read all fields
  auto rowLayout = std::dynamic_pointer_cast<Runtime::MemoryLayouts::RowLayout>(
      rowMemoryLayoutPtr);
  ...
  for (uint64_t i = 0; i < rowMemoryLayoutPtr->getSchema()->getSize(); i++) {
    ...
    auto value = load(rowMemoryLayoutPtr->getPhysicalTypes()[i], bufferAddress,
                      fieldAddress);
...
void RowMemoryProvider::write(
  ...
  for (uint64_t i = 0; i < fieldSizes.size(); i++) {
    ...
    store(rowMemoryLayoutPtr->getPhysicalTypes()[i], bufferAddress,
          fieldAddress, value);
  }
}
