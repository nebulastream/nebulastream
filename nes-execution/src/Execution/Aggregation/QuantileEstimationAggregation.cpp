...
QuantileEstimationAggregation::QuantileEstimationAggregation(
    ...
    : AggregationFunction(inputType, finalType, inputExpression,
                          resultFieldIdentifier) {}

void insert(void *memrefPtr, float inputValue) {
  // get the Tdigest instance
  QuantileEstimationValue *obj =
      static_cast<QuantileEstimationValue *>(memrefPtr);
  // insert the new input
  obj->digest.insert(inputValue);
}

void QuantileEstimationAggregation::lift(
    Nautilus::Value<Nautilus::MemRef> memref, Nautilus::Record &record) {
  ...
  if (inputValue->isType<Nautilus::Int8>()) {
    ...
    throw Exceptions::NotImplementedException(
        "Type not implemented " + inputValue->getType()->toString());
  }
}

void mergeTDigest(void *memref1Ptr, void *memref2Ptr) {
  QuantileEstimationValue *obj1 =
      static_cast<QuantileEstimationValue *>(memref1Ptr);
  QuantileEstimationValue *obj2 =
      static_cast<QuantileEstimationValue *>(memref2Ptr);
  obj1->digest.insert(obj2->digest);
}

void QuantileEstimationAggregation::combine(
    Nautilus::Value<Nautilus::MemRef> memref1,
...
double estimateTDigest(void *memrefPtr) {
  ...
  // TODO: here we can add any % once fixed issue #3889, right now we derive the
  // median
}

void QuantileEstimationAggregation::lower(
    Nautilus::Value<Nautilus::MemRef> memref, Nautilus::Record &resultRecord) {
  auto result =
      Nautilus::FunctionCall<>("estimateTDigest", estimateTDigest, memref);
...
void QuantileEstimationAggregation::reset(
