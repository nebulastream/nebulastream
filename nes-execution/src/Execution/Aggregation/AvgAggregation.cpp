...
AvgAggregationFunction::AvgAggregationFunction(
    ...
    : AggregationFunction(inputType, resultType, inputExpression,
...
Nautilus::Value<Nautilus::MemRef> AvgAggregationFunction::loadSumMemRef(
    const Nautilus::Value<Nautilus::MemRef> &memref) {
  const static int64_t sizeOfCountInBytes =
      8L; // the sum is stored after the count, and the count is of type uint64
  return (memref + sizeOfCountInBytes).as<Nautilus::MemRef>();
}

void AvgAggregationFunction::lift(Nautilus::Value<Nautilus::MemRef> state,
...
void AvgAggregationFunction::combine(Nautilus::Value<Nautilus::MemRef> state1,
  ...
  auto sumRight =
      AggregationFunction::loadFromMemref(sumRightMemref, inputType);
...
void AvgAggregationFunction::lower(Nautilus::Value<Nautilus::MemRef> memref,
...
uint64_t AvgAggregationFunction::getSize() {
  return inputType->size() +
         8L; // the count is always uint64, hence always 8bytes
}
