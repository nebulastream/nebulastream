...
std::shared_ptr<DecomposedQueryPlan> DecomposedQueryPlan::clone() const
{
    ...
    std::deque<std::shared_ptr<Operator>> operatorsToProcess{rootOperators.begin(), rootOperators.end()};
    while (!operatorsToProcess.empty())
    {
        ...
        else
        {
            ...
            operatorIdToOperatorMap[operatorId] = operatorNode->clone();
        }
