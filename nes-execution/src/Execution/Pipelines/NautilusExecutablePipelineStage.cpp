...
uint32_t NautilusExecutablePipelineStage::setup(
    PipelineExecutionContext& pipelineExecutionContext) {
  auto pipelineExecutionContextRef =
      Value<MemRef>((int8_t*)&pipelineExecutionContext);
...
uint32_t NautilusExecutablePipelineStage::open(
    Execution::PipelineExecutionContext&, WorkerContext&) {
  // nop as we don't need this function in nautilus
  return 0;
}
uint32_t NautilusExecutablePipelineStage::close(PipelineExecutionContext&,
                                                WorkerContext&) {
  // nop as we don't need this function in nautilus
  return 0;
}

uint32_t NautilusExecutablePipelineStage::stop(
    PipelineExecutionContext& pipelineExecutionContext) {
  auto pipelineExecutionContextRef =
      Value<MemRef>((int8_t*)&pipelineExecutionContext);
  auto workerContextRef = Value<MemRef>((int8_t*)nullptr);
...
std::string NautilusExecutablePipelineStage::getCodeAsString() {
  return "<no_code>";
}
