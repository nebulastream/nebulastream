...
CompiledExecutablePipelineStage::CompiledExecutablePipelineStage(
    ...
    : NautilusExecutablePipelineStage(physicalOperatorPipeline),
      compilationBackend(compilationBackend), options(options),
...
ExecutionResult CompiledExecutablePipelineStage::execute(
  ...
  pipelineFunction((void *)&pipelineExecutionContext, &workerContext,
                   std::addressof(inputTupleBuffer));
  return ExecutionResult::Ok;
}

std::shared_ptr<NES::Nautilus::IR::IRGraph>
CompiledExecutablePipelineStage::createIR(DumpHelper &dumpHelper,
                                          Timer<> &timer) {

  auto pipelineExecutionContextRef = Value<MemRef>((int8_t *)nullptr);
  pipelineExecutionContextRef.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0,
      NES::Nautilus::IR::Types::StampFactory::createAddressStamp());
  auto workerContextRef = Value<MemRef>((int8_t *)nullptr);
  workerContextRef.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 1,
      NES::Nautilus::IR::Types::StampFactory::createAddressStamp());
  auto memRef = Nautilus::Value<Nautilus::MemRef>(
      std::make_unique<Nautilus::MemRef>(Nautilus::MemRef(0)));
  memRef.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 2,
      NES::Nautilus::IR::Types::StampFactory::createAddressStamp());
...
CompiledExecutablePipelineStage::compilePipeline() {
  // compile after setup
  auto dumpHelper =
      DumpHelper::create(options.getIdentifier(), options.isDumpToConsole(),
                         options.isDumpToFile(), options.getDumpOutputPath());
  Timer timer("CompilationBasedPipelineExecutionEngine " +
              options.getIdentifier());
  timer.start();
  auto &compiler = Nautilus::Backends::CompilationBackendRegistry::getPlugin(
      compilationBackend);
...
uint32_t CompiledExecutablePipelineStage::setup(
    PipelineExecutionContext &pipelineExecutionContext) {
  ...
  pipelineFunction =
      executablePipeline.get()
          ->getInvocableMember<void, void *, void *, void *>("execute");
  return 0;
}
