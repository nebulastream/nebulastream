...
TruncExpression::TruncExpression(
 ...
 * This function is basically a wrapper for std::trunc and enables us to use it
...
Value<> TruncExpression::execute(NES::Nautilus::Record& record) const {
  ...
  if (subValue->isType<Int8>()) {
    return FunctionCall<>("calculateTrunc", calculateTrunc,
                          subValue.as<Int8>());
