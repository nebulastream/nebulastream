 ...
 * This function is basically a wrapper for std::abs and enables us to use it in
...
Value<> AbsExpression::execute(NES::Nautilus::Record &record) const {
  ...
  if (subValue->isType<Float>()) {
    return FunctionCall<>("calculateAbsFloat", calculateAbsFloat,
                          subValue.as<Float>());

  } else if (subValue->isType<Double>()) {
    return FunctionCall<>("calculateAbsDouble", calculateAbsDouble,
                          subValue.as<Double>());

  } else if (subValue->isType<UInt64>() || subValue->isType<UInt32>() ||
             subValue->isType<UInt16>() || subValue->isType<UInt8>()) {
    return subValue;

  } else if (subValue->isType<Int64>()) {
    return FunctionCall<>("calculateAbsSignedInt64", calculateAbsSignedInt64,
                          subValue.as<Int64>());

  } else if (subValue->isType<Int32>()) {
    return FunctionCall<>("calculateAbsSignedInt32", calculateAbsSignedInt32,
                          subValue.as<Int32>());

  } else if (subValue->isType<Int16>()) {
    return FunctionCall<>("calculateAbsSignedInt16", calculateAbsSignedInt16,
                          subValue.as<Int16>());

  } else if (subValue->isType<Int8>()) {
    return FunctionCall<>("calculateAbsSignedInt8", calculateAbsSignedInt8,
                          subValue.as<Int8>());

  } else {
    throw Exceptions::NotImplementedException(
        "This expression is only defined on a numeric input argument that is "
        "ether Float or Double.");
  }
}
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<AbsExpression>>
    absFunction("abs");
} // namespace NES::Runtime::Execution::Expressions
