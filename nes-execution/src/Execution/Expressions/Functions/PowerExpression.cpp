 ...
 * This function is basically a wrapper for pow(x,y) and enables us to use it in
...
Value<> PowerExpression::execute(NES::Nautilus::Record& record) const {
  ...
  // from Value to Value<Int8>. In all cases we can call the same calculatePow
  ...
  if (leftValue->isType<Int8>() && rightValue->isType<Int8>()) {
    ...
    return FunctionCall<>("calculatePow", calculatePower, leftValue.as<Int8>(),
                          rightValue.as<Int8>());
  } else if (leftValue->isType<Int16>() && rightValue->isType<Int16>()) {
    return FunctionCall<>("calculatePow", calculatePower, leftValue.as<Int16>(),
                          rightValue.as<Int16>());
  } else if (leftValue->isType<Int32>() && rightValue->isType<Int32>()) {
    return FunctionCall<>("calculatePow", calculatePower, leftValue.as<Int32>(),
                          rightValue.as<Int32>());
  } else if (leftValue->isType<Int64>() && rightValue->isType<Int64>()) {
    return FunctionCall<>("calculatePow", calculatePower, leftValue.as<Int64>(),
                          rightValue.as<Int64>());
  } else if (leftValue->isType<Float>() && rightValue->isType<Float>()) {
    return FunctionCall<>("calculatePow", calculatePower, leftValue.as<Float>(),
                          rightValue.as<Float>());
  } else if (leftValue->isType<Double>() && rightValue->isType<Double>()) {
    return FunctionCall<>("calculatePow", calculatePower,
...
static ExecutableFunctionRegistry::Add<BinaryFunctionProvider<PowerExpression>>
    powerFunction("power");
}  // namespace NES::Runtime::Execution::Expressions
