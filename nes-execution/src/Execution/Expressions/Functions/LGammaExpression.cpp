...
LGammaExpression::LGammaExpression(const NES::Runtime::Execution::Expressions::ExpressionPtr& SubExpression) : SubExpression(SubExpression)
{
}
 ...
 * @return double
 */
double calculateLGamma(double x)
{
    return std::lgamma(x);
}
...
std::unique_ptr<Expression> RegisterLGammaExpression(const std::vector<ExpressionPtr>& args)
{
    PRECONDITION(args.size() == 1, "The unary lgamma function should receive one argument");
    return std::make_unique<LGammaExpression>(args[0]);
}
} /// namespace NES::Runtime::Execution::Expressions
