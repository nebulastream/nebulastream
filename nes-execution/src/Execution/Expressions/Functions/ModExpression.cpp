 ...
 * This function is basically a wrapper for std::fmod and enables us to use it
...
Value<> ModExpression::execute(NES::Nautilus::Record &record) const {
  ...
  // and then call the function. leftValue.as<Int8>() makes an explicit cast
  // from Value to Value<Int8>. In all cases we can call the same calculateMod
  ...
  if (leftValue->isType<Int8>() && rightValue->isType<Int8>()) {
    ...
    return FunctionCall<>("calculateMod", calculateMod, leftValue.as<Int8>(),
                          rightValue.as<Int8>());
  } else if (leftValue->isType<Int16>() && rightValue->isType<Int16>()) {
    return FunctionCall<>("calculateMod", calculateMod, leftValue.as<Int16>(),
                          rightValue.as<Int16>());
  } else if (leftValue->isType<Int32>() && rightValue->isType<Int32>()) {
    return FunctionCall<>("calculateMod", calculateMod, leftValue.as<Int32>(),
                          rightValue.as<Int32>());
  } else if (leftValue->isType<Int64>() && rightValue->isType<Int64>()) {
    return FunctionCall<>("calculateMod", calculateMod, leftValue.as<Int64>(),
                          rightValue.as<Int64>());
  } else if (leftValue->isType<Float>() && rightValue->isType<Float>()) {
    return FunctionCall<>("calculateMod", calculateMod, leftValue.as<Float>(),
                          rightValue.as<Float>());
  } else if (leftValue->isType<Double>() && rightValue->isType<Double>()) {
    return FunctionCall<>("calculateMod", calculateMod, leftValue.as<Double>(),
...
static ExecutableFunctionRegistry::Add<BinaryFunctionProvider<ModExpression>>
    modFunction("mod");
} // namespace NES::Runtime::Execution::Expressions
