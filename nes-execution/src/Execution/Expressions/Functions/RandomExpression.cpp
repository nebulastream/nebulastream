...
std::unique_ptr<Expression> RegisterRandomExpression(const std::vector<ExpressionPtr>& args)
{
    PRECONDITION(args.size() == 1, "The unary random function should receive one argument");
    return std::make_unique<RandomExpression>(args[0]);
}
} /// namespace NES::Runtime::Execution::Expressions
