...
double calculateRadians(double x)
{
    return (x * M_PI) / 180;
}
...
std::unique_ptr<Expression> RegisterRadiansExpression(const std::vector<ExpressionPtr>& args)
{
    PRECONDITION(args.size() == 1, "The unary radians function should receive one argument");
    return std::make_unique<RadiansExpression>(args[0]);
}
} /// namespace NES::Runtime::Execution::Expressions
