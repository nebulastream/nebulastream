...
RadiansExpression::RadiansExpression(
...
Value<> RadiansExpression::execute(NES::Nautilus::Record& record) const {
  ...
  if (subValue->isType<Int8>()) {
    ...
    return FunctionCall<>("calculateRadians", calculateRadians,
                          subValue.as<Int8>());
...
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<RadiansExpression>>
