...
Atan2Expression::Atan2Expression(
 ...
 * This function is basically a wrapper for std::atan2 and enables us to use it
...
Value<> Atan2Expression::execute(NES::Nautilus::Record &record) const {
  ...
  if (leftValue->isType<Float>() && rightValue->isType<Float>()) {
    return FunctionCall<>("calculateAtan2", calculateAtan2,
                          leftValue.as<Float>(), rightValue.as<Float>());
...
static ExecutableFunctionRegistry::Add<BinaryFunctionProvider<Atan2Expression>>
    atan2Function("atan");
} // namespace NES::Runtime::Execution::Expressions
