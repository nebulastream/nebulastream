...
GammaExpression::GammaExpression(
...
Value<> GammaExpression::execute(NES::Nautilus::Record& record) const {
  ...
  if (leftValue->isType<Int8>()) {
    return FunctionCall<>("calculateGamma", calculateGamma,
                          leftValue.as<Int8>());
...
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<GammaExpression>>
    gammaFunction("gamma");
}  // namespace NES::Runtime::Execution::Expressions
