 ...
 * This function is basically a wrapper for std::log10 and enables us to use it
...
Value<> Log10Expression::execute(NES::Nautilus::Record& record) const {
  ...
  if (subValue->isType<Int8>()) {
    return FunctionCall<>("calculateLog10", calculateLog10,
                          subValue.as<Int8>());
...
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<Log10Expression>>
    log10Expression("log10");
}  // namespace NES::Runtime::Execution::Expressions
