...
DegreesExpression::DegreesExpression(
...
Value<> DegreesExpression::execute(NES::Nautilus::Record &record) const {
  ...
  if (subValue->isType<Int8>()) {
    ...
    return FunctionCall<>("calculateDegrees", calculateDegrees,
                          subValue.as<Int8>());
...
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<DegreesExpression>>
    degreeFunction("degree");
} // namespace NES::Runtime::Execution::Expressions
