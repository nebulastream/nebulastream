...
AcosExpression::AcosExpression(
    const NES::Runtime::Execution::Expressions::ExpressionPtr &Expression)
    : Expression(Expression) {}

/**
 * @brief This method calculates the acosinus of x.
 * This function is basically a wrapper for std::acos and enables us to use it
...
Value<> AcosExpression::execute(NES::Nautilus::Record &record) const {
  ...
  // and then call the function. Value.as<Int8>() makes an explicit cast from
  // Value to Value<Int8>. In all cases we can call the same calculateMod
...
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<AcosExpression>>
    acosFunction("acos");
} // namespace NES::Runtime::Execution::Expressions
