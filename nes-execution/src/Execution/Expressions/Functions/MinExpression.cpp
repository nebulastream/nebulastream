...
Value<> MinExpression::execute(NES::Nautilus::Record& record) const {
  ...
  if (leftValue->isType<Int8>() && rightValue->isType<Int8>()) {
    ...
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<Int8>(),
                          rightValue.as<Int8>());
  } else if (leftValue->isType<Int16>() && rightValue->isType<Int16>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<Int16>(),
                          rightValue.as<Int16>());
  } else if (leftValue->isType<Int32>() && rightValue->isType<Int32>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<Int32>(),
                          rightValue.as<Int32>());
  } else if (leftValue->isType<Int64>() && rightValue->isType<Int64>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<Int64>(),
                          rightValue.as<Int64>());
  } else if (leftValue->isType<UInt8>() && rightValue->isType<UInt8>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<UInt8>(),
                          rightValue.as<UInt8>());
  } else if (leftValue->isType<UInt16>() && rightValue->isType<UInt16>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<UInt16>(),
                          rightValue.as<UInt16>());
  } else if (leftValue->isType<UInt32>() && rightValue->isType<UInt32>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<UInt32>(),
                          rightValue.as<UInt32>());
  } else if (leftValue->isType<UInt64>() && rightValue->isType<UInt64>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<UInt64>(),
                          rightValue.as<UInt64>());
  } else if (leftValue->isType<Float>() && rightValue->isType<Float>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<Float>(),
                          rightValue.as<Float>());
  } else if (leftValue->isType<Double>() && rightValue->isType<Double>()) {
    return FunctionCall<>("calculateMin", calculateMin, leftValue.as<Double>(),
...
static ExecutableFunctionRegistry::Add<BinaryFunctionProvider<MinExpression>>
    minFunction("min");
}  // namespace NES::Runtime::Execution::Expressions
