...
LnExpression::LnExpression(const NES::Runtime::Execution::Expressions::ExpressionPtr& subExpression) : subExpression(subExpression)
{
}
...
std::unique_ptr<Expression> RegisterLnExpression(const std::vector<ExpressionPtr>& args)
{
    PRECONDITION(args.size() == 1, "The unary ln function should receive one argument");
    return std::make_unique<LnExpression>(args[0]);
}
} /// namespace NES::Runtime::Execution::Expressions
