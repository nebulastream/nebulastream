 ...
 * This function is basically a wrapper for std::floor and enables us to use it
...
Value<> FloorExpression::execute(NES::Nautilus::Record &record) const {
  ...
  // from Value to Value<Int8>. In all cases we can call the same calculateFloor
  ...
  if (leftValue->isType<Float>()) {
    return FunctionCall<>("calculateFloor", calculateFloor,
                          leftValue.as<Float>());
...
static ExecutableFunctionRegistry::Add<UnaryFunctionProvider<FloorExpression>>
    floorFunction("floor");
} // namespace NES::Runtime::Execution::Expressions
