...
/**
 * @brief This Method replaces the first occurrence of regex with the
 * replacement This function is basically a wrapper for std::regex_replace and
 * enables us to use it in our execution engine framework.
 * @param text TextValue* the text (string) to extract the regexpPattern from
 * @param regex TextValue* the pattern to match
 * @param replacement TextValue* the text (string) to replacement a pattern
 * match
 * @return TextValue*
 */
TextValue* regexReplace(TextValue* text, TextValue* reg,
  ...
  NES_DEBUG("Received the following reg string {}",
            std::string(reg->c_str(), reg->length()));
  std::string strReplacement =
      std::string(replacement->c_str(), replacement->length());
  NES_DEBUG("Received the following replacement string {}", strReplacement);
  std::string strReplaced =
      std::regex_replace(strText, tempRegex, strReplacement);
...
Value<> ReplacingRegex::execute(NES::Nautilus::Record& record) const {
  ...
  if (text->isType<Text>() && pattern->isType<Text>() &&
      replacement->isType<Text>()) {
    return FunctionCall<>("regexReplace", regexReplace,
