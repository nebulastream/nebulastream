...
/**
 * @brief This Method returns a text sequence from the target text, which is
 * matching the given regex pattern. If multiple text sequences, would match the
 * given regex pattern the idx parameter can be used to select a match.
 * @param text TextValue* the text (string) to extract the regexpPattern from
 * @param reg TextValue* the pattern to extract
 * @param idx result index as Integer defines at what position in the textValue
 * matching starts (only the first match is returned)
 * @return TextValue*
 */
...
Value<> ExtractingRegex::execute(NES::Nautilus::Record &record) const {
  ...
  if (text->isType<Text>() && pattern->isType<Text>()) {
    ...
    if (pos->isType<Int8>()) {
      return FunctionCall<>("regexExtract", regexExtract,
                            text.as<Text>()->getReference(),
                            pattern.as<Text>()->getReference(), pos.as<Int8>());
    } else if (pos->isType<Int16>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<Int16>());
    } else if (pos->isType<Int32>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<Int32>());
    } else if (pos->isType<Int64>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<Int64>());
    } else if (pos->isType<UInt8>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<UInt8>());
    } else if (pos->isType<UInt16>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<UInt16>());
    } else if (pos->isType<UInt32>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<UInt32>());
    } else if (pos->isType<UInt64>()) {
      return FunctionCall<>(
          "regexExtract", regexExtract, text.as<Text>()->getReference(),
          pattern.as<Text>()->getReference(), pos.as<UInt64>());
    } else {
      // If no type was applicable we throw an exception.
      NES_THROW_RUNTIME_ERROR("Idx only defined on a numeric input argument "
                              "that is of type Integer.");
    }
  } else {
    NES_THROW_RUNTIME_ERROR("This expression is only defined for inputs of "
                            "type Text and an index as Integer.");
  }
}
