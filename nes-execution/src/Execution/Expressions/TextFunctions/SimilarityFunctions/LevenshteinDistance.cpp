...
uint64_t textLevenshtein(const TextValue* leftText, const TextValue* rightText)
{
    ...
    NES_DEBUG(
        "The left string length is {} and the right string length is {}",
...
Value<> LevenshteinDistance::execute(NES::Nautilus::Record& record) const
{
    ...
    {
        return FunctionCall<>(
            "textLevenshtein", textLevenshtein, leftValue.as<Text>()->getReference(), rightValue.as<Text>()->getReference());
    }
    else
    {
