...
/**
 * @brief Compares two text object and returns their LevenshteinDistance
 * Distance as calculated with Wagner-Fischer Algorithm.
 * @param leftText text object
 * @param rightText text object
 * @return Hamming Distance as Integer.
 */
uint64_t textLevenshtein(const TextValue* leftText,
                         const TextValue* rightText) {
  const uint64_t lengthLeft = leftText->length();
  /** This is a variant of the Wagner-Fischer algorithm which computes a matrix
    with rows and columns corresponding to letters in each string An entry
    matrix[i][j] contains the distance from first i characters of the left
    string to first j characters of the right string */
  ...
  for (uint64_t j = 1; j <= rightText->length(); j++) {
    for (uint64_t i = 1; i <= lengthLeft; i++) {
      /*+ subCost signifies whether substitution amounts to doing nothing (no
       * increased distance) */
      ...
      /* distance is updated based on deletion, insertion or substitution as
       * "cheapest" next step */
...
Value<> LevenshteinDistance::execute(NES::Nautilus::Record& record) const {
  ...
  if (leftValue->isType<Text>() && rightValue->isType<Text>()) {
    return FunctionCall<>("textLevenshtein", textLevenshtein,
