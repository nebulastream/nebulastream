...
double textJaro(const TextValue* leftText, const TextValue* rightText) {
  ...
  for (int32_t i = 0; i < (int32_t)leftText->length(); i++) {
    ...
    for (int32_t j = fmax(0, i - maxSim);
         j < fmin(rightText->length(), i + maxSim + 1); j++) {
      /** If there is a match */
  ...
  for (int32_t i = 0; i < (int32_t)leftText->length(); i++)
    if (leftHash[i]) {
      /** Find the next matched character
         in second string */
      while (rightHash[point] == 0) point++;

      if (leftText->c_str()[i] != rightText->c_str()[point++]) transpositions++;
    }

  transpositions /= 2;

  return (((double)match) / ((double)leftText->length()) +
          ((double)match) / ((double)rightText->length()) +
          ((double)match - transpositions) / ((double)match)) /
         3.0;
}

/**
 * @brief Compares two text object and returns their Jaro Winkler Distance
 * (takes prefix into account) as Double.
 * @param leftText text object
 * @param rightText text object
 * @return Jaccard Distance as Integer.
 */
double textJaroWinkler(const TextValue* leftText, const TextValue* rightText,
                       double jaroDist) {
  int32_t prefix = 0;
  ...
  /**
   * The Jaro-Winkler similarity uses a prefix scale p, which gives more
   * favorable ratings to strings that match from the beginning for a set prefix
   * length l. p is a constant scaling factor for how much the score is adjusted
   * upwards for having common prefixes. The standard value for this constant in
   * Winkler's work is p=0.1. l is the length of common prefix at the start of
   * the string (up to a maximum of 4 characters). source:
...
Value<> JaroSimilarity::execute(NES::Nautilus::Record& record) const {
  ...
  if (leftValue->isType<Text>() && rightValue->isType<Text>() &&
      flagValue->isType<Boolean>()) {
    Value<> distance = FunctionCall<>("textJaro", textJaro,
                                      leftValue.as<Text>()->getReference(),
                                      rightValue.as<Text>()->getReference());
    if (flagValue.as<Boolean>()) {
      return FunctionCall<>("textJaroWinkler", textJaroWinkler,
                            leftValue.as<Text>()->getReference(),
                            rightValue.as<Text>()->getReference(),
                            distance.as<Double>());
    }
