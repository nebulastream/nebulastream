...
double textJaccard(const TextValue* leftText, const TextValue* rightText) {
  ...
  for (int32_t i = 0; i < (int32_t)leftText->length(); i++) {
    ...
    const char* pos =
        std::find(rightText->c_str(), rightText->c_str() + rightText->length(),
                  leftText->c_str()[i]);
    if (std::string::npos == intersectionSet.find(leftText->c_str()[i]) &&
        pos != rightText->c_str() + rightText->length()) {
  ...
  for (int32_t j = 0; j < (int32_t)rightText->length(); j++) {
    ...
    const char* pos =
        std::find(leftText->c_str(), leftText->c_str() + leftText->length(),
                  rightText->c_str()[j]);
    if (intersectionSet.find(rightText->c_str()[j]) == std::string::npos &&
        pos != leftText->c_str() + leftText->length()) {
...
Value<> JaccardDistance::execute(NES::Nautilus::Record& record) const {
  ...
  if (leftValue->isType<Text>() && rightValue->isType<Text>()) {
    return FunctionCall<>("textJaccard", textJaccard,
                          leftValue.as<Text>()->getReference(),
                          rightValue.as<Text>()->getReference());
  } else {
    // If no type was applicable we throw an exception.
    NES_THROW_RUNTIME_ERROR(
        "This expression is only defined on input arguments that are Text.");
  }
}
