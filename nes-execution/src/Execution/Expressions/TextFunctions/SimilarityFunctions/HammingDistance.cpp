...
uint64_t textHamming(const TextValue * leftText, const TextValue * rightText)
{
    ...
    if (leftText->length() != rightText->length())
    {
        throw std::invalid_argument(
            "Hamming Distance is only defined for inputs of equal length: the left string length is " + std::to_string(leftText->length())
            + "and the right string length is  " + std::to_string(rightText->length()));
    }
