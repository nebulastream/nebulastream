...
/**
 * @brief Compares two text object and returns their Hamming Distance, -1 if
...
uint64_t textHamming(const TextValue *leftText, const TextValue *rightText) {
  ...
  if (leftText->length() != rightText->length()) {
    throw std::invalid_argument("Hamming Distance is only defined for inputs "
                                "of equal length: the left string length is " +
                                std::to_string(leftText->length()) +
                                "and the right string length is  " +
...
Value<> HammingDistance::execute(NES::Nautilus::Record &record) const {
  ...
  if (leftValue->isType<Text>() && rightValue->isType<Text>()) {
    return FunctionCall<>("textHamming", textHamming,
