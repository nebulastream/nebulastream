...
PhysicalOperatorPtr PhysicalCountMinBuildOperator::create(
...
{
    return std::make_shared<PhysicalCountMinBuildOperator>(PhysicalCountMinBuildOperator(
        operatorId, statisticId, inputSchema, outputSchema, nameOfFieldToTrack, width, depth, metricHash, windowType, sendingPolicy));
}
...
uint64_t PhysicalCountMinBuildOperator::getWidth() const
{
    return width;
}

uint64_t PhysicalCountMinBuildOperator::getDepth() const
{
    return depth;
}
