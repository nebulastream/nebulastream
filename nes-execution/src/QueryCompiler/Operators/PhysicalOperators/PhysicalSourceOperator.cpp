...
PhysicalSourceOperator::create(OperatorId id, StatisticId statisticId,
                               OriginId originId, const SchemaPtr &inputSchema,
                               const SchemaPtr &outputSchema,
                               const SourceDescriptorPtr &sourceDescriptor) {
  return std::make_shared<PhysicalSourceOperator>(
      id, statisticId, originId, inputSchema, outputSchema, sourceDescriptor);
}

std::shared_ptr<PhysicalSourceOperator>
PhysicalSourceOperator::create(StatisticId statisticId, SchemaPtr inputSchema,
                               SchemaPtr outputSchema,
                               SourceDescriptorPtr sourceDescriptor) {
  return create(getNextOperatorId(), statisticId, INVALID_ORIGIN_ID,
                std::move(inputSchema), std::move(outputSchema),
...
OperatorPtr PhysicalSourceOperator::copy() {
  auto result = create(id, statisticId, originId, inputSchema, outputSchema,
                       sourceDescriptor);
