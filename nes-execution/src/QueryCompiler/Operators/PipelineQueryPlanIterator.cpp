...
PipelineQueryPlanIterator::PipelineQueryPlanIterator(
...
PipelineQueryPlanIterator::iterator::iterator(
    const PipelineQueryPlanPtr& current) {
...
bool PipelineQueryPlanIterator::iterator::operator!=(
...
PipelineQueryPlanIterator::iterator&
PipelineQueryPlanIterator::iterator::operator++() {
  if (workStack.empty()) {
    NES_DEBUG(
    ...
    for (int64_t i = children.size() - 1; i >= 0; i--) {
      ...
      NES_ASSERT(!child->getPredecessors().empty(),
