...
DefaultPipeliningPhase::DefaultPipeliningPhase(
    OperatorFusionPolicyPtr operatorFusionPolicy)
    : operatorFusionPolicy(std::move(operatorFusionPolicy)) {}

PipeliningPhasePtr DefaultPipeliningPhase::create(
    const OperatorFusionPolicyPtr &operatorFusionPolicy) {
  return std::make_shared<DefaultPipeliningPhase>(operatorFusionPolicy);
}

PipelineQueryPlanPtr
DefaultPipeliningPhase::apply(DecomposedQueryPlanPtr decomposedQueryPlan) {
  ...
  for (const auto &sinkOperators : decomposedQueryPlan->getRootOperators()) {
    ...
    processSink(pipelinePlan, pipelineOperatorMap, pipeline,
                sinkOperators->as<PhysicalOperators::PhysicalOperator>());
  }
  return pipelinePlan;
}
...
void DefaultPipeliningPhase::processMultiplex(
  ...
  // if the current pipeline has no operators we will remove it, because we want
  // to omit empty pipelines
...
void DefaultPipeliningPhase::processDemultiplex(
  ...
  if (pipelineOperatorMap.find(currentOperator) != pipelineOperatorMap.end()) {
    ...
    newPipeline->addSuccessor(currentPipeline);
    process(pipelinePlan, pipelineOperatorMap, newPipeline,
            currentOperator->getChildren()[0]
                ->as<PhysicalOperators::PhysicalOperator>());
...
void DefaultPipeliningPhase::processPipelineBreakerOperator(
  ...
  currentPipeline->prependOperator(
      currentOperator->as<PhysicalOperators::PhysicalOperator>()->copy());
...
void DefaultPipeliningPhase::processFusibleOperator(
  ...
  for (const auto &node : currentOperator->getChildren()) {
    process(pipelinePlan, pipelineOperatorMap, currentPipeline,
            node->as<PhysicalOperators::PhysicalOperator>());
  }
}
...
void DefaultPipeliningPhase::processSink(
  ...
  for (const auto &child : currentOperator->getChildren()) {
    ...
    process(pipelinePlan, pipelineOperatorMap, cp,
            child->as<PhysicalOperators::PhysicalOperator>());
  }
}
...
void DefaultPipeliningPhase::process(
  ...
  if (!currentOperators->instanceOf<PhysicalOperators::PhysicalOperator>()) {
    throw QueryCompilationException(
        "Pipelining can only be applied to physical operator. But current "
        "operator was: " +
  ...
  if (currentOperators
          ->instanceOf<PhysicalOperators::PhysicalSourceOperator>()) {
    processSource(pipelinePlan, pipelineOperatorMap, currentPipeline,
                  currentOperators);
  } else if (currentOperators
                 ->instanceOf<PhysicalOperators::PhysicalSinkOperator>()) {
    processSink(pipelinePlan, pipelineOperatorMap, currentPipeline,
                currentOperators);
  } else if (currentOperators
                 ->instanceOf<PhysicalOperators::PhysicalUnionOperator>()) {
    processMultiplex(pipelinePlan, pipelineOperatorMap, currentPipeline,
                     currentOperators);
  } else if (currentOperators->instanceOf<
                 PhysicalOperators::PhysicalDemultiplexOperator>()) {
    processDemultiplex(pipelinePlan, pipelineOperatorMap, currentPipeline,
                       currentOperators);
  } else if (operatorFusionPolicy->isFusible(currentOperators)) {
    processFusibleOperator(pipelinePlan, pipelineOperatorMap, currentPipeline,
                           currentOperators);
  } else {
    processPipelineBreakerOperator(pipelinePlan, pipelineOperatorMap,
                                   currentPipeline, currentOperators);
  }
}
} // namespace NES::QueryCompilation
