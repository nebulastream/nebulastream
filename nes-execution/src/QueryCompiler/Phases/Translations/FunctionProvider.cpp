...
std::unique_ptr<Function> FunctionProvider::lowerFunction(const NodeFunctionPtr& nodeFunction)
{
    ...
    auto function
        = Execution::Functions::RegistryFunctionExecutable::instance().tryCreate(nodeFunction->getName(), std::move(childFunction));
    if (not function.has_value())
    {
        throw UnknownFunctionType(fmt::format("Can not lower function: {}", nodeFunction->getName()));
    }
