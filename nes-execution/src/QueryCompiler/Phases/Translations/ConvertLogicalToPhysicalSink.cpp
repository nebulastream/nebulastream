...
DataSinkPtr ConvertLogicalToPhysicalSink::createDataSink(
  ...
  if (sinkDescriptor->instanceOf<PrintSinkDescriptor>()) {
    NES_DEBUG("ConvertLogicalToPhysicalSink: Creating print sink {}",
              schema->toString());
    const PrintSinkDescriptorPtr printSinkDescriptor =
        sinkDescriptor->as<PrintSinkDescriptor>();
    return createCsvPrintSink(schema, pipelineQueryPlan->getQueryId(),
                              pipelineQueryPlan->getQuerySubPlanId(),
                              nodeEngine, numOfProducers, std::cout,
                              printSinkDescriptor->getNumberOfOrigins());
  } else if (sinkDescriptor->instanceOf<NullOutputSinkDescriptor>()) {
    const NullOutputSinkDescriptorPtr nullOutputSinkDescriptor =
        sinkDescriptor->as<NullOutputSinkDescriptor>();
    NES_DEBUG("ConvertLogicalToPhysicalSink: Creating nulloutput sink {}",
              schema->toString());
    return createNullOutputSink(pipelineQueryPlan->getQueryId(),
                                pipelineQueryPlan->getQuerySubPlanId(),
                                nodeEngine, numOfProducers,
    ...
    const ZmqSinkDescriptorPtr zmqSinkDescriptor =
        sinkDescriptor->as<ZmqSinkDescriptor>();
    return createBinaryZmqSink(
        schema, pipelineQueryPlan->getQueryId(),
        pipelineQueryPlan->getQuerySubPlanId(), nodeEngine, numOfProducers,
        zmqSinkDescriptor->getHost(), zmqSinkDescriptor->getPort(),
    ...
    const MonitoringSinkDescriptorPtr monitoringSinkDescriptor =
        sinkDescriptor->as<MonitoringSinkDescriptor>();
    return createMonitoringSink(nodeEngine->getMetricStore(),
                                monitoringSinkDescriptor->getCollectorType(),
                                schema, nodeEngine, numOfProducers,
    ...
    const KafkaSinkDescriptorPtr kafkaSinkDescriptor =
        sinkDescriptor->as<KafkaSinkDescriptor>();

    if (kafkaSinkDescriptor->getSinkFormatAsString() == "CSV_FORMAT") {
      return createCsvKafkaSink(
          schema, pipelineQueryPlan->getQueryId(),
          pipelineQueryPlan->getQuerySubPlanId(), nodeEngine, numOfProducers,
          kafkaSinkDescriptor->getBrokers(), kafkaSinkDescriptor->getTopic(),
          kafkaSinkDescriptor->getTimeout(),
          kafkaSinkDescriptor->getNumberOfOrigins());
    } else {
      NES_THROW_RUNTIME_ERROR("Sinkformat "
                              << kafkaSinkDescriptor->getSinkFormatAsString()
    ...
    const OPCSinkDescriptorPtr opcSinkDescriptor =
        sinkDescriptor->as<OPCSinkDescriptor>();
    return createOPCSink(
        schema, querySubPlan->getQueryId(), querySubPlan->getQuerySubPlanId(),
        nodeEngine, opcSinkDescriptor->getUrl(), opcSinkDescriptor->getNodeId(),
    ...
    const MQTTSinkDescriptorPtr mqttSinkDescriptor =
        sinkDescriptor->as<MQTTSinkDescriptor>();
    // Two MQTT clients with the same client-id can not communicate with the
    // same broker. Therefore, client-ids should generally be unique. If the
    // user does not pass a client-id explicitly, we utilize the operatorId to
    // generate a client-id that is guaranteed to be unique.
    std::string clientId = (mqttSinkDescriptor->getClientId() != "")
                               ? mqttSinkDescriptor->getClientId()
                               : operatorId.toString();
    return createMQTTSink(
        schema, pipelineQueryPlan->getQueryId(),
        pipelineQueryPlan->getQuerySubPlanId(), nodeEngine, numOfProducers,
        mqttSinkDescriptor->getAddress(), clientId,
        mqttSinkDescriptor->getTopic(), mqttSinkDescriptor->getUser(),
        mqttSinkDescriptor->getMaxBufferedMSGs(),
        mqttSinkDescriptor->getTimeUnit(), mqttSinkDescriptor->getMsgDelay(),
    ...
    if (fileSinkDescriptor->getSinkFormatAsString() == "CSV_FORMAT") {
      return createCSVFileSink(
          schema, pipelineQueryPlan->getQueryId(),
          pipelineQueryPlan->getQuerySubPlanId(), nodeEngine, numOfProducers,
          fileSinkDescriptor->getFileName(), fileSinkDescriptor->getAppend(),
    ...
    return createNetworkSink(
        schema, networkSinkDescriptor->getUniqueId(),
        pipelineQueryPlan->getQueryId(), pipelineQueryPlan->getQuerySubPlanId(),
        networkSinkDescriptor->getNodeLocation(),
        networkSinkDescriptor->getNesPartition(), nodeEngine, numOfProducers,
    ...
    const auto statisticSinkDescriptor =
        sinkDescriptor->as<Statistic::StatisticSinkDescriptor>();
    return createStatisticSink(schema, nodeEngine, numOfProducers,
                               pipelineQueryPlan->getQueryId(),
                               pipelineQueryPlan->getQuerySubPlanId(),
