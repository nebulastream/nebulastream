    ...
    size_t numOfProducers)
{
    ...
    else if (sinkDescriptor->instanceOf<MonitoringSinkDescriptor>())
    {
        ...
        return createMonitoringSink(
            nodeEngine->getMetricStore(),
    ...
    else if (sinkDescriptor->instanceOf<KafkaSinkDescriptor>())
    {
        ...
        {
            NES_THROW_RUNTIME_ERROR("Sinkformat " << kafkaSinkDescriptor->getSinkFormatAsString() << " currently not supported for Kafka");
        }
    }
#endif
    ...
    else if (sinkDescriptor->instanceOf<MQTTSinkDescriptor>())
    {
        ...
        std::string clientId = (mqttSinkDescriptor->getClientId() != "") ? mqttSinkDescriptor->getClientId() : operatorId.toString();
