...
std::shared_ptr<Expression> ExpressionProvider::lowerExpression(
  ...
  if (auto andNode = expressionNode->as_if<AndExpressionNode>()) {
    ...
    return std::make_shared<AndExpression>(leftNautilusExpression,
    ...
    return std::make_shared<OrExpression>(leftNautilusExpression,
                                          rightNautilusExpression);
    ...
    return std::make_shared<LessThanExpression>(leftNautilusExpression,
                                                rightNautilusExpression);
    ...
    return std::make_shared<EqualsExpression>(leftNautilusExpression,
                                              rightNautilusExpression);
  } else if (auto greaterNode =
                 expressionNode->as_if<GreaterExpressionNode>()) {
    ...
    return std::make_shared<GreaterThanExpression>(leftNautilusExpression,
                                                   rightNautilusExpression);
  } else if (auto greaterEqualsNode =
                 expressionNode->as_if<GreaterEqualsExpressionNode>()) {
    auto leftNautilusExpression = lowerExpression(greaterEqualsNode->getLeft());
    auto rightNautilusExpression =
        lowerExpression(greaterEqualsNode->getRight());
    return std::make_shared<GreaterEqualsExpression>(leftNautilusExpression,
                                                     rightNautilusExpression);
  } else if (auto lessEqualsNode =
                 expressionNode->as_if<LessEqualsExpressionNode>()) {
    ...
    return std::make_shared<LessEqualsExpression>(leftNautilusExpression,
                                                  rightNautilusExpression);
  } else if (auto negateNode = expressionNode->as_if<NegateExpressionNode>()) {
    auto child =
        lowerExpression(negateNode->getChildren()[0]->as<ExpressionNode>());
    ...
    return std::make_shared<MulExpression>(leftNautilusExpression,
                                           rightNautilusExpression);
    ...
    return std::make_shared<AddExpression>(leftNautilusExpression,
                                           rightNautilusExpression);
    ...
    return std::make_shared<SubExpression>(leftNautilusExpression,
                                           rightNautilusExpression);
    ...
    return std::make_shared<DivExpression>(leftNautilusExpression,
                                           rightNautilusExpression);
  } else if (auto functionExpression =
                 expressionNode->as_if<FunctionExpression>()) {
    return lowerFunctionExpression(functionExpression);
  } else if (auto constantValue =
                 expressionNode->as_if<ConstantValueExpressionNode>()) {
    return lowerConstantExpression(constantValue);
  } else if (auto fieldAccess =
                 expressionNode->as_if<FieldAccessExpressionNode>()) {
    return std::make_shared<ReadFieldExpression>(fieldAccess->getFieldName());
  }
  NES_NOT_IMPLEMENTED();
}

ExpressionPtr ExpressionProvider::lowerConstantExpression(
    const std::shared_ptr<ConstantValueExpressionNode>& constantExpression) {
  auto value = constantExpression->getConstantValue();
  auto physicalType = DefaultPhysicalTypeFactory().getPhysicalType(
      constantExpression->getStamp());
...
std::shared_ptr<Expression> ExpressionProvider::lowerFunctionExpression(
  ...
  auto functionProvider = ExecutableFunctionRegistry::createPlugin(
