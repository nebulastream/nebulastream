...
DefaultPhysicalOperatorProvider::DefaultPhysicalOperatorProvider(
...
bool DefaultPhysicalOperatorProvider::isDemultiplex(
    const LogicalOperatorPtr &operatorNode) {
  return operatorNode->getParents().size() > 1;
}

void DefaultPhysicalOperatorProvider::insertDemultiplexOperatorsBefore(
    const LogicalOperatorPtr &operatorNode) {
  ...
  auto demultiplexOperator =
      PhysicalOperators::PhysicalDemultiplexOperator::create(
          operatorNode->getStatisticId(), operatorOutputSchema);
...
void DefaultPhysicalOperatorProvider::insertMultiplexOperatorsAfter(
    const LogicalOperatorPtr &operatorNode) {
  // the unionOperator operator has the same schema as the output schema of the
  // operator node.
  auto unionOperator = PhysicalOperators::PhysicalUnionOperator::create(
      operatorNode->getStatisticId(), operatorNode->getOutputSchema());
...
void DefaultPhysicalOperatorProvider::lowerUnaryOperator(
  ...
  // If a unary operator has more than one parent, we introduce an implicit
  // multiplex operator before.
  ...
  if (operatorNode->getParents().size() > 1) {
    throw QueryCompilationException(
        "A unary operator should only have at most one parent.");
  }
  ...
  if (operatorNode->instanceOf<SourceLogicalOperator>()) {
    ...
    auto physicalSourceOperator =
        PhysicalOperators::PhysicalSourceOperator::create(
            getNextOperatorId(), operatorNode->getStatisticId(),
    ...
    physicalSourceOperator->addProperty("LogicalOperatorId",
                                        operatorNode->getId());
    ...
    auto physicalSinkOperator = PhysicalOperators::PhysicalSinkOperator::create(
        operatorNode->getStatisticId(), logicalSinkOperator->getInputSchema(),
        logicalSinkOperator->getOutputSchema(),
        logicalSinkOperator->getSinkDescriptor());
    physicalSinkOperator->addProperty("LogicalOperatorId",
                                      operatorNode->getId());
    operatorNode->replace(physicalSinkOperator);
    decomposedQueryPlan->replaceRootOperator(logicalSinkOperator,
    ...
    auto physicalFilterOperator =
        PhysicalOperators::PhysicalFilterOperator::create(
            operatorNode->getStatisticId(), filterOperator->getInputSchema(),
            filterOperator->getOutputSchema(), filterOperator->getPredicate());
    physicalFilterOperator->addProperty("LogicalOperatorId",
                                        operatorNode->getId());
    ...
    auto physicalLimitOperator =
        PhysicalOperators::PhysicalLimitOperator::create(
            operatorNode->getStatisticId(), limitOperator->getInputSchema(),
            limitOperator->getOutputSchema(), limitOperator->getLimit());
    operatorNode->replace(physicalLimitOperator);
  } else if (operatorNode
                 ->instanceOf<Statistic::LogicalStatisticWindowOperator>()) {
    lowerStatisticBuildOperator(
        *operatorNode->as<Statistic::LogicalStatisticWindowOperator>());
  } else {
    throw QueryCompilationException("No conversion for operator " +
...
void DefaultPhysicalOperatorProvider::lowerStatisticBuildOperator(
  ...
  const auto statisticDescriptor =
      logicalStatisticWindowOperator.getWindowStatisticDescriptor();
  if (statisticDescriptor->instanceOf<Statistic::CountMinDescriptor>()) {
    const auto countMinDescriptor =
        statisticDescriptor->as<Statistic::CountMinDescriptor>();
    auto physicalCountMinBuildOperator =
        PhysicalOperators::PhysicalCountMinBuildOperator::create(
            ...
            countMinDescriptor->getWidth(), countMinDescriptor->getDepth(),
    ...
    physicalCountMinBuildOperator->as<UnaryOperator>()->setInputOriginIds(
        logicalStatisticWindowOperator.getInputOriginIds());
    logicalStatisticWindowOperator.replace(physicalCountMinBuildOperator);
  } else if (statisticDescriptor
                 ->instanceOf<Statistic::HyperLogLogDescriptor>()) {
    const auto hyperLogLogDescriptor =
        statisticDescriptor->as<Statistic::HyperLogLogDescriptor>();
    auto physicalHyperLogLogBuildOperator =
        PhysicalOperators::PhysicalHyperLogLogBuildOperator::create(
            logicalStatisticWindowOperator.getStatisticId(),
            logicalStatisticWindowOperator.getInputSchema(),
            logicalStatisticWindowOperator.getOutputSchema(),
    ...
    NES_ERROR("We currently only support a CountMinStatisticDescriptor or "
              "HyperLogLogDescriptorStatisticDescriptor")
    NES_NOT_IMPLEMENTED();
  }
}
...
void DefaultPhysicalOperatorProvider::lowerUnionOperator(
    const LogicalOperatorPtr &operatorNode) {

  auto unionOperator = operatorNode->as<LogicalUnionOperator>();
  // this assumes that we apply the ProjectBeforeUnionRule and the input across
  // all children is the same.
  if (!unionOperator->getLeftInputSchema()->equals(
          unionOperator->getRightInputSchema())) {
    throw QueryCompilationException(
  ...
  auto physicalUnionOperator = PhysicalOperators::PhysicalUnionOperator::create(
      operatorNode->getStatisticId(), unionOperator->getLeftInputSchema());
  operatorNode->replace(physicalUnionOperator);
}

void DefaultPhysicalOperatorProvider::lowerProjectOperator(
    const LogicalOperatorPtr &operatorNode) {
  auto projectOperator = operatorNode->as<LogicalProjectionOperator>();
  auto physicalProjectOperator =
      PhysicalOperators::PhysicalProjectOperator::create(
          operatorNode->getStatisticId(), projectOperator->getInputSchema(),
          projectOperator->getOutputSchema(),
          projectOperator->getExpressions());

  physicalProjectOperator->addProperty("LogicalOperatorId",
                                       projectOperator->getId());
  operatorNode->replace(physicalProjectOperator);
}

void DefaultPhysicalOperatorProvider::lowerInferModelOperator(
    LogicalOperatorPtr operatorNode) {
  auto inferModelOperator =
      operatorNode->as<InferModel::LogicalInferModelOperator>();
  auto physicalInferModelOperator =
      PhysicalOperators::PhysicalInferModelOperator::create(
          operatorNode->getStatisticId(), inferModelOperator->getInputSchema(),
          inferModelOperator->getOutputSchema(), inferModelOperator->getModel(),
          inferModelOperator->getInputFields(),
          inferModelOperator->getOutputFields());
  operatorNode->replace(physicalInferModelOperator);
}

void DefaultPhysicalOperatorProvider::lowerMapOperator(
    const LogicalOperatorPtr &operatorNode) {
  auto mapOperator = operatorNode->as<LogicalMapOperator>();
  auto physicalMapOperator = PhysicalOperators::PhysicalMapOperator::create(
      operatorNode->getStatisticId(), mapOperator->getInputSchema(),
      mapOperator->getOutputSchema(), mapOperator->getMapExpression());
  physicalMapOperator->addProperty("LogicalOperatorId", operatorNode->getId());
  operatorNode->replace(physicalMapOperator);
}

void DefaultPhysicalOperatorProvider::lowerUDFMapOperator(
    const LogicalOperatorPtr &operatorNode) {
  ...
  auto physicalMapOperator = PhysicalOperators::PhysicalMapUDFOperator::create(
      operatorNode->getStatisticId(), mapUDFOperator->getInputSchema(),
      mapUDFOperator->getOutputSchema(), mapUDFOperator->getUDFDescriptor());
  physicalMapOperator->addProperty("LogicalOperatorId", operatorNode->getId());
  operatorNode->replace(physicalMapOperator);
}

void DefaultPhysicalOperatorProvider::lowerUDFFlatMapOperator(
    const LogicalOperatorPtr &operatorNode) {
  auto flatMapUDFOperator = operatorNode->as<FlatMapUDFLogicalOperator>();
  auto physicalMapOperator =
      PhysicalOperators::PhysicalFlatMapUDFOperator::create(
          operatorNode->getStatisticId(), flatMapUDFOperator->getInputSchema(),
...
OperatorPtr DefaultPhysicalOperatorProvider::getJoinBuildInputOperator(
    const LogicalJoinOperatorPtr &joinOperator, SchemaPtr outputSchema,
    std::vector<OperatorPtr> children) {
  if (children.empty()) {
    throw QueryCompilationException(
        "There should be at least one child for the join operator " +
        joinOperator->toString());
  }

  if (children.size() > 1) {
    auto demultiplexOperator = PhysicalOperators::PhysicalUnionOperator::create(
        joinOperator->getStatisticId(), std::move(outputSchema));
...
void DefaultPhysicalOperatorProvider::lowerNautilusJoin(
    const LogicalOperatorPtr &operatorNode) {
  ...
  const auto [timeStampFieldLeft, timeStampFieldRight] =
      getTimestampLeftAndRight(joinOperator, windowType);
  const auto leftInputOperator = getJoinBuildInputOperator(
      joinOperator, joinOperator->getLeftInputSchema(),
      joinOperator->getLeftOperators());
  const auto rightInputOperator = getJoinBuildInputOperator(
      joinOperator, joinOperator->getRightInputSchema(),
      joinOperator->getRightOperators());
  const auto joinStrategy = options->getStreamJoinStrategy();

  const StreamJoinOperators streamJoinOperators(operatorNode, leftInputOperator,
                                                rightInputOperator);
  const StreamJoinConfigs streamJoinConfig(
      equiJoinKeyNames.first, equiJoinKeyNames.second, windowSize, windowSlide,
      timeStampFieldLeft, timeStampFieldRight, joinStrategy);
  ...
  case StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE:
    joinOperatorHandler =
        lowerStreamingHashJoin(streamJoinOperators, streamJoinConfig);
    break;
  case StreamJoinStrategy::NESTED_LOOP_JOIN:
    joinOperatorHandler =
        lowerStreamingNestedLoopJoin(streamJoinOperators, streamJoinConfig);
    break;
  }
  ...
  auto createBuildOperator = [&](const SchemaPtr &inputSchema,
    ...
    return PhysicalOperators::PhysicalStreamJoinBuildOperator::create(
        operatorNode->getStatisticId(), inputSchema,
        joinOperator->getOutputSchema(), joinOperatorHandler, buildSideType,
        timeStampField, joinFieldName, options->getStreamJoinStrategy(),
        options->getWindowingStrategy());
  };

  const auto leftJoinBuildOperator = createBuildOperator(
      joinOperator->getLeftInputSchema(), JoinBuildSideType::Left,
      streamJoinConfig.timeStampFieldLeft, streamJoinConfig.joinFieldNameLeft);
  const auto rightJoinBuildOperator = createBuildOperator(
      joinOperator->getRightInputSchema(), JoinBuildSideType::Right,
      streamJoinConfig.timeStampFieldRight,
      streamJoinConfig.joinFieldNameRight);
  const auto joinProbeOperator =
      PhysicalOperators::PhysicalStreamJoinProbeOperator::create(
          operatorNode->getStatisticId(), joinOperator->getLeftInputSchema(),
          joinOperator->getRightInputSchema(), joinOperator->getOutputSchema(),
          joinOperator->getJoinExpression(),
          joinOperator->getWindowStartFieldName(),
          joinOperator->getWindowEndFieldName(), joinOperatorHandler,
          options->getStreamJoinStrategy(), options->getWindowingStrategy());

  streamJoinOperators.leftInputOperator->insertBetweenThisAndParentNodes(
      leftJoinBuildOperator);
  streamJoinOperators.rightInputOperator->insertBetweenThisAndParentNodes(
...
DefaultPhysicalOperatorProvider::lowerStreamingNestedLoopJoin(
    const StreamJoinOperators &streamJoinOperators,
    const StreamJoinConfigs &streamJoinConfig) {

  using namespace Runtime::Execution;
  const auto joinOperator =
      streamJoinOperators.operatorNode->as<LogicalJoinOperator>();
  ...
  if (options->getWindowingStrategy() == WindowingStrategy::SLICING) {
    return Operators::NLJOperatorHandlerSlicing::create(
        ...
        joinOperator->getOutputOriginIds()[0], streamJoinConfig.windowSize,
        streamJoinConfig.windowSlide, joinOperator->getLeftInputSchema(),
...
Runtime::Execution::Operators::StreamJoinOperatorHandlerPtr
DefaultPhysicalOperatorProvider::lowerStreamingHashJoin(
    const StreamJoinOperators &streamJoinOperators,
    const StreamJoinConfigs &streamJoinConfig) {
  using namespace Runtime::Execution;
  const auto joinOperator =
  ...
  if (options->getWindowingStrategy() == WindowingStrategy::SLICING) {
    return Operators::HJOperatorHandlerSlicing::create(
        ...
        joinOperator->getOutputOriginIds()[0], streamJoinConfig.windowSize,
        streamJoinConfig.windowSlide, joinOperator->getLeftInputSchema(),
        joinOperator->getRightInputSchema(), streamJoinConfig.joinStrategy,
...
DefaultPhysicalOperatorProvider::getTimestampLeftAndRight(
  ...
  if (windowType->getTimeCharacteristic()->getType() ==
    ...
    // FIXME Once #3407 is done, we can change this to get the left and right
    // fieldname
    auto timeStampFieldName =
        windowType->getTimeCharacteristic()->getField()->getName();
    auto timeStampFieldNameWithoutSourceName = timeStampFieldName.substr(
        timeStampFieldName.find(Schema::ATTRIBUTE_NAME_SEPARATOR));

    // Lambda function for extracting the timestamp from a schema
    auto findTimeStampFieldName = [&](const SchemaPtr &schema) {
      for (const auto &field : schema->fields) {
        if (field->getName().find(timeStampFieldNameWithoutSourceName) !=
            std::string::npos) {
    ...
    auto timeStampFieldNameLeft =
        findTimeStampFieldName(joinOperator->getLeftInputSchema());
    auto timeStampFieldNameRight =
        findTimeStampFieldName(joinOperator->getRightInputSchema());

    NES_ASSERT(
        !(timeStampFieldNameLeft.empty() || timeStampFieldNameRight.empty()),
        "Could not find timestampfieldname "
            << timeStampFieldNameWithoutSourceName << " in both streams!");
    NES_DEBUG("timeStampFieldNameLeft:{}  timeStampFieldNameRight:{} ",
              timeStampFieldNameLeft, timeStampFieldNameRight);

    return {TimestampField::EventTime(
...
void DefaultPhysicalOperatorProvider::lowerWatermarkAssignmentOperator(
    const LogicalOperatorPtr &operatorNode) {
  auto logicalWatermarkAssignment =
      operatorNode->as<WatermarkAssignerLogicalOperator>();
  auto physicalWatermarkAssignment =
      PhysicalOperators::PhysicalWatermarkAssignmentOperator::create(
  ...
  physicalWatermarkAssignment->setOutputSchema(
      logicalWatermarkAssignment->getOutputSchema());
...
void DefaultPhysicalOperatorProvider::lowerTimeBasedWindowOperator(
  ...
  auto windowOperatorProperties = WindowOperatorProperties(
      windowOperator, windowInputSchema, windowOutputSchema, windowDefinition);

  if (windowOperator->getInputOriginIds().empty()) {
    throw QueryCompilationException("The number of input origin IDs for an "
                                    "window operator should not be zero.");
  }

  // TODO this currently just mimics the old usage of the set of input origins.
  windowDefinition->setNumberOfInputEdges(
      windowOperator->getInputOriginIds().size());
  ...
  auto preAggregationOperator =
      PhysicalOperators::PhysicalSlicePreAggregationOperator::create(
          getNextOperatorId(), operatorNode->getStatisticId(),
          windowInputSchema, windowOutputSchema, windowDefinition);
  operatorNode->insertBetweenThisAndChildNodes(preAggregationOperator);

  // if we have a sliding window and use slicing we have to create another slice
  // merge operator
  if (timeBasedWindowType->instanceOf<Windowing::SlidingWindow>() &&
    ...
    auto mergingOperator =
        PhysicalOperators::PhysicalSliceMergingOperator::create(
            getNextOperatorId(), operatorNode->getStatisticId(),
            windowInputSchema, windowOutputSchema, windowDefinition);
  ...
  auto windowSink = PhysicalOperators::PhysicalWindowSinkOperator::create(
      getNextOperatorId(), operatorNode->getStatisticId(), windowInputSchema,
      windowOutputSchema, windowDefinition);
  operatorNode->replace(windowSink);
}

void DefaultPhysicalOperatorProvider::lowerWindowOperator(
    const LogicalOperatorPtr &operatorNode) {
  auto windowOperator = operatorNode->as<WindowOperator>();
  auto windowInputSchema = windowOperator->getInputSchema();
  auto windowOutputSchema = windowOperator->getOutputSchema();
  ...
  // create window operator handler, to establish a common Runtime object for
  // aggregation and trigger phase.
  if (operatorNode->instanceOf<LogicalWindowOperator>()) {
    // handle if threshold window
    // TODO: At this point we are already a central window, we do not want the
    // threshold window to become a Gentral Window in the first place
    auto windowType = operatorNode->as<WindowOperator>()
                          ->getWindowDefinition()
                          ->getWindowType();
    if (windowType->instanceOf<Windowing::ContentBasedWindowType>()) {
      ...
      if (contentBasedWindowType->getContentBasedSubWindowType() ==
        ...
        auto thresholdWindowPhysicalOperator =
            PhysicalOperators::PhysicalThresholdWindowOperator::create(
                operatorNode->getStatisticId(), windowInputSchema,
                windowOutputSchema, windowDefinition);
        thresholdWindowPhysicalOperator->addProperty("LogicalOperatorId",
                                                     operatorNode->getId());
        ...
        throw QueryCompilationException(
            "No support for this window type." +
