...
LowerToExecutableQueryPlanPhase::LowerToExecutableQueryPlanPhase(
    DataSinkProviderPtr sinkProvider, DataSourceProviderPtr sourceProvider)
...
LowerToExecutableQueryPlanPhasePtr LowerToExecutableQueryPlanPhase::create(
    const DataSinkProviderPtr& sinkProvider,
...
LowerToExecutableQueryPlanPhase::apply(
  ...
  std::map<PipelineId, Runtime::Execution::SuccessorExecutablePipeline>
  ...
  for (const auto& pipeline : sourcePipelines) {
    processSource(pipeline, sources, sinks, executablePipelines, nodeEngine,
                  pipelineQueryPlan, pipelineToExecutableMap);
  }
  ...
  return std::make_shared<Runtime::Execution::ExecutableQueryPlan>(
      ...
      std::move(sources), std::move(sinks), std::move(executablePipelines),
      nodeEngine->getQueryManager(), nodeEngine->getBufferManager());
}
Runtime::Execution::SuccessorExecutablePipeline
LowerToExecutableQueryPlanPhase::processSuccessor(
    const OperatorPipelinePtr& pipeline, std::vector<DataSourcePtr>& sources,
    std::vector<DataSinkPtr>& sinks,
    std::vector<Runtime::Execution::ExecutablePipelinePtr>& executablePipelines,
    const Runtime::NodeEnginePtr& nodeEngine,
    const PipelineQueryPlanPtr& pipelineQueryPlan,
  ...
  if (pipelineToExecutableMap.find(pipeline->getPipelineId()) !=
      pipelineToExecutableMap.end()) {
    return pipelineToExecutableMap.at(pipeline->getPipelineId());
  }

  if (pipeline->isSinkPipeline()) {
    auto executableSink =
        processSink(pipeline, sources, sinks, executablePipelines, nodeEngine,
                    pipelineQueryPlan);
  ...
  if (pipeline->isOperatorPipeline()) {
    auto executablePipeline = processOperatorPipeline(
        pipeline, sources, sinks, executablePipelines, nodeEngine,
        pipelineQueryPlan, pipelineToExecutableMap);
    pipelineToExecutableMap.insert(
        {pipeline->getPipelineId(), executablePipeline});
    return executablePipeline;
  }
  throw QueryCompilationException(
      "The pipeline was of wrong type. It should be a sink pipeline or a "
      "operator pipeline");
}
...
void LowerToExecutableQueryPlanPhase::processSource(
  ...
  auto sourceOperator =
      rootOperator->as<PhysicalOperators::PhysicalSourceOperator>();
  ...
  if (sourceDescriptor->instanceOf<LogicalSourceDescriptor>()) {
    ...
    for (const auto& physicalSourceType :
         nodeEngine->getPhysicalSourceTypes()) {
      // Check if logical and physical source name matches with any of the
      // physical source provided by the node
      if (physicalSourceType->getLogicalSourceName() == logicalSourceName &&
          physicalSourceType->getPhysicalSourceName() == physicalSourceName) {
        sourceDescriptor = createSourceDescriptor(sourceDescriptor->getSchema(),
                                                  physicalSourceType);
    ...
    if (!foundPhysicalSource) {
      throw QueryCompilationException(
          "Unable to find the Physical source with logical source name " +
          logicalSourceName + " and physical source name " +
          physicalSourceName);
    }
  }
  ...
  for (const auto& successor : pipeline->getSuccessors()) {
    auto executableSuccessor = processSuccessor(
        successor, sources, sinks, executablePipelines, nodeEngine,
        pipelineQueryPlan, pipelineToExecutableMap);
    executableSuccessorPipelines.emplace_back(executableSuccessor);
  }

  auto source = sourceProvider->lower(
      sourceOperator->getId(), sourceOperator->getOriginId(),
      sourceOperator->getStatisticId(), sourceDescriptor, nodeEngine,
      executableSuccessorPipelines);

  // Add this source as a predecessor to the pipeline execution context's of all
  // its children. This way you can navigate upstream.
  for (auto executableSuccessor : executableSuccessorPipelines) {
    if (const auto* nextExecutablePipeline =
            std::get_if<Runtime::Execution::ExecutablePipelinePtr>(
                &executableSuccessor)) {
      NES_DEBUG(
          "Adding current source operator: {} as a predecessor to its child "
          "pipeline: {}",
          source->getOperatorId(), (*nextExecutablePipeline)->getPipelineId());
...
Runtime::Execution::SuccessorExecutablePipeline
LowerToExecutableQueryPlanPhase::processSink(
    const OperatorPipelinePtr& pipeline, std::vector<DataSourcePtr>&,
    std::vector<DataSinkPtr>& sinks,
  ...
  auto rootOperator = pipeline->getDecomposedQueryPlan()->getRootOperators()[0];
  auto sinkOperator =
      rootOperator->as<PhysicalOperators::PhysicalSinkOperator>();
  auto numOfProducers = pipeline->getPredecessors().size();
  auto sink = sinkProvider->lower(
      sinkOperator->getId(), sinkOperator->getSinkDescriptor(),
      sinkOperator->getOutputSchema(), std::move(nodeEngine), pipelineQueryPlan,
      numOfProducers);
...
Runtime::Execution::SuccessorExecutablePipeline
LowerToExecutableQueryPlanPhase::processOperatorPipeline(
    const OperatorPipelinePtr& pipeline, std::vector<DataSourcePtr>& sources,
    std::vector<DataSinkPtr>& sinks,
    std::vector<Runtime::Execution::ExecutablePipelinePtr>& executablePipelines,
    const Runtime::NodeEnginePtr& nodeEngine,
        ...
        for (const auto& executableSuccessor : executableSuccessorPipelines) {
          if (const auto* sink =
            ...
            (*sink)->writeData(buffer, workerContext);
                         ...
                         std::get_if<Runtime::Execution::ExecutablePipelinePtr>(
                             &executableSuccessor)) {
            NES_TRACE("Emit Buffer to pipeline {}",
  ...
  auto emitToQueryManagerFunctionHandler =
      [executableSuccessorPipelines,
       queryManager](Runtime::TupleBuffer& buffer) {
        for (const auto& executableSuccessor : executableSuccessorPipelines) {
  ...
  auto executionContext =
      std::make_shared<Runtime::Execution::PipelineExecutionContext>(
          pipeline->getPipelineId(), pipelineQueryPlan->getQuerySubPlanId(),
          queryManager->getBufferManager(),
          queryManager->getNumberOfWorkerThreads(),
          emitToSuccessorFunctionHandler, emitToQueryManagerFunctionHandler,
          executableOperator->getOperatorHandlers());

  auto executablePipeline = Runtime::Execution::ExecutablePipeline::create(
      pipeline->getPipelineId(), pipelineQueryPlan->getQueryId(),
      pipelineQueryPlan->getQuerySubPlanId(), queryManager, executionContext,
      executableOperator->getExecutablePipelineStage(),
      pipeline->getPredecessors().size(), executableSuccessorPipelines);

  // Add this pipeline as a predecessor to the pipeline execution context's of
  // all its children. This way you can navigate upstream.
  for (auto executableSuccessor : executableSuccessorPipelines) {
    if (const auto* nextExecutablePipeline =
            std::get_if<Runtime::Execution::ExecutablePipelinePtr>(
                &executableSuccessor)) {
      NES_DEBUG(
          ...
          (*nextExecutablePipeline)->getPipelineId());
      (*nextExecutablePipeline)
          ->getContext()
          ->addPredecessor(executablePipeline);
    }
    // note: we do not register predecessors for DataSinks.
  }
...
SourceDescriptorPtr LowerToExecutableQueryPlanPhase::createSourceDescriptor(
    SchemaPtr schema, PhysicalSourceTypePtr physicalSourceType) {
  ...
  NES_DEBUG(
      "PhysicalSourceConfig: create Actual source descriptor with physical "
      "source: {} {} ",
      physicalSourceType->toString(), magic_enum::enum_name(sourceType));
  ...
  switch (sourceType) {
    case SourceType::DEFAULT_SOURCE: {
      ...
      return DefaultSourceDescriptor::create(
          schema, logicalSourceName,
          defaultSourceType->getNumberOfBuffersToProduce()->getValue(),
          std::chrono::milliseconds(
              defaultSourceType->getSourceGatheringInterval()->getValue())
              .count());
    }
    ...
    case SourceType::SENSE_SOURCE: {
      ...
      return SenseSourceDescriptor::create(
          schema, logicalSourceName, senseSourceType->getUdfs()->getValue());
    }
    case SourceType::MEMORY_SOURCE: {
      ...
      return MemorySourceDescriptor::create(
          schema, memorySourceType->getMemoryArea(),
          ...
          memorySourceType->getTaskQueueId(), logicalSourceName,
          physicalSourceName);
    }
    case SourceType::MONITORING_SOURCE: {
      auto monitoringSourceType =
          physicalSourceType->as<MonitoringSourceType>();
      return MonitoringSourceDescriptor::create(
          monitoringSourceType->getWaitTime(),
          Monitoring::MetricCollectorType(
              monitoringSourceType->getMetricCollectorType()));
    }
    case SourceType::BENCHMARK_SOURCE: {
      ...
      return BenchmarkSourceDescriptor::create(
          schema, benchmarkSourceType->getMemoryArea(),
          ...
          benchmarkSourceType->getTaskQueueId(), logicalSourceName,
          physicalSourceName);
    }
    case SourceType::STATIC_DATA_SOURCE: {
      auto staticDataSourceType =
          physicalSourceType->as<NES::Experimental::StaticDataSourceType>();
      return NES::Experimental::StaticDataSourceDescriptor::create(
          schema, staticDataSourceType->getPathTableFile(),
    ...
    case SourceType::LAMBDA_SOURCE: {
      ...
      return LambdaSourceDescriptor::create(
          schema, lambdaSourceType->getGenerationFunction(),
          ...
          lambdaSourceType->getTaskQueueId(), logicalSourceName,
          physicalSourceName);
    }
    case SourceType::TCP_SOURCE: {
      auto tcpSourceType = physicalSourceType->as<TCPSourceType>();
      return TCPSourceDescriptor::create(schema, tcpSourceType,
                                         logicalSourceName, physicalSourceName);
    }
    case SourceType::KAFKA_SOURCE: {
      ...
      return KafkaSourceDescriptor::create(
          schema, kafkaSourceType->getBrokers()->getValue(), logicalSourceName,
          ...
          kafkaSourceType->getOffsetMode()->getValue(), kafkaSourceType,
    ...
    default: {
      ...
      throw QueryCompilationException(
          "PhysicalSourceConfig:: source type " +
          physicalSourceType->getSourceTypeAsString() + " not supported");
    }
  }
}
