...
LowerToExecutableQueryPlanPhase::processSuccessor(
  ...
  throw QueryCompilationException("The pipeline was of wrong type. It should "
                                  "be a sink pipeline or a operator pipeline");
}
  ...
  for (auto executableSuccessor : executableSuccessorPipelines) {
    if (const auto *nextExecutablePipeline =
            std::get_if<Runtime::Execution::ExecutablePipelinePtr>(
                &executableSuccessor)) {
      NES_DEBUG("Adding current source operator: {} as a predecessor to its "
                "child pipeline: {}",
...
LowerToExecutableQueryPlanPhase::processOperatorPipeline(
  ...
  for (auto executableSuccessor : executableSuccessorPipelines) {
    if (const auto *nextExecutablePipeline =
            std::get_if<Runtime::Execution::ExecutablePipelinePtr>(
                &executableSuccessor)) {
      NES_DEBUG("Adding current pipeline: {} as a predecessor to its child "
                "pipeline: {}",
...
SourceDescriptorPtr LowerToExecutableQueryPlanPhase::createSourceDescriptor(
  ...
  NES_DEBUG("PhysicalSourceConfig: create Actual source descriptor with "
            "physical source: {} {} ",
