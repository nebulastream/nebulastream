...
DataSourcePtr ConvertLogicalToPhysicalSource::createDataSource(
  ...
  if (sourceDescriptor->instanceOf<BenchmarkSourceDescriptor>()) {
    ...
    auto benchmarkSourceDescriptor =
        sourceDescriptor->as<BenchmarkSourceDescriptor>();
    ...
    if (sourceAffinity != std::numeric_limits<uint64_t>::max()) {
      ...
      NES_ASSERT2_FMT(
          0 <= numaNodeIndex &&
              numaNodeIndex <
                  nodeEngine->getHardwareManager()->getNumberOfNumaRegions(),
  ...
  if (sourceDescriptor->instanceOf<ZmqSourceDescriptor>()) {
    ...
    const ZmqSourceDescriptorPtr zmqSourceDescriptor =
        sourceDescriptor->as<ZmqSourceDescriptor>();
    return createZmqSource(
        zmqSourceDescriptor->getSchema(), bufferManager, queryManager,
        zmqSourceDescriptor->getHost(), zmqSourceDescriptor->getPort(),
        operatorId, originId, statisticId, numSourceLocalBuffers,
  ...
  if (sourceDescriptor->instanceOf<DefaultSourceDescriptor>()) {
    ...
    const DefaultSourceDescriptorPtr defaultSourceDescriptor =
        sourceDescriptor->as<DefaultSourceDescriptor>();
    return createDefaultDataSourceWithSchemaForVarBuffers(
        ...
        defaultSourceDescriptor->getSourceGatheringIntervalCount(), operatorId,
        originId, statisticId, numSourceLocalBuffers,
        sourceDescriptor->getPhysicalSourceName(), successors);
    ...
    const BinarySourceDescriptorPtr binarySourceDescriptor =
        sourceDescriptor->as<BinarySourceDescriptor>();
    return createBinaryFileSource(
        binarySourceDescriptor->getSchema(), bufferManager, queryManager,
        binarySourceDescriptor->getFilePath(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
        sourceDescriptor->getPhysicalSourceName(), successors);
  } else if (sourceDescriptor->instanceOf<CsvSourceDescriptor>()) {
    NES_INFO("ConvertLogicalToPhysicalSource: Creating CSV file source");
    const CsvSourceDescriptorPtr csvSourceDescriptor =
        sourceDescriptor->as<CsvSourceDescriptor>();
    return createCSVFileSource(
        csvSourceDescriptor->getSchema(), bufferManager, queryManager,
        csvSourceDescriptor->getSourceConfig(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
        sourceDescriptor->getPhysicalSourceName(), successors);
    ...
    const KafkaSourceDescriptorPtr kafkaSourceDescriptor =
        sourceDescriptor->as<KafkaSourceDescriptor>();
    return createKafkaSource(
        kafkaSourceDescriptor->getSchema(), bufferManager, queryManager,
        ...
        kafkaSourceDescriptor->getSourceConfigPtr(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
    ...
    const MQTTSourceDescriptorPtr mqttSourceDescriptor =
        sourceDescriptor->as<MQTTSourceDescriptor>();
    return createMQTTSource(
        mqttSourceDescriptor->getSchema(), bufferManager, queryManager,
        mqttSourceDescriptor->getSourceConfigPtr(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
        sourceDescriptor->getPhysicalSourceName(), successors);
#endif
    ...
    NES_INFO("ConvertLogicalToPhysicalSource: Creating OPC source");
    const OPCSourceDescriptorPtr opcSourceDescriptor =
        sourceDescriptor->as<OPCSourceDescriptor>();
    return createOPCSource(
        opcSourceDescriptor->getSchema(), bufferManager, queryManager,
        opcSourceDescriptor->getUrl(), opcSourceDescriptor->getNodeId(),
        opcSourceDescriptor->getUser(), opcSourceDescriptor->getPassword(),
    ...
    const SenseSourceDescriptorPtr senseSourceDescriptor =
        sourceDescriptor->as<SenseSourceDescriptor>();
    return createSenseSource(
        senseSourceDescriptor->getSchema(), bufferManager, queryManager,
        senseSourceDescriptor->getUdfs(), operatorId, originId, statisticId,
        numSourceLocalBuffers, sourceDescriptor->getPhysicalSourceName(),
        successors);
    ...
    return createNetworkSource(
        networkSourceDescriptor->getSchema(), bufferManager, queryManager,
        networkManager, networkSourceDescriptor->getNesPartition(),
        networkSourceDescriptor->getNodeLocation(), numSourceLocalBuffers,
        networkSourceDescriptor->getWaitTime(),
        networkSourceDescriptor->getRetryTimes(),
        sourceDescriptor->getPhysicalSourceName(),
        networkSourceDescriptor->getVersion(), successors,
        networkSourceDescriptor->getUniqueId());
  } else if (sourceDescriptor->instanceOf<MemorySourceDescriptor>()) {
    NES_INFO("ConvertLogicalToPhysicalSource: Creating memory source");
    auto memorySourceDescriptor =
        sourceDescriptor->as<MemorySourceDescriptor>();
    return createMemorySource(
        memorySourceDescriptor->getSchema(), bufferManager, queryManager,
        ...
        memorySourceDescriptor->getGatheringValue(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
    ...
    auto monitoringSourceDescriptor =
        sourceDescriptor->as<MonitoringSourceDescriptor>();
    auto metricCollector =
        Monitoring::MetricUtils::createCollectorFromCollectorType(
    ...
    return createMonitoringSource(
        metricCollector, monitoringSourceDescriptor->getWaitTime(),
        bufferManager, queryManager, operatorId, originId, statisticId,
        numSourceLocalBuffers, sourceDescriptor->getPhysicalSourceName(),
        successors);
  } else if (sourceDescriptor->instanceOf<
                 NES::Experimental::StaticDataSourceDescriptor>()) {
    NES_INFO("ConvertLogicalToPhysicalSource: Creating static data source");
    auto staticDataSourceDescriptor =
        sourceDescriptor->as<NES::Experimental::StaticDataSourceDescriptor>();
    return NES::Experimental::createStaticDataSource(
        staticDataSourceDescriptor->getSchema(),
        staticDataSourceDescriptor->getPathTableFile(),
        staticDataSourceDescriptor->getLateStart(), bufferManager, queryManager,
        operatorId, originId, statisticId, numSourceLocalBuffers,
        sourceDescriptor->getPhysicalSourceName(), successors);
    ...
    auto benchmarkSourceDescriptor =
        sourceDescriptor->as<BenchmarkSourceDescriptor>();
    return createBenchmarkSource(
        benchmarkSourceDescriptor->getSchema(), bufferManager, queryManager,
        ...
        benchmarkSourceDescriptor->getGatheringValue(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
    ...
    auto lambdaSourceDescriptor =
        sourceDescriptor->as<LambdaSourceDescriptor>();
    return createLambdaSource(
        lambdaSourceDescriptor->getSchema(), bufferManager, queryManager,
    ...
    return createTCPSource(
        tcpSourceDescriptor->getSchema(), bufferManager, queryManager,
        tcpSourceDescriptor->getSourceConfig(), operatorId, originId,
        statisticId, numSourceLocalBuffers,
        sourceDescriptor->getPhysicalSourceName(), successors);
  } else {
    NES_ERROR(
        "ConvertLogicalToPhysicalSource: Unknown Source Descriptor Type {}",
