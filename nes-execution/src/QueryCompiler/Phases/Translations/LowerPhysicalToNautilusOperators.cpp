...
LowerPhysicalToNautilusOperators::LowerPhysicalToNautilusOperators::create(
...
LowerPhysicalToNautilusOperators::apply(PipelineQueryPlanPtr pipelinedQueryPlan,
                                        size_t bufferSize) {
...
OperatorPipelinePtr
LowerPhysicalToNautilusOperators::apply(OperatorPipelinePtr operatorPipeline,
                                        size_t bufferSize) {
  auto decomposedQueryPlan = operatorPipeline->getDecomposedQueryPlan();
  auto nodes = PlanIterator(decomposedQueryPlan).snapshot();
  auto pipeline =
      std::make_shared<Runtime::Execution::PhysicalOperatorPipeline>();
  std::vector<Runtime::Execution::OperatorHandlerPtr> operatorHandlers;
  std::shared_ptr<Runtime::Execution::Operators::Operator> parentOperator;

  for (const auto &node : nodes) {
    NES_INFO("Lowering node: {}", node->toString());
    parentOperator = lower(*pipeline, parentOperator,
                           node->as<PhysicalOperators::PhysicalOperator>(),
                           bufferSize, operatorHandlers);
    parentOperator->setStatisticId(
        node->as<PhysicalOperators::PhysicalOperator>()->getStatisticId());
  }
...
LowerPhysicalToNautilusOperators::lower(
  ...
  if (operatorNode->instanceOf<PhysicalOperators::PhysicalScanOperator>()) {
    ...
    return scan;
                 ...
                 ->instanceOf<PhysicalOperators::PhysicalEmitOperator>()) {
    ...
    return filter;
                 ...
                 ->instanceOf<PhysicalOperators::PhysicalLimitOperator>()) {
    auto limit = lowerLimit(pipeline, operatorNode, operatorHandlers);
    parentOperator->setChild(limit);
    return limit;
  } else if (operatorNode
                 ->instanceOf<PhysicalOperators::PhysicalMapOperator>()) {
    auto map = lowerMap(pipeline, operatorNode);
    parentOperator->setChild(map);
    return map;
  } else if (operatorNode
                 ->instanceOf<PhysicalOperators::PhysicalMapUDFOperator>()) {
    // for creating the handler that the nautilus udf operator needs to execute
    // the udf
    const auto udfOperator =
        operatorNode->as<PhysicalOperators::PhysicalMapUDFOperator>();
    ...
    if (udfDescriptor->instanceOf<Catalogs::UDF::JavaUDFDescriptor>()) {
      ...
      const auto handler = std::make_shared<
          Runtime::Execution::Operators::JavaUDFOperatorHandler>(
          className, methodName, inputClassName, outputClassName, byteCodeList,
          serializedInstance, udfInputSchema, udfOutputSchema, std::nullopt);

      operatorHandlers.push_back(handler);
      const auto indexForThisHandler = operatorHandlers.size() - 1;

      auto mapJavaUDF =
          std::make_shared<Runtime::Execution::Operators::MapJavaUDF>(
              indexForThisHandler, operatorInputSchema, operatorOutputSchema);
      parentOperator->setChild(mapJavaUDF);
      return mapJavaUDF;
#endif // ENABLE_JNI
#ifdef NAUTILUS_PYTHON_UDF_ENABLED
    } else if (udfDescriptor
                   ->instanceOf<Catalogs::UDF::PythonUDFDescriptor>()) {
      // creating the python udf handler
      const auto pythonUDFDescriptor =
          udfDescriptor->as<Catalogs::UDF::PythonUDFDescriptor>(udfDescriptor);
      const auto functionString = pythonUDFDescriptor->getFunctionString();

      const auto handler = std::make_shared<
          Runtime::Execution::Operators::PythonUDFOperatorHandler>(
          functionString, methodName, udfInputSchema, udfOutputSchema);
      operatorHandlers.push_back(handler);
      const auto indexForThisHandler = operatorHandlers.size() - 1;

      // auto mapPythonUDF = lowerMapPythonUDF(pipeline, operatorNode,
      // indexForThisHandler);
      auto mapPythonUDF =
          std::make_shared<Runtime::Execution::Operators::MapPythonUDF>(
              indexForThisHandler, operatorInputSchema, operatorOutputSchema);
...
#ifdef ENABLE_JNI
  } else if (operatorNode->instanceOf<
                 PhysicalOperators::PhysicalFlatMapUDFOperator>()) {
    // for creating the handler that the nautilus udf operator needs to execute
    // the udf
    const auto udfOperator =
        operatorNode->as<PhysicalOperators::PhysicalFlatMapUDFOperator>();
    const auto udfDescriptor = udfOperator->getUDFDescriptor();
    const auto methodName = udfDescriptor->getMethodName();
    const auto udfInputSchema = udfDescriptor->getInputSchema();
    const auto udfOutputSchema = udfDescriptor->getOutputSchema();
    ...
    if (udfDescriptor->instanceOf<Catalogs::UDF::JavaUDFDescriptor>()) {
      ...
      const auto indexForThisHandler = operatorHandlers.size() - 1;
      auto flatMapJavaUDF =
          std::make_shared<Runtime::Execution::Operators::FlatMapJavaUDF>(
              indexForThisHandler, operatorInputSchema, operatorOutputSchema);
      ...
      return flatMapJavaUDF;
                 ...
                 PhysicalOperators::PhysicalThresholdWindowOperator>()) {
    auto aggs =
        operatorNode->as<PhysicalOperators::PhysicalThresholdWindowOperator>()
            ->getWindowDefinition()
    ...
    for (size_t i = 0; i < aggs.size(); ++i) {
      ...
      aggValues.emplace_back(getAggregationValueForThresholdWindow(
          aggregationType, aggs[i]->getInputStamp()));
    }
    // pass aggValues to ThresholdWindowHandler
    auto handler = std::make_shared<
        Runtime::Execution::Operators::NonKeyedThresholdWindowOperatorHandler>(
        std::move(aggValues));
    operatorHandlers.push_back(handler);
    auto indexForThisHandler = operatorHandlers.size() - 1;
    auto thresholdWindow =
        lowerThresholdWindow(pipeline, operatorNode, indexForThisHandler);
    ...
    auto preAggregationOperator =
        lowerPreAggregationOperator(pipeline, operatorNode, operatorHandlers);
    parentOperator->setChild(preAggregationOperator);
    return preAggregationOperator;
  } else if (operatorNode->instanceOf<
                 PhysicalOperators::PhysicalSliceMergingOperator>()) {
    return lowerSliceMergingOperator(pipeline, operatorNode, operatorHandlers);
  } else if (operatorNode->instanceOf<
                 PhysicalOperators::PhysicalWindowSinkOperator>()) {
    ...
    auto watermarkOperator = lowerWatermarkAssignmentOperator(
        pipeline, operatorNode, operatorHandlers);
    ...
    auto projectOperator =
        operatorNode->as<PhysicalOperators::PhysicalProjectOperator>();
    ...
    return projection;
                 ...
                 PhysicalOperators::PhysicalStreamJoinProbeOperator>()) {
    auto probeOperator =
        operatorNode->as<PhysicalOperators::PhysicalStreamJoinProbeOperator>();
    ...
    case StreamJoinStrategy::HASH_JOIN_VAR_SIZED:
      joinProbeNautilus =
          std::make_shared<Runtime::Execution::Operators::HJProbeVarSized>(
              handlerIndex, probeOperator->getJoinSchema(), joinExpression,
    ...
    case StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE:
      joinProbeNautilus =
          std::make_shared<Runtime::Execution::Operators::HJProbe>(
              handlerIndex, probeOperator->getJoinSchema(), joinExpression,
              probeOperator->getWindowMetaData(),
              probeOperator->getJoinStrategy(),
              probeOperator->getWindowingStrategy());
    ...
    case StreamJoinStrategy::NESTED_LOOP_JOIN:
      ...
      joinProbeNautilus =
          std::make_shared<Runtime::Execution::Operators::NLJProbe>(
              handlerIndex, probeOperator->getJoinSchema(), joinExpression,
              probeOperator->getWindowMetaData(), leftSchema, rightSchema,
              probeOperator->getJoinStrategy(),
              probeOperator->getWindowingStrategy());
      break;
    }
    ...
    return joinProbeNautilus;
                 ...
                 PhysicalOperators::PhysicalStreamJoinBuildOperator>()) {
    using namespace Runtime::Execution;

    auto buildOperator =
        operatorNode->as<PhysicalOperators::PhysicalStreamJoinBuildOperator>();
    ...
    case StreamJoinStrategy::HASH_JOIN_VAR_SIZED:
      ...
      case WindowingStrategy::SLICING:
        joinBuildNautilus = lowerHJSlicingVarSized(buildOperator, handlerIndex,
    ...
    case StreamJoinStrategy::HASH_JOIN_GLOBAL_LOCK_FREE: {
      ...
      case WindowingStrategy::LEGACY:
        NES_NOT_IMPLEMENTED();
      case WindowingStrategy::SLICING:
        joinBuildNautilus = lowerHJSlicing(buildOperator, handlerIndex,
                                           std::move(timeFunction));
        break;
      case WindowingStrategy::BUCKETING:
        joinBuildNautilus =
            lowerHJBucketing(buildOperator, handlerIndex,
                             std::move(timeFunction), windowSize, windowSlide);
        break;
      }
      break;
    };
    case StreamJoinStrategy::NESTED_LOOP_JOIN: {
      ...
      case WindowingStrategy::LEGACY:
        NES_NOT_IMPLEMENTED();
      case WindowingStrategy::SLICING:
        joinBuildNautilus = lowerNLJSlicing(buildOperator, handlerIndex,
                                            std::move(timeFunction));
        break;
      case WindowingStrategy::BUCKETING:
        joinBuildNautilus =
            lowerNLJBucketing(buildOperator, handlerIndex,
                              std::move(timeFunction), windowSize, windowSlide);
        break;
      }
      break;
    ...
    return joinBuildNautilus;
                 ...
                 PhysicalOperators::PhysicalCountMinBuildOperator>()) {
    const auto physicalCountMinBuild =
        operatorNode
            ->as<const PhysicalOperators::PhysicalCountMinBuildOperator>();
    auto countMinBuildOperator = lowerCountMinBuildOperator(
        *physicalCountMinBuild, operatorHandlers, bufferSize);
    parentOperator->setChild(countMinBuildOperator);
    return countMinBuildOperator;
  } else if (operatorNode->instanceOf<
                 PhysicalOperators::PhysicalHyperLogLogBuildOperator>()) {
    const auto physicalHyperLogLog =
        operatorNode
            ->as<const PhysicalOperators::PhysicalHyperLogLogBuildOperator>();
    auto hyperLogLogBuildOperator = lowerHyperLogLogBuildOperator(
        *physicalHyperLogLog, operatorHandlers, bufferSize);
...
LowerPhysicalToNautilusOperators::lowerCountMinBuildOperator(
    const PhysicalOperators::PhysicalCountMinBuildOperator
        &physicalCountMinBuild,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers,
    uint64_t bufferSize) {
  ...
  const auto fieldToTrackFieldName =
      physicalCountMinBuild.getNameOfFieldToTrack();
  const auto numberOfBitsInKey =
      8 * defaultPhysicalTypeFactory
              .getPhysicalType(physicalCountMinBuild.getInputSchema()
                                   ->getField(fieldToTrackFieldName)
                                   ->getDataType())
              ->size();
  ...
  const auto outputMemoryLayout = ::NES::Util::createMemoryLayout(
      physicalCountMinBuild.getOutputSchema(), bufferSize);
  ...
  const auto statisticFormat =
      Statistic::StatisticFormatFactory::createFromSchema(
          physicalCountMinBuild.getOutputSchema(), bufferSize,
          Statistic::StatisticSynopsisType::COUNT_MIN, sinkDataCodec);

  // 2. Getting the windowSize, windowSlide, and timestampFieldName.
  const auto windowType = physicalCountMinBuild.getWindowType()
                              ->as<Windowing::TimeBasedWindowType>();
  NES_ASSERT(windowType->instanceOf<Windowing::TumblingWindow>() ||
                 windowType->instanceOf<Windowing::SlidingWindow>(),
             "Only a tumbling or sliding window is currently supported for "
             "CountMinBuildOperator");
  auto [windowSize, windowSlide, timeFunction] =
      Util::getWindowingParameters(*windowType);

  // 3. Create operator handler
  auto countMinBuildOperatorHandler = CountMinOperatorHandler::create(
      windowSize, windowSlide, sendingPolicy, width, depth, statisticFormat,
      inputOriginIds, numberOfBitsInKey);
  operatorHandlers.push_back(countMinBuildOperatorHandler);
  auto handlerIndex = operatorHandlers.size() - 1;

  // 4. Creating the operator
  return std::make_shared<CountMinBuild>(handlerIndex, fieldToTrackFieldName,
                                         numberOfBitsInKey, width, depth,
                                         metricHash, std::move(timeFunction));
}

Runtime::Execution::Operators::ExecutableOperatorPtr
LowerPhysicalToNautilusOperators::lowerHyperLogLogBuildOperator(
    const PhysicalOperators::PhysicalHyperLogLogBuildOperator
        &physicalHLLBuildOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers,
    uint64_t bufferSize) {
  using namespace Runtime::Execution::Operators;
  ...
  const auto statisticFormat =
      Statistic::StatisticFormatFactory::createFromSchema(
          physicalHLLBuildOperator.getOutputSchema(), bufferSize,
          Statistic::StatisticSynopsisType::HLL, sinkDataCodec);

  // 2. Getting the windowSize, windowSlide, and timestampFieldName. We will
  // refactor this in #4739
  const auto windowType = physicalHLLBuildOperator.getWindowType()
                              ->as<Windowing::TimeBasedWindowType>();
  NES_ASSERT(windowType->instanceOf<Windowing::TumblingWindow>() ||
                 windowType->instanceOf<Windowing::SlidingWindow>(),
             "Only a tumbling or sliding window is currently supported for "
  ...
  // 3. Create operator handler
  auto hyperLogLogBuildOperatorHandler = HyperLogLogOperatorHandler::create(
      windowSize, windowSlide, sendingPolicy, statisticFormat, width,
      inputOriginIds);
  operatorHandlers.push_back(hyperLogLogBuildOperatorHandler);
  auto handlerIndex = operatorHandlers.size() - 1;

  // 4. Creating the operator
  return std::make_shared<HyperLogLogBuild>(
      handlerIndex, fieldToTrackFieldName, metricHash, std::move(timeFunction));
}

Runtime::Execution::Operators::ExecutableOperatorPtr
LowerPhysicalToNautilusOperators::lowerNLJSlicing(
    std::shared_ptr<PhysicalOperators::PhysicalStreamJoinBuildOperator>
        buildOperator,
  ...
  return std::make_shared<Runtime::Execution::Operators::NLJBuildSlicing>(
      operatorHandlerIndex, buildOperator->getInputSchema(),
      buildOperator->getJoinFieldName(), buildOperator->getBuildSide(),
      buildOperator->getInputSchema()->getSchemaSizeInBytes(),
      std::move(timeFunction), buildOperator->getJoinStrategy(),
      buildOperator->getWindowingStrategy());
}
Runtime::Execution::Operators::ExecutableOperatorPtr
LowerPhysicalToNautilusOperators::lowerNLJBucketing(
    std::shared_ptr<PhysicalOperators::PhysicalStreamJoinBuildOperator>
        buildOperator,
    uint64_t operatorHandlerIndex,
    Runtime::Execution::Operators::TimeFunctionPtr timeFunction,
  ...
  return std::make_shared<Runtime::Execution::Operators::NLJBuildBucketing>(
      ...
      buildOperator->getInputSchema()->getSchemaSizeInBytes(),
      std::move(timeFunction), buildOperator->getJoinStrategy(),
      buildOperator->getWindowingStrategy(), windowSize, windowSlide);
}

Runtime::Execution::Operators::ExecutableOperatorPtr
LowerPhysicalToNautilusOperators::lowerHJSlicing(
    const std::shared_ptr<PhysicalOperators::PhysicalStreamJoinBuildOperator>
        &buildOperator,
    uint64_t operatorHandlerIndex,
    Runtime::Execution::Operators::TimeFunctionPtr timeFunction) {
...
Runtime::Execution::Operators::ExecutableOperatorPtr
LowerPhysicalToNautilusOperators::lowerHJSlicingVarSized(
    const std::shared_ptr<PhysicalOperators::PhysicalStreamJoinBuildOperator>
        &buildOperator,
    uint64_t operatorHandlerIndex,
    Runtime::Execution::Operators::TimeFunctionPtr timeFunction) {
  return std::make_shared<
      Runtime::Execution::Operators::HJBuildSlicingVarSized>(
      operatorHandlerIndex, buildOperator->getInputSchema(),
      buildOperator->getJoinFieldName(), buildOperator->getBuildSide(),
      buildOperator->getInputSchema()->getSchemaSizeInBytes(),
      std::move(timeFunction), buildOperator->getJoinStrategy(),
...
Runtime::Execution::Operators::ExecutableOperatorPtr
LowerPhysicalToNautilusOperators::lowerHJBucketing(
    const std::shared_ptr<PhysicalOperators::PhysicalStreamJoinBuildOperator>
        &buildOperator,
    uint64_t operatorHandlerIndex,
    Runtime::Execution::Operators::TimeFunctionPtr timeFunction,
...
std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerWindowSinkOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {
  auto sinkOperator =
      physicalOperator->as<PhysicalOperators::PhysicalWindowSinkOperator>();
  if (sinkOperator->getWindowDefinition()->isKeyed()) {
    return lowerKeyedWindowSinkOperator(pipeline, physicalOperator,
                                        operatorHandlers);
  } else {
    return lowerNonKeyedWindowSinkOperator(pipeline, physicalOperator,
                                           operatorHandlers);
  }
}

std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerKeyedWindowSinkOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {
  auto physicalSWS =
  ...
  auto aggregations =
      physicalSWS->getWindowDefinition()->getWindowAggregation();
  auto aggregationFunctions = lowerAggregations(aggregations);
  // We assume that the first field of the output schema is the window start ts,
  // and the second field is the window end ts.
  // TODO this information should be stored in the logical window descriptor
  // otherwise this assumption may fail in the future.
  ...
  for (const auto &key : keys) {
    ...
    resultKeyDataTypes.emplace_back(
        DefaultPhysicalTypeFactory().getPhysicalType(key->getStamp()));
  }
  uint64_t keySize = 0;
  ...
  std::unique_ptr<Runtime::Execution::Operators::SliceMergingAction>
      sliceMergingAction = std::make_unique<
          Runtime::Execution::Operators::KeyedWindowEmitAction>(
          aggregationFunctions, startTs, endTs, keySize, valueSize,
          resultKeyFields, resultKeyDataTypes,
          physicalSWS->getWindowDefinition()->getOriginId());
  auto handler = std::make_shared<
      Runtime::Execution::Operators::KeyedSliceMergingHandler>();
  operatorHandlers.emplace_back(handler);
  auto sliceMergingOperator =
      std::make_shared<Runtime::Execution::Operators::KeyedSliceMerging>(
          operatorHandlers.size() - 1, aggregationFunctions,
          std::move(sliceMergingAction), resultKeyDataTypes, keySize,
          valueSize);
...
std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerNonKeyedWindowSinkOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {
  auto physicalSWS =
  ...
  std::unique_ptr<Runtime::Execution::Operators::SliceMergingAction>
      sliceMergingAction = std::make_unique<
          Runtime::Execution::Operators::NonKeyedWindowEmitAction>(
          aggregationFunctions, startTs, endTs,
          physicalSWS->getWindowDefinition()->getOriginId());

  auto handler = std::make_shared<
      Runtime::Execution::Operators::NonKeyedSliceMergingHandler>();
  operatorHandlers.emplace_back(handler);
  auto nonKeyedSliceMergingOperator =
      std::make_shared<Runtime::Execution::Operators::NonKeyedSliceMerging>(
          operatorHandlers.size() - 1, aggregationFunctions,
...
std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerSliceMergingOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {

  auto sliceMerging =
      physicalOperator->as<PhysicalOperators::PhysicalSliceMergingOperator>();
  if (sliceMerging->getWindowDefinition()->isKeyed()) {
    return lowerKeyedSliceMergingOperator(pipeline, physicalOperator,
                                          operatorHandlers);
  } else {
    return lowerNonKeyedSliceMergingOperator(pipeline, physicalOperator,
                                             operatorHandlers);
  }
}

std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerNonKeyedSliceMergingOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {
  auto physicalGSMO =
  ...
  auto aggregations =
      physicalGSMO->getWindowDefinition()->getWindowAggregation();
  auto aggregationFunctions = lowerAggregations(aggregations);
  // We assume that the first field of the output schema is the window start ts,
  // and the second field is the window end ts.
  // TODO this information should be stored in the logical window descriptor
  ...
  auto isTumblingWindow = std::dynamic_pointer_cast<Windowing::TumblingWindow>(
                              windowType) != nullptr
                              ? true
                              : false;
  ...
  std::unique_ptr<Runtime::Execution::Operators::SliceMergingAction>
      sliceMergingAction;
  if (isTumblingWindow ||
      options->getWindowingStrategy() == WindowingStrategy::BUCKETING) {
    sliceMergingAction = std::make_unique<
        Runtime::Execution::Operators::NonKeyedWindowEmitAction>(
        aggregationFunctions, startTs, endTs,
        physicalGSMO->getWindowDefinition()->getOriginId());
  } else {
    const auto timeBasedWindowType = physicalGSMO->getWindowDefinition()
                                         ->getWindowType()
                                         ->as<Windowing::TimeBasedWindowType>();
    const auto &[windowSize, windowSlide, _] =
        Util::getWindowingParameters(*timeBasedWindowType);
    auto actionHandler = std::make_shared<
        Runtime::Execution::Operators::NonKeyedAppendToSliceStoreHandler>(
        windowSize, windowSlide);
    operatorHandlers.emplace_back(actionHandler);
    sliceMergingAction = std::make_unique<
        Runtime::Execution::Operators::NonKeyedAppendToSliceStoreAction>(
        operatorHandlers.size() - 1);
  }
  auto nonKeyedSliceMergingOperator =
      std::make_shared<Runtime::Execution::Operators::NonKeyedSliceMerging>(
          sliceMergingOperatorHandlerIndex, aggregationFunctions,
          std::move(sliceMergingAction));
...
std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerKeyedSliceMergingOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {
  auto physicalGSMO =
  ...
  if (isTumblingWindow ||
    ...
    const auto windowType = physicalGSMO->getWindowDefinition()
                                ->getWindowType()
                                ->as<Windowing::TimeBasedWindowType>();
    const auto &[windowSize, windowSlide, timeFunction] =
        Util::getWindowingParameters(*windowType);
    auto actionHandler = std::make_shared<
        Runtime::Execution::Operators::KeyedAppendToSliceStoreHandler>(
        windowSize, windowSlide);
    operatorHandlers.emplace_back(actionHandler);
    sliceMergingAction = std::make_unique<
        Runtime::Execution::Operators::KeyedAppendToSliceStoreAction>(
        operatorHandlers.size() - 1);
  }
...
LowerPhysicalToNautilusOperators::lowerTimeFunction(
    const Windowing::TimeBasedWindowTypePtr &timeWindow) {
  // Depending on the window type we create a different time function.
  // If the window type is ingestion time or we use the special record creation
  // ts field, create an ingestion time function.
  // TODO remove record creation ts if it is not needed anymore
  if (timeWindow->getTimeCharacteristic()->getType() ==
  ...
  } else if (timeWindow->getTimeCharacteristic()->getType() ==
             Windowing::TimeCharacteristic::Type::EventTime) {
    // For event time fields, we look up the reference field name and create an
    // expression to read the field.
    auto timeCharacteristicField =
        timeWindow->getTimeCharacteristic()->getField()->getName();
    auto timeStampField =
        std::make_shared<Runtime::Execution::Expressions::ReadFieldExpression>(
            timeCharacteristicField);
  ...
  NES_THROW_RUNTIME_ERROR(
      "Timefunction could not be created for the following window definition: "
      << timeWindow->toString());
}

std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerPreAggregationOperator(
    Runtime::Execution::PhysicalOperatorPipeline &pipeline,
    const PhysicalOperators::PhysicalOperatorPtr &physicalOperator,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {

  auto physicalPreAggregation =
      physicalOperator
          ->as<PhysicalOperators::PhysicalSlicePreAggregationOperator>();
  if (physicalPreAggregation->getWindowDefinition()->isKeyed()) {
    return lowerKeyedPreAggregationOperator(pipeline, physicalPreAggregation,
                                            operatorHandlers);
  } else {
    return lowerNonKeyedPreAggregationOperator(pipeline, physicalPreAggregation,
                                               operatorHandlers);
  }
}
...
LowerPhysicalToNautilusOperators::lowerNonKeyedPreAggregationOperator(
  ...
  auto aggregations =
      physicalGTLPAO->getWindowDefinition()->getWindowAggregation();
  auto aggregationFunctions = lowerAggregations(aggregations);
  auto timeWindow =
      windowDefinition->getWindowType()->as<Windowing::TimeBasedWindowType>();
  auto timeFunction = lowerTimeFunction(timeWindow);

  if (options->getWindowingStrategy() == WindowingStrategy::SLICING) {

    auto handler = std::make_shared<
        Runtime::Execution::Operators::NonKeyedSlicePreAggregationHandler>(
        timeWindow->getSize().getTime(), timeWindow->getSlide().getTime(),
        windowDefinition->getInputOriginIds());

    operatorHandlers.emplace_back(handler);
    auto slicePreAggregation = std::make_shared<
        Runtime::Execution::Operators::NonKeyedSlicePreAggregation>(
        operatorHandlers.size() - 1, std::move(timeFunction),
    ...
    auto handler = std::make_shared<
        Runtime::Execution::Operators::NonKeyedBucketPreAggregationHandler>(
        timeBasedWindowType->getSize().getTime(),
        timeBasedWindowType->getSlide().getTime(),
        windowDefinition->getInputOriginIds());
    operatorHandlers.emplace_back(handler);
    auto bucketPreAggregation = std::make_shared<
        Runtime::Execution::Operators::NonKeyedBucketPreAggregation>(
        operatorHandlers.size() - 1, std::move(timeFunction),
        aggregationFunctions);
...
LowerPhysicalToNautilusOperators::lowerKeyedPreAggregationOperator(
  ...
  std::vector<Runtime::Execution::Expressions::ExpressionPtr>
      keyReadExpressions;
  ...
  if (options->getWindowingStrategy() == WindowingStrategy::SLICING) {
    ...
    operatorHandlers.emplace_back(handler);
    auto sliceMergingOperator = std::make_shared<
        Runtime::Execution::Operators::KeyedSlicePreAggregation>(
        operatorHandlers.size() - 1, std::move(timeFunction),
        keyReadExpressions, keyDataTypes, aggregationFunctions,
        std::make_unique<Nautilus::Interface::MurMur3HashFunction>());
    return sliceMergingOperator;
  } else if (options->getWindowingStrategy() == WindowingStrategy::BUCKETING) {

    auto handler = std::make_shared<
        Runtime::Execution::Operators::KeyedBucketPreAggregationHandler>(
        timeWindow->getSize().getTime(), timeWindow->getSlide().getTime(),
        windowDefinition->getInputOriginIds());

    operatorHandlers.emplace_back(handler);
    auto bucketPreAggregation = std::make_shared<
        Runtime::Execution::Operators::KeyedBucketPreAggregation>(
        operatorHandlers.size() - 1, std::move(timeFunction),
        keyReadExpressions, keyDataTypes, aggregationFunctions,
        std::make_unique<Nautilus::Interface::MurMur3HashFunction>());
    return bucketPreAggregation;
...
std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerWatermarkAssignmentOperator(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorPtr,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &) {
  auto wao =
      operatorPtr->as<PhysicalOperators::PhysicalWatermarkAssignmentOperator>();

  // Add either event time or ingestion time watermark strategy
  if (wao->getWatermarkStrategyDescriptor()
          ->instanceOf<Windowing::EventTimeWatermarkStrategyDescriptor>()) {
    auto eventTimeWatermarkStrategy =
        wao->getWatermarkStrategyDescriptor()
            ->as<Windowing::EventTimeWatermarkStrategyDescriptor>();
    auto fieldExpression = expressionProvider->lowerExpression(
        eventTimeWatermarkStrategy->getOnField());
    auto watermarkAssignmentOperator = std::make_shared<
        Runtime::Execution::Operators::EventTimeWatermarkAssignment>(
        std::make_unique<Runtime::Execution::Operators::EventTimeFunction>(
            fieldExpression, eventTimeWatermarkStrategy->getTimeUnit()));
    return watermarkAssignmentOperator;
  } else if (wao->getWatermarkStrategyDescriptor()
                 ->instanceOf<
                     Windowing::IngestionTimeWatermarkStrategyDescriptor>()) {
    auto watermarkAssignmentOperator = std::make_shared<
        Runtime::Execution::Operators::IngestionTimeWatermarkAssignment>(
        std::make_unique<
            Runtime::Execution::Operators::IngestionTimeFunction>());
    return watermarkAssignmentOperator;
  } else {
...
std::shared_ptr<Runtime::Execution::Operators::Operator>
LowerPhysicalToNautilusOperators::lowerScan(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorNode,
    size_t bufferSize) {
  auto schema = operatorNode->getOutputSchema();
  NES_ASSERT(schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT,
             "Currently only row layout is supported");
  // pass buffer size here
  auto layout =
      std::make_shared<Runtime::MemoryLayouts::RowLayout>(schema, bufferSize);
  std::unique_ptr<Runtime::Execution::MemoryProvider::MemoryProvider>
      memoryProvider = std::make_unique<
          Runtime::Execution::MemoryProvider::RowMemoryProvider>(layout);
  return std::make_shared<Runtime::Execution::Operators::Scan>(
      std::move(memoryProvider));
}

std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerEmit(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorNode,
    size_t bufferSize) {
  auto schema = operatorNode->getOutputSchema();
  ...
  std::unique_ptr<Runtime::Execution::MemoryProvider::MemoryProvider>
      memoryProvider = std::make_unique<
          Runtime::Execution::MemoryProvider::RowMemoryProvider>(layout);
  return std::make_shared<Runtime::Execution::Operators::Emit>(
      std::move(memoryProvider));
}

std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerFilter(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorPtr) {
  auto filterOperator =
      operatorPtr->as<PhysicalOperators::PhysicalFilterOperator>();
  auto expression =
      expressionProvider->lowerExpression(filterOperator->getPredicate());
  return std::make_shared<Runtime::Execution::Operators::Selection>(expression);
}

std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerLimit(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorPtr,
    std::vector<Runtime::Execution::OperatorHandlerPtr> &operatorHandlers) {
  auto limitOperator =
      operatorPtr->as<PhysicalOperators::PhysicalLimitOperator>();
  const auto handler =
      std::make_shared<Runtime::Execution::Operators::LimitOperatorHandler>(
          limitOperator->getLimit());
  operatorHandlers.push_back(handler);
  return std::make_shared<Runtime::Execution::Operators::Limit>(
      operatorHandlers.size() - 1);
}

std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerMap(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorPtr) {
  ...
  auto writeField =
      std::make_shared<Runtime::Execution::Expressions::WriteFieldExpression>(
          assignmentField->getFieldName(), expression);
  return std::make_shared<Runtime::Execution::Operators::Map>(writeField);
}

std::shared_ptr<Runtime::Execution::Operators::ExecutableOperator>
LowerPhysicalToNautilusOperators::lowerThresholdWindow(
    Runtime::Execution::PhysicalOperatorPipeline &,
    const PhysicalOperators::PhysicalOperatorPtr &operatorPtr,
    uint64_t handlerIndex) {
  NES_INFO("lowerThresholdWindow {} and handlerid {}", operatorPtr->toString(),
           handlerIndex);
  auto thresholdWindowOperator =
      operatorPtr->as<PhysicalOperators::PhysicalThresholdWindowOperator>();
  auto contentBasedWindowType = thresholdWindowOperator->getWindowDefinition()
                                    ->getWindowType()
                                    ->as<Windowing::ContentBasedWindowType>();
  auto thresholdWindowType =
      Windowing::ContentBasedWindowType::asThresholdWindow(
          contentBasedWindowType);
  NES_INFO("lowerThresholdWindow Predicate {}",
           thresholdWindowType->getPredicate()->toString());
  auto predicate =
      expressionProvider->lowerExpression(thresholdWindowType->getPredicate());
  auto minCount = thresholdWindowType->getMinimumCount();

  auto aggregations =
      thresholdWindowOperator->getWindowDefinition()->getWindowAggregation();
  auto aggregationFunctions = lowerAggregations(aggregations);
  std::vector<std::string> aggregationResultFieldNames;
  std::transform(
      aggregations.cbegin(), aggregations.cend(),
  ...
  return std::make_shared<
      Runtime::Execution::Operators::NonKeyedThresholdWindow>(
      predicate, aggregationResultFieldNames, minCount, aggregationFunctions,
      handlerIndex);
}

std::vector<
    std::shared_ptr<Runtime::Execution::Aggregation::AggregationFunction>>
LowerPhysicalToNautilusOperators::lowerAggregations(
    const std::vector<Windowing::WindowAggregationDescriptorPtr> &aggs) {
  NES_INFO("Lower Window Aggregations to Nautilus Operator");
  std::vector<
      std::shared_ptr<Runtime::Execution::Aggregation::AggregationFunction>>
      aggregationFunctions;
  std::transform(
      aggs.cbegin(), aggs.cend(), std::back_inserter(aggregationFunctions),
      [&](const Windowing::WindowAggregationDescriptorPtr &agg)
          -> std::shared_ptr<
              Runtime::Execution::Aggregation::AggregationFunction> {
        DefaultPhysicalTypeFactory physicalTypeFactory =
            DefaultPhysicalTypeFactory();

        // lower the data types
        auto physicalInputType =
            physicalTypeFactory.getPhysicalType(agg->getInputStamp());
        auto physicalFinalType =
            physicalTypeFactory.getPhysicalType(agg->getFinalAggregateStamp());

        auto aggregationInputExpression =
            expressionProvider->lowerExpression(agg->on());
        std::string aggregationResultFieldIdentifier;
        if (auto fieldAccessExpression =
                agg->as()->as_if<FieldAccessExpressionNode>()) {
          aggregationResultFieldIdentifier =
              fieldAccessExpression->getFieldName();
        } else {
          NES_THROW_RUNTIME_ERROR(
              "Currently complex expression in as fields are not supported");
        }
        switch (agg->getType()) {
        case Windowing::WindowAggregationDescriptor::Type::Avg:
          return std::make_shared<
              Runtime::Execution::Aggregation::AvgAggregationFunction>(
              physicalInputType, physicalFinalType, aggregationInputExpression,
              aggregationResultFieldIdentifier);
        case Windowing::WindowAggregationDescriptor::Type::Count:
          return std::make_shared<
              Runtime::Execution::Aggregation::CountAggregationFunction>(
              physicalInputType, physicalFinalType, aggregationInputExpression,
              aggregationResultFieldIdentifier);
        case Windowing::WindowAggregationDescriptor::Type::Max:
          return std::make_shared<
              Runtime::Execution::Aggregation::MaxAggregationFunction>(
              physicalInputType, physicalFinalType, aggregationInputExpression,
              aggregationResultFieldIdentifier);
        case Windowing::WindowAggregationDescriptor::Type::Min:
          return std::make_shared<
              Runtime::Execution::Aggregation::MinAggregationFunction>(
              physicalInputType, physicalFinalType, aggregationInputExpression,
              aggregationResultFieldIdentifier);
        ...
        case Windowing::WindowAggregationDescriptor::Type::Sum: {
          return std::make_shared<
              Runtime::Execution::Aggregation::SumAggregationFunction>(
              physicalInputType, physicalFinalType, aggregationInputExpression,
              aggregationResultFieldIdentifier);
        }
        };
...
LowerPhysicalToNautilusOperators::getAggregationValueForThresholdWindow(
  ...
  case Windowing::WindowAggregationDescriptor::Type::Avg:
    ...
    case BasicPhysicalType::NativeType::INT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<int8_t>>();
    case BasicPhysicalType::NativeType::INT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<int16_t>>();
    case BasicPhysicalType::NativeType::INT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<int32_t>>();
    case BasicPhysicalType::NativeType::INT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<int64_t>>();
    case BasicPhysicalType::NativeType::UINT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<uint8_t>>();
    case BasicPhysicalType::NativeType::UINT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<uint16_t>>();
    case BasicPhysicalType::NativeType::UINT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<uint32_t>>();
    case BasicPhysicalType::NativeType::UINT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<uint64_t>>();
    case BasicPhysicalType::NativeType::FLOAT:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<float_t>>();
    case BasicPhysicalType::NativeType::DOUBLE:
      return std::make_unique<
          Runtime::Execution::Aggregation::AvgAggregationValue<double_t>>();
    default:
  ...
  case Windowing::WindowAggregationDescriptor::Type::Count:
    ...
    case BasicPhysicalType::NativeType::INT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<int8_t>>();
    case BasicPhysicalType::NativeType::INT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<int16_t>>();
    case BasicPhysicalType::NativeType::INT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<int32_t>>();
    case BasicPhysicalType::NativeType::INT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<int64_t>>();
    case BasicPhysicalType::NativeType::UINT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<uint8_t>>();
    case BasicPhysicalType::NativeType::UINT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<uint16_t>>();
    case BasicPhysicalType::NativeType::UINT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<uint32_t>>();
    case BasicPhysicalType::NativeType::UINT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<uint64_t>>();
    case BasicPhysicalType::NativeType::FLOAT:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<float_t>>();
    case BasicPhysicalType::NativeType::DOUBLE:
      return std::make_unique<
          Runtime::Execution::Aggregation::CountAggregationValue<double_t>>();
    default:
      NES_THROW_RUNTIME_ERROR("Unsupported data type");
    }
  case Windowing::WindowAggregationDescriptor::Type::Max:
    ...
    case BasicPhysicalType::NativeType::INT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<int8_t>>();
    case BasicPhysicalType::NativeType::INT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<int16_t>>();
    case BasicPhysicalType::NativeType::INT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<int32_t>>();
    case BasicPhysicalType::NativeType::INT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<int64_t>>();
    case BasicPhysicalType::NativeType::UINT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<uint8_t>>();
    case BasicPhysicalType::NativeType::UINT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<uint16_t>>();
    case BasicPhysicalType::NativeType::UINT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<uint32_t>>();
    case BasicPhysicalType::NativeType::UINT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<uint64_t>>();
    case BasicPhysicalType::NativeType::FLOAT:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<float_t>>();
    case BasicPhysicalType::NativeType::DOUBLE:
      return std::make_unique<
          Runtime::Execution::Aggregation::MaxAggregationValue<double_t>>();
    default:
      NES_THROW_RUNTIME_ERROR("Unsupported data type");
    }
  case Windowing::WindowAggregationDescriptor::Type::Min:
    ...
    case BasicPhysicalType::NativeType::INT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<int8_t>>();
    case BasicPhysicalType::NativeType::INT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<int16_t>>();
    case BasicPhysicalType::NativeType::INT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<int32_t>>();
    case BasicPhysicalType::NativeType::INT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<int64_t>>();
    case BasicPhysicalType::NativeType::UINT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<uint8_t>>();
    case BasicPhysicalType::NativeType::UINT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<uint16_t>>();
    case BasicPhysicalType::NativeType::UINT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<uint32_t>>();
    case BasicPhysicalType::NativeType::UINT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<uint64_t>>();
    case BasicPhysicalType::NativeType::FLOAT:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<float_t>>();
    case BasicPhysicalType::NativeType::DOUBLE:
      return std::make_unique<
          Runtime::Execution::Aggregation::MinAggregationValue<double_t>>();
    default:
      NES_THROW_RUNTIME_ERROR("Unsupported data type");
    }
  case Windowing::WindowAggregationDescriptor::Type::Sum:
    ...
    case BasicPhysicalType::NativeType::INT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<int8_t>>();
    case BasicPhysicalType::NativeType::INT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<int16_t>>();
    case BasicPhysicalType::NativeType::INT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<int32_t>>();
    case BasicPhysicalType::NativeType::INT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<int64_t>>();
    case BasicPhysicalType::NativeType::UINT_8:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<uint8_t>>();
    case BasicPhysicalType::NativeType::UINT_16:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<uint16_t>>();
    case BasicPhysicalType::NativeType::UINT_32:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<uint32_t>>();
    case BasicPhysicalType::NativeType::UINT_64:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<uint64_t>>();
    case BasicPhysicalType::NativeType::FLOAT:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<float_t>>();
    case BasicPhysicalType::NativeType::DOUBLE:
      return std::make_unique<
          Runtime::Execution::Aggregation::SumAggregationValue<double_t>>();
    default:
      NES_THROW_RUNTIME_ERROR("Unsupported data type");
    }
  default:
