...
public:
  ...
  void run() {
    ...
    for (uint64_t i = 0; i < iterations; i++) {
      ...
      NES_INFO("Run {} compilation time {}, execution time {}", i,
               compileTimeTimer.getPrintTime(),
               executionTimeTimer.getPrintTime());
    }

    NES_INFO("Final {} compilation time {}, execution time {} ", compiler,
             (sumCompilation / (double)iterations),
             (sumExecution / (double)iterations));
  };
  virtual ~BenchmarkRunner() = default;
  virtual void runQuery(Timer<> &compileTimeTimer,
                        Timer<> &executionTimeTimer) = 0;
...
public:
  Query6Runner(TPCH_Scale_Factor targetScaleFactor, std::string compiler)
      : BenchmarkRunner(targetScaleFactor, compiler){};
  void runQuery(Timer<> &compileTimeTimer,
                Timer<> &executionTimeTimer) override {
...
public:
  Query1Runner(TPCH_Scale_Factor targetScaleFactor, std::string compiler)
      : BenchmarkRunner(targetScaleFactor, compiler){};
...
public:
  Query3Runner(TPCH_Scale_Factor targetScaleFactor, std::string compiler)
      : BenchmarkRunner(targetScaleFactor, compiler){};
  ...
  void runQuery(Timer<> &compileTimeTimer,
    ...
    compileTimeTimer.start();
    auto aggExecutablePipeline = provider->create(pipeline1.pipeline, options);
    auto orderCustomersJoinBuildPipeline =
        provider->create(pipeline2.pipeline, options);
    auto lineitems_ordersJoinBuildPipeline =
        provider->create(pipeline3.pipeline, options);
    ...
    auto numberOfKeys2 =
        joinHandler2->getThreadLocalState(wc->getId())->getNumberOfEntries();
    ...
    executionTimeTimer.pause();
    auto aggHandler =
        pipeline3.ctx->getOperatorHandler<BatchKeyedAggregationHandler>(1);
    aggExecutablePipeline->stop(*pipeline1.ctx);
