...
public:
    ...
    /// This method returns for each window all slices that can be triggered. It returns the slices for all windows that have been filled and have a window end smaller than the global watermark
    /// Additionally, it returns per window a sequence number that is incremented for each window and thus, it can be used to set it in the emitted tuple buffer for the probe operator.
