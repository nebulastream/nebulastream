...
struct EmittedAggregationWindow
{
    ...
    std::unique_ptr<Interface::HashMapInterface>
        finalHashMap; /// Pointer to the final hash map that the probe should use to combine all hash maps
    uint64_t numberOfHashMaps;
    Interface::HashMapInterface** hashMaps; /// Pointer to the stored pointers of all hash maps that the probe should combine
};
...
public:
    ...
    /// This might change after the [DD] Operator Representations #589 has been implemented
...
protected:
    ...
    uint64_t keySize{}, valueSize{}, pageSize{}, numberOfBuckets{};
};
