...
/**
 * @brief The AbstractSlicePreAggregationHandler provides an abstract operator
 * handler to perform slice-based pre-aggregation of time-based tumbling and
 * sliding windows. This operator handler, maintains a slice store for each
 * worker thread and provides them for the aggregation. For each processed tuple
 * buffer trigger is called, which checks if the thread-local slice store should
 * be triggered. This is decided by the current watermark timestamp.
 */
template <class SliceType, typename SliceStore>
class AbstractSlicePreAggregationHandler
...
public:
   ...
   * operator
   */
  AbstractSlicePreAggregationHandler(uint64_t windowSize, uint64_t windowSlide,
