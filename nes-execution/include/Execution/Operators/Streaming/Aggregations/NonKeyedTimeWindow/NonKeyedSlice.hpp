 ...
 * This is a wrapper around a small chuck of memory, which is dynamically
 * allocated.
 */
class State {
public:
   ...
   * As this represents a single aggregation value, it will result in a small
   * dynamic allocation.
   * @param stateSize
   */
...
/**
 * @brief A global slice that contains key value pairs for a specific interval
 * of [start, end). The aggregate value is stored in the State object.
 */
class NonKeyedSlice {
public:
  /**
   * @brief Constructor to create a new slice that covers a specific range
   * between stat and end.
   * @param entrySize entry size of the content of a slice
   * @param start of the slice
   * @param end of the slice
   * @param index of the slice (currently we assume that we can calculate a
   * slice index, to which a specific stream event is assigned).
   */
   ...
   * @return
   */
  [[nodiscard]] inline bool coversTs(uint64_t ts) const {
