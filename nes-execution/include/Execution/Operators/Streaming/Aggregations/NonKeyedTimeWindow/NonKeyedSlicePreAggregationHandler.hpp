...
class State;
/**
 * @brief The GlobalThreadLocalPreAggregationOperatorHandler provides an
 * operator handler to perform slice-based pre-aggregation of global non-keyed
 * tumbling and sliding windows. This operator handler, maintains a slice store
 * for each worker thread and provides them for the aggregation. For each
 * processed tuple buffer triggerThreadLocalState is called, which checks if the
 * thread-local slice store should be triggered. This is decided by the current
 * watermark timestamp.
 */
class NonKeyedSlicePreAggregationHandler
    : public AbstractSlicePreAggregationHandler<NonKeyedSlice,
                                                NonKeyedThreadLocalSliceStore> {
public:
  /**
   * @brief Creates the operator handler with a specific window definition, a
   * set of origins, and access to the slice staging object.
   * @param windowDefinition logical window definition
   * @param origins the set of origins, which can produce data for the window
   * operator
   */
  NonKeyedSlicePreAggregationHandler(uint64_t windowSize, uint64_t windowSlide,
