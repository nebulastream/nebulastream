 ...
 */
template <class SliceType> class BucketStore {
public:
  using SliceTypePtr = std::unique_ptr<SliceType>;
  explicit BucketStore(uint64_t windowSize, uint64_t windowSlide)
  ...
  std::vector<SliceType *> *findBucketsByTs(uint64_t ts) {
    ...
    const auto bucketStr =
        std::accumulate(localResultVector.begin(), localResultVector.end(),
                        std::string(), [&](const auto str, auto bucket) {
                          return str + std::to_string(bucket->getStart()) +
                                 ", " + std::to_string(bucket->getEnd()) + "\n";
                        });
  ...
  std::list<std::shared_ptr<SliceType>> extractBucketsUntilTs(uint64_t ts) {
    ...
    for (auto i = buckets.begin(), last = buckets.end();
         i != last && i->first + windowSize <= ts;) {
