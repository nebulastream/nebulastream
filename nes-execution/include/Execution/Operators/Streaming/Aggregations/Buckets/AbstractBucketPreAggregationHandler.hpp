...
class State;
/**
 * @brief The AbstractBucketPreAggregationHandler provides an operator handler
 * to perform bucket-based pre-aggregation of tumbling and sliding windows. This
 * operator handler, maintains a bucket store for each worker thread and
 * provides them for the aggregation. For each processed tuple buffer trigger is
 * called, which checks if the thread-local store should be triggered. This is
 * decided by the current watermark timestamp.
 */
template <class SliceType, typename SliceStore>
class AbstractBucketPreAggregationHandler
...
public:
   ...
   * @param windowSize
   * @param windowSlide
   * @param origins the set of origins, which can produce data for the window
   * operator
   */
  AbstractBucketPreAggregationHandler(uint64_t windowSize, uint64_t windowSlide,
                                      const std::vector<OriginId> &origins);

  SliceStore *getThreadLocalBucketStore(WorkerThreadId workerThreadId);
  void trigger(WorkerContext &wctx, PipelineExecutionContext &ctx,
               OriginId originId, SequenceData sequenceData,
  ...
  void stop(QueryTerminationType queryTerminationType,
