...
/**
 * @brief The slice store for sliding windows that maintains a set of slices for
 * all in-flight sliding windows.
 */
template <class SliceType> class SlidingWindowSliceStore {
public:
   ...
   */
  std::set<std::tuple<uint64_t, uint64_t>> collectWindows(uint64_t oldTs,
                                                          uint64_t currentTs);

  /**
   * @brief Collect all windows that are covered by any slices in the slice
   * store
   * @param oldTs
   * @return set of windows
   */
   ...
   */
  std::vector<SliceTypePtr> collectSlicesForWindow(uint64_t windowStart,
                                                   uint64_t windowEnd);

  /**
   * @brief Remove all slices from the slice store that are not required for any
   * windows anymore
   * @param ts
   */
