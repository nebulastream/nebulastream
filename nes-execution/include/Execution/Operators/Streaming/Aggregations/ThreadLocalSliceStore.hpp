 ...
 * thread, we dont have to protect its functions for concurrent accesses.
 */
template <class SliceType> class ThreadLocalSliceStore {
public:
  using SliceTypePtr = std::unique_ptr<SliceType>;
  explicit ThreadLocalSliceStore(uint64_t windowSize, uint64_t windowSlide)
      : windowAssigner(windowSize, windowSlide){};
  virtual ~ThreadLocalSliceStore() = default;

  /**
   * @brief Retrieves a slice which covers a specific ts.
   * If no suitable slice is contained in the slice store we add a slice at the
   * correct position. Slices cover a cover a range from [startTs, endTs - 1]
  ...
  SliceTypePtr &findSliceByTs(uint64_t ts) {
    if (ts < lastWatermarkTs) {
      throw WindowProcessingException(
          "The ts " + std::to_string(ts) +
          " can't be smaller then the lastWatermarkTs " +
    ...
    // 2. sliceIter is at a slide which slice.startTs < ts and slice.endTs <=
    // ts. In this case we have to insert a new slice after the current
    // sliceIter
    // 3. sliceIter is at a slide which slice.startTs <= ts and slice.endTs >
    // ts. In this case we found the correct slice.
    ...
    if (sliceIter == slices.rend()) {
      ...
      // We are in case 2. thus we have to append a new slice after the current
      // iterator
      auto newSliceStart = windowAssigner.getSliceStartTs(ts);
      auto newSliceEnd = windowAssigner.getSliceEndTs(ts);
      auto newSlice = allocateNewSlice(newSliceStart, newSliceEnd);
      readLock.unlock();
      auto slice = synchronizedSlices.wlock()->emplace(sliceIter.base(),
                                                       std::move(newSlice));
      return *slice;
  ...
  std::list<std::shared_ptr<SliceType>> extractSlicesUntilTs(uint64_t ts) {
    // drop all slices as long as the list is not empty and the first slice ends
    // before or at the current ts.
