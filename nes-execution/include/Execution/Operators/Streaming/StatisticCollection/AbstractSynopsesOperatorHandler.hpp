...
class AbstractSynopsesOperatorHandler : public OperatorHandler {
 public:
  AbstractSynopsesOperatorHandler(
  ...
  /**
   * @brief Returns the statistic for the statisticHash and the timestamp. If no
   * statistic exists, we create one by calling @see createInitStatistic
   * @param workerThreadId
   * @param statisticHash
   * @param timestamp
   * @return StatisticPtr
   */
  Statistic::StatisticPtr getStatistic(WorkerThreadId workerThreadId,
  ...
  /**
   * @brief Merges all statistics in statisticsPlusHashes. This is necessary, as
   * each thread creates their own count min sketches that have to be merged
   * together. We merge sketches that have the same statisticHash and time
   * period [startTs, endTs]
   * @param statisticsPlusHashes
   * @return Vector of merged statistics.
   */
  std::vector<Statistic::HashStatisticPair> mergeStatistics(
      const std::vector<Statistic::HashStatisticPair>& statisticsPlusHashes);

  /**
   * @brief Abstract method that each synopsis has to implement. It creates a
   * new statistic for the given time period.
   * @param sliceStart
   * @param sliceEnd
   * @return StatisticPtr
   */
  virtual Statistic::StatisticPtr createInitStatistic(
      Windowing::TimeMeasure sliceStart, Windowing::TimeMeasure sliceEnd) = 0;

 private:
  // For now, we simply use our existing statistic store, even though, we only
  // store one type of statistic. This might not be the most efficient, but this
  // way, we have the most flexibility moving forward.
