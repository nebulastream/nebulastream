 ...
 * of multiple buckets each consisting of a PagedVectorVarSized.
 */
...
public:
  ...
  explicit StreamJoinHashTableVarSized(size_t numPartitions,
                                       ...
                                       size_t pageSize, SchemaPtr &schema);

  StreamJoinHashTableVarSized(const StreamJoinHashTableVarSized &) = delete;

  StreamJoinHashTableVarSized &
  operator=(const StreamJoinHashTableVarSized &) = delete;

  virtual ~StreamJoinHashTableVarSized() = default;

  /**
   * @brief Inserts the key into this hash table by returning a pointer to the
   * appropriate underlying PagedVectorVarSized
   * @param key
   ...
   * @return bucket
   */
  Nautilus::Interface::PagedVectorVarSizedPtr
  getBucketPagedVector(size_t bucketPos) const;
