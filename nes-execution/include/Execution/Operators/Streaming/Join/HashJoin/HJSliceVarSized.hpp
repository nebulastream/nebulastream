...
/**
 * @brief This class is a data container for all the necessary objects in a
 * slice/window of the StreamJoin with variable sized data.
 */
...
public:
   ...
   * @param numPartitions
   */
  explicit HJSliceVarSized(size_t numberOfWorker, uint64_t sliceStart,
                           uint64_t sliceEnd, SchemaPtr &leftSchema,
                           SchemaPtr &rightSchema,
                           BufferManagerPtr &bufferManager, size_t pageSize,
  ...
  /**
   * @brief Returns the local hash table of either the left or the right join
   * side
   * @param joinBuildSide
  ...
  getHashTable(QueryCompilation::JoinBuildSideType joinBuildSide,
   ...
   */
  Operators::MergingHashTableVarSized &
  getMergingHashTable(QueryCompilation::JoinBuildSideType joinBuildSide);
   ...
   */
  uint64_t
  getNumberOfTuplesOfWorker(QueryCompilation::JoinBuildSideType joinBuildSide,
                            WorkerThreadId workerThreadId) const;

private:
  std::vector<std::unique_ptr<Operators::StreamJoinHashTableVarSized>>
      hashTableLeftSide;
  std::vector<std::unique_ptr<Operators::StreamJoinHashTableVarSized>>
      hashTableRightSide;
