...
using WLockedSlices =
    folly::Synchronized<std::list<StreamSlicePtr>>::WLockedPtr;
using RLockedSlices =
    folly::Synchronized<std::list<StreamSlicePtr>>::RLockedPtr;

/**
 * @brief This operator is the general join operator handler and implements the
 * JoinOperatorHandlerInterface. It is expected that all
 * StreamJoinOperatorHandlers inherit from this
 */
class StreamJoinOperatorHandler : public virtual OperatorHandler {
 public:
  ...
  void stop(QueryTerminationType terminationType,
            PipelineExecutionContextPtr pipelineExecutionContext) override;

  /**
   * @brief Retrieves the slice/window by a slice/window identifier. If no
   * slice/window exists for the windowIdentifier, the optional return value is
   * of nullopt.
   * @param sliceIdentifier
   * @return Optional
   */
  std::optional<StreamSlicePtr> getSliceBySliceIdentifier(
      uint64_t sliceIdentifier);
  std::optional<StreamSlicePtr> getSliceBySliceIdentifier(
      const RLockedSlices& slicesLocked, uint64_t sliceIdentifier);
  std::optional<StreamSlicePtr> getSliceBySliceIdentifier(
      const WLockedSlices& slicesLocked, uint64_t sliceIdentifier);

  /**
   * @brief Triggers all slices/windows that have not been already emitted to
   * the probe
   */
  ...
  /**
   * @brief Triggers windows that are ready. This method updates the
   * watermarkProcessor and should be thread-safe
   * @param bufferMetaData
   */
  void checkAndTriggerWindows(const BufferMetaData& bufferMetaData,
                              PipelineExecutionContext* pipelineCtx);

  /**
   * @brief Updates the corresponding watermark processor and then deletes all
   * slices/windows that are not valid anymore.
   * @param bufferMetaData
   */
  void deleteSlices(const BufferMetaData& bufferMetaData);

  /**
   * @brief Retrieves all window identifiers that correspond to this slice. For
   * bucketing, this will just return one item
   * @param slice
  ...
  /**
   * @brief Returns the number of tuples in the slice/window for the given
   * sliceIdentifier
   * @param sliceIdentifier
   * @param buildSide
   * @return number of tuples as uint64_t or -1 if no slice can be found for the
   * sliceIdentifier
   */
  uint64_t getNumberOfTuplesInSlice(
      uint64_t sliceIdentifier, QueryCompilation::JoinBuildSideType buildSide);
   ...
   */
  virtual StreamSlicePtr createNewSlice(uint64_t sliceStart,
                                        uint64_t sliceEnd) = 0;
  ...
  /**
   * @brief Returns the next sequence number for the operator that this operator
   ...
   */
  void updateWatermarkForWorker(uint64_t watermark,
