...
/**
 * @brief This class is the first phase of the join. For both streams (left and
 * right), the tuples are stored in the corresponding slice one after the other.
 * Afterwards, the second phase (NLJProbe) will start joining the tuples via two
 * nested loops.
 */
class NLJBuildSlicing : public StreamJoinBuild {
public:
  /**
   * @brief Local state, which stores the slice start, slice end,
   * joinOpHandlerReference, sliceReference, and pagedVectorRef
   */
  class LocalNestedLoopJoinState : public Operators::OperatorState {
  public:
    LocalNestedLoopJoinState(
        const Value<MemRef> &operatorHandler,
   ...
   */
  NLJBuildSlicing(const uint64_t operatorHandlerIndex, const SchemaPtr &schema,
  ...
  /**
   * @brief Updates the localJoinState by getting the values via
   * Nautilus::FunctionCalls()
   ...
   * @param timestamp: Timestamp, for which to get the sliceRef, sliceStart, and
   * sliceEnd
   */
  void
  updateLocalJoinState(LocalNestedLoopJoinState *localJoinState,
