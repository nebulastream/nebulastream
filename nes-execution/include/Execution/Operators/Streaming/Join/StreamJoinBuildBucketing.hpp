...
public:
  /**
   * @brief This class stores the no. windows to fill and the pointer to the
   * windows for a given range
   */
  class LocalStateBucketing : public OperatorState {
  public:
    explicit LocalStateBucketing(const Value<MemRef> &allWindowsToFill)
        : allWindowsToFill(allWindowsToFill), numWindowsToFill(0_u64),
          minWindowStart(0_u64), maxWindowEnd(0_u64) {}
  ...
  /**
   * @brief Updates the local hash join state, so that the correct windows are
   * stored for the timestamp (ts)
  ...
  void updateLocalState(LocalStateBucketing *localStateBucketing,
                        Value<MemRef> &opHandlerMemRef, Value<UInt64> &ts,
                        ValueId<WorkerThreadId> &workerThreadId) const;

  /**
   * @brief Checks if the current local hash join state is up-to-date, meaning
   * the correct windows are stored
   * @param ts
   * @param localStateBucketing
   * @return Boolean
   */
  Value<Boolean>
  checkIfLocalStateUpToDate(Value<UInt64> &ts,
