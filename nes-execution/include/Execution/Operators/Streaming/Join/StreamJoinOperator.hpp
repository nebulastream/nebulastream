...
class StreamJoinOperator {
 public:
  ...
  static OutputClass* getSpecificOperatorHandler(
      void* ptrOpHandler, uint64_t joinStrategyInt,
    ...
    switch (joinStrategy) {
      case QueryCompilation::StreamJoinStrategy::HASH_JOIN_VAR_SIZED:
        if (windowingStrategy ==
            QueryCompilation::WindowingStrategy::BUCKETING) {
          NES_THROW_RUNTIME_ERROR(
              "Windowing strategy was used that is not supported with this "
              "compiler!");
        }
