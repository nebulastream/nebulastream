 ...
 * Determines the watermark ts according to a WatermarkStrategyDescriptor an
 * places it in the current buffer.
 */
class EventTimeWatermarkAssignment : public ExecutableOperator {
 public:
  /**
   * @brief Creates a EventTimeWatermarkAssignment operator with a
   * watermarkExtractionExpression expression.
