...
/**
 * @brief The SliceAssigner assigner determines the start and end time stamp of
 * a slice for a specific window definition, that consists of a window size and
 * a window slide.
 * @note Tumbling windows are in general modeled at this point as sliding
 * windows with the size is equals to the slide.
 */
class SliceAssigner {
 public:
   ...
   * @param ts the timestamp for which we calculate the start of the particular
   * slice.
  ...
  [[nodiscard]] inline uint64_t getSliceStartTs(uint64_t ts) const {
    ...
    auto prevWindowStart = ts < windowSize
                               ? prevSlideStart
                               : ts - ((ts - windowSize) % windowSlide);
    return std::max(prevSlideStart, prevWindowStart);
  }

  /**
   * @brief Calculates the end of a slice for a specific timestamp ts.
   * @param ts the timestamp for which we calculate the end of the particular
   * slice.
  ...
  [[nodiscard]] inline uint64_t getSliceEndTs(uint64_t ts) const {
    ...
    auto nextWindowEnd =
        ts < windowSize ? windowSize
                        : ts + windowSlide - ((ts - windowSize) % windowSlide);
