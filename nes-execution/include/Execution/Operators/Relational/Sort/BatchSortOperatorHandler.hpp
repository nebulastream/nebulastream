...
/**
 * @brief Sort operator handler that manages the state of the BatchSort and
 * BatchSortScan operators. It stores the tuples in a PagedVector that are
 * sorted by the sort key when open() is called on BatchSortScan.
 */
class BatchSortOperatorHandler
    : public Runtime::Execution::OperatorHandler,
      public NES::detail::virtual_enable_shared_from_this<
          BatchSortOperatorHandler, false> {
public:
  /**
   ...
   */
  explicit BatchSortOperatorHandler(
    ...
    // Entry size is the sum of the sizes of the fields to sort, with will be
    // stored encoded and the complete record
    entrySize = 0;
    ...
    for (const auto &sortFieldIdentifier : sortFieldIdentifiers) {
      for (uint64_t j = 0; j < fieldIdentifiers.size(); ++j) {
        entrySize +=
            (sortFieldIdentifier == fieldIdentifiers[j]) * dataTypes[j]->size();
      }
    }
  ...
  void setup(Runtime::Execution::PipelineExecutionContext &) {
    ...
    stack = std::make_unique<Nautilus::Interface::PagedVector>(
        std::move(allocator), entrySize);
    auto tempAllocator = std::make_unique<NesDefaultMemoryAllocator>();
    tempStack = std::make_unique<Nautilus::Interface::PagedVector>(
        std::move(tempAllocator), entrySize);
  }
   ...
   * @return Stack state
   */
  Nautilus::Interface::PagedVector *getTempState() const {
