...
/**
 * @brief The batch join handler maintains the join state that is used by the
 * join build and the join probe operator. To this end, we use a two-phase
 * algorithm proposed by Leis et al. Morsel-Driven Parallelism: A NUMA-Aware
 * Query Evaluation Framework for the Many-Core Age
 * https://db.in.tum.de/~leis/papers/morsels.pdf
 *
 * The build phase, consumes input records from the build side and materializes
 * hash-entries in a thread local paged vector. If all records, are consumed, we
 * build a global hash-map on top of all materialized values (see mergeState).
 * The final probe phase, consumes the probe side and performs key lookups in
 * the global hash-map.
 * TODO: We could further, improve this operator by adding support for
 * concurrent merges of the local paged vectors to the hash table. This code
 * improve performance, but it should first investigated if the merging becomes
 * a performance bottleneck.
 */
class BatchJoinHandler
    : public Runtime::Execution::OperatorHandler,
      public NES::detail::virtual_enable_shared_from_this<BatchJoinHandler,
                                                          false> {
 public:
  /**
  ...
  void setup(Runtime::Execution::PipelineExecutionContext& ctx,
             uint64_t entrySize, uint64_t keySize, uint64_t valueSize);
   ...
   */
  Nautilus::Interface::PagedVector* getThreadLocalState(
      WorkerThreadId workerThreadId);

  /**
   * @brief This function creates the global hash map. To this end, it builds a
   * new hash map based on the thread local paged vectors.
 ...
 private:
  std::vector<std::unique_ptr<Nautilus::Interface::PagedVector>>
      threadLocalStateStores;
