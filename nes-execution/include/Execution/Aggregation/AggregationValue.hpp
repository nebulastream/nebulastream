 ...
 * the lower function
 */
template <typename T> struct AvgAggregationValue : AggregationValue {
  uint64_t count = 0;
  T sum = 0;
};
 ...
 */
template <typename T> struct SumAggregationValue : AggregationValue {
  T sum = 0;
};
 ...
 */
template <typename T> struct CountAggregationValue : AggregationValue {
  T count = 0;
};
 ...
 * tuples
 */
template <typename T> struct MinAggregationValue : AggregationValue {
 ...
 * tuples
 */
template <typename T> struct MaxAggregationValue : AggregationValue {
