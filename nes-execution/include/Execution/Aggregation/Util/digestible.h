#ifndef NES_EXECUTION_INCLUDE_EXECUTION_AGGREGATION_UTIL_DIGESTIBLE_H_
#define NES_EXECUTION_INCLUDE_EXECUTION_AGGREGATION_UTIL_DIGESTIBLE_H_
...
/**
 * @brief Tdigest quantile estimator
 * @link: @link: https://github.com/SpirentOrion/digestible
 */
...
// XXX: yes, this could be a std::pair. But being able to refer to values by
// name instead of .first and .second makes merge(), quantile(), and
// cumulative_distribution() way more readable.
template <typename Values = float, typename Weight = unsigned> struct centroid {
  Values mean;
  Weight weight;
...
template <typename Values = float, typename Weight = unsigned> class tdigest {
  ...
  struct tdigest_impl {
    ...
    explicit tdigest_impl(size_t size) : total_weight(0) {
      values.reserve(size);
    }
...
public:
  ...
  /**
   * Inserts the given value with the given weight into the t-digest input
   * buffer. If this operation fills the input buffer an automatic merge()
   * operation is triggered.
   * @param value to insert
   * @param weight associated with the provided value
   */
   ...
   * @param src source t-digest to copy values from
   */
   ...
   * equal to that value.
   * @param p percentile of interest; valid input between [0.0, 100.0]
   * @return a value from the dataset that satisfies the above condition
   */
   ...
   * @return the total weight of all data
   */
   ...
   * @return maximum value seen by this t-digest.
   */
   ...
   * Value is cleared by reset().
   * @return minimum value seen by this t-digest.
   */
...
tdigest<Values, Weight>::tdigest(size_t size)
    : one(size), two(size), buffer(size * buffer_multiplier), active(&one),
      min_val(std::numeric_limits<Values>::max()),
      max_val(std::numeric_limits<Values>::lowest()), run_forward(true) {}

template <typename Values, typename Weight>
tdigest<Values, Weight>::tdigest(const tdigest<Values, Weight> &other)
    : one(other.one), two(other.two), buffer(other.buffer),
      active(other.active == &other.one ? &one : &two), min_val(other.min_val),
      max_val(other.max_val), run_forward(other.run_forward) {}
...
template <typename Values, typename Weight>
tdigest<Values, Weight> &
tdigest<Values, Weight>::operator=(tdigest<Values, Weight> other) {
  swap(other);
  return (*this);
}
...
void tdigest<Values, Weight>::insert(const tdigest<Values, Weight> &src) {
  ...
  std::for_each(src.active->values.begin(), src.active->values.end(),
                insert_fn);
...
std::vector<std::pair<Values, Weight>> tdigest<Values, Weight>::get() const {
  ...
  std::transform(active->values.begin(), active->values.end(),
                 std::back_inserter(to_return), [](const centroid_t &val) {
...
/**
 * Based on the equivalent function in the reference implementation available
 * here: https://github.com/tdunning/t-digest
 */
template <typename Values, typename Weight>
void tdigest<Values, Weight>::merge() {
  ...
  if (run_forward) {
    ...
    // Update min/max values only if sorted first/last centroids are single
    // points.
    min_val = std::min(min_val, inputs.front().weight == 1
                                    ? inputs.front().mean
                                    : std::numeric_limits<Values>::max());

    max_val = std::max(max_val, inputs.back().weight == 1
                                    ? inputs.back().mean
    ...
    // Update min/max values only if sorted first/last centroids are single
    // points.
    min_val = std::min(min_val, inputs.back().weight == 1
                                    ? inputs.back().mean
                                    : std::numeric_limits<Values>::max());

    max_val = std::max(max_val, inputs.front().weight == 1
                                    ? inputs.front().mean
                                    : std::numeric_limits<Values>::min());
  }
  ...
  const double normalizer =
      normalizer_fn(inactive.values.capacity(), new_total_weight);
  ...
  auto compress_fn = [&inactive, new_total_weight, &k1, normalizer,
                      &next_q_limit_weight, &weight_so_far, &weight_to_add,
                      &mean_to_add](const centroid_t &current) {
    if ((weight_so_far + weight_to_add + current.weight) <=
        next_q_limit_weight) {
      weight_to_add += current.weight;
      assert(weight_to_add);
      mean_to_add = mean_to_add + (current.mean - mean_to_add) *
                                      current.weight / weight_to_add;

    } else {
      weight_so_far += weight_to_add;

      double new_q = static_cast<double>(weight_so_far) /
                     static_cast<double>(new_total_weight);
...
double tdigest<Values, Weight>::quantile(double p) const {
  ...
  if (first.weight > 1 && index < (first.weight / 2)) {
    return (lerp(min_val, first.mean,
                 static_cast<double>(index - 1) / (first.weight / 2 - 1)));
  }
  ...
  if (last.weight > 1 && active->total_weight - index <= last.weight / 2) {
    return (max_val - static_cast<double>(active->total_weight - index - 1) /
                          (last.weight / 2 - 1) * (max_val - last.mean));
  }
  ...
  // Even though we're using adjacent_find here, we don't actually intend to
  // find anything.  We just want to iterate over pairs of centroids until we
  // calculate the quantile.
  auto it = std::adjacent_find(active->values.begin(), active->values.end(),
                               quantile_fn);
