...
/**
 * @brief The RecordBuffer is a representation of a set of records that are
 * stored together. In the common case this maps to a TupleBuffer, which stores
 * the individual records in either a row or a columnar layout.
 */
class RecordBuffer {
public:
  /**
  ...
  /**
   * @brief Read number of record that are currently stored in the record
   * buffer.
   * @return Value<UInt64>
   */
  ...
  /**
   * @brief Retrieve the reference to the underling buffer from the record
   * buffer.
   * @return Value<MemRef>
   */
   ...
   * The sequence number is a monotonically increasing identifier for tuple
   * buffers from the same origin.
   ...
   *
   * @return Value<UInt64> containing the watermark timestamp of the tuple
   * buffer.
   */
   ...
   * created.
   *
   * @return Value<UInt64> containing the creation timestamp of the tuple
   * buffer.
   */
