...
/**
 * @brief This is an adopt to the legacy pipeline stage to the nautilus
 * operators.
 * TODO After finishing the migration the nautilus based we can replace the
 * interface.
 */
class NautilusExecutablePipelineStage : public ExecutablePipelineStage {
 public:
  NautilusExecutablePipelineStage(
  ...
  uint32_t open(PipelineExecutionContext& pipelineExecutionContext,
                WorkerContext& workerContext) override;
  ExecutionResult execute(TupleBuffer& inputTupleBuffer,
                          PipelineExecutionContext& pipelineExecutionContext,
                          WorkerContext& workerContext) override;
  uint32_t close(PipelineExecutionContext& pipelineExecutionContext,
                 WorkerContext& workerContext) override;
  uint32_t stop(PipelineExecutionContext& pipelineExecutionContext) override;
  /**
   * @deprecated This method will be removed as it is not applicable to the new
   * compilation backend.
   */
