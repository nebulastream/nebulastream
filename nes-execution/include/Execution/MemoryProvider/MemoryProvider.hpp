...
public:
  ...
  /**
   * @brief Helper function for creating a memory provider from the buffer size
   * and the schema
   * @param bufferSize
   * @param schema
   * @return MemoryProvider
   */
  static MemoryProviderPtr createMemoryProvider(const uint64_t bufferSize,
   ...
   * @param projections: Defines which fields of the record are accessed. Empty
   * projections means all fields.
   * @param bufferAddress: Address of the memory buffer that contains the
   * record.
   * @param recordIndex: Index of the specific value that is accessed by 'read'.
   * @return Nautilus::Record: A Nautilus record constructed using the given
   * projections.
   */
  virtual Nautilus::Record
  read(const std::vector<Nautilus::Record::RecordFieldIdentifier> &projections,
       Nautilus::Value<Nautilus::MemRef> &bufferAddress,
       Nautilus::Value<Nautilus::UInt64> &recordIndex) const = 0;

  /**
   * @brief Write record.
   * @param recordIndex: Index of the specific value that is accessed by
   * 'write'.
   * @param bufferAddress: Address of the memory buffer that contains the
   * record.
  ...
  /**
   * @brief load a scalar value of type 'type' from memory using a memory
   * reference 'memRef'
   * @param type Type of the value that is loaded from memory.
   * @param bufferReference Memory reference to buffer that is accessed.
   * @param fieldReference Memory reference to the specific field that should be
   * loaded.
   * @return Nautilus::Value<> Loaded value casted to correct Nautilus Value of
   * type 'type'.
   */
  Nautilus::Value<>
  load(const PhysicalTypePtr &type,
       Nautilus::Value<Nautilus::MemRef> &bufferReference,
       Nautilus::Value<Nautilus::MemRef> &fieldReference) const;
  /**
   * @brief store a scalar value of type 'type' from memory using a memory
   * reference 'memRef'
   * @param type Type of the value that is loaded from memory.
   * @param bufferReference Memory reference to buffer that is accessed.
   * @param fieldReference Memory reference to the specific field that should be
  ...
  /**
   * @brief Checks if given RecordFieldIdentifier projections contains the given
   * field index.
   * @param projections Projections for record fields.
   * @param fieldIndex Field index that is possibly accessed.
   * @return true if no projections (entire record accessed) or if field is in
   * projections, else return false.
   */
