...
/**
 * @brief The dump context is used to dump a node graph to multiple dump handler
 * at the same time. To this end, it manages the local registered dump handlers.
 */
class DumpContext {
 public:
  ...
  /**
   * @brief Dumps the passed node and its children on all registered dump
   * handlers.
   * @param node
   */
   ...
   */
  void dump(const std::string& scope,
            const DecomposedQueryPlanPtr& decomposedQueryPlan);

  /**
   * @brief Dumps the passed pipeline query plan on all registered dump
   * handlers.
   * @param defines the scope of this plan.
