...
/**
 * @brief Stores a window operator and window definition, as well as in- and
 * output schema
 */
struct WindowOperatorProperties {
  WindowOperatorProperties(
      WindowOperatorPtr windowOperator, SchemaPtr windowInputSchema,
...
struct StreamJoinOperators {
  StreamJoinOperators(const LogicalOperatorPtr &operatorNode,
      ...
      : operatorNode(operatorNode), leftInputOperator(leftInputOperator),
        rightInputOperator(rightInputOperator) {}
...
/**
 * @brief Stores all join configuration, e.g., window size, timestamp field
 * name, join strategy, ...
 */
struct StreamJoinConfigs {
  StreamJoinConfigs(const std::string &joinFieldNameLeft,
      ...
      : joinFieldNameLeft(joinFieldNameLeft),
        joinFieldNameRight(joinFieldNameRight), windowSize(windowSize),
        windowSlide(windowSlide), timeStampFieldLeft(timeStampFieldLeft),
        timeStampFieldRight(timeStampFieldRight), joinStrategy(joinStrategy) {}
...
/**
 * @brief Provides a set of default lowerings for logical operators to
...
public:
  ...
  static PhysicalOperatorProviderPtr
  create(const QueryCompilerOptionsPtr &options);
  void lower(DecomposedQueryPlanPtr decomposedQueryPlan,
             LogicalOperatorPtr operatorNode) override;
...
protected:
  ...
  /**
   * @brief Checks if the current operator is a demultiplexer, if it has
   * multiple parents.
   * @param operatorNode
   * @return
   */
  ...
  /**
   * @brief Lowers a union operator. However, A Union operator is not realized
   * via executable code. It is realized by using a Multiplex operation that
   * connects two sources with one sink. The two sources then form one stream
   *        that continuously sends TupleBuffers to the sink. This means a query
   * that only contains an Union operator does not lead to code that is compiled
   * and is entirely executed on the source/sink/TupleBuffer level.
   * @param operatorNode current operator
   */
   ...
   */
  void lowerStatisticBuildOperator(Statistic::LogicalStatisticWindowOperator
   ...
   */
  OperatorPtr
  getJoinBuildInputOperator(const LogicalJoinOperatorPtr &joinOperator,
                            SchemaPtr schema,
                            std::vector<OperatorPtr> children);

private:
  /**
   * @brief replaces the window sink (and inserts a SliceStoreAppendOperator)
   * depending on the time based window type for keyed windows
   * @param windowOperatorProperties
   * @param operatorNode
   */
  std::shared_ptr<Node> replaceOperatorTimeBasedWindow(
  ...
  lowerStreamingHashJoin(const StreamJoinOperators &streamJoinOperators,
