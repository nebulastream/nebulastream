 ...
 * which splits query plans in pipelines of operators according to specific
 * operator fusion policy.
 */
...
public:
   ...
   * @param operatorFusionPolicy Policy to determine if an operator can be
   * fused.
   * @return PipeliningPhasePtr
   */
  static PipeliningPhasePtr
  create(const OperatorFusionPolicyPtr &operatorFusionPolicy);
  explicit DefaultPipeliningPhase(OperatorFusionPolicyPtr operatorFusionPolicy);
  PipelineQueryPlanPtr
  apply(DecomposedQueryPlanPtr decomposedQueryPlan) override;
...
protected:
  ...
  void
  processSink(const PipelineQueryPlanPtr &pipelinePlan,
              std::map<OperatorPtr, OperatorPipelinePtr> &pipelineOperatorMap,
              const OperatorPipelinePtr &currentPipeline,
              const PhysicalOperators::PhysicalOperatorPtr &currentOperator);
  static void
  processSource(const PipelineQueryPlanPtr &pipelinePlan,
                std::map<OperatorPtr, OperatorPipelinePtr> &pipelineOperatorMap,
