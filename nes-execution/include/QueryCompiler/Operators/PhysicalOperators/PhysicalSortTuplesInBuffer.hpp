...
public:
    PhysicalSortTuplesInBuffer(
        OperatorId id, std::shared_ptr<Schema> inputSchema, std::shared_ptr<Schema> outputSchema, std::string sort_field)
        : Operator(id)
...
protected:
    ...
    {
        return os << fmt::format("PhysicalSortTuplesInBuffer({})", sortField);
    }

public:
    std::shared_ptr<Runtime::Execution::Interface::MemoryProvider::TupleBufferMemoryProvider>
    getMemoryProviderInput(uint64_t bufferSize) const
    {
