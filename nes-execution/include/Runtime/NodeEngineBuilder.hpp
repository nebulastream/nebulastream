...
namespace Runtime {
/**
 * This class is used to create instances of NodeEngine using the builder
 * pattern.
 */
...
public:
   ...
   * @param workerConfiguration contains values that configure aspects of the
   * NodeEngine
   * @return NodeEngineBuilder
   */
  static NodeEngineBuilder
  create(Configurations::WorkerConfigurationPtr workerConfiguration);
   ...
   */
  NodeEngineBuilder &
  setQueryStatusListener(AbstractQueryStatusListenerPtr nesWorker);
   ...
   */
  NodeEngineBuilder &
  setPartitionManager(Network::PartitionManagerPtr partitionManager);
  ...
  /**
   * setter used to pass a vector of buffer managers to NodeEngineBuilder.
   * Optional
   * @param std::vector<BufferManagerPtr>
   * @return NodeEngineBuilder&
   */
  NodeEngineBuilder &
  setBufferManagers(std::vector<BufferManagerPtr> bufferManagers);
   ...
   * @return NodeEngineBuilder&
   */
  NodeEngineBuilder &setLanguageCompiler(
      std::shared_ptr<Compiler::LanguageCompiler> languageCompiler);

  /**
   * setter used to pass a language a jit compiler to NodeEngineBuilder.
   * Optional
   * @param jitCompiler
   * @return NodeEngineBuilder&
   */
  ...
  /**
   * setter used to pass a language a phase factory to NodeEngineBuilder.
   * Optional
   * @param phaseFactory
   * @return NodeEngineBuilder&
   */
  NodeEngineBuilder &
  setPhaseFactory(QueryCompilation::Phases::PhaseFactoryPtr phaseFactory);
...
private:
  ...
  /**
   *  Used during build() to convert the QueryCompilerConfigurations in the
   * WorkerConfigruations to QueryCompilationOptions, which is then used to
  ...
  createQueryCompilationOptions(const Configurations::QueryCompilerConfiguration
