...
class WorkerConfig {
  ...
  private:
    ConfigOption<std::string> localWorkerIp =
        ConfigOption("localWorkerIp", std::string("127.0.0.1"), "Worker IP.", "string", ConfigType::DEFAULT, false);
    ConfigOption<std::string> coordinatorIp =
        ConfigOption("coordinatorIp", std::string("127.0.0.1"), "Server IP of the NES Coordinator to which the NES Worker should connect.",
                     "string", ConfigType::DEFAULT, false);
    ConfigOption<uint16_t> coordinatorPort =
        ConfigOption("coordinatorPort", uint16_t(0),
                     "RPC server Port of the NES Coordinator to which the NES Worker should connect. Needs to be set and needs to be the same as restPort in Coordinator.",
                     "uint16_t", ConfigType::DEFAULT, false);
    ConfigOption<uint16_t> rpcPort =
        ConfigOption("rpcPort", uint16_t(8081),
                     "RPC server port of the NES Worker.",
                     "uint16_t", ConfigType::DEFAULT, false);
    ConfigOption<uint16_t> dataPort =
        ConfigOption("dataPort", uint16_t(3001),
                     "Data port of the NES Worker.",
                     "uint16_t", ConfigType::DEFAULT, false);
    ...
    ConfigOption<uint16_t> numWorkerThreads =
        ConfigOption("numWorkerThreads", uint16_t(1),
                     "Number of worker threads.",
                     "uint16_t", ConfigType::DEFAULT, false);
    ConfigOption<std::string> parentId =
        ConfigOption("parentId", std::string("-1"),
