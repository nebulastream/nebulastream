...
use crate::query::query_fragment::{DesiredQueryFragmentState, StopMode};
...
/// Records the intent to stop the matched queries. It sets each of their
/// fragments' desired state to Stopped and stamps the stop mode: a graceful
/// stop flips only source fragments, letting downstream drain to completion,
/// while a forceful stop flips every fragment. The actual stop happens
/// asynchronously as the controller reconciles.
///
...
impl Execute for DropQuery {
    ...
    async fn execute(&self, conn: &impl ConnectionTrait) -> Result<Vec<query::Model>> {
        ...
        query_fragment::Entity::update_many()
            .col_expr(
                query_fragment::Column::StopMode,
                Expr::value(self.stop_mode),
            )
            .filter(query_fragment::Column::QueryId.is_in(query_ids.clone()))
            .exec(conn)
            .await?;
        // Graceful: only flip source fragments to Stopped, so downstream
        // fragments drain naturally to Completed. Forceful: flip every
        // fragment so the pipeline stops mid-flight.
        let mut update = query_fragment::Entity::update_many()
            .col_expr(
        ...
        if self.stop_mode == StopMode::Graceful {
            update = update.filter(query_fragment::Column::HasSource.eq(true));
        }
