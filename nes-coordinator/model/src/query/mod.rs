...
use crate::sink::{self, CreateInlineSink, CreateSink};
use crate::source::physical::{self, CreateInlineSource, CreatePhysicalSource};
use anyhow::Result;
...
impl Execute for SourceFactory {
    ...
    async fn execute(&self, conn: &impl ConnectionTrait) -> Result<physical::Model> {
        match self {
            ...
            Self::Inline(source) => source.execute(conn).await,
        }
    }
}
...
impl Execute for SinkFactory {
    ...
    async fn execute(&self, conn: &impl ConnectionTrait) -> Result<sink::Model> {
        match self {
            ...
            Self::Inline(sink) => sink.execute(conn).await,
        }
    }
}
...
pub(crate) const INVALID_TRANSITIONS: &[(
    ...
    (
        query_fragment::QueryFragmentState::Registered,
        query_fragment::QueryFragmentState::Completed,
    ),
    (
        query_fragment::QueryFragmentState::Running,
...
mod tests {
    ...
    use crate::query::query_fragment::{
        ...
        QueryFragmentTransition, StopMode,
    };
    ...
    async fn drop_sets_fragment_stop_mode(mut req: CreateQueryWithRefs, stop_mode: StopMode) {
        ...
        for f in &fragments {
            ...
            if stop_mode == StopMode::Forceful || f.has_source {
                assert_eq!(f.desired_state, DesiredQueryFragmentState::Stopped);
            }
        }
    }
    ...
    async fn query_state_derived_from_fragments(mut req: CreateQueryWithRefs) {
        ...
        let steps: &[(QueryFragmentState, QueryState)] = &[
            ...
            (QueryFragmentState::Registered, QueryState::Registered),
