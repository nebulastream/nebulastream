...
impl GetQuery {
    ...
    pub fn wait_for_poll(mut self, secs: u64) -> Self {
        self.wait_for_poll = Some(secs);
        self
    }
}
...
impl IntoCondition for GetQuery {
    fn to_condition(&self) -> Condition {
        Condition::all()
            .add_option(self.ids.clone().map(|ids| Column::Id.is_in(ids)))
