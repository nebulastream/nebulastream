...
pub use create::{CreateAnonymousSource, CreatePhysicalSource};
...
mod tests {
    ...
    use crate::source::physical::{
        AnonymousSourceWithRefs, CreatePhysicalSource, DropPhysicalSource, GetPhysicalSource,
    ...
    async fn if_not_exists_returns_existing(req: PhysicalSourceWithRefs) {
        ...
        // Same full definition (including config) is an idempotent no-op.
    ...
    #[proptest(async = "tokio")]
    async fn if_not_exists_creates_new_config(req: PhysicalSourceWithRefs) {
        let db = Database::for_test().await;
        req.logical.execute(&db).await.unwrap();
        req.worker.execute(&db).await.unwrap();
        let original = req.physical.execute(&db).await.unwrap();
    ...
    async fn if_not_exists_creates_new_source_type(req: PhysicalSourceWithRefs) {
        ...
        let new_source_type = format!("{}X", req.physical.source_type);
        let retry = CreatePhysicalSource {
            logical_source: req.physical.logical_source.clone(),
            host_addr: req.physical.host_addr.clone(),
            source_type: new_source_type.clone(),
    ...
    async fn drop_blocked_by_active_query(mut req: crate::query::CreateQueryWithRefs) {
        ...
        let source_id = *req.query.source_ids.first().unwrap();
...
#[cfg(test)]
#[derive(Debug, Clone)]
pub(crate) struct AnonymousSourceWithRefs {
    pub(crate) worker: CreateWorker,
    pub(crate) physical: CreateAnonymousSource,
}
...
#[cfg(test)]
impl Arbitrary for AnonymousSourceWithRefs {
    type Parameters = ();
    ...
    fn arbitrary_with(_: Self::Parameters) -> Self::Strategy {
        ...
        (any::<CreateWorker>(), "[A-Za-z]{1,10}", any::<bool>())
            .prop_map(|(worker, source_type, internal)| {
                let physical = CreateAnonymousSource {
                    source_type,
                    source_config: Default::default(),
                    parser_config: Default::default(),
                    host_addr: worker.host_addr.clone(),
