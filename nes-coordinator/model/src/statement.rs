...
use crate::worker;
use crate::worker::{
    CreateWorker, DropWorker, GetWorker, GetWorkerStatus, GetWorkerVersion, WorkerVersion,
};
use anyhow::Result;
...
pub enum Statement {
    ...
    GetWorkerVersion(GetWorkerVersion),
    DropWorker(DropWorker),
    CreateQuery(CreateQuery),
    ExplainQuery { explanation: String },
    GetQuery(GetQuery),
    DropQuery(DropQuery),
...
pub enum StatementResult {
    ...
    CreatedMlModel(ml_model::Model),
    DroppedMlModels(Vec<ml_model::Model>),
    MlModels(Vec<ml_model::Model>),
}
...
impl Statement {
    pub async fn execute_on(self, conn: &impl ConnectionTrait) -> Result<StatementResult> {
        match self {
            ...
            Statement::CreateMlModel(req) => {
                Ok(StatementResult::CreatedMlModel(req.execute(conn).await?))
            }
            Statement::GetMlModel(req) => Ok(StatementResult::MlModels(req.execute(conn).await?)),
            Statement::DropMlModel(req) => {
                Ok(StatementResult::DroppedMlModels(req.execute(conn).await?))
            }
        }
    }
