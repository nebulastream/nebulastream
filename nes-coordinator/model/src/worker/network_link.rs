...
/// reference `worker.host_addr`, but no foreign key is enforced: doing
/// so would force clients to insert workers in a specific order before
/// declaring links between them, which we want to avoid. Dangling links
/// are tolerated and resolved later during query planning.
