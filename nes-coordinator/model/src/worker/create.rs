...
use crate::worker::{network_link, ActiveModel, Entity as WorkerEntity};
use anyhow::Result;
...
impl Execute for CreateWorker {
    ...
    async fn execute(self, conn: &impl ConnectionTrait) -> Result<Model> {
        ...
        let worker = ActiveModel::from(self).insert(conn).await?;
        if !peers.is_empty() {
...
impl CreateWorker {
    pub(crate) fn topology_no_edges(min_nodes: u8, max_nodes: u8) -> BoxedStrategy<Vec<CreateWorker>> {
        use proptest::prelude::*;
