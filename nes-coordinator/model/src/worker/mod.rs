...
pub struct Model {
    ...
    // Remaining capacity (decremented by fragment placements). `None`
    // means unbounded; capacity arithmetic is skipped for such workers.
...
/// Observed worker-state change. The initial worker state has no
/// transition variant: workers start in it at insert time and never
/// return to it.
...
mod tests {
    ...
    use crate::query::{
        self, CreateQueryWithRefs, fragment, fragment::FragmentState, setup, walk_all,
    };
    ...
    async fn drop_cascades_active_fragments_to_failed(mut req: CreateQueryWithRefs) {
        ...
        let after = fragment::Entity::find()
            .filter(fragment::Column::HostAddr.eq(host.clone()))
            .all(db.connection())
            .await
            .unwrap();
    ...
    async fn drop_preserves_terminal_fragments(mut req: CreateQueryWithRefs) {
        ...
        let after = fragment::Entity::find()
            .filter(fragment::Column::HostAddr.eq(host))
            .all(db.connection())
            .await
            .unwrap();
        assert!(!after.is_empty());
