...
impl<R> ParkedReply<R> {
    fn outcome(&self, observation: &Observation) -> Option<anyhow::Result<StatementResult>> {
        ...
        if !satisfied {
            return expired.then(|| {
                Err(anyhow::Error::new(CodedError::new(
                    ErrorCode::QueryWaitTimeout,
                    WaitTimeout,
                )))
            });
        }
        Some(match self.until {
            Until::Reaches(target) => {
                let created = rows().next()?.clone();
            ...
            Until::Dropped => Ok(StatementResult::DroppedQueries(
                rows().map(|row| row.query.clone()).collect(),
            )),
            Until::Terminated | Until::Converged(_) => {
                Ok(StatementResult::Queries(rows().cloned().collect()))
            }
        })
    }
}
...
mod tests {
    ...
    fn a_missing_row_stays_parked_until_the_deadline() {
        ...
        assert!(matches!(released.as_slice(), [("reply", Err(e))] if is_timeout(e)));
        assert!(parked.is_empty());
    }
