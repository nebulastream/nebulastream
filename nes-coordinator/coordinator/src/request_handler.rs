...
impl RequestHandler {
    ...
    async fn handle(&mut self, req: Request) {
        ...
        let poll = match (&input, poll_for) {
            (StatementInput::Parsed(Statement::GetQuery(q)), Some(secs)) => Some((q.clone(), secs)),
            _ => None,
        };
        ...
        match result {
            ...
            Ok(StatementResult::Queries(queries)) if block_until_all_terminal => {
    ...
    async fn resolve_pending_queries(&mut self) {
        ...
        let queries: HashMap<QueryId, (query::Model, Vec<query::query_fragment::Model>)> =
            query_list
                .into_iter()
...
/// cannot arrive), or it has settled into a running state and been observed
...
mod tests {
    ...
    impl TestHandle {
        ...
        fn ask_blocking(
            ...
            self.ask_with(Payload::parsed(statement).block_until(block_until))
        }
    ...
    async fn transition_fragments(db: &Database, target: QueryFragmentState) {
        let steps = match target {
            QueryFragmentState::Registered => vec![QueryFragmentState::Registered],
            QueryFragmentState::Running => {
                vec![QueryFragmentState::Registered, QueryFragmentState::Running]
            }
            QueryFragmentState::Completed => vec![
                QueryFragmentState::Registered,
                QueryFragmentState::Running,
                QueryFragmentState::Completed,
            ],
            QueryFragmentState::Stopped => {
                vec![QueryFragmentState::Registered, QueryFragmentState::Stopped]
            }
    ...
    proptest! {
        ...
        fn blocking_drop_resolves_when_terminal(mut req in any::<CreateQueryWithRefs>()) {
            ...
            let result = handle.ask_with(
                Payload::parsed(Statement::DropQuery(drop_req)).block_until_dropped(),
            ).unwrap();
            let StatementResult::DroppedQueries(dropped) = result else {
                panic!("expected DroppedQueries");
            };
