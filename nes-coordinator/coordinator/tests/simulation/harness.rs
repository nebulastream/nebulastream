...
use crate::config::{NetworkConfig, SEND_LATENCY_HI, SEND_LATENCY_LO, SEND_TIMEOUT};
...
impl TestHarness {
        ...
        NetSim::current().update_config(|cfg| {
            if let Some(ref net) = network {
                let lo = net
                    ...
                    .unwrap_or(SEND_LATENCY_LO);
                let hi = net
                    .send_latency_hi_ms
                    .map(Duration::from_millis)
                    .unwrap_or(SEND_LATENCY_HI);
                cfg.send_latency = lo..hi;
                cfg.packet_loss_rate = net.packet_loss_rate.unwrap_or(0.0);
            } else {
                cfg.send_latency = SEND_LATENCY_LO..SEND_LATENCY_HI;
                cfg.packet_loss_rate = 0.0;
            }
        });

        // The directory is keyed by seed and wiped first, so a replay of the same seed
        // starts from an empty catalog instead of inheriting the previous run's rows.
    ...
    pub async fn worker_status(
        &self,
        workers: &[NetworkAddr],
    ) -> (Vec<QueryFragmentId>, Vec<QueryFragmentId>) {
        let (tx, rx): (
        ...
        for rsp in responses {
            active.extend(rsp.active_queries.into_iter().filter_map(|q| {
                q.query_id
                    .and_then(|id| id.fragment_id())
                    .map(QueryFragmentId::new)
            }));
            terminated.extend(rsp.terminated_queries.into_iter().filter_map(|q| {
                q.query_id
                    .and_then(|id| id.fragment_id())
                    .map(QueryFragmentId::new)
            }));
