...
//! Simulation harness: spins up the coordinator and worker nodes inside
//! madsim and gives workloads a single API to drive them.
//!
//! Every node — coordinator and each worker — is a separate madsim
...
//! clog, restart) is also just a method call, but the work happens
    ...
    pub async fn worker_status(
        ...
        for rsp in responses {
            active.extend(
                rsp.active_queries
                    .into_iter()
                    .filter_map(|q| q.query_id.map(|id| QueryFragmentId::new(id.id))),
            );
            terminated.extend(
