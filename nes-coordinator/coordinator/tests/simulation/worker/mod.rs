...
//!
//! Implements the same RPC surface a real worker exposes (register,
//! start, stop, per-fragment and aggregate status, plus health-check)
//! against an in-memory map of fragments. State transitions are
//! deterministic given a seed; `buggify` is sprinkled into each handler
...
impl WorkerRpcService for SingleNodeWorker {
    ...
    async fn register_query(
        ...
        fragments.insert(id, QueryQueryFragment::new());
