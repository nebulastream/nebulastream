...
//!
//! Links are picked independently with probability `partition_rate`, so
//! the partition is asymmetric in general — A may be unable to reach B
//! while B can still reach A — which is the realistic case for
//! controller and worker reconnect logic to handle.
