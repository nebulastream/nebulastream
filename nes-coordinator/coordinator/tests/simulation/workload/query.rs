...
use madsim::rand::Rng;
use model::query::fragment::{CreateFragment, FragmentsWithRefs};
...
pub struct QueryWorkload {
    ...
    /// Pre-generated fragment sets, one per query, respecting total capacity.
    fragment_sets: Vec<Vec<CreateFragment>>,
    ...
    async fn execute_op(
        ...
        match *op {
            QueryOp::CreateQuery(idx) => {
                let query = CreateQuery::new("SELECT 1".to_string())
                    .with_fragments(self.fragment_sets[idx].clone());
...
impl Workload for QueryWorkload {
    ...
    async fn setup(&mut self, harness: &TestHarness) {
        ...
        self.fragment_sets = arb(FragmentsWithRefs::for_queries(
            harness.workers().to_vec(),
            self.num_queries,
        ));
    }
