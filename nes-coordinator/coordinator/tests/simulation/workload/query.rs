...
//! Drives create/drop query traffic against the simulated cluster and
//! asserts that the catalog and the workers stay in agreement.
//!
//! The workload picks `num_ops` operations weighted by `create_weight`
//! and `drop_weight`, spreads them across `[begin, end)` with random
//! delays, and lets the runner replay each through the model state so
//! every operation lands in a well-formed catalog. The final checks
...
impl Workload for QueryWorkload {
    ...
    async fn check(&self, harness: &TestHarness) {
        ...
        check_invariants(
            &[
                &DbQueryFragmentsActive,
                &DbQueryFragmentsDropped,
...
async fn db_fragments_of(harness: &TestHarness, qids: Vec<QueryId>) -> Vec<query_fragment::Model> {
...
struct DbQueryFragmentsActive;

#[async_trait(?Send)]
impl Invariant<ModelState> for DbQueryFragmentsActive {
    fn name(&self) -> &str {
        "db_fragments_active"
    }
...
struct DbQueryFragmentsDropped;

#[async_trait(?Send)]
impl Invariant<ModelState> for DbQueryFragmentsDropped {
    fn name(&self) -> &str {
...
struct WorkerQueryFragmentsActive;
