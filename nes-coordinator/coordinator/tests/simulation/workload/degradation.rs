...
fn default_latency_lo_ms() -> u64 {
    config::DEGRADATION_LATENCY_LO_MS
}
fn default_latency_hi_ms() -> u64 {
    config::DEGRADATION_LATENCY_HI_MS
}
fn default_loss_rate() -> f64 {
    config::DEGRADATION_LOSS_RATE
}
...
struct DegradationConfig {
    ...
    #[serde(default = "default_latency_lo_ms")]
    latency_lo_ms: u64,
    #[serde(default = "default_latency_hi_ms")]
    latency_hi_ms: u64,
...
impl DegradationWorkload {
    ...
    pub fn from_options(options: &HashMap<String, toml::Value>) -> Self {
        let c: DegradationConfig = parse_required(options);
        Self {
    ...
    pub fn for_injection() -> Self {
        Self {
            ...
            latency_lo: Duration::from_millis(config::DEGRADATION_LATENCY_LO_MS),
            latency_hi: Duration::from_millis(config::DEGRADATION_LATENCY_HI_MS),
            loss_rate: config::DEGRADATION_LOSS_RATE,
        }
    }
}
...
inventory::submit! {
    FailureInjectorFactory {
        ...
        create: || Box::new(DegradationWorkload::for_injection()),
    }
}
