...
fn parse_host_addr(host_addr: &str) -> Result<Option<NetworkAddr>> {
    if host_addr.is_empty() {
        Ok(None)
    } else {
        Ok(Some(host_addr.parse().map_err(|e: String| anyhow!("{e}"))?))
    }
}
...
impl CoordinatorHandle {
    pub fn send(&self, input: Payload) -> Result<StatementResult> {
        ...
        self.runtime.handle().block_on(rx)?
    }
}
...
pub fn start_coordinator(
    ...
    CoordinatorHandle { sender, runtime }
}
