...
TransactionalCatalog::TransactionalCatalog(const TransactionContext& ctx) : ctx{ctx}
{
}
...
SourceDescriptor TransactionalCatalog::createInlineSource(
    ...
    std::unordered_map<Identifier, std::string> formatConfig)
{
    ...
    const auto id = PhysicalSourceId{create_inline_source(
        ctx, static_cast<bool>(kind), rust::Str{sourceType.asCanonicalString()}, schemaJson, configJson, formatJson, host.getRawValue())};

    auto descriptor = SourceDescriptor::create(
        id, LogicalSource{Identifier::fromCanonical(id.toString()), schema}, sourceType, host, config, formatConfig);
    if (!descriptor)
    {
        throw std::move(descriptor).error();
    }
...
SinkDescriptor TransactionalCatalog::createInlineSink(
    const ConnectorKind kind,
    ...
    const std::unordered_map<Identifier, std::string>& formatConfig)
{
    ...
    /// A schema-less inline sink (e.g. INTO Void() with no target schema) is stored with a null schema
    /// in the catalog and later inferred from the query output on the SinkLogicalOperator.
    const auto schemaJson = schema.has_value() ? rfl::json::write(rfl::Generic(reflect(*schema))) : std::string{"null"};
    const auto configJson = configToJson(config);
...
{
    const auto [host_addr, data_addr, max_operators] = get_worker(ctx, rust::Str{host.getRawValue().data(), host.getRawValue().size()});
    const Capacity cap
