...
std::optional<LogicalSource> getLogicalSource(const CatalogTxn& ctx, std::string_view name)
{
    try
    {
...
std::optional<std::unordered_set<SourceDescriptor>> getSourceDescriptors(const CatalogTxn& ctx, std::string_view logicalSourceName)
{
    try
    {
...
std::optional<SinkDescriptor> getSinkDescriptor(const CatalogTxn& ctx, std::string_view name)
{
    try
    {
...
std::optional<WorkerInfo> getWorker(const CatalogTxn& ctx, const Host& host)
{
    try
    {
...
NetworkTopology getTopology(const CatalogTxn& ctx)
{
