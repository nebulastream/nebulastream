...
{
    return BridgeResult{
        static_cast<uint16_t>(ex.code()), rust::String(std::string(ex.what())), rust::String(ex.trace().to_string())
    };
}
...
uint64_t to_ms(const std::optional<std::chrono::system_clock::time_point>& opt)
{
    if (!opt)
        return 0;
    return static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(opt->time_since_epoch()).
        count());
}
...
BridgeQueryStatus query_status(WorkerBridge& bridge, int64_t id)
{
    ...
    auto query_error = snapshot.metrics.error ? make_result(*snapshot.metrics.error) : ok();
    return BridgeQueryStatus{
        ok(), static_cast<int32_t>(snapshot.state), to_ms(snapshot.metrics.start), to_ms(snapshot.metrics.stop),
