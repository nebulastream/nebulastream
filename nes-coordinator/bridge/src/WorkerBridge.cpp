...
{
    return BridgeResult{static_cast<uint16_t>(ex.code()), rust::String(std::string(ex.what())), rust::String(ex.trace().to_string())};
}
...
BridgeQueryStatus query_status(WorkerBridge& bridge, int64_t id)
{
    ...
    return BridgeQueryStatus{
        ok(), static_cast<int32_t>(snapshot.state), to_ms(snapshot.metrics.start), to_ms(snapshot.metrics.stop), std::move(query_error)};
}
