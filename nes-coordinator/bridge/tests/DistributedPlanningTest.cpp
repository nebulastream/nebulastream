...
/// A throwaway catalog seeded from SQL DDL, paired with the query to place against it. Owns the `TestPlanningTransaction` so
/// the in-memory catalog outlives every optimizer run that reads it.
struct Prepared
{
    rust::Box<NES::Bridge::TestPlanningTransaction> context;
    std::string query;
    ...
    std::unordered_map<NES::Host, std::vector<NES::LogicalPlan>> optimize() const
    {
        auto output
            = NES::
                  SqlPlanner{std::make_shared<NES::CatalogBridge>(context->context(), NES::RequireHostConfig{}), NES::QueryOptimizerConfiguration{}, NES::RequireHostConfig{}}
                      .plan(query);
        if (!output)
        {
            throw std::move(output).error();
        }
        std::unordered_map<NES::Host, std::vector<NES::LogicalPlan>> byHost;
        for (auto& fragment : output->fragments)
        {
            byHost[fragment.host].push_back(std::move(fragment.plan));
        }
        return byHost;
    }
};
...
{
    auto context = NES::Bridge::create_test_planner_context();
    for (const auto& statement : splitStatements(setupSql))
    {
        const auto planned
            = NES::Bridge::plan_sql(context->context(), rust::Str{statement.data(), statement.size()}, rust::Str{}, rust::Str{});
        NES::Bridge::raiseReported(planned.error);
