...
#include <nes-coordinator-bridge/test_context.h>
...
public:
    ...
    static CoordinatorStatement plan(const std::string_view sql)
    {
        const auto context = Bridge::create_test_planner_context();
        auto output
            = SqlPlanner{std::make_shared<CatalogBridge>(context->context(), RequireHostConfig{}), QueryOptimizerConfiguration{}, RequireHostConfig{}}
                  .plan(sql);
        if (!output)
        {
...
TEST_F(DropStatementPlanningTest, FilterlessDropClearsEveryFilter)
{
    ...
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropLogicalSource>(logicalSource.variant()));
    EXPECT_FALSE(rfl::get<PlannedDropLogicalSource>(logicalSource.variant()).name.has_value());

    const auto physicalSource = plan("DROP PHYSICAL SOURCE");
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropPhysicalSource>(physicalSource.variant()));
    EXPECT_FALSE(rfl::get<PlannedDropPhysicalSource>(physicalSource.variant()).id.has_value());

    const auto sink = plan("DROP SINK");
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropSink>(sink.variant()));
    EXPECT_FALSE(rfl::get<PlannedDropSink>(sink.variant()).name.has_value());

    const auto model = plan("DROP MODEL");
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropModel>(model.variant()));
    EXPECT_FALSE(rfl::get<PlannedDropModel>(model.variant()).name.has_value());

    const auto query = plan("DROP QUERY");
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropQuery>(query.variant()));
    const auto& filters = rfl::get<PlannedDropQuery>(query.variant()).filters;
...
TEST_F(DropStatementPlanningTest, FilteredDropCarriesTheFilter)
{
    ...
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropLogicalSource>(logicalSource.variant()));
    EXPECT_EQ(rfl::get<PlannedDropLogicalSource>(logicalSource.variant()).name, "SOME_SOURCE");

    const auto physicalSource = plan("DROP PHYSICAL SOURCE WHERE ID = 7");
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropPhysicalSource>(physicalSource.variant()));
    EXPECT_EQ(rfl::get<PlannedDropPhysicalSource>(physicalSource.variant()).id, 7);

    const auto sink = plan("DROP SINK WHERE NAME = 'some_sink'");
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropSink>(sink.variant()));
    EXPECT_EQ(rfl::get<PlannedDropSink>(sink.variant()).name, "SOME_SINK");
}
...
TEST_F(DropStatementPlanningTest, DropWorkerRequiresAHostFilter)
{
    ...
    ASSERT_TRUE(rfl::holds_alternative<PlannedDropWorker>(worker.variant()));
    EXPECT_EQ(rfl::get<PlannedDropWorker>(worker.variant()).host.value(), "localhost:8080");
