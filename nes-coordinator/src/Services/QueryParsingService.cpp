...
SchemaPtr QueryParsingService::createSchemaFromCode(
  ...
  try {
    ...
    NES_ERROR(
        "QueryParsingService: Failed to create the query from input code "
        "string: {}",
        queryCodeSnippet);
    throw;
  } catch (...) {
...
QueryPlanPtr QueryParsingService::createQueryFromCodeString(
  ...
  if (queryCodeSnippet.find("Source(") != std::string::npos ||
    ...
    NES_ERROR(
        "QueryParsingService: queryIdAndCatalogEntryMapping are not allowed to "
        "specify schemas anymore.");
...
QueryPlanPtr QueryParsingService::createPatternFromCodeString(
  ...
  if (queryCodeSnippet.empty() || queryCodeSnippet.size() < 15) {
    NES_THROW_RUNTIME_ERROR(
        "QueryParsingService::createPatternFromCodeString: The query is too "
        "short, make sure you provide "
    ...
    NES_DEBUG(
        "QueryParsingService: ANTLR created the following AST from pattern "
        "string {}",
