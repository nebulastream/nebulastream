...
CoordinatorHealthCheckService::CoordinatorHealthCheckService(
    TopologyPtr topology, std::string healthServiceName,
    Configurations::CoordinatorConfigurationPtr coordinatorConfiguration)
    : topology(topology), workerRPCClient(WorkerRPCClient::create()),
...
void CoordinatorHealthCheckService::startHealthCheck() {
  ...
  healthCheckingThread = std::make_shared<std::thread>(([this]() {
    ...
    auto waitTime =
        std::chrono::seconds(this->coordinatorConfiguration
                                 ->coordinatorHealthCheckWaitTime.getValue());
    while (isRunning) {
      for (auto node : topologyIdToRPCAddressMap.lock_table()) {
        ...
        if (res) {
          NES_TRACE("NesCoordinator::healthCheck: node={} is alive",
                    destAddress);
          ...
          NES_WARNING(
              "NesCoordinator::healthCheck: node={} went dead so we remove it",
              destAddress);
          const auto &rootWorkerNodeIds = topology->getRootWorkerNodeIds();
          auto found = std::find(rootWorkerNodeIds.begin(),
                                 rootWorkerNodeIds.end(), node.first);
          if (found != rootWorkerNodeIds.end()) {
            NES_WARNING(
                "The failing node is the root node so we cannot delete it");
            ...
            if (ret) {
              NES_TRACE(
                  "NesCoordinator::healthCheck: remove node={} successfully",
                  destAddress);
            } else {
              NES_WARNING(
                  "Node went offline but could not be removed from topology");
            }
