...
RequestHandlerService::RequestHandlerService(
    ...
    const Statistic::AbstractStatisticQueryGeneratorPtr
        &statisticQueryGenerator,
    const Statistic::StatisticRegistryPtr &statisticRegistry)
    : optimizerConfiguration(optimizerConfiguration),
      queryParsingService(queryParsingService), queryCatalog(queryCatalog),
  ...
  syntacticQueryValidation =
      Optimizer::SyntacticQueryValidation::create(this->queryParsingService);
  semanticQueryValidation = Optimizer::SemanticQueryValidation::create(
...
QueryId RequestHandlerService::validateAndQueueAddQueryRequest(
  ...
  auto addRequest = RequestProcessor::AddQueryRequest::create(
      queryString, placementStrategy, RequestProcessor::DEFAULT_RETRIES,
      z3Context, queryParsingService);
  asyncRequestExecutor->runAsync(addRequest);
  auto future = addRequest->getFuture();
  return std::static_pointer_cast<RequestProcessor::AddQueryResponse>(
             future.get())
      ->queryId;
}
...
QueryId RequestHandlerService::validateAndQueueAddQueryRequest(
  ...
  auto addRequest = RequestProcessor::AddQueryRequest::create(
      queryPlan, placementStrategy, RequestProcessor::DEFAULT_RETRIES,
      z3Context);
  asyncRequestExecutor->runAsync(addRequest);
  auto future = addRequest->getFuture();
  return std::static_pointer_cast<RequestProcessor::AddQueryResponse>(
...
nlohmann::json RequestHandlerService::validateAndQueueExplainQueryRequest(
  ...
  auto explainRequest = RequestProcessor::ExplainRequest::create(
      queryPlan, placementStrategy, 1, z3Context);
  asyncRequestExecutor->runAsync(explainRequest);
  auto future = explainRequest->getFuture();
  return std::static_pointer_cast<RequestProcessor::ExplainResponse>(
             future.get())
      ->jsonResponse;
}
...
bool RequestHandlerService::validateAndQueueStopQueryRequest(QueryId queryId) {
  ...
  try {
    auto stopRequest = RequestProcessor::StopQueryRequest::create(
        queryId, RequestProcessor::DEFAULT_RETRIES);
    asyncRequestExecutor->runAsync(stopRequest);
    auto future = stopRequest->getFuture();
    auto response = future.get();
    auto success =
        std::static_pointer_cast<RequestProcessor::StopQueryResponse>(response)
            ->success;
    return success;
    ...
    return (e.getActualState() == QueryState::STOPPED ||
            e.getActualState() == QueryState::MARKED_FOR_HARD_STOP);
  }
}
...
bool RequestHandlerService::validateAndQueueFailQueryRequest(
  ...
  auto failRequest = RequestProcessor::FailQueryRequest::create(
      sharedQueryId, decomposedQueryPlanId, failureReason,
  ...
  auto returnedSharedQueryId =
      std::static_pointer_cast<RequestProcessor::FailQueryResponse>(
          future.get())
          ->sharedQueryId;
...
bool RequestHandlerService::queueNodeRelocationRequest(
  ...
  auto nodeRelocationRequest =
      RequestProcessor::Experimental::TopologyNodeRelocationRequest::create(
  ...
  auto changeResponse = std::static_pointer_cast<
      RequestProcessor::Experimental::TopologyNodeRelocationRequestResponse>(
      future.get());
  return changeResponse->success;
}

bool RequestHandlerService::queueISQPRequest(
    const std::vector<RequestProcessor::ISQPEventPtr> &isqpEvents) {

  auto isqpRequest = RequestProcessor::ISQPRequest::create(
      z3Context, isqpEvents, RequestProcessor::DEFAULT_RETRIES);
  asyncRequestExecutor->runAsync(isqpRequest);
  auto future = isqpRequest->getFuture();
  auto changeResponse =
      std::static_pointer_cast<RequestProcessor::ISQPRequestResponse>(
          future.get());
  return changeResponse->success;
}

std::vector<Statistic::StatisticKey>
RequestHandlerService::trackStatisticRequest(
  ...
  const auto statisticQuery = statisticQueryGenerator->createStatisticQuery(
      *characteristic, window, sendingPolicy, triggerCondition, *queryCatalog);

  // 2. Submitting the query to the system
  const auto queryId = validateAndQueueAddQueryRequest(
      statisticQuery.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);

  // 3. Extracting all statistic ids from the submitted queries to create
  // StatisticKeys out of the statistic ids
  const auto newStatisticIds =
      statisticIdsExtractor.extractStatisticIdsFromQueryId(queryCatalog,
                                                           queryId);
  const auto metric = characteristic->getType();

  // 4. For each statistic id, we create a new StatisticKey and also insert it
  // into the registry, if the StatisticKey is not already contained in the
  // registry
  ...
  for (const auto &statisticId : newStatisticIds) {
    ...
    if (!statisticRegistry->contains(newKey.hash())) {
      StatisticInfo statisticInfo(window, triggerCondition, callBack, queryId,
                                  metric);
      statisticRegistry->insert(newKey.hash(), statisticInfo);
    }
  }

  // 5. We return all statisticKeys that belong to this track request, so that
  // they can be used for probing the statistics
...
RequestHandlerService::trackStatisticRequest(
  ...
  return trackStatisticRequest(
      characteristic, window, Statistic::NeverTrigger::create(),
      Statistic::SENDING_ASAP(Statistic::StatisticDataCodec::DEFAULT), nullptr);
}
