...
void deserializeOpenCLDeviceInfo(
  ...
  for (const auto &serializedDeviceInfo : serializedDeviceInfos) {
    if (serializedDeviceInfo.maxworkitems_size() != 3) {
      NES_WARNING("OpenCL device {} {} {} has invalid number of maxWorkItems: "
                  "{}; skipping.",
...
Status CoordinatorRPCServer::RegisterWorker(
  ...
  if (coordinatorHealthCheckService &&
    ...
    NES_TRACE("TopologyManagerService::registerWorker: node with worker id {} "
              "is re-registering",
              configWorkerId);
...
Status
CoordinatorRPCServer::UnregisterWorker(ServerContext *,
...
Status CoordinatorRPCServer::RegisterPhysicalSource(
  ...
  if (!overallSuccess) {
    NES_WARNING("CoordinatorRPCServer::RegisterPhysicalSource Could not "
                "register all phyisical sources");
    sourceCatalogService->unregisterAllPhysicalSourcesByWorker(
        WorkerId(request->workerid()));
  }

  reply->set_success(overallSuccess);
  NES_DEBUG("CoordinatorRPCServer::RegisterPhysicalSource: request done "
            "overallSuccess={}",
            overallSuccess);
  return Status::OK;
}
...
Status CoordinatorRPCServer::NotifyQueryFailure(
  ...
  try {
    NES_ERROR("CoordinatorRPCServer::notifyQueryFailure: failure message "
              "received. id of failed query: {} subplan: {} Id of "
...
Status
CoordinatorRPCServer::GetNodesInRange(ServerContext *,
...
Status
CoordinatorRPCServer::RequestSoftStop(::grpc::ServerContext *,
                                      const ::RequestSoftStopMessage *request,
                                      ::StopRequestReply *response) {
  auto sharedQueryId = SharedQueryId(request->queryid());
  auto decomposedQueryPlanId = DecomposedQueryPlanId(request->subqueryid());
  NES_WARNING("CoordinatorRPCServer: received request for soft stopping the "
...
Status CoordinatorRPCServer::notifySourceStopTriggered(
  ...
  NES_INFO("CoordinatorRPCServer: received request for soft stopping the sub "
           "pan : {}  shared query plan id:{}",
...
Status
CoordinatorRPCServer::SendLocationUpdate(ServerContext *,
  ...
  NES_DEBUG("Coordinator received location update from node with id {} which "
            "reports [{}, {}] at TS {}",
            request->workerid(), coordinates.lat(), coordinates.lng(),
            timestamp);
...
Status
CoordinatorRPCServer::RelocateTopologyNode(ServerContext *,
