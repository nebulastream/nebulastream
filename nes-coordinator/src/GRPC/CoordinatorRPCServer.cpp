...
// Helper method to deserialize information about the OpenCL devices of a
// worker.
void deserializeOpenCLDeviceInfo(
    std::any& property,
    const google::protobuf::RepeatedPtrField<SerializedOpenCLDeviceInfo>&
        serializedDeviceInfos) {
  ...
  for (const auto& serializedDeviceInfo : serializedDeviceInfos) {
    if (serializedDeviceInfo.maxworkitems_size() != 3) {
      NES_WARNING(
          "OpenCL device {} {} {} has invalid number of maxWorkItems: {}; "
          "skipping.",
    ...
    devices.emplace_back(serializedDeviceInfo.platformvendor(),
                         serializedDeviceInfo.platformname(),
                         serializedDeviceInfo.devicename(),
                         serializedDeviceInfo.doublefpsupport(), maxWorkItems,
...
CoordinatorRPCServer::CoordinatorRPCServer(
    RequestHandlerServicePtr requestHandlerService, TopologyPtr topology,
    ...
    : requestHandlerService(std::move(requestHandlerService)),
      topology(std::move(topology)),
      sourceCatalogService(std::move(sourceCatalogService)),
      queryCatalog(std::move(queryCatalog)),
      monitoringManager(std::move(monitoringManager)),
      queryParsingService(std::move(queryParsingService)),
      coordinatorHealthCheckService(std::move(coordinatorHealthCheckService)){};

Status CoordinatorRPCServer::RegisterWorker(
    ServerContext*, const RegisterWorkerRequest* registrationRequest,
    RegisterWorkerReply* reply) {
  NES_DEBUG("Received worker registration request {}",
  ...
  workerProperties[NES::Worker::Configuration::TENSORFLOW_SUPPORT] =
      registrationRequest->tfsupported();
  workerProperties[NES::Worker::Configuration::JAVA_UDF_SUPPORT] =
      registrationRequest->javaudfsupported();
  workerProperties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
      NES::Spatial::Util::SpatialTypeUtility::protobufEnumToNodeType(
          registrationRequest->spatialtype());
  deserializeOpenCLDeviceInfo(
      workerProperties[NES::Worker::Configuration::OPENCL_DEVICES],
  ...
  if (coordinatorHealthCheckService &&
      coordinatorHealthCheckService->isWorkerInactive(configWorkerId)) {
    // node is re-registering (was inactive and became active again)
    NES_TRACE(
        "TopologyManagerService::registerWorker: node with worker id {} is "
        "re-registering",
        configWorkerId);
  ...
  NES_DEBUG("TopologyManagerService::RegisterNode: request ={}",
            registrationRequest->DebugString());
  WorkerId workerId = topology->registerWorker(
      configWorkerId, address, grpcPort, dataPort, slots, workerProperties,
      bandwidthInMbps, latencyInMs);

  NES::Spatial::DataTypes::Experimental::GeoLocation geoLocation(
      registrationRequest->waypoint().geolocation().lat(),
  ...
  auto registrationMetrics = std::make_shared<Monitoring::Metric>(
      Monitoring::RegistrationMetrics(
          registrationRequest->registrationmetrics()),
      Monitoring::MetricType::RegistrationMetric);
  registrationMetrics->getValue<Monitoring::RegistrationMetrics>().nodeId =
      workerId;
...
Status CoordinatorRPCServer::UnregisterWorker(
    ServerContext*, const UnregisterWorkerRequest* request,
    UnregisterWorkerReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::UnregisterNode: request ={}",
            request->DebugString());
  ...
  NES_DEBUG(
      "CoordinatorRPCServer::UnregisterNode: Worker successfully removed");
  reply->set_success(true);
  return Status::OK;
}

Status CoordinatorRPCServer::RegisterPhysicalSource(
    ServerContext*, const RegisterPhysicalSourcesRequest* request,
    RegisterPhysicalSourcesReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::RegisterPhysicalSource: request ={}",
            request->DebugString());
  bool overallSuccess = true;
  for (const auto& physicalSourceDefinition : request->physicalsourcetypes()) {
    auto [success, errorMessage] = sourceCatalogService->registerPhysicalSource(
        physicalSourceDefinition.physicalsourcename(),
        physicalSourceDefinition.logicalsourcename(),
        WorkerId(request->workerid()));
    auto result = reply->add_results();
    result->set_physicalsourcename(
        physicalSourceDefinition.physicalsourcename());
  ...
  // Rollback any source registrations if any source failed. Currently we assume
  // that the worker is going to fail, and reattempts to register all sources
  if (!overallSuccess) {
    NES_WARNING(
        "CoordinatorRPCServer::RegisterPhysicalSource Could not register all "
        "phyisical sources");
    sourceCatalogService->unregisterAllPhysicalSourcesByWorker(
        WorkerId(request->workerid()));
  }

  reply->set_success(overallSuccess);
  NES_DEBUG(
      "CoordinatorRPCServer::RegisterPhysicalSource: request done "
      "overallSuccess={}",
      overallSuccess);
  return Status::OK;
}

Status CoordinatorRPCServer::UnregisterPhysicalSource(
    ServerContext*, const UnregisterPhysicalSourceRequest* request,
    UnregisterPhysicalSourceReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::UnregisterPhysicalSource: request ={}",
            request->DebugString());

  bool success = sourceCatalogService->unregisterPhysicalSource(
      request->physicalsourcename(), request->logicalsourcename(),
      WorkerId(request->workerid()));
...
Status CoordinatorRPCServer::RegisterLogicalSource(
    ServerContext*, const RegisterLogicalSourceRequest* request,
    RegisterLogicalSourceReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::RegisterLogicalSource: request = {}",
            request->DebugString());

  auto schema =
      queryParsingService->createSchemaFromCode(request->sourceschema());
  bool success = sourceCatalogService->registerLogicalSource(
      request->logicalsourcename(), schema);
...
Status CoordinatorRPCServer::UnregisterLogicalSource(
    ServerContext*, const UnregisterLogicalSourceRequest* request,
    UnregisterLogicalSourceReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::RegisterLogicalSource: request ={}",
            request->DebugString());

  bool success = sourceCatalogService->unregisterLogicalSource(
...
Status CoordinatorRPCServer::AddParent(ServerContext*,
                                       const AddParentRequest* request,
                                       AddParentReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::AddParent: request = {}",
            request->DebugString());

  bool success = topology->addTopologyNodeAsChild(WorkerId(request->parentid()),
                                                  WorkerId(request->childid()));
  if (success) {
...
Status CoordinatorRPCServer::ReplaceParent(ServerContext*,
                                           const ReplaceParentRequest* request,
                                           ReplaceParentReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::ReplaceParent: request = {}",
            request->DebugString());

  bool success = topology->removeTopologyNodeAsChild(
      WorkerId(request->oldparent()), WorkerId(request->childid()));
  if (success) {
    NES_DEBUG("CoordinatorRPCServer::ReplaceParent success removeAsParent");
    bool success2 = topology->addTopologyNodeAsChild(
        WorkerId(request->newparent()), WorkerId(request->childid()));
    if (success2) {
...
Status CoordinatorRPCServer::RemoveParent(ServerContext*,
                                          const RemoveParentRequest* request,
                                          RemoveParentReply* reply) {
  NES_DEBUG("CoordinatorRPCServer::RemoveParent: request = {}",
            request->DebugString());

  bool success = topology->removeTopologyNodeAsChild(
      WorkerId(request->parentid()), WorkerId(request->childid()));
  if (success) {
...
Status CoordinatorRPCServer::NotifyQueryFailure(
    ServerContext*, const QueryFailureNotification* request,
    QueryFailureNotificationReply* reply) {
  try {
    NES_ERROR(
        "CoordinatorRPCServer::notifyQueryFailure: failure message received. "
        "id of failed query: {} subplan: {} Id of "
        "worker: {} Reason for failure: {}",
        request->queryid(), request->subqueryid(), request->workerid(),
        request->errormsg());

    NES_ASSERT2_FMT(!request->errormsg().empty(),
                    "Cannot fail query without error message "
                        << request->queryid()
                        << " subplan: " << request->subqueryid()
    ...
    if (!requestHandlerService->validateAndQueueFailQueryRequest(
            sharedQueryId, decomposedQueryPlanId, request->errormsg())) {
      NES_ERROR(
          "Failed to create Query Failure request for shared query plan {}",
          sharedQueryId);
      return Status::CANCELLED;
    }
    ...
    NES_ERROR("CoordinatorRPCServer: received broken failure message: {}",
              ex.what());
    return Status::CANCELLED;
  }
}

Status CoordinatorRPCServer::GetNodesInRange(
    ServerContext*, const GetNodesInRangeRequest* request,
    GetNodesInRangeReply* reply) {
  auto inRange = topology->getTopologyNodeIdsInRange(
...
Status CoordinatorRPCServer::SendErrors(ServerContext*,
  ...
  try {
    NES_ERROR(
        "CoordinatorRPCServer::sendErrors: failure message received."
        "Id of worker: {} Reason for failure: {}",
        request->workerid(), request->errormsg());
...
Status CoordinatorRPCServer::RequestSoftStop(
    ::grpc::ServerContext*, const ::RequestSoftStopMessage* request,
    ::StopRequestReply* response) {
  auto sharedQueryId = SharedQueryId(request->queryid());
  auto decomposedQueryPlanId = DecomposedQueryPlanId(request->subqueryid());
  NES_WARNING(
      "CoordinatorRPCServer: received request for soft stopping the shared "
      "query plan id: {}",
      sharedQueryId)

  // Check with query catalog service if the request possible
  auto softStopPossible = queryCatalog->updateDecomposedQueryPlanStatus(
      sharedQueryId, decomposedQueryPlanId,
      INVALID_DECOMPOSED_QUERY_PLAN_VERSION, QueryState::MARKED_FOR_SOFT_STOP,
...
Status CoordinatorRPCServer::notifySourceStopTriggered(
    ::grpc::ServerContext*, const ::SoftStopTriggeredMessage* request,
    ::SoftStopTriggeredReply* response) {
  auto sharedQueryId = SharedQueryId(request->queryid());
  auto decomposedQueryPlanId = DecomposedQueryPlanId(request->querysubplanid());
  NES_INFO(
      "CoordinatorRPCServer: received request for soft stopping the sub pan : "
      "{}  shared query plan id:{}",
      decomposedQueryPlanId, sharedQueryId)

  // inform catalog service
  bool success = queryCatalog->updateDecomposedQueryPlanStatus(
      sharedQueryId, decomposedQueryPlanId,
      INVALID_DECOMPOSED_QUERY_PLAN_VERSION, QueryState::SOFT_STOP_TRIGGERED,
      INVALID_WORKER_NODE_ID);
...
Status CoordinatorRPCServer::NotifySoftStopCompleted(
    ::grpc::ServerContext*, const ::SoftStopCompletionMessage* request,
  ...
  bool success = queryCatalog->updateDecomposedQueryPlanStatus(
      sharedQueryId, decomposedQueryPlanId,
      INVALID_DECOMPOSED_QUERY_PLAN_VERSION, QueryState::SOFT_STOP_COMPLETED,
      INVALID_WORKER_NODE_ID);
...
Status CoordinatorRPCServer::SendScheduledReconnect(
    ServerContext*, const SendScheduledReconnectRequest* request,
    SendScheduledReconnectReply* reply) {
  auto reconnectsToAddMessage = request->addreconnects();
  std::vector<NES::Spatial::Mobility::Experimental::ReconnectPoint>
      addedReconnects;
  for (const auto& toAdd : reconnectsToAddMessage) {
    ...
    addedReconnects.emplace_back(
        NES::Spatial::Mobility::Experimental::ReconnectPoint{
            location, WorkerId(toAdd.id()), toAdd.time()});
  }
  ...
  for (const auto& toRemove : reconnectsToRemoveMessage) {
    NES::Spatial::DataTypes::Experimental::GeoLocation location(
        toRemove.geolocation());
    removedReconnects.emplace_back(
        NES::Spatial::Mobility::Experimental::ReconnectPoint{
            location, WorkerId(toRemove.id()), toRemove.time()});
  }
...
Status CoordinatorRPCServer::SendLocationUpdate(
    ServerContext*, const LocationUpdateRequest* request,
    LocationUpdateReply* reply) {
  auto coordinates = request->waypoint().geolocation();
  auto timestamp = request->waypoint().timestamp();
  NES_DEBUG(
      "Coordinator received location update from node with id {} which reports "
      "[{}, {}] at TS {}",
      request->workerid(), coordinates.lat(), coordinates.lng(), timestamp);
  // todo #2862: update coordinator trajectory prediction
  auto geoLocation =
      NES::Spatial::DataTypes::Experimental::GeoLocation(coordinates);
  if (!topology->updateGeoLocation(WorkerId(request->workerid()),
...
Status CoordinatorRPCServer::RelocateTopologyNode(
    ServerContext*, const NodeRelocationRequest* request,
  ...
  for (const auto& removedTopologyLink : request->removedlinks()) {
    removedLinks.push_back(
        TopologyLinkInformation(WorkerId(removedTopologyLink.upstream()),
  ...
  for (const auto& addedTopologyLink : request->addedlinks()) {
    addedLinks.push_back(
        TopologyLinkInformation(WorkerId(addedTopologyLink.upstream()),
                                WorkerId(addedTopologyLink.downstream())));
  }
  auto success = requestHandlerService->queueNodeRelocationRequest(removedLinks,
                                                                   addedLinks);
  reply->set_success(success);
  if (success) {
    return Status::OK;
