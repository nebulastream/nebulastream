...
void deserializeOpenCLDeviceInfo(
    std::any& property, const google::protobuf::RepeatedPtrField<SerializedOpenCLDeviceInfo>& serializedDeviceInfos)
{
    ...
    for (const auto& serializedDeviceInfo : serializedDeviceInfos)
    {
        ...
        {
            NES_WARNING(
                "OpenCL device {} {} {} has invalid number of maxWorkItems: {}; skipping.",
        ...
        std::array<size_t, 3> maxWorkItems{
            serializedDeviceInfo.maxworkitems(0), serializedDeviceInfo.maxworkitems(1), serializedDeviceInfo.maxworkitems(2)};
...
Status CoordinatorRPCServer::RegisterWorker(ServerContext*, const RegisterWorkerRequest* registrationRequest, RegisterWorkerReply* reply)
{
    ...
    deserializeOpenCLDeviceInfo(workerProperties[NES::Worker::Configuration::OPENCL_DEVICES], registrationRequest->opencldevices());
    ...
    WorkerId workerId
        = topology->registerWorker(configWorkerId, address, grpcPort, dataPort, slots, workerProperties, bandwidthInMbps, latencyInMs);

    NES::Spatial::DataTypes::Experimental::GeoLocation geoLocation(
        registrationRequest->waypoint().geolocation().lat(), registrationRequest->waypoint().geolocation().lng());
    ...
    auto registrationMetrics = std::make_shared<Monitoring::Metric>(
        Monitoring::RegistrationMetrics(registrationRequest->registrationmetrics()), Monitoring::MetricType::RegistrationMetric);
...
Status CoordinatorRPCServer::UnregisterWorker(ServerContext*, const UnregisterWorkerRequest* request, UnregisterWorkerReply* reply)
{
...
Status CoordinatorRPCServer::RegisterPhysicalSource(
    ServerContext*, const RegisterPhysicalSourcesRequest* request, RegisterPhysicalSourcesReply* reply)
{
    ...
    for (const auto& physicalSourceDefinition : request->physicalsourcetypes())
    {
        auto [success, errorMessage] = sourceCatalogService->registerPhysicalSource(
            physicalSourceDefinition.physicalsourcename(), physicalSourceDefinition.logicalsourcename(), WorkerId(request->workerid()));
...
Status CoordinatorRPCServer::UnregisterPhysicalSource(
    ServerContext*, const UnregisterPhysicalSourceRequest* request, UnregisterPhysicalSourceReply* reply)
{
    ...
    bool success = sourceCatalogService->unregisterPhysicalSource(
        request->physicalsourcename(), request->logicalsourcename(), WorkerId(request->workerid()));
...
Status
CoordinatorRPCServer::RegisterLogicalSource(ServerContext*, const RegisterLogicalSourceRequest* request, RegisterLogicalSourceReply* reply)
{
...
Status CoordinatorRPCServer::UnregisterLogicalSource(
    ServerContext*, const UnregisterLogicalSourceRequest* request, UnregisterLogicalSourceReply* reply)
{
    NES_DEBUG("CoordinatorRPCServer::RegisterLogicalSource: request ={}", request->DebugString());
...
Status
CoordinatorRPCServer::NotifyQueryFailure(ServerContext*, const QueryFailureNotification* request, QueryFailureNotificationReply* reply)
{
    try
    {
        NES_ERROR(
            "CoordinatorRPCServer::notifyQueryFailure: failure message received. id of failed query: {} subplan: {} Id of "
...
Status CoordinatorRPCServer::GetNodesInRange(ServerContext*, const GetNodesInRangeRequest* request, GetNodesInRangeReply* reply)
{
    auto inRange = topology->getTopologyNodeIdsInRange(
        NES::Spatial::DataTypes::Experimental::GeoLocation(request->geolocation()), request->radius());
...
Status CoordinatorRPCServer::RequestSoftStop(::grpc::ServerContext*, const ::RequestSoftStopMessage* request, ::StopRequestReply* response)
{
    auto sharedQueryId = SharedQueryId(request->queryid());
    auto decomposedQueryPlanId = DecomposedQueryPlanId(request->subqueryid());
...
Status CoordinatorRPCServer::notifySourceStopTriggered(
    ::grpc::ServerContext*, const ::SoftStopTriggeredMessage* request, ::SoftStopTriggeredReply* response)
{
    auto sharedQueryId = SharedQueryId(request->queryid());
    auto decomposedQueryPlanId = DecomposedQueryPlanId(request->querysubplanid());
    NES_INFO(
        "CoordinatorRPCServer: received request for soft stopping the sub pan : {}  shared query plan id:{}",
        decomposedQueryPlanId,
        sharedQueryId)
...
Status CoordinatorRPCServer::NotifySoftStopCompleted(
    ::grpc::ServerContext*, const ::SoftStopCompletionMessage* request, ::SoftStopCompletionReply* response)
{
    //Fetch the request
    auto sharedQueryId = SharedQueryId(request->queryid());
    auto decomposedQueryPlanId = DecomposedQueryPlanId(request->querysubplanid());
...
Status CoordinatorRPCServer::SendScheduledReconnect(
    ServerContext*, const SendScheduledReconnectRequest* request, SendScheduledReconnectReply* reply)
{
    ...
    for (const auto& toAdd : reconnectsToAddMessage)
    {
        ...
        addedReconnects.emplace_back(NES::Spatial::Mobility::Experimental::ReconnectPoint{location, WorkerId(toAdd.id()), toAdd.time()});
    }
...
Status CoordinatorRPCServer::SendLocationUpdate(ServerContext*, const LocationUpdateRequest* request, LocationUpdateReply* reply)
{
    auto coordinates = request->waypoint().geolocation();
    auto timestamp = request->waypoint().timestamp();
    NES_DEBUG(
        "Coordinator received location update from node with id {} which reports [{}, {}] at TS {}",
        request->workerid(),
        coordinates.lat(),
        coordinates.lng(),
        timestamp);
...
Status CoordinatorRPCServer::RelocateTopologyNode(ServerContext*, const NodeRelocationRequest* request, NodeRelocationReply* reply)
{
    ...
    {
        addedLinks.push_back(TopologyLinkInformation(WorkerId(addedTopologyLink.upstream()), WorkerId(addedTopologyLink.downstream())));
    }
