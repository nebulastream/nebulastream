...
WorkerRPCClientPtr WorkerRPCClient::create() {
  return std::make_shared<WorkerRPCClient>(WorkerRPCClient());
}

bool WorkerRPCClient::registerQuery(
  ...
  NES_DEBUG("WorkerRPCClient::registerQuery address={} sharedQueryId={} "
            "decomposedQueryPlanId = {} ",
            address, sharedQueryId, decomposedQueryPlanId);

  // wrap the query id and the query operators in the protobuf register query
  // request object.
  RegisterQueryRequest request;

  // serialize query plan.
  auto serializedQueryPlan = request.mutable_decomposedqueryplan();
  DecomposedQueryPlanSerializationUtil::serializeDecomposedQueryPlan(
      decomposedQueryPlan, serializedQueryPlan);
  ...
  std::unique_ptr<WorkerRPCService::Stub> workerStub =
      WorkerRPCService::NewStub(chan);
  Status status = workerStub->RegisterQuery(&context, request, &reply);

  if (status.ok()) {
    NES_DEBUG("WorkerRPCClient::registerQuery: status ok return success={}",
              reply.success());
  ...
  throw Exceptions::RuntimeException(
      "Error while WorkerRPCClient::registerQuery");
}

void WorkerRPCClient::registerQueryAsync(
    const std::string &address,
    const DecomposedQueryPlanPtr &decomposedQueryPlan,
  ...
  NES_DEBUG("WorkerRPCClient::registerQueryAsync address={} sharedQueryId={} "
            "decomposedQueryPlanId = {}",
            address, sharedQueryId, decomposedQueryPlanId);
  ...
  DecomposedQueryPlanSerializationUtil::serializeDecomposedQueryPlan(
      decomposedQueryPlan, serializableQueryPlan);
  ...
  std::unique_ptr<WorkerRPCService::Stub> workerStub =
      WorkerRPCService::NewStub(channel);
  ...
  call->responseReader =
      workerStub->PrepareAsyncRegisterQuery(&call->context, request, cq.get());
...
void WorkerRPCClient::checkAsyncResult(
    const std::vector<RpcAsyncRequest> &rpcAsyncRequests) {
  ...
  for (const auto &rpcAsyncRequest : rpcAsyncRequests) {
    ...
    if (requestClientMode == RpcClientMode::Register) {
      ...
      auto *call =
          static_cast<AsyncClientCall<UnregisterQueryReply> *>(got_tag);
      status = call->status.ok();
      delete call;
...
void WorkerRPCClient::unregisterQueryAsync(const std::string &address,
                                           ...
                                           const CompletionQueuePtr &cq) {
  NES_DEBUG("WorkerRPCClient::unregisterQueryAsync address={} queryId={}",
            address, sharedQueryId);
  ...
  // Because we are using the asynchronous API, we need to hold on to
  // the "call" instance in order to get updates on the ongoing RPC.
  call->responseReader = workerStub->PrepareAsyncUnregisterQuery(
      &call->context, request, cq.get());
...
bool WorkerRPCClient::unregisterQuery(const std::string &address,
                                      SharedQueryId sharedQueryId) {
  NES_DEBUG("WorkerRPCClient::unregisterQuery address={} queryId={}", address,
            sharedQueryId);
  ...
  if (status.ok()) {
    NES_DEBUG("WorkerRPCClient::unregisterQuery: status ok return success={}",
              reply.success());
    return reply.success();
  }
  NES_DEBUG(" WorkerRPCClient::unregisterQuery error={}: {}",
            status.error_code(), status.error_message());
  throw Exceptions::RuntimeException(
      "Error while WorkerRPCClient::unregisterQuery");
}

bool WorkerRPCClient::startQuery(const std::string &address,
                                 SharedQueryId sharedQueryId,
                                 DecomposedQueryPlanId decomposedQueryPlanId) {
  NES_DEBUG("WorkerRPCClient::startQuery address={} shared queryId={} "
  ...
  if (status.ok()) {
    NES_DEBUG("WorkerRPCClient::startQuery: status ok return success={}",
              reply.success());
    return reply.success();
  }
  NES_DEBUG(" WorkerRPCClient::startQuery error={}: {}", status.error_code(),
            status.error_message());
  throw Exceptions::RuntimeException("Error while WorkerRPCClient::startQuery");
}

void WorkerRPCClient::startQueryAsync(
    const std::string &address, SharedQueryId sharedQueryId,
    DecomposedQueryPlanId decomposedQueryPlanId, const CompletionQueuePtr &cq) {
  NES_DEBUG("WorkerRPCClient::startQueryAsync address={} shared queryId={} "
            "decomposed queryId={}",
            address, sharedQueryId, decomposedQueryPlanId);
  ...
  // the "call" instance in order to get updates on the ongoing RPC.
  call->responseReader =
      workerStub->PrepareAsyncStartQuery(&call->context, request, cq.get());
...
bool WorkerRPCClient::stopQuery(const std::string &address,
  ...
  NES_DEBUG("WorkerRPCClient::markQueryForStop address={} shared queryId={}",
            address, sharedQueryId);
  ...
  if (status.ok()) {
    NES_DEBUG("WorkerRPCClient::markQueryForStop: status ok return success={}",
              reply.success());
    return reply.success();
  }
  NES_ERROR(" WorkerRPCClient::markQueryForStop error={}: {}",
            status.error_code(), status.error_message());
  throw Exceptions::RuntimeException(
      "Error while WorkerRPCClient::markQueryForStop");
}

void WorkerRPCClient::stopQueryAsync(
    const std::string &address, SharedQueryId sharedQueryId,
    DecomposedQueryPlanId decomposedQueryPlanId,
    Runtime::QueryTerminationType terminationType,
    const CompletionQueuePtr &cq) {
  NES_DEBUG("WorkerRPCClient::stopQueryAsync address={} shared queryId={}",
            address, sharedQueryId);
  ...
  // the "call" instance in order to get updates on the ongoing RPC.
  call->responseReader =
      workerStub->PrepareAsyncStopQuery(&call->context, request, cq.get());
...
bool WorkerRPCClient::registerMonitoringPlan(
    const std::string &address, const Monitoring::MonitoringPlanPtr &plan) {
  ...
  NES_THROW_RUNTIME_ERROR(" WorkerRPCClient::RequestMonitoringData error=" +
...
std::string WorkerRPCClient::requestMonitoringData(const std::string &address) {
  ...
  if (status.ok()) {
    NES_DEBUG("WorkerRPCClient::RequestMonitoringData: metrics received {}",
              reply.metricsasjson());
...
bool WorkerRPCClient::injectEpochBarrier(uint64_t timestamp, uint64_t queryId,
                                         const std::string &address) {
...
bool WorkerRPCClient::bufferData(const std::string &address,
  ...
  if (status.ok()) {
    NES_DEBUG("WorkerRPCClient::BeginBuffer: status ok return success={}",
              reply.success());
    return reply.success();
  } else {
...
bool WorkerRPCClient::updateNetworkSink(const std::string &address,
  ...
  if (status.ok()) {
    NES_DEBUG(
        "WorkerRPCClient::UpdateNetworkSinks: status ok return success={}",
        reply.success());
    return reply.success();
  } else {
    NES_ERROR(" WorkerRPCClient::UpdateNetworkSinks error={}: {}",
              status.error_code(), status.error_message());
    throw Exceptions::RuntimeException(
        "Error while WorkerRPCClient::updateNetworkSinks");
  }
}
...
bool WorkerRPCClient::checkHealth(const std::string &address,
  ...
  std::shared_ptr<::grpc::Channel> chan =
      grpc::CreateChannel(address, grpc::InsecureChannelCredentials());
  std::unique_ptr<grpc::health::v1::Health::Stub> workerStub =
  ...
  if (status.ok()) {
    NES_TRACE("WorkerRPCClient::checkHealth: status ok return success={}",
              response.status());
    return response.status();
  } else {
    NES_ERROR(" WorkerRPCClient::checkHealth error={}: {}", status.error_code(),
              status.error_message());
    return response.status();
  }
}

Spatial::DataTypes::Experimental::Waypoint
WorkerRPCClient::getWaypoint(const std::string &address) {
  ...
  if (reply.has_waypoint()) {
    ...
    if (timestamp != 0) {
      return {Spatial::DataTypes::Experimental::GeoLocation(geoLocation.lat(),
                                                            geoLocation.lng()),
              timestamp};
    }
    // no valid timestamp to include
    return NES::Spatial::DataTypes::Experimental::Waypoint(
        NES::Spatial::DataTypes::Experimental::GeoLocation(geoLocation.lat(),
                                                           geoLocation.lng()));
  }
  // location is invalid
  return Spatial::DataTypes::Experimental::Waypoint(
      Spatial::DataTypes::Experimental::Waypoint::invalid());
}
} // namespace NES
