    ...
    const TopologyPtr & topology)
{
    ...
    auto upstreamDownstreamOperatorPairs = findNetworkOperatorsForLink(sharedQueryPlanId, lockedUpstreamNode, lockedDownstreamNode);
    for (auto & [upstreamOperator, downstreamOperator] : upstreamDownstreamOperatorPairs)
    {
        ...
        auto upstreamLogicalOperatorId = std::any_cast<OperatorId>(upstreamOperator->getProperty(Optimizer::UPSTREAM_LOGICAL_OPERATOR_ID));
        ...
        downstreamOperator = queryPlanForSharedQuery->getOperatorWithOperatorId(downstreamLogicalOperatorId)->as<LogicalOperator>();
    }
    ...
    for (const auto & [upstreamOperator, downstreamOperator] : upstreamDownstreamOperatorPairs)
    {
        ...
        const auto upstreamWorkerId = std::any_cast<WorkerId>(upstreamSharedQueryOperater->getProperty(Optimizer::PINNED_WORKER_ID));
        const auto downstreamSharedQueryOperator = queryPlanForSharedQuery->getOperatorWithOperatorId(downstreamOperator->getId());
        const auto downstreamWorkerId = std::any_cast<WorkerId>(downstreamSharedQueryOperator->getProperty(Optimizer::PINNED_WORKER_ID));
    ...
    Optimizer::ExecutionNodeWLock lockedDownstreamNode)
{
    ...
    for (const auto & subPlan : upstreamSubPlans)
    {
        for (const auto & sinkOperator : subPlan->getSinkOperators())
        {
            ...
            if (upstreamNetworkSinkDescriptor && upstreamNetworkSinkDescriptor->getNodeLocation().getNodeId() == downstreamWorkerId)
            {
