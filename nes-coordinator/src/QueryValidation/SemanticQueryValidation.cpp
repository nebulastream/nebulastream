...
SemanticQueryValidation::SemanticQueryValidation(
    ...
    : sourceCatalog(sourceCatalog), performAdvanceChecks(advanceChecks),
      udfCatalog(udfCatalog) {}

SemanticQueryValidationPtr SemanticQueryValidation::create(
    const Catalogs::Source::SourceCatalogPtr &sourceCatalog,
    const Catalogs::UDF::UDFCatalogPtr &udfCatalog, bool advanceChecks) {
  return std::make_shared<SemanticQueryValidation>(sourceCatalog, udfCatalog,
                                                   advanceChecks);
}
...
void SemanticQueryValidation::advanceSemanticQueryValidation(
  ...
  try {
    ...
    auto filterOperators =
        queryPlan->getOperatorByType<LogicalFilterOperator>();

    // Looping through all filter operators of the Query, checking for
    ...
    for (const auto &filterOp : filterOperators) {
      ...
      s << solver;
      NES_INFO("{}", s.str());
      // If the filter conditions are unsatisfiable, we report the one that
      // broke satisfiability
...
void SemanticQueryValidation::createExceptionForPredicate(
...
void SemanticQueryValidation::eraseAllSubStr(std::string &mainStr,
...
void SemanticQueryValidation::findAndReplaceAll(std::string &data,
    ...
    if (sourceDescriptor->instanceOf<LogicalSourceDescriptor>()) {
      ...
      if (!sourceCatalog->containsLogicalSource(sourceName)) {
        throw InvalidQueryException(
            "The logical source '" + sourceName +
            "' can not be found in the SourceCatalog\n");
      }
    }
  }
}
...
void SemanticQueryValidation::physicalSourceValidityCheck(
  ...
  if (!invalidLogicalSourceNames.empty()) {
    ...
    invalidSources << "]";
    throw InvalidQueryException(
        "Logical source(s) " + invalidSources.str() +
        " are found to have no physical source(s) defined.");
  }
}

void SemanticQueryValidation::sinkOperatorValidityCheck(
    const QueryPlanPtr &queryPlan) {
  auto rootOperators = queryPlan->getRootOperators();
  // Check if root operator exists ina query plan
  if (rootOperators.empty()) {
    throw InvalidQueryException("Query "s + queryPlan->toString() +
                                " does not contain any root operator");
  }
  ...
  for (auto &root : rootOperators) {
    if (!root->instanceOf<SinkLogicalOperator>()) {
      throw InvalidQueryException(
          "Query "s + queryPlan->toString() +
          " does not contain a valid sink operator as root");
    }
  }
}

void SemanticQueryValidation::inferModelValidityCheck(
    const QueryPlanPtr &queryPlan) {

  auto inferModelOperators =
      queryPlan->getOperatorByType<InferModel::LogicalInferModelOperator>();
  if (!inferModelOperators.empty()) {
    ...
    for (const auto &inferModelOperator : inferModelOperators) {
      for (const auto &inputField : inferModelOperator->getInputFields()) {
        ...
        if (!field->getStamp()->isNumeric() &&
            !field->getStamp()->isBoolean() && !field->getStamp()->isText()) {
          throw InvalidQueryException(
              "SemanticQueryValidation::advanceSemanticQueryValidation: "
              "Inputted data type for "
    ...
    NES_DEBUG("SemanticQueryValidation::advanceSemanticQueryValidation: Common "
              "stamp is: {}",
              commonStamp->toString());
    if (commonStamp->isUndefined()) {
      throw InvalidQueryException(
          "SemanticQueryValidation::advanceSemanticQueryValidation: Boolean "
