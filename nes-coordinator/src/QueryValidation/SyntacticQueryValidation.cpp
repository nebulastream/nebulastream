...
SyntacticQueryValidationPtr SyntacticQueryValidation::create(
    QueryParsingServicePtr queryParsingService) {
  return std::make_shared<SyntacticQueryValidation>(queryParsingService);
}

NES::QueryPlanPtr SyntacticQueryValidation::validate(
    const std::string& inputQuery) {
  NES::QueryPlanPtr queryPlan = nullptr;
  try {
    // Compiling the query string to an object
    // first check which ParsingService (C++,PSL,SQL) is required, than try to
    // create object
    if (inputQuery.starts_with("PATTERN")) {
      NES_DEBUG(
          "SyntacticQueryValidation: parse pattern query from declarative "
          "PSL.");
...
void SyntacticQueryValidation::handleException(const std::exception& ex) {
  // We only keep the meaningful part of the exception message for better
  // readability
  ...
  // If "error:" and "^" are present, we only keep the part of the message
