...
SyntacticQueryValidation::create(QueryParsingServicePtr queryParsingService) {
  return std::make_shared<SyntacticQueryValidation>(queryParsingService);
}

NES::QueryPlanPtr
SyntacticQueryValidation::validate(const std::string &inputQuery) {
  ...
  try {
    ...
    if (inputQuery.starts_with("PATTERN")) {
      NES_DEBUG("SyntacticQueryValidation: parse pattern query from "
                "declarative PSL.");
