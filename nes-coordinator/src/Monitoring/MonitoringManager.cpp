...
MonitoringManager::MonitoringManager(
    TopologyPtr topology, RequestHandlerServicePtr requestHandlerService, Catalogs::Query::QueryCatalogPtr queryCatalog)
...
MonitoringManager::MonitoringManager(
    ...
    : MonitoringManager(topology, requestHandlerService, queryCatalog, std::make_shared<LatestEntriesMetricStore>(), enableMonitoring)
{
}
    ...
    , monitoringCollectors(MonitoringPlan::defaultCollectors())
{
    ...
    NES_DEBUG(
        "MonitoringManager: Init with monitoring= {} , storage= {} ",
        enableMonitoring,
...
nlohmann::json MonitoringManager::requestRemoteMonitoringData(WorkerId nodeId)
{
    ...
    {
        NES_ERROR(
            "MonitoringManager: Requesting monitoring data for node {} failed. Monitoring is disabled, returning empty object", nodeId);
        return metricsJson;
    }
...
{
    NES_TRACE(
        "MonitoringManager: Adding metrics of type {} for node {}", std::string(magic_enum::enum_name(metrics->getMetricType())), nodeId);
...
MonitoringPlanPtr MonitoringManager::getMonitoringPlan(WorkerId nodeId)
{
    if (monitoringPlanMap.find(nodeId) == monitoringPlanMap.end())
    {
        ...
        NES_THROW_RUNTIME_ERROR(
            "MonitoringManager: Retrieving metrics for " + nodeId.toString() + " failed. Node does not exist in topology.");
    }
    else
    {
...
MetricStorePtr MonitoringManager::getMetricStore()
{
    return metricStore;
}
...
QueryId MonitoringManager::startOrRedeployMonitoringQuery(std::string monitoringStream, bool sync)
{
    ...
    if (success)
    {
        ...
        std::string metricCollectorStr = std::string(magic_enum::enum_name(MetricUtils::createCollectorTypeFromMetricType(metricType)));
        std::string query
