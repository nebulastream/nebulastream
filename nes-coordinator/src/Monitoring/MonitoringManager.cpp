...
MonitoringManager::MonitoringManager(
    ...
    : metricStore(metricStore),
      workerClient(WorkerRPCClient::create()),
      topology(topology),
...
bool MonitoringManager::registerRemoteMonitoringPlans(
  ...
  if (!monitoringPlan) {
    NES_ERROR(
        "MonitoringManager: Register monitoring plan failed, no plan is "
        "provided.");
    return false;
  }
  if (nodeIds.empty()) {
    NES_ERROR(
        "MonitoringManager: Register monitoring plan failed, no nodes are "
        "provided.");
    return false;
  }
  ...
  for (auto nodeId : nodeIds) {
    ...
    if (node) {
      ...
      if (success) {
        ...
        NES_ERROR(
            "MonitoringManager: Node with ID {} failed to register plan over "
            "GRPC.",
            nodeId);
        return false;
      }
...
nlohmann::json MonitoringManager::requestRemoteMonitoringData(WorkerId nodeId) {
  ...
  if (!enableMonitoring) {
    NES_ERROR(
        "MonitoringManager: Requesting monitoring data for node {} failed. "
        "Monitoring is disabled, returning empty object",
        nodeId);
    return metricsJson;
  }
  ...
  NES_THROW_RUNTIME_ERROR(
      "MonitoringManager: Error receiving monitoring metrics for node with "
      "id " +
      node->getId().toString());
}

StoredNodeMetricsPtr MonitoringManager::getMonitoringDataFromMetricStore(
    WorkerId node) {
...
MonitoringPlanPtr MonitoringManager::getMonitoringPlan(WorkerId nodeId) {
  if (monitoringPlanMap.find(nodeId) == monitoringPlanMap.end()) {
    ...
    if (node) {
      NES_DEBUG(
          "MonitoringManager: No registered plan found. Returning default plan "
          "for node {}",
          nodeId);
...
bool MonitoringManager::registerLogicalMonitoringStreams(
  ...
  NES_WARNING(
      "MonitoringManager: Monitoring is disabled, registering of logical "
      "monitoring streams not possible.");
  return false;
}

QueryId MonitoringManager::startOrRedeployMonitoringQuery(
    std::string monitoringStream, bool sync) {
  ...
  if (success) {
    ...
    NES_ERROR(
        "MonitoringManager: Failed to deploy monitoring query. Queries are "
        "still running and could not be stopped for {}",
        monitoringStream);
  }
  return queryId;
}
