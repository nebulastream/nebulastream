...
MonitoringManager::MonitoringManager(
    TopologyPtr topology, RequestHandlerServicePtr requestHandlerService,
    Catalogs::Query::QueryCatalogPtr queryCatalog)
    : MonitoringManager(std::move(topology), std::move(requestHandlerService),
                        std::move(queryCatalog), true) {}

MonitoringManager::MonitoringManager(
    TopologyPtr topology, RequestHandlerServicePtr requestHandlerService,
    Catalogs::Query::QueryCatalogPtr queryCatalog, bool enableMonitoring)
    : MonitoringManager(topology, requestHandlerService, queryCatalog,
                        std::make_shared<LatestEntriesMetricStore>(),
                        enableMonitoring) {}

MonitoringManager::MonitoringManager(
    TopologyPtr topology, RequestHandlerServicePtr requestHandlerService,
    Catalogs::Query::QueryCatalogPtr queryCatalog, MetricStorePtr metricStore,
    bool enableMonitoring)
    : metricStore(metricStore), workerClient(WorkerRPCClient::create()),
      topology(topology), enableMonitoring(enableMonitoring),
...
bool MonitoringManager::registerRemoteMonitoringPlans(
    const std::vector<WorkerId> &nodeIds, MonitoringPlanPtr monitoringPlan) {
  if (!enableMonitoring) {
    NES_ERROR(
        "MonitoringManager: Register plan failed. Monitoring is disabled.");
    return false;
  }
  if (!monitoringPlan) {
    NES_ERROR("MonitoringManager: Register monitoring plan failed, no plan is "
              "provided.");
    return false;
  }
  if (nodeIds.empty()) {
    NES_ERROR("MonitoringManager: Register monitoring plan failed, no nodes "
              "are provided.");
    return false;
  }
  ...
  for (auto nodeId : nodeIds) {
    NES_DEBUG(
        "MonitoringManager: Registering monitoring plan for worker id= {}",
        nodeId);
    ...
    if (node) {
      ...
      auto success =
          workerClient->registerMonitoringPlan(destAddress, monitoringPlan);

      if (success) {
        NES_DEBUG("MonitoringManager: Node with ID {} registered successfully.",
                  nodeId);
        monitoringPlanMap[nodeId] = monitoringPlan;
      } else {
        NES_ERROR("MonitoringManager: Node with ID {} failed to register plan "
                  "over GRPC.",
                  nodeId);
        return false;
      }
...
nlohmann::json MonitoringManager::requestRemoteMonitoringData(WorkerId nodeId) {
  ...
  if (!enableMonitoring) {
    NES_ERROR("MonitoringManager: Requesting monitoring data for node {} "
              "failed. Monitoring is disabled, returning empty object",
              nodeId);
    return metricsJson;
  }
  ...
  if (!metricsAsJsonString.empty()) {
    NES_DEBUG("MonitoringManager: Received monitoring data {}",
              metricsAsJsonString);
    // convert string to json object
    metricsJson = metricsJson.parse(metricsAsJsonString);
    return metricsJson;
  }
  NES_THROW_RUNTIME_ERROR("MonitoringManager: Error receiving monitoring "
                          "metrics for node with id " +
                          node->getId().toString());
}

StoredNodeMetricsPtr
MonitoringManager::getMonitoringDataFromMetricStore(WorkerId node) {
...
MonitoringPlanPtr MonitoringManager::getMonitoringPlan(WorkerId nodeId) {
  if (monitoringPlanMap.find(nodeId) == monitoringPlanMap.end()) {
    TopologyNodePtr node = topology->getCopyOfTopologyNodeWithId(nodeId);
    if (node) {
      NES_DEBUG("MonitoringManager: No registered plan found. Returning "
                "default plan for node {}",
                nodeId);
      return MonitoringPlan::defaultPlan();
    }
    NES_THROW_RUNTIME_ERROR("MonitoringManager: Retrieving metrics for " +
                            nodeId.toString() +
...
bool MonitoringManager::registerLogicalMonitoringStreams(
    const NES::Configurations::CoordinatorConfigurationPtr config) {
  if (enableMonitoring) {
    for (auto collectorType : monitoringCollectors) {
      auto metricSchema =
          MetricUtils::getSchemaFromCollectorType(collectorType);
      // auto generate the specifics
      MetricType metricType =
          MetricUtils::createMetricFromCollectorType(collectorType)
              ->getMetricType();
      std::string logicalSourceName =
          std::string(magic_enum::enum_name(metricType));
      logicalMonitoringSources.insert(logicalSourceName);
      NES_INFO("MonitoringManager: Creating logical source {}",
               logicalSourceName);
      config->logicalSourceTypes.add(
          LogicalSourceType::create(logicalSourceName, metricSchema));
    }
    return true;
  }
  NES_WARNING("MonitoringManager: Monitoring is disabled, registering of "
              "logical monitoring streams not possible.");
  return false;
}

QueryId
MonitoringManager::startOrRedeployMonitoringQuery(std::string monitoringStream,
                                                  bool sync) {
  QueryId queryId = INVALID_QUERY_ID;

  if (!enableMonitoring) {
    NES_ERROR(
        "MonitoringManager: Deploying queries failed. Monitoring is disabled.");
  }
  ...
  if (success) {
    MetricType metricType =
        magic_enum::enum_cast<MetricType>(monitoringStream).value();
    ...
    query = std::regex_replace(query, std::regex("%COLLECTOR%"),
                               metricCollectorStr);
    ...
    queryId = requestHandlerService->validateAndQueueAddQueryRequest(
        query, Optimizer::PlacementStrategy::BottomUp);
    if ((sync && waitForQueryToStart(queryId, std::chrono::seconds(60))) ||
        (!sync)) {
      NES_INFO("MonitoringManager: Successfully started query {}::{}", queryId,
               monitoringStream);
      deployedMonitoringQueries.insert({monitoringStream, queryId});
    } else {
      NES_ERROR("MonitoringManager: Query {} :: {} failed to start in time.",
                queryId, monitoringStream);
    }
  } else {
    NES_ERROR("MonitoringManager: Failed to deploy monitoring query. Queries "
              "are still running and could not be stopped for {}",
              monitoringStream);
  }
  return queryId;
}
...
MonitoringManager::startOrRedeployMonitoringQueries(bool sync) {
  if (!enableMonitoring) {
    NES_ERROR(
        "MonitoringManager: Deploying queries failed. Monitoring is disabled.");
...
bool MonitoringManager::stopRunningMonitoringQuery(std::string streamName,
                                                   bool sync) {
  bool success = true;
  ...
  if (deployedMonitoringQueries.contains(streamName)) {
    ...
    NES_INFO("MonitoringManager: Stopping query {} for {}", queryId,
             metricType);
    if (requestHandlerService->validateAndQueueStopQueryRequest(queryId)) {
      if ((sync && checkStoppedOrTimeout(queryId, std::chrono::seconds(60))) ||
          (!sync)) {
        NES_INFO("MonitoringManager: Query {}::{} terminated", queryId,
                 metricType);
      } else {
        NES_ERROR("MonitoringManager: Failed to stop query {}::{}", queryId,
                  metricType);
        success = false;
      }
    } else {
      NES_ERROR("MonitoringManager: Failed to validate query {}::{}", queryId,
                metricType);
      success = false;
    }
  }
  if (success) {
    ...
    NES_INFO("MonitoringManager: Monitoring query stopped successfully {}",
             streamName);
  }
  return success;
}
...
bool MonitoringManager::checkStoppedOrTimeout(QueryId queryId,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    if (queryState == QueryState::STOPPED) {
      NES_TRACE("checkStoppedOrTimeout: status for {} reached stopped",
                queryId);
      return true;
    }
    NES_DEBUG(
        "checkStoppedOrTimeout: status not reached for {} as status is={}",
        queryId, magic_enum::enum_name(queryState));
    std::this_thread::sleep_for(std::chrono::seconds(10));
  }
  NES_TRACE(
      "checkStoppedOrTimeout: expected status not reached within set timeout");
  return false;
}
...
bool MonitoringManager::waitForQueryToStart(QueryId queryId,
                                            std::chrono::seconds timeout) {
  NES_INFO(
      "MonitoringManager: wait till the query {} gets into Running status.",
      queryId);
  auto start_timestamp = std::chrono::system_clock::now();

  while (std::chrono::system_clock::now() < start_timestamp + timeout) {
    auto queryState = queryCatalog->getQueryState(queryId);
    NES_TRACE("MonitoringManager: Query {} is now in status {}", queryId,
...
bool MonitoringManager::isMonitoringStream(std::string streamName) const {
  return logicalMonitoringSources.contains(streamName);
}

const std::unordered_map<std::string, QueryId> &
MonitoringManager::getDeployedMonitoringQueries() const {
  return deployedMonitoringQueries;
}
