...
{
public:
    SampleCPPCodeGenerator(QueryCompilerOptionsPtr const& options, Phases::PhaseFactoryPtr const& phaseFactory, bool sourceSharing)
    ...
    QueryCompilationResultPtr compileQuery(QueryCompilation::QueryCompilationRequestPtr request) override
    {
        ...
        try
        {
            ...
            {
                if (pipeline->getDecomposedQueryPlan()->getRootOperators()[0]->instanceOf<QueryCompilation::ExecutableOperator>())
                {
...
QueryPlanPtr SampleCodeGenerationPhase::execute(const QueryPlanPtr& queryPlan)
{
    ...
    auto decomposedQueryPlan = DecomposedQueryPlan::create(
        UNSURE_CONVERSION_TODO_4761(queryPlan->getQueryId(), DecomposedQueryPlanId),
