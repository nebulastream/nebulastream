...
void DeploymentPhase::registerOrStopDecomposedQueryPlan(
    const std::set<Optimizer::DeploymentContextPtr>& deploymentContexts, RequestType requestType)
{
        ...
        switch (decomposedQueryPlanState)
        {
            ...
            case QueryState::MARKED_FOR_REDEPLOYMENT: {
                ...
                queryCatalog->updateDecomposedQueryPlanStatus(
                    sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, decomposedQueryPlanState, workerId);
            ...
            case QueryState::MARKED_FOR_MIGRATION: {
                ...
                else if (requestType == RequestType::StopQuery)
                {
                    ...
                    // Update decomposed query plan status
                    queryCatalog->updateDecomposedQueryPlanStatus(
                        sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, QueryState::MARKED_FOR_HARD_STOP, workerId);
                ...
                else if (requestType == RequestType::FailQuery)
                {
                    ...
                    // Update decomposed query plan status
                    queryCatalog->updateDecomposedQueryPlanStatus(
                        sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, QueryState::MARKED_FOR_FAILURE, workerId);
                    asyncRequests.emplace_back(RpcAsyncRequest{queueForDeploymentContext, RpcClientMode::Stop});
                }
                else
                {
                    NES_ERROR(
                        "Unhandled request type {} for decomposed query plan in status MARKED_FOR_MIGRATION",
                        magic_enum::enum_name(requestType));
                }
                break;
            }
            default: {
                NES_WARNING("Can not handle decomposed query plan in the state {}", magic_enum::enum_name(decomposedQueryPlanState));
            }
        }
    }
    ...
    for (const auto& deploymentContext : deploymentContexts)
    {
        ...
        switch (decomposedQueryPlanState)
        {
            ...
            case QueryState::MARKED_FOR_REDEPLOYMENT: {
                ...
                // Update decomposed query plan status
                queryCatalog->updateDecomposedQueryPlanStatus(
                    sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, QueryState::RUNNING, workerId);
            ...
            case QueryState::MARKED_FOR_MIGRATION: {
                if (requestType == RequestType::AddQuery)
                {
                    // Update decomposed query plan status
                    queryCatalog->updateDecomposedQueryPlanStatus(
                        sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, QueryState::MIGRATING, workerId);
                }
                else if (requestType == RequestType::StopQuery)
                {
                    ...
                    // Update decomposed query plan status
                    queryCatalog->updateDecomposedQueryPlanStatus(
                        sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, QueryState::STOPPED, workerId);
                ...
                else if (requestType == RequestType::FailQuery)
                {
                    ...
                    // Update decomposed query plan status
                    queryCatalog->updateDecomposedQueryPlanStatus(
                        sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion, QueryState::FAILED, workerId);
                    asyncRequests.emplace_back(RpcAsyncRequest{queueForDeploymentContext, RpcClientMode::Unregister});
                }
                else
                {
