...
DeploymentPhasePtr
DeploymentPhase::create(const Catalogs::Query::QueryCatalogPtr &queryCatalog) {
...
void DeploymentPhase::execute(
    const std::set<Optimizer::DeploymentContextPtr> &deploymentContexts,
    RequestType requestType) {

  NES_INFO("Register or unregister {} decomposed queries.",
...
void DeploymentPhase::registerOrStopDecomposedQueryPlan(
    const std::set<Optimizer::DeploymentContextPtr> &deploymentContexts,
    RequestType requestType) {
  ...
  for (const auto &deploymentContext : deploymentContexts) {
    ...
    case QueryState::MARKED_FOR_DEPLOYMENT: {
      if (sharedQueryState != QueryState::FAILED &&
    ...
    case QueryState::MARKED_FOR_REDEPLOYMENT: {
      ...
      queryCatalog->updateDecomposedQueryPlanStatus(
          sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion,
          decomposedQueryPlanState, workerId);
      asyncRequests.emplace_back(
          RpcAsyncRequest{queueForDeploymentContext, RpcClientMode::Register});
      break;
    }
    case QueryState::MARKED_FOR_MIGRATION: {
      if (requestType == RequestType::AddQuery) {
        ...
        sharedQueryState = QueryState::STOPPED;
        workerRPCClient->stopQueryAsync(
            grpcAddress, sharedQueryId, decomposedQueryPlanId,
            Runtime::QueryTerminationType::HardStop, queueForDeploymentContext);
        // Update decomposed query plan status
        queryCatalog->updateDecomposedQueryPlanStatus(
            sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion,
            QueryState::MARKED_FOR_HARD_STOP, workerId);
        asyncRequests.emplace_back(
            RpcAsyncRequest{queueForDeploymentContext, RpcClientMode::Stop});
      } else if (requestType == RequestType::FailQuery) {
        sharedQueryState = QueryState::FAILED;
        // Fail the decomposed query plan
        workerRPCClient->stopQueryAsync(
            grpcAddress, sharedQueryId, decomposedQueryPlanId,
            Runtime::QueryTerminationType::Failure, queueForDeploymentContext);
        // Update decomposed query plan status
        queryCatalog->updateDecomposedQueryPlanStatus(
            sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion,
            QueryState::MARKED_FOR_FAILURE, workerId);
        asyncRequests.emplace_back(
            RpcAsyncRequest{queueForDeploymentContext, RpcClientMode::Stop});
      } else {
        NES_ERROR("Unhandled request type {} for decomposed query plan in "
...
void DeploymentPhase::startOrUnregisterDecomposedQueryPlan(
    const std::set<Optimizer::DeploymentContextPtr> &deploymentContexts,
  ...
  for (const auto &deploymentContext : deploymentContexts) {
    ...
    case QueryState::MARKED_FOR_DEPLOYMENT:
    case QueryState::MARKED_FOR_REDEPLOYMENT: {
      workerRPCClient->startQueryAsync(grpcAddress, sharedQueryId,
                                       decomposedQueryPlanId,
                                       queueForDeploymentContext);
      // Update decomposed query plan status
      queryCatalog->updateDecomposedQueryPlanStatus(
          sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion,
          QueryState::RUNNING, workerId);
      asyncRequests.emplace_back(
          RpcAsyncRequest{queueForDeploymentContext, RpcClientMode::Start});
      break;
    }
    case QueryState::MARKED_FOR_MIGRATION: {
      if (requestType == RequestType::AddQuery) {
        ...
        queryCatalog->updateDecomposedQueryPlanStatus(
            sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion,
            QueryState::MIGRATING, workerId);
      } else if (requestType == RequestType::StopQuery) {
        // Unregister the decomposed query plan
        workerRPCClient->unregisterQueryAsync(grpcAddress, sharedQueryId,
                                              queueForDeploymentContext);
        // Update decomposed query plan status
        queryCatalog->updateDecomposedQueryPlanStatus(
            sharedQueryId, decomposedQueryPlanId, decomposedQueryPlanVersion,
            QueryState::STOPPED, workerId);
        asyncRequests.emplace_back(RpcAsyncRequest{queueForDeploymentContext,
                                                   RpcClientMode::Unregister});
      } else if (requestType == RequestType::FailQuery) {
        // Unregister the decomposed query plan
        workerRPCClient->unregisterQueryAsync(grpcAddress, sharedQueryId,
                                              queueForDeploymentContext);
