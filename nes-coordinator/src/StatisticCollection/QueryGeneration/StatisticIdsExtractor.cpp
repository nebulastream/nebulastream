...
std::vector<StatisticId> StatisticIdsExtractor::extractStatisticIdsFromQueryId(
  ...
  // Waiting for the query to be in the RUNNING state before extracting the
  // statistic ids
  ...
  while (std::chrono::system_clock::now() < endTimestamp) {
    ...
    NES_TRACE("Query {} is now in status {}", queryId,
    ...
    switch (queryState) {
      ...
      case QueryState::RUNNING: {
        // We search for the queryPlan in the queryCatalog and then extract the
        // statisticIds from the queryPlan
        const auto queryPlanCopy =
            queryCatalog->getCopyOfExecutedQueryPlan(queryId);
      ...
      default: {
        NES_WARNING("Expected: Running but found {}",
                    magic_enum::enum_name(queryState));
        break;
      }
    }
  ...
  NES_ERROR("Timeout while waiting for query {} to be in RUNNING state",
            queryId);
  return {};
}

std::vector<StatisticId>
StatisticIdsExtractor::extractStatisticIdsFromQueryPlan(
    const QueryPlan& queryPlan) {
  std::vector<StatisticId> extractedStatisticIds;
  const auto allStatisticBuildOperator =
      queryPlan.getOperatorByType<LogicalStatisticWindowOperator>();

  /* We iterator over all LogicalStatisticWindowOperator in the queryPlan and
   * store the statistic id of the operator before in extractedStatisticIds
   */
  for (const auto& statisticBuildOperator : allStatisticBuildOperator) {
    ...
    /* We check if the operator before is a WatermarkAssignerLogicalOperator and
     * if so, we have to add the statistic ids of all of its children. Due to
     * the fact that we might add a watermark assigner to each logical query
     * plan, if it contains a window operator, i.e.,
    ...
    for (const auto& childOp : operatorsToTrack) {
      if (childOp->instanceOf<const WatermarkAssignerLogicalOperator>()) {
        ...
        // If the operator before is not a WatermarkAssignerLogicalOperator, we
        // add the statistic id of the operator
        extractedStatisticIds.emplace_back(
            childOp->template as<Operator>()->getStatisticId());
      }
    }
  }
