...
void NesCEPQueryPlanCreator::enterListEvents(
  ...
  NES_DEBUG(
      "NesCEPQueryPlanCreator : enterListEvents: init tree walk and initialize "
      "read out of AST ");
  this->nodeId++;
...
void NesCEPQueryPlanCreator::exitOperatorRule(
  ...
  NES_DEBUG(
      "NesCEPQueryPlanCreator : exitOperatorRule: create a node for the "
      "operator  {}",
      context->getText());
...
void NesCEPQueryPlanCreator::exitInputStream(
  ...
  NES_DEBUG(
      "NesCEPQueryPlanCreator : exitInputStream: replace alias with streamName "
      " {}",
      context->getText());
...
void NesCEPQueryPlanCreator::enterQuantifiers(
  ...
  if (context->LBRACKET()) {    // context contains []
    if (context->D_POINTS()) {  // e.g., A[2:10] means that we expect at least 2
      ...
      NES_DEBUG(
          "NesCEPQueryPlanCreator : enterQuantifiers: Times with Min: {} and "
          "Max {}",
...
void NesCEPQueryPlanCreator::exitBinaryComparisonPredicate(
  ...
  NES_DEBUG(
      "NesCEPQueryPlanCreator: exitBinaryComparisonPredicate: add filters {} "
      "{} {}",
...
QueryPlanPtr NesCEPQueryPlanCreator::createQueryFromPatternList() const {
  ...
  NES_DEBUG(
      "NesCEPQueryPlanCreator: createQueryFromPatternList: create query from "
      "AST elements")
  ...
  if (this->pattern.getOperatorList().empty() &&
    ...
    for (auto operatorNode = pattern.getOperatorList().begin();
      ...
      if (operatorName == "OR" || operatorName == "SEQ" ||
        ...
        NES_DEBUG(
            "NesCEPQueryPlanCreater: createQueryFromPatternList: add unary "
            "operator {}",
            operatorName)
        auto sourceName =
...
QueryPlanPtr NesCEPQueryPlanCreator::addProjections(
...
QueryPlanPtr NesCEPQueryPlanCreator::addBinaryOperatorToQueryPlan(
  ...
  NES_DEBUG(
      "NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: add binary "
      "operator {}",
      operaterName)
  // find left (query) and right branch (subquery) of binary operator
  // left query plan
  NES_DEBUG(
      "NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: create "
      "subqueryLeft from {}",
  ...
  if (!queryPlan || (queryPlan->getSourceConsumed().find(leftSourceName) ==
                     ...
                     queryPlan->getSourceConsumed().find(rightSourceName) ==
                         std::string::npos)) {
    NES_DEBUG(
        "NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: both sources "
        "are not in the current queryPlan")
    ...
    NES_DEBUG(
        "NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: check for the "
        "right branch")
  ...
  if (operaterName == "OR") {
    NES_DEBUG(
        "NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: "
        "addUnionOperator")
    ...
    if (timeMeasurements.first.getTime() != TimeMeasure(0).getTime()) {
      ...
      NES_DEBUG(
          "NesCEPQueryPlanCreater: add name cepLeftKey {} and name cepRightKey "
          "{}",
      ...
      if (operaterName == "SEQ") {
        ...
        NES_DEBUG(
            "NesCEPQueryPlanCreater: ExpressionItem for Left Source {} and "
            "ExpressionItem for Right Source {}",
