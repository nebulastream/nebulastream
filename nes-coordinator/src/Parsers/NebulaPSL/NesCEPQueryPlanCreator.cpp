...
void NesCEPQueryPlanCreator::enterListEvents(
    NesCEPParser::ListEventsContext *context) {
  NES_DEBUG("NesCEPQueryPlanCreator : enterListEvents: init tree walk and "
            "initialize read out of AST ");
  this->nodeId++;
  NesCEPBaseListener::enterListEvents(context);
}

void NesCEPQueryPlanCreator::enterEventElem(
    NesCEPParser::EventElemContext *cxt) {
  NES_DEBUG("NesCEPQueryPlanCreator : exitEventElem: found a stream source  {}",
  ...
  sourceCounter++;
  NES_DEBUG("NesCEPQueryPlanCreator : exitEventElem: inserted {} to sources",
            cxt->getStart()->getText());
  this->nodeId++;
  NesCEPBaseListener::exitEventElem(cxt);
}

void NesCEPQueryPlanCreator::exitOperatorRule(
    NesCEPParser::OperatorRuleContext *context) {
  NES_DEBUG("NesCEPQueryPlanCreator : exitOperatorRule: create a node for the "
            "operator  {}",
            context->getText());
...
void NesCEPQueryPlanCreator::exitInputStream(
    NesCEPParser::InputStreamContext *context) {
  NES_DEBUG("NesCEPQueryPlanCreator : exitInputStream: replace alias with "
            "streamName  {}",
            context->getText());
  ...
  for (sourceMapItem = sources.begin(); sourceMapItem != sources.end();
       sourceMapItem++) {
...
void NesCEPQueryPlanCreator::enterWhereExp(
    NesCEPParser::WhereExpContext *context) {
  inWhere = true;
  NesCEPBaseListener::enterWhereExp(context);
}

void NesCEPQueryPlanCreator::exitWhereExp(
    NesCEPParser::WhereExpContext *context) {
  inWhere = false;
...
void NesCEPQueryPlanCreator::enterOutAttribute(
    NesCEPParser::OutAttributeContext *context) {
  // get projection fields
  auto attributeField =
      NES::Attribute(context->NAME()->getText()).getExpressionNode();
...
void NesCEPQueryPlanCreator::enterSink(NesCEPParser::SinkContext *context) {
  ...
  if (sinkType == "File") {
    sinkDescriptor =
        NES::FileSinkDescriptor::create(context->NAME()->getText());
  }
  if (sinkType == "MQTT") {
...
void NesCEPQueryPlanCreator::exitSinkList(
    NesCEPParser::SinkListContext *context) {
  NesCEPBaseListener::exitSinkList(context);
}

void NesCEPQueryPlanCreator::enterQuantifiers(
    NesCEPParser::QuantifiersContext *context) {
  ...
  if (context->LBRACKET()) {   // context contains []
    if (context->D_POINTS()) { // e.g., A[2:10] means that we expect at least 2
                               // and maximal 10 occurrences of A
      NES_DEBUG("NesCEPQueryPlanCreator : enterQuantifiers: Times with Min: {} "
                "and Max {}",
                context->iterMin()->INT()->getText(),
                context->iterMin()->INT()->getText());
      timeOperator.setMinMax(
          std::make_pair(stoi(context->iterMin()->INT()->getText()),
                         stoi(context->iterMax()->INT()->getText())));
    } else { // e.g., A[2] means that we except exact 2 occurrences of A
      timeOperator.setMinMax(std::make_pair(stoi(context->INT()->getText()),
                                            stoi(context->INT()->getText())));
    }
  } else if (context->PLUS()) { // e.g., A+, means a occurs at least once
    timeOperator.setMinMax(std::make_pair(0, 0));
  } else if (context->STAR()) { //[]*   //TODO unbounded iteration variant not
                                //yet implemented #866
    NES_THROW_RUNTIME_ERROR(
        "NesCEPQueryPlanCreator : enterQuantifiers: NES currently does not "
        "support the iteration variant *");
  }
...
void NesCEPQueryPlanCreator::exitBinaryComparisonPredicate(
    NesCEPParser::BinaryComparasionPredicateContext *context) {
  // retrieve the ExpressionNode for the filter and save it in the pattern
  // expressionList
  std::string comparisonOperator = context->comparisonOperator()->getText();
  auto leftExpressionNode =
      NES::Attribute(this->currentLeftExp).getExpressionNode();
  auto rightExpressionNode =
      NES::Attribute(this->currentRightExp).getExpressionNode();
  NES::ExpressionNodePtr expression;
  NES_DEBUG("NesCEPQueryPlanCreator: exitBinaryComparisonPredicate: add "
            "filters {} {} {}",
            this->currentLeftExp, comparisonOperator, this->currentRightExp)

  if (comparisonOperator == "<") {
    expression = NES::LessExpressionNode::create(leftExpressionNode,
                                                 rightExpressionNode);
  }
  if (comparisonOperator == "<=") {
    expression = NES::LessEqualsExpressionNode::create(leftExpressionNode,
                                                       rightExpressionNode);
  }
  if (comparisonOperator == ">") {
    expression = NES::GreaterExpressionNode::create(leftExpressionNode,
                                                    rightExpressionNode);
  }
  if (comparisonOperator == ">=") {
    expression = NES::GreaterEqualsExpressionNode::create(leftExpressionNode,
                                                          rightExpressionNode);
  }
  if (comparisonOperator == "==") {
    expression = NES::EqualsExpressionNode::create(leftExpressionNode,
                                                   rightExpressionNode);
  }
  if (comparisonOperator == "&&") {
    expression =
        NES::AndExpressionNode::create(leftExpressionNode, rightExpressionNode);
  }
  if (comparisonOperator == "||") {
    expression =
        NES::OrExpressionNode::create(leftExpressionNode, rightExpressionNode);
  }
  this->pattern.addExpression(expression);
}

void NesCEPQueryPlanCreator::enterAttribute(
    NesCEPParser::AttributeContext *cxt) {
...
QueryPlanPtr NesCEPQueryPlanCreator::createQueryFromPatternList() const {
  if (this->pattern.getOperatorList().empty() &&
      this->pattern.getSources().size() == 0) {
    NES_THROW_RUNTIME_ERROR(
        "NesCEPQueryPlanCreator: createQueryFromPatternList: Received an empty "
        "pattern");
  }
  NES_DEBUG("NesCEPQueryPlanCreator: createQueryFromPatternList: create query "
            "from AST elements")
  QueryPlanPtr queryPlan;
  // if for simple patterns without binary CEP operators
  if (this->pattern.getOperatorList().empty() &&
      this->pattern.getSources().size() == 1) {
    ...
    for (auto operatorNode = pattern.getOperatorList().begin();
         operatorNode != pattern.getOperatorList().end(); ++operatorNode) {
      auto operatorName = operatorNode->second.getOperatorName();
      // add binary operators
      if (operatorName == "OR" || operatorName == "SEQ" ||
          operatorName == "AND") {
        queryPlan =
            addBinaryOperatorToQueryPlan(operatorName, operatorNode, queryPlan);
      } else if (operatorName == "TIMES") { // add times operator
        NES_DEBUG("NesCEPQueryPlanCreater: createQueryFromPatternList: add "
                  "unary operator {}",
                  operatorName)
        auto sourceName =
        ...
        queryPlan = QueryPlanBuilder::createQueryPlan(sourceName);
        NES_DEBUG(
            "NesCEPQueryPlanCreater: createQueryFromPatternList: add times "
            "operator{}",
            pattern.getSources().at(operatorNode->second.getLeftChildId()))
        ...
        // create window
        auto timeMeasurements = transformWindowToTimeMeasurements(
            pattern.getWindow().first, pattern.getWindow().second);
        if (timeMeasurements.first.getTime() != TimeMeasure(0).getTime()) {
          auto windowType = Windowing::SlidingWindow::of(
              EventTime(Attribute("timestamp")), timeMeasurements.first,
              timeMeasurements.second);
          // check and add watermark
          queryPlan = QueryPlanBuilder::checkAndAddWatermarkAssignment(
              queryPlan, windowType);

          std::vector<WindowAggregationDescriptorPtr> windowAggs;
          auto sumAgg = API::Sum(Attribute("Count"))->aggregation;

          auto timeField = windowType->as<Windowing::TimeBasedWindowType>()
          ...
          if (min != 0 &&
              max != 0) { // TODO unbounded iteration variant not yet
                          // implemented #866 (min = 1/0 max = 0)
            ...
            if (min == max) {
              ...
              predicate =
                  Attribute("Count") >= min && Attribute("Count") <= max;
            }
            queryPlan = QueryPlanBuilder::addFilter(predicate, queryPlan);
          }
        } else {
          NES_THROW_RUNTIME_ERROR(
              "NesCEPQueryPlanCreator: createQueryFromPatternList: The "
              "Iteration operator requires a time window.");
        }
      } else {
        NES_THROW_RUNTIME_ERROR(
            "NesCEPQueryPlanCreator: createQueryFromPatternList: Unkown CEP "
            "operator"
            << operatorName);
      }
    }
...
QueryPlanPtr NesCEPQueryPlanCreator::addFilters(QueryPlanPtr queryPlan) const {
  for (auto it = pattern.getExpressions().begin();
       it != pattern.getExpressions().end(); ++it) {
    queryPlan = QueryPlanBuilder::addFilter(*it, queryPlan);
  }
  return queryPlan;
}

std::pair<TimeMeasure, TimeMeasure>
NesCEPQueryPlanCreator::transformWindowToTimeMeasurements(
    std::string timeMeasure, int32_t timeValue) const {
  ...
  if (timeMeasure == "MILLISECOND") {
    ...
    NES_THROW_RUNTIME_ERROR(
        "NesCEPQueryPlanCreator: transformWindowToTimeMeasurements: Unkown "
        "time measure " +
        timeMeasure);
  }
...
QueryPlanPtr
NesCEPQueryPlanCreator::addProjections(QueryPlanPtr queryPlan) const {
  return QueryPlanBuilder::addProjection(pattern.getProjectionFields(),
                                         queryPlan);
}
...
QueryPlanPtr NesCEPQueryPlanCreator::getQueryPlan() const {
  try {
    ...
    NES_THROW_RUNTIME_ERROR(
        "NesCEPQueryPlanCreator::getQueryPlan(): Was not able to parse query: "
        << e.what());
  }
}
...
QueryPlanPtr NesCEPQueryPlanCreator::addBinaryOperatorToQueryPlan(
  ...
  NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: add binary "
            "operator {}",
            operaterName)
  // find left (query) and right branch (subquery) of binary operator
  // left query plan
  NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: create "
            "subqueryLeft from {}",
  ...
  if (!queryPlan || (queryPlan->getSourceConsumed().find(leftSourceName) ==
                         std::string::npos &&
                     queryPlan->getSourceConsumed().find(rightSourceName) ==
                         std::string::npos)) {
    NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: both "
              "sources are not in the current queryPlan")
    ...
    NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: check for "
              "the right branch")
    leftQueryPlan = queryPlan;
    rightQueryPlan = checkIfSourceIsAlreadyConsumedSource(
        leftSourceName, rightSourceName, queryPlan);
  }

  if (operaterName == "OR") {
    NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: "
              "addUnionOperator")
    leftQueryPlan = QueryPlanBuilder::addUnion(leftQueryPlan, rightQueryPlan);
  } else {
    // Seq and And require a window, so first we create and check the window
    // operator
    auto timeMeasurements = transformWindowToTimeMeasurements(
        pattern.getWindow().first, pattern.getWindow().second);
    if (timeMeasurements.first.getTime() != TimeMeasure(0).getTime()) {
      ...
      // Next, we add artificial key attributes to the sources in order to reuse
      // the join-logic later
      std::string cepLeftKey = keyAssignment("cep_leftKey");
      std::string cepRightKey = keyAssignment("cep_rightKey");

      // next: add Map operator that maps the attributes with value 1 to the
      // left and right source
      leftQueryPlan =
          QueryPlanBuilder::addMap(Attribute(cepLeftKey) = 1, leftQueryPlan);
      rightQueryPlan =
          QueryPlanBuilder::addMap(Attribute(cepRightKey) = 1, rightQueryPlan);

      // then, define the artificial attributes as key attributes
      NES_DEBUG("NesCEPQueryPlanCreater: add name cepLeftKey {} and name "
                "cepRightKey {}",
                cepLeftKey, cepRightKey);
      ExpressionItem onLeftKey =
          ExpressionItem(Attribute(cepLeftKey)).getExpressionNode();
      ExpressionItem onRightKey =
          ExpressionItem(Attribute(cepRightKey)).getExpressionNode();
      auto joinExpression = onLeftKey == onRightKey;
      auto leftKeyFieldAccess =
          onLeftKey.getExpressionNode()->as<FieldAccessExpressionNode>();
      auto rightKeyFieldAccess =
          onRightKey.getExpressionNode()->as<FieldAccessExpressionNode>();

      leftQueryPlan = QueryPlanBuilder::addJoin(
          leftQueryPlan, rightQueryPlan, joinExpression, windowType,
      ...
      if (operaterName == "SEQ") {
        ...
        // identify the correct source and its timestamp in case of composed
        // streams on the right branch
        ...
        if (sourceNameRight.find("_") != std::string::npos) {
          ...
          sourceNameRight = sourceNameRight.substr(posStart + 1, posEnd - 2) +
                            "$" + timestamp;
        ...
        if (sourceNameLeft.find("_") != std::string::npos) {
          ...
          sourceNameLeft =
              sourceNameLeft.substr(posStart + 1) + "$" + timestamp;
        ...
        NES_DEBUG("NesCEPQueryPlanCreater: ExpressionItem for Left Source {} "
                  "and ExpressionItem for Right Source {}",
                  sourceNameLeft, sourceNameRight);
        // create filter expression and add it to queryPlan
        leftQueryPlan = QueryPlanBuilder::addFilter(
            Attribute(sourceNameLeft) < Attribute(sourceNameRight),
            leftQueryPlan);
      }
    } else {
      NES_THROW_RUNTIME_ERROR(
          "NesCEPQueryPlanCreater: createQueryFromPatternList: Cannot create " +
          operaterName + "without a window.");
    }
  }
  return leftQueryPlan;
}

QueryPlanPtr NesCEPQueryPlanCreator::checkIfSourceIsAlreadyConsumedSource(
  ...
  if (queryPlan->getSourceConsumed().find(leftSourceName) !=
          std::string::npos &&
      queryPlan->getSourceConsumed().find(rightSourceName) !=
          std::string::npos) {
    NES_THROW_RUNTIME_ERROR(
        "NesCEPQueryPlanCreater: checkIfSourceIsAlreadyConsumedSource: Both "
        "sources are already consumed "
        "and combined with a binary operator");
  } else if (queryPlan->getSourceConsumed().find(leftSourceName) ==
             std::string::npos) {
    // right queryplan
    NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: create "
              "subqueryRight from {}",
  ...
  } else if (queryPlan->getSourceConsumed().find(rightSourceName) ==
             std::string::npos) {
    // right queryplan
    NES_DEBUG("NesCEPQueryPlanCreater: addBinaryOperatorToQueryPlan: create "
              "subqueryRight from {}",
