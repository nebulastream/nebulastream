...
extern void Exceptions::installGlobalErrorListener(
    std::shared_ptr<ErrorListener> const&);

NesCoordinator::NesCoordinator(
    CoordinatorConfigurationPtr coordinatorConfiguration)
    : coordinatorConfiguration(std::move(coordinatorConfiguration)),
      restIp(this->coordinatorConfiguration->restIp),
      restPort(this->coordinatorConfiguration->restPort),
      rpcIp(this->coordinatorConfiguration->coordinatorHost),
      rpcPort(this->coordinatorConfiguration->rpcPort),
      enableMonitoring(this->coordinatorConfiguration->enableMonitoring) {
  NES_DEBUG("NesCoordinator() restIp={} restPort={} rpcIp={} rpcPort={}",
            restIp, restPort, rpcIp, rpcPort);
  setThreadName("NesCoordinator");

  // TODO make compiler backend configurable
  auto cppCompiler = Compiler::CPPCompiler::create();
  auto jitCompiler = Compiler::JITCompilerBuilder()
  ...
  placementAmendmentQueue = std::make_shared<
  ...
  coordinatorHealthCheckService =
      std::make_shared<CoordinatorHealthCheckService>(
          topology, HEALTH_SERVICE_NAME, this->coordinatorConfiguration);
  ...
  // For now, we hardcode the usage of the DefaultStatisticQueryGenerator, see
  // issue #4687
  ...
  auto asyncRequestExecutor =
      std::make_shared<RequestProcessor::AsyncRequestProcessor>(
          storageDataStructures);
  requestHandlerService = std::make_shared<RequestHandlerService>(
      this->coordinatorConfiguration->optimizer, queryParsingService,
      queryCatalog, sourceCatalog, udfCatalog, asyncRequestExecutor, z3Context,
      Statistic::DefaultStatisticQueryGenerator::create(), statisticRegistry);

  udfCatalog = Catalogs::UDF::UDFCatalog::create();

  monitoringService = std::make_shared<MonitoringService>(
      topology, requestHandlerService, queryCatalog, enableMonitoring);
  monitoringService->getMonitoringManager()->registerLogicalMonitoringStreams(
      this->coordinatorConfiguration);
}
...
Statistic::StatisticProbeHandlerPtr NesCoordinator::getStatisticProbeHandler()
    const {
  return statisticProbeHandler;
}

Runtime::NodeEnginePtr NesCoordinator::getNodeEngine() {
...
uint64_t NesCoordinator::startCoordinator(bool blocking) {
  ...
  std::shared_ptr<std::promise<bool>> promRPC =
  ...
  for (const auto& logicalSourceType :
       coordinatorConfiguration->logicalSourceTypes.getValues()) {
    auto schema = Schema::createFromSchemaType(
        logicalSourceType.getValue()->getSchemaType());
    sourceCatalogService->registerLogicalSource(
        logicalSourceType.getValue()->getLogicalSourceName(), schema);
  }
  NES_DEBUG("NesCoordinator: Finished Registering Logical source");

  // start the coordinator worker that is the sink for all
  // queryIdAndCatalogEntryMapping
  NES_DEBUG("NesCoordinator::startCoordinator: start nes worker");
  // Unconditionally set IP of internal worker and set IP and port of
  // coordinator.
  coordinatorConfiguration->worker.coordinatorHost = rpcIp;
  coordinatorConfiguration->worker.coordinatorPort = rpcPort;
  // Ensure that coordinator and internal worker enable/disable monitoring
  // together.
  ...
  auto workerConfig =
      std::make_shared<WorkerConfiguration>(coordinatorConfiguration->worker);
  worker = std::make_shared<NesWorker>(
      std::move(workerConfig),
      monitoringService->getMonitoringManager()->getMetricStore());
  ...
  auto originString =
      coordinatorConfiguration->restServerCorsAllowedOrigin.getValue();
  if (!originString.empty()) {
    NES_INFO("CORS: allow origin: {}", originString);
    allowedOrigin = originString;
  }

  restServer = std::make_shared<RestServer>(
      restIp, restPort, this->inherited0::weak_from_this(), queryCatalog,
      sourceCatalogService, topology, globalExecutionPlan,
      requestHandlerService, monitoringService, queryParsingService,
      globalQueryPlan, udfCatalog, worker->getNodeEngine()->getBufferManager(),
      allowedOrigin);
  ...
  auto amendmentThreadCount = coordinatorConfiguration->optimizer
                                  .placementAmendmentThreadCount.getValue();
  placementAmendmentHandler =
      std::make_shared<Optimizer::PlacementAmendmentHandler>(
          amendmentThreadCount, placementAmendmentQueue);
  placementAmendmentHandler->start();

  if (blocking) {  // blocking is for the starter to wait here for user to send
                   // query
    ...
    NES_DEBUG("NesCoordinator started, return without blocking on port {}",
              rpcPort);
    return rpcPort;
  }
  return 0UL;
...
Catalogs::Source::SourceCatalogPtr NesCoordinator::getSourceCatalog() const {
  ...
  if (isRunning.compare_exchange_strong(expected, false)) {
    ...
    if (!successShutdownWorker) {
      ...
      NES_THROW_RUNTIME_ERROR(
          "NesCoordinator::stop error while stopping node engine");
    }
    ...
    if (!successStopRest) {
      NES_ERROR(
          "NesCoordinator::stopCoordinator: error while stopping restServer");
...
void NesCoordinator::buildAndStartGRPCServer(
  ...
  CoordinatorRPCServer service(
      requestHandlerService, topology, sourceCatalogService, queryCatalog,
      monitoringService->getMonitoringManager(), queryParsingService,
      coordinatorHealthCheckService);
  ...
  prom->set_value(true);
  NES_DEBUG(
      "NesCoordinator: buildAndStartGRPCServerServer listening on address={}",
      address);
  rpcServer->Wait();  // blocking call
  NES_DEBUG("NesCoordinator: buildAndStartGRPCServer end listening");
}

std::vector<Runtime::QueryStatisticsPtr> NesCoordinator::getQueryStatistics(
    QueryId queryId) {
  NES_INFO("NesCoordinator: Get query statistics for query Id {}", queryId);
  return worker->getNodeEngine()->getQueryStatistics(
      UNSURE_CONVERSION_TODO_4761(queryId, SharedQueryId));
}

RequestHandlerServicePtr NesCoordinator::getRequestHandlerService() {
  return requestHandlerService;
}

Catalogs::Query::QueryCatalogPtr NesCoordinator::getQueryCatalog() {
  return queryCatalog;
}

Catalogs::UDF::UDFCatalogPtr NesCoordinator::getUDFCatalog() {
  return udfCatalog;
}

Optimizer::UMPMCAmendmentQueuePtr NesCoordinator::getPlacementAmendmentQueue() {
  return placementAmendmentQueue;
}
...
void NesCoordinator::onFatalException(const std::shared_ptr<std::exception>,
                                      std::string) {}

SourceCatalogServicePtr NesCoordinator::getSourceCatalogService() const {
  return sourceCatalogService;
}

LocationServicePtr NesCoordinator::getLocationService() const {
  return locationService;
}

Optimizer::GlobalExecutionPlanPtr NesCoordinator::getGlobalExecutionPlan()
    const {
