...
int main(int argc, char** argv) {
  try {
    ...
    // if workerConfigPath to a yaml file is provided, system will use
    // physicalSources in yaml file
    if (workerConfigPath != commandLineParams.end()) {
      workerConfiguration->configPath = workerConfigPath->second;
      workerConfiguration->overwriteConfigWithYAMLFileInput(
          workerConfigPath->second);
    }

    // if command line params are provided that do not contain a path to a yaml
    // file for worker config, command line param physicalSources are used to
    // overwrite default physicalSources
    if (argc >= 1 && !commandLineParams.contains("--configPath")) {
      workerConfiguration->overwriteConfigWithCommandLineInput(
          commandLineParams);
    }

    Logger::getInstance()->changeLogLevel(
        workerConfiguration->logLevel.getValue());

    NES_INFO("NesWorkerStarter: Start with {}",
             workerConfiguration->toString());
    ...
    nesWorker->start(/**blocking*/ true,
                     /**withConnect*/ true);  // This is a blocking call
