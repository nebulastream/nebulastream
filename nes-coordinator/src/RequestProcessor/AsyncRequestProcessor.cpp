...
AsyncRequestProcessor::AsyncRequestProcessor(StorageDataStructures& storageDataStructures) : running(true)
{
    ...
    switch (storageHandlerType)
    {
        ...
        case StorageHandlerType::TwoPhaseLocking:
            ...
            break;
        default:
            NES_THROW_RUNTIME_ERROR("Unknown storage type supplied. Failed to initialize request executor.");
    }
    ...
    for (uint32_t i = 0; i < numOfThreads; ++i)
    {
        ...
        auto thread = std::thread(
            [this, i, promise = std::move(promise)]() mutable
            {
                try
                {
    ...
    if (running.compare_exchange_strong(expected, false))
    {
        try
        {
            ...
            auto result = std::accumulate(
                completionFutures.begin(), completionFutures.end(), true, [](bool acc, auto& future) { return acc && future.get(); });
