...
AsyncRequestProcessor::AsyncRequestProcessor(
  ...
  numOfThreads = storageDataStructures.coordinatorConfiguration
                     ->requestExecutorThreads.getValue();
  auto storageHandlerType = storageDataStructures.coordinatorConfiguration
                                ->storageHandlerType.getValue();
  switch (storageHandlerType) {
  case StorageHandlerType::SerialHandler:
    storageHandler = SerialStorageHandler::create(storageDataStructures);
    break;
  case StorageHandlerType::TwoPhaseLocking:
    storageHandler =
        TwoPhaseLockingStorageHandler::create(storageDataStructures);
    break;
  default:
    NES_THROW_RUNTIME_ERROR("Unknown storage type supplied. Failed to "
  ...
  for (uint32_t i = 0; i < numOfThreads; ++i) {
    ...
    auto thread =
        std::thread([this, i, promise = std::move(promise)]() mutable {
          try {
...
RequestId AsyncRequestProcessor::runAsync(AbstractRequestPtr request) {
  if (!running) {
    NES_WARNING(
        "Cannot execute request, Async request executor is not running");
  ...
  if (running.compare_exchange_strong(expected, false)) {
    try {
      ...
      auto result = std::accumulate(
          completionFutures.begin(), completionFutures.end(), true,
          [](bool acc, auto &future) { return acc && future.get(); });
...
void AsyncRequestProcessor::runningRoutine() {
  while (true) {
    ...
    if (running) {
      ...
      auto multiRequest =
          std::dynamic_pointer_cast<AbstractMultiRequest>(abstractRequest);
      if (multiRequest) {
        // remove the multi request from the queue only if it is done,
        // otherwise leave it at the front of the queue and acquire more threads
        ...
        // if the request is not a multirequest it will not have to acquire more
        // threads than the current one. it can be removed from the queue
        asyncRequestQueue.pop_front();
      }
      ...
      std::vector<AbstractRequestPtr> nextRequests =
          abstractRequest->execute(storageHandler);
