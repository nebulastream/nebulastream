...
QueryCatalogHandle
SerialStorageHandler::getQueryCatalogHandle(const RequestId) {
  return {&*queryCatalog, UnlockDeleter()};
}

GlobalQueryPlanHandle
SerialStorageHandler::getGlobalQueryPlanHandle(const RequestId) {
  return {&*globalQueryPlan, UnlockDeleter()};
}

Catalogs::Source::SourceCatalogPtr
SerialStorageHandler::getSourceCatalogHandle(const RequestId) {
  return {&*sourceCatalog, UnlockDeleter()};
}

Catalogs::UDF::UDFCatalogPtr
SerialStorageHandler::getUDFCatalogHandle(const RequestId) {
