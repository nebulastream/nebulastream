...
std::vector<AbstractRequestPtr> StopQueryRequest::executeRequestLogic(
  ...
  try {
    ...
    if (queryId == INVALID_QUERY_ID) {
      throw Exceptions::QueryNotFoundException(
          fmt::format("Cannot stop query with invalid query id {}. Please "
                      "enter a valid query id.",
                      queryId));
    }

    // mark query for hard stop
    queryCatalog->updateQueryStatus(queryId, QueryState::MARKED_FOR_HARD_STOP,
                                    "Query Stop Requested");

    auto sharedQueryId = globalQueryPlan->getSharedQueryId(queryId);
    if (sharedQueryId == INVALID_SHARED_QUERY_ID) {
      throw Exceptions::QueryNotFoundException(
          fmt::format("Could not find a a valid shared query plan for query "
                      "with id {} in the global query plan",
                      sharedQueryId));
    }
    ...
    // FIXME: #3742 This may not work well when shared query plan contains more
    // than one queries:
    //  1. The query merging feature is enabled.
    //  2. A query from a shared query plan was removed but over all shared
    //  query plan is still serving other queries. Expected Result:
    //   - only the stopped query is marked as stopped.
    //  Actual Result:
    //   - All queries are set to stopped and the whole shared query plan is
    //   removed.

    // Mark all contained queries as stopped
    queryCatalog->updateQueryStatus(
        UNSURE_CONVERSION_TODO_4761(sharedQueryId, QueryId),
        QueryState::STOPPED, "Hard Stopped");
    ...
    failureRequests.insert(failureRequests.end(), requests.begin(),
                           requests.end());
  }
  return failureRequests;
}

void StopQueryRequest::postExecution(
    [[maybe_unused]] const StorageHandlerPtr& storageHandler) {
  storageHandler->releaseResources(requestId);
}

std::string StopQueryRequest::toString() {
  return fmt::format("StopQueryRequest {{ QueryId: {}}}", queryId);
}

void StopQueryRequest::preRollbackHandle(
    [[maybe_unused]] std::exception_ptr ex,
    [[maybe_unused]] const StorageHandlerPtr& storageHandle) {}

void StopQueryRequest::postRollbackHandle(
    [[maybe_unused]] std::exception_ptr ex,
    [[maybe_unused]] const StorageHandlerPtr& storageHandle) {}

std::vector<AbstractRequestPtr> StopQueryRequest::rollBack(
  ...
  try {
    ...
    failRequest.push_back(FailQueryRequest::create(
        UNSURE_CONVERSION_TODO_4761(ex.getQueryId(), SharedQueryId),
        INVALID_DECOMPOSED_QUERY_PLAN_ID, ex.what(), MAX_RETRIES_FOR_FAILURE));
  } catch (QueryDeploymentException& ex) {
    // todo: #3821 change to more specific exceptions, remove
    // QueryDeploymentException Happens if:
    // 1. QueryDeploymentException The bytecode list of classes implementing the
    // UDF must contain the fully-qualified name of the UDF
    // 2. QueryDeploymentException: Error in call to Elegant acceleration
    // service with code
    // 3. QueryDeploymentException: QueryDeploymentPhase : unable to find query
    // sub plan with id
    failRequest.push_back(FailQueryRequest::create(
        UNSURE_CONVERSION_TODO_4761(ex.getQueryId(), SharedQueryId),
        INVALID_DECOMPOSED_QUERY_PLAN_ID, ex.what(), MAX_RETRIES_FOR_FAILURE));
    ...
    // Happens if:
    // 1. InvalidQueryException: inside QueryDeploymentPhase, if the query
    // sub-plan metadata already exists in the query catalog --> non-recoverable
    failRequest.push_back(FailQueryRequest::create(
        UNSURE_CONVERSION_TODO_4761(ex.getQueryId(), SharedQueryId),
        INVALID_DECOMPOSED_QUERY_PLAN_ID, ex.what(), MAX_RETRIES_FOR_FAILURE));
  } catch (TypeInferenceException& ex) {
    queryCatalog->updateQueryStatus(ex.getQueryId(), QueryState::FAILED,
                                    ex.what());
  } catch (Exceptions::QueryUndeploymentException& ex) {
    // In general, failures in QueryUndeploymentPhase are concerned with the
    // current sqp id and a failure with a topology node Therefore, for
    // QueryUndeploymentException, we assume that the sqp is not running on any
    // node, and we can set the sqp's status to stopped we do this as long as
    // there are retries present, otherwise, we fail the query
    queryCatalog->updateQueryStatus(ex.getQueryId(), QueryState::FAILED,
                                    ex.what());
    ...
    // 2. if check and mark for hard stop failed, means that stop is already in
    // process, hence, we don't do anything
    trySetExceptionInPromise(exception);
  } catch (Exceptions::RequestExecutionException& ex) {
    // todo: #3821 retry for these errors, add specific rpcCallException and
    // retry failed part, differentiate between deployment and undeployment
    // phase RPC call errors:
    // 1. asynchronous call to worker to stop shared query plan failed -->
    // currently invokes NES_THROW_RUNTIME_ERROR;
    // 2. asynchronous call to worker to unregister shared query plan failed -->
    // currently invokes NES_THROW_RUNTIME_ERROR:
    setExceptionInPromiseOrRethrow(exception);
  } catch (Exceptions::RuntimeException& ex) {
    // todo: #3821 change to more specific exceptions
    // 1. Called from QueryUndeploymentPhase: GlobalQueryPlan: Unable to remove
    // all child operators of the identified sink operator in the shared query
    // plan
    // 2. Called from PlacementStrategyPhase: PlacementStrategyFactory: Unknown
    // placement strategy
    setExceptionInPromiseOrRethrow(exception);
  }
