...
std::vector<AbstractRequestPtr> AbstractRequest::handleError(
  ...
  try {
    ...
    if (retry()) {
      ...
      NES_ERROR("Final failure to rollback. No retries left. Error: {}",
                exception.what());
    }
  }
...
void AbstractRequest::setExceptionInPromiseOrRethrow(
