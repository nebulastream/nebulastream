...
FailQueryRequest::FailQueryRequest(
    ...
    const std::string& failureReason, const uint8_t maxRetries)
    : AbstractUniRequest(
          {ResourceType::GlobalQueryPlan, ResourceType::QueryCatalogService,
           ResourceType::Topology, ResourceType::UdfCatalog,
           ResourceType::SourceCatalog, ResourceType::GlobalExecutionPlan,
...
FailQueryRequestPtr FailQueryRequest::create(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId failedDecomposedQueryId,
    const std::string& failureReason, uint8_t maxRetries) {
  return std::make_shared<FailQueryRequest>(
      sharedQueryId, failedDecomposedQueryId, failureReason, maxRetries);
}

void FailQueryRequest::preRollbackHandle(std::exception_ptr,
                                         const StorageHandlerPtr&) {}

std::vector<AbstractRequestPtr> FailQueryRequest::rollBack(
    std::exception_ptr ex, const StorageHandlerPtr&) {
...
void FailQueryRequest::postRollbackHandle(std::exception_ptr,
                                          const StorageHandlerPtr&) {
...
std::vector<AbstractRequestPtr> FailQueryRequest::executeRequestLogic(
  ...
  queryCatalog->checkAndMarkSharedQueryForFailure(sharedQueryId,
                                                  decomposedQueryPlanId);

  globalQueryPlan->removeQuery(
      UNSURE_CONVERSION_TODO_4761(sharedQueryId, QueryId),
      RequestType::FailQuery);
  ...
  try {
    ...
    throw Exceptions::QueryUndeploymentException(
        sharedQueryId, fmt::format("Failed to undeploy shared query with id {}",
                                   sharedQueryId));
  }

  queryCatalog->updateSharedQueryStatus(sharedQueryId, QueryState::FAILED,
                                        failureReason);

  // respond to the calling service which is the shared query id to the query
  // being undeployed
