...
ISQPAddNodeEvent::ISQPAddNodeEvent(
    WorkerType workerType, WorkerId workerId, const std::string &ipAddress,
    uint32_t grpcPort, uint32_t dataPort, uint16_t resources,
    const std::map<std::string, std::any> &properties)
    : ISQPEvent(ISQP_ADD_NODE_EVENT_PRIORITY), workerType(workerType),
      workerId(workerId), ipAddress(ipAddress), grpcPort(grpcPort),
      dataPort(dataPort), resources(resources), properties(properties) {}

ISQPEventPtr
ISQPAddNodeEvent::create(WorkerType workerType, WorkerId workerId,
                         const std::string &ipAddress, uint32_t grpcPort,
                         uint32_t dataPort, uint16_t resources,
                         const std::map<std::string, std::any> &properties) {
  return std::make_shared<ISQPAddNodeEvent>(workerType, workerId, ipAddress,
                                            grpcPort, dataPort, resources,
                                            properties);
}
