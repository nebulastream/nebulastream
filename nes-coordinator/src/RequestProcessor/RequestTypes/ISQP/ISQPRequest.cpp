...
ISQPRequest::ISQPRequest(const z3::ContextPtr &z3Context,
                         std::vector<ISQPEventPtr> events, uint8_t maxRetries)
...
ISQPRequestPtr ISQPRequest::create(const z3::ContextPtr &z3Context,
                                   std::vector<ISQPEventPtr> events,
                                   uint8_t maxRetries) {
  return std::make_shared<ISQPRequest>(z3Context, events, maxRetries);
}

std::vector<AbstractRequestPtr> ISQPRequest::executeRequestLogic(
    const NES::RequestProcessor::StorageHandlerPtr &storageHandle) {
  ...
  for (const auto &event : events) {
    if (event->instanceOf<ISQPRemoveNodeEvent>()) {
      ...
      topology->removeTopologyNodeAsChild(removeLinkEvent->getParentNodeId(),
                                          removeLinkEvent->getChildNodeId());
    } else if (event->instanceOf<ISQPAddLinkEvent>()) {
      auto addLinkEvent = event->as<ISQPAddLinkEvent>();
      topology->addTopologyNodeAsChild(addLinkEvent->getParentNodeId(),
                                       addLinkEvent->getChildNodeId());
      ...
      if (addNodeEvent->getWorkerType() == WorkerType::CLOUD) {
        topology->registerWorkerAsRoot(
            addNodeEvent->getWorkerId(), addNodeEvent->getIpAddress(),
            addNodeEvent->getGrpcPort(), addNodeEvent->getDataPort(),
            addNodeEvent->getResources(), addNodeEvent->getProperties(), 0, 0);
      } else {
        topology->registerWorker(
            addNodeEvent->getWorkerId(), addNodeEvent->getIpAddress(),
            addNodeEvent->getGrpcPort(), addNodeEvent->getDataPort(),
            addNodeEvent->getResources(), addNodeEvent->getProperties(), 0, 0);
      }
  ...
  for (const auto &event : events) {
    if (event->instanceOf<ISQPRemoveNodeEvent>()) {
      ...
      event->response.set_value(
          std::make_shared<ISQPAddQueryResponse>(queryId));
    } else if (event->instanceOf<ISQPRemoveQueryEvent>()) {
      handleRemoveQueryRequest(event->as<ISQPRemoveQueryEvent>());
      event->response.set_value(
          std::make_shared<ISQPRemoveQueryResponse>(true));
    }
  }
  ...
  for (const auto &sharedQueryPlan : sharedQueryPlans) {
    const auto &amendmentInstance =
        Optimizer::PlacementAmendmentInstance::create(
            sharedQueryPlan, globalExecutionPlan, topology, typeInferencePhase,
            coordinatorConfiguration, queryCatalog);
  ...
  responsePromise.set_value(std::make_shared<ISQPRequestResponse>(
      processingStartTime, amendmentStartTime, processingEndTime,
      numOfSQPAffected, numOfFailedPlacements, true));
  return {};
}

void ISQPRequest::handleRemoveLinkRequest(
    NES::RequestProcessor::ISQPRemoveLinkEventPtr removeLinkEvent) {
...
void ISQPRequest::handleRemoveNodeRequest(
    NES::RequestProcessor::ISQPRemoveNodeEventPtr removeNodeEvent) {

  auto removedNodeId = removeNodeEvent->getWorkerId();

  // 1. If the removed execution nodes does not have any shared query plan
  // placed then skip rest of the operation
  auto impactedSharedQueryIds =
      globalExecutionPlan->getPlacedSharedQueryIds(removedNodeId);
  if (impactedSharedQueryIds.empty()) {
    NES_INFO("Removing node {} has no effect on the running queries as there "
             "are no queries placed "
             "on the node.",
             removedNodeId);
    return;
  }

  // 2. Fetch upstream and downstream topology nodes connected via the removed
  // topology node
  ...
  // 3. If the topology node either do not have upstream or downstream node then
  // fail the request
  if (upstreamTopologyNodes.empty() || downstreamTopologyNodes.empty()) {
    // FIXME: how to handle this case? If the node to remove has physical source
    // then we may need to kill the
    //  whole query.
    NES_NOT_IMPLEMENTED();
  }

  // todo: capy block and place function above
  // 4. Iterate over all upstream and downstream topology node pairs and try to
  // mark operators for re-placement
  for (auto const &upstreamTopologyNode : upstreamTopologyNodes) {
    for (auto const &downstreamTopologyNode : downstreamTopologyNodes) {

      // 4.1. Iterate over impacted shared query plan ids to identify the shared
      // query plans placed on the
      //  upstream and downstream execution nodes
      for (auto const &impactedSharedQueryId : impactedSharedQueryIds) {

        auto upstreamExecutionNode =
            globalExecutionPlan->getLockedExecutionNode(upstreamTopologyNode);
        auto downstreamExecutionNode =
            globalExecutionPlan->getLockedExecutionNode(downstreamTopologyNode);

        // 4.2. If there exists no upstream or downstream execution nodes than
        // skip rest of the operation
        if (!upstreamExecutionNode || !downstreamExecutionNode) {
          continue;
        }

        // 4.3. Only process the upstream and downstream execution node pairs
        // when both have shared query plans
        //  with the impacted shared query id
        if (upstreamExecutionNode->operator*()
                ->hasRegisteredDecomposedQueryPlans(impactedSharedQueryId) &&
            downstreamExecutionNode->operator*()
                ->hasRegisteredDecomposedQueryPlans(impactedSharedQueryId)) {
          ...
          auto [upstreamOperatorIds, downstreamOperatorIds] =
              NES::Experimental::findUpstreamAndDownstreamPinnedOperators(
                  sharedQueryPlan, upstreamExecutionNode,
                  downstreamExecutionNode, topology);
          // perform re-operator placement on the query plan
          sharedQueryPlan->performReOperatorPlacement(upstreamOperatorIds,
                                                      downstreamOperatorIds);
        }
...
QueryId ISQPRequest::handleAddQueryRequest(
    NES::RequestProcessor::ISQPAddQueryEventPtr addQueryEvent) {
  ...
  queryCatalog->createQueryCatalogEntry("", queryPlan, queryPlacementStrategy,
  ...
  auto topologySpecificQueryRewritePhase =
      Optimizer::TopologySpecificQueryRewritePhase::create(
          topology, sourceCatalog, coordinatorConfiguration->optimizer,
          statisticProbeHandler);
  auto signatureInferencePhase = Optimizer::SignatureInferencePhase::create(
      z3Context, coordinatorConfiguration->optimizer.queryMergerRule);
  auto queryMergerPhase = Optimizer::QueryMergerPhase::create(
      z3Context, coordinatorConfiguration->optimizer);
  ...
  // 9. Perform signature inference phase for sharing identification among query
  // plans
  ...
  if (sharedQueryId == INVALID_SHARED_QUERY_ID) {
    throw Exceptions::SharedQueryPlanNotFoundException(
        "Could not find shared query id in global query plan. Shared query id "
        "is invalid.",
        sharedQueryId);
  }
...
void ISQPRequest::handleRemoveQueryRequest(
    NES::RequestProcessor::ISQPRemoveQueryEventPtr removeQueryEvent) {

  auto queryId = removeQueryEvent->getQueryId();
  if (queryId == INVALID_QUERY_ID) {
    throw Exceptions::QueryNotFoundException(
        fmt::format("Cannot stop query with invalid query id {}. Please enter "
                    "a valid query id.",
                    queryId));
  }
  ...
  if (sharedQueryId == INVALID_SHARED_QUERY_ID) {
    throw Exceptions::QueryNotFoundException(
        fmt::format("Could not find a a valid shared query plan for query with "
                    "id {} in the global query plan",
                    sharedQueryId));
  }
...
ISQPRequest::rollBack(std::exception_ptr, const StorageHandlerPtr &) {
  return std::vector<AbstractRequestPtr>();
}

void ISQPRequest::preRollbackHandle(std::exception_ptr,
                                    const StorageHandlerPtr &) {}

void ISQPRequest::postRollbackHandle(std::exception_ptr,
                                     const StorageHandlerPtr &) {}
} // namespace NES::RequestProcessor
