...
AddQueryRequest::AddQueryRequest(
    ...
    : AbstractUniRequest({ResourceType::QueryCatalogService,
                         ...
                         maxRetries),
      queryId(INVALID_QUERY_ID), queryString(queryString), queryPlan(nullptr),
...
AddQueryRequestPtr AddQueryRequest::create(
    ...
    const uint8_t maxRetries, const z3::ContextPtr &z3Context,
    const QueryParsingServicePtr &queryParsingService) {
  return std::make_shared<AddQueryRequest>(queryPlan, queryPlacementStrategy,
...
void AddQueryRequest::preRollbackHandle(
    [[maybe_unused]] std::exception_ptr ex,
    [[maybe_unused]] const StorageHandlerPtr &storageHandler) {}

std::vector<AbstractRequestPtr> AddQueryRequest::rollBack(
  ...
  // make sure the promise is set before returning in case a the caller is
  // waiting on it
  trySetExceptionInPromise(std::make_exception_ptr<RequestExecutionException>(
      RequestExecutionException("No return value set in promise")));
  return {};
}

void AddQueryRequest::removeFromGlobalQueryPlanAndMarkAsFailed(
    std::exception &e, const StorageHandlerPtr &storageHandler) {
...
void AddQueryRequest::markAsFailedInQueryCatalog(
    std::exception &e, const StorageHandlerPtr &storageHandler) {
...
AddQueryRequest::executeRequestLogic(const StorageHandlerPtr &storageHandler) {
  try {
    ...
    auto statisticIdInferencePhase =
        Optimizer::StatisticIdInferencePhase::create();
    ...
    if (!queryString.empty() && !queryPlan) {
      // Checking the syntactic validity and compiling the query string to an
      // object
      ...
      NES_ERROR("Please supply either query string or query plan while "
    ...
    // respond to the calling service with the query id after creating an entry
    // in the catalog
    ...
    if (queryPlacementStrategy ==
            Optimizer::PlacementStrategy::ELEGANT_BALANCED ||
        queryPlacementStrategy ==
            Optimizer::PlacementStrategy::ELEGANT_PERFORMANCE ||
        queryPlacementStrategy ==
            Optimizer::PlacementStrategy::ELEGANT_ENERGY) {
    ...
    // 14. Identify the number of origins and their ids for all logical
    // operators
    ...
    if (sharedQueryPlan->getStatus() == SharedQueryPlanStatus::CREATED) {
      queryCatalog->createSharedQueryCatalogEntry(sharedQueryId, {queryId},
                                                  QueryState::OPTIMIZING);
    } else {
      queryCatalog->updateSharedQueryStatus(sharedQueryId,
                                            QueryState::OPTIMIZING, "");
    }
    // Link both catalogs
    ...
    NES_ERROR(
        "Exception occurred while processing AddQueryRequest with error {}",
        exception.what());
