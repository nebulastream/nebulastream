...
ExplainRequest::ExplainRequest(
    ...
    const uint8_t maxRetries, const z3::ContextPtr &z3Context)
    : AbstractUniRequest({ResourceType::QueryCatalogService,
                          ResourceType::GlobalExecutionPlan,
                          ResourceType::Topology, ResourceType::GlobalQueryPlan,
                          ResourceType::UdfCatalog, ResourceType::SourceCatalog,
                          ResourceType::CoordinatorConfiguration,
                          ResourceType::StatisticProbeHandler},
                         maxRetries),
      queryId(INVALID_QUERY_ID), queryString(""), queryPlan(queryPlan),
      queryPlacementStrategy(queryPlacementStrategy), z3Context(z3Context),
      queryParsingService(nullptr) {}

ExplainRequestPtr ExplainRequest::create(
    const QueryPlanPtr &queryPlan,
    const Optimizer::PlacementStrategy queryPlacementStrategy,
    const uint8_t maxRetries, const z3::ContextPtr &z3Context) {
  return std::make_shared<ExplainRequest>(queryPlan, queryPlacementStrategy,
                                          maxRetries, z3Context);
}

void ExplainRequest::preRollbackHandle(
    [[maybe_unused]] std::exception_ptr ex,
    [[maybe_unused]] const StorageHandlerPtr &storageHandler) {}

std::vector<AbstractRequestPtr> ExplainRequest::rollBack(
    [[maybe_unused]] std::exception_ptr ex,
    [[maybe_unused]] const StorageHandlerPtr &storageHandler) {
  std::rethrow_exception(ex);
}

void ExplainRequest::postRollbackHandle(
    [[maybe_unused]] std::exception_ptr ex,
    [[maybe_unused]] const StorageHandlerPtr &storageHandler) {}

std::vector<AbstractRequestPtr>
ExplainRequest::executeRequestLogic(const StorageHandlerPtr &storageHandler) {
  try {
    NES_DEBUG("Acquiring required resources.");
    // Acquire all necessary resources
    auto globalExecutionPlan =
        storageHandler->getGlobalExecutionPlanHandle(requestId);
    ...
    auto statisticProbeHandler =
        storageHandler->getStatisticProbeHandler(requestId);
    ...
    auto queryMergerPhase = Optimizer::QueryMergerPhase::create(
        this->z3Context, optimizerConfigurations);
    typeInferencePhase = Optimizer::TypeInferencePhase::create(
        sourceCatalog, std::move(udfCatalog));
    auto sampleCodeGenerationPhase =
        Optimizer::SampleCodeGenerationPhase::create();
    auto queryRewritePhase =
        Optimizer::QueryRewritePhase::create(coordinatorConfiguration);
    ...
    auto topologySpecificQueryRewritePhase =
        Optimizer::TopologySpecificQueryRewritePhase::create(
            topology, sourceCatalog, optimizerConfigurations,
            statisticProbeHandler);
    auto signatureInferencePhase = Optimizer::SignatureInferencePhase::create(
        this->z3Context, optimizerConfigurations.queryMergerRule);
    auto memoryLayoutSelectionPhase =
        Optimizer::MemoryLayoutSelectionPhase::create(
            optimizerConfigurations.memoryLayoutPolicy);
    auto syntacticQueryValidation =
        Optimizer::SyntacticQueryValidation::create(queryParsingService);
    auto semanticQueryValidation = Optimizer::SemanticQueryValidation::create(
        sourceCatalog, udfCatalog,
    ...
    queryCatalog->createQueryCatalogEntry(
        queryString, queryPlan, queryPlacementStrategy, QueryState::REGISTERED);
    ...
    queryCatalog->addUpdatedQueryPlan(queryId, "Query Rewrite Phase",
                                      queryPlan);
    ...
    // 9. Perform signature inference phase for sharing identification among
    // query plans
    ...
    queryCatalog->addUpdatedQueryPlan(
        queryId, "Topology Specific Query Rewrite Phase", queryPlan);

    // 12. Perform type inference over re-written query plan
    queryPlan = typeInferencePhase->execute(queryPlan);

    // 13. Identify the number of origins and their ids for all logical
    // operators
    ...
    queryCatalog->addUpdatedQueryPlan(queryId, "Executed Query Plan",
                                      queryPlan);
    ...
    if (sharedQueryId == INVALID_SHARED_QUERY_ID) {
      throw Exceptions::SharedQueryPlanNotFoundException(
          "Could not find shared query id in global query plan. Shared query "
          "id is invalid.",
          sharedQueryId);
    }
    ...
    if (!sharedQueryPlan) {
      throw Exceptions::SharedQueryPlanNotFoundException(
          "Could not obtain shared query plan by shared query id.",
          sharedQueryId);
    }
    ...
    auto accelerateJavaUdFs =
        coordinatorConfiguration->elegant.accelerateJavaUDFs;
    auto accelerationServiceURL =
        coordinatorConfiguration->elegant.accelerationServiceURL;

    // 23. Compute the json response explaining the optimization steps
    auto response = getExecutionPlanForSharedQueryAsJson(
        sharedQueryId, globalExecutionPlan, topology, accelerateJavaUdFs,
        accelerationServiceURL, sampleCodeGenerationPhase);
    ...
    for (const auto &deploymentContext : deploymentContexts) {
      ...
      case QueryState::MARKED_FOR_DEPLOYMENT: {
        globalExecutionPlan->updateDecomposedQueryPlanState(
            WorkerId, sharedQueryId, decomposedQueryPlanId,
    ...
    NES_ERROR(
        "Exception occurred while processing ExplainRequest with error {}",
        exception.what());
...
void ExplainRequest::assignOperatorIds(const QueryPlanPtr &queryPlan) {
  ...
  for (auto itr = queryPlanIterator.begin(); itr != PlanIterator::end();
       ++itr) {
...
nlohmann::json ExplainRequest::getExecutionPlanForSharedQueryAsJson(
    SharedQueryId sharedQueryId,
    const Optimizer::GlobalExecutionPlanPtr &globalExecutionPlan,
    const TopologyPtr &topology, bool accelerateJavaUDFs,
  ...
  auto lockedExecutionNodes =
      globalExecutionPlan->getLockedExecutionNodesHostingSharedQueryId(
          sharedQueryId);
  for (const auto &lockedExecutionNode : lockedExecutionNodes) {
    ...
    auto topologyNode = topology->getCopyOfTopologyNodeWithId(
        lockedExecutionNode->operator*()->getId());

    // loop over all query sub plans inside the current executionNode
    nlohmann::json scheduledSubQueries{};
    for (const auto &decomposedQueryPlan :
         lockedExecutionNode->operator*()->getAllDecomposedQueryPlans(
             sharedQueryId)) {
      ...
      currentQuerySubPlanMetaData["querySubPlanId"] =
          decomposedQueryPlan->getDecomposedQueryPlanId();

      // add open cl acceleration code
      if (accelerateJavaUDFs) {
        addOpenCLAccelerationCode(accelerationServiceURL, decomposedQueryPlan,
                                  topologyNode);
      }

      // add logical query plan to the json object
      currentQuerySubPlanMetaData["logicalQuerySubPlan"] =
          decomposedQueryPlan->toString();

      // Generate Code Snippets for the sub query plan
      auto updatedSubQueryPlan =
          sampleCodeGenerationPhase->execute(decomposedQueryPlan);
      ...
      for (auto itr = queryPlanIterator.begin(); itr != PlanIterator::end();
        ...
        if (visitingOp->hasProperty("PIPELINE_ID")) {
          auto pipelineId =
              std::any_cast<PipelineId>(visitingOp->getProperty("PIPELINE_ID"));
          if (pipelineIds.emplace(pipelineId).second) {
            generatedCodeSnippets.emplace_back(
                std::any_cast<std::string>(visitingOp->getProperty(
                    Optimizer::ElegantPlacementStrategy::sourceCodeKey)));
          }
        }
      }
...
void ExplainRequest::addOpenCLAccelerationCode(
    const std::string &accelerationServiceURL,
  ...
  auto openCLOperators =
      decomposedQueryPlan->getOperatorByType<LogicalOpenCLOperator>();

  // 2. Iterate over all open CL operators and set the Open CL code returned by
  ...
  for (const auto &openCLOperator : openCLOperators) {
    ...
    payload[DEVICE_INFO_KEY] =
        std::any_cast<std::vector<NES::Runtime::OpenCLDeviceInfo>>(
            topologyNode->getNodeProperty(
                NES::Worker::Configuration::OPENCL_DEVICES))
    ...
    // The constructor of the Java UDF descriptor ensures that the byte code of
    // the class exists.
    jni::JavaByteCode javaByteCode =
        javaDescriptor->getClassByteCode(javaDescriptor->getClassName())
            .value();
    ...
    cpr::Response response =
        cpr::Post(cpr::Url{accelerationServiceURL},
                  cpr::Header{{"Content-Type", "application/json"}},
                  multipartPayload, cpr::Timeout(ELEGANT_SERVICE_TIMEOUT));
    if (response.status_code != 200) {
      throw QueryDeploymentException(
          UNSURE_CONVERSION_TODO_4761(
              decomposedQueryPlan->getDecomposedQueryPlanId(), SharedQueryId),
