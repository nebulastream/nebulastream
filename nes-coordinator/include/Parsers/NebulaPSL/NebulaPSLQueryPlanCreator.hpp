 ...
 * It inherits from the auto-generated ANTLR base listener to walk the AST
 * created from the pattern string. This enables the parsing of declarative
 * patterns into NES queries.
 */
class NesCEPQueryPlanCreator : public NesCEPBaseListener {
 public:
  /** the following methods read out the AST tree and collect all mined patterns
   in the global pattern list
   * An example of the AST looks as follows:
   * (query (cepPattern PATTERN test := (compositeEventExpressions (
      (listEvents (eventElem (event A)) (operatorRule AND) (eventElem (event
   B))) )) FROM (inputStreams (inputStream default_logical AS A) , (inputStream
   default_logical_b AS B)) WHERE (whereExp (expression (predicate (predicate
   (expressionAtom (eventAttribute A . (attribute currentSpeed))))
   (comparisonOperator <) (predicate (expressionAtom (eventAttribute A .
   (attribute allowedSpeed))))))) WITHIN (timeConstraints [ (interval 3
   (intervalType MINUTE)) ]) INTO (sinkList (sink (sinkType Print) ::
   testSink))) <EOF>
   * each keyword in the AST has its two functions, enter (when X is visited)
   and exist (called after all children of X have been visited), contained in
   the parent class NesCEPBaseListener,
  ...
  // EventElement
  /** @brief marks current (event) element as a child of the
   * currentOperatorPointer subPattern and move one step up in the AST hierarchy
   * @param context*/
  ...
  // Event
  /**
    * @brief marks the position of the event inside of the currentElementPointer
  subPattern
    * @param context
  ...
  /**
   * @brief leaves the current operator element and changes direction to right
   * to mine right branch for binary operators
   * @param context
   */
  void exitOperatorRule(NesCEPParser::OperatorRuleContext* context) override;

  // FROM clause
  /** @brief substitutes the input stream alias by its real name in every
   * subPattern
   * @param context
   */
   ...
   * @param context
   */
  void exitBinaryComparisonPredicate(
      NesCEPParser::BinaryComparasionPredicateContext* context) override;
  ...
  /**
   * @brief this methods parses the user-specified window information to
   * TimeMeasures
   */
  std::pair<Windowing::TimeMeasure, Windowing::TimeMeasure>
  transformWindowToTimeMeasurements(std::string timeMeasure,
  ...
  /**
   * @brief: this function creates an artificial key for the binary operators
   * AND and SEQ
  ...
  QueryPlanPtr addBinaryOperatorToQueryPlan(
      std::string operaterName,
      std::map<int, NebulaPSLOperator>::const_iterator iterator,
      QueryPlanPtr queryPlan) const;

  /**
   * @brief: checks if one of the sources for a binary operator is already part
   * of either the left or right queryPlan that are composed by the binary
   * operator, i.e., already added to the final queryPlan
  ...
  QueryPlanPtr checkIfSourceIsAlreadyConsumedSource(
      std::basic_string<char> leftSourceName,
      std::basic_string<char> rightSourceName, QueryPlanPtr queryPlan) const;

 private:
  // pointers to track relationship of AST elements while retrieving pattern
  // specification
