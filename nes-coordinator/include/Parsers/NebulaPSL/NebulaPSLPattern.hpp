 ...
 * Attributes of this class represent the different clauses and a merge into a
 * query after parsing the AST
 */
class NebulaPSLPattern {
 public:
  ...
  void setOperatorList(
      const std::map<int32_t, NebulaPSLOperator>& operatorList);
  ...
  void setProjectionFields(
      const std::vector<ExpressionNodePtr>& projectionFields);
  ...
  /**
   * @brief inserts a new ExpressionNode (filter) to the list of parsed
  ...
  /**
   * @brief inserts a new ExpressionItem (Projection Attribute) to the  list of
   * specified output attributes
   * @param expressionItem
   */
  void addProjectionField(ExpressionNodePtr expressionNode);

  /**
   * @brief inserts a new Operator from the PATTERN clause to the list of
   * operators
   * @param opNode
   */
 ...
 private:
  ...
  std::map<int32_t, NebulaPSLOperator>
      operatorList;  // contains the operators from the PATTERN clause
