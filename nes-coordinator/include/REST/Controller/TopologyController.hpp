...
class TopologyController : public oatpp::web::server::api::ApiController {
 public:
   ...
   * @param errorHandler - responsible for handling errors
   */
  static std::shared_ptr<TopologyController> create(
  ...
  ENDPOINT("GET", "", getTopology) {
    try {
      ...
      NES_ERROR(
          "TopologyController: handleGet -getTopology: Exception occurred "
          "while building the "
          "topology: {}",
          exc.what());
  ...
  ENDPOINT("POST", "/addAsChild", addParent, BODY_STRING(String, request)) {
    try {
      ...
      if (added) {
        NES_DEBUG(
            "TopologyController::handlePost:addParent: created link "
      ...
      if (success) {
        NES_DEBUG(
            "TopologyController::handlePost:addParent: added link property for "
  ...
  ENDPOINT("DELETE", "/removeAsChild", removeParent,
    ...
    try {
      ...
      if (contained) {
        ...
        if (removed) {
          NES_DEBUG(
              "TopologyController::handlePost:removeParent: deleted link "
              "successfully");
        } else {
 ...
 private:
  ...
  validateRequest(nlohmann::json reqJson) {
    ...
    if (parentId == childId) {
      return errorHandler->handleError(
          Status::CODE_400,
          "Could not add/remove parent for node in topology: childId and "
          "parentId must be different.");
    }
