...
public:
    ...
    ENDPOINT("GET", "", getTopology)
    {
        ...
        {
            NES_ERROR(
                "TopologyController: handleGet -getTopology: Exception occurred while building the "
                "topology: {}",
                exc.what());
            return errorHandler->handleError(Status::CODE_500, "Exception occurred while building topology" + std::string(exc.what()));
        }
        catch (...)
        {
    ...
    ENDPOINT("POST", "/addAsChild", addParent, BODY_STRING(String, request))
    {
        try
        {
            ...
            else
            {
                ...
                return errorHandler->handleError(
                    Status::CODE_500, "TopologyController::handlePost:addParent: Failed to add link between parent and child nodes.");
            }
            ...
            {
                NES_DEBUG(
                    "TopologyController::handlePost:addParent: added link property for the link between the parent {} and "
                    "child {} nodes.",
                    parentId,
                    childId);
            }
            else
            {
                NES_ERROR("TopologyController::handlePost:addParent: Failed");
                return errorHandler->handleError(
                    Status::CODE_500, "TopologyController::handlePost:addParent: Failed to add link property.");
            }
...
private:
    std::optional<std::shared_ptr<oatpp::web::protocol::http::outgoing::Response>> validateRequest(nlohmann::json reqJson)
    {
        ...
        {
            return errorHandler->handleError(
                Status::CODE_400, "Could not add/remove parent for node in topology: childId and parentId must be different.");
        }
