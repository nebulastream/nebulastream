...
public:
   ...
   * @param errorHandler - responsible for handling errors
   */
  static std::shared_ptr<TopologyController>
  create(const std::shared_ptr<ObjectMapper> &objectMapper,
    ...
    return std::make_shared<TopologyController>(
        objectMapper, std::move(topology), completeRouterPrefix, errorHandler);
  }
  ...
  ENDPOINT("GET", "", getTopology) {
    try {
      ...
      NES_ERROR("TopologyController: handleGet -getTopology: Exception "
                "occurred while building the "
                "topology: {}",
                exc.what());
      return errorHandler->handleError(
          Status::CODE_500, "Exception occurred while building topology" +
                                std::string(exc.what()));
    } catch (...) {
  ...
  ENDPOINT("POST", "/addAsChild", addParent, BODY_STRING(String, request)) {
    try {
      ...
      for (const auto &child : children) {
        if (child == childId) {
          return errorHandler->handleError(
              Status::CODE_400,
              fmt::format(
                  "Could not add parent for node in topology: Node with "
      ...
      if (added) {
        NES_DEBUG("TopologyController::handlePost:addParent: created link "
                  "successfully new topology is=");
      } else {
        NES_ERROR("TopologyController::handlePost:addParent: Failed");
        return errorHandler->handleError(
            Status::CODE_500,
            "TopologyController::handlePost:addParent: Failed to add link "
      ...
      bool success =
          topology->addLinkProperty(parentId, childId, bandwidth, latency);
      if (success) {
        NES_DEBUG("TopologyController::handlePost:addParent: added link "
                  "property for the link between the parent {} and "
                  "child {} nodes.",
                  parentId, childId);
      } else {
        NES_ERROR("TopologyController::handlePost:addParent: Failed");
        return errorHandler->handleError(
            Status::CODE_500, "TopologyController::handlePost:addParent: "
                              "Failed to add link property.");
      }
  ...
  ENDPOINT("DELETE", "/removeAsChild", removeParent,
    ...
    try {
      ...
      if (contained) {
        ...
        if (removed) {
          NES_DEBUG("TopologyController::handlePost:removeParent: deleted link "
                    "successfully");
        } else {
          NES_ERROR("TopologyController::handlePost:removeParent: Failed");
          return errorHandler->handleError(
              Status::CODE_500,
              "TopologyController::handlePost:removeAsParent: Failed");
        }
        // Prepare the response
        nlohmann::json response;
        ...
        return errorHandler->handleError(
            Status::CODE_400,
            fmt::format(
                "Could not remove parent for node in topology: Node with "
  ...
  validateRequest(nlohmann::json reqJson) {
    ...
    if (!reqJson.contains("parentId")) {
      return errorHandler->handleError(Status::CODE_400,
                                       " Request body missing 'parentId'");
    }
    if (!reqJson.contains("childId")) {
      return errorHandler->handleError(Status::CODE_400,
                                       " Request body missing 'childId'");
    }
    WorkerId parentId = reqJson["parentId"].get<WorkerId>();
    WorkerId childId = reqJson["childId"].get<WorkerId>();
    if (parentId == childId) {
      return errorHandler->handleError(
          Status::CODE_400, "Could not add/remove parent for node in topology: "
                            "childId and parentId must be different.");
    }

    if (!topology->nodeWithWorkerIdExists(childId)) {
      return errorHandler->handleError(Status::CODE_400,
                                       "Could not add/remove parent for node "
                                       "in topology: Node with childId=" +
                                           childId.toString() + " not found.");
    }

    if (!topology->nodeWithWorkerIdExists(parentId)) {
      return errorHandler->handleError(Status::CODE_400,
                                       "Could not add/remove parent for node "
                                       "in topology: Node with parentId=" +
