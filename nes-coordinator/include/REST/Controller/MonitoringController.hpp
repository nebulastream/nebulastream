...
public:
  ...
  MonitoringController(const std::shared_ptr<ObjectMapper> &objectMapper,
      ...
      : oatpp::web::server::api::ApiController(objectMapper,
        ...
        monitoringService(mService), bufferManager(bManager),
   ...
   */
  static std::shared_ptr<MonitoringController>
  create(const std::shared_ptr<ObjectMapper> &objectMapper,
    ...
    return std::make_shared<MonitoringController>(
        objectMapper, mService, bManager, errorHandler, completeRouterPrefix);
  }
  ...
  ENDPOINT("GET", "/stop", getMonitoringControllerStop) {
    ...
    return errorHandler->handleError(
        Status::CODE_500, "Stopping monitoring service was not successful.");
  }
  ...
  ENDPOINT("GET", "/streams", getMonitoringControllerStreams) {
    ...
    if (response == nullptr) {
      return errorHandler->handleError(
          Status::CODE_500,
          "Getting streams of monitoring service was not successful.");
    }
  ...
  ENDPOINT("GET", "/storage", getMonitoringControllerStorage) {
    ...
    if (response == nullptr) {
      return errorHandler->handleError(
          Status::CODE_500, "Getting newest monitoring data from metric store "
                            "of monitoring service was not successful.");
    }
    return createResponse(Status::CODE_200, response.dump());
  }
  ...
  ENDPOINT("GET", "/metrics", getMonitoringControllerDataFromAllNodes) {
    ...
    if (response == nullptr) {
      return errorHandler->handleError(
          Status::CODE_500,
          "Getting monitoring data from all nodes was not successful.");
    }
    return createResponse(Status::CODE_200, response.dump());
  }
  ...
  ENDPOINT("GET", "/metrics", getMonitoringControllerDataFromOneNode,
    ...
    try {
      nlohmann::json response;
      response =
          monitoringService->requestMonitoringDataAsJson(WorkerId(nodeId));
      if (!(response == nullptr)) {
        return createResponse(Status::CODE_200, response.dump());
      }
