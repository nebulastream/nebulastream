...
public:
    ...
    ENDPOINT("GET", "/allRegisteredQueries", getAllRegisteredQueires)
    {
        ...
        {
            NES_ERROR(
                "QueryCatalogController: handleGet -allRegisteredQueries: Exception occurred while building the "
                "query plan for user request: {}",
                exc.what());
            return errorHandler->handleError(
                Status::CODE_400, "Exception occurred while building query plans for user request" + std::string(exc.what()));
        }
        catch (...)
        {
    ...
    ENDPOINT("GET", "/getNumberOfProducedBuffers", getNumberOfProducedBuffers, QUERY(UInt64, queryId, "queryId"))
    {
        try
        {
            ...
            if (auto shared_back_reference = coordinator.lock())
            {
                ...
                {
                    return errorHandler->handleError(
                        Status::CODE_404, "no statistics available for query with ID: " + std::to_string(queryId));
                }
                processedBuffers = shared_back_reference->getQueryStatistics(UNSURE_CONVERSION_TODO_4761(sharedQueryId, QueryId))[0]
                                       ->getProcessedBuffers();
            }
