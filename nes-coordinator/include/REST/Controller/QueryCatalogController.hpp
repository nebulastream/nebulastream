...
class QueryCatalogController : public oatpp::web::server::api::ApiController {
 public:
   ...
   * @param errorHandler - responsible for handling errors
   */
  static std::shared_ptr<QueryCatalogController> create(
  ...
  ENDPOINT("GET", "/allRegisteredQueries", getAllRegisteredQueires) {
    try {
      ...
      NES_ERROR(
          "QueryCatalogController: handleGet -allRegisteredQueries: Exception "
          "occurred while building the "
  ...
  ENDPOINT("GET", "/status", getStatusOfQuery,
    ...
    try {
      ...
      return errorHandler->handleError(
          Status::CODE_404,
          "No query with given ID: " + std::to_string(queryId));
    } catch (...) {
      return errorHandler->handleError(Status::CODE_500, "Internal Error");
    }
  }
  ...
  ENDPOINT("GET", "/getNumberOfProducedBuffers", getNumberOfProducedBuffers,
    ...
    try {
      ...
      if (sharedQueryId == INVALID_SHARED_QUERY_ID) {
        return errorHandler->handleError(
            Status::CODE_404,
            "no query found with ID: " + std::to_string(queryId));
      }
