...
public:
   ...
   * @param completeRouterPrefix - url consisting of base router prefix (e.g
   * "v1/nes/") and controller specific router prefix (e.g
  ...
  QueryCatalogController(const std::shared_ptr<ObjectMapper> &objectMapper,
      ...
      : oatpp::web::server::api::ApiController(objectMapper,
        ...
        queryCatalog(queryCatalog), coordinator(coordinator),
   ...
   * @param coordinator - central entity of NebulaStream
   * @param globalQueryPlan - responsible for storing all currently running and
   * to be deployed QueryPlans in the NES system.
   * @param routerPrefixAddition - controller specific router prefix (e.g
   * "connectivityController/")
   * @param errorHandler - responsible for handling errors
   */
  static std::shared_ptr<QueryCatalogController>
  create(const std::shared_ptr<ObjectMapper> &objectMapper,
    ...
    return std::make_shared<QueryCatalogController>(
        objectMapper, queryCatalog, coordinator, completeRouterPrefix,
        errorHandler);
  }
  ...
  ENDPOINT("GET", "/allRegisteredQueries", getAllRegisteredQueires) {
    try {
      ...
      NES_ERROR("QueryCatalogController: handleGet -allRegisteredQueries: "
                "Exception occurred while building the "
  ...
  ENDPOINT("GET", "/queries", getQueriesWithASpecificStatus,
  ...
  ENDPOINT("GET", "/status", getStatusOfQuery,
           QUERY(UInt64, queryId, "queryId")) {
    try {
  ...
  ENDPOINT("GET", "/getNumberOfProducedBuffers", getNumberOfProducedBuffers,
           QUERY(UInt64, queryId, "queryId")) {
    try {
      NES_DEBUG("getNumberOfProducedBuffers called");
      // Prepare Input query from user string
      auto sharedQueryId =
          queryCatalog->getLinkedSharedQueryId(QueryId(queryId));
      if (sharedQueryId == INVALID_SHARED_QUERY_ID) {
        return errorHandler->handleError(Status::CODE_404,
                                         "no query found with ID: " +
                                             std::to_string(queryId));
      }
      ...
      if (auto shared_back_reference = coordinator.lock()) {
        std::vector<Runtime::QueryStatisticsPtr> statistics =
            shared_back_reference->getQueryStatistics(
                UNSURE_CONVERSION_TODO_4761(sharedQueryId, QueryId));
        if (statistics.empty()) {
          return errorHandler->handleError(
              Status::CODE_404, "no statistics available for query with ID: " +
                                    std::to_string(queryId));
        }
        processedBuffers = shared_back_reference
                               ->getQueryStatistics(UNSURE_CONVERSION_TODO_4761(
                                   sharedQueryId, QueryId))[0]
                               ->getProcessedBuffers();
      }
