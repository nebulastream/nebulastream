...
public:
  ...
  UDFCatalogController(const std::shared_ptr<ObjectMapper> &objectMapper,
      ...
      : oatpp::web::server::api::ApiController(objectMapper,
        ...
        udfCatalog(udfCatalog), errorHandler(errorHandler) {}
   ...
   * @param errorHandler - responsible for handling errors
   */
  static std::shared_ptr<UDFCatalogController>
  create(const std::shared_ptr<ObjectMapper> &objectMapper,
    ...
    return std::make_shared<UDFCatalogController>(
        objectMapper, udfCatalog, completeRouterPrefix, errorHandler);
  }
   ...
   */
  ENDPOINT("GET", "/getUdfDescriptor", getUdfDescriptor,
           QUERY(String, udf, "udfName")) {
    try {
      std::string udfName = udf.getValue("");
      auto udfDescriptor = UDFDescriptor::as<JavaUDFDescriptor>(
          udfCatalog->getUDFDescriptor(udfName));
      GetJavaUdfDescriptorResponse response;
      if (udfDescriptor == nullptr) {
        // Signal that the UDF does not exist in the catalog.
        NES_DEBUG("REST client tried retrieving UDF descriptor for "
                  "non-existing Java UDF: {}",
                  udfName);
        response.set_found(false);
        return createResponse(Status::CODE_404, response.SerializeAsString());
      } else {
        // Return the UDF descriptor to the client.
        NES_DEBUG("Returning UDF descriptor to REST client for Java UDF: {}",
                  udfName);
        response.set_found(true);
        UDFSerializationUtil::serializeJavaUDFDescriptor(
            udfDescriptor, *response.mutable_java_udf_descriptor());
   ...
   * returns 400 if request body is emtpy or if errors occur parsing protobuf
   * message into a JavaUDFDescriptor object returns 500 for internal server
   * errors
   */
  ENDPOINT("POST", "/registerJavaUdf", registerJavaUdf,
           BODY_STRING(String, request)) {
    try {
      // Convert protobuf message contents to JavaUDFDescriptor.
      std::string body = request.getValue("");
      if (body.empty()) {
        errorHandler->handleError(Status::CODE_400,
      ...
      auto javaUdfDescriptor =
          UDFSerializationUtil::deserializeJavaUDFDescriptor(descriptorMessage);
