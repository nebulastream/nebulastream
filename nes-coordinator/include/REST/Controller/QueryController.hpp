...
class QueryController : public oatpp::web::server::api::ApiController {
 public:
   ...
   * @return
   */
  static std::shared_ptr<QueryController> create(
  ...
  ENDPOINT("POST", "/execute-query", submitQuery,
    ...
    try {
      ...
      if (requestJson.contains("placement")) {
        if (!validatePlacementStrategy(
                placementStrategyString =
                    requestJson["placement"].get<std::string>())) {
          NES_ERROR(
              "QueryController: handlePost -execute-query: Invalid Placement "
              "Strategy Type provided: {}",
              placementStrategyString);
      ...
      auto placement = magic_enum::enum_cast<Optimizer::PlacementStrategy>(
                           ...
                           .value();
      NES_DEBUG(
          "QueryController: handlePost -execute-query: Params: userQuery= {}, "
          "strategyName= {}",
      ...
      NES_ERROR(
          "QueryController: handlePost -execute-query: Exception occurred "
          "during submission of a query "
          "user request: {}",
          exc.what());
  ...
  ENDPOINT("POST", "/execute-query-ex", submitQueryProtobuf,
    ...
    try {
      ...
      NES_ERROR(
          "QueryController: handlePost -execute-query-ex: Exception occurred "
          "while building the query plan for "
          "user request: {}",
          exc.what());
      return errorHandler->handleError(Status::CODE_400, exc.what());
    } catch (...) {
 ...
 private:
  ...
  validateUserRequest(nlohmann::json userRequest) {
    if (!userRequest.contains("userQuery")) {
      NES_ERROR(
          "QueryController: handlePost -execute-query: Wrong key word for user "
          "query, use 'userQuery'.");
      std::string errorMessage =
    ...
    if (!userRequest.contains("placement")) {
      NES_ERROR(
          "QueryController: handlePost -execute-query: No placement strategy "
          "specified. Specify a placement strategy "
          "using 'placement'.");
      std::string errorMessage =
