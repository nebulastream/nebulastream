...
public:
        ...
        const ErrorHandlerPtr& errorHandler)
    {
        ...
        return std::make_shared<QueryController>(
            objectMapper, requestHandlerService, queryCatalog, globalQueryPlan, globalExecutionPlan, completeRouterPrefix, errorHandler);
    }
    ...
    ENDPOINT("POST", "/execute-query", submitQuery, BODY_STRING(String, request))
    {
        try
        {
            ...
            if (requestJson.contains("placement"))
            {
                ...
                {
                    NES_ERROR(
                        "QueryController: handlePost -execute-query: Invalid Placement Strategy Type provided: {}",
            ...
            NES_DEBUG(
                "QueryController: handlePost -execute-query: Params: userQuery= {}, strategyName= {}", userQuery, placementStrategyString);
        ...
        {
            NES_ERROR(
                "QueryController: handlePost -execute-query: Exception occurred during submission of a query "
                "user request: {}",
                exc.what());
