...
public:
  ...
  QueryController(const std::shared_ptr<ObjectMapper> &objectMapper,
      ...
      : oatpp::web::server::api::ApiController(objectMapper,
        ...
        queryCatalog(queryCatalog), globalQueryPlan(globalQueryPlan),
   ...
   * @return
   */
  static std::shared_ptr<QueryController>
  create(const std::shared_ptr<ObjectMapper> &objectMapper,
    ...
    return std::make_shared<QueryController>(
        objectMapper, requestHandlerService, queryCatalog, globalQueryPlan,
        globalExecutionPlan, completeRouterPrefix, errorHandler);
  }

  ENDPOINT("GET", "/execution-plan", getExecutionPlan,
    ...
    try {
      ...
      auto sharedQueryPlanId =
          globalQueryPlan->getSharedQueryId(QueryId(queryId));
      // Return the execution nodes running the shared query plan
      auto executionPlanJson =
          globalExecutionPlan->getAsJson(sharedQueryPlanId);
      NES_DEBUG("QueryController:: execution-plan: {}",
                executionPlanJson.dump());
  ...
  ENDPOINT("GET", "/query-plan", getQueryPlan,
           QUERY(UInt64, queryId, "queryId")) {
    try {
  ...
  ENDPOINT("GET", "/optimization-phase", getOptimizationPhase,
           QUERY(UInt64, queryId, "queryId")) {
    try {
      auto response = queryCatalog->getQueryEntry(QueryId(queryId));
      return createResponse(Status::CODE_200, response.dump());
  ...
  ENDPOINT("GET", "/query-status", getQueryStatus,
           QUERY(UInt64, queryId, "queryId")) {
    // NOTE: QueryController has "query-status" endpoint. QueryCatalogController
    // has "status" endpoint with same functionality. Functionality has been
  ...
  ENDPOINT("POST", "/execute-query", submitQuery,
    ...
    try {
      ...
      if (!validatePlacementStrategy(
              requestJson["placement"].get<std::string>())) {
        std::string errorMessage = "Invalid Placement Strategy: " +
                                   requestJson["placement"].get<std::string>() +
                                   ". Further info can be found at "
                                   "https://docs.nebula.stream/cpp/"
                                   "class_n_e_s_1_1_placement_strategy.html";
      ...
      if (requestJson.contains("placement")) {
        if (!validatePlacementStrategy(
                placementStrategyString =
                    requestJson["placement"].get<std::string>())) {
          NES_ERROR("QueryController: handlePost -execute-query: Invalid "
                    "Placement Strategy Type provided: {}",
                    placementStrategyString);
          std::string errorMessage =
              "Invalid Placement Strategy Type provided: " +
              placementStrategyString +
              ". Valid Placement Strategies are: 'IN_MEMORY', 'PERSISTENT', "
              "'REMOTE', 'NONE'.";
          return errorHandler->handleError(Status::CODE_400, errorMessage);
        } else {
      ...
      auto placement = magic_enum::enum_cast<Optimizer::PlacementStrategy>(
                           placementStrategyString)
                           .value();
      NES_DEBUG("QueryController: handlePost -execute-query: Params: "
                "userQuery= {}, strategyName= {}",
                userQuery, placementStrategyString);
      ...
      NES_ERROR("QueryController: handlePost -execute-query: Exception "
                "occurred during submission of a query "
                "user request: {}",
                exc.what());
  ...
  ENDPOINT("POST", "/execute-query-ex", submitQueryProtobuf,
           BODY_STRING(String, request)) {
    try {
      std::shared_ptr<SubmitQueryRequest> protobufMessage =
          std::make_shared<SubmitQueryRequest>();
      auto optional = validateProtobufMessage(protobufMessage, request);
      if (optional.has_value()) {
        return optional.value();
      }
      SerializableQueryPlan *queryPlanSerialized =
          protobufMessage->mutable_queryplan();
      QueryPlanPtr queryPlan(QueryPlanSerializationUtil::deserializeQueryPlan(
          queryPlanSerialized));
      auto *context = protobufMessage->mutable_context();

      std::string placementStrategyString = DEFAULT_PLACEMENT_STRATEGY_TYPE;
      if (context->contains("placement")) {
        if (!validatePlacementStrategy(placementStrategyString =
                                           context->at("placement").value())) {
          NES_ERROR("QueryController: handlePost -execute-query: Invalid "
                    "Placement Strategy Type provided: {}",
                    placementStrategyString);
          std::string errorMessage =
      ...
      NES_ERROR("QueryController: handlePost -execute-query-ex: Exception "
                "occurred while building the query plan for "
                "user request: {}",
                exc.what());
      return errorHandler->handleError(Status::CODE_400, exc.what());
    } catch (...) {
    ...
    try {
      ...
      auto placementStrategy =
          magic_enum::enum_cast<Optimizer::PlacementStrategy>(
              ...
              .value();
      nlohmann::json response =
          requestHandlerService->validateAndQueueExplainQueryRequest(
              queryPlan, placementStrategy);
  ...
  ENDPOINT("DELETE", "/stop-query", stopQuery,
           QUERY(UInt64, queryId, "queryId")) {
    try {
      bool success = requestHandlerService->validateAndQueueStopQueryRequest(
          QueryId(queryId));
      Status status =
          success
              ? Status::CODE_202
              : Status::CODE_400; // QueryController catches
                                  //  InvalidQueryStatus exception, but this is
                                  //  never thrown since it was commented out
      nlohmann::json response;
...
private:
  ...
  validateUserRequest(nlohmann::json userRequest) {
    if (!userRequest.contains("userQuery")) {
      NES_ERROR("QueryController: handlePost -execute-query: Wrong key word "
                "for user query, use 'userQuery'.");
      std::string errorMessage =
          "Incorrect or missing key word for user query, use 'userQuery'. For "
          "more info check "
          "https://docs.nebula.stream/docs/clients/rest-api/";
      return errorHandler->handleError(Status::CODE_400, errorMessage);
    }
    if (!userRequest.contains("placement")) {
      NES_ERROR("QueryController: handlePost -execute-query: No placement "
                "strategy specified. Specify a placement strategy "
                "using 'placement'.");
      std::string errorMessage =
          "No placement strategy specified. Specify a placement strategy using "
          "'placement'. For "
  ...
  bool validatePlacementStrategy(const std::string &placementStrategy) {
    return magic_enum::enum_cast<Optimizer::PlacementStrategy>(
               placementStrategy)
        .has_value();
  }

  std::optional<std::shared_ptr<oatpp::web::protocol::http::outgoing::Response>>
  validateProtobufMessage(
      const std::shared_ptr<SubmitQueryRequest> &protobufMessage,
    ...
    if (!context->contains("placement")) {
      ...
      std::string errorMessage = "No placement strategy specified. Specify a "
                                 "placement strategy using 'placementStrategy'."
                                 "More info at: "
                                 "https://docs.nebula.stream/cpp/"
                                 "class_n_e_s_1_1_placement_strategy.html";
      return errorHandler->handleError(Status::CODE_400, errorMessage);
    ...
    if (!validatePlacementStrategy(placementStrategy)) {
      std::string errorMessage =
          "Invalid Placement Strategy: " + placementStrategy +
          ". Further info can be found at "
          "https://docs.nebula.stream/cpp/"
          "class_n_e_s_1_1_placement_strategy.html";
      return errorHandler->handleError(Status::CODE_400, errorMessage);
