...
public:
   ...
   * @return
   */
  static std::shared_ptr<SourceCatalogController>
  create(const std::shared_ptr<ObjectMapper> &objectMapper,
    ...
    oatpp::String completeRouterPrefix =
        BASE_ROUTER_PREFIX + routerPrefixAddition;
    return std::make_shared<SourceCatalogController>(
        objectMapper, sourceCatalogService, queryParsingService, errorHandler,
        completeRouterPrefix);
  }
  ...
  ENDPOINT("GET", "/allLogicalSource", getAllLogicalSource) {
    try {
      ...
      const auto &allLogicalSourceAsString =
  ...
  ENDPOINT("GET", "/allPhysicalSource", getPhysicalSource,
    ...
    try {
      const std::vector<Catalogs::Source::SourceCatalogEntryPtr>
          &allPhysicalSource =
      ...
      return errorHandler->handleError(Status::CODE_404,
                                       "Resource Not Found: Logical source " +
                                           logicalSourceName +
                                           " has no physical source defined.");
    } catch (const std::exception &exc) {
      NES_ERROR("SourceCatalogController: get allPhysicalSource: Exception "
                "occurred while building the query plan for user "
                "request.");
      return errorHandler->handleError(Status::CODE_500, exc.what());
    } catch (...) {
      return errorHandler->handleError(
          Status::CODE_500, "SourceCatalogController:unknown exception.");
    }
  }
  ...
  ENDPOINT("GET", "/schema", getSchema,
           QUERY(String, logicalSourceName, "logicalSourceName")) {
    try {
      auto schema = sourceCatalogService->getLogicalSource(logicalSourceName)
                        ->getSchema();
      auto serializableSchema = SchemaSerializationUtil::serializeSchema(
          schema, new SerializableSchema());
      return createResponse(Status::CODE_200,
                            serializableSchema->SerializeAsString());
    } catch (const MapEntryNotFoundException &e) {
      return errorHandler->handleError(
          Status::CODE_404,
          "Resource Not Found: No Schema found for " + logicalSourceName);
    } catch (const std::exception &exc) {
      NES_ERROR("SourceCatalogController: get schema: Exception occurred while "
                "retrieving the schema for a logical source {}",
                exc.what());
      return errorHandler->handleError(Status::CODE_500, exc.what());
    } catch (...) {
      return errorHandler->handleError(
  ...
  ENDPOINT("POST", "/addLogicalSource", addLogicalSource,
           BODY_STRING(String, request)) {

    NES_DEBUG("SourceCatalogController: addLogicalSource: REST received "
              "request to add new Logical Source.");
    try {
      ...
      if (!reqJson.contains("logicalSourceName")) {
        return errorHandler->handleError(
            Status::CODE_400, "Request body must contain 'logicalSourceName'");
      }
      if (!reqJson.contains("schema")) {
        return errorHandler->handleError(Status::CODE_400,
                                         "Request body must contain 'schema'");
      }
      auto logicalSourceName = reqJson["logicalSourceName"];
      auto schemaString = reqJson["schema"];
      NES_DEBUG("SourceCatalogController: addLogicalSource: Try to add new "
                "Logical Source {} and {}",
                logicalSourceName, schemaString);
      auto schema = queryParsingService->createSchemaFromCode(schemaString);
      bool added = sourceCatalogService->registerLogicalSource(
          logicalSourceName, schema);
      NES_DEBUG("SourceCatalogController: addLogicalSource: Successfully added "
                "new logical Source ? {}",
                added);
      ...
      if (added) {
        ...
        return errorHandler->handleError(
            Status::CODE_400, "Logical Source with same name already exists!");
      }
    } catch (const std::exception &exc) {
      NES_ERROR("SourceCatalogController: addLogicalSource: Exception occurred "
                "while trying to add new "
                "logical source {}",
                exc.what());
      return errorHandler->handleError(Status::CODE_500, exc.what());
    } catch (...) {
      return errorHandler->handleError(
          Status::CODE_500,
          "RestServer: Unable to start REST server unknown exception.");
    }
  }

  ENDPOINT("POST", "/addLogicalSource-ex", addLogicalSourceEx,
           BODY_STRING(String, request)) {
    ...
    try {
      ...
      if (!protobufMessage->ParseFromArray(req.data(), req.size())) {
        NES_DEBUG("SourceCatalogController: handlePost -addLogicalSource: "
      ...
      NES_DEBUG("SourceCatalogController: handlePost -addLogicalSource: Start "
                "trying to add new logical source");
      // decode protobuf message into c++ obj repr
      auto deserializedSchema =
          SchemaSerializationUtil::deserializeSchema(protobufMessage->schema());
      auto sourceName = protobufMessage->sourcename();

      // try to add the user supplied source
      bool added = sourceCatalogService->registerLogicalSource(
          sourceName, deserializedSchema);
      NES_DEBUG("SourceCatalogController: handlePost -addLogicalSource: "
                "Successfully added new logical Source ? {}",
                added);
      ...
      NES_ERROR("SourceCatalogController: addLogicalSource-ex: Exception "
                "occurred while trying to add new "
                "logical source {}",
                exc.what());
      return errorHandler->handleError(Status::CODE_500, exc.what());
    } catch (...) {
  ...
  ENDPOINT("POST", "/updateLogicalSource", updateLogicalSource,
    ...
    NES_DEBUG("SourceCatalogController: updateLogicalSource: REST received "
              "request to update the given Logical Source.");
    try {
      std::string req = request.getValue("{}");
      // check if json is valid
      if (!nlohmann::json::accept(req)) {
      ...
      auto schemaString = reqJson["schema"];
      NES_DEBUG("SourceCatalogController: updateLogicalSource: Try to update  "
                "Logical Source {} with schema {}",
                sourceName, schemaString);
      auto schema = queryParsingService->createSchemaFromCode(schemaString);
      bool updated =
          sourceCatalogService->updateLogicalSource(sourceName, schema);
      NES_DEBUG("SourceCatalogController: addLogicalSource: Successfully added "
                "new logical Source ? {}",
                updated);
      // Prepare the response
      if (updated) {
        ...
        NES_DEBUG("SourceCatalogController: updateLogicalSource: unable to "
                  "find given source");
        return errorHandler->handleError(Status::CODE_400,
                                         "Unable to update logical source.");
      }
    } catch (const std::exception &exc) {
      NES_ERROR("SourceCatalogController: updateLogicalSource: Exception "
  ...
  ENDPOINT("POST", "/updateLogicalSource-ex", updateLogicalSourceEx,
           BODY_STRING(String, request)) {
    ...
    try {
      ...
      if (!protobufMessage->ParseFromArray(req.data(), req.size())) {
        NES_DEBUG("SourceCatalogController: handlePost "
                  "-updateLogicalSource-ex: invalid Protobuf message");
        nlohmann::json errorResponse{};
        errorResponse["detail"] = "Invalid Protobuf message";
        return errorHandler->handleError(Status::CODE_400,
                                         errorResponse.dump());
      ...
      NES_DEBUG("SourceCatalogController: handlePost -updateLogicalSource: "
                "Start trying to update logical source");
      // decode protobuf message into c++ obj repr
      auto deserializedSchema =
          SchemaSerializationUtil::deserializeSchema(protobufMessage->schema());
      auto sourceName = protobufMessage->sourcename();
      ...
      // try to add the user supplied source
      bool updated = sourceCatalogService->updateLogicalSource(
          sourceName, deserializedSchema);
      ...
      if (updated) {
        ...
        nlohmann::json errorResponse{};
        errorResponse["detail"] =
            "Unable to update logical source " + sourceName;
        return errorHandler->handleError(Status::CODE_400,
                                         errorResponse.dump());
      }
    } catch (const std::exception &exc) {
  ...
  ENDPOINT("DELETE", "/deleteLogicalSource", deleteLogicalSource,
           QUERY(String, logicalSourceName, "logicalSourceName")) {
    NES_DEBUG("SourceCatalogController: deleteLogicalSource: REST received "
              "request to delete the given Logical Source.");
    try {
      bool deleted =
          sourceCatalogService->unregisterLogicalSource(logicalSourceName);
      NES_DEBUG("SourceCatalogController: deleteLogicalSource: Successfully "
                "deleted the given logical Source: {}",
                deleted);
      // Prepare the response
      if (deleted) {
        nlohmann::json success;
        ...
        NES_DEBUG("SourceCatalogController: deleteLogicalSource: unable to "
                  "find given source");
        return errorHandler->handleError(
            Status::CODE_400,
            "Unable to delete logical source. Either logical source doesnt "
            "exist or"
            " there are still physical sources mapped to the logical source");
      }
    } catch (const std::exception &exc) {
      NES_ERROR("SourceCatalogController: deleteLogicalSource: Exception "
                "occurred while building the query plan for user "
                "request.");
      return errorHandler->handleError(Status::CODE_500, exc.what());
    } catch (...) {
  ...
  ENDPOINT("POST", "/addSourceStatistics", addSourceStatistics,
           BODY_STRING(String, request)) {
    NES_DEBUG("SourceCatalogController: addSourceStatistics: REST received "
              "request to add source statistics.");
    try {
      std::string req = request.getValue("{}");
      // check if json is valid
      if (!nlohmann::json::accept(req)) {
      ...
      for (const auto &phSource : physicalSources) {
        if (phSource->getLogicalSource()->getLogicalSourceName() ==
          ...
          success =
              sourceCatalogService->addKeyDistributionEntry(catalogEntry, vals);
          break;
        }
      }
      ...
      if (success) {
        ...
        return createResponse(Status::CODE_500,
                              "SourceCatalogController: addSourceStatistics: "
                              "failed with success=" +
                                  successJson.dump());
      }
    } catch (const std::exception &exc) {
      NES_ERROR(
          "SourceCatalogController: addSourceStatistics: Exception occurred {}",
          exc.what());
      return errorHandler->handleError(Status::CODE_500, exc.what());
    } catch (...) {
      return errorHandler->handleError(
