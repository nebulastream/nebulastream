...
public:
        ...
        const std::string& routerPrefixAddition)
    {
        ...
        return std::make_shared<SourceCatalogController>(
            objectMapper, sourceCatalogService, queryParsingService, errorHandler, completeRouterPrefix);
    }
    ...
    ENDPOINT("GET", "/allPhysicalSource", getPhysicalSource, QUERY(String, logicalSourceName, "logicalSourceName"))
    {
        ...
        catch (const MapEntryNotFoundException& e)
        {
            return errorHandler->handleError(
                Status::CODE_404, "Resource Not Found: Logical source " + logicalSourceName + " has no physical source defined.");
        }
    ...
    ENDPOINT("GET", "/schema", getSchema, QUERY(String, logicalSourceName, "logicalSourceName"))
    {
        ...
        {
            NES_ERROR(
                "SourceCatalogController: get schema: Exception occurred while retrieving the schema for a logical source {}", exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        }
        catch (...)
        {
    ...
    ENDPOINT("POST", "/addLogicalSource", addLogicalSource, BODY_STRING(String, request))
    {
        ...
        try
        {
            ...
            NES_DEBUG(
                "SourceCatalogController: addLogicalSource: Try to add new Logical Source {} and {}", logicalSourceName, schemaString);
        ...
        {
            NES_ERROR(
                "SourceCatalogController: addLogicalSource: Exception occurred while trying to add new "
                "logical source {}",
                exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        }
    ...
    ENDPOINT("POST", "/addLogicalSource-ex", addLogicalSourceEx, BODY_STRING(String, request))
    {
        ...
        {
            NES_ERROR(
                "SourceCatalogController: addLogicalSource-ex: Exception occurred while trying to add new "
                "logical source {}",
                exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        }
    ...
    ENDPOINT("POST", "/updateLogicalSource", updateLogicalSource, BODY_STRING(String, request))
    {
        ...
        try
        {
            ...
            auto schemaString = reqJson["schema"];
            NES_DEBUG(
                "SourceCatalogController: updateLogicalSource: Try to update  Logical Source {} with schema {}", sourceName, schemaString);
            auto schema = queryParsingService->createSchemaFromCode(schemaString);
        ...
        {
            NES_ERROR(
                "SourceCatalogController: updateLogicalSource: Exception occurred while updating "
                "Logical Source {}",
                exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        }
    ...
    ENDPOINT("POST", "/addSourceStatistics", addSourceStatistics, BODY_STRING(String, request))
    {
        ...
        try
        {
            ...
            std::vector<Catalogs::Source::SourceCatalogEntryPtr> physicalSources = sourceCatalogService->getPhysicalSources(logSourceName);
            ...
            else
            {
                return createResponse(
                    Status::CODE_500, "SourceCatalogController: addSourceStatistics: failed with success=" + successJson.dump());
            }
        }
        catch (const std::exception& exc)
        {
