...
/**
 * @brief Coordinator RPC server responsible for receiving requests over GRPC
 * interface
 */
class CoordinatorRPCServer final : public CoordinatorRPCService::Service {
 public:
   ...
   * @return success
   */
  Status RegisterWorker(ServerContext* context,
   ...
   * @return success
   */
  Status UnregisterWorker(ServerContext* context,
  ...
  Status UnregisterPhysicalSource(
      ServerContext* context, const UnregisterPhysicalSourceRequest* request,
   ...
   * @return success
   */
  Status ReplaceParent(ServerContext* context,
   ...
   * @return success
   */
  Status RemoveParent(ServerContext* context,
   ...
   * @param context: the server context
   * @param request that is sent from worker to the coordinator and filled with
   * information of the failed query (Ids of query, worker, etc. and error
   * message)
   * @param reply that is sent back from the coordinator to the worker to
   * confirm that notification was successful
   * @return success
   */
  ...
  /**
   * @brief RPC Call to get a list of field nodes within a defined radius around
   * a geographical location
   * @param context: the server context
   * @param request: that is sent from worker to the coordinator containing the
   * center of the query area and the radius
   * @param reply: that is sent back from the coordinator to the worker
   * containing the ids of all nodes in the defined area and their corresponding
   * locations
   * @return success
   */
  Status GetNodesInRange(ServerContext*, const GetNodesInRangeRequest* request,
                         GetNodesInRangeReply* reply) override;
   ...
   * @return success
   */
  Status SendErrors(ServerContext*, const SendErrorsMessage* request,
                    ErrorReply* reply) override;

  /**
   * Request if soft stop can be performed for the query
   * @param context : the server context
   * @param request : that is sent from worker to the coordinator and containing
   * the query id for which the soft stop to request
   * @param response : that is sent back from the coordinator to the worker if
   * soft stop can be processed or not
  ...
  /**
   * Notify coordinator that for a subquery plan the soft stop is triggered or
   * not
   * @param context : the server context
   * @param request : that is sent from worker to the coordinator and containing
   * the query id, sub query id, and if soft stop is triggered
   * @param response : that is sent back from the coordinator to the worker if
   * request is processed
   * @return true if coordinator successfully recorded the information else
   * false
   */
  ...
  /**
   * Notify coordinator that for a subquery plan the soft stop is completed or
   * not
   * @param context : the server context
   * @param request : that is sent from worker to the coordinator and containing
   * the query id, sub query id, and if soft stop is completed
   * @param response : that is sent back from the coordinator to the worker if
   * request is processed
  ...
  /**
   * @brief inform the coordinator that a mobile devices reconnect prediction
   * has changed
   * @param request : sent from worker to coordinator containing the id of the
   * mobile device and a list of the old scheduled reconnects to be removed as
   * well as the new scheduled reconnects to be added. With each reconnect
   * consisting of the id of the node which the mobile device expects to connect
   * to and the location and time at which the reconnect is expected to happen
   * @param reply : sent from coordinator to worker not containing any data
   * @return OK if the coordinator succesfully recorded the data, CANCELLED
   * otherwise
   */
   ...
   * @param request : sent from worker to coordinator containing the id of the
   * mobile device, its new location and the time when this location was
   * recorded
   * @param reply : sent from coordinator to worker containing no data
   * @return OK in any case
   */
  Status SendLocationUpdate(ServerContext*,
                            const LocationUpdateRequest* request,
                            LocationUpdateReply* reply) override;

  Status GetParents(ServerContext*, const GetParentsRequest* request,
                    GetParentsReply* reply) override;

  /**
   * @brief modify the topology by removing and adding links and then rerun an
   * incremental placement for queries that were sending data over one of the
   * removed links
   * @param request contains a list of topology links to remvoe and a list of
   * topology links to add
   * @param reply contains a boolean which is set to true if the operation
   * succeeded
   * @return OK on success, CANCELLED otherwise
   */
  Status RelocateTopologyNode(ServerContext*,
                              const NodeRelocationRequest* request,
                              NodeRelocationReply* reply) override;
