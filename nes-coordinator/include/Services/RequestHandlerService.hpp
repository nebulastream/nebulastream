...
using AbstractStatisticQueryGeneratorPtr =
...
/**
 * @brief: This class is responsible for handling requests related to
 * submitting, fetching information, and deleting different
 * queryIdAndCatalogEntryMapping, as well as modifying the topology.
 */
class RequestHandlerService {
...
public:
  explicit RequestHandlerService(
      ...
      const NES::RequestProcessor::AsyncRequestProcessorPtr
  ...
  /**
   * @brief Register the incoming query in the system by add it to the
   * scheduling queue for further processing, and return the query Id assigned.
   ...
   * @throws InvalidArgumentException : when the placement strategy is not
   * valid.
   */
  QueryId validateAndQueueAddQueryRequest(
   ...
   * @return query id
   */
  nlohmann::json validateAndQueueExplainQueryRequest(
      const QueryPlanPtr &queryPlan,
      const Optimizer::PlacementStrategy placementStrategy);

  /**
   * Register the incoming stop query request in the system by add it to the
   * scheduling queue for further processing.
   * @param queryId : query id of the query to be stopped.
   * @returns: true if successful
   * @throws QueryNotFoundException : when query id is not found in the query
   * catalog.
   * @throws InvalidQueryStateException : when the query is found to be in an
   * invalid state.
   */
   ...
   *
   * @warning: this method is primarily designed to be called only by the
   * system.
   *
   * @param sharedQueryId : shared query plan id of the shared query plan to be
   * stopped.
   ...
   */
  bool
  validateAndQueueFailQueryRequest(SharedQueryId sharedQueryId,
  ...
  /**
   * @brief Process multiple query and topology change request represented by
   * isqp events in a batch
   * @param isqpEvents a vector of ISQP requests to be handled
   * @return true on success
   */
  bool queueISQPRequest(
      const std::vector<RequestProcessor::ISQPEventPtr> &isqpEvents);

  /**
   * @brief Processes a track requests by mapping it to a statistic query and
   * returning the statistic keys
   * @param characteristic: What type of the statistic is being tracked, i.e,
   * data, infrastructure, or workload
   * @param window: Over what window to track the statistics over
   * @param triggerCondition: Condition to trigger the callback function. Is
   * being called, if a statistic is created
   * @param sendingPolicy: Policy to send the data to the statistic sink
   * @param callBack: Function that should be called, if triggerCondition
   * evaluates to true
  ...
  trackStatisticRequest(const Statistic::CharacteristicPtr &characteristic,
                        const Windowing::WindowTypePtr &window);
