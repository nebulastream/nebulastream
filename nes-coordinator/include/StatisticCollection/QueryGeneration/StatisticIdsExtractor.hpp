...
class StatisticIdsExtractor {
 public:
  static constexpr auto DEFAULT_TIMEOUT_QUERY_STARTED =
      std::chrono::milliseconds(10000);
  /**
   * @brief Extracts the statistic ids of the operator over which some statistic
   * is being built
   * @param queryCatalog: In this catalog, we search for the queryPlan with the
   * queryId and then extract statistic ids in this queryPlan
   * @param queryId: QueryId to extract the statistic ids from its queryPlan
   * @param timeout: Timeout for the query to be in the RUNNING state. Default
   * is 10000ms
   */
  static std::vector<StatisticId> extractStatisticIdsFromQueryId(
      Catalogs::Query::QueryCatalogPtr queryCatalog, const QueryId& queryId,
   ...
   */
  static std::vector<StatisticId> extractStatisticIdsFromQueryPlan(
