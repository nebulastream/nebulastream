...
public:
   ...
   * @param failedDecomposedPlanId: The id of the decomposed plan that caused
   * the failure
  ...
  FailQueryRequest(SharedQueryId sharedQueryId,
                   ...
                   const std::string &failureReason, uint8_t maxRetries);

  /**
   * @brief creates a new FailQueryRequest object
   * @param sharedQueryId: The id of the shared query that failed
   * @param failedDecomposedQueryId: The id of the decomposed plan that caused
   * the failure
   * @param failureReason: the failure reason
   * @param maxRetries: Maximum number of retry attempts for the request
   * @return a smart pointer to the newly created object
   */
  static FailQueryRequestPtr
  create(SharedQueryId sharedQueryId,
