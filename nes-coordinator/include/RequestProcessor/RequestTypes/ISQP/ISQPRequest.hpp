...
/**
 * @brief Response to the execution of the ISQP request with the success, start
 * time, and end time.
 */
struct ISQPRequestResponse : AbstractRequestResponse {
  explicit ISQPRequestResponse(uint64_t processingStartTime,
                               ...
                               uint64_t numOfFailedPlacements, bool success)
...
/**
 * @brief This is a meta request that is capable of handling a batch of external
 * events. For example, the system can either execute 10 individual add query
 * requests or one ISQPRequest with a batch of 10 ISQPAddQueryEvents. Both will
 * result in deployment of 10 queries. However, batch based deployment will
 * result in a lower deployment latency. Similarly, we can submit an arbitrary
 * batch consisting of AddQuery, RemoveQuery, AddNode, RemoveNode, AddLink, or
 * RemoveLink events.
 */
class ISQPRequest : public AbstractUniRequest {
 public:
  ISQPRequest(const z3::ContextPtr& z3Context, std::vector<ISQPEventPtr> events,
              uint8_t maxRetries);

  static ISQPRequestPtr create(const z3::ContextPtr& z3Context,
 ...
 private:
  ...
  /**
   * @brief handle add query event by marking the affected operators for
   * placement
   * @param addQueryEvent
   */
  ...
  /**
   * @brief handle remove query event by marking the affected operators for
   * removal
  ...
  /**
   * @brief handle remove node event by marking the affected operators for
   * replacement
  ...
  /**
   * @brief handle remove link event by marking the affected operators for
   * replacement
