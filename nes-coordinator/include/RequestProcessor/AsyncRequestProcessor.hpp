...
/**
 * @brief This class asynchronously processes request to be executed by the
 * coordinator. Requests can spawn new requests if the logic requires follow up
 * actions.
 */
class AsyncRequestProcessor {
  // define an empty request type that does nothing and is used only for
  // flushing the executor
  ...
  public:
    ...
    // request type uses exception_ptr so it can set the exception state on its
    // response promise without casting or slicing
    std::vector<AbstractRequestPtr>
    rollBack(std::exception_ptr, const StorageHandlerPtr &) override {
      return {};
    }
...
public:
  /**
   * @brief constructor
   * @param storageDataStructures a struct containing pointers to the data
   * structures on which the requests operate
   */
  explicit AsyncRequestProcessor(StorageDataStructures &storageDataStructures);

  /**
   * @brief Submits a request to the executor to be executed when a thread picks
   * it up
  ...
  /**
   * @brief stop execution, clear queue and terminate threads. Only the first
   * invocation has an effect. Subsequent calls have no effect.
...
private:
  /**
   * @brief The routine executed by each thread. Retrieves requests from the
   * queue, executes them and inserts follow up requests into the queue. This is
   * repeated until the executor is shut down by calling destroy()
   */
  void runningRoutine();
