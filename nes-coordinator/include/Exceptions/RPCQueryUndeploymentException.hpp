...
/**
 * @brief This exception indicates that an rpc to undeploy a query from a worker
 * has failed
 */
// todo 3915: check if this class can be generalized to include also rpc
...
public:
   ...
   * @param failedRpcWorkerIds: the execution node ids of the workers that could
   * not be reached vie rpc
   * @param mode: the mode indicating if the rpc was a register, unregister,
   * start or stop operation
   */
  explicit RPCQueryUndeploymentException(
