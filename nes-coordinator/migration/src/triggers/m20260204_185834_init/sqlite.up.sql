...
BEGIN
    SELECT CASE
        WHEN OLD.current_state = 'Pending' AND NEW.current_state NOT IN ('Registered', 'Stopped', 'Failed') THEN
            RAISE(ABORT, 'Invalid query_fragment state transition: Pending must transition to one of (Registered, Stopped, Failed)')

        WHEN OLD.current_state = 'Registered' AND NEW.current_state NOT IN ('Pending', 'Running', 'Stopped', 'Failed') THEN
            RAISE(ABORT, 'Invalid query_fragment state transition: Registered must transition to one of (Pending, Running, Stopped, Failed)')
    ...
    SET state = COALESCE(
        (SELECT CASE
            ...
            WHEN NOT EXISTS (SELECT 1 FROM query_fragment WHERE query_id = NEW.query_id AND current_state NOT IN ('Running', 'Completed'))
                THEN 'Running'
            WHEN NOT EXISTS (SELECT 1 FROM query_fragment WHERE query_id = NEW.query_id AND current_state NOT IN ('Registered', 'Running', 'Completed'))
                THEN 'Registered'
            ELSE NULL
        END),
...
-- one query_fragment), force every other non-terminal query_fragment of that
