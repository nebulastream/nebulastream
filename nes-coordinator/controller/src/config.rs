...
/// Applies to the controller and the per-worker loops.
pub(crate) const POLL_INTERVAL: Duration = Duration::from_secs(5);

/// How long a fragment lifecycle waits between status reads
/// of a worker that it reaches over the network.
/// Each read is a round trip,
/// so the interval trades how long a finished fragment goes unnoticed against the load of asking.
pub(crate) const REMOTE_FRAGMENT_POLL_INTERVAL: Duration = Duration::from_secs(5);

/// The same wait for a worker in the coordinator's own process.
/// The read is a function call rather than a round trip, so it costs almost nothing
/// and the interval is set by how promptly a finished fragment should be noticed.
...
/// Retryable failures in a row that a fragment tolerates before it fails.
...
/// Deadline for reaching one worker and reading the version that it runs.
/// Short and tried once, because a caller asked a question about every worker
/// and would rather be told promptly that one is unreachable
/// than wait out the retries that starting a query is worth.
