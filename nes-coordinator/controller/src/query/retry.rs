...
pub(crate) enum RetryPolicy {
    Transition,
    Rollback { worker_sm: Arc<WorkerStateManager> },
}
        ...
        Rsp: Send + 'static,
    {
        ...
        match result {
            Err(e) if e.retryable() => {
                if let Self::Rollback { worker_sm } = self
                    && is_worker_removed(worker_sm, &fragment.host_addr).await
                {
...
async fn is_worker_removed(worker_sm: &WorkerStateManager, host_addr: &NetworkAddr) -> bool {
    match worker_sm
