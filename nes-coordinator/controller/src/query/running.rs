...
fn apply_status_reply(
    ...
    let state: FragmentState = reply.state.into();
    ...
    update.current_state = Set(state);
            ...
            .apply_rpc_results(
                ...
                |fragment, result| match result {
                    ...
                    Err(e) if e.fragment_not_found() => {
                        ...
                        let mut update: fragment::ActiveModel = fragment.clone().into();
                        update.current_state = Set(FragmentState::Pending);
                        Some(update)
                    }
                    Err(e) => {
