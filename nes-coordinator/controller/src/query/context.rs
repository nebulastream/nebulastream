...
fn transition_update(
    ...
    let mut update: fragment::ActiveModel = fragment.clone().into();
    match result {
        Ok(_) => {
            update.current_state = Set(target);
            let now = chrono::Local::now();
            if target == FragmentState::Started && fragment.start_timestamp.is_none() {
                update.start_timestamp = Set(Some(now));
            }
            if target.is_terminal() && fragment.stop_timestamp.is_none() {
                update.stop_timestamp = Set(Some(now));
            }
        }
        Err(e) if e.retryable() => return None,
        Err(e) if e.fragment_not_found() => update.current_state = Set(FragmentState::Pending),
        Err(e) => {
            update.current_state = Set(FragmentState::Failed);
            update.error = Set(Some(FragmentError::from(e)));
            if fragment.stop_timestamp.is_none() {
...
pub(crate) struct QueryContext {
    ...
    pub(crate) query_sm: Arc<QueryStateManager>,
    pub(crate) worker_sm: Arc<WorkerStateManager>,
...
impl QueryContext {
    ...
    fn rollback_retry(&self) -> RetryPolicy {
        RetryPolicy::Rollback {
            worker_sm: self.worker_sm.clone(),
        }
    }
