...
pub(super) trait Client: Send + Sync {
    fn register(&self, id: QueryFragmentId, plan: &[u8]) -> impl Future<Output = Outcome> + Send;
    fn start(&self, id: QueryFragmentId) -> impl Future<Output = Outcome> + Send;
    fn stop(&self, id: QueryFragmentId, mode: StopMode) -> impl Future<Output = Outcome> + Send;
    fn observe(
        &self,
        id: QueryFragmentId,
