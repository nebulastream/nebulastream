...
use model::query::fragment::{
    DesiredFragmentState, FragmentError, FragmentTransition, StopMode,
};
...
impl FragmentClient for RemoteFragmentClient {
    ...
    async fn start(&self, id: FragmentId) -> Outcome {
        ...
        match self
            ...
            .await
        {
            Ok(_) => Outcome::Transition(FragmentTransition::running_now()),
            Err(err) if err.retryable() => Outcome::Retry(FragmentError::from(&err)),
            Err(err) if err.is_not_found() => Outcome::Transition(FragmentTransition::Pending),
            Err(err) => Outcome::Failed(FragmentError::from(&err)),
        }
    ...
    async fn stop(&self, id: FragmentId, mode: StopMode) -> Outcome {
            ...
            .await
        {
            ...
            Err(err) if err.is_not_found() => Outcome::Transition(FragmentTransition::stopped_now()),
