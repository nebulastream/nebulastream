...
use crate::fragment::{FragmentTask, RawStatus};
...
/// One worker's reconciliation task for the in-process backend:
/// one lifecycle driver per actionable fragment.
pub(super) struct InProcessWorkerTask {
    worker: worker::Model,
    handle: Arc<dyn InProcessWorker>,
...
impl Reconciler for InProcessWorkerTask {
    ...
    async fn reconcile(&mut self) {
        ...
        for fragment in fragments {
            ...
            self.fragments.spawn_if_untracked(id, move || {
                let client = FragmentClient::new(InProcessTransport::new(handle));
