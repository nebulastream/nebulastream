...
TEST_F(QueryFailureTest, testQueryFailureForFaultySource) {
  ...
  std::string outputFilePath =
      getTestResourceFolder() / "testDeployTwoWorkerMergeUsingBottomUp.out";

  NES_INFO("QueryFailureTest: Submit query");
  string query =
      R"(Query::from("test").filter(Attribute("value")>2).sink(FileSinkDescriptor::create(")" +
...
TEST_F(QueryFailureTest, testExecutingOneFaultAndOneCorrectQuery) {
  ...
  auto defaultSourceType =
      DefaultSourceType::create("default_logical", "default_source");
  workerConfig1->physicalSourceTypes.add(csvSourceType);
  ...
  NES_INFO("QueryFailureTest: Submit query");
  string query1 = R"(Query::from("test").sink(FileSinkDescriptor::create(")" +
