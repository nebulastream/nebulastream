...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ExplainRequestIntegrationTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup ExplainRequestIntegrationTest test class.");
  }

  static void TearDownTestCase() {
    NES_INFO("Tear down ExplainRequestIntegrationTest class.");
  }
};
...
TEST_F(ExplainRequestIntegrationTest, executeExplainRequest) {
  ...
  auto query1 = Query::from("test_source")
                    .project(Attribute("id"))
                    .sink(NullOutputSinkDescriptor::create());
  auto response1 = requestHandlerService->validateAndQueueExplainQueryRequest(
  ...
  auto query2 = Query::from("test_source")
                    .project(Attribute("id"))
                    .sink(NullOutputSinkDescriptor::create());
  auto response2 = requestHandlerService->validateAndQueueExplainQueryRequest(
