...
class NetworkStackIntegrationTest : public Testing::BaseIntegrationTest,
 ...
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NetworkStackIntegrationTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("TearDownTestCase NetworkStackIntegrationTest.");
  }
...
class TestSink : public SinkMedium {
 public:
  ...
  TestSink(const SchemaPtr& schema, Runtime::NodeEnginePtr nodeEngine,
      ...
      : SinkMedium(std::make_shared<NesFormat>(schema, bufferManager),
                   nodeEngine, numOfProducers, sharedQueryId,
  ...
  bool writeData(Runtime::TupleBuffer& input_buffer,
    ...
    auto rowLayout = Runtime::MemoryLayouts::RowLayout::create(
        getSchemaPtr(), input_buffer.getBufferSize());
    auto testTupleBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer(rowLayout, input_buffer);
...
void fillBuffer(TupleBuffer& buf,
                const Runtime::MemoryLayouts::RowLayoutPtr& memoryLayout) {
  auto recordIndexFields =
      Runtime::MemoryLayouts::RowLayoutField<int64_t, true>::create(
          0, memoryLayout, buf);
  auto fields01 = Runtime::MemoryLayouts::RowLayoutField<int64_t, true>::create(
      1, memoryLayout, buf);
  auto fields02 = Runtime::MemoryLayouts::RowLayoutField<int64_t, true>::create(
      2, memoryLayout, buf);
...
std::shared_ptr<MockedNodeEngine> createMockedEngine(
    const std::string& hostname, uint16_t port, uint64_t bufferSize,
  ...
  try {
    class DummyQueryListener : public AbstractQueryStatusListener {
     public:
      ...
      bool canTriggerEndOfStream(SharedQueryId, DecomposedQueryPlanId,
                                 OperatorId,
                                 Runtime::QueryTerminationType) override {
        return true;
      }
      bool notifySourceTermination(SharedQueryId, DecomposedQueryPlanId,
                                   OperatorId,
                                   Runtime::QueryTerminationType) override {
        return true;
      }
      bool notifyQueryFailure(SharedQueryId, DecomposedQueryPlanId,
                              std::string) override {
        return true;
      }
    ...
    auto defaultSourceType =
        DefaultSourceType::create("default_logical", "default");
    ...
    auto queryManager = std::make_shared<Runtime::DynamicQueryManager>(
        std::make_shared<DummyQueryListener>(), bufferManagers,
        INVALID_WORKER_NODE_ID, 1, hwManager, 100);
    auto networkManagerCreator = [=](const Runtime::NodeEnginePtr& engine) {
      return Network::NetworkManager::create(
          INVALID_WORKER_NODE_ID, hostname, port,
    ...
    auto compiler =
        QueryCompilation::NautilusQueryCompiler::create(options, phaseFactory);

    return std::make_shared<MockedNodeEngine>(
        std::move(physicalSources), std::move(hwManager),
        std::move(bufferManagers), std::move(queryManager),
...
TEST_P(NetworkStackIntegrationTest, testNetworkSourceSink) {
  ...
  NodeLocation nodeLocationSource{INVALID_WORKER_NODE_ID, "127.0.0.1",
                                  *dataPort1};
  NodeLocation nodeLocationSink{INVALID_WORKER_NODE_ID, "127.0.0.1",
                                *dataPort2};

  NesPartition nesPartition{SharedQueryId(1), OperatorId(22), PartitionId(33),
                            SubpartitionId(44)};
  ThreadBarrierPtr sinkShutdownBarrier =
      std::make_shared<ThreadBarrier>(numSendingThreads + 1);
  ...
  class MockedNodeEngine : public Runtime::NodeEngine {
   public:
    ...
    explicit MockedNodeEngine(
        ...
        std::function<Network::NetworkManagerPtr(NES::Runtime::NodeEnginePtr)>&&
            networkManagerCreator,
        Network::PartitionManagerPtr&& partitionManager,
        QueryCompilation::QueryCompilerPtr&& queryCompiler,
        std::promise<bool>& completed, NesPartition nesPartition,
        std::atomic<int>& bufferCnt)
        : NodeEngine(std::move(physicalSources), std::move(hardwareManager),
                     std::move(bufferManagers), std::move(queryManager),
                     std::move(networkManagerCreator),
                     std::move(partitionManager), std::move(queryCompiler),
                     std::make_shared<DummyQueryListener>(),
                     std::make_shared<NES::Runtime::OpenCLManager>(),
                     INVALID_WORKER_NODE_ID, 64, 64, 12, false),
          nesPartition(nesPartition),
          completed(completed),
          bufferCnt(bufferCnt) {}
    ...
    void onServerError(Network::Messages::ErrorMessage ex) override {
      if (ex.getErrorType() !=
          Messages::ErrorType::PartitionNotRegisteredError) {
  ...
  try {
    std::thread receivingThread([&]() {
      auto recvEngine = createMockedEngine<MockedNodeEngine>(
          "127.0.0.1", *dataPort1, bufferSize, buffersManaged, completed,
          nesPartition, bufferCnt);
      // register the incoming channel
      auto sink = std::make_shared<NullOutputSink>(
          recvEngine, 1, INVALID_SHARED_QUERY_ID,
          INVALID_DECOMPOSED_QUERY_PLAN_ID);
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> succ = {
          sink};
      auto uniqueId = OperatorId(1);
      auto source = std::make_shared<NetworkSource>(
          schema, recvEngine->getBufferManager(), recvEngine->getQueryManager(),
          recvEngine->getNetworkManager(), nesPartition, nodeLocationSink, 64,
          NSOURCE_RETRY_WAIT, NSOURCE_RETRIES, std::move(succ), INITIAL_VERSION,
          uniqueId);
      auto qep = Runtime::Execution::ExecutableQueryPlan::create(
          INVALID_SHARED_QUERY_ID, INVALID_DECOMPOSED_QUERY_PLAN_ID, {source},
          {sink}, {}, recvEngine->getQueryManager(),
          recvEngine->getBufferManager());
      recvEngine->getQueryManager()->registerQuery(qep);
      ASSERT_EQ(
          recvEngine->getPartitionManager()->getConsumerRegistrationStatus(
              nesPartition),
          PartitionRegistrationStatus::Registered);
      completed.get_future().get();
      sinkShutdownBarrier->wait();
      while (!(qep->getStatus() ==
                   Runtime::Execution::ExecutableQueryPlanStatus::Stopped ||
               qep->getStatus() ==
    ...
    auto nodeEngineBuilder2 =
        Runtime::NodeEngineBuilder::create(workerConfig2)
            .setQueryStatusListener(std::make_shared<DummyQueryListener>());
    auto sendEngine = nodeEngineBuilder2.build();

    // when async connecting is used, the sink will not finish the normal
    // connecting process this is because the sending threads do not mimic the
    // behaviour of the actual worker thread pool so far that the the threads
    // which write the data receive the reconfiguration messages. The test
    // passes because the sinks shutdown sequence makes a connection attempt in
    // order to flush the data, not because it connects normally
    auto networkSink = std::make_shared<NetworkSink>(
        schema, INVALID_OPERATOR_ID, INVALID_SHARED_QUERY_ID,
        INVALID_DECOMPOSED_QUERY_PLAN_ID, nodeLocationSource, nesPartition,
        sendEngine, 1, NSOURCE_RETRY_WAIT, NSOURCE_RETRIES,
        DEFAULT_NUMBER_OF_ORIGINS, INITIAL_VERSION);
    networkSink->preSetup();
    for (int threadNr = 0; threadNr < numSendingThreads; threadNr++) {
      std::thread sendingThread([&] {
        // register the incoming channel
        Runtime::WorkerContext workerContext(
            Runtime::NesThread::getId(), sendEngine->getBufferManager(), 64);
        auto rt = Runtime::ReconfigurationMessage(
            INVALID_SHARED_QUERY_ID, INVALID_DECOMPOSED_QUERY_PLAN_ID,
            Runtime::ReconfigurationType::Initialize, networkSink,
        ...
        auto rt2 = Runtime::ReconfigurationMessage(
            INITIAL_SHARED_QUERY_ID, INVALID_DECOMPOSED_QUERY_PLAN_ID,
            Runtime::ReconfigurationType::SoftEndOfStream, networkSink);
 ...
 public:
  explicit TestSourceWithLatch(
      ...
      : DefaultSource(schema, bufferManager, queryManager,
                      /*bufferCnt*/ 1,
                      /*frequency*/ 0, operatorId,
                      /*oridingid*/ INVALID_ORIGIN_ID, INVALID_STATISTIC_ID,
...
TEST_F(NetworkStackIntegrationTest, testQEPNetworkSinkSource) {
  ...
  auto numThreads = 8;
  SchemaPtr schema =
      TestSchemas::getSchemaTemplate("id_one_val_64")->updateSourceName("test");

  std::vector<PhysicalSourceTypePtr> physicalSourceTypes;
  for (auto i = 0; i < numQueries; ++i) {
    auto str = std::to_string(i);
    auto defaultSourceType =
        DefaultSourceType::create("default_logical"s + str, "default"s + str);
  ...
  workerConfiguration1->numberOfBuffersInGlobalBufferManager.setValue(
      buffersManaged);
  ...
  auto nodeEngineSender =
      Runtime::NodeEngineBuilder::create(workerConfiguration1)
          .setQueryStatusListener(std::make_shared<DummyQueryListener>())
          .build();
  ...
  workerConfiguration2->numberOfBuffersInGlobalBufferManager.setValue(
      buffersManaged);
  ...
  auto nodeEngineReceiver =
      Runtime::NodeEngineBuilder::create(workerConfiguration2)
          .setQueryStatusListener(std::make_shared<DummyQueryListener>())
          .build();
  ...
  for (auto i = 1; i <= numQueries; ++i) {
    NesPartition nesPartition{SharedQueryId(i), OperatorId(i * 22),
                              PartitionId(i * 33), SubpartitionId(i * 44)};
    // create NetworkSink
    auto networkSourceDescriptor1 = std::make_shared<
        TestUtils::TestSourceDescriptor>(
        schema,
        [&](SchemaPtr schema, OperatorId, OriginId, StatisticId,
            const SourceDescriptorPtr&, const Runtime::NodeEnginePtr&,
            size_t numSourceLocalBuffers,
            const std::vector<Runtime::Execution::SuccessorExecutablePipeline>&
                successors) -> DataSourcePtr {
          auto uniqueId = OperatorId(1);
          return std::make_shared<NetworkSource>(
              schema, nodeEngineReceiver->getBufferManager(),
              nodeEngineReceiver->getQueryManager(), netManagerReceiver,
              nesPartition, nodeLocationSender, numSourceLocalBuffers,
              NSOURCE_RETRY_WAIT, NSOURCE_RETRIES, successors, 0, uniqueId);
        });

    auto testSink = std::make_shared<TestSink>(
        schema, nodeEngineReceiver, nodeEngineReceiver->getBufferManager(), 1,
        SharedQueryId(i), DecomposedQueryPlanId(subPlanId));
    finalSinks.emplace_back(testSink);
    auto testSinkDescriptor =
        std::make_shared<TestUtils::TestSinkDescriptor>(testSink);

    auto query =
        TestQuery::from(networkSourceDescriptor1).sink(testSinkDescriptor);
    auto decomposedQueryPlan = DecomposedQueryPlan::create(
        DecomposedQueryPlanId(subPlanId++), SharedQueryId(i),
    ...
    auto request = QueryCompilation::QueryCompilationRequest::create(
        decomposedQueryPlan, nodeEngineReceiver);
    ...
    auto testSourceDescriptor =
        std::make_shared<TestUtils::TestSourceDescriptor>(
            schema,
            [&](SchemaPtr schema, OperatorId, OriginId, StatisticId,
                const SourceDescriptorPtr&, const Runtime::NodeEnginePtr&,
                size_t numSourceLocalBuffers,
                std::vector<Runtime::Execution::SuccessorExecutablePipeline>
                    successors) -> DataSourcePtr {
              return std::make_shared<detail::TestSourceWithLatch>(
                  schema, nodeEngineSender->getBufferManager(),
                  nodeEngineSender->getQueryManager(), OperatorId(1 + i),
                  numSourceLocalBuffers, std::move(successors), latch);
            });

    auto networkSink = std::make_shared<NetworkSink>(
        schema, OperatorId(i), SharedQueryId(i),
        DecomposedQueryPlanId(subPlanId), nodeLocationReceiver, nesPartition,
        nodeEngineSender, 1, NSOURCE_RETRY_WAIT, NSOURCE_RETRIES,
        DEFAULT_NUMBER_OF_ORIGINS, INITIAL_VERSION);
    auto networkSinkDescriptor =
        std::make_shared<TestUtils::TestSinkDescriptor>(networkSink);
    auto query2 = TestQuery::from(testSourceDescriptor)
                      .filter(Attribute("id") < 5)
                      .sink(networkSinkDescriptor);
    auto decomposedQueryPlan2 = DecomposedQueryPlan::create(
        DecomposedQueryPlanId(subPlanId++), SharedQueryId(i),
        INVALID_WORKER_NODE_ID, query2.getQueryPlan()->getRootOperators());
    auto queryPlan2 = typeInferencePhase->execute(query2.getQueryPlan());
    queryPlan2->setQueryId(QueryId(i));
    auto request2 = QueryCompilation::QueryCompilationRequest::create(
        decomposedQueryPlan2, nodeEngineSender);
    ...
    auto f1 = std::async(std::launch::async, func, nodeEngineSender,
                         builderGeneratorQEP);
    auto f2 = std::async(std::launch::async, func, nodeEngineReceiver,
                         builderReceiverQEP);

    ASSERT_TRUE(f1.get());
    ASSERT_TRUE(f2.get());
    ASSERT_TRUE(nodeEngineSender->startQuery(
        builderGeneratorQEP->getSharedQueryId(),
        builderGeneratorQEP->getDecomposedQueryPlanId()));
    ASSERT_TRUE(nodeEngineReceiver->startQuery(
        builderReceiverQEP->getSharedQueryId(),
...
namespace detail {
struct TestEvent {
  explicit TestEvent(Runtime::EventType ev, uint32_t test)
      : ev(ev), test(test) {}
...
TEST_F(NetworkStackIntegrationTest, testSendEvent) {
  ...
  auto nesPartition = NesPartition(SharedQueryId(1), OperatorId(22),
                                   PartitionId(333), SubpartitionId(444));
  ...
  try {
    class ExchangeListener : public ExchangeProtocolListener {
     public:
      ...
      ExchangeListener(std::atomic<bool>& bufferReceived,
                       std::promise<bool>& completedProm)
      ...
      void onEvent(NesPartition, Runtime::BaseEvent& event) override {
        eventReceived =
            event.getEventType() == Runtime::EventType::kCustomEvent &&
            dynamic_cast<Runtime::CustomEventWrapper&>(event)
                    .data<detail::TestEvent>()
                    ->testValue() == 123;
    ...
    auto buffMgr =
        std::make_shared<Runtime::BufferManager>(bufferSize, buffersManaged);

    auto netManager = NetworkManager::create(
        INVALID_WORKER_NODE_ID, "127.0.0.1", *dataPort2,
        ExchangeProtocol(partMgr, std::make_shared<ExchangeListener>(
                                      eventReceived, completedProm)),
        buffMgr);
    ...
    std::thread t([&netManager, &nesPartition, &completedProm, this] {
      ...
      auto nodeLocation =
          NodeLocation(INVALID_WORKER_NODE_ID, "127.0.0.1", *dataPort2);
      netManager->registerSubpartitionConsumer(
          nesPartition, nodeLocation, std::make_shared<DataEmitterImpl>());
    ...
    auto senderChannel = netManager->registerSubpartitionProducer(
        nodeLocation, nesPartition, buffMgr, std::chrono::seconds(1), 5);

    if (senderChannel == nullptr) {
      NES_INFO(
          "NetworkStackIntegrationTest: Error in registering DataChannel!");
      completedProm.set_value(false);
    } else {
      senderChannel->sendEvent<detail::TestEvent>(
          Runtime::EventType::kCustomEvent, 123);
...
TEST_F(NetworkStackIntegrationTest, DISABLED_testSendEventBackward) {
  NodeLocation nodeLocationSender{INVALID_WORKER_NODE_ID, "127.0.0.1",
                                  *dataPort1};
  NodeLocation nodeLocationReceiver{INVALID_WORKER_NODE_ID, "127.0.0.1",
                                    *dataPort2};
  NesPartition nesPartition{SharedQueryId(1), OperatorId(22), PartitionId(33),
                            SubpartitionId(44)};
  SchemaPtr schema =
      TestSchemas::getSchemaTemplate("id_one_val_64")->updateSourceName("test");
  auto queryCompilerConfiguration =
  ...
  class TestQueryListener : public DummyQueryListener {
   public:
    explicit TestQueryListener(std::promise<bool>& queryCompleted)
        : queryCompleted(queryCompleted) {}

    bool notifyQueryStatusChange(
        SharedQueryId id, DecomposedQueryPlanId planId,
  ...
  auto nodeEngineReceiver =
      Runtime::NodeEngineBuilder::create(workerConfiguration2)
          .setQueryStatusListener(
              std::make_shared<TestQueryListener>(queryCompleted))
          .build();
  // create NetworkSink
  ...
  auto networkSourceDescriptor1 =
      std::make_shared<TestUtils::TestSourceDescriptor>(
          ...
          [&](SchemaPtr schema, OperatorId, OriginId, StatisticId,
              ...
              size_t numSourceLocalBuffers,
              const std::vector<
                  Runtime::Execution::SuccessorExecutablePipeline>& successors)
              -> DataSourcePtr {
            auto uniqueId = OperatorId(1);
            return std::make_shared<NetworkSource>(
                schema, nodeEngineReceiver->getBufferManager(),
                nodeEngineReceiver->getQueryManager(),
                nodeEngineReceiver->getNetworkManager(), nesPartition,
                nodeLocationSender, numSourceLocalBuffers, NSOURCE_RETRY_WAIT,
                NSOURCE_RETRIES, successors, 0, uniqueId);
          });

  class TestSourceEvent : public GeneratorSource {
   public:
    explicit TestSourceEvent(
        const SchemaPtr& schema, const Runtime::BufferManagerPtr& bufferManager,
        const Runtime::QueryManagerPtr& queryManager, OperatorId operatorId,
        size_t numSourceLocalBuffers,
        const std::vector<Runtime::Execution::SuccessorExecutablePipeline>&
            successors)
        : GeneratorSource(schema, bufferManager, queryManager, 1000, operatorId,
                          INVALID_ORIGIN_ID, INVALID_STATISTIC_ID,
  ...
  class TestSinkEvent : public SinkMedium {
   public:
    ...
    explicit TestSinkEvent(
        const SchemaPtr& schema, Runtime::NodeEnginePtr nodeEngine,
        const Runtime::BufferManagerPtr& bufferManager,
        uint32_t numOfProducers = 1,
        SharedQueryId sharedQueryId = INVALID_SHARED_QUERY_ID,
        DecomposedQueryPlanId decomposedQueryPlanId =
    ...
    bool writeData(Runtime::TupleBuffer&,
                   Runtime::WorkerContextRef context) override {
      auto parentPlan = nodeEngine->getQueryManager()->getQueryExecutionPlan(
          decomposedQueryPlanId);
      for (auto& dataSources : parentPlan->getSources()) {
        auto senderChannel =
            context.getEventOnlyNetworkChannel(dataSources->getOperatorId());
        senderChannel->sendEvent<detail::TestEvent>(
            Runtime::EventType::kCustomEvent, 123);
      }
      return true;
  ...
  auto testSink = std::make_shared<TestSinkEvent>(
      schema, nodeEngineReceiver, nodeEngineReceiver->getBufferManager());
  auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  ...
  auto networkSink = std::make_shared<TestNetworkSink>(
      schema, INVALID_OPERATOR_ID, INVALID_SHARED_QUERY_ID,
      INVALID_DECOMPOSED_QUERY_PLAN_ID, nodeLocationReceiver, nesPartition,
      nodeEngineSender, 1, NSOURCE_RETRY_WAIT, NSOURCE_RETRIES,
      DEFAULT_NUMBER_OF_ORIGINS, INITIAL_VERSION);
  ...
  auto testSourceDescriptor = std::make_shared<TestUtils::TestSourceDescriptor>(
      ...
      [&](SchemaPtr schema, OperatorId, OriginId, StatisticId,
          ...
          std::vector<Runtime::Execution::SuccessorExecutablePipeline>
              successors) -> DataSourcePtr {
        auto source = std::make_shared<TestSourceEvent>(
            schema, nodeEngineSender->getBufferManager(),
            nodeEngineSender->getQueryManager(), OperatorId(1),
            numSourceLocalBuffers, std::move(successors));
  ...
  auto query2 = TestQuery::from(testSourceDescriptor)
                    ...
                    .sink(networkSinkDescriptor);
  auto decomposedQueryPlan2 = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(1), SharedQueryId(0), INVALID_WORKER_NODE_ID,
...
INSTANTIATE_TEST_CASE_P(NetworkStackIntegrationTestParam,
