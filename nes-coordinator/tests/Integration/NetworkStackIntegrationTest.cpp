...
public:
    ...
    TestSink(
        ...
        : SinkMedium(std::make_shared<NesFormat>(schema, bufferManager), nodeEngine, numOfProducers, sharedQueryId, decomposedQueryPlanId)
    {
        // nop
    }
...
createMockedEngine(const std::string& hostname, uint16_t port, uint64_t bufferSize, uint64_t numBuffers, ExtraParameters&&... extraParams)
{
    try
    {
        ...
        auto queryManager = std::make_shared<Runtime::DynamicQueryManager>(
            std::make_shared<DummyQueryListener>(), bufferManagers, INVALID_WORKER_NODE_ID, 1, hwManager, 100);
        auto networkManagerCreator = [=](const Runtime::NodeEnginePtr& engine)
        {
            return Network::NetworkManager::create(
                INVALID_WORKER_NODE_ID, hostname, port, Network::ExchangeProtocol(partitionManager, engine), bufferManagers[0]);
        };
        ...
        std::thread receivingThread(
            [&]()
            {
                auto recvEngine = createMockedEngine<MockedNodeEngine>(
                    "127.0.0.1", *dataPort1, bufferSize, buffersManaged, completed, nesPartition, bufferCnt);
                // register the incoming channel
                auto sink = std::make_shared<NullOutputSink>(recvEngine, 1, INVALID_SHARED_QUERY_ID, INVALID_DECOMPOSED_QUERY_PLAN_ID);
                ...
                ASSERT_EQ(
                    recvEngine->getPartitionManager()->getConsumerRegistrationStatus(nesPartition),
                    PartitionRegistrationStatus::Registered);
                completed.get_future().get();
                sinkShutdownBarrier->wait();
                while (
                    !(qep->getStatus() == Runtime::Execution::ExecutableQueryPlanStatus::Stopped
...
TEST_F(NetworkStackIntegrationTest, testQEPNetworkSinkSource)
{
    ...
    auto nodeEngineSender
        = Runtime::NodeEngineBuilder::create(workerConfiguration1).setQueryStatusListener(std::make_shared<DummyQueryListener>()).build();
    ...
    auto nodeEngineReceiver
        = Runtime::NodeEngineBuilder::create(workerConfiguration2).setQueryStatusListener(std::make_shared<DummyQueryListener>()).build();
    ...
    for (auto i = 1; i <= numQueries; ++i)
    {
        ...
        auto testSink = std::make_shared<TestSink>(
            schema, nodeEngineReceiver, nodeEngineReceiver->getBufferManager(), 1, SharedQueryId(i), DecomposedQueryPlanId(subPlanId));
        ...
        auto decomposedQueryPlan = DecomposedQueryPlan::create(
            DecomposedQueryPlanId(subPlanId++), SharedQueryId(i), INVALID_WORKER_NODE_ID, query.getQueryPlan()->getRootOperators());
        ...
        auto decomposedQueryPlan2 = DecomposedQueryPlan::create(
            DecomposedQueryPlanId(subPlanId++), SharedQueryId(i), INVALID_WORKER_NODE_ID, query2.getQueryPlan()->getRootOperators());
        ...
        auto func = [](auto engine, auto qep) { return engine->registerExecutableQueryPlan(qep); };
        ...
        ASSERT_TRUE(f1.get());
        ASSERT_TRUE(f2.get());
        ASSERT_TRUE(nodeEngineSender->startQuery(builderGeneratorQEP->getSharedQueryId(), builderGeneratorQEP->getDecomposedQueryPlanId()));
        ASSERT_TRUE(nodeEngineReceiver->startQuery(builderReceiverQEP->getSharedQueryId(), builderReceiverQEP->getDecomposedQueryPlanId()));
    }
...
TEST_F(NetworkStackIntegrationTest, testSendEvent)
{
    ...
    try
    {
        ...
        std::thread t(
            [&netManager, &nesPartition, &completedProm, this]
            {
                // register the incoming channel
        ...
        auto senderChannel = netManager->registerSubpartitionProducer(nodeLocation, nesPartition, buffMgr, std::chrono::seconds(1), 5);
...
TEST_F(NetworkStackIntegrationTest, DISABLED_testSendEventBackward)
{
    ...
    public:
        ...
        bool notifyQueryStatusChange(
            SharedQueryId id, DecomposedQueryPlanId planId, Runtime::Execution::ExecutableQueryPlanStatus status) override
        {
    ...
    public:
        ...
        explicit TestSinkEvent(
            ...
            DecomposedQueryPlanId decomposedQueryPlanId = INVALID_DECOMPOSED_QUERY_PLAN_ID)
            : SinkMedium(
                std::make_shared<NesFormat>(schema, bufferManager), nodeEngine, numOfProducers, sharedQueryId, decomposedQueryPlanId)
        {
            // nop
        }
    ...
    auto query2 = TestQuery::from(testSourceDescriptor).filter(Attribute("id") < 5).sink(networkSinkDescriptor);
    auto decomposedQueryPlan2 = DecomposedQueryPlan::create(
        DecomposedQueryPlanId(1), SharedQueryId(0), INVALID_WORKER_NODE_ID, query2.getQueryPlan()->getRootOperators());
    auto queryPlan2 = typeInferencePhase->execute(query2.getQueryPlan());
