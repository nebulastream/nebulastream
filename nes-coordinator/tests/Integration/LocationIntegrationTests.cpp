...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LocationIntegrationTests.log",
    ...
    auto csvPath =
        std::filesystem::path(TEST_DATA_DIRECTORY) / "testLocations.csv";
    remove(csvPath.c_str());
    writeWaypointsToCsv(csvPath, waypoints);

    std::string singleLocationPath =
        std::filesystem::path(TEST_DATA_DIRECTORY) / "singleLocation.csv";
...
#ifdef S2DEF
    auto interpolatedCsv =
        std::filesystem::path(TEST_DATA_DIRECTORY) / "path1.csv";
    remove(interpolatedCsv.c_str());
    std::vector<NES::Spatial::DataTypes::Experimental::Waypoint>
        waypointsToInterpolate;
    waypointsToInterpolate.push_back(
        {{52.58210307572243, 12.987507417206261}, 0});
    waypointsToInterpolate.push_back(
        {{52.5225665088927, 13.198478869225813}, 1000000000});
    waypointsToInterpolate.push_back(
        {{52.5824815034542, 13.280594641984383}, 2000000000});
    waypointsToInterpolate.push_back(
        {{52.5251960754162, 13.400310793986574}, 3000000000});
    waypointsToInterpolate.push_back(
        {{52.51309876750171, 13.57837236374691}, 5000000000});
    auto interpolatedPath = interpolatePath(waypointsToInterpolate, 1000);
    writeWaypointsToCsv(interpolatedCsv, interpolatedPath);
#endif

    auto inputSequence =
        std::filesystem::path(TEST_DATA_DIRECTORY) / "sequence_long.csv";
  ...
  std::chrono::duration<int64_t, std::milli> defaultTimeoutInSec =
      std::chrono::seconds(TestUtils::defaultTimeout);

  struct getGeolocationParameters {
    getGeolocationParameters(TopologyNodePtr node, TopologyPtr service)
        : node(node), service(service) {}
    TopologyNodePtr node;
    TopologyPtr service;
  };

  // wrapper function so allow the util function to call the member function of
  // LocationProvider
  ...
  getLocationFromTopologyNode(const std::shared_ptr<void> &structParams) {
    auto casted =
        std::static_pointer_cast<getGeolocationParameters>(structParams);
    return casted->service->getGeoLocationForNode(casted->node->getId());
  }

  /**
   * @brief wait until the topology contains the expected number of nodes so we
   * can rely on these nodes being present for the rest of the test
   ...
   * @return true if expected number of nodes was reached. false in case of
   * timeout before number was reached
   */
  static bool waitForNodes(int timeoutSeconds, size_t nodes,
...
#ifdef S2DEF
  /**
   * @brief check if two location objects latitudes and longitudes do not differ
   * more than the specified error
   * @param location1
   * @param location2
   * @param error the tolerated difference in latitute or longitude specified in
   * degrees
   * @return true if location1 and location2 do not differ more then the
   * specified degrees in latitiude of logintude
   */
  static bool
  isClose(NES::Spatial::DataTypes::Experimental::GeoLocation location1,
  ...
  interpolatePath(
    ...
    for (auto waypointsItr = waypoints.cbegin();
         waypointsItr != waypoints.cend(); ++waypointsItr) {
      ...
      S2Point prevPoint =
          NES::Spatial::Util::S2Utilities::geoLocationToS2Point(prevLocation);
      S2Point nextPoint =
          NES::Spatial::Util::S2Utilities::geoLocationToS2Point(nextLocation);
      ...
      for (Timestamp i = 0; i < timerange; i += step) {
        ...
        interpolatedPath.emplace_back(
            NES::Spatial::DataTypes::Experimental::GeoLocation(
                interpolated.lat().degrees(), interpolated.lng().degrees()),
            prevTime + i);
      }
    }
...
TEST_F(LocationIntegrationTests, testFieldNodes) {
  ...
  wrkConf2->locationCoordinates.setValue(
      NES::Spatial::DataTypes::Experimental::GeoLocation::fromString(
          location2));
  wrkConf2->nodeSpatialType.setValue(
      NES::Spatial::Experimental::SpatialType::FIXED_LOCATION);
  ...
  wrkConf3->locationCoordinates.setValue(
      NES::Spatial::DataTypes::Experimental::GeoLocation::fromString(
          location3));
  wrkConf3->nodeSpatialType.setValue(
      NES::Spatial::Experimental::SpatialType::FIXED_LOCATION);
  ...
  wrkConf4->locationCoordinates.setValue(
      NES::Spatial::DataTypes::Experimental::GeoLocation::fromString(
          location4));
  wrkConf4->nodeSpatialType.setValue(
      NES::Spatial::Experimental::SpatialType::FIXED_LOCATION);
  ...
  ASSERT_EQ(topology->getGeoLocationForNode(node2->getId()),
            NES::Spatial::DataTypes::Experimental::GeoLocation(
                52.53736960143897, 13.299134894776092));
  topology->updateGeoLocation(
      node2->getId(), NES::Spatial::DataTypes::Experimental::GeoLocation(
                          52.51094383152051, 13.463078966025266));
  ASSERT_EQ(topology->getGeoLocationForNode(node2->getId()),
            NES::Spatial::DataTypes::Experimental::GeoLocation(
                52.51094383152051, 13.463078966025266));
  ...
  auto inRange = topology->getNodesIdsInRange(
      NES::Spatial::DataTypes::Experimental::GeoLocation(52.53736960143897,
                                                         13.299134894776092),
      50.0);
  ASSERT_EQ(inRange.size(), (size_t)3);
#endif
  topology->updateGeoLocation(
      node3->getId(), NES::Spatial::DataTypes::Experimental::GeoLocation(
                          53.559524264262194, 10.039384739854102));
...
TEST_F(LocationIntegrationTests, testMobileNodes) {
  ...
  NES_INFO("start worker 1");
  WorkerConfigurationPtr wrkConf1 = WorkerConfiguration::create();
  Configurations::Spatial::Mobility::Experimental::
      WorkerMobilityConfigurationPtr mobilityConfiguration1 =
          Configurations::Spatial::Mobility::Experimental::
              WorkerMobilityConfiguration::create();
  wrkConf1->coordinatorPort = (port);
  // we set a location which should get ignored, because we make this node
  // mobile. so it should not show up as a field node
  wrkConf1->locationCoordinates.setValue(
      NES::Spatial::DataTypes::Experimental::GeoLocation::fromString(
          location2));
  wrkConf1->nodeSpatialType.setValue(
      NES::Spatial::Experimental::SpatialType::MOBILE_NODE);
  ...
  ASSERT_EQ(wrk1->getLocationProvider()->getSpatialType(),
            NES::Spatial::Experimental::SpatialType::MOBILE_NODE);
  ASSERT_EQ(wrk2->getLocationProvider()->getSpatialType(),
            NES::Spatial::Experimental::SpatialType::FIXED_LOCATION);
  ...
  ASSERT_EQ(node1->getSpatialNodeType(),
            NES::Spatial::Experimental::SpatialType::MOBILE_NODE);
  ASSERT_EQ(node2->getSpatialNodeType(),
            NES::Spatial::Experimental::SpatialType::FIXED_LOCATION);
...
TEST_F(LocationIntegrationTests, testLocationFromCmd) {
  ...
  WorkerConfigurationPtr workerConfigPtr =
      std::make_shared<WorkerConfiguration>();
  ...
  for (int i = 0; i < argc; ++i) {
    commandLineParams.insert(std::pair<string, string>(
        string(argv[i]).substr(0, string(argv[i]).find('=')),
        string(argv[i]).substr(string(argv[i]).find('=') + 1,
                               string(argv[i]).length() - 1)));
  }

  workerConfigPtr->overwriteConfigWithCommandLineInput(commandLineParams);
  ASSERT_EQ(workerConfigPtr->locationCoordinates.getValue(),
            NES::Spatial::DataTypes::Experimental::GeoLocation(23.88, -3.4));
}
...
TEST_F(LocationIntegrationTests, testMovingDevice) {
  ...
  // set endtime to 1.5 times the duration until the last timestamp so we are
  // more likely to catch the last location
  ...
  while (currentTimeStamp <= endTime) {
    ...
    // after we receivd a location from the location index, there should always
    // be one present for all remaining iterations
    if (receivedLocation) {
      EXPECT_TRUE(currentGeoLocation.has_value());
    }
    if (currentGeoLocation.has_value()) {
      receivedLocation = true;
      if (actualWayPoints.empty() ||
          currentGeoLocation.value() != actualWayPoints.back().getLocation()) {
  ...
  for (auto actualIt = actualWayPoints.cbegin();
       actualIt != actualWayPoints.cend(); ++actualIt) {
    while (expectedIt != expectedWayPoints.cend() &&
           expectedIt->getLocation() != actualIt->getLocation()) {
      expectedIt++;
    }
    ...
    // only if an unexpected location was observed the iterator could have
    // reached the end of the list of expected waypoints
...
TEST_F(LocationIntegrationTests, testMovementAfterStandStill) {
  ...
  bool retStart1 = wrk1->start(/**blocking**/ false, /**withConnect**/ true);
  ASSERT_TRUE(retStart1);
  auto locationProvider = std::static_pointer_cast<
      NES::Spatial::Mobility::Experimental::LocationProviderCSV,
      NES::Spatial::Mobility::Experimental::LocationProvider>(
      wrk1->getLocationProvider());
...
TEST_F(LocationIntegrationTests, testMovingDeviceSimulatedStartTimeInFuture) {
  ...
  wrkConf1->mobilityConfiguration.locationProviderSimulatedStartTime.setValue(
      simulatedStartTime);
  NesWorkerPtr wrk1 = std::make_shared<NesWorker>(std::move(wrkConf1));
  bool retStart1 = wrk1->start(/**blocking**/ false, /**withConnect**/ true);
  ASSERT_TRUE(retStart1);
...
TEST_F(LocationIntegrationTests, testGetLocationViaRPC) {
  ...
  auto singleLoc =
      getWaypointsFromCsv(
          wrkConf1->mobilityConfiguration.locationProviderConfig, 0)
          .front()
          .getLocation();
...
TEST_F(LocationIntegrationTests, testReconnectingParentOutOfCoverage) {
  ...
  for (auto elem : locVec) {
    TopologyNodePtr currNode =
        TopologyNode::create(idCount, "127.0.0.1", 1, 0, 0, properties);
    topology->addNewTopologyNodeAsChild(node, currNode);
    topology->addGeoLocation(
        currNode->getId(),
        NES::Spatial::DataTypes::Experimental::GeoLocation(elem));
  ...
  auto outOfCoverageLocation =
      NES::Spatial::DataTypes::Experimental::GeoLocation(52.7091055763087,
                                                         13.128662109375002);
  TopologyNodePtr currNode =
      TopologyNode::create(initialParentId, "127.0.0.1", 1, 0, 0, properties);
  topology->addNewTopologyNodeAsChild(node, currNode);
  topology->addGeoLocation(currNode->getId(),
                           NES::Spatial::DataTypes::Experimental::GeoLocation(
                               outOfCoverageLocation));
  nodeIndex.Add(NES::Spatial::Util::S2Utilities::geoLocationToS2Point(
                    outOfCoverageLocation),
                currNode->getId());
  ...
  uint64_t parentId = 0;
  std::vector<uint64_t> reconnectSequence(
      {initialParentId, 10000, 10001, 10002, 10003, 10004, 10005});
  parentId =
      std::dynamic_pointer_cast<TopologyNode>(
          topology->findNodeWithId(wrk1->getWorkerId())->getParents().front())
          ->getId();
  uint64_t oldParentId = 0;
  std::vector<uint64_t> actualSequence;
  auto startTimestamp = std::chrono::system_clock::now();
  while (reconnectSequence.size() > 1 &&
         std::chrono::system_clock::now() <
             startTimestamp + defaultTimeoutInSec) {
...
// even if no parent was specified in the worker configuration, the worker
// should reconnect to the closest parent on startup
TEST_F(LocationIntegrationTests, testConnectingToClosestNodeNoParentInConfig) {
  ...
  // wait until the node has changed its parent from the coordinator node (id 1)
  // to a fixed location node in range
  auto startTimestamp = std::chrono::system_clock::now();
  while (parentId == 1 && std::chrono::system_clock::now() <
                              startTimestamp + defaultTimeoutInSec) {
    parentId =
        std::dynamic_pointer_cast<TopologyNode>(
            topology->findNodeWithId(wrk1->getWorkerId())->getParents().front())
  ...
  EXPECT_NE(
      std::find(reconnectSequence.begin(), reconnectSequence.end(), parentId),
      reconnectSequence.end());
...
TEST_F(LocationIntegrationTests, testReconfigWithoutRunningQuery) {
  ...
  crd->getSourceCatalog()->addLogicalSource(
      "seq",
...
TEST_F(LocationIntegrationTests, DISABLED_testSequenceWithReconnecting) {
  NES_INFO(" start coordinator");
  std::string testFile =
      getTestResourceFolder() / "sequence_with_reconnecting_out.csv";
  ...
  for (auto elem : locVec) {
    ...
    wrkConf->nodeSpatialType.setValue(
        NES::Spatial::Experimental::SpatialType::FIXED_LOCATION);
  ...
  stype->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                     "sequence_long.csv");
  ...
  QueryId queryId =
      crd->getRequestHandlerService()->validateAndQueueAddQueryRequest(
          R"(Query::from("seq").sink(FileSinkDescriptor::create(")" + testFile +
              R"(", "CSV_FORMAT", "APPEND"));)",
  ...
  auto startTimestamp = std::chrono::system_clock::now();
  while (recv_tuples < 10000 && std::chrono::system_clock::now() <
                                    startTimestamp + defaultTimeoutInSec) {
...
INSTANTIATE_TEST_CASE_P(LocationIntegrationTestsParam, LocationIntegrationTests,
                        ::testing::Values(1, 4));
#endif
} // namespace NES::Spatial
