...
public:
    ...
    void SetUp()
    {
        ...
        schema_integers_1
            = Schema::create()->addField("id", BasicType::UINT64)->addField("one", BasicType::UINT64)->addField("value", BasicType::UINT64);
...
TEST_F(StaticDataSourceIntegrationTest, testCustomerTableNotDistributed)
{
    ...
    //register query
    std::string queryString = R"(Query::from("tpch_customer").filter(Attribute("C_CUSTKEY") < 10).sink(FileSinkDescriptor::create(")"
...
TEST_F(StaticDataSourceIntegrationTest, DISABLED_testCustomerTableProjection)
{
    ...
    NES_ASSERT(
        numResultTuples == numExpectedTuples,
...
TEST_F(StaticDataSourceIntegrationTest, testNationTable)
{
    ...
    //register query
    std::string queryString = R"(Query::from("tpch_nation").filter(Attribute("N_NATIONKEY") > 20).sink(FileSinkDescriptor::create(")"
        + filePath + R"(" , "CSV_FORMAT", "APPEND"));)";
    QueryId queryId = requestHandlerService->validateAndQueueAddQueryRequest(queryString, "BottomUp");
    ASSERT_NE(queryId, INVALID_QUERY_ID);
    auto globalQueryPlan = crd->getGlobalQueryPlan();
...
TEST_F(StaticDataSourceIntegrationTest, testTableIntegersOnlyDistributed)
{
    ...
    //register query
    std::string queryString = R"(Query::from("static_integers_only_0").filter(Attribute("id") < 6).sink(FileSinkDescriptor::create(")"
        + filePath + R"(" , "CSV_FORMAT", "APPEND"));)";
    QueryId queryId = requestHandlerService->validateAndQueueAddQueryRequest(queryString, "BottomUp");
    EXPECT_NE(queryId, INVALID_QUERY_ID);
    auto globalQueryPlan = crd->getGlobalQueryPlan();
...
TEST_F(StaticDataSourceIntegrationTest, testBatchJoinNationCustomerFull)
{
    ...
    if (!benchmark)
    {
        ...
        int numResultTuples = std::count(std::istreambuf_iterator<char>(ifs), std::istreambuf_iterator<char>(), '\n');
        int numExpectedTuples = std::count(std::istreambuf_iterator<char>(ifsInputProbeSide), std::istreambuf_iterator<char>(), '\n')
            + 1; // the .csv file contains a header, so we expect one more
...
TEST_F(StaticDataSourceIntegrationTest, testBatchJoinIntegersOnly)
{
    ...
    PhysicalSourceTypePtr sourceType1 = StaticDataSourceType::create(table_path_integers_1, 0, "wrapBuffer", /* placeholder: */ 0, true);
...
TEST_F(StaticDataSourceIntegrationTest, DISABLED_testBatchJoinIntegersOnlyPartitioned)
{
    ...
    PhysicalSourceTypePtr sourceType1a = StaticDataSourceType::create(table_path_integers_1a, 0, "wrapBuffer", /* placeholder: */ 0, true);
    PhysicalSourceTypePtr sourceType1b = StaticDataSourceType::create(table_path_integers_1b, 0, "wrapBuffer", /* placeholder: */ 0, true);
...
TEST_F(StaticDataSourceIntegrationTest, testBatchJoinCustomerWithIntTable)
{
    ...
    NES_DEBUG(
        "StaticDataSourceIntegrationTest::testBatchJoinCustomerWithIntTable: num work {} num buff {}",
        crdConf->worker.numWorkerThreads.getValue(),
        crdConf->worker.numberOfBuffersInGlobalBufferManager.getValue());
