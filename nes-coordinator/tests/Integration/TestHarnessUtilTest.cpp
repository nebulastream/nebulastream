...
class TestHarnessUtilTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TestHarnessUtilTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup TestHarnessUtilTest test class.");
  }

  static void TearDownTestCase() {
    NES_INFO("TestHarnessUtilTest test class TearDownTestCase.");
  }
};
...
TEST_F(TestHarnessUtilTest, testHarnessUtilWithSingleSource) {
  ...
  auto queryWithFilterOperator =
      Query::from("car").filter(Attribute("id") < 1000);
  TestHarness testHarness =
      TestHarness(queryWithFilterOperator, *restPort, *rpcCoordinatorPort,
 ...
 */
TEST_F(TestHarnessUtilTest,
       testHarnessUtilWithTwoPhysicalSourceOfTheSameLogicalSource) {
  struct Car {
    uint32_t id;
    uint32_t value;
    uint64_t timestamp;
...
/*
 * Testing testHarness utility using two logical source with one physical source
 * each
 */
TEST_F(TestHarnessUtilTest,
       testHarnessUtilWithTwoPhysicalSourceOfDifferentLogicalSources) {
  struct Car {
    uint32_t id;
    uint32_t value;
    uint64_t timestamp;
  ...
  auto queryWithFilterOperator =
      Query::from("car").unionWith(Query::from("truck"));
  TestHarness testHarness =
      TestHarness(queryWithFilterOperator, *restPort, *rpcCoordinatorPort,
                  getTestResourceFolder())
          .addLogicalSource("car", carSchema)
...
TEST_F(TestHarnessUtilTest, testHarnessUtilWithWindowOperator) {
  ...
  auto queryWithWindowOperator =
      Query::from("car")
          .window(
              TumblingWindow::of(EventTime(Attribute("timestamp")), Seconds(1)))
          .byKey(Attribute("id"))
          .apply(Sum(Attribute("value")));
  TestHarness testHarness =
      TestHarness(queryWithWindowOperator, *restPort, *rpcCoordinatorPort,
                  getTestResourceFolder())
...
/**
 * Testing testHarness utility for query with a join operator on different
 * sources
 */
  ...
  auto queryWithJoinOperator =
      Query::from("window1")
          ...
          .window(TumblingWindow::of(EventTime(Attribute("timestamp")),
                                     Milliseconds(1000)));
  TestHarness testHarness =
      TestHarness(queryWithJoinOperator, *restPort, *rpcCoordinatorPort,
                  getTestResourceFolder())
...
TEST_F(TestHarnessUtilTest, testHarnessOnQueryWithMapOperator) {
  ...
  auto queryWithFilterOperator = Query::from("car").map(
      Attribute("value") = Attribute("value") * Attribute("id"));
  TestHarness testHarness =
      TestHarness(queryWithFilterOperator, *restPort, *rpcCoordinatorPort,
                  getTestResourceFolder())
          .addLogicalSource("car", carSchema)
...
TEST_F(TestHarnessUtilTest, testHarnessWithHiearchyInTopology) {
  ...
  TestHarness testHarness =
      TestHarness(queryWithFilterOperator, *restPort, *rpcCoordinatorPort,
          ...
          /**
            * Expected topology:
                PhysicalNode[id=1, ip=127.0.0.1, resourceCapacity=65535,
            usedResource=0]
                |--PhysicalNode[id=2, ip=127.0.0.1, resourceCapacity=8,
            usedResource=0] |  |--PhysicalNode[id=3, ip=127.0.0.1,
            resourceCapacity=8, usedResource=0] |  |  |--PhysicalNode[id=5,
            ip=127.0.0.1, resourceCapacity=8, usedResource=0] |  |
            |--PhysicalNode[id=4, ip=127.0.0.1, resourceCapacity=8,
            usedResource=0]
            */
...
TEST_F(TestHarnessUtilTest, testHarnessUtilWithNoSources) {
  ...
  EXPECT_THROW(TestHarness(queryWithFilterOperator, *restPort,
...
TEST_F(TestHarnessUtilTest, testHarnessUtilPushToNonExsistentSource) {
  ...
  EXPECT_THROW(testHarness.pushElement<Car>({30, 30, 30}, 0),
...
TEST_F(TestHarnessUtilTest, testHarnessUtilPushToWrongSource) {
  ...
  EXPECT_THROW(testHarness.pushElement<Truck>({30, 30, 30, 30, 30}, 2),
               Exceptions::RuntimeException);
}
