...
class JoinDeploymentTest
 ...
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("JoinDeploymentTest.log",
  ...
  void SetUp() override {
    ...
    joinStrategy =
        std::get<0>(NES::Runtime::Execution::JoinDeploymentTest::GetParam());
    windowingStrategy =
        std::get<1>(NES::Runtime::Execution::JoinDeploymentTest::GetParam());
    bufferManager = std::make_shared<BufferManager>();
  }

  void runJoinQueryTwoLogicalStreams(
      const Query& query, const TestUtils::CsvFileParams& csvFileParams,
    ...
    auto sourceConfig1 = TestUtils::createSourceTypeCSV(
        {logicalSourceNameOne, physicalSourceNameOne,
         csvFileParams.inputCsvFiles[0]});
    auto sourceConfig2 = TestUtils::createSourceTypeCSV(
        {logicalSourceNameTwo, physicalSourceNameTwo,
         csvFileParams.inputCsvFiles[1]});
    ...
    std::ifstream expectedFileStream(
        ...
        csvFileParams.expectedFile);
    auto actualBuffers =
        testHarness.validate()
            .setupTopology()
            .runQuery(NES::Util::countLines(expectedFileStream))
            .getOutput();

    // Comparing equality
    const auto outputSchema = testHarness.getOutputSchema();
    auto tmpBuffers = TestUtils::createExpectedBufferFromStream(
        expectedFileStream, outputSchema, testHarness.getBufferManager());
    auto expectedBuffers =
        TestUtils::createTestTupleBuffers(tmpBuffers, outputSchema);
    for (auto& buf : actualBuffers) {
      NES_INFO("Buf:\n{}", NES::Util::printTupleBufferAsCSV(
                               buf.getBuffer(), joinParams.outputSchema));
    }
    EXPECT_TRUE(
...
TEST_P(JoinDeploymentTest, testJoinWithSameSchemaTumblingWindow) {
  ...
  const auto schema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                          ->updateSourceName("test1");
  TestUtils::JoinParams joinParams({schema, schema});
  TestUtils::CsvFileParams csvFileParams("window.csv", "window.csv",
                                         "window_sink.csv");
...
TEST_P(JoinDeploymentTest,
  ...
  const auto leftSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                              ->updateSourceName("test1");
  const auto rightSchema = TestSchemas::getSchemaTemplate("id2_val2_time_u64")
...
TEST_P(JoinDeploymentTest, testJoinWithDifferentSourceTumblingWindow) {
  ...
  const auto rightSchema = TestSchemas::getSchemaTemplate("id2_val2_time_u64")
                               ->updateSourceName("test2");
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  TestUtils::CsvFileParams csvFileParams("window.csv", "window2.csv",
                                         "window_sink2.csv");
  auto query = Query::from("test1")
                   .joinWith(Query::from("test2"))
                   .where(Attribute("id") == Attribute("id2"))
...
TEST_P(JoinDeploymentTest,
  ...
  const auto leftSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                              ->updateSourceName("test1");
  const auto rightSchema =
      TestSchemas::getSchemaTemplate("id2_time_u64")->updateSourceName("test2");
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  TestUtils::CsvFileParams csvFileParams("window.csv", "window3.csv",
                                         "window_sink3.csv");
  auto query = Query::from("test1")
                   .joinWith(Query::from("test2"))
                   .where(Attribute("id") == Attribute("id2"))
...
TEST_P(JoinDeploymentTest, testJoinWithVarSizedData) {
  ...
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  TestUtils::CsvFileParams csvFileParams("window5.csv", "window6.csv",
                                         "window_sink4.csv");
  auto query = Query::from("test1")
                   .joinWith(Query::from("test2"))
...
TEST_P(JoinDeploymentTest, joinResultLargerThanSingleTupleBuffer) {
  ...
  const auto rightSchema =
      TestSchemas::getSchemaTemplate("id2_time_u64")->updateSourceName("test2");
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  TestUtils::CsvFileParams csvFileParams("window8.csv", "window9.csv",
                                         "window_sink8.csv");
  auto query = Query::from("test1")
                   .joinWith(Query::from("test2"))
                   .where(Attribute("id") == Attribute("id2"))
...
INSTANTIATE_TEST_CASE_P(
    testJoinQueries, JoinDeploymentTest, JOIN_STRATEGIES_WINDOW_STRATEGIES,
