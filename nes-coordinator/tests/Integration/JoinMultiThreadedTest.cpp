    ...
    : public Testing::BaseIntegrationTest,
      ...
      public ::testing::WithParamInterface<
          std::tuple<QueryCompilation::StreamJoinStrategy,
                     QueryCompilation::WindowingStrategy, uint64_t>> {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("JoinMultiThreadedTest.log",
  ...
  void SetUp() override {
    ...
    const uint64_t numberOfWorkerThreads =
        std::get<2>(JoinMultiThreadedTest::GetParam());
    executionEngine = std::make_shared<Testing::TestExecutionEngine>(
        dumpNone, numberOfWorkerThreads, joinStrategy, windowingStrategy);
  }
  ...
  std::vector<ResultRecord>& runQueryAndPrintMissingRecords(
    ...
    for (auto [inputSchema, fileNameInputBuffers] : inputs) {
      allInputBuffers.emplace_back(TestUtils::createExpectedBuffersFromCsv(
          fileNameInputBuffers, inputSchema, bufferManager,
          numTuplesPerBuffer));
    }
    ...
    EXPECT_TRUE(executionEngine->stopQuery(
        queryPlan, Runtime::QueryTerminationType::Graceful));
    ...
    missingRecords.erase(
        std::remove_if(missingRecords.begin(), missingRecords.end(),
                       [&result](auto item) {
                         return std::find(result.begin(), result.end(), item) !=
                                result.end();
                       }),
        missingRecords.end());
    auto missingRecordsStr = std::accumulate(
        missingRecords.begin(), missingRecords.end(), std::string(),
        [](const std::string& str, auto item) {
...
TEST_P(JoinMultiThreadedTest, testOneJoin) {
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    std::string toString() {
      ...
      oss << window1window2Start << "," << window1window2End << ","
          << "," << window1win1 << "," << window1id1 << "," << window1timestamp
          << "," << window2win2 << "," << window2id2 << "," << window2timestamp;
      return oss.str();
    }

    bool operator==(const ResultRecord& rhs) const {
      return window1window2Start == rhs.window1window2Start &&
             window1window2End == rhs.window1window2End &&
             window1win1 == rhs.window1win1 && window1id1 == rhs.window1id1 &&
             window1timestamp == rhs.window1timestamp &&
             window2win2 == rhs.window2win2 && window2id2 == rhs.window2id2 &&
             window2timestamp == rhs.window2timestamp;
    }
  };
  const auto inputSchemaLeft = TestSchemas::getSchemaTemplate("id_val_time_u64")
                                   ->updateSourceName("test1");
  const auto inputSchemaRight =
      TestSchemas::getSchemaTemplate("id2_val2_time_u64")
          ->updateSourceName("test2");
  const auto joinFieldNameLeft = "test1$id";
  const auto outputSchema = Runtime::Execution::Util::createJoinSchema(
      inputSchemaLeft, inputSchemaRight);
  ...
  const auto testSink =
      executionEngine->createCollectSink<ResultRecord>(outputSchema);
  const auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  const auto testSourceDescriptorLeft =
      executionEngine->createDataSource(inputSchemaLeft);
  const auto testSourceDescriptorRight =
      executionEngine->createDataSource(inputSchemaRight);
  const auto query =
  ...
  const auto resultRecords = runQueryAndPrintMissingRecords<ResultRecord>(
      ...
      expectedTuples, testSink, query);

  // Checking for correctness
  ASSERT_EQ(resultRecords.size(), expectedTuples.size());
  EXPECT_THAT(resultRecords,
              ::testing::UnorderedElementsAreArray(expectedTuples));
}
...
TEST_P(JoinMultiThreadedTest, testTwoJoins) {
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    bool operator==(const ResultRecord& rhs) const {
      return window1window2window3start == rhs.window1window2window3start &&
             ...
             window1window2Start == rhs.window1window2Start &&
             window1window2End == rhs.window1window2End &&
             window1win1 == rhs.window1win1 && window1id1 == rhs.window1id1 &&
             window1timestamp == rhs.window1timestamp &&
             window2win2 == rhs.window2win2 && window2id2 == rhs.window2id2 &&
             window2timestamp == rhs.window2timestamp &&
             window3win3 == rhs.window3win3 && window3id3 == rhs.window3id3 &&
  ...
  const auto joinFieldNameLeft = "test1$id";
  const auto joinSchemaLeftRight = Runtime::Execution::Util::createJoinSchema(
      inputSchemaLeft, inputSchemaRight);
  const auto outputSchema = Runtime::Execution::Util::createJoinSchema(
      joinSchemaLeftRight, inputSchemaThird);
  ...
  const auto testSourceDescriptorRight =
      executionEngine->createDataSource(inputSchemaRight);
  const auto testSourceDescriptorThird =
      executionEngine->createDataSource(inputSchemaThird);
...
TEST_P(JoinMultiThreadedTest, testThreeJoins) {
  ...
  struct ResultRecord {
    ...
    bool operator==(const ResultRecord& rhs) const {
      return window1window2window3window4start ==
                 rhs.window1window2window3window4start &&
             window1window2window3window4end ==
                 rhs.window1window2window3window4end &&
             ...
             window3win3 == rhs.window3win3 && window3id3 == rhs.window3id3 &&
             window3timestamp == rhs.window3timestamp &&
             window4win4 == rhs.window4win4 && window4id4 == rhs.window4id4 &&
  ...
  const auto joinSchemaLeftRightThird =
      Runtime::Execution::Util::createJoinSchema(joinSchemaLeftRight,
                                                 inputSchemaThird);
  const auto outputSchema = Runtime::Execution::Util::createJoinSchema(
      joinSchemaLeftRightThird, inputSchemaFourth);
  ASSERT_EQ(sizeof(ResultRecord), outputSchema->getSchemaSizeInBytes());
  ...
  const std::vector<ResultRecord> expectedTuples = {
      {12000, 13000, 12000, 13000, 12000, 13000, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {13000, 14000, 13000, 14000, 13000, 14000, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {14000, 15000, 14000, 15000, 14000, 15000, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {15000, 16000, 15000, 16000, 15000, 16000, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
  ...
  const auto testSourceDescriptorThird =
      executionEngine->createDataSource(inputSchemaThird);
  const auto testSourceDescriptorFourth =
      executionEngine->createDataSource(inputSchemaFourth);
  const auto query =
      TestQuery::from(testSourceDescriptorLeft)
          .joinWith(TestQuery::from(testSourceDescriptorRight))
          .where(Attribute("id") == Attribute("id2"))
...
TEST_P(JoinMultiThreadedTest, oneJoinSlidingWindow) {
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    std::string toString() {
      std::ostringstream oss;
      oss << window1window2Start << "," << window1window2End << ","
          << "," << window1id1 << "," << window1timestamp << "," << window2id2
          << "," << window2timestamp;
      return oss.str();
    }
    ...
    bool operator==(const ResultRecord& rhs) const {
      return window1window2Start == rhs.window1window2Start &&
             ...
             window1id1 == rhs.window1id1 &&
             window1timestamp == rhs.window1timestamp &&
             window2value2 == rhs.window2value2 &&
             window2id2 == rhs.window2id2 &&
             window2timestamp == rhs.window2timestamp;
    }
  };
  const auto inputSchemaLeft = TestSchemas::getSchemaTemplate("id_val_time_u64")
...
TEST_P(JoinMultiThreadedTest, threeJoinsSlidingWindow) {
  ...
  const std::vector<ResultRecord> expectedTuples = {
      {500, 1500, 500, 1500, 500, 1500, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {500, 1500, 500, 1500, 500, 1500, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 500, 1500, 500, 1500, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 500, 1500, 1000, 2000, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {500, 1500, 500, 1500, 1000, 2000, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 500, 1500, 1000, 2000, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 1000, 2000, 500, 1500, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {500, 1500, 1000, 2000, 500, 1500, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 1000, 2000, 500, 1500, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 1000, 2000, 1000, 2000, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {500, 1500, 1000, 2000, 1000, 2000, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {500, 1500, 1000, 2000, 1000, 2000, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 500, 1500, 500, 1500, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {1000, 2000, 500, 1500, 500, 1500, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 500, 1500, 500, 1500, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 500, 1500, 1000, 2000, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {1000, 2000, 500, 1500, 1000, 2000, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 500, 1500, 1000, 2000, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 1000, 2000, 500, 1500, 12, 1, 1001, 12, 5, 1011, 12, 1, 1300,
       12, 1, 1300},
      {1000, 2000, 1000, 2000, 500, 1500, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 1000, 2000, 500, 1500, 4, 1, 1002, 4, 3, 1112, 4, 4, 1001, 4,
       4, 1001},
      {1000, 2000, 1000, 2000, 1000, 2000, 12, 1, 1001, 12, 5, 1011, 12, 1,
       1300, 12, 1, 1300},
      {1000, 2000, 1000, 2000, 1000, 2000, 4, 1, 1002, 4, 3, 1102, 4, 4, 1001,
      ...
      {3000, 4000, 3000, 4000, 3000, 4000, 11, 3, 3001, 11, 3, 3001, 11, 9,
       3000, 11, 9, 3000},
      {11500, 12500, 11500, 12500, 11500, 12500, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {11500, 12500, 11500, 12500, 12000, 13000, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {11500, 12500, 12000, 13000, 11500, 12500, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {11500, 12500, 12000, 13000, 12000, 13000, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {12000, 13000, 11500, 12500, 11500, 12500, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {12000, 13000, 11500, 12500, 12000, 13000, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {12000, 13000, 12000, 13000, 11500, 12500, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {12000, 13000, 12000, 13000, 12000, 13000, 1, 12, 12000, 1, 12, 12000, 1,
       12, 12000, 1, 12, 12000},
      {12500, 13500, 12500, 13500, 12500, 13500, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {12500, 13500, 12500, 13500, 13000, 14000, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {12500, 13500, 13000, 14000, 12500, 13500, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {12500, 13500, 13000, 14000, 13000, 14000, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {13000, 14000, 12500, 13500, 12500, 13500, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {13000, 14000, 12500, 13500, 13000, 14000, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {13000, 14000, 13000, 14000, 12500, 13500, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {13000, 14000, 13000, 14000, 13000, 14000, 1, 13, 13000, 1, 13, 13000, 1,
       13, 13000, 1, 13, 13000},
      {13500, 14500, 13500, 14500, 13500, 14500, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {13500, 14500, 13500, 14500, 14000, 15000, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {13500, 14500, 14000, 15000, 13500, 14500, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {13500, 14500, 14000, 15000, 14000, 15000, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {14000, 15000, 13500, 14500, 13500, 14500, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {14000, 15000, 13500, 14500, 14000, 15000, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {14000, 15000, 14000, 15000, 13500, 14500, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {14000, 15000, 14000, 15000, 14000, 15000, 1, 14, 14000, 1, 14, 14000, 1,
       14, 14000, 1, 14, 14000},
      {14500, 15500, 14500, 15500, 14500, 15500, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {14500, 15500, 14500, 15500, 15000, 16000, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {14500, 15500, 15000, 16000, 14500, 15500, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {14500, 15500, 15000, 16000, 15000, 16000, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {15000, 16000, 14500, 15500, 14500, 15500, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {15000, 16000, 14500, 15500, 15000, 16000, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {15000, 16000, 15000, 16000, 14500, 15500, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000},
      {15000, 16000, 15000, 16000, 15000, 16000, 1, 15, 15000, 1, 15, 15000, 1,
       15, 15000, 1, 15, 15000}};
...
INSTANTIATE_TEST_CASE_P(
    testJoinQueriesMultiThreaded, JoinMultiThreadedTest,
