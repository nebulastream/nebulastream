...
class WindowDeploymentTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("WindowDeploymentTest.log",
  ...
  auto query =
      Query::from("window")
          .window(TumblingWindow::of(
              EventTime(Attribute("timestamp"), Seconds()), Minutes(1)))
          .byKey(Attribute("id"))
          .apply(Sum(Attribute("value")));

  auto sourceConfig = CSVSourceType::create("window", "window1");
  sourceConfig->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                            "window.csv");
...
/**
 * @brief test distributed tumbling window and event time, for now disabled see
 * issue #3324
 */
TEST_F(WindowDeploymentTest,
  ...
  auto query = Query::from("window")
                   .window(TumblingWindow::of(
                       EventTime(Attribute("ts"), Seconds()), Minutes(1)))
                   .byKey(Attribute("id"))
                   .apply(Sum(Attribute("value")));

  auto sourceConfig1 = CSVSourceType::create("window", "window1");
  sourceConfig1->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                             "window.csv");
...
TEST_F(WindowDeploymentTest, testCentralNonKeyTumblingWindowIngestionTime) {
  ...
  std::string outputFilePath =
      getTestResourceFolder() / "testGlobalTumblingWindow.out";
 ...
 */
TEST_F(WindowDeploymentTest,
       testDeploymentOfWindowWithMaxAggregationWithNegativeValues) {
  struct Car {
    int32_t id;
    int32_t value;
    int64_t timestamp;
 ...
 */
TEST_F(WindowDeploymentTest,
       testDeploymentOfWindowWithMaxAggregationWithUint64AggregatedField) {
  struct Car {
    uint64_t id;
    uint64_t value;
    uint64_t timestamp;
 ...
 */
TEST_F(WindowDeploymentTest,
       DISABLED_testDeploymentOfWindowWithMedianAggregation) {
  struct Car {
    uint64_t key;
    double value;
    uint64_t value2;
