...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("YSBDeploymentTest.log",
...
TEST_F(YSBDeploymentTest, testYSBWindow) {
  ...
  ASSERT_TRUE(
      crd->getSourceCatalogService()->registerLogicalSource("ysb", input));
  ...
  auto func = [](NES::Runtime::TupleBuffer &buffer,
    ...
    struct __attribute__((packed)) YsbRecord {
      YsbRecord() = default;
      YsbRecord(uint64_t userId, uint64_t pageId, uint64_t campaignId,
                uint64_t adType, uint64_t eventType, uint64_t currentMs,
                uint64_t ip)
          : userId(userId), pageId(pageId), campaignId(campaignId),
            adType(adType), eventType(eventType), currentMs(currentMs), ip(ip) {
      }
    ...
    NES_WARNING("Lambda last entry is={}",
                records[numberOfTuplesToProduce - 1].toString());
  };

  auto lambdaSourceType = LambdaSourceType::create(
      "ysb", "YSB_phy", func, 10, 100, GatheringMode::INTERVAL_MODE);
