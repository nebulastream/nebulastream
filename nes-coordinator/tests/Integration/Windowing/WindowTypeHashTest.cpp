...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("WindowTypeHashTest.log",
...
TEST_F(WindowTypeHashTest, SlidingWindowHashTest) {
  ...
  auto slidingWindow0 =
      SlidingWindow::of(timeCharacteristic1, Seconds(10), Seconds(5));
  auto slidingWindow1 =
      SlidingWindow::of(timeCharacteristic1, Seconds(10), Seconds(5));
  auto slidingWindow2 =
      SlidingWindow::of(timeCharacteristic2, Seconds(10), Seconds(5));
  auto slidingWindow3 =
      SlidingWindow::of(timeCharacteristic1, Seconds(10), Milliseconds(5000));
  auto slidingWindow4 =
      SlidingWindow::of(timeCharacteristic1, Seconds(5), Seconds(5));
  auto slidingWindow5 =
      SlidingWindow::of(timeCharacteristic1, Seconds(10), Seconds(10));

  // tests hash values are the same for slidingWindows with same parameters
  EXPECT_EQ(slidingWindow0->hash(), slidingWindow1->hash());
  // tests that size=Seconds(5) and size=Milliseconds(5000) gives the same hash
  // value
  EXPECT_EQ(slidingWindow1->hash(), slidingWindow3->hash());

  auto windows = {slidingWindow1, slidingWindow2, slidingWindow4,
                  slidingWindow5};
...
TEST_F(WindowTypeHashTest, TumblingWindowHashTest) {
  ...
  auto tumblingWindow4 =
      TumblingWindow::of(timeCharacteristic1, Milliseconds(5000));
...
TEST_F(WindowTypeHashTest, AllWindowTypesHashTest) {
  auto timeCharacteristic1 = EventTime(Attribute("time"));
  auto timeCharacteristic2 =
      EventTime(Attribute("timestamp")); // t2 makes no difference
  ...
  auto slidingWindow2 =
      SlidingWindow::of(timeCharacteristic2, Seconds(10), Seconds(5));
  auto slidingWindow3 =
      SlidingWindow::of(timeCharacteristic1, Seconds(10), Seconds(8));
  auto slidingWindow4 =
      SlidingWindow::of(timeCharacteristic1, Seconds(8), Seconds(5));
  ...
  auto tumblingWindow4 =
      TumblingWindow::of(timeCharacteristic1, Milliseconds(10));
  ...
  auto windows = {slidingWindow1,   slidingWindow2,  slidingWindow3,
                  slidingWindow4,   tumblingWindow1, tumblingWindow2,
                  tumblingWindow3,  tumblingWindow4, thresholdWindow1,
                  thresholdWindow2, thresholdWindow3};
