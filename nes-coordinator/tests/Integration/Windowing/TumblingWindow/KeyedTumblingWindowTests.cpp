...
/**
 * @brief In this test we assess the correctness of the thread local tumbling
 * window
 */
class KeyedTumblingWindowTests : public Testing::BaseIntegrationTest,
                                 public ::testing::WithParamInterface<int> {
 public:
  WorkerConfigurationPtr workerConfiguration;
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedTumblingWindowTests.log",
...
PhysicalSourceTypePtr createSimpleInputStream(std::string logicalSourceName,
  ...
  return LambdaSourceType::create(
      ...
      [numberOfKeys](Runtime::TupleBuffer& buffer,
      ...
      numberOfBuffers, 0, GatheringMode::INTERVAL_MODE);
}
...
class DataGeneratorMultiKey {
 public:
  DataGeneratorMultiKey(std::string logicalSourceName,
                        std::string physicalSourceName,
                        uint64_t numberOfBuffers, uint64_t numberOfKeys = 1)
...
class DataGenerator {
 public:
  DataGenerator(std::string logicalSourceName, std::string physicalSourceName,
                uint64_t numberOfBuffers)
      : logicalSourceName(std::move(logicalSourceName)),
        physicalSourceName(std::move(physicalSourceName)),
        numberOfBuffers(numberOfBuffers){};
...
TEST_F(KeyedTumblingWindowTests, testSingleTumblingWindowSingleBuffer) {
  ...
  auto testHarness = TestHarness(query, *restPort, *rpcCoordinatorPort,
                         ...
                         .attachWorkerWithLambdaSourceToCoordinator(
                             lambdaSource, workerConfiguration)
                         .validate()
                         .setupTopology();
  ASSERT_EQ(testHarness.getWorkerCount(), 1UL);
...
TEST_F(KeyedTumblingWindowTests, testMultipleTumblingWindowMultiBuffer) {
  ...
  auto testHarness = TestHarness(query, *restPort, *rpcCoordinatorPort,
                         ...
                         .addLogicalSource("window", testSchema)
                         .attachWorkerWithLambdaSourceToCoordinator(
                             dg.getSource(), workerConfiguration)
                         .validate()
                         .setupTopology();
  ASSERT_EQ(testHarness.getWorkerCount(), 1UL);
...
TEST_F(KeyedTumblingWindowTests,
       testSingleTumblingWindowMultiBufferMultipleKeys) {
  auto testSchema = TestSchemas::getSchemaTemplate("id_val_time_u64");
