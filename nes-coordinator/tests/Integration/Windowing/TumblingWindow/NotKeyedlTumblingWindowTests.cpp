...
/**
 * @brief In this test we assess the correctness of the non keyed tumbling
 * window
 */
class NonKeyedTumblingWindowTests : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("NonKeyedTumblingWindowTests.log",
...
PhysicalSourceTypePtr createSimpleInputStream(std::string logicalSourceName,
  ...
  return LambdaSourceType::create(
      std::move(logicalSourceName), std::move(physicalSourceName),
...
class DataGeneratorMultiValue {
 public:
  DataGeneratorMultiValue(std::string logicalSourceName,
                          std::string physicalSourceName,
                          uint64_t numberOfBuffers)
      : logicalSourceName(std::move(logicalSourceName)),
        physicalSourceName(std::move(physicalSourceName)),
...
TEST_F(NonKeyedTumblingWindowTests,
       testSingleGlobalTumblingWindowSingleBuffer) {
...
TEST_F(NonKeyedTumblingWindowTests,
       testMultipleGlobalTumblingWindowMultiBuffer) {
  auto testSchema = TestSchemas::getSchemaTemplate("id_val_time_u64");
...
TEST_F(NonKeyedTumblingWindowTests,
  ...
  auto dg = DataGeneratorMultiValue("window", "window1",
                                    this->numberOfGeneratedBuffers);
  auto testHarness = TestHarness(query, *restPort, *rpcCoordinatorPort,
                                 getTestResourceFolder())
                         .addLogicalSource("window", testSchema)
                         .attachWorkerWithLambdaSourceToCoordinator(
