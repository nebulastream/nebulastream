...
class MultiThreadedTest : public Testing::BaseIntegrationTest,
 ...
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MultiThreadedTest.log",
  ...
  void SetUp() override {
    ...
    executionEngine = std::make_shared<Testing::TestExecutionEngine>(
        dumpNone, numberOfWorkerThreads);
  }
  ...
  std::vector<ResultRecord>& runQuery(
...
TEST_P(MultiThreadedTest, DISABLED_testFilterQuery) {
  ...
  const auto inputSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
  ...
  const std::vector<ResultRecord> expectedTuples = {
      {1, 1, 1000},  {12, 1, 1001}, {4, 1, 1002}, {1, 2, 2000},
      {11, 2, 2001}, {16, 2, 2002}, {1, 3, 3000}, {11, 3, 3001},
      {1, 3, 3003},  {1, 3, 3200},  {1, 4, 4000}, {1, 5, 5000},
      {1, 6, 6000},  {1, 7, 7000},  {1, 8, 8000}, {1, 9, 9000}};
  ...
  // Running the query
  const auto resultRecords = runQuery<ResultRecord>(
      {{inputSchema, fileNameBuffers}}, expectedTuples.size(), testSink, query);
...
TEST_P(MultiThreadedTest, testProjectQuery) {
  ...
  const auto outputSchema =
      TestSchemas::getSchemaTemplate("id_u64")->updateSourceName("test1");

  const std::string fileNameBuffers("window.csv");
  const std::vector<ResultRecord> expectedTuples = {
      {1}, {12}, {4}, {1}, {11}, {16}, {1}, {11}, {1}, {1}, {1}, {1}, {1}, {1}};
...
struct NonKeyedResultRecord {
  ...
  bool operator==(const NonKeyedResultRecord& rhs) const {
    return windowStart == rhs.windowStart && windowEnd == rhs.windowEnd &&
           value == rhs.value;
  }
...
std::ostream& operator<<(std::ostream& os, const KeyedResultRecord& record) {
  os << record.windowStart << "-" << record.windowEnd << "-" << record.id << "-"
     << record.value;
  return os;
}

// todo 4254: re-enable after crashes are fixed
TEST_P(MultiThreadedTest,
       DISABLED_testNonKeyedEventTimeTumblingWindowAggregation) {
  const auto inputSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                               ->updateSourceName("test1");
  const auto outputSchema = TestSchemas::getSchemaTemplate("id2_val2_time_u64")
                                ->updateSourceName("test1");

  const std::string fileNameBuffers("window.csv");
  const std::vector<NonKeyedResultRecord> expectedTuples = {
      {1000, 2000, 3},    {2000, 3000, 6},    {3000, 4000, 12},
      {4000, 5000, 4},    {5000, 6000, 5},    {6000, 7000, 6},
      {7000, 8000, 7},    {8000, 9000, 8},    {9000, 10000, 9},
      {10000, 11000, 10}, {11000, 12000, 11}, {12000, 13000, 12},
      {13000, 14000, 13}, {14000, 15000, 14}, {15000, 16000, 15},
      {16000, 17000, 16}, {17000, 18000, 17}, {18000, 19000, 18},
      {19000, 20000, 19}, {20000, 21000, 20}, {21000, 22000, 21}};

  // Creating sink, source, and the query
  const auto testSink =
      executionEngine->createCollectSink<NonKeyedResultRecord>(outputSchema);
  const auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  const auto testSourceDescriptor =
      executionEngine->createDataSource(inputSchema);
  const auto query = TestQuery::from(testSourceDescriptor)
                         .window(TumblingWindow::of(
                             EventTime(Attribute("timestamp")), Seconds(1)))
                         .apply(Sum(Attribute("value")))
                         .sink(testSinkDescriptor);
  // Running the query
  const auto resultRecords = runQuery<NonKeyedResultRecord>(
      {{inputSchema, fileNameBuffers}}, expectedTuples.size(), testSink, query);
...
TEST_P(MultiThreadedTest, testNonKeyedEventTimeSlidingWindowAggregation) {
  ...
  const std::string fileNameBuffers("window.csv");
  const std::vector<NonKeyedResultRecord> expectedTuples = {
      {1000, 2000, 3},    {1500, 2500, 6},    {2000, 3000, 6},
      {2500, 3500, 12},   {3000, 4000, 12},   {3500, 4500, 4},
      {4000, 5000, 4},    {4500, 5500, 5},    {5000, 6000, 5},
      {5500, 6500, 6},    {6000, 7000, 6},    {6500, 7500, 7},
      {7000, 8000, 7},    {7500, 8500, 8},    {8000, 9000, 8},
      {8500, 9500, 9},    {9000, 10000, 9},   {9500, 10500, 10},
      {10000, 11000, 10}, {10500, 11500, 11}, {11000, 12000, 11},
      {11500, 12500, 12}, {12000, 13000, 12}, {12500, 13500, 13},
      {13000, 14000, 13}, {13500, 14500, 14}, {14000, 15000, 14},
      {14500, 15500, 15}, {15000, 16000, 15}, {15500, 16500, 16},
      {16000, 17000, 16}, {16500, 17500, 17}, {17000, 18000, 17},
      {17500, 18500, 18}, {18000, 19000, 18}, {18500, 19500, 19},
      {19000, 20000, 19}, {19500, 20500, 20}, {20000, 21000, 20},
      {20500, 21500, 21}, {21000, 22000, 21}};
...
TEST_P(MultiThreadedTest, testKeyedEventTimeTumblingWindowAggregation) {
  ...
  const std::vector<KeyedResultRecord> expectedTuples = {
      {1000, 2000, 1, 1},    {1000, 2000, 12, 1},   {1000, 2000, 4, 1},
      {2000, 3000, 11, 2},   {2000, 3000, 1, 2},    {2000, 3000, 16, 2},
      {3000, 4000, 1, 9},    {3000, 4000, 11, 3},   {4000, 5000, 1, 4},
      {5000, 6000, 1, 5},    {6000, 7000, 1, 6},    {7000, 8000, 1, 7},
      {8000, 9000, 1, 8},    {9000, 10000, 1, 9},   {10000, 11000, 1, 10},
      {11000, 12000, 1, 11}, {12000, 13000, 1, 12}, {13000, 14000, 1, 13},
      {14000, 15000, 1, 14}, {15000, 16000, 1, 15}, {16000, 17000, 1, 16},
      {17000, 18000, 1, 17}, {18000, 19000, 1, 18}, {19000, 20000, 1, 19},
      {20000, 21000, 1, 20}, {21000, 22000, 1, 21}};

  // Creating sink, source, and the query
  const auto testSink =
      executionEngine->createCollectSink<KeyedResultRecord>(outputSchema);
  const auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  const auto testSourceDescriptor =
      executionEngine->createDataSource(inputSchema);
  const auto query = TestQuery::from(testSourceDescriptor)
                         ...
                         .sink(testSinkDescriptor);
  // Running the query
  const auto resultRecords = runQuery<KeyedResultRecord>(
      {{inputSchema, fileNameBuffers}}, expectedTuples.size(), testSink, query);
...
TEST_P(MultiThreadedTest,
  ...
  const std::string fileNameBuffers("window.csv");
  const std::vector<NonKeyedResultRecord> expectedTuples = {
      {0, 2000, 3},     {2000, 4000, 18},  {4000, 6000, 9},
      {6000, 8000, 13}, {8000, 10000, 17}, {10000, 12000, 21}};
  ...
  const auto query =
      TestQuery::from(testSourceDescriptor)
          .filter(Attribute("value") < 12)  // this is merely to keep the number
                                            // of output tuples under control
          .window(
...
TEST_P(MultiThreadedTest, testMultipleKeyedEventTimeTumblingWindows) {
  ...
  const std::string fileNameBuffers("window.csv");
  const std::vector<KeyedResultRecord> expectedTuples = {
      {0, 2000, 1, 1},      {0, 2000, 4, 1},     {0, 2000, 12, 1},
      {2000, 4000, 11, 5},  {2000, 4000, 1, 11}, {2000, 4000, 16, 2},
      {4000, 6000, 1, 9},   {6000, 8000, 1, 13}, {8000, 10000, 1, 17},
      {10000, 12000, 1, 21}};
...
INSTANTIATE_TEST_CASE_P(
    testQueriesMultiThreaded, MultiThreadedTest,
