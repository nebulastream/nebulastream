...
class QueryDeploymentTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("QueryDeploymentTest.log",
...
/**
 * Test deploying unionWith query with source on two different worker node using
 * bottom up strategy.
 */
TEST_F(QueryDeploymentTest, testDeployTwoWorkerMergeUsingBottomUp) {
  struct ResultRecord {
    ...
    bool operator==(ResultRecord const& other) const {
  ...
  for (auto i = 0_u32; i < 10; ++i) {
    ...
    testHarness.pushElement<ResultRecord>(elementCarStream, 2)
  ...
  auto actualBuffers = testHarness.validate()
                           .setupTopology()
                           .runQuery(Util::countLines(expectedOutput))
                           .getOutput();

  // Comparing equality
  const auto outputSchema = testHarness.getOutputSchema();
  auto tmpBuffers = TestUtils::createExpectedBufferFromStream(
...
TEST_F(QueryDeploymentTest, testDeployOneWorkerFileOutput) {
  ...
  TestHarness testHarness = TestHarness(query, *restPort, *rpcCoordinatorPort,
                                        getTestResourceFolder())
                                .addLogicalSource("test", defaultLogicalSchema)
                                .attachWorkerWithMemorySourceToWorkerWithId(
                                    "test", WorkerId(1), workerConfig);

  const auto noTuplesInBuffer = workerConfig->bufferSizeInBytes /
                                defaultLogicalSchema->getSchemaSizeInBytes();
  const auto tuplesForThreeBuffers = noTuplesInBuffer * 3;
  for (uint32_t i = 0; i < tuplesForThreeBuffers; ++i) {
    testHarness.pushElement<Test>(
        {1, i},
        2);  // fills record store of source with id 0-(noTuplesInBuffer * 3)
  }
  ...
  // Run the query and get the actual dynamic buffers
  auto actualBuffers =
      testHarness.runQuery(Util::countLines(expectedOutput.str())).getOutput();

  // Comparing equality
  const auto outputSchema = testHarness.getOutputSchema();
  auto tmpBuffers = TestUtils::createExpectedBufferFromCSVString(
      expectedOutput.str(), outputSchema, testHarness.getBufferManager());
  auto expectedBuffers =
      TestUtils::createTestTupleBuffers(tmpBuffers, outputSchema);
  EXPECT_TRUE(
      TestUtils::buffersContainSameTuples(expectedBuffers, actualBuffers));
...
/**
 * @brief Test deploy query with print sink with one worker using top down
 * strategy
 */
TEST_F(QueryDeploymentTest, testDeployOneWorkerFileOutputUsingTopDownStrategy) {
  ...
  // Run the query and get the actual dynamic buffers
  auto actualBuffers =
      testHarness.runQuery(Util::countLines(expectedOutput)).getOutput();
...
TEST_F(QueryDeploymentTest, testSourceSharing) {
  ...
  auto logicalSource =
      Configurations::LogicalSourceType::create("window1", schemaType);
  ...
  coordinatorConfig->worker.queryCompiler.outputBufferOptimizationLevel =
      QueryCompilation::OutputBufferOptimizationLevel::NO;

  std::promise<bool> start;
  bool started = false;
  auto func1 = [&start, &started](NES::Runtime::TupleBuffer& buffer,
                                  uint64_t numTuples) {
    struct Record {
      uint64_t id;
      uint64_t value;
  ...
  auto lambdaSourceType1 =
      LambdaSourceType::create("window1", "test_stream1", std::move(func1), 2,
                               2, GatheringMode::INTERVAL_MODE);
  ...
  auto query1 = Query::from("window1").sink(
      FileSinkDescriptor::create(outputFilePath1, "CSV_FORMAT", "APPEND"));

  QueryId queryId1 = requestHandlerService->validateAndQueueAddQueryRequest(
      query1.getQueryPlan(), Optimizer::PlacementStrategy::TopDown);
  EXPECT_TRUE(TestUtils::waitForQueryToStart(queryId1, queryCatalog));

  auto query2 = Query::from("window1").sink(
      FileSinkDescriptor::create(outputFilePath2, "CSV_FORMAT", "APPEND"));

  QueryId queryId2 = requestHandlerService->validateAndQueueAddQueryRequest(
      query2.getQueryPlan(), Optimizer::PlacementStrategy::TopDown);
  EXPECT_TRUE(TestUtils::waitForQueryToStart(queryId2, queryCatalog));

  start.set_value(true);

  string expectedContent1 =
      "window1$id:INTEGER(64 bits),window1$value:INTEGER(64 "
      "bits),window1$timestamp:INTEGER(64 bits)\n"
      "0,0,0\n"
      "1,1,0\n"
      "2,2,0\n"
      "3,3,0\n"
  ...
  EXPECT_TRUE(
      TestUtils::checkOutputOrTimeout(expectedContent1, outputFilePath1));
  EXPECT_TRUE(
      TestUtils::checkOutputOrTimeout(expectedContent1, outputFilePath2));
...
TEST_F(QueryDeploymentTest, testSourceSharingWithFilter) {
  ...
  auto query1 = Query::from("window1")
                    ...
                    .sink(FileSinkDescriptor::create(outputFilePath1,
                                                     "CSV_FORMAT", "APPEND"));
  ...
  auto query2 = Query::from("window1")
                    .filter(Attribute("id") > 5)
                    .sink(FileSinkDescriptor::create(outputFilePath2,
                                                     "CSV_FORMAT", "APPEND"));
...
TEST_F(
  ...
  CSVSourceTypePtr csvSourceType1 =
      CSVSourceType::create("stream", "test_stream");
...
TEST_F(QueryDeploymentTest, testDeployOneWorkerFileOutputWithWrongProjection) {
  ...
  NES_DEBUG("QueryDeploymentTest: Start worker 1");
  WorkerConfigurationPtr workerConfig1 = WorkerConfiguration::create();
  DefaultSourceTypePtr defaultSourceType1 =
      DefaultSourceType::create("default_logical", "physical_car");
  ...
  EXPECT_THROW(
      requestHandlerService->validateAndQueueAddQueryRequest(
          query.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp),
...
TEST_F(QueryDeploymentTest,
  ...
  auto query1 = Query::from("default_logical")
                    .sink(FileSinkDescriptor::create(outputFilePath1,
                                                     "CSV_FORMAT", "APPEND"));
  QueryId queryId1 = requestHandlerService->validateAndQueueAddQueryRequest(
      query1.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);
  auto query2 = Query::from("default_logical")
                    .sink(FileSinkDescriptor::create(outputFilePath2,
                                                     "CSV_FORMAT", "APPEND"));
  QueryId queryId2 = requestHandlerService->validateAndQueueAddQueryRequest(
      query2.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);
  GlobalQueryPlanPtr globalQueryPlan = crd->getGlobalQueryPlan();

  string expectedContent =
      "default_logical$id:INTEGER(32 bits),default_logical$value:INTEGER(64 "
      "bits)\n"
      "1,1\n"
      "1,1\n"
      "1,1\n"
  ...
  EXPECT_TRUE(
      TestUtils::checkOutputOrTimeout(expectedContent, outputFilePath1));
  EXPECT_TRUE(
      TestUtils::checkOutputOrTimeout(expectedContent, outputFilePath2));
...
TEST_F(QueryDeploymentTest, testOneQueuePerQueryWithOutput) {
  ...
  workerConfig1->coordinatorPort = port;
  CSVSourceTypePtr csvSourceType1 =
      CSVSourceType::create("stream1", "test_stream");
  csvSourceType1->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                              "window.csv");
  csvSourceType1->setGatheringInterval(1);
  csvSourceType1->setNumberOfTuplesToProducePerBuffer(1);
  ...
  CSVSourceTypePtr csvSourceType2 =
      CSVSourceType::create("stream2", "test_stream");
  csvSourceType2->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                              "window.csv");
  ...
  NES_INFO("QueryDeploymentTest: Submit query");
  auto query1 = Query::from("stream1").sink(
      FileSinkDescriptor::create(outputFilePath1, "CSV_FORMAT", "APPEND"));
  QueryId queryId1 = requestHandlerService->validateAndQueueAddQueryRequest(
      query1.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);

  auto query2 = Query::from("stream2").sink(
      FileSinkDescriptor::create(outputFilePath2, "CSV_FORMAT", "APPEND"));
  QueryId queryId2 = requestHandlerService->validateAndQueueAddQueryRequest(
      query2.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);
  GlobalQueryPlanPtr globalQueryPlan = crd->getGlobalQueryPlan();
  ...
  string expectedContent1 =
      "stream1$id:INTEGER(64 bits),stream1$value:INTEGER(64 "
      "bits),stream1$timestamp:INTEGER(64 bits)\n"
      "12,1,1001\n";

  string expectedContent2 =
      "stream2$id:INTEGER(64 bits),stream2$value:INTEGER(64 "
      "bits),stream2$timestamp:INTEGER(64 bits)\n"
      "12,1,1001\n";
...
TEST_F(QueryDeploymentTest,
  ...
  DefaultSourceTypePtr defaultSourceType2 =
      DefaultSourceType::create("default_logical", "physical_car");
  ...
  NES_INFO("QueryDeploymentTest: Submit query");
  string query1 =
      R"(Query::from("default_logical").sink(FileSinkDescriptor::create(")" +
      outputFilePath1 + R"(", "CSV_FORMAT", "APPEND"));)";
  QueryId queryId1 = requestHandlerService->validateAndQueueAddQueryRequest(
      query1, Optimizer::PlacementStrategy::BottomUp);
  string query2 =
      R"(Query::from("default_logical").sink(FileSinkDescriptor::create(")" +
      outputFilePath2 + R"(", "CSV_FORMAT", "APPEND"));)";
  QueryId queryId2 = requestHandlerService->validateAndQueueAddQueryRequest(
      query2, Optimizer::PlacementStrategy::BottomUp);
...
/**
 * Test deploying joinWith query with source on two different worker node using
 * top down strategy
 */
TEST_F(QueryDeploymentTest, testDeployTwoWorkerJoinUsingTopDownOnSameSchema) {
  ...
  csvSourceType->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                             "window.csv");
...
TEST_F(QueryDeploymentTest, testOneQueuePerQueryWithHardShutdown) {
  ...
  string expectedContent1 =
      "stream1$value:INTEGER(64 bits),stream1$id:INTEGER(64 "
      "bits),stream1$timestamp:INTEGER(64 bits)\n"
      "1,12,1001\n";

  string expectedContent2 =
      "stream2$value:INTEGER(64 bits),stream2$id:INTEGER(64 "
      "bits),stream2$timestamp:INTEGER(64 bits)\n"
      "1,12,1001\n";
...
/**
 * Test deploying join query with source on two different worker node using top
 * down strategy.
 */
 ...
 */
TEST_F(QueryDeploymentTest,
       testJoinWithDifferentSourceDifferentSpeedTumblingWindow) {
  struct Window {
    int64_t win1;
    uint64_t id1;
    uint64_t timestamp1;
  ...
  const auto window2Schema =
      TestSchemas::getSchemaTemplate("id2_val2_time_u64");
      ...
      Query::from("car1")
          .joinWith(
              Query::from("car2")
                  ...
                  .byKey(Attribute("id"))
                  .apply(Count())
                  .project(Attribute("start").as("timestamp"), Attribute("end"),
                           Attribute("id"), Attribute("count")))
          .where(Attribute("id") == Attribute("id"))
          .window(TumblingWindow::of(EventTime(Attribute("timestamp")),
                                     Seconds(1)));

  auto testHarness = TestHarness(queryWithWindowOperator, *restPort,
                                 *rpcCoordinatorPort, getTestResourceFolder())
...
TEST_F(QueryDeploymentTest, testJoinWithFourSources) {
  ...
  csvSourceType1_1->setFilePath(std::string(TEST_DATA_DIRECTORY) +
                                "window.csv");
  ...
  csvSourceType1_2->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                                "window.csv");
  ...
  csvSourceType2_1->setFilePath(std::string(TEST_DATA_DIRECTORY) +
                                "window2.csv");
  ...
  csvSourceType2_2->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                                "window2.csv");
...
/**
 * @brief This tests just outputs the default source for a hierarchy with one
 relay which also produces data by itself
 * Topology:
 ...
 Join 1 |  |--PhysicalNode[id=6, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0] |  |--PhysicalNode[id=5, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0] |  |--PhysicalNode[id=4, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0]
 */
...
TEST_F(QueryDeploymentTest,
  ...
  csvSourceType3->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                              "window2.csv");
  csvSourceType3->setNumberOfTuplesToProducePerBuffer(3);
  csvSourceType3->setNumberOfBuffersToProduce(2);
  ...
  csvSourceType4->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                              "window4.csv");
  ...
  std::string outputFilePath =
      getTestResourceFolder() /
      "testTwoJoinsWithDifferentStreamTumblingWindowDistributed.out";
  ...
  EXPECT_TRUE(TestUtils::waitForQueryToStart(queryId, queryCatalog));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 2));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk2, queryId, globalQueryPlan, 2));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk3, queryId, globalQueryPlan, 2));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk4, queryId, globalQueryPlan, 2));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(crd, queryId, globalQueryPlan, 2));

  string expectedContent =
      "window1window2window3$start:INTEGER(64 "
      "bits),window1window2window3$end:INTEGER(64 "
      "bits),window1window2window3$key:INTEGER(64 bits),window1window2$"
      "start:INTEGER(64 bits),window1window2$end:INTEGER(64 "
      "bits),window1window2$key:INTEGER(64 "
      "bits),window1$win1:INTEGER(64 bits),window1$id1:INTEGER(64 "
      "bits),window1$"
      "timestamp:INTEGER(64 bits),window2$win2:INTEGER(64 "
      "bits),window2$id2:INTEGER(64 "
      "bits),window2$timestamp:INTEGER(64 bits),window3$win3:INTEGER(64 "
      "bits),window3$id3:"
...
TEST_F(QueryDeploymentTest,
  ...
  std::string outputFilePath =
      getTestResourceFolder() /
      "testTwoJoinsWithDifferentStreamSlidingWindowDistributed.out";
  remove(outputFilePath.c_str());
