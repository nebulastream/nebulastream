...
class UDFCatalogControllerTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ConnectivityControllerTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup TopologyControllerTest test class.");
  }

  static void TearDownTestCase() {
    NES_INFO("Tear down ConnectivityControllerTest test class.");
  }

  static void verifyResponseResult(const cpr::Response& response,
  ...
  static void verifySerializedInstance(
      const jni::JavaSerializedInstance& actual, const std::string& expected) {
    auto converted =
        jni::JavaSerializedInstance{expected.begin(), expected.end()};
    ASSERT_EQ(actual, converted);
  }

  static void verifyByteCodeList(
      const jni::JavaUDFByteCodeList& actual,
      const google::protobuf::RepeatedPtrField<
          JavaUdfDescriptorMessage_JavaUdfClassDefinition>& expected) {
    ASSERT_EQ(actual.size(),
              static_cast<decltype(actual.size())>(expected.size()));
    for (const auto& classDefinition : expected) {
      auto actualByteCode =
          std::find_if(actual.cbegin(), actual.cend(),
                       [&](const jni::JavaClassDefinition& c) {
                         return c.first == classDefinition.class_name();
                       });
      ASSERT_TRUE(actualByteCode != actual.end());
      auto converted = jni::JavaByteCode(classDefinition.byte_code().begin(),
...
// Test if retrieval of a UDF added directly to UDFCatalog over POST returns
// same UDF
TEST_F(UDFCatalogControllerTest, getUdfDescriptorReturnsUdf) {
  startCoordinator();
  ASSERT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));
  ...
  // send a GET request to REST API of coordinator for the previously defined
  // java udf
  auto future = cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                       "/v1/nes/udfCatalog/getUdfDescriptor"},
  ...
  GetJavaUdfDescriptorResponse udfResponse =
      extractGetJavaUdfDescriptorResponse(response);
  // from protobuf message, get the java udf descriptor
  auto descriptor = udfResponse.java_udf_descriptor();
  // and compare udf descriptor with the one registered to the coordinator
  // earlier
  ...
  verifySerializedInstance(javaUdfDescriptor->getSerializedInstance(),
                           descriptor.serialized_instance());
  verifyByteCodeList(javaUdfDescriptor->getByteCodeList(),
                     descriptor.classes());
  stopCoordinator();
}
...
TEST_F(UDFCatalogControllerTest, testErrorIfUnknownEndpointIsUsed) {
  ...
  auto future = cpr::PostAsync(
      cpr::Url{BASE_URL + std::to_string(*restPort) +
               "/v1/nes/udfCatalog/SUPER_SECRET_URL"},
...
// Test if RegisterJavaUdf endpoint handles exceptions without returning a stack
// trace
TEST_F(UDFCatalogControllerTest,
       testIfRegisterEndpointHandlesExceptionsWithoutReturningAStackTrace) {
  startCoordinator();
  ASSERT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));

  // given a REST message containing a wrongly formed Java UDF (bytecode list is
  // empty)
  auto javaUdfRequest =
      ProtobufMessageFactory::createDefaultRegisterJavaUdfRequest();
  javaUdfRequest.mutable_java_udf_descriptor()->clear_classes();

  auto future = cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                        "/v1/nes/udfCatalog/registerJavaUdf"},
                               cpr::Header{{"Content-Type", "text/plain"}},
...
TEST_F(UDFCatalogControllerTest, testIfRegisterUdfEndpointCorrectlyAddsUDF) {
  ...
  auto descriptorFromCoordinator =
      UDF::UDFDescriptor::as<UDF::JavaUDFDescriptor>(
          udfCatalog->getUDFDescriptor("my_udf"));
  // extract the udf descriptor from the udf post request
  JavaUdfDescriptorMessage descriptorFromPostRequest =
      javaUdfRequest.java_udf_descriptor();
  // and compare its fields to the java udf catalog entry now found in the
  // coordinator
  ASSERT_EQ(descriptorFromCoordinator->getClassName(),
            descriptorFromPostRequest.udf_class_name());
  ASSERT_EQ(descriptorFromCoordinator->getMethodName(),
            descriptorFromPostRequest.udf_method_name());
  verifySerializedInstance(descriptorFromCoordinator->getSerializedInstance(),
                           descriptorFromPostRequest.serialized_instance());
  verifyByteCodeList(descriptorFromCoordinator->getByteCodeList(),
                     descriptorFromPostRequest.classes());
  stopCoordinator();
}
...
TEST_F(UDFCatalogControllerTest, testRemoveUdfEndpoint) {
  ...
  auto future = cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                          "/v1/nes/udfCatalog/removeUdf"},
                                 cpr::Parameters{{"udfName", udfName}});
  future.wait();
  auto response = future.get();
  ...
  ASSERT_EQ(UDF::UDFDescriptor::as<UDF::JavaUDFDescriptor>(
                udfCatalog->getUDFDescriptor("my_udf")),
            nullptr);
...
TEST_F(UDFCatalogControllerTest, testRemoveUdfEndpointIfUdfDoesNotExist) {
  ...
  ASSERT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));
  // when a REST message is passed to the controller to remove a UDF that does
  // not exist
  auto future = cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                          "/v1/nes/udfCatalog/removeUdf"},
...
TEST_F(UDFCatalogControllerTest,
  ...
  ASSERT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));
  // when a REST message is passed to the controller that is missing the udfName
  // parameter
  auto future = cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                          "/v1/nes/udfCatalog/removeUdf"});
  future.wait();
...
TEST_F(UDFCatalogControllerTest,
  ...
  // Oatpp framework ignores all query parameters that aren't defined in the
  // endpoint, effectively ignoring them. Superfluous query parameters have no
  // effect on the behavior of an endpoint
  startCoordinator();
  ASSERT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));
  // when a REST message is passed to the controller that is contains parameters
  // other than the udfName parameter
  auto future = cpr::DeleteAsync(
      cpr::Url{BASE_URL + std::to_string(*restPort) +
               "/v1/nes/udfCatalog/removeUdf"},
