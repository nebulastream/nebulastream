...
class QueryControllerTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("QueryControllerTest.log",
  ...
  void SetUp() override {
    ...
    // Increment the query id so that the shared query and query plan ids are
    // different before the start of each test
    PlanIdGenerator::getNextQueryId();
  }

  static void TearDownTestCase() {
    NES_INFO("Tear down QueryControllerTest test class.");
  }
...
TEST_F(QueryControllerTest, testSubmitQueryNoUserQuery) {
  ...
  auto future =
      cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                              "/v1/nes/query/execute-query"},
  ...
  EXPECT_TRUE(
      errorMessage.find(
          "Incorrect or missing key word for user query, use 'userQuery'") !=
      std::string::npos);
  stopCoordinator();
}
...
TEST_F(QueryControllerTest, testSubmitQueryNoPlacement) {
  ...
  NES_DEBUG("{}", res.dump());
  std::string errorMessage = res["message"].get<std::string>();
  EXPECT_TRUE(errorMessage.find("No placement strategy specified. Specify a "
                                "placement strategy using 'placement'.") !=
              std::string::npos);
  stopCoordinator();
}

// Check if submitting a POST request with an unsupported 'placement' strategy
// returns 400
TEST_F(QueryControllerTest, testSubmitQueryInvalidPlacement) {
  ...
  NES_DEBUG("{}", res.dump());
  std::string errorMessage = res["message"].get<std::string>();
  EXPECT_TRUE(errorMessage.find("Invalid Placement Strategy: ") !=
              std::string::npos);
  stopCoordinator();
}
...
TEST_F(QueryControllerTest, testSubmitValidQuery) {
  ...
  workerConfiguration->physicalSourceTypes.add(
      DefaultSourceType::create("default_logical", "default_physical"));
  ...
  EXPECT_TRUE(TestUtils::checkRunningOrTimeout(
      queryId, std::to_string(coordinatorConfig->restPort.getValue())));
  stopCoordinator();
}
...
TEST_F(QueryControllerTest, testGetExecutionPlan) {
  ...
  request["faultTolerance"] = "AT_MOST_ONCE";
  request["lineage"] = "IN_MEMORY";
  auto f1 = cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                    "/v1/nes/query/execute-query"},
                           cpr::Header{{"Content-Type", "application/json"}},
                           cpr::Body{request.dump()});
  f1.wait();
  ...
  NES_DEBUG("{}", queryId);
  auto started =
      TestUtils::waitForQueryToStart(queryId, coordinator->getQueryCatalog());
  ASSERT_TRUE(started);
  auto f2 = cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
  ...
  for (auto executionNode : response2["ExecutionNodes"]) {
    const auto& rootWorkerNodeIds =
        coordinator->getTopology()->getRootWorkerNodeIds();
    const auto& executionNodeId = executionNode["WorkerId"].get<WorkerId>();
    auto foundInRootWorkerId = std::find(
        rootWorkerNodeIds.begin(), rootWorkerNodeIds.end(), executionNodeId);
    EXPECT_NE(foundInRootWorkerId, rootWorkerNodeIds.end());
    EXPECT_TRUE(executionNode["ScheduledDecomposedQueries"].size() != 0);
  }

  auto f3 = cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                   "/v1/nes/query/execution-plan"},
...
TEST_F(QueryControllerTest, testGetQueryPlan) {
  ...
  for (auto node : response2["nodes"]) {
    EXPECT_TRUE(node.contains("id") && node.contains("name") &&
...
TEST_F(QueryControllerTest, testGetQueryPlanNoSuchQueryId) {
  ...
  NES_INFO("QueryControllerTest: Coordinator started successfully");
  auto sourceCatalog = coordinator->getSourceCatalog();
  EXPECT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));
