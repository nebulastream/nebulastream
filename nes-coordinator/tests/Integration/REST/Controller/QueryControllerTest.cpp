...
TEST_F(QueryControllerTest, testSubmitQueryNoUserQuery)
{
    ...
    auto future = cpr::PostAsync(
        cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/query/execute-query"},
...
TEST_F(QueryControllerTest, testGetExecutionPlan)
{
    ...
    ASSERT_TRUE(started);
    auto f2 = cpr::GetAsync(
        cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/query/execution-plan"}, cpr::Parameters{{"queryId", queryId.toString()}});
    f2.wait();
    auto r2 = f2.get();
    ...
    auto f3 = cpr::GetAsync(
        cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/query/execution-plan"}, cpr::Parameters{{"queryId", std::to_string(0)}});
    f3.wait();
    auto r3 = f3.get();
...
TEST_F(QueryControllerTest, testGetQueryPlan)
{
    ...
    auto f2 = cpr::GetAsync(
        cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/query/query-plan"}, cpr::Parameters{{"queryId", queryId.toString()}});
    f2.wait();
    auto r2 = f2.get();
    EXPECT_EQ(r2.status_code, 200l);
    nlohmann::json response2;
...
TEST_F(QueryControllerTest, testGetQueryPlanNoSuchQueryId)
{
    ...
    EXPECT_TRUE(started);
    auto f2 = cpr::GetAsync(
        cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/query/query-plan"}, cpr::Parameters{{"queryId", std::to_string(0)}});
    f2.wait();
    auto r2 = f2.get();
    EXPECT_EQ(r2.status_code, 404l);
