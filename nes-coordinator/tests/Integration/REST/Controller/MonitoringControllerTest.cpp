...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MonitoringControllerTest.log",
  ...
  bool waitForMonitoringQuery(NesCoordinatorPtr coordinator,
                              ::uint64_t restPort, ::uint64_t timeout) {
    std::chrono::seconds timeoutInSec = std::chrono::seconds(timeout);
    nlohmann::json monitoringQueries =
        coordinator->getMonitoringService()->getMonitoringStreams();
  ...
  bool waitForMonitoringQueryEnd(NesCoordinatorPtr coordinator,
                                 ::uint64_t timeout) {
    std::chrono::seconds timeoutInSec = std::chrono::seconds(timeout);
    nlohmann::json monitoringQueries =
        coordinator->getMonitoringService()->getMonitoringStreams();
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down MonitoringControllerTest test class.");
  }
};

TEST_F(MonitoringControllerTest, testStartMonitoring) {
  NES_INFO("Tests for Oatpp Monitoring Controller start monitoring: Start "
           "coordinator");
  ...
  bool success = TestUtils::checkRESTServerStartedOrTimeout(
  ...
  cpr::Response r =
      cpr::Get(cpr::Url{"http://127.0.0.1:" + std::to_string(*restPort) +
                        "/v1/nes/monitoring/start"});
  ...
  NES_INFO("MonitoringControllerTest - Received Data from GetStart request: {}",
           jsonsStart.dump());
  ASSERT_EQ(jsonsStart.size(), expectedMonitoringStreams.size());
  bool check = MetricValidator::checkEntriesOfStream(expectedMonitoringStreams,
                                                     jsonsStart);
  ASSERT_TRUE(check);
  ASSERT_TRUE(waitForMonitoringQuery(
      coordinator, coordinatorConfig->restPort.getValue(), 5));
...
TEST_F(MonitoringControllerTest, testStopMonitoring) {
  ...
  cpr::Response r =
      cpr::Get(cpr::Url{"http://127.0.0.1:" + std::to_string(*restPort) +
                        "/v1/nes/monitoring/stop"});
  EXPECT_EQ(r.status_code, 200);
  bool stopCrd = coordinator->stopCoordinator(true);
  ASSERT_TRUE(stopCrd);
}

TEST_F(MonitoringControllerTest,
  ...
  auto future =
      cpr::GetAsync(cpr::Url{"http://127.0.0.1:" + std::to_string(*restPort) +
                             "/v1/nes/monitoring/start"});
  future.wait();
  auto r = future.get();
...
TEST_F(MonitoringControllerTest, testRequestAllMetrics) {
  NES_INFO("Tests for Oatpp Monitoring Controller - testRequestAllMetrics: "
           "Start coordinator");
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  ...
  NES_INFO(
      "MonitoringControllerTest - Received Data from GetAllMetrics request: {}",
      jsonsOfResponse);

  // check if content of r contains valid information (right fields and valid
  // queryIds):
  ASSERT_EQ(jsonsOfResponse.size(), 1);
  auto json = jsonsOfResponse[std::to_string(1)];
  NES_INFO("MonitoringControllerTest: Requesting monitoring data from node "
           "with ID {}",
           std::to_string(1));
  NES_INFO("Received Data for node 1: {}", json.dump());
  ASSERT_TRUE(MetricValidator::isValidAll(
      Monitoring::SystemResourcesReaderFactory::getSystemResourcesReader(),
      json));
...
TEST_F(MonitoringControllerTest, testGetMonitoringControllerDataFromOneNode) {
  NES_INFO("Tests for Oatpp Monitoring Controller - "
           "testGetMonitoringControllerDataFromOneNode: Start coordinator");
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
  ...
  nlohmann::json jsonsOfResponse;
  ASSERT_NO_THROW(jsonsOfResponse = nlohmann::json::parse(r.text));
  NES_INFO("MonitoringControllerTest - Received Data from GetMetricsForOneNode "
           "request: {}",
           jsonsOfResponse);
...
TEST_F(MonitoringControllerTest, testGetMonitoringControllerStorage) {
  NES_INFO("Tests for Oatpp Monitoring Controller - "
           "testGetMonitoringControllerStorage: Start coordinator");
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
  ...
  auto expected =
      monitoringService->requestNewestMonitoringDataFromMetricStoreAsJson();
  cpr::AsyncResponse future =
      cpr::GetAsync(cpr::Url{"http://127.0.0.1:" + std::to_string(*restPort) +
                             "/v1/nes/monitoring/storage"});
  future.wait();
  auto r = future.get();
  EXPECT_EQ(r.status_code, 200);
  ...
  NES_INFO(
      "MonitoringControllerTest - Received Data from Get-Storage request: {}",
      jsons);
  ...
  NES_INFO("MonitoringControllerTest: Requesting monitoring data from node "
           ...
           std::to_string(1));
  NES_INFO("Received Data for node 1: {}", json.dump());
  auto jsonRegistration =
      json[toString(Monitoring::MetricType::RegistrationMetric)][0]["value"];
  ASSERT_TRUE(MetricValidator::isValidRegistrationMetrics(
      Monitoring::SystemResourcesReaderFactory::getSystemResourcesReader(),
      jsonRegistration));
...
TEST_F(MonitoringControllerTest, testGetMonitoringControllerStreams) {
  NES_INFO("Tests for Oatpp Monitoring Controller - "
           "testGetMonitoringControllerStreams: Start coordinator");
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
  ...
  ASSERT_NO_THROW(jsons = nlohmann::json::parse(r.text));
  NES_INFO(
      "MonitoringControllerTest - Received Data from Get-Streams request: {}",
      jsons);
  std::set<std::string> expectedMonitoringStreams{
      toString(Monitoring::MetricType::WrappedNetworkMetrics),
      toString(Monitoring::MetricType::WrappedCpuMetrics),
      ...
      toString(Monitoring::MetricType::DiskMetric)};
  bool check =
      MetricValidator::checkEntriesOfStream(expectedMonitoringStreams, jsons);
  ASSERT_TRUE(check);
  ASSERT_TRUE(waitForMonitoringQuery(
      coordinator, coordinatorConfig->restPort.getValue(), 5));
  bool stopCrd = coordinator->stopCoordinator(true);
