...
TEST_F(MonitoringControllerTest, testStartMonitoring) {
  NES_INFO(
      "Tests for Oatpp Monitoring Controller start monitoring: Start "
      "coordinator");
...
TEST_F(MonitoringControllerTest, testRequestAllMetrics) {
  NES_INFO(
      "Tests for Oatpp Monitoring Controller - testRequestAllMetrics: Start "
      "coordinator");
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  ...
  NES_INFO(
      "MonitoringControllerTest: Requesting monitoring data from node with ID "
      "{}",
      std::to_string(1));
...
TEST_F(MonitoringControllerTest, testGetMonitoringControllerDataFromOneNode) {
  NES_INFO(
      "Tests for Oatpp Monitoring Controller - "
  ...
  ASSERT_NO_THROW(jsonsOfResponse = nlohmann::json::parse(r.text));
  NES_INFO(
      "MonitoringControllerTest - Received Data from GetMetricsForOneNode "
      "request: {}",
      jsonsOfResponse);
