...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("QueryCatalogControllerTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup QueryCatalogControllerTest test class.");
  }

  static void TearDownTestCase() {
    NES_INFO("Tear down QueryCatalogControllerTest test class.");
  }
...
// Test that allRegisteredQueries first returns an empty json when no queries
// are registered and then a non-empty one after a query has been registered
TEST_F(QueryCatalogControllerTest, testGetRequestAllRegistedQueries) {
  ...
  cpr::AsyncResponse future1 =
      cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
...
// Test queries endpoint: 400 if no status provided, otherwise 200. Depending on
// if a query is registered or not either an empty json body or non-empty
TEST_F(QueryCatalogControllerTest, testGetQueriesWithSpecificStatus) {
  ...
  cpr::AsyncResponse future1 = cpr::GetAsync(cpr::Url{
      BASE_URL + std::to_string(*restPort) + "/v1/nes/queryCatalog/queries"});
  future1.wait();
  auto r1 = future1.get();
  ...
  QueryId queryId = PlanIdGenerator::getNextQueryId();
  queryPlan->setQueryId(queryId);
  queryCatalog->createQueryCatalogEntry("queryString", queryPlan,
                                        Optimizer::PlacementStrategy::BottomUp,
                                        QueryState::REGISTERED);

  // when making a request for a query with a specific status after having
  // submitted a query
  cpr::AsyncResponse future3 =
      cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                             "/v1/nes/queryCatalog/queries"},
                    cpr::Parameters{{"status", "REGISTERED"}});
...
TEST_F(QueryCatalogControllerTest, testGetRequestStatusOfQuery) {
  ...
  // when sending a request to the status endpoint without specifying a
  // 'queryId' query parameter
  cpr::AsyncResponse f1 = cpr::GetAsync(cpr::Url{
      BASE_URL + std::to_string(*restPort) + "/v1/nes/queryCatalog/status"});
  f1.wait();
  auto r1 = f1.get();
  // return 400 BAD REQUEST
  EXPECT_EQ(r1.status_code, 400l);

  // when sending a request to the status endpoint with 'queryId' supplied but
  // no such query registered
  cpr::AsyncResponse f2 =
      cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                             "/v1/nes/queryCatalog/status"},
  ...
  // when sending a request to the status endpoint with 'queryId' supplied and a
  // query with specified id registered
