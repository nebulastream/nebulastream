...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LocationControllerIntegrationTest.log",
    ...
    std::string singleLocationPath2 =
        std::filesystem::path(TEST_DATA_DIRECTORY) / "singleLocation2.csv";
    remove(singleLocationPath2.c_str());
    writeWaypointsToCsv(singleLocationPath2, {{{53.5523, -13.3517}, 0}});
  }

  static void TearDownTestCase() {
    NES_INFO("Tear down LocationControllerIntegrationTest test class.");
  }
  ...
  void startCoordinator() {
    ...
    NES_INFO(
        "LocationControllerIntegrationTest: Coordinator started successfully");
...
TEST_F(LocationControllerIntegrationTest,
  ...
  auto future = cpr::GetAsync(
      cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/location"});
  future.wait();
...
TEST_F(LocationControllerIntegrationTest, testGetLocationNonNumericalNodeId) {
  ...
  ASSERT_EQ(errorMessage,
            "Invalid QUERY parameter 'nodeId'. Expected type is 'UInt64'");
  bool stopCrd = coordinator->stopCoordinator(true);
  ASSERT_TRUE(stopCrd);
}
...
TEST_F(LocationControllerIntegrationTest, testGetSingleLocation) {
  ...
  wrkConf1->locationCoordinates.setValue(
      NES::Spatial::DataTypes::Experimental::GeoLocation(latitude, longitude));
...
TEST_F(LocationControllerIntegrationTest,
  ...
  nlohmann::json res;
  ASSERT_NO_THROW(
      res = nlohmann::json::parse(response.text).get<allMobileResponse>());
...
#ifdef S2DEF
TEST_F(LocationControllerIntegrationTest,
       testGetAllMobileLocationMobileNodesExist) {
  startCoordinator();
  ASSERT_TRUE(TestUtils::checkRESTServerStartedOrTimeout(
      coordinatorConfig->restPort.getValue(), 5));
  ...
  wrkConf2->mobilityConfiguration.locationProviderConfig.setValue(
      std::filesystem::path(TEST_DATA_DIRECTORY) / "singleLocation.csv");
  ...
  wrkConf3->mobilityConfiguration.locationProviderType.setValue(
      NES::Spatial::Mobility::Experimental::LocationProviderType::CSV);
  wrkConf3->mobilityConfiguration.locationProviderConfig.setValue(
      std::filesystem::path(TEST_DATA_DIRECTORY) / "singleLocation2.csv");
