...
class SourceCatalogControllerTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SourceCatalogControllerTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup SourceCatalogControllerTest test class.");
  }
  /* Will be called after all tests in this class are finished. */
  static void TearDownTestCase() {
    NES_INFO("Tear down SourceCatalogControllerTest test class.");
  }
...
TEST_F(SourceCatalogControllerTest, testGetAllLogicalSource) {
  ...
  cpr::AsyncResponse future =
      cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                             "/v1/nes/sourceCatalog/allLogicalSource"});
  future.wait();
...
TEST_F(SourceCatalogControllerTest, testGetPhysicalSource) {
  ...
  NES_INFO(
      "{}",
      coordinator->getSourceCatalog()->getPhysicalSourceAndSchemaAsString());
  cpr::AsyncResponse future =
      cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                             "/v1/nes/sourceCatalog/allPhysicalSource"},
  ...
  nlohmann::json response;
  ASSERT_NO_THROW(response = nlohmann::json::parse(r.text));
  ASSERT_TRUE(response.contains("Physical Sources") &&
              response["Physical Sources"].size() != 0);
...
TEST_F(SourceCatalogControllerTest, testPostLogicalSource) {
  ...
  cpr::AsyncResponse future =
      cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                              "/v1/nes/sourceCatalog/addLogicalSource"},
  ...
  SchemaPtr schemaFromCoordinator =
      sourceCatalog->getLogicalSource("car")->getSchema();
  // TODO: is it a bug that one has to define the field name with a '$' in the
  // schema for it to be found using hasFieldName ?
...
TEST_F(SourceCatalogControllerTest, testUpdateLogicalSource) {
  ...
  cpr::AsyncResponse future =
      cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                              "/v1/nes/sourceCatalog/updateLogicalSource"},
                     cpr::Header{{"Content-Type", "application/json"}},
                     cpr::Body{request.dump()});
  future.wait();
  cpr::Response r = future.get();
...
TEST_F(SourceCatalogControllerTest, testDeleteLogicalSource) {
  ...
  cpr::AsyncResponse future =
      cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                "/v1/nes/sourceCatalog/deleteLogicalSource"},
                       cpr::Parameters{{"logicalSourceName", "test_stream"}});
  future.wait();
  cpr::Response r = future.get();
  ASSERT_EQ(r.status_code, 200l);
