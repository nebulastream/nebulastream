...
TEST_F(TopologyControllerTest, testGetTopology) {
  ...
  cpr::Response r = cpr::Get(
      cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/topology"});
  ...
  for (auto node : response["nodes"]) {
    EXPECT_TRUE(node.contains("id") && node.contains("ip_address") &&
                node.contains("nodeType") && node.contains("location") &&
                node.contains("available_resources"));
  }
  bool stopCrd = coordinator->stopCoordinator(true);
  NES_DEBUG("shut down coordinator with rest port {}",
            coordinatorConfig->restPort.getValue());
  EXPECT_TRUE(stopCrd);
}
...
TEST_F(TopologyControllerTest, testAddParentSuccess) {
  ...
  request["parentId"] = 2;
  request["childId"] = 3;
  auto response = cpr::Post(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                     "/v1/nes/topology/addAsChild"},
...
TEST_F(TopologyControllerTest, testaddAsChildNoSuchChild) {
  ...
  ASSERT_NO_THROW(res = nlohmann::json::parse(response.text));
  NES_DEBUG("{}", res.dump());
  EXPECT_EQ(res["message"], "Could not add/remove parent for node in topology: "
                            "Node with childId=7 not found.");
  bool stopCrd = coordinator->stopCoordinator(true);
  NES_DEBUG("shut down coordinator with rest port {}",
            coordinatorConfig->restPort.getValue());
  EXPECT_TRUE(stopCrd);
...
TEST_F(TopologyControllerTest, testaddAsChildNoSuchParent) {
  ...
  ASSERT_TRUE(
      TestUtils::waitForWorkers(coordinatorConfig->restPort.getValue(), 5, 1));
  ...
  EXPECT_EQ(res["message"], "Could not add/remove parent for node in topology: "
                            "Node with parentId=3 not found.");
  bool stopCrd = coordinator->stopCoordinator(true);
  NES_DEBUG("shut down coordinator with rest port {}",
            coordinatorConfig->restPort.getValue());
  EXPECT_TRUE(stopCrd);
...
TEST_F(TopologyControllerTest, testaddAsChildAlreadyExists) {
  ...
  EXPECT_EQ(res["message"],
            "Could not add parent for node in topology: Node with childId=2 is "
            "already a child of node with parentID=1.");
  bool stopWrk1 = wrk1->stop(true);
  EXPECT_TRUE(stopWrk1);
  bool stopCrd = coordinator->stopCoordinator(true);
  NES_DEBUG("shut down coordinator with rest port {}",
...
TEST_F(TopologyControllerTest, testaddAsChild) {
  ...
  request["childId"] = 2;
  auto asyncResp =
      cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                "/v1/nes/topology/removeAsChild"},
                       cpr::Header{{"Content-Type", "application/json"}},
                       cpr::Body{request.dump()}, cpr::ConnectTimeout{3000},
                       cpr::Timeout{30000});
  asyncResp.wait();
  ...
  auto response1 = cpr::Post(cpr::Url{BASE_URL + std::to_string(*restPort) +
                                      "/v1/nes/topology/addAsChild"},
                             cpr::Header{{"Content-Type", "application/json"}},
...
TEST_F(TopologyControllerTest, testRemoveParentChildIDIsNotChildOfParent) {
  ...
  NES_DEBUG("{}", res.dump());
  EXPECT_EQ(res["message"],
            "Could not remove parent for node in topology: Node with childId=3 "
            "is not a child of node with parentID=2.");
  bool stopWrk1 = wrk1->stop(true);
  EXPECT_TRUE(stopWrk1);
