...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("UnionDeploymentTest.log",
  ...
  void SetUp() override {
    ...
    // Setup sources.
    sourceCar = TestUtils::createSourceTypeCSV(
        {"car", "car_physical", "car.csv", 1, 40, 1});
    sourceTruck = TestUtils::createSourceTypeCSV(
        {"truck", "truck_physical", "truck.csv", 1, 40, 1});
    sourceRuby = TestUtils::createSourceTypeCSV(
        {"ruby", "ruby_physical", "window.csv", 1, 28, 1});
    sourceDiamond = TestUtils::createSourceTypeCSV(
        {"diamond", "diamond_physical", "window.csv", 1, 28, 1});
...
TEST_F(UnionDeploymentTest, testDeployTwoWorkerMergeUsingBottomUp) {
  ...
  // Expected output the first 40 entries are car values (0,0 - 39,39), the last
  // 40 entries are truck values (0,0 - 39,3900).
...
TEST_F(UnionDeploymentTest, testDeployTwoWorkerMergeUsingTopDown) {
  std::string outputFilePath =
      getTestResourceFolder() / "testDeployTwoWorkerMergeUsingTopDown.out";
  const auto query = Query::from("car").unionWith(Query::from("truck"));
  TestHarness testHarness =
      TestHarness(query, *restPort, *rpcCoordinatorPort,
                  getTestResourceFolder())
...
/**
 * Test deploying a union query with filters and sources on two different worker
 * nodes using bottomUp strategy.
 */
TEST_F(UnionDeploymentTest,
       testOneFilterPushDownWithMergeOfTwoDifferentSources) {
  std::string outputFilePath =
      getTestResourceFolder() / "testPushingTwoFiltersBelowAndTwoFiltersAlready"
                                "AtBottomWithMergeOfTwoDifferentSources.out";
...
TEST_F(
  ...
  // Expected output the first 17 entries are raw entries from the window.csv
  // file stream. The next 12 entries are the result of the second query of the
  // unionWith statement.
  std::stringstream expectedOutput;
...
/**
 * Test a union query with multiple filters for both sources and two different
 * worker nodes using the bottomUp strategy.
 */
TEST_F(UnionDeploymentTest,
  ...
  // Expected output the first 18 entries come from the first part of the
  // unionWith statement. The next 2 entries are the result of the second query
  // of the unionWith statement.
  std::stringstream expectedOutput;
  uint32_t numFirstQueryValues = 18;
