...
class LambdaSourceIntegrationTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LambdaSourceIntegrationTest.log",
...
TEST_F(LambdaSourceIntegrationTest, testTwoLambdaSources) {
  NES::CoordinatorConfigurationPtr coordinatorConfig =
      NES::CoordinatorConfiguration::createDefault();
  ...
  wrkConf->queryCompiler.queryCompilerType =
      QueryCompilation::QueryCompilerType::NAUTILUS_QUERY_COMPILER;

  auto func1 = [](NES::Runtime::TupleBuffer& buffer,
  ...
  auto func2 = [](NES::Runtime::TupleBuffer& buffer,
                  uint64_t numberOfTuplesToProduce) {
    struct Record {
      uint64_t id;
      uint64_t value;
  ...
  auto lambdaSourceType1 =
      LambdaSourceType::create("input1", "test_stream1", std::move(func1), 3,
                               10, GatheringMode::INTERVAL_MODE);
  auto lambdaSourceType2 =
      LambdaSourceType::create("input2", "test_stream2", std::move(func2), 3,
                               10, GatheringMode::INTERVAL_MODE);
...
TEST_F(LambdaSourceIntegrationTest, testTwoLambdaSourcesWithSamePhysicalName) {
  NES::CoordinatorConfigurationPtr crdConf =
      NES::CoordinatorConfiguration::createDefault();
  ...
  auto lambdaSourceType1 =
      LambdaSourceType::create("input1", "test_stream", std::move(func1), 3, 10,
                               GatheringMode::INTERVAL_MODE);
  auto lambdaSourceType2 =
      LambdaSourceType::create("input2", "test_stream", std::move(func2), 3, 10,
                               GatheringMode::INTERVAL_MODE);
  wrkConf->physicalSourceTypes.add(lambdaSourceType1);
  wrkConf->physicalSourceTypes.add(lambdaSourceType2);
  auto wrk1 = std::make_shared<NES::NesWorker>(std::move(wrkConf));
  ...
  auto query1 = Query::from("input1")
                    .filter(Attribute("value") > 10000)
                    .sink(NullOutputSinkDescriptor::create());

  auto query2 = Query::from("input2")
                    .filter(Attribute("value") > 10000)
                    .sink(NullOutputSinkDescriptor::create());
  ...
  NES::RequestHandlerServicePtr requestHandlerService =
      crd->getRequestHandlerService();
  auto queryCatalog = crd->getQueryCatalog();
  auto queryId1 = requestHandlerService->validateAndQueueAddQueryRequest(
      query1.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);

  auto queryId2 = requestHandlerService->validateAndQueueAddQueryRequest(
...
TEST_F(LambdaSourceIntegrationTest, testTwoLambdaSourcesMultiThread) {
  ...
  for (int64_t i = 0; i < 2; i++) {
    auto func = [](NES::Runtime::TupleBuffer& buffer,
                   uint64_t numberOfTuplesToProduce) {
      struct Record {
        uint64_t id;
        uint64_t value;
      ...
      for (auto u = 0u; u < numberOfTuplesToProduce; ++u) {
        records[u].id = u;
        // values between 0..9 and the predicate is > 5 so roughly 50%
        // selectivity
        records[u].value = u % 10;
        records[u].timestamp = ts;
      }
    ...
    auto lambdaSourceType = LambdaSourceType::create(
        "input", "test_stream" + std::to_string(i), std::move(func), 30, 0,
        GatheringMode::INTERVAL_MODE);
