...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("VariableLengthIntegrationTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup VariableLengthIntegrationTest test class.");
  }
};

// This test reads from a csv sink which contains variable-length fields and
// applies a filter and a map
TEST_F(VariableLengthIntegrationTest,
       testCsvSourceWithVariableLengthFieldsFilterOnMap) {
  const std::string inputFileName = "variable-length.csv";
  const std::string expectedCsvFile = "variable-length_3.csv";
  const std::string outputFileName =
      "testCsvSourceWithVariableLengthFields.csv";
  ...
  auto testSchema =
      Schema::create()
          ...
          ->addField("data",
                     BasicType::TEXT); // TEXT is the variable length field

  auto query = Query::from("variable_length")
                   .map(Attribute("camera_id_2") = Attribute("camera_id") + 10)
                   .filter(Attribute("camera_id_2") < 55)
                   .project(Attribute("camera_id"), Attribute("timestamp"),
                            Attribute("rows"), Attribute("cols"),
                            Attribute("type"), Attribute("data"));

  // setup csv sources
  CSVSourceTypePtr csvSourceType =
      CSVSourceType::create("variable_length", "test_stream");
  ...
  auto tmpBuffers = TestUtils::createExpectedBuffersFromCsv(
      expectedCsvFile, testSchema, testHarness.getBufferManager());
  ...
  // We require TestTupleBuffers, therefore we create them with the output
  // schema. This step can be only done after the query has been run.
  ...
  // Comparing equality
  EXPECT_TRUE(TestUtils::buffersContainSameTuples(expectedBuffers,
                                                  actualBuffers, true));
}
...
// applies a filter
TEST_F(VariableLengthIntegrationTest,
       testCsvSourceWithVariableLengthFieldsFilter) {
  const std::string inputFileName = "variable-length.csv";
  ...
  auto tmpBuffers = TestUtils::createExpectedBuffersFromCsv(
      expectedCsvFile, testSchema, testHarness.getBufferManager());
  auto expectedTuples =
      std::accumulate(tmpBuffers.begin(), tmpBuffers.end(), 0_u64,
...
// This test reads from a csv sink which contains variable-length fields without
