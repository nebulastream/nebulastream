...
// FIXME: This is a hack to fix issue with unreleased RPC port after shutting
// down the servers while running tests in continuous succession
//  by assigning a different RPC port for each test case

class ContinuousSourceTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ContinuousSourceTest.log",
...
TEST_F(ContinuousSourceTest, testMultipleOutputBufferFromDefaultSourcePrint) {
  ...
  workerConfig1->queryCompiler.queryCompilerType =
      QueryCompilation::QueryCompilerType::NAUTILUS_QUERY_COMPILER;
  auto defaultSourceType1 =
      DefaultSourceType::create("testStream", "test_stream");
  ...
  auto query = Query::from("testStream")
                   .filter(Attribute("campaign_id") < 42)
...
TEST_F(ContinuousSourceTest,
  ...
  string expectedContent =
      "testStream$campaign_id:INTEGER(64 bits)\n"
      "1\n"
      "1\n"
      "1\n"
      "1\n"
...
TEST_F(ContinuousSourceTest,
  ...
  std::string outputFilePath = getTestResourceFolder() /
                               "testMultipleOutputBufferFromDefaultSourceWriteF"
                               "ileWithLargerFrequency.txt";
  remove(outputFilePath.c_str());
...
TEST_F(ContinuousSourceTest, testMultipleOutputBufferFromCSVSourceWrite) {
  ...
  string expectedContent =
      "testStream$val1:INTEGER(64 bits),testStream$val2:INTEGER(64 "
      "bits),testStream$val3:INTEGER(64 bits)\n"
      "1,2,3\n"
      "1,2,4\n"
      "4,3,6\n";
...
TEST_F(ContinuousSourceTest, testTimestampCsvSink) {
  ...
  std::string outputFilePath =
      getTestResourceFolder() / "testTimestampCsvSink.out";
  remove(outputFilePath.c_str());
