...
class BenchmarkSourceIntegrationTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BenchmarkSourceIntegrationTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup BenchmarkSourceIntegrationTest test class.");
  }
};

/// This test checks that a deployed BenchmarkSource can write M records
/// spanning exactly N records
TEST_F(BenchmarkSourceIntegrationTest, testBenchmarkSource) {
  ...
  auto benchmarkSourceType = BenchmarkSourceType::create(
      "memory_stream", "memory_stream_0", memArea, memAreaSize, buffersToASSERT,
      0, GatheringMode::INTERVAL_MODE, SourceMode::COPY_BUFFER, 0, 0);
  ...
  ASSERT_TRUE(TestUtils::checkCompleteOrTimeout(crd, queryId, globalQueryPlan,
                                                buffersToASSERT));
  ...
  while (std::getline(infile, line)) {
    if (lineCnt > 0) {
      std::string ASSERTedString =
          std::to_string(lineCnt - 1) + "," + std::to_string(lineCnt - 1);
...
/// This test checks that a deployed MemorySource can write M records stored in
/// one buffer that is not full
TEST_F(BenchmarkSourceIntegrationTest, testMemorySourceFewTuples) {
  ...
  auto benchmarkSourceType = BenchmarkSourceType::create(
      "memory_stream", "memory_stream_0", memArea, memAreaSize, 1, 0,
      GatheringMode::INTERVAL_MODE, SourceMode::COPY_BUFFER, 0, 0);
  wrkConf->physicalSourceTypes.add(benchmarkSourceType);
  NesWorkerPtr wrk1 = std::make_shared<NesWorker>(std::move(wrkConf));
  bool retStart1 = wrk1->start(/**blocking**/ false, /**withConnect**/ true);
  ASSERT_TRUE(retStart1);
...
/// This test checks that a deployed MemorySource can write M records stored in
/// N+1 buffers with the invariant that the N+1-th buffer is half full

TEST_F(BenchmarkSourceIntegrationTest,
  ...
  constexpr auto bufferSizeInNodeEngine = 4096;  // TODO load this from config!
  constexpr auto memAreaSize =
      bufferSizeInNodeEngine * 64 + (bufferSizeInNodeEngine / 2);
  constexpr auto buffersToASSERT = memAreaSize / bufferSizeInNodeEngine;
  auto recordsToASSERT = memAreaSize / schema->getSchemaSizeInBytes();
  auto* memArea = reinterpret_cast<uint8_t*>(malloc(memAreaSize));
  auto* records = reinterpret_cast<Record*>(memArea);
  ...
  auto benchmarkSourceType = BenchmarkSourceType::create(
      "memory_stream", "memory_stream_0", memArea, memAreaSize,
      buffersToASSERT + 1, 0, GatheringMode::INTERVAL_MODE,
