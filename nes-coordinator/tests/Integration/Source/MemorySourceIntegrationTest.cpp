...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MemorySourceIntegrationTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup MemorySourceIntegrationTest test class.");
  }
};

/// This test checks that a deployed MemorySource can write M records spanning
/// exactly N records
TEST_F(MemorySourceIntegrationTest, testMemorySource) {
  ...
  auto memorySourceType = MemorySourceType::create(
      "memory_stream", "memory_stream_0", memArea, memAreaSize, buffersToExpect,
      0, GatheringMode::INTERVAL_MODE);
...
TEST_F(MemorySourceIntegrationTest, testMemorySourceFewTuples) {
  ...
  auto memorySourceType =
      MemorySourceType::create("memory_stream", "memory_stream_0", memArea,
                               memAreaSize, 1, 0, GatheringMode::INTERVAL_MODE);
  wrkConf->physicalSourceTypes.add(memorySourceType);
  NesWorkerPtr wrk1 = std::make_shared<NesWorker>(std::move(wrkConf));
  bool retStart1 = wrk1->start(/**blocking**/ false, /**withConnect**/ true);
  ASSERT_TRUE(retStart1);
...
TEST_F(MemorySourceIntegrationTest, testMemorySourceHalfFullBuffer) {
  ...
  const auto memAreaSize =
      bufferSizeInNodeEngine + (bufferSizeInNodeEngine / 2);
  ...
  NES_DEBUG("Creating {} records and a capacity of {} records...", numRecords,
            bufferCapacity);
  for (auto i = 0U; i < numRecords; ++i) {
    records[i].key = i;
    records[i].timestamp = i;
  ...
  auto memorySourceType = MemorySourceType::create(
      ...
      buffersToExpect + 1, 0, GatheringMode::INTERVAL_MODE);
