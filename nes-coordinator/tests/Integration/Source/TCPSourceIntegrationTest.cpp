...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TCPSourceIntegrationTest.log",
  ...
  void startServer() {
    ...
    // After one test was executed, the remaining test calls to create a socket
    // initially return 0 The close method does not recognize 0 as a valid file
    // descriptor and therefore will not close the socket properly, hence we use
    // a while loop to retrieve a valid file descriptor
    uint8_t counter = 0;
    while (sockfd == 0 && counter < 10) {
      sockfd = socket(AF_INET, SOCK_STREAM, 0);
      NES_TRACE("Retrieved sockfd: {} on try: {}", sockfd,
    ...
    sockaddr.sin_port = htons(*tcpServerPort); // htons is necessary to convert
                                               // a number to network byte order
    int opt = 1;
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
      NES_ERROR("TCPSourceIntegrationTest: Failed to create socket. errno: {}",
                errno);
      exit(EXIT_FAILURE);
    }
    if (bind(sockfd, (struct sockaddr *)&sockaddr, sizeof(sockaddr)) < 0) {
      ...
      NES_ERROR(
          "TCPSourceIntegrationTest: Failed to bind to port {}. errno: {}",
          tcpServerPortAsString.str(), errno);
      exit(EXIT_FAILURE);
    }
    ...
    if (listen(sockfd, 10) < 0) {
      NES_ERROR(
          "TCPSourceIntegrationTest: Failed to listen on socket. errno: {}",
          errno);
      exit(EXIT_FAILURE);
    }
  ...
  void stopServer() {
    ...
    if (closed == -1) {
      NES_ERROR(
          "TCPSourceIntegrationTest::stopServer: Could not close socket. {}",
          strerror(errno));
      exit(EXIT_FAILURE);
    }
  }
  ...
  /**
   * @brief sends multiple messages via TCP connection. Static because this is
   * run in a thread inside the test cases
  ...
  int sendMessages(std::string message, int repeatSending) {
    ...
    if (connection < 0) {
      NES_ERROR(
          "TCPSourceIntegrationTest: Failed to grab connection. errno: {}",
          strerror(errno));
      return -1;
    }
    for (int i = 0; i < repeatSending; ++i) {
      NES_TRACE("TCPSourceIntegrationTest: Sending message: {} iter={}",
                message, i);
  ...
  /**
   * @brief sending different comma seperated messages via TCP to test variable
   * length read. Static because it is run in a threat inside test cases
   */
  ...
  /**
   * @brief sending different JSON messages via TCP to test variable length
   * read. Static because it is run in a threat inside test cases
   */
...
/**
 * @brief tests TCPSource read of csv data that is seperated by a given token.
 * Here \n is used
 */
TEST_F(TCPSourceIntegrationTest, TCPSourceReadCSVDataWithSeparatorToken) {
  ...
  // register query
  auto query =
      Query::from("tcpStream").sink(FileSinkDescriptor::create(filePath));
  ...
  std::thread serverThread(
      [&connection, this] { connection = sendMessages("42,5.893,true\n", 6); });
  ASSERT_TRUE(
  ...
  string expectedContent =
      "tcpStream$id:INTEGER(32 bits),tcpStream$value:Float(32 "
      "bits),tcpStream$onTime:Boolean\n"
      "42,5.893000,1\n"
      "42,5.893000,1\n"
      "42,5.893000,1\n"
      "42,5.893000,1\n"
...
/**
 * @brief tests TCPSource read of JSON data that is seperated by a given token.
 * Here \n is used
 */
TEST_F(TCPSourceIntegrationTest,
       DISABLED_TCPSourceReadJSONDataWithSeparatorToken) {
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
...
/**
 * @brief tests TCPSource read of CSV data when obtaining the size of the data
 * from the socket. Constant length
 */
TEST_F(TCPSourceIntegrationTest,
       DISABLED_TCPSourceReadCSVDataLengthFromSocket) {
  ...
  sourceConfig->setInputFormat(Configurations::InputFormat::CSV);
  sourceConfig->setDecideMessageSize(
      Configurations::TCPDecideMessageSize::BUFFER_SIZE_FROM_SOCKET);
...
/**
 * @brief tests TCPSource read of CSV data when obtaining the size of the data
 * from the socket. Variable length
 */
TEST_F(TCPSourceIntegrationTest, DISABLED_TCPSourceReadCSVWithVariableLength) {
  ...
  int connection;
  std::thread serverThread(
      [&connection, this] { connection = sendMessageCSVVariableLength(); });
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 2));
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(crd, queryId, globalQueryPlan, 2));
...
/**
 * @brief tests TCPSource read of JSON data when obtaining the size of the data
 * from the socket. Constant length
 */
TEST_F(TCPSourceIntegrationTest,
       DISABLED_TCPSourceReadJSONDataLengthFromSocket) {
  auto coordinatorConfig = CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
  NES_INFO("TCPSourceIntegrationTest: Start coordinator");
  ...
  std::thread serverThread([&connection, this] {
    connection = sendMessages(
        "44{\"id\":\"42\", \"value\":\"5.893\", \"name\":\"hello\"}", 6);
  });
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 2));
  ASSERT_TRUE(
 ...
 */
TEST_F(TCPSourceIntegrationTest,
       DISABLED_TCPSourceReadJSONDataWithVariableLength) {
  auto coordinatorConfig = CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
  NES_INFO("TCPSourceIntegrationTest: Start coordinator");
  ...
  int connection;
  std::thread serverThread(
      [&connection, this] { connection = sendMessageJSONVariableLength(); });
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 2));
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(crd, queryId, globalQueryPlan, 2));
...
/**
 * @brief tests TCPSource read of CSV data with fixed length inputted at source
 * creation time
 */
TEST_F(TCPSourceIntegrationTest, DISABLED_TCPSourceReadCSVDataWithFixedSize) {
  ...
  TCPSourceTypePtr sourceConfig =
      TCPSourceType::create("tcpStream", "tcpStream");
  sourceConfig->setSocketPort(*tcpServerPort);
  sourceConfig->setSocketHost("127.0.0.1");
  sourceConfig->setSocketDomainViaString("AF_INET");
  sourceConfig->setSocketTypeViaString("SOCK_STREAM");
  ...
  sourceConfig->setInputFormat(Configurations::InputFormat::CSV);
  sourceConfig->setDecideMessageSize(
      Configurations::TCPDecideMessageSize::USER_SPECIFIED_BUFFER_SIZE);
  ...
  int connection;
  std::thread serverThread(
      [&connection, this] { connection = sendMessages("42,5.893,hello", 6); });
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 2));
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(crd, queryId, globalQueryPlan, 2));
...
TEST_F(TCPSourceIntegrationTest, DISABLED_TCPSourceReadJSONDataWithFixedSize) {
  ...
  std::thread serverThread([&connection, this] {
    connection = sendMessages(
        "{\"id\":\"42\", \"value\":\"5.893\", \"name\":\"hello\"}", 6);
  });
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 2));
  ASSERT_TRUE(
