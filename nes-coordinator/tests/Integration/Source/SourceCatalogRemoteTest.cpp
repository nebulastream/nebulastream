...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SourceCatalogRemoteTest.log",
...
TEST_F(SourceCatalogRemoteTest, addPhysicalToExistingLogicalSourceRemote) {
  ...
  auto csvSourceType1 =
      CSVSourceType::create("default_logical", "physical_test");
  ...
  std::vector<Catalogs::Source::SourceCatalogEntryPtr> phys =
      crd->getSourceCatalog()->getPhysicalSources("default_logical");

  EXPECT_EQ(phys.size(), 1U);
  EXPECT_EQ(phys[0]->getPhysicalSource()->getPhysicalSourceName(),
            "physical_test");
...
TEST_F(SourceCatalogRemoteTest, addPhysicalToNewLogicalSourceRemote) {
  ...
  cout << crd->getSourceCatalog()->getPhysicalSourceAndSchemaAsString() << endl;
  std::vector<Catalogs::Source::SourceCatalogEntryPtr> phys =
      crd->getSourceCatalog()->getPhysicalSources("testSource");
...
TEST_F(SourceCatalogRemoteTest, removeNotExistingSourceRemote) {
  ...
  auto invalidPhysicalSource = CSVSourceType::create(invalidLogicalSourceName,
                                                     invalidPhysicalSourceName);
  physicalSourceTypes.push_back(invalidPhysicalSource);
  bool success = wrk1->unregisterPhysicalSource(physicalSourceTypes);
  EXPECT_TRUE(!success);

  SchemaPtr sPtr =
      crd->getSourceCatalog()->getSchemaForLogicalSource("default_logical");
