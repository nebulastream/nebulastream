...
public:
    ...
    NesWorkerPtr createWorker(
        const string& defaultLogicalSourceName, const std::string& physicalSourceName, const uint64_t coordinatorRPCPort, WorkerId workerId)
    {
...
TEST_P(StatisticsIntegrationTest, singleTrackAndProbeDataCharacteristic)
{
    ...
    for (const auto& statisticKey : allStatisticKeys)
    {
        ...
        const StatisticProbeRequest probeRequest(statisticKey.hash(), Milliseconds(windowSize), ProbeExpression(expressionNodePtr));
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Combine(
        ::testing::Values(1, 2, 3), // No. workers
    ...
    {
        return std::string(
            std::to_string(std::get<0>(info.param)) + "Workers_" + std::to_string(std::get<1>(info.param)) + "NumberOfBuffers_"
            + std::to_string(std::get<2>(info.param)) + "WindowSize_" + std::to_string(info.index) + "Metric");
    });
} // namespace NES
