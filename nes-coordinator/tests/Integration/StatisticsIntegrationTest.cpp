...
class StatisticsIntegrationTest
    : public Testing::BaseIntegrationTest,
      public ::testing::WithParamInterface<
          std::tuple<int, int, int, Statistic::MetricPtr>> {
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("StatisticsIntegrationTest.log",
  ...
  void SetUp() override {
    NES_INFO("StatisticsIntegrationTest: Setup StatisticsIntegrationTest test "
             "class.");
    ...
    nesCoordinator->getSourceCatalogService()->registerLogicalSource(
        logicalSourceName, testInputSchema);
    NES_DEBUG("Coordinator started successfully");

    // Create workers
    for (uint64_t i = 0; i < numberOfWorkers; ++i) {
      const auto physicalSourceName =
          basePhysicalSourceName + std::to_string(i);
      physicalSourceNames.emplace_back(physicalSourceName);
      auto worker = createWorker(logicalSourceName, physicalSourceName,
                                 coordinatorRPCPort, WorkerId(i + 2));
  ...
  void TearDown() override {
    NES_INFO("StatisticsIntegrationTest: TearDown StatisticsIntegrationTest "
             "test class.");
    for (const auto &worker : allWorkers) {
      worker->stop(false);
    }
  ...
  NesWorkerPtr createWorker(const string &defaultLogicalSourceName,
                            ...
                            WorkerId workerId) {
    // Create the generator function that produces data id (rand() % 1000) and
    // timestamp monotonic increasing
    auto createData = [this](Runtime::TupleBuffer &buffer,
    ...
    auto lambdaSourceType = LambdaSourceType::create(
        defaultLogicalSourceName, physicalSourceName, createData,
        numberOfBuffersToProduce, gatheringValue, GatheringMode::INTERVAL_MODE);
...
/**
 * @brief Test case for creating a statistic, probing it and deleting it for a
 * cardinality
 */
TEST_P(StatisticsIntegrationTest, singleTrackAndProbeDataCharacteristic) {
  ...
  // Creating the trackStatistic request. Once we fix issue #4778 and #4776, we
  // can call it for each physical source name
  auto characteristic = DataCharacteristic::create(metric, logicalSourceName,
                                                   physicalSourceNames[0]);
  auto window = Windowing::TumblingWindow::of(
      EventTime(Attribute(timestampFieldName)), Milliseconds(windowSize));
  auto allStatisticKeys =
      requestHandlerService->trackStatisticRequest(characteristic, window);
  ASSERT_FALSE(allStatisticKeys.empty());

  // Waiting till all statistic queries are done creating the statistic. This
  // works, as the lambda source is finite
  for (const auto &statisticKey : allStatisticKeys) {
    const auto queryId = probeHandler->getStatisticQueryId(statisticKey);
    if (!TestUtils::checkStoppedOrTimeout(queryId,
  ...
  for (const auto &statisticKey : allStatisticKeys) {
    ...
    const StatisticProbeRequest probeRequest(
        statisticKey.hash(), Milliseconds(windowSize),
    ...
    if (!probeResult.getProbeItems().empty()) {
      ...
      NES_ERROR("No probe result for statistic key: {}",
                statisticKey.toString());
      ASSERT_FALSE(true);
    }
  }

  // Checking if we have received for each statistic key, at least one probe
  // result
  EXPECT_EQ(statisticKeysWithProbeResult.size(), allStatisticKeys.size());
}

INSTANTIATE_TEST_CASE_P(
    testDefaultStatisticStore, StatisticsIntegrationTest,
    ::testing::Combine(::testing::Values(1, 2, 3),     // No. workers
                       ...
                       ::testing::Values(              // Different Metrics
                           Statistic::Cardinality::create(Statistic::Over(
                               StatisticsIntegrationTest::fieldNameToTrack)),
                           Statistic::Selectivity::create(Statistic::Over(
                               StatisticsIntegrationTest::fieldNameToTrack)))),
    [](const testing::TestParamInfo<StatisticsIntegrationTest::ParamType>
           &info) {
      return std::string(std::to_string(std::get<0>(info.param)) + "Workers_" +
                         std::to_string(std::get<1>(info.param)) +
                         "NumberOfBuffers_" +
                         std::to_string(std::get<2>(info.param)) +
                         "WindowSize_" + std::to_string(info.index) + "Metric");
    });
} // namespace NES
