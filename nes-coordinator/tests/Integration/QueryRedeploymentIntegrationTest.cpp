 ...
 */
TEST_P(QueryRedeploymentIntegrationTest, testCentralReconnects) {
    ...
    while (actualReconnects < numberOfReconnectsToPerform) {
        ...
        nlohmann::json event{};
        event["parentId"] = wrk3->getWorkerId();
        event["childId"] = wrk1->getWorkerId();
        event["action"] = "add";

        //create a json array and add the event
        nlohmann::json events = nlohmann::json::array();
        events.push_back(event);

        nlohmann::json eventRemove{};
        eventRemove["parentId"] = oldWorker->getWorkerId();
        eventRemove["childId"] = wrk1->getWorkerId();
        eventRemove["action"] = "remove";
        events.push_back(eventRemove);

        auto asyncResp = cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/nes/topology/update"},
