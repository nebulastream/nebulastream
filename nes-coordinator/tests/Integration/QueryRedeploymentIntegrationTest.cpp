...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("QueryRedeploymentIntegrationTest.log",
...
constexpr std::chrono::duration<int64_t, std::milli> defaultTimeoutInSec =
    std::chrono::seconds(TestUtils::defaultTimeout);
constexpr uint32_t DATA_CHANNEL_RETRY_TIMES = 10;
// todo #4441: increase number of event channel retries and add test for
...
/**
 * @brief This tests the asynchronous connection establishment, where the sink
 * buffers incoming tuples while waiting for the network channel to become
 * available
 */
TEST_P(QueryRedeploymentIntegrationTest, testAsyncConnectingSink) {
  ...
  auto lambdaSourceFunction = [&countReached,
                               &bufferCount](NES::Runtime::TupleBuffer &buffer,
    ...
    if (currentCount > numBuffersToProduceBeforeCount) {
      // after sending the specified amount of tuples, wait until some tuples
      // actually arrived
      while (!countReached)
        ;
    }
  ...
  auto lambdaSourceType = LambdaSourceType::create(
      "seq", "test_stream", std::move(lambdaSourceFunction),
      numBuffersToProduce, gatheringValue, GatheringMode::INTERVAL_MODE);
  ...
  ASSERT_TRUE(
      TestUtils::checkOutputOrTimeout(compareStringBeforeCount, testFile));
  countReached = true;
...
/**
 * @brief This tests multiple iterations of inserting VersionDrain events to
 * trigger the reconfiguration of a network sink to point to a new source.
 */
TEST_P(QueryRedeploymentIntegrationTest, testMultiplePlannedReconnects) {
  ...
  auto lambdaSourceFunction = [&bufferCount, &waitForReconfig,
                               &waitForReconnect, &waitForFinalCount,
                               &actualReconnects](
    ...
    if (currentCount >
      ...
      // after sending the specified amount of tuples, wait until the
      // reconfiguration has been triggered, subsequent tuples will be buffered
    ...
    if (currentCount >
        numBuffersToProduceBeforeReconnect + numBuffersToProduceWhileBuffering +
            (actualReconnects * buffersToProducePerReconnectCycle)) {
      // after writing some tuples into the buffer, give signal to start the new
      // operators to finish the reconnect, tuples will be unbuffered to new
      // destination
  ...
  auto lambdaSourceType = LambdaSourceType::create(
      "seq", "test_stream", std::move(lambdaSourceFunction),
      totalBuffersToProduce, gatheringValue, GatheringMode::INTERVAL_MODE);
  ...
  auto sinkLocationWrk1 = NES::Network::NodeLocation(
      wrk1->getWorkerId(), "localhost", *wrk1DataPort);
  auto networkSourceCrdLocation = NES::Network::NodeLocation(
      crd->getNesWorker()->getWorkerId(), "localhost", *crdWorkerDataPort);

  std::vector<NesWorkerPtr> reconnectParents;

  ASSERT_TRUE(TestUtils::waitForQueryToStart(queryId, crd->getQueryCatalog()));
  SharedQueryId sharedQueryId =
      crd->getGlobalQueryPlan()->getSharedQueryId(queryId);
  ASSERT_NE(sharedQueryId, INVALID_SHARED_QUERY_ID);
  // reconfiguration
  auto subPlanIdWrk3 = 30;
  ASSERT_EQ(wrk2->getNodeEngine()->getDecomposedQueryIds(sharedQueryId).size(),
            1);
  auto oldSubplanId =
      wrk2->getNodeEngine()->getDecomposedQueryIds(sharedQueryId).front();
  auto wrk2Source = wrk2->getNodeEngine()
                        ->getExecutableQueryPlan(oldSubplanId)
                        ->getSources()
                        .front();
  Network::NesPartition currentWrk1TargetPartition(
      sharedQueryId, wrk2Source->getOperatorId(), PartitionId(0),
      SubpartitionId(0));

  ASSERT_EQ(crd->getNesWorker()
                ->getNodeEngine()
                ->getDecomposedQueryIds(sharedQueryId)
                .size(),
            1);
  auto coordinatorSubplanId = crd->getNesWorker()
                                  ->getNodeEngine()
                                  ->getDecomposedQueryIds(sharedQueryId)
                                  .front();
  auto coordinatorSource = crd->getNesWorker()
                               ->getNodeEngine()
                               ->getExecutableQueryPlan(coordinatorSubplanId)
                               ->getSources()
                               .front();
  Network::NesPartition networkSourceCrdPartition(
      sharedQueryId, coordinatorSource->getOperatorId(), PartitionId(0),
      SubpartitionId(0));
  ...
  while (actualReconnects < numberOfReconnectsToPerform) {
    ...
    for (uint64_t i = 0;
         i < (numBuffersToProduceBeforeReconnect +
              (actualReconnects * (numBuffersToProduceBeforeReconnect +
    ...
    RequestProcessor::StorageDataStructures storageDataStructures(
        coordinatorConfig, topology, crd->getGlobalExecutionPlan(),
        crd->getGlobalQueryPlan(), crd->getQueryCatalog(),
        crd->getSourceCatalog(), crd->getUDFCatalog(),
        crd->getPlacementAmendmentQueue(), crd->getStatisticProbeHandler());
    auto storageHandler =
        RequestProcessor::SerialStorageHandler::create(storageDataStructures);
    std::vector<TopologyLinkInformation> removedLinks = {
        {wrk1->getWorkerId(), oldWorker->getWorkerId()}};
    std::vector<TopologyLinkInformation> addedLinks = {
        {wrk1->getWorkerId(), wrk3->getWorkerId()}};
    std::string coordinatorAddress =
        coordinatorConfig->coordinatorHost.getValue() + ":" +
        std::to_string(*rpcCoordinatorPort);
    auto coordinatorRPCClient =
        CoordinatorRPCClient::create(coordinatorAddress);
    coordinatorRPCClient->relocateTopologyNode(removedLinks, addedLinks);

    // notify lambda source that reconfig happened and make it release more
    // tuples into the buffer
    waitForFinalCount = false;
    ...
    while (wrk1->getNodeEngine()
               ->getPartitionManager()
               ->getProducerRegistrationStatus(currentWrk1TargetPartition) ==
           Network::PartitionRegistrationStatus::Registered) {
      std::this_thread::sleep_for(std::chrono::seconds(1));
      NES_DEBUG("Partition {} has not yet been unregistered",
                currentWrk1TargetPartition);
      if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec) {
        FAIL();
      }
    }
    ASSERT_NE(wrk1->getNodeEngine()
                  ->getPartitionManager()
                  ->getProducerRegistrationStatus(currentWrk1TargetPartition),
              Network::PartitionRegistrationStatus::Registered);
    auto lockedExecutionNode =
        crd->getGlobalExecutionPlan()->getLockedExecutionNode(
            wrk3->getWorkerId());
    auto networkSourceWrk3Partition =
        std::dynamic_pointer_cast<Network::NetworkSourceDescriptor>(
            lockedExecutionNode->operator*()
                ...
                ->getSourceDescriptor())
            ->getNesPartition();
    ASSERT_EQ(wrk1->getNodeEngine()
                  ->getPartitionManager()
                  ->getProducerRegistrationStatus(networkSourceWrk3Partition),
              Network::PartitionRegistrationStatus::Registered);
    EXPECT_NE(oldWorker->getNodeEngine()
                  ->getPartitionManager()
                  ->getConsumerRegistrationStatus(currentWrk1TargetPartition),
              Network::PartitionRegistrationStatus::Registered);
    ...
    while (oldWorker->getNodeEngine()->getQueryStatus(sharedQueryId) !=
      ...
      NES_DEBUG("Query has not yet stopped on worker {}",
                oldWorker->getWorkerId());
    ...
    oldWorker = wrk3;
    EXPECT_EQ(oldWorker->getNodeEngine()
                  ->getPartitionManager()
                  ->getConsumerRegistrationStatus(currentWrk1TargetPartition),
              Network::PartitionRegistrationStatus::Registered);
    oldSubplanId = oldWorker->getNodeEngine()
                       ->getDecomposedQueryIds(sharedQueryId)
                       .front();

    // check that query has left migrating state and is running again
    ASSERT_TRUE(
        TestUtils::waitForQueryToStart(queryId, crd->getQueryCatalog()));
    auto entries =
        crd->getQueryCatalog()->getQueryEntriesWithStatus("MIGRATING");
  ...
  ASSERT_TRUE(TestUtils::checkStoppedOrTimeoutAtWorker(sharedQueryId,
                                                       lastReconnectParent));
  ...
  for (auto parent : reconnectParents) {
    ...
    ASSERT_TRUE(
        TestUtils::checkStoppedOrTimeoutAtWorker(sharedQueryId, parent));
    cout << "stopping worker" << endl;
...
INSTANTIATE_TEST_CASE_P(QueryRedeploymentIntegrationTestParam,
                        QueryRedeploymentIntegrationTest,
                        ::testing::Values(1, 4));
} // namespace NES
