...
class DeepHierarchyTopologyTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("DeepTopologyHierarchyTest.log",
...
TEST_F(DeepHierarchyTopologyTest, testOutputAndAllSensors) {
  ...
  auto testHarness =
      TestHarness(query, *restPort, *rpcCoordinatorPort,
          ...
          .attachWorkerWithMemorySourceToWorkerWithId("test",
                                                      WorkerId(2))  // idx=3
          .attachWorkerWithMemorySourceToWorkerWithId("test",
                                                      WorkerId(2))  // idx=4
          .attachWorkerWithMemorySourceToWorkerWithId("test",
...
/**
 * @brief This tests just outputs the default source for a hierarchy of two
 levels where each node produces data
 * Topology:
    PhysicalNode[id=1, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |--PhysicalNode[id=5, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    ...
    |  |--PhysicalNode[id=3, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
 */
TEST_F(DeepHierarchyTopologyTest,
...
/**
 * @brief This tests just outputs the default source for a hierarchy with one
 relay which does not produce data
 * Topology:
    PhysicalNode[id=1, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |--PhysicalNode[id=2, ip=127.0.0.1, resourceCapacity=1, usedResource=0]
    |  |--PhysicalNode[id=6, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
...
/**
 * @brief This tests just outputs the default source for a hierarchy of two
 levels where only leaves produce data
 * Topology:
    PhysicalNode[id=1, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |--PhysicalNode[id=5, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |  |--PhysicalNode[id=7, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
...
/**
 * @brief This tests just outputs the default source for a hierarchy of three
 levels where only leaves produce data
 * Topology:
    PhysicalNode[id=1, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |--PhysicalNode[id=5, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |  |--PhysicalNode[id=7, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |  |  |--PhysicalNode[id=9, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0] |  |--PhysicalNode[id=6, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0] |  |  |--PhysicalNode[id=8, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0]
    |--PhysicalNode[id=2, ip=127.0.0.1, resourceCapacity=1, usedResource=0]
    |  |--PhysicalNode[id=4, ip=127.0.0.1, resourceCapacity=12, usedResource=0]
    |  |  |--PhysicalNode[id=11, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0] |  |--PhysicalNode[id=3, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0] |  |  |--PhysicalNode[id=10, ip=127.0.0.1, resourceCapacity=12,
 usedResource=0]
 */
TEST_F(DeepHierarchyTopologyTest,
...
TEST_F(DeepHierarchyTopologyTest, testSelectProjectThreeLevel) {
  ...
  for (uint64_t i = 0; i < 4; i++) {
    CSVSourceTypePtr csvSourceType =
        CSVSourceType::create("testStream", "testStream1");
  ...
  auto query = Query::from("testStream")
                   .filter(Attribute("val1") < 3)
                   .project(Attribute("val3"));
  ...
  TestHarness testHarness =
      TestHarness(query, *restPort, *rpcCoordinatorPort,
          ...
          // Sensors
          .attachWorkerWithCSVSourceToWorkerWithId(csvSourceTypes.at(0),
                                                   WorkerId(3))  // id=8
          .attachWorkerWithCSVSourceToWorkerWithId(csvSourceTypes.at(1),
                                                   WorkerId(4))  // id=9
          .attachWorkerWithCSVSourceToWorkerWithId(csvSourceTypes.at(2),
                                                   WorkerId(6))  // id=10
          .attachWorkerWithCSVSourceToWorkerWithId(csvSourceTypes.at(3),
                                                   WorkerId(7))  // id=11
          .validate()
          .setupTopology();
...
TEST_F(DeepHierarchyTopologyTest, testUnionThreeLevel) {
  ...
  TestHarness testHarness =
      TestHarness(query, *restPort, *rpcCoordinatorPort,
          ...
          // Sensors
          .attachWorkerWithMemorySourceToWorkerWithId("truck",
                                                      WorkerId(3))  // idx=8
          .attachWorkerWithMemorySourceToWorkerWithId("car",
                                                      WorkerId(4))  // idx=9
          .attachWorkerWithMemorySourceToWorkerWithId("truck",
                                                      WorkerId(6))  // idx=10
          .attachWorkerWithMemorySourceToWorkerWithId("car",
...
TEST_F(DeepHierarchyTopologyTest,
  ...
  for (uint64_t i = 0; i < 4; i++) {
    auto csvSourceType =
        CSVSourceType::create("window", "window" + std::to_string(i));
    csvSourceType->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                               "window.csv");
  ...
  auto testHarness = TestHarness(query, *restPort, *rpcCoordinatorPort,
                         ...
                         .attachWorkerWithCSVSourceToWorkerWithId(
                             csvSourceTypes.at(0), WorkerId(2))  // 3
                         .attachWorkerWithCSVSourceToWorkerWithId(
                             csvSourceTypes.at(1), WorkerId(2))  // 4
                         .attachWorkerToCoordinator()            // 5
                         .attachWorkerWithCSVSourceToWorkerWithId(
                             csvSourceTypes.at(2), WorkerId(5))  // 6
                         .attachWorkerWithCSVSourceToWorkerWithId(
                             csvSourceTypes.at(3), WorkerId(5))  // 7
                         .validate()
                         .setupTopology();
...
TEST_F(DeepHierarchyTopologyTest, testMapAndAggregationQuery) {
  ...
  auto testHarness = TestHarness(query, *restPort, *rpcCoordinatorPort,
                         ...
                         .attachWorkerWithMemorySourceToWorkerWithId(
                             "window", WorkerId(2), workerConfigEdgeNode);  // 3

  const auto bufferCapacity = workerConfigEdgeNode->bufferSizeInBytes /
                              testSchema->getSchemaSizeInBytes();
  NES_INFO(
      "testHarness.getBufferManager()->getBufferSize = {} bufferCapacity = {}",
      workerConfigEdgeNode->bufferSizeInBytes.getValue(), bufferCapacity);
  ...
  for (auto windowStart = 0_u64; windowStart < lastWindowStart;
  ...
  // Run the query and get the actual dynamic buffers
  auto actualBuffers =
      testHarness.runQuery(Util::countLines(oss.str())).getOutput();

  // Comparing equality
  const auto outputSchema = testHarness.getOutputSchema();
  auto tmpBuffers = TestUtils::createExpectedBufferFromCSVString(
      oss.str(), outputSchema, testHarness.getBufferManager());
  auto expectedBuffers =
      TestUtils::createTestTupleBuffers(tmpBuffers, outputSchema);
  EXPECT_TRUE(
      TestUtils::buffersContainSameTuples(expectedBuffers, actualBuffers));
