...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("UpdateTopologyRemoteTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup UpdateTopologyRemoteTest test class.");
  }

  static void TearDownTestCase() {
    NES_DEBUG("Tear down UpdateTopologyRemoteTest test class.");
  }
};
...
TEST_F(UpdateTopologyRemoteTest, addAndRemovePathWithOwnId) {
  ...
  EXPECT_TRUE(topology->nodeWithWorkerIdExists(
      WorkerId(1))); // worker inside the coordinator
  EXPECT_TRUE(topology->nodeWithWorkerIdExists(
      WorkerId(2))); // first worker should get id 2
  EXPECT_TRUE(topology->nodeWithWorkerIdExists(
      WorkerId(3))); // second worker should get id 3

  auto rootWorkerNodeIds = topology->getRootWorkerNodeIds();
  EXPECT_EQ(rootWorkerNodeIds.size(), 1);
  const TopologyNodeWLock &lockTopologyNode =
      topology->lockTopologyNode(rootWorkerNodeIds[0]);
  ...
  EXPECT_TRUE(node2->getChildren()[0]->as<TopologyNode>()->getId() ==
              node1->getId());
