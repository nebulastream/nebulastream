...
class MultiWorkerTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down MultiWorkerTest class.");
  }
};
...
TEST_F(MultiWorkerTest, testMultipleWorker) {
  ...
  for (uint64_t i = 0; i < numWorkers; i++) {
    ...
    bool retStart =
        wPtrs[i]->start(/**blocking**/ false, /**withConnect**/ false);
...
TEST_F(MultiWorkerTest, startWorkersWithoutWorkerId) {
  ...
  NES_DEBUG("Worker 1 started successfully with workerId {}",
            wrk1->getWorkerId());
  ...
  NES_DEBUG("Worker 2 started successfully with workerId {}",
            wrk2->getWorkerId());
...
TEST_F(MultiWorkerTest, startWorkerWithWorkerIdBelongingToActiveWorker) {
  ...
  bool retStart2copy =
      wrk2copy->start(/**blocking**/ false, /**withConnect**/ true);
  EXPECT_TRUE(retStart2copy);
  // expected behavior: this workerId belongs to an active worker, therefore the
  // next available workerId will be assigned
  EXPECT_EQ(wrk2copy->getWorkerId(), WorkerId(3u));
  NES_DEBUG("Worker 2 started successfully with workerId {}",
            wrk2copy->getWorkerId());
...
TEST_F(MultiWorkerTest, startWorkerWithCorrectNextWorkerId) {
  // make sure that even if the given workerId is the next available one, the
  // counter is still increased start the coordinator
  CoordinatorConfigurationPtr coordinatorConfig =
      CoordinatorConfiguration::createDefault();
  coordinatorConfig->rpcPort = *rpcCoordinatorPort;
  coordinatorConfig->restPort = *restPort;
...
TEST_F(MultiWorkerTest, checkPersistenceOfWorkerIdWithNonExistingConfigFile) {
  ...
  WorkerConfigurationPtr wrkConf = WorkerConfiguration::create();
  wrkConf->coordinatorPort = port;
  std::string configPath = std::filesystem::path(TEST_DATA_DIRECTORY) /
                           "nonExistingWorkerConfig.yaml";
  wrkConf->configPath = configPath;
  NesWorkerPtr wrk1 = std::make_shared<NesWorker>(std::move(wrkConf));
  bool retStart1 = wrk1->start(/**blocking**/ false, /**withConnect**/ true);
  EXPECT_TRUE(retStart1);
