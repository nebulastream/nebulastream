...
class DistributedNemoJoinIntegrationTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("DistributedNemoJoinIntegrationTest.log",
  ...
  static CSVSourceTypePtr createCSVSourceType(
      const std::string& logicalSourceNAme,
      const std::string& physicalSourceName, const std::string& inputPath) {
    CSVSourceTypePtr csvSourceType =
        CSVSourceType::create(logicalSourceNAme, physicalSourceName);
  ...
  /**
   * Creates a TestHarness with a given query and topology parameters. The leafs
   * of the topology are sources.
  ...
  TestHarness createTestHarness(
      ...
      uint64_t layers, uint64_t nodesPerNode, uint64_t leafNodesPerNode) {
    auto inputSchema =
        Schema::create()
    ...
    for (uint64_t i = 2; i <= layers; i++) {
      ...
      for (auto parent : parents) {
        ...
        for (uint64_t j = 0; j < nodeCnt; j++) {
          nodeId++;
          // if children are leaf nodes create the sources, else just create
          ...
          if (i == layers) {
            ...
            testHarness.attachWorkerWithCSVSourceToWorkerWithId(
                csvSource, WorkerId(parent));
            ...
            NES_DEBUG(
                "DistributedNemoJoinIntegrationTest: Adding statistics for {}",
                request.dump());
            NES_DEBUG(
                "DistributedNemoJoinIntegrationTest: Adding CSV source:{} to "
                "{} for node:{}",
                logSource, parent, nodeId);
          } else {
            testHarness.attachWorkerToWorkerWithId(WorkerId(parent));
            NES_DEBUG(
                "DistributedNemoJoinIntegrationTest: Adding worker {} to "
                "worker with ID:{}",
                nodeId, parent);
          }
...
TEST_F(DistributedNemoJoinIntegrationTest, testThreeLevelsTopologyTopDown) {
  ...
  std::function<void(CoordinatorConfigurationPtr)> crdFunctor =
      [](const CoordinatorConfigurationPtr& config) {
        config->optimizer.joinOptimizationMode.setValue(
            Optimizer::DistributedJoinOptimizationMode::NEMO);
      };
  ...
  auto testHarness = createTestHarness(query, crdFunctor, layers, nodesPerNode,
                                       leafNodesPerNode);
  // 26 join keys
  ...
  auto outputString =
      actualOutput[0].toString(testHarness.getOutputSchema(), true);
