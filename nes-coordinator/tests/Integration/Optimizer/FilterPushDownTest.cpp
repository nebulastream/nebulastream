...
public:
  ...
  struct Output {
    ...
    bool operator==(Output const &rhs) const {
      return (QnV1$timestamp == rhs.QnV1$timestamp &&
              QnV1$velocity == rhs.QnV1$velocity &&
...
/* 1.Test
 * This test checks if the filter push down below map keeps the correct order of
 * operations when we apply a map with a substractions followed by a map with a
 * multiplication
 */
TEST_F(FilterPushDownTest, testCorrectResultsForFilterPushDownBelowTwoMaps) {

  NES_INFO("FilterPushDownTest: Start "
           "testCorrectResultsForFilterPushDownBelowTwoMaps");

  auto srcConf1 = CSVSourceType::create("QnV1", "PQnV1");
  srcConf1->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                        "QnV_short_R2000070.csv");
  srcConf1->setNumberOfTuplesToProducePerBuffer(5);
  srcConf1->setNumberOfBuffersToProduce(20);

  std::string outputFilePath =
      getTestResourceFolder() / "filterPushDownTest.out";
  ...
  auto query = Query::from("QnV1")
                   ...
                   .project(Attribute("timestamp"), Attribute("velocity"),
                            Attribute("quantity"));
...
/* 2.Test
 * This test checks if the filter push down below map keeps the correct order of
 * operations when we apply a map with a subtraction in parentheses and a
 * multiplication
 */
...
/* 2.Test
 * This test checks if the filter push down below a map with a new field name
 * works correctly
 */
TEST_F(FilterPushDownTest, testSameResultsForPushDownBelowMapWithNewField) {
  NES_INFO("FilterPushDownTest: Start "
           "testSameResultsForPushDownBelowMapWithNewField");
  ...
  auto query =
      Query::from("QnV1")
          ...
          .project(Attribute("timestamp"), Attribute("NewVelocity"),
                   Attribute("quantity"));
