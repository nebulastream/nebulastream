...
TEST_F(LocationProviderCSVTest, testInvalidCsv) {
  ...
  auto csvPath =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "non_existent_file.csv";
  remove(csvPath.c_str());
  auto locationProvider = std::make_shared<
      NES::Spatial::Mobility::Experimental::LocationProviderCSV>(csvPath);
  ASSERT_THROW(auto waypoint = locationProvider->getCurrentWaypoint(),
               NES::Spatial::Exception::LocationProviderException);
  ...
  csvPath =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "invalidLatitudeFormat.csv";
  remove(csvPath.c_str());
  ...
  csvPath =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "invalidLongitudeFormat.csv";
  remove(csvPath.c_str());
  ...
  csvPath =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "invalidOffsetFormat.csv";
  remove(csvPath.c_str());
  ...
  csvPath = std::filesystem::path(TEST_DATA_DIRECTORY) /
            "testLocationsNotEnoughColumns.csv";
  remove(csvPath.c_str());
...
TEST_F(LocationProviderCSVTest, testCsvMovement) {
  ...
  auto expectedWayPoints =
      getWaypointsFromCsv(csvPath, locationProvider->getStartTime());
  ...
  while (currentTimeStamp <= endTime) {
    ...
    if (actualWayPoints.empty() ||
        currentWayPoint.getLocation() != actualWayPoints.back().getLocation()) {
  ...
  for (auto actualIt = actualWayPoints.cbegin();
    ...
    while (expectedItr != expectedWayPoints.cend() &&
           expectedItr->getTimestamp().value() !=
               actualIt->getTimestamp().value()) {
      expectedItr++;
    }
...
TEST_F(LocationProviderCSVTest, testCsvMovementWithSimulatedLocationInFuture) {
  ...
  while (currentTimeStamp <= endTime) {
    ...
    if (currentWayPoint.getTimestamp().value() <=
        initialExpectedWaypoint.getTimestamp().value()) {
      NES_DEBUG("comparing initial location");
      EXPECT_EQ(initialExpectedWaypoint.getLocation(),
