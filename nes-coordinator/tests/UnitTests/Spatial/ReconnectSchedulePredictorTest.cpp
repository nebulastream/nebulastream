...
class ReconnectSchedulePredictorTest : public Testing::BaseIntegrationTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ReconnectSchedulePredictor.log",
...
TEST_F(ReconnectSchedulePredictorTest, testFindPathCoverage) {
  ...
  S2Point lineEnd(
      S2LatLng::FromDegrees(52.436244793720014, 13.708629778786937));
  pointvec.push_back(lineStart);
  pointvec.push_back(lineEnd);
  S2Polyline path = S2Polyline(pointvec);

  // we chose a point on the path as a covering point. so its coverage should be
  // equal to the supplied coverage
  ...
  std::pair<S2Point, S1Angle> resultOnLinePoint =
      NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::
          findPathCoverage(path, coveringPointOnLine, coverage);
  NES_DEBUG("point on line coverage in meters: {}",
            S2Earth::ToMeters(resultOnLinePoint.second));
  EXPECT_TRUE(S2::ApproxEquals(
      resultOnLinePoint.first,
      S2::GetPointOnLine(coveringPointOnLine, lineEnd, coverage),
      allowedError));
  ...
  NES_DEBUG("coverage end for point on line: {}",
            resultOnLinePointString.str());

  // create a point whose unit vector is orthogonal to start and end of the
  // polyline
  ...
  // we can use the orthogonal vector as a target to move away from the line in
  // an angle of 90 degrees we create a point which is exactly at the boundary
  // of the coverage area and should therefore not cover anything of the line
  auto coveringPointCovAwayFromLine =
      S2::GetPointOnLine(coveringPointOnLine, ortoVec, coverage);
  auto resultCovaway =
      NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::
          findPathCoverage(path, coveringPointCovAwayFromLine, coverage);
  ASSERT_TRUE(resultCovaway.second.degrees() == 0);

  // test different distances from the line which are greater than zero but
  // smaller than coverage
  for (int i = 1; i < 100; ++i) {
    double coverageFactor = 0.01 * i;
    NES_DEBUG("testing coverage of point which is coverage * {} away from path",
              coverageFactor);
    auto coveringPointAwayFromPath = S2::GetPointOnLine(
        coveringPointOnLine, ortoVec, coverage * coverageFactor);
    std::stringstream coveringPointAsString;
    coveringPointAsString << S2LatLng(coveringPointAwayFromPath);
    NES_DEBUG("covering point coordinates are: {}",
              coveringPointAsString.str());
    auto result =
        NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::
            findPathCoverage(path, coveringPointAwayFromPath, coverage);
    NES_DEBUG("coverage on line in meters is: {}",
              S2Earth::ToMeters(result.second));
    ...
    auto comparePoint =
        S2::GetPointOnLine(coveringPointOnLine, lineEnd, result.second);
    // check that the returned coverage end point is on the path and that the
    // distance covered on line matches
    ...
    ASSERT_TRUE(
        abs(coverage - S1Angle(result.first, coveringPointAwayFromPath)) <
        allowedError);
  }
}
#endif
