...
TEST_F(ExpressionNodeTest, predicateConstruction) {
  auto left = ConstantValueExpressionNode::create(
      DataTypeFactory::createBasicValue(BasicType::INT64, "10"));
  ASSERT_FALSE(left->isPredicate());
  auto right = ConstantValueExpressionNode::create(
      DataTypeFactory::createBasicValue(BasicType::INT64, "11"));
  ...
  auto fieldRead = FieldAccessExpressionNode::create(
      DataTypeFactory::createInt64(), "field_1");
  auto constant = ConstantValueExpressionNode::create(
      DataTypeFactory::createBasicValue(BasicType::INT64, "10"));
...
TEST_F(ExpressionNodeTest, inferenceExpressionTest) {
  auto schema =
      Schema::create()
          ...
          ->addField("test$f4", DataTypeFactory::createArray(
                                    10, DataTypeFactory::createBoolean()));

  auto addExpression = Attribute("f1") + 10;
  EXPECT_TRUE(addExpression->getStamp()->isUndefined());
  addExpression->inferStamp(schema);
  EXPECT_TRUE(addExpression->getStamp()->equals(
      DataTypeFactory::createType(BasicType::INT32)));

  auto mulExpression = Attribute("f2") * 10;
  EXPECT_TRUE(mulExpression->getStamp()->isUndefined());
  mulExpression->inferStamp(schema);
  EXPECT_TRUE(mulExpression->getStamp()->equals(
      DataTypeFactory::createType(BasicType::INT64)));

  auto increment = Attribute("f3")++;
  EXPECT_TRUE(increment->getStamp()->isUndefined());
  increment->inferStamp(schema);
  EXPECT_TRUE(increment->getStamp()->equals(
      DataTypeFactory::createType(BasicType::FLOAT64)));
...
TEST_F(ExpressionNodeTest, inferAssertionTest) {
  ...
  EXPECT_TRUE(assertion->getField()->getStamp()->equals(
      DataTypeFactory::createType(BasicType::INT8)));
}

TEST_F(ExpressionNodeTest, multiplicationInferStampTest) {
  auto schema = Schema::create()
                    ->addField("test$left", BasicType::UINT32)
                    ->addField("test$right", BasicType::INT16);
  ...
  // TODO: question about the bits in stamps: Does this result of our stamp
  // inference make sense? A 32 bit integer can not store all values between
  // [INT16_MIN,UINT32_MAX]
}

/**
 * @brief Test behaviour of special ModExpressionNode::inferStamp function.
 * (integers)
 */
TEST_F(ExpressionNodeTest, moduloIntegerInferStampTest) {
  ...
  EXPECT_EQ(upperBound,
            -INT16_MIN -
                1); // e.g. when calculating MOD(..., -128) the result will
                    // always be in range [-127, 127]. And no other INT8 divisor
                    // will yield a wider range than -128 (=INT8_MIN).
...
TEST_F(ExpressionNodeTest, moduloFloatInferStampTest) {
  auto schema = Schema::create()
                    ->addField("test$left", BasicType::UINT32)
                    ->addField("test$right", BasicType::FLOAT32);
  ...
  NES_INFO("{}", upperBound);
  EXPECT_EQ(bits, 32);
  EXPECT_EQ(lowerBound, 0); // == lower bound of UINT32, as it is is higher than
                            // range spanned by Float divisor
  EXPECT_EQ(upperBound,
            UINT32_MAX); // == upper bound of UINT32, as it  is lower than range
                         // spanned by Float divisor
}

/**
 * @brief Test behaviour of special WhenExpressionNode::inferStamp function.
 * (float)
 */
TEST_F(ExpressionNodeTest, whenInferStampTest) {
  auto schema = Schema::create()
                    ->addField("test$bool", BasicType::BOOLEAN)
                    ->addField("test$float", BasicType::FLOAT32);
