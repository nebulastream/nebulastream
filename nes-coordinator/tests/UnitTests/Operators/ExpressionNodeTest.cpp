  ...
  EXPECT_EQ(upperBound,
            -INT16_MIN -
                1);  // e.g. when calculating MOD(..., -128) the result will
                     // always be in range [-127, 127]. And no other INT8
                     // divisor will yield a wider range than -128 (=INT8_MIN).
...
TEST_F(ExpressionNodeTest, moduloFloatInferStampTest) {
  ...
  NES_INFO("{}", upperBound);
  EXPECT_EQ(bits, 32);
  EXPECT_EQ(lowerBound, 0);  // == lower bound of UINT32, as it is is higher
                             // than range spanned by Float divisor
  EXPECT_EQ(upperBound,
            UINT32_MAX);  // == upper bound of UINT32, as it  is lower than
