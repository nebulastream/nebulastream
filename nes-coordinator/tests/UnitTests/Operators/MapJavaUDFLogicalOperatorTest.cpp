...
class MapJavaUDFLogicalOperatorTest : public Testing::BaseUnitTest {
 protected:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MapJavaUDFLogicalOperatorTest",
...
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchema) {
  ...
  auto outputSchema = std::make_shared<Schema>()->addField(
  ...
  auto mapUdfLogicalOperator =
      std::make_shared<MapUDFLogicalOperator>(javaUdfDescriptor, OperatorId(1));

  // Create a SourceLogicalOperator with a source schema
  // and add it as a child to the MapUdfLogicalOperator to infer the input
  // schema.
  const std::string sourceName = "sourceName";
  auto inputSchema = std::make_shared<Schema>()->addField(
      sourceName + "$inputAttribute", DataTypeFactory::createUInt64());
  auto sourceDescriptor = std::make_shared<SchemaSourceDescriptor>(inputSchema);
  mapUdfLogicalOperator->addChild(
      std::make_shared<SourceLogicalOperator>(sourceDescriptor, OperatorId(2)));

  // After calling inferSchema on the MapUdfLogicalOperator, the input schema
  // should be the schema of the source.
  ...
  auto expectedOutputSchema = std::make_shared<Schema>()->addField(
      sourceName + "$outputAttribute", DataTypeFactory::createBoolean());
  ASSERT_TRUE(
      mapUdfLogicalOperator->getOutputSchema()->equals(expectedOutputSchema));
}

TEST_F(MapJavaUDFLogicalOperatorTest, InferStringSignature) {
  // Create a MapUdfLogicalOperator with a JavaUDFDescriptor and a source as a
  // child.
  ...
  auto child = std::make_shared<SourceLogicalOperator>(
      std::make_shared<SchemaSourceDescriptor>(
          std::make_shared<Schema>()->addField(
              "inputAttribute", DataTypeFactory::createUInt64())),
      OperatorId(2));
  mapUdfLogicalOperator->addChild(child);

  // After calling inferStringSignature, the map returned by
  // `getHashBasesStringSignature` contains an entry.
...
// The bug: When we try to add p2 as a parent to n2, the parent is not added
// because there already exists a parent with the same operator ID. Cause: n2 is
// a shallow copy of n1, so it retains the list of parents of n1 with the same
// IDs.
TEST_F(MapJavaUDFLogicalOperatorTest, AddParentToCopy) {
  ...
  auto p1 = LogicalOperatorFactory::createSinkOperator(
      NullOutputSinkDescriptor::create());
  n1->addParent(p1);

  // when: Create copies of n1 and p1 and add p2 as parent of n2. They should
  // not be in a parent-child relationship.
  auto n2 = n1->copy();
  auto p2 = p1->copy();
...
// The following tests verify that the input schema of the UDF is compatible
// with the output schema of the parent operator. This test is the happy case,
// in which both schemas are compatible.
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaCompatibleTypes) {
  ...
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  // No exception is thrown here
  op->inferSchema();
}

// The schemas are not compatible because they contain the same field names but
...
// The schemas are not compatible because they contain a different number of
// fields.
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaDifferentSize) {
  ...
  auto childOutputSchema = Schema::create()
                               ->addField("source$field1", BasicType::INT8)
                               ->addField("source$field2", BasicType::INT8);
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  EXPECT_THROW(op->inferSchema(), NES::TypeInferenceException);
...
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaMissingField) {
  auto udfInputSchema = Schema::create()
                            ->addField("field1", BasicType::INT8)
                            ->addField("missingField", BasicType::INT8);
  auto op = LogicalOperatorFactory::createMapUDFLogicalOperator(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          .setInputSchema(udfInputSchema)
          .build());
  ...
  auto childOutputSchema =
      Schema::create()->addField("source$field1", BasicType::INT8);
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  EXPECT_THROW(op->inferSchema(), NES::TypeInferenceException);
...
// The test does not verify this automatically but the debug log output can be
// manually checked.
...
// The schemas are compatible because UINT64 in the child output schema are
// mapped to Java long in the UDF. The log should contain a WARN message
// indicating the possible integer overflow. The test does not verify this
// automatically but the log output can be manually checked.
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaUnsignedLong) {
  auto udfInputSchema =
      Schema::create()->addField("unsignedLongField", BasicType::INT64);
  auto op = LogicalOperatorFactory::createMapUDFLogicalOperator(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          .setInputSchema(udfInputSchema)
          .build());
  auto childOutputSchema =
      Schema::create()->addField("source$unsignedLongField", BasicType::UINT64);
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  // No exception is thrown here
...
// The schemas are compatible if the Java UDF input type is not a complex
// object, the name of the field in the child operator output schema does not
// matter. Regression test for
// https://github.com/nebulastream/nebulastream/issues/4334
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaSingleField) {
  ...
  auto op = LogicalOperatorFactory::createMapUDFLogicalOperator(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .build());
  auto childOutputSchema =
      Schema::create()->addField("source$randomName", BasicType::INT64);
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  // No exception is thrown here
...
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaSingleFieldIncompatibleTypes) {
  ...
  auto op = LogicalOperatorFactory::createMapUDFLogicalOperator(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .build());
  auto childOutputSchema =
      Schema::create()->addField("source$randomName", BasicType::INT32);
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  EXPECT_THROW(op->inferSchema(), NES::TypeInferenceException);
...
TEST_F(MapJavaUDFLogicalOperatorTest, InferSchemaSingleFieldUnsignedLong) {
  ...
  auto op = LogicalOperatorFactory::createMapUDFLogicalOperator(
      Catalogs::UDF::JavaUDFDescriptorBuilder()
          ...
          .build());
  auto childOutputSchema =
      Schema::create()->addField("source$randomName", BasicType::UINT64);
  auto source = LogicalOperatorFactory::createSourceOperator(
      SchemaSourceDescriptor::create(std::move(childOutputSchema)));
  op->addChild(source);
  // No exception is thrown here
