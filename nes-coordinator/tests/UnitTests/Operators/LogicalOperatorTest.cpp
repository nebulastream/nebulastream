...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LogicalOperatorTest.log",
  ...
  void SetUp() override {
    ...
    logicalSource =
        sourceCatalog->getLogicalSourceOrThrowException("default_logical");
    SchemaPtr schema = logicalSource->getSchema();
    auto sourceDescriptor = DefaultSourceDescriptor::create(
        schema, /*number of buffers*/ 0, /*frequency*/ 0);
...
protected:
  ...
  LogicalOperatorPtr filterOp1, filterOp2, filterOp3, filterOp4, filterOp5,
      filterOp6, filterOp7;
  LogicalOperatorPtr filterOp1Copy, filterOp2Copy, filterOp3Copy, filterOp4Copy,
      filterOp5Copy, filterOp6Copy;
...
TEST_F(LogicalOperatorTest, getOperatorByType) {
  ...
  expected.push_back(filterOp3);
  expected.push_back(filterOp4);
  const vector<LogicalFilterOperatorPtr> children =
      filterOp1->getNodesByType<LogicalFilterOperator>();
