...
class OperatorPropertiesTest : public Testing::BaseUnitTest {
 ...
 protected:
  static void setupLogging() {
    NES::Logger::setupLogging("OperatorPropertiesTest.log",
...
TEST_F(OperatorPropertiesTest, testAssignProperties) {
  ...
  bool res =
      Util::assignPropertiesToQueryOperators(query.getQueryPlan(), properties);
  ...
  ASSERT_EQ(
      std::any_cast<int>(
          (*queryPlanIterator)->as<LogicalOperator>()->getProperty("load")),
      1);
  ASSERT_EQ(
      std::any_cast<int>(
          (*queryPlanIterator)->as<LogicalOperator>()->getProperty("dmf")),
      1);
  ++queryPlanIterator;
...
TEST_F(OperatorPropertiesTest, testAssignDifferentPropertyTypes) {
  ...
  ASSERT_EQ(
      std::any_cast<int>(
          (*queryPlanIterator)->as<LogicalOperator>()->getProperty("load_int")),
      1);
  ASSERT_EQ(std::any_cast<double>((*queryPlanIterator)
                                      ->as<LogicalOperator>()
                                      ->getProperty("dmf_double")),
            0.5);
  ASSERT_EQ(
      std::any_cast<std::string>(
          (*queryPlanIterator)->as<LogicalOperator>()->getProperty("misc_str")),
      "xyz");
  ++queryPlanIterator;
...
TEST_F(OperatorPropertiesTest, testAssignWithMorePropertiesThanOperators) {
  ...
  // this should return false as properties of all operators has to be supplied
...
TEST_F(OperatorPropertiesTest, testAssignWithBinaryOperator) {
  auto subQuery =
      Query::from("default_logical").filter(Attribute("field_1") <= 10);
  ...
  // Adding properties of each operator. The order should be the same as used in
  // the QueryPlanIterator adding property of the source
  std::map<std::string, std::any> srcProp;
  srcProp.insert(std::make_pair("load", 1));
  properties.push_back(srcProp);

  // adding property of the source watermark (watermarks are added
  // automatically)
  ...
  // adding property of the watermark in the sub query (watermarks are added
  // automatically)
