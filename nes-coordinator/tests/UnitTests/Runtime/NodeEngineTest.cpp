...
std::shared_ptr<MockedNodeEngine> createMockedEngine(
    const std::string& hostname, uint16_t port, uint64_t bufferSize = 8192,
  ...
  try {
    ...
    DefaultSourceTypePtr defaultSourceType =
        DefaultSourceType::create("test", "test1");
    ...
    auto queryManager = std::make_shared<Runtime::DynamicQueryManager>(
        std::make_shared<DummyQueryListener>(), bufferManager,
        INVALID_WORKER_NODE_ID, 1, nullptr, 100);
    ...
    auto jitCompiler = Compiler::JITCompilerBuilder()
                           ...
                           .build();
    auto queryCompiler = QueryCompilation::NautilusQueryCompiler::create(
        compilerOptions, phaseFactory);

    auto mockEngine = std::make_shared<MockedNodeEngine>(
        std::move(physicalSources), std::make_shared<HardwareManager>(),
        std::move(bufferManager), std::move(queryManager),
        std::move(networkManagerCreator), std::move(partitionManager),
        std::move(queryCompiler), INVALID_WORKER_NODE_ID, 1024, 12, 12);
...
class TextExecutablePipeline : public ExecutablePipelineStage {
 public:
  ...
  ExecutionResult execute(TupleBuffer& inputTupleBuffer,
    ...
    NES_INFO(
        "Test: query result = Processed Block:{} count: {} psum: {} sum: {}",
        inputTupleBuffer.getNumberOfTuples(), count, psum, sum);

    if (sum == 10) {
      NES_DEBUG("TEST: result correct");

      // TupleBuffer outputBuffer =
      // pipelineExecutionContext.allocateTupleBuffer(); WAS THIS CODE
...
class NodeEngineTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void TearDownTestCase() {
    NES_DEBUG("Tear down OperatorOperatorCodeGenerationTest test class.");
  }
...
auto setupQEP(const NodeEnginePtr& engine, SharedQueryId sharedQueryId,
              const std::string& outPath) {
  SchemaPtr sch = Schema::create()->addField("sum", BasicType::UINT32);

  DataSinkPtr sink = createCSVFileSink(
      sch, sharedQueryId, DecomposedQueryPlanId(sharedQueryId.getRawValue()),
      engine, 1, outPath, false);
  auto context = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sink);
  auto executable = std::make_shared<TextExecutablePipeline>();
  auto pipeline = ExecutablePipeline::create(
      INVALID_PIPELINE_ID, INVALID_SHARED_QUERY_ID,
      DecomposedQueryPlanId(sharedQueryId.getRawValue()),
      engine->getQueryManager(), context, executable, 1, {sink});
  auto source = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(1),
      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline});
  auto executionPlan = ExecutableQueryPlan::create(
      SharedQueryId(sharedQueryId.getRawValue()),
      DecomposedQueryPlanId(sharedQueryId.getRawValue()), {source}, {sink},
      {pipeline}, engine->getQueryManager(), engine->getBufferManager());
...
TEST_F(NodeEngineTest, teststartDeployStop) {
  ...
  auto [qep, pipeline] =
      setupQEP(engine, testQueryId, getTestResourceFolder() / "test.out");
  ASSERT_TRUE(engine->deployQueryInNodeEngine(qep));
  ASSERT_TRUE(engine->getQueryStatus(testQueryId) ==
...
TEST_F(NodeEngineTest, testStartDeployUndeployStop) {
  ...
  ASSERT_TRUE(engine->getQueryStatus(testQueryId) ==
              ExecutableQueryPlanStatus::Running);
  pipeline->completedPromise.get_future().get();
  while (engine->getQueryStatus(testQueryId) ==
         ExecutableQueryPlanStatus::Finished) {
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
  }
  ASSERT_TRUE(
      engine->undeployQuery(testQueryId, qep->getDecomposedQueryPlanId()));
  ASSERT_TRUE(engine->stop());
...
TEST_F(NodeEngineTest, testStartRegisterStartStopDeregisterStop) {
  ...
  ASSERT_TRUE(engine->getQueryStatus(testQueryId) ==
              ExecutableQueryPlanStatus::Stopped);
...
TEST_F(NodeEngineTest, testParallelDifferentSource) {
  ...
  //  GeneratedQueryExecutionPlanBuilder builder1 =
  //  GeneratedQueryExecutionPlanBuilder::create();
  SchemaPtr sch1 = Schema::create()->addField("sum", BasicType::UINT32);

  auto sink1 =
      createCSVFileSink(sch1, SharedQueryId(1), DecomposedQueryPlanId(1),
                        engine, 1, getTestResourceFolder() / "qep1.csv", false);
  auto context1 = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sink1);
  auto executable1 = std::make_shared<TextExecutablePipeline>();
  auto pipeline1 = ExecutablePipeline::create(
      PipelineId(0), SharedQueryId(1), DecomposedQueryPlanId(1),
      engine->getQueryManager(), context1, executable1, 1, {sink1});
  auto source1 = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(1),
      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline1});
  auto executionPlan = ExecutableQueryPlan::create(
      SharedQueryId(1), DecomposedQueryPlanId(1), {source1}, {sink1},
      {pipeline1}, engine->getQueryManager(), engine->getBufferManager());

  SchemaPtr sch2 = Schema::create()->addField("sum", BasicType::UINT32);
  auto sink2 =
      createCSVFileSink(sch2, SharedQueryId(2), DecomposedQueryPlanId(2),
                        engine, 1, getTestResourceFolder() / "qep2.csv", false);
  auto context2 = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sink2);
  auto executable2 = std::make_shared<TextExecutablePipeline>();
  auto pipeline2 = ExecutablePipeline::create(
      PipelineId(0), SharedQueryId(2), DecomposedQueryPlanId(2),
      engine->getQueryManager(), context2, executable2, 1, {sink2});
  auto source2 = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(2),
      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline2});
  auto executionPlan2 = ExecutableQueryPlan::create(
      SharedQueryId(2), DecomposedQueryPlanId(2), {source2}, {sink2},
      {pipeline2}, engine->getQueryManager(), engine->getBufferManager());
  ...
  ASSERT_TRUE(engine->startQuery(SharedQueryId(1),
                                 executionPlan->getDecomposedQueryPlanId()));
  ASSERT_TRUE(engine->startQuery(SharedQueryId(2),
                                 executionPlan2->getDecomposedQueryPlanId()));

  ASSERT_TRUE(engine->getQueryStatus(SharedQueryId(1)) ==
              ExecutableQueryPlanStatus::Running);
  ASSERT_TRUE(engine->getQueryStatus(SharedQueryId(2)) ==
              ExecutableQueryPlanStatus::Running);

  executable1->completedPromise.get_future().get();
  executable2->completedPromise.get_future().get();

  ASSERT_TRUE(engine->stopQuery(SharedQueryId(1),
                                executionPlan->getDecomposedQueryPlanId(),
                                Runtime::QueryTerminationType::HardStop));
  ASSERT_TRUE(engine->stopQuery(SharedQueryId(2),
                                executionPlan2->getDecomposedQueryPlanId(),
                                Runtime::QueryTerminationType::HardStop));
  ...
  ASSERT_TRUE(engine->getQueryStatus(SharedQueryId(1)) ==
              ExecutableQueryPlanStatus::Invalid);
  ASSERT_TRUE(engine->getQueryStatus(SharedQueryId(2)) ==
              ExecutableQueryPlanStatus::Invalid);
...
TEST_F(NodeEngineTest, testParallelSameSource) {
  ...
  auto sink1 =
      createCSVFileSink(sch1, SharedQueryId(1), DecomposedQueryPlanId(1),
                        engine, 1, getTestResourceFolder() / "qep1.txt", true);
  auto context1 = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sink1);
  auto executable1 = std::make_shared<TextExecutablePipeline>();
  auto pipeline1 = ExecutablePipeline::create(
  ...
  DataSinkPtr sink2 =
      createCSVFileSink(sch2, SharedQueryId(2), DecomposedQueryPlanId(2),
                        engine, 1, getTestResourceFolder() / "qep2.txt", true);

  auto context2 = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sink2);
  auto executable2 = std::make_shared<TextExecutablePipeline>();
  auto pipeline2 = ExecutablePipeline::create(
      PipelineId(1), SharedQueryId(2), DecomposedQueryPlanId(2),
      engine->getQueryManager(), context2, executable2, 1, {sink2});
  DataSourcePtr source2 = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(2),
      OriginId(1), INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline2});
  auto executionPlan2 = ExecutableQueryPlan::create(
      SharedQueryId(2), DecomposedQueryPlanId(2), {source2}, {sink2},
      {pipeline2}, engine->getQueryManager(), engine->getBufferManager());
  ...
  while (engine->getQueryStatus(SharedQueryId(2)) !=
         Runtime::Execution::ExecutableQueryPlanStatus::Finished) {
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
  }

  while (engine->getQueryStatus(SharedQueryId(1)) !=
         Runtime::Execution::ExecutableQueryPlanStatus::Finished) {
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
  }

  ASSERT_TRUE(engine->undeployQuery(SharedQueryId(1),
                                    executionPlan->getDecomposedQueryPlanId()));
  ASSERT_TRUE(engine->undeployQuery(
      SharedQueryId(2), executionPlan2->getDecomposedQueryPlanId()));
  ASSERT_TRUE(engine->stop());
...
//
TEST_F(NodeEngineTest,
       DISABLED_testParallelSameSink) {  // shared sinks are not supported
  DefaultSourceTypePtr defaultSourceType =
      DefaultSourceType::create("test", "test1");
  auto workerConfiguration = WorkerConfiguration::create();
  workerConfiguration->physicalSourceTypes.add(defaultSourceType);
  ...
  SchemaPtr sch1 = Schema::create()->addField("sum", BasicType::UINT32);
  auto sharedSink = createCSVFileSink(
      sch1, SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID, engine, 1,
      getTestResourceFolder() / "qep12.txt", false);
  auto context1 = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sharedSink);
  auto executable1 = std::make_shared<TextExecutablePipeline>();
  auto pipeline1 = ExecutablePipeline::create(
      PipelineId(1), SharedQueryId(1), DecomposedQueryPlanId(1),
      engine->getQueryManager(), context1, executable1, 1, {sharedSink});
  auto source1 = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(1),
      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline1});
  auto executionPlan = ExecutableQueryPlan::create(
      SharedQueryId(1), DecomposedQueryPlanId(1), {source1}, {sharedSink},
      {pipeline1}, engine->getQueryManager(), engine->getBufferManager());
  ...
  auto executable2 = std::make_shared<TextExecutablePipeline>();
  auto pipeline2 = ExecutablePipeline::create(
      PipelineId(2), SharedQueryId(2), DecomposedQueryPlanId(2),
      engine->getQueryManager(), context2, executable2, 1, {sharedSink});

  DataSourcePtr source2 = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(2),
      OriginId(3), INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline2});

  auto executionPlan2 = ExecutableQueryPlan::create(
      SharedQueryId(2), DecomposedQueryPlanId(2), {source2}, {sharedSink},
      {pipeline2}, engine->getQueryManager(), engine->getBufferManager());
...
TEST_F(NodeEngineTest, DISABLED_testParallelSameSourceAndSinkRegstart) {
  ...
  SchemaPtr sch1 = Schema::create()->addField("sum", BasicType::UINT32);
  auto sink1 = createCSVFileSink(sch1, SharedQueryId(0),
                                 INVALID_DECOMPOSED_QUERY_PLAN_ID, engine, 1,
                                 getTestResourceFolder() / "qep3.txt", true);
  auto context1 = std::make_shared<MockedPipelineExecutionContext>(
      engine->getQueryManager(), sink1);
  auto executable1 = std::make_shared<TextExecutablePipeline>();
  auto pipeline1 = ExecutablePipeline::create(
  ...
  auto pipeline2 = ExecutablePipeline::create(
      PipelineId(1), SharedQueryId(2), DecomposedQueryPlanId(2),
      engine->getQueryManager(), context2, executable2, 1, {sink1});
  auto source1 = createDefaultSourceWithoutSchemaForOneBuffer(
      engine->getBufferManager(), engine->getQueryManager(), OperatorId(1),
      OriginId(4), INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
      {pipeline1, pipeline2});
  auto executionPlan = ExecutableQueryPlan::create(
      SharedQueryId(1), DecomposedQueryPlanId(1), {source1}, {sink1},
      {pipeline1}, engine->getQueryManager(), engine->getBufferManager());
  ...
  auto executionPlan2 = ExecutableQueryPlan::create(
      SharedQueryId(2), DecomposedQueryPlanId(2), {source1}, {sink1},
      {pipeline2}, engine->getQueryManager(), engine->getBufferManager());

  // GeneratedQueryExecutionPlanBuilder builder1 =
  // GeneratedQueryExecutionPlanBuilder::create(); DataSourcePtr source1 =
  //     createDefaultSourceWithoutSchemaForOneBuffer(engine->getBufferManager(),
  //     engine->getQueryManager(), 1, 12);
  /*
  DataSinkPtr sink1 = createTextFileSink(sch1, 0, engine, 1,
  getTestResourceFolder() / "qep3.txt", true);
  ...
  auto context1 =
      std::make_shared<MockedPipelineExecutionContext>(engine->getQueryManager(),
  sink1); auto executable1 = std::make_shared<TextExecutablePipeline>();
  //auto pipeline1 = ExecutablePipeline::create(0, 1, executable1, context1, 1,
  nullptr, source1->getSchema(), sch1);
  //builder1.addPipeline(pipeline1);

  GeneratedQueryExecutionPlanBuilder builder2 =
  GeneratedQueryExecutionPlanBuilder::create();
  ...
  auto context2 =
      std::make_shared<MockedPipelineExecutionContext>(engine->getQueryManager(),
  sink1); auto executable2 = std::make_shared<TextExecutablePipeline>();
      */
  // auto pipeline2 = ExecutablePipeline::create(0, 2, executable2, context2, 1,
  // nullptr, source1->getSchema(), sch2); builder2.addPipeline(pipeline2);
...
TEST_F(NodeEngineTest, testBufferData) {
  ...
  auto engine =
      Runtime::NodeEngineBuilder::create(workerConfiguration)
          .setQueryStatusListener(std::make_shared<DummyQueryListener>())
          .build();
  EXPECT_FALSE(engine->bufferData(INVALID_DECOMPOSED_QUERY_PLAN_ID,
                                  INVALID_OPERATOR_ID));
}
...
TEST_F(NodeEngineTest, testReconfigureSink) {
  ...
  auto engine =
      Runtime::NodeEngineBuilder::create(workerConfiguration)
          .setQueryStatusListener(std::make_shared<DummyQueryListener>())
          .build();
  EXPECT_FALSE(engine->updateNetworkSink(INVALID_WORKER_NODE_ID, "test", 0,
  ...
  class MockedNodeEngine : public NodeEngine {
   public:
    ...
    explicit MockedNodeEngine(
        ...
        QueryCompilation::QueryCompilerPtr&& compiler, WorkerId nodeEngineId,
        uint64_t numberOfBuffersInGlobalBufferManager,
        uint64_t numberOfBuffersInSourceLocalBufferPool,
        uint64_t numberOfBuffersPerWorker)
        : NodeEngine(std::move(physicalSources), std::move(hardwareManager),
                     std::move(bufferManagers), std::move(queryMgr),
                     std::move(netFuncInit), std::move(partitionManager),
                     std::move(compiler), std::weak_ptr<NesWorker>(),
                     std::make_shared<OpenCLManager>(), nodeEngineId,
                     numberOfBuffersInGlobalBufferManager,
                     numberOfBuffersInSourceLocalBufferPool,
                     numberOfBuffersPerWorker, false) {}

    void onFatalException(const std::shared_ptr<std::exception> exception,
                          std::string callstack) override {
      ASSERT_TRUE(stop(false));
      ASSERT_TRUE(strcmp(exception->what(),
                         "Failed assertion on false error message: this will "
                         "fail now with a RuntimeException") == 0);
...
TEST_F(NodeEngineTest, DISABLED_testSemiUnhandledExceptionCrash) {
  class MockedNodeEngine : public Runtime::NodeEngine {
   public:
    ...
    void onFatalException(const std::shared_ptr<std::exception> exception,
      ...
      NES_ERROR("{}", str);
      ASSERT_TRUE(
          strcmp(str, "Got fatal error on thread 0: Catch me if you can!") ==
          0);
      completedPromise.set_value(true);
  ...
  class FailingTextExecutablePipeline : public ExecutablePipelineStage {
   public:
    ...
    ExecutionResult execute(TupleBuffer&, PipelineExecutionContext&,
  ...
  // GeneratedQueryExecutionPlanBuilder::create(); DataSourcePtr source =
  //     createDefaultSourceWithoutSchemaForOneBuffer(engine->getBufferManager(),
  //     engine->getQueryManager(), 1, 12);
  SchemaPtr sch = Schema::create()->addField("sum", BasicType::UINT32);
  DataSinkPtr sink = createCSVFileSink(
      sch, INVALID_SHARED_QUERY_ID, INVALID_DECOMPOSED_QUERY_PLAN_ID, engine, 1,
      getTestResourceFolder() / "test.out", true);
  ...
  // auto pipeline = ExecutablePipeline::create(0, testQueryId, executable,
  // context, 1, nullptr, source->getSchema(), sch);
  // builder.addPipeline(pipeline);
  // auto qep = builder.build();
  // ASSERT_TRUE(engine->deployQueryInNodeEngine(qep));
  ASSERT_TRUE(engine->completedPromise.get_future().get());
  ASSERT_TRUE(engine->getQueryStatus(testQueryId) ==
              ExecutableQueryPlanStatus::ErrorState);
  ASSERT_TRUE(engine->stop());
}
...
TEST_F(NodeEngineTest, DISABLED_testFatalCrash) {
  ...
  auto engine =
      Runtime::NodeEngineBuilder::create(workerConfiguration)
          .setQueryStatusListener(std::make_shared<DummyQueryListener>())
          .build();
  EXPECT_EXIT(detail::segkiller(), testing::ExitedWithCode(1),
              "Runtime failed fatally");
}
