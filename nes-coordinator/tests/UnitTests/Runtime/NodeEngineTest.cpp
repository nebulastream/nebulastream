...
createMockedEngine(const std::string& hostname, uint16_t port, uint64_t bufferSize = 8192, uint64_t numBuffers = 1024)
{
    try
    {
        ...
        auto queryManager = std::make_shared<Runtime::DynamicQueryManager>(
            std::make_shared<DummyQueryListener>(), bufferManager, INVALID_WORKER_NODE_ID, 1, nullptr, 100);
        auto networkManagerCreator = [=](const Runtime::NodeEnginePtr& engine)
        {
            return Network::NetworkManager::create(
                INVALID_WORKER_NODE_ID, hostname, port, Network::ExchangeProtocol(partitionManager, engine), bufferManager[0]);
        };
...
public:
    ...
    ExecutionResult execute(TupleBuffer& inputTupleBuffer, PipelineExecutionContext& pipelineExecutionContext, WorkerContext& wctx) override
    {
        ...
        NES_INFO(
            "Test: query result = Processed Block:{} count: {} psum: {} sum: {}", inputTupleBuffer.getNumberOfTuples(), count, psum, sum);
...
public:
    MockedPipelineExecutionContext(Runtime::QueryManagerPtr queryManager, const DataSinkPtr& sink)
        : PipelineExecutionContext(
            ...
            [sink](TupleBuffer& buffer, Runtime::WorkerContextRef worker) { sink->writeData(buffer, worker); },
            [sink](TupleBuffer&) {},
...
auto setupQEP(const NodeEnginePtr& engine, SharedQueryId sharedQueryId, const std::string& outPath)
{
    ...
    DataSinkPtr sink = createCSVFileSink(sch, sharedQueryId, DecomposedQueryPlanId(sharedQueryId.getRawValue()), engine, 1, outPath, false);
...
TEST_F(NodeEngineTest, testParallelDifferentSource)
{
    ...
    auto sink1
        = createCSVFileSink(sch1, SharedQueryId(1), DecomposedQueryPlanId(1), engine, 1, getTestResourceFolder() / "qep1.csv", false);
    ...
    auto pipeline1 = ExecutablePipeline::create(
        PipelineId(0), SharedQueryId(1), DecomposedQueryPlanId(1), engine->getQueryManager(), context1, executable1, 1, {sink1});
    auto source1 = createDefaultSourceWithoutSchemaForOneBuffer(
        ...
        {pipeline1});
    auto executionPlan = ExecutableQueryPlan::create(
        SharedQueryId(1), DecomposedQueryPlanId(1), {source1}, {sink1}, {pipeline1}, engine->getQueryManager(), engine->getBufferManager());

    SchemaPtr sch2 = Schema::create()->addField("sum", BasicType::UINT32);
    auto sink2
        = createCSVFileSink(sch2, SharedQueryId(2), DecomposedQueryPlanId(2), engine, 1, getTestResourceFolder() / "qep2.csv", false);
    ...
    auto pipeline2 = ExecutablePipeline::create(
        PipelineId(0), SharedQueryId(2), DecomposedQueryPlanId(2), engine->getQueryManager(), context2, executable2, 1, {sink2});
    ...
    auto executionPlan2 = ExecutableQueryPlan::create(
        SharedQueryId(2), DecomposedQueryPlanId(2), {source2}, {sink2}, {pipeline2}, engine->getQueryManager(), engine->getBufferManager());
    ...
    ASSERT_TRUE(engine->stopQuery(SharedQueryId(1), executionPlan->getDecomposedQueryPlanId(), Runtime::QueryTerminationType::HardStop));
    ASSERT_TRUE(engine->stopQuery(SharedQueryId(2), executionPlan2->getDecomposedQueryPlanId(), Runtime::QueryTerminationType::HardStop));
...
TEST_F(NodeEngineTest, testParallelSameSource)
{
    ...
    auto sink1 = createCSVFileSink(sch1, SharedQueryId(1), DecomposedQueryPlanId(1), engine, 1, getTestResourceFolder() / "qep1.txt", true);
    auto context1 = std::make_shared<MockedPipelineExecutionContext>(engine->getQueryManager(), sink1);
    auto executable1 = std::make_shared<TextExecutablePipeline>();
    auto pipeline1 = ExecutablePipeline::create(
        PipelineId(0), SharedQueryId(1), DecomposedQueryPlanId(1), engine->getQueryManager(), context1, executable1, 1, {sink1});
    ...
    SchemaPtr sch2 = Schema::create()->addField("sum", BasicType::UINT32);
    DataSinkPtr sink2
        = createCSVFileSink(sch2, SharedQueryId(2), DecomposedQueryPlanId(2), engine, 1, getTestResourceFolder() / "qep2.txt", true);

    auto context2 = std::make_shared<MockedPipelineExecutionContext>(engine->getQueryManager(), sink2);
    auto executable2 = std::make_shared<TextExecutablePipeline>();
    auto pipeline2 = ExecutablePipeline::create(
        PipelineId(1), SharedQueryId(2), DecomposedQueryPlanId(2), engine->getQueryManager(), context2, executable2, 1, {sink2});
...
{ // shared sinks are not supported
    DefaultSourceTypePtr defaultSourceType = DefaultSourceType::create("test", "test1");
    auto workerConfiguration = WorkerConfiguration::create();
    workerConfiguration->physicalSourceTypes.add(defaultSourceType);
    ...
    auto sharedSink = createCSVFileSink(
        sch1, SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID, engine, 1, getTestResourceFolder() / "qep12.txt", false);
    auto context1 = std::make_shared<MockedPipelineExecutionContext>(engine->getQueryManager(), sharedSink);
    auto executable1 = std::make_shared<TextExecutablePipeline>();
    auto pipeline1 = ExecutablePipeline::create(
        PipelineId(1), SharedQueryId(1), DecomposedQueryPlanId(1), engine->getQueryManager(), context1, executable1, 1, {sharedSink});
    auto source1 = createDefaultSourceWithoutSchemaForOneBuffer(
        engine->getBufferManager(),
        engine->getQueryManager(),
        OperatorId(1),
    ...
    auto executable2 = std::make_shared<TextExecutablePipeline>();
    auto pipeline2 = ExecutablePipeline::create(
        PipelineId(2), SharedQueryId(2), DecomposedQueryPlanId(2), engine->getQueryManager(), context2, executable2, 1, {sharedSink});
...
TEST_F(NodeEngineTest, DISABLED_testParallelSameSourceAndSinkRegstart)
{
    ...
    SchemaPtr sch1 = Schema::create()->addField("sum", BasicType::UINT32);
    auto sink1 = createCSVFileSink(
        sch1, SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID, engine, 1, getTestResourceFolder() / "qep3.txt", true);
    auto context1 = std::make_shared<MockedPipelineExecutionContext>(engine->getQueryManager(), sink1);
    auto executable1 = std::make_shared<TextExecutablePipeline>();
    auto pipeline1 = ExecutablePipeline::create(
        PipelineId(0), SharedQueryId(1), DecomposedQueryPlanId(1), engine->getQueryManager(), context1, executable1, 1, {sink1});
    ...
    auto executable2 = std::make_shared<TextExecutablePipeline>();
    auto pipeline2 = ExecutablePipeline::create(
        PipelineId(1), SharedQueryId(2), DecomposedQueryPlanId(2), engine->getQueryManager(), context2, executable2, 1, {sink1});
    auto source1 = createDefaultSourceWithoutSchemaForOneBuffer(
        engine->getBufferManager(),
        engine->getQueryManager(),
        OperatorId(1),
    ...
    auto executionPlan2 = ExecutableQueryPlan::create(
        SharedQueryId(2), DecomposedQueryPlanId(2), {source1}, {sink1}, {pipeline2}, engine->getQueryManager(), engine->getBufferManager());
...
TEST_F(NodeEngineTest, testReconfigureSink)
{
    ...
    auto engine
        = Runtime::NodeEngineBuilder::create(workerConfiguration).setQueryStatusListener(std::make_shared<DummyQueryListener>()).build();
    EXPECT_FALSE(engine->updateNetworkSink(INVALID_WORKER_NODE_ID, "test", 0, INVALID_DECOMPOSED_QUERY_PLAN_ID, INVALID_OPERATOR_ID));
}
...
void assertKiller()
{
    ...
    public:
        ...
        void onFatalException(const std::shared_ptr<std::exception> exception, std::string callstack) override
        {
            ...
            ASSERT_TRUE(
                strcmp(exception->what(), "Failed assertion on false error message: this will fail now with a RuntimeException") == 0);
...
TEST_F(NodeEngineTest, DISABLED_testSemiUnhandledExceptionCrash)
{
    ...
    DataSinkPtr sink = createCSVFileSink(
        sch, INVALID_SHARED_QUERY_ID, INVALID_DECOMPOSED_QUERY_PLAN_ID, engine, 1, getTestResourceFolder() / "test.out", true);
