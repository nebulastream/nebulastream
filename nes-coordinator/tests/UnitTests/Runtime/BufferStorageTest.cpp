...
protected:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BufferStorageTest.log",
                              NES::LogLevel::LOG_DEBUG);
  }
};

/**
 * @brief test inserts buffers to different queues and checks after every
 * insertion that queue size increased on one. After the insertion is fully done
 * the size of the buffer storage is checked to be buffersInserted
 */
...
/**
 * @brief test inserts buffersInserted amount of buffers into one queue but
 * starts from the biggest watermark. The queue is then checked to be sorted to
 * be exact to have the biggest watermark value at the top.
 */
...
/**
 * @brief test checks that if trimming is called on an empty buffer it doesn't
 * cause an error
 */
...
/**
 * @brief test inserts buffersInserted buffers in different queues and deletes
 * them.
 */
...
/**
 * @brief test inserts buffersInserted buffers in one queue and leaves only one
 * after trimming. The test checks that the deleted buffers are smaller that
 * passed timestamp.
 */
