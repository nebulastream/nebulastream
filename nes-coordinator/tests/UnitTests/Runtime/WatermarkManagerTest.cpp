...
class WatermarkManagerTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("WatermarkManagerTest.log",
...
TEST_F(WatermarkManagerTest, singleThreadWatermarkUpdaterTest) {
  ...
  auto watermarkManager =
      Windowing::MultiOriginWatermarkProcessor::create(/*origins*/ 1);
  // preallocate watermarks for each transaction
  std::vector<std::tuple<WatermarkTs, SequenceNumber, OriginId>>
      watermarkBarriers;
  ...
  for (int threadId = 0; threadId < threadsCount; threadId++) {
    threads.emplace_back(thread([&watermarkManager, &watermarkBarriers,
      ...
      for (auto i = 0; i < updates; i++) {
        ...
        auto oldWatermark = watermarkManager->getCurrentWatermark();
        // check if the watermark manager dose not return a watermark higher
...
TEST_F(WatermarkManagerTest, singleThreadWatermarkUpdaterMultipleOriginsTest) {
  ...
  auto origins = 10;
  auto watermarkManager =
      Windowing::MultiOriginWatermarkProcessor::create(/*origins*/ origins);
  // preallocate watermarks for each transaction
  std::vector<std::tuple<WatermarkTs, SequenceNumber, OriginId>>
      watermarkBarriers;
  for (int i = 1; i <= updates; i++) {
