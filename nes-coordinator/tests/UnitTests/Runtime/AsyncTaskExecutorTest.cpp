...
class AsyncTaskExecutorTest : public Testing::BaseUnitTest,
 ...
 public:
  void SetUp() override {
    ...
    executor = std::make_shared<Runtime::AsyncTaskExecutor>(hardwareManager,
                                                            GetParam());
  }
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("AsyncTaskExecutorTest.log",
...
TEST_P(AsyncTaskExecutorTest, submitTaskWithStoppedExecutor) {
  ...
  try {
    auto future =
        executor->runAsync([](auto x, auto y) { return x + y; }, 1, 1);
    ASSERT_TRUE(!!future);
    auto sum = future.wait();
    ASSERT_EQ(2, sum);
    FAIL();
  } catch (std::exception const& expected) {
    EXPECT_THAT(expected.what(),
                testing::HasSubstr("Async Executor is destroyed"));
  } catch (...) {
    FAIL();
  }
}

INSTANTIATE_TEST_CASE_P(AsyncTaskExecutorMTTest, AsyncTaskExecutorTest,
