...
TEST_F(TupleBufferTest, testPrintingOfTupleBuffer) {
  ...
  for (unsigned int i = 0; i < 5; ++i) {
    ...
    NES_DEBUG("{} | {} | {} | {} | {}", my_array[i].i64, my_array[i].f,
              my_array[i].d, my_array[i].i32, std::string(my_array[i].s, 5));
  }
  ...
  auto rowLayout =
      Runtime::MemoryLayouts::RowLayout::create(s, buf.getBufferSize());
  auto testTupleBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(rowLayout, buf);
