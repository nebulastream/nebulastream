...
class LineageManagerTest : public Testing::BaseUnitTest {
 protected:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LineageManagerTest.log",
                              NES::LogLevel::LOG_DEBUG);
  }
};

/**
 * @brief test inserts one buffer into bufferAncestorMapping manager and checks
 * that it was successfully inserted
 */
TEST_F(LineageManagerTest, OneBufferInsertionInLineageManager) {
  ...
  lineageManager->insert(BufferSequenceNumber(0, 0),
                         BufferSequenceNumber(1, 1));
  ASSERT_EQ(lineageManager->getLineageSize(), oneBuffer);
}

/**
 * @brief test inserts buffers into bufferAncestorMapping manager and checks
 * after every insertion that the table size increased on one.
 */
TEST_F(LineageManagerTest, bufferInsertionInLineageManager) {
  auto lineageManager = std::make_shared<Runtime::InMemoryLineageManager>();
  for (size_t i = 0; i < buffersInserted; i++) {
    lineageManager->insert(BufferSequenceNumber(i, i),
                           BufferSequenceNumber(i + 1, i + 1));
...
/**
 * @brief test inserts buffers with the same new buffer sequence number into
 * bufferAncestorMapping and checks that the vector size with a given new buffer
 * sequnce number increased.
 */
TEST_F(LineageManagerTest, bufferInsertionWithTheSameSNInLineageManager) {
  auto lineageManager = std::make_shared<Runtime::InMemoryLineageManager>();
  for (size_t i = 0; i < buffersInserted; i++) {
    lineageManager->insert(BufferSequenceNumber(0, 0),
                           BufferSequenceNumber(i, i));
  }
  ASSERT_EQ(lineageManager->findTupleBufferAncestor(BufferSequenceNumber(0, 0))
                .size(),
            buffersInserted);
}
...
/**
 * @brief test trims buffers from a bufferAncestorMapping manager and checks
 * after every deletion that the table size decreased on one.
 */
...
TEST_F(LineageManagerTest, invertExistingId) {
  ...
  ASSERT_EQ(lineageManager->getLineageSize(), buffersInserted);
  ASSERT_EQ(
      lineageManager->findTupleBufferAncestor(BufferSequenceNumber(0, 0))[0],
      BufferSequenceNumber(1, 1));
}
