...
class BufferManagerTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("BufferManagerTest.log",
...
TEST_F(BufferManagerTest, initializedBufferManager) {
  ...
  auto bufferManager = std::make_shared<Runtime::BufferManager>(
      buffer_size, buffers_managed, hardwareManager->getGlobalAllocator());
...
TEST_F(BufferManagerTest, initializedBufferManagerWithNuma) {
  auto hardwareManager = std::make_shared<Runtime::HardwareManager>();
  auto bufferManager = std::make_shared<Runtime::BufferManager>(
      buffer_size, buffers_managed, hardwareManager->getNumaAllactor(0));
  size_t buffers_count = bufferManager->getNumOfPooledBuffers();
  size_t buffers_free = bufferManager->getAvailableBuffers();
  ASSERT_EQ(buffers_count, buffers_managed);
  ASSERT_EQ(buffers_free, buffers_managed);
...
TEST_F(BufferManagerTest, initializedBufferManagerAlignment64) {
  auto bufferManager = std::make_shared<Runtime::BufferManager>(
      buffer_size, buffers_managed,
      std::make_shared<Runtime::NesDefaultMemoryAllocator>(), 64);
...
TEST_F(BufferManagerTest, singleThreadedBufferRecycling) {
  auto bufferManager =
      std::make_shared<Runtime::BufferManager>(buffer_size, buffers_managed);
  ...
  for (uint32_t i = 0ul; i < consumer_threads; ++i) {
    con_threads.emplace_back([&workQueue, &mutex, &cvar, &numBuffers]() {
      ...
      while (true) {
        ...
        workQueue.pop_front();
        lock.unlock();
        if (buf.getBuffer<uint32_t>()[buffer_size / sizeof(uint32_t) - 1] ==
            max_buffer) {
...
TupleBuffer getBufferTimeout(
    const std::shared_ptr<Runtime::BufferManager>& bufferManager,
  ...
  for (auto i = 0UL; i < consumer_threads; ++i) {
    con_threads.emplace_back([&workQueue, &mutex, &cvar]() {
      ...
      while (true) {
        ...
        lock.unlock();
        auto ctrl_val =
            buf.getBuffer<uint32_t>()[buffer_size / sizeof(uint32_t) - 1];
...
TEST_F(BufferManagerTest, singleThreadedBufferRecyclingWithChildren) {
  auto bufferManager =
      std::make_shared<Runtime::BufferManager>(buffer_size, 10);
  auto buffer0 = bufferManager->getBufferBlocking();
  auto buffer1 = bufferManager->getBufferBlocking();
  ...
  buffer4.release();
  ASSERT_EQ(buffer4.getReferenceCounter(),
            0);  // buffer4 does not exist anymore
  ASSERT_EQ(bufferManager->getAvailableBuffers(),
            7);  //.. but its underlying segment is still alive
