...
public:
    ...
    void runMetricsQueryTest(
        uint64_t workerCnt, Monitoring::MetricCollectorType collectorType, SchemaPtr schema, Monitoring::MetricType expectedType)
    {
        ...
        MonitoringSourceTypePtr sourceType = MonitoringSourceType::create("logTestMetricStream", "physMetricSource", collectorType);
        ...
        for (uint64_t nodeId = 2; nodeId <= workerCnt + 1; nodeId++)
        {
            ...
            ASSERT_TRUE(MetricValidator::isValid(
                Monitoring::SystemResourcesReaderFactory::getSystemResourcesReader(), storedMetrics, expectedType, WorkerId(nodeId), 2));
        }
