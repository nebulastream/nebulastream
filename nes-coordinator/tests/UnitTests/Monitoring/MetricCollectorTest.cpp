...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MetricCollectorTest.log",
...
TEST_F(MetricCollectorTest, testNetworkCollectorWrappedMetrics) {
  ...
  ASSERT_EQ(networkMetric->getMetricType(),
            Monitoring::MetricType::WrappedNetworkMetrics);

  Monitoring::NetworkMetricsWrapper wrappedMetric =
      networkMetric->getValue<Monitoring::NetworkMetricsWrapper>();

  if (reader->getReaderType() != SystemResourcesReaderType::AbstractReader) {
    auto bufferSize =
        Monitoring::NetworkMetrics::getSchema("")->getSchemaSizeInBytes() *
        wrappedMetric.size();
    ...
    NES_DEBUG("MetricCollectorTest:\nRead metric {}\nParsed metric: {}",
              asJson(wrappedMetric).dump(), asJson(parsedMetric).dump());
    ASSERT_EQ(wrappedMetric, parsedMetric);
    ASSERT_EQ(parsedMetric.getNodeId(), nodeId);
  } else {
    NES_DEBUG("MetricCollectorTest: Skipping "
              "testNetworkCollectorWrappedMetrics. Abstract reader found.");
  }
}
...
TEST_F(MetricCollectorTest, testNetworkCollectorSingleMetrics) {
  ...
  if (reader->getReaderType() != SystemResourcesReaderType::AbstractReader) {
    ...
    NES_DEBUG("MetricCollectorTest: Skipping "
              "testNetworkCollectorSingleMetrics. Abstract reader found.");
  }
}
...
TEST_F(MetricCollectorTest, testCpuCollectorWrappedMetrics) {
  ...
  ASSERT_EQ(cpuMetric->getMetricType(),
            Monitoring::MetricType::WrappedCpuMetrics);

  Monitoring::CpuMetricsWrapper wrappedMetric =
  ...
  if (reader->getReaderType() != SystemResourcesReaderType::AbstractReader) {
    ...
    NES_DEBUG("MetricCollectorTest: Skipping testCpuCollectorWrappedMetrics. "
              "Abstract reader found.");
  }
}
...
TEST_F(MetricCollectorTest, testCpuCollectorSingleMetrics) {
  ...
  if (reader->getReaderType() != SystemResourcesReaderType::AbstractReader) {
    ...
    NES_DEBUG("MetricCollectorTest: Skipping testcpuCollectorSingleMetrics. "
              "Abstract reader found.");
  }
}
...
TEST_F(MetricCollectorTest, testDiskCollector) {
  ...
  Monitoring::MetricPtr parsedMetric =
      std::make_shared<Monitoring::Metric>(Monitoring::DiskMetrics{});
  readFromBuffer(parsedMetric, tupleBuffer, 0);
  NES_DEBUG("MetricCollectorTest:\nRead metric {}\nParsed metric: {}",
            asJson(typedMetric).dump(), asJson(parsedMetric).dump());
...
TEST_F(MetricCollectorTest, testMemoryCollector) {
  ...
  Monitoring::MemoryMetrics typedMetric =
      memoryMetric->getValue<Monitoring::MemoryMetrics>();
  ASSERT_EQ(memoryMetric->getMetricType(),
            Monitoring::MetricType::MemoryMetric);
  auto bufferSize =
      Monitoring::MemoryMetrics::getSchema("")->getSchemaSizeInBytes();
  auto tupleBuffer = bufferManager->getUnpooledBuffer(bufferSize).value();
  writeToBuffer(typedMetric, tupleBuffer, 0);
