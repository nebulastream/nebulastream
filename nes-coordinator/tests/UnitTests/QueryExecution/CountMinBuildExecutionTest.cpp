...
class CountMinBuildExecutionTest
    : public Testing::BaseUnitTest,
      public ::testing::WithParamInterface<std::tuple<
          uint64_t, uint64_t, uint64_t, Statistic::StatisticDataCodec>> {
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("CountMinBuildExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO(
        "QueryExecutionTest: Setup CountMinBuildExecutionTest test class.");
  }
  ...
  void SetUp() override {
    ...
    const auto numWorkerThreads =
        std::get<0>(CountMinBuildExecutionTest::GetParam());
    ...
    statisticFormat = Statistic::StatisticFormatFactory::createFromSchema(
        ...
        Statistic::StatisticSynopsisType::COUNT_MIN, statisticDataCodec);
  ...
  void TearDown() override {
    NES_INFO(
        "QueryExecutionTest: Tear down CountMinBuildExecutionTest test case.");
    EXPECT_TRUE(executionEngine->stop());
    Testing::BaseUnitTest::TearDown();
  }

  /**
   * @brief Runs the statistic query and checks if the created count min
   * sketches are correct by creating count min sketches on our own and then
  ...
  void runQueryAndCheckCorrectness(
    ...
    // Emitting the input buffers and creating the expected count min sketches
    ...
    for (auto buf : allInputBuffers) {
      ...
      auto dynamicBuffer =
          MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(buf,
                                                                inputSchema);
      Util::updateTestCountMinStatistic(
          dynamicBuffer, testStatisticStore, metricHash, numberOfBitsInKey,
          windowSize, windowSlide, sketchWidth, sketchDepth,
    ...
    for (auto &[statisticHash, countMinStatistic] :
         nodeEngineStatisticStore->getAllStatistics()) {
      NES_DEBUG("Searching for statisticHash = {} countMinStatistic = {}",
                statisticHash, countMinStatistic->toString());
      auto expectedStatistics = testStatisticStore->getStatistics(
          statisticHash, countMinStatistic->getStartTs(),
          countMinStatistic->getEndTs());
...
TEST_P(CountMinBuildExecutionTest, singleInputTuple) {
  ...
  auto allInputBuffers = Util::createDataForOneFieldAndTimeStamp(
      ...
      fieldToBuildCountMinOver, timestampFieldName, isIngestionTime);

  // Creating the sink and the sources
  const auto testSinkDescriptor = StatisticSinkDescriptor::create(
      StatisticSynopsisType::COUNT_MIN, statisticDataCodec);
  ...
  auto countMinDescriptor = CountMinDescriptor::create(
      Over(fieldToBuildCountMinOver), sketchWidth, sketchDepth);
  runQueryAndCheckCorrectness(testSourceDescriptor, testSinkDescriptor,
                              countMinDescriptor, windowSize, windowSize,
                              timeCharacteristic, allInputBuffers);
}

/**
 * @brief Here we test, if we create multiple count min sketches for multiple
...
TEST_P(CountMinBuildExecutionTest, multipleInputBuffersSlidingWindow) {
  ...
  auto countMinDescriptor = CountMinDescriptor::create(
      Over(fieldToBuildCountMinOver), sketchWidth, sketchDepth);
  runQueryAndCheckCorrectness(testSourceDescriptor, testSinkDescriptor,
                              countMinDescriptor, windowSize, windowSlide,
                              timeCharacteristic, allInputBuffers);
}

/**
 * @brief Here we test, if we create multiple count min sketches for multiple
 * input buffers, but also for larger sketches. The difference is that we use
 * the ingestion time instead of an event time
 */
...
INSTANTIATE_TEST_CASE_P(
    testCountMin, CountMinBuildExecutionTest,
    ::testing::Combine(
    ...
    [](const testing::TestParamInfo<CountMinBuildExecutionTest::ParamType>
           &info) {
