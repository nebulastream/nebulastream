...
public:
        ...
        std::vector<TupleBuffer> allInputBuffers)
    {
        ...
        {
            NES_DEBUG(
                "Waiting till all statistics have been built. Currently {} and expecting {}...",
...
TEST_P(CountMinBuildExecutionTest, singleInputTuple)
{
    ...
    auto allInputBuffers = Util::createDataForOneFieldAndTimeStamp(
        numberOfTuples, *executionEngine->getBufferManager(), inputSchema, fieldToBuildCountMinOver, timestampFieldName, isIngestionTime);
    ...
    runQueryAndCheckCorrectness(
        testSourceDescriptor, testSinkDescriptor, countMinDescriptor, windowSize, windowSize, timeCharacteristic, allInputBuffers);
}
...
TEST_P(CountMinBuildExecutionTest, multipleInputBuffersSlidingWindow)
{
    ...
    auto countMinDescriptor = CountMinDescriptor::create(Over(fieldToBuildCountMinOver), sketchWidth, sketchDepth);
    runQueryAndCheckCorrectness(
        testSourceDescriptor, testSinkDescriptor, countMinDescriptor, windowSize, windowSlide, timeCharacteristic, allInputBuffers);
}
...
INSTANTIATE_TEST_CASE_P(
    ...
    [](const testing::TestParamInfo<CountMinBuildExecutionTest::ParamType>& info)
    {
        ...
        return std::to_string(numWorkerThread) + "Threads_" + std::to_string(sketchWidth) + "Width_" + std::to_string(sketchDepth)
            + "Depth_" + std::string(magic_enum::enum_name(dataCodec));
    });
} // namespace NES::Runtime::Execution
