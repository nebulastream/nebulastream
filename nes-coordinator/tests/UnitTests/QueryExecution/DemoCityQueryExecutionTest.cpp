...
class DemoCityQueryExecutionTest : public Testing::BaseUnitTest,
 ...
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("FilterQueryExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG(
        "FilterQueryExecutionTest: Setup FilterQueryExecutionTest test class.");
  }
  ...
  void TearDown() override {
    NES_DEBUG(
        "FilterQueryExecutionTest: Tear down FilterQueryExecutionTest test "
        "case.");
  ...
  /* Generates an input buffer for each source schema, using the
     inputDataGenerators, creates a source and emits the buffer using the newly
     created source. */
  void generateAndEmitInputBuffers(
      ...
      std::vector<std::function<void(Runtime::MemoryLayouts::TestTupleBuffer&)>>
          inputDataGenerators) {
    // Make sure that each source schema has one corresponding input data
    // generator.
    EXPECT_EQ(sourceSchemas.size(), inputDataGenerators.size());

    // For each source schema, create a source and an input buffer. Fill the
    // input buffer using the corresponding input data generator and finally use
    // the source to emit the input buffer.
    for (size_t sourceSchemaIdx = 0; sourceSchemaIdx < sourceSchemas.size();
         ++sourceSchemaIdx) {
      auto source = executionEngine->getDataSource(queryPlan, sourceSchemaIdx);
      auto inputBuffer =
          executionEngine->getBuffer(sourceSchemas.at(sourceSchemaIdx));
...
TEST_F(DemoCityQueryExecutionTest, demoQueryWithUnions) {
  ...
  std::function<void(Runtime::MemoryLayouts::TestTupleBuffer&)>
  ...
  const auto query =
      TestQuery::from(windTurbines)
          ...
          .map(Attribute("DifferenceProducedConsumedPower") =
                   Attribute("producedPower") - Attribute("consumedPower"))
          // Todo: #4068: Use 'Attribute("start").as("timestamp")' instead of
          // 'Attribute("start")'
          //               -> Currently not possible, because renaming and
          //               reducing the number of fields leads to an error.
  ...
  generateAndEmitInputBuffers(
      plan, {producerSchema, producerSchema, consumerSchema},
      {solarPanelDataGenerator, windTurbineDataGenerator,
       consumersDataGenerator});
  ...
  for (size_t recordIdx = 0; recordIdx < resultRecords.size(); ++recordIdx) {
    ...
    EXPECT_EQ(resultRecords.at(recordIdx).start,
              recordIdx * milliSecondsToHours);
  }
