...
public:
    ...
    void SetUp() override
    {
        ...
        executionEngine
            = std::make_shared<Testing::TestExecutionEngine>(queryCompilerDumpMode, numWorkerThreads, joinStrategy, windowingStrategy);
    }
    ...
    std::pair<std::vector<ResultRecord>, ExecutableQueryPlanPtr> runQueryWithCsvFilesWithoutStoppingQuery(
        ...
        const Query& query)
    {
        ...
        {
            allInputBuffers.emplace_back(TestUtils::createExpectedBuffersFromCsv(fileNameInputBuffers, inputSchema, bufferManager));
        }
...
TEST_P(StreamJoinQueryExecutionTest, streamJoinExecutionTestCsvFiles)
{
    ...
    struct __attribute__((packed)) ResultRecord
    {
        ...
        {
            return test1test2Start == rhs.test1test2Start && test1test2End == rhs.test1test2End && test1f1_left == rhs.test1f1_left
                && test1f2_left == rhs.test1f2_left && test1timestamp == rhs.test1timestamp && test2f1_right == rhs.test2f1_right
...
TEST_P(StreamJoinQueryExecutionTest, streamJoinExecutiontTestWithSlidingWindows)
{
    ...
    struct __attribute__((packed)) ResultRecord
    {
        ...
        bool operator==(const ResultRecord& rhs) const
        {
            return test1test2$start == rhs.test1test2$start && test1test2$end == rhs.test1test2$end && test1$key == rhs.test1$key
                && test1$values == rhs.test1$values && test1$values2 == rhs.test1$values2 && test1$timestamp == rhs.test1$timestamp
    ...
    // Getting the expected output tuples
    auto bufferManager = executionEngine->getBufferManager();
    const auto expectedSinkBuffer = TestUtils::createExpectedBuffersFromCsv(csvFileParams.expectedFile, sinkSchema, bufferManager)[0];
    ...
    const auto [resultRecords, qp] = runQueryWithCsvFilesWithoutStoppingQuery<ResultRecord>(
        {{joinParams.inputSchemas[0], csvFileParams.inputCsvFiles[0]}, {joinParams.inputSchemas[1], csvFileParams.inputCsvFiles[1]}},
        expectedSinkVector.size(),
        testSink,
        query);
...
TEST_P(StreamJoinQueryExecutionTest, streamJoinExecutiontTestWithWindows)
{
    ...
    struct __attribute__((packed)) ResultRecord
    {
        ...
        bool operator==(const ResultRecord& rhs) const
        {
            return test1test2$start == rhs.test1test2$start && test1test2$end == rhs.test1test2$end && test1$start == rhs.test1$start
                && test1$end == rhs.test1$end && test1$f2_left == rhs.test1$f2_left && test1$fieldForSum1 == rhs.test1$fieldForSum1
    ...
    TestUtils::CsvFileParams csvFileParams("stream_join_left_withSum.csv", "stream_join_right_withSum.csv", "stream_join_withSum_sink.csv");
    TestUtils::JoinParams joinParams({leftSchema, rightSchema});
