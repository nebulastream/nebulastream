...
class StreamJoinQueryExecutionTest
 ...
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("StreamJoinQueryExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO(
        "QueryExecutionTest: Setup StreamJoinQueryExecutionTest test class.");
  }
  ...
  void SetUp() override {
    ...
    joinStrategy = std::get<0>(
        NES::Runtime::Execution::StreamJoinQueryExecutionTest::GetParam());
    windowingStrategy = std::get<1>(
        NES::Runtime::Execution::StreamJoinQueryExecutionTest::GetParam());
    const auto numWorkerThreads = 1;
    executionEngine = std::make_shared<Testing::TestExecutionEngine>(
        queryCompilerDumpMode, numWorkerThreads, joinStrategy,
        windowingStrategy);
  }
  ...
  void TearDown() override {
    NES_INFO(
        "QueryExecutionTest: Tear down StreamJoinQueryExecutionTest test "
        "case.");
  ...
  /**
   * @brief Runs a join query with the inputs (Schema, CSVFile). Currently, we
   * only support csv files as the input. This test does not temrminate the
   * query, which is important if the test should examine the join results
   * without forcefully flushing the joins windows.
   * @tparam ResultRecord
   * @param inputs
   * @param expectedNumberOfTuples
   * @param testSink
   * @param query
   * @return (Vector of ResultRecords, QueryPlan which needs to be passed to
   * stopquery)
   */
  template <typename ResultRecord>
  std::pair<std::vector<ResultRecord>, ExecutableQueryPlanPtr>
  runQueryWithCsvFilesWithoutStoppingQuery(
    ...
    // A correct implementation should produce the expectedNumberOfTuples within
    // the defaultTimeout.
    testSink->waitTillCompletedOrTimeout(expectedNumberOfTuples,
                                         defaultTimeout.count());
    // Checking for correctness
    return {testSink->getResult(), plan};
  }
  /**
   * @brief Runs a join query with the inputs (Schema, CSVFile). Currently, we
   * only support csv files as the input
   * @tparam ResultRecord
   * @param inputs
   * @param expectedNumberOfTuples
   * @param testSink
   ...
   */
  template <typename ResultRecord>
  std::vector<ResultRecord> runQueryWithCsvFiles(
      const std::vector<std::pair<SchemaPtr, std::string>>& inputs,
      const uint64_t expectedNumberOfTuples,
      const std::shared_ptr<CollectTestSink<ResultRecord>>& testSink,
      const Query& query) {
  ...
  template <typename ResultRecord>
  std::vector<ResultRecord> runSingleJoinQuery(
      const TestUtils::CsvFileParams& csvFileParams,
      const TestUtils::JoinParams& joinParams, const WindowTypePtr& joinWindow,
      const std::string keyLeft, const std::string keyRight) {
    // Getting the expected output tuples
    auto bufferManager = executionEngine->getBufferManager();

    std::vector<ResultRecord> expectedSinkVector;
    const auto expectedSinkBuffers = TestUtils::createExpectedBuffersFromCsv(
        csvFileParams.expectedFile, joinParams.outputSchema, bufferManager);

    for (const auto& buf : expectedSinkBuffers) {
      const auto tmpVec =
          TestUtils::createVecFromTupleBuffer<ResultRecord>(buf);
      expectedSinkVector.insert(expectedSinkVector.end(), tmpVec.begin(),
                                tmpVec.end());
    }

    const auto testSink = executionEngine->createCollectSink<ResultRecord>(
    ...
    const auto testSourceDescriptorLeft =
        executionEngine->createDataSource(joinParams.inputSchemas[0]);
    const auto testSourceDescriptorRight =
        executionEngine->createDataSource(joinParams.inputSchemas[1]);
    ...
    auto resultRecords = runQueryWithCsvFiles<ResultRecord>(
        ...
        expectedSinkVector.size(), testSink, query);

    // Checking for correctness
    EXPECT_EQ(resultRecords.size(), expectedSinkVector.size());
    EXPECT_THAT(resultRecords,
                ::testing::UnorderedElementsAreArray(expectedSinkVector));

    // We return the result records, as someone might want to print them to
    // std::out or do something else
...
TEST_P(StreamJoinQueryExecutionTest, streamJoinExecutionTestCsvFiles) {
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    bool operator==(const ResultRecord& rhs) const {
      return test1test2Start == rhs.test1test2Start &&
             test1test2End == rhs.test1test2End &&
             test1f1_left == rhs.test1f1_left &&
             test1f2_left == rhs.test1f2_left &&
             test1timestamp == rhs.test1timestamp &&
             test2f1_right == rhs.test2f1_right &&
             test2f2_right == rhs.test2f2_right &&
  ...
  const auto window =
      TumblingWindow::of(EventTime(Attribute(timestampFieldName)), windowSize);

  // Running a single join query
  TestUtils::CsvFileParams csvFileParams(
      "stream_join_left.csv", "stream_join_right.csv", "stream_join_sink.csv");
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  const auto resultRecords = runSingleJoinQuery<ResultRecord>(
      csvFileParams, joinParams, window, "f2_left", "f2_right");
}
...
TEST_P(StreamJoinQueryExecutionTest, testJoinWithSameSchemaTumblingWindow) {
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    bool operator==(const ResultRecord& rhs) const {
      return window1window2Start == rhs.window1window2Start &&
             ...
             window2value == rhs.window2value && window2id == rhs.window2id &&
  ...
  const auto leftSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                              ->updateSourceName(*srcName);
  const auto rightSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                               ->updateSourceName(*srcName);
  ...
  TestUtils::CsvFileParams csvFileParams("window.csv", "window.csv",
                                         "window_sink.csv");
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  runSingleJoinQuery<ResultRecord>(csvFileParams, joinParams, window, "id",
                                   "id");
}
 ...
 */
TEST_P(StreamJoinQueryExecutionTest,
...
TEST_P(StreamJoinQueryExecutionTest,
  ...
  const auto leftSchema = TestSchemas::getSchemaTemplate("id_val_time_u64")
                              ->updateSourceName(*srcName);
  const auto rightSchema = TestSchemas::getSchemaTemplate("id2_time_u64")
                               ->updateSourceName(*srcName);
...
TEST_P(StreamJoinQueryExecutionTest, testJoinWithDifferentSourceSlidingWindow) {
  ...
  const auto timestampFieldName = "timestamp";
  const auto window = SlidingWindow::of(
      EventTime(Attribute(timestampFieldName)), windowSize, windowSlide);
...
TEST_P(StreamJoinQueryExecutionTest, testJoinWithLargerWindowSizes) {
  ...
  // Running a single join query
  TestUtils::CsvFileParams csvFileParams("window7.csv", "window7.csv",
                                         "window_sink7.csv");
  TestUtils::JoinParams joinParams({leftSchema, rightSchema});
  runSingleJoinQuery<ResultRecord>(csvFileParams, joinParams, window, "id",
                                   "id2");
...
TEST_P(StreamJoinQueryExecutionTest,
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    bool operator==(const ResultRecord& rhs) const {
      return test1test2$start == rhs.test1test2$start &&
             test1test2$end == rhs.test1test2$end &&
             test1$key == rhs.test1$key && test1$values == rhs.test1$values &&
             test1$values2 == rhs.test1$values2 &&
             test1$timestamp == rhs.test1$timestamp &&
             test2$timestamp == rhs.test2$timestamp &&
             test2$end == rhs.test2$end && test2$key == rhs.test2$key &&
             test2$count == rhs.test2$count;
    }
  };
  const auto leftSchema = TestSchemas::getSchemaTemplate("id_2val_time_u64")
                              ->updateSourceName(*srcName);

  const auto rightSchema = TestSchemas::getSchemaTemplate("id_2val_time_u64")
                               ->updateSourceName(*srcName);
  ...
  // Getting the expected output tuples
  auto bufferManager = executionEngine->getBufferManager();
  const auto expectedSinkBuffer = TestUtils::createExpectedBuffersFromCsv(
      csvFileParams.expectedFile, sinkSchema, bufferManager)[0];
  const auto expectedSinkVector =
      TestUtils::createVecFromTupleBuffer<ResultRecord>(expectedSinkBuffer);

  // Creating the sink and the sources
  const auto testSink =
      executionEngine->createCollectSink<ResultRecord>(sinkSchema);
  const auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  const auto testSourceDescriptorLeft =
      executionEngine->createDataSource(joinParams.inputSchemas[0]);
  ...
  auto query =
      TestQuery::from(testSourceDescriptorLeft)
          .joinWith(
              TestQuery::from(testSourceDescriptorRight)
  ...
  const auto [resultRecords, qp] =
      runQueryWithCsvFilesWithoutStoppingQuery<ResultRecord>(
          {{joinParams.inputSchemas[0], csvFileParams.inputCsvFiles[0]},
           {joinParams.inputSchemas[1], csvFileParams.inputCsvFiles[1]}},
          expectedSinkVector.size(), testSink, query);
  ...
  EXPECT_TRUE(
      executionEngine->stopQuery(qp, Runtime::QueryTerminationType::Graceful));
  ...
  struct __attribute__((packed)) ResultRecord {
    ...
    bool operator==(const ResultRecord& rhs) const {
      return test1test2$start == rhs.test1test2$start &&
             ...
             test1$f2_left == rhs.test1$f2_left &&
             test1$fieldForSum1 == rhs.test1$fieldForSum1 &&
...
INSTANTIATE_TEST_CASE_P(
    testStreamJoinQueries, StreamJoinQueryExecutionTest,
    JOIN_STRATEGIES_WINDOW_STRATEGIES,
    [](const testing::TestParamInfo<StreamJoinQueryExecutionTest::ParamType>&
           info) {
