...
class HyperLogLogBuildExecutionTest
    : public Testing::BaseUnitTest,
      public ::testing::WithParamInterface<
          std::tuple<uint64_t, uint64_t, Statistic::StatisticDataCodec>> {
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("HyperLogLogBuildExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO(
        "QueryExecutionTest: Setup HyperLogLogBuildExecutionTest test class.");
  }
  ...
  void SetUp() override {
    ...
    const auto numWorkerThreads =
        std::get<0>(HyperLogLogBuildExecutionTest::GetParam());
    ...
    executionEngine = std::make_shared<Testing::TestExecutionEngine>(
        queryCompilerDumpMode, numWorkerThreads);
    ...
    statisticFormat = Statistic::StatisticFormatFactory::createFromSchema(
        outputSchema, executionEngine->getBufferManager()->getBufferSize(),
        Statistic::StatisticSynopsisType::HLL, statisticDataCodec);
  ...
  void TearDown() override {
    NES_INFO("QueryExecutionTest: Tear down HyperLogLogBuildExecutionTest test "
             "case.");
  ...
  /**
   * @brief Runs the statistic query and checks if the created hyperloglog
   * sketches are correct by creating hyperloglog sketches on our own and then
   ...
   */
  void runQueryAndCheckCorrectness(
      SourceDescriptorPtr testSourceDescriptor,
    ...
    // Creating the query
    auto window =
        SlidingWindow::of(timeCharacteristic, Milliseconds(windowSize),
    ...
    auto decomposedQueryPlan = DecomposedQueryPlan::create(
        DecomposedQueryPlanId(defaultDecomposedQueryPlanId),
        SharedQueryId(defaultSharedQueryId), INVALID_WORKER_NODE_ID,
        query.getQueryPlan()->getRootOperators());
    auto plan = executionEngine->submitQuery(decomposedQueryPlan);

    // Emitting the input buffers and creating the expected hyperloglog sketches
    // in testStatisticStore
    auto source = executionEngine->getDataSource(plan, 0);
    for (auto buf : allInputBuffers) {
      // We call here emit buffer without changing the metadata of the buffer,
      // due to as setting it in Util::createDataForOneFieldAndTimeStamp()
      source->emitBuffer(buf, false);

      // Now creating the expected hyperloglog sketches in testStatisticStore
      auto dynamicBuffer =
          MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(buf,
                                                                inputSchema);
      Util::updateTestHyperLogLogStatistic(
          dynamicBuffer, testStatisticStore, metricHash, windowSize,
          windowSlide, sketchWidth, fieldToBuildHyperLogLogOver,
          timestampFieldName);
    }

    // Giving the execution engine time to process the tuples, so that we do not
    // just test our terminate() implementation
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // Stopping query and waiting until the test sink has received the expected
    // number of tuples
    NES_INFO("Stopping query now!!!");
    EXPECT_TRUE(executionEngine->stopQuery(
        plan, Runtime::QueryTerminationType::Graceful));
    const auto nodeEngine = executionEngine->getNodeEngine();
    const auto nodeEngineStatisticStore =
        nodeEngine->getStatisticManager()->getStatisticStore();

    while (nodeEngineStatisticStore->getAllStatistics().size() !=
           testStatisticStore->getAllStatistics().size()) {
      NES_DEBUG("Waiting till all statistics have been built. Currently {} and "
                "expecting {}...",
    ...
    for (auto &[statisticHash, hyperLogLogStatistic] :
         nodeEngineStatisticStore->getAllStatistics()) {
      NES_DEBUG("Searching for statisticHash = {} hyperLogLogStatistic = {}",
                statisticHash, hyperLogLogStatistic->toString());
      auto expectedStatistics = testStatisticStore->getStatistics(
          statisticHash, hyperLogLogStatistic->getStartTs(),
          hyperLogLogStatistic->getEndTs());
      EXPECT_EQ(expectedStatistics.size(), 1);
      NES_DEBUG("Found for statisticHash = {} expectedStatistics = {}",
                statisticHash, expectedStatistics[0]->toString());
...
/**
 * @brief Here we test, if we create a hyperloglog sketch for a single input
 * tuple
 */
TEST_P(HyperLogLogBuildExecutionTest, singleInputTuple) {
  ...
  auto allInputBuffers = Util::createDataForOneFieldAndTimeStamp(
      numberOfTuples, *executionEngine->getBufferManager(), inputSchema,
      fieldToBuildHyperLogLogOver, timestampFieldName, isIngestionTime);

  // Creating the sink and the sources
  const auto testSinkDescriptor = StatisticSinkDescriptor::create(
      StatisticSynopsisType::HLL, statisticDataCodec);
  const auto testSourceDescriptor =
      executionEngine->createDataSource(inputSchema);

  // Creating the hyperloglog descriptor and running the query
  auto hyperLogLogDescriptor = HyperLogLogDescriptor::create(
      Over(fieldToBuildHyperLogLogOver), sketchWidth);
  runQueryAndCheckCorrectness(testSourceDescriptor, testSinkDescriptor,
                              hyperLogLogDescriptor, windowSize, windowSize,
                              timeCharacteristic, allInputBuffers);
}

/**
 * @brief Here we test, if we create multiple hyperloglog sketches for multiple
 * input buffers, but also for larger sketches
 */
...
/**
 * @brief Here we test, if we create multiple hyperloglog sketches for multiple
 * input buffers, but also for larger sketches and for sliding windows (we
 * create one sketch per slice)
 */
TEST_P(HyperLogLogBuildExecutionTest, multipleInputBuffersSlidingWindow) {
  ...
  auto hyperLogLogDescriptor = HyperLogLogDescriptor::create(
      Over(fieldToBuildHyperLogLogOver), sketchWidth);
  runQueryAndCheckCorrectness(testSourceDescriptor, testSinkDescriptor,
                              hyperLogLogDescriptor, windowSize, windowSlide,
                              timeCharacteristic, allInputBuffers);
}

/**
 * @brief Here we test, if we create multiple hyperloglog sketches for multiple
 * input buffers, but also for larger sketches The difference is that we use the
 * ingestion time instead of an event time
 */
...
INSTANTIATE_TEST_CASE_P(
    testHyperLogLog, HyperLogLogBuildExecutionTest,
    ::testing::Combine(
        ::testing::Values(1, 4, 8),         // numWorkerThread
    ...
    [](const testing::TestParamInfo<HyperLogLogBuildExecutionTest::ParamType>
           &info) {
