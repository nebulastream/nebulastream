...
class MapQueryExecutionTest
    : public Testing::BaseUnitTest,
      public ::testing::WithParamInterface<
          std::tuple<QueryCompilation::QueryCompilerType, std::string,
                     std::vector<string>, std::vector<string>, int>> {
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MapQueryExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG(
        "QueryCatalogServiceTest: Setup QueryCatalogServiceTest test class.");
  }
  ...
  void TearDown() override {
    NES_DEBUG("QueryCatalogServiceTest: Tear down QueryCatalogServiceTest test "
              "case.");
  ...
  // The test data is a four-tuple which contains the nautilus compiler, the
  // name of the test, the field names for the result schema, the names for the
  // query and the sign int for the input buffer.
  static auto createMapQueryArithmeticTestData() {
    return std::make_tuple(
        QueryCompilation::QueryCompilerType::NAUTILUS_QUERY_COMPILER,
        "MapQueryArithmetic", std::vector<string>{"test$id", "test$one"},
        std::vector<string>{"id"}, 1);
  }
  static auto createLogTestData() {
    return std::make_tuple(
        ...
        std::vector<string>{"log10", "log2", "ln"}, 1);
  }
  static auto createTwoMapQueryTestData() {
    return std::make_tuple(
        QueryCompilation::QueryCompilerType::NAUTILUS_QUERY_COMPILER,
        "TwoMapQuery", std::vector<string>{"test$id", "test$new1", "test$new2"},
        std::vector<string>{"test$new1", "test$new2"}, 1);
  }
  static auto createAbsTestData() {
    return std::make_tuple(
        QueryCompilation::QueryCompilerType::NAUTILUS_QUERY_COMPILER,
        "MapAbsFunction", std::vector<string>{"test$id", "test$abs"},
        std::vector<string>{"abs"}, -1);
  }
  static auto createTrigTestData() {
    return std::make_tuple(
        ...
        std::vector<string>{"sin", "cos", "radians"}, 1);
  }
  static auto createPowerTestData() {
    return std::make_tuple(
        ...
        std::vector<string>{"left$id", "right$id", "power"}, 1);
  }
};

static auto getExpression(
    const std::string expression) { // Includes the names for the query
...
getFunction(const std::string function,
            int input) { // Includes the names for the EXPECT_EQ statement
...
TEST_P(MapQueryExecutionTest, MapAllFunctions) {
  ...
  auto resultSchema =
      Schema::create()->addField(resultArray[0], BasicType::FLOAT64);
  for (uint32_t index = 1; index < resultArray.size(); index++) {
    resultSchema =
        resultSchema->addField(resultArray[index], BasicType::FLOAT64);
  }

  auto testSink =
      executionEngine->createDataSink(resultSchema, expectedNumberOfTuples);
  ...
  for (uint32_t index = 0; index < queryArray.size(); index++) {
    query = query.map(Attribute(queryArray[index]) =
                          getExpression(queryArray[index]));
  }
  ...
  if (testCase == "MapQueryArithmetic") {
    ...
    for (uint32_t recordIndex = 0u; recordIndex < expectedNumberOfTuples;
         ++recordIndex) {
    ...
    for (int recordIndex = 0; recordIndex < 10; recordIndex++) {
      inputBuffer[recordIndex]["test$left$id"].write<double>((double)2 * sign *
                                                             recordIndex);
      inputBuffer[recordIndex]["test$right$id"].write<double>((double)sign *
                                                              recordIndex);
    }
    ...
    for (uint32_t recordIndex = 0u; recordIndex < expectedNumberOfTuples;
         ++recordIndex) {
      EXPECT_EQ(resultBuffer[recordIndex][resultArray[2]].read<double>(),
                getFunction(resultArray[2], sign * recordIndex));
    }
  } else {
    for (int recordIndex = 0; recordIndex < 10; recordIndex++) {
...
INSTANTIATE_TEST_CASE_P(
    testMapQueries, MapQueryExecutionTest,
