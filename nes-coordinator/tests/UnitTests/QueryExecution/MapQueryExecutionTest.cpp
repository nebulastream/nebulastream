...
static auto getExpression(const std::string expression)
{ // Includes the names for the query
    if (expression == "id")
    { // MapQueryArithmetic
        return Attribute("id") * 2;
    }
    else if (expression == "log10")
    { // MapLogarithmicFunctions
...
static auto getFunction(const std::string function, int input)
{ // Includes the names for the EXPECT_EQ statement
    if (function == "test$one")
    { // MapQueryArithmetic
        return (double)input * 2;
    }
    else if (function == "test$log10")
    { // MapLogarithmicFunctions
    ...
    else if (function == "test$abs")
    { // MapAbsFunction
        return std::fabs(input);
    }
    else if (function == "test$sin")
    { // MapTrigonometricFunctions
...
TEST_P(MapQueryExecutionTest, MapAllFunctions)
{
    ...
    {
        query = TestQuery::from(testSourceDescriptor).map(Attribute(queryArray[2]) = getExpression(queryArray[2])).sink(testSinkDescriptor);
    }
    ...
    else
    {
        ...
        for (uint32_t recordIndex = 0u; recordIndex < expectedNumberOfTuples; ++recordIndex)
        {
            ...
            {
                EXPECT_EQ(
                    resultBuffer[recordIndex][resultArray[index]].read<double>(), getFunction(resultArray[index], sign * recordIndex));
            }
        }
    }
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        MapQueryExecutionTest::createMapQueryArithmeticTestData(),
