...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedTumblingWindowQueryExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG("QueryExecutionTest: Setup KeyedTumblingWindowQueryExecutionTest "
              "test class.");
  }
  ...
  void TearDown() override {
    NES_DEBUG("QueryExecutionTest: Tear down "
              "KeyedTumblingWindowQueryExecutionTest test case.");
  ...
  static void TearDownTestCase() {
    NES_DEBUG("QueryExecutionTest: Tear down "
              "KeyedTumblingWindowQueryExecutionTest test class.");
  }
...
TEST_F(KeyedTumblingWindowQueryExecutionTest, singleKeyTumblingWindow) {
  ...
  auto testSink =
      executionEngine->createDataSink(sinkSchema, expectedNumberOfTuples);

  auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(TumblingWindow::of(EventTime(Attribute("test$ts")),
                                              Milliseconds(5)))
                   .byKey(Attribute("test$key", BasicType::INT64))
                   .apply(Sum(Attribute("test$value", BasicType::INT64))
...
TEST_F(KeyedTumblingWindowQueryExecutionTest, multiKeyTumblingWindow) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(TumblingWindow::of(EventTime(Attribute("test$ts")),
                                              Milliseconds(5)))
                   .byKey(Attribute("test$key", BasicType::INT64),
                          Attribute("test$key", BasicType::INT64))
                   .apply(Sum(Attribute("test$value", BasicType::INT64))
                              ->as(Attribute("test$sum")))
                   .sink(testSinkDescriptor);
