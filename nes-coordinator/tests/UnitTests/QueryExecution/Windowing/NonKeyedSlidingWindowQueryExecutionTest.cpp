...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging(
        "NonKeyedTumblingWindowQueryExecutionTest.cpp.log",
        NES::LogLevel::LOG_DEBUG);
    NES_DEBUG("QueryExecutionTest: Setup "
              "NonKeyedTumblingWindowQueryExecutionTest.cpp test class.");
  }
  ...
  void SetUp() override {
    ...
    executionEngine = std::make_shared<Testing::TestExecutionEngine>(
        dumpMode, 1, QueryCompilation::StreamJoinStrategy::HASH_JOIN_LOCAL,
        windowStrategy);
  }

  /* Will be called before a test is executed. */
  void TearDown() override {
    NES_DEBUG("QueryExecutionTest: Tear down "
              "NonKeyedTumblingWindowQueryExecutionTest.cpp test case.");
...
TEST_P(NonKeyedSlidingWindowQueryExecutionTest, testSimpleSlidingWindow) {
  auto sourceSchema = Schema::create()
                          ->addField("test$f1", BasicType::UINT64)
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(SlidingWindow::of(EventTime(Attribute("test$f1")),
...
INSTANTIATE_TEST_CASE_P(
    testNonKeyedSlidingWindow, NonKeyedSlidingWindowQueryExecutionTest,
    ::testing::Values(QueryCompilation::WindowingStrategy::SLICING,
                      QueryCompilation::WindowingStrategy::BUCKETING),
    [](const testing::TestParamInfo<
        NonKeyedSlidingWindowQueryExecutionTest::ParamType> &info) {
