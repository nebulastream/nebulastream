...
class KeyedSlidingWindowQueryExecutionTest
 ...
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("KeyedSlidingWindowQueryExecutionTest.cpp.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG(
        "QueryExecutionTest: Setup KeyedSlidingWindowQueryExecutionTest.cpp "
        "test class.");
  }
  ...
  void TearDown() override {
    NES_DEBUG(
        ...
        "KeyedSlidingWindowQueryExecutionTest.cpp test case.");
  ...
  static void TearDownTestCase() {
    NES_DEBUG(
        "QueryExecutionTest: Tear down "
        "KeyedSlidingWindowQueryExecutionTest.cpp test class.");
  }
...
void createExpectedBuffer(Runtime::MemoryLayouts::TestTupleBuffer& buf) {
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(0, 10, 0, 4));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(0, 10, 1, 3));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(0, 10, 2, 3));

  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(5, 15, 0, 3));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(5, 15, 1, 3));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(5, 15, 2, 4));

  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(10, 20, 0, 3));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(10, 20, 1, 4));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(10, 20, 2, 3));

  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(15, 25, 0, 4));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(15, 25, 1, 3));
  buf.pushRecordToBuffer(
      std::tuple<uint64_t, uint64_t, int64_t, int64_t>(15, 25, 2, 3));
}
...
TEST_P(KeyedSlidingWindowQueryExecutionTest, testKeyedSlidingWindow) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(SlidingWindow::of(EventTime(Attribute("test$ts")),
  ...
  for (auto i = 0_u64; i < testSink->getNumberOfResultBuffers(); ++i) {
    ...
    NES_INFO("Buffer: {}", NES::Runtime::Execution::Util::printTupleBufferAsCSV(
                               resultBuffer.getBuffer(), sinkSchema));
  }

  EXPECT_TRUE(
      TestUtils::buffersContainSameTuples(expectedBuffers, resultBuffers));
...
INSTANTIATE_TEST_CASE_P(
    testNonKeyedSlidingWindow, KeyedSlidingWindowQueryExecutionTest,
    ::testing::Values(QueryCompilation::WindowingStrategy::SLICING,
                      QueryCompilation::WindowingStrategy::BUCKETING),
    [](const testing::TestParamInfo<
        KeyedSlidingWindowQueryExecutionTest::ParamType>& info) {
