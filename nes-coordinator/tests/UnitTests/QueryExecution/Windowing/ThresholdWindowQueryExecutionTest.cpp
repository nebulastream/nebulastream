...
class ThresholdWindowQueryExecutionTest
    : public Testing::BaseUnitTest,
      public ::testing::WithParamInterface<
          QueryCompilation::QueryCompilerType> {
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("ThresholdWindowQueryExecutionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG("QueryExecutionTest: Setup ThresholdWindowQueryExecutionTest "
              "test class.");
  }
  ...
  void TearDown() override {
    NES_DEBUG("QueryExecutionTest: Tear down ThresholdWindowQueryExecutionTest "
              "test case.");
...
void fillBuffer(Runtime::MemoryLayouts::TestTupleBuffer &buf) {
  ...
  // The following buffer closes the window in the query in all test cases in
  // this file It's second field is 0 and our Threshold window query always
...
/**
 * Test the execution of a query with threshold window operator and apply a Sum
 * aggregation.
 */
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestSum) {
  auto sourceSchema = Schema::create()
                          ->addField("test$f1", BasicType::INT64)
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   ...
                   .apply(Sum(Attribute("test$f2", BasicType::INT64))
                              ->as(Attribute("test$Sum")))
...
/**
 * Test the execution of a query with threshold window operator and apply a Max
 * aggregation.
 */
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestWithMax) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(ThresholdWindow::of(Attribute("test$f1") > 5))
                   .apply(Max(Attribute("test$f2", BasicType::INT64))
                              ->as(Attribute("test$Max")))
                   .sink(testSinkDescriptor);
...
/**
 * Test the execution of a query with threshold window operator and apply a Min
 * aggregation.
 */
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestWithMin) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(ThresholdWindow::of(Attribute("test$f1") > 5))
                   .apply(Min(Attribute("test$f2", BasicType::INT64))
                              ->as(Attribute("test$Min")))
                   .sink(testSinkDescriptor);
...
/**
 * Test the execution of a query with threshold window operator and apply a
 * Average aggregation.
 */
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestWithAvg) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   ...
                   .apply(Avg(Attribute("test$f2", BasicType::INT64))
                              ->as(Attribute("test$Avg")))
                   .sink(testSinkDescriptor);
...
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestSumFloat) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(ThresholdWindow::of(Attribute("test$f1") > 5))
                   .apply(Sum(Attribute("test$f2", BasicType::FLOAT32))
                              ->as(Attribute("test$Sum")))
                   .sink(testSinkDescriptor);
...
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestSumInt32) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(ThresholdWindow::of(Attribute("test$f1") > 5))
                   .apply(Sum(Attribute("test$f2", BasicType::INT32))
                              ->as(Attribute("test$Sum")))
                   .sink(testSinkDescriptor);
...
TEST_F(ThresholdWindowQueryExecutionTest, simpleThresholdWindowTestSumDouble) {
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(ThresholdWindow::of(Attribute("test$f1") > 5))
                   .apply(Sum(Attribute("test$f2", BasicType::FLOAT64))
                              ->as(Attribute("test$Sum")))
                   .sink(testSinkDescriptor);
...
/**
 * Test the execution of a query with threshold window operator and apply a
 * Count aggregation in combination with a sum aggregation. This test checks the
 * behaviour of multi aggs incl. count aggregations as count agg do not have an
 * access field
 */
TEST_F(ThresholdWindowQueryExecutionTest,
  ...
  auto query = TestQuery::from(testSourceDescriptor)
                   .window(ThresholdWindow::of(Attribute("test$f1") > 5))
                   .apply(Count()->as(Attribute("test$Count")),
                          Sum(Attribute("test$f2", BasicType::INT64))
                              ->as(Attribute("test$Sum")))
                   .sink(testSinkDescriptor);
