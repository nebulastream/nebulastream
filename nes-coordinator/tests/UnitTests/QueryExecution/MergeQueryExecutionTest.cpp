...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MergeQueryExecutionTest.log",
  ...
  void TearDown() override {
    NES_DEBUG(
        "QueryExecutionTest: Tear down MergeQueryExecutionTest test case.");
  ...
  static void TearDownTestCase() {
    NES_DEBUG(
        "QueryExecutionTest: Tear down MergeQueryExecutionTest test class.");
  }
...
TEST_F(MergeQueryExecutionTest, mergeQuery) {
  ...
  auto testSink =
      executionEngine->createDataSink(result, /*expected tuples*/ 10);
  ...
  // creating P2
  auto query2 =
      TestQuery::from(testSourceDescriptor).filter(Attribute("id") < 5);
  auto mergedQuery = query2.unionWith(query1)
  ...
  auto decomposedQueryPlan = DecomposedQueryPlan::create(
      ...
      INVALID_WORKER_NODE_ID, mergedQuery.getQueryPlan()->getRootOperators());
  ...
  for (uint64_t i = 0; i < testSink->getNumberOfResultBuffers(); i++) {
    ...
    for (uint32_t recordIndex = 0u;
         recordIndex < resultBuffer.getNumberOfTuples(); ++recordIndex) {
