...
public:
  ...
  void SetUp() override {
    ...
    defaultSchema = Schema::create()
                        ->addField("test$id", BasicType::INT64)
                        ->addField("test$one", BasicType::INT64);
    defaultDataGenerator = [](Runtime::MemoryLayouts::TestTupleBuffer &buffer,
                              uint64_t numInputRecords) {
    ...
    defaultSink =
        executionEngine->createCollectSink<DefaultRecord>(defaultSchema);
    defaultTestSinkDescriptor =
        std::make_shared<TestUtils::TestSinkDescriptor>(defaultSink);
  }
...
TEST_F(UnionQueryExecutionTest, unionOperatorWithFilterOnUnionResult) {
  ...
  generateAndEmitInputBuffers(plan, {defaultSchema, defaultSchema},
                              {defaultDataGenerator, defaultDataGenerator},
                              numInputRecords);
  defaultSink->waitTillCompletedOrTimeout(numResultRecords,
...
TEST_F(UnionQueryExecutionTest, unionOperatorWithFilterOnSources) {
  ...
  // Define query plan.
  Query subQuery =
      TestQuery::from(defaultSource).filter(Attribute("test$id") > 3);
  Query query = TestQuery::from(defaultSource)
  ...
  EXPECT_EQ(resultRecords.size(), numResultRecords);
  for (size_t recordIdx = 0; recordIdx < resultRecords.size(); ++recordIdx) {
    EXPECT_EQ(resultRecords.at(recordIdx).id,
              (recordIdx + 4) % 6 + 4); // result ids: 8,9,4,5,6,7,8,9
    EXPECT_EQ(resultRecords.at(recordIdx).value, 1);
  }
...
TEST_F(UnionQueryExecutionTest,
  ...
  auto customSchema = Schema::create()
                          ->addField("custom$id", BasicType::INT64)
                          ->addField("custom$one", BasicType::INT64);
  ...
  Query query =
      TestQuery::from(defaultSource)
          .unionWith(TestQuery::from(customSource)
                         .project(Attribute("custom$id").as("test$id"),
                                  Attribute("custom$one").as("test$one")))
          .sink(defaultTestSinkDescriptor);
  auto decomposedQueryPlan = DecomposedQueryPlan::create(
      defaultDecomposedQueryPlanId, defaultSharedQueryId,
      INVALID_WORKER_NODE_ID, query.getQueryPlan()->getRootOperators());
  ...
  // Generate input and run query.
  generateAndEmitInputBuffers(plan, {defaultSchema, customSchema},
                              {defaultDataGenerator, defaultDataGenerator},
                              numInputRecords);
  defaultSink->waitTillCompletedOrTimeout(numResultRecords,
                                          defaultTimeoutInMilliseconds);
