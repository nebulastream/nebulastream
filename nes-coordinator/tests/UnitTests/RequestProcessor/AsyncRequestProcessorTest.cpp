...
{
public:
    DummyConcatRequest(const std::vector<ResourceType>& requiredResources, uint8_t maxRetries, uint32_t responseValue, uint32_t min)
...
public:
    DummyWaitOnFutureMultiRequest(
        uint8_t maxRetries, std::vector<std::pair<std::future<bool>, std::shared_ptr<AbstractSubRequest>>>& listOfResourceLists)
...
public:
    AsyncRequestProcessorPtr getProcessor(uint16_t numThreads)
    {
        ...
        StorageDataStructures storageDataStructures = {coordinatorConfig, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};
...
TEST_F(AsyncRequestProcessorTest, testWaitingForLock)
{
    ...
    try
    {
        ...
        auto request1
            = std::make_shared<DummyWaitOnFutureRequest>(std::vector<ResourceType>{ResourceType::Topology}, 0, promise1.get_future());

        std::promise<bool> promise2;
        auto request2 = std::make_shared<DummyWaitOnFutureRequest>(
            std::vector<ResourceType>{ResourceType::QueryCatalogService}, 0, promise2.get_future());

        std::promise<bool> promise3;
        auto request3 = std::make_shared<DummyWaitOnFutureRequest>(
            std::vector<ResourceType>{ResourceType::QueryCatalogService, ResourceType::Topology}, 0, promise3.get_future());

        std::promise<bool> promise4;
        auto request4
            = std::make_shared<DummyWaitOnFutureRequest>(std::vector<ResourceType>{ResourceType::Topology}, 0, promise4.get_future());

        std::promise<bool> promise5;
        auto request5 = std::make_shared<DummyWaitOnFutureRequest>(
            std::vector<ResourceType>{ResourceType::QueryCatalogService}, 0, promise5.get_future());
        ...
        ASSERT_EQ(twoplhandler->getNextAvailableTicket(ResourceType::QueryCatalogService), nextAvailableTicketQueryCatalogService);
        ...
        nextAvailableTicketQueryCatalogService++;
        start_timestamp = std::chrono::system_clock::now();
        while ((int)twoplhandler->getNextAvailableTicket(ResourceType::QueryCatalogService) < nextAvailableTicketQueryCatalogService)
        {
            if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec)
            {
                FAIL();
...
TEST_F(AsyncRequestProcessorTest, testWaitingForLockMultiRequest)
{
    ...
    try
    {
        ...
        auto request3 = std::make_shared<DummyWaitOnFutureSubRequest>(
            std::vector<ResourceType>{ResourceType::QueryCatalogService, ResourceType::Topology}, promise3.get_future());
        std::pair pair3 = {runPromise3.get_future(), request3};

        std::promise<bool> promise4;
        std::promise<bool> runPromise4;
        auto request4
            = std::make_shared<DummyWaitOnFutureSubRequest>(std::vector<ResourceType>{ResourceType::Topology}, promise4.get_future());
        ...
        auto request5 = std::make_shared<DummyWaitOnFutureSubRequest>(
            std::vector<ResourceType>{ResourceType::QueryCatalogService}, promise5.get_future());
