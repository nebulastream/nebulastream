...
/**
 * The dummy concat request is used for testing the creation of follow up
 * requests. It will create a new request for all v with responseValue > v >
 * min. So executing a request with the parameters responseValue = 12 and min =
 * 10 will cause the following request to be executed:
 *
 *              --- (10,10)
 *            /
 * (12,10) ---
...
public:
  DummyConcatRequest(const std::vector<ResourceType> &requiredResources,
                     uint8_t maxRetries, uint32_t responseValue, uint32_t min)
      : AbstractUniRequest(requiredResources, maxRetries),
        responseValue(responseValue), min(min){};
  ...
  executeRequestLogic(const StorageHandlerPtr &) override {
    ...
    for (uint32_t i = responseValue - 1; i >= min; --i) {
      auto newRequest = std::make_shared<DummyConcatRequest>(
          std::vector<ResourceType>{}, 0, i, min);
...
public:
  explicit DummyWaitOnFutureResponse(StorageHandlerPtr storageHandler)
      : storageHandler(std::move(storageHandler)) {}
  StorageHandlerPtr storageHandler;
};

class DummyWaitOnFutureRequest : public AbstractUniRequest {
public:
  DummyWaitOnFutureRequest(const std::vector<ResourceType> &requiredResources,
                           uint8_t maxRetries, std::future<bool> future)
      : AbstractUniRequest(requiredResources, maxRetries),
        future(std::move(future)){};

  std::vector<AbstractRequestPtr>
  executeRequestLogic(const StorageHandlerPtr &storageHandler) override {
    if (future.get()) {
      auto response =
          std::make_shared<DummyWaitOnFutureResponse>(storageHandler);
...
public:
  DummyWaitOnFutureMultiRequest(
      uint8_t maxRetries,
      std::vector<
          std::pair<std::future<bool>, std::shared_ptr<AbstractSubRequest>>>
          &listOfResourceLists)
      : AbstractMultiRequest(maxRetries),
        listOfResourceLists(listOfResourceLists){};
  ...
  std::vector<AbstractRequestPtr> executeRequestLogic() override {
    ...
    for (auto &f : responseFutures) {
      response =
          std::any_cast<std::shared_ptr<DummyWaitOnFutureResponse>>(f.get());
    }
    responsePromise.set_value(response);
    return {};
  }
...
private:
  std::vector<std::pair<std::future<bool>, std::shared_ptr<AbstractSubRequest>>>
      &listOfResourceLists;
};
...
public:
  DummySubRequest(std::atomic<uint32_t> &additionTarget,
                  uint32_t returnNewRequestFrequency)
      : AbstractSubRequest({}), additionTarget(additionTarget),
...
public:
  DummyMultiRequest(uint8_t maxRetries, uint32_t initialValue,
                    uint32_t additionValue, uint32_t returnNewRequestFrequency)
      : AbstractMultiRequest(maxRetries), responseValue(initialValue),
  ...
  std::vector<AbstractRequestPtr> executeRequestLogic() override {
    ...
    for (uint32_t i = 0; i < additionValue; ++i) {
      futures.push_back(scheduleSubRequest(std::make_shared<DummySubRequest>(
          responseValue, returnNewRequestFrequency)));
    }
    ...
    responsePromise.set_value(
        std::make_shared<DummyMultiResponse>(responseValue));
    return {};
  }
...
class AsyncRequestProcessorTest : public Testing::BaseUnitTest,
                                  public testing::WithParamInterface<int> {
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("AsyncRequestProcessorTest.log",
...
public:
  AsyncRequestProcessorPtr getProcessor(uint16_t numThreads) {
    ...
    StorageDataStructures storageDataStructures = {
        coordinatorConfig, nullptr, nullptr, nullptr, nullptr,
...
TEST_P(AsyncRequestProcessorTest, startAndDestroy) {
  ...
  auto request = std::make_shared<DummyConcatRequest>(
      std::vector<ResourceType>{}, 0, 10, 10);
...
TEST_F(AsyncRequestProcessorTest, testWaitingForLock) {
  ...
  try {
    // using the same value for response value and min value will create a
    // single request with no follow up requests
    std::promise<bool> promise1;
    auto request1 = std::make_shared<DummyWaitOnFutureRequest>(
        std::vector<ResourceType>{ResourceType::Topology}, 0,
        promise1.get_future());

    std::promise<bool> promise2;
    auto request2 = std::make_shared<DummyWaitOnFutureRequest>(
        std::vector<ResourceType>{ResourceType::QueryCatalogService}, 0,
        promise2.get_future());

    std::promise<bool> promise3;
    auto request3 = std::make_shared<DummyWaitOnFutureRequest>(
        std::vector<ResourceType>{ResourceType::QueryCatalogService,
                                  ResourceType::Topology},
        0, promise3.get_future());

    std::promise<bool> promise4;
    auto request4 = std::make_shared<DummyWaitOnFutureRequest>(
        std::vector<ResourceType>{ResourceType::Topology}, 0,
        promise4.get_future());

    std::promise<bool> promise5;
    auto request5 = std::make_shared<DummyWaitOnFutureRequest>(
        std::vector<ResourceType>{ResourceType::QueryCatalogService}, 0,
        promise5.get_future());

    // get 2pl handler
    std::promise<bool> handlerGetterPromise;
    auto handlerGetterRequest = std::make_shared<DummyWaitOnFutureRequest>(
        std::vector<ResourceType>{}, 0, handlerGetterPromise.get_future());
    ...
    auto twoplhandler =
        std::dynamic_pointer_cast<TwoPhaseLockingStorageHandler>(
            std::static_pointer_cast<DummyWaitOnFutureResponse>(
                handlerGetterResponseFuture.get())
                ->storageHandler);
    ...
    ASSERT_EQ(twoplhandler->getCurrentTicket(ResourceType::Topology),
              currentTicketTopology);
    ASSERT_EQ(twoplhandler->getCurrentTicket(ResourceType::QueryCatalogService),
              currentTicketQueryCatalog);
    ASSERT_EQ(twoplhandler->getNextAvailableTicket(ResourceType::Topology),
              nextAvailableTicketTopology);
    ASSERT_EQ(
        twoplhandler->getNextAvailableTicket(ResourceType::QueryCatalogService),
    ...
    // no additional waiting on query catalog because the request has to acquire
    // topology before trying ot acquire the query catalog
    nextAvailableTicketTopology++;

    auto timeoutInSec = std::chrono::seconds(TestUtils::defaultTimeout);
    auto start_timestamp = std::chrono::system_clock::now();
    while (twoplhandler->getNextAvailableTicket(ResourceType::Topology) <
           nextAvailableTicketTopology) {
    ...
    while ((int)twoplhandler->getNextAvailableTicket(ResourceType::Topology) <
           nextAvailableTicketTopology) {
      if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec) {
        FAIL();
      }
    ...
    nextAvailableTicketQueryCatalogService++;
    start_timestamp = std::chrono::system_clock::now();
    while ((int)twoplhandler->getNextAvailableTicket(
    ...
    future1.get();
    // because one lock on the topology gets released, request 3 can qcquire it
    // and will then start waiting on the query catalog as well
    nextAvailableTicketQueryCatalogService++;
    // when request one gets executed, the current ticket for the topology will
    // increase eventually
    currentTicketTopology++;
    start_timestamp = std::chrono::system_clock::now();
    while ((int)twoplhandler->getCurrentTicket(ResourceType::Topology) <
           currentTicketTopology) {
      if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec) {
        FAIL();
      }
    ...
    future2.get();
    // when request two gets executed, the current ticket for the query catalog
    // will increase eventually
    currentTicketQueryCatalog++;
    start_timestamp = std::chrono::system_clock::now();
    while ((int)twoplhandler->getCurrentTicket(
               ResourceType::QueryCatalogService) < currentTicketQueryCatalog) {
      if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec) {
        FAIL();
      }
    }
    ...
    // for threads >= 5 nothing changes, request 3 is still waiting on the query
    // catalog

    // if there is just one thread, this will increase the ticket of the
    // topology
    while ((int)twoplhandler->getNextAvailableTicket(ResourceType::Topology) <
           nextAvailableTicketTopology) {
      if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec) {
    ...
    // nothing changes, request 4 is waiting on toplogy which is still held by
    // request 3 which is waiting on query catalog
    while ((int)twoplhandler->getNextAvailableTicket(ResourceType::Topology) <
           nextAvailableTicketTopology) {
      if (std::chrono::system_clock::now() > start_timestamp + timeoutInSec) {
        FAIL();
...
TEST_F(AsyncRequestProcessorTest, testWaitingForLockMultiRequest) {
  ...
  try {
    ...
    auto request4 = std::make_shared<DummyWaitOnFutureSubRequest>(
        std::vector<ResourceType>{ResourceType::Topology},
        promise4.get_future());
    std::pair pair4 = {runPromise4.get_future(), request4};

    std::promise<bool> promise5;
    std::promise<bool> runPromise5;
    auto request5 = std::make_shared<DummyWaitOnFutureSubRequest>(
        std::vector<ResourceType>{ResourceType::QueryCatalogService},
        promise5.get_future());
    ...
    auto multiRequest =
        std::make_shared<DummyWaitOnFutureMultiRequest>(0, pairs);
...
TEST_P(AsyncRequestProcessorTest, submitRequest) {
  ...
  try {
    ...
    // single request with no follow up requests
    auto request = std::make_shared<DummyConcatRequest>(
        std::vector<ResourceType>{}, 0, responseValue, responseValue);

    auto future = request->getFuture();
    auto queryId = processor->runAsync(request);
    EXPECT_NE(queryId, INVALID_REQUEST_ID);
    EXPECT_EQ(
        std::static_pointer_cast<DummyConcatResponse>(future.get())->number,
        responseValue);
    EXPECT_TRUE(processor->stop());
  } catch (std::exception const &ex) {
    NES_DEBUG("{}", ex.what());
...
TEST_P(AsyncRequestProcessorTest, submitFollowUpRequest) {
  ...
  try {
    auto request = std::make_shared<DummyConcatRequest>(
        std::vector<ResourceType>{}, 0, responseValue, min);
    auto future = request->getFuture();
    auto queryId = processor->runAsync(request);
    EXPECT_NE(queryId, INVALID_REQUEST_ID);
    auto responsePtr =
        std::static_pointer_cast<DummyConcatResponse>(future.get());
    EXPECT_EQ(responsePtr->number, responseValue);

    // check that all follow up requests have been executed and producted the
    // expected result
    ...
    for (auto &f : responsePtr->futures) {
      ...
      if (r->number == responseValue - 1) {
        ...
        auto r2 = std::static_pointer_cast<DummyConcatResponse>(
            r->futures.front().get());
...
TEST_P(AsyncRequestProcessorTest, submitMultiRequest) {
  ...
  try {
    auto request = std::make_shared<DummyMultiRequest>(0, 0, responseValue,
                                                       additionsPerIteration);
    auto future = request->getFuture();
    auto queryId = processor->runAsync(request);
    EXPECT_NE(queryId, INVALID_REQUEST_ID);
    auto responsePtr =
        std::static_pointer_cast<DummyMultiResponse>(future.get());
    EXPECT_EQ(responsePtr->number, responseValue);
  } catch (std::exception const &ex) {
    NES_DEBUG("{}", ex.what());
    FAIL();
...
INSTANTIATE_TEST_CASE_P(AsyncRequestExecutorMTTest, AsyncRequestProcessorTest,
