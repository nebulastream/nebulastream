...
class TopologyNodeRelocationRequestTest : public Testing::BaseUnitTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TopologyNodeRelocationRequestTest.log",
  ...
  void SetUp() override {
    ...
    amendmentQueue = std::make_shared<
...
/**
 * @brief Test the algorithm to identify the upstream and downstream operators
 * used as an input for an incremental placement to be performed due to a
 * topology link removal. Construct a topology, place query subplans and
 * calculate the sets of upstream and downstream operators of an incremental
 * placement.
 *
 */
TEST_F(TopologyNodeRelocationRequestTest,
       testFindingIncrementalUpstreamAndDownstream) {
  const auto globalQueryPlan = GlobalQueryPlan::create();
  auto coordinatorConfig =
      Configurations::CoordinatorConfiguration::createDefault();
  auto optimizerConfiguration = Configurations::OptimizerConfiguration();
  optimizerConfiguration.queryMergerRule =
  ...
  RequestProcessor::StorageDataStructures storageDataStructures(
      coordinatorConfig, topology, globalExecutionPlan, globalQueryPlan,
      queryCatalog, sourceCatalog, udfCatalog, amendmentQueue,
      statisticProbeHandler);

  auto storageHandler =
      RequestProcessor::SerialStorageHandler::create(storageDataStructures);
  ...
  auto pred1 = ConstantValueExpressionNode::create(
  ...
  // root node
  // id = 1
  topology->registerWorker(WorkerId(workerIdCounter), workerAddress, restPort,
                           dataPort, 1, {}, 0, 0);
  ...
  decomposedQueryPlan = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(subPlanId), sharedQueryId, pinnedId);
  auto fileSinkOperatorId = getNextOperatorId();
  auto fileSinkDescriptor =
      FileSinkDescriptor::create(outputFileName, "CSV_FORMAT", "APPEND");
  auto fileSinkOperator = std::make_shared<SinkLogicalOperator>(
      fileSinkDescriptor, fileSinkOperatorId);
  ...
  auto nesPartition = Network::NesPartition(sharedQueryId, networkSourceId,
  ...
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      ...
      WAIT_TIME, EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  sourceLogicalOperator = std::make_shared<SourceLogicalOperator>(
      networkSourceDescriptor, networkSourceId);
  sourceLogicalOperator->addProperty(Optimizer::DOWNSTREAM_LOGICAL_OPERATOR_ID,
                                     OperatorId{1});
  sourceLogicalOperator->addProperty(Optimizer::UPSTREAM_LOGICAL_OPERATOR_ID,
                                     OperatorId{4});
  fileSinkOperator->addChild(sourceLogicalOperator);
  auto lockedTopologyNode = topology->lockTopologyNode(pinnedId);
  globalExecutionPlan->addDecomposedQueryPlan(lockedTopologyNode,
                                              decomposedQueryPlan);
  ...
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(1), workerAddress, dataPort), nesPartition,
      WAIT_TIME, DATA_CHANNEL_RETRY_TIMES, version, DEFAULT_NUMBER_OF_ORIGINS,
      networkSinkId);
  sinkLogicalOperator = std::make_shared<SinkLogicalOperator>(
      networkSinkDescriptor, networkSinkId);
  sinkLogicalOperator->addProperty(Optimizer::DOWNSTREAM_LOGICAL_OPERATOR_ID,
                                   OperatorId{1});
  sinkLogicalOperator->addProperty(Optimizer::UPSTREAM_LOGICAL_OPERATOR_ID,
                                   OperatorId{4});
  decomposedQueryPlan->addRootOperator(sinkLogicalOperator);
  auto unaryOperatorId = getNextOperatorId();
  unaryOperator =
      LogicalOperatorFactory::createFilterOperator(pred1, unaryOperatorId);
  ...
  nesPartition = Network::NesPartition(sharedQueryId, networkSourceId,
                                       PartitionId(0), SubpartitionId(0));
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      schema, nesPartition,
      Network::NodeLocation(WorkerId(3), workerAddress, dataPort), WAIT_TIME,
      EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  sourceLogicalOperator = std::make_shared<SourceLogicalOperator>(
      networkSourceDescriptor, networkSourceId);
  sourceLogicalOperator->addProperty(Optimizer::DOWNSTREAM_LOGICAL_OPERATOR_ID,
  ...
  auto linkedSinkSourcePairs = Experimental::findNetworkOperatorsForLink(
      sharedQueryId, globalExecutionPlan->getLockedExecutionNode(WorkerId(2)),
  ...
  decomposedQueryPlan = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(subPlanId), sharedQueryId, pinnedId);
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(2), workerAddress, dataPort), nesPartition,
      WAIT_TIME, DATA_CHANNEL_RETRY_TIMES, version, DEFAULT_NUMBER_OF_ORIGINS,
      networkSinkId);
  sinkLogicalOperator = std::make_shared<SinkLogicalOperator>(
      networkSinkDescriptor, networkSinkId);
  ...
  binaryOperator =
      LogicalOperatorFactory::createUnionOperator(binaryOperatorId);
  ...
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      schema, nesPartition,
      Network::NodeLocation(WorkerId(6), workerAddress, dataPort), WAIT_TIME,
      EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  auto leftsourceLogicalOperator = std::make_shared<SourceLogicalOperator>(
      networkSourceDescriptor, networkSourceId);
  leftsourceLogicalOperator->addProperty(
      Optimizer::DOWNSTREAM_LOGICAL_OPERATOR_ID, OperatorId{7});
  leftsourceLogicalOperator->addProperty(
      Optimizer::UPSTREAM_LOGICAL_OPERATOR_ID, OperatorId{13});
  ...
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      ...
      EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  auto rightsourceLogicalOperator = std::make_shared<SourceLogicalOperator>(
      networkSourceDescriptor, networkSourceId);
  rightsourceLogicalOperator->addProperty(
      Optimizer::DOWNSTREAM_LOGICAL_OPERATOR_ID, OperatorId{7});
  rightsourceLogicalOperator->addProperty(
      Optimizer::UPSTREAM_LOGICAL_OPERATOR_ID, OperatorId{17});
  ...
  decomposedQueryPlan = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(subPlanId), sharedQueryId, WorkerId(6));
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(3), workerAddress, dataPort),
      leftsourceLogicalOperator->getSourceDescriptor()
  ...
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      schema, nesPartition,
      Network::NodeLocation(WorkerId(7), workerAddress, dataPort), WAIT_TIME,
      EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  sourceLogicalOperator = std::make_shared<SourceLogicalOperator>(
      networkSourceDescriptor, networkSourceId);
  ...
  decomposedQueryPlan = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(subPlanId), sharedQueryId, pinnedId);
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(6), workerAddress, dataPort), nesPartition,
      WAIT_TIME, DATA_CHANNEL_RETRY_TIMES, version, DEFAULT_NUMBER_OF_ORIGINS,
      networkSinkId);
  sinkLogicalOperator = std::make_shared<SinkLogicalOperator>(
      networkSinkDescriptor, networkSinkId);
  ...
  auto defaultSourcedescriptor =
      CsvSourceDescriptor::create(schema, csvSourceType);
  auto defaultSourceLeft = std::make_shared<SourceLogicalOperator>(
      defaultSourcedescriptor, defaultSourceIdLeft);
  ...
  decomposedQueryPlan =
      DecomposedQueryPlan::create(DecomposedQueryPlanId(subPlanId),
                                  SharedQueryId(sharedQueryId), WorkerId(6));
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(3), workerAddress, dataPort),
      rightsourceLogicalOperator->getSourceDescriptor()
          ->as<Network::NetworkSourceDescriptor>()
          ->getNesPartition(),
      WAIT_TIME, DATA_CHANNEL_RETRY_TIMES, version, DEFAULT_NUMBER_OF_ORIGINS,
      networkSinkId);
  ...
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      schema, nesPartition,
      Network::NodeLocation(WorkerId(8), workerAddress, dataPort), WAIT_TIME,
      EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  sourceLogicalOperator = std::make_shared<SourceLogicalOperator>(
      networkSourceDescriptor, networkSourceId);
  ...
  pinnedId = WorkerId(8);
  decomposedQueryPlan = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(subPlanId), SharedQueryId(sharedQueryId), pinnedId);
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(6), workerAddress, dataPort), nesPartition,
      WAIT_TIME, DATA_CHANNEL_RETRY_TIMES, version, DEFAULT_NUMBER_OF_ORIGINS,
      networkSinkId);
  ...
  auto defaultSourceRight = std::make_shared<SourceLogicalOperator>(
      defaultSourcedescriptor, defaultSourceIdRight);
  ...
  networkSourceDescriptor = Network::NetworkSourceDescriptor::create(
      schema, nesPartition,
      Network::NodeLocation(WorkerId(9), workerAddress, dataPort), WAIT_TIME,
      EVENT_CHANNEL_RETRY_TIMES, version, OperatorId(uniqueId));
  uniqueId++;
  auto secondRightsourceLogicalOperator =
      std::make_shared<SourceLogicalOperator>(networkSourceDescriptor,
                                              networkSourceId);
  secondRightsourceLogicalOperator->addProperty(
      Optimizer::DOWNSTREAM_LOGICAL_OPERATOR_ID, OperatorId{7});
  secondRightsourceLogicalOperator->addProperty(
      Optimizer::UPSTREAM_LOGICAL_OPERATOR_ID, OperatorId{20});
  ...
  decomposedQueryPlan = DecomposedQueryPlan::create(
      DecomposedQueryPlanId(subPlanId), SharedQueryId(sharedQueryId), pinnedId);
  networkSinkDescriptor = Network::NetworkSinkDescriptor::create(
      Network::NodeLocation(WorkerId(3), workerAddress, dataPort), nesPartition,
      WAIT_TIME, DATA_CHANNEL_RETRY_TIMES, version, DEFAULT_NUMBER_OF_ORIGINS,
      networkSinkId);
  sinkLogicalOperator = std::make_shared<SinkLogicalOperator>(
      networkSinkDescriptor, networkSinkId);
  ...
  auto secondDefaultSourceRight = std::make_shared<SourceLogicalOperator>(
      defaultSourcedescriptor, secondDefaultSourceIdRight);
  auto copiedSecondDefaultSourceRight = secondDefaultSourceRight->copy();
  copiedSecondDefaultSourceRight->addProperty(Optimizer::PINNED_WORKER_ID,
                                              pinnedId);
  ...
  auto [upstreamPinned, downStreamPinned] =
      Experimental::findUpstreamAndDownstreamPinnedOperators(
          sharedQueryPlan,
          globalExecutionPlan->getLockedExecutionNode(WorkerId(6)),
          globalExecutionPlan->getLockedExecutionNode(WorkerId(3)), topology);
