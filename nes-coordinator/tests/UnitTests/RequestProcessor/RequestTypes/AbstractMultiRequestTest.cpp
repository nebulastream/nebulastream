...
public:
  DummyRequest(uint8_t maxRetries, uint32_t initialValue,
  ...
  std::vector<AbstractRequestPtr> executeRequestLogic() override {
    ...
    for (uint32_t i = 0; i < additionValue; ++i) {
      auto subRequest = std::make_shared<DummySubRequest>(
...
TEST_F(AbstractMultiRequestTest, testOneMainThreadOneExecutor) {
  ...
  auto thread = std::make_shared<std::thread>(
      [&request, &storageHandler]() { request.execute(storageHandler); });
  thread->join();
  ASSERT_TRUE(request.isDone());
  ASSERT_EQ(std::static_pointer_cast<DummyResponse>(future.get())->number,
            responseValue);
}
