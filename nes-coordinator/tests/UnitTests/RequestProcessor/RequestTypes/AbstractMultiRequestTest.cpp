...
TEST_F(AbstractMultiRequestTest, testOneMainThreadOneExecutor)
{
    ...
    auto thread = std::make_shared<std::thread>([&request, &storageHandler]() { request.execute(storageHandler); });
    thread->join();
...
TEST_F(AbstractMultiRequestTest, testOneMainThreadTwoExecutors)
{
    ...
    auto storageHandler = std::make_shared<DummyStorageHandler>();
    auto thread = std::make_shared<std::thread>([&request, &storageHandler]() { request.execute(storageHandler); });
    auto thread2 = std::make_shared<std::thread>([&request, &storageHandler]() { request.execute(storageHandler); });
