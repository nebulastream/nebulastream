...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("UnlockDeleterTest.log",
...
TEST_F(UnlockDeleterTest, TestBlocking) {
  ...
  // check that mutex gets locked by unlock deleter
  {
    ...
    std::unique_ptr<std::vector<int>, UnlockDeleter> handle({},
                                                            UnlockDeleter(mtx));
    thread = std::make_shared<std::thread>(
        [&mtx]() { ASSERT_FALSE((mtx.try_lock())); });
    thread->join();
  ...
  // check that thread can acquire lock after blocking wait
  {
    ...
    thread = std::make_shared<std::thread>([&mtx]() {
      ASSERT_NO_THROW((std::unique_ptr<std::vector<int>, UnlockDeleter>(
          {}, UnlockDeleter(mtx))));
    });
    // destructor releases lock
  }
...
TEST_F(UnlockDeleterTest, TestLocking) {
  ...
  // check that try lock will throw an exception if mutex is already locked
  {
    ...
    auto thread = std::make_shared<std::thread>([&mtx]() {
      ASSERT_THROW((std::unique_ptr<std::vector<int>, UnlockDeleter>(
                       {}, UnlockDeleter(mtx, std::try_to_lock))),
                   std::exception);
    });
    thread->join();
    // destructor releases lock
