...
TEST_F(TwoPhaseLockingStorageHandlerTest, TestLocking)
{
    ...
    thread = std::make_shared<std::thread>(
        [&twoPLAccessHandle, &releaseCount, queryId2, queryId3]()
        {
            auto thread3 = std::make_shared<std::thread>(
                [&twoPLAccessHandle, &releaseCount, queryId3]()
                {
    ...
    auto thread2 = std::make_shared<std::thread>(
        [&twoPLAccessHandle, queryId4]()
        {
            ASSERT_NO_THROW(twoPLAccessHandle->acquireResources(
                queryId4,
                {ResourceType::QueryCatalogService, ResourceType::GlobalQueryPlan, ResourceType::SourceCatalog, ResourceType::UdfCatalog}));
