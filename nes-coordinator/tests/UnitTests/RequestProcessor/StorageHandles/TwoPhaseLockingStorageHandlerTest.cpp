...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TwoPhaseLockingStorageHandlerTest.log",
...
TEST_F(TwoPhaseLockingStorageHandlerTest, TestResourceAccess) {
  ...
  ASSERT_THROW(twoPLAccessHandle->getGlobalExecutionPlanHandle(queryId1).get(),
               std::exception);
  ASSERT_THROW(twoPLAccessHandle->getTopologyHandle(queryId1).get(),
               std::exception);
  ASSERT_THROW(twoPLAccessHandle->getQueryCatalogHandle(queryId1).get(),
               std::exception);
  ASSERT_THROW(twoPLAccessHandle->getGlobalQueryPlanHandle(queryId1).get(),
               std::exception);
  ASSERT_THROW(twoPLAccessHandle->getSourceCatalogHandle(queryId1).get(),
               std::exception);
  ASSERT_THROW(twoPLAccessHandle->getUDFCatalogHandle(queryId1).get(),
               std::exception);
  ASSERT_THROW(twoPLAccessHandle->getStatisticProbeHandler(queryId1).get(),
               std::exception);
}
...
TEST_F(TwoPhaseLockingStorageHandlerTest, TestDoubleLocking) {
  ...
  auto twoPLAccessHandle = TwoPhaseLockingStorageHandler::create(
      {coordinatorConfiguration, topology, globalExecutionPlan, globalQueryPlan,
       ...
       statisticProbeHandler});
  ASSERT_NO_THROW(
      twoPLAccessHandle->acquireResources(queryId1, {ResourceType::Topology}));
  ASSERT_THROW(twoPLAccessHandle->acquireResources(queryId1, {}),
               std::exception);
}
...
TEST_F(TwoPhaseLockingStorageHandlerTest, TestLocking) {
  ...
  thread = std::make_shared<std::thread>([&twoPLAccessHandle, queryId2]() {
    ASSERT_NO_THROW(twoPLAccessHandle->acquireResources(
        ...
        {ResourceType::Topology, ResourceType::GlobalExecutionPlan,
         ResourceType::QueryCatalogService, ResourceType::GlobalQueryPlan,
  ...
  thread = std::make_shared<std::thread>([&twoPLAccessHandle, &releaseCount,
    ...
    auto thread3 = std::make_shared<std::thread>([&twoPLAccessHandle,
      ...
      while (twoPLAccessHandle->getCurrentTicket(ResourceType::Topology) < 1) {
        NES_DEBUG("Waiting request count {}",
                  twoPLAccessHandle->getCurrentTicket(ResourceType::Topology))
      }
      ASSERT_NO_THROW(twoPLAccessHandle->acquireResources(
          queryId3, {ResourceType::Topology}));
      ASSERT_EQ(releaseCount, 2);
    });
    ASSERT_NO_THROW(twoPLAccessHandle->acquireResources(
        queryId2, {ResourceType::Topology}));
    ASSERT_EQ(releaseCount, 1);
    ASSERT_NO_THROW(twoPLAccessHandle->getTopologyHandle(queryId2).get());
    ASSERT_THROW(
        twoPLAccessHandle->getGlobalExecutionPlanHandle(queryId2).get(),
        std::exception);
    ASSERT_THROW(twoPLAccessHandle->getQueryCatalogHandle(queryId2).get(),
                 std::exception);
    ASSERT_THROW(twoPLAccessHandle->getGlobalQueryPlanHandle(queryId2).get(),
                 std::exception);
    ASSERT_THROW(twoPLAccessHandle->getSourceCatalogHandle(queryId2).get(),
                 std::exception);
    ASSERT_THROW(twoPLAccessHandle->getUDFCatalogHandle(queryId2).get(),
                 std::exception);
    ASSERT_THROW(twoPLAccessHandle->getStatisticProbeHandler(queryId2).get(),
                 std::exception);
    releaseCount++;
    twoPLAccessHandle->releaseResources(queryId2);
    thread3->join();
  ...
  auto thread2 =
      std::make_shared<std::thread>([&twoPLAccessHandle, queryId4]() {
        ASSERT_NO_THROW(twoPLAccessHandle->acquireResources(
            queryId4,
            {ResourceType::QueryCatalogService, ResourceType::GlobalQueryPlan,
             ResourceType::SourceCatalog, ResourceType::UdfCatalog}));
...
TEST_F(TwoPhaseLockingStorageHandlerTest, TestNoDeadLock) {
  ...
  for (uint64_t underlying_request_id = 1; underlying_request_id < numThreads;
       ++underlying_request_id) {
    const auto i = RequestId(underlying_request_id);
    threads.emplace_back([i, &lockHolder, &resourceVector,
                          &reverseResourceVector, twoPLAccessHandle]() {
