...
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("GPUQueryExecutionTest.log",
  ...
  void SetUp() override {
    ...
    testSchemaSimple =
        Schema::create()->addField("test$value", BasicType::INT32);
    ...
    PhysicalSourcePtr sourceConf =
        PhysicalSource::create("default", "default1", defaultSourceType);
...
void cleanUpPlan(Runtime::Execution::ExecutableQueryPlanPtr plan) {
  std::for_each(plan->getSources().begin(), plan->getSources().end(),
                [plan](auto source) {
                  plan->notifySourceCompletion(
                      source, Runtime::QueryTerminationType::Graceful);
                });
  std::for_each(plan->getPipelines().begin(), plan->getPipelines().end(),
                [plan](auto pipeline) {
                  plan->notifyPipelineCompletion(
                      pipeline, Runtime::QueryTerminationType::Graceful);
                });
  std::for_each(plan->getSinks().begin(), plan->getSinks().end(),
                [plan](auto sink) {
                  plan->notifySinkCompletion(
                      sink, Runtime::QueryTerminationType::Graceful);
                });
  ASSERT_TRUE(plan->stop());
}

void fillBufferToSimpleSchema(
    TupleBuffer &buf,
    const Runtime::MemoryLayouts::RowLayoutPtr &memoryLayout) {

  auto valueField =
      Runtime::MemoryLayouts::RowLayoutField<int32_t, true>::create(
          0, memoryLayout, buf);
...
void fillBufferToMultiFieldSchema(
    TupleBuffer &buf,
    const Runtime::MemoryLayouts::RowLayoutPtr &memoryLayout) {
...
void fillBufferToWindowSchema(
  ...
  auto recordIndexFields =
      Runtime::MemoryLayouts::RowLayoutField<int32_t, true>::create(
          0, memoryLayout, buf);
  auto fields0 = Runtime::MemoryLayouts::RowLayoutField<uint32_t, true>::create(
      1, memoryLayout, buf);
  auto fields1 = Runtime::MemoryLayouts::RowLayoutField<uint64_t, true>::create(
      2, memoryLayout, buf);
...
class SimpleGPUPipelineStage
  ...
  uint32_t setup(Runtime::Execution::PipelineExecutionContext
    ...
    const char *const SimpleKernel_cu =
        "SimpleKernel.cu\n"
        "__global__ void simpleAdditionKernel(const int* recordValue, const "
    ...
    cudaKernelWrapper.setup(SimpleKernel_cu,
                            NUMBER_OF_TUPLE * sizeof(TupleDataType));
  ...
  ExecutionResult execute(Runtime::TupleBuffer &buffer,
    ...
    cudaKernelWrapper.execute(inputRecords, buffer.getNumberOfTuples(),
                              outputRecords, numberOfOutputTuples, kernel);

    ctx.emitBuffer(outputBuffer, wc);
    return ExecutionResult::Ok;
  }

  uint32_t stop(Runtime::Execution::PipelineExecutionContext
                    &pipelineExecutionContext) override {
...
public:
  uint32_t setup(Runtime::Execution::PipelineExecutionContext
                     &pipelineExecutionContext) override {
    // Prepare a simple CUDA kernel which adds 42 to the record.value and then
    // write it to the result
    const char *const MultifieldKernel_cu =
        "MultifieldKernel_cu.cu\n"
        "#include \"nes-core/tests/UnitTests/Query/GPUInputRecord.cuh\"\n"
        "__global__ void additionKernelMultipleFields(const InputRecord* "
        "recordValue, "
    ...
    const char *const header =
        "nes-core/tests/UnitTests/Query/GPUInputRecord.cuh\n"
    ...
    cudaKernelWrapper.setup(MultifieldKernel_cu,
                            NUMBER_OF_TUPLE * sizeof(InputRecord), {header});
  ...
  ExecutionResult execute(Runtime::TupleBuffer &buffer,
    ...
    // execute the kernel
    KernelDescriptor kernel = {"additionKernelMultipleFields", dim3(1),
                               dim3(32)};
    cudaKernelWrapper.execute(record, buffer.getNumberOfTuples(), outputRecords,
                              numberOfOutputTuples, kernel);
...
public:
  uint32_t setup(Runtime::Execution::PipelineExecutionContext
                     &pipelineExecutionContext) override {
    // Prepare a simple CUDA kernel which adds 42 to the record and then write
    // it to the result
    const char *const ColumnLayoutKernel_cu =
        "ColumnLayoutKernel_cu.cu\n"
        "#include \"nes-core/tests/UnitTests/Query/GPUInputRecord.cuh\"\n"
        "__global__ void additionKernelColumnLayout(const int64_t* "
        "recordValue, "
    ...
    cudaKernelWrapper.setup(ColumnLayoutKernel_cu,
                            NUMBER_OF_TUPLE * sizeof(InputRecord), {header});
    //        cudaKernelWrapper.setup(ColumnLayoutKernel_cu, NUMBER_OF_TUPLE *
    //        sizeof(int64_t));

    // define the schema (to be used to create column layout and obtaining
    // column offset)
    testSchemaColumnLayout =
        Schema::create(Schema::MemoryLayoutType::COLUMNAR_LAYOUT)
            ->addField("test$id", BasicType::INT64)
  ...
  ExecutionResult execute(Runtime::TupleBuffer &buffer,
    ...
    auto columnLayout = NES::Runtime::MemoryLayouts::ColumnLayout::create(
        testSchemaColumnLayout, buffer.getBufferSize());
    auto valColOffset =
        columnLayout->getColumnOffsets()[2]; // test$value is column 2

    // take the part of buffer in the specified offset
    auto valueBuffer =
        reinterpret_cast<int64_t *>(buffer.getBuffer() + valColOffset);
    ...
    cudaKernelWrapper.execute(valueBuffer, buffer.getNumberOfTuples(),
                              outputRecords, numberOfOutputTuples, kernel);
...
// Test the execution of an external operator using a simple GPU Kernel from a
// source of simple integer
TEST_F(GPUQueryExecutionTest, GPUOperatorSimpleQuery) {
  // creating query plan
  auto testSourceDescriptor = std::make_shared<TestUtils::TestSourceDescriptor>(
      testSchemaSimple,
      [&](SchemaPtr testSchemaSimple, OperatorId id, OriginId origin,
        ...
        return createDefaultDataSourceWithSchemaForOneBuffer(
            testSchemaSimple, nodeEngine->getBufferManager(),
            nodeEngine->getQueryManager(), id, origin, numSourceLocalBuffers,
            std::move(successors));
      });

  auto outputSchema = Schema::create()->addField("value", BasicType::INT32);
  auto testSink =
      std::make_shared<TestSink>(NUMBER_OF_TUPLE, outputSchema, nodeEngine);
  ...
  auto filterOperator =
      queryPlan->getOperatorByType<LogicalFilterOperator>()[0];

  auto customPipelineStage = std::make_shared<SimpleGPUPipelineStage>();
  auto externalOperator =
      NES::QueryCompilation::PhysicalOperators::PhysicalExternalOperator::
          create(SchemaPtr(), SchemaPtr(), customPipelineStage);

  filterOperator->insertBetweenThisAndParentNodes(externalOperator);

  auto request =
      QueryCompilation::QueryCompilationRequest::create(queryPlan, nodeEngine);
  auto queryCompiler = TestUtils::createTestQueryCompiler();
  auto result = queryCompiler->compileQuery(request);
  auto plan = result->getExecutableQueryPlan();
  // The plan should have one pipeline
  ASSERT_EQ(plan->getStatus(),
            Runtime::Execution::ExecutableQueryPlanStatus::Created);
  EXPECT_EQ(plan->getPipelines().size(), 2u);
  Runtime::WorkerContext workerContext{1, nodeEngine->getBufferManager(), 4};
  if (auto buffer = nodeEngine->getBufferManager()->getBufferBlocking();
      !!buffer) {
    auto memoryLayout = Runtime::MemoryLayouts::RowLayout::create(
        testSchemaSimple, nodeEngine->getBufferManager()->getBufferSize());
    fillBufferToSimpleSchema(buffer, memoryLayout);
    plan->setup();
    ASSERT_EQ(plan->getStatus(),
              Runtime::Execution::ExecutableQueryPlanStatus::Deployed);
    ASSERT_TRUE(plan->start(nodeEngine->getStateManager()));
    ASSERT_EQ(plan->getStatus(),
              Runtime::Execution::ExecutableQueryPlanStatus::Running);
    ASSERT_EQ(plan->getPipelines()[1]->execute(buffer, workerContext),
              ExecutionResult::Ok);
...
// Test the execution of an external operator on a source with a custom
// structure
TEST_F(GPUQueryExecutionTest, GPUOperatorWithMultipleFields) {
  // creating query plan
  auto testSourceDescriptor = std::make_shared<TestUtils::TestSourceDescriptor>(
      testSchemaMultipleFields,
      [&](SchemaPtr testSchemaMultipleFields, OperatorId id, OriginId origin,
          const SourceDescriptorPtr &, const Runtime::NodeEnginePtr &,
          size_t numSourceLocalBuffers,
          std::vector<Runtime::Execution::SuccessorExecutablePipeline>
              successors) -> DataSourcePtr {
        return createDefaultDataSourceWithSchemaForOneBuffer(
            testSchemaMultipleFields, nodeEngine->getBufferManager(),
            nodeEngine->getQueryManager(), id, origin, numSourceLocalBuffers,
            std::move(successors));
      });
...
TEST_F(GPUQueryExecutionTest, GPUOperatorOnColumnLayout) {
  ...
  auto testSourceDescriptor = std::make_shared<TestUtils::TestSourceDescriptor>(
      testSchemaColumnLayout,
      [&](SchemaPtr testSchemaColumnLayout, OperatorId id, OriginId origin,
          const SourceDescriptorPtr &, const Runtime::NodeEnginePtr &,
          size_t numSourceLocalBuffers,
          std::vector<Runtime::Execution::SuccessorExecutablePipeline>
              successors) -> DataSourcePtr {
        return createDefaultDataSourceWithSchemaForOneBuffer(
            testSchemaColumnLayout, nodeEngine->getBufferManager(),
            nodeEngine->getQueryManager(), id, origin, numSourceLocalBuffers,
            std::move(successors));
      });
  ...
  if (auto buffer = nodeEngine->getBufferManager()->getBufferBlocking();
    ...
    for (uint32_t recordIndex = 0u; recordIndex < 5u; ++recordIndex) {
      // id
      EXPECT_EQ(resultRecordValueFields[recordIndex], (recordIndex % 2) + 42);
      NES_DEBUG("expected: {} actual: {}", (recordIndex % 2) + 42,
                resultRecordValueFields[recordIndex]);
    }
  } else {
    FAIL();
  }
...
public:
  ...
  uint32_t setup(Runtime::Execution::PipelineExecutionContext
                     &pipelineExecutionContext) override {
    const char *const header =
        R"(nes-core/tests/UnitTests/Query/GPUInputRecord.cuh
            #ifndef NES_GPUINPUTRECORD_CUH
            #define NES_GPUINPUTRECORD_CUH
    ...
    cudaKernelWrapper.setup(slidingWindowKernel_cu,
                            NUMBER_OF_INPUT_TUPLES * sizeof(InputRecord),
                            {header});
  ...
  ExecutionResult execute(Runtime::TupleBuffer &buffer,
    ...
    dim3 numBlocks((numberOfInputTuples + threadsPerBlock.x - 1) /
                   threadsPerBlock.x);
    KernelDescriptor kernel = {"slidingWindowKernel", numBlocks,
                               threadsPerBlock};
    // execute the kernel
    cudaKernelWrapper.execute(inputRecords, numberOfInputTuples, outputRecords,
                              numberOfOutputTuples, kernel, WINDOW_LENGTH,
                              WINDOW_SLIDE, t_start, t_end);
...
TEST_F(GPUQueryExecutionTest, GPUOperatorWindowedAggregation) {
  ...
  auto testSourceDescriptor = std::make_shared<TestUtils::TestSourceDescriptor>(
      ...
      [&](SchemaPtr testSchemaWindowedAggregation, OperatorId id,
          OriginId origin, const SourceDescriptorPtr &,
          const Runtime::NodeEnginePtr &, size_t numSourceLocalBuffers,
          std::vector<Runtime::Execution::SuccessorExecutablePipeline>
              successors) -> DataSourcePtr {
        return createDefaultDataSourceWithSchemaForOneBuffer(
            testSchemaWindowedAggregation, nodeEngine->getBufferManager(),
            nodeEngine->getQueryManager(), id, origin, numSourceLocalBuffers,
            std::move(successors));
      });
  ...
  auto numOutputTuples =
      WindowedAggregationGPUPipelineStage::NUMBER_OF_OUTPUT_TUPLES;
  auto testSink =
      std::make_shared<TestSink>(numOutputTuples, outputSchema, nodeEngine);
  auto testSinkDescriptor =
      std::make_shared<TestUtils::TestSinkDescriptor>(testSink);
  ...
  auto customPipelineStage =
      std::make_shared<WindowedAggregationGPUPipelineStage>();
  auto externalOperator =
      NES::QueryCompilation::PhysicalOperators::PhysicalExternalOperator::
          create(SchemaPtr(), SchemaPtr(), customPipelineStage);

  auto sourceOperator =
      queryPlan->getOperatorByType<SourceLogicalOperator>()[0];
  ...
  auto memoryLayout = Runtime::MemoryLayouts::RowLayout::create(
      testSchemaWindowedAggregation,
      nodeEngine->getBufferManager()->getBufferSize());
  fillBufferToWindowSchema(
      buffer, memoryLayout,
      WindowedAggregationGPUPipelineStage::NUMBER_OF_INPUT_TUPLES);
  ...
  ASSERT_EQ(plan->getPipelines()[0]->execute(buffer, workerContext),
            ExecutionResult::Ok);
  ...
  auto resultMemoryLayout = Runtime::MemoryLayouts::RowLayout::create(
      outputSchema, nodeEngine->getBufferManager()->getBufferSize());
  auto indexField =
      Runtime::MemoryLayouts::RowLayoutField<int32_t, true>::create(
          0, resultMemoryLayout, resultBuffer);
  auto valueField =
      Runtime::MemoryLayouts::RowLayoutField<uint32_t, true>::create(
          1, resultMemoryLayout, resultBuffer);
  for (size_t recordIndex = 0; recordIndex < resultBuffer.getNumberOfTuples();
       recordIndex++) {
