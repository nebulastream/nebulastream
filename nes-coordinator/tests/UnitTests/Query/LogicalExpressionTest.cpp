...
/// Due to the order of overload resolution, the implicit conversion of 0 to a
/// nullptr has got a higher priority as an build-in conversion than the
/// user-defined implicit conversions to the type ExpressionItem. Check that
/// this does not cause any issues for any logical expression
class LogicalExpressionTest : public Testing::BaseUnitTest {
 public:
  std::shared_ptr<QueryParsingService> queryParsingService;
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LogicalExpressionTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_DEBUG(
        "LogicalExpressionTest: Setup QueryCatalogServiceTest test class.");
  }
...
/// Template that can check that different Operators can be used with all
/// designated data types that construct an ExpressionItem.
template <template <typename, auto...> typename CompilesFromArgs,
          template <typename...> typename CompilesFromTypes>
void checkBinary() {
  bool compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, int8_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, int8_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, uint8_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, uint8_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, int16_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, int16_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, uint16_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, uint16_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, int32_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, int32_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, uint32_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, uint32_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, int64_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, int64_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, uint64_t>::value &&
      CompilesFromTypes<ExpressionNodePtr, uint64_t, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, float>::value &&
      CompilesFromTypes<ExpressionNodePtr, float, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, double>::value &&
      CompilesFromTypes<ExpressionNodePtr, double, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem, bool>::value &&
      CompilesFromTypes<ExpressionNodePtr, bool, ExpressionItem>::value;
  ASSERT_TRUE(compiles);

  compiles =
      CompilesFromTypes<ExpressionNodePtr, ExpressionItem,
                        char const*>::value &&
  ...
  compiles = CompilesFromTypes<ExpressionNodePtr, ExpressionItem,
                               ExpressionNodePtr>::value &&
             CompilesFromTypes<ExpressionNodePtr, ExpressionNodePtr,
