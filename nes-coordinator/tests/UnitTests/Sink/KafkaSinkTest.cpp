...
public:
    ...
    static NES::Runtime::TupleBuffer createSimpleBuffer(uint64_t bufferSize, const std::shared_ptr<Runtime::BufferManager>& buffMgr)
    {
...
{
    auto kafkaSink
        = createCsvKafkaSink(testSchema, SharedQueryId(QUERYID), DecomposedQueryPlanId(QUERYID), nodeEngine, 1, brokers, topic, 1, 1);
}
