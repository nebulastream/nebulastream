...
public:
  ...
  void SetUp() {
    ...
    auto conf =
        DefaultSourceType::create("LogicalSourceName", "PhysicalSourceName");
  ...
  static void TearDownTestCase() {
    NES_DEBUG(
        "OPCSINKTEST::TearDownTestCases() Tear down OPCSourceTest test class.");
  }
  ...
  static void addVariable(UA_Server *server) {
    ...
    UA_Server_addVariableNode(
        server, myIntegerNodeId, parentNodeId, parentReferenceNodeId,
...
TEST_F(OPCSinkTest, OPCSourceInit) {
  auto opcSink =
      createOPCSink(test_schema, 0, 0, nodeEngine, url, nodeId, user, password);
  SUCCEED();
}
...
TEST_F(OPCSinkTest, OPCSourcePrint) {
  auto opcSink =
      createOPCSink(test_schema, 0, 0, nodeEngine, url, nodeId, user, password);
  std::string expected =
      "OPC_SINK(SCHEMA(var:INTEGER ), URL= opc.tcp://localhost:4840, "
      "NODE_INDEX= 1, NODE_IDENTIFIER= the answer. ";
...
TEST_F(OPCSinkTest, OPCSourceValue) {
  ...
  Runtime::TupleBuffer write_buffer =
      nodeEngine->getBufferManager()->getBufferBlocking();
  ...
  auto rowLayout = Runtime::MemoryLayouts::RowLayout::create(
      test_schema, write_buffer.getBufferSize());
  auto testTupleBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(rowLayout, write_buffer);
  NES_DEBUG(
      "OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) buffer before write: {}",
      testTupleBuffer.toString(test_schema));

  opcSink->writeData(write_buffer, workerContext);
  NES_DEBUG("OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) data was written");
  write_buffer.release();

  auto opcSource = createOPCSource(test_schema, nodeEngine->getBufferManager(),
                                   nodeEngine->getQueryManager(), url, nodeId,
                                   user, password, 1, 12, "physicalSource", {});
  opcSource->open();
  auto tuple_buffer = opcSource->receiveData();
  uint64_t value = 0;
  auto *tuple = (uint32_t *)tuple_buffer->getBuffer();
  NES_DEBUG(
      "OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) Received value is: {}",
      *(uint32_t *)tuple_buffer->getBuffer());
  value = *tuple;
  uint64_t expected = 45;
  NES_DEBUG("OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) expected value is: "
            "{}. Received value is: {}",
            expected, value);
