...
public:
  ...
  void SetUp() override {
    ...
    workerConfiguration->physicalSourceTypes.add(
        CSVSourceType::create("x", "x1"));
    this->nodeEngine =
...
TEST_F(SinkTest, testCSVFileSink) {
  ...
  const DataSinkPtr csvSink = createCSVFileSink(
      test_schema, SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID,
      nodeEngine, 1, path_to_csv_file, true);
  ...
  auto rowLayoutBeforeWrite = Runtime::MemoryLayouts::RowLayout::create(
      test_schema, buffer.getBufferSize());
  auto testTupleBufferBeforeWrite =
      Runtime::MemoryLayouts::TestTupleBuffer(rowLayoutBeforeWrite, buffer);
  std::string bufferContentBeforeWrite =
      testTupleBufferBeforeWrite.toString(test_schema);
  NES_TRACE("Buffer Content= {}", bufferContentBeforeWrite);

  // get buffer content as string
  auto rowLayoutAfterWrite = Runtime::MemoryLayouts::RowLayout::create(
      test_schema, buffer.getBufferSize());
  auto testTupleBufferAfterWrite =
      Runtime::MemoryLayouts::TestTupleBuffer(rowLayoutAfterWrite, buffer);
  std::string bufferContentAfterWrite =
      testTupleBufferAfterWrite.toString(test_schema);
...
TEST_F(SinkTest, testCSVPrintSink) {
  ...
  TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
  auto csvSink =
      createCSVPrintSink(test_schema, SharedQueryId(0),
                         INVALID_DECOMPOSED_QUERY_PLAN_ID, nodeEngine, 1, os);
  for (uint64_t i = 0; i < 2; ++i) {
    for (uint64_t j = 0; j < 2; ++j) {
      buffer.getBuffer<uint64_t>()[j] = j;
    }
  ...
  buffer.setNumberOfTuples(4);
  // cout << "bufferContent before write=" <<
  // Util::prettyPrintTupleBuffer(buffer, test_schema) << endl;
  write_result = csvSink->writeData(buffer, wctx);
  ...
  auto testTupleBuffer =
      Runtime::MemoryLayouts::TestTupleBuffer(rowLayout, buffer);
...
TEST_F(SinkTest, testNullOutSink) {
  ...
  auto nullSink = createNullOutputSink(
      SharedQueryId(1), INVALID_DECOMPOSED_QUERY_PLAN_ID, nodeEngine, 1);
  for (uint64_t i = 0; i < 2; ++i) {
    for (uint64_t j = 0; j < 2; ++j) {
      buffer.getBuffer<uint64_t>()[j] = j;
    }
...
TEST_F(SinkTest, testCSVZMQSink) {
  ...
  Runtime::WorkerContext wctx(Runtime::NesThread::getId(),
                              nodeEngine->getBufferManager(), 64);
  TupleBuffer buffer = nodeEngine->getBufferManager()->getBufferBlocking();
  const DataSinkPtr zmq_sink = createCSVZmqSink(
      test_schema, SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID,
      nodeEngine, 1, "localhost", zmqPort);
  ...
  buffer.setNumberOfTuples(4);
  ////cout << "buffer before send=" << Util::prettyPrintTupleBuffer(buffer,
  ///test_schema);

  // Create ZeroMQ Data Source.
  auto zmq_source = createZmqSource(
      test_schema, nodeEngine->getBufferManager(),
      nodeEngine->getQueryManager(), "localhost", zmqPort, OperatorId(1),
      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, 12, "defaultPhysicalSourceName",
  ...
  auto receiving_thread = std::thread([&]() {
    ...
    schemaStr.assign(bufSchema.getBuffer<char>(),
                     bufSchema.getNumberOfTuples());
    ...
    std::string bufferContent =
        Util::printTupleBufferAsCSV(buffer, test_schema);
    std::string dataStr;
...
TEST_F(SinkTest, testWatermarkForZMQ) {
  ...
  const DataSinkPtr zmq_sink = createBinaryZmqSink(
      test_schema, SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID,
      nodeEngine, 1, "localhost", zmqPort, false);
  for (uint64_t i = 1; i < 3; ++i) {
    for (uint64_t j = 0; j < 2; ++j) {
      buffer.getBuffer<uint64_t>()[j * i] = j;
    }
...
TEST_F(SinkTest, testMonitoringSink) {
  ...
  Monitoring::DiskMetrics typedMetric =
      diskMetric->getValue<Monitoring::DiskMetrics>();
  EXPECT_EQ(diskMetric->getMetricType(), Monitoring::MetricType::DiskMetric);
  auto bufferSize =
      Monitoring::DiskMetrics::getSchema("")->getSchemaSizeInBytes();
  auto tupleBuffer =
      nodeEngine->getBufferManager()->getUnpooledBuffer(bufferSize).value();
  ...
  Monitoring::CpuMetricsWrapper typedMetricCpu =
      cpuMetric->getValue<Monitoring::CpuMetricsWrapper>();
  EXPECT_EQ(cpuMetric->getMetricType(),
            Monitoring::MetricType::WrappedCpuMetrics);
  auto bufferSizeCpu =
      Monitoring::CpuMetrics::getSchema("")->getSchemaSizeInBytes() *
          typedMetricCpu.size() +
      64;
  auto tupleBufferCpu =
      nodeEngine->getBufferManager()->getUnpooledBuffer(bufferSizeCpu).value();
  ...
  const DataSinkPtr monitoringSink = createMonitoringSink(
      metricStore, diskCollector.getType(),
      Monitoring::DiskMetrics::getSchema(""), nodeEngine, 1, SharedQueryId(0),
      INVALID_DECOMPOSED_QUERY_PLAN_ID);
  monitoringSink->writeData(tupleBuffer, wctx);

  // write cpu metrics
  const DataSinkPtr monitoringSinkCpu =
      createMonitoringSink(metricStore, cpuCollector.getType(),
                           Monitoring::CpuMetrics::getSchema(""), nodeEngine, 1,
                           SharedQueryId(0), INVALID_DECOMPOSED_QUERY_PLAN_ID);
  monitoringSinkCpu->writeData(tupleBufferCpu, wctx);

  // test disk metrics
  Monitoring::StoredNodeMetricsPtr storedMetrics =
      metricStore->getAllMetrics(nodeId1);
  ...
  Monitoring::DiskMetrics parsedMetrics =
      retMetric->getValue<Monitoring::DiskMetrics>();

  NES_INFO("MetricStoreTest: Stored metrics{}",
           Monitoring::MetricUtils::toJson(storedMetrics));
  ASSERT_TRUE(storedMetrics->size() == 1);
  EXPECT_EQ(parsedMetrics, typedMetric);

  // test cpu metrics
  Monitoring::StoredNodeMetricsPtr storedMetricsCpu =
      metricStore->getAllMetrics(nodeId2);
  auto metricVecCpu =
      storedMetricsCpu->at(Monitoring::MetricType::WrappedCpuMetrics);
  Monitoring::TimestampMetricPtr pairedCpuMetric = metricVecCpu->at(0);
  Monitoring::MetricPtr retMetricCpu = pairedCpuMetric->second;
  Monitoring::CpuMetricsWrapper parsedMetricsCpu =
      retMetricCpu->getValue<Monitoring::CpuMetricsWrapper>();

  NES_INFO("MetricStoreTest: Stored metrics{}",
           Monitoring::MetricUtils::toJson(storedMetricsCpu));
