...
class StatisticSinkTest : public Testing::BaseIntegrationTest,
                          public ::testing::WithParamInterface<
                              std::tuple<int, Statistic::StatisticDataCodec>> {
public:
  int numberOfStatistics;
  Statistic::StatisticDataCodec statisticDataCodec;

  static void SetUpTestCase() {
    NES::Logger::setupLogging("StatisticSinkTest.log",
  ...
  /**
   * @brief Checks if the left and right vector contain the same
   * (order-insensitive) statistics
   * @param left
   * @param right
   * @return True, if vector contain the same, false otherwise
   */
  bool sameStatisticsInVectors(std::vector<Statistic::HashStatisticPair> &left,
                               std::vector<Statistic::StatisticPtr> &right) {
    if (left.size() != right.size()) {
      NES_ERROR("Vectors are not equal with {} and {} items!", left.size(),
                right.size());
      return false;
    }

    // We iterate over the left and search for the same item in the right and
    // then remove it from right vector
    for (const auto &elem : left) {
      auto it = std::find_if(right.begin(), right.end(),
                             [&](const Statistic::StatisticPtr &statistic) {
                               return statistic->equal(*elem.second);
                             });

      if (it == right.end()) {
        NES_ERROR("Can not find statistic {} in right vector!",
   ...
   */
  std::pair<std::vector<Runtime::TupleBuffer>,
  ...
  createRandomCountMinSketches(uint64_t numberOfSketches,
    ...
    for (auto curCnt = 0_u64; curCnt < numberOfSketches; ++curCnt) {
      // Creating "random" values that represent of a CountMinStatistic for a
      // tumbling window with a size of 10
      ...
      auto countMin = Statistic::CountMinStatistic::createInit(
    ...
    auto createdBuffers = statisticFormat->writeStatisticsIntoBuffers(
        statisticsWithHashes, *bufferManager);
  ...
  createRandomHyperLogLogSketches(
      uint64_t numberOfSketches, const SchemaPtr &schema,
      const Runtime::BufferManagerPtr &bufferManager) {
    std::vector<Statistic::StatisticPtr> expectedStatistics;
    constexpr auto windowSize = 10;

    for (auto curCnt = 0_u64; curCnt < numberOfSketches; ++curCnt) {
      // Creating "random" values that represent of a HyperLogLogStatistics for
      // a tumbling window with a size of 10
      const Windowing::TimeMeasure startTs(windowSize * curCnt);
      const Windowing::TimeMeasure endTs(windowSize * (curCnt + 1));
      const uint64_t width = rand() % 10 + 5;
      ...
      auto hyperLogLog =
          Statistic::HyperLogLogStatistic::createInit(startTs, endTs, width)
              ->as<Statistic::HyperLogLogStatistic>();
      for (auto i = 0; i < numberOfTuplesToInsert; ++i) {
        auto hash = rand();
...
/**
 * @brief Tests if our statistic sink can put CountMin-Sketches into the
 * StatisticStore
 */
TEST_P(StatisticSinkTest, testCountMin) {
  auto countMinStatisticSchema =
      Schema::create()
  ...
  auto [buffers, expectedStatistics] =
      createRandomCountMinSketches(numberOfStatistics, countMinStatisticSchema,
                                   nodeEngine->getBufferManager());
  auto statisticSink = createStatisticSink(
      countMinStatisticSchema, nodeEngine,
      1,                        // numOfProducers
  ...
  auto storedStatistics = nodeEngine->getStatisticManager()
                              ->getStatisticStore()
                              ->getAllStatistics();
...
/**
 * @brief Tests if our statistic sink can put HyperLogLog-Sketches into the
 * StatisticStore
 */
TEST_P(StatisticSinkTest, testHyperLogLog) {
  auto hyperLogLogStatisticSchema =
      Schema::create()
          ->addField(Statistic::WIDTH_FIELD_NAME, BasicType::UINT64)
          ->addField(Statistic::STATISTIC_DATA_FIELD_NAME, BasicType::TEXT)
          ->addField(Statistic::BASE_FIELD_NAME_START, BasicType::UINT64)
  ...
  // Creating the number of buffers
  auto [buffers, expectedStatistics] = createRandomHyperLogLogSketches(
      numberOfStatistics, hyperLogLogStatisticSchema,
      nodeEngine->getBufferManager());
  auto statisticSink = createStatisticSink(
      hyperLogLogStatisticSchema, nodeEngine,
      1, // numOfProducers
...
INSTANTIATE_TEST_CASE_P(
    testStatisticSink, StatisticSinkTest,
    ::testing::Combine(
