...
TEST_F(TopologyManagerServiceTest, testRegisterUnregisterNode) {
  ...
  auto latencyInMs = 1;
  auto nodeId =
      topology->registerWorker(INVALID_WORKER_NODE_ID, ip, publish_port, 5000,
                               6, properties, bandwidthInMbps, latencyInMs);
  EXPECT_NE(nodeId, INVALID_WORKER_NODE_ID);

  auto nodeId1 = topology->registerWorker(INVALID_WORKER_NODE_ID, ip,
                                          publish_port + 2, 5000, 6, properties,
  ...
  auto nodeId2 =
      topology->registerWorker(WorkerId(2), ip, publish_port + 4, 5000, 6,
                               properties, bandwidthInMbps, latencyInMs);
  EXPECT_EQ(nodeId2, WorkerId(3));

  // test unregister not existing node
  bool successUnregisterNotExistingNode =
      topology->unregisterWorker(WorkerId(552));
  ...
  auto nodeId3 = topology->registerWorker(INVALID_WORKER_NODE_ID, ip,
                                          publish_port + 6, 5000, 6, properties,
                                          bandwidthInMbps, latencyInMs);
  EXPECT_EQ(nodeId3, WorkerId(4));

  // test register new node with misconfigured worker id
  // when trying to register with a workerId that belongs to an already
  // registered worker, the next available workerId will be assigned
  auto nodeId4 =
      topology->registerWorker(nodeId3, ip, publish_port + 8, 5000, 6,
                               properties, bandwidthInMbps, latencyInMs);
