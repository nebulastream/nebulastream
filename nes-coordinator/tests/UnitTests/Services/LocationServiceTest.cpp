...
using allMobileResponse =
    std::map<std::string, std::vector<std::map<std::string, nlohmann::json>>>;

class LocationServiceTest : public Testing::BaseIntegrationTest {
public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("LocationServiceTest.log",
    ...
    writeWaypointsToCsv(singleLocationPath,
                        {{{52.55227464714949, 13.351743136322877}, 0}});
  }
  static void TearDownTestCase(){
      NES_INFO("Tear down LocationServiceTest test class")}
...
TEST_F(LocationServiceTest, testRequestSingleNodeLocation) {
  ...
  auto latencyInMs = 1;
  auto node1Id = topology->registerWorker(INVALID_WORKER_NODE_ID, "127.0.0.1",
                                          *rpcPortWrk1, 0, 0, properties,
                                          bandwidthInMbps, latencyInMs);

  // create fixed location node
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
      NES::Spatial::Experimental::SpatialType::FIXED_LOCATION;
  auto node2Id = topology->registerWorker(INVALID_WORKER_NODE_ID, "127.0.0.1",
                                          *rpcPortWrk2, 0, 0, properties,
                                          bandwidthInMbps, latencyInMs);
  ...
  auto node3Id = topology->registerWorker(INVALID_WORKER_NODE_ID, "127.0.0.1",
                                          *rpcPortWrk3, 0, 0, properties,
                                          bandwidthInMbps, latencyInMs);
  ...
  auto entry =
      nodeLocationInfoJson.get<std::map<std::string, nlohmann::json>>();
...
TEST_F(LocationServiceTest, testRequestAllMobileNodeLocations) {
  ...
  auto response = topology->requestLocationAndParentDataFromAllMobileNodes()
  ...
  properties[NES::Worker::Configuration::SPATIAL_SUPPORT] =
      NES::Spatial::Experimental::SpatialType::MOBILE_NODE;
  auto node4Id = topology->registerWorker(INVALID_WORKER_NODE_ID, "127.0.0.1",
                                          *rpcPortWrk4, 0, 0, properties,
                                          bandwidthInMbps, latencyInMs);

  // get mobile node info
  response = topology->requestLocationAndParentDataFromAllMobileNodes()
                 .get<allMobileResponse>();
  ...
  topology->updateGeoLocation(node4Id,
                              {53.55227464714949, -13.351743136322877});
  ...
  for (const auto &topologyEdge : edges) {
    ...
    auto sourcesIterator =
        std::find(sources.begin(), sources.end(), edgeSource);
