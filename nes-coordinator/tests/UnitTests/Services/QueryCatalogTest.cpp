...
TEST_F(QueryCatalogTest, testAddNewQuery) {
  ...
  QueryCatalogPtr queryCatalog =
      std::make_shared<Catalogs::Query::QueryCatalog>();
...
TEST_F(QueryCatalogTest, testAddNewPattern) {
  ...
  const QueryPlanPtr queryPlan =
      queryParsingService->createPatternFromCodeString(patternString);
  QueryId queryId = PlanIdGenerator::getNextQueryId();
  queryPlan->setQueryId(queryId);
  QueryCatalogPtr queryCatalog = std::make_shared<QueryCatalog>();
  queryCatalog->createQueryCatalogEntry(patternString, queryPlan,
                                        Optimizer::PlacementStrategy::BottomUp,
                                        QueryState::REGISTERED);
...
TEST_F(QueryCatalogTest, testAddNewQueryAndStop) {
  ...
  auto reg = queryCatalog->getAllQueryEntries();
  EXPECT_TRUE(reg.size() == 1U);
  auto registeredQueries =
      queryCatalog->getQueryEntriesWithStatus("REGISTERED");
  ...
  auto queriesMarkedForStop =
      queryCatalog->getQueryEntriesWithStatus("MARKED_FOR_HARD_STOP");
...
TEST_F(QueryCatalogTest, throInvalidArgumentExceptionWhenQueryStatusIsUnknown) {
  ...
  QueryCatalogPtr queryCatalog =
      std::make_shared<Catalogs::Query::QueryCatalog>();
  EXPECT_THROW(queryCatalog->getQueryEntriesWithStatus("something_random"),
