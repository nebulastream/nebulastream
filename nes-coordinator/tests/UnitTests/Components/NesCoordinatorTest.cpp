...
// Test that the worker configuration from the coordinator configuration is
// passed to the internal worker.
TEST_F(NesCoordinatorTest,
  ...
  // then
  ASSERT_EQ(3, coordinator->getNesWorker()
                   ->getWorkerConfiguration()
                   ->numWorkerThreads.getValue());
  // Stop coordinator.
  NES_DEBUG("Stopping coordinator.")
  EXPECT_TRUE(coordinator->stopCoordinator(true));
}

// Test that IP and port of the internal worker are consistent with IP and port
// from coordinator
TEST_F(NesCoordinatorTest, internalWorkerUsesIpAndPortFromCoordinator) {
  ...
  // given: Configure the worker with nonsensical IP and port, and disable
  // monitoring
  configuration->worker.coordinatorPort =
      111; // This port won't be assigned by the line above because it is below
           // 1024.
  configuration->worker.coordinatorHost = "127.0.0.2";
  configuration->worker.localWorkerHost = coordinatorWorkerIp;
  configuration->worker.enableMonitoring = false;
  // when
  auto coordinator = std::make_shared<NesCoordinator>(configuration);
  NES_DEBUG("Starting coordinator.")
  coordinator->startCoordinator(false);
  // then: the IP and port in the worker configuration are overwritten, and
  // monitoring is enabled
  auto workerConfiguration =
      coordinator->getNesWorker()->getWorkerConfiguration();
  EXPECT_EQ(*rpcCoordinatorPort,
            workerConfiguration->coordinatorPort.getValue());
  EXPECT_EQ(coordinatorHost, workerConfiguration->coordinatorHost.getValue());
  EXPECT_EQ(coordinatorWorkerIp,
            workerConfiguration->localWorkerHost.getValue());
