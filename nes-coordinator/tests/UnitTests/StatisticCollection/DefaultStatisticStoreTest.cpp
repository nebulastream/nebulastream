...
class DummyStatistic : public Statistic {
 public:
  ...
  static StatisticPtr create(Windowing::TimeMeasure startTs,
  ...
  bool equal(const Statistic& other) const override {
    if (other.instanceOf<DummyStatistic>()) {
      ...
      return startTs == otherDummy.startTs && endTs == otherDummy.endTs &&
             observedTuples == otherDummy.observedTuples &&
...
class DefaultStatisticStoreTest
 ...
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("DefaultStatisticStoreTest.log",
  ...
  /**
   * @brief Creates statistics for the number of statistics key and returns the
   * largest endTs
  ...
  TimeMeasure createData(
    ...
    for (auto keyCnt = 0; keyCnt < numberOfStatisticKeys; ++keyCnt) {
      ...
      for (auto i = 0; i < numberOfStatisticPerKey; ++i) {
        ...
        allStatisticsPlusKey.emplace_back(std::make_pair(
            statisticKey, DummyStatistic::create(startTs, endTs)));
...
TEST_F(DefaultStatisticStoreTest, singleItem) {
  ...
  ASSERT_TRUE(defaultStatisticStore.insertStatistic(statisticKey.hash(),
                                                    dummyStatistic));
  auto getStatistics =
      defaultStatisticStore.getStatistics(statisticKey.hash(), startTs, endTs);
  ASSERT_EQ(getStatistics.size(), 1);
  EXPECT_TRUE(getStatistics[0]->equal(*dummyStatistic));

  // Now, we delete the statistic and then check that we can not retrieve it
  // anymore
  ASSERT_TRUE(defaultStatisticStore.deleteStatistics(statisticKey.hash(),
                                                     startTs, endTs));
  getStatistics =
      defaultStatisticStore.getStatistics(statisticKey.hash(), startTs, endTs);
...
TEST_P(DefaultStatisticStoreTest, multipleItem) {
  ...
  const auto numberOfThreads =
      std::get<0>(DefaultStatisticStoreTest::GetParam());
  const auto numberOfStatisticKey =
      std::get<1>(DefaultStatisticStoreTest::GetParam());
  const auto numberOfStatisticsPerKey =
      std::get<2>(DefaultStatisticStoreTest::GetParam());

  // Creating for each statistic key its statistics
  std::vector<std::pair<StatisticKey, StatisticPtr>> allStatisticsPlusKey;
  std::vector<StatisticKey> allStatisticKeys;
  auto maxEndTs = createData(allStatisticsPlusKey, allStatisticKeys,
                             numberOfStatisticKey, numberOfStatisticsPerKey);
    ...
    insertThreads.emplace_back([&statisticsPos, this, &allStatisticsPlusKey]() {
      ...
      while ((nextPos = statisticsPos++) < allStatisticsPlusKey.size()) {
        ...
        auto getStatistics = defaultStatisticStore.getStatistics(
            statisticKey.hash(), dummyStatistic->getStartTs(),
  ...
  for (auto& statisticKey : allStatisticKeys) {
    ...
    auto getStatistics = defaultStatisticStore.getStatistics(
        statisticKey.hash(), Milliseconds(0), maxEndTs);
    ASSERT_EQ(getStatistics.size(), statistics.size());
    std::sort(getStatistics.begin(), getStatistics.end(),
              [](const StatisticPtr& left, const StatisticPtr& right) {
    ...
    deleteThreads.emplace_back([&statisticsPos, this, &allStatisticsPlusKey]() {
      ...
      while ((nextPos = statisticsPos++) < allStatisticsPlusKey.size()) {
        ...
        ASSERT_TRUE(defaultStatisticStore.deleteStatistics(statisticKey.hash(),
                                                           startTs, endTs));
        auto getStatistics = defaultStatisticStore.getStatistics(
            statisticKey.hash(), startTs, endTs);
        ASSERT_EQ(getStatistics.size(), 0);
      }
    });
  }
...
INSTANTIATE_TEST_CASE_P(
    testDefaultStatisticStore, DefaultStatisticStoreTest,
    // We test here over threads, number of statistic keys, and number of
    // statistics per key
    ::testing::Combine(::testing::Values(1, 2, 4, 8),  // No. threads
                       ::testing::Values(1, 5, 10),    // No. statistic key
                       ::testing::Values(1, 500,
                                         1000)),  // No. statistics per key
    [](const testing::TestParamInfo<DefaultStatisticStoreTest::ParamType>&
           info) {
      return std::string(
