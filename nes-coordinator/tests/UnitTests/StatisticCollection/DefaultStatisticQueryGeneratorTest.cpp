...
class DefaultStatisticQueryGeneratorTest : public Testing::BaseUnitTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("DefaultStatisticQueryGeneratorTest.log",
  ...
  LogicalOperatorPtr checkWindowStatisticOperatorCorrect(
      QueryPlan& queryPlan, Windowing::WindowType& window) const {
    ...
    auto statisticWindowOperatorNode =
        sinkChild[0]->as<LogicalStatisticWindowOperator>();
    EXPECT_TRUE(window.equal(statisticWindowOperatorNode->getWindowType()));
    EXPECT_TRUE(statisticWindowOperatorNode->getOutputSchema()->equals(
        outputSchemaBuildOperator, false));
...
/**
 * @brief Tests if a query is generated correctly for a cardinality, the outcome
 * should be a HyperLogLog
 */
TEST_F(DefaultStatisticQueryGeneratorTest, cardinality) {
  ...
  const auto dataCharacteristic = DataCharacteristic::create(
      Cardinality::create(Over("f1")), "car", "car_1");
  const auto window =
      TumblingWindow::of(EventTime(Attribute("ts")), Seconds(10));
  ...
  const auto statisticQuery =
      defaultStatisticQueryGenerator.createStatisticQuery(
          *dataCharacteristic, window, sendingPolicy, triggerCondition,
          *queryCatalog);
  typeInferencePhase->execute(statisticQuery.getQueryPlan());

  // Checking if the operator is correct
  auto statisticWindowOperatorNode =
      checkWindowStatisticOperatorCorrect(*statisticQuery.getQueryPlan(),
                                          *window)
          ->as<LogicalStatisticWindowOperator>();
  ...
  auto operatorTriggerCondition =
      statisticWindowOperatorNode->getTriggerCondition();
...
/**
 * @brief Tests if a query is generated correctly for a selectivity, the outcome
 * should be a CountMin
 */
TEST_F(DefaultStatisticQueryGeneratorTest, selectivity) {
  ...
  const auto dataCharacteristic = DataCharacteristic::create(
      Selectivity::create(Over("f1")), "car", "car_1");
  const auto window =
      SlidingWindow::of(EventTime(Attribute("ts")), Seconds(60), Seconds(10));
...
/**
 * @brief Tests if a query is generated correctly for an ingestionRate, the
 * outcome should be a CountMin
 */
TEST_F(DefaultStatisticQueryGeneratorTest, ingestionRate) {
  ...
  inputSchema =
      inputSchema->addField(INGESTION_RATE_FIELD_NAME, BasicType::UINT64);
  ...
  constexpr auto EXPECTED_WIDTH = 55;
  constexpr auto EXPECTED_DEPTH = 3;
  const auto dataCharacteristic =
      DataCharacteristic::create(IngestionRate::create(), "car", "car_1");
  const auto window =
      SlidingWindow::of(EventTime(Attribute("ts")), Seconds(60), Seconds(10));
  const auto sendingPolicy = SENDING_LAZY(StatisticDataCodec::DEFAULT);
  const auto triggerCondition = NeverTrigger::create();
...
/**
 * @brief Tests if a query is generated correctly for a bufferRate, the outcome
 * should be a CountMin
 */
TEST_F(DefaultStatisticQueryGeneratorTest, bufferRate) {
  ...
  inputSchema =
      inputSchema->addField(BUFFER_RATE_FIELD_NAME, BasicType::UINT64);
  ...
  constexpr auto EXPECTED_DEPTH = 3;
  const auto dataCharacteristic =
      DataCharacteristic::create(BufferRate::create(), "car", "car_1");
  const auto window =
      SlidingWindow::of(EventTime(Attribute("ts")), Hours(24), Seconds(60));
...
/**
 * @brief Tests if a query is generated correctly for a minVal, the outcome
 * should be a CountMin
 */
TEST_F(DefaultStatisticQueryGeneratorTest, minVal) {
  ...
  constexpr auto EXPECTED_DEPTH = 3;
  const auto dataCharacteristic =
      DataCharacteristic::create(MinVal::create(Over("f1")), "car", "car_1");
  const auto window =
      SlidingWindow::of(EventTime(Attribute("ts")), Seconds(60), Seconds(10));
  const auto sendingPolicy = SENDING_LAZY(StatisticDataCodec::DEFAULT);
  const auto triggerCondition = NeverTrigger::create();
...
/**
 * @brief Tests if we create a statistic query for collecting cardinality of a
 * map operator
 */
TEST_F(DefaultStatisticQueryGeneratorTest,
       workloadCharacteristicMapOperatorCardinality) {
  using namespace NES::Statistic;
  using namespace Windowing;

  // Creating the filter query and "submitting" it by inserting it into the
  // queryCatalog
  ...
  auto operatorId =
      query.getQueryPlan()->getOperatorByType<LogicalMapOperator>()[0]->getId();
  queryCatalog->createQueryCatalogEntry(
      query.getQueryPlan()->toString(), query.getQueryPlan(),
      Optimizer::PlacementStrategy::BottomUp, QueryState::REGISTERED);
  ...
  constexpr auto EXPECTED_WIDTH = 9;
  const auto dataCharacteristic = WorkloadCharacteristic::create(
      Cardinality::create(Over("f1")), queryId, operatorId);
  const auto window =
      TumblingWindow::of(EventTime(Attribute("ts")), Seconds(10));
  const auto sendingPolicy = SENDING_ASAP(StatisticDataCodec::DEFAULT);
  const auto triggerCondition = NeverTrigger::create();
  ...
  // Checking if the statistic query is correct, meaning that the source
  // operator and the filter operator is still there
  auto actualSourceOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<SourceLogicalOperator>()[0];
  auto actualFilterOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<LogicalFilterOperator>()[0];

  auto expectedSourceOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<SourceLogicalOperator>()[0];
  auto expectedFilterOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<LogicalFilterOperator>()[0];
  ...
  EXPECT_TRUE(statisticQuery.getQueryPlan()
                  ->getOperatorByType<LogicalMapOperator>()
                  .empty());
}

/**
 * @brief Tests if we create a statistic query for collecting cardinality of a
 * map operator that sits behind a join operator
 */
TEST_F(DefaultStatisticQueryGeneratorTest,
  ...
  // Checking if the statistic query is correct, meaning that the source
  // operator, the filter operator and the join operator is still there
  auto actualSourceOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<SourceLogicalOperator>()[0];
  auto actualFilterOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<LogicalFilterOperator>()[0];
  auto actualJoinOperator = statisticQuery.getQueryPlan()
                                ->getOperatorByType<LogicalJoinOperator>()[0];
  ...
  auto expectedFilterOperator =
      statisticQuery.getQueryPlan()
          ->getOperatorByType<LogicalFilterOperator>()[0];
  auto expectedJoinOperator = statisticQuery.getQueryPlan()
                                  ->getOperatorByType<LogicalJoinOperator>()[0];
  EXPECT_TRUE(actualSourceOperator->equal(expectedSourceOperator));
  EXPECT_TRUE(actualFilterOperator->equal(expectedFilterOperator));
