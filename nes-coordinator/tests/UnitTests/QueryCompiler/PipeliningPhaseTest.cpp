...
class PipeliningPhaseTest : public Testing::BaseUnitTest {
 public:
  static void SetUpTestCase() {
    NES::Logger::setupLogging("PipeliningPhaseTest.log",
 ...
 protected:
  ...
  LogicalOperatorPtr watermarkAssigner1, centralWindowOperator,
      sliceCreationOperator, windowComputation, sliceMerging;
...
TEST_F(PipeliningPhaseTest, pipelineFilterQuery) {
  auto source = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  auto filter = PhysicalFilterOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), ExpressionNodePtr());
  auto sink = PhysicalSinkOperator::create(++statisticId, SchemaPtr(),
                                           SchemaPtr(), SinkDescriptorPtr());
  ...
  ASSERT_INSTANCE_OF(
      sourcePipeline->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalSourceOperator);
  auto filterPipe = sourcePipeline->getSuccessors()[0];
  ASSERT_INSTANCE_OF(
      filterPipe->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalFilterOperator);
  auto sinkPipe = filterPipe->getSuccessors()[0];
  ASSERT_INSTANCE_OF(sinkPipe->getDecomposedQueryPlan()->getRootOperators()[0],
 ...
 * @brief Input Query Plan:
 *
 * --- Physical Sink 1 --- Physical Filter --- Physical Map --- Physical Source
 * 1
 *
 * --- | Physical Sink 1 | --- | Physical Filter - Physical Map | --- | Physical
 * Source 1 |
 *
 */
TEST_F(PipeliningPhaseTest, pipelineFilterMapQuery) {
  ...
  auto filter = PhysicalFilterOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), ExpressionNodePtr());
  auto map =
      PhysicalMapOperator::create(++statisticId, SchemaPtr(), SchemaPtr(),
...
TEST_F(PipeliningPhaseTest, pipelineMultiplexQuery) {
  auto source1 = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  auto source2 = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  ...
  ASSERT_INSTANCE_OF(
      sourcePipeline1->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalSourceOperator);
  auto sourcePipeline2 = sourcePipelines[1];
  ASSERT_INSTANCE_OF(
      sourcePipeline2->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalSourceOperator);
  ASSERT_EQ(sourcePipeline1->getSuccessors()[0],
            sourcePipeline2->getSuccessors()[0]);
...
TEST_F(PipeliningPhaseTest, pipelineJoinQuery) {
  ...
  auto joinBuildLeft = PhysicalJoinBuildOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), Join::JoinOperatorHandlerPtr(),
  ...
  auto joinSink = PhysicalJoinSinkOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SchemaPtr(),
  ...
  ASSERT_INSTANCE_OF(
      leftJoinBuildPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalJoinBuildOperator);
  auto rightJoinBuildPipeline = sourcePipeline2->getSuccessors()[0];
  ASSERT_INSTANCE_OF(
      rightJoinBuildPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalJoinBuildOperator);
  ...
  ASSERT_INSTANCE_OF(
      joinSinkPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
 ...
 *                                             \
 *                                             --- Physical Join Build ---
 * Physical Multiplex Operator  --- Physical Source 2
 *                                                                                                      \
 *                                                                                                       ---
 * Physical Source 3
 */
TEST_F(PipeliningPhaseTest, pipelineJoinWithMultiplexQuery) {
  ...
  auto source2 = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  auto source3 = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  auto multiplex = PhysicalUnionOperator::create(++statisticId, SchemaPtr());
  auto joinBuildLeft = PhysicalJoinBuildOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), Join::JoinOperatorHandlerPtr(),
  ...
  ASSERT_INSTANCE_OF(
      ...
      PhysicalSourceOperator);
  ASSERT_INSTANCE_OF(
      sourcePipeline3->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalSourceOperator);
  ...
  ASSERT_EQ(sourcePipeline2->getSuccessors()[0],
            sourcePipeline3->getSuccessors()[0]);
  auto rightJoinBuildPipeline = sourcePipeline2->getSuccessors()[0];
 ...
 *  Result Query plan:
 *
 * --- Physical Sink 1  --- Physical Window Sink --- Physical Window Pre
 * Aggregation Operator --- Physical Watermark Assigner --- Physical Source 1
 *
 * --- | Physical Sink 1 | --- | Physical Window Sink | --- | Physical Window
 * Pre Aggregation --- Watermark Assigner | --- | Physical Source 1 |
 *
 */
TEST_F(PipeliningPhaseTest, pipelineWindowQuery) {
  auto source1 = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  auto windowAssignment = PhysicalWatermarkAssignmentOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(),
      WatermarkStrategyDescriptorPtr());
  auto slicePreAggregation = PhysicalSlicePreAggregationOperator::create(
      getNextOperatorId(), ++statisticId, SchemaPtr(), SchemaPtr(),
      LogicalWindowDescriptorPtr());
  auto windowSink = PhysicalWindowSinkOperator::create(
      getNextOperatorId(), ++statisticId, SchemaPtr(), SchemaPtr(),
      LogicalWindowDescriptorPtr());
  auto sink = PhysicalSinkOperator::create(++statisticId, SchemaPtr(),
                                           SchemaPtr(), SinkDescriptorPtr());
  ...
  ASSERT_INSTANCE_OF(
      preAggregationPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalWatermarkAssignmentOperator);

  auto windowSinkPipeline = preAggregationPipeline->getSuccessors()[0];
  ASSERT_INSTANCE_OF(
      windowSinkPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
 ...
 *  Result Query plan:
 *
 * --- Physical Sink 1 --- Physical Map Operator --- Physical Filter Operator
 * --- Physical Project Operator --- Physical Source 1
 *
 * --- | Physical Sink 1 | --- | Physical Map --- Physical Filter --- Physical
 * Project |--- | Physical Source 1 |
 *
 */
TEST_F(PipeliningPhaseTest, pipelineMapFilterProjectQuery) {
  auto source = PhysicalSourceOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), SourceDescriptorPtr());
  auto project =
      PhysicalProjectOperator::create(++statisticId, SchemaPtr(), SchemaPtr(),
  ...
  ASSERT_INSTANCE_OF(
      projectFilterMapPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
...
TEST_F(PipeliningPhaseTest, pipelineDemultiplex) {
  ...
  auto filter = PhysicalFilterOperator::create(
      ++statisticId, SchemaPtr(), SchemaPtr(), ExpressionNodePtr());
  auto demultiplex =
      PhysicalDemultiplexOperator::create(++statisticId, SchemaPtr());
  auto sink1 = PhysicalSinkOperator::create(++statisticId, SchemaPtr(),
                                            SchemaPtr(), SinkDescriptorPtr());
  auto sink2 = PhysicalSinkOperator::create(++statisticId, SchemaPtr(),
                                            SchemaPtr(), SinkDescriptorPtr());
  ...
  ASSERT_INSTANCE_OF(
      projectFilterPipeline->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalFilterOperator);
  ...
  ASSERT_INSTANCE_OF(
      sinkPipeline1->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalSinkOperator);

  auto sinkPipeline2 = projectFilterPipeline->getSuccessors()[1];
  ASSERT_INSTANCE_OF(
      sinkPipeline2->getDecomposedQueryPlan()->getRootOperators()[0],
      PhysicalSinkOperator);
}
