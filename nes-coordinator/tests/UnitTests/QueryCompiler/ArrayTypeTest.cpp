...
/// Check that the null-terminator has to be present for fixed-size strings
/// only.
TEST_F(ArrayTypeTest, testCheckNullTerminator) {
  ...
  EXPECT_THROW((ExecutableTypes::Array{std::array{'a', 'b'}}),
               std::runtime_error);
  EXPECT_NO_THROW((ExecutableTypes::Array{std::array{1, 2, 3}}));
  EXPECT_THROW((ExecutableTypes::Array{std::array{'1', '2', '3'}}),
               std::runtime_error);
  EXPECT_NO_THROW((ExecutableTypes::Array{std::array{'1', '2', '\0'}}));

  // From content
  EXPECT_THROW(((volatile void)ExecutableTypes::Array{'a', 'b', 'c'}),
               std::runtime_error);
  ...
  EXPECT_THROW(((volatile void)ExecutableTypes::Array{cStyleArray}),
               std::runtime_error);
  char const cStyleArrayNull[4] = {'a', 'b', '\0', '0'};
  EXPECT_NO_THROW(((volatile void)ExecutableTypes::Array{cStyleArrayNull}));
  EXPECT_NO_THROW(
      ((volatile void)ExecutableTypes::Array<char, 3>{cStyleArrayNull}));
  EXPECT_THROW(
      ((volatile void)ExecutableTypes::Array<char, 2>{cStyleArrayNull}),
      std::runtime_error);
  int const cStyleArrayInt[3] = {1, 2, 3};
  EXPECT_NO_THROW(((volatile void)ExecutableTypes::Array{cStyleArrayInt}));

  EXPECT_THROW(
      ((volatile void)ExecutableTypes::Array<char, 3>{{'a', 'b', 'c'}}),
      std::runtime_error);
  EXPECT_NO_THROW((ExecutableTypes::Array<char, 3>{{'a', 'b', '\0'}}));
  EXPECT_NO_THROW((ExecutableTypes::Array<char, 3>{{'1', '\0'}}));
  EXPECT_NO_THROW(
      (ExecutableTypes::Array<char, 3>{{'1'}}));  //< automatically append \0

  // From vector
  EXPECT_THROW((ExecutableTypes::Array<char, 3>{std::vector{'a', 'b', 'c'}}),
               std::runtime_error);
  EXPECT_NO_THROW(
      (ExecutableTypes::Array<char, 3>{std::vector{'a', 'b', '\0'}}));
  EXPECT_THROW((ExecutableTypes::Array<char, 2>{std::vector{'a', 'b', '\0'}}),
               std::runtime_error);
  EXPECT_NO_THROW((ExecutableTypes::Array<char, 2>{std::vector{'\0'}}));
  EXPECT_NO_THROW((ExecutableTypes::Array<int, 3>{std::vector{1, 2, 3}}));
  EXPECT_THROW((ExecutableTypes::Array<int, 2>{std::vector{1, 2, 3, 4, 5}}),
               std::runtime_error);
  EXPECT_THROW((ExecutableTypes::Array<int, 2>{std::vector{1}}),
               std::runtime_error);
  ...
  EXPECT_THROW((ExecutableTypes::Array<char, 1>{std::string("abc")}),
               std::runtime_error);
}

/// Test that the noexcept specifier is set correctly for initializers, i.e.,
/// noexcept iff
///   1) type is not char && (initialization by `size` elements of type or ||
///   array || c-style array) 2) type is char and initialization by less than
///   `size` elements of type char
TEST_F(ArrayTypeTest, testExceptionSpecifier) {
  ...
  // From vector
  EXPECT_TRUE(
      !noexcept(ExecutableTypes::Array<char, 4>{std::vector{'a', 'b', 'c'}}));
  EXPECT_TRUE(!noexcept(
      ExecutableTypes::Array<char, 4>{std::vector{'a', 'b', 'c', '\0'}}));
