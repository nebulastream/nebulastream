...
TEST_F(NetworkStackTest, serverMustStartAndStop) {
  try {
    ...
    auto exchangeProtocol = ExchangeProtocol(
        partMgr, std::make_shared<DummyExchangeProtocolListener>());
...
TEST_F(NetworkStackTest, dispatcherMustStartAndStop) {
  try {
    ...
    auto netManager = NetworkManager::create(
        INVALID_WORKER_NODE_ID, "127.0.0.1", *freeDataPort,
        std::move(exchangeProtocol), buffMgr);
  } catch (...) {
    FAIL();
  }
  ASSERT_EQ(true, true);
...
TEST_F(NetworkStackTest, startCloseChannel) {
  try {
    ...
    auto buffMgr =
        std::make_shared<Runtime::BufferManager>(bufferSize, buffersManaged);
    auto ep = ExchangeProtocol(partMgr,
                               std::make_shared<InternalListener>(completed));
    auto netManager =
        NetworkManager::create(INVALID_WORKER_NODE_ID, "127.0.0.1",
                               *freeDataPort, std::move(ep), buffMgr);

    auto nesPartition =
        NesPartition(INVALID_SHARED_QUERY_ID, INVALID_OPERATOR_ID,
                     PartitionId(0), SubpartitionId(0));
    ...
    std::thread t([&netManager, &completed, &nesPartition] {
      ...
      auto cnt = netManager->registerSubpartitionConsumer(
          nesPartition, netManager->getServerLocation(),
          std::make_shared<DataEmitterImpl>());
      NES_INFO("NetworkStackTest: SubpartitionConsumer registered with cnt {}",
               cnt);
    ...
    NodeLocation nodeLocation(INVALID_WORKER_NODE_ID, "127.0.0.1",
                              *freeDataPort);
    auto senderChannel = netManager->registerSubpartitionProducer(
        nodeLocation, nesPartition, buffMgr, std::chrono::seconds(1), 3);
...
TEST_F(NetworkStackTest, startCloseChannelAsyncIndefiniteRetries) {
  try {
    ...
    class DummyQueryManager : public Runtime::AbstractQueryManager {
     public:
      // explicit DummyQueryManager() : Runtime::AbstractQueryManager({}, {}, 1,
      // 1, std::make_shared<Runtime::HardwareManager>(), {}, 1, {}) {};
      explicit DummyQueryManager()
          : Runtime::AbstractQueryManager(
                {}, {}, WorkerId(1), 1,
                std::make_shared<Runtime::HardwareManager>(), 1){};
      ExecutionResult processNextTask(bool, Runtime::WorkerContext&) override {
        return ExecutionResult::Error;
      };
      void addWorkForNextPipeline(
          TupleBuffer&, Runtime::Execution::SuccessorExecutablePipeline,
          uint32_t) override{};
      void poisonWorkers() override{};
      bool addReconfigurationMessage(SharedQueryId, DecomposedQueryPlanId,
    ...
    auto netManagerReceiver =
        NetworkManager::create(INVALID_WORKER_NODE_ID, "127.0.0.1",
                               *freeDataPort, std::move(ep), buffMgrRecv, 4, 8);

    auto partMgrSend = std::make_shared<PartitionManager>();
    auto buffMgrSend =
        std::make_shared<Runtime::BufferManager>(bufferSize, buffersManaged);
    auto dummyProtocol = ExchangeProtocol(
        partMgrSend, std::make_shared<DummyExchangeProtocolListener>());
    auto senderPort = getAvailablePort();
    auto netManagerSender =
        NetworkManager::create(INVALID_WORKER_NODE_ID, "127.0.0.1", *senderPort,
                               std::move(dummyProtocol), buffMgrSend, 4, 2);
    ...
    std::thread t([&netManagerReceiver, &completed, &nesPartition] {
      // register the incoming channel
      auto cnt = netManagerReceiver->registerSubpartitionConsumer(
          nesPartition, netManagerReceiver->getServerLocation(),
          std::make_shared<DataEmitterImpl>());
      NES_INFO("NetworkStackTest: SubpartitionConsumer registered with cnt {}",
               cnt);
      auto v = completed.get_future().get();
    ...
    auto reconf = Runtime::ReconfigurationMessage(
        SharedQueryId(1), DecomposedQueryPlanId(1),
        Runtime::ReconfigurationType::ConnectionEstablished, {},
        std::make_any<uint32_t>(1));
    // setting retry times to zero will let the channel keep attempting to
    // connect indefinitely
    auto senderChannelFuture =
        netManagerSender->registerSubpartitionProducerAsync(
            nodeLocation, nesPartition, buffMgrSend, std::chrono::seconds(1), 0,
            reconf, queryManager);
    EXPECT_FALSE(queryManager->receivedCallback);
    auto start_timestamp = std::chrono::system_clock::now();
    auto timeOut = std::chrono::seconds(10);
    while (senderChannelFuture.first.wait_for(std::chrono::seconds(0)) ==
...
TEST_F(NetworkStackTest, startCloseMaxChannel) {
  try {
    ...
    class InternalListener : public Network::ExchangeProtocolListener {
     public:
      explicit InternalListener(std::promise<bool>& p,
                                uint32_t maxExpectedConnections)
          : completed(p),
            numReceivedEoS(0),
            maxExpectedConnections(maxExpectedConnections) {}
    ...
    auto buffMgr =
        std::make_shared<Runtime::BufferManager>(bufferSize, buffersManaged);
    auto ep = ExchangeProtocol(partMgr, std::make_shared<InternalListener>(
                                            completed, maxExpectedConnections));
    auto netManagerReceiver =
        NetworkManager::create(INVALID_WORKER_NODE_ID, "127.0.0.1",
                               *freeDataPort, std::move(ep), buffMgr, 4, 8);
    ...
    auto netManagerSender =
        NetworkManager::create(INVALID_WORKER_NODE_ID, "127.0.0.1", *senderPort,
                               std::move(dummyProtocol), buffMgr, 4, 2);

    struct DataEmitterImpl : public DataEmitter {
      void emitWork(TupleBuffer&, bool) override {}
    };

    std::thread t([&netManagerReceiver, &netManagerSender, &completed,
                   &connectionsReady] {
      // register the incoming channel

      for (auto i = 0; i < maxExpectedConnections; ++i) {
        auto nesPartition = NesPartition(SharedQueryId(i), INVALID_OPERATOR_ID,
                                         PartitionId(0), SubpartitionId(0));
        ASSERT_TRUE(netManagerReceiver->registerSubpartitionConsumer(
            nesPartition, netManagerSender->getServerLocation(),
            std::make_shared<DataEmitterImpl>()));
      }
    ...
    for (auto i = 0; i < maxExpectedConnections; ++i) {
      auto nesPartition = NesPartition(SharedQueryId(i), INVALID_OPERATOR_ID,
                                       PartitionId(0), SubpartitionId(0));
      channels.emplace_back(netManagerSender->registerSubpartitionProducer(
          nodeLocation, nesPartition, buffMgr, std::chrono::seconds(1), 3));
      if (!channels.back()) {
        break;
      }
...
TEST_F(NetworkStackTest, testCorrectHandlingEOS) {
  ...
  // With these parameters, we should have around 5 handling threads and 4
  // buffers. Therefore, we guarantee that an EOS is being processed together
  // with a data buffer
  ...
  try {
    class ExchangeListener : public ExchangeProtocolListener {
     public:
      ...
      ExchangeListener(std::atomic<std::uint64_t>& bufferReceived,
                       std::promise<bool>& completedProm)
          : completedProm(completedProm), bufferReceived(bufferReceived) {}
    ...
    auto netManager = NetworkManager::create(
        ...
        ExchangeProtocol(partMgr, std::make_shared<ExchangeListener>(
                                      bufferReceived, completedProm)),
        buffMgr, -1 /*senderHighWatermark*/, numNetworkReceivingThreads);
    ...
    std::thread t([&netManager, &nesPartition, &completedProm, totalNumBuffer,
      ...
      double throughput =
          (bytes * 1'000'000'000) / (elapsed.count() * 1024.0 * 1024.0);
    ...
    if (senderChannel == nullptr) {
      ...
      senderChannel->close(Runtime::QueryTerminationType::Graceful, 1,
                           totalNumBuffer);
      senderChannel.reset();
      netManager->unregisterSubpartitionProducer(nesPartition);
    }
  ...
  try {
    ...
    if (senderChannel == nullptr) {
      ...
      for (uint64_t i = 0; i < totalNumBuffer; ++i) {
        ...
        buffer.setSequenceNumber(i + 1);
        ASSERT_EQ(buffer.getNumberOfChildrenBuffer(),
                  bufferSize / sizeof(Record));
  ...
  try {
    ...
    class ExchangeListener : public ExchangeProtocolListener {
     ...
     public:
      ...
      ExchangeListener(std::promise<bool>& serverError,
                       std::promise<bool>& channelError)
      ...
      void onServerError(Messages::ErrorMessage errorMsg) override {
        ...
        ASSERT_EQ(errorMsg.getErrorType(),
    ...
    auto channel = netManager->registerSubpartitionProducer(
        nodeLocation,
        NesPartition(SharedQueryId(1), OperatorId(22), PartitionId(333),
                     SubpartitionId(4445)),
        buffMgr, std::chrono::seconds(1), retryTimes);
...
TEST_F(NetworkStackTest, testMassiveMultiSending) {
  ...
  for (uint64_t i = 0ull; i < numSendingThreads; ++i) {
    nesPartitions.emplace_back(SharedQueryId(i), OperatorId(i + 10),
                               PartitionId(i + 20), SubpartitionId(i + 30));
  ...
  try {
    class ExchangeListenerImpl : public ExchangeProtocolListener {
     ...
     public:
      ExchangeListenerImpl(std::array<std::atomic<std::uint64_t>,
                                      numSendingThreads>& bufferCounter,
                           std::vector<std::promise<bool>>& completedPromises)
      ...
      void onEndOfStream(Messages::EndOfStreamMessage p) override {
        completedPromises
            [p.getChannelId().getNesPartition().getQueryId().getRawValue()]
                .set_value(true);
      }
      void onEvent(NesPartition, Runtime::BaseEvent&) override {}
    };
    ...
    auto netManager = NetworkManager::create(
        INVALID_WORKER_NODE_ID, "127.0.0.1", *freeDataPort,
        ExchangeProtocol(partMgr, std::make_shared<ExchangeListenerImpl>(
                                      bufferCounter, completedPromises)),
        buffMgr);

    struct DataEmitterImpl : public DataEmitter {
      void emitWork(TupleBuffer&, bool) override {}
    };
    std::thread receivingThread([&netManager, &nesPartitions,
                                 &completedPromises, this] {
      // register the incoming channel
      NodeLocation nodeLocation{INVALID_WORKER_NODE_ID, "127.0.0.1",
                                *freeDataPort};
      for (NesPartition p : nesPartitions) {
        ...
        netManager->registerSubpartitionConsumer(
            p, nodeLocation, std::make_shared<DataEmitterImpl>());
      }
    ...
    for (auto i = 0ULL; i < numSendingThreads; ++i) {
      std::thread sendingThread([&, i] {
        // register the incoming channel
        NesPartition nesPartition(SharedQueryId(i), OperatorId(i + 10),
                                  PartitionId(i + 20), SubpartitionId(i + 30));
        auto senderChannel = netManager->registerSubpartitionProducer(
            nodeLocation, nesPartition, buffMgr, std::chrono::seconds(2), 10);
        ...
        if (senderChannel == nullptr) {
          ...
          for (auto sentBuffers = 0ULL; sentBuffers < totalNumBuffer;
               ++sentBuffers) {
            auto buffer = buffMgr->getBufferBlocking();
            for (uint64_t j = 0; j < bufferSize / sizeof(uint64_t); ++j) {
              buffer.getBuffer<uint64_t>()[j] = j;
          ...
          senderChannel->close(Runtime::QueryTerminationType::Graceful, 0,
                               totalNumBuffer);
          senderChannel.reset();
          netManager->unregisterSubpartitionProducer(nesPartition);
        }
