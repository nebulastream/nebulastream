...
struct __attribute__((packed)) IngestionRecord {
  IngestionRecord(uint64_t userId, uint64_t pageId, uint64_t campaignId,
                  uint64_t adType, uint64_t eventType, uint64_t currentMs,
                  uint64_t ip)
      : userId(userId), pageId(pageId), campaignId(campaignId), adType(adType),
        eventType(eventType), currentMs(currentMs), ip(ip) {}
  ...
  [[nodiscard]] std::string toString() const {
    return "Record(userId=" + std::to_string(userId) +
...
public:
  MockDataSource(const SchemaPtr &schema,
      ...
      : DataSource(schema, bufferManager, queryManager, operatorId,
                   INITIAL_ORIGIN_ID, INVALID_STATISTIC_ID,
                   numSourceLocalBuffers, gatheringMode,
                   "defaultPhysicalStreamName", executableSuccessors){
            // nop
        };

  static auto
  create(const SchemaPtr &schema, Runtime::BufferManagerPtr bufferManager,
         Runtime::QueryManagerPtr queryManager, OperatorId operatorId,
         size_t numSourceLocalBuffers, GatheringMode gatheringMode,
         std::vector<Runtime::Execution::SuccessorExecutablePipeline>
             executableSuccessors) {
    return std::make_shared<MockDataSource>(
        schema, bufferManager, queryManager, operatorId, numSourceLocalBuffers,
...
public:
  ...
  MOCK_METHOD(std::string, toString, (), (const));
  MOCK_METHOD(SourceType, getType, (), (const));
  MOCK_METHOD(void, emitWork,
              (Runtime::TupleBuffer & buffer, bool addBufferMetaData));
  MOCK_METHOD(void, recycleUnpooledBuffer,
              (Runtime::detail::MemorySegment * buffer));

  Runtime::TupleBuffer getRecyclableBuffer() {
    auto *p = new uint8_t[5];
    auto fakeBuffer =
        Runtime::TupleBuffer::wrapMemory(p, sizeof(uint8_t) * 5, this);
    return fakeBuffer;
  }
...
private:
  ...
  FRIEND_TEST(
      SourceTest,
      testDataSourceIngestionRoutineBufWithValueWithTooSmallIngestionRate);
  FRIEND_TEST(SourceTest, testDataSourceKFRoutineBufWithValue);
  FRIEND_TEST(SourceTest,
              testDataSourceKFRoutineBufWithValueZeroIntervalUpdate);
  FRIEND_TEST(
      SourceTest,
      testDataSourceKFRoutineBufWithValueIntervalUpdateNonZeroInitialInterval);
...
public:
  BinarySourceProxy(
      const SchemaPtr &schema, Runtime::BufferManagerPtr bufferManager,
      Runtime::QueryManagerPtr queryManager, const std::string &file_path,
      ...
      : BinarySource(schema, bufferManager, queryManager, file_path, operatorId,
                     ...
                     "defaultPhysicalStreamName", successors){};
...
public:
  CSVSourceProxy(
      ...
      Runtime::QueryManagerPtr queryManager, CSVSourceTypePtr sourceConfig,
      OperatorId operatorId, size_t numSourceLocalBuffers,
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
      : CSVSource(schema, bufferManager, queryManager, sourceConfig, operatorId,
                  INVALID_ORIGIN_ID, INVALID_STATISTIC_ID,
                  numSourceLocalBuffers, GatheringMode::INTERVAL_MODE,
                  "defaultPhysicalStreamName", successors){};
...
public:
  TCPSourceProxy(
      SchemaPtr schema, Runtime::BufferManagerPtr bufferManager,
      Runtime::QueryManagerPtr queryManager, TCPSourceTypePtr sourceConfig,
      OperatorId operatorId, size_t numSourceLocalBuffers,
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
      : TCPSource(schema, bufferManager, queryManager, sourceConfig, operatorId,
                  INVALID_ORIGIN_ID, INVALID_STATISTIC_ID,
                  numSourceLocalBuffers, GatheringMode::INTERVAL_MODE,
                  "defaultPhysicalStreamName", successors){};
...
public:
  GeneratorSourceProxy(
      SchemaPtr schema, Runtime::BufferManagerPtr bufferManager,
      Runtime::QueryManagerPtr queryManager, uint64_t numbersOfBufferToProduce,
      OperatorId operatorId, size_t numSourceLocalBuffers,
      GatheringMode gatheringMode,
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
      : GeneratorSource(schema, bufferManager, queryManager,
                        numbersOfBufferToProduce, operatorId, INVALID_ORIGIN_ID,
                        INVALID_STATISTIC_ID, numSourceLocalBuffers,
                        gatheringMode, successors,
...
public:
  DefaultSourceProxy(
      ...
      const uint64_t numbersOfBufferToProduce, uint64_t gatheringInterval,
      OperatorId operatorId, size_t numSourceLocalBuffers,
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
      : DefaultSource(schema, bufferManager, queryManager,
                      numbersOfBufferToProduce, gatheringInterval, operatorId,
                      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID,
...
public:
  LambdaSourceProxy(
      ...
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
      : LambdaSource(
            schema, bufferManager, queryManager, numbersOfBufferToProduce,
            gatheringValue, std::move(generationFunction), operatorId,
            INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, numSourceLocalBuffers,
            gatheringMode, 0, 0, "defaultPhysicalStreamName", successors){};
...
public:
  MonitoringSourceProxy(
      ...
      std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors =
          {})
      : MonitoringSource(metricCollector, waitTime, bufferManager, queryManager,
                         operatorId, INVALID_ORIGIN_ID, INVALID_STATISTIC_ID,
                         numSourceLocalBuffers, "defaultPhysicalStreamName",
                         successors){};
};
...
public:
  MockedPipelineExecutionContext(Runtime::QueryManagerPtr queryManager,
      ...
      : PipelineExecutionContext(
            ...
            [sink](Runtime::TupleBuffer &buffer,
                   Runtime::WorkerContextRef worker) {
...
public:
  void SetUp() override {
    ...
    this->nodeEngine =
        Runtime::NodeEngineBuilder::create(workerConfigurations)
            ...
            .build();
    this->path_to_file = std::filesystem::path(TEST_DATA_DIRECTORY) /
                         "ysb-tuples-100-campaign-100.csv";
    this->path_to_file_head = std::filesystem::path(TEST_DATA_DIRECTORY) /
                              "ysb-tuples-100-campaign-100-head.csv";
    this->path_to_bin_file = std::filesystem::path(TEST_DATA_DIRECTORY) /
                             "ysb-tuples-100-campaign-100.bin";
    this->path_to_decimals_file =
        std::filesystem::path(TEST_DATA_DIRECTORY) / "decimals.csv";
    this->schema =
        Schema::create()
    ...
    this->numberOfTuplesToProcess =
        this->numberOfBuffers * (this->buffer_size / this->tuple_size);
    this->operatorId = 1;
    this->originId = 1;
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down SourceTest test class.");
  }
  ...
  std::optional<Runtime::TupleBuffer> GetEmptyBuffer() {
    return this->nodeEngine->getBufferManager()->getBufferBlocking();
  }
  ...
  createExecutablePipeline(
      std::shared_ptr<MockedExecutablePipeline> executableStage,
      std::shared_ptr<SinkMedium> sink) {
    auto context = std::make_shared<MockedPipelineExecutionContext>(
        this->nodeEngine->getQueryManager(), sink);
    return Runtime::Execution::ExecutablePipeline::create(
        INVALID_PIPELINE_ID, SharedQueryId(queryId),
        DecomposedQueryPlanId(queryId), this->nodeEngine->getQueryManager(),
        context, executableStage, 1, {sink});
  }
  ...
  uint64_t tuple_size, buffer_size, numberOfBuffers, numberOfTuplesToProcess,
      operatorId, originId, statisticId, numSourceLocalBuffersDefault,
...
TEST_F(SourceTest, testDataSourceGetOperatorId) {
  const DataSourcePtr source = createDefaultDataSourceWithSchemaForOneBuffer(
      ...
      this->nodeEngine->getQueryManager(), OperatorId(operatorId),
      OriginId(originId), this->statisticId, this->numSourceLocalBuffersDefault,
      "defaultPhysicalSourceName",
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
...
TEST_F(SourceTest, DISABLED_testDataSourceEmptySuccessors) {
  try {
    const DataSourcePtr source = createDefaultDataSourceWithSchemaForOneBuffer(
        this->schema, this->nodeEngine->getBufferManager(),
        this->nodeEngine->getQueryManager(), OperatorId(operatorId),
        OriginId(originId), this->statisticId,
        this->numSourceLocalBuffersDefault, "defaultPhysicalSourceName", {});
...
TEST_F(SourceTest, testDataSourceRunningImmediately) {
  MockDataSourceWithRunningRoutine mDataSource(
      this->schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), OperatorId(operatorId),
      this->numSourceLocalBuffersDefault, GatheringMode::INTERVAL_MODE,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
...
TEST_F(SourceTest, testDataSourceStartSideEffectRunningTrue) {
  MockDataSourceWithRunningRoutine mDataSource(
      ...
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ON_CALL(mDataSource, getType())
      .WillByDefault(Return(SourceType::DEFAULT_SOURCE));
...
TEST_F(SourceTest, testDataSourceHardStopSideEffect) {
  ...
  EXPECT_TRUE(mDataSource.start());
  EXPECT_TRUE(mDataSource.isRunning());
  EXPECT_EQ(mDataSource.wasGracefullyStopped,
  ...
  EXPECT_EQ(mDataSource.wasGracefullyStopped,
            Runtime::QueryTerminationType::Graceful); // private side-effect,
                                                      // use FRIEND_TEST
}
...
TEST_F(SourceTest, testDataSourceGracefulStopSideEffect) {
  ...
  EXPECT_FALSE(mDataSource.isRunning());
  EXPECT_EQ(mDataSource.wasGracefullyStopped,
            Runtime::QueryTerminationType::HardStop); // private side-effect,
                                                      // use FRIEND_TEST
}
...
TEST_F(SourceTest, testDataSourceRunningRoutineGatheringInterval) {
  auto mDataSource = MockDataSource::create(
      ...
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ON_CALL(*mDataSource, runningRoutineWithGatheringInterval())
      .WillByDefault(Return());
  EXPECT_CALL(*mDataSource, runningRoutineWithGatheringInterval())
      .Times(Exactly(1));
...
TEST_F(SourceTest, testDataSourceRunningRoutineIngestion) {
  MockDataSource mDataSource(
      this->schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), OperatorId(operatorId),
      this->numSourceLocalBuffersDefault, GatheringMode::INGESTION_RATE_MODE,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ON_CALL(mDataSource, runningRoutineWithIngestionRate())
      .WillByDefault(Return());
...
TEST_F(SourceTest, DISABLED_testDataSourceRunningRoutineKalmanFilter) {
  MockDataSource mDataSource(
      this->schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), OperatorId(operatorId),
      this->numSourceLocalBuffersDefault, GatheringMode::ADAPTIVE_MODE,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ON_CALL(mDataSource, runningRoutineAdaptiveGatheringInterval())
      .WillByDefault(Return());
  EXPECT_CALL(mDataSource, runningRoutineAdaptiveGatheringInterval())
      .Times(Exactly(1));
  mDataSource.runningRoutine();
}
...
TEST_F(SourceTest, testDataSourceGatheringIntervalRoutineBufWithValue) {
  ...
  // create sink
  auto sink =
      createCSVFileSink(this->schema, SharedQueryId(this->queryId),
                        DecomposedQueryPlanId(this->queryId), this->nodeEngine,
                        1, "source-test-freq-routine.csv", false);
  ...
  ON_CALL(*mDataSource, getType())
      .WillByDefault(Return(SourceType::CSV_SOURCE));
  ...
  auto executionPlan = Runtime::Execution::ExecutableQueryPlan::create(
      SharedQueryId(this->queryId), DecomposedQueryPlanId(this->queryId),
      {mDataSource}, {sink}, {pipeline}, this->nodeEngine->getQueryManager(),
...
TEST_F(SourceTest, testDataSourceIngestionRoutineBufWithValue) {
  ...
  auto sink =
      createCSVFileSink(this->schema, SharedQueryId(this->queryId),
                        DecomposedQueryPlanId(this->queryId), this->nodeEngine,
                        1, "source-test-ingest-routine.csv", false);
  // get mocked pipeline to add to source
  auto pipeline = this->createExecutablePipeline(executableStage, sink);
  // mock query manager for passing addEndOfStream
  DataSourceProxyPtr mDataSource = createDataSourceProxy(
  ...
  ON_CALL(*mDataSource, toString()).WillByDefault(Return("MOCKED SOURCE"));
  ON_CALL(*mDataSource, getType())
      .WillByDefault(Return(SourceType::LAMBDA_SOURCE));
  ON_CALL(*mDataSource, receiveData()).WillByDefault(Return(fakeBuf));
  ON_CALL(*mDataSource, emitWork(_, _)).WillByDefault(Return());
  auto executionPlan = Runtime::Execution::ExecutableQueryPlan::create(
      SharedQueryId(this->queryId), DecomposedQueryPlanId(this->queryId),
...
TEST_F(SourceTest, DISABLED_testDataSourceKFRoutineBufWithValue) {
    ...
    // create sink
    auto sink =
        createCSVFileSink(this->schema, this->queryId, this->queryId,
this->nodeEngine, 1, "source-test-kf-routine.csv", false);
    // get mocked pipeline to add to source
    auto pipeline = this->createExecutablePipeline(executableStage, sink);
    // mock query manager for passing addEndOfStream
    ...
    ON_CALL(*mDataSource, toString()).WillByDefault(Return("MOCKED SOURCE"));
    ON_CALL(*mDataSource,
getType()).WillByDefault(Return(SourceType::LAMBDA_SOURCE));
    ON_CALL(*mDataSource, receiveData()).WillByDefault(Return(fakeBuf));
    ON_CALL(*mDataSource, emitWork(_, _)).WillByDefault(Return());
    auto executionPlan =
Runtime::Execution::ExecutableQueryPlan::create(this->queryId, this->queryId,
                                                                         {mDataSource},
                                                                         {sink},
                                                                         {pipeline},
                                                                         this->nodeEngine->getQueryManager(),
    ...
    ASSERT_EQ(this->nodeEngine->getQueryStatus(this->queryId),
Runtime::Execution::ExecutableQueryPlanStatus::Running);
    EXPECT_CALL(*mDataSource, receiveData()).Times(Exactly(1));
    EXPECT_CALL(*mDataSource, emitWork(_,
_)).Times(Exactly(1)).WillOnce(InvokeWithoutArgs([&]() { mDataSource->running =
false; return;
    }));
    mDataSource->runningRoutine();
    EXPECT_FALSE(mDataSource->running);
...
Runtime::Execution::ExecutableQueryPlan::create(this->queryId, this->queryId,
    ...
    ASSERT_TRUE(this->nodeEngine->startQuery(this->queryId));
    ASSERT_EQ(this->nodeEngine->getQueryStatus(this->queryId),
Runtime::Execution::ExecutableQueryPlanStatus::Running); auto oldInterval =
mDataSource->gatheringInterval; EXPECT_CALL(*mDataSource,
receiveData()).Times(Exactly(1)); EXPECT_CALL(*mDataSource, emitWork(_,
_)).Times(Exactly(1)).WillOnce(InvokeWithoutArgs([&]() { mDataSource->running =
false; return;
    }));
    mDataSource->runningRoutine();
...
TEST_F(SourceTest, testDataSourceOpen) {
  DataSourceProxy mDataSource(
      ...
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  // EXPECT_ANY_THROW(mDataSource.bufferManager->getAvailableBuffers());
  // currently not possible w/ Error: success :)
  mDataSource.open();
...
TEST_F(SourceTest, testBinarySourceGetType) {
  BinarySourceProxy bDataSource(
      ...
      OperatorId(operatorId), this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
...
TEST_F(SourceTest, testBinarySourceWrongPath) {
  ASSERT_THROW(
      try {
        ...
        ASSERT_NE(msg.find(std::string("Binary input file is not valid")),
                  std::string::npos);
        throw;
      },
...
TEST_F(SourceTest, testBinarySourceFillBufferRandomTimes) {
  ...
  for (int i = 0; i < iterations; ++i) {
    bDataSource.fillBuffer(*buf);
    EXPECT_EQ(bDataSource.getNumberOfGeneratedTuples(),
              (i + 1) * numberOfTuplesToProcess);
    EXPECT_EQ(bDataSource.getNumberOfGeneratedBuffers(),
              (i + 1) * numberOfBuffers);
  }
  EXPECT_EQ(bDataSource.getNumberOfGeneratedTuples(),
            iterations * numberOfTuplesToProcess);
  EXPECT_EQ(bDataSource.getNumberOfGeneratedBuffers(),
            iterations * numberOfBuffers);
}
...
TEST_F(SourceTest, testBinarySourceFillBufferContents) {
  ...
  EXPECT_TRUE((!strcmp(content->event_type, "view") ||
               !strcmp(content->event_type, "click") ||
...
TEST_F(SourceTest, testCSVSourceGetType) {
  ...
  CSVSourceProxy csvDataSource(
      this->schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), sourceConfig, OperatorId(operatorId),
      this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
...
TEST_F(SourceTest, testCSVSourceWrongFilePath) {
  ...
  try {
    CSVSourceProxy csvDataSource(
        this->schema, this->nodeEngine->getBufferManager(),
        this->nodeEngine->getQueryManager(), csvSourceType,
        OperatorId(operatorId), this->numSourceLocalBuffersDefault,
        {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                          DecomposedQueryPlanId(1))});
    ...
    EXPECT_TRUE(
        msg.find(std::string("Could not determine absolute pathname")) !=
        std::string::npos);
  }
}
...
TEST_F(SourceTest, testCSVSourceFillBufferFileEnded) {
  ...
  Runtime::MemoryLayouts::RowLayoutPtr layoutPtr =
      Runtime::MemoryLayouts::RowLayout::create(
          schema, this->nodeEngine->getBufferManager()->getBufferSize());
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
...
TEST_F(SourceTest, testCSVSourceFillBufferOnceColumnLayout) {
  ...
  std::shared_ptr<Runtime::MemoryLayouts::ColumnLayout> layoutPtr =
      Runtime::MemoryLayouts::ColumnLayout::create(
          schema, this->nodeEngine->getBufferManager()->getBufferSize());
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
  csvDataSource.fillBuffer(buffer);
...
TEST_F(SourceTest, testCSVSourceFillBufferFullFileColumnLayout) {
  ...
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
  while (csvDataSource.getNumberOfGeneratedBuffers() <
         expectedNumberOfBuffers) { // relative to file size
    csvDataSource.fillBuffer(buffer);
  ...
  EXPECT_EQ(csvDataSource.getNumberOfGeneratedBuffers(),
...
TEST_F(SourceTest, testCSVSourceIntTypes) {
  ...
  std::string path_to_int_file =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "every-int.csv";
  CSVSourceTypePtr sourceConfig =
      CSVSourceType::create("LogicalSourceName", "PhysicalSourceName");
  ...
  sourceConfig->setGatheringInterval(this->gatheringInterval);
  CSVSourceProxy csvDataSource(
      int_schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), sourceConfig, OperatorId(operatorId),
      this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ...
  Runtime::MemoryLayouts::RowLayoutPtr layoutPtr =
      Runtime::MemoryLayouts::RowLayout::create(
          int_schema, this->nodeEngine->getBufferManager()->getBufferSize());
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
  csvDataSource.fillBuffer(buffer);
...
TEST_F(SourceTest, testCSVSourceFloatTypes) {
  ...
  SchemaPtr float_schema = Schema::create()
                               ->addField("float64", BasicType::FLOAT64)
                               ->addField("float32", BasicType::FLOAT32);
  std::string path_to_float_file =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "every-float.csv";
  CSVSourceTypePtr sourceConfig =
      CSVSourceType::create("LogicalSourceName", "PhysicalSourceName");
  ...
  sourceConfig->setGatheringInterval(this->gatheringInterval);
  CSVSourceProxy csvDataSource(
      float_schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), sourceConfig, OperatorId(operatorId),
      this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ...
  Runtime::MemoryLayouts::RowLayoutPtr layoutPtr =
      Runtime::MemoryLayouts::RowLayout::create(
          float_schema, this->nodeEngine->getBufferManager()->getBufferSize());
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
  csvDataSource.fillBuffer(buffer);
...
TEST_F(SourceTest, testCSVSourceBooleanTypes) {
  ...
  std::string path_to_bool_file =
      std::filesystem::path(TEST_DATA_DIRECTORY) / "every-boolean.csv";
  ...
  CSVSourceProxy csvDataSource(
      bool_schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), csvSourceType,
      OperatorId(operatorId), this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ...
  Runtime::MemoryLayouts::RowLayoutPtr layoutPtr =
      Runtime::MemoryLayouts::RowLayout::create(
          bool_schema, this->nodeEngine->getBufferManager()->getBufferSize());
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
  csvDataSource.fillBuffer(buffer);
...
TEST_F(SourceTest, testCSVSourceCommaFloatingPoint) {
  ...
  CSVSourceProxy csvDataSource(
      this->decimalsSchema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), csvSourceType,
      OperatorId(operatorId), this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ...
  Runtime::MemoryLayouts::RowLayoutPtr layoutPtr =
      Runtime::MemoryLayouts::RowLayout::create(
          this->decimalsSchema,
          this->nodeEngine->getBufferManager()->getBufferSize());
  Runtime::MemoryLayouts::TestTupleBuffer buffer =
      Runtime::MemoryLayouts::TestTupleBuffer(layoutPtr, *buf);
  csvDataSource.fillBuffer(buffer);
...
TEST_F(SourceTest, TCPSourceInit) {
  ...
  TCPSourceTypePtr sourceConfig =
      TCPSourceType::create("LogicalSourceName", "PhysicalSourceName");
...
TEST_F(SourceTest, TCPSourcePrint) {
  ...
  std::string expected =
      "TCPSOURCE(SCHEMA(user_id:ArrayType page_id:ArrayType "
      "campaign_id:ArrayType ad_type:ArrayType event_type:ArrayType "
...
TEST_F(SourceTest, testGeneratorSourceGetType) {
  GeneratorSourceProxy genDataSource(
      this->schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), 1, OperatorId(operatorId),
      this->numSourceLocalBuffersDefault, GatheringMode::INGESTION_RATE_MODE,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
...
TEST_F(SourceTest, testDefaultSourceGetType) {
  DefaultSourceProxy defDataSource(
      this->schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), 1, 1000, OperatorId(operatorId),
      this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
...
TEST_F(SourceTest, testLambdaSourceInitAndTypeInterval) {
  ...
  auto func = [](NES::Runtime::TupleBuffer &buffer,
    ...
    for (uint64_t i = 0; i < numberOfTuplesToProduce; i++) {
      ...
      std::cout << "Write rec i=" << i
                << " content=" << ysbRecords[i].toString()
                << " size=" << sizeof(IngestionRecord)
                << " addr=" << &ysbRecords[i] << std::endl;
    }
  };

  LambdaSourceProxy lambdaDataSource(
      lambdaSchema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), numBuffers, 0, func,
      OperatorId(operatorId), 12, GatheringMode::INTERVAL_MODE,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ...
  while (lambdaDataSource.getNumberOfGeneratedBuffers() < numBuffers) {
    ...
    EXPECT_EQ(resBuf->getNumberOfTuples(),
              lambdaDataSource.numberOfTuplesToProduce);
    auto ysbRecords = resBuf->getBuffer<IngestionRecord>();
    for (uint64_t i = 0; i < lambdaDataSource.numberOfTuplesToProduce; ++i) {
      EXPECT_TRUE(0 <= ysbRecords[i].campaignId &&
                  ysbRecords[i].campaignId < 10000);
  ...
  EXPECT_EQ(lambdaDataSource.getNumberOfGeneratedTuples(),
            numBuffers * lambdaDataSource.numberOfTuplesToProduce);
}
...
TEST_F(SourceTest, testLambdaSourceInitAndTypeIngestion) {
  ...
  LambdaSourceProxy lambdaDataSource(
      lambdaSchema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), numBuffers, 1, func,
      OperatorId(operatorId), 12, GatheringMode::INGESTION_RATE_MODE,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
  ASSERT_EQ(lambdaDataSource.getType(), SourceType::LAMBDA_SOURCE);
...
TEST_F(SourceTest, testIngestionRateFromQuery) {
  ...
  auto lambdaSourceType =
      LambdaSourceType::create("input1", "test_stream1", std::move(func1), 22,
                               11, GatheringMode::INTERVAL_MODE);
  ...
  std::string outputFilePath =
      getTestResourceFolder() / "testIngestionRateFromQuery.out";
  remove(outputFilePath.c_str());
  string query = R"(Query::from("input1").sink(FileSinkDescriptor::create(")" +
  ...
  auto start = std::chrono::duration_cast<std::chrono::seconds>(
                   ...
                   .count();
  string expectedContent = "input1$id:INTEGER(64 bits),input1$value:INTEGER(64 "
                           "bits),input1$timestamp:INTEGER(64 bits)\n"
                           "1,1,0\n"
                           "1,1,0\n"
                           "1,1,0\n"
                           "1,1,1\n"
  ...
  EXPECT_TRUE(
      TestUtils::checkOutputOrTimeout(expectedContent, outputFilePath, 60));
...
TEST_F(SourceTest, testMonitoringSourceInitAndGetType) {
  ...
  auto metrics = std::set<Monitoring::MetricType>(
      {Monitoring::MetricType::CpuMetric, Monitoring::MetricType::DiskMetric,
  ...
  MonitoringSourceProxy monitoringDataSource(
      testCollector, MonitoringSource::DEFAULT_WAIT_TIME,
      this->nodeEngine->getBufferManager(), this->nodeEngine->getQueryManager(),
      OperatorId(operatorId), this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
...
TEST_F(SourceTest, DISABLED_testMonitoringSourceBufferSmallerThanMetric) {
  ...
  auto cpuMetrics =
      testCollector->readMetric()->getValue<Monitoring::CpuMetricsWrapper>();
  ...
  Runtime::BufferManagerPtr bufferManager =
      std::make_shared<Runtime::BufferManager>(bufferSize, 12);
  auto tupleBuffer = bufferManager->getUnpooledBuffer(bufferSize)
                         .value(); // MetricCollectorTest.cpp l. 80

  MonitoringSourceProxy monitoringDataSource(
      testCollector, MonitoringSource::DEFAULT_WAIT_TIME, bufferManager,
      this->nodeEngine->getQueryManager(), OperatorId(operatorId),
      this->numSourceLocalBuffersDefault,
      {std::make_shared<NullOutputSink>(this->nodeEngine, 1, SharedQueryId(1),
                                        DecomposedQueryPlanId(1))});
