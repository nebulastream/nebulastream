...
struct __attribute__((packed)) YSBRecordTuple {
  ...
  YSBRecordTuple(const std::string& user_id, const std::string& page_id,
                 const std::string& campaign_id, const std::string& ad_type,
                 const std::string& event_type, uint64_t timestamp,
                 uint32_t ip) {
#define STR_TO_FIX_CHAR(field_name)                \
  std::memcpy(this->field_name, field_name.data(), \
              std::min(field_name.size(), sizeof(this->field_name)));
...
#define FIX_CHAR_CMP(field) \
  (!std::strncmp(lhs.field, rhs.field, sizeof(lhs.field)))
    return FIX_CHAR_CMP(user_id) && FIX_CHAR_CMP(page_id) &&
           FIX_CHAR_CMP(campaign_id) && FIX_CHAR_CMP(ad_type) &&
           FIX_CHAR_CMP(event_type) && lhs.timestamp == rhs.timestamp &&
           lhs.ip == rhs.ip;
#undef FIX_CHAR_CMP
  }
  friend bool operator!=(const YSBRecordTuple& lhs, const YSBRecordTuple& rhs) {
...
class MillisecondIntervalTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MillisecondIntervalTest.log",
  ...
  void SetUp() override {
    ...
    this->nodeEngine =
        Runtime::NodeEngineBuilder::create(workerConfigurations)
    ...
    path_to_file = std::filesystem::path(TEST_DATA_DIRECTORY) /
                   "ysb-tuples-100-campaign-100.csv";
...
TEST_F(MillisecondIntervalTest, testPipelinedCSVSource) {
  ...
  uint64_t numberOfTuplesToProcess =
      numberOfBuffers * (buffer_size / tuple_size);

  auto sink = CollectTestSink<YSBRecordTuple>::create(schema, this->nodeEngine);

  CSVSourceTypePtr csvSourceType =
      CSVSourceType::create("testStream", "physical_test");
  ...
  auto source = createCSVFileSource(
      schema, this->nodeEngine->getBufferManager(),
      this->nodeEngine->getQueryManager(), csvSourceType, OperatorId(1),
      INVALID_ORIGIN_ID, INVALID_STATISTIC_ID, 12, defaultPhysicalStreamName,
      {sink});

  auto executionPlan = ExecutableQueryPlan::create(
      SharedQueryId(queryId), DecomposedQueryPlanId(queryId), {source}, {sink},
      {}, this->nodeEngine->getQueryManager(),
      this->nodeEngine->getBufferManager());
  EXPECT_TRUE(this->nodeEngine->registerExecutableQueryPlan(executionPlan));
  EXPECT_TRUE(
      this->nodeEngine->startQuery(executionPlan->getSharedQueryId(),
                                   executionPlan->getDecomposedQueryPlanId()));
  EXPECT_EQ(this->nodeEngine->getQueryStatus(SharedQueryId(queryId)),
            ExecutableQueryPlanStatus::Running);
  ...
  EXPECT_EQ(theThing[0], YSBRecordTuple("0", "0", "0", "banner78", "view",
                                        1554420890327, 16909060));
  std::vector<long> userIds;
  std::transform(theThing.begin(), theThing.end(), std::back_inserter(userIds),
                 [](auto& t) {
...
TEST_F(MillisecondIntervalTest,
  ...
  auto testSchema =
      Schema::create()->addField(createField("campaign_id", BasicType::UINT64));
  crd->getSourceCatalogService()->registerLogicalSource("testStream",
                                                        testSchema);
  ...
  QueryId queryId = requestHandlerService->validateAndQueueAddQueryRequest(
      queryString, Optimizer::PlacementStrategy::BottomUp);
  EXPECT_NE(queryId, INVALID_QUERY_ID);
  auto globalQueryPlan = crd->getGlobalQueryPlan();
  EXPECT_TRUE(TestUtils::waitForQueryToStart(queryId, queryCatalog));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(wrk1, queryId, globalQueryPlan, 3));
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(crd, queryId, globalQueryPlan, 3));
