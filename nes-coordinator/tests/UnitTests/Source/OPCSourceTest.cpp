...
public:
  ...
  static void TearDownTestCase() {
    NES_DEBUG("OPCSOURCETEST::TearDownTestCases() Tear down OPCSourceTest test "
              "class.");
  }
...
TEST_F(OPCSourceTest, OPCSourceInit) {
  ...
  auto opcSource =
      createOPCSource(test_schema, bufferManager, queryManager, url, nodeId,
...
TEST_F(OPCSourceTest, OPCSourcePrint) {
  ...
  std::string expected =
      "OPC_SOURCE(SCHEMA(var:INTEGER ), URL= opc.tcp://localhost:4840, "
...
TEST_F(OPCSourceTest, OPCSourceValue) {
  ...
  NES_DEBUG("OPCSOURCETEST::TEST_F(OPCSourceTest, OPCSourceValue) expected "
