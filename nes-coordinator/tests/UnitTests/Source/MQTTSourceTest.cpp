...
class MQTTSourceTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void TearDownTestCase() {
    NES_DEBUG(
        "MQTTSOURCETEST::TearDownTestCases() Tear down MQTTSourceTest test "
        "class.");
  }
...
TEST_F(MQTTSourceTest, MQTTSourceInit) {
  ...
  auto mqttSource =
      createMQTTSource(test_schema, bufferManager, queryManager, mqttSourceType,
...
TEST_F(MQTTSourceTest, DISABLED_MQTTSourceValue) {
  ...
  NES_DEBUG(
      "MQTTSOURCETEST::TEST_F(MQTTSourceTest, MQTTSourceValue) expected value "
      "is: {}. Received value is: {}",
      expected, value);
  EXPECT_EQ(value, expected);
}

// Disabled, because it requires a manually set up MQTT broker and a data
// sending MQTT client
TEST_F(MQTTSourceTest, DISABLED_testDeployOneWorkerWithMQTTSourceConfig) {
  ...
  auto source =
      Schema::create()
          ->addField("type", DataTypeFactory::createArray(
                                 10, DataTypeFactory::createChar()))
