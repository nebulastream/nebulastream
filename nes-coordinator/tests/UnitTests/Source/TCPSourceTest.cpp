...
std::string get_ip_address() {
  ...
  inet_ntop(AF_INET, &(record_socket_parameters.addr_in.sin_addr), ip_string,
            INET_ADDRSTRLEN);
...
// since neither functions are called anywhere else within the test it is fine
// to completly mock them
extern "C" int socket(int domain, int type, int protocol) {
  assert(!record_socket_parameters.called_socket_create &&
         "Unexpected multiple create socket calls");
...
extern "C" int connect(int fd, const sockaddr *addr,
                       [[maybe_unused]] socklen_t addrlen) {
  assert(!record_socket_parameters.called_connect &&
         "Unexpected multiple connect calls");
  record_socket_parameters.connect_fd = fd;
  assert(addrlen == sizeof(sockaddr_in) &&
         "Unexpected size of sockaddr_in parameter");
  record_socket_parameters.addr_in =
      *reinterpret_cast<const sockaddr_in *>(addr);
  return socket_mock_data.connection_fd;
}

// read however is more tricky as it is used everywhere in the system. We can
// wrap the read function see nes-runtime cmake script ('-Wl,--wrap,read') this
// means calls to read a instead calling __wrap_read and __wrap_read can
// delegate all real calls to __real_read.
...
extern "C" ssize_t __wrap_read(int fd, void *data, size_t size) {
  ...
  const auto read_size =
      std::min(size, socket_mock_data.data.size() - socket_mock_data.index);
  std::memcpy(data, socket_mock_data.data.data() + socket_mock_data.index,
              read_size);
...
public:
  ...
  void SetUp() override {
    ...
    queryManager = std::make_shared<Runtime::DynamicQueryManager>(
        nullptr, std::vector{bufferManager}, WorkerId(1), 1,
  ...
  static void TearDownTestCase() {
    NES_DEBUG("TCPSOURCETEST::TearDownTestCases() Tear down TCPSOURCETEST test "
              "class.");
  }
...
configuration:
  ...
  auto sourceType =
      PhysicalSourceTypeFactory::createFromYaml(sourceConfiguration);
  ...
  ASSERT_EQ(tcpSourceType->getDecideMessageSize()->getValue(),
            TCPDecideMessageSize::USER_SPECIFIED_BUFFER_SIZE);
  ...
  auto tcpSource = createTCPSource(
      test_schema, bufferManager, queryManager, tcpSourceType, OPERATORID,
      ORIGINID, STATISTICID, NUMSOURCELOCALBUFFERS, "tcp-source", SUCCESSORS);
...
bytesUsedForSocketBufferSizeTransfer: 0
  ...
  EXPECT_EQ(tcpSource->toString(),
            expected + expected_part_2 + expected_part_3);
  ...
  while (socket_mock_data.index < total_number_of_bytes) {
    ...
    auto testBuffer = Runtime::MemoryLayouts::TestTupleBuffer(
        Runtime::MemoryLayouts::RowLayout::create(
            test_schema, bufferManager->getBufferSize()),
        buffer);
...
configuration:
  ...
  ASSERT_EQ(tcpSourceType->getDecideMessageSize()->getValue(),
            TCPDecideMessageSize::TUPLE_SEPARATOR);
...
configuration:
  ...
  ASSERT_EQ(tcpSourceType->getInputFormat()->getValue(), INPUTFORMAT);
  ASSERT_EQ(tcpSourceType->getDecideMessageSize()->getValue(),
            TCPDecideMessageSize::BUFFER_SIZE_FROM_SOCKET);
...
configuration:
  ...
  ASSERT_EQ(tcpSourceType->getSocketPort()->getValue(), 9090);
  ASSERT_EQ(tcpSourceType->getSocketHost()->getValue(), "127.0.0.1");
  ASSERT_EQ(tcpSourceType->getInputFormat()->getValue(),
            InputFormat::NES_BINARY);
  ASSERT_EQ(tcpSourceType->getDecideMessageSize()->getValue(),
            TCPDecideMessageSize::BUFFER_SIZE_FROM_SOCKET);
  ...
  auto tcpSource = createTCPSource(
      schema, bufferManager, queryManager, tcpSourceType, OPERATORID, ORIGINID,
      1, NUMSOURCELOCALBUFFERS, "tcp-source", SUCCESSORS);
...
bytesUsedForSocketBufferSizeTransfer: 8
  ...
  std::vector<char> data;
  for (size_t number_of_buffers = 0;
       number_of_buffers < total_number_of_buffers; number_of_buffers++) {
    auto buffer = bufferManager->getBufferBlocking();
    auto dynBuffer =
  ...
  ASSERT_EQ(socket_mock_data.index,
            (actualBufferSize + 8) * total_number_of_buffers);
