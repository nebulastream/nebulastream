...
TEST_P(WindowSerializationTest, testSerializeDeserializeWindowOperators) {
  ...
  auto descriptors = {
      ...
      SlidingWindow::of(IngestionTime(), std::get<0>(GetParam()),
                        TimeMeasure(std::get<0>(GetParam()).getTime() / 2)),
      TumblingWindow::of(
          EventTime(Attribute("event_time"), std::get<1>(GetParam())),
          std::get<0>(GetParam())),
  ...
  for (const auto &window_descriptor : descriptors) {
    auto logical_window_descriptor =
        LogicalWindowDescriptor::create({}, window_descriptor, 0xC0FFEE);
    auto windowOperator = LogicalOperatorFactory::createWindowOperator(
        logical_window_descriptor, operatorId);

    auto serializedOperator =
        OperatorSerializationUtil::serializeOperator(windowOperator);
...
TEST_P(WindowSerializationTest, testSerializeDeserializeWindowJoinOperators) {
  ...
  for (const auto &logical_window_descriptor : descriptors) {
    auto joinDescriptor = Join::LogicalJoinDescriptor::create(
        joinExpression, logical_window_descriptor, 1, 1,
        Join::LogicalJoinDescriptor::JoinType::INNER_JOIN);

    auto joinOperator =
        LogicalOperatorFactory::createJoinOperator(joinDescriptor, operatorId);
    std::dynamic_pointer_cast<OriginIdAssignmentOperator>(joinOperator)
...
INSTANTIATE_TEST_SUITE_P(
    MyInstantiation, WindowSerializationTest,
    ::testing::Combine(::testing::Values(Milliseconds(1), Milliseconds(200),
                                         Milliseconds(1000), Hours(1), Hours(6),
                                         Hours(24), Days(1), Days(10),
                                         Minutes(1), Minutes(5), Minutes(60),
                                         Seconds(1), Seconds(5), Seconds(60)),
                       ::testing::Values(Milliseconds(), Hours(), Days(),
                                         Minutes(), Seconds())));
