...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SemanticQueryValidationTest.log",
  ...
  void SetUp() override {
    ...
    jitCompiler = Compiler::JITCompilerBuilder()
  ...
  static void PrintQString(const std::string &s) {
    NES_DEBUG("\nQUERY STRING:\n{}", s);
  }
  ...
  void CallValidation(const std::string &queryString) {
    ...
    auto sourceCatalogEntry = Catalogs::Source::SourceCatalogEntry::create(
        physicalSource, logicalSource, sourceNode1);
    ...
    auto semanticQueryValidation = Optimizer::SemanticQueryValidation::create(
        sourceCatalog, true, udfCatalog);
    QueryPlanPtr filterQuery =
        queryParsingService->createQueryFromCodeString(queryString);
    filterQuery =
        QueryPlanBuilder::addSink(filterQuery, FileSinkDescriptor::create(""));
...
TEST_F(SemanticQueryValidationTest, satisfiableQueryWithSingleFilter) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").filter(Attribute("value") < 10); )";
...
TEST_F(SemanticQueryValidationTest, satisfiableQueryWithLogicalExpression) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").filter(Attribute("value") > 10 && Attribute("value") < 100); )";
...
TEST_F(SemanticQueryValidationTest, satisfiableQueryWithMultipleFilters) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").filter(Attribute("id") > 10).filter(Attribute("value") > 10); )";
...
TEST_F(SemanticQueryValidationTest, unsatisfiableQueryWithMultipleFilters) {
  ...
  std::string queryString = "Query::from(\"default_logical\").filter(Attribute("
                            "\"id\") > 10).filter(Attribute(\"value\") > "
                            "10).filter(Attribute(\"id\") < 10); ";

  TestForException(queryString);
}

// Positive test for a semantically valid query with programmatically added
// filter operators
...
// Test a query with contradicting filters over multiple programmatically added
// filter operators
...
// This test is disabled, because the signature inference breaks when using this
// operator
TEST_F(SemanticQueryValidationTest, DISABLED_validAsOperatorTest) {
  NES_INFO("Valid as operator test");

  std::string queryString =
      R"(Query::from("default_logical").as("dl").filter(Attribute("value") > 100); )";
  CallValidation(queryString);
}
...
TEST_F(SemanticQueryValidationTest, invalidAsOperatorTest) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").as("value").filter(Attribute("value") > 100); )";
  CallValidation(queryString);
}
...
TEST_F(SemanticQueryValidationTest, validProjectionTest) {
  ...
  semanticQueryValidation->validate(
      std::make_shared<Query>(query)->getQueryPlan());
}
...
TEST_F(SemanticQueryValidationTest, invalidProjectionTest) {
  ...
  EXPECT_THROW(semanticQueryValidation->validate(
                   std::make_shared<Query>(query)->getQueryPlan()),
...
TEST_F(SemanticQueryValidationTest, invalidMixedInputMLInferenceOperatorTest) {
  ...
  auto query = Query::from("irisData")
                   .inferModel(std::filesystem::path(TEST_DATA_DIRECTORY) /
                                   "iris_95acc.tflite",
                               {Attribute("f1"), Attribute("f2"),
                                Attribute("f3"), Attribute("f4")},
