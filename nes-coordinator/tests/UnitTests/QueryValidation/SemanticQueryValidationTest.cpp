...
TEST_F(SemanticQueryValidationTest, invalidLogicalSourceTest)
{
    ...
    std::string queryString = R"(Query::from("nonexistent_logical").filter(Attribute("id") > 100).filter(Attribute("value") < 10); )";
    TestForException(queryString);
}
...
TEST_F(SemanticQueryValidationTest, invalidAttributesInLogicalSourceTest)
{
    ...
    std::string queryString = R"(Query::from("default_logical").filter(Attribute("nonex_1") > 100).filter(Attribute("nonex_2") < 10); )";
    TestForException(queryString);
}
