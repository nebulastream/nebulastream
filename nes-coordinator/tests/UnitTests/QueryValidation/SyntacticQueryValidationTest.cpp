...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("SyntacticQueryValidationTest.log",
  ...
  void TestForException(const std::string &queryString) {
    ...
    EXPECT_THROW(syntacticQueryValidation->validate(queryString),
...
TEST_F(SyntacticQueryValidationTest, validQueryTest) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").filter(Attribute("id") > 10 && Attribute("id") < 100); )";
...
TEST_F(SyntacticQueryValidationTest, validAttributeRenameFromProjectOperator) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").project(Attribute("id").as("identity")); )";
...
TEST_F(SyntacticQueryValidationTest, typoInFilterTest) {
  ...
  std::string queryString =
      R"(Query::from("default_logical").fliter(Attribute("id") > 10 && Attribute("id") < 100); )";
...
// Test queryIdAndCatalogEntryMapping that calls "Attribute().as()" outside a
// Projection operator
TEST_F(SyntacticQueryValidationTest, attributeRenameOutsideProjection) {
  ...
  std::string queryStringWithFilter =
      R"(Query::from("default_logical").filter(Attribute("id").as("identity") > 10); )";
...
TEST_F(SyntacticQueryValidationTest, validQueryWithInferModelOperatorTest) {
  ...
  std::string queryString =
      "Query::from(\"default_logical\").filter(Attribute(\"id\") > 10 && "
      "Attribute(\"id\") < 100)"
