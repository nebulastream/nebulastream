...
/**
 * Utility class to create default and non-default Python UDF descriptors for
 * testing.
 *
 * The class PythonUDFDescriptor performs a number of checks in its constructor.
 * Creating the required inputs everytime a test needs a Python UDF descriptor
 * leads to code repetition.
 */
class PythonUDFDescriptorBuilder {
public:
  /**
   * Create a new builder for a PythonUDFDescriptor with valid default values
   * for the fields required by the PythonUDFDescriptor.
   */
  PythonUDFDescriptorBuilder() = default;

  /**
   * @return A Python UDF descriptor with the fields either set to default
   * values or with explicitly specified values in setters.
   */
  ...
  setFunctionName(const std::string &newFunctionName);
   ...
   */
  PythonUDFDescriptorBuilder &
  setFunctionString(const std::string &newFunctionString);
