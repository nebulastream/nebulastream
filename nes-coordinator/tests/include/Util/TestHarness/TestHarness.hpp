...
/// Create compile-time tests that allow checking a specific function's type for
/// a specific function by calling
///     [function-name]CompilesFromType<Return Type, Types of arguments, ...>
/// or  [function-name]Compiles<Return Type, argument 1, argument 2>.
///
/// Note that the non-type compile time checks for the non-type template
/// arguments are limited to consteval- constructible and non-floating point
/// types. Another limitation is that as of now, type and non type template
/// argument tests cannot be mixed.
#define SETUP_COMPILE_TIME_TESTS(name, f)                                      \
  SETUP_COMPILE_TIME_TEST(name, f);                                            \
  SETUP_COMPILE_TIME_TEST_ARGS(name, f)

/// Check if function #func compiles from constexpr arguments and produce the
/// expected return type that is provided as the check's first template
/// argument.
...
/// Check if function #func compiles from argument of given types produce the
/// expected return type that is provided as the check's first template
/// argument.
#define SETUP_COMPILE_TIME_TEST(name, func)                                    \
  ...
  struct name##FromType<decltype(func(std::declval<Ts>()...)), Ts...>          \
      : std::true_type {};                                                     \
  }                                                                            \
...
public:
   ...
   * @param numWorkers number of worker (each for one physical source) to be
   * used in the test
   ...
   */
  explicit TestHarness(Query queryWithoutSink, uint16_t restPort,
                       uint16_t rpcPort,
   ...
   */
  TestHarness &
  setJoinStrategy(QueryCompilation::StreamJoinStrategy &newJoinStrategy);
   ...
   * @return Self
   */
  TestHarness &setWindowingStrategy(
   ...
   * @param workerId id of the worker whose source will produce the pushed
   * element
   */
  template <typename T>
   ...
   * element
   */
  template <typename T> TestHarness &pushElement(T element, WorkerId workerId) {
    if (workerId > topologyId) {
      NES_THROW_RUNTIME_ERROR("TestHarness: workerId " + workerId.toString() +
                              " does not exists");
    }
      ...
      if (harnessWorkerConfig->getWorkerId() == workerId) {
        ...
        if (!std::is_class<T>::value) {
          NES_THROW_RUNTIME_ERROR(
              "TestHarness: tuples must be instances of struct");
        }

        if (harnessWorkerConfig->getSourceType() !=
            TestHarnessWorkerConfiguration::TestHarnessWorkerSourceType::
                MemorySource) {
          NES_THROW_RUNTIME_ERROR("TestHarness: Record can be pushed only for "
                                  "source of Memory type.");
        }

        SchemaPtr schema;
        for (const auto &logicalSource : logicalSources) {
          if (logicalSource->getLogicalSourceName() ==
        ...
        if (!schema) {
          NES_THROW_RUNTIME_ERROR(
              "TestHarness: Unable to find schema for logical source " +
              harnessWorkerConfig->getLogicalSourceName() +
              ". Make sure you have defined a logical source with this name in "
              "test harness");
        }

        if (sizeof(T) != schema->getSchemaSizeInBytes()) {
          NES_THROW_RUNTIME_ERROR(
              "TestHarness: tuple size and schema size does not match");
        }
    ...
    if (!found) {
      NES_THROW_RUNTIME_ERROR("TestHarness: Unable to locate worker with id " +
                              workerId.toString());
    }
  ...
  TestHarness &addLogicalSource(const std::string &logicalSourceName,
  ...
  /**
   * @brief add a memory source to be used in the test and connect to parent
   * with specific parent id
   * @param logical source name
   * @param schema schema of the source
   * @param physical source name
   ...
   * @param physical source name
   */
  TestHarness &attachWorkerWithMemorySourceToCoordinator(
   ...
   */
  TestHarness &attachWorkerWithLambdaSourceToCoordinator(
      PhysicalSourceTypePtr physicalSourceType,
      WorkerConfigurationPtr workerConfiguration);

  /**
   * @brief add a csv source to be used in the test and connect to parent with
   * specific parent id
   ...
   */
  TestHarness &
  attachWorkerWithCSVSourceToWorkerWithId(const CSVSourceTypePtr &csvSourceType,
                                          WorkerId parentId);
   ...
   */
  TestHarness &
  attachWorkerWithCSVSourceToCoordinator(const CSVSourceTypePtr &csvSourceType);

  /**
   * @brief add worker and connect to parent with specific parent id
   * @param parentId id of the Test Harness worker to connect
   * Note: The parent id can not be greater than the current testharness worker
   * id
   */
  ...
  PhysicalSourceTypePtr createPhysicalSourceOfLambdaType(
      TestHarnessWorkerConfigurationPtr workerConf);

  PhysicalSourceTypePtr createPhysicalSourceOfMemoryType(
      TestHarnessWorkerConfigurationPtr workerConf);

  /**
   * @brief Method to setup the topology
   * @param crdConfigFunctor A function pointer to specify the config changes of
   * the CoordinatorConfiguration
   * @param distributionList A distribution of keys of each source for joins in
   * JSON format
  ...
  /**
   * @brief Runs the query based on the given operator, pushed elements, and
   * number of workers.
  ...
  /**
   * @brief Returns the output for the previously run query. Support also data
   * types with variable data size
...
private:
  ...
  std::vector<TestHarnessWorkerConfigurationPtr>
      testHarnessWorkerConfigurations;
