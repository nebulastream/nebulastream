...
numberOfTuplesToProducePerBuffer(uint64_t numberOfTuplesToProducePerBuffer);
 ...
 */
[[nodiscard]] std::string
numberOfBuffersToProduce(uint64_t numberOfBuffersToProduce);
 ...
 */
[[nodiscard]] std::string
sourceGatheringInterval(uint64_t sourceGatheringInterval);
 ...
 */
[[nodiscard]] std::string
coordinatorHealthCheckWaitTime(uint64_t coordinatorWaitTime);
 ...
 */
[[nodiscard]] Util::Subprocess
startCoordinator(std::initializer_list<std::string> list);
 ...
 */
[[nodiscard]] Util::Subprocess
startWorker(std::initializer_list<std::string> flags);
 ...
 * @return worker process, which terminates if it leaves the scope
 */
[[nodiscard]] std::shared_ptr<Util::Subprocess>
startWorkerPtr(std::initializer_list<std::string> flags);
 ...
 */
[[nodiscard]] bool
waitForQueryToStart(QueryId queryId,
...
[[nodiscard]] bool
checkCompleteOrTimeout(const NesWorkerPtr &nesWorker, QueryId queryId,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    NES_TRACE("checkCompleteOrTimeout: NesWorkerPtr results are incomplete "
  ...
  auto statistics = nesWorker->getQueryStatistics(sharedQueryId);
  uint64_t processed = statistics[0]->getProcessedBuffers();
  NES_TRACE("checkCompleteOrTimeout: NesWorkerPtr expected results are not "
            "reached after timeout expected={} final result={}",
...
template <typename Predicate = std::equal_to<uint64_t>>
[[nodiscard]] bool
checkCompleteOrTimeout(const NesCoordinatorPtr &nesCoordinator, QueryId queryId,
                       const GlobalQueryPlanPtr &globalQueryPlan,
                       uint64_t expectedResult,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutSeconds) {
    ...
    if (minOneProcessedTask &&
      ...
      NES_TRACE("checkCompleteOrTimeout: A task was processed within the last "
                "{}ms, the query may still be active. "
    ...
    // return if enough buffer have been received
      ...
      NES_TRACE("checkCompleteOrTimeout: NesCoordinatorPtr results are correct "
    ...
    NES_TRACE("checkCompleteOrTimeout: NesCoordinatorPtr results are "
              "incomplete procBuffer={} procTasks={} expected={}",
              statistics[0]->getProcessedBuffers(),
 ...
 */
[[nodiscard]] bool
checkStoppedOrTimeout(QueryId queryId,
                      const Catalogs::Query::QueryCatalogPtr &queryCatalog,
 ...
 */
[[nodiscard]] bool
checkStoppedOrTimeoutAtWorker(SharedQueryId sharedQueryId, NesWorkerPtr worker,
                              std::chrono::seconds timeout = defaultTimeout);
 ...
 */
[[nodiscard]] bool
checkFailedOrTimeout(QueryId queryId,
                     const Catalogs::Query::QueryCatalogPtr &queryCatalog,
                     std::chrono::seconds timeout = defaultTimeout);
...
[[nodiscard]] bool checkOutputContentLengthOrTimeout(
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    std::this_thread::sleep_for(sleepDuration);
    NES_TRACE("TestUtil:checkBinaryOutputContentLengthOrTimeout: check content "
              "for file {}",
              outputFilePath);
    ...
    if (std::filesystem::exists(outputFilePath) &&
      ...
      if (currentContentSize > numberOfRecordsToExpect) {
        NES_DEBUG("content is larger than expected result: currentContentSize: "
        ...
        NES_DEBUG("number of content in output file match expected number of "
                  "content!");
        return true;
      }
    }
...
startQueryViaRest(const string &queryString,
                  const std::string &restPort = "8081");
 ...
 */
[[nodiscard]] nlohmann::json
addSourceStatistics(const string &queryString,
                    const std::string &restPort = "8081");
 ...
 */
[[nodiscard]] nlohmann::json
makeMonitoringRestCall(const string &restCall,
                       const std::string &restPort = "8081");
...
createExpectedBuffersFromCsv(const std::string &csvFileName,
...
uint64_t
countTuples(std::vector<Runtime::MemoryLayouts::TestTupleBuffer> &buffers);
...
createTestTupleBuffers(std::vector<Runtime::TupleBuffer> &buffers,
                       const SchemaPtr &schema);
...
inline std::vector<T> createVecFromTupleBuffer(Runtime::TupleBuffer buffer) {
  return createVecFromPointer<T>(buffer.getBuffer<T>(),
                                 buffer.getBuffer<T>() +
 ...
 */
CSVSourceTypePtr
createSourceTypeCSV(const SourceTypeConfigCSV &sourceTypeConfigCSV);
