...
/**
 * @brief This define states all join strategies that will be tested in all
 * join-specific tests
 */
...
/**
 * @brief This define states all window strategies that will be tested in all
 * join-specific tests. Note that BUCKETING is not supported for
 * HASH_JOIN_VAR_SIZED
 */
...
/**
 * @brief This combines all join strategies and window strategies that will be
 * tested in all join-specific test cases
 */
#define JOIN_STRATEGIES_WINDOW_STRATEGIES \
  ::testing::Combine(ALL_JOIN_STRATEGIES, ALL_WINDOW_STRATEGIES)
...
/**
 * @brief Struct for storing all csv file params for tests. It is solely a
 * container for grouping csv files.
 */
struct CsvFileParams {
  CsvFileParams(const string& csvFileLeft, const string& csvFileRight,
                const string& expectedFile)
      : inputCsvFiles({csvFileLeft, csvFileRight}),
        expectedFile(expectedFile) {}

  explicit CsvFileParams(const std::vector<std::string>& inputCsvFiles,
                         const string& expectedFile = "")
...
static constexpr auto defaultStartQueryTimeout =
    std::chrono::seconds(180);  // starting a query requires time
static constexpr auto sleepDuration = std::chrono::milliseconds(250);
static constexpr auto defaultCooldown = std::chrono::seconds(
    3);  // 3s after last processed task, the query should be done.

/**
 * Create a command line parameter for a configuration option for the
 * coordinator or worker.
 * @param name The name of the command line option.
 * @param value The value of the command line option.
 * @param prefix If true, prefix the name of the option with "worker." to
 * configure the internal worker of the coordinator.
 * @return A string representing the command line parameter.
 */
[[nodiscard]] const std::string configOption(const std::string& name,
 ...
 */
template <typename T>
[[nodiscard]] std::string configOption(const std::string& name, T value,
                                       bool prefix = false) {
...
/**
 * @brief Creates the command line argument for the number of tuples of tuples
 * to produce per buffer
 * @param numberOfTuplesToProducePerBuffer
 * @return Command line argument
 */
[[nodiscard]] std::string numberOfTuplesToProducePerBuffer(
...
/**
 * @brief Creates the command line argument for setting the number of buffers to
 * produce
 * @param numberOfBuffersToProduce
 * @return Command line argument
 */
[[nodiscard]] std::string numberOfBuffersToProduce(
    uint64_t numberOfBuffersToProduce);

/**
 * @brief Creates the command line argument for setting the source gathering
 * interval
 * @param sourceGatheringInterval
 * @return Command line argument
 */
[[nodiscard]] std::string sourceGatheringInterval(
...
/**
 * @brief Creates the command line argument for setting the health check wait
 * time for the worker
...
/**
 * @brief Creates the command line argument for setting the health check wait
 * time for the coordinator
 * @param coordinatorWaitTime
 * @return Command line argument
 */
[[nodiscard]] std::string coordinatorHealthCheckWaitTime(
    uint64_t coordinatorWaitTime);
...
/**
 * @brief Creates the command line argument for setting the threshold of the
 * distributed window child
 * @param val
 * @return Command line argument
 */
...
/**
 * @brief start a new instance of a nes coordinator with a set of configuration
 * flags
 * @param flags
 * @return coordinator process, which terminates if it leaves the scope
 */
[[nodiscard]] Util::Subprocess startCoordinator(
 ...
 */
[[nodiscard]] Util::Subprocess startWorker(
 ...
 * @return worker process, which terminates if it leaves the scope
 */
[[nodiscard]] std::shared_ptr<Util::Subprocess> startWorkerPtr(
    std::initializer_list<std::string> flags);

/**
 * @brief method to check the produced buffers and tasks for n seconds and
 * either return true or timeout
 * @param ptr to Runtime
 * @param queryId
 * @param expectedResult
 * @return bool indicating if the expected results are matched
 */
[[nodiscard]] bool checkCompleteOrTimeout(const Runtime::NodeEnginePtr& ptr,
                                          QueryId queryId,
                                          uint64_t expectedResult);

/**
 * @brief This method is used for waiting till the query gets into running
 * status or a timeout occurs
...
[[nodiscard]] bool waitForQueryToStart(
    QueryId queryId, const Catalogs::Query::QueryCatalogPtr& queryCatalog,
    std::chrono::seconds timeoutInSec =
        std::chrono::seconds(defaultStartQueryTimeout));
...
[[nodiscard]] bool checkCompleteOrTimeout(
    const NesWorkerPtr& nesWorker, QueryId queryId,
    const GlobalQueryPlanPtr& globalQueryPlan, uint64_t expectedResult) {
  ...
  NES_INFO("Found global query id {} for user query {}", sharedQueryId,
           queryId);
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    if (statistics.empty()) {
      NES_TRACE("checkCompleteOrTimeout: query={} stats size={}", sharedQueryId,
                statistics.size());
    ...
    if (processed >= expectedResult) {
      NES_TRACE(
          "checkCompleteOrTimeout: results are correct procBuffer={} "
          "procTasks={} procWatermarks={}",
    ...
    NES_TRACE(
        "checkCompleteOrTimeout: NesWorkerPtr results are incomplete "
        "procBuffer={} procTasks={} procWatermarks={}",
        statistics[0]->getProcessedBuffers(),
        statistics[0]->getProcessedTasks(),
        statistics[0]->getProcessedWatermarks());
    std::this_thread::sleep_for(sleepDuration);
  ...
  uint64_t processed = statistics[0]->getProcessedBuffers();
  NES_TRACE(
      "checkCompleteOrTimeout: NesWorkerPtr expected results are not reached "
      "after timeout expected={} final result={}",
...
template <typename Predicate = std::equal_to<uint64_t>>
[[nodiscard]] bool checkCompleteOrTimeout(
    const NesCoordinatorPtr& nesCoordinator, QueryId queryId,
    const GlobalQueryPlanPtr& globalQueryPlan, uint64_t expectedResult,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutSeconds) {
    ...
    // FIXME: handle vector of statistics properly in #977
    auto statistics = nesCoordinator->getQueryStatistics(
        UNSURE_CONVERSION_TODO_4761(sharedQueryId, QueryId));
    if (statistics.empty()) {
      continue;
    }
    ...
    if (minOneProcessedTask &&
        (statistics[0]->getTimestampLastProcessedTask() == 0 ||
         ...
         statistics[0]->getTimestampLastProcessedTask() >
             now - defaultCooldown.count())) {
      NES_TRACE(
          "checkCompleteOrTimeout: A task was processed within the last {}ms, "
          "the query may still be active. "
    ...
    // return if enough buffer have been received
      ...
      NES_TRACE(
          "checkCompleteOrTimeout: NesCoordinatorPtr results are correct "
          "stats={} procTasks={} procWatermarks={}",
          statistics[0]->getProcessedBuffers(),
          statistics[0]->getProcessedTasks(),
          statistics[0]->getProcessedWatermarks());
      return true;
    ...
    NES_TRACE(
        "checkCompleteOrTimeout: NesCoordinatorPtr results are incomplete "
        "procBuffer={} procTasks={} expected={}",
        statistics[0]->getProcessedBuffers(),
        statistics[0]->getProcessedTasks(), expectedResult);

    std::this_thread::sleep_for(sleepDuration);
  }
  // FIXME: handle vector of statistics properly in #977
  NES_TRACE(
      "checkCompleteOrTimeout: NesCoordinatorPtr expected results are not "
      "reached after timeout expected result={}"
 ...
 * @return true if successful
 */
[[nodiscard]] bool checkStoppedOrTimeoutAtWorker(
    SharedQueryId sharedQueryId, NesWorkerPtr worker,
    std::chrono::seconds timeout = defaultTimeout);
 ...
 * @return true if successful
 */
[[nodiscard]] bool checkFailedOrTimeout(
    QueryId queryId, const Catalogs::Query::QueryCatalogPtr& queryCatalog,
    std::chrono::seconds timeout = defaultTimeout);
 ...
 * @return true if successful
 */
[[nodiscard]] bool checkOutputOrTimeout(string expectedContent,
 ...
 * @return true if successful
 */
[[nodiscard]] bool checkIfOutputFileIsNotEmtpy(uint64_t minNumberOfLines,
                                               const string& outputFilePath,
...
[[nodiscard]] bool checkOutputContentLengthOrTimeout(
    QueryId queryId, Catalogs::Query::QueryCatalogPtr queryCatalog,
    uint64_t numberOfRecordsToExpect, const string& outputFilePath,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    std::this_thread::sleep_for(sleepDuration);
    NES_TRACE(
        "TestUtil:checkBinaryOutputContentLengthOrTimeout: check content for "
        "file {}",
        outputFilePath);

    auto currentQueryState = queryCatalog->getQueryState(queryId);
    if (currentQueryState == QueryState::FAILED) {
      // the query failed, so we return true as a failure append during
      // execution.
    ...
    if (std::filesystem::exists(outputFilePath) &&
        std::filesystem::is_regular_file(outputFilePath)) {
      // As we are writing a header in the output csv file of the query, we have
      // to subtract one line
      ...
      if (currentContentSize > numberOfRecordsToExpect) {
        NES_DEBUG(
            "content is larger than expected result: currentContentSize: {} - "
            "expectedNumberOfContent: {}",
            currentContentSize, numberOfRecordsToExpect);
        return false;
        ...
        NES_DEBUG(
            "number of content in output file match expected number of "
            "content!");
        return true;
      }
    }
 ...
 * @return true if successful
 */
[[nodiscard]] bool checkRESTServerStartedOrTimeout(uint64_t restPort,
                                                   uint64_t customTimeout = 0);

/**
 * @brief This method is used for checking if the submitted query produced the
 * expected result within the timeout
 ...
 */
[[nodiscard]] bool checkRunningOrTimeout(QueryId queryId,
                                         const std::string& restPort = "8081");
 ...
 * @return if stopped
 */
[[nodiscard]] bool stopQueryViaRest(QueryId queryId,
                                    const std::string& restPort = "8081");
 ...
 * @return if stopped
 */
[[nodiscard]] nlohmann::json startQueryViaRest(
    const string& queryString, const std::string& restPort = "8081");
 ...
 * @return if stopped
 */
[[nodiscard]] nlohmann::json addSourceStatistics(
    const string& queryString, const std::string& restPort = "8081");
 ...
 * @return the json
 */
[[nodiscard]] nlohmann::json makeMonitoringRestCall(
    const string& restCall, const std::string& restPort = "8081");
 ...
 * @return
 */
[[nodiscard]] bool addLogicalSource(const string& schemaString,
                                    const std::string& restPort = "8081");

bool waitForWorkers(uint64_t restPort, uint16_t maxTimeout,
                    uint16_t expectedWorkers);

/**
 * @brief This method is used for making a REST call to coordinator to get the
 * topology as Json
...
std::vector<Runtime::TupleBuffer> createExpectedBuffersFromCsv(
    const std::string& csvFileName, const SchemaPtr& schema,
...
std::vector<Runtime::TupleBuffer> createExpectedBuffersFromCsv(
    const std::string& csvFileName, const SchemaPtr& schema,
    const Runtime::BufferManagerPtr& bufferManager, bool skipHeader = false,
    uint64_t numTuplesPerBuffer = 0, const std::string& delimiter = ",");
...
std::vector<Runtime::TupleBuffer> createExpectedBufferFromStream(
    std::istream& istream, const SchemaPtr& schema,
    const Runtime::BufferManagerPtr& bufferManager, bool skipHeader = false,
    uint64_t numTuplesPerBuffer = 0, const std::string& delimiter = ",");
...
std::vector<Runtime::TupleBuffer> createExpectedBufferFromCSVString(
    std::string str, const SchemaPtr& schema,
    const Runtime::BufferManagerPtr& bufferManager, bool skipHeader = false,
    uint64_t numTuplesPerBuffer = 0, const std::string& delimiter = ",");
...
uint64_t countTuples(
    std::vector<Runtime::MemoryLayouts::TestTupleBuffer>& buffers);
 ...
 */
std::vector<Runtime::MemoryLayouts::TestTupleBuffer> createTestTupleBuffers(
    std::vector<Runtime::TupleBuffer>& buffers, const SchemaPtr& schema);
...
/**
 * @brief Creates a vector for the memory that this tupleBuffer is responsible
 * for
 * @tparam T
 * @param startPtr
 * @param numItems
...
inline std::vector<T> createVecFromTupleBuffer(Runtime::TupleBuffer buffer) {
  return createVecFromPointer<T>(
      buffer.getBuffer<T>(),
      buffer.getBuffer<T>() + buffer.getNumberOfTuples());
}

/**
 * @brief Creates a csv source that produces as many buffers as the csv file
 * contains
 * @param SourceTypeConfig: container for configuration parameters of a source
 * type.
 * @return CSVSourceTypePtr
 */
CSVSourceTypePtr createSourceTypeCSV(
    const SourceTypeConfigCSV& sourceTypeConfigCSV);
...
class DummyQueryListener : public AbstractQueryStatusListener {
 public:
  ...
  bool canTriggerEndOfStream(SharedQueryId, DecomposedQueryPlanId, OperatorId,
                             Runtime::QueryTerminationType) override {
    return true;
  }
  bool notifySourceTermination(SharedQueryId, DecomposedQueryPlanId, OperatorId,
                               Runtime::QueryTerminationType) override {
    return true;
  }
 ...
 * @param csvPath path to the csv with lines in the format <latitude, longitude,
 * offsetFromStartTime>
 * @param startTime the real or simulated start time of the LocationProvider
 * @return a vector of waypoints with timestamps calculated by adding startTime
 * to the offset obtained from csv
 */
std::vector<NES::Spatial::DataTypes::Experimental::Waypoint>
getWaypointsFromCsv(const std::string& csvPath, Timestamp startTime);

/**
 * @brief write mobile device path waypoints to a csv file to use as input for
 * the LocationProviderCSV class
 * @param csvPath path to the output file
 * @param waypoints a vector of waypoints to be written to the file
 */
void writeWaypointsToCsv(
    const std::string& csvPath,
    const std::vector<NES::Spatial::DataTypes::Experimental::Waypoint>&
        waypoints);
 ...
 * @return The number of occurrences of the search string within the target
 * string.
 */
uint64_t countOccurrences(const std::string& searchString,
