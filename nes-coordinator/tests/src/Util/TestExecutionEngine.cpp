...
std::shared_ptr<TestSink> TestExecutionEngine::createDataSink(
    const SchemaPtr& outputSchema, uint32_t expectedTuples) {
  return std::make_shared<TestSink>(expectedTuples, outputSchema, nodeEngine);
}

std::shared_ptr<SourceDescriptor> TestExecutionEngine::createDataSource(
...
Runtime::MemoryLayouts::TestTupleBuffer TestExecutionEngine::getBuffer(
