...
TestExecutionEngine::TestExecutionEngine(
    const QueryCompilation::DumpMode &dumpMode, const uint64_t numWorkerThreads,
  ...
  workerConfiguration->queryCompiler.nautilusBackend =
  ...
  workerConfiguration->numberOfBuffersInGlobalBufferManager =
      numWorkerThreads * DEFAULT_NO_BUFFERS_IN_GLOBAL_BM_PER_THREAD;
  workerConfiguration->numberOfBuffersInSourceLocalBufferPool =
      numWorkerThreads * DEFAULT_NO_BUFFERS_IN_SOURCE_BM_PER_THREAD;
...
std::shared_ptr<TestSink>
TestExecutionEngine::createDataSink(const SchemaPtr &outputSchema,
                                    uint32_t expectedTuples) {
  return std::make_shared<TestSink>(expectedTuples, outputSchema, nodeEngine);
}

std::shared_ptr<SourceDescriptor>
TestExecutionEngine::createDataSource(SchemaPtr inputSchema) {
  return std::make_shared<TestUtils::TestSourceDescriptor>(
      inputSchema,
      // We require inputSchema as a lambda function arg since capturing it can
      // lead to using a corrupted schema.
      [&](SchemaPtr inputSchema, OperatorId id, OriginId originId,
          StatisticId statisticId, const SourceDescriptorPtr &,
        ...
        return createNonRunnableSource(
            inputSchema, nodeEngine->getBufferManager(),
            nodeEngine->getQueryManager(), id, originId, statisticId,
...
TestExecutionEngine::submitQuery(DecomposedQueryPlanPtr decomposedQueryPlan) {
  ...
  NES_ASSERT(nodeEngine->registerDecomposableQueryPlan(decomposedQueryPlan),
             "query plan could not be started.");
  NES_ASSERT(
      nodeEngine->startQuery(decomposedQueryPlan->getSharedQueryId(),
                             decomposedQueryPlan->getDecomposedQueryPlanId()),
      "query plan could not be started.");
  return nodeEngine->getQueryManager()->getQueryExecutionPlan(
      decomposedQueryPlan->getDecomposedQueryPlanId());
}

std::shared_ptr<NonRunnableDataSource> TestExecutionEngine::getDataSource(
    std::shared_ptr<Runtime::Execution::ExecutableQueryPlan> plan,
    uint32_t source) {
  NES_ASSERT(!plan->getSources().empty(), "Query plan has no sources ");
  return std::dynamic_pointer_cast<NonRunnableDataSource>(
      plan->getSources()[source]);
}

void TestExecutionEngine::emitBuffer(
    std::shared_ptr<Runtime::Execution::ExecutableQueryPlan> plan,
    Runtime::TupleBuffer buffer) {
  // todo add support for multiple sources.
  nodeEngine->getQueryManager()->addWorkForNextPipeline(
      buffer, plan->getPipelines()[0]);
}

bool TestExecutionEngine::stopQuery(
    std::shared_ptr<Runtime::Execution::ExecutableQueryPlan> plan,
    Runtime::QueryTerminationType type) {
  return nodeEngine->getQueryManager()->stopQuery(plan, type);
}

Runtime::MemoryLayouts::TestTupleBuffer
TestExecutionEngine::getBuffer(const SchemaPtr &schema) {
