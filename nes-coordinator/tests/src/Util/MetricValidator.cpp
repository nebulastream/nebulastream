...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader, Monitoring::NetworkMetricsWrapper networkMetrics)
{
    bool check = true;
    ...
    uint64_t expectedSize)
{
    ...
    NES_INFO("MetricValidator: Stored metrics for ID {}: {}", expectedNodeId, Monitoring::MetricUtils::toJson(storedMetrics).dump());
    if (retMetric->getMetricType() != expectedType)
    {
        NES_ERROR("MetricValidator: MetricType is not as expected {} != {}", toString(retMetric->getMetricType()), toString(expectedType));
        check = false;
    }
...
bool MetricValidator::waitForMonitoringStreamsOrTimeout(
    const std::set<Monitoring::MetricType> & monitoringStreams, uint16_t maxTimeout, uint64_t restPort)
{
