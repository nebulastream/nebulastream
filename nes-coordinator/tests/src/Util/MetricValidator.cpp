...
bool MetricValidator::isValid(
  ...
  if (metric->getMetricType() == Monitoring::MetricType::DiskMetric) {
    ...
    return isValid(reader, metric->getValue<Monitoring::CpuMetricsWrapper>());
             ...
             Monitoring::MetricType::WrappedNetworkMetrics) {
    return isValid(reader,
                   metric->getValue<Monitoring::NetworkMetricsWrapper>());
  } else {
    return false;
  }
};
...
bool MetricValidator::isValid(
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING("MetricValidator: AbstractReader used for RuntimeMetrics. "
                "Returning true");
    return true;
  }
...
bool MetricValidator::isValid(
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING("MetricValidator: AbstractReader used for "
                "NodeRegistrationMetrics. Returning true");
    return true;
  }
...
bool MetricValidator::isValid(
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING("MetricValidator: AbstractReader used for CpuMetricsWrapper. "
                "Returning true");
    return true;
  }
...
bool MetricValidator::isValid(
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING("MetricValidator: AbstractReader used for "
                "NetworkMetricsWrapper. Returning true");
    return true;
  }
...
bool MetricValidator::isValid(
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING("MetricValidator: AbstractReader used for MemoryMetrics. "
                "Returning true");
    return true;
  }
...
bool MetricValidator::isValid(
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING(
        "MetricValidator: AbstractReader used for DiskMetrics. Returning true");
    return true;
  }
...
bool MetricValidator::isValid(
    ...
    Monitoring::MetricType expectedType, WorkerId expectedNodeId,
    uint64_t expectedSize) {
  bool check = true;

  if (!storedMetrics->contains(expectedType)) {
    NES_ERROR("MetricValidator: Metrics for node {} are missing type {}",
              expectedNodeId, toString(expectedType));
    return false;
  }
  ...
  if (retMetric->getMetricType() != expectedType) {
    NES_ERROR("MetricValidator: MetricType is not as expected {} != {}",
              toString(retMetric->getMetricType()), toString(expectedType));
    check = false;
  }
  ...
  if (storedMetrics->size() != expectedSize) {
    NES_ERROR(
        "MetricValidator: Stored metrics do not match expected size {} != {}",
        storedMetrics->size(), expectedSize);
    check = false;
  }
  return check;
}

bool MetricValidator::isValidAll(
  ...
  if (!json.contains(std::string(
          magic_enum::enum_name(Monitoring::MetricType::RegistrationMetric)))) {
    NES_ERROR("MetricValidator: Missing field registration");
    check = false;
  } else {
    check = isValidRegistrationMetrics(
        reader, json[std::string(magic_enum::enum_name(
                    Monitoring::MetricType::RegistrationMetric))]);
  }

  if (!json.contains(std::string(
          magic_enum::enum_name(Monitoring::MetricType::DiskMetric)))) {
  ...
  if (!json.contains(std::string(
          magic_enum::enum_name(Monitoring::MetricType::WrappedCpuMetrics)))) {
    NES_ERROR("MetricValidator: Missing field wrapped cpu");
    check = false;
  } else {
    auto numCpuFields = json[std::string(magic_enum::enum_name(
                                 Monitoring::MetricType::WrappedCpuMetrics))]
                            .size();
    if (numCpuFields <= 1) {
  ...
  if (!json.contains(std::string(magic_enum::enum_name(
          Monitoring::MetricType::WrappedNetworkMetrics)))) {
    NES_ERROR("MetricValidator: Missing field wrapped network");
    check = false;
  } else {
    auto numFields = json[std::string(magic_enum::enum_name(
                              Monitoring::MetricType::WrappedNetworkMetrics))]
                         .size();
    if (numFields < 1) {
  ...
  if (!json.contains(std::string(
          magic_enum::enum_name(Monitoring::MetricType::MemoryMetric)))) {
...
bool MetricValidator::isValidAllStorage(
    Monitoring::AbstractSystemResourcesReaderPtr reader, nlohmann::json json) {
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    NES_WARNING("MetricValidator: AbstractReader used. Returning true");
    return true;
  ...
  if (!json.contains("RegistrationMetric")) {
    ...
    check = isValidRegistrationMetrics(reader,
                                       json["RegistrationMetric"][0]["value"]);
  }
...
bool MetricValidator::isValidRegistrationMetrics(
    Monitoring::AbstractSystemResourcesReaderPtr reader, nlohmann::json json) {
  bool check = true;
  ...
  if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
    ...
    if (numFields !=
        Monitoring::RegistrationMetrics::getSchema("")->getSize()) {
...
bool MetricValidator::checkNodeIds(nlohmann::json json, WorkerId nodeId) {
  ...
  for (auto &[key, val] : json.items()) {
    if (json[key].contains("NODE_ID") &&
        json[key]["NODE_ID"].get<WorkerId>() != nodeId) {
      NES_ERROR("MetricValidator: Wrong node ID for {} where {} != {}", key,
                json[key]["NODE_ID"], nodeId);
      check = false;
    }
  }
  return check;
...
bool MetricValidator::checkNodeIdsStorage(nlohmann::json json,
                                          WorkerId nodeId) {
  bool check = true;
  for (auto &[key, val] : json.items()) {
...
bool MetricValidator::MetricValidator::checkEntriesOfStream(
    std::set<std::string> setOfStr, nlohmann::json jsons) {
  bool check = false;
...
bool MetricValidator::checkNodeIds(Monitoring::MetricPtr metric,
                                   WorkerId nodeId) {
  if (metric->getMetricType() == Monitoring::MetricType::DiskMetric) {
...
bool MetricValidator::waitForMonitoringStreamsOrTimeout(
    const std::set<Monitoring::MetricType> &monitoringStreams,
    uint16_t maxTimeout, uint64_t restPort) {
  for (int i = 0; i < maxTimeout; i++) {
    auto json =
        TestUtils::makeMonitoringRestCall("storage", std::to_string(restPort));
    ...
    for (uint64_t nodeId = 1; nodeId <= json.size(); nodeId++) {
      for (auto stream : monitoringStreams) {
        ...
        if (!json[std::to_string(nodeId)].contains(strStream)) {
          NES_ERROR("MetricValidator: Missing metric {} for node {} ",
                    strStream, nodeId);
          missing = true;
          break;
        }
