...
TestSink::TestSink(uint64_t expectedTuples, const SchemaPtr& schema,
...
std::shared_ptr<TestSink> TestSink::create(uint64_t expectedTuples,
                                           ...
                                           uint32_t numOfProducers) {
  return std::make_shared<TestSink>(expectedTuples, schema, engine,
                                    numOfProducers);
}

bool TestSink::writeData(Runtime::TupleBuffer& inputBuffer,
  ...
  // Check whether the required amount of expected result buffers has been
  // reached.
  if (receivedTuples < numOfExpectedTuples) {
    NES_DEBUG("Already saw {} tuples and expects a total of {}", receivedTuples,
              numOfExpectedTuples);
  } else if (receivedTuples == numOfExpectedTuples) {
    completed.set_value(numOfExpectedTuples);
  } else if (receivedTuples > numOfExpectedTuples) {
    NES_ERROR(
        "result number of tuples {} and expected number of tuples={} do not "
        "match",
        receivedTuples, numOfExpectedTuples);
    EXPECT_TRUE(false);
  }
  return true;
}
...
Runtime::MemoryLayouts::TestTupleBuffer TestSink::getResultBuffer(
    uint64_t index) {
...
SinkMediumTypes TestSink::getSinkMediumType() {
