...
TestHarness::TestHarness(Query queryWithoutSink, uint16_t restPort,
    ...
    : queryWithoutSink(std::make_shared<Query>(std::move(queryWithoutSink))),
      coordinatorHostAddress("127.0.0.1"),
      restPort(restPort),
      rpcPort(rpcPort),
      useNewRequestExecutor(false),
      memSrcFrequency(memSrcFrequency),
      memSrcNumBuffToProcess(memSrcNumBuffToProcess),
      bufferSize(4096),
      physicalSourceCount(0),
      topologyId(WorkerId(1)),
...
void TestHarness::checkAndAddLogicalSources() {
  ...
  for (const auto& logicalSource : logicalSources) {
    ...
    if (!sourceCatalog->containsLogicalSource(logicalSourceName)) {
      NES_TRACE(
          "TestHarness: logical source does not exist in the source catalog, "
          "adding a new logical source {}",
          logicalSourceName);
      ...
      if (!sourceCatalog->getSchemaForLogicalSource(logicalSourceName)
               ->equals(schema, true)) {
        NES_TRACE(
            "TestHarness: logical source {} exists in the source catalog with "
            "different schema, replacing it "
            "with a new schema",
            logicalSourceName);
...
TestHarness& TestHarness::runQuery(uint64_t numberOfRecordsToExpect,
  ...
  if (!TestUtils::checkOutputContentLengthOrTimeout(
    ...
    NES_THROW_RUNTIME_ERROR(
        "TestHarness: checkOutputContentLengthOrTimeout returns false, number "
        "of bytes to expect = "
