...
TestHarness::TestHarness(Query queryWithoutSink, uint16_t restPort,
                         uint16_t rpcPort,
    ...
    : queryWithoutSink(std::make_shared<Query>(std::move(queryWithoutSink))),
      coordinatorHostAddress("127.0.0.1"), restPort(restPort), rpcPort(rpcPort),
      useNewRequestExecutor(false), memSrcFrequency(memSrcFrequency),
      memSrcNumBuffToProcess(memSrcNumBuffToProcess), bufferSize(4096),
      physicalSourceCount(0), topologyId(WorkerId(1)),
...
TestHarness &TestHarness::addLogicalSource(const std::string &logicalSourceName,
...
TestHarness &TestHarness::setJoinStrategy(
    QueryCompilation::StreamJoinStrategy &newJoinStrategy) {
  this->joinStrategy = newJoinStrategy;
  return *this;
}

TestHarness &TestHarness::setWindowingStrategy(
    QueryCompilation::WindowingStrategy &newWindowingStrategy) {
...
void TestHarness::checkAndAddLogicalSources() {
  ...
  for (const auto &logicalSource : logicalSources) {
    ...
    if (!sourceCatalog->containsLogicalSource(logicalSourceName)) {
      NES_TRACE("TestHarness: logical source does not exist in the source "
                "catalog, adding a new logical source {}",
                logicalSourceName);
      sourceCatalog->addLogicalSource(logicalSourceName, schema);
    } else {
      // Check if it has the same schema
      if (!sourceCatalog->getSchemaForLogicalSource(logicalSourceName)
               ->equals(schema, true)) {
        NES_TRACE("TestHarness: logical source {} exists in the source catalog "
                  "with different schema, replacing it "
                  "with a new schema",
                  logicalSourceName);
...
TestHarness &TestHarness::attachWorkerWithMemorySourceToWorkerWithId(
    const std::string &logicalSourceName, WorkerId parentId,
  ...
  auto testHarnessWorkerConfiguration = TestHarnessWorkerConfiguration::create(
      workerConfiguration, logicalSourceName, physicalSourceName,
...
TestHarness &TestHarness::attachWorkerWithMemorySourceToCoordinator(
    const std::string &logicalSourceName) {
  // We are assuming coordinator will start with id 1
  return attachWorkerWithMemorySourceToWorkerWithId(
      std::move(logicalSourceName), WorkerId(1));
}

TestHarness &TestHarness::attachWorkerWithLambdaSourceToCoordinator(
  ...
  auto workerId = getNextTopologyId();
  auto testHarnessWorkerConfiguration = TestHarnessWorkerConfiguration::create(
      workerConfiguration, physicalSource->getLogicalSourceName(),
...
TestHarness &TestHarness::attachWorkerWithCSVSourceToWorkerWithId(
    const CSVSourceTypePtr &csvSourceType, WorkerId parentId) {
  auto workerConfiguration = WorkerConfiguration::create();
  workerConfiguration->physicalSourceTypes.add(csvSourceType);
  workerConfiguration->parentId = parentId;
  auto workerId = getNextTopologyId();
  auto testHarnessWorkerConfiguration = TestHarnessWorkerConfiguration::create(
      workerConfiguration, csvSourceType->getLogicalSourceName(),
...
TestHarness &TestHarness::attachWorkerWithCSVSourceToCoordinator(
...
TestHarness &TestHarness::attachWorkerToWorkerWithId(WorkerId parentId) {
  ...
  auto testHarnessWorkerConfiguration =
      TestHarnessWorkerConfiguration::create(workerConfiguration, workerId);
  testHarnessWorkerConfigurations.emplace_back(testHarnessWorkerConfiguration);
  return *this;
}
...
uint64_t TestHarness::getWorkerCount() {
  return testHarnessWorkerConfigurations.size();
}
...
TestHarness &TestHarness::validate() {
  ...
  if (this->logicalSources.empty()) {
    throw Exceptions::RuntimeException(
        "No Logical source defined. Please make sure you add logical source "
        "while defining up test harness.");
  }

  if (testHarnessWorkerConfigurations.empty()) {
    throw Exceptions::RuntimeException(
        "TestHarness: No worker added to the test harness.");
  }

  uint64_t sourceCount = 0;
  for (const auto &workerConf : testHarnessWorkerConfigurations) {
    if (workerConf->getSourceType() ==
            TestHarnessWorkerConfiguration::TestHarnessWorkerSourceType::
                MemorySource &&
        workerConf->getRecords().empty()) {
      throw Exceptions::RuntimeException(
          "TestHarness: No Record defined for Memory Source with logical "
          "source Name: " +
  ...
  if (sourceCount == 0) {
    throw Exceptions::RuntimeException(
        "TestHarness: No Physical source defined in the test harness.");
  }
  return *this;
}

PhysicalSourceTypePtr TestHarness::createPhysicalSourceOfLambdaType(
    TestHarnessWorkerConfigurationPtr workerConf) {
  auto logicalSourceName = workerConf->getLogicalSourceName();
  auto found = std::find_if(
      logicalSources.begin(), logicalSources.end(),
      [logicalSourceName](const LogicalSourcePtr &logicalSource) {
  ...
  if (found == logicalSources.end()) {
    throw Exceptions::RuntimeException(
        ...
        ". Make sure you are adding a logical source with the name to the test "
        "harness.");
  }
  return workerConf->getPhysicalSourceType();
};

PhysicalSourceTypePtr TestHarness::createPhysicalSourceOfMemoryType(
    TestHarnessWorkerConfigurationPtr workerConf) {
  ...
  memSrcNumBuffToProcess =
      std::ceil(static_cast<double>(memAreaSize) /
                workerConf->getWorkerConfiguration()->bufferSizeInBytes);
  NES_DEBUG("memSrcNumBuffToProcess = {} currentSourceNumOfRecords = {}",
            memSrcNumBuffToProcess, currentSourceNumOfRecords);

  auto memorySourceType = MemorySourceType::create(
      logicalSourceName, workerConf->getPhysicalSourceName(), memArea,
      memAreaSize, memSrcNumBuffToProcess, memSrcFrequency,
...
TestHarness &TestHarness::runQuery(uint64_t numberOfRecordsToExpect,
  ...
  if (!topologySetupDone || !validationDone) {
    throw Exceptions::RuntimeException(
        "Make sure to call first validate() and then setupTopology() to the "
        "test harness before checking the output");
  }
  ...
  auto placementStrategy =
      magic_enum::enum_cast<Optimizer::PlacementStrategy>(placementStrategyName)
          .value();
  queryId = INVALID_QUERY_ID;

  auto query = queryWithoutSink->sink(
      FileSinkDescriptor::create(filePath, "CSV_FORMAT", "APPEND"));
  queryId = requestHandlerService->validateAndQueueAddQueryRequest(
      query.getQueryPlan(), placementStrategy);
  ...
  if (!TestUtils::checkOutputContentLengthOrTimeout(
          queryId, queryCatalog, numberOfRecordsToExpect, filePath,
          testTimeoutInSeconds)) {
    NES_THROW_RUNTIME_ERROR("TestHarness: checkOutputContentLengthOrTimeout "
                            "returns false, number of bytes to expect = "
                            << numberOfRecordsToExpect);
  }

  if (!TestUtils::checkStoppedOrTimeout(queryId, queryCatalog)) {
    NES_THROW_RUNTIME_ERROR(
        "TestHarness: checkStoppedOrTimeout returns false for query with id= "
        << queryId);
  }
...
std::vector<Runtime::MemoryLayouts::TestTupleBuffer> TestHarness::getOutput() {
  ...
  auto tupleBuffers = TestUtils::createExpectedBuffersFromCsv(
      filePath, schema, bufferManager, true);
  return TestUtils::createTestTupleBuffers(tupleBuffers, schema);
}

TestHarness &TestHarness::setupTopology(
  ...
  auto coordinatorRPCPort =
      nesCoordinator->startCoordinator(/**blocking**/ false);
  ...
  for (auto &workerConf : testHarnessWorkerConfigurations) {
    ...
    workerConfiguration->queryCompiler.queryCompilerDumpMode =
        QueryCompilation::DumpMode::CONSOLE;
    ...
    case TestHarnessWorkerConfiguration::TestHarnessWorkerSourceType::
        MemorySource: {
    ...
    nesWorker->replaceParent(
        WorkerId(1), nesWorker->getWorkerConfiguration()->parentId.getValue());
  ...
  for (const auto &workerId : workerIds) {
    while (!topology->nodeWithWorkerIdExists(workerId)) {
      if (std::chrono::system_clock::now() >
          start_timestamp + SETUP_TIMEOUT_IN_SEC) {
        NES_THROW_RUNTIME_ERROR(
            "TestHarness: Unable to find setup topology in given timeout.");
      }
    }
  }
  ...
  if (!distributionList.empty()) {
    for (auto jsonFields : distributionList) {
      ...
      for (const auto &phSource : physicalSources) {
        if (phSource->getLogicalSource()->getLogicalSourceName() ==
                logSourceName &&
            phSource->getPhysicalSource()->getPhysicalSourceName() ==
                phSourceName &&
...
Runtime::BufferManagerPtr TestHarness::getBufferManager() const {
