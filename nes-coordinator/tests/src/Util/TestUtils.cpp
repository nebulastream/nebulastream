...
/**
     ...
     */
[[nodiscard]] bool checkCompleteOrTimeout(const Runtime::NodeEnginePtr& ptr, SharedQueryId sharedQueryId, uint64_t expectedResult)
{
    ...
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec)
    {
        ...
        NES_TRACE(
            "checkCompleteOrTimeout: NodeEnginePtr sleep because val={} < {}",
...
[[nodiscard]] bool checkStoppedOrTimeoutAtWorker(SharedQueryId sharedQueryId, NesWorkerPtr worker, std::chrono::seconds timeout)
{
    ...
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec)
    {
        ...
        NES_DEBUG(
            "checkStoppedOrTimeout: status not reached for {} at worker {}. Current state =  {}",
            sharedQueryId,
            worker->getWorkerId(),
            status);
        std::this_thread::sleep_for(sleepDuration);
...
/**
     ...
     * @return true if successful
     */
[[nodiscard]] bool checkFailedOrTimeout(QueryId queryId, const Catalogs::Query::QueryCatalogPtr& queryCatalog, std::chrono::seconds timeout)
{
    auto timeoutInSec = std::chrono::seconds(timeout);
    auto start_timestamp = std::chrono::system_clock::now();
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec)
...
[[nodiscard]] bool checkCompleteOrTimeout(QueryId queryId, uint64_t expectedNumberBuffers, const std::string& restPort)
{
    ...
    NES_DEBUG(
        "checkCompleteOrTimeout: QueryId expected results are not reached after timeout currentResult={}"
...
bool buffersContainSameTuples(
    ...
    bool orderSensitive)
{
    auto numTuplesExpected = std::accumulate(
        expectedBuffers.begin(),
        expectedBuffers.end(),
        0_u64,
        [](const uint64_t sum, const auto& buf) { return sum + buf.getNumberOfTuples(); });
    auto numTuplesActual = std::accumulate(
        actualBuffers.begin(),
        actualBuffers.end(),
        0_u64,
        [](const uint64_t sum, const auto& buf) { return sum + buf.getNumberOfTuples(); });
    ...
    if (orderSensitive)
    {
        ...
        while (expectedBufferPos < expectedBuffers.size() && actualBufferPos < actualBuffers.size())
        {
            ...
            if (expectedTuple != actualTuple)
            {
                ...
                NES_ERROR("Tuples {} and {} are not equal!", expectedTuple.toString(expectedSchema), actualTuple.toString(actualSchema));
                return false;
            }
        ...
        for (auto& expectedBuffer : expectedBuffers)
        {
            for (auto&& expectedTuple : expectedBuffer)
            {
                uint64_t occurrencesExpected = std::accumulate(
                    ...
                    expectedBuffers.end(),
                    0_u64,
                    [&](auto sum, const auto& innerActualBuffer) { return sum + innerActualBuffer.countOccurrences(expectedTuple); });
...
bool waitForWorkers(uint64_t restPort, uint16_t maxTimeout, uint16_t expectedWorkers)
{
    ...
    NES_ASSERT2_FMT(
        nodeNo == expectedWorkers, "Expected worker number not reached correctly " << nodeNo << " but expected " << expectedWorkers);
    return false;
}
...
std::vector<Runtime::TupleBuffer> TestUtils::createExpectedBuffersFromCsv(
    const string& csvFileName, const SchemaPtr& schema, const Runtime::BufferManagerPtr& bufferManager, uint64_t numTuplesPerBuffer)
{
...
uint64_t TestUtils::countTuples(std::vector<Runtime::TupleBuffer>& buffers)
{
    return std::accumulate(
        buffers.begin(), buffers.end(), 0_u64, [](const uint64_t sum, const auto& buf) { return sum + buf.getNumberOfTuples(); });
}
