...
[[nodiscard]] std::string enableSlicingWindowing(bool prefix) {
  return configOption(
      QUERY_COMPILER_CONFIG + "." + QUERY_COMPILER_WINDOWING_STRATEGY_CONFIG,
...
[[nodiscard]] bool checkCompleteOrTimeout(const Runtime::NodeEnginePtr& ptr,
  ...
  NES_TRACE(
      "checkCompleteOrTimeout: NodeEnginePtr expected results are not reached "
      "after timeout");
  return false;
}
...
[[nodiscard]] bool waitForQueryToStart(
  ...
  NES_TRACE(
      "checkCompleteOrTimeout: waitForStart expected results are not reached "
      "after timeout");
  return false;
}
...
[[nodiscard]] bool checkStoppedOrTimeoutAtWorker(SharedQueryId sharedQueryId,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    NES_DEBUG(
        "checkStoppedOrTimeout: status not reached for {} at worker {}. "
        "Current state =  {}",
...
[[nodiscard]] bool checkOutputOrTimeout(string expectedContent,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    if (ifs.good() && ifs.is_open()) {
      ...
      if (expectedlines.size() != count) {
        NES_TRACE(
            "checkoutputortimeout: number of expected lines {} not reached yet "
            "with {} lines content={} file={}",
      ...
      if (content.size() != expectedContent.size()) {
        NES_TRACE(
            "checkoutputortimeout: number of chars {} not reached yet with "
            "chars content={} lines content={}",
  ...
  NES_ERROR(
      "checkOutputOrTimeout: expected ({}) result not reached ({}) within set "
      "timeout content",
      count, found);
...
[[nodiscard]] bool checkIfOutputFileIsNotEmtpy(uint64_t minNumberOfLines,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    if (ifs.good() && ifs.is_open()) {
      ...
      if (count < minNumberOfLines) {
        NES_TRACE(
            "checkIfOutputFileIsNotEmtpy: number of min lines {} not reached "
            "yet with {} lines content={}",
  ...
  NES_ERROR(
      "checkIfOutputFileIsNotEmtpy: expected ({}) result not reached ({}) "
...
[[nodiscard]] bool checkFileCreationOrTimeout(const string& outputFilePath) {
  ...
  NES_TRACE(
      "checkFileCreationOrTimeout: expected result not reached within set "
      "timeout");
  return false;
}
...
[[nodiscard]] bool checkCompleteOrTimeout(QueryId queryId,
  ...
  NES_DEBUG(
      "checkCompleteOrTimeout: Check if the query goes into the Running status "
      "within the timeout");
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    std::string url =
  ...
  NES_DEBUG(
      "checkCompleteOrTimeout: QueryId expected results are not reached after "
      "timeout currentResult={}"
...
bool buffersContainSameTuples(
  ...
  if (numTuplesExpected != numTuplesActual) {
    NES_ERROR(
        "expected {} and actual buffers {} do not contain the same number of "
        "tuples!",
    ...
    for (auto& expectedBuffer : expectedBuffers) {
      for (auto&& expectedTuple : expectedBuffer) {
        ...
        if (occurrencesExpected == 0) {
          NES_ERROR(
              "Something is wrong as the expected tuple should be occurring at "
              "least once!");
          return false;
        }
        ...
        if (occurrencesExpected != occurrencesActual) {
          NES_ERROR(
              "Could not find same number of occurrences expected: {} actual: "
              "{}",
...
[[nodiscard]] nlohmann::json makeMonitoringRestCall(
    const string& restCall, const std::string& restPort) {
...
CSVSourceTypePtr TestUtils::createSourceTypeCSV(
...
std::vector<PhysicalTypePtr> TestUtils::getPhysicalTypes(
