...
configOption(const std::string &name, const std::string &value, bool prefix) {
...
[[nodiscard]] std::string numLocalBuffers(uint64_t localBuffers, bool prefix) {
  return configOption(NUMBER_OF_BUFFERS_IN_SOURCE_LOCAL_BUFFER_POOL_CONFIG,
                      localBuffers, prefix);
}

[[nodiscard]] std::string numGlobalBuffers(uint64_t globalBuffers,
                                           bool prefix) {
  return configOption(NUMBER_OF_BUFFERS_IN_GLOBAL_BUFFER_MANAGER_CONFIG,
                      globalBuffers, prefix);
}
...
[[nodiscard]] std::string sourceType(SourceType sourceType) {
  return "--physicalSources." + SOURCE_TYPE_CONFIG + "=" +
         std::string(magic_enum::enum_name(sourceType));
}
...
[[nodiscard]] std::string physicalSourceName(std::string physicalSourceName) {
  return "--physicalSources." + PHYSICAL_SOURCE_NAME_CONFIG + "=" +
         physicalSourceName;
}

[[nodiscard]] std::string logicalSourceName(std::string logicalSourceName) {
  return "--physicalSources." + LOGICAL_SOURCE_NAME_CONFIG + "=" +
         logicalSourceName;
}

[[nodiscard]] std::string
numberOfBuffersToProduce(uint64_t numberOfBuffersToProduce) {
  return "--physicalSources." + NUMBER_OF_BUFFERS_TO_PRODUCE_CONFIG + "=" +
         std::to_string(numberOfBuffersToProduce);
}

[[nodiscard]] std::string
sourceGatheringInterval(uint64_t sourceGatheringInterval) {
  return "--physicalSources." + SOURCE_GATHERING_INTERVAL_CONFIG + "=" +
         std::to_string(sourceGatheringInterval);
}
...
[[nodiscard]] std::string enableSlicingWindowing(bool prefix) {
  return configOption(QUERY_COMPILER_CONFIG + "." +
                          QUERY_COMPILER_WINDOWING_STRATEGY_CONFIG,
                      std::string{"SLICING"}, prefix);
}
...
startCoordinator(std::initializer_list<std::string> list) {
  auto crdPath =
      std::string(PATH_TO_BINARY_DIR) + "/nes-coordinator/nesCoordinator";
...
startWorkerPtr(std::initializer_list<std::string> flags) {
  NES_INFO("Start worker");
  return std::make_shared<Util::Subprocess>(
      std::string(PATH_TO_BINARY_DIR) + "/nes-worker/nesWorker", flags);
}
...
[[nodiscard]] bool checkCompleteOrTimeout(const Runtime::NodeEnginePtr &ptr,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    if (ptr->getQueryStatistics(sharedQueryId)[0]->getProcessedBuffers() ==
            expectedResult &&
        ptr->getQueryStatistics(sharedQueryId)[0]->getProcessedTasks() ==
            expectedResult) {
  ...
  NES_TRACE("checkCompleteOrTimeout: NodeEnginePtr expected results are not "
...
waitForQueryToStart(QueryId queryId,
  ...
  NES_TRACE("TestUtils: wait till the query {} gets into Running status.",
            queryId);
  auto start_timestamp = std::chrono::system_clock::now();

  NES_TRACE(
      "TestUtils: Keep checking the status of query {} until a fixed time out",
      queryId);
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    QueryState queryState = queryCatalog->getQueryState(queryId);
    NES_TRACE("TestUtils: Query {} is now in status {}", queryId,
  ...
  NES_TRACE("checkCompleteOrTimeout: waitForStart expected results are not "
            "reached after timeout");
  return false;
}
...
checkStoppedOrTimeout(QueryId queryId,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    if (queryState == QueryState::STOPPED) {
      NES_DEBUG("checkStoppedOrTimeout: status for {} reached stopped",
                queryId);
      return true;
    }
    NES_WARNING(
 ...
 * @return true if successful
 */
[[nodiscard]] bool checkStoppedOrTimeoutAtWorker(SharedQueryId sharedQueryId,
                                                 NesWorkerPtr worker,
                                                 std::chrono::seconds timeout) {
  auto timeoutInSec = std::chrono::seconds(timeout);
  auto start_timestamp = std::chrono::system_clock::now();
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    NES_TRACE("checkStoppedOrTimeout: check query status for {}",
              sharedQueryId);
    if (worker->getNodeEngine()->getQueryStatus(sharedQueryId) ==
    ...
    case Runtime::Execution::ExecutableQueryPlanStatus::Created:
      status = "created";
      break;
    case Runtime::Execution::ExecutableQueryPlanStatus::Deployed:
      status = "deployed";
      break;
    case Runtime::Execution::ExecutableQueryPlanStatus::Running:
      status = "running";
      break;
    case Runtime::Execution::ExecutableQueryPlanStatus::Finished:
      status = "finished";
      break;
    case Runtime::Execution::ExecutableQueryPlanStatus::Stopped:
      status = "stopped";
      break;
    case Runtime::Execution::ExecutableQueryPlanStatus::ErrorState:
      status = "error";
      break;
    case Runtime::Execution::ExecutableQueryPlanStatus::Invalid:
      status = "invalid";
      break;
    }
    NES_DEBUG("checkStoppedOrTimeout: status not reached for {} at worker {}. "
              "Current state =  {}",
              sharedQueryId, worker->getWorkerId(), status);
    std::this_thread::sleep_for(sleepDuration);
  }
  NES_TRACE(
      "checkStoppedOrTimeout: expected status not reached within set timeout");
...
[[nodiscard]] bool checkOutputOrTimeout(string expectedContent,
                                        ...
                                        uint64_t customTimeoutInSeconds) {
  std::chrono::seconds timeoutInSec;
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    found = 0;
    count = 0;
    NES_TRACE("checkOutputOrTimeout: check content for file {}",
              outputFilePath);
    std::ifstream ifs(outputFilePath);
    if (ifs.good() && ifs.is_open()) {
      std::vector<std::string> expectedlines =
          NES::Util::splitWithStringDelimiter<std::string>(expectedContent,
                                                           "\n");
      content = std::string((std::istreambuf_iterator<char>(ifs)),
                            (std::istreambuf_iterator<char>()));
      count = std::count(content.begin(), content.end(), '\n');
      if (expectedlines.size() != count) {
        NES_TRACE("checkoutputortimeout: number of expected lines {} not "
                  "reached yet with {} lines content={} file={}",
                  expectedlines.size(), count, content, outputFilePath);
        continue;
      }

      if (content.size() != expectedContent.size()) {
        NES_TRACE("checkoutputortimeout: number of chars {} not reached yet "
                  "with chars content={} lines content={}",
                  expectedContent.size(), content.size(), content);
        continue;
      }
  ...
  NES_ERROR("checkOutputOrTimeout: expected ({}) result not reached ({}) "
...
[[nodiscard]] bool checkIfOutputFileIsNotEmtpy(uint64_t minNumberOfLines,
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    std::this_thread::sleep_for(sleepDuration);
    count = 0;
    NES_TRACE("checkIfOutputFileIsNotEmtpy: check content for file {}",
              outputFilePath);
    std::ifstream ifs(outputFilePath);
    if (ifs.good() && ifs.is_open()) {
      ...
      if (count < minNumberOfLines) {
        NES_TRACE("checkIfOutputFileIsNotEmtpy: number of min lines {} not "
                  "reached yet with {} lines content={}",
                  minNumberOfLines, count, content);
        continue;
      }
      NES_TRACE("at least {} are found in content={}", minNumberOfLines,
                content);
      return true;
    }
  }
  NES_ERROR("checkIfOutputFileIsNotEmtpy: expected ({}) result not reached "
            "({}) within set timeout content",
            count, minNumberOfLines);
  return false;
}
...
[[nodiscard]] bool checkFileCreationOrTimeout(const string &outputFilePath) {
  ...
  NES_TRACE("checkFileCreationOrTimeout: expected result not reached within "
            "set timeout");
  return false;
}
...
[[nodiscard]] bool checkCompleteOrTimeout(QueryId queryId,
  ...
  NES_DEBUG("checkCompleteOrTimeout: Check if the query goes into the Running "
            "status within the timeout");
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    std::string url =
        "http://localhost:" + restPort + "/v1/nes/queryCatalog/status";
    nlohmann::json jsonReturn;
    auto future = cpr::GetAsync(
        cpr::Url{url}, cpr::Parameters{{"queryId", queryId.toString()}});
    future.wait();
    ...
    NES_DEBUG("checkCompleteOrTimeout: sleep because current status ={}",
              currentStatus);
  ...
  while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
    ...
    std::string url = "http://localhost:" + restPort +
    ...
    nlohmann::json result = nlohmann::json::parse(response.text);
    if (response.status_code == cpr::status::HTTP_OK &&
        result.contains("producedBuffers")) {
    ...
    NES_DEBUG("checkCompleteOrTimeout: sleep because val={} < {}",
              currentResult, expectedNumberBuffers);
    std::this_thread::sleep_for(std::chrono::milliseconds(sleepDuration));
  }
  NES_DEBUG("checkCompleteOrTimeout: QueryId expected results are not reached "
            "after timeout currentResult={}"
...
bool buffersContainSameTuples(
  ...
  auto numTuplesExpected =
      std::accumulate(expectedBuffers.begin(), expectedBuffers.end(), 0_u64,
                      [](const uint64_t sum, const auto &buf) {
                        return sum + buf.getNumberOfTuples();
                      });
  auto numTuplesActual =
      std::accumulate(actualBuffers.begin(), actualBuffers.end(), 0_u64,
                      [](const uint64_t sum, const auto &buf) {
                        return sum + buf.getNumberOfTuples();
                      });

  if (numTuplesExpected != numTuplesActual) {
    NES_ERROR("expected {} and actual buffers {} do not contain the same "
              "number of tuples!",
              numTuplesExpected, numTuplesActual);
    return false;
  }
  ...
  if (orderSensitive) {
    // We care about the order and therefore, we can just iterate over expected
    // and actual tuples.
    ...
    while (expectedBufferPos < expectedBuffers.size() &&
           actualBufferPos < actualBuffers.size()) {
      if (expectedBufferTupleIdx ==
          expectedBuffers[expectedBufferPos].getNumberOfTuples()) {
        expectedBufferTupleIdx = 0_u64;
      }

      if (actualBufferTupleIdx ==
          actualBuffers[actualBufferPos].getNumberOfTuples()) {
        actualBufferTupleIdx = 0_u64;
      }

      auto expectedTuple =
          expectedBuffers[expectedBufferPos][expectedBufferTupleIdx];
      auto actualTuple = actualBuffers[actualBufferPos][actualBufferTupleIdx];
      if (expectedTuple != actualTuple) {
        const auto expectedSchema =
            expectedBuffers[expectedBufferPos].getMemoryLayout()->getSchema();
        const auto actualSchema =
            actualBuffers[actualBufferPos].getMemoryLayout()->getSchema();
    ...
    // We count the occurrences of each tuple in the expectedBuffers. Then we
    // count the occurrences of the expectedTuple in the actualBuffers. If the
    // occurrences are not equal, then not all tuples are in both buffers
    // (expected and actual).
    for (auto &expectedBuffer : expectedBuffers) {
      for (auto &&expectedTuple : expectedBuffer) {
        uint64_t occurrencesExpected = std::accumulate(
            expectedBuffers.begin(), expectedBuffers.end(), 0_u64,
            [&](auto sum, const auto &innerActualBuffer) {
              return sum + innerActualBuffer.countOccurrences(expectedTuple);
            });
        if (occurrencesExpected == 0) {
          NES_ERROR("Something is wrong as the expected tuple should be "
                    "occurring at least once!");
          return false;
        }

        uint64_t occurrencesActual = std::accumulate(
            actualBuffers.begin(), actualBuffers.end(), 0_u64,
            [&](auto sum, const auto &innerActualBuffer) {
              return sum + innerActualBuffer.countOccurrences(expectedTuple);
            });

        if (occurrencesExpected != occurrencesActual) {
          NES_ERROR("Could not find same number of occurrences expected: {} "
                    "actual: {}",
                    occurrencesExpected, occurrencesActual);
          return false;
        }
      }
    }
 ...
 * @return if stopped
 */
[[nodiscard]] nlohmann::json startQueryViaRest(const string &queryString,
                                               const std::string &restPort) {
  nlohmann::json json_return;
...
[[nodiscard]] nlohmann::json addSourceStatistics(const string &queryString,
                                                 const std::string &restPort) {
  nlohmann::json json_return;

  std::string url =
      BASE_URL + restPort + "/v1/nes/sourceCatalog/addSourceStatistics";
  nlohmann::json jsonReturn;
  auto future = cpr::PostAsync(
      cpr::Url{url}, cpr::Header{{"Content-Type", "application/json"}},
      cpr::Body{queryString});
...
makeMonitoringRestCall(const string &restCall, const std::string &restPort) {
...
[[nodiscard]] bool addLogicalSource(const string &schemaString,
  ...
  std::string url =
      BASE_URL + restPort + "/v1/nes/sourceCatalog/addLogicalSource";
  auto future = cpr::PostAsync(
      cpr::Url{url}, cpr::Header{{"Content-Type", "application/json"}},
...
bool waitForWorkers(uint64_t restPort, uint16_t maxTimeout,
  ...
  auto baseUri =
      "http://localhost:" + std::to_string(restPort) + "/v1/nes/topology";
  ...
  for (int i = 0; i < maxTimeout; i++) {
    try {
      ...
      if (nodeNo == expectedWorkers + 1U) {
        NES_INFO("TestUtils: Expected worker number reached correctly {}",
                 expectedWorkers);
  ...
  NES_ASSERT2_FMT(nodeNo == expectedWorkers,
                  "Expected worker number not reached correctly "
  ...
  while (std::getline(inputStream, csvLine)) {
    ...
    waypoints.push_back(NES::Spatial::DataTypes::Experimental::Waypoint(
        NES::Spatial::DataTypes::Experimental::GeoLocation(
            std::stod(latitudeString), std::stod(longitudeString)),
        time));
  }
  return waypoints;
}
...
void writeWaypointsToCsv(
  ...
  for (auto &point : waypoints) {
    ...
    outFile << point.getLocation().toString() << ","
            << std::to_string(point.getTimestamp().value()) << std::endl;
  }
  outFile.close();
  ASSERT_FALSE(outFile.fail());
}

std::vector<Runtime::TupleBuffer> TestUtils::createExpectedBuffersFromCsv(
    const string &csvFileName, const SchemaPtr &schema,
    const Runtime::BufferManagerPtr &bufferManager,
    uint64_t numTuplesPerBuffer) {
  return createExpectedBuffersFromCsv(csvFileName, schema, bufferManager, false,
                                      numTuplesPerBuffer);
}

std::vector<Runtime::TupleBuffer> TestUtils::createExpectedBuffersFromCsv(
    const std::string &csvFileName, const SchemaPtr &schema,
    const Runtime::BufferManagerPtr &bufferManager, bool skipHeader,
    uint64_t numTuplesPerBuffer, const std::string &delimiter) {
  ...
  NES_ASSERT2_FMT(std::filesystem::exists(std::filesystem::path(fullPath)),
                  "File " << fullPath << " does not exist!!!");
  std::ifstream inputFile(fullPath);
  return createExpectedBufferFromStream(inputFile, schema, bufferManager,
                                        skipHeader, numTuplesPerBuffer,
                                        delimiter);
}

uint64_t TestUtils::countTuples(std::vector<Runtime::TupleBuffer> &buffers) {
  return std::accumulate(buffers.begin(), buffers.end(), 0_u64,
                         [](const uint64_t sum, const auto &buf) {
                           return sum + buf.getNumberOfTuples();
                         });
}

uint64_t TestUtils::countTuples(
...
std::vector<Runtime::TupleBuffer> TestUtils::createExpectedBufferFromCSVString(
    std::string str, const SchemaPtr &schema,
    const Runtime::BufferManagerPtr &bufferManager, bool skipHeader,
    uint64_t numTuplesPerBuffer, const string &delimiter) {
  std::istringstream strStream(str);
  return createExpectedBufferFromStream(strStream, schema, bufferManager,
                                        skipHeader, numTuplesPerBuffer,
                                        delimiter);
}

std::vector<Runtime::TupleBuffer> TestUtils::createExpectedBufferFromStream(
  ...
  auto maxTuplePerBuffer =
      bufferManager->getBufferSize() / schema->getSchemaSizeInBytes();
  numTuplesPerBuffer =
      (numTuplesPerBuffer == 0) ? maxTuplePerBuffer : numTuplesPerBuffer;

  // Setting the position indicator to the beginning and clearing potential any
  // error flags
  istream.clear();
  ...
  while (std::getline(istream, line)) {
    auto testBuffer =
        Runtime::MemoryLayouts::TestTupleBuffer::createTestTupleBuffer(
            tupleBuffer, schema);
    parser->writeInputTupleToTupleBuffer(line, tupleCount, testBuffer, schema,
                                         bufferManager);
    tupleCount++;
...
CSVSourceTypePtr
TestUtils::createSourceTypeCSV(const SourceTypeConfigCSV &sourceTypeConfigCSV) {
  CSVSourceTypePtr sourceType =
      CSVSourceType::create(sourceTypeConfigCSV.logicalSourceName,
                            sourceTypeConfigCSV.physicalSourceName);
  sourceType->setFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                          sourceTypeConfigCSV.fileName);
  sourceType->setGatheringInterval(sourceTypeConfigCSV.gatheringInterval);
  sourceType->setNumberOfTuplesToProducePerBuffer(
      sourceTypeConfigCSV.numberOfTuplesToProduce);
  sourceType->setNumberOfBuffersToProduce(
      sourceTypeConfigCSV.numberOfBuffersToProduce);
  sourceType->setSkipHeader(sourceTypeConfigCSV.isSkipHeader);
  return sourceType;
}

std::vector<PhysicalTypePtr>
TestUtils::getPhysicalTypes(const SchemaPtr &schema) {
...
uint64_t countOccurrences(const std::string &subString,
