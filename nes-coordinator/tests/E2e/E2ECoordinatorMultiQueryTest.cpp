...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("E2ECoordinatorMultiQueryTest.log",
...
/**
 * @brief This test starts two workers and a coordinator and submit the same
 * query but will output the results in different files
 */
TEST_F(E2ECoordinatorMultiQueryTest,
       testExecutingValidUserQueryWithFileOutputTwoQueries) {
  ...
  auto coordinator = TestUtils::startCoordinator(
      {TestUtils::rpcPort(*rpcCoordinatorPort), TestUtils::restPort(*restPort),
       TestUtils::enableDebug(), TestUtils::numberOfSlots(8, true)});
  ASSERT_TRUE(TestUtils::waitForWorkers(*restPort, timeout, 0));

  auto worker = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::dataPort(0), TestUtils::enableDebug(),
  ...
  ssQuery1
      << R"({"userQuery" : "Query::from(\"default_logical\").sink(FileSinkDescriptor::create(\")";
  ssQuery1 << pathQuery1;
  ...
  nlohmann::json jsonReturnQ1 =
      TestUtils::startQueryViaRest(ssQuery1.str(), std::to_string(*restPort));
  ...
  nlohmann::json jsonReturnQ2 =
      TestUtils::startQueryViaRest(ssQuery2.str(), std::to_string(*restPort));
  ...
  ASSERT_TRUE(TestUtils::checkCompleteOrTimeout(queryId2, 1,
                                                std::to_string(*restPort)));
  string ASSERTedContent = "default_logical$id:INTEGER(32 "
...
/**
 * @brief This test starts two workers and a coordinator and submit too many
 * queryIdAndCatalogEntryMapping such that we test if the error-prone process
 */
TEST_F(E2ECoordinatorMultiQueryTest,
       testExecutingValidUserQueryWithFileOutputThreeQueriesWithErrorTest) {
  NES_INFO("start coordinator");
  std::string pathQuery1 = getTestResourceFolder() / "query1.out";
  std::string pathQuery2 = getTestResourceFolder() / "query2.out";
  ...
  nlohmann::json jsonReturnQ2 =
      TestUtils::startQueryViaRest(ssQuery2.str(), std::to_string(*restPort));
  nlohmann::json jsonReturnQ3 =
      TestUtils::startQueryViaRest(ssQuery3.str(), std::to_string(*restPort));
...
/**
 * @brief This test starts two workers and a coordinator and submits two
 * different queryIdAndCatalogEntryMapping
 */
TEST_F(E2ECoordinatorMultiQueryTest, testTwoQueriesWithFileOutput) {
  ...
  schema << "{\"logicalSourceName\" : \"QnV\",\"schema\" : "
            ...
            "DataTypeFactory::createText())->addField(createField("
            "\\\"timestamp\\\", "
            "BasicType::UINT64))->addField(createField(\\\"velocity\\\", "
            "BasicType::FLOAT32))->addField(createField(\\\"quantity\\\", "
            "BasicType::UINT64));\"}";
  schema << endl;
  ...
  ssQuery1 << "{\"userQuery\" : ";
  ssQuery1
      << R"("Query::from(\"QnV\").filter(Attribute(\"velocity\") > 100).sink(FileSinkDescriptor::create(\")";
  ssQuery1 << Qpath1;
  ssQuery1 << R"(\", \"CSV_FORMAT\", \"APPEND\")";
  ssQuery1 << R"());","placement" : "BottomUp"})";
  ssQuery1 << endl;
  ...
  ssQuery2 << "{\"userQuery\" : ";
  ssQuery2
      << R"("Query::from(\"QnV\").filter(Attribute(\"quantity\") > 10).sink(FileSinkDescriptor::create(\")";
  ssQuery2 << Qpath2;
  ssQuery2 << R"(\", \"CSV_FORMAT\", \"APPEND\")";
  ssQuery2 << R"());","placement" : "BottomUp"})";
  ssQuery2 << endl;
...
TEST_F(E2ECoordinatorMultiQueryTest,
  ...
  std::string outputFilePath2 =
      getTestResourceFolder() /
      "ValidUserQueryWithTumbWindowFileOutputTestResult2.txt";
  ...
  std::stringstream ss;
  ss << "{\"userQuery\" : ";
  ss << "\"Query::from(\\\"window\\\").window(TumblingWindow::of(EventTime("
        "Attribute(\\\"timestamp\\\")), Seconds(10)))"
  ...
  ss2 << "{\"userQuery\" : ";
  ss2 << "\"Query::from(\\\"window\\\").window(TumblingWindow::of(EventTime("
         "Attribute(\\\"timestamp\\\")), Seconds(20)))"
         ".byKey(Attribute(\\\"id\\\")).apply(Sum(Attribute(\\\"value\\\")))."
         "sink(FileSinkDescriptor::create(\\\"";
  ss2 << outputFilePath2;
  ...
  nlohmann::json json_return_Q2 =
      TestUtils::startQueryViaRest(ss2.str(), std::to_string(*restPort));
  ...
  string ASSERTedContent1 =
      ...
      "bits),window$id:INTEGER(64 bits),"
...
TEST_F(E2ECoordinatorMultiQueryTest,
  ...
  std::string outputFilePath2 =
      getTestResourceFolder() /
      "ValidUserQueryWithSlidWindowFileOutputTestResult2.txt";
  remove(outputFilePath.c_str());
  remove(outputFilePath2.c_str());
  ...
  std::stringstream ss;
  ss << "{\"userQuery\" : ";
  ss << "\"Query::from(\\\"window\\\").window(SlidingWindow::of(EventTime("
        "Attribute(\\\"timestamp\\\")), Seconds(10), "
        "Seconds(5)))"
        ".byKey(Attribute(\\\"id\\\")).apply(Sum(Attribute(\\\"value\\\")))."
        "sink(FileSinkDescriptor::create(\\\"";
  ...
  std::stringstream ss2;
  ss2 << "{\"userQuery\" : ";
  ss2 << "\"Query::from(\\\"window\\\").window(SlidingWindow::of(EventTime("
         "Attribute(\\\"timestamp\\\")), Seconds(20), "
         "Seconds(10)))"
         ".byKey(Attribute(\\\"id\\\")).apply(Sum(Attribute(\\\"value\\\")))."
         "sink(FileSinkDescriptor::create(\\\"";
  ...
  ASSERT_TRUE(
      TestUtils::checkOutputOrTimeout(ASSERTedContent1, outputFilePath));
  ASSERT_TRUE(
      TestUtils::checkOutputOrTimeout(ASSERTedContent2, outputFilePath2));
}
