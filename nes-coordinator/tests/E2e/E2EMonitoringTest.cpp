...
class E2EMonitoringTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("E2EMonitoringTest.log",
...
TEST_F(E2EMonitoringTest, requestStoredRegistrationMetrics) {
  ...
  auto worker1 = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::enableNautilusWorker(),
       TestUtils::dataPort(0), TestUtils::coordinatorPort(*rpcCoordinatorPort),
       TestUtils::enableMonitoring(), TestUtils::workerHealthCheckWaitTime(1)});
...
TEST_F(E2EMonitoringTest, requestAllMetricsViaRest) {
  ...
  auto jsons =
      TestUtils::makeMonitoringRestCall("metrics", std::to_string(*restPort));
  NES_INFO("ResourcesReaderTest: Jsons received: \n{}", jsons.dump());

  ASSERT_EQ(jsons.size(), noWorkers + 1);
  for (uint64_t i = 1; i <= noWorkers + 1; i++) {
    NES_INFO(
        "ResourcesReaderTest: Requesting monitoring data from node with ID {}",
        i);
...
TEST_F(E2EMonitoringTest, requestAllMetricsFromMonitoringStreams) {
  auto expectedMonitoringStreams =
      Monitoring::MonitoringPlan::defaultPlan()->getMetricTypes();
  uint64_t noWorkers = 2;
  auto coordinator = TestUtils::startCoordinator(
      {TestUtils::enableNautilusCoordinator(),
       TestUtils::rpcPort(*rpcCoordinatorPort), TestUtils::restPort(*restPort),
  ...
  auto jsonStart =
      TestUtils::makeMonitoringRestCall("start", std::to_string(*restPort));
  NES_INFO("E2EMonitoringTest: Started monitoring streams {}",
           jsonStart.dump());
  ASSERT_EQ(jsonStart.size(), expectedMonitoringStreams.size());

  ASSERT_TRUE(MetricValidator::waitForMonitoringStreamsOrTimeout(
      expectedMonitoringStreams, 100, *restPort));
  auto jsonMetrics =
      TestUtils::makeMonitoringRestCall("storage", std::to_string(*restPort));
  ...
  for (uint64_t i = 1; i <= noWorkers + 1; i++) {
    ...
    auto jsonString = json.dump();
    nlohmann::json jsonLohmann = nlohmann::json::parse(jsonString);
    ASSERT_TRUE(MetricValidator::isValidAllStorage(
        Monitoring::SystemResourcesReaderFactory::getSystemResourcesReader(),
        jsonLohmann));
...
TEST_F(E2EMonitoringTest, testNemoPlacementWithMonitoringSource) {
  ...
  auto lines = 5;
  ASSERT_TRUE(
      TestUtils::checkIfOutputFileIsNotEmtpy(lines, outputFilePath, 30));
