...
class E2ECoordinatorMultiWorkerTest : public Testing::BaseIntegrationTest {
 public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("E2ECoordinatorWorkerTest.log",
...
TEST_F(E2ECoordinatorMultiWorkerTest, testHierarchicalTopology) {
  ...
  schema << "{\"logicalSourceName\" : \"QnV\",\"schema\" : "
            "\"Schema::create()->addField(\\\"sensor_id\\\", "
            "DataTypeFactory::createFixedChar(8))->addField(createField("
            "\\\"timestamp\\\", BasicType::UINT64))"
  ...
  auto worker2 = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::dataPort(0),
       TestUtils::coordinatorPort(*rpcCoordinatorPort), TestUtils::parentId(2),
       TestUtils::sourceType(SourceType::CSV_SOURCE),
       TestUtils::csvSourceFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                                    "QnV_short.csv"),
       ...
       TestUtils::numberOfTuplesToProducePerBuffer(0),
       TestUtils::sourceGatheringInterval(1000),
       TestUtils::workerHealthCheckWaitTime(1), TestUtils::enableDebug()});
  ASSERT_TRUE(TestUtils::waitForWorkers(*restPort, 10000, 2));

  auto worker3 = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::dataPort(0),
       TestUtils::coordinatorPort(*rpcCoordinatorPort), TestUtils::parentId(3),
       TestUtils::sourceType(SourceType::CSV_SOURCE),
       TestUtils::csvSourceFilePath(std::filesystem::path(TEST_DATA_DIRECTORY) /
                                    "QnV_short.csv"),
       TestUtils::physicalSourceName("test_stream2"),
...
TEST_F(E2ECoordinatorMultiWorkerTest,
       testExecutingValidQueryWithFileOutputTwoWorkerSameSource) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() /
      "testExecutingValidQueryWithFileOutputTwoWorker.out";
  ...
  string expectedContent =
      "QnV$sensor_id:Text,QnV$timestamp:INTEGER(64 bits),QnV$velocity:Float(32 "
      "bits),QnV$quantity:INTEGER(64 bits)\n"
  ...
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(queryId, 2, std::to_string(*restPort)));
...
TEST_F(E2ECoordinatorMultiWorkerTest,
       testExecutingValidQueryWithFileOutputTwoWorkerDifferentSource) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() /
      "testExecutingValidQueryWithFileOutputTwoWorker.out";
  ...
  while (std::getline(ifs, line)) {
    ...
    std::vector<string> content =
        Util::splitWithStringDelimiter<std::string>(line, ",");
...
TEST_F(E2ECoordinatorMultiWorkerTest,
  ...
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(queryId, 3, std::to_string(*restPort)));
  ASSERT_TRUE(TestUtils::stopQueryViaRest(queryId, std::to_string(*restPort)));
...
TEST_F(E2ECoordinatorMultiWorkerTest, testExecutingValidJoinQueryWithNemo) {
  ...
  schemaLeft
      << "{\"logicalSourceName\" : \"left\",\"schema\" : \"Schema::create()"
  ...
  ASSERT_TRUE(
      TestUtils::addLogicalSource(schemaLeft.str(), std::to_string(*restPort)));

  std::stringstream schemaRight;
  schemaRight
      << "{\"logicalSourceName\" : \"right\",\"schema\" : \"Schema::create()"
         "->addField(createField(\\\"key\\\", BasicType::UINT32))"
         "->addField(createField(\\\"value\\\", BasicType::UINT32))"
         "->addField(createField(\\\"timestamp\\\", BasicType::UINT64));\"}";
  schemaRight << endl;
  NES_INFO("schema left submit={}", schemaRight.str());
  ASSERT_TRUE(TestUtils::addLogicalSource(schemaRight.str(),
  ...
  auto success =
      TestUtils::addSourceStatistics(request.dump(), std::to_string(*restPort));
  ...
  // deploy query
  std::string outputFilePath =
      getTestResourceFolder() / "testExecutingValidJoinQueryWithNemo.out";
  remove(outputFilePath.c_str());

  std::stringstream ss;
  ss << "{\"userQuery\" : ";
  ss << "\"Query::from(\\\"left\\\").joinWith(Query::from(\\\"right\\\"))."
        "where(Attribute(\\\"value\\\") == Attribute("
        "\\\"value\\\"))"
        ".window(TumblingWindow::of(EventTime(Attribute(\\\"timestamp\\\")), "
        "Milliseconds(1000)))"
  ...
  ASSERT_TRUE(
      TestUtils::checkCompleteOrTimeout(queryId, 4, std::to_string(*restPort)));
  ASSERT_TRUE(TestUtils::stopQueryViaRest(queryId, std::to_string(*restPort)));

  std::ifstream ifs(outputFilePath.c_str());
  ASSERT_TRUE(ifs.good());

  uint64_t expectedTuples =
      26 * 2 * 2 - 1;  // tuples * nodesPerNode * leafNodesPerNode - header line
