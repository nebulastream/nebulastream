...
public:
  ...
  static void SetUpTestCase() {
    NES::Logger::setupLogging("E2ECoordinatorSingleWorkerTest.log",
                              NES::LogLevel::LOG_DEBUG);
    NES_INFO("Setup E2e test class.");
  }
};

TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithPrintOutput) {

  auto coordinator =
      TestUtils::startCoordinator({TestUtils::rpcPort(*rpcCoordinatorPort),
                                   TestUtils::restPort(*restPort)});
  EXPECT_TRUE(TestUtils::waitForWorkers(*restPort, timeout, 0));

  auto worker =
      TestUtils::startWorker({TestUtils::rpcPort(0), TestUtils::dataPort(0),
  ...
  nlohmann::json json_return =
      TestUtils::startQueryViaRest(ss.str(), std::to_string(*restPort));
  ...
  EXPECT_TRUE(
      TestUtils::checkCompleteOrTimeout(queryId, 1, std::to_string(*restPort)));
  // EXPECT_TRUE(TestUtils::stopQueryViaRest(queryId,
  // std::to_string(*restPort)));
}

TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithFileOutput) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() / "ValidUserQueryWithFileOutputTestResult.txt";
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       DISABLED_testExecutingValidUserQueryVariableSizeWithFileOutput) {
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithFileOutputWithFilter) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() /
      "UserQueryWithFileOutputWithFilterTestResult.txt";
  remove(outputFilePath.c_str());
  ...
  std::string content((std::istreambuf_iterator<char>(outFile)),
                      (std::istreambuf_iterator<char>()));
  NES_INFO("content={}", content);
  std::string expected = "default_logical$id:INTEGER(32 "
                         "bits),default_logical$value:INTEGER(64 bits)\n"
                         "1,1\n"
                         "1,1\n"
                         "1,1\n"
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithFileOutputAndRegisterPhysource) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() /
      "ValidUserQueryWithFileOutputAndRegisterPhysourceTestResult.txt";
  remove(outputFilePath.c_str());
  ...
  std::string expectedContent = "default_logical$id:INTEGER(32 "
                                "bits),default_logical$value:INTEGER(64 bits)\n"
                                "1,1\n"
                                "1,1\n"
                                "1,1\n"
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithFileOutputKTMUseCase) {
  NES_INFO("start coordinator");
  ...
  schema
      << "{\"logicalSourceName\" : \"ktm\",\"schema\" "
         ":\"Schema::create()->addField(createField(\\\"Time\\\",BasicType::"
         "UINT64))->"
         "addField(createField(\\\"Dist\\\",BasicType::UINT64))->"
         "addField(createField(\\\"ABS_Front_Wheel_Press\\\",BasicType::"
         "FLOAT64))->"
         "addField(createField(\\\"ABS_Rear_Wheel_Press\\\",BasicType::FLOAT64)"
         ")->"
         "addField(createField(\\\"ABS_Front_Wheel_Speed\\\",BasicType::"
         "FLOAT64))->" // 5th col.
         ...
         "addField(createField(\\\"ECU_Water_Temperature\\\",BasicType::"
         "FLOAT64))->"
         "addField(createField(\\\"ECU_Oil_Temp_Sensor_Data\\\",BasicType::"
         "UINT64))->"
  ...
  ASSERT_TRUE(
      TestUtils::addLogicalSource(schema.str(), std::to_string(*restPort)));

  auto worker = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::dataPort(0),
       TestUtils::coordinatorPort(*rpcCoordinatorPort),
       ...
       TestUtils::enableNautilusWorker(), TestUtils::sourceGatheringInterval(1),
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithTumblingWindowFileOutput) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() /
      "ValidUserQueryWithTumbWindowFileOutputTestResult.txt";
  remove(outputFilePath.c_str());
  ...
  schema << "{\"logicalSourceName\" : \"window\",\"schema\" "
            ":\"Schema::create()->addField(createField(\\\"id\\\",BasicType::"
            "UINT64))"
  ...
  // if filter is applied correctly, no output is generated
  string expectedContent =
      "window$start:INTEGER(64 bits),window$end:INTEGER(64 "
      "bits),window$id:INTEGER(64 bits),window$value:INTEGER(64 bits)\n"
      "0,10000,1,51\n"
      "0,10000,12,1\n"
      "0,10000,4,1\n"
      "0,10000,16,2\n"
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithSlidingWindowFileOutput) {
  NES_INFO("start coordinator");
  std::string outputFilePath =
      getTestResourceFolder() /
      "ValidUserQueryWithSlidWindowFileOutputTestResult.txt";
  remove(outputFilePath.c_str());
...
TEST_F(E2ECoordinatorSingleWorkerTest, testKillWorkerWithoutQuery) {
  auto coordinator = TestUtils::startCoordinator(
      {TestUtils::rpcPort(*rpcCoordinatorPort), TestUtils::restPort(*restPort),
       TestUtils::coordinatorHealthCheckWaitTime(1), TestUtils::enableDebug()});
  EXPECT_TRUE(TestUtils::waitForWorkers(*restPort, timeout, 0));
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testExecutingValidUserQueryWithThresholdWindowFileOutputKTMUseCase) {
  NES_INFO("start coordinator");
  ...
  schema
      << "{\"logicalSourceName\" : \"ktm\",\"schema\" "
         ...
         "FLOAT64))->"
         "addField(createField(\\\"ECU_Oil_Temp_Sensor_Data\\\",BasicType::"
         "INT32))->" // TODO I changed that to i32 to prevent failure because of
  ...
  auto worker = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::dataPort(0),
       ...
       TestUtils::csvSourceFilePath(
           ...
           "ktm_thresholdtest.csv"), // I created a new file to open and close a
                                     // threshold window
       TestUtils::physicalSourceName("test_stream"),
       TestUtils::logicalSourceName("ktm"),
       TestUtils::numberOfBuffersToProduce(1),
  ...
  // TODO : without logicalStreamName$Attribute (qualified att names) the
  // Nautilus query fails with the error: attr. does not exits
  ...
  string expectedContent =
      "ktm$ABS_Lean_Angle:Float(32 bits),ktm$ABS_Front_Wheel_Speed:Float(64 "
      "bits),ktm$count:INTEGER(64 bits)\n"
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       testWindowExecutionWithDifferentTimeUnits) {
  NES_INFO("start coordinator");
  std::string testFile = getTestResourceFolder() /
                         "testWindowExecutionWithDifferentTimeUnits_output.csv";
  NES_INFO("testFile = {}", testFile);
  remove(testFile.c_str());
  ...
  schema << "{\"logicalSourceName\" : \"window_data\",\"schema\" "
            ":\"Schema::create()->addField(createField(\\\"value\\\",BasicType:"
            ":INT32))->"
            "addField(createField(\\\"timestamp\\\",BasicType::UINT64));\"}";
  schema << endl;
  ...
  auto worker = TestUtils::startWorker(
      {TestUtils::rpcPort(0), TestUtils::dataPort(0),
       ...
       TestUtils::csvSourceFilePath(
           std::string(TEST_DATA_DIRECTORY) +
           "window_increasing_timesteps.csv"), // I created a new file to open
                                               // and close a threshold window
       TestUtils::physicalSourceName("window_data"),
       TestUtils::logicalSourceName("window_data"),
       TestUtils::enableNautilusWorker(), TestUtils::enableSlicingWindowing()});
  ASSERT_TRUE(TestUtils::waitForWorkers(*restPort, timeout, 1));

  // Generates and runs testcase for different TimeUnits
  auto testWithUnits = [&](const auto &event, const auto &size,
                           const auto &slide, const auto &expectedContent) {
    std::stringstream ss;
    ss << "{\"userQuery\" : ";
    ss << R"("Query::from(\"window_data\"))";
    ss << fmt::format(".window(SlidingWindow::of(EventTime(Attribute("
                      "\\\"timestamp\\\"), {}()), {}(2), {}(1)))",
                      event, size, slide);
    ss << R"(.apply(Count()))";
    ss << R"(.sink(FileSinkDescriptor::create(\")";
    ...
    nlohmann::json json_return =
        TestUtils::startQueryViaRest(q, std::to_string(*restPort));
  ...
  // Tests combiniations of different TimeUnits in WindowDefinitation and
  // EventTimeField. Equal Units
  testWithUnits(
...
259200000,259200002,1
  ...
  // Items in Window 1(0s-2s): [1ms,2ms,3ms,1000ms], 2(1s-3s): [1000ms,
  // 2000ms]...
  testWithUnits(
...
259200000000,259200002000,1
  ...
  // WindoeDefintiation Unit >> EventTime. More obscure unit multiplier because
  // of minutes Items in Window 1(0min-2min): [1ms, 2ms, 3ms, 1s, 2s, 3s, 1min],
  // 2(2min-4min): [2min, 3min]...
  testWithUnits(
...
TEST_F(E2ECoordinatorSingleWorkerTest,
       DISABLED_testExecutingThresholdWindowKTMByKey) {
  NES_INFO("start coordinator");
  std::string testFile = getTestResourceFolder() / "ktm-results.csv";
  NES_INFO("testFile = {}", testFile);
  remove(testFile.c_str());
  ...
  schema
      << "{\"logicalSourceName\" : \"ktm\",\"schema\" "
         ":\"Schema::create()->addField(createField(\\\"Time\\\",INT32))->"
         "addField(createField(\\\"Dist\\\",UINT64))->"
         "addField(createField(\\\"ABS_Front_Wheel_Press\\\",UINT64))->"
         "addField(createField(\\\"ABS_Rear_Wheel_Press\\\",FLOAT64))->"
         "addField(createField(\\\"ABS_Front_Wheel_Speed\\\",FLOAT64))->" // 5th
                                                                          // col.
  ...
  string expectedContent =
      "ktm$count:INTEGER(64 bits),ktm$ABS_Lean_Angle:Float(64 "
      "bits),ktm$ABS_Pitch_Info:Float(64 "
