...
public:
   ...
   * @tparam WindowType
   * @return returns a shared pointer of the WindowType or throws an error if
   * the type can't be casted.
   */
  template <class WindowType> std::shared_ptr<WindowType> as() {
    if (instanceOf<WindowType>()) {
      return std::dynamic_pointer_cast<WindowType>(this->shared_from_this());
    }
    throw std::logic_error(
        "WindowType:: we performed an invalid cast of operator " +
        this->toString() + " to type " + typeid(WindowType).name());
  }
   ...
   *
   * This function computes a hash value uniquely identifying the window type
   * including characteristic attributes. The hash value is different for
   * different WindowTypes and same WindowTypes with different attributes.
   * Especially a SlidingWindow of same size and slide returns a different hash
   * value, than a TumblingWindow with the same size.
   *
