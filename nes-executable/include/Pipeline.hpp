...
struct Pipeline
{
    ...
    using PipelineOperator = std::
        variant<std::shared_ptr<PhysicalOperator>, std::shared_ptr<SourceDescriptorLogicalOperator>, std::shared_ptr<SinkLogicalOperator>>;
...
struct OperatorPipeline : public Pipeline
{
    ...
    bool hasOperators() override { return operators.size() != 0; }
...
struct SourcePipeline : public Pipeline
{
    ...
    {
        PRECONDITION(
            std::holds_alternative<std::shared_ptr<SourceDescriptorLogicalOperator>>(*op),
            "Should have hold a SourceDescriptorLogicalOperator");
        sourceOperator = (std::get<std::shared_ptr<SourceDescriptorLogicalOperator>>(*op));
    }

    [[nodiscard]] std::string toString() const override;

    bool hasOperators() override { return sourceOperator != nullptr; }
...
struct SinkPipeline : public Pipeline
{
    ...
    bool hasOperators() override { return sinkOperator != nullptr; }
