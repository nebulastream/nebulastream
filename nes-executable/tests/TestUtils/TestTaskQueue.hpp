...
{
public:
    explicit TestPipelineExecutionContext(const NES::PipelineId pipelineId, const NES::WorkerThreadId workerId) : numberOfWorkerThreads(numberOfWorkerThreads), pipelineId(pipelineId), workerId(workerId)
    {
    ...
    void emitBuffer(const NES::Memory::TupleBuffer& resultBuffer, ContinuationPolicy) override
    {
        ...
        emittedResultBuffers.emplace_back(resultBuffer);
    }
    ...
    std::vector<NES::Memory::TupleBuffer> emittedResultBuffers;
private:
    NES::Memory::BufferManagerPtr bufferManager = NES::Memory::BufferManager::create(16, 10);
...
public:
    ...
    void executeStep(size_t step_index) {
        if (step_index < steps_.size()) {
            if(std::holds_alternative<TaskPipeline>(steps_.at(step_index).second)) {
                std::get<TaskPipeline>(steps_[step_index].second)->execute( /* Todo: TupleBuffer, PipelineExecutionContext */);
            }
        }
    }
    ...
    void executeAllSteps() {
        for (auto& step : steps_) {
            if(std::holds_alternative<TaskPipeline>(step.second)) {
                std::get<TaskPipeline>(step.second)->execute( /* Todo: TupleBuffer, PipelineExecutionContext */);
            }
        }
        completed_ = true;
    }
...
private:
    std::string name_;
    std::vector<std::pair<std::string, TestTask>> steps_;
    bool completed_;
...
class TestTaskQueue {
public:
    TestTaskQueue(const size_t numThreads = 1) : numberOfWorkerThreads(numThreads), running_(true) {
...
//            worker.join(); //Todo: use j thread and don't join
//        }
    }
    ...
    void executeStepForAllTasks(size_t stepIndex) {
        std::lock_guard<std::mutex> lock(mutex_);
