...
void ExecutionTrace::addOperation(TraceOperation &operation) {
  ...
  operation.operationRef = std::make_shared<OperationRef>(
      currentBlock, blocks[currentBlock].operations.size());
...
Block &ExecutionTrace::processControlFlowMerge(uint32_t blockIndex,
                                               uint32_t operationIndex) {
  // perform a control flow merge and merge the current block with operations in
  // some other block. create new block
  ...
  for (uint32_t opIndex = operationIndex; opIndex < oldBlock.operations.size();
       opIndex++) {
  ...
  oldBlock.operations.erase(oldBlock.operations.begin() + operationIndex,
                            oldBlock.operations.end());
  oldBlock.operations.emplace_back(TraceOperation(
      OpCode::JMP,
  ...
  //
  auto &lastMergeOperation =
      mergeBlock.operations[mergeBlock.operations.size() - 1];
  if (lastMergeOperation.op == OpCode::CMP ||
      lastMergeOperation.op == OpCode::JMP) {
    for (auto &input : lastMergeOperation.input) {
      ...
      std::replace(blockPredecessor.begin(), blockPredecessor.end(), blockIndex,
                   mergedBlockId);
      std::replace(blockPredecessor.begin(), blockPredecessor.end(),
                   currentBlock, mergedBlockId);
    }
  }
