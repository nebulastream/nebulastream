...
SSACreationPhase::apply(std::shared_ptr<ExecutionTrace> trace) {
...
SSACreationPhase::SSACreationPhaseContext::SSACreationPhaseContext(
    std::shared_ptr<ExecutionTrace> trace)
    : trace(std::move(trace)) {}

std::shared_ptr<ExecutionTrace>
SSACreationPhase::SSACreationPhaseContext::process() {
...
bool SSACreationPhase::SSACreationPhaseContext::isLocalValueRef(
    Block &block, ValueRef &type, uint32_t operationIndex) {
  ...
  for (uint32_t i = 0; i < operationIndex; i++) {
    ...
    if (auto resultValueRef = std::get_if<ValueRef>(&resOperation.result)) {
      if (resultValueRef->blockId == type.blockId &&
          resultValueRef->operationId == type.operationId) {
        return true;
      }
    }
  }
  // check if the operation is defined in the block arguments
  return std::find(block.arguments.begin(), block.arguments.end(), type) !=
         block.arguments.end();
}
...
void SSACreationPhase::SSACreationPhaseContext::processValueRef(
    Block &block, ValueRef &type, uint32_t operationIndex) {
    ...
    for (auto &predecessor : block.predecessors) {
      ...
      if (lastOperation.op == OpCode::JMP || lastOperation.op == OpCode::CMP) {
        ...
        NES_THROW_RUNTIME_ERROR(
            "Last operation of pred block should be JMP or CMP");
      }
    }
  }
}

void SSACreationPhase::SSACreationPhaseContext::processBlockRef(
    Block &block, BlockRef &blockRef, uint32_t operationIndex) {
  ...
  for (Block &block : trace->getBlocks()) {
    ...
    for (uint64_t i = 0; i < block.operations.size(); i++) {
      ...
      if (operation.op == OpCode::ASSIGN) {
        assignmentMap[get<ValueRef>(operation.result)] =
            get<ValueRef>(operation.input[0]);
      } else {
        for (auto &input : operation.input) {
          if (auto *valueRef = std::get_if<ValueRef>(&input)) {
            auto foundAssignment = assignmentMap.find(*valueRef);
    ...
    std::erase_if(block.operations,
                  [&](const auto &item) { return item.op == OpCode::ASSIGN; });
  }
}
  ...
  for (Block &block : trace->getBlocks()) {
    ...
    for (uint64_t argIndex = 0; argIndex < block.arguments.size(); argIndex++) {
      ...
      if (argRef.blockId != block.blockId) {
        auto newLocalRef =
            ValueRef(block.blockId,
                     block.operations.size() + blockArgumentMap.size() + 100,
                     argRef.type);
