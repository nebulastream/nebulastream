...
std::shared_ptr<NES::Nautilus::IR::IRGraph> TraceToIRConversionPhase::apply(
...
TraceToIRConversionPhase::IRConversionContext::process() {
  auto& rootBlock = trace->getBlocks().front();
  auto rootIrBlock = processBlock(0, rootBlock);

  auto& returnOperation =
      trace->getBlock(trace->getReturn()->blockId).operations.back();
  ...
  auto functionOperation =
      std::make_shared<NES::Nautilus::IR::Operations::FunctionOperation>(
          "execute",
          /*argumentTypes*/
          std::vector<NES::Nautilus::IR::Operations::PrimitiveStamp>{},
          /*arguments*/ std::vector<std::string>{}, returnType);
...
TraceToIRConversionPhase::IRConversionContext::processBlock(int32_t scope,
                                                            Block& block) {
  // create new frame and block
  ValueFrame blockFrame;
  std::vector<
      std::shared_ptr<NES::Nautilus::IR::Operations::BasicBlockArgument>>
      blockArguments;
  for (auto& arg : block.arguments) {
    auto argumentIdentifier = createValueIdentifier(arg);
    auto blockArgument =
        std::make_shared<NES::Nautilus::IR::Operations::BasicBlockArgument>(
            argumentIdentifier, arg.type);
    blockArguments.emplace_back(blockArgument);
    blockFrame.setValue(argumentIdentifier, blockArgument);
  }

  NES::Nautilus::IR::BasicBlockPtr irBasicBlock = std::make_shared<
      NES::Nautilus::IR::BasicBlock>(
      std::to_string(block.blockId), scope,
...
void TraceToIRConversionPhase::IRConversionContext::processOperation(
    int32_t scope, ValueFrame& frame, Block& currentBlock,
  ...
  switch (operation.op) {
    ...
    case OpCode::RETURN: {
      if (std::get<ValueRef>(operation.result).type->isVoid()) {
        auto operation =
            std::make_shared<NES::Nautilus::IR::Operations::ReturnOperation>();
        currentIrBlock->addOperation(operation);
      } else {
        auto returnValue =
            frame.getValue(createValueIdentifier(operation.input[0]));
        auto operation =
            std::make_shared<NES::Nautilus::IR::Operations::ReturnOperation>(
                returnValue);
        currentIrBlock->addOperation(operation);
...
void TraceToIRConversionPhase::IRConversionContext::processJMP(
    int32_t scope, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& block,
    TraceOperation& operation) {
  std::stringstream operationAsString;
  operationAsString << operation;
  NES_DEBUG("current block {} {}", block->getIdentifier(),
            operationAsString.str());
  ...
  if (blockMap.contains(blockRef.block)) {
    block->addNextBlock(blockMap[blockRef.block],
                        blockInvocation.getArguments());
    return;
  }
  ...
  auto resultTargetBlock =
      processBlock(scope - 1, trace->getBlock(blockRef.block));
  blockMap[blockRef.block] = resultTargetBlock;
  block->addNextBlock(resultTargetBlock, blockInvocation.getArguments());
}

void TraceToIRConversionPhase::IRConversionContext::processCMP(
    int32_t scope, ValueFrame& frame, Block&,
    NES::Nautilus::IR::BasicBlockPtr& currentIrBlock,
    TraceOperation& operation) {
  ...
  auto ifOperation =
      std::make_shared<NES::Nautilus::IR::Operations::IfOperation>(
          booleanValue);
  auto trueCaseBlock =
      processBlock(scope + 1, trace->getBlock(trueCaseBlockRef.block));

  ifOperation->getTrueBlockInvocation().setBlock(trueCaseBlock);
  createBlockArguments(frame, ifOperation->getTrueBlockInvocation(),
                       trueCaseBlockRef);

  auto falseCaseBlock =
      processBlock(scope + 1, trace->getBlock(falseCaseBlockRef.block));
  ifOperation->getFalseBlockInvocation().setBlock(falseCaseBlock);
  createBlockArguments(frame, ifOperation->getFalseBlockInvocation(),
                       falseCaseBlockRef);
  currentIrBlock->addOperation(ifOperation);
}

std::vector<std::string>
TraceToIRConversionPhase::IRConversionContext::createBlockArguments(
    BlockRef val) {
...
std::string
TraceToIRConversionPhase::IRConversionContext::createValueIdentifier(
    InputVariant val) {
  if (holds_alternative<ValueRef>(val)) {
    auto valueRef = std::get<ValueRef>(val);
    return std::to_string(valueRef.blockId) + "_" +
           std::to_string(valueRef.operationId);
  } else
    return "";
}

void TraceToIRConversionPhase::IRConversionContext::processAdd(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  ...
  auto addOperation =
      std::make_shared<NES::Nautilus::IR::Operations::AddOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, addOperation);
  currentBlock->addOperation(addOperation);
}

void TraceToIRConversionPhase::IRConversionContext::processSub(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto subOperation =
      std::make_shared<NES::Nautilus::IR::Operations::SubOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, subOperation);
  currentBlock->addOperation(subOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processMul(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto mulOperation =
      std::make_shared<NES::Nautilus::IR::Operations::MulOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, mulOperation);
  currentBlock->addOperation(mulOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processDiv(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto divOperation =
      std::make_shared<NES::Nautilus::IR::Operations::DivOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, divOperation);
  currentBlock->addOperation(divOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processMod(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto modOperation =
      std::make_shared<NES::Nautilus::IR::Operations::ModOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, modOperation);
  currentBlock->addOperation(modOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processNegate(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto input = frame.getValue(createValueIdentifier(operation.input[0]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto negateOperation =
      std::make_shared<NES::Nautilus::IR::Operations::NegateOperation>(
          resultIdentifier, input);
  frame.setValue(resultIdentifier, negateOperation);
  currentBlock->addOperation(negateOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processLessThan(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  ...
  auto compareOperation =
      std::make_shared<NES::Nautilus::IR::Operations::CompareOperation>(
          resultIdentifier, leftInput, rightInput,
...
void TraceToIRConversionPhase::IRConversionContext::processGreaterThan(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
...
void TraceToIRConversionPhase::IRConversionContext::processEquals(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
...
void TraceToIRConversionPhase::IRConversionContext::processAnd(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto andOperation =
      std::make_shared<NES::Nautilus::IR::Operations::AndOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, andOperation);
  currentBlock->addOperation(andOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processOr(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto orOperation =
      std::make_shared<NES::Nautilus::IR::Operations::OrOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, orOperation);
  currentBlock->addOperation(orOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processBitWiseAnd(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto orOperation =
      std::make_shared<NES::Nautilus::IR::Operations::BitWiseAndOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, orOperation);
  currentBlock->addOperation(orOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processBitWiseOr(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto orOperation =
      std::make_shared<NES::Nautilus::IR::Operations::BitWiseOrOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, orOperation);
  currentBlock->addOperation(orOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processBitWiseXor(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto orOperation =
      std::make_shared<NES::Nautilus::IR::Operations::BitWiseXorOperation>(
          resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, orOperation);
  currentBlock->addOperation(orOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processBitWiseLeftShift(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto orOperation = std::make_shared<
      NES::Nautilus::IR::Operations::BitWiseLeftShiftOperation>(
      resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, orOperation);
  currentBlock->addOperation(orOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processBitWiseRightShift(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto leftInput = frame.getValue(createValueIdentifier(operation.input[0]));
  auto rightInput = frame.getValue(createValueIdentifier(operation.input[1]));
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto orOperation = std::make_shared<
      NES::Nautilus::IR::Operations::BitWiseRightShiftOperation>(
      resultIdentifier, leftInput, rightInput);
  frame.setValue(resultIdentifier, orOperation);
  currentBlock->addOperation(orOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processLoad(
  ...
  // auto constOperation =
  // std::make_shared<NES::Nautilus::IR::Operations::LoadOperation>(createValueIdentifier(operation.result),
  ...
  auto loadOperation =
      std::make_shared<NES::Nautilus::IR::Operations::LoadOperation>(
          resultIdentifier, address, resultType);
  frame.setValue(resultIdentifier, loadOperation);
  currentBlock->addOperation(loadOperation);
}
void TraceToIRConversionPhase::IRConversionContext::processStore(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  ...
  auto storeOperation =
      std::make_shared<NES::Nautilus::IR::Operations::StoreOperation>(address,
                                                                      value);
  currentBlock->addOperation(storeOperation);
}

void TraceToIRConversionPhase::IRConversionContext::processCall(
    int32_t, ValueFrame& frame, NES::Nautilus::IR::BasicBlockPtr& currentBlock,
    TraceOperation& operation) {
  auto inputArguments =
      std::vector<NES::Nautilus::IR::Operations::OperationWPtr>{};
  ...
  auto resultType =
      std::holds_alternative<None>(operation.result)
          ? NES::Nautilus::IR::Types::StampFactory::createVoidStamp()
          : std::get<ValueRef>(operation.result).type;
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto proxyCallOperation = std::make_shared<
      NES::Nautilus::IR::Operations::ProxyCallOperation>(
      NES::Nautilus::IR::Operations::ProxyCallOperation::ProxyCallType::Other,
      uniqueFunctionSymbol.str(), functionCallTarget.functionPtr,
      resultIdentifier, inputArguments, resultType);
...
bool TraceToIRConversionPhase::IRConversionContext::isBlockInLoop(
    uint32_t parentBlockId, uint32_t currentBlockId) {
  if (currentBlockId == parentBlockId) {
    return true;
  }
  if (parentBlockId == 8) return false;
...
void TraceToIRConversionPhase::IRConversionContext::processConst(
    int32_t, TraceToIRConversionPhase::ValueFrame& frame,
    NES::Nautilus::IR::BasicBlockPtr& currentBlock, TraceOperation& operation) {
  ...
  if (auto* i8 = cast_if<Int8>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, i8->getValue(), i8->getType());
  } else if (auto* i16 = cast_if<Int16>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, i16->getValue(), i16->getType());
  } else if (auto* i32 = cast_if<Int32>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, i32->getValue(), i32->getType());
  } else if (auto* i64 = cast_if<Int64>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, i64->getValue(), i64->getType());
  } else if (auto* ui8 = cast_if<UInt8>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, ui8->getValue(), ui8->getType());
  } else if (auto* ui16 = cast_if<UInt16>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, ui16->getValue(), ui16->getType());
  } else if (auto* ui32 = cast_if<UInt32>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, ui32->getValue(), ui32->getType());
  } else if (auto* ui64 = cast_if<UInt64>(valueRef.value.get())) {
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, ui64->getValue(), ui64->getType());
  } else if (auto* float32 = cast_if<Float>(valueRef.value.get())) {
    constOperation =
        std::make_shared<NES::Nautilus::IR::Operations::ConstFloatOperation>(
            resultIdentifier, float32->getValue(), float32->getType());
  } else if (auto* float64 = cast_if<Double>(valueRef.value.get())) {
    constOperation =
        std::make_shared<NES::Nautilus::IR::Operations::ConstFloatOperation>(
            resultIdentifier, float64->getValue(), float64->getType());
  } else if (auto* boolean = cast_if<Boolean>(valueRef.value.get())) {
    constOperation =
        std::make_shared<NES::Nautilus::IR::Operations::ConstBooleanOperation>(
            resultIdentifier, boolean->getValue());
  } else if (Identifier::isIdentifier(*valueRef.value)) {
    auto identifier = std::dynamic_pointer_cast<Identifier>(valueRef.value);
    constOperation = std::make_shared<IR::Operations::ConstIntOperation>(
        resultIdentifier, identifier->getUnderlyingRawValue(),
        identifier->getType());
  } else {
...
void TraceToIRConversionPhase::IRConversionContext::processCast(
    int32_t, TraceToIRConversionPhase::ValueFrame& frame,
    NES::Nautilus::IR::BasicBlockPtr& currentBlock, TraceOperation& operation) {
  auto resultIdentifier = createValueIdentifier(operation.result);
  auto input = frame.getValue(createValueIdentifier(operation.input[0]));
  auto resultType = std::get<ValueRef>(operation.result).type;
  auto castOperation =
      std::make_shared<NES::Nautilus::IR::Operations::CastOperation>(
          resultIdentifier, input, resultType);
