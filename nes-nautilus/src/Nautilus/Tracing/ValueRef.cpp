...
ValueRef::ValueRef(uint32_t blockId, uint32_t operationId,
                   NES::Nautilus::IR::Types::StampPtr type)
    : blockId(blockId), operationId(operationId), type(std::move(type)){};

ValueRef::ValueRef(const ValueRef &other)
    : blockId(other.blockId), operationId(other.operationId),
      type(other.type){};
ValueRef::ValueRef(const ValueRef &&other)
    : blockId(other.blockId), operationId(other.operationId),
...
ValueRef createNextRef(const NES::Nautilus::IR::Types::StampPtr &stamp) {
  ...
  return ValueRef(0 /* blockId */, 0 /*operationId */,
                  NES::Nautilus::IR::Types::StampFactory::createVoidStamp());
}
