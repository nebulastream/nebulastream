 ...
 * The trace context can always be thread local, as tracable code can never span
 * multiple threads.
 */
static thread_local TraceContext* traceContext;

TraceContext* TraceContext::get() { return traceContext; }
TraceContext* TraceContext::getIfActive() {
  return shouldTrace() ? get() : nullptr;
};
...
TraceContext::TraceContext(TagRecorder& tagRecorder)
    : tagRecorder(tagRecorder),
      executionTrace(std::make_unique<ExecutionTrace>()),
...
void TraceContext::traceBinaryOperation(const OpCode& op,
  ...
  trace(op, [&]() {
    return Nautilus::Tracing::TraceOperation(op, resultRef,
                                             {leftRef, rightRef});
  });
}

void TraceContext::traceUnaryOperation(const OpCode& op, const ValueRef& input,
...
void TraceContext::traceConstOperation(const ValueRef& constRef,
                                       const AnyPtr& constantValue) {
  trace(OpCode::CONST, [&]() {
    return Nautilus::Tracing::TraceOperation(OpCode::CONST, constRef,
                                             {constantValue});
  });
}

void TraceContext::traceFunctionCall(
...
void TraceContext::traceFunctionCall(
    ...
    const std::vector<Nautilus::Tracing::InputVariant>& arguments) {
  trace(OpCode::CALL, [&]() {
    return Nautilus::Tracing::TraceOperation(OpCode::CALL, resultRef,
                                             arguments);
  });
}

void TraceContext::traceStore(const ValueRef& memref,
...
void TraceContext::traceAssignmentOperation(const ValueRef& targetRef,
                                            const ValueRef& sourceRef) {
  // check if we repeat a known trace or if this is a new operation.
  // we are in a know operation if the operation at the current
  ...
  if (!isExpectedOperation(OpCode::ASSIGN)) {
    ...
    auto operation = Nautilus::Tracing::TraceOperation(
        Nautilus::Tracing::OpCode::ASSIGN, targetRef, {sourceRef});
...
bool TraceContext::traceCMP(const ValueRef& valueRef) {
  ...
  if (!isExpectedOperation(OpCode::CMP)) {
    ...
    executionTrace->getBlock(trueBlock).predecessors.emplace_back(
        executionTrace->getCurrentBlockIndex());
    executionTrace->getBlock(falseBlock)
        .predecessors.emplace_back(executionTrace->getCurrentBlockIndex());
    auto operation = TraceOperation(
        OpCode::CMP, valueRef, {BlockRef(trueBlock), BlockRef(falseBlock)});
    executionTrace->addOperation(operation);
  } else {
    // we repeat the operation
    auto operation =
        executionTrace->getCurrentBlock().operations[currentOperationCounter];
...
ValueRef TraceContext::createNextRef(
    const NES::Nautilus::IR::Types::StampPtr& type) {
  // If the next operation already exists, we have to create the same value ref.
  // Currently, we assume that it is correctly set.
  // TODO this should be handled better at a fundamental level, which ensures
  // that value references are always correct.
...
bool TraceContext::isExpectedOperation(const OpCode& opCode) {
  ...
  while (currentOperation->op == OpCode::JMP) {
    executionTrace->setCurrentBlock(
        std::get<BlockRef>(currentOperation->input[0]).block);
    currentOperationCounter = 0;
    currentOperation =
        &executionTrace->getCurrentBlock().operations[currentOperationCounter];
  }
...
bool TraceContext::isKnownOperation(const Tag* tag) {
  if (auto ref = checkTag(tag)) {
    ...
    if (ref->blockId != this->executionTrace->getCurrentBlockIndex()) {
      auto& mergeBlock = executionTrace->processControlFlowMerge(
          ref->blockId, ref->operationId);
...
std::shared_ptr<ExecutionTrace> TraceContext::apply(
    const std::function<NES::Nautilus::Tracing::ValueRef()>& function) {
