...
void TraceContext::traceBinaryOperation(const OpCode& op, const ValueRef& leftRef, const ValueRef& rightRef, const ValueRef& resultRef)
{
    trace(op, [&]() { return Nautilus::Tracing::TraceOperation(op, resultRef, {leftRef, rightRef}); });
}

void TraceContext::traceUnaryOperation(const OpCode& op, const ValueRef& input, const ValueRef& result)
{
    trace(op, [&]() { return Nautilus::Tracing::TraceOperation(op, result, {input}); });
}
...
{
    trace(OpCode::CONST, [&]() { return Nautilus::Tracing::TraceOperation(OpCode::CONST, constRef, {constantValue}); });
}

void TraceContext::traceFunctionCall(const std::vector<Nautilus::Tracing::InputVariant>& arguments)
{
    trace(OpCode::CALL, [&]() { return Nautilus::Tracing::TraceOperation(OpCode::CALL, arguments); });
}

void TraceContext::traceFunctionCall(const ValueRef& resultRef, const std::vector<Nautilus::Tracing::InputVariant>& arguments)
{
    trace(OpCode::CALL, [&]() { return Nautilus::Tracing::TraceOperation(OpCode::CALL, resultRef, arguments); });
}

void TraceContext::traceStore(const ValueRef& memref, const ValueRef& valueRef)
{
    trace(OpCode::STORE, [&]() { return Nautilus::Tracing::TraceOperation(OpCode::STORE, {memref, valueRef}); });
}
