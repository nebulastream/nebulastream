...
std::shared_ptr<LazyValueRepresentation> provideLazyValueRepresentation(
    ...
    const DataType& type, const nautilus::val<bool>& isNull)
{
    auto args = LazyValueRepresentationRegistryArguments{.valueAddress = valueAddress, .size = size, .type = type, .isNull = isNull};
    ...
    return std::make_shared<LazyValueRepresentation>(LazyValueRepresentation(valueAddress, size, type, isNull));
}
}
