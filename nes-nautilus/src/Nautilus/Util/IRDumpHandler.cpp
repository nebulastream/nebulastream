...
IR::BasicBlockPtr NESIRDumpHandler::getNextLowerOrEqualLevelBasicBlock(
    BasicBlockPtr thenBlock, int ifParentBlockLevel) {
  ...
  if (terminatorOp->getOperationType() ==
    ...
    auto branchOp =
        std::static_pointer_cast<Operations::BranchOperation>(terminatorOp);
    if (branchOp->getNextBlockInvocation().getBlock()->getScopeLevel() <=
        (uint32_t)ifParentBlockLevel) {
      return branchOp->getNextBlockInvocation().getBlock();
    } else {
      return getNextLowerOrEqualLevelBasicBlock(
          branchOp->getNextBlockInvocation().getBlock(), ifParentBlockLevel);
    }
  } else if (terminatorOp->getOperationType() ==
             Operations::Operation::OperationType::LoopOp) {
    auto loopOp =
        std::static_pointer_cast<Operations::LoopOperation>(terminatorOp);
    auto loopIfOp = std::static_pointer_cast<Operations::IfOperation>(
        loopOp->getLoopHeadBlock().getBlock()->getOperations().back());
    if (loopIfOp->getTrueBlockInvocation().getBlock()->getScopeLevel() <=
        (uint32_t)ifParentBlockLevel) {
      return loopIfOp->getTrueBlockInvocation().getBlock();
    } else {
      return getNextLowerOrEqualLevelBasicBlock(
          loopIfOp->getFalseBlockInvocation().getBlock(), ifParentBlockLevel);
    }
  } else if (terminatorOp->getOperationType() ==
    ...
    if (ifOp->getFalseBlockInvocation().getBlock() != nullptr) {
      return getNextLowerOrEqualLevelBasicBlock(
          ifOp->getFalseBlockInvocation().getBlock(), ifParentBlockLevel);
    } else {
      return getNextLowerOrEqualLevelBasicBlock(
          ifOp->getTrueBlockInvocation().getBlock(), ifParentBlockLevel);
    }
...
void NESIRDumpHandler::dumpHelper(OperationPtr const& terminatorOp, int32_t) {
  switch (terminatorOp->getOperationType()) {
    ...
    case Operations::Operation::OperationType::IfOp: {
      auto ifOp =
          std::static_pointer_cast<Operations::IfOperation>(terminatorOp);
      BasicBlockPtr lastTerminatorOp = getNextLowerOrEqualLevelBasicBlock(
          ifOp->getTrueBlockInvocation().getBlock(),
          ifOp->getTrueBlockInvocation().getBlock()->getScopeLevel() -
              1);  // todo can lead to error #3234
...
void NESIRDumpHandler::dumpHelper(BasicBlockPtr const& basicBlock) {
  if (!visitedBlocks.contains(basicBlock->getIdentifier())) {
    ...
    for (auto operation : basicBlock->getOperations()) {
      out << std::string(4, ' ') << operation->toString() << " :"
          << operation->getStamp()->toString() << std::endl;
    }
...
void NESIRDumpHandler::dump(
    const std::shared_ptr<Operations::FunctionOperation> funcOp) {
  out << funcOp->toString() << " {";
