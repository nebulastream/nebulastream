...
void NESIRDumpHandler::dumpHelper(OperationPtr const &terminatorOp,
                                  int32_t scopeLevel) {
...
void NESIRDumpHandler::dumpHelper(BasicBlockPtr const &basicBlock) {
  if (!visitedBlocks.contains(basicBlock->getIdentifier())) {
    ...
    // out << '\n' << std::string(basicBlock->getScopeLevel() * 4, ' ') <<
    ...
    for (auto operation : basicBlock->getOperations()) {
      ...
      //  out << std::string(indent * 4, ' ') << operation->toString() <<
      //  std::endl;
    }
