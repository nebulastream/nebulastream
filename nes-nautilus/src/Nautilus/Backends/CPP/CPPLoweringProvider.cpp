...
CPPLoweringProvider::LoweringContext::LoweringContext(
...
std::string CPPLoweringProvider::LoweringContext::getType(
...
std::string CPPLoweringProvider::LoweringContext::process(
...
void CPPLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::CompareOperation>& cmpOp,
  ...
  // type.
  if (cmpOp->isEquals() && cmpOp->getLeftInput()->getStamp()->isAddress() &&
      cmpOp->getRightInput()->getStamp()->isInteger()) {
  ...
  if (cmpOp->getComparator() ==
    ...
    comperator = " > ";
             ...
             IR::Operations::CompareOperation::Comparator::GE) {
    comperator = " >= ";
  } else if (cmpOp->getComparator() ==
             IR::Operations::CompareOperation::Comparator::LE) {
    comperator = " <= ";
  } else {
    NES_NOT_IMPLEMENTED();
  }

  blocks[blockIndex] << resultVar << " = " << leftInput << comperator
                     << rightInput << ";\n";
}

void CPPLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::LoadOperation>& loadOp,
    short blockIndex, RegisterFrame& frame) {
  ...
  blocks[blockIndex] << resultVar << " = *reinterpret_cast<" << type << "*>("
                     << address << ");\n";
}

void CPPLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::StoreOperation>& storeOp,
    short blockIndex, RegisterFrame& frame) {
  ...
  blocks[blockIndex] << "*reinterpret_cast<" << type << "*>(" << address
                     << ") = " << value << ";\n";
}

void CPPLoweringProvider::LoweringContext::process(
    IR::Operations::BasicBlockInvocation& bi, short blockIndex,
  ...
  for (uint64_t i = 0; i < blockInputArguments.size(); i++) {
    ...
    if (!parentFrame.contains(blockTargetArgument)) {
      ...
      blockArguments << getType(blockTargetArguments[i]->getStamp()) << " "
                     << var << ";\n";
    }
...
void CPPLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::IfOperation>& ifOpt, short blockIndex,
    RegisterFrame& frame) {
...
void CPPLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::Operation>& opt, short blockIndex,
    RegisterFrame& frame) {
  switch (opt->getOperationType()) {
    case IR::Operations::Operation::OperationType::ConstBooleanOp: {
      processConst<IR::Operations::ConstBooleanOperation>(opt, blockIndex,
                                                          frame);
      return;
    }
    ...
    case IR::Operations::Operation::OperationType::BitWiseAnd: {
      processBinary<IR::Operations::BitWiseAndOperation>(opt, "&", blockIndex,
                                                         frame);
      return;
    }
    case IR::Operations::Operation::OperationType::BitWiseOr: {
      processBinary<IR::Operations::BitWiseOrOperation>(opt, "|", blockIndex,
                                                        frame);
      return;
    }
    case IR::Operations::Operation::OperationType::BitWiseXor: {
      processBinary<IR::Operations::BitWiseXorOperation>(opt, "^", blockIndex,
                                                         frame);
      return;
    }
    case IR::Operations::Operation::OperationType::BitWiseLeftShift: {
      processBinary<IR::Operations::BitWiseLeftShiftOperation>(
          opt, "<<", blockIndex, frame);
      return;
    }
    case IR::Operations::Operation::OperationType::BitWiseRightShift: {
      processBinary<IR::Operations::BitWiseRightShiftOperation>(
          opt, ">>", blockIndex, frame);
      return;
    }
    default: {
...
std::string CPPLoweringProvider::LoweringContext::getVariable(
    const std::string& id) {
  return "var_" + id;
}
