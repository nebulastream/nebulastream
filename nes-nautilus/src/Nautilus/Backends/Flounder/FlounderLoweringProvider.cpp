...
std::unique_ptr<flounder::Executable> FlounderLoweringProvider::lower(
    std::shared_ptr<IR::IRGraph> ir, const NES::DumpHelper& dumpHelper) {
  flounder::Compiler compiler =
      flounder::Compiler{/*do not optimize*/ false,
                         /*collect the asm code to print later*/ true};
  auto ctx = LoweringContext(std::move(ir));
  return ctx.process(compiler, dumpHelper);
}

FlounderLoweringProvider::LoweringContext::LoweringContext(
    std::shared_ptr<IR::IRGraph> ir)
    : ir(std::move(ir)) {}

std::unique_ptr<flounder::Executable>
FlounderLoweringProvider::LoweringContext::process(
    flounder::Compiler& compiler, const NES::DumpHelper& dumpHelper) {
  ...
  const auto isCompilationSuccessful =
      compiler.compile(program, *executable.get());
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::FunctionOperation>&
        functionOperation) {
  ...
  for (auto i = 0ull; i < functionBasicBlock->getArguments().size(); i++) {
    ...
    auto arg =
        createVreg(argument->getIdentifier(), argument->getStamp(), rootFrame);

    const auto intStamp =
        std::static_pointer_cast<IR::Types::IntegerStamp>(argument->getStamp());
    if (intStamp->getBitWidth() != IR::Types::IntegerStamp::BitWidth::I64) {
      auto tmpArg = program.vreg("temp_" + argument->getIdentifier());
      program << program.request_vreg64(tmpArg)
              << program.get_argument(i, tmpArg) << program.mov(arg, tmpArg);
    } else {
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::BasicBlock>& block, FlounderFrame& frame) {
...
void FlounderLoweringProvider::LoweringContext::processInline(
    const std::shared_ptr<IR::BasicBlock>& block, FlounderFrame& frame) {
  for (const auto& operation : block->getOperations()) {
    this->process(operation, frame);
  }
...
flounder::VregInstruction
FlounderLoweringProvider::LoweringContext::requestVreg(
    flounder::Register& reg, const IR::Types::StampPtr& stamp) {
  if (stamp->isInteger()) {
    ...
    switch (intStamp->getBitWidth()) {
      case IR::Types::IntegerStamp::BitWidth::I8:
        registerWidth = flounder::r8;
        break;
      case IR::Types::IntegerStamp::BitWidth::I16:
        registerWidth = flounder::r16;
        break;
      case IR::Types::IntegerStamp::BitWidth::I32:
        registerWidth = flounder::r32;
        break;
      case IR::Types::IntegerStamp::BitWidth::I64:
        registerWidth = flounder::r64;
        break;
    }
    flounder::RegisterSignType signType =
        intStamp->isSigned() ? flounder::Signed : flounder::Unsigned;
    return program.request_vreg(reg, registerWidth, signType);
  } else if (stamp->isAddress()) {
    return program.request_vreg64(reg);
  }
  NES_THROW_RUNTIME_ERROR("Stamp for vreg not supported by flounder "
                          << stamp->toString());
}

flounder::Register FlounderLoweringProvider::LoweringContext::createVreg(
    IR::Operations::OperationIdentifier id, IR::Types::StampPtr stamp,
    FlounderFrame& frame) {
...
FlounderLoweringProvider::LoweringContext::processBlockInvocation(
    IR::Operations::BasicBlockInvocation& bi, FlounderFrame& frame) {
  FlounderFrame blockFrame;
...
FlounderLoweringProvider::FlounderFrame
FlounderLoweringProvider::LoweringContext::processInlineBlockInvocation(
    IR::Operations::BasicBlockInvocation& bi, FlounderFrame& frame) {
  FlounderFrame blockFrame;
  auto blockInputArguments = bi.getArguments();
  auto blockTargetArguments = bi.getBlock()->getArguments();
  ...
  for (uint64_t i = 0; i < blockInputArguments.size(); i++) {
    ...
    blockFrame.setValue(
        blockTargetArguments[i]->getIdentifier(),
        frame.getValue(blockInputArguments[i]->getIdentifier()));
  }
  return blockFrame;
}
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::CastOperation>& castOp,
    NES::Nautilus::Backends::Flounder::FlounderLoweringProvider::FlounderFrame&
        frame) {
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::AddOperation>& addOpt,
    FlounderFrame& frame) {
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::MulOperation>& mulOp,
    FlounderFrame& frame) {
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::SubOperation>& subOp,
    FlounderFrame& frame) {
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::LoadOperation>& opt,
    FlounderFrame& frame) {
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::StoreOperation>& opt,
    FlounderFrame& frame) {
  auto address = frame.getValue(opt->getAddress()->getIdentifier());
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::BranchOperation>& branchOp,
    FlounderFrame& frame) {
  auto targetFrame =
      processInlineBlockInvocation(branchOp->getNextBlockInvocation(), frame);
...
void FlounderLoweringProvider::LoweringContext::processCmp(
    ...
    NES::Nautilus::Backends::Flounder::FlounderLoweringProvider::FlounderFrame&
        frame,
    flounder::Label& trueCase, flounder::Label& falseCase) {
  if (opt->getOperationType() ==
      IR::Operations::Operation::OperationType::CompareOp) {
  ...
  } else if (opt->getOperationType() ==
             IR::Operations::Operation::OperationType::AndOp) {
    auto left = std::static_pointer_cast<IR::Operations::AndOperation>(opt);
    processAnd(left, frame, trueCase, falseCase);
  } else if (opt->getOperationType() ==
             IR::Operations::Operation::OperationType::OrOp) {
    auto left = std::static_pointer_cast<IR::Operations::OrOperation>(opt);
    processOr(left, frame, trueCase, falseCase);
  } else {
    NES_THROW_RUNTIME_ERROR("Left is not a compare operation but a "
                            << opt->toString());
  }
}

void FlounderLoweringProvider::LoweringContext::processAnd(
    const std::shared_ptr<IR::Operations::AndOperation>& andOpt,
    FlounderFrame& frame, flounder::Label& trueCase,
    flounder::Label& falseCase) {
  auto andSecondCaseLabel =
      program.label("And_tmp_label" + andOpt->getIdentifier());
...
void FlounderLoweringProvider::LoweringContext::processOr(
    ...
    NES::Nautilus::Backends::Flounder::FlounderLoweringProvider::FlounderFrame&
        frame,
    flounder::Label& trueCase, flounder::Label& falseCase) {
  auto orSecondCaseLabel =
      program.label("Or_tmp_label" + orOpt->getIdentifier());
...
void FlounderLoweringProvider::LoweringContext::processCmp(
    const std::shared_ptr<IR::Operations::CompareOperation>& compOpt,
  ...
  switch (compOpt->getComparator()) {
    case IR::Operations::CompareOperation::Comparator::EQ:
      program << program.jne(falseCase);
      break;
    case IR::Operations::CompareOperation::Comparator::LT:
      program << program.jge(falseCase);
      break;
    case IR::Operations::CompareOperation::Comparator::GE:
      program << program.jl(falseCase);
      break;
    case IR::Operations::CompareOperation::Comparator::LE:
      program << program.jg(falseCase);
      break;
    case IR::Operations::CompareOperation::Comparator::GT:
      program << program.jle(falseCase);
      break;
    case IR::Operations::CompareOperation::Comparator::NE:
      program << program.je(falseCase);
      break;
    default:
      NES_THROW_RUNTIME_ERROR("No handler for comp");
  }
}

void FlounderLoweringProvider::LoweringContext::process(
    std::shared_ptr<IR::Operations::IfOperation> ifOpt, FlounderFrame& frame) {
  auto trueLabel = program.label(
      "Block_" + ifOpt->getTrueBlockInvocation().getBlock()->getIdentifier());
  auto falseLabel = program.label(
      "Block_" + ifOpt->getFalseBlockInvocation().getBlock()->getIdentifier());
  // clear all non args
  auto falseBlockFrame =
...
void FlounderLoweringProvider::LoweringContext::process(
  ...
  if (!opt->getStamp()->isVoid()) {
    auto resultRegister =
        createVreg(opt->getIdentifier(), opt->getStamp(), frame);
    auto call =
        program.fcall((std::uintptr_t)opt->getFunctionPtr(), resultRegister);
...
void FlounderLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::LoopOperation>& opt,
    FlounderFrame& frame) {
...
void FlounderLoweringProvider::LoweringContext::process(
  ...
  switch (opt->getOperationType()) {
    case IR::Operations::Operation::OperationType::ConstBooleanOp: {
      auto constInt =
          std::static_pointer_cast<IR::Operations::ConstBooleanOperation>(opt);
      auto flounderConst = program.constant64(constInt->getValue() ? 1 : 0);
      return;
    }
    case IR::Operations::Operation::OperationType::ConstIntOp: {
      auto constInt =
          std::static_pointer_cast<IR::Operations::ConstIntOperation>(opt);
    ...
    case IR::Operations::Operation::OperationType::CompareOp: {
      auto compOpt =
          std::static_pointer_cast<IR::Operations::CompareOperation>(opt);
    ...
    case IR::Operations::Operation::OperationType::ReturnOp: {
      auto returnOpt =
          std::static_pointer_cast<IR::Operations::ReturnOperation>(opt);
      if (returnOpt->hasReturnValue()) {
        auto returnFOp =
            frame.getValue(returnOpt->getReturnValue()->getIdentifier());
    ...
    case IR::Operations::Operation::OperationType::BranchOp: {
      auto branchOp =
          std::static_pointer_cast<IR::Operations::BranchOperation>(opt);
      process(branchOp, frame);
      return;
    }
    case IR::Operations::Operation::OperationType::LoopOp: {
      auto loopOp =
          std::static_pointer_cast<IR::Operations::LoopOperation>(opt);
    ...
    case IR::Operations::Operation::OperationType::StoreOp: {
      auto store =
          std::static_pointer_cast<IR::Operations::StoreOperation>(opt);
      process(store, frame);
      return;
    }
    case IR::Operations::Operation::OperationType::ProxyCallOp: {
      auto call =
          std::static_pointer_cast<IR::Operations::ProxyCallOperation>(opt);
      process(call, frame);
      return;
    }
    ...
    default: {
      NES_THROW_RUNTIME_ERROR("Operation " << opt->toString()
                                           << " not handled");
    }
  }
}
