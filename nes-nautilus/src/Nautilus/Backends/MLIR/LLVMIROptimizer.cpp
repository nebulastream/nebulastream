...
LLVMIROptimizer::getLLVMOptimizerPipeline(const CompilationOptions& options,
  ...
  return [options, dumpHelper](llvm::Module* llvmIRModule) {
    ...
    NES_ASSERT2_FMT(tmBuilderOrError,
    ...
    llvmIRModule->getFunction("execute")->addAttributeAtIndex(
        ~0, llvm::Attribute::get(llvmIRModule->getContext(), "target-cpu",
                                 targetMachinePtr->getTargetCPU()));
    llvmIRModule->getFunction("execute")->addAttributeAtIndex(
        ~0, llvm::Attribute::get(llvmIRModule->getContext(), "target-features",
                                 targetMachinePtr->getTargetFeatureString()));
    llvmIRModule->getFunction("execute")->addAttributeAtIndex(
        ~0, llvm::Attribute::get(llvmIRModule->getContext(), "tune-cpu",
                                 targetMachinePtr->getTargetCPU()));
    llvm::SMDiagnostic Err;

    // Load LLVM IR module from proxy inlining input path (We assert that it
    // exists in CompilationOptions).
    if (options.isProxyInlining()) {
      auto proxyFunctionsIR = llvm::parseIRFile(
          options.getProxyInliningInputPath(), Err, llvmIRModule->getContext());
      // Link the module with our generated LLVM IR module and optimize the
      // linked LLVM IR module (inlining happens during optimization).
      llvm::Linker::linkModules(*llvmIRModule, std::move(proxyFunctionsIR),
                                llvm::Linker::Flags::OverrideFromSrc);
    }
    auto optPipeline = mlir::makeOptimizingTransformer(
        options.getOptimizationLevel(), SIZE_LEVEL, targetMachinePtr);
