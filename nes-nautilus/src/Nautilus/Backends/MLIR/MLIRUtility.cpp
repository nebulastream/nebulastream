...
void MLIRUtility::writeMLIRModuleToFile(
...
int MLIRUtility::loadAndExecuteModuleFromString(
    const std::string& mlirString, const std::string& moduleString) {
  ...
  auto optPipeline =
      MLIR::LLVMIROptimizer::getLLVMOptimizerPipeline(/*inlining*/ false);

  // JIT compile LLVM IR module and return engine that provides access compiled
  // execute function.
  auto engine =
      MLIR::JITCompiler::jitCompileModule(mlirModule, optPipeline, {}, {});
...
std::unique_ptr<mlir::ExecutionEngine> MLIRUtility::compileNESIRToMachineCode(
  ...
  return MLIR::JITCompiler::jitCompileModule(
      module, optPipeline, loweringProvider->getJitProxyFunctionSymbols(),
