...
std::unique_ptr<mlir::ExecutionEngine> JITCompiler::jitCompileModule(
    ...
    const CompilationOptions &compilerOptions, const DumpHelper &dumpHelper) {
  ...
  // TODO in issue #3710 we aim to add a proxy function catalog that contains
  // the information on all proxy functions. right now, we have to statically
  // list all proxy functions here, and in 'ExtractFunctionsFromLLVMIR.cpp'.
  ...
  const auto runtimeSymbolMap = [&](llvm::orc::MangleAndInterner interner) {
    ...
    for (int i = 0; i < (int)jitProxyFunctionSymbols.size(); ++i) {
      if (!(compilerOptions.isProxyInlining() &&
            ProxyInliningFunctions.contains(jitProxyFunctionSymbols.at(i)))) {
        symbolMap[interner(jitProxyFunctionSymbols.at(i))] =
            llvm::JITEvaluatedSymbol(jitProxyFunctionTargetAddresses.at(i),
