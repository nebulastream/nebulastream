...
mlir::Type MLIRLoweringProvider::getMLIRType(IR::Types::StampPtr type) {
  if (type->isVoid()) {
    ...
    switch (value->getBitWidth()) {
      case IR::Types::FloatStamp::BitWidth::F32:
        return mlir::Float32Type::get(context);
      case IR::Types::FloatStamp::BitWidth::F64:
...
mlir::Value MLIRLoweringProvider::getConstInt(const std::string& location,
                                              IR::Types::StampPtr stamp,
                                              int64_t value) {
  auto type = getMLIRType(stamp);
  return builder->create<mlir::arith::ConstantOp>(
      getNameLoc(location), type, builder->getIntegerAttr(type, value));
}

mlir::Value MLIRLoweringProvider::getConstBool(const std::string& location,
                                               bool value) {
...
mlir::Location MLIRLoweringProvider::getNameLoc(const std::string& name) {
  auto baseLocation =
      mlir::FileLineColLoc::get(builder->getStringAttr("Query_1"), 0, 0);
  return mlir::NameLoc::get(builder->getStringAttr(name), baseLocation);
}

mlir::arith::CmpIPredicate convertToIntMLIRComparison(
    IR::Operations::CompareOperation::Comparator comparisonType,
    IR::Types::StampPtr& stamp) {
  auto integerStamp = std::dynamic_pointer_cast<IR::Types::IntegerStamp>(stamp);
  if (integerStamp->isSigned()) {
    switch (comparisonType) {
      case (IR::Operations::CompareOperation::Comparator::EQ):
        return mlir::arith::CmpIPredicate::eq;
      case (IR::Operations::CompareOperation::Comparator::NE):
        return mlir::arith::CmpIPredicate::ne;
      case (IR::Operations::CompareOperation::Comparator::LT):
        return mlir::arith::CmpIPredicate::slt;
      case (IR::Operations::CompareOperation::Comparator::LE):
        return mlir::arith::CmpIPredicate::sle;
      case (IR::Operations::CompareOperation::Comparator::GT):
        return mlir::arith::CmpIPredicate::sgt;
      case (IR::Operations::CompareOperation::Comparator::GE):
...
mlir::arith::CmpFPredicate convertToFloatMLIRComparison(
    IR::Operations::CompareOperation::Comparator comparisonType) {
  switch (comparisonType) {
    // the U in U(LT/LE/..) stands for unordered, not unsigned! Float
    // comparisons are always signed.
    case (IR::Operations::CompareOperation::Comparator::LT):
...
mlir::FlatSymbolRefAttr MLIRLoweringProvider::insertExternalFunction(
  ...
  mlir::LLVM::LLVMFunctionType llvmFnType =
      mlir::LLVM::LLVMFunctionType::get(resultType, argTypes, varArgs);
  ...
  builder->create<mlir::LLVM::LLVMFuncOp>(theModule.getLoc(), name, llvmFnType,
  ...
  jitProxyFunctionTargetAddresses.push_back(
      llvm::pointerToJITTargetAddress(functionPtr));
...
//==---------------------------------==//
MLIRLoweringProvider::MLIRLoweringProvider(mlir::MLIRContext& context)
    : context(&context) {
...
mlir::OwningOpRef<mlir::ModuleOp> MLIRLoweringProvider::generateModuleFromIR(
...
void MLIRLoweringProvider::generateMLIR(IR::BasicBlockPtr basicBlock,
                                        ValueFrame& frame) {
...
void MLIRLoweringProvider::generateMLIR(
    const IR::Operations::OperationPtr& operation, ValueFrame& frame) {
  switch (operation->getOperationType()) {
    case IR::Operations::Operation::OperationType::FunctionOp:
      generateMLIR(std::static_pointer_cast<IR::Operations::FunctionOperation>(
                       operation),
                   frame);
      break;
    case IR::Operations::Operation::OperationType::LoopOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::LoopOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::ConstIntOp:
      generateMLIR(std::static_pointer_cast<IR::Operations::ConstIntOperation>(
                       operation),
                   frame);
      break;
    case IR::Operations::Operation::OperationType::ConstFloatOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::ConstFloatOperation>(
              operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::AddOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::AddOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::SubOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::SubOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::MulOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::MulOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::DivOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::DivOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::ModOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::ModOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::LoadOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::LoadOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::AddressOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::AddressOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::StoreOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::StoreOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::BranchOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::BranchOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::IfOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::IfOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::CompareOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::CompareOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::ProxyCallOp:
      generateMLIR(std::static_pointer_cast<IR::Operations::ProxyCallOperation>(
                       operation),
                   frame);
      break;
    case IR::Operations::Operation::OperationType::ReturnOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::ReturnOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::OrOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::OrOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::AndOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::AndOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::BitWiseAnd:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::BitWiseAndOperation>(
              operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::BitWiseOr:
      generateMLIR(std::static_pointer_cast<IR::Operations::BitWiseOrOperation>(
                       operation),
                   frame);
      break;
    case IR::Operations::Operation::OperationType::BitWiseXor:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::BitWiseXorOperation>(
              operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::BitWiseLeftShift:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::BitWiseLeftShiftOperation>(
              operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::BitWiseRightShift:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::BitWiseRightShiftOperation>(
              operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::NegateOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::NegateOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::CastOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::CastOperation>(operation),
          frame);
      break;
    case IR::Operations::Operation::OperationType::ConstBooleanOp:
      generateMLIR(
          std::static_pointer_cast<IR::Operations::ConstBooleanOperation>(
              operation),
          frame);
      break;
    default:
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::NegateOperation> negateOperation,
    ValueFrame& frame) {
  auto input = frame.getValue(negateOperation->getInput()->getIdentifier());
  auto negate = builder->create<mlir::arith::CmpIOp>(
      getNameLoc("comparison"), mlir::arith::CmpIPredicate::eq, input,
      getConstBool("bool", false));
  frame.setValue(negateOperation->getIdentifier(), negate);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::OrOperation> orOperation,
    ValueFrame& frame) {
  auto leftInput = frame.getValue(orOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(orOperation->getRightInput()->getIdentifier());
  auto mlirOrOp = builder->create<mlir::LLVM::OrOp>(getNameLoc("binOpResult"),
                                                    leftInput, rightInput);
  frame.setValue(orOperation->getIdentifier(), mlirOrOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::AndOperation> andOperation,
    ValueFrame& frame) {
  auto leftInput =
      frame.getValue(andOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(andOperation->getRightInput()->getIdentifier());
  auto mlirAndOp = builder->create<mlir::LLVM::AndOp>(getNameLoc("binOpResult"),
                                                      leftInput, rightInput);
  frame.setValue(andOperation->getIdentifier(), mlirAndOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::BitWiseAndOperation> andOperation,
    ValueFrame& frame) {
  auto leftInput =
      frame.getValue(andOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(andOperation->getRightInput()->getIdentifier());
  auto mlirBitwiseAndOp = builder->create<mlir::arith::AndIOp>(
      getNameLoc("binOpResult"), leftInput, rightInput);
  frame.setValue(andOperation->getIdentifier(), mlirBitwiseAndOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::BitWiseOrOperation> orOperation,
    ValueFrame& frame) {
  auto leftInput = frame.getValue(orOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(orOperation->getRightInput()->getIdentifier());
  auto mlirBitwiseOrOp = builder->create<mlir::arith::OrIOp>(
      getNameLoc("binOpResult"), leftInput, rightInput);
  frame.setValue(orOperation->getIdentifier(), mlirBitwiseOrOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::BitWiseXorOperation> xorOperation,
    ValueFrame& frame) {
  auto leftInput =
      frame.getValue(xorOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(xorOperation->getRightInput()->getIdentifier());
  auto mlirBitwiseXorOp = builder->create<mlir::arith::XOrIOp>(
      getNameLoc("binOpResult"), leftInput, rightInput);
  frame.setValue(xorOperation->getIdentifier(), mlirBitwiseXorOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::BitWiseLeftShiftOperation>
        leftShiftOperation,
    ValueFrame& frame) {
  auto leftInput =
      frame.getValue(leftShiftOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(leftShiftOperation->getRightInput()->getIdentifier());
  auto mlirBitwiseLeftShiftOp = builder->create<mlir::arith::ShLIOp>(
      getNameLoc("binOpResult"), leftInput, rightInput);
  frame.setValue(leftShiftOperation->getIdentifier(), mlirBitwiseLeftShiftOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::BitWiseRightShiftOperation>
        rightShiftOperation,
    ValueFrame& frame) {
  auto leftInput =
      frame.getValue(rightShiftOperation->getLeftInput()->getIdentifier());
  auto rightInput =
      frame.getValue(rightShiftOperation->getRightInput()->getIdentifier());
  auto mlirBitwiseRightShiftOp = builder->create<mlir::arith::ShRSIOp>(
      getNameLoc("binOpResult"), leftInput, rightInput);
  frame.setValue(rightShiftOperation->getIdentifier(), mlirBitwiseRightShiftOp);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::FunctionOperation> functionOp,
    ValueFrame& frame) {
  ...
  for (int i = 0;
       i < (int)functionOp->getFunctionBasicBlock()->getArguments().size();
       ++i) {
    frame.setValue(functionOp->getFunctionBasicBlock()
                       ->getArguments()
                       .at(i)
                       ->getIdentifier(),
                   valueMapIterator[i]);
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::LoopOperation> loopOp, ValueFrame& frame) {
  ...
  for (auto targetBlockArgument : loopHeadBlock.getArguments()) {
    mlirTargetBlockArguments.push_back(
        frame.getValue(targetBlockArgument->getIdentifier()));
  }
  auto* mlirTargetBlock = generateBasicBlock(loopHeadBlock, frame);
  builder->create<mlir::cf::BranchOp>(getNameLoc("branch"), mlirTargetBlock,
                                      mlirTargetBlockArguments);
}
...
//==--------------------------==//
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::AddressOperation> addressOp,
    ValueFrame& frame) {
  ...
  if (addressOp->getRecordIdxName() != "") {
    recordOffset = builder->create<mlir::LLVM::MulOp>(
        ...
        getConstInt("1", IR::Types::StampFactory::createInt64Stamp(),
                    addressOp->getRecordWidthInBytes()));
  } else {
    recordOffset =
        getConstInt("0", IR::Types::StampFactory::createInt64Stamp(), 0);
  }
  mlir::Value fieldOffset = builder->create<mlir::LLVM::AddOp>(
      getNameLoc("fieldOffset"), recordOffset,
      getConstInt("1", IR::Types::StampFactory::createInt64Stamp(),
                  addressOp->getFieldOffsetInBytes()));
...
void MLIRLoweringProvider::generateMLIR(
  ...
  if (!frame.contains(constIntOp->getIdentifier())) {
    frame.setValue(constIntOp->getIdentifier(),
                   getConstInt("ConstantOp", constIntOp->getStamp(),
                               constIntOp->getValue()));
  } else {
    frame.setValue(constIntOp->getIdentifier(),
                   getConstInt("ConstantOp", constIntOp->getStamp(),
...
void MLIRLoweringProvider::generateMLIR(
    ...
    ValueFrame& frame) {
  if (auto floatStamp =
          cast_if<IR::Types::FloatStamp>(constFloatOp->getStamp().get())) {
    auto floatType =
        (floatStamp->getBitWidth() == IR::Types::FloatStamp::BitWidth::F32)
            ? builder->getF32Type()
            : builder->getF64Type();
    frame.setValue(
...
//==---------------------------==//
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::AddOperation> addOp, ValueFrame& frame) {
  ...
  if (addOp->getLeftInput()->getStamp()->isAddress()) {
    ...
    mlir::Value elementAddress = builder->create<mlir::LLVM::GEPOp>(
        getNameLoc("fieldAccess"),
        mlir::LLVM::LLVMPointerType::get(builder->getI8Type()), leftInput,
        mlir::ArrayRef<mlir::Value>({rightInput}));
    frame.setValue(addOp->getIdentifier(), elementAddress);

  } else if (addOp->getStamp()->isFloat()) {
    auto mlirAddOp = builder->create<mlir::LLVM::FAddOp>(
        getNameLoc("binOpResult"), leftInput.getType(), leftInput, rightInput,
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::SubOperation> subIntOp, ValueFrame& frame) {
  ...
  if (subIntOp->getStamp()->isFloat()) {
    auto mlirSubOp = builder->create<mlir::LLVM::FSubOp>(
        getNameLoc("binOpResult"), leftInput, rightInput,
        mlir::LLVM::FastmathFlagsAttr::get(context,
                                           mlir::LLVM::FastmathFlags::fast));
    frame.setValue(subIntOp->getIdentifier(), mlirSubOp);
  } else {
    auto mlirSubOp = builder->create<mlir::LLVM::SubOp>(
        getNameLoc("binOpResult"), leftInput, rightInput);
    frame.setValue(subIntOp->getIdentifier(), mlirSubOp);
  }
}
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::MulOperation> mulOp, ValueFrame& frame) {
  ...
  if (mulOp->getStamp()->isFloat()) {
    auto mlirMulOp = builder->create<mlir::LLVM::FMulOp>(
        getNameLoc("binOpResult"), resultType, leftInput, rightInput,
        mlir::LLVM::FastmathFlags::fast);
    frame.setValue(mulOp->getIdentifier(), mlirMulOp);
  } else {
    auto mlirMulOp = builder->create<mlir::LLVM::MulOp>(
        getNameLoc("binOpResult"), resultType, leftInput, rightInput);
    frame.setValue(mulOp->getIdentifier(), mlirMulOp);
  }
}
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::DivOperation> divIntOp, ValueFrame& frame) {
  ...
  if (divIntOp->getStamp()->isFloat()) {
    auto mlirDivOp = builder->create<mlir::LLVM::FDivOp>(
        getNameLoc("binOpResult"), resultType, leftInput, rightInput,
        mlir::LLVM::FastmathFlags::fast);
    frame.setValue(divIntOp->getIdentifier(), mlirDivOp);
  } else {
    if (resultType.isSignedInteger()) {
      auto mlirDivOp = builder->create<mlir::LLVM::SDivOp>(
          getNameLoc("binOpResult"), resultType, leftInput, rightInput);
      frame.setValue(divIntOp->getIdentifier(), mlirDivOp);
    } else {
      auto mlirDivOp = builder->create<mlir::LLVM::UDivOp>(
          getNameLoc("binOpResult"), resultType, leftInput, rightInput);
      frame.setValue(divIntOp->getIdentifier(), mlirDivOp);
    }
  }
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::ModOperation> modIntOp, ValueFrame& frame) {
  ...
  if (modIntOp->getStamp()->isInteger()) {
    if (resultType.isSignedInteger()) {
      auto mlirModOp = builder->create<mlir::LLVM::SRemOp>(
          getNameLoc("binOpResult"), resultType, leftInput, rightInput);
      frame.setValue(modIntOp->getIdentifier(), mlirModOp);
    } else {
      auto mlirModOp = builder->create<mlir::LLVM::URemOp>(
          getNameLoc("binOpResult"), resultType, leftInput, rightInput);
      frame.setValue(modIntOp->getIdentifier(), mlirModOp);
    }
  } else {
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::StoreOperation> storeOp,
    ValueFrame& frame) {
  auto value = frame.getValue(storeOp->getValue()->getIdentifier());
  auto address = frame.getValue(storeOp->getAddress()->getIdentifier());
  auto bitcast = builder->create<mlir::LLVM::BitcastOp>(
      getNameLoc("Address Bitcasted"),
      mlir::LLVM::LLVMPointerType::get(value.getType()), address);
  builder->create<mlir::LLVM::StoreOp>(getNameLoc("outputStore"), value,
                                       bitcast);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::ReturnOperation> returnOp,
    ValueFrame& frame) {
...
void MLIRLoweringProvider::generateMLIR(
  ...
  if (theModule.lookupSymbol<mlir::LLVM::LLVMFuncOp>(
          proxyCallOp->getFunctionSymbol())) {
    functionRef =
        mlir::SymbolRefAttr::get(context, proxyCallOp->getFunctionSymbol());
  } else {
    functionRef = insertExternalFunction(
        proxyCallOp->getFunctionSymbol(), proxyCallOp->getFunctionPtr(),
  ...
  if (!proxyCallOp->getStamp()->isVoid()) {
    auto res = builder->create<mlir::LLVM::CallOp>(
        getNameLoc(proxyCallOp->getFunctionSymbol()),
        getMLIRType(proxyCallOp->getStamp()), functionRef, functionArgs);
    frame.setValue(proxyCallOp->getIdentifier(), res.getResult());
  } else {
    builder->create<mlir::LLVM::CallOp>(
        builder->getUnknownLoc(), mlir::TypeRange(), functionRef, functionArgs);
  }
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::CompareOperation> compareOp,
    ValueFrame& frame) {
  auto leftStamp = compareOp->getLeftInput()->getStamp();
  auto rightStamp = compareOp->getRightInput()->getStamp();

  if ((leftStamp->isInteger() && leftStamp->isFloat()) ||
      (leftStamp->isFloat() && rightStamp->isInteger())) {
    // Avoid comparing integer to float
    NES_THROW_RUNTIME_ERROR("Type missmatch: cannot compare "
                            << leftStamp->toString() << " to "
                            << rightStamp->toString());
  } else if (compareOp->getComparator() ==
                 IR::Operations::CompareOperation::EQ &&
             compareOp->getLeftInput()->getStamp()->isAddress() &&
    ...
    auto cmpOp = builder->create<mlir::LLVM::ICmpOp>(
        getNameLoc("comparison"), mlir::LLVM::ICmpPredicate::eq,
        frame.getValue(compareOp->getLeftInput()->getIdentifier()), null);
    frame.setValue(compareOp->getIdentifier(), cmpOp);
  } else if (compareOp->getComparator() ==
                 IR::Operations::CompareOperation::EQ &&
             compareOp->getLeftInput()->getStamp()->isAddress() &&
             compareOp->getRightInput()->getStamp()->isAddress()) {
    auto cmpOp = builder->create<mlir::LLVM::ICmpOp>(
        getNameLoc("comparison"), mlir::LLVM::ICmpPredicate::eq,
    ...
    auto cmpOp = builder->create<mlir::arith::CmpIOp>(
        getNameLoc("comparison"), mlir::arith::CmpIPredicate::eq,
        frame.getValue(compareOp->getLeftInput()->getIdentifier()),
        frame.getValue(compareOp->getRightInput()->getIdentifier()));
    frame.setValue(compareOp->getIdentifier(), cmpOp);
  } else {
    NES_THROW_RUNTIME_ERROR("Unknown type to compare: "
                            << leftStamp->toString() << " and "
                            << rightStamp->toString());
  }
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::IfOperation> ifOp, ValueFrame& frame) {
  auto parentBlockInsertionPoint = builder->saveInsertionPoint();

  std::vector<mlir::Value> trueBlockArgs;
  mlir::Block* trueBlock =
      generateBasicBlock(ifOp->getTrueBlockInvocation(), frame);
  ...
  mlir::Block* elseBlock =
      generateBasicBlock(ifOp->getFalseBlockInvocation(), frame);
  ...
  builder->create<mlir::cf::CondBranchOp>(
      getNameLoc("branch"), frame.getValue(ifOp->getValue()->getIdentifier()),
      trueBlock, trueBlockArgs, elseBlock, elseBlockArgs);
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::BranchOperation> branchOp,
    ValueFrame& frame) {
  std::vector<mlir::Value> mlirTargetBlockArguments;
  for (auto targetBlockArgument :
       branchOp->getNextBlockInvocation().getArguments()) {
    mlirTargetBlockArguments.push_back(
        frame.getValue(targetBlockArgument->getIdentifier()));
  }
  auto* mlirTargetBlock =
      generateBasicBlock(branchOp->getNextBlockInvocation(), frame);
  builder->create<mlir::cf::BranchOp>(getNameLoc("branch"), mlirTargetBlock,
                                      mlirTargetBlockArguments);
}

mlir::Block* MLIRLoweringProvider::generateBasicBlock(
    IR::Operations::BasicBlockInvocation& blockInvocation, ValueFrame&) {
  ...
  for (auto headBlockHeadTypes : targetBlockArguments) {
    mlirBasicBlock->addArgument(getMLIRType(headBlockHeadTypes->getStamp()),
                                getNameLoc("arg"));
  }
  ValueFrame blockFrame;
  for (uint32_t i = 0; i < targetBlockArguments.size(); i++) {
    blockFrame.setValue(targetBlock->getArguments()[i]->getIdentifier(),
                        mlirBasicBlock->getArgument(i));
  }
...
MLIRLoweringProvider::createFrameFromParentBlock(
...
void MLIRLoweringProvider::generateMLIR(
  ...
  if (inputStamp->isInteger() && outputStamp->isInteger()) {
    ...
    if (inputIntegerStamp->getBitWidth() == outputIntegerStamp->getBitWidth()) {
      ...
      auto mlirCast = builder->create<mlir::arith::ExtSIOp>(
          getNameLoc("location"), getMLIRType(outputStamp), mlirInput);
      frame.setValue(castOperation->getIdentifier(), mlirCast);
    } else {
      auto mlirCast = builder->create<mlir::arith::ExtUIOp>(
          getNameLoc("location"), getMLIRType(outputStamp), mlirInput);
      frame.setValue(castOperation->getIdentifier(), mlirCast);
    }
    return;
    ...
    auto mlirInput = frame.getValue(castOperation->getInput()->getIdentifier());
    auto mlirCast = builder->create<mlir::arith::ExtFOp>(
        getNameLoc("location"), getMLIRType(outputFloatStamp), mlirInput);
    frame.setValue(castOperation->getIdentifier(), mlirCast);
    return;
    ...
    if (inputIntegerStamp->isSigned()) {
      auto mlirCast = builder->create<mlir::arith::SIToFPOp>(
          getNameLoc("location"), getMLIRType(outputStamp), mlirInput);
      frame.setValue(castOperation->getIdentifier(), mlirCast);
    } else {
      auto mlirCast = builder->create<mlir::arith::UIToFPOp>(
          getNameLoc("location"), getMLIRType(outputStamp), mlirInput);
      frame.setValue(castOperation->getIdentifier(), mlirCast);
    }
    return;
  } else {
    NES_THROW_RUNTIME_ERROR("Cast from " << inputStamp->toString() << " to "
                                         << outputStamp->toString()
                                         << " is not supported.");
  }
}
...
std::vector<llvm::JITTargetAddress>
MLIRLoweringProvider::getJitProxyTargetAddresses() {
