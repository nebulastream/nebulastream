...
convertToIntMLIRComparison(IR::Operations::CompareOperation::Comparator comparisonType, IR::Types::StampPtr & stamp)
{
...
void MLIRLoweringProvider::generateMLIR(std::shared_ptr<IR::Operations::NegateOperation> negateOperation, ValueFrame & frame)
{
    ...
    auto negate = builder->create<mlir::arith::CmpIOp>(
        getNameLoc("comparison"), mlir::arith::CmpIPredicate::eq, input, getConstBool("bool", false));
    ...
    if (auto floatStamp = cast_if<IR::Types::FloatStamp>(constFloatOp->getStamp().get()))
    {
        ...
        frame.setValue(
            ...
            builder->create<mlir::LLVM::ConstantOp>(
                getNameLoc("constantFloat"), floatType, builder->getFloatAttr(floatType, constFloatOp->getValue())));
    }
}
...
void MLIRLoweringProvider::generateMLIR(std::shared_ptr<IR::Operations::AddOperation> addOp, ValueFrame & frame)
{
    ...
    else if (addOp->getStamp()->isFloat())
    {
        auto mlirAddOp = builder->create<mlir::LLVM::FAddOp>(
            getNameLoc("binOpResult"), leftInput.getType(), leftInput, rightInput, mlir::LLVM::FastmathFlags::fast);
...
void MLIRLoweringProvider::generateMLIR(std::shared_ptr<IR::Operations::SubOperation> subIntOp, ValueFrame & frame)
{
    ...
    if (subIntOp->getStamp()->isFloat())
    {
        auto mlirSubOp = builder->create<mlir::LLVM::FSubOp>(
            getNameLoc("binOpResult"), leftInput, rightInput, mlir::LLVM::FastmathFlagsAttr::get(context, mlir::LLVM::FastmathFlags::fast));
...
void MLIRLoweringProvider::generateMLIR(std::shared_ptr<IR::Operations::MulOperation> mulOp, ValueFrame & frame)
{
    ...
    if (mulOp->getStamp()->isFloat())
    {
        auto mlirMulOp = builder->create<mlir::LLVM::FMulOp>(
            getNameLoc("binOpResult"), resultType, leftInput, rightInput, mlir::LLVM::FastmathFlags::fast);
...
void MLIRLoweringProvider::generateMLIR(std::shared_ptr<IR::Operations::ProxyCallOperation> proxyCallOp, ValueFrame & frame)
{
    ...
    if (!proxyCallOp->getStamp()->isVoid())
    {
        auto res = builder->create<mlir::LLVM::CallOp>(
            getNameLoc(proxyCallOp->getFunctionSymbol()), getMLIRType(proxyCallOp->getStamp()), functionRef, functionArgs);
...
void MLIRLoweringProvider::generateMLIR(std::shared_ptr<IR::Operations::IfOperation> ifOp, ValueFrame & frame)
{
    ...
    builder->create<mlir::cf::CondBranchOp>(
        getNameLoc("branch"), frame.getValue(ifOp->getValue()->getIdentifier()), trueBlock, trueBlockArgs, elseBlock, elseBlockArgs);
}
...
MLIRLoweringProvider::ValueFrame MLIRLoweringProvider::createFrameFromParentBlock(
    MLIRLoweringProvider::ValueFrame & frame, IR::Operations::BasicBlockInvocation & invocation)
{
...
void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::CastOperation> castOperation, MLIRLoweringProvider::ValueFrame & frame)
{
    ...
    {
        NES_THROW_RUNTIME_ERROR("Cast from " << inputStamp->toString() << " to " << outputStamp->toString() << " is not supported.");
    }
}

void MLIRLoweringProvider::generateMLIR(
    std::shared_ptr<IR::Operations::ConstBooleanOperation> constBooleanOp, MLIRLoweringProvider::ValueFrame & frame)
{
    auto constOp = builder->create<mlir::arith::ConstantOp>(
        getNameLoc("location"), builder->getI1Type(), builder->getIntegerAttr(builder->getI1Type(), constBooleanOp->getValue()));
