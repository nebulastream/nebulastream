...
std::tuple<Code, RegisterFile> BCLoweringProvider::lower(
    std::shared_ptr<IR::IRGraph> ir) {
...
short BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::BasicBlock>& block, RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::AndOperation>& addOpt, short block,
    RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::OrOperation>& addOpt, short block,
    RegisterFrame& frame) {
  auto leftInput = frame.getValue(addOpt->getLeftInput()->getIdentifier());
  auto rightInput = frame.getValue(addOpt->getRightInput()->getIdentifier());
  auto resultReg = getResultRegister(addOpt, frame);
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::AddOperation>& addOpt, short block,
    RegisterFrame& frame) {
  auto leftInput = frame.getValue(addOpt->getLeftInput()->getIdentifier());
  auto rightInput = frame.getValue(addOpt->getRightInput()->getIdentifier());
  auto resultReg = getResultRegister(addOpt, frame);
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::SubOperation>& subOpt, short block,
    RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::MulOperation>& mulOpt, short block,
    RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::DivOperation>& divOp, short block,
    RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::CompareOperation>& cmpOp, short block,
    RegisterFrame& frame) {
  ...
  if (cmpOp->getComparator() ==
      IR::Operations::CompareOperation::Comparator::EQ) {
    ...
    OpCode oc = {bc, leftInput, rightInput, resultReg};
    program.blocks[block].code.emplace_back(oc);
  } else if (cmpOp->getComparator() ==
             IR::Operations::CompareOperation::Comparator::LT) {
    auto type = cmpOp->getLeftInput()->getStamp();
    ByteCode bc;
    switch (getType(type)) {
      case Type::i8:
    ...
    program.blocks[block].code.emplace_back(oc);
             ...
             IR::Operations::CompareOperation::Comparator::GT) {
    ByteCode bc;
    auto type = cmpOp->getLeftInput()->getStamp();
    switch (getType(type)) {
      case Type::i8:
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::LoadOperation>& loadOp, short block,
    RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::StoreOperation>& storeOp, short block,
    RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    IR::Operations::BasicBlockInvocation& bi, short block,
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::IfOperation>& ifOpt, short block,
    RegisterFrame& frame) {
  ...
  auto falseBlockIndex =
      process(ifOpt->getFalseBlockInvocation().getBlock(), frame);
  program.blocks[block].terminatorOp =
      ConditionalJumpOp{conditionalReg, trueBlockIndex, falseBlockIndex};
}
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::Operation>& opt, short block,
    RegisterFrame& frame) {
  switch (opt->getOperationType()) {
    ...
    case IR::Operations::Operation::OperationType::ConstFloatOp: {
      auto constInt =
          std::static_pointer_cast<IR::Operations::ConstFloatOperation>(opt);
      auto defaultRegister = registerProvider.allocRegister();
      if (cast<IR::Types::FloatStamp>(constInt->getStamp())->getBitWidth() ==
          IR::Types::FloatStamp::BitWidth::F32) {
        auto floatValue = (float)constInt->getValue();
        auto floatReg =
            reinterpret_cast<float*>(&defaultRegisterFile[defaultRegister]);
        *floatReg = floatValue;
      } else {
        auto floatValue = (double)constInt->getValue();
        auto floatReg =
            reinterpret_cast<double*>(&defaultRegisterFile[defaultRegister]);
        *floatReg = floatValue;
      }
    ...
    case IR::Operations::Operation::OperationType::NegateOp: {
      auto call =
          std::static_pointer_cast<IR::Operations::NegateOperation>(opt);
      process(call, block, frame);
      return;
    }
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::ProxyCallOperation>& opt, short block,
    RegisterFrame& frame) {
  // first try to lower to a native call if the correct stub was registered.
  auto returnValue = processNativeCall(opt, block, frame);
  if (!returnValue) {
    // if it was not possible to lower the call, we try to create a dynamic call
    // using dyncall.h
  ...
  for (auto& arg : arguments) {
    ...
    switch (argType) {
      ...
      case Type::ptr:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Type not implemented");
    }
...
bool BCLoweringProvider::LoweringContext::processNativeCall(
    const std::shared_ptr<IR::Operations::ProxyCallOperation>& opt, short block,
    RegisterFrame& frame) {
  // TODO the following code is very bad and manually checks function
  // signatures. Type to come up with something more generic.
  NES_DEBUG("CREATE {}: {}", opt->toString(), opt->getStamp()->toString())
  auto arguments = opt->getInputArguments();
  ByteCode bc;
  if (opt->getStamp()->isVoid()) {
    if (arguments.empty()) {
      ...
      if (getType(arguments[0]->getStamp()) == Type::ptr &&
          getType(arguments[1]->getStamp()) == Type::ui64) {
      ...
      if (getType(arguments[0]->getStamp()) == Type::ptr &&
          getType(arguments[1]->getStamp()) == Type::ptr &&
    ...
    if (arguments.empty()) {
      ...
      if (getType(arguments[0]->getStamp()) == Type::i64 &&
          getType(arguments[1]->getStamp()) == Type::i64) {
    ...
    if (arguments.empty()) {
      ...
      if (getType(arguments[0]->getStamp()) == Type::i64 &&
          getType(arguments[1]->getStamp()) == Type::i64) {
        return false;
      } else if (getType(arguments[0]->getStamp()) == Type::ui64 &&
                 getType(arguments[1]->getStamp()) == Type::i64) {
        bc = ByteCode::CALL_ui64_ui64_i64;
      } else if (getType(arguments[0]->getStamp()) == Type::ui64 &&
                 getType(arguments[1]->getStamp()) == Type::i32) {
        bc = ByteCode::CALL_ui64_ui64_i32;
      } else if (getType(arguments[0]->getStamp()) == Type::ui64 &&
                 getType(arguments[1]->getStamp()) == Type::i8) {
  ...
  auto fcall =
      new Backends::BC::FunctionCallTarget(argRegisters, opt->getFunctionPtr());
  auto funcInfoRegister = registerProvider.allocRegister();
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::NegateOperation>& negateOperation,
    short block, RegisterFrame& frame) {
...
void BCLoweringProvider::LoweringContext::process(
    const std::shared_ptr<IR::Operations::CastOperation>& castOp, short block,
    RegisterFrame& frame) {
  ...
  if (srcType == Type::i8) {
    switch (targetType) {
      ...
      case Type::d:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Cast from i8 not supported");
    }
    ...
    switch (targetType) {
      ...
      case Type::d:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Cast from i16 not supported");
    }
    ...
    switch (targetType) {
      ...
      case Type::d:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Cast from i32 not supported");
    }
    ...
    switch (targetType) {
      ...
      case Type::d:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Cast from i64 not supported");
    }
    ...
    switch (targetType) {
      ...
      case Type::i64:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Cast from ui8 not supported");
    }
    ...
    switch (targetType) {
      ...
      case Type::i64:
        ...
        break;
      default:
        NES_THROW_RUNTIME_ERROR("Cast from ui16 not supported");
    }
...
short BCLoweringProvider::LoweringContext::getResultRegister(
  ...
  if (opt->getUsages().size() == 1) {
    ...
    if (usage->getOperationType() ==
        IR::Operations::Operation::OperationType::BlockInvocation) {
      auto bi =
          dynamic_cast<const IR::Operations::BasicBlockInvocation*>(usage);
      auto blockInputArguments = bi->getArguments();
      auto blockTargetArguments = bi->getBlock()->getArguments();
      ...
      if (matchingArguments.size() == 1) {
        auto blockTargetArgumentIdentifier =
            blockTargetArguments[matchingArguments[0]]->getIdentifier();
