...
void inline addPredecessorToBlock(BasicBlockPtr currentBlock,
    ...
    if (terminatorOp.getOperationType() == Operations::Operation::OperationType::BranchOp) {
        auto& branchOp = dynamic_cast<IR::Operations::BranchOperation&>(terminatorOp);
...
void RemoveBrOnlyBlocksPhase::RemoveBrOnlyBlocksPhaseContext::addPredecessors(BasicBlockPtr currentBlock) {
    std::stack<gsl::not_null<BasicBlock*>> newBlocks;
...
void updatePredecessorBlocks(std::vector<IR::BasicBlockPtr>& brOnlyBlocks, IR::BasicBlock& nonBrOnlyBlock) {
    ...
    std::vector<IR::BasicBlockPtr> newPredecessorBlocks;
    ...
    for (const auto& brOnlyBlock : brOnlyBlocks) {
        ...
        for (const auto& predecessor : brOnlyBlock->getPredecessors()) {
            if (!removedBlocks.contains(predecessor->getIdentifier())) {
                newPredecessorBlocks.emplace_back(predecessor);
            }
            auto& terminatorOp = predecessor->getTerminatorOp();
            ...
            if (terminatorOp.getOperationType() == Operations::Operation::OperationType::IfOp) {
                ...
                if (ifOp.getTrueBlockInvocation().getBlock()->getIdentifier() == brOnlyBlock->getIdentifier()) {
                    ...
                    if (ifOp.getTrueBlockInvocation().getBlock()->getIdentifier() == nonBrOnlyBlock.getIdentifier()) {
                        ...
                        auto oldOperation = predecessor->removeOperation(predecessor->getTerminatorOp());
                        auto newBranchOperation = std::make_unique<Operations::BranchOperation>();
                        ...
                        predecessor->addOperation(std::move(newBranchOperation));
                    }
                }
    ...
    for (const auto& newPredecessor : newPredecessorBlocks) {
        nonBrOnlyBlock.addPredecessor(*newPredecessor);
    }
}
    ...
    if (terminatorOp.getOperationType() == Operations::Operation::OperationType::BranchOp) {
        ...
        if (currentBlock->getOperations().size() == 1) {
            ...
            while (currentBlock->getOperations().size() == 1
                ...
                branchOp = &dynamic_cast<IR::Operations::BranchOperation&>(currentBlock->getTerminatorOp());
