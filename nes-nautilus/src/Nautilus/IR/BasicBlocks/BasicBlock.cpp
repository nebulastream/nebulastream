...
BasicBlock::BasicBlock(
    std::string identifier, int32_t scopeLevel,
...
std::vector<Operations::OperationPtr> BasicBlock::getOperations() {
  return operations;
}
[[nodiscard]] Operations::OperationPtr BasicBlock::getOperationAt(
    size_t index) {
  return operations.at(index);
}
Operations::OperationPtr BasicBlock::getTerminatorOp() {
  return operations.back();
}
std::vector<std::shared_ptr<Operations::BasicBlockArgument>>
BasicBlock::getArguments() {
  return arguments;
}
...
void BasicBlock::replaceTerminatorOperation(
    Operations::OperationPtr loopOperation) {
  operations.pop_back();
  operations.emplace_back(std::move(loopOperation));
}

// NESIR Assembly
std::shared_ptr<BasicBlock> BasicBlock::addOperation(
    Operations::OperationPtr operation) {
  operations.push_back(operation);
  return shared_from_this();
}
std::shared_ptr<BasicBlock> BasicBlock::addLoopHeadBlock(
    BasicBlockPtr loopHeadBlock) {
  std::static_pointer_cast<Operations::LoopOperation>(this->operations.back())
      ->getLoopHeadBlock()
      .setBlock(loopHeadBlock);
  return shared_from_this();
}
std::shared_ptr<BasicBlock> BasicBlock::addNextBlock(
    std::shared_ptr<BasicBlock> nextBlock) {
  std::static_pointer_cast<Operations::BranchOperation>(this->operations.back())
      ->getNextBlockInvocation()
      .setBlock(nextBlock);
  return shared_from_this();
}
std::shared_ptr<BasicBlock> BasicBlock::addTrueBlock(
    std::shared_ptr<BasicBlock> thenBlock) {
  std::static_pointer_cast<Operations::IfOperation>(this->operations.back())
      ->getTrueBlockInvocation()
      .setBlock(thenBlock);
  return shared_from_this();
}
std::shared_ptr<BasicBlock> BasicBlock::addFalseBlock(
    std::shared_ptr<BasicBlock> elseBlock) {
  std::static_pointer_cast<Operations::IfOperation>(this->operations.back())
...
void BasicBlock::addPredecessor(std::shared_ptr<BasicBlock> predecessor) {
  this->predecessors.emplace_back(predecessor);
}
std::vector<std::weak_ptr<BasicBlock>>& BasicBlock::getPredecessors() {
  return predecessors;
}
void BasicBlock::addNextBlock(std::shared_ptr<BasicBlock> nextBlock,
                              std::vector<Operations::OperationPtr> ops) {
...
BasicBlock::getNextBlocks() {
  ...
  if (operations.back()->getOperationType() ==
    ...
    return std::make_pair(ifOp->getTrueBlockInvocation().getBlock(),
             ...
             IR::Operations::Operation::OperationType::LoopOp) {
    auto loopOp = std::static_pointer_cast<IR::Operations::LoopOperation>(
        operations.back());
    return std::make_pair(loopOp->getLoopBodyBlock().getBlock(),
                          loopOp->getLoopFalseBlock().getBlock());
  } else if (operations.back()->getOperationType() ==
             IR::Operations::Operation::OperationType::ReturnOp) {
    return {};
  } else {
    NES_ERROR(
        "BasicBlock::getNextBlocks: Tried to get next block for unsupported "
        "operation type: {}",
