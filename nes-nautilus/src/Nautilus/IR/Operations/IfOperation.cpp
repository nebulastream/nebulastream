...
IfOperation::IfOperation(OperationPtr booleanValue)
    : Operation(Operation::OperationType::IfOp,
...
std::string IfOperation::toString() {
  std::string baseString = "if " + getValue()->getIdentifier() + " ? Block_" +
  ...
  if (trueBlockInvocation.getArguments().size() > 0) {
    ...
    for (int i = 1; i < (int)trueBlockInvocation.getArguments().size(); ++i) {
      baseString +=
          ", " + trueBlockInvocation.getArguments().at(i)->getIdentifier();
    }
  }
  if (falseBlockInvocation.getBlock()) {
    baseString +=
        ") : Block_" + falseBlockInvocation.getBlock()->getIdentifier() + '(';
    if (falseBlockInvocation.getArguments().size() > 0) {
      baseString += falseBlockInvocation.getArguments()[0]->getIdentifier();
      for (int i = 1; i < (int)falseBlockInvocation.getArguments().size();
           ++i) {
        baseString +=
            ", " + falseBlockInvocation.getArguments().at(i)->getIdentifier();
      }
    }
  }
  return baseString += ')';
