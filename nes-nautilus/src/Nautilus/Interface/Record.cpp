...
void Record::write(const RecordFieldIdentifier& recordFieldIdentifier, const VarVal& dataWithType)
{
    ...
    if (const auto [hashMapIterator, inserted] = recordFields.insert({recordFieldIdentifier, dataWithType}); not inserted)
    {
        ...
        recordFields.insert_or_assign(recordFieldIdentifier, dataWithType);
    }
}
