...
Value<MemRef> FixedPageRef::allocateEntry(const Value<UInt64> &hash) {
  ...
  if (currentPos < getCapacity()) {
    ...
    FunctionCall("addHashToBloomFilterProxy", addHashToBloomFilterProxy,
                 fixedPageRef, hash);
...
FixedPageRefIter::FixedPageRefIter(const FixedPageRefIter &it)
    : addr(it.addr), fixedPageRef(it.fixedPageRef) {}
