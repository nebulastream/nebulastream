...
FixedPage::FixedPage(uint8_t *dataPtr, size_t sizeOfRecord, size_t pageSize,
    ...
    : sizeOfRecord(sizeOfRecord), data(dataPtr),
      capacity(pageSize / sizeOfRecord), bloomFalsePosRate(bloomFalsePosRate) {
  NES_ASSERT2_FMT(0 < capacity,
                  "Capacity is zero for pageSize " + std::to_string(pageSize) +
                      " and sizeOfRecord " + std::to_string(pageSize));

  bloomFilter =
      std::make_unique<Runtime::BloomFilter>(capacity, bloomFalsePosRate);
  currentPos = 0;
}
...
std::string FixedPage::getContentAsString(SchemaPtr schema) const {
  ...
  for (auto i = 0UL; i < currentPos; i++) {
    ...
    for (auto u = 0UL; u < schema->getSize(); u++) {
      ...
      NES_ASSERT(schema->get(u)->getDataType()->isNumeric(),
                 "This method is only supported for uint64");
...
FixedPage::FixedPage(FixedPage &&otherPage)
    : sizeOfRecord(otherPage.sizeOfRecord), data(otherPage.data),
      currentPos(otherPage.currentPos.load()), capacity(otherPage.capacity),
