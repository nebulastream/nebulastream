...
ChainedHashMapRef::EntryRef ChainedHashMapRef::findChain(
...
ChainedHashMapRef::EntryRef ChainedHashMapRef::insert(
    const Value<UInt64>& hash) {
...
ChainedHashMapRef::EntryRef ChainedHashMapRef::find(
    const Value<UInt64>& hash, const std::vector<Value<>>& keys) {
  // find chain
...
ChainedHashMapRef::KeyEntryIterator ChainedHashMapRef::findAll(
    const Value<UInt64>& hash, const std::vector<Value<>>& keys) {
...
Value<Boolean> ChainedHashMapRef::compareKeys(
    EntryRef& entry, const std::vector<Value<>>& keys) {
...
ChainedHashMapRef::EntryIterator::EntryIterator(
    ...
    : hashTableRef(hashTableRef),
      ...
      inPageIndex(0_u64),
      currentPage(hashTableRef.getPage(0_u64)),
      currentPageIndex(0_u64),
      currentIndex(currentIndex) {}

ChainedHashMapRef::EntryIterator::EntryIterator(
    ChainedHashMapRef& hashTableRef, const Value<UInt64>& currentIndex)
    : hashTableRef(hashTableRef),
      entriesPerPage(0_u64),
      inPageIndex(0_u64),
...
ChainedHashMapRef::EntryRef ChainedHashMapRef::KeyEntryIterator::operator*()
    const {
  return currentEntry;
}
}  // namespace NES::Nautilus::Interface
