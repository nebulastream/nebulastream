...
#include <Nautilus/Interface/HashMap/HashMapRefInterface.hpp>
...
{
void ChainedEntryRef::copyKeysToEntry(const Nautilus::Record& keys) const
{
    memoryProviderKeys.writeRecord(entryRef, keys);
}

void ChainedEntryRef::copyKeysToEntry(const ChainedEntryRef& otherEntryRef) const
{
    memoryProviderKeys.writeEntryRef(entryRef, otherEntryRef.entryRef);
}

void ChainedEntryRef::copyValuesToEntry(const Nautilus::Record& values) const
{
    memoryProviderValues.writeRecord(entryRef, values);
}

void ChainedEntryRef::copyValuesToEntry(const ChainedEntryRef& otherEntryRef) const
{
    memoryProviderValues.writeEntryRef(entryRef, otherEntryRef.entryRef);
}

VarVal ChainedEntryRef::getKey(const Record::RecordFieldIdentifier& fieldIdentifier) const
{
    auto recordKey = memoryProviderKeys.readVarVal(entryRef, fieldIdentifier);
    return recordKey;
}

Record ChainedEntryRef::getKey() const
{
    return memoryProviderKeys.readRecord(entryRef);
}

Record ChainedEntryRef::getValue() const
{
    return memoryProviderValues.readRecord(entryRef);
}

void ChainedEntryRef::updateEntryRef(const nautilus::val<ChainedHashMapEntry*>& entryRef)
{
    ChainedEntryRef::entryRef = entryRef;
}

nautilus::val<int8_t*> ChainedEntryRef::getValueMemArea() const
{
...
HashFunction::HashValue ChainedEntryRef::getHash() const
{
...
nautilus::val<ChainedHashMapEntry*> ChainedEntryRef::getNext() const
{
    return invoke(+[](const ChainedHashMapEntry* entry) { return entry->next; }, entryRef);
}

ChainedEntryRef::ChainedEntryRef(
...
ChainedEntryRef::ChainedEntryRef(const ChainedEntryRef& other)

    = default;

ChainedEntryRef& ChainedEntryRef::operator=(const ChainedEntryRef& other) = default;
...
{
    auto entry = findChain(otherEntryRef.getHash());
    while (entry)
    {
        ChainedEntryRef const entryRef(entry, fieldKeys, fieldValues);
        if (compareKeys(entryRef, otherEntryRef))
        {
            return entryRef.entryRef;
        }
        entry = entryRef.getNext();
    }
    return nullptr;
...
EntryIterator ChainedHashMapRef::begin() const
{
    constexpr uint64_t numberOfTuples = 0;
    return {hashMapRef, numberOfTuples, fieldKeys, fieldValues};
}

EntryIterator ChainedHashMapRef::end() const
{
    const auto numberOfTuples
        = invoke(+[](HashMapInterface* hashMap) { return dynamic_cast<ChainedHashMap*>(hashMap)->getNumberOfTuples(); }, hashMapRef);
    return {hashMapRef, numberOfTuples, fieldKeys, fieldValues};
}
...
{
    return invoke(
        +[](HashMapInterface* hashMap, const HashFunction::HashValue::raw_type hashValue)
        {
...
ChainedHashMapRef::insert(const HashFunction::HashValue& hash, const nautilus::val<Memory::AbstractBufferProvider*>& bufferProvider)
{
    const auto newEntry = invoke(
        +[](HashMapInterface* hashMap, const HashFunction::HashValue::raw_type hashValue, Memory::AbstractBufferProvider* bufferProviderVal)
...
nautilus::val<bool> ChainedHashMapRef::compareKeys(const ChainedEntryRef& entryRef, const ChainedEntryRef& otherEntryRef) const
{
    nautilus::val<bool> equals = true;
    for (const auto& [fieldIdentifier, type, fieldOffset] : nautilus::static_iterable(fieldKeys))
    {
        auto keyFromEntry = entryRef.getKey(fieldIdentifier);
        auto keyFromOtherEntry = otherEntryRef.getKey(fieldIdentifier);
        equals = equals && (keyFromEntry == keyFromOtherEntry);
    }
    return equals;
}
...
ChainedHashMapRef::ChainedHashMapRef(
    const nautilus::val<HashMapInterface*>& hashMapRef,
    std::vector<MemoryProvider::FieldOffsets> fieldsKey,
    std::vector<MemoryProvider::FieldOffsets> fieldsValue,
    const uint64_t entriesPerPage,
    const uint64_t entrySize)
    : HashMapRefInterface(hashMapRef)
...
uint64_t findChainIndexProxy(const HashMapInterface* hashMap, const uint64_t tupleIndexVal)
{
    ...
    for (; chainIndex < chainedHashMap->getNumberOfChains(); chainIndex = chainIndex + 1)
    {
        auto* currentChain = chainedHashMap->getStartOfChain(chainIndex);
    ...
    throw QueryRuntimeError("Could not find the tuple with index {}", tupleIndexVal);
}
    ...
    : hashMapRef(hashMapRef), currentEntry({nullptr, fieldKeys, fieldValues}), chainIndex(0), tupleIndex(tupleIndex)
{
    ...
    currentEntry = ChainedEntryRef(
        invoke(
            +[](const HashMapInterface* hashMap, const uint64_t chainIndexVal)
            { return dynamic_cast<const ChainedHashMap*>(hashMap)->getStartOfChain(chainIndexVal); },
            hashMapRef,
            chainIndex),
        fieldKeys,
        fieldKeys);
    numberOfChains = invoke(
        +[](const HashMapInterface* hashMap) { return dynamic_cast<const ChainedHashMap*>(hashMap)->getNumberOfChains(); }, hashMapRef);
}
...
nautilus::val<bool> EntryIterator::operator==(const EntryIterator& other) const
{
...
nautilus::val<ChainedHashMapEntry*> EntryIterator::operator*() const
{
