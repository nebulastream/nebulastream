...
ChainedHashMapRef::EntryRef::EntryRef(const Value<MemRef> &ref,
                                      uint64_t keyOffset, uint64_t valueOffset)
    : ref(ref), keyOffset(keyOffset), valueOffset(valueOffset) {}

Value<MemRef> ChainedHashMapRef::EntryRef::getKeyPtr() const {
  return (ref + keyOffset).as<MemRef>();
}

Value<MemRef> ChainedHashMapRef::EntryRef::getValuePtr() const {
  return (ref + valueOffset).as<MemRef>();
}
...
bool ChainedHashMapRef::EntryRef::operator!=(std::nullptr_t) const {
  return ref != 0;
}
bool ChainedHashMapRef::EntryRef::operator==(std::nullptr_t) const {
  return ref == 0;
...
ChainedHashMapRef::ChainedHashMapRef(
    const Value<MemRef> &hashTableRef,
    const std::vector<PhysicalTypePtr> &keyDataTypes, uint64_t keySize,
    uint64_t valueSize)
    : hashTableRef(hashTableRef), keyDataTypes(keyDataTypes), keySize(keySize),
      valueSize(valueSize) {}

ChainedHashMapRef::EntryRef
ChainedHashMapRef::findChain(const Value<UInt64> &hash) {
  auto entry =
      FunctionCall<>("findChainProxy", findChainProxy, hashTableRef, hash);
  return {entry, sizeof(ChainedHashMap::Entry),
          sizeof(ChainedHashMap::Entry) + keySize};
}

ChainedHashMapRef::EntryRef
ChainedHashMapRef::insert(const Value<UInt64> &hash) {
...
ChainedHashMapRef::EntryRef
ChainedHashMapRef::find(const Value<UInt64> &hash,
                        const std::vector<Value<>> &keys) {
  // find chain
...
ChainedHashMapRef::KeyEntryIterator
ChainedHashMapRef::findAll(const Value<UInt64> &hash,
                           const std::vector<Value<>> &keys) {
  return {*this, hash, keys, 0_u64};
}

ChainedHashMapRef::EntryRef
ChainedHashMapRef::findOrCreate(const Value<UInt64> &hash,
                                const std::vector<Value<>> &keys) {
...
ChainedHashMapRef::EntryRef ChainedHashMapRef::findOrCreate(
...
void ChainedHashMapRef::insertEntryOrUpdate(
    const EntryRef &otherEntry, const std::function<void(EntryRef &)> &update) {
  auto entry = findChain(otherEntry.getHash());
  for (; entry != nullptr; entry = entry.getNext()) {
    if (MemRefUtils::memEquals(entry.getKeyPtr(), otherEntry.getKeyPtr(),
  ...
  if (entry == nullptr) {
    ...
    MemRefUtils::memCopy(newEntry.getKeyPtr(), otherEntry.getKeyPtr(),
                         Value<UInt64>(keySize + valueSize));
  }
}

Value<Boolean>
ChainedHashMapRef::compareKeys(EntryRef &entry,
                               const std::vector<Value<>> &keys) {
...
Value<UInt64> ChainedHashMapRef::getCurrentSize() {
  return getMember(this->hashTableRef, ChainedHashMap, currentSize)
      .load<UInt64>();
}

Value<UInt64> ChainedHashMapRef::getPageSize() {
  return getMember(this->hashTableRef, ChainedHashMap, pageSize).load<UInt64>();
}
...
ChainedHashMapRef::EntryIterator::EntryIterator(
    ChainedHashMapRef &hashTableRef, const Value<UInt64> &entriesPerPage,
    const Value<UInt64> &currentIndex)
    : hashTableRef(hashTableRef), entriesPerPage(entriesPerPage),
      inPageIndex(0_u64), currentPage(hashTableRef.getPage(0_u64)),
      currentPageIndex(0_u64), currentIndex(currentIndex) {}

ChainedHashMapRef::EntryIterator::EntryIterator(
    ChainedHashMapRef &hashTableRef, const Value<UInt64> &currentIndex)
    : hashTableRef(hashTableRef), entriesPerPage(0_u64), inPageIndex(0_u64),
      currentPage(/*use hash table ref as a temp mem ref that is in this case
                     never used*/
                  hashTableRef.hashTableRef),
      currentPageIndex(0_u64), currentIndex(currentIndex) {}

ChainedHashMapRef::EntryIterator &
ChainedHashMapRef::EntryIterator::operator++() {
...
bool ChainedHashMapRef::EntryIterator::operator==(
    const EntryIterator &other) const {
  return this->currentIndex == other.currentIndex;
}

ChainedHashMapRef::EntryRef ChainedHashMapRef::EntryIterator::operator*() {
  auto entry = currentPage + ((sizeof(ChainedHashMap::Entry) +
                               hashTableRef.keySize + hashTableRef.valueSize) *
                              inPageIndex);
  return {entry.as<MemRef>(), sizeof(ChainedHashMap::Entry),
          sizeof(ChainedHashMap::Entry) + hashTableRef.keySize};
}

Value<UInt64> ChainedHashMapRef::getEntriesPerPage() {
  return getMember(this->hashTableRef, ChainedHashMap, entriesPerPage)
      .load<UInt64>();
}

ChainedHashMapRef::KeyEntryIterator::KeyEntryIterator(
    ChainedHashMapRef &hashTableRef, const Value<UInt64> &hash,
    const std::vector<Value<>> &keys, const Value<UInt64> &currentIndex)
    : hashTableRef(hashTableRef), currentIndex(currentIndex), keys(keys),
      currentEntry(hashTableRef.findChain(hash)) {}

ChainedHashMapRef::KeyEntryIterator &
ChainedHashMapRef::KeyEntryIterator::operator++() {
  Value<Boolean> equals = false;
  for (; currentEntry != nullptr && !equals;
...
bool ChainedHashMapRef::KeyEntryIterator::operator==(
    KeyEntryIterator other) const {
  return currentEntry == nullptr && other.currentEntry == nullptr;
}

bool ChainedHashMapRef::KeyEntryIterator::operator==(std::nullptr_t) const {
  return currentEntry == nullptr;
}

ChainedHashMapRef::EntryRef
ChainedHashMapRef::KeyEntryIterator::operator*() const {
  return currentEntry;
}
} // namespace NES::Nautilus::Interface
