...
{
    /// For now, we assume that we the fields lie consequently in the memory like in a row layout.
    ...
    for (const auto& fieldName : fieldNameKeys)
    {
        ...
        if (const auto fieldValue = field.value_or(nullptr); fieldValue != nullptr)
        {
        ...
        {
            throw FieldNotFound("Could not find field: {}", fieldName);
        }
    }
