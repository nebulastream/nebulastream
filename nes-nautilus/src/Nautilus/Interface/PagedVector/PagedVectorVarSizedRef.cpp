...
PagedVectorVarSizedRef::PagedVectorVarSizedRef(
    const Value<MemRef>& pagedVectorVarSizedRef, SchemaPtr schema, Value<MemRef> pipelineExecutionContext)
...
void PagedVectorVarSizedRef::writeRecord(Record record)
{
    ...
    if (tuplesOnPage >= getCapacityPerPage())
    {
        Nautilus::FunctionCall(
            "allocateNewPageVarSizedProxy", allocateNewPageVarSizedProxy, pagedVectorVarSizedRef, pipelineExecutionContext);
        tuplesOnPage = 0_u64;
    }
    ...
    for (auto& field : schema->fields)
    {
        ...
        if (fieldType->type->isText())
        {
            auto textEntryMapKey = Nautilus::FunctionCall(
                "storeTextProxy", storeTextProxy, pagedVectorVarSizedRef, fieldValue.as<Text>()->getReference(), pipelineExecutionContext);
...
Record PagedVectorVarSizedRef::readRecord(const Value<UInt64>& pos)
{
    ...
    for (auto& field : schema->fields)
    {
        ...
        if (fieldType->type->isText())
        {
            ...
            pageEntry = pageEntry + Value<UInt64>((uint64_t)sizeof(uint64_t));
            auto text
                = Nautilus::FunctionCall("loadTextProxy", loadTextProxy, pagedVectorVarSizedRef, textEntryMapKey, pipelineExecutionContext);
