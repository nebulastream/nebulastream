...
PagedVector::PagedVector(std::unique_ptr<std::pmr::memory_resource> allocator,
                         uint64_t entrySize, uint64_t pageSize)
    : allocator(std::move(allocator)), entrySize(entrySize), pageSize(pageSize),
      capacityPerPage(pageSize / entrySize), numberOfEntries(0),
      totalNumberOfEntries(0) {
  appendPage();
  NES_ASSERT2_FMT(entrySize > 0,
                  "Entrysize for a pagedVector has to be larger than 0!");
  NES_ASSERT2_FMT(capacityPerPage > 0,
                  "There has to fit at least one tuple on a page!");
}
...
void PagedVector::setNumberOfEntries(uint64_t entries) {
...
void PagedVector::appendAllPages(PagedVector &other) {
  NES_ASSERT2_FMT(
      ...
      "Can not combine PagedVector of different pageSizes for now!");
  NES_ASSERT2_FMT(
      entrySize == other.entrySize,
      "Can not combine PagedVector of different entrySize for now!");

  for (auto otherPos = 0_u64; otherPos < other.totalNumberOfEntries;
       ++otherPos) {
