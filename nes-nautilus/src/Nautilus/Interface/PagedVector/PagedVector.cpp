...
NAUT_INLINE const TupleBuffer* PagedVector::getTupleBufferForEntry(const uint64_t entryPos) const
{
...
NAUT_INLINE std::optional<uint64_t> PagedVector::getBufferPosForEntry(const uint64_t entryPos) const
{
    /// We need to find the index / page that the entryPos belongs to.
...
NAUT_INLINE std::optional<size_t> PagedVector::PagesWrapper::findIdx(const uint64_t entryPos) const
{
...
NAUT_INLINE uint64_t PagedVector::PagesWrapper::getTotalNumberOfEntries() const
{
