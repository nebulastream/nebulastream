...
namespace NES
{
static uint64_t getTotalNumberOfRecordsProxy(const TupleBuffer* pagedVectorBuffer)
{
    const PagedVector pagedVector = PagedVector::load(*pagedVectorBuffer);
    return pagedVector.getTotalNumberOfRecords();
}

static uint64_t getBufferPosForEntryProxy(const TupleBuffer* pagedVectorBuffer, const uint64_t entryPos)
{
    const PagedVector pagedVector = PagedVector::load(*pagedVectorBuffer);
...
{
    return [pageBuffer](const nautilus::val<int8_t*>& fieldSlot) /// NOLINT(clang-analyzer-cplusplus.NewDeleteLeaks)
        ...
        return invoke( /// NOLINT(clang-analyzer-cplusplus.NewDeleteLeaks)
            +[](TupleBuffer* pageBuffer, AbstractBufferProvider* bufferProvider, int8_t* fieldSlot, uint64_t allocationSize) -> int8_t*
            {
                ...
                *reinterpret_cast<VariableSizedAccess*>(fieldSlot)
                ...
                return reinterpret_cast<int8_t*>( /// NOLINT(clang-analyzer-cplusplus.NewDeleteLeaks)
                    newVarSizedBuffer.getAvailableMemoryArea<>().data());
            },
...
std::vector<Record::RecordFieldIdentifier> BasicTupleLayout::getAllFieldNames() const
{
...
void BasicTupleLayout::writeRecord(const Record& record, nautilus::val<std::int8_t*> memoryForRecord, AllocateFunction allocateVarSized)
{
    const auto& fields = schema.getFields();
    const auto numFields = schema.getNumberOfFields();
    uint64_t fieldOffset = 0;
...
Record PagedVectorRef::at(const nautilus::val<uint64_t>& entryPos) const
{
    ...
    invoke(
        +[](TupleBuffer* pagedVectorBuffer, const uint64_t entryPos, TupleBuffer* pageBuffer)
        {
            ...
            /// load page
            const VariableSizedAccess::Index childBufferIndex{pageBufferIndex};
...
static void loadChildBuffer(TupleBuffer* pagedVectorBuffer, uint64_t entryPos, TupleBuffer* currPage)
{
    const PagedVector pagedVector = PagedVector::load(*pagedVectorBuffer);
    const auto pageBufferIndex = pagedVector.getPageIndex(entryPos);
    const VariableSizedAccess::Index childBufferIndex{pageBufferIndex};
...
PagedVectorRefIter::PagedVectorRefIter(
    ...
    : pagedVector(std::move(pagedVector)), pos(pos), posOnPage(posOnPage), curPage(curPage), tupleLayout(tupleLayout)
{
}
