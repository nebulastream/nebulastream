...
#include "../../../../include/Nautilus/Util.hpp"
...
const Memory::TupleBuffer* createNewEntryProxy(PagedVector* pagedVector)
{
...
PagedVectorRef::PagedVectorRef(
    ...
    : pagedVectorRef(pagedVectorRef), memoryProvider(std::move(memoryProvider))
{
}

void PagedVectorRef::writeRecord(const Record& record) const
{
    auto recordBuffer = RecordBuffer(invoke(createNewEntryProxy, pagedVectorRef));
...
PagedVectorRefIter::PagedVectorRefIter(
    const PagedVectorRef& pagedVector, const std::vector<Record::RecordFieldIdentifier>& projections, const nautilus::val<uint64_t>& pos)
    : pagedVector(pagedVector), projections(projections), pos(pos)
{
}
