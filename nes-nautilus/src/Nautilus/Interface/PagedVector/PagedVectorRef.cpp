...
PagedVectorRef::PagedVectorRef(const Value<MemRef>& pagedVectorRef,
                               uint64_t entrySize)
...
Value<UInt64> PagedVectorRef::getTotalNumberOfEntries() {
  return getMember(pagedVectorRef, PagedVector, totalNumberOfEntries)
      .load<UInt64>();
}
...
Value<MemRef> PagedVectorRef::getEntry(const Value<UInt64>& pos) {
  ...
  auto page =
      Nautilus::FunctionCall("getPagedVectorPageProxy", getPagedVectorPageProxy,
                             pagedVectorRef, Value<UInt64>(pagePos));
