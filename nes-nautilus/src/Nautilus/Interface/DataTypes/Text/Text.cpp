...
Text::Text(TypedRef<NES::Nautilus::TextValue> rawReference)
    : Any(&type), rawReference(rawReference){};

bool textEquals(const TextValue *leftText, const TextValue *rightText) {
  NES_DEBUG("Check if {} and {} are equal", leftText->c_str(),
            rightText->c_str())
  if (leftText->length() != rightText->length()) {
    return false;
  } else if (std::memcmp(leftText->c_str(), rightText->c_str(),
                         leftText->length()) == 0) {
    return true;
  }
  return false;
...
Value<Boolean> Text::equals(const Value<Text> &other) const {
  return FunctionCall<>("textEquals", textEquals, rawReference,
                        other.value->rawReference);
}
...
Value<Text> Text::substring(Value<UInt32> index, Value<UInt32> len) const {
  return FunctionCall<>("textSubstring", textSubstring, rawReference, index,
                        len);
}
...
Value<Text> Text::concat(const Value<Text> &other) const {
  return FunctionCall<>("textConcat", textConcat, rawReference,
                        other.value->rawReference);
}
...
bool textPrefix(const TextValue *leftText, const TextValue *rightText) {
  ...
  return std::memcmp(leftText->c_str(), rightText->c_str(),
                     rightText->length()) == 0;
}

Value<Boolean> Text::prefix(const Value<Text> &other) const {
  return FunctionCall<>("textPrefix", textPrefix, rawReference,
                        other.value->rawReference);
}
...
TextValue *textRepeat(const TextValue *text, uint32_t n) {
  if (n <= 0) {
    NES_THROW_RUNTIME_ERROR(
        "repetitions require a positive int, but received n<=0");
  }
...
Value<Text> Text::reverse() const {
  return FunctionCall<>("textReverse", textReverse, rawReference);
}
...
uint32_t textPosition(const TextValue *text, const TextValue *target) {
  ...
  if (text->length() < target->length()) {
    NES_THROW_RUNTIME_ERROR(
        "TargetText length is longer than sourceText length");
  }
  for (uint32_t i = 0; i < text->length(); i++) {
    if (text->c_str()[i] == target->c_str()[0] &&
...
Value<UInt32> Text::position(Value<Text> &other) const {
  return FunctionCall<>("textPosition", textPosition, rawReference,
                        other.value->rawReference);
}

TextValue *textReplace(const TextValue *text, const TextValue *source,
  ...
  if (textPosition(resultText, source) != 0) {
    ...
    // The text value is manually allocated with create. So we have to free it
    // manually.
    tmpText->~TextValue();
  }
  return resultText;
...
Value<Text> Text::replace(Value<Text> &source, Value<Text> &target) const {
  return FunctionCall<>("textReplace", textReplace, rawReference,
                        source.value->rawReference, target.value->rawReference);
}

uint32_t TextGetLength(const TextValue *text) { return text->length(); }

const Value<UInt32> Text::length() const {
  return FunctionCall<>("textGetLength", TextGetLength, rawReference);
}

AnyPtr Text::copy() { return std::make_shared<Text>(rawReference); }

int8_t readTextIndex(const TextValue *text, uint32_t index) {
  return text->c_str()[index];
}

void writeTextIndex(TextValue *text, uint32_t index, int8_t value) {
  text->str()[index] = value;
}

Value<Int8> Text::read(Value<UInt32> index) {
  return FunctionCall<>("readTextIndex", readTextIndex, rawReference, index);
}
...
uint32_t getBitLength(const TextValue *text) {
  return (text->length() * 8_u32);
}

const Value<UInt32> Text::bitLength() const {
  return FunctionCall<>("getBitLength", getBitLength, rawReference);
}
...
const Value<Text> Text::lower() const {
  return FunctionCall<>("lowercaseText", lowercaseText, rawReference);
}
...
const Value<Text> Text::right(Value<UInt32> index) {
  return FunctionCall<>("rightCharacters", rightCharacters, rawReference,
                        index);
}

TextValue *rightPad(const TextValue *text, uint32_t targetLength,
                    int8_t value) {
...
TextValue *leftTrim(const TextValue *text, const TextValue *target) {
  ...
  std::memcpy(resultText->str(), text->c_str() + spaceCount,
              text->length() - spaceCount);
  return resultText;
}

const Value<Text> Text::ltrim(Value<Text> &other) const {
  return FunctionCall<>("leftTrim", leftTrim, rawReference,
                        other.value->rawReference);
}
...
const Value<Text> Text::rtrim(Value<Text> &other) const {
  return FunctionCall<>("rightTrim", rightTrim, rawReference,
                        other.value->rawReference);
}
...
const Value<Boolean> Text::similarTo(Value<Text> &pattern) const {
  return FunctionCall<>("textSimilarTo", textSimilarTo, rawReference,
                        pattern.value->rawReference);
}

bool textLike(const TextValue *text, TextValue *inputPattern,
              Boolean caseSensitive) {
  auto pattern = std::string(inputPattern->c_str(), inputPattern->length());
  NES_DEBUG("Checking in textLike if {} and {} are a match.", text->c_str(),
            pattern);
...
const Value<Boolean> Text::like(Value<Text> &pattern,
                                Value<Boolean> caseSensitive) const {
  return FunctionCall<>("textLike", textLike, rawReference,
                        pattern.value->rawReference, caseSensitive);
}

const Value<Text> Text::trim() const {
  return FunctionCall<>("lrTrim", lrTrim, rawReference);
}
