...
uint32_t TextGetLength(const TextValue* text)
{
    return text->length();
}
...
AnyPtr Text::copy()
{
    return std::make_shared<Text>(rawReference);
}
...
IR::Types::StampPtr Text::getType() const
{
    return rawReference.getType();
}
const TypedRef<TextValue>& Text::getReference() const
{
    return rawReference;
}
