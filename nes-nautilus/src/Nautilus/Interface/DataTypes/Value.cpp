...
Nautilus::Tracing::ValueRef
createNextValueReference(Nautilus::IR::Types::StampPtr &&stamp) {
...
std::optional<Value<>> CastToOp(const Value<> &left,
                                const TypeIdentifier *toType) {
  ...
  for (auto &plugin : plugins) {
    if (plugin->IsCastable(left, toType)) {
      ...
      if (auto ctx = Tracing::TraceContext::get()) {
        ctx->traceUnaryOperation(Nautilus::Tracing::OpCode::CAST, left.ref,
                                 castedValue.ref);
      }
      return castedValue;
    }
...
template <> Value<Any> Value<Any>::castTo(const TypeIdentifier *toStamp) const {
  return CastToOp(*this, toStamp).value();
}

const TraceableType &TraceableType::asTraceableType(const Any &val) {
  return static_cast<const TraceableType &>(val);
}

Value<> evalBinary(const Value<> &left, const Value<> &right,
                   const std::function<std::optional<Value<>>(
                       std::unique_ptr<InvocationPlugin> &plugin,
                       const Value<> &left, const Value<> &right)> &function) {
  auto &plugins = InvocationPluginRegistry::getPlugins();
  for (auto &plugin : plugins) {
  ...
  NES_THROW_RUNTIME_ERROR(
      "No plugin registered that can handle this operation between "
      << left->toString() << " and " << right->toString());
}

Value<>
evalWithCast(const Value<> &left, const Value<> &right,
             const std::function<std::optional<Value<>>(
                 std::unique_ptr<InvocationPlugin> &plugin, const Value<> &left,
                 const Value<> &right)> &function) {
...
Value<> AddOp(const Value<> &left, const Value<> &right) {
  return evalWithCast(left, right,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                         const Value<> &left, const Value<> &right) {
                        auto result = plugin->Add(left, right);
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::ADD,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
}
...
Value<> BitWiseAndOp(const Value<> &leftExp, const Value<> &rightExp) {
  return evalWithCast(leftExp, rightExp,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                         const Value<> &left, const Value<> &right) {
                        auto result = plugin->BitWiseAnd(left, right);
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::BITWISE_AND,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> BitWiseOrOp(const Value<> &leftExp, const Value<> &rightExp) {
  return evalWithCast(leftExp, rightExp,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::BITWISE_OR,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> BitWiseXorOp(const Value<> &leftExp, const Value<> &rightExp) {
  return evalWithCast(leftExp, rightExp,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::BITWISE_XOR,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> BitWiseLeftShiftOp(const Value<> &leftExp, const Value<> &rightExp) {
  return evalWithCast(leftExp, rightExp,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::BITWISE_LEFT_SHIFT,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> BitWiseRightShiftOp(const Value<> &leftExp, const Value<> &rightExp) {
  return evalWithCast(leftExp, rightExp,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::BITWISE_RIGHT_SHIFT,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> EqualsOp(const Value<> &left, const Value<> &right) {
  return evalWithCast(left, right,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value() && left.isTracableType() &&
                            right.isTracableType()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::EQUALS,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> LessThanOp(const Value<> &left, const Value<> &right) {
  return evalWithCast(left, right,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::LESS_THAN,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> GreaterThanOp(const Value<> &left, const Value<> &right) {
  return evalWithCast(left, right,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::GREATER_THAN,
                              result.value().ref, left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> OrOp(const Value<> &left, const Value<> &right) {
  return evalWithCast(left, right,
                      [](std::unique_ptr<InvocationPlugin> &plugin,
                        ...
                        if (result.has_value()) {
                          Tracing::TraceUtil::traceBinaryOperation(
                              Nautilus::Tracing::OpCode::OR, result.value().ref,
                              left.ref, right.ref);
                        }
                        return result;
                      });
...
Value<> NegateOp(const Value<> &input) {
  ...
  for (auto &plugin : plugins) {
    ...
    if (result.has_value()) {
      Tracing::TraceUtil::traceUnaryOperation(Nautilus::Tracing::OpCode::NEGATE,
                                              result->ref, input.ref);
      return result.value();
    }
  };
  NES_THROW_RUNTIME_ERROR(
      "No plugin registered that can handle this operation");
}
...
template <> template <> Value<Text>::Value(const char *s) {
  std::string string(s);
...
template <> template <typename T, typename> Value<>::Value(const char *s) {
  std::string string(s);
  auto text = TextValue::create(string);
  auto textRef = TypedRef<TextValue>(text);
  this->value = std::make_shared<Text>(textRef);
