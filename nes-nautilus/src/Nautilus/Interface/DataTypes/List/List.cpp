...
template <typename T> auto transformReturnValues(ListValue<T> *value) {
...
template <typename T>
  requires std::is_base_of<BaseListValue,
...
List::ListValueIterator::ListValueIterator(List &listRef,
...
bool List::ListValueIterator::operator==(const ListValueIterator &other) const {
...
TypedList<BaseType>::TypedList(TypedRef<RawType> ref)
    : List(&type), rawReference(ref) {}
...
Value<Boolean> TypedList<BaseType>::equals(const Value<List> &otherList) {
  ...
  return FunctionCall("listEquals", listEquals<ComponentType>, rawReference,
                      value->rawReference);
}
...
template <IsListComponentType BaseType> AnyPtr TypedList<BaseType>::copy() {
  return std::make_shared<TypedList<BaseType>>(rawReference);
}

template <class T> T readListIndex(const ListValue<T> *list, int32_t index) {
  return list->c_data()[index];
}

template <class T> ListValue<T> *reverseList(const ListValue<T> *i) {
  return ListValue<T>::create(i->length());
}

template <IsListComponentType BaseType>
Value<> TypedList<BaseType>::read(Value<UInt32> &index) const {
  return FunctionCall("readListIndex", readListIndex<ComponentType>,
                      rawReference, index);
}
...
void TypedList<BaseType>::write(Value<UInt32> &index, const Value<> &value) {
  ...
  FunctionCall("writeListIndex", writeListIndex<ComponentType>, rawReference,
               index, baseValue);
}
