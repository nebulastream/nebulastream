...
template <typename T> ListValue<T> *ListValue<T>::create(uint32_t size) {
  auto *provider = Runtime::WorkerContext::getBufferProviderTLS();
  auto optBuffer =
      provider->getUnpooledBuffer(size * FIELD_SIZE + DATA_FIELD_OFFSET);
...
template <typename T> ListValue<T>::ListValue(uint32_t size) : size(size) {}

template <typename T> T *ListValue<T>::data() {
  return reinterpret_cast<T *>(this + DATA_FIELD_OFFSET);
}

template <typename T> const T *ListValue<T>::c_data() const {
  return reinterpret_cast<const T *>(this + DATA_FIELD_OFFSET);
}

template <typename T> uint32_t ListValue<T>::length() const { return size; }
...
template <class T> bool ListValue<T>::equals(const ListValue<T> *other) const {
...
template <typename T> ListValue<T>::~ListValue() {
...
ListValue<T> *ListValue<T>::concat(const ListValue<T> *other) const {
  ...
  // create the result list, which is the current list length + the other list
  // length
  ...
  std::copy(other->c_data(), other->c_data() + rightListSize,
            resultList->data() + leftListSize);
  return resultList;
}

template <class T> ListValue<T> *ListValue<T>::append(T element) const {
  // create result list
...
template <class T> ListValue<T> *ListValue<T>::prepend(T element) const {
  // create result list
  auto resultList = ListValue<T>::create(length() + 1);
  auto resultDataPtr = resultList->data();
  // copy content
...
template <class T> bool ListValue<T>::contains(T element) const {
...
template <class T> int32_t ListValue<T>::listPosition(T element) const {
  auto leftListSize = length();
  for (uint32_t i = 0; i < length(); i++) {
    if (c_data()[i] == element) {
      return i;
...
template <class T> ListValue<T> *ListValue<T>::sort() const {
  // create copy of list
...
template <class T> ListValue<T> *ListValue<T>::revers() const {
