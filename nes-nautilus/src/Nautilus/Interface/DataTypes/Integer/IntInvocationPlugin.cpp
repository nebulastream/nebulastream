...
class IntInvocationPlugin : public InvocationPlugin {
 public:
  ...
  std::optional<Value<>> performBinaryOperationAndCast(
      ...
      std::function<Value<>(const Int& left, const Int& right)> function)
      const {
    ...
    if (Int::isInteger(leftValue) && Int::isInteger(rightValue)) {
      if (leftValue.getTypeIdentifier() != rightValue.getTypeIdentifier()) {
        NES_THROW_RUNTIME_ERROR(
            "Implicit casts between different are not supported.");
      }
  ...
  std::optional<Value<>> Add(const Value<>& left,
    ...
    return performBinaryOperationAndCast(left, right,
    ...
    return performBinaryOperationAndCast(left, right,
                                         [](const Int& left, const Int& right) {
                                           if (Tracing::TraceUtil::inTracer()) {
                                             // FIXME avoid division in tracing.
                                             // For now we just substitute with
                                             // an add.
                                             auto result = left.add(right);
                                             return Value<>(std::move(result));
                                           } else {
  ...
  std::optional<Value<>> Mod(const Value<>& left,
                             const Value<>& right) const override {
    return performBinaryOperationAndCast(left, right,
                                         [](const Int& left, const Int& right) {
                                           if (Tracing::TraceUtil::inTracer()) {
                                             // FIXME avoid modulo in tracing.
                                             // For now we just substitute with
                                             // an add.
                                             auto result = left.add(right);
                                             return Value<>(std::move(result));
  ...
  std::optional<Value<>> GreaterThan(const Value<>& left,
    ...
    return performBinaryOperationAndCast(
        left, right, [](const Int& left, const Int& right) {
          auto result = left.greaterThan(right);
          return Value<>(std::move(result));
        });
  }

  std::optional<Value<>> BitWiseAnd(const Value<>& left,
                                    const Value<>& right) const override {
    return performBinaryOperationAndCast(left, right,
                                         [](const Int& left, const Int& right) {
  ...
  std::optional<Value<>> BitWiseOr(const Value<>& left,
                                   const Value<>& right) const override {
    return performBinaryOperationAndCast(left, right,
                                         [](const Int& left, const Int& right) {
  ...
  std::optional<Value<>> BitWiseXor(const Value<>& left,
                                    const Value<>& right) const override {
    return performBinaryOperationAndCast(left, right,
                                         [](const Int& left, const Int& right) {
  ...
  std::optional<Value<>> BitWiseLeftShift(const Value<>& left,
                                          const Value<>& right) const override {
    return performBinaryOperationAndCast(
        left, right, [](const Int& left, const Int& right) {
  ...
  std::optional<Value<>> BitWiseRightShift(
      const Value<>& left, const Value<>& right) const override {
    return performBinaryOperationAndCast(
        left, right, [](const Int& left, const Int& right) {
  ...
  std::optional<Value<>> CastTo(
      const Value<>& inputValue,
      const TypeIdentifier* targetType) const override {
  ...
  bool IsCastable(const Value<>& value,
    ...
    if (isa<Int8>(value.getValue()) &&
        (targetType->isType<Int16>() || targetType->isType<Int32>() ||
         targetType->isType<Int64>() || targetType->isType<UInt8>() ||
         targetType->isType<UInt16>() || targetType->isType<UInt32>() ||
         targetType->isType<UInt64>())) {
      // int8 can be casted to Int16-Int64 and UInt8-UInt64
      return true;
    } else if (isa<Int16>(value.getValue()) &&
               (targetType->isType<Int32>() || targetType->isType<Int64>() ||
                targetType->isType<UInt16>() || targetType->isType<UInt32>() ||
                targetType->isType<UInt64>())) {
      // int16 can be casted to Int32-Int64 and UInt16-UInt64
      return true;
    } else if (isa<Int32>(value.getValue()) &&
               (targetType->isType<Int64>() || targetType->isType<UInt32>() ||
                targetType->isType<UInt64>())) {
    ...
    if (isa<UInt8>(value.getValue()) &&
        (targetType->isType<Int16>() || targetType->isType<Int32>() ||
         targetType->isType<Int64>() || targetType->isType<UInt16>() ||
      ...
      return true;
    } else if (isa<UInt32>(value.getValue()) &&
               (targetType->isType<Int64>() || targetType->isType<UInt64>())) {
...
[[maybe_unused]] static InvocationPluginRegistry::Add<IntInvocationPlugin>
    intPlugin;
}  // namespace NES::Nautilus
