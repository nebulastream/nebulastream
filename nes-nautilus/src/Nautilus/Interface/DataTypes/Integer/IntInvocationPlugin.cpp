...
public:
    ...
    std::optional<Value<>> performBinaryOperationAndCast(
        const Value<>& left, const Value<>& right, std::function<Value<>(const Int& left, const Int& right)> function) const
    {
...
[[maybe_unused]] std::unique_ptr<InvocationPlugin> RegisterIntInvocationPlugin()
{
    return std::make_unique<IntInvocationPlugin>();
}
} /// namespace NES::Nautilus
