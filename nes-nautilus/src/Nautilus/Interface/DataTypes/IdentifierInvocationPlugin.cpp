...
public:
  ...
  std::optional<Value<>> performBinaryOperationAndCast(
      ...
      std::function<Value<>(const Identifier &left, const Identifier &right)>
          function) const {

    if (!Identifier::isIdentifier(left.getValue()) &&
        !Identifier::isIdentifier(right.getValue())) {
      return std::nullopt;
    }

    if ((Identifier::isIdentifier(left.getValue()) &&
         !Identifier::isIdentifier(right.getValue())) ||
        !(Identifier::isIdentifier(left.getValue()) &&
          Identifier::isIdentifier(right.getValue()))) {
      NES_ERROR(
          "Comparison between Identifier and Non-Identifier is not allowed");
      return std::nullopt;
    }

    if (left->getTypeIdentifier()->getTypeInfo() !=
        right->getTypeIdentifier()->getTypeInfo()) {
  ...
  std::optional<Value<>> Equals(const Value<> &left,
    ...
    return performBinaryOperationAndCast(
        left, right, [](const Identifier &left, const Identifier &right) {
