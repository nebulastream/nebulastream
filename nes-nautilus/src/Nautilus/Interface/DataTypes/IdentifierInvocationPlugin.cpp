...
public:
    ...
    std::optional<Value<>> performBinaryOperationAndCast(
        const Value<>& left, const Value<>& right, std::function<Value<>(const Identifier& left, const Identifier& right)> function) const
    {
        ...
        {
            NES_ERROR(
                "Comparison between different Identifier Types: {} and {}",
    ...
    std::optional<Value<>> Equals(const Value<>& left, const Value<>& right) const override
    {
        return performBinaryOperationAndCast(
            left,
            right,
            [](const Identifier& left, const Identifier& right)
            {
