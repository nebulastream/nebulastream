...
uint64_t stringtomillisecondsproxy(TextValue* t)
{
    ...
    if (timeString.find('T') != std::string::npos)
    {
        ...
        NES_DEBUG(
            " the year {} the month {} the day {} the hour {} the minute {} and the second {}", year, month, day, hour, minute, second);
    }
    else
    {
...
AnyPtr TimeStamp::copy()
{
    return create<TimeStamp>(milliseconds);
}
...
{
    return FunctionCall<>("centuryProxy", centuryProxy, milliseconds.as<UInt64>());
}
...
{
    return (FunctionCall<>("intervalproxy", intervalproxy, milliseconds.as<Int64>()));
}
...
Value<> TimeStamp::age()
{
    return FunctionCall<>("agecurrentTime", agecurrentTime, milliseconds.as<UInt64>());
}
...
{
    return FunctionCall<>("getSeconds", getSecondsChrono, milliseconds.as<UInt64>());
}
...
{
    return FunctionCall<>("getMinutes", getMinutesChrono, milliseconds.as<UInt64>());
}
...
{
    return FunctionCall<>("getMonths", getMonthsChrono, milliseconds.as<UInt64>());
}
...
{
    return FunctionCall<>("getNameOfMonth", getMonthNameProxy, milliseconds.as<Int64>());
}
