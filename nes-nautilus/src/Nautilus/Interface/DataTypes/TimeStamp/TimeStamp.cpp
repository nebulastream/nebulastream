...
// Convert a string in the format "YYYY-MM-DDTHH:MM:SS" to milliseconds since
// the year 0 (UTC (Universal Time Coordinated)). //1970-01-07T14:23:27
uint64_t stringtomillisecondsproxy(TextValue* t) {
  ...
  if (timeString.find('T') != std::string::npos) {
    std::sscanf(t->c_str(), "%d-%d-%dT%d:%d:%d", &year, &month, &day, &hour,
                &minute, &second);
    NES_DEBUG(
        " the year {} the month {} the day {} the hour {} the minute {} and "
        "the second {}",
  ...
  // TODO: Currently, we only support this format in a rather naive way, with
  // issue #3616 we want to enhance the format, e.g., by
  //  distinguishing Date and Time input strings or support user-defined
  //  formats.
  ...
  // has we current only except time formats with hour, minutes and seconds, we
  // need to multiple by 1000 to represent milliseconds
...
TimeStamp::TimeStamp(Value<> x) : Any(&type), milliseconds(x) {
  if (x->isType<UInt64>()) {
    ...
    this->milliseconds =
        FunctionCall<>("stringtomillisecondsproxy", stringtomillisecondsproxy,
                       x.as<Text>().value->getReference());
  } else {
...
Value<UInt64> TimeStamp::stringtomilliseconds(Value<Text> textValue) {
  return FunctionCall<>("stringtomillisecondsproxy", stringtomillisecondsproxy,
                        textValue->getReference());
}

AnyPtr TimeStamp::copy() { return create<TimeStamp>(milliseconds); }
AnyPtr TimeStamp::add(const TimeStamp& other) const {
  return create<TimeStamp>(milliseconds + other.milliseconds);
}
AnyPtr TimeStamp::substract(const TimeStamp& other) const {
  return create<TimeStamp>(milliseconds - other.milliseconds);
}
...
std::shared_ptr<Boolean> TimeStamp::lessThan(
    const TimeStamp& otherValue) const {
  return create<Boolean>(milliseconds < otherValue.milliseconds);
}
std::shared_ptr<Boolean> TimeStamp::greaterThan(
    const TimeStamp& otherValue) const {
  return create<Boolean>(milliseconds > otherValue.milliseconds);
}
...
Value<> TimeStamp::century() {
  return FunctionCall<>("centuryProxy", centuryProxy,
                        milliseconds.as<UInt64>());
}
...
Value<Text> TimeStamp::interval() {
  return (
      FunctionCall<>("intervalproxy", intervalproxy, milliseconds.as<Int64>()));
}
...
TextValue* weekdayNameproxy(int64_t milliseconds) {
  ...
  const std::string weekdayNames[] = {"Sunday",    "Monday",   "Tuesday",
...
Value<Text> TimeStamp::getWeekdayName() {
  return (FunctionCall<>("weekdayNameproxy", weekdayNameproxy,
...
Value<> TimeStamp::age() {
  return FunctionCall<>("agecurrentTime", agecurrentTime,
                        milliseconds.as<UInt64>());
}
...
Nautilus::IR::Types::StampPtr TimeStamp::getType() const {
...
Value<> TimeStamp::getSeconds() {
  return FunctionCall<>("getSeconds", getSecondsChrono,
                        milliseconds.as<UInt64>());
}
...
Value<> TimeStamp::getMinutes() {
  return FunctionCall<>("getMinutes", getMinutesChrono,
                        milliseconds.as<UInt64>());
}
...
Value<> TimeStamp::getHours() {
  return FunctionCall<>("getHours", getHoursChrono, milliseconds.as<UInt64>());
}
int64_t getDaysChrono(int64_t milliseconds) {
  tm utc_t = convertToUTC_TM(milliseconds);
  return utc_t.tm_mday;
}
Value<> TimeStamp::getDays() {
  return FunctionCall<>("getDays", getDaysChrono, milliseconds.as<UInt64>());
}
int64_t getMonthsChrono(int64_t milliseconds) {
  tm utc_t = convertToUTC_TM(milliseconds);
  return utc_t.tm_mon + 1;
}
Value<> TimeStamp::getMonths() {
  return FunctionCall<>("getMonths", getMonthsChrono,
                        milliseconds.as<UInt64>());
}
...
Value<> TimeStamp::getYears() {
  return FunctionCall<>("getYears", getYearsChrono, milliseconds.as<UInt64>());
}
...
TextValue* getMonthNameProxy(int64_t milliseconds) {
  ...
  switch (month) {
    case 1:
      return TextValue::create("January");
    case 2:
      return TextValue::create("February");
    case 3:
      return TextValue::create("March");
    case 4:
      return TextValue::create("April");
    case 5:
      return TextValue::create("May");
    case 6:
      return TextValue::create("June");
    case 7:
      return TextValue::create("July");
    case 8:
      return TextValue::create("August");
    case 9:
      return TextValue::create("September");
    case 10:
      return TextValue::create("October");
    case 11:
      return TextValue::create("November");
    case 12:
      return TextValue::create("December");
    default:
      NES_THROW_RUNTIME_ERROR("Invalid Month");
  }
}

Value<Text> TimeStamp::getMonthName() {
  return FunctionCall<>("getNameOfMonth", getMonthNameProxy,
                        milliseconds.as<Int64>());
}
...
std::string getStringFromMillis(int64_t milliseconds) {
  ...
  time_point milliseconds_timepoint{
...
std::string TimeStamp::toString() {
  Value<Text> result =
      FunctionCall<>("intervalproxy", intervalproxy, milliseconds.as<UInt64>());
