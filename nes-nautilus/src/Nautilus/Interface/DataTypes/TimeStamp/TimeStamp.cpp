...
uint64_t stringtomillisecondsproxy(TextValue *t) {
  ...
  if (timeString.find('T') != std::string::npos) {
    std::sscanf(t->c_str(), "%d-%d-%dT%d:%d:%d", &year, &month, &day, &hour,
                &minute, &second);
    NES_DEBUG(" the year {} the month {} the day {} the hour {} the minute {} "
              "and the second {}",
...
TimeStamp::lessThan(const TimeStamp &otherValue) const {
  return create<Boolean>(milliseconds < otherValue.milliseconds);
}
std::shared_ptr<Boolean>
TimeStamp::greaterThan(const TimeStamp &otherValue) const {
  return create<Boolean>(milliseconds > otherValue.milliseconds);
}
