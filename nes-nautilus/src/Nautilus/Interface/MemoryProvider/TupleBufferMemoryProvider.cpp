    ...
    const DataType& physicalType, const RecordBuffer& recordBuffer, const nautilus::val<int8_t*>& fieldReference)
{
    ...
    nautilus::val<VariableSizedAccess> combinedIdxOffset{Nautilus::Util::readValueFromMemRef<uint64_t>(fieldReference)};
    const auto varSizedPtr = invoke(
        +[](TupleBuffer* tupleBuffer, const VariableSizedAccess variableSizedAccess)
        {
    ...
    const nautilus::val<AbstractBufferProvider*>& bufferProvider)
{
    ...
    const auto variableSizedAccess = invoke(
        +[](TupleBuffer* tupleBuffer, AbstractBufferProvider* bufferProvider, const int8_t* varSizedPtr, const uint32_t varSizedValueLength)
        {
            ...
            const auto varSizedSpan = std::span{reinterpret_cast<const std::byte*>(varSizedPtr), varSizedValueLength};
            return MemoryLayout::storeAssociatedVarSizedValue(*tupleBuffer, *bufferProvider, varSizedSpan);
        },
        recordBuffer.getReference(),
        bufferProvider,
...
size_t TupleBufferMemoryProvider::getTupleSize() const
{
