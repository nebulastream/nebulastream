...
namespace NES
{
std::shared_ptr<Nautilus::Interface::BufferRef::TupleBufferRef>
LowerSchemaProvider::lowerSchema(const uint64_t bufferSize, const Schema& schema, const MemoryLayoutType layoutType)
{
    ...
    switch (layoutType)
    {
        case MemoryLayoutType::ROW_LAYOUT: {
            std::vector<Nautilus::Interface::BufferRef::RowTupleBufferRef::Field> fields;
            ...
            const auto tupleSize = std::accumulate(
                ...
                fields.end(),
                0UL,
                [](auto size, const Nautilus::Interface::BufferRef::RowTupleBufferRef::Field& field)
                { return size + field.type.getSizeInBytes(); });
            return std::make_shared<Nautilus::Interface::BufferRef::RowTupleBufferRef>(
                Nautilus::Interface::BufferRef::RowTupleBufferRef{std::move(fields), tupleSize, bufferSize});
        }

        case MemoryLayoutType::COLUMNAR_LAYOUT: {
            const uint64_t capacity = bufferSize / schema.getSizeOfSchemaInBytes();
            std::vector<Nautilus::Interface::BufferRef::ColumnTupleBufferRef::Field> fields;
            fields.reserve(schema.getNumberOfFields());
            ...
            const auto tupleSize = std::accumulate(
                ...
                fields.end(),
                0UL,
                [](auto size, const Nautilus::Interface::BufferRef::ColumnTupleBufferRef::Field& field)
                { return size + field.type.getSizeInBytes(); });

            return std::make_shared<Nautilus::Interface::BufferRef::ColumnTupleBufferRef>(
                Nautilus::Interface::BufferRef::ColumnTupleBufferRef{std::move(fields), tupleSize, bufferSize});
        }
    }
    std::unreachable();
}
