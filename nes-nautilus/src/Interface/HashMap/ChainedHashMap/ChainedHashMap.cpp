...
ChainedHashMap::init(TupleBuffer& tupleBuffer, const uint64_t entrySize, const uint64_t numberOfBuckets, const uint64_t pageSize)
{
    ...
    chainsArray[numberOfChains] = reinterpret_cast<ChainedHashMapEntry*>(&chainsArray[numberOfChains]);
...
void ChainedHashMap::printHeader() const
{
    ...
    std::cout << "Metadata:" << '\n';
    std::cout << "\t numBuckets: " << head.numBuckets << '\n';
    std::cout << "\t numChains: " << head.numChains << '\n';
    std::cout << "\t numTuples: " << head.numRecords << '\n';
    std::cout << "\t entriesPerPage: " << head.entriesPerPage << '\n';
    std::cout << "\t entrySize: " << head.entrySize << '\n';
    std::cout << "\t pageSize: " << head.pageSize << '\n';
    std::cout << "\t mask: " << head.mask << '\n';
    std::cout << "\t storageSpaceIndex: " << head.storageSpaceIndex << '\n';
    std::cout << "\t varSizedSpaceIndex: " << head.varSizedSpaceIndex << '\n';
}
...
void ChainedHashMap::printContentInfo() const
{
    ...
    if (head.storageSpaceIndex != TupleBuffer::INVALID_CHILD_BUFFER_INDEX_VALUE)
    {
        ...
        std::cout << "\t storageSpacePagesNum: " << storageSpace.getNumberOfChildBuffers() << '\n';
    }
    if (head.varSizedSpaceIndex != TupleBuffer::INVALID_CHILD_BUFFER_INDEX_VALUE)
    {
        ...
        std::cout << "\t varSizedSpacePagesNum: " << varSizedSpace.getNumberOfChildBuffers() << '\n';
    }
}
