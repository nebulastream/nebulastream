    ...
    if (!physicalType.isFixedSized())
    {
        switch (physicalType.type)
        {
            case DataType::Type::FIXEDSIZED: {
                ...
                for (nautilus::static_val<uint32_t> i = 0; i < physicalType.count; ++i)
                {
                    inlineVarsizedReferences(
                        ...
                        recordBuffer,
                        varValRef + nautilus::val<size_t>{i * physicalType.elementType[0].getSizeInBytesWithoutNull()});
                }
                return;
            }
            ...
            case DataType::Type::VECTOR: {
                auto vectorAccess = static_cast<nautilus::val<VariableSizedAccess*>>(varValRef);
                const auto vectorPtr = invoke(
                    {.modRefInfo = nautilus::ModRefInfo::Ref, .willReturn = true, .noUnwind = true},
                    +[](const TupleBuffer* tupleBuffer, const VariableSizedAccess* variableSizedAccessPtr)
                    {
                        INVARIANT(tupleBuffer != nullptr, "Tuplebuffer MUST NOT be null at this point");
                    ...
                    recordBuffer.getReference(),
                    vectorAccess);
                const nautilus::val<uint64_t> size = *getMemberWithOffset<uint64_t>(vectorAccess, offsetof(VariableSizedAccess, size));
                const VarVal vectorVal{VectorData(vectorPtr, physicalType.elementType[0], size), false, false};
                vectorVal.writeToMemory(varValRef);
                return;
            }
            default: {
...
TupleBufferRef::loadValue(const DataType& physicalType, const RecordBuffer& recordBuffer, const nautilus::val<int8_t*>& fieldReference)
{
    ...
    switch (physicalType.type)
    {
        ...
        case DataType::Type::VECTOR: {
            /// Vectors are stored identically to VARSIZED values
            auto vectorAccess = static_cast<nautilus::val<VariableSizedAccess*>>(varValRef);
            const auto vectorPtr = invoke(
                {.modRefInfo = nautilus::ModRefInfo::Ref, .willReturn = true, .noUnwind = true},
                +[](const TupleBuffer* tupleBuffer, const VariableSizedAccess* variableSizedAccessPtr)
                ...
                vectorAccess);
            const nautilus::val<uint64_t> size = *getMemberWithOffset<uint64_t>(vectorAccess, offsetof(VariableSizedAccess, size));
            return VarVal{VectorData(vectorPtr, physicalType.elementType[0], size), physicalType.nullable, null};
        }
        case DataType::Type::STRUCT: {
    ...
    const nautilus::val<AbstractBufferProvider*>& bufferProvider)
{
    ...
    if (physicalType.type == DataType::Type::FIXEDSIZED)
    {
        ...
        else
        {
            ...
            for (nautilus::static_val<size_t> i = 0; i < src.getNumElements(); ++i)
            {
                ...
                storeValue(physicalType.elementType[0], recordBuffer, elementReference, src.at(nautilus::val<uint64_t>{i}), bufferProvider);
            }
        }
        return value;
    }

    if (physicalType.type != DataType::Type::VARSIZED && physicalType.type != DataType::Type::VECTOR)
    {
    ...
    /// VARSIZED, VECTOR, and FIXEDSIZED both bottom out in `writeVarSized`: it copies the payload
    ...
    if (physicalType.type == DataType::Type::VECTOR)
    {
        const auto vectorData = value.getRawValueAs<VectorData>();
        payloadPtr = vectorData.getRawPtr();
        payloadLength = vectorData.getTotalSizeInBytes();
    }
    else
    {
