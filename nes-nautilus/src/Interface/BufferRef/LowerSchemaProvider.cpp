    ...
    const uint64_t bufferSize, const Schema<QualifiedUnboundField, Ordered>& schema, const OutputFormatterDescriptor& descriptor)
{
    ...
    const std::shared_ptr<OutputFormatter> outputFormatter = OutputFormatterProvider::provideOutputFormatter(
        descriptor.getOutputFormatterType().asCanonicalString(), fieldNames, descriptor);
