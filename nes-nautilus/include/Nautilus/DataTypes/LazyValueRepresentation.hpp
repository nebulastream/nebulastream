...
public:
    explicit LazyValueRepresentation(
        ...
        : size(size), ptrToLazyValue(reference), type(type), isNull(isNull), parserType(parserType)
    {
    }
...
protected:
    ...
    /// The input parser that has to be created if the lazy value is eventually parsed
