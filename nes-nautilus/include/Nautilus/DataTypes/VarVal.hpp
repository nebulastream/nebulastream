...
#define DEFINE_OPERATOR_VAR_VAL_BINARY(operatorName, op) \
    ...
    { \
        if (isNullable() and (this->isNull() or rhs.isNull())) \
        { \
            return std::visit( \
                []<typename RHS>(const RHS& rhsVal) { return VarVal{rhsVal, VarVal::NULLABLE_ENUM::NULLABLE, true}; }, this->value); \
        } \
        return std::visit( \
            ...
            { \
                ...
                { \
                    return VarVal{lhsVal op rhsVal, VarVal::joinNullable(this->nullable, rhs.nullable), false}; \
                } \
                else \
                { \
...
public:
    ...
    static NULLABLE_ENUM joinNullable(const NULLABLE_ENUM left, const NULLABLE_ENUM right)
    {
        return (left == NULLABLE_ENUM::NOT_NULLABLE and right == NULLABLE_ENUM::NOT_NULLABLE) ? NULLABLE_ENUM::NOT_NULLABLE
                                                                                              : NULLABLE_ENUM::NULLABLE;
    }
    ...
    VarVal customVisit(T t) const
    {
        if (isNullable() and isNull())
        {
            /// We decide to do nothing, if the value is null and simply return it to not call any function that would potentially throw
            /// an exception
            return value;
        }
...
protected:
    ...
    VarVal(const detail::var_val_t t, const NULLABLE_ENUM nullable = NULLABLE_ENUM::NOT_NULLABLE, nautilus::val<bool> null = false)
