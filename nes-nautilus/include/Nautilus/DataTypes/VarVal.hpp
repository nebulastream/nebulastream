...
public:
    ...
    template <typename T>
    explicit VarVal(const T value, const DataType::NULLABLE nullable, const nautilus::val<bool>& null)
    ...
    requires(detail::is_one_of<nautilus::val<T>, detail::var_val_t>::value)
        : value(nautilus::val<T>(value)), null(false), nullable(DataType::NULLABLE::NOT_NULLABLE)
    {
    }

    /// Construct via VarVal(nautilus::val<int>(32)), also allows conversion from static to dynamic
    template <typename T>
    VarVal(const nautilus::val<T> value, const DataType::NULLABLE nullable, const nautilus::val<bool>& null)
    requires(detail::is_one_of<nautilus::val<T>, detail::var_val_t>::value)
    ...
    requires(detail::is_one_of<nautilus::val<T>, detail::var_val_t>::value)
        : value(value), null(false), nullable(DataType::NULLABLE::NOT_NULLABLE)
    {
    }
    ...
    VarVal(const T value, const DataType::NULLABLE nullable, const nautilus::val<bool>& null) : value(value), null(null), nullable(nullable)
    {
    }

    template <typename T>
    requires(detail::is_one_of<T, detail::var_val_t>::value)
    VarVal(const T value) : value(value), null(false), nullable(DataType::NULLABLE::NOT_NULLABLE)
    {
    }
...
protected:
    ...
    /// NOLINTBEGIN(google-explicit-constructor)
    VarVal(detail::var_val_t t, const DataType::NULLABLE nullable, const nautilus::val<bool>& null)
        : value(std::move(t)), null(null), nullable(nullable)
    {
    }

    VarVal(detail::var_val_t t) : value(std::move(t)), null(false), nullable(DataType::NULLABLE::NOT_NULLABLE) { }

    /// NOLINTEND(google-explicit-constructor)
    detail::var_val_t value;
    nautilus::val<bool> null;
    DataType::NULLABLE nullable; /// Allows us to not run the null code parts, if the VarVal is not nullable
};
...
static_assert(
    std::is_constructible_v<VarVal, int32_t, DataType::NULLABLE, bool>,
    "Should be constructible from int32_t, VarVal::DataType::NULLABLE and bool");
static_assert(!std::is_constructible_v<VarVal, int32_t, bool>, "Should not be constructible from int32_t and bool");
static_assert(
    std::is_constructible_v<VarVal, VariableSizedData, DataType::NULLABLE, bool>,
    "Should be constructible from VariableSizedData, DataType::NULLABLE and bool");
static_assert(
    std::is_constructible_v<VarVal, nautilus::val<uint32_t>, DataType::NULLABLE, nautilus::val<bool>>,
...
static_assert(
    std::is_constructible_v<VarVal, VariableSizedData, DataType::NULLABLE, nautilus::val<bool>>,
