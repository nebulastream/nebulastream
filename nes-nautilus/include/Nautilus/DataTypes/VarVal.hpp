...
/// This requires its own design document.
class VarVal
{
public:
    ...
    T1 cast() const
    {
        ...
        NES_ERROR("Tried to cast to {} but was type {}", typeid(T1).name(), strType);
...
{
template <>
struct formatter<NES::Nautilus::VarVal> : formatter<std::string>
{
    auto format(const NES::Nautilus::VarVal& t, format_context& ctx) const -> decltype(ctx.out())
    {
        std::stringstream ss;
        ss << t;
