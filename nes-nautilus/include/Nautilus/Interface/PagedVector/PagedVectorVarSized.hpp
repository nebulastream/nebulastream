 ...
 * page_size/entry_size entries. Additionally to the fixed data types, this
 * PagedVector also supports variable sized data. Currently, similar constraints
 * apply to the variable sized data part as to the fixed data part. To know
 * where each variable sized data lies, we store the entryPtr, the entryLength
 * and the entryBufIdx in the map
 * varSizedDataEntryMap[varSizedDataEntryMapCounter] and increment for each new
 * record varSizedDataEntryMapCounter.
 *
 * There are already issues solving these shortcoming that will improve this
 * PagedVector implementation.
 * - #4639: Optimize appendAllPages()
 * - #4658: Add MemoryLayout class for PagedVectorVarSized to support different
 * layouts
 */
class PagedVectorVarSized {
public:
  ...
  /**
   * @brief Constructor. It calculates the entrySize and the capacityPerPage
   * based on the schema and the pageSize.
   * @param bufferManager
   * @param schema
   * @param pageSize
   */
  PagedVectorVarSized(Runtime::BufferManagerPtr bufferManager, SchemaPtr schema,
                      uint64_t pageSize = PAGE_SIZE);

  /**
   * @brief Appends a new page to the pages vector. It also sets the number of
   * tuples in the TupleBuffer to capacityPerPage and updates the
   * numberOfEntriesOnCurrPage.
   */
  void appendPage();

  /**
   * @brief Appends a new page to the varSizedDataPages vector. It also updates
   * the currVarSizedDataEntry pointer.
   */
  void appendVarSizedDataPage();

  /**
   * @brief Stores text of the given length in the varSizedDataPages. If the
   * current page is full, a new page is appended. It then creates a new entry
   * in the varSizedDataEntryMap with the given key and the pointer to the text
   * and its length.
   * @param text
   * @param length
   * @return uint64_t Returns the key of the new entry in the
   * varSizedDataEntryMap.
   */
  uint64_t storeText(const char *text, uint32_t length);

  /**
   * @brief Loads text from the varSizedDataPages by retrieving the pointer to
   * the text and its length from the varSizedDataEntryMap with the given key.
  ...
  /**
   * @brief Combines the pages of the given PagedVectorVarSized with the pages
   * of this PagedVectorVarSized.
