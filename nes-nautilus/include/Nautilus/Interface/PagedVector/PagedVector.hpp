...
/**
 * @brief This class provides a dynamically growing stack/list data structure of
 * entries. All data is stored in a list of pages. Entries consume a fixed size,
 * which has to be smaller then the page size. Each page can contain
 * page_size/entry_size entries.
...
class PagedVector {
 public:
   ...
   */
  PagedVector(std::unique_ptr<std::pmr::memory_resource> allocator,
              uint64_t entrySize, uint64_t pageSize = PAGE_SIZE);
  ...
  /**
   * @brief Appends a new page and updates the current page and number of
   * entries.
  ...
  /**
   * @brief Moves the entry from the oldPos to the newPos. This will overwrite
   * the data at the newPos
   * @param oldPos
   * @param newPos
   */
  ...
  /**
   * @brief Combines this PagedVector with another one by adding the other.pages
   * to these pages
   * @param other: PagedVector that contains pages, which should be added to
   * this one
   */
