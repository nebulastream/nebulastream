 ...
 * It also contains a bloom filter to have a quick check if a tuple is not on
 * the page
 */
class FixedPage {
 public:
   ...
   */
  explicit FixedPage(uint8_t* dataPtr, size_t sizeOfRecord,
  ...
  /**
   * @brief returns a pointer to a memory location on this page where to write
   * the record and checks if there is enough space for another record
   * @param hash
   * @return null pointer if there is no more space left on the page, otherwise
   * the pointer
   */
