 ...
 * It wraps a MemRef to the underling data structure and provides access
 * methods.
 */
class FixedPageRef {
 public:
...
class FixedPageRefIter {
 public:
  ...
  /**
   * @brief Dereference operator that points to a given entry in the
   * FixedPageRef
   * @return Value<MemRef>
   */
  ...
  /**
   * @brief Pre-increment operator that first increments and then returns the
   * reference
   * @return Reference to FixedPageRefIter
   */
  ...
  /**
   * @brief Post-increment count that first returns the reference and then
   * increments
   * @return FixedPageRefIter
   */
