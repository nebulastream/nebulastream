...
template <NESIdentifier T> auto transform(ValueId<T> argument) {
  return argument.value->getValue();
}

template <typename Arg> auto transform(Arg argument) {
...
template <typename Arg> Nautilus::Tracing::InputVariant getRefs(Arg &argument) {
...
template <NESIdentifier T> auto transformReturnValues(T returnValue) {
  return ValueId<T>(returnValue);
}

template <typename Arg> auto transformReturnValues(Arg argument) {
...
auto getArgumentReferences(std::string functionName, void *fnptr,
...
auto FunctionCall(std::string functionName, R (*fnptr)(FunctionArguments...),
                  ValueArguments... arguments) {
  if constexpr (std::is_void_v<R>) {
    if (Tracing::TraceUtil::inInterpreter()) {
      ...
      auto functionArgumentReferences =
          getArgumentReferences(functionName, (void *)fnptr, arguments...);
