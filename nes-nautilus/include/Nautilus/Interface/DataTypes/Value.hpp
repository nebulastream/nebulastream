...
template <typename P> struct ValueForPrimitive;

template <> struct ValueForPrimitive<int8_t> {
  using type = Int8;
};

template <> struct ValueForPrimitive<int16_t> {
  using type = Int16;
};

template <> struct ValueForPrimitive<int32_t> {
  using type = Int32;
};

template <> struct ValueForPrimitive<int64_t> {
  using type = Int64;
};

template <> struct ValueForPrimitive<uint8_t> {
  using type = UInt8;
};

template <> struct ValueForPrimitive<uint16_t> {
  using type = UInt16;
};

template <> struct ValueForPrimitive<uint32_t> {
  using type = UInt32;
};

template <> struct ValueForPrimitive<uint64_t> {
  using type = UInt64;
};

template <> struct ValueForPrimitive<float> {
  using type = Float;
};

template <> struct ValueForPrimitive<double> {
  using type = Double;
};

/**
 * @brief The Value class provides the elementary wrapper for any data value
 * that inherits from Any. Value provides operator overloading and integrates
 * with the tracing framework to track individual operations, e.g. ==, +, -.
 * @tparam ValueType
 */
template <class ValueType = Any> class Value : BaseValue {
...
public:
   ...
   */
  Value(float value) : Value(Any::create<Float>(value)) {
    Tracing::TraceUtil::traceConstOperation(this->value, this->ref);
  };
   ...
   */
  Value(double value) : Value(Any::create<Double>(value)) {
    Tracing::TraceUtil::traceConstOperation(this->value, this->ref);
  };

  template <typename T = ValueType,
            typename =
                std::enable_if_t<std::negation<std::is_same<T, MemRef>>::value>>
   ...
   */
  template <typename T = ValueType,
            typename = std::enable_if_t<std::is_same<T, MemRef>::value>>
  Value(std::int8_t *value) : Value(std::make_shared<MemRef>(value)) {
    Tracing::TraceUtil::traceConstOperation(this->value, this->ref);
  };

  Value(ValueType &&value)
      : value(std::make_shared<ValueType>(value)),
        ref(createNextValueReference(value.getType())){};

  /**
   * @brief copy constructor
   */
  Value(std::shared_ptr<ValueType> wrappedValue)
      : value(std::move(wrappedValue)),
        ref(createNextValueReference(value->getType())){};
  ...
  Value(OType wrappedValue)
      : value(std::make_shared<OType>(wrappedValue)),
        ref(createNextValueReference(value->getType())){};
  ...
  Value(std::shared_ptr<ValueType> wrappedValue,
      ...
      : value(std::move(wrappedValue)), ref(ref){};

  /**
   * @brief copy constructor
   */
  Value(const Value<ValueType> &other)
      : value(cast<ValueType>((other.value))), ref(other.ref) {}

  /**
   * @brief move constructor
   */
  template <class OType>
  Value(Value<OType> &&other) noexcept
      : value(std::exchange(other.value, nullptr)), ref(other.ref) {}
  ...
  Value &operator=(const Value &other) {
    return *this = Value<ValueType>(other);
  }
   ...
   * @return value
   */
  template <class OType> Value &operator=(Value<OType> &&other) noexcept {
   ...
   * @return
   */
  template <typename ResultType, typename T = ValueType,
            typename = std::enable_if_t<std::is_same<T, MemRef>::value>>
  auto load() {
    if (Tracing::TraceUtil::inTracer()) {
      if constexpr (std::is_same_v<ResultType, MemRef>) {
        ...
        Tracing::TraceUtil::traceUnaryOperation(Nautilus::Tracing::OpCode::LOAD,
                                                resultValue.ref, this->ref);
        return resultValue;
      } else {
   ...
   * @return void
   */
  template <typename InputValue, typename T = ValueType,
            typename = std::enable_if_t<std::is_same<T, MemRef>::value>>
  ...
  template <class T> ValueIndexReference inline operator[](Value<T> &index) {
    return ValueIndexReference(index, *this);
  };

  Value<> castTo(const TypeIdentifier *toStamp) const;

  template <class T> auto as() const { return Value<T>(cast<T>(value), ref); }

  /**
   * @brief Bool operator overload.
   * The result of this method is determined by the tracing component.
   * Symbolic tracing uses this method to manipulate the path through a
   * function. Otherwise returns the current boolean value if it is a boolean
   * @return bool.
   */
  operator bool() {
    return Tracing::TraceUtil::traceBoolOperation(value, ref);
  };
  ValueType *operator->() const { return value.get(); }
  ValueType &getValue() const { return *value.get(); };

  bool isTracableType() const {
    return std::dynamic_pointer_cast<TraceableType>(value) != nullptr;
  }
...
// Alias for Identifier Values. Otherwise, user need to type
// Value<Identifier<WorkerId>>
...
void WriteArrayIndexOp(const Value<> &input, Value<UInt32> &index,
                       const Value<> &value);
...
template <typename LHS>
typename Value<LHS>::ValueIndexReference &
Value<LHS>::ValueIndexReference::operator=(const Value<> &value) {
  auto idx = index->as<UInt32>();
...
template <typename T, typename = std::enable_if_t<
                          std::is_constructible_v<Value<>, std::decay_t<T>>>>
...
template <IsValueType LHS> auto inline operator++(const LHS &left) {
...
template <typename T> void PrintTo(const Value<T> &value, std::ostream *os) {
  *os << value.getValue().toString();
}

template <class ValueType>
typename Value<ValueType>::ValueIndexReference
Value<ValueType>::operator[](uint32_t index) {
