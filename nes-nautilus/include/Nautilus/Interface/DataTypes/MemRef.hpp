...
public:
  ...
  std::shared_ptr<Any> copy() override {
    return std::make_unique<MemRef>(this->value);
  }
  ...
  template <class ResultType> std::shared_ptr<ResultType> load() {
   ...
   * @param valueType
   */
  template <typename T> void store(Any &valueType) {
    *reinterpret_cast<typename T::RawType *>(value) =
        valueType.staticCast<T>().getValue();
  }
