 ...
 */
template <class T> class ListValue final : public BaseListValue {
public:
  using RawType = T;
  ...
  /**
   * @brief Destructor for the text value that also releases the underling tuple
   * buffer.
   */
  ~ListValue();
...
template <class T> uint32_t getLength(const ListValue<T> *list) {
  return list->length();
}
