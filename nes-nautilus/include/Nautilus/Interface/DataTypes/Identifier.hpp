...
{
    static_assert(sizeof(T) == 1 || sizeof(T) == 2 || sizeof(T) == 4 || sizeof(T) == 8, "Nautilus only handles BitWidth up to 64 Bits");
    switch (sizeof(T))
    {
        case 1:
...
public:
    ...
    {
        static_assert(std::integral<typename T::Underlying>, "Nautilus IdentifierType only supports integer based type identifier");

        if constexpr (std::is_unsigned_v<typename T::Underlying>)
        {
            return std::make_shared<IR::Types::IntegerStamp>(
                getBitWidth<typename T::Underlying>(), IR::Types::IntegerStamp::SignednessSemantics::Unsigned);
        }
        else
        {
            return std::make_shared<IR::Types::IntegerStamp>(
                getBitWidth<typename T::Underlying>(), IR::Types::IntegerStamp::SignednessSemantics::Signed);
        }
    }
