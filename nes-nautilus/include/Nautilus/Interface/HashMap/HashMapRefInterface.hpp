...
class HashMapRefInterface
{
public:
    /// Initializes the hash map reference with a hash function. We set the hash map reference and the buffer provider to nullptr for now.
    /// We set them during the nautilus open/execute method
    explicit HashMapRefInterface(const nautilus::val<HashMapInterface*>& hashMapRef) : hashMapRef(hashMapRef) { }

    virtual ~HashMapRefInterface() = default;
...
protected:
    nautilus::val<HashMapInterface*> hashMapRef;
};
