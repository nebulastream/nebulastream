 ...
 * To operate on the hash-map, {@refitem ChainedHashMapRef.hpp} provides a
 * Nautilus wrapper. The implementation origins from Kersten et.al.
 * https://github.com/TimoKersten/db-engine-paradigms and Leis et.al
 ...
 * 2. This hash map dose not clears the content of the entry. So its up to the
 * user to initialize values correctly.
 */
class ChainedHashMap {
 public:
   ...
   * Each entry contains a ptr to the next element, the hash of the current
   * value and the keys and values. The physical layout is the following | ---
   ...
   * @param numberOfKeys a number of keys, which are assumed to be stored in the
   * hash map. This is used to size the entry area. A too small number of keys
   * will increase coalitions. A too large number of keys will increase space
   * requirements.
   * @param allocator the memory allocator, which is used to allocate space for
   * entries.
   * @param pageSize the page size with DEFAULT_PAGE_SIZE as default.
   */
  ChainedHashMap(uint64_t keySize, uint64_t valueSize, uint64_t numberOfKeys,
                 std::unique_ptr<std::pmr::memory_resource> allocator,
                 size_t pageSize = DEFAULT_PAGE_SIZE);

  /**
   * @brief Inline implementation to find the start of a chain from the hash
   * map.
  ...
  /**
   * @brief Inline implementation to insert a new entry in the hash map for a
   * specific hash. This allocates a new entry, and inserts it to the hash map.
   * @param hash the hash for the key.
   * @return Entry* to the new entry.
   */
  inline Entry* insertEntry(hash_t hash) {
    auto* newEntry = allocateNewEntry();
    // call the constructor of Entry at the address of new Entry to initialize
    // the object.
   ...
   * This assumes that entries have the same shape and size as the the entries
   * of this hash map This avoids any allocations, and inserts the page directly
   * to the map, thus the ownership of the page also moves.
   * @param page
   ...
   * All pointers to entries of the hash map become invalid and accesses are
   * undefined.
   */
 ...
 private:
  /**
   * @brief Inserts a new entry to the hash map. Each new entry will be stored
   * as the first element in the bucket chain.
   ...
   * If the current page has enough space the new entry will be placed in the
   * current page. If the page is full, we will allocate a new page.
   * @return Entry*
   */
