 ...
 * This class provides convenient methods to insert and access values in the
 * hash map. Furthermore, it provides EntryRef as a wrapper for an individual
 * Entry* and an EntryIterator to iterate over all entries in the hash map.
 */
class ChainedHashMapRef {
 public:
  ...
  class EntryRef {
   public:
    EntryRef(const Value<MemRef>& ref, uint64_t keyOffset,
     ...
     * If this entry is the last entry, the returned EntryRef may point to a
     * nullptr.
     * @return EntryRef
     */
  ...
  class KeyEntryIterator {
   public:
    KeyEntryIterator(ChainedHashMapRef& hashTableRef, const Value<UInt64>& hash,
  ...
  /**
   * @brief This function performs a lookup to the hash map with a potentially
   * compound key and an associated hash. If the key as found the entry is
   * returned. Otherwise the entry is a nullptr.
   * @note the hash has to be derived by the keys using the same hash function
   * as when it was inserted the first time.
  ...
  /**
   * @brief This function performs a lookup to the hash map with a potentially
   * compound key and an associated hash. It returns an KeyEntryIterator which
   * allows to iterate through all found entries.
   * @note the hash has to be derived by the keys using the same hash function
   * as when it was inserted the first time.
   * @param hash the hash of the keys derived with a specific hash function.
   * @param keys a list of keys.
   ...
   */
  KeyEntryIterator findAll(const Value<UInt64>& hash,
                           const std::vector<Value<>>& keys);

  /**
   * @brief This function performs a lookup to the hash map with a potentially
   * compound key and an associated hash. If the key as found the entry is
   * returned. If the key was not found a new entry for the set of keys is
   * inserted and returned. New entries contain the hash and the keys but no
   * value.
   * @note the hash has to be derived by the keys using the same hash function
   * as when it was inserted the first time.
   * @param hash the hash of the keys derived with a specific hash function.
   ...
   * compound key and an associated hash. If the key as found the entry is
   * returned. If the key was not found a new entry for the set of keys is
   * inserted and returned. Furthermore, onInsert is called for each new entry,
   * which allows the initialization of the entry value.
   * @param hash the hash of the keys derived with a specific hash function.
   * @param keys a list of keys.
  ...
  /**
   * @brief This function inserts an already existing entry from another hash
   * map to this hash map. To this end, we assume that both hash maps use the
   * same hash function. If an entry with the same key already exists the update
   * function is called and allows to merge both entries.
   * @param otherEntry reference to the entry that should be inserted.
   * @param update function, which is called if an entry with the same key
   * already exists.
   */
  void insertEntryOrUpdate(const EntryRef& otherEntry,
                           const std::function<void(EntryRef&)>& update);
   ...
   * If an entry with the same hash already exists we append it as the head of
   * the chain.
   * @param hash the hash of the keys derived with a specific hash function.
   * @param keys a list of keys.
   * @return EntryRef
