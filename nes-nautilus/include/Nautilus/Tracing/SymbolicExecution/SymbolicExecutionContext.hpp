...
/**
 * @brief The symbolic execution context supports the symbolic execution of
 * functions. In general it executes a function with dummy parameters and
...
public:
   ...
   * Depending on all previous executions this function determines if a branch
   * should be explored or not.
   * @return the return value of this branch
   */
  bool executeCMP(TagRecorder &tr);
  /**
   * @brief Check if we should continue the symbolic execution or if we
   * evaluated all possible execution passes.
   * @return false if all execution passes trough a function have been
   * evaluated.
   */
  bool shouldContinue();
  /**
...
private:
   ...
   * That identifies if, we follow a previously recorded execution or if we
   * record a new one.
   */
   ...
   * If we already visited it two times, we can skip any further executions at
   * this point.
   */
