 ...
 * This implies that, each value is only assigned and that all parameters to a
 * basic block are passed by block arguments.
 */
class SSACreationPhase {
 ...
 private:
  /**
   * @brief The SSACreationPhaseContext maintains local state, which is required
   * to calculate the ssa from.
   */
  class SSACreationPhaseContext {
   ...
   private:
    ...
    /**
     * @brief Checks if an ValueRef is defined in a specific block by an
     * argument or an operation before the current operationIndex
     ...
     */
    bool isLocalValueRef(Block& block, ValueRef& valRef,
    ...
    /**
     * @brief In this step we finalise the block arguments of all blocks and
     * create unique variances of them.
     */
