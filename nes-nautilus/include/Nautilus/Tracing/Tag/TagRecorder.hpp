...
/**
 * @brief The tag recorder derives the tag from at a specific execution
 * position. To this end, it derives the backtrace, which is a unique identifier
 * of an instruction.
 */
...
public:
  ...
  static inline __attribute__((always_inline)) TagRecorder createTagRecorder() {
    // First we derive the base address, which is the first common address
    // between two bracktraces.
  ...
  /**
   * @brief Derive the tag of a specific instruction and returns the tag
   * pointer, which uniquely identifies this instruction.
   * @return Tag*
   */
  [[nodiscard]] inline __attribute__((always_inline)) Tag *createTag() {
    return this->createReferenceTag(startAddress);
  }
...
private:
  ...
  // The start address, which is used as the start to calculate tags for sub
  // instructions.
  const TagAddress startAddress;
  // The tag recorder stores the individual tags in a trie of addresses, to
