...
/**
 * @brief The trace context manages a thread local instance to record a symbolic
 * execution trace of a given Nautilus function. Tracing will be initialized
 * with ether traceFunction or traceFunctionWithReturn.
 */
class TraceContext {
 public:
  ...
  template <typename Functor>
  friend std::shared_ptr<ExecutionTrace> traceFunctionWithReturn(
      const Functor&& func);
   ...
   */
  void traceConstOperation(const ValueRef& valueReference,
                           const AnyPtr& constValue);
   ...
   */
  void traceUnaryOperation(const OpCode& op, const ValueRef& inputRef,
   ...
   * @param resultRef reference to the result.
   */
  void traceBinaryOperation(const OpCode& op, const ValueRef& leftRef,
   ...
   */
  void traceAssignmentOperation(const ValueRef& targetRef,
   ...
   * The symbolic executor decides if the cmp will be evaluated to true or
   * false.
   * @param inputRef.
 ...
 private:
  ...
  std::shared_ptr<ExecutionTrace> apply(
      const std::function<ValueRef()>& function);
