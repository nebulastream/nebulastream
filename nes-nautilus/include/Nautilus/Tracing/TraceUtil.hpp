...
/**
 * Returns a boolean value indicating whether the method is executed in the
 * interpreter.
 * @return {@code true} when executed in the interpreter, {@code false} in
 * compiled code.
 */
 ...
 * Returns a boolean value indicating whether the method is executed in the
 * tracer.
 * @return {@code true} when executed in the tracer, {@code false} in compiled
 * code.
 */
 ...
 */
void traceConstOperation(const AnyPtr& constValue,
                         const Tracing::ValueRef& valueReference);
 ...
 */
void traceAssignmentOperation(const Tracing::ValueRef& targetRef,
                              const Nautilus::Tracing::ValueRef& sourceRef);
bool traceBoolOperation(const AnyPtr& boolValue,
                        const Nautilus::Tracing::ValueRef& sourceRef);
/**
 ...
 * @param resultRef reference to the result.
 */
void traceUnaryOperation(const Tracing::OpCode& op,
                         const Tracing::ValueRef& resultRef,
                         const Tracing::ValueRef& inputRef);
 ...
 */
void traceStoreOperation(const Tracing::ValueRef& memRef,
                         const Tracing::ValueRef& valueRef);
