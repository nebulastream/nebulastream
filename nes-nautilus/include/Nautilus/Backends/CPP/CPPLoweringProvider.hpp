  ...
  private:
    ...
    class LoweringContext {
      ...
      private:
        ...
        void process(const IR::Operations::Operation& operation, short block, RegisterFrame& frame);
        void process(const IR::Operations::IfOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::CompareOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::BranchOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::LoadOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::StoreOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::ProxyCallOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::NegateOperation& opt, short block, RegisterFrame& frame);
        void process(const IR::Operations::CastOperation& opt, short block, RegisterFrame& frame);
        static std::string getVariable(const std::string& id);
        static std::string getType(const IR::Types::StampPtr& stamp);

        template<class Type>
        void processConst(const IR::Operations::Operation& opt, short blockIndex, RegisterFrame& frame) {
        ...
        template<class Type>
        void processBinary(const IR::Operations::Operation& o, const std::string& operation, short blockIndex, RegisterFrame& frame) {
