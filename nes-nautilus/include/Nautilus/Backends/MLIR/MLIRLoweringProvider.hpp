...
class MLIRLoweringProvider {
 public:
  ...
  // Control Flow can cause new ValueFrames to be created, to correctly model
  // value access rights (scopes).
  ...
  /**
   * @brief Root MLIR generation function. Takes  as an IRGraph, and recursively
   * lowers its operations to MLIR.
   * @param :  represented as an IRGraph.
   * @return mlir::mlir::OwningOpRef<mlir::ModuleOp> that is equivalent to the
   * module, and can be lowered to LLVM IR in one step.
   */
  mlir::OwningOpRef<mlir::ModuleOp> generateModuleFromIR(
      std::shared_ptr<IR::IRGraph>);

  /**
   * @return std::vector<std::string>: All proxy function symbols used in the
   * module.
   */
  std::vector<std::string> getJitProxyFunctionSymbols();

  /**
   * @return std::vector<llvm::JITTargetAddress>: All proxy function addresses
   * used in the module.
   */
 ...
 private:
  ...
  std::unordered_map<std::string, mlir::Block*>
      blockMapping;  // Keeps track of already created basic blocks.

  /**
   * @brief Generates MLIR from a  basic block. Iterates over basic block
   * operations and calls generate.
   *
   * @param basicBlock: The  basic block that MLIR code is generated for.
   * @param frame: An unordered map that MLIR operations insert their resulting
   * values, and identifiers in.
   */
   ...
   * @param Operation:  operation that the MLIRLoweringProvider generates MLIR
   * code for.
   * @param frame: An unordered map that MLIR operations insert their resulting
   * values, and identifiers in.
   */
  void generateMLIR(const IR::Operations::OperationPtr& operation,
                    ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::FunctionOperation> funcOp,
                    ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::ConstIntOperation> constIntOp,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::ConstFloatOperation> constFloatOp,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::ConstBooleanOperation> constBooleanOp,
      ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::AddOperation> addIntOp,
                    ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::SubOperation> subIntOp,
                    ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::MulOperation> mulIntOp,
                    ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::DivOperation> divFloatOp,
                    ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::ModOperation> modIntOp,
                    ValueFrame& frame);
  ...
  void generateMLIR(std::shared_ptr<IR::Operations::AddressOperation> addressOp,
                    ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::IfOperation> ifOp,
                    ValueFrame& frame);
  ...
  void generateMLIR(std::shared_ptr<IR::Operations::ReturnOperation> returnOp,
                    ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::ProxyCallOperation> proxyCallOp,
      ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::OrOperation> yieldOperation,
                    ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::AndOperation> yieldOperation,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::NegateOperation> yieldOperation,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::BitWiseAndOperation> yieldOperation,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::BitWiseOrOperation> yieldOperation,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::BitWiseXorOperation> yieldOperation,
      ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::BitWiseLeftShiftOperation> yieldOperation,
      ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::BitWiseRightShiftOperation>
                        yieldOperation,
                    ValueFrame& frame);
  void generateMLIR(
      std::shared_ptr<IR::Operations::CastOperation> castOperation,
      ValueFrame& frame);
  void generateMLIR(std::shared_ptr<IR::Operations::LoopOperation> loopOp,
                    ValueFrame& frame);

  /**
   * @brief Generates a basic block inside of the current MLIR module. Used for
   ...
   */
  mlir::Block* generateBasicBlock(
      IR::Operations::BasicBlockInvocation& blockInvocation, ValueFrame& frame);
  ...
  mlir::FlatSymbolRefAttr insertExternalFunction(
      const std::string& name, void* functionPtr, mlir::Type resultType,
   ...
   */
  std::vector<mlir::Type> getMLIRType(
      std::vector<IR::Operations::OperationPtr> types);
   ...
   */
  mlir::Value getConstInt(const std::string& location,
                          IR::Types::StampPtr stamp, int64_t value);
   ...
   * @return ValueFrame: An unordered map with identifiers and MLIR values in
   * it.
   */
  ValueFrame createFrameFromParentBlock(
      ValueFrame& frame, IR::Operations::BasicBlockInvocation& invocation);
};
}  // namespace NES::Nautilus::Backends::MLIR
