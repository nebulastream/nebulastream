...
/**
 * @brief Represents an executable object, which enables the invocation of
...
public:
  ...
  /**
   * @brief A typed invocable wrapper that is callable as a function and
   * forwards arguments a concrete implementation.
   * @tparam R return type
   * @tparam Args agument types
   */
  template <typename R, typename... Args> class Invocable {
  public:
    using FunctionType = R(Args...);
    explicit Invocable(void *fptr)
        : function(reinterpret_cast<FunctionType *>(fptr)){};
    explicit Invocable(std::unique_ptr<GenericInvocable> generic)
        : function(std::move(generic)){};
    ...
    R operator()(Args... arguments) {
      if (std::holds_alternative<FunctionType *>(function)) {
        ...
        auto &genericFunction =
            std::get<std::unique_ptr<GenericInvocable>>(function);
        if constexpr (!std::is_void_v<R>) {
...
protected:
   ...
   */
  virtual std::unique_ptr<GenericInvocable>
  getGenericInvocable(const std::string &) {
    return nullptr;
  };
};
