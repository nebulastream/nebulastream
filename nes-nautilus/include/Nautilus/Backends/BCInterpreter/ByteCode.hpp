...
enum class ByteCode : short {
  ...
  CAST_i64_d,
  // Function calls
  // TODO #3466 come up with a better approach to call dynamically into runtime
  // functions functions with void return
  CALL_v,
  CALL_v_ptr,
  CALL_v_ptr_ui64,
  CALL_v_ptr_ptr_ptr,
...
/**
 * @brief Defines a function call target, that contains all information's
 * necessary to call a function
 */
class FunctionCallTarget {
public:
  FunctionCallTarget(std::vector<std::pair<short, Type>> arguments,
                     void *functionPtr);
...
/**
 * @brief The general definition of opcode, that contains a bytecode, at max two
 * input registers and a result register.
 */
struct OpCode {
  OpCode(ByteCode op, short reg1, short reg2, short output)
      : op(op), reg1(reg1), reg2(reg2), output(output){};
  ByteCode op;
  short reg1;
  short reg2;
  short output;
 ...
 * Currently we only support external function calls with a fixed set of known
 * signatures. This should be generalized in #3466
 * @tparam ReturnType
 * @tparam Args
 * @param c
 * @param regs
...
void call(const OpCode &c, RegisterFile &regs) {
  ...
  if constexpr (std::is_void_v<ReturnType>) {
    ...
    if constexpr (sizeof...(Args) == 0) {
      ...
      auto value0 = readReg<__type_pack_element<0, Args...>>(
          regs, fcall->arguments[0].first);
      ptr(value0);
    } else if constexpr (sizeof...(Args) == 2) {
      auto value0 = readReg<__type_pack_element<0, Args...>>(
          regs, fcall->arguments[0].first);
      auto value1 = readReg<__type_pack_element<1, Args...>>(
          regs, fcall->arguments[1].first);
      ptr(value0, value1);
      ...
      auto value1 = readReg<__type_pack_element<1, Args...>>(
          regs, fcall->arguments[1].first);
      auto value2 = readReg<__type_pack_element<2, Args...>>(
          regs, fcall->arguments[2].first);
      ...
      auto value2 = readReg<__type_pack_element<2, Args...>>(
          regs, fcall->arguments[2].first);
      auto value3 = readReg<__type_pack_element<3, Args...>>(
          regs, fcall->arguments[3].first);
      auto value4 = readReg<__type_pack_element<4, Args...>>(
          regs, fcall->arguments[4].first);
      auto value5 = readReg<__type_pack_element<5, Args...>>(
          regs, fcall->arguments[5].first);
      auto value6 = readReg<__type_pack_element<6, Args...>>(
          regs, fcall->arguments[6].first);
 ...
 * @param regs
 */
template <class RegisterType> void andOp(const OpCode &c, RegisterFile &regs) {
 ...
 * @param regs
 */
template <class RegisterType> void orOp(const OpCode &c, RegisterFile &regs) {
  auto l = readReg<RegisterType>(regs, c.reg1);
  auto r = readReg<RegisterType>(regs, c.reg2);
  writeReg(regs, c.output, l || r);
}
 ...
 * @param regs
 */
template <class RegisterType> void notOp(const OpCode &c, RegisterFile &regs) {
 ...
 * @param regs
 */
template <class RegisterType> void add(const OpCode &c, RegisterFile &regs) {
  auto l = readReg<RegisterType>(regs, c.reg1);
  auto r = readReg<RegisterType>(regs, c.reg2);
  writeReg(regs, c.output, l + r);
}
 ...
 * @param regs
 */
template <class RegisterType> void sub(const OpCode &c, RegisterFile &regs) {
  auto l = readReg<RegisterType>(regs, c.reg1);
  auto r = readReg<RegisterType>(regs, c.reg2);
  writeReg(regs, c.output, l - r);
}
 ...
 * @param regs
 */
template <class RegisterType> void mul(const OpCode &c, RegisterFile &regs) {
  auto l = readReg<RegisterType>(regs, c.reg1);
  auto r = readReg<RegisterType>(regs, c.reg2);
  writeReg(regs, c.output, l * r);
}
 ...
 * @param regs
 */
template <class RegisterType> void div(const OpCode &c, RegisterFile &regs) {
  auto l = readReg<RegisterType>(regs, c.reg1);
  auto r = readReg<RegisterType>(regs, c.reg2);
  writeReg(regs, c.output, l / r);
}
 ...
 * @param regs
 */
template <class RegisterType> void equals(const OpCode &c, RegisterFile &regs) {
  auto l = readReg<RegisterType>(regs, c.reg1);
  auto r = readReg<RegisterType>(regs, c.reg2);
  writeReg(regs, c.output, l == r);
}
 ...
 * @param regs
 */
template <class RegisterType> void load(const OpCode &c, RegisterFile &regs) {
  auto address = readReg<int64_t>(regs, c.reg1);
 ...
 * @param regs
 */
template <class RegisterType> void store(const OpCode &c, RegisterFile &regs) {
  auto address = readReg<int64_t>(regs, c.reg1);
...
public:
  ...
  std::variant<BranchOp, ConditionalJumpOp, ReturnOp> terminatorOp =
      ReturnOp{0};
