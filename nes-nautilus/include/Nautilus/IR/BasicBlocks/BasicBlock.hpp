...
class BasicBlock : public std::enable_shared_from_this<BasicBlock> {
 public:
   ...
   * @param Operations: A list of Operations that are executed in the
   * BasicBlock.
   * @param nextBlocks : The BasicBlock that is next in the control flow of the
   * execution.
   */
  explicit BasicBlock(
  ...
  /**
   * @brief Get the number of edges that lead back from the loop body to the
   * loop header.
   */
  [[nodiscard]] uint32_t getNumLoopBackEdges();

  /**
   * @brief Increment counter for edges that lead back from the loop body to the
   * loop header.
   */
  void incrementNumLoopBackEdge();

  /**
   * @brief Check if the counter for edges that lead back from the loop body to
   * the loop header is > 0.
   */
  ...
  [[nodiscard]] std::vector<std::shared_ptr<Operations::BasicBlockArgument>>
  getArguments();

  // NESIR Assembly
  std::shared_ptr<BasicBlock> addOperation(Operations::OperationPtr operation);
  std::shared_ptr<BasicBlock> addLoopHeadBlock(
      std::shared_ptr<BasicBlock> loopHeadBlock);
  std::shared_ptr<BasicBlock> addNextBlock(
      std::shared_ptr<BasicBlock> nextBlock);
  void addNextBlock(std::shared_ptr<BasicBlock> nextBlock,
                    std::vector<Operations::OperationPtr> inputArguments);
  std::shared_ptr<BasicBlock> addTrueBlock(
      std::shared_ptr<BasicBlock> thenBlock);
  std::shared_ptr<BasicBlock> addFalseBlock(
      std::shared_ptr<BasicBlock> elseBlock);
  void removeOperation(Operations::OperationPtr operation);
  void addOperationBefore(Operations::OperationPtr before,
  ...
  // void popOperation();
  void replaceTerminatorOperation(
      Operations::OperationPtr newTerminatorOperation);
