...
private:
  ...
  public:
     ...
     *
     * @param ir: IRGraph to which the remove br-only-blocks phase will be
     * applied.
     */
  ...
  private:
    ...
    /**
     * @brief Removes all br-only blocks from the IR without influencing its
    ...
    /**
     * @brief Checks if the current block is a br-only-block and check whether
     * subsequent blocks are also br-only-blocks. When a sequence of size 1-n
     * br-only-blocks was found, updatePredecessorBlocks() is called.
     *
     * @param currentBlock: Current block that might be a br-only-block.
     * @param candidates: Blocks that still need to be processed.
     * @param visitedBlocks: Blocks that have already been processed and can be
     * disregarded.
     */
