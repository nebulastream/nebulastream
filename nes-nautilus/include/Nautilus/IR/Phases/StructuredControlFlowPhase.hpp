...
/**
 * @brief This phase takes an IR graph with blocks that either have branch- or
 * if-operations as terminator-operations. Subsequently, this phase matches
 * if-operations with their correct corresponding merge-blocks. Also,
 * if-operations are enriched with information on which blocks branch to the
 * merge-blocks. This is required to correctly create MLIR YieldOps.
 */
...
private:
  ...
  public:
    /**
     * @brief Helper struct used to keep track of the currently active branch of
     * an if-operation.
     */
     ...
     */
    StructuredControlFlowPhaseContext(std::shared_ptr<IR::IRGraph> ir)
        : ir(ir){};
    /**
  ...
  private:
    /**
     * @brief Iterates over IR graph, and connects all if-operations with their
     * corresponding merge-blocks.
     *
     * @param currentBlock: Initially will be the body-block of the root
     * operation.
     */
     ...
     * @brief Checks if the given currentBlocks is a merge-block with open edges
     (open: not yet traversed).
     *
     * @param currentBlock: Initially will be the body-block of the root
     operation.
     * @param ifOperations: A stack that contains all visited if-operations
                            that have not been matched with merge-blocks yet.
     * @param mergeBlockNumVisits: A HashMap that keeps track on how many times
     we visited a merge-block already.
     * @param newVisit: Signals whether we reached the currentBlock via a new
     edge.
     * @return true, if the currentBlock is a merge-block with open edges, and
     false if not.
     */
