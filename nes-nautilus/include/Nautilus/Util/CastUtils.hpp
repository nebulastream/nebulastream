...
public:
  /**
   * @brief Factory method to create a new type identifier for a specific
   * template type. The template type has to be derived from typed.
   * @tparam T derived from typed.
   * @return TypeIdentifier
   */
  template <class T> static TypeIdentifier create() {
    auto &typeId = typeid(T);
    return TypeIdentifier(typeId);
  }

  template <class name> bool isType() const { return this == &name::type; }
...
/**
 * @brief If a class inherits from Typed it enables the custom casting system
 * and allows the isa and cast methods.
 */
class Typed {
public:
  Typed(const TypeIdentifier *typeIdentifier);
  inline const TypeIdentifier *getTypeIdentifier() const {
    return typeIdentifier;
  };

  template <HasTypeIdentifier T> bool isType() const {
...
template <HasTypeIdentifier T> T *cast(Typed *typed) {
...
template <HasTypeIdentifier T, IsTyped U> bool isa(U &typed) {
...
template <HasTypeIdentifier T> T *cast_if(Typed *typed) {
  if (isa<T>(typed)) {
    return cast<T>(typed);
  }
  return nullptr;
