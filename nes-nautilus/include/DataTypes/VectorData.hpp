...
/// Vector (variable-sized array) of (currently primitive) nautilus values.
/// Unlike FixedSizedData, the number of elements are not known before receiving the vector. However, we know the element type at trace time.
...
public:
    ...
    /// Two VectorData are equal if element type, count, and the underlying
    /// bytes (memcmp over numElements * sizeof(element)) all match.
    nautilus::val<bool> operator==(const VectorData& rhs) const;
    nautilus::val<bool> operator!=(const VectorData& rhs) const;
