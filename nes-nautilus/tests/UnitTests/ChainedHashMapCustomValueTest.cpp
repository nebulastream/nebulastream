...
TEST_P(ChainedHashMapCustomValueTest, pagedVector)
{
    ...
    for (auto [buffer, keyPositionInBuffer] : std::views::zip(inputBuffers, allKeyPositions))
    {
        ...
        const auto neededBytes = memoryProviderInputBuffer->getMemoryLayout()->getSchema().sizeOfSchemaInBytes * numberOfRecordsExact;
        ...
        const auto writtenBytes
            = outputBuffer.getNumberOfTuples() * memoryProviderInputBuffer->getMemoryLayout()->getSchema().sizeOfSchemaInBytes;
