...
TEST_P(ChainedHashMapCustomValueTest, pagedVector)
{
    ...
    for (auto [buffer, keyPositionInBuffer] : std::views::zip(inputBuffers, allKeyPositions))
    {
        ...
        const auto neededBytes
            = memoryProviderInputBuffer->getMemoryLayoutPtr()->getSchema()->getSchemaSizeInBytes() * numberOfRecordsExact;
        ...
        const auto writtenBytes
            = outputBuffer.getNumberOfTuples() * memoryProviderInputBuffer->getMemoryLayoutPtr()->getSchema()->getSchemaSizeInBytes();
