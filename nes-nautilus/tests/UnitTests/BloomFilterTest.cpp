...
/// Extreme sizing inputs bracketing the collision spectrum. The no-false-negative invariant must hold at all
/// three; the FP-rate bound needs no special casing because it derives from the actual sizing.
/// Not constexpr: the validating constructor is not a constant expression.
const std::array<BloomFilterParams, 3> EXTREME_PARAMS = {
    BloomFilterParams{4, 5e-4}, /// 64 bits, 11 hashes: saturates after a handful of inserts, FP rate ~1
    BloomFilterParams{44, 0.9}, /// 64 bits, 1 hash: smallest filter, minimal hashing
    BloomFilterParams{5'814'540, 0.5}, /// 2^23 bits, 1 hash: stays near-empty, FP rate ~0
};
...
BloomFilterParams drawBloomFilterParams(uint64_t numKeysForSizing, double fpRate)
{
    ...
    return idx < EXTREME_PARAMS.size() ? EXTREME_PARAMS.at(idx) : BloomFilterParams{numKeysForSizing, fpRate};
}
...
void insertAndCheckProperty(EngineMode mode)
{
    ...
    NES_INFO(
        ...
        numTotal,
        seed,
        bfParams.getBitCount(),
        bfParams.getHashCount());
    ...
    if (trueNegatives.size() >= MIN_FP_SAMPLES_FOR_RATE_CHECK)
    {
        ...
        const double theoretical = theoreticalFpRate(insertedDistinct.size(), bfParams.getBitCount(), bfParams.getHashCount());
...
void sizingProperty()
{
    ...
    BloomFilterParams params{numKeys, fpRate};
    RC_ASSERT(params.getBitCount() >= 64);
    RC_ASSERT(params.getHashCount() >= 1);
    ...
    RC_ASSERT(params.allocationByteCount() * 8 >= params.getBitCount());
}
