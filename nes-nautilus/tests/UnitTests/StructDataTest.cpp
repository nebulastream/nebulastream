...
{
    return DataType{
        DataType::Type::FIXEDSIZED, DataType::NULLABLE::NOT_NULLABLE, DataType{element, DataType::NULLABLE::NOT_NULLABLE}, count};
}
}
...
TEST_F(StructDataTest, FixedSizedFieldYieldsFixedSizedDataView)
{
    ...
    EXPECT_EQ(fixed.getElementType().type, DataType::Type::UINT16);
...
TEST_F(StructDataTest, WriteAtFixedSizedFieldCopiesInlineBytes)
{
    ...
    const FixedSizedData sourceView(
        nautilus::val<int8_t*>(reinterpret_cast<int8_t*>(sourcePixels.data())),
        sourcePixels.size(),
