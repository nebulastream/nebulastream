...
TEST_F(VarValTest, readFromMemoryTest)
{
    ...
    testVarValReadFromMemory.operator()<int8_t>(
        -someRandomNumber, DataType{.type = DataType::Type::INT8, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<int16_t>(
        minI8Minus1, DataType{.type = DataType::Type::INT16, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<int32_t>(
        minI16Minus1, DataType{.type = DataType::Type::INT32, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<int64_t>(
        minI32Minus1, DataType{.type = DataType::Type::INT64, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<uint8_t>(
        someRandomNumber, DataType{.type = DataType::Type::UINT8, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<uint16_t>(
        maxUI8Plus1, DataType{.type = DataType::Type::UINT16, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<uint32_t>(
        maxUI16Plus1, DataType{.type = DataType::Type::UINT32, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<uint64_t>(
        maxUI32Plus1, DataType{.type = DataType::Type::UINT64, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<float>(
        someRandomNumber, DataType{.type = DataType::Type::FLOAT32, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<double>(
        someRandomNumber, DataType{.type = DataType::Type::FLOAT64, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<bool>(
        true, DataType{.type = DataType::Type::BOOLEAN, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
    testVarValReadFromMemory.operator()<bool>(
        false, DataType{.type = DataType::Type::BOOLEAN, .isNullable = DataType::NULLABLE::NOT_NULLABLE});
}
...
TEST_F(VarValTest, nullableConstruction)
{
    ...
    const VarVal nullable(42, DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{false});
    EXPECT_TRUE(nullable.isNullable());
    EXPECT_FALSE(static_cast<bool>(nullable.isNull()));

    /// Test nullable null value
    const VarVal nullValue(0, DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{true});
...
{
    const VarVal nullVal(nautilus::val<int32_t>(0), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{true});
...
TEST_F(VarValTest, nullBooleanConversion)
{
    ...
    const VarVal trueNonNullable(nautilus::val<bool>(true), DataType::NULLABLE::NOT_NULLABLE, nautilus::val<bool>{false});
    EXPECT_TRUE(static_cast<bool>(trueNonNullable));

    /// Non-null false in non-nullable context
    const VarVal falseNonNullable(nautilus::val<bool>(false), DataType::NULLABLE::NOT_NULLABLE, nautilus::val<bool>{false});
    ...
    const VarVal nullValWithTrue(nautilus::val<bool>(false), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{true});
    EXPECT_FALSE(static_cast<bool>(nullValWithTrue));

    /// NULL in nullable context, but with different underlying value (should still be false)
    const VarVal nullValWithFalse(nautilus::val<bool>(true), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{true});
    EXPECT_FALSE(static_cast<bool>(nullValWithFalse));

    /// Copy constructor/assignment: NULL remains NULL
    const VarVal nullValOriginal(nautilus::val<bool>(true), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{true});
...
{
    /// NULL value
    const VarVal nullVal(nautilus::val<bool>(true), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{true});
    /// Non-null true value
    const VarVal trueVal(nautilus::val<bool>(true), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{false});
    /// Non-null false value
    const VarVal falseVal(nautilus::val<bool>(false), DataType::NULLABLE::IS_NULLABLE, nautilus::val<bool>{false});
    ...
    /// true == true -> true
    const auto eqResult2 = trueVal == trueVal;
    ...
    /// false == false -> true
    const auto eqResult3 = falseVal == falseVal;
