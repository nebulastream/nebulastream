...
TEST_P(PagedVectorTest, storeAndRetrieveLargeValues)
{
    ...
    const auto testSchema
        = Schema(Schema::MemoryLayoutType::ROW_LAYOUT).addField("value1", DataTypeProvider::provideDataType(LogicalType::VARSIZED));
...
TEST_P(PagedVectorTest, storeAndRetrieveFixedValuesNonDefaultPageSize)
{
    bufferManager = Memory::BufferManager::create();
    const auto testSchema
        = Schema(Schema::MemoryLayoutType::ROW_LAYOUT).addField("value1", BasicType::UINT64).addField("value2", BasicType::UINT64);
