...
TEST_P(PagedVectorTest, storeAndRetrieveFixedSizeValues)
{
    ...
    const auto allRecords
        = createMonotonicallyIncreasingValues(testSchema, layoutType, numberOfItems, *bufferManager);
...
TEST_P(PagedVectorTest, storeAndRetrieveVarSizeValues)
{
    ...
    constexpr auto pageSize = PAGE_SIZE;
    const auto projections
        = testSchema | std::views::transform([](const auto& field) { return field.getFullyQualifiedName(); }) | std::ranges::to<std::vector>();
    const auto allRecords
        = createMonotonicallyIncreasingValues(testSchema, layoutType, numberOfItems, *bufferManager);
...
TEST_P(PagedVectorTest, storeAndRetrieveLargeValues)
{
    ...
    const auto projections
        = testSchema | std::views::transform([](const auto& field) { return field.getFullyQualifiedName(); }) | std::ranges::to<std::vector>();
    const auto allRecords
        = createMonotonicallyIncreasingValues(testSchema, layoutType, numberOfItems, *bufferManager, sizeVarSizedData);
