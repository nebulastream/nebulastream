...
public:
    ...
    static LegacySchema createSchemaFromBasicTypes(const std::vector<DataType::Type>& basicTypes);
    static LegacySchema createSchemaFromBasicTypes(const std::vector<DataType::Type>& basicTypes, uint64_t typeIdxOffset);
    ...
    std::vector<TupleBuffer> createMonotonicallyIncreasingValues(
        ...
        uint64_t minSizeVarSizedData);
    std::vector<TupleBuffer> createMonotonicallyIncreasingValues(
        const LegacySchema& schema, const MemoryLayoutType& memoryLayout, uint64_t numberOfTuples, BufferManager& bufferManager);
