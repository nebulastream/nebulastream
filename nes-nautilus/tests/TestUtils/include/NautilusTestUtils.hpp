...
public:
    ...
    static Schema<QualifiedUnboundField, Ordered> createSchemaFromBasicTypes(const std::vector<DataType::Type>& basicTypes, uint64_t typeIdxOffset);
    ...
    std::vector<TupleBuffer> createMonotonicallyIncreasingValues(
        const Schema<QualifiedUnboundField, Ordered>& schema, const MemoryLayoutType layoutType, uint64_t numberOfTuples, BufferManager& bufferManager, uint64_t minSizeVarSizedData);
    std::vector<TupleBuffer>
    createMonotonicallyIncreasingValues(const Schema<QualifiedUnboundField, Ordered>& schema, MemoryLayoutType layoutType, uint64_t numberOfTuples, BufferManager& bufferManager);
