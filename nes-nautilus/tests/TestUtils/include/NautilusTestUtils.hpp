...
public:
    ...
    static PhysicalSchema createSchemaFromBasicTypes(const std::vector<DataType::Type>& basicTypes);
    static PhysicalSchema createSchemaFromBasicTypes(const std::vector<DataType::Type>& basicTypes, uint64_t typeIdxOffset);
    ...
    std::vector<TupleBuffer> createMonotonicallyIncreasingValues(
        ...
        uint64_t minSizeVarSizedData);
    std::vector<TupleBuffer> createMonotonicallyIncreasingValues(
        const PhysicalSchema& schema, const MemoryLayoutType& memoryLayout, uint64_t numberOfTuples, BufferManager& bufferManager);
