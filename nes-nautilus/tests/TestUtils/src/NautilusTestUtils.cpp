...
std::vector<TupleBuffer> NautilusTestUtils::createMonotonicallyIncreasingValues(
    const Schema<QualifiedUnboundField, Ordered>& schema, const MemoryLayoutType memoryLayout, const uint64_t numberOfTuples, BufferManager& bufferManager)
{
...
Schema<QualifiedUnboundField, Ordered> NautilusTestUtils::createSchemaFromBasicTypes(const std::vector<DataType::Type>& basicTypes, const uint64_t typeIdxOffset)
{
    const auto fields =
