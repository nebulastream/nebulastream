...
/// std::monostate (null) is compatible with any nullable type.
bool isTypeCompatible(const DataType::Type type, const FieldValue& value)
{
    if (std::holds_alternative<std::monostate>(value))
    {
        return true; /// null is compatible with any nullable field
    }
    switch (type)
    {
...
VarVal fieldValueToVarVal(const FieldValue& value, const DataType& dataType)
{
    /// Null value — create a zero-valued VarVal with null flag set
    if (std::holds_alternative<std::monostate>(value))
    {
        switch (dataType.type)
        {
            case DataType::Type::BOOLEAN: return VarVal(false, true, nautilus::val<bool>{true});
            case DataType::Type::INT8:    return VarVal(int8_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::INT16:   return VarVal(int16_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::INT32:   return VarVal(int32_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::INT64:   return VarVal(int64_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::UINT8:   return VarVal(uint8_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::UINT16:  return VarVal(uint16_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::UINT32:  return VarVal(uint32_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::UINT64:  return VarVal(uint64_t{0}, true, nautilus::val<bool>{true});
            case DataType::Type::FLOAT32: return VarVal(0.0f, true, nautilus::val<bool>{true});
            case DataType::Type::FLOAT64: return VarVal(0.0, true, nautilus::val<bool>{true});
            case DataType::Type::VARSIZED:
            {
                nautilus::val<int8_t*> nullPtr{static_cast<int8_t*>(nullptr)};
                return VarVal{VariableSizedData(nullPtr, nautilus::val<uint64_t>{0}), true, nautilus::val<bool>{true}};
            }
            default: throw UnknownDataType("fieldValueToVarVal: unsupported null data type");
        }
    }

    const auto type = dataType.type;
    switch (type)
    {
        case DataType::Type::BOOLEAN:
...
{
    /// Check for null before reading the value
    if (value.isNullable() && nautilus::details::RawValueResolver<bool>::getRawValue(value.isNull()))
    {
    ...
    {
        case DataType::Type::BOOLEAN:
            return nautilus::details::RawValueResolver<bool>::getRawValue(value.getRawValueAs<nautilus::val<bool>>());
        case DataType::Type::INT8:
            return nautilus::details::RawValueResolver<int8_t>::getRawValue(value.getRawValueAs<nautilus::val<int8_t>>());
        case DataType::Type::INT16:
            return nautilus::details::RawValueResolver<int16_t>::getRawValue(value.getRawValueAs<nautilus::val<int16_t>>());
        case DataType::Type::INT32:
            return nautilus::details::RawValueResolver<int32_t>::getRawValue(value.getRawValueAs<nautilus::val<int32_t>>());
        case DataType::Type::INT64:
            return nautilus::details::RawValueResolver<int64_t>::getRawValue(value.getRawValueAs<nautilus::val<int64_t>>());
        case DataType::Type::UINT8:
            return nautilus::details::RawValueResolver<uint8_t>::getRawValue(value.getRawValueAs<nautilus::val<uint8_t>>());
        case DataType::Type::UINT16:
            return nautilus::details::RawValueResolver<uint16_t>::getRawValue(value.getRawValueAs<nautilus::val<uint16_t>>());
        case DataType::Type::UINT32:
            return nautilus::details::RawValueResolver<uint32_t>::getRawValue(value.getRawValueAs<nautilus::val<uint32_t>>());
        case DataType::Type::UINT64:
            return nautilus::details::RawValueResolver<uint64_t>::getRawValue(value.getRawValueAs<nautilus::val<uint64_t>>());
        case DataType::Type::FLOAT32:
            return nautilus::details::RawValueResolver<float>::getRawValue(value.getRawValueAs<nautilus::val<float>>());
        case DataType::Type::FLOAT64:
            return nautilus::details::RawValueResolver<double>::getRawValue(value.getRawValueAs<nautilus::val<double>>());
        case DataType::Type::VARSIZED: {
...
TestTupleBufferView TestTupleBuffer::openWithBufferRef(TupleBuffer& buffer, std::shared_ptr<TupleBufferRef> bufRef, AbstractBufferProvider* bufferProvider)
{
    TestTupleBufferView view;
    view.impl = std::make_shared<TestTupleBufferView::Impl>(
        TestTupleBufferView::Impl{.schema = schema, .buffer = buffer, .bufferProvider = bufferProvider, .bufRef = std::move(bufRef)});
...
void TestTupleBufferView::appendImpl(const FieldValue* values, const size_t count)
{
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        record.write(fields[i].name, fieldValueToVarVal(values[i], fields[i].dataType));
    }
...
FieldView& FieldView::operator=(const FieldValue& value)
{
    ...
    Record record;
    record.write(fieldName, fieldValueToVarVal(value, dataType));
