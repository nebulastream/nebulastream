    ...
    const std::vector<DataType::Type>& keyTypes, const std::vector<DataType::Type>& valueTypes, const ExecutionMode backend)
{
    ...
    const auto fieldNamesKey = inputSchemaKey | std::views::transform([](const auto& field) { return field.getFullyQualifiedName(); }) | std::ranges::to<std::vector>();
    const auto fieldNamesValue = inputSchemaValue | std::views::transform([](const auto& field) { return field.getFullyQualifiedName(); }) | std::ranges::to<std::vector>();
    inputSchema = Schema<QualifiedUnboundField, Ordered>{std::array{inputSchemaKey, inputSchemaValue} | std::views::join | std::ranges::to<std::vector>()};
    ...
    projectionKeys = inputSchemaKey | std::views::transform([](const auto& field) { return field.getFullyQualifiedName(); }) | std::ranges::to<std::vector>();
    projectionValues = inputSchemaValue | std::views::transform([](const auto& field) { return field.getFullyQualifiedName(); }) | std::ranges::to<std::vector>();
