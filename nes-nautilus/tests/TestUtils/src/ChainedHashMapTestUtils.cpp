...
void ChainedHashMapTestUtils::setUpChainedHashMapTest(
    const std::vector<BasicType>& keyTypes, const std::vector<BasicType>& valueTypes, const Nautilus::Configurations::ExecutionMode backend)
{
    ...
    const auto bufferNeeded = callsToCreateMonotonicValues * ((inputSchema.getSchemaSizeInBytes() * params.numberOfItems) / bufferSize + 1);
