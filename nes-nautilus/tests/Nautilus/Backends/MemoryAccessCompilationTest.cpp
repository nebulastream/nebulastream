...
class MemoryAccessCompilationTest : public Testing::BaseUnitTest,
                                    public AbstractCompilationBackendTest {
public:
  /* Will be called before any test in this class are executed. */
  static void SetUpTestCase() {
    NES::Logger::setupLogging("MemoryAccessCompilationTest.log",
  ...
  static void TearDownTestCase() {
    NES_DEBUG("Tear down MemoryAccessCompilationTest test class.");
  }
};
...
TEST_P(MemoryAccessCompilationTest, storeFunctionTest) {
  ...
  auto executionTrace = Nautilus::Tracing::traceFunction(
      [&tempPara]() { storeFunction(tempPara); });
  auto engine = prepare(executionTrace);
...
TEST_P(MemoryAccessCompilationTest, memScanFunctionTest) {
  ...
  memPtr.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0, IR::Types::StampFactory::createAddressStamp());
  auto size = Value<Int64>(0_s64);
  size.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 1, IR::Types::StampFactory::createInt64Stamp());
  auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn(
      [&memPtr, &size]() { return memScan(memPtr, size); });
  auto engine = prepare(executionTrace);
  auto function =
      engine->getInvocableMember<int64_t, int64_t, void *>("execute");
...
TEST_P(MemoryAccessCompilationTest, memEqualFunctionTest) {
  ...
  memRefLHS.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0, IR::Types::StampFactory::createAddressStamp());
  memRefRHS.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 1, IR::Types::StampFactory::createAddressStamp());
  auto executionTrace =
      Nautilus::Tracing::traceFunctionWithReturn([&memRefLHS, &memRefRHS]() {
...
INSTANTIATE_TEST_CASE_P(
    testLoopCompilation, MemoryAccessCompilationTest,
    ::testing::ValuesIn(
        Backends::CompilationBackendRegistry::getPluginNames().begin(),
        Backends::CompilationBackendRegistry::getPluginNames().end()),
    [](const testing::TestParamInfo<MemoryAccessCompilationTest::ParamType>
