...
Value<> loadFunction(Value<MemRef> ptr)
{
    return ptr.load<Int64>();
}
...
TEST_P(MemoryAccessCompilationTest, memScanFunctionTest)
{
    ...
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([&memPtr, &size]() { return memScan(memPtr, size); });
    auto engine = prepare(executionTrace);
...
TEST_P(MemoryAccessCompilationTest, memEqualFunctionTest)
{
    ...
    auto executionTrace
        = Nautilus::Tracing::traceFunctionWithReturn([&memRefLHS, &memRefRHS]() { return checkEqualityMemRefs(memRefLHS, memRefRHS); });
    auto engine = prepare(executionTrace);
