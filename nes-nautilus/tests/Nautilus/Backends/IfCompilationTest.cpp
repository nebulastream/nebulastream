...
class IfCompilationTest : public Testing::BaseUnitTest,
                          public AbstractCompilationBackendTest {
 public:
  /* Will be called before any test in this class are executed. */
  static void SetUpTestCase() {
    NES::Logger::setupLogging("IfCompilationTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down IfCompilationTest test class.");
  }
};
...
TEST_P(IfCompilationTest, orConditionTest) {
  ...
  value.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0, IR::Types::StampFactory::createInt32Stamp());

  auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn(
      [value]() { return orCondition(value); });
  auto engine = prepare(executionTrace);
...
TEST_P(IfCompilationTest, andConditionTest) {
  Value<Int32> x = +1_s32;
  x.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0, IR::Types::StampFactory::createInt32Stamp());

  Value<Int32> y = +1_s32;
  y.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 1, IR::Types::StampFactory::createInt32Stamp());
  ...
  auto engine = prepare(executionTrace);
  auto function =
      engine->getInvocableMember<int32_t, int32_t, int32_t>("execute");
...
INSTANTIATE_TEST_CASE_P(
    testIfCompilation, IfCompilationTest,
    ::testing::ValuesIn(
