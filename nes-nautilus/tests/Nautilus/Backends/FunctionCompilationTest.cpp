...
class FunctionCompilationTest : public Testing::BaseUnitTest,
                                public AbstractCompilationBackendTest {
 public:
  /* Will be called before any test in this class are executed. */
  static void SetUpTestCase() {
    NES::Logger::setupLogging("FunctionCompilationTest.log",
...
TEST_P(FunctionCompilationTest, voidExceptionFunctionTest) {
  ...
  auto executionTrace =
      Nautilus::Tracing::traceFunction([]() { voidExceptionFunction(); });
  auto result = prepare(executionTrace);
  auto function = result->getInvocableMember<int64_t>("execute");
...
TEST_P(FunctionCompilationTest, multiplyArgumentTest) {
  ...
  auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn(
      [&tempPara]() { return multiplyArgumentFunction(tempPara); });
  auto result = prepare(executionTrace);
...
TEST_P(FunctionCompilationTest, identifierTypesInProxyFunction) {
  ...
  tempPara.ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0, IR::Types::StampFactory::createInt64Stamp());
  auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn(
      [&tempPara]() { return testIdentifierTypes(tempPara); });
  auto result = prepare(executionTrace);
...
INSTANTIATE_TEST_CASE_P(
    testFunctionCalls, FunctionCompilationTest,
    ::testing::ValuesIn(
