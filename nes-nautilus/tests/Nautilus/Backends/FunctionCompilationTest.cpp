...
{
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([]() { return addIntFunction(); });
...
{
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([]() { return returnConstFunction(); });
    auto result = prepare(executionTrace);
    auto function = result->getInvocableMember<int64_t>("execute");
...
TEST_P(FunctionCompilationTest, multiplyArgumentTest)
{
    ...
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([&tempPara]() { return multiplyArgumentFunction(tempPara); });
    auto result = prepare(executionTrace);
...
TEST_P(FunctionCompilationTest, identifierTypesInProxyFunction)
{
    ...
    tempPara.ref = Nautilus::Tracing::ValueRef(INT32_MAX, 0, IR::Types::StampFactory::createInt64Stamp());
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([&tempPara]() { return testIdentifierTypes(tempPara); });
    auto result = prepare(executionTrace);
