...
class TypeCompilationTest : public Testing::BaseUnitTest,
                            public AbstractCompilationBackendTest {
 public:
  /* Will be called before any test in this class are executed. */
  static void SetUpTestCase() {
    NES::Logger::setupLogging("TypeCompilationTest.log",
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down TypeCompilationTest test class.");
  }

  Value<> implicitCastFunction(const Value<>& left,
...
// We should be able to create Values with unsigned ints, but currently we
// cannot.
...
TEST_P(TypeCompilationTest, DISABLED_floatTest) {
  auto executionTrace =
      Nautilus::Tracing::traceFunctionWithReturn([]() { return floatTest(); });
  auto engine = prepare(executionTrace);
...
class CustomType : public Any {
 public:
  ...
  std::shared_ptr<CustomType> mulInt(const Int64& other) const {
    return std::make_unique<CustomType>(x * other.getValue(),
                                        y * other.getValue());
  }
  ...
  std::shared_ptr<Any> copy() override {
    return std::make_shared<CustomType>(x, y);
  }
...
class CustomTypeInvocationPlugin : public InvocationPlugin {
 public:
  ...
  std::optional<Value<>> Mul(const Value<>& left,
                             const Value<>& right) const override {
...
[[maybe_unused]] static InvocationPluginRegistry::Add<
    CustomTypeInvocationPlugin>
    cPlugin;

Value<> customValueType() {
  auto c1 =
      Value<CustomType>(CustomType(Value<Int64>(32_s64), Value<Int64>(32_s64)));
  auto c2 =
      Value<CustomType>(CustomType(Value<Int64>(32_s64), Value<Int64>(32_s64)));
...
TEST_P(TypeCompilationTest, compileListLengthFunctionTest) {
    ...
    auto wc = std::make_shared<Runtime::WorkerContext>(INITIAL<WorkerThreadId>,
bm, 100);

    auto list = RawList(10);
    auto listRef = TypedRef<RawList>(list);
    Value<List> valueList = Value<TypedList<Int32>>(TypedList<Int32>(listRef));
    listRef.value->ref = Nautilus::Tracing::ValueRef(INT32_MAX, 0,
NES::Nautilus::IR::Types::StampFactory::createAddressStamp());
...
/**
 * @brief Simple text function that calls text length, uppercase and manipulates
 * the text content.
 * @param text
 * @return
 */
...
TEST_P(TypeCompilationTest, compileTextFunctionTest) {
  ...
  listRef.value->ref = Nautilus::Tracing::ValueRef(
      INT32_MAX, 0,
      NES::Nautilus::IR::Types::StampFactory::createAddressStamp());

  auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([&]() {
    Nautilus::Tracing::TraceContext::get()->addTraceArgument(
        listRef.value->ref);
...
TEST_P(TypeCompilationTest, castInteger) {
...

  {
    ...
    auto function =
        engine->getInvocableMember<int16_t, int8_t, int16_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i8, i32);
    auto function =
        engine->getInvocableMember<int32_t, int8_t, int32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i8, i64);
    auto function =
        engine->getInvocableMember<int64_t, int8_t, int64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i16, i32);
    auto function =
        engine->getInvocableMember<int32_t, int16_t, int32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i16, i64);
    auto function =
        engine->getInvocableMember<int64_t, int16_t, int64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i32, i64);
    auto function =
        engine->getInvocableMember<int64_t, int32_t, int64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
}
...
TEST_P(TypeCompilationTest, castUInteger) {
...

  {
    ...
    auto function =
        engine->getInvocableMember<uint16_t, uint8_t, uint16_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(ui8, ui32);
    auto function =
        engine->getInvocableMember<uint32_t, uint8_t, int32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(ui8, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, uint8_t, int64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(ui16, ui32);
    auto function =
        engine->getInvocableMember<uint32_t, uint16_t, uint32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(ui16, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, uint16_t, uint64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(ui32, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, uint32_t, uint64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
}
...
TEST_P(TypeCompilationTest, castIntegerToUInteger) {
  ...
  Value<> ui64 = Value<UInt64>(42_u64);
  {
    ...
    auto function =
        engine->getInvocableMember<uint8_t, int8_t, uint8_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i8, ui16);
    auto function =
        engine->getInvocableMember<uint16_t, int8_t, uint16_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i8, ui32);
    auto function =
        engine->getInvocableMember<uint32_t, int8_t, uint32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i8, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, int8_t, uint64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i16, ui16);
    auto function =
        engine->getInvocableMember<uint16_t, int16_t, uint16_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i16, ui32);
    auto function =
        engine->getInvocableMember<uint32_t, int16_t, uint32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i16, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, int16_t, uint64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i32, ui32);
    auto function =
        engine->getInvocableMember<uint32_t, int32_t, uint32_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i32, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, int32_t, uint64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i64, ui64);
    auto function =
        engine->getInvocableMember<uint64_t, int64_t, uint64_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(ui8, i16);
    auto function =
        engine->getInvocableMember<int16_t, uint8_t, int16_t>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
}
...
TEST_P(TypeCompilationTest, castFloat) {
...

  {
    ...
    auto function =
        engine->getInvocableMember<double, float, double>("execute");
  ...
  }
  {
    ...
    auto function =
        engine->getInvocableMember<float, int16_t, float>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i16, doubleV);
    auto function =
        engine->getInvocableMember<double, int16_t, double>("execute");
    ASSERT_EQ(function(42, 42.f), 84.f);
  }
  {
    auto engine = compileCast(i32, floatV);
    auto function =
        engine->getInvocableMember<float, int32_t, float>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
  {
    auto engine = compileCast(i32, doubleV);
    auto function =
        engine->getInvocableMember<double, int32_t, double>("execute");
    ASSERT_EQ(function(42, 42.f), 84.f);
  }
  {
    auto engine = compileCast(i64, floatV);
    auto function =
        engine->getInvocableMember<float, int64_t, float>("execute");
    ASSERT_EQ(function(42, 42.f), 84.f);
  }
  {
    auto engine = compileCast(i64, doubleV);
    auto function =
        engine->getInvocableMember<double, int64_t, double>("execute");
    ASSERT_EQ(function(42, 42), 84);
  }
}

// Tests all registered compilation backends.
// To select a specific compilation backend use ::testing::Values("MLIR")
// instead of ValuesIn.
INSTANTIATE_TEST_CASE_P(
    testTypeCompilation, TypeCompilationTest,
    ::testing::ValuesIn(
        Backends::CompilationBackendRegistry::getPluginNames().begin(),
