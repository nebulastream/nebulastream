...
class RemoveBranchOnlyBlocksPhaseTest : public Testing::BaseUnitTest,
 ...
 public:
  ...
  std::shared_ptr<NES::Nautilus::IR::IRGraph> createTraceAndApplyPhases(
  ...
  /**
   * @brief A helper struct that represents the correct state of a BasicBlock in
   * the IR.
   */
  ...
  /**
   * @brief Create a BasicBlock that represents the correct state of a
   * BasicBlock from a given IR.
   *
   * @param correctBlocks: A HashMap that the newly created correct block is
   * inserted into.
   * @param correctBlockId: The ID of the block that we want to check state
   * correctness for.
   * @param correctPredecessors: A set of predecessor Ids that the checked block
   * must contain.
   * @param correctNextBlocks: A set of IDs of basic blocks that the checked
   * block must point to.
   */
  void createCorrectBlock(
    ...
    correctBlocks.emplace(std::pair{
        correctBlockId, std::make_unique<CorrectBlockValues>(CorrectBlockValues{
                            correctPredecessors, correctNextBlocks})});
  }

  /**
   * @brief Iterate over IR using depth-first-search. Check for every block
   * whether it is in a correct state.
   *
   * @param currentBlock: BasicBlock that is currently checked. Is body-block of
   * root operation at first.
   * @param correctBlocks: A HashMap that contains the correct state for all
   * blocks in the IR.
   * @return true, if all blocks were in correct state, and false, if at least
   * one block was in incorrect state.
   */
  bool checkIRForCorrectness(
    ...
    do {
      ...
      // The remove branch only phase should have removed all branch only blocks
      // leaving only 'correct' blocks.
      if (correctBlocks.contains(currentBlock->getIdentifier())) {
        // If the currentBlock has predecessors, check that the number of
        // predecessors matches the correct number of predecessors and check
        // that all existing predecessors are correct.
        if (correctBlocks.at(currentBlock->getIdentifier())
                ->correctPredecessors.size() ==
            currentBlock->getPredecessors().size()) {
          for (auto predecessorBlock : currentBlock->getPredecessors()) {
            if (!correctBlocks.at(currentBlock->getIdentifier())
                     ->correctPredecessors.contains(
                         predecessorBlock.lock()->getIdentifier())) {
        ...
        // Every Block MUST have a terminator operation. Check whether the
        // terminator operation points to the correct next blocks.
        if (currentBlock->getTerminatorOp()->getOperationType() ==
          ...
          nextBlocksAreCorrect =
              correctBlocks.at(currentBlock->getIdentifier())
                  ->correctNextBlocks.contains(ifOp->getTrueBlockInvocation()
                                                   .getBlock()
                                                   ->getIdentifier()) &&
              correctBlocks.at(currentBlock->getIdentifier())
                  ->correctNextBlocks.contains(ifOp->getFalseBlockInvocation()
                                                   .getBlock()
                                                   ->getIdentifier());
        } else if (currentBlock->getTerminatorOp()->getOperationType() ==
    ...
    } while (predecessorsAreCorrect && nextBlocksAreCorrect &&
             !blocksToVisit.empty());
...
TEST_P(RemoveBranchOnlyBlocksPhaseTest, 0_SimpleIfOperationWithoutFalseBranch) {
  ...
  EXPECT_EQ(checkIRForCorrectness(
                ir->getRootOperation()->getFunctionBasicBlock(), correctBlocks),
            true);
}
...
TEST_P(RemoveBranchOnlyBlocksPhaseTest,
       3_loopMergeBlockBeforeCorrespondingIfOperation) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  // createCorrectBlock(correctBlocks, blockId, predecessors, nextBlocks)
  ...
  auto ir = createTraceAndApplyPhases(
      &loopMergeBlockBeforeCorrespondingIfOperation_3);
  EXPECT_EQ(checkIRForCorrectness(
                ir->getRootOperation()->getFunctionBasicBlock(), correctBlocks),
            true);
}
...
INSTANTIATE_TEST_CASE_P(
    testLoopCompilation, RemoveBranchOnlyBlocksPhaseTest,
    ::testing::ValuesIn(
        Backends::CompilationBackendRegistry::getPluginNames().begin(),
        Backends::CompilationBackendRegistry::getPluginNames().end()),
    [](const testing::TestParamInfo<RemoveBranchOnlyBlocksPhaseTest::ParamType>&
