...
public:
    ...
    bool checkIRForCorrectness(
        const std::vector<IR::BasicBlockPtr>& dpsSortedBlocks, const std::unordered_map<std::string, CorrectBlockValuesPtr>& correctBlocks)
    {
        ...
        for (auto currentBlock : dpsSortedBlocks)
        {
            if (currentBlock->getTerminatorOp()->getOperationType() == IR::Operations::Operation::OperationType::IfOp)
            {
                ...
                if (correctBlocks.contains(currentBlock->getIdentifier()))
                {
                    ...
                    if (!correctMergeBlockId.empty())
                    {
                        ...
                        {
                            NES_ERROR(
                                "CurrentBlock: {} did not contain a merge block even though the solution suggest it has a "
                                "merge-block with id: {}",
                    ...
                    else
                    {
                        ...
                        {
                            NES_ERROR(
                                "The current merge block: {} contains a merge-block with id: {}, even though it should "
            ...
            else if (currentBlock->getTerminatorOp()->getOperationType() == IR::Operations::Operation::OperationType::LoopOp)
            {
                ...
                if (correctBlocks.contains(currentBlock->getIdentifier()))
                {
                    if (correctBlocks.at(currentBlock->getIdentifier())->countedLoopInfo)
                    {
                        ...
                        {
                            NES_ERROR("Loop operation in block: {} should -not- contain counted loop info.", currentBlock->getIdentifier());
                        }
                        else
                        {
                            ...
                            loopInfoIsCorrect
                                &= countedLoopInfo->stepSize == correctBlocks.at(currentBlock->getIdentifier())->countedLoopInfo->stepSize;
                            ...
                            {
                                NES_ERROR(
                                    "Loop info set incorrectly. Check values: LowerBound: {} vs {}, UpperBound: {} vs {}, "
            ...
            else
            {
                if (correctBlocks.contains(currentBlock->getIdentifier()))
                {
                    mergeBlocksAreCorrect = false;
                    NES_ERROR(
                        "CurrentBlock with id: {} was part of solution set(correctBlocks), but it does not contain an "
                        "if-operation.",
                        currentBlock->getIdentifier());
                }
            }
        ...
        {
            NES_ERROR(
                "The number of checked IR blocks {} does not match the number of given 'correctBlocks': {}",
                numCheckedBlocks,
                correctBlocks.size());
        }
