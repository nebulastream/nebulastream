...
class StructuredControlFlowPhaseTest : public testing::Test,
 ...
 public:
  ...
  std::vector<IR::BasicBlockPtr> createTraceAndApplyPhases(
      std::function<Value<>()> nautilusFunction) {
   ...
   */
  std::vector<IR::BasicBlockPtr> enumerateIRForTests(
  ...
  bool checkIRForCorrectness(
    ...
    for (auto currentBlock : dpsSortedBlocks) {
      if (currentBlock->getTerminatorOp()->getOperationType() ==
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          ...
          if (!correctMergeBlockId.empty()) {
            if (!ifOp->getMergeBlock()) {
              NES_ERROR(
                  "CurrentBlock: {} did not contain a merge block even though "
                  "the solution suggest it has a "
                  "merge-block with id: {}",
            ...
            if (!noMergeBlockCorrectlySet) {
              NES_ERROR(
                  "The current merge block: {} contains a merge-block with id: "
          ...
          mergeBlocksAreCorrect = false;
          NES_ERROR(
              "CurrentBlock with id: {} was not part of solution "
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          if (correctBlocks.at(currentBlock->getIdentifier())
            ...
            if (!loopInfoIsCorrect) {
              NES_ERROR(
                  "Loop operation in block: {} should -not- contain counted "
                  "loop info.",
                  currentBlock->getIdentifier());
            } else {
              ...
              if (!loopInfoIsCorrect) {
                NES_ERROR(
                    "Loop info set incorrectly. Check values: LowerBound: {} "
                    "vs {}, UpperBound: {} vs {}, "
            ...
            if (!loopIsDefaultLoop) {
              NES_ERROR(
                  "\n Loop operation in block: {} should be default loop, but "
                  "is not.",
                  currentBlock->getIdentifier());
            }
          }
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          mergeBlocksAreCorrect = false;
          NES_ERROR(
              "CurrentBlock with id: {} was part of solution "
    ...
    if (!numCheckedBlocksMatchesNumCorrectBlocks) {
      NES_ERROR(
          "The number of checked IR blocks {} does not match the number of "
          "given 'correctBlocks': {}",
          numCheckedBlocks, correctBlocks.size());
    }
...
Value<> NonCountedLoopCases_22() {
  ...
  while (lowerBound_1 < upperBound_2) {  //<- using lowerBound_1, which is
                                         //passed through several blocks before.
    agg = agg + 1;
