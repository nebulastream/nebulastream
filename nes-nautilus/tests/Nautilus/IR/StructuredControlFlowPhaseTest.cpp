...
/**
 * @brief The StructuredControlFlowPhaseTest contains a battery of tests that
 * all do the following:
 *          -> they call a unique(in the scope of this test) Nautilus function
 * with the same name as the test
 *          -> they apply symbolic execution to the Nautilus function to get a
 * function trace
 *          -> the function trace passes the SSA-, and the TraceToIRConversion
 * phases, which generates an IR graph
 *          -> the IR graph passes the AddPredecessor- and Remove-Branch-Only
 * phases, which operate on the IR graph
 *          -> then, the IR graph passes the FindLoopHeader- and the
 * CreateIfOperation phases
 *          -> Lastly, we iterate over the resulting IR graph and check whether
 * loop blocks have been marked as loop blocks, and whether if-operations have
 * been matched with their correct corresponding merge-blocks
 */
class StructuredControlFlowPhaseTest : public testing::Test,
...
public:
  ...
  void SetUp() override {
    std::cout << "Setup TraceTest test case." << std::endl;
  }

  /* Will be called before a test is executed. */
  void TearDown() override {
    std::cout << "Tear down TraceTest test case." << std::endl;
  }

  /* Will be called after all tests in this class are finished. */
  static void TearDownTestCase() {
    std::cout << "Tear down TraceTest test class." << std::endl;
  }

  // Takes a Nautilus function, creates the trace, converts it Nautilus IR, and
  // applies all available phases.
  std::vector<IR::BasicBlockPtr>
  createTraceAndApplyPhases(std::function<Value<>()> nautilusFunction) {
    auto execution = Nautilus::Tracing::traceFunctionWithReturn(
        [nautilusFunction]() { return nautilusFunction(); });
   ...
   * @param stepSize: The step size by which the loop induction variable is
   * increased in each step.
   * @param loopEndBlockId: The id of the block that loops back to the
   * loop-header, and passes the final values.
   * @return countedLoopInfoPtr
   */
  countedLoopInfoPtr createCorrectCountedLoopInfo(uint32_t lowerBound,
                                                  uint32_t upperBound,
                                                  uint32_t stepSize,
                                                  std::string loopEndBlockId) {
    return std::make_unique<CountedLoopInfo>(
        CountedLoopInfo{lowerBound, upperBound, stepSize, loopEndBlockId});
  }
  ...
  /**
   * @brief Create a CorrectBlock, which holds the expected data for exactly one
   * block.
   *
   * @param correctBlocks: A list into which we push the newly created
   * 'correctBlock'
   * @param correctBlockId: The id of the block that we create the expected to
   * be correct values for.
   * @param correctNumberOfLoopBackEdges: The number of expected loop back
   * edges.
   * @param correctMergeBlockId: The expected merge-block id (if block contains
   * if-operation).
   * @param countedLoopInfo: The expected countedLoopInfo(lower- and upperBound,
   * stepSize, loopEndBlockId)
   */
  void createCorrectBlock(
    ...
    correctBlocks.emplace(std::pair{
        correctBlockId, std::make_unique<CorrectBlockValues>(CorrectBlockValues{
                            correctNumberOfLoopBackEdges,
                            std::move(countedLoopInfo), correctMergeBlockId})});
  }

  /**
   * @brief Takes a graph IR, enumerates all blocks depth-first, and returns a
   * vector containing all blocks.
   *
   * @param ir: Graph ir that is traversed depth-first-search (dps).
   * @return std::vector<IR::BasicBlockPtr>: A vector containing all blocks of
   * the ir-graph sorted by dps.
   */
  std::vector<IR::BasicBlockPtr>
  enumerateIRForTests(std::shared_ptr<IR::IRGraph> ir) {
    ...
    do {
      ...
      newBlocks.pop();
      if (nextBlocks.second &&
          !visitedBlocks.contains(nextBlocks.first.get())) {
  ...
  /**
   * @brief Takes a list of blocks, and a map that maps the basic block
   * identifiers to a data structure that holds the expected, correct values for
   * the respective block. The function iterates over all blocks, and checks
   * whether the basic blocks reflect the expected values.
   *
   * @param dpsSortedBlocks An IR graph represented as a list of blocks (nodes).
   * @param correctBlocks A map that maps blocks to a data structure that holds
   * the expected values for the blocks.
   * @return true, if all values in all blocks met the expected values, else,
   * return false.
   */
  bool checkIRForCorrectness(
      const std::vector<IR::BasicBlockPtr> &dpsSortedBlocks,
    ...
    for (auto currentBlock : dpsSortedBlocks) {
      if (currentBlock->getTerminatorOp()->getOperationType() ==
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          ...
          // Check that the merge-block id is set correctly, if the if-operation
          // has a merge-block.
          auto correctMergeBlockId =
              correctBlocks.at(currentBlock->getIdentifier())
                  ->correctMergeBlockId;
          if (!correctMergeBlockId.empty()) {
            if (!ifOp->getMergeBlock()) {
              NES_ERROR("CurrentBlock: {} did not contain a merge block even "
                        "though the solution suggest it has a "
                        "merge-block with id: {}",
                        currentBlock->getIdentifier(), correctMergeBlockId);
              mergeBlocksAreCorrect = false;
            } else {
              bool correctMergeBlock =
                  ifOp->getMergeBlock()->getIdentifier() == correctMergeBlockId;
              mergeBlocksAreCorrect &= correctMergeBlock;
              if (!correctMergeBlock) {
                NES_ERROR("\nMerge-Block mismatch for block {}: {} instead of "
                          "{} (correct).",
            ...
            if (!noMergeBlockCorrectlySet) {
              NES_ERROR("The current merge block: {} contains a merge-block "
                        "with id: {}, even though it should "
          ...
          mergeBlocksAreCorrect = false;
          NES_ERROR("CurrentBlock with id: {} was not part of solution "
                    "set(correctBlocks), but it contains an if-operation.",
                    currentBlock->getIdentifier());
        }
        ++numCheckedBlocks;
        ...
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          if (correctBlocks.at(currentBlock->getIdentifier())
            ...
            if (!loopInfoIsCorrect) {
              NES_ERROR("Loop operation in block: {} should -not- contain "
                        "counted loop info.",
                        currentBlock->getIdentifier());
            } else {
              auto countedLoopInfo =
                  std::static_pointer_cast<IR::Operations::CountedLoopInfo>(
                      loopOp->getLoopInfo());
              loopInfoIsCorrect &=
              ...
              loopInfoIsCorrect &=
                  ...
                  correctBlocks.at(currentBlock->getIdentifier())
                      ->countedLoopInfo->loopEndBlockId;
              if (!loopInfoIsCorrect) {
                NES_ERROR("Loop info set incorrectly. Check values: "
                          "LowerBound: {} vs {}, UpperBound: {} vs {}, "
            ...
            bool loopIsDefaultLoop =
                (loopOp->getLoopType() ==
                 IR::Operations::LoopOperation::LoopType::DefaultLoop);
            loopInfoIsCorrect &= loopIsDefaultLoop;
            if (!loopIsDefaultLoop) {
              NES_ERROR("\n Loop operation in block: {} should be default "
                        "loop, but is not.",
                        currentBlock->getIdentifier());
            }
          }
          ...
          mergeBlocksAreCorrect = false;
          NES_ERROR("CurrentBlock with id: {} was not part of solution "
                    "set(correctBlocks), but it contains a loop-operation.",
                    currentBlock->getIdentifier());
        }
        ++numCheckedBlocks;
      } else {
        if (correctBlocks.contains(currentBlock->getIdentifier())) {
          mergeBlocksAreCorrect = false;
          NES_ERROR("CurrentBlock with id: {} was part of solution "
                    "set(correctBlocks), but it does not contain an "
                    "if-operation.",
                    currentBlock->getIdentifier());
        }
      }
    }
    bool numCheckedBlocksMatchesNumCorrectBlocks =
        numCheckedBlocks == correctBlocks.size();
    if (!numCheckedBlocksMatchesNumCorrectBlocks) {
      NES_ERROR("The number of checked IR blocks {} does not match the number "
                "of given 'correctBlocks': {}",
                numCheckedBlocks, correctBlocks.size());
    }
...
TEST_P(StructuredControlFlowPhaseTest, 2_doubleVerticalDiamondInTrueBranch) {
  ...
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&doubleVerticalDiamondInTrueBranch_2);
...
TEST_P(StructuredControlFlowPhaseTest,
       3_doubleHorizontalDiamondWithOneMergeBlockThatAlsoIsIfBlock) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  ...
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &doubleHorizontalDiamondWithOneMergeBlockThatAlsoIsIfBlock_3);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
TEST_P(StructuredControlFlowPhaseTest,
       4_oneMergeBlockThatClosesOneIfAndBecomesMergeForTwo) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  createCorrectBlock(correctBlocks, "0", 0, "5");
  createCorrectBlock(correctBlocks, "1", 0, "5");
  createCorrectBlock(correctBlocks, "6", 0, "5");
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &oneMergeBlockThatClosesOneIfAndBecomesMergeForTwo_4);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
TEST_P(
  ...
  createCorrectBlock(correctBlocks, "6", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 1, "7"));
...
TEST_P(StructuredControlFlowPhaseTest,
       6_oneMergeBlockThatClosesOneIfAndBecomesMergeForTwoAndIsLoopHeader) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  createCorrectBlock(correctBlocks, "0", 0, "5");
  createCorrectBlock(correctBlocks, "1", 0, "5");
  createCorrectBlock(correctBlocks, "6", 1, "",
                     createCorrectCountedLoopInfo(1, 1000000, 2, "7"));
  createCorrectBlock(correctBlocks, "9", 0, "5");
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &oneMergeBlockThatClosesOneIfAndBecomesMergeForTwoAndIsLoopHeader_6);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
TEST_P(StructuredControlFlowPhaseTest,
       7_loopMergeBlockBeforeCorrespondingIfOperation) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  createCorrectBlock(correctBlocks, "2", 2, "");
  createCorrectBlock(correctBlocks, "3", 0, "2");
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &loopMergeBlockBeforeCorrespondingIfOperation_7);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
TEST_P(StructuredControlFlowPhaseTest, 8_mergeLoopMergeBlockWithLoopFollowUp) {
  ...
  createCorrectBlock(correctBlocks, "9", 1, "",
                     createCorrectCountedLoopInfo(0, 1000, 1, "10"));
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&mergeLoopMergeBlockWithLoopFollowUp_8);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
TEST_P(StructuredControlFlowPhaseTest, 9_loopHeaderWithNineBackLinks) {
  ...
  createCorrectBlock(correctBlocks, "34", 1, "",
                     createCorrectCountedLoopInfo(0, 1000, 2, "35"));
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&LoopHeaderWithNineBackLinks_9);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
Value<>
IfOperationFollowedByLoopWithDeeplyNestedIfOperationsWithSeveralNestedLoops_11() {
  Value agg = Value(0);
  Value limit = Value(1000000);
  if (agg < 150) {
    agg = agg + 1;
...
TEST_P(
    StructuredControlFlowPhaseTest,
    11_IfOperationFollowedByLoopWithDeeplyNestedIfOperationsWithSeveralNestedLoops) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  createCorrectBlock(correctBlocks, "0", 0, "3");
  createCorrectBlock(correctBlocks, "4", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 1, "13"));
...
TEST_P(StructuredControlFlowPhaseTest, 12_emptyIfElse) {
  ...
  ASSERT_EQ(convertedIfOperation->getOperationType(),
  ...
  ASSERT_EQ(branchOp->getNextBlockInvocation().getBlock()->getIdentifier(),
            "2");
}
...
TEST_P(StructuredControlFlowPhaseTest, 13_MergeBlockRightAfterBranchSwitch) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&MergeBlockRightAfterBranchSwitch_13);
  createCorrectBlock(correctBlocks, "0", 0, "2");
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
TEST_P(StructuredControlFlowPhaseTest,
       15_correctMergeBlockForwardingAfterFindingMergeBlocks) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &correctMergeBlockForwardingAfterFindingMergeBlocksOne_15);
  createCorrectBlock(correctBlocks, "0", 0, "2");
  createCorrectBlock(correctBlocks, "3", 0, "5");
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
  correctBlocks.clear();
  dpsSortedBlocks = createTraceAndApplyPhases(
      &correctMergeBlockForwardingAfterFindingMergeBlocksTwo_15);
  createCorrectBlock(correctBlocks, "0", 0, "3");
...
Value<>
OneMergeBlockThreeIfOperationsFalseBranchIntoTrueBranchIntoFalseBranch_16() {
  Value agg = Value(0);
  if (agg < 50) {
    agg = agg + 1;
  } else {
...
TEST_P(
    StructuredControlFlowPhaseTest,
    16_OneMergeBlockThreeIfOperationsFalseBranchIntoTrueBranchIntoFalseBranch) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &OneMergeBlockThreeIfOperationsFalseBranchIntoTrueBranchIntoFalseBranch_16);
  createCorrectBlock(correctBlocks, "0", 0, "4");
...
TEST_P(StructuredControlFlowPhaseTest,
       17_NestedLoopWithFalseBranchPointingToParentLoopHeader) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks = createTraceAndApplyPhases(
      &NestedLoopWithFalseBranchPointingToParentLoopHeader_17);
  createCorrectBlock(correctBlocks, "2", 2, "");
  createCorrectBlock(correctBlocks, "3", 0, "2");
  createCorrectBlock(correctBlocks, "6", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 1, "7"));
...
TEST_P(StructuredControlFlowPhaseTest, 18_InterruptedMergeBlockForwarding) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&InterruptedMergeBlockForwarding_18);
  createCorrectBlock(correctBlocks, "0", 0, "4");
  createCorrectBlock(correctBlocks, "5", 0, "4");
...
Value<> TracingBreaker_19() {
  ...
  if (agg < 350) {
    ...
    if (agg < 350) {
      if (agg < 350) { // the 'false' case of this if this if-operation has no
                       // operations -> Block_9
        agg = agg + 1;
      } else {
...
Value<> ExtensiveCountedLoopDetection_21() {
  Value agg = Value(2);
  // 1. Standard for loop.
  for (Value lowerBound = Value(0); lowerBound < 10;
       lowerBound = lowerBound + 1) {
    agg = agg + 1;
  }
  // 2. for loop, but condition is flipped.
  for (Value lowerBound = Value(0); 10 > lowerBound;
  ...
  // 6. for loop, but the induction variable, and the upperBound are defined
  // outside of the loop.
  ...
  // 7. for loop, but the induction variable, and the upperBound are defined
  // outside of the loop,
  ...
  // 10. while loop counted, but upperBound is defined outside, and condition is
  // flipped
...
TEST_P(StructuredControlFlowPhaseTest, 21_ExtensiveCountedLoopDetection) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&ExtensiveCountedLoopDetection_21);
  createCorrectBlock(correctBlocks, "1", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 1, "2"));
  createCorrectBlock(correctBlocks, "4", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 2, "5"));
  createCorrectBlock(correctBlocks, "7", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 3, "8"));
  createCorrectBlock(correctBlocks, "10", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 4, "11"));
  createCorrectBlock(correctBlocks, "13", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 5, "14"));
  createCorrectBlock(correctBlocks, "16", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 6, "17"));
  createCorrectBlock(correctBlocks, "19", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 7, "20"));
  createCorrectBlock(correctBlocks, "22", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 8, "23"));
  createCorrectBlock(correctBlocks, "25", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 9, "26"));
  createCorrectBlock(correctBlocks, "28", 1, "",
                     createCorrectCountedLoopInfo(0, 10, 10, "29"));
...
Value<> NonCountedLoopCases_22() {
  ...
  while (lowerBound_1 < upperBound_2) { //<- using lowerBound_1, which is passed
                                        //through several blocks before.
    agg = agg + 1;
...
Value<> twoCountedLoopsOneLargeOneSmall_23() {
  Value agg = Value(0);
  Value upperBound1 = Value(1000);
  for (Value inductionVar1 = Value(0); inductionVar1 < upperBound1;
       inductionVar1 = inductionVar1 + 1) {
    if (agg < 350) {
      if (agg < 350) {
        if (agg < 350) {
          agg = agg + 1;
...
TEST_P(StructuredControlFlowPhaseTest, twoCountedLoopsOneLargeOneSmall) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  createCorrectBlock(correctBlocks, "1", 1, "",
                     createCorrectCountedLoopInfo(0, 1000, 1, "15"));
  ...
  createCorrectBlock(correctBlocks, "34", 1, "",
                     createCorrectCountedLoopInfo(0, 1000, 2, "35"));
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&twoCountedLoopsOneLargeOneSmall_23);
  ASSERT_EQ(checkIRForCorrectness(dpsSortedBlocks, correctBlocks), true);
}
...
Value<> DetectCountedLoopsWithEqualInComparison_23() {
  ...
  for (Value inductionVar_1 = Value(0); inductionVar_1 <= 10;
       inductionVar_1 = inductionVar_1 + 1) {
    agg = agg + 1;
  }
  // 2. Counted for loop with smaller equals in condition, but condition is
  // flipped.
  ...
  // 4. Counted for loop with smaller equals condition, but condition is complex
  // (not supported).
  Value inductionVar_4 = Value(0);
  Value otherBound_4 = Value(1);
  for (; (inductionVar_4 + 10) <= (inductionVar_4 - otherBound_4);
       inductionVar_4 = inductionVar_4 + 1) {
    agg = agg + 1;
  }
  // 5. Smaller equals, but the comparison order of '<=', which is (<,==,or) is
  // faked.
  Value otherBound_5 = Value(1);
  for (Value lowerBound = Value(0);
       lowerBound < 10 || lowerBound == otherBound_5;
       lowerBound = lowerBound + 1) {
    agg = agg + 1;
  }
  // 6. Counted for loop not equals(!=) in condition (not supported).
  for (Value inductionVar_6 = Value(0); inductionVar_6 != 10;
       inductionVar_6 = inductionVar_6 + 1) {
    agg = agg + 1;
  }
  // 7. Counted for loop with equals (==) in condition (not supported).
  for (Value inductionVar_8 = Value(0); inductionVar_8 == 10;
       inductionVar_8 = inductionVar_8 + 1) {
    agg = agg + 1;
  }
  return agg;
}
TEST_P(StructuredControlFlowPhaseTest,
       23_DetectCountedLoopsWithEqualInComparison) {
  std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
  auto dpsSortedBlocks =
      createTraceAndApplyPhases(&DetectCountedLoopsWithEqualInComparison_23);
  createCorrectBlock(correctBlocks, "1", 1, "",
                     createCorrectCountedLoopInfo(0, 11, 1, "2"));
  createCorrectBlock(correctBlocks, "4", 1, "",
                     createCorrectCountedLoopInfo(0, 11, 2, "5"));
  createCorrectBlock(correctBlocks, "7", 1, "",
                     createCorrectCountedLoopInfo(0, 11, 3, "8"));
  createCorrectBlock(correctBlocks, "10", 1, "");
  createCorrectBlock(correctBlocks, "13", 1, "");
  createCorrectBlock(correctBlocks, "16", 1, "");
  createCorrectBlock(correctBlocks, "19", 1, "");
...
INSTANTIATE_TEST_CASE_P(
    testLoopCompilation, StructuredControlFlowPhaseTest,
    ::testing::ValuesIn(
        Backends::CompilationBackendRegistry::getPluginNames().begin(),
        Backends::CompilationBackendRegistry::getPluginNames().end()),
    [](const testing::TestParamInfo<StructuredControlFlowPhaseTest::ParamType>
