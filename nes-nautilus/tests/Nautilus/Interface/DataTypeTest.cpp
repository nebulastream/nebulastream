...
public:
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down DataTypeTest test class.");
  }
};
...
TEST_F(DataTypeTest, ConstructValueTest) {
  ...
  ASSERT_EQ(f1, 42_s8);
  ASSERT_EQ(
      cast<IR::Types::IntegerStamp>(f1.value->getType())->getNumberOfBits(), 8);

  // construct by rvalue
  auto f2 = Value<>(Int8(42));
  ASSERT_EQ(f2.as<Int8>()->getValue(), 42_s8);
  ASSERT_EQ(
      cast<IR::Types::IntegerStamp>(f2.value->getType())->getNumberOfBits(), 8);
  ...
  ASSERT_EQ(
      cast<IR::Types::IntegerStamp>(f3.value->getType())->getNumberOfBits(), 8);

  // construct by shared ptr
  auto f4 = Value<>(std::make_shared<Int8>(42));
  ASSERT_EQ(f4.as<Int8>()->getValue(), 42_s8);
  ASSERT_EQ(
      cast<IR::Types::IntegerStamp>(f4.value->getType())->getNumberOfBits(), 8);

  // construct by assignment to any
  Value<> f5(42_s8);
  ASSERT_EQ(f5.as<Int8>()->getValue(), 42_s8);
  ASSERT_EQ(
      cast<IR::Types::IntegerStamp>(f5.value->getType())->getNumberOfBits(), 8);

  // construct by assignment to typed
  Value<Int8> f6(42_s8);
  ASSERT_EQ(f6.as<Int8>()->getValue(), 42_s8);
  ASSERT_EQ(
      cast<IR::Types::IntegerStamp>(f6.value->getType())->getNumberOfBits(), 8);
}
...
TEST_F(DataTypeTest, AssignmentValueTest) {

  {
    ...
    ASSERT_EQ(
        cast<IR::Types::IntegerStamp>(a.value->getType())->getNumberOfBits(),
        8);
  }
...
using TestIdentifierOther =
    NESStrongType<int32_t, struct TestIdentifierOther_, 0, 1>;
...
TEST_F(DataTypeTest, IdentifierTest) {
  ...
  ASSERT_EQ(cast<IR::Types::IntegerStamp>(stamp)->getSignedness(),
