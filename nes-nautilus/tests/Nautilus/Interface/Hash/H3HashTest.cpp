...
public:
  ...
  static void TearDownTestCase() {
    NES_INFO("H3HashTest test class TearDownTestCase.");
  }
};

/**
 * @brief Gets a bitmask for a given number of bits, for example 8 --> 0xFF and
 * 32 -> 0xFFFF
...
uint64_t getBitMask(uint64_t numberOfKeyBits) {
  ...
  default:
    NES_THROW_RUNTIME_ERROR(
        "getBitMask got a numberOfKeyBits that was not expected!");
  }
}
...
TEST_F(H3HashTest, simpleH3testDouble) {
  ...
  for (uint64_t key = 0; key < NUMBER_OF_KEYS_TO_TEST; ++key) {
    // To use the same hashes as for the uint64_t, we have to memcpy it into a
    // double
    double keyDouble;
    ...
    for (auto row = 0UL; row < NUMBER_OF_ROWS; ++row) {
      ...
      auto calcHash = allH3Hashes[row]
                          ->calculateWithState(valKey, h3SeedMemRef)
                          .getValue()
                          .getValue();
...
TEST_F(H3HashTest, simpleH3testFloat) {
  ...
  for (uint32_t key = 0; key < NUMBER_OF_KEYS_TO_TEST; ++key) {
    // To use the same hashes as for the uint32_t, we have to memcpy it into a
    // double
    float keyFloat;
...
TEST_F(H3HashTest, simpleH3testUInt16) {
  std::vector<std::array<uint16_t, NUMBER_OF_ROWS>> expectedHashes = {
      {0x0, 0x0, 0x0},          {0x3db3, 0x98bb, 0xe2c5},
      {0x950e, 0xe1ec, 0x159a}, {0xa8bd, 0x7957, 0xf75f},
      {0xd447, 0x949c, 0xd13d}, {0xe9f4, 0x0c27, 0x33f8}};
...
TEST_F(H3HashTest, simpleH3testUInt8) {
  std::vector<std::array<uint8_t, NUMBER_OF_ROWS>> expectedHashes = {
      {0x00, 0x00, 0x00}, {0xb3, 0xbb, 0xc5}, {0x0e, 0xec, 0x9a},
      {0xbd, 0x57, 0x5f}, {0x47, 0x9c, 0x3d}, {0xf4, 0x27, 0xf8}};
