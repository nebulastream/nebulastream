...
public:
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down FixedArrayTest test class.");
  }
};
...
TEST_F(Fixed2DArrayTest, insertSimpleDataTypes) {
  ...
  Fixed2DArrayRef fixed2DArrayRef(Value<MemRef>((int8_t *)&fixed2DArray),
                                  entrySize, numCols);
...
TEST_F(Fixed2DArrayTest, insertCustomClass) {
  ...
  for (auto row = 0_u64; row < numRows; row = row + 1) {
    for (auto col = 0_u64; col < numCols; col = col + 1) {
      auto cellCustomClass = static_cast<CustomClass *>(
          fixed2DArrayRef[row][col].getValue().getValue());
