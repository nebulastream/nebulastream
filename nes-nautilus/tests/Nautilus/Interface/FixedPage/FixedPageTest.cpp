...
struct __attribute__((packed)) CustomClass {
  CustomClass(uint64_t id, int32_t val1, double val2)
      : id(id), val1(val1), val2(val2){};
...
public:
  std::unique_ptr<std::pmr::memory_resource> allocator =
      std::make_unique<Runtime::NesDefaultMemoryAllocator>();
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down FixedPageTest test class.");
  }

  /**
   * @brief Stores all items on the FixedPage and verifies the number of items
   * on the FixedPage
   * @tparam Item
   * @param fixedPage
   * @param allItems
  ...
  void runStoreTest(FixedPage &fixedPage, const std::vector<Item> &allItems,
  ...
  /**
   * @brief Iterates through all entries on the FixedPage and verifies their
   * content
   * @tparam Item
   * @param fixedPage
   * @param allItems
   ...
   */
  template <typename Item>
  void runRetrieveTest(FixedPage &fixedPage, const std::vector<Item> &allItems,
                       const uint64_t sizeOfRecord,
...
TEST_F(FixedPageTest, storeAndRetrieveValues) {
  ...
  std::generate_n(std::back_inserter(allItems), numItems,
...
TEST_F(FixedPageTest,
       storeAndRetrieveTooManyValuesForOnePageNonDefaultPageSize) {
  const auto sizeOfRecord = sizeof(uint64_t);
  const auto pageSize = sizeOfRecord;
...
TEST_F(FixedPageTest,
       storeAndRetrieveTooManyValuesForOnePageNonDefaultDataType) {
  const auto sizeOfRecord = sizeof(CustomClass);
  const auto pageSize = FixedPage::PAGE_SIZE;
...
TEST_F(FixedPageTest,
       storeAndRetrieveTooManyValuesForOnePageNonDefaultPageSizeAndDataType) {
  const auto sizeOfRecord = sizeof(CustomClass);
  const auto pageSize = 10 * sizeOfRecord;
...
TEST_F(FixedPageTest, bloomFilterCheckTest) {
  ...
  for (uint64_t i = 0; i < numItems; ++i) {
    ...
    if (fixedPage.append(hash) == nullptr) {
      NES_ERROR("Could not insert tuple and thus the hash {} for {} in the "
                "FixedPage and consequently, the BloomFilter!",
                hash, i);
      ASSERT_TRUE(false);
    }
  }
