...
public:
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down PagedVectorTest test class.");
  }

  template <typename Item>
  void runStoreTest(PagedVector &pagedVector, const uint64_t entrySize,
                    const uint64_t pageSize, const std::vector<Item> &allItems,
    ...
    auto pagedVectorRef =
        PagedVectorRef(Value<MemRef>((int8_t *)&pagedVector), entrySize);
  ...
  template <typename Item>
  void runRetrieveTest(const PagedVector &pagedVector, const uint64_t entrySize,
  ...
  void insertAndAppendAllPages(
    ...
    for (auto &allItems : allItemsAndVectors) {
      allPagedVectors.emplace_back(std::make_unique<PagedVector>(
          std::make_unique<Runtime::NesDefaultMemoryAllocator>(), entrySize,
          pageSize));
      runStoreTest<Item>(*allPagedVectors.back(), entrySize, pageSize, allItems,
                         allItems.size());
    ...
    // After we have appended all paged and everything except the first one, we
    // expect the size to be one
    EXPECT_EQ(allPagedVectors.size(), 1);
    runRetrieveTest<Item>(*firstPagedVec, entrySize,
                          expectedItemsAfterAppendAll);
  }
};
...
TEST_F(PagedVectorTest, storeAndRetrieveValues) {
  ...
  runStoreTest<uint64_t>(pagedVector, entrySize, pageSize, allItems,
                         allItems.size());
...
TEST_F(PagedVectorTest, storeAndRetrieveValuesWithCustomItems) {
  ...
  PagedVector pagedVector(std::move(allocator), entrySize, pageSize);
  runStoreTest<CustomClass>(pagedVector, entrySize, pageSize, allItems,
                            allItems.size());
...
TEST_F(PagedVectorTest, appendAllPagesTwoVectors) {
  ...
  std::generate_n(std::back_inserter(allItemsVec1), numItems,
                  [n = 0]() mutable { return n++; });
  std::generate_n(std::back_inserter(allItemsVec2), numItems,
                  [n = allItemsVec1.size()]() mutable { return n++; });
  allItemsAfterAppend.insert(allItemsAfterAppend.end(), allItemsVec1.begin(),
                             allItemsVec1.end());
  allItemsAfterAppend.insert(allItemsAfterAppend.end(), allItemsVec2.begin(),
                             allItemsVec2.end());

  insertAndAppendAllPages<uint64_t>(
      entrySize, pageSize, {allItemsVec1, allItemsVec2}, allItemsAfterAppend);
}
...
TEST_F(PagedVectorTest, appendAllPagesMultipleVectors) {
  ...
  std::vector<uint64_t> allItemsVec1, allItemsVec2, allItemsVec3, allItemsVec4,
      allItemsAfterAppend;
  std::generate_n(std::back_inserter(allItemsVec1), numItems,
                  [n = 0]() mutable { return n++; });
  std::generate_n(std::back_inserter(allItemsVec2), numItems,
                  [n = allItemsVec1.size()]() mutable { return n++; });
  std::generate_n(std::back_inserter(allItemsVec3), numItems,
                  [n = allItemsVec2.size()]() mutable { return n++; });
  std::generate_n(std::back_inserter(allItemsVec4), numItems,
                  [n = allItemsVec3.size()]() mutable { return n++; });

  allItemsAfterAppend.insert(allItemsAfterAppend.end(), allItemsVec1.begin(),
                             allItemsVec1.end());
  allItemsAfterAppend.insert(allItemsAfterAppend.end(), allItemsVec2.begin(),
                             allItemsVec2.end());
  allItemsAfterAppend.insert(allItemsAfterAppend.end(), allItemsVec3.begin(),
                             allItemsVec3.end());
  allItemsAfterAppend.insert(allItemsAfterAppend.end(), allItemsVec4.begin(),
                             allItemsVec4.end());
