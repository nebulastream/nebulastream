...
class PagedVectorVarSizedTest : public Testing::BaseUnitTest {
 public:
  ...
  static void TearDownTestCase() {
    NES_INFO("Tear down PagedVectorVarSizedTest test class.");
  }

  std::vector<Record> createRecords(const SchemaPtr& schema,
    ...
    for (auto i = 0_u64; i < numRecords; ++i) {
      ...
      for (auto& field : allFields) {
        ...
        if (fieldType->isText()) {
          auto buffer =
              bufferManager->getUnpooledBuffer(PagedVectorVarSized::PAGE_SIZE);
          if (buffer.has_value()) {
            std::stringstream ss;
            ss << "testing TextValue" << tupleNo;
            auto textValue =
                TextValue::create(buffer.value(), ss.str().length());
  ...
  void runStoreTest(PagedVectorVarSized& pagedVector, const SchemaPtr& schema,
                    const uint64_t entrySize, const uint64_t pageSize,
    ...
    const uint64_t numberOfPages =
        std::ceil((double)expectedNumberOfEntries / capacityPerPage);
    ...
    // As we do lazy allocation, we do not create a new page if the last tuple
    // fit on the page
    bool lastTupleFitsOntoLastPage =
        (expectedNumberOfEntries % capacityPerPage) == 0;
    const uint64_t numTuplesLastPage =
        lastTupleFitsOntoLastPage ? capacityPerPage
                                  : (expectedNumberOfEntries % capacityPerPage);
    ASSERT_EQ(pagedVector.getNumberOfEntriesOnCurrentPage(), numTuplesLastPage);
  }

  void runRetrieveTest(PagedVectorVarSized& pagedVector,
  ...
  void insertAndAppendAllPagesTest(
      const SchemaPtr& schema, const uint64_t entrySize, uint64_t pageSize,
    ...
    for (auto& allRecords : allRecordsAndVectors) {
      ...
      allPagedVectors.emplace_back(std::make_unique<PagedVectorVarSized>(
          bufferManager, schema, pageSize));
      runStoreTest(*allPagedVectors.back(), schema, entrySize, pageSize,
                   allRecords);
    ...
    EXPECT_EQ(firstPagedVec->getVarSizedDataEntryMapCounter(),
              totalNumTextFields);
...
TEST_F(PagedVectorVarSizedTest, appendAllPagesTwoVectors) {
  ...
  for (auto i = 0_u64; i < numVectors; ++i) {
    allRecordsAfterAppendAll.insert(allRecordsAfterAppendAll.end(),
                                    allRecords[i].begin(), allRecords[i].end());
  }

  insertAndAppendAllPagesTest(testSchema, entrySize, pageSize,
                              totalNumTextFields, allRecords,
                              allRecordsAfterAppendAll, 0);
}
...
TEST_F(PagedVectorVarSizedTest,
       appendAllPagesMultipleVectorsWithDifferentPageSizes) {
  auto testSchema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                        ->addField(createField("value1", BasicType::UINT64))
                        ->addField(createField("value2", BasicType::TEXT))
                        ->addField(createField("value3", BasicType::FLOAT64));
