            ...
            if(bookKeeper[current_window].compare_and_swap(timeStamp, lastTimeStamp) == lastTimeStamp)
            {
                    ...
                    if(rank != 0 && numaNode == 0)//copy and send result
                    {
                        buffer_threads.push_back(std::make_shared<std::thread>([ producerID,
                                                  sharedHT_buffer, outputTable, campaingCnt, current_window, &hashTable, connectID, rank, numaNode, &connectInfos, &connections] {
                            stringstream ss;
        ...
        #pragma omp critical
        {
            ...
            if(numberOfConnections == 1)
            {
                if(rank == 0)
                {
                    ...
                    {
                        sharedHT_buffer_for_merge = new infinity::memory::Buffer*[(numberOfNodes-1)];
                        receiveElements = new ReceiveElement*[(numberOfNodes-1)];
