...
void process_window_mem(size_t processCnt, size_t randomCnt, size_t* randomNumbers,
		 uint64_t campaign_lsb, uint64_t campaign_msb ,size_t campaingCnt, std::atomic<size_t>** hashTable,
		 record* records, size_t windowSizeInSec)
{
...
{
	cout << "usage processCnt threadCnt" << endl;
	...
	cout << "run with config processCnt=" << processCnt << " batchSize=" << batchSize << " threadCnt=" << threadCnt << endl;
...
//Generator Code
	...
	#pragma omp parallel num_threads(threadCnt)
	{
		...
		for(size_t i = 0; i < threadCnt; i++)
		{
			...
			{
				process_window_mem(batchSize, randomCnt, randomNumbers, campaign_lsb, campaign_msb, campaingCnt, hashTable,
						&recs[i][u*batchSize], windowSizeInSeconds);
			}
		}
	}
