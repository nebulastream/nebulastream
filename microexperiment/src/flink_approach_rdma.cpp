...
void produce_window_mem(size_t processCnt, record* records,
        tbb::concurrent_bounded_queue<TupleBuffer>** queue, size_t consumeCnt,
        size_t prodID) {
    size_t produced = 0;
    size_t disQTuple = 0;
    size_t qualTuple = 0;
...
void cosume_window_mem(std::atomic<size_t>** hashTable, size_t windowSizeInSec,
        tbb::concurrent_bounded_queue<TupleBuffer>** queue, size_t campaingCnt,
        size_t* consumedRet, size_t consumerID, size_t produceCnt) {
    size_t consumed = 0;
    size_t disQTuple = 0;
    size_t qualTuple = 0;
    size_t windowSwitchCnt = 0;
        ...
        while (!queue[consumerID]->empty()) {
            ...
            if (lastTimeStamp != timeStamp
                ...
                if (hashTable[current_window][campaingCnt] != timeStamp) {
                    htReset++;
                    atomic_store(&hashTable[current_window][campaingCnt],
                            timeStamp);
                    std::fill(hashTable[current_window],
                            hashTable[current_window] + campaingCnt, 0);
...
//
    ...
    cout << "param processCnt=" << processCnt << " producer" << num_Producer
            << " consumer=" << num_Consumer << " bufferSize=" << BUFFER_SIZE
...
//create hash table
    ...
    tbb::concurrent_bounded_queue<TupleBuffer>** tbbQueues =
            new tbb::concurrent_bounded_queue<TupleBuffer>*[num_Consumer];
...
#pragma omp for
        for (size_t i = 0; i < threadCnt; i++) {
            if (i < num_Producer)
                produce_window_mem(processCnt, recs[i], tbbQueues, num_Consumer,
                        i);
            else {
                cosume_window_mem(hashTable, windowSizeInSeconds, tbbQueues,
                        campaingCnt, &consumed[i - num_Producer],
                        i - num_Producer, num_Producer);
            }
        }
    }
