...
StdMap::StdMap(size_t tuple_count) {
    map = std::unordered_map<size_t, size_t>(tuple_count);
}

void StdMap::build_parallel(const BuildTable &buildTable, const Limits &build_limits, size_t threadCount)
{
...
std::vector<ResultTable> StdMap::probe_parallel(const BuildTable &buildTable, const ProbeTable &probeTable, const Limits &probe_limits, size_t threadCount) const
{
    if (threadCount == 0)
