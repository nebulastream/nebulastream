...
class DeltaPhysicalOperatorTest : public Testing::BaseUnitTest, public ::testing::WithParamInterface<bool>
{
    struct MockedPipelineContext final : PipelineExecutionContext
    {
        ...
        {
            pinnedBuffers.push_back(std::move(tupleBuffer));
...
public:
    ...
    static DeltaLayoutInfo buildLayoutInfo(const std::vector<PhysicalDeltaExpression>& deltaExprs, const Schema& inputSchema)
    {
        ...
        {
            info.deltaFieldLayout.push_back({expr.targetField, expr.targetDataType});
            info.deltaFieldsEntrySize += expr.targetDataType.getSizeInBytesWithoutNull();
        }
        for (const auto& field : inputSchema.getFields())
        {
            info.fullRecordLayout.push_back({field.name, field.dataType});
            info.fullRecordEntrySize += field.dataType.getSizeInBytesWithoutNull();
        }
        return info;
    }
...
TEST_P(DeltaPhysicalOperatorTest, SingleRecordDropped)
{
    ...
    auto results = runPipeline(
        schema, schema, {{PhysicalFunction(FieldAccessPhysicalFunction("value")), "value", {DataType::Type::INT64, DataType::NULLABLE::NOT_NULLABLE}}}, inputBuffer);
...
TEST_P(DeltaPhysicalOperatorTest, TwoRecords)
{
    ...
    auto results = runPipeline(
        schema, schema, {{PhysicalFunction(FieldAccessPhysicalFunction("value")), "value", {DataType::Type::UINT32, DataType::NULLABLE::NOT_NULLABLE}}}, inputBuffer);
...
TEST_P(DeltaPhysicalOperatorTest, WithAlias)
{
    ...
    auto results = runPipeline(
        inputSchema,
        outputSchema,
        {{PhysicalFunction(FieldAccessPhysicalFunction("value")), "delta_value", {DataType::Type::INT64, DataType::NULLABLE::NOT_NULLABLE}}},
        inputBuffer);
...
TEST_P(DeltaPhysicalOperatorTest, WithoutAlias)
{
    ...
    auto results = runPipeline(
        inputSchema,
        outputSchema,
        {{PhysicalFunction(FieldAccessPhysicalFunction("a")), "a", {DataType::Type::INT64, DataType::NULLABLE::NOT_NULLABLE}}},
        inputBuffer);
...
TEST_P(DeltaPhysicalOperatorTest, MultipleDeltaExpressions)
{
    ...
    auto results = runPipeline(
        ...
        {
            {PhysicalFunction(FieldAccessPhysicalFunction("a")), "a", {DataType::Type::INT64, DataType::NULLABLE::NOT_NULLABLE}},
            {PhysicalFunction(FieldAccessPhysicalFunction("b")), "b", {DataType::Type::INT64, DataType::NULLABLE::NOT_NULLABLE}},
        },
        inputBuffer);
...
TEST_P(DeltaPhysicalOperatorTest, MultiBufferSequential)
{
    ...
    auto results = runPipeline(
        schema, schema, {{PhysicalFunction(FieldAccessPhysicalFunction("value")), "value", {DataType::Type::INT64, DataType::NULLABLE::NOT_NULLABLE}}}, inputBuffers);
