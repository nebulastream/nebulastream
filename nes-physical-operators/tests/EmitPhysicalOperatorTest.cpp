...
public:
    ...
    void checkNumberOfBuffers(size_t numberOfBuffers, const std::source_location location = std::source_location::current())
    {
        ...
        const std::source_location location = std::source_location::current())
    {
        ...
        EXPECT_EQ(buffers.rlock()->at(index).getUsedMemorySize(), usedMemorySize) << fmt::format("Expected {} B", usedMemorySize);
    ...
    void checkForDups(const std::source_location location = std::source_location::current())
    {
    ...
    void checkLastChunks(const std::source_location location = std::source_location::current())
    {
    ...
    TupleBuffer createBuffer(
        const SequenceNumber::Underlying sequence,
        const ChunkNumber::Underlying chunkNumber,
        const bool isLastChunk = false,
        const OriginId originId = INITIAL<OriginId>,
        const size_t usedMemorySize = 0)
    {
        auto buffer = bm->getBufferBlocking();
        buffer.setUsedMemorySize(usedMemorySize);
