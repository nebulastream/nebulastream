...
protected:
    struct MockedPipelineContext final : PipelineExecutionContext
    {
        ...
        {
            pinnedBuffers.emplace_back(std::move(tupleBuffer));
...
public:
    ...
    static float readOutputField(const Record& record, const std::string& fieldName)
    {
        ...
        auto floatVal = val.getRawValueAs<nautilus::val<float>>();
    ...
    std::vector<Record> runVarsizedInference(
        const NES::Nebuli::Inference::Model& model, const std::vector<std::vector<float>>& inputsPerRecord, size_t numOutputFields)
    {
