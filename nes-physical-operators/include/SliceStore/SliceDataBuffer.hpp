...
public:
    ...
    [[nodiscard]] ChainedHashMapRef asHashMap(const HashMapOptions& options) &
    {
        return ChainedHashMapRef{buffer.asArg(), options.fieldKeys, options.fieldValues, options.entriesPerPage, options.entrySize};
    }

    ChainedHashMapRef asHashMap(const HashMapOptions&) && = delete;

    /// The underlying buffer, e.g. to store/load a hash map entry's child buffers or to hand to an aggregation function. Valid only
    /// while this handle is alive.
    [[nodiscard]] NautilusBuffer& getBuffer() & { return buffer; }

    NautilusBuffer& getBuffer() && = delete;
};
