...
private:
    /// Raises maxRetiredSliceEnd to `sliceEnd` if it is higher.
    void recordRetired(SliceEnd sliceEnd) noexcept;
    ...
    /// Highest slice end that was triggered or garbage collected. A tuple at or below it arrived after its
    /// slice was emitted (anchor side) or dropped (partner side), so it can no longer take part in a join.
    std::atomic<Timestamp::Underlying> maxRetiredSliceEnd{Timestamp::INITIAL_VALUE};
};
