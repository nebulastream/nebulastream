...
{
    SNBufferingRecord(Epoch epoch) : originEpoch(epoch) { }

    Epoch originEpoch;
    std::vector<TupleBuffer> buffers = std::vector<TupleBuffer>();
    std::unordered_set<ChunkNumber> chunkNumbers = std::unordered_set<ChunkNumber>();
    size_t iterator = 0;
...
private:
    void garbageCollect(OriginId originId);
    const std::string filePath;
    const std::vector<OriginId> origins;
    std::unordered_map<OriginId, std::unique_ptr<std::mutex>> gcMutexes;
    std::unordered_map<OriginId, folly::Synchronized<detail::WatermarkedSNMap>> state;
};
