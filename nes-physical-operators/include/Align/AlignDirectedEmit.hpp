...
class AlignDirectedEmitState : public OperatorState
{
public:
    explicit AlignDirectedEmitState(const RecordBuffer& resultBuffer) : resultBuffer(resultBuffer) { }

    nautilus::val<uint64_t> outputIndex = 0;
    RecordBuffer resultBuffer;
};

inline void openAlignDirectedEmit(ExecutionContext& ctx, OperatorId id)
{
    const RecordBuffer resultBuffer{ctx.allocateBuffer()};
    ctx.setLocalOperatorState(id, std::make_unique<AlignDirectedEmitState>(resultBuffer));
}

inline void emitAlignDirectedRecordBuffer(
    ...
    const nautilus::val<bool>& potentialLastChunk)
{
    ...
    nautilus::invoke(
            ...
            TupleBuffer* newBuffer)
        {
            ...
            dynamic_cast<AlignDirectedOperatorHandler&>(*handler).setChunkNumber(
...
inline void emitAlignDirectedRecord(
    ExecutionContext& ctx,
    OperatorId id,
    OperatorHandlerId operatorHandlerId,
    ...
    Record& record)
{
    auto* const state = dynamic_cast<AlignDirectedEmitState*>(ctx.getLocalState(id));
    ...
    {
        emitAlignDirectedRecordBuffer(ctx, operatorHandlerId, state->resultBuffer, state->outputIndex, false);
...
inline void closeAlignDirectedEmit(ExecutionContext& ctx, OperatorId id, OperatorHandlerId operatorHandlerId)
{
    auto* const state = dynamic_cast<AlignDirectedEmitState*>(ctx.getLocalState(id));
    emitAlignDirectedRecordBuffer(ctx, operatorHandlerId, state->resultBuffer, state->outputIndex, true);
}
