...
class AlignNNEmitState : public OperatorState
{
public:
    explicit AlignNNEmitState(const RecordBuffer& resultBuffer) : resultBuffer(resultBuffer) { }

    nautilus::val<uint64_t> outputIndex = 0;
    RecordBuffer resultBuffer;
};

inline void openAlignNNEmit(ExecutionContext& ctx, OperatorId id)
{
    const RecordBuffer resultBuffer{ctx.allocateBuffer()};
    ctx.setLocalOperatorState(id, std::make_unique<AlignNNEmitState>(resultBuffer));
}

inline void emitAlignNNRecordBuffer(
    ...
    const nautilus::val<bool>& potentialLastChunk)
{
    ...
    nautilus::invoke(
            ...
            TupleBuffer* newBuffer)
        {
            ...
            dynamic_cast<AlignNNOperatorHandler&>(*handler).setChunkNumber(
...
inline void emitAlignNNRecord(
    ...
    Record& record)
{
    auto* const state = dynamic_cast<AlignNNEmitState*>(ctx.getLocalState(id));
    ...
    {
        emitAlignNNRecordBuffer(ctx, operatorHandlerId, state->resultBuffer, state->outputIndex, false);
...
inline void closeAlignNNEmit(ExecutionContext& ctx, OperatorId id, OperatorHandlerId operatorHandlerId)
{
    auto* const state = dynamic_cast<AlignNNEmitState*>(ctx.getLocalState(id));
    emitAlignNNRecordBuffer(ctx, operatorHandlerId, state->resultBuffer, state->outputIndex, true);
}
