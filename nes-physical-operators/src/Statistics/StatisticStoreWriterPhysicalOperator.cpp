    ...
    const uint64_t statisticDataSize)
{
    ...
    opHandler->getStatisticStore()->insertStatistic(
        ...
        Statistic{
            StatisticId(statisticId),
            static_cast<StatisticType>(statisticType),
            Timestamp(startTs),
            Timestamp(endTs),
            numberOfSeenTuples,
...
void StatisticStoreWriterPhysicalOperator::execute(ExecutionContext& executionCtx, Record& record) const
{
    ...
    const auto numberOfSeenTuples = record.read(fieldIdentifiers.inputNumberOfSeenTuples).getRawValueAs<nautilus::val<uint64_t>>();
    ...
    invoke(
        ...
        nautilus::val<uint8_t>{static_cast<uint8_t>(statisticType)},
        startTs,
        endTs,
        numberOfSeenTuples,
    ...
    outputRecord.write(fieldIdentifiers.outputNumberOfSeenTuples, VarVal{numberOfSeenTuples});
