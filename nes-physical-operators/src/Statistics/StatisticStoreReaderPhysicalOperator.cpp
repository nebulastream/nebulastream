...
uint64_t getSeenMeasurementsByIndexProxy(const uint64_t readerId, const uint64_t index)
{
    const auto& statistics = tProbeStatistics[readerId];
...
void StatisticStoreReaderPhysicalOperator::execute(ExecutionContext& executionCtx, Record& record) const
{
    ...
    for (nautilus::val<uint64_t> i = 0; i < statisticCount; i = i + 1)
    {
        ...
        if (payload != nullptr)
        {
            ...
            const auto numberOfSeenMeasurements = invoke(getSeenMeasurementsByIndexProxy, readerId, i);
            ...
            statisticIterator->forEachRecord(
                ...
                [&](Record& statisticRecord)
                {
                    ...
                    statisticRecord.write(fieldIdentifiers.outputNumberOfSeenMeasurements, VarVal{numberOfSeenMeasurements});
