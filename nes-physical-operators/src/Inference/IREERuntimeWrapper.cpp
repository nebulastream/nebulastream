...
void IREERuntimeWrapper::execute(const std::string& functionName, void* inputData, size_t inputSize, void* outputData)
{
    ...
    if (this->function.module == nullptr)
    {
        ...
        if (!iree_status_is_ok(status))
        {
            throw InferenceRuntime("Model Execution failed. Could not initialize function call");
        }
    ...
    if (!iree_status_is_ok(status))
    {
        throw InferenceRuntime("Model Execution failed. Could not push input tensor");
    }
    ...
    if (!iree_status_is_ok(status))
    {
        throw InferenceRuntime("Model Execution failed. Could not pop output buffer");
    }
...
void IREERuntimeWrapper::setNDim(size_t n)
{
    this->nDim = n;
}
