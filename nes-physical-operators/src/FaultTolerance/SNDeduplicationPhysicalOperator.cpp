...
static void startHandlerProxy(OperatorHandler* handler)
{
    auto* deduplicationHandler = dynamic_cast<SNDeduplicationOperatorHandler*>(handler);
    deduplicationHandler->init();
}

static void stopHandlerProxy(OperatorHandler* handler)
{
    auto* deduplicationHandler = dynamic_cast<SNDeduplicationOperatorHandler*>(handler);
...
static bool filterAndBufferProxy(OperatorHandler* handler, TupleBuffer* buffer)
{
    auto* deduplicationHandler = dynamic_cast<SNDeduplicationOperatorHandler*>(handler);
    return deduplicationHandler->filterAndBuffer(buffer);
}

static TupleBuffer* probeProxy(OperatorHandler* handler, TupleBuffer* buffer)
{
    auto* deduplicationHandler = dynamic_cast<SNDeduplicationOperatorHandler*>(handler);
    return deduplicationHandler->probe(buffer);
}

static void finalizeSNProxy(OperatorHandler* handler, TupleBuffer* buffer)
{
    auto* deduplicationHandler = dynamic_cast<SNDeduplicationOperatorHandler*>(handler);
...
void SNDeduplicationPhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    if (probeFlag)
    {
        for (auto buffer = nautilus::invoke(probeProxy, globalHandler, recordBuffer.getReference());
             buffer != nautilus::val<TupleBuffer*>(nullptr);
