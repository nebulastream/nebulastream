...
SNDeduplicationOperatorHandler::SNDeduplicationOperatorHandler(std::string filePath, const std::vector<OriginId>& inputOrigins) : filePath{filePath}, origins{inputOrigins}
{
...
void SNDeduplicationOperatorHandler::init() {
    ...
    if (std::filesystem::exists(filePath))
    {
        ...
        if (!in) throw std::runtime_error("uhm? file error");
...
void SNDeduplicationOperatorHandler::save() {
    ...
    if (!out) throw std::runtime_error("uhm? file error");
...
bool SNDeduplicationOperatorHandler::checkAndInsert(OriginId originId, SequenceNumber sequenceNumber)
{
    auto snSet = seenSequenceNumbers[originId]->wlock();
    bool isDuplicate = snSet->contains(sequenceNumber);
