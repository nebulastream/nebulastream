...
std::optional<TupleBuffer*> SequenceOperatorHandler::getNextBuffer(TupleBuffer* buffer)
{
    if (auto optBuffer
        = sequencer.isNext(SequenceData(buffer->getSequenceNumber(), buffer->getChunkNumber(), buffer->isLastChunk()), *buffer))
    {
...
std::optional<TupleBuffer*> SequenceOperatorHandler::markBufferAsDone(TupleBuffer* buffer)
{
