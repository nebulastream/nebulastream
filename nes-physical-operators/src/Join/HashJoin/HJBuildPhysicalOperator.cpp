...
void HJBuildPhysicalOperator::execute(ExecutionContext& ctx, Record& record) const
{
    ...
    {
        /// Set by onInsert below on a cache miss; lets us skip re-loading the paged vector we just created.
        nautilus::val<bool> wasInserted{false};
                ...
                nautilus::invoke(
                        ...
                        TupleBuffer* pagedVecOut) -> void
                    {
                        if (auto pagedVectorBuffer = bufferProvider->getUnpooledBuffer(PagedVector::getMainBufferSize()))
                        {
                            ...
                            /// Hand the caller a usable handle now, before storeChildBuffer moves the original away.
                            *pagedVecOut = pagedVectorBuffer.value();
        ...
        /// On a cache hit, we don't have a paged vector handle yet -- load it from the entry. On a miss,
        /// onInsert above already populated pagedVecBuffer, so loading it again would be a wasted invoke.
        if (not wasInserted)
        {
