...
void HJProbePhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    for (nautilus::val<uint64_t> leftHashMapIndex = 0; leftHashMapIndex < leftNumberOfHashMaps; ++leftHashMapIndex)
    {
        ...
        auto pinnedLeftBuffer = nautilus::invoke(+[](const TupleBuffer* parent, uint32_t leftHashMapIndex, Arena* arena)
        {
        ...
        }, recordBuffer.getReference(), leftHashMapIndex, executionCtx.pipelineMemoryProvider.arena.getArena());
        ...
        for (nautilus::val<uint64_t> rightHashMapIndex = 0; rightHashMapIndex < rightNumberOfHashMaps; ++rightHashMapIndex)
        {
            ...
            auto pinnedRightBuffer = nautilus::invoke(+[](const TupleBuffer* parent, uint32_t leftNumberOfHashMaps, uint32_t rightHashMapIndex, Arena* arena)
            {
                INVARIANT(parent != nullptr, "Parent Tuplebuffer MUST NOT be null at this point");
                // Right buffers start after all left buffers
                ...
                auto buffer = parent->loadChildBuffer(bufferIndex);
                return std::addressof(arena->pinBuffer(buffer));
            }, recordBuffer.getReference(), leftNumberOfHashMaps, rightHashMapIndex, executionCtx.pipelineMemoryProvider.arena.getArena());
