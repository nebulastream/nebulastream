...
NLJProbePhysicalOperator::NLJProbePhysicalOperator(
    ...
    std::shared_ptr<TupleLayout> leftTupleLayout,
    std::shared_ptr<TupleLayout> rightTupleLayout,
...
void NLJProbePhysicalOperator::performNLJ(
    ...
    TupleLayout& outerTupleLayout,
    TupleLayout& innerTupleLayout,
...
{
    const auto outerFields = outerTupleLayout.getAllFieldNames();
    const auto innerFields = innerTupleLayout.getAllFieldNames();
...
void NLJProbePhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    const PagedVectorRef leftPagedVector(NautilusBorrowedBuffer::load(leftPagedVectorRef), leftTupleLayout);
    const PagedVectorRef rightPagedVector(NautilusBorrowedBuffer::load(rightPagedVectorRef), rightTupleLayout);
