...
{
    uint64_t pvMainBufferSize = PagedVector::getMainBufferSize();
    uint64_t pvPageBufferSize = bufferProvider.getBufferSize();
    for (uint64_t i = 0; i < numberOfWorkerThreads; ++i)
    {
        if (auto pagedVectorBuffer = bufferProvider.getUnpooledBuffer(pvMainBufferSize))
        {
            ...
            std::ignore = PagedVector::init(pagedVectorBuffer.value(), pvPageBufferSize, tupleSizeLeft);
            leftPagedVectorBuffers.emplace_back(pagedVectorBuffer.value());
        }
        else
        {
            throw BufferAllocationFailure("NNo unpooled TupleBuffer available for NLJ left paged vector main buffer");
        }
    }
    ...
    for (uint64_t i = 0; i < numberOfWorkerThreads; ++i)
    {
        ...
        {
            /// initialize the paged vector tuple buffer
            std::ignore = PagedVector::init(pagedVectorBuffer.value(), pvPageBufferSize, tupleSizeRight);
...
uint64_t NLJSlice::getNumberOfTuplesLeft() const
{
    return std::accumulate(
        ...
        [](uint64_t sum, const TupleBuffer& buf)
        {
            ...
            return sum + pv.getTotalNumberOfRecords();
        });
}
