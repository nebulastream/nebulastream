    ...
    : Slice(sliceStart, sliceEnd)
{
    ...
    for (uint64_t i = 0; i < numberOfWorkerThreads; ++i)
    {
        if (auto pagedVectorBuffer = bufferProvider.getUnpooledBuffer(pvMainBufferSize))
        {
            ...
            PagedVector pagedVector = PagedVector::init(pagedVectorBuffer.value(), pvPageBufferSize, tupleSizeLeft);
    ...
    for (uint64_t i = 0; i < numberOfWorkerThreads; ++i)
    {
        ...
        {
            /// initialize the paged vector tuple buffer
            PagedVector pagedVector = PagedVector::init(pagedVectorBuffer.value(), pvPageBufferSize, tupleSizeRight);
