    ...
    const std::vector<Record::RecordFieldIdentifier>& projectionsInner) const
{
    ...
    joinedRecord.write(windowMetaData.startField.getName(), windowStart.convertToValue());
    joinedRecord.write(windowMetaData.endField.getName(), windowEnd.convertToValue());
