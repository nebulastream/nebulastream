...
/// Anchor == Left, Partner == Right. The base only stores this. It is never read on the interval-join path.
...
/// Decrements the side's store pipeline count. On the last one, signals the handler the side is done
/// (the handler runs the end-of-stream flush once both sides are done).
...
void IntervalJoinBuildPhysicalOperator::setup(ExecutionContext& executionCtx, CompilationContext&) const
{
    ...
    /// Count this input pipeline so the handler knows how many will eventually terminate.
    invoke(
...
void IntervalJoinBuildPhysicalOperator::close(ExecutionContext& executionCtx, RecordBuffer&) const
{
    ...
    /// Advance the side's build watermark and re-run the trigger loop.
    invoke(
...
void IntervalJoinBuildPhysicalOperator::execute(ExecutionContext& executionCtx, Record& record) const
{
    ...
    PagedVectorRef pagedVectorRef{BorrowedNautilusBuffer::from(pagedVectorMemRef.asArg()), tupleLayout};
