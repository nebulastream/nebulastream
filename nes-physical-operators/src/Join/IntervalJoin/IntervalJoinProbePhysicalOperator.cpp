...
SliceEnd getPartnerSliceEndProxy(const EmittedIntervalJoinWindowTrigger* trigger, const std::uint64_t partnerIndex)
{
    ...
    return trigger->partnerSliceEnds.at(partnerIndex);
}
...
IntervalJoinProbePhysicalOperator::intervalPredicateHolds(ExecutionContext& executionCtx, Record& anchorRecord, Record& partnerRecord) const
{
    ...
    /// True iff `shifted + offsetMs <= other` in exact arithmetic, guarding against wrap-around.
    const auto shiftedIsAtMost = [](const nautilus::val<uint64_t>& shifted, const int64_t offsetMs, const nautilus::val<uint64_t>& other)
    {
        if (offsetMs >= 0)
        {
            /// shifted + offset <= other  <=>  shifted <= other - offset
            const nautilus::val<uint64_t> offset{static_cast<uint64_t>(offsetMs)};
            return (other >= offset) && (shifted <= (other - offset));
        }
        /// shifted - offset <= other, which trivially holds whenever the subtraction would underflow
        const nautilus::val<uint64_t> offset{static_cast<uint64_t>(-offsetMs)};
        return (shifted <= offset) || ((shifted - offset) <= other);
    };

    /// anchorTs + lowerBound <= partnerTs, rewritten as partnerTs - upperBound <= anchorTs for the upper side.
    return shiftedIsAtMost(anchorTs, lowerBound.getRawValue(), partnerTs)
        && shiftedIsAtMost(partnerTs, -upperBound.getRawValue(), anchorTs);
}
