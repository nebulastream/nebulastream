...
void IntervalJoinProbePhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    const auto leftSliceEnd = invoke(getLeftSliceEndFromTriggerProxy, triggerRef);
    ...
    const auto leftSliceRef = invoke(getLeftSliceByEndProxy, operatorHandlerMemRef, leftSliceEnd);
    ...
    /// Walk partner indices 0..partnerCount-1.
    for (nautilus::val<std::uint64_t> partnerIdx{0}; partnerIdx < partnerCount; partnerIdx = partnerIdx + nautilus::val<std::uint64_t>{1})
    {
        ...
        /// Nested-loop over (anchor, partner). Outer = left, inner = right.
