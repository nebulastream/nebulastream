...
VarVal AbsolutePhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    /// Signed integer types are widened by one rank during schema inference so the magnitude of the most-negative value is representable
    ...
    if (inputType.isType(DataType::Type::INT64))
    {
        ...
        const auto isMostNegative = rawValue == nautilus::val<int64_t>{std::numeric_limits<int64_t>::min()};
        /// A NULL value carries arbitrary raw bits, so only raise the overflow error for a non-null most-negative value.
        if (isMostNegative and not value.isNull())
        {
            nautilus::invoke(+[] { throw ArithmeticalError("ABS() of the most-negative INT64 value overflows"); });
        }
    }
