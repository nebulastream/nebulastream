...
VarVal OrPhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    ...
    const auto specialCondition = (leftValue.getRawValueAs<nautilus::val<bool>>() == true and not leftValue.isNull())
        or (rightValue.getRawValueAs<nautilus::val<bool>>() == true and not rightValue.isNull());
    const auto newValue
        = nautilus::select(specialCondition, nautilus::val<bool>{true}, (leftValue || rightValue).getRawValueAs<nautilus::val<bool>>());
