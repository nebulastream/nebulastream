...
From_Base64PhysicalFunction::From_Base64PhysicalFunction(PhysicalFunction childPhysicalFunction)
...
uint64_t decodeBase64(int8_t* inputPtr, uint64_t inputSize, int8_t* outputPtr)
{
    ...
    uint64_t actualLen = static_cast<uint64_t>(decodedLen);
    if (inputSize >= 1 && inputPtr[inputSize - 1] == '=') --actualLen;
    if (inputSize >= 2 && inputPtr[inputSize - 2] == '=') --actualLen;
    return actualLen;
}
}

VarVal From_Base64PhysicalFunction::execute(const Record& record, ArenaRef& arena) const
{
    const auto inputValue = childPhysicalFunction.execute(record, arena).cast<VariableSizedData>();
...
PhysicalFunctionGeneratedRegistrar::RegisterFrom_Base64PhysicalFunction(PhysicalFunctionRegistryArguments physicalFunctionRegistryArguments)
{
    ...
    return From_Base64PhysicalFunction(physicalFunctionRegistryArguments.childFunctions[0]);
}
}
