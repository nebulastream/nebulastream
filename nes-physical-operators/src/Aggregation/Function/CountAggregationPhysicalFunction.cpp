    ...
    const nautilus::val<AggregationState*>& aggregationState, PipelineMemoryProvider& pipelineMemoryProvider, const Record& record)
{
    ...
    nautilus::val<uint64_t> multiplicationFactor{1};
    if (inputType.nullable and not includeNullValues)
    {
        /// If the value is null and we do not include null values, we need to set the multiplication factor to 0
        multiplicationFactor = static_cast<nautilus::val<uint64_t>>(static_cast<unsigned long>(not value.isNull()));
    }
