    ...
    PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    if (!containsNull)
    {
        /// The O(n^2) median search is compiled as its own nautilus function. Traced inline, its nested loops carry every
        /// value live in the probe as block arguments, which makes up most of the probe's IR and compilation time.
        if (!medianFunction)
        {
        ...
        /// Keep the result's nullability: it is a static property of the VarVal, so both branches must agree on it.
    ...
    PipelineMemoryProvider& memoryProvider) const
{
    ...
    for (auto candidateIt = pagedVectorRef.begin(); candidateIt != pagedVectorEnd; ++candidateIt)
    {
        ...
        const auto candidateValue = inputFunction.execute(candidateRecord, memoryProvider.arena);

        /// Counting how many items are smaller or equal for the current candidate
        for (const auto& itemRecord : pagedVectorRef)
        {
            const auto itemValue = inputFunction.execute(itemRecord, memoryProvider.arena);
    ...
    if (medianFound1 and medianFound2)
    {
        ...
        const auto medianValue1 = inputFunction.execute(medianRecord1, memoryProvider.arena);
        const auto medianValue2 = inputFunction.execute(medianRecord2, memoryProvider.arena);
