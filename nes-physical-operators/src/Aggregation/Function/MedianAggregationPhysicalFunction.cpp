    ...
    const Record& record)
{
    ...
    nautilus::invoke(
        +[](TupleBuffer* parent, TupleBuffer* out, uint32_t* indexPtr)
    ...
    PipelineMemoryProvider& pipelineMemoryProvider)
{
    ...
    nautilus::invoke(
        +[](AbstractBufferProvider* bufferProvider, TupleBuffer* parent1, uint32_t* indexPtr1, TupleBuffer* parent2, uint32_t* indexPtr2)
            -> void
        {
            TupleBuffer vec1Buf = parent1->loadChildBuffer(VariableSizedAccess::Index{*indexPtr1});
            TupleBuffer vec2Buf = parent2->loadChildBuffer(VariableSizedAccess::Index{*indexPtr2});
