...
void AggregationProbePhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    for (const auto entry : finalHashMap)
    {
        ...
        outputRecord.write(windowMetaData.startField.getLastName(), windowStart.convertToValue());
        outputRecord.write(windowMetaData.endField.getLastName(), windowEnd.convertToValue());
