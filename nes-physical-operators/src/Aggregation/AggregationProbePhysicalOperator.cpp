...
void AggregationProbePhysicalOperator::open(ExecutionContext& executionCtx, RecordBuffer& recordBuffer) const
{
    ...
    for (const auto entry : finalHashMap)
    {
        ...
        outputRecord.write(windowMetaData.startField.getName(), windowStart.convertToValue());
        outputRecord.write(windowMetaData.endField.getName(), windowEnd.convertToValue());
