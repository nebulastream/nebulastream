...
void DeltaPhysicalOperator::open(ExecutionContext& ctx, RecordBuffer& recordBuffer) const
{
    ...
    if (found)
    {
        ...
        {
            auto val = VarVal::readVarValFromMemory(ptr, entry.type, nautilus::val<bool>(false));
            state->previousRecord.write(entry.fieldName, val);
            ptr = ptr + nautilus::val<size_t>(entry.type.getSizeInBytesWithoutNull());
        }
...
void DeltaPhysicalOperator::execute(ExecutionContext& ctx, Record& record) const
{
    ...
    if (state->isFirstRecord)
    {
        ...
        if (predecessorFound)
        {
            ...
            for (const auto& entry : nautilus::static_iterable(deltaFieldLayout))
            {
                auto predVal = VarVal::readVarValFromMemory(ptr, entry.type, nautilus::val<bool>(false));
    ...
    if (!state->isFirstRecord)
    {
        ...
        if (found)
        {
            ...
            Record firstRecord;
            {
                ...
                for (const auto& entry : nautilus::static_iterable(fullRecordLayout))
                {
                    firstRecord.write(entry.fieldName, VarVal::readVarValFromMemory(ptr, entry.type, nautilus::val<bool>(false)));
                    ptr = ptr + nautilus::val<size_t>(entry.type.getSizeInBytesWithoutNull());
                }
            }
