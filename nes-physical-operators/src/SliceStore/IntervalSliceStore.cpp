    ...
    const Timestamp timestamp, const std::function<std::vector<std::shared_ptr<Slice>>(SliceStart, SliceEnd)>& createNewSlice)
{
    ...
    if (sliceEnd.getRawValue() <= maxRetiredSliceEnd.load(std::memory_order_acquire))
    {
        /// Same contract as the other joins (DefaultTimeBasedSliceStore): the window this tuple belongs to has
        /// already been emitted, so the tuple is too late to be joined.
        throw WindowingError(
            "Out-of-order tuple with timestamp {} belongs to slice ending at {}, which was already triggered or collected.",
            timestamp,
            sliceEnd);
    }
    {
...
void IntervalSliceStore::recordRetired(const SliceEnd sliceEnd) noexcept
{
    auto current = maxRetiredSliceEnd.load(std::memory_order_relaxed);
    while (
        sliceEnd.getRawValue() > current
        && !maxRetiredSliceEnd.compare_exchange_weak(current, sliceEnd.getRawValue(), std::memory_order_acq_rel, std::memory_order_relaxed))
    {
    }
}

void IntervalSliceStore::garbageCollectSlicesAndWindows(const Timestamp gcWatermark)
{
    /// Plain sliceEnd cutoff. The interval geometry stays out of here: the caller folds the partner
    /// reach-back into gcWatermark, and the forward reach is handled at trigger time. A slice below the
    /// watermark is safe to drop, the gap-filling probe watermark cannot pass a slice an in-flight probe
    /// still reads (see IntervalJoinOperatorHandler::assembleAndEmitTriggers).
    std::vector<std::shared_ptr<IntervalJoinSlice>> expired;
    {
        auto slicesWriteLocked = slices.wlock();
        const auto cut = slicesWriteLocked->lower_bound(SliceEnd{gcWatermark.getRawValue()});
        if (cut != slicesWriteLocked->begin())
        {
            /// Partner slices are never triggered, so collection is what retires them.
            recordRetired(std::prev(cut)->first);
        }
        for (auto it = slicesWriteLocked->begin(); it != cut; ++it)
        {
            expired.emplace_back(it->second);
        }
        slicesWriteLocked->erase(slicesWriteLocked->begin(), cut);
    }
    /// Free buffers outside the lock.
}
