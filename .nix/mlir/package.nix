...
let
  ...
  # LLVM/MLIR 19 bundle kept for sanitizer builds to stay compatible with the
  # current precompiled nautilus artifacts.
  legacyMlir = pkgs.llvmPackages_19.mlir;
in {
  default = mlirBinary;
  legacy = legacyMlir;
  withSanitizer = sanitizer:
    if sanitizer.name == "none" then mlirBinary else legacyMlir;
  inherit mlirBinary;
}
