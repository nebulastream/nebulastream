...
NesWorker::NesWorker(Configurations::WorkerConfigurationPtr workerConfig,
    ...
    : workerConfig(workerConfig), localWorkerRpcPort(workerConfig->rpcPort),
      ...
      mobilityConfig(
          std::make_shared<NES::Configurations::Spatial::Mobility::
                               Experimental::WorkerMobilityConfiguration>(
              workerConfig->mobilityConfiguration)) {
  setThreadName("NesWorker");
  NES_DEBUG("NesWorker: constructed");
  NES_ASSERT2_FMT(workerConfig->coordinatorPort > 0,
                  "Cannot use 0 as coordinator port");
  rpcAddress = workerConfig->localWorkerHost.getValue() + ":" +
               std::to_string(localWorkerRpcPort);
}
...
void NesWorker::handleRpcs(WorkerRPCServer &service) {
  ...
  while (true) {
    ...
    // tells us whether there is any kind of event or completionQueue is
    // shutting down.
...
void NesWorker::buildAndStartGRPCServer(
    const std::shared_ptr<std::promise<int>> &portPromise) {
  WorkerRPCServer service(nodeEngine, monitoringAgent, locationProvider,
                          trajectoryPredictor);
  ...
  NES_DEBUG(
      "NesWorker: buildAndStartGRPCServer Server listening on address {}: {}",
      rpcAddress, actualRpcPort);
...
bool NesWorker::start(bool blocking, bool withConnect) {
  NES_DEBUG("NesWorker: start with blocking {} workerId={} coordinatorHost={} "
            "coordinatorPort={} localWorkerHost={} "
            "localWorkerRpcPort={} "
            "localWorkerZmqPort={} windowStrategy={}",
            blocking, workerConfig->workerId.getValue(),
            workerConfig->coordinatorHost.getValue(),
            workerConfig->coordinatorPort.getValue(),
            workerConfig->localWorkerHost.getValue(), localWorkerRpcPort,
            workerConfig->dataPort.getValue(),
            magic_enum::enum_name(
                workerConfig->queryCompiler.windowingStrategy.getValue()));
  ...
  try {
    NES_DEBUG("NesWorker: MonitoringAgent configured with monitoring={}",
              workerConfig->enableMonitoring.getValue());
    monitoringAgent = Monitoring::MonitoringAgent::create(
        workerConfig->enableMonitoring.getValue());
    monitoringAgent->addMonitoringStreams(workerConfig);

    nodeEngine =
        Runtime::NodeEngineBuilder::create(workerConfig)
    ...
    NES_ERROR("NesWorker: node engine could not be started with error {}",
              err.what());
    throw Exceptions::RuntimeException(
        "NesWorker error while starting node engine");
  }

  NES_DEBUG("NesWorker: request startWorkerRPCServer for accepting messages "
            "for address={}: {}",
            rpcAddress, localWorkerRpcPort.load());
  auto promRPC = std::make_shared<std::promise<int>>();

  if (workerConfig->nodeSpatialType.getValue() !=
      NES::Spatial::Experimental::SpatialType::NO_LOCATION) {
    locationProvider =
        NES::Spatial::Mobility::Experimental::LocationProvider::create(
            workerConfig);
    if (locationProvider->getSpatialType() ==
        NES::Spatial::Experimental::SpatialType::MOBILE_NODE) {
      // is s2 is activated, create a reconnect schedule predictor
      trajectoryPredictor = NES::Spatial::Mobility::Experimental::
          ReconnectSchedulePredictor::create(mobilityConfig);
    }
  }
  ...
  rpcAddress = workerConfig->localWorkerHost.getValue() + ":" +
               std::to_string(localWorkerRpcPort.load());
  NES_DEBUG("NesWorker: startWorkerRPCServer ready for accepting messages for "
            "address={}: {}",
            rpcAddress, localWorkerRpcPort.load());
  ...
  if (withConnect && locationProvider &&
      locationProvider->getSpatialType() ==
          NES::Spatial::Experimental::SpatialType::MOBILE_NODE) {
    workerMobilityHandler = std::make_shared<
        NES::Spatial::Mobility::Experimental::WorkerMobilityHandler>(
        locationProvider, coordinatorRpcClient, nodeEngine, mobilityConfig);
    // FIXME: currently the worker mobility handler will only work with exactly
    // one parent
    auto parentIds = coordinatorRpcClient->getParents(workerId);
    if (parentIds.size() > 1) {
      NES_WARNING(
          "Attempting to start worker mobility handler for worker with "
          "multiple parents. This is"
    ...
    statisticOutputThread = std::make_shared<std::thread>(([this]() {
      ...
      while (isRunning) {
        ...
        for (auto &query : stats) {
          std::cout << "Statistics "
                    << std::put_time(std::localtime(&timeNow), "%Y-%m-%d %X")
  ...
  if (isRunning.compare_exchange_strong(expected, false)) {
    ...
    if (!successShutdownNodeEngine) {
      ...
      NES_THROW_RUNTIME_ERROR(
          "NesWorker::stop  error while stopping node engine");
    }
...
void serializeOpenCLDeviceInfo(
    const NES::Runtime::OpenCLDeviceInfo &deviceInfo, unsigned deviceIndex,
...
bool NesWorker::connect() {
  ...
  std::string coordinatorAddress =
      workerConfig->coordinatorHost.getValue() + ":" +
      std::to_string(workerConfig->coordinatorPort);
  NES_DEBUG("NesWorker::connect() Registering worker with coordinator at {}",
            coordinatorAddress);
  coordinatorRpcClient = CoordinatorRPCClient::create(coordinatorAddress);

  RegisterWorkerRequest registrationRequest;
  registrationRequest.set_workerid(
      workerConfig->workerId.getValue().getRawValue());
  ...
  registrationRequest.set_dataport(
  ...
  registrationRequest.mutable_registrationmetrics()->Swap(
      monitoringAgent->getRegistrationMetrics().serialize().get());
  // todo: what about this?
  registrationRequest.set_javaudfsupported(
      workerConfig->isJavaUDFSupported.getValue());
  registrationRequest.set_spatialtype(
      NES::Spatial::Util::SpatialTypeUtility::toProtobufEnum(
          workerConfig->nodeSpatialType.getValue()));
  for (auto i = 0u; i < nodeEngine->getOpenCLManager()->getDevices().size();
       ++i) {
    serializeOpenCLDeviceInfo(
        nodeEngine->getOpenCLManager()->getDevices()[i].deviceInfo, i,
  ...
  bool successPRCRegister =
      coordinatorRpcClient->registerWorker(registrationRequest);

  NES_DEBUG("NesWorker::connect() Worker registered successfully and got id={}",
  ...
  if (successPRCRegister) {
    if (workerId != workerConfig->workerId) {
      if (workerConfig->workerId == INVALID_WORKER_NODE_ID) {
        ...
        NES_DEBUG("NesWorker::connect() Persisting workerId={} in yaml file "
                  "for the first time.",
                  workerId);
        bool success =
            getWorkerConfiguration()->persistWorkerIdInYamlConfigFile(
                workerConfig->configPath, workerId, false);
        if (!success) {
          NES_WARNING("NesWorker::connect() Could not persist workerId in yaml "
                      "config file");
        } else {
          NES_DEBUG("NesWorker::connect() Persisted workerId={} successfully "
                    "in yaml file.",
                    workerId);
        }
      } else {
        // a value was in the yaml file but it's being overwritten, because the
        // coordinator assigns a new value
        NES_DEBUG("NesWorker::connect() Coordinator assigned new workerId "
                  "value. Persisting workerId={} in yaml file",
                  workerId);
        bool success =
            getWorkerConfiguration()->persistWorkerIdInYamlConfigFile(
                workerConfig->configPath, workerId, true);
        if (!success) {
          NES_WARNING("NesWorker::connect() Could not persist workerId in yaml "
                      "config file");
        } else {
    ...
    NES_DEBUG("NesWorker::registerWorker rpc register success with id {}",
              workerId);
    connected = true;
    nodeEngine->setNodeId(workerId);
    healthCheckService = std::make_unique<WorkerHealthCheckService>(
        coordinatorRpcClient, HEALTH_SERVICE_NAME,
        this->inherited0::shared_from_this());
    NES_DEBUG("NesWorker start health check");
    healthCheckService->startHealthCheck();

    auto configPhysicalSourceTypes =
        workerConfig->physicalSourceTypes.getValues();
...
bool NesWorker::disconnect() {
  ...
  if (successfulPRCUnregister) {
    ...
    if (!configPhysicalSourceTypes.empty()) {
      ...
      NES_WARNING("NesWorker: stopping worker after de-registering the "
...
bool NesWorker::unregisterPhysicalSource(
    const std::vector<PhysicalSourceTypePtr> &physicalSources) {
  bool success =
      coordinatorRpcClient->unregisterPhysicalSource(physicalSources);
  NES_DEBUG("NesWorker::unregisterPhysicalSource success={}", success);
  return success;
}

const Configurations::WorkerConfigurationPtr &
NesWorker::getWorkerConfiguration() const {
  return workerConfig;
}

bool NesWorker::registerPhysicalSources(
  ...
  NES_ASSERT(con, "cannot connect");
  NES_ASSERT(coordinatorRpcClient->registerPhysicalSources(physicalSourceTypes),
             "Worker failed to register physical sources");
...
bool NesWorker::replaceParent(WorkerId oldParentId, WorkerId newParentId) {
  ...
  if (!success) {
    NES_WARNING("NesWorker::replaceParent() failed to replace oldParent={} "
                "with newParentId={}",
...
NesWorker::getQueryStatistics(SharedQueryId sharedQueryId) {
...
bool NesWorker::notifyQueryStatusChange(
  ...
  NES_ASSERT2_FMT(
      newStatus != Runtime::Execution::ExecutableQueryPlanStatus::Stopped,
      "Hard Stop called for query="
          << sharedQueryId << " subQueryId=" << decomposedQueryPlanId
          << " should not call notifyQueryStatusChange");
  if (newStatus == Runtime::Execution::ExecutableQueryPlanStatus::Finished) {
    NES_DEBUG("NesWorker {} about to notify soft stop completion for query {} "
              "subPlan {}",
              getWorkerId(), sharedQueryId, decomposedQueryPlanId);
    return coordinatorRpcClient->notifySoftStopCompleted(sharedQueryId,
                                                         decomposedQueryPlanId);
  } else if (newStatus ==
             Runtime::Execution::ExecutableQueryPlanStatus::ErrorState) {
    return true; // rpc to coordinator executed from async runner
  }
  return false;
}

bool NesWorker::canTriggerEndOfStream(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId,
    OperatorId sourceId, Runtime::QueryTerminationType terminationType) {
  NES_ASSERT(waitForConnect(), "cannot connect");
  NES_ASSERT(terminationType == Runtime::QueryTerminationType::Graceful,
             "invalid termination type");
  return coordinatorRpcClient->checkAndMarkForSoftStop(
      sharedQueryId, decomposedQueryPlanId, sourceId);
}

bool NesWorker::notifySourceTermination(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId,
    OperatorId sourceId, Runtime::QueryTerminationType queryTermination) {
  NES_ASSERT(waitForConnect(), "cannot connect");
  return coordinatorRpcClient->notifySourceStopTriggered(
      sharedQueryId, decomposedQueryPlanId, sourceId, queryTermination);
}

bool NesWorker::notifyQueryFailure(SharedQueryId sharedQueryId,
                                   DecomposedQueryPlanId subQueryId,
                                   std::string errorMsg) {
  bool con = waitForConnect();
  NES_ASSERT(con, "Connection failed");
  bool success = coordinatorRpcClient->notifyQueryFailure(
      sharedQueryId, subQueryId, getWorkerId(), INVALID_OPERATOR_ID, errorMsg);
  NES_DEBUG("NesWorker::notifyQueryFailure success={}", success);
  return success;
}

bool NesWorker::notifyEpochTermination(uint64_t timestamp,
                                       uint64_t querySubPlanId) {
  bool con = waitForConnect();
  NES_ASSERT(con, "Connection failed");
  bool success =
      coordinatorRpcClient->notifyEpochTermination(timestamp, querySubPlanId);
...
bool NesWorker::notifyErrors(WorkerId pWorkerId, std::string errorMsg) {
  bool con = waitForConnect();
  NES_ASSERT(con, "Connection failed");
  NES_DEBUG("NesWorker::sendErrors worker {} going to send error={}", pWorkerId,
            errorMsg);
...
void NesWorker::onFatalError(int signalNumber, std::string callstack) {
  ...
  if (callstack.empty()) {
    ...
    std::cerr << "NesWorker failed fatally"
    ...
    errorMsg = "onFatalError: signal [" + std::to_string(signalNumber) +
    ...
    // save errors in errorMsg
    errorMsg = "onFatalError: signal [" + std::to_string(signalNumber) +
               "] error [" + strerror(errno) + "] callstack " + callstack;
  }
...
void NesWorker::onFatalException(std::shared_ptr<std::exception> ptr,
  ...
  if (callstack.empty()) {
    ...
    // save errors in errorMsg
    errorMsg =
        "onFatalException: exception=[" + std::string(ptr->what()) + "] ";
  } else {
    NES_ERROR("onFatalException: exception=[{}] callstack={}", ptr->what(),
              callstack);
    std::cerr << "NesWorker failed fatally" << std::endl;
    std::cerr << "Error: " << strerror(errno) << std::endl;
    std::cerr << "Exception: " << ptr->what() << std::endl;
...
NesWorker::getLocationProvider() {
  return locationProvider;
}

NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictorPtr
NesWorker::getTrajectoryPredictor() {
  return trajectoryPredictor;
}

NES::Spatial::Mobility::Experimental::WorkerMobilityHandlerPtr
NesWorker::getMobilityHandler() {
  return workerMobilityHandler;
}
