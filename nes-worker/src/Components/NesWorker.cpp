...
NesWorker::NesWorker(Configurations::WorkerConfigurationPtr&& pWorkerConfig, Monitoring::MetricStorePtr metricStore)
    : workerConfig(std::move(pWorkerConfig)), localWorkerRpcPort(workerConfig->rpcPort), workerId(INVALID_WORKER_NODE_ID),
    ...
    workerId = INVALID_WORKER_NODE_ID;
...
NesWorker::~NesWorker() {
    try {
        ...
        NES_ERROR("NesWorker: Error occurred in destructor while stopping");
        throw Exceptions::RuntimeException("NesWorker error while stopping node engine");
    }
}
