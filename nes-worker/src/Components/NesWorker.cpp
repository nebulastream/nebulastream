...
NesWorker::NesWorker(Configurations::WorkerConfigurationPtr workerConfig,
    ...
    : workerConfig(workerConfig),
      localWorkerRpcPort(workerConfig->rpcPort),
...
bool NesWorker::start(bool blocking, bool withConnect) {
  NES_DEBUG(
      "NesWorker: start with blocking {} workerId={} coordinatorHost={} "
  ...
  NES_DEBUG(
      "NesWorker: request startWorkerRPCServer for accepting messages for "
      "address={}: {}",
  ...
  NES_DEBUG(
      "NesWorker: startWorkerRPCServer ready for accepting messages for "
      "address={}: {}",
      rpcAddress, localWorkerRpcPort.load());
  ...
  if (successPRCRegister) {
    if (workerId != workerConfig->workerId) {
      if (workerConfig->workerId == INVALID_WORKER_NODE_ID) {
        ...
        NES_DEBUG(
            "NesWorker::connect() Persisting workerId={} in yaml file for the "
            "first time.",
            workerId);
        bool success =
        ...
        if (!success) {
          NES_WARNING(
              "NesWorker::connect() Could not persist workerId in yaml config "
              "file");
        } else {
          NES_DEBUG(
              "NesWorker::connect() Persisted workerId={} successfully in yaml "
              "file.",
              workerId);
        }
      } else {
        ...
        NES_DEBUG(
            "NesWorker::connect() Coordinator assigned new workerId value. "
            "Persisting workerId={} in yaml file",
            workerId);
        bool success =
            getWorkerConfiguration()->persistWorkerIdInYamlConfigFile(
...
bool NesWorker::disconnect() {
  ...
  if (successfulPRCUnregister) {
    ...
    if (!configPhysicalSourceTypes.empty()) {
      ...
      NES_WARNING(
          "NesWorker: stopping worker after de-registering the registered "
          "physical sources.");
...
bool NesWorker::replaceParent(WorkerId oldParentId, WorkerId newParentId) {
  ...
  if (!success) {
    NES_WARNING(
        "NesWorker::replaceParent() failed to replace oldParent={} with "
        "newParentId={}",
...
std::vector<Runtime::QueryStatisticsPtr> NesWorker::getQueryStatistics(
...
bool NesWorker::notifyQueryStatusChange(
  ...
  if (newStatus == Runtime::Execution::ExecutableQueryPlanStatus::Finished) {
    NES_DEBUG(
        "NesWorker {} about to notify soft stop completion for query {} "
        "subPlan {}",
