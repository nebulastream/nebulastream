    ...
    const Configurations::Spatial::Mobility::Experimental::WorkerMobilityConfigurationPtr& configuration)
{
    ...
    {
        NES_FATAL_ERROR(
            "Default Coverage Radius: {} is bigger than the node index update threshold: {}. this would lead to "
    ...
    {
        NES_FATAL_ERROR(
            "Node info download radius: {} is smaller than the node index update threshold: {}. this would lead to "
...
bool NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::updateAverageMovementSpeed()
{
    ...
    S1Angle bufferDistance(
        Spatial::Util::S2Utilities::geoLocationToS2Point(locationBuffer.front().getLocation()),
...
bool NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::updatePredictedPath(
    const Spatial::DataTypes::Experimental::GeoLocation& newPathStart, const Spatial::DataTypes::Experimental::GeoLocation& currentLocation)
{
#ifdef S2DEF
...
#ifdef S2DEF
std::pair<S2Point, S1Angle> NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::findPathCoverage(
    const S2Polyline& path, S2Point coveringNode, S1Angle coverage)
{
    int vertexIndex = 0;
...
bool NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::scheduleReconnects(
    const S2Point& currentParentLocation, const S2PointIndex<WorkerId>& fieldNodeIndex)
{
    ...
    auto reconnectionPointTuple = findPathCoverage(trajectoryLine.value(), currentParentLocation, S1Angle(defaultCoverageRadiusAngle));
    ...
    while (currentUncoveredRemainingPathDistance > S1Angle(defaultCoverageRadiusAngle))
    {
        ...
        if (nextReconnectLocationOnPath.operator!=(reconnectLocationOnPath))
        {
            ...
            auto currLoc = std::make_shared<DataTypes::Experimental::GeoLocation>(currLatLng.lat().degrees(), currLatLng.lng().degrees());
