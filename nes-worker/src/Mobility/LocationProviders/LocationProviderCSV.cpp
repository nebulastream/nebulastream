...
void LocationProviderCSV::loadMovementSimulationDataFromCsv() {
  ...
  // read locations and time offsets from csv, calculate absolute timestamps
  // from offsets by adding start time
  while (std::getline(inputStream, csvLine)) {
    std::stringstream stringStream(csvLine);
    std::vector<std::string> values;
    try {
      values =
          NES::Util::splitWithStringDelimiter<std::string>(csvLine, delimiter);
    } catch (std::exception &exception) {
      std::string errorString =
          std::string("An error occurred while splitting delimiter of waypoint "
                      "CSV. ERROR: ") +
    ...
    if (values.size() != 3) {
      std::string errorString =
          std::string("LocationProviderCSV: could not read waypoints from csv, "
                      "expected 3 columns but input file has ") +
    ...
    try {
      ...
      std::string errorString =
          std::string(
              "An error occurred while creating the waypoint. ERROR: ") +
          strerror(errno) + " message=" + exception.what();
      NES_ERROR("LocationProviderCSV: {}", errorString);
      throw Spatial::Exception::LocationProviderException(errorString);
    }
    NES_TRACE("Read from csv: {}, {}, {}", latitudeString, longitudeString,
              time);

    // add startTime to the offset obtained from csv to get absolute timestamp
    time += startTime;

    // construct a pair containing a location and the time at which the device
    // is at exactly that point
    //  and add it to a vector containing all waypoints
    auto waypoint = DataTypes::Experimental::Waypoint(
        DataTypes::Experimental::GeoLocation(latitude, longitude), time);
    waypoints.push_back(waypoint);
  }
  if (waypoints.empty()) {
    auto errorString =
        std::string("No data in CSV, cannot start location provider");
    NES_ERROR("LocationProviderCSV: {}", errorString);
    throw Spatial::Exception::LocationProviderException(errorString);
  }
...
DataTypes::Experimental::Waypoint LocationProviderCSV::getCurrentWaypoint() {
  ...
  // get the time the request is made so we can compare it to the timestamps in
  ...
  while (nextWaypointIndex < waypoints.size() &&
         getWaypointAt(nextWaypointIndex).getTimestamp().value() <
             requestTime) {
    nextWaypointIndex++;
  }
  ...
  NES_TRACE("Location: {}; Time:{}", currentWayPoint.getLocation().toString(),
            currentWayPoint.getTimestamp().value());
  return currentWayPoint;
}

Timestamp LocationProviderCSV::getStartTime() const { return startTime; }

DataTypes::Experimental::Waypoint
LocationProviderCSV::getWaypointAt(size_t index) {
