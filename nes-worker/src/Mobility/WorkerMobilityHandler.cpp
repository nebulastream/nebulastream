...
namespace NES {
NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    WorkerMobilityHandler(
        ...
        const Configurations::Spatial::Mobility::Experimental::
            WorkerMobilityConfigurationPtr &mobilityConfiguration)
    : updateInterval(mobilityConfiguration->mobilityHandlerUpdateInterval),
      nodeInfoDownloadRadius(mobilityConfiguration->nodeInfoDownloadRadius),
      isRunning(false), nodeEngine(std::move(nodeEngine)),
      locationProvider(locationProvider),
      coordinatorRpcClient(std::move(coordinatorRpcClient)) {
#ifdef S2DEF
  locationUpdateThreshold = S2Earth::MetersToAngle(
      mobilityConfiguration->sendDevicePositionUpdateThreshold);
  coveredRadiusWithoutThreshold = S2Earth::MetersToAngle(
      nodeInfoDownloadRadius -
      mobilityConfiguration->nodeIndexUpdateThreshold.getValue());
  defaultCoverageRadiusAngle = S2Earth::MetersToAngle(
      mobilityConfiguration->defaultCoverageRadius.getValue());
  reconnectSchedulePredictor =
      std::make_shared<ReconnectSchedulePredictor>(mobilityConfiguration);
#endif
}

#ifdef S2DEF
bool NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    updateNeighbourWorkerInformation(
  ...
  auto nodeMapPtr =
      getNodeIdsInRange(currentLocation, nodeInfoDownloadRadius / 1000);
  ...
  for (auto [nodeId, location] : nodeMapPtr) {
    ...
    neighbourWorkerSpatialIndex.Add(
        S2Point(S2LatLng::FromDegrees(location.getLatitude(),
                                      location.getLongitude())),
        nodeId);
    neighbourWorkerIdToLocationMap.insert(
        {nodeId, S2Point(S2LatLng::FromDegrees(location.getLatitude(),
                                               location.getLongitude()))});
  }
  return true;
}
...
bool NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    shouldUpdateNeighbouringWorkerInformation(
  ...
  S2Point currentS2Point(S2LatLng::FromDegrees(currentLocation.getLatitude(),
                                               currentLocation.getLongitude()));
  /*check if we have moved close enough to the edge of the area covered by the
  current node index so the we need to download new node information */
  return !centroidOfNeighbouringWorkerSpatialIndex ||
         S1Angle(currentS2Point,
                 centroidOfNeighbouringWorkerSpatialIndex.value()) >
             coveredRadiusWithoutThreshold;
}
#endif

bool NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    sendNextPredictedReconnect(
        const std::optional<
            NES::Spatial::Mobility::Experimental::ReconnectSchedule>
            &scheduledReconnects,
        const std::optional<
            NES::Spatial::Mobility::Experimental::ReconnectSchedule>
            &removedReconnects) {
...
NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    getNextReconnectPoint(
        std::optional<ReconnectSchedule> &reconnectSchedule,
        const DataTypes::Experimental::GeoLocation &currentOwnLocation,
        const std::optional<NES::Spatial::DataTypes::Experimental::GeoLocation>
            &currentParentLocation,
        const S2PointIndex<WorkerId> &neighbourWorkerSpatialIndex) {

  // if the current parent location is not known, try reconnecting to the
  // closest node
  if (!currentParentLocation.has_value()) {
    auto closesNodeId =
        getClosestNodeId(currentOwnLocation, defaultCoverageRadiusAngle,
                         neighbourWorkerSpatialIndex);
    if (closesNodeId) {
      return NES::Spatial::Mobility::Experimental::ReconnectPoint{
          {}, closesNodeId.value(), 0};
    } else {
      return std::nullopt;
    }
  }

  auto currentParentPoint =
      NES::Spatial::Util::S2Utilities::geoLocationToS2Point(
          currentParentLocation.value());
  auto currentOwnPoint =
      NES::Spatial::Util::S2Utilities::geoLocationToS2Point(currentOwnLocation);
  S1Angle currentDistFromParent(currentOwnPoint, currentParentPoint);

  // get the next scheduled reconnect from the reconnect schedule if there is
  // one
  std::optional<NES::Spatial::Mobility::Experimental::ReconnectPoint>
      nextScheduledReconnect;
  ...
  if (currentDistFromParent >= defaultCoverageRadiusAngle) {
    // if there is no scheduled reconnect, connect to the closest node we can
    // find
    if (!nextScheduledReconnect) {
      ...
      if (closesNodeId) {
        ...
        return std::nullopt;
                       ...
                       NES::Spatial::Util::S2Utilities::geoLocationToS2Point(
                           nextScheduledReconnect->pointGeoLocation)) <=
               currentDistFromParent) {
      // if the next scheduled parent is closer than the current parent,
      // reconnect to the current parent
...
NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::getClosestNodeId(
    const DataTypes::Experimental::GeoLocation &currentOwnLocation,
    S1Angle radius, const S2PointIndex<WorkerId> &neighbourWorkerSpatialIndex) {
  auto currentOwnPoint =
      NES::Spatial::Util::S2Utilities::geoLocationToS2Point(currentOwnLocation);
...
NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::getNodeGeoLocation(
  ...
  if (neighbourWorkerIdToLocationMap.contains(nodeId)) {
    return NES::Spatial::Util::S2Utilities::s2pointToLocation(
        neighbourWorkerIdToLocationMap.at(nodeId));
  }
  return std::nullopt;
}
#endif
...
bool NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    triggerReconnectionRoutine(WorkerId &currentParentId,
  ...
  bool success =
      coordinatorRpcClient->relocateTopologyNode({removedLink}, {addedLink});
  if (success) {
    // update locally saved information about parent
    currentParentId = newParentId;
  } else {
    NES_WARNING("WorkerMobilityHandler::replaceParent() failed to replace "
                "oldParent={} with newParentId={}.",
                currentParentId, newParentId);
    // todo: #3572 query coordinator for actual parent to recover from faulty
    // state
  }
...
#ifdef S2DEF
bool NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    shouldSendCurrentWaypointToCoordinator(
        const std::optional<S2Point> &lastTransmittedLocation,
        const DataTypes::Experimental::GeoLocation &currentLocation) {
  auto currentPoint =
      NES::Spatial::Util::S2Utilities::geoLocationToS2Point(currentLocation);
  return !lastTransmittedLocation.has_value() ||
         S1Angle(currentPoint, lastTransmittedLocation.value()) >
             locationUpdateThreshold;
}
#endif

void NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::
    sendCurrentWaypoint(
        const DataTypes::Experimental::Waypoint &currentWaypoint) {
...
void NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::start(
    const std::vector<WorkerId> &currentParentWorkerIds) {
  // TODO  #3365: reset state of schedule predictor
  // start periodically pulling location updates and inform coordinator about
  // location changes
  if (!isRunning) {
    NES_DEBUG("Starting scheduling and location update thread")
    isRunning = true;
    workerMobilityHandlerThread = std::make_shared<std::thread>(
        &WorkerMobilityHandler::run, this, currentParentWorkerIds);
  } else {
    NES_WARNING("Scheduling and location update thread already running")
  }
}

void NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::run(
    const std::vector<WorkerId> &currentParentWorkerIds) {
  ...
  std::optional<ReconnectSchedule> lastPrediction =
      NES::Spatial::Mobility::Experimental::ReconnectSchedule::Empty();
  std::optional<NES::Spatial::DataTypes::Experimental::GeoLocation>
      currentParentLocation = std::nullopt;

  std::optional<ReconnectSchedule> currentReconnectSchedule;

  // FIXME: currently only one parent per worker is supported. We therefore only
  // ever access the parent id vectors front
  ...
  while (isRunning) {
    ...
    if (!shouldSendCurrentWaypointToCoordinator(lastTransmittedLocation,
                                                currentLocation)) {
      NES_DEBUG("device has not moved further than threshold, location will "
                "not be transmitted");
    ...
    lastTransmittedLocation =
        NES::Spatial::Util::S2Utilities::geoLocationToS2Point(
            currentWaypoint.getLocation());

    // update the neighbouring worker index if necessary
    bool indexUpdated = false;
    if (shouldUpdateNeighbouringWorkerInformation(
            centroidOfNeighbouringWorkerSpatialIndex, currentWaypoint)) {
      indexUpdated = updateNeighbourWorkerInformation(
          currentWaypoint.getLocation(), neighbourWorkerIdToLocationMap,
          neighbourWorkerSpatialIndex);
      if (indexUpdated) {
        centroidOfNeighbouringWorkerSpatialIndex =
            NES::Spatial::Util::S2Utilities::geoLocationToS2Point(
                currentLocation);
        NES_TRACE("setting last index update position to {}",
                  currentLocation.toString())
        currentParentLocation =
            getNodeGeoLocation(currentParentId, neighbourWorkerIdToLocationMap);
      } else {
    ...
    if (currentParentLocation) {
      ...
      newReconnectSchedule = reconnectSchedulePredictor->getReconnectSchedule(
          currentWaypoint, currentParentLocation.value(),
          neighbourWorkerSpatialIndex, indexUpdated);

      reconnectScheduleWasUpdated = newReconnectSchedule.has_value();
    } else if (currentReconnectSchedule.has_value()) {
      // if there is no known location for the current parent, the old reconnect
      // schedule cannot be valid
      newReconnectSchedule = std::nullopt;
      reconnectScheduleWasUpdated = true;
    }

    // remember the last prediction we made so the coordinator can be told to
    // undo it and record the new prediction
    ...
    auto nextReconnectPoint = getNextReconnectPoint(
        currentReconnectSchedule, currentLocation, currentParentLocation,
        neighbourWorkerSpatialIndex);

    // perform reconnect if needed
    if (nextReconnectPoint.has_value()) {
      triggerReconnectionRoutine(currentParentId,
                                 nextReconnectPoint.value().newParentId);
      currentParentLocation =
          getNodeGeoLocation(currentParentId, neighbourWorkerIdToLocationMap);
    }

    // if the schedule changed, the coordinator has to be informed about the
    // next predicted reconnect
    if (reconnectScheduleWasUpdated) {
...
// todo #3365: check if we need insert poison pill: request processor service
// for reference
...
NES::Spatial::Mobility::Experimental::WorkerMobilityHandler::getNodeIdsInRange(
    const DataTypes::Experimental::GeoLocation &location, double radius) {
  if (!coordinatorRpcClient) {
    NES_WARNING(
        "worker has no coordinator rpc client, cannot download node index");
    return {};
  }
  auto nodeVector = coordinatorRpcClient->getNodeIdsInRange(location, radius);
  return DataTypes::Experimental::NodeIdToGeoLocationMap{nodeVector.begin(),
                                                         nodeVector.end()};
}
} // namespace NES
