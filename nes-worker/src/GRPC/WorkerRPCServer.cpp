...
Status WorkerRPCServer::RegisterQuery(ServerContext*, const RegisterQueryRequest* request, RegisterQueryReply* reply)
{
    ...
    NES_DEBUG(
        "WorkerRPCServer::RegisterQuery: got decomposed query plan with shared query Id: {} and decomposed query plan Id: "
        "{} plan={}",
...
Status WorkerRPCServer::StartQuery(ServerContext*, const StartQueryRequest* request, StartQueryReply* reply)
{
    ...
    bool success = nodeEngine->startQuery(SharedQueryId(request->sharedqueryid()), DecomposedQueryPlanId(request->decomposedqueryid()));
    if (success)
    {
...
Status WorkerRPCServer::StopQuery(ServerContext*, const StopQueryRequest* request, StopQueryReply* reply)
{
    ...
    NES_ASSERT2_FMT(
        terminationType != Runtime::QueryTerminationType::Graceful && terminationType != Runtime::QueryTerminationType::Invalid,
        "Invalid termination type requested");
    bool success = nodeEngine->stopQuery(
        SharedQueryId(request->sharedqueryid()), DecomposedQueryPlanId(request->decomposedqueryid()), terminationType);
    if (success)
    {
...
Status WorkerRPCServer::RegisterMonitoringPlan(ServerContext*, const MonitoringRegistrationRequest* request, MonitoringRegistrationReply*)
{
    try
    {
...
Status WorkerRPCServer::UpdateNetworkSink(ServerContext*, const UpdateNetworkSinkRequest* request, UpdateNetworkSinkReply* reply)
{
