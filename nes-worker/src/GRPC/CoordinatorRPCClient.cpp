...
/**
 * @brief This methods perform an rpc by sending a request and waiting for a
 * reply. It may retry in the case of a failure.
 * @tparam ReturnType the return type of the rpc
 * @tparam RequestType type of the object sent as request
 * @tparam ReplyType type of the object expected as reply
 ...
 */
template <typename ReturnType, typename RequestType, typename ReplyType>
[[nodiscard]] ReturnType processRpc(
    const RequestType& request, uint32_t retryAttempts,
  ...
  for (uint32_t i = 0; i <= retryAttempts; ++i, backOffTime *= 2) {
    ...
    if (status.ok()) {
      ...
      usleep(std::chrono::duration_cast<std::chrono::microseconds>(backOffTime)
                 .count());
    } else {
      break;
    }
 ...
 */
template <typename ReturnType, typename RequestType, typename ReplyType>
[[nodiscard]] ReturnType processGenericRpc(
    const RequestType& request, uint32_t retryAttempts,
    std::chrono::milliseconds backOffTime,
    std::function<grpc::Status(ClientContext*, const RequestType&,
                               ReplyType*)>&& func) {
  class GenericRpcListener
      : public detail::RpcExecutionListener<bool, RequestType, ReplyType> {
   public:
    explicit GenericRpcListener(
        std::function<grpc::Status(ClientContext*, const RequestType&,
                                   ReplyType*)>&& func)
        : func(std::move(func)) {}
    virtual ~GenericRpcListener() = default;

    grpc::Status rpcCall(const RequestType& request,
                         ReplyType* reply) override {
      ClientContext context;

      return func(&context, request, reply);
    }
    bool onSuccess(const ReplyType& reply) override {
      NES_DEBUG("CoordinatorRPCClient::: status ok return success={}",
                reply.success());
      return reply.success();
    }
    bool onPartialFailure(const Status& status) override {
      NES_DEBUG(" CoordinatorRPCClient:: error={}: {}", status.error_code(),
   ...
   private:
    std::function<grpc::Status(ClientContext*, const RequestType&, ReplyType*)>
        func;
  };
...
std::shared_ptr<CoordinatorRPCClient> CoordinatorRPCClient::create(
    const std::string& address, uint32_t rpcRetryAttempts,
    std::chrono::milliseconds rpcBackoff) {
  NES_DEBUG("CoordinatorRPCClient(): creating channels to address ={}",
            address);
  auto rpcChannel =
  ...
  if (rpcChannel) {
    ...
    NES_THROW_RUNTIME_ERROR(
        "CoordinatorRPCClient::connecting error while creating channel");
  }

  return std::make_shared<CoordinatorRPCClient>(
      CoordinatorRPCService::NewStub(rpcChannel), address, rpcRetryAttempts,
      rpcBackoff);
}
...
CoordinatorRPCClient::CoordinatorRPCClient(
    ...
    std::chrono::milliseconds rpcBackoff)
    : address(address),
      coordinatorStub(std::move(coordinatorStub)),
      rpcRetryAttempts(rpcRetryAttempts),
      rpcBackoff(rpcBackoff) {}

bool CoordinatorRPCClient::registerPhysicalSources(
    const std::vector<PhysicalSourceTypePtr>& physicalSourceTypes) {
  NES_DEBUG(
      "CoordinatorRPCClient::registerPhysicalSources: got {}"
      " physical sources to register for worker with id {}",
      physicalSourceTypes.size(), workerId);
  ...
  for (const auto& physicalSourceType : physicalSourceTypes) {
    ...
    physicalSourceDefinition->set_sourcetype(
        physicalSourceType->getSourceTypeAsString());
    physicalSourceDefinition->set_physicalsourcename(
        physicalSourceType->getPhysicalSourceName());
  ...
  class PhysicalSourceRegistrationRequestListener
      : public detail::RpcExecutionListener<bool,
                                            RegisterPhysicalSourcesRequest,
                                            RegisterPhysicalSourcesReply> {
   public:
    explicit PhysicalSourceRegistrationRequestListener(
    ...
    grpc::Status rpcCall(const RegisterPhysicalSourcesRequest& request,
    ...
    bool onSuccess(const RegisterPhysicalSourcesReply& reply) override {
      if (!reply.success()) {
        for (const auto& result : reply.results()) {
          ...
          NES_ERROR(
              "Could not register physical source {} at the coordinator! "
              "Reason: {}",
              result.physicalsourcename(), result.reason());
        }
      }
      return reply.success();
    }
    ...
    bool onPartialFailure(const Status& status) override {
      ...
      // Currently the Server always returns Ok. Thus any other error might be
      // solved by a retry.
      return true;
    }
...
bool CoordinatorRPCClient::registerLogicalSource(
    const std::string& logicalSourceName, const std::string& filePath) {
  NES_DEBUG("CoordinatorRPCClient: registerLogicalSource {} with path{}",
            logicalSourceName, filePath);
  ...
  NES_DEBUG("CoordinatorRPCClient::RegisterLogicalSourceRequest request={}",
            request.DebugString());

  return detail::processGenericRpc<bool, RegisterLogicalSourceRequest,
                                   RegisterLogicalSourceReply>(
      request, rpcRetryAttempts, rpcBackoff,
      [this](ClientContext* context,
             const RegisterLogicalSourceRequest& innerRequest,
             RegisterLogicalSourceReply* reply) {
        return coordinatorStub->RegisterLogicalSource(context, innerRequest,
                                                      reply);
      });
}

bool CoordinatorRPCClient::unregisterPhysicalSource(
    const std::vector<PhysicalSourceTypePtr>& physicalSourceTypes) {
  for (const auto& physicalSourceType : physicalSourceTypes) {
    NES_DEBUG(
        "CoordinatorRPCClient: unregisterPhysicalSource physical source {} "
        "from logical source",
    ...
    request.set_logicalsourcename(logicalSourceName);
    NES_DEBUG(
        "CoordinatorRPCClient::UnregisterPhysicalSourceRequest request={}",
        request.DebugString());

    bool success =
        detail::processGenericRpc<bool, UnregisterPhysicalSourceRequest,
                                  UnregisterPhysicalSourceReply>(
            request, rpcRetryAttempts, rpcBackoff,
            [this](ClientContext* context,
                   const UnregisterPhysicalSourceRequest& innerRequest,
                   UnregisterPhysicalSourceReply* reply) {
              return coordinatorStub->UnregisterPhysicalSource(
                  context, innerRequest, reply);
            });

    if (!success) {
      NES_ERROR(
          "Unable to unregister physical source {} for the logical source {}.",
          physicalSourceName, logicalSourceName)
      return success;
    }
  }
  return true;
...
bool CoordinatorRPCClient::unregisterLogicalSource(
    const std::string& logicalSourceName) {
  NES_DEBUG("CoordinatorRPCClient: unregisterLogicalSource source{}",
            logicalSourceName);

  UnregisterLogicalSourceRequest request;
  request.set_workerid(workerId.getRawValue());
  request.set_logicalsourcename(logicalSourceName);
  NES_DEBUG("CoordinatorRPCClient::UnregisterLogicalSourceRequest request={}",
            request.DebugString());

  return detail::processGenericRpc<bool, UnregisterLogicalSourceRequest,
                                   UnregisterLogicalSourceReply>(
      request, rpcRetryAttempts, rpcBackoff,
      [this](ClientContext* context,
             const UnregisterLogicalSourceRequest& innerRequest,
             UnregisterLogicalSourceReply* reply) {
        return coordinatorStub->UnregisterLogicalSource(context, innerRequest,
                                                        reply);
      });
}

bool CoordinatorRPCClient::addParent(WorkerId parentId) {
  NES_DEBUG("CoordinatorRPCClient: addParent parentId={} workerId={}", parentId,
            workerId);

  AddParentRequest request;
  request.set_parentid(parentId.getRawValue());
  request.set_childid(workerId.getRawValue());
  NES_DEBUG("CoordinatorRPCClient::AddParentRequest request={}",
            request.DebugString());

  return detail::processGenericRpc<bool, AddParentRequest, AddParentReply>(
      request, rpcRetryAttempts, rpcBackoff,
      [this](ClientContext* context, const AddParentRequest& innerRequest,
             AddParentReply* reply) {
        return coordinatorStub->AddParent(context, innerRequest, reply);
      });
}

bool CoordinatorRPCClient::replaceParent(WorkerId oldParentId,
                                         WorkerId newParentId) {
  NES_DEBUG(
      "CoordinatorRPCClient: replaceParent oldParentId={} newParentId={} "
      "workerId={}",
      oldParentId, newParentId, workerId);

  ReplaceParentRequest request;
  request.set_childid(workerId.getRawValue());
  request.set_oldparent(oldParentId.getRawValue());
  request.set_newparent(newParentId.getRawValue());
  NES_DEBUG("CoordinatorRPCClient::replaceParent request={}",
            request.DebugString());

  class ReplaceParentListener
      : public detail::RpcExecutionListener<bool, ReplaceParentRequest,
                                            ReplaceParentReply> {
   public:
    std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub;

    explicit ReplaceParentListener(
        std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub)
        : coordinatorStub(coordinatorStub) {}

    virtual ~ReplaceParentListener() = default;

    Status rpcCall(const ReplaceParentRequest& request,
                   ReplaceParentReply* reply) override {
      ClientContext context;
    ...
    bool onSuccess(const ReplaceParentReply& reply) override {
      NES_DEBUG(
          "CoordinatorRPCClient::removeAsParent: status ok return success={}",
          reply.success());
      return reply.success();
    }
    bool onPartialFailure(const Status& status) override {
      NES_DEBUG(" CoordinatorRPCClient::removeAsParent error={}: {}",
                status.error_code(), status.error_message());
      return false;
    }
    bool onFailure() override { return false; }
...
bool CoordinatorRPCClient::removeParent(WorkerId parentId) {
  NES_DEBUG("CoordinatorRPCClient: removeAsParent parentId{} workerId={}",
            parentId, workerId);

  RemoveParentRequest request;
  request.set_parentid(parentId.getRawValue());
  request.set_childid(workerId.getRawValue());
  NES_DEBUG("CoordinatorRPCClient::RemoveParentRequest request={}",
            request.DebugString());

  class RemoveParentListener
      : public detail::RpcExecutionListener<bool, RemoveParentRequest,
                                            RemoveParentReply> {
   public:
    std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub;

    explicit RemoveParentListener(
        std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub)
        : coordinatorStub(coordinatorStub) {}

    virtual ~RemoveParentListener() = default;

    Status rpcCall(const RemoveParentRequest& request,
...
bool CoordinatorRPCClient::unregisterNode() {
  ...
  request.set_workerid(workerId.getRawValue());
  NES_DEBUG("CoordinatorRPCClient::unregisterNode request={}",
            request.DebugString());

  class UnRegisterNodeListener
      : public detail::RpcExecutionListener<bool, UnregisterWorkerRequest,
                                            UnregisterWorkerReply> {
   public:
    std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub;

    explicit UnRegisterNodeListener(
        std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub)
        : coordinatorStub(coordinatorStub) {}

    virtual ~UnRegisterNodeListener() = default;

    Status rpcCall(const UnregisterWorkerRequest& request,
                   UnregisterWorkerReply* reply) override {
      ClientContext context;
    ...
    bool onSuccess(const UnregisterWorkerReply& reply) override {
      NES_DEBUG(
          "CoordinatorRPCClient::unregisterNode: status ok return success={}",
          reply.success());
      return reply.success();
    }
    bool onPartialFailure(const Status& status) override {
...
bool CoordinatorRPCClient::registerWorker(
    const RegisterWorkerRequest& registrationRequest) {
  NES_DEBUG("CoordinatorRPCClient::RegisterNodeRequest request={}",
            registrationRequest.DebugString());

  class RegisterWorkerListener
      : public detail::RpcExecutionListener<bool, RegisterWorkerRequest,
                                            RegisterWorkerReply> {
   public:
    WorkerId& workerId;
    std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub;

    explicit RegisterWorkerListener(
        WorkerId& workerId,
        std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub)
    ...
    Status rpcCall(const RegisterWorkerRequest& request,
    ...
    bool onPartialFailure(const Status& status) override {
      NES_ERROR(" CoordinatorRPCClient::registerWorker error={}:{}",
                status.error_code(), status.error_message());
  ...
  return detail::processRpc(registrationRequest, rpcRetryAttempts, rpcBackoff,
                            listener);
}
...
bool CoordinatorRPCClient::notifyQueryFailure(SharedQueryId sharedQueryId,
  ...
  class NotifyQueryFailureListener
      : public detail::RpcExecutionListener<bool, QueryFailureNotification,
                                            QueryFailureNotificationReply> {
   public:
    std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub;

    explicit NotifyQueryFailureListener(
        std::unique_ptr<CoordinatorRPCService::StubInterface>& coordinatorStub)
        : coordinatorStub(coordinatorStub) {}

    virtual ~NotifyQueryFailureListener() = default;

    Status rpcCall(const QueryFailureNotification& request,
...
CoordinatorRPCClient::getNodeIdsInRange(
    const Spatial::DataTypes::Experimental::GeoLocation& geoLocation,
    double radius) {
  if (!geoLocation.isValid()) {
    return {};
  }
  GetNodesInRangeRequest request;
  NES::Spatial::Protobuf::GeoLocation* pCoordinates =
      request.mutable_geolocation();
  ...
  std::vector<
      std::pair<WorkerId, Spatial::DataTypes::Experimental::GeoLocation>>
      nodesInRange;
  for (const auto& workerLocation : *reply.mutable_nodes()) {
    nodesInRange.emplace_back(workerLocation.id(),
...
bool CoordinatorRPCClient::checkCoordinatorHealth(
    std::string healthServiceName) const {
  std::shared_ptr<::grpc::Channel> chan =
      grpc::CreateChannel(address, grpc::InsecureChannelCredentials());
  std::unique_ptr<grpc::health::v1::Health::StubInterface> workerStub =
      grpc::health::v1::Health::NewStub(chan);
  ...
  if (status.ok()) {
    NES_TRACE("CoordinatorRPCClient::checkHealth: status ok return success={}",
              response.status());
    return response.status();
  } else {
    NES_ERROR("CoordinatorRPCClient::checkHealth error={}:{}",
              status.error_code(), status.error_message());
    return response.status();
  }
}

bool CoordinatorRPCClient::notifyEpochTermination(uint64_t timestamp,
  ...
  ClientContext context;
  Status status =
      coordinatorStub->NotifyEpochTermination(&context, request, &reply);
  if (status.ok()) {
...
bool CoordinatorRPCClient::sendErrors(WorkerId pWorkerId,
                                      std::string errorMsg) {
  // create & fill the protobuf
...
bool CoordinatorRPCClient::checkAndMarkForSoftStop(
  ...
  ClientContext context;
  coordinatorStub->RequestSoftStop(&context, requestSoftStopMessage,
                                   &stopRequestReply);
...
bool CoordinatorRPCClient::notifySourceStopTriggered(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId,
    OperatorId sourceId, Runtime::QueryTerminationType queryTermination) {
  ...
  softStopTriggeredMessage.set_querysubplanid(
      decomposedQueryPlanId.getRawValue());
  ...
  ClientContext context;
  coordinatorStub->notifySourceStopTriggered(&context, softStopTriggeredMessage,
                                             &softStopTriggeredReply);

  // return the response
  return softStopTriggeredReply.success();
}

bool CoordinatorRPCClient::notifySoftStopCompleted(
    SharedQueryId sharedQueryId, DecomposedQueryPlanId decomposedQueryPlanId) {
  // Build request
  SoftStopCompletionMessage softStopCompletionMessage;
  softStopCompletionMessage.set_queryid(sharedQueryId.getRawValue());
  softStopCompletionMessage.set_querysubplanid(
      decomposedQueryPlanId.getRawValue());
  ...
  ClientContext context;
  coordinatorStub->NotifySoftStopCompleted(&context, softStopCompletionMessage,
                                           &softStopCompletionReply);
...
bool CoordinatorRPCClient::sendReconnectPrediction(
    const std::vector<Spatial::Mobility::Experimental::ReconnectPoint>&
        addPredictions,
    const std::vector<Spatial::Mobility::Experimental::ReconnectPoint>&
        removePredictions) {
  ClientContext context;
...
bool CoordinatorRPCClient::sendLocationUpdate(
    const Spatial::DataTypes::Experimental::Waypoint& locationUpdate) {
  ClientContext context;
  ...
  NES::Spatial::Protobuf::GeoLocation* geoLocation =
      waypoint->mutable_geolocation();
...
bool CoordinatorRPCClient::relocateTopologyNode(
    const std::vector<TopologyLinkInformation>& removedTopologyLinks,
