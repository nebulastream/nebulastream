...
processRpc(const RequestType &request, uint32_t retryAttempts,
...
CoordinatorRPCClient::create(const std::string &address,
...
CoordinatorRPCClient::CoordinatorRPCClient(
    ...
    std::chrono::milliseconds rpcBackoff)
    : address(address), coordinatorStub(std::move(coordinatorStub)),
      rpcRetryAttempts(rpcRetryAttempts), rpcBackoff(rpcBackoff) {}
    ...
    bool onSuccess(const RegisterPhysicalSourcesReply &reply) override {
      if (!reply.success()) {
        for (const auto &result : reply.results()) {
          ...
          NES_ERROR("Could not register physical source {} at the coordinator! "
                    "Reason: {}",
...
bool CoordinatorRPCClient::unregisterPhysicalSource(
  ...
  for (const auto &physicalSourceType : physicalSourceTypes) {
    ...
    NES_DEBUG("CoordinatorRPCClient: unregisterPhysicalSource physical source "
              "{} from logical source",
...
bool CoordinatorRPCClient::replaceParent(WorkerId oldParentId,
  ...
  NES_DEBUG("CoordinatorRPCClient: replaceParent oldParentId={} newParentId={} "
            "workerId={}",
