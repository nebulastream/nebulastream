...
WorkerHealthCheckService::WorkerHealthCheckService(
    CoordinatorRPCClientPtr coordinatorRpcClient, std::string healthServiceName, NesWorkerPtr worker)
    : coordinatorRpcClient(coordinatorRpcClient), worker(worker), id(coordinatorRpcClient->getId()), healthServiceName(healthServiceName)
{
}
...
void WorkerHealthCheckService::startHealthCheck()
{
    ...
    healthCheckingThread = std::make_shared<std::thread>((
        [this]()
        {
            ...
            while (isRunning)
            {
                ...
                {
                    NES_ERROR(
                        "NesWorker::healthCheck: for worker id={} coordinator went down so shutting down the worker with ip",
