  ...
  healthCheckingThread = std::make_shared<std::thread>(([this]() {
    ...
    while (isRunning) {
      ...
      if (isAlive) {
        ...
        NES_ERROR(
            "NesWorker::healthCheck: for worker id={} coordinator went down so "
            "shutting down the worker with ip",
            coordinatorRpcClient->getId());
        worker->stop(true);
      }
      {
...
void WorkerHealthCheckService::stopHealthCheck() {
  ...
  if (!isRunning.compare_exchange_strong(expected, false)) {
    NES_DEBUG(
        "AbstractHealthCheckService::stopHealthCheck health check already "
        "stopped");
    return;
  }
  {
