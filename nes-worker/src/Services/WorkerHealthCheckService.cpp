...
WorkerHealthCheckService::WorkerHealthCheckService(
    CoordinatorRPCClientPtr coordinatorRpcClient, std::string healthServiceName,
    NesWorkerPtr worker)
    : coordinatorRpcClient(coordinatorRpcClient), worker(worker),
      id(coordinatorRpcClient->getId()), healthServiceName(healthServiceName) {}
...
void WorkerHealthCheckService::startHealthCheck() {
  ...
  healthCheckingThread = std::make_shared<std::thread>(([this]() {
    ...
    auto waitTime = std::chrono::seconds(
        worker->getWorkerConfiguration()->workerHealthCheckWaitTime.getValue());
    while (isRunning) {
      NES_TRACE("NesWorker::healthCheck for worker id=  {}",
                coordinatorRpcClient->getId());

      bool isAlive =
          coordinatorRpcClient->checkCoordinatorHealth(healthServiceName);
      if (isAlive) {
        NES_TRACE("NesWorker::healthCheck: for worker id={} is alive",
                  coordinatorRpcClient->getId());
      } else {
        NES_ERROR("NesWorker::healthCheck: for worker id={} coordinator went "
                  "down so shutting down the worker with ip",
                  coordinatorRpcClient->getId());
        worker->stop(true);
      }
      {
        std::unique_lock<std::mutex> lk(cvMutex);
        cv.wait_for(lk, waitTime, [this] { return !isRunning; });
      }
    }
    //        we have to wait until the code above terminates to proceed
    //        afterwards with shutdown of the rpc server (can be delayed due to
    //        sleep)
...
void WorkerHealthCheckService::stopHealthCheck() {
  ...
  if (!isRunning.compare_exchange_strong(expected, false)) {
    NES_DEBUG("AbstractHealthCheckService::stopHealthCheck health check "
              "already stopped");
    return;
  }
  {
  ...
  if (healthCheckingThread->joinable()) {
    ...
    NES_DEBUG(
        "AbstractHealthCheckService::stopHealthCheck successfully stopped");
  } else {
