...
class CoordinatorRPCClientTest : public Testing::BaseUnitTest {
 public:
  void SetUp() override {
    ...
    // IMPORTANT: We reset the logger for every testcase, because we are
 ...
 protected:
  ...
  static std::tuple<std::unique_ptr<CoordinatorRPCClient>,
                    MockCoordinatorRPCServiceStub&>
  defaultUUT() {
    using namespace std::chrono_literals;
    auto stub = std::make_unique<MockCoordinatorRPCServiceStub>();
    auto& stubRef = *stub;
    return {std::make_unique<CoordinatorRPCClient>(std::move(stub),
                                                   "123.123.123.123", 2, 15ms),
            stubRef};
  }

  std::function<bool(std::string_view)> getLineFilter(LogLevel logLevel) const {
    // Assumes the standard SPDLOG_PATTERN
    // Assumes that the log level is represented by [X] where X is the first
    // letter of the LogLevel with the `LOG_` prefix LOG_ERROR - E, LOG_WARNING
    // - W, LOG_INFO - I, ...
    std::regex timestampRegex(
        "^\\[\\d{2}:\\d{2}:\\d{2}\\.\\d{6}\\] \\[" +
        std::string(magic_enum::enum_name(logLevel)).substr(4, 1) + "\\] ");
    return [timestampRegex =
                std::move(timestampRegex)](const std::string_view line) {
  ...
  void assertErrorMessageInLogOutput(
      std::string_view errorMessage, size_t numberOfOccurences = 1,
    ...
    if (logLevel) {
      EXPECT_GE(NES_COMPILE_TIME_LOG_LEVEL, getLogLevel(*logLevel))
          << "NES_COMPILE_TIME_LOG_LEVEL prevents log output assertions on "
             "level: "
          << magic_enum::enum_name(*logLevel);
      EXPECT_NE(LogLevel::LOG_NONE, *logLevel)
          << "Testing for logs with LogLevel::LOG_NONE is pointless";
    }
    auto lineFilterFunction =
        logLevel ? getLineFilter(*logLevel) : getNoOpLineFilter();
    ...
    EXPECT_EQ(occurences, numberOfOccurences) << fmt::format(
        "Could not find the Error Message \"{}\" in the log {} times",
        errorMessage, numberOfOccurences);
  }
...
TEST_F(CoordinatorRPCClientTest, TestEmptyPhysicalSourceRegistrationRequest) {
  ...
  EXPECT_CALL(mock, RegisterPhysicalSource(_, _, _))
      .Times(1)
      .WillOnce(DoAll(SaveArg<1>(&expected), SetArgPointee<2>(reply),
                      Return(Status::OK)));
...
TEST_F(CoordinatorRPCClientTest, TestValidPhysicalSourceRegistrationRequest) {
  ...
  auto valid = DefaultSourceType::create(
      VALID_LOGICAL_SOURCE_NAME,
      fmt::format("{}_{}", VALID_LOGICAL_SOURCE_NAME, 1));

  EXPECT_TRUE(uut->registerPhysicalSources({valid}));
  EXPECT_EQ(expected.workerid(), INVALID_WORKER_NODE_ID.getRawValue());
  EXPECT_THAT(expected.physicalsourcetypes(), SizeIs(1));
  EXPECT_EQ(expected.physicalsourcetypes(0).logicalsourcename(),
            VALID_LOGICAL_SOURCE_NAME);
  EXPECT_EQ(expected.physicalsourcetypes(0).physicalsourcename(),
            fmt::format("{}_{}", VALID_LOGICAL_SOURCE_NAME, 1));
  EXPECT_EQ(expected.physicalsourcetypes(0).sourcetype(),
            magic_enum::enum_name(SourceType::DEFAULT_SOURCE));
}

TEST_F(CoordinatorRPCClientTest,
  ...
  }
  {
    ...
    result->set_reason(
        "IF_THIS_APPEARS_IN_THE_ERROR_MESSAGE_THE_TEST_WAS_SUCCESSFUL");
  }
  ...
  auto valid = DefaultSourceType::create(
      VALID_LOGICAL_SOURCE_NAME,
      fmt::format("{}_{}", VALID_LOGICAL_SOURCE_NAME, 1));
  auto invalid = DefaultSourceType::create(
  ...
  EXPECT_EQ(expected.physicalsourcetypes(1).logicalsourcename(),
            INVALID_LOGICAL_SOURCE_NAME);
  EXPECT_EQ(expected.physicalsourcetypes(1).physicalsourcename(),
            fmt::format("{}_{}", INVALID_LOGICAL_SOURCE_NAME, 1));
  EXPECT_EQ(expected.physicalsourcetypes(1).sourcetype(),
            magic_enum::enum_name(SourceType::DEFAULT_SOURCE));

  assertErrorMessageInLogOutput(
      "IF_THIS_APPEARS_IN_THE_ERROR_MESSAGE_THE_TEST_WAS_SUCCESSFUL", 1,
      LogLevel::LOG_ERROR);
}

TEST_F(CoordinatorRPCClientTest,
       TestGenericErrorPhysicalSourceRegistrationRequest) {
  using namespace ::testing;
  auto [uut, mock] = defaultUUT();
  ...
  EXPECT_CALL(mock, RegisterPhysicalSource(_, _, _))
      ...
      .WillRepeatedly(
          Return(Status(grpc::UNAVAILABLE, "THIS_SHOULD_APPEAR_IN_THE_LOG")));
  ...
  assertErrorMessageInLogOutput("THIS_SHOULD_APPEAR_IN_THE_LOG", 3,
