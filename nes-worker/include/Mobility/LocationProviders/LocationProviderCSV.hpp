...
/**
 * @brief this class reads locations and timestamps from a csv file and
 * simulates the behaviour of a geolocation interface of a mobile device
 */
...
public:
  ...
  /**
   * @brief get the simulated current location of the device which is the
   * waypoint read from csv which has the most recent of the timestamps lying in
   * the past
   * @return a pair containing a goegraphical location and the time when this
...
private:
  /**
   * @brief reads waypoints representing simulated device locations from a csv
   * in the format
   ...
   * @param index: the iterator which marks the position in the vector of
   * waypoints
   * @return the waypoint
   */
