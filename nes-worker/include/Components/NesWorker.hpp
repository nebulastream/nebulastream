...
class NesWorker : public detail::virtual_enable_shared_from_this<NesWorker>,
 ...
 public:
  ...
  NesWorker(Configurations::WorkerConfigurationPtr workerConfig,
            Monitoring::MetricStorePtr metricStore = nullptr);
   ...
   */
  std::vector<Runtime::QueryStatisticsPtr> getQueryStatistics(
   ...
   * @return bool indicating success
   */
  bool notifyEpochTermination(uint64_t timestamp,
                              uint64_t querySubPlanId) override;

  /**
   * @brief method that enables the worker to send errors to the Coordinator.
   * Calls the notifyError method
   * @param signalNumber
  ...
  /**
   * @brief method that enables the worker to send exceptions to the
   * Coordinator. Calls the notifyError method
  ...
  /**
   * get the class containing all location info on this worker if it is a field
   * node and the functionality to query the current position if it is a field
   * node
   * @return
   */
  ...
  /**
   * get the class taking care of the trajectory prediction of mobile devices.
   * Will return nullptr if the node is not mobile
   * @return
   */
  NES::Spatial::Mobility::Experimental::ReconnectSchedulePredictorPtr
  getTrajectoryPredictor();
 ...
 private:
   ...
   * @param physicalSourceTypes: physical sources containing relevant
   * information
   * @return bool indicating success
   */
  ...
  /**
   * @brief this method will start the GRPC Worker server which is responsible
   * for reacting to calls
   */
  void buildAndStartGRPCServer(const std::shared_ptr<std::promise<int>>& prom);

  /**
   * @brief helper method to ensure client is connected before calling rpc
   * functions
   * @return
   */
  ...
  WorkerId parentId;
  NES::Configurations::Spatial::Mobility::Experimental::
      WorkerMobilityConfigurationPtr mobilityConfig;
