...
/**
 * @brief This class provides utility to interact with NES coordinator over RPC
 * interface.
 */
...
public:
  /**
   * @brief Factory method, to create the coordinator RPCClient. Internally a
   * GRPC stub is created.
   * @param address
   * @param retryAttempts: number of attempts for connecting
   * @param backOffTimeMs: backoff time to wait after a failed connection
   * attempt
   */
  static std::shared_ptr<CoordinatorRPCClient>
  create(const std::string &address, uint32_t rpcRetryAttempts = 10,
  ...
  explicit CoordinatorRPCClient(
      ...
      const std::string &address, uint32_t rpcRetryAttempts = 10,
      std::chrono::milliseconds rpcBackoff = std::chrono::milliseconds(50));
  /**
   * @brief this methods registers physical sources provided by the node at the
   * coordinator
   * @param physicalSourceTypes list of physical sources to register
   * @return bool indicating success
   */
  bool registerPhysicalSources(
   ...
   * @note the logical source is not saved in the worker as it is maintained on
   * the coordinator and all logical source can be retrieved from the physical
   * source map locally, if we later need the data we can add a map
   */
  ...
  /**
   * @brief this method removes a physical source from a logical source in the
   * coordinator
   ...
   * @param registrationRequest: request containing necessary input for worker
   * registration
   * @return bool indicating success
   */
  ...
  bool notifyQueryFailure(SharedQueryId sharedQueryId,
                          DecomposedQueryPlanId subQueryId, WorkerId workerId,
                          OperatorId operatorId, std::string errorMsg);
  ...
  /**
   * Experimental
   * @brief Method to get all field nodes (field nodes = non-mobile nodes with a
   * specified geographical location) within a certain range around a
   * geographical point
   * @param geoLocation: center of the query area
   * @param radius: radius in km to define query area
   * @return list of node IDs and their corresponding fixed coordinates as
   * Location objects
   */
  std::vector<
      std::pair<WorkerId, NES::Spatial::DataTypes::Experimental::GeoLocation>>
  getNodeIdsInRange(
      const NES::Spatial::DataTypes::Experimental::GeoLocation &geoLocation,
      double radius);
   ...
   */
  bool checkAndMarkForSoftStop(SharedQueryId sharedQueryId,
   ...
   * false
   */
  bool notifySoftStopCompleted(SharedQueryId sharedQueryId,
                               DecomposedQueryPlanId decomposedQueryPlanId);

  /**
   * @brief this method is used by a mobile worker to inform the coordinator
   * that location or time of the next expected reconnect have changed or that
   * the worker expects a reconnect to a different parent than the previously
   * scheduled one
   * @param removePredictions old predictions which should be removed
   * @param addPredictions new predictions to be added
   * @return true if the information was successfully received at coordinator
   * side
   */
  bool sendReconnectPrediction(
      const std::vector<NES::Spatial::Mobility::Experimental::ReconnectPoint>
          &addPredictions,
      const std::vector<NES::Spatial::Mobility::Experimental::ReconnectPoint>
          &removePredictions);

  /**
   * @brief this method can be called by a mobile worker to tell the
   * coordinator, that the mobile devices position has changed
   * @param locationUpdate a tuple containing the mobile devices location and a
   * timestamp indicating when the device was located at the transmitted
   * position
   * @return true if the information has benn succesfully processed
   */
  bool sendLocationUpdate(
      const NES::Spatial::DataTypes::Experimental::Waypoint &locationUpdate);

  /**
   * @brief the function queries for the ids of the parents of a node with a
   * given id
