...
TEST_F(DecideFieldMappingsTest, TestNoCollisions)
{
    ...
    const auto projectionOperator = ProjectionLogicalOperator::create(projections, ProjectionLogicalOperator::Asterisk{false})
                                        .withChildrenUnsafe({sourceDescriptorLogicalOperator});
    const auto sinkOperator = SinkLogicalOperator::create(sinkDescriptor).withChildrenUnsafe({projectionOperator});
...
TEST_F(DecideFieldMappingsTest, TestCollisions)
{
    ...
    auto projectionOperator = ProjectionLogicalOperator::create(projections, ProjectionLogicalOperator::Asterisk{false})
                                  .withChildrenUnsafe({sourceDescriptorLogicalOperator});
    const auto sinkOperator = SinkLogicalOperator::create(sinkDescriptor).withChildrenUnsafe({projectionOperator});
    const LogicalPlan logicalPlan{
        QueryId::create(LocalQueryId{generateUUID()}, getNextDistributedQueryId()), {sinkOperator->withInferredSchema()}};
...
TEST_F(DecideFieldMappingsTest, TestProjectionToNewField)
{
    ...
    const auto projectionOperator = ProjectionLogicalOperator::create(projections, ProjectionLogicalOperator::Asterisk{false})
                                        .withChildrenUnsafe({sourceDescriptorLogicalOperator});
    const auto sinkOperator = SinkLogicalOperator::create(sinkDescriptorWithNew).withChildrenUnsafe({projectionOperator});
    const LogicalPlan logicalPlan{
        QueryId::create(LocalQueryId{generateUUID()}, getNextDistributedQueryId()), {sinkOperator->withInferredSchema()}};
