...
TEST_F(CalcTargetOrderTest, JustSource)
{
    ...
    const auto sinkOp = SinkLogicalOperator::create(sinkDescriptor).withChildren({sourceOp});
...
TEST_F(CalcTargetOrderTest, JoinOverProjection)
{
    ...
    const auto sinkOp = SinkLogicalOperator::create(sinkDescriptor)->withChildren({joinOp}).withInferredSchema();
