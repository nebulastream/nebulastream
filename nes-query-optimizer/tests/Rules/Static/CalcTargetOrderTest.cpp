...
TEST_F(CalcTargetOrderTest, JustSource)
{
    ...
    const auto sinkOp = SinkLogicalOperator::create(sinkDescriptor).withChildrenUnsafe({sourceOp});

    LogicalPlan plan{QueryId::create(LocalQueryId{generateUUID()}, getNextDistributedQueryId()), {sinkOp->withInferredSchema()}};
...
TEST_F(CalcTargetOrderTest, JoinOverProjection)
{
    ...
    auto projectionOp = ProjectionLogicalOperator::create(projections, ProjectionLogicalOperator::Asterisk{false})
                            .withChildrenUnsafe({sourceOp2})
                            .withInferredSchema();
    ...
    auto joinOp = JoinLogicalOperator::create(
        ...
        JoinTimeCharacteristic{std::array{
            ...
            Windowing::BoundTimeCharacteristic{Windowing::TimeCharacteristicWrapper::createIngestionTime()}}});
    joinOp = joinOp.withChildrenUnsafe({sourceOp1, projectionOp});
    const auto sinkOp = SinkLogicalOperator::create(sinkDescriptor)->withChildrenUnsafe({joinOp}).withInferredSchema();
