...
TEST_F(ProjectionPushdownRuleTest, UnhandledOperatorIsNarrowedToRequiredFields)
{
    ...
    /// PLAN AFTER:  Sink(prediction) < Project(prediction) < Project(prediction) < InferModel < ...
    ...
    auto op1 = op0.getChildren().at(0);
    ASSERT_TRUE(op1.tryGetAs<ProjectionLogicalOperator>());
    /// the projection pushBeyondDefault computes for the unhandled operator must reach the plan
    auto op2 = op1.getChildren().at(0);
    ASSERT_TRUE(op2.tryGetAs<ProjectionLogicalOperator>());
    ASSERT_EQ(op2.getOutputSchema().size(), 1);
    ASSERT_TRUE(op2.getOutputSchema()[Identifier::parse("prediction")].has_value());
    auto op3 = op2.getChildren().at(0);
    ASSERT_TRUE(op3.tryGetAs<InferModelLogicalOperator>());
}
#endif
