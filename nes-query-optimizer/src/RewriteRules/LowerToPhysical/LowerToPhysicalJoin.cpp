...
*/
#include <RewriteRules/LowerToPhysical/LowerToPhysicalJoin.hpp>
...
std::tuple<std::shared_ptr<PhysicalOperatorWrapper>, std::shared_ptr<PhysicalOperatorWrapper>, std::shared_ptr<PhysicalOperatorWrapper>>
createNLJ(const NES::Configurations::QueryOptimizerConfiguration& conf, const LogicalOperator& logicalOperator)
{
    ...
    auto leftMemoryProvider = TupleBufferMemoryProvider::create(conf.operatorBufferSize.getValue(), leftInputSchema);
    leftMemoryProvider->getMemoryLayout()->setKeyFieldNames(getJoinFieldNames(leftInputSchema, logicalJoinFunction));
    auto rightMemoryProvider = TupleBufferMemoryProvider::create(conf.operatorBufferSize.getValue(), rightInputSchema);
    ...
    return {leftBuildWrapper, rightBuildWrapper, probeWrapper};
}
            ...
            if (firstChild->getDataType() != secondChild->getDataType())
            {
                ...
                if (joinedDataType.has_value())
                {
                    ...
                    leftJoinNames.emplace_back(
                        FieldNamesExtension{leftField.getFieldName(), leftFieldNewName, leftField.getDataType(), *joinedDataType});
                    rightJoinNames.emplace_back(
                        FieldNamesExtension{rightField.getFieldName(), rightFieldNewName, rightField.getDataType(), *joinedDataType});
                }
            }
            else
            {
...
addMapOperators(const Schema& inputSchemaOfJoin, const std::vector<FieldNamesExtension>& fieldNameExtensions)
{
    ...
    for (const auto& [oldName, newName, oldDataType, newDataType] : fieldNameExtensions)
    {
        ...
        PhysicalFunction castedPhysicalFunction = QueryCompilation::FunctionProvider::lowerFunction(castToTypeFunction);
    ...
    const NES::Configurations::QueryOptimizerConfiguration& conf)
{
    ...
    for (auto& fieldExtension : joinFieldExtensions)
    {
        ...
        if (fieldExtension.newDataType.isType(DataType::Type::VARSIZED))
        {
            ...
            bool fieldReplaceSuccess = inputSchema.replaceTypeOfField(fieldExtension.newName, fieldExtension.newDataType);
...
std::tuple<std::shared_ptr<PhysicalOperatorWrapper>, std::shared_ptr<PhysicalOperatorWrapper>, std::shared_ptr<PhysicalOperatorWrapper>>
createHashJoin(const NES::Configurations::QueryOptimizerConfiguration& conf, const LogicalOperator& logicalOperator)
{
    auto join = logicalOperator.get<JoinLogicalOperator>();
    auto handlerId = getNextOperatorHandlerId();
    ...
    auto [leftJoinFields, rightJoinFields]
        = getJoinFieldExtensionsLeftRight(join.getInputSchemas()[1], join.getInputSchemas()[0], logicalJoinFunction);
    auto [newLeftInputSchema, leftMapOperators] = addMapOperators(join.getInputSchemas()[1], leftJoinFields);
    auto [newRightInputSchema, rightMapOperators] = addMapOperators(join.getInputSchemas()[0], rightJoinFields);
    ...
    return {leftLeaf, rightLeaf, probeWrapper};
}
}

RewriteRuleResultSubgraph LowerToPhysicalJoin::apply(LogicalOperator logicalOperator)
{
    ...
    switch (conf.joinStrategy)
    {
        case NES::Configurations::StreamJoinStrategy::NESTED_LOOP_JOIN: {
            const auto& [leftLeaf, rightLeaf, probeWrapper] = createNLJ(conf, logicalOperator);
            return {.root = {probeWrapper}, .leafs = {rightLeaf, leftLeaf}};
        }
        case NES::Configurations::StreamJoinStrategy::HASH_JOIN: {
            const auto& [leftLeaf, rightLeaf, probeWrapper] = createHashJoin(conf, logicalOperator);
            return {.root = {probeWrapper}, .leafs = {rightLeaf, leftLeaf}};
        }
    }
    std::unreachable();
...
{
    return std::make_unique<LowerToPhysicalJoin>(argument.conf);
}
