...
RewriteRuleResultSubgraph LowerToPhysicalSink::apply(LogicalOperator logicalOperator)
{
    ...
    const auto inputFieldMappingOpt = sink->getChild()->getTraitSet().tryGet<FieldMappingTrait>();
    ...
    const auto childFieldOrderingOpt = sink->getChild().getTraitSet().tryGet<FieldOrderingTrait>();
    ...
    const auto inputSchema = UnboundOrderedSchema{childFieldOrderingOpt->getOrderedFields()};
    ...
    PRECONDITION(logicalOperator.getChildren().size() == 1, "Expected exactly one child for sink operator");
