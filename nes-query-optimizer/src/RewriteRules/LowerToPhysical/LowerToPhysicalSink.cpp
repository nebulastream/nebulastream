...
RewriteRuleResultSubgraph LowerToPhysicalSink::apply(LogicalOperator logicalOperator)
{
    ...
    auto wrapper = std::make_shared<PhysicalOperatorWrapper>(
        physicalOperator, logicalOperator.getChildren().at(0).getOutputSchema().unbind<std::dynamic_extent>(), memoryLayoutType, memoryLayoutType, PhysicalOperatorWrapper::PipelineLocation::INTERMEDIATE);
    ;
