        ...
        if (const auto fieldAccessExpression = dynamic_cast<FieldAccessLogicalFunction*>(descriptor->as().clone().get()))
        {
            ...
            switch (descriptor->getType())
            {
                ...
                case WindowAggregationLogicalFunction::Type::Median: {
                    auto layout = std::make_unique<Memory::MemoryLayouts::ColumnLayout>(
...
std::vector<PhysicalOperatorWithSchema>
LowerToPhysicalWindowedAggregation::applyToPhysical(DynamicTraitSet<QueryForSubtree, Operator>* traitSet)
{
    ...
    auto build = std::make_unique<AggregationBuildPhysicalOperator>(
        operatorHandlerIndex, std::move(timeFunction), std::move(keyFunctions), std::move(windowAggregationPhysicalOperator));

    auto probe = std::make_unique<AggregationProbePhysicalOperator>(

        std::move(windowAggregationPhysicalOperator), operatorHandlerIndex, ops->getWindowStartFieldName(), ops->getWindowEndFieldName());
