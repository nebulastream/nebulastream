...
std::unique_ptr<TimeFunction> getTimeFunction(const WindowedAggregationLogicalOperator& logicalOperator)
{
    ...
    switch (timeWindow->getTimeCharacteristic()->getType())
    {
        ...
        case Windowing::TimeCharacteristic::Type::EventTime: {
            ...
            return std::make_unique<EventTimeFunction>(std::move(timeStampField), timeWindow->getTimeCharacteristic()->getTimeUnit());
        }
        default: {
...
std::vector<std::unique_ptr<AggregationFunction>> getAggregationFunctions(
    ...
    for (const auto& descriptor : aggregationDescriptors)
    {
        ...
        if (const auto fieldAccessExpression = NES::Util::as_if<FieldAccessLogicalFunction>(descriptor->as()))
        {
            ...
            switch (descriptor->getType())
            {
                ...
                case Windowing::WindowAggregationFunction::Type::Sum: {
...
LowerToPhysicalWindowedAggregation::applyToPhysical(DynamicTraitSet<QueryForSubtree, Operator>* traitSet)
{
    const size_t operatorHandlerIndex = 0; /// TODO this should be a singleton as for the queryid?

    auto* ops = traitSet->get<WindowedAggregationLogicalOperator>();
    ...
    const std::unique_ptr<Nautilus::Interface::HashFunction> hashFunction = std::make_unique<Nautilus::Interface::MurMur3HashFunction>();
    ...
    const auto probe = std::make_shared<AggregationProbePhysicalOperator>(
        (
            [](auto ptr)
            {
                std::vector<std::shared_ptr<TupleBufferMemoryProvider>> vec;
...
Optimizer::RewriteRuleGeneratedRegistrar::RegisterWindowedAggregationRewriteRule(RewriteRuleRegistryArguments argument)
{
