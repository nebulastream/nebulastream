...
addMapOperators(const LogicalOperator& inputOperator, const std::vector<FieldNamesExtension>& fieldNameExtensions)
{
    ...
    {
        if (oldField.getLastName() == newField.getName() and oldField.getDataType() == newField.getDataType())
        {
            continue;
        }
        ...
        auto inputSchema = UnboundSchema{currentFields};
        currentFields.emplace_back(newField);
        const UnboundSchema outputSchema(currentFields);
        ...
        mapPhysicalOperators.emplace_back(
            std::make_shared<PhysicalOperatorWrapper>(
                MapPhysicalOperator(newField.getName(), castedPhysicalFunction), inputSchema, outputSchema));
    }

    return {UnboundSchema{currentFields}, mapPhysicalOperators};
}

HashMapOptions createHashMapOptions(
    std::vector<FieldNamesExtension>& joinFieldExtensions, UnboundSchema& inputSchema, const QueryExecutionConfiguration& conf)
{
    uint64_t keySize = 0;
    ...
    for (auto& fieldExtension : joinFieldExtensions)
    {
        ...
        keyFunctions.emplace_back(FieldAccessPhysicalFunction{fieldExtension.newField.getName()});
        fieldKeyNames.emplace_back(fieldExtension.newField.getName());
    }
...
RewriteRuleResultSubgraph LowerToPhysicalHashJoin::apply(LogicalOperator logicalOperator)
{
    ...
    const auto& leftOperator = children.at(0);
    const auto& rightOperator = children.at(1);
    ...
    auto sizeOfSchema = [](const UnboundSchema& schema)
    {
    ...
    auto leftBufferRef = Interface::BufferRef::TupleBufferRef::create(
        conf.numberOfRecordsPerKey.getValue() * sizeOfSchema(newLeftInputSchema),
        newLeftInputSchema,
        MemoryLayout::MemoryLayoutType::ROW_LAYOUT);
    auto rightBufferRef = Interface::BufferRef::TupleBufferRef::create(
        conf.numberOfRecordsPerKey.getValue() * sizeOfSchema(newRightInputSchema),
        newRightInputSchema,
        MemoryLayout::MemoryLayoutType::ROW_LAYOUT);
