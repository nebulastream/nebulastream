...
LogicalFunction resolveUdfCalls(const LogicalFunction& function, const UdfCatalog& udfCatalog)
{
    ...
    if (const auto udfCall = function.tryGetAs<UDFCallLogicalFunction>())
    {
        ...
        {
            throw UnknownUdf("Unknown function: {}. It is neither a built-in function nor a registered UDF.", udfName);
        }
...
/// NOLINTNEXTLINE(readability-convert-member-functions-to-static)
std::set<std::type_index> UDFResolutionRule::neededBy() const
{
...
PlanRuleRegistryReturnType UDFResolutionRule::create(PlanRuleRegistryArguments arguments)
{
    return UDFResolutionRule{arguments.udfCatalog};
}
