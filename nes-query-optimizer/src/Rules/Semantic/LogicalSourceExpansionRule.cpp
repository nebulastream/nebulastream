...
namespace
{
LogicalOperator applyRecursive(const LogicalOperator& visiting, const std::shared_ptr<Catalog>& catalog)
{
    auto children = visiting.getChildren() | std::views::transform([&catalog](const auto& child) { return applyRecursive(child, catalog); })
...
LogicalPlan LogicalSourceExpansionRule::apply(const LogicalPlan& queryPlan) const
{
    ...
    return queryPlan.withRootOperators({applyRecursive(queryPlan.getRootOperators().at(0), catalog)});
}
}
