    ...
    if (const auto sourceNameOperator = visiting.tryGetAs<SourceNameLogicalOperator>())
    {
        ...
        auto expandedSourceOperators
            ...
            | std::views::transform(
                  ...
                  { return SourceDescriptorLogicalOperator::create(entry).withChildren(children | std::ranges::to<std::vector>()); })
            | std::ranges::to<std::vector>();

        return UnionLogicalOperator::create().withChildren(std::move(expandedSourceOperators));
    }
