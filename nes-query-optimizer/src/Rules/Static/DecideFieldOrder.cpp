...
LogicalOperator applyRecur(const LogicalOperator& visiting)
{
    ...
    return visiting.withTraitSet(std::move(traitSet)).withChildrenUnsafe(newChildren);
}
...
LogicalPlan DecideFieldOrder::apply(const LogicalPlan& queryPlan) const
{
    ...
    const auto newRootChild = [&]
    {
        if (std::ranges::size(oldRootChild->getChildren()) > 0)
        {
            ...
            return oldRootChild.withChildrenUnsafe(std::move(newGrandchildren));
        }
        return oldRootChild;
    }();
    ...
    auto newRoot = rootSink.withChildrenUnsafe({rootChildWithTargetOrder}).withTraitSet(rootTraitSet);
