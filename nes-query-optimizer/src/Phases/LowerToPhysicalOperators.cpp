...
lowerOperatorRecursively(const LogicalOperator& logicalOperator, const RewriteRuleRegistryArguments& registryArgument)
{
    ...
    {
        auto implementationName = getTrait<ImplementationTrait>(logicalOperator.getTraitSet()).value().implementation();
        if (auto ruleOptional = RewriteRuleRegistry::instance().create(implementationName, registryArgument))
        {
