...
Bridge connect(const DecompositionContext& context, const NetworkChannel& channel)
{
    ...
    const auto networkSourceDescriptor = context.catalog->createInlineSource(
    ...
    auto networkSinkDescriptor
        = context.catalog->createInlineSink(ConnectorKind::Internal, Identifier::parse("Network"), orderedUpstreamSchema, sinkConfig, {});
