...
SerializableTrait OutputOriginIdsTrait::serialize() const
{
    ...
    OriginIdList* serializedOriginIds = serializedVariant.mutable_origin_ids();
    for (const auto& originId : originIds)
    {
        serializedOriginIds->add_origin_ids(originId.getRawValue());
    }
...
TraitRegistryReturnType TraitGeneratedRegistrar::RegisterOutputOriginIdsTrait(TraitRegistryArguments arguments)
{
    ...
    if (!std::holds_alternative<OriginIdList>(configIter->second))
    {
        throw CannotDeserialize("OutputOriginIdsTrait is not of type OriginIdList");
    }

    const auto& serializedOriginIdList = std::get<OriginIdList>(configIter->second);
    const auto originIdList = serializedOriginIdList.origin_ids()
