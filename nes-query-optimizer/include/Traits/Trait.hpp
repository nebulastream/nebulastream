...
{
    bool operator==(const TraitConcept& other) const override { return typeid(other) == typeid(Derived); }

    [[nodiscard]] size_t hash() const override { return std::type_index(typeid(Derived)).hash_code(); }

    [[nodiscard]] const std::type_info& getType() const override { return typeid(Derived); }
...
{
    std::size_t operator()(const NES::Trait& trait) const noexcept { return trait.hash(); }
};
