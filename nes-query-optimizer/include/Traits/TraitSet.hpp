...
public:
    TraitSet() = default;
    TraitSet(const TraitSet& other) = default;
    TraitSet(TraitSet&& other) noexcept = default;
    TraitSet& operator=(const TraitSet& other) = default;
    TraitSet& operator=(TraitSet&& other) noexcept = default;
    ~TraitSet() = default;
    ...
    explicit TraitSet(TraitType... traits)
    {
        traitMap = std::unordered_map<std::type_index, Trait>{
            ((std::make_pair<std::type_index, Trait>(typeid(TraitType), std::move(traits))), ...)};
    }
    ...
    template <IsTrait TraitType>
    std::optional<TraitType> tryGet() const
    {
    ...
    auto begin() const { return traitMap.cbegin(); }

    auto end() const { return traitMap.cend(); }
...
template <IsTrait TraitType>
bool insertTrait(TraitSet& traitset, TraitType trait)
{
