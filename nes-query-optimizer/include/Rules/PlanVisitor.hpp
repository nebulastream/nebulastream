...
private:
    struct Instance
    {
        Instance(FunctionDown fnDown, FunctionUp fnUp) : fnDown(fnDown), fnUp(fnUp) { };
        ...
        void traverseDown(const LogicalPlan& plan)
        {
            ...
            while (!toVisit.empty())
            {
                ...
                auto contexts = downContextMap[op];
                auto [opCon, childCon] = fnDown(op, contexts);

                operatorContextMap.insert_or_assign(op, opCon);
                for (auto [child, context] : childCon)
                {
                    downContextMap[child].emplace_back(context);
                }
        ...
        void traverseUp()
        {
            ...
            while (!toVisit.empty())
            {
                ...
                for (const auto& child : op.getChildren())
                {
                    ...
                    upContext.emplace(newChild, upContextMap.at(newChild));
                }

                auto [newOp, newUpContext] = fnUp(op, newChildren, context, upContext);

                newOperatorMap.insert_or_assign(op, newOp);
                upContextMap.insert_or_assign(newOp, newUpContext);
