...
CMAKE_BUILD_DIR=${CMAKE_BUILD_DIR:-"cmake-build-debug"}

WORKERS_FILE=$(realpath $1)
cd "$(git rev-parse --show-toplevel)"

if [ ! -f "$(pwd)/${CMAKE_BUILD_DIR}/nes-single-node-worker/nes-single-node-worker" ]; then
...
services:
  worker-image:
    ...
    pull_policy: never
    build:
      context: $(pwd)/${CMAKE_BUILD_DIR}/nes-single-node-worker
      dockerfile_inline: |
...
for i in $(seq 0 $((WORKER_COUNT - 1))); do
  ...
  $HOST_NAME:
    ...
    pull_policy: never
    healthcheck:
      test: [ "CMD-SHELL", "/bin/grpc_health_probe", '-addr', 'localhost:$GRPC_PORT' ]
    command: [
